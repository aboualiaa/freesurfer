...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (params.scanner == "siemens" &&
        (params.gefile != "" || params.magfile == "" ||
         params.phasefile == "")) {
      throw std::logic_error("Siemens scanner should only have siemens mag and "
                             "siemens phase options. See help");
    }
    if (params.scanner == "ge" &&
        (params.gefile == "" || params.magfile != "" ||
         params.phasefile != "")) {
      throw std::logic_error("GE scanner should only have a ge-file option in "
  ...
  if (params.scanner == "siemens") {
    ...
    if (mrimag == nullptr) {
      std::cerr << "ERROR: The Siemens magnitude DICOM can't be read. Check "
                   "the file\n";
      exit(1);
    }
    ...
    if (_sphase == nullptr) {
      std::cerr
          << "ERROR: The Siemens phase DICOM can't be read. Check the file\n";
      exit(1);
    }
    ...
    for (int i = 0; i < _gedcm->width; i++)
      for (int j = 0; j < _gedcm->height; j++)
        for (int k = 0; k < _gedcm->depth; k++) {
          ...
          MRIsetVoxVal(mrimag, i, j, k, 0,
                       (float)MRIgetVoxVal(_gedcm, i, j, k, 0));
          // MRIsetVoxVal(mrimag, i, j, k, 0, sqrt((_i/1000.0)*(_i/1000.0) +
          // (_r/1000.0)*(_r/1000.0)));
    ...
    for (int i = 0; i < _phildcm->width; i++)
      for (int j = 0; j < _phildcm->height; j++)
        for (int k = 0; k < _phildcm->depth; k++) {
          MRIsetVoxVal(mrimag, i, j, k, 0,
                       (float)MRIgetVoxVal(_phildcm, i, j, k, 0));
          MRIsetVoxVal(_pphase, i, j, k, 0,
                       (float)MRIgetVoxVal(_phildcm, i, j, k, 1));
        }
    ...
    std::cerr << "ERROR: You didn't specify the scanner make ( one of ge, "
                 "siemens and philips )\n";
    exit(1);
  }
...
void IoParams::parse(int argc, char *argv[]) {
  ...
  cmd.AddOptionString("scanner", &scanner,
                      "Name of the scanner ( one of ge, siemens or philips )");
  cmd.AddOptionString(
      "ge_file", &gefile,
      "Name of the input GE file (only compatible with --scanner ge option)");
  cmd.AddOptionString("philips_file", &philipsfile,
                      "Name of the input Philips file (only compatible with "
                      "--scanner philips option)");
  cmd.AddOptionString("siemens_mag", &magfile,
                      "Name of the input Siemens magnitude file (only "
                      "compatible with --scanner siemens option)");
  cmd.AddOptionString("siemens_phase", &phasefile,
                      "Name of the input Siemens phase file (only compatible "
                      "with --scanner siemens option)");
  cmd.AddOptionString("out_magnitude", &outmag,
                      "Name of the output magnitude file (after "
                      "preprocessing). Make sure it has .nii suffix");
  cmd.AddOptionString("out_phase", &outphase,
                      "Name of the output phase file (after preprocessing). "
                      "Make sure it has .nii suffix");
  if (argc == 1) {
    std::cout << "\n"
                 " Pre-process the Susceptibility-weighted images. Write out "
                 "nifti files so that they can be fed into PRELUDE ( Phase "
                 "Unwarpping Library of FSL).\n";
    cmd.PrintHelp();
    exit(0);
  }
