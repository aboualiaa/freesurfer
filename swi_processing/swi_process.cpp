...
void do_phase_mask(IoParams &params, MRI *mag, MRI *pha, MRI *out) {
  ...
  if (params.phasemethod.compare("negative") == 0) {
    std::cout << "Performing phase mask multiplication with cutoff "
              << params.phasecutoff << " ..\n";
  ...
  if (params.phasemethod.compare("positive") == 0) {
    std::cout << "Performing phase mask multiplication with right cutoff "
              << params.phasercutoff << " ..\n";
    std::cout << "Number of phase multiplications " << params.phasemult
              << " ..\n";
    for (int i = 0; i < out->width; i++)
  ...
  if (params.phasemethod.compare("symmetric") == 0) {
    ...
    if (fabs(fabs(params.phasecutoff) - fabs(params.phasercutoff)) > 0.0001) {
      std::cerr << "ERROR: symmetric option implies both left cutoff and right "
                   "cutoff are almost same\n";
      exit(1);
    }
    for (int i = 0; i < out->width; i++)
      for (int j = 0; j < out->height; j++)
...
int main(int argc, char *argv[]) {
  ...
  MRI *mrimag = nullptr, *mriphase = nullptr;
  ...
  if ((params.phasemethod.compare("negative") != 0) &&
    ...
    std::cerr << "ERROR: The phase mask method has to be one of negative, "
                 "positive, symmetric, asymmetric or sigmoid\n";
  }
...
void IoParams::parse(int argc, char *argv[]) {
  ...
  cmd.AddOptionString("mag_file", &magfile,
                      "The magnitude image ( Output from the PRELUDE program)");
  cmd.AddOptionString("phase_file", &phasefile,
                      "The phase image ( Output from the PRELUDE program)");
  cmd.AddOptionString("swi_output", &swioutput,
                      "Name of the SWI processed output image");
  cmd.AddOptionFloat("stddev", &stddev,
                     (char *)"Specify the standard deviation of the Gaussian "
                             "Smoothing Filter. Default is 2.0");
  cmd.AddOptionString(
      "phase_mask_method", &phasemethod,
      "Specify the phase mask method. One of negative, positive, symmetric, "
      "asymmetric, sigmoid. Default is negative");
  cmd.AddOptionInt("phase_multiplications", &phasemult,
                   "Specify the number of phase multiplications. Default is 4");
  cmd.AddOptionFloat(
      "phase_mask_cutoff", &phasecutoff,
      (char *)"Specify the negative phase mask cutoff frequency ( in radianѕ). "
              "Default is the minimum value of the phase image.");
  cmd.AddOptionFloat(
      "phase_mask_right_cutoff", &phasercutoff,
      (char *)"Specify the positive phase mask cutoff frequency ( in radianѕ). "
              "Default is the maximum value of the phase image.");
  cmd.AddOptionFloat("sigmoid_a", &sigmoid_a,
                     (char *)"Specify 'a' for the sigmoid formula "
                             "f(phase)=1/(1+exp(-a*(phase-b))). Default is "
                             "1.0. Meaningless with phase_method != sigmoid");
  cmd.AddOptionFloat("sigmoid_b", &sigmoid_b,
                     (char *)"Specify 'b' for the sigmoid formula "
                             "f(phase)=1/(1+exp(-a*(phase-b))). Default is "
                             "0.0. Meaningless with phase_method != sigmoid");
  cmd.AddOptionInt("mip_level", &miplevel,
                   "Specify the number of levels of mIP across the y "
                   "direction. Default is 4");
  if (argc == 1) {
    std::cout
        << "\n"
           " Process the Susceptibility-weighted images. Make sure the inputs "
           "to this program is after the phase unwrapping step using PRELUDE\n";
    cmd.PrintHelp();
    exit(0);
  }
