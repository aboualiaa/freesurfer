...
int main(int argc, char *argv[]) {
  ...
  if (MRISreadValues(mris, in_fname) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: could not read val file %s", Progname,
              in_fname);
...
static void print_usage() {
  fprintf(stderr,
          "usage: %s [options] <input surface> <input w/curv file> <output "
          "w/curv file>\n",
          Progname);
  fprintf(stderr, "where options are:\n\t-T <threshold> (default is 0)\n"
                  "\t-A <area thresh> ignore segments smaller than <area "
                  "thresh> mm(default 0)\n");
}
