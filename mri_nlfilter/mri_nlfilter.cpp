...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_nlfilter.c,v 1.17 2016/12/08 17:27:53 fischl Exp $",
      "$Name:  $");
	  ...
	  region.dz -= 2 * zborder;
          ...
          if (region.dx <= 0 || region.dy <= 0 || region.dz <= 0) {
            fprintf(stderr, "invalid region: (%d,%d,%d) --> (%d,%d,%d)\n",
                    region.x, region.y, region.z, region.dx, region.dy,
                    region.dz);
          } else
  ...
  if (!stricmp(option, "-help"))
    ...
    case 'F':
      ...
      if (filter_window_size < 3)
        ErrorExit(ERROR_BADPARM, "%s: offset window size must be > 3",
                  Progname);
      nargs = 1;
      break;
    case 'N':
    ...
    case 'B':
      if (sscanf(argv[2], "%f", &blur_sigma) < 1)
        ErrorExit(ERROR_BADPARM, "%s: could not scan blurring sigma from  '%s'",
                  Progname, argv[2]);
      if (blur_sigma < 0.0f)
        ErrorExit(ERROR_BADPARM, "%s: blurring sigma must be positive",
                  Progname);
      nargs = 1;
      break;
    case 'R':
