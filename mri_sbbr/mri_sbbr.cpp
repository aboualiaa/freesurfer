...
int main(int argc, char *argv[]) {
  ...
  SBBRmatrixPar2OptSchema(sbbr, nullptr);
  // Set any parameters input from command line
  for (n = 0; n < sbbr->nparams; n++)
    if (cmdargs->paramset[n])
      sbbr->params[n] = cmdargs->params[n];
  printf("Init Params: ");
  for (n = 0; n < sbbr->nparams; n++)
    printf(" %6.4f", sbbr->params[n]);
  printf("\n");

  // Read inputs
  sbbr->surf = MRISread(cmdargs->surffile);
  if (sbbr->surf == nullptr)
    exit(1);
  ...
  if (cmdargs->outreginv || cmdargs->outsurffile)
    ltainv = LTAinvert(lta, nullptr);
  ...
  if (cmdargs->outsurffile) {
    ...
    if (MatrixDeterminant(lta->xforms[0].m_L) < 0)
      MRISreverseFaceOrder(sbbr->surf);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--t1"))
      cmdargs->bbrsign = +1;
    else if (!strcasecmp(option, "--t2"))
      cmdargs->bbrsign = -1;
...
static void print_help() {
  ...
  printf("This program is a special implementation of boundary-based "
         "registration\n");
...
/* -------------------------------------------------- */
static void check_options() {
  if (cmdargs->movfile == nullptr) {
    printf("ERROR: must spec --mov\n");
    exit(1);
  }
  if (cmdargs->surffile == nullptr) {
...
#pragma omp parallel for if_ROMP(experimental) reduction(+ : nhits, sumcost)
    ...
    nhits++;
    fcost = BBRcost(uval, lval, sbbr->bbrslope, sbbr->bbrcenter, sbbr->bbrsign,
                    &pct);
    sumcost += fcost;

    // printf(   "%7d   %7.4lf %7.4lf %7.4f    %7.4lf %7.4lf %7.4f   %7.4f\n",
    //	      faceno,uc,ur,uval,lc,lr,lval,fcost);
    //    fprintf(fp,"%7d   %7.4lf %7.4lf %7.4f    %7.4lf %7.4lf %7.4f   %7.4f
...
double BBRcost(double vctx, double vwm, double slope, double center,
  ...
  s = (vctx + vwm);
  if (fabs(s) > .000001)
    d = 100 * (vctx - vwm) / (s / 2.0); // percent contrast
  else
    d = 0;
  if (sign == 0)
...
float SBBRcostPowell(float *pPowel) {
  ...
  if (false) {
    ...
    for (n = 0; n < sbbr->nparams; n++)
      fprintf(fp, "%12.8f ", sbbr->params[n]);
...
double *SBBRoptSchema2MatrixPar(SBBR *sbbr, double *mpar) {
  ...
  if (mpar == nullptr)
    mpar = (double *)calloc(12, sizeof(double));
...
int SBBRmatrices(SBBR *sbbr, int init) {
  ...
  if (sbbr->R0 == nullptr)
    sbbr->R0 = LTAcreate(sbbr->mov, sbbr->ref, nullptr, LINEAR_RAS_TO_RAS);
  ...
  if (!init) {
    ...
    sbbr->Q = MatrixIdentity(4, nullptr);
  sbbr->invQ = MatrixInverse(sbbr->Q, sbbr->invQ);
...
int SBBRbruteSearch(SBBR *sbbr) {
  ...
  for (p[0] = sbbr->pmin[0]; p[0] <= sbbr->pmax[0]; p[0] += sbbr->pdelta[0]) {
    for (p[1] = sbbr->pmin[1]; p[1] <= sbbr->pmax[1]; p[1] += sbbr->pdelta[1]) {
      for (p[2] = sbbr->pmin[2]; p[2] <= sbbr->pmax[2];
           p[2] += sbbr->pdelta[2]) {
        for (p[3] = sbbr->pmin[3]; p[3] <= sbbr->pmax[3];
             p[3] += sbbr->pdelta[3]) {
          for (p[4] = sbbr->pmin[4]; p[4] <= sbbr->pmax[4];
               p[4] += sbbr->pdelta[4]) {
            for (p[5] = sbbr->pmin[5]; p[5] <= sbbr->pmax[5];
                 p[5] += sbbr->pdelta[5]) {
              ...
              if (sbbr->nparams == 6) {
                ...
                if (newmin || iter == 0 || iter % 1000 == 0) {
                  ...
                  for (nthp = 0; nthp < sbbr->nparams; nthp++)
                    printf("%7.4f ", popt[nthp]);
                  // Time (min) remaining
                  printf("%7.2f \n", (((totiter - iter) * timer.milliseconds() /
                                       (iter + 1)) /
                                      1000.0) /
                                         60.0);
                  fflush(stdout);
              ...
              for (p[6] = sbbr->pmin[6]; p[6] <= sbbr->pmax[6];
                   p[6] += sbbr->pdelta[6]) {
                for (p[7] = sbbr->pmin[7]; p[7] <= sbbr->pmax[7];
                     p[7] += sbbr->pdelta[7]) {
                  ...
                  for (p[8] = sbbr->pmin[8]; p[8] <= sbbr->pmax[8];
                       p[8] += sbbr->pdelta[8]) {
                    for (p[9] = sbbr->pmin[9]; p[9] <= sbbr->pmax[9];
                         p[9] += sbbr->pdelta[9]) {
