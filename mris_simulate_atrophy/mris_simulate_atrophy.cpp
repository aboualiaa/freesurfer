...
int main(int argc, char *argv[]) {
  ...
  if (atrophy_frac > 1) {
    if (atrophy_frac > 100)
      ErrorExit(ERROR_BADPARM, "%s: must specify atrophy in [0 1]", Progname);
  ...
  {
    noise_min = noise_max = noise_sigma;
    noise_step = 1;
  }
  if (atrophy_min < 0) // only one step
  {
    atrophy_min = atrophy_max = atrophy_frac;
    atrophy_step = 1;
  }
  ...
  mri_pial->c_r = mri_aseg->c_r;
  mri_pial->c_a = mri_aseg->c_a;
  mri_pial->c_s = mri_aseg->c_s;
  ...
  add_aseg_structures_outside_ribbon(mri_seg, mri_aseg, mri_seg, WM_VAL,
                                     SUBCORT_GM_VAL, CSF_VAL);
  ...
  for (atrophy_frac = atrophy_min; atrophy_frac <= atrophy_max;
  ...
  minutes = seconds / 60;
  seconds = seconds % 60;
  fprintf(stderr, "simulation took %d minutes and %d seconds.\n", minutes,
          seconds);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "DEBUG_VOXEL")) {
    ...
    if (sscanf(argv[2], "%f:%f:%f", &noise_min, &noise_step, &noise_max) != 3)
      ErrorExit(ERROR_BADPARM, "%s: couldn't parse min:step:max from '%s'",
                Progname, argv[2]);
    printf(
        "stepping through noise values from %2.1f to %2.1f in steps of %2.2f\n",
        noise_min, noise_max, noise_step);
    nargs = 1;
  } else if (!stricmp(option, "arange") || !stricmp(option, "atrophy_range")) {
    if (sscanf(argv[2], "%f:%f:%f", &atrophy_min, &atrophy_step,
               &atrophy_max) != 3)
      ErrorExit(ERROR_BADPARM, "%s: couldn't parse min:step:max from '%s'",
                Progname, argv[2]);
    printf("stepping through atrophy values from %2.1f to %2.1f in steps of "
           "%2.2f\n",
    ...
    case 'W':
      ...
      if (whalf < 1)
        ErrorExit(ERROR_BADPARM, "half window size must be >= 1 to estimate "
                                 "unpartial-volumed intensities");
      nargs = 1;
      break;
    case 'S':
      sigma = atof(argv[2]);
...
static void usage_exit(int code) {
  printf("usage: %s [options] <subject> <hemi> <label> <atrophy fraction> "
         "<output volume>\n",
         Progname);
  printf("\noptions:\n");
  ...
  for (x = 0; x < mri_dst->width; x++) {
    ...
    for (y = 0; y < mri_dst->height; y++) {
      ...
      for (z = 0; z < mri_dst->depth; z++) {
        ...
        ya = (int)(nint(V3_Y(v2)));
        za = (int)(nint(V3_Z(v2)));
        if (xa < 0 || ya < 0 || za < 0 || xa >= mri_aseg->width ||
            ya >= mri_aseg->height || za >= mri_aseg->depth)
          continue;
        if (xa == Gx && ya == Gy && za == Gz)
          DiagBreak();
        aseg_label = nint(MRIgetVoxVal(mri_aseg, xa, ya, za, 0));
        if (seg_label != 0 &&
            !IS_MTL(aseg_label)) // already labeled and not amyg/hippo, skip it
          continue;
        switch (aseg_label) {
...
static MRI *compute_unpartial_volumed_intensities(
  ...
  double w, distsq, dx, dy, dz, norm, total_gm, total_csf, total_wm, vwm, vgm,
      vcsf;
        ...
        for (norm = 0.0, num = 0, xk = -whalfx; xk <= whalfx; xk++) {
          ...
          for (yk = -whalfy; yk <= whalfy; yk++) {
            ...
            for (zk = -whalfz; zk <= whalfz; zk++) {
              ...
              vgm = MRIgetVoxVal(mri_vfrac_cortex, xi, yi, zi, 0) +
                    MRIgetVoxVal(mri_vfrac_subcort, xi, yi, zi, 0);
              vcsf = MRIgetVoxVal(mri_vfrac_csf, xi, yi, zi, 0);
              ;
              *MATRIX_RELT(m_A3, num + 1, 1) = w * vwm;
              *MATRIX_RELT(m_A3, num + 1, 2) = w * vgm;
              *MATRIX_RELT(m_A3, num + 1, 3) = w * vcsf;
              total_gm += w * vgm;
              total_csf += w * vcsf;
              total_wm += w * vwm;
              num++;
...
static MRI *MRISsimulateAtrophy(MRI *mri_norm, MRI *mri_unpv_intensities,
  ...
  float gm_frac, csf_frac, wm_frac, wm_intensity, gm_intensity, csf_intensity,
  ...
  for (n = 0; n < lvox->n_points; n++) {
    ...
    for (xk = -1; xk <= 1; xk++)
      for (yk = -1; yk <= 1; yk++)
        for (zk = -1; zk <= 1; zk++) {
          xi = mri_norm->xi[xv + xk];
          yi = mri_norm->yi[yv + yk];
          zi = mri_norm->zi[zv + zk];
          if (MRIgetVoxVal(mri_filled, xi, yi, zi,
                           0)) // already added atrophy here
            continue;
          ...
          if (max_csf > 0 &&
              gm_frac >
                  0) // can only simulate in voxels where there is some of each
          {
  ...
  for (x = 0; x < mri_norm->width; x++)
	...
	if (FEQUAL(csf_frac,1))   // retain contralateral hemi
	  continue ;
        ...
        out_val = wm_frac * wm_intensity + gm_frac * gm_intensity +
                  csf_frac * csf_intensity;
