...
static int         nvols               = 0;
static const char *vol_names[MAX_VOLS] = {"norm.mgz"};
...
static RANDOM_FOREST *train_rforest(const char *training_file_name,
                                    RF_PARMS *parms, int wsize, int nlong,
                                    char **long_names, int nvols,
                                    const char **vol_names, const char *sdir,
                                    const char *seg_name, double *paccuracy);

static int classify_subjects(RANDOM_FOREST *rf, const char *training_file_name,
...
int main(int argc, char *argv[]) {
  ...
  case CLASSIFIER_RFOREST:
    rf = train_rforest(training_file_name, &rf_parms, wsize, nlong, long_names,
                       nvols, vol_names, sdir, seg_name, &accuracy);
    {
      ...
      sprintf(line, "%f %d %d %f\n", rf->training_fraction, rf->max_depth,
              rf->ntrees, accuracy);
      if (write(fd, line, (strlen(line)) * sizeof(char)) <
          (strlen(line)) * sizeof(char))
        ErrorPrintf(ERROR_NOFILE, "%s: could not write %d bytes to %s",
                    Progname, (strlen(line)) * sizeof(char));
      fl.l_type = F_UNLCK;     /* tell it to unlock the region */
      fcntl(fd, F_SETLK, &fl); /* set the region to unlocked */
      close(fd);
      if (classify_name)
        classify_subjects(rf, training_file_name, wsize, nlong, long_names,
                          nvols, vol_names, sdir, seg_name, classify_name);
    }
    break;
  default:
    if (nclusters > 0) {
  ...
  for (x = 0; x < mri_seg->width; x++)
    for (y = 0; y < mri_seg->height; y++)
      for (z = 0; z < mri_seg->depth; z++) {
        ...
        if (IS_HYPO(label) ==
            0) // only consider voxels that are WSMA at the end
          continue;
        for (nhypo = l = 0; l < nlong - 1;
             l++) // make sure they weren't wmsa before the end
        {
          label = MRIgetVoxVal(mri_long_seg[l], x, y, z, 0);
          if (IS_HYPO(label))
            nhypo++;
          else if (!IS_WHITE_MATTER(
                       label)) // must be either hypo or wm at every time point
            break;
        }
...
static MRI *label_wmsa_and_future_wmsa(MRI *mri_mask_src, MRI *mri_mask_dst,
                                       MRI **mri_long_seg, int nlong,
                                       int not_wmsa_label,
                                       int future_wmsa_label, int wmsa_label) {
  int x, y, z, l, label;
...
static MRI *make_mask_from_segmentation(const char *sdir, const char *subject,
  ...
  label_wmsa_and_future_wmsa(mri_mask, mri_mask, mri_long_seg, nlong, NOT_WMSA,
                             FUTURE_WMSA, WMSA);
  for (l = 0; l < nlong; l++)
    MRIfree(&mri_long_seg[l]);
  MRIfree(&mri_min_dist);
  MRIfree(&mri_max_dist);
  return (mri_mask);
}
...
static RANDOM_FOREST *train_rforest(const char *training_file_name,
  ...
  for (ntraining = n = 0; n < nsubjects; n++) {
    ...
    mri_training_mask = make_mask_from_segmentation(
        sdir, subject, long_names, nlong, MIN_WMSA_DIST, MAX_WMSA_DIST);
  ...
  if (parms->training_fraction < 1) {
    int correct;
    correct = RFcomputeOutOfBagCorrect(rf, training_classes, training_data,
                                       ntraining);
...
static int classify_subjects(RANDOM_FOREST *rf, const char *subject_list_file,
                             int wsize, int nlong, char **long_names, int nvols,
                             const char **vol_names, const char *sdir,
                             const char *seg_name, const char *classify_name) {
  FILE *fp;
  ...
  do {
    ...
    sprintf(fname, "%s/%s%s.%s_base/mri/%s", sdir, subject, long_names[0],
            subject, classify_name);
