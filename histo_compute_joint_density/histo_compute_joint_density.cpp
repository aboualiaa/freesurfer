/**
 * @file  histo_compute_joint_density.c
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: histo_compute_joint_density.c,v 1.4 "
                                "2011/03/02 00:04:09 nicks Exp $",
                                "$Name:  $");
  ...
  for (level = 1; level < MAX_LEVELS; level++) {
    mri1_pyramid[level] = MRIreduce2D(mri1_pyramid[level - 1], nullptr);
    mri2_pyramid[level] = MRIreduce2D(mri1_pyramid[level - 1], nullptr);
  ...
  for (level = nlevels - 1; level >= 0; level--) {
    pdf = DensityHistogramEstimate(mri1_pyramid[level], mri2_pyramid[level],
                                   nbins, sigma, valid1, valid2);
