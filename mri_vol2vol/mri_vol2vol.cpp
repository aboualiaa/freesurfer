...
mri_vol2vol
  ...
  --downsample N1 N2 N3 : downsample input volume (do not include a targ or
regsitration) sets --fill-average, --fill-upsample 2, and --regheader
  ...
  --lta-inv  register.lta : LTA, invert (may not be the same as --lta --inv with
--fstal)
  ...
  --noDefM3zPath : flag indicating that the code should not be looking for
       the non-linear m3z morph in the default location (subj/mri/transforms),
but should use the morph name as is
  --inv-morph    : compute and use the inverse of the m3z morph

  --fstarg <vol>      : optionally use vol from subject in --reg as target.
default is orig.mgz
  ...
  --fill-average      : compute mean of all source voxels in a given target
voxel
  --fill-conserve     : compute sum  of all source voxels in a given target
voxel
  ...
  --spm-warp mov movlta warp interp output
     ...
     movlta maps mov to the vbm input space (use 0 to ignore)
       if movlta=0, then input is anything that shares a RAS space with the VBM
input warp is typically y_rinput.nii interp 0=nearest, 1=trilin
...
perform soap bubble smoothing on the input volume, using the volume specified by
soap_ctl_point_fname as the fixed (control) points for num_iters iterations
...
MATRIX *R = nullptr, *R2 = nullptr, *invR, *XFM;
...
int main(int argc, char **argv) {
  ...
  make_cmd_version_string(
      argc, argv, "$Id: mri_vol2vol.c,v 1.89 2016/06/22 22:33:40 fischl Exp $",
      "$Name:  $", cmdline);
  ...
  if (DoCrop) {
    ...
    if (err)
      exit(1);
    crop = MRIcrop(mov, box.x, box.y, box.z, box.x + box.dx, box.y + box.dy,
                   box.z + box.dz);
    // MRIwrite(crop,"crop.mgh");

    cropnew =
        MRIalloc(nint(crop->width * CropScale), nint(crop->height * CropScale),
                 nint(crop->depth * CropScale), mov->type);
    ...
    printf("vol2vol\n");
    err = MRIvol2Vol(crop, cropnew, nullptr, SAMPLE_NEAREST, 0);
    if (err)
      exit(1);
    printf("Saving\n");
  ...
  if (fstal) {
    ...
    if (fstalres > 0)
      ...
      Rtal = MatrixIdentity(4, nullptr);
    invRtal = MatrixInverse(Rtal, nullptr);
    if (true || Gdiag_no > 0) {
  ...
  if (mri_soap_ctrl) {
    ...
    mri_out = MRIsoapBubble(mov, mri_soap_ctrl, nullptr, soap_bubble_iters, 0);
  ...
  if (fslregfile) {
    ...
    if (Rfsl2)
      R2 = MRIfsl2TkReg(targ, mov, Rfsl2);
  }
  ...
  if (Mrot) {
    ...
    printf("  Angles (deg): %lf %lf %lf\n", angles[0] * 180 / M_PI,
           angles[1] * 180 / M_PI, angles[2] * 180 / M_PI);
  ...
  if (RegFileFinal) {
    ...
    regio_write_register(RegFileFinal, subject, in->xsize, in->zsize, 1, R,
                         FLT2INT_ROUND);
  }

  if (invert) {
    printf("Inverting registration\n");
    R = MatrixInverse(R, nullptr);
  }
  ...
  if (noresample) {
    ...
    if (slice_crop_flag) {
      printf("Cropping slices from %d to %d\n", slice_crop_start,
             slice_crop_stop);
      crop = MRIcrop(in, 0, 0, slice_crop_start, in->width - 1, in->height - 1,
                     slice_crop_stop);
      if (crop == nullptr)
        exit(1);
      MRIfree(&in);
    ...
    if (SliceReverse) {
      ...
      mri = MRIreverseSlices(in, nullptr);
      if (mri == nullptr)
        exit(1);
      MRIfree(&in);
      in = mri;
  ...
  if (!DoMorph) {
    if (DoKernel) {
      ...
      printf("Running MRIvol2VolFill(), DoConserve=%d, US=%d\n", DoFillConserve,
             FillUpsample);
      if (!invert)
        ltareg = TransformRegDat2LTA(targ, mov, R);
      else
        ltareg = TransformRegDat2LTA(mov, targ, R);
      out = MRIvol2VolFill(in, nullptr, ltareg, FillUpsample, DoFillConserve,
                           out);
    } else {
    ...
    printf("Reading gcam\n");
    if (defM3zPath)
      sprintf(gcamfile, "%s/%s/mri/transforms/%s", SUBJECTS_DIR, subject,
              m3zfile);
    else
    ...
    if (!InvertMorph) {
      ...
      out = GCAMmorphToAtlas(in, gcam, nullptr, -1, interpcode);

      // sprintf(MNIgcamfile,"%s/transforms/talairach.m3z",
      // fio_dirname(gcam->atlas.fname)); printf("The MNI gcam fname is: %s\n",
      // MNIgcamfile); MNIgcam = GCAMread(MNIgcamfile); out =
      // GCAMmorphToAtlasToMNI(in, gcam, MNIgcam, NULL, -1, interpcode);
    } else {
      // mri_vol2vol --mov orig.morphed.mgz --inv-morph --s subject --o
      // origB.mgz
      gcam = GCAMread(gcamfile);
      if (gcam == nullptr)
        exit(1);
      {
        MRI *mri_tmp;
        mri_tmp = MRIalloc(gcam->image.width, gcam->image.height,
  ...
  if (SegRegCostFile) {
    ...
    for (n = 0; n < 3; n++)
      fprintf(fp, "%7.3lf ", xyztrans[n]);
    for (n = 0; n < 3; n++)
      fprintf(fp, "%5.1lf ", angles[n] * 180 / M_PI);
    fprintf(fp, "%7d %10.4lf %8.4lf ", (int)costs[0], costs[1],
            costs[2]); // WM  n mean std
    fprintf(fp, "%7d %10.4lf %8.4lf ", (int)costs[3], costs[4],
            costs[5]);                                 // CTX n mean std
  ...
  if (slice_crop_flag) {
    printf("Cropping slices from %d to %d\n", slice_crop_start,
           slice_crop_stop);
    crop = MRIcrop(out, 0, 0, slice_crop_start, out->width - 1, out->height - 1,
                   slice_crop_stop);
    if (crop == nullptr)
      exit(1);
    MRIfree(&out);
  ...
  if (fstal) {
    ...
    if (subject != nullptr)
      subject_outreg = subject;
    else
      subject_outreg = "subject-unknown";
  ...
  if (SaveReg) {
    ...
    regio_write_register(regfile0, subject_outreg, out->xsize, out->zsize, 1, R,
                         FLT2INT_ROUND);
    if (!fstal) {
      if (!usedltageom) {
        printf("To check registration, run:\n");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      invert = 1;
    else if (!strcasecmp(option, "--no-resample"))
      noresample = 1;
    else if (!strcasecmp(option, "--regheader"))
      regheader = 1;
    else if (!strcasecmp(option, "--kernel"))
      DoKernel = 1;
    else if (!strcasecmp(option, "--nomr"))
      DoSaveInputMR = 1;
    else if (!strcasecmp(option, "--mr"))
      DoSaveInputMR = 0;
    else if (!strcasecmp(option, "--delta"))
      DoDelta = 1;
    else if (!strcasecmp(option, "--no-save-reg"))
      SaveReg = 0;
    else if (!strcasecmp(option, "--save-reg"))
      SaveReg = 1;
    else if (!strcasecmp(option, "--cost-only"))
      CostOnly = 1;
      ...
      if (R == nullptr)
        R = MatrixIdentity(4, nullptr); // as subjid is not neccesary any more
      ...
      nargsused = 1;
               ...
               istringnmatch(option, "--lta-inv", 0)) {
      if (nargc < 1)
        argnerr(option, 1);
      regfile = pargv[0];
      ...
      TargMNI152 = 1;
      sprintf(tmpstr, "%s/data/standard/MNI152_T1_2mm.nii.gz",
              getenv("FSLDIR"));
      ...
      nargsused = 2;
    } else if (istringnmatch(option, "--slice-reverse", 0))
      SliceReverse = 1;
      ...
      out = MRIapplySpmWarp(mov, srclta, warp, 0, interpcode, nullptr);
      if (out == nullptr)
        exit(1);
      printf("Writing to %s\n", pargv[4]);
      ...
      targvolfile = pargv[6];
      out = MRIvol2volGCAM(mov, srclta, gcam, dstlta, vsm, interpcode, nullptr);
      if (out == nullptr)
        exit(1);
      printf("Writing to %s\n", targvolfile);
...
static void print_usage() {
  ...
  printf("  --downsample N1 N2 N3 : downsample input volume (do not include a "
         "targ or regsitration)\n");
  printf("         sets --fill-average, --fill-upsample 2, and --regheader\n");
  printf("\n");
  printf(
      "  --reg  register.dat : tkRAS-to-tkRAS matrix   (tkregister2 format)\n");
  printf("  --lta  register.lta : Linear Transform Array (usually only 1 "
         "transform)\n");
  printf("  --lta-inv  register.lta : LTA, invert (may not be the same as "
         "--lta --inv with --fstal)\n");
  ...
  printf("  --s subject         : set matrix = identity and use subject for "
         "any templates\n");
  printf("\n");
  ...
  printf("  --talxfm xfmfile    : default is talairach.xfm (looks in "
         "mri/transforms)\n");
  printf("\n");
  printf("  --m3z morph    : non-linear morph encoded in the m3z format\n");
  printf("  --noDefM3zPath : flag indicating that the code should not be "
         "looking for \n");
  printf("       the non-linear m3z morph in the default location "
         "(subj/mri/transforms), but should use \n");
  printf("       the morph name as is\n");
  printf("  --inv-morph    : compute and use the inverse of the m3z morph\n");
  printf("\n");
  printf("  --fstarg <vol>      : optionally use vol from subject in --reg as "
  ...
  printf("  --interp interptype : interpolation cubic, trilin, nearest (def is "
         "trilin)\n");
  printf("  --fill-average      : compute mean of all source voxels in a given "
         "target voxel\n");
  printf("  --fill-conserve     : compute sum  of all source voxels in a given "
         "target voxel\n");
  printf("  --fill-upsample USF : source upsampling factor for --fill-xxx "
         "(default is 2)\n");
  printf("\n");
  ...
  printf("  --kernel            : save the trilinear interpolation kernel "
         "instead\n");
  printf("\n");
  printf("  --gcam mov srclta gcam dstlta vsm interp out\n");
  ...
  printf("       if movlta=0, then input is anything that shares a RAS space "
         "with the VBM input\n");
  ...
  printf("  --reg-final regfinal.dat : final reg after rot and trans (but not "
         "inv)\n");
  printf("\n");
...
static void print_help() {
  ...
  printf("--mov movvol\n");
  printf("\n");
  printf("This volume must have the same geometry as the --mov volume passed "
         "to\n");
  printf("tkregister2 when creating/checking the registration file. By "
         "default,\n");
  printf("this will be the input volume that will be resampled. If --inv is\n");
  printf("specified, then this will become the geometry template for the "
         "output\n");
  printf("instead.\n");
  printf("\n");
  printf("--targ targvol\n");
  printf("\n");
  printf("This volume must have the same geometry as the --targ volume passed "
         "to\n");
  printf("tkregister2 when creating/checking the registration file. By "
         "default,\n");
  printf("this will be the volume will be the geometry template for the "
         "output.\n");
  printf("If --inv is specified, then this becomes the input volume that will "
         "be\n");
  printf("resampled instead. The target volume can be implicitly specified "
         "with\n");
  printf("--tal or --fstarg.\n");
  printf("\n");
  printf("--reg register.dat\n");
  printf("\n");
  printf("This simple text file contains the freesurfer registration matrix. "
         "It\n");
  printf(
      "is the same as the file passed to and generated by tkregister2 with\n");
  printf(
  ...
  printf(
      "Registration matrix created with the FSL flirt program using targ as\n");
  printf("the reference and mov as input. Note: you cannot use any of the "
         "files\n");
  printf(
      "from $FSLDIR/etc/standard as mov or targ. These volumes do not have\n");
  printf(
  ...
  printf(
      "Create a registration matrix assuuming that the mov Scanner-RAS and\n");
  printf("targ Scanner-RAS are the same. This is the same as using a "
         "register.xfm\n");
  ...
  printf(
      "Target MNI152 space. If the mov volume is in the native space of an\n");
  printf("individual, then also supply a registration (--reg). This "
         "registration\n");
  printf("is concatenated with that in "
         "subject/mri/transforms/reg.mni152.2mm.dat\n");
  printf(
      "(created with mni152reg) to produce a registration from the mov vol\n");
  printf(
      "to MNI152 (defined by $FSLDIR/data/standard/MNI152_T1_2mm.nii.gz).\n");
  ...
  printf("--inv\n");
  printf("\n");
  printf("Invert the transform. The movvol becomes the geometry template for "
         "the\n");
  printf("output, and the targvol becomes the input that will be resampled.\n");
  printf("\n");
  printf("--o outvol\n");
  printf("\n");
  printf("Output volume.  By default, this will be the movvol resmapled into "
         "the\n");
  printf("targvol space (and so will have the same geometry as the targvol). "
         "If\n");
  printf(
      "--inv is specified, then this will be the targvol resmapled into the\n");
  printf(
  ...
  printf(
      "implicitly set to $FREESURFER_HOME/average/mni305.cor.subfovV.mgz,\n");
  printf("where V is either 1 (for 1mm) or 2 (for 2mm). 2mm is used by "
         "default,\n");
  printf(
      "but this can be changed with --talres.  mni305.cor.subfovV.mgz the\n");
  printf("MNI305 (1mm or 2mm isotropic) volume in a reduced FOV that covers "
         "only\n");
  printf("the brain. Reducing the FOV saves space relative to the 256^3 COR "
         "FOV.\n");
  printf("The transformation matrix is computed as R*inv(Xtal)*inv(Rtal), "
         "where\n");
  printf("Xtal is talairach.xfm matrix, R is the matrix in the regfile, and "
         "Rtal\n");
  printf("maps from the talairach COR FOV to the SubFOV "
         "(mni305.cor.subfovV.reg).\n");
  printf("If you want to sample the targvol from talairach space into the "
         "movvol\n");
  printf(
      "space, then specify --inv. SUBJECTS_DIR is read from the environment\n");
  ...
  printf("--crop scale\n");
  printf("\n");
  printf("Crop mov volume down to minimum size to fit non-zero voxels. The "
         "size of\n");
  printf("the voxels is reduced by scale (ie, --crop 2 would crop and reduce "
         "the\n");
  printf("voxel size by a factor of 2, eg 1.0 mm becomes 0.5 mm).\n");
  printf("\n");
  printf("--slice-crop start end\n");
  printf("\n");
  printf("Crop output volume to be within slices start and end. The geometry "
         "is \n");
  ...
  printf("--nomr\n");
  printf("\n");
  printf("Don't copy the template MR parameters, but instead preserve the "
         "input volume \n");
  printf("ones\n");
  printf("\n");
  printf("--help\n");
  ...
  printf("EXAMPLES:\n");
  printf("\n");
  printf("Below are some exampls of how one might use mri_vol2vol. They are "
         "not\n");
  printf("exhaustive of all the possible combinations of options. Typically, "
         "one\n");
  printf(
      "uses a template to establish the registration, then resamples data\n");
  printf("that are in correspondence with the template.\n");
  printf("\n");
  printf("1. If a functional volume is f.bhdr (or f.nii.gz, or f.mgh, etc), "
         "and the\n");
  ...
  printf("\n");
  printf("  # This will show the low-res functional alighned with its "
         "resampled self\n");
  printf(
      "  tkregister2 --reg register.dat --mov f.nii.gz --targ f-in-anat.mgh\n");
  printf("\n");
  printf("  # This will show the resampled functional aligned with the "
         "anatomical\n");
  printf("  tkregister2 --mov f-in-anat.mgh --targ "
         "$SUBJECTS_DIR/bert/mri/orig.mgz \\\n");
  ...
  printf("  tkregister2 --mov f.new.vox2ras.nii.gz --s bert --regheader --reg "
         "/tmp/reg\n");
  printf("\n");
  printf(
      "  # Display the functional as an overlay in tkmedit (no registration\n");
  ...
  printf("\n");
  printf("1.D Map a binary mask in functional space to anatomical space. This "
         "is\n");
  ...
  printf(
      "$FREESURFER_HOME/average/mni305.cor.subfov2.mgz. This command will\n");
  printf("also create f-in-tal.2mm.mgh.reg, which will register the volume "
         "with\n");
  printf("any average MNI305 FreeSurfer subject (fsaverage is used by "
         "default).\n");
  ...
  printf("\n");
  printf("f-in-tal.1mm.mgh will take up 8 times as much space as "
         "f-in-tal.2mm.mgh\n");
  printf("\n");
  printf("3. Apply an MNI transform to data by resampling the anatomical "
         "orig.mgz\n");
  printf("into talairach space using bert/mri/transforms/talairach.xfm:\n");
  printf("\n");
  printf("First, check that the talairach.xfm is correct (this is basically "
         "the same\n");
  ...
  printf("\n");
  printf("Report bugs to analysis-bugs@nmr.mgh.harvard.edu. Include the "
         "following\n");
  printf("formatted as a list as follows: (1) command-line, (2) directory "
         "where\n");
  printf(
      "the program was run (for those in the MGH-NMR Center), (3) version,\n");
...
static void check_options() {
  ...
  if (movvolfile == nullptr && (lta == nullptr || !invert)) {
    printf("ERROR: No mov volume supplied.\n");
    exit(1);
  }
  if (fstarg && targvolfile != nullptr) {
    printf("ERROR: Do not specify a targ volume with --fstarg.\n");
    exit(1);
  }
  if (fstal && targvolfile != nullptr) {
  ...
  if (targvolfile != nullptr && DownSample[0]) {
    printf("ERROR: cannot spec target file and downsample.\n");
    exit(1);
  }

  if (targvolfile == nullptr && DownSample[0] == 0) {
    printf("ERROR: No target volume supplied.\n");
    exit(1);
  }

  if (outvolfile == nullptr && DispFile == nullptr) {
  ...
  if (TargMNI152) {
    if (regfile == nullptr) {
      ...
      MATRIX *Q, *invQ, *M;
      sprintf(tmpstr, "%s/%s/mri/transforms/reg.mni152.2mm.dat", SUBJECTS_DIR,
              subject);
  ...
  if (!fstal && !DoCrop && !fstarg && targvolfile == nullptr &&
      (lta == nullptr || invert) && !DownSample[0]) {
    printf("ERROR: No targ volume supplied.\n");
    exit(1);
  }
  if (DoCrop && targvolfile != nullptr) {
    printf("ERROR: Do not specify a targ volume with --crop.\n");
    exit(1);
  }
  if (xfmfile != nullptr && regfile != nullptr) {
    printf("ERROR: cannot specify both --xfm and --reg.\n");
    exit(1);
  }
  if (regheader && regfile != nullptr) {
    printf("ERROR: cannot specify both --regheader and --reg.\n");
    exit(1);
  }
  if (fstarg && regfile == nullptr && subject == nullptr) {
...
static void dump_options(FILE *fp) {
  ...
  if (interpcode == SAMPLE_SINC)
    fprintf(fp, "sinc hw  %d\n", sinchw);
...
static MATRIX *LoadRtal(int talres) {
  ...
  err = regio_read_register(rtalfile, &subject, &ipr, &bpr, &intensity, &Rtal,
                            &float2int);
  if (err)
    exit(1);
  return (Rtal);
...
/*
  \fn MRI *MRIvol2volGCAM(MRI *src, LTA *srclta, GCA_MORPH *gcam, LTA *dstlta,
  MRI *vsm, int sample_type, MRI *dst) \brief Converts one volume into another
  using as many as four transforms: VSM, src linear, gcam/m3z, dst linear. Any
  may be NULL except dstlta in which case they are assumed to be the identity.
  This function allows for transforming from the functional space to a sub FoV
  of CVS space including B0 distortion correction.
 */
MRI *MRIvol2volGCAM(MRI *src, LTA *srclta, GCA_MORPH *gcam, LTA *dstlta,
                    MRI *vsm, int sample_type, MRI *dst) {
  ...
  if (!gcam)
    printf("MRIvol2volGCAM(): GCAM not used\n");
  if (!srclta)
    printf("MRIvol2volGCAM(): Source LTA not used\n");
  printf("MRIvol2volGCAM(): interpolation type is %d %s\n", sample_type,
         MRIinterpString(sample_type));
  ...
  if (srclta) {
    ...
    if (LTAmriIsSource(srclta, src)) {
      ...
      Vsrc = MatrixInverse(srclta->xforms[0].m_L, nullptr);
    } else
      Vsrc = MatrixCopy(srclta->xforms[0].m_L, nullptr);
  } else
    Vsrc = MatrixIdentity(4, nullptr);
  ...
  if (vgdst_src->width != 256 || vgdst_src->height != 256 ||
    ...
    if (vgdst_dst->width != 256 || vgdst_dst->height != 256 ||
      ...
      printf("ERROR: MRIvol2volGCAM(): neither src nor dst VG of Dest LTA is "
             "conformed\n");
      return (nullptr);
    } else {
    ...
    printf("MRIvol2volGCAM(): NOT inverting Destination LTA\n");
  vgdst_src = &(dstlta->xforms[0].src);
  vgdst_dst = &(dstlta->xforms[0].dst);
  ...
  Vdst = MatrixInverse(dstlta->xforms[0].m_L, nullptr);
    ...
    for (r = 0; r < dst->height; r++) {
      for (s = 0; s < dst->depth; s++) {
        ...
        if (gcam) {
          ...
          out_of_gcam =
              GCAMsampleMorph(gcam, crsGCAM->rptr[1][1], crsGCAM->rptr[2][1],
                              crsGCAM->rptr[3][1], &crsAnat->rptr[1][1],
                              &crsAnat->rptr[2][1], &crsAnat->rptr[3][1]);
          if (out_of_gcam)
            continue;
        } else
          crsAnat = MatrixCopy(crsGCAM, crsAnat);
        ...
        if (vsm) {
          ...
          if (fabs(v) < FLT_MIN)
            continue;
          /* Performs 3D interpolation. May want to use iss instead of
             crsSrc->rptr[3][1] to make it a 2D interpolation. Not sure.*/
          MRIsampleSeqVolume(vsm, crsSrc->rptr[1][1], crsSrc->rptr[2][1],
                             crsSrc->rptr[3][1], &drvsm, 0, 0);
          if (drvsm == 0)
            continue;
        ...
        if (crsSrc->rptr[1][1] < 0 || crsSrc->rptr[1][1] >= src->width)
          continue;
        if (crsSrc->rptr[2][1] < 0 || crsSrc->rptr[2][1] >= src->height)
          continue;
        if (crsSrc->rptr[3][1] < 0 || crsSrc->rptr[3][1] >= src->depth)
          continue;

        if (sample_type != SAMPLE_TRILINEAR)
          for (f = 0; f < src->nframes; f++) {
            if (sample_type == SAMPLE_CUBIC_BSPLINE)
              MRIsampleBSpline(bspline, crsSrc->rptr[1][1], crsSrc->rptr[2][1],
                               crsSrc->rptr[3][1], f, &val);
            else
              MRIsampleVolumeFrameType(src, crsSrc->rptr[1][1],
                                       crsSrc->rptr[2][1], crsSrc->rptr[3][1],
                                       f, sample_type, &val);
            MRIsetVoxVal(dst, c, r, s, f, val);
          }
        else {
          // This will do the same as above, it is just faster with multiple
          // frames
          MRIsampleSeqVolume(src, crsSrc->rptr[1][1], crsSrc->rptr[2][1],
                             crsSrc->rptr[3][1], valvect, 0, src->nframes - 1);
          for (f = 0; f < src->nframes; f++)
            MRIsetVoxVal(dst, c, r, s, f, valvect[f]);
        }
