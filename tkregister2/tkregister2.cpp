...
void  transform(float x1, float y1, float z1, float *x2, float *y2, float *z2,
                float M[4][4]);
...
void  read_float_images(float ***fim, char *format, int nslices, int nperslice,
...
int   FuncXYZ2FuncInd(float x, float y, float z, float *col, float *row,
                      float *slice);
...
int Register(ClientData clientData, Tcl_Interp *interp, int argc,
  ...
  if (fstal) {
    ...
    int req = snprintf(talxfmdir, 2000, "%s/%s/mri/transforms", subjectsdir,
                       subjectid);
    if (req >= 2000) {
    ...
    if (!mkheaderreg) {
      ...
      if (regio_read_mincxfm(talxfmfile, &RegMat, &xfmfileinfo))
        exit(1);
      printf("%s ---------------------\n", talxfmname);
      MatrixPrint(stdout, RegMat);
      for (i = 0; i < 4; i++) {
  ...
  if (fstarg) {
    ...
    // First try .mgz
    int req = snprintf(targ_vol_path, 1000, "%s/%s/mri/%s.mgz", subjectsdir,
                       subjectid, targ_vol_id);
    if (req >= 1000) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    }
    if (!fio_FileExistsReadable(targ_vol_path)) {
      // Now try COR
      req = snprintf(targ_vol_path, 1000, "%s/%s/mri/%s", subjectsdir,
                     subjectid, targ_vol_id);
      if (req >= 1000) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
      ...
      for (n = 0; n < surf->nvertices; n++) {
        ...
        if (c < 0 || c > 255 || r < 0 || r > 255 || s < 0 || s > 255) {
          printf("ERROR: vertex %d (%g,%g,%g) is out of range (%d,%d,%d)\n", n,
                 surf->vertices[n].x, surf->vertices[n].y, surf->vertices[n].z,
                 c, r, s);
          exit(1);
        }
  ...
  jc  = xdim / 2;       // Sag
  if (slice_init > 0) {
    if (plane_init == CORONAL)
      imc = zf * slice_init;
    if (plane_init == SAGITTAL)
      jc = (int)(xdim * (slice_init / 256.0));
    if (plane_init == HORIZONTAL)
      ic = (int)(ydim * (slice_init / 256.0));
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      fstarg = 1;
    else if (!strcasecmp(option, "--nofstarg"))
      fstarg = 0;
    else if (!strcasecmp(option, "--nofix"))
      fixtkreg = 0;
    else if (!strcasecmp(option, "--fixonly"))
      fixonly = 1;
    else if (!strcasecmp(option, "--inorm"))
      use_inorm = 1;
    else if (!strcasecmp(option, "--no-inorm"))
      use_inorm = 0;
      ...
      identityreg = 1;
    else if (!strcasecmp(option, "--noedit"))
      noedit = 1;
    else if (!strcasecmp(option, "--zero-cras"))
      ZeroCRAS = 1;
    else if (!strcasecmp(option, "--no-zero-cras"))
      ZeroCRAS = 0;
    else if (!strcasecmp(option, "--fmov-targ"))
      DoFMovTarg = 1;
      ...
      fscale_2  = 1;
      ZeroCRAS  = 1;
    } else if (!strcasecmp(option, "--fixxfm"))
      fixxfm = 1;
    else if (!strcasecmp(option, "--nofixxfm"))
      fixxfm = 0;
    else if (!strcasecmp(option, "--tag"))
      tagmov = 1;
    else if (!strcasecmp(option, "--notag"))
      tagmov = 0;
      ...
      targ_vol_id = IDnameFromStem(tmpstr); // For FSL 4.0
      if (!fio_FileExistsReadable(targ_vol_id)) {
        sprintf(tmpstr, "%s/data/standard/avg152T1_LR-marked",
                getenv("FSLDIR"));
        printf("Trying %s\n", tmpstr);
        targ_vol_id = IDnameFromStem(tmpstr); // For FSL 4.0
        if (targ_vol_id == nullptr)
    ...
    } else if (!strcasecmp(option, "--conf-targ"))
      conformTarget = 1;
      ...
      DoASeg = 1;
    else if (!strcmp(option, "--aparc+aseg"))
      DoAParcASeg = 1;
    else if (!strcmp(option, "--wmparc"))
      DoWMParc = 1;
      ...
      if (nargc < 1)
        argnerr(option, 1);
      plane_init = -1000;
      if (strcmp(pargv[0], "cor") == 0)
        plane_init = CORONAL;
      if (strcmp(pargv[0], "sag") == 0)
        plane_init = SAGITTAL;
      if (strcmp(pargv[0], "ax") == 0)
        plane_init = HORIZONTAL;
      if (strcmp(pargv[0], "hor") == 0)
        plane_init = HORIZONTAL;
      if (plane_init == -1000) {
      ...
      if (strcmp(pargv[0], "mov") == 0)
        overlay_mode_init = MOVEABLE;
      if (strcmp(pargv[0], "targ") == 0)
        overlay_mode_init = TARGET;
...
static void print_usage() {
  ...
  printf("   --regheader-center : same as --regheader but aligns volume "
         "centers\n");
  printf("   --fsl-targ : use FSLDIR/data/standard/avg152T1.nii.gz\n");
  printf("   --fsl-targ-lr : use "
         "FSLDIR/data/standard/avg152T1_LR-marked.nii.gz\n");
  ...
  printf("   --talxfmname talxfmname : set mov to be tal and reg to be "
         "talxfmname  \n");
...
static void print_help() {
  ...
  printf(
      ...
      "  $FREESURFER_HOME/average/mni305.cor.mgz and sets the registration "
      "file to be\n"
      ...
      "  of the anatomical is automatically detected as mgz or COR. By "
      "default,\n"
      ...
      "  \n"
      "  Use the matrix produced by the FSL routines as the initial "
      "registration.\n"
      "  It should be an ascii file with a 4x4 matrix. Note: the matrix "
      "should\n"
      ...
      "  indices (c,r,s) to mov indices. Lines that begin with '#' are "
      "ignored.\n"
      "  \n"
      "  --s identity\n"
      "  \n"
...
void draw_image2(int imc, int ic, int jc) {
  ...
  if (firstpass)
    ...
    if (PrevInterp != interpmethod)
      update_needed = 4;
    if (PrevMaskFlag != maskflag)
      update_needed = 5;
    if (PrevOverlayMode != overlay_mode)
      update_needed = 6;
  }
  if (PixelSelected)
    update_needed = 0;
  ...
  if (update_needed != 0) {
    ...
    for (r = 0; r < ydim; r++) {
      for (c = 0; c < xdim; c++) {
        ...
        fcTarg =
            Qtarg->rptr[1][1] * c + Qtarg->rptr[1][2] * r + Qtarg->rptr[1][4];
        frTarg =
            Qtarg->rptr[2][1] * c + Qtarg->rptr[2][2] * r + Qtarg->rptr[2][4];
        fsTarg =
            Qtarg->rptr[3][1] * c + Qtarg->rptr[3][2] * r + Qtarg->rptr[3][4];
        ...
        if (UseSurf)
          surfimg[r][c] = MRIvox(mrisurf, icTarg, irTarg, isTarg);

        /* Keep track of the max and min */
        if (targimgmax < targimg[r][c])
          targimgmax = targimg[r][c];
        if (targimgmin > targimg[r][c])
          targimgmin = targimg[r][c];
        ...
        irMov = nint(frMov);
        isMov = nint(fsMov);
        if (icMov < 0 || icMov >= mov_vol->width || irMov < 0 ||
            irMov >= mov_vol->height || isMov < 0 || isMov >= mov_vol->depth) {
          if (maskflag)
            targimg[r][c] = 0;
          continue;
        }
        ...
        if (movimgmax < movimg[r][c])
          movimgmax = movimg[r][c];
        if (movimgmin > movimg[r][c])
          movimgmin = movimg[r][c];
...
void select_pixel(short sx, short sy) {
  ...
  printf("  Screen:  %3d %3d (%d,%d,%d), inorm = %d, mov_frame = %d \n",
         cScreen, rScreen, lvidbuf[3 * kScreen], lvidbuf[3 * kScreen + 1],
         lvidbuf[3 * kScreen + 2], use_inorm, mov_frame);
  printf("  Anat:    (%3d %3d %3d)   (%6.1f %6.1f %6.1f)  ", sag, hor, cor,
         xAnat, yAnat, zAnat);
  if (!soob)
  ...
  printf("  Func:    (%3d %3d %3d)   (%6.1f %6.1f %6.1f)  ", cFunc, rFunc,
         sFunc, xFunc, yFunc, zFunc);
  if (!foob)
...
int do_one_gl_event(Tcl_Interp *interp) /* tcl */
{
  ...
  if (XPending(xDisplay)) { /* do one if queue test */
    ...
    case ConfigureNotify:
      ...
      XTranslateCoordinates(xDisplay, w.wMain, wat.root, -wat.border_width,
                            -wat.border_width, &rx, &ry, &junkwin);
    ...
    case KeyPress:
      ...
      case ']':
        ...
        if (ks == ']')
          rotate_brain(+r, c); /* in tenths of deg */
        if (ks == '[')
          rotate_brain(-r, c); /* in tenths of deg */
        updateflag = TRUE;
        break;
...
void open_window(char *name) {
  ...
  if (!initpositiondoneflag)
    tkoInitPosition(MOTIF_XFUDGE + wx0, (1024 - wy0 - ydim) + MOTIF_XFUDGE,
                    xdim, ydim);
...
void read_reg(char *fname) {
  ...
  err = regio_read_register(fname, &tmpstr, &ipr, &bpr, &fscale, &RegMat,
                            &float2int);
  if (err) {
...
void write_reg(char *fname) {
  ...
  if (xfmoutfname != nullptr)
    ...
    if (!fio_IsDirectory(touchfile))
      mkdir(touchfile, 0777);
    sprintf(touchfile, "%s/%s/touch/talairach.tkregister2.touch", subjectsdir,
            pname);
...
void read_images(char *fpref) {
  ...
  */
  ...
  for (k = 0; k < numimg; k++)
    for (i = 0; i < ynum; i++)
      for (j = 0; j < xnum; j++) {
        if (im[k][i][j] / 2 > sim[3][i][j])
          sim[3][i][j] = im[k][i][j] / 2;
        if (im[k][i][j] / 2 > sim[4][k][j])
          sim[4][k][j] = im[k][i][j] / 2;
        if (im[k][i][j] / 2 > sim[5][i][k])
          sim[5][i][k] = im[k][i][j] / 2;
      }
  for (i = 0; i < ynum; i++)
    for (j = 0; j < xnum; j++)
      for (k = 0; k < 3; k++)
...
void read_float_images(float ***fim, char *format, int nslices, int nperslice,
                       int xdim, int ydim, short **buf) /* was MRIio.c */
{
#ifdef Linux
  int scount;
#endif
...
#endif
    if (imtype == APD1 || imtype == BSHORT || imtype == SKIP)
      fclose(fp);
    max = -1.0e10;
    min = 1.0e10;
    sum = sum2 = 0;
...
int W_move_window WBEGIN ERR(3, "Wrong # args: move_window <x> <y>")
    move_window(atoi(argv[1]), atoi(argv[2]));
WEND

    int W_pop_gl_window WBEGIN ERR(1, "Wrong # args: pop_gl_window")
        pop_gl_window();
WEND

    int W_redraw WBEGIN ERR(1, "Wrong # args: redraw") redraw();
WEND

    int W_upslice WBEGIN ERR(1, "Wrong # args: upslice") upslice();
WEND

    int W_downslice WBEGIN ERR(1, "Wrong # args: downslice") downslice();
WEND

    int W_rotate_brain_x WBEGIN ERR(2, "Wrong # args: rotate_brain_x <deg>")
        rotate_brain(atof(argv[1]), 'x');
WEND

    int W_rotate_brain_y WBEGIN ERR(2, "Wrong # args: rotate_brain_y <deg>")
        rotate_brain(atof(argv[1]), 'y');
WEND

    int W_rotate_brain_z WBEGIN ERR(2, "Wrong # args: rotate_brain_z <deg>")
        rotate_brain(atof(argv[1]), 'z');
WEND

    int W_translate_brain_x WBEGIN ERR(2,
                                       "Wrong # args: translate_brain_x <mm>")
        translate_brain(atof(argv[1]), 'x');
WEND

    int W_translate_brain_y WBEGIN ERR(2,
                                       "Wrong # args: translate_brain_y <mm>")
        translate_brain(atof(argv[1]), 'y');
WEND

    int W_translate_brain_z WBEGIN ERR(2,
                                       "Wrong # args: translate_brain_z <mm>")
        translate_brain(atof(argv[1]), 'z');
WEND

    int W_scale_brain_x WBEGIN ERR(2, "Wrong # args: scale_brain_x <mm>")
        scale_brain(atof(argv[1]), 'x');
WEND

    int W_scale_brain_y WBEGIN ERR(2, "Wrong # args: scale_brain_y <mm>")
        scale_brain(atof(argv[1]), 'y');
WEND

    int W_scale_brain_z WBEGIN ERR(2, "Wrong # args: scale_brain_z <mm>")
        scale_brain(atof(argv[1]), 'z');
WEND

    int W_goto_point WBEGIN ERR(1, "Wrong # args: goto_point")
        goto_point(tfname);
WEND

    int W_write_point WBEGIN ERR(1, "Wrong # args: write_point")
        write_point(tfname);
WEND

    int W_save_rgb WBEGIN ERR(1, "Wrong # args: save_rgb") save_rgb(sgfname);
WEND

    int W_record_swapbuffers WBEGIN ERR(1, "Wrong # args: record_swapbuffers")
        record_swapbuffers();
WEND

    int W_set_scale WBEGIN ERR(1, "Wrong # args: set_scale") set_scale();
WEND

    int W_blur WBEGIN ERR(2, "Wrong # args: blur <factor>") blur(atof(argv[1]));
WEND

    int W_read_reg WBEGIN ERR(1, "Wrong # args: read_reg") read_reg(regfname);
WEND

    int W_write_reg WBEGIN ERR(1, "Wrong # args: write_reg")
        write_reg(regfname);
WEND

    int W_align_points WBEGIN ERR(1, "Wrong # args: align_points")
        align_points();
WEND

    int W_mirror_brain WBEGIN ERR(1, "Wrong # args: mirror_brain")
        mirror_brain();
WEND
    /*===============================================================*/
...
int main(int argc, char **argv) /* new main */
{
  ...
  Tcl_CreateCommand(interp, "pop_gl_window", (Tcl_CmdProc *)W_pop_gl_window,
                    REND);
  ...
  Tcl_CreateCommand(interp, "rotate_brain_x", (Tcl_CmdProc *)W_rotate_brain_x,
                    REND);
  Tcl_CreateCommand(interp, "rotate_brain_y", (Tcl_CmdProc *)W_rotate_brain_y,
                    REND);
  Tcl_CreateCommand(interp, "rotate_brain_z", (Tcl_CmdProc *)W_rotate_brain_z,
                    REND);
  ...
  Tcl_CreateCommand(interp, "scale_brain_x", (Tcl_CmdProc *)W_scale_brain_x,
                    REND);
  Tcl_CreateCommand(interp, "scale_brain_y", (Tcl_CmdProc *)W_scale_brain_y,
                    REND);
  Tcl_CreateCommand(interp, "scale_brain_z", (Tcl_CmdProc *)W_scale_brain_z,
                    REND);
  ...
  Tcl_CreateCommand(interp, "align_points", (Tcl_CmdProc *)W_align_points,
                    REND);
  Tcl_CreateCommand(interp, "mirror_brain", (Tcl_CmdProc *)W_mirror_brain,
                    REND);
  /*=======================================================================*/
...
/*=== from TkMain.c ===================================================*/
static void StdinProc(clientData, mask) ClientData clientData;
int         mask;
{
...
/*=== from TkMain.c ===================================================*/
static void Prompt(interp, partial) Tcl_Interp *interp;
int         partial;
{
  char *promptCmd;
  int   code;
...
static int MRIisConformant(MRI *vol) {
  ...
  if (fabs(vol->xsize - 1) > .001)
    return (0);
  if (fabs(vol->ysize - 1) > .001)
    return (0);
  if (fabs(vol->zsize - 1) > .001)
    return (0);

  // Column Direction Cosine should be -1 0 0
  if (fabs(vol->x_r + 1) > .001)
    return (0);
  if (fabs(vol->x_a) > .001)
    return (0);
  if (fabs(vol->x_s) > .001)
  ...
  if (fabs(vol->y_a) > .001)
    return (0);
  if (fabs(vol->y_s + 1) > .001)
    return (0);

  // Slice Direction Cosine should be 0 1 0
  if (fabs(vol->z_r) > .001)
    return (0);
  if (fabs(vol->z_a - 1) > .001)
    return (0);
  if (fabs(vol->z_s) > .001)
    return (0);
