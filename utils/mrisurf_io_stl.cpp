...
// STLs are simple files that only record a collection of triangles, so when
// reading, we  have to build a list of unique vertices on our own with a simple
// hash table.
...
// STLs can be either binary or ascii, so the STLMesher class can be used as a
// universal means of registering vertices and faces.
class STLMesher {
  struct Key {
    struct Key *next;
    float x, y, z;
  ...
  int nextVertexNo = 0;
  int faceVertexNo =
      3; // to make sure there are never more than 3 vertices per face
...
public:
  ...
  // - they can always be extended later on
  STLMesher()
      : hashTable(hashTableSize, nullptr), vertices(500000), normals(150000) {}
  ...
  void addVertex(float &x, float &y, float &z) {
    ...
    for (; (keyPtr = *keyPtrPtr); keyPtrPtr = &keyPtr->next) {
      if (keyPtr->x == x && keyPtr->y == y && keyPtr->z == z)
        break;
    }
    ...
    if (faceVertexNo++ > 2)
      fs::fatal() << "STL file does not appear to represent a triangular mesh";
...
static MRIS *mrisReadAsciiSTL(const std::string &filename) {
  ...
  while ((cp = fgetl(line, STRLEN, stlfile))) {
    ...
    case 'e': {
      continue;
    } break; // end specifiers
    case 'o': {
      continue;
    } break; // outer loop
    }

    fs::error() << "ASCII STL file " << filename
                << " contains unrecognized line: " << line;
    return nullptr;
  }
...
MRIS *mrisReadSTL(const std::string &filename) {
  ...
  // If the file is binary, it's total size will be equal to (84 + ntriangles *
  // 50) bytes
  ...
  // read the number of triangles specified and calculate the predicted (binary)
  // filesize
  unsigned int ntriangles;
  stlfile.read(reinterpret_cast<char *>(&ntriangles), 4);
