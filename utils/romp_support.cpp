...
/**
 * @file  romp_support.c
 * @brief prototypes and structures for getting reproducible results from and
...
#define omp_get_thread_num romp_omp_get_thread_num
  if (n >= _MAX_FS_THREADS) {
    fprintf(stderr,
            "Freesurfer supports a maximum of %d threads, set OMP_NUM_THREADS "
            "accordingly\n",
            _MAX_FS_THREADS);
    exit(1);
  }
...
ROMP_level romp_level =             // should not be set to ROMP_serial
                                    // ROMP_level_fast;
    ROMP_level_assume_reproducible; // doesn't require the random seed to be
...
static PerThreadScopeTreeData *enterScope(PerThreadScopeTreeData *parent,
  ...
  } // might need speeding up with a hash table
  if (!*prev) {
...
int ROMP_if_parallel2(ROMP_level level, ROMP_pf_stack_struct *pf_stack) {
  ...
  if (result) {
    ...
    romp_level =
        ROMP_level__size; // disable nested parallelism because hard to analyze
  }
...
static void rompExitHandler() {
  ...
  for (tid = 0; tid < ROMP_maxWatchedThreadNum; tid++) {
    ...
    if (tidStartTime[tid].inited)
      root->in_scope = tidStartTime[tid].timer.nanoseconds();
  }
...
void ROMP_pf_begin(ROMP_pf_static_struct *pf_static,
  ...
  if (romp_level >= ROMP_level__size) {
    if (debug)
      fprintf(
          stderr,
          "%s:%d only getting child tid start times for tid:%d pf_stack:%p\n",
          __FILE__, __LINE__, tid, pf_stack);
    pf_stack->watchedThreadBeginCPUTimes[tid] = cpuTimeUsed();
  } else {
    if (debug)
      fprintf(stderr,
              "%s:%d get child tid start times for tid:%d pf_stack:%p\n",
              __FILE__, __LINE__, tid, pf_stack);
...
#pragma omp parallel for schedule(static, 1)
    ...
    for (i = 0; i < ROMP_maxWatchedThreadNum; i++) {
      int childTid =
          ...
          0;
#endif
      if (childTid >= ROMP_maxWatchedThreadNum)
        continue;
...
static void pf_end_one_thread(int tid, ROMP_pf_stack_struct *pf_stack,
                              PerThreadScopeTreeData *tos) {
  int childTid =
#ifdef HAVE_OPENMP
      omp_get_thread_num();
#else
  ...
  if (startCPUTime == 0) {
    if (debug)
      fprintf(stderr,
              "%s:%d no corresponding start time for tid:%d pf_stack:%p\n",
              __FILE__, __LINE__, tid, pf_stack);
    return;
  }
  ...
  startCPUTime = -1; // copes when another iteration of this loop is executed on
                     // the same thread

  if (debug)
    fprintf(stderr, "Adding to tid:%d childTid:%d ns:%ld\n", tid, childTid,
            threadCpuTime);
  tos->in_child_threads[childTid] += threadCpuTime;
}

void ROMP_pf_end(ROMP_pf_stack_struct *pf_stack) {
  ROMP_pf_static_struct *pf_static = pf_stack->staticInfo;
  if (!pf_static)
    return;
  ...
  if (tid < ROMP_maxWatchedThreadNum) {
    ...
    if (tos) {
      ...
      if (pf_stack->gone_parallel)
        if (debug)
          fprintf(stderr,
                  "ROMP_pf_end tid:%d pf_stack:%p getting other thread times\n",
                  omp_get_thread_num(), pf_stack);

      if (!pf_stack->gone_parallel) {
        // Not the same as putting a condition on the else loop
        // The later is implemented by openmp choosing any available thread to
        // execute the loop body, not the current thread
      ...
      if (romp_level < ROMP_level__size) {
        ...
        for (i = 0; i < ROMP_maxWatchedThreadNum; i++) {
          ...
          if (startCPUTime != 0 && startCPUTime != -1) {
            if (debug)
              fprintf(stderr, "%s:%d no end time for %d\n", __FILE__, __LINE__,
                      tid);
          }
        }
      }
...
static void node_show_stats(FILE *file, PerThreadScopeTreeData *node,
                            unsigned int depth) {
  ROMP_pf_static_struct *pf = node->key;
  StaticData *sd = pf ? (StaticData *)(pf->ptr) : (StaticData *)nullptr;
  ...
  fprintf(file, "%s:%s, %d, %d, %12ld, %12ld, %12ld, %6.3g, %6.3g\n",
          pf ? pf->file : "<file>", pf ? pf->func : "<func>", pf ? pf->line : 0,
          sd ? sd->level : 0, node->in_scope, 0L, inAllThreads, 0.0,
          (double)inAllThreads / (double)node->in_scope);
...
void ROMP_show_stats(FILE *file) {
  ...
  fprintf(file, "file, line, level, in pf, in pflb, in_pfThread, pflb/elapsed, "
                "pft/elapsed\n");

  if (getMainFile()) {
    long mainDuration = mainTimer.nanoseconds();
    fprintf(file, "%s, %d, 0, %12ld, %12ld, %12ld, %6.3g, %6.3g\n", mainFile,
            mainLine, mainDuration, 0L, 0L, 1.0, 1.0);
  }
  ...
  for (tid = 0; tid < ROMP_maxWatchedThreadNum; tid++) {
    ...
    if (node)
      node_show_stats(file, node, 0);
  }

  fprintf(file, "ROMP_show_stats end\n");
}

void ROMP_Distributor_begin(ROMP_Distributor *distributor, int lo, int hi,
