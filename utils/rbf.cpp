...
typedef struct {
  int current_class;
  int (*get_observation_func)(VECTOR *v, int obs_no, void *parm, int same_class,
                              int *classno);
  void *parm;
} CLUSTERING_PARM;
...
static int rbfGradientDescent(RBF *rbf,
                              int (*get_observation_func)(VECTOR *v_obs, int no,
                                                          void *parm,
                                                          int same_class,
                                                          int *pclass),
                              void *parm);
...
------------------------------------------------------*/
RBF *RBFinit(int ninputs, int noutputs, int max_clusters[],
             const char *names[]) {
  RBF *rbf;
  int i;
...
int RBFprint(RBF *rbf, FILE *fp) {
  ...
  for (i = 0; i < rbf->noutputs; i++) {
    ...
    for (c = 0; c < cs->nclusters; c++) {
      ...
      fprintf(fp, "cluster %d has %d observations. Center: ", c,
              cluster->nsamples);
...
/*-----------------------------------------------------
        ...
        Description
          ...
          given class.
------------------------------------------------------*/
static int rbf_get_obs_func(VECTOR *v_obs, int desired_class_obs_no,
                            void *vcp) {
  CLUSTERING_PARM *cp;
...
static float rbfGaussian(MATRIX *m_sigma_inverse, VECTOR *v_means,
                         VECTOR *v_obs, VECTOR *v_z, float norm) {
  float val = 0.0f;
  static VECTOR *v_zT = nullptr;
  static MATRIX *m_tmp1 = nullptr, *m_tmp2 = nullptr;
...
------------------------------------------------------*/
int RBFprintActivations(RBF *rbf, VECTOR *v_obs, VECTOR *v_error, int classnum,
                        FILE *fp) {
  int i;
...
static int rbfAdjustHiddenCenters(RBF *rbf, VECTOR *v_error) {
  ...
  CLUSTER *cluster;
  static VECTOR *v_dE_dui = nullptr;
...
static int rbfAdjustHiddenSpreads(RBF *rbf, VECTOR *v_error) {
  ...
  CLUSTER *cluster;
  static MATRIX *m_sigma_inv_save = nullptr, *m_dE_dsigma_inv = nullptr;
...
static float rbfTrain(RBF *rbf,
  ...
  int obs_no, classnum, epoch = 0, nsmall = 0, nnegative = 0, nobs,
                        positive = 0;
  float error, sse = 0.0f, delta_sse, rms, old_sse;
  RBF *rbf_save = nullptr;
  ...
  if (!FZERO(old_sse))
    for (epoch = 0; epoch < MAX_EPOCHS; epoch++) {
      ...
      for (sse = 0.0f, nobs = 0; nobs < rbf->nobs; nobs++) {
        ...
        obs_no = nobs;
#endif
        if ((*get_observation_func)(v_obs, obs_no, parm, 0, &classnum) !=
            NO_ERROR)
...
int RBFexamineTrainingSet(RBF *rbf,
  ...
  while ((*get_observation_func)(v_obs, obs_no++, parm, 0, &classnum) ==
         NO_ERROR) {
    for (i = 0; i < rbf->ninputs; i++) {
...
RBF *RBFcopyWeights(RBF *rbf_src, RBF *rbf_dst) {
  ...
  if (!rbf_dst) {
    ...
    rbf_dst = RBFinit(rbf_src->ninputs, rbf_src->noutputs, max_clusters,
                      const_cast<const char **>(rbf_src->class_names));
  ...
  for (i = 0; i < rbf_src->nhidden; i++) {
    rbf_dst->clusters[i]->m_inverse = MatrixCopy(
        rbf_src->clusters[i]->m_inverse, rbf_dst->clusters[i]->m_inverse);
    VectorCopy(rbf_src->clusters[i]->v_means, rbf_dst->clusters[i]->v_means);
    rbf_dst->m_delta_sigma_inv[i] = MatrixCopy(rbf_src->m_delta_sigma_inv[i],
...
static int rbfCalculateOutputWeights(
  ...
  for (obs_no = 0; obs_no < rbf->nobs; obs_no++) {
    ...
    for (i = 1; i <= rbf->nhidden + 1; i++) {
      ...
      for (k = 1; k <= rbf->nhidden + 1; k++)
        m_Gt_G->rptr[i][k] +=
            RVECTOR_ELT(v_hidden, i) * RVECTOR_ELT(v_hidden, k);
    }
  }
  ...
  if (MatrixSingular(m_Gt_G)) {
    ...
    m_Gt_G_inv = MatrixSVDInverse(m_Gt_G, nullptr);
  } else
    m_Gt_G_inv = MatrixInverse(m_Gt_G, nullptr);
  if (!m_Gt_G_inv)
    ErrorReturn(
        ERROR_BADPARM,
        (ERROR_BADPARM, "rbfCalculateOutputWeights: matrix is singular"));
...
static int rbfAllocateTrainingParameters(RBF *rbf) {
  ...
  for (i = 0; i < rbf->nhidden; i++) {
    ...
    if (!rbf->m_delta_sigma_inv[i])
      ErrorExit(ERROR_NO_MEMORY,
                "rbfAllocateTrainingParms: could not allocate delta sigma %d",
                i);
    rbf->v_delta_means[i] = VectorAlloc(rbf->ninputs, MATRIX_REAL);
    if (!rbf->v_delta_means[i])
      ErrorExit(ERROR_NO_MEMORY,
                "rbfAllocateTrainingParms: could not allocate delta means %d",
                i);
  }
...
RBF *RBFreadFrom(FILE *fp) {
  int i, c, cno, classnum, noutputs, ninputs, nhidden,
      max_clusters[MAX_OUTPUTS];
  char *cp, line[100];
  ...
  rbf = RBFinit(ninputs, noutputs, max_clusters,
                const_cast<const char **>(names));
  for (i = 0; i < noutputs; i++)
    free(names[i]);
  ...
  for (cno = classnum = 0; classnum < rbf->noutputs; classnum++) {
    if (rbf->cs[classnum])
      for (c = 0; c < rbf->cs[classnum]->nclusters; c++, cno++)
        rbf->clusters[cno] = rbf->cs[classnum]->clusters + c;
  }
