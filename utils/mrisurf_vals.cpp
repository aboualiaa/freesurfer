...
//
// vertices
//              marked  marked2     annotations     flags   marked3     val mean
//              cropped
//  count       x
//  set all     x
//  set         x                                   x
...
int MRISclearMarks(MRIS *mris) { return MRISsetMarks(mris, 0); }
...
int MRISnotMarked(MRIS *mris) { return MRISinvertMarks(mris); }
...
int MRIScopyFromCropped(MRI_SURFACE *mris, int which) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case VERTEX_MARKS:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "MRIScopyFromCropped: %d unsupported target",
                which);
    }
  }
  return (NO_ERROR);
...
int MRISripZeroThicknessRegions(MRI_SURFACE *mris) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (FZERO(v->whitex - v->pialx) && FZERO(v->whitey - v->pialy) &&
...
MRI *MRISdilateConfined(MRIS *surf, MRI *mask, int annotidmask, int niters,
  ...
  mri1 = MRIcopy(mask, nullptr);
  mri2 = MRIcopy(mask, nullptr);
...
bool mrisAnyVertexOfFaceMarked(MRIS *mris, int fno) {
  ...
  int marked = 0;
  int n;
  for (n = 0; n < VERTICES_PER_FACE;
       n++) { // check all, to avoid branch mispredicts
...
int MRISaverageVals(MRI_SURFACE *mris, int navgs) {
  ...
  for (i = 0; i < navgs; i++) {
    ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) shared(mris, i)                 \
    schedule(static, 1)
#endif
        for (vno = 0; vno < mris->nvertices; vno++) {
      ROMP_PFLB_begin
...
#endif
        for (vno = 0; vno < mris->nvertices; vno++) {
      ROMP_PFLB_begin VERTEX *const v = &mris->vertices[vno];
      if (v->ripflag)
        ROMP_PF_continue;
...
int MRIScomputeAverageCircularPhaseGradient(MRI_SURFACE *mris, LABEL *area,
  ...
  for (i = 0; i < area->n_points; i++) {
    ...
    mzt      = MatrixTranspose(mz, nullptr);
    mztz     = MatrixMultiply(mzt, mz, NULL);
    mztz_inv = MatrixSVDInverse(mztz, nullptr);
    if (mztz_inv) {
...
int MRISmodeFilterVals(MRI_SURFACE *mris, int niter) {
  ...
  if (histo == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "MRISmodeFilterVals: could not allocate histo array of %d ints",
              max_val + 1);
...
int MRISmodeFilterZeroVals(MRI_SURFACE *mris) {
  int *histo, i, n, vno, ino, max_val, index, max_histo, max_index, nchanged,
      nzero;
...
int MRISmodeFilterAnnotations(MRI_SURFACE *mris, int niter) {
  int *histo, i, n, vno, ino, index, max_histo, max_index, max_annotation,
      *annotations, nchanged = 0;
  ...
  if (annotations == nullptr)
    ErrorExit(
        ERROR_NOMEMORY,
        "MRISmodeFilterVals: could not allocate annotation array of %d ints",
        max_index + 1);
...
int SetHop(int CenterVtx, MRI_SURFACE *Surf, int HopNo, int MaxHopNo) {
  ...
  for (nbr = 0; nbr < Surf->vertices_topology[CenterVtx].vnum; nbr++) {
    ...
    if (nbr_hopno != 0 && nbr_hopno < HopNo + 1)
      continue;
  ...
  for (nthhop = 1; nthhop < nHops; nthhop++) {
    ...
    for (nthvtx = 0; nthvtx < shl->nperhop[nthhop - 1]; nthvtx++) {
      ...
      for (nthnbr = 0; nthnbr < Surf->vertices_topology[vtxno].vnum; nthnbr++) {
        ...
        if (shl->hit[nbr_vtxno])
          continue; // ignore if it has been hit already
        shl->hit[nbr_vtxno] = 1;
        if (nhits >= shl->nperhop_alloced[nthhop]) {
          // realloc if need to
          shl->nperhop_alloced[nthhop] += 100;
          shl->vtxlist[nthhop] = (int *)realloc(
              shl->vtxlist[nthhop], sizeof(int) * shl->nperhop_alloced[nthhop]);
        }
        // assign the vertex
...
LABEL *MRISannotation_to_label(MRI_SURFACE *mris, int annot_index) {
  ...
  area = LabelAlloc(nv, nullptr, mris->ct->entries[annot_index]->name);
  for (nv = vno = 0; vno < mris->nvertices; vno++) {
    v = &mris->vertices[vno];
    if (v->ripflag) {
      continue;
...
/*-----------------------------------------------------
  ...
  but the ones beyond may be a subset rather than all neighbors at that
  distance.
...
int MRISsampleDistances(MRI_SURFACE *mris, int *nbrs, int max_nbhd) {
  ...
  const char *traceFnm =
      (++traceCount > 0) ? nullptr : "./MRISsampleDistances_new.txt";

  FILE *trace = traceFnm ? fopen(traceFnm, "w") : nullptr;
  if (trace)
    fprintf(trace, "MRISsampleDistances %d max_nbhd:%d\n", traceCount,
            max_nbhd);
...
static int MRISsampleDistances_new(MRI_SURFACE *mris, int *nbrs, int max_nbhd,
                                   FILE *trace) {
  if (trace)
    fprintf(trace, "MRISsampleDistances initial mris.nsize:%d\n", mris->nsize);
  ...
  {
    const char *cp;
    if ((cp = getenv("VDIAG1")) != nullptr)
      diag_vno1 = atoi(cp);
    if ((cp = getenv("VDIAG2")) != nullptr)
      diag_vno2 = atoi(cp);
    if (diag_vno1 >= 0) {
  ...
  float const dist_scale = IS_QUADRANGULAR(mris) ? (1.0 + sqrt(2.0)) / 2.0f
                                                 : TRIANGLE_DISTANCE_CORRECTION;
  ...
  if (!vnbrs || !vall || !vnb)
    ErrorExit(ERROR_NOMEMORY,
              "MRISsampleDistances: could not allocated v buffer");
  ...
  int max_possible = 0; // The requested number upto and beyond them
  {
    ...
    if (Gdiag & DIAG_HEARTBEAT)
      fprintf(stdout,
              ...
              vtotal, (float)vtotal / ((float)max_nbhd - (float)mris->nsize),
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
...

    {
      ...
      int i;
      for (i = vt->vtotal; i < vCapacity; i++)
        vt->v[i] = v->dist[i] = v->dist_orig[i] = 0;
    }
    ...
    for (nbhd_size = 1; nbhd_size <= max_nbhd; nbhd_size++) {
      ...
      for (n = old_vnum; n < vall_num; n++) {
        ...
        for (n2 = 0; n2 < vnt->vnum; n2++) {
          ...
          // BUG - THESE SHOULD BE double!  NOT CHANGED TO AVOID RESULTS
          // CHANGING.
          //
          float xd   = vn2->x - vn->x;
          float yd   = vn2->y - vn->y;
          ...
          dist += vn2->d;
#else
          dist = (dist + vn2->d * distance_scale[vn2->marked]) /
      ...
      */
      if (trace && vno == 0)
        fprintf(trace, "nbhd_size:%d found:%d\n", nbhd_size, found);
      ...
      if (found <= nbrs[nbhd_size]) /* just copy them all in */
      {
        ...
        for (n = 0; n < found; n++, vt->vtotal++) {
          ...
          if (trace && vno == 0)
            fprintf(trace, "copy dist_orig[%d]:%f\n", vt->vtotal,
                    v->dist_orig[vt->vtotal]);
        }
      } else /* randomly sample from them */
      {
        ...
        for (n = 0; n < nbrs[nbhd_size]; n++, vt->vtotal++) {
          ...
          vnbrs[i] = -1;
          if (trace && vno == 0)
            fprintf(trace, "rand dist_orig[%d]:%f\n", vt->vtotal,
                    v->dist_orig[vt->vtotal]);
        }
      }
    }
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < immediateCount; n++) {
      ...
      if (trace && vno == 0)
        fprintf(trace, "eucliod dist_orig[%d]:%f\n", n, v->dist_orig[n]);
    }
  }

  mris->vtotalsMightBeTooBig =
      1; // TODO find the right place to turn this off again
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vtotal; n++) {
      if (!DZERO(v->dist_orig[n])) {
        if (!nonZeroesCount && vno != 0)
          fprintf(stderr, "first zero distance at v %d, n %d (vn = %d)\n", vno,
                  n, vt->v[n]);
        nonZeroesCount++;
      } else {
        zeroesCount++;
        if (zeroesCount < 50)
          fprintf(stderr, "zero distance at v %d, n %d (vn = %d)\n", vno, n,
                  vt->v[n]);
        else if (zeroesCount == 50)
          fprintf(stderr, "further zeroes elided\n");
      }
    }
  }
  if (zeroesCount > 0)
