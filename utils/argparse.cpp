...
ArgumentParser::Argument::Argument()
    : short_name(""), name(""), argtype(ArgType::Unknown), consumed(0),
      fixed(true), required(false), positional(false), valid(false),
      fixed_nargs(0) {}

ArgumentParser::Argument::Argument(const ArgumentParser::String &_short_name,
                                   const ArgumentParser::String &_name,
                                   char nargs, ArgType _argtype, bool _required)
    : short_name(_short_name), name(_name), argtype(_argtype), consumed(0),
      required(_required), valid(true) {
...
/// Configures a command line argument. This can either be a positional argument
/// or an option (if 'name' is preceded with one or two dashes).
...
/// Configures a command line option. 'short_name' must be prefaced with a
/// single dash, and 'name' must be prefaced with two
void ArgumentParser::addArgument(const ArgumentParser::String &short_name,
                                 const ArgumentParser::String &name, char nargs,
                                 ArgType argtype, bool required) {
  Argument arg(verifyOption(short_name), verifyOption(name), nargs, argtype,
               required);
  insertArgument(arg);
}
...
void ArgumentParser::parse(size_t ac, char **av) {
  ...
  for (StringVector::const_iterator in = argv.begin() + 1; in < argv.end();
    ...
    if (index.count(element) == 0) {
      ...
      if (active.fixed && (active.fixed_nargs == active.consumed)) {
        ...
        // if we're parsing a variable number of inputs, we need to make sure
        // that we've left enough for any required positional arguments at the
        // end. To do this, we need to find out the minimum possible number of
        // positional arguments left to parse
        int minimum = 0;
        for (unsigned int idx = posidx + 1; idx < positionals.size(); idx++)
          minimum += positionals[idx].min_args;
        // now we jump ahead to find the actual number of positional arguments
        // remaining
        if (minimum > 0) {
          ...
          if (minimum > positionals_remaining)
            active = positionals[++posidx];
        }
      }

      // here we add the input element, but first test if it can be converted to
      // the proper arg type
      ...
      try {
        if (active.fixed && active.fixed_nargs == 1) {
          ...
          case ArgType::Int:
            variables[N].castTo<int>() = std::stoi(element);
            break;
          case ArgType::Float:
            variables[N].castTo<float>() = std::stof(element);
            break;
          case ArgType::Bool:
            variables[N].castTo<bool>() = stob(element);
            break;
          default:
            variables[N].castTo<String>() = element;
            break;
          }
        } else {
          switch (active.argtype) {
          case ArgType::Int:
            variables[N].castTo<IntVector>().push_back(std::stoi(element));
            break;
          case ArgType::Float:
            variables[N].castTo<FloatVector>().push_back(std::stof(element));
            break;
          case ArgType::Bool:
            variables[N].castTo<std::vector<bool>>().push_back(stob(element));
            break;
          default:
            variables[N].castTo<StringVector>().push_back(element);
            break;
          }
        }
      } catch (...) {
      ...
      // we've found a new flag, now check whether the active argument has
      // parsed enough elements
      if (active.valid)
        active.validate();
  ...
  for (ArgumentVector::const_iterator it = arguments.begin();
       it != arguments.end(); ++it) {
    Argument arg = *it;
...
void ArgumentParser::insertArgument(const ArgumentParser::Argument &arg) {
  ...
  if (arg.fixed && arg.fixed_nargs <= 1) {
    switch (arg.argtype) {
    case ArgType::Int:
      variables.push_back(int(0));
      break;
    case ArgType::Float:
      variables.push_back(float(0));
      break;
    case ArgType::Bool:
    ...
    switch (arg.argtype) {
    case ArgType::Int:
      variables.push_back(IntVector());
      break;
    case ArgType::Float:
      variables.push_back(FloatVector());
      break;
    case ArgType::Bool:
      variables.push_back(std::vector<bool>());
      break;
    case ArgType::String:
  ...
  // make sure there are no illogical combinations of arguments with a variable
  // number of inputs
  if (!arg.fixed) {
    if (arg.positional) {
    ...
    if (variable_positional && variable_flag) {
      fs::fatal() << "invalid argument configuration for '"
                  ...
                  << "A positional argument and flagged argument cannot both "
                     "have a variable "
                  << "amount of inputs, as this could lead to an undefined "
