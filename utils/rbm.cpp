...
static int dump_gradients(RBM *rbm, double *dvisible_bias, double *dvariance,
                          double *dhidden_bias, double **dw, RBM_PARMS *parms,
                          int step) {
  int v, h;
...
RBM *RBMalloc(int type, int nvisible, int nhidden, int nlabels,
  ...
  if (nlabels > 0) {
    ...
    if (!rbm->label_weights)
      ErrorExit(ERROR_NOMEMORY,
                "RBMalloc: could not allocate label weight array");

    rbm->label_states = (int *)calloc(nlabels, sizeof(rbm->label_states[0]));
    if (!rbm->label_states)
      ErrorExit(ERROR_NOMEMORY,
                "RBMalloc: could not allocate label state array");

    rbm->label_pvals =
        (LABEL_PVAL *)calloc(nlabels, sizeof(rbm->label_pvals[0]));
    if (!rbm->label_pvals)
      ErrorExit(ERROR_NOMEMORY,
                "RBMalloc: could not allocate label pval array");
  ...
  if (!rbm->act || !rbm->visible || !rbm->hidden || !rbm->hidden_state ||
      !rbm->active_pvals)
    ErrorExit(ERROR_NOMEMORY,
              "RBMalloc: could not allocate hidden or input layers");
  ...
  if (!rbm->visible_bias)
    ErrorExit(ERROR_NOMEMORY,
              "RBMalloc: could not allocate visible bias array");
...
int RBMactivateForward(RBM *rbm, double *visible) {
  ...
  ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) shared(rbm, visible)            \
    schedule(static, 1)
#endif
      for (h = 0; h < rbm->nhidden; h++) {
    ROMP_PFLB_begin
...
int RBMcomputeGradients(RBM *rbm, VOXLIST *vl, double **dw,
                        double *dvisible_bias, double *dhidden_bias,
  ...
  for (ind = index; ind < index + parms->mini_batch_size;
       ind++) // do one mini batch
  {
    i = indices[ind];
    x = vl->xi[i];
    y = vl->yi[i];
    ...
    if (rbm->nlabels > 0) // compute change in label biases and weights
    {
      ...
      for (v = 0; v < rbm->nlabels; v++) {
        ...
        for (h = 0; h < rbm->nhidden; h++) {
          ...
          dlabel_weights[v][h] -=
              parms->weight_decays[0] * rbm->label_weights[v][h];
        }
      }
    }
...
int RBMtrainFromImage(RBM *rbm, MRI *mri_inputs, MRI *mri_labels,
                      RBM_PARMS *parms) {
  VOXLIST *vl;
...
int RBMtrainFromVoxlistImage(RBM *rbm, VOXLIST *vl, RBM_PARMS *parms) {
  ...
  for (v = 0; v < rbm->nvisible; v++) {
    ...
    if (!dw[v] || !last_dw[v])
      ErrorExit(ERROR_NOMEMORY,
                "RBMtrain: could not allocate weight gradients");
  }
  ...
  for (nbad = step = 0; step < parms->nsteps; step++) {
    for (b = 0; b < parms->batches_per_step; b++) {
      index = (int)nint(
          randomNumber(0, held_out_index - (parms->mini_batch_size + 1)));
      RBMcomputeGradients(rbm, vl, dw, dvisible_bias, dhidden_bias, dvariance,
                          dlabel_bias, dlabel_weights, parms, indices, index);

      if (parms->debug &&
          ((!((step + 1) % parms->write_iterations)) || (step == 0)))
        dump_gradients(rbm, dvisible_bias, dvariance, dhidden_bias, dw, parms,
                       step);
      if (rbm->nlabels > 0) {
        for (v = 0; v < rbm->nlabels; v++) {
          delta =
              training_rate * dlabel_bias[v] + momentum * last_dlabel_bias[v];
          rbm->label_bias[v] += delta;
          last_dlabel_bias[v] = delta;
          for (h = 0; h < rbm->nhidden; h++) {
            delta = training_rate * dlabel_weights[v][h] +
                    momentum * last_dlabel_weights[v][h];
      ...
      for (v = 0; v < rbm->nvisible; v++) {
        delta =
            training_rate * dvisible_bias[v] + momentum * last_dvisible_bias[v];
      ...
      for (h = 0; h < rbm->nhidden; h++) {
        delta =
            training_rate * dhidden_bias[h] + momentum * last_dhidden_bias[h];
        ...
        for (y = whalf; y < mri_inputs->height - whalf; y++)
          for (z = 0; z < mri_inputs->depth; z++) {
            ...
            */
            ...
            RBMaverageActiveHiddenReceptiveFields(rbm, mri_receptive_fields,
                                                  mri_inputs, x, y, z);
            for (h = 0; h < rbm->nhidden; h++)
...
int DBNactivateBackward(DBN *dbn, int last_layer, int first_layer) {
  ...
  for (l = last_layer; l > first_layer; l--) {
    memcpy(dbn->rbms[l - 1]->hidden_state, dbn->rbms[l]->visible,
...
int DBNtrainFromImage(DBN *dbn, MRI *mri_inputs, MRI *mri_labels,
                      RBM_PARMS *parms) {
  VOXLIST *vl;
...
int DBNtrainFromVoxlistImage(DBN *dbn, VOXLIST *vl, RBM_PARMS *parms) {
  ...
  for (l = 0; l < dbn->nlayers; l++) {
    ...
    min_rms = last_rms = DBNvoxlistRMS(dbn, l, vl, parms, indices,
                                       held_out_index, parms->held_out);
    printf("iter %3.3d: rms = %2.3f\n", 0, last_rms);
    for (nbad = step = 0; step < parms->nsteps; step++) {
      for (b = 0; b < parms->batches_per_step; b++) {
        index = (int)nint(
            randomNumber(0, held_out_index - (parms->mini_batch_size + 1)));
        DBNcomputeGradients(dbn, l, vl, dw, dvisible_bias, dhidden_bias,
        ...
        for (v = 0; v < rbm->nvisible; v++) {
          delta = training_rate * dvisible_bias[v] +
                  momentum * last_dvisible_bias[v];
          rbm->visible_bias[v] += delta;
          last_dvisible_bias[v] = delta;
      ...
      rms = DBNvoxlistRMS(dbn, l, vl, parms, indices, held_out_index,
                          parms->held_out);
      if (!((step + 1) % parms->write_iterations))
...
int DBNcomputeGradients(DBN *dbn, int layer, VOXLIST *vl, double **dw,
  ...
  for (ind = index; ind < index + parms->mini_batch_size; ind++) {
    ...
    if (y > 0)
      DiagBreak();
    RBMsetLabel(rbm, current_label = nint(vl->vsrc[i]));
    RBMfillVisible(rbm_first_layer, mri_inputs, rbm_first_layer->visible, x, y,
                   z, f, parms->ksize);
    ...
    if (rbm->nlabels > 0) // compute change in label biases and weights
    {
      ...
      for (v = 0; v < rbm->nlabels; v++) {
        ...
        for (h = 0; h < rbm->nhidden; h++) {
          ...
          dlabel_weights[v][h] += (Q0 * l0 - Qn * ln);
          dlabel_weights[v][h] -=
              parms->weight_decays[layer] * rbm->label_weights[v][h];
        }
      }
    }
...
double DBNvoxlistRMS(DBN *dbn, int layer, VOXLIST *vl, RBM_PARMS *parms,
  ...
  if (indices) {
    for (rms = 0.0, nvox = 0, ind = index; ind < index + num; ind++) {
      ...
      f = vl->fi[i];
      RBMsetLabel(rbm, nint(vl->vsrc[i]));
      RBMfillVisible(rbm_first_layer, mri_inputs, rbm_first_layer->visible, x,
                     y, z, f, parms->ksize);
      DBNactivateForward(dbn, rbm_first_layer->visible, layer + 1);
      memcpy(visible, rbm->visible, rbm->nvisible * sizeof(rbm->visible[0]));
      RBMactivateBackward(rbm);
...
int CDBNtrainFromImage(CDBN *cdbn, MRI *mri_inputs, MRI *mri_labels,
                       RBM_PARMS *parms) {
  VOXLIST *vl;
...
CDBN *CDBNalloc(int type, int nlayers, int *ksizes, int *ngroups, int nlabels,
  ...
  for (layer = 0; layer < cdbn->nlayers; layer++) {
    ...
    if (layer == cdbn->nlayers - 1)
      ...
      nl = 0;
    cdbn->rbms[layer] =
        RBMalloc(type, nvisible, ngroups[layer], nl, RBM_INPUT_IMAGE);
    cdbn->rbms[layer]->ksize = ksizes[layer];
    cdbn->rbms[layer]->mri_inputs =
        layer == 0 ? mri_inputs : cdbn->mri_outputs[layer - 1];
  }
...
int CDBNtrainFromVoxlistImage(CDBN *cdbn, VOXLIST *vl, RBM_PARMS *parms,
  ...
  for (layer = 0; layer < cdbn->nlayers; layer++) {
    ...
    printf("************* TRAINING LAYER %d, sparsity = %2.2f wt %2.2f, "
           "momentum = %2.1f, dt = %2.4f, weight decay "
           "%2.5f ************\n",
    ...
    min_rms = last_rms =
        CDBNvoxlistRMS(cdbn, layer, vl, parms, indices, held_out_index,
                       parms->held_out, &min_label_rms);
    ...
    for (nbad = step = 0; step < parms->nsteps; step++) {
      for (b = 0; b < parms->batches_per_step; b++) {
        index = (int)nint(
            randomNumber(0, held_out_index - (parms->mini_batch_size + 1)));
        CDBNcomputeGradients(cdbn, layer, vl, dw, dvisible_bias, dhidden_bias,
                             dvariance, dlabel_bias, dlabel_weights, parms,
                             indices, index);
        ...
        for (v = 0; v < rbm->nvisible; v++) {
          ...
          last_dvisible_bias[v] = delta;
          rbm->visible_bias[v] =
              rbm->visible_bias[0]; // only 1 visible vias for CDBN
          if (parms->learn_variance) {
            delta = training_rate * dvariance[v] + momentum * last_dvariance[v];
            rbm->variance[v] += delta;
            last_dvariance[v] = delta;
      ...
      if (rbm->nlabels > 0) {
        label_pct_diff =
            100 * (last_label_rms - label_rms) / (.5 * (last_label_rms));
        printf("iter %3.3d: rms = %2.5f (%2.3f%%), label rms = %2.5f (%2.3f%%) "
               "%s\n",
               step + 1, rms, pct_diff, label_rms, label_pct_diff,
               new_min ? "****" : "");
      } else
    ...
    if (rbm->nlabels > 0 && parms->discriminative_training) {
      printf(
          "************* STARTING DISCRIMINATIVE TRAINING *****************\n");
      training_rate = parms->label_trate;
      memset(dlabel_bias, 0, rbm->nlabels * sizeof(dlabel_bias[0]));
      memset(last_dlabel_bias, 0, rbm->nlabels * sizeof(dlabel_bias[0]));
      for (v = 0; v < rbm->nlabels; v++) {
        memset(dlabel_weights[v], 0,
               rbm->nhidden * sizeof(dlabel_weights[v][0]));
        memset(last_dlabel_weights[v], 0,
               rbm->nhidden * sizeof(dlabel_weights[v][0]));
      }
      memset(dhidden_bias, 0, rbm->nhidden * sizeof(dhidden_bias[0]));
      ...
      for (nbad = 0; step < parms->nsteps; step++) {
        for (b = 0; b < parms->batches_per_step; b++) {
          ...
          if (parms->debug &&
              ((!((step + 1) % parms->write_iterations)) || (step == 0)))
            dump_gradients(rbm, dvisible_bias, dvariance, dhidden_bias, dw,
                           parms, step);
          ...
          for (h = 0; h < rbm->nhidden; h++) {
            delta = training_rate * dhidden_bias[h] +
                    momentum * last_dhidden_bias[h];
            if (fabs(delta) > 1e-5)
              DiagBreak();
            rbm->hidden_bias[h] += delta;
            last_dhidden_bias[h] = delta;
        ...
        label_pct_diff =
            100 * (last_label_rms - label_rms) / (.5 * (last_label_rms));
        printf(
            "iter %3.3d: rms = %2.5f (%2.3f%%), label rms = %2.5f (%2.3f%%)\n",
            step + 1, rms, pct_diff, label_rms, label_pct_diff);
...
double CDBNvoxlistRMS(CDBN *cdbn, int layer, VOXLIST *vl, RBM_PARMS *parms,
                      int *indices, int index, int num, double *plabel_rms) {
  ...
  if (indices) {
    for (h = 0; h < rbm->nhidden; h++)
      rbm->active[h] = 0;
    for (label_rms = rms = 0.0, nvox = 0, ind = index; ind < index + num;
         ind++) {
      i = indices[ind];
      x = vl->xi[i];
      y = vl->yi[i];
...
int CDBNcomputeGradients(CDBN *cdbn, int layer, VOXLIST *vl, double **dw,
                         double *dvisible_bias, double *dhidden_bias,
                         double *dvariance, double *dlabel_bias,
                         double **dlabel_weights, RBM_PARMS *parms,
  ...
  for (ind = index; ind < index + parms->mini_batch_size; ind++) {
    ...
    for (h = 0; h < rbm->nhidden; h++) {
      ...
      for (l = 0; l < rbm->nlabels; l++) {
        ...
        if (compute_hidden_bias) {
          dhidden_bias[h] += (l0 - rbm->labels[l]) * rbm->label_weights[l][h] *
                             rbm->hidden[h] * (1 - rbm->hidden[h]);
        ...
        if (compute_weights) {
          for (v = 0; v < rbm->nvisible; v++) {
            var = exp(rbm->variance[v]);
            dw[v][h] += (l0 - rbm->labels[l]) * rbm->label_weights[l][h] *
                        rbm->hidden[h] * (1 - rbm->hidden[h]) * visible[v] /
                        var;
          }
        }
      }
    ...
    for (l = 0; l < rbm->nlabels; l++) {
      ...
      if (compute_label_weights) {
        for (h = 0; h < rbm->nhidden; h++)
          dlabel_weights[l][h] +=
              (l0 - rbm->labels[l]) *
              (rbm->hidden[h] + rbm->label_weights[l][h] * l0);
      }
    }
  }
...
int CDBNactivateBackward(CDBN *cdbn, int last_layer, int first_layer) {
  ...
  for (l = last_layer; l > first_layer; l--) {
    memcpy(cdbn->rbms[l - 1]->hidden_state, cdbn->rbms[l]->visible,
...
MRI *CDBNcreateOutputs(CDBN *cdbn, RBM_PARMS *parms, MRI *mri_inputs,
  ...
  for (layer = first_layer, mri_layer_inputs = mri_inputs; layer <= last_layer;
    ...
    mri_layer_inputs = mri_outputs;
    {
      ...
      fclose(fp);
      printf("writing hidden counts to %s (%d/%2.1f%% always active, "
             "%d/%2.1f%% never)\n",
             fname, always, 100.0 * always / rbm->nhidden, never,
