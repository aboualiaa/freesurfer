...
GCLASSIFY *GCalloc(int nclasses, int nvars, const char *class_names[]) {
  ...
  if (!gc)
    ErrorReturn(NULL, (ERROR_NO_MEMORY, "GCalloc(%d): could not allocate GC",
                       nclasses));
    ...
    if (!gcl->m_wT) {
      ...
      ErrorReturn(NULL,
                  (ERROR_NO_MEMORY,
                   "GFalloc(%d): could not allocated %d x %d matrix"
                   "for %dth class",
                   nclasses, gcl->m_u->cols, gcl->m_covariance->rows, cno));
    }
#endif
    if (class_names)
...
int GCclassify(GCLASSIFY *gc, MATRIX *m_x, MATRIX *m_priors, float *prisk) {
  ...
  */
  ...
  if (prisk && classnum >= 0 && !FZERO(sum_p))
...
GCLASSIFY *GCasciiReadFrom(FILE *fp, GCLASSIFY *gc) {
  ...
  if (!gc) {
    gc = GCalloc(nclasses, nvars, nullptr);
    if (!gc)
...
GCLASS *GCasciiReadClassFrom(FILE *fp, GCLASS *gcl) {
  ...
  if ((cp = fgetl(class_name, CLASS_NAME_LEN - 1, fp)) == nullptr)
    ErrorReturn(NULL,
                (ERROR_BADFILE,
...
int GCinit(GCLASSIFY *gc, int classnum) {
  ...
  m_sigma_inverse = MatrixInverse(gcl->m_covariance, nullptr);
