...
#pragma omp parallel for
  for (int vno = 0; vno < nvertices; vno++) {
    ...
    if (vertex->origarea < 0)
      vertex->origarea = abs(vertex_areas[vno]);
  }
...
#pragma omp parallel for
    ...
    for (int vno = 0; vno < nvertices; vno++) {
      ...
      if (vertex_areas[vno] < 0) {
        ...
        for (int n = 0; n < vertex_vnums[vno]; n++)
          moveVertexRandomly(neighbors[n], trial, &random_counter);
      }
    }
...
#pragma omp parallel for
  for (int vno = 0; vno < nvertices; vno++) {
    ...
    XYZ_NORMALIZED_LOAD(&vertex_infos[vno].xyz_normalized,
                        &vertex_infos[vno].xyz_length, pos[0], pos[1], pos[2]);
    if (FZERO(vertex_infos[vno].xyz_length))
      vertex_infos[vno].rippedOrZeroLength = (char)true;
  }
    ...
    for (int n = 0; n < vtotal; n++) {
      ...
      if (!ninfo->rippedOrZeroLength) {
        ...
        float angle = fabs(XYZApproxAngle_knownLength(
            &vinfo->xyz_normalized, ninfo->xyz_unnormalized.x,
            ninfo->xyz_unnormalized.y, ninfo->xyz_unnormalized.z,
            ninfo->xyz_length));
...
#include "romp_for_end.h"
  ...
  for (int vno = 0; vno < nvertices; vno++) {
    ...
    for (int n = 0; n < nfaces; n++)
      area += face_areas[neighbor_faces[n]];
