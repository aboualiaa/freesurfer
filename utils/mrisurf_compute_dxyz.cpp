...
void   mrisDxyzSetLocationMoveLen(double newval) { LOCATION_MOVE_LEN = newval; }
...
int mrisComputeThicknessSmoothnessTerm(MRI_SURFACE *mris, double l_tsmooth,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (num = n = 0; n < vt->vnum; n++) {
      ...
      if (!vn->ripflag) {
        dn = SQR(vn->x - vn->origx) + SQR(vn->origy - vn->y) +
             SQR(vn->origz - vn->z);
        delta = d0 - dn;
        dx -= delta * vx;
        dy -= delta * vy;
...
*/
int mrisComputeRepulsiveTerm(MRI_SURFACE *mris, double l_repulse, MHT *mht,
                             MHT *mht_faces) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (bin = bucket->bins, num = i = 0; i < bucket->nused; i++, bin++) {
      ...
      if (!vn->ripflag) {
        ...
        // actually computed.
        scale = 4 * REPULSE_K /
                (dist * dist * dist * dist * dist * dist * dist); /* ^-7 */
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int mrisComputeRepulsiveRatioTerm(MRI_SURFACE *mris, double l_repulse,
                                  MHT *mht) {
...
int mrisComputeNonlinearSpringTerm(MRI_SURFACE *mris, double l_nlspring,
                                   INTEGRATION_PARMS *parms) {
  int    vno, n;
  ...
  if (FZERO(rmin) || FZERO(rmax))
    ErrorReturn(
        ERROR_BADPARM,
        (ERROR_BADPARM, "mrisComputeNonlinearSpringTerm: rmin or rmax = 0!"));
  ...
  for (sse_spring = 0.0, vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vnum; n++) {
      ...
      if (vno == Gdiag_no)
        printf(
            "l_nlspring: f = %2.3f (r = %2.2f), dx = (%2.2f, %2.2f, %2.2f)\n",
            f, r, v->nx * f * l_nlspring, v->ny * f * l_nlspring,
            v->nz * f * l_nlspring);
...
int mrisComputeAngleAreaTerms(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
...
#if METRIC_SCALE
  if (mris->patch || (mris->status != MRIS_SPHERE &&
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    ...
    if (!FZERO(l_area) && (face->v[0] == Gdiag_no || face->v[1] == Gdiag_no ||
                           face->v[2] == Gdiag_no)) {
      printf("face %d, orig area %2.2f, area, %2.2f, delta = %2.2f\n", fno,
             orig_area, area, delta);
      DiagBreak();
    }
    ...
    if (face->v[0] == Gdiag_no && !FZERO(parms->l_area)) {
      printf("\tv %d, area/angle term = (%2.3f %2.3f, %2.3f)\n", face->v[0],
             V3_X(v_sum), V3_Y(v_sum), V3_Z(v_sum));
    }
    ...
    if (!FZERO(l_angle) || !FZERO(parms->l_pangle)) {
      for (ano = 0; ano < ANGLES_PER_TRIANGLE; ano++) {
        ...
        if (face->angle[ano] >= 0.0f &&
            ((mris->status == MRIS_PLANE) || (mris->status == MRIS_SPHERE) ||
             (mris->status == MRIS_PARAMETERIZED_SPHERE))) {
          delta = 0.0f;
        }
#endif
        ...
        if (!FZERO(parms->l_angle + parms->l_pangle) &&
            ((face->v[0] == Gdiag_no || face->v[1] == Gdiag_no ||
              face->v[2] == Gdiag_no)))
          printf("face %d, ano %d, orig angle %2.1f, current angle %2.1f, "
                 "delta %2.1f\n",
                 fno, ano, DEGREES(face->orig_angle[ano]),
                 DEGREES(face->angle[ano]), DEGREES(delta));
        delta *= parms->l_angle;
...
int mrisComputeNonlinearAreaTerm(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    ...
    if (face->v[0] == Gdiag_no || face->v[1] == Gdiag_no ||
        face->v[2] == Gdiag_no) {
      DiagBreak();
    }
    FaceNormCacheEntry const *const fNorm = getFaceNorm(mris, fno);
    ...
    nlscale =
        (mris->total_area / mris->nfaces) / ((area < 0 ? 0 : area) + 0.001);
    nlscale = 1;
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int mrisComputeSurfaceRepulsionTerm(MRI_SURFACE *mris, double l_repulse,
                                    MHT *mht) {
  int     vno, max_vno, i;
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int mrisComputeWhichSurfaceRepulsionTerm(MRI_SURFACE *mris, double l_repulse,
                                         MHT *mht, int which,
                                         float dot_thresh) {
  int     vno, max_vno, i;
  float   dx, dy, dz, x, y, z, sx, sy, sz, norm[3], dot;
...
#pragma omp parallel for if_ROMP(experimental) reduction(+ : dot_total)
    ...
    if (vno == Gdiag_no)
      fprintf(stdout, "v %d spring norm term: (%2.3f, %2.3f, %2.3f)\n", vno,
              l_spring * sx, l_spring * sy, l_spring * sz);
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    if (vno == Gdiag_no)
      fprintf(stdout, "v %d convexity term: (%2.3f, %2.3f, %2.3f)\n", vno,
              l_convex * sx, l_convex * sy, l_convex * sz);
...
*/
inline void vertexComputeNormalSpringTerm(MRIS *mris, int vno, float *dx,
                                          float *dy, float *dz,
                                          double l_spring) {
  VERTEX *const                vertex  = &mris->vertices[vno];
  VERTEX_TOPOLOGY const *const vertext = &mris->vertices_topology[vno];
...
*/
inline void vertexComputeTangentialSpringTerm(MRIS *mris, int vno, float *dx,
                                              float *dy, float *dz,
                                              double l_spring) {
  VERTEX *const                vertex  = &mris->vertices[vno];
  VERTEX_TOPOLOGY const *const vertext = &mris->vertices_topology[vno];
...
int mrisComputeNonlinearTangentialSpringTerm(MRI_SURFACE *mris, double l_spring,
                                             double min_dist) {
  int    vno, m, n;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no)
      fprintf(stdout,
              ...
              "dot=%2.3f\n",
              vno, pial ? "pial" : "white", dx, dy, dz, lx, ly, lz, v->nx,
              v->ny, v->nz, lx * v->nx + ly * v->ny + lz * v->nz);
  }
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
int mrisComputeQuadraticCurvatureTerm(
    MRI_SURFACE *mris, double l_curv) // BEVIN mris_make_surfaces 4
{
  if (FZERO(l_curv)) {
    return (NO_ERROR);
  }
...
#undef REUSE
    ...
    MATRIX *m_X =
        MatrixAlloc(vt->vtotal, 5, MATRIX_REAL); /* 2-d quadratic fit */
...
/*-----------------------------------------------------
  ...
  vertex in the normal direction to improve the fit.
  ------------------------------------------------------*/
int mrisComputeSurfaceNormalIntersectionTerm(MRI_SURFACE *mris, MHT *mht,
                                             double l_norm, double max_dist) {
  int    vno;
  double step;
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    for (dist = 0; dist < 2 * max_dist; dist += step) {
      d = dist;
      if (mrisDirectionTriangleIntersection(mris, v->x, v->y, v->z, v->nx,
                                            v->ny, v->nz, mht, &d, vno)) {
        if (d < max_dist) {
          if (vno == Gdiag_no)
            printf("v %d surface self intersection at distance %2.3f\n", vno,
                   d);
...
#define MAX_SCALE 3
      ...
      dz = (d-max_dist)/max_dist*l_norm*v->nz ;
          ...
          if (vno == Gdiag_no)
            printf(
                "\t intersection scale %2.2f, vector (%2.2f, %2.2f, %2.2f)\n",
                scale, dx, dy, dz);
          v->dx += dx;
          v->dy += dy;
          v->dz += dz;
...
int mrisComputeDuraTerm(MRI_SURFACE *mris, double l_dura, MRI *mri_dura,
                        double dura_thresh) {
  int     vno;
  VERTEX *v;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no) {
      ...
      fprintf(stdout,
              ...
              xw, yw, zw, val0, xwo, ywo, zwo, val_outside, xwi, ywi, zwi,
              val_inside);
      fprintf(stdout,
...
int mrisComputeIntensityTerm(MRI_SURFACE *mris, double l_intensity,
                             ...
                             double sigma_global, INTEGRATION_PARMS *parms) {
  int     vno;
  VERTEX *v;
  float   x, y, z, nx, ny, nz, dx, dy, dz;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (parms->grad_dir == 0) {
      ...
      step_size = MIN(
          sigma / 2,
          MIN(mri_brain->xsize, MIN(mri_brain->ysize, mri_brain->zsize)) * 0.5);
      ktotal_inside = ktotal_outside = 0.0;
      for (n = 0, val_outside = val_inside = 0.0, dist = step_size;
           dist <= 2 * sigma; dist += step_size, n++) {
        k  = exp(-dist * dist / (2 * sigma * sigma));
        xw = x + dist * nx;
        yw = y + dist * ny;
        zw = z + dist * nz;
        if (mri_interior) {
          MRISsurfaceRASToVoxelCached(mris, mri_interior, xw, yw, zw, &xvi,
                                      &yvi, &zvi);
        ...
        if (mri_interior == NULL || interior < .9) {
          ktotal_outside += k;
          MRISsurfaceRASToVoxelCached(mris, mri_brain, xw, yw, zw, &xw, &yw,
                                      &zw);
    ...
    if (vno == Gdiag_no) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_smooth, xw, yw, zw, &xwo, &ywo,
                                  &zwo);
      /* sample inward from surface */
      xw = x - mri_smooth->xsize * nx;
      yw = y - mri_smooth->ysize * ny;
      zw = z - mri_smooth->zsize * nz;
      MRISsurfaceRASToVoxelCached(mris, mri_smooth, xw, yw, zw, &xwi, &ywi,
                                  &zwi);
...
int mrisComputeTargetLocationTerm(MRI_SURFACE *mris, double l_location,
                                  INTEGRATION_PARMS *parms) {
  int     vno;
  VERTEX *v;
...
#define LOCATION_MOVE_LEN 0.25
    ...
    if (vno == Gdiag_no) {
      fprintf(stdout,
              ...
              v->targx, v->targy, v->targz, v->x, v->y, v->z, l_location * dx,
              l_location * dy, l_location * dz, norm,
...
int mrisComputeIntensityTerm_mef(MRI_SURFACE *mris, double l_intensity,
                                 MRI *mri_30, MRI *mri_5, double sigma_global,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (!FZERO(weight30)) {
      ...
      if (parms->grad_dir == 0) {
        ...
        step_size =
            MIN(sigma / 2,
                MIN(mri_30->xsize, MIN(mri_30->ysize, mri_30->zsize)) * 0.5);
    ...
    if (!FZERO(weight5)) {
      ...
      if (parms->grad_dir == 0) {
        ...
        step_size =
            MIN(sigma / 2,
                MIN(mri_5->xsize, MIN(mri_5->ysize, mri_5->zsize)) * 0.5);
        ktotal_inside = ktotal_outside = 0.0;
        for (n = 0, val_outside = val_inside = 0.0, dist = step_size;
             dist <= 2 * sigma; dist += step_size, n++) {
          k = exp(-dist * dist / (2 * sigma * sigma));
      ...
      if (vno == Gdiag_no && !FZERO(weight5)) {
        ...
        fprintf(stdout,
                ...
                vno, dx, dy, dz, delV, delI, dx * nx + dy * ny + dz * nz);
      }
      v->dx += dx;
      v->dy += dy;
...
int mrisComputeDistanceTerm(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  if (!(mris->dist_alloced_flags & 1)) {
    switch (copeWithLogicProblem("FREESURFER_fix_mrisComputeDistanceTerm",
  ...
  char *  cp;
  VECTOR *v_y[_MAX_FS_THREADS], *v_delta[_MAX_FS_THREADS],
      *v_n[_MAX_FS_THREADS];
...
#pragma omp parallel for if_ROMP(assume_reproducible) schedule(static, 1)
    ...
    for (n = 0; n < vnum; n++) {
      ...
      if (vno == Gdiag_no && Gdiag & DIAG_SHOW & 0) // deverbosified by dng
        fprintf(stdout,
                ...
                "d0 %2.2f, dt %2.2f, "
                "delta %2.3f\n\tdx=%2.3f, %2.3f, %2.3f)\n",
                n, vt->v[n], vn->x, vn->y, vn->z, d0, dt, delta, V3_X(v_y[tid]),
                V3_Y(v_y[tid]), V3_Z(v_y[tid]));
      if ((vno == diag_vno1 && vt->v[n] == diag_vno2) ||
          (vno == diag_vno2 && vt->v[n] == diag_vno1))
    ...
    if ((vno == Gdiag_no || vno == diag_vno1 || vno == diag_vno1) &&
      ...
      fprintf(stdout, "total delta=(%2.3f, %2.3f, %2.3f)\n", V3_X(v_delta[tid]),
              V3_Y(v_delta[tid]), V3_Z(v_delta[tid]));
    ...
    if (Gdiag_no == vno) {
      ...
      for (i = 0; i < vt->vtotal; i++) {
        ...
        fprintf(fp, "%03d: %05d, %f   %f   %f\n", i, vt->v[i], dist_orig_i,
                v->dist[i], v->dist[i] - dist_orig_i / scale);
      }
      fclose(fp);
    }
    ROMP_PFLB_end
...
/*-----------------------------------------------------
  Compute the effects of the gradient of the distance term
  ------------------------------------------------------*/
int mrisComputeNonlinearDistanceTerm(MRI_SURFACE *      mris,
                                     INTEGRATION_PARMS *parms) {
    ...
    for (n = 0; n < vnum; n++) {
      ...
      if (vno == Gdiag_no && Gdiag & DIAG_SHOW)
        fprintf(stdout,
                ...
                "d0 %2.2f, dt %2.2f, "
                "delta %2.3f\n\ty=%2.3f, %2.3f, %2.3f)\n",
                n, vt->v[n], vn->x, vn->y, vn->z, d0, dt, delta, V3_X(v_y),
                V3_Y(v_y), V3_Z(v_y));
    }
...
int mrisComputeIntensityGradientTerm(MRI_SURFACE *mris, double l_grad,
                                     MRI *mri_brain, MRI *mri_smooth) {
  int     vno;
  VERTEX *v;
  float   x, y, z, nx, ny, nz;
  double  val0, mag0, xw, yw, zw, del, mag_outside, mag_inside, delI, delV, dx,
      dy, dz, val_outside, val_inside, val_dist, dn, xw1, yw1, zw1;
...
int mrisComputeCorrelationTerm(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    */
    ...
    /* compute target term */
    up1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x + ux, y + uy,
                           z + uz, fno);
    um1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x - ux, y - uy,
                           z - uz, fno);
    vp1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x + vx, y + vy,
                           z + vz, fno);
    vm1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x - vx, y - vy,
                           z - vz, fno);
    du  = (up1 - um1) / (2 * d_dist);
    dv  = (vp1 - vm1) / (2 * d_dist);
    ...
    if (vno == Gdiag_no)
      printf("l_corr(%d): dx = (%2.2f, %2.2f, %2.2f), vsmooth = %2.3f\n", vno,
             coef * (du * e1x + dv * e2x), coef * (du * e1y + dv * e2y),
             coef * (du * e1z + dv * e2z), vsmooth);
  }
...
int mrisComputeVectorCorrelationTerm(MRI_SURFACE *      mris,
                                     INTEGRATION_PARMS *parms) {
  double  du, dv, up1, um1, vp1, vm1, delta, src, target, mag, max_mag, l_corr;
  VERTEX *v;
  int     vno, fno, n;
  ...
  corrs =
      (double *)malloc(nframes * sizeof(double)); /* correlation coefficients */
  ...
  upvals =
      (double *)malloc(nframes * sizeof(double)); /* without the variances */
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    y      = v->y;
    z      = v->z;
    target = MRISPfunctionVectorVals(parms->mrisp_template, mris, x, y, z,
                                     frames, 2 * nframes, vals);
    /* now compute gradient of template w.r.t. a change in vertex position */
    /*
    ...
    */
    ...
    up1 = MRISPfunctionVectorVals(parms->mrisp_template, mris, x + ux, y + uy,
                                  z + uz, wv_frames, nframes, upvals);
    um1 = MRISPfunctionVectorVals(parms->mrisp_template, mris, x - ux, y - uy,
                                  z - uz, wv_frames, nframes, umvals);
    vp1 = MRISPfunctionVectorVals(parms->mrisp_template, mris, x + vx, y + vy,
                                  z + vz, wv_frames, nframes, vpvals);
    vm1 = MRISPfunctionVectorVals(parms->mrisp_template, mris, x - vx, y - vy,
                                  z - vz, wv_frames, nframes, vmvals);
...
int mrisComputePolarCorrelationTerm(MRI_SURFACE *      mris,
                                    INTEGRATION_PARMS *parms) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    */
    ...
    dy  = -x * D_ANGLE;
    dz  = 0;
    am1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x - dx, y - dy,
                           z - dz, 0);
    ap1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x + dx, y + dy,
                           z + dz, 0);
    ...
    dy  = 0;
    dz  = x * D_ANGLE;
    bm1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x - dx, y - dy,
                           z - dz, 0);
    bp1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x + dx, y + dy,
                           z + dz, 0);
    ...
    dy  = -z * D_ANGLE;
    dz  = y * D_ANGLE;
    gm1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x - dx, y - dy,
                           z - dz, 0);
    gp1 = MRISPfunctionVal(parms->mrisp_template, mris->radius, x + dx, y + dy,
                           z + dz, 0);
...
int mrisComputePolarVectorCorrelationTerm(MRI_SURFACE *      mris,
                                          INTEGRATION_PARMS *parms) {
  ...
  double *vals, *dalpha, *dbeta, *dgamma, *apvals, *amvals, *bpvals, *bmvals,
      *gpvals, *gmvals, *corrs;
  int nframes, *frames, *wv_frames, *ind;
  ...
  corrs =
      (double *)malloc(nframes * sizeof(double)); /* correlation coefficients */
  dalpha =
      (double *)malloc(nframes * sizeof(double)); /* gradient coefficients */
  dbeta =
      (double *)malloc(nframes * sizeof(double)); /* gradient coefficients */
  dgamma =
      (double *)malloc(nframes * sizeof(double)); /* gradient coefficients */
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    */
    ...
    dy = -x * D_ANGLE;
    dz = 0;
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x - dx, y - dy, z - dz,
                            wv_frames, nframes, amvals);
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x + dx, y + dy, z + dz,
                            wv_frames, nframes, apvals);
    ...
    dz = x * D_ANGLE;
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x - dx, y - dy, z - dz,
                            wv_frames, nframes, bmvals);
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x + dx, y + dy, z + dz,
                            wv_frames, nframes, bpvals);
    ...
    dz = y * D_ANGLE;
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x - dx, y - dy, z - dz,
                            wv_frames, nframes, gmvals);
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x + dx, y + dy, z + dz,
                            wv_frames, nframes, gpvals);
...
int mrisComputeExpandwrapTerm(MRI_SURFACE *mris, MRI *mri_brain,
                              double l_expandwrap) {
  int     vno;
...
int mrisComputeShrinkwrapTerm(MRI_SURFACE *mris, MRI *mri_brain,
                              double l_shrinkwrap) {
  int     vno;
  double  xw, yw, zw, x, y, z, val, dx, dy, dz;
  VERTEX *v;
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
int mrisComputeSphereTerm(MRI_SURFACE *mris, double l_sphere, float radius,
                          int explode_flag) {
  int     vno;
  VERTEX *v;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no &&
        (r * l_sphere * x * v->nx + r * l_sphere * y * v->ny +
             r * l_sphere * z * v->nz <
         0)) {
      DiagBreak();
    }
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no)
      printf("v %d expansion term: (%2.3f, %2.3f, %2.3f)\n", vno,
             l_expand * v->nx, l_expand * v->ny, l_expand * v->nz);
  }
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int mrisComputeThicknessMinimizationTerm(MRI_SURFACE *mris, double l_thick_min,
                                         INTEGRATION_PARMS *parms) {
  int vno;
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    */
    e1p =
        mrisSampleMinimizationEnergy(mris, vno, parms, v->x + d_dist * e1x,
                                     v->y + d_dist * e1y, v->z + d_dist * e1z);
    e1m =
        mrisSampleMinimizationEnergy(mris, vno, parms, v->x - d_dist * e1x,
                                     v->y - d_dist * e1y, v->z - d_dist * e1z);
    e2p =
        mrisSampleMinimizationEnergy(mris, vno, parms, v->x + d_dist * e2x,
                                     v->y + d_dist * e2y, v->z + d_dist * e2z);
    e2m =
        mrisSampleMinimizationEnergy(mris, vno, parms, v->x - d_dist * e2x,
                                     v->y - d_dist * e2y, v->z - d_dist * e2z);
    ...
    E1 = mrisSampleMinimizationEnergy(mris, vno, parms, v->x + parms->dt * dx,
                                      v->y + parms->dt * dy,
                                      v->z + parms->dt * dz);
    ...
    if (E1 > E0) {
      ...
      E2 = mrisSampleMinimizationEnergy(mris, vno, parms, v->x - parms->dt * dx,
                                        v->y - parms->dt * dy,
                                        v->z - parms->dt * dz);
      if (E2 < E0) {
        dx *= -1;
    ...
    v->dz += dz;
    if (Gdiag_no == vno) {
      printf("l_thick_min: v %d: E0 %2.3f, E1 %2.3f, dE = (%2.2f, %2.2f), e1 = "
             "(%2.1f, %2.1f, %2.1f), e2 = (%2.1f, %2.1f, "
             "%2.1f), DX = (%2.2f, %2.2f, %2.2f)\n",

             vno, sqrt(E0), sqrt(E1), dE_de1, dE_de2, e1x, e1y, e1z, e2x, e2y,
             e2z, l_thick_min * (dE_de1 * e1x + dE_de2 * e2x),
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int mrisComputeThicknessParallelTerm(MRI_SURFACE *mris, double l_thick_parallel,
                                     INTEGRATION_PARMS *parms) {
  int vno;
  ...
  ROMP_PF_begin
    ...
    ROMP_PFLB_begin
        ...
        float dE_de1,
        dE_de2, e1p, e1m, e2p, e2m, dx, dy, dz;
    ...
    /*
      ...
      compute the derivates using them.
    */
    e1p    = mrisSampleParallelEnergy(mris, vno, parms, v->x + d_dist * e1x,
                                   v->y + d_dist * e1y, v->z + d_dist * e1z);
    e1m    = mrisSampleParallelEnergy(mris, vno, parms, v->x - d_dist * e1x,
                                   v->y - d_dist * e1y, v->z - d_dist * e1z);
    e2p    = mrisSampleParallelEnergy(mris, vno, parms, v->x + d_dist * e2x,
                                   v->y + d_dist * e2y, v->z + d_dist * e2z);
    e2m    = mrisSampleParallelEnergy(mris, vno, parms, v->x - d_dist * e2x,
                                   v->y - d_dist * e2y, v->z - d_dist * e2z);
    dE_de1 = (e1p - e1m) / (2 * d_dist);
    dE_de2 = (e2p - e2m) / (2 * d_dist);
    norm   = sqrt(dE_de1 * dE_de1 + dE_de2 * dE_de2);
    ...
    E1 = mrisSampleParallelEnergy(mris, vno, parms, v->x + parms->dt * dx,
                                  v->y + parms->dt * dy, v->z + parms->dt * dz);
#if 1
    if (E1 > E0) {
      double E2;
      DiagBreak();
      ...
      E2 = mrisSampleParallelEnergy(mris, vno, parms, v->x - parms->dt * dx,
                                    v->y - parms->dt * dy,
                                    v->z - parms->dt * dz);
      if (E2 < E0) {
        dx *= -1;
    ...
    v->dz += dz;
    if (Gdiag_no == vno) {
      printf("l_thick_parallel: v %d: E0=%f, E1=%f, dE = (%2.3f, %2.3f), e1 = "
             "(%2.1f, %2.1f, %2.1f), e2 = (%2.1f, %2.1f, "
             "%2.1f), DX = (%2.3f, %2.3f, %2.3f)\n",

             vno, E0, E1, dE_de1, dE_de2, e1x, e1y, e1z, e2x, e2y, e2z,
             parms->dt * dx, parms->dt * dy, parms->dt * dz);
    }
    ROMP_PFLB_end
  }
  ROMP_PF_end
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
#define MAX_NORM 0.1
int mrisComputeThicknessNormalTerm(MRI_SURFACE *mris, double l_thick_normal,
                                   INTEGRATION_PARMS *parms) {
  int vno;
  //  int     missed = 0 ;
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    ROMP_PFLB_begin
        ...
        float dE_de1,
        dE_de2, e1p, e1m, e2p, e2m, cx, cy, cz;
    ...
    /*
      ...
      compute the derivates using them.
    */
    e1p    = mrisSampleNormalEnergy(mris, vno, parms, v->x + d_dist * e1x,
                                 v->y + d_dist * e1y, v->z + d_dist * e1z);
    e1m    = mrisSampleNormalEnergy(mris, vno, parms, v->x - d_dist * e1x,
                                 v->y - d_dist * e1y, v->z - d_dist * e1z);
    e2p    = mrisSampleNormalEnergy(mris, vno, parms, v->x + d_dist * e2x,
                                 v->y + d_dist * e2y, v->z + d_dist * e2z);
    e2m    = mrisSampleNormalEnergy(mris, vno, parms, v->x - d_dist * e2x,
                                 v->y - d_dist * e2y, v->z - d_dist * e2z);
    dE_de1 = (e1p - e1m) / (2 * d_dist);
    dE_de2 = (e2p - e2m) / (2 * d_dist);
    norm   = sqrt(dE_de1 * dE_de1 + dE_de2 * dE_de2);
    ...
    if (E1 > E0) {
      ...
      DiagBreak();
      E2 = mrisSampleNormalEnergy(mris, vno, parms, v->x - parms->dt * dx,
                                  v->y - parms->dt * dy, v->z - parms->dt * dz);
      if (E2 < E0) {
        dx *= -1;
        dy *= -1;
    ...
    if (Gdiag_no == vno) {
      ...
      printf("l_thick_normal: v %d: E=%f-->%f (%f), dE = (%2.2f, %2.2f), e1 = "
             "(%2.1f, %2.1f, %2.1f), e2 = (%2.1f, %2.1f, "
             "%2.1f), DX = (%2.2f, %2.2f, %2.2f)\n",
      ...
      printf(
          "    N = (%2.2f, %2.2f, %2.2f), D = (%2.2f, %2.2f, %2.2f), dot= %f\n",
...
int mrisComputeThicknessSpringTerm(MRI_SURFACE *mris, double l_thick_spring,
                                   INTEGRATION_PARMS *parms) {
  int    vno, max_vno;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    */
    ...
    e1p =
        mrisSampleSpringEnergy(mris, vno, v->x + d_dist * e1x,
                               v->y + d_dist * e1y, v->z + d_dist * e1z, parms);
    e1m =
        mrisSampleSpringEnergy(mris, vno, v->x - d_dist * e1x,
                               v->y - d_dist * e1y, v->z - d_dist * e1z, parms);
    e2p =
        mrisSampleSpringEnergy(mris, vno, v->x + d_dist * e2x,
                               v->y + d_dist * e2y, v->z + d_dist * e2z, parms);
    e2m =
        mrisSampleSpringEnergy(mris, vno, v->x - d_dist * e2x,
                               v->y - d_dist * e2y, v->z - d_dist * e2z, parms);
    dE_de1 = (e1p - e1m) / (2 * d_dist);
    ...
    if (E1 > E0) {
      ...
      E2 = mrisSampleSpringEnergy(mris, vno, v->x - parms->dt * dx,
                                  v->y - parms->dt * dy, v->z - parms->dt * dz,
                                  parms);
      if (E2 < E0) {
        dx *= -1;
    ...
    if (Gdiag_no == vno) {
      ...
      printf("l_thick_spring: v %d: E=%f-->%f (%f), dE = (%2.2f, %2.2f), e1 = "
             "(%2.1f, %2.1f, %2.1f), e2 = (%2.1f, %2.1f, "
             "%2.1f), DX = (%2.2f, %2.2f, %2.2f)\n",
...
int mrisComputeAshburnerTriangleTerm(MRI_SURFACE *      mris,
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    */
    e1p = mrisSampleAshburnerTriangleEnergy(
        mris, vno, parms, v->x + d_dist * e1x, v->y + d_dist * e1y,
        v->z + d_dist * e1z);
    e1m = mrisSampleAshburnerTriangleEnergy(
        mris, vno, parms, v->x - d_dist * e1x, v->y - d_dist * e1y,
        v->z - d_dist * e1z);
    e2p = mrisSampleAshburnerTriangleEnergy(
        mris, vno, parms, v->x + d_dist * e2x, v->y + d_dist * e2y,
        v->z + d_dist * e2z);
    ...
    if (E1 > E0) {
      ...
      E2 = mrisSampleAshburnerTriangleEnergy(
          mris, vno, parms, v->x - parms->dt * dx, v->y - parms->dt * dy,
          v->z - parms->dt * dz);
      if (E2 < E0) {
        dx *= -1;
        dy *= -1;
    ...
    v->dz += dz;
    if (Gdiag_no == vno) {
      printf("l_ashburner_triangle: v %d: E=%f-->%f (%f), dE = (%2.2f, %2.2f), "
             "e1 = (%2.1f, %2.1f, %2.1f), e2 = (%2.1f, "
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (d = -2; d < .2; d += .1) {
      for (num = 0, p = 0, d2 = -2; d2 < .2; d2 += .1) {
        ...
        y = v->y + d2 * v->ny;
        z = v->z + d2 * v->nz;
        MRISsurfaceRASToVoxelCached(mris, parms->mri_brain, x, y, z, &xv, &yv,
                                    &zv);
    ...
    if (vno == Gdiag_no) {
      printf("histoTerm for v %d: val = %2.1f, optimal distance=%2.3f, p = %f "
             "moving by (%2.3f, %2.3f, %2.3f), dot = "
             "%2.3f\n",
             vno, val, best_d, best_p, dx, dy, dz,
             dx * v->nx + dy * v->ny + dz * v->nz);
...
int mrisComputePosterior2DTerm(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  vl = vl2;
  m_vox2vox =
      MRIgetVoxelToVoxelXform(mri, parms->mri_dtrans); // dist to pial surface
  for (vno = 0; vno < mris->nvertices; vno++) {
    VERTEX *const v = &mris->vertices[vno];
    if (vno == Gdiag_no)
      DiagBreak();
    ...
    if (vno == Gdiag_no && parms->h2d_out != NULL) // diags
    {
      ...
      for (dn = -MAX_DISPLACEMENT; dn <= MAX_DISPLACEMENT;
           dn += DISPLACEMENT_DELTA) {
        ll = -vlst_loglikelihood2D(mris, mri, vno, dn, vl[vno], parms->h2d_out,
                                   fp);
        if (devIsnan(ll))
          DiagBreak();
        if (ll > best_ll) {
    ...
    if (vno == Gdiag_no) {
      ...
      for (i = 0; i < vl[vno]->nvox; i++) {
        ...
        fprintf(fp, "%d %d %d %d %d %f %f %f\n", vno, i, vl[vno]->xi[i],
                vl[vno]->yi[i], vl[vno]->zi[i], val, dist, p);
      }
      fclose(fp);

      printf("l_map: vno %d, best displacement %2.3f, ll = %2.3f, D = (%2.3f, "
             "%2.3f, %2.3f)\n",
             vno, best_dn, best_ll, best_dn * v->nx * parms->l_map2d,
    ...
    if (num > 0) {
      ...
      if (vno == Gdiag_no)
        printf("l_map: vno %d, soap bubble displacement %2.3f, D = (%2.3f, "
               "%2.3f, %2.3f)\n",
               vno, dn, dn * v->nx * parms->l_map2d,
               dn * v->ny * parms->l_map2d, dn * v->nz * parms->l_map2d);
    }
  }
