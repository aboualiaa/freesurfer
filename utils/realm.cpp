...
//
// (cd utils; rm -f a.out ; gcc -DREALM_UNIT_TEST -o a.out -I ../include realm.c
// -lm |& less ; ./a.out)
//
...
MRIS *MRISoverAlloc(int max_vertices, int max_faces, int nvertices,
  ...
  mris->vertices_topology =
      (VERTEX_TOPOLOGY *)calloc(mris->nvertices, sizeof(VERTEX_TOPOLOGY));
...
static bool possiblyIntersectingGreatArcs_callback(void *void_ctx, int key,
                                                   bool *isHit) {
...
void test(int nvertices, int useDuplicates) {
  ...
  int fBenefitCount = 0, fBenefitLimit = 1, fNoBenefitCount = 0,
      fHasBenefitCount = 0;
  ...
  while (i + 2 < nvertices) {
    ...
    while (i < nvertices && fabs(center_x - ctx_x[vnos[i]]) < delta_x)
      i++;
    ...
    while (j < i) {
      ...
      while (j < i && fabs(center_y - ctx_y[vnos[j]]) < delta_y)
        j++;
      ...
      while (k < j) {
        ...
        while (k < j && fabs(center_z - ctx_z[vnos[k]]) < delta_z)
          k++;
  ...
  if (1) {
    ...
    if (1) {
      ...
      //
      {
        ...
        if (size != 1)
          ...
          fprintf(stderr, "same lines wrong key\n");
        else
    ...
    if (1) {
      ...
      // Lookup a variety of lines and make sure that at least the right ones
      // are found and not too many others
      //
      for (x = 5; x < 100; x += 5)
        for (y = 5; y < 100; y += 5) {
          PossiblyIntersectingGreatArcs_callback_context context;
          ...
          possiblyIntersectingGreatArcs(
              gas, &context, possiblyIntersectingGreatArcs_callback, 0, 1,
              x - 0.5, y - 0.5, 1000.0, x + x % 5, y + y % 3, 1000.0, true);
  ...
  if (0) {
    ...
    float yfLo, yfHi;
    float zfLo, zfHi;
    for (xfLo = -0.1; xfLo <= 1.2;
         xfLo += 0.1) // check also when the realm exceeds the original bounds
          ...
          for (yfHi = -0.1; yfHi <= 1.2; yfHi += 0.1)
            for (zfLo = -0.1; zfLo <= 1.2; zfLo += 0.1)
              for (zfHi = -0.1; zfHi <= 1.2; zfHi += 0.1) {
                ...
                Realm *realm =
                    makeRealm(realmTree, xLo, xHi, yLo, yHi, zLo, zHi);
                ...
                for (;;) {
                  ...
                  if (false && (counter == 1 || counter == 122)) {
                    fprintf(stdout, "counter:%d ri.i:%ld ri.p:%p\n", counter,
                            realmIterator.i, realmIterator.p);
                  }
#endif
                  ...
                  if (counter == 0 || states[vno]) {
                    fprintf(stdout,
                            "ERROR, vno:%d reported again when counter:%d, was "
                            "reported counter:%d\n",
                ...
                for (vno = 0; vno < mris->nvertices; vno++) {
                  if (states[vno] > 1)
                    if (states[vno] == 0) {
                      VERTEX *v = &mris->vertices[vno];
                      if (xLo <= v->cx && v->cx < xHi && yLo <= v->cy &&
                          v->cy < yHi && zLo <= v->cz && v->cz < zHi)
                        fprintf(stdout, "ERROR, vno:%d was not reported\n",
                                vno);
                    }
                }
                ...
                for (fno = 0; fno < mris->nfaces; fno++) {
                  ...
                  for (vi = 0; vi < VERTICES_PER_FACE; vi++) {
                    fxLo = MIN(fxLo, vertex->cx);
                    fxHi = MAX(fxHi, vertex->cx);
                    fyLo = MIN(fyLo, vertex->cy);
                    fyHi = MAX(fyHi, vertex->cy);
                    fzLo = MIN(fzLo, vertex->cz);
                    fzHi = MAX(fzHi, vertex->cz);
                  }
                  bool wontIntersect = fxHi < xLo || xHi <= fxLo ||
                                       fyHi < yLo || yHi <= fyLo ||
                                       fzHi < zLo || zHi <= fzLo;
                  if (wontIntersect)
                    continue; // might or might not be in the list
                  while (fnosI < fnosSize && fnos[fnosI] < fno)
                    fnosI++; // skip the ones that were reported but need not be
                  if (fnosI == fnosSize || fnos[fnosI] != fno) {
                    fprintf(stdout, "ERROR, fno:%d was not reported\n", fno);
                  }
                }

                // We are only interested in the benefits when the realm is much
                // smaller than the volume
                //
                if (mris->nfaces > 0 && (xHi - xLo) < (xMax - xMin) / 4 &&
                    (yHi - yLo) < (yMax - yMin) / 4 &&
                    (zHi - zLo) < (zMax - zMin) / 4) {

                  if (fnosSize * 3 > mris->nfaces * 2)
                    fNoBenefitCount++;
                  else
                    fHasBenefitCount++;

                  if (++fBenefitCount == fBenefitLimit) {
                    if (fBenefitLimit < 1000)
                      fBenefitLimit *= 2;
                    else
                      fBenefitLimit += 1000;
                    fprintf(stdout,
                            "fnosSize:%d mris->nfaces:%d fNoBenefitCount:%d "
                            "fHasBenefitCount:%d\n",
                            fnosSize, mris->nfaces, fNoBenefitCount,
                            fHasBenefitCount);
                  }
                }
...
#define vnosBuffSize                                                           \
  ...
  union {
    ...
    struct {
      int vnosSize; // above 2 assumes these are the same as the vnosBuff
                    // elements
      int vnosCapacity;
  ...
  int firstFnoPlus1; // the first of a list of faces for which this node is the
                     // deepest node they fully fit within
};
...
static void destroyRealmTreeNode(RealmTreeNode *n) {
  if (!n->vnos) {
    ...
    for (c = 0; c < childrenSize; c++) {
      RealmTreeNode *child = n->childIfPresent[c];
      n->childIfPresent[c] = nullptr;
      if (!child)
        continue;
...
static const int maxDepth =
    //
    // The maxDepth that can be reached when there are many (x,y,z) VERY close
    // to each other and a few a long way away
    //
    (sizeof(((RealmIterator *)nullptr)->i) * 8 // available 64
     - maxVnosSizeLog2 // needed to index the leaf nodes   20, leaving 44
     - 1               // needed to mark top of search      1, leaving 43
     ) /
    childrenSizeLog2; // bits needed per non-leaf level   43/3 = 14 - more than
                      // enough

static bool nodeContains(RealmTreeNode const *const n, float const x,
                         float const y, float const z) {
  return n->xLo <= x && x < n->xHi && n->yLo <= y && y < n->yHi &&
         n->zLo <= z && z < n->zHi;
}
...
static int chooseChild(RealmTreeNode const *n, float x, float y, float z) {
...
static void widenSubtree_wkr(RealmTreeNode *n, float xLo, float xHi, float yLo,
                             float yHi, float zLo, float zHi,
                             unsigned widen_Mask) {
  if (!n || (widen_Mask == 0))
    return;
  //
  if (widen_Mask & Widen_xLo) {
    n->xLo = xLo;
  }
  if (widen_Mask & Widen_xHi) {
    n->xHi = xHi;
  }
  if (widen_Mask & Widen_yLo) {
    n->yLo = yLo;
  }
  if (widen_Mask & Widen_yHi) {
    n->yHi = yHi;
  }
  if (widen_Mask & Widen_zLo) {
    n->zLo = zLo;
  }
  if (widen_Mask & Widen_zHi) {
    n->zHi = zHi;
  }
  ...
  //
  widenSubtree_wkr(n->childIfPresent[0], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xHi | Widen_yHi | Widen_zHi));
  widenSubtree_wkr(n->childIfPresent[1], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xLo | Widen_yHi | Widen_zHi));
  widenSubtree_wkr(n->childIfPresent[2], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xHi | Widen_yLo | Widen_zHi));
  widenSubtree_wkr(n->childIfPresent[3], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xLo | Widen_yLo | Widen_zHi));
  widenSubtree_wkr(n->childIfPresent[4], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xHi | Widen_yHi | Widen_zLo));
  widenSubtree_wkr(n->childIfPresent[5], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xLo | Widen_yHi | Widen_zLo));
  widenSubtree_wkr(n->childIfPresent[6], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xHi | Widen_yLo | Widen_zLo));
  widenSubtree_wkr(n->childIfPresent[7], xLo, xHi, yLo, yHi, zLo, zHi,
                   widen_Mask & ~(Widen_xLo | Widen_yLo | Widen_zLo));
}
...
static void widenSubtree(RealmTree *realmTree, float xLo, float xHi, float yLo,
  ...
  if (xHi > n->xHi)
    widen_Mask |= Widen_xHi;
  if (yLo < n->yLo)
    widen_Mask |= Widen_yLo;
  if (yHi > n->yHi)
    widen_Mask |= Widen_yHi;
  if (zLo < n->zLo)
    widen_Mask |= Widen_zLo;
  if (zHi > n->zHi)
    widen_Mask |= Widen_zHi;
...
#pragma omp critical
  {
    ...
    if (!child) {
      ...
      //
      if (c & 1)
        child->xLo = n->xMid, child->xHi = n->xHi;
      else
        child->xLo = n->xLo, child->xHi = n->xMid;
      if (c & 2)
        child->yLo = n->yMid, child->yHi = n->yHi;
      else
        child->yLo = n->yLo, child->yHi = n->yMid;
      if (c & 4)
        child->zLo = n->zMid, child->zHi = n->zHi;
      else
        child->zLo = n->zLo, child->zHi = n->zMid;

      // Set its splitting values, since there are no grandchildren to store
      // them in
      //
...
static RealmTreeNode *insertIntoChild(RealmTree *realmTree, RealmTreeNode *n,
                                      int vno) {
...
static RealmTreeNode *insertVnoIntoNode(RealmTree *const realmTree,
  ...
  if (n->vnos) {
    ...
    //
    int const vnosSize = n->vnosSize; // n->vnosSize and n->vnosBuf will get
...
static void removeVnoFromRealmTree(RealmTree *realmTree, int vno) {
  RealmTreeNode *n =
      realmTree->vnoToRealmTreeNode[chkBnd(0, vno, realmTree->saved_nvertices)];
  realmTree->vnoToRealmTreeNode[vno] = nullptr;
  {
    ...
    //
    for (; vi + 1 < n->vnosSize; vi++) {
      n->vnos[vi] = n->vnos[vi + 1];
    } // remove it
    n->vnosSize--;
  }
}

static RealmTreeNode *insertVnoNear(RealmTree *realmTree, RealmTreeNode *n,
                                    int vno) {
  chkBnd(0, vno, realmTree->saved_nvertices);
...
static int countXYZChanges(RealmTree const *realmTree, MRIS const *mris,
...
void freeRealmTree(RealmTree **realmTreePtr) {
  RealmTree *rt = *realmTreePtr;
  *realmTreePtr = nullptr;
  if (!rt)
    return;
...
static bool isFnoInRealmTreeNode(RealmTree *realmTree, int fno) {
  RealmTreeNode *n =
      realmTree->fnoToRealmTreeNode[chkBnd(0, fno, realmTree->saved_nfaces)];

  int prevFno = -1; // this list is usually very small
  int entryFno =
      n->firstFnoPlus1 - 1; // should this search be a performance problem
  while (entryFno >= 0 && entryFno != fno) { //      change to a per-node btree
    prevFno = entryFno;
    entryFno =
        realmTree->nextFnoPlus1[chkBnd(0, prevFno, realmTree->saved_nfaces)] -
        1;
  }
  return (entryFno == fno);
}

static void insertFnoIntoRealmTreeNode(RealmTree *realmTree, RealmTreeNode *n,
                                       int fno) {
...
static void removeFnoFromRealmTree(RealmTree *realmTree, int fno) {
  ...
  realmTree->fnoToRealmTreeNode[fno] = nullptr;
  if (prevFno < 0) {
  ...
  if (fno == interestingFno) {
    ...
    fprintf(stdout, "%s:%d interestingFno removed, count:%ld\n", __FILE__,
            __LINE__, count);
    realmTree->interestingRealmTreeNode = nullptr;
...
static void resizeRealmTree(RealmTree *rt, MRIS const *mris) {
...

  {
    ...
    if (change > 0) {
      rt->captured_VERTEX_xyz = (Captured_VERTEX_xyz *)realloc(
          rt->captured_VERTEX_xyz,
          mris->nvertices * sizeof(Captured_VERTEX_xyz));
      rt->vnoToRealmTreeNode = (RealmTreeNode **)realloc(
          rt->vnoToRealmTreeNode, mris->nvertices * sizeof(RealmTreeNode *));
      rt->nextVnoToUpdatePlus1 = (int *)realloc(rt->nextVnoToUpdatePlus1,
                                                mris->nvertices * sizeof(int));

      bzero(rt->captured_VERTEX_xyz + rt->saved_nvertices,
            change * sizeof(Captured_VERTEX_xyz));
      bzero(rt->vnoToRealmTreeNode + rt->saved_nvertices,
            change * sizeof(RealmTreeNode *));
      bzero(rt->nextVnoToUpdatePlus1 + rt->saved_nvertices,
            change * sizeof(int));
    } else {
      // remove these from the tree
      int vno;
      for (vno = mris->nvertices; vno < rt->saved_nvertices; vno++) {
        RealmTreeNode *const n = rt->vnoToRealmTreeNode[vno];
        if (!n)
          continue;
...

  {
    ...
    if (change > 0) {
      rt->fnoToRealmTreeNode = (RealmTreeNode **)realloc(
          rt->fnoToRealmTreeNode, mris->nfaces * sizeof(RealmTreeNode *));
      rt->nextFnoPlus1 =
          (int *)realloc(rt->nextFnoPlus1, mris->nfaces * sizeof(int));
      rt->nextFnoToUpdatePlus1 =
          (int *)realloc(rt->nextFnoToUpdatePlus1, mris->nfaces * sizeof(int));

      bzero(rt->fnoToRealmTreeNode + rt->saved_nfaces,
            change * sizeof(RealmTreeNode *));
      ...
      for (fno = mris->nfaces; fno < rt->saved_nfaces; fno++) {
        RealmTreeNode *const n = rt->fnoToRealmTreeNode[fno];
        if (!n)
          continue;
...
RealmTree *makeRealmTree(MRIS const *mris, GetXYZ_FunctionType getXYZ) {
  ...
  constructRealmTreeNode(&rt->root, nullptr);
  ...
  //
  // Since contains is xLo <= x < xHi etc. the bounds need to be slightly wider
  // than Hi so that the Hi is in a Node
  //
  xHi = widenHi(xHi);
  yHi = widenHi(yHi);
  zHi = widenHi(zHi);
  ...
  int fno;
  for (fno = 0; fno < mris->nfaces; fno++) {
    insertFnoIntoRealmTreeNode(rt, chooseRealmTreeNodeForFno(mris, rt, fno),
                               fno);
  }
...
void noteIfXYZChangedRealmTree(RealmTree *realmTree, MRIS const *mris,
                               GetXYZ_FunctionType getXYZ, int vno) {
  VERTEX const *vertex = &mris->vertices[vno];
  ...
  //
  if (realmTree->nextVnoToUpdatePlus1[vno] ==
      0) { // make sure not already in list
    realmTree->nextVnoToUpdatePlus1[vno] =
        realmTree->firstVnoToUpdatePlus1; // append existing list to this node
    realmTree->firstVnoToUpdatePlus1 =
        vno + 1; // make this vno the start of the list
  }
}

static int addFnoFaceSet(int firstFnoToUpdatePlus1, RealmTree *realmTree,
                         int fno) {
...
void updateRealmTree(RealmTree *realmTree, MRIS const *mris,
                     GetXYZ_FunctionType getXYZ) {
  ...
  if (previous_saved_nfaces <= interestingFno &&
      interestingFno < realmTree->saved_nfaces) {
    fprintf(stdout, "%s:%d interestingFno:%d should be added soon\n", __FILE__,
            __LINE__, interestingFno);
  }
  ...
  // add the new vertices.  recentNode tries to speed up by assuming some
  // locality.
  //
  ...
  for (vno = previous_saved_nvertices; vno < mris->nvertices; vno++) {
    ...
    firstFnoToUpdatePlus1 =
        addFacesToFaceSet(firstFnoToUpdatePlus1, realmTree, mris, vno);
  }
  ...
  for (fno = previous_saved_nfaces; fno < mris->nfaces; fno++) {
    firstFnoToUpdatePlus1 =
        addFnoFaceSet(firstFnoToUpdatePlus1, realmTree, fno);
  }
  ...
  while (vno >= 0) {
    VERTEX const *const vertex =
        &mris->vertices[chkBnd(0, vno, mris->nvertices)];
  ...
  while (fno >= 0) {
    ...
    RealmTreeNode *chosenForFno =
        chooseRealmTreeNodeForFno(mris, realmTree, fno);
...
void getRealmTreeBnds(RealmTree *realmTree, float *xLo, float *xHi, float *yLo,
                      float *yHi, float *zLo, float *zHi) {
...
Realm *makeRealm(RealmTree const *realmTree, float xLo, float xHi, float yLo,
  ...
  // so a realm - which represents the vertices and faces and not some
  // unoccupied space
  // - can be shrunk to these bounds, and needs to be if the
  // deepestContainingNode is to be set correctly, otherwise a realm whose
  ...
  if (xHi <= xLo || yHi <= yLo || zHi <= zLo)
    return r; // r->deepestContainingNode left NULL

  // == on the high bound is permissible, since points on that border are not in
  // the realm
  //
  RealmTreeNode const *n =
      deepestContainingNode(&realmTree->root, xLo, yLo, zLo);
  while (xHi > n->xHi || yHi > n->yHi || zHi > n->zHi)
    n = n->parent;
...
static bool nodeIntersectsRealm(RealmTreeNode const *c, Realm *r) {
  if (c->xHi <= r->xLo || r->xHi <= c->xLo)
    return false;
  if (c->yHi <= r->yLo || r->yHi <= c->yLo)
    return false;
  if (c->zHi <= r->zLo || r->zHi <= c->zLo)
    return false;
  return true;
}
...
GoDown:;
  ...
  for (;;) {
    ...
    if (!child->vnos ||
        child->vnosSize) // the child is a non-leaf or a leaf with children
      if (nodeIntersectsRealm(child,
                              realm)) // and it might contribute to this realm
...
void initRealmIterator(RealmIterator *realmIterator, Realm *realm) {
  ...
  // It has to keep track of how far below the deepestContainingNode it is, and
  // which of the children it is processing
  //
  // When i becomes 1, it is because we have popped the
  // realm->deepestContainingNode
  //
  RealmTreeNode const *n = realm->deepestContainingNode;
  if (!n) {
    realmIterator->i = 1;
    realmIterator->p = nullptr;
    return;
  }
  ...
  //
  if (n->vnosSize == 0 || !nodeIntersectsRealm(n, realm))
...
static int numberOffnosHereAndDeeper(RealmTreeNode const *n, Realm *realm) {
  if (!n || !nodeIntersectsRealm(n, realm))
    return 0;
  int count = n->nFaces;
  if (!n->vnos) {
    int c;
    for (c = 0; c < childrenSize; c++)
      count += numberOffnosHereAndDeeper(n->childIfPresent[c], realm);
  }
  return count;
}
...
static int fnosHere(RealmTree const *rt, RealmTreeNode const *n, int *fnos,
                    int fnosCapacity, int fnosSize) {
...
static int fnosHereAndDeeper(RealmTree const *rt, Realm *realm,
                             RealmTreeNode const *n, int *fnos,
                             int fnosCapacity, int fnosSize) {
  if (!n || !nodeIntersectsRealm(n, realm))
    return fnosSize;
  ...
  if (!n->vnos) {
    int c;
    for (c = 0; c < childrenSize; c++)
      fnosSize = fnosHereAndDeeper(rt, realm, n->childIfPresent[c], fnos,
                                   fnosCapacity, fnosSize);
  }
  return fnosSize;
}
...
int realmMightTouchFno(Realm *realm, int *fnos, int fnosCapacity) {
  ...
  if (!n)
    return 0;
  int written =
      fnosHereAndDeeper(realm->realmTree, realm, n, fnos, fnosCapacity, 0);
  while ((n = n->parent))
    written = fnosHere(realm->realmTree, n, fnos, fnosCapacity, written);
  return written;
}
...
static void summarizeRealmTreeNode(RealmTree const *realmTree,
  ...
  fprintf(stdout, "x:%f..%f y:%f..%f z:%f..:%f nFaces:%d\n", n->xLo, n->xHi,
          n->yLo, n->yHi, n->zLo, n->zHi, n->nFaces);
  if (n->vnos) {
    summarizeRealmTreeNodeIndent(n);
...
static int summarizeRealmTreeSubtree(RealmTree const *realmTree,
  ...
  if (n) {
    ...
    if (n->vnos) {
      ...
      int c;
      for (c = 0; c < childrenSize; c++)
        hasUnreachedChildren |= summarizeRealmTreeSubtree(
            realmTree, n->childIfPresent[c], targetDepth);
    } else {
...
void summarizeRealmTree(RealmTree const *realmTree) {
  int depth;
  for (depth = 0;; depth++)
    if (!summarizeRealmTreeSubtree(realmTree, &realmTree->root, depth))
      break;
}
...
static void finiCell(Cell *cell) { freeAndNULL(cell->indexs); }

typedef struct Pair { // an entry in a chain off the GreatArcSet pairHeadsPlus1
                      // hash table
...
struct GreatArcSet {
  ...
  int capacity, size, // the list of great arcs
      size_dispersed; // the size of the tail of the great arcs not yet in the
                      // cells
  int *keys;   // mrisurf gives a key to each great arc, to id the arc in the
               // callback
  ...
  int *passed; // used during one callback to note those passed to de-duplicate
               ...
               // This makes it possible to remove all elements from the set by
               // simply incrementing the passedClock
  ...
  float minH, minW, scaleH,
      scaleW; // used to project the h,w into the cell grid
...
static Cell *getCell(GreatArcSet *set, int w, int h) {
  ...
  return &set->cells[chkBnd(0, w, GRID_WIDTH) * GRID_HEIGHT +
                     chkBnd(0, h, GRID_HEIGHT)];
}

static void greatArcSet_project(GreatArcSet *set, float x, float y, float z,
                                float *w, float *h, bool *universal_cell,
                                bool trace) {

  // Project into the coordinate space invented for the defect
  //
  float px = set->cx * x + set->cy * y + set->cz * z,
  ...
  if (px <= 0.1) { // This could be zero, but making it 0.1 reduce the range of
                   // the w h
...
static void greatArcSet_getCellCoords(GreatArcSet *set, float x, float y,
                                      float z, float *wp, float *hp, int *p_wI,
                                      int *p_hI, bool *universal_cell,
                                      bool trace) {
  ...
  if (trace) {
    fprintf(stdout, "%s:%d wI:%d = (w:%g - minW:%g)*scaleW:%g\n", __FILE__,
            __LINE__, wI, w, set->minW, set->scaleW);
    fprintf(stdout, "%s:%d hI:%d = (w:%g - minH:%g)*scaleH:%g\n", __FILE__,
            __LINE__, hI, h, set->minH, set->scaleH);
  }

  if (*universal_cell || hI < 0 || GRID_HEIGHT <= hI || wI < 0 ||
      GRID_WIDTH <= wI) {
    if (false) {
      ...
      if (aFewTimes < 100) {
        aFewTimes++;
        fprintf(stdout,
                "Large defect found - a curiousity, not a problem - "
                "*universal_cell:%d\n",
                *universal_cell);
      }
    }
...
static void greatArcSet_cellInsert(GreatArcSet *set, Cell *cell, int index) {
  if (cell->size == cell->capacity)
    growCapacityAndInts(&cell->indexs, &cell->capacity, 16);
...
static void growGreatArcBuffer(GreatArcSet *set) {
  ...
  set->intersectionSupport = (IntersectionSupport *)realloc(
      set->intersectionSupport,
      set->capacity * sizeof(IntersectionSupport)); // don't need zero'ing
}

static bool isInPairs(int *p_pairHeadsIndex, GreatArcSet *set, int vno0,
                      int vno1) {
  int pairHeadsIndex = *p_pairHeadsIndex =
      chkBnd(0, ((vno0 * 12797) ^ (vno1 * 97127)) & PAIRS_HEADS_MASK,
             PAIRS_HEADS_SIZE);
...
void insertGreatArc(GreatArcSet *set, int key, int vno0, int vno1) {
  ...
  //
  int pairHeadsIndex;
  if (isInPairs(&pairHeadsIndex, set, vno0, vno1))
...
static void decideProjection(GreatArcSet *set) {
  ...
  float sumX = 0, sumY = 0, sumZ = 0;
  {
    ...
    for (index = 0; index < set->size; index++) {
      VERTEX const *v0 =
          &set->mris
               ->vertices[chkBnd(0, set->loVnos[index], set->mris->nvertices)];
      VERTEX const *v1 =
          &set->mris
               ->vertices[chkBnd(0, set->hiVnos[index], set->mris->nvertices)];
      sumX += v0->cx + v1->cx;
      sumY += v0->cy + v1->cy;
      sumZ += v0->cz + v1->cz;
    }
  ...
  // Compute the plane through the points that is going to hold the projection
  // of the defect for search purposes It is close enough to the sphere for
  // small defects that it can guide the search for intersecting lines.
  //
  // This is the center of the points, aka the vector from the origin to the
  // center, so it is the normal to the plane that is tangent to the surface
  // where the defect is centered. Make it the unit vector to use as the axis.
  //
  float cx = sumX, cy = sumY, cz = sumZ;
  {
    float clen = sqrt(cx * cx + cy * cy + cz * cz);
    if (clen == 0.0)
      clen = 1.0;
    clen = 1.0 / clen;
    cx *= clen;
  ...
  //
  float ax, ay,
      az; // construct a perpendicular to cx,cy,cz [make the dot product == 0.0]
  ...
  float // cross product to get the third axis
      bx = ay * cz -
           az * cy, // it will be a unit vector, but just in case tidy it up..
      by = az * cx - ax * cz, bz = ax * cy - ay * cx;
  {
  ...
  //
  float const vergeH = (maxH - minH) / 4;
  minH -= vergeH;
  maxH += vergeH;
  float const vergeW = (maxW - minW) / 4;
  minW -= vergeW;
  maxW += vergeW;
...
static void disperseGreatArcsIntoCells(GreatArcSet *set) {
  if (set->size_dispersed == set->size)
    return;

  if (set->size_dispersed == 0)
    decideProjection(set);
  ...
  for (index = set->size_dispersed; index < set->size; index++) {
    ...
    VERTEX const *v0 =
        &set->mris->vertices[chkBnd(0, vno0, set->mris->nvertices)];
    VERTEX const *v1 =
    ...
    greatArcSet_getCellCoords(set, v1->cx, v1->cy, v1->cz, &w1, &h1, &wI1, &hI1,
    ...
    if (trace) {
      fprintf(
          stdout,
          "%s:%d v0:%d v1:%d dispersed to cell coords (%d..%d, %d..%d) %s\n",
          __FILE__, __LINE__, vno0, vno1, wI0, wI1, hI0, hI1,
          (universal_cell0 || universal_cell1) ? "universal_cell" : "");
      fprintf(stdout, " v0 at (%g,%g,%g) v1 at (%g,%g,%g)\n", v0->cx, v0->cy,
              v0->cz, v1->cx, v1->cy, v1->cz);
    }
    ...
    // There are many ways to do this, but most edges are within a cell or
    // adjacent cells so it is not worth being clever
    //
...
static bool possiblyIntersectingCell(
    GreatArcSet *set, void *callbackCtx,
    bool (*callback)(void *callbackCtx, int key,
                     bool *isHit), // returns true if should keep going, false
                                   // if no more needed
    Cell *cell, bool tracing, int *pCallBackCount, int *pCallBackFirstFound,
    bool doFastIntersectionTest, int vno2, int vno3, float w2, float h2,
    float w3, float h3) {
  ...
  for (i = 0; i < cell->size; i++) {
    ...
    //
    if (set->passed[index] == set->passedClock)
      continue; // in the set, hence this is a duplicate
    set->passed[index] = set->passedClock; // add to the set

    // If shared vertex, don't intersect
    //
    bool const sharedVertex =
        set->loVnos[index] == vno2 || set->loVnos[index] == vno3 ||
        set->hiVnos[index] == vno2 || set->hiVnos[index] == vno3;
    ...
    if (sharedVertex) {
      ...
      //
      approachOneDone = approachTwoDone = true;
    }

    if (!sharedVertex && doFastIntersectionTest) {

      // The new approaches depend on the defect and the arc being tested
      ...
      // projected to be the straight line segment set->lineSegments[index].w0
      // ..,
      //                                                                call
      //                                                                these
      //                                                                instead
      ...
      // Approach One:  Solve the simultaneous equations - doesn't work if line
      // segments parallel, including being part of the same line which shows up
      // as the determinant of the matrix being almost zero
      //
      ...
      //
      // The only problem is when the two lines segments are segments of the
      // same line.  The following code then contains a multiply by zero, and
      // thus the comparisons get the wrong answer.  To avoid this, a second
      // approach is used when the lines are almost parallel
      //
      // ie. when w0 + p(w1-w0) = w2 + q(w3-w2)
      //      and h0 + p(h1-h0) = h2 + q(h3-h2)
      // which is an easily solved pair of simultaneous linear equations.
      //
      // If the solution's p and q are both between 0 and 1, the line segments
      // intersect!
      //
      // [ w1-w0  w2-w3 ] [ p ] = [ w2-w0 ]
      // [ h1-h0  h2-h3 ] [ q ]   [ h2-h0 ]
      ...
      // [   c      d   ] [ q ]   [   f   ]
      //
      float const a = is->w1MinusW0, b = w2 - w3, c = is->h1MinusH0,
                  d = h2 - h3, e = w2 - is->w0, f = h2 - is->h0;
      ...
      //               [ (ad-bc) q ]
      //
      // So the 0..1 test turns into 0 <= (de-bf) / (ad-bc) <= 1      0 <=
      // (af-ce) / (ad-bc) <= 1
      //
      ...
      if (fabsf(adbc) > 1e-8) {
        ...
        //      the test turns into      0         <= (de-bf)             <=
        //      (ad-bc) i.e.                   -(ad-bc)/2  <= (de-bf) -
        //      (ad-bc)/2 <= (ad-bc)/2
        //
        ...
        //
        //     the test turns into      0         >= (de-bf)             >=
        //     (ad-bc) i.e.                   -(ad-bc)/2  >= (de-bf) - (ad-bc)/2
        //     >= (ad-bc)/2
        //
        // which can be coded as
        //              |(ab-bc)/2| >= |(de-bf) - (ab-bc)/2|
      ...
      // Approach Two:  Project into a coordinate system centered on (w0,h0) and
      // using (w1-w0,h1-h0) as one axis and its perpendicular as the other.
      //
      if (!approachOneDone || comparingAnswers) {
        ...
        float const
            // w0d = 0,                    h0d = 0,                // Recenter
            // on (w0,h0)
            w1d = is->w1MinusW0,
            h1d = is->h1MinusH0, w2d = w2 - w0, h2d = h2 - h0, w3d = w3 - w0,
            h3d = h3 - h0;
        ...
        float const // Project the four points
            p0w = /* pww*w0d + pwh*h0d */ 0,
            p0h = /* phw*w0d + phh*h0d */ 0, p1w = pww * w1d + pwh * h1d,
            p1h = /* phw*w1d + phh*h1d */ 0;

        float p2w = pww * w2d + pwh * h2d, p2h = phw * w2d + phh * h2d,
              p3w = pww * w3d + pwh * h3d, p3h = phw * w3d + phh * h3d;

        // Note that p1w =  (w1-w0)*(w1-w0) + (h1-h0)*(h1-h0) = sum of two
        // squared
        ...
        if (p2w < 0 &&
            (p2h * p3w >
             p3h * p2w)) { //     p2h/p2w < p3h/p3w   with the divides removed
          goto ApproachTwoDone;
        }
    ...
    // If both new approaches tried, they should give the same answer except for
    // some really rare cases...
    //
    //      Approach one, that works except when the line segments are almost
    //      parallel Approach two, that is slower than approach one, but always
    //      works
    //
    if (comparingAnswers && approachOneDone && approachTwoDone &&
        approachOneAnswer != approachTwoAnswer) {

      fprintf(stdout, "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
                      "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
      fprintf(stdout, "%s:%d approachOneAnswer:%d approachTwoAnswer:%d\n",
              __FILE__, __LINE__, approachOneAnswer, approachTwoAnswer);
      fprintf(stdout, "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
                      "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
    }

    bool const answerIsFound = approachOneDone || approachTwoDone;
    bool const answerSoFar =
        approachOneDone ? approachOneAnswer : approachTwoAnswer;
    ...
    if (comparingAnswers || !answerIsFound || answerSoFar) {
      ...
      if (!answerIsFound ||
          answerSoFar) { // These are the circumstances where it MUST be called
        approachOldCount++;
        (*pCallBackCount)++;
      }
    ...
    //
    if (false) {
      static long hit, // only the old code tried
          hitHit,      // both tried
          hitMiss, missHit, missMiss,
          miss, // only the old code tried
          count, limit = 1;
      //
      //  count:6.71089e+07 hitHit:3.80079e+06 hitMiss:4 missHit:0
      //  missMiss:6.33081e+07
      //
      count++; // 6.7e7

      if (!answerIsFound) {
        // only the old code tried
        if (approachOldAnswer)
          hit++;
        else
          miss++;
      } else {
        ...
        if (approachOldAnswer && answerSoFar) {
          ...
          hitMiss++; //   4      new code will get a different answer, which may
                     //   be bad
          fprintf(stdout,
                  "%s:%d hit when predicted miss %ld "
                  "<######################################################\n",
                  __FILE__, __LINE__, hitMiss);
      ...
      if (count == limit) {
        limit *= 2;
        fprintf(stdout,
                "%s:%d count:%g hit:%g miss:%g hitHit:%g hitMiss:%g missHit:%g "
                "missMiss:%g\n"
                "  a1:%g a2:%g old:%g\n",
                __FILE__, __LINE__, (float)count, (float)hit, (float)miss,
                (float)hitHit, (float)hitMiss, (float)missHit, (float)missMiss,
                (float)approachOneCount, (float)approachTwoCount,
                (float)approachOldCount);
      }
    }

    if (definitiveAnswer) {
      if (!*pCallBackFirstFound)
        *pCallBackFirstFound = *pCallBackCount;
    }
    if (!keepGoing) {
      return true;
    }
...
void possiblyIntersectingGreatArcs(
    ...
    bool (*callback)(void *callbackCtx, int key,
                     bool *isHit), // returns true if should keep going, false
                                   // if no more needed
    int vno0, int vno1, float x0, float y0,
    float z0, // ends of the line, need not be a unit vector
    float x1, float y1, float z1, bool tracing) {
  tracing = false;
  ...
  if (false) {
    ...
    if (count >= limit) {
      ...
      fprintf(stdout, "%s:%d target arcs - count:%g universalCount:%g\n",
              __FILE__, __LINE__, (float)count, (float)universalCount);
    }
  }
  ...
  // There are lots of ways to do this, but most edges are within a cell or
  // adjacent cells so it is not worth being clever, however if there is more
  // than one cell, there is a risk of duplicates
  //
  //
  ...
  for (wI = wI0; wI <= wI1; wI++)
    for (hI = hI0; hI <= hI1; hI++) {
      ...
      if (possiblyIntersectingCell(set, callbackCtx, callback, cell, tracing,
                                   &callBackCount, &callBackFirstFound,
                                   !(universal_cell0 || universal_cell1), vno0,
                                   vno1, w0, h0, w1, h1))
        goto Found;
    }
  ...
  if (false) {
    ...
    if (count >= limit) {
      limit *= 2;
      fprintf(stdout,
              "%s:%d universal inserted arcs - count:%g localCount:%g "
              "universalCount:%g\n",
              __FILE__, __LINE__, (float)count, (float)localCount,
              (float)universalCount);
    }
  }

  if (possiblyIntersectingCell(set, callbackCtx, callback,
                               &set->cells[UNIVERSAL_CELL], tracing,
                               &callBackCount, &callBackFirstFound, false, vno0,
                               vno1, w0, h0, w1, h1))
    goto Found;
...
Found:
  ...
  //
  if (tracing) {
    fprintf(stderr, " callBackCount:%d, callBackFirstFound:%d total size:%d\n",
            callBackCount, callBackFirstFound, set->size);
  }
}

void possiblyIntersectingGreatArcs_Debug( // show how vno0..vno1 interacts with
                                          // the arc
    GreatArcSet *set, float x0, float y0,
    float z0, // ends of the arc, need not be a unit vector
    float x1, float y1, float z1, int vno0, int vno1) {
  sortIntSoFirstLo(&vno0, &vno1);
  ...
  for (target_index = 0; target_index < set->size; target_index++) {
    if (set->loVnos[target_index] == vno0 && set->hiVnos[target_index] == vno1)
      break;
  }
  ...
  fprintf(stderr, "Should be in (%d..%d,%d..%d) %s.  Found in ", wI0, wI1, hI0,
          hI1, (universal_cell0 || universal_cell1) ? "universal cell" : "");
  int w, h;
