...
const char *class_names[GAUSSIAN_NCLASSES] = {"CSF",          "GREY MATTER",
                                              "THIN STRANDS", "BORDER PIXELS",
                                              "WHITE MATTER", "BRIGHT MATTER"};
...
static int mricGetClassObservation(GET_INPUT_PARMS *parms, VECTOR *v_obs,
                                   int obs_no, int classnum);
...
MRIC *MRICalloc(int nrounds, int *types, int *features, void *parms) {
  ...
  for (round = 0; round < nrounds; round++) {
    ...
    if (ninputs < 1 || ninputs > MAX_INPUTS)
      ErrorReturn(NULL, (ERROR_BADPARM, "MRICalloc(%d): bad # of inputs %d",
                         round, ninputs));
...
int MRICretrain(MRIC *mric, char *file_name) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NO_FILE,
        (ERROR_NO_FILE, "MRICtrain(%s): could not open file", file_name));

  nfiles = 0;
  while ((cp = fgetl(line, 299, fp)) != nullptr)
    nfiles++;
...
MRIC *MRICread(char *fname) {
  ...
  if (sscanf(cp, "%d %s", &nrounds, prior_fname) < 1) {
    fclose(fp);
    ErrorReturn(NULL,
                (ERROR_BADFILE, "MRICread(%s): could not scan parms", fname));
  }
  for (round = 0; round < nrounds; round++) {
    cp = fgetl(line, 99, fp);
    if (sscanf(cp, "%d %d 0x%x", &type[round], &ninputs, &features[round]) <
        3) {
      fclose(fp);
      ErrorReturn(NULL,
                  (ERROR_BADFILE, "MRICread(%s): could not scan parms", fname));
  ...
  mric = MRICalloc(nrounds, type, features, nullptr);
  if (*prior_fname) {
    mric->mri_priors = MRIread(prior_fname);
...
MRI *MRICclassify(MRIC *mric, MRI *mri_src, MRI *mri_dst, float conf,
  ...
  VECTOR *v_inputs;
  GCLASSIFY *gc;
  int x, y, z, classno, nclasses, xt, yt, zt, type, round, x1, y1, z1, x0, c,
      bg, max_white_class, max_non_white_class;
  // int width, depth, height;
  BUFTYPE *psrc, src, *pdst, *pclasses;
  float prob, *pprobs = nullptr, total, min_output, fmin, fmax, white_prob,
    ...
    for (z = bounding_box.z; z <= z1; z++) {
      ...
      for (y = bounding_box.y; y <= y1; y++) {
        ...
        for (x = x0; x <= x1; x++) {
          src = *psrc++;
          if (mri_priors) {
            MRIvoxelToVoxel(mri_src, mri_priors, (double)x, (double)y,
                            (double)z, &xrt, &yrt, &zrt);
          ...
          MRICcomputeInputs(mric, mri_in, x, y, z, v_inputs,
                            mric->features[round]);
          ...
          case CLASSIFIER_RBF:
            ...
            if (!FZERO(total))
              prob = (RVECTOR_ELT(rbf->v_outputs, classno + 1) - min_output) /
                     total;
            else
              prob = 0.0f;
            break;
  ...
  if (Gdiag & DIAG_SHOW) {
    fprintf(stderr, "total = %d, buffered = %d, computed = %d\n", total_calls,
            buffered, total_computed);
    fprintf(stderr, "efficiency = %2.3f%%\n",
            100.0f *
                (float)(mri_src->width * mri_src->height * mri_src->depth) /
...
------------------------------------------------------*/
int MRICcomputeInputs(MRIC *mric, MRI *mri, int x, int y, int z,
                      VECTOR *v_inputs, int features) {
  static int old_features = 0;
  static MRI_REGION region = {0, 0, 0, 0, 0, 0};
  static MRI *mri_prev = nullptr, *mri_zscore3 = nullptr,
             *mri_zscore5 = nullptr, *mri_direction = nullptr,
             *mri_mean3 = nullptr, *mri_mean5 = nullptr, *mri_cpolv = nullptr,
             *mri_cpolv_mean3 = nullptr, *mri_cpolv_mean5 = nullptr,
             *mri_cpolv_median3 = nullptr, *mri_cpolv_median5 = nullptr,
             *mri_min3 = nullptr, *mri_min5 = nullptr, *mri_min7 = nullptr,
             *mri_cpolv_zscore5 = nullptr, *mri_cpolv_zscore7 = nullptr,
             *mri_cpolv_curv5 = nullptr, *mri_cpolv_curv7 = nullptr,
             *mri_cpolv_order = nullptr;
  ...
  if ((features & FEATURE_CPOLV) &&
      (!mri_cpolv || !MRImatch(mri, mri_cpolv) || (mri != mri_prev))) {
    if (mri_cpolv)
      MRIfree(&mri_cpolv);
    ...
    if (!mri_cpolv)
      mri_cpolv =
          MRIcentralPlaneOfLeastVarianceNormal(mri, nullptr, POLV_WSIZE);
    if (Gdiag & DIAG_WRITE)
  ...
  if (!mri_prev || mri_prev != mri || (old_features != features) ||
    ...
    MRI *mri_std, *mri_mean, *mri_region, *mri_grad, *mri_big = nullptr;
    MRI_REGION rbig;
    ...
    mri_region = MRIextractRegion(mri, nullptr, &rbig);
    x0 = region.x - rbig.x;
    y0 = region.y - rbig.y;
    z0 = region.z - rbig.z;
...
#define ORDER_THRESH 2
      mri_big = MRIpolvOrder(mri_region, nullptr, mri_cpolv, 5, ORDER_THRESH);
      mri_cpolv_order = MRIextract(mri_big, nullptr, x0, y0, z0, region.dx,
                                   region.dy, region.dz);
    ...
    if (features & FEATURE_CPOLV_ZSCORE5) {
      mri_big = MRIpolvZscore(mri_region, nullptr, mri_cpolv, 5);
      mri_cpolv_zscore5 = MRIextract(mri_big, nullptr, x0, y0, z0, region.dx,
                                     region.dy, region.dz);
      MRIfree(&mri_big);
    }

    if (features & FEATURE_CPOLV_ZSCORE7) {
      mri_big = MRIpolvZscore(mri_region, nullptr, mri_cpolv, 7);
      mri_cpolv_zscore7 = MRIextract(mri_big, nullptr, x0, y0, z0, region.dx,
                                     region.dy, region.dz);
      MRIfree(&mri_big);
    }

    if (features & FEATURE_CPOLV_CURV5) {
      mri_big = MRIpolvNormalCurvature(mri_region, nullptr, mri_cpolv, 5);
      mri_cpolv_curv5 = MRIextract(mri_big, nullptr, x0, y0, z0, region.dx,
                                   region.dy, region.dz);
      MRIfree(&mri_big);
    }

    if (features & FEATURE_CPOLV_CURV7) {
      mri_big = MRIpolvNormalCurvature(mri_region, nullptr, mri_cpolv, 7);
      mri_cpolv_curv7 = MRIextract(mri_big, nullptr, x0, y0, z0, region.dx,
                                   region.dy, region.dz);
      MRIfree(&mri_big);
    }
    ...
    if (features & FEATURE_ZSCORE3) {
      static int first = 1;
      mri_mean = MRImeanRegion(mri, nullptr, 3, &region);
      mri_std = MRIstdRegion(mri, nullptr, mri_mean, 3, &region);
      mri_zscore3 = MRIzScoreRegion(mri, nullptr, mri_mean, mri_std, &region);
    ...
    if (features & FEATURE_ZSCORE5) {
      static int first = 1;
      mri_mean = MRImeanRegion(mri, nullptr, 5, &region);
      mri_std = MRIstdRegion(mri, nullptr, mri_mean, 5, &region);
      mri_zscore5 = MRIzScoreRegion(mri, nullptr, mri_mean, mri_std, &region);
      if (Gdiag & DIAG_WRITE && first) {
        first = 0;
        MRIwrite(mri_mean, "mean.mnc");
        MRIwrite(mri_std, "std.mnc");
    ...
    if (features & FEATURE_DIRECTION) {
      ...
      mri_grad = MRIsobel(mri_region, nullptr, nullptr);
      mri_big = MRIdirectionMap(mri_grad, nullptr, 3);

      mri_direction = MRIextract(mri_big, nullptr, x0, y0, z0, region.dx,
                                 region.dy, region.dz);
      if ((Gdiag & DIAG_WRITE) && first) {
        first = 0;
    ...
    if (features & FEATURE_MEAN3)
      mri_mean3 = MRImeanRegion(mri, nullptr, 3, &region);
    if (features & FEATURE_MEAN5)
      mri_mean5 = MRImeanRegion(mri, nullptr, 5, &region);
    if (features & FEATURE_CPOLV_MEAN3)
      mri_cpolv_mean3 = MRIpolvMeanRegion(mri, nullptr, mri_cpolv, 3, &region);
    if (features & FEATURE_CPOLV_MEAN5)
      mri_cpolv_mean5 = MRIpolvMeanRegion(mri, nullptr, mri_cpolv, 5, &region);
    if (features & FEATURE_CPOLV_MEDIAN3)
      mri_cpolv_median3 =
          MRIpolvMedianRegion(mri, nullptr, mri_cpolv, 3, &region);
#if 1
    if (features & FEATURE_CPOLV_MEDIAN5) {
      static int first = 1;

      mri_cpolv_median5 =
          MRIpolvMedianRegion(mri, nullptr, mri_cpolv, 5, &region);
      if ((Gdiag & DIAG_WRITE) && first) {
        first = 0;
    ...
    if (features & FEATURE_MIN3)
      mri_min3 = MRIerodeRegion(mri, nullptr, 3, &region);
    if (features & FEATURE_MIN5)
      mri_min5 = MRIerodeRegion(mri, nullptr, 5, &region);
    if (features & FEATURE_MIN7)
      mri_min7 = MRIerodeRegion(mri, nullptr, 7, &region);
    if (features & FEATURE_MIN5) {
      static int first = 1;
  ...
  if (features & FEATURE_CPOLV_MEDIAN3)
    VECTOR_ELT(v_inputs, index++) =
        (float)MRIvox(mri_cpolv_median3, x0, y0, z0);
#if 1
  if (features & FEATURE_CPOLV_MEDIAN5)
    VECTOR_ELT(v_inputs, index++) =
        (float)MRIvox(mri_cpolv_median5, x0, y0, z0);
#else
  if (features & FEATURE_CPOLV_MEDIAN5)
  ...
  if ((features & FEATURE_PRIORS) && mric->mri_priors) {
    ...
    mri_priors = mric->mri_priors;
    MRIvoxelToVoxel(mri, mri_priors, (double)x, (double)y, (double)z, &xrt,
                    &yrt, &zrt);
    xt = mri_priors->xi[nint(xrt)];
    yt = mri_priors->yi[nint(yrt)];
    zt = mri_priors->zi[nint(zrt)];
...
MRI *MRICbuildTargetImage(MRI *mri_src, MRI *mri_target, MRI *mri_wm,
  ...
  mri_thin = MRIfindThinWMStrands(mri_wm, nullptr, 2);
  if (lo_lim <= 0)
    lo_lim = LO_LIM;
  if (hi_lim <= 0)
    hi_lim = HI_LIM;
  ...
  if (!mri_target)
    mri_target = MRIclone(mri_wm, nullptr);
...
MRI *MRICupdatePriors(MRI *mri_target, MRI *mri_priors, int scale) {
  ...
  if (!mri_priors) {
    ...
    mri_priors->linear_transform = mri_priors->inverse_linear_transform =
        nullptr;
  }
    ...
    for (y = 0; y < height; y++) {
      ...
      for (x = 0; x < width; x++) {
        ...
        MRIFseq_vox(mri_priors, xt, yt, zt, classnum) =
            MRIFseq_vox(mri_priors, xt, yt, zt, classnum) + 1.0f;
...
------------------------------------------------------*/
int MRICupdateStatistics(MRIC *mric, int round, MRI *mri_src, MRI *mri_wm,
                         MRI_REGION *box) {
  GCLASSIFY *gc;
  GCLASS *gcl;
  ...
  mri_target = MRICbuildTargetImage(mri_src, nullptr, mri_wm, LO_LIM, HI_LIM);
  v_inputs = VectorAlloc(mric->ninputs[round], MATRIX_REAL);
  ...
  for (z = box->z; z <= z1; z++) {
    ...
    for (y = box->y; y <= y1; y++) {
      ...
      for (x = box->x; x <= x1; x++) {
        ...
        MRICcomputeInputs(mric, mri_src, x, y, z, v_inputs,
                          mric->features[round]);
      ...
      for (row = 1; row <= gcl->m_covariance->rows; row++) {
        ...
        if (Gdiag & DIAG_SHOW)
          fprintf(stderr, "mean %24.24s, feature %d = %-2.3f, var = %-2.3f\n",
                  class_names[classno], row, mean_a,
...
int MRICdump(FILE *fp, MRIC *mric) {
  ...
  fprintf(stderr, "mric with %d round, prior file %s\n", mric->nrounds,
          mric->mri_priors ? mric->prior_fname : "(none)");
  for (round = 0; round < mric->nrounds; round++) {
    gc = mric->classifier[round].gc;
    nclasses = gc->nclasses;
    for (classno = 0; classno < nclasses; classno++) {
      ...
      for (fno = 0; fno < mric->ninputs[round]; fno++) {
        fprintf(stderr, "    feature %10.10s, mean = %+2.3f, std = %+2.3f\n",
                MRICfeatureName(mric, round, fno), gcl->m_u->rptr[fno + 1][1],
...
/*-----------------------------------------------------
        ...
        Description
          ...
          Gaussian classifier.
------------------------------------------------------*/
static int mricComputeGCStatistics(MRIC *mric, FILE *fp, int nfiles,
                                   int round) {
...
static int mricTrainRBF(MRIC *mric, FILE *fp, int nfiles, int round) {
  ...
  if (RBFtrain(mric->classifier[round].rbf, mricGetClassifierInput, &parms,
               0.0f) != NO_ERROR)
    return (Gerror);
...
static int mricRetrainRBF(MRIC *mric, FILE *fp, int nfiles, int round) {
  ...
  if (RBFretrain(mric->classifier[round].rbf, mricGetClassifierInput, &parms,
                 0.0f) != NO_ERROR)
    return (Gerror);
...
static int mricGetClassifierInput(VECTOR *v_inputs, int no, void *parm,
  ...
  MRIC *mric;
  FILE *fp;
  static MRI *mri_src = nullptr, *mri_target = nullptr;
...
// bin_offset = half_bins;
  ...
  if (!fp)
    ErrorReturn(ERROR_NO_FILE,
                (ERROR_NO_FILE, "MRICbuildScatterPlot(%s): could not open file",
                 training_file_name));
  ...
  mins = rbf->min_inputs;
  maxs = rbf->max_inputs;
  while (mricGetClassObservation(&parms, v_obs, obs_no++, classnum) ==
         NO_ERROR) {
...
------------------------------------------------------*/
static int mricGetClassObservation(GET_INPUT_PARMS *parms, VECTOR *v_obs,
                                   int desired_class_obs_no, int classnum) {
  ...
  do {
    ...
    if ((ret == NO_ERROR) && (classno == classnum))
      class_obs_no++;
