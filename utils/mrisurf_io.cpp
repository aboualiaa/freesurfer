...
static MRI_SURFACE *mrisReadTriangleFile(const char *fname,
                                         double nVFMultiplier);
...
int MRISwriteTriangleProperties(MRI_SURFACE *mris, const char *mris_fname) {
  ...
  if (!fp)
    ErrorReturn(ERROR_NO_FILE,
                (ERROR_NO_FILE,
                 "MRISwriteTriangleProperties: could not open %s", fname));
...
int MRISwriteCurvature(MRI_SURFACE *mris, const char *sname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "MRISwriteCurvature: could not open %s", fname));
...
int MRISwriteDists(MRI_SURFACE *mris, const char *sname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "MRISwriteDists: could not open %s", fname));
...
MRI *MRISwriteIntoVolume(MRI_SURFACE *mris, MRI *mri, int which) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case VERTEX_AREA:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "MRISwriteIntoVolume: unsupported type %d",
                which);
      break;
    }
    if (mri->width == mris->nvertices)
      ...
      ErrorReturn(NULL, (ERROR_BADPARM,
                         "MRISwriteIntoVolume: nvertices %d doesn't match with "
                         "in MRI %d x %d x %d x %d\n",
                         mris->nvertices, mri->width, mri->height, mri->depth,
                         mri->nframes));
    }
  }
  return (mri);
...
MRI_SURFACE *MRISreadFromVolume(MRI *mri, MRI_SURFACE *mris, int which) {
  ...
  if (mris->nvertices != mri->width)
    ErrorReturn(NULL, (ERROR_BADPARM,
                       "MRISreadFromVolume: nvertices %d doesn't match with in "
                       "MRI %d x %d x %d x %d\n",
                       mris->nvertices, mri->width, mri->height, mri->depth,
                       mri->nframes));
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case VERTEX_AREA:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "MRISreadFromVolume: unsupported type %d",
                which);
      break;
    }
  }
...
int MRISwriteAreaError(MRI_SURFACE *mris, const char *name) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "MRISwriteAreaError: could not open %s", fname));
...
int MRISwriteAngleError(MRI_SURFACE *mris, const char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISwriteAngleError: could not open %s", fname));
...
int MRISwriteValues(MRI_SURFACE *mris, const char *sname) {
  ...
  if (ftype != MRI_VOLUME_TYPE_UNKNOWN) {
    TempMRI = MRIcopyMRIS(nullptr, mris, 0, "val");
    if (TempMRI == nullptr) {
...
int MRISreadPatchNoRemove(MRI_SURFACE *mris, const char *pname) {
  ...
  } else if (type == MRIS_ASCII_TRIANGLE_FILE) /* .ASC */
  {
    ...
    if (!fp)
      ErrorReturn(
          ERROR_NOFILE,
          (ERROR_NOFILE, "MRISreadPatch(%s): could not open file", fname));
  ...
  /////////////////////////////////////////////////////////////////////////
    ...
    if (npts >= 0) {
      ...
      for (j = 0; j < npts; j++) {
        ...
        fread2(&iz, fp);
        // convert it to mm, i.e. change the vertex position
        MRISsetXYZ(mris, k, ix / 100.0, iy / 100.0, iz / 100.0);
        if (k == Gdiag_no && Gdiag & DIAG_SHOW)
          fprintf(stdout, "vertex %d read @ (%2.2f, %2.2f, %2.2f)\n", k,
                  mris->vertices[k].x, mris->vertices[k].y,
                  mris->vertices[k].z);
      }
    } else // new surface format
    {
      ...
      if (Gdiag & DIAG_SHOW)
        fprintf(stdout,
                "reading new surface format patch %s with %d vertices (%2.1f%% "
                "of total)\n",
                pname, npts, 100.0f * (float)npts / (float)mris->nvertices);
      // set all vertices ripflag to be true
      for (k = 0; k < mris->nvertices; k++) {
  ...
  for (k = 0; k < mris->nvertices; k++)
    if (mris->vertices_topology[k].num == 0 ||
        mris->vertices_topology[k].vnum == 0) {
...
int MRISwritePatch(MRI_SURFACE *mris, const char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISwritePatch: can't create file %s\n", fname));
  // write num points
...
/*-----------------------------------------------------
  ...
  name x y z
  ------------------------------------------------------*/
static int mrisLabelVertices(MRI_SURFACE *mris, float cx, float cy, float cz,
                             int label, float radius) {
  VERTEX *v;
  float xd, yd, zd, d;
  int vno;
...
int MRISreadTetherFile(MRI_SURFACE *mris, const char *fname, float radius) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISreadTetherFile(%s): could not open file", fname));

  /* first count # of labels in file */
  mris->nlabels = 0;
  while ((cp = fgetl(line, 199, fp)) != nullptr) {
    mris->nlabels++;
  }
  mris->labels =
  ...
  for (l = 0; l < mris->nlabels; l++) {
    ...
    mris->labels[l].cy = cy;
    mris->labels[l].cz = cz;
    mrisLabelVertices(mris, cx, cy, cz, l,
                      radius); // weirdly this is currently a no-op!
  }
...
int MRISreadAnnotation(MRI_SURFACE *mris, const char *sname) {
  ...
  if (!cp) /* no path - use same one as mris was read from */
  {
    ...
    need_hemi = stricmp(fname_no_path,
...
int MRISreadAnnotationIntoArray(const char *fname, int in_array_size,
  ...
  if (fname == nullptr || out_array == nullptr)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "Parameter was NULL."));
  if (in_array_size < 0)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "in_array_size was negative."));
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "could not read annot file %s", fname));
...
int MRISreadCTABFromAnnotationIfPresent(const char *fname,
  ...
  if (fname == nullptr || out_table == nullptr)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "Parameter was NULL."));
...
int MRISisCTABPresentInAnnotation(const char *fname, int *present) {
  ...
  if (fname == nullptr || present == nullptr)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "Parameter was NULL."));
...
int MRISwriteAnnotation(MRI_SURFACE *mris, const char *sname) {
  ...
  if (!cp) /* no path - use same one as mris was read from */
  {
    ...
    need_hemi = strncmp(fname_no_path,
                        mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh", 2);
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "could not write annot file %s", fname));
  fwriteInt(mris->nvertices, fp);
  for (vno = 0; vno < mris->nvertices; vno++) {
    if (vno == Gdiag_no) {
      DiagBreak();
...
int MRISreadValuesIntoArray(const char *sname, int in_array_size,
                            float **out_array) {
  ...
  if (sname == nullptr || out_array == nullptr)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "Parameter was NULL."));
  if (in_array_size < 0)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "in_array_size was negative."));
  ...
  if (fp == nullptr)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISreadValuesIntoArray: File %s not found\n", fname));
  ...
  if (fread3(&num, fp) < 1) {
    ...
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE,
                 "MRISreadValues(%s): could not read # of vertices", fname));
  }
  ...
  for (i = 0; i < num; i++) {
    ...
    if (fread3(&k, fp) < 1)
      ErrorReturn(ERROR_BADFILE,
                  (ERROR_BADFILE, "MRISreadValues(%s): could not read %dth vno",
                   fname, i));
...
int MRISreadValuesBak(MRI_SURFACE *mris, const char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISreadValuesBak: File %s not found\n", fname));
  fread2(&ilat, fp);
  lat = ilat / 10.0;
...
int mrisWriteSnapshot(MRI_SURFACE *mris, INTEGRATION_PARMS *parms, int t) {
  ...
  if (mris->patch) {
    ...
    if (parms->l_thick_normal > 0 || parms->l_thick_parallel > 0 ||
        parms->l_thick_min > 0 || parms->l_thick_spring) {
      int vno;
      MRI *mri_vector;
      ...
      for (vno = 0; vno < mris->nvertices; vno++) {
        ...
        MRISsampleFaceCoordsCanonical((MHT *)(parms->mht), mris, v->x, v->y,
                                      v->z, PIAL_VERTICES, &xp, &yp, &zp);
...
int MRISreadVertexPositions(MRI_SURFACE *mris, const char *name) {
  ...
  if (!fp)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE,
                 "MRISreadVertexPosition(%s): could not open file %s", name,
                 fname));
  ...
  if (version >= -2) {
    ...
    if (nvertices != mris->nvertices || nfaces != mris->nfaces) {
      ...
      ErrorReturn(
          ...
          (ERROR_BADFILE,
           ...
           fname, mris->nvertices, mris->nfaces, name, nvertices, nfaces));
    }
    ...
    for (vno = 0; vno < nvertices; vno++) {
      ...
      if (version == -1) {
        ...
        fread2(&iy, fp);
        fread2(&iz, fp);
        MRISsetXYZ(mris, vno, ix / 100.0, iy / 100.0, iz / 100.0);
      } else /* version == -2 */
      {
        float x = freadFloat(fp);
        float y = freadFloat(fp);
...
int MRISwriteNormalsAscii(MRI_SURFACE *mris, const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISwriteNormalsAscii: could not open file %s", fname));
...
int MRISwriteNormals(MRI_SURFACE *mris, const char *fname) {
  ...
  if (mri == nullptr) {
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE,
                 "MRISwriteNormals(%s): could not allocate volume", fname));
  }
...
int MRISreadNormals(MRI_SURFACE *mris, const char *fname) {
  ...
  if (mri == nullptr) {
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISreadNormals(%s): could not open volume", fname));
  }
...
int MRISwriteWhiteNormals(MRI_SURFACE *mris, const char *fname) {
  ...
  if (mri == nullptr) {
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE,
                 "MRISwriteWhiteNormals(%s): could not allocate volume",
                 fname));
  }
...
int MRISwritePrincipalDirection(MRI_SURFACE *mris, int dir_index,
  ...
  if (mri == nullptr) {
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE,
                 "MRISwritePrincipalDirections(%s): could not allocate volume",
                 fname));
  }
...
int MRISwriteVTK(MRI_SURFACE *mris, const char *fname) {
  ...
  if (!fp)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "MRISwriteVTK: could not open file %s", fname));
...
int MRISwriteCurvVTK(MRI_SURFACE *mris, const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISwriteScalarVTK: could not open file %s", fname));
...
MRI_SURFACE *MRISreadVTK(MRI_SURFACE *mris, const char *fname) {
  char line[STRLEN], *cp = nullptr;

  FILE *fp = fopen(fname, "r");
  if (!fp) {
    ErrorReturn(NULL,
                (ERROR_NOFILE, "MRISreadVTK: could not open file %s\n", fname));
  }
  ...
  */
  ...
  if (!cp || (checks != 3)) {
    fclose(fp);
    ErrorReturn(NULL,
                (ERROR_NOFILE, "MRISreadVTK: error reading file %s\n", fname));
  }
  ...
  */
  ...
  if (!cp) {
    fclose(fp);
    ErrorReturn(NULL,
                (ERROR_NOFILE, "MRISreadVTK: error parsing POLYGONS in file %s",
                 fname));
  }
  int nfaces = 0;
  ...
  if (newMris) {
    if (!MRISallocateFaces(mris, nfaces)) {
      ...
      ErrorExit(ERROR_NO_MEMORY,
                "MRISreadVTK(%d, %d): could not allocate faces", nfaces,
                sizeof(FACE));
    }
  } else {
      ...
      if ((sscanf(cp, "FIELD %*s %d\n", &nfields)) ||
        ...
        for (num_fields = 0; num_fields < nfields; num_fields++) {
          ...
          if (sscanf(cp, "%s %d %d %*s\n", fieldName, &fieldNum,
            ...
            fclose(fp);
            ErrorReturn(NULL, (ERROR_NOFILE,
                               "MRISreadVTK: error parsing FIELDs in file %s",
                               fname));
          }
        }
      }
...
int MRISwriteICO(MRI_SURFACE *mris, const char *fname) {
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    ...
    fprintf(fp, "%8d %8d %8d %8d\n", actual_fno, face->v[0] + 1, face->v[2] + 1,
            face->v[1] + 1);
    actual_fno++;
  }
  fclose(fp);
...
int MRISwritePatchAscii(MRI_SURFACE *mris, const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISwritePatchAscii: could not open file %s", fname));
...
static MRI_SURFACE *mrisReadGeoFile(const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        NULL, (ERROR_NOFILE, "mrisReadGeoFile: could not open file %s", fname));
...
static int mrisReadGeoFilePositions(MRI_SURFACE *mris, const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "mrisReadGeoFile: could not open file %s", fname));
  ...
  if (vertices_per_face != VERTICES_PER_FACE) {
    ...
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE,
                 "mrisReadGeoFile(%s): unsupported vertices/face %d.", fname,
                 vertices_per_face));
  }
...
MRIS *MRISreadOverAlloc(const char *fname, double nVFMultiplier) {
  ...
  if (useOldBehaviour) {
    switch (copeWithLogicProblem("FREESURFER_fix_MRISreadOverAlloc",
                                 "was creating triangles with vertices that "
                                 "were the same vno when reading quad files")) {
  ...
  return useOldBehaviour ? MRISreadOverAlloc_old(fname, nVFMultiplier)
...
static MRIS *MRISreadOverAlloc_new(const char *fname, double nVFMultiplier) {
  ...
  int version = -3;
  {
    ...
    {
      mris = mrisReadGIFTIfile(fname, nullptr);
      if (!mris) {
        return (nullptr);
      }
    ...
    } else // default type MRIS_BINARY_QUADRANGLE_FILE ... use magic number
    {
      ...
      if (!fp)
        ErrorReturn(NULL,
                    (ERROR_NOFILE, "MRISread(%s): could not open file", fname));
    ...
    if (version >= -2) {
      ...
      mris = MRISoverAlloc(nVFMultiplier * nvertices,
                           nVFMultiplier * 2 * nquads, nvertices,
                           0); // don't know yet how many faces there will be
      ...
      for (quad = 0; quad < nquads; quad++) {
        ...
        */
        ...
        for (n = 0; n < 2; n++) {
          if (va_0 == va_1 || va_0 == va_2 || va_1 == va_2)
            continue; // degenerate
  ...
  if ((version < 0) || type == MRIS_ASCII_TRIANGLE_FILE) {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      if (!mris->vertices_topology[vno].f)
        ErrorExit(ERROR_NOMEMORY,
                  ...
                  fname, vno, mris->vertices_topology[vno].num);
    ...
    // but I am worried that code won't get them in the same, and hence get
    // equivalent but different results
    //
    int fno;
    for (fno = 0; fno < mris->nfaces; fno++) {
      FACE *face = &mris->faces[fno];
      ...
      for (n = 0; n < VERTICES_PER_FACE; n++)
        mris->vertices_topology[face->v[n]]
            .f[mris->vertices_topology[face->v[n]].num++] = fno;
    }
  }
...
static MRIS *MRISreadOverAlloc_old(const char *fname, double nVFMultiplier) {
  ...
  if (version >= -2) {
    ...
    mris = MRISoverAlloc(nVFMultiplier * nvertices, nVFMultiplier * 2 * nquads,
                         nvertices, 2 * nquads);
    mris->type = MRIS_BINARY_QUADRANGLE_FILE;
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
MRI_SURFACE *MRISfastRead(const char *fname) { return (MRISread(fname)); }
...
MRIS *MRISread(const std::string fname) {
  MRIS *mris = MRISreadOverAlloc(fname.c_str(), 1.0);
  if (mris == nullptr)
    return (nullptr);
  MRISsetNeighborhoodSizeAndDist(mris, 3); // find nbhds out to 3-nbrs
  MRISresetNeighborhoodSize(mris, 1);      // reset current size to 1-nbrs
...
int MRISwrite(MRIS *mris, const char *name) {
  ...
  if (useOldBehaviour) {
    switch (copeWithLogicProblem("FREESURFER_fix_MRISwrite",
                                 "was combining non-abutting triangles into a "
                                 "quad when writing quad files")) {
    case LogicProblemResponse_old:
      break;
    case LogicProblemResponse_fix:
      useOldBehaviour = false;
  ...
  return useOldBehaviour ? MRISwrite_old(mris, name)
                         : MRISwrite_new(mris, name);
}

static bool quadCombine(int quad[4], int vA[3], int vB[3]) {
  if (false) { // this has been seen to PASS
...
#undef QUAD_COMBINE_TEST
  ...
  // Combine into a quad using a shared edge in triangles vA0 vA1 vA2 and vB0
  // vB1 vB2. Don't combine them when this would flip a triangle - the combined
  // edge must be in reverse order ... vA0 vA1 == vB2 vB1
  //
  int qi = 0;
  ...
  for (vAi = 0; vAi < 3; vAi++) {
    ...
    //
    if (qi == 4)
      return false; // must have shared two edges!
    quad[qi++] = edge0;
    ...
    for (vBi = 0; vBi < 3; vBi++) {
      if (edge0 != vB[vBi])
        continue; // not shared vertex
      int prev = vB[(vBi > 0) ? vBi - 1 : 2];
      if (prev != edge1)
        break; // not shared edge
      if (qi == 4)
        return false; // must have shared two edges!
      quad[qi++] = vB[(vBi < 2) ? vBi + 1
                                : 0]; // put the third vertex of B into the quad
    }
  }
...
static int MRISwrite_new(MRI_SURFACE *mris, const char *name) {
  ...
  if (type == MRIS_ASCII_TRIANGLE_FILE) {
    ...
    return MRISwriteGIFTI(mris, NIFTI_INTENT_POINTSET, fname, nullptr);
  }
  ...
  // Below was combining two adjacent faces without checking whether they had an
  // abutting edge! Calculate how many are really needed
  //
  int quadsNeeded = 0;
  for (k = 0; k < mris->nfaces; k++) {
    ...
    int quad[4];
    if ((k + 1 < mris->nfaces) &&
        quadCombine(quad, f->v, mris->faces[k + 1].v)) {
      k++;
    }
  ...
  // This was combining two adjacent faces without checking whether they had an
  // abutting edge! Now it checks, and writes degenerate quads instead if
  // necessary
  //
  int quadsWritten = 0;
  for (k = 0; k < mris->nfaces; k++) {
...
MATRIX *getSRASToTalSRAS(LT *lt) {
  MATRIX *RASFromSRAS = nullptr;
  MATRIX *sRASFromRAS = nullptr;
  MATRIX *tmpM = nullptr;
  MATRIX *SRASToTalSRAS = nullptr;
...
int mrisReadTransform(MRIS *mris, const char *mris_fname) {
  ...
  if (!lt->src.valid) {
    ...
    if (mris->vg.valid) {
      ...
      if (orig) {
        ...
        orig = nullptr;
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
          fprintf(stderr, "INFO: found the orig volume (mri/orig) "
                          "to get c_(ras) information for src\n");
          fprintf(stderr, "INFO: added info to the surface.\n");
        }
      } else {
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
          fprintf(stderr, "INFO: cannot find mri/orig volume to "
    ...
    if (mris->vg.valid) {
      if (!FZERO(lt->src.c_r - mris->vg.c_r) ||
        ...
        fprintf(stderr,
                ...
                mris->vg.c_r, mris->vg.c_a, mris->vg.c_s, lt->src.c_r,
                lt->src.c_a, lt->src.c_s);
      }
    }
  }
  // check dst info
  ...
  mris->TalSRASToSRAS_ = MatrixInverse(mris->SRASToTalSRAS_, nullptr);
...
int MRISreadCurvatureFile(MRI_SURFACE *mris, const char *sname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "MRISreadCurvature: could not open %s", fname));
...
int MRISreadCurvatureIntoArray(const char *sname, int in_array_size,
                               float **out_array) {
  int k, i, vnum, fnum;
  float *cvec;
  FILE *fp;
...
int MRISreadFloatFile(MRI_SURFACE *mris, const char *sname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "MRISreadFloatFile: could not open %s", fname));
...
mrisReadTriangleFileVertexPositionsOnly(const char *fname) {
  ...
  for (vno = 0; vno < nvertices; vno++) {
    ...
    if (fabs(v->x) > 10000 || !isfinite(v->x))
      ErrorExit(ERROR_BADFILE, "%s: vertex %d x coordinate %f!", Progname, vno,
                v->x);
    if (fabs(v->y) > 10000 || !isfinite(v->y))
      ErrorExit(ERROR_BADFILE, "%s: vertex %d y coordinate %f!", Progname, vno,
                v->y);
    if (fabs(v->z) > 10000 || !isfinite(v->z))
      ErrorExit(ERROR_BADFILE, "%s: vertex %d z coordinate %f!", Progname, vno,
                v->z);
  }
  fclose(fp);
  return (mriss);
...
static int mrisReadTriangleFilePositions(MRI_SURFACE *mris, const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "mrisReadTriangleFile(%s): could not open file", fname));
  ...
  if (nvertices != mris->nvertices || nfaces != mris->nfaces)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM,
                 "mrisReadTriangleFile opened %s okay but surface doesn't "
                 "match %s.  nvertices:%d != "
                 "mris->nvertices:%d || nfaces:%d != mris->nfaces:%d\n",
                 fname, mris->fname ? mris->fname : "NULL", nvertices,
                 mris->nvertices, nfaces, mris->nfaces));
...
static MRI_SURFACE *mrisReadTriangleFile(const char *fname,
  ...
  MRIS *mris = MRISoverAlloc(nVFMultiplier * nvertices, nVFMultiplier * nfaces,
                             nvertices, nfaces);
  mris->type = MRIS_TRIANGULAR_SURFACE;

  for (vno = 0; vno < nvertices; vno++) {
    if (vno % 100 == 0)
      exec_progress_callback(vno, nvertices, 0, 1);
  ...
  // read tags
  {
    ...
    while ((tag = TAGreadStart(fp, &len)) != 0) {
      ...
      case TAG_CMDLINE:
        if (mris->ncmds > MAX_CMDS)
          ErrorExit(ERROR_NOMEMORY,
                    "MRISread(%s): too many commands (%d) in file", fname,
                    mris->ncmds);
        mris->cmdlines[mris->ncmds] = (char *)calloc(len + 1, sizeof(char));
        if (mris->cmdlines[mris->ncmds] == NULL)
          ErrorExit(ERROR_NOMEMORY,
                    "MRISread(%s): could not allocate %d byte cmdline", fname,
                    len);
        mris->cmdlines[mris->ncmds][len] = 0;
        fread(mris->cmdlines[mris->ncmds], sizeof(char), len, fp);
        mris->ncmds++;
...
int MRISwriteDecimation(MRI_SURFACE *mris, char *fname) {
  ...
  if (fptr == nullptr)
    ErrorReturn(
        ERROR_BADFILE,
        (ERROR_BADFILE, "MRISwriteDecimation: could not create %s", fname));
  fputc('\0', fptr);
...
int MRISreadDecimation(MRI_SURFACE *mris, char *fname) {
  ...
  if (fptr == nullptr)
    ErrorReturn(
        ERROR_BADFILE,
        (ERROR_BADFILE, "MRISreadDecimation: could not create %s", fname));
  c = fgetc(fptr);
  if (c == '#') {
    fscanf(fptr, "%*s");
    fscanf(fptr, "%d", &d);
...
int MRISreadNewCurvatureFile(MRI_SURFACE *mris, const char *sname) {
  ...
  if (fp == nullptr)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NOFILE, "MRISreadBinaryCurvature: could not open %s", fname));
...
int MRISreadNewCurvatureIntoArray(const char *sname, int in_array_size,
                                  float **out_array) {
  int k, vnum, fnum;
  float *cvec;
  FILE *fp;
  ...
  if (vnum != in_array_size) {
    ...
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE,
                 "MRISreadNewCurvatureIntoArray(%s): number of vertices (%d) "
                 "doesn't match what was expected (%d)",
                 sname, vnum, in_array_size));
  }
  ...
  if (vals_per_vertex != 1) {
    ...
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE,
                 "MRISreadNewCurvatureIntoArray(%s): vals_per_vertex was not 1",
                 sname));
  }
...
MRI *MRISreadParameterizationToSurface(MRI_SURFACE *mris, char *fname) {
  ...
  if (mrisp == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE,
                 "MRISreadParameterizationToSurface: could not open file %s",
                 fname));
...
int MatlabPlotFace(FILE *fp, MRIS *surf, int faceno, char color,
  ...
  for (n = 0; n < 3; n++) {
    ...
    fprintf(fp, "[%6.4f,%6.4f],[%6.4f,%6.4f],[%6.4f,%6.4f],'%c' ", v1->x, v2->x,
            v1->y, v2->y, v1->z, v2->z, color);
    if (n != 2)
      fprintf(fp, ",");
    cx += v1->x;
    cy += v1->y;
  ...
  if (fabs(NormLen) > 0) {
    ...
    fprintf(fp, "[%6.4f,%6.4f],[%6.4f,%6.4f],[%6.4f,%6.4f],'%c'", cx,
            cx + NormLen * snorm[0], cy, cy + NormLen * snorm[1], cz,
            cz + NormLen * snorm[2], normcolor);
    fprintf(fp, ");\n");
    fprintf(fp, "hold off;\n");
  }
  fflush(fp);
...
int MatlabPlotVertex(FILE *fp, MRIS *surf, int vno, char color,
                     double NormLen) {
  ...
  if (fabs(NormLen) > 0) {
    ...
    fprintf(fp, "plot3([%6.4f,%6.4f],[%6.4f,%6.4f],[%6.4f,%6.4f],'%c')\n", v->x,
            v->x + NormLen * v->nx, v->y, v->y + NormLen * v->ny, v->z,
            v->z + NormLen * v->nz, normcolor);
    fprintf(fp, "hold off;\n");
  }
  fflush(fp);
  return (0);
...
/*!
  ...
  using MRIcopyMRIS() and writes to the given volume-style (eg, mgz) output
  file. Field names must be known to MRIcopyMRIS().
*/
