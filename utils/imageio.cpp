...
IMAGE *ImageFRead(FILE *fp, const char *fname, int start, int nframes) {
  ...
  if (end_frame >= I->num_frame)
    ErrorReturn(NULL,
                (ERROR_BADFILE, "ImageFRead(%s, %d) - frame out of bounds",
                 fname, end_frame));
  I->num_frame = nframes;
...
IMAGE *ImageReadFrames(const char *fname, int start, int nframes) {
  ...
  if (!fp)
    ErrorReturn(NULL,
                (ERROR_NO_FILE, "ImageReadFrames(%s) fopen failed\n", fname));
...
IMAGE *ImageReadHeader(const char *fname) {
  ...
  if (!fp)
    ErrorReturn(
        NULL, (ERROR_NO_FILE, "ImageReadHeader(%s, %d) failed\n", buf, frame));
...
IMAGE *ImageReadType(const char *fname, int pixel_format) {
  ...
  if (!Itmp)
    ErrorReturn(NULL,
                (ERROR_NO_FILE, "ImageReadType(%s, %d): could not read image",
                 fname, pixel_format));
...
IMAGE *ImageRead(const char *fname) {
  ...
  case HIPS_IMAGE:
    ...
    if (!fp)
      ErrorReturn(NULL,
                  (ERROR_NO_FILE, "ImageRead(%s, %d) failed\n", buf, frame));
...
int ImageWriteFrames(IMAGE *image, const char *fname, int start, int nframes) {
  ...
  tmp_image =
      ImageAlloc(image->rows, image->cols, image->pixel_format, nframes);
...
int ImageNumFrames(const char *fname) {
  ...
  if (ecode != HIPS_OK)
    ErrorReturn(
        ERROR_NO_FILE,
        (ERROR_NO_FILE, "ImageNumFrame: fread_header failed (%d)\n", ecode));
...
int ImageAppend(IMAGE *I, const char *fname) {
  ...
  } else /* seek back to start and increment # of frames */
  {
    if (fseek(fp, 0L, SEEK_SET) < 0)
      ErrorReturn(-2,
                  (ERROR_BADFILE, "ImageAppend(%s): could not seek to end"));
    ecode = fwrite_header(fp, &Iheader, fname);
    if (ecode != HIPS_OK)
      ErrorExit(ERROR_NO_FILE, "ImageAppend: fwrite_header failed (%d)\n",
                ecode);
...
//unsigned char     *buffer;
  ...
  default:
    ErrorExit(ERROR_BADPARM,
              "IMAGE: nsamples=%d, bits_per_sample=%d.  only grey scale or RGB "
              "image is supported\n",
  ...
  if (nsamples != 1 && nsamples != 3)
    ErrorExit(
        ERROR_BADPARM,
        "IMAGE: nsamples = %d.  only grey scale or RGB image is supported\n",
        nsamples);
  ...
  for (frame = 0; frame < nframe; frame++) {
    ...
    if (planar_config == PLANARCONFIG_SEPARATE)
      ErrorReturn(NULL, (ERROR_UNSUPPORTED,
                         "TiffReadImage:  PLANARCONFIG_SEPARATE unsupported"));
    ...
    for (row = 0; row < height; row++) {
      ...
      default:
        ErrorExit(
            ERROR_BADPARM,
            "IMAGE: orientation = %d. we support only topleft or botleft\n",
            orientation);
      }
      ...
      if (nsamples == 1) {
        ...
        if (TIFFReadScanline(tif, buf, row, 0) <
            0) // row must be sequentially read for compressed data
          ErrorReturn(NULL,
                      (ERROR_BADFILE,
...
static int TiffWriteImage(IMAGE *I, const char *fname, int frame) {
  ...
  for (frames = 0; frames < I->num_frame; frames++) {
    ...
    if (!FZERO(I->xsize))
      TIFFSetField(out, TIFFTAG_XRESOLUTION,
                   (double)nint(2.54 * 10.0 / I->xsize));
    if (!FZERO(I->ysize))
      TIFFSetField(out, TIFFTAG_YRESOLUTION,
                   (double)nint(2.54 * 10.0 / I->ysize));
    ...
    for (row = 0; row < I->rows; row++) {
      ...
      default:
        ErrorReturn(ERROR_UNSUPPORTED,
                    (ERROR_BADFILE, "TiffWrite: unsupported pixel format %d",
                     I->pixel_format));
      }
...
static IMAGE *PGMReadImage(const char *fname) {
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "pgm not supported"));
}
static IMAGE *PGMReadHeader(FILE *fp, IMAGE *I) {
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "pgm not supported"));
}
static int PGMWriteImage(IMAGE *I, const char *fname, int frame) {
  ErrorReturn(ERROR_UNSUPPORTED, (ERROR_UNSUPPORTED, "pgm not supported"));
}
static IMAGE *PPMReadImage(const char *fname) {
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "ppm not supported"));
}
static IMAGE *PPMReadHeader(FILE *fp, IMAGE *I) {
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "ppm not supported"));
}
static IMAGE *PBMReadImage(const char *fname) {
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "pbm not supported"));
}
static IMAGE *PBMReadHeader(FILE *fp, IMAGE *I) {
  ErrorReturn(NULL, (ERROR_UNSUPPORTED, "pbm not supported"));
}
static int PPMWriteImage(IMAGE *I, const char *fname, int frame) {
  ErrorReturn(ERROR_UNSUPPORTED, (ERROR_UNSUPPORTED, "ppm not supported"));
}
...
static int PPMWriteImage(IMAGE *I, const char *fname, int frame) {
  ...
  if (I->pixel_format != PFBYTE)
    ErrorReturn(
        ERROR_UNSUPPORTED,
        (ERROR_UNSUPPORTED, "PPMWrite: only PFBYTE currently supported"));
  ...
  if (!cpix)
    ErrorReturn(ERROR_UNSUPPORTED,
                (ERROR_UNSUPPORTED, "Could not allocate color pixel buffer"));
...
static int PGMWriteImage(IMAGE *I, const char *fname, int frame) {
  ...
  if (I->pixel_format != PFBYTE)
    ErrorReturn(
        ERROR_UNSUPPORTED,
        (ERROR_UNSUPPORTED, "PGMWrite: only PFBYTE currently supported"));

  if ((outf = fopen(fname, "wb")) == NULL)
    ErrorReturn(ERROR_UNSUPPORTED,
                (ERROR_UNSUPPORTED, "PGMWrite: could not open file"));
...
static int JPEGWriteImage(IMAGE *I, const char *fname, int frame) {
  ...
  if (I->pixel_format != PFBYTE)
    ErrorReturn(
        ERROR_UNSUPPORTED,
        (ERROR_UNSUPPORTED, "JPEGWrite: only PFBYTE currently supported"));
...
static IMAGE *RGBReadHeader(char *fname, IMAGE *I) {
  ...
  if (!I)
    ...
    init_header(I, "orig", "seq", 1, "today", rgb->ysize, rgb->xsize, PFRGB, 1,
                "temp");
