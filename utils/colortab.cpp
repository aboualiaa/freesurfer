    ...
    if (nscan >= 6) {
      ...
      if (ct->entries[structure] != nullptr) {
        printf("CTABreadASCII(%s): Line %d: Duplicate structure "
               ...
               ct->entries[structure]->ri, ct->entries[structure]->gi,
               ct->entries[structure]->bi, ct->entries[structure]->ai);
        ctabDuplicates++;
      } else {
...
COLOR_TABLE *CTABreadFromBinaryV2(FILE *fp) {
  ...
  if (nentries < 0)
    ErrorReturn(NULL, (ERROR_BADFILE, "CTABreadFromBinaryV2: nentries was %d",
                       nentries));
...
int CTABwriteIntoBinaryV2(COLOR_TABLE *ct, FILE *fp) {
  ...
  for (structure = 0; structure < ct->nentries; structure++) {
    if (nullptr != ct->entries[structure]) {
      ...
      fwrite(ct->entries[structure]->name, sizeof(char),
             strlen(ct->entries[structure]->name) + 1, fp);
...
int znzCTABwriteIntoBinaryV2(COLOR_TABLE *ct, znzFile fp) {
  ...
  for (structure = 0; structure < ct->nentries; structure++) {
    if (nullptr != ct->entries[structure]) {
      ...
      znzwrite(ct->entries[structure]->name, sizeof(char),
               strlen(ct->entries[structure]->name) + 1, fp);
        ...
        if ((nullptr != ct->entries[idx2]) && (idx1 != idx2)) {
          ...
          if (annot1 == annot2) {
            ...
            printf("ERROR: labels '%s'\n"
                   ...
                   ct->entries[idx1]->name, ct->entries[idx2]->name);
          }
        }
      }
    }
...
int CTABprintASCIItt(COLOR_TABLE *ct, FILE *fp) {
  ...
  for (structure = 0; structure < ct->ctabTissueType->nentries; structure++) {
    ...
    fprintf(fp, "#ctTType %3d  %-30s  %3d %3d %3d  %3d\n",
            structure + ct->idbase, tmpstr, cte->ri, cte->gi, cte->bi,
            255 - cte->ai);
    free(tmpstr);
  }
  ...
  for (structure = 0; structure < ct->nentries; structure++) {
    ...
    fprintf(fp, "%3d  %-30s  %3d %3d %3d  %3d  %2d\n", structure + ct->idbase,
            tmpstr, cte->ri, cte->gi, cte->bi, 255 - cte->ai, cte->TissueType);
    free(tmpstr);
  }
