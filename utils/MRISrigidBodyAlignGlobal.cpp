...
void MRISrigidBodyAlignGlobal_findMinSSE(double *new_mina, double *new_minb,
  ...
  //
  {
    for (unsigned int i = 0; i < verticesSize; i++) {
      float invLength = mris->radius /
                        sqrtf(squaref(xv[i]) + squaref(yv[i]) + squaref(zv[i]));
      xv[i] *= invLength;
      yv[i] *= invLength;
      zv[i] *= invLength;
    }
...
#define iToRadians(I) (((I)-gridCenterI) * radiansPerGridCell)
  ...
  int const forAlphasForPartitionsCapacity =
      forAlphasCapacity * numberOfVerticesPartitions;

  double *ssesForAlphasForPartitions =
      (double *)malloc(forAlphasForPartitionsCapacity * sizeof(double));
  MRISPfunctionValResultForAlpha *fvsForAlphasForPartitions =
      (MRISPfunctionValResultForAlpha *)malloc(
          forAlphasForPartitionsCapacity *
          sizeof(MRISPfunctionValResultForAlpha));

  // The old code rotates this sphere is rotated around the x- y- and z- axis
  // and compares this resulting rotated vertices with the corresponding points
  // in an image, first mapping the points into the image.  The mapping is very
  // expensive.
  // .
  // Rather than do that, this code rotates the vertices around the x and y
  // axes, and then rotates the image around the z axes which avoids a lot of
  // the rotation work
  //
...
#define centersCapacity 1
  ...
  //        nangles/2)       // nangles+1 == forAlphaCapacity because of this
  //
  ...
  for (;;) {
    ...
    // Use the outputs from the previous iteration as the best known centers so
    // far
    //
    Centers inpCenters;
    ...
    for (ici = 0; ici < inpCentersSize; ici++) {
      ...
      if (tracing) {
        fprintf(stdout,
                "%s:%d scanning %2.2f degree nbhd of centers[%d] (%2.2f, "
                "%2.2f, %2.2f), min sse = %2.2f\n",
                __FILE__, __LINE__,
                (float)DEGREES(nangles * radiansPerGridCell), ici,
      ...
      for (gj = 0; gj < nangles + 1; gj++) {
        ...
        rotateVertices(gammaRotated_xv, gammaRotated_yv, gammaRotated_zv, xv,
                       yv, zv, verticesSize,
        ...
        for (bj = 0; bj < nangles + 1; bj++) {
          ...
          rotateVertices(betaGammaRotated_xv, betaGammaRotated_yv,
                         betaGammaRotated_zv, gammaRotated_xv, gammaRotated_yv,
                         gammaRotated_zv, verticesSize,
                         0.0,  // rotate around z axis - last rotation
          ...
          // Partition the vertices so can spread across any threads in a
          // deterministic manner
          //
          int const verticesPerPartition =
              (verticesSize + numberOfVerticesPartitions - 1) /
              numberOfVerticesPartitions;

          ROMP_PF_begin int partition;
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(assume_reproducible)
#endif
          for (partition = 0; partition < numberOfVerticesPartitions;
               partition++) {
            ROMP_PFLB_begin
            ...
            double *const ssesForAlphas =
                &ssesForAlphasForPartitions[partition * forAlphasCapacity];
            ...
            for (vi = viLo; vi < viHi; vi++) {
              ...
              //      which can be used to speed up the MRISPfunctionVal_radiusR
              //      calls for each vertex
              ...
              //
              MRISPfunctionVal_radiusR(parms->mrisp_template,
                                       fvsForAlphas, // output values
                                       mris->radius, betaGammaRotated_xv[vi],
                                       betaGammaRotated_yv[vi],
                                       betaGammaRotated_zv[vi], parms->frame_no,
                                       true, alphasForAlphas,
                                       ajsSize, // input requests
                                       vertexTrace);
              ...
              for (unsigned int ajsI = 0; ajsI < ajsSize; ajsI++) {
                ...
                if (false) {
                  ...
                  // This is temporary, until it is built into
                  // MRISPfunctionVal_radiusR
                  //
                  float x = 0, y = 0, z = 0;

                  rotateVertices(&x, &y, &z, &betaGammaRotated_xv[vi],
                                 &betaGammaRotated_yv[vi],
                                 &betaGammaRotated_zv[vi], 1,
                  ...
                  if (vertexAlphaTrace) {
                    fprintf(stdout,
                            "%s:%d rotated (%g,%g,%g) by (a:%g, b:%g, g:%g) to "
                            "(%g,%g,%g)\n",
                            __FILE__, __LINE__, xv[vi], yv[vi], zv[vi], alpha,
                            beta, gamma, x, y, z);
                  }

                  MRISPfunctionValResultForAlpha targetAndStd;
                  MRISPfunctionVal_radiusR(
                      parms->mrisp_template, &targetAndStd, mris->radius, x, y,
                      z, parms->frame_no, true, &alpha, 1, vertexAlphaTrace);

                  if (vertexTrace && (aj < 2 || forAlphasCapacity - 2 <= aj)) {
                    fprintf(
                        stdout,
                        "%s:%d predicted target:%g target:%g   std:%g v %g\n",
                        __FILE__, __LINE__, target, targetAndStd.curr, std,
                        targetAndStd.next);
                  }
                }
...
#define DEFAULT_STD 4.0f
          ...
          {
            int partition;
            for (partition = 1; partition < numberOfVerticesPartitions;
                 partition++) {
          ...
          for (unsigned int ajsI = 0; ajsI < ajsSize; ajsI++) {
            ...
            if (spreadsheet) {
              ...
              fprintf(stdout, "abgi, %d,%d,%d,  abg, %g,%g,%g, sse,%g\n", ai,
                      bi, gi, alpha, beta, gamma, sse);
            }

            if (trace)
              fprintf(stdout, "%s:%d sse:%g after vno:%d\n", __FILE__, __LINE__,
                      sse, vno);
            trace = false;

            // Consider adding this center to the outCenters, either as a new or
            // as a replacement for an old
            //
            float const radius = (gridStride * nangles) / 3.0f;
            // Anything within this grid index is consider to be 'nearby' and
            // hence replaces the center
            ...
            if (oci == centersCapacity) { // A distant local minimum, and
                                          // centers is full
              oci = 0; // Select the largest existing minimum to be replaced
              int i;
              for (i = 1; i < outCentersSize; i++) {
                if (outCenters[oci].center_sse < outCenters[i].center_sse)
                  oci = i;
              }
              if (outCenters[oci].center_sse <= sse)
                oci = -1; // The largest existing min is better than this
            }
    ...
    if (false || tracing) {
      ...
      for (oci = 0; oci < outCentersSize; oci++) {
        ...
        fprintf(
            ...
            __FILE__, __LINE__, oci, (float)DEGREES(iToRadians(oc->center_ai)),
  ...
  // Free the temps
  //
  free(fvsForAlphasForPartitions);
  free(ssesForAlphasForPartitions);
  free(alphasForAlphas);
  free(ajsForAlphas);
  free(doneFlags);
  free(betaGammaRotated_zv);
  free(betaGammaRotated_yv);
  free(betaGammaRotated_xv);
  free(gammaRotated_zv);
  free(gammaRotated_yv);
  free(gammaRotated_xv);
  free(zv);
  free(yv);
  free(xv);
  free(curv);
