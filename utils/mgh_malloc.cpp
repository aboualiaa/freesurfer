//  #define MEMORY_CORRUPTION_DEBUGGING     // uncomment this to turn the
//  feature on
...
//
//      make CCLD="g++ -Wl,--wrap=free -Wl,--wrap=malloc -Wl,--wrap=calloc
//      -Wl,--wrap=realloc -Wl,--wrap=posix_memalign"
//
...
// When linked with the command line above, the original functions get renamed
// __real_name and the uses of the original functions get replaced with uses of
// __wrap_name
//
...
// use whole pages of memory to satisfy each request.  It can change the
// protection on one of these pages when it is 'free' to catch writes to free
// pages.  It also checksums the free page, and before it is reassigned, checks
// it has not been written while free. Sadly the o/s limits the number of pages
// that can be protected to just a few...
//
typedef struct Page {
...
#ifdef USE_MPROTECT
    if (mprotect(
            nLoPage, npcPageCount * 4096,
            PROT_READ |
                PROT_WRITE)) {   // the limit (see below) should not affect this
...
static void shouldNotUse(Page *oLoPage, size_t opcPageCount) {
  ...
  if (oLoPage->mostRecentlyAllocationClock !=
      memProtected_mostRecentlyAllocationClock)
    return;

#ifdef USE_MPROTECT
  if (mprotect(oLoPage, opcPageCount * 4096,
               PROT_NONE)) {   // sadly there is a limit
    perror("mprotect failed"); // on how variegated memory can be
    *(int *)-1 = 0; // weird                  // but we can use it to find a
                    // repeatable problem
  }
#endif
...
// It also puts a few guard bytes at the end of the storage that can be checked
// for overruns
//
...
static int doMallocAction(MallocAction action, void **ptr, size_t size) {
...
#else
  if (!pages) {
    size_t unalignedPage =
        (size_t)__real_malloc((pagesCapacity + 1) * sizeof(Page));
    size_t alignedPage = unalignedPage & ~(sizeof(Page) - 1);
    if (unalignedPage > alignedPage)
      alignedPage += sizeof(Page);
  ...
  if (action & MA_remove)
    getPageChainsIndexAndCount(&opci, &opcPageCount, oLoPage->size);
  size_t npci = 0, npcPageCount = 0;
  if (action & MA_insert)
    getPageChainsIndexAndCount(&npci, &npcPageCount, size);
...
static Malloced *noteMallocAction(MallocAction action, void *ptr, size_t size,
                                  size_t padding) {
  ...
  while (m->ptr != ptr) {
    if (m->ptr == 0)
      break; // reached the end of the chain, some items on which may have been
             // freed
    hash = (hash * 327 + 1) &
           mallocedSizeMask; //      but their ptr's were left non-null so they
                             //      did not act as end of chain!
    m = &malloced[hash];
    if (++stabs > 1000)
  ...
  if (action & MA_remove) {
    if (m->ptr != ptr)
      *(int *)-1 = 0; // not in chain
    if (m->size == 0)
      *(int *)-1 = 0; // in chain but freed already
    char *pad = (char *)ptr + m->size - m->padding; //
    while (m->padding--)
      if (*pad++ != 0x5c)
        *(int *)-1 = 0; // trailing padding has been corrupted
    sumAlloc -= m->size;
    m->whereAllocated = NULL;
    // m->ptr must not be zeroed' because that would end chains that others
    // might be in
...
// I have seen mris_fix_topology run almost to completion before the hash table
// filled up
//
void *__wrap_malloc(size_t size) {
  ...
  void *r = NULL;
  if (!doMallocAction(MA_insert, &r, size))
    r = __real_malloc(size);
...
void *__wrap_calloc(size_t nmemb, size_t size) {
  ...
  wrapper_lock_acq();
  void *r = NULL;
  if (!doMallocAction(MA_insert | MA_clear, &r, size))
    r = __real_calloc(1, size);
  noteMallocAction(MA_insert, r, size, padding);
  wrapper_lock_rel();
  return r;
}
...
void *__wrap_realloc(void *ptr, size_t size) {
  ...
  void *r = ptr;
  if (!doMallocAction(MA_insert | MA_copy | MA_remove, &r, size))
    r = __real_realloc(ptr, size);
  noteMallocAction(MA_insert, r, size, padding);
  wrapper_lock_rel();
  return r;
}
...
void __wrap_free(void *ptr) {
  ...
  wrapper_lock_acq();
  noteMallocAction(MA_remove, ptr, 0, 0);
  if (!doMallocAction(MA_remove, &ptr, 0))
    __real_free(ptr);
  wrapper_lock_rel();
}
...
int __wrap_posix_memalign(void **memptr, size_t alignment, size_t size) {
  ...
  int r = 0;
  if (!doMallocAction(MA_insert, memptr, size))
    r = __real_posix_memalign(memptr, alignment, size);
...
// Regardless of whether the __real_malloc etc. or the __wrap_ ones are being
// used, it is still desirable to know where in the program the allocations are
...
static void mallocStatsExitHandler(void) {
  ...
  fprintf(stderr, "MallocStats\n   file, function, line, size, inserted, "
                  "cleared, copied, removed\n");
  for (i = 0; i < count; i++) {
    MallocStats *m = &mallocStats[indexs[i]];
    fprintf(stderr, "%s, %s, %d, %g, %g, %g, %g, %g\n", m->file, m->function,
            m->line, (float)m->size, (float)m->inserted, (float)m->cleared,
            (float)m->copied, (float)m->removed);
  }
  fprintf(stderr, "MallocStats\n   file, function, line, size, inserted, "
                  "cleared, copied, removed\n");
}

static MallocStats *noteMallocStatsAction(MallocAction action, size_t size,
                                          const char *file,
...
_Pragma("GCC diagnostic ignored "
        "\"-Wunused-function\"") static void clearMallocStatsAllocCounters() {
  size_t hash;
  for (hash = 0; hash < mallocStatsSize; hash++)
    mallocStats[hash].sumCurrentAllocs = 0;
}

_Pragma("GCC diagnostic ignored "
        "\"-Wunused-function\"") static void showMallocStatsAllocCounters() {
  fprintf(stdout, "\nshowMallocStatsAllocCounters\n");
  size_t hash;
  for (hash = 0; hash < mallocStatsSize; hash++) {
    MallocStats *m = &mallocStats[hash];
    if (!m->sumCurrentAllocs || m->maxSumCurrentAllocs < m->sumCurrentAllocs)
      continue;
    fprintf(stdout, "%10g %s:%d %s\n", (double)m->sumCurrentAllocs, m->file,
            m->line, m->function);
  }
  for (hash = 0; hash < mallocStatsSize; hash++) {
    MallocStats *m = &mallocStats[hash];
    if (!m->sumCurrentAllocs || m->maxSumCurrentAllocs > m->sumCurrentAllocs)
      continue;
    fprintf(stdout, "%10g %s:%d %s, up from %g\n", (double)m->sumCurrentAllocs,
            m->file, m->line, m->function, (double)m->maxSumCurrentAllocs);
...
void freeHere(void *ptr, const char *file, const char *function, int line) {
  free(ptr);
  noteMallocStatsAction(MA_remove, 0, file, function, line);
}
