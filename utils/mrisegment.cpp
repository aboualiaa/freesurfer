...
MRI_SEGMENTATION *MRImaxsegment(MRI *mri, float low_val, float hi_val) {
  ...
  int x, y, z, width, height, depth, xi, yi, zi, xk, yk, zk, val,
  ...
  */
  ...
  if (false && Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
MRI *MRIsegmentToImage(MRI *mri_src, MRI *mri_dst, MRI_SEGMENTATION *mriseg,
                       int s) {
...
int MRIcompactSegments(MRI_SEGMENTATION *mriseg) {
  ...
  if (newseg) {
    ...
    if (newseg == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "MRIcompactSegments(): could not alloc mriseg with %d segments",
                mriseg->nsegments);
...
int MRIsegmentDilate(MRI_SEGMENTATION *mriseg, MRI *mri) {
  ...
  mri_segments = MRIclone(mri, nullptr);
            ...
            if (MRIgetVoxVal(mri, xi, yi, zi, 0) &&
              ...
              if (mseg->nvoxels >= mseg->max_voxels) {
                if (mriSegmentReallocateVoxels(
                        mriseg, segno, mseg->max_voxels * VOX_INCREASE) !=
                    NO_ERROR) {
                  /*              MRIsegmentFree(&mseg) ;*/
                  return (Gerror);
                }
              ...
              MRIsetVoxVal(mri_segments, xi, yi, zi, 0,
                           MRIgetVoxVal(mri, xi, yi, zi, 0));
              mseg->nvoxels++;
  ...
  for (s = 0; s < mriseg->max_segments; s++)
    if (mriseg->segments[s].nvoxels < min_voxels) {
      if (mriseg->segments[s].voxels) {
        ...
        mriseg->segments[s].voxels = nullptr;
      }
...
int MRIeraseSmallSegments(MRI_SEGMENTATION *mriseg, MRI *mri_seg,
                          int min_voxels) {
...
int MRIsegmentDilateThreshold(MRI_SEGMENTATION *mriseg, MRI *mri_binary,
  ...
  mri_segments = MRIclone(mri_binary, nullptr);
        ...
        for (yk = -1; yk <= 1; yk++) {
          ...
          for (zk = -1; zk <= 1; zk++) {
            ...
            if (MRIgetVoxVal(mri_binary, xi, yi, zi, 0) &&
              ...
              if (mseg->nvoxels >= mseg->max_voxels) {
                if (mriSegmentReallocateVoxels(
                        mriseg, segno, nint(mseg->max_voxels * VOX_INCREASE)) !=
                    NO_ERROR) {
                  /*                  MRIsegmentFree(&mseg) ;*/
                  return (Gerror);
                }
              ...
              mseg->voxels[mseg->nvoxels].z = zi;
              MRIsetVoxVal(mri_segments, xi, yi, zi, 0,
                           MRIgetVoxVal(mri_binary, xi, yi, zi, 0));
              mseg->nvoxels++;
              mseg->area += voxel_size;
            }
          }
...
MRI *MRIsegmentFill(MRI_SEGMENTATION *mriseg, int s, MRI *mri, float fillval) {
  ...
  if (mri == nullptr)
    mri = MRIclone(mriseg->mri, nullptr);
...
LABEL *MRIsegmentToLabel(MRI_SEGMENTATION *mriseg, MRI *mri, int ind) {
  ...
  area = LabelAlloc(mseg->nvoxels, nullptr, nullptr);

  area->coords = LABEL_COORDS_TKREG_RAS;
  area->avg_stat = 0;
  for (i = 0; i < mseg->nvoxels; i++) {
    MRIvoxelToSurfaceRAS(mri, mseg->voxels[i].x, mseg->voxels[i].y,
                         mseg->voxels[i].z, &xs, &ys, &zs);
    area->lv[i].x = xs;
    ;
    area->lv[i].y = ys;
    ;
    ...
    ;
    area->lv[i].vno = -1;
    area->lv[i].stat = MRIgetVoxVal(mri, mseg->voxels[i].x, mseg->voxels[i].y,
                                    mseg->voxels[i].z, 0);
