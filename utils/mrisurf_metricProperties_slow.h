...
#pragma omp parallel for if_ROMP(shown_reproducible)
    ...
    ROMP_PFLB_begin FACE *f = &mris->faces[k];
...
#pragma omp critical
      ...
      for (vi = 0; vi < VERTICES_PER_FACE; vi++) {
        if (!mris->vertices[f->v[vi]].ripflag)
          break;
      }
...
void MRIScomputeAvgInterVertexDist(MRIS *mris, double *StdDev) {
  ...
  if (showHashs) {
    fprintf(stdout, "%s:%d MRIScomputeAvgInterVertexDist starting ", __FILE__,
            __LINE__);
    mris_print_hash(stdout, mris, "mris ", "\n");
  }
...
#pragma omp parallel for if_ROMP(shown_reproducible)
    ...
    */
    ...
    float const xc =
        (v0->x + v1->x +
         v2->x) /* / 3 */; // These divides by three are a waste of time
    float const yc =
        (v0->y + v1->y +
         v2->y) /* / 3 */; // since we only use the magnitude of the dot product
...
float mrisSampleMinimizationEnergy(MRIS *mris, int vno,
...
float mrisSampleNormalEnergy(MRIS *mris, int vno, INTEGRATION_PARMS *parms,
...
float mrisSampleSpringEnergy(MRIS *mris, int const vno, float cx, float cy,
  ...
  MHTfindClosestFaceGeneric(mht, mris, cx, cy, cz, 4, 4, 1, &face, &fno,
                            &fdist);
  if (fno < 0)
    MHTfindClosestFaceGeneric(mht, mris, cx, cy, cz, 1000, -1, -1, &face, &fno,
                              &fdist);
  MRISsampleFaceCoords(mris, fno, cx, cy, cz, PIAL_VERTICES, CANONICAL_VERTICES,
                       &xp, &yp, &zp);
  ...
  for (num = n = 0; n < vt->vnum; n++) {
    ...
    if (vn->ripflag)
      continue;
    MRISsampleFaceCoords(mris, vn->fno, vn->x, vn->y, vn->z, PIAL_VERTICES,
                         CANONICAL_VERTICES, &xn, &yn, &zn);
    xc += xn;
    yc += yn;
    zc += zn;
    num++;
...
float mrisSampleParallelEnergyAtVertex(MRIS *mris, int const vno,
  ...
  // compute average of neighboring vectors connecting white and pial surface,
  // store it in d[xyz]n_total
...
float mrisSampleParallelEnergy(MRIS *mris, int const vno,
                               INTEGRATION_PARMS *parms, float cx, float cy,
                               float cz) {
  VERTEX_TOPOLOGY const *const vt = &mris->vertices_topology[vno];
  VERTEX *const v = &mris->vertices[vno];
...
int MRISsampleFaceCoordsCanonical(MHT *mht, MRIS *mris, float x, float y,
  ...
  if (fno < 0) {
    ...
    ret = face_barycentric_coords(mris, fno, CANONICAL_VERTICES, x, y, z,
