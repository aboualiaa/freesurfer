...
*Min_HeapAllocate(int max_size, int max_id_array_size) {
  ...
  if (MH == nullptr) {
    fprintf(stderr,
            "Min_HeapAllocate: Unable to allocate memory for min heap!!\n");
    return (nullptr);
  }
  ...
  if (max_id_array_size > 0) {
    ...
    if (MH->id_array == nullptr) {
      fprintf(stderr,
              "*Min_HeapAllocate: Unable to allocate memory for id array!!\n");
      return (nullptr);
    }
...
static int Min_HeapExchangeElements(MIN_HEAP *MH, int i, int j) {
  ...
  if (i < 0 || i >= MH->CurrHeapSize) {
    fprintf(stderr,
            "Min_HeapExchangeElements: index %d exceeds size of heap!!\n", i);
    return (ERROR);
  }
  ...
  if (i == j) {
    fprintf(stderr,
            "Min_HeapExchangeElements: The indices %d and %d are the same!!\n",
            i, j);
    return (ERROR);
  } else if (i < j) {
    if (j != 2 * i + 1 && j != 2 * i + 2) {
...
int Min_HeapEditKeyIndexID(MIN_HEAP *MH, int id, double newKey) {
  ...
  if (id < 0 || id >= MH->max_id_array_size) {
    fprintf(
        stderr,
        "Min_HeapEditKeyIndexID: ID %d is out of range. id_array size is %d\n",
...
// Min_HeapifyDown assumes index might be bigger than its children but everyone
// above it is ok
static int Min_HeapifyDown(MIN_HEAP *MH, int index) {
  ...
  while (true) {
    ...
    if (left < MH->CurrHeapSize) {
      ...
      if (right < MH->CurrHeapSize &&
          MH->MHE_array[right].HeapKey < MH->MHE_array[smallest].HeapKey)
        smallest = right;
    } else {
      smallest = curr_index;
    }
    ...
    if (smallest ==
        curr_index) // If heap property is not violated, break out of loop
      break;
...
// Min_HeapifyUp assumes index's parent might be bigger than it, but everyone
// belong it is ok.
static int Min_HeapifyUp(MIN_HEAP *MH, int index) {
  ...
  while (true) {
    ...
    parent =
        (curr_index - 1) / 2; // Note that truncation gives you correct parent
...
int Min_HeapInsert(MIN_HEAP *MH, double key, void *data, int id) {
  if (id < 0 || id >= MH->max_id_array_size) {
    fprintf(stderr,
            "Min_HeapInsert: ID %d is out of range. id_array size is %d\n", id,
            MH->max_id_array_size);
    return (ERROR);
  }
  ...
  if (MH->CurrHeapSize == MH->MaxHeapSize) {
    fprintf(stderr, "Min_HeapInsert: Heap runs out of space. Current version "
                    "doesn't allocate more space when heap is full!!\n");
    return (ERROR);
  } else {
...
void Min_HeapInternalCheck(MIN_HEAP *MH, int PrintContent) {
  ...
  fprintf(stderr,
          "Performing Heap Consistency Check: Current heap size is: %d\n",
          MH->CurrHeapSize);
  ...
  for (i = 0; i < MH->max_id_array_size; i++) {
    if (MH->id_array[i] != -1) {
      ...
      if (MH->id_array[i] < 0 || MH->id_array[i] >= MH->max_id_array_size) {
        fprintf(stderr,
                "Min_HeapInternalCheck: Index %d is out of range. Current heap "
                "size is %d\n",
      ...
      if (MH->MHE_array[MH->id_array[i]].id != i) {
        fprintf(stderr,
                "Min_HeapInternalCheck: i = %d, but "
                "MH->MHE_array[MH->id_array[i]].id = %d\n",
  ...
  for (i = 0; i < MH->CurrHeapSize; i++) {
    ...
    if (child1 < MH->CurrHeapSize) {
      if (MH->MHE_array[child1].HeapKey < MH->MHE_array[i].HeapKey) {
        fprintf(stderr, "Child index %d (key %f) < (key %f) Parent %d\n",
                child1, MH->MHE_array[child1].HeapKey, MH->MHE_array[i].HeapKey,
                i);
        fprintf(stderr, "Consistency Check Unsuccessful\n\n");
        return;
      }
    ...
    if (child2 < MH->CurrHeapSize) {
      if (MH->MHE_array[child2].HeapKey < MH->MHE_array[i].HeapKey) {
        fprintf(stderr, "Child index %d (key %f) < (key %f) Parent %d\n",
                child2, MH->MHE_array[child2].HeapKey, MH->MHE_array[i].HeapKey,
                i);
        fprintf(stderr, "Consistency Check Unsuccessful\n\n");
        return;
      }
