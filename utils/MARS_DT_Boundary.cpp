...
/*
  sv: int32 array of size N vertices, with the entries corresponding to your
boundary marked as 1, and the remaining entries marked as 0. numVerts: int32
variable, with value = N vertNbors: M x N int32 matrix. where the (i, j) entry
corresponds to vertex number of the ith neighbor of vertex j. If vertex j has
less than M neighbors, for example 4 neighbors, then entries (i > 4, j) are set
to 0. maxNeighbors: int32 variable with value = M costNbors: M x N double
matrix, where the (i, j) entry corresponds to distance between ith neighbor of
vertex j and vertex j. Like vertNbors, if the ith neighbor doesn't exist, then
the entry is set to 0. final_cost:  array of size N of type double.
MARS_DT_Boundary will save the output into final_cost.
...
void MARS_DT_Boundary(int *sv, int numVerts, int maxNeighbors, int *vertNbors,
  ...
  for (i = 0; i < numVerts; i++) {
    result = Min_HeapInsert(MH, BIG_NO, nullptr, (int)i); /*C indices*/
    if (result == ERROR)
  ...
  while (true) {
    ...
    for (i = 0; i < maxNeighbors; i++) {
      neighbor = vertNbors[index_2D_array(
          i, id, maxNeighbors)]; /*id is already C-index*/
      if (neighbor != 0)         /*real neighbor*/
      {
        ...
        if (Min_HeapIdIsInHeap(
                MH, neighbor)) /*Neighbor has not been taken out of heap yet*/
        {
          result = Min_HeapQueryKeyIndexID(MH, neighbor, &queryKey);
          if (result == ERROR)
            mexErrMsgTxt("MARS_DT_Boundary: query fail!!!");

          cost = costNbors[index_2D_array(
              i, id, maxNeighbors)]; /*note that id is already in C index*/
