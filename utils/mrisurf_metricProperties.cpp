...
double MRISpercentDistanceError(MRIS *mris) {
  ...
  if (mean_dist == 0.0f) {
    fprintf(stdout,
            "%s:%d MRISpercentDistanceError called when all dist zero\n",
            __FILE__, __LINE__);
  }

  if (mean_odist == 0.0f) {
    fprintf(stdout,
            "%s:%d MRISpercentDistanceError called when all dist_orig zero\n",
            __FILE__, __LINE__);
  }
...
int MRIScomputeMetricProperties(MRIS *mris) {
  ...
  if (useNewBehaviour) {
    ...
    MRISMP_load(&mp, mris); // Copy the input data before
                            // MRIScomputeMetricPropertiesWkr changes it
  ...
  if (useNewBehaviour)
    MRISMP_unload(mris, &mp,
                  useOldBehaviour); // Verify the two approaches got the same
                                    // answers, or use the new answers
