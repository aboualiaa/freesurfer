...
//
static void MRISsetXYZwkr1(MRIS *mris, const char *file, int line,
                           bool *laterTime) {
  if (mris->dist_alloced_flags & 1) {
    copeWithLogicProblem2(laterTime, NULL,
                          "dist not freed when setXYZ called.  Add call to "
...
void MRISsetXYZwkr(MRIS *mris, int vno, float x, float y, float z,
                   const char *file, int line, bool *laterTime) {
...
void MRISmemalignNFloats(size_t n, float **ppx, float **ppy, float **ppz) {
  ...
  if (ppx)
    cheapAssert(0 == posix_memalign((void **)ppx, 64, n * sizeof(float)));
  if (ppy)
    cheapAssert(0 == posix_memalign((void **)ppy, 64, n * sizeof(float)));
  if (ppz)
    cheapAssert(0 == posix_memalign((void **)ppz, 64, n * sizeof(float)));
}
...
void MRISimportXYZ(MRIS *mris, const float *const px, const float *const py,
                   const float *const pz) {
...
//
void MRISsetOriginalXYZwkr(MRIS *mris, int vno, float origx, float origy,
                           float origz, const char *file, int line,
                           bool *laterTime) {
  if (mris->dist_alloced_flags & 2) {
    copeWithLogicProblem2(laterTime, NULL,
                          "dist_orig not freed when setOriginalXYZ called.  "
                          "Add call to MRISfreeDistOrigs(MRIS*)",
                          file, line, "<unknown>");
  }
  ...
  const float *pcx = &v->origx;
  float *      px  = (float *)pcx;
  *px              = origx;
  const float *pcy = &v->origy;
  float *      py  = (float *)pcy;
  *py              = origy;
  const float *pcz = &v->origz;
  float *      pz  = (float *)pcz;
  *pz              = origz;
...
int MRISimportVertexCoords(MRIS *const mris, float *locations[3],
                           int const which) {
  int const nvertices = mris->nvertices;
  ...
  for (vno = 0; vno < nvertices; vno++) {
    ...
    default:
      ErrorExit(ERROR_UNSUPPORTED,
                "MRISimportVertexCoords: which %d not supported", which);
      break;
...
int MRISreverseCoords(MRIS *mris, int which_direction, int reverse_face_order,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    default:
      ErrorExit(ERROR_UNSUPPORTED,
                "MRISreverseCoords: unsupported which_vertices %d",
                which_coords);
    }
...
void MRISscaleThenTranslate(MRIS *mris, double sx, double sy, double sz,
...
MRIS *MRISrotate(MRIS *mris_src, MRIS *mris_dst, float alpha, float beta,
                 float gamma) {
  if (!mris_dst) {
...
void MRIScalculateCenterCOG2(MRIS *mris, double *xCOG, double *yCOG,
                             double *zCOG) {
...
MRIS *MRISprojectOntoTranslatedSphere(MRIS *mris_src, MRIS *mris_dst, double r,
...
void MRIStranslate_along_vertex_dxdydz(SurfaceOut surfaceOut,
                                       SurfaceInp surfaceInp, double dt) {
...
static void MRISaverageVertexPositionsWkr_part1(MRIS *mris, int navgs,
                                                float scale, bool includeCenter,
                                                int start, int step,
                                                float **ppx, float **ppy,
                                                float **ppz) {
  MRISfreeDistsButNotOrig(mris); // it is either this or adjust them...
...
static void MRISaverageVertexPositionsWkr_part2(MRIS *mris, float *px,
                                                float *py, float *pz,
                                                bool checking) {
  if (!checking)
...
int MRISaverageVertexPositions(MRIS *mris, int navgs) {
  ...
  if (checking) {
    switch (
        copeWithLogicProblem("FREESURFER_fix_MRISaverageVertexPositions",
                             "MRISaverageVertexPositions was inefficient")) {
  ...
  MRISaverageVertexPositionsWkr_part1(mris, navgs, 1.0f, true, 0, 1, &px, &py,
                                      &pz);
...
void MRISsmoothSurface(MRIS *mris, int niter, float step) {
  ...
  switch (copeWithLogicProblem(
      "FREESURFER_fix_MRISsmoothSurface1",
      "MRISsmoothSurface was calling MRIScomputeMetricProperties too soon")) {
  case LogicProblemResponse_old:
    break;
  case LogicProblemResponse_fix:
  ...
  if (checking) {
    switch (copeWithLogicProblem(
        "FREESURFER_fix_MRISsmoothSurface2",
        "MRISsmoothSurface was not using common efficient code")) {
    case LogicProblemResponse_old:
      break;
    case LogicProblemResponse_fix:
      checking = false;
  ...
  MRISaverageVertexPositionsWkr_part1(mris, niter, step, false, 0, 1, &px, &py,
                                      &pz);
...
static double estimateNRG(MRIS *mris, double cx, double cy, double cz,
                          double R2);
static double estimateSquaredRadius(MRIS *mris, double cx, double cy,
                                    double cz);
static void   computeGradient(MRIS *mris, double cx, double cy, double cz,
                              double R2, double *gx, double *gy, double *gz);
...
static double estimateSquaredRadius(MRIS *mris, double cx, double cy,
  ...
  int n;
  for (n = 0; n < mris->nvertices; n++) {
    R2 += SQR(mris->vertices[n].x - cx) + SQR(mris->vertices[n].y - cy) +
...
void MRIScenterSphere(MRIS *mris) {
  ROMP_SCOPE_begin MRIScenterSphere_old(mris);
  ROMP_SCOPE_end
}
...
static void MRIScenterSphere_old(MRIS *mris) {
  ...
  while (NRG < last_NRG) {
    ...
    while (NRG >= last_NRG) {
      epsilon /= 2.0;
      NRG = estimateNRG(
          mris, x + epsilon * dx, y + epsilon * dy, z + epsilon * dz,
          estimateSquaredRadius(mris, x + epsilon * dx, y + epsilon * dy,
                                z + epsilon * dz));
      d = sqrt(SQR(dx) + SQR(dy) + SQR(dz));
      if (epsilon * d <
          0.00000000001) // FZERO(epsilon*(SQR(dx)+SQR(dy)+SQR(dz))))
      {
        break;
      }
    }
  ...
  fprintf(WHICH_OUTPUT,
          ...
          sqrt(estimateSquaredRadius(mris, cx, cy, cz)), 10000. * cx,
          10000. * cy, 10000. * cz);
#endif
...
void MRISrecenter(MRIS *mris, int which_move, int which_target) {
  ...
  for (n = vno = 0; vno < mris->nvertices; vno++) {
    ...
    default:
      ErrorExit(ERROR_UNSUPPORTED,
                "MRISrecenter: movable vertices %d not supported", which_move);
      break;
    }
  }
...
/*-----------------------------------------------------------------
  ...
  ----------------------------------------------------------------*/
static bool mrisComputeVertexDistancesWkr(MRIS *mris, int new_dist_nsize,
                                          bool check);
static bool mrisComputeOriginalVertexDistancesWkr(MRIS *mris,
                                                  int   new_dist_nsize,
                                                  bool  check);
...
bool mrisCheckDistOrig(MRIS const *mris) {
  if (!mris->dist_orig_nsize)
    return true;
  return mrisComputeOriginalVertexDistancesWkr((MRIS *)mris,
                                               mris->dist_orig_nsize, true);
}
...
void mrisComputeOriginalVertexDistancesIfNecessaryWkr(MRIS *      mris,
                                                      bool *      laterTime,
                                                      const char *file,
                                                      int         line) {
  if (mris->dist_alloced_flags & 2)
  ...
  bool useOldBehaviour = false;
  switch (copeWithLogicProblem("FREESURFER_fix_missing_dist_orig",
                               "dist_orig not already computed")) {
  case LogicProblemResponse_old:
...
double MRISpercentDistanceError(MRIS *mris) {
  ...
  if (mean_dist == 0.0f)
    fs::debug() << "MRISpercentDistanceError called when all dist zero";
  if (mean_odist == 0.0f)
    fs::debug() << "MRISpercentDistanceError called when all dist_orig zero";
...
float mrisComputeArea(MRIS *mris, int fac, int n) {
  ...
  dot = mris->vertices[f->v[n]].x * d1 + mris->vertices[f->v[n]].y * d2 +
...
int MRIScomputeMetricProperties(MRIS *mris) {
  ...
  if (count_MRIScomputeMetricProperties_calls == debug_count) {
    fprintf(stdout, "b %s:%d   needed with debug_count:%d\n", __FILE__,
            __LINE__, count_MRIScomputeMetricProperties_calls);
  }

  bool const useOldBehaviour =
      !!getenv("FREESURFER_OLD_MRIScomputeMetricProperties");
  bool const useNewBehaviour =
      !!getenv("FREESURFER_NEW_MRIScomputeMetricProperties") ||
      !useOldBehaviour;
  ...
  if (useNewBehaviour) {
    MRISfreeDistsButNotOrig(
        mris); // So they can be stolen to avoid unnecessary mallocs and frees
    MRISMP_load(
        &mp,
        mris); // Copy the input data before MRIScomputeMetricPropertiesWkr changes it
    MRIScomputeMetricProperties(
        &mp); // It should not matter the order these are done in
  }
  ...
  if (useNewBehaviour)
    MRISMP_unload(mris, &mp, useOldBehaviour);
...
//
int load_orig_triangle_vertices(MRIS *mris, int fno, double U0[3], double U1[3],
                                double U2[3]) {
...
int load_triangle_vertices(MRIS *mris, int fno, double U0[3], double U1[3],
...
int MRISextractVertexCoords(MRIS *mris, float *locations[3], int which) {
  ...
  for (vno = 0; vno < nvertices; vno++) {
    ...
    default:
      ErrorExit(ERROR_UNSUPPORTED,
                "MRISextractVertexCoords: which %d not supported", which);
      break;
    case CURRENT_VERTICES:
      locations[0][vno] = v->x;
      locations[1][vno] = v->y;
...
int MRISltaMultiply(MRIS *surf, const LTA *lta) {
  ...
  if (vg_isEqual(&ltacopy->xforms[0].src, &ltacopy->xforms[0].dst)) {
    ...
    for (r = 1; r <= 4; r++) {
      for (c = 1; c <= 4; c++) {
        ...
        if (r == c && fabs(val - 1.0) > 10e-4)
          IsIdentity = 0;
        if (r != c && fabs(val) > 10e-4)
          IsIdentity = 0;
      }
    }
    if (!IsIdentity) {
  ...
  if (surf->vg.valid) {
    if (!vg_isEqual(&ltacopy->xforms[0].src, &surf->vg)) {
      if (!vg_isEqual(&ltacopy->xforms[0].dst, &surf->vg)) {
        ...
        printf("   src diff no %d\n",
               vg_isNotEqualThresh(&ltacopy->xforms[0].src, &surf->vg,
                                   vg_isEqual_Threshold));
        printf("   dst diff no %d\n",
               vg_isNotEqualThresh(&ltacopy->xforms[0].dst, &surf->vg,
                                   vg_isEqual_Threshold));
    ...
    printf("WARNING: source surface vg is not valid, assuming direction of "
           "transform is right\n");
  }
  ...
  if (MatrixDeterminant(ltacopy->xforms[0].m_L) < 0) {
    printf("Determinant of linear transform is negative, so reversing face "
           "order\n");
...
int MRIStransform(MRIS *mris, MRI *mri, TRANSFORM *transform, MRI *mri_dst) {
  ...
  if (transform->type == MORPH_3D_TYPE) {
    ...
    */
    ...
    if (!mri_dst) {
      dstNotGiven = 1;
      mri_dst     = MRIalloc(gcam->image.width, gcam->image.height,
                         gcam->image.depth, MRI_UCHAR);
      useVolGeomToMRI(&gcam->image, mri_dst);
    }
    if (!mri) {
      srcNotGiven = 1;
      mri = MRIalloc(gcam->atlas.width, gcam->atlas.height, gcam->atlas.depth,
                     MRI_UCHAR);
      ...
      if (mri && lt->src.valid == 1) {
        if (!(FZERO(lt->src.c_r - lt->src.c_r) &&
          ...
          fprintf(
              stderr,
              "WARNING:****************************************************\n");
          fprintf(stderr, "WARNING: c_(ras) values are not equal for the "
    ...
    if (!mri && lt->src.valid == 1) {
      ...
      mri        = MRIallocHeader(lt->src.width, lt->src.height, lt->src.depth,
                           MRI_UCHAR, 1);
      mri->x_r   = lt->src.x_r;
      mri->y_r   = lt->src.y_r;
      mri->z_r   = lt->src.z_r;
    ...
    // have it, get it from the surface
      ...
      mri      = MRIallocHeader(mris->vg.width, mris->vg.height, mris->vg.depth,
                           MRI_UCHAR, 1);
      mri->x_r = mris->vg.x_r;
      mri->y_r = mris->vg.y_r;
      mri->z_r = mris->vg.z_r;
    ...
    if (!mri_dst && lt->dst.valid == 1) {
      ...
      lt      = &lta->xforms[0];
      mri_dst = MRIallocHeader(lt->dst.width, lt->dst.height, lt->dst.depth,
                               MRI_UCHAR, 1);
      ...
      fprintf(stderr, "WARNING:************************************************"
                      "*********\n");
      fprintf(stderr, "WARNING: transform does not have valid destination "
                      "volume.       \n");
      fprintf(stderr, "WARNING: The standard CORONAL volume with c_(ras) = 0 "
                      "is assumed.\n");
      fprintf(stderr, "WARNING:************************************************"
                      "*********\n");
      mri_dst = MRIallocHeader(lt->dst.width, lt->dst.height, lt->dst.depth,
    ...
    if (lta->type == LINEAR_RAS_TO_RAS) {
      ...
      if (mri->width != mri_dst->width || mri->height != mri_dst->height ||
          mri->depth != mri_dst->depth) {
        fprintf(stderr, "WARNING:**********************************************"
                        "**********\n");
        fprintf(stderr, "WARNING:voxel-to-voxel transform must have the same "
                        "volume sizes.\n");
        fprintf(stderr,
                "WARNING:You gave src (%d, %dm, %d) vs. dst (%d, %d, %d).\n",
                mri->width, mri->height, mri->depth, mri_dst->width,
                mri_dst->height, mri_dst->depth);
        fprintf(stderr,
                "WARNING:The result of this transform is most likely wrong.\n");
        fprintf(stderr, "WARNING:**********************************************"
                        "**********\n");
      }
      voxelFromSurfaceRAS = voxelFromSurfaceRAS_(mri);
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      // transform vertex point to actual voxel point
      TransformWithMatrix(surfaceRASFromSurfaceRAS, v->x, v->y, v->z, &xw, &yw,
                          &zw);
      v->x = xw;
      v->y = yw;
      v->z = zw;
...
int          MRISsmoothCurvatures(MRIS *mris, int niterations) {
  ...
  for (i = 0; i < niterations; i++) {
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      norm = kernel[0] + vertext->vnum * kernel[1] +
             (vertext->v2num - vertext->vnum) * kernel[2];
      vertex->d = H / norm;
    }
    for (vno = 0; vno < mris->nvertices; vno++) {
      VERTEX *const vertex = &mris->vertices[vno];
...
MRISsavedXYZ *MRISsaveXYZ(MRIS *mris) {
  ...
  MRIS_XYZ *const p                            = saved->p =
      (MRIS_XYZ *)malloc(nvertices * sizeof(MRIS_XYZ));
  int vno;
  for (vno = 0; vno < nvertices; vno++) {
    VERTEX const *const v = &mris->vertices[vno];
    p[vno].x              = v->x;
...
float FACES_commonEdgeLength_find(MRIS *apmris, FACE *apFACE_I,
                                  FACE *apFACE_J) {
  //
  // PRECONDITIONS
  ...
  commonVertices =
      VERTICES_commonInFaces_find(apFACE_I, apFACE_J, pv_commonVertices);
  if (commonVertices != 2)
...
int mrisComputeOptimalPlane(MRIS *mris, int *vertices, int nvertices,
                            double *pxn, double *pyn, double *pzn, double *pxc,
                            double *pyc, double *pzc) {
  int     vno, n, vnum;
  MATRIX *M, *m_evectors;
  VERTEX *v;
  ...
  if (vnum < 4) {
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "mrisComputeOptimalPlane: must specify at "
                                "least 4 non-ripped verices\n"));
  }
...
int MRISvertexCoord2XYZ_float(VERTEX const *const v, int const which,
...
#define CASE(WHICH, FIELD)                                                     \
  ...
  default:
    ErrorExit(ERROR_UNSUPPORTED,
              "MRISvertexCoord2XYZ_float: unsupported which %d", which);
    break;
  }
...
int MRISvertexCoord2XYZ_double(VERTEX const *const v, int const which,
...
#define VERTEX_DIF(leg, v0, v1)                                                \
  leg[0] = v1->x - v0->x, leg[1] = v1->y - v0->y, leg[2] = v1->z - v0->z;
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
int mrisCalculateCanonicalFaceCentroid(MRIS *mris, int fno, float *px,
                                       float *py, float *pz) {
  float   x, y, z;
  VERTEX *v0, *v1, *v2;
  FACE *  f;
...
/*-----------------------------------------------------
  ...
  Scale a surface anisotropically.
  ------------------------------------------------------*/
static int mrisCalculateOriginalFaceCentroid(MRIS *mris, int fno, float *px,
                                             float *py, float *pz) {
  float   x, y, z;
  VERTEX *v0, *v1, *v2;
  FACE *  f;
...
/*-----------------------------------------------------
  ...
  Scale a surface anisotropically.
  ------------------------------------------------------*/
int mrisCalculateFaceCentroid(MRIS *mris, int fno, float *px, float *py,
                              float *pz) {
  float   x, y, z;
  VERTEX *v0, *v1, *v2;
  FACE *  f;
...
int mrisChooseFace(MRIS *mris, MHT *mht, VERTEX *v) {
  ...
  for (total_found = nfound = 0, dist = -.25; dist <= .25; dist += .25) {
    ...
    nfound = mrisAllNormalDirectionCurrentTriangleIntersections(mris, v, mht,
                                                                &d, fptr);
    if (nfound > 0) {
...
int MRISsampleStatVolume(MRIS *mris, void *vsv, int time_point, int coords) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    yv = nint(yt);
    zv = nint(zt);
    if (xv >= 0 && xv < width && yv >= 0 && yv < height && zv >= 0 &&
        zv < depth) {
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
double MRIScomputeFaceAreaStats(MRIS *mris, double *psigma, double *pmin,
                                double *pmax) {
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
double MRIScomputeVertexSpacingStats(MRIS *mris, double *psigma, double *pmin,
                                     double *pmax, int *pvno, int *pvno2,
                                     int which_vertices) {
  ...
  for (var = nv = total_dist = 0.0, vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vnum; n++) {
      ...
      default:
        ErrorExit(ERROR_BADPARM,
                  "MRIScomputeVertexSpacingStats: unsupported vertex set %d",
                  which_vertices);
        dist = 0;
        break;
      case ORIGINAL_VERTICES:
        dist = sqrt(SQR(vn->origx - v->origx) + SQR(vn->origy - v->origy) +
...
#endif
      case WHITE_VERTICES:
        dist = sqrt(SQR(vn->whitex - v->whitex) + SQR(vn->whitey - v->whitey) +
                    SQR(vn->whitez - v->whitez));
        break;
      case PIAL_VERTICES:
        dist = sqrt(SQR(vn->pialx - v->pialx) + SQR(vn->pialy - v->pialy) +
                    SQR(vn->pialz - v->pialz));
        break;
      case TARGET_VERTICES:
        dist = sqrt(SQR(vn->targx - v->targx) + SQR(vn->targy - v->targy) +
                    SQR(vn->targz - v->targz));
        break;
      }
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
double MRIScomputeTotalVertexSpacingStats(MRIS *mris, double *psigma,
...
static void MRISsetNeighborhoodSizeAndOptionallyDist(MRIS *mris, int nsize,
                                                     bool alwaysDoDist) {
...
static void MRISsetNeighborhoodSizeAndDistWkr(MRIS *mris, int nsize,
  ...
  */
  ...
  if (useOldBehaviour) {
    switch (
        copeWithLogicProblem("FREESURFER_fix_MRISsetNeighborhoodSizeAndDist",
                             "should not be creating dist_origs")) {
    case LogicProblemResponse_old:
      useOldBehaviour = true;
      break;
    case LogicProblemResponse_fix:
  ...
  bool shouldDoDist =
      alwaysDoDist || max_nsize_grew || mris->dist_nsize < nsize;
  bool shouldDoDistOrig = shouldDoDist && (mris->dist_alloced_flags & 2);

  if (useOldBehaviour) {
    if (shouldDoDist != alwaysDoDist) {
      copeWithLogicProblem(NULL, alwaysDoDist
                                     ? "new not computing dist when old did"
                                     : "old not computing dist");
      shouldDoDist = alwaysDoDist;
    }
    if (shouldDoDistOrig != alwaysDoDist) {
      copeWithLogicProblem(
          NULL, alwaysDoDist ? "new not computing dist_orig when old did"
                             : "old not computing dist_orig");
...
int face_barycentric_coords(double V0[3], double V1[3], double V2[3], double cx,
                            double cy, double cz, double *pl1, double *pl2,
                            double *pl3) {
  ...
  if (l1 > 0 && l1 < 1 && l2 > 0 && l2 < 1 && l3 > 0 && l3 < 1) {
    ...
    l2d = MIN(l2 - 0, 1 - l2);
    l3d = MIN(l3 - 0, 1 - l3);
    if ((l1d < 0 && !FZERO(l1d)) || (l2d < 0 && !FZERO(l2d)) ||
        (l3d < 0 && !FZERO(l3d))) {
...
int face_barycentric_coords(MRIS const *mris, int fno, int which_vertices,
                            double cx, double cy, double cz, double *pl1,
                            double *pl2, double *pl3) {
  auto surface = SurfaceFromMRIS::Analysis::Surface(const_cast<MRIS *>(mris));
  return face_barycentric_coords_template(surface, fno, which_vertices, cx, cy,
                                          cz, pl1, pl2, pl3);
}

int MRISsampleFaceCoords(MRIS *mris, int fno, double x, double y, double z,
                         int which_coords, int which_barycentric, float *px,
                         float *py, float *pz) {
  float   xv, yv, zv;
  double  lambda[3];
  int     n, ret;
  ...
  ret          = face_barycentric_coords(mris, fno, which_barycentric, x, y, z,
...
double MRISsampleFace(MRIS *mris, int fno, int which, double x, double y,
                      double z, double val0, double val1, double val2) {
  float   xv, yv, zv;
  double  lambda[3], val;
  int     n, ret;
  FACE *  face;
  ...
  xv = yv = zv = 0.0; // to get rid of mac warnings
  ret          = face_barycentric_coords(mris, fno, which, x, y, z, &lambda[0],
                                &lambda[1], &lambda[2]);
...
int MRISsampleFaceNormal(MRIS *mris, int fno, double x, double y, double z,
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISfindClosestVertex(MRIS *mris, float x, float y, float z, float *dmin,
                          int which_vertices) {
  int     vno, min_v = -1;
  VERTEX *v;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    default:
      ...
      dz = v->z - z;
      ErrorExit(ERROR_UNSUPPORTED,
                "MRISfindClosestVertex: unsupported vertex set %d\n",
                which_vertices);
      break;
    }
...
double MRISfindMinDistanceVertexWithDotCheck(MRI_SURFACE *mris, int c, int r,
  ...
  min_dist = MRISfindMinDistanceVertexWithDotCheckXYZ(mris, xs, ys, zs, mri,
                                                      dot_dir, pvtxno);
  return (min_dist);
}
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
float MRISdistanceToSurface(MRIS *mris, MHT *mht, float x0, float y0, float z0,
  ...
  for (dist = 0.0f; dist < 128; dist += .25) {
    if (mrisDirectionTriangleIntersection(mris, x0, y0, z0, nx, ny, nz, mht,
                                          &dist, -1)) {
      return (dist);
    }
  }
...
double MRIScomputeAnalyticDistanceError(MRIS *mris, int which, FILE *fp) {
  ...
  double     pct_orig, pct, mean, mean_orig_error, mean_error, smean_error,
      smean_orig_error;
  VECTOR *v1, *v2;
  ...
  if (fp) {
    ...
    fprintf(fp, "%2.3f  %2.3f  %2.3f  %2.3f  %2.3f  %2.3f  %2.3f\n",
            ...
            disturb_pct, 100.0 * pct_orig, mean_orig_error, 100.0 * pct,
            mean_error, measured_error);
#if 0
    fprintf(fp,
            "mean orig = %2.3f mm (%%%2.2f), final = %2.3f mm (%%%2.2f)\n",
            mean_orig_error, 100.0*pct_orig, mean_error, 100.0*pct) ;
...
int MRISdisturbOriginalDistances(MRIS *mris, double max_pct) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vtotal; n++) {
      v->dist_orig[n] *=
          (1.0 + randomNumber(-max_pct / 100.0f, max_pct / 100.0f));
    }
  }
...
int MRISdistanceTransform(MRIS *mris, LABEL *area, int mode) {
  ...
  if (cost == NULL) {
    ErrorExit(ERROR_NOMEMORY,
              "MRISdistanceTransform: could not allocate %d cost array\n",
              mris->nvertices);
  }
  vertices = (int *)calloc(mris->nvertices, sizeof(int));
  if (vertices == NULL) {
    ErrorExit(ERROR_NOMEMORY,
              "MRISdistanceTransform: could not allocate %d vertex array\n",
              mris->nvertices);
  }
  ...
  if (vertNbrs == NULL)
    ErrorExit(
        ERROR_NOMEMORY,
        "MRISdistanceTransform: could not allocate nbrs (%d x %d) array\n",
        mris->nvertices, max_nbrs);
  vertDists = (double *)calloc(mris->nvertices * max_nbrs, sizeof(double));
  if (vertDists == NULL)
    ErrorExit(
        ERROR_NOMEMORY,
        "MRISdistanceTransform: could not allocate dist (%d x %d) array\n",
        mris->nvertices, max_nbrs);
  ...
  MARS_DT_Boundary(vertices, mris->nvertices, max_nbrs, vertNbrs, vertDists,
                   cost);
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (vno = 0; vno < mris->nvertices; vno++)
      if (cost[vno] > 0) {
        mris->vertices[vno].val =
            mode == DTRANS_MODE_SIGNED ? -cost[vno] : cost[vno];
      }
  }
...
double MRIScomputeHausdorffDistance(MRIS *mris, int mode) {
  ...
  for (num = vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vnum; n++) {
      ...
      if (vn->val * v->val <
          0) // different signs - compute location of 0 between them
      {
        ...
        d = dist * fabs(v->val2) +
            (1 - dist) * fabs(vn->val2); // compute val2 at this point
        num++;
        hdist += d;
      }
          ...
          0) // different signs - compute location of 0 between them
      {
        ...
        num++;
        d = dist * fabs(v->val) +
            (1 - dist) * fabs(vn->val); // compute val2 at this point
        hdist += d;
      }
    }
  }
...
#define MAX_DIST 10
int MRISmeasureDistanceBetweenSurfaces(MRIS *mris, MRIS *mris2,
                                       int signed_dist) {
  int              vno;
  VERTEX *         v1, *v2;
  MRIS_HASH_TABLE *mht;
...
double MinDistToTriangleBF(double p1[3], double p2[3], double p3[3],
                           double ptest[3], double pmin[3], double dL) {
...
int MRISdistanceBetweenSurfacesExact(MRIS *surf1, MRIS *surf2) {
  ...
  for (vno1 = 0; vno1 < surf1->nvertices; vno1++) {
    ...
    if (vno1 == Gdiag_no) {
      printf("MRISdistanceBetweenSurfacesExact() %6d  %6d  %d a = "
             "[%7.3f,%7.3f,%7.3f]; b=[%7.3f,%7.3f,%7.3f]; \n",
...
short FACES_Hcurvature_determineSign(MRIS *apmris, int apFACE_O_fno,
  ...
  commonVertices =
      VERTICES_commonInFaces_find(apFACE_O, apFACE_I, pv_commonVertices);
  if (commonVertices != 2)
    ErrorExit(-4, "%s: During call %d, the passed faces do not share an edge",
              pch_function, calls);
...
int VERTEX_faceAngles_determine(MRIS *apmris, int avertex, VECTOR *apv_angle) {
  ...
  VERTEX_TOPOLOGY const *const pVERTEXt_apex =
      &apmris->vertices_topology[avertex];
  ...
  for (face = 0; face < nfaces; face++) {
    ...
    FaceNormCacheEntry const *const pFNorm_side =
        getFaceNorm(apmris, pVERTEXt_apex->f[face]);
...
int MRIS_Hcurvature_determineSign(MRIS *apmris) {
  ...
  for (vertex = 0; vertex < apmris->nvertices; vertex++) {
    ...
    signSum                               = 0;
    for (face = 0; face < nfaces; face++) {
      signSum += FACES_Hcurvature_determineSign(
          apmris, pVERTEXt->f[face], pVERTEXt->f[(face + 1) % nfaces]);
    }
...
short MRIS_discreteKH_compute(MRIS *apmris) {
  ...
  for (vertex = 0; vertex < apmris->nvertices; vertex++) {
    ...
    for (face = 0; face < nfaces; face++) {
      ...
      f_angleNormalIJ =
          FACES_angleNormal_find(apmris, pFaceIndex[faceI], pFaceIndex[faceJ]);
...
short MRIScomputeSecondFundamentalFormDiscrete(MRIS *apmris,
...
int MRISbinarizeCurvature(MRIS *mris, float thresh, float low, float high,
                          int use_abs) {
  int     vno;
  VERTEX *v;
  float   val;
        ...
        for (n = 0; n < vnt->vnum; n++) {
          ...
          if (Gdiag_no == vno) {
            printf("vno=%d A %3d %6d (%g,%g,%g) (%g,%g,%g) %g %g\n", vno, n,
                   vnt->v[n], v->origx, v->origy, v->origz, v->x, v->y, v->z,
                   dist, min_dist);
          }
          if (dist < min_dist) {
            min_n    = ns;
        ...
        for (n = 0; n < vnt->vnum; n++) {
          ...
          if (Gdiag_no == vno) {
            printf("vno=%d B %3d %6d (%g,%g,%g) (%g,%g,%g) %g %g\n", vno, n,
                   vnt->v[n], vn2->origx, vn2->origy, vn2->origz, v->x, v->y,
                   v->z, dist, min_dist);
          }
          if (dist < min_dist) {
            min_n    = ns;
...
void computeVertexPseudoNormal(MRIS const *mris, int vno, float norm[3],
                               int verbose) {
  int                          n, n0, n1, n2;
  ...
  for (n = 0; n < vt->num; n++) {
    ...
    if ((face->v[n0] != vno) || (face->v[n1] == vno) || (face->v[n2] == vno) ||
        (face->v[n2] == face->v[n1])) {
      if (verbose >= VERBOSE_MODE_MEDIUM) {
        ...
        fprintf(WHICH_OUTPUT, "face %d (%d,%d,%d) != (%d)\n", vt->f[n],
                face->v[n0], face->v[n1], face->v[n2], vno);
        ...
        if (verbose == VERBOSE_MODE_HIGH) {
          ErrorExit(ERROR_BADPARM,
                    "computeVertexPseudoNormal: SHOULD NOT HAPPEN\n");
        }
      }
    }
...
int MRIScomputeNormal(MRIS *mris, int which, int vno, double *pnx, double *pny,
                      double *pnz) {
  ...
  for (num = n = 0; n < vt->num; n++)
    if (!mris->faces[vt->f[n]].ripflag) {
      ...
      case PIAL_VERTICES:
        ...
        break;
      default:
        ErrorExit(ERROR_BADPARM, "MRIScomputeNormal: which = %d not supported",
                  which);
        break;
      }
      norm[0] += snorm[0];
...
template <class Surface>
void computeDefectFaceNormal_calculate_template(Surface surface, int const fno,
                                                float *p_nx, float *p_ny,
                                                float *p_nz,
                                                float *p_orig_area) {
...
static void computeDefectFaceNormal_calculate(MRIS const *const mris,
                                              int const fno, float *p_nx,
                                              float *p_ny, float *p_nz,
                                              float *p_orig_area) {
  SurfaceFromMRIS::XYZPositionM::Surface surface(const_cast<MRIS *>(mris));
  computeDefectFaceNormal_calculate_template(surface, fno, p_nx, p_ny, p_nz,
                                             p_orig_area);
}

void computeDefectFaceNormal_calculate(MRIS_MP const *const mris, int const fno,
                                       float *p_nx, float *p_ny, float *p_nz,
                                       float *p_orig_area) {
  SurfaceFromMRIS_MP::XYZPositionM::Surface surface(
      const_cast<MRIS_MP *>(mris));
  computeDefectFaceNormal_calculate_template(surface, fno, p_nx, p_ny, p_nz,
                                             p_orig_area);
}
...
void setFaceNorm_template(SOME_MRIS const *const mris, int fno, float nx,
                          float ny, float nz) {
...
void setFaceNorm(MRIS const *const mris, int fno, float nx, float ny,
                 float nz) {
  setFaceNorm_template(mris, fno, nx, ny, nz);
}

void setFaceNorm(MRIS_MP const *const mris, int fno, float nx, float ny,
                 float nz) {
  setFaceNorm_template(mris, fno, nx, ny, nz);
}
...
FaceNormCacheEntry const *getFaceNorm_template(SOME_MRIS const *const mris,
  ...
  FaceNormDeferredEntry volatile *fNormDeferred =
      &mris->faceNormDeferredEntries[fno];
...
void mrisurf_deferSetFaceNorms(MRIS *mris) {
  ...
  if (!use_parallel) {
    ...
    int fno;
    for (fno = 0; fno < mris->nfaces; fno++) {
      FaceNormDeferredEntry *fNormDeferred =
          &mris->faceNormDeferredEntries[fno];
      computeDefectFaceNormal_calculate(
          mris, fno, &fNorm->nx, &fNorm->ny, &fNorm->nz,
          &fNorm->orig_area);      // compute it now so can check later
      fNormDeferred->deferred = 3; // compute them again later
    }
#else
    if (sizeof(mris->faceNormDeferredEntries[0]) != 1)
      *(int *)(-1) = 0;
...
#endif
        for (fno = 0; fno < mris->nfaces; fno++) {
      ROMP_PFLB_begin FaceNormDeferredEntry *fNormDeferred =
          &mris->faceNormDeferredEntries[fno];
#ifdef CHECK_DEFERED_NORMS
      computeDefectFaceNormal_calculate(
          mris, fno, &fNorm->nx, &fNorm->ny, &fNorm->nz,
...
#ifdef CHECK_DEFERED_NORMS
    if (isRecompute)
      fNormDeferred->deferred = 3;
    getFaceNorm(mris, fno);
#else
    FaceNormCacheEntry *fNorm = &mris->faceNormCacheEntries[fno];
    if (!isRecompute && !fNormDeferred->deferred)
      continue;
    computeDefectFaceNormal_calculate(mris, fno, &fNorm->nx, &fNorm->ny,
                                      &fNorm->nz, &fNorm->orig_area);
    fNormDeferred->deferred = 0;
#endif
    ROMP_PFLB_end
  }
...
int MRIScomputeAllDistances(MRIS *mris) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISdistanceTransform(
        mris, area,
        DTRANS_MODE_UNSIGNED); // reads v->vnum distances out of v->dist
    ...
    for (n = 0; n < old_vtotal; n++) {
      ...
      v->dist[n]       = v->dist_orig[n] =
          vn->val; // v->dist will be restored by caller
      vn->marked = 1;
    }
    ...
    for (vno2 = 0; vno2 < mris->nvertices; vno2++) {
      ...
      v->dist[n] = v->dist_orig[n] =
          vn->val; // set to a different definition of 'distance'
      vt->v[n++] = vno2;
      vn->marked = 1;
...
int containsAnotherVertexOnSphere(MRIS *mris, int vno0, int vno1, int vno2,
                                  int mode) {
...
#else
int containsAnotherVertex(MRIS *mris, int vno0, int vno1, int vno2,
...
#else
{
  int     n, vno, i;
  VERTEX *v0, *v1, *v2, *v;
  double U0[3], U1[3], U2[3], dot, L0[3], L1[3], *V0, *V1, *V2, desc[3], cx, cy,
      cz, Point[3], len, norm_proj[3], U[3];
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISsurfaceRASToTalairachVoxel(MRIS *mris, MRI *mri, double xw, double yw,
                                   double zw, double *pxv, double *pyv,
                                   double *pzv) {
  //   conformed -->  SRAS
  //       |            |
  //       |            | SRASToRAS
  ...
  *MATRIX_RELT(SRASToRAS, 3, 4) = lt->src.c_s;
  ...
  ///////////////////////////////////////////////////
  talgeom = lt->dst;
  talVol  = MRIallocHeader(talgeom.width, talgeom.height, talgeom.depth,
                          MRI_UCHAR, 1);
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISvertexToVoxel(MRIS *mris, VERTEX *v, MRI *mri, double *pxv, double *pyv,
                      double *pzv) {
  double xw, yw, zw;
...
int MRISvertexNormalToVoxel(MRIS *mris, VERTEX *v, MRI *mri, double *pnx,
                            double *pny, double *pnz) {
  double xw, yw, zw;
...
int MRISvertexNormalToVoxelScaled(MRIS *mris, VERTEX *v, MRI *mri, double *pnx,
                                  double *pny, double *pnz) {
  double nx, ny, nz, len;
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISvertexCoordToVoxel(MRIS *mris, VERTEX *v, MRI *mri, int coords,
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISorigVertexToVoxel(MRIS *mris, VERTEX *v, MRI *mri, double *pxv,
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISwhiteVertexToVoxel(MRIS *mris, VERTEX *v, MRI *mri, double *pxv,
                           double *pyv, double *pzv) {
  double xw, yw, zw;
...
int MRISmeasureThicknessFromCorrespondence(MRIS *mris, MHT *mht,
  ...
  if (mht == NULL) {
    mht     = MHTcreateFaceTable_Resolution(mris, CANONICAL_VERTICES,
                                        1.0); // to lookup closest face
    alloced = 1;
  } else
    alloced = 0;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISsampleFaceCoordsCanonical(mht, mris, v->x, v->y, v->z, PIAL_VERTICES,
                                  &xp, &yp, &zp);
    dx      = xp - xw;
    dy      = yp - yw;
    dz      = zp - zw;
...
int mrisComputeNormalDotDistribution(MRIS *mris, HISTOGRAM *h_dot) {
  ...
  float bin_size, min_dot, max_dot, bin_val, dot, dx, dy, dz, nx, ny, nz, x, y,
      z;
  HISTOGRAM *h_raw;
  ...
  for (bin_val = min_dot - bin_size, bin = 0; bin < h_dot->nbins;
...
int mrisComputePrincipalCurvatureDistributions(MRIS *mris, HISTOGRAM *h_k1,
                                               HISTOGRAM *h_k2,
                                               MRI *      mri_k1_k2) {
  ...
  for (bin_val = min_k1, bin = 0; bin < h_k1->nbins;
       bin++, bin_val += k1_bin_size) {
    h_k1->bins[bin] = bin_val;
  }
  for (bin_val = min_k2, bin = 0; bin < h_k2->nbins;
       bin++, bin_val += k2_bin_size) {
...
//=====================================================================================================================
static int get_face_axes(MRIS *mris, FACE *face, float *pe1x, float *pe1y,
                         float *pe1z, float *pe2x, float *pe2y, float *pe2z) {
...
void INTEGRATION_PARMS_openFp(INTEGRATION_PARMS *parms, const char *name,
                              const char *mode) {
...
void INTEGRATION_PARMS_copyFp(INTEGRATION_PARMS *      dst,
                              INTEGRATION_PARMS const *src) {
  INTEGRATION_PARMS_setFp(dst, src->fp);
}

static double ashburnerTriangleEnergy(MRIS *mris, int fno, double lambda) {
  static MATRIX *_m_x[_MAX_FS_THREADS], *_m_x_inv[_MAX_FS_THREADS],
      *_m_y[_MAX_FS_THREADS], *_m_J[_MAX_FS_THREADS], *_m_U[_MAX_FS_THREADS],
      *_m_V[_MAX_FS_THREADS], *_m_m[_MAX_FS_THREADS],
      *_m_evectors[_MAX_FS_THREADS];
  MATRIX *m_x, *m_x_inv, *m_y, *m_J, *m_U, *m_V, *m_m, *m_evectors = NULL;
  FACE *  f;
  VERTEX *v1, *v2, *v3;
  float   e1x, e1y, e1z, e2x, e2y, e2z, x11, x12, x13, x21, x22, x23, y11, y12,
      y13, y21, y22, y23, evalues[2], det, s11, s22;
...
#define SMALL 1e-8
  ...
  energy =
      lambda * (1 + det) * (s11 + s22); // log and square of snn already taken
...
float mrisSampleAshburnerTriangleEnergy(MRIS *const mris, int const vno,
                                        INTEGRATION_PARMS *const parms,
...
static void MRISaverageGradients_optimize() {
  ...
  if (!once) {
    ...
    if (!optimize) {
      fprintf(stdout, "FREESURFER_MRISaverageGradients_optimize_suppress so "
                      "not optimizing\n");
    }
  }
  ...
  int const controls_and_data_size =
      MRISaverageGradients_controls_and_data_size;
  const MRISaverageGradients_Control *const controls =
      MRISaverageGradients_controls;
  const MRISaverageGradients_Data *const datas_init =
      MRISaverageGradients_datas_init;
  ...
  new2old_indexMap =
      (int *)realloc(new2old_indexMap, controls_and_data_size * sizeof(int));
  int ni;
  for (ni = 0; ni < controls_and_data_size; ni++) {
    new2old_indexMap[ni] = ni;
  }

  sort_MRISaverageGradients_index(new2old_indexMap, controls_and_data_size,
                                  true);

  // compute the inverse
  old2new_indexMap =
      (int *)realloc(old2new_indexMap, controls_and_data_size * sizeof(int));
  for (ni = 0; ni < controls_and_data_size; ni++) {
  ...
  Data *const new_datas_init =
      (Data *)malloc(controls_and_data_size * sizeof(Data));
  Control *const new_controls =
      (Control *)malloc(controls_and_data_size * sizeof(Control));
  ...
  for (ni = 0; ni < controls_and_data_size; ni++) {
    ...
    for (j = 0; j < numNeighbors; j++) {
      new_neighbors[new_neighborsSize++] =
          old2new_indexMap[neighbors[old_firstNeighbor + j]];
    }
    new_datas_init[ni] = datas_init[old_index];
  }

  free(MRISaverageGradients_datas_init);
  MRISaverageGradients_datas_init = new_datas_init;
  free(MRISaverageGradients_controls);
  MRISaverageGradients_controls = new_controls;
  free(MRISaverageGradients_neighbors);
  MRISaverageGradients_neighbors = new_neighbors;
...
static void
MRISaverageGradients_write(const char *filename, int num_avgs,
                           int neighbors_size, const int *neighbors,
  ...
  for (i = 0; i < index_to_vno_size; i++) {
    fprintf(f, "  %5d %5d %5d %g %g %g\n", i, controls[i].numNeighbors,
            controls[i].firstNeighbor, datas_inp[i].dx, datas_inp[i].dy,
            datas_inp[i].dz);
  }
  fclose(f);
...
int MRISaverageGradients(MRIS *mris, int num_avgs) {
  ...
  if (Gdiag_no >= 0) {
    ...
    fprintf(stdout, "before averaging %d times dot = %2.2f ", num_avgs,
    ...
    if (doNew) {
      ...
      for (vno = 0; vno < mris->nvertices; vno++) {
        ...
        if (!v->ripflag) {
          ...
          for (vnb = 0; vnb < vnum; vnb++) {
            int     neighborVno = *pnb++;
            VERTEX *vn =
                &mris->vertices[neighborVno]; // neighboring vertex pointer
            if (vn->ripflag)
              continue;
            num++;
          }
      ...
      for (index = 0; index < index_to_vno_size; index++) {
        ...
        for (vnb = 0; vnb < vnum; vnb++) {
          ...
          if (neighborIndex < 0)
            continue; // ripflag must be set
          if (neighbors_size == neighbors_capacity) {
            int const new_neighbors_capacity =
                neighbors_capacity + neighbors_capacity / 2;
            int *const new_neighbors =
                (int *)malloc(sizeof(int) * new_neighbors_capacity);
            int i;
            for (i = 0; i < neighbors_size; i++)
      ...
      if (0) {
        static int once;
        if (!once++) {
          MRISaverageGradients_write("tesselation_averaging_data.txt", num_avgs,
                                     neighbors_size, neighbors,
                                     index_to_vno_size, controls, datas_inp);
        }
      }
      ...
      if (0) {
        ...
        if (!once++) {
          MRISaverageGradients_write("tesselation_averaging_data.optimized.txt",
                                     num_avgs, neighbors_size, neighbors,
                                     index_to_vno_size, controls, datas_inp);
        }
      }
...
#define chunksCapacity 17 // 4 * omp_get_max_threads() + 1 // must be at least 3
      ...
      size_t chunksSize = 0;
      {
        ...
        for (index = 0; index < index_to_vno_size; index++) {
          ...
          if (chunkSumNumNeighbors < neighborsPerChunk)
            continue;
          chunks[chunksSize++].indexLo = index;
          chunkSumNumNeighbors         = 0;
          if (chunksSize == chunksCapacity - 1)
            break;
        }
...
#pragma omp parallel for if_ROMP(assume_reproducible)
          ...
          int indexLo = chunks[chunksIndex].indexLo,
...
#pragma omp parallel for if_ROMP(experimental) schedule(static, 1)
      ...
      for (vno = 0; vno < mris->nvertices; vno++) {
        ...
        if (old_index == -1) {
          ...
          if (!closeEnough(d->dx, v->dx) || !closeEnough(d->dy, v->dy) ||
            ...
            if (errors < 10) {
              fprintf(stdout,
                      "%s:%d vno:%d d->dx:%g v->dx:%g d->dy:%g v->dy:%g "
                      "d->dz:%g v->dz:%g - datas_inp\n",
                      __FILE__, __LINE__, vno, d->dx, v->dx, d->dy, v->dy,
                      d->dz, v->dz);
            }
          }
        }
    ...
    if (doNew) {
      ...
      for (new_index = 0; new_index < index_to_vno_size; new_index++) {
        ...
        int old_index = MRISaverageGradients_new2old_indexMap
                            ? MRISaverageGradients_new2old_indexMap[new_index]
                            : new_index;
  ...
  if (Gdiag_no >= 0) {
    ...
    fprintf(stdout,
            " after dot = %2.2f D = (%2.2f, %2.2f, %2.2f), N = (%2.1f, %2.1f, "
            "%2.1f)\n",
...
int MRIScomputeMeanCurvature(MRIS *mris) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (i = 0; i < vertext->vnum; i++) /* for each neighbor */
    {
      ...
      VECTOR_LOAD(v_i, vnb->x - vertex->x, vnb->y - vertex->y,
                  vnb->z - vertex->z);
...
int MRIScomputeSecondFundamentalFormThresholded(MRIS *mris, double pct_thresh) {
  ...
  if (pct_thresh >= 0) {
    vmean = MRIScomputeTotalVertexSpacingStats(mris, &vsigma, NULL, NULL, NULL,
                                               NULL);
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    do {
      ...
      for (n = i = 0; i < vertext->vtotal; i++) {
        ...
        */
        VECTOR_LOAD(v_yi, vnb->x - vertex->x, vnb->y - vertex->y,
                    vnb->z - vertex->z);
        ui = V3_DOT(v_yi, v_e1);
        vi = V3_DOT(v_yi, v_e2);
    ...
    m_tmp2 = MatrixMultiply(
        m_Ut, m_U, m_tmp2); /* Ut U  cols x rows * rows x cols = cols x cols */
    cond_no = MatrixConditionNumber(m_tmp2);

    MatrixBuffer m_inverse_buffer;
    m_inverse = MatrixAlloc2(m_tmp2->cols, m_tmp2->rows, MATRIX_REAL,
                             &m_inverse_buffer);
    ...
    if (vno == Gdiag_no && (Gdiag & DIAG_SHOW))
      fprintf(stdout, "v %d: k1=%2.3f, k2=%2.3f, K=%2.3f, H=%2.3f\n", vno,
              vertex->k1, vertex->k2, vertex->K, vertex->H);
...
int MRIScomputeSecondFundamentalFormAtVertex(MRIS *mris, int vno, int *vertices,
                                             int vnum) {
  int            i, n, nbad = 0;
  VERTEX *       vertex, *vnb;
...
PerThreadMRIDistance *makePerThreadMRIDistance(MRI const *const mri_distance) {
  PerThreadMRIDistance *ptd =
      (PerThreadMRIDistance *)malloc(sizeof(PerThreadMRIDistance));
...
void updateDistanceElt(volatile float *f, float distance, bool lockNeeded) {
  ...
  if (lockNeeded) {
    if (fabsf(distance) > fabsf(*f))
      return; // avoid the lock if possible
#pragma omp critical
...
void updateDistanceEltFromSignArgAndSquareLockNeeded(volatile float *f,
  ...
  //
  if (squaref(*f) * 1.01f < distanceSquared)
    return;
...
void updateDistanceEltFromSignArgAndSquareNoLockNeeded(
  ...
  //
  if (squaref(*f) * 1.01f < distanceSquared)
    return; // don't wait for the sqrtf to complete if it is not needed
...
int MRISusePrincipalCurvatureFunction(MRIS *pmris,
...
int mriSurfaceRASToVoxel(double xr, double yr, double zr, double *xv,
  ...
  bool const new_way =
      try_new_way && VoxelFromSRASmatrix->type == MATRIX_REAL &&
      VoxelFromSRASmatrix->rows == 4 && VoxelFromSRASmatrix->cols == 4;

  bool const old_way = try_old_way || !new_way;

  float nx = 0, ny = 0, nz = 0;
  if (new_way) {
    float fxr = xr, fyr = yr, fzr = zr; // cvt to float to match old way
    nx = (double)*MATRIX_RELT(VoxelFromSRASmatrix, 1, 1) * fxr +
    ...
    ny = (double)*MATRIX_RELT(VoxelFromSRASmatrix, 2, 1) * fxr +
    ...
    nz = (double)*MATRIX_RELT(VoxelFromSRASmatrix, 3, 1) * fxr +
...
//////////////////////////////////////////////////////////////
MRI *MRISbinarizeVolume(MRIS *mris, MRI_REGION *region, float resolution,
...
#pragma omp parallel for if_ROMP(assume_reproducible)
    ...
    ROMP_PFLB_begin computeDefectFaceNormal(mris, p);
    ROMP_PFLB_end
  }
  ROMP_PF_end
  ...
  for (count = 0, vn0 = 0; vn0 < mris->nvertices; vn0++) {
    ...
    for (p = 0; p < vt->vnum; p++) {
      ...
      if (mris->vertices[vn1].ripflag)
        fprintf(WHICH_OUTPUT, "vertex %d connected to ripped vertex %d !\n ",
                vn0, vn1);
      ...
      if (nfaces == 2 && (computeOrientation(mris, face1, vn0, vn1) *
                              computeOrientation(mris, face2, vn0, vn1) >
                          0)) {
        v->curv = -1;
        fprintf(WHICH_OUTPUT,
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
#if (!SPHERE_INTERSECTION)
static int mrisComputeCanonicalEdgeBasis(MRIS *mris, EDGE *edge1, EDGE *edge2,
    ...
    for (m = 0; m < VERTICES_PER_FACE; m++) {
      ...
      if (n == vt->num) /* face has vertex, but vertex doesn't have face */
      {
        printf("mrisCheckSurface: %s: face[%d].v[%d] = %d, "
               ...
               "face list\n",
               mris->fname.data(), fno, m, f->v[m], fno, f->v[m]);
        DiagBreak();
      }
    }
  }
  ...
  for (nbad = vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vnum; n++) {
      ...
      for (nfaces = m = 0; m < vt->vnum; m++) {
        ...
        if (vertexNeighbor(mris, vno2, vt->v[m]) &&
            isFace(mris, vno, vno2, vt->v[m])) {
          flist[nfaces] = findFace(mris, vno, vno2, vt->v[m]);
          nfaces++;
        }
      }
      if (((mris->patch == 0) && (nfaces != 2)) ||
          (mris->patch && (nfaces < 1 || nfaces > 2))) {
        int i;
        ...
        fprintf(stdout, "%d %d %d !!!\n", mris->faces[flist[0]].v[0],
                mris->faces[flist[0]].v[1], mris->faces[flist[0]].v[2]);
...
int MRISmaskLabel(MRIS *mris, LABEL *area) {
  ...
  for (i = 0; i < area->n_points; i++) {
    ...
    v->curv = v->stat = v->val = v->imag_val = v->val2 = v->valbak =
        v->val2bak                                     = 0.0;
  }
  return (NO_ERROR);
}
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    for (nthhop = 1; nthhop < N; nthhop++) {
      ...
      if (nnbrs_actual != 0)
        MRIFseq_vox(arN, c, r, s, nthhop) = (arsum / nnbrs_actual);
    } /* end loop over hop */
...
/*-----------------------------------------------------------------------
  ...
  kernel = ACF^2
  -----------------------------------------------------------------------*/
MRI *MRISsmoothKernel(MRIS *surf, MRI *src, MRI *mask, MRI *mrikern,
                      MATRIX *globkern, SURFHOPLIST ***pshl, MRI *out) {
  int           vtx, **crslut, nvox;
  double *      kern;
  int           n, nhops;
  SURFHOPLIST **shl;
  ...
  if (mrikern && globkern) {
    printf(
        "ERROR: MRISsmoothKernel(): cannot spec both mrikern and globkern\n");
    return (NULL);
  }
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    for (frame = 0; frame < src->nframes; frame++) {
      ...
      if (nnbrs_actual != 0)
        MRIFseq_vox(out, c, r, s, frame) = (kvsum / ksum);
...
int MRISextendedNeighbors(MRIS *SphSurf, int TargVtxNo, int CurVtxNo,
                          ...
                          double *XNbrDotProd, int *nXNbrs, int nXNbrsMax,
                          int DistType) {
  static int ncalls = 0;
  VERTEX *   vtarg, *vcur;
  ...
  if (DistType == 1) {
    ...
    DotProd =
        (vtarg->x * vcur->x) + (vtarg->y * vcur->y) + (vtarg->z * vcur->z);
...
MRI *MRIcomputeLaminarVolumeFractions(MRIS *mris, double resolution,
  ...
  nvox = nint(
      MAX(MAX((mri_src->xsize / resolution), (mri_src->ysize / resolution)),
  ...
  if (mri_fractions == NULL) {
    mri_fractions = MRIallocSequence(mri_src->width, mri_src->height,
...
void mrisFindMiddleOfGray(MRIS *mris) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISsetXYZ(mris, vno, v->origx + thickness * nx, v->origy + thickness * ny,
...
MRIS *MRISunion(MRIS const *mris, MRIS const *mris2) {
  ...
  for (vno = 0, vno3 = 0; vno < mris->nvertices; vno++, vno3++) {
    MRISsetXYZ(mris3, vno3, mris->vertices[vno].x, mris->vertices[vno].y,
               mris->vertices[vno].z);
  }
  for (vno2 = 0; vno2 < mris2->nvertices; vno2++, vno3++) {
    MRISsetXYZ(mris3, vno3, mris2->vertices[vno2].x, mris2->vertices[vno2].y,
...
MRIS *MRISclone(MRIS const *mris_src) {
  ...
  for (fno = 0; fno < mris_src->nfaces; fno++) {
    ...
    if (fsrc->norm)
      fdst->norm = DMatrixCopy(fsrc->norm, NULL);
    int i;
    for (i = 0; i < 3; i++) {
      if (fsrc->gradNorm[i])
        fdst->gradNorm[i] = DMatrixCopy(fsrc->gradNorm[i], NULL);
    }
  }
      ...
      for (evno = 0; evno < 2; evno++) {
        if (e->vtxno[evno] == vertexno) {
          printf("Edge %6d %d  %4d %4d %7.3lf %7.3lf\n", edgeno, evno,
                 e->vtxno[0], e->vtxno[1], e->len, e->angle);
        }
      }
    }
  }
...
int MRISprintSurfQualityStats(FILE *fp, MRIS *surf) {
  ...
  // mean, stddev, min, max
  fprintf(fp, "%7.5f %7.5f %7.5f %7.5f  ", astats[1], astats[2], astats[3],
          astats[4]);
  fprintf(fp, "%7.5f %7.5f %7.5f %7.5f  ", cstats[1], cstats[2], cstats[3],
          cstats[4]);
  fprintf(fp, "%7.5f %7.5f %7.5f %7.5f  ", estats[1], estats[2], estats[3],
          estats[4]);
  fprintf(fp, "%7.5f %7.5f %7.5f %7.5f  ", hstats[1], hstats[2], hstats[3],
          hstats[4]);
  fprintf(fp, "\n");
  fflush(fp);
...
int MRISprettyPrintSurfQualityStats(FILE *fp, MRIS *surf) {
  ...
  // mean, stddev, min, max
  fprintf(fp, "Area   %7d %8.5f %8.5f %8.6f %8.4f\n", (int)astats[0], astats[1],
          astats[2], astats[3], astats[4]);
  fprintf(fp, "Corner %7d %8.5f %8.5f %8.6f %8.4f\n", (int)cstats[0], cstats[1],
          cstats[2], cstats[3], cstats[4]);
  fprintf(fp, "Edge   %7d %8.5f %8.5f %8.6f %8.4f\n", (int)estats[0], estats[1],
          estats[2], estats[3], estats[4]);
  fprintf(fp, "Hinge  %7d %8.5f %8.5f %8.6f %8.4f\n", (int)hstats[0], hstats[1],
          hstats[2], hstats[3], hstats[4]);
  if (0) {
