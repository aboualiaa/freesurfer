...
int tri_tri_intersect(double V0[3], double V1[3], double V2[3], double U0[3],
  ...
  du0du1 = du0 * du1;
  du0du2 = du0 * du2;
  if ((fdu0 > EPSILON || fdu1 > EPSILON || fdu2 > EPSILON) &&
      (du0du1 > 0.0f && du0du2 > 0.0f))
  ...
  if ((fdv0 > EPSILON || fdv1 > EPSILON || fdv2 > EPSILON) &&
      (dv0dv1 > 0.0f && dv0dv2 > 0.0f))
    return 0; /* no intersection occurs */
#if USE_EPSILON_TEST == TRUE
  if (fabs(dv0) < EPSILON)
    dv0 = 0.0;
...
int triangle_ray_intersect(double orig_pt[3], double dir[3], double U0[3],
  ...
  double basis1[3], basis2[3], tmp[3], L0[3], L1[3], len, t, dot, norm_proj[3],
      *V0, *V1, *V2, desc[3], Point[3];
  // douoble a, b;
  MATRIX *m_U, *m_inv;
  VECTOR *v_p, *v_r;
  int i;
  ...
  */
  ...
  m_inv = MatrixSVDInverse(m_U, nullptr);
...
int intersect_RayTriangle(double ray[2][3], double V0[3], double V1[3],
...
int project_point_to_plane(double point[3], double V0[3], double V1[3],
                           double V2[3], double proj[3], double pe1[3],
                           double pe2[3]) {
