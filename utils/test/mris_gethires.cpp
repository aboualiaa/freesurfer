...
int main(int argc, char *argv[]) {
  MRI *src = 0;
  if (argc < 2) {
    std::cerr << "Usage: mris_gethighres <surface> <highresvol> <outsurf> "
                 "[<lowresvol>]"
              << std::endl;
    std::cerr
        << "   where lowresvol is only used when surface does not contain the "
           "volume info"
        << std::endl;
    return 0;
  }
  ...
  if (!mris) {
    std::cerr << "could not load surface: " << argv[1] << std::endl;
    return 0;
  }
  if (mris->vg.valid == 0) {
    std::cerr << "Trying to read lowres volume to get info" << std::endl;
    src = MRIreadHeader(argv[4], MRI_VOLUME_TYPE_UNKNOWN);
    if (!src) {
      std::cerr << "could not read lowres volume : " << argv[2] << std::endl;
      return 0;
    }
  ...
  if (!mri_hires) {
    std::cerr << "could not highres volume : " << argv[2] << std::endl;
    return 0;
  }
