...
MRI *ComputeDifference(MRI_SURFACE *Mesh1, MRI *mri_data1, MRI_SURFACE *Mesh2,
                       MRI *mri_data2, MRI *mri_res);
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_diff.cpp,v 1.11 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  ...
  if (debugflag) {
    printf("Data%d at vertex %d has value %g\n", 1, debugvtx,
           MRIFseq_vox(SrcVal1, debugvtx, 0, 0, 0));
  }

  if (register_flag && (xform_fname != NULL)) {
    printf("A LTA is specified, so only apply the LTA, and no ICP will be "
           "computed\n");
    register_flag = 0;
  }
  ...
  if (label_fname != NULL) {
    ...
    mri_distance =
        MRIallocSequence(width, height, depth, MRI_FLOAT, mri_label->nframes);
    ...
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          if (label == 4 || label == 5 || label == 9 || label == 10 ||
              label == 11 || label == 12 || label == 14 || label == 15 ||
              label == 16 || label == 28 || label == 30 || label == 43 ||
              label == 44 || label == 48 || label == 49 || label == 50 ||
              label == 51 || label == 62 || label == 60)
  ...
  std = 0.0;
  for (index = 0; index < Surf1->nvertices; index++) {
    if (Surf1->vertices[index].border == 1)
      continue;
    ...
    if (maplike_fname && mapout_fname) {
      ...
      if (x < 0 || x >= mri_map->width || y < 0 || y >= mri_map->height ||
          z < 0 || z >= mri_map->depth)
        continue;
  ...
  printf("difference stats: max = %g, min = %g, mean = %g, abs_mean = %g, std "
         "= %g\n",
...
static void print_usage(void) {
  ...
  fprintf(stdout,
          "   -register  Force a rigid registration of surface2 to surface1\n");
  fprintf(stdout, "   -xform %%s apply LTA transform to align input surface1 "
                  "to  surface2\n");
...
static void print_help(void) {
  print_usage();
  printf(
      "This program computes the difference of two surface data sets defined "
      "on the \n"
      ...
      "  -trg_type typestring\n"
      "\n"
      "    Format type string. Can be paint or w (for FreeSurfer paint "
      "files).\n"
      "\n"
      "  -out filename\n"
      "\n"
      "  Output the difference to the paint file, note the # of entries is "
      "same as VN of surface1."
      "\n"
      "  -nsmooth #\n"
      "\n"
      ...
      "  -register\n"
      "\n"
      "  Perform ICP rigid registration before computing closet-vertex "
      "difference."
      "\n"
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    register_flag = 1;
    printf(
        "Do a rigid alignment of two surfaces before mapping to each other\n");
    ...
    nargs = 1;
  } else if (!stricmp(option, "out") || !stricmp(option, "out_file") ||
...
MRI *ComputeDifferenceNew(MRI_SURFACE *Mesh1, MRI *mri_data1,
  ...
  for (index = 0; index < Mesh1->nvertices; index++) {
    ...
    /* annIndex gives the closest vertex on mris_template to the vertex #index
     * of mris */
    ...
    value = sumcurv - MRIgetVoxVal(mri_data1, index, 0, 0, 0);

    if (percentage) {
      value /=
          0.5 * (sumcurv + MRIgetVoxVal(mri_data1, index, 0, 0, 0) + 1e-15);
    }
  ...
  if (compute_distance) {
    ...
    printf("Max, average, and std of vertex-to-vertex distances of the two "
           "surfaces are %g, %g and %g\n",
...
MRI *ComputeDifference(MRI_SURFACE *Mesh1, MRI *mri_data1, MRI_SURFACE *Mesh2,
  ...
  for (index = 0; index < Mesh1->nvertices; index++) {
    ...
    /*
       ...
       MRIgetVoxVal(mri_data1,index,0,0,0), annIndex[0],
    ...
    if (percentage) {
      value /= 0.5 * (MRIgetVoxVal(mri_data2, annIndex[0], 0, 0, 0) +
                      MRIgetVoxVal(mri_data1, index, 0, 0, 0) + 1e-15);
    }
...
void register2to1(MRI_SURFACE *Surf1, MRI_SURFACE *Surf2) {
  ...
  while ((error_old - error_new) > 0.0001 && iter < 50) {
    ...
    iter++;
    if (debugflag)
      printf(" iteration %d, error = %15.4f\n", iter, error_new);
  }
...
// V1 is equavilent to left frame in Horn paper
{
  ...
  /* printf("\nThe Matrix = \n");
     ...
     M[1][4]); printf("\t %15.9f %15.9f %15.9f %15.9f\n",
     M[2][1],M[2][2],M[2][3], M[2][4]); printf("\t %15.9f %15.9f %15.9f
     %15.9f\n", M[3][1],M[3][2],M[3][3], M[3][4]); printf("\t %15.9f %15.9f
  ...
  /* R is not symmetric, because it's a rotation around an arbitrary axis, not
   * just the origin */
  ...
  TR[0][0] = scale1 * (temp[0][0] * R[0][0] + temp[0][1] * R[1][0] +
                       temp[0][2] * R[2][0]);
  TR[0][1] = scale1 * (temp[0][0] * R[0][1] + temp[0][1] * R[1][1] +
                       temp[0][2] * R[2][1]);
  TR[0][2] = scale1 * (temp[0][0] * R[0][2] + temp[0][1] * R[1][2] +
                       temp[0][2] * R[2][2]);

  TR[1][0] = scale1 * (temp[1][0] * R[0][0] + temp[1][1] * R[1][0] +
                       temp[1][2] * R[2][0]);
  TR[1][1] = scale1 * (temp[1][0] * R[0][1] + temp[1][1] * R[1][1] +
                       temp[1][2] * R[2][1]);
  TR[1][2] = scale1 * (temp[1][0] * R[0][2] + temp[1][1] * R[1][2] +
                       temp[1][2] * R[2][2]);

  TR[2][0] = scale1 * (temp[2][0] * R[0][0] + temp[2][1] * R[1][0] +
                       temp[2][2] * R[2][0]);
  TR[2][1] = scale1 * (temp[2][0] * R[0][1] + temp[2][1] * R[1][1] +
                       temp[2][2] * R[2][1]);
  TR[2][2] = scale1 * (temp[2][0] * R[0][2] + temp[2][1] * R[1][2] +
                       temp[2][2] * R[2][2]);
  ...
  shift[1] = scale1 * (R[0][1] * (temp[0][0] - centroid1a.x) +
                       ...
                       R[2][1] * (temp[0][2] - centroid1a.z)) +
             centroid2a.y;
  shift[2] = scale1 * (R[0][2] * (temp[0][0] - centroid1a.x) +
                       R[1][2] * (temp[0][1] - centroid1a.y) +
                       R[2][2] * (temp[0][2] - centroid1a.z)) +
             centroid2a.z;

  /* The following is just the shift at the current step.
   * Note the first point data set is constantly updated/transformed every
   * iteration
   */
