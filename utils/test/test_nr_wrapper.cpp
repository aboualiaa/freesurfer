...
public:
  ...
  bool AreSplinesEqual(MATRIX *x, MATRIX *y, MATRIX *xx, std::string yyFile,
                       float derivative1, float derivativeN);

  bool AreEqualWithinTolerance(const float expected, const float actual,
  ...
  int TestSVDcmpHelper(std::string matrixFile, int numberOfRows,
                       int numberOfColumns);

  bool TestPowellHelper(const int numberOfParameters, float expectedFret,
                        float *expectedP, float (*function)(float[]));

  bool TestDFPMinHelper(int numberOfParameters, float expectedFret,
                        float *expectedP, float (*function)(float[]),
                        void (*functionGradient)(float[], float[]),
                        void (*stepFunction)(int itno, float sse, void *parms,
                                             float *p),
                        void *params);
  ...
  bool TestGammaIncompleteHelper(const float a, std::string resultFileName);
...
void NRWrapperTest::setUp() {}

void NRWrapperTest::tearDown() {}

bool NRWrapperTest::IsBetween0And1(float x) {
  bool isBetween = true;
...
bool NRWrapperTest::AreMatricesEqual(MATRIX *m1, MATRIX *m2, float tolerance,
                                     int numberOfRows, int numberOfColumns) {

  bool areEqual = true;

  if (m1->rows >= numberOfRows && m2->rows >= numberOfRows &&
      m1->cols >= numberOfColumns && m2->cols >= numberOfColumns) {

    for (int row = 0; row < numberOfRows; row++) {
      for (int column = 0; column < numberOfColumns; column++) {
        float difference =
            fabs(m1->rptr[row + 1][column + 1] - m2->rptr[row + 1][column + 1]);
...
bool NRWrapperTest::TestBetaIncompleteHelper(const float a, const float b,
  ...
  const MATRIX *expectedResults = MatrixRead((char *)(resultFileName.c_str()));
...
void NRWrapperTest::TestOpenBetaIncomplete() {
...
void NRWrapperTest::TestOpenGammaIncomplete() {
...
bool NRWrapperTest::TestDFPMinHelper(
    int numberOfParameters, float expectedFret, float *expectedP,
    float (*function)(float[]), void (*functionGradient)(float[], float[]),
...
void NRWrapperTest::TearDownMinimizationTestResults() {
  delete[] mExpectedP1;
  mExpectedP1 = NULL;
...
void NRWrapperTest::TestOpenDFPMin() {
  ...
  isSuccess =
      TestDFPMinHelper(mFunction1Size, mExpectedFret1, mExpectedP1,
                       TestFunction1, TestFunction1Gradient, NULL, NULL);
  CPPUNIT_ASSERT(isSuccess);

  isSuccess =
      TestDFPMinHelper(mFunction2Size, mExpectedFret2, mExpectedP2,
...
// static
float NRWrapperTest::TestFunction1(float *p) {
  float x = p[1];
  float y = (x - 1) * (x - 5);
...
// static
float NRWrapperTest::TestFunction2(float *p) {
  float Ax[2];
  Ax[0] = 3 * p[1] + 2 * p[2];
  Ax[1] = 2 * p[1] + 6 * p[2];
...
void NRWrapperTest::TestFunction2Gradient(float *p, float *g) {
  float x1 = p[1];
  float x2 = p[2];
...
bool NRWrapperTest::AreEqualWithinTolerance(const float expected,
                                            const float actual,
                                            const float tolerance) {
...
bool NRWrapperTest::TestPowellHelper(const int numberOfParameters,
...
bool NRWrapperTest::AreNByNArraysEqual(float *expected, float *actual, int n,
                                       float tolerance) {
...
MATRIX *NRWrapperTest::ReconstructFromSVD(MATRIX *u, VECTOR *wVector,
                                          MATRIX *v) {
...
bool NRWrapperTest::AreSplinesEqual(MATRIX *x, MATRIX *y, MATRIX *xx,
                                    std::string yyFile, float derivative1,
                                    float derivativeN) {
...
void NRWrapperTest::TestOpenSplineAndSplInt() {
  ...
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxCourse, SINE_SPLINE_YY_0_0_COURSE, 0.0, 0.0));
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxCourse, SINE_SPLINE_YY_5_5_COURSE, 5.0, 5.0));
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxCourse, SINE_SPLINE_YY_0_17_COURSE, 0.0, 17.0));
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxCourse, SINE_SPLINE_YY_19_2_COURSE, 19.0, 2.0));

  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxFine, SINE_SPLINE_YY_0_0_FINE, 0.0, 0.0));
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxFine, SINE_SPLINE_YY_5_5_FINE, 5.0, 5.0));
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxFine, SINE_SPLINE_YY_0_17_FINE, 0.0, 17.0));
  CPPUNIT_ASSERT(
      AreSplinesEqual(x, y, xxFine, SINE_SPLINE_YY_19_2_FINE, 19.0, 2.0));
  ...
  CPPUNIT_ASSERT(AreSplinesEqual(x, y, xxResolution, SPLINE_TEST_YY, 0.0, 0.0));
...
bool NRWrapperTest::TestLUMatrixInverseHelper(std::string matrixFile,
                                              std::string expectedInverseFile) {
  ...
  MATRIX *expectedInverse = MatrixRead((char *)(expectedInverseFile.c_str()));
  if (expectedInverse == NULL) {
    std::cerr << "Failed MatrixRead of " << expectedInverseFile.c_str() << "\n";
    return false;
  }
  MATRIX *actualInverse = MatrixAlloc(matrix->rows, matrix->cols, MATRIX_REAL);
...
void NRWrapperTest::TestOpenLUMatrixInverse() {
  ...
  //  std::cout << "\tIDENTITY_MATRIX\n";
  CPPUNIT_ASSERT(TestLUMatrixInverseHelper(IDENTITY_MATRIX, IDENTITY_MATRIX));
  ...
  //  std::cout << "\tSINGULAR_MATRIX\n";
  CPPUNIT_ASSERT(TestLUMatrixInverseHelper(SINGULAR_MATRIX, SINGULAR_MATRIX) ==
                 false);

  //  std::cout << "\tZEROES_MATRIX\n";
  CPPUNIT_ASSERT(TestLUMatrixInverseHelper(ZEROES_MATRIX, ZEROES_MATRIX) ==
                 false);
}

bool NRWrapperTest::DoesCreateValidEigenSystem(std::string matrixFile) {
  MATRIX *matrix = MatrixRead((char *)(matrixFile.c_str()));
  if (matrix == NULL) {
    std::cerr << "Failed MatrixRead of " << matrixFile.c_str() << "\n";
    return false;
...
void NRWrapperTest::TestOpenEigenSystem() {
