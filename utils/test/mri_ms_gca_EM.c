 ...
 * only 14.6. Of cause, lower threshold includes more partial volumed voxels,
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_ms_gca_EM.c,v 1.9 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  ...
  if (gca->flags & GCA_NO_MRF)
    ErrorExit(ERROR_BADPARM, "%s: gca %s built without Markov priors", Progname,
              gca_fname);
  ...
  if (label_fname == NULL) {
    ...
    mri_label = MRIalloc(mri_flash[0]->width, mri_flash[0]->height,
                         mri_flash[0]->depth, MRI_SHORT);
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
  ...
  /* Now find all relevant labels, i.e., structures adjacent to structure of
   * interest */
  for (i = 0; i < 256; i++)
    label_to_index[i] = -1;
  ...
  printf("Be careful in later use of GCA if truncating input volumes to "
         "smaller size here\n");
  ...
  while (max_change > tolerance && iter < max_iters) {
    ...
    printf("iteration # %d:\n", iter);
    /* Update centroids */
    update_centroids(mri_flash, mri_mem, NULL, mri_mask, centroids,
    ...
    update_F(F, mri_flash, mri_mem, NULL, mri_mask, centroids, nvolumes_total,
             num_classes);
      ...
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          if (debug_flag && x == Gx && y == Gy && z == Gz) {
            for (c = 0; c < num_classes; c++) {
              printf("Prior for label %d = %g\n", index_to_label[c],
                     getPrior(gcap, index_to_label[c]));
            }
          }
          ...
          for (c = 0; c < num_classes; c++) {
            ...
            distance2 =
                distancems(mri_flash, centroids[c], F[c], detF[c], x, y, z);
            ...
            distance2 = exp(-0.5 * distance2) *
                        getPrior(gcap, index_to_label[c]) * sqrt(detF[c]);
            if (debug_flag && x == Gx && y == Gy && z == Gz) {
              printf("prob =%g \n", distance2);
            }
...
static void usage_exit(int code) {
  ...
  printf(
      "\t -regularize # to set covariance matrix regularization parameter\n");
  ...
  for (i = 0; i < rows; i++) {
    if (debug_flag && x == Gx && y == Gy && z == Gz)
      printf("(mri_flash[%d] = %g, centroid[%d] = %g\n", i,
             MRIFvox(mri_flash[i], x, y, z), i, centroids[i]);
