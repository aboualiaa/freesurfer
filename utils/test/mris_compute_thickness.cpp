...
static char vcid[] =
    "$Id: mris_compute_thickness.cpp,v 1.5 2016/02/27 20:38:29 nicks Exp $";
...
int main(int argc, char *argv[]) {
  ...
  if ((int)Surf1->nvertices != (int)Surf2->nvertices)
    ErrorExit(ERROR_NOFILE,
              "%s:the two surfaces have different numbers of vertices",
              Progname);
  ...
  printf("thickness stats: max = %g, min = %g, mean = %g, abs_mean = %g, std = "
         "%g\n",
  ...
  if (middle_name) {
    for (index = 0; index < Surf1->nvertices; index++) {
      Surf1->vertices[index].x =
          (Surf1->vertices[index].x + Surf2->vertices[index].x) * 0.5;
      Surf1->vertices[index].y =
          (Surf1->vertices[index].y + Surf2->vertices[index].y) * 0.5;
      Surf1->vertices[index].z =
          (Surf1->vertices[index].z + Surf2->vertices[index].z) * 0.5;
    }
