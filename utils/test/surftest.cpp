...
void MPrint(MATRIX *m) {
  ...
  cout.precision(10);
  cout << endl;
  cout << setw(15) << *MATRIX_RELT(m, 1, 1) << setw(15) << *MATRIX_RELT(m, 1, 2)
       << setw(15) << *MATRIX_RELT(m, 1, 3) << setw(15) << *MATRIX_RELT(m, 1, 4)
       << '\n'
       << setw(15) << *MATRIX_RELT(m, 2, 1) << setw(15) << *MATRIX_RELT(m, 2, 2)
       << setw(15) << *MATRIX_RELT(m, 2, 3) << setw(15) << *MATRIX_RELT(m, 2, 4)
       << '\n'
       << setw(15) << *MATRIX_RELT(m, 3, 1) << setw(15) << *MATRIX_RELT(m, 3, 2)
       << setw(15) << *MATRIX_RELT(m, 3, 3) << setw(15) << *MATRIX_RELT(m, 3, 4)
       << '\n'
       << setw(15) << *MATRIX_RELT(m, 4, 1) << setw(15) << *MATRIX_RELT(m, 4, 2)
       << setw(15) << *MATRIX_RELT(m, 4, 3) << setw(15) << *MATRIX_RELT(m, 4, 4)
       << endl;
}
    ...
    for (y = 0; y < mri->height; ++y) {
      for (x = 0; x < mri->width; ++x) {
        ...
        if (!zero(XR1 - XR2, ep) || !zero(YR1 - YR2, ep) ||
            !zero(ZR1 - ZR2, ep)) {
          cerr << "error : (XR1, YR1, ZR1)= (" << XR1 << ", " << YR1 << ", "
               << ZR1 << ")" << endl;
          cerr << "error : (XR2, YR2, ZR2)= (" << XR2 << ", " << YR2 << ", "
               << ZR2 << ")" << endl;
          cerr << "diff  :  XR1 - XR2 = " << XR1 - XR2
               << "  YR1 - YR2 = " << YR1 - YR2 << "  ZR1 - ZR2 = " << ZR1 - ZR2
               << endl;
        }
        // backward path
        double dx, dy, dz;
        ...
        if (!zero(x - dx, ep) || !zero(y - dy, ep) || !zero(z - dz, ep)) {
          ...
          cerr << "error : (  x,   y,   z)= (" << x << ", " << y << ", " << z
               << ")" << endl;
          cerr << "error : ( dx,  dy,  dz)= (" << dx << ", " << dy << ", " << dz
               << ")" << endl;
          cerr << "diff  : (" << (x - dx) << ", " << (y - dy) << ", "
               << (z - dz) << ")" << endl;
        }
