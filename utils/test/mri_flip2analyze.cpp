...
int main(int argc, char *argv[]) {
  ...
  //        0 0 c      0 b 0     0 b 0     0 0 c     a 0 0     a 0 0  where a,
  //        b, c = +/- 1
  //
  ...
  int which_xras = (fabs(src->x_r) > fabs(src->x_a))
                       ? ((fabs(src->x_r) > fabs(src->x_s)) ? 0 : 2)
                       : ((fabs(src->x_a) > fabs(src->x_s)) ? 1 : 2);
  int which_yras = (fabs(src->y_r) > fabs(src->y_a))
                       ? ((fabs(src->y_r) > fabs(src->y_s)) ? 0 : 2)
                       : ((fabs(src->y_a) > fabs(src->y_s)) ? 1 : 2);
  ...
  MRI *dst =
      MRIallocSequence(dstWidth, dstHeight, dstDepth, src->type, src->nframes);
  dst->xsize = dstXsize;
  dst->ysize = dstYsize;
  dst->zsize = dstZsize;
