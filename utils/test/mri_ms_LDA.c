...
void update_LDAmeans(MRI **mri_flash, MRI *mri_label, MRI *mri_mask,
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_ms_LDA.c,v 1.11 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  ...
  if (have_weight) {
    ...
    printf("Compute LDA weights to maximize CNR for region %d and region %d\n",
           class1, class2);
    /* Compute class means */
    update_LDAmeans(mri_flash, mri_label, mri_mask, LDAmean1, LDAmean2,
                    nvolumes_total, class1, class2);
    printf("class means computed \n");

    /* Compute Fisher's LDA weights */
    computeLDAweights(LDAweight, mri_flash, mri_label, mri_mask, LDAmean1,
                      LDAmean2, nvolumes_total, class1, class2);
...
void computeLDAweights(float *weights, MRI **mri_flash, MRI *mri_label,
  ...
  for (z = 0; z < depth; z++)
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if (label != classID1 && label != classID2)
          continue;
  ...
  if (USE_ONE) {
    ...
    for (m1 = 1; m1 <= nvolumes_total; m1++) {
      for (m2 = m1; m2 <= nvolumes_total; m2++) {
        SW1->rptr[m1][m2] =
            SW1->rptr[m1][m2] / denom1 + SW2->rptr[m1][m2] / denom2;
        SW1->rptr[m2][m1] = SW1->rptr[m1][m2];
      }
    } /* for m1, m2 */
    ...
    if (regularize) {
      ...
      SW1 = MatrixInverse(SW2, SW1); // this inverse is quite redundant, since
                                     // it will be inverted back again later
    }
  }
  ...
  if (CHOICE == 1) {
    ...
    for (m1 = 1; m1 <= nvolumes_total; m1++) {
      weights[m1 - 1] =
          (LDAmean1[m1 - 1] - LDAmean2[m1 - 1]) / (SW1->rptr[m1][m1] + 1e-15);
