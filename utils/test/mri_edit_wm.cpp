...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_edit_wm.c,v 1.3 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  argc -= nargs;
  if (1 == argc)
    exit(0);
  ...
  if (mri_wm->width != mri_seg->width || mri_wm->height != mri_seg->height ||
      mri_wm->depth != mri_seg->depth) {
    ErrorExit(ERROR_BADPARM,
              "%s: the two input volumes have unequal size. Exit.", Progname);
  }
