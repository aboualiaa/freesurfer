...
bool MatrixTest::AreMatricesEqual(MATRIX *m1, MATRIX *m2,
                                  float tolerance = 0.0) {
  bool areEqual = true;
  ...
  if (m1->rows == m2->rows && m2->cols == m2->cols) {
    ...
    std::cerr << "MatrixTest::AreMatricesEqual() not equal: m1: (" << m1->rows
              << ", " << m1->cols << "), m2: (" << m2->rows << ", " << m2->cols
              << ")\n";
    areEqual = false;
  }
...
void MatrixTest::DeleteMatrix(MATRIX *m) {
  MatrixFree(&m);
  m = NULL;
}
...
void MatrixTest::TestMatrixDeterminant() {
  ...
  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mPascalMatrix), 1.0,
                               tolerance);

  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mZeroesMatrix), 0.0,
                               tolerance);

  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mIdentityMatrix), 1.0,
                               tolerance);

  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mOneMatrix), 5.0,
                               tolerance);

  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mSingularMatrix), 0.0,
                               tolerance);
  ...
  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mBuckyMatrix),
                               2985984.0, buckyTolerance);

  // non-square matrices will have a determinant of 0 for us
  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)MatrixDeterminant(mNonSquareMatrix), 0.0,
                               tolerance);
...
void MatrixTest::TestOpenMatrixDeterminant() {
  double tolerance = 1e-4;
  ...
  CPPUNIT_ASSERT_DOUBLES_EQUAL((double)OpenMatrixDeterminant(mOneMatrix), 5.0,
                               tolerance);
...
int MatrixTest::DoesCreateValidEigenSystem(MATRIX *matrix) {
...
void MatrixTest::TestMatrixEigenSystem() {
  ...
  CPPUNIT_ASSERT(DoesCreateValidEigenSystem(mBuckyMatrix) == EIGENSYSTEM_VALID);
  ...
  CPPUNIT_ASSERT(DoesCreateValidEigenSystem(mOneMatrix) == EIGENSYSTEM_VALID);

  CPPUNIT_ASSERT(DoesCreateValidEigenSystem(mOneSmallMatrix) ==
                 EIGENSYSTEM_VALID);

  CPPUNIT_ASSERT(DoesCreateValidEigenSystem(mNonSymmetricMatrix) ==
                 EIGENSYSTEM_VALID);
}

void MatrixTest::TestMatrixSVDPseudoInverse() {
