...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_apply_INU.c,v 1.3 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  argc -= nargs;
  ...
  if (fname_before == NULL || fname_after == NULL) {
    printf(
        "Use options to specify template volumes for gain field computation\n");
    usage(1);
  }
  ...
  if (!mri_before)
    ErrorExit(ERROR_BADPARM, "%s: could not read tempate volume %s", Progname,
              fname_before);
...
void usage(int exit_val) {
  ...
  fprintf(fout, "this program estimates gain field from volumes given in "
                "options, and apply it to input \n");
