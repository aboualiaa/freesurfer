...
int main(int argc, char *argv[]) {
  ...
  if (!template_fname) {
    fprintf(stderr, "A template surface file name must be specified using the "
                    "-template option. Exit\n");
    exit(1);
  }
  ...
  if (xform_fname != NULL) {
    printf("Apply the given LTA xfrom to align input surface to template "
           "surface\n ...");
    // read transform
  ...
  printf("Now mapping the spherical map from the template surface to the input "
         "surface\n");
...
static void print_usage(void) {
  ...
  fprintf(stdout,
          "   -template_map  %%s template map name (sphere or sphere.reg)\n");
  fprintf(stdout,
          "   -register  use ICP registration to align surface to template\n");
  fprintf(
      stdout,
      "   -xform %%s apply LTA transform to align input surface to template\n");
...
static void print_help(void) {
  print_usage();
  printf("This program aligns the input surface to the template surface "
         "through ICP\n"
         "registration or a given LTA transform, and then maps the spherical "
         "morphing\n"
         ...
         "\n"
         " Spherical morphing of the template that is to be sampled to the "
         "input surface\n"
         "\n"
         "  -xform LTA_file_name\n"
         "\n"
         ...
         "  -register\n"
         "\n"
         "  Perform ICP registration to align input surface to template "
         "instead of using given LTA transform."
         "\n");
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "help"))
    ...
    register_flag = 1;
    printf("Perform ICP rigid registration of the input surface to the "
           "template\n");
...
void MRISsampleTemplateMappingToSource(MRI_SURFACE *mris,
  ...
  for (index = 0; index < mris->nvertices; index++) {
    ...
    //    vertex->origx = sopt*V3->origx + topt*V2->origx +
    //    (1.0-sopt-topt)*V1->origx;
    // vertex->origy = sopt*V3->origy + topt*V2->origy +
    // (1.0-sopt-topt)*V1->origy; vertex->origz = sopt*V3->origz +
    // topt*V2->origz
    // + (1.0-sopt-topt)*V1->origz;
    if (normflag) {
      length =
          sqrt(vertex->origx * vertex->origx + vertex->origy * vertex->origy +
               vertex->origz * vertex->origz);
