...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    std::cerr
        << "ERROR: 4 args are required: mris mri_brain mri_smooth mri_aseg\n";
    exit(1);
  }
  ...
  err = MRIScomputeBorderValues(mris, mri_brain, mri_smooth, 120.0, 112.28,
                                64.0, 53.24, 112.28, 1.0, 10.0, NULL, 1, NULL,
                                0.0, 0, mri_aseg);
  if (err) {
  ...
  if (!FEQUAL(val_avg, 80.09034729)) {
    err = 1;
  }
  if (!FEQUAL(d_avg, -0.26023445)) {
    err = 1;
  }
  if (!FEQUAL(mean_avg, 6.81196499)) {
    err = 1;
  }
  if (err == 1) {
