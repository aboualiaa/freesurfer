...
float ReCompute(float Nv, float Sv, float Ev, float Wv, float Fv, float Bv,
                unsigned char Nl, unsigned char Sl, unsigned char El,
                unsigned char Wl, unsigned char Fl, unsigned char Bl) {
...
void fmarching3d(MRI *Ori, MRI *T, float Thred) {
  ...
  int LX, HX, LY, HY, LZ,
      HZ; /* Logical variables aid in dealing with boundary */
  ...
  /* Memeory allocation */
  label = MRIalloc(Ori->width, Ori->height, Ori->depth, MRI_UCHAR);

  MRIcopyHeader(Ori, label);

  BackPointer = MRIalloc(Ori->width, Ori->height, Ori->depth, MRI_INT);
  ...
  for (d = 0; d < ZN; d++) {
    for (i = 0; i < YN; i++) {
      for (j = 0; j < XN; j++) {
        MRIvox(label, j, i, d) =
            (unsigned char)FAWAY; /*All points are labelled as FAR AWAY */
      }
    }
  }
  ...
  for (d = 0; d < ZN; d++) {
    for (i = 0; i < YN; i++) {
      for (j = 0; j < XN; j++) {
        ...
        if (value < 0.0000001 &&
            value >
                -0.0000001) { /* This grid point lies exactly on the contour */
          MRIsetVoxVal(T, j, i, d, 0,
                       0); /* Its distance value is exactly zero */
          ...
          if (result == 0)
            continue; /* Not within 1 pixel away */

          MRIvox(label, j, i, d) =
              (unsigned char)ALIVE; /*Within 1 pixel away, put into ALIVE */
    ...
    for (i = 0; i < YN; i++) {
      for (j = 0; j < XN; j++) {
        ...
        for (koff = 0; koff <= 5; koff++) { /*Find six neighbouring points */
          ...
          if (newi < 0 || newi >= YN || newj < 0 || newj >= XN || newd < 0 ||
              newd >= ZN)
            continue; /* Out of computational Boundary*/

          if (MRIvox(label, newj, newi, newd) != (unsigned char)FAWAY)
            continue;
          ...
          if (newi < YN - 1) {
            Sv = MRIgetVoxVal(T, newj, newi + 1, newd,
                              0); // T[newd][newi+1][newj];
            Sl = MRIvox(label, newj, newi + 1,
                        newd); // label[newd][newi+1][newj];
          } else
            Sl = 0;
          ...
          if (newj > 0) {
            Wv = MRIgetVoxVal(T, newj - 1, newi, newd,
                              0); // T[newd][newi][newj-1];
            Wl = MRIvox(label, newj - 1, newi,
                        newd); // label[newd][newi][newj-1];
          } else
            Wl = 0;
          ...
          if (newd < ZN - 1) {
            Fv = MRIgetVoxVal(T, newj, newi, newd + 1,
                              0); // T[newd+1][newi][newj];
            Fl = MRIvox(label, newj, newi,
