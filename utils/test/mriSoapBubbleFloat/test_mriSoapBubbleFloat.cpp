...
int main(int argc, char *argv[]) {
  ...
  // make sure to test mriSoapBubbleFloat:
  ...
  // run:
  std::cout << "running MRIsoapBubble...\n";
  MRI *mri_dst = MRIsoapBubble(mri_src, mri_ctrl, NULL, 20, 1);
  if (!mri_dst) {
    std::cerr << "ERROR: could not run mriSoapBubbleFloat!\n";
    exit(1);
  }
