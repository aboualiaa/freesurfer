...
typedef enum { purpose_Test = 0, purpose_ListResults } purpose_t;
...
int TestNearestVtxInConcentricIcos(int surfacenum, purpose_t purpose) {
  ...
  case 1:
  case 2:
    searchrange = 0.5 * mhtres;
    break;
  case 3:
    searchrange = 1.0 * mhtres;
    break;
  case 4:
    searchrange = 2.0 * mhtres;
    break;
  }
  ...
  for (probeix = 0; probeix < ProbeRepetitions; probeix++) {
    ...
    // brute force
    //------------------------------------
    brute_vno = MRISfindClosestVertex(mris, probex, probey, probez, &brute_dist,
                                      CURRENT_VERTICES);
    if (brute_vno >= 0)
    ...
    fcvit_vtx =
        MHTfindClosestVertexInTable(mht, mris, probex, probey, probez, 0);
    fcvn_vno = MHTfindClosestVertexNo(mht, mris, &probe_vtx, &fcvn_dist);

    MHTfindClosestVertexGeneric(mht, mris, probex, probey, probez, searchrange,
                                -1, &gnrc_vtx, &gnrc_vno, &gnrc_dist);

    //------------------------------------
    // Calc some distances if possible
    //------------------------------------
    if (fcv_vtx)
      fcv_dist = sqrt(SQR(probex - fcv_vtx->x) + SQR(probey - fcv_vtx->y) +
                      SQR(probez - fcv_vtx->z));
    if (fcvit_vtx)
      fcvit_dist =
          sqrt(SQR(probex - fcvit_vtx->x) + SQR(probey - fcvit_vtx->y) +
               SQR(probez - fcvit_vtx->z));
    ;
    //  fcvn_dist done
    errnum = 0;
    if (LegacyAnswerExpected) {
      if (fcv_vtx != brute_vtx) {
        errnum = 1;
        goto error_found;
      }
      if (fcvit_vtx != brute_vtx) {
        errnum = 2;
        goto error_found;
      }
      if (fcvn_vno != brute_vno) {
        errnum = 3;
        goto error_found;
      }
    ...
    if (GenericAnswerExpected) {
      if (gnrc_vtx != brute_vtx) {
        errnum = 5;
        goto error_found;
      }
      if (gnrc_vno != brute_vno) {
        errnum = 6;
        goto error_found;
      }
  ...
  error_found:
    ...
    case purpose_Test:
      ...
      sprintf(msg,
              "%5d  %8.4f %8.4f %8.4f %5d %8.4f %8.4f "
              "%8.4f %6d %8.4f %d %d %d %d",
              surfacenum, radius1, radius2, mhtres, probeix, probex, probey,
              probez, brute_vno, brute_dist, LegacyAnswerExpected,
      ...
      if (rslt)
        goto done; // quit on first error if testing
      break;
    ...
    case purpose_ListResults:
      ...
      sprintf(msg, "%5d  %8.4f %6d ( %8.4f %8.4f %8.4f ) %5d %d %d", surfacenum,
              mhtres, probeix, probex, probey, probez, LegacyAnswerExpected,
              GenericAnswerExpected, errnum);
      gw_log_message(msg);
      printf("%s\n", msg);
      ...
      gw_log_message(msg);
      printf("%s\n", msg);
      sprintf(msg, "fcv     %s    ???  %8.4f", fcv_vtx == brute_vtx ? "Y" : "N",
              fcv_dist);
      gw_log_message(msg);
      printf("%s\n", msg);
      ...
      gw_log_message(msg);
      printf("%s\n", msg);
      sprintf(msg, "fcvn    %s %6d  %8.4f", fcvn_vno == brute_vno ? "Y" : "N",
              fcvn_vno, fcvn_dist);
      gw_log_message(msg);
      printf("%s\n", msg);
      sprintf(msg, "gnrc    %s %6d  %8.4f", gnrc_vtx == brute_vtx ? "Y" : "N",
              gnrc_vno, gnrc_dist);
      gw_log_message(msg);
      printf("%s\n", msg);
...
int main(int argc, char *argv[]) {
  ...
  switch (purpose) {
  case purpose_Test:
    gw_log_message(testhead);
    printf("%s\n", testhead);
    break;
  case purpose_ListResults:
    gw_log_message(listhead1);
    printf("%s\n", listhead1);
    gw_log_message(listhead2);
    printf("%s\n", listhead2);
    break;
  } // switch
