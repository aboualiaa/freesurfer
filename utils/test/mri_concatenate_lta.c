...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_concatenate_lta.c,v 1.6 2016/02/27 20:38:29 nicks Exp $",
      "$Name:  $");
  ...
  if (type == MNI_TRANSFORM_TYPE) {
    ...
    if (tal_src_file == 0 && lta2->xforms[0].src.valid == 0)
      ErrorExit(ERROR_BADFILE,
                "%s: pls use -tal option to give talairach src and template "
                "filenames",
                Progname);
  ...
  if (vg_isEqual(&lta1->xforms[0].dst, &lta2->xforms[0].src) == 0) {
    /*    ErrorExit(ERROR_BADFILE, "%s: dst volume of lta1 doesn't match src
  ...
  printf("Combine the two LTAs to get a RAS-to-RAS from src of LTA1 to dst of "
         "LTA2\n");
...
void usage(int exit_val) {
  ...
  fprintf(fout, "If lta2 is talairach.xfm, use -tal file1 file2 to specify src "
                "(file1) and template (file2) for the talairach xfm \n");
  exit(exit_val);
