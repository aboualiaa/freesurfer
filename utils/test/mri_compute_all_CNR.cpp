...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_compute_all_CNR.c,v 1.3 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  ...
  for (z = 0; z < depth; z++)
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if ((label >= 6 && label <= 8) || label == 16 || label == 24 ||
            label == 28)
  ...
  for (i = 0; i < num_classes; i++) {
    computeClassStats(&LDAmeans[i], &Variances[i], &classSize[i], mri_src,
                      mri_label, mri_mask, MINLABEL + i);
  }
...
void usage(int exit_val) {
  ...
  fprintf(fout, "If label2 is also specified, the program computes the CNR of "
                "both regions. \n");
