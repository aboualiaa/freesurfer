...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_apply_EM_mask.c,v 1.3 2011/03/02 00:04:55 nicks Exp $",
      "$Name:  $");
  argc -= nargs;
  ...
  if (!mri_dura)
    ErrorExit(ERROR_BADPARM, "%s: could not read dura volume %s", Progname,
              fname_dura);
...
void usage(int exit_val) {
  ...
  fprintf(fout, "this program estimates dura from given membership function, "
                "and clear it from input volume \n");
