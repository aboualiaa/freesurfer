...
MRI *MRIcomputeVolumeFractionFromSurface(MRI_SURFACE *mris, double acc,
  ...
  mri_shell = MRIclone(mri_src, nullptr);
...
volFraction MRIcomputeVoxelFractions(octTreeVoxel V, int vno, double acc,
                                     int current_depth, MRI_SURFACE *mris) {
  /* inputs:
  ...
  for (j = 0; j < 8; j++) {
    dotProduct = meanNorm[0] * (V.corn[j][0] - meanVert[0]) +
                 meanNorm[1] * (V.corn[j][1] - meanVert[1]) +
  ...
  } else /* some in some out */
  {
    ...
    {
      for (k = 0; k < 8; k++) {
        volFraction frac_new = MRIcomputeVoxelFractions(
            octTreeVoxelDivide(k + 1, V), vno, acc, current_depth + 1, mris);
