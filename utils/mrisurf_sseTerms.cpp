...
// Furthermore it would be more efficient to do these using the dense MRIS_MP
// format rather than the MRIS format We are working towards this ideal...
//
...
struct SseTerms_DistortedSurfaces : public SseTermsBase {
  ...
  //========================================
  // Energy terms that iterate over the vertices, and for each iterate over all
  // their immediate (1-hop) neighbours
  //
  double RepulsiveRatioEnergy(double l_repulse) {
    ...
    for (int vno = vnoBegin; vno < vnoEnd; vno++) {
      ...
      for (int n = 0; n < v.vnum(); n++) {
        ...
        double const canon_dist =
            sqrt(cdx * cdx + cdy * cdy + cdz * cdz) + REPULSE_E;
  ...
  //========================================
  // Error terms that iterate over the vertices, and for each iterate over all
  // their neighbours
  //
  ...
  double NonlinearDistanceSSE() {
    ...
    for (int vno = vnoBegin; vno < vnoEnd; vno++) {
      ...
      double v_sse = 0.0;
      for (int n = 0; n < v.vtotal();
           n++) { // why is this vtotal and others are vnum?
...
#define sse ROMP_PARTIALSUM(0)
    ...
    double const ratio =
        min(max(-MAX_NEG_RATIO, area_scale * face.area()), MAX_NEG_RATIO);
    double const error =
        (log(1.0 + exp(NEG_AREA_K * ratio)) / NEG_AREA_K) - ratio;
...
// for them whether done using the SseTerms_DistortedSurfaces implementations or
// specific implementations or not at all
//
using SseTerms_Template_for_SurfaceFromMRIS =
    SseTerms_DistortedSurfaces<SSE_Surface_type_MRIS>;

struct SseTerms_MRIS : public SseTerms_Template_for_SurfaceFromMRIS {
  MRIS *const mris;
  SseTerms_MRIS(MRIS *const mris, int selector)
      : SseTerms_Template_for_SurfaceFromMRIS(Surface(mris), selector),
        mris(mris) {}
...
using SseTerms_Template_for_SurfaceFromMRIS_MP =
    SseTerms_DistortedSurfaces<SSE_Surface_types_MRIS_MP>;

struct SseTerms_MRIS_MP : public SseTerms_Template_for_SurfaceFromMRIS_MP {
  MRIS_MP *const mris;
  SseTerms_MRIS_MP(MRIS_MP *const mris, int selector)
      : SseTerms_Template_for_SurfaceFromMRIS_MP(Surface(mris), selector),
        mris(mris) {}
...
#define SEP
// #define ELT(NAME, SIGNATURE, CALL) double NAME SIGNATURE;
// andrew temporarily adding these definitions since they don't get defined
// anywhere else
#define ELT(NAME, SIGNATURE, CALL)                                             \
  inline double NAME SIGNATURE {                                               \
    fs::fatal() << #NAME                                                       \
                << "() has not been implemented for SseTerms_MRIS_MP";         \
    return 0;                                                                  \
  };
  LIST_OF_SSETERMS
#undef ELT
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
double SseTerms_MRIS::QuadraticCurvatureSSE(
    double l_curv) // BEVIN mris_make_surfaces 3
{
  if (FZERO(l_curv)) {
    return (NO_ERROR);
  }
...
#define REUSE(NAME, DIM)                                                       \
  VECTOR *NAME = reused->NAME;                                                 \
  if (!NAME)                                                                   \
    NAME = reused->NAME = VectorAlloc(DIM, MATRIX_REAL);
  REUSE(v_n, 3)
  REUSE(v_P, 5)
  REUSE(v_e1, 3)
  REUSE(v_e2, 3)
...
#undef REUSE
  ...
  MATRIX *m_X_inv = MatrixPseudoInverse(m_X, nullptr);
  if (!m_X_inv) {
    MatrixFree(&m_X);
    VectorFree(&v_Y);
    continue;
...
#include "romp_for_end.h"
...

  {
    ...
    for (tid = 0; tid < maxThreads; tid++) {
      Reused *reused = reusedByThread + tid;
      if (reused->v_n)
        VectorFree(&reused->v_n);
      if (reused->v_e1)
        VectorFree(&reused->v_e1);
      if (reused->v_e2)
        VectorFree(&reused->v_e2);
      if (reused->v_nbr)
        VectorFree(&reused->v_nbr);
      if (reused->v_P)
        VectorFree(&reused->v_P);
    }
  }
...
double SseTerms_MRIS::ThicknessParallelEnergy(double l_thick_parallel,
  ...
  ROMP_PF_begin
    ...
    if ((vno < MAXVERTICES) &&
        (sse > last_sse[vno] && cno > 1 && vno == Gdiag_no))
      DiagBreak();
...
double SseTerms_MRIS::ThicknessSmoothnessEnergy(double             l_tsmooth,
  ...
  for (sse_tsmooth = 0.0, vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISsampleFaceCoordsCanonical((MHT *)(parms->mht), mris, v->x, v->y, v->z,
    ...
    for (v_sse = 0.0, n = 0; n < vt->vnum; n++) {
      VERTEX const *const vn = &mris->vertices[vt->v[n]];
      if (!vn->ripflag) {
        MRISsampleFaceCoordsCanonical((MHT *)(parms->mht), mris, vn->x, vn->y,
                                      vn->z, PIAL_VERTICES, &xp, &yp, &zp);
...
#pragma omp parallel for if_ROMP(experimental) reduction(+ : sse_tnormal)
    ...
    if (Gdiag_no == vno) {
      ...
      printf("E_thick_normal: vno %d, E=%f, N = (%2.2f, %2.2f, %2.2f), D = "
             "(%2.2f, %2.2f, %2.2f), dot= %f\n",
...
double SseTerms_MRIS::ThicknessSpringEnergy(double             l_thick_spring,
  ...
  for (sse_spring = 0.0, vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (Gdiag_no == vno) {
      ...
      printf("E_thick_spring: vno %d, E=%f, N = (%2.2f, %2.2f, %2.2f), D = "
             "(%2.2f, %2.2f, %2.2f), dot= %f\n",
             vno, E, v->wnx, v->wny, v->wnz, dx, dy, dz,
             v->wnx * dx + v->wny * dy + v->wnz * dz);
    }
...
/*!
  \fn double SseTerms_MRIS::RepulsiveEnergy( double l_repulse, MHT *mht, MHT
  *mht_faces) \brief The repulsive term causes vertices to push away from each
...
double SseTerms_MRIS::RepulsiveEnergy(double l_repulse, MHT *mht,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (v_sse = 0.0, bin = bucket->bins, num = i = 0; i < bucket->nused;
      ...
      bool const debugBin = debugNonDeterminism &&
                            (0 <= hash_count && hash_count < hash_limit_max);
      if (debugBin) {
...
double SseTerms_MRIS::NonlinearSpringEnergy(INTEGRATION_PARMS *parms) {
  ...
  if (FZERO(rmin) || FZERO(rmax))
    ErrorReturn(
        ERROR_BADPARM,
        (ERROR_BADPARM, "mrisComputeNonlinearSpringEnergy: rmin or rmax = 0!"));
    ...
    for (y = 0; y < parms->mri_brain->height; y++) {
      ...
      for (z = 0; z < parms->mri_brain->height; z++) {
        ...
        if (MRIindexNotInVolume(parms->mri_labels, xv, yv, zv)) {
          ...
          label =
              MRIgetVoxVal(parms->mri_labels, nint(xv), nint(yv), nint(zv), 0);
        }
...
double SseTerms_MRIS::NegativeLogPosterior(INTEGRATION_PARMS *parms,
  ...
  if (Gdiag & DIAG_WRITE) {
    ...
    int req = snprintf(fname, STRLEN, "%s.vfrac.%4.4d.mgz", parms->base_name,
                       parms->t);
    if (req >= STRLEN) {
  ...
  if (parms->hgm == nullptr) {
      ...
      for (y = 0; y < mri->height; y++)
        for (z = 0; z < mri->depth; z++) {
          ...
          if (parms->mri_aseg) {
            ...
            if (FZERO(gm_frac) &&
                IS_CORTEX(
                    label)) // aseg thinks it is but outside ribbon - ambiguous
              continue;
          }
    ...
    if (Gdiag & DIAG_WRITE) {
      char fname[STRLEN];
      int  req = snprintf(fname, STRLEN, "hin.%s.%3.3d.plt", parms->base_name,
                         parms->t);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
      ...
      HISTOplot(hin, fname);
      req = snprintf(fname, STRLEN, "hout.%s.%3.3d.plt", parms->base_name,
                     parms->t);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
  ...
  for (sse = 0.0, nvox = x = 0; x < mri->width; x++)
    for (y = 0; y < mri->height; y++)
      for (z = 0; z < mri->depth; z++) {
        ...
        if (Gx == x && Gy == y && Gz == z)
          printf("voxel(%d, %d, %d) = %d, vfracs = (%2.1f, %2.1f, %2.1f), ll = "
                 "%2.1f\n",
  ...
  if (mri_ll) {
    char fname[STRLEN];
    int  req =
        snprintf(fname, STRLEN, "%s.ll.%4.4d.mgz", parms->base_name, parms->t);
    if (req >= STRLEN) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    }
...
double SseTerms_MRIS::NegativeLogPosterior2D(INTEGRATION_PARMS *parms,
  ...
  if (parms->h2d == nullptr) {
    printf("creating 2D intensity histograms\n");
    parms->h2d =
        HISTO2Dinit(parms->h2d, 128, 101, (double)vmin, (double)vmax, -10, 10);
    MRISclearMarks(mris);
    ...
    for (x = 0; x < mri->width; x++)
      for (y = 0; y < mri->height; y++)
        for (z = 0; z < mri->depth; z++) {
          ...
          if (FZERO(val) &&
              (dist > 1 ||
               dist < 0)) // don't allow 0s inside ribbon or too far out
            continue;
          if (FZERO(val))
            continue;
          if (FZERO(val) && dist < 0)
          ...
          if (v != nullptr) // compute distance from surface in normal direction
          {
            double dx, dy, dz;
    ...
    if (Gdiag & DIAG_WRITE) {
      ...
      int req =
          snprintf(fname, STRLEN, "h.%s.%3.3d.plt", parms->base_name, parms->t);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
      }
    ...
    hs = HISTO2DmakePDF(parms->h2d, nullptr);
    if (Gdiag & DIAG_WRITE) {
      char fname[STRLEN];
      int  req = snprintf(fname, STRLEN, "h.%s.%3.3d.pdf.plt", parms->base_name,
                         parms->t);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
    ...
    parms->h2d_out = HISTO2DsmoothAnisotropic(hs, nullptr, .25, 1);
    HISTO2Dsmooth(hs, parms->h2d, 1);
    HISTO2DmakePDF(parms->h2d_out, hs);
    if (Gdiag & DIAG_WRITE) {
      char fname[STRLEN];
      int  req = snprintf(fname, STRLEN, "h.%s.%3.3d.pdf.smooth.plt",
  ...
  for (sse = 0.0, nvox = x = 0; x < mri->width; x++)
    for (y = 0; y < mri->height; y++)
      for (z = 0; z < mri->depth; z++) {
        ...
        if (Gx == x && Gy == y && Gz == z) {
          printf(
              "voxel(%d, %d, %d) = %d, dist=%2.2f, ll = %2.1f, bins = %d, %d\n",
              x, y, z, nint(val), dist, ll, HISTO2DfindBin1(parms->h2d, val),
...
double SseTerms_MRIS::DistanceError(INTEGRATION_PARMS *parms) {
  if (!(mris->dist_alloced_flags & 1)) {
    switch (copeWithLogicProblem("FREESURFER_fix_mrisComputeDistanceError",
                                 "should have computed distances already")) {
  ...
  if (!(mris->dist_alloced_flags & 2)) {
    switch (copeWithLogicProblem("FREESURFER_fix_mrisComputeDistanceError",
                                 "should have computed dist_origs already")) {
    case LogicProblemResponse_old:
      break;
    case LogicProblemResponse_fix:
    ...
    int const acceptableNumberOfZeros =
        (mris->status == MRIS_PARAMETERIZED_SPHERE ||
         mris->status == MRIS_SPHERE)
            ? mris->nvertices * mris->avg_nbrs *
                  0.01 // only the direction from 000 has to match
...
#pragma omp parallel for if_ROMP(fast) reduction(+ : sse_dist)
    ...
    for (n = 0; n < vt->vtotal; n++) {
      ...
      if (vertexRipflags[vn_vno])
        continue;

#if NO_NEG_DISTANCE_TERM
      if (mris->vertices[vn_vno].neg)
        continue;

#endif
      float const dist_orig_n = !v->dist_orig ? 0.0 : v->dist_orig[n];

      if (dist_orig_n >= UNFOUND_DIST)
        continue;

      if (DZERO(dist_orig_n) &&
          (count_dist_orig_zeros++ > acceptableNumberOfZeros)) {
        fprintf(stderr,
                "v[%d]->dist_orig[%d] = %f!!!!, count_dist_orig_zeros:%d\n",
        ...
        if (++err_cnt > max_errs) {
          ...
          for (dump_vno = 0; dump_vno < mris->nvertices; dump_vno++) {
            VERTEX_TOPOLOGY const *const dump_vt =
                &mris->vertices_topology[dump_vno];
            ...
            for (dump_n = 0; dump_n < dump_vt->vtotal; dump_n++) {
              ...
              if (vertexRipflags[dump_vn_vno])
                continue;
              float const dump_dist_orig_n =
                  !dump_v->dist_orig ? 0.0 : dump_v->dist_orig[n];
              if (!DZERO(dump_dist_orig_n))
                continue;
              dump_count_dist_orig_zeros++;
              fprintf(
                  stderr,
                  "v[%d]->dist_orig[%d] = %f!!!!, count_dist_orig_zeros:%d\n",
                  dump_vno, dump_n, dump_dist_orig_n,
                  dump_count_dist_orig_zeros);
              if (dump_count_dist_orig_zeros > 30)
                goto dump_done;
            }
          }
        dump_done:;
          ErrorExit(ERROR_BADLOOP,
    ...
    if (trial == 0) {
      ...
      if (sse_dist_trial0 != sse_dist) {
        fprintf(stderr, "%s:%d diff thread count, diff result %g %g %g\n",
                __FILE__, __LINE__, sse_dist_trial0, sse_dist,
                sse_dist_trial0 - sse_dist);
      }
    }
  } // trial
...
#pragma omp parallel for if_ROMP(fast) reduction(+ : sse)
  ...
  z = v->z;
    ...
    src = v->curv;
#endif
  target = MRISPfunctionValTraceable(parms->mrisp_template, mris->radius, x, y,
                                     z, parms->frame_no, vertexTrace);

#define DISABLE_STDS 0
#if DISABLE_STDS
  std = 1.0f;
#else
    std = MRISPfunctionValTraceable(parms->mrisp_template, mris->radius, x, y,
                                    z, parms->frame_no + 1, vertexTrace);
    std = sqrt(std);
    if (FZERO(std)) {
...
/*!
  ...
  \brief Computes the sum of the squares of the value at a vertex minus the
  v->val. Ignores ripped vertices or any with v->val<0. Does not normalize by
  the number of vertices. Basically same computation as mrisRmsValError() but
...
#else
    MRISsurfaceRASToVoxelCached(mris, parms->mri_smooth, x, y, z, &xw, &yw,
                                &zw);
#endif
...
double SseTerms_MRIS::SphereError(double l_sphere, double r0) {
  ...
  //  sse:4.33023e+09
  // I tried summing in groups of 1024 numbers then summing the sums, and got
  // the same answer in %g format
  //        /Bevin
  //
  sse = 0.0;
  ROMP_PF_begin
...
double SseTerms_MRIS::VectorCorrelationError(INTEGRATION_PARMS *parms,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISPfunctionVectorVals(parms->mrisp_template, mris, x, y, z, frames,
                            2 * nframes, vals);
#if DISABLE_STDS
    for (n = 0; n < nframes; n++) {
...
double SseTerms_MRIS::Error(INTEGRATION_PARMS *parms, float *parea_rms,
                            float *pangle_rms, float *pcurv_rms,
  ...
  *pangle_rms =
      (float)sqrt(sse_angle / (double)(ntriangles * ANGLES_PER_TRIANGLE));
...
int mrisCreateLikelihoodHistograms(MRIS *mris, INTEGRATION_PARMS *parms) {
  ...
  {
    parms->mri_white = MRIupsampleN(parms->mri_brain, nullptr, 3);
    MRISsaveVertexPositions(mris, TMP_VERTICES);
    MRISrestoreVertexPositions(mris, WHITE_VERTICES);
    MRISfillInterior(mris, parms->mri_white->xsize, parms->mri_white);
    MRISrestoreVertexPositions(mris, TMP_VERTICES);
  }
  mri_pial = MRIclone(parms->mri_white, nullptr);
  MRISfillInterior(mris, mri_pial->xsize, mri_pial);
  if (parms->mri_labels == nullptr) {
    parms->mri_labels = MRIclone(parms->mri_white, nullptr);
  }
  parms->mri_dist =
...
//==================================================================================
// MRIScomputeSSE and MRIScomputeSSEExternal are used for the numerical
// integration
//
// As such, they represent the exact error function being minimized, as opposed
// to computeError above.
//
// The performance critical ones are ...
//
//      sse_area sse_neg_area       in this function here iterates over faces
//      computing face normals sse_nl_area mrisComputeNonlinearAreaSSE(mris)
//      mrisurf_deform.c    iterates over faces doing a simple calculation
//      sse_dist                    mrisComputeNonlinearDistanceSSE(mris)
//      mrisurf_deform.c    iterates over vertices over their dist and dist_orig
//      sse_corr                    mrisComputeCorrelationError(mris, parms, 1)
//      mrisurf_deform.c    iterates over vertices using xyz calling mrisp.c
//      MRISPfunctionValTraceable
//                                                                                                                          which does a lot more work than the others
//
// These are in the order the original code computed them, so that side effects
// are not reordered In older code the ashburner_triangle is computed but not
// used , here it is not computed at all

// The ELTS terms have a working overloading of mrisCompute### that can take a
// SurfaceFromMRIS_MP::XYZPositionConsequences::Surface as their first parameter
//      They are implemented below in template <class _Surface> struct
//      SseTerms_DistortedSurfaces {...}
//
// The ELTM terms have a working overloading of mrisCompute### that can take a
// MRIS* as their first parameter
//      They also have an asserting overloading that can take a
//      SurfaceFromMRIS_MP::XYZPositionConsequences::Surface as their first
//      parameter which will not be called because MRIScomputeSSE_canDo(MRIS_MP*
//      usedOnlyForOverloadingResolution, INTEGRATION_PARMS *parms) returns
//      false for these
//
#define SSE_TERMS                                                              \
  ELTM(sse_area, parms->l_parea, true, computed_area)                          \
  ELTM(sse_neg_area, parms->l_area, true, computed_neg_area)                   \
  ELTM(sse_repulse, 1.0, (parms->l_repulse > 0),                               \
       mrisComputeRepulsiveEnergy(mris, parms->l_repulse, mht_v_current,       \
                                  mht_f_current))                              \
  ELTM(sse_repulsive_ratio, 1.0, true,                                         \
       mrisComputeRepulsiveRatioEnergy(mris, parms->l_repulse_ratio))          \
  ELTM(sse_tsmooth, 1.0, true,                                                 \
       mrisComputeThicknessSmoothnessEnergy(mris, parms->l_tsmooth, parms))    \
  ELTM(                                                                        \
      sse_thick_min, parms->l_thick_min, true,                                 \
      mrisComputeThicknessMinimizationEnergy(mris, parms->l_thick_min, parms)) \
  ELTM(sse_ashburner_triangle, parms->l_ashburner_triangle, false,             \
       mrisComputeAshburnerTriangleEnergy(mris, parms->l_ashburner_triangle,   \
                                          parms))                              \
  ELTM(sse_thick_parallel, parms->l_thick_parallel, true,                      \
       mrisComputeThicknessParallelEnergy(mris, parms->l_thick_parallel,       \
                                          parms))                              \
  ELTM(sse_thick_normal, parms->l_thick_normal, true,                          \
       mrisComputeThicknessNormalEnergy(mris, parms->l_thick_normal, parms))   \
  ELTM(sse_thick_spring, parms->l_thick_spring, true,                          \
       mrisComputeThicknessSpringEnergy(mris, parms->l_thick_spring, parms))   \
  ELTM(sse_nl_area, parms->l_nlarea, !FZERO(parms->l_nlarea),                  \
       mrisComputeNonlinearAreaSSE(mris))                                      \
  ELTM(sse_nl_dist, parms->l_nldist, !DZERO(parms->l_nldist),                  \
       mrisComputeNonlinearDistanceSSE(mris))                                  \
  ELTM(sse_dist, parms->l_dist, !DZERO(parms->l_dist),                         \
       mrisComputeDistanceError(mris, parms))                                  \
  ELTM(sse_spring, parms->l_spring, !DZERO(parms->l_spring),                   \
       mrisComputeSpringEnergy(mris))                                          \
  ELTM(sse_lap, parms->l_lap, !DZERO(parms->l_lap),                            \
       mrisComputeLaplacianEnergy(mris))                                       \
  ELTM(sse_tspring, parms->l_tspring, !DZERO(parms->l_tspring),                \
       mrisComputeTangentialSpringEnergy(mris))                                \
  ELTM(sse_nlspring, parms->l_nlspring, !DZERO(parms->l_nlspring),             \
       mrisComputeNonlinearSpringEnergy(mris, parms))                          \
  ELTM(sse_curv, l_curv_scaled, !DZERO(parms->l_curv),                         \
       mrisComputeQuadraticCurvatureSSE(mris, parms->l_curv))                  \
  ELTM(sse_corr, l_corr, !DZERO(l_corr),                                       \
       mrisComputeCorrelationError(mris, parms, 1))                            \
  ELTM(sse_val, parms->l_intensity, !DZERO(parms->l_intensity),                \
       mrisComputeIntensityError(mris, parms))                                 \
  ELTM(sse_loc, parms->l_location, !DZERO(parms->l_location),                  \
       mrisComputeTargetLocationError(mris, parms))                            \
  ELTM(sse_dura, parms->l_dura, !DZERO(parms->l_dura),                         \
       mrisComputeDuraError(mris, parms))                                      \
  ELTM(sse_histo, parms->l_histo, !DZERO(parms->l_histo),                      \
       mrisComputeHistoNegativeLikelihood(mris, parms))                        \
  ELTM(sse_map, parms->l_map, !DZERO(parms->l_map),                            \
       mrisComputeNegativeLogPosterior(mris, parms, NULL))                     \
  ELTM(sse_map2d, parms->l_map2d, !DZERO(parms->l_map2d),                      \
       mrisComputeNegativeLogPosterior2D(mris, parms, NULL))                   \
  ELTM(sse_grad, parms->l_grad, !DZERO(parms->l_grad),                         \
       mrisComputeIntensityGradientError(mris, parms))                         \
  ELTM(sse_sphere, parms->l_sphere, !DZERO(parms->l_sphere),                   \
       mrisComputeSphereError(mris, parms->l_sphere, parms->a))                \
  ELTM(                                                                        \
      sse_shrinkwrap, parms->l_shrinkwrap, !DZERO(parms->l_shrinkwrap),        \
      mrisComputeShrinkwrapError(mris, parms->mri_brain, parms->l_shrinkwrap)) \
  ELTM(sse_expandwrap, parms->l_expandwrap, !DZERO(parms->l_expandwrap),       \
       mrisComputeExpandwrapError(mris, parms->mri_brain, parms->l_expandwrap, \
                                  parms->target_radius))                       \
  ELTM(sse_vectorCorrelationError, 1.0, use_multiframes,                       \
       mrisComputeVectorCorrelationError(mris, parms, 1))                      \
  // end of list

template <class Surface, class Some_MRIS>
double MRIScomputeSSE_template(Surface surface, Some_MRIS *mris,
                               INTEGRATION_PARMS *parms) {
...
#include "romp_for_end.h"
    ...
    ROMP_PF_end
      ...
      if (trial == 0) {
        ...
        if (relevant_angle_trial0 != relevant_angle) {
          fprintf(stderr, "%s:%d diff thread count, diff result %g %g %g\n",
                  __FILE__, __LINE__, relevant_angle_trial0, relevant_angle,
                  relevant_angle_trial0 - relevant_angle);
        }
        if (computed_neg_area_trial0 != computed_neg_area) {
          fprintf(stderr, "%s:%d diff thread count, diff result %g %g %g\n",
                  __FILE__, __LINE__, computed_neg_area_trial0,
                  computed_neg_area,
                  computed_neg_area_trial0 - computed_neg_area);
        }
        if (computed_area_trial0 != computed_area) {
          fprintf(stderr, "%s:%d diff thread count, diff result %g %g %g\n",
                  __FILE__, __LINE__, computed_area_trial0, computed_area,
                  computed_area_trial0 - computed_area);
        }
      }
  ...
  if (!FZERO(parms->l_repulse)) {
    double vmean, vsigma;
    vmean = MRIScomputeTotalVertexSpacingStats(mris, &vsigma, nullptr, nullptr,
                                               nullptr, nullptr);
    mht_v_current =
        MHTcreateVertexTable_Resolution(mris, CURRENT_VERTICES, vmean);
    mht_f_current =
        MHTcreateFaceTable_Resolution(mris, CURRENT_VERTICES, vmean);
  }

#define ELTS(NAME, MULTIPLIER, COND, EXPR)                                     \
  double const NAME = (COND) ? (EXPR) : 0.0;
...
#undef ELTS
  ...
  if (parms->l_thick_spring > 0 || parms->l_thick_min > 0 ||
      parms->l_thick_parallel > 0 /* && DIAG_VERBOSE_ON*/)
    printf("min=%2.3f, parallel=%2.4f, normal=%2.4f, spring=%2.4f, "
           "ashburner=%2.3f, tsmooth=%2.3f\n",
...
#undef ELTS
  ...
  // This code matches code Bevin added to the previous good code to compare old
  // and new runs
  //
  static int logSSECount, logSSE;
  if (!logSSECount) {
    logSSE = !!getenv("FREESURFER_logSSE");
  }
  logSSECount++;
...
#define ELTM(NAME, MULTIPLIER, COND, EXPR)                                     \
  if (COND) {                                                                  \
    ...
    if (!reported) {                                                           \
      reported = true;                                                         \
      fprintf(stdout, "%s:%d can't do %s %s\n", __FILE__, __LINE__, #NAME,     \
              #EXPR);                                                          \
    }                                                                          \
    result = false;                                                            \
  }
...
double MRIScomputeSSEExternal(MRIS *mris, INTEGRATION_PARMS *parms,
                              double *ext_sse) {
  double sse;
