...
static MRIS *MRISprojectOntoSphereWkr(MRIS *mris, double r) {
  ...
  for (int vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (showTotalDist)
      total_dist += sqrt((double)(dx * dx + dy * dy + dz * dz));
  }
  ...
  mris->status = (mris->status == MRIS_PARAMETERIZED_SPHERE)
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    project_point_onto_sphere(v->x, v->y, v->z, mris->radius, &v->x, &v->y,
                              &v->z);
    MHTfindClosestFaceGeneric(mht, mris, v->x, v->y, v->z, 8, 8, 1, &face, &fno,
                              &fdist);
    if (fno < 0)
      MHTfindClosestFaceGeneric(mht, mris, v->x, v->y, v->z, 1000, -1, -1,
                                &face, &fno, &fdist);
