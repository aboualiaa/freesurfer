...
int MRIgtmSeg(GTMSEG *gtmseg) {
  ...
  sprintf(tmpstr, "%s/%s/mri/%s", SUBJECTS_DIR, gtmseg->subject,
          gtmseg->apasfile);
  printf("Loading %s\n", tmpstr);
  apas = MRIread(tmpstr);
  if (apas == nullptr)
  ...
  gtmseg->anat = MRIcopyHeader(apas, nullptr); // keep anat header around

  aseg = MRIunsegmentCortex(apas, gtmseg->lhmin, gtmseg->lhmax, gtmseg->rhmin,
                            gtmseg->rhmax, nullptr); // aseg+head
  if (aseg == nullptr)
    return (1);
  MRIfree(&apas);
  ...
  fflush(stdout);
  if (gtmseg->wmannotfile != nullptr) {
    sprintf(tmpstr, "%s/%s/label/lh.%s", SUBJECTS_DIR, gtmseg->subject,
            gtmseg->wmannotfile);
    err = MRISreadAnnotation(gtmseg->lhw, tmpstr);
    if (err) {
      printf("Try running mri_annotation2label --s %s --hemi lh --lobesStrict "
             "lobefilename\n",
             gtmseg->subject);
      return (1);
    }
    sprintf(tmpstr, "%s/%s/label/rh.%s", SUBJECTS_DIR, gtmseg->subject,
            gtmseg->wmannotfile);
    err = MRISreadAnnotation(gtmseg->rhw, tmpstr);
    if (err) {
      printf("Try running mri_annotation2label --s %s --hemi rh --lobesStrict "
             "lobefilename\n",
             gtmseg->subject);
      return (1);
    }
  ...
  printf(" Relabeling any unlateralized hypointenities as lateralized "
         "hypointenities\n");
  MRIrelabelHypoHemi(aseg, gtmseg->lhw, gtmseg->rhw, nullptr, aseg);
  ...
  if (nlist > 0)
    MRIunsegmentWM(aseg, gtmseg->lhw, gtmseg->rhw, list, nlist, nullptr, aseg);
  ...
  printf(" t = %6.4f\n", timer.seconds());
  fflush(stdout);
  hrseg = MRIhiresSeg(aseg, gtmseg->lhw, gtmseg->lhp, gtmseg->rhw, gtmseg->rhp,
                      gtmseg->USF, &gtmseg->anat2seg);
  if (hrseg == nullptr)
    return (1);
  ...
  printf(" t = %6.4f\n", timer.seconds());
  fflush(stdout);
  ctxseg = MRIannot2CorticalSeg(hrseg, gtmseg->lhw, gtmseg->lhp, gtmseg->rhw,
                                gtmseg->rhp, nullptr, nullptr);
  MRIfree(&hrseg);
  ...
  if (gtmseg->wmannotfile != nullptr) {
    ...
    printf(" t = %6.4f\n", timer.seconds());
    fflush(stdout);
    ctxseg = MRIannot2CerebralWMSeg(ctxseg, gtmseg->lhw, gtmseg->rhw,
                                    gtmseg->dmax, nullptr, ctxseg);
  } else
    printf("Not subsegmenting WM\n");

  gtmseg->seg = MRIreplaceList(ctxseg, gtmseg->srclist, gtmseg->targlist,
                               gtmseg->nlist, nullptr, nullptr);
  if (gtmseg == nullptr)
    return (1);
  MRIfree(&ctxseg);
...
/*-----------------------------------------------------------------------------*/
/*
\fn int GTMdefaultSegReplacmentList(int *nReplace, int *ReplaceThis,int
*WithThat) \brief Creates a list of segids to replace with other seg ids. All
...
int GTMdefaultSegReplacmentList(int *nReplace, int *ReplaceThis,
                                int *WithThat) {
  int nlist;
  ...
  // ReplaceThis[nlist] = 1034; WithThat[nlist] = 1030; nlist++; //
  // transtemp=stg ReplaceThis[nlist] = 2034; WithThat[nlist] = 1030; nlist++;
  // // transtemp=stg ReplaceThis[nlist] = 1001; WithThat[nlist] = 1015;
  // nlist++; // bankssts=mtg ReplaceThis[nlist] = 2001; WithThat[nlist] = 2015;
  // nlist++; // bankssts=mtg ReplaceThis[nlist] = 1032; WithThat[nlist] = 1027;
  // nlist++; // frontpole=rmf ReplaceThis[nlist] = 2032; WithThat[nlist] =
  // 2027; nlist++; // frontpole=rmf ReplaceThis[nlist] = 1016; WithThat[nlist]
  // = 1006; nlist++; // parahip=entorhinal ? ReplaceThis[nlist] = 2016;
  // WithThat[nlist] = 2006; nlist++; // parahip=entorhinal ?
  ...
  // Note: the location as last two items makes it so that
  // --default-seg-merge-choroid in mri_gtmpvc works. ReplaceThis[nlist] =   31;
  // WithThat[nlist] =   24; nlist++; // LChoroidP ? ReplaceThis[nlist] =   63;
  // WithThat[nlist] =   24; nlist++; // RChoroidP ?

  *nReplace = nlist;
  return (0);
}
/*!
  \fn int GTMoptSegReplacmentList(int *nReplace, int *ReplaceThis, int
  *WithThat) \brief Reduces the number of ROIs by merging left and right and
...
COLOR_TABLE *GTMSEGctab(GTMSEG *gtmseg, COLOR_TABLE *ctSubCort) {
  ...
  for (n = 0; n < ct0->nentries; n++) {
    ...
    int err = snprintf(cte->name, STRLEN - 1, "ctx-lh-%s",
                       cte0->name); // new name reflects cortex and hemi
    if (err >= STRLEN - 1) {
      std::cerr << __FUNCTION__ << ": Truncation prepending ctx-lh-"
                << std::endl;
    }
    cte->TissueType = 1;
  }
  ...
  for (n = 0; n < ct0->nentries; n++) {
    ...
    if (err >= STRLEN - 1) {
      std::cerr << __FUNCTION__ << ": Truncation prepending ctx-rh-"
                << std::endl;
    }
    cte->TissueType = 1;
  }
  ...
  if (gtmseg->SubSegWM) {
    ...
    for (n = 0; n < ct0->nentries; n++) {
      ...
      if (hit == 0) {
        if (Gdiag_no > 1)
          printf(
              "INFO: GTMSEGctab(): lhw n=%d, segid=%d, %s not in segidlist\n",
              n, segid, cte0->name);
        continue;
      }
      segidhit[m]++;
      ...
      if (err >= STRLEN - 1) {
        std::cerr << __FUNCTION__ << ": Truncation prepending wm-lh-"
                  << std::endl;
      }
      cte->TissueType = 3;
    }
    ...
    for (n = 0; n < ct0->nentries; n++) {
      ...
      if (hit == 0) {
        if (Gdiag_no > 1)
          printf(
              "INFO: GTMSEGctab(): lhp n=%d, segid=%d, %s not in segidlist\n",
              n, segid, cte0->name);
        continue;
      }
      segidhit[m]++;
      ...
      if (err >= STRLEN - 1) {
        std::cerr << __FUNCTION__ << ": Truncation prepending wm-rh-"
                  << std::endl;
      }
      cte->TissueType = 3;
    }
...
int GTMmatrixY(GTM *gtm) {
  gtm->y = GTMvol2mat(gtm, gtm->yvol, nullptr);
  return (0);
}
/*------------------------------------------------------------------*/
/*
...
int GTMnPad(GTM *gtm) {
  ...
  if (maxFWHM > 0) {
    ...
    gtm->nPad =
        ceil(sqrt(-log(gtm->PadThresh * maxStd * sqrt(2 * M_PI)) * 2 * maxStd));
    printf("maxFWHM = %g (voxels), PadThresh=%g, nPad=%d\n", maxFWHM,
           gtm->PadThresh, gtm->nPad);
  } else
    gtm->nPad = 1;
  return (0);
}
...
int GTMsolve(GTM *gtm) {
  ...
  for (f = 0; f < gtm->res->cols; f++) {
    ...
    for (n = 0; n < gtm->res->rows; n++)
      sum +=
          ((double)gtm->res->rptr[n + 1][f + 1] * gtm->res->rptr[n + 1][f + 1]);
    gtm->rvar->rptr[1][f + 1] = sum / gtm->dof;
    if (gtm->rescale)
      gtm->rvarUnscaled->rptr[1][f + 1] =
          gtm->rvar->rptr[1][f + 1] / (gtm->scale * gtm->scale);
    else
...
MRI *GTMmat2vol(GTM *gtm, MATRIX *m, MRI *vol) {
  ...
  if (vol == nullptr) {
    vol = MRIallocSequence(gtm->yvol->width, gtm->yvol->height,
                           gtm->yvol->depth, MRI_FLOAT, m->cols);
    if (vol == nullptr)
      return (nullptr);
...
int GTMrbv(GTM *gtm) {
  ...
  MRI *yhatseg = nullptr; // smoothed yhat in seg space (used with RBV)
  ...
  if (gtm->mask_rbv_to_brain) {
    ...
    // (and so also that of the conformed anat) so no new registration is
    // necessary
    ...
    for (n = 0; n < gtm->ctGTMSeg->nentries; n++) {
      if (gtm->ctGTMSeg->entries[n] == nullptr)
        continue;
    ...
    gtm->rbvsegmasked = MRIreplaceList(gtm->rbvseg, ReplaceThis, WithThat,
                                       nReplace, nullptr, nullptr);
    printf("  computing bounding box  %d %d %d ", gtm->rbvseg->width,
           gtm->rbvseg->height, gtm->rbvseg->depth);
    region = REGIONgetBoundingBox(gtm->rbvsegmasked, 10);
    REGIONprint(stdout, region);
    MRI *tmp = MRIextractRegion(gtm->rbvsegmasked, nullptr, region);
    MRIfree(&gtm->rbvsegmasked);
    gtm->rbvsegmasked = tmp;
  } else
    gtm->rbvsegmasked = gtm->rbvseg;
  gtm->anat2rbv =
      TransformRegDat2LTA(gtm->anatconf, gtm->rbvsegmasked, nullptr);
  ...
  printf("   Allocating RBV nvox=%d\n",
         gtm->rbvsegmasked->width * gtm->rbvsegmasked->height *
             gtm->rbvsegmasked->depth * gtm->nframes);
  fflush(stdout);
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  gtm->rbv =
      MRIallocSequence(gtm->rbvsegmasked->width, gtm->rbvsegmasked->height,
                       gtm->rbvsegmasked->depth, MRI_FLOAT, gtm->nframes);
  if (gtm->rbv == nullptr) {
  ...
  yseg = MRIallocSequence(gtm->rbvseg->width, gtm->rbvseg->height,
                          gtm->rbvseg->depth, MRI_FLOAT, 1);
  if (yseg == nullptr) {
  ...
  for (f = 0; f < gtm->nframes; f++) {
    ...
    if (Gdiag_no > 0)
      PrintMemUsage(stdout);
    yhatseg =
        MRIgaussianSmoothNI(yhat0seg, gtm->cStd, gtm->rStd, gtm->sStd, yhatseg);
    if (yhatseg == nullptr) {
    ...
    if (Gdiag_no > 0)
      PrintMemUsage(stdout);
    lta = LTAcopy(gtm->rbvseg2pet, nullptr);
...
int GTMrbvseg(GTM *gtm) {
  ...
  gtm->rbvseg =
      MRIchangeSegRes(gtm->anatseg, res, res, res, gtm->ctGTMSeg, &new2seg);
  if (gtm->rbvseg == nullptr)
    return (1);
  ;
  gtm->rbvseg2pet = LTAconcat2(new2seg, gtm->seg2pet, 1);
  if (gtm->rbvseg2pet == nullptr)
    exit(1);
  LTAfree(&new2seg);
...
int GTMrbv0(GTM *gtm) {
  ...
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  yhat0seg = GTMsegSynth(gtm, -1, nullptr);
  if (yhat0seg == nullptr) {
  ...
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  yhatseg =
      MRIgaussianSmoothNI(yhat0seg, gtm->cStd, gtm->rStd, gtm->sStd, nullptr);
  if (yhatseg == nullptr) {
  ...
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  yseg = MRIallocSequence(gtm->anatseg->width, gtm->anatseg->height,
                          gtm->anatseg->depth, MRI_FLOAT, gtm->yvol->nframes);
  if (yseg == nullptr) {
  ...
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  gtm->rbv =
      MRIallocSequence(gtm->anatseg->width, gtm->anatseg->height,
                       gtm->anatseg->depth, MRI_FLOAT, gtm->yvol->nframes);
  if (gtm->rbv == nullptr) {
  ...
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  gtm->rbvsegmean =
      MRIallocSequence(gtm->nsegs, 1, 1, MRI_FLOAT, gtm->yvol->nframes);
  nhits = MatrixAlloc(gtm->beta->rows, 1, MATRIX_REAL);
  ...
  if (gtm->mask_rbv_to_brain) {
    ...
    printf("  replacing head voxels with 0\n");
    segtmp = MRIreplaceList(gtm->anatseg, ReplaceThis, WithThat, nReplace,
                            nullptr, nullptr);
    printf("  computing bounding box  ");
    region = REGIONgetBoundingBox(segtmp, 10);
    REGIONprint(stdout, region);
    printf("  extracting bounding box\n");
    rbvtmp = MRIextractRegion(gtm->rbv, nullptr, region);
    free(region);
    region = nullptr;
    MRIfree(&segtmp);
    MRIfree(&gtm->rbv);
...
MRI *GTMmgxpvc(GTM *gtm, int Target) {
  ...
  for (nthseg = 0; nthseg < gtm->nsegs; nthseg++) {
    ...
    if (Target == 1 || Target == 3) {
      if (strcmp("cortex", cte->name) == 0)
        continue;
      if (strcmp("cortex-lh", cte->name) == 0)
        continue;
      if (strcmp("cortex-rh", cte->name) == 0)
        continue;
    }
    if (Target == 2 || Target == 3) {
      if (strcmp("subcort_gm", cte->name) == 0)
        continue;
      if (strcmp("subcort_gm-lh", cte->name) == 0)
        continue;
      if (strcmp("subcort_gm-rh", cte->name) == 0)
        continue;
      if (strcmp("subcort_gm-mid", cte->name) == 0)
        continue;
    }
    if (Target == 4 && strcmp("cortex-lh", cte->name) == 0)
      continue;
    if (Target == 5 && strcmp("cortex-rh", cte->name) == 0)
      continue;
    if (Target == 6 && strcmp("subcort_gm-lh", cte->name) == 0)
      continue;
    if (Target == 7 && strcmp("subcort_gm-rh", cte->name) == 0)
      continue;
    if (Target == 8 && strcmp("subcort_gm-mid", cte->name) == 0)
      continue;
    // otherwise
    for (f = 0; f < gtm->nframes; f++)
  ...
  yNotTarg = MatrixMultiplyD(gtm->X, betaNotTarg, nullptr);
  // Subtract to resdiualize the PET wrt the non-target tissue
  ydiff = MatrixSubtract(gtm->y, yNotTarg, nullptr);
    ...
    for (nthseg = 0; nthseg < gtm->nsegs; nthseg++) {
      ...
      if (Target == 3) { // asking for any GM
        if (strcmp("cortex", cte->name) != 0 &&
            ...
            strcmp("subcort_gm-lh", cte->name) != 0 &&
            strcmp("subcort_gm-rh", cte->name) != 0 &&
            strcmp("subcort_gm-mid", cte->name) != 0)
          continue; // but this is not GM
      }
      if (Target == 4 && strcmp("cortex-lh", cte->name) != 0)
        continue;
      if (Target == 5 && strcmp("cortex-rh", cte->name) != 0)
  ...
  mgx = GTMmat2vol(gtm, ydiff, nullptr);
...
int GTMmgpvc(GTM *gtm) {
  ...
  if (gtm->mg)
    MRIfree(&gtm->mg);
  gtm->mg = MRIallocSequence(gtm->yvol->width, gtm->yvol->height,
  ...
  // NULL); Need to add MB here

  // WM PVF
  wmpvf = fMRIframe(gtm->ttpvf, 2, nullptr);
...
int GTMmeltzerpvc(GTM *gtm) {
  ...
  if (gtm->meltzer)
    MRIfree(&gtm->meltzer);
  gtm->meltzer =
      MRIallocSequence(gtm->yvol->width, gtm->yvol->height, gtm->yvol->depth,
                       MRI_FLOAT, gtm->yvol->nframes);
  if (gtm->meltzer == nullptr)
    return (1);
  ...
  if (gtm->MeltzerBinThresh > 0.0) {
    ...
    mritmp = MRIbinarize(gmwmpvf, nullptr, gtm->MeltzerBinThresh, 0, 1);
    MRIfree(&gmwmpvf);
    gmwmpvf = mritmp;
  ...
  gmwmpvfpsf =
      MRIgaussianSmoothNI(gmwmpvf, gtm->cStd, gtm->rStd, gtm->sStd, nullptr);
  // Need to add MB here
...
int GTMsynth(GTM *gtm, int NoiseSeed, int nReps) {
  ...
  if (gtm->ysynth)
    MRIfree(&gtm->ysynth);
  gtm->ysynth =
      MRIallocSequence(gtm->gtmseg->width, gtm->gtmseg->height,
                       gtm->gtmseg->depth, MRI_FLOAT, gtm->beta->cols);
  if (gtm->yvol) {
...
int GTMsmoothSynth(GTM *gtm) {
  if (gtm->ysynth == nullptr)
    GTMsynth(gtm, 0, 0);
  gtm->ysynthsm = MRIgaussianSmoothNI(gtm->ysynth, gtm->cStd, gtm->rStd,
...
int GTMbuildX(GTM *gtm) {
  ...
  if (gtm->X == nullptr || gtm->X->rows != gtm->nmask ||
      gtm->X->cols != gtm->nsegs) {
    // Alloc or realloc X
    if (gtm->X)
      MatrixFree(&gtm->X);
  ...
  if (gtm->X0 == nullptr || gtm->X0->rows != gtm->nmask ||
      gtm->X0->cols != gtm->nsegs) {
    if (gtm->X0)
      MatrixFree(&gtm->X0);
...
#pragma omp parallel for if_ROMP(assume_reproducible) reduction(+ : err)
    ...
    if (gtm->DoVoxFracCor)
      nthsegpvf =
          fMRIframe(gtm->segpvf, nthseg, nullptr); // extract PVF for this seg
    else
      nthsegpvf = MRIbinarizeMatch(gtm->gtmseg, &segid, 1, 0,
                                   nullptr); // or get binary mask
    // Extract a region for the seg. This speeds up smoothing.
    region =
        REGIONgetBoundingBox(nthsegpvf, gtm->nPad); // tight+pad bounding box
    if (region->dx < 0) {
      printf("ERROR: creating region for nthseg=%d, segid=%d, %s\n", nthseg,
             segid, gtm->ctGTMSeg->entries[segid]->name);
      printf("It may be that there are no voxels for this seg when mapped "
             "into the input space. \nCheck %s/aux/seg.nii.gz and the "
             "registration\n",
             gtm->OutDir);
      err++;
      continue;
    }
    nthsegpvfbb = MRIextractRegion(nthsegpvf, nullptr, region); // extract BB
    if (nthsegpvfbb == nullptr) {
      printf("ERROR: extracting nthseg=%d, segid=%d, %s\n", nthseg, segid,
             gtm->ctGTMSeg->entries[segid]->name);
      printf("It may be that there are no voxels for this seg when mapped "
             "into the input space. \nCheck %s/aux/seg.nii.gz and the "
             "registration\n",
             gtm->OutDir);
    ...
    for (s = 0; s < gtm->yvol->depth; s++) {
      for (c = 0; c < gtm->yvol->width; c++) {
        for (r = 0; r < gtm->yvol->height; r++) {
          ...
          if (!gtm->Optimizing)
            gtm->X0->rptr[k][nthseg + 1] = MRIgetVoxVal(
                nthsegpvfbb, c - region->x, r - region->y, s - region->z, 0);

          gtm->X->rptr[k][nthseg + 1] = MRIgetVoxVal(
              nthsegpvfbbsm, c - region->x, r - region->y, s - region->z, 0);
        }
      }
    }
    MRIfree(&nthsegpvf);
...
/*
  ...
  \brief Creates a volume that is the same size as the anatseg in which the
  value at a voxel equals the beta of the seg ID at that voxel. This is used for
  RBV. If frame < 0, then all frames are are computed. If frame >= 0, then synth
...
MRI *GTMsegSynth(GTM *gtm, int frame, MRI *synth) {
  ...
  if (synth == nullptr) {
    synth = MRIallocSequence(gtm->rbvseg->width, gtm->rbvseg->height,
                             gtm->rbvseg->depth, MRI_FLOAT, nframes);
    if (synth == nullptr) {
    ...
    for (r = 0; r < gtm->rbvseg->height; r++) {
      for (s = 0; s < gtm->rbvseg->depth; s++) {
        ...
        if (frame < 0) {
          ...
          MRIsetVoxVal(synth, c, r, s, 0,
                       gtm->beta->rptr[segno + 1][frame + 1]);
      }
    }
  }
...
/*
  ...
  The factor = 100/mean(beta_i) where i is the list of scale seg IDs
  (scale_ref_ids)
*/
...
int GTMttest(GTM *gtm) {
  ...
  for (n = 0; n < gtm->nContrasts; n++) {
    ...
    for (f = 0; f < nframes; f++) {
      ...
      gtmc->t->rptr[1][f + 1] =
          gtmc->gamma->rptr[1][f + 1] / sqrt(gtmc->gammavar->rptr[1][f + 1]);
...
/*
  ...
  \brief Creates an ASCII file in the output folder for each contrast with
    ...
    for (n = 0; n < gtm->n_scale_refids; n++) {
      ...
      if (!ok) {
        printf("ERROR: scale reference id %d cannot be found in seg id list\n",
               gtm->scale_refids[n]);
        fprintf(gtm->logfp,
                "ERROR: scale reference id %d cannot be found in seg id list\n",
                gtm->scale_refids[n]);
        return (1);
      }
    }
  ...
  if (gtm->DoKMHB) {
    for (n = 0; n < gtm->n_km_hbids; n++) {
      ...
      if (!ok) {
        ...
        fprintf(
            gtm->logfp,
            "ERROR: scale high binding id %d cannot be found in seg id list\n",
            gtm->km_hbids[n]);
        return (1);
      }
    }
...
int GTMprintRefIds(GTM *gtm, FILE *fp) {
  ...
  if (gtm->rescale) {
    ...
    for (n = 0; n < gtm->n_scale_refids; n++)
      fprintf(fp, "%4d %s\n", gtm->scale_refids[n],
              gtm->ctGTMSeg->entries[gtm->scale_refids[n]]->name);
  }
  ...
  if (gtm->DoMGPVC) {
    ...
    for (n = 0; n < gtm->n_mg_refids; n++)
      fprintf(fp, "%4d %s\n", gtm->mg_refids[n],
              gtm->ctGTMSeg->entries[gtm->mg_refids[n]]->name);
  }
  ...
  if (gtm->DoKMRef) {
    ...
    for (n = 0; n < gtm->n_km_refids; n++)
      fprintf(fp, "%4d %s\n", gtm->km_refids[n],
              gtm->ctGTMSeg->entries[gtm->km_refids[n]]->name);
  }
  ...
  if (gtm->DoKMHB) {
    ...
    for (n = 0; n < gtm->n_km_hbids; n++)
      fprintf(fp, "%4d %s\n", gtm->km_hbids[n],
              gtm->ctGTMSeg->entries[gtm->km_hbids[n]]->name);
  }
  fflush(fp);
  return (0);
}
...
/*
  \fn int GTMprintReplaceList(FILE *fp, const int nReplace, const int
  *ReplaceThis, const int *WithThat) \brief Prints the replacement list to the
  FILE pointer.
 */
int GTMprintReplaceList(FILE *fp, const int nReplace, const int *ReplaceThis,
                        const int *WithThat) {
  int n;
...
/*
  \fn int GTMcheckReplaceList(const int nReplace, const int *ReplaceThis, const
  int *WithThat) \brief Checks replacement list to make sure that no item in
  ReplaceThis list appears in the WithThat list.
 */
int GTMcheckReplaceList(const int nReplace, const int *ReplaceThis,
                        const int *WithThat) {
  int n, m;
  for (n = 0; n < nReplace; n++) {
    for (m = 0; m < nReplace; m++) {
      if (ReplaceThis[n] == WithThat[m]) {
        printf("ERROR: item %d appears as both source and target seg id in "
               "replacement list\n",
               ReplaceThis[n]);
        return (1);
      }
...
/*
  \fn int GTMloadReplacmentList(const char *fname, int *nReplace, int
  *ReplaceThis, int *WithThat) \brief Loads in data from a file. The file should
  have two columns of numbers. The first is the segmentation ID to replace the
  second is the segmentation ID to replace it with. It will ignore any line that
  begins with a #.
 */
int GTMloadReplacmentList(const char *fname, int *nReplace, int *ReplaceThis,
  ...
  while (true) {
    ...
    if (r != 2) {
      printf("ERROR: line %d in %s has the wrong format (has %d elements, "
             "expecting 2)\n",
             nth, fname, r);
      return (1);
    }
...
int GTMautoMask(GTM *gtm) {
  ...
  if (LTAmriIsSource(gtm->seg2pet, gtm->yvol))
    lta = LTAcopy(gtm->seg2pet, nullptr);
  else
    lta = LTAinvert(gtm->seg2pet, nullptr);
  LTAchangeType(lta, LINEAR_VOX_TO_VOX);
      ...
      for (c = 0; c < gtm->yvol->width; c++) {
        for (r = 0; r < gtm->yvol->height; r++) {
          ...
          if (ct->entries[segid] == nullptr) {
            printf("WARNING: GTMrvarGM(): segid=%d (%d,%d,%d) does not exist "
                   "in ctab\n",
                   segid, c, r, s);
          ...
          if (tt == 1 || tt == 2) {
            // GM (hardcoded)
            sum += ((double)gtm->res->rptr[n + 1][f + 1] *
                    gtm->res->rptr[n + 1][f + 1]);
            nhits++;
          }
          if (tt == 1 || tt == 2 || tt == 3) {
            // GM  or WM (hardcoded)
            sumb += ((double)gtm->res->rptr[n + 1][f + 1] *
                     gtm->res->rptr[n + 1][f + 1]);
            nhitsb++;
          }
        }
...
MRI **GTMlocal(GTM *gtm, MRI **pvc) {
  ...
  MRI *const pvfpsf =
      MRIgaussianSmoothNI(gtm->ttpvf, gtm->cStd, gtm->rStd, gtm->sStd, nullptr);
  // Need to add MB here
  ...
  if (pvc == nullptr) {
    ...
    for (tt = 0; tt < nTT; tt++) {
      pvc[tt] =
          MRIallocSequence(gtm->yvol->width, gtm->yvol->height,
                           gtm->yvol->depth, MRI_FLOAT, gtm->yvol->nframes);
      if (pvc[tt] == nullptr) {
  ...
  int const nvmax = (2 * gtm->lgtm->nrad + 1) * (2 * gtm->lgtm->nrad + 1) *
                    (2 * gtm->lgtm->nrad + 1);
    ...
    for (r = 0; r < gtm->yvol->height; r++) {
      for (s = 0; s < gtm->yvol->depth; s++) {
        ...
        for (dc = -gtm->lgtm->nrad; dc <= gtm->lgtm->nrad; dc++) {
          ...
          for (dr = -gtm->lgtm->nrad; dr <= gtm->lgtm->nrad; dr++) {
            ...
            for (ds = -gtm->lgtm->nrad; ds <= gtm->lgtm->nrad; ds++) {
              ...
              for (f = 0; f < gtm->yvol->nframes; f++)
                y->rptr[nth + 1][f + 1] =
                    MRIgetVoxVal(gtm->yvol, c + dc, r + dr, s + ds, f);
              for (tt = 0; tt < pvfpsf->nframes; tt++) {
        ...
        if (nkeep != pvfpsf->nframes || nv != nvmax) {
          // Either not all tissue types will be kept or not all nvmax were
          // avail
        ...
        eres = MatrixSubtract(y, yhat, nullptr);
        for (f = 0; f < gtm->yvol->nframes; f++)
...
int GTMttPercent(GTM *gtm) {
  ...
  for (nthseg = 0; nthseg < gtm->nsegs; nthseg++) {
    ...
    for (tt = 0; tt < nTT; tt++)
      gtm->ttpct->rptr[nthseg + 1][tt + 1] =
          100 * gtm->ttpct->rptr[nthseg + 1][tt + 1] / sum;
  }
