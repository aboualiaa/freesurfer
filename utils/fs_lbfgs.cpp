...
fs_lbfgs::fs_lbfgs() : fs_lbfgs_subject(), f_(nullptr) { init_parameters(); }

//: Constructor. f is the cost function to be minimized.
// Calls init_parameters
fs_lbfgs::fs_lbfgs(vnl_cost_function &f) : fs_lbfgs_subject(), f_(&f) {
  init_parameters();
}
...
bool fs_lbfgs::minimize(vnl_vector<double> &x) {
  ...
  if (verbose_)
    std::cerr << "vnl_lbfgs: n = " << n << ", memory = " << m
              << ", Workspace = " << w.size() << "[ "
              << (w.size() / 128.0 / 1024.0)
              << " MB], ErrorScale = " << f_->reported_error(1)
...
#define print_(i, a, b, c, d)                                                  \
  std::cerr << std::setw(6) << i << ' ' << std::setw(20) << a << ' '           \
            << std::setw(20) << b << ' ' << std::setw(20) << c << ' '          \
            << std::setw(20) << d << '\n'

    if (check_derivatives_) {
      std::cerr << "vnl_lbfgs: f = " << f_->reported_error(f)
      ...
      std::cerr << "   ERROR = "
                << (fdg - g).squared_magnitude() / std::sqrt(double(n)) << "\n";
    }
    ...
    v3p_netlib_lbfgs_(&n, &m, x.data_block(), &f, g.data_block(), &diagco,
                      diag.data_block(), iprint, &eps, &local_xtol,
                      w.data_block(), &iflag, &lbfgs_global);
