...
HISTOGRAM *HISTOrealloc(HISTOGRAM *histo, int nbins) {
  ...
  if (!histo->counts || !histo->bins)
    ErrorExit(ERROR_NOMEMORY, "HISTOrealloc(%d): could not allocate histogram",
              nbins);
...
HISTOGRAM *HISTOalloc(int nbins) {
  ...
  if (!histo->counts || !histo->bins)
    ErrorExit(ERROR_NOMEMORY, "HISTOalloc(%d): could not allocate histogram",
              nbins);
  histo->max_bins = histo->nbins = nbins;
...
HISTOGRAM *HISTOcopy(HISTOGRAM *histo_src, HISTOGRAM *histo_dst) {
  ...
  memmove(histo_dst->counts, histo_src->counts,
          sizeof(*histo_src->counts) * histo_src->nbins);
  memmove(histo_dst->bins, histo_src->bins,
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
HISTOGRAM *HISTOclearBins(HISTOGRAM *histo_src, HISTOGRAM *histo_dst,
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
#define MAX_LEN 2000
HISTOGRAM *HISTOsmooth(HISTOGRAM *histo_src, HISTOGRAM *histo_dst,
                       float sigma) {
  ...
  if (!histo_dst) {
    histo_dst = HISTOcopy(histo_src, nullptr);
...
int HISTOfindLastPeakInRegion(HISTOGRAM *h, int wsize, float min_pct, int I0,
                              int I1) {
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int HISTOfindFirstPeakInRegion(HISTOGRAM *h, int wsize, float min_pct, int b0,
                               int b1) {
  int   peak, b, bw, nbins, whalf;
...
/*
   ...
   two or above.
*/
int HISTOcountPeaksInRegion(HISTOGRAM *h, int wsize, float min_pct, int *peaks,
                            int max_peaks, int b0, int b1) {
...
int HISTOaddFractionalSample(HISTOGRAM *histo, float val, float bmin,
                             float bmax, float frac) {
  int   bin_no;
  float bin_size;
...
int HISTOfillHoles(HISTO *h) {
  ...
  if (filled == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "HISTOfillHoles: couldn't allocate %d-len buffer\n", h->nbins);
...
static double histoComputeLinearFitCorrelation(HISTOGRAM *h1, HISTOGRAM *h2,
                                               double a, double b) {
...
#define NSTEPS 1000
double HISTOfindLinearFit(HISTOGRAM *h1, HISTOGRAM *h2, double amin,
                          double amax, double bmin, double bmax, float *pa,
                          float *pb) {
...
HISTOGRAM *HISTOmakeReverseCDF(HISTOGRAM *hsrc, HISTOGRAM *hdst) {
  ...
  habs = HISTOabs(hsrc, nullptr);

  if (hdst == nullptr)
    hdst = HISTOcopy(hsrc, nullptr);
...
int HISTOrobustGaussianFit(HISTOGRAM *h, double max_percentile, double *poffset,
                           double *psigma) {
  int        peak, bin, min_bin, max_bin, zbin, n;
  HISTOGRAM *habs, *hcdf, *hz, *hs;
  double thresh, sigma, delta_sigma, max_sigma, predicted_val, val, sqrt_2pi,
      sse, best_sigma, best_sse, error, scale, mean;

  if (h->bin_size <= 0)
    h->bin_size = 1;
  sqrt_2pi = sqrt(2 * M_PI);
  hs       = HISTOsmooth(h, nullptr, 2.0 / h->bin_size);
  peak     = HISTOfindHighestPeakInRegion(hs, 0, hs->nbins);
  mean = *poffset = hs->bins[peak];
  hz              = HISTOlinearScale(h, nullptr, 1.0, -mean);
  habs            = HISTOabs(hz, nullptr);
  hcdf            = HISTOmakeCDF(hz, nullptr);
  ...
  bin = HISTOfindNextValley(
      hs,
      peak + 10 / hs->bin_size); // real valley must not be right next to peak
  if (bin < 0) {
    bin    = HISTOfindBinWithCount(hcdf, max_percentile);
    thresh = hcdf->bins[bin]; // threshold for computing variance
  } else // found a valid next valley. Now look backwards to find bulk of
         // distribution
  {
  ...
  if (hz->nbins == 0)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "HISTOrobustGaussianFit: 0 length cdf"));
...
#define MIN_BINS 12 // need enough data to estimate parameters of gaussian
  ...
  for (sigma = delta_sigma; sigma <= max_sigma; sigma += delta_sigma) {
    ...
    for (n = 0, bin = min_bin; bin <= max_bin; bin++) {
      val = hz->bins[bin];
      predicted_val =
          1 / (sigma * sqrt_2pi) * exp(-val * val / (2 * sigma * sigma));
    ...
    for (bin = min_bin; bin <= max_bin; bin++) {
      val = hz->bins[bin];
      predicted_val =
          scale / (sigma * sqrt_2pi) * exp(-val * val / (2 * sigma * sigma));
  ...
  *psigma = best_sigma;
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    ...
    for (bin = 0; bin < hp->nbins; bin++) {
      ...
      predicted_val = scale / (best_sigma * sqrt_2pi) *
                      exp(-val * val / (2 * best_sigma * best_sigma));
...
HISTOGRAM *HISTOgaussianCDF(HISTOGRAM *h, double mean, double sigma,
                            int nbins) {
  HISTOGRAM *hpdf;
...
HISTOGRAM *HISTOgaussianPDF(HISTOGRAM *h, double mean, double sigma,
                            int nbins) {
  int    bin;
  double val, sqrt_2pi;
    ...
    for (b = 0; b < hsrc->nbins; b++) {
      if (control[b])
        ...
        if (b == 0)
          ...
          tmp[b] =
              (hdst->counts[b + 1] + hdst->counts[b] + hdst->counts[b - 1]) / 3;
      }
    }
    for (b = 0; b < hsrc->nbins; b++)
      hdst->counts[b] = tmp[b];
...
int HISTOfindMaxDerivative(HISTOGRAM *h, double min_count, double max_count,
                           int whalf, int grad_dir) {
  ...
  for (index = 0; index < h->nbins; index++) {
    for (prev_val = 0.0, num = 0, i0 = MAX(0, index - whalf); i0 < index;
         i0++, num++)
...
HISTOGRAM2D *HISTO2Dalloc(int nbins1, int nbins2) {
  ...
  if (!histo)
    ErrorExit(ERROR_NO_MEMORY, "HISTO2Dalloc(%d, %d): allocation failed",
              nbins1, nbins2);
  ...
  if (!histo->counts || !histo->bins1 || !histo->bins2)
    ErrorExit(ERROR_NOMEMORY,
              "HISTO2Dalloc(%d, %d): could not allocate histogram", nbins1,
              nbins2);
  histo->nbins1 = nbins1;
  histo->nbins2 = nbins2;
  for (i = 0; i < nbins1; i++) {
    ...
    if (!histo->counts[i])
      ErrorExit(ERROR_NOMEMORY,
                "HISTO2Dalloc(%d, %d): could not allocate histogram slice %d",
                nbins1, nbins2, i);
  }
      ...
      for (bin2 = 0; bin2 < histo->nbins2; bin2++)
        if (histo->counts[bin1][bin2])
          fprintf(fp, "bin[%d, %d] = %2.1f, %2.1f = %2.2f\n", bin1, bin2,
                  histo->bins1[bin1], histo->bins2[bin2],
...
HISTOGRAM2D *HISTO2Dread(char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "HISTO2Dread(%s); could not open file", fname));
...
HISTOGRAM2D *HISTO2Dclear(HISTOGRAM2D *histo_src, HISTOGRAM2D *histo_dst) {
  ...
  for (b = 0; b < histo_dst->nbins1; b++)
    memset(histo_dst->counts[b], 0,
           histo_dst->nbins2 * sizeof(*histo_dst->counts[b]));

  return (histo_dst);
}
HISTOGRAM2D *HISTO2Dinit(HISTOGRAM2D *h, int nbins1, int nbins2, double mn1,
                         int mx1, double mn2, double mx2) {
  int b;
...
HISTOGRAM2D *HISTO2Drealloc(HISTOGRAM2D *histo, int nbins1, int nbins2) {
  ...
  if (!histo->counts || !histo->bins1 || !histo->bins2)
    ErrorExit(ERROR_NOMEMORY,
              "HISTO2Drealloc(%d, %d): could not allocate histogram", nbins1,
              nbins2);
  histo->nbins1 = nbins1;
  histo->nbins2 = nbins2;
  for (b = 0; b < histo->nbins1; b++)
...
int HISTO2DaddFractionalSample(HISTOGRAM2D *histo, float val1, float val2,
                               float bmin1, float bmax1, float bmin2,
                               float bmax2, float frac) {
  int   bin_no1, bin_no2;
...
int HISTO2DaddSample(HISTOGRAM2D *histo, float val1, float val2, float bmin1,
                     float bmax1, float bmin2, float bmax2) {
  int   bin_no1, bin_no2;
  float bin_size1, bin_size2;
...
HISTOGRAM2D *HISTO2Dcopy(HISTOGRAM2D *histo_src, HISTOGRAM2D *histo_dst) {
  ...
  // use memmove, not memcpy, as src and dst could overlap
  for (b = 0; b < histo_dst->nbins1; b++)
    memmove(histo_dst->counts[b], histo_src->counts[b],
            sizeof(*histo_src->counts[b]) * histo_src->nbins2);

  memmove(histo_dst->bins1, histo_src->bins1,
          sizeof(*histo_src->bins1) * histo_src->nbins1);
  memmove(histo_dst->bins2, histo_src->bins2,
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
#define MAX_LEN 2000
HISTOGRAM2D *HISTO2Dsmooth(HISTOGRAM2D *histo_src, HISTOGRAM2D *histo_dst,
                           float sigma) {
  float norm, two_sigma, fx, k, kernel[MAX_LEN], total;
      ...
      for (norm = 0.0, total = 0.0f, x = 0; x < len; x++)
        for (y = 0; y < len; y++) {
          ...
          if ((float)histo_src->counts[b1k][b2k] > 0.00027)
            DiagBreak();
          if (!std::isfinite(norm) || !std::isfinite(total) ||
              !std::isfinite(kernel[i]))
            DiagBreak();
        }
      if (b1 == Gx && b2 == Gy)
        DiagBreak();
...
#if 1
HISTOGRAM2D *HISTO2DsmoothAnisotropic(HISTOGRAM2D *histo_src,
                                      HISTOGRAM2D *histo_dst, float sigma1,
                                      float sigma2) {
  float norm, k, total;
...
HISTOGRAM2D *HISTO2DsmoothAnisotropic(HISTOGRAM2D *histo_src,
                                      HISTOGRAM2D *histo_dst, float sigma1,
                                      float sigma2) {
  float norm, two_sigma, fx, k, k1, k2, kernel1[MAX_LEN], kernel2[MAX_LEN],
      total;
  int x, half1, half2, len1, len2, b1, b2, kx, nbins1, nbins2, y, ky, b1k, b2k;
...
#endif
#endif
HISTOGRAM2D *HISTO2DsmoothBins1(HISTOGRAM2D *histo_src, HISTOGRAM2D *histo_dst,
                                float sigma) {
  float norm, two_sigma, fx, k, kernel[MAX_LEN], total;
...
HISTOGRAM2D *HISTO2DsmoothBins2(HISTOGRAM2D *histo_src, HISTOGRAM2D *histo_dst,
                                float sigma) {
  float norm, two_sigma, fx, k, kernel[MAX_LEN], total;
...
HISTOGRAM2D *HISTO2DsoapBubbleZeros(HISTOGRAM2D *hsrc, HISTOGRAM2D *hdst,
                                    int niters) {
  ...
  // first set values of all zero bins that are adjacent to non-zero to the avg
  // of the nbrs
  for (b1 = 0; b1 < hsrc->nbins1; b1++)
    for (b2 = 0; b2 < hsrc->nbins2; b2++) {
      if (control[b1][b2])
...
double HISTOearthMoversDistance(HISTOGRAM *h1, HISTOGRAM *h2) {
  ...
  h1 = HISTOcopy(h1, nullptr);
  HISTOmakePDF(h1, h1);
  h2 = HISTOcopy(h2, nullptr);
  HISTOmakePDF(h2, h2);

  HISTOfree(&h1);
  HISTOfree(
      &h2); // these were copied at the beginning, not the caller's version
  return (emd);
}
...
HISTOGRAM *HISTOeraseRightmostPeak(HISTOGRAM *hsrc, HISTOGRAM *hdst, int whalf,
                                   float min_pct, int min_val, int max_val) {
  int bmin, bmax, b;
