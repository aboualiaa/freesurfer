...
LookupTable::LookupTable(std::string filename) { importFromFile(filename); }
...
std::vector<int> LookupTable::labels() {
  ...
  for (auto const &l : labelmap)
    labellist.push_back(l.first);
  return labellist;
}

bool LookupTable::empty() { return labelmap.empty(); }

bool LookupTable::hasNameInfo() {
  bool has_names = false;
  for (auto const &label : labelmap)
    if (!label.second.name.empty())
      has_names = true;
  return has_names;
}
