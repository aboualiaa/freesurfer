...
int ModifyTalairachCRAS(MRI *mri_tal, const LTA *lta) {
  ...
  if (lta != nullptr) {
        ...
        if (tran->dst.valid == 1) // transform dst is valid
        {
          ...
          if (DIAG_VERBOSE_ON) {
            fprintf(stderr, "INFO: Modifying talairach volume c_(r,a,s) based "
                            "on average_305.\n");
            fprintf(stderr, "INFO: If not preferred, set environmental varible "
                            "NO_AVERAGE305 true.\n");
          }
          mri_tal->c_r = -0.095;
          mri_tal->c_a = -16.51;
    ...
    if (getenv("NO_AVERAGE305")) // if this is set
      ...
      // use average_305 value
      if (DIAG_VERBOSE_ON)
        fprintf(stderr, "INFO: Modifying talairach volume c_(r,a,s) based on "
                        "average_305\n");
      mri_tal->c_r = -0.095;
      mri_tal->c_a = -16.51;
      mri_tal->c_s = 9.75;
...
MATRIX *MtalairachFromVoxel(MRI *mri_src, const LTA *lta) {
  ...
  if (lta) {
    ...
    talRASfromVoxel = MatrixMultiply(lta->xforms[0].m_L, RASfromVoxel,
                                     NULL); // allocate memory
  } else                                    // no transform.  just copy
    talRASfromVoxel = MatrixCopy(RASfromVoxel, nullptr);
...
MATRIX *MtalVoxelFromVoxel(MRI *mri_src, const LTA *lta) {
  ...
  mri_talvol = MRIallocHeader(mri_src->width, mri_src->height, mri_src->depth,
                              mri_src->type, 1);
...
#define DOUBLE_INTERNAL 0
  ...
  *px =
      mat->data[0] * xf + mat->data[1] * yf + mat->data[2] * zf + mat->data[3];
  *py =
      mat->data[4] * xf + mat->data[5] * yf + mat->data[6] * zf + mat->data[7];
  *pz = mat->data[8] * xf + mat->data[9] * yf + mat->data[10] * zf +
        mat->data[11];
#endif
...
int MRItalairachVoxelToWorldEx(MRI *mri_dst, double xt, double yt, double zt,
...
int MRItalairachVoxelToVoxelEx(MRI *mri_dst, double xtv, double ytv, double ztv,
                               double *pxv, double *pyv, double *pzv,
                               const LTA *lta) {
...
MRI *MRItoTalairachExInterp(MRI *mri_src, MRI *mri_tal, const LTA *lta,
  ...
  if (!mri_tal) {
    mri_tal = MRIclone(mri_src, nullptr); // data is not copied
...
MRI *MRIfromTalairachEx(MRI *mri_tal, MRI *mri_dst, const LTA *lta) {
  ...
  if (!mri_dst) {
    ErrorExit(ERROR_BADPARM, "%s: Needs target volume to recover c_(ras)",
              Progname);
