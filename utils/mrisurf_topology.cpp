...
static bool shouldReportWkr(int line) {
  bool wasReported;
  if (copeWithLogicProblem2(&wasReported, "FREESURFER_crash_on_bad_topology",
                            "Bad vertex or face found", __FILE__, line,
...
bool mrisCheckVertexVertexTopologyWkr(const char *file, int line,
                                      MRIS const *mris, bool always) {
  {
    ...
    if (!laterTime) {
      ...
      if (forceAlways)
        fprintf(stdout, "%s:%d FREESURFER_always_check_vv set\n", __FILE__,
                __LINE__);
    }
    always |= forceAlways;
  }
  if (!always && !spendTimeCheckingForLogicProblem(file, line))
    return true;

  int reported = 0;
#define shouldReport                                                           \
  (shouldReportWkr(__LINE__) &&                                                \
   (reported = 1)) // sets reported only if should report
  ...
  for (vno1 = 0; vno1 < mris->nvertices; vno1++) {
    ...
    if ((v_nontopo->dist && !(mris->dist_alloced_flags & 1)) ||
      ...
      if (shouldReport) {
        fprintf(stdout,
                "dist or dist_orig non-null when !mris->dist_alloced\n");
      }
    }
    ...
    if (mris->nsize > 0 && mris->nsize != v->nsizeCur && vSize > 0) {
      if (shouldReport) {
        fprintf(stdout,
                "[vno1:%d].nsizeCur:%d != mris->nsize:%d vSize:%d vnum:%d "
                "vtotal:%d\n",
                vno1, v->nsizeCur, mris->nsize, vSize, v->vnum, v->vtotal);
      }
    }

    if (v->nsizeMax > 0 && v->nsizeCur > v->nsizeMax) {
      if (shouldReport) {
    ...
    if (v->nsizeCur > 0 &&
        (mris->vtotalsMightBeTooBig ? (v->vtotal < vtotalExpected)
                                    : (v->vtotal != vtotalExpected))) {
      //
      // MRISsampleDistances sets vtotal beyond vtotalExpected
      //
      if (shouldReport) {
        fprintf(stdout,
                "[vno1:%d].vtotal:%d differs from expected:%d for nsize:%d, "
                "ripflag:%d\n",
  ...
  if (!laterTime) {
    ...
    if (checkDist)
      fprintf(stdout, "%s:%d checking dist[] contents\n", __FILE__, __LINE__);
  }
  if (checkDist) {
...
void MRIScheckIsPolyhedron(MRIS *mris, const char *file, int line) {
  ...
  for (maxNeighbours = 16; maxNeighbours < 1024; maxNeighbours *= 2) {
    ...
    for (vno = 0; vno < nvertices; vno++) {
      ...
      for (n = 0; n < vt->vnum; n++) {
        ...
        if (mris->vertices[vno2].ripflag)
          continue;
        if (vno < vno2) {
          if (hiVnosSize == hiVnosCapacity)
            goto TryLargerCapacity;
          hiVnos[hiVnosSize++] = vno2;
        } else {
          Edges const *const edgesForVno2 = &edges[vno2];
          int                count        = 0;
          for (unsigned int i = 0; i < edgesForVno2->hiVnosCount; i++) {
            if (hiVnos[edgesForVno2->hiVnosBegin + i] == vno)
              count++;
          }
  ...
  do {
    ...
    for (fno = 0; fno < nfaces; fno++) {
      ...
      for (n1 = 0; n1 < VERTICES_PER_FACE; n1++) {
        ...
        if (vno1 > vno2) {
          ...
          cheapAssert(vno1 != vno2);
        Edges const *const edgesForVno = &edges[vno1];
        for (unsigned int i = 0; i < edgesForVno->hiVnosCount; i++) {
          if (hiVnos[edgesForVno->hiVnosBegin + i] != vno2)
            continue;
          // found the entry for an edge
          if (!badCount) {
            nFacesPerEdge[edgesForVno->hiVnosBegin +
                          i]++; // yeah, might wrap - unlikely
          } else {
            int const count = nFacesPerEdge[edgesForVno->hiVnosBegin + i];
            if (count != 2) {
              if ((count == 1) ? (reports1++ < 10) : (reportsNot1++ < 10)) {
                fprintf(
                    stdout,
                    "Fno %d contributes edge %d..%d towards a count of %d\n",
                    fno, vno1, vno2, count);
              }
            }
          }
    ...
    for (unsigned int edgeNo = 0; edgeNo < hiVnosSize; edgeNo++) {
      ...
      badCount = true;
      fprintf(stdout,
              "MRIScheckIsPolyhedron failed during call %d at %s:%d, "
              "previously good at %s:%d because count:%d for edgeNo:%d\n",
              MRIScheckIsPolyhedronCalls, file, line, prevFile, prevLine, count,
              edgeNo);
      break;
    }
    ...
    for (vno = 0; vno < VERTICES_PER_FACE; vno++)
      if (face->v[vno] == vno1) {
        if ((vno == VERTICES_PER_FACE - 1) && (face->v[0] == vno2)) {
          nvalid++;
        } else if ((vno < VERTICES_PER_FACE - 1) &&
                   (face->v[vno + 1] == vno2)) {
          nvalid++;
        }
      }
...
short modVnum(MRIS const *mris, int vno, short add, bool clear) {
  ...
  *p += add;
  ...
  if (*p > maxVnumSeen) {
    maxVnumSeen = *p;
    fs::debug() << "modVnum: vertex " << vno << " has " << *p
  ...
  auto adjustV = [&](int vnoA, int vnoB) {
    ...
    if (!v->v)
      ErrorExit(ERROR_NO_MEMORY,
                "mrisAddEdge(%d, %d): could not allocate %d len vlist",
                v->vnum);

    v->v[vnumAdd(mris, vnoA, 1)] = vnoB;
    v->vtotal                    = v->vnum;
    v->nsizeCur = v->nsizeMax = 1;

    static int maxVnum = 20;
    if (maxVnum < v->vnum) {
      maxVnum = v->vnum;
      fs::debug() << "vertex " << vnoA << " has " << v->vnum
                  << " immediate neighbours";
    }
  };
...
//
int MRIScomputeEulerNumber(MRIS *mris, int *pnvertices, int *pnfaces,
                           int *pnedges) {
        ...
        for (n = 0; n < v0t->num; n++) { // go thru faces of vtx1
          ...
          for (m = 0; m < v1t->num; m++) { // go thru faces of vtx2
            if (v0t->f[n] == v1t->f[m]) {  // same face
              if (k > 1) {
                printf("ERROR: MRISedges(): too many faces: %d %d n=%d, m=%d, "
                       "k=%d\n",
                       vtxno0, vtxno1, n, m, k);
                return (1);
              }
...
int MRIScorners(MRIS *surf) {
  ...
  for (cornerno = 0; cornerno < surf->ncorners; cornerno++) {
    ...
    for (nthedge = 0; nthedge < 2; nthedge++) {
      ...
      if (!hit) {
        printf("MRIScorners(): ERROR: could not find an edge for corner %d, "
               "edge %d, %d->%d\n",
...
static void resizeVertexV(MRIS *mris, int vno, int newSize, int oldSize) {
  ...
  //
  if (v->dist)
    MRISgrowDist(mris, vno, newSize);
}

void mrisVertexReplacingNeighbors(MRIS *const mris, int const vno,
                                  int const vnum) {
  VERTEX_TOPOLOGY *const vt = &mris->vertices_topology[vno];

  mris->max_nsize = vt->nsizeMax = vt->nsizeCur = 1;
  if (mris->nsize != 1)
    mris->nsize = -1;            // vertices are now inconsistent
...
int MRIScountTotalNeighbors(MRIS *mris, int nsize) {
  ...
  for (total_nbrs = vno = 0; vno < mris->nvertices; vno++) {
    ...
    case 3:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "MRIScountNeighbors(%d): max nbhd size = 3",
                nsize);
    }
  }
  return (total_nbrs);
...
int MRISresetNeighborhoodSize(MRI_SURFACE *mris, int nsize) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    default: /* reset back to original */
      ...
      if (new_mris_nsize < 0)
        new_mris_nsize = vt->nsizeMax;
      else
        cheapAssert(new_mris_nsize == vt->nsizeMax);
      break;
    case 1:
    case 2:
    case 3:
...
void MRISgetNeighborsBeginEnd(
  ...
  if (inner_nbhd_size <= outer_nbhd_size) {
    ...
    cheapAssert(mris->nsizeMaxClock == vt->nsizeMaxClock ||
                outer_nbhd_size <= 1);
...
void MRIS_VertexNeighbourInfo_load_from_VERTEX(MRIS_VertexNeighbourInfo *info,
  ...
  default:
    cheapAssert(false);
  case 3:
    info->vnum[3] = vt->v3num;
  case 2:
    info->vnum[2] = vt->v2num;
  case 1:
    info->vnum[1] = vt->vnum;
  case 0:
    info->vnum[0] = 1;
...
void MRIS_VertexNeighbourInfo_load_from_vlist(MRIS_VertexNeighbourInfo *info,
                                              MRIS *mris, int vno,
                                              size_t listSize, int *vlist,
                                              int *hops) {
  info->vnum[0] = 1;
...
static int MRISfindNeighborsAtVertex_new(MRIS *mris, int vno, int nlinks,
                                         size_t listCapacity, int *vlist,
                                         int *hops, bool noCache);
static int MRISfindNeighborsAtVertex_old(MRIS *mris, int vno, int nlinks,
                                         size_t listCapacity, int *vlist,
                                         int *hops);
...
int MRISfindNeighborsAtVertex(MRIS *mris, int vno, int nlinks,
  ...
  int result_old = use_old ? MRISfindNeighborsAtVertex_old(
  ...
  int result_new =
      use_new
          ...
          //
          ? MRISfindNeighborsAtVertex_new(
                mris, vno, nlinks, use_both ? MAX_NEIGHBORS : listCapacity,
                use_both ? vlistTmp : vlist, use_both ? hopsTmp : hops, false)
          : 0;
  ...
  if (use_both) {
    ...
    for (i = 0; i < result_old && i < result_new; i++) {
      if (vlist[i] != vlistTmp[i] || hops[i] != hopsTmp[i]) {
        fprintf(stdout,
                " vlist[%d] old:%d != new:%d  or  hops[%d] old:%d != new:%d\n",
                i, vlist[i], vlistTmp[i], i, hops[i], hopsTmp[i]);
        good = false;
      }
    }
    ...
    if (!good) {
      fprintf(stdout, "%s:%d laterTime:%d interestingLaterTime:%d\n", __FILE__,
              breakLine, laterTime, interestingLaterTime);
      cheapAssert(false);
    }
  }

  if (use_both) {
    freeAndNULL(vlistTmp);
    freeAndNULL(hopsTmp);
  }
...
void MRIS_check_vertexNeighbours(MRIS *mris) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    int size = MRISfindNeighborsAtVertex_new(mris, vno, vt->nsizeMax,
                                             MAX_NEIGHBORS, vlist, hops, true);

    MRIS_VertexNeighbourInfo_load_from_vlist(&info1, mris, vno, size, vlist,
                                             hops);
...
static int MRISfindNeighborsAtVertex_newWkr(MRIS *mris, int vno, int nlinks,
                                            size_t listCapacity, int *vlist,
                                            int *hops, bool noCache,
                                            bool debug = false) {
  /*
  ...
  */
  ...
  if (temp->capacity < (unsigned)mris->nvertices) {
    ...
    temp->status   = (unsigned char *)realloc(
        temp->status, temp->capacity * sizeof(unsigned char));
  ...
  int nsize = 0;
  {
    ...
    if (!noCache) {
      if (vt->nsizeMax >= 2)
        vnums[nsize++] = vt->v2num;
      if (vt->nsizeMax >= 3)
        vnums[nsize++] = vt->v3num;
    }
  }
  ...
  for (ringLinks = 1; ringLinks < nsize && ringLinks <= nlinks; ringLinks++) {
    ...
    for (n = ringBegin; n < ringEnd; n++) {
      ...
      if (debug)
        fprintf(stdout, "  vnoCandidate:%d known to be in ring:%d\n",
  ...
  for (; ringLinks <= nlinks; ringLinks++) {
    ...
    for (i = knownRingBegin; i < knownRingEnd; i++) {
      ...
      for (j = 0; j < vtRing->vnum; j++) {
        ...
        if (vCandidate->ripflag)
          continue;
        if (temp->status[vnoCandidate] != Status_notInSet)
          continue;
        ...
        if (debug)
          fprintf(stdout,
                  "  vnoCandidate:%d is neighbour of vno:%d in ring:%d\n",
  ...
  if (!noCache) {
    ...
    if (nsize < newPossibleNsizeMax) {
      ...
      int cachedRing;
      for (cachedRing = nsize + 1; cachedRing <= newPossibleNsizeMax;
           cachedRing++) {
        switch (cachedRing) {
        case 1:
          modVnum(mris, vno, vnums[cachedRing], true);
          break; // happens when encounters ripped vertexs
        case 2:
          vt->v2num = vnums[cachedRing];
          break;
        case 3:
          vt->v3num = vnums[cachedRing];
          break;
        default:
          cheapAssert(false);
        }
        vt->nsizeMax      = newPossibleNsizeMax;
        vt->nsizeMaxClock = mris->nsizeMaxClock;
      }
...
static int MRISfindNeighborsAtVertex_new(MRIS *mris, int vno, int nlinks,
                                         size_t listCapacity, int *vlist,
                                         int *hops, bool noCache) {
  auto neighborCount = MRISfindNeighborsAtVertex_newWkr(
      mris, vno, nlinks, listCapacity, vlist, hops, noCache);
  if (neighborCount > listCapacity) {
    fprintf(stdout,
            "MRISfindNeighborsAtVertex: vno:%d has %d neighbours, but "
            "listCapacity is %ld\n",
            vno, neighborCount, listCapacity);
    MRISfindNeighborsAtVertex_newWkr(mris, vno, nlinks, listCapacity, vlist,
                                     hops, noCache, true);
...
static int MRISfindNeighborsAtVertex_old(MRIS *mris, int vno, int nlinks,
  ...
  for (n = 0; n < vt->vtotal; n++) {
    vlist[n] = vt->v[n];
    mris->vertices[vt->v[n]].marked =
        n < vt->vnum ? 1 : (n < vt->v2num ? 2 : 3);
  }
  ...
  if (nlinks <= mris->nsize) {
    ...
    default:
      vtotal = 0;
      ErrorExit(ERROR_BADPARM, "MRISfindNeighborsAtVertex: nlinks=%d invalid",
                nlinks);
      break;
    }
...
#define shouldReport                                                           \
  ...
  if (!mrisCheckVertexVertexTopologyWkr(file, line, mris, true))
    reported = 1;
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    ...
    for (n = 0; n < VERTICES_PER_FACE; n++) {
      ...
      if ((f->v[0] | f->v[1] | f->v[2]) == 0)
        continue; // HACK this is what an unattached face looks like
        ...
        for (iTrial = 0; iTrial < v->num; iTrial++) {
          if (v->f[iTrial] == fno) {
            if (i == v->num) {
              if (shouldReport) {
                fprintf(stdout,
                        "fno:%d found twice in [vno:%d].f[i:%d && iTrial:%d]\n",
                        fno, vno, i, iTrial);
              }
            }
            i = iTrial;
      ...
      if (!v->n) {
        ...
        if (v->n[i] != n) {
          if (shouldReport) {
            fprintf(stdout,
                    "[fno:%d].v[n:%d] holds vno:%d but [vno:%d].n[i:%d]:%d != "
                    "n:%d\n",
      ...
      if (!mrisVerticesAreNeighbors(mris, vno, prevVno)) {
        if (shouldReport) {
          fprintf(stdout,
                  "[fno:%d] holds adjacent vno:%d and vno:%d but they are not "
                  "neighbours\n",
                  fno, vno, prevVno);
        }
      }
...
int mrisCountAttachedFaces(MRIS *mris, int vno0, int vno1) {
  ...
  for (n = 0; n < vt->num; n++) { // For each face attached to vno0
    ...
    for (m = 0; m < VERTICES_PER_FACE; m++) { // For each vertex of the face
      if (f->v[m] == vno1)
        count++; // Count the faces sharing this edge
    }
  }
...
bool mrisCanAttachFaceToVertices(MRIS *mris, int vno0, int vno1, int vno2) {
  ...
  //
  return mrisCountAttachedFaces(mris, vno0, vno1) < 2 &&
...
static void mrisAddFaceToVertex(MRIS *mris, int vno, int fno, int n) {
  costlyAssert(mrisVertexFaceIndex(mris, vno, fno) <
               0); // check not already added
...
static void mrisAttachFaceWkr(MRIS *mris, int fno, int vno0, int vno1, int vno2,
                              bool edgesMustExist) {
...
void setFaceAttachmentDeferred(MRIS *mris, bool to) {
  ...
  if (!to)
    cheapAssert(mrisCheckVertexFaceTopology(mris));
}
...
MRIS *MRISfromVerticesAndFaces(const float *vertices, int nvertices,
  ...
  for (int n = 0; n < mris->nvertices; n++, v += 3)
    MRISsetXYZ(mris, n, v[0], v[1], v[2]);
  ...
  for (int n = 0; n < mris->nfaces; n++, f += 3) {
    ...
    if ((f->v[0] | f->v[1] | f->v[2]) == 0) {
      for (int i = 0; i < 3; i++)
        mrisAddFaceToVertex(mris, vno[i], n, i);
    }
  }
...
static void mrisDivideFace(MRIS *mris, int fno, int vno1, int vno2,
                           int vnew_no);

int mrisDivideEdgeTopologically(MRIS *const mris, int const vno1,
                                int const vno2) {
  if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
  ...
  if (mris->nfaces >= mris->max_faces - 1)
    DiagBreak();
  cheapAssert(!(v1->ripflag || v2->ripflag ||
                mris->nvertices >= mris->max_vertices ||
                mris->nfaces >= (mris->max_faces - 1)));

  /* check to make sure these vertices or the faces they are part of
     have enough room to expand.
  */
  cheapAssert(!(v1t->vnum >= MAX_VERTEX_NEIGHBORS ||
                v2t->vnum >= MAX_VERTEX_NEIGHBORS || v1t->num >= MAX_FACES ||
                v2t->num >= MAX_FACES));
  ...
  modVnum(mris, vnew_no, 2,
          true); /* at least connected to two bisected vertices */
...
static void mrisCompleteTopology_old(MRI_SURFACE *mris) // was mrisFindNeighbors
{
  ...
  for (k = 0; k < mris->nvertices; k++) {
    ...
    mris->vertices_topology[k].v =
        (int *)calloc(mris->vertices_topology[k].vnum, sizeof(int));
    if (!mris->vertices_topology[k].v)
  ...
  for (out_vno = vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (v->ripflag) {
      ...
      if (vno != out_vno) // at least one compressed out already
        memcpy(mris->vertices + out_vno, mris->vertices + vno,
               sizeof(VERTEX)); // GROSS HACK
      out_vno++;
    }
  }
...
void MRISremoveRipped(MRIS *mris) {
  ...
  //
  {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      if (!v->ripflag) {
        if (largestNsizeMax < vt->nsizeMax)
          largestNsizeMax = vt->nsizeMax;
        continue;
      }
  ...
  for (nsize = 0; nsize <= largestNsizeMax; nsize++) {
    ...
    int vno, vnoPlus2;
    for (vnoPlus2 = headAffectedVnosPlus2; vnoPlus2 != endOfList;
         vnoPlus2 = affectedVnosPlus2[vno]) {
      vno                             = vnoPlus2 - 2;
      VERTEX_TOPOLOGY const *const vt = &mris->vertices_topology[vno];
      if (affectedVnosPlus2[vno])
        continue; // already noted, includes ripped
      int n;
      for (n = 0; n < vt->vnum; n++) {
        int const vnoNbr = vt->v[n];
        if (affectedVnosPlus2[vnoNbr])
          continue; // already noted, includes ripped
    ...
    while (vnoPlus2 != startOfRippedTail) {
      ...
      if (v->dist)
        for (i = 0; i < vt->vtotal; i++)
          distCache[vt->v[i]] = v->dist[i];
      if (v->dist_orig)
        for (i = 0; i < vt->vtotal; i++)
          distOrigCache[vt->v[i]] = v->dist_orig[i];
      ...
      if (old_vtotal > old_cached) {
        ...
        cheapAssert(old_vno_beyond_nsizeMax == -1 ||
                    old_vno_beyond_nsizeMax == vt->v[old_cached]);
      ...
      if (v->dist)
        for (i = 0; i < vt->vtotal; i++)
          v->dist[i] = distCache[vt->v[i]];
      if (v->dist_orig)
        for (i = 0; i < vt->vtotal; i++)
          v->dist_orig[i] = distOrigCache[vt->v[i]];
  ...
  int count = 0;
  int vnoPlus2;
  for (vnoPlus2 = headAffectedVnosPlus2; vnoPlus2 >= 2;
       vnoPlus2 = affectedVnosPlus2[vnoPlus2 - 2]) {
    ...
    for (i = 0; i < vt->num; i++) {
      ...
      FACE const *const f   = &mris->faces[fno];
      if (f->ripflag)
        continue; // entry to be deleted
      if (num != i) {
        vt->f[num] = fno;
        vt->n[num] =
            vt->n[i]; // the face will not have had any of its vertexs removed
...
static short FACES_aroundVertex_reorder(MRIS *apmris, int avertex,
...
int MRIS_facesAtVertices_reorder(MRIS *apmris) {
  ...
  for (vno = 0; vno < apmris->nvertices; vno++) {
    MRIS_vertexProgress_print(apmris, vno,
    ...
    if (ret < 0) {
      ...
      for (n = 0; n < nfaces; n++) {
        ...
        int const orderedFno =
            VECTOR_ELT(pv_logicalOrderFace, orderedIndex + 1);
...
static short FACES_aroundVertex_reorder(MRIS *apmris, int avertex,
  ...
  for (i = 0; i < nfaces; i++) {
    ...
    for (j = 0; j < nfaces; j++) {
      ...
      commonVertices =
          VERTICES_commonInFaces_find(pFACE_I, pFACE_J, pv_commonVertices);
      if (commonVertices == 2) {
