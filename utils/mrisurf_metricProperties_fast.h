...
//
// By having mrisurf_metricProperties_{fast,slow}.h it is easier to keep the two
// versions in sync
...
// so the first calculation is a waste of time, except that it also writes
// origarea which the later one seems not to...
//
// There is a lot of overlap between these two functions that needs to be
// rationalized

void MRISMP_ctr(MRIS_MP *mp) { bzero(mp, sizeof(*mp)); }
...
static void MRISMP_makeDist2(MRIS_MP *mp, int vno, int neededCapacity) {
  ...
  if (neededCapacity < vSize)
    neededCapacity = vSize;
  ...
  if (mp->v_dist_buffer[vno]) {
    mp->v_dist[vno] = mp->v_dist_buffer[vno] = (float *)realloc(
        mp->v_dist_buffer[vno], neededCapacity * sizeof(float));
  } else {
    mp->v_dist[vno] = mp->v_dist_buffer[vno] =
        mrisStealDistStore(mp->underlyingMRIS, vno, neededCapacity);
  }
...
void MRISMP_copy(MRIS_MP *dst, MRIS_MP *src, bool only_inputs,
...
#define ELTX(C, T, N) ELT(C, T, N)
#define ELT(C, T, N)                                                           \
  *(T *)&(dst->N) = src->N; // support initializing the const members
  MRIS_MP__LIST_MRIS_IN SEP MRIS_MP__LIST_MRIS_IN_OUT if (!only_inputs)
...
#undef SEP
      ...
      if (dst->status != MRIS_PLANE) {
    freeAndNULL(dst->v_neg);
    v_neg = NULL;
  }
...
#define ELT(C, T, N) v_##N[vno] = src->v_##N[vno];
    if (!no_need_to_copy_xyz) {
      MRIS_MP__LIST_V_IN_OUT_XYZ
    }
    MRIS_MP__LIST_V_IN_OUT_NOXYZ
    if (!only_inputs) {
      if (v_neg)
        v_neg[vno] = src->v_neg[vno];
      if (src->v_dist[vno]) {
        MRISMP_makeDist2(dst, vno, src->v_dist_capacity[vno]);
        memcpy(v_dist[vno], src->v_dist[vno],
               src->v_dist_capacity[vno] * sizeof(*v_dist[vno]));
      }
      MRIS_MP__LIST_V_OUT
    }
#undef ELT
...
void MRISMP_load(MRIS_MP *mp, MRIS *mris, bool loadOutputs, float *dx_or_NULL,
                 float *dy_or_NULL, float *dz_or_NULL) {
...
#define ELTX(C, T, N)
  {
    ...
    //
    FACE_TOPOLOGY *ft =
        (FACE_TOPOLOGY *)malloc(mris->nfaces * sizeof(FACE_TOPOLOGY));
    cheapAssert(sizeof(ft->v) == sizeof(mris->faces[0].v));
    int i;
    for (i = 0; i < mris->nfaces; i++)
      memcpy(ft[i].v, mris->faces[i].v, sizeof(ft->v));
    *(FACE_TOPOLOGY **)(&mp->faces_topology) = ft;
  }

#define ELT(C, T, N)                                                           \
  *(T *)&(mp->N) = mris->N; // support initializing the const members
  MRIS_MP__LIST_MRIS_IN SEP MRIS_MP__LIST_MRIS_IN_OUT if (loadOutputs) {
...
#define comparison(NO, LHS, RHS)                                               \
  { comparisonWkr((NO), (LHS), (RHS), #LHS, __LINE__, &errorCount); }
static void comparisonWkr(int vnoOrFno, double lhs, double rhs,
                          const char *expr, int line, int *errorCount) {
  if (lhs == rhs) {
  ...
  fprintf(stdout,
          "no:%d  %s  %f != %f  during "
          "count_MRIScomputeMetricProperties_calls:%d\n",
          vnoOrFno, expr, lhs, rhs, count_MRIScomputeMetricProperties_calls);

  static int count;
  count++;
  if (count == 1)
    fprintf(stdout, "b %s:%d  needed\n", __FILE__, __LINE__);
}
...
#define ELT(C, T, N)                                                           \
  if (check)                                                                   \
    comparison(-1, mris->N, mp->N) else mris->N = mp->N;
...
#define ELTX(C, T, N) // these are the special cases dealt with here
    if (mp->v_neg) {
      if (check)
        comparison(vno, v->neg, mp->v_neg[vno]) else v->neg = mp->v_neg[vno];
    }
    if (mp->v_dist[vno]) {
      if (!check) { // this requires that the dist capacity match, not just the
                    // vtotal
        mrisSetDist(mris, vno, mp->v_dist[vno],
        ...
        for (n = 0; n < vtotal; n++) {
          if (check)
            comparison(vno, mris_dist[n], mp_dist[n]) else mris_dist[n] =
                mp_dist[n];
        }
      }
    }
...
#define ELT(C, T, N)                                                           \
  if (check)                                                                   \
    comparison(vno, v->N, mp->v_##N[vno]) else v->N = mp->v_##N[vno];
    MRIS_MP__LIST_V_IN_OUT SEP MRIS_MP__LIST_V_OUT
#undef ELT
#undef ELTX
#undef SEP
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    ...
    if (mp->f_ripflag[fno] && mris->faces[fno].ripflag)
      continue;
    FaceNormCacheEntry const *const fNorm = getFaceNorm(mris, fno);
#define SEP
#define ELTX(C, T, N) // these are the special cases dealt with here
    if (check)
      comparison(fno, fNorm->orig_area,
                 mp->f_norm_orig_area
                     [fno]) else setFaceOrigArea(mris, fno,
                                                 mp->f_norm_orig_area[fno]);
    if (check)
      comparison(
          fno, cmpAnglesPerTriangle(f->orig_angle, mp->f_orig_angle[fno]),
          0) else copyAnglesPerTriangle(f->orig_angle, mp->f_orig_angle[fno]);
    if (mp->f_normSet[fno]) {
...
#define ELT(C, T, N)                                                           \
  if (check)                                                                   \
    comparison(fno, f->N, mp->f_##N[fno]);
    MRIS_MP__LIST_F_IN
#undef ELT
#define ELT(C, T, N)                                                           \
  if (check)                                                                   \
    comparison(fno, f->N, mp->f_##N[fno]) else f->N = mp->f_##N[fno];
    MRIS_MP__LIST_F_OUT
#undef ELT
#undef ELTX
#undef SEP
...
/*!
  ...
  The definition here results in the length being the sin of the angle at the
  vertex and is used to bias the sum of the face normal vectors used to compute
...
#pragma omp critical
  ...
  // Process the pending vertices, keeping those that need more work on the
  // nextPending list Try only a few times, because the problem might be
  // insoluable
  //
  int trial = 0;
...
#pragma omp parallel for if_ROMP(shown_reproducible)
      ...
      for (n = 0; n < vt->num; n++) {
        ...
        // The vertex normal is biased towards being perpendicular to 90degree
        // contributors...
...
#pragma omp critical // Retry after various adjustments below
    ...
    for (p = 0; p < nextPendingSize; p++) {
      ...
      // When written, assumed being done in parallel, hence fnv_hash giving
      ...
      if (mris->status == MRIS_PLANE || mris->status == MRIS_CUT) {
        ...
        // Note: this will recompute some, but I suspect the number is too small
        // to be relevant
        //
        for (n = 0; n < vt->num; n++) {
          int const fno = vt->f[n];
    ...
    // Try the moved ones again - although not their moved neighbors, weirdly
    // enough
    //
...
static void MRISMP_computeAvgInterVertexDist(MRIS_MP *mris, double *StdDev) {
  ...
  if (showHashs) {
    fprintf(stdout, "%s:%d MRISMP_computeAvgInterVertexDist starting ",
            __FILE__, __LINE__);
    // mris_print_hash(stdout, mris, "mris ", "");
    fprintf(stdout, "\n");
  }
...
#include "romp_for_end.h"
  ...
  // NOTE - This is a poor algorithm for computing the std dev because of how
  // the floating point errors accumulate but it seems to work for us because
  // the double has enough accuracy to sum the few hundred thousand small but
  // not too small floats that we have
  double Avg = Sum / N;
  if (StdDev != NULL) {
...
#define neg_orig_area ROMP_PARTIALSUM(2)
  ...
  */
  ...
  float const xc =
      (x0 + x1 + x2) /* / 3 */; // These divides by three are a waste of time
  float const yc =
      (y0 + y1 +
       y2) /* / 3 */; // since we only use the magnitude of the dot product
...
void MRIScomputeMetricProperties(MRIS_MP *mris) {
  ...
  if (mris->status == MRIS_PARAMETERIZED_SPHERE ||
      mris->status == MRIS_RIGID_BODY || mris->status == MRIS_SPHERE) {
