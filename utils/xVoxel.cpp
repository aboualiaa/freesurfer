...
void xVoxl_Delete(xVoxelRef *ioppVoxel) {
  xVoxelRef ref = nullptr;

  if (nullptr == ioppVoxel) {
    return;
  }
...
void xVoxl_Copy(xVoxelRef ipVoxDest, xVoxelRef ipVoxSrc) {
  xVoxl_SetFloat(ipVoxDest, xVoxl_GetFloatX(ipVoxSrc),
                 xVoxl_GetFloatY(ipVoxSrc), xVoxl_GetFloatZ(ipVoxSrc));
}
...
tBoolean xVoxl_IncrementWithMinUntilLimit(xVoxelRef ref, float inMin,
                                          float inLimit) {
  if (ref->mfX < inLimit) {
    ref->mfX += 1.0;
    return TRUE;
...
tBoolean xVoxl_IncrementUntilLimits(xVoxelRef ref, float inXLimit,
...
tBoolean xVoxl_IncrementWithMinsUntilLimits(xVoxelRef ref, float inXMin,
...
int xVoxl_ExpandToIndex(xVoxelRef ref, int inDimensionX, int inDimensionY) {
  return (ref->mfZ * inDimensionX * inDimensionY) + (ref->mfY * inDimensionX) +
         ref->mfX;
}

void xVoxl_PrintDebug(xVoxelRef ref) {
  DebugPrint(("Voxel: %d, %d, %d\n", xVoxl_GetX(ref), xVoxl_GetY(ref),
              xVoxl_GetZ(ref)));
}
