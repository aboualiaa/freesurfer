...
MRI::Shape::Shape(const std::vector<int> &shape) {
  ...
  int dims = shape.size();
  if ((dims != 3) && (dims != 4))
    fs::fatal() << "volume must be 3D or 4D (provided shape has " << dims
                << " dimensions)";

  // validate size
  for (auto const &len : shape) {
    if (len <= 0)
      fs::fatal() << "volume size must be greater than 0 in every dimension";
  }
...
MRI::MRI(Shape volshape, int dtype, bool alloc) : shape(volshape), type(dtype) {
  ...
  if (bytes_per_vox < 1)
    fs::fatal() << "unsupported MRI data type: " << type;
  vox_per_row   = width;
