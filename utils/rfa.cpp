...
RFA *RFAalloc(RFA_PARMS *parms, int alloc_trees) {
  ...
  rfa->nfeatures =
      (parms->wsize * parms->wsize * parms->wsize) * parms->nvols + 3;
  rfa->nodes = (RF_NODE ***)calloc(rfa->vg.width, sizeof(RF_NODE **));
  if (rfa->nodes == nullptr)
  ...
  for (x = 0; x < rfa->vg.width; x++) {
    ...
    for (y = 0; y < rfa->vg.height; y++) {
      ...
      if (rfa->nodes[x][y] == nullptr)
        ErrorExit(ERROR_NOMEMORY, "RFAalloc: could not allocate node Z");
      for (z = 0; z < rfa->vg.depth; z++) {
        node = &rfa->nodes[x][y][z];
        node->label_priors =
            (float *)calloc(MAX_LABELS_PER_NODE, sizeof(node->label_priors[0]));
        if (node->label_priors == nullptr)
          ErrorExit(ERROR_NOMEMORY,
                    "RFAalloc: could not allocate node label_priors");
        node->labels = (unsigned short *)calloc(MAX_LABELS_PER_NODE,
                                                sizeof(node->labels[0]));
        ...
        if (alloc_trees)
          node->rf = RFalloc(parms->ntrees, rfa->nfeatures, MAX_LABELS_PER_NODE,
                             parms->max_depth, nullptr, 10);
      }
    }
  }
...
int RFAaddInput(RFA *rfa, MRI *mri_seg, MRI *mri_inputs, TRANSFORM *transform,
                RFA_PARMS *parms) {
  int index;
...
int RFAvoxelToNode(const RFA *rfa, double xt, double yt, double zt, double *px,
                   double *py, double *pz) {
  *px = xt / rfa->spacing;
  *py = yt / rfa->spacing;
  *pz = zt / rfa->spacing;
...
int RFAnodeToVoxel(const RFA *rfa, double xt, double yt, double zt, double *px,
                   double *py, double *pz) {
  *px = xt * rfa->spacing;
  *py = yt * rfa->spacing;
  *pz = zt * rfa->spacing;
...
int RFAsourceVoxelToNode(const RFA *rfa, MRI *mri, TRANSFORM *transform, int xv,
  ...
  *pz = (int)(zd);
  ...
  // if (*px < 0 || *py < 0 || *pz < 0 || *px >= rfa->width || *py >=
  // rfa->height || *pz >= rfa->depth)
...
int RFAupdateTraining(RFA *rfa, RFA_PARMS *parms) {
  int ntraining_sets, ntraining, start, x, y, z, xn, yn, zn, i,
  ...
  if (training_data == nullptr || training_classes == nullptr)
    ErrorExit(
        ERROR_NOFILE,
        "RFAupdateTraining: could not allocate %d-length training buffers",
        ntraining);
       ...
       i++) // allow for augmenting with other wmsa examples
  {
    ...
    if (training_data[i] == nullptr)
      ErrorExit(
          ERROR_NOMEMORY,
          "RFAupdateTraining: could not allocate %d-len feature vector #%d",
          rfa->nfeatures, i);
  }
      ...
      for (y = 0; y < mri_in->height; y++)
        for (z = 0; z < mri_in->depth; z++) {
          ...
          if (IS_HYPO(label)) {
            ...
            if (wmsa_data[nwmsa] == nullptr)
              ErrorExit(ERROR_NOMEMORY,
                        "RFAtrain: could not allocate %dth wmsa feature vector",
                        nwmsa);
            extract_feature(mri_in, rfa->wsize, x, y, z, wmsa_data[nwmsa++], -1,
                            -1, -1);
          }
        }
  }
      ...
      for (zn = 0; zn < rfa->depth; zn++) {
          ...
          for (xk = 0; xk < rfa->spacing; xk++) {
            ...
            for (yk = 0; yk < rfa->spacing; yk++) {
              ...
              for (zk = 0; zk < rfa->spacing; zk++) {
                zi = mri_in->zi[z + zk];
                RFAsourceVoxelToAtlas(rfa, mri_in, transform, xi, yi, zi,
                                      &xatlas, &yatlas, &zatlas);
                extract_feature(mri_in, rfa->wsize, xi, yi, zi,
                                training_data[tno], xatlas, yatlas, zatlas);
                ...
                if (index >= node->nlabels) {
                  if (index >= node->max_labels) {
                    ...
                    memmove(labels, node->labels,
                            node->nlabels * sizeof(node->labels[0]));
                    free(node->label_priors);
                    free(node->labels);
                    node->max_labels += 5;
                    node->labels = (unsigned short *)calloc(
                        node->max_labels, sizeof(node->labels[0]));
                    if (node->labels == nullptr)
                      ErrorExit(
                          ERROR_NOMEMORY,
                          "could not reallocate %d node labels @(%d %d %d)",
                          node->max_labels, xn, yn, zn);
                    node->label_priors = (float *)calloc(
                        node->max_labels, sizeof(node->label_priors[0]));
                    if (node->label_priors == nullptr)
                      ErrorExit(
                          ERROR_NOMEMORY,
                          "could not reallocate %d node priors @(%d %d %d)",
                          node->max_labels, xn, yn, zn);
                    memmove(node->label_priors, label_priors,
                            node->nlabels * sizeof(node->label_priors[0]));
                    memmove(node->labels, labels,
                            node->nlabels * sizeof(node->labels[0]));
                  }
        ...
        if (node->nlabels > 1) {
          ...
          if (wmsa_count > 0)
            DiagBreak();
          if (wmsa_count > 0 &&
              max_count > 5 * wmsa_count) // augment training set
          {
            extra = (max_count / 2 - wmsa_count);
            extra = 0; // disabled!!
            start_wmsa_index =
                (int)randomNumber(0.0, (double)((nwmsa - extra) - .00001));
            for (wmsa_index = start_wmsa_index;
                 wmsa_index < start_wmsa_index + extra; wmsa_index++) {
  ...
  printf("out of bag accuracy: %d of %d = %2.2f%%\n", total_correct,
         total_training, 100.0 * total_correct / total_training);
...
int RFAwrite(RFA *rfa, char *fname) {
  ...
  if (!fp)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "RFAwrite(%s): could not open file", fname));
  fprintf(fp, "%d %d %d %d %d %d %d %2.1f\n", rfa->total_training, rfa->wsize,
          rfa->nfeatures, rfa->wsize, rfa->width, rfa->height, rfa->depth,
          rfa->spacing);
...
RFA *RFAread(char *fname) {
  ...
  if (!fp)
    ErrorReturn(NULL,
                (ERROR_BADPARM, "RFAwrite(%s): could not open file", fname));
  if (fscanf(fp, "%d %d %d %d %d %d %d %f\n", &total_training, &parms.wsize,
             &nfeatures, &parms.wsize, &parms.width, &parms.height,
             &parms.depth, &parms.spacing) != 8) {
  ...
  for (xn = 0; xn < rfa->width; xn++)
    for (yn = 0; yn < rfa->height; yn++)
      for (zn = 0; zn < rfa->depth; zn++) {
        ...
        free(node->label_priors);
        node->labels =
            (unsigned short *)calloc(node->max_labels, sizeof(node->labels[0]));
        if (node->labels == nullptr)
          ErrorExit(ERROR_NOMEMORY,
                    "could not reallocate %d node labels @(%d %d %d)",
                    node->max_labels, xn, yn, zn);
        node->label_priors =
            (float *)calloc(node->max_labels, sizeof(node->label_priors[0]));
        if (node->label_priors == nullptr)
          ErrorExit(ERROR_NOMEMORY,
                    "could not reallocate %d node priors @(%d %d %d)",
                    node->max_labels, xn, yn, zn);
  ...
  for (x = 0; x < mri_in->width; x++) {
    for (y = 0; y < mri_in->height; y++)
      for (z = 0; z < mri_in->depth; z++) {
        ...
        node = &rfa->nodes[xn][yn][zn];
        RFAsourceVoxelToAtlas(rfa, mri_in, transform, x, y, z, &xatlas, &yatlas,
                              &zatlas);
        extract_feature(mri_in, rfa->wsize, x, y, z, feature, xatlas, yatlas,
                        zatlas);
        if (x == Gx && y == Gy && z == Gz) {
          int j;
          printf("voxel (%d, %d, %d) -> node (%d, %d, %d), labels = ", x, y, zn,
                 xn, yn, zn);
...
int extract_feature(MRI *mri_in, int wsize, int x, int y, int z,
                    double *feature, int xatlas, int yatlas, int zatlas) {
...
int extract_long_features(MRI *mri_in, MRI *mri_seg, TRANSFORM *transform,
                          GCA *gca, int wsize, int x, int y, int z,
                          double *feature) {
  int xi, yi, zi, xk, yk, zk, whalf, n, f;
  ...
  feature[f++] =
      100 * MRIcountValInNbhd(mri_seg, 3, x, y, z, WM_hypointensities);
  return (NO_ERROR);
}
