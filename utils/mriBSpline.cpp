...
static void getZLineStop(const MRI *mri, int x, int y, int f, double Line[],
                         int zstop) {
  int z;
  for (z = 0; z < zstop; z++)
    Line[z] = MRIgetVoxVal(mri, x, y, z, f);
...
static void getYLineStop(const MRI *mri, int x, int z, int f, double Line[],
                         int ystop) {
  int y;
  for (y = 0; y < ystop; y++)
    Line[y] = MRIgetVoxVal(mri, x, y, z, f);
...
// static void	getXLineStop (const MRI	* mri, int	y, int z, int f, double
// Line[], int xstop)
// {
// 	int	x;
...
static void BsplineWeights(int SplineDegree, const double *val,
                           const int *index, double *weights) {
  ...
  case 6:
    ...
    weights[1] =
        (361.0 / 192.0 -
         w * (59.0 / 8.0 +
              w * (-185.0 / 16.0 + w * (25.0 / 3.0 + w * (-5.0 / 2.0 + w) *
                                                         (1.0 / 2.0 + w))))) /
        120.0;
    weights[2] =
        (10543.0 / 960.0 +
         w * (-289.0 / 16.0 +
              w * (79.0 / 16.0 +
                   w * (43.0 / 6.0 + w * (-17.0 / 4.0 + w * (-1.0 + w)))))) /
        48.0;
    w2 = w * w;
    weights[3] =
        (5887.0 / 320.0 - w2 * (231.0 / 16.0 - w2 * (21.0 / 4.0 - w2))) / 36.0;
    weights[4] =
        (10543.0 / 960.0 +
         w * (289.0 / 16.0 +
              w * (79.0 / 16.0 +
  ...
  case 7:
    ...
    weights[1] =
        (120.0 / 7.0 +
         w * (-56.0 +
              w * (72.0 + w * (-40.0 + w2 * (12.0 + w * (-6.0 + w)))))) /
        720.0;
    weights[2] =
        (397.0 / 7.0 -
         w * (245.0 / 3.0 +
              w * (-15.0 + w * (-95.0 / 3.0 +
                                w * (15.0 + w * (5.0 + w * (-5.0 + w))))))) /
        240.0;
    weights[3] =
        (2416.0 / 35.0 + w2 * (-48.0 + w2 * (16.0 + w2 * (-4.0 + w)))) / 144.0;
    weights[4] =
        (1191.0 / 35.0 -
         w * (-49.0 + w * (-9.0 + w * (19.0 + w * (-3.0 + w) * (-3.0 + w2))))) /
        144.0;
    weights[5] =
        (40.0 / 7.0 +
         w * (56.0 / 3.0 +
              w * (24.0 + w * (40.0 / 3.0 + w2 * (-4.0 + w * (-2.0 + w)))))) /
        240.0;
    weights[7] = w2;
  ...
  case 8:
    ...
    w2 = w * w;
    weights[1] =
        (39.0 / 16.0 - w * (6.0 + w * (-9.0 / 2.0 + w2))) *
        (21.0 / 16.0 + w * (-15.0 / 4.0 + w * (9.0 / 2.0 + w * (-3.0 + w)))) /
        5040.0;
    ...
    weights[4] =
        (2337507.0 / 8960.0 +
         w2 * (-2601.0 / 16.0 + w2 * (387.0 / 8.0 + w2 * (-9.0 + w2)))) /
        576.0;
  ...
  case 9:
    ...
    weights[4] =
        (78095.0 / 63.0 -
         w2 * (700.0 + w2 * (-190.0 + w2 * (100.0 / 3.0 + w2 * (-5.0 + w))))) /
        2880.0;
    weights[5] =
        (44117.0 / 63.0 +
         w * (809.0 +
...
/* ----------------------------------------------------------------------------
        ...
        Purpose:  	Initializes down- and up-sampling filter arrays for
                                least squares splines of order 0 to 3.  (little
l_2 norm) g : reduce filter h : expand filter

        Author:		Michael Unser, NIH, BEIP, May 1992

---------------------------------------------------------------------------- */
static void PyramidFilterSplinel2(double g[], long *ng, double *h, long *nh,
                                  long Order) {
  switch (Order) {
  case 0L:
    *ng = 1L;
...
/* ----------------------------------------------------------------------------
        ...
        Purpose:	Initializes down- and up-sampling filter arrays for
                                least squares CENTERED splines of order 0 to 4.
(little l_2 norm) g : reduce filter h : expand filter
        ...
        Author:		Patrick Brigger, NIH, BEIP	May 1996
                                Daniel Sage, EPFL, Biomedical Imaging Group,
November 1999

---------------------------------------------------------------------------- */
static void PyramidFilterCentered(double g[], long *ng, double h[], long *nh,
                                  long Order) {
  switch (Order) {
  case 0:
    g[0] = 1;
...
/* ----------------------------------------------------------------------------
        ...
        Purpose:	Initializes the symmetric down- and up-sampling filter
arrays for L2 spline pyramid of order 0 to 5 when the downsampled grid is
centered. These filters have then to be followed by a Haar filter. g: reduce
filter h: expand filter
...
---------------------------------------------------------------------------- */
static void PyramidFilterCenteredL2(double g[], long *ng, double h[], long *nh,
                                    long Order) {
  switch (Order) {
  case 0:
    g[0] = 1.;
...
/* ----------------------------------------------------------------------------
        ...
        Purpose:	Initializes the symmetric down- and up-sampling filter
arrays for L2 DERIVATIVE spline pyramid of order 0 to 5 when the downsampled
                                grid is centered.
                                These filters have then to be followed by a
Derivative Haar filter. g : reduce filter h : expand filter
        Note:		filter arrays should be defined as
                                        float g[35],h[35]	filter arrays
                                        short *ng,*nh	number of taps
                                        short Order	order of the spline
...
---------------------------------------------------------------------------- */
// static  void PyramidFilterCenteredL2Derivate(double g[],long *ng,double
// h[],long *nh,long Order)
// {
// 	switch (Order) {
// 	case 0 :
// 		g[0] = 1.;
...
/* ----------------------------------------------------------------------------
        ...
        Parameters:
                ...
                Order is the order for the filters based on splines
                        ...
                        For the "Centered Spline" filter, Order is 0, 1, 2, 3 or
4 For the "Centered Spline L2" filter, Order is 0, 1, 2, 3 or 4

                IsCentered is a return value indicates if the filter is a
centered filter TRUE if it is a centered filter FALSE if it is not a centered
filter
...
/* ----------------------------------------------------------------------------
        ...
        Parameters:
                In[NxIn] is the input signal	(NxIn should be greater than 2
and even) Out[NxIn/2] is the output signal g[ng] is an array that contains the
...
---------------------------------------------------------------------------- */
static void ReduceStandard_1D(double In[], long NxIn, double Out[], double g[],
                              long ng) {
  long k, i, i1, i2;
  long kk, kn, nred, n;
...
---------------------------------------------------------------------------- */
static void ExpandStandard_1D(double In[], long NxIn, double Out[], double h[],
                              long nh) {
  long k, j, i, i1, i2;
  long kn, nexp, n;
...
---------------------------------------------------------------------------- */
static void ExpandCentered_1D(double In[], long NxIn, double Out[], double h[],
                              long nh) {
  long k, i, j, i1, k0, i2;
  long kk, kn, nexp, n;
...
---------------------------------------------------------------------------- */
static void Reduce_1D(double In[], long NxIn, double Out[], double g[], long ng,
                      short IsCentered) {
  if (IsCentered)
...
---------------------------------------------------------------------------- */
static void Expand_1D(double In[], long NxIn, double Out[], double h[], long nh,
                      short IsCentered) {
  if (IsCentered)
...
extern MRI_BSPLINE *MRIallocBSpline(int width, int height, int depth,
                                    int nframes) {
...
extern int MRIfreeBSpline(MRI_BSPLINE **pbspline) {
  ...
  if (!bspline)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "MRIfreeBSpline: null pointer\n"));
...
extern MRI_BSPLINE *MRItoBSpline(const MRI *mri_src, MRI_BSPLINE *bspline,
                                 int degree) {
  ...
  if (bspline->coeff->width != Width || bspline->coeff->depth != Depth ||
      bspline->coeff->height != Height || bspline->coeff->nframes != Frames) {
    printf("ERROR MRItoBSpline: BSPline Coeff MRI dimensions must be same as "
           "Source MRI!\n");
    exit(1);
  }
...
/* convert the image samples into interpolation coefficients */
  ...
  ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) firstprivate(tid, y, f, Lines)  \
    shared(Depth, Height, Width, Frames, Pole, NbPoles, bspline)               \
        schedule(static, 1)
#endif
      for (z = 0; z < Depth; z++) {
    ROMP_PFLB_begin
      ...
      for (f = 0; f < Frames; f++) {
        ...
        if (!bspline->srcneg) {
          for (x = 0; x < Width; x++) {
            if (Lines[tid][x] < 0.0) {
              bspline->srcneg =
                  TRUE; // write to shared, racing OK (only set to true)
              break;
            }
          }
        }
        ...
        if (Width > 1) {
          ConvertToInterpolationCoefficients(Lines[tid], Width, Pole, NbPoles,
                                             DBL_EPSILON);
        }
  ...
  if (Height > 1) {
    ...
    ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) firstprivate(tid, x, f, Lines)  \
    shared(Depth, Height, Width, Frames, Pole, NbPoles, bspline)               \
        schedule(static, 1)
#endif
        for (z = 0; z < Depth; z++) {
      ...
      for (x = 0; x < Width; x++) {
        for (f = 0; f < Frames; f++) {
          ...
          ConvertToInterpolationCoefficients(Lines[tid], Height, Pole, NbPoles,
                                             DBL_EPSILON);
    ...
    shared(Depth, Height, Width, Frames, Pole, NbPoles, bspline)               \
      ...
      for (x = 0; x < Width; x++) {
        for (f = 0; f < Frames; f++) {
          ...
          ConvertToInterpolationCoefficients(Lines[tid], Depth, Pole, NbPoles,
                                             DBL_EPSILON);
...

{
  ...
  for (k = 0; k <= sdk; k++) {
    ...
    for (j = 0; j <= sdj; j++) {
      w = 0.0;
      for (i = 0; i <= sdi; i++) {
        w += xWeight[i] * MRIgetVoxVal(bspline->coeff, xIndex[i], yIndex[j],
                                       zIndex[k], frame);
      }
      w2 += yWeight[j] * w;
    }
...
extern int MRIsampleSeqBSpline(const MRI_BSPLINE *bspline, double x, double y,
                               double z, float *valvect, int firstframe,
                               int lastframe) {
  int f;
  ...
  for (f = firstframe; f <= lastframe; f++) {
    ...
    for (k = 0; k <= sdk; k++) {
      ...
      for (j = 0; j <= sdj; j++) {
        ...
        for (i = 0; i <= sdi; i++) {
          w += xWeight[i] *
               MRIgetVoxVal(bspline->coeff, xIndex[i], yIndex[j], zIndex[k], f);
        }
        w2 += yWeight[j] * w;
      }
      interpolated += zWeight[k] * w2;
...
MRI *MRIlinearTransformBSpline(const MRI_BSPLINE *bspline, MRI *mri_dst,
                               MATRIX *mA)
// mainly copied from mri.c
{
  ...
  if (!mAinv)
    ErrorReturn(NULL,
                (ERROR_BADPARM, "MRIlinearTransformSpline: xform is singular"));
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(tid, y2, y1, x1, x2, x3, frame, val, v_X, v_Y)                \
        shared(depth, height, width, mAinv, bspline, mri_dst)                  \
            schedule(static, 1)
#endif
...
MRI *MRIapplyRASlinearTransformBSpline(const MRI_BSPLINE *bspline, MRI *mri_dst,
                                       MATRIX *m_ras_xform) {
  MATRIX *m_voxel_xform;

  m_voxel_xform =
      MRIrasXformToVoxelXform(bspline->coeff, mri_dst, m_ras_xform, nullptr);
  ...
  if (lta->num_xforms == 1) {
    ...
    if (!mri_dst) {
      if (tran->dst.valid == 1) // transform dst is valid
      {
        ...
        mri_dst =
            MRIallocSequence(tran->dst.width, tran->dst.height, tran->dst.depth,
                             bspline->srctype, bspline->coeff->nframes);
    ...
    } else if (lta->type == LINEAR_VOX_TO_VOX) // vox-to-vox
    {
      ...
      ErrorExit(ERROR_BADPARM,
                "LTAtransformBSpline: unknown linear transform\n");
  }

  ErrorExit(ERROR_BADPARM,
            "LTAtransformBSpline: numxforms != 1 not supported\n");
...
extern MRI *MRIdownsample2BSpline(const MRI *mri_src, MRI *mri_dst) {
  ...
  if (!GetPyramidFilter(SPLINE_CENT, 3, g, &ng, h, &nh, &IsCentered)) {
    printf("MRIdownsample2BSpline ERROR: Unable to load the filter "
           "coefficients\n");
    exit(1);
  }
  ...
  if (!mri_tmp)
    ErrorExit(ERROR_NO_MEMORY,
              "MRIdownsample2BSpline: could not allocate tmp mri\n");
  InBuffer = (double *)malloc((size_t)(NxIn * (long)sizeof(double)));
  if (InBuffer == (double *)nullptr) {
    MRIfree(&mri_tmp);
    ErrorExit(ERROR_NO_MEMORY,
              "MRIdownsample2BSpline: could not allocate inbuffer mem\n");
  }
  OutBuffer = (double *)malloc((size_t)(NxOut * (long)sizeof(double)));
  if (OutBuffer == (double *)nullptr) {
    MRIfree(&mri_tmp);
    free(InBuffer);
    ErrorExit(ERROR_NO_MEMORY,
              "MRIdownsample2BSpline: could not allocate outbuffer mem\n");
  }
  for (kf = 0; kf < NfIn; kf++)
    for (kz = 0; kz < NzIn; kz++)
      for (ky = 0; ky < NyIn; ky++) {
        if (NxIn > 1) {
          ...
          // printf(" f %i  z %i  y %i  NxOut %i  width
  ...
  if (mri_dst->width != NxOut || mri_dst->height != NyOut ||
      mri_dst->depth != NzOut || mri_dst->nframes != NfIn) {
...
extern MRI *MRIupsample2BSpline(const MRI *mri_src, MRI *mri_dst) {
  ...
  if (!GetPyramidFilter(SPLINE_CENT, 3, g, &ng, h, &nh, &IsCentered)) {
    printf(
        "MRIupsample2BSpline ERROR: Unable to load the filter coeffiients\n");
    exit(1);
  }
...
//
// ----------------------------------------------------------------------------
// */ extern int Reduce_2D( 				float *In, long NxIn, long NyIn,
// float *Out, 				double
// g[], long ng, 				short IsCentered
// 				)
// {
// float	*Tmp;
// double 	*InBuffer;		/* Input buffer to 1D process */
...
// 	if (NxIn > 1L) {
// 		InBuffer = (double
// *)malloc((size_t)(NxIn*(long)sizeof(double))); 		if (InBuffer ==
// (double
// *)NULL) { 			free(Tmp); 			printf("Unable to allocate
// memory"); 			return(ERROR);
// 		}
// 		OutBuffer = (double
// *)malloc((size_t)(NxOut*(long)sizeof(double))); 		if (OutBuffer ==
// (double
// *)NULL) { 			free(Tmp); 			free(InBuffer);
// printf("Unable to allocate memory"); 			return(ERROR);
// 		}
...
// 	if (NyIn > 1L) {
// 		InBuffer = (double
// *)malloc((size_t)(NyIn*(long)sizeof(double))); 		if (InBuffer ==
// (double
// *)NULL) { 			printf("Unable to allocate memory");
// return(ERROR);
// 		}
// 		OutBuffer = (double
// *)malloc((size_t)(NyOut*(long)sizeof(double))); 		if (OutBuffer ==
// (double
// *)NULL) { 			free(InBuffer); 			printf("Unable to
...
// IsCentered); 			PutRow( Out, NxOut, NyOut, ky, OutBuffer,
// NxOut);
// 		}
// 		free(InBuffer);
// 		free(OutBuffer);
