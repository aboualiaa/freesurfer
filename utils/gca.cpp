...
static double gcaGibbsLogPosterior(GCA *gca, MRI *mri_labels, float *vals,
                                   int label, int x, int y, int z,
                                   GCA_PRIOR *gcap, GCA_NODE *gcan,
                                   TRANSFORM *transform);
#define INTERP_PRIOR 0
#if INTERP_PRIOR
static float gcaComputePrior(GCA *gca, MRI *mri, TRANSFORM *transform, int x,
                             int y, int z, int label);
#endif
...
#endif
static int gcaScale(GCA *gca, int *labels, int *contra_labels, float *scales,
                    int nlabels, int dir);
#if 1
static int gcaMaxPriorLabel(GCA *gca, MRI *mri, TRANSFORM *transform, int x,
                            int y, int z);
#endif
static GCA_NODE * gcaBuildNbhdGCAN(GCA *gca, int x, int y, int z, float sigma);
static GCA_PRIOR *gcaBuildNbhdGCAP(GCA *gca, int x, int y, int z, float sigma,
                                   GCA *gca_smooth);

static HISTOGRAM *gcaGetLabelHistogram(GCA *gca, int label, int frame,
                                       int border);
int               GCAmaxLabel(GCA *gca);
static int gcaRelabelSegment(GCA *gca, TRANSFORM *transform, MRI *mri_inputs,
                             MRI *mri_dst, MRI_SEGMENT *mseg);

static int     gcapGetMaxPriorLabel(GCA_PRIOR *gcap, double *p_prior);
double         compute_partial_volume_log_posterior(GCA *gca, GCA_NODE *gcan,
                                                    GCA_PRIOR *gcap, float *vals,
                                                    int l1, int l2);
static double  gcaComputeSampleConditionalLogDensity(GCA_SAMPLE *gcas,
                                                     float *vals, int ninputs,
                                                     int label);
static VECTOR *load_sample_mean_vector(GCA_SAMPLE *gcas, VECTOR *v_means,
                                       int ninputs);
static MATRIX *load_sample_covariance_matrix(GCA_SAMPLE *gcas, MATRIX *m_cov,
                                             int ninputs);
static double  sample_covariance_determinant(GCA_SAMPLE *gcas, int ninputs);
static GC1D *  gcanGetGC(GCA_NODE *gcan, int label);
static GC1D *  findGCInWindow(GCA *gca, int x, int y, int z, int label,
                              int wsize);

static int gcaCheck(GCA *gca);
double gcaVoxelLogPosterior(GCA *gca, MRI *mri_labels, MRI *mri_inputs, int x,
                            int y, int z, TRANSFORM *transform);
static double gcaGibbsImpossibleConfiguration(GCA *gca, MRI *mri_labels, int x,
                                              int y, int z,
                                              TRANSFORM *transform);
...
static GCA_SAMPLE *gcaExtractThresholdedRegionLabelAsSamples(
    ...
    int xn, int yn, int zn, int wsize, float pthresh);
static double gcaComputeSampleConditionalDensity(GCA_SAMPLE *gcas, float *vals,
                                                 int ninputs, int label);
static double gcaComputeLogDensity(GC1D *gc, float *vals, int ninputs,
                                   float prior, int label);
static double gcaComputeSampleLogDensity(GCA_SAMPLE *gcas, float *vals,
                                         int ninputs);
...
static int GCAupdatePrior(GCA *gca, MRI *mri, int xn, int yn, int zn,
                          int label);
...
static int       mriFillRegion(MRI *mri, int x, int y, int z, int frame,
                               float fill_val, int whalf);
static int       gcaFindMaxPriors(GCA *gca, float *max_priors);
static int       gcaFindIntensityBounds(GCA *gca, float *pmin, float *pmax);
static int       dump_gcan(GCA *gca, GCA_NODE *gcan, FILE *fp, int verbose,
                           GCA_PRIOR *gcap);
...
GCA_PRIOR *getGCAP(GCA *gca, MRI *mri, TRANSFORM *transform, int xv, int yv,
                   int zv);
GCA_PRIOR *getGCAPfloat(GCA *gca, MRI *mri, TRANSFORM *transform, float xv,
                        float yv, float zv);
static int gcaNodeToPrior(const GCA *gca, int xn, int yn, int zn, int *pxp,
                          int *pyp, int *pzp);
...
int GCApriorToNode(const GCA *gca, int xp, int yp, int zp, int *pxn, int *pyn,
                   int *pzn);
...
static void   load_vals_xyzInt(const MRI *mri_inputs, int x, int y, int z,
                               float *vals, int ninputs);
static double gcaComputeSampleLogDensity_1_input(GCA_SAMPLE *gcas, float val);
static double gcaComputeSampleConditionalLogDensity_1_input(GCA_SAMPLE *gcas,
                                                            float       val,
                                                            int         label);
static double GCAsampleMahDist_1_input(GCA_SAMPLE *gcas, float val);
#endif

double GCAimageLikelihoodAtNode(GCA *gca, GCA_NODE *gcan, float *vals,
                                int label) {
  GC1D *gc;
  float likelihood;
...
static int initialize_ventricle_alignment(MRI *mri_seg, MRI *mri, MATRIX *m_L,
                                          char *base_name, int border,
                                          int label);
...
void GCAsetup(GCA *gca) {
  ...
  gca->mri_node__ = MRIallocHeader(gca->node_width, gca->node_height,
                                   gca->node_depth, MRI_UCHAR, 1);
  ...
  gca->mri_prior__ = MRIallocHeader(gca->prior_width, gca->prior_height,
                                    gca->prior_depth, MRI_UCHAR, 1);
...
GCA_PRIOR *getGCAPfloat(GCA *gca, MRI *mri, TRANSFORM *transform, float xv,
  ...
  if (!GCAsourceFloatVoxelToPrior(gca, mri, transform, xv, yv, zv, &xp, &yp,
                                  &zp)) {
    gcap = &gca->priors[xp][yp][zp];
  }
...
GCA_NODE *getGCAN(GCA *gca, MRI *mri, TRANSFORM *transform, int xv, int yv,
                  int zv) {
  int       xn, yn, zn;
  GCA_NODE *gcan = nullptr;
...
int GCAisPossible(GCA *gca, MRI *mri, int label, TRANSFORM *transform, int x,
                  int y, int z, int use_mrf) {
...
static int dump_gcan(GCA *gca, GCA_NODE *gcan, FILE *fp, int verbose,
  ...
  GC1D *  gc;
  float   prior;
  VECTOR *v_means = nullptr;
  MATRIX *m_cov   = nullptr;
  ...
  for (n = 0; n < gcan->nlabels; n++) {
    ...
    fprintf(fp, "%d: label %s (%d), prior %2.3f, ntr %d, means ", n,
            cma_label_to_name(gcan->labels[n]), gcan->labels[n], prior,
            gc->ntraining);
    ...
    if (verbose) {
      ...
      for (i = 0; i < GIBBS_NEIGHBORS; i++) {
        fprintf(fp, "\tnbr %d (%d,%d,%d): %d labels\n", i, xnbr_offset[i],
                ynbr_offset[i], znbr_offset[i], gc->nlabels[i]);
        fflush(fp);
...
int GCAvoxelToNode(GCA *gca, MRI *mri, int xv, int yv, int zv, int *pxn,
                   int *pyn, int *pzn) {
...
int GCAvoxelToNodeNearest(GCA *gca, MRI *mri, int xv, int yv, int zv, int *pxn,
                          int *pyn, int *pzn) {
  double xn, yn, zn;
  int    ixn, iyn, izn;
  int    errCode = NO_ERROR;
...
////////////////////////////////////////////////////////////////////
int GCAvoxelToPriorReal(const GCA *gca, const MRI *mri, const double xv,
                        const double yv, const double zv, double *pxp,
                        double *pyp, double *pzp) {
...
int GCAvoxelToPrior(const GCA *gca, const MRI *mri, const int xv, const int yv,
                    const int zv, int *pxp, int *pyp, int *pzp) {
  double xp, yp, zp;
  int    ixp, iyp, izp;
  int    errCode = NO_ERROR;
...
// use rounding to compute the index, not truncation
int GCAvoxelToPriorNearest(GCA *gca, MRI *mri, int xv, int yv, int zv, int *pxp,
                           int *pyp, int *pzp) {
  double xp, yp, zp;
  int    ixp, iyp, izp;
  int    errCode = NO_ERROR;
...
#endif
  priorToVoxel[tid] =
      MatrixMultiply(voxelFromRAS, rasFromPrior, priorToVoxel[tid]);
...
int GCApriorToVoxel(GCA *gca, MRI *mri, int xp, int yp, int zp, int *pxv,
                    int *pyv, int *pzv) {
  double xv, yv, zv;
  int    ixv, iyv, izv;
  int    errCode = NO_ERROR;
...
int GCAsourceVoxelToPrior(const GCA *gca, MRI *mri, TRANSFORM *transform,
                          int xv, int yv, int zv, int *pxp, int *pyp,
                          int *pzp) {
...
*/
  ...
  if (*pxp < 0 || *pyp < 0 || *pzp < 0 || *pxp >= gca->prior_width ||
      *pyp >= gca->prior_height || *pzp >= gca->prior_depth) {
...
int GCAsourceFloatVoxelToPrior(GCA *gca, MRI *mri, TRANSFORM *transform,
                               float xv, float yv, float zv, int *pxp, int *pyp,
                               int *pzp) {
  float  xt = 0, yt = 0, zt = 0;
...
int GCAsourceFloatVoxelToPriorReal(GCA *gca, MRI *mri, TRANSFORM *transform,
                                   float xv, float yv, float zv, double *pxp,
                                   double *pyp, double *pzp) {
  float  xt = 0, yt = 0, zt = 0;
  double xrt, yrt, zrt, xrp, yrp, zrp;
...
// transform from source -> template space -> prior
/////////////////////////////////////////////////////////////////////
int GCAsourceVoxelToNode(const GCA *gca, MRI *mri, TRANSFORM *transform, int xv,
                         int yv, int zv, int *pxn, int *pyn, int *pzn) {
#if 1
  int xp, yp, zp;
  ...
  {
    double xrn, yrn, zrn;
    GCAvoxelToNodeReal((GCA *)gca, gca->mri_tal__, xt, yt, zt, &xrn, &yrn,
                       &zrn);
    *pxn = nint(xrn);
    *pyn = nint(yrn);
    *pzn = nint(zrn);
  ...
  if (*pxn < 0 || *pyn < 0 || *pzn < 0 || *pxn >= gca->node_width ||
      *pyn >= gca->node_height || *pzn >= gca->node_depth) {
    return (ERROR_BADPARM);
  }
...
////////////////////////////////////////////////////////////////////
MATRIX *GCAgetPriorToSourceVoxelMatrix(GCA *gca, const MRI *mri,
                                       TRANSFORM *transform) {
  MATRIX *rasFromPrior = gca->prior_i_to_r__;
  // extract_i_to_r(gca->mri_prior__);
  ...
  if (transform->type != LINEAR_VOX_TO_VOX) // from src to talairach volume
    ErrorExit(ERROR_BADPARM,
              "GCAgetPriorToSourceVoxelMatrix: needs vox-to-vox transform");
...
////////////////////////////////////////////////////////////////////
int GCApriorToSourceVoxelFloat(GCA *gca, const MRI *mri, TRANSFORM *transform,
...
int GCAnodeToSourceVoxelFloat(GCA *gca, MRI *mri, TRANSFORM *transform, int xn,
                              int yn, int zn, float *pxv, float *pyv,
                              float *pzv) {
  int    width, height, depth;
  double xt, yt, zt;
  float  xv, yv, zv;
...
int GCAnodeToSourceVoxel(GCA *gca, MRI *mri, TRANSFORM *transform, int xn,
                         int yn, int zn, int *pxv, int *pyv, int *pzv) {
  float xf, yf, zf;
  int   errCode = NO_ERROR;
  errCode =
...
int GCApriorToSourceVoxel(GCA *gca, const MRI *mri, TRANSFORM *transform,
                          const int xp, const int yp, const int zp, int *pxv,
                          int *pyv, int *pzv) {
  float xf, yf, zf;
  int   errCode = NO_ERROR;

  errCode = GCApriorToSourceVoxelFloat(gca, mri, transform, xp, yp, zp, &xf,
                                       &yf, &zf);
  if (xf < 0) {
    errCode = ERROR_BADPARM;
  } else if (yf < 0) {
...
GCA *GCAalloc(int ninputs, float prior_spacing, float node_spacing, int width,
  ...
  return (gcaAllocMax(ninputs, prior_spacing, node_spacing, width, height,
                      depth, max_labels, flags));
}

GCA *gcaAllocMax(int ninputs, float prior_spacing, float node_spacing,
                 int width, int height, int depth, int max_labels, int flags) {
  GCA *      gca;
  GCA_NODE * gcan;
  GCA_PRIOR *gcap;
  int        x, y, z;
  ...
  if (max_labels >= 0) {
    ...
    for (x = 0; x < gca->node_width; x++) {
      ...
      for (y = 0; y < gca->node_height; y++) {
        ...
        if (!gca->nodes[x][y])
          ErrorExit(ERROR_NOMEMORY, "GCAalloc: could not allocate %d,%dth *", x,
                    y);
        for (z = 0; z < gca->node_depth; z++) {
          ...
          if (max_labels > 0) {
            ...
            gcan->labels = (unsigned short *)calloc(gcan->max_labels,
      ...
      for (y = 0; y < gca->prior_height; y++) {
        ...
        for (z = 0; z < gca->prior_depth; z++) {
          ...
          if (max_labels > 0) {
            ...
            gcap->labels =
                (unsigned short *)calloc(max_labels, sizeof(unsigned short));
            if (!gcap->labels)
              ErrorExit(ERROR_NOMEMORY,
                        "GCANalloc: couldn't allocate labels to %d",
                        gcap->max_labels);
  ...
  gca->prior_i_to_r__ = nullptr;
  gca->prior_r_to_i__ = nullptr;
  gca->tal_i_to_r__ = gca->tal_r_to_i__ = nullptr;
...
void PrintInfoOnLabels(GCA *gca, int label, int xn, int yn, int zn, int xp,
                       int yp, int zp, int x, int y, int z) {
  GCA_NODE * gcan;
  GCA_PRIOR *gcap;
  GC1D *     gc;
  int        i;
  ...
  if (gc) {
    gcan = &gca->nodes[xn][yn][zn];
    fprintf(stdout,
            "\n Node (%3d, %3d, %3d) pos (%3d, %3d, %3d) label=%d, labels:", xn,
            yn, zn, x, y, z, label);
...
int GCAtrainCovariances(GCA *gca, MRI *mri_inputs, MRI *mri_labels,
                        TRANSFORM *transform) {
  ...
  */
  mri_mapped =
      MRIalloc(gca->node_width, gca->node_height, gca->node_depth, MRI_UCHAR);
  ...
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        // src -> talairach -> node
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
                                  &zn)) {
          if (!GCAsourceVoxelToPrior(gca, mri_inputs, transform, x, y, z, &xp,
                                     &yp, &zp)) {
            ///////////////// debug code ////////////////////////////
            ...
            if (xn == Ggca_x && yn == Ggca_y && zn == Ggca_z &&
              ...
              if (gc) {
                ...
                printf(" --> node(%d,%d,%d), "
                       "label %s (%d), mean ",
                       xn, yn, zn, cma_label_to_name(label), label);
                for (i = 0; i < gca->ninputs; i++) {
                  printf("%2.1f ", gc->means[i]);
                }
                printf("\ncovariances (det=%f):\n",
                       covariance_determinant(gc, gca->ninputs) /
                           pow((double)nsamples, (double)gca->ninputs));
                m = load_covariance_matrix(gc, nullptr, gca->ninputs);
  ...
  for (holes_filled = xn = 0; xn < gca->node_width; xn++) {
    for (yn = 0; yn < gca->node_height; yn++) {
      for (zn = 0; zn < gca->node_depth; zn++) {
        ...
        if (!GCAnodeToSourceVoxel(gca, mri_inputs, transform, xn, yn, zn, &x,
                                  &y, &z)) {
          GC1D *gc;
          label = nint(MRIgetVoxVal(mri_labels, x, y, z, 0));
          gc    = GCAfindGC(gca, xn, yn, zn, label);
          if (gc == nullptr) // label doesn't exist at this position
          {
            if (xn == Gx && yn == Gy && zn == Gz)
...
int GCAtrain(GCA *gca, MRI *mri_inputs, MRI *mri_labels, TRANSFORM *transform,
  ...
  if (first_time) {
    first_time = 0;
    logging    = getenv("GCA_LOG") != nullptr;
    if (logging) {
      ...
      for (z = 0; z < depth; z++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
          ...
          if (!GCAsourceVoxelToPrior(gca, mri_inputs, transform, x, y, z, &xp,
            ...
            if ((GCAupdateNode(gca, mri_inputs, xn, yn, zn, vals, label,
                               gca_prune, noint) == NO_ERROR) &&
                !(gca->flags & GCA_NO_MRF))
              //                 node        label point
              GCAupdateNodeGibbsPriors(gca, mri_labels, xn, yn, zn, x, y, z,
                                       label);
                ...
                for (n = 0; n < gcap->nlabels; n++) {
                  ...
                  if (gc == nullptr) {
                    printf("(%d, %d, %d): gcap[%d][%d][%d]->labels[%d] ="
                           " %s - node (%d, %d, %d): no gc!\n",
                           x, y, z, xp, yp, zp, n,
                           cma_label_to_name(gcap->labels[n]), xn, yn, zn);
                    DiagBreak();
                  }
                }
            }
  ...
  for (holes_filled = xp = 0; xp < gca->prior_width; xp++) {
    for (yp = 0; yp < gca->prior_height; yp++) {
      for (zp = 0; zp < gca->prior_depth; zp++) {
        ...
        if (!GCApriorToSourceVoxel(gca, mri_inputs, transform, xp, yp, zp, &x,
                                   &y, &z)) {
          GC1D *gc;
          label = nint(MRIgetVoxVal(mri_labels, x, y, z, 0));
          GCAupdatePrior(gca, mri_inputs, xp, yp, zp, label);
...
int GCAwrite(GCA *gca, const char *fname) {
  ...
  if (znz_isnull(file)) {
    ...
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "GCAwrite(%s): could not open file", fname));
  }
...
GCA *GCAread(const char *fname) {
  ...
  if (znz_isnull(file)) {
    ErrorReturn(NULL,
                (ERROR_BADPARM, "GCAread(%s): could not open file", fname));
  }

  if (!znzreadFloatEx(&version, file)) {
    ErrorReturn(NULL,
                (ERROR_BADPARM, "GCAread(%s): could not read file", fname));
  }
  ...
  if (version < GCA_UCHAR_VERSION) {
    ...
    gca = gcaAllocMax(ninputs, node_spacing, node_spacing,
                      node_spacing * node_width, node_spacing * node_height,
                      node_spacing * node_depth, 0, flags);
    if (!gca) {
      ErrorReturn(NULL, (Gerror, nullptr));
    }
    ...
    for (x = 0; x < gca->node_width; x++) {
      for (y = 0; y < gca->node_height; y++) {
        for (z = 0; z < gca->node_depth; z++) {
          ...
          if (gcan->nlabels) {
            gcan->labels =
                (unsigned short *)calloc(gcan->nlabels, sizeof(unsigned short));
            if (!gcan->labels)
              ErrorExit(ERROR_NOMEMORY,
            ...
            for (i = 0; i < GIBBS_NEIGHBORS; i++) {
              ...
              if (!gc->label_priors[i])
                ErrorExit(ERROR_NOMEMORY,
                          ...
                          fname, gc->nlabels);
              gc->labels[i] = (unsigned short *)calloc(gc->nlabels[i],
                                                       sizeof(unsigned short));
              if (!gc->labels)
                ErrorExit(ERROR_NOMEMORY,
                            ...
                            resolution than densities */
  {
    ...
    gca = gcaAllocMax(ninputs, prior_spacing, node_spacing,
                      ...
                      node_spacing * node_depth, 0, flags);
    if (!gca) {
      ErrorReturn(NULL, (Gdiag, nullptr));
    }
...
static int GCAupdatePrior(GCA *gca, MRI *mri, int xn, int yn, int zn,
  ...
  if (label >= MAX_CMA_LABEL)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM,
                 "GCAupdatePrior(%d, %d, %d, %d): label out of range", xn, yn,
                 zn, label));
...
static int GCAupdateNode(GCA *gca, MRI *mri, int xn, int yn, int zn,
  ...
  if (label >= MAX_CMA_LABEL)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM,
                 "GCAupdateNode(%d, %d, %d, %d): label out of range", xn, yn,
                 zn, label));
  ...
  if (label > 0 && gca_prune != nullptr && !noint) {
    GCA_NODE *gcan_prune;
    GC1D *    gc_prune;
    int       nprune;
          ...
          if (nsamples <= 0) {
            ...
            if (!gc_nbr) {
              ErrorPrintf(ERROR_BADPARM,
                          ...
                          "%s (%d)",
                          x, y, z, n, cma_label_to_name(gcan->labels[n]),
                          gcan->labels[n]);
              continue;
            }
            holes_filled++;
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          if (x == Ggca_x && y == Ggca_y && z == Ggca_z &&
            ...
            printf("final covariance matrix for %s "
                   "(nsamples=%d, det=%f):\n",
                   cma_label_to_name(gcan->labels[n]), nsamples, det);
            m = load_covariance_matrix(gc, nullptr, gca->ninputs);
            MatrixPrint(stdout, m);
            MatrixFree(&m);
            fflush(stdout);
    ...
    for (y = 0; y < gca->node_height; y++) {
      for (z = 0; z < gca->node_depth; z++) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          gc        = &gcan->gcs[n];
          m_cov     = load_covariance_matrix(gc, nullptr, gca->ninputs);
          ...
          if (m_cov_inv == nullptr) /* singular */
          {
            MATRIX *m_I;
            m_I = MatrixIdentity(gca->ninputs, nullptr);
            MatrixScalarMul(m_I, MIN_VAR, m_I);
            MatrixAdd(m_cov, m_I, m_cov);
            m_cov_inv = MatrixInverse(m_cov, nullptr);
            if (m_cov_inv == nullptr)
              ErrorExit(ERROR_BADPARM,
...
MRI *GCAlabel(MRI *mri_inputs, GCA *gca, MRI *mri_dst, TRANSFORM *transform) {
  ...
  use_partial_volume_stuff = (getenv("USE_PARTIAL_VOLUME_STUFF") != nullptr);
  ...
  if (!mri_dst) {
    mri_dst = MRIalloc(mri_inputs->width, mri_inputs->height, mri_inputs->depth,
                       MRI_INT);
    if (!mri_dst) {
...
#if INTERP_PRIOR
            prior = gcaComputePrior(gca, mri_inputs, transform, x, y, z,
                                    gcap->labels[n]);
            p     = gcaComputeLogDensity(gc, vals, gca->ninputs, prior,
                                     gcap->labels[n]);
#else
            p = gcaComputeLogDensity(gc, vals, gca->ninputs, gcap->priors[n],
                                     gcap->labels[n]);
#endif
#endif
            // look for largest p
          ...
          //////////// start of partial volume stuff
          {
            ...
            for (n = 0; n < gcap->nlabels; n++)
              for (n1 = n + 1; n1 < gcap->nlabels; n1++) {
                l1 = gcap->labels[n];
                l2 = gcap->labels[n1];
                p  = compute_partial_volume_log_posterior(gca, gcan, gcap, vals,
                                                         l1, l2);
                if (p > max_p_pv) {
                  max_l1   = l1;
                  max_l2   = l2;
...
MRI *GCAlabelProbabilities(MRI *mri_inputs, GCA *gca, MRI *mri_dst,
                           TRANSFORM *transform) {
    ...
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
          ...
          gcan = &gca->nodes[xn][yn][zn];
          gcap = getGCAP(gca, mri_inputs, transform, x, y, z);
          if (gcap == nullptr || gcap->nlabels <= 0) {
            continue;
          }
          // label = 0;
          max_p = 2 * GIBBS_NEIGHBORS * BIG_AND_NEGATIVE;
          ...
          for (total_p = 0.0, n = 0; n < gcan->nlabels; n++) {
            ...
            p = GCAcomputePosteriorDensity(gcap, gcan, n, -1, vals,
                                           gca->ninputs, xn, yn, zn, gca);
            if (p > max_p) {
              max_p = p;
              // label = gcan->labels[n];
      ...
      for (z = 0; z < depth; z++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
          ...
          for (total_p = 0.0, n = 0; n < gcap->nlabels; n++) {
            ...
            MRIsetVoxVal(mri_labels, x, y, z, 0,
                         gcap->labels[n]); // change it for markov calculation

            p = gcaGibbsLogPosterior(gca, mri_labels, vals, gcap->labels[n], x,
                                     y, z, gcap, gcan, transform);
          ...
          qsort(label_probs, gcap->nlabels, sizeof(LABEL_PROB),
                compare_sort_probabilities);

          MRIsetVoxVal(mri_labels, x, y, z, 0,
                       label); // change it back to original label
          for (n = 0; n < max_labels; n++) {
            if (n < gcap->nlabels) {
              MRIsetVoxVal(mri_dst, x, y, z, 2 * n, label_probs[n].label);
              MRIsetVoxVal(
                  mri_dst, x, y, z, 2 * n + 1,
                  -label_probs[n].prob); // remove the negative from before
            } else {
...
MRI *GCAannealUnlikelyVoxels(MRI *mri_inputs, GCA *gca, MRI *mri_dst,
  ...
  int x, y, z, width, depth, height, *x_indices, *y_indices, *z_indices,
      nindices, index, iter, nchanged, xn, yn, zn, n, nbad, old_label;
  double log_posterior, T, delta_E, p, rn, new_posterior, old_posterior,
      total_posterior;
  GCA_NODE *gcan;
  MRI *     mri_bad;
  ...
  for (nindices = x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
                                  &zn)) {
          log_posterior = GCAvoxelGibbsLogPosterior(
              gca, mri_dst, mri_inputs, x, y, z, transform, prior_factor);
          gcan = &gca->nodes[xn][yn][zn];
  ...
  do {
    ...
    for (nbad = nchanged = index = 0; index < nindices; index++) {
      ...
      if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
        ...
        old_posterior = GCAnbhdGibbsLogPosterior(gca, mri_dst, mri_inputs, x, y,
                                                 z, transform, prior_factor);
        old_label     = nint(MRIgetVoxVal(mri_dst, x, y, z, 0));
        MRIsetVoxVal(mri_dst, x, y, z, 0, gcan->labels[n]);
        new_posterior = GCAnbhdGibbsLogPosterior(gca, mri_dst, mri_inputs, x, y,
                                                 z, transform, prior_factor);
    ...
    fprintf(stdout, "%03d: T = %2.2f, nchanged %d, nbad = %d, ll=%2.2f\n", iter,
            T, nchanged, nbad, total_posterior / (double)nindices);
    if (!nchanged) {
      break;
    }
  } while (iter++ < max_iter);
...
MRI *GCAclassify(MRI *mri_inputs, GCA *gca, MRI *mri_dst, TRANSFORM *transform,
                 int max_labels) {
...
int GCAremoveOutlyingSamples(GCA *gca, GCA_SAMPLE *gcas, MRI *mri_inputs,
  ...
  */
  ...
  for (nremoved = i = 0; i < nsamples; i++) {
    ...
    yp = gcas[i].yp;
    zp = gcas[i].zp;
    if (!GCApriorToSourceVoxel(gca, mri_inputs, transform, xp, yp, zp, &x, &y,
                               &z)) {
      load_vals(mri_inputs, x, y, z, vals, gca->ninputs);
    ...
    if (!GCApriorToSourceVoxel(gca, mri_inputs, transform, xp, yp, zp, &x, &y,
      ...
      if (!GCApriorToNode(gca, xp, yp, zp, &xn, &yn, &zn)) {
        ...
        for (norm_log_p = 0.0f, n = 0; n < gcan->nlabels; n++) {
          gc = &gcan->gcs[n];
          norm_log_p += GCAcomputeConditionalDensity(gc, vals, gca->ninputs,
                                                     gcan->labels[n]);
        }
        norm_log_p = log(norm_log_p);
        gc         = GCAfindPriorGC(gca, xp, yp, zp, gcas[i].label);
        log_p =
            GCAcomputeConditionalDensity(gc, vals, gca->ninputs, gcas[i].label);
        if (log_p < -clamp) {
          log_p = -clamp;
        }
...
float GCAcomputeLabelIntensityVariance(GCA *gca, GCA_SAMPLE *gcas,
  ...
  MATRIX *m_prior2source_voxel = nullptr;
...
*/
  ...
  if (transform->type == MORPH_3D_TYPE)
    m_prior2source_voxel = nullptr;
  else
    m_prior2source_voxel =
        GCAgetPriorToSourceVoxelMatrix(gca, mri_inputs, transform);

  max_label = 0;
  total_var = 0.0;
  // ifdef HAVE_OPENMP
  // pragma omp parallel for if_ROMP(experimental) firstprivate(gcas, tid,
  // m_prior2source_voxel) reduction(max:max_label) endif
  for (i = 0; i < nsamples; i++) {
    ...
    if (MRIindexNotInVolume(mri_inputs, x, y, z) == 0) {
      ...
      load_vals(mri_inputs, x, y, z, vals, gca->ninputs);
      if (label > 0 &&
          vals[0] < 20) // real tissue shouldn't land where there is no data
      {
        total_var += 1000 * 1000;
      }
  ...
  for (nlabels = label = 0; label <= max_label; label++) {
    if (label_counts[label] > 0) {
      ...
      label_vars[label] = label_vars[label] / label_counts[label] -
                          label_means[label] * label_means[label];
...
float GCAcomputeLogSampleProbability(GCA *const gca, GCA_SAMPLE *const gcas,
                                     MRI *const       mri_inputs,
                                     TRANSFORM *const transform,
                                     int const nsamples, double const clamp) {
  ...
  */
  ...
  MATRIX *m_prior2source_voxel_nonconst =
      (transform->type == MORPH_3D_TYPE)
          ? nullptr
          : GCAgetPriorToSourceVoxelMatrix(gca, mri_inputs, transform);
  MATRIX const *const m_prior2source_voxel =
      m_prior2source_voxel_nonconst; // just to make sure not changed below
...
#include "romp_for_end.h"
  ...
  if (m_prior2source_voxel_nonconst)
    MatrixFree(&m_prior2source_voxel_nonconst);
      ...
      for (z = 0; z < depth; z++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
          ...
          if (n < gcan->nlabels) {
            ...
            total_log_p += gcaComputeLogDensity(gc, vals, gca->ninputs,
...
int GCAtransformSamples(GCA *gca_src, GCA *gca_dst, GCA_SAMPLE *gcas,
                        int nsamples) {
  int scale, i, label, xd, yd, zd, xs, ys, zs, n, xk, yk, zk, xd0, yd0, zd0,
      min_y_i, xdn, ydn, zdn;
  ...
  vscale = 1;
  mri_found =
      MRIalloc(gca_dst->node_width * vscale, gca_dst->node_height * vscale,
  ...
  fprintf(stdout, "min_y = (%d, %d, %d) at i=%d, label=%d\n", gcas[i].xp,
          gcas[i].yp, gcas[i].zp, i, gcas[i].label);
  MRIfree(&mri_found);
  fflush(stdout);
  return (NO_ERROR);
}
...
GCA_SAMPLE *GCAfindStableSamplesByLabel(GCA *gca, int nsamples,
  ...
  GCA_PRIOR * gcap;
  GC1D *      gc;
  int found[MAX_DIFFERENT_LABELS], x, y, z, width, height, depth, n, label,
      nfound, samples_added[MAX_DIFFERENT_LABELS];
  float histo[MAX_DIFFERENT_LABELS], spacing, scale;
  int   volume[TILES][TILES], max_class, total, extra, total_found,
      label_counts[MAX_DIFFERENT_LABELS], current_index[MAX_DIFFERENT_LABELS],
      ordered_label_counts[MAX_DIFFERENT_LABELS], i, index, *x_indices,
      *y_indices, *z_indices, nindices;
  ...
  for (n = 0;
       (unsigned)n < sizeof(exclude_classes) / sizeof(exclude_classes[0]);
       n++) {
  ...
  for (index = 0; index < nindices; index++) {
    ...
    for (n = 0; n < gcap->nlabels; n++) {
      ...
      if (histo[label] > 0) {
        ...
        ordered_labels[label][i].covars = (float *)calloc(
            (gca->ninputs * (gca->ninputs + 1)) / 2, sizeof(float));
  ...
  for (spacing = MAX_SPACING; spacing >= gca->node_spacing; spacing /= 2) {
    ...
    mri_found =
        MRIalloc(width * scale, height * scale, depth * scale, MRI_UCHAR);
...
#else
  ErrorReturn(
      NULL, (ERROR_UNSUPPORTED, "GCAfindContrastSamples: no longer supported"));
#endif
}

GCA_SAMPLE *GCAfindStableSamples(GCA *gca, int *pnsamples, int min_spacing,
                                 float min_prior, int *exclude_list,
                                 int unknown_nbr_spacing, int vent_spacing) {
  GCA_SAMPLE *gcas;
  ...
  MRI * mri_filled, *mri_vent_dist = nullptr;

  if (gca->max_label >= MAX_DIFFERENT_LABELS)
    ErrorExit(ERROR_UNSUPPORTED,
              "GCAfindStableSamples: max label %d too large (%d)\n",
              gca->max_label, MAX_DIFFERENT_LABELS);

#define MIN_UNKNOWN_DIST 2

  if (vent_spacing > 0) {
    MRI *mri_labels = GCAbuildMostLikelyLabelVolume(gca, nullptr);
    MRI *mri_vent;
    mri_vent = MRIclone(mri_labels, nullptr);
    ...
    mri_vent_dist = MRIdistanceTransform(mri_vent, nullptr, 1, mri_vent->width,
  ...
  if (!gcas)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %dK x %d samples\n",
              "GCAfindStableSamples", width * height * depth / 1024,
              sizeof(GCA_SAMPLE));
  ...
  for (nzeros = nfound = x = 0; x < width; x += prior_stride) {
    ...
    for (y = 0; y < height; y += prior_stride) {
      ...
      for (z = 0; z < depth; z += prior_stride) {
        ...
        if (abs(x - 31) <= prior_stride && abs(y - 22) <= prior_stride &&
            abs(z - 36) <= prior_stride) {
          DiagBreak();
        }
        ...
        if ((different_nbr_max_labels(gca, x, y, z, unknown_nbr_spacing, 0) >
             0) &&
            (exclude_list && exclude_list[0] == 0) &&
            (priors[0] >= min_prior) && (priors[0] >= .5 * max_priors[0])) {
          best_label = 0;
        } else {
          best_label = -1;
        }
        ...
        for (label = 1; label < MAX_DIFFERENT_LABELS; label++) {
          ...
          */
          ...
          if ((IS_WM(label) || IS_THALAMUS(label) || IS_CAUDATE(label)) &&
              mri_vent_dist &&
                   ...
                   (means[0] >= max_unknown))) /* disabled check */
              {
                if (!GCApriorToVoxel(gca, mri_filled, x, y, z, &xv, &yv, &zv)) {
                  if (MRIgetVoxVal(mri_filled, xv, yv, zv, 0) == 0) {
                    mriFillRegion(mri_filled, xv, yv, zv, 0, 1,
                                  MIN_UNKNOWN_DIST);
...
GCA_SAMPLE *GCAfindExteriorSamples(GCA *gca, int *pnsamples, int min_spacing,
                                   float min_prior, int unknown_nbr_spacing,
                                   int use_ventricles) {
  GCA_SAMPLE *gcas;
  ...
  for (nzeros = nfound = x = 0; x < width; x += prior_stride) {
    ...
    for (y = 0; y < height; y += prior_stride) {
      ...
      for (z = 0; z < depth; z += prior_stride) {
        ...
        if (best_label == 0 &&
            (different_nbr_max_labels(gca, x, y, z, unknown_nbr_spacing, 0) ==
             0)) {
          continue;
        }
        ...
        if (gcaFindBestSample(gca, x, y, z, best_label, prior_stride / 2,
          ...
          if (best_label != 0 || ((means[0][0] <= max_unknown[0]) ||
...
int GCAwriteSamples(GCA *gca, MRI *mri, GCA_SAMPLE *gcas, int nsamples,
  ...
  for (n = 0; n < nsamples; n++) {
    ...
    if (!GCApriorToVoxel(gca, mri_dst, gcas[n].xp, gcas[n].yp, gcas[n].zp, &xv,
                         &yv, &zv)) {
...
MRI *GCAmri(GCA *gca, MRI *mri) {
  ...
  if (!mri) {
    mri = MRIallocSequence(gca->node_width, gca->node_height, gca->node_depth,
                           MRI_UCHAR, gca->ninputs);
    mri->xsize = gca->xsize * gca->node_spacing;
    mri->ysize = gca->ysize * gca->node_spacing;
    mri->zsize = gca->zsize * gca->node_spacing;
  }
    ...
    for (x = 0; x < width; x++) {
      for (y = 0; y < height; y++) {
        for (z = 0; z < depth; z++) {
          ...
          if (!GCAsourceVoxelToNode(gca, mri, transform, x, y, z, &xn, &yn,
                                    &zn)) {
            gcan = &gca->nodes[xn][yn][zn];
            if (gcan->nlabels < 1) {
              continue;
...
#endif
static int gcaRegionStats(GCA *gca, int x, int y, int z, int wsize,
                          float *priors, float *vars,
                          float means[MAX_DIFFERENT_LABELS][MAX_GCA_INPUTS]) {
  int xk, yk, zk, xi, yi, zi, width, height, depth, label, n, total_training, r,
      l;
  GCA_PRIOR *gcap;
  GC1D *     gc;
  float      dof;
        ...
        for (n = 0; n < gcap->nlabels; n++) {
          ...
          if (label > MAX_DIFFERENT_LABELS) {
            printf("ERROR: label = %d > %d n = %d, xyzi = %d %d %d\n", label,
                   MAX_DIFFERENT_LABELS, n, xi, yi, zi);
            fflush(stdout);
          }
...
static int gcaFindBestSample(GCA *gca, int x, int y, int z, int label,
                             int wsize, GCA_SAMPLE *gcas) {
  int xk, yk, zk, xi, yi, zi, width, height, depth, n, best_x, best_y, best_z,
      xn, yn, zn, r, c, v;
  // int best_n;
  GCA_PRIOR *gcap;
  GC1D *     gc, *best_gc;
  ...
  gcas->covars =
      (float *)calloc((gca->ninputs * (gca->ninputs + 1)) / 2, sizeof(float));
...
int GCAtransformAndWriteSamplePvals(GCA *gca, MRI *mri, GCA_SAMPLE *gcas,
                                    int nsamples, const char *fname,
                                    TRANSFORM *transform) {
  int        i, n, xv, yv, zv;
  MRI *      mri_dst;
  float      vals[MAX_GCA_INPUTS];
  ...
  for (n = 0; n < nsamples; n++) {
    ...
    if (!GCApriorToSourceVoxel(gca, mri_dst, transform, gcas[n].xp, gcas[n].yp,
                               gcas[n].zp, &xv, &yv, &zv)) {
      gcap = &gca->priors[gcas[n].xp][gcas[n].yp][gcas[n].zp];
      load_vals(mri, xv, yv, zv, vals, gca->ninputs);
      pval = gcaComputeSampleConditionalDensity(&gcas[n], vals, gca->ninputs,
                                                gcas[n].label);
      pval *= getPrior(gcap, gcas[n].label);
      pval_total = 0.0;
      for (i = 0; i < gcap->nlabels; i++) {
        p = gcaComputeSampleConditionalDensity(&gcas[n], vals, gca->ninputs,
                                               gcap->labels[i]);
        p *= gcap->priors[i];
        pval_total += p;
      }
...
int GCAtransformAndWriteSampleMeans(GCA *gca, MRI *mri, GCA_SAMPLE *gcas,
                                    int nsamples, const char *fname,
                                    TRANSFORM *transform) {
  int  n, xv, yv, zv, f;
  MRI *mri_dst;
  ...
  mri_dst = MRIallocSequence(mri->width, mri->height, mri->depth, MRI_FLOAT,
                             gca->ninputs);
  MRIcopyHeader(mri, mri_dst);
...
int GCAtransformAndWriteSamples(GCA *gca, MRI *mri, GCA_SAMPLE *gcas,
  ...
  mri_dst = MRIalloc(mri->width, mri->height, mri->depth, MRI_UCHAR);
  MRIcopyHeader(mri,
                mri_dst); // TransformSetMRIVolGeomToDst(transform, mri_dst) ;
    ...
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          offset = 0.5 * pow(covariance_determinant(gc, gca->ninputs),
...
static int GCAupdateNodeGibbsPriors(GCA *gca, MRI *mri, int xn, int yn, int zn,
  ...
  // not found
  if (n >= gcan->nlabels) /* have to allocate a new classifier */
    ErrorExit(ERROR_BADPARM, "gca(%d, %d, %d): could not find label %d", xn, yn,
              zn, label);
  ...
  for (i = 0; i < GIBBS_NEIGHBORHOOD; i++) {
    ...
    if (xn == Ggca_x && yn == Ggca_y && zn == Ggca_z && label == Ggca_label &&
      ...
      printf("(%d, %d, %d) --> node (%d, %d, %d), "
             ...
             xl, yl, zl, xn, yn, zn, cma_label_to_name(label), label, xnbr,
             ynbr, znbr, cma_label_to_name(nbr_label), nbr_label);
    }
...
MRI *GCAanneal(MRI *mri_inputs, GCA *gca, MRI *mri_dst, TRANSFORM *transform,
  ...
  int x, y, z, width, height, depth, label, iter, xn, yn, zn, n, nchanged,
  ...
  mri_changed = MRIclone(mri_dst, nullptr);
  ...
  do {
    ...
    for (index = 0; index < nindices; index++) {
      ...
      if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
        ...
        min_posterior = GCAnbhdGibbsLogPosterior(gca, mri_dst, mri_inputs, x, y,
                                                 z, transform, prior_factor);
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          MRIsetVoxVal(mri_dst, x, y, z, 0, gcan->labels[n]);
          new_posterior = GCAnbhdGibbsLogPosterior(
              gca, mri_dst, mri_inputs, x, y, z, transform, prior_factor);
    ...
    if (nchanged > 10000) {
      ll = GCAgibbsImageLogPosterior(gca, mri_dst, mri_inputs, transform,
                                     prior_factor);
      ll /= (double)(width * depth * height);
      if (!FZERO(lcma))
        printf("pass %d: %d changed. image ll: %2.3f (CMA=%2.3f)\n", iter + 1,
...
MRI *GCAreclassifyUsingGibbsPriors(MRI *mri_inputs, GCA *gca, MRI *mri_dst,
                                   TRANSFORM *transform, int max_iter,
                                   MRI *mri_fixed, int restart,
                                   void (*update_func)(MRI *),
                                   double min_prior_factor,
                                   double max_prior_factor) {
  int x, y, z, width, height, depth, iter, nchanged, min_changed, index,
      nindices, fixed;
  short *x_indices, *y_indices, *z_indices;
  ...
  */
  ...
  if (!restart) {
    ...
    old_posterior = GCAgibbsImageLogPosterior(gca, mri_dst, mri_inputs,
  ...
  do {
    if (restart) {
      ...
      mri_probs = GCAlabelProbabilities(mri_inputs, gca, nullptr, transform);
    ...
    for (index = 0; index < nindices; index++) {
      ...
      max_posterior = GCAnbhdGibbsLogPosterior(gca, mri_dst, mri_inputs, x, y,
                                               z, transform, prior_factor);
      ...
      for (n = 0; n < gcap->nlabels; n++) {
        ...
        if (new_posterior > max_posterior) {
          if (x == Ggca_x && y == Ggca_y && z == Ggca_z &&
              (label == Ggca_label || old_label == Ggca_label ||
               Ggca_label < 0))
            fprintf(stdout,
                    "NbhdGibbsLogLikelihood at (%d, %d, %d):"
                    " old = %d (ll=%.2f) new = %d (ll=%.2f)\n",
                    x, y, z, old_label, max_posterior, gcap->labels[n],
                    new_posterior);
    ...
    // reclassify connected compontents of unlikely labels as a whole
    {
      ...
      do {
        nchanged       = 0;
        mri_impossible = GCAmarkImpossible(gca, mri_dst, nullptr, transform);
        if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
          MRIwrite(mri_impossible, "imp.mgz");
        }
        mri_impossible_label = MRIclone(mri_impossible, nullptr);
        ...
        for (label = 1; label <= max_label; label++) {
          ...
          if (MRIcopyLabeledVoxels(mri_impossible, mri_dst,
                                   mri_impossible_label, label) == 0) {
          ...
          if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
            printf("%d low probability segments "
                   "found for label %s (%d)...\n",
                   mriseg->nsegments, cma_label_to_name(label), label);

          for (s = 0; s < mriseg->nsegments; s++) {
            if (gcaRelabelSegment(gca, transform, mri_inputs, mri_dst,
                                  &mriseg->segments[s]) > 0) {
        ...
        new_posterior = GCAgibbsImageLogPosterior(gca, mri_dst, mri_inputs,
                                                  transform, prior_factor) /
                        (double)(width * depth * height);
        printf("%02d: ll %2.5f (%d segments changed)\n", iter + 1,
               new_posterior, nchanged);
...
double GCAgibbsImageLogPosterior(GCA *gca, MRI *mri_labels, MRI *mri_inputs,
  ...
  // ifdef HAVE_OPENMP
  // pragma omp parallel for if_ROMP(experimental) reduction(+:
  // total_log_posterior) endif
  for (x = 0; x < width; x++) {
    int    y, z;
    ...
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        log_posterior = GCAvoxelGibbsLogPosterior(
            gca, mri_labels, mri_inputs, x, y, z, transform, prior_factor);
...
static double gcaGibbsImpossibleConfiguration(GCA *gca, MRI *mri_labels, int x,
  ...
  /* find the node associated with this coordinate and classify */
  if (!GCAsourceVoxelToNode(gca, mri_labels, transform, x, y, z, &xn, &yn,
                            &zn)) {
    gcan = &gca->nodes[xn][yn][zn];
...
double GCAnbhdGibbsLogPosterior(GCA *gca, MRI *mri_labels, MRI *mri_inputs,
                                int x, int y, int z, TRANSFORM *transform,
                                double gibbs_coef) {
  double total_log_posterior /*, log_posterior*/;
  /*  int    i, xnbr, ynbr, znbr ;*/

  total_log_posterior = GCAvoxelGibbsLogPosterior(
      gca, mri_labels, mri_inputs, x, y, z, transform, gibbs_coef);
  ...
  if (check_finite("GCAnbhdGibbsLogPosterior: final", total_log_posterior) ==
      0) {
    DiagBreak();
  }
...
double GCAwindowPosteriorLogProbability(GCA *gca, MRI *mri_labels,
                                        MRI *mri_inputs, TRANSFORM *transform,
    ...
    for (yk = -whalf; yk <= whalf; yk++) {
      ...
      for (zk = -whalf; zk <= whalf; zk++) {
        ...
        if (xi == Ggca_x && yi == Ggca_y && zi == Ggca_z)
          DiagBreak();
        posterior = GCAvoxelGibbsLogPosterior(gca, mri_labels, mri_inputs, xi,
                                              yi, zi, transform, 1);
...
double GCAvoxelGibbsLogPosterior(GCA *gca, MRI *mri_labels, MRI *mri_inputs,
                                 int x, int y, int z, TRANSFORM *transform,
                                 double gibbs_coef) {
  ...
  if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
    ...
    if (label > MAX_CMA_LABEL) {
      ...
      if (gcan) {
        printf("gcan->nlabels = %d, gcan->total_training = %d ", gcan->nlabels,
               gcan->total_training);
        printf("log(return) = %.2f\n",
               log(0.01f / ((float)gcan->total_training * GIBBS_NEIGHBORS)));
        printf("labels for this location\n");
        for (n = 0; n < gcan->nlabels; n++)
          printf("label=%s (%d); ", cma_label_to_name(gcan->labels[n]),
                 gcan->labels[n]);
      }
    }
    /////////////////////////////////////////////////////////////////
    ...
    /* compute 1-d Mahalanobis distance */
    log_posterior =
        GCAcomputeConditionalLogDensity(gc, vals, gca->ninputs, label);
    if (check_finite("GCAvoxelGibbsLogPosterior: conditional log density",
                     log_posterior) == 0) {
      DiagBreak();
    }

    nbr_prior = 0.0;
    if (gc->nlabels == nullptr) {
...
double GCAimagePosteriorLogProbability(GCA *gca, MRI *mri_labels,
                                       MRI *mri_inputs, TRANSFORM *transform) {
  double log_posterior;
  int    x, y, z, num = 0;
  ...
  for (log_posterior = 0.0, x = 0; x < mri_labels->width; x++)
    for (y = 0; y < mri_labels->height; y++)
      for (z = 0; z < mri_labels->depth; z++) {
        ...
        log_posterior += GCAvoxelLogPosterior(gca, mri_labels, mri_inputs, x, y,
                                              z, transform);
        num++;
...
double GCAvoxelLogPosterior(GCA *gca, MRI *mri_labels, MRI *mri_inputs, int x,
  ...
  if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
    ...
    /* compute 1-d Mahalanobis distance */
    log_posterior = GCAcomputeConditionalLogDensity(gc, vals, gca->ninputs,
                                                    gcan->labels[n]);
    if (check_finite("GCAvoxelGibbsLogPosterior: conditional log density",
                     log_posterior) == 0) {
      DiagBreak();
...
static int gcaReclassifySegment(GCA *gca, MRI *mri_inputs, MRI *mri_labels,
                                MRI_SEGMENT *mseg, int old_label,
                                TRANSFORM *transform, int *exclude_labels,
                                int nexcluded);
static int gcaReclassifyVoxel(GCA *gca, MRI *mri_inputs, MRI *mri_labels, int x,
                              int y, int z, int old_label, TRANSFORM *transform,
...

{
  ...
  ndilate =
      nint(vent_topo_dist /
           ((mri_inputs->xsize + mri_inputs->ysize + mri_inputs->zsize) / 3));
  ...
  for (i = 1; i <= MAX_CMA_LABEL; i++) {
    ...
    /*
      ...
      the main/biggest one, but being wary of small disconnections. Dilate the
      labels a few times to close small disconnections, then erase segments that
      are still not connected.
    */
    if (IS_LAT_VENT(i)) {
      int max_segno;

      mri_dilated         = MRIdilateLabel(mri_src, nullptr, i, ndilate);
      mriseg              = MRIsegment(mri_dilated, (float)i, (float)i);
      max_segno           = MRIfindMaxSegmentNumber(mriseg);
      mri_in_main_segment = MRIsegmentFill(mriseg, max_segno, nullptr,
                                           1); // only retain biggest segment
      MRIfree(&mri_dilated);
      MRIsegmentFree(&mriseg);
      nex = NCHOROID;
      ex  = choroid_labels;
    } else {
      mri_in_main_segment = nullptr;
      nex                 = 0;
      ex                  = nullptr;
    }
    ...
    for (j = 0; j < mriseg->nsegments; j++) {
      ...
      // found above, or just big enough and keep it
      if (IS_LAT_VENT(i) &&
          ((mriseg->segments[j].nvoxels * voxel_volume >
            vent_topo_volume_thresh2) ||
           ((mriseg->segments[j].nvoxels * voxel_volume >
             vent_topo_volume_thresh1) &&
            MRIgetVoxVal(mri_in_main_segment, mriseg->segments[j].voxels[0].x,
                         mriseg->segments[j].voxels[0].y,
                         mriseg->segments[j].voxels[0].z, 0) > 0))) {
        if (mriseg->segments[j].nvoxels * voxel_volume >
            vent_topo_volume_thresh2)
          printf(" !!!!!!!!! ventricle segment %d with volume %2.0f above "
                 "threshold %2.0f - not erasing !!!!!!!!!!\n",
                 j, mriseg->segments[j].nvoxels * voxel_volume,
                 vent_topo_volume_thresh2);
        continue;
      }
...
static int gcaReclassifySegment(GCA *gca, MRI *mri_inputs, MRI *mri_labels,
                                MRI_SEGMENT *mseg, int old_label,
                                TRANSFORM *transform, int *exclude_list,
                                int nexcluded) {
  int i;

  for (i = 0; i < mseg->nvoxels; i++) {
#if 1
    gcaReclassifyVoxel(gca, mri_inputs, mri_labels, mseg->voxels[i].x,
                       mseg->voxels[i].y, mseg->voxels[i].z, old_label,
                       transform, exclude_list, nexcluded);
#else
...
static int gcaReclassifyVoxel(GCA *gca, MRI *mri_inputs, MRI *mri_labels, int x,
                              int y, int z, int old_label, TRANSFORM *transform,
                              int *exclude_list, int nexcluded) {
  int nbr_labels[MAX_CMA_LABELS], xi, yi, zi, xk, yk, zk, i, new_label, ex,
      skip;
  double max_p, p;
  ...
  for (i = 0; i < MAX_CMA_LABEL; i++) {
    ...
    if (nbr_labels[i] > 0) // if neighbors has this label, then
    {
      ...
      p = GCAvoxelGibbsLogPosterior(gca, mri_labels, mri_inputs, x, y, z,
                                    transform, PRIOR_FACTOR);
      // debug ///////////////////////////////////////
      if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
        ...
        if (p >= max_p)
          printf("   replacing max_p with this p and "
                 ...
                 cma_label_to_name(old_label), old_label, cma_label_to_name(i),
                 i);
      }
      ////////////////////////////////////////////////
      if (p >= max_p) {
...
#if 1
MRI *GCAexpandVentricle(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst,
                        TRANSFORM *transform, int target_label) {
  int nchanged, x, y, z, xn, yn, zn, xk, yk, zk, xi, yi, zi, label,
      total_changed, i, j, count, found, xmin, xmax, ymin, ymax, zmin, zmax;
  // int width, height, depth;
  // GCA_NODE *gcan;
  GC1D *gc;
  float v_means[MAX_GCA_INPUTS], v_var, dist_ven, dist_label,
      vals[MAX_GCA_INPUTS];
      ...
      for (y = ymin; y <= ymax; y++) {
        for (x = xmin; x <= xmax; x++) {
          ...
          for (yk = -1; found == 0 && yk <= 1; yk += 2) {
            ...
            count = MRIlabelsInPlanarNbhd(mri_dst, x, yi, z, V_WHALF,
                                          target_label, MRI_HORIZONTAL);
            if (count >= V_THRESH) {
              found = 1;
            }
          }
          for (xk = -1; found == 0 && xk <= 1; xk += 2) {
            ...
            // only change it if there is a "wall" of
            // ventricle superior or inferior
            count = MRIlabelsInPlanarNbhd(mri_dst, xi, y, z, V_WHALF,
                                          target_label, MRI_SAGITTAL);
            if (count >= V_THRESH) {
              found = 1;
            }
          }
          for (zk = -1; found == 0 && zk <= 1; zk += 2) {
            ...
            count = MRIlabelsInPlanarNbhd(mri_dst, x, y, zi, V_WHALF,
                                          target_label, MRI_CORONAL);
            if (count >= V_THRESH) {
              found = 1;
            }
          }
          ...
          if (GCAsourceVoxelToNode(gca, mri_dst, transform, x, y, z, &xn, &yn,
                                   &zn) == NO_ERROR) {
            // gcan = &gca->nodes[xn][yn][zn];
                                           ...
                                           // than anything else
            {
              if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
                ...
                printf("GCAexpandVentricle:voxel"
                       ...
                       x, y, z, cma_label_to_name(olabel), olabel,
...
MRI *GCAexpandVentricle(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst,
                        TRANSFORM *transform, int target_label) {
  int nchanged, x, y, z, width, height, depth, xn, yn, zn, xi, yi, zi, xk, yk,
      zk, label, total_changed, i;
  GCA_NODE *gcan;
  GC1D *gc, *gc_vent;
              ...
              for (zk = -1; zk <= 1; zk++) {
                for (yk = -1; yk <= 1; yk++) {
                  for (xk = -1; xk <= 1; xk++) {
                    ...
                    {
                      if (!GCAsourceVoxelToNode(gca, mri_dst, transform, xi, yi,
                                                zi, &xn, &yn, &zn)) {
                        gcan = &gca->nodes[xn][yn][zn];
                        ...
                        if (gc == NULL) {
                          ...
                          plabel = GCAcomputeConditionalDensity(
                              gc, vals, gca->ninputs, label);
                        gc = GCAfindGC(gca, xn, yn, zn, target_label);
                        if (gc == NULL)
                        /* use neighboring gc */
                        ...
                        pv = GCAcomputeConditionalDensity(
                            gc, vals, gca->ninputs, target_label);
                        if (pv > 5 * plabel) {
                          if (xi == Ggca_x && yi == Ggca_y && zi == Ggca_z) {
                            int olabel =
                            ...
                            printf("GCAexpandVentricle:voxel"
                                   ...
                                   "label p = %.2f "
                                   "and new lable p = %.2f\n",
                                   xi, yi, zi, cma_label_to_name(olabel),
                                   olabel, cma_label_to_name(target_label),
                                   target_label, plabel, pv);
                          }
...
MRI *GCAexpandCortex(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst,
                     TRANSFORM *transform) {
  int nchanged, x, y, z, width, height, depth, xn, yn, zn, xi, yi, zi, xk, yk,
      zk, label, total_changed, i, wm_nbr, gray_nbr, left;
  // GCA_NODE *gcan;
  float wm_means[MAX_GCA_INPUTS], wm_var, gray_means[MAX_GCA_INPUTS], gray_var,
      ldist, wdist, gdist;
  MRI * mri_tmp;
  float vals[MAX_GCA_INPUTS];
    ...
    for (z = 0; z < depth; z++) {
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          if (label == Unknown || label == Left_Cerebral_Cortex ||
                ...
                for (yk = -1; left < 0 && yk <= 1; yk++) {
                  for (xk = -1; left < 0 && xk <= 1; xk++) {
                    ...
                    if (label == Left_Cerebral_Cortex ||
                        label == Left_Cerebral_White_Matter) {
                      left = 1;
                    } else if (label == Right_Cerebral_Cortex ||
                               label == Right_Cerebral_White_Matter) {
                      left = 0;
                    }
                  }
            ...
            if (gc_wm)
              wm_nbr = left ? Left_Cerebral_White_Matter
                            : Right_Cerebral_White_Matter;
            if (gc_gm == nullptr || gc_wm == nullptr)
              for (zk = -1; zk <= 1; zk++) {
                for (yk = -1; yk <= 1; yk++) {
                  for (xk = -1; xk <= 1; xk++) {
                    if (fabs(xk) + fabs(yk) + fabs(zk) > 1) {
                    ...
                    if ((label == Left_Cerebral_Cortex ||
                        ...
                        (gc_gm == nullptr)) {
                      gray_nbr = label;
                      gc_gm = GCAfindSourceGC(gca, mri_src, transform, xi, yi,
                                              zi, label);
                      if (!gc_gm) {
                        gray_nbr = 0;
                      }
                      /* shouldn't ever happen */
                    } else if ((label == Left_Cerebral_White_Matter ||
                                label == Right_Cerebral_White_Matter) &&
                               (gc_wm == nullptr)) {
                      wm_nbr = label;
                      gc_wm  = GCAfindSourceGC(gca, mri_src, transform, xi, yi,
                                              zi, label);
                      if (!gc_wm) {
                        wm_nbr = 0;
                      }
            ...
            if (wm_nbr)
              wdist =
                  sqrt(GCAmahDistIdentityCovariance(gc_wm, vals, gca->ninputs));
            else {
              wdist = 1e10;
            }
            if (gray_nbr)
              gdist =
                  sqrt(GCAmahDistIdentityCovariance(gc_gm, vals, gca->ninputs));
            else {
              gdist = 1e10;
            }
            ...
            if (!GCAsourceVoxelToNode(gca, mri_dst, transform, x, y, z, &xn,
                                      &yn, &zn)) {
              // gcan = &gca->nodes[xn][yn][zn];
              ...
              /* might change to wm */
              {
                if (wdist < ldist && GCAisPossible(gca, mri_inputs, wm_nbr,
                                                   transform, x, y, z, 0)) {
                  if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
                    ...
                    if (olabel != wm_nbr)
                      printf("GCAexpandCortex:voxel (%d, %d, %d)"
                             ...
                             " ldist=%.2f\n",
                             x, y, z, cma_label_to_name(olabel), olabel,
                             cma_label_to_name(wm_nbr), wm_nbr, wdist, gdist,
                             ldist);
                  }
                  nchanged++;
              ...
              {
                if (gdist < ldist && GCAisPossible(gca, mri_inputs, gray_nbr,
                                                   transform, x, y, z, 0)) {
                  int olabel = nint(MRIgetVoxVal(mri_tmp, x, y, z, 0));
                  if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
                    if (olabel != gray_nbr)
                      printf("GCAexpandCortex:voxel "
                             ...
                             "gdist=%.2f, ldist=%.2f\n",
                             x, y, z, cma_label_to_name(olabel), olabel,
                             cma_label_to_name(gray_nbr), gray_nbr, wdist,
                             gdist, ldist);
                  }
                  if (olabel != gray_nbr) {
                    nchanged++;
...
MRI *GCAexpandLabelIntoWM(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst,
                          TRANSFORM *transform, MRI *mri_fixed,
                          int target_label) {
  int nchanged, x, y, z, width, height, depth, xn, yn, zn, xi, yi, zi, xk, yk,
      zk, nbr_label, n, label, total_changed, i;
        ...
        for (x = 0; x < width; x++) {
          ...
          if (!GCAsourceVoxelToNode(gca, mri_dst, transform, x, y, z, &xn, &yn,
            ...
            if (label == target_label) {
              ...
              gc_wm = nullptr;
              for (n = 0; n < gcan->nlabels; n++) {
                if ((gcan->labels[n] == Left_Cerebral_White_Matter) ||
                  ...
                  for (xk = -1; xk <= 1; xk++) {
                    ...
                    if ((nbr_label == Right_Cerebral_White_Matter) ||
                      ...
                      if (!GCAsourceVoxelToNode(gca, mri_dst, transform, xi, yi,
                        ...
                        // if wm is bigger
                        if (GCAmahDistIdentityCovariance(gc_wm, vals,
                                                         gca->ninputs) >
                            GCAmahDistIdentityCovariance(gc_label, vals,
                                                         gca->ninputs)) {
                          gcan_nbr = &gca->nodes[xn][yn][zn];
                          for (prior = 0.0f, n = 0; n < gcan_nbr->nlabels;
                               n++) {
                            // look for the
                            // target label
                            // in this neighbor
                            if (gcan_nbr->labels[n] == target_label) {
                              prior =
                                  get_node_prior(gca, target_label, xn, yn, zn);
                              if (prior != 0) {
                                break;
                              }
                            }
                          ...
                          /* target is possible */
                          {
                            if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
                              printf("GCAexpandLabelIntoWM:voxel (%d, %d, %d) "
                                     "changed from %s (%d) "
                                     "to %s (%d), prior=%.2f\n",
                                     xi, yi, zi, cma_label_to_name(nbr_label),
                                     nbr_label, cma_label_to_name(target_label),
                                     target_label, prior);
                            }
                            nchanged++;
                            MRIsetVoxVal(mri_tmp, xi, yi, zi, 0, target_label);
...
#include "mrinorm.h"
MRI *GCAnormalizeSamples(MRI *mri_in, GCA *gca, GCA_SAMPLE *gcas, int nsamples,
...
#define MIN_BIAS 750
  ...
  if (ctl_point_fname) {
    ...
    MRInormAddFileControlPoints(mri_ctrl, CONTROL_MARKED, nullptr);
  }
...
void GCAnormalizeSamplesOneChannel(MRI *mri_in, GCA *gca, GCA_SAMPLE *gcas,
...
#define MIN_BIAS 750
  ...
  if (ctl_point_fname) {
    MRI3dUseFileControlPoints(mri_ctrl, ctl_point_fname);
    nctrl = MRInormAddFileControlPoints(mri_ctrl, CONTROL_MARKED, nullptr);
  } else {
    nctrl = 0;
  }
  ...
  for (n = 0; n < nsamples; n++) {
    ...
    yv = gcas[n].y;
    zv = gcas[n].z;
    if (xv >= 0 && xv <= mri_dst->width - 1 && yv >= 0 &&
        yv <= mri_dst->height - 1 && zv >= 0 && zv <= mri_dst->depth - 1)
#endif
    {
      if (xv == 181 && yv == 146 && zv == 128) {
        DiagBreak();
  ...
  if (nctrl == 0) // smooth it quite a bit
  {
    ...
    if (mri_bias->width != mri_in->width ||
      ...
      MRIextractInto(mri_bias, mri_tmp, 0, 0, 0, mri_bias->width,
                     mri_bias->height, mri_bias->depth, 0, 0, 0);
      MRIfree(&mri_bias);
      mri_bias = mri_tmp;
    }
    sigma = 2.0f;
  ...
  } else // if manually specified control points, don't let them be overwhelmed
  {
    ...
    mri_smooth = MRIsoapBubble(mri_bias, mri_ctrl, nullptr, 10, -1);
...
MRI *GCAnormalizeSamplesAllChannels(MRI *mri_in, GCA *gca, GCA_SAMPLE *gcas,
                                    int nsamples, TRANSFORM *transform,
                                    char *ctl_point_fname, double bias_sigma) {
  MRI *mri_dst;
  int  input;

  mri_dst = MRIcopy(mri_in, nullptr);

  if (gca->ninputs == 1 && mri_dst->nframes > 1) // normalize each frame
  {
    for (input = 0; input < mri_dst->nframes; input++) {
      MRI *mri_frame = MRIcopyFrame(mri_dst, nullptr, input, 0);
      GCAnormalizeSamplesOneChannel(mri_frame, gca, gcas, nsamples, transform,
                                    ctl_point_fname, 0, bias_sigma);
      MRIcopyFrame(mri_frame, mri_dst, 0, input);
      MRIfree(&mri_frame);
    }
  } else {
    for (input = 0; input < gca->ninputs; input++) {
      GCAnormalizeSamplesOneChannel(mri_dst, gca, gcas, nsamples, transform,
                                    ctl_point_fname, input, bias_sigma);
    }
  }
...
MRI *GCAnormalizeSamplesT1PD(MRI *mri_in, GCA *gca, GCA_SAMPLE *gcas,
                             int nsamples, TRANSFORM *transform,
                             char *ctl_point_fname) {
  MRI *mri_dst, *mri_ctrl, *mri_bias;
  int  xv, yv, zv, n, x, y, z, width, height, depth, out_val, xn, yn, zn, num,
      total;
  double val;
  float  bias;
  double mean, sigma;
...
float GCAlabelProbability(MRI *mri_src, GCA *gca, TRANSFORM *transform, float x,
                          float y, float z, int label) {
  int xn, yn, zn;
  // GCA_NODE *gcan;
  GC1D *gc;
...
#endif
MRI *GCAmaxPosteriorBorders(GCA *gca, MRI *mri_inputs, MRI *mri_src,
      ...
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          if (borderVoxel(mri_dst, x, y, z)) {
            label = GCAmaxLikelihoodBorderLabel(gca, mri_inputs, mri_dst,
                                                transform, x, y, z, min_ratio);
            if (x == Ggca_x && y == Ggca_y && z == Ggca_z &&
                (label == Ggca_label ||
...
static int GCAmaxLikelihoodBorderLabel(GCA *gca, MRI *mri_inputs,
  ...
  if (best_label != nint(MRIgetVoxVal(mri_labels, x, y, z, 0))) {
    label = nint(MRIgetVoxVal(mri_labels, x, y, z, 0));
    MRIsetVoxVal(mri_labels, x, y, z, 0,
                 best_label); /* test potential new label */
    if (gcaGibbsImpossibleConfiguration(gca, mri_labels, x, y, z, transform)) {
...
int GCAcomputeLabelStats(GCA *gca, int target_label, float *pvar,
                         float *means) {
  int       x, y, z, n, r;
      ...
      for (z = 0; z < gca->node_depth; z++) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          if (gcan->labels[n] == target_label) {
            ...
            if (fval != 0) {
              dof = get_node_prior(gca, target_label, x, y, z) *
                    gcan->total_training;
              for (r = 0; r < gca->ninputs; r++) {
...
int GCAhistogramTissueStatistics(GCA *gca, MRI *mri_T1, MRI *mri_PD,
                                 MRI *mri_labeled, TRANSFORM *transform,
                                 const char *fname) {
  int               x, y, z, n, label, biggest_label, T1, PD, xp, yp, zp;
  GCA_NODE *        gcan;
  GCA_TISSUE_PARMS *gca_tp;
  VECTOR *          v_parc, *v_T1;
  static VECTOR *   v_ras_cor = nullptr, *v_ras_flash;
  FILE *            fp;
  float             xf, yf, zf;
      ...
      for (y = 0; y < mri_T1->height; y++) {
        ...
        for (x = 0; x < mri_T1->width; x++) {
          ...
          if (transform) {
            ...
            TransformSample(transform, x * mri_T1->xsize, y * mri_T1->ysize,
                            z * mri_T1->zsize, &xf, &yf, &zf);
            xp         = nint(xf);
            yp         = nint(zf);
            zp         = nint(zf);
            V3_X(v_T1) = xp;
...
MRI *GCArelabel_cortical_gray_and_white(GCA *gca, MRI *mri_inputs, MRI *mri_src,
                                        MRI *mri_dst, TRANSFORM *transform) {
  int nchanged, x, y, z, width, height, depth, total_changed, label, xn, yn, zn,
      left, new_wm, new_gray;
  MRI *mri_tmp;
  // GCA_NODE *gcan;
  GC1D *gc_gray, *gc_wm;
      ...
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          if (!GCAsourceVoxelToNode(gca, mri_dst, transform, x, y, z, &xn, &yn,
            ...
            gray_dist =
                sqrt(GCAmahDistIdentityCovariance(gc_gray, vals, gca->ninputs));
            wm_dist =
                sqrt(GCAmahDistIdentityCovariance(gc_wm, vals, gca->ninputs));

            // get the prior coordinate
            if (!GCAsourceVoxelToPrior(gca, mri_dst, transform, x, y, z, &xp,
                                       &yp, &zp)) {
              gcap = &gca->priors[xp][yp][zp];
              if (gcap == nullptr) {
                continue;
...
int GCAdump(GCA *gca, MRI *mri, int x, int y, int z, TRANSFORM *transform,
            FILE *fp, int verbose) {
...
static GCA_SAMPLE *gcaExtractThresholdedRegionLabelAsSamples(
    GCA *gca, MRI *mri_labeled, TRANSFORM *transform, int *pnsamples, int label,
    int xp, int yp, int zp, int wsize, float pthresh) {
  ...
  TransformInvert(transform, mri_labeled);
  if (!GCApriorToSourceVoxel(gca, mri_labeled, transform, xp, yp, zp, &x, &y,
                             &z)) {
    for (nsamples = 0, zk = -whalf; zk <= whalf; zk++) {
      zi = mri_labeled->zi[z + zk];
      for (yk = -whalf; yk <= whalf; yk++) {
  ...
  for (i = 0, zk = -whalf; zk <= whalf; zk++) {
    ...
    for (yk = -whalf; yk <= whalf; yk++) {
      ...
      for (xk = -whalf; xk <= whalf; xk++) {
        ...
        if (!GCAsourceVoxelToPrior(gca, mri_labeled, transform, xi, yi, zi, &xp,
                                   &yp, &zp)) {
          gc = GCAfindPriorGC(gca, xp, yp, zp, label);
          if (gcap) {
  ...
  for (i = z = 0; z < depth; z++) {
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        if (!GCAsourceVoxelToPrior(gca, mri_labeled, transform, x, y, z, &xp,
                                   &yp, &zp)) {
          gcap = &gca->priors[xp][yp][zp];
          if (gcap == nullptr) {
            continue;
...
int GCArenormalize(MRI *mri_in, MRI *mri_labeled, GCA *gca,
                   TRANSFORM *transform) {
  int x, y, z, n, label, biggest_label, nsamples, val, *ordered_indices, i,
  ...
  if (!gca_means || !means || !stds)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %d vector", Progname,
              biggest_label + 1);
  ...
  if (!FZERO(nsamples)) {
    ...
    printf("scaling label %s by %2.2f (%2.2f / %2.2f) "
           "(%d samples, std=%2.1f)\n",
           cma_label_to_name(label), means[label] / gca_means[label],
  ...
  for (label = 1; label <= biggest_label; label++) {
    gcas =
        gcaExtractLabelAsSamples(gca, mri_labeled, transform, &nsamples, label);
    if (!nsamples) {
      continue;
    }
    ...
    for (var = mean = 0.0f, i = 0; i < nsamples; i++) {
      ...
      val =
          MRIgetVoxVal(mri_in, gcas[index].x, gcas[index].y, gcas[index].z, 0);
      mean += val;
      var += val * val;
    }
    mean /= (float)nsamples;
...
int GCArenormalizeAdaptive(MRI *mri_in, MRI *mri_labeled, GCA *gca,
                           TRANSFORM *transform, int wsize, float pthresh) {
...
#define MIN_SAMPLES 20
            ...
            do {
              gcas = gcaExtractThresholdedRegionLabelAsSamples(
                  gca, mri_labeled, transform, &nsamples, label, xp, yp, zp,
                  wsize, pthresh);
            ...
            for (frame = 0; frame < gca->ninputs; frame++) {
              ...
              if (IS_WM(label) && gca->ninputs == 1)
                peak =
                    HISTOfindLastPeakRelative(hsmooth, HISTO_WINDOW_SIZE, .3);
              else if (IS_LAT_VENT(label) && gca->ninputs == 1)
                peak =
                    HISTOfindFirstPeakRelative(hsmooth, HISTO_WINDOW_SIZE, .3);
              else
                peak = HISTOfindHighestPeakInRegion(
                    hsmooth, 0, (fmax - fmin) * hsmooth->bin_size);
              if (peak < 0) {
                continue;
              }
#if 0
...
int GCArenormalizeLabels(MRI *mri_in, MRI *mri_labeled, GCA *gca,
                         TRANSFORM *transform) {
  int x, y, z, n, label, biggest_label, nsamples, xv, yv, zv, *ordered_indices,
      i, index, width, height, depth;
  float       mean, var, val;
  GCA_NODE *  gcan;
  GCA_SAMPLE *gcas;
  GC1D *      gc;
  ...
  for (label = 1; label <= biggest_label; label++) {
    ...
    MRIsetResolution(mri_means, MEAN_RESOLUTION, MEAN_RESOLUTION,
                     MEAN_RESOLUTION);
    MRIsetResolution(mri_control, MEAN_RESOLUTION, MEAN_RESOLUTION,
                     MEAN_RESOLUTION);
    ...
    for (i = 0; i < nsamples; i++) {
      index = ordered_indices[i];
      if (!GCApriorToSourceVoxel(gca, mri_means, transform, gcas[index].xp,
                                 gcas[index].yp, gcas[index].zp, &x, &y, &z)) {
        val = MRIgetVoxVal(mri_in, gcas[index].x, gcas[index].y, gcas[index].z,
                           0);
        if (x == 19 && y == 14 && z == 15) {
          DiagBreak();
        }
        ...
        for (x = 0; x < width; x++) {
          ...
          for (n = 0; n < gcan->nlabels; n++) {
            if (gcan->labels[n] == label) {
              ...
              gc = &gcan->gcs[n];
              if (GCAnodeToVoxel(gca, mri_means, x, y, z, &xv, &yv, &zv) ==
                  NO_ERROR)
    ...
    for (yn = 0; yn < gca->node_height; yn++) {
      for (xn = 0; xn < gca->node_width; xn++) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          gc = &gcan->gcs[n];
          if (gc->means[0] < 1 &&
              gcan->labels[n] == 0) // too close to 0 to scale
          {
            ...
            if ((xn == Gx && yn == Gy && zn == Gz) &&
              ...
              printf("scaling gc for %s at (%d %d %d) from %2.1f --> %2.1f\n",
                     cma_label_to_name(label), xn, yn, zn, gc->means[0],
                     scales[i] * gc->means[0]);
...
int GCAmeanFilterConditionalDensities(GCA *gca, float navgs) {
  ...
  mri_means = MRIallocSequence(gca->node_width, gca->node_height,
                               gca->node_depth, MRI_FLOAT, gca->ninputs);
...
int    GCAhistoScaleImageIntensities(GCA *gca, MRI *mri, int noskull) {
  ...
  int   mri_peak, r, max_T1_weighted_image = 0, min_real_bin, peak, label,
                   left_wm, right_wm;
...
// on 08-01-05 by xhan
  ...
  mri_frame = MRIcopyFrame(mri, nullptr, max_T1_weighted_image, 0);
  mri_tmp   = MRImean(mri_frame, nullptr, 5);
  MRIfree(&mri_frame);
  mri_frame = mri_tmp;
  MRIvalRange(mri_frame, &fmin, &fmax);
  h_mri = MRIhistogram(mri_frame, nint(fmax - fmin + 1));
  HISTOclearZeroBin(h_mri); /* ignore background */
  h_smooth = HISTOsmooth(h_mri, nullptr, 2);
#if 0
...
#define MAX_PEAK_BINS 40
  ...
  printf("skull bounding box = (%d, %d, %d) --> (%d, %d, %d)\n", box.x, box.y,
         box.z, box.x + box.dx - 1, box.y + box.dy - 1, box.z + box.dz - 1);
  mri_mask = MRIbinarize(mri_frame, nullptr, min_real_val, 0, 1);
  MRIfree(&mri_frame);
  HISTOfree(&h_mri);
  HISTOfree(&h_smooth);
  ...
  mri_seg  = GCAbuildMostLikelyLabelVolume(gca, nullptr);
...
#endif
  for (r = 0; r < gca->ninputs; r++) {
    mri_frame = MRIcopyFrame(mri, nullptr, r, 0);
    MRImask(mri_frame, mri_mask, mri_frame, 0, 0); /* remove stuff that is
                                                      background or csf */

    printf("mean wm in atlas = %2.0f, using box (%d,%d,%d) --> (%d, %d,%d) "
           "to find MRI wm\n",
           wm_means[r], box.x, box.y, box.z, box.x + box.dx - 1,
           box.y + box.dy - 1, box.z + box.dz - 1);

    h_mri = MRIhistogramRegion(mri_frame, 0, nullptr, &box);
    if (h_mri == nullptr)
      return (Gerror);
    if (gca->ninputs == 1) {
...
#define MAX_CONFORMED_WM_VAL 240 // assume wm than this
    ...
    }
    {
      ...
      if (getenv("FS_HISTO_STD_THRESH")) {
        ...
        std_thresh = atof(cp);
        printf("FS_HISTO_STD_THRESH found in the environment resetting from 10 "
               "to %2.1f\n",
               std_thresh);
      }
      if (mri->xsize < .9)
...
int GCAhistoScaleImageIntensitiesLongitudinal(GCA *gca, MRI *mri, int noskull) {
  ...
  for (frame = 0; frame < mri->nframes; frame++) {
    mri_tmp   = MRIcopyFrame(mri, nullptr, frame, 0);
    mri_frame = MRImean(mri_tmp, nullptr, 5);
    MRIvalRange(mri_frame, &fmin, &fmax);
    h_mri = MRIhistogram(mri_frame, nint(fmax - fmin + 1));
    HISTOclearZeroBin(h_mri); /* ignore background */
    h_smooth     = HISTOsmooth(h_mri, nullptr, 2);
...
#define MAX_PEAK_BINS 40
    ...
    printf("skull bounding box = (%d, %d, %d) --> (%d, %d, %d)\n", tbox.x,
           tbox.y, tbox.z, tbox.x + tbox.dx - 1, tbox.y + tbox.dy - 1,
           tbox.z + tbox.dz - 1);
    box.x += tbox.x;
    box.y += tbox.y;
    box.z += tbox.z;
  ...
  mri_tmp   = MRIcopyFrame(mri, nullptr, 0, 0);
  mri_frame = MRImean(mri_tmp, nullptr, 5);
  mri_mask  = MRIbinarize(mri_frame, nullptr, min_real_val, 0, 1);
  MRIfree(&mri_frame);
  MRIfree(&mri_tmp);
  ...
  for (x = 0; x < gca->node_width; x++) {
    for (y = 0; y < gca->node_height; y++) {
      for (z = 0; z < gca->node_depth; z++) {
        ...
        if (gcan->nlabels == 0 ||
            (gcan->nlabels == 1 &&
             GCAfindGC(gca, x, y, z, Unknown) != nullptr)) {
          continue;
        }
        counts[gcan->nlabels]++;
      }
  ...
  for (z = 0; z < depth; z++) {
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        if (label > MAX_CMA_LABEL) {
          ErrorPrintf(ERROR_BADPARM, "GCArenormalizeToExample: bad label %d",
                      label);
          continue;
        }
...
static GC1D *findGCInWindow(GCA *gca, int x0, int y0, int z0, int label,
                            int wsize) {
  GC1D *    gc, *gc_min;
  int       x, y, z, n, whalf;
...
GC1D *GCAfindClosestValidGC(GCA *gca, int x0, int y0, int z0, int label,
                            int check_var) {
  GC1D *         gc, *gc_min;
  int            x, y, z, n, wsize;
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          for (v = r = 0; r < gca->ninputs; r++) {
            if (gcan->labels[n] > 1 && gc->means[0] < 1 && gc->ntraining > 10)
              DiagBreak();
            for (c = r; c < gca->ninputs; c++, v++)
              if (!check_finite("gcaCheckN1", gc->means[r]) ||
                  !check_finite("gcaCheckN2", gc->covars[v])) {
                ret = ERROR_BADPARM;
                DiagBreak();
              }
          }
...
int GCAcomputeVoxelLikelihoods(GCA *gca, MRI *mri_in, int x, int y, int z,
                               TRANSFORM *transform, int *labels,
                               double *likelihoods) {
  GCA_NODE *gcan;
  int       xn, yn, zn, n;
  float     vals[MAX_GCA_INPUTS];
  ...
  if (!GCAsourceVoxelToNode(gca, mri_in, transform, x, y, z, &xn, &yn, &zn)) {
    ...
    for (n = 0; n < gcan->nlabels; n++) {
      ...
      likelihoods[n] = GCAcomputeConditionalDensity(&gcan->gcs[n], vals,
...
int GCAmaxLikelihoodLabel(const GCA_NODE *gcan, float *vals, int ninputs,
                          float *plikelihood) {
  double p, max_p;
  int    n, best_label;
  ...
  max_p      = GCAcomputeConditionalDensity(&gcan->gcs[0], vals, ninputs,
                                       gcan->labels[0]);

  for (n = 1; n < gcan->nlabels; n++) {
    p = GCAcomputeConditionalDensity(&gcan->gcs[n], vals, ninputs,
                                     gcan->labels[n]);
    if (p >= max_p) {
      max_p      = p;
...
double GCAcomputeConditionalDensity(GC1D *gc, float *vals, int ninputs,
                                    int label) {
  double p, dist;
...
/* compute 1-d Mahalanobis distance */
...
#endif
  {
    ...
    p    = (1.0 / (pow(2 * M_PI, ninputs / 2.0) *
                sqrt(covariance_determinant(gc, ninputs)))) *
        exp(-0.5 * dist);
  }
  return (p);
}
double GCAcomputeNormalizedConditionalDensity(GCA *gca, int xp, int yp, int zp,
                                              float *vals, int label) {
  GCA_PRIOR *gcap;
  GC1D *     gc;
  int        n;
...
double GCAcomputeConditionalLogDensity(const GC1D *gc, float *vals, int ninputs,
                                       int label) {
  double log_p, det;
...
int GCAcomputeMAPlabelAtLocation(GCA *gca, int xp, int yp, int zp, float *vals,
  ...
  if (gc)
    max_log_p = gcaComputeLogDensity(gc, vals, gca->ninputs, gcap->priors[0],
                                     max_label);
  else {
    max_log_p = -100000;
  }
  ...
  for (n = 1; n < gcap->nlabels; n++) {
    ...
    // calculate log_p
    log_p = gcaComputeLogDensity(gc, vals, gca->ninputs, gcap->priors[n],
                                 gcap->labels[n]);
    if (log_p > max_log_p) {
      max_log_p = log_p;
      max_n     = n;
...
int GCAcomputeMLElabelAtLocation(GCA *gca, int xp, int yp, int zp, float *vals,
                                 int *pmax_n, float *plog_p) {
  GCA_PRIOR *gcap;
  GC1D *     gc;
  int        n, max_n, max_label;
  ...
  if (gc)
    max_log_p =
        GCAcomputeConditionalLogDensity(gc, vals, gca->ninputs, max_label);
  else {
    max_log_p = -100000;
  }
  for (n = 1; n < gcap->nlabels; n++) {
...
GC1D *alloc_gcs(int nlabels, int flags, int ninputs) {
  ...
  for (i = 0; i < nlabels; i++) {
    ...
    gcs[i].covars =
        (float *)calloc((ninputs * (ninputs + 1)) / 2, sizeof(float));
  ...
  for (i = 0; i < nlabels; i++) {
    ...
    if (!gcs[i].nlabels || !gcs[i].labels || !gcs[i].label_priors)
      ErrorExit(ERROR_NOMEMORY,
                "alloc_gcs(%d, %x): could not allocated %d gcs(%d)", nlabels,
                flags, nlabels, i);
  }
  return (gcs);
}
...
int copy_gcs(int nlabels, GC1D *gcs_src, GC1D *gcs_dst, int ninputs) {
  ...
  for (i = 0; i < nlabels; i++) {
    ...
    if (gcs_dst[i].nlabels == nullptr) /* NO_MRF flag must be set */
    {
      continue;
    }
    for (j = 0; j < GIBBS_NEIGHBORHOOD; j++) {
      ...
      if (!gcs_dst[i].label_priors[j]) {
        ...
        gcs_dst[i].labels[j] = (unsigned short *)calloc(gcs_src[i].nlabels[j],
                                                        sizeof(unsigned short));
...
int GCAcomputeSampleCoords(GCA *gca, MRI *mri, GCA_SAMPLE *gcas, int nsamples,
                           TRANSFORM *transform) {
  int n, x, y, z;
  ...
  for (n = 0; n < nsamples; n++) {
    ...
    if (!GCApriorToSourceVoxel(gca, mri, transform, gcas[n].xp, gcas[n].yp,
                               gcas[n].zp, &x, &y, &z)) {
      gcas[n].x = x;
      gcas[n].y = y;
      gcas[n].z = z;
      if (DIAG_VERBOSE_ON && Gdiag & DIAG_SHOW)
        printf("label %d: (%d, %d, %d) <-- (%d, %d, %d)\n", gcas[n].label,
               gcas[n].xp, gcas[n].yp, gcas[n].zp, x, y, z);
    }
  }
  return (NO_ERROR);
}
...
void load_vals(const MRI *mri_inputs, float x, float y, float z, float *vals,
               int ninputs) {
...
static void load_vals_xyzInt(const MRI *mri_inputs, int x, int y, int z,
                             float *vals, int ninputs) {
  MRIsampleVolumeFrame_xyzInt_nRange_floats(mri_inputs, x, y, z, 0, ninputs,
                                            vals);
}
#endif

double GCAmahDist(const GC1D *gc, const float *vals, const int ninputs) {
  static VECTOR *v_means = nullptr, *v_vals = nullptr;
  static MATRIX *m_cov = nullptr, *m_cov_inv;
  int            i;
  double         dsq;
...
double GCAcomputePosteriorDensity(GCA_PRIOR *gcap, GCA_NODE *gcan, int node_n,
                                  int prior_n, float *vals, int ninputs, int xn,
                                  int yn, int zn, GCA *gca) {
  GC1D * gc;
  double p;
  ...
  } else // n refers to gcap index
  {
    ...
    if (gc == nullptr) {
      ...
      if (gc == nullptr) {
        return ((gcap->total_training == 0
                     ? 1e-10
                     : 1.0 / (10 * gcap->total_training)));
      }
    }
...
MATRIX *load_covariance_matrix(const GC1D *gc, MATRIX *m_cov,
                               const int ninputs) {
  int n, m, i;
...
MATRIX *load_inverse_covariance_matrix(GC1D *gc, MATRIX *m_inv_cov,
                                       int ninputs) {
  static MATRIX *m_cov[_MAX_FS_THREADS] = {nullptr};
#ifdef HAVE_OPENMP
...
static double gcaComputeSampleLogDensity(GCA_SAMPLE *gcas, float *vals,
  ...
  log_p =
      gcaComputeSampleConditionalLogDensity(gcas, vals, ninputs, gcas->label);
...
#endif
int GCAlabelExists(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y,
                   int z, int label) {
#if 0
  int  xn, yn, zn ;
...
GC1D *GCAfindSourceGC(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y,
                      int z, int label) {
  int   xn, yn, zn;
  GC1D *gc = nullptr;
  ...
  for (nvals = x = 0; x < mri_inputs->width; x++) {
    for (y = 0; y < mri_inputs->height; y++) {
      for (z = 0; z < mri_inputs->depth; z++) {
        ...
        for (label = brain_node = 0, n = 0; !brain_node && n < gcap->nlabels;
             n++)
        ...
        if (gc) {
          MRIsampleVolumeFrameType(mri_inputs, x, y, z, 1, SAMPLE_NEAREST,
                                   &val);
          nvals++;
          mri_PD += val;
          gca_PD += gc->means[1];
...
GCA *GCAcreateWeightedFlashGCAfromParameterGCA(GCA *gca_T1PD, double *TR,
                                               double *fa, double *TE,
  ...
  GCA_NODE * gcan_src, *gcan_dst;
  GC1D *     gc_src, *gc_dst;
  MATRIX *   m_jacobian, *m_cov_src = nullptr, *m_cov_dst = nullptr,
                      *m_jacobian_T = nullptr, *m_tmp = nullptr;
  ...
  if (gca_T1PD->ninputs != 2)
    ErrorExit(ERROR_BADPARM,
              ...
              gca_T1PD->ninputs);
  gca_flash =
      GCAalloc(nflash, gca_T1PD->prior_spacing, gca_T1PD->node_spacing,
  ...
  v_wts_T = VectorTranspose(v_wts, nullptr);
    ...
    for (y = 0; y < gca_flash->prior_height; y++) {
      for (z = 0; z < gca_flash->prior_depth; z++) {
        ...
        if (gcap_src->nlabels > gcap_dst->max_labels) {
          ...
          gcap_dst->labels = (unsigned short *)calloc(gcap_src->nlabels,
                                                      sizeof(unsigned short));
          if (!gcap_dst->labels)
            ErrorExit(ERROR_NOMEMORY,
                      "GCAcreateWeightedFlashGCAfromParameterGCA:"
    ...
    for (y = 0; y < gca_flash->node_height; y++) {
      for (z = 0; z < gca_flash->node_depth; z++) {
        ...
        if (gcan_src->nlabels > gcan_dst->max_labels) {
          ...
          gcan_dst->labels = (unsigned short *)calloc(gcan_src->nlabels,
                                                      sizeof(unsigned short));
          if (!gcan_dst->labels)
            ErrorExit(ERROR_NOMEMORY,
                      "GCAcreateWeightedFlashGCAfromParameterGCA:"
          ...
          for (i = 0; i < GIBBS_NEIGHBORS; i++) {
            ...
            if (!gc_dst->label_priors[i])
              ErrorExit(ERROR_NOMEMORY,
                        ...
                        "to %d",
                        gc_src->nlabels[i]);
            gc_dst->labels[i] = (unsigned short *)calloc(
                gc_src->nlabels[i], sizeof(unsigned short));
            if (!gc_dst->labels)
              ErrorExit(ERROR_NOMEMORY,
                        "GCAcreateWeightedFlashGCAfromParameterGCA:"
                        " to %d",
          ...
          for (i = 0; i < gca_flash->ninputs; i++) {
            ...
            if (x == Ggca_x && y == Ggca_y && z == Ggca_z &&
              ...
              printf("gcan(%d, %d, %d) %s: image[%d] "
                     ...
                     "(%2.1f,%2.1f) --> %2.1f\n",
                     x, y, z, cma_label_to_name(gcan_dst->labels[n]), i,
                     DEGREES(fa[i]), T1, PD, gc_dst->means[i]);
...
#define MIN_T1 50
          if (T1 < MIN_T1) {
            ...
            m_cov_src =
                load_covariance_matrix(gc_src, m_cov_src, gca_T1PD->ninputs);
  ...
  if (Ggca_label >= 0) {
    printf("label %s (%d): means = ", cma_label_to_name(Ggca_label),
           Ggca_label);
    for (i = 0; i < gca_flash->ninputs; i++) {
...
GCA *GCAcreateFlashGCAfromParameterGCA(GCA *gca_T1PD, double *TR, double *fa,
                                       double *TE, int nflash, double lambda) {
  GCA *      gca_flash;
  GCA_PRIOR *gcap_src, *gcap_dst;
  GCA_NODE * gcan_src, *gcan_dst;
  GC1D *     gc_src, *gc_dst;
    ...
    for (y = 0; y < gca->node_height; y++) {
      for (z = 0; z < gca->node_depth; z++) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          gc        = &gcan->gcs[n];
          det       = covariance_determinant(gc, gca->ninputs);
          m_cov_inv = load_inverse_covariance_matrix(gc, nullptr, gca->ninputs);
          ...
          } else /* not singular - check if it is ill-conditioned */
          {
            ...
            if ((gc->ntraining * gca->ninputs < 2 * nparams && det < 0.1) ||
                (det < min_det)) {
              gc->regularized = 1;
              regularized++;
              for (i = r = 0; r < gca->ninputs; r++) {
...
GCA *GCAcreateFlashGCAfromFlashGCA(GCA *gca_flash_src, double *TR, double *fa,
                                   double *TE, int nflash) {
  GCA *      gca_flash_dst;
  GCA_PRIOR *gcap_src, *gcap_dst;
  GCA_NODE * gcan_src, *gcan_dst;
  GC1D *     gc_src, *gc_dst;
  MATRIX *   m_jac_dst, *m_jac_src, *m_cov_src, *m_cov_dst, *m_pinv_src,
      *m_cov_T1PD;
  int    n, x, y, z, i, j, v, label_count = 0;
  double T1, PD, label_means[MAX_GCA_INPUTS];

  m_cov_T1PD = m_cov_dst = m_pinv_src = m_cov_src = nullptr;
  FlashBuildLookupTables(gca_flash_src->ninputs, gca_flash_src->TRs,
                         gca_flash_src->FAs, gca_flash_src->TEs);

  gca_flash_dst = GCAalloc(
      nflash, gca_flash_src->prior_spacing, gca_flash_src->node_spacing,
        ...
        for (n = 0; n < gcan_src->nlabels; n++) {
          ...
          for (i = 0; i < GIBBS_NEIGHBORS; i++) {
            ...
            gc_dst->label_priors[i] =
                (float *)calloc(gc_src->nlabels[i], sizeof(float));
            if (!gc_dst->label_priors[i])
              ErrorExit(ERROR_NOMEMORY, "GCAcreateFlashGCAfromFlashGCA: to %d",
                        gc_src->nlabels[i]);
            gc_dst->labels[i] = (unsigned short *)calloc(
                gc_src->nlabels[i], sizeof(unsigned short));
            if (!gc_dst->labels)
          ...
          /* now map intensity and covariance info over */
          compute_T1_PD(gca_flash_src->ninputs, gc_src->means,
                        gca_flash_src->TRs, gca_flash_src->FAs,
                        gca_flash_src->TEs, &T1, &PD);
          T1 = MAX(T1, 10);
          PD = MAX(PD, 0);
          if (x == Ggca_x && y == Ggca_y && z == Ggca_z &&
              (Ggca_label < 0 || Ggca_label == gcan_src->labels[n])) {
            int j;
            printf("gcan(%d, %d, %d) %s: means=[", x, y, z,
                   cma_label_to_name(gcan_src->labels[n]));
          ...
          for (i = 0; i < gca_flash_dst->ninputs; i++) {
            ...
            *MATRIX_RELT(m_jac_dst, i + 1, 1) =
                dFlash_dT1(T1, PD, gca_flash_dst->TRs[i], gca_flash_dst->FAs[i],
                           gca_flash_dst->TEs[i]);
            *MATRIX_RELT(m_jac_dst, i + 1, 2) =
                dFlash_dPD(T1, PD, gca_flash_dst->TRs[i], gca_flash_dst->FAs[i],
                           gca_flash_dst->TEs[i]);
          ...
          for (i = 0; i < gca_flash_src->ninputs; i++) {
            *MATRIX_RELT(m_jac_src, i + 1, 1) =
                dFlash_dT1(T1, PD, gca_flash_src->TRs[i], gca_flash_src->FAs[i],
                           gca_flash_src->TEs[i]);
            *MATRIX_RELT(m_jac_src, i + 1, 2) =
                dFlash_dPD(T1, PD, gca_flash_src->TRs[i], gca_flash_src->FAs[i],
                           gca_flash_src->TEs[i]);
          }
...
#define MIN_T1 50
          ...
          if (T1 < MIN_T1)
            ...
            m_cov_src  = load_covariance_matrix(gc_src, m_cov_src,
                                               gca_flash_src->ninputs);
            m_pinv_src = MatrixPseudoInverse(m_jac_src, m_pinv_src);
            m_cov_T1PD =
                MatrixSimilarityTransform(m_cov_src, m_pinv_src, m_cov_T1PD);
            m_cov_dst =
                MatrixSimilarityTransform(m_cov_T1PD, m_jac_dst, m_cov_dst);
          }
      ...
      for (xn = 0; xn < gca->node_width; xn++) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          if (prior != 0) {
            ...
            if (m_total == nullptr) {
              m_total = MatrixCopy(m_cov, nullptr);
            } else {
...
int GCAlabelMeanFromImage(GCA *gca, TRANSFORM *transform, MRI *mri, int label,
                          float *means) {
  ...
  for (wt = 0.0, zn = 0; zn < gca->node_depth; zn++) {
    for (yn = 0; yn < gca->node_height; yn++) {
      for (xn = 0; xn < gca->node_width; xn++) {
        gcan = &gca->nodes[xn][yn][zn];
        if (GCAnodeToSourceVoxel(gca, mri, transform, xn, yn, zn, &xv, &yv,
                                 &zv) == NO_ERROR) {
          for (n = 0; n < gcan->nlabels; n++) {
            /* find index in lookup table for this label */
            if (gcan->labels[n] != label) {
...
static int gm_labels[] = {
    Left_Hippocampus, Right_Hippocampus, Left_Amygdala, Right_Amygdala,
    //    Left_Caudate,
    //    Right_Caudate,
...
static int wm_labels[] = {Left_Cerebral_White_Matter,
                          Right_Cerebral_White_Matter};
#define NWM_LABELS (sizeof(wm_labels) / sizeof(wm_labels[0]))

static int csf_labels[] = {Left_Lateral_Ventricle, Right_Lateral_Ventricle,
...
int GCAmapRenormalizeWithAlignment(GCA *gca, MRI *mri, TRANSFORM *transform,
                                   ...
                                   LTA **plta, int handle_expanded_ventricles) {
  return (GCAcomputeRenormalizationWithAlignment(
      gca, mri, transform, logfp, base_name, plta, handle_expanded_ventricles,
      nullptr, nullptr, nullptr, nullptr));
}

int GCAmapRenormalizeWithAlignmentLongitudinal(
    GCA *gca, MRI *mri, TRANSFORM *transform, FILE *logfp,
    const char *base_name, LTA **plta, int handle_expanded_ventricles) {
...
int GCAseqRenormalizeWithAlignment(GCA *gca, MRI *mri, TRANSFORM *transform,
                                   FILE *logfp, const char *base_name,
                                   LTA **plta, int handle_expanded_ventricles,
  ...
  if (old_label_scales == nullptr || old_label_offsets == nullptr ||
      old_label_peaks == nullptr || old_label_computed == nullptr)
    ErrorExit(ERROR_BADPARM,
              "%s ERROR:  GCA seqRenormalize old_label needs to "
              "be set for sequential call.\n",
              Progname);

  float plabel_scales[MAX_CMA_LABELS], plabel_offsets[MAX_CMA_LABELS],
      plabel_peaks[MAX_CMA_LABELS];
  int plabel_computed[MAX_CMA_LABELS];
  int ret = GCAcomputeRenormalizationWithAlignment(
      gca, mri, transform, logfp, base_name, plta, handle_expanded_ventricles,
      plabel_scales, plabel_offsets, plabel_peaks, plabel_computed);
  ...
  for (l = 0; l < MAX_CMA_LABELS; l++) {
    if (plabel_computed[l] != 0) // we computed this label
    {
      if (old_label_computed[l] != 0) // merge offsets and scales
      {
        ...
        // plabel_scales[l]);
        old_label_offsets[l] =
            plabel_scales[l] * old_label_offsets[l] + plabel_offsets[l];
    ...
    for (l = 0; l < MAX_CMA_LABELS; l++)
      if (old_label_computed[l] != 0)
        fprintf(logfp, "label %s: scaling by %2.2f  + %2.1f to %2.0f\n",
                cma_label_to_name(l), old_label_scales[l], old_label_offsets[l],
                old_label_peaks[l]);
    fflush(logfp);
  }
  if (DIAG_VERBOSE_ON) {
  ...
  if (base_name) {
    ...
    fp = fopen(fname, "w");
    if (fp == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not open intensity tracking file %s",
                Progname, fname);

    for (l = 0; l < MAX_CMA_LABELS; l++)
      if (old_label_computed[l] != 0)
        fprintf(fp, "%d %s %2.2f %2.1f %2.0f\n", l, cma_label_to_name(l),
                old_label_scales[l], old_label_offsets[l], old_label_peaks[l]);

    fflush(fp);
    fclose(fp);
    if (getenv("EXIT_AFTER_INT") != nullptr)
      exit(0);
  }
  return ret;
}

static int mtl_labels[] = {Left_Amygdala, Right_Amygdala, Left_Hippocampus,
                           Right_Hippocampus};
...
MRI *MRImarkPossibleBorders(MRI *mri, MRI *mri_borders, float border_thresh) {
  ...
  if (mri_borders == nullptr) {
    mri_borders = MRIclone(mri, nullptr);
  }
...
static LABEL_HISTOS *lhAlloc(int nsubjects, int nlabels, int ntypes) {
  ...
  for (n = 0; n < lh->nlabels; n++) {
    lh->mri_histos[n] = HISTOinit(nullptr, 256, 0, 255);
  }
  lh->mri_histo = HISTOinit(nullptr, 256, 0, 255);
  for (s = 0; s < nsubjects; s++) {
    lh->subject_histos[s] = HISTOinit(nullptr, 256, 0, 255);
    lh->histos[s]         = (HISTOGRAM **)calloc(nlabels, sizeof(HISTOGRAM *));
    for (n = 0; n < lh->nlabels; n++) {
      lh->histos[s][n] = HISTOinit(nullptr, 256, 0, 255);
    }
...
static int lhComputeMRIHistos(LH *lh, MRI *mri_norm, int nerode) {
  ...
  mri_label  = MRIclone(mri_norm, nullptr);
  mri_eroded = MRIclone(mri_norm, nullptr);
#if 0
  for (n = 0 ; n < lh->nlabels ; n++)
  {
    MRIclear(mri_label) ;
...
int GCAmapRenormalizeWithHistograms(GCA *gca, MRI *mri, TRANSFORM *transform,
                                    FILE *logfp, const char *base_name,
  ...
  if (plabel_scales == nullptr) {
    plabel_scales = label_scales;
  }
  if (plabel_offsets == nullptr) {
    plabel_offsets = label_offsets;
  }
  if (plabel_peaks == nullptr) {
  ...
  for (frame = 0; frame < mri->nframes; frame++) {
    ...
    for (j = 0; j < lh->nlabels; j++) {
      ...
      for (avg_scale = avg_offset = 0.0, num = s = 0; s < lh->nsubjects; s++) {
        ...
        num++;
        HISTOfindLinearFit(h_gca, lh->histos[s][j], .025, 4, 0, 0, &scale,
                           &offset);
        avg_scale += scale;
        avg_offset += offset;
      }
      ...
      if (gca_peak >= 0) {
        printf("%s: gca peak = %2.5f (%2.0f), scaling to %2.1f\n",
               cma_label_to_name(l), h_gca->counts[gca_peak],
               h_gca->bins[gca_peak],
...
int GCAcomputeRenormalizationWithAlignment(
    GCA *gca, MRI *mri, TRANSFORM *transform, FILE *logfp,
    const char *base_name, LTA **plta, int handle_expanded_ventricles,
    float *plabel_scales, float *plabel_offsets, float *plabel_peaks,
    int *plabel_computed) {
  HISTOGRAM *  h_mri, *h_gca, *h_mtl = nullptr, *h_caudate;
  unsigned int k, j;
  int l, nbins, i, x, y, z, num, frame, bin, n, computed[MAX_CMA_LABELS], b,
      label, border = BORDER_SIZE, gca_peak, mri_peak;
  float fmin, fmax, label_scales[MAX_CMA_LABELS], overlap, mean_gm_scale,
      mean_wm_scale, mean_csf_scale, label_peaks[MAX_CMA_LABELS],
      label_offsets[MAX_CMA_LABELS], mean_wm_offset, mean_csf_offset,
      mean_gm_offset, lower_thresh, upper_thresh;
  double  val /*, scale*/;
  MRI *   mri_seg = nullptr, *mri_aligned, *mri_labels = nullptr, *mri_borders;
  char    fname[STRLEN];
  MATRIX *m_L, *m_by_label[MAX_CMA_LABELS];
  LTA *   lta;
  ...
  mri_borders = MRImarkPossibleBorders(mri, nullptr, 15);
  for (frame = 0; frame < mri->nframes; frame++) {
    for (l = 0; l < MAX_CMA_LABELS; l++) {
      if (l == Gdiag_no) {
        DiagBreak();
      ...
      label_offsets[l] = 0.0;
      computed[l]      = 0;
      m_by_label[l]    = nullptr; // not estimated yet
    }
    ...
    for (j = 0; j < NALIGN_LABELS; j++) {
      ...
      */
      if (transform->type != MORPH_3D_TYPE) {
        if (IS_HIPPO(l) || IS_AMYGDALA(l) || IS_CAUDATE(l) || IS_PUTAMEN(l) ||
            IS_PALLIDUM(l)) {
          border = BORDER_SIZE + 1; // need more context for hippo
        } else if (IS_GM(l)) {
          border = 0;
        ...
        GCAbuildMostLikelyVolumeForStructure(gca, mri_seg, l, border, transform,
                                             mri_labels);
        for (x = 0; x < mri_labels->width; x++) {
          for (y = 0; y < mri_labels->height; y++) {
            for (z = 0; z < mri_labels->depth; z++) {
        ...
        */
        ...
        if (l == Left_Cerebral_White_Matter ||
          ...
          GCAbuildMostLikelyVolumeForStructure(gca, mri_tmp, l, BORDER_SIZE,
                                               transform, nullptr);
          mri_border = MRIsubtract(mri_seg, mri_tmp, nullptr);
          // just outside
        ...
        if (n < 0) // no transform - compute one
        {
          ...
          if (!IS_GM(l)) {
            ...
            if ((l == Left_Lateral_Ventricle || l == Right_Lateral_Ventricle) &&
              ...
              initialize_ventricle_alignment(mri_seg, mri, m_L, label_base_name,
                                             border, l);
            }
        ...
        {
          m_L = MatrixCopy(lta->xforms[n].m_L, nullptr);
        }
        ...
        if (Gdiag & DIAG_WRITE) {
          sprintf(fname, "%s_label%d_after.mgz", base_name, l);
          mri_aligned = MRIlinearTransform(mri_seg, nullptr, m_L);
          MRIwrite(mri_aligned, fname);
          MRIfree(&mri_aligned);
        }
      ...
      } else // 3d morph already done - don't bother aligning
      {
        ...
        mri_aligned = MRIerode(mri_seg, nullptr);
      }
      ...
      for (num = x = 0; x < mri_aligned->width; x++) {
        for (y = 0; y < mri_aligned->height; y++) {
          for (z = 0; z < mri_aligned->depth; z++) {
            ...
            if (MRIgetVoxVal(mri_borders, x, y, z, 0) >
                0) // avoid border voxels - matches erosion
            {
              continue;
            }
            bin = nint((val - fmin) / h_mri->bin_size);
      ...
      if (l == Left_Hippocampus || l == Right_Hippocampus ||
          l == Left_Amygdala || l == Right_Amygdala) {
        h_mtl->bin_size = 1;
        h_mtl->min      = 0;
        h_mtl->max      = 255;
        for (i = 0; i < nbins; i++) {
      ...
      if (IS_LAT_VENT(l) || overlap > overlap_threshold) {
        ...
        HISTOfindLinearFit(h_gca, h_mri, .025, 4, 0, 0, &label_scales[l],
                           &label_offsets[l]);
        ...
        if ((val < lower_thresh || val > upper_thresh) ||
          ...
          //          if (transform->type != MORPH_3D_TYPE)
          {
            printf("%s: unreasonable value (%2.1f/%2.1f), "
                   ...
                   cma_label_to_name(l), val, h_mri->bins[mri_peak],
        ...
        printf("%s (%d): linear fit = %2.2f x + %2.1f "
               "(%d voxels, overlap=%2.3f)\n",
               cma_label_to_name(l), l, label_scales[l], label_offsets[l], num,
               overlap);
        ...
        if ((label_scales[l] < 0.67 || (label_scales[l] > 1.5)) &&
            !IS_LAT_VENT(l)) {
          /*
           if(IS_CSF(l)){
    ...
    if (getenv("FS_USE_HISTO_COMBOS") != nullptr) {
      unsigned int j;
      ...
      for (j = 0; j <= 1; j++) {
        ...
        HISTOmakePDF(h_gca, h_gca);
        label_peaks[l] = h_gca->bins[gca_peak];
        HISTOfindLinearFit(h_gca, h_caudate, .025, 4, 0, 0, &label_scales[l],
                           &label_offsets[l]);
        val = h_gca->bins[gca_peak] * label_scales[l] + label_offsets[l];
        lower_thresh = 50;
        upper_thresh = 100;
        if ((val < lower_thresh || val > upper_thresh) ||
            (h_caudate->bins[mri_peak] < lower_thresh ||
             h_caudate->bins[mri_peak] > upper_thresh)) {
          printf("%s: unreasonable value (%2.1f/%2.1f), "
                 "not in range [%2.0f, %2.0f] - rejecting\n",
                 cma_label_to_name(l), val, h_caudate->bins[mri_peak],
                 lower_thresh, upper_thresh);
          label_scales[l]  = 1.0;
          label_offsets[l] = 1.0;
        } else {
      ...
      for (j = 0; j < MTL_LABELS; j++) {
        ...
        HISTOmakePDF(h_gca, h_gca);
        label_peaks[l] = h_gca->bins[gca_peak];
        HISTOfindLinearFit(h_gca, h_mtl, .025, 4, 0, 0, &label_scales[l],
                           &label_offsets[l]);
        val = h_gca->bins[gca_peak] * label_scales[l] + label_offsets[l];
        lower_thresh = 50;
        upper_thresh = 90;
        if ((val < lower_thresh || val > upper_thresh) ||
            (h_mtl->bins[mri_peak] < lower_thresh ||
             h_mtl->bins[mri_peak] > upper_thresh)) {
          printf("%s: unreasonable value (%2.1f/%2.1f), "
                 "not in range [%2.0f, %2.0f] - rejecting\n",
                 cma_label_to_name(l), val, h_mtl->bins[mri_peak], lower_thresh,
                 upper_thresh);
          label_scales[l]  = 1.0;
          label_offsets[l] = 1.0;
        } else {
    ...
    if (computed[Left_Cerebral_Cortex] == 0 &&
      ...
      if (m_by_label[Left_Cerebral_White_Matter])
        m_by_label[Left_Cerebral_Cortex] =
            MatrixCopy(m_by_label[Left_Cerebral_White_Matter], nullptr);
      label_scales[Left_Cerebral_Cortex]  = mean_gm_scale;
      label_offsets[Left_Cerebral_Cortex] = mean_gm_offset;
      computed[Left_Cerebral_Cortex]      = 1;
    ...
    if (computed[Right_Cerebral_Cortex] == 0 &&
      ...
      if (m_by_label[Right_Cerebral_White_Matter])
        m_by_label[Right_Cerebral_Cortex] =
            MatrixCopy(m_by_label[Right_Cerebral_White_Matter], nullptr);
      label_scales[Right_Cerebral_Cortex]  = mean_gm_scale;
      label_offsets[Right_Cerebral_Cortex] = mean_gm_offset;
...
#define NUM_EIG 5
    ...
    for (xn = 0; xn < gca->node_width; xn++) {
      ...
      for (yn = 0; yn < gca->node_height; yn++) {
        for (zn = 0; zn < gca->node_depth; zn++) {
          ...
          qsort(ranks_before, gcan->nlabels, sizeof(LABEL_PROB),
                compare_sort_probabilities);
          niter = 0;
          for (i = 0; i < gcan->nlabels; i++) {
            gc = &gcan->gcs[i];
          ...
          qsort(ranks_after, gcan->nlabels, sizeof(LABEL_PROB),
                compare_sort_probabilities);
          for (i = 0; i < gcan->nlabels; i++) {
#if 1
            if (ranks_before[i].label != ranks_after[i].label) {
              ...
              delta_i =
                  (means_after[ind_i] - means_after[ind_j]) * (1 - lambda);
              ...
              ranks_after[j].prob = means_after[ind_j] =
                  means_after[ind_j] - delta_j;
              ranks_after[i].prob = means_after[ind_i] =
                  means_after[ind_i] - delta_i;
              if ((xn == Gx && yn == Gy && zn == Gz) &&
                  (ranks_after[i].label == gcan->labels[i] ||
                ...
                printf("ordering of labels %s and %s changed, "
                       ...
                       "and %2.0f (%2.1f)\n",
                       cma_label_to_name(ranks_after[i].label),
                       cma_label_to_name(ranks_after[j].label), means_after[i],
                       delta_i, means_after[j], delta_i);
              }
  ...
  if (plta) // return linear transform array to caller
  {
    ...
    // count # of xforms
    for (i = l = 0; l < MAX_CMA_LABELS; l++) {
      if (m_by_label[l] != nullptr && computed[l] == 2) {
        i++;
      }
    }
...
/*
  ...
  frame is the same modality but a different time point of the same subject.
*/
int GCAcomputeRenormalizationWithAlignmentLongitudinal(
    GCA *gca, MRI *mri, TRANSFORM *transform, FILE *logfp,
    const char *base_name, LTA **plta, int handle_expanded_ventricles,
    float *plabel_scales, float *plabel_offsets, float *plabel_peaks,
    int *plabel_computed) {
  HISTOGRAM *  h_mri, *h_gca, *h_mtl = nullptr, *h_caudate;
  unsigned int k, j;
  int l, nbins, i, x, y, z, num, frame, bin, computed[MAX_CMA_LABELS], label,
      gca_peak, mri_peak;
  float fmin, fmax, label_scales[MAX_CMA_LABELS], overlap, mean_gm_scale,
      mean_wm_scale, mean_csf_scale, label_peaks[MAX_CMA_LABELS],
      label_offsets[MAX_CMA_LABELS], mean_wm_offset, mean_csf_offset,
  ...
  for (j = 0; j < NALIGN_LABELS; j++) {
    ...
    */
    ...
    if (l == Left_Hippocampus || l == Right_Hippocampus || l == Left_Amygdala ||
        l == Right_Amygdala) {
      h_mtl->bin_size = 1;
      h_mtl->min      = 0;
      h_mtl->max      = 255;
    ...
    if (IS_LAT_VENT(l) || overlap > overlap_threshold) {
      ...
      if ((val < lower_thresh || val > upper_thresh) ||
        ...
        //          if (transform->type != MORPH_3D_TYPE)
        {
          printf("%s: unreasonable value (%2.1f/%2.1f), "
                 "not in range [%2.0f, %2.0f] - rejecting\n",
                 cma_label_to_name(l), val, h_mri->bins[mri_peak], lower_thresh,
                 upper_thresh);
          label_scales[l]  = 1.0;
          label_offsets[l] = 1.0;
          continue;
    ...
    for (j = 0; j <= 1; j++) {
      ...
      if ((val < lower_thresh || val > upper_thresh) ||
        ...
        fprintf(stdout,
                "%s (%d): linear fit = %2.2f x + "
                "%2.1f (peak = %2.0f), gca=%2.1f\n",
                cma_label_to_name(l), l, label_scales[l], label_offsets[l], val,
                label_peaks[l]);
        fflush(stdout);
        computed[l] = 1;
      }
  ...
  for (k = 0; k < NHEMI_LABELS; k++) {
    ...
    if (computed[lh_labels[k]] && !computed[rh_labels[k]]) {
      ...
      fprintf(stdout, "setting label %s based on %s = %2.2f x + %2.0f: %2.0f\n",
              cma_label_to_name(rhl), cma_label_to_name(lhl), label_scales[rhl],
              label_offsets[rhl], label_peaks[rhl]);
    } else if (computed[rh_labels[k]] && !computed[lh_labels[k]]) {
      lhl                = lh_labels[k];
      rhl                = rh_labels[k];
      label_scales[lhl]  = label_scales[rhl];
      ...
      computed[lhl]      = 1;
      fprintf(stdout, "setting label %s based on %s = %2.2f x + %2.0f: %2.0f\n",
              cma_label_to_name(lhl), cma_label_to_name(rhl), label_scales[lhl],
              label_offsets[lhl], label_peaks[lhl]);
    }
  }
...
int GCAapplyRenormalization(GCA *gca, float *label_scales, float *label_offsets,
                            int frame) {
  int       xn, yn, zn, l, i;
  GCA_NODE *gcan;
  GC1D *    gc;
...
int          GCAmapRenormalize(GCA *gca, MRI *mri, TRANSFORM *transform) {
  ...
  float      fmin, fmax, prior, label_scales[MAX_CMA_LABELS],
      label_modes[MAX_CMA_LABELS], modes[MAX_GCA_INPUTS], std, peak,
      smooth_peak;
  double     val /*, scale*/;
  GCA_PRIOR *gcap;
  GCA_NODE * gcan;
  GC1D *     gc;
  MATRIX *   m_cov;
  MRI *      mri_fsamples = nullptr; // diag volume
  ...
  for (frame = 0; frame < mri->nframes; frame++) {
    ...
    for (l = 0; l <= MAX_CMA_LABELS; l++) /* don't do Unknown class */
    {
      ...
      printf("%s (%d): mode = %2.2f +- %2.1f\n", cma_label_to_name(l), l,
             label_modes[l], std);
      if (l == Gdiag_no) {
        mri_fsamples = MRIclone(mri, nullptr);
        DiagBreak();
      }
      ...
      for (num = xp = 0; xp < gca->prior_width; xp++) {
        for (yp = 0; yp < gca->prior_height; yp++) {
          for (zp = 0; zp < gca->prior_depth; zp++) {
            ...
            if (!GCApriorToSourceVoxel(gca, mri, transform, xp, yp, zp, &x, &y,
                                       &z)) {
              MRIsampleVolumeFrame(mri, x, y, z, frame, &val);
              if (FZERO(val)) // skull stripped
              {
      ...
      smooth_peak =
          hsmooth
              ->bins[HISTOfindHighestPeakInRegion(hsmooth, 0, hsmooth->nbins)];

      label_scales[l] = (float)smooth_peak / label_modes[l];
      printf("%s (%d): peak at %2.2f, smooth at %2.2f (%d voxels), "
             "scaling by %2.2f\n",
             cma_label_to_name(l), l, peak, smooth_peak, num, label_scales[l]);
          ...
          for (i = 0; i < gcan->nlabels; i++) {
            if (ranks_before[i].label != ranks_after[i].label) {
              ...
              avg = (means_after[ranks_after[i].index] +
                     means_after[ranks_before[i].index]) /
                    2;
              ranks_after[i].prob = means_after[ranks_after[i].index] =
                  avg + diff / 4;
              ranks_after[j].prob = means_after[ranks_after[j].index] =
                  avg - diff / 4;
#endif
              qsort(ranks_after, gcan->nlabels, sizeof(LABEL_PROB),
                    compare_sort_probabilities);
...
int GCAmapRenormalizeByClass(GCA *gca, MRI *mri, TRANSFORM *transform) {
  HISTOGRAM *h, *hsmooth;
  int l, nbins, i, x, y, z, max_p_label, xn, yn, zn, num, frame, bin, n, c,
      max_label;
  // int label;
  float fmin, fmax, prior, label_scales[MAX_CMA_LABELS],
      ...
      for (num = x = 0; x < mri->width; x++) {
        for (y = 0; y < mri->height; y++) {
          for (z = 0; z < mri->depth; z++) {
            ...
            max_p       = GCAcomputePosteriorDensity(gcap, gcan, 0, -1, vals,
                                               gca->ninputs, xn, yn, zn, gca);
...
#define L_VENTRALDC 0.2 // mostly white
      ...
      if (IS_GRAY_CLASS(l)) {
        ...
        case Right_VentralDC:
        case Left_VentralDC:
          label_scales[l] = L_VENTRALDC * class_scales[GM_CLASS] +
        ...
        case Right_Thalamus:
          ...
          label_scales[l] = L_THALAMUS * class_scales[GM_CLASS] +
                            (1 - L_THALAMUS) * class_scales[WM_CLASS];
          break;
        case Left_Pallidum:
        case Right_Pallidum:
          label_scales[l] = L_PALLIDUM * class_scales[GM_CLASS] +
                            (1 - L_PALLIDUM) * class_scales[WM_CLASS];
          break;
        case Left_Putamen:
        case Right_Putamen:
          label_scales[l] = (L_PUTAMEN)*class_scales[GM_CLASS] +
      ...
      printf("%s (%d): scaling by %2.2f = %2.1f (was %2.1f)\n",
             cma_label_to_name(l), l, label_scales[l],
             modes[frame] * label_scales[l], modes[frame]);
    }
...
MRI *GCArelabelNonbrain(GCA *gca, MRI *mri_inputs, MRI *mri_src, MRI *mri_dst,
                        TRANSFORM *transform) {
  int x, y, z, xn, yn, zn, width, height, depth, label, i, total_changed = 0, n,
                                                           nchanged;
  int    max_i = 0;
  GC1D * gcs[NLABELS];
  double pvals[NLABELS], max_p;
  // GCA_NODE *gcan;
  float vals[MAX_GCA_INPUTS], means[NLABELS][MAX_GCA_INPUTS],
    ...
    for (x = 0; x < width; x++) {
      for (y = 0; y < height; y++) {
        for (z = 0; z < depth; z++) {
          ...
          if (label != Dura && label != Bone && label != SC_FAT_MUSCLE &&
              label != CSF_SA) {
            continue;
          }
          ...
          load_vals(mri_inputs, x, y, z, vals, gca->ninputs);
          if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn,
                                    &yn, &zn)) {
            // gcan = &gca->nodes[xn][yn][zn];
            max_i = -1;
            max_p = -1;
      ...
      for (y = 0; y < height; y++) {
        for (z = 0; z < depth; z++) {
          ...
          if (((MRIneighborsInWindow(mri_tmp, x, y, z, 3, Bone) < 7) ||
               (MRIneighborsInWindow(mri_tmp, x, y, z, 3, SC_FAT_MUSCLE) <
                7)) &&
...
GCA_NODE *GCAbuildRegionalGCAN(const GCA *gca, int xn, int yn, int zn,
                               int wsize) {
  GCA_NODE * gcan, *gcan_nbr;
  GCA_PRIOR *gcap;
  int        n, xi, yi, zi, xk, yk, zk, nlabels = 0, whalf, xp, yp, zp, label,
                                 total_training[MAX_CMA_LABELS + 1],
                                 used[MAX_CMA_LABELS + 1];
  float   label_priors[MAX_CMA_LABEL + 1], p;
  MATRIX *m_cov[MAX_CMA_LABEL + 1], *m_tmp   = nullptr;
  VECTOR *v_means[MAX_CMA_LABEL + 1], *v_tmp = nullptr;
    ...
    for (yk = -whalf; yk <= whalf; yk++) {
      ...
      for (zk = -whalf; zk <= whalf; zk++) {
        ...
        for (n = 0; n < gcan_nbr->nlabels; n++) {
          ...
          {
            used[label]  = 1;
            m_cov[label] = load_covariance_matrix(&gcan_nbr->gcs[n], nullptr,
                                                  gca->ninputs);
            v_means[label] =
                load_mean_vector(&gcan_nbr->gcs[n], nullptr, gca->ninputs);
          ...
          m_tmp =
              load_covariance_matrix(&gcan_nbr->gcs[n], m_tmp, gca->ninputs);
...
GCA_PRIOR *GCAbuildRegionalGCAP(const GCA *gca, int xp, int yp, int zp,
                                int wsize) {
  GCA_PRIOR *gcap, *gcap_nbr;
  int        n, xi, yi, zi, xk, yk, zk, nlabels = 0, whalf, label,
                                 total_training[MAX_CMA_LABELS + 1],
                                 used[MAX_CMA_LABELS + 1];
  float  label_priors[MAX_CMA_LABEL + 1], p;
  double total_p;
...
MRI *GCAreplaceImpossibleLabels(MRI *mri_inputs, GCA *gca, MRI *mri_in_labels,
                                MRI *mri_out_labels, TRANSFORM *transform) {
...
#define D_ALPHA 0.01
  for (p = alpha = 0.0; alpha <= 1.0; alpha += D_ALPHA) {
    ...
    dist = SQR(u - vals[0]) / v;
    p_alpha =
        1.0 / sqrt(v) * exp(-0.5 * dist) * pow(p1, alpha) * pow(p2, 1 - alpha);
    p += (p_alpha * D_ALPHA);
  }
  return (log(p));
}
...
static int gcaRelabelSegment(GCA *gca, TRANSFORM *transform, MRI *mri_inputs,
                             MRI *mri_dst, MRI_SEGMENT *mseg) {
  int i, n, x, y, z, labels[MAX_CMA_LABEL + 1], label, max_label, old_label,
      debug = 0;
  ...
  for (max_posterior = 0.0, old_label = max_label = i = 0; i < mseg->nvoxels;
       i++) {
    x = mseg->voxels[i].x;
    y = mseg->voxels[i].y;
    z = mseg->voxels[i].z;
    ...
    max_posterior += GCAnbhdGibbsLogPosterior(gca, mri_dst, mri_inputs, x, y, z,
                                              transform, PRIOR_FACTOR);
  ...
  if ((old_label != max_label) && (debug || getenv("DEBUG_MRM")))
    printf("changing segment at (%2.0f, %2.0f, %2.0f) "
           "from %s (%d) to %s (%d)\n",
           mseg->cx, mseg->cy, mseg->cz, cma_label_to_name(old_label),
           old_label, cma_label_to_name(max_label), max_label);
  for (i = 0; i < mseg->nvoxels; i++) {
    x = mseg->voxels[i].x;
    y = mseg->voxels[i].y;
    z = mseg->voxels[i].z;
...
MRI *GCAmarkImpossible(GCA *gca, MRI *mri_labeled, MRI *mri_dst,
                       TRANSFORM *transform) {
  int x, y, z, label;

  if (mri_dst == nullptr) {
    mri_dst = MRIclone(mri_labeled, nullptr);
  }
  for (x = 0; x < mri_labeled->width; x++) {
    for (y = 0; y < mri_labeled->height; y++) {
      for (z = 0; z < mri_labeled->depth; z++) {
        ...
        if (GCAisPossible(gca, mri_labeled, label, transform, x, y, z, 1) ==
            0) {
...
MRI *GCAbuildMostLikelyVolumeForStructure(const GCA *gca, MRI *mri, int label,
                                          int border, TRANSFORM *transform,
                                          MRI *mri_labels) {
    ...
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        if (GCAsourceVoxelToNode(gca, mri, transform, x, y, z, &xn, &yn, &zn) ==
            NO_ERROR) {
          // get prior value
          if (GCAsourceVoxelToPrior(gca, mri, transform, x, y, z, &xp, &yp,
                                    &zp) == NO_ERROR) {
            gcan = &gca->nodes[xn][yn][zn];
            gcap = &gca->priors[xp][yp][zp];
            if (gcap == nullptr || gcap->nlabels <= 0) {
        ...
        for (n = 0; n < gcan->nlabels; n++) {
          ...
          if (prior != 0) {
            //  for (r = 0 ; r < gca->ninputs ; r++)
            {
              ...
              h_gca->counts[b] += prior;
              if (!std::isfinite(gc->means[frame]))
                DiagBreak();
            }
          }
        }
...
#if INTERP_PRIOR
static float gcaComputePrior(GCA *gca, MRI *mri, TRANSFORM *transform, int x0,
                             int y0, int z0, int label) {
...
MRI *GCAbuildMostLikelyLabelProbabilityVolume(GCA *gca) {
  ...
  mri = MRIallocSequence(gca->prior_width, gca->prior_height, gca->prior_depth,
...
int GCAcomputeLabelMeansAndCovariances(GCA *gca, int target_label,
  ...
  GCA_NODE *gcan;
  float     fval;
  MATRIX *  m_cov = nullptr, *m_cov_total;
  VECTOR *  v_means;
...
#define WSIZE 5
#define WHALF ((WSIZE - 1) / 2)
static double compute_conditional_density(MATRIX *m_inv_cov, VECTOR *v_means,
                                          VECTOR *v_vals) {
  double p, dist, det;
  int    ninputs;
...
MRI *GCAlabelWMandWMSAs(GCA *gca, MRI *mri_inputs, MRI *mri_src_labels,
                        MRI *mri_dst_labels, TRANSFORM *transform) {
  int h, wm_label, wmsa_label, x, y, z, label, nwm, nwmsa, nunknown, ngm,
      ncaudate, caudate_label, gm_label, n, found, i, NotWMSA;
  MATRIX *m_cov_wm, *m_cov_wmsa, *m_inv_cov_wmsa, *m_inv_cov_wm, *m_cov_un,
      *m_inv_cov_un;
  // MATRIX *m_I;
  ...
  for (h = 0; h <= 1; h++) {
    ...
    GCAcomputeLabelMeansAndCovariances(gca, caudate_label, &m_cov_wm,
                                       &v_mean_caudate);
    GCAcomputeLabelMeansAndCovariances(gca, wmsa_label, &m_cov_wmsa,
                                       &v_mean_wmsa);
    m_inv_cov_wm = MatrixInverse(m_cov_wm, nullptr);
    if (m_inv_cov_wm == nullptr)
      ErrorExit(ERROR_BADPARM,
                "%s: could not compute inverse covariance for %s (%d)",
                Progname, cma_label_to_name(wm_label), wm_label);
    m_inv_cov_un = MatrixInverse(m_cov_un, nullptr);
    if (m_inv_cov_un == nullptr)
      ErrorExit(ERROR_BADPARM,
                "%s: could not compute inverse covariance for %s (%d)",
                Progname, cma_label_to_name(Unknown), Unknown);
    m_inv_cov_wmsa = MatrixInverse(m_cov_wmsa, nullptr);
    if (m_inv_cov_wmsa == nullptr)
      ErrorExit(ERROR_BADPARM,
                "%s: could not compute inverse covariance for %s (%d)",
        ...
        for (z = 0; z < mri_inputs->depth; z++) {
          ...
          if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
            printf("(%d, %d, %d) - %s (nbrs = %d + %d = %2.2f%%)\n", x, y, z,
                   cma_label_to_name(label), nwm, nwmsa,
          ...
          pwmsa =
              compute_conditional_density(m_inv_cov_wmsa, v_mean_wmsa, v_vals);
          if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
          ...
          if (label == wm_label && pwmsa > pwm) {
            ...
            wm_mdist =
                MatrixMahalanobisDistance(v_mean_wm, m_inv_cov_wm, v_vals);
            wmsa_mdist =
                MatrixMahalanobisDistance(v_mean_wmsa, m_inv_cov_wmsa, v_vals);
            if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
              printf("         - wm_dist = %2.0f, "
            ...
            if ((wm_dist > wmsa_dist) && (wm_mdist > wmsa_mdist)) {
              ...
              */
              NotWMSA = 0;
              for (n = 0; n < mri_inputs->nframes; n++)
                if (fabs(VECTOR_ELT(v_dif_wmsa, n + 1)) >=
                    fabs(VECTOR_ELT(v_dif_label, n + 1)))
                  NotWMSA = 1;
              if (!NotWMSA ||
      ...
      for (x = 0; x < mri_inputs->width; x++) {
        for (y = 0; y < mri_inputs->height; y++) {
          for (z = 0; z < mri_inputs->depth; z++) {
            ...
            if (label != wm_label && label != Unknown &&
                label != caudate_label) {
              continue;
            }
            load_val_vector(v_vals, mri_inputs, x, y, z);
...
#endif
#define WSIZE 7
static MRI *expand_ventricle(MRI *mri_src, MRI *mri, MRI *mri_dst, float thresh,
                             int nvox_thresh, int xdir) {
  int nfilled, x, y, z, xmin, xmax, ymin, ymax, zmin, zmax, nvox, prev_nfilled,
      iter = 0;
  MRI_REGION box;
  MRI *      mri_tmp = nullptr;
    ...
    for (x = xmin; x <= xmax; x++)
      for (y = ymin; y <= ymax; y++)
        for (z = zmin; z <= zmax; z++) {
          ...
          if (MRIcountNonzeroInNbhd(mri_dst, WSIZE, x, y, z) <
              (WSIZE * WSIZE)) {
            continue; // doesn't nbr any enough voxels
          }
          nvox = MRIcountThreshInNbhd(mri, WSIZE, x, y, z, thresh);
          nvox = (WSIZE * WSIZE * WSIZE) -
                 nvox; // # below thresh instead of above it
          if (nvox < nvox_thresh) {
...
static MRI *fill_ventricles(MRI *mri_seg, MRI *mri, int border, MRI *mri_dst,
                            int label) {
  int   i, left;
  float fmin, fmax;
...
static int initialize_ventricle_alignment(MRI *mri_seg, MRI *mri, MATRIX *m_L,
  ...
  mri_gca_vent = MRIbinarize(mri_seg, nullptr, 1, 0, 1);
  while (border-- > 0) {
    MRIerode(mri_gca_vent, mri_gca_vent);
  }

  mri_vent      = fill_ventricles(mri_seg, mri, border, nullptr, label);
  mri_vent_dist = MRIdistanceTransform(mri_vent, nullptr, 1, mri_vent->width,
                                       DTRANS_MODE_SIGNED, nullptr);
  mri_gca_vent_dist =
      MRIdistanceTransform(mri_gca_vent, nullptr, 1, mri_gca_vent->width,
                           DTRANS_MODE_SIGNED, nullptr);
  MRIwrite(mri_gca_vent, "gv.mgz");
  MRIwrite(mri_vent, "v.mgz");
  MRIwrite(mri_gca_vent_dist, "gvd.mgz");
  MRIwrite(mri_vent_dist, "vd.mgz");
  mri_mask = MRIcopy(mri_gca_vent, nullptr);
  ...
  MRIpowellAlignImages(mri_gca_vent_dist, mri_vent_dist, m_L, nullptr, nullptr,
  ...
  MRIcomputeOptimalLinearXform(mri_seg, mri, m_L, -RADIANS(20), RADIANS(20),
                               .75,
                               1.5, // allow for ventricular expansion
                               -10, 10, 3, 4, 3, 4, base_name, 0);
  MRIcomputeOptimalLinearXform(mri_seg, mri, m_L, -RADIANS(5), RADIANS(5), .8,
                               1.2, // allow for ventricular expansion
                               -5, 5, 3, 3, 3, 3, base_name, 0);
  return (NO_ERROR);
}
...
double GCAimageLogLikelihood(GCA *gca, MRI *mri_inputs, TRANSFORM *transform,
  ...
  if (DIAG_VERBOSE_ON)
    mri_posterior = MRIalloc(mri_inputs->width, mri_inputs->height,
                             mri_inputs->depth, MRI_FLOAT);
  else {
    mri_posterior = nullptr;
  }

  if (Gx >= 0) {
    GCAsourceVoxelToPrior(gca, mri_inputs, transform, Gx, Gy, Gz, &Gxp, &Gyp,
                          &Gzp);
    GCApriorToSourceVoxel(gca, mri_inputs, transform, Gxp, Gyp, Gzp, &x, &y,
                          &z);
  }
    ...
    for (yp = 0; yp < gca->prior_height; yp++) {
      for (zp = 0; zp < gca->prior_depth; zp++) {
        ...
        for (n = 0; n < gcap->nlabels; n++) {
          ...
          if (penalize_zero_brain && FZERO(vals[0]) && IS_BRAIN(label) &&
              !IS_CSF(label)) {
...
#else
double GCAimageLogLikelihood(GCA *gca, MRI *mri_inputs, TRANSFORM *transform) {
  int x, y, z, width, height, depth, xn, yn, zn, n, label /*, found*/, xp, yp,
      zp;
  GCA_NODE *gcan;
  GCA_PRIOR *gcap;
  GC1D *gc;
  ...
  */
  ...
  for (total_log_p = 0.0, x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        /* find the node associated with this coordinate and classify */
        GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
                             &zn);
        if (xn < 0) {
          xn = 0;
        ...
        gcan = &gca->nodes[xn][yn][zn];
        GCAsourceVoxelToPrior(gca, mri_inputs, transform, x, y, z, &xp, &yp,
                              &zp);
        if (xp < 0) {
          xp = 0;
  ...
  total_log_p /=
      (double)(mri_inputs->width * mri_inputs->height * mri_inputs->depth);
  return (total_log_p);
}
#endif
  ...
  for (x = 0; x < mri_inputs->width; x++)
    for (y = 0; y < mri_inputs->height; y++)
      for (z = 0; z < mri_inputs->depth; z++) {
        ...
        load_vals(mri_inputs, x, y, z, vals, gca->ninputs);
        likelihood =
            GCAcomputeConditionalDensity(gc, vals, gca->ninputs, label);
        if (DZERO(likelihood))
          likelihood = 1e-10;
...
#define NUM_CONTRA_LABELS                                                      \
  (sizeof(contra_entropy_labels) / sizeof(contra_entropy_labels[0]))

static int compute_posterior_scale_change(GCA *gca, MRI *mri, MRI *mri_aseg,
                                          TRANSFORM *transform, int *labels,
                                          int *contra_labels, float *scales,
                                          int nlabels, float step_size) {
  ...
  for (x = 0; x < mri->width; x++) {
    for (y = 0; y < mri->height; y++) {
      for (z = 0; z < mri->depth; z++) {
        ...
        if (gcap == nullptr || gcap->nlabels <= 1 || ind < 0) {
          continue;
        }
        if (MRIlabelsInNbhd(mri_aseg, x, y, z, 1, label) <
            26) // avoid border voxels
        {
          continue;
        }
        if (!GCAsourceVoxelToNode(gca, mri, transform, x, y, z, &xn, &yn,
...
int GCArenormalizeWithEntropyMinimization(GCA *gca, MRI *mri,
                                          TRANSFORM *transform, FILE *logfp) {
  float scales[NUM_ENTROPY_LABELS], ll, last_posterior,
      peaks[NUM_ENTROPY_LABELS], contra_peaks[NUM_CONTRA_LABELS], pct_change,
  ...
  mri_aseg = GCAlabel(mri, gca, nullptr, transform);
  if (Gdiag & DIAG_WRITE) {
    char fname[STRLEN];
  ...
  do {
    ...
    compute_posterior_scale_change(gca, mri, mri_aseg, transform,
                                   entropy_labels, contra_entropy_labels,
                                   scales, NUM_ENTROPY_LABELS, .1);
    gcaScale(gca, entropy_labels, contra_entropy_labels, scales,
             NUM_ENTROPY_LABELS, 1);
    {
  ...
  for (i = 0; i < NUM_ENTROPY_LABELS; i++) {
    printf("scaling %s by %2.3f from %2.0f to %2.0f (rh=%2.0f)\n",
           cma_label_to_name(entropy_labels[i]), scales[i], peaks[i],
           peaks[i] * scales[i], contra_peaks[i] * scales[i]);
    if (logfp)
  ...
  for (entropy_total = 0.0, x = 0; x < mri->width; x++) {
    for (y = 0; y < mri->height; y++) {
      for (z = 0; z < mri->depth; z++) {
        ...
        gcap = getGCAP(gca, mri, transform, x, y, z);
        if (gcap == nullptr || gcap->nlabels <= 1) {
          continue;
        }
        if (!GCAsourceVoxelToNode(gca, mri, transform, x, y, z, &xn, &yn,
                                  &zn)) {
          ...
          for (ptotal = 0, c = 0; c < gcap->nlabels; c++) {
            ...
            p[c] = GCAcomputeConditionalDensity(gc, vals, gca->ninputs,
                                                gcap->labels[c]);
            ptotal += p[c];
            if (p[c] > max_p) {
              max_p = p[c];
...
int GCAgetMaxPriorLabelAtVoxel(GCA *gca, MRI *mri, int x, int y, int z,
                               TRANSFORM *transform, double *p_prior) {
  int    xp, yp, zp;
  double xpf, ypf, zpf;
...
#endif
float GCAcomputeLabelPosterior(GCA *gca, TRANSFORM *transform, MRI *mri,
...
static GCA_NODE *gcaBuildNbhdGCAN(GCA *gca, int x, int y, int z, float sigma) {
  ...
  int labels[MAX_CMA_LABELS], n, xi, yi, zi, xk, yk, zk, whalf, nlabels, n2, n3,
      n4, r, c, v, i;
  GC1D * gc, *gc_total;
  double norm, wt, two_sigma_sq, dsq, total_wt,
      gc_training[MAX_LABELS_PER_GCAN], total_training;
            ...
            if (!(gca->flags & GCA_NO_MRF)) {
              for (i = 0; i < GIBBS_NEIGHBORS; i++) {
                for (n2 = 0; n2 < gcan->gcs[n].nlabels[i]; n2++) {
                  nbr_labels[i][gcan->labels[n]][gcan->gcs[n].labels[i][n2]] =
                      1;
                }
              }
            }
  ...
  if (nlabels > MAX_LABELS_PER_GCAN)
    ErrorExit(ERROR_NOMEMORY,
              ...
              x, y, z, wsize, sigma, nlabels, MAX_LABELS_PER_GCAN);

  gcan_total->labels =
      (unsigned short *)calloc(nlabels, sizeof(unsigned short));
  gcan_total->nlabels = nlabels;
  if (gcan_total->labels == nullptr) {
    ErrorExit(ERROR_NOMEMORY,
              "gcaBuildNbhdGCAN: could not allocate node labels");
  }

  gcan_total->gcs = alloc_gcs(nlabels, gca->flags, gca->ninputs);
  if (gcan_total->gcs == nullptr) {
    ...
    if (!(gca->flags & GCA_NO_MRF)) {
      for (i = 0; i < GIBBS_NEIGHBORS; i++) {
        if (gc_total->nlabels[i] == 0) {
          ...
          gc_total->labels[i] =
              (unsigned short *)calloc(gc_total->nlabels[i], sizeof(short));
          if (gc_total->labels[i] == nullptr)
            ErrorExit(ERROR_NOMEMORY,
                      "gcaBuildNbhdGCAN: could not allocate %d:%d labels",
                      gc_total->nlabels[i], i);

          gc_total->label_priors[i] =
              (float *)calloc(gc_total->nlabels[i], sizeof(float));
          if (gc_total->label_priors[i] == nullptr)
            ErrorExit(ERROR_NOMEMORY,
                      "gcaBuildNbhdGCAN: could not allocate %d:%d label priors",
                      gc_total->nlabels[i], i);
        }
        for (n2 = nlabels = 0; n2 <= max_label; n2++) {
          if (nbr_labels[i][gcan_total->labels[n]][n2]) {
            if (nlabels >= gc_total->nlabels[i]) {
              ...
              gc_total->labels[i][nlabels++] =
                  n2; // label n2 is nbr in position i
            }
          }
        }
      }
...
static GCA_PRIOR *gcaBuildNbhdGCAP(GCA *gca, int x, int y, int z, float sigma,
      ...
      for (zi = -whalf; zi <= whalf; zi++) {
        ...
        for (n = 0; n < gcap->nlabels; n++) {
          ...
          if (gca_smooth) {
            ...
            if (GCAfindGC(gca_smooth, xn2, yn2, zn2, gcap->labels[n]) ==
                nullptr) {
              continue;
            }
          }
          if (gcap->labels[n] == Gdiag_no && x == Gxp && y == Gyp && z == Gzp)
            DiagBreak();
          if (true || GCAfindGC(gca, xn, yn, zn, gcap->labels[n]) !=
                          nullptr) { // DISABLED (BRF)
            labels[gcap->labels[n]]++;
          }
        }
      }
  ...
  gcap_total->labels =
      (unsigned short *)calloc(nlabels, sizeof(unsigned short));
  if (gcap_total->labels == nullptr) {
    ErrorExit(ERROR_NOMEMORY,
              "gcaBuildNbhdGCAP: could not allocate node labels");
  }

  gcap_total->priors = (float *)calloc(nlabels, sizeof(float));
  if (gcap_total->labels == nullptr) {
    ErrorExit(ERROR_NOMEMORY,
              "gcaBuildNbhdGCAP: could not allocate node priors");
  }
    ...
    for (yi = -whalf; yi <= whalf; yi++) {
      ...
      for (zi = -whalf; zi <= whalf; zi++) {
        ...
        for (n = 0; n < gcap->nlabels; n++) {
          ...
          if (false && GCAfindGC(gca, xn, yn, zn, gcap->labels[n]) ==
                           nullptr) { // DISABLED (BRF)
            continue;
          }
        ...
        total_training +=
            gcap->total_training == 0 ? wt : wt * gcap->total_training;
      }
    }
  }
...
MRI *GCAreclassifyUnlikelyVoxels(GCA *gca, TRANSFORM *transform,
                                 ...
                                 MRI *mri_aseg_changed, float mah_dist_thresh,
        ...
        for (z = 0; z < depth; z++) {
          ...
          if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn,
            ...
            if (d_old > mah_dist_thresh) {
              GCAsourceVoxelToPrior(gca, mri_inputs, transform, x, y, z, &xp,
                                    &yp, &zp);
              ...
              gcap_total = gcaBuildNbhdGCAP(gca, xp, yp, zp, sigma, nullptr);
              max_label  = 0;
              // max_n = -1;
              max_p  = 2 * GIBBS_NEIGHBORS * BIG_AND_NEGATIVE;
              max_gc = nullptr;
              ...
              for (n = 0; n < gcap_total->nlabels; n++) {
                ...
                p = gcaComputeLogDensity(gc, vals, gca->ninputs,
                                         gcap_total->priors[n],
                                         gcap_total->labels[n]);
                p = gcaGibbsLogPosterior(gca, mri_aseg_changed, vals,
                                         gcap_total->labels[n], x, y, z,
                                         gcap_total, gcan_total, transform);
                if (p > max_p) {
                  max_p     = p;
              ...
              MRIsetVoxVal(mri_changed, x, y, z, 0,
                           0); // will change to 1 if changed
              if (max_gc) {
...
MRI *GCAcomputeOptimalScale(GCA *gca, TRANSFORM *transform, MRI *mri_inputs,
                            MRI *mri_aseg, MRI *mri_sigma, int wsize,
      ...
      for (z = 0; z < depth; z++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_inputs, transform, x, y, z, &xn, &yn,
          ...
          for (scale = max_sigma / 2; scale >= 1; scale /= 2) {
            for (sigma = first_sigma; sigma <= last_sigma;
                 sigma += scale * delta_sigma) {
              gcan_total = gcaBuildNbhdGCAN(gca, xn, yn, zn, sigma);
              gcap_total = gcaBuildNbhdGCAP(gca, xp, yp, zp, sigma, nullptr);
              ...
              total_p = 0;
              for (n = 0; n < gcap_total->nlabels; n++) {
                p = gcaGibbsLogPosterior(gca, mri_aseg, vals,
                                         gcap_total->labels[n], x, y, z,
                                         gcap_total, gcan_total, transform);
                total_p += exp(p);
              }
...
MRI *GCAreclassifyVoxelsAtOptimalScale(GCA *gca, TRANSFORM *transform,
                                       MRI *mri_inputs, MRI *mri_aseg,
                                       MRI *mri_aseg_changed, MRI *mri_sigma,
                                       int wsize) {
  int        x, y, z, max_label, nchanged, iter = 0;
  GCA_NODE * gcan_total;
  GCA_PRIOR *gcap_total;
            ...
            if (max_gc && (max_label != label)) {
              ...
              if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
                printf("GCAreclassifyVoxelsAtOptimalScale: "
                       ...
                       x, y, z, sigma, cma_label_to_name(label), label,
                       cma_label_to_name(max_label), max_label);
              MRIsetVoxVal(mri_aseg_changed, x, y, z, 0, max_label);
            }
            GCANfree(gcan_total, gca->ninputs);
        ...
        for (n = 0; n < gcap->nlabels; n++) {
          ...
          if (gc == nullptr)
            printf("gcap(%d, %d, %d), labels[%d] = %s (%d) --> node (%d, %d, "
                   "%d), no gc!\n",
                   xp, yp, zp, n, cma_label_to_name(label), label, xn, yn, zn);
          error = ERROR_BADPARM;
        }
      }
...
GCA *GCAcopy(GCA *gca_src, GCA *gca_dst) {
  if (gca_dst == nullptr)
    gca_dst = GCAalloc(
        gca_src->ninputs, gca_src->prior_spacing, gca_src->node_spacing,
        gca_src->node_width * gca_src->node_spacing,
        gca_src->node_height * gca_src->node_spacing,
        gca_src->node_depth * gca_src->node_spacing, gca_src->flags);
  ...
  memmove(gca_dst->tissue_parms, gca_src->tissue_parms,
          sizeof(gca_src->tissue_parms));
...
GCA *GCAsmooth(GCA *gca, double sigma) {
  ...
  gca_smooth = GCAcopy(gca, nullptr);
    ...
    for (yn = 0; yn < gca->node_height; yn++) {
      for (zn = 0; zn < gca->node_depth; zn++) {
        ...
        if (gcan_total->nlabels == 0) {
          ...
          gcan_total->gcs =
              alloc_gcs(gcan_total->nlabels, gca->flags, gca->ninputs);
          gcan_total->gcs[0].covars[0] = 25;
        }
        if (gcan->max_labels < gcan_total->nlabels) {
          free(gcan->labels);
          gcan->labels = (unsigned short *)calloc(gcan_total->nlabels,
                                                  sizeof(unsigned short));
          if (gcan->labels == nullptr)
            ErrorExit(ERROR_NOMEMORY,
                      "GCAsmooth(%2.2f) couldn't allocate %d label node", sigma,
                      gcan_total->nlabels);
        }
...
int GCAsourceVoxelToPriorReal(GCA *gca, MRI *mri, TRANSFORM *transform, int xv,
                              int yv, int zv, double *pxp, double *pyp,
                              double *pzp) {
  float  xt = 0, yt = 0, zt = 0;
  double xrt, yrt, zrt;
  // int retval;
    ...
    for (xk = -whalf; xk <= whalf; xk++)
      for (yk = -whalf; yk <= whalf; yk++)
        for (zk = -whalf; zk <= whalf; zk++) {
          ...
          found = 0;
          if (!GCAsourceVoxelToNode(gca, mri, transform, xi, yi, zi, &xn, &yn,
                                    &zn)) {
            gcan = &gca->nodes[xn][yn][zn];
            for (i = 0; i < gcan->nlabels; i++)
            ...
            if (found == 0) // insert it
            {
              ...
              if ((gca->flags & GCA_NO_MRF) == 0) {
                for (i = 0; i < GIBBS_NEIGHBORS; i++) {
                  gc->nlabels[i] =
                      gcan->nlabels + 1; // assume any of existing labels can
                                         // occur plus this one
                  gc->labels[i] = (unsigned short *)calloc(
                      gc->nlabels[i], sizeof(unsigned short));
                  if (!gc->labels) {
                    ErrorExit(ERROR_NOMEMORY,
          ...
          if (!GCAsourceVoxelToPrior(gca, mri, transform, xi, yi, zi, &xp, &yp,
                                     &zp)) {
            found = 0;
            gcap  = &gca->priors[xp][yp][zp];
            for (i = 0; i < gcap->nlabels; i++)
...
int GCAreadLabelIntensities(char *fname, float *label_scales,
                            float *label_offsets) {
  FILE *fp;
  int   l;
  char *cp, line[STRLEN];
...
int GCAupdateDistributions(GCA *gca, MRI *mri, TRANSFORM *transform) {
  ...
  all_vals = (float *)calloc(
      gca->node_width * gca->node_height * gca->node_depth, sizeof(float));
  for (label = 0; label <= MAX_CMA_LABEL; label++) {
    ...
    for (x = 0; x < gca->node_width; x++) {
      for (y = 0; y < gca->node_height; y++) {
        for (z = 0; z < gca->node_depth; z++) {
          gcan = &gca->nodes[x][y][z];
          if (GCAnodeToSourceVoxel(gca, mri, transform, x, y, z, &xv, &yv,
                                   &zv) == NO_ERROR) {
...
int is_possible_wmsa(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y,
                     int z, int whalf) {
  GCA_PRIOR *gcap;
  ...
  for (xi = -whalf; xi <= whalf; xi++)
    for (yi = -whalf; yi <= whalf; yi++)
      for (zi = -whalf; zi <= whalf; zi++) {
        GCAsourceVoxelToPrior(gca, mri, transform, x + xi, y + yi, z + zi, &xp,
                              &yp, &zp);
        gcap = getGCAP(gca, mri, transform, x, y, z);
        if (gcap == nullptr) {
          continue;
        ...
        if (left_wmsa > never_thresh || right_wmsa > never_thresh ||
            wmsa > never_thresh) {
...
double cortex_prior(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y,
                    int z) {
  GCA_PRIOR *gcap;
  int        xp, yp, zp;
...
double csf_prior(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y,
                 int z) {
  GCA_PRIOR *gcap;
  int        xp, yp, zp, n;
  float      prior;
...
int GCAisLeftHemisphere(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y,
                        int z) {
  GCA_PRIOR *gcap;
...
/*
  ...
  **seg, MRI *out) \brief Samples the intensities of the GCA into the
  ...
  1 The result can be fed to mri_ca_normalize
  ...
  requires).
*/
MRI *GCAsampleToVol(MRI *mri, GCA *gca, TRANSFORM *transform, MRI **seg,
                    MRI *out) {
  int z;
...
MRI *GCAsampleToVolWMSAprob(MRI *mri, GCA *gca, TRANSFORM *transform,
                            MRI *out) {
  int z;
...
int MRIwmsaHalo(MRI *mri_inputs, MRI *mri_labeled, int n) {
  int h, label, nwmsa, ngm, x, y, z, wmsa_label, wm_label, gm_label, a, b,
      changed;
  // int nwm;
  double  wm_mdist = 0, wmsa_mdist = 0, caudate_mdist = 0, vent_mdist = 0;
  VECTOR *v_vals = nullptr, *wm_means, *wmsa_means, *caudate_means, *vent_means;
  MATRIX *wmsa_cov, *wm_cov, *caudate_cov, *vent_cov,
      *m_inv_cov_wmsa = nullptr, *m_inv_cov_wm = nullptr,
      *m_inv_cov_caudate = nullptr, *m_inv_cov_vent = nullptr;
  MRI *label_copy = nullptr;
  ...
  MRIcomputeWMSAMeansandCovariances(mri_inputs, mri_labeled, &wmsa_cov,
                                    &wmsa_means);
  m_inv_cov_wmsa = MatrixAlloc(wmsa_cov->rows, wmsa_cov->cols, wmsa_cov->type);
  MatrixInverse(wmsa_cov, m_inv_cov_wmsa);

  MRIcomputeCaudateMeansandCovariances(mri_inputs, mri_labeled, &caudate_cov,
                                       &caudate_means);
  m_inv_cov_caudate =
      MatrixAlloc(caudate_cov->rows, caudate_cov->cols, caudate_cov->type);
  MatrixInverse(caudate_cov, m_inv_cov_caudate);

  MRIcomputeVentMeansandCovariances(mri_inputs, mri_labeled, &vent_cov,
                                    &vent_means);
  ...
  for (b = 0; b < 2; b++) {
    ...
    label_copy = MRIcopy(mri_labeled, nullptr);
      ...
      for (x = 0; x < mri_labeled->width; x++) {
        for (y = 0; y < mri_labeled->height; y++) {
          for (z = 0; z < mri_labeled->depth; z++) {
            ...
            // wm_label, x, y, z, n, &wm_cov, &wm_means); printf("Computing
            // means and covariances\n"); fflush(stdout);
            // MRIcomputeNbhdMeansandCovariances(mri_inputs, label_copy,
            // wmsa_label, x, y, z, n, &wmsa_cov, &wmsa_means);
            // printf("finished\n");
            // fflush(stdout);

            // m_inv_cov_wm = MatrixAlloc(wm_cov->rows, wm_cov->cols,
            // wm_cov->type) ; m_inv_cov_wmsa = MatrixAlloc(wmsa_cov->rows,
            // wmsa_cov->cols, wmsa_cov->type) ;
            ...
            wmsa_mdist =
                MatrixMahalanobisDistance(wmsa_means, m_inv_cov_wmsa, v_vals);

            if (x == Gx && y == Gy && z == Gz)
              printf(
                  "WM_mdist is %f and WMSA_mdist is %f for voxel %d, %d, %d\n",
                  wm_mdist, wmsa_mdist, x, y, z);

            if (IS_WM(label)) {
              wm_mdist =
                  MatrixMahalanobisDistance(wm_means, m_inv_cov_wm, v_vals);
              if ((VECTOR_ELT(v_vals, 1) < VECTOR_ELT(wm_means, 1)) &&
                  ...
                  ((7 * wmsa_mdist < wm_mdist) ||
                   (3 * wmsa_mdist < wm_mdist && nwmsa > 3))) {
                MRIsetVoxVal(mri_labeled, x, y, z, 0, wmsa_label);
                changed++;
              }
            }
            if (IS_CAUDATE(label)) {
              caudate_mdist = MatrixMahalanobisDistance(
                  caudate_means, m_inv_cov_caudate, v_vals);
              if ((VECTOR_ELT(v_vals, 1) < VECTOR_ELT(caudate_means, 1)) &&
                  ...
                  ((7 * wmsa_mdist < caudate_mdist) ||
                   (2 * wmsa_mdist < caudate_mdist && nwmsa > 3))) {
                MRIsetVoxVal(mri_labeled, x, y, z, 0, wmsa_label);
                changed++;
              }
            }
            ...
            if (IS_VENTRICLE(label)) {
              vent_mdist =
                  MatrixMahalanobisDistance(vent_means, m_inv_cov_vent, v_vals);
...
int MRIwmsaHalo2(WMSA *wmsa) {
  int ok, i, j, k, a, b, c, d, x, y, z, changed, nwmsa1, nwmsa2, nwmsa, ngm1,
      ngm2, ngm, nmatches, label;
  int     wmsa_labels[]  = {78, 79};
  int     gm_labels[]    = {3, 42};
  double  reftissue_dist = 0, wmsa_mdist;
  MRI *   label_copy     = nullptr;
  MATRIX *v_vals;
  VECTOR *reftissue_means[100], *wmsa_means = nullptr;
  MATRIX *reftissue_covs[100], *reftissue_inv_covs[100],
      *wmsa_covs = nullptr, *wmsa_inv_covs = nullptr;
  ...
  // matrix across all modalities 0 = dont use any neighbors
  printf("size %d\n", (int)sizeof(llabel));
  for (i = 0; i < nreftissues; i++) {
    llabel[0] = wmsa->reftissues[i];
    MRIcomputeLabelMeansandCovariances(wmsa->modalities, wmsa->seg,
                                       &reftissue_covs[i], &reftissue_means[i],
                                       &llabel[0], 1, 0);
    reftissue_inv_covs[i] = MatrixInverse(reftissue_covs[i], nullptr);
  }

  // Means and Cov of WMSA voxels
  MRIcomputeLabelMeansandCovariances(wmsa->modalities, wmsa->seg, &wmsa_covs,
                                     &wmsa_means, wmsa_labels, 2, 0);
  wmsa_inv_covs = MatrixInverse(wmsa_covs, nullptr);
  ...
  for (b = 0; b < wmsa->niters; b++) {
    ...
    // true label file
    label_copy = MRIcopy(wmsa->seg, nullptr);
    ...
    for (x = 0; x < wmsa->seg->width; x++) {
      for (y = 0; y < wmsa->seg->height; y++) {
        for (z = 0; z < wmsa->seg->depth; z++) {
          ...
          nwmsa1 = MRIlabelsInNbhd(label_copy, x, y, z, wmsa->nbrwhalf,
                                   wmsa_labels[0]); // lh
          nwmsa2 = MRIlabelsInNbhd(label_copy, x, y, z, wmsa->nbrwhalf,
                                   wmsa_labels[1]); // rh
          ...
          // should have the same intensity relationship with WMSA as the
          // reference tissue. Use wmsacomp matrix to check if voxel vals should
          // be higher or lower than seg of interest If wmsacomp=1, then
          nmatches = 0;
          ...
          wmsa_mdist =
              MatrixMahalanobisDistance(wmsa_means, wmsa_inv_covs, v_vals);

          // Compute distance to ref tissue
          reftissue_dist = MatrixMahalanobisDistance(
              reftissue_means[c], reftissue_inv_covs[c], v_vals);
          ...
          if (wmsa->softthresh[c] * wmsa_mdist < reftissue_dist &&
              nwmsa > wmsa->nbrthresh)
            ok = 1;
          if (!ok)
            continue;
...
void GCAinitLabelsFromMRI(GCA *gca, MRI *mri_labels) {
  ...
  if (gca->width != mri_labels->width || gca->height != mri_labels->height ||
      gca->depth != mri_labels->depth)
    ErrorExit(ERROR_BADPARM,
              "GCAinitLabelsFromMRI: GCA and MRI must have same dimensions");
...
/*!
\fn unsigned short *GCAmergeLabelLists(unsigned short *labels1, int nlabels1,
unsigned short *labels2, int nlabels2, int *pnlist) \brief Merges two unsigned
short integer lists into a single list. Good for working with GCA label lists.
*/
unsigned short *GCAmergeLabelLists(unsigned short *labels1, int nlabels1,
                                   unsigned short *labels2, int nlabels2,
                                   int *pnlist) {
...
GCA_PRIOR *GCAPcopy(GCA_PRIOR *gcap, int symmetrize, GCA_PRIOR *gcapcopy) {
  ...
  if (gcapcopy == nullptr) {
    ...
    if (gcapcopy->labels)
      free(gcapcopy->labels);
    if (gcapcopy->priors)
      free(gcapcopy->priors);
  }
  gcapcopy->nlabels = gcap->nlabels;
  gcapcopy->labels =
      (unsigned short *)calloc(sizeof(unsigned short), gcap->nlabels);
...
GCA_PRIOR *GCAPmerge(GCA_PRIOR *gcap1, GCA_PRIOR *gcap2, GCA_PRIOR *gcapm) {
  ...
  // Count and make a list of the unique labels from both gcaps
  gcapm->labels         = GCAmergeLabelLists(gcap1->labels, gcap1->nlabels,
                                     gcap2->labels, gcap2->nlabels, &nlabels);
...
int GCANprint(FILE *fp, GCA_NODE *node, int ninputs) {
  ...
  for (n = 0; n < node->nlabels; n++) {
    printf("nthLabel=%d LabelCode=%d (contra=%d)==========\n", n,
           node->labels[n], MRIasegContraLatLabel(node->labels[n]));
...
GC1D *GC1Dcopy(GC1D *gc, int ninputs, int symmetrize, GC1D *gccopy) {
  ...
  if (gccopy == nullptr) {
    ...
    if (gccopy->label_priors) {
      for (r = 0; r < GIBBS_NEIGHBORS; r++) {
        if (gccopy->label_priors[r])
          free(gccopy->label_priors[r]);
      }
  ...
  gccopy->covars = (float *)calloc(sizeof(float),
                                   (ninputs * ninputs - ninputs) / 2 + ninputs);
  v              = 0;
  for (r = 0; r < ninputs; r++) {
  ...
  gccopy->labels =
      (unsigned short **)calloc(sizeof(unsigned short *), GIBBS_NEIGHBORS);
  gccopy->label_priors = (float **)calloc(sizeof(float *), GIBBS_NEIGHBORS);

  for (r = 0; r < GIBBS_NEIGHBORS; r++) {
    rr = r;
    if (symmetrize) {
      // When symmetrizing, the neighbor order reverses for the column face
      // neighbors It is assumed that the col nhbrs are the first two see
      // xnbr_offset[] = {1, -1, 0, 0, 0, 0};
      if (r == 0)
        rr = 1;
      if (r == 1)
        rr = 0;
    ...
    gccopy->labels[r] =
        (unsigned short *)calloc(sizeof(unsigned short), gccopy->nlabels[r]);
    ...
    gccopy->label_priors[r] =
        (float *)calloc(sizeof(float), gccopy->nlabels[r]);
...
GC1D *GC1Dmerge(GC1D *gc1, GC1D *gc2, int ninputs, GC1D *gcm) {
  ...
  if (gcm->label_priors) {
    for (r = 0; r < GIBBS_NEIGHBORS; r++) {
      if (gcm->label_priors[r])
        free(gcm->label_priors[r]);
    }
  ...
  gcm->covars = (float *)calloc(sizeof(float),
                                (ninputs * ninputs - ninputs) / 2 + ninputs);
  v           = 0;
  for (r = 0; r < ninputs; r++) {
  ...
  for (r = 0; r < GIBBS_NEIGHBORS; r++) {
    // Merge the labels
    gcm->labels[r] =
        GCAmergeLabelLists(gc1->labels[r], gc1->nlabels[r], gc2->labels[r],
                           gc2->nlabels[r], &nlabels);
...
/*!
\fn GCA_NODE *GCANmerge(GCA_NODE *node1, GCA_NODE *node2, int ninputs, int
symmetrize, GCA_NODE *nodem) \brief Merges two nodes to create a new node. If
symmetrize=1, then the second node is symmetrized
*/
GCA_NODE *GCANmerge(GCA_NODE *node1, GCA_NODE *node2, int ninputs,
                    int symmetrize, GCA_NODE *nodem) {
  int             n, n1, n2, n1ok, n2ok;
  unsigned short *node2labels;
  GC1D *          gc2;

  if (symmetrize) {
    // Create a new array, convert the node2 labels to contralateral
    // counterparts
    node2labels =
        (unsigned short *)calloc(sizeof(unsigned short), node2->nlabels);
  ...
  if (nodem == nullptr) {
    ...
    if (nodem->labels)
      free(nodem->labels);
    if (nodem->gcs)
      GC1Dfree(&nodem->gcs, ninputs);
  }
  // Get list of unique labels
  nodem->labels = GCAmergeLabelLists(node1->labels, node1->nlabels, node2labels,
                                     node2->nlabels, &nodem->nlabels);
  ...
  for (n = 0; n < nodem->nlabels; n++) {
    ...
    if (n1ok && n2ok) {
      if (symmetrize)
        gc2 = GC1Dcopy(&node2->gcs[n2], ninputs, symmetrize, nullptr);
      else
        gc2 = &node2->gcs[n2];
      GC1Dmerge(&node1->gcs[n1], gc2, ninputs, &nodem->gcs[n]);
      if (symmetrize)
        GC1Dfree(&gc2, ninputs);
      continue;
    }
    ...
    if (n2ok) { // This label not reprsented in node1, just copy GC from node2,
                // sym if needed
...
statistics are computed effectively by averaging the non-flipped with the
flipped. Takes into account the changing of segmentation labels to the
corresponding contralateral code.
 */
  ...
  for (c = 0; c < gca->node_width; c++) {
    ...
    for (r = 0; r < gca->node_height; r++) {
      for (s = 0; s < gca->node_depth; s++) {
        ...
        if (symnode->max_labels > gcasym->max_label)
          gcasym->max_label = symnode->max_labels;
      } // s
    }   // r
  }     // c
  ...
  for (c = 0; c < gca->prior_width; c++) {
    ...
    for (r = 0; r < gca->prior_height; r++) {
      for (s = 0; s < gca->prior_depth; s++) {
        ...
        contraprior = GCAPcopy(&(gca->priors[csym][r][s]), 1, nullptr);
        symprior    = &(gcasym->priors[c][r][s]);
        GCAPmerge(prior, contraprior, symprior);
        if (false && (c == 28 || csym == 28) && r == 51 && s == 51) {
        ...
        if (symprior->max_labels > gcasym->max_label)
          gcasym->max_label = symprior->max_labels;
      } // s
    }   // r
  }     // c
      ...
      for (s = 1; s < gca->node_depth; s++) {
        ...
        if (node->nlabels != symnode->nlabels) {
          printf("Node (%d/%d,%d,%d) not sym nlabels %d %d\n", c, csym, r, s,
                 node->nlabels, symnode->nlabels);
          return (1);
        }

        for (n = 0; n < node->nlabels; n++) {
          // Test that label identity is symmetric. The order might not be the
          // same
          ok = 0;
          ...
          for (k = 0; k < GIBBS_NEIGHBORS; k++) {
            ...
            if (gcs->nlabels[k] != symgcs->nlabels[kk]) {
              printf("Node (%d/%d,%d,%d), label %d, MRF nbr %d,  not sym in "
            ...
            for (m = 0; m < gcs->nlabels[k]; m++) {
              ...
              if (!ok) {
                printf("Node (%d/%d,%d,%d), label %d, MRF nbr %d, cannot find "
                       "a match for label %d/%d\n",
              ...
              if (gcs->label_priors[k][m] != symgcs->label_priors[kk][q]) {
                printf("Node (%d/%d,%d,%d), label %d, MRF nbr %d, probs dont "
                       "match %f %f\n",
                       c, csym, r, s, n, k, gcs->label_priors[k][m],
                       symgcs->label_priors[kk][q]);
                printf("gca -------------------------------\n");
                GC1Dprint(stdout, gcs, gca->ninputs);
                printf("gca sym -------------------------------\n");
                GC1Dprint(stdout, symgcs, gca->ninputs);
  ...
  for (c = 1; c < gca->prior_width; c++) {
    ...
    for (r = 1; r < gca->prior_height; r++) {
      for (s = 1; s < gca->prior_depth; s++) {
        ...
        // if(prior->nlabels < 2 && symprior->nlabels < 2) continue;// not sure
        // why this needed,edge
        if (prior->nlabels != symprior->nlabels) {
          printf("Prior ((%d,%d),%d,%d) not sym nlabels %d %d\n", c, csym, r, s,
                 prior->nlabels, symprior->nlabels);
          return (11);
        }
        ...
        for (n = 0; n < prior->nlabels; n++) {
          ...
          if (!ok) {
            printf("Prior (%d/%d,%d,%d) cannot find label %d in sym\n", c, csym,
                   r, s, prior->labels[n]);
            for (m = 0; m < prior->nlabels; m++)
              printf(" %d", prior->labels[m]);
            printf("\n");
            for (m = 0; m < prior->nlabels; m++)
              printf(" %d", symprior->labels[m]);
            printf("\n");
            return (12);
          }
          // Test whether the probability of this label at this point is
          // symmetric
          if (prior->priors[n] != symprior->priors[m]) {
            printf("Prior (%d/%d,%d,%d) not sym pval n=%d m=%d   %lf %lf\n", c,
                   csym, r, s, n, m, prior->priors[n], symprior->priors[n]);
            return (13);
          }
        }
      } // s
