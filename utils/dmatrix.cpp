...
/*!
  ...
  \brief Checks DMATRIX operations against their float Matrix counterparts.
  These are only negative tests (ie, expect it not to fail).
*/
int DMatrixTest() {
  DMATRIX *m1d, *m2d, *md;
  MATRIX *m1f, *m2f, *mf;
  ...
  md = DMatrixConstVal(14, 7, 2, nullptr);
  if (md == nullptr)
    return (1);
  ...
  m1f = MatrixDRand48(14, 7, nullptr);
  m2f = MatrixDRand48(14, 7, nullptr);
  ...
  mf = MatrixScalarMul(m1f, 2.3, nullptr);
  md = DMatrixScalarMul(m1d, 2.3, nullptr);
  if (md == nullptr)
    return (1);
  v = DMatrixCompareFMatrix(mf, md);
  if (v > .0001) {
  ...
  DMatrixFree(&m1d);
  DMatrixFree(&m2d);
  m1f = MatrixDRand48(7, 14, nullptr);
  m2f = MatrixDRand48(14, 7, nullptr);
  m1d = DMatrixCopyFMatrix(m1f, nullptr);
  m2d = DMatrixCopyFMatrix(m2f, nullptr);
  ...
  if (err)
    printf("DMatrix: failed with %d errors \n", err);
  else
    printf("DMatrix: passed all tests\n");

  return (err);
}

/*!
  \fn int DMatrixCheckDims(const DMATRIX *m1, const DMATRIX *m2, const int
  checktype, FILE *fp, const char *str) \brief Checks the dimensions of two
  matrices, returns 0 if no error. Returns non-zero if either matrix is NULL.
...
DMATRIX *DMatrixAlloc(const int rows, const int cols, const int type) {
  ...
  if (!mat)
    ErrorExit(ERROR_NO_MEMORY,
              "DMatrixAlloc(%d, %d, %d): could not allocate mat", rows, cols,
              type);
  ...
  */
  ...
  if (!mat->rptr) {
    free(mat->data);
    free(mat);
    ErrorExit(ERROR_NO_MEMORY, "DMatrixAlloc(%d, %d): could not allocate rptr",
              rows, cols);
  }
...
/*!
  \fn DMATRIX *DMatrixMultiply(const DMATRIX *m1, const DMATRIX *m2, DMATRIX
  *m3) \brief Multiplies two matrices. The accumulation is done with long
  double.
*/
...
int DMatrixPrintFmt(FILE *fp, const char *fmt, DMATRIX *mat) {
  ...
  if (mat == nullptr)
    ErrorReturn(ERROR_BADPARM, (ERROR_BADPARM, "DMatrixPrint: mat = NULL!"));
...
/*!
  \fn DMATRIX *DMatrixAddMul(DMATRIX *m1, DMATRIX *m2, double v1, double v2,
  DMATRIX *mOut) \brief Computes v1*m1+v2*m2 so it can be used for adding,
  subtracting, and scaling. If a matrix is NULL or its value is 0, it is
  ignored.
*/
...
DMATRIX *DMatrixScalarMul(DMATRIX *m, double v, DMATRIX *mout) {
  ...
  return (DMatrixAddMul(m, nullptr, v, 0, mout));
}
...
double DVectorDot(const DVECTOR *v1, const DVECTOR *v2) {
  ...
  dot = 0.0;
  for (i = 1; i <= v1->rows; i++)
    dot += ((long double)v1->rptr[i][1] * v2->rptr[i][1]);
  return (dot);
}
...
/*!
  \fn DVECTOR *DVectorCrossProduct(const DVECTOR *v1, const DVECTOR *v2, DVECTOR
...
DVECTOR *DVectorCrossProduct(const DVECTOR *v1, const DVECTOR *v2,
  ...
  if (!vdst)
    vdst = DMatrixAlloc(3, 1, MATRIX_REAL);
...
DMATRIX *DMatrixDRand48(int rows, int cols, DMATRIX *m) {
  int r, c;
  if (m == nullptr)
    m = DMatrixAlloc(rows, cols, MATRIX_REAL);
