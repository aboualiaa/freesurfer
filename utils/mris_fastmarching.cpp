...
static MRI_REGION *mriFindLabel(MRI *mri, int label, int offset) {
  ...
  if (nlabels == 0)
    ErrorExit(1, "No labels %d could be found in the volume\n", nlabels);
...
void MRISextractOutsideDistanceMap(MRIS *mris, MRI *mri_src, int label,
  ...
  mri_fastmarching = MRIclone(mri_distance, nullptr);

  /* set values to zero */
  mapMRI_XYZ(mri_fastmarching, x, y, z) MRIFvox(mri_fastmarching, x, y, z) =
      0.0f;

  FastMarching<+1> fastmarching_out(mri_fastmarching, nullptr);
  ...
  mapMRI_XYZ(mri_fastmarching, x, y, z) if (MRIFvox(mri_distance, x, y, z) < 0)
