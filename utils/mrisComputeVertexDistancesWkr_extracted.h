...
static bool FUNCTION_NAME(
  ...
  if (false && mris->status != mris->INPUT_STATUS) {
    ...
    fprintf(stdout, "%s:%d %s mris->status:%s != mris->INPUT_STATUS:%s\n",
            __FILE__, __LINE__, __MYFUNCTION__, MRIS_Status_text(mris->status),
...
#ifdef COMPILING_MRIS_MP
  if (!mris->v_dist_buffer)
    mris->v_dist_buffer = (float **)calloc(mris->nvertices, sizeof(float *));
#endif
...
#pragma omp parallel for if_ROMP(shown_reproducible) reduction(+:errors)  reduction(+:nonZeroInputXCount)
...
#else
        float const vnx = mris->INPUT_X[vnon], vny = mris->INPUT_Y[vnon],
                    vnz = mris->INPUT_Z[vnon];
#endif
        float xd = vx - vnx;
        float yd = vy - vny;
        float zd = vz - vnz;
  ...
  case MRIS_Status_DistanceFormula_1: {
    // Sped up by computing the normalized vectors for all the vertices once
    // rather than repeatedly and storing them along with their ripflag in a
    // structure!
    //
...
#pragma omp parallel for if_ROMP(shown_reproducible) reduction(+:errors) reduction(+:nonZeroInputXCount)
      ...
      XYZ_NORMALIZED_LOAD(&vertexInfo[vno].xyz_normalized,
                          &vertexInfo[vno].xyz_length, vx, vy, vz);
      // length 1 along radius vector

      if (FZERO(vertexInfo[vno].xyz_length))
        vertexInfo[vno].rippedOrZeroLength = (char)true;
...
#pragma omp parallel for if_ROMP(shown_reproducible) reduction(+ : errors)
      ...
      if (vi->ripped)
        continue;
      // Non-ripflag with a radius of zero still need to get their distances set
      // to zero
      ...
      for (n = 0; n < vtotal; n++) {
        ...
        if (!vi2->rippedOrZeroLength) {
          ...
          float angle = fabs(XYZApproxAngle_knownLength(
              &vi->xyz_normalized, vi2->xyz_unnormalized.x,
              vi2->xyz_unnormalized.y, vi2->xyz_unnormalized.z,
              vi2->xyz_length));
          // radians, so 2pi around the circumference

          d = angle * radius;
          // the length of the arc, rather than the straight line distance
          // but this definition is still weird because the two points are not
          // equal radius from center so the distance from a to b might not be
          // the same as b to a! the vertices are usually very close and far
          // from the origin, making this almost irrelevant.
        }
