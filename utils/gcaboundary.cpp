...
static int gcabUpdateDistributions(GCAB *gcab, MRI *mri_int, MRI *mri_seg,
                                   MRI *mri_dist, TRANSFORM *transform, int x,
...
#define PAD 5
  ...
  for (x = 0; x < width; x++) {
    ...
    for (y = 0; y < height; y++) {
      ...
      if (!gcab->bs[x][y])
        ErrorExit(ERROR_NOMEMORY, "GCABalloc: could not allocate %d,%dth *", x,
                  y);
      for (z = 0; z < depth; z++) {
        if (x == Gx && y == Gy && z == Gz)
          DiagBreak();
        ...
        for (vno = 0; vno < gcab->nvertices; vno++) {
          gcabs->pdfs[vno] =
              (double **)calloc(nintensity_bins, sizeof(double *));
          gcabs->h_grad_pdfs[vno] = HISTOinit(nullptr, gcab->ngrad_bins,
                                              gcab->min_grad, gcab->max_grad);
          gcabs->h_Iin_pdfs[vno] =
              HISTOinit(nullptr, gcab->nintensity_bins, gcab->min_intensity,
                        gcab->max_intensity);
          gcabs->h_Iout_pdfs[vno] =
              HISTOinit(nullptr, gcab->nintensity_bins, gcab->min_intensity,
                        gcab->max_intensity);
    ...
    for (y = 0; y < gcab->height; y++)
      for (z = 0; z < gcab->depth; z++) {
        ...
        for (vno = 0; vno < gcab->ico->nvertices; vno++) {
          ...
          //          HISTOsmooth(gcabs->h_Iin_pdfs[vno],
          //          gcabs->h_Iin_pdfs[vno], .5) ;
          //          HISTOsmooth(gcabs->h_Iout_pdfs[vno],
          //          gcabs->h_Iout_pdfs[vno], .5) ;
          //          HISTOsmooth(gcabs->h_grad_pdfs[vno],
          //          gcabs->h_grad_pdfs[vno], .5) ;
...
double GCABgetProbability(GCAB *gcab, MRI *mri_int, MRI *mri_dist,
  ...
  // Note: warnings fix: commented out variables should be refactored into the
  // #else below if ever used again - clarsen

  float xn, yn, zn, xi, yi, zi, xo, yo, zo, xi0, yi0, zi0, d, dx, dy, dz, xo0,
      yo0, zo0;
  // float  xd, yd, zd;
  int vno, oob, x, y, z, num;
  // int xu, yu, zu, xb, yb, zb;
  double grad, p, pdelI, mag, val1, val2, pI_in, pI_out, e1x, e2x, e1y, e2y,
      e1z, e2z, d1, d2;
  // double pI_total;
  GCABS *gcabs;

  oob = GCABsourceVoxelToNode(gcab, mri_int, transform, x0, y0, z0, &xn, &yn,
                              &zn);
  if (oob != NO_ERROR)
    return (0.0);
  xo = x0 + SDIST * nx;
  ...
  dy /= d;
  dz /= d;
  MRIsampleVolumeDerivativeScale(mri_int, x0, y0, z0, nx, ny, nz, &grad,
                                 GRAD_SIGMA);
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    ...
    mri = gcabWritePDFToMRI(gcab, nullptr, x, y, z);
    ...
    for (y = yb; y <= yu; y++) {
      for (z = zb; z <= zu; z++) {
        ...
        for (d = 0.0, num = 0; d > -10; d--) // start 1/2mm inside border
        {
          ...
          yi = yi0 + d * dy;
          zi = zi0 + d * dz;
          MRIsampleVolumeDerivativeScale(mri_dist, xi, yi, zi, dx, dy, dz, &mag,
                                         0);
          if (mag < 0) // closer to the opposite side of the surface
            break;
          num++;
...
double GCABgetPin(GCAB *gcab, MRI *mri_int, MRI *mri_dist, TRANSFORM *transform,
                  float x0, float y0, float z0, float nx, float ny, float nz) {
...
double GCABgetPout(GCAB *gcab, MRI *mri_int, MRI *mri_dist,
                   TRANSFORM *transform, float x0, float y0, float z0, float nx,
                   float ny, float nz) {
  float xn, yn, zn, xi, yi, zi, xo, yo, zo, xi0, yi0, zi0, d, dx, dy, dz, xo0,
      yo0, zo0;
  // float xd, yd, zd;
...
int GCABtrain(GCAB *gcab, MRI *mri_int, MRI *mri_seg, TRANSFORM *transform,
  ...
  mri_dist = MRIdistanceTransform(mri_tmp, nullptr, target_label, 10,
                                  DTRANS_MODE_SIGNED, nullptr);
  MRIfree(&mri_tmp);

  gcab->ntraining++; // # of volumes that went into this atlas
  mri_border = MRImarkLabelBorderVoxels(mri_seg, nullptr, target_label, 1, 1);
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
  ...
  for (x = 0; x < mri_int->width; x++) {
    for (y = 0; y < mri_int->height; y++) {
      for (z = 0; z < mri_int->depth; z++) {
        ...
        if (border == 0)
          continue;
        oob = GCABsourceVoxelToNode(gcab, mri_int, transform, x, y, z, &xn, &yn,
                                    &zn);
        if (oob != NO_ERROR) // not in bounding box
          continue;
        DiagBreak();
        gcabUpdateDistributions(gcab, mri_int, mri_seg, mri_dist, transform, x,
                                y, z, xn, yn, zn, target_label);
      }
    }
  }
...
int GCABsourceVoxelToNode(GCAB *gcab, MRI *mri, TRANSFORM *transform, float xv,
  ...
  if (Ggca_x == xv && Ggca_y == yv && Ggca_z == zv && DIAG_VERBOSE_ON)
    fprintf(stdout,
            "source (%2.1f, %2.1f, %2.1f) to talposition (%.2f, %.2f, %.2f)\n",
            xv, yv, zv, xt, yt, zt);
  fflush(stdout);
...
////////////////////////////////////////////////////////////////
int GCABvoxelToNode(GCAB *gcab, float xv, float yv, float zv, float *pxn,
                    float *pyn, float *pzn) {
  float xn, yn, zn;
  ...
  if (xn < 0 || yn < 0 || zn < 0 || xn >= gcab->width || yn >= gcab->height ||
      zn >= gcab->depth)
...
int GCABwrite(GCAB *gcab, char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE, "GCABwrite(%s) fopen failed", fname));
  ...
  if (where == -1L) {
    ErrorPrintf(ERROR_BADFILE,
                "GCABwrite(%s) ftell returned invalid position indicator",
                fname);
  }
...
GCAB *GCABread(char *fname, GCA *gca) {
  ...
  if (gca == nullptr) {
    ...
    if (gca == nullptr)
      ErrorReturn(NULL,
                  (ERROR_BADFILE, "GCABread(%s): could not read gca from %s",
                   fname, gca_fname));
  }
  where = ftell(fp);
  if (where == -1L) {
...
static int gcabUpdateDistributions(GCAB *gcab, MRI *mri_int, MRI *mri_seg,
                                   ...
                                   int y, int z, float xn, float yn, float zn,
                                   int target_label) {
  float nx, ny, nz, xi, yi, zi, xo, yo, zo, dx, dy, dz, d, xi0, yi0, zi0, xo0,
      yo0, zo0, x0, y0, z0;
  // float xd, yd, zd;
  int vno, label;
  // int xb, yb, zb, xu, yu, zu;
  ...
  vno = gcabComputeBorderNormal(gcab, mri_seg, transform, x, y, z, &nx, &ny,
                                &nz, target_label);
  if (vno < 0)
...
static int gcabUpdateNodeIout(GCAB *gcab, int xn, int yn, int zn, double val2,
  ...
  if (xn < 0 || xn >= gcab->width || yn < 0 || yn >= gcab->height || zn < 0 ||
      zn >= gcab->depth)
    return (NO_ERROR);
...
static MRI *gcabWritePDFToMRI(GCAB *gcab, MRI *mri, int x, int y, int z) {
  ...
  if (mri == nullptr)
    mri = MRIalloc(gcab->nvertices, gcab->nintensity_bins,
                   gcab->nintensity_bins, MRI_FLOAT);
...
static int gcabSmoothPDF(GCAB *gcab, int x, int y, int z, MRI *mri_kernel) {
  ...
  // depth = mri_pdf->depth;
  mri_tmp = MRIclone(mri_pdf, nullptr);
