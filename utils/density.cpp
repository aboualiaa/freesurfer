...
DENSITY *DensityHistogramEstimate(MRI *mri1, MRI *mri2, int nbins, float sigma,
  ...
  if (!pdf->valid1 || !pdf->valid2)
    ErrorExit(
        ERROR_NOMEMORY,
        "DensityHistogramEstimate: could not allocate lookup tables (%d, %d)\n",
        n1, n2);
  if (valid1) {
    for (n = 0; n < n1; n++)
  ...
  if (sigma > 0) {
    ...
    Ismooth = ImageConvolveGaussian(pdf->Ipdf, Ikernel, nullptr, 0);
    ImageFree(&pdf->Ipdf);
    ImageFree(&Ikernel);
    pdf->Ipdf = Ismooth;
    if ((Gdiag & DIAG_WRITE) && DIAG_VERBOSE_ON)
...
int DensityWrite(DENSITY *pdf, char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(
        ERROR_BADPARM,
        (ERROR_BADPARM, "DensityWrite(%s): could not open file", fname));
...
DENSITY *DensityRead(char *fname) {
  ...
  if (pdf->Ipdf == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "DensityRead(%d): could not allocate density image", nbins);
  if (fscanf(fp,
             ":vol1 %f %f\n:vol2 %f %f\n:sigma %f\n:dof %d\n:mri1 %s\n:mri2 "
             "%s\n:valid1 ",
             &pdf->min_val1, &pdf->max_val1, &pdf->min_val2, &pdf->max_val2,
             &pdf->sigma, &pdf->dof, pdf->fname1, pdf->fname2) != 8) {
    ErrorPrintf(ERROR_BAD_FILE, "DensityRead(%s): could not read parameter(s)",
                fname);
  }
  ...
  if (!pdf->valid1)
    ErrorExit(
        ERROR_NOMEMORY,
        "DensityHistogramEstimate: could not allocate lookup tables (%d)\n", n);
  for (i = 0; i < n; i++) {
    if (fscanf(fp, "%d ", &pdf->valid1[i]) != 1) {
      ErrorPrintf(ERROR_BAD_FILE,
                  "DensityRead(%s): could not read parameter(s)", fname);
    }
  }
  if (fscanf(fp, "\n:valid2 ") != 0) {
  ...
  for (i = 0; i < pdf->Ipdf->rows; i++) {
    for (j = 0; j < pdf->Ipdf->cols; j++) {
      ...
      if (!DZERO(*IMAGEFpix(pdf->Ipdf, i, j)) &&
          (*IMAGEFpix(pdf->Ipdf, i, j) < pdf->min_p)) {
        pdf->min_p = *IMAGEFpix(pdf->Ipdf, i, j);
      }
    }
    if (fscanf(fp, "\n") != 0) {
      ErrorPrintf(ERROR_BAD_FILE, "DensityRead(%s): reached end of file",
                  fname);
    }
  }
