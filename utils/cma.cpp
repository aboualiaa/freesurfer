...
CMAoutlineField *CMAoutlineFieldAlloc(int width, int height) {
  ...
  if (of->outline_points_field == nullptr) {
    ...
    ErrorReturn(
        NULL,
        (ERROR_NOMEMORY,
         "CMAoutlineFieldAlloc(): error allocating outline points field"));
  }
...
int CMAclaimPoints(CMAoutlineField *field, short label, short *points,
  ...
  for (i = 0; i < n_points; i++) {
    ...
    if (x < 0 || x >= field->width)
      ErrorReturn(
          ERROR_BADPARM,
          (ERROR_BADPARM, "CMAclaimPoints(): outline point out of range (x)"));
    ...
    for (j = 0; j < field->height; j++) {
      ...
      if (field->fill_field[j][i] == CMA_FILL_OUTLINE) {
        if (field->claim_field[j][i].n_claims < MAX_OUTLINE_CLAIMS) {
          field->claim_field[j][i]
              .claim_labels[field->claim_field[j][i].n_claims] = label;
...
int CMAaddWeightedTotals(CMAoutlineClaim *claim, float weight,
                         float *claim_totals) {
  int i;
...
short CMAtotalClaims(CMAoutlineField *field, int x, int y) {
  ...
  if (x < 0 || x >= field->width)
    ErrorReturn(-1,
                (ERROR_BADPARM,
                 "CMAtotalClaims(): x out of range (x = %d, field->width = %d)",
                 x, field->width));

  if (y < 0 || y >= field->height)
    ErrorReturn(
        -1, (ERROR_BADPARM,
             "CMAtotalClaims(): y out of range (y = %d, field->height = %d)", y,
             field->height));
  ...
  CMAaddWeightedTotals(&(field->claim_field[y][x]), OTL_CLAIM_WEIGHT_CENTER,
                       claim_totals);

  /* immediately adjoining points */
  if (x - 1 >= 0)
    CMAaddWeightedTotals(&(field->claim_field[y][x - 1]), OTL_CLAIM_WEIGHT_SIDE,
                         claim_totals);
  if (x + 1 < field->width)
    CMAaddWeightedTotals(&(field->claim_field[y][x + 1]), OTL_CLAIM_WEIGHT_SIDE,
                         claim_totals);
  if (y - 1 >= 0)
    CMAaddWeightedTotals(&(field->claim_field[y - 1][x]), OTL_CLAIM_WEIGHT_SIDE,
                         claim_totals);
  if (y + 1 < field->height)
    CMAaddWeightedTotals(&(field->claim_field[y + 1][x]), OTL_CLAIM_WEIGHT_SIDE,
                         claim_totals);
  ...
  if (x - 1 >= 0 && y - 1 >= 0)
    CMAaddWeightedTotals(&(field->claim_field[y - 1][x - 1]),
                         OTL_CLAIM_WEIGHT_SIDE, claim_totals);
...
int insert_ribbon_into_aseg(MRI *mri_src_aseg, MRI *mri_aseg,
  ...
  gm_label =
      hemi == LEFT_HEMISPHERE ? Left_Cerebral_Cortex : Right_Cerebral_Cortex;
  ...
  for (x = 0; x < mri_aseg->width; x++)
    for (y = 0; y < mri_aseg->height; y++)
      for (z = 0; z < mri_aseg->depth; z++) {
        ...
        if (nint(MRIgetVoxVal(mri_ribbon, x, y, z, 0)) ==
            255) // in ribbon, set to GM
        {
        ...
        {
          if (MRIgetVoxVal(mri_white, x, y, z, 0) >
              0) // inside white surface - disambiguate
          {
  ...
  for (c = 0; c < aseg->width; c++) {
    for (r = 0; r < aseg->height; r++) {
      for (s = 0; s < aseg->depth; s++) {
        ...
        if (asegid == 2 || asegid == 41 || asegid == 3 || asegid == 42 ||
            asegid == 0) {
...
std::vector<double> ComputeBrainVolumeStats(const std::string &subject,
  ...
  if (!FileExists(fname.c_str())) {
    std::cout << fname << " doesn't exist - using old naming conventions"
              << std::endl;
    fname = subjfile("mri/aseg.mgz");
    if (!FileExists(fname.c_str()))
      fname = subjfile("mri/aseg.long.mgz");
  }
  MRI *aseg = MRIread(fname.c_str());
  if (!aseg)
    fs::fatal() << "cannot compute vol stats without " << fname;
  ...
  for (int c = 0; c < aseg->width; c++) {
    for (int r = 0; r < aseg->height; r++) {
      for (int s = 0; s < aseg->depth; s++) {
        ...
        if (asegfixedid == 41 || asegfixedid == 79 ||
            (asegfixedid == 77 && c >= 128))
          rhCtxWMCount += VoxelVol;
      }
    }
  }
  ...
  double eBSVnvSurf = lhCtxGM + rhCtxGM + lhCtxWM + rhCtxWM + SubCortGMVol +
                      CerebellumVol + VesselVol;
...
void CacheBrainVolumeStats(const std::vector<double> &stats,
  ...
  if (!statsfile)
    fs::fatal() << "could not open file " << filename << " for writing";

  statsfile << std::fixed << std::setprecision(12);
  statsfile << "# Measure BrainSeg, BrainSegVol, Brain Segmentation Volume, "
            << stats[0] << ", mm^3" << std::endl;
  statsfile << "# Measure BrainSegNotVent, BrainSegVolNotVent, Brain "
               "Segmentation Volume Without Ventricles, "
            << stats[1] << ", mm^3" << std::endl;
  statsfile
      << "# Measure SupraTentorial, SupraTentorialVol, Supratentorial volume, "
      << stats[2] << ", mm^3" << std::endl;
  statsfile << "# Measure SupraTentorialNotVent, SupraTentorialVolNotVent, "
               "Supratentorial volume, "
            << stats[3] << ", mm^3" << std::endl;
  statsfile << "# Measure SubCortGray, SubCortGrayVol, Subcortical gray matter "
               "volume, "
            << stats[4] << ", mm^3" << std::endl;
  statsfile << "# Measure lhCortex, lhCortexVol, Left hemisphere cortical gray "
               "matter volume, "
            << stats[5] << ", mm^3" << std::endl;
  statsfile << "# Measure rhCortex, rhCortexVol, Right hemisphere cortical "
               "gray matter volume, "
            << stats[6] << ", mm^3" << std::endl;
  statsfile
      << "# Measure Cortex, CortexVol, Total cortical gray matter volume, "
      << stats[7] << ", mm^3" << std::endl;
  statsfile << "# Measure TotalGray, TotalGrayVol, Total gray matter volume, "
            << stats[8] << ", mm^3" << std::endl;
  statsfile << "# Measure lhCerebralWhiteMatter, lhCerebralWhiteMatterVol, "
               "Left hemisphere cerebral white matter volume, "
            << stats[9] << ", mm^3" << std::endl;
  statsfile << "# Measure rhCerebralWhiteMatter, rhCerebralWhiteMatterVol, "
               "Right hemisphere cerebral white matter volume, "
            << stats[10] << ", mm^3" << std::endl;
  statsfile << "# Measure CerebralWhiteMatter, CerebralWhiteMatterVol, Total "
               "cerebral white matter volume, "
            << stats[11] << ", mm^3" << std::endl;
  statsfile << "# Measure Mask, MaskVol, Mask Volume, " << stats[12] << ", mm^3"
            << std::endl;
  statsfile
      << "# Measure SupraTentorialNotVentVox, SupraTentorialVolNotVentVox, "
         "Supratentorial volume voxel count, "
      << stats[13] << ", mm^3" << std::endl;
  statsfile << "# Measure BrainSegNotVentSurf, BrainSegVolNotVentSurf, Brain "
               "Segmentation Volume Without Ventricles from Surf, "
            << stats[14] << ", mm^3" << std::endl;
  statsfile << "# Measure VentricleChoroidVol, VentricleChoroidVol, Volume of "
               "ventricles and choroid plexus, "
            << stats[15] << ", mm^3" << std::endl;
}
...
std::vector<double> ReadCachedBrainVolumeStats(const std::string &subject,
  ...
  for (int n = 0; n < nstats; n++) {
    if (stats[n] < 0)
      fs::fatal() << "could not find stat for " << volumeNames[n] << " in "
                  << filename;
  }
  ...
  for (c = 0; c < seg->width; c++) {
    for (r = 0; r < seg->height; r++) {
      for (s = 0; s < seg->depth; s++) {
        ...
        if (ct->entries[segid]->TissueType < 0) {
          printf(
              "ERROR: CheckSegTissueType() tissue type for seg %d %s not set\n",
              segid, ct->entries[segid]->name);
          return (err);
        }
