...
static double rfFeatureInfoGain(RANDOM_FOREST *rf, TREE *tree, NODE *parent,
                                NODE *left, NODE *right, int fno,
                                int *ptotal_count);
...
RANDOM_FOREST *RFalloc(int ntrees, int nfeatures, int nclasses, int max_depth,
  ...
  if (rf->class_names == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFalloc(%d, %d, %d, %d): could not allocate class names", ntrees,
              nfeatures, nclasses, max_depth);
  if (class_names) {
    for (c = 0; c < rf->nclasses; c++) {
      rf->class_names[c] =
          (char *)calloc(strlen(class_names[c]) + 1, sizeof(char));
      if (rf->class_names[c] == nullptr)
        ErrorExit(ERROR_NOMEMORY,
  ...
  if (rf->trees == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFalloc(%d, %d, %d, %d): could not allocate trees", ntrees,
              nfeatures, nclasses, max_depth);
  rf->feature_min = (double *)calloc(rf->nfeatures, sizeof(rf->feature_min[0]));
  if (rf->feature_min == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFalloc(%d, %d, %d, %d): could not allocate fmin", ntrees,
              nfeatures, nclasses, max_depth);
  rf->feature_max = (double *)calloc(rf->nfeatures, sizeof(rf->feature_max[0]));
  if (rf->feature_max == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFalloc(%d, %d, %d, %d): could not allocate fmax", ntrees,
              nfeatures, nclasses, max_depth);
  for (n = 0; n < rf->ntrees; n++) {
    tree = &rf->trees[n];
    tree->root.class_counts =
        (int *)calloc(rf->nclasses, sizeof(tree->root.class_counts[0]));
    if (tree->root.class_counts == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "RFalloc(%d, %d, %d, %d): could not allocate root %d", ntrees,
                nfeatures, nclasses, max_depth, c);
  }
  return (rf);
}

static double compute_info_gain(RF *rf, TREE *tree, NODE *parent, NODE *left,
                                NODE *right, double **training_data, int fno,
                                double thresh) {
  ...
  entropy_before =
      entropy(parent->class_counts, rf->nclasses, tree->root.class_counts);
...
static int adjust_optimal_threshold(RF *rf, TREE *tree, NODE *parent,
                                    NODE *left, NODE *right,
                                    double **training_data, int fno,
                                    double *pbest_thresh) {
  double previous_thresh, next_thresh, info_gain, best_info_gain, step, thresh,
      best_thresh;

  best_thresh    = *pbest_thresh;
  best_info_gain = compute_info_gain(rf, tree, parent, left, right,
                                     training_data, fno, best_thresh);
  if (rf->min_step_size > 0)
    step = rf->min_step_size;
  else
    step =
        (rf->feature_max[fno] - rf->feature_min[fno]) / (10 * rf->nsteps - 1);
  for (thresh = best_thresh; thresh <= rf->feature_max[fno]; thresh += step) {
    info_gain = compute_info_gain(rf, tree, parent, left, right, training_data,
                                  fno, thresh);
    if (info_gain < 0)
      DiagBreak();
  ...
  if (info_gain < best_info_gain) // don't use it
    compute_info_gain(rf, tree, parent, left, right, training_data, fno,
                      best_thresh);
  else
    best_thresh = thresh;
...
static double find_optimal_threshold(RF *rf, TREE *tree, NODE *parent,
                                     NODE *left, NODE *right,
                                     double **training_data, int ntraining,
                                     int fno, double *pinfo_gain, int nsteps,
  ...
  for (thresh = fmin; thresh < fmax; thresh += step) {
    ...
    if (info_gain < 0)
      DiagBreak();
    if (info_gain > best_info_gain && left->total_counts > 0 &&
        right->total_counts > 0) {
      best_info_gain = info_gain;
      best_thresh    = thresh;
    }
  }
...
static int find_optimal_feature_and_threshold(
    RANDOM_FOREST *rf, TREE *tree, NODE *parent, NODE *left, NODE *right,
    int *training_classes, double **training_data, int ntraining) {
  ...
  for (f = 0; f < tree->nfeatures; f++) {
    ...
    do {
      thresh = find_optimal_threshold(rf, tree, parent, left, right,
                                      rf->training_data, rf->ntraining, fno,
                                      &info_gain, nsteps, fmin, fmax);
      if (info_gain < 0)
        DiagBreak();
      nsteps *= 5;
  ...
  info_gain = compute_info_gain(rf, tree, parent, left, right, training_data,
                                best_f, best_thresh);
  adjust_optimal_threshold(rf, tree, parent, left, right, training_data, best_f,
                           &best_thresh);
...
static NODE *rfAllocateNode(int ntraining, int depth, int nclasses) {
  ...
  if (node->class_counts == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "rfAllocateNode: could not allocate node class counts");
  node->training_set = (int *)calloc(ntraining, sizeof(node->training_set[0]));
  if (node->training_set == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "rfAllocateNode: could not allocate node training_set");
  node->depth = depth;
  return (node);
}
...
static int rfTrainNode(RANDOM_FOREST *rf, TREE *tree, NODE *node,
                       int *training_classes, double **training_data,
                       int ntraining) {
  if (node->left == nullptr) // not trained yet
  {
    node->left =
        rfAllocateNode(node->total_counts, node->depth + 1, rf->nclasses);
    node->right =
        rfAllocateNode(node->total_counts, node->depth + 1, rf->nclasses);
    if (find_optimal_feature_and_threshold(rf, tree, node, node->left,
                                           node->right, training_classes,
  ...
  if (node->depth < rf->max_depth) {
    if (entropy(node->left->class_counts, rf->nclasses,
                tree->root.class_counts) > 0)
      rfTrainNode(rf, tree, node->left, training_classes, training_data,
                  ntraining);
    if (entropy(node->right->class_counts, rf->nclasses,
                tree->root.class_counts) > 0)
      rfTrainNode(rf, tree, node->right, training_classes, training_data,
                  ntraining);
  }
  return (1);
}
...
static int rfTrainTree(RANDOM_FOREST *rf, TREE *tree, int *training_classes,
                       double **training_data, int ntraining) {
  ...
  while (!done && !FZERO(total_entropy)) {
    done = rfTrainNode(rf, tree, &tree->root, training_classes, training_data,
                       rf->ntraining);
    rfFindLeaves(tree);
...
int RFtrain(RANDOM_FOREST *rf, double feature_fraction,
            double training_fraction, int *training_classes,
            double **training_data, int ntraining) {
  int n, ii, nfeatures_per_tree = 0, *feature_permutation,
             *training_permutation, f, index, start_no, end_no,
             ntraining_per_tree = 0, total_to_remove = 0;
  TREE *tree = nullptr;

  if (rf->max_class_ratio > 0) {
    int class_counts[MAX_CLASSES], max_class, max_class_count, min_class,
        min_class_count;
    ...
    for (min_class_count = ntraining + 1,
        min_class = max_class = max_class_count = n = 0;
         n < rf->nclasses; n++) {
    ...
    total_to_remove =
        (max_class_count - nint(min_class_count * rf->max_class_ratio));
    if (total_to_remove > 0) {
      int *class_indices, class_index, new_index,
          new_ntraining = ntraining - total_to_remove;

      printf("class %s (%d) has too many examples (%d) relative to class %s "
             "(%d) with %d\n",
      ...
      for (class_index = new_index = n = 0; n < ntraining; n++) {
        ...
        else // copy over other class features and class
        {
          ...
          new_training_data[new_index] =
              (double *)calloc(rf->nfeatures, sizeof(double));
      ...
      for (n = 0; n < max_class_count - total_to_remove; n++) {
        ...
        new_training_data[new_index] =
            (double *)calloc(rf->nfeatures, sizeof(double));
        for (ii = 0; ii < rf->nfeatures; ii++)
          new_training_data[new_index][ii] =
              training_data[class_indices[new_index]][ii];
        new_index++;
      }
  ...
  feature_permutation  = compute_permutation(rf->nfeatures, nullptr);
  training_permutation = compute_permutation(ntraining, nullptr);
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(tree, start_no, end_no, ii, index)                            \
        shared(rf, nfeatures_per_tree, Gdiag, training_classes, training_data) \
            schedule(static, 1)
#endif
...
#pragma omp critical
    ...
    tree->feature_list =
        (int *)calloc(nfeatures_per_tree, sizeof(tree->feature_list[0]));
    if (tree->feature_list == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "RFtrain: could not allocate feature list %d (%d)", n,
                nfeatures_per_tree);
    tree->nfeatures = nfeatures_per_tree;
    if (rf->ntrees > 1)
      start_no = nint(n * ((double)(rf->nfeatures - nfeatures_per_tree)) /
                      (rf->ntrees - 1.0));
    else
      start_no = 0; // only 1 tree
    ...
    if (tree->root.training_set == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "RFtrain: could not allocate root training set");
    tree->root.total_counts = 0;
    if (rf->ntrees > 1)
      start_no = nint(n * ((double)(rf->ntraining - ntraining_per_tree)) /
                      (rf->ntrees - 1.0));
    else
      start_no = 0; // only 1 tree
    ...
    for (ii = start_no; ii <= end_no; ii++) {
      ...
      if (training_classes[index] < 0 ||
          training_classes[index] >= rf->nclasses) {
...
int RFtrainTree(RANDOM_FOREST *rf, int tno, int *training_classes,
                double **training_data, int ntraining) {
  int   i, f;
  TREE *tree;
  ...
  tree->feature_list =
      (int *)calloc(rf->nfeatures, sizeof(tree->feature_list[0]));
  if (tree->feature_list == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFtrain: could not allocate feature list %d (%d)", tno,
              rf->nfeatures);
  ...
  if (tree->root.training_set == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFtrainTree: could not allocate root training set");

  for (i = 0; i < ntraining; i++) {
    tree->root.class_counts[training_classes[i]]++;
    tree->root.training_set[tree->root.total_counts] =
        i; // should be +ntraining
    tree->root.total_counts++;
  }
  rf->ntraining += ntraining;
...
static int rfWriteNode(RANDOM_FOREST *rf, NODE *node, FILE *fp) {
  ...
  fprintf(fp, "NODE %d %d %d %lf\n", node->left == nullptr, node->depth,
          node->feature, node->thresh);
  for (c = 0; c < rf->nclasses; c++)
...
static int rfReadNode(RANDOM_FOREST *rf, NODE *node, FILE *fp) {
  ...
  node->training_set =
      (int *)calloc(node->total_counts, sizeof(node->training_set[0]));
#if 0
  cp = fgetl(line, MAX_LINE_LEN, fp) ;
  cp = strtok(line, " ") ;
  for (n = 0 ; n < node->total_counts ; n++)
...
int RFwriteInto(RANDOM_FOREST *rf, FILE *fp) {
  ...
  fprintf(fp, "%d %d %d %d %d %d %lf\n", rf->nfeatures, rf->nclasses,
          rf->ntrees, rf->max_depth, rf->ntraining, rf->nsteps,
          rf->feature_fraction);
  for (c = 0; c < rf->nclasses; c++)
    fprintf(fp, "%s\n", rf->class_names[c]);
    ...
    if (node->left) // not a leaf
    {
      ...
      if (rf->feature_names)
        printf("\tnode: feature %s (#%d): thresh = %2.2f, input val = %2.2f, ",
               rf->feature_names[node->feature], node->feature, node->thresh,
...
int RFcomputeOutOfBagCorrect(RANDOM_FOREST *rf, int *training_classes,
                             double **training_data, int ntraining) {
  ...
  if (tree_training == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFcomputeOutOfBagCorrect: could not allocate %d tree features",
              rf->ntrees);

  for (n = 0; n < rf->ntrees; n++) {
    tree             = &rf->trees[n];
    tree_training[n] = (int *)calloc(rf->ntraining, sizeof(int));
    if (tree_training[n] == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "RFcomputeOutOfBagCorrect: could not allocate %dth %d-len tree "
                "features",
                rf->ntrees, rf->nfeatures);
  ...
  for (correct = i = 0; i < ntraining; i++) {
    ...
    for (n = 0; n < rf->ntrees; n++) {
      ...
      if (tree_training[n][i] > 0)
        continue; // this tree was trained on this data - don't use it for
                  // accuracy estimation

      node = rfFindLeaf(rf, &rf->trees[n].root, training_data[i]);
      for (total_count = c = 0; c < rf->nclasses; c++) {
        if (tree->root.class_counts[c] > 0)
          total_count += (double)node->class_counts[c] /
                         (double)tree->root.class_counts[c];
      }
      for (c = 0; c < rf->nclasses; c++) {
        if (tree->root.class_counts[c] > 0)
          class_counts[c] += ((double)node->class_counts[c] /
                              (double)tree->root.class_counts[c]) /
                             total_count;
      }
        ...
        for (c = 0; c < tree->nleaves; c++) // diagnostic
        {
          ...
          if (node == tree->leaves[c]) {
            ...
            for (c2 = 0; c2 < rf->nclasses; c2++)
              printf("%2.2f ", ((double)node->class_counts[c2] /
                                (double)tree->root.class_counts[c2]) /
                                   total_count);
            printf("\n");
            break;
          }
...
int RFclassify(RANDOM_FOREST *rf, double *feature, double *p_pval,
               int true_class) {
  int    max_class = -1, n, c;
  NODE * node;
  ...
  for (n = 0; n < rf->ntrees; n++) {
    ...
    // now prune trees that only had one training class as they just bias
    // labeling
    for (c = 0; c < rf->nclasses; c++)
  ...
  if (DIAG_VERBOSE_ON)
    for (c = 0; c < rf->nclasses; c++) {
      if (true_class >= 0) {
        if (c == max_class) {
          printf("%s: p = %2.2f ", rf->class_names[c],
                 100.0 * class_counts[c] / total_count);
          if (c == true_class)
            printf("CORRECT\n");
          else
            printf("(true = %s, p = %2.2f)\n", rf->class_names[true_class],
...
RANDOM_FOREST *RFreadFrom(FILE *fp) {
  ...
  cp = fgetl(line, MAX_LINE_LEN, fp);
  sscanf(cp, "%d %d %d %d %d %d %lf", &nfeatures, &nclasses, &ntrees,
         &max_depth, &ntraining, &nsteps, &feature_fraction);
  ...
  if (rf->feature_names == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "RFreadFrom: could not allocate feature name array");
  for (f = 0; f < rf->nfeatures; f++) {
    cp                   = fgetl(line, MAX_LINE_LEN, fp);
    rf->feature_names[f] = (char *)calloc(strlen(cp) + 1, sizeof(char));
    if (rf->feature_names[f] == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "RFreadFrom: could not allocate %d-len feature name array [%d]",
                strlen(cp) + 1, f);
...
int RFsetNumberOfClasses(RANDOM_FOREST *rf, int nclasses) {
  ...
  if (nclasses > rf->nclasses) {
    ...
    if (rf->class_names == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "RFsetNumberOfClasses(%d): could not allocate class names",
                nclasses);

    for (c = 0; c < rf->nclasses; c++) {
      if (old_class_names[c] == nullptr)
        continue;
      rf->class_names[c] =
          (char *)calloc(strlen(old_class_names[c]) + 1, sizeof(char));
    ...
    for (n = 0; n < rf->ntrees; n++) {
      ...
      tree->root.class_counts =
          (int *)calloc(nclasses, sizeof(tree->root.class_counts[0]));
      if (tree->root.class_counts == nullptr)
        ErrorExit(ERROR_NOMEMORY,
                  "RFsetNumberOfClasses(%d): could not allocate class names",
                  nclasses);
...
int RFevaluateFeatures(RANDOM_FOREST *rf, FILE *fp) {
  ...
  for (fno = 0; fno < rf->nfeatures; fno++) {
    ...
    for (total_info_gain = 0.0, tno = 0; tno < rf->ntrees; tno++) {
      tree       = &rf->trees[tno];
      tree_count = 0;
      info_gain  = rfFeatureInfoGain(rf, tree, &tree->root, tree->root.left,
                                    tree->root.right, fno, &tree_count);
...
static double rfFeatureInfoGain(RANDOM_FOREST *rf, TREE *tree, NODE *parent,
  ...
  if (left != nullptr) //  not a leaf and uses this feature
  {
    if (parent->feature == fno) {
      ...
      entropy_after = wl * entropy(left->class_counts, rf->nclasses,
                                   tree->root.class_counts) +
                      wr * entropy(right->class_counts, rf->nclasses,
                                   tree->root.class_counts);
      info_gain =
          ((double)parent->total_counts * (entropy_before - entropy_after));
      *ptotal_count += parent->total_counts;
    } else
      info_gain = 0;

    info_gain += rfFeatureInfoGain(rf, tree, left, left->left, left->right, fno,
                                   ptotal_count);
    info_gain += rfFeatureInfoGain(rf, tree, right, right->left, right->right,
                                   fno, ptotal_count);
  }
  return (info_gain);
}
