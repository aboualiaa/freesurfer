...
#pragma omp parallel for if_ROMP(assume_reproducible)
    ...
    ROMP_PFLB_begin MRISedgeMetricEdge(surf, edgeno, DoGrad);
    ROMP_PFLB_end
  }
  ROMP_PF_end return (0);
}
...
#pragma omp parallel for if_ROMP(assume_reproducible)
    ...
    ROMP_PFLB_begin MRIScornerMetricCorner(surf, cornerno, DoGrad);
    ROMP_PFLB_end
  }
  ROMP_PF_end
...
double MRISfaceNormalGradFaceTest(MRIS *surf, int faceno, int wrtvtxno,
  ...
  for (wrtdimno = 0; wrtdimno < 3; wrtdimno++) {
    ...
    for (c = 0; c < 3; c++) {
      ...
      diff =
          fabs(gnum->rptr[c + 1][wrtdimno + 1] - g0->rptr[c + 1][wrtdimno + 1]);
      if (maxdiff < diff)
        maxdiff = diff;
    }
  }
...
int MRISedgeAngleCostEdgeVertexTest(MRIS *surf, int edgeno, int wrtvtxno,
  ...
  if (false) {
    ...
    printf("gnum  = [");
    for (c = 0; c < 3; c++)
      printf("%12.8f ", gnum->rptr[1][c + 1]);
    printf("];\n");
    printf("gdiff = [");
    for (c = 0; c < 3; c++)
      printf("%12.8f ", gnum->rptr[1][c + 1] - g0->rptr[1][c + 1]);
    printf("];\n");
  }
...
int MRISfacePrint(MRIS *surf, int faceno, FILE *fp) {
  ...
  fprintf(fp, "area %f\n", f->area);
  if (f->norm)
    fprintf(fp, "norm %f %f %f\n", f->norm->rptr[1][1], f->norm->rptr[2][1],
            f->norm->rptr[3][1]);
...
int BBRFaceFree(BBRFACE **pbbrf) {
  ...
  if (bbrf->pc)
    DMatrixFree(&bbrf->pc);
  if (bbrf->pin)
    DMatrixFree(&bbrf->pin);
  if (bbrf->vin)
    DMatrixFree(&bbrf->vin);
  if (bbrf->pout)
    DMatrixFree(&bbrf->pout);
  if (bbrf->vout)
    DMatrixFree(&bbrf->pout);
  for (n = 0; n < 3; n++) {
...
BBRFACE *BBRFaceDiff(const BBRFACE *bbrf1, const BBRFACE *bbrf2,
  ...
  for (n = 0; n < 3; n++) {
    DMatrixAddMul(bbrf1->gradPin[n], bbrf2->gradPin[n], f, -f,
                  bbrfout->gradPin[n]);
    DMatrixAddMul(bbrf1->gradPout[n], bbrf2->gradPout[n], f, -f,
                  bbrfout->gradPout[n]);
    DMatrixAddMul(bbrf1->gradVin[n], bbrf2->gradVin[n], f, -f,
                  bbrfout->gradVin[n]);
    DMatrixAddMul(bbrf1->gradVout[n], bbrf2->gradVout[n], f, -f,
                  bbrfout->gradVout[n]);
    DMatrixAddMul(bbrf1->gradInterpIn[n], bbrf2->gradInterpIn[n], f, -f,
                  bbrfout->gradInterpIn[n]);
    DMatrixAddMul(bbrf1->gradInterpOut[n], bbrf2->gradInterpOut[n], f, -f,
                  bbrfout->gradInterpOut[n]);
    DMatrixAddMul(bbrf1->gradIin[n], bbrf2->gradIin[n], f, -f,
                  bbrfout->gradIin[n]);
    DMatrixAddMul(bbrf1->gradIout[n], bbrf2->gradIout[n], f, -f,
                  bbrfout->gradIout[n]);
...
int BBRFacePrint(FILE *fp, BBRFACE *bbrf, BBRPARAMS *bbrpar) {
  ...
  fprintf(fp, "fnorm ");
  for (n = 0; n < 3; n++)
    fprintf(fp, "%12.10lf ", f->norm->rptr[n + 1][1]);
  fprintf(fp, "\n");
  fprintf(fp, "pc ");
  for (n = 0; n < 3; n++)
    fprintf(fp, "%6.4lf ", bbrf->pc->rptr[n + 1][1]);
  fprintf(fp, "\n");
  fprintf(fp, "pin ");
  for (n = 0; n < 3; n++)
    fprintf(fp, "%6.4lf ", bbrf->pin->rptr[n + 1][1]);
  fprintf(fp, "\n");
  fprintf(fp, "pout ");
  for (n = 0; n < 3; n++)
    fprintf(fp, "%6.4lf ", bbrf->pout->rptr[n + 1][1]);
  fprintf(fp, "\n");
  fprintf(fp, "vin ");
  for (n = 0; n < 3; n++)
    fprintf(fp, "%6.4lf ", bbrf->vin->rptr[n + 1][1]);
  fprintf(fp, "\n");
  fprintf(fp, "vout ");
  for (n = 0; n < 3; n++)
    fprintf(fp, "%6.4lf ", bbrf->vout->rptr[n + 1][1]);
  fprintf(fp, "\n");
  fprintf(fp, "Iin  %lf\n", bbrf->Iin);
  fprintf(fp, "Iout %lf\n", bbrf->Iout);
  fprintf(fp, "Q %lf\n", bbrf->Q);
...
BBRFACE *BBRCostFace(int faceno, int wrtvtxno, BBRPARAMS *bbrpar,
  ...
  if (wrtvtxno < 0) {
    ...
    for (n = 0; n < 3; n++)
      bbrf->pin->rptr[n + 1][1] =
          bbrf->pc->rptr[n + 1][1] - (bbrpar->Din * f->norm->rptr[n + 1][1]);
    ...
    // Note: pc and norm are 3x1 whereas pin is 4x1
    for (n = 0; n < 3; n++)
      bbrf->pout->rptr[n + 1][1] =
          bbrf->pc->rptr[n + 1][1] + (bbrpar->Dout * f->norm->rptr[n + 1][1]);
  ...
  bbrf->gradPin[wrtvtxno] = DMatrixScalarMul(
      f->gradNorm[wrtvtxno], -bbrpar->Din, bbrf->gradPin[wrtvtxno]);
  for (n = 0; n < 3; n++)
    bbrf->gradPin[wrtvtxno]->rptr[n + 1][n + 1] += 1 / 3.0;
  ...
  if (bbrpar->interp == SAMPLE_TRILINEAR)
    bbrf->gradInterpIn[wrtvtxno] =
        MRIgradTrilinInterp(bbrpar->mri, c, r, s, bbrf->gradInterpIn[wrtvtxno]);
  if (bbrpar->interp == SAMPLE_CUBIC)
    bbrf->gradInterpIn[wrtvtxno] =
  ...
  bbrf->gradIin[wrtvtxno] =
      DMatrixMultiply(bbrf->gradInterpIn[wrtvtxno], bbrf->gradVin[wrtvtxno],
                      bbrf->gradIin[wrtvtxno]);

  // gradPout = gradPc + Dout*gradNorm, gradPc = eye(3)/3
  bbrf->gradPout[wrtvtxno] = DMatrixScalarMul(
      f->gradNorm[wrtvtxno], +bbrpar->Dout, bbrf->gradPout[wrtvtxno]);
  for (n = 0; n < 3; n++)
    bbrf->gradPout[wrtvtxno]->rptr[n + 1][n + 1] += 1 / 3.0;
  ...
  s = bbrf->vout->rptr[1][3];
  if (bbrpar->interp == SAMPLE_TRILINEAR)
    bbrf->gradInterpOut[wrtvtxno] = MRIgradTrilinInterp(
        bbrpar->mri, c, r, s, bbrf->gradInterpOut[wrtvtxno]);
  if (bbrpar->interp == SAMPLE_CUBIC)
  ...
  bbrf->gradQ[wrtvtxno] =
      DMatrixScalarMul(bbrf->gradQ[wrtvtxno], 200, bbrf->gradQ[wrtvtxno]);
  DMatrixFree(&tmp1);
  DMatrixFree(&tmp2);

  // Finally, compute the gadient of the cost = M*(1-tanh^2)*gradQ
  bbrf->gradCost[wrtvtxno] =
      DMatrixScalarMul(bbrf->gradQ[wrtvtxno], bbrpar->M * (1.0 - vtanh * vtanh),
                       bbrf->gradCost[wrtvtxno]);
...
double TestBBRCostFace(BBRPARAMS *const bbrpar, int const faceno,
                       int const wrtvtxno, double const delta,
                       int const verbose) {
  ...
  for (wrtdimno = 0; wrtdimno < 3; wrtdimno++) {
    ...
    if (verbose) {
      ...
      printf("gradPin  = ");
      for (n = 0; n < 3; n++)
        printf("%lf ", bbrf1->gradPin[wrtvtxno]->rptr[n + 1][wrtdimno + 1]);
      printf("\n");
      printf("gradPout = ");
      for (n = 0; n < 3; n++)
        printf("%lf ", bbrf1->gradPout[wrtvtxno]->rptr[n + 1][wrtdimno + 1]);
      printf("\n");
      printf("gradVin  = ");
      for (n = 0; n < 3; n++)
        printf("%lf ", bbrf1->gradVin[wrtvtxno]->rptr[n + 1][wrtdimno + 1]);
      printf("\n");
      printf("gradVout = ");
      for (n = 0; n < 3; n++)
        printf("%lf ", bbrf1->gradVout[wrtvtxno]->rptr[n + 1][wrtdimno + 1]);
      printf("\n");
      printf("gradIin  = %lf\n",
      ...
      for (wrtvtxno = 0; wrtvtxno < 3; wrtvtxno++) {
        ...
        for (n = 0; n < 3; n++)
          gradCostTh[threadno]->rptr[svtxno + 1][n + 1] +=
              bbrf->gradCost[wrtvtxno]->rptr[1][n + 1];
      }
    }
  }
...
int MRISedgeLengthCostTest(MRIS *surf, double delta, double thresh, double L0) {
  ...
  for (vno = 0; vno < surf->nvertices; vno++) {
    ...
    if (angle > thresh) {
      ...
      for (k = 0; k < 3; k++)
        printf("%14.9Lf  %14.9Lf   ", expdcost[k], dcost[k]);
      printf("\n");
      fflush(stdout);
    }
  }
  ...
  printf("#VMPC# VmPeak  %d\n", vmpeak1);
  if (vmpeak1 > vmpeak0)
    printf("WARNING: there might be a memory leak\n");
  else
...
int MRISedgeAngleCostTest(MRIS *surf, double delta, double anglethresh,
  ...
  for (vno = 0; vno < surf->nvertices; vno++) {
    ...
    if (angle > anglethresh || magerr > magthresh) {
      nerrs++;
      printf("%5d %4d  %6.4f %6.4f   %8.8f %8.8f %8.8f ", nerrs, vno, angle,
             anglemax, expmag, empmag, magerr);
      for (k = 0; k < 3; k++)
        printf("%14.9Lf ", expdcost[k]);
      printf("  ");
      for (k = 0; k < 3; k++)
        printf("%14.9Lf ", dcost[k]);
      printf("\n");
      fflush(stdout);
    }
...
double MRIScornerMetricCorner(MRIS *surf, const int cornerno,
  ...
  c->dot = 0;
  for (k = 0; k < 3; k++)
    c->dot += (c->edgedir[0] * e1->u[k]) * (c->edgedir[1] * e2->u[k]);
  ...
  for (k = 0; k < 3; k++)
    u1mat->rptr[1][k + 1] = (c->edgedir[0] * e1->u[k]);
...
double *MRIScornerStats(MRIS *surf, int metricid, MRI *mask, double *stats) {
  ...
  for (cornerno = 0; cornerno < surf->ncorners; cornerno++) {
    ...
    switch (metricid) {
    case 0:
      metric[ncorners] = c->dot;
      break;
    case 1:
      metric[ncorners] = c->angle;
      break;
    default:
...
int MRIScornerDotCostTest(MRIS *surf, const double delta,
  ...
  for (vno = 0; vno < surf->nvertices; vno++) {
    ...
    if (angle > anglethresh || magerr > magthresh) {
      nerrs++;
      printf("%5d   %6d  %6.4f %6.4f   %8.8f %8.8f %8.8f ", nerrs, vno, angle,
             anglemax, expmag, empmag, magerr);
      for (k = 0; k < 3; k++)
        printf("%14.9Lf ", expdcost[k]);
      printf("  ");
  ...
  if (vmpeak1 > vmpeak0)
    printf("WARNING: there might be a memory leak (%d,%d)\n", vmpeak1, vmpeak0);
  else
    printf("No memory leak detected\n");
