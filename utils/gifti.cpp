...
static double gifti_get_DA_value_2D(giiDataArray *da, int row, int col) {
  ...
  /* Check the indices. */
  if (dim0_index < 0 || dim0_index >= dims_0 || dim1_index < 0 ||
      dim1_index >= dims_1) {
    fprintf(stderr,
  ...
  case NIFTI_TYPE_UINT8: {
    if (GIFTI_IND_ORD_ROW_MAJOR == da->ind_ord)
      return (double)*((unsigned char *)(da->data) + (dim0_index * dims_1) +
                       dim1_index);
    else
      return (double)*((unsigned char *)(da->data) + dim0_index +
                       (dim1_index * dims_0));
    break;
  }
  case NIFTI_TYPE_INT16: {
    if (GIFTI_IND_ORD_ROW_MAJOR == da->ind_ord)
      return (double)*((short *)(da->data) + (dim0_index * dims_1) +
                       dim1_index);
    else
      return (double)*((short *)(da->data) + dim0_index +
                       (dim1_index * dims_0));
    break;
  }
  ...
  case NIFTI_TYPE_FLOAT32: {
    if (GIFTI_IND_ORD_ROW_MAJOR == da->ind_ord)
      return (double)*((float *)(da->data) + (dim0_index * dims_1) +
                       dim1_index);
    else
      return (double)*((float *)(da->data) + dim0_index +
                       (dim1_index * dims_0));
    break;
  }
  case NIFTI_TYPE_FLOAT64: {
    if (GIFTI_IND_ORD_ROW_MAJOR == da->ind_ord)
      return (double)*((double *)(da->data) + (dim0_index * dims_1) +
                       dim1_index);
    else
      return (double)*((double *)(da->data) + dim0_index +
                       (dim1_index * dims_0));
    break;
  }
  ...
  case NIFTI_TYPE_UINT16: {
    if (GIFTI_IND_ORD_ROW_MAJOR == da->ind_ord)
      return (double)*((unsigned short *)(da->data) + (dim0_index * dims_1) +
                       dim1_index);
    else
      return (double)*((unsigned short *)(da->data) + dim0_index +
                       (dim1_index * dims_0));
    break;
  }
  case NIFTI_TYPE_UINT32: {
    if (GIFTI_IND_ORD_ROW_MAJOR == da->ind_ord)
      return (double)*((unsigned int *)(da->data) + (dim0_index * dims_1) +
                       dim1_index);
    else
      return (double)*((unsigned int *)(da->data) + dim0_index +
                       (dim1_index * dims_0));
    break;
  }
  }
 ...
 *
 */
static void gifti_set_DA_value_2D(giiDataArray *da, int row, int col,
                                  double value) {
  int dim0_index, dim1_index;
  int dims_0 = 0, dims_1 = 0;
...
MRIS *mrisReadGIFTIdanum(const char *fname, MRIS *mris, int daNum) {
  ...
  if (image->labeltable.length > 0) {
    ...
    if (nullptr == rgba) {
      ...
      if (nullptr == image->labeltable.rgba) {
      ...
      int label_index;
      for (label_index = 0; label_index < image->labeltable.length;
           label_index++) {
    ...
    for (label_index = 0; label_index < image->labeltable.length;
      ...
      if (ct->entries[label_index] == nullptr) {
        fprintf(stderr,
    ...
    ct->entries[label_index] = nullptr;
  ...
  if (coords && faces) {
    ...
    char *hemi =
        gifti_get_meta_value(&coords->meta, "AnatomicalStructurePrimary");
    ...
    for (face_index = 0; face_index < num_faces; face_index++) {
      ...
      for (face_vertex_index = 0; face_vertex_index < VERTICES_PER_FACE;
           face_vertex_index++) {
        vertex_index =
    ...
    for (vertex_index = 0; vertex_index < num_vertices; vertex_index++) {
      mris->vertices_topology[vertex_index].f =
          (int *)calloc(mris->vertices_topology[vertex_index].num, sizeof(int));
      mris->vertices_topology[vertex_index].n = (uchar *)calloc(
          mris->vertices_topology[vertex_index].num, sizeof(uchar));
      mris->vertices_topology[vertex_index].num =
          0; // this gets re-calc'd next...
    }
    ...
    for (vertex_index = 0; vertex_index < num_vertices; vertex_index++) {
      ...
      for (n = 0; n < mris->vertices_topology[vertex_index].num; n++) {
        for (m = 0; m < VERTICES_PER_FACE; m++) {
          if (mris->faces[mris->vertices_topology[vertex_index].f[n]].v[m] ==
              vertex_index) {
    ...
    if (coords->coordsys && (coords->numCS > 0)) {
      ...
      for (idx = 0; idx < coords->numCS; idx++) {
        if (0 ==
            strcmp(coords->coordsys[idx]->dataspace, "NIFTI_XFORM_UNKNOWN")) {
  ...
  for (numDA = startDAnum; numDA < endDAnum; numDA++) {
    ...
    if (darray->intent == NIFTI_INTENT_NODE_INDEX) {
      ...
      if (num_vertices <= 0 || num_vertices != mris->nvertices ||
        ...
        fprintf(
            ...
            numDA, fname, (int)num_vertices, (int)num_cols, mris->nvertices,
    ...
    if (darray->intent == NIFTI_INTENT_SHAPE) {
      ...
      if ((nullptr == mris->ct) || (nullptr == ct)) // sanity-check
      {
...
int MRISwriteGIFTI(MRIS *mris, int intent_code, const char *out_fname,
  ...
  if (nullptr == mris || nullptr == out_fname) {
    fprintf(stderr, "MRISwriteGIFTI: invalid parameter\n");
    return ERROR_BADPARM;
  }

  if (intent_code == NIFTI_INTENT_SHAPE && nullptr == curv_fname) {
  ...
  if (intent_code == NIFTI_INTENT_POINTSET ||
    ...
    if (mris->SRASToTalSRAS_ && mris->SRASToTalSRAS_->rows == 4 &&
    ...
    gifti_datatype_sizes(coords->datatype, &coords->nbyper, nullptr);

    /* Allocate the data array. */
    coords->data = nullptr;
    coords->data = (void *)calloc(coords->nvals, coords->nbyper);
    if (nullptr == coords->data) {
      fprintf(stderr,
    ...
    if (strlen(mris->fname) != 0) {
      ...
      if (primary)
        gifti_add_to_meta(&coords->meta, "AnatomicalStructurePrimary", primary,
                          1);
      if (secondary)
        gifti_add_to_meta(&coords->meta, "AnatomicalStructureSecondary",
                          secondary, 1);
      if (geotype)
  ...
  if (intent_code == NIFTI_INTENT_LABEL) {
    ...
    for (idx = 0; idx < labeltable.length; idx++) {
      ...
      if (strlen(mris->ct->entries[idx]->name) != 0) {
        ...
        printf(
            "idx=%d, name=NULL, assigned as %s (is the colortable correct?)\n",
            idx, tmpname);
    ...
    for (label_index = 0; label_index < mris->nvertices; label_index++) {
      ...
      result = CTABfindAnnotation(
          mris->ct, mris->vertices[label_index].annotation, &theIdx);
      if (result) {
        return ERROR_BADFILE;
      }
   ...
   * Statistics file
   */
  if (intent_code == NIFTI_INTENT_CORREL || intent_code == NIFTI_INTENT_TTEST ||
      intent_code == NIFTI_INTENT_FTEST || intent_code == NIFTI_INTENT_ZSCORE ||
      intent_code == NIFTI_INTENT_CHISQ || intent_code == NIFTI_INTENT_BETA ||
      intent_code == NIFTI_INTENT_BINOM || intent_code == NIFTI_INTENT_GAMMA ||
      ...
      intent_code == NIFTI_INTENT_WEIBULL || intent_code == NIFTI_INTENT_CHI ||
      intent_code == NIFTI_INTENT_INVGAUSS ||
      intent_code == NIFTI_INTENT_EXTVAL || intent_code == NIFTI_INTENT_PVAL ||
    ...
    gifti_add_to_meta(&stats->meta, "Intent_code",
                      gifti_intent_to_string(intent_code), 1);
...
int mriWriteGifti(MRI *mri, const char *out_fname) {
  if (nullptr == mri || nullptr == out_fname) {
