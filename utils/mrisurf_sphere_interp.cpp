...
SphericalInterpolator::SphericalInterpolator(MRIS *surf) : mris(surf) {
  ...
  mht = MHTcreateFaceTable_Resolution(mris, CURRENT_VERTICES,
                                      mris->avg_vertex_dist / 2);
...
float SphericalInterpolator::interp(double phi, double theta) {
  ...
  for (int xvi = 0; xvi <= 1; xvi++) {
    ...
    for (int yvi = 0; yvi <= 1; yvi++) {
      ...
      for (int zvi = 0; zvi <= 1; zvi++) {
        int zv = zvi + offsetz;
        if (searchBucket(ix + xv, iy + yv, iz + zv, x, y, z, &value))
          return value;
  ...
  fs::error() << "Could not locate a face corresponding to phi=" << phi
              << ", theta=" << theta
...
*/
bool SphericalInterpolator::searchBucket(int bx, int by, int bz, float x,
                                         float y, float z, float *value) {
  // checkout the bucket
...
/*
  ...
  intersection and returns true.
*/
bool SphericalInterpolator::testRayIntersection(int fno, float x, float y,
                                                float z, float *value) {
