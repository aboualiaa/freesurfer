...
*/
void setErrorLog(const std::string &filename) { errorlog = filename; }

/**
  Configures a global setting to throw exceptions instead of exiting
  when `fs::fatal()` and `ErrorExit()` are called.
*/
void throwExceptions(bool setting) { exceptions = setting; }

/**
  Writes a message the global error log if configured with
...
/**
  Exits with a given return code, or throws exceptions if enabled via
...
/**
  A c-style error function that wraps the cpp stream logging utilities. Ideally,
  `fs::error() << message` should be used instead.
*/
void ErrorPrintf(int errcode, const char *format, ...) {
  Gerror = errcode;
  if (errno)
    fs::error() << strerror(errno);
  vargsToString(format, message);
  fs::error() << message;
}

/**
  A c-style fatal error that wraps the cpp stream logging utilities. Ideally,
  `fs::fatal(code) << message` should be used instead.
*/
