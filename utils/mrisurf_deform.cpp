...
static void mris_project_point_into_face(MRIS *mris, FACE *face, int which,
  ...
  default:
    ErrorExit(ERROR_BADPARM,
              "mris_project_point_into_face: which %d not supported", which);
    break;
...
static void mrisProjectOntoSurface(MRI_SURFACE *mris, int which_vertices) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MHTfindClosestFaceGeneric((MHT *)(mris->mht), mris, v->x, v->y, v->z, 8.0,
                              8, 1, &face, &fno, &fdist);
    if (face == NULL ||
        fdist > 0.5) // current coord doesn't project easily into a nearby face
    {
      double lambda[3], xv, yv, zv, norm;
      int    n;

      // no projection in this call - it will find a face
      MHTfindClosestFaceGeneric((MHT *)(mris->mht), mris, v->x, v->y, v->z,
      ...
      mris_project_point_into_face(mris, face, which_vertices, v->x, v->y, v->z,
                                   &px, &py, &pz);
    }

    if (vno == Gdiag_no)
      printf("v %d: (%2.2f %2.2f %2.2f) projected to (%2.2f %2.2f %2.2f), "
             "delta = (%2.2f %2.2f %2.2f)\n",
             vno, v->x, v->y, v->z, px, py, pz, v->x - px, v->y - py,
             v->z - pz);
    v->x = px;
    v->y = py;
    v->z = pz;
    if (face_barycentric_coords(mris, fno, which_vertices, v->x, v->y, v->z,
                                NULL, NULL, NULL) < 0) {
      DiagBreak();
    }
  }
...
template <class MRIS_Representation> class mrisSurfaceProjector {
public:
  bool canDo(int mris_status);
  void project(MRIS_Representation *representation);
};

template <> class mrisSurfaceProjector<MRIS_MP> {
public:
  bool canDo(int mris_status) {
    switch (mris_status) {
  ...
  void project(MRIS_MP *mris_mp) {
    ...
    case MRIS_SPHERE:
      MRISprojectOntoSphere(mris_mp, mris_mp->radius);
      break;
    default:
      break;
    }
...
template <> class mrisSurfaceProjector<MRIS> {
public:
...
int mrisComputePlaneTerm(MRIS *mris, double l_plane, double l_spacing) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no)
      printf("vno %d, best fitting plane = (%2.1f %2.1f %2.1f %2.1f), dist = "
             "%2.2f, moving by (%2.2f, %2.2f, %2.2f)\n",
...
int mrisLogStatus(MRI_SURFACE *mris, INTEGRATION_PARMS *parms, FILE *fp,
                  float dt, float old_sse) {
  if (!(Gdiag & DIAG_SHOW)) {
    return (NO_ERROR);
  }
  ...
  if (parms->l_thick_min || parms->l_thick_parallel || parms->l_thick_normal ||
      parms->l_ashburner_triangle || parms->l_tspring ||
    ...
    fprintf(fp,
            ...
            parms->t, dt, sse, negative,
            100.0 * mris->neg_area / (mris->neg_area + mris->total_area),
            100.0 * mris->neg_orig_area / (mris->orig_area), parms->n_averages,
            pct_change);
  } else {
    float area_rms = 0, angle_rms = 0, curv_rms = 0, dist_rms = 0, corr_rms = 0;
    float sse = !doSSE ? 0.0f
                       : mrisComputeError(mris, parms, &area_rms, &angle_rms,
                                          &curv_rms, &dist_rms, &corr_rms);
    ...
    if (FZERO(parms->l_corr) && FZERO(parms->l_pcorr) &&
      ...
      for (n = 0; n < parms->nfields; n++) {
        ...
        fprintf(stdout, "  (%d: %2.3f : %2.3f)", n, parms->fields[n].sse,
                sqrt(parms->fields[n].sse / nv));
      }
      fprintf(stdout, "\n");
...
int mrisApplyGradientPositiveAreaPreserving(MRI_SURFACE *mris, double dt) {
  ...
  for (vno = 0; vno < nvertices; vno++) {
    ...
    if (!std::isfinite(v->dx) || !std::isfinite(v->dy) ||
        !std::isfinite(v->dz)) {
      ErrorPrintf(ERROR_BADPARM, "vertex %d position is not finite!\n", vno);
    }
    ...
    for (n = 0; n < vt->num && step < last_step; n++) {
      ...
      while (step < last_step) {
        eps = epsilon[step];
        mrisSphericalProjectXYZ(x + eps * dx, y + eps * dy, z + eps * dz, &v->x,
                                &v->y, &v->z);
...
struct MRIScomputeSSE_asThoughGradientApplied_ctx::Impl {
  ...
  void init(MRIS *mris) {
    ...
    MRISMP_load(
        &orig, mris, true, dx, dy,
        dz); // needs to load the outputs because those are inputs to ProjectSurface
...
double MRIScomputeSSE_asThoughGradientApplied(
    MRIS *mris, double delta_t, INTEGRATION_PARMS *parms,
    MRIScomputeSSE_asThoughGradientApplied_ctx &ctx) {
  bool const canUseNewBehaviour =
      mrisProjectSurface_CanDo(mris->status) &&
      MRIScomputeSSE_canDo((MRIS_MP *)nullptr, parms);

  bool useOldBehaviour =
      !!getenv("FREESURFER_OLD_MRIScomputeSSE_asThoughGradientApplied");
  bool useNewBehaviour =
      !!getenv("FREESURFER_NEW_MRIScomputeSSE_asThoughGradientApplied") ||
      !useOldBehaviour;
...
int mrisApplyTopologyPreservingGradient(MRI_SURFACE *mris, double dt,
  ...
  for (vno = 0; vno < nvertices; vno++) {
    ...
    if (which_gradient) {
      if (!std::isfinite(v->odx) || !std::isfinite(v->ody) ||
          !std::isfinite(v->odz)) {
        ErrorPrintf(ERROR_BADPARM, "vertex %d position is not finite!\n", vno);
      }
    ...
    if (vno == ver1 || vno == ver2 || vno == ver3 || vno == ver4) {
      fprintf(stderr, "\nbf %d : %f %f %f - %f %f %f  ", vno, v->x, v->y, v->z,
              v->cx, v->cy, v->cz);
      ...
      for (n = 0; n < vt->num && step < last_step; n++) {
        ...
        if ((orig_area <= 0)) {
          ...
          fprintf(stderr, "face %d (%d,%d,%d) at vertex %d\n", vt->f[n],
                  mris->faces[vt->f[n]].v[0], mris->faces[vt->f[n]].v[1],
                  mris->faces[vt->f[n]].v[2], vno);
          v1 = mris->faces[vt->f[n]].v[0];
          v2 = mris->faces[vt->f[n]].v[1];
          v3 = mris->faces[vt->f[n]].v[2];
          fprintf(stderr, "cur: vertex %d (%f,%f,%f)\n", v1,
                  mris->vertices[v1].x, mris->vertices[v1].y,
                  mris->vertices[v1].z);
          fprintf(stderr, "cur: vertex %d (%f,%f,%f)\n", v2,
                  mris->vertices[v2].x, mris->vertices[v2].y,
                  mris->vertices[v2].z);
          fprintf(stderr, "cur: vertex %d (%f,%f,%f)\n", v3,
                  mris->vertices[v3].x, mris->vertices[v3].y,
                  mris->vertices[v3].z);
          // if(parms->verbose==VERBOSE_MODE_HIGH)
          ErrorExit(ERROR_BADPARM,
                    "mrisApplyTopologyPreservingGradient:SHOULD NOT HAPPEN\n");
        }
        while (step < last_step) {
          mrisSphericalProjectXYZ(x + epsilon[step] * dx,
                                  y + epsilon[step] * dy,
                                  z + epsilon[step] * dz, &v->x, &v->y, &v->z);
          v->cx = v->x;
          v->cy = v->y;
          v->cz = v->z;
          area  = mrisComputeArea(mris, vt->f[n], (int)vt->n[n]);
...
#endif
      mrisSphericalProjectXYZ(x + epsilon[step] * dx, y + epsilon[step] * dy,
                              z + epsilon[step] * dz, &v->x, &v->y, &v->z);
      v->cx = v->x;
      v->cy = v->y;
      v->cz = v->z;
...
#if DEBUG_PRESERVING_GRADIENT
    if (vno == ver1 || vno == ver2 || vno == ver3 || vno == ver4) {
      fprintf(stderr, "\naf %d : %f %f %f - %f %f %f  ", vno, v->x, v->y, v->z,
              v->cx, v->cy, v->cz);
...
int mrisComputePosteriorTerm(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (v->ripflag || vlst_enough_data(mris, vno, vl[vno], 0.0))
      continue;
    for (n = 0; n < vt->vnum; n++) {
      if (vl2[vno]->nvox + vl[vt->v[n]]->nvox >= vl2[vno]->max_vox)
        break;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    best_ll = -1e10;
    best_dn = 0;
    for (dn = -MAX_DISPLACEMENT; dn <= MAX_DISPLACEMENT;
         dn += DISPLACEMENT_DELTA) {
    ...
    if (vno == Gdiag_no) {
      ...
      for (i = 0; i < vl[vno]->nvox; i++) {
        val  = MRIgetVoxVal(mri, vl[vno]->xi[i], vl[vno]->yi[i], vl[vno]->zi[i],
                           0);
        dx   = vl[vno]->xd[i] - v->x;
        dy   = vl[vno]->yd[i] - v->y;
        dz   = vl[vno]->zd[i] - v->z;
        ...
        fprintf(fp, "%d %d %d %d %d %f %f %f %f\n", vno, i, vl[vno]->xi[i],
                vl[vno]->yi[i], vl[vno]->zi[i], val, dist, pin, pout);
      }
      fclose(fp);

      printf("l_map: vno %d, best displacement %2.3f, ll = %2.3f, D = (%2.3f, "
             "%2.3f, %2.3f)\n",
             vno, best_dn, best_ll, best_dn * v->nx * parms->l_map,
             best_dn * v->ny * parms->l_map, best_dn * v->nz * parms->l_map);
      DiagBreak();
    }
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (num > 0) {
      ...
      v->d = dn;
      if (vno == Gdiag_no)
        printf("l_map: vno %d, soap bubble displacement %2.3f, D = (%2.3f, "
               "%2.3f, %2.3f)\n",
...
int mrisComputePositioningGradients(MRI_SURFACE *      mris,
...
double mrisRmsValError_mef(MRI_SURFACE *mris, MRI *mri_30, MRI *mri_5,
...
double mrisComputeSSE_MEF(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,
                          MRI *mri30, MRI *mri5, double weight30,
                          double weight5, MHT *mht) {
...
int mrisRemoveNegativeArea(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,
                           int base_averages, float min_area_pct,
                           int max_passes) {
  unsigned int npasses;
  int          total_steps, done, steps, n_averages, old_averages, niter;
  float        pct_neg, ratio;
  const char * snum, *sdenom;
  float l_area, l_parea, l_corr, l_spring, l_dist, l_nlarea, *pnum, *pdenom,
      cmod;
  double tol;
  ...
  l_dist        = parms->l_dist;
  l_corr        = parms->l_corr;
  parms->l_area = parms->l_parea = parms->l_dist = parms->l_corr =
      parms->l_spring = parms->l_nlarea = 0.0;
  ...
  */
  ...
  for (total_steps = npasses = 0; npasses < MAX_PASSES; npasses++) {
    *pnum   = 1.0;
    *pdenom = cmod * (npasses >= MAX_PASSES ? neg_area_ratios[MAX_PASSES - 1]
                                            : neg_area_ratios[npasses]);
...
/* print transform matrix in tmp dir */
  ...
  if (Gdiag & DIAG_SHOW && Gdiag_no >= 0) {
    ...
    fprintf(stdout,
            ...
            Gdiag_no, v->x, v->y, v->z, v->area, v->origarea, v->nz, vt->vnum);
    for (n = 0; n < vt->vnum; n++) {
      ...
      fprintf(stdout,
              ...
              vt->v[n], vn->x, vn->y, vn->z, vn->area, v->origarea, vn->nz,
              vnt->vnum, v->dist[n], v->dist_orig[n]);
    }
    fprintf(stdout, "\n");
  }
...
int mrisTearStressedRegions(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  if (parms->explode_flag)
    printf("max stress = %2.2f, nrip = %d, nv = %d, nf = %d\n", max_stress,
           nrip, mris->nvertices, mris->nfaces);
  if (parms->explode_flag && ncalls == EXPLODE_ITER &&
      parms->stressthresh < 0) {
...
int MRISsoapBubbleVertexPositions(MRI_SURFACE *mris, int navgs) {
  ...
  for (i = 0; i < navgs; i++) {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      if (vno == Gdiag_no) {
        printf("iter %d: moving v %d from (%2.1f, %2.1f, %2.1f) --> (%2.1f, "
               "%2.1f, %2.1f)\n",
...
MRIS *MRISextractMainComponent(MRI_SURFACE *mris, int do_not_extract,
  ...
  for (count = 0, vn0 = 0; vn0 < mris->nvertices; vn0++) {
    ...
    if (verbose)
      fprintf(stderr,
              "\n   %d voxel in cpt #%d: X=%d [v=%d,e=%d,f=%d] located at (%f, "
              "%f, %f)",
              ncpt, count, nX, nv, ne, nf, cx, cy, cz);
  }
  if (verbose) {
    fprintf(stderr, "\nFor the whole surface: X=%d [v=%d,e=%d,f=%d]", tX, tv,
            te, tf);
  }
  ...
  if (count <= 1) {
    ...
    if (!do_not_extract && verbose)
      fprintf(WHICH_OUTPUT, "\nkeeping component #%d with %d vertices", max_c,
              max_nbr);
  }
...
int MRIScombine(MRI_SURFACE *mris_src, MRI_SURFACE *mris_total,
  ...
  for (vno = 0; vno < mris_src->nvertices; vno++) {
    ...
    case VERTEX_COORDS: {
      int const vdst_vno =
          vdst -
          mris_total
              ->vertices; // GROSS HACK since MHTfindClosestVertex doesn't return the vno
      MRISsetOriginalXYZ(mris_total, vdst_vno, vdst->origx + v->origx,
                         vdst->origy + v->origy, vdst->origz + v->origz);
    } break;
    case VERTEX_AREA:
      vdst->d += v->origarea;
      break;
  ...
  for (vno = 0; vno < mris_total->nvertices; vno++) {
    ...
    switch (which) {
    case VERTEX_COORDS:
      MRISsetOriginalXYZ(mris_total, vno, vdst->origx / (float)vdst->marked,
  ...
  for (vno = 0; vno < mris_total->nvertices; vno++) {
    ...
    if (!mht_src) {
      ...
      mean = MRIScomputeVertexSpacingStats(mris_src, &sigma, NULL, &max_len,
                                           NULL, NULL, CURRENT_VERTICES);
      if (max_len > mean + 3 * sigma) {
        max_len = mean + 3 * sigma;
      }
      mht_src = MHTcreateVertexTable_Resolution(mris_src, CURRENT_VERTICES,
                                                2 * max_len);
    }
    float min_dist;
...
#if 1
int MRISsphericalCopy(MRI_SURFACE *mris_src, MRI_SURFACE *mris_dst,
                      MRIS_HASH_TABLE *mht, int which) {
  switch (which) {
  case VERTEX_COORDS:
  ...
  MRIScomputeVertexSpacingStats(mris_src, NULL, NULL, &max_len, NULL, NULL,
                                CURRENT_VERTICES);
  mht_src =
      MHTcreateVertexTable_Resolution(mris_src, CURRENT_VERTICES, 2 * max_len);
  ...
  for (vno = 0; vno < mris_dst->nvertices; vno++) {
    ...
    float               min_dist;
    VERTEX const *const v =
        MHTfindClosestVertex2(mht_src, mris_src, mris_dst, vdst, &min_dist);
    if (!v) {
      ErrorPrintf(ERROR_BADPARM, "MRISsphericalCopy: cannot map dst vno %d",
                  vno);
      continue;
    }
    if (v - mris_src->vertices == Gdiag_no) {
...
MRIS *MRISremoveRippedSurfaceElements(MRIS *mris) {
  ...
  mris_corrected =
      MRISoverAlloc(nint(1.5 * kept_vertices), nint(1.5 * kept_faces),
  ...
  mris_corrected->type = MRIS_TRIANGULAR_SURFACE;
  mris_corrected->status =
      mris->status; // this should have been set, but wasn't
  mris_corrected->origxyz_status = mris->origxyz_status; // this is new
  ...
  fprintf(stderr,
          "%d vertices and %d faces have been removed from triangulation\n",
...
static int mrisGraphLaplacian(MRI_SURFACE *mris, int vno, double *pdx,
                              double *pdy, double *pdz, int which) {
...
int MRIStaubinSmooth(MRI_SURFACE *mris, int niters, double lambda, double mu,
  ...
  if (lambda < 0 || lambda > 1 || mu >= -lambda) {
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "MRIStaubinSmooth: mu < -lambda < 0 violated"));
  }
...
MRI *MRISgaussianSmooth(MRIS *Surf, MRI *Src, double GStd, MRI *Targ,
  ...
  for (vtxno1 = 0; vtxno1 < Surf->nvertices; vtxno1++) {
    ...
    for (n = 0; n < nXNbrs; n++) {
      ...
      if (vtxno1 == 10000 && 1) {
        printf("%d %d %g %g %g %g %g\n", vtxno1, vtxno2, dotprod, costheta,
               theta, d, g);
        fflush(stdout);
      }
...
int MRISremoveIntersections(MRI_SURFACE *mris) {
  ...
  while (num > 0) {
    if ((num > old_num) ||
        ((num == old_num) && (no_progress >= nbrs))) // couldn't remove any
    {
      no_progress++;
      ...
      if (nbrs >=
          MAX_INT_REMOVAL_NEIGHBORS) // don't let neighborhood get too big
      {
        if (no_progress >=
            nbrs) // couldnt' find any better configurations - give up
          break;
    ...
    MRISnotMarked(
        mris); // turn off->on and on->off so soap bubble is correct (marked are fixed)
    MRISsoapBubbleVertexPositions(mris, 100);
    if (writeit) {
      char fname[STRLEN];
      sprintf(fname, "%s.avg%03d",
              mris->hemisphere == RIGHT_HEMISPHERE ? "rh" : "lh", n);
  ...
  if (num > min_int) {
    MRISrestoreVertexPositions(
        mris, TMP2_VERTICES); // the least number of negative vertices
    num = mrisMarkIntersections(mris);
  }
...
int MRISwriteCoordsToIco(MRI_SURFACE *mris, MRI_SURFACE *mris_ico,
  ...
  for (vno = 0; vno < mris_ico->nvertices; vno++) {
    ...
    MHTfindClosestFaceGeneric(mht, mris, v->cx, v->cy, v->cz, 1000, -1, 1,
                              &face, &fno, &fdist);
    if (face == NULL) {
      DiagBreak();
      continue;
    }
    debug = (face->v[0] == Gdiag_no || face->v[1] == Gdiag_no ||
             face->v[2] == Gdiag_no);
    MRISsampleFaceCoordsCanonical(mht, mris, v->cx, v->cy, v->cz,
                                  which_vertices, &x, &y, &z);
    if (debug) {
    ...
    case PIAL_VERTICES:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "%s: unsupported vertex set %d",
                __MYFUNCTION__, which_vertices);
      break;
    }
  }
  MHTfree(&mht);
...
#define SCALE .01
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vn->ripflag == 0) {
      vn->val = val;
      if (vno !=
          target_vno) // give it a target which maintains its relative position
      {
        v         = &mris->vertices[target_vno];
...
static void MRIScopyOneVertex(MRI_SURFACE *out_mris, int const out_vno,
                              MRI_SURFACE *inp_mris, int const inp_vno) {
  ...
  if (out_vt->v == NULL)
    ErrorExit(ERROR_NOMEMORY,
              "MRISconcat: could not allocate %dth vertex array", out_vno);
  int n;
  for (n = 0; n < out_vt->vtotal; n++)
    out_vt->v[n] = inp_vt->v[n];
  out_vt->f = (int *)calloc(out_vt->num, sizeof(int));
  if (out_vt->f == NULL)
    ErrorExit(ERROR_NOMEMORY, "MRISconcat: could not allocate %dth face array",
              out_vno);
  for (n = 0; n < out_vt->vtotal; n++)
    out_vt->f[n] = inp_vt->f[n];
}
...
MRI_SURFACE *MRISconcat(MRI_SURFACE *mris1, MRI_SURFACE *mris2,
  ...
  if (mris == NULL)
    mris = MRISalloc(mris1->nvertices + mris2->nvertices,
                     mris1->nfaces + mris2->nfaces);
...
int MRISfixAverageSurf7(MRIS *surf7) {
  ...
  // Make sure that the xyz are the same at these two vertices
  {
    ...
    if (d > .001) {
      printf("INFO: MRISfixAverageSurf7(): this surface appears to have been "
             "fixed already\n");
...
MRIS *MRISsortVertices(MRIS *mris0) {
  VERTEX_SORT *vtxsort =
      (VERTEX_SORT *)calloc(mris0->nvertices, sizeof(VERTEX_SORT));
  int nthvtx;
  ...
  for (nthvtx = 0; nthvtx < mris0->nvertices; nthvtx++) {
    VERTEX_TOPOLOGY const *const vtxtold =
        &(mris0->vertices_topology[vtxsort[nthvtx].vtxno]);
    ...
    vtxnew->y                      = vtxold->y;
    vtxnew->z                      = vtxold->z;
    modVnum(mris, nthvtx, vtxtold->vnum,
            true); // number of neighboring vertices
  ...
  for (nthface = 0; nthface < mris0->nfaces; nthface++) {
    FACE const *const f =
        &(mris0->faces[nthface]); // old and new faces are the same
    int vno_new_min   = 0;
    int n_vno_new_min = 0;
    int n;
    int vnolist[3];
...
int MRISrigidBodyAlignLocal(MRI_SURFACE *mris, INTEGRATION_PARMS *old_parms) {
  ...
  if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
    ...
    rms = mrisComputeError(mris, &parms, &area_rms, &angle_rms, &curv_rms,
                           &dist_rms, &corr_rms);
    fprintf(stdout, "rms = %2.3f, corr_rms = %2.3f ", rms, corr_rms);
    rms = mrisComputeError(mris, old_parms, &area_rms, &angle_rms, &curv_rms,
                           &dist_rms, &corr_rms);
...
int MRISrigidBodyAlignGlobal(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,
  ...
  static bool once, use_old, use_new;

  if (!once) {
    once    = true;
    use_old = !!getenv("FREESURFER_MRISrigidBodyAlignGlobal_useOld");
    use_new =
        !!getenv("FREESURFER_MRISrigidBodyAlignGlobal_useNew") || !use_old;
  }

  double new_mina = 666.0, new_minb = 666.0, new_ming = 666.0, new_sse = 666.0;

  double const ext_sse =
      gMRISexternalSSE ? (*gMRISexternalSSE)(mris, parms) : 0.0;
  ...
  if (use_new) {
    ...
    double ext_sse =
        (gMRISexternalSSE) ? (*gMRISexternalSSE)(mris, parms) : 0.0;
    MRISrigidBodyAlignGlobal_findMinSSE(&new_mina, &new_minb, &new_ming,
                                        &new_sse, mris, parms, min_radians,
                                        max_radians, ext_sse, nangles);
    ...
    printf("  new MRISrigidBodyAlignGlobal_findMinSSE"
           " min @ (%2.2f, %2.2f, %2.2f) sse = %2.1f, elapsed since "
           "starting=%6.4f min\n",
           (float)DEGREES(new_mina), (float)DEGREES(new_minb),
           (float)DEGREES(new_ming), new_sse, msec / (1000 * 60.0));
  }
  ...
  if (use_old) {
    ...
    for (radians = max_radians; radians >= min_radians; radians /= 2.0f) {
      ...
      if (tracing) {
        fprintf(stdout,
                "scanning %2.2f degree nbhd of (%2.2f, %2.2f, %2.2f), min sse "
                "= %2.2f\n",
                (float)DEGREES(radians), (float)DEGREES(center_a),
                (float)DEGREES(center_b), (float)DEGREES(center_g),
                center_sse_known ? (float)(center_sse + ext_sse) : -666.0);
      }

      double const delta = 2 * radians / (float)nangles;
      double       mina = center_a, minb = center_b, ming = center_g,
             min_sse = center_sse;
...
#else
      for (gamma = center_g - radians; gamma <= center_g + radians;
           gamma += delta) {
        for (beta = center_b - radians; beta <= center_b + radians;
             beta += delta) {
          for (alpha = center_a - radians; alpha <= center_a + radians;
               alpha += delta) {
#endif
      VERTEX const *vertex0   = &mris->vertices[0];
      float         vertex0_x = vertex0->x, vertex0_y = vertex0->y,
            vertex0_z = vertex0->z;
      MRISrotate(mris, mris, center_a + alpha, center_b + beta,
                 center_g + gamma);

      if (trace) {
        fprintf(
            stdout,
            "%s:%d rotated (%g,%g,%g) by (a:%g, b:%g, g:%g) to (%g,%g,%g)\n",
            __FILE__, __LINE__, vertex0_x, vertex0_y, vertex0_z,
            center_a + alpha, center_b + beta, center_g + gamma, vertex0->x,
            vertex0->y, vertex0->z);
      }

      double sse = mrisComputeCorrelationError(mris, parms, 1);
      if (trace)
        fprintf(stdout, "%s:%d sse:%g\n", __FILE__, __LINE__, sse);
      ...
      if (false && tracing) {
        fprintf(stdout,
                "\r  gamma "
                "min @ (%2.2f, %2.2f, %2.2f) sse:%2.1f   try @ (%+2.2f, "
                "%+2.2f, %+2.2f) sse:%2.2f",
                (float)DEGREES(mina), (float)DEGREES(minb),
                (float)DEGREES(ming), (float)(min_sse + ext_sse),
                (float)DEGREES(alpha), (float)DEGREES(beta),
                (float)DEGREES(gamma), (float)(sse + ext_sse));
        fflush(stdout);
      }
    } // gamma
    if (false && tracing) {
      fprintf(
          ...
          (float)(min_sse + ext_sse), (float)DEGREES(alpha),
          (float)DEGREES(beta));
    }
  } // beta
  if (false && tracing) {
...
printf("  min @ (%2.2f, %2.2f, %2.2f) sse = %2.1f, elapsed since "
       "starting=%6.4f min\n",
       (float)DEGREES(mina), (float)DEGREES(minb), (float)DEGREES(ming),
       (float)(min_sse + ext_sse), msec / (1000 * 60.0));
fflush(stdout);
...
old_mina = center_a;
old_minb = center_b, old_ming = center_g, old_sse = center_sse;

int msec = old_timer.milliseconds();
printf("  old MRISrigidBodyAlignGlobal_findMinSSE"
       " min @ (%2.2f, %2.2f, %2.2f) sse = %2.1f, elapsed since starting=%6.4f "
       "min\n",
       (float)DEGREES(old_mina), (float)DEGREES(old_minb),
       (float)DEGREES(old_ming), old_sse, msec / (1000 * 60.0));
...
if (tracing) {
  fprintf(stdout, "rotating brain by (%2.2f, %2.2f, %2.2f), final sse: %2.2f\n",
          (float)DEGREES(new_mina), (float)DEGREES(new_minb),
          (float)DEGREES(new_ming), (float)(new_sse + ext_sse));
}
if (tracingWithSnapshot) {
  fprintf(parms->fp,
      ...
      for (beta = -degrees; beta <= degrees; beta += delta) {
        if (Gdiag & DIAG_SHOW)
          fprintf(stdout,
                  ...
                  (float)DEGREES(alpha), (float)DEGREES(beta),
                  (float)DEGREES(-degrees), (float)DEGREES(mina),
        ...
        for (gamma = -degrees; gamma <= degrees; gamma += delta) {
          ...
          sse = mrisComputeVectorCorrelationError(mris, parms,
                                                  1); /* was 0 !!!! */
          /* if (gMRISexternalSSE) */
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
static int mrisPlaceVertexInOrigFace(MRIS *const mris_vno, int const vno,
                                     MRIS *const mris, int const fno) {
  VERTEX *const v = &mris_vno->vertices[vno];

  double U0[3], U1[3], U2[3], pt[3], dir[3], int_pt[3], l1[3], l2[3], l2_len,
      l1_len, l_len, P[3], theta1, theta2, dot, theta_ratio, len_scale, e1[3],
      e2[3], etmp[3], x, y;
  int ret;
  ...
  */
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    if (vcount[fno] == max_count)
      fprintf(stdout, "\t%d (%d, %d, %d)\n", fno, mris->faces[fno].v[0],
              mris->faces[fno].v[1], mris->faces[fno].v[2]);
  }
  MHTfree(&mht);
  free(vcount);
...
int mrisAddFace(MRI_SURFACE *mris, int vno0, int vno1, int vno2) {
  ...
  if (vno0 < 0 || vno1 < 0 || vno2 < 0)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "mrisAddFace(%d,%d,%d)!\n", vno0, vno1, vno2));
  if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
