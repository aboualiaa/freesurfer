...
// because they can tile along the vertices dimension to optimize for cache
// behaviour
...
// Only one implementation is needed because the axes are permuted below
// accordingly
//
static void rotateVertices1axis_wkr(float *xv_out, float *yv_out,
...
//
static inline void
rotateVertices_wkr(float *xv_out, float *yv_out, float *zv_out,
                   float const *xv_inp, float const *yv_inp,
                   float const *zv_inp, size_t nvertices,
                   ...
                   float gamma) // rotate around x axis - first rotation
{
  ...
  for (unsigned int vno = 0; vno < nvertices; vno++) {
    ...
    float xp = x * cacb + z * (-cacgsb - sasg) +
               y * (cgsa - casbsg); //             x            x cb - z sb x ca
                                    //             + y sa
    float yp = -x * cbsa + z * (cgsasb - casg) +
               y * (cacg + sasbsg); // - z sg + y cg            y -x sa + y ca
    float zp =
        z * cbcg + x * sb + y * cbsg; //   z cg + y sg            x sb - z cb z
...
void rotateVertices(float *xv_out, float *yv_out, float *zv_out,
                    float const *xv_inp, float const *yv_inp,
                    float const *zv_inp, size_t nvertices,
                    ...
                    float gamma) // gamma is the first rotation
{
  ...
  if (alpha == 0.0 && beta == 0.0) {
    rotateVertices1axis_wkr(yv_out, zv_out, yv_inp, zv_inp, xv_out, xv_inp,
                            nvertices, -gamma);
    return;
  }
  if (alpha == 0.0 && gamma == 0.0) {
    rotateVertices1axis_wkr(xv_out, zv_out, xv_inp, zv_inp, yv_out, yv_inp,
                            nvertices, -beta);
    return;
  }
  if (beta == 0.0 && gamma == 0.0) {
    rotateVertices1axis_wkr(xv_out, yv_out, xv_inp, yv_inp, zv_out, zv_inp,
                            nvertices, alpha);
    return;
  }
  ...
  if (alpha != 0.0 && beta != 0.0 && gamma != 0.0) {
    rotateVertices_wkr(xv_out, yv_out, zv_out, xv_inp, yv_inp, zv_inp,
                       nvertices, alpha, beta, gamma);
    return;
  }
  ...
  if (alpha != 0.0 && beta != 0.0) {
    rotateVertices_wkr(xv_out, yv_out, zv_out, xv_inp, yv_inp, zv_inp,
                       nvertices, alpha, beta, 0.0);
    return;
  }
  if (alpha != 0.0 && gamma != 0.0) {
    rotateVertices_wkr(xv_out, yv_out, zv_out, xv_inp, yv_inp, zv_inp,
                       nvertices, alpha, 0.0, gamma);
    return;
  }
  if (beta != 0.0 && gamma != 0.0) {
    rotateVertices_wkr(xv_out, yv_out, zv_out, xv_inp, yv_inp, zv_inp,
                       nvertices, 0.0, beta, gamma);
    return;
  }
}
...
static void test(float xi, float yi, float zi, float xo, float yo, float zo,
  ...
  check(yt, yo);
  check(zt, zo);
  if (saved_errorCount != errorCount)
    printf(" when alpha:%g beta:%g gamma:%g\n", alpha, beta, gamma);
}
