...
static int    CSAEcomputeGradient(CSAE *csae, AE *ae, VOXEL_LIST *vl, MRI *mri,
                                  int start_index, int end_index, int *indices,
...
SAE *SAEalloc(int whalf, int nlevels, int type, double scale) {
  ...
  if (sae == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "SAEalloc(%s, %d): could not alloc sae", whalf));
  sae->whalf     = whalf;
  sae->nencoders = 1;
  sae->nlevels   = nlevels;
  sae->type      = type;
  noutputs       = type & FOCUSED_AUTOENCODER ? 1 : ninputs;
  sae->first     = AEalloc(nullptr, ninputs, nhidden, noutputs);
  sae->scale     = scale;
  if (whalf == 0) {
...
static AE *AEalloc(AE *prev, int ninputs, int nhidden, int noutputs) {
  ...
  if (ae == nullptr)
    ErrorReturn(NULL, (ERROR_NOFILE, "AEaalloc(%d, %d): could not alloc ae",
                       ninputs, nhidden));
  ...
  if (prev)
    ...
    if (ae->v_output == nullptr)
      ErrorReturn(NULL,
                  (ERROR_NOFILE, "AEaalloc(%d, %d): could not alloc v_output",
                   ninputs, nhidden));
  }

  ae->v_input =
      VectorAlloc(ninputs, MATRIX_REAL); // will be a copy of previous layer to
                                         // allow calculation of errors
  if (ae->v_input == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "AEaalloc(%d, %d): could not alloc v_input",
                 ninputs, nhidden));

  ae->m_input_to_hidden = MatrixAlloc(nhidden, ninputs, MATRIX_REAL);
  if (ae->m_input_to_hidden == nullptr)
    ErrorReturn(NULL, (ERROR_NOFILE,
                       "AEaalloc(%d, %d): could not alloc m_input_to_hidden",
                       ninputs, nhidden));
  ae->m_hidden_to_output = MatrixAlloc(noutputs, nhidden, MATRIX_REAL);
  if (ae->m_hidden_to_output == nullptr)
    ErrorReturn(NULL, (ERROR_NOFILE,
                       "AEaalloc(%d, %d): could not alloc m_hidden_output_to",
                       ninputs, nhidden));
  ae->v_output = VectorAlloc(noutputs, MATRIX_REAL);
  if (ae->v_output == nullptr)
    ErrorReturn(NULL,
  ...
  if (ae->v_hidden_bias == nullptr)
    ErrorReturn(NULL, (ERROR_NOFILE,
                       "AEaalloc(%d, %d): could not alloc v_hidden_bias",
                       ninputs, nhidden));
  ae->hidden_class_labels = (int *)calloc(nhidden, sizeof(int));
  if (ae->hidden_class_labels == nullptr)
    ErrorReturn(NULL, (ERROR_NOFILE,
                       "AEaalloc(%d, %d): could not alloc hidden class labels",
                       ninputs, nhidden));
  ae->v_hidden_net = VectorAlloc(nhidden, MATRIX_REAL);
  if (ae->v_hidden_net == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "AEaalloc(%d, %d): could not alloc v_hidden_net",
                 ninputs, nhidden));
  ae->v_hidden = VectorAlloc(nhidden, MATRIX_REAL);
  if (ae->v_hidden == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "AEaalloc(%d, %d): could not alloc v_hidden",
                 ninputs, nhidden));
  ae->v_output_bias = VectorAlloc(noutputs, MATRIX_REAL);
  if (ae->v_output_bias == nullptr)
    ErrorReturn(NULL, (ERROR_NOFILE,
                       "AEaalloc(%d, %d): could not alloc v_output_bias",
                       ninputs, nhidden));
  ae->prev = prev;
  if (ae->prev)
    ae->prev->next = ae;
  ...
  for (x = 0; x < mri[0]->width; x++)
    for (y = 0; y < mri[0]->height; y++)
      for (z = 0; z < mri[0]->depth; z++) {
        SAEfillInputVector(mri, sae->nlevels, x, y, z, sae->whalf,
                           sae->first->v_input);
        SAEactivateNetwork(sae);
      }
  return (sae);
...
static int AEactivateLayer(AE *ae, VECTOR *v_input) {
  ...
  if (ae->noise_fraction > 0) {
    ...
    max_i = nint(ninputs * ae->noise_fraction);
    for (i = 0; i < max_i; i++)
      VECTOR_ELT(ae->v_input, ae->zero_indices[i] + 1) =
          ae->saved_inputs[i]; // restore inputs for training
  }
  MatrixAdd(ae->v_hidden_net, ae->v_hidden_bias, ae->v_hidden_net);
  for (row = 1; row <= ae->v_hidden->rows;
       row++) // apply hidden layer nonlinearity
  {
  ...
  if (ae->next) {
    AEactivateLayer(
        ae->next,
        ae->v_hidden); // recurse - this hidden unit is next layers input
    MatrixCopy(ae->next->v_output,
               ae->v_hidden); // output of next layer is hidden activation of
                              // this one since it autoencodes
  }
...
double SAEcomputeRMS(SAE *sae) {
  ...
  if (sae->type == NORMAL_AUTOENCODER) {
    for (rms = 0.0, row = 1; row <= sae->first->v_input->rows; row++) {
      error = VECTOR_ELT(sae->first->v_input, row) -
              VECTOR_ELT(sae->first->v_output, row);
      rms += error * error;
    }
  } else {
    row   = (sae->first->v_input->rows + 1) / 2;
    error = VECTOR_ELT(sae->first->v_input, row) -
            VECTOR_ELT(sae->first->v_output, 1);
    rms = error * error;
...
double SAEtrainFromMRI(SAE *sae, MRI **mri_pyramid,
                       SAE_INTEGRATION_PARMS *parms) {
  double error = 0.0, rms, last_rms, total_rms, last_total_rms, pct_decrease,
         running_last_rms, G_rms, G_last_rms = 1e10;
  ...
  if (mri_pyramid[0]->type != MRI_FLOAT)
    ErrorExit(ERROR_BADPARM,
              "SAEtrainFromMRI: input type must be MRI_FLOAT scaled to [0->1]");
  ...
  if (!x_ind || !y_ind || !z_ind)
    ErrorExit(ERROR_NOMEMORY,
              "SAEtrainFromMRI: could not allocate permutation indices");

  ae_train       = SAEfindLastLayer(sae, nullptr);
  last_total_rms = 1;
  ...
  if (Gx >= 0) {
    ...
    SAEfillInputVector(mri_pyramid, sae->nlevels, Gx, Gy, Gz, sae->whalf,
                       sae->first->v_input);
    SAEactivateNetwork(sae);
    G_last_rms = SAEcomputeRMS(sae);
    printf("voxel (%d, %d, %d), I = %2.2f, o = %2.2f, rms = %2.4f\n", Gx, Gy,
           Gz, sae->first->v_input->rptr[ind][1],
           sae->first->v_output->rptr[ind][1], G_last_rms);
  }
  do {
    MRIcomputeVoxelPermutation(mri, x_ind, y_ind, z_ind);
    for (running_last_rms = total_rms = 0.0, visited = ind = 0; ind < nvox;
         ind++) {
      if (ind && !(ind % MAX((nvox / 500), 2))) {
        ...
        running_pct_dec = 100 *
                          ...
                          (running_last_rms / visited + total_rms / visited);
        pct_decrease = 100 * (last_total_rms - total_rms / visited) /
                       (last_total_rms + total_rms / visited);
        printf("%d of %d (%2.2f%%): rms = %2.4f (%2.3f%%) (%2.3f%%)\n", ind,
               nvox, 100 * ind / (float)nvox, total_rms / visited, pct_decrease,
               running_pct_dec);
      }
      x = x_ind[ind];
      y = y_ind[ind];
      ...
      if (x < sae->whalf || y < sae->whalf || z < sae->whalf ||
          x >= mri->width - sae->whalf || y >= mri->height - sae->whalf ||
          z >= mri->depth - sae->whalf)
        continue;
      if (!FZERO(
              MRIgetVoxVal(mri, x, y, z, 0))) // there is real data at this spot
      {
        visited++;
        if (++calls == Gdiag_no)
          DiagBreak();
        SAEfillInputVector(mri_pyramid, sae->nlevels, x, y, z, sae->whalf,
                           sae->first->v_input);
        SAEactivateNetwork(sae);
...
#endif
    pct_decrease =
        100 * (last_total_rms - total_rms) / (last_total_rms + total_rms);
...
// NOTE: input MRI must be type float and scaled to be 0->1 (that is, scale down
// MRI_UCHAR vols by 255)
double SAEtrainFromVoxlist(SAE *sae, VOXEL_LIST *vl, MRI **mri_pyramid,
                           SAE_INTEGRATION_PARMS *parms) {
  double error = 0.0, rms, last_rms, total_rms, last_total_rms, pct_decrease,
         running_last_rms, G_rms, G_last_rms = 1e10;
  ...
  do {
    ...
    for (running_last_rms = total_rms = 0.0, visited = ind = 0; ind < vl->nvox;
         ind++) {
      if (ind && !(ind % MAX((vl->nvox / 100), 2))) {
        ...
        pct_decrease = 100 * (last_total_rms - total_rms / visited) /
                       (last_total_rms + total_rms / visited);
        printf("%d of %d (%2.2f%%): rms = %2.4f (%2.3f%%) (%2.3f%%)\n", ind,
               vl->nvox, 100 * ind / (float)vl->nvox, total_rms / visited,
               pct_decrease, running_pct_dec);
      }
      i = indices[ind];
      x = vl->xi[i];
      y = vl->yi[i];
      z = vl->zi[i];
      if (x < sae->whalf || y < sae->whalf ||
          (z < sae->whalf && mri->depth > 1) || x >= mri->width - sae->whalf ||
...
static int CSAEcomputeGradient(CSAE *csae, AE *ae, VOXEL_LIST *vl, MRI *mri,
  ...
  for (i = start_index; i <= end_index; i++) {
    ...
    if (x < whalf || y < whalf || (z < whalf && mri->depth > 1) ||
        x >= mri->width - whalf || y >= mri->height - whalf ||
  ...
  MatrixScalarMul(ae->m_grad_input_to_hidden, 1.0 / num_indices,
                  ae->m_grad_input_to_hidden);
  MatrixScalarMul(ae->m_grad_hidden_to_output, 1.0 / num_indices,
                  ae->m_grad_hidden_to_output);
  MatrixScalarMul(ae->v_grad_hidden_bias, 1.0 / num_indices,
                  ae->v_grad_hidden_bias);
  MatrixScalarMul(ae->v_grad_output_bias, 1.0 / num_indices,
                  ae->v_grad_output_bias);
  ...
  for (j = 1; j <= nhidden; j++) {
    ae->average_act[j - 1] /=
        (double)num_indices; // make it the average activation level
    VECTOR_ELT(ae->v_grad_hidden_bias, j) +=
        parms->sparsity_trate *
        (ae->average_act[j - 1] - parms->sparsity_target);
    for (i = 1; i <= ninputs; i++)
      *MATRIX_RELT(ae->m_grad_input_to_hidden, j, i) +=
          (parms->sparsity_trate / ninputs) *
          (ae->average_act[j - 1] - parms->sparsity_target);
  }
  ...
  for (i = 1; i <= ninputs; i++)
    for (j = 1; j <= nhidden; j++)
      *MATRIX_RELT(ae->m_grad_input_to_hidden, j, i) +=
          parms->weight_decays[parms->layer] *
          *MATRIX_RELT(ae->m_input_to_hidden, j, i);
  for (j = 1; j <= nhidden; j++)
    for (k = 1; k <= noutputs; k++)
      *MATRIX_RELT(ae->m_grad_hidden_to_output, k, j) +=
          parms->weight_decays[parms->layer] *
          *MATRIX_RELT(ae->m_hidden_to_output, k, j);
...
static int AEclearGradients(AE *ae) {
  ...
  if (ae->m_grad_input_to_hidden == nullptr)
    return (NO_ERROR);
...
static double AEaccumulateGradient(AE *ae, SAE_INTEGRATION_PARMS *parms) {
  ...
  if (noutputs == ninputs) // normal autoencoder
  {
    for (k = 1; k <= ae->v_input->rows; k++) {
      error = VECTOR_ELT(ae->v_error, k) =
          VECTOR_ELT(ae->v_output, k) - VECTOR_ELT(ae->v_input, k);
      rms += error * error;
    }
  } else {
    int ind;
    ind = (ninputs + 1) / 2;
    rms = VECTOR_ELT(ae->v_error, 1) =
        VECTOR_ELT(ae->v_output, 1) - VECTOR_ELT(ae->v_input, ind);
    rms *= rms;
  }
  ...
  ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) firstprivate(noutputs, nhidden) \
    shared(ae) schedule(static, 1)
#endif
      for (k = 1; k <= noutputs; k++) {
    ROMP_PFLB_begin double error, hidden;
    int                    j;
  ...
  ROMP_PF_end
    ...
    for (j = 1; j <= nhidden; j++) {
      ...
      for (dE_dwij = 0.0, i = 1; i <= noutputs; i++) {
        ...
        *MATRIX_RELT(ae->m_grad_input_to_hidden, j, i) +=
            parms->class_weight * dE_dwij;
      }

      dE_dbj = error * fprimej;

      VECTOR_ELT(ae->v_grad_hidden_bias, j) +=
          parms->class_weight * dE_dbj; // XXX added division by noutputs
    }
  }
...
static double aeApplyAccumulatedGradient(AE *ae, SAE_INTEGRATION_PARMS *parms) {
  ...
  if (parms->integration_type !=
      INTEGRATE_CONJUGATE_GRADIENT) // don't scale grads by -dt for conjugate
                                    // gradient
  {
    ...
    MatrixScalarMul(ae->m_grad_hidden_to_output, -dt,
                    ae->m_grad_hidden_to_output);
    MatrixScalarMul(ae->m_grad_input_to_hidden, -dt,
                    ae->m_grad_input_to_hidden);
  }
  if (!FZERO(momentum)) // use momentum
  {
    MatrixScalarMul(ae->v_previous_step_output_bias, momentum,
                    ae->v_previous_step_output_bias);
    MatrixAdd(ae->v_grad_output_bias, ae->v_previous_step_output_bias,
              ae->v_grad_output_bias);

    MatrixScalarMul(ae->v_previous_step_hidden_bias, momentum,
                    ae->v_previous_step_hidden_bias);
    MatrixAdd(ae->v_grad_hidden_bias, ae->v_previous_step_hidden_bias,
              ae->v_grad_hidden_bias);

    MatrixScalarMul(ae->m_previous_step_input_to_hidden, momentum,
                    ae->m_previous_step_input_to_hidden);
    MatrixAdd(ae->m_grad_input_to_hidden, ae->m_previous_step_input_to_hidden,
              ae->m_grad_input_to_hidden);

    MatrixScalarMul(ae->m_previous_step_hidden_to_output, momentum,
                    ae->m_previous_step_hidden_to_output);
    MatrixAdd(ae->m_grad_hidden_to_output, ae->m_previous_step_hidden_to_output,
              ae->m_grad_hidden_to_output);
  }
  ...
  if (parms->integration_type == INTEGRATE_GRADIENT_DESCENT) {
    ...
    MatrixAdd(ae->m_grad_hidden_to_output, ae->m_hidden_to_output,
              ae->m_hidden_to_output);
    MatrixAdd(ae->m_grad_input_to_hidden, ae->m_input_to_hidden,
              ae->m_input_to_hidden);
    ...
    MatrixCopy(ae->m_grad_hidden_to_output,
               ae->m_previous_step_hidden_to_output);
    ...
    if (parms->v_dir_hidden_bias ==
        nullptr) // allocate everything the first time
    {
      ...
      parms->m_prev_grad_change_input_to_hidden =
          MatrixClone(ae->m_input_to_hidden);
      parms->m_prev_grad_change_hidden_to_output =
          MatrixClone(ae->m_hidden_to_output);
      ...
      parms->norm_hidden_bias             = parms->norm_output_bias =
          parms->norm_hidden_to_output = parms->norm_input_to_hidden = 1.0;
    }

    if (DZERO(parms->norm_hidden_bias) || !devFinite(parms->norm_hidden_bias))
      DiagBreak();
    beta = VectorDot(parms->v_prev_grad_change_hidden_bias,
                     parms->v_prev_grad_hidden_bias) /
           parms->norm_hidden_bias;
    if (!devFinite(beta))
      DiagBreak();
    MatrixScalarMul(parms->v_dir_hidden_bias, beta, parms->v_dir_hidden_bias);
    MatrixSubtract(parms->v_dir_hidden_bias, ae->v_grad_hidden_bias,
                   parms->v_dir_hidden_bias);
    ...
    static MATRIX *m_hidden_to_output_delta = nullptr,
                  *m_input_to_hidden_delta  = nullptr,
                  *v_hidden_bias_delta      = nullptr,
                  *v_output_bias_delta      = nullptr;
    ...
    m_hidden_to_output_delta = MatrixDRand48ZeroMean(
        ae->m_hidden_to_output->rows, ae->m_hidden_to_output->cols,
        m_hidden_to_output_delta);
    m_input_to_hidden_delta = MatrixDRand48ZeroMean(ae->m_input_to_hidden->rows,
                                                    ae->m_input_to_hidden->cols,
                                                    m_input_to_hidden_delta);
    v_hidden_bias_delta     = MatrixDRand48ZeroMean(
        ae->v_hidden_bias->rows, ae->v_hidden_bias->cols, v_hidden_bias_delta);
    v_output_bias_delta = MatrixDRand48ZeroMean(
        ae->v_output_bias->rows, ae->v_output_bias->cols, v_output_bias_delta);

    MatrixScalarMul(m_hidden_to_output_delta, dt * parms->proposal_sigma,
                    m_hidden_to_output_delta);
    MatrixScalarMul(m_input_to_hidden_delta, dt * parms->proposal_sigma,
                    m_input_to_hidden_delta);
    MatrixScalarMul(v_hidden_bias_delta, dt * parms->proposal_sigma,
                    v_hidden_bias_delta);
    MatrixScalarMul(v_output_bias_delta, dt * parms->proposal_sigma,
                    v_output_bias_delta);
    ...
    MatrixAdd(m_hidden_to_output_delta, ae->m_hidden_to_output,
              ae->m_hidden_to_output);
    MatrixAdd(m_input_to_hidden_delta, ae->m_input_to_hidden,
              ae->m_input_to_hidden);
    AEactivateLayer(ae, ae->v_input);
...
static int aeApplyGradient(AE *ae, SAE_INTEGRATION_PARMS *parms, double dt) {
  ...
  if (v_grad_output_bias == nullptr) {
    v_grad_output_bias      = VectorCopy(ae->v_grad_output_bias, nullptr);
    v_grad_hidden_bias      = VectorCopy(ae->v_grad_hidden_bias, nullptr);
    m_grad_hidden_to_output = MatrixCopy(ae->m_grad_hidden_to_output, nullptr);
    m_grad_input_to_hidden  = MatrixCopy(ae->m_grad_input_to_hidden, nullptr);
  }

  v_grad_output_bias =
      MatrixScalarMul(parms->v_dir_output_bias, dt, v_grad_output_bias);
  v_grad_hidden_bias =
      MatrixScalarMul(parms->v_dir_hidden_bias, dt, v_grad_hidden_bias);
  m_grad_hidden_to_output = MatrixScalarMul(parms->m_dir_hidden_to_output, dt,
                                            m_grad_hidden_to_output);
  m_grad_input_to_hidden =
      MatrixScalarMul(parms->m_dir_input_to_hidden, dt, m_grad_input_to_hidden);
  ...
  MatrixAdd(m_grad_hidden_to_output, ae->m_hidden_to_output,
            ae->m_hidden_to_output);
  MatrixAdd(m_grad_input_to_hidden, ae->m_input_to_hidden,
            ae->m_input_to_hidden);
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(noutputs, nhidden, dt) shared(ae) schedule(static, 1)
#endif
      for (k = 1; k <= noutputs; k++) {
    ROMP_PFLB_begin
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    ...
    VECTOR_ELT(ae->v_grad_hidden_bias, j) +=
        wt * dE_dbj; // XXX added division by noutputs
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    ...
    for (j = 1; j <= nhidden; j++) {
      ...
      if (dE_dwij < 0)
        DiagBreak();
      *MATRIX_RELT(ae->m_grad_input_to_hidden, j, i) +=
          wt * dE_dwij; // XXX added division by noutputs
                                    ...
                                    // gradient
  {
    // compute sparsity
    for (j = 0; j < nhidden; j++) {
      ae->average_act[j] = .999 * ae->average_act[j] +
                           (1 - .999) * VECTOR_ELT(ae->v_hidden, j + 1);
...
VECTOR *SAEfillInputVector(MRI **mri, int nlevels, int x0, int y0, int z0,
                           int whalf, VECTOR *v_input) {
  int    i, n, xk, yk, zk;
...
MRI *SAEinputWeightsToMRI(SAE *sae, MRI *mri) {
  ...
  if (mri == nullptr) {
    if (sae->type & AUTOENCODER_2D)
      mri = MRIallocSequence(wsize, wsize, 1, MRI_FLOAT,
                             sae->first->v_hidden->rows * sae->nlevels);
    else
      mri = MRIallocSequence(wsize, wsize, wsize, MRI_FLOAT,
                             sae->first->v_hidden->rows * sae->nlevels);
  }
      ...
      for (xk = 0; xk < wsize; xk++)
        for (yk = 0; yk < wsize; yk++)
          for (zk = 0; zk < mri->depth; zk++, i++)
            MRIsetVoxVal(
                mri, xk, yk, zk, frame,
                *MATRIX_RELT(sae->first->m_input_to_hidden, hidden + 1, i));
...
int SAEwrite(SAE *sae, char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "SAEwrite(%s): could not open file", fname));

  n = fprintf(fp, "%d %d %lf %d %d\n", sae->whalf, sae->nencoders, sae->scale,
              sae->type, sae->nlevels);
  if (n != 5) {
    fprintf(stderr,
            "File(%s) Warning: expected number of values to read does not "
            "match those read\n",
            fname);
  }
...
static AE *AEread(FILE *fp, AE *prev) {
  ...
  if (feof(fp))
    return (nullptr);
  m_input_to_hidden = MatrixReadFrom(fp, nullptr);
  v_output_bias     = MatrixReadFrom(fp, nullptr);
  ae = AEalloc(prev, m_input_to_hidden->cols, m_input_to_hidden->rows,
               v_output_bias->rows);
...
SAE *SAEread(char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "SAEread(%s): could not open file", fname));

  n = fscanf(fp, "%d %d %lf %d %d\n", &whalf, &nencoders, &scale, &type,
             &nlevels);
  if (n != 5) {
    fprintf(stderr,
            "File(%s) Warning: expected number of values to read does not "
  ...
  ae = sae->first = AEread(fp, nullptr);
...
void AEdump(AE *ae) {
  if (ae->v_input->rows == 1) {
    printf("f(I=%2.2f * wij=%2.2f + bj=%2.2f = %2.2f) = %2.2f * wjk=%2.2f + "
           "bk=%2.2f = %2.2f, E=%2.2f\n",
           ae->v_input->rptr[1][1], ae->m_input_to_hidden->rptr[1][1],
           ae->v_hidden_bias->rptr[1][1], ae->v_hidden_net->rptr[1][1],
           ae->v_hidden->rptr[1][1], ae->m_hidden_to_output->rptr[1][1],
           ae->v_output_bias->rptr[1][1], ae->v_output->rptr[1][1],
  ...
  if (ae->m_grad_input_to_hidden && false) {
...
static int AEsaveState(AE *ae) {
  ae->m_saved_input_to_hidden =
      MatrixCopy(ae->m_input_to_hidden, ae->m_saved_input_to_hidden);
  ae->m_saved_hidden_to_output =
      MatrixCopy(ae->m_hidden_to_output, ae->m_saved_hidden_to_output);
  ae->v_saved_hidden_bias =
      MatrixCopy(ae->v_hidden_bias, ae->v_saved_hidden_bias);
  ae->v_saved_output_bias =
      MatrixCopy(ae->v_output_bias, ae->v_saved_output_bias);
  return (NO_ERROR);
}
...
CSAE *CSAEalloc(int type, int nlayers, int *ksizes, int *ngroups,
                MRI *mri_inputs) {
  ...
  for (layer = 0; layer < nlayers; layer++) {
    csae->mri_outputs[layer] =
        MRIallocSequence(mri_inputs->width, mri_inputs->height,
                         mri_inputs->depth, MRI_FLOAT, ngroups[layer]);
    MRIcopyHeader(mri_inputs, csae->mri_outputs[layer]);
  }
  return (csae);
}
int CSAEfillInputs(CSAE *csae, MRI *mri_inputs, VECTOR *v_visible, int x0,
...
MRI *CSAEcreateOutputs(CSAE *csae, MRI *mri_inputs, int first_layer,
                       int last_layer) {
  int layer, x, y, z, h;
  // int whalf;
  MRI *      mri_layer_inputs, *mri_outputs = nullptr;
  AE *       ae, *next;
  static int callno = 0;
    ...
    for (x = 0; x < mri_inputs->width; x++) {
      ...
      for (y = 0; y < mri_inputs->height; y++)
        for (z = 0; z < mri_inputs->depth; z++) {
          if (x == Gx && y == Gy)
            DiagBreak();
          CSAEfillInputs(csae, mri_layer_inputs, ae->v_input, x, y, z,
                         ae->ksize);
          next     = ae->next;
          ae->next = nullptr;
          AEactivateLayer(ae, ae->v_input);
          ...
          for (h = 0; h < ae->v_hidden->rows; h++)
            MRIsetVoxVal(mri_outputs, x, y, z, h,
                         VECTOR_ELT(ae->v_hidden, h + 1));
        }
    }
...
// NOTE: input MRI must be type float and scaled to be 0->1 (that is, scale down
// MRI_UCHAR vols by 255)
double CSAEtrainLayerFromVoxlist(CSAE *csae, int layer, VOXEL_LIST *vl,
                                 MRI **                 mri_pyramid,
                                 SAE_INTEGRATION_PARMS *parms) {
  double total_rms, last_total_rms, pct_decrease, running_last_rms = 0,
                                                  running_rms;
  int iter = 0, ind, *indices, end_index, end_mini_batch, always, never, nbad,
      i;
  // double dt;
  ...
  if (mri_pyramid[0]->type != MRI_FLOAT)
    ErrorExit(ERROR_BADPARM, "CSAEtrainLayerFromVoxlist: input type must be "
                             "MRI_FLOAT scaled to [0->1]");

  ae_train = SAEfindLastLayer(csae->sae, nullptr);
  ...
  do {
    ...
    if (iter == 0) {
      ...
      min_rms = last_total_rms =
          CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices, 0,
                                end_index, &always, &never);
      printf("%3.3d: rms = %2.4f\n", iter, last_total_rms);
    }
    ...
    for (i = ind = 0; ind <= end_index; ind += parms->mini_batch_size) {
      end_mini_batch =
          MIN(ind + parms->mini_batch_size - 1, vl->nvox - parms->held_out);
      if (i && (i % (nint(200.0 / parms->mini_batch_size)) == 0))
        running_last_rms =
            CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices,
                                  ind, end_mini_batch, &always, &never);

      CSAEcomputeGradient(csae, ae_train, vl, mri_pyramid[0], ind,
                          end_mini_batch, indices, parms);
      ...
      if (i && (i % (nint(200.0 / parms->mini_batch_size)) == 0)) {
        ...
        total_rms =
            CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl, indices,
                                  0, end_index, &always, &never);
        running_pct_dec =
            100 * (running_last_rms - running_rms) / (running_last_rms);
        pct_decrease = 100 * (last_total_rms - total_rms) / (last_total_rms);
        printf("%d of %d (%2.2f%%): rms = %2.4f (%2.3f%%) (%2.3f%%)\n",
               end_mini_batch + 1, end_index, 100 * ind / (float)end_index,
               total_rms, pct_decrease, running_pct_dec);
      }
...
//    total_rms /= visited ; last_total_rms = running_last_rms / visited ;
    ...
    total_rms = CSAEcomputeVoxlistRMS(csae, parms, layer, mri_pyramid, vl,
                                      indices, 0, end_index, &always, &never);
    if (always || never)
    ...
    if (total_rms >= min_rms) {
      ...
      printf("RMS %2.4f >=  min RMS %2.4f --> no progress = %d (max %d)\n",
             total_rms, min_rms, nbad, parms->max_no_progress);
    } else {
...
MRI *CSAElayerWeightsToMRI(CSAE *csae, int layer) {
  ...
  if (layer >= csae->sae->nencoders)
    ErrorExit(ERROR_BADPARM,
              "CSAlayerWeightsToMRI: invalid layer %d (%d encoders)\n", layer,
              csae->sae->nencoders);

  if (layer <= 0)
    return (SAEinputWeightsToMRI(csae->sae, nullptr));
  else if (layer >= 1) {
    ae         = csae->aes[layer];
          ...
          for (y = 0; y < ae->ksize; y++, v++) {
            for (xp = 0; xp < ae_prev->ksize; xp++) {
              ...
              for (yp = 0; yp < ae_prev->ksize; yp++) {
                ...
                if (xk == Gx && yk == Gy)
                  DiagBreak();
                if (h == 0 && x == ae->ksize - 1 && (Gdiag & DIAG_SHOW) &&
...
MRI *SAElayerWeightsToMRI(SAE *sae, int layer) {
  ...
  if (layer <= 0)
    return (SAEinputWeightsToMRI(sae, nullptr));
  else if (layer >= 1) {
    int l;
...
double CSAEcomputeVoxlistRMS(CSAE *csae, SAE_INTEGRATION_PARMS *parms,
                             int layer, MRI **mri, VOXEL_LIST *vl, int *indices,
                             int start_index, int end_index, int *always,
                             int *never) {
  int            x, y, z, nvox, i, ind, *histo, h, nhidden, iz, num_indices;
  double         class_rms, rms, total_rms, total_class_rms;
  AE *           ae;
  static double *last_rms         = nullptr;
  static double *last_class_rms   = nullptr;
  ...
  if (last_num_indices != num_indices && last_rms) {
    free(last_rms);
    free(last_class_rms);
    last_rms = last_class_rms = nullptr;
  }
  if (last_rms == nullptr) {
  ...
  total_rms = (1.0 - parms->class_weight) * total_rms +
              parms->class_weight * total_class_rms;
  if (always) {
    *always = *never = 0;
    for (h = 0; h < nhidden; h++) {
      if (histo[h] == 0)
...
CSAE *CSAEread(char *fname) {
  ...
  for (layer = 0; layer < sae->nencoders; layer++) {
    ae = csae->aes[layer];
    csae->mri_outputs[layer] =
        MRIallocSequence(csae->sae->vg.width, csae->sae->vg.height,
                         csae->sae->vg.depth, MRI_FLOAT, ae->v_hidden->rows);
