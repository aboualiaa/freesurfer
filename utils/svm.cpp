...
#if USE_SVM_LIB
int SVMtrain(SVM *svm, float **x, float *y, int ntraining, double C, double tol,
             int max_iter) {
  SVMparam *svmp;
  SVMreal **data, *alpha;
  ...
  for (negCount = i = 0; i < ntraining; i++) {
    ...
    data[negCount + posCount] =
        (SVMreal *)calloc(svm->ninputs, sizeof(SVMreal));
...
int SVMtrain(SVM *svm, float **x, float *y, int ntraining, double C, double tol,
             int max_iter) {
  int iter, i, j, k, Nsupport, *permutation, *moved, *failed, nmoved, dir,
      keep_alpha;
  double **dot_matrix, dot, step_size, L, Lold, Lprev, *da_old, da, b,
  ...
  if (nullptr == dot_matrix || nullptr == svm->alpha || nullptr == da_old)
    ErrorExit(ERROR_NOMEMORY, "SVMtrain: could not allocated dot_matrix");

  for (i = 0; i < ntraining; i++) {
    dot_matrix[i] = (double *)calloc(ntraining, sizeof(double));
    if (nullptr == dot_matrix[i])
      ErrorExit(ERROR_NOMEMORY, "SVMtrain: could not allocated dot_matrix[i]",
                i);
    if (keep_alpha == 0)
      svm->alpha[i] = 0.0;
  }
  ...
  for (iter = 0; iter < max_iter; iter++) {
    ...
    for (k = 0; k < ntraining; k++) {
      ...
      /* pick a second training vector that wants to move in the opposite
         direction to preserve sum(yi*alpha(i)) == 0.
      */
...
SVM *SVMread(char *fname) {
  ...
  while (nullptr != cp) {
    ...
    if (stricmp(token, "C") == 0) {
      ...
      svm->alpha = (double *)calloc(svm->ntraining, sizeof(double));
      if (svm->alpha == nullptr)
        ErrorExit(ERROR_NOMEMORY, "SVMread(%s) - %d calloc failed", fname,
                  svm->ntraining);
      ...
      if (!svm->args)
        ErrorExit(ERROR_BADPARM,
                  "SVMread(%s): could not allocate %d extra args",
                  svm->extra_args);
      for (i = 0; i < svm->extra_args; i++) {
        cp = fgetl(line, STRLEN - 1, fp);
        if (!cp)
          ErrorExit(ERROR_BADFILE,
                    "SVMread(%s): couldn't read %dth extra arg from file",
                    fname, i);

        svm->args[i] = (char *)calloc(strlen(cp) + 1, sizeof(char));
        if (!svm->args[i])
          ErrorExit(ERROR_BADPARM,
                    "SVMread(%s): could not allocate %dth extra arg", i);
