...
MRI *MRIconform(MRI *mri) {
  ...
  res = MRIcopy(mri, nullptr); /* don't mess with the input */
  ...
  if (res->xsize != templ->xsize || res->ysize != templ->ysize ||
      res->zsize != templ->zsize || res->width != templ->width ||
      res->height != templ->height || res->depth != templ->depth ||
      res->x_r != templ->x_r || res->x_a != templ->x_a ||
      res->x_s != templ->x_s || res->y_r != templ->y_r ||
      res->y_a != templ->y_a || res->y_s != templ->y_s ||
      res->z_r != templ->z_r || res->z_a != templ->z_a ||
      res->z_s != templ->z_s) {
...
MRI *MRIconformedTemplate(MRI *mri, int conform_width, double conform_size,
  ...
  mri_template->xstart = mri_template->ystart = mri_template->zstart =
      -conform_width / 2;
  mri_template->xend = mri_template->yend = mri_template->zend =
      conform_width / 2;
  ...
  if (KeepDC) {
    ...
    // pad is the number of conformed voxels of padding when Nvox !=
    // conform_width set pad this way makes the C_RASs be about the same under
    // general conditions
