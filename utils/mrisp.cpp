...

{
    ...
    for (u = 0; u <= U_MAX_INDEX(mrisp); u++) {
      for (v = 0; v <= V_MAX_INDEX(mrisp); v++) {
        ...
        if (filled[u][v] == UNFILLED_ELT) {
          ...
          MHTfindClosestFaceGeneric(mht, mris, x, y, z, 1000, -1, -1, &face,
                                    &fno, &fdist);
          if (face) {
            float val;
            VERTEX *v0, *v1, *v2;
            ...
            val = MRISsampleFace(mris, fno, CANONICAL_VERTICES, x, y, z,
                                 v0->curv, v1->curv, v2->curv);
            filled[u][v] = 1;
  ...
  MHTfree(&mht);
  if ((Gdiag & DIAG_SHOW) && DIAG_VERBOSE_ON)
    fprintf(stderr, "backsampling filled %d elements \n", nfilled);
...
MRI_SP *MRIStoParameterizationBarycentric(MRIS *mris, MRI_SP *mrisp,
                                          const float *overlay, float scale,
                                          int frameno) {
  float a, b, c, phi, theta, x, y, z, uf, vf, d, total_d, **distances, *fp;
  int vno, u, v, unfilled, **filled, nfilled;
  VERTEX *vertex;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    /* 0,0 */
    total_d = distances[u][v];
    if ((total_d > 10000.0) || (overlay[vno] > 1000.0))
      DiagBreak();
    if (total_d > 0.0)
      ...
      for (v = 0; v <= V_MAX_INDEX(mrisp); v++) {
        ...
        if (filled[u][v] == UNFILLED_ELT) {
          ...
          if (face) {
            ...
            val = MRISsampleFace(mris, fno, CANONICAL_VERTICES, x, y, z, v0, v1,
                                 v2);
            filled[u][v] = 1;
            *IMAGEFseq_pix(Ip, u, v, frameno) = val;
            nfilled++;
