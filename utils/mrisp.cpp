...
static MRIS *makeCenteredSphere(MRIS *surf) {
  ...
  float distance =
      std::sqrt((surf->xctr * surf->xctr) + (surf->yctr * surf->yctr) +
                (surf->zctr * surf->zctr));

  if (distance > 0.1) {
    fs::warning() << "Sphere is not centered (distance = " << distance
                  << "). Moving center to origin - this might be unstable.";
    surf = MRISclone(surf);
...
static void resetCenteredSphere(MRIS *orig, MRIS *centered) {
  if (orig != centered) {
    for (int vno = 0; vno < orig->nvertices; vno++)
      orig->vertices[vno].curv = centered->vertices[vno].curv;
    MRISfree(&centered);
  }
}
...
MRI_SP *MRIStoParameterization(MRIS *mris, MRI_SP *mrisp, float scale,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (u == 0 && v == 56)
      DiagBreak();
    if ((((u == DEBUG_U) && (v == DEBUG_V)) || (vno == Gdiag_no)) &&
        (fno == 0)) {
...
MRI_SP *MRIStoParameterizationBarycentric(MRIS *mris, MRI_SP *mrisp,
                                          float scale, int frameno) {
  MRIS *original = mris;
  mris           = makeCenteredSphere(mris);
  ...
  for (int vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (total_dist > 0.0)
      *IMAGEFseq_pix(mrisp->Ip, u, v, frameno) += vertex->curv / total_dist;
  }
...
int MRIScoordsFromParameterizationBarycentric(MRIS *mris, MRI_SP *mrisp,
  ...
  if (which_vertices != WHITE_VERTICES)
    ErrorExit(ERROR_UNSUPPORTED,
              "MRIScoordsToParameterizationBarycentric: unsupported "
              "which_vertices %d",
              which_vertices);
  ...
  if (curvs == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "MRIScoordsToParameterizationBarycentric: could not allocate "
              "%d-len curvature vector",
              mris->nvertices);
...
MRI_SP *MRIScoordsToParameterization(MRIS *mris, MRI_SP *mrisp, float scale,
                                     int which_vertices) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if ((((u == DEBUG_U) && (v == DEBUG_V)) || (Gdiag_no == vno)) &&
      ...
      fprintf(stderr,
              ...
              "x = (%2.3f,%2.3f,%2.3f)\n",
              vno, x, y, z, d, vertex->origx, vertex->origy, vertex->origz);
  }
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    default:
      ErrorExit(ERROR_UNSUPPORTED,
                "MRIScoordsToParameterization: unsupported vertex set %d",
                which_vertices);
    }
    if (total_d > 0.0) {
...
/*-----------------------------------------------------
        ...
        Description
------------------------------------------------------*/
MRIS *MRIScoordsFromParameterization(MRI_SP *mrisp, MRIS *mrisInit,
                                     int which_vertices) {
  float rad, phi, theta, uf, vf, du, dv, origx, origy, origz, d, val1, val2,
      val3, val4;
  int vno, u0, v0, u1, v1;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    float y = vertex->y;
    float z = vertex->z;
    // DNG 7/19/18: changed to use a vertex specific radius rather than the mean
    // radius
    ...
    origx = du * dv * *IMAGEFseq_pix(mrisp->Ip, u1, v1, 0) +
    ...
    origy = du * dv * *IMAGEFseq_pix(mrisp->Ip, u1, v1, 1) +
    ...
    origz = du * dv * *IMAGEFseq_pix(mrisp->Ip, u1, v1, 2) +
    ...
    default:
      ErrorExit(ERROR_UNSUPPORTED,
                "MRIScoordsFromParameterization: which = %d not supported",
                which_vertices);
    }
  }
...
/*-----------------------------------------------------
        ...
        Description
------------------------------------------------------*/
void MRISPfunctionVal_radiusR( // returns the value that would be stored in
                               // resultsForEachFno[0] for fnoLo
    MRI_SURFACE_PARAMETERIZATION *mrisp,
    MRISPfunctionValResultForAlpha
        * resultsForEachAlpha, // must be numAlphas elements
    float r, float x, float y, float z, int fnoLo,
    bool getNextAlso, // always fills in resultsForEachAlpha.curr for fno,
                      // optionally fills in .next for fno+1
    float const *alphas,
    float
         numAlphas, // rotate x,y,z by these alphas (radians) and get the values
    bool trace) // note: this rotation is around the z axis, hence z does not
                // change
{
  float phi;
  {
  ...
  for (alphaIndex = 0; alphaIndex < numAlphas; alphaIndex++) {
    ...
    int u0_v1 = v1 + u0_voff;
    while (u0_v1 >= V_DIM(mrisp))
      u0_v1 -= V_DIM(mrisp);
    int u0_v0 = v0 + u0_voff;
    while (u0_v0 >= V_DIM(mrisp))
      u0_v0 -= V_DIM(mrisp);
    int u1_v1 = v1 + u1_voff;
    while (u1_v1 >= V_DIM(mrisp))
      u1_v1 -= V_DIM(mrisp);
    int u1_v0 = v0 + u1_voff;
    ...
    if (false) {
      ...
      if (statsCount == statsLimit) {
        if (statsLimit < 10000000)
          statsLimit *= 2;
        else
          statsLimit += 10000000;
        fprintf(stdout, "%s:%d statsCount:%g V_DIM avg:%g\n", __FILE__,
                __LINE__, (float)statsCount,
                (double)statsV_DIMSum / (double)statsCount);
      }
    }
    ...
    for (i = 0; i < (getNextAlso ? 2 : 1); i++) {
      ...
      double val =
          ...
          (1.0f - du) * (1.0f - dv) *
              *IMAGEFseq_pix(mrisp->Ip, u0, u0_v0, fno) +
          du * (1.0f - dv) * *IMAGEFseq_pix(mrisp->Ip, u1, u1_v0, fno);
      if (trace) {
        fprintf(stdout,
                "%s:%d x:%g y:%g z:%g returns %g for fno:%d phi:%g "
                "alphaIndex:%d theta:%g\n",
                __FILE__, __LINE__, x, y, z, val, fno, phi, alphaIndex, theta);
      }
      *(i ? &resultsForEachAlpha[alphaIndex].next
          : &resultsForEachAlpha[alphaIndex].curr) = val;
    }
  }
}
...
double MRISPfunctionValTraceable(MRI_SURFACE_PARAMETERIZATION *mrisp,
  ...
  MRISPfunctionVal_radiusR(mrisp, &result, r, x, y, z, fno, false, &zero, 1,
                           trace);
  return result.curr;
}
...
MRI_SP *MRISPclone(MRI_SP *mrisp_src) {
  ...
  mrisp_dst->Ip    = ImageCopy(mrisp_src->Ip, nullptr);
...
MRI_SP *MRISPconvolveGaussian(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma,
  ...
  double d, k, total, ktotal, sigma_sq_inv, theta, phi, theta1, phi1, sin_phi,
      cos_phi, sin_phi1, cos_phi1;
...
/*-----------------------------------------------------
        ...
        Description
------------------------------------------------------*/
static MRI_SP *MRISPblur_new(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma,
                             int fno);
static MRI_SP *MRISPblur_old(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma,
                             int fno);
...
MRI_SP *MRISPblur(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma, int fno) {
  ...
  return (do_old ? MRISPblur_old : MRISPblur_new)(mrisp_src, mrisp_dst, sigma,
                                                  fno);
}
...
static MRI_SP *MRISPblur_new(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma,
  ...
  IMAGE *Ip_src_init;
  {
    no_sphere_init = getenv("NO_SPHERE") != nullptr;
    if (no_sphere_init)
  ...
  static int *uToKHalfCache           = nullptr;

  static int     kHalfHi          = 0;
  static double *uukvkToExpResult = nullptr;

  if (uMax != U_DIM(mrisp_src) || uToKHalfCache_PHI_DIM != PHI_DIM(mrisp_src) ||
    ...
    uukvkToExpResult = (double *)realloc(
        uukvkToExpResult, uMax * kHalfHi * kHalfHi * sizeof(double));
...
#pragma omp parallel for if_ROMP(assume_reproducible) collapse(2)
  ...
  {
    for (u = 0; u < U_DIM(mrisp_src); u++) {
      ROMP_PFLB_begin double const *const ukvkToExpResult =
          uukvkToExpResult + u * kHalfHi * kHalfHi;
...
static MRI_SP *MRISPblur_old(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma,
  ...
  IMAGE *Ip_src_init;
  {
    ...
    no_sphere = getenv("NO_SPHERE") != nullptr;
    if (no_sphere)
      fprintf(stderr, "disabling spherical geometry\n");
...
#pragma omp parallel for if_ROMP(assume_reproducible) collapse(2)
  ...
  {
    for (u = 0; u < U_DIM(mrisp_src); u++) {
      ROMP_PFLB_begin int k, klen, khalf;
      double              phi, sin_sq_u;
...
MRI_SP *MRISPalloc(float scale, int nfuncs) {
  ...
  if (!mrisp)
    ErrorExit(ERROR_NOMEMORY, "MRISPalloc(%d, %d): allocation failed", u_dim,
              v_dim);
...
int MRISPwrite(MRI_SP *mrisp, const char *fname) {
  ...
  if (!strcmp(FileNameExtension(fname, ext), "mgz")) {
    ...
    mri = MRIalloc(mrisp->Ip->cols, mrisp->Ip->rows, mrisp->Ip->num_frame,
                   MRI_FLOAT);
...
MRI_SP *MRISPread(char *fname) {
  ...
  if (type == MRI_MGH_FILE) {
    ...
    mri = MRIread(fname);
    if (mri == nullptr)
      ErrorReturn(NULL, (ERROR_NOFILE, "MRISPread(%s): could not read MGH file",
                         fname));
  ...
  if (!mrisp->Ip)
    ErrorReturn(NULL,
                (ERROR_NOFILE, "MRISPread(%s): could not open file", fname));
...
double MRISParea(MRI_SP *mrisp) {
  double total_area = 0.0, phi0, theta0, phi1, theta1, radius, l1, l2, x0, y0,
...
int MRISPcoordinate(MRI_SP *mrisp, float x, float y, float z, int *pu,
                    int *pv) {
...
int MRISPfunctionVectorVals(MRI_SURFACE_PARAMETERIZATION *mrisp, MRIS *mris,
                            float x, float y, float z, int *frames, int nframes,
                            double *vals) {
...
MRIS *MRISfromParameterizations(MRI_SP *mrisp, MRIS *mris, int *frames,
...
MRI_SP *MRIStoParameterizations(MRIS *mris, MRI_SP *mrisp, float scale,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (total_d > 0.0) {
      for (m = 0; m < nframes; m++) {
        *IMAGEFseq_pix(mrisp->Ip, u, v, frames[m]) +=
            vp->vals[indices[m]] / total_d;
      }
    }
  }
    ...
    for (u = 0; u <= U_MAX_INDEX(mrisp); u++) {
      for (v = 0; v <= V_MAX_INDEX(mrisp); v++) {
        ...
        if (filled[u][v] == UNFILLED_ELT) {
          ...
          for (m = 0; m < nframes; m++)
            *IMAGEFseq_pix(Itmp, u, v, frames[m]) =
                *IMAGEFseq_pix(Ip, u, v, frames[m]);
        }
      }
    }
    for (u = 0; u <= U_MAX_INDEX(mrisp); u++) {
...
*/
BarycentricSphericalProjector::BarycentricSphericalProjector(MRIS *surf, MRI_SP *param) :
  mris(surf),
  mrisp(param)
{
  // cache input surface
  ...
  // build interpolator for backwards-sampling
...
BarycentricSphericalProjector::~BarycentricSphericalProjector()
{
  // free the fill-marker and distance arrays
  for (int u = 0; u <= u_max_index; u++) {
    free(filled[u]);
...
/*
  ...
  given frame index.
*/
void BarycentricSphericalProjector::projectOverlay(const float *overlay, int frameno)
{
  // clear frame
  ImageClearArea(mrisp->Ip, -1, -1, -1, -1, 0, frameno);
  ...
  for (int vno = 0; vno < mris->nvertices; vno++) {
    ...
    int v = vertex_v[vno];
    float total_dist = distances[u][v];
    if (total_dist > 0.0) *IMAGEFseq_pix(mrisp->Ip, u, v, frameno) += overlay[vno] / total_dist;
  }
