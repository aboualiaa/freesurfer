...
static int labelGetVoxelCoords(LABEL *area, LABEL_VERTEX *lv, float *px,
                               float *py, float *pz);
static int labelGetSurfaceRasCoords(LABEL *area, LABEL_VERTEX *lv, float *px,
                                    float *py, float *pz);
...
LABEL *LabelReadFrom(const char *subject_name, FILE *fp) {
  ...
  if (!sscanf(cp, "%d", &area->n_points)) {
    printf("\n%s\n", cp);
    ErrorReturn(NULL,
                (ERROR_BADFILE, "%s: could not scan # of lines from label file",
                 Progname));
  }
  ...
  if (!area->lv)
    ErrorExit(ERROR_NOMEMORY,
              "%s: LabelReadFrom could not allocate %d-sized vector", Progname,
  ...
  while ((cp = fgetl(line, STRLEN, fp)) != nullptr) {
    if (sscanf(cp, "%d %f %f %f %f", &vno, &x, &y, &z, &stat) != 5)
      ErrorReturn(NULL, (ERROR_BADFILE,
                         "%s: could not parse %dth line '%s' in label file",
...
int LabelFromCanonical(LABEL *area, MRI_SURFACE *mris) {
  ...
  for (n = 0; n < area->n_points; n++) {
    MRISPcoordinate(mrisp, area->lv[n].x, area->lv[n].y, area->lv[n].z, &ui,
                    &vi);
...
int LabelFromTalairach(LABEL *area, MRI_SURFACE *mris) {
  ...
  for (n = 0; n < area->n_points; n++) {
    ...
    vno             = MRIStalairachToVertex(mris, area->lv[n].x, area->lv[n].y,
                                area->lv[n].z);
    v               = &mris->vertices[vno];
    area->lv[n].vno = vno;
    area->lv[n].x   = v->origx;
...
int LabelToFlat(LABEL *area, MRI_SURFACE *mris) {
  ...
  for (n = 0; n < area->n_points; n++) {
    ...
    {
      vno = MHTfindVnoOfClosestVertexInTable(mht, mris, area->lv[n].x,
                                             area->lv[n].y, area->lv[n].z, 0);
      if (vno < 0) {
        continue;
      }
      VERTEX *v = &mris->vertices[vno];
  ...
  for (n = 0; n < area->n_points; n++)
    if (!area->lv[n].deleted) {
      nbytes = fprintf(fp, "%d  %2.3f  %2.3f  %2.3f %10.10f\n", area->lv[n].vno,
                       area->lv[n].x, area->lv[n].y, area->lv[n].z,
                       area->lv[n].stat);
      if (nbytes < 0) {
...
int LabelWrite(LABEL *area, const char *label_name) {
  ...
  label_name = lname;
  cp         = strrchr(lname, '/');
  if ((cp == nullptr) && strlen(area->subject_name) > 0) {
    cp = getenv("SUBJECTS_DIR");
    if (!cp)
      ErrorExit(ERROR_BADPARM,
                "%s: no subject's directory specified in environment "
  ...
  if (!fp)
    ErrorReturn(
        ERROR_NOFILE,
        (ERROR_NO_FILE, "%s: could not open label file %s", Progname, fname));
...
int LabelIntersect(LABEL *area1, LABEL *area2) {
  ...
  if (!isec)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate LabelIntersect struct.",
              Progname);
  for (n = 0; n < vnum; n++) {
    isec[n] = -1;
  }
...
LABEL *LabelAlloc(int max_points, const char *subject_name,
  ...
  if (!area->lv)
    ErrorExit(ERROR_NOMEMORY,
              "%s: LabelAlloc(%s) could not allocate %d-sized vector", Progname,
              label_name ? label_name : "", sizeof(LV) * area->n_points);
  for (n = 0; n < area->max_points; n++)
    area->lv[n].vno = -1; // mark them as unassigned (0 is a valid vertex index)
  strcpy(area->space, "TkReg");
  area->coords = LABEL_COORDS_TKREG_RAS; // would like to use scanner RAS, but
                                         // need an volume or header
  return (area);
}
/*-----------------------------------------------------
        Parameters:
...
int LabelCurvFill(LABEL *area, int *vertex_list, int nvertices,
  ...
  do {
    ...
    for (n = 0;
         n < area->n_points && area->n_points + nfilled < area->max_points;
         n++) {
...
static Transform *labelLoadTransform(const char *subject_name, const char *sdir,
  ...
  sprintf(xform_fname, "%s/%s/mri/transforms/talairach.xfm", sdir,
          subject_name);
...
MATRIX *LabelCovarianceMatrix(LABEL *area, MATRIX *mCov) {
  ...
  mCov = MatrixCovariance(mInputs, mCov, nullptr);
...
LABEL *LabelCombine(LABEL *asrc, LABEL *adst) {
  ...
  if (adst->max_points <
      asrc->n_points + adst->n_points) /* won't fit - expand */
  {
    atmp = LabelAlloc(2 * (asrc->n_points + adst->n_points), asrc->subject_name,
                      asrc->name);
    LabelCopy(adst, atmp);
    LabelFree(&adst);
    adst = atmp;
...
LABEL *LabelCopy(LABEL *asrc, LABEL *adst) {
  ...
  if (asrc->vertex_label_ind) {
    ...
    if (adst->vertex_label_ind == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "LabelCompact: could not allocate %d long label index array",
                asrc->max_points);
    memmove(adst->vertex_label_ind, asrc->vertex_label_ind,
            asrc->max_points * sizeof(int));
  }
  ...
  for (n1 = 0; n1 < area->n_points; n1++) {
    ...
    for (n2 = n1 + 1; n2 < area->n_points; n2++) {
      ...
      if (lv1->vno >= 0 && lv2->vno >= 0 && lv1->vno == lv2->vno) {
        ...
        if (FEQUAL(lv1->x, lv2->x) && FEQUAL(lv1->y, lv2->y) &&
            FEQUAL(lv1->z, lv2->z)) {
          deleted++;
          lv2->deleted = 1;
        }
  ...
  for (n1 = 0; n1 < area->n_points; n1++) {
    ...
    for (n2 = n1 + 1; n2 < area->n_points; n2++) {
      ...
      if (lv1->vno >= 0 && lv2->vno >= 0 && lv1->vno == lv2->vno) {
        ...
        if (fabs(lv1->x - lv2->x) < dist && fabs(lv1->y - lv2->y) < dist &&
            fabs(lv1->z - lv2->z) < dist) {
          deleted++;
          lv2->deleted = 1;
        }
      }
...
LABEL *LabelCompact(LABEL *lsrc, LABEL *ldst) {
  ...
  if (ldst != lsrc && lsrc->vertex_label_ind) {
    ...
    if (ldst->vertex_label_ind == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "LabelCompact: could not allocate %d long label index array",
                lsrc->max_points);
  }
...
double LabelMeanIntensity(LABEL *area, MRI *mri) {
  ...
  area2 = LabelToVoxel(area, mri, nullptr);
...
int LabelMarkUndeleted(LABEL *area, MRI_SURFACE *mris) {
  ...
  if (nullptr == area || nullptr == mris)
    ErrorReturn(ERROR_BADPARM,
                (ERROR_BADPARM, "LabelMarkUndeleted: area or mris was NULL"));
...
LABEL *LabelFromMarkedSurface(MRI_SURFACE *mris) {
  ...
  area = LabelAlloc(npoints, nullptr, nullptr);
...
int LabelFillUnassignedVertices(MRI_SURFACE *mris, LABEL *area, int coords) {
  ...
  MRIScomputeVertexSpacingStats(mris, nullptr, nullptr, &max_spacing, nullptr,
                                &max_vno, coords);
  ...
  num_not_found = 0;
  if (area->mri_template == nullptr) {
    area->mri_template =
        MRIalloc(mris->vg.width, mris->vg.height, mris->vg.depth, MRI_UCHAR);
    useVolGeomToMRI(&mris->vg, area->mri_template);
  }

  if (area->mris == nullptr)
    area->mris = mris;
  for (n = 0; n < area->n_points; n++) {
    lv = &area->lv[n];
    if (lv->vno >= 0 && lv->vno <= mris->nvertices) {
    ...
    if (min_vno == -1) {
      ...
      case CURRENT_VERTICES:
        min_vno = MRISfindClosestVertex(mris, lv->x, lv->y, lv->z, nullptr,
                                        CURRENT_VERTICES);
        break;
...
LABEL *LabelSphericalCombine(MRI_SURFACE *mris, LABEL *asrc,
  ...
  for (n = 0; n < asrc->n_points; n++) {
    ...
    float         min_dist;
    VERTEX *const vdst =
        MHTfindClosestVertex2(mht, mris_dst, mris, v, &min_dist);
    if (!vdst) {
  ...
  MRIScomputeVertexSpacingStats(mris, nullptr, nullptr, &max_len, nullptr,
                                nullptr, CURRENT_VERTICES);
  mht_src =
      MHTcreateVertexTable_Resolution(mris, CURRENT_VERTICES, 2 * max_len);
    ...
    */
    ...
    for (n = 0; n < adst->n_points; n++) {
      VERTEX_TOPOLOGY const *const vt =
          &mris_dst->vertices_topology[adst->lv[n].vno];
      ...
      {
        float               min_dist;
        VERTEX const *const vsrc =
            MHTfindClosestVertex2(mht_src, mris, mris_dst, v, &min_dist);
      ...
      for (m = 0; m < vt->vnum; m++) {
        ...
        float               min_dist;
        VERTEX const *const vsrc =
            MHTfindClosestVertex2(mht_src, mris, mris_dst, vn, &min_dist);
        if (vsrc == nullptr) {
          DiagBreak();
        }
        ...
        if (vsrc->marked) {
          if (adst->n_points >= adst->max_points - 1) {
            ...
            atmp =
                LabelAlloc(2 * adst->n_points, adst->subject_name, adst->name);
            LabelCopy(adst, atmp);
            LabelFree(&adst);
            adst = atmp;
          }
...
int LabelErode(LABEL *area, MRI_SURFACE *mris, int num_times) {
  ...
  for (n = 0; n < num_times; n++) {
    ...
    for (label_vno = 0; label_vno < area->n_points; label_vno++) {
      ...
      if (vno >= mris->nvertices)
        ErrorExit(ERROR_BADPARM,
                  "LabelErode: label vertex %d too big for surface (%d)", vno,
                  mris->nvertices);
      if (vno == Gdiag_no)
        DiagBreak();
      ...
      for (neighbor_index = 0;
           neighbor_index < mris->vertices_topology[vno].vnum;
           neighbor_index++) {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      if (found) {
        ...
        if (area->mris && area->mri_template) {
          double xv, yv, zv;
          MRISsurfaceRASToVoxel((MRIS *)area->mris, area->mri_template, lv->x,
                                lv->y, lv->z, &xv, &yv, &zv);
          lv->xv = nint(xv);
          lv->yv = nint(yv);
          lv->zv = nint(zv);
...
LABEL *LabelRealloc(LABEL *lb, int max_points) {
  ...
  if (lb == nullptr)
    return (LabelAlloc(max_points, nullptr, nullptr));
  ...
  memset(lvtmp + lb->max_points, 0,
         (max_points - lb->max_points) * sizeof(LABEL_VERTEX));
...
#endif
    if (xv < nvox || yv < nvox || zv < nvox || xv > (mri->width - 1) - nvox ||
        yv > (mri->height - 1) - nvox || zv > (mri->depth - 1) - nvox) {
      continue;
    }
    npoints++;
  }
...
LABEL *LabelTranslate(LABEL *area, LABEL *area_offset, float dx, float dy,
  ...
  if (area_offset == nullptr) {
    area_offset = LabelCopy(area, nullptr);
  }
...
LABEL *MRISlabelInvert(MRIS *surf, LABEL *label) {
  ...
  tmpmri = MRISlabel2Mask(surf, label, nullptr);
  ...
  invlabel           = LabelAlloc(ninv, label->subject_name, nullptr);
...
int LabelMaskSurfaceCurvature(LABEL *area, MRI_SURFACE *mris) {
  int     vno;
  VERTEX *v;
...
int LabelMaskSurface(LABEL *area, MRI_SURFACE *mris) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    v->stat = v->annotation = v->val = v->imag_val = v->val2 = v->valbak =
        v->val2bak                                           = 0.0;
  }
  MRISclearMarks(mris);
  return (NO_ERROR);
...
/*
  convert the label coords from tkreg (surface) RAS to scanner RAS. Note that
  this assumes that the label coords are in tkreg space
*/
...
LABEL *LabelToSurfaceRAS(LABEL *lsrc, MRI *mri, LABEL *ldst) {
  ...
  default:
    ErrorExit(ERROR_UNSUPPORTED,
              "LabelToSurfaceRAS: unsupported coords (was %d)", lsrc->coords);
    break;
  }
  M_surface_from_RAS = MatrixInverse(M_surface_to_RAS, nullptr);
...
/*
  convert the label coords from tkreg (surface) RAS to voxels. Note that this
  assumes that the label coords are in tkreg space
*/
...
LABEL *LabelTransform(LABEL *lsrc, TRANSFORM *xform, MRI *mri, LABEL *ldst) {
  ...
  if (xform->type != LINEAR_RAS_TO_RAS)
    ErrorExit(ERROR_NOFILE,
              "LabelTransform: unsupported type %d. Must be RAS->RAS",
              xform->type);
  if (strstr(ldst->space, "scanner") == nullptr)
    ErrorExit(ERROR_NOFILE,
              "LabelTransform: label must be in scanner RAS not %s",
              ldst->space);
...
LABEL *LabelVoxelToSurfaceRAS(LABEL *lsrc, MRI *mri, LABEL *ldst) {
  ...
  for (i = 0; i < lsrc->n_points; i++) {
    if (lsrc->mris)
      MRISsurfaceRASToVoxel((MRIS *)lsrc->mris, mri, lsrc->lv[i].x,
                            lsrc->lv[i].y, lsrc->lv[i].z, &xs, &ys, &zs);
    else
      MRIvoxelToSurfaceRAS(mri, lsrc->lv[i].x, lsrc->lv[i].y, lsrc->lv[i].z,
                           &xs, &ys, &zs);
    ldst->lv[i].x = xs;
    ldst->lv[i].y = ys;
    ldst->lv[i].z = zs;
...
LABEL *LabelBaryFill(MRIS *mris, LABEL *srclabel, double delta) {
  ...
  mri = MRISlabel2Mask(mris, srclabel, nullptr);
  ...
  // printf("nPerTriangle = %d, nTriangles = %d,
  // %d\n",nPerTriangle,nTriangles,nPerTriangle*nTriangles);

  outlabel =
      LabelAlloc(nPerTriangle * nTriangles, srclabel->subject_name, nullptr);
...
LABEL *LabelSampleToSurface(MRI_SURFACE *mris, LABEL *area, MRI *mri_template,
  ...
  static MRI_SURFACE *mris_cached = nullptr;
  float               dx, dy, dz, x, y, z, dist, min_dist;
  int                 num_not_found, nchanged, num_brute_force;
  float               vx, vy, vz;
  double              max_spacing;
  static MRI *        mri = nullptr, *mri_cached = nullptr;
  LABEL *             area_dst;
  double              xw, yw, zw, xv, yv, zv;
  ...
  if (i <= 0) {
    printf("LabelSampleToSurface: no hole filling needed, returning (%d "
           "vertices)\n",
           area_dst->n_points);
  ...
  for (n = 0; n < area_dst->n_points; n++) {
...

    {
      ...
      if (bucket) {
        MHB *bin;
        for (bin = bucket->bins, i = 0; i < bucket->nused;
             i++, bin++) // find min dist vertex
        {
          vno                   = bin->fno;
          VERTEX const *const v = &mris->vertices[vno];
          if (vno == Gdiag_no)
  ...
  if (mri_template != mri_cached) {
    if (mri)
      MRIfree(&mri);
    mri        = MRIclone(mri_template, nullptr);
...
int LabelInit(LABEL *area, MRI *mri_template, MRI_SURFACE *mris, int coords) {
  ...
  x = y = z = -1;
  LabelRealloc(area, mris->nvertices); // allocate enough room in the label for
                                       // the whole surface
  area->mris = mris;
  ...
  if (area->vertex_label_ind == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "LabelInit: could not allocate %d-long vertex index array",
              mris->nvertices);
  for (n = 0; n < mris->nvertices; n++)
    area->vertex_label_ind[n] =
        -1; // means that this vertex is not in th elabel

  double vxl_spacing =
      (mri_template->xsize < mri_template->ysize ? mri_template->xsize
                                                 : mri_template->ysize);
  vxl_spacing =
      (vxl_spacing < mri_template->zsize ? vxl_spacing : mri_template->zsize);
  ...
  for (n = 0; n < area->n_points; n++) {
    ...
    // vertex not assigned to this label vertex - assign all surface vertices at
    // this location to the label note that this includes the closest vertex,
    // even if it does not sit in the same voxel
    ...
    case LABEL_COORDS_VOXEL:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "LabelInit: area->coords %d unsupported",
                area->coords);
      break;
    }
    lv->xv = nint(xv);
    ...
    if (area->mris)
      MRISsurfaceRASFromVoxel((MRIS *)area->mris, area->mri_template, xv, yv,
                              zv, &vx, &vy, &vz); // surfaceRASToVoxel BRF!!
    else
    ...
    float distance;
    min_vno = MHTfindClosestVertexNoXYZ((MHT *)area->mht, (MRIS *)area->mris,
                                        vx, vy, vz, &distance);

    lv->vno = min_vno;
    if (min_vno >= 0 &&
        area->vertex_label_ind[min_vno] < 0) // found one that isn't in label
    {
      ...
      VERTEX_TOPOLOGY const *const min_vt =
          &((MRI_SURFACE *)((MRIS *)area->mris))->vertices_topology[min_vno];
      for (i = 0; i < min_vt->vnum; i++) // find min dist vertex
      {
        ...
        MRISsurfaceRASToVoxel((MRIS *)area->mris, area->mri_template, x, y, z,
                              &vx, &vy, &vz);
...
int LabelAddVoxel(LABEL *area, int xv, int yv, int zv, int coords,
  ...
  if (area->mri_template) {
    ...
    case LABEL_COORDS_SCANNER_RAS:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "LabelAddVoxel: coords %d not supported yet",
                area->coords);
    }
    lv->x = vx;
    lv->y = vy;
  ...
  // must use surface coords for finding vertex
  if (area->mris)
    MRISsurfaceRASFromVoxel((MRIS *)area->mris, area->mri_template, xv, yv, zv,
                            &vx, &vy, &vz); // surfaceRASToVoxel BRF!!
  else
    MRIvoxelToSurfaceRAS(area->mri_template, xv, yv, zv, &vx, &vy, &vz);
  x = y = z = -1;
  ...
  float distance;
  min_vno = MHTfindClosestVertexNoXYZ((MHT *)area->mht, (MRIS *)area->mris, vx,
                                      vy, vz, &distance);
  ...
  if (min_vno >= 0) {
    ...
    } else //
    {
      ...
      LV *lv2 = &area->lv[n];
      if (FEQUAL(lv->x, lv2->x) && FEQUAL(lv->y, lv2->y) &&
          FEQUAL(lv->z, lv2->z)) {
        lv->deleted = 1;
      } else {
        lv->vno = -1;
  ...
  // now add other vertices that also map to this voxel
  VERTEX_TOPOLOGY const *min_vt =
      &((MRI_SURFACE *)(area->mris))->vertices_topology[min_vno];
  for (i = 0; i < min_vt->vnum; i++) // find min dist vertex
  {
    vno = min_vt->v[i];
    if (area->vertex_label_ind[vno] >= 0)
    ...
    MRISgetCoords(v, coords, &x, &y, &z);
    MRISsurfaceRASToVoxel((MRIS *)area->mris, area->mri_template, x, y, z, &vx,
                          &vy, &vz);
    if ((xv == nint(vx)) && (yv == nint(vy)) && (zv == nint(vz))) {
      if (pnvertices) {
        int n       = *pnvertices;
...
LABEL *LabelAddPoint(LABEL *label, LV *lv) {
  // alloc label if nec
  if (label == nullptr)
    label = LabelAlloc(100, nullptr, nullptr);
...
int LabelAddVertex(LABEL *area, int vno, int coords) {
  ...
  v  = &((MRI_SURFACE *)(area->mris))->vertices[vno];
  MRISgetCoords(v, coords, &x, &y, &z);
  MRISsurfaceRASToVoxel((MRIS *)area->mris, area->mri_template, v->x, v->y,
                        v->z, &xv, &yv, &zv);
...
static int update_vertex_indices(LABEL *area) {
  ...
  if (area->vertex_label_ind == nullptr || mris == nullptr)
    return (NO_ERROR);
...
static int labelGetSurfaceRasCoords(LABEL *area, LABEL_VERTEX *lv, float *px,
  ...
  default:
    ErrorExit(ERROR_UNSUPPORTED,
              "labelGetSurfaceRAScoords: coords %d not supported yet",
              area->coords);
    break;
  }
  return (NO_ERROR);
...
static int labelGetVoxelCoords(LABEL *area, LABEL_VERTEX *lv, float *px,
  ...
  case LABEL_COORDS_TKREG_RAS:
    if (area->mris)
      ...
      MRIvoxelToSurfaceRAS(area->mri_template, lv->x, lv->y, lv->z, &xv, &yv,
                           &zv);
    break;
  case LABEL_COORDS_SCANNER_RAS:
    MRIworldToVoxel(area->mri_template, lv->x, lv->y, lv->z, &xv, &yv, &zv);
...
LABEL *LabelFromSurface(MRI_SURFACE *mris, int which, double thresh) {
  ...
  for (npoints = vno = 0; vno < mris->nvertices; vno++) {
    ...
    case VERTEX_STATS:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "LabelFromSurface: unsupported which %d",
                which);
      val = 0;
      break;
    }
...
LABEL *LabelRemoveIslandsSurf(MRIS *surf, LABEL *lb) {
  ...
  n = 0;
  for (vtxno = 0; vtxno < surf->nvertices; vtxno++) {
    if (surf->vertices[vtxno].undefval != nMax + 1)
      continue;
