...
float **CAbuildTrainingData(VOXEL_LIST *vl_total, int target_label,
                            float *ca_classes, MRI **mri_smooth, MRI **mri_grad,
                            MRI **mri_laplacian, MRI *mri_dtrans,
  ...
  if (ca_inputs == nullptr)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %d SVM input vector",
              Progname, vl_total->nvox);

  ninputs = NINPUTS(which_inputs, wsize, nscales);
  if (which_inputs & CA_INPUT_DTRANS_GRAD)
    mri_dtrans_grad = MRIsobel(mri_dtrans, nullptr, nullptr);
  ...
  for (i = 0; i < vl_total->nvox; i++) {
    ...
    ca_inputs[i] =
        CAbuildInputsAtVoxel(vl_total, i, mri_smooth, mri_grad, mri_laplacian,
                             mri_dtrans, mri_dtrans_grad, mri_2nd_deriv_s,
                             wsize, nscales, nullptr, which_inputs);
    if (ca_inputs[i] == nullptr)
      ErrorExit(ERROR_NOMEMORY,
                "%s: could not allocate %dth CA input vector size %d", Progname,
                vl_total->nvox, ninputs);
  }
  if (which_inputs & CA_INPUT_DTRANS_GRAD)
    MRIfree(&mri_dtrans_grad);
  return (ca_inputs);
}
float *CAbuildInputsAtVoxel(VOXEL_LIST *vl, int i, MRI **mri_smooth,
                            MRI **mri_grad, MRI **mri_laplacian,
                            MRI *mri_dtrans, MRI *mri_dtrans_grad,
                            MRI **mri_2nd_deriv_s, int wsize, int nscales,
...
float **CAbuildInputs(VOXEL_LIST *vl_total, MRI *mri_intensity, MRI *mri_labels,
  ...
  for (i = 0; i < nscales; i++) {
    ...
    if (which_inputs & CA_INPUT_LAPLACIAN)
      mri_laplacian[i] = MRIlaplacian(mri_smooth[i], nullptr);
    if (which_inputs & CA_INPUT_D2I_S) {
      mri_2nd_deriv_s[i] =
          MRI2ndDirectionalDerivative(mri_smooth[i], nullptr, 0, -1, 0);
      if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
        MRIwrite(mri_2nd_deriv_s[i], "deriv.mgz");
    }
    MRIfree(&mri_kernel);
  }
  svm_inputs = CAbuildTrainingData(vl_total, 0, nullptr, mri_smooth, mri_grad,
                                   mri_laplacian, mri_dtrans, mri_2nd_deriv_s,
                                   wsize, nscales, which_inputs);
...
CA *CAalloc(int width, int height, int depth, MATRIX *m_vox2index, int type,
  ...
  if (ca->classifiers == nullptr)
    ErrorExit(ERROR_NOMEMORY, "CAalloc: could not allocate %d classifers",
              width);
  for (x = 0; x < width; x++) {
    ca->classifiers[x] = (CLASSIFIER ***)calloc(height, sizeof(CLASSIFIER **));
    if (ca->classifiers[x] == nullptr)
      ErrorExit(ERROR_NOMEMORY, "CAalloc: could not allocate X %d classifers",
                height);
    for (y = 0; y < height; y++) {
      ca->classifiers[x][y] =
          (CLASSIFIER **)calloc(depth, sizeof(CLASSIFIER *));
      if (ca->classifiers[x][y] == nullptr)
        ErrorExit(ERROR_NOMEMORY, "CAalloc: could not allocate Y %d classifers",
                  depth);
      for (z = 0; z < depth; z++) {
        ca->classifiers[x][y][z] =
            (CLASSIFIER *)calloc(ICO0_NVERTICES, sizeof(CLASSIFIER));
        if (ca->classifiers[x][y][z] == nullptr)
          ErrorExit(ERROR_NOMEMORY,
                    "CAalloc: could not allocate Z %d classifers",
                    ICO0_NVERTICES);
        for (o = 0; o < ICO0_NVERTICES; o++) {
          ca->classifiers[x][y][z][o].type = type;
          switch (type) {
          case CA_SVM:
            ca->classifiers[x][y][z][o].svm =
                SVMalloc(ninputs, c1_name, c2_name);
            break;
          case CA_GAUSSIAN:
            ca->classifiers[x][y][z][o].c1_means =
                (double *)calloc(ninputs, sizeof(double));
            ca->classifiers[x][y][z][o].c1_vars =
                (double *)calloc(ninputs, sizeof(double));
            ca->classifiers[x][y][z][o].c2_means =
                (double *)calloc(ninputs, sizeof(double));
            ca->classifiers[x][y][z][o].c2_vars =
                (double *)calloc(ninputs, sizeof(double));
            if (ca->classifiers[x][y][z][o].c1_means == nullptr ||
                ca->classifiers[x][y][z][o].c1_vars == nullptr ||
                ca->classifiers[x][y][z][o].c2_means == nullptr ||
                ca->classifiers[x][y][z][o].c2_vars == nullptr)
              ErrorExit(ERROR_NOMEMORY,
                        "CAalloc: could not alloc (%d, %d, %d, %d)", x, y, z,
                        o);
            break;
          }
        }
...
int CAtrain(CA *ca, VOXEL_LIST *vl, MRI *mri_norm, MRI *mri_aseg,
  ...
  if (classes == nullptr)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %d class vector",
              Progname, vl->nvox);
  for (i = 0; i < vl->nvox; i++)
    classes[i] = ((int)vl->vdst[i] == target_label) ? 1 : -1;
  inputs = CAbuildInputs(vl, mri_norm, mri_aseg, target_label, ca->which_inputs,
                         ca->wsize, ca->nscales, ca->sigmas);

  ca->c1_label = target_label;
  ca->c2_label = source_label;
  VLSTtransformCoords(vl, ca->m_vox2index, 0);
  mri_normals = caComputeSurfaceNormals(mri_aseg, nullptr, source_label);
  ...
  for (i = 0; i < vl->nvox; i++) {
    ...
    if (i == Gdiag_no)
      DiagBreak();
    if (xi < 0 || yi < 0 || zi < 0 || xi >= ca->width || yi >= ca->height ||
        zi >= ca->depth)
      continue;
    ...
    IcoFindNClosestVertices(ic0_vertices, ICO0_NVERTICES, nx, ny, nz, 3,
                            vertices);
    for (v = 0; v < 3; v++) {
      o = vertices[v];
      if (o < 0)
      ...
      switch (ca->type) {
      case CA_SVM:
        SVMtrain(cl->svm, inputs, classes, vl->nvox, ca->svm_C, ca->tol,
                 ca->max_iter);
        break;
      default:
...
int CAvoxelToIndex(CA *ca, double x, double y, double z, double *pxd,
                   double *pyd, double *pzd) {
...
static int clCompleteTraining(CA *ca, CLASSIFIER *cl) {
  ...
  if (cl->c1_ntraining > 0) {
    for (i = 0; i < ca->ninputs; i++) {
      ...
      cl->c1_vars[i] =
          cl->c1_vars[i] / cl->c1_ntraining - cl->c1_means[i] * cl->c1_means[i];
    }
  }
  ...
  if (cl->c2_ntraining > 0) {
    for (i = 0; i < ca->ninputs; i++) {
      ...
      cl->c2_vars[i] =
          cl->c2_vars[i] / cl->c2_ntraining - cl->c2_means[i] * cl->c2_means[i];
    }
  }
  return (NO_ERROR);
}
...
int CAwrite(CA *ca, char *fname) {
  ...
  fp = fopen(fname, "wb");
  if (fp == nullptr)
    ErrorExit(ERROR_NOFILE, "CAwrite(%s): could not open file for writing",
              fname);
...
MRI *CAclassifyBorder(CA *ca, MRI *mri_norm, MRI *mri_aseg, MRI *mri_output,
  ...
  mri_normals = caComputeSurfaceNormals(mri_aseg, nullptr, label);

  mri_border = MRImarkLabelBorderVoxels(mri_aseg, nullptr, label, 1, 1);
  while (border-- > 1) {
    mri_tmp = MRIdilate(mri_border, nullptr);
  ...
  ca_inputs = CAbuildInputs(vl, mri_norm, mri_aseg, label, ca->which_inputs,
                            ca->wsize, ca->nscales, ca->sigmas);
  for (i = 0; i < vl->nvox; i++) {
    out = CAclassifyVoxel(ca, mri_normals, vl->xi[i], vl->yi[i], vl->zi[i],
                          ca_inputs[i]);
...
float CAclassifyVoxel(CA *ca, MRI *mri_normals, int x, int y, int z,
  ...
  if (xi < 0 || xi >= ca->width || yi < 0 || yi >= ca->height || zi < 0 ||
      zi >= ca->depth)
    return (0.0);

  if (x == Gx && y == Gy && z == Gz &&
      ((Gdiag_no >= 0 && Gdiag_no == o) || Gdiag_no < 0))
    DiagBreak();
  cl = &ca->classifiers[xi][yi][zi][o];
  if (xi == Gx && yi == Gy && zi == Gz) {
    printf(
        "classifying voxel (%d, %d, %d), CL = (%d, %d, %d, %d), NT=(%d, %d)\n",
        Gx, Gy, Gz, xi, yi, zi, o, cl->c1_ntraining, cl->c2_ntraining);
    DiagBreak();
  }
...
static MATRIX *compute_ras_basis_vectors(MRI *mri_aseg_orig, MRI *mri_aseg_edit,
  ...
  *MATRIX_RELT(m_trans, 3, 4) = means[2];
  ...
  // width/height/depth
  mri_tmp =
      MRIlinearTransformInterp(mri_aseg_edit, nullptr, m_xform, SAMPLE_NEAREST);
  mri_aligned = MRIdilateLabel(mri_tmp, nullptr, label, pad);
  MRIlabelBoundingBox(mri_aligned, label, &box1);
  MRIfree(&mri_aligned);
  MRIfree(&mri_tmp);
  mri_tmp =
      MRIlinearTransformInterp(mri_aseg_orig, nullptr, m_xform, SAMPLE_NEAREST);
  mri_aligned = MRIdilateLabel(mri_tmp, nullptr, label, pad);
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
