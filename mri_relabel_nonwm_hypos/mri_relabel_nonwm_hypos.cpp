...
static void print_usage() {
  printf("USAGE: %s \n", Progname);
  printf("\n");
  printf("   --i inputseg  : input segmentation with non-wm-hypos labeled as "
         "80, 81, or 82\n");
  printf("   --o outputseg : segmentation with non-wm-hypos relabeled \n");
  printf("   --seg seg newseg : relabel hypos adjacent to seg as newseg (can "
         "use multiple --seg args)\n");
...
static void print_help() {
  ...
  printf("Eg, --seg 11 270 would replace non-wm hypos next to left caudate "
         "with \n");
  printf("the value 270. \nAlternatively, --seg-default will create the "
         "following list\n");
  printf("\n");
  nsegs = DefaultSegIds(segidlist, outsegidlist);
  PrintSegIds(nsegs, segidlist, outsegidlist);
  printf("\n");
  printf("The user will need to create a custom color to view/analyze the new "
         "segmentation\n");
  printf("The color table must be formated like "
         "$FREESURFER_HOME/FreeSurferColorLUT.txt, eg\n");
  printf("   270 left-caudate-nonwm-hypos   120 120 0 0\n");
  printf("This can be used with tkmedit, like tkmedit subject nu.mgz -seg "
         "newseg.mgz newcolortable\n");
...
int PrintSegIds(int nsegs, int *segidlist, int *outsegidlist) {
  ...
  for (n = 0; n < nsegs; n++) {
    ...
    printf("%4d %-25s will be relabeled as %4d\n", segid,
           ct->entries[segid]->name, outsegidlist[n]);
  }
