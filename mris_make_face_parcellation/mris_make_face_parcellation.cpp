...
static int write_annot_correlations(MRI_SURFACE *mris, MRI *mri_cmatrix,
                                    PARMS *parms, char *fname);
static double compute_distance_energy(MRI_SURFACE *mris, PARMS *parms,
                                      MRI *mri_cmatrix, double *penergy_num,
                                      double *penergy_den);
static double compute_variance_energy(MRI_SURFACE *mris, PARMS *parms,
                                      MRI *mri_cmatrix, MRI *mri_means,
                                      MRI *mri_vars, double *penergy_num,
                                      double *penergy_den);
static double compute_variance_energy_for_vertex_change(
    MRI_SURFACE *mris, PARMS *parms, MRI *mri_cmatrix, MRI *mri_means,
    MRI *mri_vars, double *penergy_num, double *penergy_den, int vno,
...
static MRI *compute_eta_matrix(MRI_SURFACE *mris, MRI *mri_cmatrix,
                               MRI *mri_ematrix, double thresh,
                               double max_dist);
static MRI *compute_similarity_matrix(MRI_SURFACE *mris, MRI *mri_cmatrix,
                                      MRI *mri_smatrix, double thresh,
                                      double max_dist);
static int allocate_stats(MRI_SURFACE *mris, PARMS *parms, int nparcels,
                          MRI *mri_cmatrix);
...
static int adjust_parcellation_boundaries(MRI_SURFACE *mris,
                                          MRI_SURFACE *mris_ico,
                                          MRI *mri_cmatrix, PARMS *parms);

static double compute_parcellation_energy_change(
    MRI_SURFACE *mris, PARMS *parms, MRI *mri_cmatrix, MRI *mri_means,
    MRI *mri_vars, int vno, int parcel_to_move_to, double *den_old,
    double *den_new, double *num_old, double *num_new);
static double compute_parcellation_energy(MRI_SURFACE *mris, MRI *mri_stats,
                                          PARMS *parms, double *penergy,
                                          MRI *mri_means, MRI *mri_vars,
                                          MRI *mri_cmatrix);
static double compute_parcellation_median_energy(MRI_SURFACE *mris,
                                                 MRI *mri_stats, PARMS *parms,
                                                 MRI *mri_means,
                                                 MRI *mri_cmatrix);
static int build_parcellation_border_permutation(MRI_SURFACE *mris,
                                                 int *vertex_permutation,
                                                 int *pnborder);
static int compute_parcellation_statistics(MRI_SURFACE *mris, MRI *mri_cmatrix,
                                           MRI *mri_stats, MRI *mri_means,
                                           MRI *mri_vars, PARMS *parms);

static int update_parcellation_statistics(MRI_SURFACE *mris, int vno,
                                          int old_parcel, int new_parcel,
                                          MRI *mri_cmatrix, MRI *mri_means,
                                          MRI *mri_vars, MRI *mri_stats,
                                          PARMS *parms);
const char *Progname;
...
int main(int argc, char *argv[]) {
  ...
  if (!mris_ico)
    ErrorExit(ERROR_NOFILE, "%s: could not read ico file %s", Progname,
              ico_fname);
                     ...
                     // parcellation
  {
    ...
    if (mri_cmatrix->width != mris->nvertices ||
        mri_cmatrix->nframes != mris->nvertices)
      ErrorExit(ERROR_BADFILE, "%s: cmatrix must be %d x 1 x 1 x %d", Progname,
                mris->nvertices, mris->nvertices);
  ...
  if (write_annot_fname) {
    if (MRISreadAnnotation(mris, write_annot_fname) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not load annotation from %s", Progname,
                write_annot_fname);
                      ...
                      // energy
  {
    ...
    if (MRISreadAnnotation(mris, evaluate_fname) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not load annotation %s", Progname,
                evaluate_fname);
    MRIScopyAnnotationsToMarkedIndex(mris);
    ...
    mri_means =
        MRIallocSequence(nparcels, 1, 1, MRI_FLOAT, mri_cmatrix->nframes);
    mri_vars =
        MRIallocSequence(nparcels, 1, 1, MRI_FLOAT, mri_cmatrix->nframes);
    parms.mri_stats = mri_stats;
    parms.mri_means = mri_means;
    parms.mri_vars = mri_vars;
    compute_parcellation_statistics(mris, mri_cmatrix, mri_stats, mri_means,
                                    mri_vars, &parms);
    penergy = energy = compute_parcellation_median_energy(
        mris, mri_stats, &parms, mri_means, mri_cmatrix);
    printf("energy = %2.3f\n", penergy);
    if (randomize) {
      MRI *mri_rand;
...
#define MAX_RAND 10
      ...
      for (total_rand_energy = total_rand_penergy = 0.0, n = 0; n < MAX_RAND;
           n++) {
        ...
        if (parms.energy_type == ENERGY_VARIANCE)
          compute_parcellation_statistics(mris, mri_rand, mri_stats, mri_means,
                                          mri_vars, &parms);
#if 1
        rand_energy = rand_penergy = compute_parcellation_median_energy(
            mris, mri_stats, &parms, mri_means, mri_cmatrix);
#else
        rand_energy =
            compute_parcellation_energy(mris, mri_stats, &parms, &rand_penergy,
                                        mri_means, mri_vars, mri_cmatrix);
#endif
        printf("rand_energy = %2.5f (%2.5f)\n", rand_energy, rand_penergy);

        total_rand_energy += rand_energy;
        total_rand_penergy += rand_penergy;
        //        MRIfree(&mri_rand) ;
      }
      total_rand_energy /= n;
      total_rand_penergy /= n;
      energy /= total_rand_energy;
      penergy /= total_rand_penergy;
    }
  ...
  if (do_vertices) {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      MHTfindClosestFaceGeneric(
          mht, mris, v->x, v->y, v->z, mris_ico->avg_vertex_dist / 10,
  ...
  if (mri_smatrix) {
    ...
    allocate_stats(mris, &parms,
                   do_vertices ? mris_ico->nvertices : mris_ico->nfaces,
                   mri_cmatrix);
    ...
    if (parms.l_gaussian > 0) {
      parms.gaussian_std =
          0.1 * sqrt(mris_ico->total_area / (double)mris_ico->nfaces);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    case 'E':
      ...
      log_fname = argv[3];
      printf(
          "evalauting energy of annotation file %s and logging results to %s\n",
          evaluate_fname, log_fname);
      nargs = 2;
      break;
    case '?':
    case 'U':
...
static int adjust_parcellation_boundaries(MRI_SURFACE *mris,
                                          ...
                                          MRI *mri_cmatrix, PARMS *parms) {
  MRI *mri_stats, *mri_means, *mri_vars;
  int *vertex_permutation, parcel, nborder, done, nchanged, iter, index, *nbrs,
      nparcels, min_parcel, vno, n, nframes;
  ...
  do {
    ...
    if (iter == 0) {
      ...
      printf("iter %d: nchanged = %d, nborder = %d, energy = %2.5f (%2.5f, "
             "%2.5f)\n",
    ...
    for (index = 0; index < nborder; index++) {
      ...
      if (min_parcel != parcel) {
        ...
        energy_change = compute_parcellation_energy_change(
            mris, parms, mri_cmatrix, mri_means, mri_vars, vno, min_parcel,
            &den_old, &den_new, &num_old, &num_new);
        ...
        if (parms->energy_type == ENERGY_VARIANCE)
          update_parcellation_statistics(mris, vno, v->marked, min_parcel,
                                         mri_cmatrix, mri_means, mri_vars,
                                         mri_stats, parms);
        v->marked = min_parcel;
        nchanged++;
        ...
        if (vno == Gdiag_no) {
          ...
          e = compute_parcellation_energy(mris, mri_stats, parms, &pe,
                                          mri_means, mri_vars, mri_cmatrix);
          if (e > last_energy)
            DiagBreak();
          DiagBreak();
    ...
    compute_parcellation_statistics(mris, mri_cmatrix, mri_stats, mri_means,
                                    mri_vars, parms);
    energy = compute_parcellation_energy(mris, mri_stats, parms, &parc_energy,
                                         mri_means, mri_vars, mri_cmatrix);
    ...
    menergy = compute_parcellation_median_energy(mris, mri_stats, parms,
                                                 mri_means, mri_cmatrix);
    printf(
        "iter %d: nchanged = %d, nborder = %d, energy = %2.5f (%2.5f, %2.5f)\n",
        iter, nchanged, nborder, energy, parc_energy, menergy);
  } while (!done);
  if (parms->write_iterations > 0 && ((iter % parms->write_iterations) == 0))
    write_snapshot(mris, parms, iter, mri_cmatrix);
...
static double compute_parcellation_energy(MRI_SURFACE *mris, MRI *mri_stats,
  ...
  if (parms->l_var > 0) {
    ...
    case ENERGY_DISTANCE:
      energy = compute_distance_energy(mris, parms, mri_cmatrix, &energy_num,
                                       &energy_den);
      parms->stats.energy_num = energy_num;
      parms->stats.energy_den = energy_den;
      break;
    case ENERGY_VARIANCE:
      energy = compute_variance_energy(mris, parms, mri_cmatrix, mri_means,
                                       mri_vars, &energy_num, &energy_den);
      break;
    default:
      ErrorExit(ERROR_BADPARM,
                "compute_parcellation_energy: unknown energy type %d", Progname,
                parms->energy_type);
    }
  } else
    energy = 0.0;
...
static double compute_distance_energy(MRI_SURFACE *mris, PARMS *parms,
  ...
  for (dist_within = dist_between_total = 0.0, vno = 0; vno < mris->nvertices;
       vno++) {
    v = &mris->vertices[vno];
    if (vno == Gdiag_no)
      DiagBreak();
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (frame = 0; frame < nframes; frame++) {
      ...
      if (parcel == 133 && frame == 322 && DIAG_VERBOSE_ON) {
        printf(
            "adding vno %d: %f to parcel %d, now %f +- %f (var=%f), dof=%d\n",
...
static int update_parcellation_statistics(MRI_SURFACE *mris, int vno,
  ...
  for (total_mean = total_var = 0.0, frame = 0; frame < nframes; frame++) {
    ...
    if (old_parcel == 133 && frame == 322 && DIAG_VERBOSE_ON)
      printf("removing vno %d: %f from parcel %d, now %f +- %f (var=%f), "
             "dofs=%d\n",
  ...
  for (total_mean = total_var = 0.0, frame = 0; frame < nframes; frame++) {
    ...
    if (new_parcel == 133 && frame == 322 && DIAG_VERBOSE_ON)
      printf("contributing vno %d: %f to parcel %d, now %f +- %f (var=%f), "
             "dofs=%d\n",
...
static double gaussian_energy(MRI_SURFACE *mris, PARMS *parms) {
  ...
  for (nparcels = 0, total_energy = 0.0, parcel = 0;
       parcel < parms->stats.nparcels; parcel++) {
    if (parcel == Gdiag_no)
      DiagBreak();
    if (parms->stats.num[parcel] > 0) {
...
static double compute_parcellation_median_energy(MRI_SURFACE *mris,
                                                 ...
                                                 MRI *mri_means,
                                                 MRI *mri_cmatrix) {
  double energy, var_within, var_between, mean, mean_nbr, var_between_total,
      val, var_within_total, var_between_nbr;
...
static int find_parcel_min_timecourses(MRI_SURFACE *mris, MRI *mri_cmatrix,
                                       PARMS *parms) {
  int nframes, parcel, vertices[MAX_PARCEL_VERTICES], nvertices, vno1, vno2, n1,
      n2, min_vno;
  double dist, min_dist, avg_dist,
      dmat[MAX_PARCEL_VERTICES][MAX_PARCEL_VERTICES];
  ...
  for (parcel = 0; parcel < parms->stats.nparcels; parcel++) {
    for (nvertices = vno1 = 0; vno1 < mris->nvertices; vno1++)
      if (mris->vertices[vno1].marked == parcel)
        vertices[nvertices++] =
            vno1; // make a list of all vertices in this parcel
    ...
    min_dist = mri_cmatrix->nframes * 1e10;
    min_vno = 0;
    for (n1 = 0; n1 < nvertices; n1++) {
  ...
  if (parms->read_dmat) {
    ...
    if (parms->energy_type != ENERGY_VARIANCE) {
      for (vno = 0; vno < mris->nvertices; vno++) {
        for (vno2 = vno + 1; vno2 < mris->nvertices; vno2++) {
          L1_dist =
              L1_distance(mri_cmatrix, mri_cmatrix, vno, 0, 0, vno2, 0, 0);
          MRIsetVoxVal(parms->mri_dmat, vno, vno2, 0, 0, L1_dist);
          MRIsetVoxVal(parms->mri_dmat, vno2, vno, 0, 0, L1_dist);
        }
      }
...
static double compute_parcellation_energy_change(MRI_SURFACE *mris,
  ...
  double dist_within_old, dist_between_old, dist_within_new, dist_between_new,
      dist, dist_between, markov_old, markov_new, energy_old, energy_new,
      gauss_old, gauss_new, area_old, area_new, border_old, border_new;
  int parcel, n, nparcels, n_nbrs, nframes;
  VERTEX *v;

  dist_between_old = dist_between_new = energy_new = energy_old =
      dist_within_old = dist_within_new = 0.0;
  nparcels = parms->stats.nparcels;
  ...
  if (parms->l_var > 0) {
    if (parms->energy_type == ENERGY_VARIANCE) {
      ...
      update_parcellation_statistics(mris, vno, parcel, parcel_to_move_to,
                                     mri_cmatrix, mri_means, mri_vars,
                                     parms->mri_stats, parms);
      energy_new = compute_variance_energy_for_vertex_change(
          mris, parms, mri_cmatrix, mri_means, mri_vars, &dist_within_new,
          &dist_between_new, vno, parcel, parcel_to_move_to);
      update_parcellation_statistics(mris, vno, parcel_to_move_to, parcel,
                                     mri_cmatrix, mri_means, mri_vars,
                                     parms->mri_stats, parms);
      energy_old = compute_variance_energy_for_vertex_change(
          mris, parms, mri_cmatrix, mri_means, mri_vars, &dist_within_new,
          &dist_between_new, vno, parcel_to_move_to, parcel);
#endif
...
#if 1
      energy_new =
          (parms->stats.energy_num - dist_within_old + dist_within_new) /
          (parms->l_eden *
           (parms->stats.energy_den - dist_between_old + dist_between_new));
      energy_old =
          parms->stats.energy_num / (parms->l_eden * parms->stats.energy_den);
#endif
    }
  } else
...
static MRI *compute_similarity_matrix(MRI_SURFACE *mris, MRI *mri_cmatrix,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < mris->nvertices; n++) {
      ...
      for (norm1 = norm2 = count = 0.0, vno3 = 0; vno3 < mris->nvertices;
           vno3++) {
#if 0
...
static int segment_similarity_matrix(MRI_SURFACE *mris, MRI_SURFACE *mris_ico,
                                     MRI *mri_smatrix, PARMS *parms) {
  int nchanged, min_border = 1, vno, vno2, n, max_parcel, iter, nparcels,
                parcel;
...
static double compute_variance_energy(MRI_SURFACE *mris, PARMS *parms,
                                      ...
                                      MRI *mri_vars, double *penergy_num,
                                      double *penergy_den) {
  double energy, var_within, var_between, var, var_between_total,
      var_within_total, val, mean;
  ...
  memset(parms->stats.energy, 0, sizeof(double) * nparcels);
  for (var_within_total = var_between_total = 0.0, parcel = 0;
       parcel < nparcels; parcel++) {
    if (parcel == Gdiag_no)
      DiagBreak();
    if (parms->stats.num[parcel] <= 0)
      continue;
    for (var_within = 0.0, frame = 0; frame < nframes; frame++) {
      ...
      if (parms->stats.num[parcel] > 1)
        var = (var - parms->stats.num[parcel] * mean * mean) /
              (parms->stats.num[parcel] - 1);
      else
        var = (var / (parms->stats.num[parcel]) - mean * mean);
      var_within += var;
    }
    parms->stats.var_within[parcel] =
        parms->stats.num[parcel] * (var_within / nframes);
    var_within_total += parms->stats.num[parcel] * (var_within / nframes);

    for (var_between = 0.0, n_nbrs = n = 0;
         n < parms->mris_ico->vertices_topology[parcel].vnum; n++) {
      nbr_parcel = parms->mris_ico->vertices_topology[parcel].v[n];
      for (var = 0.0, frame = 0; frame < nframes; frame++) {
        mean = MRIFseq_vox(mri_means, parcel, 0, 0, frame) /
               parms->stats.num[parcel];
        val = MRIFseq_vox(mri_means, nbr_parcel, 0, 0, frame) /
              parms->stats.num[nbr_parcel];
        var += SQR(mean - val);
      }
      var /= nframes;
      var_between += var;
    ...
    var_between_total +=
        (var_between / parms->mris_ico->vertices_topology[parcel].vnum);
  }
...
static double area_energy(MRI_SURFACE *mris, PARMS *parms) {
  ...
  for (parcel = 0, energy = 0.0; parcel < parms->stats.nparcels; parcel++)
    energy +=
        SQR((parms->stats.num[parcel] - parms->avg_area) / parms->avg_area);
...
static double compute_variance_energy_for_vertex_change(
  ...
  for (var_within_total = var_between_total = 0.0, parcel = 0;
    ...
    var_between_total +=
        (var_between / parms->mris_ico->vertices_topology[parcel].vnum);
    parms->stats.var_between[parcel] =
        (var_between / parms->mris_ico->vertices_topology[parcel].vnum);
  }
  var_within_total /= mris->nvertices;
  if (FZERO(var_between_total))
...
static int write_annot_correlations(MRI_SURFACE *mris, MRI *mri_cmatrix,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (frame = 0; frame < nframes; frame++) {
      ...
      MRIsetVoxVal(mri_corr, source_parcel, target_parcel, 0, 0,
                   total_corr + corr);
