...
int main(int argc, char *argv[]) {
  ...
  if (nsmooth > 0) {
    ...
    mritmp = MRISsmoothMRI(surf, mri, nsmooth, nullptr, nullptr);
    MRIfree(&mri);
    mri = mritmp;
  }
  ...
  if (EccenOut) {
    mritmp = MRIcopyMRIS(nullptr, surf, 0, "val");
    MRIwrite(mritmp, EccenOut);
    MRIfree(&mritmp);
  }
  if (PolarOut) {
    mritmp = MRIcopyMRIS(nullptr, surf, 0, "valbak");
    MRIwrite(mritmp, PolarOut);
    MRIfree(&mritmp);
  }

  if (usenew)
    RETcompute_fieldsign2(surf);
  else
    RETcompute_fieldsign(surf);

  mritmp = MRIcopyMRIS(nullptr, surf, 0, "fieldsign");
  if (ReverseSign) {
  ...
  if (CMFFile) {
    mritmp = MRIcopyMRIS(nullptr, surf, 0, "stat");
    MRIwrite(mritmp, CMFFile);
    MRIfree(&mritmp);
  }

  if (NNbrFile) {
    mritmp = MRIcopyMRIS(nullptr, surf, 0, "K");
    MRIwrite(mritmp, NNbrFile);
    MRIfree(&mritmp);
  }
  if (RVarFile) {
    mritmp = MRIcopyMRIS(nullptr, surf, 0, "H");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--occip"))
      PatchFile = "occip.patch.flat";
    else if (!strcasecmp(option, "--rev"))
      ReverseSign = 1;
    else if (!strcasecmp(option, "--sfa-true"))
      SFATrue = 1;
...
static void print_usage() {
  ...
  printf("   --sfa sfadir :  \n");
  printf("   --sfa-true          : use true real and imag (only affects when "
         "smoothing)\n");
  printf("\n");
...
static void check_options() {
  ...
  if (PatchFile == nullptr && !UseSphere) {
...
MRI *SFA2MRI(MRI *eccen, MRI *polar, int SFATrue) {
  ...
  mri =
      MRIallocSequence(eccen->width, eccen->height, eccen->depth, MRI_FLOAT, 4);
...
int MRISextendedHopNeighbors(MRIS *surf, int TargVtxNo, int CurVtxNo, int nhops,
  ...
  for (n = 0; n < nNNbrs; n++) {
    NbrVtxNo = surf->vertices_topology[CurVtxNo].v[n];
    MRISextendedHopNeighbors(surf, TargVtxNo, NbrVtxNo, nhops, XNbrVtxNo,
                             nXNbrs);
  }
    ...
    for (n = 0; n < nlist; n++) {
      ...
      if (false) {
        fprintf(fp, "%6d %6d  %g %g  %g %g %g %g\n", n, k, deccen, dpolar,
                uctx[k], uctx[knbr], vctx[k], uctx[knbr]);
      }
    }
