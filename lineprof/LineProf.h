...
public:
  ...
  LineProf(const std::vector<std::vector<double>> &points2d,
           const std::vector<int> &segment0, const std::vector<int> &segment1,
           const std::vector<int> &segmentL, const std::vector<int> &segmentR);
  ...
  //! Laplace solver
  void solveLaplace(int paddingL, int paddingR, double resolution,
                    int convergenceCriterion);

  //! Compute Profiles as array of lines (each line an array of 2d coordinates)
  std::vector<std::vector<std::vector<double>>> ComputeProfiles(double offset,
                                                                double spacing);

private:
  //! Samples points along midline
  std::vector<std::vector<double>> samplePointsMidline(double offset,
                                                       double spacing);
