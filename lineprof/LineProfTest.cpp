...
void checkProfiles(
  ...
  if (profiles.size() != 40) {
    std::cerr << "ERROR: expecting 40 profiles, but got " << profiles.size()
              << " !" << std::endl;
    exit(1);
  }
  ...
  for (unsigned int p = 0; p < profiles.size(); p++) {
    if (profiles[p].size() != 21) {
      std::cerr << "ERROR: expecting 21 points in profile " << p << ", but got "
                << profiles[p].size() << " !" << std::endl;
      exit(1);
    }
    xp = profiles[p][0][0];
    // std::cout << " p " << p << " x " << xp << std::endl;
    if (fabs(xp - (p * xdelta + xstart)) > eps) {
      std::cerr << "ERROR: Profile " << p << "  x-coordinate difference!"
                << std::endl;
      std::cerr << setprecision(9) << p * xdelta + xstart << " != " << xp
                << std::endl;
      exit(1);
    }
    ...
    for (unsigned int i = 0; i < profiles[p].size(); i++) {
      if (profiles[p][i].size() != 2) {
        std::cerr << "ERROR: expecting 2 coords in profile " << p << " point "
                  << i << " , but got " << profiles[p][i].size() << " !"
                  << std::endl;
        exit(1);
      }
      if (fabs(profiles[p][i][0] - xp) > eps) {
        std::cerr << "ERROR: Profile " << p << "  point " << i
                  << " x-coordinate difference!" << std::endl;
        std::cerr << setprecision(9) << xp << " != " << profiles[p][i][0]
                  << std::endl;
        exit(1);
      }
      ...
      if (fabs(profiles[p][i][1] - (i * ydelta + ystart)) > eps) {
        std::cerr << "ERROR: Profile " << p << "  point " << i
                  << " y-coordinate difference!" << std::endl;
        std::cerr << setprecision(9) << i * ydelta + ystart
                  << " != " << profiles[p][i][1] << std::endl;
        exit(1);
      }
    }
  }
  std::cout << std::endl
            << "Profile check successfull ! " << std::endl
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  // Parameter for the convergence of laplace solver (can probably be keept at
  // 8)
  int convergence = 8;
