...
void LineProf::solveLaplace(int paddingL, int paddingR, double dresolution,
  ...
  solver.SetInputMask(pre.GetOutputLabel(), pre.GetMaskInsideValue(),
...
LineProf::ComputeProfiles(double offset, double dspacing) {
  if (_tracer == NULL) {
    std::cerr << "Line Profile Error: call solveLaplace before trying to "
                 "compute profiles!"
              << std::endl;
  ...
  //  _points2d[_segment0[0]][1]); const std::vector < std::vector < double > >
  //  & referenceLine = vml[0];
  //
  ...
  //
  //   double ddist(0);
  //   for( int count(offset), maxVal(referenceLine.size()-offset); count <
  //   maxVal; ++count )
  //   {
  ...
  std::vector<std::vector<double>> points =
      samplePointsMidline(offset, dspacing);
...
LineProf::samplePointsMidline(double offset, double dspacing) {
  // std::cout << "LineProf::samplePointsMidline(offset " << offset << " ,
...
std::vector<std::vector<std::vector<double>>>
LineProf::ComputeIsolines(const std::vector<double> &vec, double x0,
                          double y0) {
  // std::cout << "LineProf::computeIsolines (vec, x0 = " << x0 << " , y0 = " <<
  // y0 << " )" <<std::endl;
  if (_tracer == NULL) {
    std::cerr << "Line Profile Error: call solveLaplace before trying to "
                 "compute isolines!"
...
LineProf::ConvertLine(const Tracer::LineType &line) {
  ...
  for (Tracer::LineType::const_iterator cit = line.begin(); cit != line.end();
       ++cit) {
