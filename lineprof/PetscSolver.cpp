...
int PetscSolver::Update(double convergence) {
  ...
  CHKERRQ(ierr);
  ierr = MatSetSizes(A, PETSC_DECIDE, PETSC_DECIDE, pixelCount, pixelCount);
  CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr = VecSetSizes(rhs, PETSC_DECIDE, pixelCount);
  CHKERRQ(ierr);
  ...
  ierr = KSPSetTolerances(ksp, tol / pixelCount, PETSC_DEFAULT, PETSC_DEFAULT,
                          pixelCount / 3);
  CHKERRQ(ierr);
...
int PetscSolver::SetupIndexCorrespondence() {
    ...
    if (mask_cni.GetPixel(offset_zero) == labelInside) {
      ...
      for (off_cit = vecOffsets.begin(); off_cit != vecOffsets.end();
        ...
        if (mask_cni.GetPixel(*off_cit) == labelInside) {
          ...
          map_rhs[row] += data_cni.GetPixel(*off_cit);
      } // next off_cit
