...
*/
void Tracer::SetInputData(ImagePointer image) {
...
void Tracer::SetInputMask(MaskImagePointer inputMask) {
...
Tracer::LineType Tracer::ComputeMidline() const {
...
Tracer::LineType Tracer::ComputeIsoline(double dval) const {
  ...
  for (LineType::const_iterator cit = line.begin(); cit != line.end(); ++cit) {
...
*/
Tracer::LineType Tracer::ComputeProfile(double x, double y) const {
  PointType seed;
  seed[0] = x;
  seed[1] = y;
...
void Tracer::ValueAt(const PointType &pt, double &dval) const {
...
void Tracer::DerivativeAt(const PointType &pt, VectorType &derivative) const {
  InterpolatorType::PointType cpt;
  std::copy(pt.Begin(), pt.End(), cpt.Begin());
...
Tracer::VectorType Tracer::GetTangent(const PointType &pt) const {
  VectorType gradient;
...
Tracer::FrameType Tracer::GetLocalFrame(const PointType &pt) const {
  VectorType gradient;
  this->DerivativeAt(pt, gradient);
  gradient.Normalize();
  gradient *= stepSize;
...
bool Tracer::StepGradient(const PointType &pt, PointType &returnedPoint,
                          bool negative) const {
  ...
  vtkIdType id =
      locator->FindClosestPointWithinRadius(1.41 * stepSize, buf, dist2);
  if (id >= 0) {
    for (unsigned int ui = 0; ui < Dimension; ++ui)
  ...
  bool inside = mask->IsInsideInObjectSpace(returnedPoint);
...
bool Tracer::StepTangent(const PointType &pt, PointType &returnedPoint,
  ...
  if (!mask->IsInsideInObjectSpace(returnedPoint))
    return false;
  ...
  this->ValueAt(ptBuf, dbuf);
  if (std::abs(dbuf - dval) < dif)
    returnedPoint = ptBuf;

  return mask->IsInsideInObjectSpace(returnedPoint);
}
