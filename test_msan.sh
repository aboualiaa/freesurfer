...
FSTEST_TESTDATA_DIR="${FSTEST_CWD}/msan/testdata"
...
function compare_vol() {
  diffcmd=$(find_path $FSTEST_CWD mri_diff/msan/mri_diff)
...
function compare_surf() {
  diffcmd=$(find_path $FSTEST_CWD mris_diff/msan/mris_diff)
  run_comparison $diffcmd $@ --debug
}

# runs lta_diff on an output and reference transform - all extra opts are passed to lta_diff
function compare_lta() {
  diffcmd=$(find_path $FSTEST_CWD mri_robust_register/msan/lta_diff)
