...
int main(int argc, char *argv[]) {
  ...
  printf("    original surface average area per vertex: %2.2f\n subdivision "
         "surface average area per vertex: %2.2f\n",
...
int mris_mesh_subdivide__VTK(MRI_SURFACE *mris, int iter) {
  ...
  std::cout << setw(7) << inputMesh->GetNumberOfPolys() << " triangle faces."
            << std::endl;

  std::cout << " -> subdivision surface: ";
  std::cout << setw(7) << outputMesh->GetNumberOfPoints() << " vertices, ";
  std::cout << setw(7) << outputMesh->GetNumberOfPolys() << " triangle faces."
            << std::endl;
...
int mris_mesh_subdivide__convert_VTK_mris(vtkPolyData *mesh,
  ...
  // for performance reasons, defer doing work that would change if another
  // attached to the same vertex

  for (fno = 0; fno < mris_dst->nfaces; fno++) {
    mesh->GetCellPoints(fno, pointIdList);
    mrisAttachFaceToVertices(mris_dst, fno, pointIdList->GetId(0),
                             pointIdList->GetId(1), pointIdList->GetId(2));
  }

  setFaceAttachmentDeferred(mris_dst, false);
  // finish all the work needed to have the topology of the surface fully
  // defined
...
static void print_usage() {
  ...
  printf("                           'butterfly' (default), 'loop', or "
         "'linear'\n");
