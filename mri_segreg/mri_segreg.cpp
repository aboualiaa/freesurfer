...
double *GetSurfCosts(MRI *mov, MRI *notused, MATRIX *R0, MATRIX *R, double *p,
                     int dof, double *costs);
int MinPowell(MRI *mov, MRI *notused, MATRIX *R, double *params, int dof,
              double ftol, double linmintol, int nmaxiters, char *costfile,
              double *costs, int *niters);
float compute_powell_cost(float *p);
double RelativeSurfCost(MRI *mov, MATRIX *R0);

char *costfile_powell = nullptr;
...
MRI *lhsegmask = nullptr, *rhsegmask = nullptr;
MRI *lhlabel, *rhlabel;
int UseMask = 0;
char *cmdline2;
struct utsname uts;
...
int PreOptDim = 2;
char *PreOptFile = nullptr;
int PreOptOnly = 0;

char *surfcostbase = nullptr, *lhcostfile = nullptr, *rhcostfile = nullptr;
char *surfconbase = nullptr, *lhconfile = nullptr, *rhconfile = nullptr;
char *surfcost0base = nullptr, *lhcost0file = nullptr, *rhcost0file = nullptr;
char *surfcostdiffbase = nullptr;

char *TargConLHFile = nullptr, *TargConRHFile = nullptr;
MRI *TargConLH = nullptr, *TargConRH = nullptr;

int UseLH = 1;
int UseRH = 1;

MATRIX *MrotPre = nullptr, *MtransPre = nullptr, *MscalePre = nullptr,
       *MshearPre = nullptr;
double TransRandMax = 0;
double RotRandMax = 0;
char *MinCostFile = nullptr;
char *InitCostFile = nullptr;

int DoRMSDiff = 1; // this is fast, so ok to do automatically
char *RMSDiffFile = nullptr;
...
int main(int argc, char **argv) {
  ...
  int nth, n, vno;
  MATRIX *R = nullptr, *R00 = nullptr, *Rdiff = nullptr;
  Timer mytimer;
  double secCostTime;
  FILE *fp, *fpMinCost, *fpInitCost, *fpRMSDiff, *fpPreOpt = nullptr,
                                                 *fpRelCost, *fpParam;
  double rmsDiffSum, rmsDiffSum2, rmsDiffMean = 0, rmsDiffMax = 0, d,
                                  rmsDiffStd = 0;
  double rcost0, rcost;
  VERTEX *v;
  int nsubsampsave = 0;
  ...
  make_cmd_version_string(
      argc, argv, "$Id: mri_segreg.c,v 1.113 2016/05/10 03:23:20 greve Exp $",
      "$Name:  $", cmdline);
  ...
  if (regheader) {
    ...
    R0 = MRItkRegMtx(anat, mov, nullptr);
  }

  R00 = MatrixCopy(R0, nullptr);
  ...
  if (DoMidFrame)
    frame = nint(mov->nframes / 2);

  if (mov->nframes > 1) {
    printf("Extracting frame %d\n", frame);
    mritmp = fMRIframe(mov, frame, nullptr);
    MRIfree(&mov);
    mov = mritmp;
  }
  ...
  if (DoSmooth) {
    ...
    MRImaskedGaussianSmooth(mov, nullptr, gstd, mov);
  }
  ...
  if (UseLH && UseCortexLabel) {
    ...
    if (label == nullptr) {
      ...
      lhCortexLabel = nullptr;
    } else {
      printf("Using lh.cortex.label\n");
      lhCortexLabel = MRISlabel2Mask(lhwm, label, nullptr);
      if (lhCortexLabel == nullptr)
        exit(1);
      LabelFree(&label);
    }
  }
  if (UseRH && UseCortexLabel) {
    ...
    if (label == nullptr) {
      ...
      rhCortexLabel = nullptr;
    } else {
      printf("Using rh.cortex.label\n");
      rhCortexLabel = MRISlabel2Mask(rhwm, label, nullptr);
      if (rhCortexLabel == nullptr)
        exit(1);
      LabelFree(&label);
    }
  }
  ...
  if (AddNoise) {
    ...
    noise = MRIrandn(mov->width, mov->height, mov->depth, mov->nframes, 0,
                     NoiseStd, nullptr);
    mov = MRIadd(mov, noise, mov);
  }

  R = MatrixCopy(R0, nullptr);
  ...
  if (surfcost0base) {
    ...
    nsubsamp = 1;
    GetSurfCosts(mov, nullptr, R0, R, p, dof, costs);
    nsubsamp = nsubsampsave;
    if (UseLH)
      lhcost0 = MRIcopy(lhcost, nullptr);
    if (UseRH)
      rhcost0 = MRIcopy(rhcost, nullptr);
    if (InitSurfCostOnly) {
      if (lhcost0file)
        MRIwrite(lhcost0, lhcost0file);
      if (rhcost0file)
        MRIwrite(rhcost0, rhcost0file);
  ...
  if (InitCostFile) {
    ...
    fprintf(fpInitCost, "%lf %lf %lf %lf \n", costs[7], costs[1], costs[4],
            costs[6]);
    fclose(fpInitCost);
  }

  if (costs[6] < 0 && PenaltySign < 0)
    PrintT1Warning = 1;
  if (PrintT1Warning) {
    fprintf(fp, "\n\n");
    fprintf(
  ...
  if (PrintT2Warning) {
    ...
    fprintf(
        fp,
        "WARNING: initial G-W contrast is positive, but expecting negative.\n");
  ...
  if (BruteForce) {
    ...
    {
      PreOptMinTrans = PreOptMin;
      PreOptMaxTrans = PreOptMax;
      PreOptDeltaTrans = PreOptDelta;
    }
    if (UseLH && mask_label)
    ...
    for (tx = PreOptMinTrans; tx <= PreOptMaxTrans; tx += PreOptDeltaTrans) {
      for (ty = PreOptMinTrans; ty <= PreOptMaxTrans; ty += PreOptDeltaTrans) {
        for (tz = PreOptMinTrans; tz <= PreOptMaxTrans;
             tz += PreOptDeltaTrans) {
              ...
              for (az = PreOptMin; az <= PreOptMax; az += PreOptDelta) {
                ...
                if (costs[7] < mincost) {
                  ...
                  for (n = 0; n < 6; n++)
                    pmin[n] = p[n];
                  secCostTime = mytimer.seconds();
                  printf("%6d %8.4lf %8.4lf %8.4lf %8.4lf %8.4lf %8.4lf    "
                         "%8.4lf %8.4lf %4.1f\n",
                         nth, tx, ty, tz, ax, ay, az, costs[7], mincost,
                         secCostTime / 60);
                  fprintf(fp,
                          "%6d %8.4lf %8.4lf %8.4lf %8.4lf %8.4lf %8.4lf    "
                          "%8.4lf %8.4lf %4.1f\n",
                          nth, tx, ty, tz, ax, ay, az, costs[7], mincost,
                          secCostTime / 60);
                  fflush(stdout);
                ...
                if (PreOptFile)
                  fprintf(
                      fpPreOpt,
                      "%8.8lf %8.8lf %8.8lf %8.8lf %8.8lf %8.8lf    %8.8lf\n",
  ...
  MinPowell(mov, nullptr, R, p, dof, TolPowell, LinMinTolPowell,
  ...
  if (surfcostdiffbase) {
    if (UseLH) {
      ...
      mritmp = MRIsubtract(lhcost, lhcost0, nullptr);
    ...
    if (UseRH) {
      ...
      mritmp = MRIsubtract(rhcost, rhcost0, nullptr);
      MRIwrite(mritmp, tmpstr);
      MRIfree(&mritmp);
    }
  }
  ...
  if (ParamFile) {
    ...
    fprintf(fpParam, "%lf %lf %lf %lf %lf %lf  ", p[0], p[1], p[2], p[3], p[4],
            p[5]);
    if (dof > 6) {
      fprintf(fpParam, "%lf %lf %lf ", p[6], p[7], p[8]);
      if (dof > 9)
        fprintf(fpParam, "%lf %lf %lf ", p[9], p[10], p[11]);
      else
        fprintf(fpParam, "%lf %lf %lf ", 0.0, 0.0, 0.0);
    } else
      fprintf(fpParam, "%lf %lf %lf %lf %lf %lf ", 1.0, 1.0, 1.0, 0.0, 0.0,
              0.0);
    fprintf(fpParam, "\n");
    fclose(fpParam);
  }
  ...
  fprintf(fp, "nhits=%7d wmmn=%10.4lf wmstd=%8.4lf ", (int)costs[0], costs[1],
          costs[2]);
  fprintf(fp, "constd=%10.4lf ctxmn=%10.4lf ctxstd=%8.4lf ", costs[3], costs[4],
          costs[5]); // constd ctxmn ctxstd
  fprintf(fp, "conmn = %8.4lf costmn = %8.4lf ", costs[6],
          costs[7]); // conmn costmn
  fprintf(fp, "\n");
  fprintf(fp,
          "#C# %7.3lf %7.3lf %7.3lf %6.3lf %6.3lf %6.3lf %lf %lf %lf %lf %lf "
          "%lf %lf %lf %5d %d\n",
          p[0], p[1], p[2], p[3], p[4], p[5], rmsDiffMean, rmsDiffMax, costs[7],
          costs[1], costs[4], costs[6], PenaltyCenter, PenaltySlope, nsubsamp,
          UseMask);
  ...
  if (outregfile) {
    ...
    if (type == TRANSFORM_ARRAY_TYPE) {
      ...
      regio_write_register(outregfile, subject, mov->xsize, mov->zsize,
                           intensity, R, FLT2INT_ROUND);
  }

  if (outfile) {
    // Allocate the output
    out =
        MRIallocSequence(anat->width, anat->height, anat->depth, MRI_FLOAT, 1);
  ...
  Rdiff = MatrixSubtract(R00, R, nullptr);
  ...
  if (DoRMSDiff) {
    ...
    if (rhwm) {
      ...
      if (lhwm)
        n = (lhwm->nvertices + rhwm->nvertices);
      else
        n = rhwm->nvertices;
      rmsDiffMean = rmsDiffSum / n;
      rmsDiffStd =
          sqrt((rmsDiffSum2 - n * rmsDiffMean * rmsDiffMean) / (n - 1));
    }
    printf("Surface-RMS-Diff-mm %lf %lf %lf\n", rmsDiffMean, rmsDiffStd,
           rmsDiffMax);
    fprintf(fp, "Surface-RMS-Diff-mm %lf %lf %lf\n", rmsDiffMean, rmsDiffStd,
            rmsDiffMax);
    if (RMSDiffFile) {
      fpRMSDiff = fopen(RMSDiffFile, "w");
      // rmsDiffMean rmsDiffStd rmsDiffMax MinCost Nevals Tx Ty Tz Rx Ry Rz
      // WMMean CtxMean PctContrast
      fprintf(fpRMSDiff,
              "%12.9lf %12.9lf %12.9lf   %12.9lf %4d  %7.3lf %7.3lf %7.3lf "
              "%6.3lf %6.3lf %6.3lf  %lf %lf %lf \n",
              rmsDiffMean, rmsDiffStd, rmsDiffMax, costs[7], nCostEvaluations,
              p[0], p[1], p[2], p[3], p[4], p[5], costs[1], costs[4], costs[6]);
  ...
  if (PrintT2Warning) {
    ...
    fprintf(
        fp,
        "WARNING: initial G-W contrast was positive, but expected negative.\n");
    fprintf(fp, "If the mov data has a T2 contrast, re-run with --T2\n");
    fprintf(fp, "\n\n");
    printf("\n\n");
    printf(
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      DoAbs = 1;
    else if (!strcasecmp(option, "--no-abs"))
      DoAbs = 0;
    else if (!strcasecmp(option, "--no-cortex-label"))
      UseCortexLabel = 0;
      ...
      DoMidFrame = 1;
    else if (!strcasecmp(option, "--no-mask"))
      UseMask = 0;
      ...
      mask_label = LabelRead(nullptr, pargv[0]);
      ...
      UseLH = 1;
      UseRH = 0;
    } else if (!strcasecmp(option, "--rh-only")) {
      UseLH = 0;
      UseRH = 1;
      ...
      PreOptFile = pargv[0];
      nargsused = 1;
    } else if (istringnmatch(option, "--preopt-only", 0))
      PreOptOnly = 1;
      ...
      if (nargc < 1)
        argnerr(option, 1);
      regfile = pargv[0];
      err = regio_read_register(regfile, &subject, &ipr, &bpr, &intensity, &R0,
                                &float2int);
      if (err)
        exit(1);
      nargsused = 1;
      ...
      MtransPre = MatrixIdentity(4, nullptr);
      ...
      MscalePre = MatrixIdentity(4, nullptr);
      ...
      MshearPre = MatrixIdentity(4, nullptr);
      ...
      fp = fopen(pargv[0], "w");
      for (v = -30; v < +30; v++) {
        c = VertexCost(10 * (1 + v / 200) / (1 - v / 200), 10, PenaltySlope,
                       PenaltyCenter, PenaltySign, &d);
        fprintf(fp, "%lf %lf\n", d, c);
      }
      fclose(fp);
      exit(0);
...
static void print_usage() {
  ...
  printf("\n");
  printf("  --brute_trans min max delta : brute force translation in all "
         "directions\n");
  ...
  printf("  --subsamp-brute nsub : only sample every nsub vertices during "
         "brute-force search (%d)\n",
         nsubsampbrute);
  printf("\n");
  printf("  --preopt-file file : save preopt results in file\n");
  printf("  --preopt-dim dim : 0-5 (def 2) "
         "(0=TrLR,1=TrSI,2=TrAP,3=RotLR,4=RotSI,5=RotAP)\n");
  ...
  printf("  --gm-gt-wm slope : set cost slope, spec gray matter brighter than "
         "WM\n");
  printf("  --wm-gt-gm slope : set cost slope, spec WM brighter than gray "
         "matter\n");
  ...
  printf("\n");
  printf("  --label <label file> : only use the portion of the surface in the "
         "label (only for --?h-only\n");
  printf("\n");
  ...
  printf("\n");
  printf("  --gm-proj-frac frac : fraction of cortical thickness (default, "
         "0.5)\n");
  ...
  printf("\n");
  printf("  --interp interptype : interpolation trilinear or nearest (def is "
         "trilin)\n");
  printf("  --profile : print out info about exec time\n");
  printf("\n");
  printf("  --noise stddev : add noise with stddev to input for testing "
         "sensitivity\n");
  ...
  printf(
      "  --init-surf-cost basename0 : saves init cost as basename0.?h.mgh\n");
  printf("  --surf-cost-diff diffbase : saves final-init cost as "
         "diffbase.?h.mgh\n");
...
static void check_options() {
  ...
  if (regfile == nullptr && !regheader) {
    printf("ERROR: need --reg or --regheader.\n");
    exit(1);
  }
  if (outregfile == nullptr) {
    printf("ERROR: need --out-reg.\n");
    exit(1);
  }
  if (SegRegCostFile == nullptr) {
...
static void dump_options(FILE *fp) {
  ...
  if (DoGMProjFrac)
    fprintf(fp, "GMProjFrac %lf\n", GMProjFrac);
  if (DoGMProjAbs)
    fprintf(fp, "GMProjAbs %lf\n", GMProjAbs);
  if (DoWMProjFrac)
    fprintf(fp, "WMProjFrac %lf\n", WMProjFrac);
  if (DoWMProjAbs)
    fprintf(fp, "WMProjAbs %lf\n", WMProjAbs);
...
float compute_powell_cost(float *p) {
  ...
  GetSurfCosts(mov, nullptr, R0, R, pp, dof, costs);
  ...
  if (costfile_powell != nullptr) {
    // write costs to file
    if (nCostEvaluations == 0)
      fp = fopen(costfile_powell, "w");
    else
    ...
    if (dof > 6)
      fprintf(fp, "sc: %4.3lf %4.3lf %4.3lf ", pp[6], pp[7], pp[8]);
    if (dof > 9)
      fprintf(fp, "sh: %6.3lf %6.3lf %6.3lf ", pp[9], pp[10], pp[11]);
    ...
    // fprintf(fp,"%8.4lf %12.10lf   %12.10lf ",costs[6],costs[7],copt); // t,
    // cost=1/t fprintf(fp,"\n");
    fclose(fp);
  }
  ...
  if (newopt) {
    ...
    if (curregfile)
      regio_write_register(curregfile, subject, mov->xsize, mov->zsize,
                           intensity, R, FLT2INT_ROUND);
  }
...
int MRISbbrSurfs(char *subject) {
  ...
  if (UseLH) {
    ...
    for (n = 0; n < lhwm->nvertices; n++) {
      if (DoWMProjAbs)
        ProjNormDist(&fx, &fy, &fz, lhwm, n, -WMProjAbs);
      if (DoWMProjFrac)
        ProjNormFracThick(&fx, &fy, &fz, lhwm, n, -WMProjFrac);
      MRISsetXYZ(lhwm, n, fx, fy, fz);
      if (DoGMProjAbs)
        ProjNormDist(&fx, &fy, &fz, lhctx, n, +GMProjAbs);
      if (DoGMProjFrac)
        ProjNormFracThick(&fx, &fy, &fz, lhctx, n, +GMProjFrac);
  ...
  if (UseRH) {
    ...
    for (n = 0; n < rhwm->nvertices; n++) {
      if (DoWMProjAbs)
        ProjNormDist(&fx, &fy, &fz, rhwm, n, -WMProjAbs);
      if (DoWMProjFrac)
        ProjNormFracThick(&fx, &fy, &fz, rhwm, n, -WMProjFrac);
      MRISsetXYZ(rhwm, n, fx, fy, fz);
      if (DoGMProjAbs)
        ProjNormDist(&fx, &fy, &fz, rhctx, n, +GMProjAbs);
      if (DoGMProjFrac)
        ProjNormFracThick(&fx, &fy, &fz, rhctx, n, +GMProjFrac);
      MRISsetXYZ(rhctx, n, fx, fy, fz);
    }
    if (UseMask && rhsegmask == nullptr) {
...
double VertexCost(double vctx, double vwm, double slope, double center,
  ...
  if (sign == 0)
    a = -fabs(slope * (d - center)); // not sure this is useful
  if (sign == -1)
    a = -(slope * (d - center));
  if (sign == +1)
    a = +(slope * (d - center));
...
double *GetSurfCosts(MRI *mov, MRI *notused, MATRIX *R0, MATRIX *R, double *p,
                     int dof, double *costs) {
  static MRI *vlhwm = nullptr, *vlhctx = nullptr, *vrhwm = nullptr,
             *vrhctx = nullptr;
  ...
  double angles[3], d, dsum, dsum2, dstd, dmean, vwm, vctx, c, csum, csum2,
      cstd, cmean, val;
  MATRIX *Mrot = nullptr, *Mtrans = nullptr, *Mscale = nullptr,
         *Mshear = nullptr;
  int nhits, n;
  // FILE *fp;
  ...
  if (dof > 3) {
    ...
    Mrot = MatrixIdentity(4, nullptr);

  Mscale = MatrixIdentity(4, nullptr);
  if (dof > 6) {
    Mscale->rptr[1][1] = p[6];
    Mscale->rptr[2][2] = p[7];
    Mscale->rptr[3][3] = p[8];
  ...
  if (UseLH) {
    vlhwm = MRIvol2surfVSM(mov, R, lhwm, vsm, interpcode, nullptr, 0, 0,
                           nsubsamp, vlhwm);
    vlhctx = MRIvol2surfVSM(mov, R, lhctx, vsm, interpcode, nullptr, 0, 0,
                            nsubsamp, vlhctx);
    if (lhcost == nullptr)
      lhcost = MRIclone(vlhctx, nullptr);
    if (lhcon == nullptr)
      lhcon = MRIclone(vlhctx, nullptr);
  }
  if (UseRH) {
    vrhwm = MRIvol2surfVSM(mov, R, rhwm, vsm, interpcode, nullptr, 0, 0,
                           nsubsamp, vrhwm);
    vrhctx = MRIvol2surfVSM(mov, R, rhctx, vsm, interpcode, nullptr, 0, 0,
                            nsubsamp, vrhctx);
    if (rhcost == nullptr)
      rhcost = MRIclone(vrhctx, nullptr);
    if (rhcon == nullptr)
      rhcon = MRIclone(vrhctx, nullptr);
  }
  ...
  if (UseLH) {
    for (n = 0; n < lhwm->nvertices; n += nsubsamp) {
      if (lhwm->vertices[n].ripflag != 0)
        continue;
      if (lhcostfile || lhcost0file)
        MRIsetVoxVal(lhcost, n, 0, 0, 0, 0.0);
      if (lhconfile)
        MRIsetVoxVal(lhcon, n, 0, 0, 0, 0.0);
      if (lhCortexLabel && MRIgetVoxVal(lhCortexLabel, n, 0, 0, 0) < 0.5)
        continue;
      if (UseMask && MRIgetVoxVal(lhsegmask, n, 0, 0, 0) < 0.5)
        continue;
      if (UseLabel && MRIgetVoxVal(lhlabel, n, 0, 0, 0) < 0.5)
        continue;
      ...
      csum2 += (c * c);
      if (lhcostfile || lhcost0file)
        MRIsetVoxVal(lhcost, n, 0, 0, 0, c);
      if (lhconfile)
        MRIsetVoxVal(lhcon, n, 0, 0, 0, d);
      // fprintf(fp,"%6d %lf %lf %lf %lf %lf %lf
  ...
  if (UseRH) {
    for (n = 0; n < rhwm->nvertices; n += nsubsamp) {
      if (rhwm->vertices[n].ripflag != 0)
        continue;
      if (rhcostfile || rhcost0file)
        MRIsetVoxVal(rhcost, n, 0, 0, 0, 0.0);
      if (rhconfile)
        MRIsetVoxVal(rhcon, n, 0, 0, 0, 0.0);
      if (rhCortexLabel && MRIgetVoxVal(rhCortexLabel, n, 0, 0, 0) < 0.5)
        continue;
      if (UseMask && MRIgetVoxVal(rhsegmask, n, 0, 0, 0) < 0.5)
        continue;
      if (UseLabel && MRIgetVoxVal(rhlabel, n, 0, 0, 0) < 0.5)
        continue;
      ...
      csum2 += (c * c);
      if (rhcostfile || rhcost0file)
        MRIsetVoxVal(rhcost, n, 0, 0, 0, c);
      if (rhconfile)
        MRIsetVoxVal(rhcon, n, 0, 0, 0, d);
      // fprintf(fp,"%6d %lf %lf %lf %lf %lf %lf
      // %lf\n",nhits,vwm,vctx,d,dsum,a,c,csum);
    }
    // fclose(fp);
...
int MinPowell(MRI *mov, MRI *notused, MATRIX *R, double *params, int dof,
  ...
  R0 = MatrixCopy(R, nullptr);

  OpenPowell2(pPowel, xi, dof, ftol, linmintol, nmaxiters, niters, &fret,
              compute_powell_cost);
  printf("Powell done niters = %d\n", *niters);

  for (n = 0; n < dof; n++)
    params[n] = pPowel[n + 1];
  GetSurfCosts(mov, nullptr, R0, R, params, dof, costs);
...
double RelativeSurfCost(MRI *mov, MATRIX *R0) {
  ...
  GetSurfCosts(mov, nullptr, R0, R, params, 6, costs);
  cost0 = costs[7];
