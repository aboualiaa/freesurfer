...
int main(int argc, char *argv[]) {
  ...
  if (extract) {
    ...
    mri_tmp = MRIextract(mri_T1, nullptr, dx / 2, dy / 2, dz / 2, dx, dy, dz);
    MRIfree(&mri_T1);
    mri_T1 = mri_tmp;
  }
  if (jpdf_name) {
  ...
  if (use_weighting) {
    mri_out =
        MRIsynthesizeWeightedVolume(mri_T1, mri_PD, w5, TR, w30, TR, 110, TE);
  } else {
    printf("synthesizing volume with TR=%2.1f msec, TE=%2.1f msec, and "
           "alpha=%2.2f degrees...\n",
           TR, TE, alpha);
    if (normalize)
    ...
    if (nfaf > 0)
      mri_out = MRIsynthesizeWithFAF(mri_T1, mri_PD, nullptr, TR,
                                     RADIANS(alpha), TE, nfaf, faf_coefs);
    else
      mri_out = MRIsynthesize(mri_T1, mri_PD, mri_T2star, nullptr, TR,
                              RADIANS(alpha), TE);
  }
  ...
  if (!stricmp(option, "-help"))
      ...
      for (j = 0; j < 2; j++) {
        ...
        if (!bias_coefs[i][j])
          ErrorExit(ERROR_NOMEMORY,
                    "%s: could not allocate bias coefficient array (%d, %d)",
                    Progname, i, j);
      }
    ...
    for (i = 0; i < 3; i++)
      for (j = 0; j < 2; j++) {
        ...
        if (!faf_coefs[i][j])
          ErrorExit(ERROR_NOMEMORY,
                    "%s: could not allocate faf coefficient array (%d, %d)",
                    Progname, i, j);
      }
...
static void print_usage() {
  printf("usage: %s [options] <TR> <alpha (deg)> <TE> <T1 volume> <PD volume> "
         "<output volume>\n",
         Progname);
  printf("The -w switch will use a fixed weighting in order to generate an "
         "output volume with\n"
         "optimal gray/white contrast\n");
}

static void print_help() {
  fprintf(stderr, "\nThis program will synthesize a flash acquisition based on "
                  "previously computed T1/PD maps\n");
  exit(1);
}
...
MRI *MRIsynthesizeWeightedVolume(MRI *mri_T1, MRI *mri_PD, float w5, float TR5,
  ...
  mri30 =
      MRIsynthesize(mri_T1, mri_PD, nullptr, nullptr, TR30, RADIANS(30), TE);
  mri5 = MRIsynthesize(mri_T1, mri_PD, nullptr, nullptr, TR5, RADIANS(5), TE);
#if 0
...
static int transform_T1_values_using_joint_pdf(MRI *mri_T1, char *jpdf_name,
  ...
  if (!jpdf)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate %d x %d bin jpdf lookup table\n",
...
static int apply_bias_field(MRI *mri, int nbias, float *bias_coefs[3][2]) {
  ...
  for (x = 0; x < mri->width; x++) {
    for (xb = 1.0, n = 1; n <= nbias; n++)
      xb += bias_coefs[0][0][n - 1] * cos(w0x * n * (x - x0)) +
            bias_coefs[0][1][n - 1] * sin(w0x * n * (x - x0));
    for (y = 0; y < mri->height; y++) {
      for (yb = 1.0, n = 1; n <= nbias; n++)
        yb += bias_coefs[1][0][n - 1] * cos(w0y * n * (y - y0)) +
              bias_coefs[1][1][n - 1] * sin(w0y * n * (y - y0));
      for (z = 0; z < mri->depth; z++) {
        for (zb = 1.0, n = 1; n <= nbias; n++)
          zb += bias_coefs[2][0][n - 1] * cos(w0z * n * (z - z0)) +
                bias_coefs[2][1][n - 1] * sin(w0z * n * (z - z0));
  ...
  for (x = 0; x < width; x++) {
    for (xb = 1.0, n = 1; n <= nfaf; n++)
      xb += faf_coefs[0][0][n - 1] * cos(w0x * n * (x - x0)) +
            faf_coefs[0][1][n - 1] * sin(w0x * n * (x - x0));
    for (y = 0; y < height; y++) {
      for (yb = 1.0, n = 1; n <= nfaf; n++)
        yb += faf_coefs[1][0][n - 1] * cos(w0y * n * (y - y0)) +
              faf_coefs[1][1][n - 1] * sin(w0y * n * (y - y0));
      for (z = 0; z < depth; z++) {
        for (zb = 1.0, n = 1; n <= nfaf; n++)
          zb += faf_coefs[2][0][n - 1] * cos(w0z * n * (z - z0)) +
                faf_coefs[2][1][n - 1] * sin(w0z * n * (z - z0));
        if (x == Gx && y == Gy && z == Gz)
          DiagBreak();
        MRIsampleVolume(mri_T1, x, y, z, &T1);
        if (T1 <= 0)
