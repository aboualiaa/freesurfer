...
int main(int argc, char **argv) {
  ...
  if (DoCheck) {
    ...
    for (nthin = 0; nthin < ninputs; nthin++) {
      ...
      if (mritmp->width != nc || mritmp->height != nr || mritmp->depth != ns) {
  ...
  for (nthin = 0; nthin < ninputs; nthin++) {
    if (DoRMS)
      break; // MRIrms reads the input frames
    if (Gdiag_no > 0 || debug) {
      printf("Loading %dth input %s\n", nthin + 1,
             fio_basename(inlist[nthin], nullptr));
      fflush(stdout);
    }
  ...
  if (DoPrune) {
    ...
    PruneMask = MRIframeBinarize(mriout, FLT_MIN, nullptr);
  ...
  if (M != nullptr) {
    ...
    mritmp = fMRImatrixMultiply(mriout, M, nullptr);
    if (mritmp == nullptr) {
      exit(1);
    }
    MRIfree(&mriout);
  ...
  if (DoMean) {
    ...
    mritmp = MRIframeMean(mriout, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  if (DoMedian) {
    printf("Computing median across frames\n");
    mritmp = MRIframeMedian(mriout, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  if (DoMeanDivN) {
    printf("Computing mean2 = sum/(nframes^2)\n");
    mritmp = MRIframeSum(mriout, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  ...
  if (DoFNorm) {
    ...
    mritmp = MRIframeNorm(mriout, nullptr, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  if (DoTAR1) {
    printf("Computing temoral AR1 %d\n", mriout->nframes - TAR1DOFAdjust);
    mritmp = fMRItemporalAR1(mriout, TAR1DOFAdjust, nullptr, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  ...
  if (DoStd || DoVar) {
    ...
    mritmp = fMRIcovariance(mriout, 0, -1, nullptr, nullptr);
    if (DoStd) {
  ...
  if (DoMax) {
    ...
    mritmp = MRIvolMax(mriout, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  ...
  if (DoMaxIndex) {
    ...
    mritmp = MRIvolMaxIndex(mriout, 1, nullptr, nullptr);
    if (DoMaxIndexPrune) {
  ...
  if (DoConjunction) {
    ...
    mritmp = MRIconjunct(mriout, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  ...
  if (DoMin) {
    ...
    mritmp = MRIvolMin(mriout, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  ...
  if (DoSort) {
    printf("Sorting \n");
    mritmp = MRIsort(mriout, mask, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  ...
  if (DoVote) {
    printf("Voting \n");
    mritmp = MRIvote(mriout, mask, nullptr);
    MRIfree(&mriout);
    mriout = mritmp;
  }
  ...
  if (NReplications > 0) {
    ...
    mritmp = MRIallocSequence(mriout->width, mriout->height, mriout->depth,
                              mriout->type, mriout->nframes * NReplications);
    if (mritmp == nullptr) {
      exit(1);
    }
    printf("Done allocing\n");
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help")) {
      ...
      if (!isdigit(pargv[0][0]) && pargv[0][0] != '-' && pargv[0][0] != '+') {
        printf("ERROR: value passed to the --max-index-add flag must be a "
               "number\n");
...
static void print_usage() {
  ...
  printf("   --paired-diff-norm : same as paired-diff but scale by TP1,2 "
         "average \n");
  ...
  printf("   --gmean Ng          : create matrix to average Ng groups, "
         "Nper=Ntot/Ng\n");
  printf("\n");
  ...
  printf("   --max-index  : compute index of max of concatenated volumes "
         "(1-based)\n");
  printf("   --max-index-prune  : max index setting to 0 any voxel where all "
         "frames are 0 (not the same as --prune)\n");
  ...
  printf("   --fnorm : normalize time series at each voxel (remove mean, "
         "divide by SSS)\n");
  printf(
      "   --conjunct  : compute voxel-wise conjunction concatenated volumes\n");
  printf("   --vote : most frequent value at each voxel and fraction of "
         "occurances\n");
  ...
  printf("   --pca  : output is pca. U is output.u.mtx and S is "
         "output.stats.dat\n");
  printf("   --pca-mask mask  : Only use voxels whose mask > 0.5\n");
  printf("   --scm  : compute spatial covariance matrix (can be huge!)\n");
  printf("\n");
  printf("   --max-bonfcor  : compute max and bonferroni correct (assumes "
         "-log10(p))\n");
...
static void check_options() {
  ...
  if (DoPairedDiffNorm1 && DoPairedDiffNorm2) {
    printf("ERROR: cannot specify both --paried-diff-norm1 and "
           "--paried-diff-norm2 \n");
    exit(1);
  }
  if (DoPairedDiffNorm && DoPairedDiffNorm1) {
    printf("ERROR: cannot specify both --paried-diff-norm and "
           "--paried-diff-norm1 \n");
    exit(1);
  }
