...
int main(int argc, char **argv) {
  ...
  if (parser.exists("measures")) {
    ...
    for (auto const &str :
         parser.retrieve<std::vector<std::string>>("measures")) {
      if (str == "dice")
  ...
  if (parser.exists("labels")) {
    ...
    // use all labels found in segs (we'll determine these after already looping
    // through each voxel)
    all_labels = true;
  }
  ...
  if (all_labels) {
    // if we're using all of the unique labels in the volumes, populate the
    // labels vector with the keys of the intermediate_metrics map
    for (auto const &m : intermediate_metrics)
      if (m.first != 0)
    ...
    labels.erase(std::remove_if(labels.begin(), labels.end(),
                                [&intermediate_metrics](int l) {
                                  return intermediate_metrics.count(l) < 1;
                                }),
                 labels.end());
  }
  ...
  for (int l : labels) {
    ...
    // check if the label is wm or cortex so we can created a weighted mean of
    // all other structures
    bool subcortical = false;
    if ((parser.exists("seg")) && (l != 2) && (l != 41) && (l != 3) &&
        (l != 42))
      subcortical = true;
    // compute measures
    for (auto &measure : measures) {
      ...
      if (subcortical)
        measure.wmean_sc += value * combined_volume;
    }
    total_volume += combined_volume;
    if (subcortical)
      subcortical_volume += combined_volume;
  }
  ...
  for (auto &measure : measures) {
    ...
    for (auto const &v : measure.labels)
      sqr_diff_sum += pow(v.second - measure.mean, 2);
    measure.std = sqrt(sqr_diff_sum / labels.size());
    // weighted means
    measure.wmean /= total_volume;
    if (parser.exists("seg"))
      measure.wmean_sc /= subcortical_volume;
  }
  ...
  if (!parser.exists("quiet")) {
    ...
    if (reportNames) {
      for (int l : labels)
        if (labelnames[l].length() + 2 > name_width)
          name_width = labelnames[l].length() + 2;
    }
    ...
    if (reportNames)
      std::cout << std::setw(name_width) << "names";
    for (auto const &measure : measures)
      std::cout << std::setw(8) << measure.name;
    std::cout << term::reset() << std::endl;
    // print rows of results
    for (int l : labels) {
      std::cout << std::setw(7) << l;
      if (reportNames)
        std::cout << std::setw(name_width) << labelnames[l];
      for (auto &measure : measures)
        std::cout << std::setw(8) << measure.labels[l];
      std::cout << std::endl;
    }
  }
  ...
  if (parser.exists("out")) {
    ...
    for (auto &measure : measures) {
      ...
      for (int l : labels)
        subjson["labels"][std::to_string(l)] = measure.labels[l];
      json["measures"][measure.name] = subjson;
    }
    // label names
    if (reportNames)
      for (int l : labels)
        json["names"][std::to_string(l)] = labelnames[l];
    // inputs
