...
static int find_rh_seed_point(MRI *mri, int *prh_vol_x, int *prh_vol_y,
                              int *prh_vol_z);
...
static char *atlas_name = nullptr;
#if 0
    ...
    for (k = 0; k < mri_seg->depth - 1; k++)
      for (j = 0; j < mri_seg->height - 1; j++)
        for (i = 0; i < mri_seg->width - 1; i++) {
          ...
          if (MRIvox(mri_seg, ind1_i[refp], ind1_j[refp], ind1_k[refp]) !=
              label) {
          ...
          if (MRIvox(mri_seg, ind2_i[refp], ind2_j[refp], ind2_k[refp]) !=
              label) {
...
static int mri_topofix(MRI *mri_seg, MRI *mri_orig) {
  ...
  int i, j, k, niter = 100, ntotal = 0, nmodified, label, forbidden_label,
               nvoxels;
  ...
  while (niter--) {
    ...
    topofix_pbm = 0;
    nmodified += mriRemoveBackgroundCornerConfiguration(mri_seg, mri_orig,
...
static char *segmentation_fname = nullptr;
...
int verifyLRSplit(MRI *mri_fill, LTA *lta, double cc_tal_x, int *pbadRH,
                  int *pbadLH, int *ptotRH, int *ptotLH) {
  int           x, y, z;
...
static MRI *find_cutting_plane(MRI *mri, double x_tal, double y_tal,
                               double z_tal, int orientation, int *pxv,
                               int *pvy, int *pzv, int seed_set,
                               const LTA *lta);
static int  find_slice_center(MRI *mri, int *pxo, int *pyo);
static int  find_cc_seed_with_segmentation(MRI *mri_tal, MRI *mri_seg,
                                           double *pcc_tal_x, double *cc_tal_y,
                                           double *cc_tal_z);
static int find_corpus_callosum(MRI *mri, double *ccx, double *ccy, double *ccz,
                                const LTA *lta);
static int find_pons(MRI *mri, double *p_ponsx, double *p_ponsy,
                     double *p_ponsz, int x_cc, int y_cc, int z_cc, int which);
...
static MRI *mriReadBinaryProbabilities(const char *atlas_name,
                                       const char *suffix, M3D *m3d,
...
int main(int argc, char *argv[]) {
  ...
  MRI *mri_cc = nullptr, *mri_pons = nullptr, *mri_norm = nullptr, *mri_lh_fill,
      *mri_rh_fill, *mri_lh_im, *mri_rh_im /*, *mri_blur*/, *mri_labels,
      *mri_tal, *mri_tmp, *mri_tmp2, *mri_saved_labels, *mri_seg;
  ...
  if (fillonly) {
    mri_rh_fill = MRIclone(mri_im, nullptr);
    mri_rh_im   = MRIcopy(mri_im, nullptr);
    if (find_rh_voxel || rhv) // rh specified
      MRIfillVolume(mri_rh_fill, mri_rh_im, rh_vol_x, rh_vol_y, rh_vol_z,
                    rh_fill_val);
    else // assume it's lh
      MRIfillVolume(mri_rh_fill, mri_rh_im, lh_vol_x, lh_vol_y, lh_vol_z,
                    lh_fill_val);
  ...
  */
  ...
  if (atlas_name && false) {
    ...
    fprintf(stderr, "removing extraneous white matter using atlas %s...\n",
            atlas_name);
    mri_p_wm =
        mriReadBinaryProbabilities(atlas_name, "wm", m3d, sname, nullptr);
  ...
  ////////////////////////////////////////////////////////////////////////////
  mri_talheader = MRIallocHeader(mri_im->width, mri_im->height, mri_im->depth,
                                 mri_im->type, mri_im->nframes);
  ...
  } else // lta is not loaded and thus we create
  {
    ...
    lt->sigma = 1.0f;
    lt->x0 = lt->y0 = lt->z0 = 0;
    m_L = lt->m_L = MatrixIdentity(4, nullptr);
    ...
    if (mri_im->linear_transform) {
      ...
      if (getenv("NO_AVERAGE305")) // if this is set
      {
        ...
        fprintf(stderr, "INFO: Modifying talairach volume c_(r,a,s) based "
                        "on average_305\n");
        dst->valid = 1;
  ...
  //////////////////////////////////////////////////////////////////
  mri_tal =
      MRIalloc(mri_im->width, mri_im->height, mri_im->depth, mri_im->type);
  ...
  MRIbinarize(mri_tal, mri_tal, DEFAULT_DESIRED_WHITE_MATTER_VALUE / 2 - 1, 0,
              110);
  ...
  if (segmentation_fname) {
    ...
    if (MRIlabelInVolume(mri_seg, CC_Central)) {
      ...
      mri_tmp = MRIreplaceCCwithWM(mri_seg, nullptr);
      MRIfree(&mri_seg);
      mri_seg = mri_tmp;
    }
#if 0
  ...
  if (cc_seed_set) {
    ...
    MRItalairachToVoxelEx(mri_im, cc_tal_x, cc_tal_y, cc_tal_z, &xv, &yv, &zv,
                          lta);
    if (xv > (mri_im->width - 1) || xv < 0 || yv > (mri_im->height - 1) ||
        yv < 0 || zv > (mri_im->depth - 1) || zv < 0) {
      fprintf(stderr,
              "The seed point (%.2f, %.2f, %.2f) "
              "is mapped to a voxel (%.2f, %.2f. %.2f).\n",
              cc_tal_x, cc_tal_y, cc_tal_z, xv, yv, zv);
      fprintf(stderr, "Make sure that the seed point is "
  ...
  // cc_seed is give by voxel position
    ...
    MRIvoxelToTalairachEx(mri_im, cc_vol_x, cc_vol_y, cc_vol_z, &cc_tal_x,
                          &cc_tal_y, &cc_tal_z, lta);
  ...
  {
    if (mri_seg) {
      MRI *mri_seg_tal = MRIclone(mri_seg, nullptr);

      MRIcopyHeader(mri_talheader, mri_seg_tal);

      MRItoTalairachExInterp(mri_seg, mri_seg_tal, lta, SAMPLE_NEAREST);

      if (find_cc_seed_with_segmentation(mri_tal, mri_seg_tal, &cc_tal_x,
                                         &cc_tal_y, &cc_tal_z) == NO_ERROR) {
        cc_seed_set = 1;
        mri_cc      = find_cutting_plane(mri_tal, cc_tal_x, cc_tal_y, cc_tal_z,
                                    MRI_SAGITTAL, &x_cc, &y_cc, &z_cc,
                                    cc_seed_set, lta);
    ...
    if (mri_cc == nullptr) {
      if (find_corpus_callosum(mri_tal, &cc_tal_x, &cc_tal_y, &cc_tal_z, lta) !=
          NO_ERROR) {
        mri_cc = nullptr; // could not find corpus_callosum
      } else
        mri_cc = find_cutting_plane(mri_tal, cc_tal_x, cc_tal_y, cc_tal_z,
                                    MRI_SAGITTAL, &x_cc, &y_cc, &z_cc,
                                    cc_seed_set, lta);
  ...
  mri_tmp = MRIcopy(mri_im, nullptr); // src volume space
  // if (mri_seg == NULL)
  ...
  MRIvoxelToWorld(mri_tal, (double)x_cc, (double)y_cc, (double)z_cc, &cc_tal_x,
                  &cc_tal_y, &cc_tal_z);
  ...
  if (log_fp)
    fprintf(log_fp, "CC:   %d, %d, %d (TAL: %2.1f, %2.1f, %2.1f)\n", x_cc, y_cc,
            z_cc, cc_tal_x, cc_tal_y, cc_tal_z);
  if (alog_fp) {
  ...
  if (mri_seg == nullptr) {
    ...
    if (pons_seed_vol_set) {
      ...
      double xv, yv, zv;
      printf("Verify whether the seed point is inside the volume first\n");
      MRItalairachToVoxelEx(mri_im, pons_tal_x, pons_tal_y, pons_tal_z, &xv,
                            &yv, &zv, lta);
      if (xv > (mri_im->width - 1) || xv < 0 || yv > (mri_im->height - 1) ||
          yv < 0 || zv > (mri_im->depth - 1) || zv < 0) {
        fprintf(stderr,
        ...
        fprintf(stderr, "Make sure that the seed point is given "
    ...
    } else // automatic find pons
    {
      ...
      if (cc_mask) /* limit pons to be directly below corpus callosum */
      {
        ...
        mri_tmp  = MRIcopy(mri_im, nullptr); // src volume
        mri_mask = MRIcopy(mri_cc, nullptr); // in src volume space
      ...
      mri_tal = MRIalloc(mri_tmp->width, mri_tmp->height, mri_tmp->depth,
                         mri_tmp->type);
      MRIcopyHeader(mri_talheader, mri_tal);
      ...
      // binarize
      MRIbinarize(mri_tal, mri_tal, DEFAULT_DESIRED_WHITE_MATTER_VALUE / 2 - 1,
                  0, 110);
      // the last arg is method = 0 (default)
      find_pons(mri_tal, &pons_tal_x, &pons_tal_y, &pons_tal_z, x_cc, y_cc,
                z_cc, 0);
      /////////////////////////////////////////////    // up to here OK
      if (mri_tmp != mri_im) {
        MRIfree(&mri_tmp);
    ...
    mri_pons = find_cutting_plane(mri_tal, pons_tal_x, pons_tal_y, pons_tal_z,
                                        ...
                                        try different */
    {
      ...
      MRIbinarize(mri_tal, mri_tal, DEFAULT_DESIRED_WHITE_MATTER_VALUE / 2 - 1,
                  0, 110);
      find_pons(mri_tmp, &pons_tal_x, &pons_tal_y, &pons_tal_z, x_cc, y_cc,
                z_cc, 1);
      if (mri_tmp != mri_im) {
        MRIfree(&mri_tmp);
        if (mri_mask) {
                      ...
                      the pons - try Talairach coords */
    {
      ...
      mri_pons = find_cutting_plane(mri_im, pons_tal_x, pons_tal_y, pons_tal_z,
                                    MRI_HORIZONTAL, &x_pons, &y_pons, &z_pons,
                                    pons_seed_set, lta);
      if (!mri_pons && !mri_seg) {
    ...
    if (mri_pons) {
      ...
      if (log_fp) {
        fprintf(log_fp, "PONS: %d, %d, %d (TAL: %2.1f, %2.1f, %2.1f)\n", x_pons,
                y_pons, z_pons, pons_tal_x, pons_tal_y, pons_tal_z);
        fclose(log_fp);
      }
      if (alog_fp) {
        fprintf(alog_fp, "PONS-CRS %d %d %d\n", x_pons, y_pons, z_pons);
        fprintf(alog_fp, "PONS-TAL %5.1f %5.1f %5.1f\n", pons_tal_x, pons_tal_y,
                pons_tal_z);
        fclose(alog_fp);
      }

      mri_tmp2 = MRIcopy(mri_im, nullptr); // cutting plane
      if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
  ...
  */
  ...
  if (fill_val) {
    fprintf(stderr, "writing out image with cutting planes to 'planes.mgz'.\n");
  ...
  if (rh_seed_set) {
    if (rhv) {
      MRIvoxelToTalairachEx(mri_im, rh_vol_x, rh_vol_y, rh_vol_z, &xr, &yr, &zr,
                            lta);
      rh_tal_x = xr;
      rh_tal_y = yr;
      rh_tal_z = zr;
      ...
      MRItalairachToVoxelEx(mri_im, rh_tal_x, rh_tal_y, rh_tal_z, &xr, &yr, &zr,
                            lta);
      wm_rh_x = nint(xr);
      wm_rh_y = nint(yr);
      wm_rh_z = nint(zr);
    ...
    MRItalairachToVoxelEx(mri_im, cc_tal_x + 2 * SEED_SEARCH_SIZE, cc_tal_y,
                          cc_tal_z, &xr, &yr, &zr, lta);
    ...
    if (lhonly)
      ...
      wm_rh_y = nint(yr);
      wm_rh_z = nint(zr);
      if (wm_rh_x < 0 || wm_rh_x >= mri_im->width || wm_rh_y < 0 ||
          wm_rh_y >= mri_im->height || wm_rh_z < 0 || wm_rh_z >= mri_im->depth)
        ErrorExit(ERROR_BADPARM,
           ...
           MIN_NEIGHBORS)) // one voxel neighbors are dark
      {
        ...
        for (z = wm_rh_z - seed_search_size; z <= wm_rh_z + seed_search_size;
             z++) {
          zi = mri_im->zi[z];
          for (y = wm_rh_y - seed_search_size; y <= wm_rh_y + seed_search_size;
               y++) {
            yi = mri_im->yi[y];
            for (x = wm_rh_x - seed_search_size;
                 x <= wm_rh_x + seed_search_size; x++) {
              xi = mri_im->xi[x];
        ...
        if (!found)
          ErrorExit(ERROR_BADPARM,
                    "could not find rh seed point around (%d, %d, %d)", wm_rh_x,
                    wm_rh_y, wm_rh_z);
        wm_rh_x = xnew;
        wm_rh_y = ynew;
        wm_rh_z = znew;
  ...
  if (lh_seed_set) {
    if (lhv) {
      MRIvoxelToTalairachEx(mri_im, lh_vol_x, lh_vol_y, lh_vol_z, &xr, &yr, &zr,
                            lta);
      lh_tal_x = xr;
      lh_tal_y = yr;
      lh_tal_z = zr;
      ...
      MRItalairachToVoxelEx(mri_im, lh_tal_x, lh_tal_y, lh_tal_z, &xr, &yr, &zr,
                            lta);
      wm_lh_x = nint(xr);
      wm_lh_y = nint(yr);
      wm_lh_z = nint(zr);
    ...
    if (rhonly)
      ...
      wm_lh_y = nint(yr);
      wm_lh_z = nint(zr);
      if (wm_lh_x < 0 || wm_lh_x >= mri_im->width || wm_lh_y < 0 ||
          wm_lh_y >= mri_im->height || wm_lh_z < 0 || wm_lh_z >= mri_im->depth)
        ErrorExit(ERROR_BADPARM,
      ...
      if ((MRIvox(mri_im, wm_lh_x, wm_lh_y, wm_lh_z) <= WM_MIN_VAL) ||
        ...
        for (z = wm_lh_z - seed_search_size; z <= wm_lh_z + seed_search_size;
             z++) {
          zi = mri_im->zi[z];
          for (y = wm_lh_y - seed_search_size; y <= wm_lh_y + seed_search_size;
               y++) {
            yi = mri_im->yi[y];
            for (x = wm_lh_x - seed_search_size;
                 x <= wm_lh_x + seed_search_size; x++) {
              xi = mri_im->xi[x];
              if ((MRIvox(mri_im, xi, yi, zi) >= WM_MIN_VAL) &&
                  (neighbors_on(mri_im, xi, yi, zi) >= MIN_NEIGHBORS)) {
                found = 1;
        ...
        if (!found)
          ErrorExit(ERROR_BADPARM,
                    "could not find lh seed point around (%d, %d, %d)", wm_lh_x,
                    wm_lh_y, wm_lh_z);
        if (Gdiag & DIAG_SHOW) {
          fprintf(stderr, "found at (%d, %d, %d)\n", xnew, ynew, znew);
        }
  ...
  if (!lhonly &&
      (wm_rh_x < 0 || wm_rh_x >= mri_im->width || wm_rh_y < 0 ||
       wm_rh_y >= mri_im->height || wm_rh_z < 0 || wm_rh_z >= mri_im->depth))
    ErrorExit(ERROR_BADPARM,
              "rh white matter seed point out of bounds (%d, %d, %d)\n",
              wm_rh_x, wm_rh_y, wm_rh_z);
  if (!rhonly &&
      (wm_lh_x < 0 || wm_lh_x >= mri_im->width || wm_lh_y < 0 ||
       wm_lh_y >= mri_im->height || wm_lh_z < 0 || wm_lh_z >= mri_im->depth))
    ErrorExit(ERROR_BADPARM,
              "lh white matter seed point out of bounds (%d, %d, %d)\n",
              wm_lh_x, wm_lh_y, wm_lh_z);
  ...
  if (segmentation_fname && (mri_seg != nullptr)) {
    ...
    mri_lh_fill = MRIclone(mri_im, nullptr);
    mri_rh_fill = MRIclone(mri_im, nullptr);
    mri_lh_im   = MRIcopy(mri_im, nullptr);
    mri_rh_im   = MRIcopy(mri_im, nullptr);
    ...
    if (!rhonly) {
      ...
      MRIfillVolume(mri_lh_fill, mri_lh_im, wm_lh_x, wm_lh_y, wm_lh_z,
                    lh_fill_val);
    }
    if (!lhonly) {
      fprintf(stderr, "filling right hemisphere...\n");
      MRIfillVolume(mri_rh_fill, mri_rh_im, wm_rh_x, wm_rh_y, wm_rh_z,
                    rh_fill_val);
    }
    MRIfree(&mri_lh_im);
    MRIfree(&mri_rh_im);
    ...
    */
    ...
    mri_lh_im = MRIcopy(mri_lh_fill, nullptr);
    mri_rh_im = MRIcopy(mri_rh_fill, nullptr);
    ...
    MRIvoxelToTalairachEx(mri_lh_fill, (double)wm_lh_x, (double)wm_lh_y,
                          (double)wm_lh_z, &xr, &yr, &zr, lta);
    MRIvoxelToTalairachEx(mri_rh_fill, (double)wm_rh_x, (double)wm_rh_y,
                          (double)wm_rh_z, &xr, &yr, &zr, lta);

    mri_fill =
        MRIcombineHemispheres(mri_lh_fill, mri_rh_fill, nullptr, wm_lh_x,
                              wm_lh_y, wm_lh_z, wm_rh_x, wm_rh_y, wm_rh_z);
  ...
  if (atlas_name) {
    ...
    int req = snprintf(fname, STRLEN, "%s/%s/mri/transforms/%s.m3d",
                       subjects_dir, sname, atlas_name);
    if (req >= STRLEN) {
    ...
    mri_p_ventricle = mriReadBinaryProbabilities(atlas_name, "left_ventricle",
                                                 m3d, sname, nullptr);
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
...
#define TMP_FILL_VAL 5
...
#else
    MRItalairachToVoxelEx(mri_fill, cc_tal_x - 10, cc_tal_y, cc_tal_z, &xr, &yr,
                          &zr, lta);
    mri_ventricle =
        MRIfillVentricle(mri_fill, mri_p_ventricle, mri_T1, nullptr, 90.0f,
    ...
    mri_p_ventricle = mriReadBinaryProbabilities(atlas_name, "right_ventricle",
                                                 m3d, sname, nullptr);
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    ...
    MRItalairachToVoxelEx(mri_fill, cc_tal_x + 10, cc_tal_y, cc_tal_z, &xr, &yr,
                          &zr, lta);
    mri_ventricle = MRIfillVentricle(mri_fill, mri_p_ventricle, mri_T1, nullptr,
                                     90.0f, rh_fill_val, 0, nint(xr));
    ...
    mri_fg                                    = MRIclone(mri_fill, nullptr);
    ...
    mri_bg                  = MRIclone(mri_fill, nullptr);
  ...
  if (topofix) {
    ...
    mri_lh = MRIclone(mri_fill, nullptr);
    mri_rh = MRIclone(mri_fill, nullptr);
    ...
    if (mri_norm == nullptr) {
      ...
      mri_topofix(mri_rh,
                  mri_norm); // making sure to remove corner/edge configurations
      mri_topofix(mri_lh,
                  mri_norm); // making sure to remove corner/edge configurations
      MRIfree(&mri_norm);
      MRIfree(&mri_fill);
    ...
    for (z = 1; z != mri_fill->depth - 1; z++)
      for (i = 1; i != mri_fill->height - 1; i++)
        for (x = 1; x != mri_fill->width - 1; x++)
          if (MRIvox(mri_fill, x, i, z) == 0 && i > ylim0 - 10 &&
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    case 'A':
      ...
      nargs = 1;
      fprintf(stderr, "logging cutting plane coordinates to %s...\n", argv[2]);
      fprintf(alog_fp, "# This file contains the column, row, slice (CRS) "
...
static MRI *find_cutting_plane(MRI *mri_tal, double x_tal, double y_tal,
                               double z_tal, int orientation, int *pxv,
                               int *pyv, int *pzv, int seed_set,
                               const LTA *lta) {
  // here is the limitation on the voxsize being up to .5 mm
  MRI *mri_slices[MAX_SLICES * 2], *mri_filled[MAX_SLICES * 2], *mri_cut = NULL,
                                                                *mri_cut_vol;
  ...
  mri_slices[0] = MRIextractPlane(mri_tal, nullptr, orientation, where);
  // get the blob in the slice
  mri_filled[0] =
      MRIfillFG(mri_slices[0], nullptr, xo, yo, 0, WM_MIN_VAL, 127, &area[0]);
...
#define DIAG_VERBOSE_ON 1
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    // this works fine
    sprintf(fname, "cutting_plane_%s_slice0.mgz",
            orientation == MRI_SAGITTAL ? "cc" : "pons");
    MRIwrite(mri_slices[0], fname);
    sprintf(fname, "cutting_plane_%s_filled0.mgz",
            orientation == MRI_SAGITTAL ? "cc" : "pons");
  ...
  */
  ...
  done       = ((area[0] >= min_area) && (area[0] <= max_area) &&
          (aspect >= MIN_ASPECT) && (aspect <= MAX_ASPECT) && (region.y > 0) &&
          (region.x > 0) &&
  ...
  if (seed_set) /* just fill around the specified seed */
  {
    ...
    mri_cut = MRIcopy(mri_filled[0], nullptr);
    ...
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
      sprintf(fname, "%s_filled.mgz",
              orientation == MRI_SAGITTAL ? "cc" : "pons");
      MRIwrite(mri_slices[0], fname);
      sprintf(fname, "%s_cut.mgz", orientation == MRI_SAGITTAL ? "cc" : "pons");
      ...
      for (z0 = zv - offset; !done && z0 <= zv + offset; z0 += offset) {
        ...
        for (y0 = yv - offset; !done && y0 <= yv + offset; y0 += offset) {
          ...
          for (x0 = xv - offset; !done && x0 <= xv + offset; x0 += offset) {
            ...
            mri_slices[0] =
                MRIextractPlane(mri_tal, nullptr, orientation, where);
            /////////////////////////////////////////////////
            ...
            mri_filled[0] = MRIfillFG(mri_slices[0], nullptr, xoo, yoo, 0,
                                      WM_MIN_VAL, 127, &area[0]);
            MRIboundingBox(mri_filled[0], 1, &region);
            ...
            */
            ...
            if ((area[0] >= min_area) && (area[0] <= max_area) &&
                (aspect >= MIN_ASPECT) && (aspect <= MAX_ASPECT) &&
                (region.y > 0) && (region.x > 0) &&
                (region.x + region.dx < slice_size - 1) && // SLICE_SIZE-1) &&
                (region.y + region.dy < slice_size - 1))   // SLICE_SIZE-1))
            {
              fprintf(stderr,
                      "area[0] = %d (min = %d, max = %d), "
                      "aspect = %.2f (min = %.2f, max = %.2f)\n",
                      area[0], min_area, max_area, aspect, MIN_ASPECT,
                      MAX_ASPECT);

              /* center the seed */
              // xv and yv get modified, but done = 1 is
              // set and thus ok.
              find_slice_center(mri_filled[0], &xv,
                                &yv); // center index in the plane
              // getting blob center
              switch (orientation) {
              default:
              case MRI_HORIZONTAL:
  ...
  if (!seed_set) /* find slice with smallest cross-section */
  {
    for (slice = 0; slice < max_slices; slice++) {
      ...
      mri_slices[slice] = MRIextractPlane(mri_tal, nullptr, orientation, where);
      mri_filled[slice] = MRIfillFG(mri_slices[slice], nullptr, xo, yo, 0,
                                    WM_MIN_VAL, 127, &area[slice]);
    ...
    *pzv = nint(z);
    ...
    mri_cut = MRIcopy(mri_filled[min_slice], nullptr);
    if (Gdiag & DIAG_SHOW)
      fprintf(stderr, "%s: cutting at slice %d, area %d, (%d, %d, %d)\n", name,
              min_slice, min_area0, *pxv, *pyv, *pzv);
  ...
  mri_cut_vol = MRIclone(mri_tal, nullptr);
  switch (orientation) {
  default:
  case MRI_HORIZONTAL:
    where = *pyv;
...
static int find_corpus_callosum(MRI *mri_tal, double *pccx, double *pccy,
  ...
  if (mri_tal->linear_transform || lta) {
    MRIworldToVoxel(mri_tal, 0.0, 0.0, 0.0, &xr, &yr, &zr); /* everything is
                             now in tal coords */
    xv = nint(xr);
    yv = nint(yr);
    zv = nint(zr);
...
static int find_pons(MRI *mri, double *p_ponsx, double *p_ponsy,
                     double *p_ponsz, int x_cc, int y_cc, int z_cc,
                     int method) {
  MRI *  mri_slice, *mri_filled, *mri_closed;
  double xr, yr, zr;
  int xv, yv, zv, x, y, width, height, thickness, xstart, area, xmax, bheight;
  MRI_REGION region;
  ...
  // get sagittal slice
  mri_slice = MRIextractPlane(mri, nullptr, MRI_SAGITTAL, xv);
  ...
  switch (method) {
  case 1:
    mri_closed = MRIclose(mri_slice, nullptr); /* remove small holes */
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
  ...
  */
  ...
  mri_filled = MRIfillFG(mri_slice, nullptr, xstart - (thickness - 1) / 2, y, 0,
                         WM_MIN_VAL, 127, &area);

  if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
    fprintf(stderr, "found brainstem at y=%d, x = (%d --> %d), area=%d\n", y,
            xstart, xstart - thickness + 1, area);
  ...
  */
  ...
  for (thickness = 0, xstart = x;
       (x >= 0) && (thickness < max_brainstem_thickness) &&
  ...
  if (Gdiag & DIAG_SHOW)
    fprintf(stderr,
            ...
            xstart - thickness / 2, y, xv, yv, zv, *p_ponsx, *p_ponsy,
            *p_ponsz);
...
*/
static int find_cc_slice(MRI *mri_tal, double *pccx, double *pccy, double *pccz,
                         const LTA *lta) {
  // here we can handle only up to .5 mm voxel size
  int area[MAX_SLICES * 2], min_area, min_slice, slice, offset, xv, yv, zv, xo,
      yo;
  ...
  for (slice = 0; slice < max_slices; slice++) {
    ...
    yv        = nint(y);
    zv        = nint(z);
    mri_slice = MRIextractPlane(mri_tal, nullptr, MRI_SAGITTAL, xv);
    mri_filled =
        MRIfillFG(mri_slice, nullptr, zv, yv, 0, WM_MIN_VAL, 127, &area[slice]);
  ...
  if (min_slice < 0)
    ErrorReturn(
        ERROR_BADPARM,
        (ERROR_BADPARM, "%s: could not find valid seed for the cc", Progname));
...
MRI *MRIcombineHemispheres(MRI *mri_lh, MRI *mri_rh, MRI *mri_dst, int wm_lh_x,
                           int wm_lh_y, int wm_lh_z, int wm_rh_x, int wm_rh_y,
                           int wm_rh_z) {
  int width, height, depth, x, y, z, v, ncorrected, ambiguous, lh, rh, nfilled,
      xi, yi, zi, xk, yk, zk, nleft, nright, nambiguous, i;
  ...
  if (!mri_dst) {
    mri_dst = MRIclone(mri_lh, nullptr);
  }
  ...
  if (ambiguous) /* mark whole brain as ambiguous - will fill seeds later */
  {
    ...
    fprintf(stderr, "using variable coefficient diffusion "
...
static MRI *mriReadConditionalProbabilities(MRI *mri_T1, const char *atlas_name,
  ...
  sprintf(fname, "%s/average/%s_%s.mgz#%d", mri_dir, atlas_name, suffix,
          offset);
  fprintf(stderr, "reading atlas means from %s...\n", fname);
  mri_mean = MRIread(fname);
  if (!mri_mean)
    ErrorExit(ERROR_NOFILE, "%s: could not read mean file %s", Progname, fname);
  if (m3d) {
    mri_tmp = MRIapplyInverse3DMorph(mri_mean, m3d, nullptr);
    MRIfree(&mri_mean);
    mri_mean = mri_tmp;
  }
  ...
  if (!mri_std)
    ErrorExit(ERROR_NOFILE, "%s: could not read std file %s", Progname, fname);
  if (m3d) {
    mri_tmp = MRIapplyInverse3DMorph(mri_std, m3d, nullptr);
    MRIfree(&mri_std);
    mri_std = mri_tmp;
  }
  mri_dst =
      MRIcomputeConditionalProbabilities(mri_T1, mri_mean, mri_std, nullptr);
  MRIfree(&mri_mean);
  MRIfree(&mri_std);
  return (mri_dst);
}
...
static MRI *mriReadBinaryProbabilities(const char *atlas_name,
  ...
  if (!mri_priors)
    ErrorExit(ERROR_NOFILE, "%s: could not read priors file %s", Progname,
              fname);
  if (m3d) {
    mri_tmp = MRIapplyInverse3DMorph(mri_priors, m3d, nullptr);
    MRIfree(&mri_priors);
    mri_priors = mri_tmp;
  }

  mri_on_conditional  = mriReadConditionalProbabilities(mri_T1, atlas_name,
                                                       suffix, 1, m3d, nullptr);
  mri_off_conditional = mriReadConditionalProbabilities(
      mri_T1, atlas_name, suffix, 3, m3d, nullptr);
  mri_dst = MRIapplyBayesLaw(mri_priors, mri_on_conditional,
                             mri_off_conditional, nullptr);
  MRIfree(&mri_T1);
...
static MRI *MRIfillVentricle(MRI *mri_src, MRI *mri_prob, MRI *mri_T1,
                             MRI *mri_dst, float threshold, int out_label,
                             int xabs_min, int xabs_max) {
  BUFTYPE *pprob, *pdst, *psrc, out_val, prob, in_val;
  int width, height, depth, x, y, z, ymin, ymax, zmin, zmax, nfilled, xk, yk,
      zk, niter, xmin, xmax, expanded, expansions, xi, yi, zi, ventricle_voxels,
      total_expanded;
...
static int fillDiagonals(MRI *mri, int fillval) {
  int nfilled, x, y, z, width, height, depth, block[2][2][2], xk, yk, zk, x1,
      y1, z1, mxk, myk, mzk, diagonals, min_diagonals, filled;
...
#define WHALF ((11 - 1) / 2)
static int find_cc_seed_with_segmentation(MRI *mri, MRI *mri_seg,
                                          double *pcc_tal_x, double *pcc_tal_y,
                                          double *pcc_tal_z) {
...
static int mri_erase_nonmidline_voxels(MRI *mri_cc, MRI *mri_seg) {
  int  x, y, z, label, xl, xr, rlabel, llabel;
  MRI *mri_mid;

  mri_mid = MRIcopy(mri_cc, nullptr);
...
MRI *fill_with_aseg(MRI *mri_img, MRI *mri_seg) {
  ...
  MRIsetLabelValues(mri_img, mri_seg, mri_img, Right_Cerebellum_White_Matter,
                    0);
    ...
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if (val == WM_EDITED_OFF_VAL) {
          ...
          lh = MRIlabelsInNbhd(mri_seg, x, y, z, whalf,
                               Left_Cerebral_White_Matter) +
               MRIlabelsInNbhd(mri_seg, x, y, z, whalf, Left_Cerebral_Cortex);
          rh = MRIlabelsInNbhd(mri_seg, x, y, z, whalf,
                               Right_Cerebral_White_Matter) +
               MRIlabelsInNbhd(mri_seg, x, y, z, whalf, Right_Cerebral_Cortex);
          while (lh == 0 && rh == 0) {
            whalf++;
            lh = MRIlabelsInNbhd(mri_seg, x, y, z, whalf,
                                 Left_Cerebral_White_Matter) +
                 MRIlabelsInNbhd(mri_seg, x, y, z, whalf, Left_Cerebral_Cortex);
            rh =
                MRIlabelsInNbhd(mri_seg, x, y, z, whalf,
                                Right_Cerebral_White_Matter) +
                MRIlabelsInNbhd(mri_seg, x, y, z, whalf, Right_Cerebral_Cortex);
            if (whalf > 20) // give up
              break;
...
static MRI *MRIreplaceCCwithWM(MRI *mri_src, MRI *mri_dst) {
  ...
  mri_dist_lh =
      MRIdistanceTransform(mri_src, nullptr, Left_Cerebral_White_Matter, 100,
                           DTRANS_MODE_SIGNED, nullptr);
  mri_dist_rh =
      MRIdistanceTransform(mri_src, nullptr, Right_Cerebral_White_Matter, 100,
                           DTRANS_MODE_SIGNED, nullptr);
