...
int main(int argc, char *argv[]) {
  ...
  for (i = gcsa->ninputs - 1; i >= 0; i--) {
    printf("input %d: %s, flags %0x, avgs %d, name %s\n", i,
           gcsa->inputs[i].type == GCSA_INPUT_CURV_FILE ? "CURVATURE FILE"
                                                        : "MEAN CURVATURE",
           gcsa->inputs[i].flags, gcsa->inputs[i].navgs, gcsa->inputs[i].fname);
...
static int relabel_unknowns_with_cortex_label(GCSA *gcsa, MRI_SURFACE *mris,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    } else               // cortex label says it is in cortex
    {
      ...
      {
        if (vno == Gdiag_no)
          printf("vertex label unknown but in cortex.label - marking for "
                 "reclassification\n");
      ...
      for (n = 0; n < nexcluded; n++)
        if (v->annotation == exclude_list[n]) {
          if (vno == Gdiag_no)
            printf("vertex label in exclude list - marking for "
                   "reclassification\n");
          v->marked = MARK_RELABEL;
          num_marked_for_relabel++;
          break;
