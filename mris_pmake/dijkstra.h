...
/// \brief Add another node to the linked list of minimum paths. This node is
/// added in the correct position to maintain a sorted cost order. \param  surf
/// The surface containing all the vertices \param  vno The vertex index to add
/// to the list \return  (int) Error condition
...
int mark(MRIS *surf, int vno, int m, bool b_overwrite = false);

/// \fn int dijkstra(s_env& st_env, float *cost_fct)
/// \fn int dijkstra(s_env& st_env, float *cost_fct)
/*/// \fn int dijkstra */
/// \brief The core entry point into the dijkstra engine. The start and end
/// vertex indices are contained in
///  the environment structure, st_env. This function determines the shortest
///  path between these
/// vertices. If the optional af_maxAllowedCost is non-zero, if any path cost is
/// found to exceed this value, the function will return. \param  st_env The
/// problem environment \param  *cost_fct A cost function to use on the weighted
/// graph \param af_maxAllowedCost An optional termination condition on the
/// dijkstra function. \return    ERROR | NO_ERROR
int dijkstra(s_env &st_env, float af_maxAllowedCost = 0.0,
...
public:
  ...
  //
  C_dlist(MRIS *apMS_surface);
  ~C_dlist();

  //
  // Add / remove / mark nodes
  //
  int node_addOrdered(int avertex);
  int node_remove(int avertex);
  int node_mark(int avertex, e_DIJKMARK ae_dijkMark, bool ab_forceMark = false);
};
