...
C_mpmProg_NOP::C_mpmProg_NOP(s_env *aps_env) : C_mpmProg(aps_env) {
  //
  // ARGS
  //
  // DESC
...
C_mpmProg_pathFind::C_mpmProg_pathFind(s_env *aps_env, int amvertex_start,
                                       int amvertex_end)
    : C_mpmProg(aps_env) {
  //
  // ARGS
  ...
  if (amvertex_start >= mvertex_total || amvertex_start < 0)
    vertexStart_set(0);
  else
    vertexStart_set(amvertex_start);

  if (amvertex_end == -1 || amvertex_end >= mvertex_total || amvertex_end < 0)
...
float C_mpmProg_pathFind::cost_compute(int a_start, int a_end) {
  //
  // ARGS
  ...
  pULOUT(colsprintf(-50, 20, pch_buffer, "Marking (rip) path along vertices",
                    "[ ok ]\n"));
...
int C_mpmProg_pathFind::run() {
  ...
  colsprintf(mps_env->lw, mps_env->rw, pch_buffer, "Start->End vertices",
  ...
  colsprintf(mps_env->lw, mps_env->rw, pch_buffer, "Total path cost",
             " [ %f ]\n", f_cost);
  if (Gb_stdout)
    printf("%s", pch_buffer);
  pULOUT(pch_buffer);
  ...
  if (mps_env->b_patchFile_save) {
    str_patchFQName = mps_env->str_workingDir + mps_env->str_patchFileName;
    if (MRISwritePatch(mps_env->pMS_primary, (char *)str_patchFQName.c_str()) !=
        NO_ERROR)
      exit(1);
  ...
  if (mps_env->b_labelFile_save) {
    ...
    pULOUT(colsprintf(mps_env->lw, mps_env->rw, pch_buffer,
                      "Labeling and saving all target vertices...",
                      " [ ok ]\n"));
  }
...
C_mpmProg_autodijk::C_mpmProg_autodijk(s_env *aps_env) : C_mpmProg(aps_env) {
  //
  // ARGS
  //
  // DESC
...
C_mpmProg_autodijk::~C_mpmProg_autodijk() {
  ...
  if (mpOverlayDistance)
    delete mpOverlayDistance;
}

void C_mpmProg_autodijk::worldMap_set(int avalue) {
  /*
...
bool C_mpmProg_autodijk::worldMap_shouldCreate() { return (mb_worldMap); }

/*!
  \fn C_mpmProg_autodijk::CURV_fileWrite()
  \brief Write a FreeSurfer curvature array to a file
  \return If curvature file is successfully written, return e_OK, else return
  e_WRITEACCESSERROR.
*/
...
float C_mpmProg_autodijk::cost_compute(int a_start, int a_end) {
  //
  // ARGS
  // a_start          in              start vertex index
  // a_end            in              end vertex index
  ...
  if (!(calls % (mprogressIter))) {
    if (mb_worldMap && !mb_worldMapDistanceCalc) {
      ...
      mps_env->pcsm_stdout->colprintf(
          "here/end->opposite/total = cost", "[ %6d/%6d -> %6d/%6d = ", a_start,
          mvertex_end, ms_stats.indexMax, mvertex_total);
    }
    if (!mb_worldMap) {
      mps_env->pcsm_stdout->colprintf(
          "start->end = cost", "[ %6d -> %6d/%6d = 0.0]\n",
          mps_env->startVertex, mps_env->endVertex, mvertex_total);
    }
  }
  ok = dijkstra(*mps_env); // costs are written to vertex elements along
                           //+ the path in the MRIS structure.
...
int C_mpmProg_autodijk::vertexCosts_pack(e_stats &a_stats) {
  /*
...
int C_mpmProg_autodijk::run() {
  ...
  if (mb_performExhaustive) {
    ...
    mps_env->pcsm_stdout->colprintf("Progress iteration interval", "[ %d ]\n",
                                    mprogressIter);
    for (int v = mvertex_start; v <= mvertex_end; v += mvertex_step) {
  ...
  if (mb_simpleStatsShow) {
    mps_env->pcsm_stdout->colprintf("max(cost) @ index", "[ %f : %d ]\n",
                                    ms_stats.f_max, ms_stats.indexMax);
  }
...
int C_mpmProg_autodijk_fast::run() {
  ...
  // implementation to use.  This version of Dijkstra selects which devices to
  // use automatically.
...
int C_mpmProg_externalMesh::run() {
  //
  // DESC
  // Main entry to the actual 'run' core of the mpmProg
  //
