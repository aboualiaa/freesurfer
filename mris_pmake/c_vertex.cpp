...
bool vertex_alwaysTrue(VERTEX *pvertex, void *pv_void) { return true; }
...
void vertex_signumFunctional(VERTEX *apvertex, void *apv_functional) {
  s_signumFunctional *ps_signum = (s_signumFunctional *)apv_functional;
...
void vertex_rawCurveSum(VERTEX *apvertex, void *apv_functional) {
  s_rawCurve *ps_rawC = (s_rawCurve *)apv_functional;

  if (apvertex->curv < 0)
    ps_rawC->f_negCount += apvertex->curv;
  if (apvertex->curv > 0)
    ps_rawC->f_posCount += apvertex->curv;
}
...
void vertex_curveAreaSum(VERTEX *apvertex, void *apv_functional) {
  s_integratedCurve *ps_totalIC = (s_integratedCurve *)apv_functional;
