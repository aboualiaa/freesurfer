...
C_SMessage::C_SMessage(std::string astr_body, e_SMessageFormat ae_format,
  ...
  if (ae_IO != eSM_c)
    error("This constructor can only be used for C-style C_SMessage objects",
          -1);
...
C_SMessage::C_SMessage(std::string astr_body, e_SMessageFormat ae_format,
  ...
  //   to stream is now handled exclusively by the dump() method. The
  //   constructor merely opens an output file if so specified and sets a
  //   boolean flag correspondingly.
  //
  // 05 April 2005
...
void C_SMessage::dump(bool ab_syslogPrepend, std::string astr_outOfBand) {
  ...
  if (e_format_get() != eSM_devnull) {
    ...
    case eSM_c:
      if (astr_outOfBand.length())
        fprintf(pFILE_out, "%s%s", str_syslogPrepend.c_str(),
...
bool C_SMessage::timer(e_SMTimerAction e_timerAction) {
  //
  // ARGS
