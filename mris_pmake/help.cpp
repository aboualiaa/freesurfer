...
void synopsis_show() {
  outputHelpXml(mris_pmake_help_xml, mris_pmake_help_xml_len);
}
...
void version_show() {
  ...
  std::cout << std::endl << "\t\t" << G_VERSION;
  std::cout << std::endl << "";
  exit(0);
}

bool mpmProg_check(s_env &st_env, std::string str_mpmProg) {
  ...
  if (!str_mpmProg.length()) {
    ...
    error_exit("processing command line options,",
               "you must also specify '--mpmProg <mpmProg>'.", 20);
  }
  if (str_mpmProg == "NULL") {
    b_validMpmProg          = true;
...
bool mpmOverlay_check(s_env &st_env, std::string str_mpmOverlay) {
  ...
  if (!str_mpmOverlay.length()) {
    s_env_mpmPrint(
        ...
        e_mpmOverlay);
    error_exit("processing command line options,",
               "you must also specify '--mpmOverlay <mpmOverlayID>'.", 20);
  }
...
std::string commandLineOptions_process(int argc, char **ppch_argv,
  ...
  std::string str_subjectsDir = "";
  char *      pch_subjectsDir;
  std::string str_subject = "";
  std::string str_hemi    = "";
  int         port        = 1701;

  std::string str_mpmProg        = "";
  std::string str_mpmArgs        = "-x";
  std::string str_mpmOverlay     = "";
  std::string str_mpmOverlayArgs = "-x";

  std::string str_mainSurfaceFileName   = "inflated";
  std::string str_auxSurfaceFileName    = "smoothwm";
  std::string str_mainCurvatureFileName = "smoothwm.H.crv";
  std::string str_auxCurvatureFileName  = "sulc";
  ...
  while (true) {
    ...
    case 'V':
      ...
      break;
    default:
      std::cout << "?? getopt returned character code " << opt << std::endl;
      break;
    }
  }
  st_env.str_hemi         = str_hemi;
  std::string str_p       = str_subjectsDir + "/" + str_subject + "/surf/";
  ...
  while (!st_env.b_optionsFileUse) {
    ...
    st_env.str_primarySurfaceFileName =
        str_p + str_hemi + "." + str_mainSurfaceFileName;
    st_env.str_secondarySurfaceFileName =
        str_p + str_hemi + "." + str_auxSurfaceFileName;
    st_env.str_primaryCurvatureFileName =
        str_p + str_hemi + "." + str_mainCurvatureFileName;
    st_env.str_secondaryCurvatureFileName =
        str_p + str_hemi + "." + str_auxCurvatureFileName;
    if (!mpmProg_check(st_env, str_mpmProg)) {
      s_env_mpmPrint(
          ...
          e_mpmProg);
      error_exit("processing command line options,",
                 "I didn't recognize the <mpmProg>.", 20);
    }
    st_env.b_mpmProgUse     = true;
    ...
    if (!mpmOverlay_check(st_env, str_mpmOverlay)) {
      s_env_mpmPrint(
          ...
          e_mpmOverlay);
      error_exit("processing command line options,",
                 "I didn't recognize the <mpmOverlay>.", 20);
    }
