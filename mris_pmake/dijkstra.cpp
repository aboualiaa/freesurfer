...
void unlist(int vno) {
  ...
  last = nullptr;
  for (dn = d_list; dn != nullptr; dn = dn->next) {
    if (dn->vno == vno) {
      if (last == nullptr)
        d_list = dn->next;
      else
...
int mark(MRIS *surf, int vno, int m, bool b_overwrite) {
  // HISTORY
  // 18 November 2004
...
int dijkstra(s_env &st_env, float af_maxAllowedCost, bool ab_surfaceCostVoid) {
  ...
  for (i = 0; i < surf->nvertices; i++) {
    ...
    if (!calls || ab_surfaceCostVoid)
      surf->vertices[i].val = -1;
  }
  calls++;
  ...
  while (vno_c != vno_f || !totalLoops) {
    ...
    for (j = 0; j < (int)v_ct->vnum; j++) {
      ...
      if (v_n->marked == DIJK_DONE)
        continue; // for "circular" path searches
      ...
      if (af_maxAllowedCost && (f_pathCost > af_maxAllowedCost))
        continue;
...
clean:
  for (dn = d_list; dn != nullptr; dn = dn_next) {
    dn_next = dn->next;
    free(dn);
  }
  d_list = nullptr;
