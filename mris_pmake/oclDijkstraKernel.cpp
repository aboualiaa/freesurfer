...
//
//  Description:
//      Implementation of Dijkstra's Single-Source Shortest Path (SSSP)
//      algorithm on the GPU. The basis of this implementation is the paper:
//
...
///
void allocateOCLBuffers(cl_context gpuContext, cl_command_queue commandQueue,
                        GraphData *graph, cl_mem *vertexArrayDevice,
  ...
  // First, need to create OpenCL Host buffers that can be copied to device
  // buffers
  hostVertexArrayBuffer = clCreateBuffer(
      gpuContext, CL_MEM_COPY_HOST_PTR | CL_MEM_ALLOC_HOST_PTR,
      sizeof(int) * graph->vertexCount, graph->vertexArray, &errNum);
  oclCheckError(errNum, CL_SUCCESS);

  hostEdgeArrayBuffer =
      clCreateBuffer(gpuContext, CL_MEM_COPY_HOST_PTR | CL_MEM_ALLOC_HOST_PTR,
                     sizeof(int) * graph->edgeCount, graph->edgeArray, &errNum);
  oclCheckError(errNum, CL_SUCCESS);

  hostWeightArrayBuffer = clCreateBuffer(
      gpuContext, CL_MEM_COPY_HOST_PTR | CL_MEM_ALLOC_HOST_PTR,
  ...
  *vertexArrayDevice =
      clCreateBuffer(gpuContext, CL_MEM_READ_ONLY, sizeof(int) * globalWorkSize,
                     NULL, &errNum);
  oclCheckError(errNum, CL_SUCCESS);
  *edgeArrayDevice =
      clCreateBuffer(gpuContext, CL_MEM_READ_ONLY,
                     sizeof(int) * graph->edgeCount, NULL, &errNum);
  oclCheckError(errNum, CL_SUCCESS);
  *weightArrayDevice =
      clCreateBuffer(gpuContext, CL_MEM_READ_ONLY,
                     sizeof(float) * graph->edgeCount, NULL, &errNum);
  oclCheckError(errNum, CL_SUCCESS);
  *maskArrayDevice =
      clCreateBuffer(gpuContext, CL_MEM_READ_WRITE,
                     sizeof(int) * globalWorkSize, NULL, &errNum);
  oclCheckError(errNum, CL_SUCCESS);
  *costArrayDevice =
      clCreateBuffer(gpuContext, CL_MEM_READ_WRITE,
                     sizeof(float) * globalWorkSize, NULL, &errNum);
  oclCheckError(errNum, CL_SUCCESS);
  *updatingCostArrayDevice =
  ...
  errNum = clEnqueueCopyBuffer(commandQueue, hostVertexArrayBuffer,
                               *vertexArrayDevice, 0, 0,
                               sizeof(int) * graph->vertexCount, 0, NULL, NULL);
  oclCheckError(errNum, CL_SUCCESS);

  errNum =
      clEnqueueCopyBuffer(commandQueue, hostEdgeArrayBuffer, *edgeArrayDevice,
                          0, 0, sizeof(int) * graph->edgeCount, 0, NULL, NULL);
  oclCheckError(errNum, CL_SUCCESS);

  errNum = clEnqueueCopyBuffer(commandQueue, hostWeightArrayBuffer,
                               *weightArrayDevice, 0, 0,
...
void initializeOCLBuffers(cl_command_queue commandQueue,
  ...
  errNum =
      clEnqueueNDRangeKernel(commandQueue, initializeKernel, 1, NULL,
...
/// This function will compute the shortest path distance from sourceVertices[n]
/// -> endVertices[n] and store the cost in outResultCosts[n].  The number of
/// results it will compute is given by numResults.
///
/// This version of the function will run the algorithm on either just the CPU,
/// CPU + GPU, GPU, or Multi GPU depending on what compute resources are
...
void runDijkstraOpenCL(GraphData *graph, int *sourceVertices,
  ...
  gpuContext =
      clCreateContextFromType(0, CL_DEVICE_TYPE_GPU, NULL, NULL, &errNum);
...
void runDijkstra(cl_context gpuContext, cl_device_id deviceId, GraphData *graph,
  ...
  allocateOCLBuffers(gpuContext, commandQueue, graph, &vertexArrayDevice,
                     &edgeArrayDevice, &weightArrayDevice, &maskArrayDevice,
                     &costArrayDevice, &updatingCostArrayDevice,
                     globalWorkSize);

  // Create the Kernels
  cl_kernel initializeBuffersKernel;
  initializeBuffersKernel =
      clCreateKernel(program, "initializeBuffers", &errNum);
  oclCheckError(errNum, CL_SUCCESS);
  ...
  errNum |= clSetKernelArg(initializeBuffersKernel, 1, sizeof(cl_mem),
                           &costArrayDevice);
  errNum |= clSetKernelArg(initializeBuffersKernel, 2, sizeof(cl_mem),
                           &updatingCostArrayDevice);
  ...
  errNum |=
      clSetKernelArg(ssspKernel1, 5, sizeof(cl_mem), &updatingCostArrayDevice);
  ...
  errNum |=
      clSetKernelArg(ssspKernel2, 5, sizeof(cl_mem), &updatingCostArrayDevice);
  ...
  for (int i = 0; i < numResults; i++) {
    ...
    cl_event readDone;
    errNum = clEnqueueReadBuffer(commandQueue, maskArrayDevice, CL_FALSE, 0,
    ...
    while (!maskArrayEmpty(maskArrayHost, graph->vertexCount)) {
      size_t localWorkSize = maxWorkGroupSize;
      size_t globalWorkSize =
          oclRoundWorkSizeUp(localWorkSize, graph->vertexCount);
    ...
    errNum = clEnqueueReadBuffer(commandQueue, costArrayDevice, CL_FALSE, 0,
                                 sizeof(float) * graph->vertexCount,
                                 &outResultCosts[i * graph->vertexCount], 0,
                                 NULL, &readDone);
    oclCheckError(errNum, CL_SUCCESS);
    clWaitForEvents(1, &readDone);
  }
...
/// results it will compute is given by numResults.
///
/// This function will run the algorithm on as many GPUs as is available.  It
/// will create N threads, one for each GPU, and chunk the workload up to
/// perform (numResults / N) searches per GPU.
///
/// \param gpuContext Current GPU context, must be created by caller
/// \param graph Structure containing the vertex, edge, and weight arra
///              for the input graph
...
///
///
void runDijkstraMultiGPU(cl_context gpuContext, GraphData *graph,
  ...
  errNum =
      clGetContextInfo(gpuContext, CL_CONTEXT_DEVICES, 0, NULL, &deviceBytes);
  oclCheckError(errNum, CL_SUCCESS);
  ...
  DevicePlan *devicePlans =
      (DevicePlan *)malloc(sizeof(DevicePlan) * deviceCount);
  ...
  for (unsigned int i = 0; i < deviceCount; i++) {
    pthread_create(&threadIDs[i], NULL, (void *(*)(void *))dijkstraThread,
                   (void *)(devicePlans + i));
  }
...
/// results it will compute is given by numResults.
///
/// This function will run the algorithm on as many GPUs as is available along
/// with the CPU.  It will create N threads, one for each device, and chunk the
/// workload up to perform (numResults / N) searches per device.
///
/// \param gpuContext Current GPU context, must be created by caller
...
///
///
void runDijkstraMultiGPUandCPU(cl_context gpuContext, cl_context cpuContext,
  ...
  errNum =
      clGetContextInfo(cpuContext, CL_CONTEXT_DEVICES, 0, NULL, &deviceBytes);
  oclCheckError(errNum, CL_SUCCESS);
  ...
  DevicePlan *devicePlans =
      (DevicePlan *)malloc(sizeof(DevicePlan) * totalDeviceCount);
  pthread_t *threadIDs =
      (pthread_t *)malloc(sizeof(pthread_t) * totalDeviceCount);
...
void runDijkstraRef(GraphData *graph, int *sourceVertices,
  ...
  for (int i = 0; i < numResults; i++) {
      ...
      for (int tid = 0; tid < graph->vertexCount; tid++) {
        if (maskArray[tid] != 0) {
          ...
          for (int edge = edgeStart; edge < edgeEnd; edge++) {
            ...
            //  found that the correct thing to do was weightArray[edge].  I
            //  think this was a typo in the paper.  Either that, or I
            //  misunderstood the data structure.
            if (updatingCostArray[nid] >
                (costArray[tid] + graph->weightArray[edge])) {
              updatingCostArray[nid] =
                  (costArray[tid] + graph->weightArray[edge]);
            }
          }
        }
    ...
    // Copy the result back
    memcpy(&outResultCosts[i * graph->vertexCount], costArray,
           sizeof(float) * graph->vertexCount);
  }
