...
void s_weights_scan(s_weights &st_costWeight, C_scanopt &cso_options) {
  ...
  if (cso_options.scanFor("wd", &str_value))
    ...
    error_exit("scanning user options", "I couldn't find a wd weight.", 100);
  if (cso_options.scanFor("wc", &str_value))
    st_costWeight.wc = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wc weight.", 101);
  if (cso_options.scanFor("wh", &str_value))
    st_costWeight.wh = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wh weight.", 102);
  if (cso_options.scanFor("wdc", &str_value))
    st_costWeight.wdc = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wdc weight.", 103);
  if (cso_options.scanFor("wdh", &str_value))
    st_costWeight.wdh = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wdh weight.", 104);
  if (cso_options.scanFor("wch", &str_value))
    st_costWeight.wch = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wch weight.", 105);
  if (cso_options.scanFor("wdch", &str_value))
    st_costWeight.wdch = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wdch weight.", 106);
  if (cso_options.scanFor("wdir", &str_value))
    st_costWeight.wdir = atof(str_value.c_str());
  else
    error_exit("scanning user options", "I couldn't find a wdir weight.", 107);
}
...
int s_env_costFctSetIndex(s_env *apst_env, int aindex) {
  ...
  switch (aindex) {
  case 0:
    s_env_costFctSet(apst_env, costFunc_defaultDetermine, (e_COSTFUNCTION)0);
    break;
  case 1:
    s_env_costFctSet(apst_env, costFunc_unityReturn, (e_COSTFUNCTION)1);
    break;
  case 2:
    s_env_costFctSet(apst_env, costFunc_EuclideanReturn, (e_COSTFUNCTION)2);
    break;
  case 3:
    s_env_costFctSet(apst_env, costFunc_distanceReturn, (e_COSTFUNCTION)3);
    break;
  default:
    s_env_costFctSet(apst_env, costFunc_defaultDetermine, (e_COSTFUNCTION)0);
    break;
...
float costFunc_defaultDetermine(s_env &st_env, s_iterInfo *pst_iterInfo,
                                int vno_c, int j, bool b_relNextReference) {
  //
  // HISTORY
  // 09 November 2004
  // o Added st_iterInfo
