...
public:
  ...
  c_SSocket(int a_protocol, string astr_name = "Unnamed SSocket", int aid = 0);
  ~c_SSocket();
  ...
  const string str_obj_get() const { return str_obj; };
  const string str_name_get() const { return str_name; };
  ...
  void str_obj_set(string astr_val) { str_obj = astr_val; };
  void str_name_set(string astr_val) { str_name = astr_val; };
  ...
  const string str_proc_get() const { return str_proc[stackDepth_get()]; };
  const string str_proc_get(int i) { return str_proc[i]; };
...
public:
  c_SSocket_UDP(string astr_name = "Unnamed SSocket", int aid = 0);
  ~c_SSocket_UDP();
  ...
  //
  void print();
  int sendStr(string astr_msg, bool ab_localecho = false);

  int sendFile(string astr_fileName, bool ab_localecho = false);
  int recv_nonblocking(string &astr_msg, struct sockaddr *apST_saFrom = nullptr,
                       int *ap_fromLen = nullptr);
  int recv_timeout(string &astr_msg, struct sockaddr *apST_saFrom = nullptr,
                   int *ap_fromLen = nullptr);
  int recv(string &astr_msg, struct sockaddr *apST_saFrom = nullptr,
           int *ap_fromLen = nullptr);
};
...
public:
  ...
  // Constructor / destructor block
  //
  c_SSocket_UDP_transmit(string astr_remoteHostName, int a_port,
