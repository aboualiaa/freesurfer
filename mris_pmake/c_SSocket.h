...
public:
  ...
  std::string str_proc[SSTACKDEPTH]; // Used to track the current
  //      procedure being
  //      executed
protected:
  // base class info
  ...
  std::string        str_buf;    // payload buffer
...
public:
  ...
  c_SSocket(int a_protocol, std::string astr_name = "Unnamed SSocket",
            int aid = 0);
  ~c_SSocket();
  c_SSocket(const c_SSocket &c_SSocket);
  ...
  const std::string str_obj_get() const { return str_obj; };
  const std::string str_name_get() const { return str_name; };
  ...
  void str_obj_set(std::string astr_val) { str_obj = astr_val; };
  void str_name_set(std::string astr_val) { str_name = astr_val; };
  void str_proc_set(int depth, std::string astr_proc) {
    str_proc[depth] = astr_proc;
  };
  void              id_set(int value) { id = value; };
  void              verbosity_set(int value) { verbosity = value; };
  void              warnings_set(int value) { warnings = value; };
  void              stackDepth_set(int value) { stackDepth = value; };
  const std::string str_proc_get() const { return str_proc[stackDepth_get()]; };
  const std::string str_proc_get(int i) { return str_proc[i]; };

  //
  // get/set member routines
  //
  int sockID_get() const { return sockID; };
  int protocol_get() const { return protocol; };
};
...
public:
  c_SSocket_UDP(std::string astr_name = "Unnamed SSocket", int aid = 0);
  ~c_SSocket_UDP();

  //
  // Access block
  //
  int  port_get() const { return port; };
  int  timeoutSec_get() const { return timeoutSec; };
  int  timeoutUsec_get() const { return timeoutUsec; };
  int  readBufSize_get() const { return readBufSize; };
  void readBufSize_set(int aval) { readBufSize = aval; };

  //
  // Functional block
  //
  void print();
  int  sendStr(std::string astr_msg, bool ab_localecho = false);

  int sendFile(std::string astr_fileName, bool ab_localecho = false);
  int recv_nonblocking(std::string &    astr_msg,
                       struct sockaddr *apST_saFrom = nullptr,
                       int *            ap_fromLen  = nullptr);
  int recv_timeout(std::string &    astr_msg,
                   struct sockaddr *apST_saFrom = nullptr,
                   int *            ap_fromLen  = nullptr);
  int recv(std::string &astr_msg, struct sockaddr *apST_saFrom = nullptr,
           int *ap_fromLen = nullptr);
};
...
public:
  ...
  // Constructor / destructor block
  //
  c_SSocket_UDP_receive(int a_port, int a_timeoutSec = 0, int a_timeoutUsec = 0,
                        std::string astr_name = "Unnamed SSocket_UDP_receive",
                        int         aid       = 0);
...
protected:
  std::string     str_remoteHostName; // remotehost name for "write" sockets
...
public:
  ...
  // Constructor / destructor block
  //
  c_SSocket_UDP_transmit(std::string astr_remoteHostName, int a_port,
                         std::string astr_name = "Unnamed SSocket",
                         int         aid       = 0);
