...
float s_env_plyDepth_get(s_env &ast_env) {
  ...
  if (!pC_ROI_cast(ast_env.pCmpmProg, pC_ROI))
    error_exit("setting plyDepth", "internal mpmProg is not of type ROI", 20);
...
std::string s_env_HUP(s_env &st_env, c_SSocket_UDP_receive **pCSSocketReceive) {
  ...
  if (stat(str_optionsFQName.c_str(), &st_fileInfo))
    error_exit("checking on the options file,",
               "I couldn't access the options file. Does it exist?", 40);
  if (st_env.pcso_options)
    delete st_env.pcso_options;
...
void s_env_optionsFile_write(s_env &st_env, bool ab_setToDefaults) {
  //
  // ARGS
  ...
  if (O) {
    delete O;
    O = new C_SMessage("", eSM_raw, st_env.str_optionsFileName, eSM_c,
                       eOverwrite);
...
void s_env_scan(s_env &st_env) {
  ...
  if (str_sysMsgFileName != str_sysMsgFileNameOld) {
    ...
    st_env.pcsm_syslog =
        new C_SMessage("", eSM_raw, str_sysMsgFileName, esm_io);
  ...
  if (str_resultMsgFileName != str_resultMsgFileNameOld) {
    ...
    st_env.pcsm_resultlog =
        new C_SMessage("", eSM_raw, str_resultMsgFileName, esm_io);
  ...
  if (str_surfaceFileName != str_surfaceFileNameOld) {
    ...
    if (!pMS_primary) {
      nULOUT("\t\t\t[ failure ]\n");
      error_exit(
          "reading the primary surface,",
          "I couldn't access the file. Does it exist? Are permissions OK?", 30);
    }
    nULOUT("\t\t\t[ ok ]\n");
    if (st_env.b_secondarySurface) {
  ...
  if (str_curvatureFileName != str_curvatureFileNameOld &&
    ...
    if (MRISreadCurvature(pMS_primary,
      ...
      nULOUT("\t\t\t[ failure ]\n");
      error_exit(
          "reading the primary curvature file,",
          "I couldn't access the file. Does it exist? Are permissions OK?", 30);
    }
    nULOUT("\t\t\t[ ok ]\n");
  }
  if (str_secondaryCurvatureFile != str_secondaryCurvatureFileOld &&
    ...
    if (MRISreadCurvature(pMS_secondary,
      ...
      nULOUT("\t\t\t[ failure ]\n");
      error_exit(
          "reading the secondary curvature file,",
          "I couldn't access the file. Does it exist? Are permissions OK?", 30);
    }
    nULOUT("\t\t\t[ ok ]\n");
  }
...
bool s_env_surfaceCurvature_set(s_env &st_env, std::string astr_fileName) {
  ...
  ULOUT("Mapping curvature texture on primary surface...");
  MRISreadCurvature(st_env.pMS_primary, (char *)astr_fileName.c_str());
  nULOUT("\t\t\t[ ok ]\n");
...
bool s_env_secondarySurface_setCurvature(s_env &     st_env,
  ...
  MRISreadCurvature(st_env.pMS_secondary, (char *)astr_fileName.c_str());
  nULOUT("\t\t\t[ ok ]\n");
...
int s_env_mpmProgSetIndex(s_env *apst_env, int aindex) {
  ...
  case emp_NOP:
    ...
    break;
  case emp_pathFind:
    apst_env->pCmpmProg = new C_mpmProg_pathFind(
    ...
    if (apst_env->str_mpmArgs != "-x") {
      ...
      if (cso_mpm.scanFor("vertexFile", &str_option)) {
        if (!pC_ROI->vertexFile_load(str_option))
          error_exit("reading the vertexFile", "a file access error occurred",
                     10);
      }
      if (cso_mpm.scanFor("labelFile", &str_option)) {
        if (!pC_ROI->labelFile_load(str_option))
          error_exit("reading the labelFile", "a file access error occurred",
                     10);
      }
...
int s_env_mpmOverlaySetIndex(s_env *apst_env, int aindex) {
  ...
  case emo_curvature:
    ...
    if (apst_env->str_mpmOverlayArgs != "-x") {
      C_scanopt cso_mpm(apst_env->str_mpmOverlayArgs, ",", e_EquLink, "", ":");
      if (!cso_mpm.scanFor("primaryCurvature", &str_curvatureFile)) {
        error_exit("checking for curvature file to read",
                   "it seems no file was specified", 10);
      }
    }
    apst_env->pCmpmOverlay =
        new C_mpmOverlay_curvature(apst_env, str_curvatureFile);
    break;
  default:
