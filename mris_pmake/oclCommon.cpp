...
void oclPrintDevInfo(cl_device_id device) {
  ...
  // CL_DEVICE_NAME
  clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(device_string), &device_string,
                  NULL);
  ...
  cl_uint max_dims;
  clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(max_dims),
                  &max_dims, NULL);
  fprintf(stdout, "  CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS:\t%d\n", max_dims);

  // CL_DEVICE_MAX_WORK_ITEM_SIZES
  size_t workitem_size[3];
  clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(workitem_size),
                  &workitem_size, NULL);
  ...
  clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(workgroup_size),
                  &workgroup_size, NULL);
  ...
  cl_uint addr_bits;
  clGetDeviceInfo(device, CL_DEVICE_ADDRESS_BITS, sizeof(addr_bits), &addr_bits,
                  NULL);
  ...
  fprintf(stdout,
          "  CL_DEVICE_IMAGE_MAX_WIDTH:\t\t2d width %d, 2d height %d, 3d width "
          "%d, 3d height %d, 3d depth %d\n",
          (int)image2d_max_width, (int)image2d_max_height,
          (int)image3d_max_width, (int)image3d_max_height,
  ...
  clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(mem_size), &mem_size,
                  NULL);
  fprintf(stdout, "  CL_DEVICE_LOCAL_MEM_SIZE:\t\t%d KByte\n",
          (unsigned int)(mem_size / 1024));

  // CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE
  clGetDeviceInfo(device, CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE, sizeof(mem_size),
                  &mem_size, NULL);
  ...
  clGetDeviceInfo(device, CL_DEVICE_QUEUE_PROPERTIES, sizeof(queue_properties),
