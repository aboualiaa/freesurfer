...
void FreeSurferExecutable::SetArgument(std::vector<Argument *> *iArguments,
                                       std::string              iFlag,
...
void FreeSurferExecutable::SetNextRequiredArgument(
    std::string iFlag, std::string iShortDescription,
...
void FreeSurferExecutable::SetNextOptionalArgument(
    std::string iFlag, std::string iShortDescription,
    std::string iLongDescription) {
...
void FreeSurferExecutable::SetName(std::string iName,
                                   std::string iDescription) {
  m_Name        = iName;
  m_Description = iDescription;
}

std::string FreeSurferExecutable::GetName() { return m_Name; }

std::string FreeSurferExecutable::GetSynposis() {
  std::string synopsis = m_Name;

  for (std::vector<Argument *>::const_iterator arguments =
           m_RequiredArguments.begin();
...
void FreeSurferExecutable::SetOutput(std::string iOutput) {
  m_Output = iOutput;
}

std::string FreeSurferExecutable::GetExampleUsage() {
...
void FreeSurferExecutable::SetVersion(std::string iVersion) {
  m_Version = iVersion;
}

std::string FreeSurferExecutable::GetVersion() { return m_Version; }

void FreeSurferExecutable::SetBugEmail(std::string iEmail) {
  m_BugEmail = iEmail;
}

void FreeSurferExecutable::PrintArgumentsHelp(
  ...
  for (std::vector<Argument *>::const_iterator arguments = iArguments.begin();
    ...
    Argument *argument = *arguments;
    std::cout << "   " << argument->flag << " <" << argument->shortDescription
              << ">" << std::endl;

    if (argument->longDescription != "") {
      std::cout << FormatString(argument->longDescription, "     ")
                << std::endl;
    }
...
void FreeSurferExecutable::PrintHelp() {
  std::string spaces = "   ";
...
void FreeSurferExecutable::PrintError(std::string iError) {
  std::cout << "ERROR: " << iError << std::endl;
}

void FreeSurferExecutable::PrintErrorArgument(std::string iArgument,
...
std::string *FreeSurferExecutable::GetRequiredArguments() {
...
void FreeSurferExecutable::WriteData(const std::string fileName,
...
void FreeSurferExecutable::WriteData(const std::string fileName,
                                     double **dataArray, const int nRows,
                                     const int nCols) {
...
void FreeSurferExecutable::WriteData(const std::string fileName,
                                     const float *data, const int nRows,
                                     const int nCols) {
...
void FreeSurferExecutable::WriteDataAppend(const std::string fileName,
                                           double **dataArray, const int nRows,
                                           const int nCols) {
...
void FreeSurferExecutable::ReadData(const char *fileName, const int cCols,
...
MRI *FreeSurferExecutable::ReadMRI(const char *fileName) {
...
std::string FreeSurferExecutable::GetCommandArguments(int argc, char *argv[]) {
...
std::string FreeSurferExecutable::GetFieldAndParameter(std::string field,
...
std::string FreeSurferExecutable::GetCurrentDirectory() {
...
std::string FreeSurferExecutable::GetOperatingSystem() {
  utsname uts;
  uname(&uts);
...
std::string FreeSurferExecutable::GetHostName() {
  utsname uts;
  uname(&uts);
...
std::string FreeSurferExecutable::GetMachine() {
  utsname uts;
  uname(&uts);
...
std::string FreeSurferExecutable::GetEnvironmentVariable(std::string variable) {
...
std::string FreeSurferExecutable::GetFreeSurferHome() {
  return GetEnvironmentVariable("FREESURFER_HOME");
}

std::string FreeSurferExecutable::GetSubjectsDirectory() {
  return GetEnvironmentVariable("SUBJECTS_DIR");
}

std::string FreeSurferExecutable::GetFileExtension(std::string fileName) {
...
void FreeSurferExecutable::PrintUsageError(std::string error) {
  std::cerr << "\n*** usage error: " << error << std::endl << std::endl;
}

bool FreeSurferExecutable::IsWhitespace(const char x) {
  return x == ' ' || x == '\n' || x == '\r' || x == '\t';
}

std::string FreeSurferExecutable::FormatString(std::string       toBeFormatted,
