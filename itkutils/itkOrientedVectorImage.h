 ...
 * <tt>Image< VariableLengthVector< double >, 3 ></tt>. The difference lies in
 * the memory organization. The latter results in a fragmented organization with
 * each location in the Image holding a pointer to an \c VariableLengthVector
 * holding the actual pixel. The former stores the \e k pixels instead of a
 * pointer reference, which apart from avoiding fragmentation of memory also
 * avoids storing a 8 bytes of pointer reference for each pixel. The parameter
 * \e k can be set using \c SetVectorLength.
 *
 ...
 * pointing to the buffer. (the container does not manage the memory). Similarly
 * SetPixel calls can be made with VariableLengthVector< double >.
 *
 * The API of this class is similar to Image.
 *
 * \section Caveats:
 * When using Iterators on this image, you cannot use the it.Value(). You must
...
class ITK_EXPORT OrientedVectorImage
...
public:
  ...
  itkTypeMacro(VectorImage, OrientedImage);
  ...
  typedef VectorImageNeighborhoodAccessorFunctor<Self>
...
protected:
  ...
  virtual ~OrientedVectorImage(){};

private:
  OrientedVectorImage(const Self &); // purposely not implementated
...
#include "itkOrientedVectorImage.txx"
#endif
