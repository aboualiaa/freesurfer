...
public:
  ...
  inline RealType Evaluate(const RealType &u) const {
    ...
    if (static_cast<unsigned int>(which) <
        this->m_BSplineShapeFunctions.rows()) {
      return PolynomialType(this->m_BSplineShapeFunctions.get_row(which))
          .evaluate(absValue);
    } else {
  ...
  inline RealType EvaluateDerivative(const double &u) const {
    ...
    if (which < this->m_BSplineShapeFunctions.rows()) {
      RealType der =
          PolynomialType(this->m_BSplineShapeFunctions.get_row(which))
              .devaluate(absValue);
