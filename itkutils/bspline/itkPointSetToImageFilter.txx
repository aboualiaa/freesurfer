...
template <class TInputPointSet, class TOutputImage>
void PointSetToImageFilter<TInputPointSet, TOutputImage>::SetInput(
...
template <class TInputPointSet, class TOutputImage>
void PointSetToImageFilter<TInputPointSet, TOutputImage>::SetInput(
    unsigned int index, const TInputPointSet *pointset) {
  // Process object is not const-correct so the const_cast is required here
...
template <class TInputPointSet, class TOutputImage>
const typename PointSetToImageFilter<TInputPointSet,
                                     TOutputImage>::InputPointSetType *
PointSetToImageFilter<TInputPointSet, TOutputImage>::GetInput(void) {
  if (this->GetNumberOfInputs() < 1) {
    return 0;
  }

  return static_cast<const TInputPointSet *>(this->ProcessObject::GetInput(0));
}
...
/** Update */
template <class TInputPointSet, class TOutputImage>
void PointSetToImageFilter<TInputPointSet, TOutputImage>::GenerateData(void) {
  unsigned int i;
  ...
  // spatial PointSet's bounding box will be used as default.
  ...
  // from the point-set is used as default.
  ...
  typedef
      typename InputPointSetType::PointsContainer::ConstIterator PointIterator;
...
template <class TInputPointSet, class TOutputImage>
void PointSetToImageFilter<TInputPointSet, TOutputImage>::PrintSelf(
