...
BSplineDataPointSetToImageFilter<
    TInputPointSet, TOutputImage>::BSplineDataPointSetToImageFilter() {
...
void BSplineDataPointSetToImageFilter<
    TInputPointSet, TOutputImage>::SetSplineOrder(unsigned int order) {
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<
    TInputPointSet, TOutputImage>::SetSplineOrder(ArrayType order) {
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<
    TInputPointSet, TOutputImage>::SetNumberOfLevels(unsigned int levels) {
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<TInputPointSet,
    ...
    for (unsigned int i = 0; i < ImageDimension; i++) {
      if (this->m_CurrentLevel < this->m_NumberOfLevels[i]) {
        this->m_CurrentNumberOfControlPoints[i] =
            2 * this->m_CurrentNumberOfControlPoints[i] -
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<TInputPointSet,
                                      TOutputImage>::RefineControlLattice() {
  ...
  while (!It.IsAtEnd()) {
    ...
    for (unsigned int i = 0;
      ...
      for (unsigned int j = 0; j < ImageDimension; j++) {
        tmp[j] = idx[j] + off[j];
        if (static_cast<unsigned int>(tmp[j]) >= NumberOfNewControlPoints[j] &&
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<TInputPointSet,
                                      TOutputImage>::GenerateControlLattice() {
  typename RealImageType::RegionType::SizeType size;
  for (unsigned int i = 0; i < ImageDimension; i++) {
    if (this->m_CloseDimension[i]) {
      size[i] =
  ...
  ImageRegionIteratorWithIndex<RealImageType> Itw(w, w->GetBufferedRegion());
  ImageRegionIteratorWithIndex<PointDataImageType> Itp(
      phi, phi->GetBufferedRegion());
  ...
  for (It = this->m_InputPointData->Begin();
    ...
    for (Itw.GoToBegin(); !Itw.IsAtEnd(); ++Itw) {
      ...
      for (unsigned int i = 0; i < ImageDimension; i++) {
        RealType u =
            static_cast<RealType>(p[i] - static_cast<unsigned>(p[i]) - idx[i]) +
  ...
  ImageRegionIterator<PointDataImageType> Itl(
      this->m_PhiLattice, this->m_PhiLattice->GetBufferedRegion());
  ImageRegionIterator<PointDataImageType> Itd(delta,
                                              delta->GetBufferedRegion());
  ImageRegionIterator<RealImageType> Ito(omega, omega->GetBufferedRegion());

  for (Itl.GoToBegin(), Ito.GoToBegin(), Itd.GoToBegin(); !Itl.IsAtEnd();
       ++Itl, ++Ito, ++Itd) {
    PointDataType P;
    P.Fill(0);
    if (Ito.Get() != 0) {
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<TInputPointSet,
                                      TOutputImage>::GenerateOutputImage() {
  ImageRegionIteratorWithIndex<ImageType> It(
...
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<TInputPointSet, TOutputImage>::Evaluate(
    PointType params, PointDataType &data) {
  vnl_vector<RealType> p(ImageDimension);
  for (unsigned int i = 0; i < ImageDimension; i++) {
    if (params[i] >= 1.0) {
      params[i] = 0.99999;
  ...
  for (Itw.GoToBegin(); !Itw.IsAtEnd(); ++Itw) {
    ...
    for (unsigned int i = 0; i < ImageDimension; i++) {
      RealType u = p[i] -
                   static_cast<RealType>(static_cast<unsigned>(p[i]) + idx[i]) +
                   0.5 * static_cast<RealType>(this->m_SplineOrder[i] - 1);
      B *= this->m_Kernel[i]->Evaluate(u);
    }
    for (unsigned int i = 0; i < ImageDimension; i++) {
...
void BSplineDataPointSetToImageFilter<
  ...
  for (unsigned int j = 0; j < gradient.cols(); j++) {
    for (Itw.GoToBegin(); !Itw.IsAtEnd(); ++Itw) {
      ...
      for (unsigned int k = 0; k < ImageDimension; k++) {
        RealType u =
            p[k] - static_cast<RealType>(static_cast<unsigned>(p[k]) + idx[k]) +
 ...
 */
template <class TInputPointSet, class TOutputImage>
void BSplineDataPointSetToImageFilter<TInputPointSet, TOutputImage>::PrintSelf(
