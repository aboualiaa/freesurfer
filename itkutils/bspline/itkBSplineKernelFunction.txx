...
BSplineKernelFunction<VSplineOrder>::BSplineKernelFunction() {
...
template <unsigned int VSplineOrder>
void BSplineKernelFunction<VSplineOrder>::SetSplineOrder(unsigned int order) {
...
template <unsigned int VSplineOrder>
void BSplineKernelFunction<VSplineOrder>::GenerateBSplineShapeFunctions(
    unsigned int order) {
  ...
  for (unsigned int i = 0; i < NumberOfPieces; i++) {
    PolynomialType poly = this->CoxDeBoor(
        order, knots, 0, static_cast<unsigned int>(0.5 * (order)) + i);
...
BSplineKernelFunction<VSplineOrder>::CoxDeBoor(unsigned short order,
  ...
  if (den == NumericTraits<RealType>::Zero) {
    ...
    tmp /= den;
    poly1 =
        PolynomialType(tmp) * this->CoxDeBoor(order - 1, knots, i, whichPiece);
  }
...
BSplineKernelFunction<VSplineOrder>::GetShapeFunctionsInZeroToOneInterval() {
  ...
  for (unsigned int i = 0; i < knots.size(); i++) {
    knots[i] =
        -static_cast<RealType>(this->m_SplineOrder) + static_cast<RealType>(i);
  }
...
template <unsigned int VSplineOrder>
void BSplineKernelFunction<VSplineOrder>::PrintSelf(std::ostream &os,
                                                    Indent indent) const {
