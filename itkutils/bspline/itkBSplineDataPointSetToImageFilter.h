...
/** \class BSplineDataPointSetToImageFilter.h
 * \brief Image filter which provides a B-spline output approximation.
 *
 * This class has been renamed to BSplineDataPointSetToImageFilter from
 * BSplineScatteredDataPointSetToImageFilter because the maximum character limit
 * for a tar file is 100 characters and we've exceeded that.
 *
...
protected:
  ...
  virtual ~BSplineDataPointSetToImageFilter();
  void PrintSelf(std::ostream &os, Indent indent) const;

  void GenerateData();

private:
  BSplineDataPointSetToImageFilter(const Self &); // purposely not implemented
  ...
  IndexToSubscript(unsigned int index, typename RealImageType::SizeType size) {
    ...
    for (unsigned int i = 0; i < ImageDimension; i++) {
      sub[ImageDimension - i - 1] =
          static_cast<unsigned int>(index / k[ImageDimension - i - 1]);
...
#include "itkBSplineDataPointSetToImageFilter.txx"
#endif
