/**
 * @file  mris_segment.c
 * @brief segments cortical areas based on connectivity/correlation/intensity
 * profiles
 *
 */
/*
...
MATRIX *MatrixFromMRI(MRI *mri_cmat, MATRIX *cmat, int frame) {
  ...
  for (r = 0; r < mri_cmat->height; r++)
    for (c = 0; c < mri_cmat->width; c++) {
      *MATRIX_RELT(cmat, r + 1, c + 1) =
          (float)MRIgetVoxVal(mri_cmat, c, r, 0, frame);
    }
...
VECTOR *MRIcmatDotProductFrames(MRI *mri1, int frame1, MRI *mri2, int frame2,
                                VECTOR *v_dot) {
  int r1, c1;
  double dot;
  ...
  for (r1 = 0; r1 < mri1->height; r1++) {
    for (dot = 0.0, c1 = 0; c1 < mri1->width; c1++) {
      dot +=
          MRIgetVoxVal(mri1, c1, r1, 0, 0) * MRIgetVoxVal(mri2, r1, c1, 0, 0);
    }
...
int MRIcmatNormalizeRows(MRI *mri_cmat) {
  int row, col, frame;
  double norm;
  ...
  for (frame = 0; frame < mri_cmat->nframes; frame++) {
    for (row = 0; row < mri_cmat->height; row++) {
      ...
      for (col = 0; col < mri_cmat->width; col++) {
        MRIsetVoxVal(mri_cmat, col, row, 0, frame,
                     MRIgetVoxVal(mri_cmat, col, row, 0, frame) / norm);
      }
    }
  }
  return (NO_ERROR);
}
int MatrixMaxRowIndex(MATRIX *m, int row) {
  int c, max_c;
  double max_val;
...
VECTOR *MatrixRowNorm(MATRIX *cmat, VECTOR *v_norm) {
  double norm, val;
  int r, c;
...
*/
double VectorLogLikelihood(VECTOR *v, VECTOR *v_mean, VECTOR *v_var) {
...
static MRI *classify_vertices(MRI_SURFACE *mris, MRI *mri_prior, MRI *mri_cmat,
  ...
  printf("classifying %d vertices in the FOV (%2.1f%%)\n", nvertices,
         100.0 * nvertices / (float)mris->nvertices);
  ...
  for (nevals = vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (!(vno % 100)) {
      printf("%d of %d complete (%d considered)\r", vno,
             end_index - start_index, nevals);
      fflush(stdout);
    }
    v = &mris->vertices[vno];
    ...
    if (classifier == CLASSIFY_GAUSSIAN) {
      ...
      if (nsubj_at_this_vertex > 0) // in at least one subject's label
      {
        ...
        for (total_corr = 0.0, sno = 0; sno < nsubjects - 1; sno++) {
          corrs[sno] =
              MatrixRowDotProduct(m_trains[sno], vno + 1 + start_index, v_test);
...
static LABEL *segment_area(MRI_SURFACE *mris, MRI *mri, LABEL *area,
                           int nvertices) {
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_segment.c,v 1.10 2016/12/11 14:33:41 fischl Exp $",
      "$Name:  $");
  ...
  for (sno = 0; sno < nsubjects; sno++) {
    ...
    if (mri_frame->height == 1 && mri_frame->nframes == mri_frame->width) {
      MRI *mri_tmp;
      mri_tmp =
          mri_reshape(mri_frame, mri_frame->width, mri_frame->nframes, 1, 1);
      MRIfree(&mri_frame);
      mri_frame = mri_tmp;
    }
    if (sno == 0) {
  ...
  mri_out = classify_vertices(mris, mri_prior, mri, labels, nsubjects,
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "cmat") || !stricmp(option, "input") ||
      !stricmp(option, "data")) {
    nargs = 1;
