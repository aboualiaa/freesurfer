...
int main(int argc, char *argv[]) {
  ...
  if (vol) {
    ...
    */
    ...
    if (area == NULL)
      ErrorExit(ERROR_NOFILE, "%s: could not read label %s", Progname, argv[2]);
  ...
  if (vol) {
    ...
    if (divide > 1) {
      ...
      if (MRISreadVertexPositions(mris, divide_surf_name) != NO_ERROR)
        ErrorExit(ERROR_BADPARM, "%s: could not read vertex coords from %s",
                  Progname, divide_surf_name);
      MRIScomputeSecondFundamentalForm(mris);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    vol = 1;
    printf(
        "computing distance to surface in volume instead of label distance\n");
...
static void print_help() {
  ...
  fprintf(stderr, "\t-posterior <dist>  - only use posteriormost <dist> "
                  "portion of label\n");
  fprintf(stderr, "\t-divide <n>        - divide label into <n> units along "
                  "primary eigendirection\n");
  fprintf(stderr, "\t-olabel            - output label subdivisions\n");
  exit(1);
}
