...
static char vcid[] =
    "$Id: mris_thickness_comparison.c,v 1.5 2011/03/02 00:04:34 nicks Exp $";
...
int main(int argc, char *argv[]) {
  ...
  if (strlen(subjects_dir) == 0) {
    ...
    if (!cp)
      ErrorExit(ERROR_UNSUPPORTED,
                "%s: must specify -sdir or SUBJECTS_DIR in env", Progname);
  ...
  if (MRISreadValues(mris, wfile_name) != NO_ERROR)
    ErrorExit(ERROR_BADFILE, "%s: could not read wfile from %s", Progname,
              wfile_name);
  ...
  for (i = 5; i < argc; i++) {
    ...
    printf("%s: manual thickness = %2.2f+-%2.2f (%d points)\n", label_name,
           mean_w, sqrt(var_w), npoints);
    printf("%s: auto   thickness = %2.2f+-%2.2f\n", label_name, mean_thick,
           sqrt(var_thick));
    printf("\n");
    if (log_fp)
      fprintf(log_fp, "%s %f %f %f %f %d\n", label_name, mean_w, sqrt(var_w),
              mean_thick, sqrt(var_thick), npoints);
    LabelFree(&area);
  }
  exit(0);
  return (0); /* for ansi */
