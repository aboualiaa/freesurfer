...
int main(int argc, char *argv[]) {
  ...
  if (in_surf_name) // create transform based on surface correspondences
  {
    ...
    if (MRISreadCanonicalCoordinates(mris, out_surf_name) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not read output surface from %s",
                Progname, out_surf_name);

    MRISrecenter(mris, CURRENT_VERTICES, CANONICAL_VERTICES);
    sprintf(fname, "./%s.sphere.centered",
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
    MRISwriteVertexLocations(mris, fname, CANONICAL_VERTICES);

    mri_warp = MRIScreateVolumeWarpFromSphere(
        mri_in, mri_out, mris, CURRENT_VERTICES, CANONICAL_VERTICES, 1);
    ...
    if (transform->type != MORPH_3D_TYPE) {
      ...
      /* convert it to a voxel transform */
      {
        ...
        ErrorExit(ERROR_UNSUPPORTED,
                  "%s: transforms must be all RAS or all voxel", Progname);
...
#else
  fprintf(stderr,
          "\nThis program will apply a linear transform to mri volume and "
          "write out the result.  The output volume is by default 256^3 1mm^3 "
          "isotropic, or you can specify an -out_like volume.  I think there's "
          "a bug in -i behavior if you're specifying multiple transforms.\n");
...
MATRIX *MT_CoronalRasXformToVoxelXform(MRI *mri_in, MRI *mri_out,
                                       MATRIX *m_corras_s2corras_t,
...
//#define _MTCRX2RX_DEBUG
  ...
  fprintf(stderr,
          "m_vox_s2vox_t = D_trg_inv * m_corras_s2corras_t * D_src = \n");
...
MRI *MRIScreateVolumeWarpFromSurface(MRI *mri_in, MRI *mri_out,
  ...
  mri_warp_x = MRIallocSequence(mri_in->width, mri_in->height, mri_in->depth,
                                MRI_FLOAT, 1);
  mri_warp_y = MRIallocSequence(mri_in->width, mri_in->height, mri_in->depth,
                                MRI_FLOAT, 1);
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (xv < 0 || yv < 0 || zv < 0 || xv >= mri_warp->width ||
      ...
      continue;
    if (vno == Gdiag_no) {
      printf("v %d: (%d, %d, %d) <- (%2.1f, %2.1f, %2.1f) (Dx=%2.1f, %2.1f, "
             "%2.1f)\n",
  ...
  MRIfree(&mri_warp_y);
  MRIfree(&mri_warp_z);
  MRIfree(&mri_warp_x_smooth);
  MRIfree(&mri_warp_y_smooth);
  MRIfree(&mri_warp_z_smooth);
...
static MRI *MRIScreateVolumeWarpFromSphere(MRI *mri_in, MRI *mri_out,
    ...
    for (n = 0; n < vt->vnum; n++) {
      ...
      for (dist = 0; dist <= 1.0; dist += sample_dist) {
        ...
        for (r = -sample_dist; r >= -MAX_SAMPLE_DIST; r -= sample_dist) {
          ...
          yo = cy0 + r * cny;
          zo = cz0 + r * cnz;
          MRISsurfaceRASToVoxelCached(mris, mri_in, xi, yi, zi, &xiv, &yiv,
                                      &ziv);
          MRISsurfaceRASToVoxelCached(mris, mri_out, xo, yo, zo, &xov, &yov,
                                      &zov);
          xv = nint(xov);
          yv = nint(yov);
          zv = nint(zov);
          ...
          if (nint(MRIgetVoxVal(mri_ctrl, xv, yv, zv, 0)) == CONTROL_MARKED)
            continue;
          if (vno == Gdiag_no) {
            printf("v %d: n %2.1f, (%d, %d, %d) <- (%2.1f, %2.1f, %2.1f) "
                   "(Dx=%2.1f, %2.1f, %2.1f)\n",
                   vno, r, xv, yv, zv, xiv, yiv, ziv, xov - xiv, yov - yiv,
                   zov - ziv);
            DiagBreak();
          }
          MRIsetVoxVal(mri_warp_x, xv, yv, zv, 0, xiv - xov);
  ...
  MRISsurfaceRASToVoxelCached(mris, mri_in, mris->x0, mris->y0, mris->z0, &cx0,
                              &cy0, &cz0);
      ...
      for (xv = 0; xv < mri_ctrl->width; xv++)
        for (yv = 0; yv < mri_ctrl->height; yv++)
          for (zv = 0; zv < mri_ctrl->depth; zv++) {
            ...
            if (MRIgetVoxVal(mri_ctrl, xv, yv, zv, 0) == CONTROL_MARKED)
              continue;
            if (MRIgetVoxVal(mri_dilated, xv, yv, zv, 0) ==
                0) // doesn't border a control point
              continue;
