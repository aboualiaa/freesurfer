...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_map_cuts.c,v 1.3 2011/03/02 00:04:33 nicks Exp $",
      "$Name:  $");
  ...
  if (MRISreadVertexPositions(mris_out, inf_surf_name) != NO_ERROR)
    ErrorExit(ERROR_BADPARM, "%s: could not inflated surface %s", Progname,
              inf_surf_name);
  ...
  if (dilate) {
    ...
    printf("%d valid vertices (%2.1f %% of total)\n",
           MRISvalidVertices(mris_out),
           100.0 * MRISvalidVertices(mris_out) / mris_out->nvertices);
  }
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input patch> <output patch>", Progname);
#if 0
  printf(
