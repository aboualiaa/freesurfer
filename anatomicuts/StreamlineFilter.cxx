...
int main(int argc, char *argv[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << argv[0]
              << " -i streamlines -o streamlines -l minLength  -m "
                 "mask/segmentation -nu (filter ushape fibers) -s subsample -c "
                 "cleaningThreshold (std times) -d outputDirectory"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (std::string inputName = std::string(cl.follow("", 2, "-i", "-I"));
       access(inputName.c_str(), 0) == 0;
       inputName = std::string(cl.next(""))) {
    inputFiles.push_back(inputName);
  }

  std::vector<std::string> outputFiles;
  for (std::string outputName = std::string(cl.follow("", 2, "-o", "-O"));
       outputName.size() > 4; outputName = std::string(cl.next(""))) {
  ...
  if (clean) {
    ...
    clusterTools->SetDirectionalNeighbors(
        histoMeshes, mask, *clusterTools->GetDirections(DirectionsType::ALL),
        false);
  ...
  for (int i = 0; i < meshes->size(); i++) {
    ...
    if (clean) {
      ...
      stdCluster =
          clusterTools->GetStandardDeviation((*histoMeshes)[i], averageId) *
          cleaningThreshold;
      std::cout << (*fixMeshes)[i]->GetNumberOfCells() << " "
                << (*histoMeshes)[i]->GetNumberOfCells() << " " << stdCluster
                << " " << cleaningThreshold << std::endl;
    }
    ...
    if (outputFiles.size() == inputFiles.size()) {
      ...
      std::string filename =
          inputFiles[i].substr(inputFiles[i].find_last_of("/\\") + 1);
