...
function forAll() {
  ...
  for s in */; do #for s in `ls -dir [a-zA-Z]*`;
    subject=${s//[\/]/}
    echo ${subject}
...
function tractography() {
  ...
  bb=$3
  echo ${subject}
  if [[ -e ${DMRI_DIR}/${subject}/dmri/data.nii.gz ]]; then
...
function filterStreamlines() {
  ...
  model2=$2
  lenght=$3
  if [[ -e ${DMRI_DIR}/${subject}/dmri/wm2009parc2dwi.nii.gz ]]; then
...
function Measures() {
  ...
  for c in ${clusters[@]}; do
    ...
    if [[ -e ${SUBJECTS_DIR}/${subject}/dmri/DKI/dki_RD.nii.gz ]]; then
...
function ToAnat() {
  ...
  if [[ -e ${SUBJECTS_DIR}/${subject}/dmri/wm2009parc2dwi.nii.gz ]]; then
