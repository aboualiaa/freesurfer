...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -i anatomicutsFolder -n numClusters -c correspondenceFile "
                 "-m <numMeasures> <measure1Name> <measure1> ... "
                 "<measureNName> <measureN> -o output"
              << std::endl;
    return -1;
  }
  ...
  HierarchicalClusteringPruner<MeshType, ImageType>::Pointer hierarchyPruner =
      HierarchicalClusteringPruner<MeshType, ImageType>::New();
  ...
  for (unsigned int i = 0; i < measuresNames.size(); i++) {
    csv_file << " , mean" << measuresNames[i] << " ,   stde" << measuresNames[i]
             << " ,   meanCentroid" << measuresNames[i] << " ,   stdeCentroid"
             << measuresNames[i];
  }
  csv_file << std::endl;

  for (unsigned int k = 0; k < correspondences.size(); k++) {
    std::cout << correspondences[k] << " " << bundlesIndeces[correspondences[k]]
              << std::endl;
    ...
    for (unsigned int m = 0; m < measures.size(); m++) {
      ...
      for (; cells != mesh->GetCells()->End(); cells++) {
        ...
        for (pointIdIt = cells.Value()->PointIdsBegin();
          ...
          dist_inv +=
              avgPoints[avgPoints.Size() - j - 1].EuclideanDistanceTo(pt);
