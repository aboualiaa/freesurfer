    ...
    if (rep) {
      ...
      if (arrayCellData != NULL) {
        if (arrayCellData->GetNumberOfTuples() > 1) {
          std::cout << "WARNING: There are more than one representative index "
                       "in field data "
                    << std::endl;
        }
        ...
        for (vtkIdType k = 0; k < pointCount; k++) {
          pointBuf[k] =
              allPoints->InsertNextPoint(bundle->GetPoint(pointBuf[k]));
        }
        intArrayRepresentativesWeights->InsertNextValue(
