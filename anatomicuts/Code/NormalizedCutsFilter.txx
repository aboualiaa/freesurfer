...
void NormalizedCutsFilter<TMesh, TMembershipFunctionType>::Update() {
  if (!this->GetInput())
    itkExceptionMacro(<< "Mesh not set");

  int numberOfPoints =
      this->GetInput()->GetCells()->Begin().Value()->GetNumberOfPoints();
  ...
  while (queue.size() < this->GetNumberOfClusters()) {
    ...
    std::vector<std::pair<int, int>> centroidIndeces =
        this->SelectCentroidsParallel(
            sample, (*this->GetMembershipFunctionVector())[0]);
    ...
    if (sample->Size() > this->GetNumberOfFibersForEigenDecomposition()) {
      ...
      for (int i = 0; i < centroidIndeces.size(); i++) {
        for (int j = 0; j < sample->Size(); j++) {
          inIndeces.push_back(
              std::pair<int, int>(j, centroidIndeces[i].second));
    ...
    if (samplePositives->Size() > 0 && sampleNegatives->Size() > 0) {
      this->m_clusterIdHierarchy.push_back(
          std::make_pair(node._id, lastLabel + "0"));
      this->m_clusterIdHierarchy.push_back(
          std::make_pair(node._id, lastLabel + "1"));

      queue.push(PriorityNode<SampleType>(samplePositives->Size(),
                                          lastLabel + "0", samplePositives));
      queue.push(PriorityNode<SampleType>(sampleNegatives->Size(),
                                          lastLabel + "1", sampleNegatives));
      std::cout << "queue size " << queue.size() << " positives "
                << samplePositives->Size() << " negatives "
                << sampleNegatives->Size() << std::endl;
    } else {
...
NormalizedCutsFilter<TMesh, TMembershipFunctionType>::SelectCentroidsParallel(
  ...
  typename ThreadedMembershipFunctionType::Pointer threadedMembershipFunction =
      ThreadedMembershipFunctionType::New();
  typename ThreadedMembershipFunctionType::DomainType domain;
  domain[0] = 0;
  domain[1] = inIndeces.size() - 1;
...
NormalizedCutsFilter<TMesh, TMembershipFunctionType>::SelectCentroids(
  ...
  const unsigned int n = min(
      ...
      (int)samples
          ->Size()); //(int)samples->Size(); //this->GetNumberOfClusters()*10;
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    for (unsigned j = i; j < n; j++) {
      double val = membershipFunction->Evaluate(
          &samples->GetMeasurementVector(selected[i]),
          &samples->GetMeasurementVector(selected[j]));
      ms(i, j) = ms(j, i) = val;
    }
    diagonal(i, i) = ms.sum_row(i);
    identity(i, i) = 1;
