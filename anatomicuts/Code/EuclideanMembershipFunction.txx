...
double EuclideanMembershipFunction<TVector>::Evaluate(
  ...
  for (int i = 0; i < labels1->size() - 1; i++) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      euclid_inv += pow((*m1)[i * 3 + k] -
                            (*m2)[(labels2->size() - 1) * 3 - i * 3 + k - 3],
                        2);
    }
    if (this->m_withCosine) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        vec2_inv[k] = (*m2)[labels2->size() * 3 - ii * 3 + k - 3] -
                      (*m2)[labels2->size() * 3 - (ii + 1) * 3 + k - 3];
...
template <class TVector>
void EuclideanMembershipFunction<TVector>::AddChild(
    const MeasurementVectorType *child) {
...
void EuclideanMembershipFunction<TVector>::RecalculateCentroid() {
  ...
  for (int i = 0; i < this->childs.size(); i++) {
    ...
    for (int j = 0; j < numPoints; j++) {
      double euclid = 0, euclid_inv = 0;
      for (int k = 0; k < 3; k++) {
        euclid += pow((*this->GetCentroid())[j * 3 + k] -
                          (*this->childs[i])[j * 3 + k],
                      2);
        euclid_inv += pow((*this->GetCentroid())[j * 3 + k] -
                              (*this->childs[i])[numPoints * 3 - j * 3 + k - 3],
                          2);
      }
  ...
  for (int i = 0; i < this->childs.size(); i++) {
    ...
    for (int j = 0; j < numPoints; j++) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        euclid_inv += pow((averageMv)[j * 3 + k] -
                              (*this->childs[i])[numPoints * 3 - j * 3 + k - 3],
                          2);
      }
...
template <class TVector>
void EuclideanMembershipFunction<TVector>::PrintSelf(std::ostream &os,
