...
void FilterFibers<TInputMesh, TOutputMesh>::GenerateData() {
  ...
  std::cout << std::endl
            << this->GetInput()->GetNumberOfCells() << " " << unif(eng) << '\n';
  int index = 0;
  while (cellId <
         this->GetPercentage() * this->GetInput()->GetNumberOfCells()) {
    ...
    typedef itk::PolylineCell<typename OutputMeshType::CellType>
        PolylineCellType;
    line.TakeOwnership(new PolylineCellType);

    this->GetInput()->GetCell(number, cell);
    int k = 0;
    for (typename OutputCellType::PointIdIterator it = cell->PointIdsBegin();
