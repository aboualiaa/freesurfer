...
public:
  ...
  typedef
      typename InputMeshCellTraits::PointIdIterator InputMeshPointIdIterator;
  ...
  float GetPercentage() { return this->m_percentage; }

  void SetPercentage(float percentage) {

    if (percentage < 0.0 || percentage > 1.0) {
      std::cout << "Percentage should be between 0.0 and 1.0: dividing by 100. "
                << std::endl;
      percentage /= 100;
    }
