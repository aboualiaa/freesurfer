...
template <class TImage> void TrkVTKPolyDataFilter<TImage>::GenerateData() {}

template <class TImage> void TrkVTKPolyDataFilter<TImage>::TrkToVTK() {
  ...
  // std::cout << orientation[0]<< trkheadin.voxel_order_original[0] <<
  // std::endl; std::cout << orientation[1]<< trkheadin.voxel_order_original[1]
  ...
  while (trkreader.GetNextPointCount(&npts)) {
    ...
    if (this->m_color != nullptr) {
      allColors->InsertNextTuple3(this->m_color[0], this->m_color[1],
                                  this->m_color[2]);
    }
    delete[] ids;
  }
...
void TrkVTKPolyDataFilter<TImage>::VTKToTrk(std::string outputName) {
  ...
  if (this->m_refHeader == 0) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        if (i == j)
          trkheadout.vox_to_ras[i][j] =
              m_refImage->GetDirection()[i][j] * m_refImage->GetSpacing()[i];
        else
          trkheadout.vox_to_ras[i][j] = m_refImage->GetDirection()[i][j];
      }
      trkheadout.vox_to_ras[i][3] =
          m_refImage->GetOrigin()[i]; //*m_refImage->GetSpacing()[i];
    }
    // std::cout <<
    // m_refImage->GetLargestPossibleRegion().GetIndex()[0]<<std::endl; std::cout
    // << m_refImage->GetOrigin()[0]<< std::endl;
  ...
  for (int i=0; i<4 ; i++)
  {
          ...
          {
                  hola[i][j]=i+j; //
  m_refImage->GetDirection()[i][j]*m_refImage->GetSpacing()[i]; std::cout <<
  trkheadout.vox_to_ras[i][j]<< " ";
          }
  }
...
/*
   ...
   input->GetCellData()->Begin(); int i=0; for(;cellData!=
   input->GetCellData()->End(); ++cellData)
   {
...
void TrkVTKPolyDataFilter<TImage>::TrkToVTK()
{
        ...
        //std::cout << orientation[0]<< trkheadin.voxel_order_original[0] <<
std::endl;
        //std::cout << orientation[1]<< trkheadin.voxel_order_original[1] <<
std::endl;
        //std::cout << orientation[2]<< trkheadin.voxel_order_original[2] <<
std::endl; while (trkreader.GetNextPointCount(&npts))
        {
                float *iraw, *rawpts = new float[npts*3];
                std::vector<float> newpts;
                ...
                for (int ipt=0 ; ipt <npts; ipt++) {
                        // Divide by input voxel size and make 0-based to get
voxel coords


                        for (int k = 0; k < 3; k++) {
                                pt[k] = *iraw  ;// / trkheadin.voxel_size[k] ;//
*trkheadin.voxel_size[k]; index[k]= pt[k]/trkheadin.voxel_size[k]; iraw++;
                        }
                        if (!this->m_refImage.IsNull())
                        {
                                this->m_refImage->TransformContinuousIndexToPhysicalPoint(index,pt);
                ...
                {
                        allColors->InsertNextTuple3 ( this->m_color[0],
...
void TrkVTKPolyDataFilter<TImage>::VTKToTrk(std::string outputName)
{
        ...
        if (! m_refImage.IsNull())
        {
                ...
                for (int i=0; i<3 ; i++)
                {
                        ...
                        trkheadout.voxel_size[i] = m_refImage->GetSpacing()[i];
                        trkheadout.dim[i] =
m_refImage->GetLargestPossibleRegion().GetSize()[i];

                        for(int j=0;j<3;j++)
                        {
                                trkheadout.vox_to_ras[i][j]=
m_refImage->GetDirection()[i][j];
                        }
                }
                trkheadout.image_orientation_patient[0] =  -
trkheadout.vox_to_ras[0][0] / trkheadout.voxel_size[0];
