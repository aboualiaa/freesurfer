...
#if defined(WIN32) || defined(SOLARIS_RAW) || (__GNUC__ == 2) ||               \
    defined(__HP_aCC)
...
public:
  ...
  inline GetPot(const int argc_, char **argv_,
                const char *FieldSeparator = nullptr);
  inline GetPot(const char *FileName, const char *CommentStart = nullptr,
  ...
  inline double operator()(const char *VarName, const double &Default,
                           unsigned Idx) const;
  inline const char *operator()(const char *VarName, const char *Default,
                                unsigned Idx) const;

  //     -- setting variables
  //                  i) from outside of GetPot (considering prefix etc.)
  //                  ii) from inside, use '__set_variable()' below
  inline void set(const char *VarName, const char *Value,
                  const bool Requested = true);
  inline void set(const char *VarName, const double &Value,
                  const bool Requested = true);
  inline void set(const char *VarName, const int Value,
                  const bool Requested = true);
  ...
  inline double follow(const double &Default, unsigned No, const char *Option,
                       ...);
  inline const char *follow(const char *Default, unsigned No,
                            const char *Option, ...);
...
private:
  ...
  //     -- some functions return a char pointer to a temporarily existing
  //     string
  //        this container makes them 'available' until the getpot object is
  //        destroyed.
  std::vector<char *> __internal_string_container;

  //     -- keeping track about arguments that are requested, so that the UFO
  //     detection
  //        can be simplified
  ...
  //     -- if an argument is requested record it and the 'tag' the section
  //     branch to which
  ...
  inline double __convert_to_type(const std::string &String,
  ...
  //        create an argument vector based on data found in an input file,
  //        i.e.:
  ...
  STRING_VECTOR __get_section_tree(const std::string &FullPath) {
    // -- cuts a variable name into a tree of sub-sections. this is requested
    // for recording
...
//.............................................................................
//
inline void GetPot::__basic_initialization() {
  cursor = 0;
  nominus_cursor = -1;
...
inline GetPot::GetPot(const int argc_, char **argv_,
...
// leave 'char**' non-const to honor less capable compilers ...
{
  ...
  STRING_VECTOR _apriori_argv;
  // -- for the sake of clarity: we do want to include the first argument in the
  // argument vector !
  //    it will not be a nominus argument, though. This gives us a minimun
  //    vector size of one which facilitates error checking in many functions.
  //    Also the user will be able to retrieve the name of his application by
  //    "get[0]"
...
inline GetPot::GetPot(const char *FileName,
...
inline GetPot::GetPot(const GetPot &That) { GetPot::operator=(That); }

inline GetPot::~GetPot() {
  // may be some return strings had to be created, delete now !
  victorate(char *, __internal_string_container, it) delete[] * it;
}

inline GetPot &GetPot::operator=(const GetPot &That) {
  if (&That == this)
    return *this;
...
inline void GetPot::absorb(const GetPot &That) {
  if (&That == this)
    return;
...
inline void GetPot::clear_requests() {
  _requested_arguments.erase(_requested_arguments.begin(),
                             _requested_arguments.end());
  _requested_variables.erase(_requested_variables.begin(),
                             _requested_variables.end());
  _requested_sections.erase(_requested_sections.begin(),
                            _requested_sections.end());
}

inline void GetPot::__parse_argument_vector(const STRING_VECTOR &ARGV) {
  if (ARGV.size() == 0)
    return;
  ...
  // -- do not parse the first argument, so that it is not interpreted a s a
  // nominus or so.
  argv.push_back(*it);
  it++;
  ...
  for (; it != ARGV.end(); it++, i++) {
    ...
    if (arg.length() > 1 && arg[0] == '[' && arg[arg.length() - 1] == ']') {
      ...
      const std::string Name =
          __DBE_expand_string(arg.substr(1, arg.length() - 2));
      section = __process_section_label(Name, section_stack);
      // new section --> append to list of sections
      if (find(section_list.begin(), section_list.end(), section) ==
          section_list.end())
        if (section.length() != 0)
    ...
    for (; *p; p++) {
      if (*p == '=') {
        ...
        //     arguments carriying variables are always treated as 'requested'
        //     arguments. unrequested variables have to be detected with the
        //     ufo-variable detection routine.
        if (request_recording_f)
          _requested_arguments.push_back(arg);
        ...
        *o = '\0'; // set temporary terminating zero
        __set_variable(arg.c_str(),
                       p); // v-name = c_str() bis 'p', value = rest
...
inline STRING_VECTOR GetPot::__read_in_file(const char *FileName) {
...
inline STRING_VECTOR GetPot::__read_in_stream(std::istream &istr) {
...
inline void GetPot::__skip_whitespace(std::istream &istr)
...
inline const std::string GetPot::__get_next_token(std::istream &istr)
...
inline const std::string GetPot::__get_string(std::istream &istr)
...
inline const std::string GetPot::__get_until_closing_bracket(std::istream &istr)
// parse input until next matching }
{
  std::string str = "";
  int tmp = 0;
...
GetPot::__process_section_label(const std::string &Section,
  ...
  if (section_stack.size() != 0) {
    victorate(std::string, section_stack, it) section += *it + "/";
  }
  return section;
}

// convert string to DOUBLE, if not possible return Default
inline double GetPot::__convert_to_type(const std::string &String,
                                        double Default) const {
  double tmp;
...
inline int GetPot::__convert_to_type(const std::string &String,
                                     int Default) const {
  int tmp;
...
GetPot::__get_remaining_string(const std::string &String,
                               const std::string &Start) const
...
inline bool GetPot::search(const char *Option) {
...
inline bool GetPot::search(unsigned No, const char *P, ...) {
...
inline void GetPot::reset_cursor() {
  search_failed_f = false;
  cursor = 0;
}

inline void GetPot::init_multiple_occurrence() {
  disable_loop();
  reset_cursor();
}
///////////////////////////////////////////////////////////////////////////////
...
//.............................................................................
//
inline const char *GetPot::operator[](unsigned idx) const {
  return idx < argv.size() ? argv[idx].c_str() : nullptr;
}

inline int GetPot::get(unsigned Idx, int Default) const {
  if (Idx >= argv.size())
    return Default;
  return __convert_to_type(argv[Idx], Default);
}

inline double GetPot::get(unsigned Idx, const double &Default) const {
  if (Idx >= argv.size())
    return Default;
  return __convert_to_type(argv[Idx], Default);
}

inline const char *GetPot::get(unsigned Idx, const char *Default) const {
  if (Idx >= argv.size())
    return Default;
  else
...
inline unsigned GetPot::size() const { return argv.size(); }

//     -- next() function group
inline int GetPot::next(int Default) {
  if (search_failed_f)
    return Default;
  cursor++;
  if (cursor >= argv.size()) {
...
inline double GetPot::next(const double &Default) {
  if (search_failed_f)
    return Default;
  cursor++;
...
inline const char *GetPot::next(const char *Default) {
  if (search_failed_f)
    return Default;
  cursor++;
...
inline int GetPot::follow(int Default, const char *Option) {
  // (*) record requested of argument is entirely handled in 'search()' and
  // 'next()'
...
inline double GetPot::follow(const double &Default, const char *Option) {
  // (*) record requested of argument is entirely handled in 'search()' and
  // 'next()'
  if (search(Option) == false)
    return Default;
...
inline const char *GetPot::follow(const char *Default, const char *Option) {
  // (*) record requested of argument is entirely handled in 'search()' and
  // 'next()'
  if (search(Option) == false)
    return Default;
...
inline int GetPot::follow(int Default, unsigned No, const char *P, ...) {
  // (*) record requested of argument is entirely handled in 'search()' and
  // 'next()'
  if (No == 0)
    return Default;
...
inline double GetPot::follow(const double &Default, unsigned No, const char *P,
                             ...) {
  // (*) record requested of argument is entirely handled in 'search()' and
  // 'next()'
  if (No == 0)
...
inline const char *GetPot::follow(const char *Default, unsigned No,
                                  const char *P, ...) {
  // (*) record requested of argument is entirely handled in 'search()' and
  // 'next()'
  if (No == 0)
...
//.............................................................................
//
inline int GetPot::direct_follow(int Default, const char *Option) {
...
inline double GetPot::direct_follow(const double &Default, const char *Option) {
  const char *FollowStr = __match_starting_string(Option);
...
inline const char *GetPot::direct_follow(const char *Default,
                                         const char *Option) {
  if (search_failed_f)
    return Default;
  const char *FollowStr = __match_starting_string(Option);
...
inline const char *GetPot::__match_starting_string(const char *StartString)
...
//.............................................................................
//
inline bool GetPot::options_contain(const char *FlagList) const {
  ...
  for (; it != argv.end(); it++) {
    ...
    if (str.length() >= 2 && str[0] == '-' && str[1] != '-')
      if (__check_flags(str, FlagList))
        return true;
  }
  return false;
}

inline bool GetPot::argument_contains(unsigned Idx,
                                      const char *FlagList) const {
  if (Idx >= argv.size())
    return false;
...
inline bool GetPot::__check_flags(const std::string &Str,
                                  const char *FlagList) const {
...
///////////////////////////////////////////////////////////////////////////////
// (*) nominus arguments
inline STRING_VECTOR GetPot::nominus_vector() const
// return vector of nominus arguments
{
  ...
  for (; it != idx_nominus.end(); it++) {
    ...
    // (*) record for later ufo-detection
    //     when a nominus vector is requested, the entire set of nominus
    //     arguments are tagged as 'requested'
    ((GetPot *)this)->__record_argument_request(argv[*it]);
  }
  return nv;
}

inline const char *GetPot::next_nominus() {
...
inline void GetPot::reset_nominus_cursor() { nominus_cursor = -1; }
///////////////////////////////////////////////////////////////////////////////
// (*) variables
//.............................................................................
//
inline int GetPot::operator()(const char *VarName, int Default) const {
...
inline double GetPot::operator()(const char *VarName,
                                 const double &Default) const {
  // (*) recording of requested variables happens in '__find_variable()'
  const variable *sv = __find_variable(VarName);
  if (sv == nullptr)
    return Default;
...
inline const char *GetPot::operator()(const char *VarName,
...
inline int GetPot::operator()(const char *VarName, int Default,
                              unsigned Idx) const {
  // (*) recording of requested variables happens in '__find_variable()'
  const variable *sv = __find_variable(VarName);
  if (sv == nullptr)
...
inline double GetPot::operator()(const char *VarName, const double &Default,
                                 unsigned Idx) const {
  // (*) recording of requested variables happens in '__find_variable()'
  const variable *sv = __find_variable(VarName);
  if (sv == nullptr)
...
inline const char *GetPot::operator()(const char *VarName, const char *Default,
                                      unsigned Idx) const {
  // (*) recording of requested variables happens in '__find_variable()'
  const variable *sv = __find_variable(VarName);
  if (sv == nullptr)
...
inline void GetPot::__record_argument_request(const std::string &Name) {
  if (!request_recording_f)
    return;
  ...
  victorate(std::string, STree,
            it) if (find(_requested_sections.begin(), _requested_sections.end(),
                         *it) ==
                    _requested_sections.end()) if (section.length() != 0)
      _requested_sections.push_back(*it);
}

inline void GetPot::__record_variable_request(const std::string &Name) {
  if (!request_recording_f)
    return;
...
// (*) following functions are to be used from 'outside', after getpot has
// parsed its
//     arguments => append an argument in the argument vector that reflects the
//     addition
inline void GetPot::__set_variable(const char *VarName, const char *Value) {
  const GetPot::variable *Var = __find_variable(VarName);
  if (Var == nullptr)
    variables.push_back(variable(VarName, Value, _field_separator.c_str()));
  else
    ((GetPot::variable *)Var)->take(Value, _field_separator.c_str());
}

inline void GetPot::set(const char *VarName, const char *Value,
                        const bool Requested /* = yes */) {
  const std::string Arg =
      prefix + std::string(VarName) + std::string("=") + std::string(Value);
  argv.push_back(Arg);
  ...
  if (Requested)
    __record_variable_request(Arg);
}

inline void GetPot::set(const char *VarName, const double &Value,
                        const bool Requested /* = yes */) {
  __set_variable(VarName, __double2string(Value).c_str());
}

inline void GetPot::set(const char *VarName, const int Value,
                        const bool Requested /* = yes */) {
  __set_variable(VarName, __int2string(Value).c_str());
}

inline unsigned GetPot::vector_variable_size(const char *VarName) const {
  const variable *sv = __find_variable(VarName);
  if (sv == nullptr)
    return 0;
...
inline STRING_VECTOR GetPot::get_variable_names() const {
  STRING_VECTOR result;
...
inline STRING_VECTOR GetPot::get_section_names() const { return section_list; }
...
//
//           "  ${Number}.a ${: Das Marmorbild} AB-${& Author=
//           ${Eichendorf}-1870}"
//
...
//---------------------------------------------------------------------------
inline std::string GetPot::__DBE_expand_string(const std::string str) {
...
inline STRING_VECTOR GetPot::__DBE_get_expr_list(const std::string str_,
                                                 const unsigned ExpectedNumber)
...
inline const GetPot::variable *GetPot::__DBE_get_variable(std::string VarName) {
...
inline std::string GetPot::__DBE_expand(const std::string expr) {
  // ${: } pure text
  if (expr[0] == ':')
  ...
  // ${==  } ${<=  } ${>= } comparisons (return the number of the first 'match'
    ...
    STRING_VECTOR a;
    if (op == GT || op == LT)
      a = __DBE_get_expr_list(expr.substr(1), 2);
    else
      a = __DBE_get_expr_list(expr.substr(2), 2);
    ...
    for (y_orig++; y_orig != a.end(); y_orig++) {
      ...
      if (x == 1e37 || y == 1e37) {
        ...
        if ((op == EQ && x_orig == *y_orig) ||
            (op == GEQ && x_orig >= *y_orig) ||
            (op == LEQ && x_orig <= *y_orig) ||
            (op == GT && x_orig > *y_orig) || (op == LT && x_orig < *y_orig))
          return __int2string(i);
      } else {
        // it's a number comparison
        if ((op == EQ && x == y) || (op == GEQ && x >= y) ||
            (op == LEQ && x <= y) || (op == GT && x > y) || (op == LT && x < y))
          return __int2string(i);
      }
      i++;
    }
...
//.............................................................................
//
inline bool GetPot::__search_string_vector(const STRING_VECTOR &VecStr,
                                           const std::string &Str) const {
...
inline STRING_VECTOR GetPot::unidentified_arguments(unsigned Number,
                                                    const char *KnownArgument1,
                                                    ...) const {
...
inline STRING_VECTOR GetPot::unidentified_arguments() const {
...
inline STRING_VECTOR GetPot::unidentified_options(unsigned Number,
                                                  const char *KnownOption1,
                                                  ...) const {
...
inline STRING_VECTOR GetPot::unidentified_options() const {
  ...
  //    THEN they were requested as 'follow' and 'next' arguments and not as
  //    real options.
  //
...
inline std::string GetPot::unidentified_flags(const char *KnownFlagList,
                                              int ArgumentNumber = -1) const
// Two modes:
...
inline STRING_VECTOR GetPot::unidentified_variables(unsigned Number,
                                                    const char *KnownVariable1,
                                                    ...) const {
...
inline STRING_VECTOR GetPot::unidentified_variables() const {
  return unidentified_variables(_requested_variables);
}

inline STRING_VECTOR GetPot::unidentified_sections(unsigned Number,
                                                   const char *KnownSection1,
                                                   ...) const {
...
inline STRING_VECTOR GetPot::unidentified_sections() const {
...
inline STRING_VECTOR GetPot::unidentified_nominuses() const {
...
inline GetPot::variable::variable(const variable &That) {
#ifdef WIN32
  operator=(That);
#else
...
inline GetPot::variable::variable(const char *Name, const char *Value,
...
inline void GetPot::variable::take(const char *Value,
                                   const char *FieldSeparator) {
  ...
  while (follow_token != nullptr) {
    value.push_back(std::string(follow_token));
    follow_token = strtok_r(nullptr, FieldSeparator, &spt);
  }
