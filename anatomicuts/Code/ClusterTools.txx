...
template <class TColorMesh, class TImage, class THistogramMesh>
std::vector<itk::Vector<float>> *
ClusterTools<TColorMesh, TImage, THistogramMesh>::GetDirections(
    DirectionsType dir) {
  std::vector<itk::Vector<float>> *direcciones =
      new std::vector<itk::Vector<float>>();
  ...
  for (int i = 0; i < 3; i++) {
    for (int k = 0; k < 3; k++) {
      for (int j = 0; j < 3; j++) {
        ...
        if (k == 0)
          howManyZeros++;
        if ((dir == DirectionsType::ALL && howManyZeros != 3) ||
            (DirectionsType::STRAIGHT == dir && howManyZeros == 2) ||
            (DirectionsType::DIAGONAL == dir && howManyZeros >= 1 &&
             howManyZeros != 3)) {
...
template <class TColorMesh, class TImage, class THistogramMesh>
std::vector<typename THistogramMesh::Pointer> *
ClusterTools<TColorMesh, TImage, THistogramMesh>::ColorMeshToHistogramMesh(
    std::vector<typename TColorMesh::Pointer> basicMeshes,
    typename TImage::Pointer segmentation, bool removeInterHemispheric) {
  if (ct == NULL) {
    FSENV *fsenv = FSENVgetenv();
    char   tmpstr[2000];
    sprintf(tmpstr, "%s/FreeSurferColorLUT.txt", fsenv->FREESURFER_HOME);
  ...
  std::vector<typename HistogramMeshType::Pointer> *meshes =
      new std::vector<typename HistogramMeshType::Pointer>();
  for (unsigned int i = 0; i < basicMeshes.size(); i++) {
    ...
    typedef
        typename HistogramMeshType::PointDataContainer PointDataContainerType;
    ...
    for (CellIterator cellIt = basicMesh->GetCells()->Begin();
      ...
      typename PointDataContainerType::Pointer dataContainer =
      ...
      if (right == 0 || left == 0 || !removeInterHemispheric) {
        for (pointIdIt = cellIt.Value()->PointIdsBegin();
          ...
          line.TakeOwnership(
              new itk::PolylineCell<typename HistogramMeshType::CellType>);
...
template <class TColorMesh, class TImage, class THistogramMesh>
void ClusterTools<TColorMesh, TImage, THistogramMesh>::SetDirectionalNeighbors(
    std::vector<typename HistogramMeshType::Pointer> *meshes,
    typename ImageType::Pointer                       segmentation,
      ...
      if (segmentation->TransformPhysicalPointToIndex(pt1, index)) {
        ...
        for (unsigned int k = 0; k < direcciones.size(); k++) {
          ...
          while (vecino == labelOrig) {
            ...
            if (!segmentation->GetLargestPossibleRegion().IsInside(
                    roundedIndex))
              break;
        ...
        std::cout
            << "ClusterTools::SetDirectionalNeighbors: Point outside image. "
            << index << std::endl;
      }
    }
  }
...
template <class TColorMesh, class TImage, class THistogramMesh>
void ClusterTools<TColorMesh, TImage, THistogramMesh>::GetPolyDatas(
...
template <class TColorMesh, class TImage, class THistogramMesh>
void ClusterTools<TColorMesh, TImage, THistogramMesh>::SaveMesh(
    typename TColorMesh::Pointer mesh, typename TImage::Pointer image,
    std::string outputName, std::string refFiber) {
...
template <class TColorMesh, class TImage, class THistogramMesh>
std::vector<typename TColorMesh::Pointer> *
ClusterTools<TColorMesh, TImage, THistogramMesh>::PolydataToMesh(
    std::vector<vtkSmartPointer<vtkPolyData>> polydatas) {
  std::vector<typename ColorMeshType::Pointer> *meshes =
      new std::vector<typename ColorMeshType::Pointer>();
...
template <class TColorMesh, class TImage, class THistogramMesh>
std::vector<typename TColorMesh::Pointer> *
ClusterTools<TColorMesh, TImage, THistogramMesh>::FixSampleClusters(
    std::vector<vtkSmartPointer<vtkPolyData>> polydatas, int numberOfPoints) {
...
int ClusterTools<TColorMesh, TImage, THistogramMesh>::GetAverageStreamline(
  ...
  typename ColorMeshType::CellsContainer::ConstIterator cells =
      mesh->GetCells()->Begin();
  std::vector<typename ColorMeshType::PointType> avgPoints(
      cells.Value()->GetNumberOfPoints(), 0);
    ...
    for (pointIdIt = cells.Value()->PointIdsBegin();
      ...
      for (int k = 0; k < 3; k++) {
        if (dist < dist_inv)
          ...
          avgPoints[avgPoints.size() - j - 1][k] +=
              pt[k] / mesh->GetNumberOfCells();
      }
    }
  }
...
template <class TColorMesh, class TImage, class THistogramMesh>
float ClusterTools<TColorMesh, TImage, THistogramMesh>::GetStandardDeviation(
    typename THistogramMesh::Pointer mesh, int clusterMean) {
...
template <class TColorMesh, class TImage, class THistogramMesh>
float ClusterTools<TColorMesh, TImage, THistogramMesh>::GetDistance(
    typename THistogramMesh::Pointer mesh, int clusterMean, int cellId) {
  typename MembershipFunctionType::Pointer function =
      MembershipFunctionType::New();
  function->SetMeanEuclidean(true);
  // function->SetLabels(true);
