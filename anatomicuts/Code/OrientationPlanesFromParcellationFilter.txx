...
void OrientationPlanesFromParcellationFilter<TInputImage,
  ...
  radius.Fill(1);
  itk::ConstNeighborhoodIterator<InputImageType> iter(
  ...
  for (; !iter.IsAtEnd(); ++iter) {
    ...
    if ((pixel == 251 && !m_baby) || (pixel == 3026 && m_baby) ||
        (pixel == 4026 && m_baby)) {
      pointsCC1->InsertNextPoint(point[0], point[1], point[2]);
    }
    if ((pixel == 255 && !m_baby) || (pixel == 3010 && m_baby) ||
        (pixel == 4010 && m_baby)) {
    ...
    if (pixel == 252 || pixel == 254 || (pixel == 3026 && m_baby) ||
        (pixel == 4026 && m_baby) || (pixel == 3010 && m_baby) ||
        (pixel == 4010 && m_baby)) // || pixel == 255 || pixel == 251)
    {
...
*/
  ...
  double weightSum = 0;
  for (vtkIdType pointId = 0; pointId < pointsCC_3V->GetNumberOfPoints();
       pointId++) {
    double x[3];
    double xp[3];
  ...
  vtkSmartPointer<vtkPlaneSource> axial =
  ...
  itk::ConstNeighborhoodIterator<InputImageType> iter2(
      radius, this->GetInput(), this->GetInput()->GetLargestPossibleRegion());
  ...
  m_FrontBack.SetVnlVector(
      vnl_cross_3d(m_LeftRight.GetVnlVector(), m_UpDown.GetVnlVector()));
...
template <class TInputImage, class TOutputImage>
void OrientationPlanesFromParcellationFilter<
    TInputImage, TOutputImage>::GeneratePlanes(MRI *image) {
  ...
  for (int i = 0; i < image->width; i++) {
    for (int j = 0; j < image->height; j++) {
      for (int k = 0; k < image->depth; k++) {
        ...
        int pixel = MRIgetVoxVal(image, i, j, k, 0);
...
template <class TInputImage, class TOutputImage>
float OrientationPlanesFromParcellationFilter<
    TInputImage, TOutputImage>::DistanceToMidline(double x, double y,
                                                  double z) {
  typename InputImageType::PointType point;
  point[0] = x;
  point[1] = y;
  ...
  return point[0] - proj[0] + point[1] - proj[1] + point[2] - proj[2];
}
