...
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::SetOutputSize(
    const SizeType &size) {
...
template <class TInputMesh, class TOutputImage>
const typename MeshToImageFilter<TInputMesh, TOutputImage>::SizeType &
MeshToImageFilter<TInputMesh, TOutputImage>::GetOutputSize() {
  return this->m_OutputRegion.GetSize();
}

// Set the output image index.
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::SetOutputIndex(
...
template <class TInputMesh, class TOutputImage>
const typename MeshToImageFilter<TInputMesh, TOutputImage>::IndexType &
MeshToImageFilter<TInputMesh, TOutputImage>::GetOutputIndex() {
  return this->m_OutputRegion.GetIndex();
}

// Set the output image spacing.
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::SetOutputSpacing(
...
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::SetOutputOrigin(
    const double *origin) {
  PointType p(origin);
...
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::SetOutputParametersFromImage(
...
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::GenerateOutputInformation(
    void) {
...
template <class TInputMesh, class TOutputImage>
void MeshToImageFilter<TInputMesh, TOutputImage>::GenerateData() {
  ...
  //			 max=index;
  for (typename MeshType::PointsContainer::Iterator it =
           this->GetInput()->GetPoints()->Begin();
       it != this->GetInput()->GetPoints()->End(); it++) {
  ...
  std::cout << " ouside pixels " << outsidePixels << "min " << min << " max "
            << max << std::endl;
...
float MeshToImageFilter<TInputMesh, TOutputImage>::BinaryImageOfLabels(
  ...
  for (typename MeshType::CellsContainer::ConstIterator cells =
    ...
    if (label == -1 || cellData.Value() == label) {
      for (typename MeshType::CellTraits::PointIdIterator pointIdIt =
               cells.Value()->PointIdsBegin();
           pointIdIt != cells.Value()->PointIdsEnd(); pointIdIt++) {
