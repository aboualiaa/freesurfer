...
double LabelsEntropyAndIntersectionMembershipFunction<TVector>::Evaluate(
  ...
  if (this->m_labelsAndEuclid) {
    ...
    affinity = intersection * adhocEntropy.size() /
    ...
    for (int i = 0; i < labels1_->size(); i++) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        euclid_inv += pow(
            (*m1)[i * 3 + k] - (*m2)[labels2_->size() * 3 - i * 3 + k - 3], 2);
      }
    ...
    for (int i = 0; i < labels1.size(); i++) {
      ...
      for (typename LabelsMapType::const_iterator it = labels2[i].cbegin();
           it != labels2[i].cend(); it++) {
    ...
    for (int i = 0; i < labels1->size(); i++) {
      ...
      for (int k = 0; k < 3; k++) {
        euclid += pow((*m1)[i * 3 + k] - (*m2)[i * 3 + k], 2);
        euclid_inv += pow(
            (*m1)[i * 3 + k] - (*m2)[labels2->size() * 3 - i * 3 + k - 3], 2);
      }
      dist += sqrt(euclid);
