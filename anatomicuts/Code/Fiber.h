...
public:
  ...
  void SetPointList(const FiberPointListType &l) { m_FiberPointList = l; }

  /** get the list of points */
  FiberPointListType GetPointList(void) const { return m_FiberPointList; }
  ...
  unsigned int GetNumberOfPoints(void) const { return m_FiberPointList.size(); }

  /** Empties the point list */
  void Clear(void) { m_FiberPointList.clear(); }

  /** Integrates the FA along the fiber */
  void GetFAStatistics(double &mean, double &min, double &max,
                       double &var) const;

  /** Integrates the ADC along the fiber */
  void GetADCStatistics(double &mean, double &min, double &max,
                        double &var) const;

  enum StatisticsType { FA, ADC };

  void GetStatistics(StatisticsType type, double &mean, double &min,
                     double &max, double &var) const;

  Fiber(){};
  ~Fiber(){};
  Fiber(const Self &f) { m_FiberPointList = f.GetPointList(); }
...
std::ostream &operator<<(std::ostream &os, const Fiber<T, NDimension> &f) {
  for (unsigned int i = 0; i < f.GetNumberOfPoints(); i++)
    os << f.GetPointList()[i].Point << " " << f.GetPointList()[i].Tensor
       << std::endl;
