...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -s segmentationFile -f fiber.vtk -c #clusters -n #points  "
                 "-e #fibers for eigen  -o outputFolder -d [s:straight "
                 "d:diagonal a:all o:none] "
              << std::endl;
    return -1;
  }
    ...
    for (int i = 0; i < 3; i++) {
      for (int k = 0; k < 3; k++) {
        for (int j = 0; j < 3; j++) {
          ...
          if (k == 0)
            howManyZeros++;
          if ((neighbors[0] == 'a' && howManyZeros != 3) ||
              (neighbors[0] == 's' && howManyZeros == 2) ||
              (neighbors[0] == 'd' && howManyZeros >= 1 &&
               howManyZeros !=
                   3)) // ||neighbors[0]== '1' && direcciones.size()==0  )
          {
    ...
    //Copy meshes
    {
      ...
      while (leftWrongPixels > 0 && iteration < 10) {
        ...
        for (; !iter.IsAtEnd(); ++iter) {
          if (iter.GetCenterPixel() == 0 || iter.GetCenterPixel() == 5001 ||
              iter.GetCenterPixel() == 5002 || iter.GetCenterPixel() == 41 ||
              iter.GetCenterPixel() == 2) {
            int newPixel = -1;
      ...
      if (std::string(fiberFile).find(std::string(".trk")) !=
          std::string::npos) {
        std::cout << "reading trk : " << fiberFile << " "
                  << std::string(fiberFile).find(".trk") << std::endl;
      ...
      for (CellIterator cellIt = basicMesh->GetCells()->Begin();
        ...
        if (cl.search(1, "-meanAndCovInvert") ||
            cl.search(1, "-meanAndCovGaussian")) {
          for (int yy = 0; yy < 3; yy++)
    ...
    for (int i = 0; i < numMem; i++) {
      if (cl.search(1, "-labelsPTPNN") || cl.search(1, "-labelsPTP")) {
        ...
        if (cl.search(1, "-labelsPTP"))
          function2->SetLabelsCount(direcciones.size() +
                                    1); // all directions plus the label itself
        else
          function2->SetLabelsCount(1);
        MembershipFunctionBaseType::Pointer function =
            static_cast<MembershipFunctionBaseType::Pointer>(
        ...
        function2->SetLabels(cl.search(1, "-labels2") ||
                             cl.search(1, "-labels"));
        ...
        function2->SetMeanClosestPointInvert(
            cl.search(1, "-meanClosestPointInvert"));
        function2->SetMeanClosestPointGaussian(
            cl.search(1, "-meanClosestPointGaussian"));
  ...
  i = 0;
  for (std::map<std::string, MeshBasicType::Pointer>::iterator it =
           newMeshes.begin();
       it != newMeshes.end(); ++it) {
  ...
  for (std::vector<std::pair<std::string, std::string>>::iterator it =
           clusterIdHierarchy.begin();
       it != clusterIdHierarchy.end(); ++it)
