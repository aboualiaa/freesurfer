  ...
  for (unsigned int i = 0; i < meshes.size(); i++) {
    ...
    for (; pointId != pointIdEnd; pointId++) {
      ...
      if (segmentation->TransformPhysicalPointToIndex(pt1, index)) {
        ...
        std::cout << pointId << " " << pointIdEnd << " " << pt1 << " " << index
                  << std::endl;
      }
    }
...
std::vector<int> GetCentroidIndices(std::vector<MeshType::Pointer> meshes) {
  ...
  for (unsigned int i = 0; i < meshes.size(); i++) {
    std::vector<MeshType::PointType> avgPoints(
        meshes[i]->GetCells()->Begin().Value()->GetNumberOfPoints(), 0);
    MeshType::CellsContainer::ConstIterator cells =
        meshes[i]->GetCells()->Begin();
    int cell_i = 0;
        ...
        for (unsigned int k = 0; k < 3; k++) {
          if (dist < dist_inv)
            ...
            avgPoints[avgPoints.size() - j - 1][k] +=
                pt[k] / meshes[i]->GetNumberOfCells();
        }
      }
    }
...
std::vector<BasicMeshType::Pointer>
    ...
    if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
      ...
      std::cout << arg[0]
                << " -s1 parcellation1 -s2 parcellation2 -c numClusters -h1 "
                   ...
                   "Eucildean) -bb -sym interHemiRatioClusterRemoval -o output"
                << std::endl;
      return -1;
    }
