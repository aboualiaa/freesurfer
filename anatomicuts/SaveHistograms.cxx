...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -p parcellation -f numberOfBundles <list of vtk bundles> -o "
                 "output.csv -bb"
              << std::endl;
    return -1;
  }
    ...
    for (int k = 0; k < 3; k++) {
      for (int j = 0; j < 3; j++) {
        ...
        for (int w = 0; w < 3; w++) {
          ...
          dir[w] = possibles[i] * orientations[0][w] +
                   possibles[j] * orientations[1][w] +
                   possibles[k] * orientations[2][w];
        }
        dir.Normalize();
        int howManyZeros = 0;
        if (i == 0)
  ...
  while (numFiles) {
    ...
    for (unsigned int i = 0; i < indeces.size(); i++) {
      csv_file << fiberFile << ",D:" << indeces[i][0] << ":" << indeces[i][1]
               << ":" << indeces[i][2] << ",";
      for (std::map<int, int>::iterator it = histograms[i].begin();
           it != histograms[i].end(); ++it) {
