    ...
    for (; inputCellIt != input->GetCells()->End(); ++inputCellIt, ++counter) {
      ...
      if (FA_FOUND) {
        ...
        for (int p = 0; p < volumes.size(); p++) {
          ...
          float val = (firstPt[0] - lastPt[0]) * (firstPt[1] - lastPt[1]) *
                      (firstPt[2] - lastPt[2]);
          ...
          for (; it != inputCellIt.Value()->PointIdsEnd(); it++) {
            ...
            if (volumes.at(p)->TransformPhysicalPointToIndex(lastPt, index)) {
              ...
              if (val > 0)
                ...
                allFA[p].insert(allFA[p].begin(),
                                volumes.at(p)->GetPixel(index));
            }
          }
      ...
      if (ID1 >= 0) {
        if (ID1 == Left_ID1) {
          CTABfindAnnotation(surfCL->ct, surfCL->vertices[ID1].annotation,
                             &structure);
          std::cout << "L" << ID1 << " " << surfCL->vertices[ID1].annotation
                    << " " << structure << std::endl;
        } else {
          CTABfindAnnotation(surfCR->ct, surfCR->vertices[ID1].annotation,
                             &structure);
          std::cout << "R" << ID1 << " " << surfCR->vertices[ID1].annotation
                    << " " << structure << std::endl;
        }
        oFile << structure << ",";
      } else {
      ...
      if (ID2 >= 0) {
        if (ID2 == Left_ID2) {
          CTABfindAnnotation(surfCL->ct, surfCL->vertices[ID2].annotation,
                             &structure);
          std::cout << "L" << ID2 << " " << surfCL->vertices[ID2].annotation
                    << " " << structure << std::endl;
        } else {
          CTABfindAnnotation(surfCR->ct, surfCR->vertices[ID2].annotation,
                             &structure);
          std::cout << "R" << ID2 << " " << surfCR->vertices[ID2].annotation
                    << " " << structure << std::endl;
        }
