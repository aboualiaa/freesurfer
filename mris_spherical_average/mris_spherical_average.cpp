...
#define FIRST_SUBJECT 5
  for (nsubjects = 0, i = FIRST_SUBJECT; i < argc - 1; i++, nsubjects++) {
    ...
    if (which == VERTEX_COORDS) {
      ...
      if (MRISreadVertexPositions(mris, orig_name) != NO_ERROR) {
        ErrorExit(ERROR_BADPARM, "could not read surface positions from %s",
                  orig_name);
      }
    ...
    if (i == FIRST_SUBJECT) /* scale the icosahedron up */
    {
      ...
      mean = MRIScomputeVertexSpacingStats(mris_avg, &sigma, NULL, &max_len,
                                           NULL, NULL, CURRENT_VERTICES);
      if (max_len > mean + 3 * sigma) {
        max_len = mean + 3 * sigma;
      }
      mht = MHTcreateVertexTable_Resolution(mris_avg, CURRENT_VERTICES,
                                            2 * max_len);
    }
      ...
      if (mask_name) {
        ...
        if (!area)
          ErrorPrintf(ERROR_BADFILE,
                      "%s: could not read label file %s for %s (%s).\n",
  ...
  if (output_subject_name) {
    sprintf(fname, "%s/%s/%s/%s.%s", osdir, output_subject_name, surf_dir,
            ohemi, osurf);
  ...
  if (stat_flag) /* write out summary statistics files */
  {
    ...
    case VERTEX_LABEL:
      ...
      if (compute_average_label_area) {
        ...
        printf("threshold that best approximates average area of %2.2f mm^2 is "
               "%2.2f (%2.2f mm^2)\n",
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help")) {
    ...
    segment = 1;
    printf(
        "segmenting input labels and only using largest connected component\n");
...
static void print_help(void) {
  ...
  fprintf(stderr, "-n              normalize output so it can be interpreted "
                  "as a probability\n");
  fprintf(
      stderr,
      "-orig  <name>   use <name> as original surface position default=orig\n");
  fprintf(stderr,
          "-o  <output subject name>   use <output subject> as the space to "
          "write the results in instead of the last subject given\n");
  fprintf(stderr, "-osdir  <output subject dir>   use <output subject dir> as "
                  "the subjects dir for the output subject'\n");
  fprintf(stderr,
          "-sdir  <subjects dir>   use <subject dir> as the subjects dir'\n");
  fprintf(stderr, "-average_area   compute threshold for label that will give "
                  "the average label apporximately the average surface area\n");
