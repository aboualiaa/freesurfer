...
int main(int argc, char *argv[]) {
  ...
  mri = MRISribbon(inner_mris, outer_mris, mri_src, nullptr);
...
MRI *MRIcropVolumeToLabel(MRI *mri_src, MRI *mri_dst, LABEL *area,
  ...
  MRISclearMarks(mris_white);
  MRISclearMarks(mris_pial);
  LabelMarkSurface(area, mris_white);
  LabelMarkSurface(area, mris_pial);
    ...
    for (y = 0; y < mri_src->height; y++) {
      for (z = 0; z < mri_src->depth; z++) {
        ...
        if (MRIgetVoxVal(mri_src, x, y, z, 0) > 0) {
          ...
          v_white =
              MHTfindClosestVertexInTable(mht_white, mris_white, xs, ys, zs, 1);
          v_pial =
              MHTfindClosestVertexInTable(mht_pial, mris_pial, xs, ys, zs, 1);
