...
static char vcid[] =
    "$Id: mris_BA_segment.c,v 1.4 2015/02/05 23:34:40 zkaufman Exp $";
...
int          main(int argc, char *argv[]) {
  ...
  if (!mri)
    ErrorExit(ERROR_NOFILE, "%s: could not read profiles from %s", Progname,
              profile_name);
  make_profiles_zero_baseline(mri);

  lprior = LabelRead(nullptr, prior_name);
  if (lprior == nullptr)
    ErrorExit(ERROR_NOFILE, "%s: could not read prior label from %s", Progname,
              prior_name);
...
static LABEL *segment_MT(MRI_SURFACE *mris, MRI *mri_profiles, LABEL *lprior,
  ...
  if (vertices == nullptr)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %d vertex array",
              Progname, nvertices);
  for (vno = nvertices = 0; vno < mris->nvertices; vno++)
    if (mris->vertices[vno].val > 0.01)
...
static double compute_MT_log_likelihood(MRI_SURFACE *mris, MRI *mri_profiles,
  ...
  if (u_interior == nullptr) {
    ...
    if (u_interior == nullptr || v_interior == nullptr || v_exterior == nullptr)
      ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %d elt vector",
                Progname, mri_profiles->nframes);
  } else {
