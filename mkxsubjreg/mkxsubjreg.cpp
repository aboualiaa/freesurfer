...
int main(int argc, char **argv) {
  ...
  err = regio_read_register(srcregpath, &srcsubj, &inplaneres, &betplaneres,
  ...
  Xsrc = DevolveXFM(srcsubj, nullptr, xfmrname);
  if (Xsrc == nullptr)
    exit(1);
  invXsrc = MatrixInverse(Xsrc, nullptr);

  /* Load the target subject xfm */
  Xtarg = DevolveXFM(targsubj, nullptr, xfmrname);
  if (Xtarg == nullptr)
    exit(1);
  ...
  err = regio_write_register(targregpath, targsubj, inplaneres, betplaneres,
                             intensity, Rtarg, float2int);
...
static int parse_commandline(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mkxsubjreg.c,v 1.5 2011/03/02 00:04:13 nicks Exp $",
      "$Name:  $");
