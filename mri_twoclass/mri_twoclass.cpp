...
#define ARGV_OFFSET 4
  ...
  if (read_dir) /* read in precomputed data */
  {
    ...
    for (n = 0; n < num_class1 + num_class2; n++) {
      ...
      if ((stricmp(xform_fname, "null") == 0) ||
          (stricmp(xform_fname, "identity") == 0)) {
#if 0
      ...
      if (!wm_flag && !vol_flag) {
        if (!width) {
          ...
          if (!vli1 || !vli2)
            ErrorExit(ERROR_NOMEMORY, "%s: could not allocate VL ***",
                      Progname);
      ...
      } else /* compute white matter density statistics */
      {
        ...
        if (!mri_mean1 || !mri_mean2 || !mri_var1 || !mri_var2 || !mri_atlas_wm)
          ErrorExit(ERROR_NOMEMORY,
                    "%s: could not allocate %dx%dx%d summary volumes", Progname,
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    vol_flag = 1;
    fprintf(stderr,
            "generating maps of volumetric (non-labeled) differences...\n");
      ...
      for (z = 0; z < depth; z++) {
        ...
        if (index < 0) /* allocate space for it */
        {
          if (vl->nlabels > 0) {
            ...
            if (!vl->labels)
              ErrorExit(ERROR_NOMEMORY,
                        "%s: could not reallocate %d labels "
                        "at %d,%d,%d",
                        Progname, vl->nlabels + 1, xv, yv, zv);
            memmove(vl->labels, old_labels,
                    vl->nlabels * sizeof(unsigned char));
            memmove(vl->counts, old_counts,
                    vl->nlabels * sizeof(unsigned short));
            index = vl->nlabels++;
            free(old_labels);
            free(old_counts);
          } else {
...
static int voxel_to_node_float(MRI *mri, float resolution, int xv, int yv,
  ...
  TransformSample(transform, xv * mri->xsize, yv * mri->ysize, zv * mri->zsize,
                  &xt, &yt, &zt);
...
static int node_to_voxel(MRI *mri, float resolution, int xn, int yn, int zn,
  ...
  TransformSampleInverse(transform, xn * resolution, yn * resolution,
                         zn * resolution, &xt, &yt, &zt);
...
static int find_label_index(VL ***voxel_labels, int x, int y, int z,
                            int label) {
  int index;
  VL *vl;
...
static MRI *compute_voxel_statistics(VL ***voxel_labels_class1,
                                     ...
                                     int height, int depth, float resolution,
                                     int nclass1, int nclass2, MRI *mri_stats) {
  int x, y, z, label_counts_c1[MAX_CMA_LABELS], label_counts_c2[MAX_CMA_LABELS],
      index, dof;
  int xc, yc, zc, xp, yp, zp;
  VOXEL_LABELS *vl;
  double chisq, pi, xi1, xi2, numer, denom, n1, n2, ntotal, p, max_chisq, min_p;
  ...
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
...
#endif
        voxel_to_node_float(mri, resolution, x, y, z, &xvf, &yvf, &zvf,
                            transform);
        ...
        MRIFvox(mri_atlas_wm, xm, ym, zm) =
            xpd * ypd * zpd * label + MRIFvox(mri_atlas_wm, xm, ym, zm);
        MRIFvox(mri_atlas_wm, xm, ym, zp) =
            xpd * ypd * zmd * label + MRIFvox(mri_atlas_wm, xm, ym, zp);
        MRIFvox(mri_atlas_wm, xm, yp, zm) =
            xpd * ymd * zpd * label + MRIFvox(mri_atlas_wm, xm, yp, zm);
        MRIFvox(mri_atlas_wm, xm, yp, zp) =
            xpd * ymd * zmd * label + MRIFvox(mri_atlas_wm, xm, yp, zp);
        MRIFvox(mri_atlas_wm, xp, ym, zm) =
            xmd * ypd * zpd * label + MRIFvox(mri_atlas_wm, xp, ym, zm);
        MRIFvox(mri_atlas_wm, xp, ym, zp) =
            xmd * ypd * zmd * label + MRIFvox(mri_atlas_wm, xp, ym, zp);
        MRIFvox(mri_atlas_wm, xp, yp, zm) =
            xmd * ymd * zpd * label + MRIFvox(mri_atlas_wm, xp, yp, zm);
        MRIFvox(mri_atlas_wm, xp, yp, zp) =
            xmd * ymd * zmd * label + MRIFvox(mri_atlas_wm, xp, yp, zp);
...
static int accumulate_white_matter_density(MRI *mri_atlas_wm, MRI *mri_mean,
                                           MRI *mri_var) {
  ...
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        var = MRIFvox(mri_var, x, y, z) =
            MRIFvox(mri_var, x, y, z) / n - mean * mean;
      }
    }
  }
    ...
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
          ...
          if (num_class1 == 1 || num_class2 == 1) {
            ...
            */
            denom = sqrt((dof_out - 1) *
                         ((var1 / num_class1) + (var2 / num_class2)));
          }
          t = numer / denom;
        ...
        if (t > 0)
          p *= -1;
        if (x == Gxn && y == Gyn && z == Gzn)
          printf("node(%d, %d, %d): t = (%2.1f - %2.1f) / ((%2.1f/%d) + "
                 "(%2.1f/%d)) = %2.3f, -logp = %2.1f\n",
                 x, y, z, mean1, mean2, var1, num_class1, var2, num_class2, t,
                 p);
        switch (stat_type) {
        case STAT_T:
        ...
        default:
          ErrorExit(ERROR_UNSUPPORTED,
                    "compute_white_matter_statistics: unsupported stat type %d",
                    stat_type);
        }
      }
    }
