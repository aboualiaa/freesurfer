...
class TestMRISOBBTree : public CppUnit::TestFixture {
  ...
  CPPUNIT_TEST(
      TestComputeOBB); // tests whether a single OBB Node is computed correctly
...
public:
  ...
  ~TestMRISOBBTree() { delete OBBTree; }
  ...
  void TestBuildTree() {
    std::cerr << "TestBuildTree (tests the # of OBBNodes created for a 12 "
                 "level tree). "
              << std::endl;
  ...
  void TestPointInclusion() {
    std::cerr << "TestPointInclusion ( tests a bunch of points whether they "
                 "are inside the mesh)"
              << std::endl;
    OBBTree->ConstructTree();
    CPPUNIT_ASSERT_EQUAL(OBBTree->PointInclusionTest(152, 106, 128),
                         1); // defn inside
    CPPUNIT_ASSERT_EQUAL(OBBTree->PointInclusionTest(179, 117, 128),
                         1); // borderline inside

    CPPUNIT_ASSERT_EQUAL(OBBTree->PointInclusionTest(177, 117, 128),
                         -1); // borderline outside
    CPPUNIT_ASSERT_EQUAL(OBBTree->PointInclusionTest(0, 0, 0),
                         -1); // definitely outside
    CPPUNIT_ASSERT_EQUAL(OBBTree->PointInclusionTest(300, 300, 300),
                         -1); // embarassingly outside
  }
