...
public:
  ...
  int GetArgs() const { return m_iPars; }

  std::string GetHelp() const {
    char chBuf[10];
    sprintf(chBuf, "%d", m_iPars);
    std::string strBuf = std::string("--") + m_strOption +
                         "\t pars = " + chBuf + "\t" + m_strHelp;
    return strBuf;
  }
...
public:
  ...
  std::string GetHelp() const { return m_strHelp; }

  void outStatus() const { std::cout << *m_pStrItem << "\n"; }
};
...
public:
  CCmdLineInterface(char *i_strProgName) { m_strProgName = i_strProgName; }
  ...
  void AddOptionInt(const char *i_cstrName, int *piVal,
                    const char *help = nullptr);
  void AddOptionInt(const char *i_cstrName, int *piVal1, int *piVal2,
                    const char *help = nullptr);
  void AddOptionFloat(const char *i_cstrName, float *pfVal,
                      char *help = nullptr);
  void AddOptionFloat(const char *i_cstrName, float *pfVal1, float *pfVal2,
                      char *help = nullptr);
  void AddOptionString(const char *i_cstrName, std::string *pstrVal,
                       const char *help = nullptr);
  void AddOptionString(const char *i_cstrName, std::string *pstrVal_1,
                       std::string *pstrVal_2, const char *help = nullptr);
  // a bool option is basically just a flag
  // no parameters at all
  void AddOptionBool(const char *i_cstrName, bool *pbVal,
                     const char *help = nullptr);
  void AddIoItem(std::string *pStrItem, const char *help = nullptr) {
