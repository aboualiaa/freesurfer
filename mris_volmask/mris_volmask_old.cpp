...
public:
  IoError(const std::string &what) : m_what(what) {}
...
std::string LoadInputFiles(const IoParams &params, MRI *&mriInput,
                           MRIS *&surfLeftWhite, MRIS *&surfLeftPial,
                           MRIS *&surfRightWhite,
...
MRI *CombineMasks(MRI *maskOne, MRI *maskTwo,
                  const unsigned char lblBackground);

//  return a binary mask
MRI *FilterLabel(MRI *initialMask, const unsigned char lbl);
...
int main(int ac, char *av[]) {
  ...
  try {
    outputPath = LoadInputFiles(params, mriTemplate, surfLeftWhite,
                                surfLeftPial, surfRightWhite, surfRightPial);
  ...
  if (params.bEditAseg) {
    ...
    insert_ribbon_into_aseg(mriTemplate, mriTemplate, surfLeftWhite,
                            surfLeftPial, LEFT_HEMISPHERE);
    printf("inserting RH into aseg...\n");
    insert_ribbon_into_aseg(mriTemplate, mriTemplate, surfRightWhite,
                            surfRightPial, RIGHT_HEMISPHERE);
...
#ifndef NO_VTK
  ComputeSurfaceDistanceFunction(surfLeftWhite, params.capValue, dLeftWhite);
#else
...
#ifndef NO_VTK
  ComputeSurfaceDistanceFunction(surfLeftPial, params.capValue, dLeftPial);
#else
...
#ifndef NO_VTK
  ComputeSurfaceDistanceFunction(surfRightWhite, params.capValue, dRightWhite);
#else
...
#ifndef NO_VTK
  ComputeSurfaceDistanceFunction(surfRightPial, params.capValue, dRightPial);
#else
  ...
  */
  MRI *finalMask =
      CombineMasks(maskLeftHemi, maskRightHemi, params.labelBackground);
  ...
  */
  if (params.bSaveRibbon) {
    ...
    MRI *ribbon = FilterLabel(maskLeftHemi, params.labelLeftRibbon);
...
std::string LoadInputFiles(const IoParams &params, MRI *&mriTemplate,
  ...
  std::string pathSurfLeftWhite, pathSurfLeftPial, pathSurfRightWhite,
  ...
  if (!mriTemplate)
    throw IoError(std::string("failed to read template mri ") + pathMriInput);
...
void ComputeSurfaceDistanceFunction(MRIS *mris, float thickness, MRI *vol) {
  ...
  for (unsigned int ui(0), nvertices(mris->nvertices); ui < nvertices;
       ++ui, ++pvtx) {
  ...
  for (unsigned int ui(0), nfaces(mris->nfaces); ui < nfaces; ++ui, ++pface) {
...
#endif
  double bounds[6] = {0,   (double)dims[0] - 1.0, 0.0, (double)dims[1] - 1.0,
                      0.0, (double)dims[2] - 1.0};
  ...
  for (vtkIdType i = 0; i < npoints; ++i) {
    ...
    if (_inout == 0)
      std::cerr << "Warning: voxel could not be classified\n";
    ...
    while (!filo.empty()) {
      ...
      if (dist > 1) {
        ...
        if (z > 0 && !inout[j - incz])
          filo.push(j - incz);
        if (x < dims[0] - 1 && !inout[j + incx])
          filo.push(j + incx);
        if (y < dims[1] - 1 && !inout[j + incy])
          filo.push(j + incy);
        if (z < dims[2] - 1 && !inout[j + incz])
          filo.push(j + incz);
      }
    }
  } // next i
...
MRI *CreateHemiMask(MRI *dpial, MRI *dwhite, const unsigned char lblWhite,
  ...
  MRI *mri = MRIalloc(dpial->width, dpial->height, dpial->depth, MRI_UCHAR);

  for (unsigned int z(0), depth(dpial->depth); z < depth; ++z)
    for (unsigned int y(0), height(dpial->height); y < height; ++y)
      for (unsigned int x(0), width(dpial->width); x < width; ++x) {
        if (x == (unsigned int)Gx && y == (unsigned int)Gy &&
            z == (unsigned int)Gz)
          DiagBreak();
#ifndef NO_VTK
...
MRI *CombineMasks(MRI *maskOne, MRI *maskTwo,
                  const unsigned char lblBackground) {
  MRI *mri =
      MRIalloc(maskOne->width, maskOne->height, maskOne->depth, MRI_UCHAR);
  unsigned char voxOne, voxTwo;

  unsigned int overlap = 0;
  for (unsigned int z(0), depth(maskOne->depth); z < depth; ++z)
    for (unsigned int y(0), height(maskOne->height); y < height; ++y)
      for (unsigned int x(0), width(maskOne->width); x < width; ++x) {
...
MRI *FilterLabel(MRI *initialMask, const unsigned char lbl) {
  MRI *mri = MRIalloc(initialMask->width, initialMask->height,
                      initialMask->depth, MRI_UCHAR);

  for (unsigned int z(0), depth(initialMask->depth); z < depth; ++z)
    for (unsigned int y(0), height(initialMask->height); y < height; ++y)
      for (unsigned int x(0), width(initialMask->width); x < width; ++x) {
