...
MRI_SP *MRIScongeal(MRI_SURFACE *mris_ico, MRI_SURFACE **mris_array,
                    int nsubjects, MRI_SP *mrisp, INTEGRATION_PARMS *parms);
int main(int argc, char *argv[]) {
  ...
  make_cmd_version_string(
      argc, argv, "$Id: mris_congeal.c,v 1.4 2011/03/02 00:04:30 nicks Exp $",
      "$Name:  $", cmdline);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    n = parms.nfields++;
    SetFieldLabel(&parms.fields[n], which_field, where_in_atlas, l_corr,
                  l_pcorr, 0, which_norm);
    nargs = 4;
  }
  ...
  /*   } */
    ...
    printf("reading overlay from %s and smoothing it %d times\n", argv[2],
           navgs);
    n = parms.nfields++;
    SetFieldLabel(&parms.fields[n], OVERLAY_FRAME, atlas_size, l_ocorr, 0.0,
                  navgs, which_norm);
    ...
    printf("reading overlay from %s and smoothing it %d times\n", argv[2],
           navgs);
    n = parms.nfields++;
    SetFieldLabel(&parms.fields[n], DISTANCE_TRANSFORM_FRAME, atlas_size,
                  l_ocorr, 0.0, navgs, NORM_MAX);
    SetFieldName(&parms.fields[n], argv[2]);
    atlas_size++;
    nargs = 2;
    ...
    case 'L':
      ...
      if (label_indices[nlabels] < 0)
        ErrorExit(ERROR_NOFILE, "%s: could not map name %s to index", Progname,
                  argv[3]);
...
int MRIScongealRigidBodyAlignGlobal(MRI_SURFACE *mris_ico, MRI_SURFACE *mris,
                                    ...
                                    int nangles);
double MRIScongealComputeSSE(MRI_SURFACE *mris_ico, MRI_SURFACE **mris_array,
...
double MRIScongealEstimateTemplate(MRI_SURFACE *mris_ico,
                                   MRI_SURFACE **mris_array, MHT **mht_array,
                                   int nsubjects, INTEGRATION_PARMS *parms,
                                   int exclude_i) {
  int vno, i;
  VERTEX *v, *vsurf;
  double mean, var, sse;
...
double MRIScongealUpdateRegistration(MRI_SURFACE *mris_ico,
                                     MRI_SURFACE **mris_array, MHT **mht_array,
                                     int nsubjects, INTEGRATION_PARMS *parms) {
  double sse;

  sse =
      MRIScongealComputeSSE(mris_ico, mris_array, mht_array, nsubjects, parms);
  return (sse);
}

double MRIScongealUpdateRigidRegistration(MRI_SURFACE *mris_ico,
                                          MRI_SURFACE **mris_array,
                                          MHT **mht_array, MHT *mht_ico,
                                          int nsubjects, double degrees,
                                          INTEGRATION_PARMS *parms) {
  double sse;
  int i;
  ...
  for (i = 0; i < nsubjects; i++) {
    MRIScongealEstimateTemplate(mris_ico, mris_array, mht_array, nsubjects,
                                parms, i);
    parms->start_t = start_t;
    MRIScongealRigidBodyAlignGlobal(mris_ico, mris_array[i], mht_ico, parms,
                                    degrees, degrees, 6);
    MHTfree(&mht_array[i]);
  ...
  for (sno = parms->flags & IP_USE_INFLATED ? 0 : 1; sno < nsurfaces; sno++) {
    ...
    if (curvature_names[sno]) {
      ...
      for (i = 0; i < nsubjects; i++) {
        ...
        if (MRISreadVertexPositions(mris_array[i], surface_names[sno]) !=
            NO_ERROR)
    ...
    last_sse = MRIScongealEstimateTemplate(mris_ico, mris_array, mht_array,
                                           nsubjects, parms, -1);
    for (angle = max_angle; angle >= 0.5; angle /= 2) {
      ...
      MRIScongealUpdateRigidRegistration(mris_ico, mris_array, mht_array,
                                         mht_ico, nsubjects, angle, parms);
      sse = MRIScongealEstimateTemplate(mris_ico, mris_array, mht_array,
                                        nsubjects, parms, -1);
    ...
    do {
      MRIScongealUpdateRegistration(mris_ico, mris_array, mht_array, nsubjects,
                                    parms);
      sse = MRIScongealEstimateTemplate(mris_ico, mris_array, mht_array,
                                        nsubjects, parms, -1);
      pct_change = 100 * (last_sse - sse) / last_sse;
...
double MRIScongealComputeSurfaceError(MRI_SURFACE *mris_ico, MRI_SURFACE *mris,
                                      MHT *mht_ico, INTEGRATION_PARMS *parms) {
  double sse, error;
  int vno;
  VERTEX *v, *vsurf;
