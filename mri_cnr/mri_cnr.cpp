...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_cnr.c,v 1.11 2016/05/05 18:44:03 fischl Exp $",
      "$Name:  $");
  ...
  for (cnr_total = 0.0, j = 0; j <= 1; j++) {
    ...
    for (i = 2; i < argc; i++) {
      ...
      if (!j && !FZERO(mri->tr))
        fprintf(
            stderr,
            "TR = %2.1f msec, flip angle = %2.0f degrees, TE = %2.1f msec\n",
      ...
      if (slope_fname) {
        ...
        if (m_linfit == nullptr)
          ErrorExit(ERROR_NOMEMORY,
                    "%s: could not allocate slope/offset matrix", Progname);
        MRIScomputeSlope(mris, mri, dist_in, dist_out, step_in, step_out,
                         m_linfit);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    fprintf(stderr, "reading lh and rh labels from %s and %s\n", argv[2],
            argv[3]);
    lh_area = LabelRead(nullptr, argv[2]);
    if (lh_area == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not load label %s", Progname, argv[2]);
    rh_area = LabelRead(nullptr, argv[3]);
    if (rh_area == nullptr)
...
static void print_usage() {
  fprintf(
      ...
      Progname);
  fprintf(stderr, "usage: %s [options] <surf directory> <vol 1> <vol 2> ...\n",
          Progname);
  fprintf(stderr,
          "usage example (assumes fs pipeline has finished for subject subj1): "
          "%s subj1/surf subj1/mri/orig.mgz\n",
          Progname);
  fprintf(stderr, "Available options:\n");
  fprintf(stderr,
          "\t-s <slope_fname> <dist in> <dist out> <step in> <step out>: "
          "compute slope based on given values, write it to slope and offset "
          "files labeled <slope_fname> (e.g., `lh.<slope_fname>.slope.mgz')\n");
  fprintf(stderr, "\t-t : print only the total CNR to stdout (stderr still "
  ...
  fprintf(stderr, "\t-u, -?, -help : print usage information and quit\n");
...
static double compute_volume_cnr(MRI_SURFACE *mris, MRI *mri, char *log_fname) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISsurfaceRASToVoxelCached(mris, mri, v->x + v->nx, v->y + v->ny,
                                v->z + v->nz, &x, &y, &z);
    //    MRIsurfaceRASToVoxel(mri, v->x+v->nx, v->y+v->ny, v->z+v->nz, &x, &y,
    //    &z) ;
    MRIsampleVolume(mri, x, y, z, &csf);
    thickness = v->curv * .5;
    // MRIworldToVoxel(mri, v->x-thickness*v->nx, v->y-thickness*v->ny,
    // v->z-thickness*v->nz, &x, &y, &z) ;
    //    MRIsurfaceRASToVoxel(mri, v->x-thickness*v->nx, v->y-thickness*v->ny,
    //    v->z-thickness*v->nz, &x, &y, &z) ;
    MRISsurfaceRASToVoxelCached(mris, mri, v->x - thickness * v->nx,
                                v->y - thickness * v->ny,
                                v->z - thickness * v->nz, &x, &y, &z);
  ...
  if (only_total == 0)
    printf("\twhite = %2.1f+-%2.1f, gray = %2.1f+-%2.1f, csf = %2.1f+-%2.1f\n",
           white_mean, sqrt(white_var), gray_mean, sqrt(gray_var), csf_mean,
           sqrt(csf_var));
  ...
  if (log_fname) {
    ...
    if (fp == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not open file %s", Progname,
                log_fname);
    fprintf(fp, "%f %f %f %f %f %f %f %f\n", gray_white_cnr, gray_csf_cnr,
            white_mean, gray_mean, csf_mean, sqrt(white_var), sqrt(gray_var),
            sqrt(csf_var));
  }
