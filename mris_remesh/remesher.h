...
inline double &Vector::operator[](int index) { return v[index]; }

inline const double &Vector::operator[](int index) const { return v[index]; }
...
public:
  Matrix() {}
  Matrix(const double m11, const double m12, const double m13, const double m21,
         const double m22, const double m23, const double m31, const double m32,
         const double m33) {
    m[0] = m11;
    m[1] = m12;
    m[2] = m13;
...
inline double &Matrix::operator()(int i, int j) { return m[3 * i + j]; }

inline Matrix Matrix::operator-(const Matrix &mat) const {
  return Matrix(m[0] - mat.m[0], m[1] - mat.m[1], m[2] - mat.m[2],
                m[3] - mat.m[3], m[4] - mat.m[4], m[5] - mat.m[5],
                m[6] - mat.m[6], m[7] - mat.m[7], m[8] - mat.m[8]);
}

inline Matrix Matrix::operator*(const Matrix &mat) const {
  return Matrix(m[0] * mat.m[0] + m[1] * mat.m[3] + m[2] * mat.m[6],
...
inline Matrix Matrix::operator*(const double scalar) const {
  return Matrix(m[0] * scalar, m[1] * scalar, m[2] * scalar, m[3] * scalar,
                m[4] * scalar, m[5] * scalar, m[6] * scalar, m[7] * scalar,
                m[8] * scalar);
}

inline Vector Matrix::operator*(const Vector &v) const {
  return Vector(m[0] * v[0] + m[1] * v[1] + m[2] * v[2],
...
private:
  ...
  int    replaceVertexInEdges(int vold, int vnew, bool simulate = false,
                              int vtip0 = -1, int vtip1 = -1);
  ...
  void tangentialSmoothing(int it = 1, bool gravity = true,
                           bool tangent = true);
};
