...
static void print_usage() {
  ...
  printf("   --conformed-to-min : print whether a volume is conformed-to-min "
         "stdout\n");
  ...
  printf("   --center : print the RAS at the actual center of the volume (at "
         "(nv-1)/2)\n");
...
int PrettyMatrixPrint(MATRIX *mat) {
  ...
  for (row = 1; row < 5; ++row) {
    printf("              %8.4f %8.4f %8.4f %10.4f\n", mat->rptr[row][1],
           mat->rptr[row][2], mat->rptr[row][3], mat->rptr[row][4]);
  }
...
static void do_file(char *fname) {
  ...
  if (ext == nullptr)
    ErrorExit(ERROR_UNSUPPORTED, "%s: could not parse extension from %s",
              Progname, fname);
  if (!(strstr(ext, "m3d") == nullptr && strstr(ext, "m3z") == nullptr &&
        strstr(ext, "M3D") == nullptr && strstr(ext, "M3Z") == nullptr)) {
  ...
  if (PrintIs1mmIso) {
    if (fabs(mri->xsize - 1.0) < .001 && fabs(mri->ysize - 1.0) < .001 &&
        fabs(mri->zsize - 1.0) < .001)
      fprintf(fpout, "yes\n");
    else
      fprintf(fpout, "no\n");
    return;
  ...
  if (PrintTR || PrintTE || PrintFlipAngle || PrintFlipAngleDeg || PrintType ||
      PrintTI)
    return;
  ...
  // multi-frame info:
  {
    ...
    for (i = 0; i < mri->nframes; i++) {
      ...
      if ((frame->type != 0) || (!FZERO(frame->TE)) || (!FZERO(frame->TR)) ||
          (!FZERO(frame->flip)) || (frame->label != 0) ||
          (!FZERO(frame->thresh))) {
        frame_valid = 1;
      }
