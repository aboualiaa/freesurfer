...
int main(int argc, char *argv[]) {
  ...
  if (MRISreadOriginalProperties(mris, orig_name) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: could not read original surface %s", Progname,
              orig_name);
  ...
  if (MRISreadVertexPositions(mris, inflated_name) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: could not read inflated surface %s", Progname,
              inflated_name);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    fprintf(stderr,
            "terminating evolution after %d generations "
            "without change...\n",
            parms.max_unchanged);
    nargs = 1;
    ...
    parms.niters      = atoi(argv[2]);
    fprintf(stderr,
            "using random search with %d iterations to "
            "retessellate\n",
            parms.niters);
    nargs = 1;
...
static void print_usage(void) {
  outputHelpXml(mris_fix_topology_help_xml, mris_fix_topology_help_xml_len);
}
