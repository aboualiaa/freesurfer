...
int Getifh(char *imgfile, IFH *ifhdr) {
  ...
  while (fgets(line, MAXL, fp)) {
    ...
    *str = '\0'; /* terminate keyword part of line */
    ...
    if (strstr(line, "center")) {
      sscanf(parameter, "%f %f %f", ifhdr->center + 0, ifhdr->center + 1,
             ifhdr->center + 2);
      center_flag++;
    }
    if (strstr(line, "mmppix")) {
      sscanf(parameter, "%f %f %f", ifhdr->mmppix + 0, ifhdr->mmppix + 1,
             ifhdr->mmppix + 2);
      mmppix_flag++;
    }
  }
  ...
  if (!center_flag) {
    ifhdr->center[0] = ifhdr->mmppix[0] * (float)(ifhdr->matrix_size[0] -
                                                  ifhdr->matrix_size[0] / 2);
    ifhdr->center[1] =
        ifhdr->mmppix[1] * (float)(1 + ifhdr->matrix_size[1] / 2);
    ifhdr->center[2] =
        ifhdr->mmppix[2] * (float)(1 + ifhdr->matrix_size[2] / 2);
  }

  return 0;
ERR:
  if ((str = strrchr(parameter, '\n')))
    *str = '\0';
  fprintf(stderr, ">>> %s := %s <<<\n", line, parameter);
  return -1;
}
...
int Writeifh(char *program, char *outfile, IFH *ifhdr, char control) {
  ...
  for (i = 0; i < 3; i++) {
    fprintf(ifhfp, "scaling factor (mm/pixel) [%d]	:= %f\n", i + 1,
            ifhdr->scaling_factor[i]);
  }
  fprintf(ifhfp, "mmppix	:= %10.6f%10.6f%10.6f\n", ifhdr->mmppix[0],
          ifhdr->mmppix[1], ifhdr->mmppix[2]);
  fprintf(ifhfp, "center	:= %10.4f%10.4f%10.4f\n", ifhdr->center[0],
          ifhdr->center[1], ifhdr->center[2]);
...
int writeifhmc(char *program, char *outfile, int *imgdim, float *voxdim,
  ...
  if (!(ifhfp = fopen(imgroot, "w")))
    errw(program, imgroot);
  fprintf(ifhfp, "INTERFILE	:=\n");
  fprintf(ifhfp, "version of keys	:= 3.3\n");
  fprintf(ifhfp, "number format		:= float\n");
  fprintf(ifhfp, "conversion program	:= %s\n", program);
