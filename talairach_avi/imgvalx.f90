!$Id: imgvalx.f,v 1.1 2007/05/04 22:33:59 nicks Exp $
!$Log: imgvalx.f,v $
!Revision 1.1  2007/05/04 22:33:59  nicks
!new talairach alignment utility, using Avi Snyders registration tools
!
! Revision 1.2  2007/04/25  05:15:34  avi
! gcc v3 compliant
!
! Revision 1.1  1998/10/02  20:23:27  mcavoy
! Initial revision
!
! Revision 1.1  1997/11/14  17:16:24  tscull
! Initial revision
!
! Revision 1.3  1997/04/15  08:21:03  avi
! trap input image NaNs in subroutine imgvalx
!
! Revision 1.2  1996/12/28  01:40:42  avi
! minor changes - don't evaluate nonexistant memory if nz.eq.1
!
! Revision 1.1  1996/12/27  20:48:20  avi
! Initial revision
!
! revision 1.2  1993/10/25  17:37:33  avi

subroutine imgvalx(imgt, nx, ny, nz, center, mmppix, x, v, lslice)
    !     avi snyder 08-06-94
    !     returns in v the interpolated value of imgt at locus x.
    !     lslice returns the slice from which most of the data is taken.
    !     lslice = 0 (and v = 0.) if imgt is undefined at locus x.
...
subroutine peak_find(mode, imgt, nx, ny, nz, mmppix, centert, coord)
    ...
    !     6          y z
    !     7          x y z
    !     sign of extremum found = sign of mode
    real*4 imgt(nx, ny, nz)
...
subroutine edge_trace(imgt, nx, ny, nz, mmppix, center, param, edge, npmax, np)
    ...
    real*4 param(16)
    !     param(01:03)	normal to osculating plane
    !     param(04)		del arclength
    !     param(05)		sigma eval
    ...
    do 21 k = 0, nr - 1
        ...
        s = s + sin(a) * g
        c = c + cos(a) * g
        !     if(ldebug)write(*, "(i6,3f7.2,f7.0)")k,x,g
    21 continue
    theta = atan2(s, c)
    ...
    do 9 k = -nk - 1, nk + 1            ! sample image along normal
        ...
        if(lslice.le.0)goto 99
        !     if(ldebug)write(*, "('imgvalx x,v',3f7.2,f9.1)")x,v(k)
    9 continue
