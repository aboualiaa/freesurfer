...
void conc_open_quiet(CONC_BLOCK *conc_block, char *lstfile) {
  ...
  if (!(conc_block->lstfp = fopen(conc_block->lstfile, "r")))
    errr(conc_block->program, conc_block->lstfile);
  while (fgets(string, MAXL, conc_block->lstfp)) {
    if (!(ptr = strstr(string, "number_of_files:")))
      continue;
    conc_block->rnfile = atoi(ptr + 16);
  }
  if (!(conc_block->imgfile0 =
            (char **)malloc(conc_block->rnfile * sizeof(char *))))
    errm(conc_block->program);
  if (!(conc_block->nvol = (int *)malloc(conc_block->rnfile * sizeof(int))))
    errm(conc_block->program);
  for (i = 0; i < conc_block->rnfile; i++) {
    if (!(conc_block->imgfile0[i] = (char *)malloc(MAXL * sizeof(char))))
      errm(conc_block->program);
  }
  ...
  for (conc_block->imgdim[3] = i = 0; i < conc_block->rnfile; i++) {
    if (Getifh(conc_block->imgfile0[i], &ifh))
    ...
    if (!i) {
      ...
      status |= (conc_block->isbig !=
                 strcmp(ifh.imagedata_byte_order, "littleendian"));
      for (k = 0; k < 3; k++) {
  ...
  if (status) {
    fprintf(stderr, "%s: %s %s dimensional or byte order inconsistency\n",
            conc_block->program, conc_block->imgfile0[0],
            conc_block->imgfile0[i]);
    exit(-1);
  }
...
void conc_open(CONC_BLOCK *conc_block, char *lstfile) {
  ...
  printf("imgdim %10d%10d%10d%10d\n", conc_block->imgdim[0],
         conc_block->imgdim[1], conc_block->imgdim[2], conc_block->imgdim[3]);
  printf("voxdim %10.6f%10.6f%10.6f\n", conc_block->voxdim[0],
         conc_block->voxdim[1], conc_block->voxdim[2]);
  printf("mmppix %10.6f%10.6f%10.6f\n", conc_block->mmppix[0],
         conc_block->mmppix[1], conc_block->mmppix[2]);
  printf("center %10.4f%10.4f%10.4f\n", conc_block->center[0],
         conc_block->center[1], conc_block->center[2]);
  conc_block->rifile = conc_block->rivol = 0;
}
...
void conc_read_vol(CONC_BLOCK *conc_block, float *imgt) {
  ...
  if (!conc_block->rivol) {
    if (!(conc_block->imgfp =
              fopen(conc_block->imgfile0[conc_block->rifile], "rb")))
  ...
  if (++conc_block->rivol == conc_block->nvol[conc_block->rifile]) {
    ...
    if (++conc_block->rifile == conc_block->rnfile)
      conc_block->rifile = 0;
  }
}
...
void conc_write_vol(CONC_BLOCK *conc_block, float *imgt) {
  ...
  if (!conc_block->wivol) {
    if (!(conc_block->outfp =
              fopen(conc_block->imgfile1[conc_block->wifile], "wb")))
  ...
  if (++conc_block->wivol == conc_block->nvol[conc_block->wifile]) {
    ...
    conc_block->wivol = 0;
    if (++conc_block->wifile == conc_block->wnfile)
      conc_block->wifile = 0;
  }
}
...
void conc_new(CONC_BLOCK *conc_block, char *trailer) {
  ...
  if (!strlen(conc_block->lstfile) || !conc_block->rnfile) {
    fprintf(stderr,
            "%s: conc_new not possible because conc read not initialized\n",
            conc_block->program);
    exit(-1);
  }
  conc_block->wnfile = conc_block->rnfile;
  if (!(conc_block->imgfile1 =
            (char **)malloc(conc_block->wnfile * sizeof(char *))))
    errm(conc_block->program);
  sprintf(conc_block->outfile, "%s_%s.conc", conc_block->lstroot, trailer);
  if (!(conc_block->outfp = fopen(conc_block->outfile, "w")))
    errw(conc_block->program, conc_block->outfile);
  fprintf(conc_block->outfp, "number_of_files: %d\n", conc_block->wnfile);
  for (i = 0; i < conc_block->wnfile; i++) {
    if (!(conc_block->imgfile1[i] = (char *)malloc(MAXL * sizeof(char))))
      errm(conc_block->program);
...
int conc_ifh_hdr_rec(CONC_BLOCK *conc_block, int argc, char *argv[],
  ...
  case 'L':
    ...
    break;
  default:
    conc_block->osbig = CPU_is_bigendian();
    break;
  }

  for (i = 0; i < 3; i++)
    imgdim[i] = conc_block->imgdim[i];
  for (i = 0; i < conc_block->wnfile; i++) {
    imgdim[3] = conc_block->nvol[i];
    writeifhmce(conc_block->program, conc_block->imgfile1[i], imgdim,
                conc_block->voxdim, conc_block->orient, conc_block->mmppix,
                conc_block->center, conc_block->control);
    sprintf(command, "ifh2hdr %s", conc_block->imgfile1[i]);
...
void conc_free(CONC_BLOCK *conc_block) {
  ...
  if (conc_block->rnfile) {
    for (i = 0; i < conc_block->rnfile; i++)
      free(conc_block->imgfile0[i]);
    free(conc_block->imgfile0);
    free(conc_block->nvol);
  }
  if (conc_block->wnfile) {
    for (i = 0; i < conc_block->wnfile; i++)
      free(conc_block->imgfile1[i]);
