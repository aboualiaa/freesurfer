...
int main(int argc, char *argv[]) {
  ...
  for (k = 0; k < imgdim[3]; k++) {
    if (gread((char *)imgt, sizeof(float), dimension, imgfp, isbig))
      errr(program, imgfile);
    if (xflag)
      flipx(imgt, imgdim + 0, imgdim + 1, imgdim + 2);
    if (yflag)
      flipy(imgt, imgdim + 0, imgdim + 1, imgdim + 2);
    if (zflag)
      flipz(imgt, imgdim + 0, imgdim + 1, imgdim + 2);
    if (gwrite((char *)imgt, sizeof(float), dimension, outfp, control))
      errw(program, outfile);
  }
  fclose(imgfp);
  fclose(outfp);
