...
int main(int argc, char *argv[]) {
  ...
  if (!(fpimg = fopen(imgfile, "rb")) ||
      fread(&hdr, sizeof(struct dsr), 1, fpimg) != 1 || fclose(fpimg))
  ...
  printf("dimensions");
  for (i = 0; i < 4; i++)
    printf("%10d", hdr.dime.dim[i + 1]);
  printf("\n");
  for (i = 0; i < 4; i++)
    imgdim[i] = hdr.dime.dim[i + 1];
  for (i = 0; i < 3; i++)
    voxsiz[i] = hdr.dime.pixdim[i + 1];
  vdim = imgdim[0] * imgdim[1] * imgdim[2];

  if (!(imgu = (unsigned char *)malloc(vdim * bytepix)))
    errm(program);
  if (!(imgi = (short *)malloc(vdim * bytepix)))
    errm(program);
  ...
  for (k = 0; k < imgdim[3]; k++) {
    ...
    case 16:
      if (fread(imgf, bytepix, vdim, fpimg) != vdim)
        errr(program, imgfile);
      if (swab_flag)
        for (i = 0; i < vdim; i++)
          swab4((char *)&imgf[i]);
      break;
    case 4:
      if (fread(imgi, bytepix, vdim, fpimg) != vdim)
        errr(program, imgfile);
      if (swab_flag)
        for (i = 0; i < vdim; i++)
          swab2((char *)&imgi[i]);
      ...
      break;
    case 2:
      if (fread(imgu, bytepix, vdim, fpimg) != vdim)
        errr(program, imgfile);
      for (i = 0; i < vdim; i++)
        imgf[i] = (float)imgu[i];
      break;
    ...
    if (Weber_flag)
      flipy(imgf, imgdim + 0, imgdim + 1, imgdim + 2);
    switch (orient) { /* 0 = transverse; 1 = coronal; 2 = sagittal */
    case 2:
      flipx(imgf, imgdim + 0, imgdim + 1, imgdim + 2);
    case 1:
      flipz(imgf, imgdim + 0, imgdim + 1, imgdim + 2);
    case 0:
      flipy(imgf, imgdim + 0, imgdim + 1, imgdim + 2);
      break;
    default:
    ...
    for (i = 0; i < vdim; i++) {
      if (scale_flag)
        imgf[i] *= ROIScaleFactor;
      if (imgf[i] > fmax)
        fmax = imgf[i];
      if (imgf[i] < fmin)
        fmin = imgf[i];
    }
    if (ewrite(imgf, vdim, control, fpout))
      errw(program, outfile);
  }
  fclose(fpimg);
  fclose(fpout);
  ...
  /* hdr */
  /*******/
  sprintf(command, "ifh2hdr %s -r%dto%d", outroot, (int)(fmin - 0.5),
          (int)(fmax + 0.5));
  printf("%s\n", command);
  ...
  if (Weber_flag) {
    sprintf(command,
            "Orientation of second axis flipped according to Fred Weber\n");
    printrec(command);
  }
  if (xflag) {
