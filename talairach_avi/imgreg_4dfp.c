...
extern void t4file2param_(int *mode, char *t4file,
                          float *params); /* t4_sub.f */
extern void ffind_(float *img1, short *msk1, int *nx1, int *ny1, int *nz1,
                   float *mmppix1, float *center1, float *img2, short *msk2,
                   int *nx2, int *ny2, int *nz2, float *mmppix2, float *center2,
                   float *params, int *mode);
extern void fimgreg_(float *img1, short *msk1, int *nx1, int *ny1, int *nz1,
                     float *mmppix1, float *center1, float *img2, short *msk2,
...
void read_file_float(char *filename, float *stack, int dimension, char *program,
                     int isbig) {
  FILE *fp;

  if (!(fp = fopen(filename, "rb")) || eread(stack, dimension, isbig, fp) ||
      fclose(fp))
...
int main(int argc, char **argv) {
  ...
  for (k = 0, i = 1; i < argc; i++) {
    if (*argv[i] == '-')
      ...
      switch (k) {
      case 0:
        getroot(argv[i], imgroot[0]);
        k++;
        break;
      case 1:
        getroot(argv[i], mskroot[0]);
        k++;
        break;
      case 2:
        getroot(argv[i], imgroot[1]);
        k++;
        break;
      case 3:
        getroot(argv[i], mskroot[1]);
        k++;
        break;
      case 4:
        strcpy(t4file, argv[i]);
        k++;
        break;
      case 5:
        mode = atoi(argv[i]);
  ...
  if (k < 6) {
    printf("Usage:\t%s target_imag target_mask source_imag source_mask t4file "
           "mode\n",
           program);
    printf(
        "or:\t%s target_imag        none source_imag source_mask t4file mode\n",
        program);
    printf(
        "or:\t%s target_imag        none source_imag        none t4file mode\n",
        program);
    exit(1);
  }
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (get_4dfp_dimoe(filespc, imgdim, voxdim, &orientation, &isbig) < 0)
      errr(program, filespc);
    nx[j] = imgdim[0];
    ny[j] = imgdim[1];
    nz[j] = imgdim[2];
    ...
    if (strcmp(mskroot[j], "none")) {
      ...
      for (i = 0; i < 3; i++)
        k |= (mskdim[i] != imgdim[i]);
      for (i = 0; i < 3; i++)
        k |= (fabs(ifhm.mmppix[i] - ifh[j].mmppix[i]) > 0.0001);
      if (k) {
      ...
      for (k = i = 0; i < dimension; i++)
        if ((mask[j][i] = (short)imag[j][i]))
          k++;
      printf(" (%d pixels)\n", k);
      for (i = 0; i < dimension; i++)
        mask[j][i] = (short)imag[j][i];
    } else {
      for (i = 0; i < dimension; i++)
        mask[j][i] = 1;
    }
  ...
  /***********/
  if (mode & FIND) {
    ffind_(imag[0], mask[0], &nx[0], &ny[0], &nz[0], ifh[0].mmppix,
           ifh[0].center, imag[1], mask[1], &nx[1], &ny[1], &nz[1],
           ifh[1].mmppix, ifh[1].center, param, &mode);
  } else {
    fimgreg_(imag[0], mask[0], nx + 0, ny + 0, nz + 0, ifh[0].mmppix,
             ifh[0].center, imag[1], mask[1], nx + 1, ny + 1, nz + 1,
             ifh[1].mmppix, ifh[1].center, param, &mode);
  }
