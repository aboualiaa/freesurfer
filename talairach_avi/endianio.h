...
swab_hdr 		byte swaps the Analyze header (see ANALYZE.h for struct
dsr). CPU_is_bigendian 	returns 1 if the CPU on which it is executed is BIG
ENDIAN architecture.
...
getroot 		peels off the extensions from filespc and populates
imgroot. eread 			reads n*sizeof(float) bytes from fp and
populates imgt. eread also performs swaps if necessary depending on isbig and
CPU_is_bigendian.
ewrite 			writes n*sizeof(float) bytes to fp from imgt. ewrite
also performs swaps if necessary depending on control and CPU_is_bigendian.
gread 			reads n*bytes from fp and populates imgt. gread also
performs swaps if necessary depending on isbig and CPU_is_bigendian. gwrite
writes n*bytes to fp from imgt. gwrite also performs swaps if necessary
depending on control and CPU_is_bigendian. load_4dfp_frame 	loads the volume
identified by frame into fimg. It performs the necessary swaps. get_4dfp_dimoe
reads the IFH file and returns the image dimensions into imgdim,voxel size into
voxsiz, orientation into orient and returns the isbig status.
