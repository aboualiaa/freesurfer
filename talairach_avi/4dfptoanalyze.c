...
int main(int argc, char *argv[]) {
  ...
  if (k < 1) {
    ...
    printf("\t-SPM99\tinclude origin and scale in hdr "
           "(http:/wideman-one.com/gw/brain/analyze/format.doc)\n");
  ...
  /*****************************/
  if (get_4dfp_dimoe(imgroot, imgdim, voxsiz, &orient, &isbig))
    errr(program, imgroot);
  vdim = imgdim[0] * imgdim[1] * imgdim[2];
  if (uchar) {
    if (!(imgu = (unsigned char *)malloc(vdim * sizeof(char))))
      errm(program);
  } else {
    if (!(imgi = (short *)malloc(vdim * sizeof(short))))
      errm(program);
  }
  if (!(imgf = (float *)malloc(vdim * sizeof(float))))
    errm(program);
  ...
  for (k = 0; k < imgdim[3]; k++) {
    if (eread(imgf, vdim, isbig, fpimg))
      errr(program, imgfile);
    switch (orient) {
    case 4:
      flipx(imgf, imgdim + 0, imgdim + 1, imgdim + 2); /* sagittal */
    case 3:
      flipz(imgf, imgdim + 0, imgdim + 1, imgdim + 2); /* coronal */
    case 2:
      flipy(imgf, imgdim + 0, imgdim + 1, imgdim + 2); /* transverse */
      break;
    default:
    ...
    if (uchar) {
      if (fwrite(imgu, sizeof(char), vdim, fpout) != vdim)
        errw(program, outfile);
    } else {
      if (gwrite((char *)imgi, sizeof(short), vdim, fpout, control))
        errw(program, outfile);
    }
  }
  fclose(fpimg);
  ...
  if (spm99) {
    if (Getifh(imgroot, &ifh))
      errr(program, imgroot);
    for (i = 0; i < 3; i++)
      origin[i] = 0.4999 + ifh.center[i] / ifh.mmppix[i];
    /*************************************************/
  ...
  printf("Writing: %s\n", outfile);
  if (!(fpout = fopen(outfile, "wb")) ||
      fwrite(&hdr, sizeof(struct dsr), 1, fpout) != 1 || fclose(fpout))
    errw(program, outfile);
