!$Header: /space/repo/1/dev/dev/talairach_avi/fftsol.f,v 1.1 2007/05/04 22:33:59 nicks Exp $
!$Log: fftsol.f,v $
!Revision 1.1  2007/05/04 22:33:59  nicks
!new talairach alignment utility, using Avi Snyders registration tools
!
! Revision 1.2  2007/03/05  00:26:13  avi
! dimension nfac(11) -> dimension nfac(13)
! nfac array index limit (maxnfac) check installed
!
! Revision 1.1  2007/03/04  23:20:21  avi
! Initial revision
!
...
!-------------------------------------------------------------------
! SUBROUTINE:  REALS
! USED WITH FFT TO COMPUTE FOURIER TRANSFORM OR INVERSE
! FOR REAL DATA
!-------------------------------------------------------------------
!
...
SUBROUTINE REALT(A, B, NSEG, N, NSPN, ISN)
    ...
    !       PASS THE INITIAL ADDRESS FOR THE SEQUENCE OF IMAGINARY
    !       VALUES, E.G.
    !         CALL FFT  (A,A(2),1,N,1,-2)
    !         CALL REALT(A,A(2),1,N,1,-2)
    !      IN THIS CASE, THE COSINE AND SINE COEFFICIENTS ALTERNATE IN A.
    !
