...
extern void pt4ixyz_(int *imgdim, float *voxdim, float *centert, float *mmppixt,
                     float *t4mat, float *centero, float *mmppixo, float *t4atl,
                     float *t4); /* ft4ixyz.f */
extern void ft4ixyz_(int *mode, float *t4, float *imgt, int *nxt, int *nyt,
                     int *nzt, float *imgo, int *nxo, int *nyo,
                     int *nzo); /* ft4ixyz.f */
extern void ft4imgn_(float *t4, float *imgt, int *nxt, int *nyt, int *nzt,
                     float *centert, float *mmppixt, float *imgo, int *nxo,
                     int *nyo, int *nzo, float *centero,
                     float *mmppixo); /* ft4imgn.f */
extern void ft4imgo_(float *t4, float *imgt, int *nxt, int *nyt, int *nzt,
                     float *centert, float *mmppixt, float *imgo, int *nxo,
                     int *nyo, int *nzo, float *centero,
                     float *mmppixo); /* ft4imgo.f */
extern void vrtflip_(int *iori, int *imgdim, float *centeri, float *mmppixi,
                     float *centert, float *mmppixt); /* ft4imgo.f */
extern void splineza_(float *imgt, int *nx, int *ny, int *nz,
                      float *d2zi); /* spline3dvgh.f */
extern void splinezf_(float *imgt, int *nx, int *ny, int *nz,
                      float *d2zi); /* spline3dvgh.f */
extern void splinex_(float *imgt, int *nx, int *ny, int *nz,
                     float *d2xi); /* spline3dvgh.f */
extern void spliney_(float *imgt, int *nx, int *ny, int *nz,
...
int main(int argc, char *argv[]) {
  ...
  /* scan input list */
  /*******************/
  if (!(lstfp = fopen(lstfile, "r")))
    errr(program, lstfile);
  nstack = 0;
  while (fgets(string, 2 * MAXL, lstfp))
    nstack++;
  rewind(lstfp);
  if (!(stackspc = (RUN_INFO *)malloc(nstack * sizeof(RUN_INFO))))
    errm(program);
  nstack = nframe = 0;
  while (fgets(string, 2 * MAXL, lstfp)) {
    ...
    i = m = 0;
    while (m < 256) {
      while (!isgraph((int)string[i]) && string[i])
        i++;
      if (!string[i])
        break;
      srgv[m] = string + i;
      m++;
      while (isgraph((int)string[i]))
        i++;
      if (!string[i])
        break;
      string[i] = '\0';
    ...
    if (get_4dfp_dimoe_quiet(imgroot, imgdim, voxdim, &k, &isbig))
      errr(program, stackspc[nstack].imgfile);
    if (k < 2 || k > 4) {
    ...
    if (strlen(stackspc[nstack].t4file)) {
      if (access(stackspc[nstack].t4file, R_OK))
        errr(program, stackspc[nstack].t4file);
    }
    ...
    if (strlen(stackspc[nstack].mskfile)) {
      ...
      if (access(stackspc[nstack].mskfile, R_OK))
        errr(program, stackspc[nstack].mskfile);
    }
  ...
  mino = FLT_MAX;
  for (i = 0; i < nstack; i++) {
    get_4dfp_dimoe_quiet(stackspc[i].imgfile, imgdim, voxdim,
                         &stackspc[i].orient, &stackspc[i].isbig);
    ...
    if (!(imgfp = fopen(stackspc[i].imgfile, "r")))
      errr(program, stackspc[i].imgfile);
    ...
    if (!(imgt = (float *)malloc(idim * ((spline_flag) ? 4 : 1) *
                                 sizeof(float))) ||
        !(imgm = (float *)malloc(idim * sizeof(float))))
      errm(program);
    if (strlen(stackspc[i].mskfile)) {
      if (get_4dfp_dimoe_quiet(stackspc[i].mskfile, imgdimm, voxdimm, &orientm,
                               &isbigm))
      ...
      if (status) {
        fprintf(stderr, "%s: %s %s dimension mismatch\n", program,
                stackspc[i].mskfile, stackspc[i].imgfile);
        exit(-1);
      }
      fprintf(stdout, "Reading: %s\n", stackspc[i].mskfile);
      if (!(mskfp = fopen(stackspc[i].mskfile, "r")) ||
          eread(imgm, idim, isbigm, mskfp) || fclose(mskfp))
        errr(program, stackspc[i].mskfile);
    } else {
      for (k = 0; k < idim; k++)
        imgm[k] = 1.0;
    ...
    if (stackspc[i].lo_slice) {
      for (k = 0; k < xdim * ydim * (stackspc[i].lo_slice - 1); k++)
        imgm[k] = 0.;
    }
    if (stackspc[i].hi_slice) {
      for (k = xdim * ydim * stackspc[i].hi_slice; k < idim; k++)
        imgm[k] = 0.;
    }
    ...
    printf("input stack mmppix     %10.6f%10.6f%10.6f\n", ifh.mmppix[0],
           ifh.mmppix[1], ifh.mmppix[2]);
    printf("input stack center     %10.4f%10.4f%10.4f\n", ifh.center[0],
           ifh.center[1], ifh.center[2]);
    printf("imgfile=%s\n", stackspc[i].imgfile);
    if (strlen(stackspc[i].mskfile))
      printf("maskfile=%s\n", stackspc[i].mskfile);
    if (strlen(stackspc[i].t4file))
      printf("t4file=%s\n", stackspc[i].t4file);
    if (stackspc[i].lo_slice)
      printf("slices=%dto%d  ", stackspc[i].lo_slice, stackspc[i].hi_slice);
    ...
    for (j = jstart; j < jend; j++) {
      printf(" %d", j + 1);
      fflush(stdout);
      if (eread(imgt, idim, stackspc[i].isbig, imgfp))
        errr(program, stackspc[i].imgfile);
      if (spline_flag)
      ...
      /*****************************************/
      vrtflip_(&stackspc[i].orient, imgdim, ifh.center, ifh.mmppix, centert,
               mmppixt);
      /*********************/
      /* execute transform */
      /*********************/
      if (spline_flag) {
        pt4ixyz_(imgdim, voxdim, centert, mmppixt, t4mat, centero, mmppixo,
                 t4atl, t4);
        ft4ixyz_(&mode, t4, imgt, imgdim + 0, imgdim + 1, imgdim + 2, imgo,
                 outdim + 0, outdim + 1, outdim + 2);
      } else if (nearest_neighbor) {
        ft4imgn_(t4atl, imgt, &xdim, &ydim, &zdim, centert, mmppixt, imgo, &nxo,
                 &nyo, &nzo, centero, mmppixo);
      } else {
        ft4imgo_(t4atl, imgt, &xdim, &ydim, &zdim, centert, mmppixt, imgo, &nxo,
                 &nyo, &nzo, centero, mmppixo);
      }
      /*******************************/
  ...
  if (!(imgfp = fopen(outfile, "wb")) ||
      ewrite(imgs, odim * nframe, control, imgfp) || fclose(imgfp))
    errw(program, outfile);

  /***************************/
  /* create interfile header */
  /***************************/
  writeifhmce(program, outfile, outdim, voxdim, orio, mmppixo, centero,
              control);

  /*******/
  /* hdr */
  /*******/
  sprintf(command, "ifh2hdr %s -r%dto%d", outroot, (int)(mino - 0.5),
          (int)(maxo + 0.5));
  printf("%s\n", command);
...
int x4dfp2ecat(float *imag, int *dim, int orientation) {
  switch (orientation) {
  case 2:
    flipx(imag, dim + 0, dim + 1, dim + 2); /* transverse */
    flipz(imag, dim + 0, dim + 1, dim + 2);
    break;
  case 3:
    flipx(imag, dim + 0, dim + 1, dim + 2); /* coronal */
    break;
  case 4:
    break; /* sagittal */
  default:
