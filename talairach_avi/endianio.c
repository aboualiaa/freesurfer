...
void getroot(char *filespc, char *imgroot) {
  ...
  while ((str = strrchr(imgroot, '.'))) {
    if (!strcmp(str, ".rec"))
      ...
      *str = '\0';
    else if (!strcmp(str, ".ifh"))
      *str = '\0';
    else if (!strcmp(str, ".4dfp"))
      *str = '\0';
    else if (!strcmp(str, ".hdr"))
      *str = '\0';
    else if (!strcmp(str, ".conc"))
      *str = '\0';
    else
      break;
  }
...
int eread(float *imgt, int n, int isbig, FILE *fp) {
  ...
  if (0)
    printf("eread swab_flag=%d\n", swab_flag);
  if (fread(imgt, sizeof(float), n, fp) != n)
    return -1;
  if (swab_flag)
    for (i = 0; i < n; i++)
      swab4((char *)(imgt + i));
...
int gread(char *imgt, size_t bytes, int n, FILE *fp, int isbig) {
  ...
  swab_flag = (CPU_is_bigendian() != 0) != (isbig != 0);
  if (0)
    printf("gread swab_flag=%d\n", swab_flag);

  if (fread(imgt, bytes, n, fp) != n)
    return -1;
  if (swab_flag)
    for (i = 0; i < n; i++)
      switch (bytes) {
...
int gwrite(char *imgt, size_t bytes, int n, FILE *fp, char control) {
  ...
  swab_flag =
      ...
      ((CPU_is_bigendian() == 0) && (control == 'b' || control == 'B'));
  if (0)
    printf("gwrite swab_flag=%d\n", swab_flag);

  status = 0;
  if (swab_flag) {
    if (!(imgb = malloc(bytes * n)))
      errm("gwrite");
    for (i = 0; i < bytes * n; i++)
      imgb[i] = imgt[i];
    for (i = 0; i < n; i++)
...
void load_4dfp_frame(char *fileroot, int *imgdim, int frame, int isbig,
  ...
  if (!(fp = fopen(filespc, "rb")) ||
      fseek(fp, (long)frame * vdim * sizeof(float), SEEK_SET) ||
      fread(fimg, sizeof(float), vdim, fp) != vdim || fclose(fp))
    errr(subr, filespc);
...
int get_4dfp_dimoe(char *fileroot, int *imgdim, float *voxsiz, int *orient,
  ...
  if (Getifh(filespc, &ifh))
    errr(subr, filespc);
  for (k = 0; k < 3; k++)
    voxsiz[k] = ifh.scaling_factor[k];
  for (k = 0; k < 4; k++)
    imgdim[k] = ifh.matrix_size[k];

  fprintf(stdout, "%s\n", fileroot);
  fprintf(stdout, "%10d%10d%10d%10d\n", imgdim[0], imgdim[1], imgdim[2],
          imgdim[3]);
...
void eread_ewrite_test() {
  ...
  if (!(fp = fopen(filespc, "wb")))
    errw(program, filespc);
  if (ewrite(x, n, 'b', fp) || fclose(fp))
    errw(program, filespc);
  printf("Reading: %s\n", filespc);
  fflush(stdout);
  if (!(fp = fopen(filespc, "rb")))
    errr(program, filespc);
  if (eread(y, n, 1, fp) || fclose(fp))
    errr(program, filespc);
  for (i = 0; i < n; i++)
    assert(y[i] == x[i]);
  ...
  if (!(fp = fopen(filespc, "wb")))
    errw(program, filespc);
  if (ewrite(x, n, 'l', fp) || fclose(fp))
    errw(program, filespc);
  printf("Reading: %s\n", filespc);
  fflush(stdout);
  if (!(fp = fopen(filespc, "rb")))
    errr(program, filespc);
  if (eread(y, n, 0, fp) || fclose(fp))
    errr(program, filespc);
  for (i = 0; i < n; i++)
    assert(y[i] == x[i]);
...
void gread_gwrite_test() {
  ...
  if (!(fp = fopen(filespc, "wb")))
    errw(program, filespc);
  if (gwrite((char *)x, sizeof(float), n, fp, 'b') || fclose(fp))
    errw(program, filespc);
  printf("Reading: %s\n", filespc);
  fflush(stdout);
  if (!(fp = fopen(filespc, "rb")))
    errr(program, filespc);
  if (gread((char *)y, sizeof(float), n, fp, 1) || fclose(fp))
    errr(program, filespc);
  if (0)
    for (i = 0; i < n; i++)
      printf("%10f%10f\n", x[i], y[i]);
  ...
  if (!(fp = fopen(filespc, "wb")))
    errw(program, filespc);
  if (gwrite((char *)x, sizeof(float), n, fp, 'l') || fclose(fp))
    errw(program, filespc);
  printf("Reading: %s\n", filespc);
  fflush(stdout);
  if (!(fp = fopen(filespc, "rb")))
    errr(program, filespc);
  if (gread((char *)y, sizeof(float), n, fp, 0) || fclose(fp))
    errr(program, filespc);
  if (0)
    for (i = 0; i < n; i++)
      printf("%10f%10f\n", x[i], y[i]);
