!$Id: imgvalm.f,v 1.1 2007/05/04 22:33:59 nicks Exp $
!$Log: imgvalm.f,v $
...
! Initial revision
!
! Revision 1.2  1996/12/28  01:41:46  avi
! correct error in y bound checking and make algorithm
! more like imgvalx.f
!
! Revision 1.1  1996/12/27  23:26:08  avi
! Initial revision
!
subroutine imgvalm(imag, mask, nx, ny, nz, t, x, v, lslice)
    !     returns in v imag value at t transformed coordinate x
    !     lslice=-1 coordinate not in imag array
    !     lslice= 0 coordinate not in mask
...
subroutine imgvalmg(imag, mask, nx, ny, nz, t, x, eps, grad, lslice)
    !     returns grad imag value at t transformed coordinate x
    !     lslice=-1 coordinate not in imag array
    !     lslice= 0 coordinate not in mask
    real*4 imag(nx, ny, nz)
    integer*2 mask(nx, ny, nz)
