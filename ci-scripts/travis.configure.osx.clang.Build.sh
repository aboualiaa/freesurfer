...
cmake -G Ninja -DCMAKE_BUILD_TYPE=Release -DFS_INTEGRATION_TESTING=ON -DFS_LLVM_PATH=/usr/local/opt/llvm/bin -DGEMS_BUILD_MATLAB=OFF -DFS_ADD_INSTRUMENTATION=$INSTRUMENTATION ..
cmake -G Ninja -DCMAKE_BUILD_TYPE=Release -DFS_INTEGRATION_TESTING=ON -DFS_LLVM_PATH=/usr/local/opt/llvm/bin -DGEMS_BUILD_MATLAB=OFF -DFS_ADD_INSTRUMENTATION=$INSTRUMENTATION .. # again so that AppleClang finds OpenMP
