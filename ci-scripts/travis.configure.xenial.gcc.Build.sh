...
(
  ...
  sudo apt-get install -y --no-install-suggests --no-install-recommends git wget tcsh build-essential \
    gfortran-9 libblas-dev liblapack-dev zlib1g-dev libxmu-dev \
  ...
  wget https://www.dropbox.com/s/drijzemquap0rqk/itk5.0.1_linux_xenial.tar?dl=0 >/dev/null 2>&1
  tar -xf "itk5.0.1_linux_xenial.tar?dl=0"
