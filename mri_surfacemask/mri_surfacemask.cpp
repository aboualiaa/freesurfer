    ...
    */
    for (v = 0; v <= numv; v++) {
      ...
      for (u = 0; u <= numu; u++) {
        ...
        MRIsurfaceRASToVoxel(const_cast<MRI *>(mri_filled), px, py, pz, &tx,
                             &ty, &tz);
...
int main(int argc, char *argv[]) {
  ...
  int nargs = handle_version_option(
      argc, argv,
      "$Id: mri_surfacemask.cpp,v 1.5 2011/03/02 00:04:25 nicks Exp $",
      "$Name:  $");
  ...
  // prepare the out put
  MRI *mri_out = MRIcopy(mri_input, nullptr);
  if (!mri_out) {
    MRIfree(&mri_input);
    MRIfree(&mri_filled);
