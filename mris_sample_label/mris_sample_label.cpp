...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(FileNameExtension(in_label_fname, ext), "mgz")) {
    ...
    if (mri == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read input volume from %s",
                Progname, in_label_fname);
    label = LabelfromASeg(mri, voxval);
    MRIfree(&mri);
  } else {
    label = LabelRead(nullptr, in_label_fname);
    if (!label)
...
static void print_usage() {
  fprintf(stderr,
          "usage: %s [options] <input label file> <input surface file> <output "
          "label file>\n",
          Progname);
}
