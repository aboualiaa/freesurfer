  ...
  */
  if (label_name) {
    ...
    for (n = 0; n < area->n_points; n++) {
      if (area->lv[n].vno >= mris->nvertices) {
        printf("ERROR: label point %d has vertex number = %d, but surface only "
               "has %d vertices\n",
               n, area->lv[n].vno, mris->nvertices);
        printf("Make sure that the label and surface come from the same "
               "hemisphere of the same subject.\n");
  ...
  if (tablefile != nullptr) {
    ...
    if (strcmp(surf_name, "white") == 0)
      fprintf(fp,
              "# Measure Cortex, WhiteSurfArea, White Surface Total Area, %g, "
              "mm^2\n",
              total_cortex_area);
    else if (strcmp(surf_name, "pial") == 0)
      fprintf(
          fp,
          "# Measure Cortex, PialSurfArea, Pial Surface Total Area, %g, mm^2\n",
          total_cortex_area);
    else
    ...
    if (cortex_label) {
      fprintf(fp, "# Measure Cortex, MeanThickness, Mean Thickness, %g, mm\n",
    ...
    if (DoGlobalStats) {
      ...
      std::vector<double> BrainVolStats =
          ReadCachedBrainVolumeStats(sname, sdir);
      ...
      fprintf(fp,
              "# Measure BrainSeg, BrainSegVol, Brain Segmentation Volume, %f, "
              "mm^3\n",
              BrainVolStats[0]);
      fprintf(fp,
              "# Measure BrainSegNotVent, BrainSegVolNotVent, Brain "
              "Segmentation Volume Without Ventricles, %f, mm^3\n",
              BrainVolStats[1]);
      fprintf(fp,
              "# Measure BrainSegNotVentSurf, BrainSegVolNotVentSurf, Brain "
              "Segmentation Volume Without Ventricles from Surf, %f, mm^3\n",
              BrainVolStats[14]);
      fprintf(fp,
              ...
              BrainVolStats[7]);
      fprintf(fp,
              "# Measure SupraTentorial, SupraTentorialVol, Supratentorial "
              "volume, %f, mm^3\n",
              BrainVolStats[2]);
      fprintf(fp,
...

  {
    ...
    for (i = 0; i < MAX_INDICES; i++) {
      ...
      MRIScomputeCurvatureIndicesMarked(mris, &intrinsic_curvature_index,
                                        &folding_index, i);
      ...
      if (tablefile != nullptr) {
        ...
        fprintf(fp, "  %5.3f %5.3f", thicknesses[i], sqrt(thickness_vars[i]));
      ...
      if (tabular_output_flag) {
        ...
        fprintf(stdout, "  %5.3f %5.3f", thicknesses[i],
        ...
        fprintf(stdout, "folding index                           = %2.0f\n",
                folding_index);
        fprintf(stdout, "intrinsic curvature index               = %2.1f\n",
      ...
      if (log_fp) {
        ...
        fprintf(
            ...
            wm_volume, areas[i], volumes[i], thicknesses[i],
            sqrt(thickness_vars[i]), mean_abs_mean_curvature,
            mean_abs_gaussian_curvature, folding_index,
            intrinsic_curvature_index);
#else
        if (!noheader)
          fprintf(log_fp,
                  "%% %s: <surf area> <gray vol> "
                  "<thick mean> <thick var> <integ rect. mean curv> "
                  "<integ rect. Gauss curv> <fold index> <intr curv ind>\n",
                  sname);
        fprintf(log_fp,
                "%2.0f\t%2.0f\t%2.3f\t%2.3f\t%2.3f\t%2.3f\t%2.0f\t%2.1f\n",
                areas[i], volumes[i], thicknesses[i], sqrt(thickness_vars[i]),
                mean_abs_mean_curvature, mean_abs_gaussian_curvature,
                folding_index, intrinsic_curvature_index);
#endif
        fclose(log_fp);
      }
    }
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    fs::warning() << "the --suffix flag has been removed. Brain volume stats "
                     "will be computed normally";
    nargs = 1;
