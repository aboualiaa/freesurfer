...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_average.c,v 1.47 2016/06/01 17:49:55 fischl Exp $",
      "$Name:  $");
  ...
  if (fromFile) {
    ...
    if (!FileExists(list_fname))
      ...
      while (fscanf(fp, "%s", fname) != EOF) {
        ...
        if (filecount == 1) {
          if (!FZERO(tx) || !FZERO(ty) || !FZERO(tz)) {
            ...
            fprintf(stderr,
                    ...
                    ty, tz);
            mri_tmp = MRItranslate(mri_src, nullptr, tx, ty, tz);
            MRIfree(&mri_src);
            mri_src = mri_tmp;
          }
          if (!FZERO(rzrot)) {
            ...
            fprintf(stderr,
                    ...
                    (float)DEGREES(rzrot));
            mri_tmp = MRIrotateZ_I(mri_src, nullptr, rzrot);
            MRIfree(&mri_src);
            mri_src = mri_tmp;
          }
          if (!FZERO(rxrot)) {
            ...
            fprintf(stderr,
                    ...
                    (float)DEGREES(rxrot));
            mri_tmp = MRIrotateX_I(mri_src, nullptr, rxrot);
            MRIfree(&mri_src);
            mri_src = mri_tmp;
          }
          if (!FZERO(ryrot)) {
            ...
            fprintf(stderr,
                    ...
                    (float)DEGREES(ryrot));
            mri_tmp = MRIrotateY_I(mri_src, nullptr, ryrot);
            MRIfree(&mri_src);
            mri_src = mri_tmp;
          }
        }
        ...
        if (mri_avg && ((mri_src->width != mri_avg->width) ||
    ...
    for (num = 0, i = 1; i < argc - 1; i++) {
      ...
      if (i == 2) {
        ...
        if (!FZERO(ryrot) || !FZERO(rxrot) || !FZERO(rzrot)) {
          ...
          fprintf(stderr,
                  ...
                  (float)DEGREES(rxrot),
                  (float)DEGREES(ryrot)(float) DEGREES(rzrot));
      ...
      if (mri_avg && ((mri_src->width != mri_avg->width) ||
                      ...
                      (mri_src->depth != mri_avg->depth))) {
        fprintf(stderr,
                "src image (%d, %d, %d) not compatible with "
                "avg image (%d, %d, %d)\n",
                mri_src->width, mri_src->height, mri_src->depth, mri_avg->width,
                mri_avg->height, mri_avg->depth);
        skipped++;
        continue;
      }
      ...
      if (argc == 3) {
        fprintf(stderr, "Single input, working on %d input frames instead.\n",
                mri_src->nframes);
        mri_avg = MRIsumFrames(mri_src, nullptr, sqr_images);
      } else {
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "dt")) {
    ...
    sqr_images = 1;
    fprintf(stderr,
            "computing sqrt of sum of squares instead of average (RMS)...\n");
    ...
    case 'B':
      ...
      printf(
          "binarizing images with thresh %2.1f to compute pct at each voxel\n",
          binarize_thresh);
      nargs = 1;
      break;
    case 'R':
...
static void usage_exit(int code) {
  ...
  printf("\t-F              read volumes from an input file (first argument is "
         "the input filename)\n");
  printf("\t-dt <float n>   set dt to n (default=1e-6)\n");
  printf("\t-tol <float n>  set tol to n (default=1e-5)\n");
  printf("\t-conform        interpolate volume to be isotropic 1mm^3 (this "
...
static MRI *align_with_average(MRI *mri_src, MRI *mri_avg) {
  ...
  if (Gdiag & DIAG_WRITE) {
    if (sinc_flag)
      mri_aligned = MRIsincTransform(mri_src, nullptr, m_L, sinchalfwindow);
    else
      mri_aligned = MRIlinearTransform(mri_src, nullptr, m_L);
  ...
  if (window_flag) {
    mri_in_windowed =
        MRIwindow(mri_src, nullptr, WINDOW_HANNING, 127, 127, 127, 100.0f);
    mri_ref_windowed =
        MRIwindow(mri_avg, nullptr, WINDOW_HANNING, 127, 127, 127, 100.0f);
  ...
  if (sinc_flag)
    mri_aligned = MRIsincTransform(mri_src, nullptr, parms.lta->xforms[0].m_L,
                                   sinchalfwindow);
  else
    mri_aligned =
        MRIlinearTransform(mri_src, nullptr, parms.lta->xforms[0].m_L);
  if (Gdiag & DIAG_WRITE)
