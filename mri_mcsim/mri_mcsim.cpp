...
int main(int argc, char *argv[]) {
    ...
    for (nthThresh = 0; nthThresh < nThreshList; nthThresh++) {
      for (nthSign = 0; nthSign < nSignList; nthSign++) {
        ...
        if (fio_FileExistsReadable(fname)) {
          ...
          if (fpLog)
            fprintf(fpLog, "ERROR: output file %s exists\n", fname);
          exit(1);
        }
  ...
  if (LabelFile) {
    ...
    sprintf(tmpstr, "%s/%s/label/%s.%s.label", SUBJECTS_DIR, subject, hemi,
            LabelFile);
  ...
  if (mask && (MRIgetVoxVal(mask, 102161, 0, 0, 0) > 0.5 ||
               MRIgetVoxVal(mask, 102162, 0, 0, 0) > 0.5))
  ...
  for (nthFWHM = 0; nthFWHM < nFWHMList; nthFWHM++) {
    ...
    printf("%2d %5.1f  %4d\n", nthFWHM, FWHMList[nthFWHM],
           nSmoothsList[nthFWHM]);
    if (fpLog)
      fprintf(fpLog, "%2d %5.1f  %4d\n", nthFWHM, FWHMList[nthFWHM],
              nSmoothsList[nthFWHM]);
  }
  printf("\n");
  ...
  for (n = 0; n < nThreshList; n++)
    printf("%5.2f ", ThreshList[n]);
  printf("\n");
  printf("Signs (%d): ", nSignList);
  for (n = 0; n < nSignList; n++)
    printf("%2d ", SignList[n]);
  printf("\n");
  printf("FWHM (%d): ", nFWHMList);
  for (n = 0; n < nFWHMList; n++)
    printf("%5.2f ", FWHMList[n]);
  printf("\n");
  ...
  for (nthRep = 0; nthRep < nRepetitions; nthRep++) {
    ...
    if (fpLog)
      fprintf(fpLog, "\n");
    if (SaveEachIter || fio_FileExistsReadable(SaveFile))
      SaveOutput();
...
finish:
  ...
  printf("Total Sim Time %g min (%g per rep)\n", msecTime / (1000 * 60.0),
         (msecTime / (1000 * 60.0)) / nthRep);
  if (fpLog)
    fprintf(fpLog, "Total Sim Time %g min (%g per rep)\n",
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--save-weight"))
      SaveWeight = 1;
    else if (!strcasecmp(option, "--no-fix-fsalh"))
      FixFSALH = 0;
      ...
      UseAvgVtxArea = 1;
    else if (!strcasecmp(option, "--no-avgvtxarea"))
      UseAvgVtxArea = 0;
      ...
      nargsused = 1;
    } else if (!strcasecmp(option, "--no-save-mask"))
      SaveMask = 0;
...
static void print_help() {
  ...
  printf(
      "mri_glmfit-sim with the --cache option. The tables in mult-comp-cor\n");
  printf("are for fsaverage and fsaverage_sym for a search space over the "
         "whole\n");
  printf(
      "cortex. mri_mcsim can be used to create new tables for a new subject\n");
  ...
  printf("\n");
  printf("mri_mcsim --o /path/to/mult-comp-cor/newsubject/lh/cortex --base "
         "mc-z \n");
  ...
  printf(
      "mri_annotation2label --subject fsaverage --hemi lh --outdir labeldir\n");
  printf("\n");
  printf("mri_mcsim --o /path/to/mult-comp-cor/fsaverage/lh/superiortemporal "
         "--base mc-z \n");
  printf("  --save-iter  --surf fsaverage lh --nreps 10000 \n");
  printf("  --label labeldir/lh.superiortemporal.label\n");
  printf("\n");
  printf("When running mri_glmfit, make sure to use   --label "
         "labeldir/lh.superiortemporal.label\n");
  printf("When running mri_glmfit-sim, add --cache-dir /path/to/mult-comp-cor "
         "--cache-label superiortemporal\n");
  printf("\n");
  printf("Example 3: running simulations in parallel (two jobs, 5000 "
         "iterations\n");
  ...
  printf("  --csd "
         "/path/to/mult-comp-cor/fsaverage/lh/superiortemporal/fwhm10/abs/th20/"
         "mc-z.j001.csd \n");
  printf("  --csd "
         "/path/to/mult-comp-cor/fsaverage/lh/superiortemporal/fwhm10/abs/th20/"
         "mc-z.j002.csd \n");
  ...
  printf("  --csdpdf  "
         "/path/to/mult-comp-cor/fsaverage/lh/superiortemporal/fwhm10/abs/th20/"
         "mc-z.cdf --csdpdf-only\n");
  printf("\n");
  printf("Repeat the above command for each FWHM, sign (pos, neg, abs) and "
         "threshold\n");
  printf("\n");
    ...
    for (nthThresh = 0; nthThresh < nThreshList; nthThresh++) {
      for (nthSign = 0; nthSign < nSignList; nthSign++) {
        ...
        if (csd->threshsign == -1)
          signstr = "neg";
        csd->nreps = nthRep;
        sprintf(tmpstr, "%s/fwhm%02d/%s/th%02d/%s.csd", OutTop,
                (int)round(FWHMList[nthFWHM]), signstr,
                (int)round(10 * ThreshList[nthThresh]), csdbase);
        fp = fopen(tmpstr, "w");
