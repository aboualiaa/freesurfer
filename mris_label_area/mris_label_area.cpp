...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mris_label_area.c,v 1.6 2011/03/02 00:04:32 nicks Exp $",
      "$Name:  $");
  ...
  if (MRISreadAnnotation(mris, annot_name) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: could not read annot file %s", Progname,
              annot_name);
  ...
  for (i = 5; i < argc; i++) {
    ...
    if (compute_pct) {
      printf("%2.3f mm^2 in label %d (%s), %%%2.6f of total cortical area "
             "(%2.2f)\n",
             area, label, name, 100.0 * (float)area / (float)total_area,
             total_area);
      if (log_fp) {
  ...
  if (!stricmp(option, "sdir")) {
    ...
    case 'C':
      if (read_named_annotation_table(argv[2]) != NO_ERROR)
        ErrorExit(ERROR_NOFILE, "%s: could not read lookup table %s", Progname,
                  argv[2]);
      nargs = 1;
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <subject name> <hemi> <surf name> <annot name> "
         "<label 1> <label 2> ...\n",
         Progname);
  printf(
