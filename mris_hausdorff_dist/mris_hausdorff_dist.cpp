...
int main(int argc, char *argv[]) {
  ...
  if (annot_name) {
    ...
    for (a1 = 0; a1 < mris->ct->nentries; a1++) {
      ...
      for (a2 = 0; a2 < mris->ct->nentries; a2++) {
        ...
        printf("%s %s %d %d %2.3f\n", mris->ct->entries[a1]->name,
               mris->ct->entries[a2]->name, a1, a2, hdist);
        fflush(stdout);
        LabelFree(&area2);
      }
      LabelFree(&area1);
    ...
    if (mri_identify(argv[2]) != MGH_LABEL_FILE) {
      ...
      if (mri_label == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not read overlay %s", Progname,
                  argv[2]);
      ...
      area1 = LabelRead(nullptr, argv[2]);
    if (area1 == nullptr)
    ...
    if (mri_identify(argv[3]) != MGH_LABEL_FILE) {
      ...
      area2 = LabelRead(nullptr, argv[3]);
    if (area2 == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read label %s", Progname, argv[3]);
