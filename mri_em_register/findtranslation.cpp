...
void CheckInverseTranslation(const MATRIX *mat, const float dx, const float dy,
                             const float dz) {
  ...
  if (!exactInverse) {
    ...
    std::cout << std::setprecision(12) << std::setw(20) << dx
              ...
              << std::setprecision(12) << std::setw(20) << dz << std::endl;
  }
}
...
double find_optimal_translation(GCA *gca, GCA_SAMPLE *gcas, MRI *mri,
  ...
  max_log_p = local_GCAcomputeLogSampleProbability(gca, gcas, mri, m_L,
                                                   nsamples, exvivo, clamp);

  for (i = 0; i <= nreductions; i++) {
#ifdef OUTPUT_STAGES
    std::stringstream numString;
    numString << std::setw(2) << std::setfill('0') << i;
        ...
        *MATRIX_RELT(m_trans, 2, 4) = y_trans;
        for (z_trans = min_trans; z_trans <= max_trans; z_trans += delta) {
          *MATRIX_RELT(m_trans, 3, 4) = z_trans;
          ...
          CheckInverseTranslation(inv_m_L, m_L->rptr[1][4] + x_trans,
    ...
    if (Gdiag & DIAG_SHOW) {
      printf("max log p = %12.6f @ (%4.3f, %4.3f, %4.3f)\n", max_log_p, x_max,
             y_max, z_max);
    }
...
#ifdef OUTPUT_STAGES
  std::cerr << __FUNCTION__ << ": Early exit for debugging" << std::endl;
  exit(0);
#endif
