...
double local_GCAcomputeLogSampleProbability(GCA *gca, GCA_SAMPLE *gcas,
  ...
  if (exvivo) {
    ...
    return ((SQR(gm - wm) + SQR(gm - fluid) + SQR(fluid - wm) + SQR(gm) -
             SQR(fluid)));
  }
  ...
  if (robust) {
    ...
    result = GCAcomputeNumberOfGoodFittingSamples(gca, gcas, mri, transform,
                                                  nsamples);
  } else {
    if (use_variance)
      result =
          GCAcomputeLabelIntensityVariance(gca, gcas, mri, transform, nsamples);
    else
      result = GCAcomputeLogSampleProbability(gca, gcas, mri, transform,
                                              nsamples, clamp);
  }
...
int compute_tissue_modes(MRI *mri_inputs, GCA *gca, GCA_SAMPLE *gcas,
  ...
  */
  ...
  for (ngm = nwm = nfluid = 0, wm = gm = fluid = 0.0, i = 0; i < nsamples;
       i++) {
    /////////////////// diag code /////////////////////////////
    if (i == Gdiag_no) {
      DiagBreak();
