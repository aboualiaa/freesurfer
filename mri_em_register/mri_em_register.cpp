...
static double Gclamp = 6; // robust threshold - everything less likely than
                          // -Gclamp will be set to -Gclamp
...
static double find_optimal_linear_xform(
    ...
    float max_scale, float min_trans, float max_trans, float angle_steps,
...
static MRI *apply_transform(MRI *mri, GCA *gca, MATRIX *m_L) {
  MRI *mri_aligned;

  mri->nframes = 1;
  Glta->xforms[0].m_L = m_L;
  mri_aligned = MRIlinearTransform(mri, nullptr, m_L);
  ...
  if (mri->xsize < gca->xsize || mri->ysize < gca->ysize ||
      mri->zsize < gca->zsize) {
    MRI *mri_tmp;
    mri_tmp = MRIextract(mri_aligned, nullptr, 0, 0, 0, gca->width, gca->height,
                         gca->depth);
    GCAcopyDCToMRI(gca, mri_tmp);
    MRIfree(&mri_aligned);
    mri_aligned = mri_tmp;
    mri_aligned->xsize = gca->xsize;
    mri_aligned->ysize = gca->ysize;
    mri_aligned->zsize = gca->zsize;
  }
  return (mri_aligned);
}
...
int main(int argc, char *argv[]) {
  ...
  if (remove_lh) {
    Gvx = nint(gca->width *
               .4); // only one hemi - assume it is left/right centered in FOV
  ...
  for (i = 0; i < ninputs; i++) {
    ...
    if (T2_mask_fname) {
      ...
      MRImask_with_T2_and_aparc_aseg(mri_tmp, mri_tmp, mri_T2, mri_aparc_aseg,
                                     T2_thresh, MM_FROM_EXTERIOR);
      MRIfree(&mri_T2);
  ...
  ///////////////////////////////////////////////////////////////
    ...
    if (nomap == 0 && need_map_flag == 1) {
      ...
      gca_tmp =
          GCAcreateFlashGCAfromFlashGCA(gca, TRs, fas, TEs, mri_in->nframes);
      GCAfree(&gca);
      gca = gca_tmp;
    }
#if 0
  ...
  if (!FZERO(tx) || !FZERO(ty) || !FZERO(tz)) {
    ...
    mri_tmp = MRItranslate(mri_in, nullptr, tx, ty, tz);
    MRIfree(&mri_in);
    mri_in = mri_tmp;
  }
  ////////////////////////  -r rxrot ryrot rzrot ///////////////////////
  ...
  if (!FZERO(rzrot)) {
    ...
    printf("rotating second volume by %2.1f degrees around Z axis\n",
           (float)DEGREES(rzrot));
    mri_tmp = MRIrotateZ_I(mri_in, nullptr, rzrot);
    MRIfree(&mri_in);
    mri_in = mri_tmp;
  }
  if (!FZERO(rxrot)) {
    ...
    printf("rotating second volume by %2.1f degrees around X axis\n",
           (float)DEGREES(rxrot));
    mri_tmp = MRIrotateX_I(mri_in, nullptr, rxrot);
    MRIfree(&mri_in);
    mri_in = mri_tmp;
  }
  if (!FZERO(ryrot)) {
    ...
    printf("rotating second volume by %2.1f degrees around Y axis\n",
           (float)DEGREES(ryrot));
    mri_tmp = MRIrotateY_I(mri_in, nullptr, ryrot);
    MRIfree(&mri_in);
    mri_in = mri_tmp;
  }
  //////////////////// if -t is not used
  if (!transform_loaded) /* wasn't preloaded */
  {
    ...
    parms.lta->xforms[0].m_L = MatrixIdentity(4, nullptr);
    printf("accounting for voxel sizes in initial transform\n");
    m_voxsize = MatrixIdentity(4, nullptr);
  ...
  if (ninsertions > 0)
    GCAinsertLabels(gca, mri_in, transform, ninsertions, insert_labels,
                    insert_intensities, insert_coords, insert_whalf);

  if (parms.write_iterations != 0) {
    char fname[STRLEN];
    MRI *mri_gca;
    mri_gca = MRIclone(mri_in, nullptr);
  ...
  // default nscales = 1
  for (spacing = max_spacing, scale = 0; scale < nscales;
       scale++, spacing /= 2) {
    if (false && skull)
      parms.gcas = GCAfindExteriorSamples(gca, &nsamples, spacing, min_prior,
                                          unknown_nbr_spacing, 0);
    else if (use_contrast) // -contrast option
    {
      parms.gcas = GCAfindContrastSamples(gca, &nsamples, spacing, min_prior);
    } else
      parms.gcas =
          GCAfindStableSamples(gca, &nsamples, spacing, min_prior, exclude_list,
                               unknown_nbr_spacing, vent_spacing);
    ...
    old_log_p = local_GCAcomputeLogSampleProbability(
        gca, parms.gcas, mri_in, ((LTA *)(transform->xform))->xforms[0].m_L,
        nsamples, exvivo, Gclamp);
    // real work done here
  ...
  if (false && skull)
    parms.gcas = GCAfindExteriorSamples(gca, &nsamples, spacing / 2, min_prior,
                                        unknown_nbr_spacing, 0);
  else
    parms.gcas = GCAfindAllSamples(gca, &nsamples, exclude_list,
                                   unknown_nbr_spacing); // HACK, bigvent) ;
  mark_gcas_classes(parms.gcas, nsamples);
  parms.nsamples = nsamples;
  parms.tol = 1e-7;
  parms.start_t++;
  ...
  if ((Gdiag & DIAG_WRITE) && (parms.write_iterations != 0)) {
    ...
    GCAtransformAndWriteSamples(gca, mri_in, parms.gcas, nsamples, fname,
                                transform);
    sprintf(fname, "%s%3.3d_pvals.mgz", parms.base_name, parms.start_t);
    GCAtransformAndWriteSamplePvals(gca, mri_in, parms.gcas, nsamples, fname,
                                    transform);
    sprintf(fname, "%s%3.3d_means.mgz", parms.base_name, parms.start_t);
    GCAtransformAndWriteSampleMeans(gca, mri_in, parms.gcas, nsamples, fname,
                                    transform);
  }
  /////////////////////////////////////////////////////////////////////////
  if (exvivo) {
  ...
  if (!stricmp(out_fname + strlen(out_fname) - 3, "XFM")) {
    ...
    fflush(stdout);
    MatrixPrint(stdout, parms.lta->xforms[0].m_L);
    MRIvoxelXformToRasXform(mri_in, mri_dst, parms.lta->xforms[0].m_L,
  ...
  if (parms.lta->type == LINEAR_RAS_TO_RAS) /* convert back to voxel */
  {
    ...
    fflush(stdout);
    MRIrasXformToVoxelXform(mri_in, mri_dst, parms.lta->xforms[0].m_L,
                            parms.lta->xforms[0].m_L);
    parms.lta->type = LINEAR_VOX_TO_VOX;
  }
  MRIfree(&mri_dst);
  ...
  if (norm_fname) {
    ...
    GCAremoveOutlyingSamples(gca, parms.gcas, mri_in, transform, nsamples, 2.0);
    ...
    for (nleft_used = nright_used = i = 0; i < nint(nused * ctl_point_pct);
         i++) {
      if (parms.gcas[ordered_indices[i]].label ==
          Left_Cerebellum_White_Matter) {
        nleft_used++;
    ...
    printf("%d/%d (l/r) cerebellar points initially in "
           "top %d%%, min (%d,%d)\n",
           nleft_used, nright_used, (int)(ctl_point_pct * 100.0f), min_left_cbm,
           min_right_cbm);

    for (i = nint(nused * ctl_point_pct); i < nsamples; i++) {
      if ((parms.gcas[ordered_indices[i]].label ==
           Left_Cerebellum_White_Matter) &&
          nleft_used < min_left_cbm) {
        nleft_used++;
      } else if ((parms.gcas[ordered_indices[i]].label ==
                  Right_Cerebellum_White_Matter) &&
    ...
    if (nint(nused * ctl_point_pct) == 0)
      ErrorPrintf(ERROR_BADPARM,
                  "%s: too few control points (%d) for normalization", Progname,
                  nused);
    else {
...
static int register_mri(MRI *mri_in, GCA *gca, MORPH_PARMS *parms, int passno,
                        int spacing) {
  MATRIX *m_L;
  // get the stored transform (vox-to-vox transform)
  m_L = MatrixCopy(parms->lta->xforms[0].m_L, nullptr);
...
static MATRIX *find_optimal_transform(MRI *mri, GCA *gca, GCA_SAMPLE *gcas,
  ...
  double gca_means[3], /*in_means[3], dx, dy, dz,*/ max_log_p, old_max,
      max_angle, angle_steps, min_scale, max_scale, scale_steps, scale, delta,
      mean;
  ...
  *MATRIX_RELT(m_origin, 4, 4) = 1;
  ...
  if (passno == 0) {
    ...
    if (Gdiag & DIAG_WRITE && write_iterations > 0) {
      ...
      GCAtransformAndWriteSamplePvals(gca, mri, gcas, nsamples, fname,
                                      transform);
      sprintf(fname, "%s000_means.mgz", parms.base_name);
      GCAtransformAndWriteSampleMeans(gca, mri, parms.gcas, nsamples, fname,
                                      transform);
      MRIfree(&mri_aligned);
    }
  }
  ...
  if (passno == 0 && !transform_loaded) /* only first time*/
  {
    ...
    *MATRIX_RELT(m_L, 3, 4) = dz ;
    ...
    max_log_p = find_optimal_translation(gca, gcas, mri, nsamples, m_L, -200,
                                         200, 19, 7, Gclamp);
    max_log_p = local_GCAcomputeLogSampleProbability(gca, gcas, mri, m_L,
                                                     nsamples, exvivo, Gclamp);
    fprintf(stdout, "Found translation: (%2.1f, %2.1f, %2.1f): log p = %4.3f\n",
            *MATRIX_RELT(m_L, 1, 4), *MATRIX_RELT(m_L, 2, 4),
            *MATRIX_RELT(m_L, 3, 4), max_log_p);
    ...
    */
    ...
    if (write_iterations != 0) {
      ...
      GCAtransformAndWriteSampleMeans(gca, mri, gcas, nsamples, fname,
                                      transform);
      MRIfree(&mri_aligned);
      mri_aligned = MRIinverseLinearTransform(mri, nullptr, m_L);
  ...
  do {
    ...
    max_log_p = find_optimal_linear_xform(
        gca, gcas, mri, nsamples, m_L, m_origin, -(max_angle * scale),
        (max_angle * scale), min_scale, max_scale,
        -scale * (spacing / 16.0) * MAX_TRANS,
        scale * (spacing / 16.0) * MAX_TRANS, max_angles, scale_samples,
        /*MAX_TRANS_STEPS*/ 3, 2);
    fflush(stdout);
...
#else
      Glta->xforms[0].m_L = m_L;
      sprintf(fname, "%s%3.3d_fsamples.mgz", parms.base_name,
              parms.start_t + niter + 1);
      GCAtransformAndWriteSamples(gca, mri, gcas, nsamples, fname, transform);
      sprintf(fname, "%s%3.3d_means.mgz", parms.base_name,
              parms.start_t + niter + 1);
      GCAtransformAndWriteSampleMeans(gca, mri, parms.gcas, nsamples, fname,
                                      transform);
#endif
...
static void printUsage() {
  outputHelpXml(mri_em_register_help_xml, mri_em_register_help_xml_len);
}
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!strcmp(option, "DIST") || !strcmp(option, "DISTANCE")) {
    ...
    exvivo = 1;
    printf("assuming input is ex vivo image with unknown conditional "
           "distributions\n");
    ...
    printf("inserting label %d (%s) at (%d, %d, %d) with intensity = %d\n",
           ...
           insert_coords[ninsertions][0], insert_coords[ninsertions][1],
           insert_coords[ninsertions][2], insert_intensities[ninsertions]);
    ...
    nargs = 1;
    printf("disallowing WM voxels within %d mm of ventricles in atlas (useful "
           "for big ventricles)\n",
           vent_spacing);
  }
  /////// debug options //////////////////////////////////
  ...
  ///////////////////
    ...
    case 'L': /* for longitudinal analysis */
    {
      ...
      if (reg_transform == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not read registration from %s",
                  Progname, argv[3]);
      transform_loaded = 1;
      TransformInvert(reg_transform, nullptr);
    ...
    case 'S':
      ...
      nargs = 1 ;
#else
      Gscale_samples = max_scales = MAX_ANGLES = MAX_TRANS_STEPS = max_angles =
          (float)atoi(argv[2]);
      nargs = 1;
