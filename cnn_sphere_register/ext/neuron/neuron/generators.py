...
class Vol(object):
    def __init__(
        ...
        if (
            ...
            and all(f is not None for f in patch_size)
        ):
...
def vol(
    ...
    expected_files=None,
    data_proc_fn=None,
    # processing function that takes in one arg (the volume)
    relabel=None,  # relabeling array
    nb_labels_reshape=0,
    # reshape to categorial format for keras, need # labels
    keep_vol_size=False,
    # whether to keep the volume size on categorical resizing
    ...
    while 1:
        ...
        try:
            ...
            debug_error_msg = (
                "#files: %d, fileidx: %d, nb_restart_cycle: %d. error: %s"
            )
            print(
                debug_error_msg
        ...
        assert ~np.any(np.isnan(vol_data)), (
            "Found a nan for %s" % volfiles[fileidx]
        )
        assert np.all(np.isfinite(vol_data)), (
            "Found a inf for %s" % volfiles[fileidx]
        )
        ...
        for lpatch in patch_gen:
            ...
            if np.mod(feat_idx, nb_feats) == 0:
                ...
                vol_data_feats = np.concatenate(
                    [vol_data_feats, lpatch], np.ndim(lpatch) - 1
                )
            feat_idx += 1
            ...
            if np.mod(feat_idx, nb_feats) == 0:
                ...
                final_batch = (
                    yield_incomplete_final_batch
                    and files_done
                    and patch_idx == (nb_patches_per_vol - 1)
                )
                if final_batch:  # verbose and
                    print(
                        "last batch in %s cycle %d. nb_batch:%d"
        ...
        if empty_gen:
            raise ValueError(
                "Patch generator was empty for file %s", volfiles[fileidx]
            )
...
def patch(
    ...
    nb_labels_reshape=1,
    # number of labels for categorical resizing. 0 if no resizing
    keep_vol_size=False,
    # whether to keep the volume size on categorical resizing
    ...
    infinite=False,
):  # whether the generator should continue (re)-generating patches
    """
    ...
    while True:
        ...
        for lpatch in gen:
            ...
            lpatch = _categorical_prep(
                lpatch, nb_labels_reshape, keep_vol_size, patch_size
            )

            if collapse_2d is not None:
                lpatch = np.squeeze(
                    lpatch, collapse_2d + 1
                )  # +1 due to batch in first dim
...
def vol_seg(
    ...
    ** kwargs are any named arguments for vol(...),
    ...
    vol_files = [
        f.replace(vol_subname, seg_subname)
    ...
    while 1:
        ...
        output_vol = next(seg_gen).astype(
            "float16"
        )  # was int8. Why? need float possibility...
...
def vol_cat(
    volpaths,  # expect two folders in here
    crop=None,
    resize_shape=None,
    rescale=None,  # processing parameters
    verbose=False,
    ...
    vol_rand_seed=None,
    **kwargs
):  # named arguments for vol(...), except verbose, data_proc_fn, ext, nb_labels_reshape and name (which this function will control when calling vol())
    """
    ...
    # compute processing function
    proc_vol_fn = lambda x: nrn_proc.vol_proc(
        x, crop=crop, resize_shape=resize_shape, interp_order=2, rescale=rescale
    )
    ...
    for folder in folders:
        ...
        generators_len += (
            len(_get_file_list(os.path.join(volpaths, folder), ".npz")),
        )
        generators += (vol_gen,)
    ...
    while 1:
        ...
        order = np.hstack(
            (np.zeros(generators_len[0]), np.ones(generators_len[1]))
        ).astype("int")
...
def add_prior(
    ...
    assert (
        np.ndim(prior_vol) == 4 or np.ndim(prior_vol) == 3
    ), "prior is the wrong size"
...
def vol_prior(
    *args,
    ...
    patch_rand=False,
    **kwargs
):  # anything else you'd like to pass to vol()
    """
...
def vol_sr_slices(
    ...
    def indices_to_batch(
        vol_data, start_indices, nb_slices_in_subvol, nb_slice_spacing
    ):
        idx = start_indices[0]
        output_batch = np.expand_dims(
            vol_data[:, :, idx : idx + nb_slices_in_subvol], 0
        )
        input_batch = np.expand_dims(
            vol_data[
                :, :, idx : (idx + nb_slices_in_subvol) : (nb_slice_spacing + 1)
            ],
            0,
        )

        for idx in start_indices[1:]:
            out_sel = np.expand_dims(
                vol_data[:, :, idx : idx + nb_slices_in_subvol], 0
            )
        ...
        output_batch = np.reshape(
            output_batch, [batch_size, -1, output_batch.shape[-1]]
        )

        return (input_batch, output_batch)

    print(
        "vol_sr_slices: SHOULD PROPERLY RANDOMIZE accross different subjects",
        file=sys.stderr,
    )
    ...
    while 1:
        ...
        try:
            vol_data = _load_medical_volume(
                os.path.join(volpath, volfiles[fileidx]), ext, verbose
            )
        except:
            debug_error_msg = (
                "#files: %d, fileidx: %d, nb_restart_cycle: %d. error: %s"
        ...
        # prepare batches
        if (
            simulate_whole_sparse_vol
        ):  # if essentially simulate a whole sparse volume for consistent inputs, and yield slices like that:
            init_slice = 0
            if rand_slices:
                init_slice = np.random.randint(0, high=nb_start_slices - 1)

            all_start_indices = list(
                range(init_slice, nb_start_slices, nb_slice_spacing + 1)
            )

            for batch_start in range(
                0, len(all_start_indices), batch_size * (nb_input_slices - 1)
            ):
                start_indices = [
                    all_start_indices[s]
        ...
        # if just random slices, get a batch of random starts from this volume and that's it.
            ...
            start_indices = np.random.choice(
                range(nb_start_slices), size=batch_size, replace=False
            )
            input_batch, output_batch = indices_to_batch(
                vol_data, start_indices, nb_slices_in_subvol, nb_slice_spacing
            )
...
def ext_data(
    ...
    # compute subvolume split
    vol_data = _load_medical_volume(
        os.path.join(volpath, expected_files[0]), ".npz"
    )
    # process volume
    if data_proc_fn is not None:
        vol_data = data_proc_fn(vol_data)
    ...
    while 1:
        ...
        for _ in range(nb_patches_per_vol):
            if batch_idx == -1:
                ...
                ext_data_batch = [
                    [*ext_data_batch[f], this_ext_data[f]]
...
def img_seg(
    ...
    **kwargs
    ...
    while 1:
        input_vol = np.vstack(
            [next(img_gen).astype("float16") / 255 for i in range(batch_size)]
        )
        input_vol = np.expand_dims(input_vol, axis=-1)

        output_vols = [
            np_utils.to_categorical(next(seg_gen).astype("int8"), num_classes=2)
...
def _get_file_list(volpath, ext=None, vol_rand_seed=None):
    ...
    """
    files = [
        f for f in sorted(os.listdir(volpath)) if ext is None or f.endswith(ext)
    ]
