...
class SpatialTransformer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.is_affine = len(trf_shape) == 1 or (
            len(trf_shape) == 2
            and all([f == (self.ndims + 1) for f in trf_shape])
        )
    ...
    def call(self, inputs):
        ...
        # check shapes
        assert len(inputs) == 2, "inputs has to be len 2, found: %d" % len(
            inputs
        )
        vol = inputs[0]
        trf = inputs[1]
        ...
        # map transform across batch
        return tf.map_fn(
            self._single_transform, [inputs[0], trf], dtype=tf.float32
        )
    ...
    def _single_aff_to_shift(self, trf, volshape):
        ...
        trf += tf.eye(self.ndims + 1)[
            : self.ndims, :
        ]  # add identity, hence affine is a shift from identitiy
...
class LocalBiasLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        super(LocalBiasLayer, self).build(
            input_shape
...
class LocalLinearLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        super(LocalLinearLayer, self).build(
            input_shape
        )  # Be sure to call this somewhere!
...
class LocallyConnected3D(Layer):
    ...
    def local_conv3d(
        ...
        for i in range(output_row):
            for j in range(output_col):
                for k in range(output_z):
                    slice_row = slice(
                        i * stride_row, i * stride_row + kernel_size[0]
                    )
                    slice_col = slice(
                        j * stride_col, j * stride_col + kernel_size[1]
                    )
        ...
        output = K.reshape(
            output, (output_row, output_col, output_z, -1, filters)
        )
