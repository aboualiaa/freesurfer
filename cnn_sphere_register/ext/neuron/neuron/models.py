...
def conv_enc(
    ...
    for level in range(nb_levels):
            ...
            if (
                ...
                if conv_dropout > 0:
                    ...
                    last_tensor = KL.Dropout(
                        conv_dropout, noise_shape=noise_shape
                    )(last_tensor)
        ...
        if batch_norm is not None:
            ...
            last_tensor = KL.BatchNormalization(axis=batch_norm, name=name)(
                last_tensor
            )
        ...
        if level < (nb_levels - 1):
            ...
            last_tensor = maxpool(
                pool_size=pool_size, name=name, padding=padding
            )(last_tensor)
...
def conv_dec(
    ...
    for level in range(nb_levels - 1):
        ...
        conv_kwargs["dilation_rate"] = dilation_rate_mult ** (
            nb_levels - 2 - level
        )
    ...
    if final_pred_activation == "softmax":
        print(
            "using final_pred_activation %s for %s"
...
def add_prior(
    ...
    if use_logp:
        ...
        print(
            "Breaking change: use_logp option now requires log input!",
            file=sys.stderr,
        )
        merge_op = KL.add
...
def single_ae(
    ...
    if input_model is None:
        ...
        ), "input_shape of input_model is necessary"
        input_tensor = KL.Input(shape=input_shape, name=input_name)
        last_tensor = input_tensor
    else:
        input_tensor = input_model.input
    ...
    if ae_type == "dense":
        assert (
            len(enc_size) == 1
        ), "enc_size should be of length 1 for dense layer"
        ...
        if (
            list(enc_size)[:-1] != list(input_shape)[:-1]
            and all([f is not None for f in input_shape[:-1]])
            and all([f is not None for f in enc_size[:-1]])
        ):

            assert (
                len(enc_size) - 1 == 2
            ), "Sorry, I have not yet implemented non-2D resizing -- need to check out interpn!"
            name = "%s_ae_mu_enc_conv" % (prefix)
            last_tensor = convL(
                enc_size[-1], conv_size, name=name, **conv_kwargs
            )(pre_enc_layer)
            ...
            enc_size[-1] is None
        ):  # convolutional, but won't tell us bottleneck
            name = "%s_ae_mu_enc" % (prefix)
    ...
    if ae_type == "dense":
        ...
        name = "%s_ae_%s_dec" % (prefix, ae_type)
        last_tensor = convL(
            input_nb_feats, conv_size, name=name, **conv_kwargs
        )(last_tensor)
...
def design_dnn(
    ...
    for level in range(nb_levels):
        for conv in range(nb_conv_per_level):
            ...
            nb_lvl_feats = np.round(nb_features * feat_mult ** level).astype(
                int
            )
        ...
        if use_strided_convolution_maxpool:
            ...
            enc_tensors[name] = convL(
                nb_lvl_feats, pool_size, **conv_kwargs, name=name
            )(last_tensor)
            last_tensor = enc_tensors[name]
        else:
            name = "%s_maxpool_%d" % (prefix, level)
    ...
    if final_layer == "dense-sigmoid":
        ...
        name = "%s_dense" % prefix
        enc_tensors[name] = KL.Dense(1, name=name, activation="sigmoid")(
            last_tensor
        )
        ...
        enc_tensors[name] = KL.Activation(activation="tanh", name=name)(
            last_tensor
        )
        ...
        enc_tensors[name] = KL.Dense(
            nb_labels, name=name, activation="softmax"
        )(last_tensor)
    ...
    # global max pooling layer
        ...
        enc_tensors[name] = KL.BatchNormalization(axis=batch_norm, name=name)(
            last_tensor
        )
        last_tensor = enc_tensors[name]
        ...
        enc_tensors[name] = KL.Conv1D(
            1, 1, name=name, activation="sigmoid", use_bias=True
        )(last_tensor)

    elif final_layer == "globalmaxpooling":

        name = "%s_conv_to_featmaps" % prefix
        enc_tensors[name] = KL.Conv3D(2, 1, name=name, activation="relu")(
            last_tensor
        )
        last_tensor = enc_tensors[name]
    ...
    # create the model
    model = Model(
        inputs=[enc_tensors["%s_input" % prefix]],
        outputs=[last_tensor],
        name=model_name,
    )
    return model
...
class _VAESample:
    ...
    def sample_z(self, args):
        ...
        shape = K.shape(mu)
        eps = K.random_normal(shape=shape, mean=0.0, stddev=1.0)
