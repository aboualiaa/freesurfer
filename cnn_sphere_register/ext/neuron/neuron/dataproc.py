    ...
    for fileidx in tqdm(range(len(files)), ncols=80):
        ...
        if slice_nrs is None:
            slice_nrs_sel = range(
                ...
                vol_data.shape[dim_idx]
                - slice_pad
                - vol_inner_pad_for_slice_nrs,
            )
        else:
...
def vol_proc(
    ...
    if resize_shape is not None and resize_shape != vol_data.shape:
        ...
        if resize_shape[-1] is None:
            ...
            resize_shape[-1] = np.round(
                resize_ratio * vol_data.shape[-1]
            ).astype("int")
...
def prior_to_weights(
    ...
    if verbose:
        ...
        ax3.bar(
            range(weights.size), np.log((weights)) - np.min(np.log((weights)))
        )
...
def ml_split(
    ...
    for cat_idx, cat in enumerate(cat_titles):
        ...
        cat_subj_idx = subj_order[
            cat_subj_start[cat_idx] : nb_cat_subj[cat_idx]
        ]
