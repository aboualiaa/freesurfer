...
def interpn(vol, loc, interp_method="linear"):
    ...
    if interp_method == "linear":
        ...
        loc1 = [
            tf.clip_by_value(loc0[d] + 1, 0, max_loc[d]) for d in range(nb_dims)
        ]
        locs = [loc0, loc1]
        ...
        for ci, c in enumerate(cube_pts):
            ...
            wlm = tf.stack(
                [weights_loc[c[d]][d] for d in range(nb_dims)], axis=0
            )
    ...
    return (
        interp_vol  # tf.reshape(interp_vol, loc.shape[:-1] + [vol.shape[-1]])
    )
    ...
    if method in ["ss", "scaling_and_squaring"]:
        ...
        if time_dep:
            ...
            for _ in range(nb_steps):
                svec = svec[0::2] + tf.map_fn(
                    transform, svec[1::2, :], svec[0::2, :]
                )
        ...
        assert (
            not time_dep
        ), "odeint not implemented with time-dependent vector field"
        fn = lambda disp, _: transform(vec, disp)

        # process time point.
        out_time_pt = (
            kwargs["out_time_pt"] if "out_time_pt" in kwargs.keys() else 1
        )
...
def gaussian_kernel(sigma, windowsize=None):
    ...
    exp_term = [
        -K.square(diff[f]) / (2 * (sigma[f] ** 2)) for f in range(nb_dims)
    ]
    norms = [
        exp_term[f] - np.log(sigma[f] * np.sqrt(2 * np.pi))
...
def stack_models(models, connecting_node_ids=None):
    ...
    for mi in range(1, len(models)):
        ...
        if connecting_node_ids is None:
            ...
            assert len(new_input_nodes) == len(
                models[mi - 1].outputs
...
def mod_submodel(orig_model, new_input_nodes=None, input_layers=None):
    ...
    def _layer_dependency_dict(orig_model):
        ...
        node_list = [
            ol._inbound_nodes[out_node_idx[i]]
        ...
        while len(node_list) > 0:
            ...
            if len(dct.setdefault(node.outbound_layer, [])) > 0:
                for li, layers in enumerate(dct[node.outbound_layer]):
                    if (
                        layers == node.inbound_layers
                        and dct_node_idx[node.outbound_layer][li]
                        == node.node_indices
                    ):
                        add = False
                        break
            if add:
                ...
                dct_node_idx.setdefault(node.outbound_layer, []).append(
                    node.node_indices
                )
    ...
    assert len(output_layers) == len(
        orig_model.outputs
    ), "Number of output layers don't match"
...
def reset_weights(model, session=None):
    ...
    for layer in model.layers:
        ...
        if not reset:
            print(
                "Could not find initializer for layer %s. skipping", layer.name
            )
...
def predict_volumes(
    ...
    with timer.Timer("predict_volume_stack", verbose):
        vol_stack = predict_volume_stack(
            models, data_generator, batch_size, grid_size, verbose
        )
    if len(models) == 1:
    ...
    for midx, _ in enumerate(models):
        ...
        vol_true_label = _quilt(all_true_label, *args, **label_kwargs).astype(
            "int"
        )
        vol_pred_label = _quilt(all_pred_label, *args, **label_kwargs).astype(
            "int"
        )
        ...
        if do_extra_vol:
            ...
            if do_prior:
                ...
                vol_prior_label = _quilt(
                    all_prior_label, *args, **label_kwargs
                ).astype("int")
...
def predict_volume_stack(
    models, data_generator, batch_size, grid_size, verbose=False
):
    """
    ...
    for batch_idx in batch_gen:
        ...
        if batch_idx == 0:
            ...
            all_true = [
                np.zeros((nb_patches, nb_vox * nb_labels)) for f in models
            ]
            all_pred = [
                np.zeros((nb_patches, nb_vox * nb_labels)) for f in models
            ]
...
def next_pred_label(model, data_generator, verbose=False):
    ...
    with timer.Timer("prediction", verbose):
        pred = model.predict(sample[0])
    sample_input = (
        sample[0] if not isinstance(sample[0], (list, tuple)) else sample[0][0]
    )
...
def _quilt(
    patches, patch_size, grid_size, patch_stride, verbose=False, **kwargs
):
    ...
    # quilt
    quilted_vol = pl.quilt(
        patches, patch_size, grid_size, patch_stride=patch_stride, **kwargs
    )
    assert quilted_vol.ndim == len(
        patch_size
    ), "problem with dimensions after quilt"
