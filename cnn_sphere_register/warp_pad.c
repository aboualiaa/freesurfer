...
float resampleatpolarcoord(float **mtx, float theta, float phi) {
  ...
  // val = du * dv * *MATRIX_RELT(mtx, v1, u1) + (1.0f - du) * dv *
  // *MATRIX_RELT(mtx, v1, u0) +
  //       (1.0f - du) * (1.0f - dv) * *MATRIX_RELT(mtx, v0, u0) + du * (1.0f -
  //       dv) * *MATRIX_RELT(mtx, v0, u1);

  val = du * dv * mtx[v1][u1] + (1.0f - du) * dv * mtx[v1][u0] +
        (1.0f - du) * (1.0f - dv) * mtx[v0][u0] +
        du * (1.0f - dv) * mtx[v0][u1];
...
float **readtxt(const char *fname, int rows, int cols) {
  ...
  int i;
  for (i = 0; i < rows; i++)
    mtx[i] = (float *)calloc(cols, sizeof(float));
...
int main(int argc, char *argv[]) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    } else // high-latitude region
    {
      ...
      if (newtheta1 < 0.0f)
        newtheta1 = 2.0f * M_PI + newtheta1;
      else if (newtheta1 >= 2.0f * M_PI)
        newtheta1 -= 2.0f * M_PI;
