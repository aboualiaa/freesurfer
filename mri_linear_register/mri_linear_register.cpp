...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_linear_register.c,v 1.13 2011/03/02 00:04:22 nicks Exp $",
      "$Name:  $");
  ...
  if (xform_mean_fname) {
    ...
    if (!parms.m_xform_mean)
      ErrorExit(Gerror, "%s: could not read parameter means from %s", Progname,
                xform_mean_fname);
    ...
    for (sno = 0; sno < nsubjects; sno++) {
      parms.m_xforms[sno] = MatrixAsciiReadFrom(fp, nullptr);
      if (!parms.m_xforms[sno])
        ErrorExit(ERROR_NOMEMORY, "%s: could not allocate %dth xform", Progname,
                  sno);
    }
    parms.m_xform_covariance = MatrixAsciiReadFrom(fp, nullptr);
  ...
  if (var_fname) /* read in a volume of standard deviations */
  {
    ...
    if (!mri_var)
      ErrorExit(ERROR_NOFILE, "%s: could not open variance volume %s.\n",
                Progname, var_fname);
    mri_tmp = MRIconcatenateFrames(mri_ref, mri_var, nullptr);
    MRIfree(&mri_var);
    MRIfree(&mri_ref);
    mri_ref = mri_tmp;
  }
  ...
  if (mri_in->width != mri_ref->width || mri_in->height != mri_ref->height ||
    ...
    mri_tmp =
        MRIallocSequence(width, height, depth, MRI_UCHAR, mri_ref->nframes);
    MRIextractInto(mri_ref, mri_tmp, 0, 0, 0, mri_ref->width, mri_ref->height,
                   mri_ref->depth, 0, 0, 0);
#if 0
    MRIfree(&mri_ref) ;
#else
  ...
  if (!FZERO(rzrot)) {
    ...
    fprintf(stderr, "rotating second volume by %2.1f degrees around Z axis\n",
            (float)DEGREES(rzrot));
  ...
  if (!FZERO(rxrot)) {
    ...
    fprintf(stderr, "rotating second volume by %2.1f degrees around X axis\n",
            (float)DEGREES(rxrot));
  ...
  if (!FZERO(ryrot)) {
    ...
    fprintf(stderr, "rotating second volume by %2.1f degrees around Y axis\n",
            (float)DEGREES(ryrot));
  ...
  if (use_gradient) {
    ...
    mri_grad = MRIsobel(mri_in, nullptr, mri_mag);
    MRIfree(&mri_grad);

    /* convert it to ubytes */
    MRIvalScale(mri_mag, mri_mag, 0.0f, 255.0f);
    mri_in_mag = MRIclone(mri_in, nullptr);
    ...
    mri_mag =
        MRIalloc(mri_ref->width, mri_ref->height, mri_ref->depth, MRI_FLOAT);
    MRIcopyHeader(mri_ref, mri_mag);
    mri_grad = MRIsobel(mri_ref, nullptr, mri_mag);
    MRIfree(&mri_grad);

    /* convert it to ubytes */
    MRIvalScale(mri_mag, mri_mag, 0.0f, 255.0f);
    mri_ref_mag = MRIclone(mri_ref, nullptr);
  ...
  if (voxel_coords) {
    ...
    MRIrasXformToVoxelXform(mri_in_orig, mri_ref_orig, parms.lta->xforms[0].m_L,
                            parms.lta->xforms[0].m_L);
    if (Gdiag & DIAG_WRITE) {
      MRI *mri_tmp;

      mri_tmp =
          MRIlinearTransform(mri_in_orig, nullptr, parms.lta->xforms[0].m_L);
  ...
  if (invert_flag) {
    ...
    m_tmp = MatrixInverse(parms.lta->xforms[0].m_L, nullptr);
...
#if 1
static int register_mri(MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms,
                        MATRIX *m_L) {
  ...
  if (window_size > 0) {
    ...
    mri_in_windowed =
        MRIwindow(mri_in, nullptr, WINDOW_HANNING, nint(in_means[0]),
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    ...
    mri_aligned =
        MRIapplyRASlinearTransform(mri_in, nullptr, parms->lta->xforms[0].m_L);
...
static MATRIX *compute_pca(MRI *mri_in, MRI *mri_ref) {
  ...
  for (i = 1; i <= 3; i++)
    fprintf(stderr, "\t\t%2.2f    %2.2f    %2.2f\n", m_ref_evectors->rptr[i][1],
            m_ref_evectors->rptr[i][2], m_ref_evectors->rptr[i][3]);

  fprintf(stderr, "\nin_evectors = \n");
  for (i = 1; i <= 3; i++)
    fprintf(stderr, "\t\t%2.2f    %2.2f    %2.2f\n", m_in_evectors->rptr[i][1],
            m_in_evectors->rptr[i][2], m_in_evectors->rptr[i][3]);
...
static int order_eigenvectors(MATRIX *m_src_evectors, MATRIX *m_dst_evectors) {
  ...
  for (row = 1; row <= 3; row++) {
    *MATRIX_RELT(m_dst_evectors, row, 1) =
        *MATRIX_RELT(m_src_evectors, row, xcol);
    *MATRIX_RELT(m_dst_evectors, row, 2) =
        *MATRIX_RELT(m_src_evectors, row, ycol);
    *MATRIX_RELT(m_dst_evectors, row, 3) =
        *MATRIX_RELT(m_src_evectors, row, zcol);
  }
  return (NO_ERROR);
}
...
#define MAX_Z_ANGLE (RADIANS(15))
  if (fabs(x_angle) > MAX_X_ANGLE || fabs(y_angle) > MAX_Y_ANGLE ||
    ...
    fprintf(stderr,
            "eigenvector swap detected (%2.0f, %2.0f, %2.0f): ignoring "
            "rotational PCA...\n",
            DEGREES(x_angle), DEGREES(y_angle), DEGREES(z_angle));

    m_I = MatrixIdentity(3, nullptr);
    MatrixCopy(m_I, mRot);
    MatrixFree(&m_I);
...
static int register_mri(MRI *mri_in, MRI *mri_ref, MORPH_PARMS *parms) {
  ...
  {
    if (MRIfindNeck(mri_in_red, mri_in_red, thresh_low, thresh_hi, NULL, -1,
                    NULL) == NULL)
...
static MATRIX *initialize_transform(MRI *mri_in, MRI *mri_ref, MP *parms) {
  ...
  if (Gdiag & DIAG_WRITE && parms->write_iterations > 0) {
    ...
    mri_aligned = MRIapplyRASlinearTransform(parms->mri_in, nullptr, m_L);
