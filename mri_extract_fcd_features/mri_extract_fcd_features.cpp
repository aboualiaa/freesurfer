 ...
 * @brief program to extract features from a recon dir to use as input to an FCD
 * classifier.
 *
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_extract_fcd_features.c,v 1.1 2016/06/15 17:51:09 fischl Exp $",
      "$Name:  $");
  ...
  if (false)
    ...
    MRI *mri_ohemi_features, *mri_ohemi_mapped_to_hemi_features,
        *mri_flair_features, *mri_ohemi_flair_features,
        *mri_ohemi_mapped_to_hemi_flair_features;
    char fname[STRLEN], ext[STRLEN], fname_no_ext[STRLEN];

    mri_features = MRIcomputeSurfaceDistanceIntensities(
        mris, mri_ribbon, mri_aparc, mri, mri_aseg, whalf);
    mri_ohemi_features = MRIcomputeSurfaceDistanceIntensities(
    ...
    mri_flair_features = MRIcomputeFlairRatio(mris, mri_ribbon, mri_aparc, mri,
                                              mri_flair, mri_aseg, whalf);
    mri_ohemi_flair_features = MRIcomputeFlairRatio(
  ...
  if (Gdiag_no >= 0)
    printf("feature(%d) = %f\n", Gdiag_no,
           MRIgetVoxVal(mri_features, Gdiag_no, 0, 0, 0));
...
static void usage_exit(int code) {
  ...
  printf("\tsdir <subjects dir> - specify SUBJECTS_DIR on the command line "
         "instead of in env\n");
  exit(code);
}
...
#define SKIP_LABEL(l)                                                          \
  (IS_CEREBELLAR_GM(l) || IS_VENTRICLE(l) || l == Brain_Stem || IS_CC(l) ||    \
   IS_WMSA(l) || IS_CAUDATE(l) || IS_AMYGDALA(l) || IS_HIPPO(l))
...
static MRI *MRIcomputeSurfaceDistanceProbabilities(MRI_SURFACE *mris,
  ...
  HISTO2Dinit(hw, NBINS, nwhite_bins, 0, NBINS - 1, min_white_dist,
              max_white_dist);
  HISTO2Dinit(hp, NBINS, npial_bins, 0, NBINS - 1, min_pial_dist,
              max_pial_dist);
  ...
  for (x = 0; x < mri->width; x++)
    for (y = 0; y < mri->height; y++)
      for (z = 0; z < mri->depth; z++) {
        ...
        if (IS_CEREBELLAR_GM(label) || IS_VENTRICLE(label) ||
            label == Brain_Stem || IS_CC(label))
          continue;
...
static MRI *MRIcomputeSurfaceDistanceIntensities(MRI_SURFACE *mris,
  ...
  mri_features = MRIallocSequence(
      mris->nvertices, 1, 1, MRI_FLOAT,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISsurfaceRASToVoxelCached(mris, mri_aseg, x + v->nx, y + v->ny, z + v->nz,
                                &snx, &sny, &snz);
    snx -= xl;
    sny -= yl;
    snz -= zl;
...
static MRI *MRIcomputeFlairRatio(MRI_SURFACE *mris, MRI *mri_ribbon,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISsurfaceRASToVoxelCached(mris, mri_dist_white, v->whitex, v->whitey,
                                v->whitez, &x0, &y0, &z0);
    MRISsurfaceRASToVoxelCached(mris, mri_dist_white, xs, ys, zs, &xd, &yd,
                                &zd);
    MRIsampleVolumeGradient(mri_dist_white, xd, yd, zd, &dx, &dy, &dz);
    dx *= -1;
    dy *= -1;
    dz *= -1;
    ...
    if (dist > -SAMPLE_DIST / 2) // white matter is too thin here
    {
      if (vno == Gdiag_no)
        printf(
            "vno %d: white dist %2.2f not close enough to sample dist %2.2f\n",
    ...
    MRIsampleVolume(mri, xf, yv, zf, &T1);
    T1 = 1;
    if (FZERO(T1))
      continue;
