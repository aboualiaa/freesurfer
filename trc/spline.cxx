...
vector<float> step(3, 0);
  ...
  for (auto ipt = InPoints.begin(); ipt < InPoints.end();
       ipt += 3) {
...
const int mingap = (int)ceil(seglen / 2);
  ...
  vector<int> peeksegs;
  vector<float> peekcurvs;
vector<float> arcparam(npts);
  vector<vector<int>::const_iterator> peekpts;
vector<vector<int>::const_iterator> dompts(mNumControl);
...
const float mincurv = accumulate(mCurvature.begin() + mingap,
...
vector<int>::const_iterator iptL = dompts.at(iseg);
vector<int>::const_iterator iptR = dompts.at(iseg + 1);
...
float curvR;
float dshape;
float dshapemin = numeric_limits<double>::infinity();
      vector<int>::const_iterator iptL = dompts.at(kdom - 1);
vector<int>::const_iterator iptR = dompts.at(kdom + 1);
      auto icurv =
      ...
      for (auto ipt = iptL; ipt <= iptR; ipt += 3) {
        dshape =
            fabs((lenL - lenR) / static_cast<float>(lentot) + (curvL - curvR) / curvtot);
  ...
  for (int kdom = 1; kdom < mNumControl; kdom++) {
    const int seglen = static_cast<int>(dompts.at(kdom) - dompts.at(kdom - 1)) / 3;
...
unsigned int Spline::PointToSegment(unsigned int PointIndex) {
  ...
  for (auto iarc = mArcLength.begin() + PointIndex;
...
void Spline::ComputeTangent(const bool DoAnalytical) {
  ...
  for (auto itang = mTangent.begin(); itang < mTangent.end();
       itang += 3) {
...
void Spline::ComputeNormal(const bool DoAnalytical) {
  vector<float>::const_iterator id2;
vector<float>::const_iterator itang = mTangent.begin();
  ...
  for (auto inorm = mNormal.begin(); inorm < mNormal.end();
       inorm += 3) {
...
vector<float>::const_iterator id2;
  ...
  // Curvature = |r' x r''| / |r'|^3
  for (auto icurv = mCurvature.begin();
...
void Spline::WriteVolume(const char *VolumeFile, const bool ShowControls) {
  if (ShowControls) {
    for (auto icpt = mControlPoints.begin();
...
void Spline::WriteValues(vector<MRI *> &ValueVolumes, const char *TextFile) {
  ...
  for (auto ipt = mAllPoints.begin();
    ...
    for (auto ivol = ValueVolumes.begin();
...
const float d = .5 * (t3 - t2);
  ...
  for (int k = 0; k < 3; k++) {
    InterpPoint[k] = static_cast<int>(round(a * ControlPoint1[k] + b * ControlPoint2[k] +
...
const float ds = smax / (npts - 1);
  ...
  for (auto iarc = arcparam.begin(); iarc < arcparam.end();
       iarc++) {
    *iarc = s;
    s += ds;
  }
