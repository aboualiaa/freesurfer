...
int main(int argc, char **argv) {
  ...
  float distmin, darc, arcmin, arcmax, arc1m, arc2m, arc1s, arc2s, lthresh1,
      lthresh2, uthresh1, uthresh2;
    ...
    for (vector<vector<float>>::const_iterator jallp = allpaths.begin();
      ...
      if (jallp != iallp) {
        ...
        for (vector<unsigned int>::const_iterator jknot = jallk->begin();
          ...
          const unsigned int ioffset = (*iknot) * 3, joffset = (*jknot) * 3;
  ...
  for (vector<vector<float>>::const_iterator iallp = allpaths.begin();
       iallp < allpaths.end(); iallp++) {
    float dmin = numeric_limits<float>::infinity(), arc0 = 0;
    ...
    for (vector<float>::const_iterator ipath = iallp->begin();
      ...
      const float dx = ipath[0] - icenter[0], dy = ipath[1] - icenter[1],
  ...
  for (vector<vector<float>>::const_iterator ialla = allarc.begin();
    ...
    for (unsigned int ilen = 0; ilen < narc; ilen++) {
      float                         slope;
      vector<float>::const_iterator iarc = ialla->begin(), imeas1, imeas0;
