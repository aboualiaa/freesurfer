...
bool CTrackReader::Open(const char *filename, TRACK_HEADER *header) {
  ...
  if (m_header.image_orientation_patient[0] == 0 &&
    ...
    m_header.image_orientation_patient[0] =
...
bool CTrackReader::GetNumberOfTracks(int *cnt) {
  ...
  if (m_header.n_count == 0) {
    ...
    fseek(m_pFile,
          m_bOldFormat ? 3 * (sizeof(int) + sizeof(float))
                       : sizeof(TRACK_HEADER),
          SEEK_SET);
    ...
    *cnt = 0;
    while (GetNextPointCount(&n)) {
      *cnt += 1;
      fseek(m_pFile,
            sizeof(float) *
                (n * (3 + m_header.n_scalars) + m_header.n_properties),
            SEEK_CUR);
    }
...
bool CTrackWriter::Initialize(const char *filename, short int *dim,
