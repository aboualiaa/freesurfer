...
int QdecProject::LoadProjectFile(const char *ifnProject,
  ...
  this->FormatCommandString(fnProject.c_str(), fnExpandedProjectBase.c_str(),
                            ifnDataDir, msUnzipCommandFormat.c_str(), sCommand);
  ...
  if (1 != version) {
    fprintf(stderr,
            "ERROR: QdecProject::LoadProjectFile: Version "
  ...
  for (int nToken = 0; nToken < 3; nToken++) {
    ...
    if (sToken != asCorrectTokens[nToken]) {
      fprintf(stderr,
              "ERROR: QdecProject::LoadProjectFile: Invalid "
  ...
  while (fMetadata >> sToken && !fMetadata.eof()) {
    if (sToken == "Subject")
      fMetadata >> sSubject;
    else if (sToken == "Hemisphere")
      fMetadata >> sHemisphere;
    else if (sToken == "AnalysisName")
      fMetadata >> sAnalysisName;
    else if (sToken == "DataTable")
      fMetadata >> fnDataTableBase;
    else if (sToken == "Measure")
      fMetadata >> sMeasure;
    else if (sToken == "Smoothness")
      fMetadata >> smoothness;
    else if (sToken == "DiscreteFactor1")
      fMetadata >> sDiscreteFactor1;
    else if (sToken == "DiscreteFactor2")
      fMetadata >> sDiscreteFactor2;
    else if (sToken == "ContinuousFactor1")
      fMetadata >> sContinuousFactor1;
    else if (sToken == "ContinuousFactor2")
      fMetadata >> sContinuousFactor2;
    else {
      fprintf(stderr,
              "WARNING: QdecProject::LoadProjectFile: Unrecognized "
  ...
  errorCode = this->mGlmDesign->Create(
      this->mDataTable, sAnalysisName.c_str(), sDiscreteFactor1.c_str(),
      sDiscreteFactor2.c_str(), sContinuousFactor1.c_str(),
      sContinuousFactor2.c_str(), nullptr, 0, sMeasure.c_str(),
...
int QdecProject::SaveProjectFile(const char *ifnProject,
  ...
  */
  ...
  // Surfaces
  sCommand = "ln -s " + fnSubjectsDir + "/" + sSubjectName + "/surf/*.curv " +
...
int QdecProject::LoadDataTable(const char *isFileName) {
  ...
  if (ret)
    return ret;
  if (strlen(subjectsDir) > 0)
    ret = this->SetSubjectsDir(subjectsDir);
  if (ret)
    return ret;
  delete this->mGlmDesign;
...
int QdecProject::VerifySubjects() {
  ...
  for (unsigned int i = 0; i < theSubjects.size(); i++) {
    ...
    if (0 != rSystem) {
      fprintf(stderr,
              "ERROR: QdecProject::VerifySubjects: Couldn't "
...
bool QdecProject::HaveDataTable() {
  if (this->mDataTable && (this->mDataTable->GetNumberOfSubjects() > 0))
    return true;
 ...
 */
QdecDataTable *QdecProject::GetDataTable() { return this->mDataTable; }
 ...
 */
int QdecProject::CreateGlmDesign(
    const char *isName, const char *isFirstDiscreteFactor,
    const char *isSecondDiscreteFactor, const char *isFirstContinuousFactor,
    const char *isSecondContinuousFactor, const char **isNuisanceFactors,
    int iNumNuisanceFactors, const char *isMeasure, const char *isHemi,
    int iSmoothnessLevel, ProgressUpdateGUI *iProgressUpdateGUI) {

  int errorCode;
  errorCode = this->mGlmDesign->Create(
      this->mDataTable, isName, isFirstDiscreteFactor, isSecondDiscreteFactor,
      isFirstContinuousFactor, isSecondContinuousFactor, isNuisanceFactors,
      iNumNuisanceFactors, isMeasure, isHemi, iSmoothnessLevel,
      iProgressUpdateGUI);
  if (errorCode)
    return errorCode;
...
int QdecProject::CreateGlmDesign(const char * isName,
  ...
  errorCode = this->mGlmDesign->Create(
      this->mDataTable, isName, isFirstDiscreteFactor, isSecondDiscreteFactor,
      isFirstContinuousFactor, isSecondContinuousFactor, isNuisanceFactors,
      iNumNuisanceFactors, isMeasure, nullptr, 0, iProgressUpdateGUI);
  if (errorCode)
    return errorCode;
 ...
 * @return int
 */
int QdecProject::RunGlmFit() { return this->mGlmFitter->Run(mGlmDesign); }
...
int QdecProject::GenerateMappedLabelForAllSubjects(
  ...
  for (int i = 0; i < numSubjects; i++) {
    ...
    ssCommand << "mri_label2label"
              ...
              << " --trglabel " << ifnLabel << " --regmethod surface"
              << " --hemi " << this->GetHemi();
    ...
    if (iProgressUpdateGUI) {
      ...
      iProgressUpdateGUI->UpdateProgressPercent((float)nStep++ * stepIncrement);
    }
 ...
 */
QdecGlmDesign *QdecProject::GetGlmDesign() { return this->mGlmDesign; }

const char *QdecProject::GetMetadataFileName() const {
    ...
    for (p = 0; p < parc.size(); p++) // for each parcellation type
    {
      for (m = 0; m < meas.size(); m++) // for each measure
      {
        ...
        ssCommand << "aparcstats2table"
                  ...
                  << this->msStatsDataTablesDir << "/" << ssFname.str()
                  << ".stats.dat"
                  << " --subjects";
        for (int i = 0; i < numSubjects; i++) {
          ssCommand << " " << subjects[i];
...
int QdecProject::RunMonteCarloSimulation(const char * isThreshold,
  ...
  ssCommand << "mri_surfcluster"
            ...
            << " --cwsig " << ssClusterSigFileName.str().c_str() << " --vwsig "
            << sWorkDir << "/" << isContrast << "/mc-z." << isSign << "."
            << isThreshold << ".sig.vertex.mgh"
            << " --sum " << sWorkDir << "/" << isContrast << "/mc-z." << isSign
            << "." << isThreshold << ".sig.cluster.summary"
            << " --ocn " << sWorkDir << "/" << isContrast << "/mc-z." << isSign
            << "." << isThreshold << ".sig.ocn.mgh"
            << " --oannot " << sWorkDir << "/" << isContrast << "/mc-z."
            << isSign << "." << isThreshold << ".sig.ocn.annot"
            << " --csdpdf " << sWorkDir << "/" << isContrast << "/mc-z."
            << isSign << "." << isThreshold << ".pdf.dat"
            << " --annot aparc"
