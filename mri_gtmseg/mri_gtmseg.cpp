...
int main(int argc, char *argv[]) {
  ...
  ctMaster = TissueTypeSchema(nullptr, "default-apr-2019+head");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      nargsused = 1;
    } else if (!strcasecmp(option, "--keep-hypo"))
      gtmseg->KeepHypo = 1;
    else if (!strcasecmp(option, "--no-keep-hypo"))
      gtmseg->KeepHypo = 0;
    else if (!strcasecmp(option, "--keep-cc"))
      gtmseg->KeepCC = 1;
    else if (!strcasecmp(option, "--no-keep-cc"))
      gtmseg->KeepCC = 0;
...
static void print_usage() {
  ...
  printf("   --internal-usf USF : upsampling factor (default %d)\n",
         gtmseg->USF);
  printf("   --apas apasfile : defines extra-cerebral and subcortical "
         "segmentations (%s)\n",
         gtmseg->apasfile);
  printf("   --ctx-annot annot lhbase rhbase : use annot to segment cortex "
         "(%s,%d,%d)\n",
         gtmseg->ctxannotfile, gtmseg->ctxlhbase, gtmseg->ctxrhbase);
  printf("   --subseg-wm : turn on segmenting of WM into smaller parts (off by "
         "default)\n");
  ...
  printf("   --dmax dmax : distance from ctx for wmseg to be considered "
         "'unsegmented' (%f)\n",
         gtmseg->dmax);
  printf("   --keep-hypo : do not convert WM hypointensities to a white matter "
         "label \n");
  printf("   --keep-cc : do not convert corpus callosum to a white matter "
         "label \n");
  printf("   --ctab ctab.lut : copy items in ctab.lut into master ctab merging "
         "or overwriting what is there \n");
  printf("   --lhminmax lhmin lhmax : for defining ribbon in apas (default: %d "
         "%d) \n",
         gtmseg->lhmin, gtmseg->lhmax);
  printf("   --rhminmax rhmin rhmax : for defining ribbon in apas (default: %d "
         "%d) \n",
         gtmseg->rhmin, gtmseg->rhmax);
  printf("   --output-usf OutputUSF : set actual output resolution. Default is "
         "to be the same as the --internal-usf");
  printf("\n");
#ifdef HAVE_OPENMP
...
static void print_help() {
  print_usage();
  printf("This program creates a segmentation that can be used with the "
...
#pragma omp parallel for if_ROMP(experimental)
  ...
  ROMP_PF_end MRIwrite(wm, "wm0.mgh");
...
#pragma omp parallel for if_ROMP(experimental)
    ...
    for (r = 0; r < seg->height; r++) {
      for (s = 0; s < seg->depth; s++) {
        ...
        val = MRIgetVoxVal(seg, c, r, s, 0);
        if (val == 2)
          MRIsetVoxVal(outseg, c, r, s, 0, 5001);
        if (val == 41)
          MRIsetVoxVal(outseg, c, r, s, 0, 5002);
      }
    }
    ROMP_PFLB_end
  }
