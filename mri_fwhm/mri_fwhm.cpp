...
MRI *MRImaskedGaussianSmoothTo(MRI *invol, MRI *mask, double ToFWHM, double tol,
                               ...
                               double *pToFWHMActual, int *niters, MRI *outvol);
...
MRI *MRIbinarize2(MRI *mri_src, MRI *mri_dst, double threshold, double low_val,
                  double hi_val);
...
int main(int argc, char *argv[]) {
  ...
  if ((nframes < 0 && synth) || !synth)
    nframes = InVals->nframes;
  ...
  if (synth) {
    ...
    MRIfree(&InVals);
    MRIrandn(mritmp->width, mritmp->height, mritmp->depth, nframes, 0, 1,
             mritmp);
    InVals = mritmp;
  }
  ...
  if (mask) {
    ...
    //---- Save mask -----
    if (outmaskpath)
      MRIwrite(mask, outmaskpath);
  } else
    nsearch = InVals->width * InVals->height * InVals->depth;
  printf("Search region is %d voxels = %lf mm3\n", nsearch,
         nsearch * voxelvolume);

  if (DoMedian == 0) {
    if ((infwhm > 0 || infwhmc > 0 || infwhmr > 0 || infwhms > 0 || mb2drad ||
         mb2dtan) &&
        SmoothOnly) {
      if (SaveUnmasked)
        mritmp = nullptr;
  ...
  if (DetrendOrder >= 0) {
    ...
    if (DetrendOrder >= 1)
      for (n = 0; n < Ntp; n++)
        X->rptr[n + 1][2] = (n - ftmp) / ftmp;
    if (DetrendOrder >= 2)
      for (n = 0; n < Ntp; n++)
        X->rptr[n + 1][3] = pow((n - ftmp), 2.0) / (ftmp * ftmp);
  }
  if (X) {
    printf("Detrending\n");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      synth = 1;
    else if (!strcasecmp(option, "--nosynth"))
      synth = 0;
    else if (!strcasecmp(option, "--mask-inv"))
      maskinv = 1;
    else if (!strcasecmp(option, "--save-detrended"))
      SaveDetrended = 1;
    else if (!strcasecmp(option, "--save-unmasked"))
      SaveUnmasked = 1;
    else if (!strcasecmp(option, "--smooth-only"))
      SmoothOnly = 1;
    else if (!strcasecmp(option, "--so"))
      SmoothOnly = 1;
    else if (!strcasecmp(option, "--sqr"))
      DoSqr = 1;
    else if (!strcasecmp(option, "--ispm"))
      InValsType = MRI_ANALYZE_FILE;
    else if (!strcasecmp(option, "--ar2"))
      DoAR2 = 1;
...
static void print_help() {
  ...
  printf(
      "FreeSurfer program to estimate the global Gaussian smoothness of a\n");
  printf("multi-frame, volume-based data set. The smoothness is measured as "
         "the\n");
  printf("Full-Width-Half-Max (FWHM) in mm.  Gaussian stddev =\n");
  printf(
      "fwhm/sqrt(log(256.0)). The voxels used in the fwhm estimation can be\n");
  printf("constrained to be inside of a mask. It is STRONGLY recommended that "
         "a\n");
  ...
  printf("possible. Alternatively, the user can specify a detrending matrix. "
         "The\n");
  ...
  printf("turning this program into a smoother). If smoothing is to be done, "
         "it\n");
  ...
  printf("--o outputvol\n");
  printf("\n");
  printf("Save input after smoothing. See also --save-detended and "
         "--save-unmasked.\n");
  printf("\n");
  printf("--smooth-only\n");
  printf("\n");
  printf("Does not attempt to compute FWHM. Smooths the input, saves to "
         "outputvol,\n");
  printf("and exists. Respects --save-unmasked, but not --save-detended. This "
         "allows\n");
  ...
  printf("--mask maskvol\n");
  printf("\n");
  printf("Compute FWHM only over voxels in the given mask. Format can be "
         "anything\n");
  printf(
      "accepted by mri_convert. If smoothing is to be done, it will only be\n");
  printf("done inside the mask. It is strongly recommended that a masked be "
         "used\n");
  printf("(see also --auto-mask and --save-unmasked).\n");
  printf("\n");
  printf("--mask-thresh thresh\n");
  printf("\n");
  printf("Threshold mask at thresh. Default is 0.5 (ie, it expects a binary "
         "mask).\n");
  printf("\n");
  ...
  printf(
      "Compute a mask based on a fraction (rthresh) of the global mean. If\n");
  printf("rthresh is 0.1, then all voxels in the mean image above "
         "0.1*globalmean\n");
  printf("are in the mask.\n");
  printf("\n");
  printf("--mask-inv\n");
  printf("\n");
  printf("Invert mask, ie, compute FWHM only over voxels outside the given "
         "mask.\n");
  printf("\n");
  printf("--nerode n\n");
  printf("\n");
  printf("Erode mask n times (ie, make it smaller). Occurs after any mask "
         "inversion.\n");
  printf("\n");
  ...
  printf("--fwhm fwhm\n");
  printf("\n");
  printf("Smooth BY fwhm mm before estimating the fwhm. This is mainly good "
         "for\n");
  ...
  printf("--synth\n");
  printf("\n");
  printf("Synthesize input with white gaussian noise. Ten frames are used by "
         "default,\n");
  printf("but this can be changed with --synth-frames. Uses input volume as "
         "template.\n");
  printf("This functionality is useful for degugging. Eg, when using --synth "
         "and --fwhm,\n");
  printf("it should measure the resulting fwhm to be that passed by --fwhm. "
         "Can do\n");
  printf("the same with --to-fwhm.\n");
  printf("\n");
  printf("--synth-frames nframes\n");
  printf("\n");
  printf("Synthesize input with white gaussian noise with the given number of "
         "frames.\n");
  ...
  printf("--inorm\n");
  printf("\n");
  printf("Spatial intensity normalization. Subtract the in-mask mean and "
         "divide by the in-mask \n");
  printf("stddev. \n");
  printf("\n");
  printf("EXAMPLES:\n");
  printf("\n");
  printf("1. Measure the fwhm of an input data set, compute mask automatically "
         "by\n");
  printf("   thresholding input at 20%% of global mean. The time series will "
         "be\n");
  printf(
      "   have its mean removed prior to computing the fwhm. Save result in\n");
  printf("   a summary file (one example uses mgh format, the other gzipped "
         "NIFTI):\n");
  printf("\n");
...
MRI *MRImaskedGaussianSmoothTo(MRI *invol, MRI *mask, double ToFWHM, double tol,
                               int nitersmax, double *pByFWHM,
                               double *pToFWHMActual, int *niters,
                               MRI *outvol) {
  ...
  *niters = 0;
  ...
  printf("niters=%3d  by=%6.4lf  to=%6.4lf targ=%6.4lf  err=%g\n", nth,
         *pByFWHM, *pToFWHMActual, ToFWHM, err);
  outvol = MRIcopy(volsm, outvol);
  MRIfree(&volsm);
  return (outvol);
}
