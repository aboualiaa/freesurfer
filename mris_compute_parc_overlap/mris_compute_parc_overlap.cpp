...
int main(int argc, char *argv[]) {
  ...
  if (annot1) {
    sprintf(filename, "%s/%s/label/%s.%s.annot", SUBJECTS_DIR, subject, hemi,
            annot1);
  ...
  if (surface1->ct == nullptr) {
    printf("ERROR: %s does not contain color table!\n", annot1);
    exit(1);
  }
  if (surface2->ct == nullptr) {
    printf("ERROR: %s does not contain color table!\n", annot2);
    exit(1);
  }
  ...
  */
  ...
  if (err != NO_ERROR) {
    printf("ERROR: could not retrieve annotation for label 'unknown'\n");
    exit(1);
  ...
  if (label1 && label2) {
    ...
    if (label_index == -1) {
      printf("ERROR: could not retrieve index for label '%s'\n", label1name);
      exit(1);
    } // else printf("label '%s' has index %d\n",userLabel.name,label_index);
    if (label_index >= MAX_LABELS) {
      printf("ERROR: label_index=%d >= MAX_LABELS=%d\n", label_index,
             MAX_LABELS);
      exit(1);
    }
    int err =
        CTABannotationAtIndex((CT *)&miniColorTable, label_index, &label_annot);
    if (err != NO_ERROR) {
      printf("ERROR: could not retrieve annotation for label '%s'\n",
             userLabel.name);
      exit(1);
  ...
  if (label1 && label2) {
    ...
    for (vno = 0; vno < surface1->nvertices; vno++) {
      ...
      for (n = 0; n < label1->n_points; n++) {
        if (label1->lv[n].vno == vno) {
          if (use_label1_xyz) {
            ...
            MRISsetXYZ(surface1, vno, label1->lv[n].x, label1->lv[n].y,
                       label1->lv[n].z);
          }
          if (check_label1_xyz) {
      ...
      for (n = 0; n < label2->n_points; n++) {
        if (label2->lv[n].vno == vno) {
          if (use_label2_xyz) {
            // replace surface x,y,z coordinates with those found in label file
            MRISsetXYZ(surface2, vno, label2->lv[n].x, label2->lv[n].y,
                       label2->lv[n].z);
          }
          if (check_label2_xyz) {
            // this vertex is a label vertex, so check its coordinates
    ...
    if (debug_labels) {
      sprintf(tmpstr, "%s/%s/label/%s.label1.annot", SUBJECTS_DIR, subject,
              hemi);
      printf("Writing %s...", tmpstr);
      MRISwriteAnnotation(surface1, tmpstr);
      printf("done.\n");
      sprintf(tmpstr, "%s/%s/label/%s.label2.annot", SUBJECTS_DIR, subject,
              hemi);
      printf("Writing %s...", tmpstr);
  ...
  for (n = 0; n < surface1->nvertices; n++) {
    ...
    if (colorTabIndex1 >= MAX_LABELS) {
      ...
      if (surf1BoundaryLabels[colorTabIndex1].annotation == 0) {
        ...
        if (surf1BoundaryLabels[colorTabIndex1].annotation != v1->annotation) {
          printf("ERROR: "
                 ...
                 colorTabIndex1, surf1BoundaryLabels[colorTabIndex1].annotation,
                 v1->annotation);
          exit(1);
        }
      }
      ...
      if (v1->border)
        surf1BoundaryLabels[colorTabIndex1].vnos[n] = 1;
    }
    ...
    if (colorTabIndex2 >= MAX_LABELS) {
      ...
      if (surf2BoundaryLabels[colorTabIndex2].annotation == 0) {
        ...
        if (surf2BoundaryLabels[colorTabIndex2].annotation != v2->annotation) {
          printf("ERROR:"
                 ...
                 colorTabIndex2, surf2BoundaryLabels[colorTabIndex2].annotation,
                 v2->annotation);
          exit(1);
        }
      }
      // flag this vertex number as occupied, if it is a boundary vertex
      if (v2->border)
        surf2BoundaryLabels[colorTabIndex2].vnos[n] = 1;
    }
  ...
  if (debug_overlap) {
    sprintf(tmpstr, "%s/%s/label/%s.overlap.annot", SUBJECTS_DIR, subject,
            hemi);
    printf("Writing %s...", tmpstr);
...
static int isExcludedLabel(int colortabIndex) {
  if (excludedLabelsList) {
    ...
    for (i = 0; i < numExcludedLabels; i++) {
      if (excludedLabelsList[i] == colortabIndex)
        return 1; // excluded label
    }
  }
...
static int isIncludedLabel(int colortabIndex) {
  if (includedLabelsList) {
    ...
    for (i = 0; i < numIncludedLabels; i++) {
      if (includedLabelsList[i] == colortabIndex)
        return 1; // included label
    }
  }
...
static void calcMeanMinLabelDistances() {
  if (debug_boundaries) {
    ...
    sprintf(tmpstr, "%s/%s/label/%s.boundary1.annot", SUBJECTS_DIR, subject,
            hemi);
    printf("Writing %s...", tmpstr);
    MRISwriteAnnotation(surface1, tmpstr);
    printf("done.\n");
    ...
    sprintf(tmpstr, "%s/%s/label/%s.boundary2.annot", SUBJECTS_DIR, subject,
            hemi);
    printf("Writing %s...", tmpstr);
    MRISwriteAnnotation(surface2, tmpstr);
    printf("done.\n");
  ...
  for (cti = 0; cti < MAX_LABELS; cti++) {
    ...
    if (isExcludedLabel(cti))
      continue;

    // if a label-list file was specified on command-line,
    // then skip labels not found in that list
    if ((nullptr != labelsfile) && (!isIncludedLabel(cti)))
      continue;
        ...
        if (surf1BoundaryLabels[cti].vnos[vno]) // found a boundary vertex
        {
          ...
          if (minDist == 100000000) {
            ...
            printf("ERROR: minDist==100000000 (%f) "
                   ...
                   minDist, cti, vno, surf1BoundaryLabels[cti].annotation,
                   surf2BoundaryLabels[cti].annotation);
        ...
        if (surf2BoundaryLabels[cti].vnos[vno]) // found a boundary vertex
        {
          ...
          if (minDist == 100000000) {
            ...
            printf("ERROR: minDist==100000000 (%f) "
                   ...
                   minDist, cti, vno, surf1BoundaryLabels[cti].annotation,
                   surf2BoundaryLabels[cti].annotation);
            //            exit(1);  not an error - in CD parcellation some folds
            //            may not occur
          }
          // save away the min distance info for this vertex
  ...
  for (cti = 0; cti < MAX_LABELS; cti++) {
    if (isExcludedLabel(cti))
      continue; // skip excluded labels from calc
    ...
    if (surf1BoundaryLabels[cti].annotation != 0 &&
      ...
      if (surf1BoundaryLabels[cti].annotation !=
          surf2BoundaryLabels[cti].annotation) {
        const char *n1, *n2;
        n1 = CTABgetAnnotationName(surface1->ct,
                                   surf1BoundaryLabels[cti].annotation);
        n2 = CTABgetAnnotationName(surface2->ct,
                                   surf2BoundaryLabels[cti].annotation);

        printf("ERROR: surf1BoundaryLabels[%d].annotation=0x%8.8X (%s) != "
               "surf2BoundaryLabels[%d].annotation=0x%8.8X (%s) \n",
               cti, surf1BoundaryLabels[cti].annotation, n1, cti,
               surf2BoundaryLabels[cti].annotation, n2);
        //        exit(1);   Not an error - in CD parcellation some folds may
        //        not occur
      }
      // calc the average
...
static void usage(int exit_val) {
  ...
  fprintf(fout, "Compares two parcellated (annotated or labeled) surfaces\n"
  ...
  fprintf(fout, "  --log filename           output the overall DICE and min "
                "dist to filename\n");
  ...
  fprintf(
      ...
      "input, except wherever the labels are identical with annot1, that "
      "label\n"
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      print_version();
    else if (!strcasecmp(option, "--debug-overlap"))
      debug_overlap = 1;
    else if (!strcasecmp(option, "--nodebug-overlap"))
      debug_overlap = 0;
    else if (!strcasecmp(option, "--debug-boundaries"))
      debug_boundaries = 1;
    else if (!strcasecmp(option, "--nodebug-boundaries"))
      debug_boundaries = 0;
    else if (!strcasecmp(option, "--debug-labels"))
      debug_labels = 1;
    else if (!strcasecmp(option, "--nodebug-labels"))
      debug_labels = 0;
    else if (!strcasecmp(option, "--check-label1-xyz"))
      check_label1_xyz = 1;
    else if (!strcasecmp(option, "--nocheck-label1-xyz"))
      check_label1_xyz = 0;
    else if (!strcasecmp(option, "--check-label2-xyz"))
      check_label2_xyz = 1;
    else if (!strcasecmp(option, "--nocheck-label2-xyz"))
      check_label2_xyz = 0;
    else if (!strcasecmp(option, "--nocheck-xyz")) {
      check_label1_xyz = 0;
      check_label2_xyz = 0;
    } else if (!strcasecmp(option, "--use-label1-xyz"))
      use_label1_xyz = 1;
    else if (!strcasecmp(option, "--use-label2-xyz"))
      use_label2_xyz = 1;
...
/* --------------------------------------------- */
static void print_help() { usage(1); }
...
static void check_options() {
  ...
  if ((annot1 == nullptr) && (annot2 == nullptr) && (label1name == nullptr) &&
      (label2name == nullptr)) {
