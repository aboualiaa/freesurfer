 ...
 * @brief MCMC for computing posterior of splines connecting cortical
 * parcellation with itself
 *
...
static VOXEL_LIST *find_optimal_spline(VOXEL_LIST *vl, MRI *mri_aseg,
                                       MRI *mri_wm_dist, int mcmc_samples,
                                       double spline_length_penalty,
                                       double spline_interior_penalty);
...
extern VOXLIST *MRIcomputeLaplaceStreamline(MRI *mri_laplace, int max_steps,
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: mris_init_global_tractography.c,v 1.4 "
                                "2016/07/18 15:42:20 fischl Exp $",
                                "$Name:  $");
  ...
  for (nlabels = x = 0; x < mri_aseg->width; x++)
    for (y = 0; y < mri_aseg->height; y++)
      for (z = 0; z < mri_aseg->depth; z++) {
        ...
        if ((label >= MIN_CORTEX && label <= MAX_CORTEX) &&
            (label_found[label - MIN_CORTEX] == 0)) {
  ...
  if (label1_target > 0)        // operate in two-label mode
  {
    ...
    if (use_laplace) {
      ...
      mri_laplace = MRIsolveLaplaceEquation(
          mri_wm_only, mri_aseg, label2_target, label1_target, LAPLACE_SOURCE,
          LAPLACE_TARGET, LAPLACE_OUTSIDE);
        ...
        for (yv = 0; yv < mri_aseg->height; yv++)
          for (zv = 0; zv < mri_aseg->depth; zv++) {
            if (MRIgetVoxVal(mri_aseg, xv, yv, zv, 0) == label1_target)
              DiagBreak();
            if (MRIgetVoxVal(mri_aseg, xv, yv, zv, 0) == label1_target &&
                MRIlabelsInNbhd6(mri_wm, xv, yv, zv, 1)) {
              for (xk = -1; xk <= 1; xk++)
                for (yk = -1; yk <= 1; yk++)
                  for (zk = -1; zk <= 1; zk++) {
                    if (abs(xk) + abs(yk) + abs(zk) != 1)
                      continue; // enforce 6-connectivity
                    xi = mri_laplace->xi[(xv + xk)];
                    yi = mri_laplace->yi[(yv + yk)];
                    zi = mri_laplace->zi[(zv + zk)];
...
#ifdef HAVE_OPENMP
        val = 0 ; l = 0 ;
#pragma omp parallel for if_ROMP(experimental) firstprivate(val, l)            \
    shared(mri_aseg, mri_tmp, labels, y, z) schedule(static, 1)
#endif
#endif
          ...
          for (y = 0; y < mri_aseg->height; y++) {
            for (z = 0; z < mri_aseg->depth; z++) {
              ...
              if (IS_WMH(l) || l == label2_target)
                continue;
              val = MRImaxInRegion(mri_smooth, x, y, z, 1) +
                    1; // one greater than max
...
#pragma omp parallel for if_ROMP(experimental) firstprivate(val, l)            \
    ...
    for (label2 = 0; label2 < nlabels; label2++) {
      ...
      if (write_diags) {
        sprintf(fname, "%s.%d.%d.label", base_name, labels[label],
                labels[label2]);
        printf(
            "writing %d control point spline to %s for %s (%d) --> %s (%d)\n",
            vl_splines[label][label2]->nvox, fname,
            cma_label_to_name(labels[label2]), labels[label2],
  ...
  seconds = seconds % 60;
  fprintf(stderr,
          "minimal WM path-length calculation took %d hours, %d minutes and %d "
          "seconds.\n",
...
static MRI *MRIinteriorDistanceTransform(MRI *mri_aseg, MRI *mri_wm_interior,
                                         MRI *mri_paths, int target_label,
                                         int hemi) {
          ...
          for (zk = -1; zk <= 1; zk++) {
                                     ...
                                     //  correct hemi
            {
              ...
              if ((label >= MIN_LH_CORTEX && label <= MAX_LH_CORTEX) ||
                  ((label == target_label) &&
                   (hemi == LEFT_HEMISPHERE))) // in the left hemi
              {
                if (MRIgetVoxVal(mri_aseg, xi, yi, zi, 0) ==
...
static VOXEL_LIST *
compute_optimal_number_of_control_points(MRI *mri_aseg, VOXEL_LIST *vl,
                                         int min_points, double allowable_dist);

static VOXEL_LIST *compute_spline_initialization(
    MRI *mri_aseg, MRI *mri_wm, MRI *mri_wm_dist, MRI *mri_label1_dist,
    MRI *mri_dist_grad, int label1, int label2, int min_spline_control_points) {
  VOXEL_LIST *vl_spline, *vl, *vl_optimal, *vl_init_spline, *vl_optimal_final,
      *vl_interp;
...
static VOXEL_LIST *compute_path_to_label(MRI *mri_aseg, MRI *mri_wm,
  ...
  VLSTaddWithValue(vl, nint(xv), nint(yv), nint(zv), xv, yv, zv,
                   MRIgetVoxVal(mri_dist, xv, yv, zv, 0),
      ...
      for (yk = -1; yk <= 1; yk++)
        for (zk = -1; zk <= 1; zk++) {
          ...
          if ((label == label1 &&
              ...
              ((dist <
                min_dist) && // or find min_dist voxel that hasn't been visited
...
#define PATH_START 10
    if (found == 0) {
      if (vl->nvox - 1 >
          (backtrack_steps -
           PATH_START)) // near the start of the path, start somewhere random
      ...
      printf("vl: (%d, %d, %d) --> (%d, %d, %d)\n", vl->xi[0], vl->yi[0],
             vl->zi[0], vl->xi[vl->nvox - 1], vl->yi[vl->nvox - 1],
             vl->zi[vl->nvox - 1]);
    ...
    yv += dy;
    zv += dz;
    if (MRIindexNotInVolume(mri_aseg, xv, yv, zv) ||
        (vl->nvox >= max_steps)) // zzz
    {
      printf("!!!!!!!!!!!!!!  path to label %d from label %d left volume or "
             "too long !!!!!!!!!!!\n",
             label1, label2);
      if (write_diags) {
        MRIwrite(mri_path, "p.mgz");
      ...
      ErrorReturn(NULL, (ERROR_BADPARM,
                         "!!!!!!!!!!!!!! path to label %d from label %d left "
                         "volume !!!!!!!!!!!",
                         label1, label2));
    }
    odx = dx;
...
static VOXEL_LIST *find_optimal_spline(VOXEL_LIST *vl, MRI *mri_aseg,
  ...
  for (n = 0; n < mcmc_samples; n++) {
    ...
    energy =
        compute_spline_energy(vl_spline, mri_wm_dist, energy_flags,
                              spline_length_penalty, spline_interior_penalty);
...
static double compute_spline_energy(VOXEL_LIST *vl, MRI *mri_wm_dist, int which,
  ...
  if (which & SPLINE_LENGTH)
    energy +=
        spline_length_penalty * length_energy; // used to be sqrt(length_energy)
  if (which & SPLINE_WM_DIST)
    energy += spline_interior_penalty *
              interior_energy; // used to be sqrt(interior_energy) ;
