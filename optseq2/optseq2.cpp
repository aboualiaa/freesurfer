...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: optseq2.c,v 2.22 2011/04/21 19:48:51 greve Exp $",
      "$Name:  $");
  if (nargs && argc - nargs == 1)
    exit(0);
  argc -= nargs;

  infilelist[0] = nullptr;
  ...
  if (PolyOrder >= 0) {
    ...
    for (n = 0; n < Ntp; n++)
      Xpoly->rptr[n + 1][1] = 1.0;
    ftmp = Ntp / 2.0;
    if (PolyOrder >= 1)
      for (n = 0; n < Ntp; n++)
        Xpoly->rptr[n + 1][2] = (n - ftmp) / ftmp;
    if (PolyOrder >= 2)
      for (n = 0; n < Ntp; n++)
        Xpoly->rptr[n + 1][3] = pow((n - ftmp), 2.0) / (ftmp * ftmp);
  } else
    Xpoly = nullptr;
    ...
    if (!CheckIntMult(EvDuration[n] + tNullMin, dPSD, dPSD / 100)) {
      if (tNullMin > 0)
        printf("ERROR: Duration of EventType %d (%g sec) + "
               ...
               n + 1, EvDuration[n], tNullMin, EvDuration[n] + tNullMin, dPSD);
      else
        printf("ERROR: Duration of EventType %d (%g sec) is not an integer "
               "multiple "
               "of the dPSD (%g sec)\n",
  ...
  if (nEVContrast > 0) {
    C = ContrastMatrix(EVContrast, nEvTypes, nPSDWindow, PolyOrder + 1,
                       ContrastSumDelays);
  }
  if (C) {
  ...
  dump_options(fplog);
  fprintf(fplog, "\n");
  fprintf(fplog, "Pct nSrch MinSrch Cost Eff Cb1Err VRFAvg VRFStd VRFMin "
                 "VRFMax VRFRange nSince\n");
  fprintf(fplog, "\nBeginUpdateLog\n");
  ...
  while (true) {
    ...
    if ((tSearch > 0) && (tSearched >= tSearch))
      break;
    if ((nSearch > 0) && (nSearched >= nSearch))
      break;
    nSearched++;
    ...
    if (PctVarEvReps > 0.0) {
      if (!VarEvRepsPerCond)
        ftmp = 1.0 + 2 * (drand48() - 0.5) * PctVarEvReps / 100;
      ...
      MatrixFree(&XtXIdeal);
      XtXIdeal = EVSfirXtXIdeal(nEvTypes, EvReps, EvDuration, TR, Ntp, PSDMin,
                                PSDMax, dPSD);
    }

    /* Synthesize a Sequence and Schedule */
    EvSch = EVSsynth(nEvTypes, EvReps, EvDuration, dPSD, TR * Ntp, TPreScan,
                     nCB1Opt, tNullMin, tNullMax);
    if (EvSch == nullptr) {
    ...
    if (penalize)
      EVSrefractory(EvSch, penalpha, penT, pendtmin);
    ...
    if (EffMax < EvSch->eff)
      EffMax = EvSch->eff;
    if (VRFAvgMax < EvSch->vrfavg)
      VRFAvgMax = EvSch->vrfavg;

    /* Save data on each iteration to a file */
    if (SvAllFile != nullptr) {
      fprintf(fpSvAll, "%g  %g  %g  %g  %g  %g  %g %g", EvSch->cost, EvSch->eff,
              EvSch->cb1err, EvSch->vrfavg, EvSch->vrfstd, EvSch->vrfmin,
              EvSch->vrfmax, EvSch->idealxtxerr);
      if (PctVarEvReps > 0.0)
        for (m = 0; m < nEvTypes; m++)
          fprintf(fpSvAll, "%d ", EvReps[m]);
      fprintf(fpSvAll, "\n");
    }
    ...
    if (nSearch > 0)
      PctDone = 100 * nSearched / nSearch;
    else
      PctDone = 100 * tSearched / tSearch;
...
PastSearch:
  ...
  if (C != nullptr) {
    ...
    for (m = 0; m < nEvTypes; m++)
      fprintf(fpsum, "%6.3f ", EVContrast[m]);
    fprintf(fpsum, "\n");
  ...
  fprintf(fpsum, "\n");
  fprintf(fpsum, "Rank     Cost   ZCost NthIter  Eff   CB1Err   VRFAvg VRFStd "
                 "VRFMin VRFMax VRFRng IXtXErr  ");
  if (PctVarEvReps > 0.0)
    fprintf(fpsum, "NReps");
  fprintf(fpsum, "\n");
  ...
  for (n = 0; n < nKeep; n++) {
    fprintf(fpsum,
            ...
            EvSchList[n]->nthsearched, EvSchList[n]->eff, EvSchList[n]->cb1err,
            EvSchList[n]->vrfavg, EvSchList[n]->vrfstd, EvSchList[n]->vrfmin,
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      Update = 1;
    else if (!strcasecmp(option, "--noupdate"))
      Update = 0;
    else if (!strcasecmp(option, "--nosearch"))
      NoSearch = 1;
    else if (!strcasecmp(option, "--sumdelays"))
      ContrastSumDelays = 1;
      ...
      C = MatrixReadTxt(pargv[0], nullptr);
      if (C == nullptr) {
...
static void print_help() {
  ...
  printf(
      ...
      "response. RPER is highly resistant to habituation, expectation, and "
      "set\n"
      ...
      "\n"
      "Specifications for the FIR event response window. It will be assumed "
      "that \n"
      ...
      "Nth order polynomial was well as all lower orders. Max order is "
      "currently\n"
      ...
      " Note: it may not be possible for a given parameter set to keep the "
      "NULL \n"
      ...
      "--ar1 rho \n"
      " \n"
      "Optimize while whitening with an AR(1) model with parameter rho. rho "
      "must\n"
      "be between -1 and +1.\n"
      " \n"
      "--pen alpha T dtmin\n"
      ...
      "presentation. The weight is computed as 1 - alpha*exp(-(dt+dtmin)/T), "
      "where \n"
      ...
      "will be reduced in amplitude by the weight factor. alpha and T were "
      "fit\n"
      ...
      "vrfavgstd, effinv. Default is eff. params as any parameters which "
      "accompany \n"
      ...
      "per event type corresponding to the delay in the FIR window. By "
      "default, \n"
      ...
      "with --pctupdate. Each status line has 12 columns: (1) percent "
      "complete, \n"
      ...
      "\n"
      "solution Bhat = inv(XtX)Xy. A contrast is Ghat = C*Bhat, where C is "
      "the\n"
      "the contrast matrix.\n"
      " \n"
      "CONTRAST MATRIX \n"
      ...
      "least 2 event types. \n"
      " \n"
      "Efficiency (eff). Efficiency is defined as eff = "
      "1/trace(C*inv(Xt*X)*Ct) \n"
      ...
      "quantity trace(C*inv(XtX)*Ct) is a measure of the sum square error in "
      "Ghat \n"
      "(ie, G-Bhat) relative to the noise inherent in the experiment. "
      "Therefore,  \n"
      "maximizing eff is a way of finding a schedule that will result in, on \n"
      "average, the least error in Ghat. \n"
      " \n"
      "Average Variance Reduction Factor (vrfavg). The Variance Reduction "
      "Factor \n"
      "(VRF) is the amount by which the variance of an individual estimator "
      "(ie,  \n"
      ...
      "\n"
      "component on the diagonal of C*inv(XtX)*Ct. The average VRF is this "
      "value \n"
      "averaged across all estimators.  This will yield similar results as "
      "when \n"
      "the efficiency is optimized. \n"
      " \n"
      "Average/StdDev Variance Reduction Factor (vrfavgstd). The cost is "
      "defined \n"
      ...
      "schedules. Note: the printed ideal matrix is based on the nominal "
      "number \n"
      ...
      "number of time points (Ntp), (2) the TR, (3) the prescan window "
      "(tPreScan), \n"
      "(4) the duration of each event type (tEv), and (5) the number of "
      "repetitions \n"
      "of each event type (nReps). The most basic relationship requires that \n"
      "the total amount of stimulation time (tStimTot) be less than or equal "
      "to \n"
      "the total  amount of scan time (tScanTot), where tStimTot = "
      "sum(tEv*nReps) \n"
      ...
      "Violation' Error. The total amount of time dedicated to the Null "
      "stimulus \n"
      ...
      "\n"
      " \n"
      "If the parameters are chosen such that equality results in equation "
      "(1), \n"
      "then there will not be any time for the Null stimulus, which is "
      "generally \n"
      "not good because inter-stimulus jitter is dependent upon inserting  \n"
      "random amounts of Null between non-Null stimuli.  \n"
      " \n"
      "A rule of thumb is to allocate as much time for the Null as one would "
      "for  \n"
      ...
      "this nominal value. \n"
      " \n"
      "There is also a DOF constraint which requires that the number of "
      "parameters \n"
      ...
      "\n"
      "in the post-stimulus time window (ie, (PSDMax-PSDMin)/dPSD), and "
      "PolyOrder \n"
      "is the order of the nuisance polynomial specified with -polyfit. If "
      "this \n"
      "constraint is not met, you will receive a 'DOF Constraint Violation' "
      "Error. \n"
      " \n"
      ...
      "\n"
      "would it be possible to determine how long it would take a random "
      "search \n"
      "to get there. That said, there are some rules of thumb that can be "
      "followed, \n"
      "the most basic being that if a 'large' number of schedules have been  \n"
      "searched and the best cost has not changed 'much' in a 'long time', "
      "then \n"
      "you are done. Of course, you still have to define 'large', 'much', and "
      "\n"
      "'long time'. The summary file can help with this. In particular, there "
      "is a \n"
      "line with the number of iterations since the last substitution (ie,  \n"
      "the number of iterations since one of the best nKeep schedules "
      "changed). \n"
      "This can be used to judge how long a 'long time' is. The same "
      "information \n"
      "can be extracted from the NthIter column of the summary table. At a "
      "minimum \n"
      "let it run for 10000 iterations. \n"
      " \n"
      "PARADIGM FILE FORMAT\n"
      "The schedules will be saved in 'paradigm file' format. This format has "
      "four \n"
      "columns: time, numeric event id, event duration, and event label. A "
      "numeric \n"
      "id of 0 indicates the Null Stimulus. \n"
      " \n"
      "BUGS \n"
      " \n"
      " Also see the Optseq Home page at "
      "http://surfer.nmr.mgh.harvard.edu/optseq\n"
      " \n"
      " The vrfavgstd cost function does not work properly if the number of "
      "reps\n"
      " is different for different event types. A prescan window should also "
      "be\n"
      " specified\n"
      " \n"
      ...
      "BUG REPORTING \n"
      " \n"
      " optseq2 is offered with no guarantees and with no obligation or "
      "implication\n"
      ...
      " operating system, (3) command-line, (4) description of the problem, "
      "and\n"
...
static void check_options() {
  ...
  if (!NoSearch) {
    if (nSearch < 0 && tSearch < 0) {
      printf(
          "ERROR: must specify search time or number of search iterations\n");
      exit(1);
    }
    if (nSearch > 0 && tSearch > 0) {
      printf("ERROR: cannot specify both search time and number of search "
             "iterations\n");
      exit(1);
    }
    if (nEvTypes == 0) {
  ...
  if (outstem == nullptr && infilelist[0] == nullptr) {
...
static void dump_options(FILE *fp) {
  ...
  fprintf(fp, "AR1 = %g\n", ar1rho);
  if (!penalize)
    fprintf(fp, "No refractory penalty\n");
  else
    fprintf(fp, "Refractory penalty: alpha = %g, T = %g, dtmin = %g\n",
...
/*------------------------------------------------------------*/
static int PrintUpdate(FILE *fp, int n) {
  if (EvSchList[n] == nullptr)
    return (1);

  fprintf(fp, "%2.0f %7d  %6.1f   %g  %g  %6.4f  %g  %g  %g  %g  %g %g %d\n",
          PctDone, nSearched, tSearched * 60, EvSchList[n]->cost,
          EvSchList[n]->eff, EvSchList[n]->cb1err, EvSchList[n]->vrfavg,
          EvSchList[n]->vrfstd, EvSchList[n]->vrfmin, EvSchList[n]->vrfmax,
          EvSchList[n]->vrfrange, EvSchList[n]->idealxtxerr, nSince);
  fflush(fp);
  return (0);
}
...
/*------------------------------------------------------------
  ...
  ------------------------------------------------------------*/
static int CheckIntMult(float val, float res, float tol) {
  if (fabs(val / res - rint(val / res)) > tol)
    return (0);
  return (1);
}
/*------------------------------------------------------------*/
static MATRIX *ContrastMatrix(float *EVContrast, int nEVs, int nPer, int nNuis,
  ...
  C = MatrixZero(nPer, nbeta, nullptr);
