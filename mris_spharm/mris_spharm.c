...
int main(int argc, char *argv[]) {
  ...
  // MRISwrite(mris_out,
  // "/space/xrt/1/users/btquinn/buckner_paper/010223_61223/surf/lh.ic7") ;
  ...
  MRISwrite(
      mris_out,
      "/space/xrt/1/users/btquinn/buckner_paper/010223_61223/surf/lh.sampled");
  ...
  if (COMPARE) {
    ...
    fprintf(stderr, "original area becomes %f\n", area);
    // MRISwrite(mris_out,
    // "/space/xrt/1/users/btquinn/buckner_paper/011121_vc8048/surf/lh.sampled")
    // ;
    MRISsaveVertexPositions(mris_out, ORIGINAL_VERTICES);
    MRISrestoreVertexPositions(mris_out, TMP_VERTICES);
#endif
...
MATRIX *ComplexMatrixPseudoInverse(MATRIX *m, MATRIX *m_pseudo_inv) {
...
MATRIX *ComplexMatrixTranspose(MATRIX *mIn, MATRIX *mOut) {
...
static MRI_SURFACE *sample_origposition(MRI_SURFACE *mris_src,
  ...
  for (index = 0; index < mris_dst->nvertices; index++) {
    ...
    x1 = mris_src->vertices[mris_src->faces[fnum].v[1]].x -
         mris_dst->vertices[index].x;
    y1 = mris_src->vertices[mris_src->faces[fnum].v[1]].y -
         mris_dst->vertices[index].y;
    z1 = mris_src->vertices[mris_src->faces[fnum].v[1]].z -
         mris_dst->vertices[index].z;
    x2 = mris_src->vertices[mris_src->faces[fnum].v[2]].x -
         mris_dst->vertices[index].x;
    y2 = mris_src->vertices[mris_src->faces[fnum].v[2]].y -
         mris_dst->vertices[index].y;
    z2 = mris_src->vertices[mris_src->faces[fnum].v[2]].z -
         mris_dst->vertices[index].z;
    r = 0.5 * fabs(x1 * x2 + y1 * y2 + z1 * z2);

    x1 = mris_src->vertices[mris_src->faces[fnum].v[0]].x -
         mris_dst->vertices[index].x;
    y1 = mris_src->vertices[mris_src->faces[fnum].v[0]].y -
         mris_dst->vertices[index].y;
    z1 = mris_src->vertices[mris_src->faces[fnum].v[0]].z -
         mris_dst->vertices[index].z;
    x2 = mris_src->vertices[mris_src->faces[fnum].v[2]].x -
         mris_dst->vertices[index].x;
    y2 = mris_src->vertices[mris_src->faces[fnum].v[2]].y -
    ...
    z1 = mris_src->vertices[mris_src->faces[fnum].v[1]].z -
         mris_dst->vertices[index].z;
    x2 = mris_src->vertices[mris_src->faces[fnum].v[0]].x -
         mris_dst->vertices[index].x;
    y2 = mris_src->vertices[mris_src->faces[fnum].v[0]].y -
         mris_dst->vertices[index].y;
    z2 = mris_src->vertices[mris_src->faces[fnum].v[0]].z -
         mris_dst->vertices[index].z;
