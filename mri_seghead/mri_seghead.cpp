...
FOUND:
  ...
  MRI *mri_box = MRIextract(mri_lia, nullptr, 0, hmin, start, mri_head->width,
                            hmax - hmin, end - start);
  if (outbox != nullptr)
...
int main(int argc, char **argv) {
  ...
  if (!fio_DirIsWritable(outvolid, 1) && fio_DirIsWritable(outvolid, 0)) {
    ...
    for (r = 0; r < invol->height; r++) {
      for (s = 0; s < invol->depth; s++) {
        ...
        if (cmin >= dilate)
          ...
          cmin = 0;
        if (cmax <= invol->width - 1 - dilate)
          cmax += dilate;
        else
          cmax = invol->width - 1;

        for (c = cmin; c <= cmax; c++)
          MRIseq_vox(cvol, c, r, s, 0) = fillval;
        // printf("%3d %3d  %3d %3d\n",r,s,cmin,cmax);
      }
    }
  }
    ...
    for (c = 0; c < invol->width; c++) {
      for (s = 0; s < invol->depth; s++) {
        ...
        if (rmin >= dilate)
          ...
          rmin = 0;
        if (rmax <= invol->height - 1 - dilate)
          rmax += dilate;
        else
          rmax = invol->height - 1;

        for (r = rmin; r <= rmax; r++)
          MRIseq_vox(rvol, c, r, s, 0) = fillval;
        // printf("%3d %3d  %3d %3d\n",r,s,rmin,rmax);
      }
    }
  }
    ...
    for (c = 0; c < invol->width; c++) {
      for (r = 0; r < invol->height; r++) {
        ...
        if (smin >= dilate)
          ...
          smin = 0;
        if (smax <= invol->depth - 1 - dilate)
          smax += dilate;
        else
          smax = invol->depth - 1;

        for (s = smin; s <= smax; s++)
          MRIseq_vox(svol, c, r, s, 0) = fillval;
        // printf("%3d %3d  %3d %3d\n",r,s,rmin,rmax);
      }
    }
  }
  ...
  for (c = 0; c < invol->width; c++) {
    for (r = 0; r < invol->height; r++) {
      for (s = 0; s < invol->depth; s++) {
        if (MRIseq_vox(cvol, c, r, s, 0) && MRIseq_vox(rvol, c, r, s, 0) &&
  ...
  printf("Growing\n");
  fflush(stdout);
  outvol = MRIfill(mrgvol, nullptr, (int)(mrgvol->width / 2.0),
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--fillrows"))
      fillrows = 1;
    else if (!strcasecmp(option, "--fillcols"))
      fillcols = 1;
...
static void print_help() {
  ...
  printf(
      ...
      " \n"
      "The program first creates a binarized mass just a few mm below the "
      "skin.\n"
      ...
      "Once the skin is found coming from both directions, everything in "
      "between\n"
      ...
      "\n"
      "filling in all of the subepidermal features that would normally be "
      "below \n"
      "threshold. A seed point is chosen at the center of the volume. The "
      "final \n"
      "binarized volume is computed as all the voxels above thresh2 connected "
      "to  \n"
      "the seed point. \n"
      " \n"
...
/*
  ...
  \brief Project surface along the normal a distance given by the value in
  mridist at the voxel corresponding to the vertex.
 */
int MRISprojectDist(MRIS *surf, const MRI *mridist) {
  int vno, nthstep;
  ...
  for (vno = 0; vno < surf->nvertices; vno++) {
    ...
    for (nthstep = 0; nthstep < mridist->nframes; nthstep++) {
      ...
      MRISsetXYZ(surf, vno, vtx->x + dist * vtx->nx, vtx->y + dist * vtx->ny,
                 vtx->z + dist * vtx->nz);
    }
  }
  return (0);
