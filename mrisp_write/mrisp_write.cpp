...
int main(int argc, char *argv[]) {
  ...
  if (coords >= 0) {
    ...
    mrisp = MRIScoordsToParameterization(mris, nullptr, scale, coords);
  ...
  if (file_type == MRI_MGH_FILE || file_type == NIFTI1_FILE ||
    ...
    if (mri_overlay == nullptr)
      ErrorExit(ERROR_NOFILE,
                "%s: could not read surface-encoded volume file from %s",
                Progname, in_overlay);
    {
      MRI *mri_tmp;
    ...
    if (compute_corr) // store a frame of correlations for each vertex in the
                      // label in the mrisp
    {
      MRI *mri_corr;

      area = LabelRead(nullptr, seed_label_fname);
      if (area == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not read label file from %s",
                  seed_label_fname);
      printf("computing %d surface correlations\n", area->n_points);
      mri_corr = mrisComputeLabelCorrelations(mris, area, mri_overlay, nullptr);
      MRIfree(&mri_overlay);
    ...
    if (clabel_fname) {
      carea = LabelRead(nullptr, clabel_fname);
      if (carea == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not read label file from %s",
                  clabel_fname);
    } else
    ...
    if (mri_overlay->width != mris->nvertices)
      ErrorExit(ERROR_UNSUPPORTED,
                "%s: overlay width (%d) does not match number of vertices in "
                "surface (%d)",
                Progname, mri_overlay->width, mris->nvertices);
    ...
    for (frame = 0; frame < mri_overlay->nframes; frame++) {
      ...
      if (label_fname) // if compute_corr==TRUE then label is a set of seeds and
                       // don't mask
      ...
      if (mrisp_contra) // doing cross-hemi correlations
      {
        ...
        if (barycentric)
          MRIStoParameterizationBarycentric(mris_contra, mrisp_contra, scale,
                                            frame);
        else
  ...
  } else // process a 'curvature' file like thickness with a single frame
  {
        ...
        MGH_LABEL_FILE) // read in a label and create an overlay from it
    {
      ...
      area = LabelRead(nullptr, in_overlay);
      if (area == nullptr)
        ErrorExit(ERROR_NOFILE,
                  "%s: could not load label file %s to generate overlay",
                  in_overlay);
      ...
      if (MRISreadCurvatureFile(mris, in_overlay) != NO_ERROR)
        ErrorExit(ERROR_NOFILE, "%s: could not read input overlay %s", Progname,
                  in_overlay);
    }
    ...
    if (clabel_fname) {
      if (mris_contra == nullptr)
        ErrorExit(
            ERROR_NOFILE,
            "no contra surface specified (use -contra or remove -c <label>)");
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    if (mri_contra_overlay == nullptr)
      ErrorExit(ERROR_NOFILE,
                "%s: could not read contra overlay volume from %s", Progname,
                argv[3]);
    nargs = 2;
    printf(
        "reading contra hemi overlay and sphere for cross-hemi calculations\n");
...
static void print_usage() {
  outputHelpXml(mrisp_write_help_xml, mrisp_write_help_xml_len);
}
...
MRI *mrisComputeLabelCorrelations(MRI_SURFACE *mris, LABEL *area,
  ...
  if (mri_corr == nullptr)
    mri_corr =
        MRIallocSequence(mris->nvertices, 1, 1, MRI_FLOAT, area->n_points);
  if (mri_corr == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate %d x %d correlation matrix",
              mris->nvertices, area->n_points);
...
static MRI_SP *mrispComputeCorrelations(MRI_SP *mrisp, MRI_SP *mrisp_contra) {
  ...
  if (mrisp_contra) {
    mrisp_contra =
        MRISPclone(mrisp_contra); // we will modify this one and free it later
    nframes *= 2;
  }
  ...
  if (!norms)
    ErrorExit(ERROR_NOFILE,
              "mrispComputeCorrelations: could not allocate norm buffer",
              Progname);
  //(48,51) and (48,115)
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
    ...
    for (y = 0; y < height; y++) {
      ...
      for (x1 = 0; x1 < width; x1++)
        for (y1 = 0; y1 < height; y1++) {
          double v1, v2;
          if (x == 0 && y == 0 && x1 == 10 &&
              y1 == 42) // vertices 0 and 1000 in fsaverage5
            DiagBreak();
...
#pragma omp parallel for
    ...
    for (x = 0; x < width; x++) {
      ...
      for (y = 0; y < height; y++) {
        ...
        for (x1 = 0; x1 < width; x1++)
          for (y1 = 0; y1 < height; y1++) {
            frame = (width * height) + x1 * height +
                    y1; // contra data starts after ipsi data
            frame = (width * height) + y1 * width +
                    x1; // contra data starts after ipsi data
            norm2 = cnorms[x1][y1];
            if (FZERO(norm2))
              continue;
            for (dot = 0.0, t = 0; t < mrisp->Ip->num_frame; t++)
              dot += *IMAGEFseq_pix(mrisp->Ip, x, y, t) *
                     *IMAGEFseq_pix(mrisp_contra->Ip, x1, y1, t);
            *IMAGEFseq_pix(mrisp_sphere->Ip, x, y, frame) =
                dot / (norm1 * norm2);
            corrs[x][y][x1 + width][y1] =
                *IMAGEFseq_pix(mrisp_sphere->Ip, x, y, frame);
            if (x == Gx && y == Gy)
              DiagBreak();
          }
