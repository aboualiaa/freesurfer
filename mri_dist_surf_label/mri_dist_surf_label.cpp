...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_label_vals.c,v 1.16 2015/08/24 18:22:05 fischl Exp $",
      "$Name:  $");
  ...
  fprintf(
      stderr,
      "Computing distance from input surface and label points / waypoints.\n");
  ...
  surf_name = argv[1];
  fprintf(stderr, "surface file name is %s\n", surf_name);
  label_name = argv[2];
  fprintf(stderr, "label file name is %s\n", label_name);
  mri_name = argv[3];
  fprintf(stderr, "mri vol file name is %s\n", mri_name);
  outfname = argv[4];
  ...
  MRI *mri_surf_dist;
  // mri_surf_dist = MRIScomputeDistanceToSurface(mris, NULL, 1.0); // original
  // by BF NOTE: need to use the input volume dimensions in order to compute
  // distance accurately from surfaces converted from gii
  MRI *tmpvol = MRIallocSequence(mri->width, mri->height, mri->depth, MRI_FLOAT,
                                 mri->nframes);
  ...
  for (int i = 0; i < area->n_points;
       i++) { // going through all points stored in the label file
    double dist = 0;
