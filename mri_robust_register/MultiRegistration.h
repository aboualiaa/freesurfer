...
public:
  ...
  //! Load all inputs
  int loadMovables(
      const std::vector<std::string> &pmov,
      const std::vector<std::string> &masks = std::vector<std::string>());
  ...
  void setIscaleOnly(bool i) { iscaleonly = i; }

  //! Toggle multi-res or only high res
  void setNoMulti(bool i) { nomulti = i; }
  ...
  void useCRAS(bool b) { crascenter = b; }
