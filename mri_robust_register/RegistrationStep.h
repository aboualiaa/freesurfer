...
public:
  ...
  computeRegistrationStep(MRI *mriS, MRI *mriT);
  ...
  void setFloatSVD(bool fsvd) { floatsvd = fsvd; }
...
RegistrationStep<T>::computeRegistrationStep(MRI *mriS, MRI *mriT) {
  ...
  if (costfun == Registration::ROB) {
    ...
    if (mri_weights && (mri_weights->width != mriS->width ||
                        mri_weights->height != mriS->height ||
                        mri_weights->depth != mriS->depth))
      MRIfree(&mri_weights);

    if (!mri_weights) {
      mri_weights = MRIallocSequence(mriS->width, mriS->height, mriS->depth,
  ...
  zeroweights =
      R.getLastWeightPercent(); // does not need pointers A and B to be valid
...
void RegistrationStep<T>::constructAb(MRI *mriS, MRI *mriT, vnl_matrix<T> &A,
  ...
  mri_indexing = MRIallocSequence(mriS->width, mriS->height, mriS->depth, itype,
                                  mriS->nframes);
  ...
  if (counti == 0) {
    ...
    std::cerr
        ...
        << std::endl;
    std::cerr << "    This can have several reasons (i.e. different "
                 "modalities, different "
              << std::endl;
    std::cerr << "    intensity scales, large non-linearities, too diff. voxel "
                 "sizes ...)"
              << std::endl;
    std::cerr << "    Try calling with --noinit (if the original images are "
                 "well aligned)"
              << std::endl;
      ...
      for (y = fxstart; y < fxh; y++) {
        ...
        for (f = 0; f < fxf; f++) {
          ...
          vnl_vector<double> grad =
              trans->getGradient(x, fxval, y, fyval, z, fzval);
          int dof = grad.size();
