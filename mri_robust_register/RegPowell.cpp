...
double RegPowell::costFunction(const vnl_vector<double> &p) {
  ...
  if (tocurrent->costfun == LS || tocurrent->costfun == TB ||
      tocurrent->costfun == LNCC || tocurrent->costfun == SAD ||
    ...
    case LNCC:
      ...
      break;
    case NCC:
      dd = -CostFunctions::NCC(scf, tcf, msi, mti, tocurrent->subsamp,
                               tocurrent->subsamp, tocurrent->subsamp);
      break;
    case SAD:
      dd = CostFunctions::absDiff(scf, tcf, msi, mti, tocurrent->subsamp,
                                  tocurrent->subsamp, tocurrent->subsamp, si,
                                  sii);
      break;
    default:
      std::cout
 ...
 */
void RegPowell::computeIterativeRegistrationFull(int nmax, double epsit,
                                                 MRI *mriS, MRI *mriT,
