...
public:
  ...
  inline virtual int getDegree() const { return degree; };
...
public:
  ...
  InterpolatorNearest(MRI *tmri) : degree(0) { mri = MRIcopy(tmri, NULL); };
  //! Destructor frees copy of the image
  ~InterpolatorNearest() { MRIfree(&mri); };
...
public:
  //! Constructor creates a copy of the image
  InterpolatorLinear(MRI *tmri) : degree(1) { mri = MRIcopy(tmri, NULL); };
  //! Destructor frees copy of the image
  ~InterpolatorLinear() { MRIfree(&mri); };

  inline virtual double sample(double x, double y, double z, int frame) const {
    double val;
    return MRIsampleVolumeFrameType(mri, x, y, z, frame, SAMPLE_TRILINEAR,
                                    &val);
    return val;
  };
  inline virtual MRI *transformVOX(MRI *mri_dst, MATRIX *mA) const {
...
public:
  ...
  InterpolatorBSpline(MRI *tmri, int tdegree) : degree(tdegree) {
    bspline = MRItoBSpline(tmri, NULL, tdegree);
  };
  //! Destructor frees coefficient image
  virtual ~InterpolatorBSpline() { MRIfreeBSpline(&bspline); };
...
public:
  //! Constructor creates bspline coefficient image
  InterpolatorCubic(MRI *tmri) : InterpolatorBSpline(tmri, 3){};
};
