...
public:
  ...
  //! Mean Rotation
  static vnl_matrix_fixed<double, 3, 3>
  RotationMean(const std::vector<vnl_matrix_fixed<double, 3, 3>> &vm);

  //! Geometric mean
  static vnl_matrix<double>
  GeometricMean(const std::vector<vnl_matrix<double>> &vm, int n = -1);
  static vnl_matrix<double>
  GeometricMean(const std::vector<vnl_matrix_fixed<double, 3, 3>> &vm,
                int                                                n = -1);
  ...
  static vnl_matrix<double> MatrixLog(const vnl_matrix<double> &A,
                                      int maxlogiter = 100);
  ...
  static double
  RigidTransDistSq(const vnl_matrix<double> &a,
                   const vnl_matrix<double> &b = vnl_matrix<double>());

  //! Squared distance based on affine transformation
  static double
  AffineTransDistSq(const vnl_matrix<double> &a,
                    const vnl_matrix<double> &b = vnl_matrix<double>(),
                    double                    r = 100);

  //! Distance based on frobenius norm
  static double getFrobeniusDiff(const vnl_matrix<double> &m1,
  ...
  static LTA *VOXmatrix2LTA(const vnl_matrix_fixed<double, 4, 4> &m, MRI *src,
                            MRI *dst);

  //! Construct LTA from ras2ras matrix and src, dst geometries
  static LTA *RASmatrix2LTA(const vnl_matrix_fixed<double, 4, 4> &m, MRI *src,
                            MRI *dst);
...
private:
  ...
  static vnl_matrix<double> PadeApproximantOfDegree(const vnl_matrix<double> &A,
                                                    unsigned int m);
  static void               expmchk(const vnl_matrix<double> & A,
