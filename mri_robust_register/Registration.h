...
public:
  ...
  Registration()
      : iscale(false), iscaleonly(false), transonly(false), rigid(true),
        isoscale(false), affine(false), trans(NULL), subsamplesize(-1),
        minsize(-1), maxsize(-1), debug(0), verbose(1), initorient(false),
        inittransform(true), initscaling(false), highit(-1), mri_source(NULL),
        mri_target(NULL), iscaleinit(1.0), iscalefinal(1.0), doubleprec(false),
        symmetry(true), sampletype(SAMPLE_TRILINEAR), resample(false),
        costfun(ROB), converged(false) {}
...
protected:
  ...
  vnl_matrix<double> initializeTransform(MRI *mri_in, MRI *mri_ref);
