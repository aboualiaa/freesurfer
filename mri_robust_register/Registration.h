 ...
 * Implements multi resolution and iterative registration, as well as
 * initializations
 */
class Registration {
public:
  //! The different cost functions
  enum Cost { LS = 0, ROB, MI, NMI, ECC, NCC, SCR, TB, LNCC, SAD };

  //! Default constructor
  Registration()
      : iscale(false), iscaleonly(false), transonly(false), rigid(true),
        isoscale(false), affine(false), trans(nullptr), subsamplesize(-1),
        minsize(-1), maxsize(-1), debug(0), verbose(1), initorient(false),
        inittransform(true), initscaling(false), highit(-1),
        mri_source(nullptr), mri_target(nullptr), iscaleinit(1.0),
  ...
  //! Initialize registration (keep source, target, gauss pyramid and
  //! transformation type)
  virtual void clear();
  ...
  //! setSource and setTarget needed by MultiRegistration (no reslicing
  //! necessary there)
  ...
  virtual MRI *getWeights() { return nullptr; }
  ...
  // std::pair <MATRIX*, double> computeIterativeRegSat( int n,double epsit,MRI
  // * mriS=NULL, MRI* mriT=NULL, MATRIX* Minit = NULL, double iscaleinit
  // = 1.0);
  ...
  //! Convert parameter vector to affine matrix (using current transformation
  //! model)
  std::pair<vnl_matrix_fixed<double, 4, 4>, double>
  convertP2Md(const vnl_vector<double> &p) const;

  //! Get final transform (might be different from mfinal due to possible
  //! resampling)
  ...
  //   bool warpSource(const std::string & fname, MATRIX* M = NULL, double is =
  //   -1); bool warpSource(MRI* orig, MRI* target, const std::string &fname,
  //   MATRIX* M = NULL, double is = -1);

  //! Some code for testing
  void testRobust(const std::string &fname, int testno);

  //  double computeSatEstimate (int reslevel, int n,double epsit, MRI *
  //  mriS=NULL, MRI* mriT=NULL, MATRIX* mi=NULL, double scaleinit=1.0 );
  ...
  //! Return centroid of mov image in dst space (of input, not of resampled
  //! internal version)
...
protected:
  ...
  //   returns weights:
  // std::pair < vnl_vector <double >, MRI* > computeRegistrationStepW(MRI *
  // mriS = NULL, MRI* mriT=NULL);
  //   returns param vector:
  //  vnl_vector <double > computeRegistrationStepP(MRI * mriS, MRI* mriT);
  //   returns 4x4 matrix and iscale:
  //  std::pair < vnl_matrix_fixed <double,4,4 >, double>
  //  computeRegistrationStep(MRI * mriS = NULL, MRI* mriT = NULL);
  ...
  // epsit, MRI * mriS, MRI* mriT, const vnl_matrix < double >& m, double
  // scaleinit);
  ...
  // conversion
  // std::pair < vnl_matrix_fixed <double,4,4 >, double > convertP2Md(const
  // vnl_vector <double >& p); std::pair < MATRIX*, double >
  // convertP2MATRIXd(MATRIX* p);

  // MRI * applyParams(MRI * mri_in, const vnl_vector<double>& p, MRI *
  // mri_dst=NULL, bool inverse=false);
  ...
  //! Resample both images (or if not symmetric only src) to new space, return
  //! half way transforms
...
private:
  ...
  // std::pair < vnl_matrix <double>, vnl_vector <double> > constructAb(MRI
  // *mriS, MRI *mriT);
  //  void constructAb(MRI *mriS, MRI *mriT, vnl_matrix < double > &A,
  //  vnl_vector < double > &b);
  ...
  makeIsotropic(MRI *mri, MRI *out, double vsize = -1, int xdim = -1,
                int ydim = -1, int zdim = -1, bool fixtype = true);
...
// template < class T >
// void Registration::iterativeRegistrationHelper( int nmax,double epsit, MRI *
...
//
//   std::pair < vnl_matrix_fixed<double , 4, 4>, double >
//   cmd(vnl_matrix_fixed<double, 4, 4>(),0.0); std::pair <
//   vnl_matrix_fixed<double , 4, 4>, double > fmd(vnl_matrix_fixed<double, 4,
//   4>(),0.0);
//
...
//   RStep.setFloatSVD(true); // even with double, use float for SVD to save
//   some memory T tt;
//
//   converged = false;
//   while (!converged && i<=nmax  )
//   {
//     if (verbose >0) std::cout << " Iteration(" << typeid(tt).name() << "): "
//     << i << std::endl; if (verbose == 1 && subsamplesize > 0) if (mriS->width
//     > subsamplesize && mriS->height > subsamplesize && (mriS->depth >
//     subsamplesize || mriS->depth ==1))
//       std::cout << " (subsample " << subsamplesize << ") " << std::flush;
//     if (verbose >1) std::cout << std::endl;
//
//     // map source (and if symmetric also target) to new space for next
//     iteration
...
//     //
//     if (verbose >1) std::cout << "   - compute new registration" <<
//     std::endl; cmd = RStep.computeRegistrationStep(mri_Swarp,mri_Twarp);
...
//     }
//     else fmd.first = cmd.first * fmd.first; // was '* mh' which should be the
//     same
//
//     // ISCALECHANGE:
//     if (iscale)
...
//       iscalefinal = exp(fmd.second); // compute full factor (source to
//       target) idiff = fabs(cmd.second); std::ostringstream istar; if (idiff
//       <= ieps) istar << " <= " << ieps << "  :-)" ; if (verbose >0 )
...
//     if (verbose >1) std::cout << "     -- old diff. to prev. transform: " <<
//     diff << std::endl; diff = sqrt(MyMatrix::AffineTransDistSq(fmd.first,
//     fmdtmp, 100)); std::ostringstream star; if (diff <= epsit) star << "  <=
//     " << epsit << "   :-)" ; else if (i == nmax) star<< " max it: " << nmax
//     << " reached!"; if (verbose >0) std::cout << "     -- diff. to prev.
//     transform: " << diff << star.str() << std::endl;
...
//         {
//           if (MRIFvox(mri_hweights,x,y,z) < 0) MRIFvox(mri_hweights,x,y,z) =
//           1;
//         }
//
...
//   }
//
//   //vnl_matlab_print(vcl_cerr,fmd.first,"fmd",vnl_matlab_print_format_long);std::cerr
//   << std::endl;
//   //vnl_matlab_print(vcl_cerr,cmd.first,"cmd",vnl_matlab_print_format_long);std::cerr
//   << std::endl;
//   //vnl_matlab_print(vcl_cerr,mh,"mov1hw",vnl_matlab_print_format_long);std::cerr
//   << std::endl;
//   //vnl_matlab_print(vcl_cerr,mhi,"dst1hw",vnl_matlab_print_format_long);std::cerr
//   << std::endl;
//
//   // adjust half way maps to new midpoint based on final transform
//   if (verbose >1) std::cout << "     -- adjusting half-way maps " <<
//   std::endl; vnl_matrix_fixed < double, 4, 4 > ch =
//   MyMatrix::MatrixSqrt(fmd.first);
...
//   dst2weights = chi;
//
//   //vnl_matlab_print(vcl_cerr,mov2weights,"mov2hw",vnl_matlab_print_format_long);std::cerr
//   << std::endl;
//   //vnl_matlab_print(vcl_cerr,dst2weights,"dst2hw",vnl_matlab_print_format_long);std::cerr
//   << std::endl;
//
//
//   MRIfree(&mri_Twarp);
