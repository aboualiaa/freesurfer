...
public:
  ...
  static double mean(MRI *i, const vnl_matrix_fixed<double, 4, 4> &Mi,
                     int frame, int d1, int d2, int d3);
  static double mean(MRI *mri, const vnl_matrix_fixed<double, 4, 4> &Mi,
                     int frame, int d1, int d2, int d3, int xmin, int xmax,
                     int ymin, int ymax, int zmin, int zmax);
  //! Get variance of intensity values
  static double var(MRI *i, int frame);
  //! Get standard deviation of intensity values
  static double sdev(MRI *i, int frame) { return sqrt(var(i, frame)); }
  ...
  static double absDiff(MRI *si, MRI *ti,
                        const vnl_matrix_fixed<double, 4, 4> &Msi,
                        const vnl_matrix_fixed<double, 4, 4> &Mti, int d1,
                        int d2, int d3, const double &s1, const double &s2);
  ...
  static double NCC(MRI *mriS, MRI *mriT,
                    const vnl_matrix_fixed<double, 4, 4> &Msi,
                    const vnl_matrix_fixed<double, 4, 4> &Mti, int d1, int d2,
                    int d3);
  ...
  static double entropyCorrelationCoefficient(MRI *i1, MRI *i2,
                                              double fwhm = 7) {
    JointHisto H(i1, i2);
    H.smooth(fwhm);
    return -H.computeECC();
...
protected:
  ...
  static MRI *mapMRI(MRI *mri, const vnl_matrix_fixed<double, 4, 4> &Mi, int d1,
                     int d2, int d3);
};
...
inline MRIiterator &MRIiterator::operator++(int i) { return (this->*opinc)(i); }
...
inline float MRIiterator::fromUCHAR() { return ((float)*(unsigned char *)pos); }

inline float MRIiterator::fromSHORT() { return ((float)*(short *)pos); }

inline float MRIiterator::fromINT() { return ((float)*(int *)pos); }

inline float MRIiterator::fromLONG() { return ((float)*(long *)pos); }

inline float MRIiterator::fromFLOAT() { return ((float)*(float *)pos); }
