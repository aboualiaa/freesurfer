...
void JointHisto::save(const std::string &fname, const std::string &n) {
  std::ostream *s = new std::ofstream(fname.c_str());
  if (!s || !(*s)) {
...
std::pair<double, double> JointHisto::getMinMax() {
  int                       i, j;
  std::pair<double, double> mm(-1, -1);
  double                    tmp;
  if (n == 0)
    return mm;
  mm.first  = histo[0][0];
...
void JointHisto::create(MRI *mriS, MRI *mriT,
  ...
  //  vnl_matlab_print(std::cerr,Msi,"Msi",vnl_matlab_print_format_long);
  //  vnl_matlab_print(std::cerr,Mti,"Mti",vnl_matlab_print_format_long);

  //   static double uniform[101] = { 0.825960894329359, 0.391799656122844,
  //   0.606822280998442, 0.923192169644637,
  //                                  0.772662867376799, 0.394913419972908,
  //                                  0.562648884384293, 0.821213453107628,
  //                                  0.077777173551299, 0.536306917657496,
  //                                  0.514107239245956, 0.761227511728186,
  //                                  0.978425890910315, 0.157030594951506,
  //                                  0.147496203406248, 0.783403101626463,
  //                                  0.637363106003275, 0.257349279177570,
  //                                  0.109403726553149, 0.136954287985675,
  //                                  0.654171911591805, 0.204476823923606,
  //                                  0.164700938803686, 0.566376934840801,
  //                                  0.854095286894540, 0.430581644078462,
  //                                  0.397193805113614, 0.045536084142315,
  //                                  0.140620297119782, 0.724179303888238,
  //                                  0.210184975302471, 0.346103835004574,
  //                                  0.226683913788046, 0.308131967172401,
  //                                  0.451070768412025, 0.831843821825446,
  //                                  0.086760750364257, 0.854135129404520,
  //                                  0.742590231477529, 0.053858310273945,
  //                                  0.122791324135168, 0.526967625411493,
  //                                  0.320307444448230, 0.520062337463421,
  //                                  0.878893684505441, 0.443226585521493,
  //                                  0.320014638636649, 0.868171615160687,
  //                                  0.764138797526944, 0.864611801159897,
  //                                  0.083244000805187, 0.340050247951733,
  //                                  0.581607039757426, 0.291315250124114,
  //                                  0.467637373824935, 0.596640965216293,
  //                                  0.897387480194169, 0.121159463814373,
  //                                  0.018178277226624, 0.747666956017887,
  //                                  0.670108792666926, 0.092391423049263,
  //                                  0.917528192569663, 0.347581829497044,
  //                                  0.857229173664249, 0.632564501155682,
  //                                  0.363593396666436, 0.385827512659221,
  //                                  0.003355759148503, 0.969283884272977,
  //                                  0.459201707491767, 0.800715562489883,
  //                                  0.296945204007638, 0.619917383791628,
  //                                  0.748749776544515, 0.394902341568457,
  //                                  0.354479046607957, 0.867388084434840,
  //                                  0.627567204750354, 0.984294463747630,
  //                                  0.824472893444168, 0.295272747204669,
  //                                  0.927251152590456, 0.119266587069099,
  //                                  0.241265513320426, 0.527775796652046,
  //                                  0.060203502196487, 0.835363100840541,
  //                                  0.148398316485924, 0.121273963075904,
  //                                  0.683207184266160, 0.500002874704566,
  //                                  0.561265939626174, 0.234256657698203,
  //                                  0.486854859925734, 0.071141206079346,
  //                                  0.442630693187859, 0.327200604299592,
  //                                  0.226827609433358, 0.971944076189183,
  //                                  0.302612670611030 };
  //   int opos = 0;
  //
  //    int    upos=0;
  ...
  int    ivs, ivt;
  double sdiff, tdiff;
  double dt[3] = {(double)mriT->width, (double)mriT->height,
                  (double)mriT->depth};
  ...
  for (z = 0; z < dt[2] - d3 + 1; z += d3) {
    for (y = 0; y < dt[1] - d2 + 1; y += d2) {
      for (x = 0; x < dt[0] - d1 + 1; x += d1) {
        ...
        // if (zs>=0.0 && zs<ds[2]-1 && ys>=0.0 && ys<ds[1]-1 && xs>=0.0 &&
        // xs<ds[0]-1 &&
        //    zt>=0.0 && zt<dt[2]-1 && yt>=0.0 && yt<dt[1]-1 && xt>=0.0 &&
        //    xt<dt[0]-1 )
        {
          ...
          if (dim != 1)
            vs /= dim;
          // if (mask && vs == 0.0) continue;  // bad idea, try registering
          // skull stripped to full
          ivs = (int)floor(vs);
          ...
          if (ivs < 0 || ivt < 0 || ivs >= n || ivt >= n) {
            std::cout << "ERROR at ( " << x << " " << y << " " << z << " ) "
                      << std::endl;
            std::cout << " PS: ( " << xs << " " << ys << " " << zs
                      << " )  PT: ( " << xt << " " << yt << " " << zt << " )"
                      << std::endl;
            std::cout << " ivs: " << ivs << "  ivt: " << ivt << "  n: " << n
                      << std::endl;
            exit(1);
          }
          sdiff = vs - ivs;
...
void JointHisto::create(MRI_BSPLINE *mriS, MRI_BSPLINE *mriT,
  ...
  int    ivs, ivt;
  double sdiff, tdiff;
  double dt[3] = {(double)mriT->coeff->width, (double)mriT->coeff->height,
                  (double)mriT->coeff->depth};
  // double ds[3] = { mriS->width, mriS->height, mriS->depth};
...
// nmi = [sum_i(Pr(i) log2(Pr(i))) + sum_i(Pc(i) log2(Pc(i))) ] / sum_ij (P(i,j)
// log2(P(i,j))) (H(M) + H(N)) / H(M,N)
{
  double s1 = 0;
  double s2 = 0;
  int    i, j;
