...
double CostFunctions::mean(MRI *mri, const vnl_matrix_fixed<double, 4, 4> &Mi,
                           int frame, int d1, int d2, int d3) {
  return mean(mri, Mi, frame, d1, d2, d3, 0, mri->width, 0, mri->height, 0,
              mri->depth);
}

double CostFunctions::mean(MRI *mri, const vnl_matrix_fixed<double, 4, 4> &Mi,
                           int frame, int d1, int d2, int d3, int xmin,
  ...
  if (frame < 0 || frame >= mri->nframes) {
    std::cerr << " ERROR: CostFunctions::mean frame " << frame
...
#pragma omp atomic
  ...
  unsigned int n = (((xmax - xmin - 1) / d1) + 1) *
                       (((ymax - ymin - 1) / d2) + 1) *
                       (((zmax - zmin - 1) / d3) + 1) -
                   ocount;
  assert(n == count);
...
double CostFunctions::leastSquares(MRI *mriS, MRI *mriT,
                                   const vnl_matrix_fixed<double, 4, 4> &Msi,
                                   const vnl_matrix_fixed<double, 4, 4> &Mti,
                                   int d1, int d2, int d3, const double &s1,
                                   const double &s2) {
