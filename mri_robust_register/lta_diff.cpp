...
static char vcid[] = "$Id: lta_diff.cpp,v 1.77 2016/01/20 23:36:17 greve Exp $";
...
static bool parseCommandLine(int argc, char *argv[], Parameters &P) {
  ...
  if (ISOPTION(*argv[0])) {
    ...
    cerr << endl
         << endl
         << "ERROR: Please specify a transform file first !  " << endl
         << endl;
  ...
  if (argc > 0 && !ISOPTION(*argv[0])) {
    ...
    if (P.t2name == "identity.nofile") {
      P.t2name = "";
      cout << "No need to pass identity.nofile as 2nd, omitting it will work "
              "the same."
           << endl;
    } else
  ...
  if (argc > 0 && !ISOPTION(*argv[0])) {
    ...
    if (P.disttype == 0) {
      cerr << "ERROR: --dist: \"" << P.disttype
           << "\" not valid, expecting <int> 1... !" << endl;
      exit(1);
    }
  ...
  if (!test1) {
    ...
    cerr << endl
         << endl
         << "ERROR: Please specify second transform with --invert2 !  " << endl
         << endl;
    exit(1);
  }
...
int main(int argc, char *argv[]) {
  ...
  /*  if (argc < 3)
    {
      ...
      cout << endl;
      cout << "    dist-type " << endl;
      cout << "       1            Rigid Transform Distance (||log(R)|| +
    ||T||)"
          << endl;
      ...
      cout
          << "       6            Interpolation Smoothing (only for first
    transform)"
          << endl;
      ...
      cout << "                       pass 'identity.nofile' for second lta "
          << endl;
      cout
          << "    norm-div  (=1)  divide final distance by this (e.g. step
    adjustment)"
          << endl;
      cout
          << "    invert          invert LTA: 0 false (default), 1 invert first,
    2 second"
          << endl;
      cout << endl;
      cout
          << " For the RMS and Sphere we use a ball with radius 10cm at the RAS
    center."
          << endl;
    ...
    if (argc > 3)
    {
      ...
      if (disttype == 0)
      {
        cerr << "ERROR: dist-type: \"" << argv[3] << "\" not valid, expecting
    <int>!" << endl; exit(1);
      }
    }
    if (argc > 4)
  ...
  */
  ...
  if (P.invert2) {
    if (!lta2) {
      cerr << "ERROR: cannot invert 2nd transform, as it is not given or "
              "identity!\n";
      exit(1);
    }
