...
main(int argc, char *argv[])
{
  ...
  if (MRISreadCanonicalCoordinates(mris, sphere_name) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: MRISreadCanonicalCoordinates(%s) failed",
              Progname, sphere_name);
  ...
  for (n = 0; n < area->n_points; n++) {
    MRI *mri_tmp;
    mri_tmp =
        MRISextractVolumeWindow(mris, mri_norm, wsize, area->lv[n].vno, 0);
...
#define MAX_ITER 1000000
  if (random_patches > 0) {
    ...
    if (iter >= MAX_ITER)
      ErrorExit(
          ERROR_NOFILE,
          "%s: max iterations %d reached without finding enough random patches",
          Progname, iter);
    for (i = 0; n < npoints; n++, i++) {
      MRI *mri_tmp;
      mri_tmp = MRISextractVolumeWindow(mris, mri_norm, wsize,
                                        non_fcd_vertices[i], 0);
      MRIcopyFrame(mri_tmp, mri_patches, 0, n);
      MRIfree(&mri_tmp);
  ...
  for (a = 0; a < augment; a++) {
    ...
    for (n1 = 0; n1 < area->n_points; n1++, n++) {
      ...
      mri_tmp = MRISextractVolumeWindow(mris, mri_norm, wsize, area->lv[n1].vno,
                                        theta);
      MRIcopyFrame(mri_tmp, mri_patches, 0, n);
      MRIfree(&mri_tmp);
    ...
    for (n1 = 0; n1 < area->n_points; n1++, n++) {
      ...
      if (ovno < 0)
        ErrorExit(ERROR_BADPARM, "%s: could not find closest vertex to %d\n",
                  area->lv[n1].vno);
      mri_tmp =
          MRISextractVolumeWindow(mris_ohemi, mri_onorm, wsize, ovno, theta);
      MRIcopyFrame(mri_tmp, mri_patches, 0, n);
      MRIfree(&mri_tmp);
      MRIsetVoxVal(mri_labels, n, 0, 0, 0, ovno);
      MRIsetVoxVal(mri_labels, n, 1, 0, 0, 0); // mark it as an FCD
  ...
  seconds = seconds % 60;
  fprintf(stdout,
          "patch extraction took %d minutes"
          " and %d seconds.\n",
          minutes, seconds);
