...
static XtActionsRec actions[] = {
    {"Loop", loop_proc},      {"Swing", swing_proc},
    {"Faster", faster_proc},  {"Slower", slower_proc},
    {"Stop", stop_proc},      {"Forward", forward_proc},
    {"Back", back_proc},      {"Quit", quit_proc},
    {"Update", update_proc},  {"Mark", mark_proc},
...
static void XInit(int *argc, char ***argv) {
  ...
  xi.disp =
      XtOpenDisplay(xi.context, NULL, "NMovie", "NMovie", NULL, 0, argc, *argv);
  if (!xi.disp)
...
static void XSetupDisplay(int nframes) {
  ...
  toplevel = XtVaAppCreateShell("NMovie", "NMovie", applicationShellWidgetClass,
                                xi.disp, XtNvisual, xi.vis, XtNcolormap,
                                xi.colormap, NULL);
  ...
  loop_bt = XtVaCreateManagedWidget("Loop", commandWidgetClass, buttons, NULL);
  swing_bt =
      XtVaCreateManagedWidget("Swing", commandWidgetClass, buttons, NULL);
  fast_bt =
      XtVaCreateManagedWidget("Faster", commandWidgetClass, buttons, NULL);
  slow_bt =
      XtVaCreateManagedWidget("Slower", commandWidgetClass, buttons, NULL);
  stop_bt = XtVaCreateManagedWidget("Stop", commandWidgetClass, buttons, NULL);
  back_bt = XtVaCreateManagedWidget("Back", commandWidgetClass, buttons, NULL);
  forward_bt =
      XtVaCreateManagedWidget("Forward", commandWidgetClass, buttons, NULL);
  quit_bt = XtVaCreateManagedWidget("Quit", commandWidgetClass, buttons, NULL);
  // canvas
  canvas = XtVaCreateManagedWidget("Canvas", simpleWidgetClass, frame, XtNwidth,
                                   cols, XtNheight, rows, XtNaccelerators, keys,
                                   NULL);
  ...
  ximg =
      XCreateImage(xi.disp, xi.vis, xi.depth, ZPixmap, 0, NULL,
...
void ConvertImages(int nframes, char **argv) {
  ...
  for (i = 0; i < nframes; i++) {
    ...
    if (!I)
      ErrorExit(ERROR_NOFILE, "%s: could not read image file %s\n", Progname,
                argv[i + 1]);
    if (i == 0) {
      rows = I->rows;
      cols = I->cols;
...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: nmovie.c,v 1.31 2012/03/17 20:33:11 nicks Exp $",
      "$Name:  $");
