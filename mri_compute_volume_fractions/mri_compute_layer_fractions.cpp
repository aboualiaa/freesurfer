...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: mri_compute_layer_fractions.c,v 1.4 "
                                "2012/04/11 17:52:45 fischl Exp $",
                                "$Name:  $");
  ...
  if (FS_names && nlayers != 1)
    ErrorExit(ERROR_UNSUPPORTED,
              "%s: if specifying FS_names must use -nlayers 1", Progname);
  ...
  if (noaseg == 0)
    add_aseg_structures_outside_ribbon(mri_layers, mri_aseg, mri_layers, WM_VAL,
...

  {
         ...
         // space
    {
      ...
      m_seg_to_epi_vox2vox = MatrixMultiply(m_conformed_to_epi_vox2vox,
                                            m_seg_to_conformed_vox2vox, NULL);
      MatrixFree(&m_regdat);
    ...
    mri_fractions = MRIallocSequence(mri_in->width, mri_in->height,
                                     mri_in->depth, MRI_FLOAT, NLABELS);
  ...
  for (x = 0; x < mri_dst->width; x++) {
    ...
    for (y = 0; y < mri_dst->height; y++) {
      ...
      for (z = 0; z < mri_dst->depth; z++) {
        ...
        if (seg_label > wm_val &&
            seg_label < csf_val) // already labeled as cortex
          continue;
        V3_Z(v1) = z;
        MatrixMultiply(m_vox2vox, v1, v2);
        xa = (int)(nint(V3_X(v2)));
