...
static MRI *compute_layer_intensities(MRI *mri_intensities,
                                      ...
                                      int curv_bins);
static MRI *compute_thresholded_layer_intensities(
    MRI *mri_intensities, MRI *mri_volume_fractions, MRI_SURFACE **mris,
    int nlayers, int whalf0, MRI *mri_layer_intensities, int curv_bins,
    double vfrac_thresh);
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: mris_compute_layer_intensities.c,v 1.9 "
                                "2016/04/19 13:31:39 fischl Exp $",
                                "$Name:  $");
  ...
  if (mri_volume_fractions == nullptr)
    ErrorExit(ERROR_NOFILE, "%s: could not load volume fractions from %s",
              Progname, argv[2]);
  if (mri_volume_fractions->nframes < nlayers + 1)
    ErrorExit(
        ERROR_BADFILE,
        "%s: volume fraction input has fewer frames (%d) than needed (%d)\n",
        mri_volume_fractions->nframes, nlayers + 1);
  if (FS_names && subject_name == nullptr)
    ErrorExit(ERROR_UNSUPPORTED,
              "%s: if specifying FS_names must use -s <subject>", Progname);
  for (i = 0; i <= nlayers; i++) {
  ...
  if (vfrac_thresh > 0)
    mri_layer_intensities = compute_thresholded_layer_intensities(
        mri_intensities, mri_volume_fractions, mris, nlayers, Gwhalf, nullptr,
        curv_bins, vfrac_thresh);
  else
    mri_layer_intensities =
        compute_layer_intensities(mri_intensities, mri_volume_fractions, mris,
                                  nlayers, Gwhalf, nullptr, curv_bins);
  {
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input intensity volume> <layer volume fractions "
         "file> <input surface> <output overlay>\n",
         Progname);
  printf("\t\n");
  exit(code);
}
...
static MRI *compute_layer_intensities(MRI *mri_intensities,
  ...
  /*
    try to find voxels with a lot of each layer, but the more layers there are
    the less likely we are to be able to do so. The higher res the data the
    higher the threshold should be (as the more likely we are to be able to find
    voxels with a lot of one layer. xsize = 1,  nlayers = 2 --> vfrac = .75
  ...
  */
  ...
  if (mri_layer_intensities == nullptr)
    mri_layer_intensities =
        MRIallocSequence(mris[0]->nvertices, 1, 1, MRI_FLOAT, nlayers + 2);
  ...
  if (curv_bins > 1) {
    ...
    for (vno = 0; vno < mris[0]->nvertices; vno++) {
      ...
      for (n = 0; n <= nlayers; n++) {
        ...
        MRISsurfaceRASToVoxelCached(mris[n], mri_intensities, v->x, v->y, v->z,
                                    &xv, &yv, &zv);
    ...
    /*
      set bin to unused value for each spot that has no non-zero fractions so
      that that voxel it isn't used below
    */
...
#define MAX_VALS 2000
  ...
  mL = MatrixAlloc(nlayers + 2, 1,
                   MATRIX_REAL); // wm+nlayers+csf+ (subcortical gray maybe)
  for (vno = 0; vno < mris[0]->nvertices; vno++) {
    if ((vno % 500) == 0)
      printf("processing vno %d of %d (%2.1f%%)\n", vno, mris[0]->nvertices,
             100.0f * vno / mris[0]->nvertices);
    v = &mris[0]->vertices[vno];
    if (vno == Gdiag_no)
      DiagBreak();
    ...
    do {
      ...
      for (nvals = 0, n = 0; nvals < MAX_VALS && n <= nlayers;
           n++) // in normal direction
      {
        for (t1 = -whalf; nvals < MAX_VALS && t1 <= whalf;
             t1++) // in tangent plane
        {
          for (t2 = -whalf; nvals < MAX_VALS && t2 <= whalf; t2++) {
            ...
            MRISsurfaceRASToVoxelCached(mris[n], mri_intensities, xs, ys, zs,
                                        &xv, &yv, &zv);
            if (MRIindexNotInVolume(mri_intensities, xv, yv, zv) != 0) {
              out_of_fov = 1;
              break;
            ...
            if (MRIgetVoxVal(mri_visited, xvi, yvi, zvi, 0) ==
                0) // new voxel found
            {
              ...
              *MATRIX_RELT(mF, nvals + 1, 1) =
                  MRIgetVoxVal(mri_volume_fractions, xvi, yvi, zvi, 0); // wm
              *MATRIX_RELT(mF, nvals + 1, nlayers + 2) = MRIgetVoxVal(
                  mri_volume_fractions, xvi, yvi, zvi, nlayers + 1); // csf
              nvals++;
            }
          }
        }
      ...
      /*
        now go whalf into the wm and whalf outside the last surface to get good
        estimates of WM and CSF
      */
        ...
        for (nd = -1; nvals < MAX_VALS && nd <= whalf + 1; nd++) {
               ...
               t1++) // in tangent plane
          {
            for (t2 = -whalf; nvals < MAX_VALS && t2 <= whalf; t2++) {
              ...
              zs = v->z + step * t1 * v->e1z + step * t2 * v->e2z +
                   nd * step * nz;
              MRISsurfaceRASToVoxelCached(mris[nlayers], mri_intensities, xs,
                                          ys, zs, &xv, &yv, &zv);
              if (MRIindexNotInVolume(mri_intensities, xv, yv, zv) != 0) {
                out_of_fov = 1;
                break;
      ...
      if (vno == Gdiag_no)
        DiagBreak();
      mF->rows = nvals;
      mI->rows = nvals; // temporary - will be reset below (avoids allocating
                        // frequently)
      ...
      if (estimable) {
        if (MatrixNSConditionNumber(mF) >
            100000) // can't reliably estimate parameters
          estimable = 0;
        else // matrix is well-conditioned and we have found voxels with
             // majority of each layer
        {
          mFinv = MatrixPseudoInverse(mF, nullptr);
          if (mFinv ==
              nullptr) // shouldn't happen since we check condition # above
            estimable = 0;
      ...
      whalf++; // if couldn't estimate inverse use a bigger neighborhood and
               // more data
      mF->rows = MAX_VALS;
      mI->rows = MAX_VALS;
      if (whalf > 10 * (whalf0 + 1) / step ||
          (!estimable && nvals >= MAX_VALS - 1))
        break; // failed to find enough vals
    } while (
        !estimable); // most values are 0 - make sure we have enough to estimate
    if (estimable) {
      whalf_total += (step * whalf);
      nwindows++;
      mris[0]->vertices[vno].val = whalf / step;
    }

    mF->rows = MAX_VALS;
    mI->rows = MAX_VALS; // in case loop was broken out of
  }

  printf("%d vertices estimated, average window size = %2.1fmm\n", nwindows,
         whalf_total / nwindows);
  MatrixFree(&mF);
  MatrixFree(&mI);
  MatrixFree(&mL);
...
static MRI *compute_thresholded_layer_intensities(
  ...
  for (vno = 0; vno < mris[0]->nvertices; vno++) {
    ...
    if (FEQUAL(mris[0]->vertices[vno].x, mris[nlayers]->vertices[vno].x) &&
        ...
        FEQUAL(mris[0]->vertices[vno].z, mris[nlayers]->vertices[vno].z)) {
      DiagBreak();
      continue; // pial and white in same place - vertex is not cortical and
                // can't be estimated
    }
    if (bin_size == 0)
      bin0 = 0;
    ...
    for (n = 0; n <= nlayers; n++) // in normal direction
    {
      ...
      for (t1 = -whalf; t1 <= whalf; t1++) // in tangent plane
        for (t2 = -whalf; t2 <= whalf; t2++) {
          ...
          ys = v->y + step * t1 * v->e1y + step * t2 * v->e2y;
          zs = v->z + step * t1 * v->e1z + step * t2 * v->e2z;
          MRISsurfaceRASToVoxelCached(mris[n], mri_intensities, xs, ys, zs, &xv,
                                      &yv, &zv);
          if (MRIindexNotInVolume(mri_intensities, xv, yv, zv) != 0)
            continue;
          xvi = nint(xv);
    ...
    for (n = 0; n <= nlayers; n += nlayers) {
      ...
      for (nd = -1; nd <= whalf + 1; nd++) {
        for (t1 = -whalf; t1 <= whalf; t1++) // in tangent plane
        {
          for (t2 = -whalf; t2 <= whalf; t2++) {
            xs =
                v->x + step * t1 * v->e1x + step * t2 * v->e2x + nd * step * nx;
            ys =
                v->y + step * t1 * v->e1y + step * t2 * v->e2y + nd * step * ny;
            zs =
                v->z + step * t1 * v->e1z + step * t2 * v->e2z + nd * step * nz;
            MRISsurfaceRASToVoxelCached(mris[nlayers], mri_intensities, xs, ys,
                                        zs, &xv, &yv, &zv);
            if (MRIindexNotInVolume(mri_intensities, xv, yv, zv) != 0) {
              out_of_fov = 1;
              break;
