 ...
 * image using the aseg and the surfaces. Uses a high-resolution internal
 * representation to compute the cortical fractions.
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  if (UseAseg) {
    ...
    if (FillCSF) {
      printf("Filling empty voxels with extracerebral CSF (if not there "
             "already), nDil=%d\n",
             nDil);
      if (aseg->type == MRI_UCHAR) {
    ...
    if (ttsegfile) {
      MRI *ttseg;
      ttseg = MRIseg2TissueType(aseg, ct, nullptr);
      if (ttseg == nullptr)
        exit(1);
  ...
  if (RegHeader) {
    ...
    if (temp == nullptr)
      exit(1);
    aseg2vol = TransformRegDat2LTA(orig, temp, nullptr);
  ...
  if (resmm == 0) {
    if (aseg)
      resmm = aseg->xsize / (USF);
    else
      resmm = 1.0 / (USF);
  }
  printf("Computing PVF (USF=%d, resmm=%lf)\n", USF, resmm);
  pvf = MRIpartialVolumeFractionAS(aseg2vol, aseg, lhw, lhp, rhw, rhp, USF,
                                   resmm, ct, nullptr);
  if (pvf == nullptr)
    exit(1);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--no-aseg"))
      UseAseg = 0;
    else if (!strcasecmp(option, "--fill-csf"))
      FillCSF = 1;
    else if (!strcasecmp(option, "--no-fill-csf"))
      FillCSF = 0;
    else if (!strcasecmp(option, "--ttype+head"))
      ct = TissueTypeSchema(nullptr, "default-jan-2014+head");
...
static void print_usage() {
  printf("USAGE: %s \n", Progname);
  printf("\n");
  printf("   --o   outstem : output will be "
         "oustem.{cortex,subcort_gm,wm,csf}.mgz\n");
  printf("   --reg regfile : can be LTA or reg.dat (if reg.dat, then need "
         "template volume)\n");
  ...
  printf("   --resmm resmm : set functional upsampling resolution (default is "
         "aseg->xsize/(USF))\n");
  ...
  printf("   --stack stackfile : put ctx,subcortgm,wm,csf into a single "
         "multi-frame file\n");
  printf("   --gm gmfile : put ctx+subcortgm into a single-frame file\n");
  printf("   --no-fill-csf : do not attempt to fill voxels surrounding seg "
         "with the extracerebral CSF segmetation \n");
  printf("     Note: when the fill is done, there is no attempt to actually "
         "segment xCSF voxels.\n");
  printf("     The passed segmentation is dilated and the new voxels become "
         "xCSF\n");
  printf("     Note: if the passed seg already has the CSF_ExtraCerebral seg, "
         "nothing will be done\n");
  printf("   --dil N : for xCSF fill, dilate by N (default is %d); use -1 to "
         "fill the entire volume \n",
         nDil);
  printf("   --out-seg outseg : save seg (after adding xcsf voxels)\n");
  printf("   --ttseg ttseg : save tissue type segmentation (probably not that "
         "useful)\n");
  printf("   --ttseg-ctab ctab : save tissue type segmentation ctab (probably "
         "not that useful)\n");
  printf("\n");
  ...
  printf("   --ttype+head : use default+head instead of default tissue type "
         "info for seg\n");
  printf("\n");
...
static void print_help() {
  print_usage();
  printf("Computes partial volume fractions for cortex, subcortical GM, WM and "
         "CSF\n");
  printf("\n");
  printf("Example\n");
  printf("   mri_compute_volume_fractions --nii.gz --reg reg.lta --usf 3 --o "
         "pvf.\n");
  printf(
      "This will create files called pvf.{cortex,subcort_gm,wm,csf}.nii.gz\n");
  printf("The value at each voxel will be between 0 and 1 and represent the "
         "fraction of \n");
...
static void check_options() {
  ...
  if (outstem == nullptr && stackfile == nullptr) {
  ...
  if ((regtype == REGISTER_DAT || RegHeader) && TempVolFile == nullptr) {
    printf("ERROR: template volume needed with register.dat file or "
           "--reg-header\n");
    exit(1);
  }
  if (!RegHeader) {
    ...
    if (debug)
      LTAprint(stdout, aseg2vol);
  }

  if (!RegHeader) {
    if (subjectoverride == nullptr)
      subject = aseg2vol->subject;
    else
      subject = subjectoverride;
  }
  if (ct == nullptr)
    ct = TissueTypeSchema(nullptr, "default-jan-2014");
...
static void dump_options(FILE *fp) {
  ...
  fprintf(fp, "psurf %s\n", psurf);
  if (TempVolFile)
    fprintf(fp, "TempVolFile %s\n", regfile);
  fprintf(fp, "USF %d\n", USF);
  if (subjectoverride)
    fprintf(fp, "subjectoverride %s\n", subjectoverride);
  return;
}
