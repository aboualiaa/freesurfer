...
int main(int argc, char **argv) {
  ...
  // mri =
  // MRIread("/autofs/cluster/fsm/users/greve/subjects/t1fit.fsm010/resample/coreg.0037.T2w_SPC_vNav.conf.mgz");
  mri = MRIread("/autofs/cluster/fsm/users/greve/subjects/fsm010/mri/norm.mgz");

  // printf("Reading surface\n"); fflush(stdout);
  // surf = MRISread("/homes/4/greve/subjects/vrfp-mar14-anat/surf/lh.white");
  surf =
      MRISread("/autofs/cluster/fsm/users/greve/subjects/fsm010/surf/lh.pial");
  ...
  for (s = 0; s < surf->nvertices; s++) {
    ...
    for (c = 0; c < 3; c++)
      printf("%12.10lf ",
             100000 * (dJ->rptr[s + 1][c + 1] - gnum->rptr[1][c + 1]));
    printf("\n");
    printf("g0   = ");
    for (c = 0; c < 3; c++)
      printf("%12.8lf ", 100000 * dJ->rptr[s + 1][c + 1]);
    printf("\n");
    printf("gnum = ");
    for (c = 0; c < 3; c++)
      printf("%12.8lf ", 100000 * gnum->rptr[1][c + 1]);
    printf("\n");
    fflush(stdout);
  }
  exit(0); //--------------------------------------
  ...
  printf("Smoothing\n");
  mytimer.reset();
  mri2 = GeoSmooth(mri, 10, surf, geod, mriindex, nullptr);
  msec = mytimer.milliseconds();
  printf("t = %g min\n", msec / (1000.0 * 60));
  fflush(stdout);
  MRIwrite(mri2, argv[5]);
  mri2 = MRIcopyMRIS(nullptr, surf, 0, "val2bak");
...
int MRISmatrix2Vertex(MRIS *surf, int vtxno, DMATRIX *vm) {
  ...
  MRISsetXYZ(surf, vtxno, vm->rptr[1][1], vm->rptr[2][1], vm->rptr[3][1]);
  return (0);
}
...
int MRISscaleVertices(MRIS *surf, double scale) {
  ...
  for (vtxno = 0; vtxno < surf->nvertices; vtxno++) {
    ...
    MRISsetXYZ(surf, vtxno, v->x * scale, v->y * scale,
...
#pragma omp parallel for
  ...
  for (vtxno = 0; vtxno < surf->nvertices; vtxno++) {
    ...
    MRISsetXYZ(surf, vtxno, v->x - (stepsize * grad->rptr[vtxno + 1][1]),
...
int PlaceSurf(MRI *mri, MRIS *surf) {
  ...
  for (n = 0; n < 1000; n++) {
    ...
    fflush(stdout);
    if (n == 20)
      MRISwrite(surf, "./lh.fsm010.pial.020");
    if (n == 100)
      MRISwrite(surf, "./lh.fsm010.pial.100");
    if (n == 200)
      MRISwrite(surf, "./lh.fsm010.pial.200");
    if (n == 400)
      MRISwrite(surf, "./lh.fsm010.pial.400");
    if (n == 700)
      MRISwrite(surf, "./lh.fsm010.pial.700");
  }
