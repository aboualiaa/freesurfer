...
int main(int argc, char *argv[]) {
  ...
  if (tp1_ctrl_fname == nullptr || tp1_T1_fname == nullptr) {
  ...
  if (!mri_in)
    ErrorExit(ERROR_BADPARM, "%s: could not read input volume %s", Progname,
              argv[1]);
  ...
  if (!mri_ctrl)
    ErrorExit(ERROR_BADPARM,
              "%s: could not read control points volume for tp1 %s", Progname,
              tp1_ctrl_fname);

  if ((mri_in->width != mri_T1->width) || (mri_in->height != mri_T1->height) ||
      (mri_in->depth != mri_T1->depth) || (mri_in->width != mri_ctrl->width) ||
  ...
  if (mask1_fname) {
    ...
    if (!mri_mask1)
      ErrorExit(ERROR_BADPARM, "%s, could not read mask volume for tp1 %s",
  ...
  if (mask2_fname) {
    ...
    if (!mri_mask2)
      ErrorExit(ERROR_BADPARM, "%s, could not read mask volume for tp2 %s",
  ...
  if (xform_fname != nullptr) {
    ...
    mri_tmp = MRIalloc(mri_ctrl->width, mri_ctrl->height, mri_ctrl->depth,
                       mri_ctrl->type);
    ...
    if (mask1_fname != nullptr && mask2_fname == nullptr) {
      printf("map mask for tp1 to get mask for tp2 ...\n");
      mri_mask2 = MRIalloc(mri_in->width, mri_in->height, mri_in->depth,
                           mri_mask1->type);
      MRIcopyHeader(mri_in, mri_mask2);

      mri_mask2 = LTAtransformInterp(mri_mask1, mri_mask2, lta, SAMPLE_NEAREST);
      mask2_set = 1;
      if (debug_flag)
      ...
      mri_mask1 = MRIalloc(mri_T1->width, mri_T1->height, mri_T1->depth,
                           mri_mask2->type);
      MRIcopyHeader(mri_T1, mri_mask1);

      mri_mask1 = LTAtransformInterp(mri_mask2, mri_mask1, lta, SAMPLE_NEAREST);
      mask1_set = 1;
      if (debug_flag)
  ...
  if (mask1_set == 0) {
    // create mask1
    mri_mask1 =
        MRIalloc(mri_T1->width, mri_T1->height, mri_T1->depth, MRI_UCHAR);
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
  ...
  if (mask2_set == 0) {
    // create mask2
    mri_mask2 =
        MRIalloc(mri_in->width, mri_in->height, mri_in->depth, MRI_UCHAR);
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
  ...
  mri_bias = MRIbuildBiasImage(mri_in, mri_ctrl, nullptr, bias_sigma);
...
static void usage(int exit_val) {
  ...
  fprintf(fout,
          "\t -mask1 %%s: brain mask for tp1 (will be "
