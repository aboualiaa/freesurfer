...
PanelSurface::PanelSurface(QWidget *parent)
  ...
  m_widgetsSlope << ui->sliderSlope << ui->lineEditSlope << ui->labelSlope;

  m_widgetsMidPoint << ui->sliderMidPoint << ui->lineEditMidPoint
                    << ui->labelMidPoint;

  m_widgetsVector << ui->colorpickerVectorColor << ui->spinBoxVectorPointSize
                  << ui->labelVectorColor << ui->labelVectorPointSize;

  m_widgetsVertex << ui->colorpickerVertexColor << ui->spinBoxVertexPointSize
                  << ui->labelVertexColor << ui->labelVertexPointSize;

  m_widgetsMesh << ui->comboBoxMeshColor << ui->labelMeshColor;

  m_widgetsLabel << ui->colorpickerLabelColor << ui->labelLabelColor
                 << ui->checkBoxLabelOutline << ui->treeWidgetLabels
                 << ui->widgetLabelOrderButtons << ui->labelLabelThreshold
                 << ui->comboBoxLabelColorCode << ui->lineEditLabelThreshold
                 << ui->lineEditLabelHeatscaleMin
                 << ui->lineEditLabelHeatscaleMax << ui->labelHeatscaleRange
                 << ui->labelLabelZOrderLabel << ui->spinBoxZOrderLabel
                 << ui->labelLabelOpacity << ui->lineEditLabelOpacity
                 << ui->toolButtonMoreLabelOptions;

  m_widgetsOverlay << ui->pushButtonConfigureOverlay
                   << ui->pushButtonRemoveOverlay << ui->labelLabelZOrderOverlay
  ...
  connect(ag, SIGNAL(triggered(QAction *)), this,
          SLOT(OnChangeSurfaceType(QAction *)));
  ...
  connect(mainwnd->GetLayerCollection("Surface"),
          SIGNAL(ActiveLayerChanged(Layer *)), m_wndConfigureOverlay,
          SLOT(OnActiveSurfaceChanged(Layer *)));
  connect(m_wndConfigureOverlay, SIGNAL(ActiveFrameChanged(int)), mainwnd,
          SLOT(UpdateInfoPanel()));
  connect(m_wndConfigureOverlay, SIGNAL(ActiveFrameChanged(int)), mainwnd,
          SLOT(SetCurrentTimeCourseFrame(int)));
  connect(mainwnd, SIGNAL(SlicePositionChanged()), m_wndConfigureOverlay,
          SLOT(OnCurrentVertexChanged()));
  connect(m_wndConfigureOverlay, SIGNAL(MaskLoadRequested(QString)), mainwnd,
          SLOT(OnLoadSurfaceLabelRequested(QString)));
  connect(m_wndConfigureOverlay, SIGNAL(OverlayChanged()),
          SLOT(UpdateWidgets()));
  ...
  connect(ui->actionCutClosedLine, SIGNAL(triggered(bool)),
          SLOT(OnButtonCutClosedLine()));
  connect(ui->actionCutClear, SIGNAL(triggered(bool)),
          SLOT(OnButtonClearCuts()));
  connect(ui->actionFillUncutArea, SIGNAL(triggered(bool)),
          SLOT(OnButtonFillUncutArea()));
  connect(ui->actionUndoCut, SIGNAL(triggered(bool)), SLOT(OnButtonUndoCut()));

  connect(ui->actionMakePath, SIGNAL(triggered(bool)),
          SLOT(OnButtonMakePath()));
  connect(ui->actionMakeClosedPath, SIGNAL(triggered(bool)),
          SLOT(OnButtonMakeClosedPath()));
  connect(ui->actionDeletePath, SIGNAL(triggered(bool)),
          SLOT(OnButtonDeletePath()));
  connect(ui->actionPathFill, SIGNAL(triggered(bool)),
          SLOT(OnButtonCustomFillPath()));

  connect(ui->actionClearMarks, SIGNAL(triggered(bool)),
          SLOT(OnButtonClearMarks()));

  connect(ui->toolButtonLabelUp, SIGNAL(clicked(bool)),
          SLOT(OnButtonLabelUp()));
  connect(ui->toolButtonLabelDown, SIGNAL(clicked(bool)),
          SLOT(OnButtonLabelDown()));

  m_dlgCustomFill = new DialogCustomFill(this);
  m_dlgCustomFill->hide();
  connect(m_dlgCustomFill, SIGNAL(CustomFillTriggered(QVariantMap)),
          SLOT(OnCustomFillTriggered(QVariantMap)));

  m_dlgLabelOps = new DialogSurfaceLabelOperations(this);
  m_dlgLabelOps->hide();
  connect(m_dlgLabelOps, SIGNAL(OperationTriggered(QVariantMap)),
          SLOT(OnLabelOperation(QVariantMap)));
...
PanelSurface::~PanelSurface() { delete ui; }
...
void PanelSurface::ConnectLayer(Layer *layer_in) {
  ...
  connect(p, SIGNAL(PropertyChanged()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceAnnotationAdded(SurfaceAnnotation *)), this,
          SLOT(UpdateWidgets()), Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceLabelAdded(SurfaceLabel *)), this,
          SLOT(UpdateWidgets()), Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceRGBAdded()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceLabelDeleted(SurfaceLabel *)), this,
          SLOT(UpdateWidgets()));
  ...
  connect(layer, SIGNAL(SurfaceOverlayAdded(SurfaceOverlay *)), this,
          SLOT(UpdateWidgets()));
  connect(layer, SIGNAL(SurfaceSplineAdded(SurfaceSpline *)), this,
          SLOT(UpdateWidgets()), Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceSplineDeleted(SurfaceSpline *)), this,
          SLOT(UpdateWidgets()));
  ...
  connect(ui->checkBoxShowVertices, SIGNAL(toggled(bool)), p,
          SLOT(ShowVertices(bool)));
  connect(ui->colorpickerEdgeColor, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetEdgeColor(QColor)));
  connect(ui->colorpickerVectorColor, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetVectorColor(QColor)));
  connect(ui->colorpickerSurfaceColor, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetBinaryColor(QColor)));
  connect(ui->colorpickerVertexColor, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetVertexColor(QColor)));
  connect(ui->comboBoxMeshColor, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetMeshColorMap(int)));
  connect(ui->comboBoxRender, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetSurfaceRenderMode(int)));
  connect(ui->spinBoxEdgeThickness, SIGNAL(valueChanged(int)), p,
          SLOT(SetEdgeThickness(int)));
  connect(ui->spinBoxVectorPointSize, SIGNAL(valueChanged(int)), p,
          SLOT(SetVectorPointSize(int)));
  connect(ui->spinBoxVertexPointSize, SIGNAL(valueChanged(int)), p,
          SLOT(SetVertexPointSize(int)));
  connect(ui->checkBoxUseSurfaceColor2D, SIGNAL(toggled(bool)), p,
          SLOT(SetUseSurfaceColorOn2D(bool)));
  connect(ui->checkBoxAnnotationOutline, SIGNAL(toggled(bool)), layer,
          SLOT(SetActiveAnnotationOutline(bool)));
  connect(ui->checkBoxHideIn3DView, SIGNAL(toggled(bool)), layer,
          SLOT(SetHideIn3D(bool)));
  connect(ui->colorpickerSplineColor, SIGNAL(colorChanged(QColor)), this,
          SLOT(OnColorPickerSplineColor(QColor)));
  connect(ui->checkBoxSplineProjection, SIGNAL(toggled(bool)), this,
          SLOT(OnCheckBoxSplineProjection(bool)));
  connect(ui->spinBoxZOrderAnnotation, SIGNAL(valueChanged(int)),
          SLOT(OnSpinBoxZOrder(int)));
  connect(ui->spinBoxZOrderLabel, SIGNAL(valueChanged(int)),
          SLOT(OnSpinBoxZOrder(int)));
  connect(ui->spinBoxZOrderOverlay, SIGNAL(valueChanged(int)),
          SLOT(OnSpinBoxZOrder(int)));
  ...
  connect(ui->lineEditMappingSurface, SIGNAL(textChanged(QString)), layer,
          SLOT(SetMappingSurfaceName(QString)));
}
...
void PanelSurface::DoIdle() {
  ...
  ui->actionSurfaceMain->setChecked(layer && layer->GetActiveSurface() ==
                                                 FSSurface::SurfaceMain);
  ui->actionSurfaceInflated->setEnabled(
      surf && surf->IsSurfaceLoaded(FSSurface::SurfaceInflated));
  ui->actionSurfaceInflated->setChecked(
      layer && layer->GetActiveSurface() == FSSurface::SurfaceInflated);
  ui->actionSurfaceOriginal->setEnabled(
      surf && surf->IsSurfaceLoaded(FSSurface::SurfaceOriginal));
  ui->actionSurfaceOriginal->setChecked(
      layer && layer->GetActiveSurface() == FSSurface::SurfaceOriginal);
  ui->actionSurfaceWhite->setEnabled(
      surf && surf->IsSurfaceLoaded(FSSurface::SurfaceWhite));
  ui->actionSurfaceWhite->setChecked(layer && layer->GetActiveSurface() ==
                                                  FSSurface::SurfaceWhite);
  ui->actionSurfacePial->setEnabled(
      surf && surf->IsSurfaceLoaded(FSSurface::SurfacePial));
  ui->actionSurfacePial->setChecked(layer && layer->GetActiveSurface() ==
                                                 FSSurface::SurfacePial);
  ui->actionShowOverlay->setEnabled(layer && layer->GetNumberOfOverlays() > 0);
  ui->actionShowOverlay->setChecked(layer &&
                                    layer->GetProperty()->GetShowOverlay());
  ui->actionShowAnnotation->setEnabled(layer &&
                                       layer->GetNumberOfAnnotations() > 0);
  ui->actionShowAnnotation->setChecked(
      layer && layer->GetProperty()->GetShowAnnotation());
  ...
  */
  ...
  for (int i = 0; i < this->allWidgets.size(); i++) {
    if (allWidgets[i] != ui->toolbar &&
        ...
        allWidgets[i] != ui->toolbarPath &&
        allWidgets[i]->parentWidget() != ui->toolbarPath) {
      allWidgets[i]->setEnabled(layer);
    }
  }
  ui->actionCut->setChecked(MainWindow::GetMainWindow()->GetMode() ==
                            RenderView::IM_SurfaceCut);
  ui->actionPath->setChecked(MainWindow::GetMainWindow()->GetMode() ==
                             RenderView::IM_SurfacePath);
  FSSurface *surf = NULL;
  ...
  if (layer) {
    ...
    ui->colorpickerSurfaceColor->setCurrentColor(
        QColor((int)(rgb[0] * 255), (int)(rgb[1] * 255), (int)(rgb[2] * 255)));
    rgb = layer->GetProperty()->GetEdgeColor();
    ui->colorpickerEdgeColor->setCurrentColor(
        QColor((int)(rgb[0] * 255), (int)(rgb[1] * 255), (int)(rgb[2] * 255)));
    rgb = layer->GetProperty()->GetVectorColor();
    ui->colorpickerEdgeColor->setEnabled(
        !layer->GetProperty()->GetUseSurfaceColorOn2D());
    ui->checkBoxUseSurfaceColor2D->setChecked(
        layer->GetProperty()->GetUseSurfaceColorOn2D());
    ui->colorpickerVectorColor->setCurrentColor(
        QColor((int)(rgb[0] * 255), (int)(rgb[1] * 255), (int)(rgb[2] * 255)));
    ...
    ui->spinBoxEdgeThickness->setValue(
        layer->GetProperty()->GetEdgeThickness());
    ui->spinBoxVectorPointSize->setValue(
        layer->GetProperty()->GetVectorPointSize());

    ui->comboBoxCurvature->setCurrentIndex(
        layer->GetProperty()->GetCurvatureMap());
    ChangeLineEditNumber(ui->lineEditMidPoint,
                         layer->GetProperty()->GetThresholdMidPoint());
    ChangeLineEditNumber(ui->lineEditSlope,
                         layer->GetProperty()->GetThresholdSlope());
    double range[2];
    layer->GetCurvatureRange(range);
    ui->sliderMidPoint->setValue(
        (int)((layer->GetProperty()->GetThresholdMidPoint() - range[0]) /
              (range[1] - range[0]) * 100));
    ui->sliderSlope->setValue((int)(layer->GetProperty()->GetThresholdSlope()));

    ui->comboBoxRender->setCurrentIndex(
        layer->GetProperty()->GetSurfaceRenderMode());
    ui->comboBoxMeshColor->setCurrentIndex(
        layer->GetProperty()->GetMeshColorMap());
    ui->checkBoxShowVertices->setChecked(
        layer->GetProperty()->GetShowVertices());
    rgb = layer->GetProperty()->GetVertexColor();
    ui->colorpickerVertexColor->setCurrentColor(
        QColor((int)(rgb[0] * 255), (int)(rgb[1] * 255), (int)(rgb[2] * 255)));
    ChangeSpinBoxValue(ui->spinBoxVertexPointSize,
                       layer->GetProperty()->GetVertexPointSize());

    double *dPos = layer->GetProperty()->GetPosition();
    ChangeLineEditText(
        ui->lineEditPositionOffset,
        QString("%1 %2 %3").arg(dPos[0]).arg(dPos[1]).arg(dPos[2]));
  ...
  ui->comboBoxVectorDisplay->setCurrentIndex(surf ? 1 + surf->GetActiveVector()
                                                  : 0);
  ...
  ui->comboBoxOverlay->setCurrentIndex(
      layer ? 1 + layer->GetActiveOverlayIndex() : 0);
  ...
  if (layer) {
    for (int i = 0; i < layer->GetNumberOfAnnotations(); i++) {
      ...
      ui->comboBoxAnnotation->setItemData(
          i + 1, layer->GetAnnotation(i)->GetFilename(), Qt::ToolTipRole);
    }
  }
  ui->comboBoxAnnotation->addItem("Load from file...");
  ui->comboBoxAnnotation->setItemData(ui->comboBoxAnnotation->count() - 1,
                                      "Load from file", Qt::ToolTipRole);
  ui->comboBoxAnnotation->setCurrentIndex(
      layer ? 1 + layer->GetActiveAnnotationIndex() : 0);
  ui->comboBoxAnnotation->addItem("New...");

  // update label controls
  QList<SurfaceLabel *> selected_labels;
  if (layer &&
      ui->treeWidgetLabels->topLevelItemCount() == layer->GetNumberOfLabels())
  ...
  if (layer &&
      ui->treeWidgetSplines->topLevelItemCount() == layer->GetNumberOfSplines())
  ...
  if (layer && layer->GetActiveAnnotation()) {
    ui->checkBoxAnnotationOutline->setChecked(
        layer->GetActiveAnnotation()->GetShowOutline());
  }
  ...
  ShowWidgets(m_widgetsSlope,
              nCurvatureMap == LayerPropertySurface::CM_Threshold);
  ...
  ShowWidgets(m_widgetsMesh,
              layer && layer->GetProperty()->GetSurfaceRenderMode() !=
                           LayerPropertySurface::SM_Surface);
  ...
  ui->colorpickerSurfaceColor->setEnabled(
      layer); // && nCurvatureMap != LayerPropertySurface::CM_Threshold );
  ...
  if (layer) {
    ui->spinBoxZOrderAnnotation->setValue(
        layer->GetProperty()->GetZOrderAnnotation());
    ui->spinBoxZOrderLabel->setValue(layer->GetProperty()->GetZOrderLabel());
    ui->spinBoxZOrderOverlay->setValue(
        layer->GetProperty()->GetZOrderOverlay());
  }
...
void PanelSurface::UpdateLabelWidgets(bool block_signals) {
  ...
  if (layer && layer->GetActiveLabel()) {
    ...
    ChangeLineEditNumber(ui->lineEditLabelHeatscaleMin,
                         label->GetHeatscaleMin());
    ChangeLineEditNumber(ui->lineEditLabelHeatscaleMax,
                         label->GetHeatscaleMax());
    ...
    ui->labelHeatscaleRange->setVisible(label->GetColorCode() ==
                                        SurfaceLabel::Heatscale);
    ui->lineEditLabelHeatscaleMin->setVisible(label->GetColorCode() ==
                                              SurfaceLabel::Heatscale);
    ui->lineEditLabelHeatscaleMax->setVisible(label->GetColorCode() ==
                                              SurfaceLabel::Heatscale);
    ui->colorpickerLabelColor->setVisible(label->GetColorCode() ==
                                          SurfaceLabel::SolidColor);

    ui->toolButtonLabelUp->setEnabled(label != layer->GetLabel(0));
    ui->toolButtonLabelDown->setEnabled(
        label != layer->GetLabel(layer->GetNumberOfLabels() - 1));
...
QList<SurfaceLabel *> PanelSurface::GetSelectedLabels() {
  ...
  foreach (QTreeWidgetItem *item, selected_items) {
    selected_labels << qobject_cast<SurfaceLabel *>(
...
QList<SurfaceSpline *> PanelSurface::GetSelectedSplines() {
  ...
  foreach (QTreeWidgetItem *item, selected_items) {
    selected_splines << reinterpret_cast<SurfaceSpline *>(
...
void PanelSurface::OnSliderMidPoint(int nVal) {
  ...
  foreach (LayerSurface *layer, layers) {
    double range[2];
    layer->GetCurvatureRange(range);
    layer->GetProperty()->SetThresholdMidPoint(
        nVal / 100.0 * (range[1] - range[0]) + range[0]);
  }
}
...
void PanelSurface::OnSliderSlope(int nVal) {
  ...
  foreach (LayerSurface *layer, layers) {
    double fMin = 0;
    double fMax = 100;
    layer->GetProperty()->SetThresholdSlope(nVal / 100.0 * (fMax - fMin) +
                                            fMin);
  }
}
...
void PanelSurface::OnComboOverlay(int nSel_in) {
  ...
  if (surf) {
    ...
    if (nSel < surf->GetNumberOfOverlays()) {
      ...
      MainWindow::GetMainWindow()->LoadSurfaceOverlay(
          nSel > surf->GetNumberOfOverlays());
    }
    UpdateWidgets();
  }
}
...
void PanelSurface::OnButtonDeleteLabel() {
  ...
  SurfaceLabel *label = qobject_cast<SurfaceLabel *>(
      item->data(0, Qt::UserRole).value<QObject *>());
  if (label) {
    LayerSurface *surf = GetCurrentLayer<LayerSurface *>();
    if (surf) {
...
void PanelSurface::OnEditPositionOffset() {
  QList<LayerSurface *> layers = GetSelectedLayers<LayerSurface *>();
  foreach (LayerSurface *layer, layers) {
    QStringList args = ui->lineEditPositionOffset->text().trimmed().split(
...
void PanelSurface::OnComboLabelColorCode(int nSel) {
  ...
  if (surf) {
    QList<SurfaceLabel *> labels = GetSelectedLabels();
    foreach (SurfaceLabel *label, labels) { label->SetColorCode(nSel); }
    surf->UpdateColorMap();
  }
}
...
void PanelSurface::OnLockLayer(bool b) {
  QList<LayerSurface *> layers = GetSelectedLayers<LayerSurface *>();
  foreach (LayerSurface *layer, layers) { layer->Lock(b); }
}
...
void PanelSurface::OnButtonCutLine() {
  ...
  if (surf && surf->GetMarks()) {
    if (surf->GetMarks()->GetPathVerts().size() < 2)
      QMessageBox::warning(this->parentWidget(), "Error",
                           "Need at least 2 marks to cut line");
    else
...
void PanelSurface::OnButtonCutClosedLine() {
  ...
  if (surf && surf->GetMarks()) {
    if (surf->GetMarks()->GetPathVerts().size() < 3)
      QMessageBox::warning(this->parentWidget(), "Error",
                           "Need at least 3 marks to cut closed line");
    else
  ...
  if (surf) {
    ...
    if (vno < 0)
      QMessageBox::information(this->parentWidget(), "Fill Cut Area",
                               "Please move cursor to a valid vertex");
...
void PanelSurface::OnLabelResample() {
  ...
  if (!mri) {
    QMessageBox::warning(this, "Error",
                         "Could not find a MRI template for resampling");
    return;
  }
...
void PanelSurface::OnButtonMakePath() {
  ...
  if (surf && surf->GetMarks()) {
    if (surf->GetMarks()->GetPathVerts().size() < 2)
      QMessageBox::warning(this->parentWidget(), "Error",
                           "Need at least 2 marks to make path");
    else
...
void PanelSurface::OnButtonMakeClosedPath() {
  ...
  if (surf && surf->GetMarks()) {
    if (surf->GetMarks()->GetPathVerts().size() < 3)
      QMessageBox::warning(this->parentWidget(), "Error",
                           "Need at least 3 marks to make closed path");
    else
...
void PanelSurface::OnButtonSaveAnnotation() {
  ...
  if (surf && surf->GetActiveAnnotation()) {
    ...
    if (annot->HasUnassignedLabels()) {
      QMessageBox::information(this, "Save Annotatino",
                               "There are unassigned labels in the annotation. "
                               "Please assign all the labels first.");
      return;
    }
    QDir dir = QFileInfo(surf->GetFileName()).absoluteDir();
    dir.cdUp();
    ...
    if (fn.isEmpty()) {
      ...
      if (!def_fn.contains(".annot"))
        def_fn += ".annot";
      def_fn = dir.absoluteFilePath(def_fn);
      fn = QFileDialog::getSaveFileName(this, "Select annotation file", def_fn,
