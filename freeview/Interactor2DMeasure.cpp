...
Interactor2DMeasure::Interactor2DMeasure(QObject *parent)
    : Interactor2D(parent), m_bEditing(false), m_bDrawing(false),
      m_nPointIndex(-1), m_region(NULL) {}
  ...
  if (event->button() == Qt::LeftButton) {
    ...
    if (mri) {
      ...
      if (m_region && m_bDrawing) // drawing
      {
        ...
        Region2D *reg =
            view->GetRegion(m_nMousePosX, m_nMousePosY, &m_nPointIndex);
        if (!reg) // new region
        {
          if (m_nAction == MM_Line) {
            Region2DLine *reg_line = new Region2DLine(view);
            reg_line->SetLine(m_nMousePosX, m_nMousePosY, m_nMousePosX,
                              m_nMousePosY);
            view->AddRegion(reg_line);
            m_region = reg_line;
          } else if (m_nAction == MM_Spline || m_nAction == MM_Polyline) {
            Region2DPolyline *reg_polyline =
                new Region2DPolyline(view, m_nAction == MM_Spline);
            reg_polyline->AddPoint(m_nMousePosX, m_nMousePosY);
            reg_polyline->AddPoint(m_nMousePosX,
                                   m_nMousePosY); // add second point
    ...
    if (m_nMousePosX != event->x() || m_nMousePosY != event->y()) {
      ...
      if (m_region) {
        if (m_nAction == MM_Line) {
          ...
          ((Region2DRectangle *)m_region)
              ->SetBottomRight(m_nMousePosX, m_nMousePosY);
        }
      }
    } else {
  ...
  if (wnd->hasFocus()) {
    if (event->type() == QEvent::MouseButtonPress ||
      ...
      if (((e->button() == Qt::MidButton || e->button() == Qt::RightButton) &&
        ...
        return;
      } else if ((!m_bEditing && !m_bDrawing &&
                  view->GetRegion(e->x(), e->y())) ||
