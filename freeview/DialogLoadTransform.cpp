...
DialogLoadTransform::DialogLoadTransform(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogLoadTransform) {
  ui->setupUi(this);
  connect(ui->toolButtonOpenRegistration, SIGNAL(clicked(bool)),
          SLOT(OnButtonOpen()));
}

DialogLoadTransform::~DialogLoadTransform() { delete ui; }

void DialogLoadTransform::OnOK() {
  if (ui->lineEditRegistration->text().trimmed().isEmpty()) {
    QMessageBox::warning(this, tr("Error"),
                         tr("Please select transformation file to Load"));
  } else
    accept();
}
