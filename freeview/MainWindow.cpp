...
MainWindow::MainWindow(QWidget *parent, MyCmdLineParser *cmdParser)
    : QMainWindow(parent), ui(new Ui::MainWindow), m_bResampleToRAS(false),
  ...
  connect(m_volumeCropper, SIGNAL(CropBoundChanged(LayerMRI *)), this,
          SLOT(RequestRedraw()));
  ...
  for (int i = 0; i < 3; i++) {
    ...
    connect(m_views[i], SIGNAL(CursorLocationClicked()), this,
            SLOT(On2DCursorClicked()));
  }
  connect(m_layerCollections["MRI"], SIGNAL(LayerModified()), this,
          SLOT(RequestRedraw()));
  connect(m_layerCollections["Supplement"], SIGNAL(LayerModified()), this,
          SLOT(RequestRedraw()));

  connect(ui->widgetAllLayers, SIGNAL(ToReorderLayers(QList<Layer *>)), this,
          SLOT(ReorderLayers(QList<Layer *>)));
  for (int i = 0; i < 4; i++)
    connect(ui->widgetAllLayers, SIGNAL(CurrentLayerSelected(Layer *)),
            m_views[i], SLOT(SetScalarBarLayer(Layer *)));

  connect(m_layerCollections["MRI"], SIGNAL(LayerAdded(Layer *)), m_views[3],
          SLOT(OnLayerVisibilityChanged()));
  connect(m_layerCollections["MRI"], SIGNAL(LayerRemoved(Layer *)), m_views[3],
          SLOT(OnLayerVisibilityChanged()));
  connect(m_layerCollections["MRI"], SIGNAL(LayerVisibilityChanged()),
          m_views[3], SLOT(OnLayerVisibilityChanged()));
  connect(m_layerCollections["Surface"], SIGNAL(LayerAdded(Layer *)),
          m_views[3], SLOT(OnLayerVisibilityChanged()));
  connect(m_layerCollections["Surface"], SIGNAL(LayerRemoved(Layer *)),
          m_views[3], SLOT(OnLayerVisibilityChanged()));
  connect(m_layerCollections["Surface"], SIGNAL(LayerVisibilityChanged()),
          m_views[3], SLOT(OnLayerVisibilityChanged()));
  ...
  connect(m_layerCollections["MRI"], SIGNAL(LayerAdded(Layer *)),
          m_dlgThresholdVolume, SLOT(UpdateVolumes()));
  connect(m_layerCollections["MRI"], SIGNAL(LayerRemoved(Layer *)),
          m_dlgThresholdVolume, SLOT(UpdateVolumes()));
  ...
  connect(m_layerCollections["MRI"], SIGNAL(LayerAdded(Layer *)),
          m_dlgVolumeSegmentation, SLOT(UpdateVolumes()));
  connect(m_layerCollections["MRI"], SIGNAL(LayerRemoved(Layer *)),
          m_dlgVolumeSegmentation, SLOT(UpdateVolumes()));
  ...
  connect(m_dlgWriteMovieFrames, SIGNAL(Started()), m_statusBar,
          SLOT(ShowProgress()));
  connect(m_dlgWriteMovieFrames, SIGNAL(Stopped()), m_statusBar,
          SLOT(HideProgress()));
  connect(m_dlgWriteMovieFrames, SIGNAL(Progress(int)), m_statusBar,
          SLOT(SetProgress(int)));

  m_term = new TermWidget(this);
  m_term->hide();
  connect(ui->actionShowCommandConsole, SIGNAL(toggled(bool)), m_term,
          SLOT(setVisible(bool)));
  connect(ui->actionRunCommand, SIGNAL(triggered()), m_term, SLOT(show()));

  m_dlgMessage =
      new QMessageBox(QMessageBox::Warning, "Warning", "", QMessageBox::Ok,
  ...
  connect(ui->view3D, SIGNAL(SurfaceVertexClicked(LayerSurface *)), this,
          SLOT(OnSurfaceVertexClicked(LayerSurface *)));
  ...
  connect(this, SIGNAL(SurfaceRepositionIntensityChanged()),
          m_dlgRepositionSurface, SLOT(UpdateIntensity()),
          Qt::QueuedConnection);
  ...
  connect(this, SIGNAL(SlicePositionChanged()), m_wndTimeCourse,
          SLOT(UpdateData()));
  connect(ui->actionTimeCourse, SIGNAL(toggled(bool)), m_wndTimeCourse,
          SLOT(setVisible(bool)));
  connect(m_layerCollections["MRI"], SIGNAL(LayerAdded(Layer *)),
          m_wndTimeCourse, SLOT(UpdateUI()));
  connect(m_layerCollections["MRI"], SIGNAL(LayerRemoved(Layer *)),
          m_wndTimeCourse, SLOT(UpdateUI()));
  connect(m_layerCollections["MRI"], SIGNAL(ActiveLayerChanged(Layer *)),
          m_wndTimeCourse, SLOT(UpdateUI()));
  connect(m_wndTimeCourse, SIGNAL(OverlayFrameChanged(int)),
          ui->widgetAllLayers->GetPanel("Surface"), SLOT(SetOverlayFrame(int)));
  ...
  connect(this, SIGNAL(SlicePositionChanged()), m_dlgLabelStats,
          SLOT(UpdateStats()), Qt::QueuedConnection);
  connect(m_layerCollections["MRI"], SIGNAL(ActiveLayerChanged(Layer *)),
          m_dlgLabelStats, SLOT(UpdateStats()), Qt::QueuedConnection);
  connect(m_layerCollections["ROI"], SIGNAL(ActiveLayerChanged(Layer *)),
          m_dlgLabelStats, SLOT(UpdateStats()), Qt::QueuedConnection);

  m_dlgLineProfile = new DialogLineProfile(this);
  m_dlgLineProfile->hide();
  connect(m_layerCollections["PointSet"], SIGNAL(LayerAdded(Layer *)),
          m_dlgLineProfile, SLOT(UpdatePointSetList()));
  connect(m_layerCollections["PointSet"], SIGNAL(LayerRemoved(Layer *)),
          m_dlgLineProfile, SLOT(UpdatePointSetList()));
  connect(m_layerCollections["PointSet"], SIGNAL(LayerNameChanged()),
          m_dlgLineProfile, SLOT(UpdatePointSetList()));
  for (int i = 0; i < 3; i++) {
    connect(
        this->m_views[i], SIGNAL(LineProfileIdPicked(LayerLineProfile *, int)),
  ...
  connect(m_wndTractCluster, SIGNAL(TreeDataLoaded(QVariantMap)),
          SLOT(OnTractClusterLoaded(QVariantMap)));
  ...
  connect(m_views[3], SIGNAL(MouseIn()), ui->treeWidgetMouseInfo,
          SLOT(ShowHeaderText()));
  connect(m_views[3], SIGNAL(MouseOut()), ui->treeWidgetMouseInfo,
          SLOT(ClearHeaderText()));

  for (int i = 0; i < 4; i++) {
    connect(this, SIGNAL(SlicePositionChanged(bool)), m_views[i],
            SLOT(OnSlicePositionChanged(bool)));
  }
  ...
  connect(m_layerCollections["MRI"], SIGNAL(LayerTransformed()), m_views[3],
          SLOT(UpdateBounds()));
  connect(m_layerCollections["Surface"], SIGNAL(ActiveLayerChanged(Layer *)),
          m_views[3], SLOT(UpdateAxesActor()));
  connect(m_layerCollections["MRI"], SIGNAL(ActiveLayerChanged(Layer *)), this,
          SLOT(UpdateLayerInfo(Layer *)), Qt::QueuedConnection);
  connect(m_layerCollections["Surface"], SIGNAL(ActiveLayerChanged(Layer *)),
  ...
  for (int i = 0; i < MAX_RECENT_FILES; i++) {
    ...
    connect(act, SIGNAL(triggered()), this, SLOT(OnRecentVolumeFile()));
    m_actionRecentVolumes << act;
    act = new QAction(this);
    act->setVisible(false);
    connect(act, SIGNAL(triggered()), this, SLOT(OnRecentSurfaceFile()));
  ...
  connect(actionGroupView, SIGNAL(triggered(QAction *)), this,
          SLOT(OnSetMainView(QAction *)), Qt::QueuedConnection);
  connect(actionGroupLayout, SIGNAL(triggered(QAction *)), this,
          SLOT(OnSetViewLayout(QAction *)), Qt::QueuedConnection);

  connect(ui->actionQuickReference, SIGNAL(triggered()), this->m_wndQuickRef,
          SLOT(show()));
  connect(ui->actionShowSlices, SIGNAL(toggled(bool)), this->ui->view3D,
          SLOT(SetShowAllSlices(bool)));
  connect(ui->view3D, SIGNAL(VolumeTrackMouseOver(Layer *, QVariantMap)),
          ui->treeWidgetMouseInfo,
          SLOT(UpdateTrackVolumeAnnotation(Layer *, QVariantMap)));

  m_threadIOWorker = new ThreadIOWorker(this);
  connect(m_threadIOWorker, SIGNAL(Error(Layer *, int)), this,
          SLOT(OnIOError(Layer *, int)), Qt::QueuedConnection);
  connect(m_threadIOWorker, SIGNAL(Finished(Layer *, int)), this,
          SLOT(OnIOFinished(Layer *, int)), Qt::QueuedConnection);
  connect(m_threadIOWorker, SIGNAL(FCDLoadFinished(LayerFCD *)), this,
          SLOT(OnFCDLoadFinished(LayerFCD *)), Qt::QueuedConnection);
  connect(m_threadIOWorker, SIGNAL(started()), this, SLOT(SetProcessing()));

  connect(m_threadIOWorker, SIGNAL(Progress(int)), m_statusBar,
          SLOT(SetProgress(int)));
  connect(m_threadIOWorker, SIGNAL(started()), m_statusBar,
          SLOT(ShowProgress()));
  connect(m_threadIOWorker, SIGNAL(finished()), m_statusBar,
          SLOT(HideProgress()));

  connect(this, SIGNAL(SlicePositionChanged()), ui->treeWidgetCursorInfo,
          SLOT(OnCursorPositionChanged()), Qt::QueuedConnection);
  connect(ui->treeWidgetCursorInfo,
          SIGNAL(RASChangeTriggered(double, double, double)), this,
          SLOT(SetSlicePosition(double, double, double)));
  connect(m_layerCollections["MRI"], SIGNAL(MouseRASPositionChanged()),
          ui->treeWidgetMouseInfo, SLOT(OnMousePositionChanged()),
          Qt::QueuedConnection);
  connect(ui->treeWidgetMouseInfo,
          SIGNAL(RASChangeTriggered(double, double, double)),
          m_layerCollections["MRI"],
          SLOT(SetMouseRASPosition(double, double, double)));

  m_threadVolumeFilter = new VolumeFilterWorkerThread(this);
  connect(m_threadVolumeFilter, SIGNAL(Finished(VolumeFilter *)), this,
          SLOT(OnVolumeFilterFinished(VolumeFilter *)));
  connect(m_threadVolumeFilter, SIGNAL(Progress(int)), m_statusBar,
          SLOT(SetProgress(int)));
  connect(m_threadVolumeFilter, SIGNAL(started()), m_statusBar,
          SLOT(ShowProgress()));
  connect(m_threadVolumeFilter, SIGNAL(finished()), m_statusBar,
          SLOT(HideProgress()));

  connect(m_layerCollections["Surface"], SIGNAL(LayerAdded(Layer *)),
          SLOT(UpdateSurfaceContralateralInfo()), Qt::QueuedConnection);
  connect(m_layerCollections["Surface"], SIGNAL(LayerRemoved(Layer *)),
          SLOT(UpdateSurfaceContralateralInfo()), Qt::QueuedConnection);
  connect(m_layerCollections["HiddenSurface"], SIGNAL(LayerAdded(Layer *)),
          SLOT(UpdateSurfaceContralateralInfo()), Qt::QueuedConnection);
  connect(m_layerCollections["HiddenSurface"], SIGNAL(LayerRemoved(Layer *)),
          SLOT(UpdateSurfaceContralateralInfo()), Qt::QueuedConnection);
  ...
  connect(ui->actionNextLabelPoint, SIGNAL(triggered()),
          ui->widgetAllLayers->GetPanel("MRI"), SLOT(OnGoToNextPoint()));
...
void MainWindow::SetProgress(int n) { m_statusBar->SetProgress(n); }

void MainWindow::LoadSettings() {
  QSettings settings;
  restoreGeometry(settings.value("MainWindow/Geometry").toByteArray());
  restoreState(settings.value("MainWindow/WindowState").toByteArray());
  ui->splitterMain->restoreState(
      settings.value("MainWindow/SplitterState").toByteArray());
  ui->splitterInfoPanel->restoreState(
      settings.value("InfoPanel/SplitterState").toByteArray());
  ...
  m_settingsScreenshot.Magnification =
      settings.value("ScreenShot/Magnification", 1).toInt();
  m_settingsScreenshot.AntiAliasing =
      settings.value("ScreenShot/AntiAliasing", false).toBool();
  m_settingsScreenshot.HideCoords =
      settings.value("ScreenShot/HideAnnotation", true).toBool();
  m_settingsScreenshot.HideScaleBar =
      settings.value("ScreenShot/HideScaleBar", true).toBool();
  m_settingsScreenshot.HideCursor =
      settings.value("ScreenShot/HideCursor", true).toBool();
  m_settingsScreenshot.AutoTrim =
      settings.value("ScreenShot/AutoTrim", false).toBool();
...
void MainWindow::SaveSettings() {
  ...
  settings.setValue("InfoPanel/SplitterState",
                    ui->splitterInfoPanel->saveState());
...
void MainWindow::closeEvent(QCloseEvent *event) {
  if (this->IsBusy()) {
    if (QMessageBox::question(this, "Warning",
                              "There is on-going data processing. If you force "
                              "quit, any data that is being saved can be lost. "
                              "Do you really want to force quit?",
                              QMessageBox::Yes,
  ...
  for (int i = 0; i < keys.size(); i++) {
    for (int j = 0; j < m_layerCollections[keys[i]]->GetNumberOfLayers(); j++) {
      LayerEditable *layer = qobject_cast<LayerEditable *>(
          m_layerCollections[keys[i]]->GetLayer(j));
...
bool MainWindow::ParseCommand(const QString &cmd, bool bAutoQuit) {
  ...
  return (m_cmdParser->Parse(strg) && DoParseCommand(m_cmdParser, bAutoQuit));
}

bool MainWindow::ParseCommand(MyCmdLineParser *parser, const QString &cmd,
                              bool bAutoQuit) {
  QString strg = cmd;
  strg.replace("~", QDir::homePath());
  return (parser->Parse(strg) && DoParseCommand(parser, bAutoQuit));
}
...
bool MainWindow::DoParseCommand(MyCmdLineParser *parser, bool bAutoQuit) {
  ...
  if (parser->Found("ras", &sa)) {
    ...
    for (int i = 0; i < 3 && bOK; i++)
      sa[i].toDouble(&bOK);
    if (!bOK) {
      std::cerr << "Invalid argument for 'ras'. Arguments must be valid float "
                   "values.\n";
      return false;
    }
  ...
  if (parser->Found("slice", &sa)) {
    ...
    for (int i = 0; i < 3 && bOK; i++)
      sa[i].toInt(&bOK);
    if (!bOK) {
      std::cerr << "Invalid argument for 'slice'. Arguments must be valid "
                   "integers.\n";
      return false;
    }
  ...
  if (parser->Found("write-slice-intersection", &sa)) {
    ...
    for (int i = start; i <= end; i++) {
      //    slice[n] = i;
      //    this->AddScript(QStringList("slice") << QString::number(slice[0]) <<
      //    QString::number(slice[1]) << QString::number(slice[2]));
      this->AddScript(QStringList("writesurfaceintersection")
                      << sa[0] << sa[1].replace("%d", "%1").arg(i)
                      << QString::number(i));
    }
  }
  ...
  if (parser->Found("zoom", &sa)) {
    ...
    if (!bOK || dValue == 0) {
      std::cerr << "Invalid argument for 'zoom'. Argument must be a valid "
                   "float value.\n";
      return false;
    }
...
void MainWindow::SyncZoom(bool bSync) {
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      if (i != j) {
        disconnect(m_views[i], SIGNAL(Zooming(RenderView2D *)), m_views[j],
                   SLOT(SyncZoomTo(RenderView2D *)));
        if (bSync)
          connect(m_views[i], SIGNAL(Zooming(RenderView2D *)), m_views[j],
                  SLOT(SyncZoomTo(RenderView2D *)), Qt::UniqueConnection);
      }
    }
  }
}
...
void MainWindow::AddScript(const QStringList &command) { m_scripts << command; }
...
void MainWindow::OnIdle() {
  ...
  if (!bBusy && m_scripts.isEmpty() && property("from_cmd").toBool()) {
    ...
    std::cout << "Commands finished" << std::endl;
  }
  ...
  if (nMode == RenderView::IM_ROIEdit ||
      (nMode == RenderView::IM_Navigate && type == "ROI")) {
  ...
  } else if (nMode == RenderView::IM_VoxelEdit ||
             nMode == RenderView::IM_ReconEdit ||
             (nMode == RenderView::IM_Navigate && type == "MRI")) {
    ...
    ui->actionRedo->setEnabled(mri && mri->IsVisible() && mri->HasRedo());
             ...
             (nMode == RenderView::IM_Navigate && type == "PointSet")) {
  ...
  foreach (Layer *layer, volumes) {
    if (((LayerMRI *)layer)->GetProperty()->GetColorMap() ==
  ...
  ui->actionPointSetEdit->setEnabled(layerPointSet &&
                                     layerPointSet->IsEditable());
  ...
  ui->actionCreateOptimalCombinedVolume->setEnabled(
      GetLayerCollection("MRI")->GetNumberOfLayers() > 1);
  //  ui->actionCycleLayer      ->setEnabled( lc && lc->GetNumberOfLayers() > 1
  //  ); ui->actionReverseCycleLayer      ->setEnabled( lc &&
  //  lc->GetNumberOfLayers() > 1 ); ui->actionHideAllLayers   ->setEnabled( lc
  //  && !lc->IsEmpty() ); ui->actionShowAllLayers   ->setEnabled( lc &&
  //  !lc->IsEmpty() );
  ...
  ui->actionResetViewNearestAxis->setEnabled(bHasLayer &&
                                             ui->view3D->isVisible());
  ...
  ui->actionSaveVolume->setEnabled(!bBusy && layerVolume &&
                                   layerVolume->IsModified());
  ...
  ui->actionSavePointSet->setEnabled(layerPointSet &&
                                     layerPointSet->IsModified());
  ui->actionSavePointSetAs->setEnabled(layerPointSet);
  ui->actionSaveSurface->setEnabled(!bBusy && layerSurface &&
                                    layerSurface->IsModified());
  ...
  ui->actionShowSliceFrames->setEnabled(bHasLayer &&
                                        ui->view3D->GetShowSlices());
  ...
  ui->actionShowLabelStats->setEnabled((layerVolume && bHasLabelLayer) ||
                                       layerROI);
  ...
  ui->actionVolumeFilterClose->setEnabled(!bBusy && layerVolume &&
                                          layerVolume->IsEditable());
  ui->actionVolumeFilterBoundary->setEnabled(!bBusy && layerVolume &&
                                             layerVolume->IsEditable());
  ...
  ui->actionShowCoordinateAnnotation->setChecked(
      ui->viewAxial->GetShowCoordinateAnnotation());
  ui->actionShowColorScale->setChecked(view->GetShowScalarBar());

  ui->actionToggleCursorVisibility->setChecked(
      ui->viewAxial->GetCursor2D()->IsShown());
  ...
  } else if (m_dlgCropVolume->isVisible() &&
             nMode != RenderView::IM_VolumeCrop) {
    m_dlgCropVolume->hide();
  }

  ui->actionShowCommandConsole->setChecked(m_term->isVisible());
  ui->actionTimeCourse->setEnabled(
      (layerVolume && layerVolume->GetNumberOfFrames() > 1 &&
       !layerVolume->GetCorrelationSurface()) ||
      (layerSurface && layerSurface->GetActiveOverlay() &&
       layerSurface->GetActiveOverlay()->GetNumberOfFrames() > 1));
...
bool MainWindow::IsBusy() {
  return m_threadIOWorker->isRunning() || m_bProcessing ||
         m_threadVolumeFilter->isRunning();
}
...
void MainWindow::RunScript() {
  ...
  if (cmd == "loadcommand") {
    ...
    LayerSurface *surf =
        qobject_cast<LayerSurface *>(GetActiveLayer("Surface"));
    if (surf) {
      bool bOk;
    ...
    std::cerr << "Command '" << qPrintable(cmd) << "' was not recognized.\n";
  }
...
void MainWindow::CommandLoadCommand(const QStringList &sa) {
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    std::cerr << "Can not open for read: " << qPrintable(sa[1]) << ".\n";
    return;
  }
  QStringList lines =
      QString(file.readAll()).trimmed().split("\n", QString::SkipEmptyParts);
  foreach (QString line, lines) {
    QStringList args =
        line.trimmed().split(QRegExp("\\s+"), QString::SkipEmptyParts);
    if (args.size() > 0 &&
  ...
  std::cout << "Executing commands from " << qPrintable(sa[1]) << std::endl;
...
void MainWindow::CommandLoadSubject(const QStringList &sa) {
  ...
  QString subject_path =
      QProcessEnvironment::systemEnvironment().value("SUBJECTS_DIR");
  if (subject_path.isEmpty()) {
    std::cerr << "SUBJECTS_DIR is not set. Can not load subject.\n";
    return;
  }
  subject_path += "/" + sa[1];
  QString args =
...
void MainWindow::CommandLoadVolume(const QStringList &sa) {
  ...
  QString     lut_name;
  QString     vector_display = "no", vector_inversion = "none",
          vector_render = "line", tensor_display = "no",
          tensor_render = "boxoid", vector_width = "1", vector_norm_th = "0";
  ...
  for (int i = 1; i < sa_vol.size(); i++) {
    ...
    if (n != -1) {
      ...
      if (subOption == "colormap") {
      ...
      } else if (subOption == "grayscale" || subOption == "heatscale" ||
                 subOption == "colorscale") {
        colormap_scale =
            subOption; // colormap scale might be different from colormap!
        ...
        if (vector_render.isEmpty()) {
          std::cerr << "Missing render argument.\n";
          vector_render = "line";
          tensor_render = "boxoid";
        }
      } else if (subOption == "reg" || subOption == "transform") {
        reg_fn = subArgu;
      } else if (subOption == "sample" || subOption == "resample" ||
                 subOption == "interpolation") {
        ...
        if (color.isValid())
          ...
          std::cerr << "Unrecognized color input for :binary_color.\n";
      } else if (!subOption.isEmpty()) {
        std::cerr << "Unrecognized sub-option flag '"
                  << strg.toLatin1().constData() << "'.\n";
        return;
      }
    } else {
      std::cerr << "Unrecognized sub-option flag '"
  ...
  if (scales.size() > 0 ||
      (colormap != "grayscale" && colormap != "greyscale")) {
  ...
  if (!tensor_display.isEmpty() && tensor_display != "no") {
    QStringList script = QStringList("setdisplaytensor")
                         << tensor_display << tensor_render << vector_inversion
                         << vector_width;
    m_scripts.insert(0, script);
  } else if (!vector_display.isEmpty() && vector_display != "no") {
    QStringList script = QStringList("setdisplayvector")
                         << vector_display << vector_render << vector_inversion
                         << vector_width << vector_norm_th << "new";
    m_scripts.insert(0, script);
  }
...
void MainWindow::CommandSetColorMap(const QStringList &sa) {
  ...
  if (strg == "heat" || strg == "heatscale") {
    ...
    std::cerr << "Unrecognized colormap name '" << strg.toLatin1().constData()
              << "'.\n";
  }
  ...
  if (sa.size() > 2) {
    ...
    for (int i = nStart; i < sa.size(); i++) {
      ...
      if (!bOK) {
        std::cerr << "Invalid color scale value(s). \n";
        break;
      } else {
        pars << dValue;
      }
...
void MainWindow::CommandSetDisplayVector(const QStringList &cmd) {
  if (cmd[1].toLower() == "yes" || cmd[1].toLower() == "true" ||
      cmd[1] == "1") {
    LayerMRI *mri = (LayerMRI *)GetLayerCollection("MRI")->GetActiveLayer();
    if (mri) {
      if (!mri->IsTypeOf("DTI") && mri->GetNumberOfFrames() < 3) {
        std::cerr
            << "Volume has less than 3 frames. Can not display as vectors.\n";
      } else {
        ...
        if (cmd[3].toLower() != "none") {
          if (cmd[3].toLower() == "x") {
            ...
            std::cerr << "Unknown inversion flag '"
                      << cmd[2].toLatin1().constData() << "'.\n";
          }
        }
        ...
        if (ok) {
          ...
          std::cerr << "Unknown vector width value '"
                    << cmd[4].toLatin1().constData() << "'.\n";
        }
        ...
        if (ok) {
          ...
          std::cerr << "Unknown vector norm threshold value '"
                    << cmd[5].toLatin1().constData() << "'.\n";
        }
        ...
        if (val == 1 && cmd.size() > 6) {
          ...
          foreach (Layer *layer, list) {
            ...
            if (mlayer != mri && mlayer->GetProperty()->GetDisplayVector()) {
              mri->GetProperty()->SetVectorDisplayScale(
                  mlayer->GetProperty()->GetVectorDisplayScale());
              mri->GetProperty()->SetVectorLineWidth(
                  mlayer->GetProperty()->GetVectorLineWidth());
              mri->GetProperty()->SetNormalizeVector(
                  mlayer->GetProperty()->GetNormalizeVector());
              mri->GetProperty()->SetVectorRepresentation(
                  mlayer->GetProperty()->GetVectorRepresentation());
              break;
            }
          }
        }
  ...
  if (cmd[1].toLower() == "yes" || cmd[1].toLower() == "true" ||
    ...
    if (mri) {
      if (mri->GetNumberOfFrames() < 9) {
        std::cerr
            << "Volume has less than 9 frames. Can not display as tensor.\n";
      } else {
        mri->GetProperty()->SetDisplayTensor(true);

        if (cmd[2].toLower().indexOf("box") != -1) {
          mri->GetProperty()->SetVectorRepresentation(
              LayerPropertyMRI::TR_Boxoid);
        } else if (cmd[2].toLower().indexOf("ellips") != -1) {
          mri->GetProperty()->SetVectorRepresentation(
              LayerPropertyMRI::TR_Ellipsoid);
        } else {
          std::cerr << "Unrecognized argument '"
                    << cmd[2].toLatin1().constData()
                    << "' for tensor rendering.\n";
        }
...
void MainWindow::CommandSetOpacity(const QStringList &sa) {
  ...
  if (!bOK) {
    std::cerr << "Opacity value is not valid.\n";
    return;
  }
  if (sa.size() > 2) {
    ...
    if (type == "mri") {
      ...
      if (lc && lc->GetActiveLayer())
        ((LayerSurface *)lc->GetActiveLayer())
            ->GetProperty()
            ->SetOpacity(dValue);
    } else if (type == "roi") {
      ...
      if (lc && lc->GetActiveLayer())
        ((LayerPointSet *)lc->GetActiveLayer())
            ->GetProperty()
            ->SetOpacity(dValue);
    } else if (type == "tract") {
...
void MainWindow::CommandSetActiveFrame(const QStringList &sa) {
  ...
  if (mri) {
    ...
    if (bOK && val >= 0 && val < mri->GetNumberOfFrames()) {
      ...
      std::cerr << "Frame value is not valid.\n";
    }
  }
}
  ...
  if (mri) {
    ...
    if (sa.size() > 1) {
      ...
      if (bOK) {
        ...
        std::cerr << "Isosurface threshold value is not valid.\n";
      }
    }
    if (sa.size() > 2) {
...
void MainWindow::CommandSetIsoSurfaceColor(const QStringList &cmd) {
  ...
  if (mri) {
    ...
    if (color.isValid()) {
      mri->GetProperty()->SetContourColor(color.redF(), color.greenF(),
                                          color.blueF());
    } else {
      std::cerr << "Invalid color name or value "
                << cmd[1].toLatin1().constData() << ".\n";
    }
  }
}
...
void MainWindow::CommandSetIsoSurfaceUpsample(const QStringList &cmd) {
  LayerMRI *mri = (LayerMRI *)GetLayerCollection("MRI")->GetActiveLayer();
  if (mri) {
    if (cmd[1].toLower() == "on" || cmd[1].toLower() == "true" ||
        cmd[1].toLower() == "1") {
  ...
  if (mri) {
    if (sa.size() > 1) {
      if (!mri->LoadSurfaceRegions(sa[1])) {
        std::cerr << "Can not load surfacer region(s) from "
                  << sa[1].toLatin1().constData() << ".\n";
      }
    }
  }
}
    ...
    for (int i = 1; i < sa_vol.size(); i++) {
      ...
      if (n != -1) {
        if (strg.left(n).toLower() == "vector") {
          ...
          if (vector_inversion.isEmpty()) {
            std::cerr << "Missing vector inversion argument.\n";
            vector_inversion = "none";
          }
          ...
          {
            if (vector_render.isEmpty()) {
              std::cerr << "Missing vector render argument.\n";
              vector_render = "line";
            }
          }
...
void MainWindow::CommandLoadPVolumes(const QStringList &cmd) {
  ...
  if (cmd.size() > 3) {
    ...
    if (!ct) {
      std::cerr << "Can not load look up table " << lut.toLatin1().constData()
                << ".\n";
      return;
    }
  }
...
void MainWindow::LoadConnectomeMatrixFile(const QString &fn_cmat,
                                          const QString &fn_parcel,
                                          const QString &fn_ctab) {
...
void MainWindow::OnCloseConnectomeMatrix() {
  LayerConnectomeMatrix *layer =
      (LayerConnectomeMatrix *)GetActiveLayer("CMAT");
  if (!layer) {
    return;
  }
...
void MainWindow::CommandLoadSurface(const QStringList &cmd) {
  QStringList rawoverlay_list =
      cmd[1].split("overlay=", QString::SkipEmptyParts, Qt::CaseInsensitive);
  QStringList overlay_list;
  for (int i = 1; i < rawoverlay_list.size(); i++) {
    QStringList sublist = rawoverlay_list[i].split(
        "correlation=", QString::SkipEmptyParts, Qt::CaseInsensitive);
  ...
  valid_overlay_options << "overlay_reg"
                        << "overlay_method"
                        << "overlay_threshold"
                        << "overlay_color"
                        << "overlay_rh"
  ...
  for (int nOverlay = 0; nOverlay < overlay_list.size(); nOverlay++) {
    ...
    if (nOverlay ==
        0) // first one is not overlay file but actually surface file
      surface_fn = sa_fn[0];
    bool bLoadAll = false;
    ...
    for (int k = sa_fn.size() - 1; k >= 0; k--) {
      ...
      if (n != -1) {
        ...
        else if (subOption == "overlay_rh" &&
                 (subArgu == "1" || subArgu == "true"))
      ...
      if (n != -1) {
        ...
        if (subOption == "color") {
          ...
          m_scripts.insert(0, QStringList("setsurfaceedgecolor") << subArgu);
                   ...
                   subOption == "edge_thickness") {
          m_scripts.insert(0, QStringList("setsurfaceedgethickness")
                                  << subArgu);
        ...
        } else if (subOption == "curvature_method" ||
                   subOption == "curvature_map" ||
                   subOption == "curvature_setting") {
          m_scripts.insert(0, QStringList("setsurfacecurvaturemap") << subArgu);
        } else if (subOption == "overlay" || subOption == "correlation" ||
                   subOption == "mrisp" ||
                   subOption == "parameterization_overlay") {
          ...
          if (overlay_method != "linearopaque" ||
              !overlay_thresholds.isEmpty()) {
            script = QStringList("setsurfaceoverlaymethod")
                     << overlay_method << overlay_thresholds;
            // insert right AFTER loadsurfaceoverlay command
            m_scripts.insert(1, script);
          }

          if (!overlay_custom.isEmpty())
            m_scripts.insert(1, QStringList("setsurfaceoverlaycustom")
                                    << overlay_custom);

          if (!overlay_opacity.isEmpty())
            m_scripts.insert(1, QStringList("setsurfaceoverlayopacity")
                                    << overlay_opacity);

          if (!overlay_color.isEmpty())
            m_scripts.insert(1, QStringList("setsurfaceoverlaycolormap")
                                    << overlay_color);

          if (!overlay_frame.isEmpty())
            m_scripts.insert(1, QStringList("setsurfaceoverlayframe")
                                    << overlay_frame);

          if (!overlay_smooth_steps.isEmpty())
            m_scripts.insert(1, QStringList("setsurfaceoverlaysmooth")
                                    << overlay_smooth_steps);

          if (!overlay_mask.isEmpty())
            m_scripts.insert(1, QStringList("setsurfaceoverlaymask")
                                    << overlay_mask);
        } else if (subOption == "mrisps") {
          m_scripts.insert(0, QStringList("loadsurfaceparameterization")
                                  << subArgu);
          ...
          for (int i = annot_fns.size() - 1; i >= 0; i--) {
            m_scripts.insert(0, QStringList("loadsurfaceannotation")
                                    << annot_fns[i]);
          }
        } else if (subOption == "annot_outline" ||
                   subOption == "annotation_outline" ||
                   subOption == "aparc_outline") {
          if (subArgu.toLower() == "true" || subArgu.toLower() == "yes" ||
              subArgu == "1") {
            for (int i = 0; i < m_scripts.size(); i++) {
              if (m_scripts[i][0] == "loadsurfaceannotation") {
                m_scripts.insert(
                    i + 1, QStringList("setsurfaceannotationoutline") << "1");
                break;
              }
            }
          }
        ...
        } else if (subOption == "label_outline" ||
                   subOption == "labeloutline") {
          if (subArgu.toLower() == "true" || subArgu.toLower() == "yes" ||
              subArgu == "1") {
            for (int i = 0; i < m_scripts.size(); i++) {
              if (m_scripts[i][0] == "loadsurfacelabel") {
                m_scripts.insert(i + 1, QStringList("setsurfacelabeloutline")
                                            << "1");
                break;
              }
            }
          }
        } else if (subOption == "label_opacity" ||
                   subOption == "labelopacity") {
          if (!subArgu.isEmpty()) {
            for (int i = 0; i < m_scripts.size(); i++) {
              if (m_scripts[i][0] == "loadsurfacelabel") {
                m_scripts.insert(i + 1, QStringList("setsurfacelabelopacity")
                                            << subArgu);
                break;
              }
            }
        ...
        } else if (subOption == "label_centroid" ||
                   subOption == "labelcentroid") {
          if (!subArgu.isEmpty()) {
            for (int i = 0; i < m_scripts.size(); i++) {
              if (m_scripts[i][0] == "loadsurfacelabel") {
        ...
        } else if (subOption == "annot_zorder" ||
                   subOption == "annotation_zorder") {
          ...
          for (int i = vector_fns.size() - 1; i >= 0; i--) {
            m_scripts.insert(0, QStringList("loadsurfacevector")
                                    << vector_fns[i]);
          }
          ...
          m_scripts.insert(0, QStringList("setlayername")
                                  << "Surface" << subArgu);
        } else if (subOption == "lock" || subOption == "locked") {
          ...
          m_scripts.insert(0, QStringList("setsurfaceoffset")
                                  << subArgu.split(","));
        ...
        } else if (subOption == "ignore_vg" ||
                   subOption == "ignore_volume_geometry") {
          if (subArgu.toLower() == "true" || subArgu.toLower() == "yes" ||
              subArgu == "1") {
  ...
  if (surf) {
    ...
    if (overlay) {
      ...
      if (ok) {
        ...
        std::cerr << "Invalid input for overlay opacity.\n";
      }
    }
  }
}
  ...
  if (surf) {
    ...
    if (overlay) {
      ...
      if (ok) {
        ...
        std::cerr << "Invalid input for overlay frame.\n";
      }
    }
  }
}
  ...
  if (surf) {
    ...
    if (overlay) {
      ...
      if (ok && steps > 0) {
        ...
        if (!ok)
          std::cerr << "Invalid input for overlay smoothing.\n";
      }
    }
  }
}
  ...
  if (surf) {
    ...
    if (overlay) {
      ...
      if (cmd[1] == "linear") {
        ...
        std::cerr << "Unrecognized overlay method name '"
                  << cmd[1].toLatin1().constData() << "'.\n";
        return;
      }
      ...
      if (bOK) {
        ...
        std::cerr << "Invalid input for overlay threshold.\n";
      }
...
void MainWindow::CommandSetSurfaceOverlayCustom(const QStringList &cmd_in) {
  ...
  if (surf) {
    ...
    if (overlay) {
      if (cmd.size() < 2) {
        std::cerr << "Insufficient overlay_custom argments\n";
        return;
      }
      ...
      if (!bOK || !c.isValid()) {
        std::cerr << "Invalid input for customized overlay color.\n";
        return;
      }
...
void MainWindow::CommandSetSurfaceOverlayColormap(const QStringList &cmd) {
  ...
  if (surf) {
    ...
    if (overlay) {
      for (int i = 1; i < cmd.size(); i++) {
        if (cmd[i] == "colorwheel")
          overlay->GetProperty()->SetColorScale(
              SurfaceOverlayProperty::CS_ColorWheel);
...
void MainWindow::CommandSetSurfaceColor(const QStringList &cmd) {
  ...
  if (surf && cmd[1] != "null") {
    QColor color = ParseColorInput(cmd[1]);
    if (color.isValid()) {
      surf->GetProperty()->SetBinaryColor(color.redF(), color.greenF(),
                                          color.blueF());
    } else {
      std::cerr << "Invalid color name or value "
                << cmd[1].toLatin1().constData() << ".\n";
...
void MainWindow::CommandSetSurfaceEdgeColor(const QStringList &cmd) {
  ...
  if (surf && cmd[1] != "null") {
    if (cmd[1].toLower() == "off" || cmd[1].toLower() == "surface" ||
        cmd[1].toLower() == "overlay") {
      surf->GetProperty()->SetUseSurfaceColorOn2D(true);
    } else {
      QColor color = ParseColorInput(cmd[1]);
      if (color.isValid()) {
        surf->GetProperty()->SetEdgeColor(color.redF(), color.greenF(),
                                          color.blueF());
      } else {
        std::cerr << "Invalid color name or value "
                  << cmd[1].toLatin1().constData() << ".\n";
...
void MainWindow::CommandSetSurfaceEdgeThickness(const QStringList &cmd) {
  ...
  if (surf) {
    ...
    if (!bOK) {
      std::cerr << "Invalid edge thickness value. Must be a integer.\n";
    } else {
...
void MainWindow::CommandSetSurfaceOpacity(const QStringList &cmd) {
  ...
  if (surf) {
    ...
    if (!bOK || opacity < 0 || opacity > 1) {
      std::cerr << "Invalid opacity value. Must be between 0 and 1.\n";
    } else {
...
void MainWindow::CommandSetSurfaceVertexColor(const QStringList &cmd) {
  ...
  if (surf && cmd[1] != "null") {
    QColor color = ParseColorInput(cmd[1]);
    if (color.isValid()) {
      surf->GetProperty()->SetVertexColor(color.redF(), color.greenF(),
                                          color.blueF());
    } else {
      std::cerr << "Invalid color name or value "
                << cmd[1].toLatin1().constData() << ".\n";
...
void MainWindow::CommandSetSurfaceOffset(const QStringList &cmd) {
  ...
  if (surf) {
    ...
    double pos[3];
    if (cmd.size() < 4) {
      std::cerr << "Invalid surface offset inputs. Need 3 numbers.\n";
    } else {
...
void MainWindow::CommandGoToLabel(const QStringList &cmd) {
  ...
  if (mri) {
    ...
    if (nSlice >= 0) {
      ...
      std::cerr << "Did not find slice with most structure of "
                << qPrintable(cmd[1]);
    }
  }
}
...
void MainWindow::CommandLoadSurfaceOverlay(const QStringList &cmd_in) {
  ...
  if (cmd[3] == "mrisp")
    ...
    LoadSurfaceOverlayFile(cmd[1], reg_file,
                           cmd.size() > 3 && cmd[3] == "correlation",
                           cmd.size() > 4 && cmd[4] == "rh");
}
...
void MainWindow::CommandLoadSurfaceCoordsFromParameterization(
...
void MainWindow::CommandLoadWayPoints(const QStringList &cmd) {
  ...
  if (color != "null" || spline_color != "null") {
    m_scripts.insert(0, QStringList("setpointsetcolor")
                            << color << spline_color);
  }

  if (radius != "1" || spline_radius != "0") {
    m_scripts.insert(0, QStringList("setpointsetradius")
                            << radius << spline_radius);
  }
  if (!spline_heatmap.isEmpty()) {
    m_scripts.insert(0,
                     QStringList("setpointsetheatmap")
                         << spline_heatmap.split(",", QString::SkipEmptyParts));
  }
...
void MainWindow::CommandSetPointSetColor(const QStringList &cmd) {
  LayerPointSet *wp =
      (LayerPointSet *)GetLayerCollection("PointSet")->GetActiveLayer();
  if (wp) {
    if (cmd[1] != "null") {
      QColor color = ParseColorInput(cmd[1]);

      if (color.isValid()) {
        wp->GetProperty()->SetColor(color.redF(), color.greenF(),
                                    color.blueF());
      } else {
        std::cerr << "Invalid color name or value "
                  << cmd[1].toLatin1().constData() << ".\n";
    ...
    if (cmd.size() > 2 && cmd[2] != "null") {
      ...
      if (color.isValid()) {
        wp->GetProperty()->SetSplineColor(color.redF(), color.greenF(),
                                          color.blueF());
      } else {
        std::cerr << "Invalid color name or value "
                  << cmd[2].toLatin1().constData() << ".\n";
  ...
  if (wp) {
    if (!cmd[1].isEmpty()) {
      ...
      if (bOK && dvalue >= 0) {
        ...
        std::cerr << "Invalid way points radius.\n";
      }
    }
    ...
    if (cmd.size() > 2 && cmd[2] != "0") {
      ...
      if (bOK) {
        ...
        std::cerr << "Invalid spline radius.\n";
      }
    }
  }
}
  ...
  if (wp) {
    if (wp->GetProperty()->LoadScalarsFromFile(cmd[1])) {
      ...
      if (cmd.size() == 4) {
        ...
        wp->GetProperty()->SetHeatScaleMid(
            (cmd[2].toDouble() + cmd[3].toDouble()) / 2.0);
      } else if (cmd.size() > 4) {
        wp->GetProperty()->SetHeatScaleMid(cmd[3].toDouble());
        wp->GetProperty()->SetHeatScaleMax(cmd[4].toDouble());
      }
    } else {
      std::cerr << "Could not load scalar map from file " << qPrintable(cmd[1])
                << "\n";
    }
  }
}
...
void MainWindow::CommandScreenCapture(const QStringList &cmd) {
  ...
  if (!m_views[m_nMainView]->SaveScreenShot(cmd[1],
    ...
    std::cerr << "Failed to save screen shot to "
              << cmd[1].toLatin1().constData() << ".\n";
  }
}
...
void MainWindow::CommandFlyThrough(const QStringList &cmd) {
  ...
  if (GetMainViewId() > 2) {
    std::cerr
        << "Can not fly through. Please set main viewport to 2D slice view.\n";
    return;
  }
}
...
void MainWindow::CommandSetCamera(const QStringList &cmd) {
  if (cmd[1].toLower() == "load") {
    ...
    for (int i = 1; i < cmd.size(); i += 2) {
      ...
      if (!bOK) {
        std::cerr << "Invalid input value for " << cmd[i].toLatin1().constData()
                  << ".\n";
        return;
      } else {
...
void MainWindow::CommandSetRAS(const QStringList &cmd) {
  ...
  if (bOK) {
    ...
    std::cerr << "Invalid input values for RAS coordinates. \n";
  }
}
...
void MainWindow::CommandSetSlice(const QStringList &cmd) {
  ...
  if (!lc_mri->IsEmpty()) {
    LayerMRI *mri =
        (LayerMRI *)lc_mri->GetLayer(lc_mri->GetNumberOfLayers() - 1);
    int  x, y, z;
    bool bOK;
    ...
    if (bOK) {
      ...
      std::cerr << "Invalide slice number(s). \n";
    }
  } else {
    std::cerr << "No volume was loaded. Set slice failed.\n";
  }
}
...
void MainWindow::UpdateRecentFileActions() {
  QSettings   settings;
  QStringList files =
      settings.value("MainWindow/RecentVolumeFiles").toStringList();

  int numRecentFiles = qMin(files.size(), (int)MAX_RECENT_FILES);
  for (int i = 0; i < numRecentFiles; ++i) {
    QString text =
        tr("&%1 %2").arg(i + 1).arg(MyUtils::Win32PathProof(files[i]));
...
bool MainWindow::SetSlicePosition(int nPlane, double dPos, bool bRoundToGrid) {
  ...
  for (int i = 0; i < keys.size(); i++) {
    ...
    if (m_layerCollections[keys[i]]->SetSlicePosition(nPlane, dPos,
                                                      bRoundToGrid)) {
      bRet = true;
    }
...
bool MainWindow::OffsetSlicePosition(int nPlane, double dPosDiff,
                                     bool bRoundToGrid) {
  bool             bRet   = false;
  QStringList      keys   = m_layerCollections.keys();
  ...
  if (!lc_mri->IsEmpty()) {
    ...
    for (int i = 0; i < keys.size(); i++) {
      m_layerCollections[keys[i]]->blockSignals(true);
      if (m_layerCollections[keys[i]]->OffsetSlicePosition(nPlane, dPosDiff,
                                                           bRoundToGrid)) {
        bRet = true;
      }
      m_layerCollections[keys[i]]->blockSignals(false);
...
void MainWindow::OnLoadVolume() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    for (int i = 0; i < filenames.size(); i++) {
      ...
      if ((!bHasVolume && bHasSurface) || (!bHasVolume && dlg.IsToResample()) ||
          m_bResampleToRAS) {
        script << "r";
      }
...
void MainWindow::LoadVolumeFile(const QString &filename,
                                const QString &reg_filename, bool bResample_in,
                                int nSampleMethod, bool bConform,
  ...
  if (GetLayerCollection("MRI")->IsEmpty()) {
    if (!GetLayerCollection("Surface")->IsEmpty() ||
        !GetLayerCollection("Tract")->IsEmpty()) {
      bResample = true;
    }
  }
  ...
  foreach (Layer *layer, layers) {
    if (qobject_cast<LayerMRI *>(layer)->IsModified()) {
      if (QMessageBox::question(this, "Volume Not Saved",
                                "Volume has been modifed and not been saved. "
...
void MainWindow::OnNewVolume() {
  ...
  if (!col_mri->GetActiveLayer()) {
    QMessageBox::warning(
        this, "Error",
        "Can not create new volume without any visible template volume.");
    return;
  }
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (layer_template->IsTransformed()) {
      ...
      //                            "New volume is constructed on template that
      //                            has been rotated/transformed. It is
      //                            recommended that you save the transformed
      //                            volume and reload it before creating new
      //                            volumes.");
    }
    LayerMRI *layer_new = new LayerMRI(layer_template);

    if (!layer_new->Create(dlg.GetTemplate(), dlg.GetCopyVoxel(),
                           dlg.GetDataType(), dlg.GetVoxelDataOption())) {
...
void MainWindow::ConnectMRILayer(LayerMRI *mri) {
  for (int i = 0; i < 4; i++)
    connect(mri->GetProperty(), SIGNAL(ColorMapChanged()), m_views[i],
            SLOT(UpdateScalarBar()), Qt::UniqueConnection);
}
...
void MainWindow::OnSaveVolume() {
  ...
  if (!layer) {
    ...
    QMessageBox::warning(this, "Error",
                         "Current volume layer is not visible. Please turn it "
                         "on before saving.");
    return;
  }
  ...
  if (fn.isEmpty()) {
    ...
    fn = QFileDialog::getSaveFileName(
        this, "Save volume file", AutoSelectLastDir("mri"),
        "Volume files (*.mgz *.mgh *.nii *.nii.gz *.img *.mnc);;All files (*)");
  }

  if (!fn.isEmpty()) {
    QFileInfo fi(fn);
    if (fi.suffix() != "nii" && fi.suffix() != "img" && fi.suffix() != "mgz" &&
        fi.suffix() != "mgh" && !fi.completeSuffix().contains("nii.gz")) {
      fn += ".mgz";
    }
    layer->SetFileName(fn);
    m_scripts.append(QStringList("savelayer")
                     << QString::number(layer->GetID()));
  } else {
    m_scripts.clear();
  }
}
...
bool MainWindow::SaveVolumeAs() {
  ...
  if (layer_mri->IsTransformed()) {
    ...
    fn = QFileDialog::getSaveFileName(
        this, "Save volume", QFileInfo(layer_mri->GetFileName()).absolutePath(),
        "Volume files (*.mgz *.mgh *.nii *.nii.gz *.img *.mnc);;All files (*)");
  if (!fn.isEmpty()) {
...
void MainWindow::OnLoadDTI() {
  ...
  this->LoadDTIFile(dlg.GetVectorFileName(), dlg.GetFAFileName(),
                    dlg.GetEigenvalueFileName(), dlg.GetRegFileName(),
                    dlg.IsToResample());
}

void MainWindow::OnLoadTrackVolume() {
  QString fn = QFileDialog::getOpenFileName(
      this, "Open track volume", AutoSelectLastDir("mri"),
      "Volume files (*.mgz *.mgh *.nii *.nii.gz *.img *.mnc);;All files (*)");
  if (!fn.isEmpty())
    this->LoadVolumeTrackFile(fn, this->m_bResampleToRAS);
}

void MainWindow::LoadDTIFile(const QString &fn_vector, const QString &fn_fa,
                             const QString &fn_scale,
                             const QString &reg_filename, bool bResample) {
  m_bResampleToRAS = bResample;
...
void MainWindow::LoadPVolumeFiles(const QStringList &filenames,
                                  const QString &prefix, const QString &lut) {
...
void MainWindow::OnLoadROI() {
  QStringList filenames = QFileDialog::getOpenFileNames(
      this, "Select label file", AutoSelectLastDir("label"), "Label files (*)");
  for (int i = 0; i < filenames.size(); i++) {
...
void MainWindow::LoadROIFile(const QString &fn, const QString &ref_vol,
  ...
  LayerCollection *col_mri = GetLayerCollection("MRI");
  if (ref_vol.isEmpty()) {
    //   cout << "No template volume given, using current volume as template for
    //   ROI " << fn.toLatin1().constData() << ".\n";
    ref = (LayerMRI *)col_mri->GetActiveLayer();
  } else {
    for (int i = 0; i < col_mri->GetNumberOfLayers(); i++) {
      LayerMRI *mri = (LayerMRI *)col_mri->GetLayer(i);
      if (ref_vol == QString::number(mri->GetID()) ||
          ref_vol == mri->GetName() ||
    ...
    if (ref == NULL) {
      std::cerr << "Can not find given template volume: "
                << ref_vol.toLatin1().constData()
                << ". Using current volume as template for ROI "
...
void MainWindow::OnSaveROI() {
  ...
  if (!layer_roi) {
    ...
    QMessageBox::warning(
        this, "Error",
        "Current ROI layer is not visible. Please turn it on before saving.");
    return;
  }
  QString fn = layer_roi->GetFileName();
  if (fn.isEmpty()) {
    QString def_fn =
        AutoSelectLastDir("label") + "/" + layer_roi->GetName() + ".label";
...
void MainWindow::OnSaveROIAs() {
  ...
  QString def_fn =
      AutoSelectLastDir("label") + "/" + layer_roi->GetName() + ".label";
  QString fn = QFileDialog::getSaveFileName(this, "Select label file", def_fn,
                                            "Label files (*)");
  ...
  foreach (Layer *layer, layers) {
    if (qobject_cast<LayerROI *>(layer)->IsModified()) {
      if (QMessageBox::question(this, "ROI Not Saved",
                                "ROI has been modifed and not been saved. Do "
...
void MainWindow::LoadPointSetFile(const QString &fn, int type,
  ...
  if (wp->LoadFromFile(fn)) {
    ...
    delete wp;
    QMessageBox::warning(this, "Error",
                         QString("Can not load Way Points from ") + fn);
  }
}
...
void MainWindow::OnSavePointSet(bool bForce) {
  ...
  if (!layer) {
    ...
    QMessageBox::information(this, "Error",
                             "Current Way Points layer is not visible. Please "
                             "turn it on before saving.");
    return;
  }

  QString fn = layer->GetFileName();
  if (fn.isEmpty() ||
      (!bForce && layer->IsEnhanced() &&
       layer->GetProperty()->GetType() != LayerPropertyPointSet::Enhanced)) {
    if (layer->IsEnhanced())
      QMessageBox::information(
          this, "Save Point Set",
          "To save comments or other added information, it is recommended to "
          "save point set in enhanced format in Json.");
    OnSavePointSetAs();
  } else {
    layer->SetFileName(fn);
    layer->ResetModified();
    if (layer->Save()) {
      ...
      QMessageBox::warning(
          this, "Error",
          "Could not save point set. Check console for more information.");
      return;
    }
  }
}
  ...
  foreach (Layer *layer, layers) {
    if (qobject_cast<LayerPointSet *>(layer)->IsModified()) {
      if (QMessageBox::question(this, "Point Set Not Saved",
                                "Point set has been modifed and not been "
                                "saved. Do you still want to continue?",
                                QMessageBox::Yes,
                                QMessageBox::No) == QMessageBox::No) {
        return;
      }
...
void MainWindow::OnLoadTrack() {
  QStringList filenames =
      QFileDialog::getOpenFileNames(this, "Select tract file", m_strLastDir,
...
void MainWindow::OnLoadSurface() {
  ...
  QStringList filenames = QFileDialog::getOpenFileNames(
      this, "Select surface file", AutoSelectLastDir("surf"),
...
void MainWindow::LoadSurfaceFile(const QString &    filename,
                                 const QString &    fn_patch,
                                 const QString &    fn_target,
                                 const QStringList &sup_files_in,
                                 const QVariantMap &sup_options) {
  QFileInfo fi(filename);
  m_strLastDir        = fi.absolutePath();
  LayerSurface *layer = new LayerSurface(m_layerVolumeRef);
  connect(layer, SIGNAL(CurrentVertexChanged(int)), m_wndGroupPlot,
          SLOT(SetCurrentVertex(int)), Qt::UniqueConnection);
  connect(ui->treeWidgetCursorInfo, SIGNAL(VertexChangeTriggered(int)),
          m_wndGroupPlot, SLOT(SetCurrentVertex(int)), Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceOverlyDataUpdated()), ui->treeWidgetCursorInfo,
          SLOT(UpdateAll()), Qt::UniqueConnection);
  connect(layer, SIGNAL(ActiveSurfaceChanged(int)), ui->view3D,
          SLOT(OnLayerVisibilityChanged()), Qt::UniqueConnection);
  connect(this, SIGNAL(SlicePositionChanged(bool)), layer,
          SLOT(OnSlicePositionChanged3D()), Qt::UniqueConnection);
  connect(layer, SIGNAL(SurfaceOverlayAdded(SurfaceOverlay *)), m_wndTimeCourse,
          SLOT(UpdateUI()), Qt::UniqueConnection);
  connect(layer, SIGNAL(ActiveOverlayChanged(int)), m_wndTimeCourse,
          SLOT(UpdateAll()), Qt::UniqueConnection);
  ...
  if (sup_options.contains("ZOrderAnnotation"))
    layer->GetProperty()->SetZOrderAnnotation(
        sup_options["ZOrderAnnotation"].toInt());
  ...
  if (sup_options.contains("ZOrderOverlay"))
    layer->GetProperty()->SetZOrderOverlay(
...
void MainWindow::OnLoadPatch() {
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, "Select patch file", AutoSelectLastDir("surf"), "Patch files (*)",
      0, QFileDialog::DontConfirmOverwrite);
  if (!filename.isEmpty() && !surf->LoadPatch(filename)) {
    QMessageBox::warning(this, "Error",
                         QString("Could not load patch from %1").arg(filename));
  }
}
...
void MainWindow::OnSavePatchAs() {
  ...
  if (!layer_surf) {
    ...
    QMessageBox::warning(this, "Error",
                         "Current surface layer is not visible. Please turn it "
                         "on before saving.");
    return;
  }

  QString fn = QFileDialog::getSaveFileName(
      this, "Save patch as", layer_surf->GetFileName(), "Patch files (*)");
  if (!fn.isEmpty() && !layer_surf->WritePatch(fn)) {
    QMessageBox::warning(this, "Error",
                         QString("Could not save patch to %1").arg(fn));
  }
}
...
void MainWindow::OnIOError(Layer *layer, int jobtype) {
  ...
  if (jobtype != ThreadIOWorker::JT_LoadSurfaceOverlay) {
    ...
    if (jobtype != ThreadIOWorker::JT_SaveVolume &&
        jobtype != ThreadIOWorker::JT_SaveSurface)
      delete layer;
  } else {
    msg += "overlay.";
    if (!bQuit)
...
void MainWindow::OnIOFinished(Layer *layer, int jobtype) {
  ...
  if (jobtype == ThreadIOWorker::JT_LoadVolume && layer->IsTypeOf("MRI")) {
  ...
  } else if (jobtype == ThreadIOWorker::JT_SaveVolume &&
             layer->IsTypeOf("MRI")) {
    SetCurrentFile(layer->GetFileName(), 0);
  } else if (jobtype == ThreadIOWorker::JT_LoadSurface &&
             layer->IsTypeOf("Surface")) {
      ...
      if (lc_mri->IsEmpty()) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          if (worigin[i] >= mri_origin[i])
            ...
            worigin[i] =
                mri_origin[i] -
                ((int)((mri_origin[i] - worigin[i]) / vs[i] + 1)) * vs[i];
          if (upper <= mri_origin[i] + mri_size[i])
            wsize[i] = mri_origin[i] + mri_size[i] - worigin[i];
          else
            wsize[i] =
                mri_origin[i] + mri_size[i] +
                ((int)((upper - mri_origin[i] - mri_size[i]) / vs[i] + 1)) *
                    vs[i];
        }
    ...
    if (!sf->HasValidVolumeGeometry()) {
      ...
      //                      "Either this surface does not contain valid volume
      //                      geometry information, or freeview failed to read
      //                      the information. This surface may not align with
      //                      volumes and other surfaces.");
      std::cerr << "Did not find any volume info" << std::endl;
    }
  ...
  } else if (jobtype == ThreadIOWorker::JT_LoadSurfaceOverlay &&
             layer->IsTypeOf("Surface")) {
  ...
  } else if (jobtype == ThreadIOWorker::JT_LoadConnectome &&
             layer->IsTypeOf("CMAT")) {
  ...
  if (jobtype == ThreadIOWorker::JT_SaveVolume) {
    std::cout << qPrintable(qobject_cast<LayerMRI *>(layer)->GetFileName())
              << " saved successfully.\n";
  } else if (jobtype == ThreadIOWorker::JT_SaveSurface) {
    std::cout << qPrintable(qobject_cast<LayerSurface *>(layer)->GetFileName())
              << " saved successfully.\n";
...
void MainWindow::OnCycleLayer() {
  ...
  if (lc) {
    lc->CycleLayer(true, true); // ui->viewAxial->GetInteractionMode() ==
                                // RenderView2D::IM_ReconEdit);
  }
}
...
void MainWindow::OnReverseCycleLayer() {
  LayerCollection *lc = GetLayerCollection(GetCurrentLayerType());
  if (lc) {
    lc->CycleLayer(false, true); // ui->viewAxial->GetInteractionMode() ==
                                 // RenderView2D::IM_ReconEdit );
  }
}
...
void MainWindow::OnEditUndo() {
  ...
  } else if (ui->viewAxial->GetInteractionMode() ==
                 RenderView2D::IM_VoxelEdit ||
             ui->viewAxial->GetInteractionMode() ==
                 RenderView2D::IM_ReconEdit ||
...
QString MainWindow::AutoSelectLastDir(const QString &lastDir_in,
...
void MainWindow::LoadSurfaceOverlayFile(const QString &filename,
                                        const QString &reg_file,
                                        bool           bCorrelation,
...
void MainWindow::LoadSurfaceAnnotation() {
  QString filename = QFileDialog::getOpenFileName(
      this, "Select annotation file", AutoSelectLastDir("label"),
      "Annotation files (*)");
  if (!filename.isEmpty()) {
...
void MainWindow::LoadSurfaceAnnotationFile(const QString &filename) {
  ...
  if (layer && !layer->LoadAnnotationFromFile(filename)) {
    QMessageBox::warning(
        this, "Error",
        QString("Could not load annotation from %1").arg(filename));
  }
}

void MainWindow::LoadSurfaceLabel() {
  QStringList filenames = QFileDialog::getOpenFileNames(
      this, "Select label files", AutoSelectLastDir("label"),
      "Label files (*)");
  if (!filenames.isEmpty()) {
    for (int i = 0; i < filenames.size(); i++)
...
bool MainWindow::LoadSurfaceLabelFile(const QString &filename) {
  ...
  if (layer && !layer->LoadLabelFromFile(filename)) {
    QMessageBox::warning(this, "Error",
                         QString("Could not load label from %1").arg(filename));
    return false;
  }
  return true;
}
...
void MainWindow::LoadSurfaceSpline() {
  QString filename =
      QFileDialog::getOpenFileName(this, "Select surface spline file",
                                   AutoSelectLastDir("surf"), "All files (*)");
  if (!filename.isEmpty()) {
...
void MainWindow::LoadSurfaceSplineFile(const QString &filename) {
  ...
  if (layer && !layer->LoadSplineFromFile(filename)) {
    QMessageBox::warning(
        this, "Error",
        QString("Could not load spline data from %1").arg(filename));
    ;
  }
}
...
void MainWindow::LoadSurfaceVector() {
  QString filename =
      QFileDialog::getOpenFileName(this, "Select surface vector file",
                                   AutoSelectLastDir("surf"), "All files (*)");
  if (!filename.isEmpty()) {
...
void MainWindow::LoadLUT() {
  QString filename = QFileDialog::getOpenFileName(
      this, "Select lookup table file", m_strLastDir, "LUT files (*)");
  if (!filename.isEmpty()) {
    LayerMRI *mri = (LayerMRI *)GetLayerCollection("MRI")->GetActiveLayer();
    if (mri) {
...
int MainWindow::GetMode() { return m_views[0]->GetInteractionMode(); }

void MainWindow::OnSetModeNavigate() { SetMode(RenderView::IM_Navigate); }

void MainWindow::OnSetModeMeasure() { SetMode(RenderView::IM_Measure); }

void MainWindow::OnSetModeVoxelEdit() { SetMode(RenderView::IM_VoxelEdit); }

void MainWindow::OnSetModeReconEdit() { SetMode(RenderView::IM_ReconEdit); }

void MainWindow::OnSetModeROIEdit() { SetMode(RenderView::IM_ROIEdit); }
    ...
    if (!scales_in.isEmpty()) {
      ...
      case LayerPropertyMRI::Grayscale:
        if (scales.size() >= 2) {
          ...
          std::cerr << "Need 2 values for grayscale.\n";
        }
        break;
      case LayerPropertyMRI::Heat:
        if (scales.size() >= 3) {
          ...
          std::cerr << "Need 2 or 3 values for heatscale.\n";
        }
        break;
      case LayerPropertyMRI::LUT:
        if (scales.size() >= 1) {
        } else if (!scales.empty()) {
          std::cerr << "Need a value for lut.\n";
        }
        break;
      default:
        if (scales.size() >= 2) {
          ...
          std::cerr << "Need 2 values for colorscale.\n";
        }
        break;
      }
    }
...
void MainWindow::OnTransformVolume() {
  if (!m_dlgTransformVolume->isVisible()) {
    std::cout
        << "Warning: Transformation can only apply to volumes for now. If "
    ...
    if (mri && nWnd >= 0 && nWnd < 3) {
      ...
      if (!mri->CopyStructure(nWnd, pos)) {
        QMessageBox::warning(this, "Copy Structure Failed",
                             "Please move the cursor to the structure you want "
                             "to copy and try again.");
      }
    }
  }
}
...
void MainWindow::OnToggleWm() { ToggleSpecialVolume("wm.mgz"); }

void MainWindow::OnToggleAseg() { ToggleSpecialVolume("aseg.mgz"); }

void MainWindow::OnToggleBrainmask() { ToggleSpecialVolume("brainmask.mgz"); }
...
void MainWindow::OnToggleShowVolume() { ToggleShowLayer("MRI"); }

void MainWindow::OnToggleShowSurface() { ToggleShowLayer("Surface"); }

void MainWindow::OnToggleShowROI() { ToggleShowLayer("ROI"); }

void MainWindow::OnToggleShowPointSet() { ToggleShowLayer("PointSet"); }
...
void MainWindow::OnActiveLayerChanged(Layer *layer) {
  if (!layer) {
    ...
    this->setWindowTitle(QString("FreeView (%1)").arg(fn));
    if (layer->IsTypeOf("MRI") && !layer->IsTypeOf("DTI") &&
      ...
      if (((LayerMRI *)layer)->GetNumberOfFrames() > 1 &&
        ...
        connect(layer, SIGNAL(ActiveFrameChanged(int)),
                ...
                Qt::UniqueConnection);
        connect(layer, SIGNAL(ActiveFrameChanged(int)), ui->treeWidgetMouseInfo,
                SLOT(OnMousePositionChanged()), Qt::UniqueConnection);
        connect(layer, SIGNAL(ActiveFrameChanged(int)), m_dlgLabelStats,
                SLOT(UpdateStats()), Qt::UniqueConnection);
        m_wndTimeCourse->UpdateData();
        if (ui->actionTimeCourse->isChecked() &&
            !layer->IsTypeOf("VolumeTrack")) {
      ...
      if (surf->GetActiveOverlay() &&
          surf->GetActiveOverlay()->GetNumberOfFrames() > 1) {
        m_wndTimeCourse->UpdateData();
        if (ui->actionTimeCourse->isChecked()) {
          m_wndTimeCourse->show();
        }
      ...
      if (tract && tract->IsCluster()) {
        connect(m_wndTractCluster, SIGNAL(TreeNodeActivated(QStringList)),
                tract, SLOT(LoadTrackFromFiles(QStringList)),
                Qt::UniqueConnection);
...
void MainWindow::OnLoadCommand() {
  QString filename = QFileDialog::getOpenFileName(
      this, "Select command file", m_strLastDir, "Command files (*)");
  if (!filename.isEmpty()) {
...
void MainWindow::SaveSurface() {
  // first check if there is any volume/MRI layer and if the current one is
  // visible
  LayerSurface *layer_surf = (LayerSurface *)GetActiveLayer("Surface");
  if (!layer_surf) {
    return;
  ...
  if (fn.isEmpty()) {
    ...
    name.replace(" ", "_");
    fn = QFileDialog::getSaveFileName(
        this, "Save surface", AutoSelectLastDir("surf"), "Surface files (*)");
  }
...
void MainWindow::SaveSurfaceAs() {
  ...
  QString fn = QFileDialog::getSaveFileName(
      this, "Save surface as", layer_surf->GetFileName(), "Surface files (*)");
  if (!fn.isEmpty()) {
    layer_surf->SetFileName(fn);
    SaveSurface();
    ui->widgetAllLayers->UpdateWidgets();
...
void MainWindow::OnPlot() {
  QString fn =
      QFileDialog::getOpenFileName(this, "Select FSGD File", m_strLastFsgdDir,
...
void MainWindow::ToggleSplinePicking() {
  ...
  std::cout << qPrintable(QString("Surface spline picking %1")
  ...
  if (!sel_layers.isEmpty()) {
    ...
    if (dlg.Execute(sel_layers) == QDialog::Accepted) {
      ...
      for (int i = 0; i < sel_layers.size(); i++) {
        ...
        m_layerSettings[mri->GetID() + (bCloseFirst ? 0 : LAYER_ID_OFFSET)] =
            map;
      }
      ...
      if (bCloseFirst) {
        for (int i = 0; i < sel_layers.size(); i++) {
          layer_ids << QString::number(sel_layers[i]->GetID() +
                                       LAYER_ID_OFFSET);
        }
      }
      for (int i = sel_layers.size() - 1; i >= 0; i--) {
        ...
        if (bCloseFirst) {
          ...
          AddScript(QStringList("unloadlayers")
                    << "mri" << QString::number(mri->GetID()));
        }
      }

      if (bCloseFirst) {
        AddScript(QStringList("reorderlayers")
                  << "mri" << layer_order.join(","));
        for (int i = 0; i < layer_ids.size(); i++)
          layer_ids[i] =
              QString::number(layer_ids[i].toInt() - LAYER_ID_OFFSET);
        AddScript(QStringList("setactivelayer")
                  << "mri" << QString::number(active_layer_id)
                  << layer_ids.join(","));
      }
    }
  } else
    QMessageBox::warning(this, "Reload Volume",
                         "Select at least one volume to reload.");
}
...
void MainWindow::OnReloadROI() {
  ...
  if (!sel_layers.isEmpty()) {
    ...
    if (dlg.Execute(sel_layers) == QDialog::Accepted) {
      ...
      //        m_layerSettings[roi->GetID()] =
      //        roi->GetProperty()->GetFullSettings();
      //      }
      ...
      for (int i = sel_layers.size() - 1; i >= 0; i--) {
        ...
        args += QString(":id=%1:color=%2,%3,%4:opacity=%5:threshold=%6:ref=%7")
                    .arg(roi->GetID() + (bCloseFirst ? 0 : LAYER_ID_OFFSET))
                    .arg((int)(rgb[0] * 255))
                    .arg((int)(rgb[1] * 255))
                    .arg((int)(rgb[2] * 255))
                    .arg(roi->GetProperty()->GetOpacity())
                    .arg(roi->GetProperty()->GetThreshold())
                    .arg(roi->GetRefMRI()->GetID());
        ...
        if (bCloseFirst)
          AddScript(QStringList("unloadlayers")
                    << "roi" << QString::number(roi->GetID()));
      }
      if (bCloseFirst) {
        AddScript(QStringList("reorderlayers")
                  << "roi" << layer_order.join(","));
        for (int i = 0; i < layer_ids.size(); i++)
          layer_ids[i] =
              QString::number(layer_ids[i].toInt() - LAYER_ID_OFFSET);
        AddScript(QStringList("setactivelayer")
                  << "roi" << QString::number(active_layer_id)
                  << layer_ids.join(","));
      }
    }
  }
  ...
  if (!sel_layers.isEmpty()) {
    ...
    if (dlg.Execute(sel_layers) == QDialog::Accepted) {
      ...
      for (int i = sel_layers.size() - 1; i >= 0; i--) {
        ...
        QString        args =
            filename +
            QString(":id=%1:color=%2,%3,%4:name=%5:radius=%6:visible=%7")
                .arg(ps->GetID() + (bCloseFirst ? 0 : LAYER_ID_OFFSET))
                .arg((int)(rgb[0] * 255))
                .arg((int)(rgb[1] * 255))
                .arg((int)(rgb[2] * 255))
                .arg(ps->GetName())
                .arg(ps->GetProperty()->GetRadius())
                .arg(ps->IsVisible() ? 1 : 0);
        ...
        if (ps->GetProperty()->GetType() == LayerPropertyPointSet::ControlPoint)
          ...
          args += QString(":splinecolor=%1,%2,%3:splineradius=%4")
                      .arg((int)(rgb[0] * 255))
                      .arg((int)(rgb[1] * 255))
                      .arg((int)(rgb[2] * 255))
          ...
          if (bCloseFirst)
            AddScript(QStringList("unloadlayers")
                      << "pointset" << QString::number(ps->GetID()));
        }
      }
      if (bCloseFirst) {
        AddScript(QStringList("reorderlayers")
                  << "pointset" << layer_order.join(","));
        for (int i = 0; i < layer_ids.size(); i++)
          layer_ids[i] =
              QString::number(layer_ids[i].toInt() - LAYER_ID_OFFSET);
        AddScript(QStringList("setactivelayer")
                  << "pointset" << QString::number(active_layer_id)
                  << layer_ids.join(","));
      }
    }
  }
  ...
  if (!sel_layers.isEmpty()) {
    ...
    if (dlg.Execute(sel_layers) == QDialog::Accepted) {
      ...
      for (int i = sel_layers.size() - 1; i >= 0; i--) {
        ...
        m_layerSettings[surf->GetID() + (bCloseFirst ? 0 : LAYER_ID_OFFSET)] =
            surf->GetProperty()->GetFullSettings();
      }
      ...
      for (int i = sel_layers.size() - 1; i >= 0; i--) {
        LayerSurface *surf = qobject_cast<LayerSurface *>(sel_layers[i]);
        QString       args =
            QString("%1:name=%2:id=%3")
                .arg(surf->GetFileName())
                .arg(surf->GetName())
                .arg(surf->GetID() + (bCloseFirst ? 0 : LAYER_ID_OFFSET));
        ...
        for (int j = surf->GetNumberOfLabels() - 1; j >= 0; j--) {
          ...
          if (label) {
            ...
            if (label->GetOpacity() != 1)
              AddScript(QStringList("setsurfacelabelopacity")
                        << QString::number(label->GetOpacity()));
        ...
        for (int j = surf->GetNumberOfAnnotations() - 1; j >= 0; j--) {
          ...
          if (annot) {
            AddScript(QStringList("loadsurfaceannotation")
                      << annot->GetFilename());
        ...
        if (dlg.GetCloseLayerFirst())
          AddScript(QStringList("unloadlayers")
                    << "surface" << QString::number(surf->GetID()));
      }

      if (bCloseFirst) {
        AddScript(QStringList("reorderlayers")
                  << "surface" << layer_order.join(","));
        for (int i = 0; i < layer_ids.size(); i++)
          layer_ids[i] =
              QString::number(layer_ids[i].toInt() - LAYER_ID_OFFSET);
        AddScript(QStringList("setactivelayer")
                  << "surface" << QString::number(active_layer_id)
                  << layer_ids.join(","));
      }
    }
  }
...
void MainWindow::UpdateInfoPanel() {
  QTimer::singleShot(0, ui->treeWidgetCursorInfo,
                     SLOT(OnCursorPositionChanged()));
  QTimer::singleShot(0, ui->treeWidgetMouseInfo,
...
void MainWindow::OnLoadConnectomeMatrix() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    AddScript(QStringList("loadconnectome") << QString("%1:lut=%2")
                                                   .arg(dlg.GetCMATFilename())
                                                   .arg(dlg.GetCTABFilename())
                                            << dlg.GetParcelFilename());
  }
}

void MainWindow::CommandSetVolumeMask(const QStringList &cmd) {
  LayerMRI *mri  = qobject_cast<LayerMRI *>(this->GetActiveLayer("MRI"));
  LayerMRI *mask = qobject_cast<LayerMRI *>(
      GetLayerCollection("MRI")->GetLayerByName(cmd[1]));
  if (!mask) {
    std::cerr << "Can not find volume by name of " << qPrintable(cmd[1])
              << std::endl;
    return;
  }
  if (mri && mask) {
...
void MainWindow::OnGoToSurfaceLabel(bool center) {
  ...
  if (surf && surf->GetActiveLabelCentroidPosition(pos)) {
    bool mappedFromInflated =
        ((RenderView3D *)m_views[3])->MapInflatedCoords(surf, pos, pos, true);
    SetSlicePosition(pos);
    if (center) {
      GetMainView()->CenterAtWorldPosition(pos);
...
void MainWindow::LoadFCD(const QString &subdir, const QString &subject,
  ...
  connect(layer->GetWorkerThread(), SIGNAL(Progress(int)), m_statusBar,
          SLOT(SetProgress(int)));
  connect(layer->GetWorkerThread(), SIGNAL(started()), m_statusBar,
          SLOT(ShowProgress()));
  connect(layer->GetWorkerThread(), SIGNAL(finished()), m_statusBar,
          SLOT(HideProgress()));
  if (suffix.isEmpty())
...
void MainWindow::OnLoadFCD() {
  QString subject_dir =
      QFileDialog::getExistingDirectory(this, "Select Subject", m_strLastDir);
...
void MainWindow::UpdateSettings() {
  if (m_dlgPreferences) {
    ...
    if (old["AutoSetMidToMin"].toBool() !=
        m_settings["AutoSetMidToMin"].toBool()) {
      QList<Layer *> layers = GetLayers("MRI");
...
void MainWindow::OnToolSaveCamera() {
  QString fn = QFileDialog::getSaveFileName(this, "Save Camera", m_strLastDir,
                                            "All files (*)");
  if (!fn.isEmpty()) {
...
void MainWindow::OnToolLoadCamera(const QString &fn_in) {
  QString fn = fn_in;
  if (fn.isEmpty())
    fn = QFileDialog::getOpenFileName(this, "Load Camera", m_strLastDir,
                                      "All files (*)");
  if (!fn.isEmpty()) {
    QFile file(fn);
  ...
  if (layer) {
    ...
    if (surf) {
      if (surf->IsContralateralReady())
        ...
        AddScript(QStringList("gotocontralateralsurface")
                  << QString::number(reinterpret_cast<quintptr>(surf)));
      }
    }
  }
}
  ...
  if (nvo >= 0) {
    ...
    if (layer) {
      ...
      if (bInflated) {
        ...
        ((RenderView3D *)m_views[3])
            ->MapInflatedCoords(layer, pos, pos,
                                m_settings["AutoReorientView"].toBool(), true);
      } else
        layer->GetTargetAtVertex(nvo, pos);
      SetSlicePosition(pos);
      CenterAtWorldPosition(pos);
    ...
    std::cout << "Did not find any vertex at cursor on "
              << qPrintable(layer->GetName()) << std::endl;
  }
}
...
LayerSurface *GetContralateralSurfaceLayer(LayerSurface *        surf,
...
void MainWindow::UpdateSurfaceContralateralInfo() {
  ...
  QList<Layer *> sphere_surfs =
      GetLayerCollection("HiddenSurface")->GetLayers("Surface");
  ...
  for (int i = 0; i < surfs.size(); i++) {
    ...
    if (surf2) {
      ...
      LayerSurface *sphere1 = (LayerSurface *)GetLayerByFilename(
          QFileInfo(path + "/lh.sphere.d1.left_right").absoluteFilePath(),
          sphere_surfs);
      LayerSurface *sphere2 = (LayerSurface *)GetLayerByFilename(
          QFileInfo(path + "/rh.sphere.d1.left_right").absoluteFilePath(),
          sphere_surfs);
      if (sphere1 && sphere2) {
...
bool MainWindow::LoadSurfaceRGBMap(const QString &fn) {
  ...
  if (filename.isEmpty())
    filename = QFileDialog::getOpenFileName(
        this, "Select RGB file", AutoSelectLastDir("surf"), "All files (*)");
  if (!filename.isEmpty()) {
    LayerSurface *layer =
        qobject_cast<LayerSurface *>(GetActiveLayer("Surface"));
    if (layer) {
...
void MainWindow::OnApplyVolumeTransform() {
  ...
  if (mri) {
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      this->LoadVolumeFile(filename, dlg.GetFilename(), m_bResampleToRAS,
                           dlg.GetSampleMethod(), false, -1, "", sup_data);
    }
  }
}
...
void MainWindow::OnTractClusterLoaded(const QVariantMap &data) {
  ...
  if (filenames.isEmpty()) {
    QMessageBox::warning(this, "Error",
                         "Could not find any tract files in selected folder");
  } else {
    ShowTractClusterMap();
...
void MainWindow::OnLoadSurfaceParameterization() {
  QString filename = QFileDialog::getOpenFileName(
      this, "Select Parameterization File", AutoSelectLastDir("surf"),
...
void MainWindow::LoadSurfaceParameterization(const QString &filename) {
  ...
  if (surf && !surf->LoadParameterization(filename)) {
    QMessageBox::warning(
        this, "Error",
        QString("Could not load parameterization from %1").arg(filename));
  }
}
...
void MainWindow::LoadSurfaceCoordsFromParameterization(
...
void MainWindow::OnExportLabelStats() {
  QString fn = QFileDialog::getSaveFileName(
      this, "Save Label Stats", AutoSelectLastDir("mri"), "CSV files (*.csv)");
  if (!fn.isEmpty()) {
    LayerMRI *mri = (LayerMRI *)GetActiveLayer("MRI");
    if (!mri->ExportLabelStats(fn))
      QMessageBox::warning(this, "Error",
                           QString("Could not save label stats to %1").arg(fn));
  }
}
...
void MainWindow::CommandExportLineProfileThickness(const QStringList &cmd) {
  ...
  if (!ExportLineProfileThickness(fn, opts))
    std::cerr << "Failed to export line profile thickness to " << qPrintable(fn)
              << std::endl;
}
...
bool MainWindow::ExportLineProfileThickness(const QString &    filename,
  ...
  if (lines.size() > 1) {
    LayerLineProfile *lp = new LayerLineProfile(GetMainViewId(), this,
    ...
    if (!lp->Solve(spacing, dVoxelSize, resolution, offset)) {
      std::cerr << "Could not solve line profile\n";
      lp->deleteLater();
      return false;
    } else if (!lp->ExportThickness(filename, lines, samples)) {
      std::cerr << "Could not export line profile thickness\n";
      lp->deleteLater();
      return false;
    } else
      lp->deleteLater();
