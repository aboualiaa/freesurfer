...
DialogLineProfile::~DialogLineProfile() { delete ui; }

bool DialogLineProfile::Validate(LayerPointSet *&spline0_out,
                                 LayerPointSet *&spline1_out) {
  QVariant v =
...
void DialogLineProfile::OnExport() {
  ...
  QString fn =
      QFileDialog::getSaveFileName(this, "Export Line Profiles to File", "",
  ...
  if (!fn.isEmpty()) {
    ...
    if (!lp) {
      ...
      if (Validate(ptset0, ptset1)) {
        MainWindow *mainwnd = MainWindow::GetMainWindow();
        lp = new LayerLineProfile(mainwnd->GetActiveViewId(), NULL, ptset0,
                                  ptset1);
      }
    }
    if (lp && !lp->Save(fn)) {
...
void DialogLineProfile::OnLoad() {
  ...
  if (!fn.isEmpty()) {
    ...
    if (!lp) {
      ...
      ui->lineEditSamplePoints->setText(
          QString::number(lp->GetNumberOfSamples()));
...
void DialogLineProfile::UpdatePointSetList() {
  ...
  foreach (Layer *layer, layers) {
    ui->comboBoxIsoLine1->addItem(layer->GetName(),
                                  qVariantFromValue<QObject *>(layer));
    ui->comboBoxIsoLine2->addItem(layer->GetName(),
                                  qVariantFromValue<QObject *>(layer));
  }
...
void DialogLineProfile::OnExportThickness() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    QString fn =
        QFileDialog::getSaveFileName(this, "Export Thickness to File", "",
                                     "CSV files (*.csv);;All Files (*)");
    if (!fn.isEmpty()) {
      if (!m_lineProfile->ExportThickness(fn, dlg.GetSelectedPointSets(),
                                          GetNumberOfSamples())) {
        ui->labelError->setText("Failed to export");
        ui->labelError->show();
      }
