...
LayerPropertyTrack::LayerPropertyTrack(QObject *parent)
    : LayerProperty(parent), m_nColorCode(Directional),
      m_nDirectionScheme(EndPoints), m_nDirectionMapping(RGB),
      m_color(Qt::yellow), m_nRenderRep(Line), m_dTubeRadius(0.2),
      m_nNumberOfSides(5), m_dOpacity(1) {
  connect(this, SIGNAL(ColorCodeChanged(int)), this, SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(DirectionSchemeChanged(int)), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(DirectionMappingChanged(int)), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(SolidColorChanged(QColor)), this,
          SIGNAL(PropertyChanged()));
