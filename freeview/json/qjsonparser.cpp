...
#define JSONERR_UNTERM_OBJ                                                     \
  QT_TRANSLATE_NOOP("QJsonParseError", "unterminated object")
#define JSONERR_MISS_NSEP                                                      \
  QT_TRANSLATE_NOOP("QJsonParseError", "missing name separator")
#define JSONERR_UNTERM_AR                                                      \
  QT_TRANSLATE_NOOP("QJsonParseError", "unterminated array")
#define JSONERR_MISS_VSEP                                                      \
  QT_TRANSLATE_NOOP("QJsonParseError", "missing value separator")
#define JSONERR_ILLEGAL_VAL                                                    \
  QT_TRANSLATE_NOOP("QJsonParseError", "illegal value")
#define JSONERR_END_OF_NUM                                                     \
  QT_TRANSLATE_NOOP("QJsonParseError", "invalid termination by number")
#define JSONERR_ILLEGAL_NUM                                                    \
  QT_TRANSLATE_NOOP("QJsonParseError", "illegal number")
#define JSONERR_STR_ESC_SEQ                                                    \
  QT_TRANSLATE_NOOP("QJsonParseError", "invalid escape sequence")
#define JSONERR_STR_UTF8                                                       \
  QT_TRANSLATE_NOOP("QJsonParseError", "invalid UTF8 string")
#define JSONERR_UTERM_STR                                                      \
  QT_TRANSLATE_NOOP("QJsonParseError", "unterminated string")
#define JSONERR_MISS_OBJ                                                       \
  QT_TRANSLATE_NOOP("QJsonParseError", "object is missing after a comma")
#define JSONERR_DEEP_NEST                                                      \
  QT_TRANSLATE_NOOP("QJsonParseError", "too deeply nested document")
#define JSONERR_DOC_LARGE                                                      \
  QT_TRANSLATE_NOOP("QJsonParseError", "too large document")
...
/*!
    ...
    \brief The QJsonParseError class is used to report errors during JSON
   parsing.
...
/*!
    ...
    This enum describes the type of error that occurred during the parsing of a
   JSON document.

    \value NoError                  No error occurred
    \value UnterminatedObject       An object is not correctly terminated with a
   closing curly bracket \value MissingNameSeparator     A comma separating
   different items is missing \value UnterminatedArray        The array is not
   correctly terminated with a closing square bracket \value
   MissingValueSeparator    A colon separating keys from values inside objects
   is missing \value IllegalValue             The value is illegal \value
   TerminationByNumber      The input stream ended while parsing a number \value
   IllegalNumber            The number is not well formed \value
   IllegalEscapeSequence    An illegal escape sequence occurred in the input
    \value IllegalUTF8String        An illegal UTF8 sequence occurred in the
   input \value UnterminatedString       A string wasn't terminated with a quote
    \value MissingObject            An object was expected but couldn't be found
    \value DeepNesting              The JSON document is too deeply nested for
   the parser to parse it \value DocumentTooLarge         The JSON document is
   too large for the parser to parse it
*/

/*!
    \variable QJsonParseError::error

    Contains the type of the parse error. Is equal to QJsonParseError::NoError
   if the document was parsed correctly.
...
/*!
  Returns the human-readable message appropriate to the reported JSON parsing
  error.
...
Parser::Parser(const char *json, int length)
    : head(json), json(json), data(0), dataLength(0), current(0),
      nestingLevel(0), lastError(QJsonParseError::NoError) {
  end = json + length;
}
...
void Parser::eatBOM() {
  ...
  if (end - json > 3 && (uchar)json[0] == utf8bom[0] &&
      (uchar)json[1] == utf8bom[1] && (uchar)json[2] == utf8bom[2])
    json += 3;
}
...
void Parser::ParsedObject::insert(uint offset) {
  const QJsonPrivate::Entry *newEntry =
...
static inline bool scanEscapeSequence(const char *&json, const char *end,
                                      uint *ch) {
  ++json;
  if (json >= end)
    return false;
...
static inline bool scanUtf8Char(const char *&json, const char *end,
  ...
  if (isUnicodeNonCharacter(uc) || uc >= 0x110000 || (uc < min_uc) ||
      (uc >= 0xd800 && uc <= 0xdfff)) {
    return false;
  }
...
bool Parser::parseString(bool *latin1) {
  *latin1 = true;
  ...
  while (json < end) {
    ...
    // bail out if the string is not pure latin1 or too long to hold as a
    // latin1string (which has only 16 bit for the length)
  ...
  if (*latin1) {
    // write string length
    *(QJsonPrivate::qle_ushort *)(data + stringPos) =
        ushort(current - outStart - sizeof(ushort));
  ...
  *latin1 = false;
  ...
  // write string length
  *(QJsonPrivate::qle_int *)(data + stringPos) =
      (current - outStart - sizeof(int)) / 2;
  int pos = reserveSpace((4 - current) & 3);
  while (pos & 3)
    data[pos++] = 0;
  END;
