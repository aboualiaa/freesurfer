...
void Region2DPolyline::UpdateWorldCoords() {
  ...
  for (int i = m_worldPts.size(); i < m_screenPts.size(); i++) {
    m_view->MousePositionToRAS(m_screenPts[i].pos[0], m_screenPts[i].pos[1],
                               wp.pos);
    if (i > 0 &&
        vtkMath::Distance2BetweenPoints(wp.pos, m_worldPts[i - 1].pos) < 1e-8) {
      wp.pos[0] += 0.00001;
      wp.pos[1] += 0.00001;
      wp.pos[2] += 0.00001;
    }
...
void Region2DPolyline::Update() {
  ...
  for (int i = 0; i < m_worldPts.size(); i++) {
    m_view->WorldToViewport(m_worldPts[i].pos[0], m_worldPts[i].pos[1],
                            m_worldPts[i].pos[2], pt[0], pt[1], pt[2]);
...
void Region2DPolyline::UpdateStats() {
  ...
  if (m_bSpline) {
    ...
    for (int i = 0; i < nSteps; i++) {
      ...
      int count = 0;
      if (layer) {
        layer->GetVoxelsOnLine(pt1, pt2, m_view->GetViewPlane(), indices,
                               values, &count);
        for (int j = 0; j < (i == nSteps - 1 ? count : count - 1); j++) {
          sprintf(ch, "[%d, %d, %d]  %.2f", indices[j * 3], indices[j * 3 + 1],
                  indices[j * 3 + 2], values[j]);
    ...
    for (int i = 1; i < m_worldPts.size(); i++) {
      ...
      int count = 0;
      if (layer) {
        layer->GetVoxelsOnLine(m_worldPts[i - 1].pos, m_worldPts[i].pos,
                               m_view->GetViewPlane(), indices, values, &count);
        for (int j = 0; j < (i == m_worldPts.size() - 1 ? count : count - 1);
             j++) {
          sprintf(ch, "[%d, %d, %d]  %.2f", indices[j * 3], indices[j * 3 + 1],
                  indices[j * 3 + 2], values[j]);
          m_strsLongStats.push_back(ch);
...
bool Region2DPolyline::Contains(int x, int y, int *indexOut) {
  ...
  for (int i = 0; i < m_worldPts.size() - 1; i++) {
    if (vtkLine::DistanceToLine(pt, m_worldPts[i].pos, m_worldPts[i + 1].pos, t,
                                closestPt) < dTh2) {
      if (t >= 0 && t <= 1) {
        return true;
      }
