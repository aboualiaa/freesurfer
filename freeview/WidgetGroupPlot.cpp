...
WidgetGroupPlot::WidgetGroupPlot(QWidget *parent)
    : QWidget(parent), m_bAutoScale(true), m_fsgd(NULL),
      m_nCurrentVariableIndex(0), m_nCurrentDataIndex(-1), m_nPlotType(Point) {
...
void WidgetGroupPlot::paintEvent(QPaintEvent *e) {
  ...
  fnt.setPixelSize(11);
  int nTextLen = qMax(
      QFontMetrics(fnt).width(QString::number(m_fsgd->m_variables[0].range[0])),
      QFontMetrics(fnt).width(
          QString::number(m_fsgd->m_variables[0].range[1])));
  ...
  for (int n = nStart; n <= nEnd; n++) {
    ...
    for (int i = 0; i < data_y.size(); i++) {
      ...
      if (m_nPlotType == Point) {
        DrawMarker(
            &p, pts[i],
            m_fsgd->m_classes[m_fsgd->m_data[i].class_id].marker.toLower(), c,
      ...
      if (m_nCurrentDataIndex == i) {
        ...
        for (int m = n; m <= n; m++) {
          tooltip_strg += QString("\n%1: %2")
                              .arg(m_fsgd->m_variables[m].label)
    ...
    p.drawLine(pts[m_nCurrentFrame] - QPointF(0, 20),
               pts[m_nCurrentFrame] + QPointF(0, qMin(20.,
    rc_plot.bottom()-pts[m_nCurrentFrame].y())));
               */
    delete[] pts;
    ...
    while (y > rc_plot.top()) {
      if (y <= rc_plot.bottom()) {
        ...
        if (qAbs(dMetricPos) < 1e-8)
          strg = "0";
        p.drawText(QRectF(rect().left(), y - 10,
                          rc_plot.left() - 5 - rect().left(), 20),
    ...
    if (true) {
      ...
      while (x < rc_plot.right()) {
        ...
        x = rc_plot.left() +
            (dMetricPos - dMin_x) / (dMax_x - dMin_x) * rc_plot.width();
        p.drawLine(QPoint(x, rc_plot.bottom()),
                   QPoint(x, rc_plot.bottom() + 2));
      }
      if (dMin_x < 0 && dMax_x > 0) {
        double y =
            rc_plot.bottom() - (0 - dMin) / (dMax - dMin) * rc_plot.height();
    ...
    p.drawText(rc, Qt::AlignBottom | Qt::AlignHCenter,
               m_fsgd->m_variables[m_nCurrentVariableIndex].label);

    // draw current stats
    if (m_nCurrentDataIndex >= 0) {
      rc = p.boundingRect(QRectF(0, 0, 1, 1), Qt::AlignLeft | Qt::AlignVCenter,
                          tooltip_strg);
      int bw = 6;
...
void WidgetGroupPlot::DrawMarker(QPainter *p, const QPointF &pt,
                                 const QString &marker, const QColor &c,
                                 bool bHighlight) {
...
void WidgetGroupPlot::mousePressEvent(QMouseEvent *e) {
  if (m_rectPlot.contains(e->pos())) {
    ...
    for (int i = 0; i < m_fsgd->m_data.size(); i++) {
      QPointF pt = QPointF(
          m_rectPlot.left() + (m_fsgd->m_data[i].variable_values[n] - dMin_x) /
                                  (dMax_x - dMin_x) * m_rectPlot.width(),
          m_rectPlot.bottom() - (m_fsgd->m_data[i].measurement - dMin) /
                                    (dMax - dMin) * m_rectPlot.height());
      if (m_nPlotType == Point) {
        bool bHit = ((pt.x() - c_pt.x()) * (pt.x() - c_pt.x()) +
                         (pt.y() - c_pt.y()) * (pt.y() - c_pt.y()) <
                     DISTANCE_THRESHOLD2);
        if (bHit)
          m_nCurrentDataIndex = i;
      }
...
void WidgetGroupPlot::mouseMoveEvent(QMouseEvent *e) { Q_UNUSED(e); }

void WidgetGroupPlot::leaveEvent(QEvent *e) { Q_UNUSED(e); }

void WidgetGroupPlot::keyPressEvent(QKeyEvent *e) { Q_UNUSED(e); }
