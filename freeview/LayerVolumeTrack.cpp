...
LayerVolumeTrack::LayerVolumeTrack(LayerMRI *ref, QObject *parent)
    : LayerMRI(ref, parent), m_ctabStripped(NULL) {
  m_strTypeNames.push_back("VolumeTrack");
  SetEditable(false);
  connect(GetProperty(), SIGNAL(ContourSmoothIterationChanged(int)), this,
          SLOT(RebuildActors()));
}
...
void LayerVolumeTrack::UpdateData() {
  if (!m_volumeSource->GetEmbeddedColorTable()) {
    std::cerr << "Did not find color table in track volume.\n";
...
void LayerVolumeTrack::RebuildActors() {
  ...
  for (int i = 0; i < mri->nframes; i++) {
    ...
    MyVTKUtils::BuildContourActor(extract->GetOutput(), mri->frames[i].thresh,
                                  1e8, actor, nSmoothIterations, NULL, false,
                                  false);
...
void LayerVolumeTrack::UpdateFrameActor(int n) {
  ...
  extract->Update();
  MRI *mri = m_volumeSource->GetMRI();
  MyVTKUtils::BuildContourActor(extract->GetOutput(), mri->frames[n].thresh,
                                1e8, actor);
...
void LayerVolumeTrack::Append3DProps(vtkRenderer *renderer,
...
void LayerVolumeTrack::UpdateColorMap() { LayerMRI::UpdateColorMap(); }
