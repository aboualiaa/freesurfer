...
bool LayerPLabel::LoadVolumeFiles() {
  ...
  for (int i = 0; i < m_sFilenames.size(); i++) {
    ...
    int r, g, b;
    if (CTABrgbAtIndexi(ct, CTABentryNameToIndex(fn.toLatin1().data(), ct), &r,
                        &g, &b) != 0 &&
        CTABrgbAtIndexi(
            ct,
            CTABentryNameToIndex(
                QString(fn).replace("-", "/").toLatin1().data(), ct),
            &r, &g, &b) != 0 &&
        CTABrgbAtIndexi(
            ct,
            CTABentryNameToIndex(
    ...
    for (int ni = 0; ni < dim[0]; ni++) {
      for (int nj = 0; nj < dim[1]; nj++) {
        for (int nk = 0; nk < dim[2]; nk++) {
          float pvalue = MyVTKUtils::GetImageDataComponent(
              img_ptr, img_dim, img_n_frames, ni, nj, nk, 0, img_scalar_type);
          if (i == 0) {
            MyVTKUtils::SetImageDataComponent(index_ptr, index_dim,
                                              index_n_frames, ni, nj, nk, 0,
                                              index_scalar_type, 0);
            MyVTKUtils::SetImageDataComponent(index_ptr, index_dim,
                                              index_n_frames, ni, nj, nk, 1,
                                              index_scalar_type, pvalue);
          } else {
            float old_pvalue = MyVTKUtils::GetImageDataComponent(
                index_ptr, index_dim, index_n_frames, ni, nj, nk, 0,
                index_scalar_type);
          ...
          for (int m = 0; m < 4; m++) {
            float fvalue = 0;
            if (i != 0) {
              fvalue = MyVTKUtils::GetImageDataComponent(
                  ptr, dim, n_frames, ni, nj, nk, m, scalar_type);
            }
            ...
            MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, ni, nj, nk, m,
                                              scalar_type, fvalue);
          }
        }
      }
