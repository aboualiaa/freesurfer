...
WindowEditAnnotation::WindowEditAnnotation(QWidget *parent)
  ...
  connect(ui->lineEditName, SIGNAL(returnPressed()),
          SLOT(OnEditNameReturnPressed()));
  ...
  connect(ui->treeWidgetAvailableLabels,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
          SLOT(OnAvailableLabelClicked(QTreeWidgetItem *)));
...
void WindowEditAnnotation::PopulateColorTable(bool bForce) {
  ...
  if (ct) // && (bForce || ct != m_ctab))
  {
    ...
    //      QTreeWidgetItem(ui->treeWidgetAvailableLabels); item->setText( 0,
    //      QString("%1").arg(nl.name) ); item->setToolTip( 0, nl.name );
    //      QPixmap pix(13, 13);
    //      pix.fill( nl.color );
    //      item->setIcon(0, QIcon(pix) );
    ...
    //      selectedLabels.contains(nl.id)?Qt::Checked:Qt::Unchecked);
    //    }
    if (nSel >= 0) {
      ...
      for (int n = 0; n < ui->treeWidgetExistingLabels->topLevelItemCount();
        ...
        if (item != itemTo && item->data(0, ANNOT_INDEX_ROLE).toInt() == i) {
          if (QMessageBox::question(
                  this, "Confirm",
                  "This label already exists in the annotation. Do you want to "
                  "combine them?") != QMessageBox::Yes)
            return;
          else
            bMerged = true;
        }
