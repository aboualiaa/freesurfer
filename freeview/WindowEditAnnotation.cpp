...
WindowEditAnnotation::WindowEditAnnotation(QWidget *parent)
  ...
  connect(ui->treeWidgetExistingLabels,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
          SLOT(OnExistingLabelClicked(QTreeWidgetItem *)),
          Qt::QueuedConnection);
  connect(ui->treeWidgetExistingLabels,
          SIGNAL(itemChanged(QTreeWidgetItem *, int)),
          SLOT(OnExistingLabelItemChanged(QTreeWidgetItem *)));
  connect(ui->checkBoxShowAll, SIGNAL(stateChanged(int)),
          SLOT(OnCheckBoxShowAllLabels(int)));
  ...
  connect(ui->lineEditColor, SIGNAL(textChanged(QString)),
          SLOT(OnEditColorTextChanged()));
  connect(ui->colorpicker, SIGNAL(colorChanged(QColor)),
          SLOT(OnColorChanged(QColor)), Qt::QueuedConnection);
  connect(ui->treeWidgetAvailableLabels,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
  ...
  connect(ui->toolButtonFromCTab, SIGNAL(clicked(bool)),
          SLOT(OnButtonFromCTab()));
...
void WindowEditAnnotation::OnActiveSurfaceChanged(Layer *layer) {
  ...
  if (m_layerSurface) {
    connect(m_layerSurface, SIGNAL(ActiveAnnotationChanged(int)),
            SLOT(UpdateUI()), Qt::QueuedConnection);
    connect(m_layerSurface, SIGNAL(ActiveAnnotationChanged(int)),
            SLOT(PopulateAvailableColorTable()), Qt::QueuedConnection);
    connect(this, SIGNAL(LabelClicked(int)), m_layerSurface,
            SLOT(SetHighlightedLabelOnAnnotation(int)), Qt::QueuedConnection);
    UpdateUI();
  }
}
...
void WindowEditAnnotation::UpdateUI(int nIndex) {
  if (m_layerSurface) {
    ...
    ui->toolButtonFromCTab->setEnabled(
        ui->treeWidgetAvailableLabels->currentItem() != NULL);
...
void WindowEditAnnotation::UpdateActions() {
  ...
  ui->toolButtonFromCTab->setEnabled(
      !ui->treeWidgetAvailableLabels->selectedItems().isEmpty() &&
    ...
    for (int i = 0; i < nTotalCount; i++) {
      ...
      if (nValid) {
        ...
        QTreeWidgetItem *item =
            new QTreeWidgetItem(ui->treeWidgetAvailableLabels);
  ...
  if (ct) // && (bForce || ct != m_ctab))
  {
    ...
    for (int n = 0; n < annotIds.size(); n++) {
      ...
      if (bUnassigned || nValid) {
        QTreeWidgetItem *item =
            new QTreeWidgetItem(ui->treeWidgetExistingLabels);
        if (bUnassigned) {
    ...
    if (nSel >= 0) {
      ui->treeWidgetExistingLabels->setCurrentItem(
          ui->treeWidgetExistingLabels->topLevelItem(nSel));
...
void WindowEditAnnotation::OnExistingLabelItemChanged(QTreeWidgetItem *item) {
  ...
  if (m_layerSurface && m_layerSurface->GetActiveAnnotation()) {
    ...
    m_layerSurface->GetActiveAnnotation()->SetSelectLabel(
        nVal, item->checkState(0) == Qt::Checked);
    ui->checkBoxShowAll->setCheckState(
        m_layerSurface->GetActiveAnnotation()->GetVisibleLabels().isEmpty()
            ? Qt::Unchecked
            : Qt::PartiallyChecked);
  }
...
void WindowEditAnnotation::OnSurfaceVertexClicked(LayerSurface *surf) {
  if (surf == m_layerSurface) {
    if (surf->GetActiveAnnotation() &&
        m_layerSurface->GetCurrentVertex() >= 0) {
      int n = surf->GetActiveAnnotation()->GetIndexAtVertex(
          m_layerSurface->GetCurrentVertex());
      for (int i = 0; i < ui->treeWidgetExistingLabels->topLevelItemCount();
           i++) {
        QTreeWidgetItem *item = ui->treeWidgetExistingLabels->topLevelItem(i);
...
void WindowEditAnnotation::OnEditColorTextChanged() {
  QStringList list =
      ui->lineEditColor->text().split(" ", QString::SkipEmptyParts);
  if (list.size() == 3) {
    int rgb[3];
    for (int i = 0; i < 3; i++) {
      bool ok;
  ...
  if (item) {
    ...
    if (name.isEmpty()) {
      QMessageBox::warning(this, "Error",
                           "Please provide an unique name for annotation");
      return;
    }
    for (int n = 0; n < ui->treeWidgetExistingLabels->topLevelItemCount();
         n++) {
      QTreeWidgetItem *aitem = ui->treeWidgetExistingLabels->topLevelItem(n);
      if (aitem != item &&
          aitem->data(0, ANNOT_INDEX_ROLE).toInt() == index_new) {
        if (QMessageBox::question(this, "Confirm",
                                  "This label already exists in the "
                                  "annotation. Do you want to combine them?") !=
            QMessageBox::Yes)
          return;
        else
    ...
    if (m_layerSurface && m_layerSurface->GetActiveAnnotation()) {
      if (index_new != index_old)
        m_layerSurface->GetActiveAnnotation()->ReassignNewLabel(
            index_old, index_new, name, color);
      else
        m_layerSurface->GetActiveAnnotation()->UpdateLabelInfo(index_old, name,
                                                               color);
...
void WindowEditAnnotation::OnColorChanged(const QColor &color) {
  ...
  if (item) {
    for (int i = 0; i < ui->treeWidgetExistingLabels->topLevelItemCount();
         i++) {
      QTreeWidgetItem *other_item =
          ui->treeWidgetExistingLabels->topLevelItem(i);
      if (item != other_item &&
          other_item->data(0, ANNOT_COLOR_ROLE).value<QColor>() == color) {
        QMessageBox::information(this, "Existing Color",
                                 "This color already exists in the color "
                                 "table. Please choose another one.");
        ui->colorpicker->blockSignals(true);
        ui->colorpicker->setCurrentColor(
            item->data(0, ANNOT_COLOR_ROLE).value<QColor>());
        ui->colorpicker->blockSignals(false);
        return;
      }
    }
    ...
    ui->lineEditColor->setText(QString("%1 %2 %3")
                                   .arg(color.red())
                                   .arg(color.green())
                                   .arg(color.blue()));
...
void WindowEditAnnotation::OnButtonDelete() {
  ...
  QTreeWidgetItem *item = ui->treeWidgetExistingLabels->currentItem();
  if (item) {
    m_layerSurface->GetActiveAnnotation()->DeleteLabel(
        item->data(0, ANNOT_INDEX_ROLE).toInt());
    ...
    if (n < ui->treeWidgetExistingLabels->topLevelItemCount())
      ui->treeWidgetExistingLabels->setCurrentItem(
          ui->treeWidgetExistingLabels->topLevelItem(n));
    else if (ui->treeWidgetExistingLabels->topLevelItemCount() > 0)
      ui->treeWidgetExistingLabels->setCurrentItem(
          ui->treeWidgetExistingLabels->topLevelItem(0));
