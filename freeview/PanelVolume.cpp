...
PanelVolume::PanelVolume(QWidget *parent)
    : PanelLayer("MRI", parent), ui(new Ui::PanelVolume), m_curCTAB(NULL),
      m_bShowExistingLabelsOnly(true), m_nCurrentVoxelIndex(-1) {
  ui->setupUi(this);
  ui->treeWidgetColorTable->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(ui->treeWidgetColorTable, SIGNAL(customContextMenuRequested(QPoint)),
  ...
  m_widgetlistGrayScale << ui->checkBoxClearBackground << ui->labelWindow
                        << ui->labelLevel << ui->lineEditWindow
                        << ui->lineEditLevel << ui->sliderWindow
                        << ui->sliderLevel;

  m_widgetlistHeatScale << ui->sliderMid << ui->sliderOffset << ui->lineEditMid
                        << ui->lineEditOffset << ui->checkBoxClearHigher
                        << ui->checkBoxTruncate << ui->checkBoxSetMidToMin
                        << ui->checkBoxInvert << ui->labelMid
                        << ui->labelOffset;

  m_widgetlistGenericColorMap << ui->lineEditMin << ui->lineEditMax
                              << ui->sliderMin << ui->sliderMax << ui->labelMin
                              << ui->labelMax << ui->checkBoxPercentile
                              << ui->pushButtonResetWindowLevel;

  m_widgetlistLUT << ui->treeWidgetColorTable << ui->labelLookUpTable
                  << ui->comboBoxLookUpTable << ui->checkBoxShowExistingLabels
                  << ui->checkBoxSelectAllLabels;

  m_widgetlistDirectionCode << ui->comboBoxDirectionCode
                            << ui->labelDirectionCode;

  m_widgetlistFrame << ui->sliderFrame << ui->spinBoxFrame << ui->labelFrame
  ...
  m_widgetlistVector << ui->labelInversion << ui->comboBoxInversion
                     << ui->labelRenderObject << ui->comboBoxRenderObject
                     << ui->checkBoxNormalizeVectors << ui->lineEditVectorScale
                     << ui->labelVectorScale << ui->labelVectorLineWidth
                     << ui->lineEditVectorLineWidth << ui->labelVectorSkip
                     << ui->spinBoxVectorSkip << ui->labelVectorNormThreshold
  ...
  m_widgetlistContour << ui->sliderContourThresholdLow
                      ...
                      << ui->checkBoxContourExtractAll << ui->colorPickerContour
                      << ui->labelContourThresholdHigh
                      << ui->labelContourThresholdLow << ui->labelContourColor
                      << ui->sliderContourSmoothIteration
                      << ui->lineEditContourSmoothIteration
                      << ui->labelSmoothIteration << ui->pushButtonContourSave
                      << ui->checkBoxShowLabelContour
                      << ui->checkBoxUpsampleContour
                      << ui->checkBoxVoxelizedContour
                      << ui->labelContourSpaceHolder;

  m_widgetlistContourNormal
      << ui->sliderContourThresholdLow << ui->sliderContourThresholdHigh
      << ui->lineEditContourThresholdLow << ui->lineEditContourThresholdHigh
      << ui->checkBoxUseColorMap << ui->checkBoxContourExtractAll
      << ui->colorPickerContour << ui->labelContourThresholdHigh
      << ui->labelContourThresholdLow << ui->labelContourColor
      << ui->pushButtonContourSave;

  m_widgetlistEditable << ui->labelBrushValue << ui->lineEditBrushValue;

  m_widgetlistNormalDisplay << ui->labelOpacity << ui->sliderOpacity
  ...
  combo << m_widgetlistGrayScale << m_widgetlistHeatScale
        ...
        << ui->labelColorMap << ui->comboBoxColorMap << ui->checkBoxShowContour
        << ui->checkBoxShowOutline;
  ...
  connect(ui->actionLockLayer, SIGNAL(toggled(bool)), this,
          SLOT(OnLockLayer(bool)));
  connect(ui->actionMoveLayerUp, SIGNAL(triggered()), lc, SLOT(MoveLayerUp()));
  connect(ui->actionMoveLayerDown, SIGNAL(triggered()), lc,
          SLOT(MoveLayerDown()));

  connect(mainwnd, SIGNAL(MainViewChanged(int)), this, SLOT(UpdateWidgets()),
          Qt::QueuedConnection);
}

PanelVolume::~PanelVolume() { delete ui; }
...
void PanelVolume::ConnectLayer(Layer *layer_in) {
  ...
  connect(p, SIGNAL(PropertyChanged()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
  connect(p, SIGNAL(OpacityChanged(double)), this, SLOT(UpdateOpacity(double)),
          Qt::UniqueConnection);
  connect(ui->doubleSpinBoxOpacity, SIGNAL(valueChanged(double)), p,
          SLOT(SetOpacity(double)));
  connect(ui->checkBoxSmooth, SIGNAL(stateChanged(int)), p,
          SLOT(SetTextureSmoothing(int)));
  connect(ui->checkBoxShowContour, SIGNAL(clicked(bool)), p,
          SLOT(SetShowAsContour(bool)));
  connect(ui->checkBoxShowLabelContour, SIGNAL(clicked(bool)), p,
          SLOT(SetShowAsLabelContour(bool)));
  connect(ui->sliderFrame, SIGNAL(valueChanged(int)), layer,
          SLOT(SetActiveFrame(int)));
  connect(ui->spinBoxFrame, SIGNAL(valueChanged(int)), layer,
          SLOT(SetActiveFrame(int)));
  connect(ui->checkBoxDisplayVector, SIGNAL(toggled(bool)), this,
          SLOT(OnCheckBoxSetDisplayVector(bool)));
  connect(ui->checkBoxDisplayTensor, SIGNAL(toggled(bool)), this,
          SLOT(OnCheckBoxSetDisplayTensor(bool)));
  connect(ui->checkBoxDisplayRGB, SIGNAL(toggled(bool)), this,
          SLOT(OnCheckBoxSetDisplayRGB(bool)));
  connect(ui->checkBoxNormalizeVectors, SIGNAL(toggled(bool)), this,
          SLOT(OnCheckBoxSetNormalizeVector(bool)));
  connect(ui->comboBoxRenderObject, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetVectorRepresentation(int)));
  connect(ui->comboBoxInversion, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetVectorInversion(int)));
  connect(ui->comboBoxProjectionMapType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(OnComboProjectionMapType(int)));
  connect(ui->checkBoxSetMidToMin, SIGNAL(toggled(bool)), this,
          SLOT(OnCheckBoxSetAutoMid(bool)));
  if (layer->IsTypeOf("DTI"))
    connect(ui->comboBoxDirectionCode, SIGNAL(currentIndexChanged(int)),
            qobject_cast<LayerDTI *>(layer)->GetProperty(),
            SLOT(SetDirectionCode(int)));
  connect(layer, SIGNAL(ActiveFrameChanged(int)), this, SLOT(UpdateWidgets()));
  connect(layer, SIGNAL(ActiveFrameChanged(int)), this,
          SLOT(OnActiveFrameChanged(int)));
  ...
  connect(ui->checkBoxClearBackground, SIGNAL(toggled(bool)), p,
          SLOT(SetClearBackground(bool)));
  connect(ui->lineEditClearBackgroundValue, SIGNAL(textChanged(QString)), this,
          SLOT(OnLineEditClearBackgroundValue(QString)));
  connect(ui->checkBoxClearHigher, SIGNAL(toggled(bool)), p,
          SLOT(SetHeatScaleClearHigh(bool)));
  connect(ui->checkBoxTruncate, SIGNAL(toggled(bool)), p,
          SLOT(SetHeatScaleTruncate(bool)));
  connect(ui->checkBoxInvert, SIGNAL(toggled(bool)), p,
          SLOT(SetHeatScaleInvert(bool)));
  connect(ui->checkBoxShowOutline, SIGNAL(toggled(bool)), p,
          SLOT(SetShowLabelOutline(bool)));
  connect(ui->checkBoxContourExtractAll, SIGNAL(toggled(bool)), p,
          SLOT(SetContourExtractAllRegions(bool)));
  connect(ui->checkBoxUseColorMap, SIGNAL(toggled(bool)), p,
          SLOT(SetContourUseImageColorMap(bool)));
  connect(ui->checkBoxShowInfo, SIGNAL(toggled(bool)), p,
          SLOT(SetShowInfo(bool)));
  connect(ui->colorPickerContour, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetContourColor(QColor)));
  connect(ui->checkBoxUpsampleContour, SIGNAL(toggled(bool)), p,
          SLOT(SetContourUpsample(bool)));
  connect(ui->checkBoxRememberFrame, SIGNAL(toggled(bool)), p,
          SLOT(SetRememberFrameSettings(bool)));
  connect(ui->checkBoxAutoAdjustFrameLevel, SIGNAL(toggled(bool)), p,
          SLOT(SetAutoAdjustFrameLevel(bool)));
  connect(ui->lineEditProjectionMapRange, SIGNAL(returnPressed()), this,
          SLOT(OnLineEditProjectionMapRangeChanged()));
  connect(layer, SIGNAL(IsoSurfaceUpdating()), ui->widgetBusyIndicator,
          SLOT(show()));
  connect(layer, SIGNAL(IsoSurfaceUpdated()), ui->widgetBusyIndicator,
          SLOT(hide()));
  connect(ui->pushButtonResetWindowLevel, SIGNAL(clicked(bool)),
          SLOT(OnButtonResetWindowLevel()));
  connect(ui->spinBoxVectorSkip, SIGNAL(valueChanged(int)), p,
          SLOT(SetVectorSkip(int)));

  ui->colorLabelBrushValue->installEventFilter(this);
}

bool PanelVolume::eventFilter(QObject *watched, QEvent *event) {
  if (watched == ui->colorLabelBrushValue &&
...
void PanelVolume::DoIdle() {
  ...
  ui->actionMoveLayerUp->setEnabled(layer && m_layerCollection &&
                                    m_layerCollection->GetLayerIndex(layer) >
                                        0);
  ui->actionMoveLayerDown->setEnabled(
      layer && m_layerCollection &&
      m_layerCollection->GetLayerIndex(layer) <
          m_layerCollection->GetNumberOfLayers() - 1);
...
void PanelVolume::DoUpdateWidgets() {
  ...
  for (int i = 0; i < this->allWidgets.size(); i++) {
    if (allWidgets[i] != ui->toolbar &&
        ...
        allWidgets[i] != ui->toolbar2 &&
        allWidgets[i]->parentWidget() != ui->toolbar2) {
  ...
  if (layer) {
    ...
    ui->checkBoxClearBackground->setChecked(
        layer->GetProperty()->GetClearBackground());
    ChangeLineEditNumber(ui->lineEditClearBackgroundValue,
                         layer->GetProperty()->GetClearBackgroundValue());
    if (layer->IsTypeOf("DTI")) {
      ui->lineEditFileName->setText(
          MyUtils::Win32PathProof(((LayerDTI *)layer)->GetVectorFileName()));
    } else {
      ui->lineEditFileName->setText(
    ...
    ui->checkBoxUpsample->setChecked(
        layer->GetProperty()->GetUpSampleMethod() != 0);
    ui->checkBoxUpsample->setEnabled(layer->GetProperty()->GetColorMap() !=
                                     LayerPropertyMRI::LUT);
    ...
    ui->comboBoxLookUpTable->setCurrentIndex(nSel >= 0 ? nSel
                                                       : m_luts->GetCount());
    ...
    ui->sliderMin->setValue(
        (int)((dMinTh - range_min) / (range_max - range_min) * 100));
    ui->sliderMax->setValue(
        (int)((dMaxTh - range_min) / (range_max - range_min) * 100));

    ui->sliderMid->setValue(
        (int)((dHeatMidTh - range_min) / (range_max - range_min) * 100));
    ui->sliderOffset->setValue(
        (int)((dHeatOffset + dmaxvalue) / (dmaxvalue + dmaxvalue) * 100));
    ChangeLineEditNumber(ui->lineEditMid, dHeatMidTh);
    ChangeLineEditNumber(ui->lineEditOffset, dHeatOffset);
    ui->checkBoxClearHigher->setChecked(
        layer->GetProperty()->GetHeatScaleClearHigh());
    ui->checkBoxTruncate->setChecked(
        layer->GetProperty()->GetHeatScaleTruncate());
    ui->checkBoxInvert->setChecked(layer->GetProperty()->GetHeatScaleInvert());
    ui->checkBoxSetMidToMin->setChecked(
        layer->GetProperty()->GetHeatScaleAutoMid());

    ui->comboBoxColorMap->clear();
    ui->comboBoxColorMap->addItem("Grayscale", LayerPropertyMRI::Grayscale);
    if (layer->IsTypeOf("DTI")) {
      ui->comboBoxColorMap->addItem("Direction-coded",
                                    LayerPropertyMRI::DirectionCoded);
      ui->comboBoxDirectionCode->setCurrentIndex(
          ((LayerDTI *)layer)->GetProperty()->GetDirectionCode());
    } else {
    ...
    if (nFrames > 1) {
      ...
      ui->checkBoxRememberFrame->setChecked(
          layer->GetProperty()->GetRememberFrameSettings());
      ui->checkBoxAutoAdjustFrameLevel->setChecked(
          layer->GetProperty()->GetAutoAdjustFrameLevel());
    }
    ui->sliderFrame->setValue(layer->GetActiveFrame());
    ChangeSpinBoxValue(ui->spinBoxFrame, layer->GetActiveFrame());

    ui->checkBoxShowContour->setChecked(
        layer->GetProperty()->GetShowAsContour());
    ui->sliderContourThresholdLow->setValue(
        (int)((layer->GetProperty()->GetContourMinThreshold() - dminvalue) /
              (dmaxvalue - dminvalue) * 100));
    ui->sliderContourThresholdHigh->setValue(
        (int)((layer->GetProperty()->GetContourMaxThreshold() - dminvalue) /
              (dmaxvalue - dminvalue) * 100));
    ChangeLineEditNumber(ui->lineEditContourThresholdLow,
                         layer->GetProperty()->GetContourMinThreshold());
    ChangeLineEditNumber(ui->lineEditContourThresholdHigh,
                         layer->GetProperty()->GetContourMaxThreshold());
    ui->checkBoxUseColorMap->setChecked(
        layer->GetProperty()->GetContourUseImageColorMap());
    ui->checkBoxUpsampleContour->setChecked(
        layer->GetProperty()->GetContourUpsample());
    ui->checkBoxContourExtractAll->setChecked(
        layer->GetProperty()->GetContourExtractAllRegions());
    ui->sliderContourSmoothIteration->setValue(
        layer->GetProperty()->GetContourSmoothIterations());
    ChangeLineEditNumber(ui->lineEditContourSmoothIteration,
                         layer->GetProperty()->GetContourSmoothIterations());

    ui->colorPickerContour->setEnabled(
        !layer->GetProperty()->GetContourUseImageColorMap());
    double rgb[3];
    layer->GetProperty()->GetContourColor(rgb);
    ui->colorPickerContour->setCurrentColor(
    ...
    if (layer->GetProperty()->GetDisplayVector()) {
      ...
      ui->comboBoxRenderObject->setCurrentIndex(
          layer->GetProperty()->GetVectorRepresentation());
      ui->comboBoxInversion->setCurrentIndex(
          layer->GetProperty()->GetVectorInversion());
      ...
      ui->comboBoxRenderObject->setCurrentIndex(
          layer->GetProperty()->GetTensorRepresentation());
      ui->comboBoxInversion->setCurrentIndex(
          layer->GetProperty()->GetTensorInversion());
    }
    ui->checkBoxNormalizeVectors->setChecked(
        layer->GetProperty()->GetNormalizeVector());
    ChangeLineEditNumber(ui->lineEditVectorScale,
                         layer->GetProperty()->GetVectorDisplayScale());
    ChangeLineEditNumber(ui->lineEditVectorLineWidth,
                         layer->GetProperty()->GetVectorLineWidth());
    ChangeLineEditNumber(ui->lineEditVectorNormThreshold,
                         layer->GetProperty()->GetVectorNormThreshold());

    ui->checkBoxShowInfo->setChecked(layer->GetProperty()->GetShowInfo());

    ui->checkBoxShowOutline->setChecked(
        layer->GetProperty()->GetShowLabelOutline());
    ui->widgetProjectionMapType->setVisible(!layer->IsTypeOf("DTI"));
    ui->widgetProjectionMapSettings->setVisible(
        ui->widgetProjectionMapType->isVisible() &&
        layer->GetProperty()->GetShowProjectionMap());
    ui->comboBoxProjectionMapType->setCurrentIndex(
        layer->GetProperty()->GetProjectionMapType());
    int nRange[2];
    ...
    ui->lineEditProjectionMapRange->setText(
        QString("%1, %2").arg(nRange[0]).arg(nRange[1]));
    ui->checkBoxShowOutline->setVisible(
        ui->comboBoxProjectionMapType->currentIndex() == 0 &&
        !layer->IsTypeOf("DTI"));

    //    ui->m_choiceUpSampleMethod->SetSelection(
    //    layer->GetProperty()->GetUpSampleMethod() );
    ui->checkBoxShowExistingLabels->setEnabled(
        !layer->GetAvailableLabels().isEmpty());
      ...
      for (int i = 0; i < surfs.size(); i++) {
        ...
        for (int j = 0; j < surf->GetNumberOfOverlays(); j++) {
          ...
          if (overlay->GetNumberOfFrames() == layer->GetNumberOfFrames()) {
            ui->comboBoxCorrelationSurface->addItem(
                surf->GetName(), QVariant::fromValue((QObject *)surf));
  ...
  bool bNormalDisplay =
      (layer && !layer->GetProperty()->GetDisplayVector() &&
  ...
  if (layer && layer->IsTypeOf("VolumeTrack")) {
    ...
    ShowWidgets(m_widgetlistGrayScale,
                bNormalDisplay && nColorMap == LayerPropertyMRI::Grayscale);
    ShowWidgets(m_widgetlistHeatScale,
                bNormalDisplay && nColorMap == LayerPropertyMRI::Heat);
    ShowWidgets(m_widgetlistGenericColorMap,
                (bNormalDisplay && nColorMap != LayerPropertyMRI::LUT &&
                 nColorMap != LayerPropertyMRI::DirectionCoded &&
                 nColorMap != LayerPropertyMRI::Binary) ||
                    (layer && layer->IsTypeOf("DTI") &&
                     !layer->GetProperty()->GetDisplayVector() &&
                     !bDisplayRGB));
    ShowWidgets(m_widgetlistLUT,
                bNormalDisplay && nColorMap == LayerPropertyMRI::LUT);
    ShowWidgets(m_widgetlistDirectionCode,
                bNormalDisplay &&
                    nColorMap == LayerPropertyMRI::DirectionCoded);
    ShowWidgets(m_widgetlistEditable, bNormalDisplay && layer->IsEditable());
    ShowWidgets(m_widgetlistFrame, layer && !layer->IsTypeOf("DTI") &&
                                       layer->GetNumberOfFrames() > 1 &&
                                       !layer->GetCorrelationSurface() &&
                                       layer->GetDataType() != MRI_RGB);
    ui->lineEditClearBackgroundValue->setVisible(
        layer && ui->checkBoxClearBackground->isVisible() &&
        ui->checkBoxClearBackground->isChecked());
    if (bDisplayRGB) {
    ...
    ui->labelCorrelationSurface->setVisible(
        layer && layer->GetNumberOfFrames() > 1 &&
        ui->comboBoxCorrelationSurface->count() > 1);
    ui->comboBoxCorrelationSurface->setVisible(
        ui->labelCorrelationSurface->isVisible());
    ...
    ui->spinBoxFrame->setEnabled(layer &&
                                 !layer->GetProperty()->GetDisplayVector() &&
                                 !layer->GetProperty()->GetDisplayTensor());
    ui->checkBoxDisplayVector->setVisible(
        layer && (layer->IsTypeOf("DTI") || layer->GetNumberOfFrames() == 3 ||
                  layer->GetNumberOfFrames() == 6));
    ui->checkBoxDisplayVector->setChecked(
        layer && layer->GetProperty()->GetDisplayVector());
    ui->checkBoxDisplayTensor->setVisible(layer &&
                                          layer->GetNumberOfFrames() == 9);
    ui->checkBoxDisplayTensor->setChecked(
        layer && layer->GetProperty()->GetDisplayTensor());
    ui->checkBoxDisplayRGB->setVisible(layer &&
                                       layer->GetNumberOfFrames() == 3);
    ui->checkBoxDisplayRGB->setChecked(layer &&
                                       layer->GetProperty()->GetDisplayRGB());
    ShowWidgets(m_widgetlistVector, ui->checkBoxDisplayVector->isChecked() ||
                                        ui->checkBoxDisplayTensor->isChecked());
    ShowWidgets(m_widgetlistContour,
                ui->checkBoxShowContour->isChecked() && layer &&
                    !layer->GetProperty()->GetDisplayRGB());

    ui->checkBoxShowContour->setVisible(
        bNormalDisplay && layer &&
        !layer->GetProperty()->GetShowProjectionMap());
    ui->checkBoxShowContour->setEnabled(
        nColorMap != LayerPropertyMRI::LUT ||
        ui->checkBoxShowExistingLabels->isEnabled());
  ...
  ui->labelBrushValueWarning->hide();
  ui->colorLabelBrushValue->setVisible(nColorMap == LayerPropertyMRI::LUT ||
                                       nColorMap == LayerPropertyMRI::Binary);

  if (layer) {
    if (nColorMap == LayerPropertyMRI::LUT &&
        !layer->GetProperty()->IsValueInColorTable(layer->GetFillValue())) {
...
void PanelVolume::UpdateTrackVolumeThreshold() {
  ...
  if (item && layer) {
    ...
    ui->sliderTrackVolumeThresholdLow->setValue(
        (int)((layer->GetThreshold(nLabel) - fMin) / (fMax - fMin) * 100));
    ChangeLineEditNumber(ui->lineEditTrackVolumeThresholdLow,
                         layer->GetThreshold(nLabel));
  ...
  if (ct && (bForce || ct != m_curCTAB)) {
    ...
    for (int i = 0; i < nTotalCount; i++) {
      ...
      if (nValid) {
        ...
        item->setCheckState(0, selectedLabels.contains(i) ? Qt::Checked
                                                          : Qt::Unchecked);
        if (i > 0) {
    ...
    if (nSel >= 0) {
      ui->treeWidgetColorTable->setCurrentItem(
          ui->treeWidgetColorTable->topLevelItem(nSel));
    }
    ...
    if (!labels.isEmpty() && m_bShowExistingLabelsOnly) {
      for (int i = 0; i < ui->treeWidgetColorTable->topLevelItemCount(); i++) {
        QTreeWidgetItem *item = ui->treeWidgetColorTable->topLevelItem(i);
        item->setHidden(
            !labels.contains(item->data(0, Qt::UserRole + 1).toInt()));
      }
    }
  }
}
  ...
  foreach (LayerMRI *layer, layers) {
    ...
    case LayerPropertyMRI::Grayscale:
      if (layer->GetProperty()->GetUsePercentile())
        layer->GetProperty()->SetMinGrayscaleWindow(
            layer->GetHistoValueFromPercentile(nVal / 100.0));
      else
        layer->GetProperty()->SetMinGrayscaleWindow(
            nVal / 100.0 * (fScaleMax - fScaleMin) + fScaleMin);
      break;
    case LayerPropertyMRI::Heat:
      if (layer->GetProperty()->GetUsePercentile())
        layer->GetProperty()->SetHeatScaleMinThreshold(
            layer->GetHistoValueFromPercentile(nVal / 100.0), bAutoMidToMin);
      else
        layer->GetProperty()->SetHeatScaleMinThreshold(
            nVal / 100.0 * (fMax - fMin) + fMin, bAutoMidToMin);
      break;
    default:
      if (layer->GetProperty()->GetUsePercentile())
        layer->GetProperty()->SetMinGenericThreshold(
            layer->GetHistoValueFromPercentile(nVal / 100.0));
      else
        layer->GetProperty()->SetMinGenericThreshold(
            nVal / 100.0 * (fScaleMax - fScaleMin) + fScaleMin);
...
void PanelVolume::OnSliderMid(int nVal) {
  ...
  foreach (LayerMRI *layer, layers) {
    if (layer->GetProperty()->GetUsePercentile())
      layer->GetProperty()->SetHeatScaleMidThreshold(
          layer->GetHistoValueFromPercentile(nVal / 100.0));
    else
      layer->GetProperty()->SetHeatScaleMidThreshold(
...
void PanelVolume::OnSliderMax(int nVal) {
  ...
  foreach (LayerMRI *layer, layers) {
    ...
    case LayerPropertyMRI::Grayscale:
      if (layer->GetProperty()->GetUsePercentile())
        layer->GetProperty()->SetMaxGrayscaleWindow(
            layer->GetHistoValueFromPercentile(nVal / 100.0));
      else
        layer->GetProperty()->SetMaxGrayscaleWindow(
            nVal / 100.0 * (fScaleMax - fScaleMin) + fScaleMin);
    ...
    case LayerPropertyMRI::Heat:
      if (layer->GetProperty()->GetUsePercentile())
        layer->GetProperty()->SetHeatScaleMaxThreshold(
            layer->GetHistoValueFromPercentile(nVal / 100.0), bAutoMidToMin);
      else
        layer->GetProperty()->SetHeatScaleMaxThreshold(
            nVal / 100.0 * (fMax - fMin) + fMin, bAutoMidToMin);
    ...
    default:
      if (layer->GetProperty()->GetUsePercentile())
        layer->GetProperty()->SetMaxGenericThreshold(
            layer->GetHistoValueFromPercentile(nVal / 100.0));
      else
        layer->GetProperty()->SetMaxGenericThreshold(
            nVal / 100.0 * (fScaleMax - fScaleMin) + fScaleMin);
...
void PanelVolume::OnLineEditMin(const QString &text) {
  ...
  foreach (LayerMRI *layer, layers) {
    ...
    if (layer && bOK) {
      ...
      case LayerPropertyMRI::Heat:
        layer->GetProperty()->SetHeatScaleMinThreshold(
            dVal, MainWindow::GetMainWindow()
                      ->GetSetting("AutoSetMidToMin")
                      .toBool());
        break;
      default:
...
void PanelVolume::OnCheckBoxSetAutoMid(bool b) {
  ...
  foreach (LayerMRI *layer, layers) {
    if (layer && layer->GetProperty()->GetHeatScaleAutoMid() != b) {
      layer->GetProperty()->SetHeatScaleAutoMid(
          b,
          MainWindow::GetMainWindow()->GetSetting("AutoSetMidToMin").toBool());
    }
  }
...
void PanelVolume::OnContourValueChanged() {
  ...
  if (ui->checkBoxShowLabelContour->isChecked()) {
    ...
    if (bOK) {
      nSmooth =
          ui->lineEditContourSmoothIteration->text().trimmed().toInt(&bOK);
    }
    QList<LayerMRI *> layers = GetSelectedLayers<LayerMRI *>();
    foreach (LayerMRI *layer, layers) {
      if (layer && bOK) {
...
void PanelVolume::OnComboMask(int sel) {
  LayerMRI *mask = qobject_cast<LayerMRI *>(
      ui->comboBoxMask->itemData(sel).value<QObject *>());
  LayerMRI *layer = GetCurrentLayer<LayerMRI *>();
  if (layer) {
...
void PanelVolume::OnComboCorrelationSurface(int nSel) {
  LayerSurface *surf = qobject_cast<LayerSurface *>(
      ui->comboBoxCorrelationSurface->itemData(nSel).value<QObject *>());
  LayerMRI *layer = GetCurrentLayer<LayerMRI *>();
  if (layer) {
...
void PanelVolume::OnLockLayer(bool b) {
  QList<LayerMRI *> layers = GetSelectedLayers<LayerMRI *>();
  foreach (LayerMRI *layer, layers) { layer->Lock(b); }
}

void PanelVolume::OnCheckBoxSetDisplayVector(bool b) {
  QList<LayerMRI *> layers = GetSelectedLayers<LayerMRI *>();
  foreach (LayerMRI *layer, layers) {
    if (layer->GetNumberOfFrames() == 3 || layer->GetNumberOfFrames() == 6 ||
        layer->GetEndType() == "DTI")
...
void PanelVolume::OnCheckBoxSetDisplayRGB(bool b) {
  QList<LayerMRI *> layers = GetSelectedLayers<LayerMRI *>();
  foreach (LayerMRI *layer, layers) { layer->GetProperty()->SetDisplayRGB(b); }
}
...
void PanelVolume::OnLineEditProjectionMapRangeChanged() {
  QList<LayerMRI *> layers = GetSelectedLayers<LayerMRI *>();
  foreach (LayerMRI *layer, layers) {
    QStringList list = ui->lineEditProjectionMapRange->text().trimmed().split(
        ",", QString::SkipEmptyParts);
    if (list.size() < 2)
      list = ui->lineEditProjectionMapRange->text().trimmed().split(
          " ", QString::SkipEmptyParts);
    ...
    if (list.size() > 1) {
      ...
      if (nRange[1] >= nRange[0])
        layer->GetProperty()->SetProjectionMapRange(nPlane, nRange[0],
                                                    nRange[1]);
    }
  }
  UpdateWidgets();
...
void PanelVolume::OnCheckBoxSelectAllLabels(int nState) {
  ...
  for (int i = 0; i < ui->treeWidgetColorTable->topLevelItemCount(); i++) {
    ...
    item->setCheckState(0,
                        nState == Qt::Unchecked ? Qt::Unchecked : Qt::Checked);
  }
  ui->treeWidgetColorTable->blockSignals(false);
      ...
      if (layer) {
        ...
        if (layer->GetLayerLabelCenter(val, pos)) {
          ...
          connect(act, SIGNAL(triggered()),
                  SLOT(OnColorTableItemDoubleClicked()));
          menu.addAction(act);
...
void PanelVolume::OnGoToNextPoint() {
  if (m_nCurrentVoxelIndex < 0 ||
      m_nCurrentVoxelIndex >= m_voxelList.size() / 3) {
    OnGoToFirstPoint();
  ...
  if (m_nCurrentVoxelIndex >= 0 && !m_voxelList.isEmpty()) {
    ...
    QToolTip::showText(pt,
                       QString(" %1 / %2 ")
                           .arg(m_nCurrentVoxelIndex + 1)
                           .arg(m_voxelList.size() / 3),
                       NULL, QRect(), 1500);
#else
    QToolTip::showText(pt,
                       QString(" %1 / %2 ")
