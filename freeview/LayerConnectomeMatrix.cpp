...
LayerConnectomeMatrix::LayerConnectomeMatrix(LayerMRI *ref, QObject *parent)
    : Layer(parent), m_mriRef(ref), m_mriParcel(NULL), m_cmat(NULL),
      m_ctab(NULL), m_bToAllLabels(false), m_bVisible(true) {
...
void LayerConnectomeMatrix::SetColorTable(COLOR_TABLE *ctab) { m_ctab = ctab; }
...
void LayerConnectomeMatrix::BuildLabelActors() {
  for (int i = 0; i < m_listLabels.size(); i++) {
    ...
    MyVTKUtils::BuildContourActor(m_mriParcel->GetImageData(), m_listLabels[i],
                                  m_listLabels[i], actor, 0, 0, true);
...
QList<int> LayerConnectomeMatrix::GetLabelList() { return m_listLabels; }
...
bool LayerConnectomeMatrix::IsVisible() { return m_bVisible; }
    ...
    for (int j = 0; j < m_listFromLabelIndices.size(); j++) {
      ...
      if (m_bToAllLabels || m_listToLabelIndices.contains(i)) {
        ...
        if (label) {
          ...
          for (int n = 0; n < label->n_points; n++) {
            points->InsertNextPoint(label->lv[n].x, label->lv[n].y,
                                    label->lv[n].z);
...
void LayerConnectomeMatrix::UpdateOpacity() {
  for (int i = 0; i < m_actorLabels.size(); i++) {
    if (m_listFromLabelIndices.contains(i))
      m_actorLabels[i]->GetProperty()->SetOpacity(
          GetProperty()->GetFromLabelOpacity());
    else
      m_actorLabels[i]->GetProperty()->SetOpacity(
          GetProperty()->GetToLabelOpacity());
  }
  emit ActorUpdated();
}
