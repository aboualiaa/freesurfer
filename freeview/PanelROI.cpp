...
PanelROI::PanelROI(QWidget *parent)
  ...
  if (mainwnd) {
    ...
    ui->toolbar->insertAction(ui->actionMoveLayerUp,
                              mainwnd->ui->actionLoadROI);
    ui->toolbar->insertAction(ui->actionMoveLayerUp,
                              mainwnd->ui->actionCloseROI);
    ui->toolbar->insertAction(ui->actionMoveLayerUp,
                              mainwnd->ui->actionSaveROI);
    ui->toolbar->insertSeparator(ui->actionMoveLayerUp);

    m_listWidgetsHeatscale << ui->labelHeatscaleRange
                           << ui->lineEditHeatscaleMin
                           << ui->lineEditHeatscaleMax;
    ...
    connect(mainwnd->GetLayerCollection("Surface"),
            SIGNAL(LayerRemoved(Layer *)), this, SLOT(UpdateWidgets()),
            Qt::QueuedConnection);
  }
}

PanelROI::~PanelROI() { delete ui; }
...
void PanelROI::ConnectLayer(Layer *layer_in) {
  ...
  connect(ui->colorPickerColor, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetColor(QColor)));
  connect(ui->comboBoxColorMap, SIGNAL(currentIndexChanged(int)), p,
...
void PanelROI::DoUpdateWidgets() {
  ...
  */
  ...
  if (layer) {
    ...
    ui->colorPickerColor->setCurrentColor(
    ...
    ChangeLineEditNumber(ui->lineEditThreshold,
                         layer->GetProperty()->GetThreshold());

    ui->comboBoxColorMap->setCurrentIndex(layer->GetProperty()->GetColorCode());
    ShowWidgets(m_listWidgetsHeatscale, layer->GetProperty()->GetColorCode() ==
                                            LayerPropertyROI::Heatscale);
    ui->colorPickerColor->setVisible(layer->GetProperty()->GetColorCode() ==
                                     LayerPropertyROI::SolidColor);

    ChangeLineEditNumber(ui->lineEditHeatscaleMin,
                         layer->GetProperty()->GetHeatscaleMin());
    ChangeLineEditNumber(ui->lineEditHeatscaleMax,
  ...
  if (layer) {
    for (int i = 0; i < surfs.size(); i++) {
      ui->comboBoxMappedSurface->addItem(
          surfs[i]->GetName(), QVariant::fromValue((QObject *)surfs[i]));
...
void PanelROI::OnComboMappedSurface(int nIndex) {
  LayerSurface *surf = qobject_cast<LayerSurface *>(
      ui->comboBoxMappedSurface->itemData(nIndex).value<QObject *>());
  LayerROI *layer = GetCurrentLayer<LayerROI *>();
  if (layer) {
