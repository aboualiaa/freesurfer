...
bool SurfaceSpline::Load(const QString &filename) {
  ...
  if (!m_mriSurf) {
    MRIS *mris =
        qobject_cast<LayerSurface *>(parent())->GetSourceSurface()->GetMRIS();
    m_mriSurf = MRIallocHeader(mris->vg.width, mris->vg.height, mris->vg.depth,
                               MRI_UCHAR, 1);
...
void SurfaceSpline::SetColor(const QColor &c) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    m_actor2DSpheres[i]->GetProperty()->SetColor(c2.redF(), c2.greenF(),
                                                 c2.blueF());
  }
  emit SplineChanged();
}
