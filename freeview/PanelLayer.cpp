...
PanelLayer::PanelLayer(const QString &layerType, QWidget *parent)
    : QScrollArea(parent), UIUpdateHelper(), m_bToUpdate(false),
      m_currentLayer(NULL), treeWidgetLayers(NULL), m_layerType(layerType) {
  m_layerCollection =
      MainWindow::GetMainWindow()->GetLayerCollection(layerType);
  connect(m_layerCollection, SIGNAL(destroyed()), this,
          SLOT(ResetLayerCollection()));
...
void PanelLayer::UpdateWidgets() { m_bToUpdate = true; }

void PanelLayer::OnIdle() { DoIdle(); }
...
void PanelLayer::InitializeLayerTreeWidget(QTreeWidget *treeWidget) {
  ...
  Qt::UniqueConnection ); connect( cl, SIGNAL(LayerRemoved(Layer*)), this,
  SLOT(OnLayerRemoved(Layer*)), Qt::UniqueConnection ); connect( cl,
  SIGNAL(LayerMoved(Layer*)), this, SLOT(OnLayerMoved(Layer*)),
  Qt::UniqueConnection ); connect( cl, SIGNAL(ActiveLayerChanged(Layer*)), this,
  SLOT(OnActiveLayerChanged(Layer*)), Qt::UniqueConnection ); connect( cl,
  SIGNAL(LayerCycled(Layer*)), this, SLOT(UpdateWidgets()), Qt::UniqueConnection
  ); connect( cl, SIGNAL(LayerNameChanged()), this, SLOT(OnLayerNameChanged()),
  Qt::UniqueConnection ); connect( cl, SIGNAL(LayerVisibilityChanged()), this,
  SLOT(UpdateWidgets()), Qt::UniqueConnection ); connect( treeWidget,
  SIGNAL(currentItemChanged(QTreeWidgetItem*,QTreeWidgetItem*)), this,
  SLOT(OnCurrentItemChanged(QTreeWidgetItem*)), Qt::UniqueConnection ); connect(
  treeWidget, SIGNAL(itemSelectionChanged()), this,
           SLOT(OnItemSelectionChanged()));
  connect( treeWidget,
  SIGNAL(currentItemChanged(QTreeWidgetItem*,QTreeWidgetItem*)), this,
  SLOT(UpdateWidgets())); connect( treeWidget,
  SIGNAL(itemChanged(QTreeWidgetItem*,int)), this,
  SLOT(OnItemChanged(QTreeWidgetItem*)) ); connect( treeWidget,
...
void PanelLayer::DisconnectAllLayers() {
  LayerCollection *lc =
      MainWindow::GetMainWindow()->GetLayerCollection(m_layerType);
