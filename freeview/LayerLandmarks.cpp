...
LayerLandmarks::LayerLandmarks(QObject *parent)
    : LayerEditable(parent), m_dRadius(3.0) {
...
void LayerLandmarks::SetMRIRef(LayerMRI *mri) { m_mriRef = mri; }
...
void LayerLandmarks::SetLandmarkColor(int n, const QColor &color) {
  ...
  m_landmarks[n].actorSphere->GetProperty()->SetColor(
      color.redF(), color.greenF(), color.blueF());
  for (int i = 0; i < 3; i++)
    m_landmarks[n].actorSlice[i]->GetProperty()->SetColor(
        color.redF(), color.greenF(), color.blueF());
  emit ActorUpdated();
}
...
void LayerLandmarks::UpdateActors(bool bBuild3D) {
  ...
  for (int i = 0; i < m_landmarks.size(); i++) {
    ...
    for (int j = 0; j < 3; j++) {
      vtkSmartPointer<vtkPolyDataMapper> mapper =
          vtkSmartPointer<vtkPolyDataMapper>::New();
      if (m_landmarks[i].valid &&
          fabs(m_dSlicePosition[j] - m_landmarks[i].pos[j]) <
              voxel_size[j] / 2) {
...
void LayerLandmarks::DoTransform(double *mat, int sample_method) {
  ...
  for (int i = 0; i < m_landmarks.size(); i++) {
    double pos[4] = {m_landmarks[i].pos[0], m_landmarks[i].pos[1],
                     m_landmarks[i].pos[2], 1};
