...
LayerFCD::LayerFCD(LayerMRI *layerMRI, QObject *parent)
  ...
  connect(p, SIGNAL(ThicknessThresholdChanged(double)), this,
          SLOT(Recompute()));
  ...
  connect(m_mri_norm, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  connect(m_mri_flair, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  connect(m_mri_t2, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  connect(m_mri_aseg, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  connect(m_mri_difference, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);

  m_surf_lh = new LayerSurface(NULL);
  m_surf_rh = new LayerSurface(NULL);
  connect(m_surf_lh, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  connect(m_surf_rh, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  m_surf_lh_pial = new LayerSurface(NULL);
  m_surf_rh_pial = new LayerSurface(NULL);
  connect(m_surf_lh_pial, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);
  connect(m_surf_rh_pial, SIGNAL(destroyed()), this, SLOT(OnLayerDestroyed()),
          Qt::UniqueConnection);

  m_surf_lh_sphere_d1 = new LayerSurface(NULL);
  m_surf_rh_sphere_d1 = new LayerSurface(NULL);
  connect(m_surf_lh_sphere_d1, SIGNAL(destroyed()), this,
          SLOT(OnLayerDestroyed()), Qt::UniqueConnection);
  connect(m_surf_rh_sphere_d1, SIGNAL(destroyed()), this,
          SLOT(OnLayerDestroyed()), Qt::UniqueConnection);
...
bool LayerFCD::IsBusy() { return m_worker->isRunning(); }
...
bool LayerFCD::LoadFromFile() {
  ...
  try {
    m_fcd = ::FCDloadData(
        m_sSubjectDir.toLatin1().data(), m_sSubject.toLatin1().data(),
...
void LayerFCD::DoCompute(bool resetProgress) {
  if (m_fcd) {
    ...
    try {
      ::FCDcomputeThicknessLabels(m_fcd, GetProperty()->GetThicknessThreshold(),
  ...
  for (int cnt = 0; cnt < m_fcd->nlabels; cnt++) {
    ...
    for (int i = 0; i < label->n_points; i++) {
      ...
      if (label->coords == LABEL_COORDS_VOXEL) {
        MRIvoxelToWorld(ref_vol->GetMRI(), pos[0], pos[1], pos[2], pos, pos + 1,
                        pos + 2);
      ...
      if (n[0] >= 0 && n[0] < dim[0] && n[1] >= 0 && n[1] < dim[1] &&
          n[2] >= 0 && n[2] < dim[2]) {
        MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, n[0], n[1], n[2],
                                          0, scalar_type, label->lv[i].vno);
      }
    }
  }
}
...
bool LayerFCD::IsVisible() { return m_sliceActor2D[0]->GetVisibility() > 0; }
...
void LayerFCD::SetLabelVisible(int nIndex, bool visible) {
  ...
  for (int i = 0; i < label->n_points; i++) {
    ...
    if (n[0] >= 0 && n[0] < dim[0] && n[1] >= 0 && n[1] < dim[1] && n[2] >= 0 &&
        n[2] < dim[2]) {
      MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, n[0], n[1], n[2], 0,
                                        scalar_type,
                                        visible ? label->lv[i].vno : 0);
    }
  }
  for (int i = 0; i < 3; i++) {
    mReslice[i]->Modified();
...
QThread *LayerFCD::GetWorkerThread() { return m_worker; }
