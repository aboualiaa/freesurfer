...
LayerROI::LayerROI(LayerMRI *layerMRI, QObject *parent)
  ...
  connect(mProperty, SIGNAL(OpacityChanged(double)), this,
          SLOT(UpdateOpacity()));
  connect(mProperty, SIGNAL(ThresholdChanged(double)), this,
          SLOT(UpdateThreshold()));

  connect(this, SIGNAL(BaseVoxelEdited(QVector<int>, bool)),
          SLOT(OnBaseVoxelEdited(QVector<int>, bool)));
  UpdateProperties();
}
...
bool LayerROI::IsVisible() { return m_sliceActor2D[0]->GetVisibility() > 0; }
...
bool LayerROI::SaveROI() {
  ...
  //        m_label->UpdateLabelFromImage( m_imageData,
  //        m_layerSource->GetSourceVolume() );
  ...
  for (int i = 0; i < dim[0]; i++) {
    for (int j = 0; j < dim[1]; j++) {
      for (int k = 0; k < dim[2]; k++) {
        if (ptr[k * dim[0] * dim[1] + j * dim[0] + i] >
            GetProperty()->GetThreshold()) {
          cnt++;
          //          indices << i << j << k;
...
void LayerROI::SetMappedSurface(LayerSurface *s) {
  ...
  if (s) {
    ...
    m_label->Initialize(m_layerSource->GetSourceVolume(), s->GetSourceSurface(),
                        s->IsInflated() ? WHITE_VERTICES : CURRENT_VERTICES);
...
void LayerROI::OnUpdateLabelRequested() {
  ...
  if (m_layerMappedSurface) {
    ...
    //        m_layerSource->GetSourceVolume(), coords);
...
void LayerROI::MapLabelColorData(unsigned char *colordata, int nVertexCount) {
  ...
  for (int i = 0; i < label->n_points; i++) {
    ...
    if (vno < nVertexCount && vno >= 0 && !label->lv[i].deleted) {
      ...
      colordata[vno * 4] =
          (int)(colordata[vno * 4] * (1 - opacity) + rgb[0] * 255 * opacity);
      colordata[vno * 4 + 1] = (int)(colordata[vno * 4 + 1] * (1 - opacity) +
                                     rgb[1] * 255 * opacity);
      colordata[vno * 4 + 2] = (int)(colordata[vno * 4 + 2] * (1 - opacity) +
                                     rgb[2] * 255 * opacity);
    }
  }
}
    ...
    for (int i = 0; i < voxel_list.size(); i += 3) {
      ...
      if (m_layerMappedSurface)
        coords = m_layerMappedSurface->IsInflated() ? WHITE_VERTICES
                                                    : CURRENT_VERTICES;
      m_label->EditVoxel(n[0], n[1], n[2], coords, bAdd, m_nVertexCache,
                         m_nVertexCache ? (&cnt) : NULL);
  ...
  if (m_layerMappedSurface) {
    ...
    if (m_nBrushRadius == 1)
      ...
      for (int i = 0; i < list_nvo_in.size(); i++)
        list_nvo << list_nvo_in[i]
                 << m_layerMappedSurface->GetVertexNeighbors(list_nvo_in[i]);
    }
    ...
    for (int i = 0; i < list_nvo.size(); i++) {
      ...
      if (bAdd)
        ...
        ret =
            qMax(ret, ::LabelDeleteVertex(m_label->GetRawLabel(), nvo, coords));
    }
    if (ret >= 0) {
      OnLabelDataUpdated();
    }
  ...
  for (size_t i = 0; i < dim[0]; i++) {
    for (size_t j = 0; j < dim[1]; j++) {
      for (size_t k = 0; k < dim[2]; k++) {
        double val1 = MyVTKUtils::GetImageDataComponent(ptr1, dim, 1, i, j, k,
                                                        0, scalar_type);
        double val2 = MyVTKUtils::GetImageDataComponent(ptr2, dim, 1, i, j, k,
                                                        0, scalar_type);
        if (val1 == 0 && val2 >= 1) {
          listAdd << i << j << k;
          MyVTKUtils::SetImageDataComponent(ptr_img, dim, 1, i, j, k, 0,
                                            scalar_type, m_fFillValue);
        } else if (val1 == 2 && val2 <= 1) {
          listRemove << i << j << k;
          MyVTKUtils::SetImageDataComponent(ptr_img, dim, 1, i, j, k, 0,
                                            scalar_type, m_fBlankValue);
        }
      }
    }
  }
...
void LayerROI::Dilate(int nTimes) {
  SaveForUndo();
  if (m_layerMappedSurface) {
    ::LabelDilate(m_label->GetRawLabel(),
                  m_layerMappedSurface->GetSourceSurface()->GetMRIS(), nTimes,
                  m_layerMappedSurface->IsInflated() ? WHITE_VERTICES
                                                     : CURRENT_VERTICES);
  } else {
    vtkSmartPointer<vtkImageData> data1 = GetThresholdedMaskImage();
    vtkSmartPointer<vtkImageData> data2 = data1;
    for (int i = 0; i < nTimes; i++) {
      vtkSmartPointer<vtkImageDilateErode3D> filter =
...
void LayerROI::Erode(int nTimes) {
  SaveForUndo();
  if (m_layerMappedSurface) {
    ::LabelErode(m_label->GetRawLabel(),
                 m_layerMappedSurface->GetSourceSurface()->GetMRIS(), nTimes);
  } else {
    vtkSmartPointer<vtkImageData> data1 = GetThresholdedMaskImage();
    vtkSmartPointer<vtkImageData> data2 = data1;
...
void LayerROI::Resample() {
  ...
  if (m_layerMappedSurface) {
    ...
    LABEL *label = ::LabelSampleToSurface(
        m_layerMappedSurface->GetSourceSurface()->GetMRIS(), old_label,
...
bool LayerROI::HasUndo() { return m_label->HasUndo(); }

bool LayerROI::HasRedo() { return m_label->HasRedo(); }
...
void LayerROI::Undo() {
  ...
  SetModified();
  if (m_layerMappedSurface) {
    m_label->Initialize(m_layerSource->GetSourceVolume(),
                        m_layerMappedSurface->GetSourceSurface(),
                        m_layerMappedSurface->IsInflated() ? WHITE_VERTICES
                                                           : CURRENT_VERTICES);
    m_layerMappedSurface->UpdateOverlay(true, true);
  }
...
LABEL *LayerROI::GetRawLabel() { return m_label->GetRawLabel(); }
