...
public:
  ...
  FSVolume *GetSourceVolume() { return m_volumeSource; }

  FSVolume *GetRefVolume() { return m_volumeRef; }

  void SetRefVolume(FSVolume *ref) { m_volumeRef = ref; }

  bool SaveVolume();

  void SetResampleToRAS(bool bResample);

  bool GetResampleToRAS() { return m_bResampleToRAS; }
  ...
  bool GetVoxelValueRange(const double *pt0, const double *pt1, int nPlane,
                          double *range_out);

  bool GetVoxelStatsRectangle(const double *pt0, const double *pt1, int nPlane,
                              double *mean_out, double *sd_out = NULL,
                              int *cnt_out = NULL);

  bool GetVoxelsOnLine(const double *pt0, const double *pt1, int nPlane,
                       int *&indice_out, double *&value_out, int *cnt_out);

  bool GetVoxelStats(QVector<int> &indices, double *mean_out,
                     double *sd_out = NULL);

  bool GetVoxelStatsByTargetRAS(QVector<float> &coords, double *mean_out,
                                double *sd_out = NULL);
  ...
  void GetCurrentLabelStats(int nPlane, float *label_out, int *count_out,
                            float *area_out, LayerMRI *underlying_mri = NULL,
                            double *mean_out = NULL, double *sd_out = NULL);
  ...
  SurfaceRegion *GetCurrentSurfaceRegion() { return m_currentSurfaceRegion; }

  bool DeleteCurrentSurfaceRegion();

  int GetNumberOfSurfaceRegions() { return m_surfaceRegions.size(); }
  ...
  void GetLabelStats(LayerMRI *label, int nPlane, std::vector<int> &id,
                     std::vector<int> &number, std::vector<double> &mean,
  ...
  void SetWriteResampled(bool bResample) { m_bWriteResampled = bResample; }

  bool GetWriteResampled() { return m_bWriteResampled; }
  ...
  QList<int> GetAvailableLabels() { return m_nAvailableLabels; }
  ...
  double GetMaskThreshold() { return m_dMaskThreshold; }

  LayerMRI *GetMaskLayer() { return m_layerMask; }

  void SetCorrelationSurface(LayerSurface *surf);

  LayerSurface *GetCorrelationSurface() { return m_correlationSurface; }
  ...
  void Threshold(int frame, LayerMRI *src, int src_frame, double th_low,
  ...
  bool GeodesicSegmentation(LayerMRI *seeds, double lambda, int wsize,
                            double max_dist, double smoothing_std,
                            LayerMRI *mask, double max_foreground_dist = 0);
...
protected:
  ...
  void BuildTensorGlyph(vtkImageData *imagedata, int i, int j, int k,
                        double *pt, double scale, vtkPolyData *sourcepolydata,
                        vtkUnsignedCharArray *scalars, vtkPolyDataAlgorithm *a);
