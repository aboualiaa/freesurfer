...
WindowGroupPlot::WindowGroupPlot(QWidget *parent)
  ...
  connect(ui->comboBoxViewBy, SIGNAL(currentIndexChanged(int)),
          SLOT(OnComboViewBy(int)));
  connect(ui->comboBoxConfigClass, SIGNAL(currentIndexChanged(int)),
          SLOT(OnComboConfigClass(int)));
  connect(ui->comboBoxConfigShape, SIGNAL(currentIndexChanged(QString)),
          SLOT(OnComboConfigShape(QString)));
  connect(ui->widgetConfigColor, SIGNAL(colorChanged(QColor)),
          SLOT(OnConfigColor(QColor)));
  connect(ui->listWidget,
          SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)),
          SLOT(OnCurrentItemChanged(QListWidgetItem *)));
  connect(ui->widgetPlot, SIGNAL(CurrentDataIndexChanged(int)),
...
void WindowGroupPlot::OnComboConfigClass(int nIndex) {
  UpdateCurrentConfig(m_fsgd->m_classes[nIndex].marker,
                      m_fsgd->m_classes[nIndex].color);
}
