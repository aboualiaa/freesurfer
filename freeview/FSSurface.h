...
public:
  ...
  bool IsCurvatureLoaded() { return m_bCurvatureLoaded; }

  double GetCurvatureValue(int nVertex);

  bool SetActiveSurface(int nIndex);

  int GetActiveSurface() { return m_nActiveSurface; }

  bool HasVectorSet() { return m_vertexVectors.size() > 0; }

  int GetNumberOfVectorSets() { return m_vertexVectors.size(); }

  QString GetVectorSetName(int nSet);

  int GetActiveVector() { return m_nActiveVector; }

  bool SetActiveVector(int nIndex);

  vtkPolyData *GetPolyData() { return m_polydata; }

  vtkPolyData *GetTargetPolyData() { return m_polydataTarget; }

  vtkPolyData *GetVectorPolyData() { return m_polydataVector; }

  vtkPolyData *GetVector2DPolyData(int n) { return m_polydataVector2D[n]; }

  vtkPolyData *GetVertexPolyData() { return m_polydataVertices; }

  void GetVectorAtVertex(int nVertex, double *vec_out, int nVector = -1);

  vtkPolyData *GetWireframePolyData() { return m_polydataWireframes; }
  ...
  void Reposition(FSVolume *volume, int target_vnos, double target_val,
                  int nsize, double sigma, int flags = 0);

  void Reposition(FSVolume *volume, int target_vnos, double *coord, int nsize,
  ...
  bool HasValidVolumeGeometry() { return m_bValidVolumeGeometry; }

  void ResetVolumeRef() { m_volumeRef = NULL; }

  double GetMaxSegmentLength() { return m_dMaxSegmentLength; }
...
protected:
  ...
  bool ProjectVectorPoint2D(double *pt_in, vtkPoints *contour_pts,
                            vtkCellArray *contour_lines, double *pt_out);
