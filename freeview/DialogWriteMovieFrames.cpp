...
DialogWriteMovieFrames::DialogWriteMovieFrames(QWidget *parent)
  ...
  connect(MainWindow::GetMainWindow(), SIGNAL(MainViewChanged(int)), this,
          SLOT(UpdateUI()));
  m_timer.setInterval(1000);
  connect(&m_timer, SIGNAL(timeout()), this, SLOT(OnTimeOut()));
  QSettings s;
  ui->lineEditOutputLocation->setText(
      s.value("MovieFrames/OutputLocation").toString());
}

DialogWriteMovieFrames::~DialogWriteMovieFrames() {
  QSettings s;
  s.setValue("MovieFrames/OutputLocation",
             ui->lineEditOutputLocation->text().trimmed());
  delete ui;
}
...
void DialogWriteMovieFrames::closeEvent(QCloseEvent *e) {
  if (m_timer.isActive()) {
    if (QMessageBox::question(
            this, "Close",
            "Still writing movie frames. Do you want to abort?") !=
        QMessageBox::Yes) {
      e->ignore();
      return;
    }
...
void DialogWriteMovieFrames::UpdateUI(bool bUpdateNumbers) {
  if (bUpdateNumbers) {
    ...
    if (mri) {
      ui->comboBoxFlyThrough->setItemData(
          1, (mri->GetNumberOfFrames() > 1 ? 33 : 0), Qt::UserRole - 1);
    }
  }
  QList<QWidget *> widgets = findChildren<QWidget *>();
  foreach (QWidget *w, widgets) {
    if (w != ui->pushButtonAbort && w != ui->pushButtonClose &&
        w != ui->pushButtonWrite)
...
void DialogWriteMovieFrames::OnOpen() {
  ...
  if (!strg.isEmpty()) {
    ...
    ui->lineEditOutputLocation->setCursorPosition(
        ui->lineEditOutputLocation->text().length());
  }
}
...
void DialogWriteMovieFrames::OnWrite() {
  ...
  if (m_strOutputDir.isEmpty()) {
    QMessageBox::warning(this, "Error",
                         "Please enter output directory to save movie frames.");
    return;
  }
  ...
  {
    m_nTotalSteps =
        (int)((ui->spinBoxEnd->value() - ui->spinBoxStart->value()) /
                  m_dStepSize +
              1);
  }
...
void DialogWriteMovieFrames::OnTimeOut() {
  ...
  if (nIndex == 0) // slice
  {
    ...
    if (m_nStepCount + 1 >= m_nTotalSteps ||
        !((RenderView2D *)m_view)->SetSliceNumber(nStart + m_nStepSize)) {
      OnAbort();
    }
  } else if (nIndex == 1) // frame
  {
