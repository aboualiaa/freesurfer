...
DialogLabelStats::DialogLabelStats(QWidget *parent)
    : QWidget(parent), ui(new Ui::DialogLabelStats) {
  ui->setupUi(this);
  setWindowFlags(Qt::Dialog);
}

DialogLabelStats::~DialogLabelStats() { delete ui; }
...
void DialogLabelStats::UpdateStats() {
  ...
  foreach (Layer *layer, layers) {
    ...
    if (!label &&
        mri_layer->GetProperty()->GetColorMap() == LayerPropertyMRI::LUT)
      label = mri_layer;
    if (!mri &&
        mri_layer->GetProperty()->GetColorMap() != LayerPropertyMRI::LUT)
      mri = mri_layer;
  ...
  if (mri) {
    if (mri->GetNumberOfFrames() > 1)
      ui->labelVolume->setText(QString("%1 (frame %2)")
                                   .arg(mri->GetName())
                                   .arg(mri->GetActiveFrame()));
    else
