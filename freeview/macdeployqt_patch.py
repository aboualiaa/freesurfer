...
def main():
    ...
    find_macho_files_cmd = (
        "( cd "
        + target_dir
        + " && find . -type f -exec file {} \; | grep -i mach | sed 's;:.*;;' | tr -s "
        + "'\\n'"
        + " "
        + "' '"
        + ")"
    ...
    if file_string:
        ...
        print (
            "POSTPROCESSING OF MACDEPLOYQT OUTPUT by script %s"
        ) % os.path.abspath(whoami)
