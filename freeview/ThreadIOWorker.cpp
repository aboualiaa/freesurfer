...
void ThreadIOWorker::run() {
  connect(qApp, SIGNAL(GlobalProgress(int)), this, SIGNAL(Progress(int)),
          Qt::UniqueConnection);
  if (m_nJobType == JT_LoadVolume) {
    ...
    LayerConnectomeMatrix *layer =
        qobject_cast<LayerConnectomeMatrix *>(m_layer);
    if (!layer)
      return;
    ...
    if (!layer)
      return;
    if (!layer->Load(m_args["SubjectDir"].toString(),
                     m_args["Subject"].toString(),
                     m_args["Suffix"].toString())) {
      emit Error(m_layer, m_nJobType);
    } else {
