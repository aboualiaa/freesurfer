...
PanelFCD::PanelFCD(QWidget *parent)
    : PanelLayer("FCD", parent), ui(new Ui::PanelFCD) {
  ui->setupUi(this);
}

PanelFCD::~PanelFCD() { delete ui; }
...
void PanelFCD::ConnectLayer(Layer *layer_in) {
  ...
  connect(p, SIGNAL(PropertyChanged()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
  connect(layer, SIGNAL(LabelsChanged()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
  connect(layer, SIGNAL(StatusChanged()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
}
...
void PanelFCD::DoUpdateWidgets() {
  ...
  if (layer) {
    ...
    ui->sliderThreshold->setValue(
        (int)(layer->GetProperty()->GetThicknessThreshold() * 10));
    ...
    ChangeLineEditNumber(ui->lineEditThreshold,
                         layer->GetProperty()->GetThicknessThreshold());
    ChangeLineEditNumber(ui->lineEditMinArea,
                         layer->GetProperty()->GetMinArea());
  ...
  if (layer && layer->GetFCDData()) {
    ...
    for (int i = 0; i < fcd->nlabels; i++) {
      //   if (fcd->labels[i]->n_points > 0)
      {
        ...
        item->setText(0, QString("%1   %2mm")
                             .arg(QString(fcd->label_names[i]))
...
void PanelFCD::OnSliderThresholdReleased() {
  LayerFCD *layer = GetCurrentLayer<LayerFCD *>();
  if (layer)
    layer->GetProperty()->SetThicknessThreshold(ui->sliderThreshold->value() /
                                                10.0);
}
...
void PanelFCD::OnLabelSelectionChanged() {
  int n = ui->treeWidgetLabels->indexOfTopLevelItem(
      ui->treeWidgetLabels->currentItem());
  LayerFCD *layer = GetCurrentLayer<LayerFCD *>();
  if (layer && n >= 0) {
    double pos[3];
...
void PanelFCD::OnButtonSaveFCDLabels() {
  ...
  if (layer) {
    QString dir = QFileDialog::getExistingDirectory(
        this, tr("Select a directory to save FCD labels to"),
        QDir::currentPath());
    if (!dir.isNull()) {
