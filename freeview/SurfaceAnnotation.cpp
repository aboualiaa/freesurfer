...
SurfaceAnnotation::SurfaceAnnotation(LayerSurface *surf)
    : QObject(surf), m_nIndices(NULL), m_nOutlineIndices(NULL),
      m_nCenterVertices(NULL), m_lut(NULL), m_surface(surf),
      m_bShowOutline(false), m_dOpacity(1.0), m_nHighlightedLabel(-1),
      m_data(NULL) {
...
SurfaceAnnotation::~SurfaceAnnotation() { Reset(); }
...
void SurfaceAnnotation::UpdateData() {
  ...
  for (int i = 0; i < nAnnots; i++) {
    if (vcount[i] > 0) {
      ...
      int nVertex =
          m_surface->GetSourceSurface()->FindVertexAtSurfaceRAS(pts[i], NULL);
        ...
        if (label->lv[n].vno >= 0) {
          ...
          for (int m = 0; m < vt->vnum; m++) {
            if (mris->vertices[vt->v[m]].marked == 0) {
              m_nOutlineIndices[label->lv[n].vno] =
                  m_nIndices[label->lv[n].vno];
              break;
            }
          }
        }
...
QString SurfaceAnnotation::GetName() { return m_strName; }

void SurfaceAnnotation::SetName(const QString &name) { m_strName = name; }
  ...
  for (int i = 0; i < m_nIndexSize; i++) {
    if (indices[i] >= 0 && m_listVisibleLabels.contains(indices[i])) {
      if (indices[i] >= UNASSIGNED_ANNOT_BASE) {
        ...
        colordata[i * 4]      = (int)(colordata[i * 4] * (1 - m_dOpacity) +
                                 nl.color.red() * m_dOpacity);
        colordata[i * 4 + 1]  = (int)(colordata[i * 4 + 1] * (1 - m_dOpacity) +
                                     nl.color.green() * m_dOpacity);
        colordata[i * 4 + 2]  = (int)(colordata[i * 4 + 2] * (1 - m_dOpacity) +
                                     nl.color.blue() * m_dOpacity);
      } else {
        char name[128] = {0};
        if (CTABcopyName(m_lut, indices[i], name, 128) == 0 &&
            QString(name).toLower() != "unknown") {
          if (CTABrgbAtIndexi(m_lut, indices[i], c, c + 1, c + 2) ==
              0) // no error & no black color
          {
            colordata[i * 4] =
                (int)(colordata[i * 4] * (1 - m_dOpacity) + c[0] * m_dOpacity);
            colordata[i * 4 + 1] =
                (int)(colordata[i * 4 + 1] * (1 - m_dOpacity) +
                      c[1] * m_dOpacity);
            colordata[i * 4 + 2] =
                (int)(colordata[i * 4 + 2] * (1 - m_dOpacity) +
                      c[2] * m_dOpacity);
          }
        }
      }
...
void SurfaceAnnotation::EditLabel(const QVector<int> &verts, int fill_index,
...
void SurfaceAnnotation::UpdateColorTable(int nIndex, const QString &name,
                                         const QColor &color) {
  CTE *cte;
...
void SurfaceAnnotation::UpdateLabelInfo(int i, const QString &name,
                                        const QColor &color) {
  SaveForUndo();
...
bool SurfaceAnnotation::HasUndo() { return m_bufferUndo.size() > 0; }

bool SurfaceAnnotation::HasRedo() { return m_bufferRedo.size() > 0; }
...
void SurfaceAnnotation::RestoreFromUndoRedoBuffer(
    const AnnotUndoRedoBufferItem &item) {
