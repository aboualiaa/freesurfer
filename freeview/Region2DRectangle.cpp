...
void Region2DRectangle::Update() {
  ...
  m_view->WorldToViewport(m_dPt[0][0], m_dPt[0][1], m_dPt[0][2], pt0[0], pt0[1],
                          pt0[2]);
  m_view->WorldToViewport(m_dPt[1][0], m_dPt[1][1], m_dPt[1][2], pt1[0], pt1[1],
                          pt1[2]);
  m_view->WorldToViewport(m_dPt[2][0], m_dPt[2][1], m_dPt[2][2], pt2[0], pt2[1],
                          pt2[2]);
...
bool Region2DRectangle::Contains(int nX, int nY, int *nIndexOut) {
  ...
  double range[3][2];
  GetRange(range);
  if (pt[0] >= range[0][0] && pt[0] <= range[0][1] && pt[1] >= range[1][0] &&
      pt[1] <= range[1][1] && pt[2] >= range[2][0] && pt[2] <= range[2][1]) {
    return true;
  } else {
    return false;
  }
...
void Region2DRectangle::UpdatePoint(int nIndex, int x, int y) {
  ...
  */
  if (nIndex >= 0 && nIndex < 4) {
    ...
    int xp, yp;
    m_view->WorldToScreen(m_dPt[nDiag][0], m_dPt[nDiag][1], m_dPt[nDiag][2], xp,
                          yp);
    int nX1, nY1, nX2, nY2;
    if (nIndex == 0) {
      nX1 = x;
...
QString Region2DRectangle::DataToString() {
  ...
  for (int i = 0; i < 4; i++)
    strg +=
        QString("%1,%2,%3,").arg(m_dPt[i][0]).arg(m_dPt[i][1]).arg(m_dPt[i][2]);
  strg.chop(1);
  return strg;
}
