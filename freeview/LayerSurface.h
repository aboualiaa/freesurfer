...
public:
  ...
  bool LoadOverlayFromFile(const QString &filename, const QString &fn_reg,
  ...
  FSSurface *GetSourceSurface() { return m_surfaceSource; }

  void SetPatchFileName(const QString &fn) { m_sPatchFilename = fn; }

  void SetVectorFileName(const QString &fn) { m_sVectorFilename = fn; }

  void SetTargetFileName(const QString &fn) { m_sTargetFilename = fn; }
  ...
  QList<SurfaceOverlay *> GetOverlays() { return m_overlays; }
  ...
  int GetActiveLabelIndex() { return m_nActiveLabel; }
  ...
  LayerMRI *GetRefVolume() { return m_volumeRef; }

  int GetNumberOfSplines() { return m_splines.size(); }
  ...
  void RepositionSurface(LayerMRI *mri, int nVertex, double value, int size,
                         double sigma, int flags = 0);
  void RepositionSurface(LayerMRI *mri, int nVertex, double *pos, int size,
                         double sigma, int flags = 0);
  ...
  SurfaceROI *GetSurfaceROI() { return m_roi; }

  int GetCurrentVertex() { return m_nCurrentVertex; }
  ...
  bool GetVisibleIn3D() { return m_bVisibleIn3D; }

  void GetSmoothedVertexNormal(int nVertex, double *v_out);

  int GetActiveRGBMap() { return m_nActiveRGBMap; }

  void SetActiveRGBMap(int n);

  QStringList GetRGBMapNames();

  int GetNumberOfRGBMaps() { return m_rgbMaps.size(); }

  QString GetMappingSurfaceName() { return m_sMappingSurfaceName; }

  void SetNeighborhoodSize(int nSize);

  QVector<int> GetVertexNeighbors(int vno);

  bool IsContralateralReady() { return (m_surfaceContralateral != NULL); }

  bool IsContralateralPossible();

  LayerSurface *GetContralateralSurface() { return m_surfaceContralateral; }

  int GetContralateralVertex(int vno);

  int GetMouseVertex() { return m_nMouseVertex; }

  void SetMouseVertex(int n) { m_nMouseVertex = n; }
  ...
  void SetSphereFileName(const QString &fn) { m_sSphereFilename = fn; }

  SurfaceAnnotation *CreateNewAnnotation(const QString &ct_file,
                                         const QString &name = "");

public slots:
  void SetActiveSurface(int nSurfaceType);
  void UpdateOverlay(bool bAskRedraw = true, bool pre_cached = false);
  void SetLoadAllSurfaces(bool bLoadAll) { m_bLoadAll = bLoadAll; }

  void SetLoadSupSurfaces(const QStringList &names) { m_listSupFiles = names; }
  ...
  void SetHideIn3D(bool bHide) { SetVisibleIn3D(!bHide); }
  ...
  void UpdateOverlayLabels() { UpdateOverlay(true, true); }

  void SetContralateralLayer(LayerSurface *layer, LayerSurface *sphere1,
  ...
  SurfacePath *GetMarks() { return m_marks; }
