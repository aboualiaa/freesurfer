...
bool FSLabel::LabelRead(const QString &filename) {
  ...
  if (m_label->coords != LABEL_COORDS_SCANNER_RAS && m_mri_template) {
    ...
    qDebug() << "Label coordinates are converted to scanner ras for "
             << qPrintable(filename);
  }
...
void FSLabel::UpdateLabelFromImage(vtkImageData *rasImage, FSVolume *ref_vol) {
  if (m_label) {
    ::LabelFree(&m_label);
  }
...
void FSLabel::FillUnassignedVertices(FSSurface *surf, FSVolume *mri_template,
                                     int coords) {
  LABEL *l = LabelSampleToSurface(surf->GetMRIS(), m_label,
                                  mri_template->GetMRI(), coords);
  ::LabelFree(&m_label);
  m_label = l;
...
void FSLabel::Initialize(FSVolume *ref_vol, FSSurface *surf, int coords) {
  ...
  */
  ::LabelInit(m_label, ref_vol->GetMRI(), surf ? surf->GetMRIS() : NULL,
              coords);
}
...
void FSLabel::UpdateRASImage(vtkImageData *rasImage, FSVolume *ref_vol,
  ...
  for (int i = 0; i < m_label->n_points; i++) {
    if (!m_label->lv[i].deleted &&
        (m_label->lv[i].stat >= threshold || m_dStatsRange[0] <= 0)) {
      ...
      if (n[0] >= 0 && n[0] < dim[0] && n[1] >= 0 && n[1] < dim[1] &&
        ...
        MyVTKUtils::SetImageDataComponent(
            ptr, dim, 1, n[0], n[1], n[2], 0, scalar_type,
            m_label->lv[i].stat >= threshold ? m_label->lv[i].stat
                                             : (m_dStatsRange[0] - 1.0));
      } else {
...
void FSLabel::EditVoxel(int nx, int ny, int nz, int coords, bool bAdd,
                        int *vertices, int *pnum) {
  if (bAdd)
...
bool FSLabel::HasUndo() { return !m_undoBuffer.isEmpty(); }

bool FSLabel::HasRedo() { return !m_redoBuffer.isEmpty(); }
