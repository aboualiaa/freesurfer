...
DialogLoadVolume::DialogLoadVolume(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogLoadVolume) {
  ui->setupUi(this);
#ifdef Q_OS_MAC
...
DialogLoadVolume::~DialogLoadVolume() { delete ui; }
...
void DialogLoadVolume::SetRecentFiles(const QStringList &filenames) {
  ...
  if (!filenames.isEmpty()) {
    ...
    ui->comboBoxFilenames->lineEdit()->setCursorPosition(
        ui->comboBoxFilenames->currentText().size());
  }
}
...
void DialogLoadVolume::OnLUT(int nSel) {
  ...
  if (nSel >= luts->GetCount()) {
    QString filename = QFileDialog::getOpenFileName(
        this, "Load lookup table file", m_strLastDir, "LUT files (*)");
...
QStringList DialogLoadVolume::GetVolumeFileNames() {
  QStringList fns = ui->comboBoxFilenames->currentText().trimmed().split(
...
void DialogLoadVolume::OnOK() {
  ...
  if (ui->checkBoxRegistration->isChecked() && GetRegFileName().isEmpty()) {
    QMessageBox::warning(this, "Error",
                         "Please specify registration file to use.");
    return;
  }
  accept();
}
