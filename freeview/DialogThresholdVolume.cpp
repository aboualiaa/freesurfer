...
DialogThresholdVolume::DialogThresholdVolume(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogThresholdVolume) {
  ui->setupUi(this);
}

DialogThresholdVolume::~DialogThresholdVolume() { delete ui; }

void DialogThresholdVolume::OnButtonRun() {
  if (ValidateInputs()) {
    LayerMRI *src = qobject_cast<LayerMRI *>(
        ui->comboBoxSourceVolume
            ->itemData(ui->comboBoxSourceVolume->currentIndex())
            .value<QObject *>());
    LayerMRI *target = qobject_cast<LayerMRI *>(
        ui->comboBoxTargetVolume
    ...
    target->Threshold(ui->lineEditTargetFrame->text().trimmed().toInt(), src,
                      ...
                      ui->lineEditLowThreshold->text().trimmed().toDouble(),
                      th_high, ui->checkBoxReplaceIn->isChecked(),
                      ui->lineEditInValue->text().trimmed().toDouble(),
                      ui->checkBoxReplaceOut->isChecked(),
                      ui->lineEditOutValue->text().trimmed().toDouble());
  }
}
...
void DialogThresholdVolume::UpdateVolumes() {
  ...
  foreach (Layer *layer, layers) {
    ...
    ui->comboBoxSourceVolume->addItem(mri->GetName(),
                                      qVariantFromValue<QObject *>(mri));
    if (src == mri)
      ui->comboBoxSourceVolume->setCurrentIndex(
          ui->comboBoxSourceVolume->count() - 1);
    ui->comboBoxTargetVolume->addItem(mri->GetName(),
                                      qVariantFromValue<QObject *>(mri));
    if (target == mri)
      ui->comboBoxTargetVolume->setCurrentIndex(
          ui->comboBoxTargetVolume->count() - 1);
  }
}
...
bool DialogThresholdVolume::ValidateInputs() {
  ...
  if (!ok || source_frame < 0) {
    ...
    QMessageBox::warning(
        this, "Error",
        "Source frame number is out of bound. Please enter a valid number.");
    return false;
  }
  int target_frame = ui->lineEditTargetFrame->text().toInt(&ok);
  if (!ok || target_frame < 0) {
    QMessageBox::warning(this, "Error",
                         "Please enter a valid target frame number.");
    return false;
  } else if (target_frame >= target->GetNumberOfFrames()) {
    QMessageBox::warning(
        this, "Error",
        "Target frame number is out of bound. Please enter a valid number.");
    return false;
  }
