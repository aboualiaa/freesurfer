...
DialogVolumeSegmentation::DialogVolumeSegmentation(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogVolumeSegmentation) {
  ui->setupUi(this);
}

DialogVolumeSegmentation::~DialogVolumeSegmentation() { delete ui; }
...
bool DialogVolumeSegmentation::ValidateInput() {
  ...
  if (!ok || nval < 0) {
    QMessageBox::warning(this, "Error",
                         "Please enter a valid maximum label index");
    return false;
  }
  nval = ui->lineEditMinNumberOfVoxels->text().trimmed().toInt(&ok);
  if (!ok || nval < 0) {
    QMessageBox::warning(this, "Error",
                         "Please enter a valid minimum number of voxels");
    return false;
  }
  return true;
}
...
void DialogVolumeSegmentation::UpdateVolumes() {
  ...
  foreach (Layer *layer, layers) {
    ...
    ui->comboBoxVolume->addItem(mri->GetName(),
