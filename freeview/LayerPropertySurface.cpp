...
LayerPropertySurface::LayerPropertySurface(QObject *parent)
    : LayerProperty(parent), m_dOpacity(1), m_nEdgeThickness(2),
      m_nVectorPointSize(3), m_dThresholdMidPoint(0), m_dThresholdSlope(10),
      m_nCurvatureMap(CM_Threshold), m_nSurfaceRenderMode(SM_Surface),
      m_bShowVertices(false), m_nVertexPointSize(3), m_nMeshColorMap(0),
      m_surface(NULL), m_bShowOverlay(true), m_bShowAnnotation(true),
      m_bUseSurfaceColorOn2D(false), m_nZOrderLabel(3), m_nZOrderAnnotation(2),
      m_nZOrderOverlay(1) {
  ...
  connect(this, SIGNAL(EdgeThicknessChanged(int)), this,
  ...
  connect(this, SIGNAL(RenderModeChanged(int)), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(VectorPointSizeChanged(int)), this,
          SIGNAL(PropertyChanged()));
...
void LayerPropertySurface::RebuildCurvatureLUT() { SetColorMapChanged(); }
...
void LayerPropertySurface::RestoreFullSettings(const QVariantMap &map) {
  ...
  if (map.contains("Position_X")) {
    double pos[3] = {map["Position_X"].toDouble(), map["Position_Y"].toDouble(),
...
void LayerPropertySurface::BuildCurvatureLUT(vtkRGBAColorTransferFunction *lut,
                                             int nMap) {
  double hiRGB[3];
  ...
  switch (nMap) {
  case CM_Threshold:
    lut->AddRGBAPoint(-dSlope + m_dThresholdMidPoint, m_dRGBThresholdLow[0],
                      m_dRGBThresholdLow[1], m_dRGBThresholdLow[2], 1);
    lut->AddRGBAPoint(m_dThresholdMidPoint, m_dRGB[0], m_dRGB[1], m_dRGB[2], 1);
    lut->AddRGBAPoint(dSlope + m_dThresholdMidPoint, m_dRGBThresholdHigh[0],
                      m_dRGBThresholdHigh[1], m_dRGBThresholdHigh[2], 1);
    break;
  case CM_Binary:
    lut->AddRGBAPoint(m_dThresholdMidPoint, hiRGB[0], hiRGB[1], hiRGB[2], 1);
    lut->AddRGBAPoint(m_dThresholdMidPoint + 1e-8, m_dRGB[0], m_dRGB[1],
                      m_dRGB[2], 1);
    break;
  default:
...
double LayerPropertySurface::GetOpacity() const { return m_dOpacity; }
