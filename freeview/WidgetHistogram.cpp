...
bool WidgetHistogram::GetAutoRange() { return m_bAutoRange; }
...
int WidgetHistogram::GetNumberOfBins() { return m_nNumberOfBins; }
...
void WidgetHistogram::paintEvent(QPaintEvent *event) {
  ...
  if (m_nOutputData) {
    ...
    for (int i = 0; i < m_nNumberOfBins; i++) {
      ...
      painter.setBrush(
          QBrush(QColor(m_nColorTable[i * 4], m_nColorTable[i * 4 + 1],
                        m_nColorTable[i * 4 + 2])));
      y     = (int)(nOrigin[1] +
                nCavHeight * (1.0 - (double)m_nOutputData[i] / nMaxCnt));
      ...
      if (m_bUsePercentile) {
        ...
        if (i == 0 || i == m_nNumberOfBins - 1) {
          ...
          if (((int)val1) != ((int)val2) && x - nLastPos > 40) {
            ...
            bDraw    = true;
            dVal     = (int)val2;
            nPos     = (x2 - x) * (((int)val2) - val1) / (val2 - val1) + x -
                   (x2 - x) / 2.0;
          }
        }
        if (bDraw) {
    ...
    if (!m_bUsePercentile) {
      ...
      x = (int)(nOrigin[0] + (dMetricStart - m_dOutputRange[0]) /
                                 (m_dOutputRange[1] - m_dOutputRange[0]) *
                                 nCavWidth);
      while (x <= m_rectGraph.right() && dMetricStep > 0) {
        if (x >= m_rectGraph.left()) {
          painter.drawLine(x, m_rectGraph.bottom(), x,
                           m_rectGraph.bottom() + 4);
        }
        QString value_strg = QString::number(dMetricStart);
        QRect   tmp_rc =
        ...
        if (x - tmp_rc.width() / 2 > 0) {
          tmp_rc.moveCenter(
              QPoint(x, m_rectGraph.bottom() + 5 + tmp_rc.height() / 2));
          painter.drawText(tmp_rc, value_strg);
        }
        ...
        x = (int)(m_rectGraph.left() +
                  (dMetricStart - m_dOutputRange[0]) /
                      (m_dOutputRange[1] - m_dOutputRange[0]) * nCavWidth);
      }
    }
  }
}
...
void WidgetHistogram::DrawMarker(QPainter *p, LineMarker marker) {
  ...
  int x = (int)(m_rectGraph.left() +
                m_rectGraph.width() * (marker.position - m_dOutputRange[0]) /
                    (m_dOutputRange[1] - m_dOutputRange[0]));
  int y = m_rectGraph.y();
...
void WidgetHistogram::DrawMarkerThumb(QPainter *p, LineMarker marker, int x,
                                      int y) {
...
void WidgetHistogram::SetColorTableData(unsigned char *colortable,
                                        bool           bRefresh) {
...
QPolygon WidgetHistogram::MakeMarkerThumb(int x, int y) {
  ...
  int             r = 4;
  points << QPoint(x - r, y - r) << QPoint(x + r, y - r) << QPoint(x + r, y)
         << QPoint(x, y + r) << QPoint(x - r, y);
...
void WidgetHistogram::mousePressEvent(QMouseEvent *event) {
  ...
  double dValue = ( event->x() - m_rectGraph.left() ) *
                  ( m_dOutputRange[1] - m_dOutputRange[0] ) /
  m_rectGraph.width() + m_dOutputRange[0]; if ( event->button() ==
  Qt::LeftButton )
  {
  ...
  */
  if (event->button() == Qt::LeftButton) {
    if (FindMarker(event->x(), event->y(), &m_nActiveMarker,
                   &m_bActiveMarkerMirrored)) {
      if (m_bMarkerEditable && (event->modifiers() & Qt::ShiftModifier)) {
        if (m_markers.size() == 1) {
          QMessageBox::warning(this, "Error",
                               "Can not remove the only color stop left.");
        } else {
