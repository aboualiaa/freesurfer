...
DialogScreenshotOverlay::DialogScreenshotOverlay(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogScreenshotOverlay) {
  ui->setupUi(this);
...
DialogScreenshotOverlay::~DialogScreenshotOverlay() { delete ui; }
...
void DialogScreenshotOverlay::OnButtonOpen() {
  QString path = QFileDialog::getExistingDirectory(
      this, "", ui->lineEditDirectory->text().trimmed());
  if (!path.isEmpty()) {
...
void DialogScreenshotOverlay::OnButtonSave() {
  ...
  if (format.isEmpty()) {
    QMessageBox::information(this, "Error",
                             "Please enter a valid filename format");
    return;
  }
...
void DialogScreenshotOverlay::OnTimeOut() {
  ...
  LayerSurface *surface =
      qobject_cast<LayerSurface *>(mainwnd->GetActiveLayer("Surface"));
  ...
  view->SaveScreenShot(fn, settings.AntiAliasing, settings.Magnification,
                       settings.AutoTrim);
  m_nIndex++;
  ui->labelProgress->setText(
      QString("%1/%2").arg(m_nIndex).arg(surface->GetNumberOfOverlays()));
