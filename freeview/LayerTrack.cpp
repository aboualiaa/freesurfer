...
LayerTrack::LayerTrack(LayerMRI *ref, QObject *parent, bool bCluster)
  ...
  connect(mProperty, SIGNAL(ColorCodeChanged(int)), this,
          SLOT(RebuildActors()));
  connect(mProperty, SIGNAL(DirectionSchemeChanged(int)), this,
          SLOT(RebuildActors()));
  connect(mProperty, SIGNAL(DirectionMappingChanged(int)), this,
          SLOT(RebuildActors()));
  connect(mProperty, SIGNAL(SolidColorChanged(QColor)), this,
          SLOT(UpdateColor()));
  connect(mProperty, SIGNAL(RenderRepChanged()), this, SLOT(RebuildActors()));
  connect(mProperty, SIGNAL(OpacityChanged(double)), this,
          SLOT(UpdateOpacity(double)));
}
...
void LayerTrack::OnSlicePositionChanged(int nPlane) { Q_UNUSED(nPlane); }
...
void LayerTrack::RebuildActors() {
  ...
  for (int i = 0; i < m_trackData->m_tracks.size(); i++) {
    ...
    if (p->GetColorCode() == LayerPropertyTrack::EmbeddedColor) {
      ...
      VectorToColor(t.fPts, t.fPts + (t.nNum - 1) * 3, vals,
                    p->GetDirectionMapping());
    else if (p->GetDirectionScheme() == LayerPropertyTrack::MidSegment)
      VectorToColor(t.fPts + t.nNum / 2 * 3, t.fPts + (t.nNum / 2 - 1) * 3,
                    vals, p->GetDirectionMapping());
    for (int n = 0; n < t.nNum; n++) {
      ...
      if (p->GetDirectionScheme() == LayerPropertyTrack::EverySegment) {
        if (n == 0)
          VectorToColor(t.fPts + n * 3, t.fPts + n * 3 + 3, vals,
                        p->GetDirectionMapping());
        else
          VectorToColor(t.fPts + n * 3, t.fPts + n * 3 - 3, vals,
                        p->GetDirectionMapping());
...
void LayerTrack::UpdateColor(bool emitSignal) {
  ...
  foreach (vtkActor *actor, m_actors) {
    ...
    actor->GetMapper()->SetScalarVisibility(GetProperty()->GetColorCode() !=
...
void LayerTrack::VectorToColor(float *pt1, float *pt2, float *c_out,
                               int nMappingType) {
  double v[3] = {1, 0, 0}, t;
...
vtkActor *LayerTrack::ConstructActor(vtkPoints *points, vtkCellArray *lines,
...
bool LayerTrack::IsCluster() { return !m_mapCluster.isEmpty(); }
