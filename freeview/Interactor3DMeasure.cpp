...
Interactor3DMeasure::Interactor3DMeasure(QObject *parent)
    : Interactor3D(parent), m_bSelectRegion(false) {}
...
#endif
  {
    if (event->modifiers() & CONTROL_MODIFIER &&
      ...
      LayerMRI *mri =
          (LayerMRI *)MainWindow::GetMainWindow()->GetActiveLayer("MRI");
      if (mri && mri->GetCurrentSurfaceRegion()) {
        if (mri->GetCurrentSurfaceRegion()->DeleteCell(view, event->x(),
                                                       event->y())) {
          view->RequestRedraw();
        }
        return false;
...
#endif
    {
      if (e->modifiers() & CONTROL_MODIFIER &&
