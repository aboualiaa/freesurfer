...
SurfaceLabel::SurfaceLabel(LayerSurface *surf, bool bInitializeLabel)
    : QObject(surf), m_label(NULL), m_surface(surf), m_bVisible(true),
      m_nOutlineIndices(NULL), m_dThreshold(0), m_nColorCode(SolidColor),
      m_dHeatscaleMin(0), m_dHeatscaleMax(1), m_dOpacity(1.0),
      m_bModified(false) {
  m_rgbColor[0] = 1.0;
  m_rgbColor[1] = 1.0;
  m_rgbColor[2] = 0.0;
...
QString SurfaceLabel::GetName() { return m_strName; }

void SurfaceLabel::SetName(const QString &name) { m_strName = name; }
...
bool SurfaceLabel::LoadLabel(const QString &filename) {
  ...
  MRIScomputeVertexSpacingStats(mris, NULL, NULL, &max_spacing, NULL, &max_vno,
                                CURRENT_VERTICES);
  MHT *hash =
      MHTcreateVertexTable_Resolution(mris, CURRENT_VERTICES, max_spacing / 4);
...
void SurfaceLabel::MapLabel(unsigned char *colordata, int nVertexCount) {
  ...
  for (int i = 0; i < m_label->n_points; i++) {
    ...
    if (vno < nVertexCount && !m_label->lv[i].deleted &&
        (!m_bShowOutline || m_nOutlineIndices[i] > 0)) {
      double opacity;
...
bool SurfaceLabel::GetCentroid(double *x, double *y, double *z, int *nvo) {
  return (LabelCentroid(m_label, m_surface->GetSourceSurface()->GetMRIS(), x, y,
                        z, nvo) == 0);
}
...
void SurfaceLabel::Resample(LayerMRI *mri) {
  ...
  if (!m_label->mri_template)
    LabelInit(m_label, mri->GetSourceVolume()->GetMRI(),
              m_surface->GetSourceSurface()->GetMRIS(), coord);
  LABEL *label =
      ::LabelSampleToSurface(m_surface->GetSourceSurface()->GetMRIS(), m_label,
