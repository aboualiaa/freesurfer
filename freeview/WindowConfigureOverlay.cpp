...
WindowConfigureOverlay::WindowConfigureOverlay(QWidget *parent)
    : QWidget(parent), UIUpdateHelper(), ui(new Ui::WindowConfigureOverlay),
      m_dSavedOffset(0) {
  ui->setupUi(this);
  ...
  connect(ui->checkBoxFixedAxes, SIGNAL(toggled(bool)),
          SLOT(OnCheckFixedAxes(bool)));
  m_layerSurface = NULL;
  QSettings settings;
  ...
  ui->checkBoxFixedAxes->setChecked(
      settings.value("WindowConfigureOverlay/FixedAxes", true).toBool());
  ...
  connect(MainWindow::GetMainWindow(), SIGNAL(CycleOverlayRequested()),
          SLOT(OnCycleOverlay()));
...
WindowConfigureOverlay::~WindowConfigureOverlay() {
  ...
  settings.setValue("WindowConfigureOverlay/FixedAxes",
                    ui->checkBoxFixedAxes->isChecked());
...
void WindowConfigureOverlay::resizeEvent(QResizeEvent *e) { UpdateGeometry(); }
...
void WindowConfigureOverlay::UpdateGeometry() {
  ...
  rc.moveCenter(QPoint(rc.center().x(),
                       ui->comboBoxOverlayList->geometry().center().y() + 1));
...
void WindowConfigureOverlay::OnActiveSurfaceChanged(Layer *layer) {
  ...
  if (m_layerSurface) {
    disconnect(m_layerSurface, 0, this, 0);
    connect(m_layerSurface, SIGNAL(SurfaceOverlyDataUpdated()), this,
            SLOT(UpdateUI()), Qt::UniqueConnection);
    connect(m_layerSurface, SIGNAL(SurfaceOverlyDataUpdated()), this,
            SLOT(UpdateGraph()), Qt::QueuedConnection);
    connect(m_layerSurface, SIGNAL(ActiveOverlayChanged(int)), this,
            SLOT(OnActiveOverlayChanged()), Qt::UniqueConnection);
    connect(m_layerSurface, SIGNAL(SurfaceLabelAdded(SurfaceLabel *)), this,
            SLOT(OnSurfaceLabelAdded(SurfaceLabel *)), Qt::UniqueConnection);
    connect(m_layerSurface, SIGNAL(SurfaceLabelDeleted(SurfaceLabel *)), this,
            SLOT(UpdateUI()), Qt::UniqueConnection);
  }
...
void WindowConfigureOverlay::UpdateUI() {
  if (m_layerSurface && m_layerSurface->GetActiveOverlay()) {
    ...
    ui->labelFrameRange->setText(
        QString("0-%1").arg(overlay->GetNumberOfFrames() - 1));
    ui->groupBoxFrame->setVisible(overlay->GetNumberOfFrames() > 1);

    //   ui->radioButtonGreenRed ->setChecked( p->GetColorScale() ==
    //   SurfaceOverlayProperty::CS_GreenRed ); ui->radioButtonBlueRed
    //   ->setChecked( p->GetColorScale() == SurfaceOverlayProperty::CS_BlueRed
    //   );
    ui->radioButtonHeat->setChecked(p->GetColorScale() ==
                                    SurfaceOverlayProperty::CS_Heat);
    ui->radioButtonColorWheel->setChecked(
        p->GetColorScale() == SurfaceOverlayProperty::CS_ColorWheel);
    ui->radioButtonCustom->setChecked(p->GetColorScale() ==
    ...
    if (p->GetUsePercentile()) {
      ChangeLineEditNumber(ui->lineEditMin,
                           overlay->PositionToPercentile(p->GetMinPoint()));
      ChangeLineEditNumber(ui->lineEditMid,
                           overlay->PositionToPercentile(p->GetMidPoint()));
      ChangeLineEditNumber(ui->lineEditMax,
                           overlay->PositionToPercentile(p->GetMaxPoint()));
    } else {
    ...
    ui->radioButtonLinear->setChecked(p->GetColorMethod() ==
                                      SurfaceOverlayProperty::CM_Linear);
    ui->radioButtonLinearOpaque->setChecked(
        p->GetColorMethod() == SurfaceOverlayProperty::CM_LinearOpaque);
    ui->radioButtonPiecewise->setChecked(p->GetColorMethod() ==
                                         SurfaceOverlayProperty::CM_Piecewise);
    ...
    ui->checkBoxComputeCorrelation->setChecked(
        overlay->GetComputeCorrelation());
    ...
    foreach (Layer *mri, layers) {
      if ((qobject_cast<LayerMRI *>(mri))->GetNumberOfFrames() == nFrames) {
        ui->comboBoxVolumes->addItem(mri->GetName(),
                                     QVariant::fromValue((QObject *)mri));
        if (overlay->GetCorrelationSourceVolume() == mri) {
          ui->comboBoxVolumes->setCurrentIndex(ui->comboBoxVolumes->count() -
                                               1);
        }
      }
    }
    ...
    for (int i = 0; i < m_layerSurface->GetNumberOfLabels(); i++) {
      ...
      ui->comboBoxMask->addItem(label->GetName(),
                                QVariant::fromValue((QObject *)label));
    ...
    ui->checkBoxComputeCorrelation->setEnabled(ui->comboBoxVolumes->count() >
                                               0);
    ui->groupBoxCorrelation->setVisible(nFrames > 1 &&
                                        ui->comboBoxVolumes->count() > 0);
...
void WindowConfigureOverlay::OnButtonClicked() {
  if (sender() == ui->pushButtonHelp) {
    QMessageBox::information(
        this, "Help",
        "Drag the handle to move point.\n\nAt Custom mode:\nDouble-click on "
        "the handle to change point color.\nShift+Click on the handle to "
        "remove point.");
...
void WindowConfigureOverlay::OnApply() {
  ...
  bool                    smooth_changed =
      (p->GetSmooth() != ui->checkBoxEnableSmooth->isChecked() ||
...
bool WindowConfigureOverlay::UpdateOverlayProperty(SurfaceOverlayProperty *p) {
  ...
  p->SetMask(qobject_cast<SurfaceLabel *>(
      ui->comboBoxMask->itemData(ui->comboBoxMask->currentIndex())
          .value<QObject *>()));
  ...
  if (m_layerSurface && m_layerSurface->GetActiveOverlay()) {
    ...
    if (overlay) {
      ...
      if (m_fDataCache)
        ...
        ui->widgetHistogram->SetInputData(overlay->GetData(),
                                          overlay->GetDataSize(), range);
      ui->widgetHistogram->SetSymmetricMarkers(
          p->GetColorScale() <= SurfaceOverlayProperty::CS_BlueRed);
      ui->widgetHistogram->SetMarkerEditable(p->GetColorScale() ==
                                             SurfaceOverlayProperty::CS_Custom);
...
void WindowConfigureOverlay::UpdateThresholdChanges() {
  if (!ui->radioButtonPiecewise->isChecked()) // do not adjust mid point
                                              // automatically in Piecewise mode
  {
    bool   bOK;
...
void WindowConfigureOverlay::OnHistogramMouseButtonPressed(int    button,
                                                           double value) {
  if (!m_layerSurface || !m_layerSurface->GetActiveOverlay())
    return;
  ...
  if (ui->checkBoxUsePercentile->isChecked())
    value = overlay->PositionToPercentile(
        value, ui->checkBoxUseNonZeroVertices->isChecked());
...
void WindowConfigureOverlay::OnHistogramMarkerChanged() {
  ...
  if (ui->radioButtonCustom->isChecked()) {
      ...
      if (i == 0) {
        if (bUsePercentile)
          ChangeLineEditNumber(
              ui->lineEditMin,
              overlay->PositionToPercentile(
                  markers[i].position - m_dSavedOffset, bIgnoreZeros),
              2, true);
        else
          ChangeLineEditNumber(ui->lineEditMin,
                               markers[i].position - m_dSavedOffset, 2, true);
      }
      if (i == 1) {
        if (ui->radioButtonPiecewise->isChecked() &&
            ui->radioButtonHeat->isChecked()) {
          if (bUsePercentile)
            ChangeLineEditNumber(
                ui->lineEditMid,
                overlay->PositionToPercentile(
    ...
    if (bUsePercentile)
      ChangeLineEditNumber(
          ui->lineEditMax,
          overlay->PositionToPercentile(markers[markers.size() - 1].position -
                                            m_dSavedOffset,
                                        bIgnoreZeros),
          2, true);
    else
      ChangeLineEditNumber(
          ui->lineEditMax,
          markers[markers.size() - 1].position - m_dSavedOffset, 2, true);
    UpdateThresholdChanges();
  }
}
...
void WindowConfigureOverlay::OnButtonAdd() {
  ...
  if (!bOK) {
    QMessageBox::warning(this, "Error",
                         "Please enter valid new point position.");
    return;
  }
  double range[2];
  ui->widgetHistogram->GetOutputRange(range);
  ...
  if (m_layerSurface && m_layerSurface->GetActiveOverlay()) {
    ...
    if (overlay) {
      ...
      if (ui->checkBoxEnableSmooth->isChecked()) {
        ...
        memcpy(m_fDataCache, overlay->GetUnsmoothedData(),
               sizeof(float) * overlay->GetDataSize());
      }
      UpdateGraph(true);
    }
  }
...
void WindowConfigureOverlay::OnTextThresholdChanged(const QString &strg) {
  ...
  if (sender() == ui->lineEditMax || sender() == NULL) {
    ...
    if (ui->checkBoxUsePercentile->isChecked())
      marker.position =
          overlay->PercentileToPosition(val, bIgnoreZeros) + dOffset;
    else
...
void WindowConfigureOverlay::OnCurrentVertexChanged() {
  if (m_layerSurface && m_layerSurface->GetActiveOverlay() &&
    ...
    if (m_layerSurface->IsInflated())
      ...
      nVertex = m_layerSurface->GetVertexIndexAtTarget(
          m_layerSurface->GetSlicePosition(), NULL);
...
void WindowConfigureOverlay::OnComboCorrelationVolume(int n) {
  if (m_layerSurface && m_layerSurface->GetActiveOverlay()) {
    ...
    LayerMRI *      mri     = qobject_cast<LayerMRI *>(
        ui->comboBoxVolumes->itemData(n).value<QObject *>());
...
void WindowConfigureOverlay::OnCycleOverlay() {
  if (m_layerSurface && m_layerSurface->GetNumberOfOverlays() > 1) {
    ui->comboBoxOverlayList->setCurrentIndex(
        (m_layerSurface->GetActiveOverlayIndex() + 1) %
        m_layerSurface->GetNumberOfOverlays());
  }
}

void WindowConfigureOverlay::OnCheckFixedAxes(bool bChecked,
                                              bool bUpdateGraph) {
