...
public:
  ...
  bool MRIWrite(const QString &filename, int nSampleMethod = SAMPLE_NEAREST,
                bool resample = true);
  bool MRIWrite();
  bool SaveRegistration(const QString &filename);
  bool Restore(const QString &filename, const QString &reg_filename);

  int OriginalIndexToRAS(float iIdxX, float iIdxY, float iIdxZ, float &oRASX,
                         float &oRASY, float &oRASZ);
  int RASToOriginalIndex(float iRASX, float iRASY, float iRASZ, float &oIdxX,
                         float &oIdxY, float &oIdxZ);
  int RASToOriginalIndex(float iRASX, float iRASY, float iRASZ, int &oIdxX,
                         int &oIdxY, int &oIdxZ);
  ...
  double GetMinValue() { return m_fMinValue; }

  double GetMaxValue() { return m_fMaxValue; }

  double GetFullMaxValue() { return m_fMaxValueFull; }

  void GetFrameValueRange(int nFrame, double *range);

  double *GetRASToVoxelMatrix() { return m_RASToVoxelMatrix; }

  double *GetVoxelToRASMatrix() { return m_VoxelToRASMatrix; }

  bool GetResampleToRAS() { return m_bResampleToRAS; }

  void SetResampleToRAS(bool bRemap) { m_bResampleToRAS = bRemap; }

  void TargetToRAS(const double *pos_in, double *pos_out);

  void TargetToRAS(const float *pos_in, float *pos_out);

  void TargetToRAS(double x_in, double y_in, double z_in, double &x_out,
  ...
  void RASToNativeRAS(
      const double *pos_in,
      double *pos_out); // when there is registration/transformation involved,
  ...
  MRI *GetMRITarget() { return m_MRITarget; }

  MRI *GetMRI() { return m_MRI; }

  MATRIX *GetRegMatrix() { return m_matReg; }

  MATRIX *GetTransformMatrixInRAS();

  bool HasOriginalTarget() { return m_MRIOrigTarget != NULL; }

  bool Rotate(std::vector<RotationElement> &rotations, int nSampleMethod = -1);

  int GetInterpolationMethod() { return m_nInterpolationMethod; }
  ...
  COLOR_TABLE *GetEmbeddedColorTable() { return m_ctabEmbedded; }

  QString GetOrientationString() { return m_strOrientation; }

  void SetCroppingBounds(double *bound);

  vtkTransform *GetTransform();

  void ConvertTransformFromTargetToRAS(vtkMatrix4x4 *target_in,
                                       vtkMatrix4x4 *ras_out);

  void SetConform(bool bConform);

  // for saving only
  void SetCropToOriginal(bool bCrop) { m_bCropToOriginal = bCrop; }

  static MATRIX *LoadRegistrationMatrix(const QString &filename, MRI *target,
                                        MRI *src, LTA **lta_out = NULL);
  ...
  bool HasValidHistogram() { return m_bValidHistogram; }
