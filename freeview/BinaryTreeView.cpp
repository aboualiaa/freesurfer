...
TreeDataLoader::TreeDataLoader(QObject *parent) : QObject(parent) {
  connect(this, SIGNAL(LoadRequested(QString, BinaryTreeView *)),
          SLOT(DoLoad(QString, BinaryTreeView *)));
}
...
BinaryTreeView::BinaryTreeView(QWidget *parent)
    : QGraphicsView(parent), m_selectedNode(NULL) {
  ...
  m_threadLoad.start();
  connect(m_dataLoader, SIGNAL(DataLoaded(QVariantMap)),
          SLOT(OnDataLoaded(QVariantMap)), Qt::QueuedConnection);
}
...
void BinaryTreeView::mousePressEvent(QMouseEvent *event) {
  ...
  if (!gitem || gitem->type() != (QGraphicsItem::UserType + 1))
    return;
  BinaryTreeNode *item = (BinaryTreeNode *)gitem; // Get the node at the
                                                  // position
  if (item) // if there is a node at that position
  {
    ...
    qDebug() << "Leaves"
             << list; // print list of all leaves tracts to show for this node
    if (!list.isEmpty()) {
...
void BinaryTreeView::drawBackground(QPainter *painter, const QRectF &rect) {
  ...
  //  sceneRect.height()); QRectF bottomShadow(sceneRect.left() + 5,
  //  sceneRect.bottom(), sceneRect.width(), 5); if
  //  (rightShadow.intersects(rect) || rightShadow.contains(rect))
...
void BinaryTreeView::ZoomIn() { ScaleView(qreal(1.2)); }

void BinaryTreeView::ZoomOut() { ScaleView(1 / qreal(1.2)); }
...
void BinaryTreeView::SetData(const QVariantMap &data) {
  ...
  for (int i = 0; i < keys.size(); i++) {
    ...
    if (key == "1") {
      ...
      // add edges to nodes
      if (key.size() > 1) {
        m_scene->addItem(new BinaryTreeEdge(m_mapNode[key.left(key.size() - 1)],
                                            m_mapNode[key]));
      }
    }
  }
