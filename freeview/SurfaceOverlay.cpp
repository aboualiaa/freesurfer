...
SurfaceOverlay::SurfaceOverlay(LayerSurface *surf)
  ...
  connect(m_property, SIGNAL(ColorMapChanged()), surf, SLOT(UpdateOverlay()),
          Qt::UniqueConnection);
  connect(m_property, SIGNAL(SmoothChanged()), this, SLOT(UpdateSmooth()),
          Qt::UniqueConnection);
}
...
void SurfaceOverlay::InitializeData(float *data_buffer_in, int nvertices,
                                    int nframes) {
  if (m_surface) {
    if (m_fDataRaw)
      delete[] m_fDataRaw;
...
void SurfaceOverlay::UpdateCorrelationAtVertex(int nVertex, int nHemisphere) {
  bool bSingleHemiData =
      (((qlonglong)m_mriCorrelation->width) * m_mriCorrelation->height *
           m_mriCorrelation->nframes ==
       m_nDataSize * m_nDataSize);
  if (nHemisphere == -1) {
  ...
  if (m_mriCorrelation->height > 1)
    m_dMaxValue = m_dMinValue = MRIFseq_vox(
        m_mriCorrelation, nVertex + nVertexOffset, nDataOffset, 0, 0);
  else
    m_dMaxValue = m_dMinValue = MRIFseq_vox(
        m_mriCorrelation, nVertex + nVertexOffset, 0, 0, nDataOffset);
  m_dNonZeroMinValue = 1e10;
  for (int i = 0; i < m_nDataSize; i++) {
    if (m_mriCorrelation->height > 1) {
      m_fData[i] = MRIFseq_vox(m_mriCorrelation, nVertex + nVertexOffset,
                               i + nDataOffset, 0, 0);
    } else {
      m_fData[i] = MRIFseq_vox(m_mriCorrelation, nVertex + nVertexOffset, 0, 0,
                               i + nDataOffset);
    }
...
QString SurfaceOverlay::GetName() { return m_strName; }

void SurfaceOverlay::SetName(const QString &name) { m_strName = name; }
...
double SurfaceOverlay::GetDataAtVertex(int nVertex) { return m_fData[nVertex]; }
...
void SurfaceOverlay::SmoothData(int nSteps_in, float *data_out) {
  MRI *mri = NULL;
  try {
    mri = MRIallocSequence(m_nDataSize, 1, 1, MRI_FLOAT, 1);
  } catch (int ret) {
    return;
  }
  ...
  MRI *mri_smoothed = ::MRISsmoothMRI(m_surface->GetSourceSurface()->GetMRIS(),
                                      mri, nSteps, NULL, NULL);
  if (mri_smoothed) {
    if (data_out)
      memcpy(data_out, &MRIFseq_vox(mri_smoothed, 0, 0, 0, 0),
             sizeof(float) * m_nDataSize);
    else
      memcpy(m_fData, &MRIFseq_vox(mri_smoothed, 0, 0, 0, 0),
             sizeof(float) * m_nDataSize);
...
void SurfaceOverlay::SetActiveFrame(int nFrame) {
  ...
  memcpy(m_fData, m_fDataRaw + m_nActiveFrame * m_nDataSize,
         sizeof(float) * m_nDataSize);
  memcpy(m_fDataUnsmoothed, m_fData, sizeof(float) * m_nDataSize);
...
void SurfaceOverlay::UpdateCorrelationCoefficient(double *pos_in) {
  if (m_bComputeCorrelation) {
    if (m_volumeCorrelationSource &&
      ...
      m_volumeCorrelationSource->GetVoxelValueByOriginalIndexAllFrames(
          n[0], n[1], n[2], m_fCorrelationSourceData);
    } else {
    ...
    for (int i = 0; i < m_nDataSize; i++) {
      ...
      m_fData[i] = MyUtils::CalculateCorrelationCoefficient(
          m_fCorrelationSourceData, m_fCorrelationDataBuffer, m_nNumOfFrames);
...
void SurfaceOverlay::SetCorrelationSourceVolume(LayerMRI *vol) {
  ...
  if (vol)
    connect(vol, SIGNAL(destroyed(QObject *)), this,
            SLOT(OnCorrelationSourceDeleted(QObject *)));
  UpdateCorrelationCoefficient();
}
...
double SurfaceOverlay::PercentileToPosition(double percentile_in,
  ...
  if (dArea > percentile * m_dOutputTotalArea && n > 0) {
    dPos -= (dArea - percentile * m_dOutputTotalArea) * m_dBinWidth /
            m_nOutputData[n - 1];
  }
