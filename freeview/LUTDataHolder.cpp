...
COLOR_TABLE *LUTDataHolder::GetColorTable(const QString &name) {
  for (int i = 0; i < GetCount(); i++) {
    if (m_tables[i].name == name ||
        (QFileInfo(m_tables[i].filename).canonicalFilePath() ==
             QFileInfo(name).canonicalFilePath() &&
...
COLOR_TABLE *LUTDataHolder::LoadColorTable(const QString &filename) {
  ...
  // if so, reload the lut file and update the lut data. do not create new entry
  // in the lut list
  if (ct) {
...
int LUTDataHolder::GetCount() { return m_tables.size(); }
