...
ToolWindowMeasure::ToolWindowMeasure(QWidget *parent)
  ...
  connect(actGroup, SIGNAL(triggered(QAction *)), this,
          SLOT(OnAction(QAction *)));

  m_widgets2D << ui->pushButtonCopy << ui->pushButtonExport;

  m_widgets3D << ui->pushButtonSave << ui->pushButtonLoad
              << ui->pushButtonSaveAll << ui->spinBoxId << ui->spinBoxGroup
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (i < 3) {
      connect(((RenderView2D *)view), SIGNAL(RegionRemoved(Region2D *)), this,
              SLOT(SetRegion()));
      connect(((RenderView2D *)view), SIGNAL(RegionSelected(Region2D *)), this,
              SLOT(SetRegion(Region2D *)));
    } else {
      connect(((RenderView3D *)view),
              SIGNAL(SurfaceRegionSelected(SurfaceRegion *)), this,
              SLOT(SetSurfaceRegion(SurfaceRegion *)));
      connect(((RenderView3D *)view),
              SIGNAL(SurfaceRegionRemoved(SurfaceRegion *)), this,
              SLOT(SetSurfaceRegion()));
    }
  }
  LayerCollection *col_mri = mainwnd->GetLayerCollection("MRI");
  connect(col_mri, SIGNAL(LayerAdded(Layer *)), this, SLOT(UpdateWidgets()),
          Qt::QueuedConnection);
  connect(col_mri, SIGNAL(LayerRemoved(Layer *)), this, SLOT(UpdateWidgets()),
          Qt::QueuedConnection);
  connect(col_mri, SIGNAL(LayerPropertyChanged()), this, SLOT(UpdateWidgets()),
          Qt::QueuedConnection);
...
ToolWindowMeasure::~ToolWindowMeasure() {
  QSettings settings;
  settings.setValue("ToolWindowMeasure/Position",
...
void ToolWindowMeasure::SetRegion(Region2D *reg) {
  ...
  m_region = reg;
  if (m_region) {
    connect(m_region, SIGNAL(StatsUpdated()), this, SLOT(UpdateWidgets()),
            Qt::QueuedConnection);
    if (m_surfaceRegion) {
...
void ToolWindowMeasure::SetSurfaceRegion(SurfaceRegion *reg) {
  m_surfaceRegion = reg;
  if (m_surfaceRegion) {
    connect(m_surfaceRegion, SIGNAL(ColorChanged(QColor)), this,
...
void ToolWindowMeasure::OnIdle() {
  ...
  ui->actionRectangle->setChecked(view->GetAction() ==
                                  Interactor::MM_Rectangle);
  ...
  for (int i = 0; i < col_mri->GetNumberOfLayers(); i++) {
    if (((LayerMRI *)col_mri->GetLayer(i))->GetProperty()->GetColorMap() ==
        LayerPropertyMRI::LUT) {
      bLabelExist = true;
      break;
    }
  }
  ui->actionLabel->setEnabled(MainWindow::GetMainWindow()->GetMainViewId() <
                                  3 &&
  ...
  if (m_surfaceRegion) {
    ...
    ui->spinBoxGroup->setRange(
        1, m_surfaceRegion->GetMRI()->GetSurfaceRegionGroups()->GetGroupIdRange(
               m_surfaceRegion));
  ...
  bool bSurfaceRegionValid = (mri && mri->GetProperty()->GetShowAsContour() &&
                              mri->GetNumberOfSurfaceRegions() > 0);
...
void ToolWindowMeasure::OnLoad() {
  QString filename = QFileDialog::getOpenFileName(
      this, "Load region(s) from file", "", "All files (*)");
  ...
  if (mri && !filename.isEmpty()) {
    if (!mri->LoadSurfaceRegions(filename)) {
      QMessageBox::warning(this, "Error",
                           QString("Can not load file ") + filename);
    }
    UpdateWidgets();
  }
}
...
void ToolWindowMeasure::OnSave() {
  QString filename =
      QFileDialog::getSaveFileName(this, "Save region", "", "All files (*)");
  if (m_surfaceRegion && !filename.isEmpty()) {
    if (!m_surfaceRegion->Write(filename)) {
      QMessageBox::warning(this, "Error",
                           QString("Can not write to file ") + filename);
    }
  }
}
...
void ToolWindowMeasure::OnUpdate() { UpdateWidgets(); }
...
void ToolWindowMeasure::OnExport() {
  QString filename = QFileDialog::getSaveFileName(this, "Export stats to file",
                                                  "", "All files (*)");
  if (!filename.isEmpty()) {
    QFile file(filename);
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
      QMessageBox::warning(this, "Error",
                           QString("Unable to write to file ") + filename);
      return;
    }
...
void ToolWindowMeasure::OnSpinBoxId(int val) {
  RenderView3D *view =
      (RenderView3D *)MainWindow::GetMainWindow()->GetRenderView(3);
...
void ToolWindowMeasure::OnColorGroup(const QColor &color) {
  if (m_surfaceRegion) {
    m_surfaceRegion->GetMRI()->GetSurfaceRegionGroups()->SetGroupColor(
        m_surfaceRegion->GetGroup(), color);
    UpdateWidgets();
  }
}
