...
void SurfaceOverlayProperty::Copy(SurfaceOverlayProperty *p) {
  ...
  if (m_bUsePercentile) {
    ...
    m_dMidPoint = m_overlay->PercentileToPosition(
        p->m_overlay->PositionToPercentile(p->m_dMidPoint));
    m_dMaxPoint = m_overlay->PercentileToPosition(
        p->m_overlay->PositionToPercentile(p->m_dMaxPoint));
  } else {
...
double SurfaceOverlayProperty::GetOpacity() const { return m_dOpacity; }
...
int SurfaceOverlayProperty::GetColorScale() const { return m_nColorScale; }
...
void SurfaceOverlayProperty::SetColorScale(int nScale) {
  ...
  if (nScale <= CS_BlueRed) {
    if (!m_bColorTruncate || m_bColorInverse) {
      if (m_bColorInverse) {
        m_lut->AddRGBAPoint(-m_dMaxPoint + m_dOffset, m_colorMax[0] / 255.0,
                            m_colorMax[1] / 255.0, m_colorMax[2] / 255.0, 1);
        m_lut->AddRGBAPoint(-m_dMidPoint + m_dOffset, m_colorMin[0] / 255.0,
                            m_colorMin[1] / 255.0, m_colorMin[2] / 255.0, 1);
        if (m_nColorMethod == CM_LinearOpaque) {
          m_lut->AddRGBAPoint(-m_dMinPoint + m_dOffset, m_colorMin[0] / 255.0,
                              m_colorMin[1] / 255.0, m_colorMin[2] / 255.0, 1);
        } else {
          m_lut->AddRGBAPoint(-m_dMinPoint + m_dOffset, 0.4, 0.4, 0.4, 1);
        }
      } else {
        m_lut->AddRGBAPoint(-m_dMaxPoint + m_dOffset, 1 - m_colorMax[0] / 255.0,
                            m_colorMax[1] / 255.0, 1 - m_colorMax[2] / 255.0,
                            1);
        m_lut->AddRGBAPoint(-m_dMidPoint + m_dOffset, 1 - m_colorMin[0] / 255.0,
                            m_colorMin[1] / 255.0, 1 - m_colorMin[2] / 255.0,
                            1);
        if (m_nColorMethod == CM_LinearOpaque) {
          m_lut->AddRGBAPoint(-m_dMinPoint + m_dOffset,
                              1 - m_colorMin[0] / 255.0, m_colorMin[1] / 255.0,
                              1 - m_colorMin[2] / 255.0, 1);
        } else {
          m_lut->AddRGBAPoint(-m_dMinPoint + m_dOffset, 0.4, 0.4, 0.4, 1);
        }
      }
      m_lut->AddRGBAPoint(-m_dMinPoint + m_dOffset + 0.0000000001, 0.4, 0.4,
                          0.4, 1);
    }
    ...
    if (!(m_bColorTruncate && m_bColorInverse)) {
      m_lut->AddRGBAPoint(m_dMinPoint + m_dOffset - 0.0000000001, 0.4, 0.4, 0.4,
                          1);
      if (m_bColorInverse) {
        if (m_nColorMethod == CM_LinearOpaque) {
          m_lut->AddRGBAPoint(m_dMinPoint + m_dOffset,
    ...
    if (!m_bColorInverse) {
      ...
      m_lut->AddRGBAPoint(m_dMinPoint + m_dOffset +
                              (m_dMaxPoint - m_dMinPoint) * .25,
                          1, 1, 0, 1);
      m_lut->AddRGBAPoint(m_dMinPoint + m_dOffset +
                              (m_dMaxPoint - m_dMinPoint) * .5,
                          0, 1, 0, 1);
      m_lut->AddRGBAPoint(m_dMinPoint + m_dOffset +
                              (m_dMaxPoint - m_dMinPoint) * .75,
                          0, 1, 1, 1);
    ...
    for (int i = 0; i < m_customScale.size(); i++) {
      ...
      m_lut->AddRGBAPoint(m_customScale.at(i).first + m_dOffset, c.redF(),
                          c.greenF(), c.blueF(), 1);
    }
  }
...
int SurfaceOverlayProperty::GetColorMethod() { return m_nColorMethod; }

void SurfaceOverlayProperty::SetColorMethod(int nTh) { m_nColorMethod = nTh; }
...
double SurfaceOverlayProperty::GetMinPoint() { return m_dMinPoint; }
...
double SurfaceOverlayProperty::GetMidPoint() { return m_dMidPoint; }
...
double SurfaceOverlayProperty::GetMaxPoint() { return m_dMaxPoint; }
...
double SurfaceOverlayProperty::GetOffset() { return m_dOffset; }

bool SurfaceOverlayProperty::GetColorInverse() { return m_bColorInverse; }
...
bool SurfaceOverlayProperty::GetColorTruncate() { return m_bColorTruncate; }
  ...
  if (m_nColorMethod == CM_LinearOpaque) {
    for (int i = 0; i < nPoints; i++) {
      ...
      if (nPoints == m_overlay->GetDataSize())
        bInMask = (!m_mask || ((!m_bInverseMask && m_maskData[i]) ||
                               (m_bInverseMask && !m_maskData[i])));
      if (data[i] >= m_dMinPoint + m_dOffset &&
          !(m_bColorInverse && m_bColorTruncate) && bInMask) {
        double r = 0;
        ...
        colordata[i * 4] =
            (int)(colordata[i * 4] * (1 - m_dOpacity) + c[0] * m_dOpacity);
        colordata[i * 4 + 1] =
            (int)(colordata[i * 4 + 1] * (1 - m_dOpacity) + c[1] * m_dOpacity);
        colordata[i * 4 + 2] =
            (int)(colordata[i * 4 + 2] * (1 - m_dOpacity) + c[2] * m_dOpacity);
      }
      // map negative values
      else if (data[i] <= -m_dMinPoint + m_dOffset &&
               !(m_bColorTruncate && !m_bColorInverse) && bInMask) {
        double r = 0;
        if (m_dMaxPoint != dMidPoint) {
...
void SurfaceOverlayProperty::MapOverlayColorFullScale(float *        data,
  ...
  for (int i = 0; i < nPoints; i++) {
    ...
    if (bInMask && data[i] >= dThLow && data[i] <= dThHigh) {
      ...
      colordata[i * 4] =
          (int)(colordata[i * 4] * (1 - m_dOpacity) + c[0] * 255 * m_dOpacity);
      colordata[i * 4 + 1] = (int)(colordata[i * 4 + 1] * (1 - m_dOpacity) +
                                   c[1] * 255 * m_dOpacity);
      colordata[i * 4 + 2] = (int)(colordata[i * 4 + 2] * (1 - m_dOpacity) +
                                   c[2] * 255 * m_dOpacity);
    }
  }
}
  ...
  if (label != m_mask) {
    ...
    if (label) {
      ...
      connect(label, SIGNAL(destroyed(QObject *)),
              SLOT(OnLabelMaskDestroyed(QObject *)), Qt::UniqueConnection);
    }
    emit MaskChanged();
  }
}
...
bool SurfaceOverlayProperty::LoadCustomColorScale(const QString &filename) {
  ...
  if (list.isEmpty()) {
    ...
    foreach (QVariant v, list) {
      ...
      stops << QGradientStop(
          map["val"].toDouble(),
          QColor(map["r"].toInt(), map["g"].toInt(), map["b"].toInt()));
    }
