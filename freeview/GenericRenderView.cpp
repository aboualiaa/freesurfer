...
void GenericRenderView::SetLightIntensity(double key, double head, double fill,
                                          double back) {
...
void GenericRenderView::SetLightToDefault() {
  SetLightIntensity(DEFAULT_KEY_LIGHT, DEFAULT_HEAD_LIGHT, DEFAULT_FILL_LIGHT,
                    DEFAULT_BACK_LIGHT);
}

double GenericRenderView::GetKeyLightIntensity() {
  return (m_lightKit->GetKeyLightIntensity() - MIN_KEY_LIGHT) /
         (MAX_KEY_LIGHT - MIN_KEY_LIGHT);
}

void GenericRenderView::SetKeyLightIntensity(double d, bool redraw) {
  m_lightKit->SetKeyLightIntensity(MIN_KEY_LIGHT +
                                   (MAX_KEY_LIGHT - MIN_KEY_LIGHT) * d);
  if (redraw) {
    Render();
  }
...
double GenericRenderView::GetHeadLightIntensity() {
  return (1.0 / m_lightKit->GetKeyToHeadRatio() - MIN_RATIO_LIGHT) /
         (1 - MIN_RATIO_LIGHT);
}

void GenericRenderView::SetHeadLightIntensity(double d, bool redraw) {
  m_lightKit->SetKeyToHeadRatio(1.0 /
                                (MIN_RATIO_LIGHT + (1 - MIN_RATIO_LIGHT) * d));
  if (redraw) {
    Render();
  }
...
double GenericRenderView::GetFillLightIntensity() {
  return (1.0 / m_lightKit->GetKeyToFillRatio() - MIN_RATIO_LIGHT) /
         (1 - MIN_RATIO_LIGHT);
}

void GenericRenderView::SetFillLightIntensity(double d, bool redraw) {
  m_lightKit->SetKeyToFillRatio(1.0 /
                                (MIN_RATIO_LIGHT + (1 - MIN_RATIO_LIGHT) * d));
  if (redraw) {
    Render();
  }
...
double GenericRenderView::GetBackLightIntensity() {
  return (1.0 / m_lightKit->GetKeyToBackRatio() - MIN_RATIO_LIGHT) /
         (1 - MIN_RATIO_LIGHT);
}

void GenericRenderView::SetBackLightIntensity(double d, bool redraw) {
  m_lightKit->SetKeyToBackRatio(1.0 /
                                (MIN_RATIO_LIGHT + (1 - MIN_RATIO_LIGHT) * d));
  if (redraw) {
    Render();
  }
...
// avoid sending key event to QVTKOpenGLNativeWidget because of a bug in
// QVTKInteractor
...
void GenericRenderView::SetStereoTypeToLeftRight(bool b) {
  if (b) {
    ...
    connect(this, SIGNAL(RenderTriggeredByInteractor()), this,
            SLOT(UpdateCamera2()));
  } else {
    disconnect(this, SIGNAL(PropsUpdated()), this, SLOT(UpdateRenderer2()));
    disconnect(this, SIGNAL(RenderTriggeredByInteractor()), this,
               SLOT(UpdateCamera2()));
    if (m_renderer2) {
...
bool GenericRenderView::SetCameraOperations(CameraOperations ops) {
  ...
  for (int i = 0; i < ops.size(); i++) {
    ...
    } else if (ops[i].first.toLower() == "dolly" ||
               ops[i].first.toLower() == "zoom") {
      cam->Dolly(ops[i].second);
    } else if (ops[i].first.toLower() == "elevation" ||
               ops[i].first.toLower() == "elevate") {
