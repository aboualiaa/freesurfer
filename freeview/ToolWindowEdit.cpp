...
ToolWindowEdit::ToolWindowEdit(QWidget *parent)
  ...
  connect(ui->spinBoxTolerance, SIGNAL(valueChanged(int)), bp,
          SLOT(SetBrushTolerance(int)));
  connect(ui->comboBoxReference, SIGNAL(currentIndexChanged(int)), this,
          SLOT(OnComboReference(int)));
  connect(ui->checkBoxConstrain, SIGNAL(toggled(bool)), bp,
          SLOT(SetDrawConnectedOnly(bool)));
  connect(ui->checkBoxDrawRange, SIGNAL(toggled(bool)), bp,
          SLOT(SetDrawRangeEnabled(bool)));
  connect(ui->checkBoxExcludeRange, SIGNAL(toggled(bool)), bp,
          SLOT(SetExcludeRangeEnabled(bool)));
  connect(ui->checkBoxFill3D, SIGNAL(toggled(bool)), bp, SLOT(SetFill3D(bool)));
  connect(ui->checkBoxEraseRange, SIGNAL(toggled(bool)), bp,
          SLOT(SetEraseRangeEnabled(bool)));
  connect(ui->checkBoxEraseExcludeRange, SIGNAL(toggled(bool)), bp,
          SLOT(SetEraseExcludeRangeEnabled(bool)));
  connect(mainwnd->GetLayerCollection("MRI"), SIGNAL(LayerAdded(Layer *)), this,
          SLOT(UpdateWidgets()));
  connect(mainwnd->GetLayerCollection("MRI"), SIGNAL(LayerRemoved(Layer *)),
  ...
  connect(ui->pushButtonGeoClear, SIGNAL(clicked(bool)),
          SLOT(OnButtonGeoSegClear()));
  connect(ui->pushButtonGeoClearFilling, SIGNAL(clicked(bool)),
          SLOT(OnButtonGeoSegClearFilling()));
  connect(ui->pushButtonGeoGo, SIGNAL(clicked(bool)), SLOT(OnButtonGeoSegGo()));
  connect(ui->pushButtonGeoApply, SIGNAL(clicked(bool)),
          SLOT(OnButtonGeoSegApply()));
  connect(ui->pushButtonGeoUndo, SIGNAL(clicked(bool)),
          SLOT(OnButtonGeoSegUndo()));
  connect(ui->colorPickerGeoInside, SIGNAL(colorChanged(QColor)),
          SLOT(OnColorPickerGeoSeg(QColor)));
  connect(ui->colorPickerGeoOutside, SIGNAL(colorChanged(QColor)),
          SLOT(OnColorPickerGeoSeg(QColor)));
  connect(ui->colorPickerGeoFill, SIGNAL(colorChanged(QColor)),
          SLOT(OnColorPickerGeoSeg(QColor)));
  connect(ui->sliderGeoOpacity, SIGNAL(valueChanged(int)),
          SLOT(OnSliderGeoOpacity(int)));
  connect(ui->pushButtonAbort, SIGNAL(clicked(bool)),
          SLOT(OnButtonGeoSegAbort()));
  ...
  for (int i = 0; i < 3; i++) {
    ...
    connect(ui->checkBoxSmooth, SIGNAL(toggled(bool)), view->GetContour2D(),
            SLOT(SetSmooth(bool)));
    connect(view->GetContour2D(), SIGNAL(ValueChanged()), this,
            SLOT(UpdateWidgets()));
  }
  ...
  m_widgetsSmooth << ui->checkBoxSmooth << ui->labelSD << ui->lineEditSD;

  m_widgetsContour << ui->labelContourColor << ui->labelContourValue
                   << ui->lineEditContourValue << ui->colorPickerContour
                   << ui->labelTipsContour;

  m_widgetsGeoSeg << ui->labelGeoLambda << ui->labelGeoMaxDistance
                  << ui->labelGeoWsize << ui->lineEditGeoLambda
                  << ui->spinBoxGeoWsize << ui->lineEditGeoMaxDistance
                  << ui->pushButtonGeoGo << ui->pushButtonGeoClear
                  << ui->pushButtonGeoClearFilling << ui->widgetGeoColors
                  << ui->sliderGeoOpacity << ui->pushButtonGeoApply
                  << ui->pushButtonGeoUndo << ui->labelTipsGeoS
                  << ui->widgetBusyIndicator << ui->checkBoxApplySmoothing
                  << ui->lineEditSmoothingStd << ui->pushButtonAbort
                  << ui->checkBoxGeoSegOverwrite << ui->labelGeoMessage;
...
ToolWindowEdit::~ToolWindowEdit() {
  QSettings settings;
  settings.setValue("ToolWindowVoxelEdit/Position",
...
void ToolWindowEdit::UpdateWidgets() { m_bToUpdateWidgets = true; }
...
void ToolWindowEdit::OnIdle() {
  ...
  ui->actionColorPicker->setChecked(view->GetAction() ==
                                    Interactor2DVoxelEdit::EM_ColorPicker);
  ui->actionContour->setChecked(view->GetAction() ==
                                Interactor2DVoxelEdit::EM_Contour);
  ui->actionFill->setChecked(view->GetAction() ==
                             Interactor2DVoxelEdit::EM_Fill);
  ui->actionLiveWire->setChecked(view->GetAction() ==
                                 Interactor2DVoxelEdit::EM_Livewire);
  ui->actionFreeHand->setChecked(view->GetAction() ==
                                 Interactor2DVoxelEdit::EM_Freehand);
  ui->actionPolyLine->setChecked(view->GetAction() ==
                                 Interactor2DVoxelEdit::EM_Polyline);
  ui->actionClone->setChecked(view->GetAction() ==
                              Interactor2DVoxelEdit::EM_Clone);
  ui->actionAutoSeg->setChecked(view->GetAction() ==
                                Interactor2DVoxelEdit::EM_GeoSeg);

  ui->spinBoxBrushSize->setEnabled(view->GetAction() !=
                                   Interactor2DVoxelEdit::EM_Fill);
  ...
  for (int i = 0; i < lc->GetNumberOfLayers(); i++) {
    ...
    ui->comboBoxReference->addItem(mri->GetName(),
  ...
  if (geos.contains("FillColor"))
    ui->colorPickerGeoInside->setCurrentColor(
        geos["ForegroundColor"].value<QColor>());
  if (geos.contains("BackgroundColor"))
    ui->colorPickerGeoOutside->setCurrentColor(
        geos["BackgroundColor"].value<QColor>());

  int nAction = view->GetAction();
  ShowWidgets(m_widgetsBrushSize,
  ...
  if (bOK && value > 0) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (c2d->GetInputImage()) {
        ...
        c2d->SetInput(mri->GetSliceImageData(view->GetViewPlane()), value,
                      mri->GetSlicePosition()[i], mri->GetActiveFrame());
        c2d->SetVisible(true);
      }
    }
    UpdateWidgets();
...
void ToolWindowEdit::OnComboReference(int sel) {
  LayerVolumeBase *layer = qobject_cast<LayerVolumeBase *>(
      ui->comboBoxReference->itemData(sel).value<QObject *>());
  BrushProperty *bp = MainWindow::GetMainWindow()->GetBrushProperty();
...
void ToolWindowEdit::OnButtonGeoSegClear() {
  LayerMRI *mri = qobject_cast<LayerMRI *>(
      MainWindow::GetMainWindow()->FindSupplementLayer("GEOS_DRAW"));
  if (mri) {
    mri->SaveForUndo();
    mri->ClearVoxels();
...
void ToolWindowEdit::OnButtonGeoSegGo() {
  ...
  LayerMRI *mri =
      (LayerMRI *)MainWindow::GetMainWindow()->GetActiveLayer("MRI");
  if (mri) {
    LayerMRI *mri_draw = qobject_cast<LayerMRI *>(
        MainWindow::GetMainWindow()->FindSupplementLayer("GEOS_DRAW"));
    ...
    if (mri_draw && mri_fill) {
      ...
      ui->labelGeoMessage->setText(
          "<span style=\"color:red\">No inside pixels found</span>");
    }
  }
}
...
void ToolWindowEdit::OnGeoSegFinished(double time_in_secs) {
  ...
  if (time_in_secs < 0) {
    ...
    ui->labelGeoMessage->setText(
        QString::asprintf("Time taken: %.3fs", time_in_secs));
}
...
void ToolWindowEdit::OnButtonGeoSegApply() {
  ...
  LayerMRI *mri =
      (LayerMRI *)MainWindow::GetMainWindow()->GetActiveLayer("MRI");
  if (mri && mri_fill) {
    connect(mri, SIGNAL(GeodesicSegmentationApplied()),
            SLOT(OnButtonGeoSegClearFilling()), Qt::UniqueConnection);
