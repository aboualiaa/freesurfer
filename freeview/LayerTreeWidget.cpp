...
void LayerTreeWidget::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::RightButton)
    ...
    if (rectCheckbox.isEmpty() || rectCheckbox.width() <= 0) {
      ...
      if (item)
        rectCheckbox =
            m_itemDelegate->GetCheckBoxRect(indexFromItem(item), option);
    }

    if (item && item->childCount() == 0 && event->x() < rectCheckbox.right() &&
        event->x() > rectCheckbox.left()) {
...
void LayerTreeWidget::mouseReleaseEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    ...
    if (item && item->childCount() == 0 && event->x() < rectCheckbox.right() &&
        event->x() > rectCheckbox.left()) {
      item->setCheckState(0, item->checkState(0) == Qt::Checked ? Qt::Unchecked
                                                                : Qt::Checked);
...
void LayerTreeWidget::contextMenuEvent(QContextMenuEvent *e) {
  ...
  if (layers.size() > 0 && items.contains(item)) {
    ...
    if (layers[0]->IsTypeOf("MRI") || layers[0]->IsTypeOf("Surface")) {
      ...
      menu->addAction(act);
      act = new QAction(
          layers.size() > 1 ? "Hide All in Info Panel" : "Hide Info", this);
    ...
    if (layers[0]->GetEndType() == "MRI") {
      ...
      if (((LayerMRI *)layers[0])->GetRefVolume()) {
        ...
        connect(act, SIGNAL(triggered(bool)), MainWindow::GetMainWindow(),
                SLOT(OnApplyVolumeTransform()));
        menu->addAction(act);
      }
...
void LayerTreeWidget::SelectAll() {
  ...
  foreach (QTreeWidgetItem *item, items) { item->setSelected(true); }
}
...
void LayerTreeWidget::DeselectAll() {
  ...
  foreach (QTreeWidgetItem *item, items) { item->setSelected(false); }
}
...
void LayerTreeWidget::dropEvent(QDropEvent *event) {
  ...
  if (droppedIndex.isValid()) {
      ...
      if (parent) {
        if (parent->data(0, Qt::UserRole).toString() == type) {
          ...
          parent->insertChildren(
              parent->indexOfChild(itemTo) +
                  (drop_pos == QTreeWidget::BelowItem ? 1 : 0),
              itemsFrom);
        }
    ...
    foreach (QTreeWidgetItem *item, itemsFrom) { item->setSelected(true); }
