...
void Region2DLine::Update() {
  ...
  m_view->WorldToViewport(m_dPt1[0], m_dPt1[1], m_dPt1[2], pt1[0], pt1[1],
                          pt1[2]);
  m_view->WorldToViewport(m_dPt2[0], m_dPt2[1], m_dPt2[2], pt2[0], pt2[1],
                          pt2[2]);
  for (int i = 0; i < 3; i++) {
...
void Region2DLine::UpdateStats() {
  ...
  if (layer) {
    ...
    for (int i = 0; i < count; i++) {
      sprintf(ch, "[%d, %d, %d]  %.2f", indices[i * 3], indices[i * 3 + 1],
              indices[i * 3 + 2], values[i]);
