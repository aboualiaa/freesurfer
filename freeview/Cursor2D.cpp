...
void Cursor2D::Update(bool bConnectPrevious) {
  ...
  double pos[3];
  if (bConnectPrevious) {
    m_view->WorldToViewport(m_dPosition2[0], m_dPosition2[1], m_dPosition2[2],
                            pos[0], pos[1], pos[2]);
    points->InsertNextPoint(pos);
    lines->InsertNextCell(2 + m_dInterpolationPoints.size() / 3);
    lines->InsertCellPoint(n++);
    for (size_t i = 0; i < m_dInterpolationPoints.size(); i += 3) {
      m_view->WorldToViewport(
          m_dInterpolationPoints[i], m_dInterpolationPoints[i + 1],
          m_dInterpolationPoints[i + 2], pos[0], pos[1], pos[2]);
      points->InsertNextPoint(pos[0], pos[1], pos[2]);
      lines->InsertCellPoint(n++);
    }
    m_view->WorldToViewport(m_dPosition[0], m_dPosition[1], m_dPosition[2],
                            pos[0], pos[1], pos[2]);
    points->InsertNextPoint(pos);
    lines->InsertCellPoint(n++);
  } else {
...
); points->InsertNextPoint( m_dPosition[0] - dLen, m_dPosition[1],
m_dPosition[2] ); lines->InsertNextCell( 2 ); lines->InsertCellPoint( n++ );
  lines->InsertCellPoint( n++ );
  points->InsertNextPoint( m_dPosition[0], m_dPosition[1] + dLen, m_dPosition[2]
); points->InsertNextPoint( m_dPosition[0], m_dPosition[1] - dLen,
m_dPosition[2] ); lines->InsertNextCell( 2 ); lines->InsertCellPoint( n++ );
  lines->InsertCellPoint( n++ );
  points->InsertNextPoint( m_dPosition[0], m_dPosition[1], m_dPosition[2] + dLen
); points->InsertNextPoint( m_dPosition[0], m_dPosition[1], m_dPosition[2] -
dLen ); lines->InsertNextCell( 2 ); lines->InsertCellPoint( n++ );
  lines->InsertCellPoint( n++ );
  ...
  polydata->SetLines( lines );
  vtkSmartPointer<vtkPolyDataMapper2D> mapper =
vtkSmartPointer<vtkPolyDataMapper2D>::New(); mapper->SetInput( polydata );
...
int Cursor2D::GetSize() { return m_nSize; }

double *Cursor2D::GetPosition() { return m_dPosition; }
...
void Cursor2D::Show(bool bShow) { m_actorCursor->SetVisibility(bShow ? 1 : 0); }

bool Cursor2D::IsShown() { return m_actorCursor->GetVisibility() > 0; }
