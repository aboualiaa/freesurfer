...
LayerPropertyMRI::LayerPropertyMRI(QObject *parent)
    : LayerProperty(parent), mColorMapType(LayerPropertyMRI::Grayscale),
      mResliceInterpolation(0), mTextureSmoothing(0), mbClearBackground(false),
      mMinVoxelValue(0), mMaxVoxelValue(0), mMinVisibleValue(0),
      mMaxVisibleValue(0), mMinGrayscaleWindow(0), mMaxGrayscaleWindow(0),
      mHeatScaleMinThreshold(0), mHeatScaleMidThreshold(0),
      mHeatScaleMaxThreshold(0), mHeatScaleOffset(0), mbReverseHeatScale(false),
      mbShowPositiveHeatScaleValues(true), mbShowNegativeHeatScaleValues(true),
      m_bHeatScaleClearHigh(false), m_bHeatScaleTruncate(false),
      m_bHeatScaleInvert(false), mOpacity(1), mFreeSurferCTAB(NULL),
      mbShowAsContour(false), mMinContourThreshold(0), mMaxContourThreshold(0),
      m_bDisplayVector(false), m_nVectorInversion(VI_None),
      m_nVectorRepresentation(VR_Line), m_bDisplayTensor(false),
      m_nTensorInversion(VI_None), m_nTensorRepresentation(TR_Boxoid),
      m_bContourUseImageColorMap(false), m_bContourExtractAll(true),
      m_bShowLabelOutline(false), m_nUpSampleMethod(UM_None),
      m_nContourSmoothIterations(5), mSource(NULL),
      m_bRememberFrameSettings(false), m_nActiveFrame(0),
      m_bShowAsLabelContour(false), m_bShowVoxelizedContour(false),
      m_bContourUpsample(false), m_dVectorScale(1.0), m_bUsePercentile(false),
      m_bAutoAdjustFrameLevel(false), m_bNormalizeVector(true),
      m_dVectorDisplayScale(1.0), m_bHeatScaleAutoMid(true),
      m_nProjectionMapType(0), m_bDisplayRGB(false), m_dVectorLineWidth(1),
      m_nVectorSkip(0), m_dVectorNormThreshold(0), m_colorBinary(Qt::white) {
  ...
  connect(this, SIGNAL(ContourSmoothIterationChanged(int)), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(DisplayModeChanged()), this, SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(LabelOutlineChanged(bool)), this,
          SIGNAL(PropertyChanged()));
  //  connect( this, SIGNAL(OpacityChanged(double)), this,
  //  SIGNAL(PropertyChanged()) );
  connect(this, SIGNAL(ResliceInterpolationChanged()), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(TextureSmoothingChanged()), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(UpSampleMethodChanged(int)), this,
          SIGNAL(PropertyChanged()));
  connect(this, SIGNAL(VectorLineWidthChanged(double)), this,
          SIGNAL(PropertyChanged()));
}
...
void LayerPropertyMRI::RestoreSettings(const QVariantMap &map) {
  ...
  if (map.contains("AutoAdjustFrameLevel") &&
      map["AutoAdjustFrameLevel"].toBool()) {
  ...
  if (m_bUsePercentile) {
    if (map.contains("MinGrayscaleWindowInPercentile")) {
      mMinGrayscaleWindow = mri->GetHistoValueFromPercentile(
          map["MinGrayscaleWindowInPercentile"].toDouble());
    }

    if (map.contains("MaxGrayscaleWindowInPercentile")) {
      mMaxGrayscaleWindow = mri->GetHistoValueFromPercentile(
          map["MaxGrayscaleWindowInPercentile"].toDouble());
    }

    if (map.contains("HeatScaleMinThresholdInPercentile")) {
      mHeatScaleMinThreshold = mri->GetHistoValueFromPercentile(
          map["HeatScaleMinThresholdInPercentile"].toDouble());
    }

    if (map.contains("HeatScaleMidThresholdInPercentile")) {
      mHeatScaleMidThreshold = mri->GetHistoValueFromPercentile(
          map["HeatScaleMidThresholdInPercentile"].toDouble());
    }

    if (map.contains("HeatScaleMaxThresholdInPercentile")) {
      mHeatScaleMaxThreshold = mri->GetHistoValueFromPercentile(
          map["HeatScaleMaxThresholdInPercentile"].toDouble());
    }

    if (map.contains("MinGenericThresholdInPercentile")) {
      mMinGenericThreshold = mri->GetHistoValueFromPercentile(
          map["MinGenericThresholdInPercentile"].toDouble());
    }

    if (map.contains("MaxGenericThresholdInPercentile")) {
      mMaxGenericThreshold = mri->GetHistoValueFromPercentile(
          map["MaxGenericThresholdInPercentile"].toDouble());
    }
  }
...
QVariantMap LayerPropertyMRI::GetActiveSettings() {
  ...
  case Grayscale:
    ...
    map["MaxGrayscaleWindow"] = mMaxGrayscaleWindow;
    if (m_bUsePercentile) {
      map["MinGrayscaleWindowInPercentile"] =
          mri->GetHistoPercentileFromValue(mMinGrayscaleWindow);
      map["MaxGrayscaleWindowInPercentile"] =
          mri->GetHistoPercentileFromValue(mMaxGrayscaleWindow);
    }
    break;
  case Heat:
    map["HeatScaleMinThreshold"] = mHeatScaleMinThreshold;
    ...
    map["HeatScaleOffset"]       = mHeatScaleOffset;
    if (m_bUsePercentile) {
      map["HeatScaleMinThresholdInPercentile"] =
          mri->GetHistoPercentileFromValue(mHeatScaleMinThreshold);
      map["HeatScaleMidThresholdInPercentile"] =
          mri->GetHistoPercentileFromValue(mHeatScaleMidThreshold);
      map["HeatScaleMaxThresholdInPercentile"] =
          mri->GetHistoPercentileFromValue(mHeatScaleMaxThreshold);
    }
    break;
  case Jet:
  case GEColor:
  ...
  case PET:
    ...
    map["MaxGenericThreshold"] = mMaxGenericThreshold;
    if (m_bUsePercentile) {
      map["MinGenericThresholdInPercentile"] =
          mri->GetHistoPercentileFromValue(mMinGenericThreshold);
      map["MaxGenericThresholdInPercentile"] =
          mri->GetHistoPercentileFromValue(mMaxGenericThreshold);
    }
    break;
  }
...
COLOR_TABLE *LayerPropertyMRI::GetLUTCTAB() const { return mFreeSurferCTAB; }
  ...
  if (mFreeSurferCTAB) {
    ...
    for (int nEntry = 1; nEntry < cEntries; nEntry++) {
      ...
      if (bValid) {
        ...
        mLUTTable->AddRGBAPoint(nEntry, red, green, blue,
                                m_listVisibleLabels.contains(nEntry) ? 1 : 0);
...
void LayerPropertyMRI::SetCustomColors(const QMap<int, QColor> &colors) {
  ...
  foreach (int id, idx) {
    mLUTTable->AddRGBAPoint(id, colors[id].redF(), colors[id].greenF(),
                            colors[id].blueF(), colors[id].alphaF());
  }
  mLUTTable->Build();
}
...
void LayerPropertyMRI::OnColorMapChanged() {
  ...
  case Grayscale:
    ...
    if (mbClearBackground) {
      if (mClearBackgroundValue < MinGrayscaleWindow) {
        ...
        mGrayScaleTable->AddRGBAPoint(mClearBackgroundValue + tiny_fraction, 0,
                                      0, 0, 1);
        mGrayScaleTable->AddRGBAPoint(MinGrayscaleWindow, 0, 0, 0, 1);
      } else {
        mGrayScaleTable->AddRGBAPoint(mClearBackgroundValue, 0, 0, 0, 0);
        double val = (mClearBackgroundValue - mMinGrayscaleWindow) /
                     (mMaxGrayscaleWindow == mMinGrayscaleWindow
                          ? 1
                          : (mMaxGrayscaleWindow - mMinGrayscaleWindow));
        mGrayScaleTable->AddRGBAPoint(mClearBackgroundValue + tiny_fraction,
                                      val, val, val, 1);
      }
    } else {
      mGrayScaleTable->AddRGBAPoint(mMinVisibleValue - tiny_fraction, 0, 0, 0,
                                    0);
    ...
    //    mGrayScaleTable->AddRGBAPoint( mMaxVisibleValue + tiny_fraction, 1, 1,
    //    1, 0 );
  ...
  case Heat:
    ...
    if (m_bHeatScaleTruncate && m_bHeatScaleInvert) {
      if (m_bHeatScaleClearHigh) {
        mHeatScaleTable->AddRGBAPoint(-HeatScaleMaxThreshold + HeatScaleOffset -
                                          tiny_fraction,
                                      1, 1, 0, 0);
      }
      mHeatScaleTable->AddRGBAPoint(-HeatScaleMaxThreshold + HeatScaleOffset, 1,
                                    1, 0, 1);
      mHeatScaleTable->AddRGBAPoint(-HeatScaleMidThreshold + HeatScaleOffset, 1,
                                    0, 0, 1);
      mHeatScaleTable->AddRGBAPoint(-HeatScaleMinThreshold + HeatScaleOffset, 1,
                                    0, 0, 0);
      ...
      mHeatScaleTable->AddRGBAPoint(HeatScaleMinThreshold + HeatScaleOffset, 0,
                                    0, 1, 0);
      mHeatScaleTable->AddRGBAPoint(HeatScaleMidThreshold + HeatScaleOffset, 0,
                                    0, 1, 1);
      mHeatScaleTable->AddRGBAPoint(HeatScaleMaxThreshold + HeatScaleOffset, 0,
                                    1, 1, 1);
      if (m_bHeatScaleClearHigh) {
        mHeatScaleTable->AddRGBAPoint(HeatScaleMaxThreshold + HeatScaleOffset +
                                          tiny_fraction,
  ...
  case Jet:
    ...
    mColorMapTable->AddRGBAPoint(
        MinGenericThreshold + (MaxGenericThreshold - MinGenericThreshold) / 4,
        0, 1, 1, 1);
    mColorMapTable->AddRGBAPoint(
        (MinGenericThreshold + MaxGenericThreshold) / 2, 0, 1, 0, 1);
    mColorMapTable->AddRGBAPoint(
        MaxGenericThreshold - (MaxGenericThreshold - MinGenericThreshold) / 4,
        1, 1, 0, 1);
    mColorMapTable->AddRGBAPoint(MaxGenericThreshold, 1, 0, 0, 1);
    //  mColorMapTable->AddRGBAPoint( mMaxGenericThreshold +
    //  (mMaxGenericThreshold - mMinJGenericThreshold), 1, 0, 0, 1 );
  ...
  mColorMapTable->AddRGBAPoint(mMinGenericThreshold, 0, 0, 0, 0);
  {
    ...
    for (int i = 1; i < 256; i++) {
      mColorMapTable->AddRGBAPoint(mMinGenericThreshold + stepsize * i,
                                   colors[i][0] / 255.0, colors[i][1] / 255.0,
                                   colors[i][2] / 255.0, 1);
    }
  }
  mColorMapTable->Build();
...
int LayerPropertyMRI::GetColorMap() const { return mColorMapType; }

void LayerPropertyMRI::SetColorMapToGrayScale() { SetColorMap(Grayscale); }

void LayerPropertyMRI::SetColorMapToHeatScale() { SetColorMap(Heat); }

void LayerPropertyMRI::SetColorMapToLUT() { SetColorMap(LUT); }
...
int LayerPropertyMRI::GetTextureSmoothing() const { return mTextureSmoothing; }
...
double LayerPropertyMRI::GetMinVisibleValue() { return mMinVisibleValue; }
...
double LayerPropertyMRI::GetMaxVisibleValue() { return mMaxVisibleValue; }

void LayerPropertyMRI::SetMinMaxVisibleValue(double iMinValue,
                                             double iMaxValue) {
  if (mMinVisibleValue != iMinValue || mMaxVisibleValue != iMaxValue) {
...
void LayerPropertyMRI::SetWindow(double iWindow) {
  ...
  this->SetMinMaxGrayscaleWindow(level - iWindow / 2.0, level + iWindow / 2.0);
}
...
void LayerPropertyMRI::SetLevel(double iLevel) {
  ...
  this->SetMinMaxGrayscaleWindow(iLevel - window / 2.0, iLevel + window / 2.0);
}
...
double LayerPropertyMRI::GetLevel() {
  ...
  return ((MaxGrayscaleWindow - MinGrayscaleWindow) / 2.0) + MinGrayscaleWindow;
}

void LayerPropertyMRI::SetWindowLevel(double iWindow, double iLevel) {
  this->SetMinMaxGrayscaleWindow(iLevel - iWindow / 2.0,
                                 iLevel + iWindow / 2.0);
}
...
void LayerPropertyMRI::SetMinMaxGrayscaleWindow(double iMin, double iMax) {
  ...
  if (MinGrayscaleWindow != iMin || MaxGrayscaleWindow != iMax) {
    if (m_bRememberFrameSettings) {
      QVariantMap map =
          m_frameSettings[QString::number(m_nActiveFrame)].toMap();
  ...
  if (bAutoMid != m_bHeatScaleAutoMid) {
    ...
    if (bAutoMid) {
      if (bAutoMidToMin)
        ...
        mHeatScaleMidThreshold =
            (mHeatScaleMinThreshold + mHeatScaleMaxThreshold) / 2;
    }
    this->OnColorMapChanged();
  }
}
...
bool LayerPropertyMRI::GetReverseHeatScale() { return mbReverseHeatScale; }
...
double LayerPropertyMRI::GetOpacity() const { return mOpacity; }
...
bool LayerPropertyMRI::GetClearBackground() { return mbClearBackground; }
...
double LayerPropertyMRI::GetMinValue() { return mMinVoxelValue; }

double LayerPropertyMRI::GetMaxValue() { return mMaxVoxelValue; }
...
void LayerPropertyMRI::SetVolumeSource(FSVolume *source) {
  ...
  m_dVectorScale =
      qMin(qMin(voxel_size[0], voxel_size[1]), voxel_size[2]) / 2.0;
...
void LayerPropertyMRI::UpdateMinMaxValues() {
  ...
  if (mSource->HasValidHistogram()) {
    if (m_mapMinMaxValues.contains(m_nActiveFrame)) {
      ...
      mMinGrayscaleWindow =
          mSource->GetHistoValueFromPercentile(0.05, m_nActiveFrame);
      mMaxGrayscaleWindow =
          mSource->GetHistoValueFromPercentile(0.99, m_nActiveFrame);
      mMaxGrayscaleWindow =
          mMaxGrayscaleWindow + (mMaxGrayscaleWindow - mMinGrayscaleWindow) / 5;
