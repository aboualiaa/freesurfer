...
double KDE::pdf(vector<double> &data) {
  ...
  for (unsigned int i = 0; i < data_matrix[0].size(); i++) {
    ...
    for (curr_var = 0; curr_var < data_matrix.size(); curr_var++) {
      switch (kernel) {
      case 2:
        a *= box_pdf(data[curr_var], data_matrix[curr_var][i],
                     bandwidth_map[curr_var]);
        break;
      case 3:
        a *= epanechnikov_pdf(data[curr_var], data_matrix[curr_var][i],
                              bandwidth_map[curr_var]);
        break;
      default:
        a *= gauss_pdf(data[curr_var], data_matrix[curr_var][i],
                       bandwidth_map[curr_var]);
        break;
      }
    }
...
double KDE::cdf(vector<double> &data) {
  ...
  for (unsigned int i = 0; i < data_matrix[0].size(); i++) {
    ...
    for (curr_var = 0; curr_var < data_matrix.size(); curr_var++) {
      switch (kernel) {
      case 2:
        a *= box_cdf(data[curr_var], data_matrix[curr_var][i],
                     bandwidth_map[curr_var]);
        break;
      case 3:
        a *= epanechnikov_cdf(data[curr_var], data_matrix[curr_var][i],
                              bandwidth_map[curr_var]);
        break;
      default:
        a *= gauss_cdf(data[curr_var], data_matrix[curr_var][i],
                       bandwidth_map[curr_var]);
        break;
      }
    }
...
void KDE::optimal_bandwidth(int maxiters, double eps) {
  ...
  double y0 = optimal_bandwidth_equation(
      x0, get_min(curr_var), get_max(curr_var), data_matrix[curr_var]);
  double x = 0.8 * x0;
  ...
  while (i < maxiters) {
    x -= y * (x0 - x) / (y0 - y);
    y = optimal_bandwidth_equation(x, get_min(curr_var), get_max(curr_var),
                                   data_matrix[curr_var]);
    if (abs(y) < eps * y0) {
      break;
    }
...
void KDE::optimal_bandwidth_safe(double eps) {
  ...
  double y0 = optimal_bandwidth_equation(
      x0, min_map[curr_var], max_map[curr_var], data_matrix[curr_var]);
  double y1 = optimal_bandwidth_equation(
      x1, min_map[curr_var], max_map[curr_var], data_matrix[curr_var]);

  if (y0 * y1 >= 0) {
    //	std::cerr << "Interval [ f(x0=$x0)=$y0 : f(x1=$x1)=$y1 ] does not
    //bracket root." << std::endl;
  }
  ...
  while (abs(x0 - x1) > eps * x1) {
    i += 1;
    x = (x0 + x1) / 2;
    y = optimal_bandwidth_equation(x, min_map[curr_var], max_map[curr_var],
                                   data_matrix[curr_var]);
