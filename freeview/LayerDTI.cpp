...
LayerDTI::LayerDTI(LayerMRI *ref, QObject *parent)
    : LayerMRI(ref, parent), m_vectorSource(NULL), m_eigenvalueSource(NULL) {
...
bool LayerDTI::LoadDTIFromFile() {
  ...
  if (!m_vectorSource->MRIRead(m_sVectorFileName, m_sRegFilename.isEmpty()
                                                      ? NULL
                                                      : m_sRegFilename)) {
    return false;
  }
  ...
  if (!m_sEigenvalueFileName.isEmpty()) {
    ...
    if (!m_eigenvalueSource->MRIRead(
            m_sEigenvalueFileName,
            m_sRegFilename.isEmpty() ? NULL : m_sRegFilename)) {
      return false;
    } else {
      double scale = GetProperty()->GetVectorScale() /
                     m_eigenvalueSource->GetMaxValue() * 2;
...
void LayerDTI::UpdateVectorActor(int nPlane) {
  LayerMRI::UpdateVectorActor(
      nPlane, m_vectorSource->GetImageOutput(),
