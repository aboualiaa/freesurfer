...
VolumeFilter::VolumeFilter(LayerMRI *input, LayerMRI *output, QObject *parent)
    : QObject(parent), m_nKernelSize(3) {
...
void VolumeFilter::SetInputOutputVolumes(LayerMRI *input, LayerMRI *output) {
  ...
  if (m_volumeInput) {
    connect(m_volumeInput, SIGNAL(destroyed()), this,
            SLOT(OnLayerObjectDeleted()), Qt::UniqueConnection);
  }
  if (m_volumeOutput) {
    connect(m_volumeOutput, SIGNAL(destroyed()), this,
            SLOT(OnLayerObjectDeleted()), Qt::UniqueConnection);
  }
}
...
bool VolumeFilter::ReadyToUpdate() { return (m_volumeInput && m_volumeOutput); }
        ...
        for (int nFrame = 0; nFrame < mri->nframes; nFrame++) {
          ...
          case MRI_UCHAR:
            MRIseq_vox(mri, i, j, k, nFrame) =
                (unsigned char)MyVTKUtils::GetImageDataComponent(
                    ptr, dim, n_frames, i, j, k, nFrame, scalar_type);
            break;
          case MRI_INT:
            MRIIseq_vox(mri, i, j, k, nFrame) =
                (int)MyVTKUtils::GetImageDataComponent(ptr, dim, n_frames, i, j,
                                                       k, nFrame, scalar_type);
            break;
          case MRI_LONG:
            MRILseq_vox(mri, i, j, k, nFrame) =
                (long)MyVTKUtils::GetImageDataComponent(
                    ptr, dim, n_frames, i, j, k, nFrame, scalar_type);
            break;
          case MRI_FLOAT:
            MRIFseq_vox(mri, i, j, k, nFrame) =
                (float)MyVTKUtils::GetImageDataComponent(
                    ptr, dim, n_frames, i, j, k, nFrame, scalar_type);
            break;
          case MRI_SHORT:
        ...
        for (int nFrame = 0; nFrame < zFrames; nFrame++) {
          ...
          case MRI_UCHAR:
            MyVTKUtils::SetImageDataComponent(
                ptr, dim, n_frames, nX, nY, nZ, nFrame, scalar_type,
