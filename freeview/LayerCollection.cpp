...
LayerCollection::~LayerCollection() { Clear(); }
...
bool LayerCollection::IsEmpty() { return m_layers.size() == 0; }
...
bool LayerCollection::AddLayer(Layer *layer, bool initializeCoordinate) {
  ...
  connect(layer, SIGNAL(NameChanged(QString)), this,
  ...
  if (layer->GetProperty()) {
    connect(layer->GetProperty(), SIGNAL(PropertyChanged()), this,
            SIGNAL(LayerPropertyChanged()));
    connect(layer->GetProperty(), SIGNAL(ShowInfoChanged(bool)), this,
            SIGNAL(LayerShowInfoChanged()));
  }
  connect(layer, SIGNAL(VisibilityChanged(bool)), this,
...
bool LayerCollection::RemoveLayers(QList<Layer *> layers) {
  ...
  foreach (Layer *layer, layers) { emit LayerRemoved(layer); }
  return true;
}
...
int LayerCollection::GetNumberOfLayers() { return (int)m_layers.size(); }
...
Layer *LayerCollection::GetActiveLayer() { return m_layerActive; }

bool LayerCollection::SetSlicePosition(int nPlane, double dPos_in,
                                       bool bRoundToGrid) {
  double dPos = dPos_in;
  ...
  if (bRoundToGrid) {
    ...
    if (layer && layer->IsTypeOf("MRI")) {
      ...
      for (int i = 0; i < 3; i++)
        origin[i] = wo[i] - ((int)((wo[i] - origin[i]) / voxel_size[i] + 1)) *
                                voxel_size[i];
      layer->GetWorldVoxelSize(voxel_size);
    }
    dPos = ((int)((dPos - origin[nPlane]) / voxel_size[nPlane])) *
               voxel_size[nPlane] +
           origin[nPlane];
  }
...
bool LayerCollection::OffsetSlicePosition(int nPlane, double dPosDiff,
                                          bool bRoundToGrid) {
  return SetSlicePosition(nPlane, m_dSlicePosition[nPlane] + dPosDiff,
                          bRoundToGrid);
}
...
double *LayerCollection::GetSlicePosition() { return m_dSlicePosition; }
...
double *LayerCollection::GetCursorRASPosition() { return m_dCursorRASPosition; }
...
QList<Layer *> LayerCollection::GetLayers() { return m_layers; }

QString LayerCollection::GetType() { return m_strType; }

double *LayerCollection::GetWorldOrigin() { return m_dWorldOrigin; }
...
double *LayerCollection::GetWorldSize() { return m_dWorldSize; }
...
double *LayerCollection::GetWorldVoxelSize() { return m_dWorldVoxelSize; }
