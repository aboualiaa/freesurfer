...
void SurfacePath::SetColor(const QColor &color) {
  m_color = color;
  m_actorOutline->GetProperty()->SetColor(color.redF(), color.greenF(),
                                          color.blueF());
  emit ColorChanged(color);
}

QColor SurfacePath::GetColor() { return m_color; }

void SurfacePath::Show(bool bShow) {
  m_actorOutline->SetVisibility(bShow ? 1 : 0);
}

vtkActor *SurfacePath::GetActor() { return m_actorOutline; }

bool SurfacePath::Contains(int nvo) { return m_listVertices.contains(nvo); }
