...
PanelTrack::PanelTrack(QWidget *parent)
    : PanelLayer("Tract", parent), ui(new Ui::PanelTrack) {
  ui->setupUi(this);
  ...
  m_widgetlistDirectionalColor
      << ui->labelDirectionScheme << ui->comboBoxDirectionScheme
      << ui->labelDirectionMapping << ui->comboBoxDirectionMapping;
  m_widgetlistSolidColor << ui->labelSolidColor << ui->colorPickerSolidColor;
  connect(ui->pushButtonShowClusterMap, SIGNAL(clicked()), mainwnd,
          SLOT(ShowTractClusterMap()));
}

PanelTrack::~PanelTrack() { delete ui; }
...
void PanelTrack::ConnectLayer(Layer *layer_in) {
  ...
  connect(p, SIGNAL(PropertyChanged()), this, SLOT(UpdateWidgets()),
          Qt::UniqueConnection);
  connect(ui->comboBoxColorCode, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetColorCode(int)));
  connect(ui->comboBoxDirectionScheme, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetDirectionScheme(int)));
  connect(ui->comboBoxDirectionMapping, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetDirectionMapping(int)));
  connect(ui->colorPickerSolidColor, SIGNAL(colorChanged(QColor)), p,
          SLOT(SetSolidColor(QColor)));
  connect(ui->comboBoxRenderRep, SIGNAL(currentIndexChanged(int)), p,
          SLOT(SetRenderRep(int)));
  connect(ui->sliderOpacity, SIGNAL(valueChanged(int)),
          SLOT(OnSliderOpacity(int)));
  connect(ui->lineEditOpacity, SIGNAL(textChanged(QString)),
          SLOT(OnLineEditOpacity(QString)));
}
...
void PanelTrack::DoUpdateWidgets() {
  ...
  */
  ...
  if (layer) {
    ...
    ui->comboBoxColorCode->setCurrentIndex(
        layer->GetProperty()->GetColorCode());
    ui->comboBoxColorCode->setItemData(LayerPropertyTrack::EmbeddedColor,
                                       layer->HasEmbeddedColor() ? 33 : 0,
                                       Qt::UserRole - 1);
    ui->comboBoxDirectionMapping->setCurrentIndex(
        layer->GetProperty()->GetDirectionMapping());
    ui->comboBoxDirectionScheme->setCurrentIndex(
        layer->GetProperty()->GetDirectionScheme());
    ui->colorPickerSolidColor->setCurrentColor(
        layer->GetProperty()->GetSolidColor());
    ui->comboBoxRenderRep->setCurrentIndex(
        layer->GetProperty()->GetRenderRep());
    ChangeLineEditNumber(ui->lineEditOpacity,
                         layer->GetProperty()->GetOpacity());
    ui->sliderOpacity->setValue(layer->GetProperty()->GetOpacity() * 100);
  }
  ShowWidgets(m_widgetlistDirectionalColor,
              layer && layer->GetProperty()->GetColorCode() ==
                           LayerPropertyTrack::Directional);
  ShowWidgets(m_widgetlistSolidColor,
              layer && layer->GetProperty()->GetColorCode() ==
                           LayerPropertyTrack::SolidColor);
