...
WindowTimeCourse::WindowTimeCourse(QWidget *parent)
    : QWidget(parent), ui(new Ui::WindowTimeCourse), lastMRI(NULL),
      lastSurface(NULL), lastOverlay(NULL) {
  ui->setupUi(this);
  ...
  connect(ui->widgetPlot, SIGNAL(FrameChanged(int)), this,
          SLOT(OnFrameChanged(int)), Qt::QueuedConnection);
  connect(ui->widgetPlot, SIGNAL(PlotRangeChanged()), this,
  ...
  ui->checkBoxAutoScale->setChecked(
      s.value("WindowTimeCourse/AutoScale", true).toBool());
  ...
  if (type == "MRI") {
    ...
    for (int nl = layers.size() - 1; nl >= 0; nl--) {
      ...
      if (layer && layer->GetNumberOfFrames() > 1) {
        ...
        connect(layer, SIGNAL(CorrelationSurfaceChanged(LayerSurface *)), this,
                SLOT(OnLayerCorrelationSurfaceChanged()), Qt::UniqueConnection);
    ...
    LayerSurface *surf = qobject_cast<LayerSurface *>(
        MainWindow::GetMainWindow()->GetActiveLayer(type));
    if (surf) {
      Clear();
      double pos[3];
      MainWindow::GetMainWindow()
          ->GetLayerCollection("Surface")
          ->GetSlicePosition(pos);
      int nVert = -1;
      if (surf->GetFileName().contains("inflated", Qt::CaseInsensitive) &&
...
void WindowTimeCourse::OnFrameChanged(int frame) {
  ...
  if (type != "MRI" && type != "Surface")
    type = "MRI";
  if (type == "MRI") {
    LayerMRI *layer = qobject_cast<LayerMRI *>(
        MainWindow::GetMainWindow()->GetActiveLayer("MRI"));
    if (layer && frame != layer->GetActiveFrame() &&
        frame < layer->GetNumberOfFrames()) {
...
void WindowTimeCourse::OnLineEditScaleReturnPressed() {
  QStringList list =
      ui->lineEditScale->text().trimmed().split(",", QString::SkipEmptyParts);
  if (list.size() != 2)
    list =
        ui->lineEditScale->text().trimmed().split(" ", QString::SkipEmptyParts);
  if (list.size() != 2)
...
void WindowTimeCourse::OnComboSecondPlot(int nSel) { UpdateData(); }
