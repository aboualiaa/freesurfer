...
LayerVolumeBase::LayerVolumeBase(QObject *parent) : LayerEditable(parent) {
  ...
  connect(m_propertyBrush, SIGNAL(FillValueChanged(double)), this,
          SLOT(SetFillValue(double)));
  if (GetEndType() != "ROI")
    connect(m_propertyBrush, SIGNAL(EraseValueChanged(double)), this,
            SLOT(SetBlankValue(double)));
  connect(m_propertyBrush, SIGNAL(BrushSizeChanged(int)), this,
  ...
  for (int i = -nsize[0] + 1; i < nsize[0]; i++) {
    for (int j = -nsize[1] + 1; j < nsize[1]; j++) {
      for (int k = -nsize[2] + 1; k < nsize[2]; k++) {
        ...
        n[1] = n_in[1] + j;
        n[2] = n_in[2] + k;
        if (n[0] >= 0 && n[0] < nDim[0] && n[1] >= 0 && n[1] < nDim[1] &&
            n[2] >= 0 && n[2] < nDim[2] &&
            MyUtils::GetDistance<int>(n, n_in) <= nBrushSize / 2.0) {
          double fvalue = MyVTKUtils::GetImageDataComponent(
              ref_ptr, ref_dim, ref_n_frames, n[0], n[1], n[2], nActiveCompRef,
              ref_scalar_type);
          if (bAdd) {
            if ((bNotROI && m_propertyBrush->GetDrawRangeEnabled() &&
              ...
              //   m_imageData->SetScalarComponentFromFloat( n[0], n[1], n[2],
              //   nActiveComp, m_fFillValue );
              MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, n[0], n[1],
                                                n[2], nActiveComp, scalar_type,
                                                m_fFillValue);
              indices << n[0] << n[1] << n[2];
  ...
  for (int i = nBounds[0]; i <= nBounds[1]; i++) {
    for (int j = nBounds[2]; j <= nBounds[3]; j++) {
      for (int k = nBounds[4]; k <= nBounds[5]; k++) {
        ...
        n[1] = n_in[1] + j;
        n[2] = n_in[2] + k;
        if (abs(i + j + k) == 1 && n[0] >= 0 && n[0] < nDim[0] && n[1] >= 0 &&
            n[1] < nDim[1] && n[2] >= 0 && n[2] < nDim[2]) {
          double fvalue = MyVTKUtils::GetImageDataComponent(
              ptr, nDim, n_frames, n[0], n[1], n[2], nFrame, scalar_type);
          if (fvalue > 0) {
            return true;
          }
        }
...
QVector<int> LayerVolumeBase::SetVoxelByIndex(int *n1, int *n2, int nPlane,
                                              bool bAdd,
                                              bool ignore_brush_size) {
  int nx = 1, ny = 2;
  if (nPlane == 1) {
...
QVector<int> LayerVolumeBase::BorderFillByRAS(int *n, int nPlane) {
  ...
  case 0:
    for (int i = 0; i < nx; i++) {
      for (int j = 0; j < ny; j++) {
        if (fabs(MyVTKUtils::GetImageDataComponent(
                     ref_ptr, ref_dim, ref_n_frames, i, j, 0, nActiveCompRef,
                     scalar_type) -
                 fVoxelValue) <= fTolerance) {
          MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, n[nPlane], i, j,
    ...
    for (int i = 0; i < nx; i++) {
      for (int j = 0; j < ny; j++) {
        if (fabs(MyVTKUtils::GetImageDataComponent(
                     ...
                     ref_scalar_type) -
                 fVoxelValue) <= fTolerance) {
          MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, i, n[nPlane], j,
                                            nActiveComp, scalar_type,
                                            m_fFillValue);
    ...
    for (int i = 0; i < nx; i++) {
      for (int j = 0; j < ny; j++) {
        if (fabs(MyVTKUtils::GetImageDataComponent(
                     ...
                     ref_scalar_type) -
                 fVoxelValue) <= fTolerance) {
          MyVTKUtils::SetImageDataComponent(ptr, dim, n_frames, i, j, n[nPlane],
                                            nActiveComp, scalar_type,
                                            m_fFillValue);
...
bool LayerVolumeBase::FloodFillByRAS(double *ras, int nPlane, bool bAdd,
  ...
  if (!b3D) {
    QVector<int> list =
        FloodFillByIndex(n, nPlane, bAdd, true, mask_out, ignore_exclusion);
    if (!list.isEmpty()) {
      if (!mask_out) {
        SetModified();
      }
...
QVector<int> LayerVolumeBase::FloodFillByIndex(int *n, int nPlane, bool bAdd,
  ...
  double fTolerance = (fRange[1] - fRange[0]) *
                      m_propertyBrush->GetBrushTolerance() /
                      100.0; // tolerance is percentage
  bool bNotROI = (GetEndType() != "ROI");
  switch (nPlane) {
  case 0:
    if (ref == m_imageData) {
      for (i = 0; i < nx; i++) {
        for (j = 0; j < ny; j++) {
          mask[j][i] = (fabs(MyVTKUtils::GetImageDataComponent(
                                 ptr, dim, n_frames, n[nPlane], i, j,
                                 nActiveCompRef, scalar_type) -
                             fVoxelValue) <= fTolerance
                            ? 1
                            : 0);
        }
      ...
      for (i = 0; i < nx; i++) {
        for (j = 0; j < ny; j++) {
          mask[j][i] =
              (MyVTKUtils::GetImageDataComponent(ptr, dim, n_frames, n[nPlane],
                                                 i, j, nActiveComp,
                                                 scalar_type) <= 0 &&
                       fabs(MyVTKUtils::GetImageDataComponent(
                                ref_ptr, ref_dim, ref_n_frames, n[nPlane], i, j,
                                nActiveCompRef, scalar_type) -
                            fVoxelValue) <= fTolerance
                   ? 1
                   : 0);
  ...
  case 1:
    if (ref == m_imageData) {
      ...
      for (i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
          mask[j][i] =
              (MyVTKUtils::GetImageDataComponent(ptr, dim, n_frames, i,
                                                 n[nPlane], j, nActiveComp,
                                                 scalar_type) <= 0 &&
                       fabs(MyVTKUtils::GetImageDataComponent(
                                ref_ptr, ref_dim, ref_n_frames, i, n[nPlane], j,
                                nActiveCompRef, ref_scalar_type) -
                            fVoxelValue) <= fTolerance
                   ? 1
                   : 0);
        }
  ...
  case 2:
    if (ref == m_imageData) {
      ...
      for (i = 0; i < nx; i++) {
        for (j = 0; j < ny; j++) {
          mask[j][i] =
              (MyVTKUtils::GetImageDataComponent(ptr, dim, n_frames, i, j,
                                                 n[nPlane], nActiveComp,
                                                 scalar_type) <= 0 &&
                       fabs(MyVTKUtils::GetImageDataComponent(
                                ref_ptr, ref_dim, ref_n_frames, i, j, n[nPlane],
                                nActiveCompRef, ref_scalar_type) -
                            fVoxelValue) <= fTolerance
                   ? 1
                   : 0);
  ...
  case 0:
    ...
    for (i = 0; i < nx; i++) {
      for (j = 0; j < ny; j++) {
        if (mask[j][i] == 2) {
          double fvalue = MyVTKUtils::GetImageDataComponent(
              ref_ptr, ref_dim, ref_n_frames, n[nPlane], i, j, nActiveCompRef,
              ref_scalar_type);
          if ((bNotROI && m_propertyBrush->GetDrawRangeEnabled() &&
               (fvalue < draw_range[0] || fvalue > draw_range[1])) ||
              (bNotROI && !ignore_exclusion &&
            ...
            if (mask_out) {
              ...
              MyVTKUtils::SetImageDataComponent(
                  ptr, dim, n_frames, n[nPlane], i, j, nActiveComp, scalar_type,
                  bAdd ? m_fFillValue : m_fBlankValue);
            }
            voxel_list << n[nPlane] << i << j;
          }
        }
  ...
  case 1:
    ...
    for (i = 0; i < nx; i++) {
      for (j = 0; j < ny; j++) {
        if (mask[j][i] == 2) {
          double fvalue = MyVTKUtils::GetImageDataComponent(
              ref_ptr, ref_dim, ref_n_frames, i, n[nPlane], j, nActiveCompRef,
              ref_scalar_type);
          if ((bNotROI && m_propertyBrush->GetDrawRangeEnabled() &&
               (fvalue < draw_range[0] || fvalue > draw_range[1])) ||
              (bNotROI && m_propertyBrush->GetExcludeRangeEnabled() &&
            ...
            if (mask_out) {
              ...
              MyVTKUtils::SetImageDataComponent(
                  ptr, dim, n_frames, i, n[nPlane], j, nActiveComp, scalar_type,
                  bAdd ? m_fFillValue : m_fBlankValue);
            }
            voxel_list << i << n[nPlane] << j;
          }
  ...
  case 2:
    ...
    for (i = 0; i < nx; i++) {
      for (j = 0; j < ny; j++) {
        if (mask[j][i] == 2) {
          double fvalue = MyVTKUtils::GetImageDataComponent(
              ref_ptr, ref_dim, ref_n_frames, i, j, n[nPlane], nActiveCompRef,
              ref_scalar_type);
          if ((bNotROI && m_propertyBrush->GetDrawRangeEnabled() &&
               (fvalue < draw_range[0] || fvalue > draw_range[1])) ||
              (bNotROI && m_propertyBrush->GetExcludeRangeEnabled() &&
            ...
            if (mask_out) {
              ...
              MyVTKUtils::SetImageDataComponent(
                  ptr, dim, n_frames, i, j, n[nPlane], nActiveComp, scalar_type,
                  bAdd ? m_fFillValue : m_fBlankValue);
            }
            voxel_list << i << j << n[nPlane];
          }
...
bool LayerVolumeBase::HasUndo() { return m_bufferUndo.size() > 0; }

bool LayerVolumeBase::HasRedo() { return m_bufferRedo.size() > 0; }
...
bool LayerVolumeBase::CopyStructure(int nPlane, double *ras) {
  ...
  if (m_imageData->GetScalarComponentAsDouble(nSlice[0], nSlice[1], nSlice[2],
                                              0) == 0) {
    return false;
  }
  ...
  if (FloodFillByRAS(ras, nPlane, true, false, mask, true)) {
    ...
    SaveBufferItem(m_bufferClipboard, nPlane, nSlice[nPlane], GetActiveFrame(),
                   mask);
    return true;
  } else {
    return false;
...
void LayerVolumeBase::SaveBufferItem(UndoRedoBufferItem &item, int nPlane,
                                     int nSlice, int nFrame, const char *mask) {
  item.plane = nPlane;
  item.slice = nSlice;
  item.frame = nFrame;
  if (nPlane >= 0) {
    ...
    for (size_t i = nStart[0]; i < (size_t)nStart[0] + nDim[0]; i++) {
      for (size_t j = nStart[1]; j < (size_t)nStart[1] + nDim[1]; j++) {
        for (size_t k = nStart[2]; k < (size_t)nStart[2] + nDim[2]; k++) {
          if (!mask || mask[n] > 0) {
            memcpy(item.data + ((k - nStart[2]) * nDim[1] * nDim[0] +
                                (j - nStart[1]) * nDim[0] + (i - nStart[0])) *
                                   scalar_size,
                   ptr +
                       ((k * nOrigDim[0] * nOrigDim[1] + j * nOrigDim[0] + i) *
                            n_frames +
                        nFrame) *
                           scalar_size,
                   scalar_size);
  ...
  } else // save whole volume in cache file
  {
    ...
    m_imageData->GetDimensions(nDim);
    long long nSize =
        ((long long)nDim[0]) * nDim[1] * nDim[2] *
...
void LayerVolumeBase::LoadBufferItem(UndoRedoBufferItem &item,
  ...
  if (item.plane >= 0) {
    ...
    for (size_t i = nStart[0]; i < (size_t)nStart[0] + nDim[0]; i++) {
      for (size_t j = nStart[1]; j < (size_t)nStart[1] + nDim[1]; j++) {
        for (size_t k = nStart[2]; k < (size_t)nStart[2] + nDim[2]; k++) {
          double dValue = MyVTKUtils::GetImageDataComponent(
              ptr, nOrigDim, n_frames, i, j, k, item.frame, scalar_type);
          memcpy(ptr + ((k * nOrigDim[0] * nOrigDim[1] + j * nOrigDim[0] + i) *
                            n_frames +
                        item.frame) *
                           scalar_size,
                 item.data + ((k - nStart[2]) * nDim[1] * nDim[0] +
                              (j - nStart[1]) * nDim[0] + (i - nStart[0])) *
                                 scalar_size,
                 scalar_size);
          if (bIgnoreZeros) {
            if (MyVTKUtils::GetImageDataComponent(ptr, nOrigDim, n_frames, i, j,
                                                  k, item.frame,
                                                  scalar_type) == 0) {
              MyVTKUtils::SetImageDataComponent(ptr, nOrigDim, n_frames, i, j,
                                                k, item.frame, scalar_type,
                                                dValue);
            }
          }
        }
    ...
    m_imageData->GetDimensions(nDim);
    int nSize =
        nDim[0] * nDim[1] * nDim[2] * m_imageData->GetScalarSize() *
        (item.frame >= 0 ? 1 : m_imageData->GetNumberOfScalarComponents());
    ;
    char *p = (char *)m_imageData->GetScalarPointer() +
              (item.frame >= 0 ? nSize * item.frame : 0);
    file.read(p, nSize);
    file.close();
    if (this->IsTypeOf("MRI")) {
      LayerMRI *mri = qobject_cast<LayerMRI *>(this);
...
double LayerVolumeBase::GetFillValue() { return m_fFillValue; }
...
double LayerVolumeBase::GetBlankValue() { return m_fBlankValue; }
...
int LayerVolumeBase::GetBrushRadius() { return m_nBrushRadius; }
...
QString LayerVolumeBase::GenerateCacheFileName() {
  QString strg =
      QDir::tempPath() + "/freeview-cache-" + QString::number(qrand());
...
void LayerVolumeBase::ClearVoxels() {
  int *dim = m_imageData->GetDimensions();
  memset(m_imageData->GetScalarPointer(), 0,
         ((size_t)dim[0]) * dim[1] * dim[2] * m_imageData->GetScalarSize());
  m_imageData->Modified();
}
    ...
    for (size_t j = nStart[1]; j < (size_t)nStart[1] + nDim[1]; j++) {
      for (size_t k = nStart[2]; k < (size_t)nStart[2] + nDim[2]; k++) {
        ...
        if (ii >= 0 && ii < nDim[0] && jj >= 0 && jj < nDim[1] && kk >= 0 &&
            kk < nDim[2])
          val = MyVTKUtils::GetImageDataComponent(
              m_shiftForegroundData, nDim, 1, ii, jj, kk, 0, scalar_type);
        if (val > 0)
          MyVTKUtils::SetImageDataComponent(ptr, nOrigDim, n_frames, i, j, k,
                                            nFrame, scalar_type, m_fFillValue);
        else
          memcpy(ptr + ((k * nOrigDim[0] * nOrigDim[1] + j * nOrigDim[0] + i) *
                            n_frames +
                        nFrame) *
                           scalar_size,
                 m_shiftBackgroundData +
                     ((k - nStart[2]) * nDim[1] * nDim[0] +
                      (j - nStart[1]) * nDim[0] + (i - nStart[0])) *
                         scalar_size,
                 scalar_size);
      }
    ...
    for (size_t j = nStart[1]; j < (size_t)nStart[1] + nDim[1]; j++) {
      for (size_t k = nStart[2]; k < (size_t)nStart[2] + nDim[2]; k++) {
        double val = MyVTKUtils::GetImageDataComponent(
            ptr, nOrigDim, n_frames, i, j, k, nFrame, scalar_type);
        if (val == m_fFillValue) {
          MyVTKUtils::SetImageDataComponent(m_shiftForegroundData, nDim, 1,
                                            i - nStart[0], j - nStart[1],
                                            k - nStart[2], 0, scalar_type, 1);
        } else {
          memcpy(m_shiftBackgroundData +
                     ((k - nStart[2]) * nDim[1] * nDim[0] +
                      (j - nStart[1]) * nDim[0] + (i - nStart[0])) *
                         scalar_size,
                 ptr + ((k * nOrigDim[0] * nOrigDim[1] + j * nOrigDim[0] + i) *
                            n_frames +
                        nFrame) *
                           scalar_size,
                 scalar_size);
