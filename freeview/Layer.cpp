...
double *Layer::GetWorldOrigin() { return m_dWorldOrigin; }
...
double *Layer::GetWorldSize() { return m_dWorldSize; }
...
double *Layer::GetWorldVoxelSize() { return m_dWorldVoxelSize; }
...
double *Layer::GetSlicePosition() { return m_dSlicePosition; }
...
void Layer::GetDisplayBounds(double *bounds) { this->GetBounds(bounds); }
...
void Layer::SetTranslateByCenterPosition(double *c_pos /* in target space */) {
  for (int i = 0; i < 3; i++) {
    double pos = ((int)(m_dWorldSize[i] / 2 / m_dWorldVoxelSize[i]) + 0.0) *
                     m_dWorldVoxelSize[i] +
                 m_dWorldOrigin[i];
