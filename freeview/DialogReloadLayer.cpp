...
DialogReloadLayer::DialogReloadLayer(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogReloadLayer) {
  ui->setupUi(this);
  ui->checkBoxCloseFirst->setChecked(true);
}

DialogReloadLayer::~DialogReloadLayer() { delete ui; }

int DialogReloadLayer::Execute(const QList<Layer *> &layers) {
  QString type = layers.first()->GetPrimaryType();
  if (type == "MRI")
    type = "Volume";
  ui->checkBoxCloseFirst->setText(
      tr("Close selected %1(s) first before reloading them").arg(type));
