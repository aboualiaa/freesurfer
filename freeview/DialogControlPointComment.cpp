...
DialogControlPointComment::DialogControlPointComment(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogControlPointComment) {
  ui->setupUi(this);

  m_listCheckBoxes << ui->checkBoxMain << ui->checkBoxMain_2
                   << ui->checkBoxMain_3 << ui->checkBoxMain_4
                   << ui->checkBoxMain_5 << ui->checkBoxMain_6
                   << ui->checkBoxMain_7 << ui->checkBoxSolution
                   << ui->checkBoxSolution_2 << ui->checkBoxSolution_3
                   << ui->checkBoxSolution_4 << ui->checkBoxProgress
                   << ui->checkBoxProgress_2 << ui->checkBoxProgress_3;

  foreach (QCheckBox *box, m_listCheckBoxes)
    connect(box, SIGNAL(toggled(bool)), SLOT(OnCheckBoxToggled(bool)),
            Qt::QueuedConnection);
}

DialogControlPointComment::~DialogControlPointComment() { delete ui; }
