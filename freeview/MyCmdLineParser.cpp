...
void MyCmdLineParser::AddValidCmdLineEntry(int nType, const char *shortname,
                                           const char *longname,
                                           const char *arguname,
                                           const char *desc, int min_num,
                                           int max_num) {
...
void MyCmdLineParser::PrintHelp() {
  ...
  while (desc.length() > CONSOLE_WIDTH) {
    ...
    if (n >= 0) {
      std::cout << desc.substr(0, n).c_str() << "\n";
    }
...
bool MyCmdLineParser::Parse(const string_array &args) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg.substr(0, 6) != "-psn_0") {
      if (arg[0] == '-' && arg.length() > 1 && !IsNumber(arg[1]) &&
          arg[1] != '.') {
        sa = new string_array;
        sa->clear();
  ...
  for (size_t i = 0; i < entries.size(); i++) {
    ...
    if (!IsValid(strgs[0].c_str(),
                 &e)) // && !IsValid( strgs[0].c_str() + 1, &e ) )
    {
      bSucceed = false;
  ...
  */
  ...
  for (size_t i = 0; i < m_cmdLineEntries.size(); i++) {
    ...
    else if (QString(m_cmdLineEntries[i].shortName) == "v" &&
             !default_colormap.isEmpty()) {
    ...
    } else if (QString(m_cmdLineEntries[i].shortName) == "f" &&
               !default_edgecolor.isEmpty()) {
      string_array new_args, old_args = m_cmdLineEntries[i].arguments;
      for (size_t n = 0; n < old_args.size(); n++) {
        QString strg = QString::fromStdString(old_args[n]);
