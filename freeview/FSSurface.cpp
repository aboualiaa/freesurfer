...
FSSurface::FSSurface(FSVolume *ref, QObject *parent)
  ...
  if (ref) {
    ...
    for (int i = 0; i < 16; i++) {
      m_targetToRasMatrix[i] =
          (double)*MATRIX_RELT((mat), (i / 4) + 1, (i % 4) + 1);
    }
  }
...
bool FSSurface::MRISRead(const QString &    filename,
  ...
  if (m_MRIS == NULL) {
    ...
    return InitializeData(vector_filename, patch_filename, target_filename,
                          sup_files);
  }
}
...
void FSSurface::UpdateHashTable(int nSet, int coord) {
  ...
  MRIScomputeVertexSpacingStats(m_MRIS, NULL, NULL, &max_spacing, NULL,
                                &max_vno, coord);
  m_HashTable[nSet] =
      MHTcreateVertexTable_Resolution(m_MRIS, coord, max_spacing / 2);
}
...
bool FSSurface::LoadSurface(const QString &filename, int nSet) {
  if (::MRISreadVertexPositions(m_MRIS, filename.toLatin1().data()) != 0) {
    ...
    UpdateHashTable(nSet,
                    (filename == "white" ? WHITE_VERTICES : CURRENT_VERTICES));
    ComputeNormals();
...
bool FSSurface::LoadOverlay(const QString &filename, const QString &fn_reg,
                            float **data_out, int *nvertices_out,
                            int *nframes_out, bool bUseSecondHalfData) {
  ...
  if (mriheader &&
    ...
    if (mriheader->height == 1 && mriheader->depth == 1) {
      ...
      printf("Number of vertices in overlay data (%d or %d) does not match "
             "with surface (%d).\n",
             mriheader->width * mriheader->height * mriheader->depth,
             mriheader->width * mriheader->height * mriheader->depth *
                 mriheader->nframes,
    ...
    if (!data)
      return false;
    for (int i = 0; i < m_MRIS->nvertices; i++) {
      double v[4] = {m_MRIS->vertices[i].x, m_MRIS->vertices[i].y,
                     m_MRIS->vertices[i].z, 1};
      vtkMatrix4x4::MultiplyPoint(m, v, v);
      int nx = (int)(v[0] + 0.5);
      int ny = (int)(v[1] + 0.5);
      int nz = (int)(v[2] + 0.5);
      if (nx >= 0 && nx < mri->width && ny >= 0 && ny < mri->height &&
          nz >= 0 && nz < mri->depth) {
...
{
  // user read curvature routine because read values does not handle filename
properly int nCount = m_MRIS->nvertices; for ( int i = 0; i < nCount; i++ )
    m_MRIS->vertices[i].val = m_MRIS->vertices[i].curv;
  float fMin = m_MRIS->min_curv;
  float fMax = m_MRIS->max_curv;

  if ( ::MRISreadCurvatureFile( m_MRIS, (char*)( filename.toLatin1().data()) )
!= 0 )
  {
  ...
  if (mri) {
    if (mri->type != MRI_FLOAT) {
      ...
      return true;
                 ...
                 m_MRIS, (char *)filename.toLatin1().data()) == 0) {
    // compute vectors
    if (ComputeVectors(m_MRIS, vector.data)) {
      m_vertexVectors.push_back(vector);
      m_nActiveVector = m_vertexVectors.size() - 1;

      // restore original vertices in m_MRIS because MRISreadVertexPositions
      // changed it!
...
void FSSurface::UpdatePolyData(MRIS *mris, vtkPolyData *polydata,
  ...
  for (int fno = 0; fno < cFaces; fno++) {
    if (mris->faces[fno].ripflag == 0) {
      ...
      if (create_segs) {
        dx = qMax(dx,
                  qAbs(mris->vertices[face[0]].x - mris->vertices[face[1]].x));
        dx = qMax(dx,
                  qAbs(mris->vertices[face[1]].x - mris->vertices[face[2]].x));
        dx = qMax(dx,
                  qAbs(mris->vertices[face[0]].x - mris->vertices[face[2]].x));
        dy = qMax(dy,
                  qAbs(mris->vertices[face[0]].y - mris->vertices[face[1]].y));
        dy = qMax(dy,
                  qAbs(mris->vertices[face[1]].y - mris->vertices[face[2]].y));
        dy = qMax(dy,
                  qAbs(mris->vertices[face[0]].y - mris->vertices[face[2]].y));
        dz = qMax(dz,
                  qAbs(mris->vertices[face[0]].z - mris->vertices[face[1]].z));
        dz = qMax(dz,
                  qAbs(mris->vertices[face[1]].z - mris->vertices[face[2]].z));
        dz = qMax(dz,
                  qAbs(mris->vertices[face[0]].z - mris->vertices[face[2]].z));
      }
    }
  }
  if (create_segs) {
...
void FSSurface::UpdateVectors() {
  if (HasVectorSet() && m_nActiveVector >= 0) {
    ...
    vtkSmartPointer<vtkPolyData>::New(); testPolydata->SetPoints(testPoints);
    vtkSmartPointer<vtkDelaunay3D> selectEnclosedPoints =
    ...
    {
        cellId =
    selectEnclosedPoints->GetOutput()->FindCell(testPoints->GetPoint(i), NULL,
    0, .0001, subId, pcoords, weights); if (cellId >= 0)
        {
        ...
        */
        ...
        if (contour_pts) {
          ...
          if (target_polydata && target_polydata->GetPoints()) {
            ProjectVectorPoint2D(point, target_polydata->GetPoints(),
                                 target_polydata->GetLines(), point);
          }
          points->InsertNextPoint(point);
        } else {
...
void FSSurface::ConvertSurfaceToRAS(float iX, float iY, float iZ, float &oX,
...
void FSSurface::ConvertSurfaceToRAS(double iX, double iY, double iZ, double &oX,
...
void FSSurface::ConvertRASToSurface(float iX, float iY, float iZ, float &oX,
                                    float &oY, float &oZ) const {
  float ras[3];
  float surface[3];
...
void FSSurface::ConvertRASToSurface(double iX, double iY, double iZ, double &oX,
                                    double &oY, double &oZ) const {
  double ras[3];
  double surface[3];
...
void FSSurface::GetBounds(float oRASBounds[6]) {
  ...
  if (m_bBoundsCacheDirty) {
    ...
    for (int vno = 0; vno < m_MRIS->nvertices; vno++) {
      ...
      this->ConvertSurfaceToRAS(
          m_fVertexSets[SurfaceMain][vno].x, m_fVertexSets[SurfaceMain][vno].y,
          m_fVertexSets[SurfaceMain][vno].z, rasX, rasY, rasZ);
...
int FSSurface::FindVertexAtSurfaceRAS(float const iSurfaceRAS[3],
                                      float *oDistance, int surface_type) {
  VERTEX v;
  v.x = iSurfaceRAS[0];
  v.y = iSurfaceRAS[1];
  v.z = iSurfaceRAS[2];
...
int FSSurface::FindVertexAtSurfaceRAS(double const iSurfaceRAS[3],
                                      double *oDistance, int surface_type) {
  VERTEX v;
...
void FSSurface::Reposition(FSVolume *volume, int target_vno, double target_val,
                           int nsize, double sigma, int flags) {
  MRISsaveVertexPositions(m_MRIS, INFLATED_VERTICES);
  float fval = (float)target_val;
  MRISrepositionSurface(m_MRIS, volume->GetMRI(), &target_vno, &fval, 1, nsize,
                        sigma, flags);
  PostEditProcess();
}

void FSSurface::Reposition(FSVolume *volume, int target_vno, double *coord,
                           int nsize, double sigma, int flags) {
  MRISsaveVertexPositions(m_MRIS, INFLATED_VERTICES);
  MRISrepositionSurfaceToCoordinate(m_MRIS, volume->GetMRI(), target_vno,
                                    coord[0], coord[1], coord[2], nsize, sigma,
                                    flags);
  PostEditProcess();
}

void FSSurface::RepositionSmooth(int vert_n, int nbhd_size,
...
bool FSSurface::Smooth(int nMethod, int niters, double lambda, double K_bp) {
  ...
  if (nMethod == 0) // Taubin
  {
    ...
    if (MRIStaubinSmooth(m_MRIS, niters, lambda, mu, TAUBIN_UNIFORM_WEIGHTS) !=
        0)
      return false;
    ...
    for (vno = min_vno; vno != max_vno; vno += step_vno) {
      if (filled[vno]) {
        ...
        for (neighbor_index = 0; neighbor_index < vt->vnum; neighbor_index++) {
          ...
          //          if (params->dont_cross_fthresh)
          //          {
          //            sclv_get_value (neighbor_v, sclv_current_field,
          //            &fvalue); if ((fthresh != 0 &&
          //                 seed_fvalue > 0 &&
          //                 fvalue < fthresh) ||
          //                (fthresh != 0 &&
