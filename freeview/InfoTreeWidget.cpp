...
void InfoTreeWidget::OnMousePositionChanged() {
  MainWindow::GetMainWindow()->GetLayerCollection("MRI")->GetCurrentRASPosition(
      m_dRAS);
  UpdateAll();
}
...
void InfoTreeWidget::ClearHeaderText() { headerItem()->setText(1, ""); }
...
void InfoTreeWidget::UpdateAll() {
  ...
  if (!lc_mri->IsEmpty()) {
    qobject_cast<LayerMRI *>(lc_mri->GetActiveLayer())
        ->RemapPositionToRealRAS(m_dRAS, ras);
  }
  QVariantMap map;
    ...
    if (layer->GetProperty()->GetShowInfo()) {
      ...
      if (layer->GetNumberOfFrames() > 1 && layer->GetNumberOfFrames() <= 6) {
        ...
        if (layer->GetDataType() == MRI_RGB) {
          ...
          values.clear();
          values << (nval & 0x00ff) << ((nval >> 8) & 0x00ff)
                 << ((nval >> 16) & 0x00ff);
        }
        QStringList strgs;
      ...
      if (bDecimalIndex)
        editable =
            QString("%1, %2, %3").arg(fIndex[0]).arg(fIndex[1]).arg(fIndex[2]);
      else {
        int nIndex[3];
        layer->RASToOriginalIndex(ras, nIndex);
        editable =
            QString("%1, %2, %3").arg(nIndex[0]).arg(nIndex[1]).arg(nIndex[2]);
      }
  ...
  for (int i = 0; i < lc_surf->GetNumberOfLayers(); i++) {
    ...
    if (surf->GetProperty()->GetShowInfo()) {
      ...
      if (bMappingVertex)
        nVertex =
            (m_bForCursor ? surf->GetCurrentVertex() : surf->GetMouseVertex());
      ...
      if (nVertex >= 0) {
        ...
        if (surf->HasCurvature() && m_bShowSurfaceCurvature) {
          ...
          item->setText(1,
                        QString("Curvature \t%1")
                            .arg(MyUtils::RealToNumber(
                                surf->GetCurvatureValue(nVertex), nPrecision)));
        }
        ...
        for (int i = 0; i < nOverlays; i++) {
          ...
          item->setText(
              1, QString("%1 \t%2")
                     .arg(overlay->GetName())
                     .arg(MyUtils::RealToNumber(
                         overlay->GetDataAtVertex(nVertex), nPrecision)));
        ...
        if (nPath >= 0) {
          ...
          if (path) {
            item = new QTreeWidgetItem(this);
            item->setText(1, QString("Path %1 \t%2 mm")
                                 .arg(nPath)
                                 .arg(path->GetLength(), 0, 'f', 2));
          }
        }
      } else {
        QTreeWidgetItem *item = new QTreeWidgetItem(this);
...
void InfoTreeWidget::OnEditFinished() {
  ...
  QStringList list =
      m_editor->text().trimmed().split(",", QString::SkipEmptyParts);
  ...
  if (type == "SurfaceVertex") {
    ...
    if (list.size() < 3) {
      ...
      if (bOK) {
        if (type == "RAS") {
          LayerMRI *mri = (LayerMRI *)MainWindow::GetMainWindow()
  ...
  if (bSuccess) {
    m_editor->hide();
    if (surf) {
      RenderView3D *view = qobject_cast<RenderView3D *>(
          MainWindow::GetMainWindow()->GetRenderView(3));
      if (surf->IsInflated())
...
void InfoTreeWidget::contextMenuEvent(QContextMenuEvent *e) {
  ...
  if (!surfs.isEmpty()) {
    ...
    connect(act, SIGNAL(toggled(bool)), this,
            SLOT(OnToggleSurfaceCurvature(bool)));
    menu->addAction(act);
    act = new QAction("Show Surface Normal", this);
    act->setCheckable(true);
    act->setChecked(m_bShowSurfaceNormal);
    connect(act, SIGNAL(toggled(bool)), this,
            SLOT(OnToggleSurfaceNormal(bool)));
    menu->addAction(act);
  }
