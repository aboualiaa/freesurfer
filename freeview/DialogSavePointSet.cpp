...
DialogSavePointSet::~DialogSavePointSet() { delete ui; }
...
void DialogSavePointSet::OnOK() {
  ...
  if (fn.isEmpty()) {
    ...
    if (m_bRemind && GetType() != LayerPropertyPointSet::Enhanced) {
      if (QMessageBox::question(
              this, "Warning",
              "Point set data contains enhanced information such as comments. "
              "Saving it in legacy formats will lose all the enhanced info. Do "
...
void DialogSavePointSet::SetType(int nType) {
  ui->radioButtonControlPoint->setChecked(nType ==
