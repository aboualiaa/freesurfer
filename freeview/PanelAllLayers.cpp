...
PanelAllLayers::PanelAllLayers(QWidget *parent)
    : QScrollArea(parent), ui(new Ui::PanelAllLayers) {
  ui->setupUi(this);
  ...
  foreach (QString type, layer_types) {
    connect(wnd->GetLayerCollection(type), SIGNAL(LayerAdded(Layer *)), this,
            SLOT(OnLayerAdded(Layer *)));
    connect(wnd->GetLayerCollection(type), SIGNAL(LayerRemoved(Layer *)), this,
            SLOT(OnLayerRemoved(Layer *)));
    connect(wnd->GetLayerCollection(type), SIGNAL(LayerMoved(Layer *)), this,
            SLOT(RefreshLayerList()));
    connect(wnd->GetLayerCollection(type), SIGNAL(LayerCycled(Layer *)), this,
            SLOT(RefreshLayerList()));
    connect(wnd->GetLayerCollection(type), SIGNAL(LayersReordered()), this,
            SLOT(RefreshLayerList()));
    connect(wnd->GetLayerCollection(type), SIGNAL(LayerNameChanged()), this,
            SLOT(OnLayerChanged()));
    connect(wnd->GetLayerCollection(type), SIGNAL(LayerVisibilityChanged()),
            this, SLOT(OnLayerChanged()));
    connect(wnd->GetLayerCollection(type), SIGNAL(ActiveLayerChanged(Layer *)),
            this, SLOT(OnActiveLayerChanged(Layer *)));
  }
  ...
  for (int i = 0; i < ui->stackedWidget->count(); i++) {
    PanelLayer *panel =
        qobject_cast<PanelLayer *>(ui->stackedWidget->widget(i));
    if (panel)
...
PanelAllLayers::~PanelAllLayers() {
  QSettings s;
  s.setValue("ControlPanel/SplitterState",
             ui->splitterControlPanel->saveState());
  delete ui;
}
...
void PanelAllLayers::OnActiveLayerChanged(Layer *curlayer) {
  ...
  if (item && curlayer) {
    QObject *sel = reinterpret_cast<QObject *>(
...
void PanelAllLayers::RefreshLayerList(const QList<Layer *> &selectedLayers_in,
  ...
  if (selectedLayers.isEmpty()) {
    ...
    foreach (QTreeWidgetItem *item, items) {
      Layer *layer_sel = reinterpret_cast<Layer *>(
          item->data(0, Qt::UserRole).value<quintptr>());
      if (layer_sel)
  ...
  MainWindow *wnd = MainWindow::GetMainWindow();
  AddLayers(wnd->GetLayers("MRI"), "Volumes", wnd->GetActiveLayer("MRI"),
            selectedLayers, layer);
  AddLayers(wnd->GetLayers("Surface"), "Surfaces",
            wnd->GetActiveLayer("Surface"), selectedLayers, layer);
  AddLayers(wnd->GetLayers("ROI"), "ROIs", wnd->GetActiveLayer("ROI"),
            selectedLayers, layer);
  AddLayers(wnd->GetLayers("PointSet"), "Point Sets",
            wnd->GetActiveLayer("PointSet"), selectedLayers, layer);
  ...
  if (!layers.isEmpty()) {
    ...
    for (int i = 0; i < layers.size(); i++) {
      ...
      if (layers[i]->IsTypeOf("Surface")) {
        ...
        connect(surf->GetProperty(), SIGNAL(EdgeColorChanged()), this,
                SLOT(OnLayerChanged()), Qt::UniqueConnection);
      } else if (layers[i]->IsTypeOf("ROI")) {
        LayerROI *roi = (LayerROI *)layers[i];
        SetItemColor(item, roi->GetProperty()->GetColor());
        connect(roi->GetProperty(), SIGNAL(ColorMapChanged()), this,
                SLOT(OnLayerChanged()), Qt::UniqueConnection);
      } else if (layers[i]->IsTypeOf("PointSet")) {
        LayerPointSet *layer = (LayerPointSet *)layers[i];
        SetItemColor(item, layer->GetProperty()->GetColor());
        connect(layer->GetProperty(), SIGNAL(ColorChanged()), this,
                SLOT(OnLayerChanged()), Qt::UniqueConnection);
      }
      item->setData(0, Qt::UserRole,
                    QVariant::fromValue(reinterpret_cast<quintptr>(layers[i])));
      item->setCheckState(0,
                          layers[i]->IsVisible() ? Qt::Checked : Qt::Unchecked);
...
void PanelAllLayers::OnLayerRemoved(Layer *removed_layer) {
  ...
  for (int i = 0; i < ui->treeWidgetLayers->topLevelItemCount(); i++) {
    ...
    for (int j = 0; j < topItem->childCount(); j++) {
      ...
      if (reinterpret_cast<Layer *>(
              item->data(0, Qt::UserRole).value<quintptr>()) == removed_layer) {
        topItem->removeChild(item);
        delete item;
        if (topItem->childCount() == 0) {
          ui->treeWidgetLayers->takeTopLevelItem(
              ui->treeWidgetLayers->indexOfTopLevelItem(topItem));
          delete topItem;
          topItem = NULL;
        }
        ui->treeWidgetLayers->blockSignals(false);
...
void PanelAllLayers::OnCurrentItemChanged(QTreeWidgetItem *item) {
  ...
  Layer *layer =
      reinterpret_cast<Layer *>(item->data(0, Qt::UserRole).value<quintptr>());
  QString type;
  if (layer) {
...
void PanelAllLayers::OnItemSelectionChanged() {
  ...
  PanelLayer *panel =
      qobject_cast<PanelLayer *>(ui->stackedWidget->currentWidget());
  if (panel) {
    panel->DisconnectAllLayers();

    QList<Layer *> layers = GetSelectedLayers(panel->GetLayerType());
    if (!layers.isEmpty()) {
      foreach (Layer *l, layers) { panel->ConnectLayer(l); }
    }
  }
}
  ...
  for (int i = 0; i < ui->treeWidgetLayers->topLevelItemCount(); i++) {
    ...
    for (int j = 0; j < topItem->childCount(); j++) {
      ...
      if (layer) {
        ...
        item->setCheckState(0,
                            layer->IsVisible() ? Qt::Checked : Qt::Unchecked);
        if (layer->IsTypeOf("Surface")) {
          ...
          connect(ps->GetProperty(), SIGNAL(ColorChanged()), this,
                  SLOT(OnLayerChanged()), Qt::UniqueConnection);
        }
      }
    }
...
void PanelAllLayers::SelectAllLayers() { ui->treeWidgetLayers->SelectAll(); }
