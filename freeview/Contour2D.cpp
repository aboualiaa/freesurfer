...
vtkImageActor *Contour2D::GetActor() { return m_actorContour; }

vtkImageData *Contour2D::GetInputImage() { return m_imageInput; }
...
void Contour2D::SetInput(vtkImageData *imagedata, double dContourValue,
                         double dSliceLocation, int active_frame) {
  ...
  m_filterResample->SetOutputSpacing(vs[0] / IMAGE_RESAMPLE_FACTOR,
                                     vs[1] / IMAGE_RESAMPLE_FACTOR,
                                     vs[2] / IMAGE_RESAMPLE_FACTOR);
...
void Contour2D::DrawPatchLineOnMask(vtkImageData *image, double *ras1,
                                    double *ras2, int nDrawValue) {
  if (!image) {
    return;
  }
...
void Contour2D::UpdateSliceLocation(double dSliceLocation, bool bForced) {
  vtkImageData *imagedata =
      vtkImageData::SafeDownCast(m_filterThreshold->GetInput());
  if (!imagedata) {
    return;
  }
...
bool Contour2D::IsVisible() { return m_actorContour->GetVisibility(); }
...
void Contour2D::SetSmooth(bool bSmooth) {
  ...
  if (m_imageInput.GetPointer()) {
    ...
    m_filterThreshold->SetInputData(bSmooth ? m_filterSmooth->GetOutput()
                                            : m_imageInput.GetPointer());
#else
    m_filterSmooth->SetInput(m_imageInput);
    m_filterThreshold->SetInput(bSmooth ? m_filterSmooth->GetOutput()
                                        : m_imageInput.GetPointer());
#endif
  }
}
