...
vtkActor *SurfaceRegion::GetMeshActor() { return m_actorMesh; }
    ...
    if (m_polydataHolder.GetPointer()) {
      ...
      while (polys->GetNextCell(npts, pts)) {
        ...
        for (size_t i = 0; i < idList.size() && !bFound; i += 3) {
          if (pts[0] == idList[i] && pts[1] == idList[i + 1] &&
              pts[2] == idList[i + 2]) {
            bFound = true;
          }
        }
      ...
      return (polydata && polydata->GetPoints() &&
...
void SurfaceRegion::SetColor(const QColor &color) {
  m_color = color;
  m_actorMesh->GetProperty()->SetColor(color.redF(), color.greenF(),
                                       color.blueF());
  emit ColorChanged(color);
}

QColor SurfaceRegion::GetColor() { return m_color; }
...
void SurfaceRegion::Highlight(bool bHighlight) {
  if (bHighlight) {
    ...
    m_actorMesh->GetProperty()->SetColor(m_color.redF(), m_color.greenF(),
                                         m_color.blueF());
  }
}
...
bool SurfaceRegion::DeleteCell(RenderView3D *view, int pos_x, int pos_y) {
  ...
  if (nIdPicked >= 0) {
    vtkPolyData *polydata =
        vtkPolyDataMapper::SafeDownCast(m_actorMesh->GetMapper())->GetInput();
    vtkSmartPointer<vtkCellArray> new_polys =
        vtkSmartPointer<vtkCellArray>::New();
    vtkCellArray *polys = polydata->GetPolys();
    polys->InitTraversal();
