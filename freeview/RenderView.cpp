...
RenderView::RenderView(QWidget *parent)
    : GenericRenderView(parent), m_bNeedRedraw(false),
  ...
  vtkSmartPointer<vtkPolyDataMapper2D> pMapper =
  ...
  m_actorScalarBar->GetPositionCoordinate()
  ...
  connect(this, SIGNAL(BackgroundColorChanged(QColor)), this,
          SLOT(RequestRedraw()));

  QTimer *timer = new QTimer(this);
  connect(timer, SIGNAL(timeout()), this, SLOT(OnIdle()));
  connect(this, SIGNAL(ActorsUpdated()), this, SLOT(RequestRedraw()),
          Qt::QueuedConnection);
  timer->start(50);
}

int RenderView::GetInteractionMode() { return m_nInteractionMode; }

void RenderView::SetInteractionMode(int nMode) { m_nInteractionMode = nMode; }
...
void RenderView::mousePressEvent(QMouseEvent *event) {
  if (!hasFocus()) {
    ...
    //  if ( MainWindow::GetMainWindowPointer()->GetPreviousActiveView() != this
    //  && event.LeftDown() )
    //    return;
  }
...
void RenderView::SetWorldCoordinateInfo(const double *origin,
...
void RenderView::ViewportToWorld(double x, double y, double &world_x,
                                 double &world_y, double &world_z) {
  MyVTKUtils::ViewportToWorld(m_renderer, x, y, world_x, world_y, world_z);
}

void RenderView::NormalizedViewportToWorld(double x, double y, double &world_x,
                                           double &world_y, double &world_z) {
  MyVTKUtils::NormalizedViewportToWorld(m_renderer, x, y, world_x, world_y,
                                        world_z);
}

void RenderView::WorldToViewport(double world_x, double world_y, double world_z,
                                 double &x, double &y, double &z) {
  MyVTKUtils::WorldToViewport(m_renderer, world_x, world_y, world_z, x, y, z);
}

void RenderView::WorldToScreen(double world_x, double world_y, double world_z,
                               int &x, int &y) {
  double dx, dy, dz;
  MyVTKUtils::WorldToViewport(m_renderer, world_x, world_y, world_z, dx, dy,
                              dz);
#if VTK_MAJOR_VERSION > 7
...
void RenderView::ScreenToWorld(int x, int y, int z, double &world_x,
                               double &world_y, double &world_z) {
  y = rect().height() - y;
#if VTK_MAJOR_VERSION > 7
  if (devicePixelRatio() > 1) {
...
void RenderView::MoveLeft() {
  ...
  double scale = qMax(qMax(m_dWorldSize[0], m_dWorldSize[1]), m_dWorldSize[2]) /
                 SCALE_FACTOR;
  for (int i = 0; i < 3; i++) {
...
int RenderView::GetAction() { return m_interactor->GetAction(); }

void RenderView::SetAction(int nAction) { m_interactor->SetAction(nAction); }
...
void RenderView::UpdateScalarBar() {
  if (m_layerScalarBar.isNull()) {
    ...
    foreach (Layer *layer, layers) {
      ...
      if (mri && mri->GetProperty()->GetColorMap() != LayerPropertyMRI::LUT) {
        m_actorScalarBar->SetLookupTable(
            mri->GetProperty()->GetActiveLookupTable());
    ...
    if (m_layerScalarBar->IsTypeOf("MRI")) {
      ...
      if (ps->GetProperty()->GetColorMap() ==
          LayerPropertyPointSet::HeatScale) {
...
bool RenderView::SaveScreenShot(const QString &filename, bool bAntiAliasing,
                                int nMag, bool bAutoTrim) {
  blockSignals(true);
...
int RenderView::PickCell(vtkProp *prop, int posX, int posY, double *pos_out) {
  vtkCellPicker *picker = vtkCellPicker::SafeDownCast(
      GetRenderWindow()->GetInteractor()->GetPicker());
  if (!picker) {
    return -1;
  }
