...
vtkActor *VolumeCropper::GetProp() { return m_actorBox; }
...
void VolumeCropper::SetVolume(LayerMRI *mri) {
  if (m_mri != mri) {
    ...
    m_mri = mri;
    connect(m_mri, SIGNAL(IsoSurfaceUpdated()), this, SLOT(Apply()),
            Qt::UniqueConnection);
    Reset();
  }
}
...
void VolumeCropper::Apply() { m_box->SetBounds(m_bounds); }
...
void VolumeCropper::UpdateProps() {
  ...
  m_mri->m_sliceActor2D[0]->SetDisplayExtent(ext[2], ext[3], ext[4], ext[5], 0,
                                             0);
  m_mri->m_sliceActor2D[1]->SetDisplayExtent(ext[0], ext[1], ext[4], ext[5], 0,
                                             0);
  m_mri->m_sliceActor2D[2]->SetDisplayExtent(ext[0], ext[1], ext[2], ext[3], 0,
                                             0);
  for (int i = 0; i < 3; i++) {
    m_mri->m_sliceActor3D[i]->SetDisplayExtent(
        m_mri->m_sliceActor2D[i]->GetDisplayExtent());
  }

  vtkPolyDataMapper *mapper =
      vtkPolyDataMapper::SafeDownCast(m_mri->m_actorContour->GetMapper());
  if (mapper && mapper->GetInput() &&
      m_clipper->GetOutput() != mapper->GetInput()) {
#if VTK_MAJOR_VERSION > 5
...
bool VolumeCropper::IsShown() { return m_actorBox->GetVisibility(); }
...
void VolumeCropper::MoveActiveBound(RenderView *view, int nx, int ny) {
  ...
  if (fabs(x1 - x2) > fabs(y1 - y2)) {
    ratio = (x2 == x1)
                ? 0
                : (nx / sqrt((x2 - x1) * (x2 - x1)) * (x2 > x1 ? 1 : -1));
  } else {
    ratio = (y2 == y1)
                ? 0
                : (ny / sqrt((y2 - y1) * (y2 - y1)) * (y2 > y1 ? 1 : -1));
  }
...
void VolumeCropper::ValidateActiveBound() {
  ...
  if (m_nActivePlane % 2 == 0 &&
      m_bounds[m_nActivePlane] >= m_bounds[m_nActivePlane + 1]) {
    m_bounds[m_nActivePlane] = m_bounds[m_nActivePlane + 1] - voxelsize[n] / 2;
  } else if (m_nActivePlane % 2 == 1 &&
             m_bounds[m_nActivePlane] < m_bounds[m_nActivePlane - 1]) {
    m_bounds[m_nActivePlane] = m_bounds[m_nActivePlane - 1] + voxelsize[n] / 2;
  }
}
...
void VolumeCropper::UpdateActivePlane() {
  if (m_nActivePlane >= 0) {
    ...
    m_planeSource->SetOrigin(
        1000, 200,
        50); // random set origin/points first to work around a dumb bug in vtk
