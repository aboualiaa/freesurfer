...
WidgetTimeCoursePlot::WidgetTimeCoursePlot(QWidget *parent)
    : QWidget(parent), m_bAutoScale(true), m_nCurrentFrame(0), m_dMinPlot(0),
      m_dMaxPlot(1), m_bShowFrameNumber(false), m_bShowCursorInfo(false) {
...
void WidgetTimeCoursePlot::paintEvent(QPaintEvent *e) {
  ...
  for (int n = 0; n < m_data.size(); n++) {
    ...
    if (td.m_bShow) {
      ...
      for (int i = 0; i < td.m_points.size(); i++) {
        pts[i] =
            QPointF(rc_plot.left() + dSpacing * i,
                    rc_plot.bottom() - (td.m_points[i] - dMin) / (dMax - dMin) *
                                           rc_plot.height());
      }
  ...
  while (y > rc_plot.top()) {
    if (y <= rc_plot.bottom()) {
      ...
      p.drawText(
          QRectF(rect().left(), y - 10, rc_plot.left() - rect().left() - 6, 20),
  ...
  if (!m_bShowFrameNumber) {
    ...
    dMetricStep = (m_nFrames - 1) * qAbs(td.m_dXInterval) / rc_plot.width() *
                  nMetricInterval;
  ...
  while (x <= rc_plot.right() + dMetricStep / 10) {
    ...
    if (m_bShowFrameNumber)
      ...
      strg = QString::number((td.m_dXOffset + td.m_dXInterval * dMetricPos) *
                                 dScale,
                             'f', nPrecise);
    p.drawText(QRectF(x - 100, rc_plot.bottom() + 5, 200, 20),
               Qt::AlignTop | Qt::AlignHCenter, strg);
    if (x - 1 >= rc_plot.left() && x - 1 <= rc_plot.right())
      p.drawLine(QPointF(x - 1, rc_plot.bottom()),
                 QPointF(x - 1, rc_plot.bottom() + 2));
  ...
  if (!strXUnit.isEmpty())
    x_strg +=
        QString(" / %1 (%2)")
            .arg((td.m_dXOffset + m_nCurrentFrame * td.m_dXInterval) * dScale)
            .arg(strXUnit);
      ...
      for (int n = 0; n < m_data.size(); n++) {
        ...
        if (td.m_bShow) {
          ...
          if (m_nCurrentFrame < td.m_points.size()) {
            ...
            p.drawText(rc, Qt::AlignLeft,
                       QString::number(td.m_points[m_nCurrentFrame]));
          }
