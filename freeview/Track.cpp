...
float Track::GetTrackLength(const float *track_pts, int n) {
  ...
  for (int i = 1; i < n; i++) {
    len +=
        MyUtils::GetDistance<float>(track_pts + i * 3, track_pts + i * 3 - 3);
  }
  return (float)len;
}
...
void GetVoxels(short *n1, short *n2, QList<short> &indices_out) {
  ...
  for (k = 0; k < n_out; k++) {
    if (n1[0] == indices_out[k * 3] && n1[1] == indices_out[k * 3 + 1] &&
        n1[2] == indices_out[k * 3 + 2]) {
      break;
    }
  }
  if (k >= n_out) {
  ...
  if (qAbs(n1[0] - n2[0]) > 1 || qAbs(n1[1] - n2[1]) > 1 ||
      qAbs(n1[2] - n2[2]) > 1) {
    short m[3];
    for (int i = 0; i < 3; i++) {
      m[i] = (n1[i] + n2[i]) / 2;
...
void Track::GetVoxels(const float *track_pts_in, int n_pts_in, const short *dim,
  ...
  for (int i = 0; i < n_pts_in - 1; i++) {
    for (int j = 0; j < 3; j++) {
      n[j] = qMax(
          0, qMin(dim[j] - 1, (int)(track_pts_in[i * 3 + j] / voxel_size[j])));
      m[j] = qMax(0, qMin(dim[j] - 1,
                          (int)(track_pts_in[i * 3 + 3 + j] / voxel_size[j])));
    }
...
void Track::GetVoxels(const short *dim, const float *voxel_size,
                      short **indices_out, int *n_out) {
  GetVoxels(fPts, nNum, dim, voxel_size, indices_out, n_out);
}

/*
bool Track::IntersectWithPlane(float* track_pts, int nNum, double* normal,
double* pt, double* x, int* index)
  {
    for (int i = 1; i < nNum; i++)
    {
      double t;
      if (CMyMath::PlaneIntersectWithLine(track_pts+i*3, track_pts+(i-1)*3,
normal, pt, t, x))
      {
        if (index)
          *index = i-1;
...
bool Track::IntersectWithDisk(float* track_pts, int nNum, double* normal,
double* pt, double r, double* x, int* indices, double angle_end, double
angle_start)
{
...
bool Track::IntersectWithPlane(double* normal, double* pt, double* x, int*
index)
{
...
{
  int n[2];
  return IntersectWithDisk(fPts, nNum, normal, pt, r, x, n, angle_end,
angle_start);
}
