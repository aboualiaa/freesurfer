...
FSVolume::FSVolume(FSVolume *ref, QObject *parent)
    : QObject(parent), m_MRI(NULL), m_MRITarget(NULL), m_MRIRef(NULL),
      m_MRIOrigTarget(NULL), m_MRITemp(NULL), m_matReg(NULL),
      m_ctabEmbedded(NULL), m_volumeRef(ref), m_fMinValue(0), m_fMaxValue(1),
      m_bResampleToRAS(false), m_bBoundsCacheDirty(true),
      m_nInterpolationMethod(SAMPLE_NEAREST), m_bConform(false), m_bCrop(false),
      m_bCropToOriginal(false), m_histoCDF(NULL), m_nHistoFrame(0),
      m_bValidHistogram(false), m_bSharedMRI(false), m_lta(NULL),
...
bool FSVolume::LoadMRI(const QString &filename, const QString &reg_filename) {
  ...
  //  {
  //    cerr << "Error: A target volume must be loaded first to apply
  //    registration matrix.\n"; return false;
  //  }

  // save old header to release later so there is no gap where m_MRI becomes
  // NULL during re-loading process
  ...
  */
  ...
  if (!reg_filename.isEmpty() && !LoadRegistrationMatrix(reg_filename)) {
    std::cerr << "Read registration failed\n";
    return false;
  }
  ...
  if (m_bValidHistogram) {
    double val =
        GetHistoValueFromPercentile(0.95) * 1.1; //+m_histoCDF->bin_size/2;
...
bool FSVolume::MRIRead(const QString &filename, const QString &reg_filename) {
  ...
  if (LoadMRI(filename, reg_filename)) {
    ...
    if (m_volumeRef && m_volumeRef->m_MRIOrigTarget && !m_MRIOrigTarget) {
      m_MRIOrigTarget = CreateTargetMRI(m_MRI, m_volumeRef->m_MRIOrigTarget,
                                        false, m_bConform);
    }
...
bool FSVolume::CreateFromMRIData(MRI *mri) {
  ...
  if (m_volumeRef && m_volumeRef->m_MRIOrigTarget && !m_MRIOrigTarget) {
    m_MRIOrigTarget =
        CreateTargetMRI(m_MRI, m_volumeRef->m_MRIOrigTarget, false, m_bConform);
  }
  return true;
}
...
bool FSVolume::Restore(const QString &filename, const QString &reg_filename) {
  ...
  if (LoadMRI(filename, reg_filename)) {
    ...
    MRI *mri = MRIallocHeader(m_MRI->width, m_MRI->height, m_MRI->depth,
...
MATRIX *FSVolume::LoadRegistrationMatrix(const QString &filename, MRI *target,
                                         MRI *src, LTA **lta_out) {
  ...
  } else if (ext == "dat") // tkregister style
  {
    ...
    int   float2int;
    if (regio_read_register(filename.toLatin1().data(), &subject, &inplaneres,
                            &betplaneres, &intensity, &matReg,
                            &float2int) != 0) {
      return NULL;
    }
  ...
  } else // LTA style & all possible other styles
  {
    ...
    if (lta->type != LINEAR_RAS_TO_RAS) {
      std::cout << "INFO: LTA input is not RAS to RAS...converting...\n";
      lta = LTAchangeType(lta, LINEAR_RAS_TO_RAS);
    }
    if (lta->type != LINEAR_RAS_TO_RAS) {
      std::cerr << "ERROR: LTA input is not RAS to RAS\n";
...
bool FSVolume::Create(FSVolume *src_vol, bool bCopyVoxelData, int data_type) {
  ...
  memcpy(m_VoxelToVoxelMatrix, src_vol->m_VoxelToVoxelMatrix,
         16 * sizeof(double));
  ...
  try {
    m_MRI = MRIallocSequence(src_vol->m_MRI->width, src_vol->m_MRI->height,
                             src_vol->m_MRI->depth, data_type, 1);
  } catch (int ret) {
    return false;
  }

  if (NULL == m_MRI) {
    std::cerr << "Could not allocate new mri volume.\n";
    return false;
  }
  ...
  if (src_vol->m_MRIOrigTarget) {
    ...
    m_MRIOrigTarget = MRIallocHeader(mri->width, mri->height, mri->depth,
                                     data_type, mri->nframes);
  ...
  if (bCopyVoxelData) {
    if (data_type != src_vol->GetDataType()) {
      std::cerr << "Can not copy voxel data with different data type.\n";
    } else {
      m_imageData->DeepCopy(src_vol->m_imageData);
    }

    if (!m_imageData->GetScalarPointer()) {
      std::cerr << "Unable to allocate voxel data.\n";
      return false;
    }
  } else {
...
void FSVolume::SetMRI(MRI *&mri_out, MRI *mri_in) {
  ...
  if (mri_in) {
    mri_out = MRIallocHeader(mri_in->width, mri_in->height, mri_in->depth,
                             mri_in->type, mri_in->nframes);
...
void FSVolume::SetMRITarget(MRI *mri) {
  if (mri) {
    ...
    m_MRITarget = MRIallocHeader(mri->width, mri->height, mri->depth,
                                 m_MRI->type, m_MRI->nframes);
    MRIcopyHeader(mri, m_MRITarget);
  }
}

vtkTransform *FSVolume::GetTransform() { return m_transform; }
...
void FSVolume::ConvertTransformFromTargetToRAS(vtkMatrix4x4 *target_in,
                                               vtkMatrix4x4 *ras_out) {
  MATRIX *m1 = MatrixAlloc(4, 4, MATRIX_REAL);
  for (int i = 0; i < 16; i++) {
    *MATRIX_RELT(m1, (i / 4) + 1, (i % 4) + 1) =
        target_in->Element[i / 4][i % 4];
  }
...
bool FSVolume::SaveRegistration(const QString &filename) {
  ...
  bool  ret = true;
  if (!fp) {
    std::cerr << "ERROR: cannot open for writing: " << qPrintable(filename)
              << "\n";
    ret = false;
  } else {
    LTAprint(fp, lta);
...
bool FSVolume::MRIWrite(const QString &filename, int nSampleMethod,
                        bool resample) {
  if (!m_MRITemp) {
    std::cerr << "Volume not ready for save.\n";
    return false;
  }
  ...
  double scale =
      qMin(m_MRITemp->xsize, qMin(m_MRITemp->ysize, m_MRITemp->zsize));
  ...
  if (!MyUtils::IsIdentity(mat->Element, scale)) {
    if (resample) // && MyUtils::IsOblique(mat->Element))
    {
        ...
        for (cornerFactor[1] = 0; cornerFactor[1] <= 1; cornerFactor[1]++) {
          for (cornerFactor[0] = 0; cornerFactor[0] <= 1; cornerFactor[0]++) {
            ::MRIvoxelToWorld(m_MRITemp,
                              ...
                              cornerFactor[2] * (m_MRITemp->depth - 1), &RAS[0],
                              &RAS[1], &RAS[2]);
            mat->MultiplyPoint(RAS, RAS);
            ::MRIworldToVoxel(m_MRITemp, RAS[0], RAS[1], RAS[2], &index[0],
                              &index[1], &index[2]);
      ...
      MRI *mri = NULL;
      try {
        mri = MRIallocSequence(dim[0], dim[1], dim[2], m_MRITemp->type,
                               m_MRITemp->nframes);
      } catch (int ret) {
        return false;
      }
      ...
      if (!m_bCropToOriginal) {
        double p0[3];
        ::MRIvoxelToWorld(m_MRITemp, (int)indexBounds[0], (int)indexBounds[2],
                          (int)indexBounds[4], &p0[0], &p0[1], &p0[2]);
        MRIp0ToCRAS(mri, p0[0], p0[1], p0[2]);
      }

      /*
      MATRIX* M = m;
      qDebug() << M->rptr[1][1] << " " << M->rptr[1][2] << " " << M->rptr[1][3]
      << " " << M->rptr[1][4]
          << M->rptr[2][1] << " " << M->rptr[2][2] << " " << M->rptr[2][3] << "
      " << M->rptr[2][4]
          << M->rptr[3][1] << " " << M->rptr[3][2] << " " << M->rptr[3][3] << "
      ...
      */
      ...
      mri        = MRIapplyRASlinearTransformInterp(
          m_MRITemp, mri, bRefTransformed ? mi : m, nSampleMethod);
      MatrixFree(&mi);
      if (!mri) {
        MatrixFree(&m);
        std::cerr << "MRIapplyRASlinearTransformInterp failed.\n";
        return false;
      }
      MRIfree(&m_MRITemp);
      m_MRITemp = mri;
      ...
      */
      MATRIX *new_v2r  = MatrixMultiply(m, old_v2r, 0);
      m_MRITemp->xsize = sqrt(new_v2r->rptr[1][1] * new_v2r->rptr[1][1] +
                              new_v2r->rptr[2][1] * new_v2r->rptr[2][1] +
                              new_v2r->rptr[3][1] * new_v2r->rptr[3][1]);
      m_MRITemp->ysize = sqrt(new_v2r->rptr[1][2] * new_v2r->rptr[1][2] +
                              new_v2r->rptr[2][2] * new_v2r->rptr[2][2] +
                              new_v2r->rptr[3][2] * new_v2r->rptr[3][2]);
      m_MRITemp->zsize = sqrt(new_v2r->rptr[1][3] * new_v2r->rptr[1][3] +
                              new_v2r->rptr[2][3] * new_v2r->rptr[2][3] +
                              new_v2r->rptr[3][3] * new_v2r->rptr[3][3]);
    ...
    for (int i = 0; i < 16; i++) {
      m_RASToTkRegMatrix[i] =
          (double)*MATRIX_RELT((m1), (i / 4) + 1, (i % 4) + 1);
    }
    MatrixFree(&m);
    MatrixFree(&tkreg);
    MatrixFree(&m1);
  ...
  if (m_bCrop) {
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 2; j < 4; j++) {
        for (int k = 4; k < 6; k++) {
          this->TargetToRAS(m_dBounds[i], m_dBounds[j], m_dBounds[k], rx, ry,
                            rz);
    ...
    if (nmin[0] != 0 || nmin[1] != 0 || nmin[2] != 0 ||
        ...
        dz != m_MRITemp->depth) {
      MRI *mri =
          MRIextract(m_MRITemp, NULL, nmin[0], nmin[1], nmin[2], dx, dy, dz);
      if (!mri) {
        std::cerr << "MRIextract failed.\n";
        return false;
      }
      MRIfree(&m_MRITemp);
      m_MRITemp = mri;
  ...
  if (!fp) {
    std::cerr
        << "Failed to open file " << qPrintable(filename)
        << " for write. Please check if the directory exists and you have "
  ...
  if (err != 0) {
    std::cerr << "MRIwrite failed\n";
  } else if (bTransformed) // save lta file only if volume was saved
                           // successfully
  {
...
bool FSVolume::UpdateMRIFromImage(vtkImageData *rasImage,
  ...
  try {
    mri = MRIallocSequence(
        m_MRITarget->width, m_MRITarget->height, m_MRITarget->depth,
        data_type >= 0 ? data_type : m_MRITarget->type, m_MRI->nframes);
  } catch (int ret) {
    return false;
  }

  if (mri == NULL) {
    std::cout << "Can not allocate mri volume for buffering.\n";
    return false;
  }
      ...
      for (int k = 0; k < mri->depth; k++) {
        for (int i = 0; i < mri->width; i++) {
          for (int nFrame = 0; nFrame < mri->nframes; nFrame++) {
            //            float val = rasImage->GetScalarComponentAsFloat(i, j,
            //            k, nFrame);
            float val = (float)MyVTKUtils::GetImageDataComponent(
                ptr, dim, nNumberOfFrames, i, j, k, nFrame, scalar_type);
            switch (mri->type) {
            case MRI_UCHAR:
  ...
  if (resampleToOriginal) {
    try {
      m_MRITemp = MRIallocSequence(m_MRI->width, m_MRI->height, m_MRI->depth,
...
double FSVolume::GetVoxelValue(int i, int j, int k, int frame) {
  if (i < 0 || i >= m_MRI->width || j < 0 || j >= m_MRI->height || k < 0 ||
      k >= m_MRI->depth || frame >= m_MRI->nframes) {
    return 0;
  } else {
...
bool FSVolume::RASToTalairachVoxel(const double *pos_in, double *pos_out) {
  if (m_MRI->linear_transform) {
    double x, y, z;
    ::MRIworldToTalairachVoxel(m_MRI, pos_in[0], pos_in[1], pos_in[2], &x, &y,
                               &z);
    pos_out[0] = x;
    pos_out[1] = y;
    pos_out[2] = z;
...
void FSVolume::TalairachVoxelToRAS(const double *pos_in, double *pos_out) {
  double x, y, z;
  ::MRItalairachVoxelToWorld(m_MRI, pos_in[0], pos_in[1], pos_in[2], &x, &y,
                             &z);
  pos_out[0] = x;
  pos_out[1] = y;
  pos_out[2] = z;
...
MRI *FSVolume::CreateTargetMRI(MRI *src, MRI *refTarget, bool bAllocatePixel,
  ...
  for (cornerFactor[2] = 0; cornerFactor[2] <= 1; cornerFactor[2]++) {
    for (cornerFactor[1] = 0; cornerFactor[1] <= 1; cornerFactor[1]++) {
      for (cornerFactor[0] = 0; cornerFactor[0] <= 1; cornerFactor[0]++) {
        ::MRIvoxelToWorld(src, cornerFactor[0] * src->width - 0.5,
                          cornerFactor[1] * src->height - 0.5,
                          cornerFactor[2] * src->depth - 0.5, &RAS[0], &RAS[1],
                          &RAS[2]);
        ::MRIworldToVoxel(refTarget, RAS[0], RAS[1], RAS[2], &index[0],
                          &index[1], &index[2]);
  ...
  if (bConform) {
    ...
    // calculate dimension of the converted target volume
    int dim[3];
    dim[0] = (int)((indexBounds[1] - indexBounds[0]) * refTarget->xsize /
                       pixelSize[0] +
                   0.5);
    dim[1] = (int)((indexBounds[3] - indexBounds[2]) * refTarget->ysize /
                       pixelSize[1] +
                   0.5);
    dim[2] = (int)((indexBounds[5] - indexBounds[4]) * refTarget->zsize /
                       pixelSize[2] +
                   0.5);
    ...
    if (true) {
      ...
      ::MRIworldToVoxel(mri, refTarget->c_r, refTarget->c_a, refTarget->c_s,
                        &cpt[0], &cpt[1], &cpt[2]);
      ::MRIworldToVoxel(mri, mri->c_r, mri->c_a, mri->c_s, &cpt_ext[0],
                        &cpt_ext[1], &cpt_ext[2]);
      //        double vs[3] = { mri->xsize, mri->ysize, mri->zsize };
      //        double vs2[3] = { refTarget->xsize, refTarget->ysize,
      //        refTarget->zsize };
      for (int i = 0; i < 3; i++) {
      ...
      ::MRIvoxelToWorld(mri, cpt[0], cpt[1], cpt[2], &cpt_ext[0], &cpt_ext[1],
                        &cpt_ext[2]);
...
bool FSVolume::MapMRIToImage(bool do_not_create_image) {
  ...
  if (m_matReg && m_MRIRef && m_volumeRef) {
    ...
    try {
      rasMRI = MRIallocSequence(mri->width, mri->height, mri->depth,
                                m_MRI->type, m_MRI->nframes);
    } catch (int ret) {
      return false;
    }
    ...
    if (rasMRI == NULL) {
      std::cerr << "Can not allocate memory for volume transformation\n";
      MatrixFree(&m);
      return false;
    }
    ...
    try {
      rasMRI =
          MRIallocSequence(dim[0], dim[1], dim[2], m_MRI->type, m_MRI->nframes);
    } catch (int ret) {
      return false;
    }
    ...
    *MATRIX_RELT(m, 3, 4) = bounds[4];
    ...
    if (!m_volumeRef) {
      ...
      *MATRIX_RELT(m, 4, 4) = 1;
      ...
      try {
        rasMRI = MRIallocSequence(dim[0], dim[1], dim[2], m_MRI->type,
                                  m_MRI->nframes);
      } catch (int ret) {
        return false;
      }
      ...
      rasMRI =
          CreateTargetMRI(m_MRI, m_volumeRef->m_MRITarget, true, m_bConform);
      if (rasMRI == NULL) {
        std::cerr << "Can not allocate memory for volume transformation\n";
        MatrixFree(&m);
        return false;
...
bool FSVolume::CreateImage(MRI *rasMRI) {
  ...
  if (m_bResampleToRAS && !m_volumeRef) {
    ...
    //    qDebug() << m_volumeRef->m_MRITarget->x_r <<
    //    m_volumeRef->m_MRITarget->y_r << m_volumeRef->m_MRITarget->z_r;
    //    qDebug() << m_volumeRef->m_MRITarget->x_a <<
    //    m_volumeRef->m_MRITarget->y_a << m_volumeRef->m_MRITarget->z_a;
    //    qDebug() << m_volumeRef->m_MRITarget->x_s <<
    //    m_volumeRef->m_MRITarget->y_s << m_volumeRef->m_MRITarget->z_s;

    //    qDebug() << rasMRI->c_r << rasMRI->c_a << rasMRI->c_s <<
    //    m_volumeRef->m_MRITarget->c_r << m_volumeRef->m_MRITarget->c_a
    ...
    double ras[3], ras2[3];
    ::MRIvoxelToWorld( rasMRI, 0., 0., 0., &ras[0], &ras[1], &ras[2] );
    ::MRIvoxelToWorld( m_volumeRef->m_MRITarget, 0., 0., 0., &ras2[0], &ras2[1],
    &ras2[2] ); m_volumeRef->GetImageOutput()->GetOrigin( origin ); for ( int i
    = 0; i < 3; i++ ) origin[i] += ( ras[i] - ras2[i] );
    */
  }
...
bool FSVolume::ResizeRotatedImage(MRI *rasMRI, MRI *refTarget,
                                  vtkImageData *refImageData,
                                  double *      rasPoint) {
  // first copy mri data to image
  vtkDataArray *        scalars      = NULL;
  vtkUnsignedCharArray *ucharScalars = NULL;
  ...
  double vox[3], tvox[3];
  ::MRIworldToVoxel(rasMRI, rasPoint[0], rasPoint[1], rasPoint[2], &vox[0],
                    &vox[1], &vox[2]);
...
bool FSVolume::Rotate(std::vector<RotationElement> &rotations,
  ...
  if (!m_MRITemp) {
    std::cerr << "Volume not ready for rotation\n";
    return false;
  }
  ...
  if (rotations.size() == 0 || rotations[0].Plane == -1) // restore
  {
    ...
    {
      std::cout << "Can not restore because no original space was defined.\n";
      return false;
    }

    try {
      rasMRI =
          MRIallocSequence(m_MRIOrigTarget->width, m_MRIOrigTarget->height,
                           m_MRIOrigTarget->depth, m_MRI->type, m_MRI->nframes);
    } catch (int ret) {
      return false;
    }

    if (rasMRI == NULL) {
      std::cout << "Can not allocate memory for volume transformation.\n";
      return false;
    }
  ...
  } else // rotate
  {
    ...
    for (size_t i = 0; i < rotations.size(); i++) {
      MATRIX *m_tmp  = GetRotationMatrix(rotations[i].Plane, rotations[i].Angle,
                                        rotations[i].Point);
    ...
    rasMRI = MRIallocHeader(m_MRITarget->width, m_MRITarget->height,
                            m_MRITarget->depth, m_MRI->type, m_MRI->nframes);
    MRIsetResolution(rasMRI, m_MRITarget->xsize, m_MRITarget->ysize,
                     m_MRITarget->zsize);
    ...
    {
      m_MRIOrigTarget = MRIallocHeader(m_MRITarget->width, m_MRITarget->height,
                                       m_MRITarget->depth, m_MRITarget->type,
                                       m_MRITarget->nframes);
      MRIcopyHeader(m_MRITarget, m_MRIOrigTarget);
    }
  }

  // change the field of view of the new target to full coverage of the image
  // data
  ...
  // copy mri pixel data to vtkImage we will use for display
  {
    bUpdateImage = ResizeRotatedImage(rasMRI, m_MRITarget, m_imageData,
                                      rotations[0].Point);
  }
...
void FSVolume::CopyMRIDataToImage(MRI *mri, vtkImageData *image) {
      ...
      for (int nX = 0; nX < zX; nX++) {
        if (mri->type == MRI_RGB) {
          ...
          for (int nFrame = 0; nFrame < zFrames; nFrame++) {
            switch (mri->type) {
            case MRI_UCHAR:
              ptr[nTuple * zFrames + nFrame] =
                  MRIseq_vox(mri, nX, nY, nZ, nFrame);
              break;
            case MRI_INT:
              ((int *)ptr)[nTuple * zFrames + nFrame] =
                  MRIIseq_vox(mri, nX, nY, nZ, nFrame);
              break;
            case MRI_LONG:
              ((long *)ptr)[nTuple * zFrames + nFrame] =
                  MRILseq_vox(mri, nX, nY, nZ, nFrame);
              break;
            case MRI_FLOAT:
              ((float *)ptr)[nTuple * zFrames + nFrame] =
                  MRIFseq_vox(mri, nX, nY, nZ, nFrame);
              break;
            case MRI_SHORT:
              ((short *)ptr)[nTuple * zFrames + nFrame] =
...
vtkImageData *FSVolume::GetImageOutput() { return m_imageData; }
    ...
    for (cornerFactor[2] = 0; cornerFactor[2] <= 1; cornerFactor[2]++) {
      for (cornerFactor[1] = 0; cornerFactor[1] <= 1; cornerFactor[1]++) {
        for (cornerFactor[0] = 0; cornerFactor[0] <= 1; cornerFactor[0]++) {
          ...
          this->OriginalIndexToRAS(
              cornerFactor[0] * m_MRI->width, cornerFactor[1] * m_MRI->height,
              cornerFactor[2] * m_MRI->depth, RAS[0], RAS[1], RAS[2]);
...
void FSVolume::GetBounds(MRI *mri, float oRASBounds[6]) {
  ...
  for (cornerFactor[2] = 0; cornerFactor[2] <= 1; cornerFactor[2]++) {
    for (cornerFactor[1] = 0; cornerFactor[1] <= 1; cornerFactor[1]++) {
      for (cornerFactor[0] = 0; cornerFactor[0] <= 1; cornerFactor[0]++) {
        ::MRIvoxelToWorld(mri, cornerFactor[0] * (mri->width - 1),
                          cornerFactor[1] * (mri->height - 1),
                          cornerFactor[2] * (mri->depth - 1), &RAS[0], &RAS[1],
                          &RAS[2]);
...
void FSVolume::TargetToRAS(double x_in, double y_in, double z_in, double &x_out,
...
void FSVolume::RASToTarget(const double *pos_in, double *pos_out) {
  double pos[4] = {0};
  ::MRIworldToVoxel(m_MRITarget, (float)pos_in[0], (float)pos_in[1],
                    (float)pos_in[2], &pos[0], &pos[1], &pos[2]);
  double vs[3]     = {1, 1, 1};
  double origin[3] = {0};
  if (m_imageData != NULL) {
    m_imageData->GetSpacing(vs);
...
void FSVolume::SetConform(bool bConform) { m_bConform = bConform; }
...
HISTOGRAM *MRIhistogramWithHighThreshold(MRI *mri, int nbins, HISTOGRAM *histo,
                                         float thresh, bool highThresh,
                                         int frame, float fmin_in,
                                         float fmax_in) {
...
#ifdef HAVE_OPENMP
#pragma omp parallel for shared(histos, width, height, depth, fmin, fmax,      \
                                frame, x0, y0, z0, histo)
#endif
  for (z = z0; z < depth; z++) {
    int   y, x, tid;
    float val;
    for (y = y0; y < height; y++) {
      for (x = x0; x < width; x++) {
        ...
        if (FZERO(val) || (highThresh && val > thresh))
          continue;
//        val = MRIgetVoxVal(mri, x, y, z, frame);
#ifdef HAVE_OPENMP
...
void FSVolume::UpdateHistoCDF(int frame, float threshold, bool highThresh) {
  ...
  for (int x = 0; x < m_MRI->width; x++)
    for (int y = 0; y < m_MRI->height; y++)
      for (int z = 0; z < m_MRI->depth; z++) {
        ...
        if (FZERO(val) || (highThresh && val > threshold))
          continue;
        HISTOaddSample(histo, val, 0, 0);
      }
      */
  HISTO *histo = MRIhistogramWithHighThreshold(
      m_MRI, 1000, NULL, threshold, highThresh, frame, fMinValue, fMaxValue);
  ...
  HISTOfree(&histo);
  if (!m_histoCDF) {
    std::cout << "Could not create HISTO" << std::endl;
    return;
  }
...
bool FSVolume::Segment(int min_label_index, int max_label_index,
                       int min_number_of_voxels) {
