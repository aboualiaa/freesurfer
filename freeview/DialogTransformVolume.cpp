...
DialogTransformVolume::DialogTransformVolume(QWidget *parent)
    : QDialog(parent), UIUpdateHelper(), m_dIncrementRotate(ROTATION_INCREMENT),
      m_dIncrementTranslate(TRANSLATION_INCREMENT),
      m_dIncrementScale(SCALE_INCREMENT), ui(new Ui::DialogTransformVolume) {
  ui->setupUi(this);
  ...
  m_colorPickerLandmark << ui->colorPickerLandmark1 << ui->colorPickerLandmark2
                        << ui->colorPickerLandmark3 << ui->colorPickerLandmark4;
  m_comboLandmark << ui->comboBoxAxis11 << ui->comboBoxAxis12
                  << ui->comboBoxAxis21 << ui->comboBoxAxis22;
  ...
  connect(MainWindow::GetMainWindow()->GetLayerCollection("MRI"),
          SIGNAL(ActiveLayerChanged(Layer *)), this,
          SLOT(OnActiveLayerChanged()));
  ...
  LayerLandmarks *landmarks =
      (LayerLandmarks *)MainWindow::GetMainWindow()->GetSupplementLayer(
          "Landmarks");
...
DialogTransformVolume::~DialogTransformVolume() { delete ui; }
...
void DialogTransformVolume::UpdateUI(int scope) {
  ...
  if (layer) {
    ...
    if (scope == 1 || scope == 2) {
      ...
      for (int i = 0; i < 3; i++) {
        if (scale[i] >= 1) {
          m_scrollScale[i]->setValue(nmax / 2 +
                                     (int)((scale[i] - 1.0) * nmax / 2));
        } else {
    ...
    ui->groupBoxAxis->setDisabled(angle[0] != 0 || angle[1] != 0 ||
                                  angle[2] != 0);
    for (int i = 0; i < 3; i++) {
      double val = angle[i];
      while (val > 180)
...
bool DialogTransformVolume::GetRotation(int nIndex_in, int &plane_out,
                                        double &angle_out) {
  if (nIndex_in < 0 || nIndex_in > 2 ||
...
void DialogTransformVolume::OnApply() {
  if (ui->radioButtonRotateManual->isChecked()) {
    ...
    if (!m_checkRotate[0]->isChecked() && !m_checkRotate[1]->isChecked() &&
      ...
      QMessageBox::warning(this, "Error", "Must at least select one rotation.");
      return;
    } else if ((m_checkRotate[0]->isChecked() &&
                !GetRotation(0, plane, angle)) ||
               (m_checkRotate[1]->isChecked() &&
                !GetRotation(1, plane, angle)) ||
               (m_checkRotate[2]->isChecked() &&
                !GetRotation(2, plane, angle))) {
      QMessageBox::warning(this, "Error",
                           "Please enter correct rotation angle.");
      return;
    }
  } else {
    if (ui->comboBoxAxis11->currentIndex() ==
            ui->comboBoxAxis12->currentIndex() ||
        ui->comboBoxAxis21->currentIndex() ==
            ui->comboBoxAxis22->currentIndex() ||
        ui->comboBoxAxisTarget1->currentIndex() ==
            ui->comboBoxAxisTarget2->currentIndex() ||
        (ui->comboBoxAxis11->currentIndex() ==
             ui->comboBoxAxis21->currentIndex() &&
         ui->comboBoxAxis12->currentIndex() ==
             ui->comboBoxAxis22->currentIndex()) ||
        (ui->comboBoxAxis11->currentIndex() ==
      ...
      QMessageBox::warning(
          this, "Error",
          "Something is wrong on the mapping settings. Please correct it.");
      return;
    }
...
void DialogTransformVolume::OnActiveLayerChanged() {
  ...
  if (!layer)
    return;
  connect(layer->GetProperty(), SIGNAL(ColorMapChanged()), this,
          SLOT(OnActiveLayerChanged()), Qt::UniqueConnection);
  LayerLandmarks *landmarks =
      (LayerLandmarks *)MainWindow::GetMainWindow()->GetSupplementLayer(
          "Landmarks");
  ...
  if (isVisible()) {
    ...
    if (bOK) {
      ...
      if (layer) {
        ...
        m_scrollTranslate[n]->setValue(range / 2 +
                                       (int)(pos[n] / m_dIncrementTranslate));
  ...
  if (isVisible()) {
    ...
    if (bOK && dvalue > 0) {
      ...
      if (layer) {
        ...
        if (dvalue >= 1) {
          m_scrollScale[n]->setValue(nmax / 2 +
                                     (int)((dvalue - 1.0) * nmax / 2));
        } else {
