...
void MyVTKUtils::ViewportToWorld(vtkRenderer *renderer, double x, double y,
                                 double z, double &world_x, double &world_y,
                                 double &world_z) {
  world_x = x;
  world_y = y;
  renderer->ViewportToNormalizedViewport(world_x, world_y);
  NormalizedViewportToWorld(renderer, world_x, world_y, z, world_x, world_y,
                            world_z);
}
...
void MyVTKUtils::ViewportToWorld(vtkRenderer *renderer, double x, double y,
  ...
  world_y = y;
  renderer->ViewportToNormalizedViewport(world_x, world_y);
  NormalizedViewportToWorld(renderer, world_x, world_y, world_x, world_y,
                            world_z);
}

void MyVTKUtils::NormalizedViewportToWorld(vtkRenderer *renderer, double x,
                                           double y, double z, double &world_x,
...
void MyVTKUtils::NormalizedViewportToWorld(vtkRenderer *renderer, double x,
                                           double y, double &world_x,
                                           double &world_y, double &world_z) {
  NormalizedViewportToWorld(renderer, x, y, 0.0, world_x, world_y, world_z);
}

void MyVTKUtils::WorldToViewport(vtkRenderer *renderer, double world_x,
                                 double world_y, double world_z, double &x,
                                 double &y, double &z) {
  x = world_x;
  y = world_y;
  z = world_z;
...
bool MyVTKUtils::BuildLabelContourActor(vtkImageData *data_in, int labelIndex,
                                        vtkActor *actor_out,
  ...
  contour->SetInputConnection(bUpsample ? resampler->GetOutputPort()
    ...
    for (int i = 0; i < dim[0]; i++) {
      for (int j = 0; j < dim[1]; j++) {
        for (int k = 0; k < dim[2]; k++) {
          ...
          if (val > 0) {
            points->InsertNextPoint(origin[0] + voxel_size[0] * i,
                                    origin[1] + voxel_size[1] * j,
                                    origin[2] + voxel_size[2] * k);
...
bool MyVTKUtils::BuildLabelContourActor(vtkImageData *    data_in,
  ...
  {
    vtkPolyDataMapper* mapper = vtkPolyDataMapper::SafeDownCast(
  actor_out->GetMapper() ); mapper->SetInput( polydata ); ret = false;
  }
  else*/
  {
    vtkSmartPointer<vtkPolyDataConnectivityFilter> conn =
    ...
    vtkPolyDataMapper *mapper =
        vtkPolyDataMapper::SafeDownCast(actor_out->GetMapper());
    mapper->SetInputConnection(cleaner->GetOutputPort());
    mapper->ScalarVisibilityOn();
  }
...
bool MyVTKUtils::BuildContourActor(vtkImageData *data_in, double dTh1,
  ...
  resampler->SetOutputSpacing(vs[0] / 2, vs[1] / 2, vs[2] / 2);
  if (ext) {
    vtkSmartPointer<vtkImageClip> clipper =
...
bool MyVTKUtils::BuildVolume(vtkImageData *data_in, double dTh1, double dTh2,
                             vtkVolume *vol_out) {
  ...
  */
  ...
  // volumeMapper->SetCroppingRegionPlanes(0, dim[0]*2, 0, dim[1]*2, 0,
  // dim[2]*2-16*2); volumeMapper->CroppingOn();
...
void MyVTKUtils::GetLivewirePoints(vtkImageData *image_in, int nPlane_in,
                                   int nSlice_in, double *pt1_in,
                                   double *pt2_in, vtkPoints *pts_out) {
...
double MyVTKUtils::GetImageDataComponent(char *ptr, int *dim,
                                         size_t nNumberOfFrames, size_t i,
                                         size_t j, size_t k, size_t nframe,
                                         int data_type) {
  switch (data_type) {
  case VTK_UNSIGNED_CHAR:
    return ((unsigned char *)
                ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
                     nframe];
  case VTK_INT:
    return (
        (int *)ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
                    nframe];
  case VTK_LONG:
    return (
        (long *)ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
                     nframe];
  case VTK_FLOAT:
    return (
        (float *)ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
                      nframe];
  case VTK_SHORT:
    return (
        (short *)ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
                      nframe];
  }
  return 0;
}

void MyVTKUtils::SetImageDataComponent(char *ptr, int *dim,
                                       size_t nNumberOfFrames, size_t i,
                                       size_t j, size_t k, size_t nframe,
                                       int data_type, double val) {
  switch (data_type) {
  case VTK_UNSIGNED_CHAR:
    ((unsigned char *)
         ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
              nframe] = ((unsigned char)val);
    break;
  case VTK_INT:
    ((int *)ptr)[(k * dim[0] * dim[1] + j * dim[0] + i) * nNumberOfFrames +
                 nframe] = ((int)val);
