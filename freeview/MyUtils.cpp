...
void MyUtils::FloodFill(char **data, int x, int y, int min_x, int min_y,
                        int max_x, int max_y, int fill_value,
                        int border_value) {
  int         left, x1, x2, dy;
  ...
  while (sp > stack) {
    ...
    for (x = x1;
         x >= min_x && data[y][x] != border_value && data[y][x] != fill_value;
         x--) {
      data[y][x] = fill_value;
    }
    ...
    do {
      for (;
           x <= max_x && data[y][x] != border_value && data[y][x] != fill_value;
           x++) {
        data[y][x] = fill_value;
      }
    ...
    SKIP:
      for (x++;
           x <= x2 && (data[y][x] == border_value || data[y][x] == fill_value);
           x++) {
        ;
      }
...
void MyUtils::FloodFill(char *data, int x, int y, int dim_x, int dim_y,
                        int fill_value, int border_value) {
  int         left, x1, x2, dy;
  LINESEGMENT stack[MAXDEPTH], *sp = stack;

  if (data[y * dim_x + x] == border_value ||
  ...
  while (sp > stack) {
    ...
    for (x = x1; x >= min_x && data[y * dim_x + x] != border_value &&
                 data[y * dim_x + x] != fill_value;
         x--) {
      data[y * dim_x + x] = fill_value;
    }
    ...
    do {
      for (; x <= max_x && data[y * dim_x + x] != border_value &&
             data[y * dim_x + x] != fill_value;
           x++) {
        data[y * dim_x + x] = fill_value;
      }
    ...
    SKIP:
      for (x++; x <= x2 && (data[y * dim_x + x] == border_value ||
                            data[y * dim_x + x] == fill_value);
           x++) {
        ;
      }
...
template <class T>
bool CalculateOptimalVolume_t(int *vox1, int nsize1, int *vox2, int nsize2,
                              std::vector<void *> input_volumes,
                              T *output_volume, int vol_size) {
  ...
  std::cout << "condition number: " << MatrixConditionNumber(cov) << "\n";
...
bool MyUtils::CalculateOptimalVolume(int *vox1, int nsize1, int *vox2,
                                     int                 nsize2,
                                     std::vector<void *> input_volumes,
                                     float *output_volume, int vol_size) {
  return CalculateOptimalVolume_t(vox1, nsize1, vox2, nsize2, input_volumes,
                                  output_volume, vol_size);
}
...
template <class T>
double calculate_correlation_coefficient(float *x, T *y, int n) {
...
bool MyUtils::FindIntersection(const double &x0, const double &y0,
                               ...
                               const double &a1, const double &b1, double *x,
                               double *y) {
...
bool MyUtils::FindIntersection(std::vector<std::vector<double>> &line0,
                               ...
                               double *x, double *y, int *n0, int *n1) {
  for (size_t i = 0; i < line0.size() - 1; i++) {
    for (size_t j = 0; j < line1.size() - 1; j++) {
      if (FindIntersection(line0[i][0], line0[i][1], line0[i + 1][0],
                           line0[i + 1][1], line1[j][0], line1[j][1],
                           line1[j + 1][0], line1[j + 1][1], x, y)) {
        if (n0)
          *n0 = i;
        if (n1)
          *n1 = j;
...
QStringList MyUtils::SplitString(const QString &strg_to_split,
                                 const QString &divider, int nIgnoreStart,
                                 int nIgnoreLength) {
  QStringList sa;
  ...
  while (n != -1) {
    if (nMark < nIgnoreStart || nMark >= nIgnoreStart + nIgnoreLength) {
      ...
      while (n != -1 && (nMark + n + (int)divider.length()) >= nIgnoreStart &&
             (nMark + n + (int)divider.length()) <
                 (nIgnoreStart + nIgnoreLength)) {
