...
RenderView3D::RenderView3D(QWidget *parent) : RenderView(parent) {
  m_interactorStyle =
      vtkSmartPointer<vtkInteractorStyleMyTrackballCamera>::New();
...
void RenderView3D::OnIdle() {
  if (m_bToUpdateRASPosition) {
    if (QApplication::mouseButtons() == Qt::NoButton)
      DoUpdateRASPosition(m_nPickCoord[0], m_nPickCoord[1], false,
                          m_bSlicePickOnly);
...
void RenderView3D::UpdateSurfaceCorrelationData() {
  QList<Layer *> layers =
      MainWindow::GetMainWindow()->GetLayerCollection("Surface")->GetLayers();
...
void RenderView3D::UpdateSliceFrames() {
  ...
  vtkPolyDataMapper::SafeDownCast(m_actorSliceFrames[0]->GetMapper())
  ...
  polydata->SetLines(lines);
#if VTK_MAJOR_VERSION > 5
  vtkPolyDataMapper::SafeDownCast(m_actorSliceFrames[1]->GetMapper())
      ->SetInputData(polydata);
#else
  vtkPolyDataMapper::SafeDownCast(m_actorSliceFrames[1]->GetMapper())
      ->SetInput(polydata);
  ...
  polydata->SetLines(lines);
#if VTK_MAJOR_VERSION > 5
  vtkPolyDataMapper::SafeDownCast(m_actorSliceFrames[2]->GetMapper())
      ->SetInputData(polydata);
#else
  vtkPolyDataMapper::SafeDownCast(m_actorSliceFrames[2]->GetMapper())
      ->SetInput(polydata);
...
void RenderView3D::ResetViewLeft() { Reset(); }
...
void RenderView3D::RefreshAllActors(bool bForScreenShot) {
  ...
  if (!mainwnd->GetLayerCollection("MRI")->IsEmpty() ||
      !mainwnd->GetLayerCollection("Surface")->IsEmpty()) {
...
void RenderView3D::DoUpdateRASPosition(int posX, int posY, bool bCursor,
  ...
  LayerCollection *lc_roi =
      MainWindow::GetMainWindow()->GetLayerCollection("ROI");
  LayerCollection *lc_surface =
  ...
  vtkCellPicker *picker = vtkCellPicker::SafeDownCast(
      this->GetRenderWindow()->GetInteractor()->GetPicker());
    ...
    for (int i = 0; i < 3; i++) {
      if (m_actorSliceBoundingBox[i].GetPointer() == prop) {
        if (fabs(pos[0] - m_dBounds[0]) < tolerance ||
          ...
          screen_pt[1]   = y;
          screen_pt[2]   = 0;
          vtkPoints *pts = vtkPolyDataMapper::SafeDownCast(
                               m_actorSliceFrames[i]->GetMapper())
                               ->GetInput()
                               ->GetPoints();
          for (int j = 0; j < 4; j++) {
          ...
          for (int j = 0; j < 4; j++) {
            double dist = vtkLine::DistanceToLine(
                screen_pt, screen_pts[ids[j][0]], screen_pts[ids[j][1]]);
            if (dist < dMinDist) {
              dMinDist = dist;
            }
          }
    ...
    if (!bFramePicked && !bSlicePickOnly) {
      ...
      if (lc_mri->HasProp(prop) || lc_roi->HasProp(prop)) {
        if (bCursor) {
          ...
          LayerVolumeTrack *vt =
              qobject_cast<LayerVolumeTrack *>(lc_mri->HasProp(prop));
          if (vt) {
            QVariantMap info = vt->GetLabelByProp(prop);
            if (!info.isEmpty()) {
              this->setToolTip(QString("%1 %2")
                                   .arg(info["label"].toInt())
                                   .arg(info["name"].toString()));
        ...
        if (layer) {
          ...
          QVariantMap   settings =
              MainWindow::GetMainWindow()->GetGeneralSettings();

          MapInflatedCoords(surf, pos, pos,
                            ((!bCursor) || GetInteractionMode() == IM_ROIEdit)
                                ? false
                                : settings["AutoReorientView"].toBool(),
                            bCursor);
        }
...
bool RenderView3D::MapInflatedCoords(LayerSurface *surf, double *pos_in,
                                     double *pos_out, bool bAutoOrient,
                                     bool bCursor) {
  ...
  if (QFileInfo(surf->GetFileName()).fileName().contains("inflated") ||
      surf->GetActiveSurface() == FSSurface::SurfaceInflated) {
    ...
    foreach (Layer *s, layers) {
      ...
      if (f != surf && QFileInfo(f->GetFileName())
                           .fileName()
                           .contains(surf->GetMappingSurfaceName())) {
        if (f->GetHemisphere() == surf->GetHemisphere() &&
            QFileInfo(f->GetFileName()).absolutePath() ==
    ...
    if (!bFoundMappingSurface &&
        fsurf->IsSurfaceLoaded(FSSurface::SurfaceWhite)) {
...
void RenderView3D::MapToInflatedCoords(double *pos_in) {
  ...
  foreach (Layer *s, layers) {
    LayerSurface *f = (LayerSurface *)s;
    if (f != inflated && QFileInfo(f->GetFileName())
                             .fileName()
                             .contains(inflated->GetMappingSurfaceName())) {
      if (f->GetHemisphere() == inflated->GetHemisphere() &&
          QFileInfo(f->GetFileName()).absolutePath() ==
              QFileInfo(inflated->GetFileName()).absolutePath()) {
...
int RenderView3D::PickCurrentSurfaceVertex(int posX, int posY,
                                           LayerSurface *curSurf) {
  LayerCollection *lc_surface =
      MainWindow::GetMainWindow()->GetLayerCollection("Surface");
...
bool RenderView3D::GetShowSliceFrames() { return m_bShowSliceFrames; }
...
void RenderView3D::MoveSliceToScreenCoord(int x, int y) {
  ...
  } else if (new_pt[m_nSliceHighlighted] >
             bounds[m_nSliceHighlighted * 2 + 1]) {
    new_pt[m_nSliceHighlighted] = bounds[m_nSliceHighlighted * 2 + 1];
  }
  mainwnd->OffsetSlicePosition(
      m_nSliceHighlighted,
      new_pt[m_nSliceHighlighted] - slicepos[m_nSliceHighlighted], false);
...
bool RenderView3D::InitializeSelectRegion(int posX, int posY) {
  ...
  for (int i = 0; i < lc_mri->GetNumberOfLayers(); i++) {
    ...
    if (mri_temp->HasProp(prop) &&
        mri_temp->GetProperty()->GetShowAsContour()) {
      mri = mri_temp;
      break;
    }
  }
...
bool RenderView3D::PickCroppingBound(int nX, int nY) {
  ...
  if (prop &&
      MainWindow::GetMainWindow()->GetVolumeCropper()->PickActiveBound(prop)) {
    RequestRedraw(true);
    return true;
  } else {
    return false;
...
void RenderView3D::MoveCroppingBound(int nX, int nY) {
  MainWindow::GetMainWindow()->GetVolumeCropper()->MoveActiveBound(this, nX,
                                                                   nY);
}
...
void RenderView3D::Rotate90() { Azimuth(90); }
...
void RenderView3D::TriggerContextMenu(QMouseEvent *event) {
  ...
  slice_names << "Show Sagittal Slice"
              << "Show Coronal Slice"
              << "Show Axial Slice";
  for (int i = 0; i < 3; i++) {
...
void RenderView3D::SetCamera(const QVariantMap &info) {
  ...
  if (cam) {
    ...
    if (info.contains("ViewSize")) {
      ...
      MainWindow::GetMainWindow()->SetViewSize(map["width"].toInt(),
                                               map["height"].toInt());
    }
  }
}
    ...
    foreach (Layer *layer, list) {
      ...
      if (!surf->IsInflated() && surf->IsVisible() && surf->GetVisibleIn3D() &&
        ...
        m_cursor3D->Show();
               ...
               surf->GetVisibleIn3D() && m_bShowCursor)
        m_cursorInflatedSurf->Show();
    }
  }
  if (bShowCursor != m_cursor3D->IsShown() ||
      bShowInflated != m_cursorInflatedSurf->IsShown())
    RequestRedraw();
}
