...
void Interactor2DVolumeEdit::PreprocessMouseEvent(QMouseEvent *event) {
  ...
  bool bRightButtonErase =
      MainWindow::GetMainWindow()->GetSetting("RightButtonErase").toBool();
  if (bRightButtonErase && event->button() == Qt::RightButton &&
      event->modifiers() == Qt::NoModifier) {
    QMouseEvent e(event->type(), event->pos(), Qt::LeftButton, Qt::LeftButton,
                  Qt::ShiftModifier);
    *event = e;
  }
}
...
bool Interactor2DVolumeEdit::ProcessMouseDownEvent(QMouseEvent *event,
  ...
  if (m_nAction == EM_GeoSeg) {
    if ((event->button() == Qt::LeftButton ||
      ...
      LayerMRI *layer_draw = qobject_cast<LayerMRI *>(
          MainWindow::GetMainWindow()->FindSupplementLayer("GEOS_DRAW"));
      LayerCollection *lc =
          MainWindow::GetMainWindow()->GetLayerCollection("Supplement");
      if (!layer_draw) {
        layer_draw = new LayerMRI(mri);
        if (!layer_draw->Create(mri, false, MRI_UCHAR)) {
          //  QMessageBox::warning( this, "Error", "Can not create drawing
          //  layer." );
          delete layer_draw;
          return false;
        }
        ...
        colors[1] = map.contains("ForegroundColor")
                        ? map["ForegroundColor"].value<QColor>()
                        : QColor(0, 255, 0);
        colors[2] = map.contains("BackgroundColor")
        ...
        colors.clear();
        colors[0] = QColor(0, 0, 0, 0);
        colors[1] = map.contains("FillColor") ? map["FillColor"].value<QColor>()
                                              : QColor(255, 255, 0);
        ...
        if (map.contains("Opacity"))
          layer_fill->GetProperty()->SetOpacity(
              map.value("Opacity").toDouble());
  ...
  if (event->button() == Qt::LeftButton ||
    ...
    LayerCollection *lc =
        MainWindow::GetMainWindow()->GetLayerCollection(m_strLayerTypeName);
    QList<Layer *> layers =
        MainWindow::GetMainWindow()->GetSelectedLayers(m_strLayerTypeName);
    if (layers.isEmpty())
    ...
    foreach (LayerVolumeBase *mri, mriLayers) {
      if ((!mri || !mri->IsVisible())) //&& ( event->ControlDown() || m_nAction
                                       //== EM_Polyline ) )
      {
        emit Error(
            QString("Selected layer '%1' is not visible").arg(mri->GetName()),
            mri);
        break;
                                     ...
                                     // EM_Polyline ) )
      {
        emit Error(
            ...
            mri);
        break;
      } else if (m_strLayerTypeName == "MRI" &&
                 ((LayerMRI *)mri)->IsTransformed()) {
        emit Error(
            QString("Selected layer '%1' is not editable for transformation")
                .arg(mri->GetName()),
            mri);
        break;
      } else {
        m_nMousePosX = event->x();
        m_nMousePosY = event->y();
        bool bFill3D = (LayerMRI *)MainWindow::GetMainWindow()
                           ->GetBrushProperty()
                           ->GetFill3D();

        double ras[3];
        view->MousePositionToRAS(m_nMousePosX, m_nMousePosY, ras);
        bool bCondition = !(event->modifiers() & Qt::ShiftModifier) &&
                          !(event->buttons() & Qt::RightButton);
        if (bCondition && (m_nAction == EM_Freehand || m_nAction == EM_Fill ||
                           m_nAction == EM_Polyline)) {
          if (mri->IsTypeOf("MRI") &&
              ((LayerMRI *)mri)->GetProperty()->GetColorMap() ==
                  LayerPropertyMRI::LUT &&
              !((LayerMRI *)mri)
                   ->GetProperty()
                   ->IsValueInColorTable(mri->GetFillValue())) {
            emit Error("Brush value is not in the current color table. I don't "
                       "know what color to use. Drawing cannot continue.");
            return false;
          }
        }

        if ((m_nAction == EM_ColorPicker || m_bColorPicking) &&
            mri->IsTypeOf("MRI")) {
          if (event->modifiers() & CONTROL_MODIFIER) {
            ...
            if (dValue != 0) {
              MainWindow::GetMainWindow()->GetBrushProperty()->SetFillValue(
                  (float)dValue);
            }
          }
        ...
        } else if (m_nAction == EM_Freehand) //&& ( event->ControlDown() ) )
        {
          ...
          LayerMRI *mri_ref = (LayerMRI *)MainWindow::GetMainWindow()
                                  ->GetBrushProperty()
                                  ->GetReferenceLayer();
          if (!mri_ref) {
          ...
          if ((event->modifiers() & CONTROL_MODIFIER) &&
            ...
            if (dValue != 0) {
              m_bEditing = true;
              c2d->SetInput(mri_ref->GetSliceImageData(view->GetViewPlane()),
                            dValue, ras[view->GetViewPlane()],
                            mri_ref->GetActiveFrame());
              c2d->SetVisible(true);
              view->RequestRedraw();
    ...
    if (m_nAction == EM_Polyline || m_nAction == EM_Livewire) {
      if (event->button() == Qt::MidButton) {
        ...
        if (m_dPolylinePoints.size() > 0 && m_nAction == EM_Polyline) {
          LayerCollection *lc = MainWindow::GetMainWindow()->GetLayerCollection(
              m_strLayerTypeName);
          QList<Layer *> layers =
              MainWindow::GetMainWindow()->GetSelectedLayers(
                  m_strLayerTypeName);
          if (layers.isEmpty())
            layers << lc->GetActiveLayer();

          double ras1[3] = {m_dPolylinePoints[0], m_dPolylinePoints[1],
                            m_dPolylinePoints[2]};
          double ras2[3];
          view->GetCursor2D()->GetPosition(ras2);
  ...
  return Interactor2D::ProcessMouseDownEvent(event,
                                             renderview); // pass down the event
}
...
bool Interactor2DVolumeEdit::ProcessMouseUpEvent(QMouseEvent *event,
  ...
  //  if (m_nAction == EM_GeoSeg)
  //    QTimer::singleShot(0, MainWindow::GetMainWindow(),
  //    SIGNAL(SupplementLayerChanged()));
...
bool Interactor2DVolumeEdit::ProcessMouseMoveEvent(QMouseEvent *event,
  ...
  if (m_nAction == EM_GeoSeg && m_bEditing) {
    LayerCollection *lc =
        MainWindow::GetMainWindow()->GetLayerCollection("Supplement");
    QList<Layer *> layers =
        lc->GetLayers("MRI"); // Get foreground/background drawing layers
  ...
  if (m_bEditing) {
    ...
    if (m_nAction == EM_Freehand) {
      ...
      view->GetCursor2D()->SetPosition(view->GetCursor2D()->GetPosition(),
                                       true);
      view->RequestRedraw();
    } else if (m_nAction == EM_Shift) {
      double ras1[3], ras2[3];
...
bool Interactor2DVolumeEdit::ProcessKeyDownEvent(QKeyEvent * event,
  ...
  if (false) // (event->modifiers() & Qt::ShiftModifier) && m_nAction == EM_Shift )
  {
    ...
    LayerCollection *lc =
        MainWindow::GetMainWindow()->GetLayerCollection(m_strLayerTypeName);
    LayerVolumeBase *mri =
        qobject_cast<LayerVolumeBase *>(lc->GetActiveLayer());
    if (mri) {
...
void Interactor2DVolumeEdit::UpdateCursor(QEvent *event, QWidget *wnd) {
  if (wnd->hasFocus()) {
    ...
    if (m_nAction != EM_Fill) {
      ...
      if (bMouseEvent &&
          ((QMouseEvent *)event)->modifiers() & CONTROL_MODIFIER &&
