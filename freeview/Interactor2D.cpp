...
Interactor2D::Interactor2D(QObject *parent)
    : Interactor(parent), m_nMousePosX(-1), m_nMousePosY(-1),
      m_bWindowLevel(false), m_bChangeSlice(false), m_bMovingCursor(false),
      m_bSelecting(false) {}
...
bool Interactor2D::ProcessMouseDownEvent(QMouseEvent *event,
  ...
  if (event->button() == Qt::LeftButton) {
      ...
      if (mainwnd->IsRepositioningSurface()) {
        if (event->modifiers() & CONTROL_MODIFIER &&
          ...
          QTimer::singleShot(0, mainwnd,
                             SIGNAL(SurfaceRepositionIntensityChanged()));
    ...
    return Interactor::ProcessMouseDownEvent(event,
  ...
  if (m_bChangeSlice) {
    ...
    if (layer) {
      ...
      case LayerPropertyMRI::Heat:
        w += layer->GetProperty()->GetHeatScaleMaxThreshold() -
             layer->GetProperty()->GetHeatScaleMinThreshold();
        l += (layer->GetProperty()->GetHeatScaleMaxThreshold() +
              layer->GetProperty()->GetHeatScaleMinThreshold()) /
             2;
        if (w < 0) {
          w = 0;
        ...
        break;
      default:
        w += layer->GetProperty()->GetMaxGenericThreshold() -
             layer->GetProperty()->GetMinGenericThreshold();
        l += (layer->GetProperty()->GetMaxGenericThreshold() +
              layer->GetProperty()->GetMinGenericThreshold()) /
             2;
        if (w < 0) {
          w = 0;
...
bool Interactor2D::ProcessKeyDownEvent(QKeyEvent * event,
  ...
  if ( event->modifiers() & Qt::ShiftModifier )
  {
    ...
    view->MoveSlice(-1);
  } else if (nKeyCode == Qt::Key_3 /*|| nKeyCode == 'W' || nKeyCode == 'S'*/ ||
             nKeyCode == Qt::Key_R || nKeyCode == Qt::Key_F) {
