...
LayerPointSet::LayerPointSet(LayerMRI *ref, int nType, QObject *parent)
  ...
  connect(p, SIGNAL(SnapToVoxelCenterChanged(bool)), this,
          SLOT(UpdateSnapToVoxelCenter()));
  connect(p, SIGNAL(SplineVisibilityChanged(bool)), this,
          SLOT(UpdateSplineVisibility()));
}
...
bool LayerPointSet::LoadFromFile(const QString &filename) {
  if (!LoadFromJsonFile(filename)) {
    ...
    GetProperty()->SetStatRange(m_pointSetSource->GetMinStat(),
                                m_pointSetSource->GetMaxStat());
    m_points.clear();
...
bool LayerPointSet::LoadFromJsonFile(const QString &filename) {
  ...
  foreach (QVariant v, list) {
    ...
    if (coord_strg == "tkreg") {
      ...
      MRIvoxelToWorld(ref_vol->GetMRI(), wp.pt[0], wp.pt[1], wp.pt[2], wp.pt,
                      wp.pt + 1, wp.pt + 2);
    }
...
bool LayerPointSet::HasUndo() { return m_bufferUndo.size() > 0; }

bool LayerPointSet::HasRedo() { return m_bufferRedo.size() > 0; }
...
bool LayerPointSet::IsVisible() { return m_actorSlice[0]->GetVisibility() > 0; }
...
void LayerPointSet::RebuildActors(bool bRebuild3D) {
  ...
  if (GetProperty()->GetShowSpline()) {
    ...
    if (GetProperty()->GetScalarType() == LayerPropertyPointSet::ScalarSet) {
      ...
      spline->SetNumberOfSubdivisions(GetProperty()->GetActiveScalarSet().nNum -
                                      1);
    }
    if (m_points.size() > 1) {
      //  polydata->Update();
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < m_points.size(); j++) {
      if (radius > 0 &&
        ...
        vtkSmartPointer<vtkPolyData> cutpoly =
    ...
    if (polydata_tube) {
      ...
      cutpoly->SetPoints(stripper->GetOutput()->GetPoints());
      cutpoly->SetPolys(stripper->GetOutput()->GetLines());
      cutpoly->GetPointData()->SetScalars(
  ...
  if (true) {
    ...
    for (int i = 0; i < nPts; i++) {
      if (true) // (i%NUM_OF_SIDES) == 0 )
      {
        ...
        } else if (GetProperty()->GetScalarType() ==
...
int LayerPointSet::AddPoint(double *ras_in, double value) {
  ...
  if (m_points.size() < 2 || !GetProperty()->GetShowSpline()) {
    ...
    double d3 =
        vtkMath::Distance2BetweenPoints(m_points[n1].pt, m_points[n2].pt);
...
void LayerPointSet::UpdateOpacity() {
  for (int i = 0; i < 3; i++) {
    ...
    m_actorSplineSlice[i]->GetProperty()->SetOpacity(
...
void LayerPointSet::UpdateColorMap() {
  for (int i = 0; i < 3; i++) {
    ...
    m_actorSplineSlice[i]->GetProperty()->SetColor(
        GetProperty()->GetSplineColor());
  }
  ...
  case LayerPropertyPointSet::HeatScale:
    ...
    if (m_actorSplineSlice[0]->GetMapper()) {
      for (int i = 0; i < 3; i++) {
        ...
        m_actorSplineSlice[i]->GetMapper()->SetLookupTable(
            GetProperty()->GetHeatScaleLUT());
      }
    }
    break;
  }
...
bool LayerPointSet::Rotate(std::vector<RotationElement> &rotations,
...
int LayerPointSet::GetNumberOfPoints() { return m_points.size(); }

bool LayerPointSet::GetCentroidPosition(double *pos) {
  m_pointSetSource->UpdateLabel(m_points, m_layerRef->GetSourceVolume());
  if (m_pointSetSource->GetCentroidRASPosition(pos,
                                               m_layerRef->GetSourceVolume())) {
...
bool LayerPointSet::IsEnhanced() { return !m_mapEnhancedData.isEmpty(); }
