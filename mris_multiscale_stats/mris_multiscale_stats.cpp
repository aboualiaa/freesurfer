...
static int cvector_track_best_stats(float *vsnr, float *vbest_snr,
                                    float *vbest_avgs, float *c1_var,
                                    float *c2_var, float *c1_best_var,
                                    float *c2_best_var, float *c1_mean,
                                    float *c2_mean, float *c1_best_mean,
                                    float *c2_best_mean, int avgs, int num);
...
int main(int argc, char *argv[]) {
  ...
  char **av, *curv_name, *surf_name, *hemi, fname[STRLEN], *cp, *subject_name,
      subjects_dir[STRLEN], **c1_subjects, **c2_subjects;
  int ac, nargs, n, num_class1, num_class2, i, nvertices, avgs, max_snr_avgs,
      nlabels;
  float **c1_curvs, **c2_curvs, *curvs, *c1_mean, *c2_mean, *class_mean,
...
#define ARGV_OFFSET 4
  ...
  if (use_no_distribution)
    snr = cvector_compute_dist_free_snr(c1_curvs, num_class1, c2_curvs,
                                        num_class2, c1_mean, c2_mean, vsnr,
                                        nvertices, &i);
  else
    snr = cvector_compute_snr(c1_mean, c2_mean, vtotal_var, vsnr, nvertices, &i,
                              0.0f);
  ...
  cvector_track_best_stats(vsnr, vbest_snr, vbest_avgs, c1_var, c2_var,
                           ...
                           c1_best_mean, c2_best_mean, 0, nvertices);
  ...
  */
  ...
  for (avgs = 1; avgs <= max_avgs; avgs++) {
    ...
    cvector_track_best_stats(vsnr, vbest_snr, vbest_avgs, c1_var, c2_var,
                             c1_best_var, c2_best_var, c1_mean, c2_mean,
                             c1_best_mean, c2_best_mean, avgs, nvertices);
  }
...
static int segment_and_write_labels(char *subject, char *name,
                                    MRI_SURFACE *mris, LABEL ***plabels,
                                    int *pnlabels, int offset) {
  LABEL **labels, *area;
  int nlabels, i;
