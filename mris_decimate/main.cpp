...
int main(int argc, char *argv[]) {
  ...
  gDecimationOptions.decimationLevel =
      0.5; // Default decimation level if not specified
  ...
  if (gDecimationOptions.desiredNumFaces > 0 &&
      gDecimationOptions.desiredFaceArea > 0) {
  ...
  std::cout << "Original Surface Number of vertices: " << mris->nvertices
            << std::endl;
  std::cout << "Original Surface Number of faces: " << mris->nfaces
            << std::endl;

  if (gDecimationOptions.desiredNumFaces > 0) {
    gDecimationOptions.decimationLevel =
        (float)gDecimationOptions.desiredNumFaces / mris->nfaces;
  ...
  if (gDecimationOptions.desiredFaceArea > 0) {
    ...
    printf("Setting decimation level to %f based on %g/%g\n",
           gDecimationOptions.decimationLevel, avgfacearea,
           gDecimationOptions.desiredFaceArea);
    if (gDecimationOptions.decimationLevel > 1.0) {
      printf("  INFO: decimation level > 1, so setting to 1. There will be no "
...
static void print_usage() {
  outputHelpXml(mris_decimate_help_xml, mris_decimate_help_xml_len);
}
