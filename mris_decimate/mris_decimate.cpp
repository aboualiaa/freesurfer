...
///  with less triangles.  The decimation level and other options can be
...
static void faceLoad(GtsTriangle *t, gpointer *data) {
  ...
  mris->faces[faceLoadContext->nextFace].v[0] =
      (guint)(glong)GTS_OBJECT(v1)->reserved;
  mris->faces[faceLoadContext->nextFace].v[1] =
      (guint)(glong)GTS_OBJECT(v2)->reserved;
  mris->faces[faceLoadContext->nextFace].v[2] =
...
/// &decimationOptions) \brief This function performs decimation on the input
/// surface and outputs the new surface to a file
...
int decimateSurface(MRI_SURFACE **pmris,
  ...
  for (int vno = 0; vno < mris->nvertices; vno++) {
    gtsVertices[vno] =
        gts_vertex_new(gtsSurface->vertex_class, mris->vertices[vno].x,
                       mris->vertices[vno].y, mris->vertices[vno].z);
  }
  ...
  for (int fno = 0; fno < mris->nfaces; fno++) {
    gts_surface_add_face(gtsSurface,
                         gts_face_new(gtsSurface->face_class, gtsEdges[edge],
                                      gtsEdges[edge + 1], gtsEdges[edge + 2]));
    edge += 3;
  }
  ...
  gts_surface_coarsen(gtsSurface, (GtsKeyFunc)gts_volume_optimized_cost,
                      ...
                      &params, (GtsStopFunc)gts_coarsen_stop_number, &stop,
                      fold);
  ...
  // DNG 7/16/18: these two frees were in the original. I don't know how it ever
  // worked because the realloc below needs these to be valid pointers
  // free(mris->vertices);
  // free(mris->faces);
  ...
  MRISreallocVerticesAndFaces(mris, vertexLoadContext.nextVertex,
                              faceLoadContext.nextFace);
