...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_xvolavg.c,v 1.6 2011/03/02 00:04:26 nicks Exp $",
      "$Name:  $");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--nodebug"))
      debug = 0;
...
static void print_help() {
  ...
  printf("This program will average multiple volumes together (including 4D "
         "volumes).\n"
         "\n"
         "OPTIONS\n"
         "\n"
         ...
         "\n"
         "mri_xvolavg accepts all input types as mri_convert (see mri_convert "
         "--help \n"
         "for more information).\n"
         "\n"
         "NOTES\n"
         "\n"
         "There is another program called mri_average which has similar "
         "functionality.\n"
         "mri_average can align the input volumes and force them to conform to "
         "the COR\n"
