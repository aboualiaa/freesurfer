...
int MRIScomputeBorderValues_MEF_WHITE(MRI_SURFACE *mris, MRI *mri_em_combined,
                                      MRI *mri_30, MRI *mri_5, float wm_mean[2],
                                      float wm_std[2], float gm_mean[2],
                                      float gm_std[2], double sigma,
                                      float max_thickness, FILE *log_fp);

int MRIScomputeBorderValues_PD_WHITE(MRI_SURFACE *mris, MRI *mri_PD,
                                     float wm_mean, float wm_std, float gm_mean,
                                     float gm_std, double sigma,
                                     float max_thickness, FILE *log_fp);

int MRIScomputeBorderValues_MEF_PIAL(MRI_SURFACE *mris, MRI *mri_em_combined,
                                     MRI *mri_30, MRI *mri_5, float wm_mean[2],
                                     float wm_std[2], float gm_mean[2],
                                     float gm_std[2], double sigma,
                                     float max_thickness, FILE *log_fp,
...
int MRIScomputeBorderValues_PD_PIAL(MRI_SURFACE *mris, MRI *mri_PD,
                                    float wm_mean, float wm_std, float gm_mean,
                                    float gm_std, double sigma,
                                    float max_thickness, FILE *log_fp,
                                    int formalin, int first);
int MRIScomputeBorderValues_T1_PIAL(MRI_SURFACE *mris, MRI *mri_PD,
                                    float wm_mean, float wm_std, float gm_mean,
                                    float gm_std, double sigma,
                                    float max_thickness, FILE *log_fp,
                                    int formalin, int first);
...
static char *PD_name = nullptr;
static char *T1_name = nullptr;
...
main(int argc, char *argv[]) {
  ...
  parms.fill_interior =
      0; // don't let gradient use exterior information (slows things down)
  parms.tol = 5e-3;
  parms.check_tol = 1;
  parms.dt = 0.5f;
  ...
  MRIcomputeClassStatistics_mef(mri_T1_30, mri_T1_5, mri_filled, white_mean,
  ...
  if (PD_name) {
    ...
    if (mri_PD == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not load PD volume %s", Progname,
                fname);
    MRIcomputeClassStatistics_mef(mri_PD, mri_T1_5, mri_filled, PD_white_mean,
                                  PD_white_std, PD_gray_mean, PD_gray_std);
#define USE_PD_FOR_WHITE 1
#if USE_PD_FOR_WHITE
    MRIcomputeClassStatistics_mef(mri_T1_30, mri_PD, mri_filled, white_mean,
                                  white_std, gray_mean, gray_std);
#endif
  }
  ...
  if (T1_name) {
    ...
    mri_wm_labeled =
        MRIbinarize(mri_filled, nullptr, MIN_WM_VAL, MRI_NOT_WHITE, MRI_WHITE);
...
#define MAX_GM_T1 245
    if (mri_T1 == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not load T1 volume %s", Progname,
                fname);
    MRIcomputeClassStatistics(mri_T1, mri_wm_labeled, -1, -1, &T1_white_mean,
  ...
  printf("WM: mean = (%g, %g) std = (%g, %g)\n", white_mean[0], white_mean[1],
         white_std[0], white_std[1]);
  printf("GM: mean = (%g, %g) std = (%g, %g)\n", gray_mean[0], gray_mean[1],
  ...
  parms.grad_dir = 1;
  for (n_averages = max_white_averages, i = 0; n_averages >= min_white_averages;
...
#define USE_PD_FOR_WHITE 1
#if USE_PD_FOR_WHITE
    if (mri_PD)
      MRIScomputeBorderValues_PD_WHITE(
          mris, mri_PD, PD_white_mean[0], PD_white_std[0], PD_gray_mean[0],
          PD_gray_std[0], current_sigma, max_thickness, parms.fp);
    else
#endif
      MRIScomputeBorderValues_MEF_WHITE(
          mris, mri_T1_30, mri_T1_30, mri_T1_5, white_mean, white_std,
          gray_mean, gray_std, current_sigma, max_thickness, parms.fp);
    ...
    if (vavgs) {
      ...
      if (Gdiag_no > 0) {
        ...
        if (mri_PD)
          ...
          fprintf(stderr,
                  "v %d, target value = %2.1f:%2.1f, mag = %2.1f, dist=%2.2f\n",
    ...
    parms.grad_dir =
        0; // surface should be close enough to true location to trust grad now
  ...
  } else /* read in previously generated white matter surface */
  {
    ...
    if (MRISreadVertexPositions(mris, fname) != NO_ERROR)
      ErrorExit(Gerror, "%s: could not read white matter surfaces.", Progname);
    MRIScomputeMetricProperties(mris);
  }

  if (white_only) {
    msec = then.milliseconds();
    fprintf(stderr, "refinement took %2.1f minutes\n",
  ...
  // as the surface moves out into the brigher gray matter and only start to
  // decrease after it is getting closer to the true pial surface
  if (mri_PD == nullptr && mri_T1 == nullptr) {
    parms.grad_dir = -1;
    parms.check_tol = 0;
    gMRISexternalTimestep =
        externalTimestep; // will turn checking back on when decreasing starts
  } else if (mri_T1) {
    parms.check_tol = 0;
    gMRISexternalTimestep =
        externalTimestep; // will turn checking back on when decreasing starts
  ...
  for (j = 0; j <= 0; parms.l_intensity *= 2, j++) /* only once for now */
  {
    ...
    for (n_averages = max_pial_averages, i = 0; n_averages >= min_pial_averages;
      ...
      */
      ...
      if (mri_T1)
        MRIScomputeBorderValues_T1_PIAL(mris, mri_T1, T1_white_mean,
                                        T1_white_std, T1_gray_mean, T1_gray_std,
        ...
        MRIScomputeBorderValues_PD_PIAL(
            mris, mri_PD, PD_white_mean[0], PD_white_std[0], PD_gray_mean[0],
            PD_gray_std[0], current_sigma, 2 * max_thickness, parms.fp,
            formalin, j);
      else
...
static void print_help() {
  ...
  fprintf(stderr, "\nvalid options are:\n\n");
  fprintf(stderr, "-q    omit self-intersection and only generate "
                  "gray/white surface.\n");
  fprintf(stderr,
          "-c    create curvature and area files from white matter surface\n");
  fprintf(stderr,
          "-a <avgs>   average curvature values <avgs> times (default=10)\n");
  fprintf(stderr, "-whiteonly  only generate white matter surface\n");
  fprintf(stderr, "-formalin <0,1>  assume hemi is in formalin\n");
  exit(1);
}
...
int MRIScomputeBorderValues_MEF_WHITE(MRI_SURFACE *mris, MRI *mri_em_combined,
                                      ...
                                      float gm_std[2], double sigma,
                                      float max_thickness, FILE *log_fp) {
  double val30, val5, x, y, z, max_mag_val30, max_mag_val5, xw, yw, zw, mag,
      max_mag, max_mag_dist = 0.0f, min_val30, min_val5, inward_dist,
    ...
    */
    ...
    for (current_sigma = sigma; current_sigma <= 10 * sigma;
      ...
      for (dist = 0; dist > -max_thickness; dist -= step_size) {
        ...
        z = v->z + v->nz * dist;
        MRISsurfaceRASToVoxelCached(mris, mri_30, x, y, z, &xw, &yw, &zw);
        MRIsampleVolumeDerivativeScale(mri_30, xw, yw, zw, nx, ny, nz, &mag30,
                                       current_sigma);
    ...
    if (vno == Gdiag_no) {
      ...
      fprintf(stdout,
              ...
              inward_dist, outward_dist, current_sigma);
    }
    ...
    */
    ...
    for (dist = inward_dist; dist <= outward_dist; dist += STEP_SIZE) {
      ...
      if (previous_val30 > low30 && previous_val5 < gm_mean[1]) {
        ...
        */
        if (((mag) >
             (previous_mag)) && // use signed mags - they should be positive
            ((mag) > (next_mag)) &&
          ...
          */
          if ((((local_max_found == 0) && ((mag) > max_mag)) ||
               (((mag) > 2 * max_mag))) &&
    ...
    if (vno == Gdiag_no)
      fprintf(stdout,
              "v %d, target value = 30:%2.1f, 5:%2.1f, mag = %2.1f, dist = "
              "%2.2f, %s\n",
              Gdiag_no, v->val, v->valbak, v->mean, v->d,
              local_max_found ? "local max"
                              : max_mag_val30 > 0 ? "grad" : "min");
  }
...
int MRIScomputeBorderValues_PD_WHITE(MRI_SURFACE *mris, MRI *mri_PD,
  ...
  double val, x, y, z, max_mag_val, xw, yw, zw, mag, max_mag,
      max_mag_dist = 0.0f, min_val, inward_dist, outward_dist, xw1, yw1, zw1,
    ...
    */
    ...
    for (current_sigma = sigma; current_sigma <= 10 * sigma;
      ...
      for (dist = 0; dist > -max_thickness; dist -= step_size) {
        ...
        z = v->z + v->nz * dist;
        MRISsurfaceRASToVoxelCached(mris, mri_PD, x, y, z, &xw, &yw, &zw);
        MRIsampleVolumeDerivativeScale(mri_PD, xw, yw, zw, nx, ny, nz, &mag,
                                       current_sigma);
        if (mag < 0.0) // intensity has started to increase again - opposite
                       // bank
          break;
  ...
  for (total_vertices = vno = 0; vno < mris->nvertices; vno++) {
    ...
    */
    ...
    for (current_sigma = sigma; current_sigma <= 10 * sigma;
      ...
      inward_dist = dist + step_size / 2;
      for (dist = callno == 0 ? 1 : 0; dist < max_thickness;
           dist += step_size) {
        dx = v->x - v->origx;
        dy = v->y - v->origy;
        dz = v->z - v->origz;
    ...
    */
    ...
    for (dist = inward_dist; dist <= outward_dist; dist += STEP_SIZE) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_30, x, y, z, &xw, &yw, &zw);
      MRIsampleVolume(mri_30, xw, yw, zw, &previous_val30);
      //   MRIsampleVolume(mri_em_combined, xw, yw, zw, &previous_val5) ;
      //   //borrowed
...
static int MRIcomputeClassStatistics_mef(MRI *mri_T1_30, MRI *mri_T1_5,
  ...
  mri_tmp = MRIbinarize(mri_fill, nullptr, 10, MRI_NOT_WHITE, MRI_WHITE);
  mri_border = MRImarkBorderVoxels(mri_tmp, nullptr);
  ...
  for (z = 0; z < depth; z++)
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if (val30 <= 1e-10 || val5 <= 1e-10 || border_label == MRI_AMBIGUOUS)
          continue;
...
int MRIScomputeBorderValues_PD_PIAL(MRI_SURFACE *mris, MRI *mri_PD,
  ...
  // or equivalently, the EM_combined
  // dura may be still low at both flash30 and flash5, but not that low
  double val, x, y, z, next_val, max_mag_val, xw, yw, zw, mag, max_mag,
      max_mag_dist = 0.0f, max_PD, inward_dist, outward_dist, xw1, yw1, zw1,
      max_val_dist, orig_dist, dx, dy, dz;
  double previous_mag, next_mag, previous_val;
  int total_vertices, vno, was_negative,
      nmissing = 0, nout = 0, nin = 0, nfound = 0, nalways_missing = 0,
      local_max_found, ngrad_max, ngrad, nmin, num_changed = 0;
  float mean_border, mean_in, mean_out, dist, nx, ny, nz, mean_dist, step_size;
  double current_sigma;
  VERTEX *v;
  FILE *fp = nullptr;
  ...
  for (total_vertices = vno = 0; vno < mris->nvertices; vno++) {
    ...
    */
    ...
    for (dist = inward_dist; dist <= outward_dist; dist += STEP_SIZE) {
      ...
      /* the previous point was inside the surface */
      if (previous_val > wm_mean && previous_val > (gm_mean - 3 * gm_std)) {
        /* see if we are at a local maximum in the gradient magnitude */
        x = v->x + v->nx * dist;
        y = v->y + v->ny * dist;
        z = v->z + v->nz * dist;
        ...
        */
        ...
        and it is in the right intensity range....
        */
        if (((mag) > (previous_mag)) && ((mag) > (next_mag)) &&
...
int MRIScomputeBorderValues_T1_PIAL(MRI_SURFACE *mris, MRI *mri_T1,
                                    float wm_mean, float wm_std, float gm_mean,
                                    float gm_std, double sigma,
                                    float max_thickness, FILE *log_fp,
                                    int formalin, int callno) {
  ...
  // dura may be still low at both flash30 and flash5, but not that low
  double val, x, y, z, next_val, max_mag_val, xw, yw, zw, mag, max_mag,
      max_mag_dist = 0.0f, max_T1, inward_dist, outward_dist, xw1, yw1, zw1,
      max_val_dist, orig_dist, dx, dy, dz;
  double previous_mag, next_mag, previous_val;
  int total_vertices, vno, was_negative,
      nmissing = 0, nout = 0, nin = 0, nfound = 0, nalways_missing = 0,
    ...
    for (current_sigma = sigma; current_sigma <= 10 * sigma;
      ...
      if (callno == 0)
        ...
        for (dist = 0; dist > -max_thickness; dist -= step_size) {
          ...
          z = v->z + v->nz * dist;
          MRISsurfaceRASToVoxelCached(mris, mri_T1, x, y, z, &xw, &yw, &zw);
          MRIsampleVolumeDerivativeScale(mri_T1, xw, yw, zw, nx, ny, nz, &mag,
                                         current_sigma);
    ...
    */
    ...
    for (dist = inward_dist; dist <= outward_dist; dist += STEP_SIZE) {
      ...
      if (previous_val > wm_mean && previous_val > (gm_mean - 3 * gm_std)) {
        ...
        */
        if (((mag) > (previous_mag)) && ((mag) > (next_mag)) &&
            (val >= (gm_mean + 0.5 * gm_std)) && (val < MAX_GM_T1) &&
          ...
          */
          if ((local_max_found == 0) && ((mag) > max_mag) &&
              (val < MAX_GM_T1) && (val >= (gm_mean + 0.5 * gm_std))) {
            x = v->x + v->nx * (dist + 1);
            y = v->y + v->ny * (dist + 1);
            z = v->z + v->nz * (dist + 1);
            MRISsurfaceRASToVoxelCached(mris, mri_T1, x, y, z, &xw, &yw, &zw);
