...
int MRISripSegs(MRIS *surf, MRI *seg, const double dmin, const double dmax,
                const double dstep);
...
int main(int argc, char **argv) {
  ...
  if (mmvolpath) {
    ...
    max_pial_averages = 4;
    max_outward_dist  = 3;
    if (mm_contrast_type == CONTRAST_FLAIR)
      Ghisto_left_inside_peak_pct = 0.01; //0.01 for flair. typo?
    // Check dims
  }
  ...
  for (i = 0; n_averages >= n_min_averages;
    ...
    if (mmvol == NULL) {
      ...
      MRIScomputeBorderValues(surf, involCBV, NULL, inside_hi, border_hi,
                              border_low, outside_low, outside_hi,
                              current_sigma, 2 * max_cbv_dist, parms.fp,
                              surftype, NULL, 0, parms.flags, seg, -1, -1);
	...
	intensities). */
      ...
      if (!UseMMRefine) {
        MRIScomputePialTargetLocationsMultiModal(
            surf, mmvol, NULL, 0, mm_contrast_type, seg, T2_min_inside,
            T2_max_inside, T2_min_outside, T2_max_outside, max_outward_dist,
        ...
        MRISwrite(surftarget,
                  std::string(std::string(outsurfpath) + std::string(".target"))
                      .c_str());
        MRISfree(&surftarget);
      }
    }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    //else if(!strcmp(option, "--long")) longitudinal = 1; currently does nothing
      ...
      ripmngr.RipWMSA = 0;
    else if (!strcmp(option, "--rip-freeze"))
      ripmngr.RipFreeze = 1;
    else if (!strcmp(option, "--no-rip-freeze"))
      ripmngr.RipFreeze = 0;
    else if (!strcmp(option, "--rip-lesion"))
      ripmngr.RipLesion = 1;
    else if (!strcmp(option, "--no-rip-lesion"))
      ripmngr.RipLesion = 0;
      ...
      if (pinlabel == NULL)
        exit(1);
      nargsused = 1;
    } else if (!strcmp(option, "--no-pin-medial-wall"))
      pinlabel = NULL;
      ...
      surftype  = GRAY_CSF;
      nargsused = 2;
    } else if (!strcasecmp(option, "--cover-seg") ||
               !strcasecmp(option, "--cover_seg")) {
      if (nargc < 1)
        CMDargNErr(option, 1);
      ...
      nargsused = 1;
    } else if (!strcasecmp(option, "--adgws") ||
               !strcasecmp(option, "--adgws-in")) {
      if (nargc < 1)
        CMDargNErr(option, 1);
      adgwsinfile = pargv[0];
...
int MRISripSegs(MRIS *surf, MRI *seg, const int *SegNo, const int nSegNos,
  ...
  for (vno = 0; vno < surf->nvertices; vno++) {
    ...
    for (d = dmin; d <= dmax; d += dstep) {
      ...
      if (v->ripflag)
        break; // no need to continue along normal
    }          // dist
...
int RIP_MNGR::RipVertices(void) {
  ...
  if (ripsurffile) {
    ...
    if (ripsurfneeded) {
      printf("INFO: rip surface needed but not specified, so using input "
             "surface\n");
    }
    ripsurf = surf;
  }
