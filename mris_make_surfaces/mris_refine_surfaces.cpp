...
int main(int argc, char *argv[]) {
  ...
  MRI_SURFACE *mris;
  MRI *        mri_wm = NULL, *mri_smooth = NULL, *mri_hires, *mri_hires_pial,
      *mri_mask = NULL, *mri_hires_orig;
  /*  MRI           *mri_tran = NULL;*/
  float   max_len;
  ...
  *dot = 0;
  ...
  if (0) {
    ...
    mri_tmp = MRIextract(mri_hires, NULL, box.x, box.y, box.z, box.dx, box.dy,
                         box.dz);
  ...
  if (argc > 4) // label specified explicitly
  {
    ...
    for (i = 0; i < num_other_labels; i++) {
      LABEL *olabel;
      req = snprintf(fname, STRLEN, "%s/%s/label/%s", sdir, sname,
                     other_label_names[i]);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
  ...
  req = snprintf(parms.base_name, STRLEN, "%s%s%s", white_matter_name,
  ...
  req = snprintf(parms.base_name, STRLEN, "%s%s%s", pial_name, output_suffix,
                 suffix);
  if (req >= STRLEN) {
    std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
              << std::endl;
  ...
  for (j = 0; j <= 0; parms.l_intensity *= 2, j++) /* only once for now */
  {
    ...
    for (n_averages = max_pial_averages, i = 0; n_averages >= min_pial_averages;
      ...
      */
      ...
      if (mri_mask == NULL)
        mri_mask =
            make_pial_location_mask(mris, mri_hires, max_thickness, NULL);
      if (inverted_contrast)
