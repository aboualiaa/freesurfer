...
// whalf) ;
// compute_pial_target_locations() is now
// MRIScomputePialTargetLocationsMultiModal() in mrisurf_mri.cpp
...
int main(int argc, char *argv[]) {
  ...
  if (auto_detect_stats) {
    ...
    if (use_mode) {
      ...
      MRIScomputeClassModes(mrisAutoDet, mri_T1, &white_mode, &gray_mode, NULL,
                            NULL, NULL, NULL);
  ...
  for (i = 0; n_averages >= min_white_averages;
    ...
    if (mri_cover_seg) {
      ...
      if (i == 0) {
        ...
        mri_tmp = MRIdistanceTransform(mri_bin, NULL, 1, 20, DTRANS_MODE_SIGNED,
                                       NULL);
    ...
    for (n_averages = max_pial_averages, i = 0; n_averages >= min_pial_averages;
      ...
      if (flair_or_T2_name) {
        ...
        if (followGradients) {
          ...
          MRIScomputePialTargetLocationsMultiModal(
    ...
    for (n_averages = max_white_averages, i = 0;
      ...
      if (flair_or_T2_name == NULL) // otherwise already done
        ...
        MRIScomputeBorderValues(
            ...
            parms.fp, GRAY_WHITE, NULL, 0, parms.flags, mri_aseg, -1, -1);
      MRISfindExpansionRegions(mris);
      if (vavgs) {
        fprintf(stdout, "averaging target values for %d iterations...\n",
                vavgs);
...
static int get_option(int argc, char *argv[]) {
  ...
  } else if (!stricmp(option, "ghi")) // same flag name as mri_segment
  {
    ...
    MRIScomputeBorderValues(mris, mri, NULL, inside_hi, border_hi, border_low,
                            outside_low, outside_hi, sigma, 2 * max_thickness,
                            NULL, which, NULL, 0, 0, mriaseg, Gdiag_no,
                            Gdiag_no + 1);
    exit(1);
...
static int compute_white_target_locations(MRI_SURFACE *mris, MRI *mri_T2,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    hs = HISTOsmooth(h_inside, NULL, 2);
    HISTOfree(&h_inside);
    h_inside = hs;
    hs = HISTOsmooth(h_outside, NULL, 2);
    HISTOfree(&h_outside);
    h_outside = hs;
    hs = HISTOsmooth(h_grad, NULL, 2);
    HISTOfree(&h_grad);
    h_grad = hs;
