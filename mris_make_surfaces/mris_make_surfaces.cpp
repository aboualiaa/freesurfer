...
static int find_and_mark_pinched_regions(MRI_SURFACE *mris, MRI *mri_T2,
                                         float nstd_below, float nstd_above);
...
static double max_outward_dist =
    1; // for normal cases, for pathology might be much larger

// 0 disables use of max_gray in setting outside_hi in white surfacee
// deformation the larger this is, the closer outside_hi will be to max_gray.
// Set it high if the white surface is settling too far into the white matter
...
// static int  near_bright_structure(MRI *mri_aseg, int xi, int yi, int zi, int
// whalf) ;
static int compute_pial_target_locations(
    MRI_SURFACE *mris, MRI *mri_T2, LABEL **labels, int nlabels,
    int contrast_type, MRI *mri_aseg, double T2_min_inside,
    double T2_max_inside, double T2_min_outside, double T2_max_outside,
    double max_outward_dist, double left_inside_peak_pct,
    double right_inside_peak_pct, double left_outside_peak_pct,
    double right_outside_peak_pct, double wm_weight, MRI *mri_T1);
static int compute_white_target_locations(MRI_SURFACE *mris, MRI *mri_T2,
                                          MRI *mri_aseg, float nstd_below,
                                          float nstd_above, int contrast_type,
                                          float T2_min_inside,
                                          float T2_max_inside, int below_set,
                                          int above_set, double sigma,
                                          double max_out);
...
static double compute_brain_thresh(MRI_SURFACE *mris, MRI *mri_ratio,
                                   float nstd);
static int fix_midline(MRI_SURFACE *mris, MRI *mri_aseg, MRI *mri_brain,
...
static char *read_pinch_fname = nullptr;
...
static char *dura_echo_name = nullptr;
static char *T2_name = nullptr;
static char *flair_or_T2_name = nullptr;
static int nechos = 0;
...
static MRI *mri_cover_seg = nullptr;
...
static char *AutoDetSurfName = nullptr; // surf to use when autodet stats
...
char *ripfilename = nullptr, *whitesigmafilename = nullptr,
     *pialsigmafilename = nullptr;
...
int main(int argc, char *argv[]) {
  ...
  MRI *mri_wm, *mri_kernel = nullptr;
  MRI *mri_smooth = nullptr, *mri_mask = nullptr;
  MRI *mri_filled, *mri_T1, *mri_labeled, *mri_T1_white = nullptr, *mri_T1_pial;
  float max_len;
  ...
  if (xform_fname) {
    ...
    if (!m3d)
      ErrorExit(ERROR_NOFILE, "%s: could not open transform file %s\n",
                Progname, fname);
    sprintf(ventricle_fname, "%s/average/%s_ventricle.mgz#0@mgh", mdir,
            !stricmp(hemi, "lh") ? "left" : "right");
    ...
    mri_inv_lv = MRIapplyInverse3DMorph(mri_lv, m3d, nullptr);
    MRI3DmorphFree(&m3d);
    MRIfree(&mri_lv);
    fprintf(stdout, "filling in ventricle...\n");
    mri_lv = MRIfillVentricle(mri_inv_lv, mri_T1, 100,
                              DEFAULT_DESIRED_WHITE_MATTER_VALUE, nullptr);
    MRIfree(&mri_inv_lv);
  ...
  if (aseg_name) {
    ...
    if (mri_aseg == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read segmentation volume %s",
                Progname, fname);
    if (nowmsa) {
      MRIreplaceValuesOnly(mri_aseg, mri_aseg, Left_WM_hypointensities,
                           Left_Cerebral_White_Matter);
      MRIreplaceValuesOnly(mri_aseg, mri_aseg, Right_WM_hypointensities,
                           Right_Cerebral_White_Matter);
      MRIreplaceValuesOnly(mri_aseg, mri_aseg, WM_hypointensities,
                           Left_Cerebral_White_Matter);
    }
    if (erase_brainstem) {
    ...
    if (erase_cerebellum) {
      ...
      if (mri_T1_white && mri_T1_white != mri_T1) {
        MRImaskLabel(mri_T1_white, mri_T1_white, mri_aseg,
      ...
      if (mri_T1_pial && mri_T1_pial != mri_T1) {
        MRImaskLabel(mri_T1_pial, mri_T1_pial, mri_aseg,
                     Left_Cerebellum_White_Matter, 0);
        MRImaskLabel(mri_T1_pial, mri_T1_pial, mri_aseg, Left_Cerebellum_Cortex,
                     0);
        MRImaskLabel(mri_T1_pial, mri_T1_pial, mri_aseg,
                     Right_Cerebellum_White_Matter, 0);
        MRImaskLabel(mri_T1_pial, mri_T1_pial, mri_aseg,
      ...
      MRImaskLabel(mri_filled, mri_filled, mri_aseg,
                   Left_Cerebellum_White_Matter, 0);
      MRImaskLabel(mri_filled, mri_filled, mri_aseg, Left_Cerebellum_Cortex, 0);
      MRImaskLabel(mri_filled, mri_filled, mri_aseg,
                   Right_Cerebellum_White_Matter, 0);
      MRImaskLabel(mri_filled, mri_filled, mri_aseg, Right_Cerebellum_Cortex,
                   0);
    }
  } else {
    mri_aseg = nullptr;
  ...
  if (overlay) {
    ...
    MRImask(mri_T1, mri_wm, mri_T1, WM_EDITED_ON_VAL,
  ...
  if (orig_white)
    printf("  .. but with overwrite the positions with %s...\n", orig_white);
  ...
  if (AutoDetSurfName) {
    sprintf(fname, "%s/%s/surf/%s.%s%s", sdir, sname, hemi, AutoDetSurfName,
            suffix);
  ...
  MRISfaceMetric(
      mris, 0); // just computes f->norm, does not replace MRISmetricProperties
  MRISedges(mris);
  MRIScorners(mris);
  ...
  if (parms.l_hinge > 0 || parms.l_spring_nzr > 0) {
    if (parms.l_spring_nzr) {
      double *edgestats = MRISedgeStats(mris, 0, nullptr, nullptr);
  ...
  if (auto_detect_stats) {
    ...
    // Binarize wm.mgz by thresholding at WM_MIN_VAL. Voxels below threshold
    // will take a value of MRI_NOT_WHITE; those above will get MRI_WHITE.
    printf("Binarizing %s thresholding at %d\n", wm_name, WM_MIN_VAL);
    mri_tmp =
        MRIbinarize(mri_wm, nullptr, WM_MIN_VAL, MRI_NOT_WHITE, MRI_WHITE);
    ...
    if (use_mode) {
      ...
      // important! It will be orig_name by default but will become white_name
      // if white_name specified.
      MRIScomputeClassModes(mrisAutoDet, mri_T1, &white_mode, &gray_mode,
    ...
    if (min_gray_at_white_border < MAX_SCALE_DOWN * MIN_GRAY_AT_WHITE_BORDER)
      min_gray_at_white_border =
          nint(MAX_SCALE_DOWN * MIN_GRAY_AT_WHITE_BORDER);
    ...
    if (!max_gray_at_csf_border_set) {
      ...
      max_gray_at_csf_border =
          gray_mean - 1.0 * gray_std; // changed to push pial surfaces further
                                      // out BRF 12/10/2015
    }
  ...
  for (i = 0; n_averages >= min_white_averages;
       n_averages /= 2, current_sigma /= 2, i++) {
    if (nowhite)
      break; // skip if not placing the white surface

    printf("Iteration %d white =========================================\n", i);
    printf("n_averages=%d, current_sigma=%g\n", n_averages, current_sigma);
    fflush(stdout);

    // This does not look like it actually smooths anything. It creates the
    // kernel and frees it without ever applying it. mri_smooth is just a copy
    // of mri_T1. current sigma is used below
    printf("Smoothing T1 volume with sigma = %2.3f\n", current_sigma);
    fflush(stdout);
    parms.sigma = current_sigma;
    mri_kernel = MRIgaussian1d(current_sigma, 100);
    if (!mri_smooth)
      mri_smooth = MRIcopy(mri_T1, nullptr);
    MRIfree(&mri_kernel);
    ...
    if (mri_aseg) {
      ...
      if (Gdiag_no > 0) {
        ...
        printf("vno=%d  v->val=%g v->d=%g v->marked=%d, v->ripflag=%d\n",
               Gdiag_no, vgdiag->val, vgdiag->d, vgdiag->marked,
               vgdiag->ripflag);
      }
    ...
    if (mri_cover_seg) {
      ...
      if (i == 0) {
        mri_bin = MRIclone(mri_T1, nullptr);
        ...
        mri_tmp = MRIdistanceTransform(mri_bin, nullptr, 1, 20,
    ...
    else if (flair_or_T2_name == nullptr) { // otherwise already done
      ...
      double outside_hi = (max_border_white + max_gray_scale * max_gray) /
                          (max_gray_scale + 1.0);

      if (!FZERO(max_gray_scale))
        printf("setting outside hi = %2.1f (was %2.1f) in white surface "
               "deformation\n",
      ...
      printf("  MAX_WHITE  %d, max_border_white %g, min_border_white %g, "
             ...
             min_gray_at_white_border);
      printf("  outside_hi  %g, max_thickness %g, max_gray_scale %g,  max_gray "
             "%g\n",
      ...
      MRIScomputeBorderValues(
          ...
          2 * max_thickness, parms.fp, GRAY_WHITE, nullptr, 0, parms.flags,
          mri_aseg, -1, -1);
      ...
      if (mri_flair == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not load flair volume %s", Progname,
                  fname);

      if (orig_white) {
        if (i > 0)
          MRISsaveVertexPositions(mris,
                                  WHITE_VERTICES); // update estimate of white
      ...
      compute_white_target_locations(
          mris, mri_flair, mri_aseg, 3, 3, CONTRAST_FLAIR, T2_min_inside,
          T2_max_inside, below_set, above_set, white_sigma, max_outward_dist);
      if (Gdiag & DIAG_WRITE) {
        char fname[STRLEN];
        static int callno = 0;
        sprintf(fname, "%ch.dist.%3.3d.mgz",
                mris->hemisphere == LEFT_HEMISPHERE ? 'l' : 'r', callno);
        printf("writing distances to %s\n", fname);
        MRISwriteD(mris, fname);
        MRISsaveVertexPositions(mris, TMP2_VERTICES);
        MRISrestoreVertexPositions(mris, TARGET_VERTICES);
        sprintf(fname, "%ch.targets.%3.3d",
                mris->hemisphere == LEFT_HEMISPHERE ? 'l' : 'r', callno);
    ...
    */
    ...
    parms.l_tspring *= spring_scale;
    parms.l_tspring *= spring_scale;

    // parms.l_tspring = MIN(1.0,parms.l_tspring) ; // This had a bad effect on
    // highres and no effect on 1mm
    parms.l_nspring = MIN(1.0, parms.l_nspring);
    parms.l_spring = MIN(1.0, parms.l_spring);
    printf("Positioning White Surface: tspring = %g, nspring = %g, spring = "
           "%g, niters = %d ",
           parms.l_tspring, parms.l_nspring, parms.l_spring, parms.niterations);
    printf("l_repulse = %g, checktol = %d\n", parms.l_repulse, parms.check_tol);
    fflush(stdout);
    if (Gdiag_no > 0) {
      vgdiag = &mris->vertices[Gdiag_no];
      printf("vno=%d  v->val=%g v->d=%g v->marked=%d, v->ripflag=%d, "
             ...
             vgdiag->x, vgdiag->y, vgdiag->z, vgdiag->nx, vgdiag->ny,
             vgdiag->nz);
    }
  ...
  if (!nowhite) {
    ...
    if (ripfilename) {
      ...
      MRI *mritmp = MRIcopyMRIS(nullptr, mris, 0, "ripflag");
      MRIwrite(mritmp, ripfilename);
      MRIfree(&mritmp);
    }
    if (whitesigmafilename) {
      printf("Saving white sigma overlay to %s\n", whitesigmafilename);
      MRI *mritmp = MRIcopyMRIS(nullptr, mris, 0, "val2");
  ...
  if (nowhite) { /* read in previously generated white matter surface */
    if (orig_white) {
      ...
      MRISremoveIntersections(mris);
    } else // read default white (something needs to be read if nowhite was
           // created)
    {
  ...
  if (!nowhite && strcmp(white_matter_name, "NOWRITE")) {
    ...
    if (SaveResidual) {
      ...
      RmsValErrorRecord = 0;
      MRI *ValResid;
      ValResid = MRIcopyMRIS(nullptr, mristarget, 3, "d"); // target distance
      ...
      if (getenv("FS_POSIX")) {
        sprintf(fname, "./%s.%s.res%s%s.mgz", hemi, white_matter_name,
                output_suffix, suffix);
      } else {
        sprintf(fname, "%s/%s/surf/%s.%s.res%s%s.mgz", sdir, sname, hemi,
    ...
    if (SaveTarget) {
      if (getenv("FS_POSIX")) {
        sprintf(fname, "./%s.%s.target%s%s", hemi, white_matter_name,
                output_suffix, suffix);
      } else {
        sprintf(fname, "%s/%s/surf/%s.%s.target%s%s", sdir, sname, hemi,
                white_matter_name, output_suffix, suffix);
      }
      printf("writing white target surface to %s...\n", fname);
      MRISwrite(mristarget, fname);
    ...
    if (getenv("FS_POSIX")) {
      sprintf(fname, "./%s.%s%s%s", hemi, white_matter_name, output_suffix,
              suffix);
    } else {
    ...
    if (mri_aseg && label_cortex) {
      ...
      if (getenv("FS_POSIX")) {
        sprintf(fname, "./%s.%s%s%s.label", hemi, "cortex", output_suffix,
                suffix);
      } else {
        sprintf(fname, "%s/%s/label/%s.%s%s%s.label", sdir, sname, hemi,
                "cortex", output_suffix, suffix);
      }
  ...
  if (smooth && !nowhite) {
    printf("smoothing surface for %d iterations...\n", smooth);
    if (orig_pial)
      printf(" .. but this will be overwritten using vertex positions from "
             "%s...\n",
             orig_pial);
    MRISaverageVertexPositions(mris, smooth);
  }
  ...
  if (fill_interior) { // off by default
    ...
    if (fill_interior == 255 && mri_T1_pial->type == MRI_UCHAR) {
      ...
      printf("!!!!!!!! WARNING - 255 is a protected value for uchar volumes. "
             "Changing to 254... !!!!!!!!!!!!!!!!!!!!!!\n");
      fill_interior = 254;
    }
    mri_interior = MRIcopy(mri_T1_pial, nullptr);
...
#define MAX_VOLUMES 100
    ...
    mri_ratio = MRIdivide(mri_echos[0], mri_echos[nechos - 1], nullptr);
    if (dura_thresh < 0)
  ...
  MRISclearMark2s(mris); // will be used to keep track of which vertices found
                         // pial surface in previous cycle
  ...
  for (j = 0; j < 1; j++) /* only once for now */
  {
    ...
    for (n_averages = max_pial_averages, i = 0; n_averages >= min_pial_averages;
      ...
      if (flair_or_T2_name) {
        ...
        printf("flair_or_T2_name: repositioning pial surface locations using  "
               "%s\n",
               fname);
        if (mri_flair)
          MRIfree(&mri_flair);
        ...
        mri_flair = MRIread(fname);
        if (mri_flair == nullptr)
          ErrorExit(ERROR_NOFILE, "%s: could not load flair volume %s",
                    Progname, fname);
        ...
        if (mri_aseg && erase_cerebellum) { // off by default
          MRImaskLabel(mri_flair, mri_flair, mri_aseg,
                       Left_Cerebellum_White_Matter, 0);
          MRImaskLabel(mri_flair, mri_flair, mri_aseg, Left_Cerebellum_Cortex,
                       0);
          MRImaskLabel(mri_flair, mri_flair, mri_aseg,
                       Right_Cerebellum_White_Matter, 0);
          MRImaskLabel(mri_flair, mri_flair, mri_aseg, Right_Cerebellum_Cortex,
                       0);
        }
        ...
        if (read_pinch_fname) { // off by default
          ...
          find_and_mark_pinched_regions(mris, mri_flair, nsigma_below,
                                        nsigma_above);
        ...
        printf("Pre Pial Targ Loc VmPeak %d\n", memusage[1]);
        fflush(stdout);

        printf("CPTL: nlabels=%d, T2_min_inside=%g, T2_max_inside=%g, "
               "T2_min_outside=%g, T2_max_outside=%g, max_out_d=%g\n",
               nlabels, T2_min_inside, T2_max_inside, T2_min_outside,
               T2_max_outside, max_outward_dist);
        printf("CPTL: inside_peak_pct=%g,%g outside_peak_pct=%g,%g , "
               "wm_weight=%g\n",
               Ghisto_left_inside_peak_pct, Ghisto_right_inside_peak_pct,
               Ghisto_left_outside_peak_pct, Ghisto_right_outside_peak_pct,
               wm_weight);
        if (followGradients) {
          std::cout << "T2/FLAIR compute target based on gradients "
                    << std::endl;
          ...
          compute_pial_target_locations(
              mris, mri_flair, labels, nlabels, contrast_type, mri_aseg,
              T2_min_inside, T2_max_inside, T2_min_outside, T2_max_outside,
              max_outward_dist, Ghisto_left_inside_peak_pct,
              Ghisto_right_inside_peak_pct, Ghisto_left_outside_peak_pct,
              Ghisto_right_outside_peak_pct, wm_weight, mri_T1_pial);
        }
        //	parms.l_location /= spring_scale ;
        GetMemUsage(memusage);
        printf("Post Pial Targ Loc VmPeak %d\n", memusage[1]);
        fflush(stdout);
      ...
      // T2=========================================
      if (T2_name) {
        // Note: when T2 is specified, it is assigned to flair_or_T2_name, and
        // this branch is not activated. It does not appear that there is a way
        // to activate this branch. Note: l_location is set to 0.5 at the end
        // and l_intensity is turned off
        MRI *mri_T2 = nullptr;
        int n = 0;
        LABEL **labels;
        int nlabels;

        printf("removing non-brain from pial surface locations using T2 volume "
               "%s\n",
               T2_name);
        ...
        if (mri_T2 == nullptr)
          ErrorExit(ERROR_NOFILE, "%s: could not load T2 volume %s", Progname,
                    T2_name);
        ...
        if (read_pinch_fname) {
          ...
          if (mri_aseg && erase_cerebellum) {
            MRImaskLabel(mri_T2, mri_T2, mri_aseg, Left_Cerebellum_White_Matter,
                         0);
          ...
          find_and_mark_pinched_regions(mris, mri_T2, nsigma_below,
                                        nsigma_above);
          if (MRIScountMarked(mris, 1) > 0) {
            INTEGRATION_PARMS saved_parms;
            ...
            printf("%d vertices found in imminent self-intersecting regions, "
                   "deforming to remove...\n",
                   MRIScountMarked(mris, 1));
            MRISinvertMarks(mris);
            MRISerodeMarked(mris, 2);
        ...
        if (0) {
          std::cout << " nooooooooooooooooooooooooooooooooooooooooooooooooooo"
                    << std::endl;
          compute_pial_target_locations(
              mris, mri_T2, labels, nlabels, contrast_type, mri_aseg,
              T2_min_inside, T2_max_inside, T2_min_outside, T2_max_outside,
              max_outward_dist, Ghisto_left_inside_peak_pct,
              Ghisto_right_inside_peak_pct, Ghisto_left_outside_peak_pct,
              Ghisto_right_outside_peak_pct, wm_weight, mri_T1_pial);
      ...
      if (mri_cover_seg) { // off by default
        ...
        if (i == 0) {
          ...
          if (Gdiag & DIAG_WRITE) {
            sprintf(fname, "%s.cseg.dtrans%s%s.mgz", hemi, output_suffix,
                    suffix);
      ...
      else if (flair_or_T2_name == nullptr) {
      ...
      if (pial_blur_sigma > 0) { // off by default
        ...
        mri_tmp = MRIconvolveGaussian(mri_T1_pial, nullptr, mri_kernel);
        MRIfree(&mri_kernel);
        MRIfree(&mri_T1_pial);
      ...
      if (vavgs) { // 5 by default, should do nothing when T2/FLAIR
        ...
        if (Gdiag_no >= 0) {
          ...
          printf("v %d, target value = %2.1f, mag = %2.1f, dist=%2.2f, "
                 "ripflag=%d\n",
                 Gdiag_no, v->val, v->mean, v->d, v->ripflag);
        }
      }
      ...
      // First time increase smoothness to prevent pinching when pushing out
      // from white surface Generally has no effect with T2/FLAIR because
      // orig_pial usually != NULL
      if (j == 0 && i == 0 && orig_pial == nullptr) {
        // Problem: this stage is not run in recon-all because orig_pial is
        // specified, but orig_pial is actually white.preaparc, which is not a
        // real pial surface.
        INTEGRATION_PARMS saved_parms;
        int k, start_t;

        printf("perforing initial smooth deformation to move away from white "
               "surface\n");
        ...
        for (k = 0; k < 3; k++) {
          ...
          INTEGRATION_PARMS_copy(&parms, &old_parms);
          if (!FZERO(
                  parms.l_intensity)) { // l_intensity should be 0 for T2/FLAIR

            MRIScomputeBorderValues(
                mris, mri_T1, mri_smooth, max_gray, max_gray_at_csf_border,
                min_gray_at_csf_border, min_csf,
                (max_csf + max_gray_at_csf_border) / 2, current_sigma,
                2 * max_thickness, parms.fp, GRAY_CSF, mri_mask, thresh,
      ...
      if (parms.l_location > 0) { // 0 by default, but turned on with T2
        ...
        for (vno = 0; vno < mris->nvertices; vno++) {
          VERTEX *const v = &mris->vertices[vno];
          dist = sqrt(SQR(v->targx - v->x) + SQR(v->targy - v->y) +
                      SQR(v->targz - v->z));
  ...
  if (SaveResidual && flair_or_T2_name == nullptr) {
    RmsValErrorRecord = 1;
    mrisRmsValError(mris, mri_T1);
    RmsValErrorRecord = 0;
    MRI *ValResid;
    ...
    MRIcopyMRIS(ValResid, mris, 0, "val2bak"); // residual = sample-target
    if (getenv("FS_POSIX")) {
      sprintf(fname, "./%s.%s.res%s%s.mgz", hemi, pial_name, output_suffix,
              suffix);
    } else {
      sprintf(fname, "%s/%s/surf/%s.%s.res%s%s.mgz", sdir, sname, hemi,
  ...
  if (SaveTarget) {
    if (getenv("FS_POSIX")) {
      sprintf(fname, "./%s.%s.target%s%s", hemi, pial_name, output_suffix,
              suffix);
    } else {
      sprintf(fname, "%s/%s/surf/%s.%s.target%s%s", sdir, sname, hemi,
              pial_name, output_suffix, suffix);
  ...
  {
    sprintf(parms.base_name, "%s%s%s", white_matter_name, output_suffix,
            suffix);
    MRIScomputeMetricProperties(mris); /* recompute surface normals */
    MRISstoreMetricProperties(mris);
    MRISsaveVertexPositions(mris, TMP_VERTICES);
    fprintf(stdout, "repositioning cortical surface to gray/white boundary\n");
    ...
    for (n_averages = max_white_averages, i = 0;
      ...
      fprintf(stdout, "smoothing T1 volume with sigma = %2.3f\n",
              current_sigma);
      if (!mri_smooth) {
        mri_smooth = MRIclone(mri_T1, nullptr);
      }
      MRIfree(&mri_kernel);
      if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
        char fname[STRLEN];
      ...
      parms.n_averages = n_averages;
      MRISprintTessellationStats(mris, stdout);
      if (flair_or_T2_name == nullptr) // otherwise already done
                                       // within if(in_out_in_flag)
        MRIScomputeBorderValues(
            mris, mri_T1, mri_smooth, MAX_WHITE, max_border_white,
            min_border_white, min_gray_at_white_border,
            max_border_white /*max_gray*/, current_sigma, 2 * max_thickness,
            parms.fp, GRAY_WHITE, nullptr, 0, parms.flags, mri_aseg, -1, -1);
      MRISfindExpansionRegions(mris);
      if (vavgs) {
        fprintf(stdout, "averaging target values for %d iterations...\n",
                vavgs);
      ...
      if (Gdiag_no > 0) {
        vgdiag = &mris->vertices[Gdiag_no];
        printf(
            "vno=%d  v->val=%g v->d=%g v->marked=%d, v->ripflag=%d(%g,%g,%g)\n",
            Gdiag_no, vgdiag->val, vgdiag->d, vgdiag->marked, vgdiag->ripflag,
            vgdiag->x, vgdiag->y, vgdiag->z);
      }
      MRISpositionSurface(mris, mri_T1, mri_smooth, &parms);
      if (Gdiag_no > 0) {
        vgdiag = &mris->vertices[Gdiag_no];
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    print_version();
             ...
             !stricmp(option, "-erasecerebellum") ||
             !stricmp(option, "erase_cerebellum")) {
    erase_cerebellum = 1;
    printf("erasing cerebellum in aseg before deformation\n");
  } else if (!stricmp(option, "nobrainstem") ||
             !stricmp(option, "-erasebrainstem") ||
             !stricmp(option, "erase_brainstem")) {
    erase_brainstem = 1;
    ...
    fprintf(stderr,
            "using peak pct %2.2f,%2.2f and %2.2f,%2.2f (default=%2.2f) to "
            "find bounds of GM range\n",
            Ghisto_left_inside_peak_pct, Ghisto_right_inside_peak_pct,
            Ghisto_left_outside_peak_pct, Ghisto_right_outside_peak_pct,
            MIN_PEAK_PCT);
    nargs = 4;
    ...
    fprintf(stderr,
            "setting max_gray_scale (mgs) = %2.1f in calculation of outside_hi "
            "= (max_border_white + mgs*max_gray) / (mgs+1.0)\n",
            max_gray_scale);
    nargs = 1;
    ...
    fprintf(
        stderr,
        "using min T2 gray matter (interior and exterior) threshold %2.1f\n",
        T2_min_inside);
    nargs = 1;
    ...
    fprintf(stderr,
            ...
            nsigma);
    nargs = 1;
  } else if (!stricmp(option, "dura_thresh") ||
             !stricmp(option, "dura+thresh")) {
    dura_thresh = atof(argv[2]);
    fprintf(stderr,
            "setting dura threshold to %2.2f instead of estimating it\n",
            dura_thresh);
    nargs = 1;
    ...
    nechos = atoi(argv[3]);
    fprintf(stderr, "detecting dura using %d echos from %s\n", nechos,
            dura_echo_name);
    nargs = 2;
    ...
    fprintf(stderr, "refining pial surfaces placement using T2 volume %s\n",
            flair_or_T2_name);
    parms.check_tol = 1;
    nargs = 1;
    ...
    parms.check_tol = 1;
    flair_or_T2_name = argv[2];
    fprintf(stderr, "deforming surfaces based on FLAIR volume %s\n",
            flair_or_T2_name);
    nargs = 1;
    if (T2_max_inside_set == 0)
      T2_max_inside = 300;
    ...
    if ((label_cortex != 0 && label_cortex != 1) || isalpha(argv[2][0]) ||
        argv[2][0] == '-') {
      printf("ERROR: -cortex flag takes one argument that must be 0 or 1; you "
             "passed %s\n",
             argv[2]);
      exit(1);
    }
    ...
    if (white_num >= MAX_VERTICES) {
      ErrorExit(ERROR_NOMEMORY, "%s: too many white vertex vals specified",
                Progname);
    }
    ...
    if (pial_num >= MAX_VERTICES) {
      ErrorExit(ERROR_NOMEMORY, "%s: too many pial vertex vals specified",
                Progname);
    }
  ...
  } else if (!stricmp(option, "ghi")) // same flag name as mri_segment
  {
    ...
    nargs = 1;
    fprintf(stderr,
            "smoothing pial surface for %d iterations before deforming\n",
            smooth_pial);
    ...
    fprintf(stderr,
            ...
            nwhite);
  } else if (!stricmp(option, "flair_white") ||
             !stricmp(option, "flairwhite")) {
    contrast_type = CONTRAST_FLAIR;
    flairwhite = 1;
    nbrs = 3;
    parms.l_location = .5;
    ...
    nargs = 1;
    fprintf(stderr, "setting tol = %2.1f%% change in sse/rms\n", parms.tol);
    ...
    // 1=vno 2=surf 3=mri 4=aseg 5=which 6=insidehi 7=blow 8=bhi 9=olow 10=ohi
    // 11=sigma
    ...
    MRIScomputeBorderValues(mris, mri, nullptr, inside_hi, border_hi,
                            border_low, outside_low, outside_hi, sigma,
                            2 * max_thickness, nullptr, which, nullptr, 0, 0,
                            mriaseg, Gdiag_no, Gdiag_no + 1);
    exit(1);
...
static MRI *MRIsmoothMasking(MRI *mri_src, MRI *mri_mask, MRI *mri_dst,
                             int mask_val, int wsize) {
  int width, height, depth, x, y, z, xi, yi, zi, xk, yk, zk, whalf, nvox, mean,
      avg;
  BUFTYPE *psrc, *pdst;
...
MRI *DeletemeMRIfindBrightNonWM(MRI *mri_T1, MRI *mri_wm) {
...
static int fix_midline(MRI_SURFACE *mris, MRI *mri_aseg, MRI *mri_brain,
  ...
  printf("Entering: fix_midline(): inhibiting deformation at non-cortical "
         "midline structures...\n");
...
static double compute_brain_thresh(MRI_SURFACE *mris, MRI *mri_ratio,
  ...
  mri_filled = MRIclone(mri_ratio, nullptr);
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
...
static int compute_pial_target_locations(
  ...
  VERTEX *v;
  double min_gray_inside, min_gray_outside, max_gray_outside, max_gray_inside,
      thickness, nx, ny, nz, sample_dist, pix_size;
  ...
  printf("inside_peak_pct = %g, %g, outside_peak_pct = %g, %g\n",
         left_inside_peak_pct, right_inside_peak_pct, left_outside_peak_pct,
         right_outside_peak_pct);
  ...
  if (mri_aseg) {
    ...
    int x, y, z, label;
    double T1, T2;
    mri_aseg = MRIcopy(mri_aseg,
                       nullptr); // so it can be modified (does this get freed?)
    ...
    MRIdistanceTransform(mri_aseg, mri_dist_lh, Left_Cerebral_Cortex, 100,
                         DTRANS_MODE_SIGNED, nullptr);
    MRIdistanceTransform(mri_aseg, mri_dist_rh, Right_Cerebral_Cortex, 100,
                         DTRANS_MODE_SIGNED, nullptr);
  }
  ...
  fflush(stdout);
  mri_tmp = MRISfillInterior(mris, mri_T2->xsize / 2, nullptr);
  mri_filled = MRIextractRegionAndPad(mri_tmp, nullptr, nullptr,
                                      nint(30 / mri_T2->xsize));
  MRIfree(&mri_tmp);
  ...
  MRIScomputeMetricProperties(mris);
  mri_tmp = MRISfillInterior(mris, mri_T2->xsize / 2, nullptr);
  mri_filled_pial = MRIextractRegionAndPad(mri_tmp, nullptr, nullptr,
                                           nint(30 / mri_T2->xsize));
  MRIfree(&mri_tmp);
  mri_dist_pial = MRIcloneDifferentType(mri_filled_pial, MRI_FLOAT);
  MRIdistanceTransform(mri_filled_pial, mri_dist_pial, 1, 100,
                       DTRANS_MODE_SIGNED, nullptr);
  ...
  printf("locating cortical regions not in interior range [%2.1f --> %2.1f], "
         "and not in exterior range [%2.1f --> %2.1f]\n",
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRISvertexToVoxel(mris, v, mri_T2, &xv, &yv,
                      &zv); // Get volume crs corresponding to vertex v->xyz
    // Build a region around it
    region.x = nint(xv) - whalf;
    region.y = nint(yv) - whalf;
    region.z = nint(zv) - whalf;
    region.dx = wsize;
    region.dy = wsize;
    region.dz = wsize;
    // Build hist of voxels in the region that are also in cortex
    h1 = MRIhistogramLabelRegion(mri_T2, mri_aseg, &region,
                                 Left_Cerebral_Cortex, 0);
    if (h1->nbins == 1)
      DiagBreak();
    h2 = MRIhistogramLabelRegion(mri_T2, mri_aseg, &region,
                                 Right_Cerebral_Cortex, 0);
    if (h2->nbins == 1)
      DiagBreak();
    HISTOadd(h1, h2, h1);
    hs = HISTOsmooth(h1, nullptr, 4);
    hcdf_rev = HISTOmakeReverseCDF(h1, nullptr);
    hcdf = HISTOmakeCDF(h1, nullptr);
    ...
    hwm = MRIhistogramLabelRegion(mri_T2, mri_aseg, &region,
                                  Left_Cerebral_White_Matter, 0);
    hwm2 = MRIhistogramLabelRegion(mri_T2, mri_aseg, &region,
                                   Right_Cerebral_White_Matter, 0);
    HISTOadd(hwm, hwm2, hwm);
    hwms = HISTOsmooth(hwm, nullptr, 4);
    HISTOrobustGaussianFit(hwms, .2, &mean_wm, &sigma_wm);

    MRISvertexToVoxel(mris, v, mri_aseg, &xv, &yv, &zv);
    near_cerebellum = (MRIcountValInNbhd(mri_aseg, 7, xv, yv, zv,
                                         Left_Cerebellum_Cortex) > 0);
    near_cerebellum =
        near_cerebellum || (MRIcountValInNbhd(mri_aseg, 7, xv, yv, zv,
                                              Right_Cerebellum_Cortex) > 0);

    // one of the primary uses of the T2 deformation is to find the thin line of
    // dark (flair)/bright (T2) voxels that mark the boundary of the cortex and
    // the cerebellum. These get partial volumed and setting a global threshold
    // causes the surfaces to settle too far in over much of the brain. instead
    // in regions that are close to cerebellum, make the thresholds less
    // conservative DNG: this appears to only apply to FLAIR
    if (contrast_type == CONTRAST_FLAIR) {
      int bin, peak;
      double thresh;

      peak = HISTOfindHighestPeakInRegion(
          hs, 0, hs->nbins - 1); // most likely GM value

      /* need to worry about dark intensities in the interior of the ribbon in
         FLAIR, so find the first dark value (leftwards from the peak) that is
         unlikely to be GM, and don't allow it to be in the interior of the
         ribbon.   */
      ...
      if (bin >= 0) {
        ...
        min_gray_inside =
            MAX(min_gray_inside,
                T2_min_inside + near_cerebellum * CEREBELLUM_OFFSET);
        if (vno == Gdiag_no)
      ...
      /* Now do the same thing for exterior values, using a different threshold.
         That is, look for values outside the current ribbon that are likely to
         be GM. This threshold is typically larger than the inside
         one since we trust the T1 more than the T2 - only deform if stuff
         outside really looks like GM */
      ...
      if (bin >= 0) {
        ...
        min_gray_outside =
            MAX(min_gray_outside,
                T2_min_outside + near_cerebellum * CEREBELLUM_OFFSET);
        if (vno == Gdiag_no)
      ...
      // thresh *= 10 ;  // for T2 there shouldn't be any dark stuff - it is
      // dura
      thresh = hs->counts[peak] * left_inside_peak_pct;
      for (bin = peak - 1; bin >= 0; bin--)
        if (hs->counts[bin] < thresh)
    ...
    // T2_min_inside = T2_min_outside = mean-nsigma_below*sigma ; T2_max_inside
    // = T2_max_outside = mean + nsigma_above*sigma ;
    if (vno == Gdiag_no) {
      printf("T2 range %2.1f --> %2.1f, %2.1f +- %2.1f\n", hcdf->bins[bin1],
             hcdf_rev->bins[bin2], mean, sigma);
    ...
    xs = v->whitex + d * nx;
    ys = v->whitey + d * ny;
    zs = v->whitez + d * nz; // WHITE XYZ
    outside_of_white = 0;
    for (; d <= thickness; d += sample_dist) {
      // project a distance of d from the WHITE surface toward the pial along
      // normal
      xs = v->whitex + d * nx;
      ys = v->whitey + d * ny;
      zs = v->whitez + d * nz; // WHITE XYZ
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_dist_white, xs, ys, zs, &xvf, &yvf,
                                  &zvf);
      ...
      if (val < min_gray_inside && (val > nint(min_gray_outside * .75)) &&
          !outside_of_white)
        continue; // all dark intensities - white boundary probably in wrong
                  // place
      ...
      if (mri_aseg != nullptr) {
        ...
        // the other hemi.
        MRISsurfaceRASToVoxelCached(mris, mri_aseg, xs, ys, zs, &xvf, &yvf,
                                    &zvf);
        xv = nint(xvf);
        yv = nint(yvf);
        zv = nint(zvf);
        ...
        if ((mris->hemisphere == LEFT_HEMISPHERE && IS_RH_CLASS(label)) ||
          ...
          if (vno == Gdiag_no)
            printf("v %d: terminating search at distance %2.2f due to presence "
                   "of contra tissue (%s)\n",
...
#if 1
      // for T2 images intensity should increase monotonically. If it starts to
      // go down we are probably at borders of skull stripping or into dura.
      // Checking val<mn-2*sigma
      // just reduces the chances of false positives and makes it more likely we
      // are really transitioning from brain to dura
      if (contrast_type == CONTRAST_T2 && dist_to_white > mri_aseg->xsize &&
          val < previous_val && val < mean - 2 * sigma) {
        if (vno == Gdiag_no)
          printf("illegal intensity decrease %2.1f->%2.1f found at d=%2.2f, "
                 "vox=(%2.1f, %2.1f, %2.1f)\n",
      ...
      if (contrast_type == CONTRAST_T2 && dist_to_white > mri_aseg->xsize &&
        ...
        dout = d + 1;
        xs1 = v->whitex + dout * nx;
        ys1 = v->whitey + dout * ny;
        zs1 = v->whitez + dout * nz;
        MRISsurfaceRASToVoxelCached(mris, mri_T2, xs1, ys1, zs1, &xv1, &yv1,
                                    &zv1);
        MRIsampleVolumeType(mri_T2, xv1, yv1, zv1, &next_val, SAMPLE_TRILINEAR);
        if (next_val < min_gray_inside) {
          if (vno == Gdiag_no)
            printf("v %d: prev %2.1f, current %2.1f>%2.1f, next %2.1f<%2.1f, "
                   "illegal\n",
                   vno, previous_val, val, mean + 2 * sigma, next_val,
                   min_gray_inside);
          found_bad_intensity = 1;
          break; // from loop over the ribbon
        }
      ...
      if (val < min_gray_inside || val > max_gray_inside) {
        if (vno == Gdiag_no)
          printf("illegal intensity %2.1f found at d=%2.2f, vox=(%2.1f, %2.1f, "
                 "%2.1f)\n",
                 val, d, xv, yv, zv);
        found_bad_intensity = 1;
        break; // from loop over the ribbon
    ...
    if (found_bad_intensity) {
      num_in++;
      // Set the target point so that interior is good value and exterior is bad
      // gm value DNG: not sure what is happening here
      v->targx = xs - (sample_dist / 2 * nx);
      v->targy = ys - (sample_dist / 2 * ny);
      v->targz = zs - (sample_dist / 2 * nz);
      MRISsurfaceRASToVoxelCached(mris, mri_T2, v->targx, v->targy, v->targz,
                                  &xv, &yv, &zv);
      MRIsampleVolumeType(mri_T2, xv, yv, zv, &val, SAMPLE_TRILINEAR);
      v->val = val;
      v->marked = 1;
      v->marked2++; // marked2 will keep track of how many times pial surface
                    // was found
      v->val2 = pial_sigma;
      v->d = (d + sample_dist / 2) - thickness;
      if (vno == Gdiag_no) {
        printf("vno %d: resetting target location to be d=%2.2f, "
               "(%2.1f %2.1f %2.1f), val @ (%2.1f, %2.1f, %2.1f) = %2.0f\n",
               vno, d - thickness, v->targx, v->targy, v->targz, xv, yv, zv,
               val);
        DiagBreak();
      }
    } else {
      ...
      found = 1;
      last_white = 0;
      MRISsurfaceRASToVoxelCached(mris, mri_dist_pial, v->x, v->y, v->z, &xvf,
                                  &yvf, &zvf);
      ...
      for (d = 0; d <= max_outward_dist; d += sample_dist) {
        // [xyz]s are in surface coords, shared by all vols. [xyz]v* are in
        // specific volume voxel coords
        xs = v->x + d * v->nx;
        ys = v->y + d * v->ny;
        zs = v->z + d * v->nz;
        MRISsurfaceRASToVoxelCached(mris, mri_dist_pial, xs, ys, zs, &xp, &yp,
                                    &zp); // used?
        MRISsurfaceRASToVoxelCached(mris, mri_dist_white, xs, ys, zs, &xvf,
                                    &yvf, &zvf);
        MRIsampleVolume(mri_dist_white, xvf, yvf, zvf, &dist_to_white);

        if (MRIindexNotInVolume(mri_dist_white, nint(xvf), nint(yvf),
                                nint(zvf)) ||
            dist_to_white > 0)
          outside_of_white = 1;
        else if (!outside_of_white) // haven't gotten out of the wm yet - ignore
                                    // intensities
        {
          last_white = d;
          continue;
        ...
        MRISsurfaceRASToVoxelCached(mris, mri_dist_pial, xs, ys, zs, &xvf, &yvf,
                                    &zvf);
        MRIsampleVolume(mri_dist_pial, xvf, yvf, zvf, &dist_to_pial);

        if (dist_to_pial - last_dist_to_pial <
            -sample_dist) // oblique to pial surface - tried to step outwards
                          // but distance decreased
        {
          DiagBreak();
          if (vno == Gdiag_no)
            printf("v %d: terminating search at distance %2.1f (%2.1f, %2.1f, "
                   "%2.1f) due to pial dist decrease\n",
                   vno, d, xs, ys, zs);
          if (val > min_gray_outside && val < max_gray_outside) // still in GM
          {
            found = 0; // hmmm, I guess don't trust FLAIR over T1 here. Assume
        ...
        if (dist_to_pial > mri_dist_pial->xsize) // outside of pial surface
          outside_of_pial = 1;
        else if (outside_of_pial &&
                 (dist_to_pial < mri_dist_pial->xsize ||
                  dist_to_pial < last_dist_to_pial)) // was outside of pial and
                                                     // reentered it
        {
          if (vno == Gdiag_no)
            printf("v %d: terminating searrch at distance %2.1f (%2.1f, %2.1f, "
                   "%2.1f) due to pial reentry\n",
                   vno, d, xs, ys, zs);
          d = 0;
          found = 0;
          break;
        ...
        if (outside_of_white &&
            (dist_to_white <=
             0)) // interior of wm surface, probably normals are messed up
        {
          if (d - last_white > .5) // really out of white and not just grazing a
                                   // corner of the surface
          {
            if (vno == Gdiag_no)
              printf("v %d: terminating search at distance %2.1f (%2.1f, "
                     "%2.1f, %2.1f) due to wm reentry\n",
                     vno, d, xs, ys, zs);
            d = 0;
            found = 0;
            break;
        ...
        if (mri_aseg != nullptr) {
          ...
          label = MRIgetVoxVal(mri_aseg, xv, yv, zv, 0);
          if (vno == Gdiag_no)
            printf("v %d: label distance %2.2f = %s @ (%d %d %d), dist to "
                   "white, pial: %2.1f, %2.1f, val = %2.1f\n",
                   vno, d, cma_label_to_name(label), xv, yv, zv, dist_to_white,
                   dist_to_pial, val);
          if ((mris->hemisphere == LEFT_HEMISPHERE && IS_RH_CLASS(label)) ||
              (mris->hemisphere == RIGHT_HEMISPHERE && IS_LH_CLASS(label))) {
            if (vno == Gdiag_no)
          ...
          if (IS_UNKNOWN(label)) {
                ...
                (hemi_dist + SAMPLE_DIST)) // keep them from crossing each other
            {
              if (vno == Gdiag_no)
                printf("v %d: terminating search at distance %2.2f due to "
                       "presence of contra hemi %2.1fmm dist <= hemi dist "
                       "%2.1fmm\n",
          ...
          if (IS_CEREBELLAR_GM(label)) {
            ...
            if (vno == Gdiag_no)
              printf("v %d: terminating search at distance %2.2f due to "
                     "presence of cerebellar gm (%s))\n",
                     vno, d, cma_label_to_name(label));
            break;
          }
        }

        if ((val < min_gray_inside && dist_to_white > 1.2) ||
            (val > max_gray_inside))
          break;
      } // end loop over distance
      ...
      if (!found || d > max_outward_dist) // couldn't find pial surface
      {
        ...
        // check just outside current  pial surface, and nudge outwards if it
        // looks a lot like gm
        found = 1;
        xs = v->x + NUDGE_DIST * v->nx;
        ys = v->y + NUDGE_DIST * v->ny;
        zs = v->z + NUDGE_DIST * v->nz;
        MRISsurfaceRASToVoxelCached(mris, mri_aseg, xs, ys, zs, &xvf, &yvf,
                                    &zvf);
        xv = nint(xvf);
        yv = nint(yvf);
        ...
        if (found)
          d = NUDGE_DIST +
              sample_dist; // will be decremented later as here it is the dist
                           // to the outside, not the border

        if (vno == Gdiag_no) {
          if (found)
            printf("v %d:  nudging pial surface 0.5mm outwards despite not "
                   "detecting exterior\n",
                   vno);
          else
      ...
      {
        d = v->d = 0;
        //	v->targx = v->x+d*v->nx ; v->targy = v->y+d*v->ny ; v->targz =
        // v->z+d*v->nz ;
      ...
      if (Gdiag) // debugging
      {
        MRISsurfaceRASToVoxelCached(mris, mri_dist_pial, v->targx, v->targy,
                                    v->targz, &xvf, &yvf, &zvf);
        MRIsampleVolume(mri_dist_pial, xvf, yvf, zvf, &dist_to_pial);
        if (dist_to_pial < -1.5)
          DiagBreak();
        MRISsurfaceRASToVoxelCached(mris, mri_dist_white, v->targx, v->targy,
                                    v->targz, &xvf, &yvf, &zvf);
        MRIsampleVolume(mri_dist_white, xvf, yvf, zvf, &dist_to_white);
        if (dist_to_white < .5)
          DiagBreak();
  ...
  if (Gdiag & DIAG_WRITE) {
    char fname[STRLEN];
    sprintf(fname, "%ch.dist.mgz",
            mris->hemisphere == LEFT_HEMISPHERE ? 'l' : 'r');
    printf("writing distances to %s\n", fname);
    MRISwriteD(mris, fname);
    MRISsaveVertexPositions(mris, TMP2_VERTICES);
    MRISrestoreVertexPositions(mris, TARGET_VERTICES);
    sprintf(fname, "%ch.targets",
            mris->hemisphere == LEFT_HEMISPHERE ? 'l' : 'r');
    printf("writing target locations to %s\n", fname);
    MRISwrite(mris, fname);
    MRISrestoreVertexPositions(mris, TMP2_VERTICES);
...
static int compute_white_target_locations(MRI_SURFACE *mris, MRI *mri_T2,
                                          ...
                                          float T2_min_inside,
                                          float T2_max_inside, int below_set,
                                          int above_set, double wsigma,
                                          double max_out) {
  int vno, vnum, outer_nbhd_size = 11, inner_nbhd_size = 3, n;
  int num_in, num_out;
  VERTEX *v, *vn;
  double min_grad, max_grad, min_inside, max_inside, min_outside, max_outside,
      sample_dist;
  ...
  if (!vlist || !hops)
    ErrorExit(ERROR_NOFILE,
              "compute_white_target_locations: could not allocate vlist");
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRIsampleVolumeDerivativeScale(mri_T2, xv, yv, zv, nx, ny, nz, &grad_val,
                                   2);
    xs = v->x - d * v->nx;
    ys = v->y - d * v->ny;
    zs = v->z - d * v->nz;
  ...
  MRISclearMarks(
      mris); // used by the old implementation of MRISfindNeighborsAtVertex
  MRISclearMark2s(mris);

  for (vno = 0; vno < mris->nvertices; vno++) {
    double dist_from_current_white, pin, pout, best_dist, best_dist_prelim;
    double pin0, pout0, mean_gm, mean_wm, sigma_wm, sigma_gm, grad_outside,
        grad_inside;
    int found;
    ...
    d = 2 * sample_dist;
    vnum = MRISfindNeighborsAtVertex(mris, vno, outer_nbhd_size, vlistCapacity,
                                     vlist, hops);
    for (n = 0; n < vnum; n++) {
      ...
      // sample further into interior of WM to avoid biasing distribution by
      ...
      // sample inside and outside and make sure we are at a local max of the
      // gradient. If not, don't trust
      xs = vn->x - 2 * d * vn->nx;
      ys = vn->y - 2 * d * vn->ny;
      zs = vn->z - 2 * d * vn->nz;
      MRISsurfaceRASToVoxelCached(mris, mri_T2, xs, ys, zs, &xv, &yv, &zv);
      MRIsampleVolumeDerivativeScale(mri_T2, xv, yv, zv, nx, ny, nz,
                                     &grad_inside, grad_scale);
      xs = vn->x + 2 * d * vn->nx;
      ys = vn->y + 2 * d * vn->ny;
      zs = vn->z + 2 * d * vn->nz;
      MRISsurfaceRASToVoxelCached(mris, mri_T2, xs, ys, zs, &xv, &yv, &zv);
      MRIsampleVolumeDerivativeScale(mri_T2, xv, yv, zv, nx, ny, nz,
                                     &grad_outside, grad_scale);
      if ((grad_val > 1) &&
          (grad_val > grad_inside &&
           grad_val > grad_outside)) // only regions we are confident of
      {
        double dist;
    ...
    HISTOfillHoles(h_grad);
    HISTOfillHoles(h_inside);
    HISTOfillHoles(h_outside);
    hs = HISTOsmooth(h_inside, nullptr, 2);
    HISTOfree(&h_inside);
    h_inside = hs;
    hs = HISTOsmooth(h_outside, nullptr, 2);
    HISTOfree(&h_outside);
    h_outside = hs;
    hs = HISTOsmooth(h_grad, nullptr, 2);
    HISTOfree(&h_grad);
    h_grad = hs;
    HISTOmakePDF(h_grad, h_grad);
    HISTOmakePDF(h_inside, h_inside);
    HISTOmakePDF(h_outside, h_outside);
...
#define MIN_GM_DIST_FROM_WM_PEAK 15
    ...
    best_dist = 0;
    max_grad = 0;
    for (dist_from_current_white = -max_out; dist_from_current_white <= max_out;
         dist_from_current_white += sample_dist) {
      ...
      MRIsampleVolumeDerivativeScale(mri_T2, xv, yv, zv, nx, ny, nz, &grad_val,
                                     grad_scale);
      if (grad_val < 0)
        continue; // don't consider locations with gradient pointing the wrong
                  // way
      ...
      if (vno == Gdiag_no) {
        MRISsurfaceRASToVoxelCached(mris, mri_T2, xs0, ys0, zs0, &xv, &yv, &zv);
        printf("vno %d: dist %2.2f, vox=(%2.1f %2.1f %2.1f),  in=%2.1f (%2.3f, "
               "WM/GM=%2.1f/%2.1f), out=%2.1f (%2.3f, WM/GM=%2.1f/%2.1f), "
               "grad=%2.1f\n",
               vno, dist_from_current_white, xv, yv, zv, inside_val, pin,
               wm_dist_in, gm_dist_in, outside_val, pout, wm_dist_out,
               gm_dist_out, grad_val);
      }

      if (dist_from_current_white > 0 &&
          (wm_dist_in > 6 || gm_dist_in < wm_dist_in)) {
        if (vno == Gdiag_no)
          printf("terminating search due to unlikely WM found (interior "
                 "wm_dist=%2.1f, gm_dist=%2.1f)\n",
                 wm_dist_in, gm_dist_in);
        break;
      }
      wm_dist_out = abs(mean_wm - outside_val) /
                    sigma_wm; // now compute dist to exterior val
      gm_dist_out = abs(mean_gm - outside_val) /
                    sigma_gm; // now compute dist to exterior val
      if (dist_from_current_white < 0 && gm_dist_out > 4 &&
          !FZERO(best_dist)) // found illegal interior val
      {
        if (vno == Gdiag_no)
          printf("resetting search due to non-allowable gm val %2.1f "
                 "(dist=%2.1f)\n",
                 outside_val, gm_dist_out);
        max_grad = 0;
        best_dist = 0;
      }
      if (grad_val > max_grad) // this is the largest gradient found yet, so if
                               // vals are reasonable
      {
        if ((wm_dist_in < 1 &&
             gm_dist_out < 1) || // either it really looks correct
            ((wm_dist_in < 2 &&
              gm_dist_out < 2) && // or it  almost looks correct and is better
                                  // than current
             (pin0 * pout0 < pin * pout)) ||
            ((wm_dist_in < gm_dist_in) &&
             (wm_dist_out >
              gm_dist_out)) // inside more likely wm and out more likely gm
        ) {
          found = 1;
          max_grad = grad_val;
    ...
    if (vno == Gdiag_no) {
      printf("vno %d: found %d, prelim best distance %2.2f, searching for "
             "local grad max\n",
             vno, found, best_dist_prelim);
      DiagBreak();
    }
    // tweak location to be at peak of gradient mag
    for (dist_from_current_white = best_dist_prelim - 2 * sample_dist;
         dist_from_current_white <= best_dist_prelim + 2 * sample_dist;
         dist_from_current_white += sample_dist) {
      double xs0, ys0, zs0;
      ...
      if (grad_val > max_grad) {
        if (vno == Gdiag_no) {
          printf("vno %d: updating max grad dist to %2.2f, vox=(%2.1f %2.1f "
                 "%2.1f),  grad=%2.1f\n",
    ...
    v->targx = v->x + best_dist * v->nx;
    v->targy = v->y + best_dist * v->ny;
    v->targz = v->z + best_dist * v->nz;
    v->d = best_dist;
  ...
  printf("%d surface locations found to contain inconsistent values (%d in, %d "
         "out)\n",
         num_in + num_out, num_in, num_out);
...
static int find_and_mark_pinched_regions(MRI_SURFACE *mris, MRI *mri_T2,
  ...
  do {
    ...
    if (bin < ceil(PERCENTILE * HISTO_NBINS) /
                  4) // data range is too compressed for histogram to represent
    {
      done = (niter > 10);
      if (niter++ > 0) {
        mx /= 2;
  ...
  MRISsaveVertexPositions(mris, TMP2_VERTICES);
  MRISrestoreVertexPositions(mris, WHITE_VERTICES);
  mri_filled = MRIclone(mri_T2, nullptr);
  ...
  for (num_in = num_out = vno = 0; vno < mris->nvertices; vno++) {
    ...
    found_bad_intensity = 0;
    for (d = thickness / 2; d <= thickness; d += sample_dist) {
      if (MRIgetVoxVal(mri_filled, nint(xv), nint(yv), nint(zv), 0) >
          0) // interior of white surface
      {
        continue;
      }
      xs = v->whitex + d * nx;
    ...
    } else // no invalid intensities found in the interior, check for valid ones
           // in the exterior?
    {
#define MAX_OUTWARD_DIST2 10
      for (d = 0; d <= MAX_OUTWARD_DIST2; d += sample_dist) {
        ...
        if (val < mn - sig ||
            val > mn + sig) // only look for a very narrow range of intensities
        {
          break;
        }
      }
          ...
          for (xk = -whalf; xk <= whalf; xk++) {
            ...
            for (yk = -whalf; yk <= whalf; yk++) {
              ...
              for (zk = -whalf; zk <= whalf; zk++) {
                ...
                MRIsampleVolumeType(mri_T2, xi, yi, zi, &val, SAMPLE_TRILINEAR);
                if (val < 0) {
                  continue;
                }
                if (val <
                    mn - .5 * sig) // found something that could be csf in FLAIR
                {
                  found = 1;
                  break;
                }
...
#define Mu (1.0) / ((K_bp)-1.0 / Lambda)
  ...
  for (num_in = num_out = vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_T2, v->x, v->y, v->z, &xv0, &yv0,
                                  &zv0);
      MRISsurfaceRASToVoxelCached(mris, mri_T2, v->targx, v->targy, v->targz,
                                  &xv, &yv, &zv);
...
static int labels_to_correct[] = {Left_Hippocampus, Right_Hippocampus,
...
static int edit_aseg_with_surfaces(MRI_SURFACE *mris, MRI *mri_aseg) {
  ...
  mri_filled = MRISfillInterior(mris, mri_aseg->xsize / 4, nullptr);
  ...
  counts = MRIhistogramLabels(mri_aseg, nullptr, MAX_CMA_LABEL + 1);
