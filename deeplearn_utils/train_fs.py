...
class DeepImageSynth(object):
    def __init__(
        ...
        self.feature_generator = FeatureGenerator(
            self.feature_shape, temp_folder=temp_folder, storage_loc=storage_loc
        )
        self._weight_file = None

    @classmethod
    def from_file(
        cls, model_filename, storage_loc="memory", temp_folder=os.getcwd()
    ):
        model = load_model(model_filename)
        input_shape = model.input_shape

        feature_shape = tuple(input_shape[1:-1])
        cls_init = cls(1, feature_shape, storage_loc, temp_folder)
    ...
    def load_input_to_synth_images(self, image_filenames):
        ...
        self.feature_generator.create_feature_array(
            image_filenames,
            seg_filenames=None,
            array_name="input_to_synth",
            indices=None,
        )
    ...
    def load_training_images(
        ...
        self.feature_generator.generate_src_trg_training_data(
            source_filenames,
            target_filenames,
            source_seg_filenames,
            target_seg_filenames,
        )
    ...
    def synthesize_image(self, in_img_file, out_img_filename, step_size):
        ...
        (
            ...
            out_patches, in_indices, padded_img_size, patch_crop_size
        )
        out_img_data = intensity_standardize_utils.wm_peak_normalize(
            out_img_data
        )
...
class FeatureGenerator(object):
    ...
    def create_training_feature_array(
        self, image_filenames, seg_filenames, array_name, indices_list
    ):
        ...
        seg_array = self.data_storage.create_earray(
            self.data_storage.root,
            array_name + "_seg",
        ...
        if indices_list == None:
            ...
            for input_file, seg_file, curr_indices in zip(
                image_filenames, seg_filenames, indices_list
            ):
    ...
    def create_feature_array(
        self, image_filenames, seg_filenames, array_name, indices
    ):
        ...
        for input_file, seg_file in zip(image_filenames, seg_filenames):
            if indices == None:
                (features, indices) = self.extract_patches(
                    input_file,
                    intensity_threshold=0,
                    step_size=[1, 1, 1],
                    indices=None,
    ...
    def extract_training_patches(
        self, in_img_file, seg_img_file, intensity_threshold, step_size, indices
    ):
        ...
        if indices is not None:
            ...
            (seg_patches, seg_indices, _) = self.extract_patches(
                seg_img_file,
                intensity_threshold,
                step_size,
                is_label_img=True,
            ...
            for l in unique_labels:
                ...
                if l == 0:
                    ...
                    rand_idx_idxs = np.random.randint(
                        0, len(l_idxs), (samples_per_label)
                    )
                    ...
                    samples_per_label = np.maximum(
                        onepercent_samples, min_samples_per_label
                    )
                    rand_idx_idxs = np.random.randint(
                        0, len(l_idxs), (samples_per_label)
                    )
    ...
    def extract_patches(
        ...
        indices = np.concatenate(
            (idx_x.reshape(-1, 1), idx_y.reshape(-1, 1), idx_z.reshape(-1, 1)),
            axis=1,
        )
        return patches, np.int32(indices), padded_img_size

    def build_image_from_patches(
        self, in_patches, indices, padded_img_size, patch_crop_size
    ):
        ...
        patch_mask[
            0 + patch_crop_size[0] : self.feature_shape[0] - patch_crop_size[0],
            0 + patch_crop_size[1] : self.feature_shape[1] - patch_crop_size[1],
            0 + patch_crop_size[2] : self.feature_shape[2] - patch_crop_size[2],
        ] = 1
        ...
        for patch_iter in range(len(idx_x)):
            out_img_data[
                ...
                np.reshape(in_patches[patch_iter, :], in_patches.shape[1:4]),
                patch_mask,
            )
    ...
    def generate_src_trg_training_data(
        ...
        (_, _, indices_list) = self.create_training_feature_array(
            source_filenames, source_seg_filenames, "src", indices_list=None
        )

        print("Creating target image patches.")
        self.create_training_feature_array(
            target_filenames, target_seg_filenames, "trg", indices_list
        )
...
if __name__ == "__main__":
    ...
    src_filenames = fetch_training_data_files(
        fs_dir, src_scanner, src_img_input_type, np.array([0, 2, 8, 9])
    )
    src_seg_img_input_type = "aparc+aseg"
    src_seg_filenames = fetch_training_data_files(
        fs_dir, src_scanner, src_seg_img_input_type, np.array([0, 2, 8, 9])
    )
    ...
    trg_filenames = fetch_training_data_files(
        fs_dir, trg_scanner, trg_img_input_type, np.array([0, 2, 8, 9])
    )
    trg_seg_filenames = fetch_training_data_files(
        fs_dir, trg_scanner, trg_seg_img_input_type, np.array([0, 2, 8, 9])
    )

    src_img_input_type = "orig/001"
    src_scanner = "TRIOmecho"
    val_src_filenames = fetch_training_data_files(
        fs_dir, src_scanner, src_img_input_type, np.array([3])
    )
    src_seg_img_input_type = "aparc+aseg"
    val_src_seg_filenames = fetch_training_data_files(
        fs_dir, src_scanner, src_seg_img_input_type, np.array([3])
    )
    ...
    val_trg_filenames = fetch_training_data_files(
        fs_dir, trg_scanner, trg_img_input_type, np.array([3])
    )
    val_trg_seg_filenames = fetch_training_data_files(
        fs_dir, src_scanner, src_seg_img_input_type, np.array([3])
    )

    fg = FeatureGenerator(
        feature_shape=(64, 64, 1),
        temp_folder="/local_mount/space/nike/1/users/amod/tmp",
        storage_loc="disk",
    )
    fg.create_data_storage()
    fg.generate_src_trg_training_data(
        src_filenames, trg_filenames, src_seg_filenames, trg_seg_filenames
    )
    fg.generate_src_trg_validation_data(
        val_src_filenames,
        val_trg_filenames,
        val_src_seg_filenames,
        val_trg_seg_filenames,
    )

    # feature_shape = (16, 16, 16)
#    mecho_to_mprage_synth = DeepImageSynth(unet_downsampling_factor=1, feature_shape=feature_shape,
#                                           storage_loc="disk", temp_folder="/local_mount/space/nike/1/users/amod/tmp")
# mecho_to_mprage_synth.load_training_images(src_filenames, trg_filenames, src_seg_filenames, trg_seg_filenames)
# mecho_to_mprage_synth.load_validation_images(val_src_filenames, val_trg_filenames,
#                                              val_src_seg_filenames, val_trg_seg_filenames)
#  output_dir = "/autofs/space/mreuter/users/amod/deep_learn/results/mecho_to_mprage"
#  subprocess.call(['mkdir', '-p', output_dir])
# mecho_to_mprage_synth.train_network(output_prefix=opj(output_dir, "mecho_to_mprage"),epochs=5, initial_epoch=1,
#                                   batch_size=64, steps_per_epoch=10000, save_per_epoch=True, save_weights=True)


# predict
# mecho_to_mprage_synth.from_file("/autofs/space/mreuter/users/amod/deep_learn/results/mecho_to_mprage
# /mecho_to_mprage_model_epoch3.h5")


# mtest = DeepImageSynth.from_file("/autofs/space/mreuter/users/amod/deep_learn/results/mecho_to_mprage"
#                                  "/mecho_to_mprage_model_epoch3.h5")
#
#
...
#
#     out_dir = opj(output_dir, src_scanner, subj_id, "unet3dv0")
#     subprocess.call(['mkdir', '-p', out_dir])
#     out_file = opj(out_dir, split_filename(test_file)[1] + '_unet_mae_3D_trainid_0_2_8_9.mgz')
#     mtest.synthesize_image(test_file, out_file, step_size=[4,4,4])
#


# mecho_to_mprage_synth4 = DeepImageSynth(unet_downsampling_factor=4, feature_shape=feature_shape,
#                                         storage_loc="disk", temp_folder="/local_mount/space/nike/1/users/amod/tmp")
# mecho_to_mprage_synth4.load_training_images(src_filenames, trg_filenames, src_seg_filenames, trg_seg_filenames)
# mecho_to_mprage_synth4.load_validation_images(val_src_filenames, val_trg_filenames,
#                                               val_src_seg_filenames, val_trg_seg_filenames)
# output_dir = "/autofs/space/mreuter/users/amod/deep_learn/results/mecho_to_mprage"
# subprocess.call(['mkdir', '-p', output_dir])
#
# mecho_to_mprage_synth4.train_network(output_prefix=opj(output_dir, "mecho_to_mprage4"), epochs=5, initial_epoch=1,
#                                      batch_size=64, steps_per_epoch=10000, save_per_epoch=True, save_weights=True)
#
#
#
# mecho_to_mprage_synth2 = DeepImageSynth(unet_downsampling_factor=2, feature_shape=feature_shape,
#                                        storage_loc="disk", temp_folder="/local_mount/space/nike/1/users/amod/tmp")
# mecho_to_mprage_synth2.load_training_images(src_filenames, trg_filenames, src_seg_filenames, trg_seg_filenames)
# mecho_to_mprage_synth2.load_validation_images(val_src_filenames, val_trg_filenames,
#                                               val_src_seg_filenames, val_trg_seg_filenames)
# output_dir = "/autofs/space/mreuter/users/amod/deep_learn/results/mecho_to_mprage"
# subprocess.call(['mkdir', '-p', output_dir])
#
# mecho_to_mprage_synth2.train_network(output_prefix=opj(output_dir, "mecho_to_mprage2"), epochs=5, initial_epoch=1,
#                                                     batch_size=64, steps_per_epoch=10000, save_per_epoch=True,
#                                     save_weights=True)

# mecho_to_mprage_synth4 = DeepImageSynth(unet_num_filters=16,  unet_depth=2, unet_downsampling_factor=1,
...
# mecho_to_mprage_synth4.train_network(output_prefix=opj(output_dir, "mecho_to_mprage_test"), epochs=5, initial_epoch=1,
#                                    batch_size=64, steps_per_epoch=10000, save_per_epoch=True, save_weights=True)
