...
static int compute_optimal_translation(MRI *mri_src, MRI *mri_dst, MRI *mri_seg,
                                       ...
                                       int max_trans, int nsteps, int skip,
...
int main(int argc, char *argv[]) {
  ...
  if (probe) {
    ...
    mri_aligned = align_block_to_histo(mri_histo, mri_block, mri_seg, pdfs,
                                       nlevels, mat, cost_type);
  }
...
static MRI *align_block_to_histo(MRI *mri_histo, MRI *mri_block, MRI *mri_seg,
  ...
  mri_aligned =
      MRIalloc(mri_histo->width, mri_histo->height, 1, mri_histo->type);
  MRIcopyHeader(mri_histo, mri_aligned);
  MRIextractInto(mri_block, mri_aligned, 0, 0, 0, mri_aligned->width,
                 mri_aligned->height, 1, 0, 0, 0);
  compute_optimal_xform(mri_block, mri_histo, mri_seg, pdfs, nlevels, mat,
                        cost_type, skip);
...
static int align_pyramid_level(MRI *mri_src, MRI *mri_dst, MRI *mri_seg,
  ...
  *MATRIX_RELT(m_scale, 3, 3) = 1.0;
  ...
  write_snapshot(mri_fullres_src, mri_src, mri_dst, m_best, base, snapshots++,
                 level, pdf, mri_seg);
  for (scale = 1; scale >= 0.005; scale /= 2) {
    ...
    if (sstep < 0.00000001)
      sstep = 1.0;
    printf(
        "scale %2.4f, searching angles [%2.3f --> %2.3f] by %2.3f, scales "
        "[%2.2f->%2.2f] by %2.4f, and translations to %2.3f voxels by %2.3f\n",
        scale, DEGREES(mina), DEGREES(maxa), DEGREES(astep), mins, maxs, sstep,
        maxt, tstep);
    found = compute_optimal_translation(mri_src, mri_dst, mri_seg, m_best, pdf,
                                        maxt, 50, level, skip, mri_fullres_src,
                                        cost_type);
    found = compute_optimal_translation(mri_src, mri_dst, mri_seg, m_best, pdf,
                                        maxt / 50, 10, level, skip,
                                        mri_fullres_src, cost_type) ||
            found;
    if (found)
      write_snapshot(mri_fullres_src, mri_src, mri_dst, m_best, base,
...
static double compute_alignment_error(MRI *mri_src, MRI *mri_dst, MRI *mri_seg,
                                      ...
                                      int level, int cost_type) {
  if (min_overlap > 0 &&
      (compute_overlap(mri_src, mri_dst, m_total) < min_overlap) && !probe)
    return (100000000);

  switch (cost_type) {
  case COST_MAXIMUM_LIKELIHOOD:
    return (compute_ml_alignment_error(mri_src, mri_dst, mri_seg, pdf, m_total,
                                       skip, level));
    break;
  case COST_CORRELATION_RATIO:
    return (compute_cr_alignment_error(mri_src, mri_dst, mri_seg, pdf, m_total,
                                       skip, level));
    break;
  }
...
static int write_snapshot(MRI *mri, MRI *mri1, MRI *mri2, MATRIX *m, char *base,
                          int n, int level, DENSITY *pdf, MRI *mri_seg) {
...
static int compute_optimal_translation(MRI *mri_src, MRI *mri_dst, MRI *mri_seg,
                                       MATRIX *m_best, DENSITY *pdf, int maxt,
...
static int powell_minimize(MRI *mri_block, MRI *mri_histo, MRI *mri_seg,
  ...
  do {
    ...
    printf("%3.3d: best alignment at after powell: %2.3f (%d steps)\n",
           snapshots, fret, iter);
    write_snapshot(mri_block, mri_block, mri_histo, mat, base, snapshots++, 0,
                   pdf, mri_seg);
  } while (fret < fstart);
...
static float compute_powell_sse(float *p) {
  ...
  error = compute_alignment_error(Gmri_block, Gmri_histo, Gmri_seg, Gpdf, mat,
                                  skip, 0, Gcost_type);
  return (error);
}
