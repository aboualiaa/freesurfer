...
int main(int argc, char *argv[]) {
  ...
  if (osurf_fname) {
    ...
    if (mris2 == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read 2nd surface from %s",
                Progname, osurf_fname);
  ...
  if (laplace_thick) {
    ...
    MRISmeasureLaplaceStreamlines(mris, mri_laplace, nullptr, nullptr);
  } else if (fmin_thick) {
    ...
    if (Gdiag & DIAG_WRITE) {
      ...
      MRISsaveVertexPositions(
          mris, CANONICAL_VERTICES); // restore spherical canonical coords
    }
  ...
  if (cortex_label) // fill in thickness in holes in the cortex label where it
                    // isn't to be trusted
  {
  ...
  fprintf(stderr, "writing %s to curvature file %s...\n",
          write_vertices ? "vertex correspondence" : "thickness", out_fname);
  MRISwriteCurvature(mris, out_fname);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    fsaverage_label = LabelRead(nullptr, argv[3]);
    if (fsaverage_label == nullptr)
...
static void print_help() {
  ...
  fprintf(stderr, "-fill_holes <cortex label> <fsaverage cortex label> fill in "
                  "thickness in holes in the cortex label\n");
  exit(1);
}
...
static int fill_thickness_holes(MRI_SURFACE *mris, LABEL *cortex_label,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (v->marked ==
        2) // marked only in fsaverage surface, don't make it a fixed point
      v->marked = 0;
    else
      v->marked = 1;
  }
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (v->marked < 2)
      v->val =
          0; // fix everything not in the fsaverage label to have 0 thickness
  }
