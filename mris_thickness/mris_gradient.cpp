 ...
 *
 * Compute the gradient of a surface based vector field by first computing the
 * individual partial derivatives using a least squared fit to the local
 * neighborhood using a Taylor expansion f1 = f0 + dx * delX1 + dy *delY1 ...,
 * then use the Froebenius norm to collapse this to a scalar field
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  mri_grad = MRIScomputeGradient(mris, mri, FROBENIUS_NORM, nullptr);
  if (mask_label) {
    int vno, f, l;
...
static void print_usage() {
  fprintf(stderr,
          "usage: %s [options] <input surface> <input vector field> <output "
          "gradient file>\n",
          Progname);
}
...
static void print_help() {
  print_usage();
  fprintf(stderr, "\nThis program computes the gradient of an intensity "
                  "profile of the cortical ribbon\n"
...
MRI *MRIScomputeGradient(MRI_SURFACE *mris, MRI *mri, int which_norm,
  ...
  if (mri_grad == nullptr)
    mri_grad =
        MRIallocSequence(mris->nvertices, 1, 1, MRI_FLOAT, mri->nframes + 1);
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (n = 0; n < vt->vtotal; n++) {
      ...
      if (vno2 == Gdiag_no)
        DiagBreak();
      x1 = v->e1x * (vn->x - v->x) + v->e1y * (vn->y - v->y) +
           v->e1z * (vn->z - v->z);
      y1 = v->e2x * (vn->x - v->x) + v->e2y * (vn->y - v->y) +
           v->e2z * (vn->z - v->z);
    ...
    m_inv = MatrixSVDInverse(m_xtx, nullptr);
    if (m_inv == nullptr) {
      MatrixFree(&m_x);
      MatrixFree(&m_xt);
      MatrixFree(&m_xtx);
