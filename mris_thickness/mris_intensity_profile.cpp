...
int main(int argc, char *argv[]) {
  ...
  if (use_normals) {
    ...
    mri_profiles = MRISmeasureNormalCorticalIntensityProfiles(
        mris, mri, dist_in, dist_out, max_samples);
  ...
  if (xform_fname != nullptr) {
    ...
    if (transform_type == MNI_TRANSFORM_TYPE ||
      ...
      if (transform_type == FSLREG_TYPE) {
        if (lta_src == nullptr || lta_dst == nullptr) {
          fprintf(stderr, "ERROR: fslmat does not have information on the src "
                          "and dst volumes\n");
          fprintf(stderr,
                  "ERROR: you must give options '-src' and '-dst' to specify "
                  "the src and dst volume infos for the registration\n");
        }

        LTAmodifySrcDstGeom(lta, lta_src,
                            lta_dst); // add src and dst information
      ...
      if (lta->xforms[0].src.valid == 0) {
        if (lta_src == nullptr) {
          ...
          LTAmodifySrcDstGeom(lta, lta_src, nullptr); // add src information
        }
      }
      if (lta->xforms[0].dst.valid == 0) {
        if (lta_dst == nullptr) {
          ...
          LTAmodifySrcDstGeom(lta, nullptr, lta_dst); // add  dst information
        }
      }
  ...
  if (laplace_thick) {
    ...
    if (read_laplace_name) {
      ...
      MRISsaveVertexPositions(mris, WHITE_VERTICES);
      mri_dist_white = MRIScomputeDistanceToSurface(mris, nullptr, laplace_res);
      for (nmissing = nwmissing = npmissing = vno = 0; vno < mris->nvertices;
           vno++) {
        v = &mris->vertices[vno];
        MRISsurfaceRASToVoxel(mris, mri_dist_pial, v->pialx, v->pialy, v->pialz,
                              &xv, &yv, &zv);
        MRIsampleVolumeFrameType(mri_dist_pial, xv, yv, zv, 0, SAMPLE_NEAREST,
                                 &pial_val);
        MRISsurfaceRASToVoxel(mris, mri_dist_white, v->whitex, v->whitey,
                              v->whitez, &xv, &yv, &zv);
        MRIsampleVolumeFrameType(mri_dist_white, xv, yv, zv, 0, SAMPLE_NEAREST,
                                 &white_val);
      ...
      printf("%d of %d pial surface nodes not resolved - %2.3f %%\n", npmissing,
             mris->nvertices, 100.0 * npmissing / mris->nvertices);
      ...
      fprintf(fp, "%f %d %d %f %d %f %d %f\n", laplace_res, mris->nvertices,
              nwmissing, 100.0 * nwmissing / mris->nvertices, npmissing,
              100.0 * npmissing / mris->nvertices, nmissing,
              100.0 * nmissing / mris->nvertices);
      fclose(fp);
      MRISsaveVertexPositions(mris, PIAL_VERTICES);
      MRISrestoreVertexPositions(mris, ORIGINAL_VERTICES);
      MRISsaveVertexPositions(mris, WHITE_VERTICES);
      mri_laplace = MRISsolveLaplaceEquation(mris, nullptr, laplace_res);
    }
    mri_profiles =
  ...
  if (nlabels > 0) {
    ...
    for (l = 0; l < nlabels; l++) {
      sprintf(label_name, "%s/%s/label/%s.%s.label", sdir, sname, hemi,
              label_names[l]);

      label = LabelRead(nullptr, label_name);
      if (!label)
        ErrorExit(ERROR_NOFILE, "%s: could not read label file %s...\n",
                  Progname, label_name);
      if (num_erode > 0) {
        printf("eroding label %d times, npoints went from %d ", num_erode,
               label->n_points);
  ...
  if (norm_white > 0)
    norm = mrisComputeWhiteMatterIntensities(mris, mri, 15, nullptr, 100,
                                             mri_aseg, wm_border_mm);
  if (flat_name) {
    if (MRISreadFlattenedCoordinates(mris, flat_name) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not load flat patch %d\n", Progname,
                flat_name);
    mri_profiles =
        MRIScomputeFlattenedVolume(mris, mri, flat_res, max_samples, normalize,
                                   nullptr, smooth_iters, 0, 0);
  } else if (!laplace_thick && !fmin_thick)
    mri_profiles = MRISfindNearestVerticesAndMeasureCorticalIntensityProfiles(
        mris, mri, nbhd_size, max_thick, normalize, curv_thresh, norm);
  ...
  if (norm_csf > 0) {
    ...
    mri_ven = MRIclone(mri_aseg, nullptr);
    MRIcopyLabel(mri_aseg, mri_ven, Left_Lateral_Ventricle);
    MRIcopyLabel(mri_aseg, mri_ven, Right_Lateral_Ventricle);
    mri_tmp = MRIcopy(mri_ven, nullptr);
    // erode as many times as we can while still having enough for stable
    // estimate of CSF
    n = 0;
    do {
  ...
  if (normal_sigma > 0) {
    ...
    mri_tmp = fMRItemporalGaussian(mri_profiles, normal_sigma, nullptr);
    MRIfree(&mri_profiles);
    mri_profiles = mri_tmp;
    if (Gdiag_no >= 0) {
#if 0
      float v1, v2, v3 ;
      v1 = MRIgetVoxVal(mri_profiles, Gdiag_no, 0, 0, 0) ;
  ...
  if (quadfit) {
    ...
    mri_quad = MRIfitQuadratic(mri_profiles, nullptr);
    ...
    mri_poly = MRIfitPolynomial(mri_profiles, nullptr, polyfit);
    if (zero_mean) {
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    nargs = 1;
  } else if (!stricmp(option, "new") || !stricmp(option, "fmin") ||
             !stricmp(option, "variational")) {
    fmin_thick = 1;
    ...
    fprintf(stderr,
            ...
            -dist_in, dist_out, max_samples,
            (dist_in + dist_out) / (float)(max_samples - 1));
    nargs = 2;
    ...
    fprintf(
        stderr, "computing overlay as ratio of intervals [%d %d] / [%d %d]\n",
    ...
    norm_gw = atof(argv[2]);
    fprintf(stderr,
            "normalizing intensities to have be %2.2f at gray/white boundary\n",
            norm_gw);
    nargs = 1;
    ...
    fprintf(
        stderr,
        "normalizing intensities to have be %2.2f relative to white matter\n",
        norm_white);
    nargs = 1;
  } else if (!stricmp(option, "norm_csf")) {
    norm_csf = atof(argv[2]);
    fprintf(stderr,
            "normalizing intensities to have be %2.2f relative to csf\n",
            norm_csf);
    nargs = 1;
  } else if (!stricmp(option, "norm_pial")) {
    norm_pial = atof(argv[2]);
    fprintf(stderr,
            "normalizing intensities to have be %2.2f at gray/csf boundary\n",
            norm_pial);
    nargs = 1;
  } else if (!stricmp(option, "norm_mid")) {
    norm_mid = atof(argv[2]);
    fprintf(stderr,
            "normalizing intensities to have be %2.2f at mid point of ribbon\n",
            norm_mid);
    nargs = 1;
    ...
    nargs = 1;
  } else if (!stricmp(option, "lta_dst") || !stricmp(option, "dst")) {
...
static void print_help() {
  ...
  fprintf(stderr, "-mean %%s outputs the mean profile-integral to the "
                  "specified file (output is in curv format).\n");
  fprintf(
      stderr,
      "-xform %%s specify the registration xform that maps the T1 volume (from "
      "which the surfaces were computed) to the input volume to be sampled.\n");
  fprintf(
      stderr,
      "-src %%s source volume used when computing the registration xform\n");
  fprintf(stderr, "-dst %%s destination volume used when computing the "
                  "registration xform\n");
  fprintf(
      stderr,
...
MRI *MRISfindNearestVerticesAndMeasureCorticalIntensityProfiles(
  ...
  int vno, n, vlist[100000], vtotal, ns, i, vnum, min_n, pial_vno, nsamples,
      min_vox;
...
#define SAMPLE_DIST 0.5
    ...
    if (normalize) {
      ...
      if (norm_mean > 0) {
        for (nsamples = 0, d = 0.0, scale = 0.0; nsamples < max_samples;
             d += sample_dist, nsamples++) {
          x = v->origx + d * dx;
          y = v->origy + d * dy;
          z = v->origz + d * dz;
      ...
      for (nsamples = 0, d = 0.0; nsamples < max_samples;
           d += sample_dist, nsamples++) {
        x = v->origx + d * dx;
        y = v->origy + d * dy;
        z = v->origz + d * dz;
      ...
      //   for (nsamples = 0, d = 0.0 ; d <= max_thick ; d += sample_dist,
      //   nsamples++)
      if (norm_mean > 0) {
        for (nsamples = 0, d = 0.0, scale = 0.0; nsamples < max_samples;
             d += sample_dist, nsamples++) {
      ...
      for (nsamples = 0, d = -extra * sample_dist; nsamples < max_samples;
           d += sample_dist, nsamples++) {
#if 0
  ...
  if (FZERO(norm_gw) && false) {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri, v->whitex, v->whitey, v->whitez,
                                  &xv, &yv, &zv);
      if (xv < min_vox || yv < min_vox || zv < min_vox ||
          xv > mri->width - min_vox || yv > mri->height - min_vox ||
          zv > mri->depth - min_vox)
        continue;   // near edge of FOV
      if (bad == 0) // check to make sure it's not all too dark
      {
        for (nsamples = 0; nsamples < 2 * mri_profiles->nframes / 3;
             nsamples++) {
          val = MRIgetVoxVal(mri_profiles, vno, 0, 0, nsamples);
          if (val < min_gray)
            bad = 1;
        ...
        for (t = whalf; t < nsamples - whalf; t++) {
          for (tk = -whalf; tk <= whalf; tk++) {
            ...
            *MATRIX_RELT(mX, tk + whalf + 1, 3) = 1.0f;
            ...
            VECTOR_ELT(vY, tk + whalf + 1) =
                MRIgetVoxVal(mri_profiles, x, y, z, ti);
          }
...
static int remove_bad_profiles(MRI_SURFACE *mris, MRI *mri_profiles, MRI *mri,
  ...
  if (MRISreadAnnotation(mris, "ad_aparc") != NO_ERROR) {
    if (MRISreadAnnotation(mris, "aparc") != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not read annotation file ad_aparc",
                Progname);
  }
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (xv < nvox || yv < nvox || zv < nvox || xv > mri->width - nvox ||
        yv > mri->height - nvox || zv > mri->depth - nvox ||
        (annot_index == unknown_index) || (annot_index == med_index))
...
static float *mrisComputeWhiteMatterIntensities(MRI_SURFACE *mris, MRI *mri,
  ...
  if (norm == nullptr)
    ErrorExit(ERROR_NOMEMORY,
              "mrisComputeWhiteMatterIntensities: couldn't allocate array");
          ...
          for (zk = -whalf; zk <= whalf; zk++) {
            ...
            if (val > 0.5) // in wm
            {
              ...
              if (mri_aseg) {
                ...
                MRIsampleVolumeType(mri_aseg, V3_X(v2), V3_Y(v2), V3_Z(v2),
                                    &val, SAMPLE_NEAREST);
              } else
    ...
    for (dist = -dist_in, n = 0; n < nsamples; n++, dist += del) {
      ...
      if (vno == Gdiag_no)
        printf("d %2.2f: (%2.1f, %2.1f, %2.1f) --> (%d, %d, %d): %2.1f\n", dist,
               x, y, z, (int)(nint(xv)), (int)(nint(yv)), (int)(nint(zv)), val);
    }
  }
