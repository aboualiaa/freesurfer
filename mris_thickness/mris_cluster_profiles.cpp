...
static char vcid[] =
    "$Id: mris_cluster_profiles.c,v 1.4 2011/03/02 00:04:34 nicks Exp $";
...
static int remove_vertex_from_cluster(MRI_SURFACE *mris, MRI *mri_profiles,
                                      CLUSTER *ct, int c, int vno);
static int add_vertex_to_cluster(MRI_SURFACE *mris, MRI *mri_profiles,
                                 CLUSTER *ct, int c, int vno);
static int find_most_likely_unmarked_vertex(MRI_SURFACE *mris,
                                            MRI *mri_profiles, CLUSTER *ct,
                                            int k, int *pcluster_no);
static int load_vals(MRI *mri_profile, VECTOR *v, int vno);
static int find_most_likely_vertex_to_swap(MRI_SURFACE *mris, MRI *mri_profiles,
                                           CLUSTER *ct, int k,
                                           int *pcluster_no);
...
static int mark_clusters(MRI_SURFACE *mris, MRI *mri_profiles, CLUSTER *ct,
                         int k);
static int compute_cluster_statistics(MRI_SURFACE *mris, MRI *mri_profiles,
                                      CLUSTER *ct, int k);
...
static int initialize_kmeans_centers(MRI_SURFACE *mris, MRI *mri_profiles,
                                     CLUSTER *ct, int k);
static int initialize_cluster_centers(MRI_SURFACE *mris, MRI *mri_profiles,
                                      CLUSTER *ct, int k);
static int initialize_cluster_centers_with_ico(MRI_SURFACE *mris,
                                               MRI *mri_profiles, CLUSTER *ct,
                                               MRI_SURFACE *mris_ico);
  ...
  MRISwriteAnnotation(mris, seg_fname);
  {
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      for (i = 0; i < mri_profiles->nframes; i++)
        MRIsetVoxVal(mri_profiles, vno, 0, 0, i,
                     VECTOR_ELT(ct[c].v_mean, i + 1));
    }
...
static void print_usage(void) {
  fprintf(stderr,
          "usage: %s [options] <input profile file> <spherical surface> "
          "<output clustering>\n",
          Progname);
}
...
CLUSTER *MRIScluster(MRI_SURFACE *mris, MRI *mri_profiles, int cluster_type,
  ...
  case K_MEANS:
    cluster_table =
        MRISclusterKMeans(mris, mri_profiles, k, start_fname, mris_ico);
    break;
  case AGGLOMERATIVE:
    cluster_table =
        MRISclusterAgglomerative(mris, mri_profiles, k, start_fname, mris_ico);
    break;
  }
...
static CLUSTER *MRISclusterAgglomerative(MRI_SURFACE *mris, MRI *mri_profiles,
  ...
  if (start_fname) {
    ...
    if (!ct)
      ErrorExit(ERROR_BADPARM, "%s: could not allocate %d clusters", Progname,
                k);
    ...
    do {
      vno =
          find_most_likely_unmarked_vertex(mris, mri_profiles, ct, k, &cluster);
      if (vno < 0)
        break;
  ...
  do {
    ...
    if (vno < 0)
      break;
    remove_vertex_from_cluster(mris, mri_profiles, ct, mris->vertices[vno].curv,
                               vno);
    add_vertex_to_cluster(mris, mri_profiles, ct, cluster, vno);
    if (write_iters > 0 && ((iter % write_iters) == 0)) {
...
static int initialize_kmeans_centers(MRI_SURFACE *mris, MRI *mri_profiles,
  ...
  for (i = 0; i < k; i++) {
    ...
    for (j = 0; j < nsamples; j++)
      VECTOR_ELT(ct[i].v_mean, j + 1) =
          MRIgetVoxVal(mri_profiles, vno, 0, 0, j);
  }
  for (i = 0; i < k; i++) {
...
static int compute_cluster_statistics(MRI_SURFACE *mris, MRI *mri_profiles,
  ...
  for (cluster = 0; cluster < k; cluster++)
    if (num[cluster] > 0)
      VectorScalarMul(ct[cluster].v_mean, 1.0 / (double)num[cluster],
                      ct[cluster].v_mean);
...
static int compute_cluster_statistics(MRI_SURFACE *mris, MRI *mri_profiles,
  ...
  for (cluster = 0; cluster < k; cluster++)
    if (num[cluster] > 0)
      VectorScalarMul(v_means[cluster], 1.0 / (double)num[cluster],
                      v_means[cluster]);
  ...
  for (cluster = 0; cluster < k; cluster++)
    if (num[cluster] > 0)
      MatrixScalarMul(m_covs[cluster], 1.0 / (double)num[cluster],
                      m_covs[cluster]);
...
static int find_most_likely_vertex_to_swap(MRI_SURFACE *mris, MRI *mri_profiles,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    dist_current =
        MatrixMahalanobisDistance(ct[c1].v_mean, ct[c1].m_cov, v_vals);
    for (n = 0; n < v->vnum; n++) {
      vn = &mris->vertices[v->v[n]];
      if (vn->curv < 0)
        continue;
      ...
      dist_changed =
          MatrixMahalanobisDistance(ct[c2].v_mean, ct[c2].m_cov, v_vals);
...
static int load_vals(MRI *mri_profiles, VECTOR *v, int vno) {
  int i;
...
static int clusters_connected(MRI_SURFACE *mris, int vno1, int vno2, int num) {
