...
int main(int argc, char **argv) {
  ...
  // help text
  parser.addHelp(mris_defects_pointset_help_xml,
                 mris_defects_pointset_help_xml_len);
  ...
  // mris_apply_reg --src-label ../label/lh.cortex.label --streg lh.orig
  // lh.orig.nofix --trg ../label/lh.cortex.nofix.label
  ...
  for (int v = 0; v < surf->nvertices; v++) {
    ...
    if (defect_idx >= 0)
      defects[defect_idx].push_back(v);
  }
  ...
  for (auto &defect : defects) {
    if (!defect.empty()) {
      ...
      centroid.y /= npoints;
      centroid.z /= npoints;
      // if the surface contains volume geometry, convert the point to scanner
      // RAS
      if (surf->vg.valid)
        centroid = sras2ras(surf, centroid);
...
// returns a point transformed from surface RAS to scanner RAS coordinates -
// ideally this function should be moved to mrisurf.c or somewhere else more
// appropriate
static PointSet::Point sras2ras(MRIS *surf, PointSet::Point point) {
  static MATRIX *sras2ras_matrix;
  static VECTOR *v1, *v2;

  // allocate the matrix and vectors (this will only be done once in the entire
  // program)
  if (!sras2ras_matrix) {
    // quick way to get the sras2ras matrix by using RASFromSurfaceRAS_ with a
    // tmp volume
    MRI *tmp = MRIallocHeader(surf->vg.width, surf->vg.height, surf->vg.depth,
                              MRI_UCHAR, 1);
