...
static int write_surface_warp_into_volume(MRI_SURFACE *mris, MRI *mri,
  ...
  mri_ctrl =
      MRIallocSequence(mri->width, mri->height, mri->depth, MRI_UCHAR, 1);
  MRIcopyHeader(mri, mri_weights);
  MRIcopyHeader(mri, mri_ctrl);
  //   build a 3 frame volume with the voxel-coords warp (dx, dy, dz) in frames
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    zvi = nint(zv);
    if (vno == Gdiag_no) {
      printf("surface vertex %d: inflated (%2.0f, %2.0f, %2.0f), orig (%2.0f, "
             "%2.0f, %2.0f), "
             "dx=(%2.0f, %2.0f, %2.0f)\n",
             vno, xv1, yv1, zv1, xv, yv, zv, dx, dy, dz);
      DiagBreak();
    }
    if (xvi < 0 || xvi >= mri->width || yv < 0 || yv >= mri->height || zv < 0 ||
        zv >= mri->depth) {
      continue;
    }
  ...
  // normalize the warp field using a weighted average of all vertices that map
  // to every voxel
  for (xvi = 0; xvi < mri->width; xvi++)
    for (yvi = 0; yvi < mri->height; yvi++)
      for (zvi = 0; zvi < mri->depth; zvi++) {
        ...
        MRIsetVoxVal(mri_ctrl, xvi, yvi, zvi, 0,
                     CONTROL_MARKED); // it is a control point
      }
...
static MRI *expand_mri_to_fit_surface(MRI_SURFACE *mris, MRI *mri) {
  ...
  mri_dst = MRIextractRegionAndPad(mri, nullptr, nullptr, pad);
  return (mri_dst);
}
...
int main(int argc, char *argv[]) {
  ...
  /*
    note that a "forward" morph means a retraction, so we reverse the order of
    the argvs here. This means that for every voxel in the inflated image we
    have a vector that points to where in the original image it came from, and
    *NOT* the reverse.
  */
  ...
  if (like_vol_name == nullptr) {
    mri = MRIallocSequence(mris->vg.width, mris->vg.height, mris->vg.depth,
                           MRI_FLOAT, 3);
    ...
    mri = MRIallocSequence(mri_tmp->width, mri_tmp->height, mri_tmp->depth,
                           MRI_FLOAT, 3);
 ...
 GCAMinit(gcam, mri, NULL, NULL, 0) ;
  ...
  if (mri_in) {
    ...
    if (Gdiag_no >= 0) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri, v->origx, v->origy, v->origz, &xi,
                                  &yi, &zi);
      ...
      printf(
          ...
          gcamn->x, gcamn->y, gcamn->z, gcamn->origx, gcamn->origy,
          gcamn->origz, val);
      DiagBreak();
    }
  }
