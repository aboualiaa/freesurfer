...
static int assemble_training_data_and_free_mris(
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_rf_train.c,v 1.3 2016/12/11 14:33:34 fischl Exp $",
      "$Name:  $");
  ...
  if (noverlays == 0)
    ErrorExit(ERROR_NOFILE,
              "%s: must specify at least one training overlay with -overlay "
              "<overlay>",
              Progname);
  ...
  for (n = ARGC_OFFSET; n < argc - 1; n++) {
    ...
    if (FileExists(fname) == 0) {
      ...
      if (FileExists(fname) == 0)
        ErrorExit(
            ERROR_NOFILE,
            "%s: subject %s has no training label (%s) for either hemisphere",
            Progname, subject, fname);
      hemi = "rh";
    } else
      hemi = "lh";
  ...
  assemble_training_data_and_free_mris(mris, mri_overlays, nsubjects, noverlays,
                                       ...
                                       &ntraining);
  RFtrain(rf, 1.0, training_fraction, training_classes, training_data,
          ntraining);
  correct =
...
static void usage_exit(int code) {
  printf("usage: %s [options]  <subject 1> <subject 2> ... <output name>\n",
         Progname);
  printf("\t--hemi <hemi>    - process <hemi> instead of lh\n"
         "\t--surf <surf>    - change default surface name from 'white' to "
         "<surf>\n");
  exit(code);
}
static int assemble_training_data_and_free_mris(
          ...
          for (f = 0; f < mri_overlays[sno][0]->nframes; f++, vno++) {
            ...
            if (training_data[tno] == nullptr)
              ErrorExit(
                  ...
                  Progname, noverlays, tno);
            for (i = 0; i < noverlays; i++)
              training_data[tno][i] =
                  MRIgetVoxVal(mri_overlays[sno][i], x, y, z, f);
            tno++;
          }
