...
static int    avgs              = 0;
static int    nclose            = 0;
static char * color_table_fname = nullptr;
...
int main(int argc, char *argv[]) {
  ...
  if (FZERO(proj_mm)) {
    if (MRISreadCurvatureFile(mris, thickness_name) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not read thickness file %s", Progname,
                thickness_name);
  }
  ...
  } else /* sample from surface to volume */
  {
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      v->annotation = v->val = MRIfindNearestNonzero(
          mri_parc, wsize, xw, yw, zw, ((float)wsize - 1) / 2);
      if (v->val == 0xffffffff)
        DiagBreak();
    }
  }
...
static void print_help() {
  ...
  fprintf(stderr, "surfaces using mris_sample_parc.\n");
  fprintf(stderr,
          "mris_ca_train is used to create an atlas from a set of \n"
          "annotated subjects. The information output by mris_ca_train\n");
  fprintf(stderr, "is then used by mris_ca_label to automatically assign a\n"
                  "neuroanatomical label to each location on a cortical "
                  "surface model.\n\n");
  fprintf(stderr, "Required args:\n"
                  "--------------\n\n");
  fprintf(stderr, "  <subject name>       the subject id\n\n");
  fprintf(stderr, "  <hemi>               hemisphere: rh or lh\n\n");
  fprintf(stderr, "  <parc name>          parcellation filename\n\n");
  fprintf(stderr, "  <output annot>       annotation filename\n\n");
  fprintf(stderr, "Optional args:\n"
                  "--------------\n\n");
  fprintf(stderr,
          "  -sdir <directory>    use <directory> as subjects directory \n"
          "                       (default: $SUBJECTS_DIR)\n\n");
  fprintf(stderr, "  -surf <filename>     use <filename> as surface "
                  "(default: 'white')\n\n");
  fprintf(stderr, "  -fix <number>        fix topology of all labels smaller \n"
                  "                       than <number> vertices (default=-1, "
                  "do all)\n\n");
  fprintf(stderr, "  -replace <number>    replace label <number> with deeper "
                  "ones\n\n");
  fprintf(stderr,
  ...
  fprintf(stderr, "  -projmm <number>     project <number> millimeters along \n"
                  "                       surface normal (default=0.0)\n\n");
  fprintf(stderr, "  -proj <number>       same as -projmm\n\n");
  fprintf(stderr,
  ...
  fprintf(stderr,
          ...
          "                       annotation file\n\n");
  fprintf(stderr, "  -v <number>          diagnostic level (default=0)\n\n");
  fprintf(stderr,
          "  -f <number>          apply mode filter <number> times to \n"
          "                       parcellation (default=0)\n\n");
  fprintf(stderr, "  -a <number>          smooth surface <number> times "
                  "(default=0)\n\n");
  fprintf(stderr,
          "  -w <number>          use window size <number> for sampling\n"
          "                       (default=7)\n\n");
  fprintf(stderr, "  -t <filename>        use thickness file <filename> \n"
                  "                       (default: 'thickness')\n\n");
  fprintf(stderr,
...
static int fix_label_topology(MRI_SURFACE *mris, int nvertices) {
  ...
  do {
    ...
    printf("pass %d: %d segments changed\n", iter + 1, nchanged);
           ...
           iter++ < 10); // can oscillate forever, so terminate after 10
...
#define MAX_SEARCH_LEN 6 // mm
    ...
    if (proj_mm > 0) {
      ...
      for (d = proj_mm; d >= -MAX_SEARCH_LEN; d += proj_mm) {
        ...
        z = v->z + d * v->nz;
        MRIsurfaceRASToVoxel(mri, x, y, z, &xw, &yw, &zw);
        new_label = (int)MRIfindNearestNonzero(mri_tmp, wsize, xw, yw, zw,
                                               ((float)wsize - 1) / 2);
