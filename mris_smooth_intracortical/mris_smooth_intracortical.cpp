  ...
  for (f = ic_start; f < ic_size + ic_start; f++) {
    ...
    if ((f > ic_start) && (over[f - ic_start]->nframes != over[0]->nframes))
      ErrorExit(ERROR_BADPARM,
                "Number of frames in overlay %s = %d is different than number "
    ...
    if (nb_rad) {
      ...
      if ((f > ic_start) &&
          (surf[f - ic_start]->nvertices != surf[0]->nvertices))
        ErrorExit(ERROR_BADPARM,
                  "Number of vertices in surface %s = %d is different than "
                  "number of vertices in surface %s = %d.\n",
                  surf_list.gl_pathv[f], surf[f - ic_start]->nvertices,
                  surf_list.gl_pathv[0], surf[0]->nvertices);
      if (over[f - ic_start]->width != surf[f - ic_start]->nvertices)
        ErrorExit(ERROR_BADPARM,
                  "Number of data points in overlay %s = %d is different than "
                  "number of vertices in surface %s = %d.\n",
                  over_list.gl_pathv[f], over[f - ic_start]->width,
                  surf_list.gl_pathv[f], surf[f - ic_start]->nvertices);
    }
  }
    ...
    for (f = 0; f < ic_size; f++) {
      for (v = 0; v < surf[f]->nvertices; v++) {
        ...
        for (t = 0; t < over[f]->nframes; t++) {
          ...
          for (n = 0; n < nb_num; n++) {
            val +=
                (MRIgetVoxVal(over[f], neighbors[n], 0, 0, t) * nb_weights[n]);
            count += nb_weights[n];
          }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!stricmp(option, "--surf_dir")) {
      ...
      if (nargc < 1)
        ErrorExit(ERROR_BADPARM, "Flag %s needs an argument\n", option);
      if (!stricmp(pargv[0], "gauss"))
        nb_wf = 0;
      else if (!stricmp(pargv[0], "distance"))
        nb_wf = 1;
      else
        fprintf(stderr,
                "Unknown value %s for flag %s, a default gaussian weighting "
                "function will be applied instead.\n",
                pargv[0], option);
      nargsused = 1;
      ...
      fprintf(stderr, "Unrecognized flag/argument %s will be ignored.\n",
              option);
    nargc -= nargsused;
    pargv += nargsused;
  }
...
static void check_options() {
  ...
  if (over_num < 1)
    ErrorExit(ERROR_BADPARM, "At least 1 input overlay is required. \n");
  if ((nb_rad < 0) || (nb_rad > MAX_NB))
    ErrorExit(
        ERROR_BADPARM,
        "Tangential neighborhood radius = %d exceeds the range of [0, %d].\n",
        nb_rad, MAX_NB);
  if ((ic_size < 1) || (ic_size > surf_num))
    ErrorExit(ERROR_BADPARM,
              "Radial smoothing extent = %d exceeds the range of [1,number of "
              "input surfaces = %d].\n",
              ic_size, surf_num);
  if ((ic_start < 0) || (ic_start > (surf_num - ic_size)))
    ErrorExit(ERROR_BADPARM,
              "Radial smoothing starting surface = %d exceeds the range of [0, "
              "(number of input surfaces - radial smoothing extent) = %d].\n",
              ic_start, surf_num - ic_size);

  // no smoothing case, default parameters
  if ((ic_size == 1) && (nb_rad == 0)) {
    fprintf(stderr,
            "Neighborhood radius = %d & radial extent = %d => no smoothing "
            "will be performed.\n",
            nb_rad, ic_size);
    exit(0);
    ...
    if (surf_num != 1)
      ErrorExit(ERROR_BADPARM,
                "Number of input surfaces = %d. Tangential smoothing requires "
                "1 input surface.\n",
                surf_num);
    if (over_num != 1)
      ErrorExit(ERROR_BADPARM,
                "Number of input overlays = %d. Tangential smoothing requires "
                "1 input overlay.\n",
                over_num);
    fprintf(stderr,
            "Neighborhood radius = %d & radial extent = %d => tangential "
            "smoothing will be performed for an input overlay.\n",
            nb_rad, ic_size);
    // intracortical smoothing
  } else if ((ic_size > 1) && (nb_rad >= 0)) {
    if (surf_num != over_num)
      ErrorExit(ERROR_BADPARM,
                "Number of input surfaces = %d has to be the same as number of "
                "input overlays = %d for intracortical smoothing.\n",
                surf_num, over_num);
    fprintf(stderr,
            "Neighborhood radius = %d & radial extent = %d from a starting "
            "surface = %d => intracortical smoothing will be performed.\n",
...
static void print_usage() {
  printf(
      ...
      "--tan-size tansize --rad-size radsize --rad-start radstart]\n",
      Progname);
  printf("\n"
         "  --surf_dir surfdir      : path to the directory with surface "
         "meshes (use mris_extend for creating intermediate surfaces between "
         "white and pial)\n"
         "  --surf_name surfname    : name of a surface file(s) (use * and ? "
         "to pass multiple names, maximum %d)\n"
         "                            multiple surface names have to be sorted "
         "from wm to pial\n"
         "  --overlay_dir overdir   : path to the directory with surface mesh "
         "overlays (use mris_vol2surf to map data onto the surface meshes and "
         "create overlays)\n"
         "  --overlay_name overname : name of an overlay file(s) (use * and ? "
         "to pass multiple names, maximum %d)\n"
         "                            multiple overlays names have to be "
         "sorted from wm to pial\n"
         "                            corresponding surfaces and overlays must "
         "have the same numbers of vertices\n"
         "  --output_dir outdir     : path to the output directory [default: "
         "overdir]\n"
         "  --output_name outname   : name of a output overlay file [default: "
         "based on the name of 1st overlay file]\n"
         "\n"
         "  --tan-size tansize      : tangential extent of the smoothing "
         "kernel [default: 0 = no smoothing, max = %d]\n"
         "                            depending on the weighting scheme (see "
         "--tan-weights):\n"
         "                            gauss: tansize = FWHM of the 2D gaussian "
         "kernel applied in the tangential direction\n"
         "                            distance: tansize = radius of the "
         "neighborhood around the central vertex of the smoothing kernel in "
         "tangential direction (number of vertices) \n"
         "\n"
         "  --rad-size radsize      : radial extent of the intracortical "
         "smoothing kernel (number of adjacent meshes) [default: 1 = no "
         "smoothing, max = number of input surfaces/overlays]\n"
         "  --rad-start radstart    : starting surface mesh of the "
         "intracortical smoothing kernel in the radial direction [default: 0 = "
         "white, max = number of input surfaces/overlays--radsize]\n"
         "\n"
         "  --tan-weights type      : weighting function for tangential "
         "smoothing [default: gauss]\n"
         "                            gauss = gaussian with FWHM = tansize\n"
         "                            distance = 1/tansize\n"
         "  --rad-weights type      : weighting function for radial extent of "
         "the kernel: not yet implemented\n"
         "\n"
         "  --help                  : prints this info\n"
         "\n"
         "Tangential-only smoothing: \n"
         "  - requires exactly 1 input surface mesh with Nv vertices and "
         "exactly 1 corresponding overlay with Nv values\n"
         "  - tansize has to be in range [1, %d]\n"
         "  - one output overlay \n"
         "\n"
         "Intracortical smoothing (radial + tangential): \n"
         "  - requires 2 or more input surfaces spanning from white (=0) to "
         "pial (=n)"
         "  - requires 2 or more input overlays - one for each input surface "
         "with a corresponding number of vertices\n"
         "  - radsize has to be larger than 2 but smaller than or equal to the "
         "number of input surfaces/overlays\n"
         "  - radstart is the index of the starting surface of the "
         "intracortical smoothing kernel, where white (=0) & pial (= total "
         "number of surfaces-1)\n"
         "  - intracortical smoothing kernel will include radsize of adjacent "
         "surfaces starting from radstart surface and moving towards pial "
         "surface\n"
         "  - tansize has to be in range [0, %d]\n"
         "\n"
         "If you use mris_smooth_intracortical in your research, please cite:\n"
         "Blazejewska AI, Fischl B, Wald LL, Polimeni JR, Intracortical "
         "smoothing of small-voxel fMRI data can provide increased detection "
         "power without spatial "
         "resolution losses compared to conventional large-voxel fMRI "
         "data.\nNeuroImage 2019. 189:601-614. DOI: "
