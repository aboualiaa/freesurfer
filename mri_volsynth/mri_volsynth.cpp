...
char *precision = nullptr; /* not used yet */
MRI *mri, *mrism, *mritemp, *mri2;
long seed = -1; /* < 0 for auto */
char *seedfile = nullptr;
...
int main(int argc, char **argv) {
  ...
  if (nctrpoints > 0) {
    ...
    crsctrpoints =
        ControlPoints2Vox(ctrpoints, nctrpoints, CPUseRealRAS, mritemp);
  }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      DoAbs = 1;
    else if (!strcasecmp(option, "--nogmnnorm"))
      gmnnorm = 0;
    else if (!strcasecmp(option, "--rescale"))
      rescale = 1;
    else if (!strcasecmp(option, "--norescale"))
      rescale = 0;
    else if (!strcasecmp(option, "--no-output"))
      NoOutput = 1;
    else if (!strcasecmp(option, "--fft"))
      UseFFT = 1;
    else if (!strcasecmp(option, "--offset"))
      AddOffset = 1;
      ...
      DoCurv = 1;
      nargsused = 2;
      sprintf(tmpstr, "%s/%s/surf/%s.thickness", getenv("SUBJECTS_DIR"),
              subject, hemi);
      ...
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &cras[i]);
      nargsused = 3;
    } else if (!strcmp(option, "--p0")) {
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &p0[i]);
      usep0 = 1;
      nargsused = 3;
...
static void print_usage() {
  ...
  printf("   --curv subject hemi : save output as curv (uses lh.thickness as "
         "template)\n");
  printf("\n");
  printf(" Specify geometry explicitly\n");
  printf("   --dim nc nr ns nf : required without template (overrides "
         "template)\n");
  printf("   --res dc dr ds df : voxel resolution (df is TR, use msec) "
         "(overrides template)\n");
  ...
  printf("   --hsc min max : multiply each frame by a random number bet min "
         "and max\n");
...
static void check_options() {
  ...
  if (DoCurv) {
    sprintf(tmpstr, "%s/%s/surf/%s.orig", getenv("SUBJECTS_DIR"), subject,
            hemi);
    printf("Loading %s\n", tmpstr);
