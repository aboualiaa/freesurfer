...
mris_divide_parcellation subj001 rh aparc.annot 100 rh.aparc.split.100mmsquared,
and saves the result to rh.aparc.split.100mmsquared

This reads in rh.aparac.annot, splits every parcellation until each subdivision
is less than 100 mm^2.
...
int main(int argc, char *argv[]) {
  ...
  *subject, *hemi ;
  ...
  if (MRISreadAnnotation(mris, annot_name) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: could not read annotation %s", Progname,
              annot_name);
  if (mris->ct == nullptr)
  ...
  if (!nunits)
    ErrorExit(ERROR_BADPARM, "%s: could not allocate %d nunits table", Progname,
              mris->ct->nentries);
  ...
  } else // interpret it as a file with parcellation names and # of units
  {
    ...
    if (fp == nullptr)
      ErrorExit(ERROR_NOFILE,
                "%s: could not open parcellation division file %s", Progname,
                argv[4]);
    while ((cp = fgetl(line, STRLEN - 1, fp)) != nullptr) {
      if (sscanf(line, "%s %d", name, &num) != 2)
        ErrorExit(ERROR_BADFILE, "%s: could not parse name/num pair from '%s'",
                  Progname, line);
      CTABfindName(mris->ct, name, &index);
      if (index < 0)
        ErrorExit(ERROR_BADFILE, "%s: could not find name '%s' in color table",
                  Progname, name);
      nunits[index] = num;
    }
  }
...
static void print_usage() {
  printf("\n");
  printf("mris_divide_parcellation [options] subject hemi sourceannot "
         "[splitfile|areathresh] outannot\n");
  printf("\n");
  printf("\n");
  printf("options\n");
  printf("  -help\n");
  printf("  -scale <scale>   specify offset scaling for rgb values "
         "(default=20)\n");
  printf("  -l <label name>  only process the label <label name> (not "
         "implemented yet)\n");
}
...
static void print_help() {
  ...
  printf("perpendicular to the long axis of the label.  The number of "
         "divisions\n");
  printf("can be specified in one of two ways, depending upon the nature of "
         "the\n");
  ...
  printf(
      "the number of units that label should be divided into. The names of\n");
  printf("the labels depends upon the source parcellation.  For aparc.annot "
         "and\n");
  printf("aparc.a2005.annot, the names can be found in\n");
  printf("$FREESURFER_HOME/FreeSurferColorLUT.txt.  For aparc.annot, the "
         "labels\n");
  printf("are between the ranges of 1000-1034.  For aparc.a2005s.annot, the\n");
  printf("labels are between the ranges of 1100-1181.  The name for the label "
         "is\n");
  printf("the name of the segmentation without the 'ctx-lh'. Note that the "
         "name\n");
  ...
  printf("divisions. The first division has the same name as the original "
         "label.\n");
  printf("\n");
  printf("\n");
  printf("EXAMPLE:\n");
  ...
  printf("Run program:\n");
  printf("\n");
  printf("mris_divide_parcellation mysubj001 rh aparc.annot splittable.txt "
         "rh.aparc.split-sfg+pc\n");
  printf("\n");
  printf("This reads in rh.aparc.annot, splits the SFG and PC, and and saves "
         "the\n");
  ...
  printf("superiorfrontal_div2, superiorfrontal_div3, superiorfrontal_div4. "
         "The\n");
  printf(
      "PC divisions will be precentral, precentral_div2, precentral_div3.\n");
  printf("\n");
