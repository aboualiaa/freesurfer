...
static int extract_labeled_image(MRI *mri_src, TRANSFORM *transform, int label,
                                 MRI *mri_dst) {
  MRI *mri_binarized, *mri_tmp;

  mri_binarized = MRIclone(mri_src, nullptr);
  nvoxels += MRIcopyLabel(mri_src, mri_binarized, label);
  MRIbinarize(mri_binarized, mri_binarized, 1, 0, UNIT_VOLUME);
  if (transform)
    mri_tmp = TransformCreateDensityMap(transform, mri_binarized, nullptr);
  else
    mri_tmp = MRIcopy(mri_binarized, nullptr);
