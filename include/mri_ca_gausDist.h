...
public:
  ...
  void init(TypeVectorFloat &vectFloatMeasureMean,
    ...
    for (int i = 0; i < nDim; i++) {
      ...
      TypeVectorFloat vectFloatRow;
      for (int j = 0; j < nDim; j++) {
        vectFloatRow.push_back(floatNumMeasuresEntered *
                               matrixFloatMeasureVariance[i][j]);
        // reconstruct the sum[xy] so we can add new measures to the
        // distribution and recompute the parameters
        // Cov(x,y)=E[xy]-E[x]E[y]=(1/numMeasEntered)*wtdsum[xy]  - E[x] * E[y]
        // thus sum[xy]=numMeasEntered*(Cov(x,y) + E[x] * E[y]);
        vectFloatRow.push_back(floatNumMeasuresEntered *
                               (mean[i] * mean[j] + covariance[i][j]));
      }
      sumXOuterProdX.push_back(vectFloatRow);
    }
  }
  ...
  TypeMatrixFloat covMatrix() {
    ...
    if (!bCovMatrixIsUpToDate) {
      ...
      for (int i = 0; i < nDim; i++) {
        TypeVectorFloat vectFloatRow;
        for (int j = 0; j < nDim; j++) { // Cov[x,y] = E[xy] - E[x]E[y]
          vectFloatRow.push_back(
              (sumXOuterProdX[i][j] / floatNumMeasuresEntered -
               mean[i] * mean[j]));
        }
        covariance.push_back(vectFloatRow);
      }
    ...
    if (bInitialized == false) {
      ...
      if (measure.size() == (unsigned int)nDim) {
        ...
        for (int i = 0; i < nDim; i++) { // add mean to sum of means
          ...
          for (int j = 0; j < nDim;
               j++) { // E[X^2] = wtd sum of Xi^2  / number of measurements
  ...
  float numMeasuresEntered() { return (floatNumMeasuresEntered); }
  ...
  bool write(CConfigFile &configfile) {
    ...
    configfile.write(floatNumMeasuresEntered, strSectionName,
...
istream &operator>>(istream &is, CGaussianDistribution &gausDist) {
  ...
  is.read(&gausDist.floatNumMeasuresEntered,
          sizeof(gausDist.floatNumMeasuresEntered));

  // Dont read or write this because it wastes space, instead assume that the
  // input gausDist has the appropriate size is.read(&gausDist.nDim,
  // sizeof(gausDist.nDim));
  ...
  for (int i = 0; i < gausDist.nDim; i++) {
    gausDist.sumX.push_back(gausDist.floatNumMeasuresEntered *
                            gausDist.mean[i]);

    TypeVectorFloat vectFloatRow;
    for (int j = 0; j < gausDist.nDim; j++) {
      // reconstruct the sum[xy] so we can add new measures to the distribution
      // and recompute the parameters
      // Cov(x,y)=E[xy]-E[x]E[y]=(1/numMeasEntered)*sum[xy]  - E[x] * E[y]
      // thus sum[xy]=numMeasEntered*(Cov(x,y) + E[x] * E[y]);
      vectFloatRow.push_back(
          gausDist.floatNumMeasuresEntered *
          (gausDist.mean[i] * gausDist.mean[j] + gausDist.covariance[i][j]));
    }
...
ostream &operator<<(ostream &os, CGaussianDistribution &gausDist) {
  ...
  os.write(&gausDist.floatNumMeasuresEntered,
           sizeof(gausDist.floatNumMeasuresEntered));
  // Dont write this out bc it wastes space since it is the same for every gaus
  // dist in the sparse 2D matrix in which this class is contained
