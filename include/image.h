...
IMAGE *ImageDifferentialScale(IMAGE *inImage, IMAGE *outImage, int outRows,
                              int outCols);
int    ImageDifferentialScaleUp(IMAGE *inImage, IMAGE *outImage, int outRows,
                                int outCols);
int    ImageDifferentialScaleDown(IMAGE *inImage, IMAGE *outImage, int outRows,
                                  int outCols);
...
IMAGE *  ImageCopyArea(IMAGE *Isrc, IMAGE *Idst, int srow, int scol, int drow,
                       int dcol, int rows, int cols);
...
int    ImageCheckSize(IMAGE *inImage, IMAGE *outImage, int rows, int cols,
                      int nframes);
...
int    ImageAdjustOffsetLen(IMAGE *inImage, IMAGE *outImage, int wsize);
IMAGE *ImageCalculateNitShiOffset(IMAGE *Ix, IMAGE *Iy, int wsize, float mu,
                                  float c, IMAGE *offsetImage);
IMAGE *ImageCalculateOffsetDirection(IMAGE *Ix, IMAGE *Iy, int wsize,
                                     IMAGE *offsetImage);
IMAGE *ImageOffsetDirectionMap(IMAGE *Ix, IMAGE *Iy, int wsize, IMAGE *Iorient,
...
IMAGE *ImageFilterMinMax(IMAGE *Imin, IMAGE *Imax, IMAGE *Ioffset, IMAGE *Idir,
                         IMAGE *Idst);

/* various filters */
int    ImageMedianFilter(IMAGE *inImage, int wsize, IMAGE *offsetImage,
                         IMAGE *outImage);
IMAGE *ImageSigmaFilter(IMAGE *Isrc, int wsize, float nsigma, IMAGE *Ioffset,
                        IMAGE *Idst);
int    ImageBuildExponentialFilter(IMAGE *inImage, int wsize, float k,
                                   IMAGE *offsetImage, IMAGE *filterSequnce);
int ImageExponentialFilter(IMAGE *inImage, IMAGE *gradImage, int wsize, float k,
...
int ImageSobel(IMAGE *inImage, IMAGE *gradImage, IMAGE *xImage, IMAGE *yImage);
...
void   ImageCircularConvolve1d(IMAGE *imageI, IMAGE *J, float k[], int len,
                               int axis);
void   ImageConvolve1d(IMAGE *imageI, IMAGE *J, float k[], int len, int axis);
void ImageConvolve1dByte(IMAGE *imageI, IMAGE *J, float k[], int len, int axis);
...
void   ImageCircularConvolve1d(IMAGE *imageI, IMAGE *J, float k[], int len,
                               int axis);
IMAGE *ImageConvolveGaussianByte(IMAGE *inImage, IMAGE *gImage, IMAGE *outImage,
                                 int dst_frameno);
IMAGE *ImageConvolveGaussian(IMAGE *inImage, IMAGE *gImage, IMAGE *outImage,
                             int dst_frameno);
...
IMAGE *ImageExtractInto(IMAGE *inImage, IMAGE *outImage, int x0, int y0, int dx,
                        int dy, int xdst, int ydst);
IMAGE *ImageExtract(IMAGE *inImage, IMAGE *outImage, int x0, int y0, int dx,
                    int dy);
...
IMAGE *ImageCorrelateRegion(IMAGE *Isrc, IMAGE *Ikernel, IMAGE *Idst, int row0,
                            int col0, int wsize);
...
// hips functions
int init_header(IMAGE *I, const char *onm, const char *snm, int nfr,
                const char *odt, int rw, int cl, int pfmt, int nc,
                const char *desc);
...
#define ImageClone(I)                                                          \
  ImageAlloc(I->rows, I->cols, I->pixel_format, I->num_frame)
#define ImageClear(I) ImageClearArea(I, 0, 0, I->rows, I->cols, 0.0f, -1)

/* IMAGEpix is for byte size images */
#define IMAGEpix(im, x, y) ((im->image) + (((long)y) * im->ocols) + (x))

#define IMAGEIpix(im, x, y)                                                    \
  (((unsigned int *)im->image) + (((long)y) * im->ocols) + (x))
#define IMAGEFpix(im, x, y)                                                    \
  (((float *)im->image) + (((long)y) * im->ocols) + (x))
#define IMAGESpix(im, x, y)                                                    \
  (((short *)im->image) + (((long)y) * im->ocols) + (x))

#define IMAGEDpix(im, x, y)                                                    \
  (((double *)im->image) + (((long)y) * im->ocols) + (x))
/* for complex images */
#define IMAGECpix(im, x, y)                                                    \
  (((CPIX *)im->image) + ((((long)y) * im->ocols)) + (x))

/* for double complex images */
#define IMAGEDCpix(im, x, y)                                                   \
  (((DCPIX *)im->image) + ((((long)y) * im->ocols)) + (x))

/* IMAGEpix is for byte size images */
#define IMAGEseq(im, n) ((im->image) + (((long)n) * im->ocols * im->orows))

/* for float images */
#define IMAGEFseq(im, n)                                                       \
  ((float *)((im)->image) + (((long)n) * (im)->ocols * (im)->orows))

/* for double images */
#define IMAGEDseq(im, n)                                                       \
  ((double *)((im)->image) + (((long)n) * (im)->ocols * (im)->orows))

/* for integer images */
#define IMAGEIseq(im, n)                                                       \
  ((int *)((im)->image) + ((n) * (im)->ocols * (im)->orows))
/* for complex images */
#define IMAGECseq(im, n)                                                       \
  ((float *)(im->image) + (((long)n) * 2 * im->ocols * im->orows))

// RGB images are 3 bytes per pixel
#define IMAGERGBpix(im, x, y)                                                  \
  ((im->image) + (((int)y) * im->ocols * 3) + (x * 3))

#define IMAGEDseq_pix(im, x, y, n)                                             \
  ((IMAGEDseq(im, n)) + (((long)y) * im->ocols) + (x))
#define IMAGEFseq_pix(im, x, y, n)                                             \
  ((IMAGEFseq(im, n)) + (((long)y) * im->ocols) + (x))
#define IMAGEseq_pix(im, x, y, n)                                              \
  ((IMAGEseq(im, n)) + (((long)y) * im->ocols) + (x))
#define IMAGEIseq_pix(im, x, y, n)                                             \
  ((IMAGEIseq(im, n)) + (((long)y) * im->ocols) + (x))

#define COMPLEX_IMAGE(i)                                                       \
  (((i)->pixel_format == PFCOMPLEX) || ((i)->pixel_format == PFDBLCOM))
typedef struct {
  float real;
  float imag;
} CPIX;
