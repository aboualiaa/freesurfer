...
Trns_tErr Trns_DeepClone(mriTransformRef ThisKRT, mriTransformRef *opTransform);
...
Trns_tErr Trns_CopyAtoRAS(mriTransformRef ThisKRT, MATRIX *iAtoRAS);
Trns_tErr Trns_CopyBtoRAS(mriTransformRef ThisKRT, MATRIX *iBtoRAS);
Trns_tErr Trns_CopyARAStoBRAS(mriTransformRef ThisKRT, MATRIX *iARAStoBRAS);
...
/* Alternatively, use ThisKRT if you only have one matrix to use. It will
   ...
   matrices. */
Trns_tErr Trns_CopyAtoB(mriTransformRef ThisKRT, MATRIX *iAtoB);

/* access internal matrices */
Trns_tErr Trns_GetAtoRAS(mriTransformRef ThisKRT, MATRIX **opMatrix);
Trns_tErr Trns_GetType(mriTransformRef ThisKRT, int *ptype);
Trns_tErr Trns_GetBtoRAS(mriTransformRef ThisKRT, MATRIX **opMatrix);
Trns_tErr Trns_GetARAStoBRAS(mriTransformRef ThisKRT, MATRIX **opMatrix);
Trns_tErr Trns_GetAtoB(mriTransformRef ThisKRT, MATRIX **opMatrix);
Trns_tErr Trns_GetBtoA(mriTransformRef ThisKRT, MATRIX **opMatrix);

/* apply a transformation to the ARAStoBRAS matrix */
Trns_tErr Trns_ApplyTransform(mriTransformRef ThisKRT, MATRIX *iTransform);
...
/* */
Trns_tErr Trns_Translate(mriTransformRef ThisKRT, float ifAmount, tAxis iAxis);
Trns_tErr Trns_Rotate(mriTransformRef ThisKRT, float ifDegrees, tAxis iAxis);
Trns_tErr Trns_Scale(mriTransformRef ThisKRT, float ifFactor, tAxis iAxis);

/* converts from a voxel in A space to one in B space, i.e.
   A -> A_RAS, A_RAS -> B_RAS, B_RAS -> B */
Trns_tErr Trns_ConvertAtoB(mriTransformRef ThisKRT, xVoxelRef iAVoxel,
                           xVoxelRef oBVoxel);
Trns_tErr Trns_ConvertAtoRAS(mriTransformRef ThisKRT, xVoxelRef iAVoxel,
                             xVoxelRef oRASVoxel);
Trns_tErr Trns_ConvertBtoA(mriTransformRef ThisKRT, xVoxelRef iBVoxel,
                           xVoxelRef oAVoxel);
Trns_tErr Trns_ConvertBtoRAS(mriTransformRef ThisKRT, xVoxelRef iBVoxel,
                             xVoxelRef oRASVoxel);
Trns_tErr Trns_ConvertBRAStoB(mriTransformRef ThisKRT, xVoxelRef iBRASVoxel,
                              xVoxelRef oBVoxel);

/* converts matricies between a and b */
Trns_tErr Trns_ConvertMatrixAtoB(mriTransformRef ThisKRT, MATRIX *iAMatrix,
                                 MATRIX *oBMatrix);
Trns_tErr Trns_ConvertMatrixAtoRAS(mriTransformRef ThisKRT, MATRIX *iAMatrix,
                                   MATRIX *oRASMatrix);
Trns_tErr Trns_ConvertMatrixBtoA(mriTransformRef ThisKRT, MATRIX *iBMatrix,
                                 MATRIX *oAMatrix);
Trns_tErr Trns_ConvertMatrixBtoRAS(mriTransformRef ThisKRT, MATRIX *iBMatrix,
                                   MATRIX *oRASMatrix);
