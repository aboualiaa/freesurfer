...
SV *      StatReadTalairachVolume(const char *prefix, const char *xform_fname,
                                  const char *subject_name);
fMRI_REG *StatReadRegistration(const char *fname);
fMRI_REG *StatReadTalairachRegistration(const char *fname, char *subject_name);
...
int       StatFree(SV **psv);
SV *StatAllocVolume(SV *sv, int nevents, int width, int height, int nslices,
                    int time_points, int which);
