/**
 * @file  fnv_hash.c
 * @brief A hash function that can be used to replace getting a sequence of
 * random numbers
 *
 */
/*
...
static unsigned long fnv_add(unsigned long hash, const unsigned char *p,
                             unsigned long size) {
  unsigned long c;
  for (c = 0; c < size; c++) {
    hash ^= p[c];
...
static float fnv_hash(int i, int k, int *random_counter, double LO, double HI) {
  ...
  unsigned int combined[3];
  combined[0] = i;
  combined[1] = k;
  combined[2] = (*random_counter)++;

  unsigned long hash =
      fnv_add(fnv_init(), (unsigned char *)&combined[0], sizeof(combined));
