 ...
 * @brief Given a MRIS surface, generate a distance field where the value at a
 * voxel is the distance between the voxel and the closest point on the surface
 *
 ...
 * and X, Y, Z number of voxels in x, y and z direction the complexity of the
 * bruce force approach is O(N*X*Y*Z). This algorithm differs in the sense it
 * calculates distances for voxels only within a bounding box of each face.
 * Hence its complexity is around O(N*k) where k is the size of the bounding
 * box. ( distance factor ) Thus, the distance factor is critical to how quickly
 * the algorithm executes. If you are only interested in the transition region
 * make sure the distance factor you give is the minimum needed.
 */
...
public:
  ...
  void SetMaxDistance(double dist) { this->max_distance = dist; }

  //! Set if a signed field is desired ( NOT IMPLEMENTED )
  void SetSignedField() { this->signedfield = true; }
  ...
  void Generate() {
    ...
    for (int fno = 0; fno < mris->nfaces; fno++) {
      ...
      if (bbox.minc[2] < 0)
        bbox.minc[0] = 0;
      if (bbox.maxc[0] >= mri_distfield->width)
        bbox.maxc[0] = mri_distfield->width - 1;
      if (bbox.maxc[1] >= mri_distfield->height)
        bbox.maxc[1] = mri_distfield->height - 1;
      if (bbox.maxc[2] >= mri_distfield->depth)
        bbox.maxc[2] = mri_distfield->depth - 1;
