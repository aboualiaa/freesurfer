...
template <int sign = +1> class FastMarching {
  ...
  using CoordHeap =
      std::priority_queue<stCoord, std::vector<stCoord>, HeapCompare>;
...
public:
  ...
  void Run(float _limit) {
    ...
    while (!trial_heap.empty()) {
      ...
      // On enleve le point de la pile et on l'ajoute a la liste des points
      // alive One removes the point of the pile and one adds it to the alive
      // list of the points
      if (MRIvox(status, pt.x, pt.y, pt.z) == eForbidden)
        DiagBreak();
      trial_heap.pop();
  ...
  void InitFromMRI(MRI *_mri, const int label) {
    ...
    if (sign > 0) {
      ...
      mapMRI_XYZ(_mri, x, y, z) {
        ...
        const int voxel =
            static_cast<int>(round(MRIgetVoxVal(_mri, x, y, z, 0)));
