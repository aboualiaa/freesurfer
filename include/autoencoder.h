...
typedef struct _AE {
  ...
  VECTOR *v_input; // input vector for this layer (unless previous is null, same
                   // as output of prev. layer
  VECTOR *v_output; // output vector (same as input of next layer unless next is
                    // null)
  ...
  struct _SAE *sae;
  double       noise_fraction; // what fraction of inputs to zero out instead of
                               // propagating forward
  int *   zero_indices;
  double *saved_inputs;   // to restore for training
  double *average_act;    // exponentially decaying average activation: p_t =
                          // p_t-1*.999 + .001*act
...
typedef struct {
  ...
  SAE *sae;
  MRI *mri_outputs[MAX_AE_LAYERS]; // outputs of the [ith] layer. Each frame is
                                   // a "group"
...
typedef struct {
  ...
  int     max_no_progress;
  double  sparsity_decay; // l in active = l * previous_active_pval + (1-l) *
                          // current_active_pval
...
void    AEdump(AE *ae);
VECTOR *SAEfillInputVector(MRI **mri, int nlevels, int x, int y, int z,
...
int    CSAEfillInputse(CSAE *csae, MRI *mri_inputs, VECTOR *v_visible, int x0,
                       int y0, int z0, int ksize);
...
double CSAEcomputeVoxlistRMS(CSAE *csae, SAE_INTEGRATION_PARMS *parms,
                             ...
                             int start_index, int num_indices, int *always,
                             int *never);
