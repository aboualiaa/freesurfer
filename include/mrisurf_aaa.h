...
#define checkOrigXYZCompatible(S1, S2)                                         \
  checkOrigXYZCompatibleWkr((S1), (S2), __FILE__, __LINE__);

struct _mht;
using MHT = struct MRIS_HASH_TABLE;

typedef struct LABEL_VERTEX LABEL_VERTEX, LV;
using LABEL = struct LABEL;
typedef struct face_type_ face_type, FACE;
using FACE_TOPOLOGY = struct face_topology_type_;
typedef struct vertex_type_ vertex_type, VERTEX;
using VERTEX_TOPOLOGY = struct VERTEX_TOPOLOGY;
...
static int cmpAnglesPerTriangle(angles_per_triangle_t const &dst,
                                angles_per_triangle_t const &src) {
...
// but can get some of its properties from an underlying Surface so it doesn't
// have to implement or copy ones that are rarely used.
//
// It is defined in mrisurf_MRIS_MPPropertiesInVectors.h with some additional
// macros in mrisurf_mp.h
//
// It is optimized to cope with the XYZ changing as the shape is mutated to
// optimize some SSE. Its representation keeps the data in a format that fills
// cache lines with immediately needed information.
//
using MRIS_MP = struct MRIS_MP;
...
// virtual functions or C++ templates to implement these functions on top of
// both MRIS and MRIS_MP
//
...
static MRISBaseConst MRISBaseToConst(MRISBase src) {
  return MRISBaseConstCtr(src.mris_mp, src.mris);
}
