...
#define MRIS_MP__LIST_MRIS_IN                                                  \
  ELT(const, MRIS_Status, status)                                              \
  SEP ELT(const, MRIS_Status, origxyz_status) SEP ELT(const, int, nvertices)   \
      SEP                                                                      \
...
// In out
#define MRIS_MP__LIST_MRIS_IN_OUT ELT(, char, dist_nsize)

// Out
#define MRIS_MP__LIST_MRIS_OUT                                                 \
  ELT(, float, xlo)                                                            \
  SEP ELT(, float, xhi) SEP ELT(, float, ylo) SEP ELT(, float, yhi) SEP ELT(   \
      , float, zlo) SEP                                                        \
  ELT(, float, zhi) SEP                                                        \
  ELT(, float, xctr) SEP                                                       \
  ELT(, float, yctr) SEP                                                       \
...
#define MRIS_MP__LIST_V_IN_OUT_XYZ                                             \
  ELT(, float, x) SEP ELT(, float, y) SEP ELT(, float, z)

#define MRIS_MP__LIST_V_IN_OUT_NOXYZ                                           \
  ELTX(, int, dist_capacity)                                                   \
  SEP ELT(, char, border) SEP ELT(, float, cx) SEP ELT(, float, cy) SEP ELT(   \
      , float, cz) SEP                                                         \
  ELT(, float, curv) SEP                                                       \
...
#define MRIS_MP__LIST_V_OUT                                                    \
  ELT(, float, area)                                                           \
  SEP ELT(, float, nx) SEP ELT(, float, ny) SEP ELT(, float, nz) SEP ELTX(     \
      , char, neg) SEP                                                         \
  ELTX(, float *, dist)
...
#define MRIS_MP__LIST_F_IN                                                     \
  ELT(const, char, ripflag)                                                    \
  SEP ELTX(const, float, norm_orig_area) SEP ELTX(                             \
      const, angles_per_triangle_t, orig_angle)

#define MRIS_MP__LIST_F_OUT                                                    \
  ELT(, float, area)                                                           \
  SEP ELTX(, char, normSet) SEP ELTX(, FloatXYZ, norm)                         \
      SEP ELTX(, angles_per_triangle_t, angle)
...
void MRISMP_load(
    ...
    float *dx_or_NULL = nullptr, // loaded if not nullptr. The dx,dy,dz for
                                 // ripped vertices are set to zero
    float *dy_or_NULL = nullptr, float *dz_or_NULL = nullptr);
