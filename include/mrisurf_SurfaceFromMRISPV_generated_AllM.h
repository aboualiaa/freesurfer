...
struct Face : public Repr_Elt {
  using Surface = AllM::Surface;
  using Vertex = AllM::Vertex;
  inline Face();
...
struct Vertex : public Repr_Elt {
  ...
  // ordering is no use
  inline Face f(size_t i) const; // size() is num.    array[v->num] the fno's of
                                 // the neighboring faces
  inline size_t n(size_t i) const; // size() is num.    array[v->num] the
  ...
  inline float dist(size_t i) const; // size() is vtotal.    distance to
  ...
  inline int
  dist_orig_capacity() const; //  -- should contain at least vtx_vtotal elements
  //
  ...
  //
  inline float
  fieldsign() const;           //  fieldsign--final: -1, "0", "1" (file: rh.fs)
  inline float fsmask() const; //  significance mask (file: rh.fm)
  inline float d() const;      //  for distance calculations
  inline int
  annotation() const; //  area label (defunct--now from label file name!)
  inline char oripflag() const;
  ...
  inline void set_f(size_t i, Face to);   // size() is num.    array[v->num] the
                                          // fno's of the neighboring faces
  inline void set_n(size_t i, size_t to); // size() is num.    array[v->num] the
                                          // face.v[*] index for this vertex
  inline void set_e(size_t i, int to); //  edge state for neighboring vertices
  inline void set_v(size_t i,
                    Vertex to); // size() is vtotal.    array[v->vtotal or more]
                                // of vno, head sorted by hops
  inline void set_vnum(
  ...
  inline void
  set_vtotal(short to); //  total # of neighbors. copy of vnum.nsizeCur
  inline void
  set_nsizeMaxClock(short to); //  copy of mris->nsizeMaxClock when v#num
  inline void
  set_nsizeMax(uchar to); //  the max nsize that was used to fill in vnum etc
  ...
  inline void
  set_e1z(float to); //  1st basis vector for the local tangent plane
  inline void set_e2x(float to);
  inline void set_e2y(float to);
  inline void
  set_e2z(float to); //  2nd basis vector for the local tangent plane
  inline void set_pe1x(float to);
  inline void set_pe1y(float to);
  inline void
  set_pe1z(float to); //  1st basis vector for the local tangent plane
  inline void set_pe2x(float to);
  inline void set_pe2y(float to);
  inline void
  set_pe2z(float to);           //  2nd basis vector for the local tangent plane
  ...
  //
  inline void
  set_fieldsign(float to); //  fieldsign--final: -1, "0", "1" (file: rh.fs)
  inline void set_fsmask(float to); //  significance mask (file: rh.fm)
  inline void set_d(float to);      //  for distance calculations
  inline void
  set_annotation(int to); //  area label (defunct--now from label file name!)
  inline void set_oripflag(char to);
...
struct Surface : public Repr_Elt {
  ...
  inline int nfaces() const;    //  # of faces on surface, change by calling
                                //  MRISreallocVerticesAndFaces et al
  inline bool faceAttachmentDeferred()
  ...
  inline p_p_void
  dist_storage() const; //  the malloced/realloced vertex dist fields, so those
                        //  fields can be quickly nulled and restored
  inline p_p_void dist_orig_storage()
      const; //  the malloced/realloced vertex dist_orig fields, so those fields
             //  can be quickly nulled and restored
  inline int tempsAssigned() const; //  State of various temp fields that can be
                                    //  borrowed if not already in use
  ...
  inline int max_vertices() const; //  may be bigger than nvertices, set by
  ...
  inline void
  set_avg_vertex_dist(double to); //  set by MRIScomputeAvgInterVertexDist
  inline void set_std_vertex_dist(double to);
  inline void set_orig_area(float to);
  inline void set_neg_area(float to);
  inline void
  set_neg_orig_area(float to); //  amount of original surface in folds
  ...
  inline void
  set_status(MRIS_Status to); //  type of surface (e.g. sphere, plane)
  inline void
  set_origxyz_status(MRIS_Status to); //  type of surface (e.g. sphere, plane)
                                      //  that this origxyz were obtained from
