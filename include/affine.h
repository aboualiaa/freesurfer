...
#if (__GNUC__ > 3) &&                                                          \
    !defined(HAVE_MCHECK) // mcheck does not understand _mm_alloc et. al.
...
inline static void SetAffineVector(AffineVector *av, const float x,
...
inline static void GetAffineVector(const AffineVector *av, float *x, float *y,
                                   float *z) {
  *x = av->vec[0];
  *y = av->vec[1];
  *z = av->vec[2];
...
inline static void GetFloorAffineVector(const AffineVector *av, int *x, int *y,
                                        int *z) {
...
inline static void SetAffineMatrix(AffineMatrix *am, const MATRIX *src) {
...
inline static void GetAffineMatrix(MATRIX *dst, const AffineMatrix *am) {
  unsigned int i, j;
...
inline static void AffineMatrixFree(AffineMatrix **am) {
  if (*am != nullptr) {
    free(*am);
    *am = nullptr;
  }
...
inline static void AffineMatrixAlloc(AffineMatrix **am) {

  void *tmp = nullptr; // gcc complains otherwise
...
inline static void AffineMV(AffineVector *const y, const AffineMatrix *const A,
...
inline static void AffineMM(AffineMatrix *const C, const AffineMatrix *const A,
  ...
  for (j = 0; j < kAffineVectorSize; j++) {
    for (k = 0; k < kAffineVectorSize; k++) {
      for (i = 0; i < kAffineVectorSize; i++) {
        Cmat[i + (j * kAffineVectorSize)] += Amat[i + (k * kAffineVectorSize)] *
                                             Bmat[k + (j * kAffineVectorSize)];
      }
    }
  }
#endif
