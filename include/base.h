...
#define cheapAssert(TST)                                                       \
  {                                                                            \
    if (!(TST))                                                                \
      assertFailed(__FILE__, __LINE__, #TST);                                  \
  }
#define costlyAssert(                                                          \
    TST) //    { if (!(TST)) assertFailed(__FILE__, __LINE__, #TST); }

#define cheapAssertValidFno(_MRIS, _FNO)                                       \
  cheapAssert((0 <= _FNO) && (_FNO < _MRIS->nfaces))
#define cheapAssertValidVno(_MRIS, _VNO)                                       \
  cheapAssert((0 <= _VNO) && (_VNO < _MRIS->nvertices))
...
LogicProblemResponse copeWithLogicProblem2(
    ...
    const char *file,
    int line, // multiple reports at the same location are partially elided
    const char *function);
#define copeWithLogicProblem(ENV, MSG)                                         \
  copeWithLogicProblem2(NULL, (ENV), (MSG), __FILE__, __LINE__, __MYFUNCTION__)

// Regardless of whether the __real_malloc etc. or the __wrap_ ones, it is still
// desirable to know where in the program the allocations are happening.  This
...
#define realloc(PTR, SIZE)                                                     \
  reallocHere((PTR), (SIZE), __FILE__, __MYFUNCTION__, __LINE__)
#define posix_memalign(MEMPTR, ALIGN, SIZE)                                    \
  posix_memalignHere((MEMPTR), (ALIGN), (SIZE), __FILE__, __MYFUNCTION__,      \
                     __LINE__)
...
using ptr_to_const_float = const float *;
