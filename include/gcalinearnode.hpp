...
public:
  GCAlinearNode()
      : hasGibbsNeighbourhood(true), xDim(0), yDim(0), zDim(0), n4D(0), n5D(0),
        n6D(0), gc1dNeighbourDim(GIBBS_NEIGHBORHOOD), offsets4D(), offsets5D(),
        offsets6D(), nodeMaxLabels(), nodeTotalTraining(), nodeLabels(),
        means(), variances(), nJustPriors(), nTraining(), regularised(),
        gc1dDirecLabelPriors(), gc1dDirecLabels(){};
  ...
  inline int totalTraining(const int ix, const int iy, const int iz) const {
    const size_t idx = this->index3D(ix, iy, iz);
  ...
  inline int &totalTraining(const int ix, const int iy, const int iz) {
    const size_t idx = this->index3D(ix, iy, iz);
  ...
  inline int gc1dCount(const int ix, const int iy, const int iz) const {
    /*!
  ...
  inline unsigned short labelsAtNode(const int ix, const int iy, const int iz,
  ...
  //! Accessor for means
  inline float meansAtNodeGC1D(const int ix, const int iy, const int iz,
                               const int iGC1D) const {
    const size_t idx = this->index4D(ix, iy, iz, iGC1D);
  ...
  inline float variancesAtNodeGC1D(const int ix, const int iy, const int iz,
                                   const int iGC1D) const {
    const size_t idx = this->index4D(ix, iy, iz, iGC1D);
  ...
  inline short nJustPriorsAtNodeGC1D(const int ix, const int iy, const int iz,
                                     const int iGC1D) const {
    const size_t idx = this->index4D(ix, iy, iz, iGC1D);
  ...
  inline int nTrainingAtNodeGC1D(const int ix, const int iy, const int iz,
                                 const int iGC1D) const {
    const size_t idx = this->index4D(ix, iy, iz, iGC1D);
  ...
  inline char regularisedAtNodeGC1D(const int ix, const int iy, const int iz,
                                    const int iGC1D) const {
    const size_t idx = this->index4D(ix, iy, iz, iGC1D);
  ...
  inline int nLabelsAtNodeGC1Ddirection(const int ix, const int iy,
                                        const int iz, const int iGC1D,
  ...
  inline float labelPriorsAtNodeGC1Ddirection(const int ix, const int iy,
                                              const int iz, const int iGC1D,
                                              const int iDirec,
  ...
  inline unsigned short labelsAtNodeGC1Ddirection(const int ix, const int iy,
                                                  const int iz, const int iGC1D,
                                                  const int iDirec,
                                                  const int iLabel) const {
    const size_t idx = this->index6D(ix, iy, iz, iGC1D, iDirec, iLabel);
...
private:
  ...
  const_GCAnode GetConstNode(const int ix, const int iy, const int iz) const;
  ...
  inline size_t index3D(const int ix, const int iy, const int iz) const {
    if ((ix < 0) || (ix >= this->xDim) || (iy < 0) || (iy >= this->yDim) ||
        (iz < 0) || (iz >= this->zDim)) {
      std::cerr << __FUNCTION__ << ": Index out of range" << std::endl;
      abort();
    }
  ...
  inline size_t index4D(const int ix, const int iy, const int iz,
                        const int iGC1D) const {
    if ((ix < 0) || (ix >= this->xDim) || (iy < 0) || (iy >= this->yDim) ||
        (iz < 0) || (iz >= this->zDim)) {
      std::cerr << __FUNCTION__ << ": Index out of range" << std::endl;
    ...
    if ((iGC1D < 0) || (iGC1D >= (nextOffset - currOffset))) {
      std::cerr << __FUNCTION__ << ": iGC1D out of range" << std::endl;
      abort();
    }
  ...
  inline size_t index5D(const int ix, const int iy, const int iz,
                        const int iGC1D, const int iDirec) const {
    /*!
    ...
    */
    ...
    if ((iDirec < 0) || (iDirec >= (nextOffset - currOffset))) {
      std::cerr << __FUNCTION__ << ": iDirec out of range" << std::endl;
      abort();
    }
  ...
  inline size_t index6D(const int ix, const int iy, const int iz,
                        const int iGC1D, const int iDirec,
                        const int iLabel) const {
    ...
    if ((iGC1D < 0) || (iGC1D >= (nextgc1DOffset - currgc1DOffset))) {
      std::cerr << __FUNCTION__ << ": iGC1D outof range" << std::endl;
      abort();
    }
    ...
    if ((iLabel < 0) || (iLabel >= (nextOffset - currOffset))) {
      std::cerr << __FUNCTION__ << ": iLabel out of range" << std::endl;
      abort();
    }
...
public:
  ...
  const_GCAnode(const int _ix, const int _iy, const int _iz,
                const GCAlinearNode &src)
      : gcaln(src), idx3d(src.index3D(_ix, _iy, _iz)),
        offset4d(src.offsets4D.at(idx3d)),
        myGC1Dcount(src.offsets4D.at(idx3d + 1) - offset4d), ix(_ix), iy(_iy),
        iz(_iz) {}

  // --------------------

  //! Accessor for max_labels
  inline int maxLabels() const {
    return (this->gcaln.nodeMaxLabels.at(this->idx3d));
  }

  //! Accessor for totalTraining
  inline int totalTraining() const {
    return (this->gcaln.nodeTotalTraining.at(this->idx3d));
  }

  //! Accessor for gc1dCount
  inline int gc1dCount() const { return (this->myGC1Dcount); }
...
class const_GCAnode_GC1D {
public:
  const_GCAnode_GC1D(const int _ix, const int _iy, const int _iz,
                     const int _iGC1D, const GCAlinearNode &src)
      : gcaln(src), offset4d(src.index4D(_ix, _iy, _iz, _iGC1D)) {}

  // -----------------------------
  // 4D data

  //! Accessor for the mean
  inline float mean() const { return (this->gcaln.means.at(this->offset4d)); }
...
private:
  ...
  inline size_t index6D(const int iDirec, const int iLabel) const {
