...
struct MRISPV {
  ...
  pSeveralInt *v_v;   // size() is vtotal.    array[v->vtotal or more] of vno,
                      // head sorted by hops
  ...
  pSeveralFloat *v_dist;      // size() is vtotal.    distance to neighboring
                              // vertices based on  xyz
  pSeveralFloat *v_dist_orig; // size() is vtotal.    distance to neighboring
  ...
  char *  v_border;  //  flag
  char *  v_ripflag; //  vertex no longer exists - placed last to load the next
                     //  vertex into cache
