...
using Representation = MRIS;

namespace Existence {
Face::Face() {}
Face::Face(Representation *representation, size_t idx)
    : Repr_Elt(representation, idx) {}
...
char Face::ripflag() const { return repr->faces[idx].ripflag; }
char Face::oripflag() const { return repr->faces[idx].oripflag; }
int Face::marked() const { return repr->faces[idx].marked; }

void Face::set_ripflag(char to) { repr->faces[idx].ripflag = to; }
void Face::set_oripflag(char to) { repr->faces[idx].oripflag = to; }
void Face::set_marked(int to) { repr->faces[idx].marked = to; }

Vertex::Vertex() {}
Vertex::Vertex(Representation *representation, size_t idx)
    : Repr_Elt(representation, idx) {}
...
char Vertex::ripflag() const { //  vertex no longer exists - placed last to load
...
void Vertex::set_ripflag(char to) { //  vertex no longer exists - placed last to
                                    //  load the next vertex into cache
  repr->vertices[idx].ripflag = to;
}

Surface::Surface() {}
Surface::Surface(Representation *representation)
    : Repr_Elt(representation, 0) {}
...
Surface::Surface(XYZPositionConsequencesM::Surface const &src)
    : Repr_Elt(src) {}
...
float Surface::b() const { return repr->b; }
...
MRIS_Status
Surface::origxyz_status() const { //  type of surface (e.g. sphere, plane) that
                                  //  this origxyz were obtained from
  return repr->origxyz_status;
}
int Surface::patch() const { //  if a patch of the surface
  return repr->patch;
}
int Surface::max_vertices() const { //  may be bigger than nvertices, set by
                                    //  calling MRISreallocVerticesAndFaces
  return repr->max_vertices;
}
int Surface::max_faces() const { //  may be bigger than nfaces,    set by
                                 //  calling MRISreallocVerticesAndFaces
...
VOL_GEOM Surface::vg() const { //  volume info from which this surface is
...
int Surface::group_avg_vtxarea_loaded()
...
p_void
Surface::user_parms() const { //  for whatever the user wants to hang here
...
p_void Surface::mht() const { return repr->mht; }
p_void Surface::temps() const { return repr->temps; }
...
Vertex Face::v(size_t i) const { return Vertex(repr, repr->faces[idx].v[i]); }
char Face::ripflag() const { return repr->faces[idx].ripflag; }
char Face::oripflag() const { return repr->faces[idx].oripflag; }
int Face::marked() const { return repr->faces[idx].marked; }
...
Face Vertex::f(size_t i) const { // size() is num.    array[v->num] the fno's of
                                 // the neighboring faces
  return Face(repr, repr->vertices_topology[idx].f[i]);
}
size_t Vertex::n(size_t i) const { // size() is num.    array[v->num] the
                                   // face.v[*] index for this vertex
...
Vertex Vertex::v(size_t i) const { // size() is vtotal.    array[v->vtotal or
                                   // more] of vno, head sorted by hops
  return Vertex(repr, repr->vertices_topology[idx].v[i]);
}
short Vertex::vnum()
    const { //  number of 1-hop neighbors    should use [p]VERTEXvnum(i)
...
uchar Vertex::nsizeMax()
    const { //  the max nsize that was used to fill in vnum etc
...
void Vertex::set_f(size_t i, Face to) { // size() is num.    array[v->num] the
                                        // fno's of the neighboring faces
  cheapAssert(repr == to.repr);
  repr->vertices_topology[idx].f[i] = to.idx;
}
void Vertex::set_n(size_t i, size_t to) { // size() is num.    array[v->num] the
                                          // face.v[*] index for this vertex
...
int Surface::nvertices() const { //  # of vertices on surface, change by calling
                                 //  MRISreallocVerticesAndFaces et al
  return repr->nvertices;
}
int Surface::nfaces() const { //  # of faces on surface, change by calling
                              //  MRISreallocVerticesAndFaces et al
  return repr->nfaces;
}
bool Surface::faceAttachmentDeferred()
    const { //  defer connecting faces to vertices for performance reasons
...
p_p_void Surface::dist_orig_storage()
    const { //  the malloced/realloced vertex dist_orig fields, so those fields
            //  can be quickly nulled and restored
  return repr->dist_orig_storage;
}
int Surface::tempsAssigned() const { //  State of various temp fields that can
...
float Vertex::dist(size_t i) const { // size() is vtotal.    distance to
                                     // neighboring vertices based on  xyz
  return repr->vertices[idx].dist[i];
}
float Vertex::dist_orig(
    size_t i) const { // size() is vtotal.    distance to neighboring vertices
                      // based on origxyz
  return repr->vertices[idx].dist_orig[i];
}
int Vertex::dist_capacity()
    const { //  -- should contain at least vtx_vtotal elements
  return repr->vertices[idx].dist_capacity;
}
int Vertex::dist_orig_capacity()
    const { //  -- should contain at least vtx_vtotal elements
...
float Vertex::z() const { return repr->vertices[idx].z; }
float Vertex::origx()
    const { //  original coordinates, see also MRIS::origxyz_status
...
float Vertex::cx() const { return repr->vertices[idx].cx; }
float Vertex::cy() const { return repr->vertices[idx].cy; }
...
void Vertex::set_cx(float to) { repr->vertices[idx].cx = to; }
void Vertex::set_cy(float to) { repr->vertices[idx].cy = to; }
...
float Surface::da() const { return repr->da; }
float Surface::db() const { return repr->db; }
...
void Surface::set_da(float to) { repr->da = to; }
void Surface::set_db(float to) { repr->db = to; }
...
Vertex Face::v(size_t i) const { return Vertex(repr, repr->faces[idx].v[i]); }
float Face::area() const { return repr->faces[idx].area; }
angles_per_triangle_t Face::angle() const { return repr->faces[idx].angle; }
...
char Face::oripflag() const { return repr->faces[idx].oripflag; }
int Face::marked() const { return repr->faces[idx].marked; }
PDMATRIX Face::norm() const { return repr->faces[idx].norm; }
A3PDMATRIX Face::gradNorm() const { return repr->faces[idx].gradNorm; }
...
float Vertex::nx() const { return repr->vertices[idx].nx; }
float Vertex::ny() const { return repr->vertices[idx].ny; }
...
float Vertex::area() const { return repr->vertices[idx].area; }
char Vertex::ripflag() const { //  vertex no longer exists - placed last to load
                               //  the next vertex into cache
  return repr->vertices[idx].ripflag;
}
...
void Vertex::set_nx(float to) { repr->vertices[idx].nx = to; }
void Vertex::set_ny(float to) { repr->vertices[idx].ny = to; }
...
STRIP Surface::strips(size_t i) const { return repr->strips[i]; }
...
void Surface::set_strips(size_t i, STRIP to) { repr->strips[i] = to; }
void Surface::set_xctr(float to) { repr->xctr = to; }
void Surface::set_yctr(float to) { repr->yctr = to; }
void Surface::set_zctr(float to) { repr->zctr = to; }
void Surface::set_xlo(float to) { repr->xlo = to; }
void Surface::set_ylo(float to) { repr->ylo = to; }
void Surface::set_zlo(float to) { repr->zlo = to; }
void Surface::set_xhi(float to) { repr->xhi = to; }
void Surface::set_yhi(float to) { repr->yhi = to; }
void Surface::set_zhi(float to) { repr->zhi = to; }
void Surface::set_x0(float to) { //  center of spherical expansion
  repr->x0 = to;
}
void Surface::set_y0(float to) { repr->y0 = to; }
void Surface::set_z0(float to) { repr->z0 = to; }
void Surface::set_v_temporal_pole(Vertex to) {
  cheapAssert(repr == to.repr);
  repr->v_temporal_pole = repr->vertices + to.idx;
}
void Surface::set_v_frontal_pole(Vertex to) {
  cheapAssert(repr == to.repr);
  repr->v_frontal_pole = repr->vertices + to.idx;
}
void Surface::set_v_occipital_pole(Vertex to) {
  cheapAssert(repr == to.repr);
  repr->v_occipital_pole = repr->vertices + to.idx;
}
void Surface::set_max_curv(float to) { repr->max_curv = to; }
void Surface::set_min_curv(float to) { repr->min_curv = to; }
void Surface::set_total_area(float to) { repr->total_area = to; }
void Surface::set_avg_vertex_area(double to) { repr->avg_vertex_area = to; }
void Surface::set_zeros(int to) { repr->zeros = to; }
...
void Surface::set_nlabels(int to) { repr->nlabels = to; }
void Surface::set_labels(
    PMRIS_AREA_LABEL to) { //  nlabels of these (may be null)
  repr->labels = to;
}
void Surface::set_vp(p_void to) { //  for misc. use
  repr->vp = to;
...
float Vertex::pnx() const { return repr->vertices[idx].pnx; }
float Vertex::pny() const { return repr->vertices[idx].pny; }
float Vertex::pnz() const { //  pial normal
  return repr->vertices[idx].pnz;
}
float Vertex::wnx() const { return repr->vertices[idx].wnx; }
float Vertex::wny() const { return repr->vertices[idx].wny; }
float Vertex::wnz() const { //  white normal
  return repr->vertices[idx].wnz;
}
float Vertex::onx() const { return repr->vertices[idx].onx; }
float Vertex::ony() const { return repr->vertices[idx].ony; }
float Vertex::onz() const { //  original normal
  return repr->vertices[idx].onz;
}
float Vertex::dx() const { return repr->vertices[idx].dx; }
float Vertex::dy() const { return repr->vertices[idx].dy; }
float Vertex::dz() const { //  current change in position
  return repr->vertices[idx].dz;
}
float Vertex::odx() const { return repr->vertices[idx].odx; }
float Vertex::ody() const { return repr->vertices[idx].ody; }
float Vertex::odz() const { //  last change of position (for momentum,
  return repr->vertices[idx].odz;
}
float Vertex::tdx() const { return repr->vertices[idx].tdx; }
float Vertex::tdy() const { return repr->vertices[idx].tdy; }
...
float Vertex::curvbak() const { return repr->vertices[idx].curvbak; }
...
float Vertex::tx() const { return repr->vertices[idx].tx; }
float Vertex::ty() const { return repr->vertices[idx].ty; }
float Vertex::tz() const { //  tmp coordinate storage
  return repr->vertices[idx].tz;
}
float Vertex::t2x() const { return repr->vertices[idx].t2x; }
float Vertex::t2y() const { return repr->vertices[idx].t2y; }
float Vertex::t2z() const { //  another tmp coordinate storage
  return repr->vertices[idx].t2z;
}
float Vertex::targx() const { return repr->vertices[idx].targx; }
float Vertex::targy() const { return repr->vertices[idx].targy; }
float Vertex::targz() const { //  target coordinates
  return repr->vertices[idx].targz;
}
float Vertex::pialx() const { return repr->vertices[idx].pialx; }
float Vertex::pialy() const { return repr->vertices[idx].pialy; }
float Vertex::pialz() const { //  pial surface coordinates
  return repr->vertices[idx].pialz;
}
float Vertex::whitex() const { return repr->vertices[idx].whitex; }
float Vertex::whitey() const { return repr->vertices[idx].whitey; }
float Vertex::whitez() const { //  white surface coordinates
  return repr->vertices[idx].whitez;
}
float Vertex::l4x() const { return repr->vertices[idx].l4x; }
float Vertex::l4y() const { return repr->vertices[idx].l4y; }
float Vertex::l4z() const { //  layerIV surface coordinates
  return repr->vertices[idx].l4z;
}
float Vertex::infx() const { return repr->vertices[idx].infx; }
float Vertex::infy() const { return repr->vertices[idx].infy; }
float Vertex::infz() const { //  inflated coordinates
  return repr->vertices[idx].infz;
}
float Vertex::fx() const { return repr->vertices[idx].fx; }
float Vertex::fy() const { return repr->vertices[idx].fy; }
float Vertex::fz() const { //  flattened coordinates
  return repr->vertices[idx].fz;
}
int Vertex::px() const { return repr->vertices[idx].px; }
int Vertex::qx() const { return repr->vertices[idx].qx; }
int Vertex::py() const { return repr->vertices[idx].py; }
int Vertex::qy() const { return repr->vertices[idx].qy; }
int Vertex::pz() const { return repr->vertices[idx].pz; }
int Vertex::qz() const { //  rational coordinates for exact calculations
  return repr->vertices[idx].qz;
}
float Vertex::e1x() const { return repr->vertices[idx].e1x; }
float Vertex::e1y() const { return repr->vertices[idx].e1y; }
float Vertex::e1z() const { //  1st basis vector for the local tangent plane
  return repr->vertices[idx].e1z;
}
float Vertex::e2x() const { return repr->vertices[idx].e2x; }
float Vertex::e2y() const { return repr->vertices[idx].e2y; }
float Vertex::e2z() const { //  2nd basis vector for the local tangent plane
  return repr->vertices[idx].e2z;
}
float Vertex::pe1x() const { return repr->vertices[idx].pe1x; }
float Vertex::pe1y() const { return repr->vertices[idx].pe1y; }
float Vertex::pe1z() const { //  1st basis vector for the local tangent plane
  return repr->vertices[idx].pe1z;
}
float Vertex::pe2x() const { return repr->vertices[idx].pe2x; }
float Vertex::pe2y() const { return repr->vertices[idx].pe2y; }
...
float Vertex::fieldsign()
    const { //  fieldsign--final: -1, "0", "1" (file: rh.fs)
...
int Vertex::annotation()
    const { //  area label (defunct--now from label file name!)
  return repr->vertices[idx].annotation;
}
char Vertex::oripflag() const { return repr->vertices[idx].oripflag; }
...
float Vertex::theta() const { return repr->vertices[idx].theta; }
float Vertex::phi() const { //  parameterization
  return repr->vertices[idx].phi;
}
float Vertex::area() const { return repr->vertices[idx].area; }
float Vertex::origarea() const { return repr->vertices[idx].origarea; }
...
float Vertex::k1() const { return repr->vertices[idx].k1; }
float Vertex::k2() const { //  the principal curvatures
  return repr->vertices[idx].k2;
}
float Vertex::mean() const { return repr->vertices[idx].mean; }
float Vertex::mean_imag() const { //  imaginary part of complex statistic
  return repr->vertices[idx].mean_imag;
}
float Vertex::std_error() const { return repr->vertices[idx].std_error; }
uint Vertex::flags() const { return repr->vertices[idx].flags; }
int Vertex::fno() const { //  face that this vertex is in
  return repr->vertices[idx].fno;
}
int Vertex::cropped() const { return repr->vertices[idx].cropped; }
short Vertex::marked() const { //  for a variety of uses
  return repr->vertices[idx].marked;
}
short Vertex::marked2() const { return repr->vertices[idx].marked2; }
short Vertex::marked3() const { return repr->vertices[idx].marked3; }
...
void Vertex::set_pnx(float to) { repr->vertices[idx].pnx = to; }
void Vertex::set_pny(float to) { repr->vertices[idx].pny = to; }
void Vertex::set_pnz(float to) { //  pial normal
  repr->vertices[idx].pnz = to;
}
void Vertex::set_wnx(float to) { repr->vertices[idx].wnx = to; }
void Vertex::set_wny(float to) { repr->vertices[idx].wny = to; }
void Vertex::set_wnz(float to) { //  white normal
  repr->vertices[idx].wnz = to;
}
void Vertex::set_onx(float to) { repr->vertices[idx].onx = to; }
void Vertex::set_ony(float to) { repr->vertices[idx].ony = to; }
void Vertex::set_onz(float to) { //  original normal
  repr->vertices[idx].onz = to;
}
void Vertex::set_dx(float to) { repr->vertices[idx].dx = to; }
void Vertex::set_dy(float to) { repr->vertices[idx].dy = to; }
void Vertex::set_dz(float to) { //  current change in position
  repr->vertices[idx].dz = to;
}
void Vertex::set_odx(float to) { repr->vertices[idx].odx = to; }
void Vertex::set_ody(float to) { repr->vertices[idx].ody = to; }
void Vertex::set_odz(float to) { //  last change of position (for momentum,
  repr->vertices[idx].odz = to;
}
void Vertex::set_tdx(float to) { repr->vertices[idx].tdx = to; }
void Vertex::set_tdy(float to) { repr->vertices[idx].tdy = to; }
...
void Vertex::set_curvbak(float to) { repr->vertices[idx].curvbak = to; }
...
void Vertex::set_tx(float to) { repr->vertices[idx].tx = to; }
void Vertex::set_ty(float to) { repr->vertices[idx].ty = to; }
void Vertex::set_tz(float to) { //  tmp coordinate storage
  repr->vertices[idx].tz = to;
}
void Vertex::set_t2x(float to) { repr->vertices[idx].t2x = to; }
void Vertex::set_t2y(float to) { repr->vertices[idx].t2y = to; }
void Vertex::set_t2z(float to) { //  another tmp coordinate storage
  repr->vertices[idx].t2z = to;
}
void Vertex::set_targx(float to) { repr->vertices[idx].targx = to; }
void Vertex::set_targy(float to) { repr->vertices[idx].targy = to; }
void Vertex::set_targz(float to) { //  target coordinates
  repr->vertices[idx].targz = to;
}
void Vertex::set_pialx(float to) { repr->vertices[idx].pialx = to; }
void Vertex::set_pialy(float to) { repr->vertices[idx].pialy = to; }
void Vertex::set_pialz(float to) { //  pial surface coordinates
  repr->vertices[idx].pialz = to;
}
void Vertex::set_whitex(float to) { repr->vertices[idx].whitex = to; }
void Vertex::set_whitey(float to) { repr->vertices[idx].whitey = to; }
void Vertex::set_whitez(float to) { //  white surface coordinates
  repr->vertices[idx].whitez = to;
}
void Vertex::set_l4x(float to) { repr->vertices[idx].l4x = to; }
void Vertex::set_l4y(float to) { repr->vertices[idx].l4y = to; }
void Vertex::set_l4z(float to) { //  layerIV surface coordinates
  repr->vertices[idx].l4z = to;
}
void Vertex::set_infx(float to) { repr->vertices[idx].infx = to; }
void Vertex::set_infy(float to) { repr->vertices[idx].infy = to; }
void Vertex::set_infz(float to) { //  inflated coordinates
  repr->vertices[idx].infz = to;
}
void Vertex::set_fx(float to) { repr->vertices[idx].fx = to; }
void Vertex::set_fy(float to) { repr->vertices[idx].fy = to; }
void Vertex::set_fz(float to) { //  flattened coordinates
  repr->vertices[idx].fz = to;
}
void Vertex::set_px(int to) { repr->vertices[idx].px = to; }
void Vertex::set_qx(int to) { repr->vertices[idx].qx = to; }
void Vertex::set_py(int to) { repr->vertices[idx].py = to; }
void Vertex::set_qy(int to) { repr->vertices[idx].qy = to; }
void Vertex::set_pz(int to) { repr->vertices[idx].pz = to; }
void Vertex::set_qz(int to) { //  rational coordinates for exact calculations
  repr->vertices[idx].qz = to;
}
void Vertex::set_e1x(float to) { repr->vertices[idx].e1x = to; }
void Vertex::set_e1y(float to) { repr->vertices[idx].e1y = to; }
void Vertex::set_e1z(
    float to) { //  1st basis vector for the local tangent plane
  repr->vertices[idx].e1z = to;
}
void Vertex::set_e2x(float to) { repr->vertices[idx].e2x = to; }
void Vertex::set_e2y(float to) { repr->vertices[idx].e2y = to; }
void Vertex::set_e2z(
    float to) { //  2nd basis vector for the local tangent plane
  repr->vertices[idx].e2z = to;
}
void Vertex::set_pe1x(float to) { repr->vertices[idx].pe1x = to; }
void Vertex::set_pe1y(float to) { repr->vertices[idx].pe1y = to; }
void Vertex::set_pe1z(
    float to) { //  1st basis vector for the local tangent plane
  repr->vertices[idx].pe1z = to;
}
void Vertex::set_pe2x(float to) { repr->vertices[idx].pe2x = to; }
void Vertex::set_pe2y(float to) { repr->vertices[idx].pe2y = to; }
void Vertex::set_pe2z(
    float to) { //  2nd basis vector for the local tangent plane
...
void Vertex::set_fieldsign(
    float to) { //  fieldsign--final: -1, "0", "1" (file: rh.fs)
...
void Vertex::set_annotation(
    int to) { //  area label (defunct--now from label file name!)
  repr->vertices[idx].annotation = to;
}
void Vertex::set_oripflag(char to) { repr->vertices[idx].oripflag = to; }
...
void Vertex::set_theta(float to) { repr->vertices[idx].theta = to; }
void Vertex::set_phi(float to) { //  parameterization
  repr->vertices[idx].phi = to;
}
void Vertex::set_origarea(float to) { repr->vertices[idx].origarea = to; }
...
void Vertex::set_k1(float to) { repr->vertices[idx].k1 = to; }
void Vertex::set_k2(float to) { //  the principal curvatures
  repr->vertices[idx].k2 = to;
}
void Vertex::set_mean(float to) { repr->vertices[idx].mean = to; }
void Vertex::set_mean_imag(float to) { //  imaginary part of complex statistic
  repr->vertices[idx].mean_imag = to;
}
void Vertex::set_std_error(float to) { repr->vertices[idx].std_error = to; }
void Vertex::set_flags(uint to) { repr->vertices[idx].flags = to; }
void Vertex::set_fno(int to) { //  face that this vertex is in
  repr->vertices[idx].fno = to;
}
void Vertex::set_cropped(int to) { repr->vertices[idx].cropped = to; }
void Vertex::set_marked(short to) { //  for a variety of uses
  repr->vertices[idx].marked = to;
}
void Vertex::set_marked2(short to) { repr->vertices[idx].marked2 = to; }
void Vertex::set_marked3(short to) { repr->vertices[idx].marked3 = to; }
...
void Surface::set_fname(
    MRIS_fname_t to) { //  file it was originally loaded from
  repr->fname = to;
}
void Surface::set_status(
    MRIS_Status to) { //  type of surface (e.g. sphere, plane)
  repr->status = to;
}
void Surface::set_origxyz_status(
    MRIS_Status to) { //  type of surface (e.g. sphere, plane) that this origxyz
                      //  were obtained from
...
void Vertex::set_v(size_t i,
                   Vertex to) { // size() is vtotal.    array[v->vtotal or more]
                                // of vno, head sorted by hops
  cheapAssert(repr == to.repr);
  repr->vertices_topology[idx].v[i] = to.idx;
}
void Vertex::set_vnum(
    short to) { //  number of 1-hop neighbors    should use [p]VERTEXvnum(i)
...
void Vertex::set_vtotal(
    short to) { //  total # of neighbors. copy of vnum.nsizeCur
  repr->vertices_topology[idx].vtotal = to;
}
void Vertex::set_nsizeMaxClock(
    short to) { //  copy of mris->nsizeMaxClock when v#num
  repr->vertices_topology[idx].nsizeMaxClock = to;
}
void Vertex::set_nsizeMax(
    uchar to) { //  the max nsize that was used to fill in vnum etc
...
void Vertex::set_z(float to) { repr->vertices[idx].z = to; }
void Vertex::set_cx(float to) { repr->vertices[idx].cx = to; }
void Vertex::set_cy(float to) { repr->vertices[idx].cy = to; }
void Vertex::set_cz(float to) { //  coordinates in canonical coordinate system
  repr->vertices[idx].cz = to;
...
void Face::set_area(float to) { repr->faces[idx].area = to; }
void Face::set_angle(angles_per_triangle_t to) { repr->faces[idx].angle = to; }
void Face::set_orig_angle(angles_per_triangle_t to) {
  repr->faces[idx].orig_angle = to;
}
void Face::set_ripflag(char to) { repr->faces[idx].ripflag = to; }
void Face::set_oripflag(char to) { repr->faces[idx].oripflag = to; }
void Face::set_marked(int to) { repr->faces[idx].marked = to; }
void Face::set_norm(PDMATRIX to) { repr->faces[idx].norm = to; }
void Face::set_gradNorm(A3PDMATRIX to) { repr->faces[idx].gradNorm = to; }
...
void Vertex::set_area(float to) { repr->vertices[idx].area = to; }
void Vertex::set_ripflag(char to) { //  vertex no longer exists - placed last to
                                    //  load the next vertex into cache
  repr->vertices[idx].ripflag = to;
}
...
void Surface::set_total_area(float to) { repr->total_area = to; }
void Surface::set_avg_vertex_area(double to) { repr->avg_vertex_area = to; }
void Surface::set_avg_vertex_dist(
    double to) { //  set by MRIScomputeAvgInterVertexDist
  repr->avg_vertex_dist = to;
}
void Surface::set_std_vertex_dist(double to) { repr->std_vertex_dist = to; }
void Surface::set_orig_area(float to) { repr->orig_area = to; }
void Surface::set_neg_area(float to) { repr->neg_area = to; }
void Surface::set_neg_orig_area(
    float to) { //  amount of original surface in folds
