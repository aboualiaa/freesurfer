...
#define MATRIX_ELT(m, r, c)                                                    \
  (m->type == MATRIX_REAL ? *MATRIX_RELT(m, r, c) : *MATRIX_CELT(m, r, c))
#define MATRIX_PTR(m, r, c)                                                    \
  (m->type == MATRIX_REAL ? MATRIX_RELT(m, r, c)                               \
...
MATRIX *MatrixAlloc2_wkr(const int rows, const int cols, const int type,
                         ...
                         int callSiteLine);
#define MatrixAlloc(ROWS, COLS, TYPE)                                          \
  MatrixAlloc_wkr((ROWS), (COLS), (TYPE), __FILE__, __LINE__)
...
MATRIX *MatrixMultiply_wkr(const MATRIX *m1, const MATRIX *m2, MATRIX *m3,
                           const char *callSiteFile, int callSiteLine);
#define MatrixMultiply(M1, M2, M3)                                             \
  MatrixMultiply_wkr((M1), (M2), (M3), __FILE__, __LINE__)
// (r1 x c1) * (r2 x c2) = (r1 x c2)
// c1 must equal r2

MATRIX *MatrixMultiplyElts(MATRIX *m1, MATRIX *m2,
                           MATRIX *m12); // like matlab m1.*m2
MATRIX *MatrixDivideElts(MATRIX *num, MATRIX *den,
                         MATRIX *quotient); // like matlab num./den
...
MATRIX *MatrixCopyRegion(const MATRIX *mSrc, MATRIX *mDst, const int start_row,
                         const int startol, const int rows, const int cols,
...
#define V3_ADD(va, vb, vc)                                                     \
  V3_X(vc) = V3_X(va) + V3_X(vb), V3_Y(vc) = V3_Y(va) + V3_Y(vb),              \
...
#define V3_SUBTRACT(va, vb, vc)                                                \
  ...
  V3_Z(vc) = V3_Z(va) - V3_Z(vb);
#define V3_DOT(va, vb)                                                         \
  (V3_X(va) * V3_X(vb) + V3_Y(va) * V3_Y(vb) + V3_Z(va) * V3_Z(vb))
#define V3_SCALAR_MUL(va, s, vb)                                               \
  (V3_X(vb) = V3_X(va) * s, V3_Y(vb) = V3_Y(va) * s, V3_Z(vb) = V3_Z(va) * s)

#define V3_NORMALIZE(va, vb)                                                   \
  {                                                                            \
    float len = (V3_LEN(va));                                                  \
    if (FZERO(len))                                                            \
      len = 1.0f;                                                              \
