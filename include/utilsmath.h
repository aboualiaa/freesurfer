 ...
 * much greater than the number of vertex points
 ...
 \param mri_template - the MRI template of the same subject which is needed for
 the ras2vox call
*/
...
inline void ComputeNormal(const double v1[3], const double v2[3],
...
//! Sort the eigen vectors of a 3x3 matrix in descending order based on its
//! eigen values and return the eigen vectors in max, mid and min arrays
/*!
...
inline double DistanceToLine(double x[3], double p1[3], double p2[3]) {
  ...
  num = p21[0] * (x[0] - p1[0]) + p21[1] * (x[1] - p1[1]) +
        p21[2] * (x[2] - p1[2]);
    ...
    for (double xk = -distance; xk <= distance; xk += distance / 5.0)
      for (double yk = -distance; yk <= distance; yk += distance / 5.0)
        for (double zk = -distance; zk <= distance; zk += distance / 5.0) {
          if (xk * xk + yk * yk + zk * zk > distance * distance)
            continue;
