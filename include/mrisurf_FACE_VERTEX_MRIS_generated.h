...
struct VERTEX_TOPOLOGY {
  //  put the pointers before the ints, before the shorts, before uchars, to
...
struct vertex_type_ {
  ...
  pSeveralFloat dist; // size() is vtotal.    distance to neighboring vertices
                      // based on  xyz
  pSeveralFloat dist_orig;   // size() is vtotal.    distance to neighboring
  ...
  char   border;  //  flag
  char   ripflag; //  vertex no longer exists - placed last to load the next
                  //  vertex into cache
};                // vertex_type_
...
struct MRIS {
  ...
  int nvertices;               //  # of vertices on surface, change by calling
                               //  MRISreallocVerticesAndFaces et al
  int nfaces;                  //  # of faces on surface, change by calling
                               //  MRISreallocVerticesAndFaces et al
  bool faceAttachmentDeferred; //  defer connecting faces to vertices for
                               //  performance reasons
  ...
  p_p_void dist_storage; //  the malloced/realloced vertex dist fields, so those
                         //  fields can be quickly nulled and restored
  p_p_void
      dist_orig_storage; //  the malloced/realloced vertex dist_orig fields, so
                         //  those fields can be quickly nulled and restored
  int tempsAssigned; //  State of various temp fields that can be borrowed if
                     //  not already in use
  pSeveralFACE                  faces;
  ...
  float                         y0;
  float                         z0;
  //  v_temporal_pole, v_frontal_pole, and v_occipital_pole don't appear to be
  //  used, and are unusual being pointers to vertices
  ...
  MRIS_Status origxyz_status; //  type of surface (e.g. sphere, plane) that this
  ...
  short nsizeMaxClock; //  changed whenever an edge is added or removed, which
                       //  invalidates the vertex v#num values
  char max_nsize;  //  max the neighborhood size has been set to (typically 3)
  char dist_nsize; //  max mrisComputeVertexDistances has computed distances out
                   //  to
  char dist_orig_nsize; //  max mrisComputeOriginalVertexDistances has computed
                        //  distances out to
  char dist_alloced_flags; //  two flags, set when any dist(1) or dist_orig(2)
                           //  allocated
  ...
  int    max_vertices; //  may be bigger than nvertices, set by calling
                       //  MRISreallocVerticesAndFaces
  int max_faces;       //  may be bigger than nfaces,    set by calling
                       //  MRISreallocVerticesAndFaces
  ...
  pSeveralFloat       dz2;
  PCOLOR_TABLE        ct;
  int useRealRAS; //  if 0 (default), vertex position is a conformed volume RAS
                  //  with c_(r,"a","s")=0.  else is a real RAS (volume stored
                  //  RAS)
  VOL_GEOM vg;    //  volume info from which this surface is created. valid iff
                  //  vg.valid = 1
...
#define LIST_OF_FACE_ELTS                                                      \
  ELTT(vertices_per_face_t, v)                                                 \
  SEP ELTT(float, area) SEP ELTT(angles_per_triangle_t, angle)                 \
      SEP                   ELTT(angles_per_triangle_t, orig_angle) SEP        \
                            ELTT(char, ripflag) SEP                            \
                            ELTT(char, oripflag) SEP                           \
      ELTT(int, marked) SEP ELTP(DMATRIX, norm)                                \
          SEP               ELTX(A3PDMATRIX, gradNorm) // end of macro

#define LIST_OF_VERTEX_TOPOLOGY_ELTS                                           \
  ELTP(int, f)                                                                 \
  SEP ELTP(uchar, n) SEP ELTP(int, e) SEP ELTP(int, v) SEP ELTT(short, vnum)   \
      SEP                                                                      \
      ELTT(short, v2num) SEP                                                   \
      ELTT(short, v3num) SEP                                                   \
      ELTT(short, vtotal) SEP                                                  \
      ELTX(short, nsizeMaxClock) SEP ELTT(uchar, nsizeMax)                     \
          SEP ELTT(uchar, nsizeCur) SEP ELTT(uchar, num) // end of macro

#define LIST_OF_VERTEX_ELTS_1                                                  \
  ELTP(float, dist)                                                            \
  SEP ELTP(float, dist_orig) SEP ELTT(int, dist_capacity) SEP ELTT(            \
      int, dist_orig_capacity) SEP                                             \
  ELTT(float, x) SEP                                                           \
  ELTT(float, y) SEP                                                           \
  ELTT(float, z) SEP                                                           \
  ...
  ELTT(float, pe2x) SEP                                                        \
  ELTT(float, pe2y) SEP                                                        \
  ELTT(float, pe2z) SEP // end of macro

#define LIST_OF_VERTEX_ELTS_3                                                   \
  ELTT(float, nc)                                                               \
  SEP ELTT(float, val2) SEP ELTT(float, valbak) SEP ELTT(float, val2bak) SEP    \
                                                    ELTT(float, stat) SEP       \
...
#define LIST_OF_VERTEX_ELTS_5                                                  \
  ELTT(int, annotation)                                                        \
  SEP ELTT(char, oripflag) SEP ELTT(char, origripflag) SEP // end of macro

#define LIST_OF_VERTEX_ELTS_7                                                  \
  ELTX(p_void, vp)                                                             \
  SEP ELTT(float, theta) SEP ELTT(float, phi) SEP ELTT(float, area) SEP ELTT(  \
      float, origarea) SEP                                                     \
  ...
  ELTT(float, std_error) SEP ELTT(uint, flags) SEP                             \
                             ELTT(int, fno) SEP                                \
                             ...
                             ELTT(char, ripflag) SEP // end of macro

#define LIST_OF_VERTEX_ELTS                                                    \
  LIST_OF_VERTEX_ELTS_1 SEP LIST_OF_VERTEX_ELTS_3 SEP LIST_OF_VERTEX_ELTS_5    \
      SEP LIST_OF_VERTEX_ELTS_7 // end of macro

#define LIST_OF_MRIS_ELTS_1                                                      \
  ELTT(int, nverticesFrozen)                                                     \
  SEP ELTT(int, nvertices) SEP ELTT(int, nfaces) SEP ELTT(                       \
      bool, faceAttachmentDeferred) SEP                                          \
  ELTT(int, nedges) SEP                                                          \
  ELTT(int, ncorners) SEP                                                        \
  ELTT(int, nstrips) SEP ELTP(VERTEX_TOPOLOGY, vertices_topology)                \
      SEP ELTP(VERTEX, vertices) SEP ELTX(p_p_void, dist_storage)                \
          SEP                        ELTX(p_p_void, dist_orig_storage) SEP       \
          ELTT(int, tempsAssigned) SEP ELTP(FACE, faces)                         \
              SEP ELTP(MRI_EDGE, edges) SEP ELTP(MRI_CORNER, corners)            \
                  SEP     ELTP(FaceNormCacheEntry, faceNormCacheEntries)         \
                      SEP ELTP(FaceNormDeferredEntry, faceNormDeferredEntries)   \
                          SEP ELTP(STRIP, strips) SEP                            \
                              ELTT(float, xctr) SEP                              \
                              ELTT(float, yctr) SEP                              \
                              ELTT(float, zctr) SEP                              \
                              ELTT(float, xlo) SEP                               \
                              ...
                              ELTT(float, x0) SEP                                \
                              ELTT(float, y0) SEP                                \
                          ELTT(float, z0) SEP ELTP(VERTEX, v_temporal_pole)      \
                              SEP             ELTP(VERTEX, v_frontal_pole)       \
                                  SEP         ELTP(VERTEX, v_occipital_pole) SEP \
...
#define LIST_OF_MRIS_ELTS_3                                                    \
  ELTP(LTA, lta)                                                               \
  SEP ELTP(MATRIX, SRASToTalSRAS_) SEP ELTP(MATRIX, TalSRASToSRAS_) SEP ELTT(  \
      int, free_transform) SEP                                                 \
  ELTT(double, radius) SEP                                                     \
  ELTT(float, a) SEP                                                           \
  ELTT(float, b) SEP                                                           \
  ELTT(float, c) SEP ELTT(MRIS_fname_t, fname) SEP                             \
                     ELTT(float, Hmin) SEP                                     \
                     ELTT(float, Hmax) SEP                                     \
                     ELTT(float, Kmin) SEP                                     \
                     ELTT(float, Kmax) SEP                                     \
  ELTT(double, Ktotal) SEP ELTT(MRIS_Status, status)                           \
      SEP                  ELTT(MRIS_Status, origxyz_status) SEP               \
                           ELTT(int, patch) SEP                                \
      ELTT(int, nlabels) SEP ELTP(MRIS_AREA_LABEL, labels) SEP                 \
      ELTT(char, nsize) SEP ELTT(uchar, vtotalsMightBeTooBig) SEP              \
      ...
      ELTT(float, avg_nbrs) SEP ELTX(p_void, vp) SEP                           \
      ...
      ELTT(int, max_faces) SEP ELTT(MRIS_subject_name_t, subject_name) SEP     \
                               ELTT(float, canon_area) SEP                     \
                               ELTT(int, noscale) SEP                          \
                               ELTP(float, dx2) SEP                            \
                               ELTP(float, dy2) SEP                            \
      ELTP(float, dz2) SEP ELTP(COLOR_TABLE, ct) SEP                           \
      ELTT(int, useRealRAS) SEP ELTT(VOL_GEOM, vg)                             \
          SEP                   ELTX(MRIS_cmdlines_t, cmdlines) SEP            \
                                ELTT(int, ncmds) SEP                           \
                                ELTT(float, group_avg_surface_area) SEP        \
                                ELTT(int, group_avg_vtxarea_loaded) SEP        \
          ELTT(int, triangle_links_removed) SEP ELTX(p_void, user_parms)       \
              SEP ELTP(MATRIX, m_sras2vox) SEP ELTP(MRI, mri_sras2vox)         \
                  SEP ELTX(p_void, mht) SEP ELTX(p_void, temps)                \
                      SEP // end of macro

#define LIST_OF_MRIS_ELTS                                                      \
  LIST_OF_MRIS_ELTS_1 SEP LIST_OF_MRIS_ELTS_3 // end of macro
