...
typedef struct {
  ...
  int id; // this is a hack that allows user to find and modify a node of a heap
          // really easily, for example id can be equal to 5 for a particular
          // node. (note that id >= 0)Also, note that id should be unique to
          // each node and is provided by the users. For example, it can
          // correspond to vertex number. Currently, one need to set a maximum
          // possible id size so that we can allocate the table accordingly.
} MIN_HEAP_ELEMENT, MHE;

typedef struct {
  MHE *MHE_array;
  int *id_array; // This allows the user to find and modify a node of a heap
                 // really easily. For example, id_array[5] = 10 means
                 // MHE_array[10].id = 5. Thus, if a user wants to modify the
                 // key of the node of id 5, This can be done through the
                 // function: Min_HeapEditKeyIndexID
  int CurrHeapSize;
  int MaxHeapSize;
