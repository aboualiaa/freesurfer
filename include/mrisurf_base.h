...
// Configurable support for enabling the new code that does get reproducible
// results
//
...
template <typename T> T max3(T const &lhs, T const &mid, T const &rhs) {
  return max(max(lhs, mid), rhs);
}
template <typename T> T min3(T const &lhs, T const &mid, T const &rhs) {
  return min(min(lhs, min), rhs);
}
...
static int closeEnough(float a, float b) {
  ...
  if (mag < 1.0e-6)
    return 1;
  if (fabs(a - b) < mag * 1.0e-2)
    return 1;
  return 0;
}

static int project_point_onto_sphere(float cx, float cy, float cz, float radius,
...
#define D_DIST                                                                 \
  0.1 // sampling distance along tangent plane for computing derivatives
...
#define USED_IN_NEW_TESSELLATION                                               \
  3 // used in the new tessellation (was NOT_USED before)
#define USED_IN_BOTH_TESSELLATION                                              \
  4 // used in both tessellations (was USED_IN_NEW_TESSELLATION before)
...
/* defect becomes a enclosed patch requires ADD_VERTEX_VERTICES to avoid top.
 * inconsistencies */
...
static bool hasActiveRealmTrees() { return mrisurf_activeRealmTreesSize > 0; }
