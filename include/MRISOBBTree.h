...
public:
  ...
  ~MRISOBBTree() {
    ...
    if (!this->pointslist
             .empty()) // the temp list of verts  used while computing OBB
  ...
  void ConstructTree() {
    // create an axis aligned bounding box used to trivially reject points in
    // inclusion tests
  ...
  /*!
   ...
   When called, all the faces, the root node and 0 ( topmost level ) are given
   as the arguments
   */
  void BuildTreeRecursively(std::vector<int> faceids, OBBNode *pOBB,
                            int _level) {
  ...
  //! Given the list of faces, compute OBB bounds for them and fill the pOBB
  //! object
  /*!
   \param faceids - the list of faces
   \param pOBB - the node which will be filled with values after the OBB is
   computed
   */
  void ComputeOBB(std::vector<int> faceids, OBBNode *pOBB) {
    ...
    for (fv = faceids.begin(); fv != faceids.end(); ++fv) {
      ...
      // diagonal terms
      a0[0] += tri_mass *
               (9 * c[0] * c[0] + p[0] * p[0] + q[0] * q[0] + r[0] * r[0]) / 12;
      a1[1] += tri_mass *
               (9 * c[1] * c[1] + p[1] * p[1] + q[1] * q[1] + r[1] * r[1]) / 12;
      a2[2] += tri_mass *
               (9 * c[2] * c[2] + p[2] * p[2] + q[2] * q[2] + r[2] * r[2]) / 12;
      // off-diagonal terms
      a0[1] += tri_mass *
               (9 * c[0] * c[1] + p[0] * p[1] + q[0] * q[1] + r[0] * r[1]) / 12;
      a0[2] += tri_mass *
               (9 * c[0] * c[2] + p[0] * p[2] + q[0] * q[2] + r[0] * r[2]) / 12;
      a1[2] += tri_mass *
               (9 * c[1] * c[2] + p[1] * p[2] + q[1] * q[2] + r[1] * r[2]) / 12;
      // Insert the points
      for (j = 0; j < 3; j++) {
    ...
    // check area of face
    if (iszero(tot_mass)) {
      printf("error: triangle face defined by vertices %d, %d, and %d has no "
             "area!\n",
  ...
  int PointInclusionTest(double x, double y, double z) {
    ...
    // First check whether the point can be rejected trivially by just checking
    // its AABB AABB test is very fast and a large number of pts can be rejected
    // this way.
  ...
  //! Check  if the line segment connecting pts (x,y,z) and pt[3] intersects the
  //! OBB Tree
  /*!
  ...
  int DoesLineIntersectOBBTree(double x, double y, double z, double pt[3]) {
    ...
    while (depth > 0) {
      --depth;
      node = obbstack[depth];
      // Check if the line drawn from current point to the definitely inside
      // point intersects the node
  ...
  //! Check if a line intersects an OBBNode. Returns 1 if even at-least some
  //! portion
  ...
  bool DoesOBBoverlapLine(OBBNode *pOBB, double p0[3], double p1[3]) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      if ((rangeAmax < rangePmin) || (rangePmax < rangeAmin))
        return false; // doesn't overlap
    }
    return true; // OBB overlaps line
  }

  //! Check whether the line p1-p2 intersects the triangle formed by v1, v2 and
  //! v3 parametric coordinate 't' of the line is returned sense of intersection
  //! is also returned ( +1 if entering, -1 if exiting according to the normal
  //! of the triangle) Algorithm based on the vtkOBBtreeLineIntersectsTriangle
  //! method
  /*!
   ...
   */
  inline int LineIntersectsTriangle(double p1[3], double p2[3], double pt1[3],
                                    double pt2[3], double pt3[3],
                                    double normal[3], double &t, int &sense) {
                                ...
                                double &t, int &sense)
  {
    ...
    edge1[0] = pt2[0] - pt1[0]; edge1[1] = pt2[1] - pt1[1]; edge1[2] = pt2[2] -
  pt1[2]; edge2[0] = pt3[0] - pt1[0]; edge2[1] = pt3[1] - pt1[1]; edge2[2] =
  pt3[2] - pt1[2];
    ...
    tvec[0] = p1[0] - pt1[0]; tvec[1] = p1[1] - pt1[1]; tvec[2] = p1[2] -
  pt1[2];
