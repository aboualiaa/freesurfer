...
//
typedef enum { MHTFNO_FACE = 0, MHTFNO_VERTEX = 1 } MHTFNO_t;
...
struct MRIS_HASH_TABLE {
  ...
  MRIS_HASH_TABLE(MHTFNO_t fno_usage, float vres, int which_vertices)
      : m_fno_usage(fno_usage), m_vres(vres), m_which_vertices(which_vertices) {
  }
  virtual ~MRIS_HASH_TABLE() {}

  MRIS_HASH_TABLE_NoSurface *toMRIS_HASH_TABLE_NoSurface() {
    return toMRIS_HASH_TABLE_NoSurface_Wkr();
  }
  MRIS_HASH_TABLE_NoSurface const *toMRIS_HASH_TABLE_NoSurface() const {
    return toMRIS_HASH_TABLE_NoSurface_Wkr();
  }
...
MRIS_HASH_TABLE *MHTcreateFaceTable_Resolution(MRIS *mris, int which,
                                               float res);
MRIS_HASH_TABLE *MHTcreateVertexTable(MRIS *mris, int which);
MRIS_HASH_TABLE *MHTcreateVertexTable_Resolution(MRIS *mris, int which,
                                                 float res);

// Version
//
int MHT_gw_version(); // version of that unit
