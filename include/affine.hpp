...
//! Affine vector class
template <typename T> class AffineVector {
...
public:
  ...
  template <typename U> void Set(const U &x, const U &y, const U &z) {
    this->vec[iX] = x;
    this->vec[iY] = y;
    this->vec[iZ] = z;
    this->vec[iW] = 1;
  ...
  template <typename U> void Get(U &x, U &y, U &z) const {
    x = this->vec[iX];
    y = this->vec[iY];
    z = this->vec[iZ];
  }
  ...
  void Print(std::ostream &os = std::cout) const {
    os << "(" << std::setw(9) << std::setprecision(3) << this->vec[iX] << ","
...
public:
  ...
  void Clear() { memset(&(this->mat[0]), 0, kAffineMatrixSize * sizeof(T)); }
