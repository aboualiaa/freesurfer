...
constexpr auto NOT_PROCESSED_YET = 1000;
...
float mrisDefectFaceMRILogLikelihood(MRIS *mris, MRI *mri, TP *tp,
                                     HISTOGRAM *h_white, HISTOGRAM *h_gray,
                                     HISTOGRAM *h_grad, MRI *mri_gray_white);

float mrisDefectVertexMRILogLikelihood(MRIS *mris, MRI *mri, TP *tp,
                                       HISTOGRAM *h_white, HISTOGRAM *h_gray,
                                       HISTOGRAM *h_grad, MRI *mri_gray_white);
...
float mrisSampleMinimizationEnergy(MRIS *mris, int const vno,
...
float mrisSampleParallelEnergy(MRIS *mris, int const vno,
                               INTEGRATION_PARMS *parms, float cx, float cy,
                               float cz);
float mrisSampleNormalEnergy(MRIS *mris, int const vno,
                             INTEGRATION_PARMS *parms, float cx, float cy,
                             float cz);
...
static void
constructComputeDefectContext(ComputeDefectContext *computeDefectContext) {
...
#define VERTEX_ORIG_EDGE(vec, v0, v1)                                          \
  VECTOR_LOAD(vec, v1->origx - v0->origx, v1->origy - v0->origy,               \
              v1->origz - v0->origz)
