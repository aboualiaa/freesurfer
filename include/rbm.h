...
#endif
#define GAUSSIAN(x, u, s)                                                      \
  ((1.0 / (s * sqrt(2 * M_PI))) * exp(-(((x)-u) * ((x)-u)) / (2.0 * s * s)))
...
typedef struct {
  ...
  int *       sorted_labels;
  LABEL_PVAL *label_pvals; // list of sorted indices of labels (1st has highest
                           // pval, etc...)
...
typedef struct {
  ...
  double sparsity[MAX_LAYERS]; // what fraction of the time we want hidden nodes
                               // active
  ...
  double sparsity_decay; // l in active = l * previous_active_pval + (1-l) *
                         // current_active_pval
  int discriminative_training; // do discriminative training after normal CD is
                               // done
...
int    DBNcomputeGradients(DBN *dbn, int layer, VOXLIST *vl, double **dw,
                           ...
                           double **dlabel_weights, RBM_PARMS *parms, int *indices,
                           int index);
double DBNvoxlistRMS(DBN *dbn, int nlayers, VOXLIST *vl, RBM_PARMS *parms,
...
double CDBNvoxlistRMS(CDBN *cdbn, int nlayers, VOXLIST *vl, RBM_PARMS *parms,
