...
typedef struct {
  ...
  MRI *anatseg;  // segmentation in anatomical space, may be hires (ie, not
                 // conformed)
  ...
  double scale,
      scale_refval; // scale = scale factor, scalerefval = 1 or 100, etc
  ...
  MRI *volperseg; // volume of each segment (nperseg * voxsize) measure in anat
                  // seg space
  ...
  MATRIX *rvar, *rvargm, *rvarbrain,
      *rvarUnscaled; // residual variance, all vox and only GM
  ...
  // Steady-state blood plasma concentration, unit scale, decay correction
  // factor
