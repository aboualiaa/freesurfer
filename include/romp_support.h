...
// so that the romp support knows of their existence.  Other omp loops hjave no
// data collection.
//
...
#pragma omp parallel for if_ROMP(                                              \
    experimental)... // decides whether it should be parallel
#endif
  	for (n=0; n < nsegid; n++)
	{
...
// The omp for loops should be conditionalized to only be parallel if the
// romp_level allows it
//
...
// it is the call of this function that tells the ROMP code what level the loop
// is so there is more done here than just supplying the condition value to the
// omp if clause
...
// the macros below that add these are usually defined as noops rather than
// adding them.
//
...
// The conditionalized macros that either do or don't add the variables and
// calls based on the above
//
...
#define if_ROMPLEVEL(LEVEL)                                                    \
  if (ROMP_pf_stack.staticInfo && (ROMP_if_parallel2(LEVEL, &ROMP_pf_stack)))  \
  // end of macro

#define if_ROMP2(CONDITION, LEVEL)                                             \
  if ((CONDITION) && ROMP_pf_stack.staticInfo &&                               \
...
#define ROMP_PFLB_continue                                                     \
  { /* if (!ROMP_pf_stack.skip_pflb_timing) ROMP_PFLB_end; */                  \
    continue;                                                                  \
  }                                                                            \
  // end of macro
...
#define ROMP_PF_continue ROMP_PFLB_continue
...
// but floating point add and mul are not precisely commutative, and are
// compiler and thead-count specific so they are causing small initial
// variations that can cascade into test system failures. The following
// implements are deterministic ordering of the operations independent of
// thread-count.
//
...
//
using ROMP_Distributor = struct ROMP_Distributor;
...
#define ROMP_PARTIALSUM(REDUCTION_INDEX)                                       \
  ROMP_distributor.partials[ROMP_index].partialSum[REDUCTION_INDEX]
