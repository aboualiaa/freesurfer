...
int mrisComputeRepulsiveTerm(MRIS *mris, double l_repulse, MHT *mht_v,
                             MHT *mht_f);
int mrisComputeShrinkwrapTerm(MRIS *mris, MRI *mri_brain, double l_shrinkwrap);
int mrisComputeSphereTerm(MRIS *mris, double l_sphere, float radius,
                          int explode_flag);
int mrisComputeSurfaceNormalIntersectionTerm(MRIS *mris, MHT *mht,
                                             double l_norm, double max_dist);
int mrisComputeSurfaceRepulsionTerm(MRIS *mris, double l_repulse, MHT *mht);
int mrisComputeTargetLocationTerm(MRIS *mris, double l_location,
                                  INTEGRATION_PARMS *parms);
int mrisComputeThicknessMinimizationTerm(MRIS *mris, double l_thick_min,
                                         INTEGRATION_PARMS *parms);
int mrisComputeThicknessNormalTerm(MRIS *mris, double l_thick_normal,
                                   INTEGRATION_PARMS *parms);
int mrisComputeThicknessParallelTerm(MRIS *mris, double l_thick_parallel,
                                     INTEGRATION_PARMS *parms);
int mrisComputeThicknessSmoothnessTerm(MRIS *mris, double l_tsmooth,
                                       INTEGRATION_PARMS *parms);
int mrisComputeThicknessSpringTerm(MRIS *mris, double l_thick_spring,
                                   INTEGRATION_PARMS *parms);
int mrisComputeVectorCorrelationTerm(MRIS *mris, INTEGRATION_PARMS *parms);
int mrisComputeWhichSurfaceRepulsionTerm(MRIS *mris, double l_repulse, MHT *mht,
                                         int which, float max_dot);

// normal spring
void mrisComputeNormalSpringTerm(MRIS *mris, double l_spring);
void vertexComputeNormalSpringTerm(MRIS *mris, int vno, float *dx, float *dy,
                                   float *dz, double l_spring = 1.0);

// tangential spring
void mrisComputeTangentialSpringTerm(MRIS *mris, double l_spring);
void vertexComputeTangentialSpringTerm(MRIS *mris, int vno, float *dx,
                                       float *dy, float *dz,
                                       double l_spring = 1.0);
