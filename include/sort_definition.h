...
//
// Returns 0 only when all values are equal, otherwise partitions into two
// non-empty sets with all the elts in the [0..result) set being less than or
// equal to all the elts in the [result..size) set.
//
{
  if (size < 2)
    return 0;
...
Partition:;
  ...
  size_t hi = size;
  while (lo < hi) {
    while (SORT_LE(elts[lo], splitter)) {
      lo++;
      if (lo == hi)
        break;
    }
    while (lo < hi && !SORT_LE(elts[hi - 1], splitter)) {
      hi--;
    }
  ...
  //      (b) filter at least some of the splitter values out of the low
  //      partition
  ...
  if (hi == size) {
    while (0 < hi) {
      ...
      if (SORT_LE(splitter, temp))
        hi--; // accept some of the values equal to splitter into [hi..size)
      else {
        splitter = temp;
        goto Partition;
      } // found a lower splitter
    }
  }
...
static void SORT_NAME(SORT_ELEMENT *elts, size_t size, bool useThreads) {
  ...
  // This is done by recursively partitioning the data until all the threads are
  // busy. Currently allow up to 16 partitions
  //
...
#pragma omp parallel for schedule(guided)
  ...
  //
  if (size > 16) {
    size_t middle = SORT_NAME_partition(
        elts, size); // returns 0 only when all values are equal
    if (middle > 0) {
...
static bool SORT_NAME_isSorted(SORT_ELEMENT *elts, size_t size) {
  for (; size > 1; size--) {
    if (SORT_LE(elts[size - 2], elts[size - 1]))
      continue;
    return false;
  }
  return true;
