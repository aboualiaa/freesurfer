...
#define gcas_setPrior(GCAS, TO)                                                \
  {                                                                            \
    ((GCAS).prior_access_via_setGetPrior) = (TO);                              \
    (GCAS).prior_log = log((GCAS).prior_access_via_setGetPrior);               \
  }
...
MRI *GCAclassify(MRI *mri_src, GCA *gca, MRI *mri_dst, TRANSFORM *transform,
                 int max_labels);
...
int GCAsourceFloatVoxelToPrior(GCA *gca, MRI *mri, TRANSFORM *transform,
                               ...
                               int *pzp);
int GCAnodeToSourceVoxel(GCA *gca, MRI *mri, TRANSFORM *transform, int xv,
                         int yv, int zv, int *pxn, int *pyn, int *pzn);
...
MRI *GCAbuildMostLikelyVolumeForStructure(const GCA *gca, MRI *mri_seg,
...
int  GCAcomputeMLElabelAtLocation(GCA *gca, int x, int y, int z, float *vals,
                                  int *pmax_n, float *plog_p);
...
int   GCArenormalizeWithEntropyMinimization(GCA *gca_affine, MRI *mri,
...
int GCArenormalizeIntensities(GCA *gca, int *labels, float *intensities,
                              int num);
int GCArenormalizeIntensitiesAbsolute(GCA *gca, int *labels, float *intensities,
                                      int num);
...
GC1D *GCAfindClosestValidGC(GCA *gca, int x, int y, int z, int label,
                            int check_var);
...
GCA *  GCAcreateFlashGCAfromParameterGCA(GCA *gca_parms, double *TR, double *fa,
                                         double *TE, int nflash, double lambda);
GCA *  GCAcreateWeightedFlashGCAfromParameterGCA(GCA *gca_parms, double *TR,
                                                 double *fa, double *TE,
                                                 int nflash, double *wts,
                                                 double lambda);
GCA *  GCAcreateFlashGCAfromFlashGCA(GCA *gca_parms, double *TR, double *fa,
...
GCA_NODE * GCAbuildRegionalGCAN(const GCA *gca, int x, int y, int z, int wsize);
GCA_PRIOR *GCAbuildRegionalGCAP(const GCA *gca, int x, int y, int z, int wsize);
...
double GCAgibbsImpossibleConfiguration(GCA *gca, MRI *mri_labels, int x, int y,
                                       int z, TRANSFORM *transform);
