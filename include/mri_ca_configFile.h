...
private:
  ...
  bool getNextVectorElement(fstream &ifs, std::string &strNextElement,
    ...
    skipOver(ifs, ' ');
    strNextElement = getDelimitedString(ifs, ',', bEOLFound, bEQUALFound,
                                        bSqBracketFound, bEOFFound);
...
public:
  ...
  void init(std::string strTheFilepath, int nProt = 0) {
    ...
    if (!fs) {
      ...
      sprintf(strErrorMsg, "Error: could not open config file %s",
              strFilepath.c_str());
      throw(strErrorMsg);
    }
  }
    ...
    if (skipTo(strSectionName, valueName)) {
      ...
      while (bMoreRows == true) {
        ...
        if (vectString.size() != 0)
          matrixStrValues.push_back(vectString);
      }
    }
  }
           ...
           std::string valueName) // get 2D matrix of ints
  {
    ...
    if (skipTo(strSectionName, valueName)) {
      ...
      while (bMoreRows == true) {
        ...
        if (vectInt.size() != 0)
          matrixIntValues.push_back(vectInt);
      }
    }
  }
  ...
  bool get(TypeMatrixDouble &matrixDoubleValues, std::string strSectionName,
    ...
    if (skipTo(strSectionName, valueName)) {
      ...
      while (bMoreRows == true) {
        ...
        if (vectDouble.size() != 0)
          matrixDoubleValues.push_back(vectDouble);
      }
    }
  }
  ...
  bool get(TypeMatrixFloat &matrixFloatValues, std::string strSectionName,
    ...
    if (skipTo(strSectionName, valueName)) {
      ...
      while (bMoreRows == true) {
        ...
        if (vectFloat.size() != 0)
          matrixFloatValues.push_back(vectFloat);
      }
    }
  }
  ...
  void write(TypeMatrixString matrixStrValues, std::string strValueName,
    ...
    writeValueName(strValueName, bEchoToStdOut);
    if (bEchoToStdOut)
      printType(matrixStrValues);
    else
      printType(matrixStrValues, fs);
  }
  ...
  void write(TypeMatrixInt matrixIntValues, std::string strValueName,
    ...
    writeValueName(strValueName, bEchoToStdOut);
    if (bEchoToStdOut)
      printType(matrixIntValues);
    else
      printType(matrixIntValues, fs);
  }
  ...
  void write(TypeMatrixDouble matrixDoubleValues, std::string strValueName,
    ...
    writeValueName(strValueName, bEchoToStdOut);
    if (bEchoToStdOut)
      printType(matrixDoubleValues);
    else
      printType(matrixDoubleValues, fs);
  }
