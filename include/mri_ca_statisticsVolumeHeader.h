...
typedef map<unsigned char, TypeFractionalAreaAndPoints>
    TypeMapFractionalAreaAndPoints;
...
public:
  ...
  bool writeStatisticsVolumeInfoFile(string strStatisticsVolumePath,
    ...
    statisticsVolumeInfoConfigFile.writeSection("StatisticsVolumeInfo",
                                                bEchoToStdOut);
    statisticsVolumeInfoConfigFile.write(nNumSubjects, "numberOfSubjects",
                                         bEchoToStdOut);
    statisticsVolumeInfoConfigFile.write(nNumSlices, "numberOfSlices",
                                         bEchoToStdOut);
    statisticsVolumeInfoConfigFile.write(nNumMeasures, "numberOfMeasures",
                                         bEchoToStdOut);
    ...
    statisticsVolumeInfoConfigFile.write(strLabelLUTPath, "LabelLUT",
                                         bEchoToStdOut);
    statisticsVolumeInfoConfigFile.write(fSliceThickness, "fSliceThickness",
                                         bEchoToStdOut);
    statisticsVolumeInfoConfigFile.write(fSquarePixelSize, "fSquarePixelSize",
                                         bEchoToStdOut);
  ...
  bool getStatisticsVolumeInfoFile(
    ...
    try {
      ...
      statisticsVolumeInfoConfigFile.init(strStatisticsVolumePath,
                                          ios::nocreate);
      statisticsVolumeInfoConfigFile.get(nNumSubjects, "StatisticsVolumeInfo",
                                         "numberOfSubjects");
      statisticsVolumeInfoConfigFile.get(arr2DstrTrainedScans,
                                         "TrainedSubjectsAndMeasures", "");
      statisticsVolumeInfoConfigFile.get(nNumSlices, "StatisticsVolumeInfo",
                                         "numberOfSlices");
      statisticsVolumeInfoConfigFile.get(nNumMeasures, "StatisticsVolumeInfo",
                                         "numberOfMeasures");
      ...
      statisticsVolumeInfoConfigFile.get(strLabelLUTPath,
                                         "StatisticsVolumeInfo", "LabelLUT");
      statisticsVolumeInfoConfigFile.get(
          fSliceThickness, "StatisticsVolumeInfo", "fSliceThickness");
      statisticsVolumeInfoConfigFile.get(
          fSquarePixelSize, "StatisticsVolumeInfo", "fSquarePixelSize");
      statisticsVolumeInfoConfigFile.get(fStartX, "StatisticsVolumeInfo",
                                         "fStartX");
      statisticsVolumeInfoConfigFile.get(fEndX, "StatisticsVolumeInfo",
                                         "fEndX");
      statisticsVolumeInfoConfigFile.get(fStartY, "StatisticsVolumeInfo",
                                         "fStartY");
      statisticsVolumeInfoConfigFile.get(fEndY, "StatisticsVolumeInfo",
                                         "fEndY");
      statisticsVolumeInfoConfigFile.get(fStartZ, "StatisticsVolumeInfo",
                                         "fStartZ");
      statisticsVolumeInfoConfigFile.get(fEndZ, "StatisticsVolumeInfo",
                                         "fEndZ");
  ...
  int mapLabelToAFLT(int nLabel) { // AFLTNonWhite=1, AFLTWhite,
                                   // AFLTEditedWhite, AFLTEditedNonWhite
  ...
  bool nativeIndexToTalairachIndex(MRI *pNativeVolume, int nNiX, int nNiY,
                                   int nNiZ, float &fTiX, float &fTiY,
                                   float &fTiZ) {
    double realTrX;
    double realTrY;
    double realTrZ;
    MRIvoxelToTalairach(pNativeVolume, nNiX, nNiY, nNiZ, &realTrX, &realTrY,
                        &realTrZ); // Index->TalRAS
    double realTiX;
    double realTiY;
    double realTiZ;
    MRIworldToVoxel(pTalairachMRI, realTrX, realTrY, realTrZ, &realTiX,
                    &realTiY, &realTiZ); // RAS->Index
    fTiX = realTiX;
    fTiY = realTiY;
    fTiZ = realTiZ;
  ...
  // note this routine does NOT do bounds checking - this is done by the caller
  bool talairachIndexToNativeIndex(MRI *pNativeVolume, float fTiX, float fTiY,
                                   float fTiZ, float &fNiX, float &fNiY,
                                   float &fNiZ) {
    ...
    */
    ...
    double realTrY;
    double realTrZ;
    MRIvoxelToWorld(pTalairachMRI, fTiX, fTiY, fTiZ, &realTrX, &realTrY,
                    &realTrZ); // Tal Index -> Tal RAS
    double realNiX;
    double realNiY;
    double realNiZ;
    MRItalairachToVoxel(pNativeVolume, realTrX, realTrY, realTrZ, &realNiX,
                        &realNiY, &realNiZ);
  ...
  void mapPrevNext(float fCoordinate, int nMin, int nMax, float &fPrev,
                   float &fNext) {
  ...
  float round(float fValue) {
    return ((floor(fValue - 0.5) == floor(fValue)) ? ceil(fValue)
                                                   : floor(fValue));
  }
  ...
  // interpolated  value
  bool
  find8ClosestVoxels(float fTiX, float fTiY, float fTiZ,
  ...
  float interpolateLinear(
    ...
    fInterpolatedValue = fXFractionOfPrev * fYFractionOfPrev *
                             fZFractionOfPrev * vectorValueAtTriplets[0] +
                         fXFractionOfPrev * fYFractionOfPrev *
                             fZFractionOfNext * vectorValueAtTriplets[1] +
                         fXFractionOfPrev * fYFractionOfNext *
                             fZFractionOfPrev * vectorValueAtTriplets[2] +
                         fXFractionOfPrev * fYFractionOfNext *
                             fZFractionOfNext * vectorValueAtTriplets[3] +
                         fXFractionOfNext * fYFractionOfPrev *
                             fZFractionOfPrev * vectorValueAtTriplets[4] +
                         fXFractionOfNext * fYFractionOfPrev *
                             fZFractionOfNext * vectorValueAtTriplets[5] +
                         fXFractionOfNext * fYFractionOfNext *
                             fZFractionOfPrev * vectorValueAtTriplets[6] +
                         fXFractionOfNext * fYFractionOfNext *
                             fZFractionOfNext * vectorValueAtTriplets[7];
  ...
  // Returns a map from label (uchar) to fractional volume that the given
  // canonical space voxel is labeled with the label
  //  (labels which do not label the voxel are not included in the map)
  // Along with each label is a list of the Native space index coordinates of
  // which had that label. This is used when computing the measurement vector
  // which contributes to the conditional density
  bool AreasAndLabelsForVoxel(
      MRI *pLabeledVolume, float fTiX, float fTiY, float fTiZ,
    ...
    /* Strategy for computing the fractionalIntersectionVolume
      ...
      Project the 8 verticies of the Talairach voxel into native space
        These verticies are found by adding and subtracting .4999 from the
      Talairach index point These are projected by mapping them to Native space
      index coordinates using the talairach transform and scale factors in the
      mri struct Next the indexes of the smallest native index space
      rectanguloid which bounds the talairach voxel is found These indexes are
      found by rounding the projected coordinates to the indexes of the nearest
      native space voxel Next boundary conditions are handled by moving points
      less than the minimum to the minimum native space index coordinate and by
      moving points more than the maximum to the maximum natice space index
      coordinate Next the largest and smallest native space index coordinate is
      found in ea dim. x,y,z Next these native space index coord bound are used
      to bound an integral over the labeled volume which is used to approximate
      the amount of volume with each label (from calc we have that as the label
      resolution goes to infinity the error goes to zero) Thus we 1) Count the
      number of Native space voxel-labels with each label 2) Compute fractional
      volume by normalizing by the number of voxels in the rectanguloid

      Notes:
        For talairach voxel centroids which map to a location outside the bounds
    ...
    talairachIndexToNativeIndex(pLabeledVolume, fTiX, fTiY, fTiZ, fNiX, fNiY,
                                fNiZ);
    if (!inBounds(pLabeledVolume->width, pLabeledVolume->height,
                  pLabeledVolume->depth, fNiX, fNiY, fNiZ)) {
      bReturnValue = false;
    } else // corresponding Ni point is in bounds
    {

      talairachIndexToNativeIndex(pLabeledVolume, fTiX - AREA_FACTOR,
                                  fTiY - AREA_FACTOR, fTiZ - AREA_FACTOR,
                                  arr1DXPoints[0], arr1DYPoints[0],
                                  arr1DZPoints[0]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX - AREA_FACTOR,
                                  fTiY - AREA_FACTOR, fTiZ + AREA_FACTOR,
                                  arr1DXPoints[1], arr1DYPoints[1],
                                  arr1DZPoints[1]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX - AREA_FACTOR,
                                  fTiY + AREA_FACTOR, fTiZ - AREA_FACTOR,
                                  arr1DXPoints[2], arr1DYPoints[2],
                                  arr1DZPoints[2]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX - AREA_FACTOR,
                                  fTiY + AREA_FACTOR, fTiZ + AREA_FACTOR,
                                  arr1DXPoints[3], arr1DYPoints[3],
                                  arr1DZPoints[3]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX + AREA_FACTOR,
                                  fTiY - AREA_FACTOR, fTiZ - AREA_FACTOR,
                                  arr1DXPoints[4], arr1DYPoints[4],
                                  arr1DZPoints[4]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX + AREA_FACTOR,
                                  fTiY - AREA_FACTOR, fTiZ + AREA_FACTOR,
                                  arr1DXPoints[5], arr1DYPoints[5],
                                  arr1DZPoints[5]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX + AREA_FACTOR,
                                  fTiY + AREA_FACTOR, fTiZ - AREA_FACTOR,
                                  arr1DXPoints[6], arr1DYPoints[6],
                                  arr1DZPoints[6]);
      talairachIndexToNativeIndex(pLabeledVolume, fTiX + AREA_FACTOR,
                                  fTiY + AREA_FACTOR, fTiZ + AREA_FACTOR,
                                  arr1DXPoints[7], arr1DYPoints[7],
                                  arr1DZPoints[7]);
      ...
      // the indexes of the smallest native index space rectanguloid which
      // bounds the talairach voxel is found
      for (int i = 0; i < 8; i++) {
        arr1DXPoints[i] = round(arr1DXPoints[i]);
        arr1DYPoints[i] = round(arr1DYPoints[i]);
        arr1DZPoints[i] = round(arr1DZPoints[i]);

        if (arr1DXPoints[i] < MIN_NATIVE_INDEX_X)
          arr1DXPoints[i] = MIN_NATIVE_INDEX_X;
        if (arr1DXPoints[i] > MAX_NATIVE_INDEX_X)
          arr1DXPoints[i] = MAX_NATIVE_INDEX_X;
        if (arr1DYPoints[i] < MIN_NATIVE_INDEX_Y)
          arr1DYPoints[i] = MIN_NATIVE_INDEX_Y;
        if (arr1DYPoints[i] > MAX_NATIVE_INDEX_Y)
          arr1DYPoints[i] = MAX_NATIVE_INDEX_Y;
        if (arr1DZPoints[i] < MIN_NATIVE_INDEX_Z)
          arr1DZPoints[i] = MIN_NATIVE_INDEX_Z;
        if (arr1DZPoints[i] > MAX_NATIVE_INDEX_Z)
          arr1DZPoints[i] = MAX_NATIVE_INDEX_Z;
        ...
        if (i == 0) {
          ...
          if (arr1DXPoints[i] > fMaxX)
            fMaxX = arr1DXPoints[i];
          if (arr1DXPoints[i] < fMinX)
            fMinX = arr1DXPoints[i];
          if (arr1DYPoints[i] > fMaxY)
            fMaxY = arr1DYPoints[i];
          if (arr1DYPoints[i] < fMinY)
            fMinY = arr1DYPoints[i];
          if (arr1DZPoints[i] > fMaxZ)
            fMaxZ = arr1DZPoints[i];
          if (arr1DZPoints[i] < fMinZ)
            fMinZ = arr1DZPoints[i];
        }
      }
      ...
      int nNumNiVoxelsInTightestBoundingRectanguloid =
          (int)((fMaxX - fMinX + 1) * (fMaxY - fMinY + 1) *
                (fMaxZ - fMinZ + 1));
      if (nNumNiVoxelsInTightestBoundingRectanguloid <= 0)
        nNumNiVoxelsInTightestBoundingRectanguloid = 1;
      ...
      for (int nNiX = (int)fMinX; nNiX <= fMaxX; nNiX++)
        for (int nNiY = (int)fMinY; nNiY <= fMaxY; nNiY++)
          for (int nNiZ = (int)fMinZ; nNiZ <= fMaxZ; nNiZ++) {
            ...
            if ((it = mapFractionalAreaAndPoints.find(ucharLabel)) ==
                mapFractionalAreaAndPoints.end()) {
              ...
              mapFractionalAreaAndPoints.insert(
                  TypeMapFractionalAreaAndPoints::value_type(
                      ucharLabel, fractionalAreaAndPoints));
            } else {
              it->second.fFractionalArea =
                  it->second.fFractionalArea +
                  1; // use fractional area to count the number of voxels with
                     // the same label
              it->second.vectorNativeIndexPoints.push_back(aPoint);
            }
          }

      // for ea label compute an approx fraction of volume intersection with the
      // canonical voxel
      for (it = mapFractionalAreaAndPoints.begin();
           it != mapFractionalAreaAndPoints.end(); it++) {
        it->second.fFractionalArea =
            (float)it->second.fFractionalArea /
  ...
  bool setLabeledVolumeSourceType(
      LabeledVolumeSourceType enumNewLabeledVolumeSourceType) {
