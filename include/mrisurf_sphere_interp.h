...
public:
  ...
  inline Vec3 operator+(const Vec3 &vect) const {
    return Vec3(v[0] + vect[0], v[1] + vect[1], v[2] + vect[2]);
  }
  inline Vec3 operator-(const Vec3 &vect) const {
    return Vec3(v[0] - vect[0], v[1] - vect[1], v[2] - vect[2]);
  }
  inline double operator*(const Vec3 &vect) const {
...
inline Vec3 operator*(const double scalar, const Vec3 vect) {
  return vect * scalar;
}
...
private:
  ...
  bool searchBucket(int bx, int by, int bz, float x, float y, float z,
                    float *value);
