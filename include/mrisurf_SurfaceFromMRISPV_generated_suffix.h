...
void Face::set_marked(int to) { repr->f_marked[idx] = to; }
...
float Vertex::z() const { return repr->v_z[idx]; }
float Vertex::origx()
...
float Vertex::cx() const { return repr->v_cx[idx]; }
float Vertex::cy() const { return repr->v_cy[idx]; }
...
void Vertex::set_cx(float to) { repr->v_cx[idx] = to; }
void Vertex::set_cy(float to) { repr->v_cy[idx] = to; }
...
float Vertex::nx() const { return repr->v_nx[idx]; }
float Vertex::ny() const { return repr->v_ny[idx]; }
...
void Vertex::set_nx(float to) { repr->v_nx[idx] = to; }
void Vertex::set_ny(float to) { repr->v_ny[idx] = to; }
...
float Vertex::pnx() const { return repr->v_pnx[idx]; }
float Vertex::pny() const { return repr->v_pny[idx]; }
float Vertex::pnz() const { //  pial normal
  return repr->v_pnz[idx];
}
float Vertex::wnx() const { return repr->v_wnx[idx]; }
float Vertex::wny() const { return repr->v_wny[idx]; }
float Vertex::wnz() const { //  white normal
  return repr->v_wnz[idx];
}
float Vertex::onx() const { return repr->v_onx[idx]; }
float Vertex::ony() const { return repr->v_ony[idx]; }
float Vertex::onz() const { //  original normal
  return repr->v_onz[idx];
}
float Vertex::dx() const { return repr->v_dx[idx]; }
float Vertex::dy() const { return repr->v_dy[idx]; }
float Vertex::dz() const { //  current change in position
  return repr->v_dz[idx];
}
float Vertex::odx() const { return repr->v_odx[idx]; }
float Vertex::ody() const { return repr->v_ody[idx]; }
float Vertex::odz() const { //  last change of position (for momentum,
  return repr->v_odz[idx];
}
float Vertex::tdx() const { return repr->v_tdx[idx]; }
float Vertex::tdy() const { return repr->v_tdy[idx]; }
...
float Vertex::curvbak() const { return repr->v_curvbak[idx]; }
...
float Vertex::tx() const { return repr->v_tx[idx]; }
float Vertex::ty() const { return repr->v_ty[idx]; }
float Vertex::tz() const { //  tmp coordinate storage
  return repr->v_tz[idx];
}
float Vertex::t2x() const { return repr->v_t2x[idx]; }
float Vertex::t2y() const { return repr->v_t2y[idx]; }
float Vertex::t2z() const { //  another tmp coordinate storage
  return repr->v_t2z[idx];
}
float Vertex::targx() const { return repr->v_targx[idx]; }
float Vertex::targy() const { return repr->v_targy[idx]; }
float Vertex::targz() const { //  target coordinates
  return repr->v_targz[idx];
}
float Vertex::pialx() const { return repr->v_pialx[idx]; }
float Vertex::pialy() const { return repr->v_pialy[idx]; }
float Vertex::pialz() const { //  pial surface coordinates
  return repr->v_pialz[idx];
}
float Vertex::whitex() const { return repr->v_whitex[idx]; }
float Vertex::whitey() const { return repr->v_whitey[idx]; }
float Vertex::whitez() const { //  white surface coordinates
  return repr->v_whitez[idx];
}
float Vertex::l4x() const { return repr->v_l4x[idx]; }
float Vertex::l4y() const { return repr->v_l4y[idx]; }
float Vertex::l4z() const { //  layerIV surface coordinates
  return repr->v_l4z[idx];
}
float Vertex::infx() const { return repr->v_infx[idx]; }
float Vertex::infy() const { return repr->v_infy[idx]; }
float Vertex::infz() const { //  inflated coordinates
  return repr->v_infz[idx];
}
float Vertex::fx() const { return repr->v_fx[idx]; }
float Vertex::fy() const { return repr->v_fy[idx]; }
float Vertex::fz() const { //  flattened coordinates
  return repr->v_fz[idx];
}
int Vertex::px() const { return repr->v_px[idx]; }
int Vertex::qx() const { return repr->v_qx[idx]; }
int Vertex::py() const { return repr->v_py[idx]; }
int Vertex::qy() const { return repr->v_qy[idx]; }
int Vertex::pz() const { return repr->v_pz[idx]; }
int Vertex::qz() const { //  rational coordinates for exact calculations
  return repr->v_qz[idx];
}
float Vertex::e1x() const { return repr->v_e1x[idx]; }
float Vertex::e1y() const { return repr->v_e1y[idx]; }
float Vertex::e1z() const { //  1st basis vector for the local tangent plane
  return repr->v_e1z[idx];
}
float Vertex::e2x() const { return repr->v_e2x[idx]; }
float Vertex::e2y() const { return repr->v_e2y[idx]; }
float Vertex::e2z() const { //  2nd basis vector for the local tangent plane
  return repr->v_e2z[idx];
}
float Vertex::pe1x() const { return repr->v_pe1x[idx]; }
float Vertex::pe1y() const { return repr->v_pe1y[idx]; }
float Vertex::pe1z() const { //  1st basis vector for the local tangent plane
  return repr->v_pe1z[idx];
}
float Vertex::pe2x() const { return repr->v_pe2x[idx]; }
float Vertex::pe2y() const { return repr->v_pe2y[idx]; }
...
char Vertex::oripflag() const { return repr->v_oripflag[idx]; }
...
float Vertex::theta() const { return repr->v_theta[idx]; }
float Vertex::phi() const { //  parameterization
  return repr->v_phi[idx];
}
float Vertex::area() const { return repr->v_area[idx]; }
float Vertex::origarea() const { return repr->v_origarea[idx]; }
float Vertex::group_avg_area() const { return repr->v_group_avg_area[idx]; }
...
float Vertex::k1() const { return repr->v_k1[idx]; }
float Vertex::k2() const { //  the principal curvatures
  return repr->v_k2[idx];
}
float Vertex::mean() const { return repr->v_mean[idx]; }
...
void Vertex::set_pnx(float to) { repr->v_pnx[idx] = to; }
void Vertex::set_pny(float to) { repr->v_pny[idx] = to; }
void Vertex::set_pnz(float to) { //  pial normal
  repr->v_pnz[idx] = to;
}
void Vertex::set_wnx(float to) { repr->v_wnx[idx] = to; }
void Vertex::set_wny(float to) { repr->v_wny[idx] = to; }
void Vertex::set_wnz(float to) { //  white normal
  repr->v_wnz[idx] = to;
}
void Vertex::set_onx(float to) { repr->v_onx[idx] = to; }
void Vertex::set_ony(float to) { repr->v_ony[idx] = to; }
void Vertex::set_onz(float to) { //  original normal
  repr->v_onz[idx] = to;
}
void Vertex::set_dx(float to) { repr->v_dx[idx] = to; }
void Vertex::set_dy(float to) { repr->v_dy[idx] = to; }
void Vertex::set_dz(float to) { //  current change in position
  repr->v_dz[idx] = to;
}
void Vertex::set_odx(float to) { repr->v_odx[idx] = to; }
void Vertex::set_ody(float to) { repr->v_ody[idx] = to; }
void Vertex::set_odz(float to) { //  last change of position (for momentum,
  repr->v_odz[idx] = to;
}
void Vertex::set_tdx(float to) { repr->v_tdx[idx] = to; }
void Vertex::set_tdy(float to) { repr->v_tdy[idx] = to; }
...
void Vertex::set_curvbak(float to) { repr->v_curvbak[idx] = to; }
...
void Vertex::set_tx(float to) { repr->v_tx[idx] = to; }
void Vertex::set_ty(float to) { repr->v_ty[idx] = to; }
void Vertex::set_tz(float to) { //  tmp coordinate storage
  repr->v_tz[idx] = to;
}
void Vertex::set_t2x(float to) { repr->v_t2x[idx] = to; }
void Vertex::set_t2y(float to) { repr->v_t2y[idx] = to; }
void Vertex::set_t2z(float to) { //  another tmp coordinate storage
  repr->v_t2z[idx] = to;
}
void Vertex::set_targx(float to) { repr->v_targx[idx] = to; }
void Vertex::set_targy(float to) { repr->v_targy[idx] = to; }
void Vertex::set_targz(float to) { //  target coordinates
  repr->v_targz[idx] = to;
}
void Vertex::set_pialx(float to) { repr->v_pialx[idx] = to; }
void Vertex::set_pialy(float to) { repr->v_pialy[idx] = to; }
void Vertex::set_pialz(float to) { //  pial surface coordinates
  repr->v_pialz[idx] = to;
}
void Vertex::set_whitex(float to) { repr->v_whitex[idx] = to; }
void Vertex::set_whitey(float to) { repr->v_whitey[idx] = to; }
void Vertex::set_whitez(float to) { //  white surface coordinates
  repr->v_whitez[idx] = to;
}
void Vertex::set_l4x(float to) { repr->v_l4x[idx] = to; }
void Vertex::set_l4y(float to) { repr->v_l4y[idx] = to; }
void Vertex::set_l4z(float to) { //  layerIV surface coordinates
  repr->v_l4z[idx] = to;
}
void Vertex::set_infx(float to) { repr->v_infx[idx] = to; }
void Vertex::set_infy(float to) { repr->v_infy[idx] = to; }
void Vertex::set_infz(float to) { //  inflated coordinates
  repr->v_infz[idx] = to;
}
void Vertex::set_fx(float to) { repr->v_fx[idx] = to; }
void Vertex::set_fy(float to) { repr->v_fy[idx] = to; }
void Vertex::set_fz(float to) { //  flattened coordinates
  repr->v_fz[idx] = to;
}
void Vertex::set_px(int to) { repr->v_px[idx] = to; }
void Vertex::set_qx(int to) { repr->v_qx[idx] = to; }
void Vertex::set_py(int to) { repr->v_py[idx] = to; }
void Vertex::set_qy(int to) { repr->v_qy[idx] = to; }
void Vertex::set_pz(int to) { repr->v_pz[idx] = to; }
void Vertex::set_qz(int to) { //  rational coordinates for exact calculations
  repr->v_qz[idx] = to;
}
void Vertex::set_e1x(float to) { repr->v_e1x[idx] = to; }
void Vertex::set_e1y(float to) { repr->v_e1y[idx] = to; }
void Vertex::set_e1z(
    float to) { //  1st basis vector for the local tangent plane
  repr->v_e1z[idx] = to;
}
void Vertex::set_e2x(float to) { repr->v_e2x[idx] = to; }
void Vertex::set_e2y(float to) { repr->v_e2y[idx] = to; }
void Vertex::set_e2z(
    float to) { //  2nd basis vector for the local tangent plane
  repr->v_e2z[idx] = to;
}
void Vertex::set_pe1x(float to) { repr->v_pe1x[idx] = to; }
void Vertex::set_pe1y(float to) { repr->v_pe1y[idx] = to; }
void Vertex::set_pe1z(
    float to) { //  1st basis vector for the local tangent plane
  repr->v_pe1z[idx] = to;
}
void Vertex::set_pe2x(float to) { repr->v_pe2x[idx] = to; }
void Vertex::set_pe2y(float to) { repr->v_pe2y[idx] = to; }
...
void Vertex::set_oripflag(char to) { repr->v_oripflag[idx] = to; }
...
void Vertex::set_theta(float to) { repr->v_theta[idx] = to; }
void Vertex::set_phi(float to) { //  parameterization
  repr->v_phi[idx] = to;
}
void Vertex::set_origarea(float to) { repr->v_origarea[idx] = to; }
void Vertex::set_group_avg_area(float to) { repr->v_group_avg_area[idx] = to; }
...
void Vertex::set_k1(float to) { repr->v_k1[idx] = to; }
void Vertex::set_k2(float to) { //  the principal curvatures
  repr->v_k2[idx] = to;
}
void Vertex::set_mean(float to) { repr->v_mean[idx] = to; }
void Vertex::set_mean_imag(float to) { //  imaginary part of complex statistic
  repr->v_mean_imag[idx] = to;
}
void Vertex::set_std_error(float to) { repr->v_std_error[idx] = to; }
void Vertex::set_flags(uint to) { repr->v_flags[idx] = to; }
void Vertex::set_fno(int to) { //  face that this vertex is in
  repr->v_fno[idx] = to;
}
void Vertex::set_cropped(int to) { repr->v_cropped[idx] = to; }
void Vertex::set_marked(short to) { //  for a variety of uses
  repr->v_marked[idx] = to;
}
void Vertex::set_marked2(short to) { repr->v_marked2[idx] = to; }
void Vertex::set_marked3(short to) { repr->v_marked3[idx] = to; }
...
void Vertex::set_z(float to) { repr->v_z[idx] = to; }
void Vertex::set_cx(float to) { repr->v_cx[idx] = to; }
void Vertex::set_cy(float to) { repr->v_cy[idx] = to; }
void Vertex::set_cz(float to) { //  coordinates in canonical coordinate system
  repr->v_cz[idx] = to;
...
void Face::set_angle(angles_per_triangle_t to) { repr->f_angle[idx] = to; }
void Face::set_orig_angle(angles_per_triangle_t to) {
  repr->f_orig_angle[idx] = to;
}
void Face::set_ripflag(char to) { repr->f_ripflag[idx] = to; }
void Face::set_oripflag(char to) { repr->f_oripflag[idx] = to; }
void Face::set_marked(int to) { repr->f_marked[idx] = to; }
void Face::set_norm(PDMATRIX to) { repr->f_norm[idx] = to; }
void Face::set_gradNorm(A3PDMATRIX to) { repr->f_gradNorm[idx] = to; }
...
void Vertex::set_area(float to) { repr->v_area[idx] = to; }
void Vertex::set_ripflag(char to) { //  vertex no longer exists - placed last to
                                    //  load the next vertex into cache
  repr->v_ripflag[idx] = to;
}
