...
MRI_SURFACE *MRISoverAlloc(int max_vertices, int max_faces, int nvertices,
                           int nfaces);
...
MRIS *MRIScreateWithSimilarTopologyAsSubset(
    ...
    int const *
           mapToDstVno, // src->nvertices entries, with the entries being -1 (vertex should be ignored) or the vno within the dst surface the face maps to
    size_t nfaces,      // the mapToDstFno entries must each be less than this
    int const *
        mapToDstFno); // src->nfaces entries, with the entries being -1 (face should be ignored) or the fno within the dst surface the face maps to
                      //
...
void MRIScreateSimilarTopologyMapsForNonripped(
    ...
    int const **
            pmapToDstVno, // src->nvertices entries, with the entries being -1 (vertex should be ignored) or the vno within the dst surface the face maps to
    size_t *pnfaces,      // the mapToDstFno entries must each be less than this
    int const **
        pmapToDstFno); // src->nfaces entries, with the entries being -1 (face should be ignored) or the fno within the dst surface the face maps to
...
int MRIS_acquireTemp(
    MRIS *mris, MRIS_TempAssigned temp); // save the result to use later to ...
void MRIS_checkAcquiredTemp(
    MRIS *mris, MRIS_TempAssigned temp,
    int MRIS_acquireTemp_result); // ... check that you own it
void MRIS_releaseTemp(
    MRIS *mris, MRIS_TempAssigned temp,
    int MRIS_acquireTemp_result); // ... be allowed to release it
...
#define IPFLAG_PRESERVE_TOPOLOGY_CONVEXHULL                                    \
  0x1000 /* apply topology
preserving gradient */
#define IPFLAG_PRESERVE_SPHERICAL_POSITIVE_AREA                                \
  0x2000 /* apply gradients
that preserve
positive areas */
...
public:
  ...
  float
        l_pangle; /* coefficient of "positive" angle term - only penalize narrower angles */
  ...
  float
      l_thick_normal; // coefficient to keep vector field close to surface normal
  float
  ...
  float
      l_spring_nzr; /* coefficient of spring term with non-zero resting length*/
  float
        l_spring_nzr_len; /* resting length of spring term with non-zero resting length*/
  ...
  FILE *const
      fp; /* for logging results, write by calling INTEGRATION_PARMS_<various> functions */
...
#define MNOFIV                                         MAX_NUMBER_OF_FIELDS_IN_VECTORIAL_REGISTRATION
  ...
  HISTOGRAM2D *h2d;
  MRI *
         mri_volume_fractions; // the partial volume fractions associated with the boundaries in this mris
  ...
  INTEGRATION_PARMS(FILE *file = nullptr)
      : tol(0), l_angle(0), l_pangle(0), l_area(0), l_parea(0), l_nlarea(0),
        l_nldist(0), l_thick_normal(0), l_thick_spring(0),
        l_ashburner_triangle(0), l_ashburner_lambda(0), l_corr(0), l_ocorr(0),
        l_pcorr(0), l_curv(0), l_norm(0), l_scurv(0), l_lap(0), l_link(0),
        l_spring(0), l_nlspring(0), l_max_spring(0), l_spring_norm(0),
        l_tspring(0), l_nltspring(0), l_nspring(0), l_spring_nzr(0),
        l_spring_nzr_len(0), l_hinge(0), l_repulse(0), l_repulse_ratio(0),
        l_boundary(0), l_dist(0), l_location(0), l_neg(0), l_intensity(0),
        l_sphere(0), l_expand(0), l_grad(0), l_convex(0), l_tsmooth(0),
        l_surf_repulse(0), l_osurf_repulse(0), l_external(0),
        l_thick_parallel(0), l_thick_min(0), l_shrinkwrap(0), l_expandwrap(0),
        l_unfold(0), l_dura(0), l_histo(0), l_map(0), l_map2d(0),
        dura_thresh(0), mri_dura(nullptr), n_averages(0), min_averages(0),
        first_pass_averages(0), nbhd_size(0), max_nbrs(0), write_iterations(0),
        ...
        nfields(0), fields(), /* Array should initialize to zero */
        ...
        abs_norm(0), grad_dir(0), fill_interior(0), rms(0),
        complete_dist_mat(0), nsubjects(0), nlabels(0), mht_array(nullptr),
        mris_array(nullptr), mris_ico(nullptr), mht(nullptr),
        smooth_averages(0), ico_order(0), remove_neg(0), mri_hires(nullptr),
        mri_hires_smooth(nullptr), mri_vno(nullptr), mri_template(nullptr),
        which_surface(0), trinarize_thresh(0), nonmax(0),
        smooth_intersections(0), uncompress(0), min_dist(0), h_wm(nullptr),
        h_gm(nullptr), h_nonbrain(nullptr), mri_labels(nullptr),
        mri_white(nullptr), mri_aseg(nullptr), hwm(nullptr), hgm(nullptr),
        hout(nullptr), h2d_wm(nullptr), h2d_gm(nullptr), h2d_out(nullptr),
        h2d(nullptr), mri_volume_fractions(nullptr), mri_dtrans(nullptr),
        resolution(0), target_intensity(0), stressthresh(0), explode_flag(0) {}
};
...
int  MRISrigidBodyAlignVectorGlobal(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,
...
MRI *MRISbinarizeVolume(MRI_SURFACE *mris, MRI_REGION *region, float resolution,
...
int    MRISreadNewCurvatureIntoArray(const char *fname, int in_array_size,
                                     float **out_array);
float *MRISreadCurvatureVector(MRI_SURFACE *mris, const char *sname);
int    MRISreadCurvatureIntoArray(const char *fname, int in_array_size,
                                  float **out_array);
...
int MRISorigVertexToVoxel(MRI_SURFACE *, VERTEX *v, MRI *mri, double *pxv,
                          double *pyv, double *pzv);
int MRISwhiteVertexToVoxel(MRI_SURFACE *, VERTEX *v, MRI *mri, double *pxv,
                           double *pyv, double *pzv);
int MRISvertexToVoxel(MRI_SURFACE *, VERTEX *v, MRI *mri, double *pxv,
                      double *pyv, double *pzv);
int MRISvertexCoordToVoxel(MRI_SURFACE *, VERTEX *v, MRI *mri, int coord,
...
void MRIS_useRAS2VoxelMap(
    MRIS_SurfRAS2VoxelMap *cache_nonconst, // accesses cache thread safely
    MRI const *const mri, double r, double a, double s, double *px, double *py,
    double *pz);
...
MRIS_SurfRAS2VoxelMap *MRIS_makeRAS2VoxelMap( // not thread safe
    MRI const *const mri, MRI_SURFACE const *const mris);
void                   MRIS_freeRAS2VoxelMap(
                      MRIS_SurfRAS2VoxelMap **const cachePtr); // not thread safe
...
int MRISsurfaceRASToTalairachVoxel(MRI_SURFACE *mris, MRI *mri, double xw,
                                   double yw, double zw, double *pxv,
                                   double *pyv, double *pzv);
...
int          MRISregister(MRI_SURFACE *mris, MRI_SP *mrisp_template,
                          INTEGRATION_PARMS *parms, int max_passes, float min_degrees,
                          float max_degrees, int nangles);
int MRISrigidBodyAlignLocal(MRI_SURFACE *mris, INTEGRATION_PARMS *parms);
int MRISrigidBodyAlignGlobal(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,
                             float min_degrees, float max_degrees, int nangles);
...
MRI_SURFACE *MRISremoveNegativeVertices(MRI_SURFACE *      mris,
                                        INTEGRATION_PARMS *parms, int min_neg,
                                        float min_neg_pct);
...
int MRIScomputeEulerNumber(MRI_SURFACE *mris, int *pnvertices, int *pnfaces,
                           int *pnedges);
int MRIStopologicalDefectIndex(MRI_SURFACE *mris);
int MRISremoveTopologicalDefects(MRI_SURFACE *mris, float curv_thresh);

int  MRIScomputeSecondFundamentalFormAtVertex(MRI_SURFACE *mris, int vno,
...
int MRISspringTermWithGaussianCurvature(MRI_SURFACE *mris, double gaussian_norm,
                                        double l_spring);
int MRISmarkedSpringTerm(MRI_SURFACE *mris, double l_spring);
double MRISmomentumTimeStep(MRI_SURFACE *mris, float momentum, float dt,
...
void MRISPfunctionVal_radiusR( // returns the value that would be stored in resultsForEachFno[0] for fnoLo
    ...
    MRISPfunctionValResultForAlpha
        * resultsForEachAlpha, // must be numAlphas elements
    float r, float x, float y, float z, int fno,
    bool
                 getNextAlso, // always fills in resultsForEachAlpha.curr for fno, optionally fills in .next for fno+1
    const float *alphas,
    float
        numAlphas, // rotate x,y,z by these alphas (radians) and get the values
    bool
        trace); // note: this rotation is around the z axis, hence z does not change

double MRISPfunctionValTraceable(MRI_SURFACE_PARAMETERIZATION *mrisp,
                                 float desired_radius, float x, float y,
                                 float z, int fno, bool trace);
double MRISPfunctionVal(MRI_SURFACE_PARAMETERIZATION *mrisp,
                        float desired_radius, float x, float y, float z,
                        int fno);

MRI_SP *     MRIStoParameterizationBarycentric(MRI_SURFACE *mris, MRI_SP *mrisp,
                                               float scale, int fno);
MRI_SURFACE *MRISfromParameterizationBarycentric(MRI_SP *     mrisp,
                                                 MRI_SURFACE *mris, int fno);

MRI_SP *MRIStoParameterization(MRI_SURFACE *mris, MRI_SP *mrisp, float scale,
                               int fno);
MRI_SURFACE *MRISfromParameterization(MRI_SP *mrisp, MRI_SURFACE *mris,
                                      int fno);
MRI_SURFACE *MRISnormalizeFromParameterization(MRI_SP *mrisp, MRI_SURFACE *mris,
                                               int fno);
...
MRI_SP *MRISPalign(MRI_SP *mrisp_orig, MRI_SP *mrisp_src, MRI_SP *mrisp_tmp,
                   MRI_SP *mrisp_dst);
...
int MRISmeasureThicknessFromCorrespondence(MRI_SURFACE *mris, MHT *mht,
                                           float max_thick);
int MRISfindClosestOrigVertices(MRI_SURFACE *mris, int nbhd_size);
int MRISfindClosestPialVerticesCanonicalCoords(MRI_SURFACE *mris,
                                               int          nbhd_size);
...
int    MRISreverseCoords(MRI_SURFACE *mris, int which_direction,
                         int reverse_face_order, int which_coords);
...
int MRISaccumulateImaginaryMeansOnSurface(MRI_SURFACE *mris, int total_dof,
                                          int new_dof);
int MRISaccumulateStandardErrorsOnSurface(MRI_SURFACE *mris, int total_dof,
                                          int new_dof);
#define GRAY_WHITE 1
#define GRAY_CSF   2
...
int MRIScomputeInvertedPialBorderValues(MRI_SURFACE *mris, MRI *mri_brain,
                                        ...
                                        FILE *log_fp);
int MRIScomputeBorderValues(MRI_SURFACE *mris, MRI *mri_brain, MRI *mri_smooth,
                            double inside_hi, double border_hi,
                            double border_low, double outside_low,
                            double outside_hi, double sigma, float max_dist,
                            FILE *log_fp, int white, MRI *mri_mask,
...
int MRIStransform(MRI_SURFACE *mris, MRI *mri, TRANSFORM *transform,
                  MRI *mri_dst);
...
double MRIScomputeTotalVertexSpacingStats(MRI_SURFACE *mris, double *psigma,
                                          double *pmin, double *pmax, int *pvno,
                                          int *pvno2);
double MRIScomputeFaceAreaStats(MRI_SURFACE *mris, double *psigma, double *pmin,
                                double *pmax);
int    MRISprintTessellationStats(MRI_SURFACE *mris, FILE *fp);
int    MRISprintVertexStats(MRI_SURFACE *mris, int vno, FILE *fp,
                            int which_vertices);
...
int MRISextractVertexCoords(MRI_SURFACE *mris, float *locations[3],
                            int which_vertices);
int MRISimporttVertexCoords(MRI_SURFACE *mris, float *locations[3],
                            int which_vertices);
...
int    MRISextractCurvatureVectorDouble(MRI_SURFACE *mris, double *curvs,
                                        int offset);
...
float MRISdistanceToSurface(MRI_SURFACE *mris, MHT *mht, float x0, float y0,
                            float z0, float nx, float ny, float nz);
...
MRI *MRISpartialaccentuate(MRI *mri_src, MRI *mri_dst, int lo_thresh,
                           int hi_thresh);
...
int          MRISPfunctionVectorVals(MRI_SURFACE_PARAMETERIZATION *mrisp,
                                     MRI_SURFACE *mris, float x, float y, float z,
                                     int *frames, int nframes, double *vals);
MRI_SURFACE *MRISfromParameterizations(MRI_SP *mrisp, MRI_SURFACE *mris,
                                       int *frames, int *indices, int nframes);
MRI_SP *MRIStoParameterizations(MRI_SURFACE *mris, MRI_SP *mrisp, float scale,
                                int *frames, int *indices, int nframes);
MRI_SP *MRISPblurFrames(MRI_SP *mrisp_src, MRI_SP *mrisp_dst, float sigma,
...
int  MRIScombine(MRIS *mris_src, MRIS *mris_total, MRIS_HASH_TABLE *mht,
                 int which);
...
MRI *MRISsmoothMRI(MRIS *Surf, MRI *Src, int nSmoothSteps, MRI *IncMask,
                   MRI *Targ);
...
MRI *MRISdilateConfined(MRIS *surf, MRI *mask, int annotidmask, int niters,
                        int newid);
...
MRIS * MRISfromVerticesAndFaces(const float *vertices, int nvertices,
...
int    MRISbinarizeCurvature(MRI_SURFACE *mris, float thresh, float low,
                             float high, int use_abs);
int    MRISsetVal2(MRI_SURFACE *mris, float val);
MRI *  MRIScomputeDistanceToSurface(MRI_SURFACE *mris, MRI *mri,
                                    float resolution);
...
int MRISminMaxCurvatureIndicesLookup(MRI_SURFACE *apmris, int *ap_vertexMin,
                                     int *ap_vertexMax);

int MRISvertexCurvature_set(MRI_SURFACE *apmris, int aindex, float af_val);

int MRISzeroCurvature(MRI_SURFACE *apmris);
...
// thickness stuff
int MRISminimizeThicknessFunctional(MRI_SURFACE *mris, INTEGRATION_PARMS *parms,
                                    float max_thick);
int MRISmeasureDistanceBetweenSurfaces(MRI_SURFACE *mris, MRI_SURFACE *mris2,
                                       int signed_dist);
int MRISwriteCoordsToIco(MRI_SURFACE *mris, MRI_SURFACE *mris_ico,
                         int which_vertices);

#define MRISvertexCoord2XYZ_float  MRISvertexCoord2XYZ
#define MRISvertexCoord2XYZ_double MRISvertexCoord2XYZ
int MRISvertexCoord2XYZ(VERTEX const *v, int which, float *x, float *y,
                        float *z);
int MRISvertexCoord2XYZ(VERTEX const *v, int which, double *x, double *y,
                        double *z);
...
int MRISsampleFaceNormal(MRI_SURFACE *mris, int fno, double x, double y,
                         double z, float *px, float *py, float *pz);
int MRISsampleFaceCoordsCanonical(MHT *mht, MRI_SURFACE *mris, float x, float y,
...
int    MRISsampleFaceCoords(MRI_SURFACE *mris, int fno, double x, double y,
                            double z, int which_coords, int which_barcyentric,
...
double MRISsampleFace(MRI_SURFACE *mris, int fno, int which, double x, double y,
                      double z, double val0, double val1, double val2);
int    MRISrepositionSurface(MRI_SURFACE *mris, MRI *mri, int *target_vnos,
                             float *target_vals, int nv, int nsize, double sigma,
                             int flags);
int    MRISrepositionSurfaceToCoordinate(MRI_SURFACE *mris, MRI *mri,
                                         int target_vno, float tx, float ty,
                                         float tz, int nsize, double sigma,
                                         int flags);

MRI *MRIScomputeFlattenedVolume(MRI_SURFACE *mris, MRI *mri, double res,
                                int nsamples, int normalize,
                                MRI **pmri_vertices, int smooth_iters,
...
int  MRIStaubinSmooth(MRI_SURFACE *mris, int niters, double lambda, double mu,
                      int which);
MRI_SURFACE *MRISconcat(MRI_SURFACE *mris1, MRI_SURFACE *mris2,
                        MRI_SURFACE *mris);
...
int MRISvertexNormalToVoxelScaled(MRI_SURFACE *mris, VERTEX *v, MRI *mri,
                                  double *pnx, double *pny, double *pnz);
int MRISvertexNormalToVoxel(MRI_SURFACE *mris, VERTEX *v, MRI *mri, double *pnx,
                            double *pny, double *pnz);
MRI * MRIcomputeLaminarVolumeFractions(MRI_SURFACE *mris, double res,
                                       MRI *mri_src, MRI *mri_vfracs);
MRIS *MRIStessellate(MRI *mri, int value, int all_flag);
void  TESSaddFace(MRI *mri, int imnr, int i, int j, int f, int prev_flag,
                  int *pface_index, tface_type *face, int *face_index_table0,
                  int *face_index_table1);
void  TESScheckFace(MRI *mri, int im0, int i0, int j0, int im1, int i1, int j1,
                    int f, int n, int v_ind, int prev_flag, int all_flag,
                    int value, tface_type *face, int *pface_index,
                    int *face_index_table0, int *face_index_table1,
                    tvertex_type *vertex);
int   TESSaddVertex(MRI *mri, int imnr, int i, int j, int *pvertex_index,
                    int *vertex_index_table, tvertex_type *vertex);
int   TESSfacep(MRI *mri, int im0, int i0, int j0, int im1, int i1, int j1,
                int value, int all_flag);
...
//
bool               mrisCheckVertexVertexTopologyWkr(const char *file, int line,
                                                    MRIS const *mris, bool always);
bool               mrisCheckVertexFaceTopologyWkr(const char *file, int line,
                                                  MRIS const *mris, bool always);
inline static bool returnTrue() { return true; };
#define mrisCheckVertexFaceTopology(_MRIS)                                     \
  returnTrue() // mrisCheckVertexFaceTopologyWkr  (__FILE__,__LINE__,_MRIS,false)
#define mrisCheckVertexVertexTopology(_MRIS)                                   \
  returnTrue() // mrisCheckVertexVertexTopologyWkr(__FILE__,__LINE__,_MRIS,false)
...
void MRISsetNeighborhoodSize(
    MRIS *mris, int nsize); // Doesn't compute distances if not already present
void MRISsetNeighborhoodSizeAndDist(MRIS *mris,
                                    int   nsize); // Always computes distances

int MRISfindNeighborsAtVertex(MRIS *mris, int vno, int nlinks,
                              size_t listCapacity, int *vlist, int *hops);
...
#define MRISsetXYZ(_MRIS, _VNO, _X, _Y, _Z)                                    \
  {                                                                            \
    ...
    MRISsetXYZwkr((_MRIS), (_VNO), (_X), (_Y), (_Z), __FILE__, __LINE__,       \
                  &_laterTime);                                                \
  }
//
...
void MRISblendXYZandTXYZ(
    MRIS *mris, float xyzScale,
    float txyzScale); // x = x*xyzScale + tx*txyzScale  etc.
void MRISblendXYZandNXYZ(
    MRIS *mris, float nxyzScale); // x = x          + nx*nxyzScale  etc.

void MRIStranslate_along_vertex_dxdydz(
    MRIS *dst, MRIS *src, double dt); // dst.x = src.x + src.dx*dt, etc.
...
void MRISmakeDistOrig(
    MRIS *mris, int vno); // makes it the same size as the current VERTEX.dist
void MRISgrowDistOrig(MRIS *mris, int vno,
                      int minimumCapacity); // same size as current or bigger
...
void setFaceAttachmentDeferred(
    MRIS *mris,
    bool
        to); // for performance reasons, defer adding them; or do all the deferred ones
int  mrisCountAttachedFaces(MRIS *mris, int vno0, int vno1);
bool mrisCanAttachFaceToVertices(
    MRIS *mris, int vno1, int vno2,
    int vno3); // returns whether such a face would be legal
void mrisAttachFaceToEdges(MRIS *mris, int fno, int vno1, int vno2,
                           int vno3); // edges must already exist
void mrisAttachFaceToVertices(MRIS *mris, int fno, int vno1, int vno2,
                              int vno3); // adds any needed edges
...
void MRISsetOriginalXYZwkr(MRIS *mris, int vno, float x, float y, float z,
                           const char *file, int line, bool *laterTime);
#define MRISsetOriginalXYZ(_MRIS, _VNO, _X, _Y, _Z)                            \
  {                                                                            \
    static bool laterTime;                                                     \
    MRISsetOriginalXYZwkr((_MRIS), (_VNO), (_X), (_Y), (_Z), __FILE__,         \
                          __LINE__, &laterTime);                               \
  }
//
...
#define mrisComputeOriginalVertexDistancesIfNecessary(_MRIS)                   \
  {                                                                            \
    static bool laterTime;                                                     \
    mrisComputeOriginalVertexDistancesIfNecessaryWkr((_MRIS), &laterTime,      \
                                                     __FILE__, __LINE__);      \
  }
...
int    MatlabPlotFace(FILE *fp, MRIS *surf, int faceno, char color,
                      double NormLen);
int MatlabPlotVertex(FILE *fp, MRIS *surf, int vno, char color, double NormLen);
int MatlabPlotVertexNbhd(FILE *fp, MRIS *surf, int cvno, int nhops, char color,
                         double NormLen);

int    MRISshrinkFaceCorner(MRIS *surf, int faceno, int nthv, double dist,
...
#if defined(COMPILING_MRISURF_TOPOLOGY) ||                                     \
    defined(COMPILING_MRISURF_TOPOLOGY_FRIEND_CHECKED)
...
#define CONST_EXCEPT_MRISURF_TOPOLOGY const
    ...
    defined(COMPILING_MRISURF_METRIC_PROPERTIES_FRIEND)
...
static short setVnum(MRIS const *mris, int vno, short to) {
  return modVnum(mris, vno, to, true);
}
static short clearVnum(MRIS const *mris, int vno) {
  return modVnum(mris, vno, 0, true);
}
static short vnumAdd(MRIS const *mris, int vno, short add = +1) {
  return modVnum(mris, vno, add, false) - add;
}
static short addVnum(MRIS const *mris, int vno, short add = -1) {
  return modVnum(mris, vno, add, false);
}
...
static int mrisVertexNeighborIndex(MRIS const *mris, int vno1, int vno2) {
  ...
  if (!vt->v)
    return -1; // happens in invalid surfaces
  int n;
  for (n = 0; n < vt->vnum; n++) {
    if (vt->v[n] == vno2)
      return n;
...
static bool mrisVerticesAreNeighbors(MRIS const *const mris, int const vno1,
                                     int const vno2) {
