/**
 * @file class_array.h
 * @brief prototypes and structs for using an array of classifiers to reclassify
 * aseg voxels
 *
 */
/*
...
float **CAbuildTrainingData(VOXEL_LIST *vl_total, int target_label,
                            float *classes, MRI **mri_smooth, MRI **mri_grad,
...
float * CAbuildInputsAtVoxel(VOXEL_LIST *vl, int i, MRI **mri_smooth,
                             ...
                             MRI **mri_2nd_deriv_s, int wsize, int nscales,
                             float *ca_inputs, int which_inputs);
...
#define NINPUTS(which_inputs, wsize, nscales)                                  \
  ((wsize * wsize * wsize) * nscales * ca_ninputs(which_inputs))
