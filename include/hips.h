...
using hips_byte = unsigned char;
using sbyte     = char;
using h_ushort  = unsigned short;
using h_uint    = unsigned int;
typedef float  h_complex[2];
typedef double h_dblcom[2];
using fs_hsize_t = unsigned long;
...
struct header {
  ...
  hips_byte *image;        /* The image itself */
  h_boolean  imdealloc;    /* if nonzero, free image when requested */
  hips_byte *firstpix;     /* Pointer to first pixel (for ROI) */
  int        sizehist; /* Number of bytes in history (excluding null, including
                           <newline>) */
  char *     seq_history; /* The sequence's history of transformations */
  h_boolean  histdealloc; /* If nonzero, free history when requested */
  int   sizedesc; /* Number of bytes in description (excluding null, including
                     <newline>) */
  ...
  h_boolean      paramdealloc; /* If nonzero, free param structures and/or param
...
struct extpar {
  ...
  union {
    hips_byte  v_b;  /* PFBYTE/PFASCII, count = 1 */
    int        v_i;  /* PFINT, count = 1 */
    short      v_s;  /* PFSHORT, count = 1 */
    float      v_f;  /* PFFLOAT, count = 1 */
    hips_byte *v_pb; /* PFBYT/PFASCIIE, count > 1 */
...
int    alloc_histo(struct hips_histo *histo, union pixelval *min,
                   union pixelval *max, int nbins, int format);
...
int h_minmax(struct header *hd, union pixelval *minval, union pixelval *maxval,
             int nzflag);
...
void canny(int *magmax, int *hthresh, int *lthresh, int *image, int *xsize,
           ...
           int *bordermode, double *hfrac, double *lfrac, int *pflag, short *gx,
           short *gy, short *mag, int *hist, int *histsize, unsigned char *nms,
...
void gauss_filter(short *inimage, int inx, int iny, int direction, int boundary,
                  int masksize, float sigma, short *grad, int *outx, int *outy,
