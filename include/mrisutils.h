...
typedef struct {
  ...
  double
       dmax; // max allowable distance in mm bet voxel center and surface vertex
  LTA *vol2surf;  // reg bet vol and surf, direction unimportant, set to null to
                  // use header reg
  ...
  int DilateWithinVoxel; // require neighboring vertices to be within voxel to
                         // include in label
  int DilateContiguous;  // require neighboring vertices to be continguous with
                         // the center vertex
  int debug;             // set to 1 to print out a bunch of stuff
  MATRIX
  *volcrs2surfxyz; // precomputed matrix converts vol CRS to surface tkRAS
...
void  MRIScenterCOG2(MRI_SURFACE *mris, double *xCOG, double *yCOG,
                     double *zCOG);
...
double *MRISsampleProfile(MRI_SURFACE *mris, MRI *mri, double *profile,
                          int nsamples, int wm_samples, int outside_samples,
                          double wx, double wy, double wz, double l4x,
                          double l4y, double l4z, double px, double py,
                          double pz);
