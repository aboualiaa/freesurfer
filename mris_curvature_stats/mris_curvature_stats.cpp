...
const char *Gppch[] = {"Raw",    "K",           "H",  "k1", "k2",
                       "S",      "C",           "SI", "BE", "Normalised",
                       "Scaled", "ScaledTrans", "FI"};
...
short MRIS_surfaceIntegral_compute(MRI_SURFACE *pmris, int *p_verticesCounted,
...
int comp(const void *p,
         const void *q); // Compare p and q for qsort()

void histogram_wrapper(MRIS *amris, e_secondOrderType aesot);

void histogram_create(MRIS *amris_curvature, float af_minCurv,
                      double af_binSize, int abins, float *apf_histogram,
                      e_secondOrderType aesot);

void OFSP_create(char *apch_prefix, char *apch_suffix, e_OFSP ae_OFSP);

void outputFileNames_create();
void outputFiles_open();
void outputFiles_close();

int MRIS_curvatureStats_analyze(MRIS *apmris, e_secondOrderType aesot);

int MRISminMaxCurvaturesSearchSOT(MRI_SURFACE *apmris, int *ap_vertexMin,
                                  int *ap_vertexMax, float *apf_min,
                                  float *apf_max, e_secondOrderType aesot);

int MRISminMaxCurvaturesSearch(MRI_SURFACE *apmris, int *ap_vertexMin,
                               int *ap_vertexMax, float *apf_min,
                               float *apf_max) {
  MRISminMaxCurvaturesSearchSOT(apmris, ap_vertexMin, ap_vertexMax, apf_min,
                                apf_max, e_Raw);
  return (NO_ERROR);
};

int MRISvertexAreaPostProcess(MRI_SURFACE *pmris);

int MRIS_surfaceRipFlags_filter(MRI_SURFACE *apmris, float *apf_notRippedArea);
...
void surface_applyFilter(MRI_SURFACE *apmris) {
  cprints("Scanning surface for labels and/or filters...", "");
  G_regionalTotalVertexCount =
      MRIS_surfaceRipFlags_filter(apmris, &Gf_regionalTotalSurfaceArea);
  cprints("", "ok");
  cprintd("Filtered vertices", G_regionalTotalVertexCount);
  cprintf("Filtered Area (mm^2)", Gf_regionalTotalSurfaceArea);
}

int LABEL_RipSurface(MRI_SURFACE *apmris, LABEL *aplbl) {
  //
  // PRECONDITIONS
...
int main(int argc, char *argv[]) {
  ...
  // mris_curvature_stats <str_subjectName> <str_hemi> [<str_curv0> ...
  // <str_curvN>]
  //    0   1   2     3     ...     N
  //
  // The <str_curvN> are not necessary if the Gaussian curvatures are
  // calculated.
...
short surfaceIntegrals_compute(MRIS *apmris, e_surfaceIntegral aeSI_type,
                               float *apf_surfaceIntegral,
                               float *apf_meanIntegral, int *ap_vertices,
  ...
  case e_natural:
    b_ret = MRIS_surfaceIntegral_compute(
        apmris, ap_vertices, apf_area, f_allVertices, f_pass,
        apf_surfaceIntegral, apf_meanIntegral, apf_areaNormalizedIntegral);
    break;
  case e_rectified:
    b_ret = MRIS_surfaceIntegral_compute(
        apmris, ap_vertices, apf_area, f_allVertices, f_absCurv,
        apf_surfaceIntegral, apf_meanIntegral, apf_areaNormalizedIntegral);
    break;
  case e_pos:
    b_ret = MRIS_surfaceIntegral_compute(
        apmris, ap_vertices, apf_area, f_greaterEqualZero, f_absCurv,
        apf_surfaceIntegral, apf_meanIntegral, apf_areaNormalizedIntegral);
    break;
  case e_neg:
    b_ret = MRIS_surfaceIntegral_compute(
        apmris, ap_vertices, apf_area, f_lessThanZero, f_absCurv,
        apf_surfaceIntegral, apf_meanIntegral, apf_areaNormalizedIntegral);
    break;
  }
  return b_ret;
...
int MRIS_minMaxCurvature_analyze(MRIS *apmris, e_secondOrderType aesot,
                                 s_MINMAX *aps_minMax) {
  //
  // PRECONDITIONS
  ...
  MRISminMaxCurvaturesSearch(apmris, &vmin, &vmax, &f_minExplicit,
                             &f_maxExplicit);
...
short MRIS_curvatures_prepare(MRIS *apmris, e_secondOrderType aesot) {
  //
  // PRECONDITIONS
  ...
  case e_K2:
    ...
    break;
  case e_S:
    ret = MRISusePrincipalCurvatureFunction(apmris, f_sharpnessCurvature);
    break;
  case e_C:
    ret = MRISusePrincipalCurvatureFunction(apmris, f_curvednessCurvature);
    break;
  case e_BE:
    ret = MRISusePrincipalCurvatureFunction(apmris, f_bendingEnergyCurvature);
    break;
  case e_SI:
    ret = MRISusePrincipalCurvatureFunction(apmris, f_shapeIndexCurvature);
    break;
  case e_FI:
    ret = MRISusePrincipalCurvatureFunction(apmris, f_foldingIndexCurvature);
    break;
  case e_Raw:
  case e_Normal:
  case e_Scaled:
  ...
  if (Gb_vertexAreaNormalize || Gb_vertexAreaWeigh ||
      Gb_vertexAreaNormalizeFrac || Gb_vertexAreaWeighFrac) {
    MRISvertexAreaPostProcess(apmris);
  }
  return ret;
}

short MRIS_minMaxCurve_report(MRIS *apmris, e_secondOrderType aesot,
                              char *apch_report) {
  //
  // PRECONDITIONS
  ...
  strcpy(apch_report, "");
  sprintf(apch_report, "%*s%-*s", G_leftCols, Gppch[aesot], G_rightCols,
          " Min:");
  sprintf(tmp, "%12.5f at vertex %-8d\n", s_minMax.f_min, s_minMax.vertexMin);
  strcat(apch_report, tmp);
  sprintf(tmp, "%*s%-*s", G_leftCols, Gppch[aesot], G_rightCols, " Max:");
  strcat(apch_report, tmp);
...
short MRIS_surfaceIntegrals_report(MRIS *apmris, e_secondOrderType aesot,
                                   char *apch_report) {
  //
  // PRECONDITIONS
  ...
  surfaceIntegrals_compute(apmris, e_natural, &f_SInatural, &f_SInaturalMean,
                           &SInaturalVertices, &f_SInaturalAreaNorm,
                           &f_SInaturalArea);

  surfaceIntegrals_compute(apmris, e_rectified, &f_SIabs, &f_SIabsMean,
                           &SIabsVertices, &f_SIabsAreaNorm, &f_SIabsArea);

  surfaceIntegrals_compute(apmris, e_pos, &f_SIpos, &f_SIposMean,
                           &SIposVertices, &f_SIposAreaNorm, &f_SIposArea);

  surfaceIntegrals_compute(apmris, e_neg, &f_SIneg, &f_SInegMean,
                           &SInegVertices, &f_SInegAreaNorm, &f_SInegArea);
  ...
  sprintf(tmp, "%12.5f across %d (%05.2f%s) vertices\n", f_SInaturalMean,
          SInaturalVertices, 100 * (float)SInaturalVertices / totalVertices,
          "%");
  strcat(apch_report, tmp);
  ...
  sprintf(tmp, "%12.5f across %f (%05.2f%s) mm^2\n", f_SInegAreaNorm,
          f_SInegArea, 100 * f_SInegArea / f_totalSurfaceArea, "%");
  strcat(apch_report, tmp);
...
short LABEL_save(MRI_SURFACE *apmris, int a_labelSize, int *ap_labelMark,
                 char *apch_filename) {
  //
  // DESCRIPTION
...
int MRIS_surfaceRipFlags_filterVertex(MRI_SURFACE *apmris, int avno) {
  //
  // DESCRIPTION
...
short MRIS_surfaceIntegral_compute(MRI_SURFACE *pmris, int *p_verticesCounted,
  ...
  *p_verticesCounted = 0;
  ...
  for (f_n = f_total = f_totalArea = 0.0, vno = 0; vno < pmris->nvertices;
       vno++) {
...
void OFSP_create(char *apch_prefix, char *apch_suffix, e_OFSP ae_OFSP) {
  ...
  case e_None:
    ...
    break;
  case e_Partial:
    sprintf(apch_prefix, "%s.%s.%s", hemi, surf_name, apch_suffix);
    break;
  case e_Full:
    sprintf(apch_prefix, "%s.%s.%s.%s", output_fname, hemi, surf_name,
            apch_suffix);
    break;
  }
}
...
void histogram_wrapper(MRIS *amris, e_secondOrderType aesot) {
  ...
  if (f_maxCurv <= f_minCurv)
    ErrorExit(ERROR_SIZE, "%s: f_maxCurv (%f) < f_minCurv (%f)", Progname,
              f_maxCurv, f_minCurv);

  b_error = (long)(((double)f_minCurv + (double)G_bins * (double)f_binSize) *
                   1e10) > (long)(((double)f_maxCurv) * 1e10);
  ...
  histogram_create(amris, f_minCurv, f_binSize, G_bins, pf_histogram, aesot);

  printf("%*s%*s%*s\n", G_leftCols, "\tbin start", G_leftCols * 2, "\tbin end",
         G_leftCols * 2, "\tcount");
  for (i = 0; i < G_bins; i++) {
    printf("%*.4f%*.4f%*.4f ", G_leftCols * 2, (i * f_binSize) + f_minCurv,
           G_leftCols * 2, ((i + 1) * f_binSize) + f_minCurv, G_leftCols * 2,
           pf_histogram[i]);
    printf("\n");
    switch (aesot) {
    case e_Raw:
    ...
    if (b_writeToFile)
      fprintf(pFILE, "%f\t%f\t%f\n", (i * f_binSize) + f_minCurv,
              ((i + 1) * f_binSize) + f_minCurv, pf_histogram[i]);
  }
  free(pf_histogram);
}
...
void histogram_create(MRIS *amris_curvature, float af_minCurv,
  ...
  fprintf(GpSTDOUT, "\n%*s%s = %f\n", G_leftCols, pch_sot, " bin size",
          af_binSize);
  fprintf(GpSTDOUT, "%*s%s = %d\n", G_leftCols, pch_sot, " surface vertices",
          nvertices);
  ...
  fprintf(GpSTDOUT, "%*s%s = %d\n", G_leftCols, pch_sot, " sorted vertices",
          totalCount);
  fprintf(GpSTDOUT, "%*s%s = %f\n", G_leftCols, pch_sot, " ratio",
          (float)totalCount / (float)nvertices);
  free(pf_curvature);
}
...
void outputFiles_open() {
  ...
  if (Gb_output2File) {
    ...
    if (Gb_gaussianAndMean) {
      if (Gb_histogram) {
        if ((GpFILE_KHist = fopen(Gpch_KHist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_KHist);
        }
        if ((GpFILE_HHist = fopen(Gpch_HHist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_HHist);
        }
        if ((GpFILE_K1Hist = fopen(Gpch_K1Hist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_K1Hist);
        }
        if ((GpFILE_K2Hist = fopen(Gpch_K2Hist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_K2Hist);
        }
        if ((GpFILE_SHist = fopen(Gpch_SHist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_SHist);
        }
        if ((GpFILE_CHist = fopen(Gpch_CHist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_CHist);
        }
        if ((GpFILE_BEHist = fopen(Gpch_BEHist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_BEHist);
        }
        if ((GpFILE_SIHist = fopen(Gpch_SIHist, "w")) == nullptr &&
            Gb_shapeIndex) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
                    Progname, Gpch_SIHist);
        }
        if ((GpFILE_FIHist = fopen(Gpch_FIHist, "w")) == nullptr) {
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
    ...
    if (Gb_scale || (Gb_scaleMin && Gb_scaleMax)) {
      if (Gb_histogram) {
        if ((GpFILE_scaledHist = fopen(Gpch_scaledHist, "w")) == nullptr)
          ErrorExit(ERROR_NOFILE, "%s: Could not open file '%s' for writing.\n",
...
void outputFiles_close() {
  ...
  GpFILE_rawHist = nullptr;
  if (GpFILE_normHist) {
    fclose(GpFILE_normHist);
  }
  GpFILE_normHist = nullptr;
  if (GpFILE_KHist) {
    fclose(GpFILE_KHist);
  }
  GpFILE_KHist = nullptr;
  if (GpFILE_HHist) {
    fclose(GpFILE_HHist);
  }
  GpFILE_HHist = nullptr;
  if (GpFILE_K1Hist) {
    fclose(GpFILE_K1Hist);
  }
  GpFILE_K1Hist = nullptr;
  if (GpFILE_K2Hist) {
    fclose(GpFILE_K2Hist);
  }
  GpFILE_K2Hist = nullptr;
  if (GpFILE_SHist) {
    fclose(GpFILE_SHist);
  }
  GpFILE_SHist = nullptr;
  if (GpFILE_CHist) {
    fclose(GpFILE_CHist);
  }
  GpFILE_CHist = nullptr;
  if (GpFILE_BEHist) {
    fclose(GpFILE_BEHist);
  }
  GpFILE_BEHist = nullptr;
  if (GpFILE_SIHist) {
    fclose(GpFILE_SIHist);
  }
  GpFILE_SIHist = nullptr;
  if (GpFILE_FIHist) {
    fclose(GpFILE_FIHist);
  }
  GpFILE_FIHist = nullptr;
  if (GpFILE_scaledHist) {
    fclose(GpFILE_scaledHist);
  }
  GpFILE_scaledHist = nullptr;
}
...
static void print_usage() { print_help(); }
