  ...
  */
  ...
  if (mri_like != NULL) {
    ...
    if (sscanf(argv[3], "%d", &y0) != 1)
      ErrorExit(ERROR_BADPARM, "%s: could not scan y0 from '%s'", Progname,
                argv[3]);
    if (sscanf(argv[4], "%d", &z0) != 1)
      ErrorExit(ERROR_BADPARM, "%s: could not scan z0 from '%s'", Progname,
                argv[4]);
    if (sscanf(argv[5], "%d", &dx) != 1)
      ErrorExit(ERROR_BADPARM, "%s: could not scan dx from '%s'", Progname,
                argv[5]);
    if (sscanf(argv[6], "%d", &dy) != 1)
      ErrorExit(ERROR_BADPARM, "%s: could not scan dy from '%s'", Progname,
                argv[6]);
    if (sscanf(argv[7], "%d", &dz) != 1)
      ErrorExit(ERROR_BADPARM, "%s: could not scan dz from '%s'", Progname,
                argv[7]);
    out_dir = argv[8];
  }
  ...
  if (mri_like) {
    ...
    MRIvoxelToVoxel(mri_like, mri_src, mri_like->width - 1,
                    mri_like->height - 1, mri_like->depth - 1, &xv1, &yv1,
                    &zv1);
    printf("corners of template MRI point to (%2.1f, %2.1f, %2.1f) and (%2.1f, "
           "%2.1f, %2.1f)\n",
    ...
    printf("template volume size (%d, %d, %d)\n", mri_like->width,
           mri_like->height, mri_like->depth);
  }
  ...
  printf("using bounding box (%d, %d, %d) -> (%d, %d, %d)\n", x0, y0, z0,
         x0 + dx - 1, y0 + dy - 1, z0 + dz - 1);

  mri_dst = MRIextract(mri_src, nullptr, x0, y0, z0, dx, dy, dz);
