...
static MRI *mosaic_images_with_xforms(MRI **mri, int nimages, MRI *mri_weights,
                                      int pad);
static int insert_image_into_mosaic(MRI *mri_mosaic, MRI *mri, double x0,
                                    double y0);
...
static double compute_pairwise_deformation_energy(MRI *mri1, MRI *mri2,
                                                  double dx, double dy,
                                                  double *ax, double *ay,
                                                  double a, double b, double c,
                                                  double d);
static int undistorted_coords(MRI *mri, double x, double ax, double ay,
...
static MRI *build_best_mosaic(MRI **mri, MRI **mri_smooth, double *x0d,
                              double *y0d, int nimages);
static int powell_minimize(MRI *mri1, MRI *mri2, double dx, double dy, double a,
                           double b, double c, double d, double *ax, double *ay,
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: oct_register_mosaic.c,v 1.4 2016/04/18 19:19:35 fischl Exp $",
      "$Name:  $");
...
#pragma omp parallel for if_ROMP(experimental)                                 \
  ...
  if (downsample > 0)
    printf("final downsampling image size = %d x %d x %d, size %2.3f x %2.3f x "
           "%2.3f um^3\n",
           mri[0]->width, mri[0]->height, mri[0]->depth, 1000 * mri[0]->xsize,
           1000 * mri[0]->ysize, 1000 * mri[0]->zsize);
  ...
  for (i = 1; i < nimages; i++) {
    powell_minimize(mri_mosaic, mri[i], x0d[i], y0d[i], a, b, c, d, ax, ay, &a,
                    &b, &c, &d, &dxd, &dyd);

    if (NPARMS == 2)
      printf("%d: best parameters found at dr = (%2.1f, %2.1f)\n", i, dxd, dyd);
    else {
      printf("%d: best parameters found at r = %2.4f r^4 + %2.4f r^3 + %2.4f "
             "r^2 + %2.4f r, dr = (%2.1f, %2.1f)\n",
             i, a, b, c, d, dxd, dyd);
      printf("optic axes:\n");
      for (i = 0; i < nimages; i++)
        printf("(%2.1f, %2.1f)\n", ax[i], ay[i]);
      ...
      for (i = 0; i < nimages; i++) {
        ...
        for (dx = -max_trans; dx <= max_trans; dx += skip)
          for (dy = -max_trans; dy <= max_trans; dy += skip) {
            x0[frame] = xbest[frame] + dx;
            y0[frame] = ybest[frame] + dy;
            mri_mosaic = mosaic_images(mri, x0, y0, nimages, 0);
...
static int get_option(int argc, char *argv[]) {
  ...
  else if (!stricmp(option, "unwarp"))
  {
    ...
    case 'D':
      ...
      nargs = 1;
      printf("downsampling input volumes %d times (a factor of %d) before "
             "mosaicing\n",
...
static void usage_exit(int code) {
  ...
  printf("\toptions:\n");
  printf(
      "\t-D <downsample>\t-\tuse Gaussian downsampling <downsample> times\n");
  printf("\t-W <weight fname>\t-\tread tile of weights from <weight fname> and "
         "use it in tile averaging\n");
  exit(code);
}
static MRI *mosaic_images(MRI **mri, int *x0, int *y0, int nimages, int pad) {
  ...
  for (i = 0; i < nimages; i++) {
    x1 = x0[i] - pad;
    y1 = y0[i] - pad;
    x2 = 2 * pad + x1 + mri[i]->width - 1;
    y2 = 2 * pad + y1 + mri[i]->height - 1;
    if (x1 < xmin)
      xmin = x1;
    if (y1 < ymin)
      ymin = y1;
...
#define VALID(a, b, c, d)                                                      \
  (fabs(a) < .1 && fabs(b) < .1 && fabs(c) < .1 && fabs(1 - d) < .1)
static double compute_pairwise_deformation_energy(MRI *mri1, MRI *mri2,
                                                  double dx, double dy,
                                                  double *ax, double *ay,
                                                  double a, double b, double c,
  ...
  xmax = mri1->width - 1;
  ymax = mri1->height - 1;
  if (ymin < 0 || xmin < 0 || xmax >= mri1->width || ymax >= mri1->height ||
      !devFinite(a) || !devFinite(b) || !devFinite(c) || !devFinite(d))
    return (1e10);
...
#pragma omp parallel for if_ROMP(experimental) reduction(+ : energy, nvox)
    ...
    for (y1 = ymin; y1 <= ymax; y1++) {
      ...
      if ((MRIindexNotInVolume(mri1, x1u, y1u, 0) == 0) &&
          (MRIindexNotInVolume(mri1, x1u, y1u, 0) == 0)) {
        nvox++;
...
static int undistorted_coords(MRI *mri, double x, double y, double ax,
                              double ay, double a, double b, double c, double d,
                              double *px, double *py) {
...

{
  ...
  OpenPowell2(p, xi, NPARMS, intensity_tolerance, translation_tolerance, niters,
              &iter, &fret, compute_powell_sse);
...
static float compute_powell_sse(float *p) {
  ...
  energy = compute_pairwise_deformation_energy(Gmri1, Gmri2, dx, dy, ax, ay, a,
                                               b, c, d);
  return (energy);
}
#if 0
...
static MRI *undistort_and_mosaic_images(MRI **mri, double *x0, double *y0,
  ...
  mri_mosaic = MRIallocSequence(width, height, 1, MRI_FLOAT, nimages + 1);
  mri_jac =
      MRIallocSequence(mri[0]->width, mri[0]->height, 1, MRI_FLOAT, nimages);
...
static double Gx0,
    Gy0; // offset of first image that won't be modified by Powell
...
static void powell_step_func(float *p, int nparms) {
  static int step = 0;
  int i, nimages;
  char fname[STRLEN];
  MRI *mri_mosaic;
...
static MRI *build_best_mosaic(MRI **mri, MRI **mri_smooth, double *x0d,
  ...
  nparms =
      nimages * 2 - 2; // dx and dy for each image, but first image is fixed
  p = vector(1, nparms);
  xi = matrix(1, nparms, 1, nparms);
  ...
  OpenPowell2(p, xi, nparms, intensity_tolerance, translation_tolerance, niters,
              &iter, &fret, compute_powell_global_sse);
  ...
  do {
    ...
    fstart = fret;
    OpenPowell2(p, xi, nparms, intensity_tolerance, translation_tolerance,
                niters, &iter, &fret, compute_powell_global_sse);
    printf("best energy after powell: %2.3f (%d steps)\n", fret, iter);
...
static MRI *mosaic_images_with_xforms(MRI **mri, int nimages, MRI *mri_weights,
  ...
  printf("mosaic size = (%d x %d x %d) : (%2.2f-->%2.2f, %2.2f-->%2.2f, "
         "%2.2f-->%2.2f \n",
        ...
        for (z = 0; z < mri[i]->depth; z++) {
          ...
          if (MRIindexNotInVolume(mri_counts, xm, ym, zm) != 0) {
            ...
            if (first) {
              ...
              y1 = V3_Y(v_ras);
              z1 = V3_Z(v_ras);
              printf("image %d: voxel coord (%d %d %d) -> (%d %d %d), RAS "
                     "(%2.2f, %2.2f %2.2f) OOB!\n",
          ...
          if (mri_weights)
            w = MRIgetVoxVal(mri_weights, x, y, z,
                             0); /* use confocal weighting */
          else
            w = 1;
