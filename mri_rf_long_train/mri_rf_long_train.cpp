...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(tp1_name, tp2_name, mri_in, mri_tmp, input, xform_name,       \
                 transform, subjects_dir, force_inputs, conform, Progname,     \
                 mri_seg, subject_name, s1_name, s2_name, sname, t, fname)     \
        shared(mri_inputs, transforms, mri_segs, argv) schedule(static, 1)
#endif
  for (i = 0; i < max_index; i++) {
    ...
    printf("processing subject %s, %d of %d (%s and %s)...\n", subject_name,
           i + 1 - nargs, nsubjects, s1_name, s2_name);
    ...
    for (t = 0; t < 2; t++) {
      ...
      if (wmsa_fname) {
        ...
        if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
          char s[STRLEN];
          sprintf(s, "%s/%s/mri/seg_%s", subjects_dir, subject_name,
                  wmsa_fname);
          MRIwrite(mri_seg, s);
        }
      }
      ...
      for (input = 0; input < ninputs; input++) {
        ...
        // input check 2
        if (conform && (mri_tmp->xsize != 1 || mri_tmp->ysize != 1 ||
                        mri_tmp->zsize != 1)) {
          ErrorExit(ERROR_BADPARM,
      ...
      if (xform_name) {
        ...
        sprintf(fname, "%s/%s/mri/transforms/%s", subjects_dir, sname,
                xform_name);
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
...
train_rforest(MRI *mri_inputs[MAX_SUBJECTS][MAX_TIMEPOINTS],
  ...
  */
  ...
  //  if (Gx >= 0)
  {
    ...
    strcpy(rf->feature_names[i], buf);
    i++;
    sprintf(buf, "gm prior");
    rf->feature_names[i] = (char *)calloc(strlen(buf) + 1, sizeof(char));
    strcpy(rf->feature_names[i], buf);
    i++;
    sprintf(buf, "wm prior");
    rf->feature_names[i] = (char *)calloc(strlen(buf) + 1, sizeof(char));
    strcpy(rf->feature_names[i], buf);
    i++;
    sprintf(buf, "csf prior");
    rf->feature_names[i] = (char *)calloc(strlen(buf) + 1, sizeof(char));
    strcpy(rf->feature_names[i], buf);
    i++;
    sprintf(buf, "WMSA in nbhd");
    rf->feature_names[i] = (char *)calloc(strlen(buf) + 1, sizeof(char));
    strcpy(rf->feature_names[i], buf);
    ...
    for (x = 0; x < mri_in->width; x++)
      for (y = 0; y < mri_in->height; y++)
        for (z = 0; z < mri_in->depth; z++) {
          ...
          extract_long_features(mri_in, mri_seg, transform, gca, parms->wsize,
