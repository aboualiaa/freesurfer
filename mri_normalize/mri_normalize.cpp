...
static int        threshold_control_points(MRI *mri_ctrl, MRI *mri_intensity,
                                           float thresh);
static MRI *      build_outside_of_brain_mask(MRI *mri_src, GCA *gca,
                                              TRANSFORM *xform, double prior_thresh,
                                              int whalf);
static int        remove_surface_outliers(MRI *mri_ctrl_src, MRI *mri_dist,
                                          MRI *mri_src, MRI *mri_ctrl_dst,
                                          float min_dist);
static MRI *      remove_surface_outliers_T2(MRI *mri_ctrl_src, MRI *mri_dist,
                                             MRI *mri_src, MRI *mri_ctrl_dst);
static MRI *      MRIremoveWMOutliersAndRetainMedialSurface(MRI *mri_src,
                                                            MRI *mri_src_ctrl,
                                                            MRI *mri_dst_ctrl,
                                                            int  intensity_below);
static MRI *      MRIremoveWMOutliers(MRI *mri_src, MRI *mri_src_ctrl,
                                      MRI *mri_dst_ctrl, int intensity_below);
int               main(int argc, char *argv[]);
static int        remove_outliers_near_surface(MRI *mri_ctrl, MRI *mri_dist,
                                               MRI *mri_dst, MRI *mri_ctrl_out,
                                               float min_dist, float nsigma);
...
static MRI *      add_interior_points(MRI *mri_src, MRI *mri_vals,
                                      ...
                                      MRI_SURFACE *mris_interior2, MRI *mri_aseg,
                                      MRI *mri_dst);
...
static int   remove_controlpoints_outside_range(MRI *mri_ctrl, MRI *mri,
                                                float fmin, float fmax);
static char *checknorm_fname = nullptr;
static float check_min       = 90;
static float check_max       = 120;

static char *renorm_fname = nullptr;
static int   renorm_val   = DEFAULT_DESIRED_WHITE_MATTER_VALUE;

static float bias_sigma = 8.0;

static char *mask_fname;
static char *interior_fname1 = nullptr;
static char *interior_fname2 = nullptr;

static float mask_sigma       = 0.0;
static float mask_thresh      = 0.0;
static char *mask_orig_fname  = nullptr;
...
static char *long_control_volume_fname = nullptr;
static char *long_bias_volume_fname    = nullptr;
...
int main(int argc, char *argv[]) {
  ...
  if (gca) // atlas and transform specified - build mask of regions that cannot
           // have control points
  {
    mri_not_control =
        build_outside_of_brain_mask(mri_src, gca, xform, 0, brain_distance);
    GCAfree(&gca);
    TransformFree(&xform);
  }
  ...
  if (renorm_fname) {
    ...
    if (erode) {
      int i;
      for (i = 0; i < erode; i++) {
        mri_tmp = MRIerode(mri_ctrl, nullptr);
    ...
    mri_bias = MRIbuildBiasImage(mri_src, mri_ctrl, nullptr, 0.0);
    ...
    mri_dst = MRIapplyBiasCorrectionSameGeometry(
        mri_src, mri_bias, nullptr, DEFAULT_DESIRED_WHITE_MATTER_VALUE);
  ...
  if (nsurfs > 0) {
    MRI_SURFACE *mris;
    MRI *mri_dist = nullptr, *mri_dist_sup = nullptr, *mri_ctrl, *mri_dist_one;
    // LTA          *lta ;
    int        i;
    TRANSFORM *surface_xform;

    if (control_point_fname ||
        control_point_label) // do one pass with only file control points first
    {
      if (control_point_fname)
      ...
      mri_dst = MRI3dGentleNormalize(
          mri_src, nullptr, DEFAULT_DESIRED_WHITE_MATTER_VALUE, nullptr,
    ...
    for (i = 0; i < nsurfs; i++) {
      ...
      if (mris == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not surface %s", Progname,
                  surface_fnames[i]);
      surface_xform = surface_xforms[i];
      TransformInvert(surface_xform, nullptr);
      ...
      if (stricmp(surface_xform_fnames[i], "identity.nofile") != 0)
        MRIStransform(mris, nullptr, surface_xform, nullptr);
      ...
      if (i == 0)
        mri_dist = MRIcopy(mri_dist_one, nullptr);
      else
    ...
    if (nonmax_suppress) {
      ...
      mri_dist_sup = MRInonMaxSuppress(mri_dist, nullptr, 0, 1);
      mri_ctrl     = MRIcloneDifferentType(mri_dist_sup, MRI_UCHAR);
      MRIbinarize(mri_dist_sup, mri_ctrl, min_dist, CONTROL_NONE,
                  CONTROL_MARKED);
    } else if (erode) {
      int i;
    ...
    if (mask_sigma > 0) {
      ...
      mri_smooth = MRIgaussianSmooth(mri_dst, mask_sigma, 1, nullptr);
      mri_mag    = MRIcloneDifferentType(mri_dst, MRI_FLOAT);
      mri_grad   = MRIsobel(mri_smooth, nullptr, mri_mag);
    ...
    if (checknorm_fname) {
      ...
      if (mri == nullptr)
        ErrorExit(ERROR_NOFILE, "%s: could not load %s", Progname,
                  checknorm_fname);
    ...
    remove_outliers_near_surface(mri_ctrl, mri_dist, mri_dst, mri_ctrl,
                                 min_dist + 1, 2.5);
    ...
    mri_bias = MRIbuildBiasImage(mri_dst, mri_ctrl, nullptr, 0.0);
    if (mri_dist)
      MRIfree(&mri_dist);
    if (mri_dist_sup)
  ...
  if (read_flag) {
    ...
    if (!mri_bias)
      ErrorExit(ERROR_BADPARM, "%s: could not read bias volume %s", Progname,
                bias_volume_fname);
    mri_ctrl = MRIread(control_volume_fname);
    if (!mri_ctrl)
      ErrorExit(ERROR_BADPARM, "%s: could not read control volume %s", Progname,
                control_volume_fname);
    MRIbinarize(mri_ctrl, mri_ctrl, 1, 0, 128);
    mri_dst = MRImultiply(mri_bias, mri_src, nullptr);
  ...
  if (long_flag) {
    ...
    if (!mri_bias)
      ErrorExit(ERROR_BADPARM, "%s: could not read bias volume %s", Progname,
                long_bias_volume_fname);
    mri_ctrl = MRIread(long_control_volume_fname);
    if (!mri_ctrl)
      ErrorExit(ERROR_BADPARM, "%s: could not read control volume %s", Progname,
                long_control_volume_fname);
    ...
    mri_dst = MRIscalarMul(mri_src, nullptr, scale);
  ...
  if (grad_thresh > 0) {
    ...
    mri_smooth = MRIconvolveGaussian(mri_src, nullptr, mri_kernel);
    thresh     = grad_thresh;
  ...
  if (aseg_fname) {
    ...
    mri_aseg = MRIread(aseg_fname);
    if (mri_aseg == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read aseg from file %s", Progname,
                aseg_fname);
  ...
  if (long_flag == 0) // if long, then this will already have been done with
                      // base control points
  {
    if (control_point_fname != nullptr ||
        control_point_label != nullptr) /* do one pass with only
  ...
  if (mri_aseg) {
    ...
    if (interior_fname1) {
      ...
      if (mris_interior1 == nullptr)
        ErrorExit(ERROR_NOFILE,
                  "%s: could not read white matter surface from %s\n", Progname,
                  interior_fname1);
      mris_interior2 = MRISread(interior_fname2);
      if (mris_interior2 == nullptr)
        ErrorExit(ERROR_NOFILE,
                  "%s: could not read white matter surface from %s\n", Progname,
                  interior_fname2);
      add_interior_points(mri_ctrl, mri_dst, intensity_above,
                          1.25 * intensity_below, mris_interior1,
                          mris_interior2, mri_aseg, mri_ctrl);
    ...
    if (!no1d) {
      ...
      printf("MRInormalize(): \n");
      mri_dst = MRInormalize(mri_src, nullptr, &mni);
      if (!mri_dst) {
        no1d = 1;
    ...
    if (no1d) {
      if ((file_only && nosnr) ||
          ((gentle_flag != 0) && (control_point_fname != nullptr))) {
        if (mri_dst == nullptr) {
          mri_dst = MRIcopy(mri_src, nullptr);
        }
      } else {
        if (nosnr) {
          if (interior_fname1) {
            ...
            mri_dst = MRIapplyBiasCorrectionSameGeometry(
                mri_src, mri_bias, mri_dst, DEFAULT_DESIRED_WHITE_MATTER_VALUE);
            MRIfree(&mri_ctrl);
          ...
          mri_dst = MRInormalizeHighSignalLowStd(
              mri_src, mri_dst, bias_sigma, DEFAULT_DESIRED_WHITE_MATTER_VALUE);
        }
      }
      if (!mri_dst)
  ...
  if (file_only == 0)
    MRI3dGentleNormalize(mri_dst, nullptr, DEFAULT_DESIRED_WHITE_MATTER_VALUE,
                         mri_dst, intensity_above, intensity_below / 2,
                         file_only, bias_sigma, mri_not_control);

  mri_orig = MRIcopy(mri_dst, nullptr);
  printf("\n");
  ...
  for (n = 0; n < num_3d_iter; n++) {
    ...
    if (gentle_flag)
      MRI3dGentleNormalize(mri_dst, nullptr, DEFAULT_DESIRED_WHITE_MATTER_VALUE,
                           mri_dst, intensity_above / 2, intensity_below / 2,
                           file_only, bias_sigma, mri_not_control);
    else
      MRI3dNormalize(mri_orig, mri_dst, DEFAULT_DESIRED_WHITE_MATTER_VALUE,
                     mri_dst, intensity_above, intensity_below, file_only,
                     prune, bias_sigma, scan_type, mri_not_control);
  }
  ...
  if (bias_volume_fname) {
    mri_bias = compute_bias(mri_src, mri_dst, nullptr);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    printf("reading gca from %s, xform from %s and setting brain distance to "
           "%d voxels\n",
    ...
    if (surface_xforms[nsurfs] == nullptr) {
    ...
    control_point_label = LabelRead(nullptr, argv[2]);
    if (control_point_label == nullptr)
...
static MRI *MRIremoveWMOutliersAndRetainMedialSurface(MRI *mri_src,
                                                      ...
                                                      MRI *mri_dst_ctrl,
                                                      int  intensity_below) {
  MRI *      mri_bin, *mri_dist, *mri_dist_sup, *mri_outliers = nullptr;
  ...
  mri_bin  = MRIbinarize(mri_dst_ctrl, nullptr, 1, 0, 1);
  mri_dist = MRIdistanceTransform(mri_bin, nullptr, 1, -1, DTRANS_MODE_SIGNED,
                                  nullptr);
  MRIscalarMul(mri_dist, mri_dist, -1);
  mri_dist_sup = MRInonMaxSuppress(mri_dist, nullptr, 0, 1);
  mri_dst_ctrl = MRIbinarize(mri_dist_sup, mri_dst_ctrl, 1, 0, 1);
  histo        = MRIhistogramLabel(mri_src, mri_src_ctrl, 1, 256);
  hsmooth      = HISTOcopy(histo, nullptr);
  ...
  HISTOfree(&hsmooth);
  if (Gdiag & DIAG_WRITE) {
    mri_outliers = MRIclone(mri_dst_ctrl, nullptr);
  }
  for (x = 0; x < mri_src->width; x++) {
    for (y = 0; y < mri_src->height; y++) {
      for (z = 0; z < mri_src->depth; z++) {
...
static MRI *MRIremoveWMOutliers(MRI *mri_src, MRI *mri_src_ctrl,
                                MRI *mri_dst_ctrl, int intensity_below) {
  MRI *      mri_bin, *mri_outliers = nullptr;
  float      max, thresh, val;
  HISTOGRAM *histo, *hsmooth;
  ...
  if (mri_dst_ctrl == nullptr) {
    mri_dst_ctrl = MRIcopy(mri_src_ctrl, nullptr);
  }
  mri_bin = MRIbinarize(mri_src_ctrl, nullptr, 1, 0, CONTROL_MARKED);
  ...
  printf("%d control points removed (%2.1f%%)\n", nremoved,
         100.0 * (double)nremoved / (double)total);
  if (mri_outliers) {
    printf("writing out.mgz outlier volume\n");
    MRIwrite(mri_outliers, "out.mgz");
    MRIfree(&mri_outliers);
...
static MRI *add_interior_points(MRI *mri_src, MRI *mri_vals,
                                ...
                                MRI_SURFACE *mris_white2, MRI *mri_aseg,
                                MRI *mri_dst) {
  ...
  mri_interior = MRIclone(mri_src, nullptr);
  ...
  mri_core = MRIerode(mri_interior, nullptr);
        ...
        for (z = 0; z < mri_src->depth; z++) {
          ...
          if (ctrl == 0) // add in some missed ones that are inside the surface
          {
            ...
            if (IS_GM(label) || IS_WM(label)) {
              ...
              if ((val >= 110 - intensity_below &&
                   val <= 110 + intensity_above) &&
...
static int remove_surface_outliers(MRI *mri_ctrl_src, MRI *mri_dist,
  ...
  MRI *      mri_outlier = MRIclone(mri_ctrl_src, nullptr);
    ...
    for (y = 0; y < mri_src->height; y++)
      for (z = 0; z < mri_src->depth; z++) {
        ...
        if ((int)MRIgetVoxVal(mri_ctrl_src, x, y, z, 0) == 0) {
          if (x == Gx && y == Gy && z == Gz)
            printf("(%d, %d, %d) - not a control point, skipping in outlier "
                   "detection\n",
                   Gx, Gy, Gz);
          continue; // not a control point
        }
        ...
        if (val > 100 && val < 120) {
          if (x == Gx && y == Gy && z == Gz)
            printf("(%d, %d, %d) -  a control point with reasonable value "
                   "%2.0f, keeping\n",
                   Gx, Gy, Gz, val);
          continue; // not an outlier
        }
#endif
        h = MRIhistogramVoxel(mri_src, 0, nullptr, x, y, z, wsize, mri_dist,
                              min_dist);
        HISTOsoapBubbleZeros(h, h, 100);
        hs = HISTOsmooth(h, nullptr, .5);
...
#define MAX_SIGMA 10 // for intensity normalized images
        ...
        if (fabs((mean - val) / sigma) > 1) {
          MRIsetVoxVal(mri_ctrl_dst, x, y, z, 0,
                       0); // remove it as a control point
          MRIsetVoxVal(mri_outlier, x, y, z, 0, 1); // diagnostics
          if (x == Gx && y == Gy && z == Gz)
            printf("(%d, %d, %d) -  removing control point with val %2.0f - to "
                   "far from mean %2.0f +- %2.0f\n",
    ...
    for (y = 0; y < mri_ctrl_src->height; y++)
      for (z = 0; z < mri_ctrl_src->depth; z++) {
        ...
        case Unknown:
          ...
          break;
        default:
          if ((MRIlabelsInNbhd(mri_aseg, xa, ya, za, 1,
                               Left_Lateral_Ventricle) > 0) ||
              (MRIlabelsInNbhd(mri_aseg, xa, ya, za, 1,
...
static int remove_outliers_near_surface(MRI *mri_ctrl, MRI *mri_dist,
    ...
    for (y = 0; y < mri_ctrl->height; y++)
      for (z = 0; z < mri_ctrl->depth; z++) {
            ...
            (c > 0)) // it is a control point and it's close to the edge
        {
          MRIsetVoxVal(
              mri_ctrl, x, y, z, 0,
              0); // so it won't be included in mean/variance calculation
          mean =
              MRImeanInLabelInRegion(mri_dst, mri_ctrl, c, x, y, z, 9, &sigma);
          val = MRIgetVoxVal(mri_dst, x, y, z, 0);
  ...
  printf("%d control points within %2.1fmm of the surface removed as intensity "
         "outliers\n",
         nremoved, min_dist);
  return (nremoved);
}
...
static MRI *build_outside_of_brain_mask(MRI *mri_src, GCA *gca,
  ...
  for (x = 0; x < mri_src->width; x++)
    for (y = 0; y < mri_src->height; y++)
      for (z = 0; z < mri_src->depth; z++) {
        if (GCAsourceVoxelToPrior(gca, mri_src, xform, x, y, z, &xp, &yp,
                                  &zp) != NO_ERROR) {
          MRIsetVoxVal(mri_outside_of_brain, x, y, z, 0,
                       1); // outside of FOV of atlas - definitely out of brain
          continue;
        }
...
static MRI *remove_surface_outliers_T2(MRI *mri_ctrl_src, MRI *mri_dist,
  ...
  float ddx, ddy, ddz, sdx, sdy, sdz, dot, xf, yf, zf, dist,
      step_size = 0.5, odx, ody, odz, norm;

  if (mri_ctrl_dst != mri_ctrl_src)
    mri_ctrl_dst = MRIcopy(mri_ctrl_src, mri_ctrl_dst);

  mri_mag       = MRIcloneDifferentType(mri_src, MRI_FLOAT);
  mri_grad      = MRIsobel(mri_src, nullptr, mri_mag);
  mri_dist_grad = MRIsobel(mri_dist, nullptr, nullptr);
  mri_dist_sup  = MRInonMaxSuppress(mri_dist, nullptr, 0, 1);
  MRIwrite(mri_dist_grad, "dg.mgz");
  MRIwrite(mri_grad, "g.mgz");
  MRIwrite(mri_mag, "m.mgz");
  mri_nonmax = MRInonMaxSuppress(mri_mag, nullptr, nonmax_thresh, 1);
  ...
  for (x = 0; x < mri_dist->width; x++)
    for (y = 0; y < mri_dist->height; y++)
      for (z = 0; z < mri_dist->depth; z++) {
        ...
        if (dot > 0) // for T2-weighted direction of distance transform grad
                     // should be opposite image grad
        ...
        do {
          ...
          if (MRIgetVoxVal(mri_nonmax, xv, yv, zv, 0) > 0)
            ...
            if (MRIgetVoxVal(mri_dist_sup, xv, yv, zv, 0) > 0)
              done = 1; // got to skeleton without intersecting and egdge -
                        // assume it's ok
          }
          if (dist > 10)
            break;
  ...
  for (x = 0; x < mri_ctrl->width; x++)
    for (y = 0; y < mri_ctrl->height; y++)
      for (z = 0; z < mri_ctrl->depth; z++) {
        ...
        if (val < thresh) {
          if (x == Gx && y == Gy && z == Gz)
            printf("(%d, %d, %d) - val %2.0f lower than threshold %2.0f - "
                   "removing\n",
                   x, y, z, val, thresh);
    ...
    for (y = 0; y < mri->height; y++)
      for (z = 0; z < mri->depth; z++) {
        ...
        if (val < fmin || val > fmax) {
          if (x == Gx && y == Gy && z == Gz)
            printf("voxel(%d, %d, %d) = %2.1f, removing due to intensity "
                   "outlier\n",
                   x, y, z, val);
          nremoved++;
