...
int main(int argc, char **argv) {
  ...
  if (DoETIV || DoETIVonly || DoOldETIVonly) {
    ...
    if (talxfmfile) {
      ...
      sprintf(tmpstr, "%s/%s/mri/transforms/talairach.xfm", SUBJECTS_DIR,
              subject);
    }
    if (DoOldETIVonly) {
  ...
  if (SegVolFile) {
    ...
    if (nReplace > 0) {
      printf("Replacing %d\n", nReplace);
      mritmp = MRIreplaceList(seg, SrcReplace, TrgReplace, nReplace, nullptr,
                              nullptr);
      MRIfree(&seg);
      seg = mritmp;
    }
    ...
    if (nErodeSeg) {
      ...
      tmp = MRIerodeSegmentation(seg, nullptr, nErodeSeg, 0);
      MRIfree(&seg);
      seg = tmp;
    }
    if (DoVox) {
    ...
    if (mris == nullptr)
      exit(1);
    if (fio_FileExistsReadable(annot))
      sprintf(tmpstr, "%s", annot);
    else
    ...
    if (err) {
      ...
      if (!err)
        printf(" Successfully read local annot\n\n");
    }
    if (err)
      exit(1);
    ...
    if (UseLabelThresh)
      printf(" Label Threshold = %g\n", LabelThresh);
    label = LabelRead(nullptr, LabelFile);
    if (label == nullptr)
      exit(1);
    sprintf(tmpstr, "%s/%s/surf/%s.%s", SUBJECTS_DIR, subject, hemi,
            whitesurfname);
    ...
    for (n = 0; n < label->n_points; n++) {
      if (UseLabelThresh && label->lv[n].stat < LabelThresh)
        continue;
  ...
  if (DoSurfWMVol || DoSurfCtxVol || DoSupraTent || BrainVolFromSeg ||
      DoSubCortGrayVol) {
  ...
  if (InVolFile != nullptr) {
    ...
    if (InVolReg || InVolRegHeader) {
      ...
      printf("Vol2Vol\n");
      err = MRIvol2VolVSM(invol, tmp, vox2vox, SAMPLE_NEAREST, -1, nullptr);
      if (err) {
        exit(1);
      }
      MRIfree(&invol);
    ...
    if (DoSquare) {
      ...
      MRIsquare(invol, nullptr, invol);
    }
    if (DoSquareRoot) {
      printf("Computing square root of input\n");
      MRIsquareRoot(invol, nullptr, invol);
    }
  }

  /* Load the partial volume mri */
  if (PVVolFile != nullptr) {
    printf("Loading %s\n", PVVolFile);
    fflush(stdout);
  ...
  if (MaskVolFile != nullptr) {
    ...
    mri_binarize(maskvol, maskthresh, masksign, maskinvert, maskvol,
                 &nmaskhits);
    if (nmaskhits == 0) {
...
#pragma omp parallel for if_ROMP(assume_reproducible)                          \
    firstprivate(DoContinue, nx, skip, n0, vol, nhits, c, min, max, range,     \
                 mean, std, snr) schedule(guided)
#endif
      for (n = 0; n < nsegid; n++) {
    ROMP_PFLB_begin if (DoExclSegId) {
      DoContinue = 0;
    ...
    if (InVolFile != nullptr && !dontrun) {
      if (nhits > 0) {
        if (UseRobust == 0)
          MRIsegStats(seg, StatSumTable[n].id, invol, frame, &min, &max, &range,
                      &mean, &std);
        else
          MRIsegStatsRobust(seg, StatSumTable[n].id, invol, frame, &min, &max,
                            &range, &mean, &std, RobustPct);
      ...
      if (DoAccumulate == 1)
        StatSumTable[n].mean = mean * nhits;
  ...
  ROMP_PF_end
    ...
    if (Gdiag_no > 1) {
      printf("%3d   %3d  %33s  %6d  %10.3f\n", n, StatSumTable[n].id,
             StatSumTable[n].name, StatSumTable[n].nhits, StatSumTable[n].vol);
      fflush(stdout);
    }
  }
  printf("\n");
  ...
  if (debug) {
    for (n = 0; n < nsegid; n++) {
      ...
      if (InVolFile != nullptr) {
        printf("%10.4f %10.4f %10.4f %10.4f %10.4f ", StatSumTable[n].min,
               StatSumTable[n].max, StatSumTable[n].range, StatSumTable[n].mean,
               StatSumTable[n].std);
        if (DoSNR) {
  ...
  if (StatTableFile != nullptr) {
    ...
    if (UseRobust)
      fprintf(fp, "# RobustPct %g\n", RobustPct);
    if (!BrainVolStats.empty()) {
      if (fabs(voxelvolume - 1) > .01) {
        // This indicates that the global stats has been fixed
        fprintf(fp, "# BrainVolStatsFixed see "
                    "surfer.nmr.mgh.harvard.edu/fswiki/BrainVolStatsFixed\n");
      } else {
        fprintf(fp,
    ...
    if (!BrainVolStats.empty()) {
      fprintf(fp,
              "# Measure VentricleChoroidVol, VentricleChoroidVol, "
    ...
    if (DoETIV && BrainVolFromSeg) {
      fprintf(fp,
              "# Measure BrainSegVol-to-eTIV, BrainSegVol-to-eTIV, "
              "Ratio of BrainSegVol to eTIV, %f, unitless\n",
              BrainVolStats[0] / atlas_icv);
      fprintf(fp,
              "# Measure MaskVol-to-eTIV, MaskVol-to-eTIV, "
              "Ratio of MaskVol to eTIV, %f, unitless\n",
              BrainVolStats[12] / atlas_icv);
    }
    if (DoEuler) {
      fprintf(fp,
              "# Measure lhSurfaceHoles, lhSurfaceHoles, "
              "Number of defect holes in lh surfaces prior to fixing, %d, "
              "unitless\n",
              (1 - lheno / 2));
      fprintf(fp,
              "# Measure rhSurfaceHoles, rhSurfaceHoles, "
              "Number of defect holes in rh surfaces prior to fixing, %d, "
              "unitless\n",
              (1 - rheno / 2));
      fprintf(fp,
              "# Measure SurfaceHoles, SurfaceHoles, "
              "Total number of defect holes in surfaces prior to fixing, %d, "
              "unitless\n",
              (1 - lheno / 2) + (1 - rheno / 2));
    }
    if (DoETIV) {
    ...
    if (UsePrintSegStat) {
      ...
      if (InVolFile) {
        fprintf(fp, "%sMean %sStdDev %sMin %sMax %sRange  ", InIntensityName,
                InIntensityName, InIntensityName, InIntensityName,
                InIntensityName);
        if (DoSNR) {
  ...
  if (ctabfileOut != nullptr) {
    fp = fopen(ctabfileOut, "w");
    for (n = 0; n < nsegid; n++)
      fprintf(fp, "%d %-30s %3d %3d %3d 0\n", StatSumTable[n].id,
              StatSumTable[n].name, StatSumTable[n].red, StatSumTable[n].green,
              StatSumTable[n].blue);
    fclose(fp);
  }
  ...
  if (DoFrameAvg) {
    ...
    for (n = 0; n < nsegid; n++) {
      ...
      for (f = 0; f < invol->nframes; f++) {
        if (DoFrameSum)
          favg[n][f] *= nvox; // Undo spatial average
        favgmn[n] += favg[n][f];
      }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help") || !strcasecmp(option, "--usage") ||
      ...
      debug = 1;
    } else if (!strcasecmp(option, "--newprint"))
      UsePrintSegStat = 1;
    else if (!strcasecmp(option, "--no-newprint"))
      UsePrintSegStat = 0;
    else if (!strcasecmp(option, "--dontrun")) {
      dontrun = 1;
    } else if (!strcasecmp(option, "--nonempty") ||
               !strcasecmp(option, "--non-empty"))
      NonEmptyOnly = 1;
      ...
      DoSurfWMVol = DoSurfCtxVol = DoSupraTent = BrainVolFromSeg =
          DoSubCortGrayVol = DoTotalGrayVol = DoEuler = 0;
      ...
      nExcl++;
    } else if (!strcmp(option, "--excludeid") || !strcmp(option, "--exclude")) {
      if (nargc < 1) {
        argnerr(option, 1);
      }
      nth = 0;
      ...
      GTMdefaultSegReplacmentList(&nReplace, &(SrcReplace[0]),
                                  &(TrgReplace[0]));
      ...
      if (nargc < 1)
        CMDargNErr(option, 1);
      int err = GTMloadReplacmentList(pargv[0], &nReplace, &(SrcReplace[0]),
                                      &(TrgReplace[0]));
      if (err)
        exit(1);
      nargsused = 1;
...
static void print_usage() {
  outputHelpXml(mri_segstats_help_xml, mri_segstats_help_xml_len);
}
...
static void check_options() {
  ...
  if (ctabfile != nullptr && gcafile != nullptr) {
    printf("ERROR: cannot specify ctab and gca\n");
    exit(1);
  }
  if (DoSurfCtxVol && subject == nullptr) {
...
STATSUMENTRY *LoadStatSumFile(char *fname, int *nsegid) {
  ...
  *nsegid = 0;
  while (fgets(tmpstr, 1000, fp) != nullptr) {
    ...
    sscanf(tmpstr, "%*d %d %d %f %s %f %f %f %f %f", &e->id, &e->nhits, &e->vol,
           &e->name[0], &e->mean, &e->std, &e->min, &e->max, &e->range);
    (*nsegid)++;
  }
  fclose(fp);
...
int CountEdits(char *subject, char *outfile) {
  ...
  for (c = 0; c < mri->width; c++) {
    for (r = 0; r < mri->height; r++) {
      for (s = 0; s < mri->depth; s++) {
        ...
        if (v1 == WM_EDITED_OFF_VAL)
          nWMErase++;
        if (v1 == WM_EDITED_ON_VAL)
          nWMFill++;
      }
    }
  }
  ...
  std::vector<double> BrainVolStats =
      ReadCachedBrainVolumeStats(subject, SUBJECTS_DIR);
  ...
  printf("%s nc %3d, nWMErase %3d, nWMFill %3d, nBMErase %3d, nBMClone %3d, "
         "nASegChanges %3d, "
  ...
  if (outfile) {
    fp = fopen(outfile, "w");
    fprintf(fp,
            "%s %3d    %4d %4d    %4d %4d   %4d  %4d %4d %4d   %7.5f %6.2f "
