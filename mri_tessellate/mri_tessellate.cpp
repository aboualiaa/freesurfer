...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3)
    usage_exit(1);
  if (argc == 3) // value not specified on cmdline - figure it out from hemi in
                 // output surf
  {
...
static void check_face(MRI *mri, int im0, int i0, int j0, int im1, int i1,
  ...
  if ((im0 >= 0 && im0 < numimg && i0 >= imin && i0 < imax && j0 >= jmin &&
       j0 < jmax && im1 >= 0 && im1 < numimg && i1 >= imin && i1 < imax &&
       j1 >= jmin && j1 < jmax)) {
    if ((all_flag && ((MRIvox(mri, j0, i0, im0) !=
                       MRIvox(mri, j1, i1,
                              im1)) /* && (MRIvox(mri, j1, i1, im1) == 0)*/)) ||
        (((MRIvox(mri, j0, i0, im0) == value) &&
          (MRIvox(mri, j1, i1, im1) != value)))) {
      if (n == 0) {
    ...
    for (i = 0; i <= ynum; i++)
      for (j = 0; j <= xnum; j++) {
        ...
        if (facep(mri, imnr, i - 1, j - 1, imnr - 1, i - 1, j - 1) ||
          ...
          check_face(mri, imnr, i - 1, j - 1, imnr - 1, i - 1, j - 1, 0, 2,
                     v_ind, 0);
          ...
          check_face(mri, imnr - 1, i, j - 1, imnr - 1, i - 1, j - 1, 2, 2,
                     v_ind, 1);
          ...
          check_face(mri, imnr - 1, i - 1, j, imnr - 1, i - 1, j - 1, 4, 2,
                     v_ind, 1);
          ...
          check_face(mri, imnr - 1, i - 1, j - 1, imnr, i - 1, j - 1, 1, 2,
                     v_ind, 1);
          ...
          check_face(mri, imnr - 1, i - 1, j - 1, imnr - 1, i, j - 1, 3, 2,
                     v_ind, 1);
          ...
          check_face(mri, imnr - 1, i - 1, j - 1, imnr - 1, i - 1, j, 5, 2,
                     v_ind, 1);
...
static void write_binary_surface(char *fname, MRI *mri, std::string &cmdline) {
  ...
  fp = fopen(fname, "w");
  if (fp == nullptr)
    ErrorExit(ERROR_BADPARM, "%s: could not write a file %s", Progname, fname);
  else {
  ...
  if (useRealRAS ==
      0) // messes up scuba if realras is true - don't know why (BRF)
  {
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "an option")) {
    ...
    case 'N':
      compatibility = 0;
      printf("surface saved uses coordinates "
...
static void usage_exit(int code) {
  outputHelpXml(mri_tessellate_help_xml, mri_tessellate_help_xml_len);
  exit(code);
}
