...
int crossline(float *plane, int *itr, int *nlm, float (*rlm)[2], int npnt,
              float (*pnt)[3], int ndhk, int (*dhk)[3]);
...
int lmoutr(float *p1, float *p2, float *p3, float *r, float *lamba, float *mu,
           float *dif);
...
void trinorm(float (*n)[3], int npnt, float (*pnt)[3], int ndhk, int (*dhk)[3],
             int do_normate);
...
int dhkinp(char *filename, int *npnt, int *ndhk, float (**pnt1)[3],
           int (**dhk1)[3]);
...
int solangle(double r1[3], double r2[3], double r3[3], double *res1,
             double *res2, double *res3, double small);

/* rdds.c   */

void rdds(double r1[3], double r2[3], double r3[3], double *res1, double *res2,
          double *res3);

/* rxds.c   */

int rxds(double r1[3], double r2[3], double r3[3], double res1[3],
         double res2[3], double res3[3]);
...
void svdsolve(float **u, int m, int n, float *sigma, float **v, float *b,
              float *x, float crit);
void svdnsolve(float **u, int m, int n, float *sigma, float **v, float *b,
               float *x, int rang);
...
int decouple(int nopt, int nlpar, int nnpar, int ntim, float **v, float **phi,
             float *p, float **s, int (*f_bound)(int nnpar, float *p),
             float *res,
             void (*Aeval)(int nopt, int nlpar, int nnpar, float *p, float **A),
             void (*dAeval)(int nopt, int nlpar, int nnpar, float *p,
                            float ***dA),
             int maxstp, float fstop, float *cond, int display, float *sdphi,
...
void pnt_cont2tri(float c[3], int ntrack, int nouter, int manual, int log,
                  int diag, int unreg, float t2, int ncpnt, float (*cpnt)[3],
                  int fpnt, float (*pnt)[3], int fdhk, int (*dhk)[3],
                  int *nnpnt, int *nndhk);

/* voisine.c    */

int voisine(int (*dhk)[3], int ndhk, int npnt, int ***nb1, int **nnb1,
            int **opn1, int ***nbtr1, int **nnbtr1, int nnbmax);
