...
static int get_crs(MATRIX *Tras2vox, float x, float y, float z, int *c, int *r,
                   int *s, MRI *vol);
...
char *TempVolId = nullptr;
char *RegMatFile = nullptr;
int RegHeader = 0;
int RegIdentity = 0;
int InvertMtx = 0;
...
double ProjStart = 0;
double ProjStop = 0;
char *ProjType = nullptr;
char *OutVolId = nullptr;
char *HitVolId = nullptr;
char *PVFVolId = nullptr;
char *subject = nullptr;
char *hemi = nullptr;
char *AnnotFile = nullptr;
char *ASegFSpec = nullptr;
char *SurfId = "white";
char *LabelVolFile = nullptr;

MRI_SURFACE *Surf = nullptr;
MRI *OutVol = nullptr, *TempVol = nullptr, *HitVol = nullptr, *PVFVol = nullptr;
MRI *ASeg = nullptr;
MRI *LabelStatVol = nullptr;
char *LabelStatVolFSpec = nullptr;
...
COLOR_TABLE *ctTissueType = nullptr;
...
int main(int argc, char **argv) {
  ...
  make_cmd_version_string(
      argc, argv, "$Id: mri_label2vol.c,v 1.46 2014/12/08 21:11:54 greve Exp $",
      "$Name:  $", cmdline);
  ...
  if (TempVolId) {
    ...
    Tras2vox = MatrixInverse(Tvox2ras, nullptr);
    ...
    if (RegMatFile != nullptr) {
      // Load registration matrix
      printf("Loading registration from %s\n", RegMatFile);
      err = regio_read_register(RegMatFile, &regsubject, &ipr, &bpr, &intensity,
                                &R, &float2int);
      if (err)
        exit(1);
    } else {
      if (RegHeader) {
        ...
        R = MRItkRegMtx(LabelVol, TempVol, nullptr);
      }
      if (RegIdentity) {
  ...
  if (SurfNeeded) {
    ...
    sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, subject, hemi,
            thicknessname);
  ...
  if (ASegFSpec != nullptr) {
    ...
    if (!UseNewASeg2Vol) {
      ...
      OutVol = MRIaseg2volMU(ASeg, aseg2vol, FillThresh, &HitVol,
                             UpsampleFactor, ctTissueType);
      ...
      if (PVFVolId != nullptr) {
        ...
        PVFVol = MRImultiplyConst(HitVol, 1.0 / TempVoxVol, nullptr);
      ...
      if (resmm == 0)
        resmm = ASeg->xsize / UpsampleFactor;
      OutVol = MRIseg2SegPVF(ASeg, aseg2vol, resmm, segidlist, nsegs, nullptr,
                             0, ctTissueType, nullptr);
      MRIaddCommandLine(OutVol, cmdline);
      err = MRIwrite(OutVol, OutVolId);
      if (err)
        exit(1);
  ...
  if (FillRibbon) {
    ...
    for (nthlabel = 0; nthlabel < nlabels; nthlabel++) {
      if (AnnotFile == nullptr && ASegFSpec == nullptr) {
        printf("Loading %s\n", LabelList[nthlabel]);
        labels[nthlabel] = LabelRead(nullptr, LabelList[nthlabel]);
        if (labels[nthlabel] == nullptr) {
          printf("ERROR reading %s\n", LabelList[nthlabel]);
          exit(1);
        }
      }
    ...
    OutVol = MRIsurfaceLabel2VolOpt(ribbon, Surf, labels, nlabels, lta,
                                    DoStatThresh, StatThresh, 0, nullptr);
    MRIaddCommandLine(OutVol, cmdline);
    err = MRIwrite(OutVol, OutVolId);
    if (err)
      exit(err);
    ...
    if (DoLabelStatVol) {
      ...
      LabelStatVol = MRIsurfaceLabel2VolOpt(ribbon, Surf, labels, nlabels, lta,
                                            DoStatThresh, StatThresh,
                                            DoLabelStatVol, nullptr);
  ...
  HitVol = MRIallocSequence(TempVol->width, TempVol->height, TempVol->depth,
                            MRI_SHORT, nlabels);
  if (HitVol == nullptr) {
  ...
  for (nthlabel = 0; nthlabel < nlabels; nthlabel++) {
    ...
    if (AnnotFile == nullptr && ASegFSpec == nullptr) {
      printf("Loading %s\n", LabelList[nthlabel]);
      srclabel = LabelRead(nullptr, LabelList[nthlabel]);
      if (srclabel == nullptr) {
        printf("ERROR reading %s\n", LabelList[nthlabel]);
        exit(1);
      }
    ...
    for (nthpoint = 0; nthpoint < srclabel->n_points; nthpoint++) {
      if (DoStatThresh)
        if (srclabel->lv[nthpoint].stat < StatThresh)
          continue;
      ...
      if (DoProj) { // Project along the surface normal
        ...
        while (ProjDepth <= ProjStop) {
          ...
          if (debug)
            printf("   ProjDepth %g   %g %g %g (%g)  %d %d %d   %d\n",
                   ProjDepth, x, y, z, Surf->vertices[vtxno].curv, c, r, s,
                   oob);
          ...
          if (!oob) {
            ...
            if (DoLabelStatVol)
              MRIFseq_vox(LabelStatVol, c, r, s, 0) =
                  srclabel->lv[nthpoint].stat;
          }
  ...
  if (PVFVolId != nullptr) {
    ...
    printf("Computing PVF %g\n", voxvol);
    PVFVol = MRImultiplyConst(HitVol, 1 / voxvol, nullptr);
    MRIwrite(PVFVol, PVFVolId);
    exit(1);
  }

  // Create output volume based on template, but use 1 frame int
  OutVol = MRIallocSequence(TempVol->width, TempVol->height, TempVol->depth,
                            MRI_INT, 1);
  if (OutVol == nullptr) {
  ...
  for (c = 0; c < OutVol->width; c++) {
    for (r = 0; r < OutVol->height; r++) {
      for (s = 0; s < OutVol->depth; s++) {
        ...
        else if (ASegFSpec != nullptr)
          LabelCode = ASegLabelList[nhitsmax_label] + LabelCodeOffset;
        else if (AnnotFile != nullptr)
          LabelCode = nhitsmax_label +
                      LabelCodeOffset; // dont +1, keeps consist with ctab
        else
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--native-vox2ras"))
      UseNativeVox2RAS = 1;
    else if (!strcasecmp(option, "--invertmtx"))
      InvertMtx = 1;
    else if (!strcasecmp(option, "--new-aseg2vol"))
      UseNewASeg2Vol = 1;
    else if (!strcasecmp(option, "--no-new-aseg2vol"))
      UseNewASeg2Vol = 0;
    else if (!strcasecmp(option, "--aparc+aseg"))
      UseAParcPlusASeg = 1;
    else if (!strcasecmp(option, "--ttype"))
      ctTissueType = TissueTypeSchema(nullptr, "default-jan-2014");
    else if (!strcasecmp(option, "--ttype+head"))
      ctTissueType = TissueTypeSchema(nullptr, "default-jan-2014+head");
    else if (!strcasecmp(option, "--fill-ribbon"))
      FillRibbon = 1;
      ...
      if (CMDnthIsArg(nargc, pargv, 0)) {
        ...
        if (ASegFSpec)
          LabelVolFile = ASegFSpec;
        else {
          printf("ERROR: --regheader requires one argument unless you\n");
          printf("are going to use a --seg. If so specify that before "
                 "--regheader\n");
          exit(1);
        }
      }
      ...
      if (nargc < 6) {
        printf("usage: --defects surf defectno.mgz template.mgz offset "
               "mergeflag\n");
        argnerr(option, 6);
      }
      ...
      if (defmerge == 0) {
        MRIconst(deftemp->width, deftemp->height, deftemp->depth,
                 deftemp->nframes, 0, deftemp); // set MRI to 0
...
static void print_usage() {
  ...
  printf("     --new-aseg2vol : use new function (eventually will become "
         "default)\n");
  ...
  printf("   --regheader volid : label template volume (needed with --label or "
         "--annot)\n");
  ...
  printf("   --offset k : add k to segmentation numbers (good when 0 should "
         "not be ignored)\n");
  printf("   --defects surf defectno.mgz orig.mgz offset mergeflag (ignores "
         "all other inputs)\n");
  printf("     Creates a segmentation volume of the surface defects, eg, \n");
  printf("       mri_label2vol --defects surf/lh.orig surf/lh.defect_labels "
         "mri/orig.mgz    1000 0 mri/defects.mgz\n");
  printf("       mri_label2vol --defects surf/rh.orig surf/rh.defect_labels "
         "mri/defects.mgz 2000 1 mri/defects.mgz\n");
  printf("\n");
  printf("   --native-vox2ras : use native vox2ras xform instead of "
         "tkregister-style\n");
...
static void print_help() {
  ...
  printf(
      ...
      "surface-based label, then the vertex number will be -1. Not with "
      "--annot\n"
      "or --seg\n"
      "\n"
      "--annot annotfile\n"
      ...
      "--seg segpath\n"
      "\n"
      "Path to input segmentation. A segmentation is a volume in which each "
      "voxel\n"
      "is assigned a number indicating it's class. The output volume will "
      "keep\n"
      "the same numbering. Voting is used to resolve multiple segs mapping to\n"
      "a single output voxel. The registration in this case goes from the seg "
      "to\n"
      "the template volume. Not with --label or --annot.\n"
      "\n"
      "--temp tempvolid\n"
      "\n"
      "Template volume. The output volume will have the same size and "
      "geometry\n"
      "as the template. Template must have geometry information (ie, "
      "direction\n"
      "cosines and voxel sizes). Required.\n"
      "\n"
      "--reg regmatfile\n"
      "\n"
      "tkregister-style registration matrix (see tkregister2 --help) which "
      "maps\n"
      "the XYZ of the label to the XYZ of the template volume. If not "
      "specified,\n"
      "then the identity is assumed.\n"
      "\n"
      "--regheader labelvolume\n"
      "\n"
      "Construct registration based on the header info between the template "
      "volume\n"
      "and the labelvolume. The label should have been defined on labelvolume "
      "(or\n"
      "its equivalent).\n"
      "\n"
      "--identity\n"
      ...
      "any format accepted by mri_convert (eg, nii, mgh, spm, analyze, "
      "bshort).\n"
      "\n"
      "--hits hitvolid\n"
      "\n"
      ...
      "nii, spm, analyze, bshort, mgh). With --seg, this is a single frame "
      "volume\n"
      "with the number of hits from the winning seg id.\n"
      "\n"
      "--native-vox2ras\n"
      "\n"
      "Use the 'native' voxel-to-RAS transform instead of the "
      "tkregister-style.\n"
      "The 'native' vox2ras is what is stored with the volume (usually "
      "scanner)\n"
      "This may be needed depending upon how the label was created (eg, with "
      "scuba).\n"
      "\n"
      "--label-stat labelstatvol\n"
      "\n"
      "Create a volume in which the value at a voxel is the label stat for "
      "that voxel.\n"
      "Note: uses the value from last label point hit. Not recommended for "
      "multiple\n"
      "labels.\n"
      "\n"
      "RESOLVING MULTI-LABEL AMBIGUITIES\n"
      ...
      "\n"
      "value of 1 whereas those assigned to lh-avg_calcarine_sulcus.label "
      "will\n"
      "have a value of 2.\n"
      "\n"
      "KNOWN BUGS\n"
...
/* --------------------------------------------- */
static void check_options() {
  if (DoProj || FillRibbon)
    SurfNeeded = 1;

  if (UseAParcPlusASeg && subject == nullptr && RegMatFile == nullptr) {
    printf("ERROR: you must spec --subject or --reg with --aparc+aseg\n");
    exit(1);
  }

  if (nlabels == 0 && AnnotFile == nullptr && ASegFSpec == nullptr &&
      !UseAParcPlusASeg) {
    printf("ERROR: you must spec at least one label or annot file\n");
    exit(1);
  }
  if (nlabels != 0 && AnnotFile != nullptr) {
    printf("ERROR: you cannot specify a label AND an annot file\n");
    exit(1);
  }
  if (nlabels != 0 && ASegFSpec != nullptr) {
  ...
  if (subject != nullptr && !DoProj && AnnotFile == nullptr &&
      !UseAParcPlusASeg)
    printf("INFO: subject not needed, igorning.\n");
  if (hemi != nullptr && !DoProj && AnnotFile == nullptr && !FillRibbon)
  ...
  if (TempVolId == nullptr && !FillRibbon) {
...
static int get_proj_type_id(char *projtype) {
  ...
  if (!strcmp(projtype, "none"))
    return (PROJ_TYPE_NONE);
  if (!strcmp(projtype, "abs"))
    return (PROJ_TYPE_ABS);
  if (!strcmp(projtype, "frac"))
    return (PROJ_TYPE_FRAC);
...
static int get_crs(MATRIX *Tras2vox, float x, float y, float z, int *c, int *r,
  ...
  cf = Tras2vox->rptr[1][1] * x + Tras2vox->rptr[1][2] * y +
       Tras2vox->rptr[1][3] * z + Tras2vox->rptr[1][4];

  rf = Tras2vox->rptr[2][1] * x + Tras2vox->rptr[2][2] * y +
       Tras2vox->rptr[2][3] * z + Tras2vox->rptr[2][4];

  sf = Tras2vox->rptr[3][1] * x + Tras2vox->rptr[3][2] * y +
       Tras2vox->rptr[3][3] * z + Tras2vox->rptr[3][4];
...
MRI *MRIsurfaceLabel2VolOpt(MRI *ribbon, MRIS *surf, LABEL **labels,
  ...
  for (nthlabel = 0; nthlabel < nlabels; nthlabel++) {
    ...
    for (nthpoint = 0; nthpoint < label->n_points; nthpoint++) {
      if (DoStatThresh)
        if (label->lv[nthpoint].stat < StatThresh)
          continue;
      vtxno = label->lv[nthpoint].vno;
      if (DoLabelStatVol)
        val = label->lv[nthpoint].stat;
      else
        val = 1.0;
