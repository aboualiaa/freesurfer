...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_surf2vol.c,v 1.28 2014/12/08 19:49:02 greve Exp $",
      "$Name:  $");
  ...
  if (narray > 0) {
    ...
    OutVol = MRIsurf2VolOpt(ribbon, surfarray, overlayarray, narray, ArrayLTA,
                            nullptr);
    if (OutVol == nullptr)
      exit(1);
    if (DoAddVal) {
  ...
  if (UseVolRegIdentity) {
    ...
    Ma2vTKR = MatrixIdentity(4, nullptr);
  } else {
    /* Read in the tkregister registration */
    err = regio_read_register(volregfile, &srcsubject, &ipr, &bpr, &intensity,
                              &Ma2vTKR, &float2int);
    if (err)
      exit(1);
  }
  ...
  if ((projfrac != 0) || (fillribbon != 0)) {
    sprintf(fname, "%s/%s/surf/%s.%s", subjectsdir, srcsubject, hemi,
            "thickness");
  ...
  if (!mksurfmask) {
    if (istringnmatch(surfvalfmt, "paint", 0)) {
      ...
      if (SurfVal->height != 1 || SurfVal->depth != 1) {
        ...
        mritmp = mri_reshape(SurfVal, reshapefactor * SurfVal->width, 1, 1,
                             SurfVal->nframes);
        MRIfree(&SurfVal);
        SurfVal = mritmp;
      }
  ...
  if (fillribbon) { /* fill entire ribbon */
    VtxVol = MRIconst(TempVol->width, TempVol->height, TempVol->depth, 1, -1,
                      nullptr);
    printf("VtxVol fixed\n");
    nhits = 0;
    for (projfrac = ProjFracStart; projfrac <= ProjFracStop;
  ...
  if (outvolpath != nullptr) {
    printf("INFO: writing output volume to %s\n", outvolpath);
    MRIwriteType(OutVol, outvolpath, outvolfmtid);
  }

  if (vtxvolpath != nullptr) {
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--mkmask"))
      mksurfmask = 1;
      ...
      if (surfarray[narray] == nullptr)
        exit(1);
      overlayarray[narray] = MRIread(pargv[1]);
      if (overlayarray[narray] == nullptr)
        exit(1);
      narray++;
      nargsused = 2;
      ...
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &res[i]);
      nargsused = 3;
    } else if (!strcmp(option, "--xyz0")) {
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &xyz0[i]);
      nargsused = 3;
    } else if (!strcmp(option, "--cdircos")) {
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &cdircos[i]);
      nargsused = 3;
    } else if (!strcmp(option, "--rdircos")) {
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &rdircos[i]);
      nargsused = 3;
    } else if (!strcmp(option, "--sdircos")) {
      if (nargc < 3)
        argnerr(option, 3);
      for (i = 0; i < 3; i++)
        sscanf(pargv[i], "%f", &sdircos[i]);
      nargsused = 3;
...
static void print_help() {
  ...
  printf(
      ...
      "\n"
      "  --so surface overlay\n"
      "     Specify full path to a surface (eg, lh.white) and full path to the "
      "overlay.\n"
      "     Multiple --so flags are possible. Eg, to fill both lh and rh or "
      "specify one\n"
      "     overlay for white and another for pial.\n"
      "\n"
      "  --lta LTA : registration file\n"
      "     LTA contains info about the output space. Do not specify if you "
      "want the\n"
      "     the output to be in conformed space (but spec --subject or "
      "--ribbon)\n"
      "\n"
      "  --subject subjectname \n"
      ...
      "\n"
      "  --merge mergevolume\n"
      "     Merge the output volume with this volume. Ie, each voxel that is "
      "not\n"
      "     assigned a value from the surface inherits its value from the "
      "merge\n"
      "     volume. \n"
      "\n"
      "Example: to create a conformed volume with the thickness\n"
      "  mri_surf2vol --o thickness-in-volume.nii.gz --subject bert \\\n"
      "     --so $SUBJECTS_DIR/bert/surf/lh.white "
      "$SUBJECTS_DIR/bert/surf/lh.thickness \\\n"
      "     --so $SUBJECTS_DIR/bert/surf/rh.white "
      "$SUBJECTS_DIR/bert/surf/rh.thickness \n"
      "\n"
      "Method 2 (old method)\n"
      "  Option not to fill the ribbon. Ribbon fill uses projection instead of "
      "construction\n"
      ...
      "Note that the volume can be filled 'into' the surface by setting stop < "
      "0,\n"
...
static void check_options() {
  if (narray > 0) {
    if (ribbon == nullptr && ArrayLTA == nullptr && subject == nullptr) {
      printf("ERROR: if not specifying LTA or subject, then must specify "
             "ribbon\n");
      exit(1);
    }
    if (outvolpath == nullptr) {
  ...
  if (mksurfmask && surfvalpath != nullptr) {
  ...
  if (volregfile != nullptr && UseVolRegIdentity) {
