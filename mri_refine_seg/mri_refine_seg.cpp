...
struct Component {
  ...
  int additional; // additional label (like hypointensities) used to define a
                  // component
  Component(int l) : label(l), max(1), additional(0){};
};

// Finds clusters of a label set. Being able to pass a mask buffer prevents
// having to reallocate a new mask volume everytime this function gets called.
static VOLCLUSTER **getLabelClusters(MRI *seg, MRI *mask_buffer, intvec labels,
                                     int *nclusters) {
  // mask away non-matching labels in the segmentation
  MRIbinarizeMatch(seg, &labels[0], labels.size(), 0, mask_buffer);
  return clustGetClusters(mask_buffer, 0, 1, 1, 1, 0, mask_buffer, nclusters,
                          nullptr);
}
...
static int commonNeighbor(MRI *seg, int c, int r, int s) {
  ...
  if (c - 1 >= 0)
    frequencies[MRIgetVoxVal(seg, c - 1, r, s, 0)]++;
  if (r - 1 >= 0)
    frequencies[MRIgetVoxVal(seg, c, r - 1, s, 0)]++;
  if (s - 1 >= 0)
    frequencies[MRIgetVoxVal(seg, c, r, s - 1, 0)]++;
  if (c + 1 < seg->width)
    frequencies[MRIgetVoxVal(seg, c + 1, r, s, 0)]++;
  if (r + 1 < seg->height)
    frequencies[MRIgetVoxVal(seg, c, r + 1, s, 0)]++;
  ...
  // if the most frequent neighbor is the value of the center voxel, return the
  // second-most frequent
  int most_frequent_neighor = neighbors[0].first;
  if ((most_frequent_neighor == MRIgetVoxVal(seg, c, r, s, 0)) &&
      (neighbors.size() > 1)) {
...
// Recodes cluster voxels (of a particular label) with the values of their
...
static void recodeCluster(MRI *seg, VOLCLUSTER *clr, int label) {
  ...
  for (unsigned int i = 0; i < fill_indices.size(); i++)
    fill_indices[i] = i;

  // continuously loop through voxels in cluster until every valid voxel has
  // been recoded
...
static bool clusterSurrounded(MRI *seg, VOLCLUSTER *clr, intvec labels) {
  for (int i = 0; i < clr->nmembers; i++) {
    ...
    if (r - 1 >= 0)
      surrounding.push_back(MRIgetVoxVal(seg, c, r - 1, s, 0));
    if (s - 1 >= 0)
      surrounding.push_back(MRIgetVoxVal(seg, c, r, s - 1, 0));
    if (c + 1 < seg->width)
      surrounding.push_back(MRIgetVoxVal(seg, c + 1, r, s, 0));
    if (r + 1 < seg->height)
      surrounding.push_back(MRIgetVoxVal(seg, c, r + 1, s, 0));
    ...
    for (int label : surrounding) {
      if (!vectorContains(labels, label))
        return false;
    }
  }
  return true;
...
int main(int argc, char **argv) {
  ...
  if (!seg)
    fs::fatal() << "could not read input volume " << segname;
  ...
  for (int i = 0; i < num_unique; i++) {
    ...
    // create a valid component if the label isn't excluded from stray-cluster
    // refinement
    if (vectorContains(exclude_labels, label))
      continue;
    Component comp(label);
    // allow for non-wm hypos to be included in the definition of some gm
    // structures
    if (vectorContains(subcortical_gm_labels, label))
      comp.additional = 80;
    // allow for wm hypos to be included in the white matter definition
    if (vectorContains({2, 41}, label))
      comp.additional = 77;
    // increase the allowed number of clusters for lateral ventricles
    if (vectorContains({4, 43}, label))
      comp.max = 2;
    // add component
  ...
  // loop through each component and recode any extra clusters using majority
  // vote
  ...
  while (stray_labels) {
    ...
    for (Component &comp : components) {
      ...
      if (comp.additional > 0)
        labels.push_back(comp.additional);
      ...
      for (int clustid = comp.max; clustid < nclusters; clustid++) {
        // for components that are defined by multiple labels (i.e. structures
        // that also include hypointensities), make sure the cluster actually
        // contains some of the primary structure before recoding
  ...
  // recode any non-wm hypo clusters that are surrounded by wm and ventricle to
  // wm hypos
  ...
  for (int clustid = 0; clustid < nclusters; clustid++) {
    if (clusterSurrounded(seg, clusters[clustid], {80, 2, 41, 4, 43}))
      fillCluster(seg, clusters[clustid], 77);
  }
  clustFreeClusterList(&clusters, nclusters);
  ...
  // // a small strip of white matter called the extreme capsule seperates the
  // putamen and
  // // insula, and this can be reflected in segmentations by recoding putamen
  // voxels that neighbor insula std::cout << "correcting extreme capsule" <<
  // std::endl; clusters = getLabelClusters(seg, mask_buffer, {12, 51},
  // &nclusters);
  ...
  // if debugging is turned on, save the original segmentation, the difference
  // mask, and a json pointset that points to the changed regions
  if (parser.exists("debug")) {

    std::cout << "saving debug pointset, mask, and original segmentation"
              << std::endl;

    // saving a pointset representing each changed voxel will be quite large
    // (and tedious to use), so instead we'll compute clusters of the
    // change-mask and save a point from there
    clusters = clustGetClusters(mask_buffer, 0, 1, 1, 1, 0, mask_buffer,
                                &nclusters, nullptr);
    ...
    MRIwrite(mask_buffer,
             std::string(dirname + "/refinement.mask.mgz").c_str());
  }
