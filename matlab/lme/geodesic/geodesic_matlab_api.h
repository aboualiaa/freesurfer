...
long new_algorithm(
    long mesh_id, // creates a geodesic algorithm for a given mesh
    long type, long subdivision);
...
long trace_back(
    long algorithm_id, // using procomputed distance field, compute a shortest
                       // path from destination to the closest source
    double *destination, double **path);

long distance_and_source(
    long algorithm_id, // quickly find what source this point belongs to and
                       // what is the distance to this source
    double *destination, double *best_source_distance);
