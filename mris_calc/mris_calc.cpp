...
char *Gppch_filetype[] = {"Unknown", "Volume", "Curvature", "Surface",
                          "FloatArg"};

char *Gppch_fileExt[] = {"null", "mgz", "crv", "surf", "dummy"};

char *Gppch_fileDotExt[] = {".null", ".mgz", ".crv", ".surf", ".dummy"};
...
const char *Gppch_operation[] = {"multiply",
                                 "divide",
                                 "modulus",
                                 "add",
                                 "pow",
...
static char *G_pch_curvFile1 = nullptr;
static float *G_pf_arrayCurv1 = nullptr;
static float *G_pf_arrayCurv1Copy = nullptr;
...
static char *G_pch_curvFile2 = nullptr; //+ file is optional.
static int G_sizeCurv2 = 0;
static float *G_pf_arrayCurv2 = nullptr;
static float *G_pf_arrayCurv2Copy = nullptr;
...
// "Helper" pointers
static MRI *Gp_MRI = nullptr; // Pointer to most
//+ recently read
//+ MRI_VOLUME struct
//+ and used in volume
//+ handling... mostly
...
static char *G_pch_operator = nullptr;
static e_operation Ge_operation = e_unknown;

// Output file
static int G_sizeCurv3 = 0;
static char G_pch_curvFile3[STRBUF];
static float *G_pf_arrayCurv3 = nullptr;
static float *G_pf_arrayCurv3Copy = nullptr;
...
static LABEL *G_plabel = nullptr;
static float *G_pf_labelMask = nullptr;
static float *G_pf_labelMaskIndex = nullptr;
...
static void synopsis_show();
static void version_print();

// Setup functions
static int options_parse(int argc, char *apchv[]);
static int options_print();
...
double fn_sig2p(float af_A) { return (pow(10, -fabs(af_A))); }
...
double fn_andbw(float af_A, float af_B) { return ((int)af_A & (int)af_B); }
double fn_orbw(float af_A, float af_B) { return ((int)af_A | (int)af_B); }
...
double fn_inv(float af_A) { return (1.0 / af_A); }
...
// Info functions
e_FILETYPE fileType_find(char *apch_inputFile, int *ap_FSFILETYPE);

// I/O functions
short CURV_arrayProgress_print(int asize, int acurrent, char *apch_message);

e_FILEACCESS VOL_fileRead(char *apch_VolFileName, int *ap_vectorSize,
                          float *apf_volData[]);

e_FILEACCESS VOL_fileWrite(char *apch_VolFileName, int a_vectorSize,
                           float *apf_volData);

e_FILEACCESS CURV_fileRead(char *apch_curvFileName, int *ap_vectorSize,
                           float *apf_curv[]);

e_FILEACCESS CURV_fileWrite(char *apch_curvFileName, int a_vectorSize,
                            float *apf_curv);
...
static void synopsis_show() {
  outputHelpXml(mris_calc_help_xml, mris_calc_help_xml_len);
  exit(1);
}
...
void verbosity_set() {
  if (G_verbosity) {
    ...
    ErrorExit(ERROR_NOFILE, "%s: Could not open /dev/null for console sink!",
              G_pch_progname);
}
...
void error_exit(char *apch_action, char *apch_error, int exitCode) {
...
void error_unmatchedSizes() {
  error_exit("checking on input filetype sizes",
             "I found a size mismatch, i.e. len(input1)!=len(input2)", 10);
}

void error_noVolumeStruct() {
  error_exit("checking on output volume",
             "it seems that internal volume structure is invalid.", 10);
}

void error_volumeWriteSizeMismatch() {
  error_exit("checking on output volume",
             "I found a size mismatch, internal data does not fit into vol.",
             10);
}

void error_incompatibleInputFileTypes() {
  error_exit("checking on input filetypes",
             "it seems that you specified incompatible types.", 10);
}

void error_incompatibleOutputFileType() {
  error_exit("checking on output filetype",
             "it seems there is a type mismatch to the input type.", 10);
}

void error_zeroLengthInput() {
  error_exit("checking input 1",
             "it seems that the input file has no contents.", 10);
}

void error_surfacesNotHandled() {
  error_exit("checking inputs", "FreeSurfer surface files are not handled yet.",
             10);
}

void error_unknownFileType(char *apch_inputFile) {
  char pch_action[65536];
  sprintf(pch_action, "accessing file '%s'", apch_inputFile);
  error_exit(pch_action,
             "I was unable to identify this file type. Does it exist?", 10);
}

void error_primaryCurvsBackup() {
  error_exit("making backup of internal data arrays",
             "it seems that some of the backups already exist.", 11);
}

void error_primaryCurvsCompress() {
  error_exit("compressing primary internal data arrays about labels",
             "an internal memory error occurred.", 11);
}

void error_expandCurv3() {
...
void fileIO_errorHander(char *pch_filename, e_FILEACCESS eERROR) {
  switch (eERROR) {
  case e_UNSPECIFIED:
    break;
  case e_OK:
...
e_FILEACCESS fileRead(char *apch_fileName, int *ap_vectorSize,
                      float *apf_curv[], e_FILETYPE *aeFILETYPE,
                      int *a_FSFILETYPE) {
...
e_FILEACCESS fileWrite(char *apch_fileName, int a_vectorSize, float *apf_curv) {
...
short CURV_set(float *apf_array[], int a_size, float af_val) {
  //
  // PRECONDITIONS
...
short CURV_copy(float apf_arraySrc[], float apf_arrayDst[], int a_size) {
  //
  // PRECONDITIONS
...
/*!
  \fn array_compressUsingMask(float* apf_input, int a_sizeInput, float*
  apf_output[], int a_sizeOutput) \brief Using the internal mask label, compress
  the input array to masked values \param apf_input The input source array to
  compress. \param a_sizeInput The size of the input source array. \param
  apf_output The output array containing the compression. This array is
  destroyed and rebuilt \param a_sizeOutput The size of the output source array.
  \return TRUE -> OK; FALSE -> Error
*/
short array_compressUsingLabelMask(float *apf_input, int a_sizeInput,
                                   float *apf_output[], int a_sizeOutput) {
  //
  // PRECONDITIONS
...
short label_initialize() {
  ...
  if (!Gb_labelMask) {
    ...
    G_plabel = LabelRead(nullptr, G_pch_labelFile);
    if (G_verbosity) {
      cprints("", "ok");
    }
    if (!G_plabel)
...
/*!
  \fn array_expandUsingLabelMask(float* apf_compressedIn, int a_sizeInput,
  float* apf_output[], int a_sizeOutput) \brief Using the outputTemplate, expand
  the compressedInput array. \param apf_compressedIn The input source array to
  compress. \param a_sizeInput The size of the input source array. \param
  apf_outputTemplate The output array containing the compression. This array is
  destroyed and rebuilt \param a_sizeOutput The size of the output source array.
  \return TRUE -> OK; FALSE -> Error
*/
short array_expandUsingLabelMask(float *apf_compressedIn[], int a_sizeInput,
                                 float *apf_outputTemplate, int a_sizeOutput) {
  //
  // PRECONDITIONS
  // o Label mask array and size must be non-zero (i.e. defined)
  ...
  int n = 0;
  int m = 0;
  float *pf_inputCopy = nullptr;
  float *pf_compressed = nullptr;
...
int main(int argc, char *argv[]) {
  ...
  eACCESS = fileRead(G_pch_curvFile1, &G_sizeCurv1, &G_pf_arrayCurv1,
                     &G_eFILETYPE1, &G_FSFILETYPE1);
  if (eACCESS != e_OK) {
  ...
  if (Gb_curvFile2) {
    eACCESS = fileRead(G_pch_curvFile2, &G_sizeCurv2, &G_pf_arrayCurv2,
                       &G_eFILETYPE2, &G_FSFILETYPE2);
...
/*!
  ...
  \return If volume file is successfully read, return e_OK. If file could not be
  opened, return e_READACCESSERROR.
*/
e_FILEACCESS VOL_fileRead(char *apch_volFileName, int *ap_vectorSize,
                          float *apf_data[]) {
  ...
  if ((pMRI = MRIread(apch_volFileName)) == nullptr) {
...
/*!
  ...
  \return If volume file is successfully written, return e_OK, else return
  e_WRITEACCESSERROR.
*/
e_FILEACCESS VOL_fileWrite(char *apch_volFileName, int a_vectorSize,
                           float *apf_data) {
  ...
  sprintf(pch_readMessage, "Packing %s", apch_volFileName);
  out = MRIallocSequence(Gp_MRI->width, Gp_MRI->height, Gp_MRI->depth,
                         MRI_FLOAT, Gp_MRI->nframes);
...
/*!
  \fn CURV_fileRead(char* apch_curvFileName, int* ap_vectorSize, float*
  apf_curv) \brief Read a FreeSurfer curvature file into a float array \param
  apch_curvFileName The name of a FreeSurfer curvature file. \param
  ap_vectorSize Pointer to the size (i.e. number of elements) in file. \param
  apf_curv Array containing the curvature values. \return If curvature file is
  successfully read, return e_OK. If file could not be opened, return
  e_READACCESSERROR. If file not a curvature format file, return
  e_WRONGMAGICNUMBER.
*/
...
e_FILEACCESS CURV_fileRead(char *apch_curvFileName, int *ap_vectorSize,
  ...
  if ((FP_curv = fopen(apch_curvFileName, "r")) == nullptr) {
    return (e_READACCESSERROR);
  }
  fread3(&vnum, FP_curv);
...
/*!
  ...
  \return If output file is successfully written, return e_OK, else return
  e_WRITEACCESSERROR.
*/
e_FILEACCESS ascii_fileWrite(char *apch_fileName, float *apf_data) {
  FILE *FP_curv;
  int i;
  char pch_readMessage[STRBUF];

  if ((FP_curv = fopen(apch_fileName, "w")) == nullptr) {
...
/*!
  \fn CURV_fileWrite(char* apch_curvFileName, int* ap_vectorSize, float*
  apf_curv) \brief Write a FreeSurfer curvature array to a file \param
  apch_curvFileName The name of a FreeSurfer curvature file. \param a_vectorSize
  Size (i.e. number of elements) of data array. \param apf_curv Array containing
  the curvature values. \return If curvature file is successfully written,
  return e_OK, else return e_WRITEACCESSERROR.
*/
e_FILEACCESS CURV_fileWrite(char *apch_curvFileName, int a_vectorSize,
                            float *apf_curv) {
  FILE *FP_curv;
  ...
  if ((FP_curv = fopen(apch_curvFileName, "w")) == nullptr) {
    return (e_WRITEACCESSERROR);
  }
...
/*!
  ...
  \see
  \return A simple TRUE or FALSE indicating if for the passed operation an
...
short b_outCurvFile_write(e_operation e_op) {
  ...
  if (e_op == e_mul || e_op == e_div || e_op == e_mod || e_op == e_pow ||
      e_op == e_add || e_op == e_sub || e_op == e_sub0 || e_op == e_sratio ||
      e_op == e_pctdiff || e_op == e_pctdiff0 || e_op == e_sqd ||
      e_op == e_sqr || e_op == e_sqrt || e_op == e_not || e_op == e_set ||
      e_op == e_atan2 || e_op == e_bcor || e_op == e_sig2p || e_op == e_mag ||
      e_op == e_log10 || e_op == e_abs || e_op == e_inv || e_op == e_sign ||
      e_op == e_eq || e_op == e_lt || e_op == e_lte || e_op == e_gt ||
      e_op == e_gte || e_op == e_masked || e_op == e_and || e_op == e_or ||
      e_op == e_andbw || e_op == e_orbw || e_op == e_upperlimit ||
      e_op == e_lowerlimit || e_op == e_norm || e_op == e_log) {
    b_ret = 1;
  }
...
short CURV_process() {
  ...
  if (Ge_operation == e_ascii) {
    ...
    if ((ascii_fileWrite(G_pch_curvFile3, G_pf_arrayCurv1)) ==
        e_WRITEACCESSERROR) {
