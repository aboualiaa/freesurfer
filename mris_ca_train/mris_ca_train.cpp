...
int main(int argc, char *argv[]) {
    ...
    if (label_name) {
      ...
      if (label_index < 0)
        ErrorExit(ERROR_BADPARM,
                  "%s: could not find index for label %s in table %s", Progname,
                  label_name, ptable_fname);
  ...
  if (sulconly) {
    ...
    GCSAputInputType(gcsa, GCSA_INPUT_CURVATURE, "mean_curvature", navgs,
                     input1_flags, 0);
    if (ninputs > 1)
      GCSAputInputType(gcsa, GCSA_INPUT_CURV_FILE, sulc_name, 0, input2_flags,
                       1);
    if (ninputs > 2)
...
static void print_help() {
  ...
  fprintf(stderr, "  <canon surf>         canonical surface filename\n\n");
  fprintf(stderr, "  <annot file>         annotation filename\n\n");
  fprintf(stderr, "  <subject 1> <subject 2> ...  the subject id(s)\n\n");
  fprintf(stderr, "  <outputfile>         classifier array output file\n\n");
  fprintf(stderr, "Optional args:\n"
                  "--------------\n\n");
  fprintf(stderr,
          "  -sdir <directory>    use <directory> as subjects directory \n"
          "                       (default: $SUBJECTS_DIR)\n\n");
  fprintf(stderr, "  -nbrs <number>       neighborhood size (default=2)\n\n");
  fprintf(stderr,
  ...
  fprintf(stderr, "  -sulconly             same as -sulc\n\n");
  fprintf(stderr, "  -a <number>           number of averages (default=5)\n\n");
  fprintf(stderr,
          "  -t <filename>         specify parcellation table input file \n"
          "                        (default: none)\n\n");
  fprintf(stderr, "  -v <number>           diagnostic level (default=0)\n\n");
  fprintf(stderr, "  -n <number>           number of inputs (default=1)\n\n");
