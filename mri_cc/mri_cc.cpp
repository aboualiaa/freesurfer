...
int main(int argc, char *argv[]) {
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    int req = snprintf(ifname, STRLEN, "%s/%s/mri/cc_volume_%d.txt", data_dir,
                       argv[1], dxi);
    if (req >= STRLEN) {
    ...
    if ((fp = fopen(ifname, "a")) == NULL) {
      ErrorReturn(ERROR_BADFILE,
                  (ERROR_BADFILE,
                   "could not open cc volume measurement file %s", ifname));
    }
  ...
  if (use_aseg) {
    ...
    int req =
        snprintf(ifname, STRLEN, "%s/%s/mri/%s", data_dir, argv[1], aseg_fname);
    if (req >= STRLEN) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    }
    ...
    if (mri_aseg == NULL)
      ErrorExit(ERROR_NOFILE, "%s: could not read aseg volume from %s",
                Progname, ifname);
    if (lh_only || rh_only) {
      int req = snprintf(ofname, STRLEN, "%s/%s/mri/%s", data_dir, argv[1],
                         output_fname);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
    ...
    req =
        snprintf(ifname, STRLEN, "%s/%s/mri/%s", data_dir, argv[1], norm_fname);
    if (req >= STRLEN) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    }
    ...
    req = snprintf(ifname, STRLEN, "%s/%s/mri/transforms/talairach.xfm",
                   data_dir, argv[1]);
    if (req >= STRLEN) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    ...
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
      int req =
          snprintf(ofname, STRLEN, "%s/%s/mri/wm_tal.mgz", data_dir, argv[1]);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
      }
    ...
    *MATRIX_RELT(mrot, 3, 4) = mtrans->rptr[3][4];
    ...
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
      int req =
          snprintf(ofname, STRLEN, "%s/%s/mri/cc_tal.mgz", data_dir, argv[1]);
      if (req >= STRLEN) {
        std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                  << std::endl;
      }
  ...
  for (y = 0; y < mri_cc->height; y++) {
    for (z = 0; z < mri_cc->depth; z++) {
      if (nint(MRIgetVoxVal(mri_cc, xi, y, z, 0))) {
        zf = (xi - means[0]) * ez_x + (y - means[1]) * ez_y +
             (z - means[2]) * ez_z;
        if (zf < zf_low) {
          zf_low = zf;
        }
    ...
    for (j = 0; j < mri_cc->height; j++) {
      for (k = 0; k < mri_cc->depth; k++) {
        if (nint(MRIgetVoxVal(mri_cc, i, j, k, 0)) > 0) {
          zf = (i - means[0]) * ez_x + (j - means[1]) * ez_y +
               (k - means[2]) * ez_z;
          if (zf >= zf_low - 10) {
            temp =
                floor((zf - zf_low) / ((zf_high - zf_low + 1) / cc_divisions));
            if (temp < 0) {
              temp = 0;
            }
  ...
  if (use_aseg) {
    ...
    mri_tmp = MRIlinearTransformInterp(mri_cc, NULL, lta->inv_xforms[0].m_L,
                                       SAMPLE_NEAREST);
    MRIcopy(mri_tmp, mri_cc);
    MRIfree(&mri_tmp);
    mri_tmp = MRIlinearTransformInterp(mri_fornix, NULL, lta->inv_xforms[0].m_L,
                                       SAMPLE_NEAREST);
        ...
        for (i = 0; i < mseg->nsegments; i++) {
          ...
          if (mseg->segments[i].y0 > mseg->segments[j].y1) {
            ...
            for (k = 0; k < seg->nvoxels; k++)
              MRIsetVoxVal(mri_aseg, seg->voxels[k].x, seg->voxels[k].y,
                           seg->voxels[k].z, 0, CC_Anterior);
          }
        }
      }
      MRIsegmentFree(&mseg);
  ...
  if (write_cc) {
    int req =
        snprintf(ofname, STRLEN, "%s/%s/mri/cc_new.mgz", data_dir, argv[1]);
    if (req >= STRLEN) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    }
  ...
  if (mri_fornix && fornix) {
    int req =
        snprintf(ofname, STRLEN, "%s/%s/mri/fornix.mgz", data_dir, argv[1]);
    if (req >= STRLEN) {
      std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__
                << std::endl;
    }
...
static int find_corpus_callosum(MRI *mri_tal, double *pccx, double *pccy,
                                double *pccz, const LTA *lta, MRI *mri_cc_tal) {
    ...
    while (thickness == 0 && yv - extension >= 0 && yv + extension <= 256) {
      ...
      if (y < yv + extension) // within the region and bigger than so far
      {
        for (y1 = y, thickness = 0; y1 < slice_size; y1++, thickness++)
          if (!nint(MRIgetVoxVal(mri_tal, x, y1, zv,
                                 0))) // if becomes zero, then break out
          {
            break;
          }
...
static int find_cc_slice(MRI *mri_tal, double *pccx, double *pccy, double *pccz,
  ...
  int area[MAX_SLICES * 2], flag[MAX_SLICES * 2], min_area, min_slice, slice,
      offset, xv, yv, zv, i, total_area = 0, left = 0, right = 0;
...
static MRI *remove_fornix(MRI *mri_filled, int xv, int yv, int zv) {
    ...
    if (edge_count >= 3 && flag == 0) {
      ...
      if (y2 <= yi_low + 20) {
        break;
               ...
               (section1[i] >= 19 || (4 * section1[i - 1] < 3 * section1[i]))) {
      x2 = old_temp;
      y2 = y_edge;
      if (y2 <= yi_low + 22) {
        break;
  ...
  if (x2 > 0 && x1 > xi_low + 8 && x1 > x2 && x1 < zv + 5) {
    ...
    for (x = x1; x <= x2; x++) {
      for (y = nint(y1 + (y2 - y1) * (x - x1) / (x2 - x1)) + 1; y < height;
           y++) {
...
static void print_usage() {
  outputHelpXml(mri_cc_help_xml, mri_cc_help_xml_len);
}
static int get_option(int argc, char *argv[]) {
  int   nargs = 0;
  char *option;
    ...
    case 'D':
      ...
      if (cc_divisions > MAX_CC_DIVISIONS)
        ErrorExit(ERROR_UNSUPPORTED,
                  "%s: too many CC divisions specified (max = %d)\n",
...
static MRI *find_cc_with_aseg(MRI *mri_aseg_orig, MRI *mri_cc, LTA **plta,
                              double *pxc, double *pyc, double *pzc, int thick,
                              MRI *mri_norm, MRI *mri_fornix) {
  LTA *       lta;
  ...
  float       evalues[3];
  int y0, z0, y0_best, z0_best, label, i, correct, max_correct, xmin, xmax,
      scale, slice_voxels[MAX_CENTRAL_SLICES], best_slice, changed, xleft_min,
      xright_max;
  ...
  *plta = lta         = LTAalloc(1, NULL);
  ...
  mri_midline = find_voxels_close_to_both_hemis(mri_aseg, Left_Cerebral_Cortex,
                                                Right_Cerebral_Cortex, 5);
  if (Gdiag & DIAG_WRITE) {
  ...
  y0      = means[1];
  z0      = means[2];
  correct = cc_cutting_plane_correct(mri_aseg, x0, y0, z0, dx, dy, dz, vl_left,
                                     vl_right, 0);
  max_correct = correct;
  x0_best     = x0;
  y0_best     = y0;
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON)
    correct = cc_cutting_plane_correct(mri_aseg, x0_best, y0_best, z0_best, dx,
                                       dy, dz, vl_left, vl_right, 2);
  ...
  *MATRIX_RELT(m_trans_inv, 3, 4) = -128;
  ...
  mri_tmp = MRIlinearTransformInterp(mri_aseg, NULL, lta->xforms[0].m_L,
                                     SAMPLE_NEAREST);
  ...
  if (xleft_min > xright_max)
    ErrorExit(
        ERROR_UNSUPPORTED,
        "%s: no WM voxels found with norm > %d -- check skull stripping\n",
  ...
  do {
    ...
    for (y0 = 0; y0 < mri_cc->height; y0++)
      for (z0 = 0; z0 < mri_cc->depth; z0++) {
        ...
        if (MRIlabelsInPlanarNbhd(mri_cc, best_slice, y0, z0, 1, LABEL_IN_CC,
                                  MRI_SAGITTAL) > 1) {
  ...
  *pzc  = (double)z0_best;
  ...
  for (x0 = best_slice - thick; x0 <= best_slice + thick;
       x0++) // build fornix model slice-by-slice
  {
    if (x0 == Gdiag_no) {
      DiagBreak();
    }
    ...
    for (y0 = 0; y0 < mri_cc->height; y0++)
      for (z0 = 0; z0 < mri_cc->depth; z0++) {
        ...
        if ((label == Left_Cerebral_White_Matter ||
            ...
            (MRIlabelsInPlanarNbhd(mri_cc, x0, y0, z0, 1, LABEL_IN_CC,
                                   MRI_SAGITTAL) > 1)) {
          MRIsetVoxVal(mri_cc, x0, y0, z0, 0, LABEL_IN_CC);
        }
      }
  ...
  do {
    for (y0 = 0; y0 < mri_cc->height; y0++)
      for (z0 = 0; z0 < mri_cc->depth; z0++) {
        ...
        if (label &&
            MRIlabelsInNbhd(mri_cc, best_slice, y0, z0, 1, LABEL_IN_CC) >= i) {
          changed = 1;
          *pyc    = y0;
          y0_best = y0;
          *pzc    = z0;
...
static int cc_cutting_plane_correct(MRI *mri_aseg, double x0, double y0,
                                    ...
                                    int debug) {
  int i, xv_left, yv_left, zv_left, xv_right, yv_right, zv_right, width, height,
      label_left, label_right, correct;
...
#define SDIST 0.25
        ...
        if (xv_left < 0 || xv_left >= mri_aseg->width || yv_left < 0 ||
            yv_left >= mri_aseg->height || zv_left < 0 ||
            zv_left >= mri_aseg->depth) {
          continue;
        }
        label_left = MRIgetVoxVal(mri_aseg, xv_left, yv_left, zv_left, 0);

        if (xv_right < 0 || xv_right >= mri_aseg->width || yv_right < 0 ||
            yv_right >= mri_aseg->height || zv_right < 0 ||
            zv_right >= mri_aseg->depth) {
          continue;
        }
        if (mri_debug) {
  ...
  for (i = 0; i < vl_left->nvox; i++) {
    if ((vl_left->xi[i] == Gx) && (vl_left->yi[i] == Gy) &&
        (vl_left->zi[i] == Gz)) {
      DiagBreak();
    }
  ...
  for (i = 0; i < vl_right->nvox; i++) {
    if ((vl_right->xi[i] == Gx) && (vl_right->yi[i] == Gy) &&
        (vl_right->zi[i] == Gz)) {
      DiagBreak();
    }
  ...
  */
  {
    ...
    for (x = xmin; x <= xmax; x++)
      for (y = ymax - 10; y <= ymax; y++) {
        ...
        if ((nint(MRIgetVoxVal(mri_slice_edited, x - 1, y, 0, 0)) ==
             LABEL_IN_CC) &&
            (nint(MRIgetVoxVal(mri_slice_edited, x - 2, y, 0, 0)) ==
             LABEL_IN_CC) &&
            (nint(MRIgetVoxVal(mri_slice_edited, x - 3, y, 0, 0)) ==
             LABEL_IN_CC)) {
          MRIsetVoxVal(mri_slice_edited, x, y, 0, 0, LABEL_IN_CC);
        }
      }
