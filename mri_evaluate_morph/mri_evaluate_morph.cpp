  ...
  for (overlap = 0.0f, x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      for (z = 0; z < depth; z++) {
        ...
        TransformSample(transform1, x, y, z, &x1, &y1,
                        &z1); /* atlas coords of s1 */
        TransformSampleInverseVoxel(transform2, width, height, depth, nint(x1),
                                    nint(y1), nint(z1), &x2, &y2, &z2);
