...
// Revision Date  : $Date: 2011/03/02 00:04:22 $
...
static char *orig_fname = nullptr;
...
int main(int argc, char *argv[]) {
  ...
  MRI *mri_in, *mri_tmp, *mri_orig = nullptr;
  GCA *gca;
  ...
  if (!gca)
    ErrorExit(ERROR_NOFILE, "%s: could not read input atlas from %s", Progname,
              gca_fname);
  ...
  if (orig_fname) {
    ...
    if (mri_orig == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read orig volume from %s",
                Progname, orig_fname);
  }
