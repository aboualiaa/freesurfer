...
int main(int argc, char *argv[]) {
  ...
  if (label) {
      ...
      if (FileExists(control_fname)) {
        ...
        if (useRealRAS)
          ErrorExit(ERROR_UNSUPPORTED,
                    "%s: cannot combine existing RAS and current tkregRAS "
                    "control points",
                    Progname);
        duplicate = 0;
    ...
    for (n = 0; n < label->n_points; n++) {
      xv = nint(label->lv[n].x);
      yv = nint(label->lv[n].y);
      zv = nint(label->lv[n].z);
  ...
  if (strlen(sdir) == 0) {
    ...
    if (cp)
      ...
      ErrorExit(
          ERROR_UNSUPPORTED,
          "%s: must specify SUBJECTS_DIR in env or with -sdir on cmd line",
          Progname);
  }
  ...
  for (i = 1; i < argc - 1; i++) {
    ...
    if (i == 1) {
      ...
      MRIcalcCRASforExtractedVolume(
          mri_orig, mri_bias, pad, pad, pad, pad + mri_orig->width,
          pad + mri_orig->height, pad + mri_orig->depth, &c_r, &c_a, &c_s);
...
static int update_bias(MRI *mri_orig, MRI *mri_T1, MRI *mri_brain,
  ...
  mri_tmp =
      MRIalloc(mri_orig->width, mri_orig->height, mri_orig->depth, MRI_FLOAT);
