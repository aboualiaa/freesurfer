...
FSTEST_TESTDATA_DIR="${FSTEST_CWD}/fulllto/testdata"
...
function compare_vol() {
  diffcmd=$(find_path $FSTEST_CWD mri_diff/fulllto/mri_diff)
...
function compare_surf() {
  diffcmd=$(find_path $FSTEST_CWD mris_diff/fulllto/mris_diff)
  run_comparison $diffcmd $@ --debug
}

# runs lta_diff on an output and reference transform - all extra opts are passed to lta_diff
function compare_lta() {
  diffcmd=$(find_path $FSTEST_CWD mri_robust_register/fulllto/lta_diff)
