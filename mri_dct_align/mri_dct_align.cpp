/**
 * @file  mri_dct_align.c
 * @brief program for computing a nonlinear alignment using a discrete cosine
 * transform
 *
...
static int quasi_newton_minimize(DCT *dct, MRI *mri_source, MRI *mri_target,
                                 GCA_MORPH_PARMS *gmp);
...
static float compute_sse(float *p) {
  ...
  x2 = Gsource_box.x + Gsource_box.dx;
  y2 = Gsource_box.y + Gsource_box.dy;
  z2 = Gsource_box.z + Gsource_box.dz;
  nvox =
      Gsource_box.dx * Gsource_box.dy * Gsource_box.dz / (pow(skip + 1, 3.0));
  nvox = 0;
  sse = 0.0;
