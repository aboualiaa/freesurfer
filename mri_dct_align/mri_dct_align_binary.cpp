...
#define EXTRA 5
      ...
      MORPH_3D_TYPE) // initializing m3d from a linear transform
  {
    ...
    ErrorExit(ERROR_UNSUPPORTED, "%s: can't initialize DCT with GCAM",
              Progname);
  }
  ...
  for (i = 0; i < mp.npasses; i++) {
    ...
    dct = find_optimal_dct(dct, mri_source, mri_target, vl_source, vl_target,
                           Gncoef, skip);
...
static void usage_exit(int ecode) {
  printf("usage: %s <source> <destination> <output xform>\n", Progname);
  exit(ecode);
}
...
static DCT *find_optimal_dct(DCT *dct, MRI *mri_source, MRI *mri_target,
  ...
  if (dct == nullptr) {
    ...
    sse = compute_global_dct_optimum(dct, mri_source, mri_target, vl_source,
                                     vl_target);
...
static double compute_global_dct_optimum(DCT *dct, MRI *mri_source,
  ...
  for (i = 0; i < NSCALES; i++) {
    ...
    overlap = (*pf_overlap)(vl_target, vl_source, dct);
    overlap = compute_recursive_optimum(dct, scale, vl_source, vl_target, 0, 0,
                                        overlap);
  }
...
static double compute_recursive_optimum(DCT *dct, double scale,
  ...
  for (coef = orig_coef - scale; coef <= orig_coef + scale * 1.1;
       coef += scale) {
    *pk = coef;
    if (k < dct->ncoef - 1)
