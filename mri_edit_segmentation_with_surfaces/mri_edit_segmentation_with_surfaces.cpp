...
static int relabel_hypointensities(MRI *mri, MRI *mri_inputs, MRI_SURFACE *mris,
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 2; i++) {
    for (h = 0; h <= 1; h++) {
      ...
      if (mris->ct == nullptr) /* color table not in annotation file */
      {
    ...
    if (transform && i == 0) {
      ...
      mri_tmp =
          GCAlabelWMandWMSAs(gca, mri_inputs, mri_aseg, mri_tmp, transform);
      MRIfree(&mri_aseg);
      mri_aseg = mri_tmp;
    }
...
static int get_option(int argc, char *argv[]) {
  ...
  // ADDED BY EMILY 02/20/15
    ...
    if (!mri_vals)
      ErrorExit(ERROR_NOFILE, "%s: could not read MRI volume from %s", Progname,
                argv[2]);
    nargs = 1;
...
static void print_usage() {
  fprintf(stderr,
          "usage: %s [options] <aseg name> <surface dir> <norm volume> <output "
          "volume>\n",
          Progname);
}
  ...
  for (changed = x = 0; x < mri->width; x++) {
    for (y = 0; y < mri->height; y++) {
      for (z = 0; z < mri->depth; z++) {
        ...
        if (right == 0 && ((label != Left_Cerebral_Cortex) &&
        ...
        if (v == nullptr) { // no vertices within range - either way inside or
                            // way outside
        ...
        temp_z = (float)zw;
        if (x == Gx && y == Gy && z == Gz) {
          printf("(%f, %f, %f,) is the WM surface coordinate for voxel (%i, "
                 "%i, %i)\n",
        ...
        if (dot > 0 && (MRIgetVoxVal(mri_inside_eroded, x, y, z, 0) >
                        0)) { // should be inside
          dot *= -1;
        } else if (dot < 0 && (MRIgetVoxVal(mri_inside_dilated, x, y, z, 0) ==
                               0)) { // should be outside
          dot *= -1;
        }
        ...
        if (x == Gx && y == Gy && z == Gz)
          printf("(%d, %d, %d), label %s, dist = %2.1f, dot = %2.1f\n", Gx, Gy,
                 Gz, cma_label_to_name(label), dist, dot);

        gcap = getGCAP(gca, mri, transform, x, y, z);

        switch (label) {
        case Left_Cerebral_Cortex:
        case Right_Cerebral_Cortex: // check to see if it's inside ribbon and
                                    // change it to hypointensity
        ...
        case Right_WM_hypointensities: // check to see if it's outside ribbon
          ...
          if (gca) // if we have a gca, check to make sure gm is possible here
          {
            ...
            for (found_ven = found = 0, xk = -whalf; !found_ven && xk <= whalf;
                 xk++) {
              xi = mri->xi[x + xk];
  ...
  for (changed = x = 0; x < mri->width; x++) {
    for (y = 0; y < mri->height; y++) {
      for (z = 0; z < mri->depth; z++) {
        ...
        yi = nint(V3_Y(v2));
        zi = nint(V3_Z(v2));
        if (xi < 0 || yi < 0 || zi < 0 || xi >= mri_pial_dist->width ||
            yi >= mri_pial_dist->height || zi >= mri_pial_dist->depth) {
          continue;
        }
        if (x == Gx && y == Gy && z == Gz) {
          DiagBreak();
  ...
  for (changed = x = 0; x < mri->width; x++) {
    for (y = 0; y < mri->height; y++) {
      for (z = 0; z < mri->depth; z++) {
        ...
        yi = nint(V3_Y(v2));
        zi = nint(V3_Z(v2));
        if (xi < 0 || yi < 0 || zi < 0 || xi >= mri_white_dist->width ||
            yi >= mri_white_dist->height || zi >= mri_white_dist->depth) {
          continue;
        }
        if (x == Gx && y == Gy && z == Gz) {
          DiagBreak();
...
/*static WMSA populateWMSA(FILE *config, MRI *seg ){
  ...
  while(fgets(line,sizeof(line),config)){
    strtok(line," ");
    switch line
      //NEED TO MAKE SURE WE PUT THE T1 in as the first modality! (not in config
file)
...
#define IS_INF_TO_HIPPO(index)                                                 \
  (index == PARAHIPPOCAMPAL_GYRUS || index == LINGUAL_SULCUS ||                \
   index == LINGUAL_SULCUS2 || index == COLLATERAL_SULCUS_ANT ||               \
   index == COLLATERAL_SULCUS_POS)
