...
def readAsegStats(path_aseg_stats):
    ...
    for line in aseg_stats:
        if "# Measure BrainSeg," in line:
            ...
            aseg.update(
                {"Left-Cerebellum-White-Matter": float(line.split()[3])}
            )
            ...
            aseg.update(
                {"Right-Cerebellum-White-Matter": float(line.split()[3])}
            )
            ...
            aseg.update(
                {"Right-non-WM-hypointensities": float(line.split()[3])}
            )
    ...
    outlierDict = dict(
        [
            ...
            (
                "Left-Amygdala",
                dict(
                    [("lower", 1179.73655974083), ("upper", 1935.09415214717)]
                ),
            ),
            (
                "Right-Amygdala",
                dict(
                    [("lower", 1161.54746836742), ("upper", 2002.14187676668)]
                ),
            ),
            (
                "Brain-Stem",
                dict(
                    [("lower", 18048.54263155760), ("upper", 25300.51090318110)]
                ),
            ),
            (
                "Left-Caudate",
                dict(
                    [("lower", 2702.73311142764), ("upper", 4380.54479618196)]
                ),
            ),
            (
                "Right-Caudate",
                dict(
                    [("lower", 2569.61140834210), ("upper", 4412.61035536070)]
                ),
            ),
            (
                "Left-Hippocampus",
                dict(
                    [("lower", 3432.26483953083), ("upper", 4934.43236139507)]
                ),
            ),
            (
                "Right-Hippocampus",
                dict(
                    [("lower", 3580.74371035841), ("upper", 5067.49668145829)]
                ),
            ),
            (
                "Left-Pallidum",
            ...
            (
                "Right-Pallidum",
                dict(
                    [("lower", 1078.14975428593), ("upper", 1864.08951102817)]
                ),
            ),
            (
                "Left-Putamen",
                dict(
                    [("lower", 3956.23134409153), ("upper", 6561.97642872937)]
                ),
            ),
            (
                "Right-Putamen",
                dict(
                    [("lower", 3768.88684356957), ("upper", 6142.52870810603)]
                ),
            ),
            (
                "Left-Thalamus-Proper",
                dict(
                    [("lower", 6483.36121320953), ("upper", 9489.46749012527)]
                ),
            ),
            (
                "Right-Thalamus-Proper",
                dict(
                    [("lower", 6065.70220487045), ("upper", 8346.88382091555)]
                ),
            ),
            (
                "Left-VentralDC",
                dict(
                    [("lower", 3182.42264293449), ("upper", 4495.77412707751)]
                ),
            ),
            (
                "Right-VentralDC",
                dict(
                    [("lower", 3143.88280953869), ("upper", 4407.63641978371)]
                ),
            ),
        ]
    )
...
def outlierDetection(
    subjects, subjects_dir, output_dir, outlierDict, min_no_subjects=10
):
    """
    ...
    if len(subjects) >= min_no_subjects:
        ...
        for subject in aseg:
            ...
            for key in aseg[subject]:
                ...
                if (aseg[subject][key] < sample_nonpar_lower[key]) or (
                    aseg[subject][key] > sample_nonpar_upper[key]
                ):
                    nonparDict.update({key: True})
                else:
                    nonparDict.update({key: False})

                if (aseg[subject][key] < sample_param_lower[key]) or (
                    aseg[subject][key] > sample_param_upper[key]
                ):
            ...
            outlierSampleNonparNum.update(
                {subject: np.sum(list(nonparDict.values()))}
            )
            outlierSampleParamNum.update(
                {subject: np.sum(list(paramDict.values()))}
            )
    ...
    for subject in aseg:
        ...
        for key in aseg[subject]:
            ...
            if key in outlierDict:
                ...
                if (aseg[subject][key] < outlierDict[key]["lower"]) or (
                    aseg[subject][key] > outlierDict[key]["upper"]
                ):
    ...
    with open(
        os.path.join(output_dir, "all.outliers.norms.stats"), "w"
    ) as datafile:
        csvwriter = csv.DictWriter(
            datafile,
            fieldnames=asegFieldnames,
