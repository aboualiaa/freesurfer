...
def _parse_arguments():
    ...
    required.add_argument(
        "--subjects_dir",
        dest="subjects_dir",
        help="subjects directory with a set of Freesurfer 6.0 \nprocessed individual datasets.",
        metavar="<directory>",
        required=True,
    )
    required.add_argument(
    ...
    optional.add_argument(
        "--subjects",
        dest="subjects",
        help="list of subject IDs. If omitted, all suitable sub-\ndirectories witin the subjects directory will be \nused.",
        default=[],
        nargs="+",
        metavar="SubjectID",
        required=False,
    ...
    optional.add_argument(
        ...
        action="store_true",
        required=False,
    )  # shape is currently a hidden option
    optional.add_argument(
        "--screenshots",
        dest="screenshots",
...
def _check_arguments(
        ...
        if os.path.isdir(os.path.join(output_dir, "screenshots")):
            ...
            try:
                testfile = tempfile.TemporaryFile(
                    dir=os.path.join(output_dir, "screenshots")
                )
                testfile.close()
            except OSError as e:
                if e.errno != errno.EACCES:  # 13
        ...
        if os.path.isdir(os.path.join(output_dir, "fornix")):
            ...
            try:
                ...
                print(
                    ...
                    + os.path.join(output_dir, "fornix")
                    + "\n"
                )
                sys.exit(1)
            ...
            try:
                testfile = tempfile.TemporaryFile(
                    dir=os.path.join(output_dir, "fornix")
                )
                testfile.close()
            except OSError as e:
                if e.errno != errno.EACCES:  # 13
    ...
    if shape is True:
        if os.path.isdir(os.path.join(output_dir, "brainprint")):
            print(
                ...
                os.path.join(output_dir, "brainprint"),
            )
        else:
            try:
            ...
            try:
                testfile = tempfile.TemporaryFile(
                    dir=os.path.join(output_dir, "brainprint")
                )
                testfile.close()
            except OSError as e:
                if e.errno != errno.EACCES:  # 13
    ...
    if outlier is True:
        if os.path.isdir(os.path.join(output_dir, "outliers")):
            print(
                "Found outliers directory", os.path.join(output_dir, "outliers")
            )
        else:
            try:
                ...
                print(
                    ...
                    + os.path.join(output_dir, "outliers")
                    + "\n"
                )
                sys.exit(1)
            ...
            try:
                testfile = tempfile.TemporaryFile(
                    dir=os.path.join(output_dir, "outliers")
                )
                testfile.close()
            except OSError as e:
                if e.errno != errno.EACCES:  # 13
    ...
    for subject in subjects:
        ...
        path_check = os.path.join(
            subjects_dir, subject, "surf", "lh.w-g.pct.mgh"
        )
        if not os.path.isfile(path_check):
            print("Could not find", path_check, "for subject", subject)
            subjects_to_remove.extend([subject])
        ...
        path_check = os.path.join(
            subjects_dir, subject, "surf", "rh.w-g.pct.mgh"
        )
        if not os.path.isfile(path_check):
            print("Could not find", path_check, "for subject", subject)
            subjects_to_remove.extend([subject])
        ...
        if fornix is True:
            ...
            path_check = os.path.join(
                subjects_dir, subject, "mri", "transforms", "cc_up.lta"
            )
            if not os.path.isfile(path_check):
                print("Could not find", path_check, "for subject", subject)
                subjects_to_remove.extend([subject])
...
def _do_qatools(
    ...
    for subject in subjects:
        ...
        wm_snr_orig, gm_snr_orig = checkSNR(
            subjects_dir, subject, SNR_AMOUT_EROSION, ref_image="orig.mgz"
        )

        # get WM and GM SNR for norm.mgz
        wm_snr_norm, gm_snr_norm = checkSNR(
            subjects_dir, subject, SNR_AMOUT_EROSION, ref_image="norm.mgz"
        )
        ...
        (
            ...
            topo_lh,
            topo_rh,
        ) = checkTopology(subjects_dir, subject)
        ...
        if shape is True:
            ...
            class options:
                ...
                num = 15
                bcond = 1
                brainprint = os.path.join(
                    brainprint_outdir, subject + ".brainprint.csv"
                )
            ...
            class optionsPostProc:
                ...
                csvfiles = [
                    os.path.join(brainprint_outdir, subject + ".brainprint.csv")
                ]
        ...
        if screenshots is True:
            ...
            screenshots_outdir = os.path.join(
                output_dir, "screenshots", subject
            )
