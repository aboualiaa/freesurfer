...
def checkSNR(subjects_dir, subject, nb_erode=3, ref_image="norm.mgz"):
    ...
    try:
        path_reference_image = os.path.join(
            subjects_dir, subject, "mri", ref_image
        )
    ...
    nb_erode = nb_erode
    b_wm_data = binary_erosion(
        b_wm_data, np.ones((nb_erode, nb_erode, nb_erode))
    )
