...
def evaluateFornixSegmentation(
    ...
    cmd = (
        ...
        + os.path.join(SUBJECTS_DIR, SUBJECT, "mri", "transforms", "cc_up.lta")
        + " --outmni "
        + os.path.join(OUTPUT_DIR, "cc_up.xfm")
    )
    ...
    cmd = (
        "mri_binarize --i "
        + os.path.join(OUTPUT_DIR, "asegCCup.mgz")
        + " --match 251 252 253 254 255 --dilate 2 --erode 2 --surf "
        + os.path.join(OUTPUT_DIR, "cc.surf")
        + " --surf-smooth 1 --o "
    ...
    if CREATE_SCREENSHOT is True:
        createScreenshots(
            ...
            SURF=[os.path.join(OUTPUT_DIR, "cc.surf")],
            OUTFILE=os.path.join(OUTPUT_DIR, "cc.png"),
        )
