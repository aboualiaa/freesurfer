 ...
 * It also implements the Geometric Transfer Matrix (GTM) as it is needed by
 * RBV.
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  if (nReplace > 0) {
    printf("Replacing %d\n", nReplace);
    for (f = 0; f < nReplace; f++)
      printf("%2d:  %4d %4d\n", f + 1, SrcReplace[f], TrgReplace[f]);
    for (f = 0; f < nReplace; f++)
      fprintf(logfp, "%2d:  %4d %4d\n", f + 1, SrcReplace[f], TrgReplace[f]);
    mritmp = MRIreplaceList(gtm->anatseg, SrcReplace, TrgReplace, nReplace,
                            NULL, NULL);
    MRIfree(&gtm->anatseg);
    gtm->anatseg = mritmp;
  }
  ...
  if (ApplyFWHM > 0) {
    ...
    mritmp =
        MRIgaussianSmoothNI(gtm->yvol, cStdApply, rStdApply, sStdApply, NULL);
    MRIfree(&gtm->yvol);
    gtm->yvol = mritmp;
  }
  ...
  if (MaskVolFile) {
    ...
    gtm->mask = NULL; // should already be NULL
  ...
  if (gtm->rescale) {
    ...
    for (n = 0; n < gtm->n_scale_refids; n++)
      printf("%4d ", gtm->scale_refids[n]);
    printf("\n");
    fprintf(logfp, "Rescaling using segids ");
    for (n = 0; n < gtm->n_scale_refids; n++)
      fprintf(logfp, "%4d ", gtm->scale_refids[n]);
    fprintf(logfp, "\n");
  ...
  if (DoMGPVC) {
    ...
    for (c = 0; c < gtm->n_mg_refids; c++)
      fprintf(logfp, "%4d ", gtm->mg_refids[c]);
    fprintf(logfp, "\n");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      SrcReplace[nReplace] = 78;
      TrgReplace[nReplace] = 77;
      nReplace++;
      SrcReplace[nReplace] = 79;
      TrgReplace[nReplace] = 77;
      nReplace++;
...
static void print_usage(void) {
  ...
  printf("   --no-rescale   : do not global rescale such that mean of "
         "cerebellum WM is 100\n");
  printf("   --mask volfile : ignore areas outside of the mask (in input vol "
         "space)\n");
  printf("   --ttype+head : use tissue type def that includes head "
         "segmentation\n");
  printf(
  ...
  printf("   --mgpvc gmthresh : perform Mueller-Gaertner PVC, gmthresh is min "
         "gm pvf bet 0 and 1\n");
  printf("   --X Xfile : save X matrix (it will be big)\n");
  printf("\n");
  printf("   --synth gtmbeta synthvolume : synthesize unsmoothed volume with "
         "gtmbeta as input\n");
  printf("\n");
#ifdef _OPENMP
  printf("   --threads N : use N threads (with Open MP)\n");
  printf("   --threads-max : use the maximum allowable number of threads for "
         "this computer\n");
  printf("   --threads-max-1 : use one less than the maximum allowable number "
...
int GTMOPTsetup(GTMOPT *gtmopt) {
  ...
  gtmopt->pvfseg =
      MRIhiresSeg(gtmopt->pvfseganat, gtmopt->lhw, gtmopt->lhp, gtmopt->rhw,
                  gtmopt->rhp, gtmopt->USF, &gtmopt->anat2pvfseg);
  if (gtmopt->pvfseg == NULL)
    return (1);
  ...
  if (gtmopt->gtmseganat == NULL)
    exit(1);
  gtmopt->gtmsegmu = MRImaskAndUpsample(gtmopt->gtmseganat, NULL, gtmopt->USF,
                                        2, 0, &gtmopt->anat2gtmsegmu);
...
double GTMOPTcost(GTMOPT *gtmopt) {
  ...
  gtmopt->gtm->gtmseg = MRIaseg2volMU(gtmopt->gtmsegmu, gtmopt->gtmsegmu2pet,
                                      0.0, &hitvol, -1, gtmopt->gtm->ctGTMSeg);
  if (gtmopt->gtm->gtmseg == NULL)
    return (-1);
...
int GTMrbv(GTM *gtm) {
  ...
  fflush(stdout);
  gtm->yhatseg =
      MRIgaussianSmoothNI(gtm->yhat0seg, gtm->cStd, gtm->rStd, gtm->sStd, NULL);
  if (gtm->yhatseg == NULL) {
...
int MinPowell() {
  ...
  LTAapplyAffineParametersTKR(gtmopt->anat2pet, gtmopt->params, gtmopt->nparams,
                              gtmopt->anat2pet);
  printf("#@# %4d  ", gtmopt->nCostEvaluations);
  for (n = 0; n < gtmopt->nparams; n++)
    printf("%10.6f ", gtmopt->params[n]);
  printf("\n");
...
float compute_powell_cost(float *pPowel) {
  ...
  gtmopt->anat2pet = LTAcopy(gtmopt->anat2pet0, gtmopt->anat2pet);
  LTAapplyAffineParametersTKR(gtmopt->anat2pet, pp, gtmopt->nparams,
                              gtmopt->anat2pet);
  GTMOPTcost(gtmopt);
...
int dngtest(LTA *aseg2vol) {
  ...
  if (aseg == NULL)
    exit(1);
  if (aseg->type != MRI_INT)
    aseg = MRIchangeType(aseg, MRI_INT, 0, 0, 1);
  ...
  for (f = 0; f < 2; f++) {
    printf("f = %d -------------------------------------------\n", f);
    segf =
        MRIseg2SegPVF(seg, seg2vol, 0.5, segidlist, nsegs, mask, 0, NULL, segf);
...
int GTMbuildX(GTM *gtm) {
  ...
  if (gtm->X0 == NULL || gtm->X0->rows != gtm->nmask ||
...
MRI *GTMsegSynth(GTM *gtm) {
  ...
  synth = MRIallocSequence(gtm->anatseg->width, gtm->anatseg->height,
                           gtm->anatseg->depth, MRI_FLOAT, gtm->beta->cols);
