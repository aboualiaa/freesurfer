...
static int mrisLogIntegrationParms2(FILE *fp, MRI_SURFACE *mris,
...
static void print_usage() {
  fprintf(stderr,
          "usage: %s [options] <input surface> <average surface> <output "
          "surface>\n",
          Progname);
}
  ...
  for (sno = 0; sno < SURFACES; sno++) {
    ...
    for (i = 0; i < NSIGMAS; i++) /* for each spatial scale (blurring) */
    {
      ...
      if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
        sprintf(fname, "%s/%s.%4.4dtarget%2.2f", path,
                mris->hemisphere == RIGHT_HEMISPHERE ? "rh" : "lh",
                parms->start_t, sigma);
        if (Gdiag & DIAG_SHOW)
          fprintf(stdout, "writing curvature file %s...", fname);
      ...
      if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
        sprintf(fname, "%s/%s.%4.4dblur%2.2f", path,
                mris->hemisphere == RIGHT_HEMISPHERE ? "rh" : "lh",
                parms->start_t, sigma);
        if (Gdiag & DIAG_SHOW)
          fprintf(stdout, "writing curvature file %s...", fname);
...
static int mrisLogIntegrationParms2(FILE *fp, MRI_SURFACE *mris,
  ...
  fprintf(fp, "tol=%2.1e, host=%5.5s, nav=%d, nbrs=%d", (float)parms->tol,
          host_name, parms->n_averages, mris->nsize);
  if (!FZERO(parms->l_area))
