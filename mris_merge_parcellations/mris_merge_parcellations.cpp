...
static void print_help() {
  ...
  fprintf(stderr,
          ...
          "the surface\n");
  fprintf(stderr, "\nvalid options are:\n\n");
  fprintf(stderr, "(see the source code!)\n");
  exit(1);
}
...
static int merge_annotations(COLOR_TABLE *ct, MRI_SURFACE *mris1,
  ...
  int s_caudal_acc, s_posterior_cingulate, s_rostral_acc, s_caudal_peri,
      s_rostral_peri, s_posterior_peri;
  ...
  if (mris2->hemisphere == RIGHT_HEMISPHERE) {
    caudal_acc =
        CTABentryNameToAnnotation("ctx-rh-caudalanteriorcingulate", ct);
    if (caudal_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-caudalanteriorcingulate");
    posterior_cingulate =
        CTABentryNameToAnnotation("ctx-rh-posteriorcingulate", ct);
    if (posterior_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-posteriorcingulate");
    rostral_acc =
        CTABentryNameToAnnotation("ctx-rh-rostralanteriorcingulate", ct);
    if (rostral_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-rostralanteriorcingulate");

    s_caudal_acc =
        CTABentryNameToAnnotation("ctx-rh-S_cingulate-caudal_ACC", ct);
    if (s_caudal_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-S_cingulate-caudal_ACC");
    s_rostral_acc =
        CTABentryNameToAnnotation("ctx-rh-S_cingulate-rostral_ACC", ct);
    if (s_rostral_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-S_cingulate-rostral_ACC");
    s_posterior_cingulate =
        CTABentryNameToAnnotation("ctx-rh-S_cingulate-posterior", ct);
    if (s_posterior_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-S_cingulate-posterior");

    g_caudal_acc =
        CTABentryNameToAnnotation("ctx-rh-G_cingulate-caudal_ACC", ct);
    if (g_caudal_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-G_cingulate-caudal_ACC");
    g_rostral_acc =
        CTABentryNameToAnnotation("ctx-rh-G_cingulate-rostral_ACC", ct);
    if (g_rostral_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-G_cingulate-rostral_ACC");
    g_posterior_cingulate =
        CTABentryNameToAnnotation("ctx-rh-G_cingulate-posterior", ct);
    if (g_posterior_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-G_cingulate-posterior");

    s_caudal_peri =
        CTABentryNameToAnnotation("ctx-rh-S_pericallosal-caudal", ct);
    if (s_caudal_peri < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-S_pericallosal-caudal");
    s_rostral_peri =
        CTABentryNameToAnnotation("ctx-rh-S_pericallosal-rostral", ct);
    if (s_rostral_peri < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-S_pericallosal-rostral");
    s_posterior_peri =
        CTABentryNameToAnnotation("ctx-rh-S_pericallosal-posterior", ct);
    if (s_posterior_peri < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-rh-S_pericallosal-posterior");

    s_cingulate = CTABentryNameToAnnotation(
        "ctx-rh-S_cingulate-Main_part_and_Intracingulate", ct);
    if (s_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
  ...
  {
    caudal_acc =
        CTABentryNameToAnnotation("ctx-lh-caudalanteriorcingulate", ct);
    if (caudal_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-caudalanteriorcingulate");
    posterior_cingulate =
        CTABentryNameToAnnotation("ctx-lh-posteriorcingulate", ct);
    if (posterior_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-posteriorcingulate");
    rostral_acc =
        CTABentryNameToAnnotation("ctx-lh-rostralanteriorcingulate", ct);
    if (rostral_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-rostralanteriorcingulate");

    s_caudal_acc =
        CTABentryNameToAnnotation("ctx-lh-S_cingulate-caudal_ACC", ct);
    if (s_caudal_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-S_cingulate-caudal_ACC");
    s_rostral_acc =
        CTABentryNameToAnnotation("ctx-lh-S_cingulate-rostral_ACC", ct);
    if (s_rostral_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-S_cingulate-rostral_ACC");
    s_posterior_cingulate =
        CTABentryNameToAnnotation("ctx-lh-S_cingulate-posterior", ct);
    if (s_posterior_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-S_cingulate-posterior");

    g_caudal_acc =
        CTABentryNameToAnnotation("ctx-lh-G_cingulate-caudal_ACC", ct);
    if (g_caudal_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-G_cingulate-caudal_ACC");
    g_rostral_acc =
        CTABentryNameToAnnotation("ctx-lh-G_cingulate-rostral_ACC", ct);
    if (g_rostral_acc < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-G_cingulate-rostral_ACC");
    g_posterior_cingulate =
        CTABentryNameToAnnotation("ctx-lh-G_cingulate-posterior", ct);
    if (g_posterior_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-G_cingulate-posterior");

    s_caudal_peri =
        CTABentryNameToAnnotation("ctx-lh-S_pericallosal-caudal", ct);
    if (s_caudal_peri < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-S_pericallosal-caudal");
    s_rostral_peri =
        CTABentryNameToAnnotation("ctx-lh-S_pericallosal-rostral", ct);
    if (s_rostral_peri < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-S_pericallosal-rostral");
    s_posterior_peri =
        CTABentryNameToAnnotation("ctx-lh-S_pericallosal-posterior", ct);
    if (s_posterior_peri < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
                "ctx-lh-S_pericallosal-posterior");

    s_cingulate = CTABentryNameToAnnotation(
        "ctx-lh-S_cingulate-Main_part_and_Intracingulate", ct);
    if (s_cingulate < 0)
      ErrorExit(ERROR_UNSUPPORTED, "Cannot find annotation %s in LUT",
  ...
  if (Gdiag_no >= 0) {
    ...
    printf("mris1->annot[%d] = %s (%d)\n", Gdiag_no,
           mris1->ct->entries[index]->name,
           mris1->vertices[Gdiag_no].annotation);
    CTABfindAnnotation(mris2->ct, mris2->vertices[Gdiag_no].annotation, &index);
    printf("mris2->annot[%d] = %s (%d)\n", Gdiag_no,
           mris2->ct->entries[index]->name,
           mris2->vertices[Gdiag_no].annotation);
  }
