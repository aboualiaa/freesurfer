...
static int udim = 256, vdim = 128, maxlabels = 25, maxvertices = 500000,
           TotalCount;
...
int main(int argc, char *argv[]) {
  ...
  sprintf(fname, "%s/%s/label/%s%s.annot.freq", sdir, argv[i - 1], hemi,
          out_fname);
  WriteAnnotFreqFile(mris, fname);
  sprintf(fname, "%s/%s/label/%s%s.annot.hist", sdir, argv[i - 1], hemi,
          out_fname);
...
static void print_usage() {
  fprintf(
      ...
      Progname);
  fprintf(stderr,
          "the output curvature file will be painted onto the last "
    ...
    for (u = 0; u < udim; u++)
      for (v = 0; v < vdim; v++)
        if (AnnotMapLabel[u][v] == -1) /* undefined AnnotLabel */
        {
          ...
          for (du = -1; du <= 1; du++)
            for (dv = -1; dv <= 1; dv++)
              if ((u + du >= 0) && (u + du < udim) && (v + dv >= 0) &&
                  (v + dv < vdim) && (AnnotMapLabel[u + du][v + dv] != -1))
...
static int WriteAnnotFile(MRI_SURFACE *mris, char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "WriteAnnotFile(%s) - could not open file (%d)",
                 fname, errno));
  for (k = 0, num = 0; k < vertex_index; k++)
    if (AnnotLabel[k] != 0)
      num++;
  printf("num = %d\n", num);
  fwriteFloat(num, fp);
...
static int WriteAnnotFreqFile(MRI_SURFACE *mris, char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE, "WriteAnnotFreqFile(%s): can't create file (%d)",
                 fname, errno));
  for (k = 0, num = 0; k < vertex_index; k++)
    if (AnnotLabel[k] != 0)
      num++;
...
static int WriteAnnotHistFile(MRI_SURFACE *mris, char *fname) {
  ...
  if (fp == nullptr)
    ErrorReturn(ERROR_NOFILE,
                (ERROR_NOFILE,
                 "WriteAnnotHistFile(%s) - could not open file (%d)", fname,
                 errno));
  for (k = 0, num = 0; k < vertex_index; k++)
