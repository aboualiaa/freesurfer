...
vtkKWQdecView::vtkKWQdecView()
    : mfnSurface(""), mVertexPlot(NULL), mnCursorVertexIndex(-1),
...
void vtkKWQdecView::CreateWidget() {
...
vtkKWQdecView::ViewInteractor *vtkKWQdecView::ViewInteractor::New() {
...
void vtkKWQdecView::ViewInteractor::SetView(vtkKWQdecView *iView) {
  mView = iView;
}

void vtkKWQdecView::ViewInteractor::OnLeftButtonDown() {
  ...
  const char *sGdfButton = QdecUtilities::GetQdecrcResourceString("GDF_BUTTON");
...
void vtkKWQdecView::ViewInteractor::OnLeftButtonUp() {
...
int vtkKWQdecView::ViewInteractor::GetVertexAtPicker() {
...
void vtkKWQdecView::SetSurface(vtkFSSurfaceSource *iSurface) {
  ...
  if (mCurrentScalars.GetPointer() && mCurrentSurfaceSource.GetPointer())
    mCurrentSurfaceSource->GetOutput()->GetPointData()->SetScalars(
        mCurrentScalars);
  ...
  if (mCurrentSurfaceSource.GetPointer()) {
    ...
    mSurfaceOverlayInflator->SetInputConnection(
        mCurrentSurfaceSource->GetOutputPort());
  }
  ...
  if (mCurrentOverlayScalars.GetPointer() &&
    ...
    mSurfaceOverlayInflator->GetOutput()->GetPointData()->SetScalars(
        mCurrentOverlayScalars);
...
void vtkKWQdecView::RestoreView(const char *isHemi) {
...
void vtkKWQdecView::ZoomBy(float iFactor) {
  this->GetRenderer()->GetActiveCamera()->Zoom(iFactor);
  this->GetRenderer()->GetRenderWindow()->Render();
}

bool vtkKWQdecView::GetShowCursor() {
  if (mCursor)
    return mCursor->GetAxes();

  return false;
}
void vtkKWQdecView::SetShowCursor(bool ibShow) {
...
void vtkKWQdecView::SetSurfaceScalars(vtkFloatArray *iScalars) {
...
void vtkKWQdecView::SetSurfaceScalarsColors(
    vtkScalarsToColors *iScalarsColors) {
...
void vtkKWQdecView::SetSurfaceLookupScalars(vtkFloatArray *iScalars) {

  // Save the pointer.
  mCurrentLookupScalars = iScalars;
}

void vtkKWQdecView::SetSurfaceOverlayScalarsAndColors(
    vtkFloatArray *iScalars, vtkScalarsToColors *iColors) {
...
void vtkKWQdecView::SetSurfaceOverlayOpacity(double iOpacity) {
...
double vtkKWQdecView::GetSurfaceOverlayOpacity() { return mOverlayOpacity; }

void vtkKWQdecView::SetSurfaceLegendColors(vtkScalarsToColors *iLegendColors) {
...
void vtkKWQdecView::SetROI(vtkPolyData *iROIPolyData) {
...
void vtkKWQdecView::SetAnnotationMessage(const char *isMessage) {
...
void vtkKWQdecView::SetShowLegend(int ibShow) {
...
void vtkKWQdecView::SetShowAnnotation(int ibShow) {

  this->SetAnnotationsVisibility(ibShow);
}

void vtkKWQdecView::SetVertexAnnotationLookup(
    QdecVertexAnnotationLookup *iLookup) {

  mAnnotationLookup = iLookup;
}

vtkPoints *vtkKWQdecView::GetSurfaceSelectionPoints() {
...
void vtkKWQdecView::SelectSurfaceVertex(int inVertex) {
  ...
  if (-1 == inVertex) {
    ...
    ssLabel << "(" << fixed << setprecision(2) << surfaceRAS[0] << ", "
  ...
  this->InvokeEvent(QdecEvents::UserSelectedVertex, static_cast<void *>(&info));

  // Render the view.
  this->Render();
}

void vtkKWQdecView::InitializeScalarBar() {
...
void vtkKWQdecView::DeleteScalarBar() {
...
void vtkKWQdecView::InitializeROI() {
...
void vtkKWQdecView::AnimateCameraElevatePositive() {

  double rate = this->GetRenderWindowInteractor()->GetDesiredUpdateRate();
  this->GetRenderWindowInteractor()->SetDesiredUpdateRate(
      kcAnimateSteps / kAnimateTimeInSeconds);
  for (double nStep = 1.0; nStep <= kcAnimateSteps; nStep += 1.0) {
    this->GetRenderer()->GetActiveCamera()->Elevation(kAnimationDegrees /
                                                      kcAnimateSteps);
...
void vtkKWQdecView::AnimateCameraElevateNegative() {
...
void vtkKWQdecView::AnimateCameraAzimuthNegative() {

  double rate = this->GetRenderWindowInteractor()->GetDesiredUpdateRate();
  this->GetRenderWindowInteractor()->SetDesiredUpdateRate(
      kcAnimateSteps / kAnimateTimeInSeconds);
  for (double nStep = 1.0; nStep <= kcAnimateSteps; nStep += 1.0) {
    this->GetRenderer()->GetActiveCamera()->Azimuth(-kAnimationDegrees /
                                                    kcAnimateSteps);
    this->GetRenderer()->GetActiveCamera()->OrthogonalizeViewUp();
    this->Render();
  }
...
void vtkKWQdecView::AnimateCameraAzimuthPositive() {
...
void vtkKWQdecView::AnimateCameraRollNegative() {

  double rate = this->GetRenderWindowInteractor()->GetDesiredUpdateRate();
  this->GetRenderWindowInteractor()->SetDesiredUpdateRate(
      kcAnimateSteps / kAnimateTimeInSeconds);
  for (double nStep = 1.0; nStep <= kcAnimateSteps; nStep += 1.0) {
    this->GetRenderer()->GetActiveCamera()->Roll(-kAnimationDegrees /
                                                 kcAnimateSteps);
    this->Render();
  }
  this->GetRenderWindowInteractor()->SetDesiredUpdateRate(rate);
...
void vtkKWQdecView::AnimateCameraRollPositive() {
...
void vtkKWQdecView::AddVertexToPath(int inVertex) {
...
void vtkKWQdecView::RebuildPathLine() {
