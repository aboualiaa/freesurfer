...
vtkKWQdecWindow::vtkKWQdecWindow()
    : vtkKWWindow(), mCurrentNotebookPanelName(NULL),
      mbUseHistogramEditor(true), mMenuLoadDataTable(NULL),
      mMenuLoadProjectFile(NULL), mMenuLoadLabel(NULL),
      mMenuLoadAnnotation(NULL), mMenuSaveDataTable(NULL),
      mMenuSaveProjectFile(NULL), mMenuSaveScatterPlotPostscript(NULL),
      mMenuSaveTIFF(NULL), mMenuQuickSnapsTIFF(NULL),
      mMenuSaveGDFPostscript(NULL), mMenuSaveLabel(NULL), mMenuMapLabel(NULL),
      mMenuClearCurvature(NULL), mMenuClearSurfaceScalars(NULL),
      mMenuRestoreView(NULL), mMenuZoomOut(NULL), mMenuZoomIn(NULL),
      mMenuShowCursor(NULL), mMenuShowCurvature(NULL),
      mMenuAddSelectionToROI(NULL), mMenuRemoveSelectionFromROI(NULL),
      mMenuClearROI(NULL), mMenuSmoothCurvatureScalars(NULL),
      mMenuSmoothSurfaceScalars(NULL), mMenuGraphAverageROI(NULL),
      mMenuButtonSimulationThresh(NULL), mMenuButtonSimulationSign(NULL),
      mbFrameSurfaceInited(false), mbFrameOverlayInited(false),
      mbFrameSurfaceScalarsInited(false), mScatterPlotSelection(-1),
      mScatterPlotLegend(""), mVertexPlot(NULL), mQdecProject(NULL),
      mcVertices(-1), msCurrentSurfaceSource(""), mnCurrentSurfaceScalars(-1),
      maAnnotationIndicies(NULL), mAnnotationTable(NULL), mbShowCurvature(true),
      mSurfaceScalarsColorMin(2.0),
#if USE_MID
      mSurfaceScalarsColorMid(2.0001),
#endif
      mSurfaceScalarsColorMax(5.0), mSurfaceScalarsColorOffset(0.0),
      mbSurfaceScalarsColorReverse(false),
      mbSurfaceScalarsColorShowPositive(true),
      mbSurfaceScalarsColorShowNegative(true), mnNegativeMaxRedValue(0.0),
      mnNegativeMaxGreenValue(1.0), mnNegativeMaxBlueValue(1.0),
#if USE_MID
      mnNegativeMidRedValue(0.0), mnNegativeMidGreenValue(0.0),
      mnNegativeMidBlueValue(1.0),
#endif
      mnNegativeMinRedValue(0.0), mnNegativeMinGreenValue(0.0),
      mnNegativeMinBlueValue(1.0), mnPositiveMinRedValue(1.0),
      mnPositiveMinGreenValue(0.0), mnPositiveMinBlueValue(0.0),
#if USE_MID
      mnPositiveMidRedValue(1.0), mnPositiveMidGreenValue(0.0),
      mnPositiveMidBlueValue(0.0),
#endif
      mnPositiveMaxRedValue(1.0), mnPositiveMaxGreenValue(1.0),
      mnPositiveMaxBlueValue(0.0), mClusterStats(NULL), mnClusters(0),
      mCurrentCluster(0), mbDrawCurvatureGreenRedIfNoScalars(false),
      mSurfaceScalarsColorsFDRRate(0.05), msOverlayDescription("") {
...
void vtkKWQdecWindow::SetUseHistogramEditor(bool ibUse) {

  if (this->IsCreated())
    vtkErrorMacro(
        << "SetUseHistogramEditor() should be called before Create()");

  mbUseHistogramEditor = ibUse;
}

void vtkKWQdecWindow::CreateWidget() {
  ...
  // ---------------------------------------------------------------------
  // Make a toolbar.
  vtkSmartPointer<vtkKWToolbar> toolbar = vtkSmartPointer<vtkKWToolbar>::New();
  ...
  mBtnLoadDataTable.TakeReference(
      this->MakeToolbarButton(toolbar, "Load a .dat file into the Design tab",
                              this, "LoadDataTableFromDlog", "LoadDataTable"));

  // Load project file
  mBtnLoadProjectFile.TakeReference(
      this->MakeToolbarButton(toolbar, "Load a project file", this,
                              "LoadProjectFileFromDlog", "LoadAnalyzedData"));

  // Load label
  mBtnLoadLabel.TakeReference(
      this->MakeToolbarButton(toolbar, "Load a label as an ROI", this,
                              "LoadLabelFromDlog", "LoadLabel"));

  this->AddSpacerToToolbar(toolbar);

  // Save TIFF
  mBtnSaveTIFF.TakeReference(
      this->MakeToolbarButton(toolbar, "Save a picture of the view as a TIFF",
                              this, "SaveTIFFImageFromDlog", "SaveTIFF"));

  // Save Label
  mBtnSaveLabel.TakeReference(
      this->MakeToolbarButton(toolbar, "Save the current label", this,
                              "SaveLabelFromDlog", "SaveLabel"));

  this->AddSpacerToToolbar(toolbar);

  // Restore View
  mBtnRestoreView.TakeReference(
      this->MakeToolbarButton(toolbar, "Restore the camera to a lateral view",
                              this, "RestoreView", "RestoreView"));

  this->AddSpacerToToolbar(toolbar);

  // Zoom out and in
  mBtnZoomOut.TakeReference(this->MakeToolbarButton(
      toolbar, "Zoom out a step", this, "ZoomOut", "ZoomOut"));
  mBtnZoomIn.TakeReference(this->MakeToolbarButton(toolbar, "Zoom in a step",
                                                   this, "ZoomIn", "ZoomIn"));

  this->AddSpacerToToolbar(toolbar);

  // Rotate buttons.
  mBtnCameraElevateNegative.TakeReference(this->MakeToolbarButton(
      toolbar, "Rotate the surface (pitch forward)", mView,
      "AnimateCameraElevateNegative", "AnimateCameraElevateNegative"));
  mBtnCameraElevatePositive.TakeReference(this->MakeToolbarButton(
      toolbar, "Rotate the surface (pitch backward)", mView,
      "AnimateCameraElevatePositive", "AnimateCameraElevatePositive"));
  mBtnCameraAzimuthNegative.TakeReference(this->MakeToolbarButton(
      toolbar, "Rotate the surface (yaw ccw)", mView,
      "AnimateCameraAzimuthNegative", "AnimateCameraAzimuthNegative"));
  mBtnCameraAzimuthPositive.TakeReference(this->MakeToolbarButton(
      toolbar, "Rotate the surface (yaw cw)", mView,
      "AnimateCameraAzimuthPositive", "AnimateCameraAzimuthPositive"));
  mBtnCameraRollPositive.TakeReference(this->MakeToolbarButton(
      toolbar, "Rotate the surface (roll ccw)", mView,
      "AnimateCameraRollPositive", "AnimateCameraRollPositive"));
  mBtnCameraRollNegative.TakeReference(this->MakeToolbarButton(
      toolbar, "Rotate the surface (roll cw)", mView,
      "AnimateCameraRollNegative", "AnimateCameraRollNegative"));

  this->AddSpacerToToolbar(toolbar);

  // Show cursor
  mBtnShowCursor.TakeReference(this->MakeToolbarCheckButton(
      toolbar, "Show the cursor", this, "ShowCursor", "ShowCursor"));
  if (mView)
  ...
  // Show curvature
  mBtnShowCurvature.TakeReference(this->MakeToolbarCheckButton(
      toolbar, "Show curvature", this, "ShowCurvature", "ShowCurvature"));
  ...
  mBtnAddSelectionToROI.TakeReference(
      this->MakeToolbarButton(toolbar, "Add the selection to the ROI", this,
                              "AddSelectionToROI", "AddSelectionToROI"));

  mBtnRemoveSelectionFromROI.TakeReference(this->MakeToolbarButton(
      toolbar, "Remove the selection from the ROI", this,
      "RemoveSelectionFromROI", "RemoveSelectionFromROI"));
  ...
  mEntrySelectVertex->RemoveBinding("<Unmap>", mEntrySelectVertex,
                                    "ValueCallback");
  ...
  mMenuLoadDataTable->MakeCommand(
      this->GetFileMenu(), nItem++, "L&oad Data Table...", this,
      "LoadDataTableFromDlog", "Ctrl+O", "LoadDataTable");

  // Load Project File..
  mMenuLoadProjectFile = new MenuItem();
  mMenuLoadProjectFile->MakeCommand(
      this->GetFileMenu(), nItem++, "Load Pro&ject File...", this,
      "LoadProjectFileFromDlog", "Ctrl+J", "LoadAnalyzedData");

  // Load Label
  mMenuLoadLabel = new MenuItem();
  mMenuLoadLabel->MakeCommand(this->GetFileMenu(), nItem++, "Load &Label...",
                              this, "LoadLabelFromDlog", "Ctrl+L", "LoadLabel");

  // Load Annotation
  mMenuLoadAnnotation = new MenuItem();
  mMenuLoadAnnotation->MakeCommand(
      this->GetFileMenu(), nItem++, "Load &Annotation...", this,
      "LoadAnnotationFromDlog", "Ctrl+A", "LoadAnnotation");

  // These menu items are for loading pieces of data individually, and
  // can be enabled for debugging.
#if 1
  this->GetFileMenu()->InsertCommand(nItem++, "Load Surface...", this,
                                     "LoadSurfaceFromDlog");
  //  this->GetFileMenu()->InsertCommand( nItem++, "Load GDF...",
  //                                      this, "LoadGDFFromDlog" );
  this->GetFileMenu()->InsertCommand(nItem++, "Load Scalars...", this,
                                     "LoadSurfaceScalarsFromDlog");
  this->GetFileMenu()->InsertCommand(nItem++, "Load Curvature...", this,
                                     "LoadCurvatureFromDlog");
#endif
  this->GetFileMenu()->InsertSeparator(nItem++);

  // Save Data Table.
  mMenuSaveDataTable = new MenuItem();
  mMenuSaveDataTable->MakeCommand(this->GetFileMenu(), nItem++,
                                  "Save Data Table...", this,
                                  "SaveDataTableFromDlog", NULL, NULL);

  // Save Project File.
  mMenuSaveProjectFile = new MenuItem();
  mMenuSaveProjectFile->MakeCommand(this->GetFileMenu(), nItem++,
                                    "Save Project File...", this,
                                    "SaveProjectFileFromDlog", NULL, NULL);

  // Save Postcript of continuous factor plot
  mMenuSaveScatterPlotPostscript = new MenuItem();
  mMenuSaveScatterPlotPostscript->MakeCommand(
      this->GetFileMenu(), nItem++, "Save Scatter Plot as Postcript...", this,
      "SaveScatterPlotPostscriptFromDlog", NULL, NULL);

  // Save TIFF
  mMenuSaveTIFF = new MenuItem();
  mMenuSaveTIFF->MakeCommand(this->GetFileMenu(), nItem++, "Save TIFF...", this,
                             "SaveTIFFImageFromDlog", NULL, "SaveTIFF");

  // QuickSnaps TIFF
  mMenuQuickSnapsTIFF = new MenuItem();
  mMenuQuickSnapsTIFF->MakeCommand(this->GetFileMenu(), nItem++,
                                   "QuickSnaps TIFF...", this, "QuickSnapsTIFF",
                                   NULL, "SaveTIFF");

  // Save Postcript of GDF
  mMenuSaveGDFPostscript = new MenuItem();
  mMenuSaveGDFPostscript->MakeCommand(this->GetFileMenu(), nItem++,
                                      "Save GDF Plot as Postcript...", this,
                                      "SaveGDFPostscriptFromDlog", NULL, NULL);

  // Save Label
  mMenuSaveLabel = new MenuItem();
  mMenuSaveLabel->MakeCommand(this->GetFileMenu(), nItem++, "Save Label...",
                              this, "SaveLabelFromDlog", NULL, "SaveLabel");
  ...
  mMenuMapLabel->MakeCommand(this->GetFileMenu(), nItem++,
                             "Map Label to Subjects...", this,
                             "MapLabelFromDlog", NULL, NULL);
  ...
  mMenuClearCurvature->MakeCommand(this->GetEditMenu(), nItem++,
                                   "Clear Curvature", this, "ClearCurvature",
                                   NULL, NULL);

  // Clear scalars.
  mMenuClearSurfaceScalars = new MenuItem();
  mMenuClearSurfaceScalars->MakeCommand(this->GetEditMenu(), nItem++,
                                        "Clear Surface Scalars", this,
                                        "ClearSurfaceScalars", NULL, NULL);

  this->GetEditMenu()->InsertSeparator(nItem++);

  // Add selection to ROI
  mMenuAddSelectionToROI = new MenuItem();
  mMenuAddSelectionToROI->MakeCommand(this->GetEditMenu(), nItem++,
                                      "Add Selection to ROI", this,
                                      "AddSelectionToROI", NULL, NULL);

  // Remove selection remove from ROI
  mMenuRemoveSelectionFromROI = new MenuItem();
  mMenuRemoveSelectionFromROI->MakeCommand(
      this->GetEditMenu(), nItem++, "Remove Selection from ROI", this,
      "RemoveSelectionFromROI", NULL, NULL);

  // Clear ROI
  mMenuClearROI = new MenuItem();
  mMenuClearROI->MakeCommand(this->GetEditMenu(), nItem++, "Clear ROI", this,
                             "ClearROI", NULL, NULL);
  ...
  mMenuSmoothCurvatureScalars->MakeCommand(
      this->GetEditMenu(), nItem++, "Smooth Curvature Scalars", this,
      "SmoothCurvatureScalarsFromDlog", NULL, NULL);

  // Smooth surface scalars.
  mMenuSmoothSurfaceScalars = new MenuItem();
  mMenuSmoothSurfaceScalars->MakeCommand(
      this->GetEditMenu(), nItem++, "Smooth Surface Scalars", this,
      "SmoothSurfaceScalarsFromDlog", NULL, NULL);

  // View menu.
  // Restore view.
  nItem            = this->GetViewMenuInsertPosition();
  mMenuRestoreView = new MenuItem();
  mMenuRestoreView->MakeCommand(this->GetViewMenu(), nItem++, "Restore &View",
                                this, "RestoreView", "Ctrl+V", "RestoreView");

  this->GetViewMenu()->InsertSeparator(nItem++);

  // Zoom Out.
  mMenuZoomOut = new MenuItem();
  mMenuZoomOut->MakeCommand(this->GetViewMenu(), nItem++, "Zoom Out", this,
                            "ZoomOut", NULL, "ZoomOut");

  // Zoom In.
  mMenuZoomIn = new MenuItem();
  mMenuZoomIn->MakeCommand(this->GetViewMenu(), nItem++, "Zoom In", this,
                           "ZoomIn", NULL, "ZoomIn");

  // Show Cursor
  mMenuShowCursor = new MenuItem();
  mMenuShowCursor->MakeCheckButton(this->GetViewMenu(), nItem++, "Show Cursor",
                                   this, "SetShowCursorFromMenu", NULL,
                                   "ShowCursor");

  // Show Curvature
  mMenuShowCurvature = new MenuItem();
  mMenuShowCurvature->MakeCheckButton(
      this->GetViewMenu(), nItem++, "Show Curvature", this,
      "SetShowCurvatureFromMenu", NULL, "ShowCurvature");

  this->GetViewMenu()->InsertSeparator(nItem++);

  // Graph average ROI.
  mMenuGraphAverageROI = new MenuItem();
  mMenuGraphAverageROI->MakeCommand(this->GetViewMenu(), nItem++,
                                    "Graph Average ROI", this,
                                    "GraphAverageROIInGDF", NULL, NULL);
  ...
  mPanel->Create();
  mPanel->AddPage(ksSubjectsPanelName,
                  "Explore data from the subjects data table", NULL);
  mPanel->AddPage(ksDesignPanelName, "Create the GLM Design matrix", NULL);
#if 0 // HACK disable Contrast tab for now
  mPanel->AddPage(
    ksContrastPanelName, "Create the Contrast matrix, and begin the analysis",
    NULL );
#endif
  mPanel->AddPage(ksDisplayPanelName, "Configure the view of the results",
                  NULL);
  ...
  mEntrySubjectsDir->RemoveBinding("<Unmap>", mEntrySubjectsDir,
                                   "ValueCallback");
  mEntrySubjectsDir->SetCommand(this, "SetSubjectsDir");
  if (this->mQdecProject) {
    std::string sSubjectsDir = this->mQdecProject->GetSubjectsDir();
  ...
  mEntryAverageSubject->RemoveBinding("<Unmap>", mEntryAverageSubject,
                                      "ValueCallback");
  ...
  mListScatterPlot->SetSelectionCommand(this, "ScatterPlotListBoxCallback");
  ...
  this->Script("pack %s -side top -fill x -expand yes", label->GetWidgetName());
  ...
  mEntryExcludeSubjectGT->RemoveBinding("<Unmap>", mEntryExcludeSubjectGT,
                                        "ValueCallback");
  ...
  mEntryExcludeSubjectLT->RemoveBinding("<Unmap>", mEntryExcludeSubjectLT,
                                        "ValueCallback");
  ...
  mEntryExcludeSubjectET->RemoveBinding("<Unmap>", mEntryExcludeSubjectET,
                                        "ValueCallback");
  ...
  mEntryDesignName->RemoveBinding("<Unmap>", mEntryDesignName, "ValueCallback");
  ...
  std::string sCmd = string("SetCurrentMeasure ") + radBtn->GetText();
  ...
  mListDiscreteFactors->SetSelectionCommand(this,
                                            "DiscreteFactorsListBoxCallback");
  ...
  mListContinuousFactors->SetSelectionCommand(
      this, "ContinuousFactorsListBoxCallback");
  ...
  mListNuisanceFactors->SetSelectionCommand(this,
                                            "NuisanceFactorsListBoxCallback");
  ...
  this->Script("pack %s %s %s -side top -fill x -pady 5",
               mFrameSurface->GetWidgetName(), mFrameOverlay->GetWidgetName(),
...
void vtkKWQdecWindow::FinishCreating() {
...
void vtkKWQdecWindow::BeginActionWithProgress(const char *isTitle) {
  this->SetStatusText(isTitle);
}
void vtkKWQdecWindow::UpdateProgressMessage(const char *isMessage) {
  this->SetStatusText(isMessage);
}
void vtkKWQdecWindow::UpdateProgressPercent(float iPercent) {
  this->GetProgressGauge()->SetValue(iPercent);
}
void vtkKWQdecWindow::EndActionWithProgress() {}
// ------------------------------------

void vtkKWQdecWindow::SetCurrentMeasure(const char *isMeasure) {

  // remove the existing frame contents
  if (mFrameSurfaceMeasures)
    mFrameSurfaceMeasures->Unpack();
  if (mFrameVolumeMeasures)
    mFrameVolumeMeasures->Unpack();
  ...
  if (0 == strcmp(isMeasure, "Surface-based")) {
    ...
    radBtn->SelectedStateOn(); // this one is the default, so its on
    std::string sCmd = string("SetCurrentSurfaceMeasure ") + radBtn->GetText();
    radBtn->SetCommand(this, sCmd.c_str());
    ...
    this->Script("grid %s -column 0 -row %d -sticky ne", label->GetWidgetName(),
                 nRow);
  }
...
void vtkKWQdecWindow::SetCurrentSurfaceMeasure(const char *isMeasure) {

  if (mFrameMorphMeasures)
    mFrameMorphMeasures->Unpack();
  if (mFrameFunctionalMeasures)
    mFrameFunctionalMeasures->Unpack();
  ...
  if (0 == strcmp(isMeasure, "Morphometric")) {
    ...
    if (mMenuMorphMeasure)
      mMenuMorphMeasure->Unpack();
    ...
    // NJS: not meaningful    mMenuMorphMeasure->GetMenu()->AddRadioButton(
    // "jacobian_white" );
    ...
    if (mMenuMorphSmoothness)
      mMenuMorphSmoothness->Unpack();
    ...
    if (mMenuMorphHemisphere)
      mMenuMorphHemisphere->Unpack();
    ...
    this->Script("pack %s -fill x", mFrameFunctionalMeasures->GetWidgetName());
    // temporary message:
    int                         nRow  = 0;
    vtkSmartPointer<vtkKWLabel> label = vtkSmartPointer<vtkKWLabel>::New();
...
void vtkKWQdecWindow::SetDesignMatrixType(const char *isType) {
...
void vtkKWQdecWindow::LoadDataTableFromDlog() {
  ...
  if (this->mQdecProject) {
    std::string sWorkingDir = this->mQdecProject->GetWorkingDir();
    dialog->SetLastPath(sWorkingDir.c_str());
    std::string sSubjectsDir = this->mQdecProject->GetSubjectsDir();
    mEntrySubjectsDir->SetValue(sSubjectsDir.c_str());
  }
  if (dialog->Invoke()) {
    std::string fnDataTable(dialog->GetFileName());
    try {
...
void vtkKWQdecWindow::LoadProjectFileFromDlog() {
  ...
  std::string sQdecDir = this->mQdecProject->GetSubjectsDir() + "/qdec";
  dialog->SetLastPath(sQdecDir.c_str());

  if (dialog->Invoke()) {
    std::string fnProject(dialog->GetFileName());
...
void vtkKWQdecWindow::LoadLabelFromDlog() {
  ...
  if (dialog->Invoke()) {
    ...
    std::string fnLabel(dialog->GetFileName());
    this->LoadLabel(fnLabel.c_str());
  }
}

void vtkKWQdecWindow::LoadSurfaceFromDlog() {
  ...
  std::string sQdecDir = this->mQdecProject->GetSubjectsDir() + +"/" +
                         this->mQdecProject->GetAverageSubject() + "/surf";
  dialog->SetLastPath(sQdecDir.c_str());

  if (dialog->Invoke()) {
    dialog->SaveLastPathToRegistry("LoadSurface");
    std::string fn(dialog->GetFileName());
    this->LoadSurface(fn.c_str());
  }
}

void vtkKWQdecWindow::LoadGDFFromDlog() {
...
void vtkKWQdecWindow::LoadSurfaceScalarsFromDlog() {
...
void vtkKWQdecWindow::LoadCurvatureFromDlog() {
...
void vtkKWQdecWindow::LoadAnnotationFromDlog() {
...
void vtkKWQdecWindow::SaveDataTableFromDlog() {
  ...
  std::string fnDefault = this->mQdecProject->GetDataTable()->GetFileName();
...
void vtkKWQdecWindow::SaveProjectFileFromDlog() {
  ...
  std::string fnDefault = mQdecProject->GetGlmDesign()->GetName() + ".qdec";
  dialog->SetInitialFileName(fnDefault.c_str());
...
void vtkKWQdecWindow::SaveScatterPlotPostscriptFromDlog() {
  ...
  if (dialog->Invoke()) {
    ...
    std::string fnPostscript(dialog->GetFileName());
    mGraph->SavePostscript(fnPostscript.c_str());
  }
}

void vtkKWQdecWindow::SaveTIFFImageFromDlog() {
  ...
  vtkSmartPointer<vtkKWLabel> sizeLabel = vtkSmartPointer<vtkKWLabel>::New();
  sizeLabel->SetParent(labeledFrame->GetFrame());
  sizeLabel->Create();
  std::stringstream ssCurrentSize;
  ssCurrentSize << "Current size: " << mView->GetWidth() << ", "
                << mView->GetHeight() << std::endl;
  ...
  this->Script("pack %s %s -side top -anchor w", sizeLabel->GetWidgetName(),
               labeledEntry->GetWidgetName());

  this->Script("pack %s -side top -fill x", labeledFrame->GetWidgetName());

  if (dialog->Invoke()) {
    dialog->SaveLastPathToRegistry("SaveTIFFImage");
    std::string fnTIFF(dialog->GetFileName());
...
void vtkKWQdecWindow::QuickSnapsTIFF() {
  ...
  try {
    if (maSurfaceSource.find(msCurrentSurfaceSource) == maSurfaceSource.end() ||
        maSurfaceScalars.find(mnCurrentSurfaceScalars) ==
            maSurfaceScalars.end()) {
      throw std::runtime_error("Must select from analysis results first.");
    }
  } catch (exception &e) {
    this->GetApplication()->ErrorMessage(e.what());
    return;
  ...
  if (vtkKWMessageDialog::PopupOkCancel(this->GetApplication(), this, "",
    ...
    std::string sBaseFileName = this->mQdecProject->GetWorkingDir() + "/";
    ...
    // - Save TIFF image
    std::string sFileName = sBaseFileName + "-inflated-lateral.tiff";
...
void vtkKWQdecWindow::SaveGDFPostscriptFromDlog() {
...
void vtkKWQdecWindow::SaveLabelFromDlog() {
  ...
  std::string            fnLabel(mROISource->GetLabelFileName());
  std::string::size_type lastSlash = fnLabel.rfind("/");
  fnLabel = fnLabel.substr(lastSlash + 1, std::string::npos);
...
void vtkKWQdecWindow::MapLabelFromDlog() {
  ...
  if (dialog->Invoke()) {
    std::string fnLabel = dialog->GetEntry()->GetWidget()->GetValue();
    this->MapLabelToSubjects(fnLabel.c_str());
  }
}

void vtkKWQdecWindow::SmoothCurvatureScalarsFromDlog() {
...
void vtkKWQdecWindow::SmoothSurfaceScalarsFromDlog() {
...
void vtkKWQdecWindow::LoadDataTable(const char *ifnDataTable) {

  if (mView) {
    try {

      if (this->mQdecProject->LoadDataTable(ifnDataTable))
        throw std::runtime_error("Error loading the data table.");
      else {
        this->mQdecProject->DumpDataTable(stdout);
        std::cout << "Data table loading completed successfully." << std::endl;
      }
...
void vtkKWQdecWindow::LoadProjectFile(const char *ifnProject) {
    ...
    try {
      if (ifnProject) {
        ...
        if (this->mQdecProject->LoadProjectFile(ifnProject))
          throw std::runtime_error("Error loading the project file.");
...
void vtkKWQdecWindow::SaveProjectFile(const char *ifnProject) {
    ...
    try {
      if (ifnProject) {
        ...
        if (this->mQdecProject->SaveProjectFile(ifnProject))
          throw std::runtime_error("Error saving the project file.");
...
void vtkKWQdecWindow::LoadAnalyzedData(QdecGlmFitResults *iGlmResults) {

  if (mView) {
    try {

      if (NULL == iGlmResults) {
        throw std::runtime_error("No results have been generated!");
      }
      ...
      std::string sSubjectsDir = iGlmResults->GetGlmDesign()->GetSubjectsDir();
      std::string sAverageSubj =
          iGlmResults->GetGlmDesign()->GetAverageSubject();
      std::string sHemisphere = iGlmResults->GetGlmDesign()->GetHemi();

      // Load the white surface at
      // SUBJECTS_DIR/fsaverage/surf/$hemi.white if it's available.
      std::string fnWhite =
          sSubjectsDir + "/" + sAverageSubj + "/surf/" + sHemisphere + ".white";
      ...
      std::string fnPial =
          sSubjectsDir + "/" + sAverageSubj + "/surf/" + sHemisphere + ".pial";
      ...
      this->GetProgressGauge()->SetValue((float)nStep++ * stepIncrement);
      std::string fnInflated = sSubjectsDir + "/" + sAverageSubj + "/surf/" +
                               sHemisphere + ".inflated";
      ...
      std::string fnCurvature =
          sSubjectsDir + "/" + sAverageSubj + "/surf/" + sHemisphere + ".curv";
      ...
      this->GetProgressGauge()->SetValue((float)nStep++ * stepIncrement);
      std::string fnAnnot = sSubjectsDir + "/" + sAverageSubj + "/label/" +
                            sHemisphere + ".aparc.annot";
      ...
      this->GetProgressGauge()->SetValue((float)nStep++ * stepIncrement);
      std::vector<std::string> fnContrastSigFiles =
          iGlmResults->GetContrastSigFiles();
      std::vector<std::string> sContrastQuestions =
          iGlmResults->GetContrastQuestions();
      std::vector<std::string> sContrastNames = iGlmResults->GetContrastNames();
      assert(fnContrastSigFiles.size() == sContrastQuestions.size());
      assert(sContrastQuestions.size() == sContrastNames.size());
      for (unsigned int i = 0; i < iGlmResults->GetContrastSigFiles().size();
           i++) {
        ...
        if (nScalar < 0)
          throw std::runtime_error(string("Couldn't load scalars from ") +
      ...
      this->GetProgressGauge()->SetValue((float)nStep++ * stepIncrement);
      std::string fnRSTD = iGlmResults->GetResidualErrorStdDevFile();
      QdecUtilities::AssertFileIsReadable(fnRSTD);
      int nScalar =
          this->LoadSurfaceScalars(fnRSTD.c_str(), "Residual Error StdDev");
      if (nScalar < 0)
        throw std::runtime_error(string("Couldn't load scalars from ") +
                                 fnRSTD);
      ...
      this->GetProgressGauge()->SetValue((float)nStep++ * stepIncrement);
      std::string fnBeta = iGlmResults->GetRegressionCoefficientsFile();
      QdecUtilities::AssertFileIsReadable(fnBeta);
      nScalar =
          this->LoadSurfaceScalars(fnBeta.c_str(), "Regression Coefficients");
      if (nScalar < 0)
        throw std::runtime_error(string("Couldn't load scalars from ") +
                                 fnBeta);
      ...
      this->GetProgressGauge()->SetValue((float)nStep++ * stepIncrement);
      std::string fnGDF = iGlmResults->GetFsgdFile();
...
void vtkKWQdecWindow::LoadSurface(const char *ifnSurface, const char *isLabel) {
...
void vtkKWQdecWindow::LoadGDFFile(const char *ifnGDFFile) {

  if (mView) {
    try {
      if (mVertexPlot->ReadFile(ifnGDFFile))
        return;
...
int vtkKWQdecWindow::LoadSurfaceScalars(const char *ifnScalars,
                                        const char *isLabel,
                                        const char *isLabel2, int inFrame) {
  int rnEntry = -1;

  try {

    if (maSurfaceSource.size() < 1)
      throw std::runtime_error("Must load a surface before loading scalars.");
    ...
    if (isLabel) {
      ...
      std::string            fnScalars = ifnScalars;
      std::string::size_type lastSlash = fnScalars.rfind("/");
      maSurfaceScalars[nEntry].msLabel =
          fnScalars.substr(lastSlash + 1, std::string::npos).c_str();
    }
    if (isLabel2) {
...
void vtkKWQdecWindow::LoadSurfaceCurvatureScalars(const char *ifnScalars) {
  ...
  try {
    ...
    // Try to load the scalars.
    vtkSmartPointer<vtkFloatArray> scalars;
    scalars.TakeReference(this->NewScalarsFromSurfaceScalarsFile(ifnScalars));
...
void vtkKWQdecWindow::LoadAnnotation(const char *ifnAnnotation) {

  assert(ifnAnnotation);
  if (maSurfaceSource.size() < 1)
    throw std::runtime_error("Must load a surface before loading annotation.");
  ...
  try {
    ...
    int eRead = MRISreadAnnotationIntoArray(ifnAnnotation, cValues, &indices);
    if (0 != eRead)
      throw std::runtime_error("Could not read annotation file");

    // Try to read a color table too.
    eRead = MRISreadCTABFromAnnotationIfPresent(ifnAnnotation, &table);
    if (0 != eRead)
      throw std::runtime_error("Could not read color table file");
...
void vtkKWQdecWindow::LoadSurfaceOverlayScalars(const char *ifnScalars,
  ...
  try {
    ...
    // Try to read the annotation.
    int eRead = MRISreadAnnotationIntoArray(ifnScalars, cValues, &lookupTable);
    if (0 != eRead)
      throw std::runtime_error("Could not read annotation file");
    ...
    if (0 != eRead) {
      ...
      // name for one.
      if (ifnColors == NULL) {
        throw std::runtime_error("No color table present in overlay file; "
      ...
      free(fnColors);
      if (NULL == table)
        throw std::runtime_error(string("Couldn't read color table ") +
                                 ifnColors);
    }
    ...
    // Set it in the view.
    mView->SetSurfaceOverlayScalarsAndColors(mOverlayScalars, mOverlayColors);

    // If the suffix is .annot, use "annotation" as a label, otherwise
    // use its name as the label.
    std::string fnScalars = ifnScalars;
    if (fnScalars.find(".annot") != std::string::npos) {
...
void vtkKWQdecWindow::LoadLabel(const char *ifnLabel) {

  assert(ifnLabel);
  if (maSurfaceSource.size() < 1)
    throw std::runtime_error("Must load a surface before loading labels.");
...
void vtkKWQdecWindow::SaveLabel(const char *ifnLabel) {
...
void vtkKWQdecWindow::MapLabelToSubjects(const char *ifnLabel) {
...
void vtkKWQdecWindow::SaveTIFFImage(const char *ifnTIFF,
...
void vtkKWQdecWindow::RestoreView() {
...
void vtkKWQdecWindow::SetCurrentSurfaceScalars(int inEntry) {
...
void vtkKWQdecWindow::ClearSurfaceScalars() {
...
void vtkKWQdecWindow::UnloadSurfaceScalars() {
...
void vtkKWQdecWindow::ClearCurvature() {
...
void vtkKWQdecWindow::ZoomBy(float iFactor) {
...
void vtkKWQdecWindow::ZoomIn() { this->ZoomBy(1.1); }

void vtkKWQdecWindow::ZoomOut() { this->ZoomBy(0.9090); }

void vtkKWQdecWindow::ShowCursor(int ibShow) {
...
void vtkKWQdecWindow::SetShowCursorFromMenu() {
...
void vtkKWQdecWindow::ShowCurvature(int ibShow) {
...
void vtkKWQdecWindow::SetShowCurvatureFromMenu() {
...
void vtkKWQdecWindow::SetCurrentSurface(const char *isLabel) {
  ...
  if (NULL == isLabel) {
    ...
    if (maSurfaceSource.find(string(isLabel)) == maSurfaceSource.end())
      throw std::runtime_error(
          "SetCurrentSurface: isLabel surface was not found");
    ...
    if (NULL != mROISource) {
      mROISource->SetMris(maSurfaceSource[msCurrentSurfaceSource]->GetMRIS());
      mView->SetROI(mROISource->GetOutput());
    }
  }
}

void vtkKWQdecWindow::SetSurfaceOverlayOpacity(double iOpacity) {
...
void vtkKWQdecWindow::SetCurrentSurfaceScalarsFromTableSelection() {
...
void vtkKWQdecWindow::ScatterPlotListBoxCallback() {
  assert(mListScatterPlot.GetPointer());

  mScatterPlotSelection = mListScatterPlot->GetSelectionIndex();

  if (mScatterPlotSelection != -1 && mEntryExcludeFactor &&
      mEntryExcludeSubjectGT && mEntryExcludeSubjectLT &&
      mEntryExcludeSubjectET) {
    // Get the name of the selected factor.
    std::string sFactor(mListScatterPlot->GetItem(mScatterPlotSelection));
    ...
    if (lFactor->IsDiscrete()) {
      std::vector<std::string> lLevelNames = lFactor->GetLevelNames();
...
void vtkKWQdecWindow::DiscreteFactorsListBoxCallback() {

  assert(mListDiscreteFactors.GetPointer());

  this->ManageFactorListBoxSelections(
      mListDiscreteFactors, maDiscreteFactorSelection, kMaxDiscreteFactors);
  ...
  for (int i = 0; i < kMaxDiscreteFactors; i++) {
    if (-1 != maDiscreteFactorSelection[i]) {
      design->AddDiscreteFactor(
          strdup(mListDiscreteFactors->GetItem(maDiscreteFactorSelection[i])));
    }
  }
...
void vtkKWQdecWindow::ContinuousFactorsListBoxCallback() {
  ...
  for (int i = 0; i < kMaxContinuousFactors; i++) {
    if (-1 != maContinuousFactorSelection[i]) {
      design->AddContinuousFactor(strdup(
          mListContinuousFactors->GetItem(maContinuousFactorSelection[i])));
    }
  }
...
void vtkKWQdecWindow::NuisanceFactorsListBoxCallback() {
  ...
  for (int nItem = 0; nItem < mListNuisanceFactors->GetNumberOfItems();
    ...
    if (mListNuisanceFactors->GetSelectState(nItem)) {
      design->AddNuisanceFactor(strdup(mListNuisanceFactors->GetItem(nItem)));
    }
  }
...
void vtkKWQdecWindow::ManageFactorListBoxSelections(vtkKWListBox *iListBox,
                                                    int iaSelections[2],
                                                    int iMaxFactors) {
...
void vtkKWQdecWindow::UpdateSubjectsPage() {
...
void vtkKWQdecWindow::UpdateNumberOfSubjects() {
  ...
  mEntryNumberOfSubjects->SetValueAsInt(dTable->GetSubjectIDs().size() -
                                        design->GetNumberOfExcludedSubjects());
}

void vtkKWQdecWindow::UpdateDesignPage() {

  assert(mListDiscreteFactors.GetPointer());
  assert(mListContinuousFactors.GetPointer());
  assert(mListNuisanceFactors.GetPointer());
  assert(mQdecProject);
  if (!mQdecProject->HaveDataTable())
    return;
  assert(mQdecProject->GetDataTable());
  assert(mQdecProject->GetGlmDesign());
  ...
  for (unsigned int i = 0; i < factors.size(); i++) {
    ...
    for (unsigned int j = 0; j < lDiscreteFactors.size(); j++) {
      if (lDiscreteFactors[j]) {
        std::string factorName = lDiscreteFactors[j]->GetFactorName();
  ...
  for (unsigned int i = 0; i < factors.size(); i++) {
    ...
    for (unsigned int j = 0; j < lContinuousFactors.size(); j++) {
      if (lContinuousFactors[j]) {
        std::string factorName = lContinuousFactors[j]->GetFactorName();
        if (factorName == factors[i]) {
  ...
  for (unsigned int i = 0; i < factors.size(); i++) {
    ...
    for (unsigned int j = 0; j < lNuisanceFactors.size(); j++) {
      if (lNuisanceFactors[j]) {
        std::string factorName = lNuisanceFactors[j]->GetFactorName();
        if (factorName == factors[i]) {
...
void vtkKWQdecWindow::UpdateDisplayPage() {
  ...
  if (maSurfaceSource.size() > 1 && !mbFrameSurfaceInited) {
    ...
    this->Script("pack %s -side top -fill both", surfaceFrame->GetWidgetName());
    ...
    for (tSurface = maSurfaceSource.begin(); tSurface != maSurfaceSource.end();
         ++tSurface) {

      std::string sSurface = tSurface->first;
      ...
      std::string sCmd = string("SetCurrentSurface ") + sSurface;
      radBtn->SetCommand(this, sCmd.c_str());

      // If this is our currently displayed surface, select the button.
      if (sSurface == msCurrentSurfaceSource)
        radBtn->SelectedStateOn();
    }
  ...
  if (maSurfaceSource.size() > 1 && mbFrameSurfaceInited) {
    ...
    do {
      ...
      if (radBtn) {
        std::string sSurface = radBtn->GetText();
        // If this is our currently displayed surface, select the button.
        if (sSurface == msCurrentSurfaceSource)
          radBtn->SelectedStateOn();
      }
  ...
  if (mOverlayScalars.GetPointer() && mOverlayColors.GetPointer() &&
    ...
    this->Script("pack %s -side top -fill both", overlayFrame->GetWidgetName());
    ...
    this->Script("pack %s -side top -fill both", labeledScale->GetWidgetName());
  ...
  if (maSurfaceScalars.size() > 0 && !mbFrameSurfaceScalarsInited) {
    ...
    this->Script("pack %s -side top -fill both", scalarsFrame->GetWidgetName());
    ...
    this->Script("pack %s -side top -fill both", scrolls->GetWidgetName());
    ...
    mTableSurfaceScalars->SetPotentialCellColorsChangedCommand(
    ...
    mCheckSurfaceScalarsColorReverse->SetSelectedState(
        mbSurfaceScalarsColorReverse);
    mCheckSurfaceScalarsColorReverse->SetCommand(
        this, "SetSurfaceScalarsColorReverse");
    ...
    framePosNeg->Create();
    this->Script("pack %s -side top -fill both", framePosNeg->GetWidgetName());
    ...
    mCheckSurfaceScalarsColorShowPositive->SetSelectedState(
        mbSurfaceScalarsColorShowPositive);
    mCheckSurfaceScalarsColorShowPositive->SetCommand(
        this, "SetSurfaceScalarsColorShowPositive");
    ...
    mCheckSurfaceScalarsColorShowNegative->SetSelectedState(
        mbSurfaceScalarsColorShowNegative);
    mCheckSurfaceScalarsColorShowNegative->SetCommand(
        this, "SetSurfaceScalarsColorShowNegative");

    // Pack the inner frame, then the frame.
    this->Script(
        "pack %s %s %s -side left -fill both", labelShowValues->GetWidgetName(),
        labeledCheckPos->GetWidgetName(), labeledCheckNeg->GetWidgetName());
    this->Script("pack %s -side top -fill both", framePosNeg->GetWidgetName());
    ...
    if (mbUseHistogramEditor) {
      ...
      mEditorSurfaceScalarColors->SetFunctionChangedCommand(
          this, "SurfaceScalarColorsEditorChanged");
    ...
    mEntrySurfaceScalarsColorMin->SetCommand(this, "SetSurfaceScalarsColorMin");
    ...
    mEntrySurfaceScalarsColorMid->SetCommand(this, "SetSurfaceScalarsColorMid");
    ...
    mEntrySurfaceScalarsColorMax->SetCommand(this, "SetSurfaceScalarsColorMax");
    ...
    mEntrySurfaceScalarsColorOffset->SetCommand(this,
                                                "SetSurfaceScalarsColorOffset");
    mEntrySurfaceScalarsColorOffset->SetValueAsDouble(
        mSurfaceScalarsColorOffset);

#if USE_MID
    this->Script(
        "pack %s %s %s %s -side left -padx 5 -fill x",
        labeledEntryMin->GetWidgetName(), labeledEntryMid->GetWidgetName(),
        labeledEntryMax->GetWidgetName(), labeledEntryOffset->GetWidgetName());
#else
...
#endif
    this->Script("pack %s -side top -fill x", frameEntries->GetWidgetName());
    ...
    this->Script(
        "pack %s %s %s -side left", buttonGenerateClusterStats->GetWidgetName(),
        buttonNextCluster->GetWidgetName(), buttonPrevCluster->GetWidgetName());
    ...
    frameFDR->Create();
    this->Script("pack %s -side top -fill x", frameFDR->GetWidgetName());
    ...
    labeledEntryFDR->GetWidget()->SetCommand(this,
                                             "SetSurfaceScalarsColorsFDRRate");
    labeledEntryFDR->GetWidget()->SetValueAsDouble(
        mSurfaceScalarsColorsFDRRate);
    ...
    this->Script("pack %s -side top -fill x", frameSimulation->GetWidgetName());

    // sub-frames to allow clean arrangement of the stuff
    vtkSmartPointer<vtkKWFrame> frameSim1 = vtkSmartPointer<vtkKWFrame>::New();
    frameSim1->SetParent(frameSimulation->GetFrame());
    frameSim1->Create();
    vtkSmartPointer<vtkKWFrame> frameSim2 = vtkSmartPointer<vtkKWFrame>::New();
    frameSim2->SetParent(frameSimulation->GetFrame());
    frameSim2->Create();
    this->Script("pack %s %s -side top -fill x", frameSim1->GetWidgetName(),
    ...
    this->Script(
        "pack %s %s %s %s -side left -expand y -fill x -padx 5",
        label1->GetWidgetName(), mMenuButtonSimulationThresh->GetWidgetName(),
        label2->GetWidgetName(), mMenuButtonSimulationSign->GetWidgetName());
    ...
    this->Script("pack %s", buttonSimulation->GetWidgetName());

    // Pack the multiple comparisons items in the user frame.
    this->Script("pack %s %s -side top -fill x -pady 2",
                 editorUserFrame->GetWidgetName(), cmcFrame->GetWidgetName());
  ...
  if (mTableSurfaceScalars.GetPointer() && mbFrameSurfaceScalarsInited) {
    ...
    for (tScalar = maSurfaceScalars.begin(); tScalar != maSurfaceScalars.end();
         ++tScalar) {

      // Set the cell text.
      mTableSurfaceScalars->InsertCellText((*tScalar).second.mnEntry, 0,
      ...
      mTableSurfaceScalars->SetCellWindowCommand(
          (*tScalar).second.mnEntry, 0, this, "CreateScalarTableEntry");
    }
  }
}

void vtkKWQdecWindow::UpdateSurfaceScalarsColorsEditor() {
  ...
  mnNegativeMaxValue =
      colors->AddRGBAPoint(-mSurfaceScalarsColorMax, mnNegativeMaxRedValue,
                           mnNegativeMaxGreenValue, mnNegativeMaxBlueValue, 1);
#if USE_MID
  mnNegativeMidValue =
      colors->AddRGBAPoint(-mSurfaceScalarsColorMid, mnNegativeMidRedValue,
                           mnNegativeMidGreenValue, mnNegativeMidBlueValue, 1);
#endif
  mnNegativeMinValue =
      colors->AddRGBAPoint(-mSurfaceScalarsColorMin, mnNegativeMinRedValue,
                           mnNegativeMinGreenValue, mnNegativeMinBlueValue, 1);
  mnPositiveMinValue =
      colors->AddRGBAPoint(mSurfaceScalarsColorMin, mnPositiveMinRedValue,
                           mnPositiveMinGreenValue, mnPositiveMinBlueValue, 1);
#if USE_MID
  mnPositiveMidValue =
      colors->AddRGBAPoint(mSurfaceScalarsColorMid, mnPositiveMidRedValue,
                           mnPositiveMidGreenValue, mnPositiveMidBlueValue, 1);
#endif
  mnPositiveMaxValue =
      colors->AddRGBAPoint(mSurfaceScalarsColorMax, mnPositiveMaxRedValue,
                           mnPositiveMaxGreenValue, mnPositiveMaxBlueValue, 1);
  colors->Build();
  ...
  mEditorSurfaceScalarColors->SetPointSymmetry(mnNegativeMaxValue,
                                               mnPositiveMaxValue);
#if USE_MID
  mEditorSurfaceScalarColors->SetPointSymmetry(mnNegativeMidValue,
                                               mnPositiveMidValue);
#endif
  mEditorSurfaceScalarColors->SetPointSymmetry(mnNegativeMinValue,
                                               mnPositiveMinValue);
  ...
  if (maSurfaceScalars.find(mnCurrentSurfaceScalars) !=
    ...
    for (int nVertex = 0; nVertex < surfaceScalars->GetNumberOfTuples();
         nVertex++) {
      float val = surfaceScalars->GetValue(nVertex);
      if (val > 100)
        surfaceScalars->SetValue(nVertex, 100);
      else if (val < -100)
        surfaceScalars->SetValue(nVertex, -100);
    }
    ...
    double highestAbs =
        MAX(MAX(fabs(range[0]), fabs(range[1])), mSurfaceScalarsColorMax);
    mEditorSurfaceScalarColors->SetWholeParameterRange(-highestAbs, highestAbs);
    mEditorSurfaceScalarColors->SetVisibleParameterRangeToWholeParameterRange();
...
void vtkKWQdecWindow::CreateScalarTableEntry(const char *iTable, int iRow,
                                             int iColumn, const char *iWidget) {
  ...
  std::string sLabel = mTableSurfaceScalars->GetCellText(iRow, iColumn);
...
void vtkKWQdecWindow::SurfaceScalarColorsEditorChanged() {
  ...
  double value;
  mEditorSurfaceScalarColors->GetFunctionPointParameter(mnPositiveMinValue,
                                                        &value);
  if (mSurfaceScalarsColorMin != value)
    this->SetSurfaceScalarsColorMin(value);

#if USE_MID
  mEditorSurfaceScalarColors->GetFunctionPointParameter(mnPositiveMidValue,
                                                        &value);
  if (mSurfaceScalarsColorMid != value)
    this->SetSurfaceScalarsColorMid(value);
#endif

  mEditorSurfaceScalarColors->GetFunctionPointParameter(mnPositiveMaxValue,
                                                        &value);
...
void vtkKWQdecWindow::SetSurfaceScalarsColorMin(double iMin) {

  assert(mEntrySurfaceScalarsColorMin.GetPointer());

  // If not already equal...
  if (fabs(iMin - mSurfaceScalarsColorMin) > std::numeric_limitsdouble >
      ::epsilon()) {
    ...
    if (fabs(mEntrySurfaceScalarsColorMin->GetValueAsDouble() -
             mSurfaceScalarsColorMin) > std::numeric_limitsdouble > ::epsilon())
      mEntrySurfaceScalarsColorMin->SetValueAsDouble(mSurfaceScalarsColorMin);
...
void vtkKWQdecWindow::SetSurfaceScalarsColorMid(double iMid) {
#if USE_MID
  assert(mEntrySurfaceScalarsColorMin.GetPointer());

  // If not already equal...
  if (fabs(iMid - mSurfaceScalarsColorMid) > std::numeric_limitsdouble >
      ::epsilon()) {
    ...
    if (fabs(mEntrySurfaceScalarsColorMid->GetValueAsDouble() -
             mSurfaceScalarsColorMid) > std::numeric_limitsdouble > ::epsilon())
      mEntrySurfaceScalarsColorMid->SetValueAsDouble(mSurfaceScalarsColorMid);
...
void vtkKWQdecWindow::SetSurfaceScalarsColorMax(double iMax) {

  assert(mEntrySurfaceScalarsColorMax.GetPointer());

  // If not already equal...
  if (fabs(iMax - mSurfaceScalarsColorMax) > std::numeric_limitsdouble >
      ::epsilon()) {
    ...
    if (fabs(mEntrySurfaceScalarsColorMax->GetValueAsDouble() -
             mSurfaceScalarsColorMax) > std::numeric_limitsdouble > ::epsilon())
      mEntrySurfaceScalarsColorMax->SetValueAsDouble(mSurfaceScalarsColorMax);
...
void vtkKWQdecWindow::SetSurfaceScalarsColors(double iMin, double iMid,
                                              double iMax) {
...
void vtkKWQdecWindow::SetSurfaceScalarsColorOffset(double iOffset) {

  assert(mEntrySurfaceScalarsColorOffset.GetPointer());

  // If not already equal...
  if (fabs(iOffset - mSurfaceScalarsColorOffset) > std::numeric_limitsdouble >
      ::epsilon()) {
    ...
    if (fabs(mEntrySurfaceScalarsColorOffset->GetValueAsDouble() -
             mSurfaceScalarsColorOffset) > std::numeric_limitsdouble >
        ::epsilon())
      mEntrySurfaceScalarsColorOffset->SetValueAsDouble(
          mSurfaceScalarsColorOffset);
...
void vtkKWQdecWindow::GenerateClusterStats() {
  ...
  try {
    ...
    if (maSurfaceSource.find(msCurrentSurfaceSource) == maSurfaceSource.end() ||
        maSurfaceScalars.find(mnCurrentSurfaceScalars) ==
            maSurfaceScalars.end()) {
      throw std::runtime_error(
          "Must have a surface loaded and results selected.");
    }
    ...
    fprintf(stdout,
            "============================================================\n");
    std::cout << "Generating cluster stats using min threshold of "
    ...
    if (NULL == mClusterStats)
      throw std::runtime_error("Unable to generate cluster stats\n");
    else
      this->SetStatusText("Completed generation of cluster stats");
    std::cout << "Found " << mnClusters << " clusters" << std::endl;
    std::cout << "Contrast: '"
              << maSurfaceScalars[mnCurrentSurfaceScalars].msLabel2 << "', "
              << mQdecProject->GetGlmDesign()->GetSmoothness() << "fwhm, DOF: "
              << mQdecProject->GetGlmDesign()->GetDegreesOfFreedom()
              << std::endl;

    // screen dump cluster info
    fprintf(stdout, "ClusterNo  Max   VtxMax  Size(mm2)   "
                    "TalX   TalY   TalZ NVtxs Annotation\n"
                    "---------  ---   ------  ---------   "
                    "----   ----   ---- ----- ----------\n");
    for (int n = 0; n < mnClusters; n++) {
      fprintf(stdout, " %4d  %8.4f  %6d    %6.2f  %6.1f %6.1f %6.1f %4d  %s\n",
              n + 1, mClusterStats[n].maxval, mClusterStats[n].vtxmaxval,
              mClusterStats[n].area, mClusterStats[n].xxfm,
              mClusterStats[n].yxfm, mClusterStats[n].zxfm,
...
void vtkKWQdecWindow::GotoNextCluster() {
...
void vtkKWQdecWindow::GotoPrevCluster() {
...
void vtkKWQdecWindow::GotoCluster(int iCurrentCluster) {
  ...
  try {
    ...
    if (maSurfaceSource.find(msCurrentSurfaceSource) == maSurfaceSource.end() ||
        maSurfaceScalars.find(mnCurrentSurfaceScalars) ==
            maSurfaceScalars.end()) {
      throw std::runtime_error(
          "Must have a surface loaded and scalar selected.");
    }

    if (NULL == mClusterStats) {
      throw std::runtime_error("Must generate cluster stats first.");
    }

    if (0 == mnClusters) {
      throw std::runtime_error("There are zero clusters.");
    }
    ...
    fprintf(stdout, " %4d  %8.4f  %6d    %6.2f  %6.1f %6.1f %6.1f %4d  %s\n",
            n + 1, mClusterStats[n].maxval, mClusterStats[n].vtxmaxval,
            mClusterStats[n].area, mClusterStats[n].xxfm, mClusterStats[n].yxfm,
            mClusterStats[n].zxfm, mClusterStats[n].nmembers,
            this->GetAnnotationForVertex(mClusterStats[n].vtxmaxval));
...
void vtkKWQdecWindow::SetSurfaceScalarsColorsUsingFDR() {
  ...
  try {
    ...
    int    err = MRISfdr2vwth(mris, mSurfaceScalarsColorsFDRRate, sign,
                           ...
                           &threshold);
    if (err)
      throw std::runtime_error("Error from MRISfdr2vwth");
    else
...
void vtkKWQdecWindow::SetSurfaceScalarsColorsFDRRate(const char *isValue) {

  std::stringstream ssValue(isValue);
  ssValue >> mSurfaceScalarsColorsFDRRate;
}

// Creates a script to run mri_glmfit/mris_surfcluster for multiple-
// comparisons correction
void vtkKWQdecWindow::RunSimulation() {
  ...
  try {
    ...
    if (0 !=
        strcmp(this->mQdecProject->GetAverageSubject().c_str(), "fsaverage")) {
      throw std::runtime_error(
    ...
    if (0 == strlen(contrast)) {
      throw std::runtime_error("Select an Analysis Result corresponding to a "
                               "contrast");
    }

    std::string threshold;
    if (0 == strcmp(mMenuButtonSimulationThresh->GetValue(), "1.3 (0.05)")) {
      threshold = "th13";
    } else if (0 ==
               strcmp(mMenuButtonSimulationThresh->GetValue(), "2.0 (0.01)")) {
      threshold = "th20";
    } else if (0 ==
               strcmp(mMenuButtonSimulationThresh->GetValue(), "2.3 (0.005)")) {
      threshold = "th23";
    } else if (0 ==
               strcmp(mMenuButtonSimulationThresh->GetValue(), "3.0 (0.001)")) {
      threshold = "th30";
    ...
    if (this->mQdecProject->RunMonteCarloSimulation(
            threshold.c_str(), mMenuButtonSimulationSign->GetValue(), contrast,
            &fnClusterSigFile)) {
      throw std::runtime_error("Error running mri_surfcluster!");
    }
    int nScalar = this->LoadSurfaceScalars(fnClusterSigFile,
                                           NULL, // will show filename
                                           contrast);
    if (nScalar < 0) {
      throw std::runtime_error(string("Could not load results file ") +
                               fnClusterSigFile);
    }
...
void vtkKWQdecWindow::UpdateScatterPlot() {
  ...
  if (mScatterPlotSelection != -1) {
    ...
    std::vector<double> lPoints;

    // Get the name of the factor.
    std::string sFactor(mListScatterPlot->GetItem(mScatterPlotSelection));

    // For each subject...
    std::vector<QdecSubject *>::iterator tSubject;
    std::vector<QdecSubject *>           lSubjects =
        mQdecProject->GetDataTable()->GetSubjects();
    int nIndex = 0;
    for (tSubject = lSubjects.begin(); tSubject != lSubjects.end();
         ++tSubject, nIndex++) {
      ...
      if (isnan(value)) {
        std::cout << std::endl
                  << "WARNING: Excluding subject "
                  << (*tSubject)->GetId().c_str()
                  << " from plot due to NaN data point!" << std::endl;
      ...
      mGraph->AddElement((*tSubject)->GetId().c_str(), lPoints, sSymbol.c_str(),
                         0, color[0], color[1], color[2]);

      // and the axis labels
      mGraph->SetYAxisTitle(sFactor.c_str());
      std::string sXaxis = "Subject";
...
void vtkKWQdecWindow::SelectSurfaceVertex(int inVertex) {
...
void vtkKWQdecWindow::AddSelectionToROI() {

  assert(mView.GetPointer());
  if (maSurfaceSource.size() < 1)
    throw std::runtime_error("Must load a surface before "
  ...
  vtkSmartPointer<vtkIntArray> vertices = vtkSmartPointer<vtkIntArray>::New();
  ...
  if (!mROISource.GetPointer()) {
    ...
    std::string sLabelName = mMenuMorphHemisphere->GetValue();
...
void vtkKWQdecWindow::RemoveSelectionFromROI() {

  assert(mView.GetPointer());
  if (maSurfaceSource.size() < 1)
    throw std::runtime_error("Must load a surface "
...
void vtkKWQdecWindow::GraphAverageROIInGDF() {
  ...
  std::vector<int>::iterator tVertex;
  ...
  // Set the info string.
  std::string info = "\"Average of ROI\"";
  mVertexPlot->SetInfo(info.c_str());
}

void vtkKWQdecWindow::SmoothCurvatureScalars(int icSteps) {
  ...
  maSurfaceSource.begin()->second->SmoothValuesOnSurface(*mCurvatureScalars,
                                                         icSteps);
...
void vtkKWQdecWindow::SmoothSurfaceScalars(int icSteps) {
  ...
  // Get the first surface to use to do the smoothing, and smooth the
  // values using that surface topography.
  maSurfaceSource.begin()->second->SmoothValuesOnSurface(
      *maSurfaceScalars[mnCurrentSurfaceScalars].mValues, icSteps);

  // Redraw.
  this->ComposeSurfaceScalarsAndShow();
}

void vtkKWQdecWindow::NotebookPageRaised(const char *isTitle) {
...
void vtkKWQdecWindow::NotebookRaisePageCallback(vtkObject *   iCaller,
  ...
  try {
    ...
    vtkKWQdecWindow *window = static_cast<vtkKWQdecWindow *>(iClientData);

    if (window)
      window->NotebookPageRaised(sTitle);
  } catch (...) {
    std::cerr << "Invalid call or client data in NotebookRaisePageCallback"
              << std::endl;
  }
}

void vtkKWQdecWindow::ScatterPlotGraphMouseoverEnterElement(
...
void vtkKWQdecWindow::ScatterPlotGraphMouseoverExitElement() {

  // Clear our status bar.
  this->SetStatusText("");
}

void vtkKWQdecWindow::ScatterPlotGraphSetUpContextualMenu(const char *isElement,
                                                          vtkKWMenu * iMenu) {
  ...
  if (NULL != envVars) {
    ...
    iMenu->AddSeparator();
    std::stringstream ssTkmeditCmd;
    std::stringstream ssTkmeditCommand;
    // catch { exec tkmedit $s norm.mgz $hemi.white -segmentation aseg.mgz }
    ssTkmeditCmd << envVars->FREESURFER_HOME << "/tktools/tkmedit " << isElement
                 << " norm.mgz -aux T1.mgz -surfs "
                 << "-segmentation aparc+aseg.mgz &";
    ssTkmeditCommand << "set err [catch { exec " << ssTkmeditCmd.str().c_str()
                     << " }] ; if { $err != 0 } { "
    ...
    // tksurfer, left hemi
    std::stringstream ssTksurferCmd;
    std::stringstream ssTksurferCommand;
    ...
    // tksurfer, right hemi
    std::stringstream ssTksurferCmd2;
    std::stringstream ssTksurferCommand2;
...
void vtkKWQdecWindow::ScatterPlotGraphMouseoverEnterElementCallback(
  ...
  try {
    ...
    std::cerr << "Invalid call or client data in "
              << "ScatterPlotGraphMouseoverEnterElementCallback" << std::endl;
  }
}

void vtkKWQdecWindow::ScatterPlotGraphMouseoverExitElementCallback(
    vtkObject *iCaller, unsigned long iEventId, void *iClientData,
    void *iCallData) {
  ...
  try {
    ...
    std::cerr << "Invalid call or client data in "
              << "ScatterPlotGraphMouseoverExitElementCallback" << std::endl;
  }
}

void vtkKWQdecWindow::ScatterPlotGraphContextualMenuOpeningCallback(
    vtkObject *iCaller, unsigned long iEventId, void *iClientData,
    void *iCallData) {
  ...
  try {
    ...
    std::cerr << "Invalid call or client data in "
              << "ScatterPlotGraphContextualMenuOpeningCallback" << std::endl;
  }
}

void vtkKWQdecWindow::SetExcludeSubjectID(const char *isElement,
                                          int         ibExclude) {
...
void vtkKWQdecWindow::SetExcludeSubjectGT(double inExcludeGT) {

  if (this->mQdecProject) {
    std::string value = this->mEntryExcludeSubjectGT->GetValue();
    if (value == "")
      return;
...
void vtkKWQdecWindow::SetExcludeSubjectGT(const char *isExcludeGT) {
  if (strcmp(isExcludeGT, "") == 0)
    return;
}

void vtkKWQdecWindow::SetExcludeSubjectLT(double inExcludeLT) {
...
void vtkKWQdecWindow::SetExcludeSubjectLT(const char *isExcludeLT) {
  if (strcmp(isExcludeLT, "") == 0)
    return;
}

void vtkKWQdecWindow::SetExcludeSubjectET(double inExcludeET) {

  if (this->mQdecProject) {
    std::string value = this->mEntryExcludeSubjectET->GetValue();
    if (value == "")
      return;
...
void vtkKWQdecWindow::SetExcludeSubjectET(const char *isExcludeET) {
  if (strcmp(isExcludeET, "") == 0)
    return;
}

void vtkKWQdecWindow::ClearAllExcludedSubjects() {
...
void vtkKWQdecWindow::ResetStatsImportFrame() {
  ...
  if (mMenuStatsData)
    mMenuStatsData->Unpack();
  if (mListBoxStatsImport)
    mListBoxStatsImport->Unpack();
  if (mBtnStatsAddToDataTable)
    mBtnStatsAddToDataTable->Unpack();
  ...
  mMenuStatsData->GetMenu()->AddRadioButton("<none selected>", this,
...
void vtkKWQdecWindow::GenerateStatsDataTables() {
  ...
  if (this->mQdecProject) {
    ...
    std::vector<std::string> statsNames =
        this->mQdecProject->CreateStatsDataTables();

    if (statsNames.size()) { // it sppears to have given us some data
      // remove the generate button...
      if (mBtnStatsGenerate)
        mBtnStatsGenerate->Unpack();

      // ...and replace with a menu button containing names of the generated
      // stats tables
      for (unsigned int i = 0; i < statsNames.size(); i++) {
        std::string sCmd =
            string("SetStatsImportItem ") + statsNames[i].c_str();
        mMenuStatsData->GetMenu()->AddRadioButton(statsNames[i].c_str(), this,
                                                  sCmd.c_str());
      }
      // this->Script( "pack %s -fill x", mFrameStatsImport->GetWidgetName() );
...
void vtkKWQdecWindow::SetStatsImportItem(const char *isStatsImportItem) {
  // remove prior selections
  mListStatsImportFactors->DeleteAll();

  // handle case of <none selected>
  if (!strcmp(isStatsImportItem, "none"))
    return;
  ...
  std::stringstream ssFsIdColName;
  if (!strcmp(isStatsImportItem, "aseg.vol"))
    ssFsIdColName << "volume";
  else
    ssFsIdColName << isStatsImportItem;

  // load the table
  mStatsImportDataTable = new QdecDataTable();
  int ret = mStatsImportDataTable->Load(ssFname.str().c_str(), NULL,
  ...
  std::vector<std::string> factors =
      mStatsImportDataTable->GetContinuousFactorNames();
  for (unsigned int i = 0; i < factors.size(); i++) {
...
void vtkKWQdecWindow::AddStatsToDataTable() {
  ...
  int nItemsMerged = 0;
  for (int nItem = 0; nItem < this->mListStatsImportFactors->GetNumberOfItems();
       nItem++) {
    if (this->mListStatsImportFactors->GetSelectState(nItem)) {
      // Get the name of the factor.
      std::string sFactor(this->mListStatsImportFactors->GetItem(nItem));
...
void vtkKWQdecWindow::RemoveFactorFromDataTable() {
  ...
  mScatterPlotSelection = mListScatterPlot->GetSelectionIndex();
  if (-1 == mScatterPlotSelection)
    return; // nothing selected

  // Get the name of the selected factor.
  std::string sFactor(mListScatterPlot->GetItem(mScatterPlotSelection));

  std::string sPopUpMsg = "Remove factor '" + sFactor + "'?";
...
char const *vtkKWQdecWindow::GetAnnotationForVertex(int inVertex) {

  // Bail if we have nothing to lookup?
  if (NULL == mAnnotationTable || NULL == maAnnotationIndicies)
    return NULL;
  ...
  int nEntry = 0;
  CTABfindAnnotation(mAnnotationTable, maAnnotationIndicies[inVertex], &nEntry);
...
void vtkKWQdecWindow::FindVerticesInsidePath(vtkPoints *  iPath,
...
vtkKWQdecWindow::NewScalarsFromSurfaceScalarsFile(const char *ifnScalars,
  ...
  if (0 != eRead) {
    if (values)
      free(values);
    throw std::runtime_error("Could not read scalar file");
  }
...
void vtkKWQdecWindow::ComposeSurfaceScalarsAndShow() {
  ...
  if (mView.GetPointer()) {
    ...
    const double EPS = std::numeric_limitsdouble > ::epsilon() * 2.0;
    ...
    composedColors->AddRGBPoint(-mSurfaceScalarsColorMax, mnNegativeMaxRedValue,
                                mnNegativeMaxGreenValue,
                                mnNegativeMaxBlueValue);
#if USE_MID
    composedColors->AddRGBPoint(-mSurfaceScalarsColorMid, mnNegativeMidRedValue,
                                mnNegativeMidGreenValue,
                                mnNegativeMidBlueValue);
#endif
    composedColors->AddRGBPoint(-mSurfaceScalarsColorMin, mnNegativeMinRedValue,
    ...
    if (bUseGray && mSurfaceScalarsColorMin != 0) {
      composedColors->AddRGBPoint(-mSurfaceScalarsColorMin + EPS, 0.5, 0.5,
                                  0.5);
      if (mCurvatureScalars.GetPointer() && mbShowCurvature) {
    ...
    if (mCurvatureScalars.GetPointer() && mbShowCurvature &&
      ...
      if (mbDrawCurvatureGreenRedIfNoScalars && NULL == surfaceScalars) {
    ...
    if (bUseGray && mSurfaceScalarsColorMin != 0) {
      if (mCurvatureScalars.GetPointer() && mbShowCurvature)
        composedColors->AddRGBPoint(curvatureMin + EPS, 0.5, 0.5, 0.5);
      composedColors->AddRGBPoint(mSurfaceScalarsColorMin - EPS, 0.5, 0.5, 0.5);
    }

    // Positive scalar range.
    composedColors->AddRGBPoint(mSurfaceScalarsColorMax, mnPositiveMaxRedValue,
                                mnPositiveMaxGreenValue,
                                mnPositiveMaxBlueValue);
#if USE_MID
    composedColors->AddRGBPoint(mSurfaceScalarsColorMid, mnPositiveMidRedValue,
                                mnPositiveMidGreenValue,
                                mnPositiveMidBlueValue);
#endif
    composedColors->AddRGBPoint(mSurfaceScalarsColorMin, mnPositiveMinRedValue,
    ...
    if (surfaceScalars) {
      ...
      if ("" != maSurfaceScalars[mnCurrentSurfaceScalars].msLabel2) {
        ...
        ssLabel << std::endl
                ...
                << maSurfaceScalars[mnCurrentSurfaceScalars].msLabel2
                << std::endl
                << "n=" << mEntryNumberOfSubjects->GetValue() << ", DOF="
                << mQdecProject->GetGlmDesign()->GetDegreesOfFreedom();
      }
      ...
      if (mbSurfaceScalarsColorReverse) { // 'Reverse Values' check-boxed
        ...
        surfaceScalarsColors->AddRGBPoint(
            -mSurfaceScalarsColorMax, mnPositiveMaxRedValue,
            mnPositiveMaxGreenValue, mnPositiveMaxBlueValue);
#if USE_MID
        surfaceScalarsColors->AddRGBPoint(
            -mSurfaceScalarsColorMid, mnPositiveMidRedValue,
            mnPositiveMidGreenValue, mnPositiveMidBlueValue);
#endif
        surfaceScalarsColors->AddRGBPoint(
            -mSurfaceScalarsColorMin, mnPositiveMinRedValue,
            mnPositiveMinGreenValue, mnPositiveMinBlueValue);
        surfaceScalarsColors->AddRGBPoint(-mSurfaceScalarsColorMin + EPS, 0.5,
                                          0.5, 0.5);
        surfaceScalarsColors->AddRGBPoint(mSurfaceScalarsColorMin - EPS, 0.5,
                                          0.5, 0.5);
        surfaceScalarsColors->AddRGBPoint(
            mSurfaceScalarsColorMin, mnNegativeMinRedValue,
            mnNegativeMinGreenValue, mnNegativeMinBlueValue);
#if USE_MID
        surfaceScalarsColors->AddRGBPoint(
            mSurfaceScalarsColorMid, mnNegativeMidRedValue,
            mnNegativeMidGreenValue, mnNegativeMidBlueValue);
#endif
        surfaceScalarsColors->AddRGBPoint(
            mSurfaceScalarsColorMax, mnNegativeMaxRedValue,
            mnNegativeMaxGreenValue, mnNegativeMaxBlueValue);
...
void vtkKWQdecWindow::UserSelectedVertexCallback(vtkObject *   iCaller,
                                                 unsigned long iEventId,
                                                 void *        iClientData,
                                                 void *        iCallData) {
  ...
  try {
    ...
    std::cerr << "Invalid client data in UserTransformChanged callback"
              << std::endl;
  }
}

void vtkKWQdecWindow::UserSelectedVertex(
  ...
  // command.
  if (iInfo.mnVertexIndex >= 0 && mVertexPlot->IsLoaded() &&
...
void vtkKWQdecWindow::UpdateCommandStatus() {
  ...
  if ((mScatterPlotSelection != -1) && mGraph &&
  ...
  if ((maSurfaceSource.size() > 0) && mView &&
  ...
  assert(mMenuSaveGDFPostscript);
  if (mVertexPlot->IsLoaded() && mVertexPlot->IsWindowShowing()) {
    mMenuSaveGDFPostscript->SetStateToNormal();
  } else {
    mMenuSaveGDFPostscript->SetStateToDisabled();
  }
  ...
  if (mROISource.GetPointer() && maSurfaceSource.size() > 0 && mQdecProject &&
      mQdecProject->GetSubjectIDs().size() > 0) {
...
void vtkKWQdecWindow::AnalyzeDesign() {
  ...
  try {
    ...
    this->mQdecProject->SetAverageSubject(mEntryAverageSubject->GetValue());
    std::string fnWorkingDir = this->mQdecProject->GetDefaultWorkingDir();
    fnWorkingDir += "/";
    ...
    if (-1 != maDiscreteFactorSelection[0]) {
      df1 = strdup(mListDiscreteFactors->GetItem(maDiscreteFactorSelection[0]));
    } else {
      df1 = strdup(sNone);
    }
    const char *df2;
    if (-1 != maDiscreteFactorSelection[1]) {
      df2 = strdup(mListDiscreteFactors->GetItem(maDiscreteFactorSelection[1]));
    } else {
      df2 = strdup(sNone);
    }
    const char *cf1;
    if (-1 != maContinuousFactorSelection[0]) {
      cf1 = strdup(
          mListContinuousFactors->GetItem(maContinuousFactorSelection[0]));
    } else {
      cf1 = strdup(sNone);
    }
    const char *cf2;
    if (-1 != maContinuousFactorSelection[1]) {
      cf2 = strdup(
          mListContinuousFactors->GetItem(maContinuousFactorSelection[1]));
    } else {
      cf2 = strdup(sNone);
    }
...
void vtkKWQdecWindow::SetSubjectsDir(const char *isSubjectsDir) {
...
void vtkKWQdecWindow::SetAverageSubject(const char *isAverageSubject) {

  if (this->mQdecProject) {
    this->mQdecProject->SetAverageSubject(isAverageSubject);
    std::string sAverageSubject = this->mQdecProject->GetAverageSubject();
    this->mEntryAverageSubject->SetValue(sAverageSubject.c_str());
  }
}

void vtkKWQdecWindow::SetDesignName(const char *isDesignName) {
...
bool vtkKWQdecWindow::GetShowCurvature() { return mbShowCurvature; }

void vtkKWQdecWindow::SetShowCurvature(bool ibShow) {

  mbShowCurvature = ibShow;
  this->ComposeSurfaceScalarsAndShow();
}

void vtkKWQdecWindow::SetDrawCurvatureGreenRed(int ibDraw) {

  mbDrawCurvatureGreenRedIfNoScalars = ibDraw;
  this->ComposeSurfaceScalarsAndShow();
}

void vtkKWQdecWindow::SetSurfaceScalarsColorMin(const char *isMin) {

  double            value;
  std::stringstream ssMin(isMin);
  ssMin >> value;

  this->SetSurfaceScalarsColorMin(value);
}

void vtkKWQdecWindow::SetSurfaceScalarsColorMid(const char *isMid) {
#if USE_MID
  double            value;
  std::stringstream ssMid(isMid);
  ssMid >> value;
...
void vtkKWQdecWindow::SetSurfaceScalarsColorMax(const char *isMax) {

  double            value;
  std::stringstream ssMax(isMax);
  ssMax >> value;

  this->SetSurfaceScalarsColorMax(value);
}

void vtkKWQdecWindow::SetSurfaceScalarsColorReverse(int ibReverse) {

  mbSurfaceScalarsColorReverse = ibReverse;
  this->ComposeSurfaceScalarsAndShow();
}

void vtkKWQdecWindow::SetSurfaceScalarsColorShowPositive(int ibShow) {

  mbSurfaceScalarsColorShowPositive = ibShow;
  this->ComposeSurfaceScalarsAndShow();
}

void vtkKWQdecWindow::SetSurfaceScalarsColorShowNegative(int ibShow) {

  mbSurfaceScalarsColorShowNegative = ibShow;
  this->ComposeSurfaceScalarsAndShow();
}

vtkKWPushButton *vtkKWQdecWindow::MakeToolbarButton(
    vtkKWToolbar *iToolbar, const char *isBalloonHelpText,
    vtkObject *iCommandObject, const char *isCommand, const char *isIconKey) {

  if (NULL == iToolbar)
    throw std::runtime_error("MakeToolbarButton: iToolbar was NULL");
...
vtkKWCheckButton *vtkKWQdecWindow::MakeToolbarCheckButton(
    vtkKWToolbar *iToolbar, const char *isBalloonHelpText,
    vtkObject *iCommandObject, const char *isCommand, const char *isIconKey) {

  if (NULL == iToolbar)
    throw std::runtime_error("MakeCheckToolbarButton: iToolbar was NULL");
...
void vtkKWQdecWindow::AddSpacerToToolbar(vtkKWToolbar *iToolbar, int iWidth) {

  // Create a frame of the specified width inside the toolbar.
  vtkSmartPointer<vtkKWFrame> spacer = vtkSmartPointer<vtkKWFrame>::New();
...
vtkKWQdecWindow::MenuItem::MenuItem() : mMenu(NULL), mnItem(-1) {}

void vtkKWQdecWindow::MenuItem::MakeCommand(
    vtkKWMenu *iMenu, int inItem, const char *isText, vtkObject *iCommandObject,
    const char *isCommand, const char *isAccelerator, const char *isIconKey) {
  ...
  int nIndex = iMenu->InsertCommand(inItem, isText, iCommandObject, isCommand);

  if (-1 == nIndex)
    throw std::runtime_error("Couldn't create menu item");
...
void vtkKWQdecWindow::MenuItem::MakeCheckButton(
    vtkKWMenu *iMenu, int inItem, const char *isText, vtkObject *iCommandObject,
    const char *isCommand, const char *isAccelerator, const char *isIconKey) {
...
void vtkKWQdecWindow::MenuItem::SetStateToDisabled() {

  if (NULL == mMenu.GetPointer())
    throw std::runtime_error("MenuItem::SetStateToDisabled: mMenu was NULL");
  if (-1 == mnItem)
    throw std::runtime_error("MenuItem::SetStateToDisabled: mnItem was -1");

  mMenu->SetItemStateToDisabled(mnItem);
}

void vtkKWQdecWindow::MenuItem::SetStateToNormal() {

  if (NULL == mMenu.GetPointer())
    throw std::runtime_error("MenuItem::SetStateToNormal: mMenu was NULL");
  if (-1 == mnItem)
    throw std::runtime_error("MenuItem::SetStateToNormal: mnItem was -1");

  mMenu->SetItemStateToNormal(mnItem);
}

void vtkKWQdecWindow::MenuItem::SetSelectedState(int ibOn) {

  if (NULL == mMenu.GetPointer())
    throw std::runtime_error("MenuItem::SetSelectedState: mMenu was NULL");
  if (-1 == mnItem)
    throw std::runtime_error("MenuItem::SetSelectedState: mnItem was -1");

  mMenu->SetItemSelectedState(mnItem, ibOn);
}

int vtkKWQdecWindow::MenuItem::GetSelectedState() {

  if (NULL == mMenu.GetPointer())
    throw std::runtime_error("MenuItem::GetSelectedState: mMenu was NULL");
  if (-1 == mnItem)
    throw std::runtime_error("MenuItem::GetSelectedState: mnItem was -1");

  return mMenu->GetItemSelectedState(mnItem);
}

int vtkKWQdecWindow::MenuItem::GetIndex() const { return mnItem; }
