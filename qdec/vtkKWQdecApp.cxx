...
vtkKWQdecApp::vtkKWQdecApp() : vtkKWApplication() {

  // Set us up with some global styles.
  this->GetOptionDataBase()->AddEntry("vtkKWWidget", "SetBackgroundColor",
                                      "1 1 1");
  this->GetOptionDataBase()->AddFontOptions("helvetica 12");
...
int vtkKWQdecApp::Exit() { return this->Superclass::Exit(); }

void vtkKWQdecApp::AddAboutText(ostream &os) {
...
void vtkKWQdecApp::LoadDataTable(const char *ifnDataTable) {
  if (mWindow.GetPointer())
    mWindow->LoadDataTable(ifnDataTable);
}

void vtkKWQdecApp::LoadProjectFile(const char *ifnProject) {
  if (mWindow.GetPointer())
...
void vtkKWQdecApp::LoadGDFFile(const char *ifnGDFFile) {
  if (mWindow.GetPointer())
    mWindow->LoadGDFFile(ifnGDFFile);
}

void vtkKWQdecApp::LoadSurfaceScalars(const char *ifnScalars) {
  if (mWindow.GetPointer())
    mWindow->LoadSurfaceScalars(ifnScalars);
}

void vtkKWQdecApp::LoadSurfaceCurvatureScalars(const char *ifnScalars) {
  if (mWindow.GetPointer())
    mWindow->LoadSurfaceCurvatureScalars(ifnScalars);
}

void vtkKWQdecApp::LoadAnnotation(const char *ifnAnnotation) {
  if (mWindow.GetPointer())
    mWindow->LoadAnnotation(ifnAnnotation);
}

void vtkKWQdecApp::LoadSurfaceOverlayScalars(const char *ifnScalars,
                                             const char *ifnColors) {
  if (mWindow.GetPointer())
    mWindow->LoadSurfaceOverlayScalars(ifnScalars, ifnColors);
}

void vtkKWQdecApp::LoadLabel(const char *ifnLabel) {
  if (mWindow.GetPointer())
    mWindow->LoadLabel(ifnLabel);
}

void vtkKWQdecApp::SetAverageSubject(const char *isAvgSubj) {
  if (mWindow.GetPointer())
    mWindow->SetAverageSubject(isAvgSubj);
}

void vtkKWQdecApp::ErrorMessage(const char *isMessage) {
...
void vtkKWQdecApp::DisplayHelpDialog(vtkKWTopLevel *iTop) {
