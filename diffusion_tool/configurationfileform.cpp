...
ConfigurationFileForm::ConfigurationFileForm(QDialog *parent)
    : QDialog(parent) {
  ui.setupUi(this);
...
void ConfigurationFileForm::loadDefaults(QString fileName) {
  ...
  QFileInfoList list = parent_directory.entryInfoList(QDir::Files | QDir::Dirs |
                                                      QDir::NoDotAndDotDot);
    ...
    if (temp_list[i].isFile()) {
      if (temp_list[i].fileName().length() >= 5) {
        ...
        if (end_of_file == ".trk") {
          ...
          QTextStream(stdout)
              << temp_list[i].absoluteFilePath() << " was just rejected with "
              << end_of_file << " being the remainder." << endl;
        }
      }
    }
  }
...
void ConfigurationFileForm::on_checkBox_26_stateChanged(int state) {
  if (state == 2) {
    ...
    if (!replace_subject_dcm() || !replace_subject_bval() ||
        !replace_subject_bvec()) {
      QMessageBox msgBox;
      msgBox.setText("warning: subject name was not found in one or more of "
                     "the dcm, bval, and bvec filepaths");
      msgBox.exec();
    }
  }
...
void ConfigurationFileForm::on_pushButton_clicked(bool checked) {
  ...
  my_text += ("1. trac-all -prep -c " + ui.dtrootLineEdit->text() +
              "/config.tutorial\n\n");
  my_text += ("2. trac-all -bedp -c " + ui.dtrootLineEdit->text() +
              "/config.tutorial\n\n");
  my_text += ("3. trac-all -path -c " + ui.dtrootLineEdit->text() +
              "/config.tutorial\n\n");
...
void ConfigurationFileForm::on_pushButton_2_clicked(bool checked) {
  ...
  /*else if (!validTrk(trk_info))
  {
    ...
    this->setMaximumHeight(1051);
    QString my_text = "Lines to Run Via Terminal: \n\n";
    my_text += ("1. AnatomiCuts -s " + ui.testingLineEdit->text() + " -f " +
                ui.testingLineEdit_2->text() + " -l " +
                ui.comboBox_2->currentText()[0] + " -c " +
                QString::number(ui.spinBox_19->value()) + " -n " +
                QString::number(ui.spinBox_20->value()) + " -e " +
                QString::number(ui.spinBox_21->value()) + " -labels -o " +
...
void ConfigurationFileForm::on_buttonBox_2_rejected() { this->close(); }
...
QString ConfigurationFileForm::bvecSeeker(QFileInfoList list) {
  ...
  for (int i = 0; i < list.size(); ++i) {
    current = list[i];
    if (current.isFile()) {
      if (current.fileName() == file_root + ".bvecs" ||
          current.fileName() == file_root + ".voxel_space.bvals") {
...
QString ConfigurationFileForm::bvalSeeker(QFileInfoList list) {
  ...
  for (int i = 0; i < list.size(); ++i) {
    current = list[i];
    if (current.isFile()) {
      if (current.fileName() == file_root + ".bvals" ||
          current.fileName() == file_root + ".voxel_space.bvecs") {
        return current.filePath();
      }
    }
  }
...
void ConfigurationFileForm::writeFile() {
  ...
  if (file.open(QIODevice::ReadWrite)) {
    ...
    stream << "# FreeSurfer SUBJECTS_DIR" << endl
           << "# T1 images and FreeSurfer segmentations are expected to be "
              "found here"
           << endl
           << "#" << endl
           << "setenv SUBJECTS_DIR " << original_parent_path << endl;
    stream << endl
           << "# Output directory where trac-all results will be saved" << endl
           << "# Default: Same as SUBJECTS_DIR" << endl
           << "#" << endl
           << "set dtroot = " << ui.dtrootLineEdit->text() << endl;
    ...
    stream << endl
           << "# Diffusion gradient tables (if there is a different one for "
              "each scan)"
           << endl
           << "#" << endl
           << "set bveclist = ( ";
    ...
    stream << endl
           << "# Diffusion b-value tables (if there is a different one for "
              "each scan)"
           << endl
           << "#" << endl
           << "set bvallist = ( ";
    ...
    stream << endl
           << "set doeddy = " << ui.checkBox->isChecked() << endl
           << endl
           << "set dorotbvecs = " << ui.checkBox_2->isChecked() << endl
           << endl
           << "set usemaskanat = " << ui.checkBox_3->isChecked() << endl;
    stream << endl
           << "set thrbet = " << ui.doubleSpinBox_2->value() << endl
           << endl
           << "set doregflt = " << ui.checkBox_4->isChecked() << endl
           << "set doregbbr = " << ui.checkBox_5->isChecked() << endl
           << endl
           << "set doregmni = " << ui.checkBox_7->isChecked() << endl
           << "set doregcvs = " << ui.checkBox_6->isChecked() << endl;
...
void ConfigurationFileForm::matcher(QDir directory, QString pattern,
                                    QVector<QString> &vector) {
  QFileInfoList directory_contents =
  ...
  for (int i = 0; i < directory_contents.length(); i++) {
    if (directory_contents[i].isDir()) {
      ...
      // QTextStream(stdout) << "the name of this directory is " <<
      ...
      // QTextStream(stdout) << "about to peep the contents of " <<
      // directory_contents[i].absoluteFilePath() << endl;
