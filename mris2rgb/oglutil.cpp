...
void OGLUsetLightingModel(float lite0, float lite1, float lite2, float lite3,
  ...
  if (lite1 < 0.0)
    lite1 = light1_diffuse[0];
  if (lite2 < 0.0)
    lite2 = light2_diffuse[0];
  if (lite3 < 0.0)
    lite3 = light3_diffuse[0];
  if (newoffset < 0.0)
    newoffset = offset;
  else
    offset = newoffset;
  ...
  mat0_emission[0] = mat0_emission[1] = mat0_emission[2] =
      (GLfloat)0.1 * newoffset;
    ...
    if (!mris->faces[k].ripflag) {
      ...
      for (n = 0; n < VERTICES_PER_FACE; n++) {
        ...
        if (flags & COORD_FLAG && getenv("THIN_LINES") == nullptr) {
          ...
          itheta = nint((DEGREES(v->theta) / oglu_coord_spacing)) *
                   oglu_coord_spacing;
          iphi =
...
#define BRIGHTNESS brightness
          ...
          if (flags & VAL_FLAG && (fabs(v->val) >= fthresh)) {
            ...
            green = BRIGHTNESS *
                    (MIN_GRAY * (1.0f - abs_color_val) + MAX(0, -color_val));
            blue = BRIGHTNESS * (MIN_GRAY * (1.0f - abs_color_val));
  ...
  if (flags & COORD_FLAG && getenv("THIN_LINES") != nullptr) {
    ...
    for (k = 0; k < mris->nfaces; k++) {
      ...
      for (cno = 0; cno < 2; cno++) /* once per coordinate */
      {
        ...
        for (fvno = 0; nintersections < 25 && fvno < VERTICES_PER_FACE;
             fvno++) {
          coorda = fcoords[fvno];
          ...
          if (nintersections >= 2) /* draw the line */
          {
            ...
            glNormal3fv(v1);
            load_brain_coords(cross_x[0] + cheight * nx,
                              cross_y[0] + cheight * ny,
                              cross_z[0] + cheight * nz, v1);
            glVertex3fv(v1);
            load_brain_coords(cross_x[1] + cheight * nx,
                              cross_y[1] + cheight * ny,
...
#define LINE_LEN 0.1 * MAX(MAX(mris->xhi, mris->yhi), mris->zhi)
  ...
  for (i = 0; i < 100 - 1; i++) {
    ...
    v1[1] = FOV * SCALE_FACTOR *
            (COLSCALEBAR_YPOS + COLSCALEBAR_HEIGHT * (i / (100 - 1.0) - 0.5));
    ...
    v1[1] =
        FOV * SCALE_FACTOR *
        (COLSCALEBAR_YPOS + COLSCALEBAR_HEIGHT * ((i + 1) / (100 - 1.0) - 0.5));
    glVertex3fv(v1);
    v1[0] = FOV * SCALE_FACTOR * (COLSCALEBAR_XPOS - COLSCALEBAR_WIDTH / 2);
    glVertex3fv(v1);
    glEnd();
...
static int set_stat_color(float f, float *rp, float *gp, float *bp,
  ...
  if (fabs(f) > fthresh && fabs(f) < fmid) {
    ...
    if (fcurv != 1.0)
      ...
      c2 = 0;
    ftmp = fcurv * c1 * SQR(ftmp - fthresh) +
           c2 * (1 - fcurv) * (ftmp - fthresh) + fthresh;
    f = (f < 0) ? -ftmp : ftmp;
  }
...
static void draw_colscalebar() {
  ...
  for (i = 0; i < NSEGMENTS - 1; i++) {
    ...
    */
    ...
    v[1] =
        fov * sf *
        (colscalebar_ypos + colscalebar_height * (i / (NSEGMENTS - 1.0) - 0.5));
    v[2] = fov * sf * 9.99;
    glVertex3fv(v);
    v[0] = fov * sf * (colscalebar_xpos + colscalebar_width / 2);
    glVertex3fv(v);
    v[1] = fov * sf *
           (colscalebar_ypos +
            colscalebar_height * ((i + 1) / (NSEGMENTS - 1.0) - 0.5));
    glVertex3fv(v);
    v[0] = fov * sf * (colscalebar_xpos - colscalebar_width / 2);
    glVertex3fv(v);
    glEnd();
