...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris2rgb.c,v 1.38 2011/03/02 00:04:26 nicks Exp $",
      "$Name:  $");
  ...
  for (ino = 1; ino < argc - 1; ino++) {
    ...
    is_flat = (strstr(name, ".flat") != nullptr || strstr(name, ".patch"));
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    fprintf(stderr, "drawing %d coordinate lines, with thickness %2.2f\n",
    ...
    case 'V':
      if (nmarked >= MAX_MARKED - 1)
        ErrorExit(ERROR_NOMEMORY, "%s: too many vertices marked (%d)", Progname,
                  nmarked);
...
static void xinit() {
  ...
  context = glXCreateContext(display, visual, nullptr, False);
  if (!context)
...
static void grabPixels(unsigned int width, unsigned int height,
  ...
  glReadPixels(0, 0, width, height, GL_GREEN, GL_UNSIGNED_SHORT,
               (GLvoid *)green);
