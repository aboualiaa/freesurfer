...
static MRI *warpReplace(TRANSFORM *trx, MRI *mri_src, const MRI *mri_dst,
                        int interp_method);
static MRI *MRIfuseSegmentations(MRI *mri_in, std::vector<MRI *> mri_asegs,
...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < num_tp; i++) {
    ...
    mri_nocc_asegs[i] =
        warpReplace(trx, mri_nocc_asegs[i], mri_in, SAMPLE_NEAREST);
      ...
      for (int z = 0; z < mri_fused->depth; z++) {
        ...
        case CC_Anterior:
          ...
          for (zk = -1; zk <= 1; zk++) {
            ...
            for (yk = -1; yk <= 1; yk++) {
              ...
              for (xk = -1; xk <= 1; xk++) {
                ...
                if (lbl == Left_Cerebral_White_Matter)
                  leftwm++;
                else if (lbl == Right_Cerebral_White_Matter)
                  rightwm++;
              }
            }
          }
          ...
          if (leftwm > rightwm) {
            ...
            MRIsetVoxVal(mri_fused, x, y, z, frame,
