...
int main(int argc, char *argv[]) {
  ...
  if (!mris_lh)
    ErrorExit(ERROR_NOFILE, "%s: could not read lh surface file %s", Progname,
              lh_surf_fname);

  fprintf(stderr, "reading rh surface from %s...\n", rh_surf_fname);
  mris_rh = MRISread(rh_surf_fname);
  if (!mris_rh)
    ErrorExit(ERROR_NOFILE, "%s: could not read rh surface file %s", Progname,
              rh_surf_fname);
  ...
  if (MRISreadOriginalProperties(mris_lh, orig_name) != NO_ERROR)
    ErrorExit(ERROR_NOFILE, "%s: could not read lh original properties from %s",
              Progname, orig_name);
  if (MRISreadCanonicalCoordinates(mris_lh, canon_name) != NO_ERROR)
    ErrorExit(ERROR_BADFILE, "%s: could not read lh canon surface %s", Progname,
              canon_name);
       ...
       h++) // register left to right and right to left
  {
    ...
    for (sno = 0; sno < SURFACES; sno++) {
      ...
      {
        int req = snprintf(fname, STRLEN, "%s/%s.%s", surf_dir, template_hemi,
                           curvature_names[sno]);
        if (req >= STRLEN) {
      ...
      MRIStoParameterization(mris_template, mrisp_template, scale,
       ...
       h++) // register left to right and right to left
  {
    ...
    for (vno = 0; vno < mris_mov->nvertices; vno++) {
      ...
      float x = (v->cx + coords[0][vno]) / 2, y = (v->cy + coords[1][vno]) / 2,
            z = (v->cz + coords[2][vno]) / 2;

      float norm = sqrt(x * x + y * y + z * z);

      MRISsetXYZ(mris_mov, vno, (x / norm) * radius, (y / norm) * radius,
                 (z / norm) * radius);
    }
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    strcpy(const_cast<char *>(curvature_names[0]),
           fname); // strcpy???? And the const_cast is.... ugly
    parms.flags |= IP_USE_INFLATED;
