...
int main(int argc, char *argv[]) {
  ...
  write_roc_curve(mris, labels, mri_overlays, min_area, dilate, erode,
                  out_fname, nsubjects);
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <overlay name> <segmentation label name> "
         "<subject 1>... <roc file>",
         Progname);
  exit(code);
}
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(i, thresh, dilate, erode, nlabels, segments, min_area)        \
        shared(mris, labels) schedule(static, 1)
#endif
        for (n = 0; n < nsubjects; n++) {
      ROMP_PFLB_begin MRISthresholdValIntoMarked(mris[n], thresh);
      MRISdilateMarked(mris[n], dilate);
      MRISerodeMarked(mris[n], erode);
      MRISsegmentMarked(mris[n], &segments, &nlabels, min_area);
      compute_segmentation_stats(mris[n], labels[n], segments, nlabels, &tps[n],
                                 &tns[n], &fps[n], &fns[n]);
      for (i = 0; i < nlabels; i++)
        LabelFree(&segments[i]);
      ROMP_PFLB_end
    }
    ROMP_PF_end for (n = 0; n < nsubjects; n++) {
      fpos += fps[n];
      fneg += fns[n];
      tpos += tps[n];
      tneg += tns[n];
