...
static const int labels_overall_Dice[18] = {
    Left_Lateral_Ventricle, Right_Lateral_Ventricle, Left_Hippocampus,
    Right_Hippocampus,      Left_Thalamus_Proper,    Right_Thalamus_Proper,
    Left_Caudate,           Right_Caudate,           Left_Putamen,
    Right_Putamen,          Left_Pallidum,           Right_Pallidum,
    Left_Amygdala,          Right_Amygdala,          Third_Ventricle,
    Fourth_Ventricle,       Left_Inf_Lat_Vent,       Right_Inf_Lat_Vent};
...
int main(int argc, char *argv[]) {
  ...
  if (!mri_seg1)
    ErrorExit(ERROR_BADPARM, "%s: could not read label volume1 %s", Progname,
              argv[1]);

  mri_seg2 = MRIread(argv[2]);
  if (!mri_seg2)
    ErrorExit(ERROR_BADPARM, "%s: could not read label volume2 %s", Progname,
              argv[2]);
    ...
    for (z = 0; z < depth; z++) {
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          if (v1 > MAX_CLASS_NUM || v1 < 0 || v2 > MAX_CLASS_NUM || v2 < 0)
            continue;
  ...
  if (all_labels_flag) {
    ...
    for (i = 0; i < MAX_CLASSES; i++) {
      if (correct_ratio[i] >
          0.0) // This will include zero overlap areas as well (not just
               // non-existing labels. If it is a problem, should flag existing
               // labels....
      {
  ...
  if (all_labels_flag) {
    ...
    for (skipped = i = 0; i < num_labels; i++) {
      ...
      if (ctab == NULL)
        ...
        printf("%4d  %-30s %8.6lf %8.6lf %6d %6d %6d %9.4lf\n", j,
               ctab->entries[j]->name, correct_ratio[j], correct_ratio2[j],
               Volume_from1[j], Volume_from2[j], Volume_overlap[j], voldiff);
      if (table_fname)
        fprintf(tablefp, "%4d  %-30s %8.6lf %8.6lf %6d %6d %6d %9.4lf\n", j,
                ctab->entries[j]->name, correct_ratio[j], correct_ratio2[j],
                Volume_from1[j], Volume_from2[j], Volume_overlap[j], voldiff);
  ...
  if (log_fname != NULL) {
    ...
    fprintf(log_fp, "%6.4f \n",
            subcorvolume_overlap * 2.0 /
                (float)(subcorvolume1 + subcorvolume2));
    fclose(log_fp);
  }
