...
int main(int argc, char *argv[]) {
  ...
  make_cmd_version_string(
      argc, argv, "$Id: mris_smooth.c,v 1.30 2014/01/21 18:48:21 fischl Exp $",
      "$Name:  $", cmdline);
  ...
  if (gaussian_norm > 0) {
    ...
    do {
      for (i = start_t; i < niterations + start_t; i++) {
        ...
        for (j = 0; j < 5; j++) {
          MRISaverageVertexPositions(mris,
                                     2); // turn flat spikes into tubular ones
          MRIScomputeMetricProperties(mris);
          MRIScomputeSecondFundamentalForm(mris);
          MRIShistoThresholdGaussianCurvatureToMarked(
              mris, (float)(mris->nvertices - 20) / mris->nvertices);
        }
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help") || !stricmp(option, "-usage")) {
    ...
    case 'G':
      ...
      printf("using Gaussian curvature smoothing with norm %2.2f with %d "
             "smooth steps\n",
...
int MRIShistoThresholdGaussianCurvatureToMarked(MRI_SURFACE *mris, double pct) {
  HISTOGRAM *h;
  double min_curv, max_curv, K, bin_size, total, mode = 0.0, mode_peak, mean,
                                                 std, dmean, dmin, dmax, dsigma;
  int vno, num, b, bin_no, bin_thresh = 0, vno_min, vno_max, skipped, nvertices;

  dmean = MRIScomputeVertexSpacingStats(mris, &dsigma, &dmin, &dmax, &vno_min,
                                        &vno_max, CURRENT_VERTICES);
  min_curv = 1e8;
  max_curv = -min_curv;
