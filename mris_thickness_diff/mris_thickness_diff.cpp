...
int main(int argc, char *argv[]) {
  ...
  if (srctypestring == nullptr ||
      (out_name != nullptr && trgtypestring == nullptr)) {
  ...
  if (nSmoothSteps > 0) {
    ...
    MRISsmoothMRI(Surf1, SrcVal1, nSmoothSteps, nullptr, SrcVal1);
  }
  ...
  if (!strcmp(srctypestring, "curv")) { /* curvature file */
    ...
    SrcVal2 = MRIcopyMRIS(nullptr, Surf2, 0, "curv");
  } else if (!strcmp(srctypestring, "paint") || !strcmp(srctypestring, "w")) {
    MRISreadValues(Surf2, data2_name);
    SrcVal2 = MRIcopyMRIS(nullptr, Surf2, 0, "val");
  } else {
  ...
  if (nSmoothSteps > 0) {
    ...
    MRISsmoothMRI(Surf2, SrcVal2, nSmoothSteps, nullptr, SrcVal2);
  }
  ...
  if (register_flag && (xform_fname != nullptr)) {
  ...
  if (xform_fname != nullptr) {
    ...
    if (transform_type == MNI_TRANSFORM_TYPE ||
      ...
      if (transform_type == FSLREG_TYPE) {
        if (mri == nullptr || mri_dst == nullptr) {
          fprintf(stderr, "ERROR: fslmat does not have information on the "
                          "src and dst volumes\n");
          fprintf(stderr, "ERROR: you must give options '-src' and '-dst' "
      ...
      if (lta->xforms[0].src.valid == 0) {
        if (mri == nullptr) {
          ...
          LTAmodifySrcDstGeom(lta, mri, nullptr); // add src information
      ...
      if (lta->xforms[0].dst.valid == 0) {
        if (mri_dst == nullptr) {
          ...
          LTAmodifySrcDstGeom(lta, nullptr, mri_dst); // add  dst information
        }
      }
    } else {
    ...
    if (invert) {
      ...
      if (lt->dst.valid == 0 || lt->src.valid == 0) {
        fprintf(stderr,
                "WARNING:*************************************************\n");
        fprintf(stderr, "WARNING:dst volume infor is invalid. "
  ...
  for (index = 0; index < Surf1->nvertices; index++) {
    if (Surf1->vertices[index].border == 1)
      continue;
    if (Surf1->vertices[index].marked != 1)
      continue;
...
static void print_help() {
  print_usage();
  printf("mris_thickness_diff computes the difference of two surface data\n"
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    fprintf(stderr,
            "src volume for the given transform "
            "(given by -xform) is %s\n",
            argv[2]);
...
MRI *ComputeDifferenceNew(MRI_SURFACE *Mesh1, MRI *mri_data1,
  ...
  for (index = 0; index < Mesh1->nvertices; index++) {
    ...
    for (k = 0; k < Mesh2->vertices_topology[nnindex].num; k++) {
      ...
      facenumber =
          Mesh2->vertices_topology[nnindex].f[k]; /* index of the k-th face */
      ...
      /* Here it would be better to compute the correct distance together
                           with the closest point and then use barycentric
...
void register2to1(MRI_SURFACE *Surf1, MRI_SURFACE *Surf2) {
  ...
  double3d *closevtx2a =
      (double3d *)malloc(Surf2->nvertices * sizeof(double3d));
  ...
  while ((error_old - error_new) > 0.0001 && iter < 50) {
    ...
    for (k = 0; k < Surf2->nvertices; k++) {
      MRISsetXYZ(Surf2, k, mesh2avtx[k].x, mesh2avtx[k].y, mesh2avtx[k].z);
    }
