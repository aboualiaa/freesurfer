...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_strip_nonwhite.c,v 1.9 2011/03/02 00:04:25 nicks Exp $",
      "$Name:  $");
  ...
  if (strchr(template_fname, '#') == nullptr)
  ...
  mri_inverse_template = MRIapplyInverse3DMorph(mri_template, m3d, nullptr);
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s <input volume> <transform> <template volume> <output "
         "volume>\n",
         Progname);
  exit(code);
}
