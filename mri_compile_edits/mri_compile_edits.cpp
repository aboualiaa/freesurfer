...
int main(int argc, char *argv[]) {
  ...
  if (mri) {
    ...
    int edits = MRIsetVoxelsWithValue(mri, mri_edits, WM_EDITED_OFF_VAL,
                                      EDIT_BRAIN_OFF);
    if (edits)
      printf("Found %d OFF edits in brain.mgz\n", edits);
    nedits += edits;
    edits =
        MRIsetVoxelsWithValue(mri, mri_edits, WM_EDITED_ON_VAL, EDIT_BRAIN_ON);
    if (edits)
      printf("Found %d ON edits in brain.mgz\n", edits);
    nedits += edits;
    MRIfree(&mri);
    fflush(stdout);
  }
  ...
  if (mri) {
    ...
    if (edits)
      printf("Found %d OFF edits in wm.mgz\n", edits);
    nedits += edits;
    edits = MRIsetVoxelsWithValue(mri, mri_edits, WM_EDITED_ON_VAL, EDIT_WM_ON);
    if (edits)
      printf("Found %d ON edits in wm.mgz\n", edits);
    nedits += edits;
    MRIfree(&mri);
    fflush(stdout);
  }
  ...
  if (mri) {
    ...
    if (mri_bm_auto) {
      ...
      MRIfree(&mri_bm_auto);
      if (edits)
        printf("Found %d edits in brainmask.mgz\n", edits);
      fflush(stdout);
    }
  }
  ...
  if (mri) {
    ...
    int edits = MRIsetVoxelsWithValue(mri, mri_edits, WM_EDITED_OFF_VAL,
                                      EDIT_FINALSURFS_OFF);
    if (edits)
      printf("Found %d OFF edits in brain.finalsurfs.mgz\n", edits);
    nedits += edits;
    edits = MRIsetVoxelsWithValue(mri, mri_edits, WM_EDITED_ON_VAL,
                                  EDIT_FINALSURFS_ON);
    if (edits)
      printf("Found %d ON edits in brain.finalsurfs.mgz\n", edits);
    nedits += edits;
    MRIfree(&mri);
    fflush(stdout);
  }
  ...
  if (mri) {
    ...
    if (mri_aseg_auto) {
      ...
      if (edits)
        printf("Found %d edits in aseg.mgz\n", edits);
      fflush(stdout);
    }
  }
