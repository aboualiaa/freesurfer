...
int main(int argc, char *argv[]) {
  ...
  if (annot_name) {
    if (MRISreadAnnotation(mris, annot_name) != NO_ERROR)
      ErrorExit(ERROR_BADPARM, "%s: could not read annot file %s", Progname,
                annot_name);
  ...
  if (!FZERO(dalpha) || !FZERO(dbeta) || !FZERO(dgamma))
    MRISrotate(mris, mris, RADIANS(dalpha), RADIANS(dbeta), RADIANS(dgamma));
  ...
  if (single_surf) {
    ...
    cp = strchr(cp, '.');
    if (cp == NULL)
      ErrorExit(ERROR_NOFILE, "%s: could no scan hemi from %s", Progname,
                template_fname);
    strncpy(hemi, cp - 2, 2);
    hemi[2] = 0;
  ...
  if (MRISreadCanonicalCoordinates(mris, canon_name) != NO_ERROR)
    ErrorExit(ERROR_BADFILE, "%s: could not read canon surface %s", Progname,
              canon_name);
  ...
  if (multiframes) {
    if (use_initial_registration)
      MRISvectorRegister(mris, mrisp_template, &parms, max_passes, min_degrees,
                         max_degrees, nangles);
    ...
    if (multi_scale > 0) {
      ...
      for (i = 0; i < multi_scale; i++) {
        printf("*************** round %d, l_dist = %2.3f **************\n", i,
               parms.l_dist);
        MRISregister(mris, mrisp_template, &parms, max_passes, min_degrees,
                     max_degrees, nangles);
...
static int get_option(int argc, char *argv[]) {
  ...
  else if (!stricmp(option, "infname"))
  {
    ...
    strcpy(const_cast<char *>(curvature_names[0]),
           fname); // strcpy _and_ const_cast in a single line....
    parms.flags |= IP_USE_INFLATED;
