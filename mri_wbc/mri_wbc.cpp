...
MRI *WholeBrainCon(WBC *wbc) {
  ...
  nperthread0 =
      (long)round((double)npairs / nthreads - 1); // don't use nint(), need long
  if (nperthread0 <= 0)
    nperthread0 = 1;
  ...
  for (threadno = 0; threadno < nthreads; threadno++) {
    ...
    conth[threadno] =
        MRIallocSequence(wbc->ntot, 1, 1, MRI_FLOAT, wbc->nrholist);
    if (wbc->DoDist) {
      conSth[threadno] =
          MRIallocSequence(wbc->ntot, 1, 1, MRI_FLOAT, wbc->nrholist);
      conLth[threadno] =
...
#endif
      for (threadno = 0; threadno < nthreads; threadno++) {
    ROMP_PFLB_begin int k1, k2, t, n, thno, q, ct1, ct2;
...
int WBCprep(WBC *wbc) {
  ...
  if (wbc->lhlabel)
    wbc->lhmask = MRISlabel2Mask(wbc->lh, wbc->lhlabel, NULL);
  if (wbc->rhlabel)
    wbc->rhmask = MRISlabel2Mask(wbc->rh, wbc->rhlabel, NULL);
  ...
  if (wbc->fvol) {
    if (wbc->volmask)
      wbc->nvolmask = MRIcountAboveThreshold(wbc->volmask, 0.5);
    else
      wbc->nvolmask = wbc->fvol->width * wbc->fvol->height * wbc->fvol->depth;
  }
  if (wbc->flh) {
    if (wbc->lhmask)
      wbc->nlhmask = MRIcountAboveThreshold(wbc->lhmask, 0.5);
    else
      wbc->nlhmask = wbc->flh->width;
  }
  if (wbc->frh) {
    if (wbc->rhmask)
      wbc->nrhmask = MRIcountAboveThreshold(wbc->rhmask, 0.5);
    else
      wbc->nrhmask = wbc->frh->width;
  }

  wbc->ntot = wbc->nvolmask + wbc->nlhmask + wbc->nrhmask;
  printf("nmask %d %d %d   %d\n", wbc->nvolmask, wbc->nlhmask, wbc->nrhmask,
         wbc->ntot);
    ...
    for (s = 0; s < wbc->fvol->depth; s++) {
      for (c = 0; c < wbc->fvol->width; c++) {
        for (r = 0; r < wbc->fvol->height; r++) {
          if (wbc->volmask && MRIgetVoxVal(wbc->volmask, c, r, s, 0) < 0.5)
            continue;
  ...
  if (wbc->flh) {
    for (vtxno = 0; vtxno < wbc->flh->width; vtxno++) {
      if (wbc->lhmask && MRIgetVoxVal(wbc->lhmask, vtxno, 0, 0, 0) < 0.5)
        continue;
  ...
  if (wbc->frh) {
    for (vtxno = 0; vtxno < wbc->frh->width; vtxno++) {
      if (wbc->rhmask && MRIgetVoxVal(wbc->rhmask, vtxno, 0, 0, 0) < 0.5)
        continue;
      MRIsetVoxVal(wbc->vertexno, nthvox, 0, 0, 0, vtxno);
...
int WBCfinish(WBC *wbc) {
  ...
  if (wbc->fvol) {
    ...
    wbc->volcon = MRIallocSequence(wbc->fvol->width, wbc->fvol->height,
                                   wbc->fvol->depth, MRI_FLOAT, wbc->nrholist);
  ...
  if (wbc->flh) {
    ...
    wbc->lhcon = MRIallocSequence(wbc->flh->width, wbc->flh->height,
                                  wbc->flh->depth, MRI_FLOAT, wbc->nrholist);
    MRIcopyHeader(wbc->flh, wbc->lhcon);
    MRIcopyPulseParameters(wbc->flh, wbc->lhcon);
    if (wbc->DoDist) {
      wbc->lhconS = MRIallocSequence(wbc->flh->width, wbc->flh->height,
                                     wbc->flh->depth, MRI_FLOAT, wbc->nrholist);
      MRIcopyHeader(wbc->flh, wbc->lhconS);
      MRIcopyPulseParameters(wbc->flh, wbc->lhconS);
      wbc->lhconL = MRIallocSequence(wbc->flh->width, wbc->flh->height,
                                     wbc->flh->depth, MRI_FLOAT, wbc->nrholist);
  ...
  if (wbc->frh) {
    ...
    wbc->rhcon = MRIallocSequence(wbc->frh->width, wbc->frh->height,
                                  wbc->frh->depth, MRI_FLOAT, wbc->nrholist);
    MRIcopyHeader(wbc->frh, wbc->rhcon);
    MRIcopyPulseParameters(wbc->frh, wbc->rhcon);
    if (wbc->DoDist) {
      wbc->rhconS = MRIallocSequence(wbc->frh->width, wbc->frh->height,
                                     wbc->frh->depth, MRI_FLOAT, wbc->nrholist);
      MRIcopyHeader(wbc->frh, wbc->rhconS);
      MRIcopyPulseParameters(wbc->frh, wbc->rhconS);
      wbc->rhconL = MRIallocSequence(wbc->frh->width, wbc->frh->height,
                                     wbc->frh->depth, MRI_FLOAT, wbc->nrholist);
...
WBC *WBCtestSynth(WBC *wbc) {
  ...
  if (wbc->wbcsynth->ForceFail)
    c2 = c0 + floor(wbc->distthresh / volres) - 1;
  ...
  for (hemi = 0; hemi < 2; hemi++) {
    ...
    sprintf(tmpstr, "%s/subjects/fsaverage/surf/%s.white",
            fsenv->FREESURFER_HOME, hemistr);
    surf = MRISread(tmpstr);
    sprintf(tmpstr, "%s/subjects/fsaverage/surf/%s.inflated",
            fsenv->FREESURFER_HOME, hemistr);
    ...
    for (vno = 0; vno < surf->nvertices; vno++) {
      ...
      if (d < .9 * wbc->wbcsynth->distthresh &&
          nshort < wbc->wbcsynth->nshorttarg[hemi]) {
        for (t = 0; t < wbc->nframes; t++)
      ...
      if (d > 1.1 * wbc->wbcsynth->distthresh &&
          nlong < wbc->wbcsynth->nlongtarg[hemi]) {
        for (t = 0; t < wbc->nframes; t++)
          MRIsetVoxVal(func, vno, 0, 0, t, wf[t]);
        MRIsetVoxVal(mask, vno, 0, 0, 0, 1);
        //printf("%s long %d %d\n",hemistr,nlong,vno);
