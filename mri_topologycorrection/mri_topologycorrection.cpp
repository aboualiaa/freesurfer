...
static int resegment_erased_voxels(MRI *mri_T1, MRI *mri_in, MRI *mri_out,
                                   int label);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!strcmp(option, "nothing")) {
    ...
    fprintf(stderr,
            "Mode:          Making corrections on the background first\n");
    nargs = 0;
  } else if (!strcmp(option, "background")) {
    parms.background_priority = 1;
    fprintf(stderr,
            "Mode:          Making corrections on the foreground first\n");
    nargs = 0;
  } else if (!strcmp(option, "only")) {
    parms.only = 1;
    fprintf(stderr, "Mode:          Making corrections on the "
    ...
    fprintf(stderr,
            "Mode:          Cost computed from the sum of the voxel prob\n");
    nargs = 0;
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    fprintf(stderr,
            "\nUsage: %s options input_orig_file input_segmented_file "
            "output_folder\n",
            Progname);
    exit(1);
  };
  ...
  for (n = 0; n < parms.nlabels; n++)
    fprintf(stderr, "label = %d: %s \n", parms.labels[n],
            cma_label_to_name(parms.labels[n]));
  ...
  mri_out = MRIcorrectTopology(mri_orig, mri_seg, nullptr, &parms);
  ...
  if (parms.nlabels == 1) {
    ...
    */
    ...
    mri_tmp = MRIcopy(mri_seg, nullptr);
