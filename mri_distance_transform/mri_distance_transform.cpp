...
int main(int argc, char *argv[]) {
  ...
  fprintf(stderr, "mri_distance_transform <input volume> <label> "
                  "<max_distance> <mode[=1]> <output volume>\n");
  fprintf(stderr, "mode : 1 = outside , mode : 2 = inside , mode : 3 = both, "
                  "mode : 4 = both unsigned \n");
  ...
  if (surf_name) {
    ...
    if (mris == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read surface from %s", Progname,
                surf_name);
    mri_white = MRIclone(mri, nullptr);
    ...
    if (mri_thresh != nullptr) // remove voxels that are in the ventricles
    {
      int csf_vox = 0;

      mri_csf = MRIclone(mri_thresh, nullptr);
    ...
    mri_aseg = MRIdilate(mri_white, nullptr);
    for (x = 0; x < mri_aseg->width; x++)
      for (y = 0; y < mri_aseg->height; y++)
        for (z = 0; z < mri_aseg->depth; z++) {
  ...
  mri_distance = MRIextractDistanceMap(mri, mri_distance, label, max_distance,
                                       mode, mri_white);
  ...
  if (!stricmp(option, "wm")) {
    ...
    if (mri_thresh == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read threshold volume %s",
                Progname, argv[2]);
    wthresh = atof(argv[3]);
    printf("threshing interior volume %s at %2.0f to remove ventricles from "
           "mask\n",
           argv[2], wthresh);
...
#define MAX_STEPS (1000 / STEP_SIZE)
        ...
        do {
          ...
          // take next step
          MRIsampleVolumeGradient(mri_distance, xf, yf, zf, &dx, &dy, &dz);
          xf -= STEP_SIZE * dx;
          yf -= STEP_SIZE * dy;
          zf -= STEP_SIZE * dz;
