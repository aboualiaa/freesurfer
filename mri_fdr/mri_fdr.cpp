...
int main(int argc, char *argv[]) {
  ...
  for (n = 0; n < cmdargs->ninputs; n++) {
    if (cmdargs->framelist[n] < 0)
      cmdargs->framelist[n] = cmdargs->defaultframe;
    ...
    if (cmdargs->outputlist[n]) {
      outputlist[n] =
          MRIallocSequence(inputlist[n]->width, inputlist[n]->height,
                           inputlist[n]->depth, MRI_FLOAT, 1);
      if (outputlist[n] == nullptr)
        exit(1);
      MRIcopyHeader(inputlist[n],
                    outputlist[n]); /* does not change dimensions */
      MRIcopyPulseParameters(inputlist[n], outputlist[n]);
    }
  }

  err = MRIfdr2vwth(inputlist, cmdargs->ninputs, cmdargs->framelist,
                    cmdargs->fdr, cmdargs->signid, cmdargs->log10flag, masklist,
                    &vthresh, outputlist);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      cmdargs->signid = 0;
    else if (!strcasecmp(option, "--no-log10p"))
      cmdargs->log10flag = 0;
      ...
      if (CMDnthIsArg(nargc, pargv, 1)) {
        ...
        if (CMDnthIsArg(nargc, pargv, 2)) {
          if (strcmp(pargv[2], "nooutput") != 0)
            cmdargs->outputlist[k] = pargv[2];
          nargsused++;
...
static void print_usage() {
  printf("USAGE: %s \n", Progname);
  printf("\n");
  printf("   --i input <mask> <output> <frame>: source volume or surface "
         "overlay\n");
  printf("     --i input <mask> <output> <frame>: another source volume or "
         "surface overlay\n");
...
static void print_help() {
  ...
  printf("Outputs are optional (in which case you probably just want the "
         "threshold)\n");
  printf("  You can capture the threshold in a text file with --thfile\n");
  printf("The input is assumed to be -log10(p). If that is not the case then "
         "--no-log10p\n");
  printf("Output threshold will be -log10(p) unless --no-log10p\n");
  printf("If you want to spec an output but not a mask,  then set the mask "
         "file to 'nomask'\n");
  printf("If you want to spec a frame but not an output, then set the output "
         "file to 'nooutput'\n");
  printf("\n");
  printf(
      "  Thresholding of Statistical Maps in Functional Neuroimaging Using\n");
  printf("  the False Discovery Rate.  Christopher R. Genovese, Nicole A. "
         "Lazar,\n");
