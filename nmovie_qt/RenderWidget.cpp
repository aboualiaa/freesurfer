...
void RenderWidget::paintEvent(QPaintEvent *) {
  ...
  if (m_bAutoResize) {
    ...
    if (image.isNull()) {
      image = m_images[m_nCurrentImageIndex].scaled(
          rc.size(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
      m_resizedImages[m_nCurrentImageIndex] = image;
    }

    p.drawImage((rc.width() - image.width()) / 2,
                (rc.height() - image.height()) / 2, image);
  } else
...
void RenderWidget::OnStop() { m_timerRender.stop(); }
