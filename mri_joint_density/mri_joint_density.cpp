...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_joint_density.c,v 1.6 2011/03/02 00:04:20 nicks Exp $",
      "$Name:  $");
  ...
  mri1 = MRIread(argv[1]);
  if (!mri1)
    ErrorExit(ERROR_NOFILE, "%s: could not read MR volume 1 from %s", Progname,
              argv[1]);
  printf("reading volume 2 from %s...\n", argv[2]);
  mri2 = MRIread(argv[2]);
  if (!mri2)
    ErrorExit(ERROR_NOFILE, "%s: could not read MR volume 2 from %s", Progname,
              argv[2]);
  ...
  mri_nonbrain = MRIand(mri1, mri2, nullptr, 1);
  MRIbinarize(mri_nonbrain, mri_nonbrain, 1, 128, 0);
  mri_tmp = MRIallocSequence(mri1->width, mri1->height, mri1->depth, MRI_UCHAR,
                             mri1->nframes);
  ...
  fp = fopen(argv[3], "w");
  if (fp == nullptr)
    ErrorExit(ERROR_NOFILE, "%s: could not open output file %s", Progname,
              argv[3]);
  fprintf(fp, "nbins = %d;\n cfmin = %f ;\n cfmax = %f  ; \nfstep = %f;\n",
