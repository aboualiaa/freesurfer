...
(
    ...
    study_list,
    subject_name_list,
) = load_timepoints(bdir, target_shape, ndilations=1)

print("%d training subjects found" % len(vol_list))


def BatchGenerator(
    vol_list, dtrans_list, batch_size=4, return_warp=False, bidir=False
):
    ...
    batch_dtrans_source = np.zeros(
        tuple([batch_size]) + im0.shape[0:2] + tuple([d0.shape[2]])
    )
    batch_dtrans_target = np.zeros(
        tuple([batch_size]) + im0.shape[0:2] + tuple([d0.shape[2]])
    )
    found = 0
    while True:
            ...
            if return_warp == True:
                ...
                if bidir == True:
                    ...
                    yield (
                        ...
                        [batch_target, batch_dtrans_target],
                    )
            found = 0


def BatchGeneratorSurf(
    vol_list,
    seg_list,
    dtrans_list,
    npoints,
    ...
    batch_dtrans_src = np.zeros(
        tuple([batch_size]) + im0.shape[0:2] + tuple([d0.shape[2]])
    )
    batch_dtrans_trg = np.zeros(
        tuple([batch_size]) + im0.shape[0:2] + tuple([d0.shape[2]])
    )
...
scale = 1
print(
    "running on host %s, GPU %d, train_affine %s"
    % (host, gpu_number, str(train_affine))
)
...
npoints = 5000
model = vxm.networks.VxmAffineSurfaceSemiSupervised(
    target_shape,
    enc_nf_affine,
    transform_type=transform_type,
    nb_labels_sample=nb_labels,
    nb_surface_points=npoints,
    bidir=bidir,
    blurs=affine_blurs,
)
...
affine_wt_fname = "cxr.vxm.mpr.%s.%s.sym.%s.train_affine.%s.h5" % (
    transform_type,
    which_loss,
    str(symmetrize),
    str(train_affine),
...
callbacks = [
    ModelSaver(model, 100, affine_wt_fname, cp_iters=10),
    reduce_lr,
...
affine_model.compile(
    optimizer=keras.optimizers.Adam(lr=0.1 * learning_rate),
    loss=losses,
    loss_weights=[0.00001, 1, 1],
)
initial_epoch = 0
epochs = initial_epoch + 50
fithistr = affine_model.fit_generator(
    bg,
...
if 1:
    ...
    fv.show()
    affine_pred_model = keras.models.Model(
        affine_model.inputs, affine_model.references.affines[0]
    )
...
nl_wt_fname = "cxr.vxm.mpr.%s.%s.sym.%s.train_affine.%s.nl.h5" % (
    transform_type,
    which_loss,
    str(symmetrize),
    str(train_affine),
...
smoothness_weight = 0.001 * 100
model.compile(
    optimizer=keras.optimizers.Adam(lr=0.1 * learning_rate),
    loss=losses,
    loss_weights=[1, smoothness_weight],
)

bg_nl = BatchGenerator(
    vol_list, dtrans_list, batch_size=batch_size, return_warp=True, bidir=False
)

nl_epochs = 50
initial_epoch = 0
epochs = initial_epoch + nl_epochs
fithistr = model.fit_generator(
    bg_nl,
    steps_per_epoch=steps_per_epoch,
...
if 1:
    ...
    if which_loss == "tukey":
        name = "pred.%s.%s.sym.%s.affine.%s" % (
            transform_type,
            str(which_loss) + str(tukey_c),
            str(symmetrize),
            str(train_affine),
        )
    else:
