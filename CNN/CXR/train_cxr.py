...
affine_blurs = [1]
bidir = False
model = vxm.networks.VxmAffineDense(
    target_shape,
    enc_nf,
    dec_nf,
...
affine_model.compile(
    optimizer=keras.optimizers.Adam(lr=0.1 * learning_rate), loss=aloss
)
initial_epoch = 50
...
if 1:
    ...
    if which_loss == "tukey":
        name = "pred.rigid.%s.%s.sym.%s.affine.%s" % (
            str(rigid),
