/**
 * @file  mri_gtmpvc.c
 * @brief Peforms Partial volume correction based on the geometric transfer
 * matrix GTM
 *
 *
 * Implementation of geometric transfer matrix (GTM). Also includes
 * Muller-Gartner (MG) and Region-based Voxelwise (RBV) partial volume
 * correction.
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  if (MergeHypos && gtm->ctGTMSeg->entries[77] == nullptr) {
  ...
  if (DoRegHeader)
    gtm->anat2pet = TransformRegDat2LTA(gtm->anatseg, gtm->yvol, nullptr);
  VOL_GEOM vg;
  ...
  if (gtm->nReplace > 0) {
    printf("Replacing %d\n", gtm->nReplace);
    fflush(stdout);
    mritmp = MRIreplaceList(gtm->anatseg, gtm->SrcReplace, gtm->TrgReplace,
                            gtm->nReplace, nullptr, nullptr);
    if (Gdiag_no > 0)
  ...
  if (ttReduce > 0) {
    ...
    ttseg = MRIseg2TissueType(gtm->anatseg, gtm->ctGTMSeg, nullptr);
    if (ttseg == nullptr)
      exit(1);
    MRIfree(&gtm->anatseg);
    gtm->anatseg = ttseg;
  ...
  if (MaskVolFile) {
    ...
    gtm->mask = nullptr; // should already be NULL
  ...
  if (gtm->reduce_fov && gtm->mask) {
    ...
    printf("region %d %d %d reduced to ", gtm->yvol->width, gtm->yvol->height,
           gtm->yvol->depth);
    REGIONprint(stdout, gtm->automaskRegion);
    fflush(stdout);
    masktmp = MRIextractRegion(gtm->mask, nullptr, gtm->automaskRegion);
    yvoltmp = MRIextractRegion(gtm->yvol, nullptr, gtm->automaskRegion);
    ...
    if (LTAmriIsSource(gtm->anat2pet, gtm->yvol))
      ...
      lta = LTAcopy(gtm->anat2pet, nullptr);
    anat2bbpet = LTAconcat2(lta, gtm->pet2bbpet, 1);
    LTAfree(&lta);
    gtm->yvol_full_fov = MRIallocHeader(gtm->yvol->width, gtm->yvol->height,
                                        gtm->yvol->depth, MRI_FLOAT, 1);
    ...
    gtm->pet2bbpet =
        TransformRegDat2LTA(gtm->yvol, gtm->yvol, nullptr); // identity
  ...
  if (gtm->UseMBrad)
    fprintf(logfp, "MB Rad: %g\n", gtm->mbrad->slope);
  if (gtm->UseMBtan)
    fprintf(logfp, "MB Tan: %g\n", gtm->mbtan->slope);
  fprintf(logfp, "nPad %d, PadThresh %g\n", gtm->nPad, gtm->PadThresh);
  if (gtm->DoMeltzerPVC)
    fprintf(logfp, "Meltzer: %lf %lf %d\n", gtm->MeltzerMaskThresh,
            gtm->MeltzerBinThresh, gtm->MeltzerNDil);

  for (n = 0; n < gtm->nContrasts; n++) {
    if (gtm->contrasts[n]->C->cols != gtm->nsegs) {
      printf("ERROR: contrast %d %s has %d columns, expecting %d\n", n,
             gtm->contrasts[n]->name, gtm->contrasts[n]->C->cols, gtm->nsegs);
  ...
  /* Create the "SegPVF". This is a multi-frame volume, each frame corresponds
     to a different Seg ID. The value is the PVF of that SegID. This is
     independent of the PSF (but accounts for the volume fraction effect). */
  // Note: tissue type lateralization (LateralizeTT) will sligthly affect this
  // since the tissue type is used to resolve ties.
  ...
  PrintMemUsage(logfp);
  gtm->segpvf =
      MRIseg2SegPVF(gtm->anatseg, gtm->seg2pet, segpvfresmm, gtm->segidlist,
                    gtm->nsegs, gtm->mask, 0, nullptr, gtm->segpvf);
  if (gtm->segpvf == nullptr)
    exit(1);
  if (Gdiag_no > 0)
    PrintMemUsage(stdout);
  PrintMemUsage(logfp);
  ...
  if (DoSimulation && !gtm->DoVoxFracCor) {
    ...
    printf("Turning VoxFracCor ON for simulation. It will be turned OFF for "
           "anlaysis.\n");
    fprintf(logfp, "Turning VoxFracCor ON for simulation. It will be turned "
                   "OFF for anlaysis.\n");
    gtm->DoVoxFracCor = 1;
  }
  ...
  // Create GTM pvf in pet space (why?). This is actually the fraction of each
  // tissue type
  ...
  gtm->gmpvfpsf = MRIgaussianSmoothNI(gmpvf, gtm->cStd, gtm->rStd, gtm->sStd,
                                      nullptr); // just GM
  ttpvfpsf = MRIgaussianSmoothNI(gtm->ttpvf, gtm->cStd, gtm->rStd, gtm->sStd,
  ...
  ltatmp = LTAinvert(gtm->anat2pet, nullptr);
  ...
  ltatmp = LTAinvert(gtm->pet2bbpet, nullptr);
  ...
  if (DoSimulation) {
    ...
    printf("Smoothing synthsized %g %g %g\n", SynthPSFStdCol, SynthPSFStdRow,
           SynthPSFStdSlice);
    gtm->ysynthsm = MRIgaussianSmoothNI(
    ...
    MRIfree(&gtm->yvol);
    gtm->yvol = MRIcopy(gtm->ysynthsm, nullptr);
    MRIfree(&gtm->ysynth);
    MRIfree(&gtm->ysynthsm);
    MatrixFree(&gtm->beta);
    if (!DoVoxFracCorTmp) {
      ...
      gtm->ttpvf =
          MRIsegPVF2TissueTypePVF(gtm->segpvf, gtm->segidlist, gtm->nsegs,
                                  gtm->ctGTMSeg, gtm->mask, gtm->ttpvf);
  ...
  for (c = 0; c < gtm->nsegs; c++)
    MRIsetVoxVal(mritmp, c, 0, 0, 0, gtm->vrf->rptr[c + 1][1]);
  ...
  mritmp = MRIallocSequence(gtm->nsegs, 1, 1, MRI_FLOAT, 1);
  for (c = 0; c < gtm->nsegs; c++)
    MRIsetVoxVal(mritmp, c, 0, 0, 0, gtm->nvox->rptr[c + 1][1]);
  err = MRIwrite(mritmp, tmpstr);
  if (err)
    exit(1);
  MRIfree(&mritmp);
  ...
  if (SaveInput) {
    if (gtm->rescale || gtm->DoSteadyState)
  ...
  if (yhat0File)
    MRIwrite(gtm->ysynth, yhat0File);

  printf("Freeing X0\n");
  MatrixFree(&gtm->X0);

  if (yhatFile || yhatFullFoVFile) {
    printf("Smoothing synthesized ... ");
    fflush(stdout);
    mytimer.reset();
    GTMsmoothSynth(gtm);
    printf(" %4.1f sec\n", mytimer.seconds());
  ...
  if (yhatFullFoVFile) {
    ...
    mritmp = MRIinsertRegion(gtm->ysynthsm, gtm->automaskRegion,
                             gtm->yvol_full_fov, nullptr);
    MRIwrite(mritmp, yhatFullFoVFile);
    MRIfree(&mritmp);
  }

  if (eresFile) {
    gtmres = GTMmat2vol(gtm, gtm->res, nullptr);
  ...
  if (DoRBV) {
    ...
    if (Gdiag_no > 0)
      PrintMemUsage(stdout);
    GTMrbv(gtm);
    printf("Writing output to %s ...", RBVVolFile);
    fflush(stdout);
    mytimer.reset();
    ...
    sprintf(tmpstr, "%s/aux/anat2rbv.lta", OutDir);
    LTAwrite(gtm->anat2rbv, tmpstr);
    ltatmp = LTAinvert(gtm->anat2rbv, nullptr);
  ...
  printf("Freeing y\n");
  fflush(stdout);
  mritmp = MRIcopyHeader(gtm->yvol, nullptr);
  MRIcopyPulseParameters(gtm->yvol, mritmp);
  MRIfree(&gtm->yvol);
  gtm->yvol = mritmp;
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--gtmmat"))
      DoGTMMat = 1;
    else if (!strcasecmp(option, "--no-gtmmat"))
      DoGTMMat = 0;
    else if (!strcasecmp(option, "--no-tfe"))
      gtm->DoVoxFracCor = 0;
    else if (!strcasecmp(option, "--no-vfc"))
      gtm->DoVoxFracCor = 0;
    else if (!strcasecmp(option, "--no-vox-frac"))
      gtm->DoVoxFracCor = 0;
    else if (!strcasecmp(option, "--no-vox-frac-cor"))
      gtm->DoVoxFracCor = 0;
    else if (!strcasecmp(option, "--no-gm-rvar"))
      DoGMRvar = 0;
    else if (!strcasecmp(option, "--sim-anat-seg"))
      DoSimAnatSeg = 1;
    else if (!strcasecmp(option, "--chunk"))
      setenv("FS_USE_MRI_CHUNK", "1", 1);
    else if (!strcasecmp(option, "--no-chunk"))
      ...
      nargsused = 1;
    } else if (!strcasecmp(option, "--last-frame"))
      Frame = -2;
      ...
      nargsused = 1;
    } else if (!strcasecmp(option, "--no-reduce-fov"))
      gtm->reduce_fov = 0;
    else if (!strcasecmp(option, "--reduce-fov"))
      gtm->reduce_fov = 1;
    else if (!strcasecmp(option, "--reduce-fov-eqodd"))
      gtm->reduce_fov = 2;
      ...
      if (gtm->ctGTMSeg == nullptr)
        exit(1);
      if (gtm->ctGTMSeg->ctabTissueType == nullptr) {
      ...
      gtm->ctGTMSeg = TissueTypeSchemaDefaultHead(nullptr);
    else if (!strcmp(option, "--lat"))
      LateralizeTT = 1;
    else if (!strcmp(option, "--no-lat"))
      LateralizeTT = 1;
    else if (!strcmp(option, "--update-tt"))
      UpdateTT = 1;
      ...
      GTMdefaultSegReplacmentList(&gtm->nReplace, &(gtm->SrcReplace[0]),
                                  &(gtm->TrgReplace[0]));
    else if (!strcasecmp(option, "--opt-seg-merge"))
      GTMoptSegReplacmentList(&gtm->nReplace, &(gtm->SrcReplace[0]),
                              &(gtm->TrgReplace[0]));
      ...
      if (nargc < 1)
        CMDargNErr(option, 1);
      int err =
          GTMloadReplacmentList(pargv[0], &gtm->nReplace, &(gtm->SrcReplace[0]),
                                &(gtm->TrgReplace[0]));
      if (err)
        exit(1);
      nargsused = 1;
      ...
      if (regtype == REGISTER_DAT) {
        printf("ERROR: cannot use register.dat-style registration matrix, must "
               "be LTA\n");
        exit(1);
        // gtm->anatseg2pet = ltaReadRegisterDat(regfile, SrcVolFile,
        // SegVolFile);
      }
      gtm->anat2pet = LTAread(regfile);
      if (gtm->anat2pet == nullptr) {
      ...
      if (nargc < 6)
        CMDargNErr(option, 6);
      // Note that the slice and row params are not in the order you might
      // expect Dcol Dslice Drow Rcol Rslice Rrow Translations are in mm,
      ...
      regidentity = 1;
    else if (!strcasecmp(option, "--identity"))
      regidentity = 1;
      ...
      nargsused = 3;
    } else if (!strcasecmp(option, "--opt-gm"))
      gtmopt->optmask = 1;
    else if (!strcasecmp(option, "--opt-brain"))
      gtmopt->optmask = 2;
      ...
      printf("Meltzer thresholds %lf %lf %d\n", gtm->MeltzerBinThresh,
             gtm->MeltzerMaskThresh, gtm->MeltzerNDil);
      nargsused = 3;
      ...
      m++;
      gtm->mg_refids[m] = 3204;
      m++;
      gtm->mg_refids[m] = 3205;
      m++;
      gtm->mg_refids[m] = 3206;
      m++;
      gtm->mg_refids[m] = 3207;
      m++;
      gtm->mg_refids[m] = 4201;
      m++;
      gtm->mg_refids[m] = 4203;
      m++;
      gtm->mg_refids[m] = 4204;
      m++;
      gtm->mg_refids[m] = 4205;
      m++;
      gtm->mg_refids[m] = 4206;
      m++;
      gtm->mg_refids[m] = 4207;
      m++;
      gtm->mg_refids[m] = 5001;
      m++;
      gtm->mg_refids[m] = 5002;
      m++;
      gtm->n_mg_refids = m;
    ...
    } else if (!strcasecmp(option, "--save-input"))
      SaveInput = 1;
    else if (!strcasecmp(option, "--X0"))
      SaveX0 = 1;
      ...
      SaveGTMText = 1;
    else if (!strcasecmp(option, "--save-yhat0"))
      SaveYhat0 = 1;
    else if (!strcasecmp(option, "--save-yhat-full-fov"))
      SaveYhatFullFoV = 1;
    else if (!strcasecmp(option, "--save-rbv-seg"))
      SaveRBVSeg = 1;
      ...
      gtm->nReplace++;
      gtm->SrcReplace[gtm->nReplace] = 79;
      gtm->TrgReplace[gtm->nReplace] = 77;
      gtm->nReplace++;
    } else if (!strcasecmp(option, "--merge-cblum-wm-gyri")) {
      gtm->SrcReplace[gtm->nReplace] = 690;
      gtm->TrgReplace[gtm->nReplace] = 7;
      gtm->nReplace++;
      gtm->SrcReplace[gtm->nReplace] = 691;
      gtm->TrgReplace[gtm->nReplace] = 46;
      gtm->nReplace++;
      ...
      SynthOnly = 1;
      SaveSynth = 1;
    } else if (!strcasecmp(option, "--save-synth"))
      SaveSynth = 1;
    else if (!strcasecmp(option, "--synth-save"))
      SaveSynth = 1;
      ...
      omp_set_num_threads(nthreads);
               ...
               !strcasecmp(option, "--max-threads-minus-1")) {
      nthreads = 1;
#ifdef _OPENMP
...
static void print_usage() {
  ...
  printf("\n");
  printf("   --mask volfile : ignore areas outside of the mask (in input vol "
         "space)\n");
  printf("   --auto-mask FWHM thresh : automatically compute mask\n");
  printf("   --no-reduce-fov : do not reduce FoV to encompass mask\n");
  printf("   --reduce-fov-eqodd : reduce FoV to encompass mask but force nc=nr "
         "and ns to be odd\n");
  printf(
  ...
  printf("   --merge-cblum-wm-gyri : cerebellum WM gyri back into cerebellum "
         "WM\n");
  ...
  printf("   --rescale Id1 <Id2...>  : specify reference region(s) used to "
         "rescale (default is pons)\n");
  printf("   --no-rescale   : do not global rescale such that mean of "
         "reference region is scaleref\n");
  printf("   --scale-refval refval : scale such that mean in reference region "
         "is refval\n");
  printf("   --ctab ctab : specify color table explicitly\n");
  printf("   --ctab-default : use default ctab\n");
  printf("   --tt-update : changes tissue type of VentralDC, BrainStem, and "
         "Pons to be SubcortGM\n");
  printf("     This will become the default for gtmseg in post FS6 versions\n");
  printf("   --lat : lateralize tissue types (--no-lat will turn off "
         "lateralization flag)\n");
  printf("      Mainly used with MGX to produce laterized maps (good for "
         "asymmetry studies)\n");
  printf("      Note: tissue type lateralization will sligthly affect results "
         "since \n");
  printf("      the tissue type is used to resolve ties.\n");
  printf("\n");
  printf("   --no-tfe : do not correction for tissue fraction effect (with "
         "--psf 0 turns off PVC entirely)\n");
  printf("   --segpvfres resmm : set the tissue fraction resolution parameter "
         "(def is %g)\n",
         segpvfresmm);
  printf("     if a negative number is used, then it is treated as an "
         "upsampling factor (eg, -3)\n");
  printf("     The TF is computed by dividing the PET voxel into subvoxels of "
         "size segpvfres\n");
  printf("   --rbv             : perform RBV PVC\n");
  printf("   --rbv-res voxsize : set RBV voxel resolution (good for when "
         "standard res takes too much memory)\n");
  printf("   --mg gmthresh RefId1 RefId2 ...: perform Mueller-Gaertner PVC, "
         "gmthresh is min gm pvf bet 0 and 1\n");
  printf("   --mg-ref-cerebral-wm : set MG RefIds to 2 and 41\n");
  printf("   --mg-ref-lobes-wm : set MG RefIds to those for lobes when using "
         "wm subseg\n");
  printf("   --mgx gmxthresh : GLM-based Mueller-Gaertner PVC, gmxthresh is "
         "min gm pvf bet 0 and 1\n");
  printf("   --km-ref RefId1 RefId2 ... : compute reference TAC for KM as mean "
         "of given RefIds\n");
  printf("   --km-hb  RefId1 RefId2 ... : compute HiBinding TAC for KM as mean "
         "of given RefIds\n");
  printf("   --ss bpc scale dcf : steady-state analysis spec blood plasma "
         "concentration, unit scale\n");
  printf("     and decay correction factor. You must also spec --km-ref. Turns "
         "off rescaling\n");
  printf("\n");
  printf(
  ...
  printf("   --X0 : save X0 matrix in matlab4 format as X0.mat (it will be "
         "big)\n");
  ...
  printf("   --save-yhat-with-noise seed nreps : saves yhat with noise, seed < "
         "0 for TOD\n");
  printf("   --save-yhat-full-fov : saves yhat in full FoV (if FoV was "
         "reduced)\n");
  printf("   --save-yhat0 : saves yhat prior to smoothing\n");
  printf("\n");
  printf("   --synth gtmbeta C R S MBs seed nreps : synthesize volume with "
         "gtmbeta as input\n");
  printf("       spec all other inputs the same; CRS are PSF for col, row, "
         "slice, MBs motion blur slope\n");
  printf(
  ...
  printf("   --synth-save : with --synth saves synthesized volume to "
         "outdir/synth.nii.gz\n");
  printf("   --save-text : save demeaned GTM values out to text files named "
         "after the seg.\n");
  printf("\n");
#ifdef _OPENMP
  printf("   --threads N : use N threads (with Open MP)\n");
  printf("   --max-threads : use the maximum allowable number of threads for "
         "this computer\n");
  printf("   --max-threads-minus-1 : use one less than the maximum allowable "
         "number of threads for this computer\n");
#endif
  printf("   --sd SUBJECTS_DIR\n");
  printf("   --vg-thresh thrshold : threshold for  'ERROR: LTAconcat(): LTAs 0 "
         "and 1 do not match'\n");
...
static void check_options() {
  ...
  if (!fio_FileExistsReadable(SegVolFile)) {
    ...
    sprintf(tmpstr, "%s/%s/mri/%s", SUBJECTS_DIR, gtm->anat2pet->subject,
            SegVolFile);
    if (!fio_FileExistsReadable(tmpstr)) {
  ...
  if (Frame >= 0) {
    ...
    mritmp = fMRIframe(gtm->yvol, Frame, nullptr);
    MRIfree(&gtm->yvol);
    gtm->yvol = mritmp;
  }
  ...
  if (regidentity) {
    ...
    gtm->anat2pet = TransformRegDat2LTA(gtm->yvol, gtm->yvol, nullptr);
  } else {
    if (regfile != nullptr && DoRegHeader) {
      printf("ERROR: cannot spec both regfile and --regheader\n");
      exit(1);
    }
    if (regfile == nullptr && DoRegHeader == 0) {
  ...
  if (gtm->DoMGPVC && gtm->n_mg_refids == 0) {
    printf(
        "ERROR: you must specify the ref segids for MG PVC either as extra \n"
        "options to --mg or with --mg-ref-cerebral-wm\n");
    exit(1);
  }
  if (gtm->DoSteadyState && gtm->n_km_refids == 0) {
    printf(
        "ERROR: with --ss you must specify km reference IDs with --km-ref\n");
    exit(1);
  }
...
MRI *MRIdownSmoothUp(MRI *src, int Fc, int Fr, int Fs, double cFWHM,
  ...
  if (Fc == 1 && Fr == 1 && Fs == 1)
    ...
    printf("    downsample\n");
    fflush(stdout);
    dvol = MRIdownsampleN(src, nullptr, Fc, Fr, Fs, 0);
    if (dvol == nullptr)
      return (nullptr);
...
double GTMcostPSF(GTM *gtm) {
  ...
  if (gtmopt->optmask == 1)
    return (gtm->rvargm->rptr[1][1]);
  if (gtmopt->optmask == 2)
    return (gtm->rvarbrain->rptr[1][1]);
...
float compute_powell_cost(float *pPowel) {
  ...
  if (gtmopt->gtm->cFWHM < 0)
    return (10e10);
  if (gtmopt->gtm->rFWHM < 0)
    return (10e10);
  if (gtmopt->gtm->sFWHM < 0)
    return (10e10);
  if (gtmopt->gtm->UseMBrad && gtmopt->gtm->mbrad->offset < 0)
    return (10e10);
  if (gtmopt->gtm->UseMBrad && gtmopt->gtm->mbrad->slope < 0)
    return (10e10);
  if (gtmopt->gtm->UseMBtan && gtmopt->gtm->mbtan->offset < 0)
    return (10e10);
  if (gtmopt->gtm->UseMBtan && gtmopt->gtm->mbtan->slope < 0)
    return (10e10);
  ...
  for (n = 0; n < gtmopt->nparams; n++)
    fprintf(fp, "%12.8f ", gtmopt->params[n]);
...
int MinPowell() {
  ...
  OpenPowell2(pPowel, xi, dof, gtmopt->ftol, gtmopt->linmintol,
              gtmopt->nitersmax, &gtmopt->niters, &gtmopt->fret,
              compute_powell_cost);
...
int GTMsom(GTM *gtm) {
  ...
  /* Normalize SOM(rNoPVC,cGTM) is the proportion that cGTM
     ...
     cGTM into rNoPVC. */
  if (false) { // Do normalization later
