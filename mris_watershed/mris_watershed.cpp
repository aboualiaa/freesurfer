...
static void print_help() {
  ...
  fprintf(stderr, "\t-mask_label <label>: read in and mask the input volume "
                  "that is not in the specified label\n");
  exit(1);
}
...
static int compare_vertex_vals(const void *v1, const void *v2) {
...
static VERTEX_VALUE *MRISgetSortedVertexValues(MRI_SURFACE *mris, MRI *mri,
...
static int MRISfindMostSimilarBasins(MRI_SURFACE *mris, MRI *mri, int *pb2) {
  int best_basin, vno, basin, *nbr_vertices, max_basin, nbasins, best_b1,
      best_b2, basin1;
...
static int MRISmergeMostSimilarBasin(MRI_SURFACE *mris, MRI *mri) {
...
int MRISmergeBasins(MRI_SURFACE *mris, int b1, int b2) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (v->annotation == other_basin)
      ...
      v->annotation =
          v->annotation - 1; // we removed one basin - compact the rest
  }
  return (NO_ERROR);
}
int MRISwatershed(MRI_SURFACE *mris, MRI *mri, int max_clusters,
