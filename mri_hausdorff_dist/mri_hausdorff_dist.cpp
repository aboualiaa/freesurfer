...
static void print_usage() {
  ...
  printf(
      "\t-b <thresh>        binarize input volumes with threshold <thresh>\n");
  printf("\t-F                 read volumes from an input file (first argument "
         "is the input filename)\n");
  printf("\t-g <sigma>         blur the input image with Gaussian <sigma>\n");
  printf("\t-max               compute the max of the min distances instead of "
         "the mean\n");
...
static double compute_hdist(MRI **mri, int nvolumes, int index, double *hdists,
  ...
  for (hdist = 0.0, n = 0; n < nvolumes; n++) {
    if (n == index)
      continue;
    for (max_hdists[n] = hdists_sigma[n] = hdists[n] = 0.0, nvox = x = 0;
         x < width; x++)
      for (y = 0; y < height; y++)
        for (z = 0; z < depth; z++) {
    ...
    hdists_sigma[n] =
        sqrt(hdists_sigma[n] / (double)nvox - hdists[n] * hdists[n]);
    hdist += hdists[n];
  }
...
static double compute_hdist(MRI **mri, int nvolumes, int index) {
  ...
  width = mri[index]->width;
  height = mri[index]->height;
  depth = mri[index]->depth;
  ...
  for (hdist = 0.0, n = 0; n < nvolumes; n++) {
    ...
    if (vl1->nvox > 0)
      ...
      hdists[n] = 0;
    hdists_sigma[n] =
        sqrt(hdists_sigma[n] / (double)vl1->nvox - hdists[n] * hdists[n]);
    hdist += hdists[n];
  }
