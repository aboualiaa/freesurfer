...
def dice_coef_loss2(y_true, y_pred):
    ...
    y_true_reshape = K.batch_flatten(
        y_true
    )  # K.reshape(y_true, (num_samples, num_voxels, num_labels))

    y_pred /= K.sum(y_pred, axis=-1, keepdims=True)
    y_pred = K.clip(y_pred, K.epsilon(), 1)
    y_pred_reshape = K.batch_flatten(
        y_pred
    )  # K.reshape(y_pred, (num_samples, num_voxels, num_labels))

    sum_over_axis = 1
    numerator = 2 * K.sum(y_true_reshape * y_pred_reshape, sum_over_axis)
    denominator = K.sum(K.square(y_true_reshape), sum_over_axis) + K.sum(
        K.square(y_pred_reshape), sum_over_axis
    )
...
def pure_grad_loss(y_true, y_pred):
    ...
    y_true_zgrad = tf.nn.conv3d(
        y_true, tf_sobel_z, strides=[1, 1, 1, 1, 1], padding="SAME"
    )
    y_true_ygrad = tf.nn.conv3d(
        y_true, tf_sobel_y, strides=[1, 1, 1, 1, 1], padding="SAME"
    )
    y_true_xgrad = tf.nn.conv3d(
        y_true, tf_sobel_x, strides=[1, 1, 1, 1, 1], padding="SAME"
    )

    y_pred_zgrad = tf.nn.conv3d(
        y_pred, tf_sobel_z, strides=[1, 1, 1, 1, 1], padding="SAME"
    )
    y_pred_ygrad = tf.nn.conv3d(
        y_pred, tf_sobel_y, strides=[1, 1, 1, 1, 1], padding="SAME"
    )
    y_pred_xgrad = tf.nn.conv3d(
        y_pred, tf_sobel_x, strides=[1, 1, 1, 1, 1], padding="SAME"
    )
...
def unet_model_3d(
    ...
    if is_seg_network == False:
        ...
        if num_gpus > 1:
            with tf.device("/cpu:0"):
                ...
                if loss == "grad_loss":
                    print(loss)
                    model.compile(
                        optimizer=Adam(lr=initial_learning_rate), loss=grad_loss
                    )
                    ...
                    model.compile(
                        optimizer=Adam(lr=initial_learning_rate), loss=loss
                    )
                    parallel_model.compile(
                        optimizer=Adam(lr=initial_learning_rate), loss=loss
                    )
        ...
        if n_labels > 1:
            conv = Conv3D(
                n_labels, (1, 1, 1), activation="softmax", name="final_conv_3d"
            )(up)
            ...
            conv = Conv3D(
                1, (1, 1, 1), activation="sigmoid", name="final_conv_3d"
            )(up)
...
def class_net(
    ...
    model.add(
        ConvL(
            ...
            padding="same",
            activation="relu",
            input_shape=input_shape_append,
        )
    )
    model.add(
        ConvL(
            unet_num_filters, filter_shape, padding="valid", activation="relu"
        )
    )
    ...
    model.add(
        ConvL((2 ** 2) * unet_num_filters, filter_shape, activation="relu")
    )
    model.add(MaxPoolingL(pool_size=pool_shape))
    model.add(Dropout(0.25))
    ...
    model.add(
        ConvL((2 ** 3) * unet_num_filters, filter_shape, activation="relu")
    )
    model.add(MaxPoolingL(pool_size=pool_shape))
    model.add(Dropout(0.25))
    ...
    model.add(
        ConvL((2 ** 4) * unet_num_filters, filter_shape, activation="relu")
    )
    model.add(MaxPoolingL(pool_size=pool_shape))
    model.add(Dropout(0.25))
...
def unet_model(
    ...
    x = ConvL(32, (3, 3, 3), strides=(1, 1, 1), padding="same", name="conv1")(
        input_img
    )
    x = BatchNormalization(name="bn_conv1")(x)
    x = Activation("relu")(x)
    x = ConvL(32, (3, 3, 3), strides=(1, 1, 1), padding="same", name="conv2")(
        input_img
    )
    ...
    # #
    x = conv_block(
        x, [16, 32, 32], stage=4, block="a", strides=(1, 1, 1), dilation_rate=4
    )
    ...
    x = conv_block(
        x, [16, 32, 32], stage=5, block="a", strides=(1, 1, 1), dilation_rate=8
    )
    ...
    if is_seg_network == False:
        ...
        if n_labels > 1:
            ...
            x = ConvL(1, (1, 1, 1), activation="sigmoid", name="final_conv_3d")(
                x
            )
...
def build_oriented_unet_2d(
    ...
    for i in range(unet_depth):
        ...
        conv = ConvL(
            ...
            padding="same",
            kernel_initializer="he_normal",
            name=("conv3D_D_1_%d" % (i) + str(0) + layer_prefix),
        )(prev)
        conv = BatchNormalization(
            name=("bnorm_D_1_%d" % (i) + str(0) + layer_prefix)
        )(conv)
        convs.append(conv)
        ...
        for iter in range(1, depth_per_level):
            ...
            conv = ConvL(
                ...
                padding="same",
                kernel_initializer="he_normal",
                name=("conv3D_D_1_%d" % (i) + str(iter) + layer_prefix),
            )(convs[-1])
            conv = BatchNormalization(
                name=("bnorm_D_1_%d" % (i) + str(iter) + layer_prefix)
            )(conv)
            convs.append(conv)
    ...
    for i in range(unet_depth - 1):
        ...
        up = concatenate(
            [
                UpSamplingL(
                    size=pool_size,
                    name=("upsampling_U_%d" % (level + 1) + layer_prefix),
                )(convs[index]),
                convs[level],
            ],
            axis=-1,
            name=("concat_%d" % (level) + layer_prefix),
        )
        conv = ConvL(
            num_filters * (2 ** actual_level),
            filter_shape,
            padding="same",
            activation="relu",
            kernel_initializer="he_normal",
        ...
        conv = BatchNormalization(
            name=("bnorm_U_1_%d" % (level) + str(0) + layer_prefix)
        )(conv)
        convs.append(conv)
        ...
        for iter in range(1, depth_per_level):
            ...
            conv = ConvL(
                ...
                activation="relu",
                kernel_initializer="he_normal",
                name=("conv3D_U_2_%d" % (level) + str(iter) + layer_prefix),
            )(convs[-1])
            conv = BatchNormalization(
                name=("bnorm_U_2_%d" % (level) + str(iter) + layer_prefix)
            )(conv)
            convs.append(conv)
    ...
    up = concatenate(
        inputs + endpoints, axis=-1, name="final_concat" + layer_prefix
    )
    ...
    if is_seg_network == False:
        ...
        conv = ConvL(
            ...
            out_filter_shape,
            activation="linear",
            name="final_conv_3d" + layer_prefix,
        )(up)
...
def unet_2d_v1(
    ...
    conv_ax = build_oriented_unet_2d(
        input_img_ax,
    ...
    (model_ax, parallel_model_ax) = build_compile_model(
        input_img_ax,
        input_shape,
        conv_ax,
        n_labels,
...
if __name__ == "__main__":
    ...
    def dice_coef_loss2(y_true, y_pred):
        ...
        y_true_reshape = K.reshape(
            y_true, (num_samples, num_voxels, num_labels)
        )
        ...
        y_pred_reshape = K.reshape(
            y_pred, (num_samples, num_voxels, num_labels)
        )
