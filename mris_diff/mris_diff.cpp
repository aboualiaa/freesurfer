...
// Because slight numerical differences can cause triangles, especially those on
// folds, to end up at significantly different orientations (seen as big
// variations in nx,ny,nz) it is important to understand how well the whole
// surface fits, rather than just looking for the few bad matches.  So
// histograms of the various properties are used...
//
...
static int printfHistogramOfFit(HistogramOfFit *histogramOfFit,
  ...
  while (i < head) {
    ...
    if (requiredFit[requiredFitI + 1] >= 0.0)
      requiredFitI++;
    fit *= 3;
    i++;
  }
...
static const char *printHistograms() {
  ...
  printOneHistogram(&vertexXyzHistogram, "vertex xyz", vertexRequiredFit,
                    &badHistogram);
  printOneHistogram(&vertexRelativeXyzHistogram, "vertex rel xyz",
                    relVtxRequiredFit, &badHistogram);
  printOneHistogram(&vertexNxnynzHistogram, "vertex nxnynz", otherRequiredFit,
                    &badHistogram);
  printOneHistogram(&faceNxnynzHistogram, "face nxnynz", otherRequiredFit,
                    &badHistogram);
  printOneHistogram(&faceAreaHistogram, "face area", otherRequiredFit,
                    &badHistogram);
  printOneHistogram(&vertexCurvHistogram, "vertex curv", otherRequiredFit,
                    &badHistogram);
  return badHistogram;
}

static bool compareVertexPositions(MRIS *const lhs, MRIS *const rhs,
                                   std::vector<int> &lhsVno2rhsVno) {
  ...
  static auto closeEnoughV = [](VERTEX const &lhs, VERTEX const &rhs) {
    return closeEnoughF(lhs.x, rhs.x) && closeEnoughF(lhs.y, rhs.y) &&
  ...
  auto initList = [&](MRIS *const mris, List &list) {
    for (size_t i = 0; i < list.size(); i++)
      list[i] = i;
    std::sort(list.begin(), list.end(), [&](size_t li, size_t ri) -> bool {
      return mris->vertices[li].x < mris->vertices[ri].x;
    });
  };
  ...
  for (size_t li = 0; li < lhsList.size(); li++) {
    ...
    while (rLo < rhsList.size() &&
      ...
      rLo++;
    while (rHi < rhsList.size() &&
           rhs->vertices[rhsList[rHi]].x < lv.x + maxDistortion)
      rHi++;
    if (rLo == rhsList.size())
    ...
    if (found == 0) {
      ...
      std::cout << "compareVertexPositions more than 1 candidate matched, in "
                   "fact found: "
                << found << std::endl;
    } else {
  ...
  std::cout << "compareVertexPositions matched:" << matched
  ...
  if (matched < 0.99 * lhs->nvertices)
    badHistogram = "too many not matched";
...
int main(int argc, char *argv[]) {
  ...
  if (SUBJECTS_DIR1 == nullptr)
    SUBJECTS_DIR1 = SUBJECTS_DIR;
  ...
  if (surf2path == nullptr) {
    sprintf(tmpstr, "%s/%s/surf/%s.%s", SUBJECTS_DIR2, subject2, hemi,
            surfname);
  ...
  if (surf1->nvertices > surf2->nvertices) {
    printf("Swapping surf1 and surf2 to make the surf1 be the one with the "
           "least vertices\n");
  ...
  if (!renumberedSpecified) {
    ...
    for (int i = 0; i < surf1->nvertices; i++)
      surf1Vno_to_surf2Vno[i] = i;
    ...
    printf("Surfaces have enough vertices in about the same locations to try "
           "to match up the faces\n");
  }

  // Number of Faces ------------------------------------------

  // for every face1 in surf1, find a vno1 on it, find the corresponding vno2 in
  // surf2, find the corresponding surf2 face

  // Even if there are the same number of faces, they need to be matched this
  // way
  ...
  if (!renumberedSpecified) {
    ...
    for (int fno1 = 0; fno1 < surf1->nfaces; fno1++) {
      ...
      int vnos2[3];
      for (int i = 0; i < 3; i++)
        vnos2[i] = surf1Vno_to_surf2Vno[f1->v[i]];
  ...
  if (xyzRMSFile) {
    ...
    for (nthvtx = 0; nthvtx < surf1->nvertices; nthvtx++) {
      ...
      VERTEX const *const vtx1 = &(surf1->vertices[nthvtx]);
      VERTEX const *const vtx2 = &(surf2->vertices[nthvtx2]);
      rms = sqrt(pow(vtx1->x - vtx2->x, 2) + pow(vtx1->y - vtx2->y, 2) +
                 pow(vtx1->z - vtx2->z, 2));
  ...
  if (CheckSurf) {
    ...
    for (nthvtx = 0; nthvtx < surf1->nvertices; nthvtx++) {
      ...
      for (nthnbr = 0; nthnbr < nnbrs1; nthnbr++) {
        ...
        if (surf1Vno_to_surf2Vno[vtx1t->v[nthnbr]] < 0)
          continue; // this neighbor no longer exists

        if (nnbrs2 == nthnbr2) {
          if (false)
            printf("Surf2->vertices[%d] doesn't have enough neighbors\n",
                   nthvtx);
          has_bad_neighbours = true;
          break;
        }
        ...
        if (surf1Vno_to_surf2Vno[nbrvtxno1] != nbrvtxno2) {
          if (false)
            printf("Surf1->Vertices[%d] and Surf2->Vertices[%d] differs in the "
                   "identity of the "
                   "v[%d:%d], namely Surf2 vertices[%d:%d]\n",
                   nthvtx, nthvtx2, nthnbr, nthnbr2,
      ...
      if (nthnbr2 != nnbrs2) {
        if (false)
          printf("Surf2->vertices[%d] has extra neighbors\n", nthvtx);
        has_bad_neighbours = true;
      }
    ...
    if (gridFile && gridspacing > 0.0) {
          ...
          if (false) {
            ...
            if (count++ > limit) {
              ...
              printf("%6ld d0:(%f,%f,%f) d1:(%f,%f,%f) result:%d\n", count, v.x,
                     v.y, v.z, v2.x, v2.y, v2.z, interesting);
            }
          }
        }
      ...
      if (area)
        LabelWrite(area, gridFile);
      else
        printf("No interesting points, so gridfile not written\n");
    }
  ...
  if (CheckAParc) {
    ...
    sprintf(tmpstr, "%s/%s/label/%s.%s.annot", SUBJECTS_DIR1, subject1, hemi,
            aparcname);
    ...
    if (aparc2name)
      aparcname = aparc2name;
    sprintf(tmpstr, "%s/%s/label/%s.%s.annot", SUBJECTS_DIR2, subject2, hemi,
            aparcname);
    printf("Loading aparc file %s\n", tmpstr);
    fflush(stdout);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--no-check-xyz"))
      CheckXYZ = 0;
    else if (!strcasecmp(option, "--no-check-nxyz"))
      CheckNXYZ = 0;
    else if (!strcasecmp(option, "--renumbered"))
      ...
      while (int c = *p++) {
        ...
        case 'Z':
          ...
          continue;
        default:
          fprintf(stderr, "--grid only supports {[xyz]}, not %s\n", pargv[1]);
          continue;
        }
        break;
      }
      ...
      if (UseExact) {
        ...
        mindist = MRIcopyMRIS(nullptr, surf2, 0, "curv");
      } else
      ...
      exit(0);
      nargsused = 4;
    } else if (!strcasecmp(option, "--simple")) {
      if (nargc < 2)
        CMDargNErr(option, 2);
...
static void print_usage() {
  ...
  printf("   --renumbered  the vertices or faces may have been renumbered and "
         "a few deleted\n");
  printf("   --worst-bucket worstbucketfile : compute the worst histogram "
         "bucket each vertex is in\n");
  printf("   --grid {[xyz]} spacingfloat grid_file : label the vertices of "
         "edges that span a grid\n");
  printf("\n");
  ...
  printf("   --min-dist surf1 surf2 mindist : compute vertex-by-vert RMS "
         "distance between surfs\n");
...
/* --------------------------------------------- */
static void check_options() {
  if (surf1path == nullptr && subject1 == nullptr) {
    printf("ERROR: surface 1 not specified\n");
    exit(1);
  }
  if (surf2path == nullptr && subject2 == nullptr) {
    printf("ERROR: surface 2 not specified\n");
    exit(1);
  }
  if ((surf1path == nullptr && surf2path != nullptr) ||
      (surf2path == nullptr && surf1path != nullptr)) {
  ...
  if (surfname != nullptr && curvname != nullptr) {
    printf("ERROR: cannot specify both surf and curv.\n");
    exit(1);
  }
  if (surfname != nullptr && aparcname != nullptr) {
    printf("ERROR: cannot specify both surf and aparc.\n");
    exit(1);
  }
  if (curvname != nullptr && aparcname != nullptr) {
    printf("ERROR: cannot specify both curv and aparc.\n");
    exit(1);
  }
  if (surf1path == nullptr && (subject1 == nullptr || subject2 == nullptr)) {
    printf("ERROR: need subject names for relative path.\n");
    exit(1);
  }
  if (surf1path == nullptr && surfname == nullptr && curvname == nullptr &&
      aparcname == nullptr) {
    printf("ERROR: need --surf or --curv or --aparc with relative path.\n");
    exit(1);
  }
  if (surf1path == nullptr && hemi == nullptr) {
...
static void dump_options(FILE *fp) {
  ...
  if (SUBJECTS_DIR1)
    fprintf(fp, "SUBJECTS_DIR1   %s\n", SUBJECTS_DIR1);
  if (SUBJECTS_DIR2)
    fprintf(fp, "SUBJECTS_DIR2   %s\n", SUBJECTS_DIR2);
  fprintf(fp, "cwd       %s\n", cwd);
  ...
  if (subject1)
    fprintf(fp, "subject1  %s\n", subject1);
  if (subject2)
    fprintf(fp, "subject2  %s\n", subject2);
  if (hemi)
    fprintf(fp, "hemi      %s\n", hemi);
  if (surfname)
    fprintf(fp, "surfname  %s\n", surfname);
  if (curvname)
    fprintf(fp, "curvname  %s\n", curvname);
  if (aparcname)
    fprintf(fp, "aparcname %s\n", aparcname);
  if (aparc2name)
    fprintf(fp, "aparc2name %s\n", aparc2name);
  fprintf(fp, "seed %ld\n", seed);
  fprintf(fp, "\n");
  fprintf(fp, "\n");
...
MRI *MRISminDist(MRIS *srcsurf, MRIS *trgsurf) {
  ...
  for (svtx = 0; svtx < srcsurf->nvertices; svtx++) {
    ...
    if (dmin > MRIgetVoxVal(mindist, tvtx, 0, 0, 0))
      MRIsetVoxVal(mindist, tvtx, 0, 0, 0, dmin);
  }
  MHTfree(&srchash);
  MHTfree(&trghash);
...
int MRISdiffSimple(MRIS *surf1, MRIS *surf2, int ndiffmin, double rmsthresh,
  ...
  for (n = 0; n < surf1->nvertices; n++) {
    ...
    if (vt1->vnum != vt2->vnum) {
      if (verbosity > 0) {
        printf(
            "%d Surfaces differ in number of neighbors at vertex %d   %d %d\n",
  ...
  for (n = 0; n < surf1->nvertices; n++) {
    ...
    if (rms > rmsthresh) {
      if (verbosity > 0) {
        printf("%d Surfaces differ in xyz at vertex %d  rms=%g (%g,%g,%g) "
               "(%g,%g,%g)\n",
               ndiff, n, rms, v1->x, v1->y, v1->z, v2->x, v2->y, v2->z);
        fflush(stdout);
      }
      ndiff++;
    }
