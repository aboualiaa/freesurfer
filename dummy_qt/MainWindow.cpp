...
void MainWindow::OnButtonRun() {
  QString cmd = QString("mri_convert %1 %2")
                    .arg(ui->lineEditInput->text().trimmed())
...
bool MainWindow::OnButtonAbort() {
  if (QMessageBox::question(this, "Abort",
                            "Do you really want to abort current process?",
...
void MainWindow::AddMessage(const QString &msg, int type) {
  ...
  if (type != Normal)
    strg = QString("<span style=\"font-weight:bold; color:%1;\">%2</span>")
               .arg(color)
               .arg(msg);
  else
    strg = QString("<span style=\"font-weight:normal; color:%1;\">%2</span>")
               .arg(color)
               .arg(msg);
  strg.replace("\n", "<br />");
