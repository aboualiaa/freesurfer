...
static double rectangle_error(MRI_SURFACE *mris, double xmin, double ymin,
  ...
  x0 = (xmin + xmax) / 2;
  y0 = (ymin + ymax) / 2; // center of rectangle
  xtop = xmax - x0;
  ytop = ymax - y0;
  xbottom = xmin - x0;
  ybottom = ymin - y0;
...
static int find_biggest_inscribed_rectangle(MRI_SURFACE *mris, double *pxmin,
                                            double *pymin, double *pxmax,
                                            double *pymax) {
  ...
  if (DIAG_VERBOSE_ON)
    printf("centroid of flatmap found at (%2.1f, %2.1f), bounding box (%2.1f "
           "--> %2.1f, %2.1f --> %2.1f) : %2.2f\n",
...
MRI *MRIflattenOverlay(MRI_SURFACE *mris, MRI *mri_overlay, MRI *mri_flat,
                       double res, LABEL *label_overlay, MRI **pmri_vertices) {
  double xmin, ymin, xmax, ymax, fdist, lambda[3], xf, yf, val0, val1, val2,
      val;
  ...
  width = (int)(floor(width / 2.0) * 2.0 + 1);
  xmax = xmin + width;
  height = (int)ceil((ymax - ymin) / res);
  height = (int)(floor(height / 2.0) * 2.0 + 1);
  ymax = ymin + height;
  ...
  mri_vertices->xstart = mri_flat->xstart = xmin;
  mri_vertices->xend = mri_flat->xend = xmax;
  mri_vertices->ystart = mri_flat->ystart = ymin;
  mri_vertices->yend = mri_flat->yend = ymax;
  mri_vertices->zstart = mri_flat->zstart = 0;
  mri_vertices->zend = mri_flat->zend = mri_overlay->nframes - 1;
  mri_vertices->c_r = mri_flat->c_r = xmin;
  mri_vertices->c_a = mri_flat->c_a = ymin;
  ...
  for (x = 0; x < width; x++)
    for (y = 0; y < height; y++) {
      xf = x * res + xmin;
      yf = y * res + ymin; // back to flattened coords
      MHTfindClosestFaceGeneric(mht, mris, xf, yf, 0.0, 10 * res, 2, 1, &face,
                                &fno, &fdist);
      if (fno >= 0) // otherwise this point is not in a face
      {
        ret = face_barycentric_coords(mris, fno, CURRENT_VERTICES, xf, yf, 0,
        ...
        if (ret >= 0) {
          ...
          MHTfindClosestFaceGeneric(mht, mris, xf, yf, 0.0, 1000, -1, 1, &face,
                                    &fno, &fdist);
          printf("(%d, %d) --> %f %f unmapped (goes to face %d, v (%d, %d, %d) "
                 "if projected\n",
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_flatten.c,v 1.42 2016/12/10 22:57:46 fischl Exp $",
      "$Name:  $");
  ...
  if (Gdiag_no >= 0) {
    ...
    printf("vertex %d has %d nbrs before patch:\n", Gdiag_no,
           mris->vertices_topology[Gdiag_no].vnum);
  ...
  if (one_surf_flag) /* only have the 1 surface - no patch file */
  {
    ...
    MRISresetNeighborhoodSize(
        mris, mris->vertices_topology[0].nsizeMax); // set back to max
    if (label_fname) // read in a label instead of a patch
    {
      ...
      if (MRISreadPatch(mris, in_patch_fname) != NO_ERROR)
        ErrorExit(ERROR_BADPARM, "%s: could not read patch file %s", Progname,
                  in_patch_fname);
      if (dilate) {
  ...
  if (mri_overlay) {
    ...
    if (mri_overlay->width == mri_overlay->height && mri_overlay->depth == 1 &&
        mri_overlay->nframes == 1) {
      MRI *mri_tmp;
      printf("reshaping to move 2nd dimension to time\n");
      mri_tmp = mri_reshape(mri_overlay, mri_overlay->width, 1, 1,
                            mri_overlay->height);
      MRIfree(&mri_overlay);
      mri_overlay = mri_tmp;
    }
    ...
    printf(
        "resampling overlay (%d x %d x %d x %d) into flattened coordinates..\n",
        mri_overlay->width, mri_overlay->height, mri_overlay->depth,
        mri_overlay->nframes);
    if (synth_name) {
...
static void print_help() {
  ...
  fprintf(stderr, " -dilate <# of dilations>\n\t"
                  "specify the number of times to dilate the ripped edges to "
                  "ensure a clean cut\n");
  exit(1);
}
