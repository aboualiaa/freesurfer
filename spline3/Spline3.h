 ...
 *   ynew = S.eval(y);  // using cached xnew, pass reference to y again (not
 * cached earlier)
 *
 */
template <class T> class Spline3 {
public:
  Spline3();
  ...
  const std::vector<T> &eval(const std::vector<T> &xnew,
                             const std::vector<T> &yold);
  T *eval(unsigned int M, const T xnew[], unsigned int N, const T yold[],
          T ynew[]);
...
private:
  ...
  //! Computes 2nd derivatives ypp via M ypp = b (based on cached values M and
  //! b)
  void computeYpp();
...
template <class T> Spline3<T>::Spline3() : n(0) {}
...
template <class T> void Spline3<T>::preCacheX(const std::vector<T> &x) {
  n = x.size();
  if (n < 3) {
...
template <class T> void Spline3<T>::preCacheX(unsigned int N, const T x[]) {
  n = N;
  if (n < 3) {
    std::cerr << "Spline3 ERROR: we need at least 3 knots" << std::endl;
    exit(1);
 ...
 */
template <class T>
const std::vector<T> &Spline3<T>::eval(const std::vector<T> &xnew,
                                       const std::vector<T> &yold) {
  preCacheXnew(xnew);
  return eval(yold);
}
...
template <class T>
T *Spline3<T>::eval(unsigned int M, const T xnew[], unsigned int N,
                    const T yold[], T ynew[]) {
  preCacheXnew(M, xnew);
...
template <class T> void Spline3<T>::preCacheXnew(const std::vector<T> &xnew) {
  m = xnew.size();
  intervals.resize(m);
  d.resize(m);
  unsigned int i, j;
...
const std::vector<T> &Spline3<T>::eval(const std::vector<T> &yold) {
  for (unsigned int j = 0; j < m; j++) {
    ...
    ynew[j] =
        yold[ival] +
        dval * (ydhiival - (ypp[ival + 1] / 6.0 + ypp[ival] / 3.0) * hval +
                dval * (0.5 * ypp[ival] +
...
template <class T> void Spline3<T>::computeHi(const std::vector<T> &x) {
...
template <class T> void Spline3<T>::computeM(const std::vector<T> &x) {
  unsigned int nm1 = n - 1;
  M0.resize(n);
  M1.resize(n);
  M2.resize(n);
...
/** Chache xm based on M (coefficients necessary for solving the system M x =
   b). also M1 is being updated. */
template <class T> void Spline3<T>::cacheMi() {
...
template <class T> void Spline3<T>::computeYpp() {
