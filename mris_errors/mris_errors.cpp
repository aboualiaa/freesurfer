...
static char vcid[] =
    "$Id: mris_errors.c,v 1.11 2011/03/02 00:04:31 nicks Exp $";
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_errors.c,v 1.11 2011/03/02 00:04:31 nicks Exp $",
      "$Name:  $");
  ...
  for (fno = 0; fno < mris->nfaces; fno++) {
    ...
    for (ano = 0; ano < ANGLES_PER_TRIANGLE; ano++) {
      ...
      fprintf(fp, "%d %2.3f %2.3f %2.3f %2.1f\n", fno,
              ...
              (float)DEGREES(face->angle[ano]), (float)DEGREES(error),
              (float)pct_error);
      total_sq_error += (error * error);
      ferror += fabs(error);
      total_error += error;
