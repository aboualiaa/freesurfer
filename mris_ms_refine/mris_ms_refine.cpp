...
#define ORIG_EXPANSION_DIST                                                    \
  1.0 /* average thickness of the cortex in mm (approx) */
...
#define IS_CSF(T1, PD)                                                         \
  (!((PD > MIN_NONBRAIN_PD) && (T1 > MIN_NONBRAIN_T1 && T1 < MIN_CSF_T1)))
#define IS_PV_CSF(T1, PD) (T1 > MIN_PARTIAL_VOLUMED_CSF_T1)
#if 0
#define IS_WM(T1, PD)                                                          \
  ((T1 >= MIN_WM_T1) && (T1 <= MAX_WM_T1) && (PD <= MAX_WM_PD) &&              \
   (PD >= MIN_WM_PD))
#define IS_GM(T1, PD)                                                          \
  ((T1 >= MIN_GM_T1) && (T1 <= MAX_GM_T1) && (PD >= MIN_GM_PD))
#define IS_BRAIN(T1, PD) (IS_GM(T1, PD) || IS_WM(T1, PD))

#else

#define IS_WM(T1, PD, vno, ep)                                                 \
  ((T1 >= ep->cv_min_wm_T1[vno]) && (T1 <= ep->cv_max_wm_T1[vno]) &&           \
   (PD <= ep->cv_max_wm_PD[vno]) && (PD >= ep->cv_min_wm_PD[vno]))
#define IS_GM(T1, PD, vno, ep)                                                 \
  ((T1 >= ep->cv_min_gm_T1[vno]) && (T1 <= ep->cv_max_gm_T1[vno]) &&           \
   (PD >= ep->cv_min_gm_PD[vno]))
#define IS_BRAIN(T1, PD, vno, ep)                                              \
  (IS_GM(T1, PD, vno, ep) || IS_WM(T1, PD, vno, ep))

#endif

#define WM_PARM_DIST(T1, PD)                                                   \
  (T1 > MAX_WM_T1 ? (T1 - MAX_WM_T1) : T1 < MIN_WM_T1 ? MIN_WM_T1 - T1 : 0)
...
#endif
static double compute_optimal_parameters(MRI_SURFACE *mris, int vno,
...
static double image_forward_model(double TR, double flip_angle, double dist,
                                  ...
                                  double T1_gm, double PD_gm, double T1_csf,
                                  double PD_csf);
static int write_map(MRI_SURFACE *mris, float *cv, const char *name, int suffix,
                     const char *output_suffix);
static int write_maps(MRI_SURFACE *mris, EXTRA_PARMS *ep, int suffix,
                      const char *output_suffix);
static int smooth_maps(MRI_SURFACE *mris, EXTRA_PARMS *ep, int smooth_parms);
static int smooth_marked_maps(MRI_SURFACE *mris, EXTRA_PARMS *ep,
                              int smooth_parms);
static int smooth_map(MRI_SURFACE *mris, float *cv, int navgs);
static int smooth_csf_map(MRI_SURFACE *mris, float *cv_T1, float *cv_PD,
                          int navgs);
static int smooth_marked_map(MRI_SURFACE *mris, float *cv, int navgs);
static int smooth_marked_csf_map(MRI_SURFACE *mris, float *cv_T1, float *cv_PD,
                                 int navgs);
...
#endif
static int compute_maximal_distances(
    MRI_SURFACE *mris, float sigma, MRI **mri_flash, int nvolumes,
    float *cv_inward_dists, float *cv_outward_dists, int *nearest_pial_vertices,
    int *nearest_white_vertices, double dstep, double max_inward_dist,
    double max_outward_dist, MRI *mri_T1, MRI *mri_PD, EXTRA_PARMS *ep);
static int find_nearest_pial_vertices(MRI_SURFACE *mris,
                                      int *nearest_pial_vertices,
                                      int *nearest_white_vertices);
static int find_nearest_white_vertices(MRI_SURFACE *mris,
                                       int *nearest_white_vertices);
static int sample_parameter_maps(
    MRI_SURFACE *mris, MRI *mri_T1, MRI *mri_PD, float *cv_wm_T1,
    float *cv_wm_PD, float *cv_gm_T1, float *cv_gm_PD, float *cv_csf_T1,
    float *cv_csf_PD, float *cv_inward_dists, float *cv_outward_dists,
    EXTRA_PARMS *ep, int fix_T1, INTEGRATION_PARMS *parms, int start_vno);
#if 0
...
static double compute_vertex_sse(
    EXTRA_PARMS *ep, double image_vals[MAX_FLASH_VOLUMES][MAX_SAMPLES],
    int max_j, double white_dist, double cortical_dist, double T1_wm,
...
#define FLASH_START 4
  ...
  for (i = FLASH_START; i < argc; i++) {
    ...
    case SAMPLE_NEAREST:
      ...
      break;
    default:
      ErrorExit(ERROR_UNSUPPORTED, "%s: unsupported sample type %d", Progname,
                sample_type);
      break;
    }
  ...
  if (orig_flag) {
    ...
    if (start_white_name) {
      if (MRISreadOriginalProperties(mris, start_white_name) != NO_ERROR)
        ErrorExit(ERROR_NOFILE, "%s: could not read gray/white surface from %s",
  ...
  if (!cv_inward_dists || !cv_outward_dists || !cv_wm_T1 || !cv_wm_PD ||
      ...
      !ep.cv_min_wm_PD || !ep.cv_max_wm_PD || !ep.cv_min_gm_PD ||
      !ep.cv_max_gm_PD)
  ...
  for (n_averages = max_averages, i = 0; n_averages >= min_averages;
    ...
    printf("computing inner and outer bounds on error functional, "
           "avgs=%d,sigma=%2.2f,dstep=%2.3f,"
           "max_dist=%2.2f:%2.2f...\n",
           n_averages, current_sigma, ep.dstep, ep.max_inward_dist,
           ep.max_outward_dist);
    {
    ...
    find_nearest_pial_vertices(mris, ep.nearest_pial_vertices,
                               ep.nearest_white_vertices);
    find_nearest_white_vertices(mris, ep.nearest_white_vertices);
    compute_maximal_distances(
        mris, current_sigma, mri_flash, nvolumes, cv_inward_dists,
        cv_outward_dists, ep.nearest_pial_vertices, ep.nearest_white_vertices,
        ep.dstep, ep.max_inward_dist, ep.max_outward_dist, ep.mri_T1, ep.mri_PD,
        &ep);
#if 0
    ...
    sample_parameter_maps(mris, mri_T1, mri_PD, cv_wm_T1, cv_wm_PD, cv_gm_T1,
                          cv_gm_PD, cv_csf_T1, cv_csf_PD, cv_inward_dists,
                          cv_outward_dists, &ep, fix_T1, &parms, MIN_VNO);
    ...
    if (Gdiag_no >= 0 && Gdiag_no < mris->nvertices) {
      printf("v %d: wm = (%2.0f, %2.0f), gm = (%2.0f, %2.0f), csf = (%2.0f, "
             "%2.0f)\n",
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    if (!mri_PD)
      ErrorExit(ERROR_NOFILE, "%s: could not read PD volume from %s", Progname,
                argv[2]);
    nargs = 1;
    ...
    case 'Q':
      ...
      printf("doing quick (no self-intersection) surface positioning.\n");
      break;
    case 'P':
      plot_stuff = 1;
...
static void print_help() {
  ...
  printf("\nvalid options are:\n\n");
  printf("-q    omit self-intersection and only generate "
         "gray/white surface.\n");
  printf("-c    create curvature and area files from white matter surface\n");
  printf("-a <avgs>   average curvature values <avgs> times (default=10)\n");
  printf("-whiteonly  only generate white matter surface\n");
  exit(1);
}
...
static double ms_errfunc_gradient(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
...
#if 1
  compute_maximal_distances(
      mris, ep->current_sigma, ep->mri_flash, ep->nvolumes, ep->cv_inward_dists,
      ep->cv_outward_dists, ep->nearest_pial_vertices,
      ep->nearest_white_vertices, ep->dstep, ep->max_inward_dist,
      ep->max_outward_dist, ep->mri_T1, ep->mri_PD, ep);
#endif
  sse = 0.0;
  for (vno = MIN_VNO; vno < mris->nvertices; vno++) {
    ...
    if (Gdiag_no == vno)
      printf("v %d: sse %2.2f\n", vno,
             (white_delta * white_delta) + (pial_delta * pial_delta));
    ...
    if (Gdiag_no == white_vno || Gdiag_no == pial_vno) {
      printf("v %d,%d: delta = %2.2f, %2.2f along n = (%2.2f, %2.2f, %2.2f)\n",
             white_vno, pial_vno, lambda * white_delta, lambda * pial_delta, nx,
             ny, nz);
      printf("      current location   = (%2.2f, %2.2f, %2.2f) --> (%2.2f, "
             "%2.2f, %2.2f)\n",
             v_white->origx, v_white->origy, v_white->origz, v_pial->pialx,
             v_pial->pialy, v_pial->pialz);
      printf("      predicted location = (%2.2f, %2.2f, %2.2f) --> (%2.2f, "
             "%2.2f, %2.2f)\n",
    ...
    if (white_vno == Gdiag_no)
      printf("v %d white intensity   :  (%2.3f, %2.3f, %2.3f)\n", vno,
             lambda * white_delta * nx, lambda * white_delta * ny,
             lambda * white_delta * nz);
    if (pial_vno == Gdiag_no)
      printf("v %d pial intensity    :  (%2.3f, %2.3f, %2.3f)\n", vno,
             lambda * pial_delta * nx, lambda * pial_delta * ny,
  ...
  if (Gdiag_no >= 0 && DIAG_VERBOSE_ON) {
    ...
    for (n = 0; n < vt->vnum; n++) {
      n_vno = vt->v[n];
      dot =
          mris->dx2[n_vno] * dx + mris->dy2[n_vno] * dy + mris->dz2[n_vno] * dz;
      if (dot < 0) {
        printf("vertex %d: dx = (%2.1f, %2.1f, %2.1f), dot = %2.2f\n", n_vno,
               mris->dx2[n_vno], mris->dy2[n_vno], mris->dz2[n_vno], dot);
      }
    }
  }
...
static int ms_errfunc_timestep(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  ep = (EXTRA_PARMS *)parms->user_parms;
  update_distances(mris,
                   ep); /* will mark vertices that need parms recomputed */
  store_current_positions(mris, ep);
  if (ep->nvertices < mris->nvertices) {
    ...
    sample_parameter_maps(
        mris, mri_T1, mri_PD, ep->cv_wm_T1, ep->cv_wm_PD, ep->cv_gm_T1,
        ep->cv_gm_PD, ep->cv_csf_T1, ep->cv_csf_PD, ep->cv_inward_dists,
        ep->cv_outward_dists, ep, fix_T1, parms, ep->nvertices);
    ep->nvertices = mris->nvertices;
  }
  return (NO_ERROR);
}
...
static int compute_maximal_distances(
    ...
    float *cv_inward_dists, float *cv_outward_dists, int *nearest_pial_vertices,
    int *nearest_white_vertices, double dstep, double max_inward_dist,
    double max_outward_dist_total, MRI *mri_T1, MRI *mri_PD, EXTRA_PARMS *ep) {
  int vno, i, found_csf, j, max_j = 0, found_wm, past_wm, wm_dist, csf_dist,
                            min_j, low_pd_csf;
  /*  int    nreversed, max_reversed,done;*/
  double T1, PD, max_outward_dist;
  VERTEX *v_white, *v_pial;
  MRI *mri;
  float nx, ny, nz, min_inward_dist, min_outward_dist, dist, min_parm_dist,
      parm_dist;
  ...
  for (vno = MIN_VNO; vno < mris->nvertices; vno++) {
    ...
    for (i = 0; i < nvolumes; i++) {
      ...
      if (TOO_SMALL(dist)) {
        ...
        MRIsurfaceRASToVoxel(mri, v_pial->pialx + v_white->nx,
                             v_pial->pialy + v_white->ny,
                             v_pial->pialz + v_white->nz, &xp, &yp, &zp);
        nx = xp - xw;
        ny = yp - yw;
    ...
    if (vno == Gdiag_no)
      printf("v %d: inward_dist = %2.2f, X=(%2.1f, %2.1f, %2.1f)\n", vno,
             min_inward_dist, v_white->origx, v_white->origy, v_white->origz);
  }
  ...
  for (vno = MIN_VNO; vno < mris->nvertices; vno++) {
    ...
    {
      if (vno == Gdiag_no)
        printf("couldn't find unambiguous csf - searching for partial volumed "
               "csf...");
      csf_dist = 0;
      min_parm_dist = MAX_T1 * MAX_T1;
      min_j = 0;
      ...
      for (j = 0; j <= max_j; j++) {
        ...
        if (past_wm && (IS_WM(T1, PD, vno, ep) ||
                        (!IS_CSF(T1, PD) && !IS_GM(T1, PD, vno, ep))))
    ...
    if (!found_csf) {
      for (j = 0; j < max_j;
           j++) /* assume T1 and PD are monotonically increasing */
      {
    ...
    */
    for (done = 0, j = 0; !done && j <= max_j; j++) {
      ...
      zo = zp + dist * nz;
      for (i = 0; i < nvolumes; i++) {
        MRIsampleVolumeDerivativeScale(mri_flash[i], xo, yo, zo, nx, ny, nz,
                                       &dIdN, sigma);
    ...
    if (vno == Gdiag_no && min_outward_dist >= 2)
      DiagBreak();
    if (vno == Gdiag_no)
      printf("v %d: outward_dist = %2.2f, X=(%2.1f, %2.1f, %2.1f), N=(%2.1f, "
             "%2.1f, %2.1f)\n",
             vno, min_outward_dist, v_pial->pialx, v_pial->pialy, v_pial->pialz,
             nx, ny, nz);
  }
...
static int sample_parameter_maps(
  ...
  double /*inward_dist, outward_dist,*/ dstep, best_white_delta = 0,
                                               best_pial_delta = 0, sse;
  ...
  for (nholes = 0, vno = start_vno; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no) {
      printf("v %d: wm = (%2.0f, %2.0f), gm = (%2.0f, %2.0f), "
             ...
             ep->cv_gm_T1[vno], ep->scale * ep->cv_gm_PD[vno],
             ep->cv_csf_T1[vno], ep->scale * ep->cv_csf_PD[vno],
  ...
  if (mri_PD) {
    sample_parameter_map(mris, mri_PD, mri_res, cv_wm_PD, cv_inward_dists, -1,
                         ORIGINAL_VERTICES, "wm PD", NULL, ep->dstep,
                         ep->max_dist);
    sample_parameter_map(mris, mri_PD, mri_res, cv_gm_PD, NULL, 1,
                         ORIGINAL_VERTICES, "gm PD", NULL, ep->dstep,
                         ep->max_dist);
    sample_parameter_map(mris, mri_PD, mri_res, cv_csf_PD, cv_outward_dists, 1,
                         PIAL_VERTICES, "csf PD", NULL, ep->dstep,
                         ep->max_dist);
  } else {
    for (vno = 0; vno < mris->nvertices; vno++) {
      v = &mris->vertices[vno];
    ...
    for (vno = 0; vno < mris->nvertices; vno++) {
      ...
      for (num = 1.0f, vnb = 0; vnb < vnum; vnb++) {
        ...
        VERTEX const *const vn =
            &mris->vertices[n_vno]; /* neighboring vertex pointer */
        if (vn->ripflag)
          continue;
        T1_nbr = cv_T1[n_vno];
        PD_nbr = cv_PD[n_vno];
...
static int write_map(MRI_SURFACE *mris, float *cv, const char *name, int suffix,
  ...
  if (suffix >= 0)
    sprintf(fname, "%s.%s_%d%s",
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh", name, suffix,
            output_suffix);
  else
    sprintf(fname, "%s.%s%s", mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh",
            name, output_suffix);
...
static double image_forward_model(double TR, double flip_angle, double dist,
                                  ...
                                  double T1_gm, double PD_gm, double T1_csf,
                                  double PD_csf) {
  static double predicted_white, predicted_gray, predicted_csf,
      last_TR = 0, last_flip_angle = 0, last_T1_wm, last_PD_wm, last_T1_gm,
      last_PD_gm, last_T1_csf, last_PD_csf;
...
static double compute_optimal_parameters(MRI_SURFACE *mris, int vno,
                                         EXTRA_PARMS *ep, double *pwhite_delta,
                                         double *ppial_delta) {
  double predicted_val, dx, dy, dz, dist, inward_dist, outward_dist,
      cortical_dist, PD_wm, PD_gm, PD_csf, T1_wm, T1_gm, T1_csf, white_dist,
      pial_dist, sse, best_sse, best_white_dist, best_pial_dist,
      orig_cortical_dist, total_dist, orig_white_index, orig_pial_index, sigma,
      best_T1_wm, best_PD_wm, best_T1_gm, best_PD_gm, best_T1_csf, best_PD_csf,
      T1, PD;
  ...
  double image_vals[MAX_FLASH_VOLUMES][MAX_SAMPLES], xw, yw, zw, xp, yp, zp, x,
      y, z, T1_vals[MAX_SAMPLES], PD_vals[MAX_SAMPLES],
  ...
  MRIsurfaceRASToVoxel(ep->mri_flash[0], v_white->origx, v_white->origy,
                       v_white->origz, &xw, &yw, &zw);
  MRIsurfaceRASToVoxel(ep->mri_flash[0], v_pial->pialx, v_pial->pialy,
                       v_pial->pialz, &xp, &yp, &zp);
  ...
  if (TOO_SMALL(cortical_dist)) {
    ...
    //                v_pial->pialz+v_pial->nz,
    //                &xp,&yp,&zp);
    MRIsurfaceRASToVoxel(ep->mri_flash[0], v_pial->pialx + v_pial->nx,
                         v_pial->pialy + v_pial->ny, v_pial->pialz + v_pial->nz,
                         &xp, &yp, &zp);
    dx = xp - xw;
    dy = yp - yw;
    dz = zp - zw;
  ...
  if ((plot_stuff == 1 && Gdiag_no == vno) || plot_stuff > 1) {
    ...
    fprintf(fp, "%f %f %f\n%f %f %f\n", orig_white_index, 0.0, 0.0,
            orig_white_index, 0.0, 120.0);
    fclose(fp);
    fp = fopen("orig_pial.plt", "w");
    fprintf(fp, "%f %f %f\n%f %f %f\n", orig_pial_index, cortical_dist, 0.0,
            orig_pial_index, cortical_dist, 120.0);
    fclose(fp);
  }
  ...
  if ((plot_stuff == 1 && Gdiag_no == vno) || plot_stuff > 1) {
    ...
    for (i = 0; i < ep->nvolumes; i++) {
      ...
      for (j = 0; j <= max_j; j++) {
        ...
        predicted_val = image_forward_model(
            mri->tr, mri->flip_angle, dist, cortical_dist, best_T1_wm,
            best_PD_wm, best_T1_gm, best_PD_gm, best_T1_csf, best_PD_csf);
  ...
  if ((plot_stuff == 1 && Gdiag_no == vno) || plot_stuff > 1) {
    ...
    fprintf(fp, "%d %f %f\n%d %f %f\n", best_white_index, 0.0, 0.0,
            best_white_index, 0.0, 120.0);
    fclose(fp);
    fp = fopen("best_pial.plt", "w");
    fprintf(fp, "%d %f %f\n%d %f %f\n", best_pial_index, cortical_dist, 0.0,
            best_pial_index, cortical_dist, 120.0);
    fclose(fp);
  }
  ...
  *ppial_delta = best_pial_dist - (orig_pial_index * ep->dstep);
  if (*pwhite_delta < 0.1 || *ppial_delta > 0.1)
    DiagBreak();
  if (vno == Gdiag_no) {
    double pwhite[MAX_FLASH_VOLUMES], pgray[MAX_FLASH_VOLUMES],
        pcsf[MAX_FLASH_VOLUMES];

    printf("\n");
    for (i = 0; i < ep->nvolumes; i++) {
      mri = ep->mri_flash[i];
      pwhite[i] = image_forward_model(
          mri->tr, mri->flip_angle, -1, cortical_dist, best_T1_wm, best_PD_wm,
          best_T1_gm, best_PD_gm, best_T1_csf, best_PD_csf);
      pgray[i] =
          image_forward_model(mri->tr, mri->flip_angle, cortical_dist / 2,
                              cortical_dist, best_T1_wm, best_PD_wm, best_T1_gm,
                              best_PD_gm, best_T1_csf, best_PD_csf);
      pcsf[i] =
          image_forward_model(mri->tr, mri->flip_angle, cortical_dist + 1,
                              cortical_dist, best_T1_wm, best_PD_wm, best_T1_gm,
                              best_PD_gm, best_T1_csf, best_PD_csf);
      printf("\tpredicted image intensities %d: (%2.0f, %2.0f, %2.0f)\n", i,
             ep->scale * pwhite[i], ep->scale * pgray[i], ep->scale * pcsf[i]);
    }

    printf("v %d: best_white_delta = %2.2f, best_pial_delta = %2.2f, best_sse "
           "= %2.2f\n",
           vno, *pwhite_delta, *ppial_delta, best_sse);
    printf("      inward_dist = %2.2f, outward_dist = %2.2f, cortical_dist = "
           "%2.2f\n",
           inward_dist, outward_dist, orig_cortical_dist);
    cortical_dist = best_pial_dist - best_white_dist;
    sse = compute_vertex_sse(ep, image_vals, max_j, best_white_dist,
                             cortical_dist, best_T1_wm, best_PD_wm, best_T1_gm,
                             best_PD_gm, best_T1_csf, best_PD_csf, plot_stuff,
                             T1_vals, PD_vals, vno);
    if (!isfinite(sse))
      ErrorPrintf(ERROR_BADPARM, "sse not finite at v %d", vno);
  }
...
static double compute_vertex_sse(
  ...
  if (debug) {
    ...
    fp = fopen(fname, "w");
    fprintf(fp,
            "call %d: T1 = (%2.0f, %2.0f, %2.0f), PD = (%2.0f, %2.0f, %2.0f)\n",
...
static double compute_optimal_vertex_positions(MRI_SURFACE *mris, int vno,
  ...
  double predicted_val, dx, dy, dz, dist, inward_dist, outward_dist,
      ...
      pial_dist, sse, best_sse, best_white_dist, best_pial_dist,
      orig_cortical_dist, total_dist, orig_white_index, orig_pial_index, sigma,
      pial_delta, wm_delta, gw_border, pial_border;
  int i, j, white_vno, pial_vno, max_j, best_white_index, best_pial_index,
      pial_index, white_index, max_white_index, nwm, npial;
  VERTEX *v_white, *v_pial;
  MRI *mri;
  double image_vals[MAX_FLASH_VOLUMES][MAX_SAMPLES], xw, yw, zw, xp, yp, zp, x,
      y, z;
  double pwhite[MAX_FLASH_VOLUMES], pgray[MAX_FLASH_VOLUMES],
      pcsf[MAX_FLASH_VOLUMES], T1_vals[MAX_SAMPLES], PD_vals[MAX_SAMPLES];
  ...
  for (i = 0; i < ep->nvolumes; i++) {
    ...
    if ((plot_stuff == 1 && Gdiag_no == vno) || plot_stuff > 1) {
      ...
      sprintf(fname, "vol%d.plt", i);
      fp = fopen(fname, "w");
      for (j = 0, dist = -inward_dist;
           dist <= orig_cortical_dist + outward_dist; dist += ep->dstep, j++)
        fprintf(fp, "%d %f %f\n", j, dist, image_vals[i][j]);
      fclose(fp);
    }
  }
  if ((plot_stuff == 1 && Gdiag_no == vno) || plot_stuff > 1) {
    ...
    fclose(fp);
    fp = fopen("orig_pial.plt", "w");
    fprintf(fp, "%f %f %f\n%f %f %f\n", orig_pial_index, orig_cortical_dist,
            0.0, orig_pial_index, orig_cortical_dist, 120.0);
    fclose(fp);
  }
  ...
  cortical_dist = best_pial_dist - best_white_dist;
  best_sse = compute_vertex_sse(ep, image_vals, max_j, best_white_dist,
                                cortical_dist, T1_wm, PD_wm, T1_gm, PD_gm,
                                T1_csf, PD_csf, 0, T1_vals, PD_vals, vno);
  for (white_index = 0; white_index <= max_white_index; white_index++) {
    white_dist = white_index * ep->dstep;
    for (pial_index = white_index + nint(1.0 / ep->dstep); pial_index <= max_j;
         pial_index++) {
      pial_dist = pial_index * ep->dstep;
      cortical_dist = pial_dist - white_dist;
      sse = compute_vertex_sse(ep, image_vals, max_j, white_dist, cortical_dist,
  ...
  for (nwm = npial = 0, pial_delta = wm_delta = 0.0, i = 0; i < ep->nvolumes;
       i++) {
    double I0, I1, Idist;

    mri = ep->mri_flash[i];
    pwhite[i] = image_forward_model(mri->tr, mri->flip_angle, -1, cortical_dist,
                                    T1_wm, PD_wm, T1_gm, PD_gm, T1_csf, PD_csf);
    pgray[i] = image_forward_model(mri->tr, mri->flip_angle, cortical_dist / 2,
                                   cortical_dist, T1_wm, PD_wm, T1_gm, PD_gm,
                                   T1_csf, PD_csf);
    pcsf[i] = image_forward_model(mri->tr, mri->flip_angle, cortical_dist + 1,
                                  cortical_dist, T1_wm, PD_wm, T1_gm, PD_gm,
                                  T1_csf, PD_csf);
    ...
    if (best_white_index > 0) {
      ...
      if (fabs(gw_border - I0) < fabs(Idist) &&
          fabs(gw_border - I1) <
              fabs(Idist)) { /* predicted value is between best and previous */
  ...
  *ppial_delta = (best_pial_dist - (orig_pial_index * ep->dstep));
  ...
  if ((abs(vno - Gdiag_no) < 100) &&
      (ep->scale * image_vals[3][nint(orig_pial_index)] < 100) &&
      (*ppial_delta > 0.5)) {
    FILE *fp;
    fp = fopen("v.log", "a");
    fprintf(
        fp,
        "v %d, T1=(%2.0f,%2.0f,%2.0f), PD=(%2.0f,%2.0f,%2.0f), D=%2.1f,%2.1f\n",
        vno, T1_wm, T1_gm, T1_csf, PD_wm, PD_gm, PD_csf, *pwhite_delta,
        *ppial_delta);
    fclose(fp);
    DiagBreak();
  }
  ...
  if (vno == Gdiag_no && debug_flag) {
    ...
    for (i = 0; i < ep->nvolumes; i++) {
      ...
      pwhite[i] =
          image_forward_model(mri->tr, mri->flip_angle, -1, cortical_dist,
                              T1_wm, PD_wm, T1_gm, PD_gm, T1_csf, PD_csf);
      pgray[i] = image_forward_model(mri->tr, mri->flip_angle,
                                     cortical_dist / 2, cortical_dist, T1_wm,
                                     PD_wm, T1_gm, PD_gm, T1_csf, PD_csf);
      pcsf[i] = image_forward_model(mri->tr, mri->flip_angle, cortical_dist + 1,
                                    cortical_dist, T1_wm, PD_wm, T1_gm, PD_gm,
                                    T1_csf, PD_csf);
      printf("\tpredicted image intensities %d: (%2.0f, %2.0f, %2.0f)\n", i,
    ...
    printf("v %d: best_white_delta = %2.2f (%2.2f), best_pial_delta = %2.2f "
           "(%2.2f), best_sse = %2.2f\n",
           vno, *pwhite_delta,
           (best_white_dist - (orig_white_index * ep->dstep)), *ppial_delta,
    ...
    cortical_dist = best_pial_dist - best_white_dist;
    sse = compute_vertex_sse(ep, image_vals, max_j, best_white_dist,
                             cortical_dist, T1_wm, PD_wm, T1_gm, PD_gm, T1_csf,
                             PD_csf, plot_stuff, T1_vals, PD_vals, vno);
  }
...
static int compute_T1_PD(double *image_vals, MRI **mri_flash, int nvolumes,
  ...
  if (!norms) {
    ...
    for (j = 0, T1 = MIN_T1; T1 < MAX_T1; T1 += T1_STEP_SIZE, j++) {
      for (norm_pred = 0.0, i = 0; i < nvolumes; i++) {
        mri = mri_flash[i];
        pred_vals[i] =
            FLASHforwardModelLookup(mri->flip_angle, mri->tr, 1.0, T1);
  ...
  for (mid_sse = 0.0, i = 0; i < nvolumes; i++) {
    mri = mri_flash[i];
    pred_vals[i] =
        FLASHforwardModelLookup(mri->flip_angle, mri->tr, 1.0, mid_T1);
  ...
  do {
    ...
    for (lower_sse = upper_sse = 0.0, i = 0; i < nvolumes; i++) {
      ...
      pred_vals[i] =
          FLASHforwardModelLookup(mri->flip_angle, mri->tr, 1.0, upper_T1);
      pred_vals[i] /= upper_norm; /* normalize them */
      error = (pred_vals[i] - image_vals[i]);
      upper_sse += (error * error);

      pred_vals[i] =
          FLASHforwardModelLookup(mri->flip_angle, mri->tr, 1.0, lower_T1);
  ...
  for (sse = 0.0, i = 0; i < nvolumes; i++) {
    mri = mri_flash[i];
    pred_vals[i] =
        FLASHforwardModelLookup(mri->flip_angle, mri->tr, best_PD, best_T1);
    error = (pred_vals[i] - image_vals[i]);
    sse += (error * error);
  }
...
static double scale_all_images(MRI **mri_flash, int nvolumes, MRI_SURFACE *mris,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    T1_wm_total += T1;
    PD_wm_total += PD;
    if (!isfinite(T1) || !isfinite(PD) || !isfinite(T1_wm_total) ||
        !isfinite(PD_wm_total))
      DiagBreak();
    ep->cv_wm_T1[vno] = T1;
    ep->cv_wm_PD[vno] = PD;
...
static int update_parameters(MRI_SURFACE *mris, EXTRA_PARMS *ep) {
  int vno, nmarked = 0;
  VERTEX *v;
  ...
  if (Gdiag_no > 0 && mris->vertices[Gdiag_no].marked > 0) {
    ...
    printf("v %d: wm = (%2.0f, %2.0f), gm = (%2.0f, %2.0f), "
           "csf = (%2.0f, %2.0f)\n",
           vno, ep->cv_wm_T1[vno], ep->scale * ep->cv_wm_PD[vno],
           ep->cv_gm_T1[vno], ep->scale * ep->cv_gm_PD[vno], ep->cv_csf_T1[vno],
  ...
  for (vno = MIN_VNO; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no)
      printf("v %d: updating inward dist by %2.1f to %2.1f (d=%2.2f)\n", vno,
             dot, parms->cv_inward_dists[vno], v->d);
  }
  return (NO_ERROR);
}
static int constrain_parameters(int nvertices, EXTRA_PARMS *ep) {
...
static int ms_errfunc_rip_vertices(MRI_SURFACE *mris,
  ...
  printf("%d vertices ripped (%2.3f%% of surface)\n", ripped,
         100.0f * (float)ripped / mris->nvertices);
  if (Gdiag_no > 0)
...
static int compute_PD_T1_limits(MRI_SURFACE *mris, EXTRA_PARMS *ep, int navgs) {
  ...
  double x, y, z, T1, PD, PD_min, PD_max, T1_min, T1_max, n, cortical_dist, xp,
      yp, zp, xw, yw, zw, dx, dy, dz;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRIsurfaceRASToVoxel(ep->mri_T1, v_white->origx, v_white->origy,
                         v_white->origz, &xw, &yw, &zw);
    MRIsurfaceRASToVoxel(ep->mri_T1, v_pial->pialx, v_pial->pialy,
                         v_pial->pialz, &xp, &yp, &zp);
    dx = xp - xw;
    dy = yp - yw;
    dz = zp - zw;
