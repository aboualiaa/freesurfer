...
static int    nbrs             = 3;
// static float    weight_quadcur = 0.0, weight_label = 1.0, weight_repulse =
// 0.0, weight_Nspring = 0.1, weight_Tspring = 0.1;
...
int main(int argc, char *argv[]) {
  ...
  float energy_new = 0;
  float energy_old = 0;
  // float         weight_quadcur = 0.2, weight_label = 0.5, weight_repulse =
  // 0.0,
...
static int mrisComputeLabelTerm1(MRI_SURFACE *mris, double weight_label,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    MRIsampleVolumeDerivative(mri_smooth[table[0][vno]], xw, yw, zw, nx, ny, nz,
                              &dn);
    MRIsurfaceRASToVoxel(mri_orig, x + 0.5 * v->nx, y + 0.5 * v->ny,
                         z + 0.5 * v->nz, &xw, &yw, &zw);
    MRIsampleVolumeType(mri_label[table[0][vno]], xw, yw, zw, &outlabel,
                        SAMPLE_NEAREST);
    ...
    MRIsampleVolumeDerivative(mri_smooth[table[1][vno]], xw, yw, zw, nx, ny, nz,
                              &dn);
    MRIsurfaceRASToVoxel(mri_orig, x - 0.5 * v->nx, y - 0.5 * v->ny,
                         z - 0.5 * v->nz, &xw, &yw, &zw);
    MRIsampleVolumeType(mri_label[table[1][vno]], xw, yw, zw, &inlabel,
                        SAMPLE_NEAREST);
...
static double mrisComputeLabelEnergy(MRI_SURFACE *mris, MRI *mri_label[5],
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (outval != target_O || inval != target_I)
      fprintf(stderr, "vertex %d is not correct: outval=%f inval=%f \n", vno,
              outval, inval);
    energy += (target_O - outval) * (target_O - outval) +
              (target_I - inval) * (target_I - inval);
#endif
  }
  return (energy);
}
...
static int mrisExaminemovelength(MRI_SURFACE *mris) {
  ...
  for (k = 0; k < mris->nvertices; k++) {
    ...
    MRISsetXYZ(mris, k, v->x + v->odx, v->y + v->ody, v->z + v->odz);
#endif
  }
  ...
  if (stricmp(option, "an option") == 0) {
    ...
    case 'P':
      ...
      printf("changing term weighting to %2.2f %2.2f %2.2f %2.2f %2.2f\n",
             weight_quadcur, weight_label, weight_repulse, weight_Nspring,
             weight_Tspring);
      nargs = 5;
      break;
    case 'M':
...
static int FindSpikes(MRI_SURFACE *mris, int iter) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if ((fabs(vertex->curv) >= 3) || (vertex->K * vertex->K >= 4) ||
        fabs(vertex->k1) >= 3 || fabs(vertex->K) < 0.01) {
      if (vertex->marked == 0) {
        counter++;
        vertex->marked = 1;
        if (iter == 0) {
  ...
  for (i = 0; i < niter; i++) {
    ...
    for (j = 0; j < 2; j++) {
      for (vno = 0; vno < nvertices; vno++) {
        ...
        if ((fabs(vertex->curv) >= 4) || (vertex->K * vertex->K >= 4) ||
            fabs(vertex->k1) >= 4 || fabs(vertex->K) < 0.01) {
          int   num = 1; /* account for central vertex */
          float x   = px[vno];
          float y   = py[vno];
