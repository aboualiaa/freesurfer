...
static int dump_window(MRI *mri, const char *fname, int x0, int y0, int z0,
                       int wsize);
...
int main(int argc, char *argv[]) {
  ...
  if (training_src_fname) {
    ...
    if (!mri_train_src)
      ErrorExit(ERROR_NOFILE, "%s: could not read training src file %s",
                Progname, training_src_fname);
    mri_train_dst = MRIread(training_dst_fname);
    if (!mri_train_dst)
      ErrorExit(ERROR_NOFILE, "%s: could not read training dst file %s",
                Progname, training_dst_fname);
  ...
  seconds = seconds % 60;
  fprintf(stderr,
          "image synthesis took %d minutes"
          " and %d seconds.\n",
          minutes, seconds);
  exit(0);
  return (0);
    ...
    for (y = 0; y < mri_train_src->height; y++) {
      for (z = 0; z < mri_train_src->depth; z++) {
        ...
        if (FZERO(sval))
          continue;
        if ((val < 240) && (val != 4080) &&
            MRIindexNotInVolume(mri_train_dst, xh, yh, zh) == 0) {
  ...
  if (flags & MRI_SPACE) {
    ...
    for (x = box_test.x; x < box_test.x + box_test.dx; x++) {
      ...
      for (y = box_test.y; y < box_test.y + box_test.dy; y++) {
        ...
        find_most_similar_location(mri, f, train_slice, &xd, &yd, &zd, mri_mask,
                                   &box_train, flags, xind, yind, zind, nind,
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(m_histo2mri, m_mri2histo, xind, yind, zind, hsynth, y, val,   \
                 histo, test_slice, Gx, Gy, xm, ym, zm, identity, mri_mask,    \
                 mri, train_slice, min_training_dist, tol, num_notfound, xh,   \
                 yh, zh, xd, yd, zd, f, farray) shared(v1, v2)                 \
        schedule(static, 1)
#endif
    for (x = 0; x < width; x++) // synth is in histo coords
    {
      ...
      for (y = 0; y < hsynth->height; y++) {
        ...
        if (identity) {
          ...
          xm = nint(V3_X(v2));
          ym = nint(V3_Y(v2));
          zm = nint(V3_Z(v2)); // MRI coord
        }

        extract_feature_vector(mri, xm, ym, zm, f);
        find_most_similar_location(
            mri_train_src, f, train_slice, &xd, &yd, &zd, mri_mask, &box_train,
            flags, xind, yind, zind, nind, tol, num_notfound, xm, ym, zm, 0);
        if (x == Gx && y == Gy && test_slice == Gz) {
  ...
  for (x = 0; x < _MAX_FS_THREADS; x++)
    free(farray[x].vals);
  MatrixFree(&m_histo2mri);
  MatrixFree(&m_mri2histo);
  MatrixFree(&v1);
  MatrixFree(&v2);
  return (hsynth);
}
...
static int find_most_similar_location(MRI *mri, FEATURE *fsrc, int z, int *pxd,
  ...
  dist = 0 ; x = y = 0 ;
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(nind, x, y, x1, y1, Gx, Gy, Gz, box, x0, y0, fsrc, f, flags,  \
                 min_training_dist, dist, num_notfound) shared(min_dist, num)  \
        schedule(static, 1)
#endif
#endif
  for (ind = 0; ind < nind; ind++) {
    ...
    if (MRIgetVoxVal(mri_mask, x, y, z, 0) == 0)
      continue;
    if ((mri->depth == 1) && ((abs(x - x0) < min_training_dist) ||
                              (abs(y - y0) < min_training_dist)))
