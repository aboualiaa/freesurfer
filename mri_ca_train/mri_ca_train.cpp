...
static char *histo_fname = nullptr;
...
int main(int argc, char *argv[]) {
  ...
  Timer start;
  GCA *gca, *gca_prune = nullptr;
  MRI *mri_seg, *mri_tmp, *mri_eq = nullptr, *mri_inputs;
  TRANSFORM *transform;
  LTA *lta;
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_ca_train.c,v 1.70 2015/07/27 20:52:08 greve Exp $",
      "$Name:  $");
  ...
  try {
    ...
    do {
      ...
      gca = GCAalloc(gca_inputs, parms.prior_spacing, parms.node_spacing,
                     DEFAULT_VOLUME_SIZE, DEFAULT_VOLUME_SIZE,
                     DEFAULT_VOLUME_SIZE, gca_flags);
      ...
      for (nargs = i = 0; i < nsubjects + options; i++) {
        ...
        if (stricmp(subject_name, "-NOINT") == 0) {
          printf(
              "not using intensity information for subsequent subjects...\n");
          noint = 1;
          nargs++;
          continue;
                       ...
                       reorder it to be in the same order as 1st */
        {
          ...
          if (counts != ninputs) {
            ...
            printf("Input TR, TE, FlipAngle for each subjects do not match, "
                   "but mismatch allowed.\n");
          }
        } else
          for (o = 0; o < ninputs; o++)
            ordering[o] = o;
        ...
        for (input = 0; input < ninputs; input++) {
          ...
          if (input == 0) {
            ...
            ///////////////////////////////////////////////////////////
            mri_inputs =
                MRIallocSequence(mri_tmp->width, mri_tmp->height,
        ...
        if (map_to_flash) {
          ...
          mri_tmp = MRIparameterMapsToFlash(mri_inputs, nullptr, TRs, TEs, FAs,
                                            gca_inputs);
          MRIfree(&mri_inputs);
          mri_inputs = mri_tmp;
        }
      ...
      for (nargs = i = 0; i < nsubjects + options; i++) {
        ...
        printf("computing covariances for subject %s, %d of %d...\n",
               subject_name, i + 1 - nargs, nsubjects);
        sprintf(fname, "%s/%s/mri/%s", subjects_dir, subject_name, seg_dir);
        if (DIAG_VERBOSE_ON)
    ...
    if (GCAwrite(gca, out_fname) != NO_ERROR)
      ErrorExit(ERROR_BADFILE, "%s: could not write gca to %s", Progname,
                out_fname);

    {
      // Why is this here?
      MRI *mri;
      mri = GCAbuildMostLikelyVolume(gca, nullptr);
      MRIfree(&mri);
    }
    ...
    if (histo_fname) {
      ...
      if (!fp)
        ErrorExit(ERROR_BADFILE, "%s: could not open histo file %s", Progname,
                  histo_fname);
...
static void usage_exit(int code) {
  ...
  printf(
      ...
      "\t -makesym input.gca symmetrized.gca : symmetrize an already existing "
      "atlas "
      "\t -checksym input.gca symmetrized.gca : check the symmetry of an "
      "already existing atlas"
...
static void modify_transform(TRANSFORM *transform, MRI *mri_inputs, GCA *gca) {
  ...
  mri_buf = MRIallocHeader(mri_inputs->width, mri_inputs->height,
                           mri_inputs->depth, mri_inputs->type, 1);
  ...
  if (transform->type == MORPH_3D_TYPE) {
    ...
    if (gcam->atlas.valid) // means it contains the dst volume information
    {
      ...
      if (warned == 0) {
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
          fprintf(stderr, "INFO: modified c_(r,a,s) using the non-linear "
                          "transform dst value.\n");
        warned = 1;
      }
  ...
  ////////////////////////////////////////////////////////////////////////////
                                       ...
                                       // transform)
    {
      ...
      if (warned == 0) {
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
          ...
          fprintf(stderr, "INFO: if this is not preferred, set environment "
    ...
    } else // keep old behavior
    {
      ...
      if (warned == 0) {
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
          fprintf(stderr,
                  "INFO: xform.dst invalid thus modified c_(r,a,s) = 0.\n");
        warned = 1;
      }
    }
    /////////////////////////////////////////////////////////////////
