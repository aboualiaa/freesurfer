    ...
    do {
      ...
      for (nargs = i = 0; i < nsubjects + options; i++) {
        ...
        if (map_to_flash) {
          ...
          mri_tmp = MRIparameterMapsToFlash(mri_inputs, NULL, TRs, TEs, FAs,
                                            gca_inputs);
          MRIfree(&mri_inputs);
          mri_inputs = mri_tmp;
        }
...
static void configure_transform(TRANSFORM *transform, MRI *src, GCA *gca) {
  ...
  if (geom->valid) {
    ...
    bool in305space =
        ((transform->type == LINEAR_RAS_TO_RAS) && (getenv("USE_AVERAGE305")));
    if (in305space && (!warned)) {
      fprintf(stdout, "INFO: Using average_305 CRAS. Disable by unsetting "
                      "USE_AVERAGE305 env variable.\n");
      warned = true;
    }
