/*
  ...
  perform_front_propagation_3d - perform a fast marching front propagation.
    ...
    'start_points' is a 3 x num_start_points matrix where k is the number of
  starting points. 'H' is an heuristic (distance that remains to goal). This is
  a 2D matrix.
...
/*
  ...
  from given starting points. This is a python port of the
  perform_front_propagation_3d tool written by Gabriel Peyre.
*/
pyarrayd performFrontPropagation3D(const pyarrayd &weights,
                                   const pyarrayd &start, int max_iters,
                                   const pyarrayd &values) {
  Propagator prop;
  ...
  if (values.shape(0) != prop.nb_start_points)
    throw std::runtime_error(
        "len of values does not equal number of start points");
...
bool Propagator::end_points_reached(const int i, const int j, const int k) {
  for (int s = 0; s < nb_end_points; ++s) {
    if (i == ((int)end_points_(0, s)) && j == ((int)end_points_(1, s)) &&
        k == ((int)end_points_(2, s))) {
      return true;
    }
  }
  return false;
...
void Propagator::perform_front_propagation_3d(
    T_callback_intert_node callback_insert_node) {
  ...
  while (!fh_isempty(open_heap) && num_iter < nb_iter_max && !stop_iteration) {
    ...
    for (int s = 0; s < 6; ++s) {
      ...
      if (ii >= 0 && jj >= 0 && ii < (int)n && jj < (int)p && kk >= 0 &&
          kk < (int)q && bInsert) {
...
#define SWAPIF(a, b)                                                           \
  if (a > b) {                                                                 \
    SWAP(a, b);                                                                \
        ...
        SWAPIF(a2, a3)
        // update its distance
        // now the equation is   (a-a1)^2+(a-a2)^2+(a-a3)^2 - P^2 = 0, with a >=
        // a3 >= a2 >= a1.
        // =>    3*a^2 - 2*(a2+a1+a3)*a - P^2 + a1^2 + a3^2 + a2^2
        // => delta = (a2+a1+a3)^2 - 3*(a1^2 + a3^2 + a2^2 - P^2)
        double delta = (a2 + a1 + a3) * (a2 + a1 + a3) -
                       3 * (a1 * a1 + a2 * a2 + a3 * a3 - P * P);
        double A1 = 0;
        if (delta >= 0) {
        ...
        if (((int)S_(ii, jj, kk)) == kDead) {
          ...
          if (A1 < D_(ii, jj, kk)) {
            ...
            if (cur_el != NULL) {
              fh_replacedata(open_heap, cur_el,
                             cur_el->fhe_data); // use same data for update
            } else {
          ...
          if (D_(ii, jj, kk) != GW_INFINITE) {
            std::cerr << "warning: distance must be initialized to Inf."
                      << std::endl;
          }
  ...
  // free point pool
  for (point_list::iterator it = existing_points.begin();
       it != existing_points.end(); ++it) {
    GW_DELETE(*it);
  }
