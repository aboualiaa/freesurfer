/*
  ...
  The original continuous max-flow algorithm was proposed in the following
  papers:
  ...
  Please email Jing Yuan (cn.yuanjing@gmail.com) for any questions, suggestions
  and bug reports
...
pyarrayf CMF3D_ML(const pyarrayf &bound, int iNbIters, float fError, float cc,
  ...
  pfbx =
      (float *)calloc((unsigned)(iNy * (iNx + 1) * iNz * iLab), sizeof(float));
  if (!pfbx)
    throw std::runtime_error("memory allocation failure");

  // allocate the memory for py1
  pfby =
      (float *)calloc((unsigned)((iNy + 1) * iNx * iNz * iLab), sizeof(float));
  if (!pfby)
    throw std::runtime_error("memory allocation failure");

  // allocate the memory for pz1
  pfbz =
      (float *)calloc((unsigned)(iNy * iNx * (iNz + 1) * iLab), sizeof(float));
  if (!pfbz)
    throw std::runtime_error("memory allocation failure");
        ...
        for (ix = 0; ix < iNx; ix++) {
          ...
          for (iy = 0; iy < iNy; iy++) {
            ...
            fpt = SQRT((SQR(pfbx[index1 + iNy]) + SQR(pfbx[index1]) +
                        SQR(pfby[index1 + 1]) + SQR(pfby[index1]) +
                        SQR(pfbz[index1 + S2D]) + SQR(pfbz[index1])) *
                       0.5);
