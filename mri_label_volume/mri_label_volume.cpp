  ...
  for (i = 2; i < argc; i++) {
      ...
      for (label = 0; label <= MAX_CMA_LABEL; label++) {
        ...
        if (partial_volume)
          volume += MRIvoxelsInLabelWithPartialVolumeEffects(
              mri, mri_vals, label, nullptr, nullptr);
        else
    ...
    if (compute_pct || icv_fname || atlas_icv > 0) {
      printf("%d voxels (%2.1f mm^3) in label %d, "
             ...
             atlas_icv > 0 ? "eTIV" : "brain", brain_volume);
      if (log_fp) {
        if (spread_sheet)
          fprintf(log_fp, "%2.6f ",
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "ICV")) {
    ...
    if (!stricmp(option, "eTIV_matdat")) {
      ...
      if (dat_fp) {
        fprintf(dat_fp, "eTIV_scale_factor = %f;\n", eTIV_scale_factor);
        fprintf(dat_fp, "det_eTIV(length(det_eTIV)+1).det = %f;\n", atlas_det);
        fprintf(dat_fp, "det_eTIV(length(det_eTIV)).eTIV = %f;\n", atlas_icv);
        fprintf(dat_fp, "det_eTIV(length(det_eTIV)).id = '%s';\n\n\n", argv[4]);
        fclose(dat_fp);
      }
      nargs = 3;
    }
