 ...
 *
 *
 * Program to compute a rigid alignment between a surface and a label by
...
int main(int argc, char **argv) {
  ...
  if (regfile != NULL) {
    ...
    if (lta == NULL)
      ErrorExit(ERROR_NOFILE, "%s: could not load initial registration from %s",
                regfile);
  ...
  find_optimal_linear_xform(
      mri_dist, mri_src, area, R0, -max_rot, max_rot, min_scale, max_scale,
      -max_trans, max_trans, angle_steps, scale_steps, trans_steps, 0,
      cost_function, tx0, ty0, tz0, ax0, ay0, az0, mri_targ_vol);
  printf("writing final matrix to %s:\n", outregfile);
  MatrixPrint(stdout, R0);
  if (TransformFileNameType(outregfile) == REGISTER_DAT)
    regio_write_surfacexform_to_register_dat(R0, outregfile, mris, mri_src,
...
LABEL *LabelFromVoxelCoords(LABEL *lsrc, MRI *mri, LABEL *ldst,
                            MATRIX *Mvox2ras) {
  int i;
  //  double r, a, s ;
  static VECTOR *v1 = NULL, *v2;
...
static double compute_error(MRI *mri_dist, MATRIX *R0, MATRIX *Mras2vox,
  ...
  case COST_MAX:
    ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) firstprivate(val, mri_dist)     \
    shared(lvol) schedule(static, 1)
#endif
        for (i = 0; i < lvol->n_points; i++) {
      ROMP_PFLB_begin MRIsampleVolume(mri_dist, lvol->lv[i].x, lvol->lv[i].y,
                                      lvol->lv[i].z, &val);
      lvol->lv[i].stat = val;
    ...
    ROMP_PF_end
        ...
        if (!FZERO(max_val)) return (max_val);

    for (i = 0; i < lvol->n_points; i++) {
      MRIsampleVolume(mri_dist, lvol->lv[i].x, lvol->lv[i].y, lvol->lv[i].z,
                      &val);
      if (val > max_val) {
        max_val = val;
        if (val > current_error)
...
#pragma omp parallel for if_ROMP(experimental) firstprivate(val, mri_dist) shared(lvol) schedule(static,1) reduction(+: sse)
    ...
    ROMP_PF_end return (sqrt(sse / lras->n_points));
    break;
  case COST_L1:
    ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) firstprivate(val, mri_dist) shared(lvol) schedule(static,1) reduction(+: sse)
    ...
    ROMP_PF_end return (sse / lras->n_points);
    break;
  }
...
static double find_optimal_linear_xform(
  ...
  double mean_angle, x_angle, y_angle, z_angle, x_scale, y_scale, z_scale,
      min_z_scale, max_z_scale, delta_z_scale;
  double mean_scale, x_max_trans, y_max_trans, z_max_trans, mean_trans, x_trans,
      y_trans, z_trans;
  int i, nfound = 0;
  ...
  if (mri_targ_vol &&
      regfile == NULL) // if registration already specified, don't mess with it
  {
  ...
  *MATRIX_RELT(m_origin_inv, 3, 4) *= -1;
  ...
  for (i = 0; i < nreductions + 1; i++) {
    ...
    if (write_diags) {
      ...
      printf("writing new optimal label to %s, v(0): RAS=%2.1f %2.1f %2.1f, "
             "VOX=(%d %d %d)\n",
             fname, l->lv[0].x, l->lv[0].y, l->lv[0].z, nint(lvol->lv[0].x),
             nint(lvol->lv[0].y), nint(lvol->lv[0].z));
      LabelWrite(l, fname);
      if (debug)
      ...
      MRIfree(&mri_tmp);
      LabelFree(&l);
      MatrixFree(&OCT_voxel_to_RAS);
      MatrixFree(&OCT_voxel_to_xformed_RAS);
      ...
      if (mri_targ_vol) {
        ...
        MRIfree(&mri_tmp);
        MatrixFree(&MRI_voxel_to_RAS);
        MatrixFree(&MRI_voxel_to_xformed_RAS);
        ...
        if (TransformFileNameType(outregfile) == REGISTER_DAT)
          regio_write_surfacexform_to_register_dat(m_best, fname, mris, mri_src,
                                                   subject, FLT2INT_ROUND);
        else
      ...
      *MATRIX_RELT(m_scale, 1, 1) = x_scale;
      for (y_scale = min_scale; y_scale <= max_scale; y_scale += delta_scale) {
        *MATRIX_RELT(m_scale, 2, 2) = y_scale;
        printf("y_scale = %2.3f\n", y_scale);
        for (z_scale = min_z_scale; z_scale <= max_z_scale;
          ...
          *MATRIX_RELT(m_scale, 1, 4) = *MATRIX_RELT(m_scale, 2, 4) =
          ...
          // angle //////////////////////////////
          for (x_angle = min_angle + ax0; x_angle <= max_angle + ax0;
               x_angle += delta_rot) {
            if (debug)
              x_angle = RADIANS(-9);
            printf("------------ x_angle = %2.3f ----------------\n",
            ...
            for (y_angle = min_angle + ay0; y_angle <= max_angle + ay0;
                 y_angle += delta_rot) {
              if (debug)
                y_angle = RADIANS(9.5);
              ...
              for (z_angle = min_angle + az0; z_angle <= max_angle + az0;
                ...
                // translation //////////
                for (x_trans = min_trans + tx0; x_trans <= max_trans + tx0;
                     x_trans += delta_trans) {
                  if (debug)
                    x_trans = -9;
                  *MATRIX_RELT(m_trans, 1, 4) = x_trans;
                  for (y_trans = min_trans + ty0; y_trans <= max_trans + ty0;
                       y_trans += delta_trans) {
                    if (debug)
                      y_trans = -5;
                    *MATRIX_RELT(m_trans, 2, 4) = y_trans;
                    for (z_trans = min_trans + tz0; z_trans <= max_trans + tz0;
                         z_trans += delta_trans) {
                      if (debug)
                        z_trans = -5;
                      *MATRIX_RELT(m_trans, 3, 4) = z_trans;
                      ...
                      if (error < min_error || debug) {
                        ...
                        printf("%s: Rotation (%4.3f, %4.3f, %4.3f)\n",
                               __FUNCTION__, DEGREES(x_angle), DEGREES(y_angle),
                               DEGREES(z_angle));
                        ...
                        if (write_diags) {
                          ...
                          // compute scanner vox2ras
                          OCT_voxel_to_RAS = MRIgetVoxelToRasXform(mri_src);
                          OCT_voxel_to_xformed_RAS = MatrixMultiply(
                              m_best_inv, OCT_voxel_to_RAS, NULL);
                          mri_tmp = MRIcopy(mri_src, NULL);
                          MRIsetVoxelToRasXform(mri_tmp,
                                                OCT_voxel_to_xformed_RAS);

                          if (debug)
                            sprintf(fname, "%s.opt.%4.4d.label", "debug",
                                    ++nfound);
                          else
                            sprintf(fname, "%s.opt.%4.4d.label", base_name,
                                    ++nfound);
                          printf("writing new optimal label to %s, v(0): "
                                 "RAS=%2.1f %2.1f %2.1f, VOX=(%d %d %d)\n",
                                 fname, l->lv[0].x, l->lv[0].y, l->lv[0].z,
                                 nint(lvol->lv[0].x), nint(lvol->lv[0].y),
                                 nint(lvol->lv[0].z));
                          LabelWrite(l, fname);
                          if (debug)
                            sprintf(fname, "%s.opt.%4.4d.mgz", "debug", nfound);
                          else
                            sprintf(fname, "%s.opt.%4.4d.mgz", base_name,
                                    nfound);
                          printf("saving %s\n", fname);
                          MRIwrite(mri_tmp, fname);
                          ...
                          if (mri_targ_vol) {
                            ...
                            MRI_voxel_to_RAS =
                                MRIgetVoxelToRasXform(mri_targ_vol);
                            MRI_voxel_to_xformed_RAS =
                                MatrixMultiply(m_L_tmp, MRI_voxel_to_RAS, NULL);
                            mri_tmp = MRIcopy(mri_targ_vol, NULL);
                            MRIsetVoxelToRasXform(mri_tmp,
                                                  MRI_voxel_to_xformed_RAS);
                            sprintf(fname, "%s.targ.%4.4d.mgz", base_name,
                                    nfound);
                            printf("writing reoriented target volume to %s\n",
                                   fname);
                            MRIwrite(mri_tmp, fname);
                            ...
                            FileNameExtension(outregfile, ext);
                            FileNameRemoveExtension(outregfile, fname_only);
                            sprintf(fname, "%s.%4.4d.%s", fname_only, nfound,
                                    ext);
                            printf("writing intermediate registration to %s\n",
                                   fname);

                            if (TransformFileNameType(outregfile) ==
                                REGISTER_DAT)
                              regio_write_surfacexform_to_register_dat(
                                  m_best, fname, mris, mri_src, subject,
                                  FLT2INT_ROUND);
                            else
                              write_lta(m_best, fname, mri_dist, mri_src);
                          }
