...
static int   powell_minimize_rigid(
      MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask, MATRIX *mat, int skip,
      double (*similarity_func)(MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask,
                              MATRIX *m, int skip, double scale, int diag));

static MRI *mri_surface_edges(MRI *mri, MRI *mri_grad, float ndist, float tdist,
                              int dir, MRI *mri_edge);
;
static int    parse_commandline(int argc, char **argv);
static double mrisRegistrationCNRSimilarity(MRI_SURFACE *mris, MRI *mri_reg,
                                            MRI *mri_mask, MATRIX *m, int skip,
                                            double scale, int diag);
static double mrisRegistrationOverlapSimilarity(MRI_SURFACE *mris, MRI *mri_reg,
                                                MRI *mri_mask, MATRIX *m,
                                                int skip, double scale,
                                                int diag);
static double mrisRegistrationDistanceSimilarity(MRI_SURFACE *mris,
                                                 MRI *mri_dist, MRI *mri_mask,
                                                 MATRIX *m, int skip,
                                                 double scale, int diag);
static double mrisRegistrationGradientSimilarity(MRI_SURFACE *mris,
                                                 MRI *mri_reg, MRI *mri_mask,
                                                 MATRIX *m, int skip,
                                                 double scale, int diag);
static double mrisRegistrationGradientSimilarity(MRI_SURFACE *mris,
                                                 MRI *mri_reg, MRI *mri_mask,
                                                 MATRIX *m, int skip,
                                                 double scale, int diag);
static double mrisRegistrationGradientNormalSimilarity(MRI_SURFACE *mris,
                                                       MRI *        mri_reg,
                                                       MRI *mri_mask, MATRIX *m,
                                                       int skip, double scale,
                                                       int diag);
...
static double find_optimal_translations(
    MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask, MATRIX *R0,
    double max_trans, int skip,
    double (*similarity_func)(MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask,
                              MATRIX *m, int skip, double scale, int diag));
static double find_optimal_rotations(
    MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask, MATRIX *R0, double max_rot,
    int skip,
    double (*similarity_func)(MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask,
                              MATRIX *m, int skip, double scale, int diag));
static double find_optimal_rigid_alignment(
    MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask, MATRIX *R0,
    double min_trans, double max_trans, double min_rot, double max_rot,
    int skip,
    double (*similarity_func)(MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask,
                              MATRIX *m, int skip, double scale, int diag));

static double (*similarity_func)(MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask,
                                 MATRIX *m, int skip, double scale, int diag) =
...
int main(int argc, char **argv) {
  ...
  if (regfile) {
    ...
    R0 = regio_read_surfacexform_from_register_dat(regfile, mris, mri_reg,
                                                   &subject);
    if (R0 == NULL)
      exit(Gerror);
  }
  ...
  for (skip = max_skip; skip >= min_skip; skip /= 2) {
    for (sigma = max_sigma; sigma >= min_sigma; sigma /= 2) {
      ...
      if (false) {
        ...
        mri_edge = mri_surface_edges(mri_reduced, mri_reduced_grad, 1.0, 2.0,
                                     1.0, NULL);
        ...
        MRIwrite(mri_edge, "eup.mgz");
        {
          ...
          mri_distance = MRIdistanceTransform(mri_edge, NULL, 1, 1000,
                                              DTRANS_MODE_UNSIGNED, NULL);
          MRIscalarMul(mri_distance, mri_distance, -1);
          MRIwrite(mri_distance, "dist.mgz");
          mri_edge->outside_val = -1000;
          MRIfree(&mri_edge);
          mri_edge = mri_distance;
          HISTOfree(&h);
          HISTOfree(&hcdf);
        }
        ...
        find_optimal_rigid_alignment(mris, mri_edge, mri_mask, R0, 0,
                                     max_trans / tscale, .1, max_rot, skip,
                                     mrisRegistrationOverlapSimilarity);
        MRIfree(&mri_edge);
        MRIfree(&mri_reduced);
      }
      ...
      if (do_global_search != 0) {
        find_optimal_translations(mris, mri_reg, mri_mask, R0, max_trans, skip,
                                  similarity_func);
#if 0
        find_optimal_rotations(mris, mri_reg, mri_mask, R0, max_rot, skip, similarity_func) ;
#endif
        find_optimal_rigid_alignment(mris, mri_reg, mri_mask, R0,
                                     2 * mri_reg->xsize, max_trans / tscale, 1,
                                     max_rot, skip, similarity_func);
        find_optimal_rigid_alignment(
      ...
      do {
        ...
        if (pial_only)
          ...
          similarity = mrisRegistrationCNRSimilarity(mris, mri_reg, mri_mask,
                                                     R0, 0, 1, 0);
      ...
      regio_write_surfacexform_to_register_dat(R0, outregfile, mris, mri_reg,
                                               subject, FLT2INT_ROUND);
  ...
  fprintf(stderr, "registration took %2.1f minutes\n",
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      UseASeg = 1;
    else if (!strcasecmp(option, "--no-crop"))
      DoCrop = 0;
    else if (!strcasecmp(option, "--crop"))
      DoCrop = 1;
    else if (!strcasecmp(option, "--rm"))
      read_median = 1;
    else if (!strcasecmp(option, "--rg"))
      read_grad = 1;
    else if (!strcasecmp(option, "--noglobal"))
      do_global_search = 0;
...
static void print_usage() {
  ...
  printf("  --skip min max  : # of vertices to skip (starting at max and "
         "reducing)\n");
  printf("  --sigma min max  : size of blurring kernels to use (starting at "
         "max and reducing)\n");
  ...
  printf("  --patch <patch>   : load patch <patch> and limit calculations to "
         "it\n");
  printf("  --label <label>   : load label <label> and limit calculations to "
         "it\n");
  printf("\n");
...
static double mrisRegistrationCNRSimilarity(MRI_SURFACE *mris, MRI *mri_reg,
                                            MRI *mri_mask, MATRIX *m, int skip,
                                            double scale, int diag) {
  double similarity, grad, grad_var, grad_mean, xv, yv, zv, nx, ny, nz, mag,
      gm_mean, wm_mean, sample_dist, vertex_similarity, xw, yw, zw, xg, yg, zg;
  ...
  for (num = vno = 0; vno < mris->nvertices; vno++) {
    ...
    {
      num_in_fov++;
      V3_X(v1) = v->whitex + v->nx;
      V3_Y(v1) = v->whitey + v->ny;
      ...
      nx -= xv;
      ny -= yv;
      nz -= zv; // vertex normal in voxel coords
      ...
      ny /= mag;
      nz /= mag;
      MRIsampleVolumeDerivativeScale(mri_reg, xv, yv, zv, nx, ny, nz, &grad,
                                     .5 / mri_reg->xsize);
      xw = xv - sample_dist * nx;
      yw = yv - sample_dist * ny;
      zw = zv - sample_dist * nz;
      xg = xv + sample_dist * nx;
      yg = yv + sample_dist * ny;
      zg = zv + sample_dist * nz;
  ...
  total_contrast = total_noise = 0.0;
  similarity                   = 0;
  for (vno = 0; vno < mris->nvertices; vno += skip) {
    ...
    for (n = 0, num_nbrs = 1; n < vt->vtotal; n++) {
      ...
      gm_mean += vn->val2;
      wm_mean += vn->val;
      gm_var += vn->val2 * vn->val2;
      wm_var += vn->val * vn->val;
    }
    ...
    if (mri) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_reg, V3_X(v2), V3_Y(v2), V3_Z(v2),
                                  &xv, &yv, &zv);
      MRIfillRegion(mri, xv, yv, zv, val < 0 ? -.02 : .02,
                    nint(.25 / mri->xsize));
...
static double mrisRegistrationGradientNormalSimilarity(MRI_SURFACE *mris,
  ...
  for (similarity = 0.0, num = vno = 0; vno < mris->nvertices;
       vno += (skip + 1)) {
    v = &mris->vertices[vno];
    if (vno == Gdiag_no)
      DiagBreak();
...
#define MAX_THETA (RADIANS(60))
        ...
        if (mri) {
          ...
          if (FZERO(white_dot))
            ...
            val = white_dot;
          MRIfillRegion(mri, nint(xv), nint(yv), nint(zv),
                        val < 0 ? -.02 : 0.02, nint(.25 / mri->xsize));
    ...
    if (pial_fname) // only if pial loaded and white was reasonable
    {
      ...
      {
        num_in_fov++;
        V3_X(v1) = v->pialx + v->pnx;
        V3_Y(v1) = v->pialy + v->pny;
        ...
        v->marked = 1;
        pial_dot  = -1 * (nx * dx + ny * dy +
                         nz * dz); // pial contrast should be inverse
        if (pial_dot < 0)
          pial_dot = 0;
        else
          DiagBreak();
...
static double find_optimal_translations(
    MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask, MATRIX *M_min,
    double max_trans, int skip,
    double (*similarity_func)(MRI_SURFACE *mris, MRI *mri_reg, MRI *mri_mask,
                              MATRIX *m, int skip, double scale, int diag)) {
      ...
      for (ty = -WHALF; ty <= WHALF; ty++)
        for (tz = -WHALF; tz <= WHALF; tz++) {
          ...
          *MATRIX_RELT(m_trans, 3, 4) = ztrans;
          ...
          similarity = (*similarity_func)(mris, mri_reg, mri_mask, m, skip,
                                          skip / 8.0, 0);
          if (similarity > max_similarity) {
            ...
            if (logfp) {
              fprintf(
                  logfp,
                  "%03d: new max %2.2f found at T = (%2.1f, %2.1f, %2.1f)\n",
...
{
  double tx, ty, tz, rot_scale, trans_scale, max_similarity, similarity, xtrans,
      ytrans, ztrans, rx, ry, rz, whalf = WHALF;
  ...
  printf("computing optimal rigid alignment in R=[%2.1f, %2.1f], T=[%2.2f, "
         "%2.2f]\n",
  ...
  *MATRIX_RELT(M_ctr, 3, 4) = mris->zctr;
  ...
  do {
    for (rot_scale = max_rot / whalf; rot_scale >= min_rot; rot_scale /= 2)
            ...
            for (tx = -whalf; tx <= whalf; tx++)
              for (ty = -whalf; ty <= whalf; ty++)
                for (tz = -whalf; tz <= whalf; tz++) {
                  ...
                  xtrans                      = trans_scale * tx;
                  ytrans                      = trans_scale * ty;
                  ztrans                      = trans_scale * tz;
                  *MATRIX_RELT(M_trans, 1, 4) = xtrans;
                  *MATRIX_RELT(M_trans, 2, 4) = ytrans;
                  *MATRIX_RELT(M_trans, 3, 4) = ztrans;
                  ...
                  MatrixFree(&M_rot);
                  similarity = (*similarity_func)(mris, mri_reg, mri_mask,
                                                  M_test, skip, skip / 8.0, 0);
                  if (similarity > max_similarity) {
                    MatrixCopy(M_test, M_min);
                    max_similarity = similarity;
                    printf(
                        "%03d: new max %2.2f found at T=(%2.3f, %2.3f, %2.3f), "
                        "R=(%2.3f, %2.3f, %2.3f)\n",
                        niter + 1, max_similarity, xtrans, ytrans, ztrans,
                        DEGREES(angles[0]), DEGREES(angles[1]),
                        DEGREES(angles[2]));
                    if (logfp) {
                      fprintf(logfp,
                              "%03d: new max %2.2f found at T=(%2.3f, %2.3f, "
                              "%2.3f), "
                              "R=(%2.3f, %2.3f, %2.3f)\n",
                              niter + 1, max_similarity, xtrans, ytrans, ztrans,
                              DEGREES(angles[0]), DEGREES(angles[1]),
    ...
    for (rx = -WHALF; rx <= WHALF; rx++)
      for (ry = -WHALF; ry <= WHALF; ry++)
        for (rz = -WHALF; rz <= WHALF; rz++) {
          ...
          MatrixFree(&M_rot);
          similarity = (*similarity_func)(mris, mri_reg, mri_mask, M_test, skip,
                                          skip / 8.0, 0);
          if (similarity > max_similarity) {
            MatrixCopy(M_test, M_min);
            max_similarity = similarity;
            if (logfp) {
              fprintf(logfp,
                      "%03d: new max %2.2f found at R=(%2.3f, %2.3f, %2.3f)\n",
            ...
            if ((++niter % write_iter) == 0)
              write_snapshot(mris, M_min, outregfile, niter);
            similarity = (*similarity_func)(mris, mri_reg, mri_mask, M_min,
                                            skip, skip / 8.0, 1);
            good_step  = 1;
          }
        }
...
static int write_snapshot(MRI_SURFACE *mris, MATRIX *m, char *name, int n) {
  static VECTOR *v1, *v2;
...
static int powell_minimize_rigid(
  ...
  if (logfp) {
    fprintf(logfp,
            "%3.3d: best alignment after %s powell: "
            "%2.5f (%d steps)\n\tparms = R:(%2.4f, %2.4f, %2.4f) + T:(%2.2f, "
            "%2.2f, %2.2f)\n",
            niter + 1, sname, fret, iter, DEGREES(p[1]), DEGREES(p[2]),
            DEGREES(p[3]), p[4], p[5], p[6]);
    fflush(logfp);
  }
  ...
  do {
    ...
    fstart = fret;
    OpenPowell(p, xi, NPARMS_RIGID, TOL, &iter, &fret,
               compute_powell_rigid_sse);
    MatrixFromRigidParameters(mat, p[1], p[2], p[3], p[4], p[5], p[6]);
#if 0
    ...
    *MATRIX_RELT(mat, 4, 4) = 1.0 ;
    ...
    printf("%3.3d: best alignment after %s powell: "
           "%2.5f (%d steps)\n\tparms = (%2.4f, %2.4f, %2.4f) + (%2.2f, %2.2f, "
           "%2.2f)\n",
           niter + 1, sname, fret, iter, DEGREES(p[1]), DEGREES(p[2]),
           DEGREES(p[3]), p[4], p[5], p[6]);
    if ((++niter % write_iter) == 0)
...
static float compute_powell_rigid_sse(float *p) {
  ...
  MatrixFromRigidParameters(mat, p[1], p[2], p[3], p[4], p[5], p[6]);
  similarity =
      (*Gsimilarity_func)(Gmris, Gmri_reg, Gmri_mask, mat, Gskip, Gskip / 8, 0);
...
#endif
static MRI *mri_surface_edges(MRI *mri, MRI *mri_grad, float normal_dist,
                              float tangent_dist, int dir, MRI *mri_edge) {
  int    x, y, z, nout, nin;
  double dx, dy, dz, d, norm, mean_in, mean_out, var_in, var_out, step_size, x1,
      y1, z1, val, t1x, t1y, t1z, t2x, t2y, t2z, d1, d2;
  ...
  for (x = 0; x < mri->width; x++) {
    for (y = 0; y < mri->height; y++) {
      for (z = 0; z < mri->depth; z++) {
        ...
        nout = nin = 0;
        for (d1 = -tangent_dist / mri->xsize; d1 <= tangent_dist / mri->xsize;
             d1++)
          for (d2 = -tangent_dist / mri->xsize; d2 <= tangent_dist / mri->xsize;
               d2++)
        ...
        val     = dir * (mean_out - mean_in) /
              (pow((mean_out + mean_in) / 2, 0.333) * sqrt(var_out + var_in));
        if (val < 0)
          val = 0;
        if (val > 100)
          DiagBreak();
...
static int write_lta(MATRIX *m, char *fname_from_caller, MRI_SURFACE *mris,
                     MRI *mri_reg) {
