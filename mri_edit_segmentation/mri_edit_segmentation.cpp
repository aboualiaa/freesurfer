...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_edit_segmentation.c,v 1.11 2011/03/02 00:04:15 nicks Exp $",
      "$Name:  $");
  ...
  if (!mri_in_labeled)
    ErrorExit(ERROR_NO_FILE, "%s: could not open labeled file %s", Progname,
              in_fname);

  mri_T1 = MRIread(T1_fname);
  if (!mri_T1)
    ErrorExit(ERROR_NO_FILE, "%s: could not open T1 file %s", Progname,
              in_fname);

  start.reset();

  if (unknown_only == 0 && border_only == 0) {
    mri_out_labeled = edit_hippocampus(mri_in_labeled, mri_T1, nullptr);
    edit_amygdala(mri_out_labeled, mri_T1, mri_out_labeled);
    edit_caudate(mri_out_labeled, mri_T1, mri_out_labeled);
    edit_lateral_ventricles(mri_out_labeled, mri_T1,
                            mri_out_labeled); /* must be after hippo */
    edit_cortical_gray_matter(mri_out_labeled, mri_T1, mri_out_labeled);
  } else
    mri_out_labeled = MRIcopy(mri_in_labeled, nullptr);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "no1d")) {
    ...
    unknown_only = 1;
    printf("only correcting unknown voxels between ventricle and wm or near "
           "hypointensities\n");
...
static MRI *edit_hippocampus(MRI *mri_in_labeled, MRI *mri_T1,
                             MRI *mri_out_labeled) {
  int width, height, depth, x, y, z, nchanged, dleft, label, dright, dpos, dant,
      dup, ddown, i, left, dgray, dhippo, dwhite, olabel;
  MRI *mri_tmp;
      ...
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          case Right_Cerebral_White_Matter:
            ...
            dhippo = distance_to_label(
                mri_out_labeled, left ? Left_Hippocampus : Right_Hippocampus, x,
                y, z, 0, -1, 0, 3);
            /* change bright hippocampus that borders white matter to white
             * matter */
          ...
          case Unknown: {
            ...
            */
            dl = distance_to_label(mri_out_labeled, Left_Hippocampus, x, y, z,
                                   0, -1, 0, 4);
            dr = distance_to_label(mri_out_labeled, Right_Hippocampus, x, y, z,
                                   0, -1, 0, 4);
            if (dl > 4 && dr > 4) /* could be inferior to inf-lat-ven also */
            {
              dl = distance_to_label(mri_out_labeled, Left_Inf_Lat_Vent, x, y,
                                     z, 0, -1, 0, 4);
              dr = distance_to_label(mri_out_labeled, Right_Inf_Lat_Vent, x, y,
                                     z, 0, -1, 0, 4);
            }
          ...
          case Right_Cerebral_Cortex:
            ...
            */
            ...
            dpos =
                distance_to_label(mri_out_labeled, Right_Cerebral_White_Matter,
                                  x, y, z, 0, 0, -1, 3);
            dant =
                distance_to_label(mri_out_labeled, Right_Cerebral_White_Matter,
                                  x, y, z, 0, 0, 1, 3);

            /* if the current label is gray and the is white matter directly
              above, and hippocampus within 3 mm, then change label to MLE of
  ...
  /* go through and change wm labels that have hippo both above and below them
   * to hippo */
  for (z = 0; z < depth; z++) {
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        if (IS_HIPPO(
                MRIvox(mri_out_labeled, x, mri_out_labeled->yi[y - 1], z)) &&
            IS_HIPPO(
                MRIvox(mri_out_labeled, x, mri_out_labeled->yi[y + 1], z))) {
          nchanged++;
          MRIvox(mri_out_labeled, x, y, z) =
...
static MRI *edit_amygdala(MRI *mri_in_labeled, MRI *mri_T1,
                          MRI *mri_out_labeled) {
  int width, height, depth, x, y, z, nchanged, label, total_changed, dup, ddown,
      left;
  MRI *mri_tmp;
...
static MRI *edit_caudate(MRI *mri_in_labeled, MRI *mri_T1,
                         MRI *mri_out_labeled) {
  int width, height, depth, x, y, z, nchanged, label, total_changed, left,
      niter, change;
  MRI *mri_tmp;
    ...
    for (z = 0; z < depth; z++) {
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          case Unknown:
          case Right_Cerebral_Cortex:
            if (neighborLabel(mri_tmp, x, y, z, 1,
                              Left_Cerebral_White_Matter) &&
                neighborLabel(mri_tmp, x, y, z, 1,
                              Right_Cerebral_White_Matter)) {
              left = sagittalNeighbors(mri_tmp, x, y, z, 3,
                                       Left_Cerebral_White_Matter) >
...
static MRI *edit_lateral_ventricles(MRI *mri_in_labeled, MRI *mri_T1,
                                    MRI *mri_out_labeled) {
  int width, height, depth, x, y, z, nchanged, label, total_changed, left,
      niter, change, dvent, dwhite, olabel;
  MRI *mri_tmp;
        ...
        for (x = 0; x < width; x++) {
          ...
          case Right_Inf_Lat_Vent:
            ...
            dvent = distance_to_label(
                ...
                x, y, z, 0, -1, 0, 3);
            if (dvent <= 1 &&
                dwhite <= 1) /* borders ventricle superior and wm inferior */
            {
              mle_label(mri_T1, mri_tmp, x, y, z, 9,
...
static MRI *edit_ventricular_unknowns(MRI *mri_in_labeled, MRI *mri_T1,
  ...
  int label, x, y, z, xm1, xp1, ym1, yp1, zm1, zp1, lxp1, lxm1, lyp1, lym1,
      lzp1, lzm1, nchanged, l1 = 0, l2 = 0, change, i, total_changed, dwm,
                            dhypo, dven, left;
  MRI *mri_in;
    ...
    for (x = 0; x < mri_T1->width; x++) {
      for (y = 0; y < mri_T1->height; y++) {
        for (z = 0; z < mri_T1->depth; z++) {
          ...
          dhypo = distance_to_label(mri_in, WM_hypointensities, x, y, z, 0, 1,
                                    0, 2); /* look inferior */
          if (dhypo > 2)
            dhypo = distance_to_label(mri_in, WM_hypointensities, x, y, z, 0, 0,
                                      -1, 2); /* look inferior */
          if (dhypo > 2)
            dhypo = distance_to_label(mri_in, WM_hypointensities, x, y, z, 0, 0,
                                      1, 2); /* look anterior */
...
#define WM_DIST 4
          ...
          dwm = distance_to_label(mri_in, Left_Cerebral_White_Matter, x, y, z,
                                  0, -1, 0, WM_DIST); /* superior */
          if (dwm > WM_DIST) {
            dwm = distance_to_label(mri_in, Right_Cerebral_White_Matter, x, y,
                                    z, 0, -1, 0, WM_DIST);
          } else
            left = 1;
          ...
          if (dwm > WM_DIST) /* not superior, look anterior */
          {
            dwm = distance_to_label(mri_in, Right_Cerebral_White_Matter, x, y,
                                    z, 0, 0, 1, WM_DIST);
            if (dwm > WM_DIST) {
              dwm = distance_to_label(mri_in, Left_Cerebral_White_Matter, x, y,
                                      z, 0, 0, 1, WM_DIST);
              if (dwm <= WM_DIST)
                left = 1;
            }
          ...
          if (dwm > WM_DIST) /* couldn't find wm anywhere (didn't look
                                inferior), check for region of hypos */
          {
            dwm = distance_to_label(mri_in, WM_hypointensities, x, y, z, 0, -1,
                                    0, WM_DIST);
            if (distance_to_label(mri_in, Left_Cerebral_White_Matter, x, y, z,
                                  0, -1, 0, 30) <= 30)
              left = 1;
          }
          if (dwm > WM_DIST)
            continue;
          dven = distance_to_label(
              mri_in, left ? Left_Lateral_Ventricle : Right_Lateral_Ventricle,
              x, y, z, 0, 1, 0, 15);
          if (dven > 15) {
            dven = distance_to_label(
                mri_in, left ? Left_Lateral_Ventricle : Right_Lateral_Ventricle,
                x, y, z, 0, 0, -1, 3);
            if (dven > 3)
              distance_to_label(mri_in,
                                left ? Left_Lateral_Ventricle
          ...
          nchanged++;
          mle_label(mri_T1, mri_out_labeled, x, y, z, 15,
                    left ? Left_Cerebral_White_Matter
                         : Right_Cerebral_White_Matter,
                    WM_hypointensities);
        }
    ...
    for (nchanged = x = 0; x < mri_T1->width; x++) {
      for (y = 0; y < mri_T1->height; y++) {
        for (z = 0; z < mri_T1->depth; z++) {
          ...
          /* if it has ventricle on one side and wm on the other, change it to
            wm or ventricle these are caused by slight discrepencies between the
            FreeSurfer ribbon and the CMA subcortical segs.
          */
...
#define WLEN 4
            ...
            if (change <= 0) /* look in posterior/anterior direction. If
                                everthing is wm and ven change it */
            {
              int zi;
              ...
              if (change >= 0 && ((ven + wm) >= WLEN) && (ven >= WLEN / 2) &&
                  (wm >= WLEN / 2))
                change = 1;
              if (change > 0) {
                nchanged++;
            ...
            if (change <= 0) /* look in medial/lateral direction. If everthing
                                is wm and ven change it */
            {
              int xi;
  ...
  for (nchanged = x = 0; x < mri_T1->width; x++) {
    for (y = 0; y < mri_T1->height; y++) {
      for (z = 0; z < mri_T1->depth; z++) {
        ...
        if (label == lxm1 && label == lxp1 && label == lym1 && label == lyp1 &&
            label == lzm1 && label == lzp1)
          continue; /* it's not a border label */
        if (!(IS_WM(label) || IS_HIPPO(label) || IS_GM(label) ||
        ...
        if (!(IS_WM(olabel) || IS_HIPPO(olabel) || IS_GM(olabel) ||
              IS_UNKNOWN(olabel) || IS_LAT_VENT(olabel)))
          continue;
        if ((IS_GM(label) && IS_HIPPO(olabel)) ||
          ...
          continue; /* don't change hippo to gm based on intensity - too similar
                     */
        if ((label != olabel) &&
            MRIneighborsInWindow(mri_in_labeled, x, y, z, 3, olabel) > 5) {
          nchanged++;
