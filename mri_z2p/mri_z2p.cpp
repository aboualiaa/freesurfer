...
int main(int argc, char *argv[]) {
  ...
  if (fmt == nullptr) {
    if (strcmp(getenv("FSLOUTPUTTYPE"), "NIFTI") == 0)
      fmt = "nii";
    if (strcmp(getenv("FSLOUTPUTTYPE"), "NIFTI_GZ") == 0)
      fmt = "nii.gz";
    if (strcmp(getenv("FSLOUTPUTTYPE"), "ANALYZE") == 0)
      fmt = "img";
    if (fmt == nullptr) {
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--two-sided"))
      TwoSided = 1;
    else if (!strcasecmp(option, "--unsigned"))
      TwoSided = 1;
    else if (!strcasecmp(option, "--one-sided"))
      TwoSided = 0;
    else if (!strcasecmp(option, "--signed"))
      TwoSided = 0;
...
static void print_usage() {
  ...
  printf("   --two-sided : (default) assume a two-sided, unsigned test (keeps "
         "sign of input)\n");
  ...
  printf("   --featfmt extension : use given format (eg, nii, nii.gz, mgh, "
         "etc)\n");
...
static void check_options() {
  if (featdir == nullptr) {
    ...
    if (PVolFile == nullptr && Log10PVolFile == nullptr) {
