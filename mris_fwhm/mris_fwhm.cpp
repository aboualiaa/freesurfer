...
int main(int argc, char *argv[]) {
  ...
  if (DoDetrend) {
    ...
    printf("Not Polynomial detrending\n");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      DoSqr = 1;
    else if (!strcasecmp(option, "--fast"))
      setenv("USE_FAST_SURF_SMOOTHER", "1", 1);
    else if (!strcasecmp(option, "--no-fast"))
...
static void print_usage(void) {
  ...
  printf("   --detrend order : polynomial detrending (default 0, turned off "
         "with output)\n");
  ...
  printf("   --prune - remove any voxel that is zero in any subject (after any "
         "inversion)\n");
...
static void print_help(void) {
  ...
  printf("--subject subject (--s)\n");
  printf("\n");
  printf("Subject whose surface the input is defined on. Can use --s instead "
         "of\n");
  printf("--subject.\n");
  printf("\n");
  ...
  printf("--mask maskfile\n");
  printf("\n");
  printf("Compute AR1 only over voxels in the given mask. Format can be "
         "anything\n");
  printf("accepted by mri_convert. See also --label.\n");
  printf("\n");
  printf("--mask-inv\n");
  printf("\n");
  printf("Invert mask, ie, compute AR1 only over voxels outside the given "
         "mask.\n");
  printf("\n");
  printf("--label label\n");
  printf("\n");
  ...
  printf("--X x.mat\n");
  printf("\n");
  printf("Detrend data with the matrix in x.mat. Ie, y = (I-inv(X'*X)*X')*y, "
         "where\n");
  printf("y is the input. x.mat must be a matlab4 matrix.\n");
  printf("\n");
  printf("--detrend order\n");
  printf("\n");
  printf("Detrend data with polynomial regressors upto order. If no output is "
         "specified,\n");
  printf("then order=0 by default. If an output is specified, then no "
         "detrending is done.\n");
  printf("\n");
  printf("--sum sumfile\n");
  printf("\n");
  ...
  printf("\n");
  printf("Only report the number of iterations needed to achieve the FWHM "
         "given\n");
  ...
  printf("--o outfile\n");
  printf("\n");
  printf("Save (possibly synthesized and/or smoothed) data to outfile. "
         "Automatically\n");
  printf("detects format. Format must be one accepted as by mri_convert. Note: "
         "do\n");
  printf("not use analyze or nifit as these cannot store more than 32k in a "
         "dimension.\n");
  printf("mri_surf2surf can store surface data in those formats. When an "
         "output\n");
  printf("is specified, detrending is turned off. Normally the mean is "
         "removed.\n");
  printf("\n");
  printf("--synth\n");
  printf("\n");
...
static void check_options(void) {
  ...
  if (UseCortexLabel) {
    ...
    labelpath = std::string(SUBJECTS_DIR) + '/' + std::string(subject) +
                "/label/" + std::string(hemi) + ".cortex.label";
  }
  return;
}
