...
int main(int argc, char **argv) {
  ...
  if (!regheader) {
    ...
    if (lta->xforms[0].src.valid && lta->xforms[0].dst.valid) {
      ...
      if (LTAmriIsTarget(lta, SrcVol)) {
        ...
        LTAprint(stdout, lta);
        printf("\n\n");
        printf("ERROR: source volume is neither source nor target of the "
               "registration\n");
        exit(1);
      }
      ...
      if (fabs(lta->xforms[0].src.xsize - SrcVol->xsize) > .001 ||
        ...
        printf(
            "         from that listed in the registration file (%g,%g,%g)\n",
            lta->xforms[0].src.xsize, lta->xforms[0].src.ysize,
            lta->xforms[0].src.zsize);
        if (fixtkreg) {
          printf(
              "ERROR: cannot fix tkreg matrix with resolution inconsistency\n");
          exit(1);
        }
      }
    }
  ...
  if ((ProjFrac != 0 && ProjDistFlag == 0) || (ProjOpt != 0)) {
    sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, srcsubject, hemi,
            thicknessname);
  ...
  if (ProjOpt != 0) {
    ...
    nproj = 0;
    for (ProjFrac = ProjFracMin; ProjFrac <= ProjFracMax;
         ProjFrac += ProjFracDelta) {
      printf("%2d %g %g %g\n", nproj + 1, ProjFrac, ProjFracMin, ProjFracMax);
      if (UseOld) {
        printf("using old\n");
        SurfValsP = vol2surf_linear(SrcVol, Qsrc, Fsrc, Wsrc, Dsrc, Surf,
                                    ProjFrac, interpmethod, float2int,
      ...
      if (nproj == 0)
        ...
        if (!GetProjMax)
          ...
          MRImax(SurfVals, SurfValsP, SurfVals);
      }
      MRIfree(&SurfValsP);
      nproj++;
    }
  ...
  if (trgsubject != nullptr && strcmp(trgsubject, srcsubject)) {
    ...
    if (strcmp(trgsubject, "ico")) { /* not ico */
      sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, trgsubject, hemi,
              surfreg);
  ...
  if (outtypestring != nullptr && (!strcasecmp(outtypestring, "w") ||
    ...
    if (reshape) {
      if (reshapefactor == 0) {
        if (outtype == MRI_ANALYZE4D_FILE || outtype == MRI_ANALYZE_FILE ||
          ...
          reshapefactor =
              GetClosestPrimeFactor(SurfVals2->width, reshapetarget);
      }
    ...
    printf("Writing to %s\n", outfile);
    printf("Dim: %d %d %d\n", SurfVals2->width, SurfVals2->height,
           SurfVals2->depth);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      reshape = 0;
    else if (!strcasecmp(option, "--fixtkreg"))
      fixtkreg = 1;
    ...
    /* -------- source volume inputs ------ */
      ...
      if (nargc < 9) {
        ...
        printf("USAGE: --vol2surf vol surf projtype projdist projmap reg vsm "
               "interp output\n");
        printf(" projtype 0=frac, 1=absdist (if 1, projmap arg will be "
               "ignored)\n");
        printf(" projmap : map to get the values for projection (usually "
               "thickness)\n");
        printf(" reg : LTA registration file (or 'regheader', surf must have "
               "vol geom) \n");
        printf("   LTA files can go in either direction if surf has a valid "
               "vol geom\n");
...
static void print_usage() {
  ...
  printf("      the white surface, then display it on any surface, including "
         "inflated\n");
  ...
  printf("   --projopt <fraction stem> : use optimal linear estimation and "
         "previously\n"
         "computed volume fractions (see mri_compute_volume_fractions)\n");
  printf("   --projdist-max min max del : max along normal\n");
  printf("   --mask label : mask the output with the given label file (usually "
         "cortex)\n");
  ...
  printf("     If sigma >= 0, then the gradient is estimated with smoothing "
         "parameter sigma\n");
  ...
  printf("   --vg-thresh thrshold : threshold for  'ERROR: LTAconcat(): LTAs 0 "
         "and 1 do not match'\n");
  printf("\n");
  printf("   --vol2surf vol surf projtype projdist projmap reg vsm interp "
         "output\n");
  printf("    This is an alternative way to run vol2surf that does not rely on "
         "the recon-all\n");
  printf("    directory structure. Generates the same output as standard "
         "invocation as long \n");
  printf("    as --use-new is added. --vol2surf does not have all the "
         "functionality. \n");
  printf(
      "      projtype 0=frac, 1=absdist (if 1, projmap arg will be ignored)\n");
  printf("      projmap : map to get the values for projection (usually "
         "thickness)\n");
  printf("      reg : LTA registration file (or 'regheader', surf must have "
         "vol geom) \n");
  printf("        LTA files can go in either direction if surf has a valid vol "
         "geom\n");
  printf("      vsm : voxel shift map for B0 correction (or 'novsm')\n");
  printf("      interp 0=nearest, 1=trilin, 5=cubicbspline\n");
  printf("\n");
...
static void print_help() {
  ...
  printf(
      ...
      "    can be an LTA (better) or a register.dat, in which case it has the "
      "following format:\n"
      "\n"
      "        subjectname\n"
      ...
      "    volume geometry between the mov volume and the "
      ...
      "\n"
      "  --surf surfacename : the surface on which to resample. The default "
      "is\n"
      "  white. It will look for "
      "$SUBJECTS_DIR/subjectname/surf/?h.surfacename\n"
      "  DO NOT specify 'inflated'. If you want to display on the inflated "
      "surface,\n"
      "  or any other surface, specify 'white' here then display it on any "
      "surface,\n"
      ...
      "    zero, in which case it will project into the white matter. See "
      "also\n"
      "    --projdist.\n"
      "\n"
      "  --projdist mmdist\n"
      ...
      "points\n"
      "    between min and max at a spacing of delta. The samples are then "
      "averaged\n"
      ...
      "  --rf R\n"
      "\n"
      "    Explicity set the reshaping factor to R. R must be an integer "
      "factor \n"
      ...
      "    each voxel is the number of surface vertices it maps to. The number "
      "of\n"
      ...
      "    --srcsynth. This is only useful for keeping track of the "
      "distribution\n"
      ...
      "NOTES\n"
      "\n"
      "  The output will be a data set with Nv/R colums, 1 row, R slices, and "
      "Nf frames,\n"
      "  where Nv is the number of verticies in the output surface, and Nf is "
      "the \n"
      "  number of frames in the input volume (unless the output format is "
      "paint, in\n"
      "  which case only one frame is written out). R is the reshaping factor. "
      "R is 6 \n"
      "  for the icosaheron. For non-ico, the prime factor of Nv closest to 6 "
      "is chosen. \n"
      "  Reshaping can be important for logistical reasons (eg, Nv can easily "
      "exceed \n"
      "  the maximum number of elements allowed in the analyze format). R can "
      "be forced \n"
      "  to 1 with --noreshape. The user can also explicity set R from the "
      "command-line\n"
      ...
      "  to resample variance and not standard deviation. This is "
      "automatically\n"
      ...
      "EXAMPLES:\n"
      "\n"
      "  1. To paint the third frame of bfloat volume sig registered with "
      "tkregister\n"
      ...
      "\n"
      "     This will create sig-lh.mgh in the current directory, which can "
      "then\n"
      ...
      "BUGS:\n"
      "\n"
      "  When the output format is paint, the output file must be specified "
      "with\n"
      "  a partial path (eg, ./data-lh.w) or else the output will be written "
      "into\n"
      "  the subject's anatomical directory.\n"
      "\n"
      "  Currently no support for searching along the surface normal for a "
      "maximum\n"
      "  value (as can be done with the paint program)\n"
      "\n"
      "  The ability to put the float2int conversion method in the "
      "registration file\n"
      "  is new as of Fall 2001. Consequently, there may be some registration "
      "files\n"
      "  that do not have a method string, which will force the method to be "
      "that\n"
      "  of tkregister. This is can be overridden with --float2int.\n"
      "\n"
      "\n"
      "AUTHOR: Douglas N. Greve, Ph.D., MGH-NMR Center "
...
MRI *MRIvol2surf(MRI *SrcVol, MATRIX *Rtk, MRI_SURFACE *TrgSurf, MRI *vsm,
  ...
  for (vtx = 0; vtx < TrgSurf->nvertices; vtx += nskip) {
    ...
    if (Gdiag_no == vtx) {
      printf("diag -----------------------------\n");
      printf("vtx = %d  %g %g %g\n", vtx, Txyz->rptr[1][1], Txyz->rptr[2][1],
             Txyz->rptr[3][1]);
...
MRI *build_sample_array(MRI_SURFACE *mris, MRI *mri_src, MATRIX *m, float din,
                        float dout, int nsamples, MRI *mri_wm, MRI *mri_gm,
                        MRI *mri_csf) {
  float fcol_src, frow_src, fslc_src, Tx, Ty, Tz, dist, thick, sample_dist;
  int vno, vno_nbr, srcval, icol_src, irow_src, islc_src, n, nfound, index, nbr,
    ...
    do {
      ...
      for (nbr = 0; nbr < vnum; nbr++) {
        ...
        for (n = 0; n < nsamples; n++) {
          ...
          if (MRIgetVoxVal(mri_sampled, icol_src, irow_src, islc_src, 0) ==
              vno + 1)
...
#endif
      done = ((nfound >= (min_needed / 2)) && (found & FOUND_GM) &&
              (index >= min_needed) && (nsize >= 2));
      if (nsize++ >= 3 && !done) {
        ErrorPrintf(ERROR_UNSUPPORTED,
                    "GM estimation at vno %d couldn't find enough samples",
                    vno);
        failed = 1;
        break;
      }
