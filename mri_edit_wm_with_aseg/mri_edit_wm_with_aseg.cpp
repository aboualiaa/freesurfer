...
#if SPACKLE_MTL
static int distance_to_lateral_edge(MRI *mri_seg, int label, int x, int y,
                                    int z, int left);
static int distance_to_superior_edge(MRI *mri_seg, int label, int x, int y,
                                     int z);
static int distance_to_inferior_edge(MRI *mri_seg, int label, int x, int y,
                                     int z);
static int anterior_edge_of_amygdala(MRI *mri_aseg, int x, int y, int z);

static int medial_edge_of_hippocampus(MRI *mri_seg, int x, int y, int z,
                                      int dx);
static int MRInonfilledInNbhd(MRI *mri, int x, int y, int z, int whalf);
static int distance_to_nonzero(MRI *mri_wm, int x, int y, int z, int dx, int dy,
                               int dz, int max_dist);

static int distance_to_zero(MRI *mri_wm, int x, int y, int z, int dx, int dy,
                            int dz, int max_dist);
...
static int distance_to_anterior_edge(MRI *mri_seg, int label, int x, int y,
                                     int z);
static int remove_lateral_and_anterior_hippocampus(MRI *mri_roi, MRI *mri_aseg,
                                                   int left);
...
int main(int argc, char *argv[]) {
  ...
  Timer then;
  int msec, nargs;
  char cmdline[CMD_LINE_LEN], *output_file_name, *input_file_name,
      *edits_file_name;

  make_cmd_version_string(
      argc, argv,
      "$Id: mri_edit_wm_with_aseg.c,v 1.25 2012/03/29 13:17:38 fischl Exp $",
      "$Name:  $", cmdline);
  ...
  if (!mri_wm)
    ErrorExit(ERROR_NOFILE, "%s: could not read wm volume from %s", Progname,
              input_file_name);

  if (mri_wm->type != MRI_UCHAR) {
    ErrorExit(ERROR_UNSUPPORTED, "%s: volume %s must be MRI_UCHAR", Progname,
              argv[1]);
  }
...
static int edit_segmentation(MRI *mri_wm, MRI *mri_T1, MRI *mri_seg) {
  int i, width, height, depth, x, y, z, label, non, noff, xi, yi, zi, xk, yk,
      zk, nchanged, wsize, alabel, hlabel, slabel, olabel, left;
  MRI *mri_filled;
  ...
  for (z = 0; z < depth; z++) {
    for (y = height - 2; y > 0; y--) {
      for (x = 1; x < width - 1; x++) {
        ...
        case Unknown:
          wsize = 5;
          if (MRIlabelsInNbhd(mri_seg, x, y, z, (wsize - 1) / 2, Unknown) <
              (wsize * wsize * wsize - 1)) {
            break;
          }
	  ...
	  // I don't think these make sense
        ...
        case Right_Substancia_Nigra:
          ...
          if ((neighborLabel(mri_seg, x, y, z, 1, Left_Cerebral_Cortex) == 0) &&
              (neighborLabel(mri_seg, x, y, z, 1, Right_Cerebral_Cortex) ==
               0)) {
        ...
        case Right_Lateral_Ventricle:
          if (((neighborLabel(mri_seg, x, y, z, 1, Left_Cerebral_White_Matter) >
                0) ||
               (neighborLabel(mri_seg, x, y, z, 1,
        ...
        case Right_Inf_Lat_Vent:
          ...
          be some wm there also don't allow it to be diagonally connected */
          if (IS_CORTEX(olabel) && (MRIvox(mri_wm, xi, y, z) < WM_MIN_VAL)) {
            if (xi == Gx && y == Gy && z == Gz) {
              printf("changing label (%d, %d, %d) to wm (gm lateral to "
                     "inf-lat-vent)\n",
                     xi, y, z);
            }
          ...
          hlabel = ((label == Left_Lateral_Ventricle) ||
        ...
        case Right_Hippocampus: {
          ...
          // don't mess around with voxels near the medial or superior edge of
          // hippocampus
#if 0
          if (left)
          {
        ...
        case Right_Cerebral_White_Matter:
          ...
          if (IS_INF_LAT_VENT(slabel) && MRIvox(mri_wm, x, y, z) < WM_MIN_VAL) {
            if (x == Gx && y == Gy && z == Gz) {
              DiagBreak2();
              printf("changing voxel (%d, %d, %d) to WM, due to superior "
                     "inf-lat-vent\n",
                     x, y, z);
            }
            MRIvox(mri_wm, x, y, z) = AUTO_FILL;
  ...
  /* fill in the borders of the ventricle - 2mm thick. This shouldn't affect the
  folds but will prevent small holes from ventricle into wm
  */
  for (z = 0; z < depth; z++) {
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        case Unknown:
          if (((neighborLabel(mri_seg, x, y, z, 1, Left_Lateral_Ventricle) >
                0) &&
               (neighborLabel(mri_seg, x, y, z, 1, Left_Cerebral_White_Matter) >
                0)) ||
              ((neighborLabel(mri_seg, x, y, z, 1, Right_Lateral_Ventricle) >
                0) &&
               (neighborLabel(mri_seg, x, y, z, 1,
                              Right_Cerebral_White_Matter) > 0))) {
            if (x == Gx && y == Gy && z == Gz) {
  ...
  */
  ...
  // now look for voxels in which there is no wm inferior to hippo and spackle
  // them.
  for (z = 1; z < depth - 1; z++) {
    for (y = height - 2; y > 0; y--) {
      for (x = 2; x < width - 2; x++) {
  ...
  */
      ...
      for (y = height - 1; y > 0; y--) {
        for (x = 2; x < width - 2; x++) {
          ...
          case Right_Amygdala:
            ...
            if ((distance_to_label(mri_seg, Unknown, x, y, z, 0, 1, 0, 10) >
                 5) &&
                (distance_to_label(mri_seg, olabel, x, y, z, 0, 1, 0, 10) >
                 5)) {
              continue;
            }
            ...
            if (MRIvox(mri_wm, x, y + 1, z - 1) <
                MIN_WM_VAL) // fill one of these
            {
      ...
      for (x = 2; x < width - 2; x++) {
        ...
        case Right_Cerebral_Cortex:
          ...
          if ((MRIgetVoxVal(mri_seg, x, y - 1, z + 1, 0) != alabel) &&
              (MRIgetVoxVal(mri_seg, x, y - 1, z + 1, 0) != hlabel)) {
            continue;
          }
          // check to make sure we are at inferior border of hippo or amygdala
          if ((distance_to_label(mri_seg, alabel, x, y, z, 0, 1, 0, 8) < 8) ||
          ...
          if (MRInonfilledInNbhd(mri_wm, x, y, z, 1) <
              2) // only if there is enough wm in vicinity
          {
            continue;
          }
          non++;
    ...
    for (y = 1; y < height - 1; y++) {
      for (x = 2; x < width - 2; x++) {
        ...
        case Unknown:
          ...
          if ((distance_to_label(mri_seg, hlabel, x, y, z, 0, 1, 0, 8) <
               8)) // no hippo inferior
          {
            continue;
          }
  ...
  // don't allow cortex to be directly lateral to hippo or inf lat vent - must
  // be some wm
  for (z = 1; z < depth - 1; z++) {
    for (y = 1; y < height - 1; y++) {
      for (x = 2; x < width - 2; x++) {
  ...
  for (count = 0, zk = -whalf; zk <= whalf; zk++) {
    ...
    for (yk = -whalf; yk <= whalf; yk++) {
      ...
      for (xk = -whalf; xk <= whalf; xk++) {
        xi = mri->xi[x + xk];
        if ((MRIgetVoxVal(mri, xi, yi, zi, 0) >= MIN_WM_VAL) &&
            (MRIgetVoxVal(mri, xi, yi, zi, 0) != AUTO_FILL)) {
          count++;
        }
      }
    }
...
static int remove_paths_to_cortex(MRI *mri_wm, MRI *mri_T1, MRI *mri_aseg) {
  ...
  for (niter = 0; niter < 5; niter++) {
    MRIdilate(
        mri_roi,
        mri_roi); // build a mask and don't let things escape from that region
  }
  ...
  // find the most medial hippocampal voxel then erase everything within 1 cm of
  // it

  // LH: now fill inferior and lateral until no voxels are filled or we reach
  // cortex
  niter = 0;
  do {
    ...
    for (x = x1; x <= x2; x++)
      for (y = y1; y <= y2; y++)
        for (z = z1; z <= z2; z++) {
          ...
          case Left_Inf_Lat_Vent:
            yi = y + 1;
            if (MRIgetVoxVal(mri_wm, x, yi, z, 0) >
                WM_MIN_VAL) // inf voxel is wm
            {
    ...
    for (x = x1; x <= x2; x++)
      for (y = y1; y <= y2; y++)
        for (z = z1; z <= z2; z++)
          if (MRIvox(mri_hippo, x, y, z) &&
              ((x < xmedial + 10) || (z > zanterior - 15))) {
            ...
            for (xk = -1; xk <= 1 && !path_to_cortex; xk++) {
              ...
              for (yk = 0; yk <= 1 && !path_to_cortex; yk++) {
                ...
                for (zk = -1; zk <= 1 && !path_to_cortex; zk++) {
                  ...
                  if (MRIgetVoxVal(mri_wm, xi, yi, zi, 0) >
                      WM_MIN_VAL) // wm barrier there
                  {
                    continue;
                  }
                  if (IS_CORTEX(MRIgetVoxVal(mri_aseg, xi, yi, zi,
...
#define WHALF 3
      ...
      for (xk = -WHALF; xk <= WHALF; xk++) {
        ...
        for (yk = -WHALF; yk <= 0; yk++) {
          ...
          for (zk = -WHALF; zk <= WHALF; zk++) {
            ...
            if (MRIgetVoxVal(mri_filled, xi, yi, zi, 0) ==
                0) // wasn't in the path
            {
              continue;
            }
            if (MRIgetVoxVal(mri_wm, xi, yi, zi, 0) >
                MIN_WM_VAL) // already filled
            {
              continue;
            }
  ...
  // RH: now fill inferior and lateral until no voxels are filled or we reach
  // cortex
  niter = 0;
  do {
    xmedial = 0;
...
#define WHALF 3
  ...
  printf(
      "%d voxels added to wm to prevent paths from MTL structures to cortex\n",
      total_filled);
  return (NO_ERROR);
}
  ...
  for (z = 0; z < mri_roi->depth; z++) {
    ...
    for (x = 0; x < mri_roi->width; x++) {
      for (y = 0; y < mri_roi->height; y++) {
        label = MRIgetVoxVal(mri_aseg, x, y, z, 0);
        if ((left && (label == Left_Hippocampus || label == Left_Amygdala)) ||
            ((left == 0) &&
    ...
    for (x = 0; x < mri_roi->width; x++) {
      for (y = 0; y < mri_roi->height; y++) {
        ...
        label = MRIgetVoxVal(mri_aseg, x, y, z, 0);
        if (left) {
          if (((label == Left_Amygdala) || (label == Left_Hippocampus) ||
        ...
        {
          if (((label == Right_Amygdala) || (label == Right_Hippocampus) ||
               (label == Right_Cerebral_Cortex)) &&
              (x > xmid)) {
            MRIvox(mri_roi, x, y, z) = 0;
          }
  ...
  for (z = 0; z < mri_roi->depth; z++) {
    for (x = 0; x < mri_roi->width; x++) {
      for (y = 0; y < mri_roi->height; y++) {
        ...
        alabel =
            (label == Left_Cerebral_Cortex ? Left_Amygdala : Right_Amygdala);
  ...
  for (x = 0; x < mri_wm->width; x++) {
    for (y = 1; y < mri_wm->height; y++) {
      for (z = 0; z < mri_wm->depth; z++) {
        ...
        xi = left ? x + 1 : x - 1;
        if (IS_AMYGDALA(label) &&
            IS_CORTEX(MRIgetVoxVal(mri_aseg, xi, y, z, 0)) &&
        ...
        if (IS_AMYGDALA(label) &&
            IS_CORTEX(MRIgetVoxVal(mri_aseg, xi, yi, z, 0)) &&
