...
MRI *preprocessingStep(tesselation_parms *parms) {
  ...
  mri = MRIclone(parms->mri, nullptr);
...
int saveTesselation2(tesselation_parms *parms) {
  ...
  mris = MRISoverAlloc(nVFMultiplier * parms->vertex_index,
                       nVFMultiplier * parms->face_index, parms->vertex_index,
                       parms->face_index);
    ...
    for (j = ymin; j < ymax; j++) {
      for (i = xmin; i < xmax; i++) {
        ...
        if (((k + 1) < mri->depth) && MRIvox(mri, i, j, k + 1))
          ref += 16;
        if (((k + 1) < mri->depth) && ((i + 1) < mri->width) &&
            MRIvox(mri, i + 1, j, k + 1))
          ref += 32;
        if (((k + 1) < mri->depth) && ((j + 1) < mri->height) &&
            MRIvox(mri, i, j + 1, k + 1))
          ref += 64;
        if (((k + 1) < mri->depth) && ((j + 1) < mri->height) &&
            ((i + 1) < mri->width) && MRIvox(mri, i + 1, j + 1, k + 1))
          ref += 128;
        ...
        case 1:
          while (MC6p[ref][3 * nf] >= 0)
            nf++;
          break;
        case 2:
          while (MC18[ref][3 * nf] >= 0)
            nf++;
          break;
        case 3:
          while (MC6[ref][3 * nf] >= 0)
            nf++;
          break;
        default:
          while (MC26[ref][3 * nf] >= 0)
            nf++;
          break;
        }
        if (nf == 0)
...
int main(int argc, char *argv[]) {
  ...
  if (downsample > 0) {
    MRI *mri_tmp;
    mri_tmp = MRIdownsample2(mri, nullptr);
    MRIfree(&mri);
    mri = mri_tmp;
  }
  {
    MRI *mri_tmp;
    mri_tmp =
        MRIalloc(mri->width + 2, mri->height + 2, mri->depth + 2, mri->type);
    MRIextractInto(mri, mri_tmp, 0, 0, 0, mri->width, mri->height, mri->depth,
                   1, 1, 1);
    MRIfree(&mri);
    mri = mri_tmp;
  }
  ...
  if (argc == 5)
    parms->connectivity = atoi(argv[4]); // connectivity;
  else
  ...
  mris_corrected = MRISextractMainComponent(mris, 0, 1, nullptr);
