...
static int mriRemoveCornerConfiguration(MRI *mri_seg, MRI *mri_orig,
                                        int label) {
  static int niter = 0;
...
int main(int argc, char *argv[]) {
  MRI *mri_seg, *mri_orig, *mri_seg_orig, *mri_old = nullptr;
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_pretess.c,v 1.22 2013/08/30 18:12:25 mreuter Exp $",
      "$Name:  $");
  ...
  if (mri_orig->width != mri_seg->width ||
      mri_orig->height != mri_seg->height || mri_orig->depth != mri_seg->depth)
...
static void print_help() {
  outputHelpXml(mri_pretess_help_xml, mri_pretess_help_xml_len);
}
