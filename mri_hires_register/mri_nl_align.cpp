    ...
    for (y = 0; y < mri_dst->height; y++)
      for (z = 0; z < mri_dst->depth; z++) {
        ...
        if (IS_WMSA(label)) {
          ...
          rh =
              MRIlabelsInNbhd(mri_dst, x, y, z, 4, Right_Cerebral_White_Matter);
          label = lh > rh ? Left_Cerebral_White_Matter
...
int main(int argc, char *argv[]) {
  ...
  if (use_aseg == 0) {
    if (match_peak_intensity_ratio)
      MRImatchIntensityRatio(mri_source, mri_target, mri_source, .8, 1.2, 100,
                             125);
  ...
  if (gcam->width != mri_source->width || gcam->height != mri_source->height ||
  ...
  GCAMregister(gcam, mri_source,
               &mp); // atlas is target, morph target into register with it
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "debug_voxel")) {
    ...
    if (mp.noneg >= 0)
      ...
      printf("allowing folds during numerical minimization and removing them "
             "before termination\n");
    ...
    nargs     = 1;
    printf("levels = %d\n", mp.levels);
  } else if (!stricmp(option, "area_smoothness") ||
             !stricmp(option, "asmooth")) {
    ...
    case 'Z': {
      ...
      if (nozero >= 0)
        ...
        printf(
            "disabling zero image locations for initial run, then enabling\n");
...
static void usage_exit(int ecode) {
  ...
  printf("  -noneg val		: val = 0 or 1; <not> allowing temporary folds "
         "during numerical minimization (default = True)\n");
  printf("  -renormalize val	: val = 0 or 1; <not> renormalizing "
         "intensities\n");
  printf("  -aseg			: treating inputs as segmentations; no "
         "intenisty renormalization; log_likelihood = 0; distance transform "
         "coefficient (dtrans) = 1\n");
  printf("  -diag2 volname	: writing d2 diagnostics for input volume "
         "(*volname*)\n");
  printf("  -OPTIMAL		: using line search optimization (default = "
         "GCAM_INTEGRATE_BOTH)\n");
  printf("  -MOMENTUM		: using fixed time-step integration (default = "
         "GCAM_INTEGRATE_BOTH)\n");
  printf("  -FIXED		: using fixed time-step integration (default = "
         "GCAM_INTEGRATE_BOTH)\n");
  printf("  -distance val		: expanding border by *val* mm every "
         "outer cycle\n");
  printf("  -dtrans val		: setting distance transform coefficient to "
         "*val*\n");
  printf("  -match_peak		: matching peak of intensity ratio "
         "histogram\n");
  printf("  -erode val		: eroding source and target image *val* times "
         "before morphing\n");
  printf("  -match_mean val	: val = 0 or 1; <not> matching peak of "
         "intensity ratio histogram\n");
  printf("  -intensity val	: setting l_log_likelihood to *val* (default = "
         "0.025)\n");
  printf("  -ll val		: setting l_log_likelihood to *val* (default = "
         "0.025)\n");
  ...
  printf("  -asmooth val		: setting l_area_smoothness to "
         "*val*\n");
  ...
  printf("  -min_sigma val	: setting minimum value for sigma to be *val* "
         "(default = .4)\n");
  printf("  -ribbon fname		: reading ribbon from *fname* and "
         "inserting into aseg\n");
  printf("  -rthresh val		: setting compression ratio threshold "
         "to *val*\n");
  printf("  -scale val		: scaling input values by *val*\n");
  printf("  -dt val		: setting dt to *val* (default = 0.005)\n");
  printf("  -passes	val	: integrating in *val* number of passes / go "
         "through all levels *val* number of times (default=3)\n");
  printf("  -skip val		: skipping *val* number of voxels in source "
         "data\n");
  printf("  -apply val		: val = 0 or 1; <not> applying transform after "
         "registration\n");
  printf("  -D val		: setting l_distance to *val* (default = 0)\n");
  printf("  -M val		: setting momentum to *val* (default = .9)\n");
  printf("  -N val		: setting number of iterations to *val* "
         "(default = 1000)\n");
  printf("  -s val		: setting l_smoothness to *val* (default = "
         "2)\n");
  printf("  -T fname		: reading the forward transform / deformation "
         "from *fname*\n");
  printf("  -I fname		: reading the inverse of the transform / "
         "deformation in *fname* \n");
  printf("  -B val		: setting l_binary to *val*\n");
  printf("  -J val		: setting l_jacobian to *val* (default = 1)\n");
  printf("  -Z val		: val = 0 or 1; <not> disabling zero image "
         "locations\n");
  printf(
