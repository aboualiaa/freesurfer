...
static int find_gcam_node(GCA_MORPH *gcam, int label, float x_vox, float y_vox,
                          float z_vox);
...
static int powell_minimize(VOXEL_LIST *vl_lowres, VOXEL_LIST *vl_hires,
                           MATRIX *mat);
...
static double compute_overlap(VOXEL_LIST *vl_lowres, VOXEL_LIST *vl_hires,
                              MATRIX *m_L);
static double compute_distance_transform_sse(VOXEL_LIST *vl_lowres,
                                             VOXEL_LIST *vl_hires, MATRIX *m_L);

static double (*pf_overlap)(VOXEL_LIST *vl_lowres, VOXEL_LIST *vl_hires,
...
static double find_optimal_linear_xform(
    VOXEL_LIST *vl_lowres, VOXEL_LIST *vl_hires, MATRIX *m_L, MATRIX *m_origin,
...
int main(int argc, char *argv[]) {
  char **av, *hires_fname, *aseg_fname, *intensity_fname, *out_fname,
      fname[STRLEN];
  ...
  if (!mri_lowres)
    ErrorExit(ERROR_NOFILE, "%s: could not read aseg label volume %s", Progname,
              aseg_fname);
  if (angio) {
  ...
  } else /* linear transform already computed - compute 3d morph */
  {
    ...
    if (gcam->width != mri_hires->width || gcam->height != mri_hires->height ||
        ...
        for (level = nlevels; level >= 0; level--) {
          ...
          do {
            ...
            printf("outer loop iteration %d completed "
                   ...
                   niter, level_steps, num_this_scale + 1, mp.end_rms,
                   pct_change, mp.start_rms);
...
static void usage_exit(int ecode) {
  printf("usage: %s <hires labeling> <input intensity> <input aseg>"
         " <output xform>\n",
         Progname);
  exit(ecode);
}
...
#define MIN_SEARCH_SCALE 0.01
  ...
  *MATRIX_RELT(m_origin, 4, 4) = 1;
  ...
  trans =
      MAX(MAX_TRANS,
          MAX(MAX(mri_hires->width, mri_hires->height), mri_hires->depth) / 8);
...
#define MIN_SCALES 3
  ...
  do {
    ...
    max_overlap = find_optimal_linear_xform(
        vl_lowres, vl_hires, m_vox_xform, m_origin, -MAX_ANGLE * scale,
        MAX_ANGLE * scale, 1 - MAX_SCALE * scale, 1 + MAX_SCALE * scale,
        -scale * MAX_TRANS, scale * MAX_TRANS, 3, 3, 3, 2);
...
static double find_optimal_translation(VOXEL_LIST *vl_lowres,
  ...
  for (i = 0; i <= nreductions; i++) {
    ...
    if (Gdiag & DIAG_SHOW)
      printf("max overlap = %2.4f @ (%2.1f, %2.1f, %2.1f)\n", max_overlap,
             x_max, y_max, z_max);
