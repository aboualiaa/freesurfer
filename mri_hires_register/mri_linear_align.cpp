...
static int write_snapshot(MRI *mri_target, MRI *mri_source, MATRIX *m_vox_xform,
                          MORPH_PARMS *parms, int fno, int conform,
                          char *fname);
...
static double compute_likelihood(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
                                 MATRIX *m_L, float intensity_scale);
...
static double (*pf_likelihood)(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
                               MATRIX *m_L,
...
static double find_optimal_linear_xform(
    VOXEL_LIST *vl_target, VOXEL_LIST *vl_source, MATRIX *m_L, MATRIX *m_origin,
    float min_angle, float max_angle, float min_scale, float max_scale,
...
int main(int argc, char *argv[]) {
  ...
  mri_tmp = MRIextractRegion(mri_source, nullptr, &box);
  MRIfree(&mri_source);
  mri_source = mri_tmp;
  ...
  mri_tmp = MRIextractRegion(mri_target, nullptr, &box);
  MRIfree(&mri_target);
  mri_target = mri_tmp;
  ...
  vl_target           = VLSTcreate(mri_target, 1, fmax + 1, nullptr, skip, 0);
  vl_target->mri2     = mri_target;
  vl_target->mri_grad = MRIsobel(mri_target, nullptr, nullptr);
  for (i = 0; i < 1; i++) {
    MRI *mri_grad = nullptr;

    printf("------------- outer loop iteration %d ---------------\n", i);
    MRIvalRange(mri_target, &fmin, &fmax);
    if ((mri_grad == nullptr) && (pf_likelihood == compute_gradient_match)) {
      mri_grad = MRIsobel(mri_source, nullptr, nullptr);
    }
    vl_source           = VLSTcreate(mri_source, 1, fmax + 1, nullptr, skip, 0);
    vl_source->mri2     = mri_source;
    vl_source->mri_grad = mri_grad;

    transform =
        compute_optimal_transform(vl_target, vl_source, &parms, transform);
    VLSTfree(&vl_source);
    vl_source       = VLSTcreate(mri_source, 1, fmax + 1, nullptr, skip / 4, 0);
    vl_source->mri2 = mri_source;
    vl_source->mri_grad = mri_grad;
  ...
  if (apply_transform) {
    ...
    LTArasToVoxelXform((LTA *)(transform->xform), mri_orig_source,
                       mri_orig_target);
    FileNameRemoveExtension(out_fname, fname);
    strcat(fname, ".mgz");
    m_vox_xform = ((LTA *)(transform->xform))->xforms[0].m_L;
    mri_aligned = MRIclone(mri_orig_target, nullptr);
    MRIlinearTransformInterp(mri_orig_source, mri_aligned, m_vox_xform,
                             SAMPLE_SINC);
    printf("writing transformed output volume to %s...\n", fname);
    MRIwrite(mri_aligned, fname);
    MRIfree(&mri_aligned);
    LTAvoxelToRasXform((LTA *)(transform->xform), mri_orig_source,
                       mri_orig_target);
  }
  ...
  *MATRIX_RELT(m_origin, 4, 4) = 1;
  ...
  if (transform == nullptr) {
    ...
    *MATRIX_RELT(m_trans, 3, 4) = -dz;
    ...
    VectorFree(&v_ch);
    if (Gdiag & DIAG_WRITE && parms->write_iterations > 0) {
      write_snapshot(mri_target, mri_source, m_vox_xform, parms, parms->start_t,
                     1, nullptr);
    }
    parms->start_t++;
    scale = 1;
...
#define MIN_SCALES 3
  ...
  do {
    ...
    printf("****************************************\n");
    max_likelihood = find_optimal_linear_xform(
        vl_target, vl_source, m_vox_xform, m_origin, -MAX_ANGLE * scale,
        MAX_ANGLE * scale, 1 - MAX_SCALE * scale, 1 + MAX_SCALE * scale,
        -scale * MAX_TRANS, scale * MAX_TRANS, 3, 3, 3, 2, parms->rigid);

    if (Gdiag & DIAG_WRITE && parms->write_iterations > 0) {
      write_snapshot(mri_target, mri_source,
    ...
    niter++;
  } while (nscales < (MIN_SCALES + 2 * parms->rigid) ||
           (done == FALSE)); // allow extra scales for rigid
...
static double compute_likelihood(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
  ...
  if (m_L_inv == nullptr)
    ErrorExit(ERROR_BADPARM, "compute_likelihood: singular matrix.");
...
static double compute_trimmed_likelihood(VOXEL_LIST *vl_target,
                                         VOXEL_LIST *vl_source, MATRIX *m_L,
                                         float intensity_scale) {
  int width, height, depth, hwidth, hheight, hdepth, i, min_bin, max_bin, bin,
      total_count, zero_bin, count_thresh;
  MRI *      mri_target, *mri_source;
...
#define NBINS 256
  ...
  for (min_bin = zero_bin - 1, max_bin = zero_bin + 1;
       total_count < count_thresh; min_bin--, max_bin++) {
    if (min_bin >= 0)
...
static double find_optimal_translation(VOXEL_LIST *vl_target,
  ...
  double  x_trans, y_trans, z_trans, x_max, y_max, z_max, delta, likelihood,
      max_likelihood, mean_trans;
  int i;
  ...
  for (i = 0; i <= nreductions; i++) {
    ...
    if (Gdiag & DIAG_SHOW) {
      printf("scanning translations %2.4f->%2.4f (step %2.4f) ", min_trans,
             max_trans, delta);
      fflush(stdout);
    }
    ...
    if (Gdiag & DIAG_SHOW)
      printf("max likelihood = %2.4f @ (%2.1f, %2.1f, %2.1f)\n", max_likelihood,
             x_max, y_max, z_max);
...
static double find_optimal_linear_xform(
  ...
  double x_angle, y_angle, z_angle, x_max_rot, y_max_rot, z_max_rot, delta_rot,
      ...
      x_max_trans, y_max_trans, likelihood, max_likelihood, z_max_trans,
      mean_trans;
  int  i;
  MRI *mri_target, *mri_source;
    ...
    if (Gdiag & DIAG_SHOW) {
      if (rigid)
        printf("  scanning %2.4f degree nbhd (%2.4f)\n"
               ...
               (float)DEGREES(max_angle), (float)DEGREES(delta_rot), min_trans,
               max_trans, delta_trans);
      else
        printf("  scanning %2.4f degree nbhd (%2.4f)\n"
...
static int powell_minimize(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
  ...
  if (Gdiag & DIAG_WRITE && parms.write_iterations > 0)
    write_snapshot(vl_target->mri, vl_source->mri, mat, &parms, parms.start_t,
                   1, nullptr);
  parms.start_t++;
  Gdiag = diag;
  if (iter != 0)
    do {
      ...
      *MATRIX_RELT(mat, 4, 4) = 1.0 ;
      ...
      printf("%3.3d: best alignment after powell: "
             ...
             parms.start_t, fret, iter, intensity_scale);
      if (Gdiag & DIAG_WRITE && parms.write_iterations > 0)
        write_snapshot(vl_target->mri, vl_source->mri, mat, &parms,
                       parms.start_t, 1, nullptr);
      parms.start_t++;
    } while (fret < fstart);
...
static int powell_minimize_rigid(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
  ...
  printf("%3.3d: best alignment at after powell: "
         "%2.5f (%d steps, iscale=%2.3f)\n\tparms = (%2.4f, %2.4f, %2.4f) + "
         "(%2.2f, %2.2f, %2.2f)\n",
         parms.start_t, fret, iter, intensity_scale, DEGREES(p[1]),
         DEGREES(p[2]), DEGREES(p[3]), p[4], p[5], p[6]);
  if (Gdiag & DIAG_WRITE && parms.write_iterations > 0)
    write_snapshot(vl_target->mri, vl_source->mri, mat, &parms, parms.start_t,
                   1, nullptr);
  parms.start_t++;
...
static double compute_gradient_match(VOXEL_LIST *vl_target,
  ...
  double  match;
  double  num, xd, yd, zd, dxs, dys, dzs, dxt, dyt, dzt, dens, dx, dy, dz, dent,
      ctheta, dot, norms, normt, den, e1x, e1y, e1z, e2x, e2y, e2z, e3x, e3y,
      e3z;
  ...
  *MATRIX_RELT(m_I, 4, 3) = 1.0;
  ...
  m_L_inv = MatrixInverse(m_L, nullptr);
  if (m_L_inv == nullptr)
    ErrorExit(ERROR_BADPARM, "compute_gradient_match: singular matrix.");
  ...
  */
  ...
  for (i = 0; i < vl_source->nvox; i++) {
    ...
    if (zd < 0)
      ...
      zd = depth - 1;
    MRIsampleVolumeFrameType(mri_grad_target, xd, yd, zd, 0, SAMPLE_TRILINEAR,
                             &dx);
    MRIsampleVolumeFrameType(mri_grad_target, xd, yd, zd, 1, SAMPLE_TRILINEAR,
                             &dy);
    MRIsampleVolumeFrameType(mri_grad_target, xd, yd, zd, 2, SAMPLE_TRILINEAR,
                             &dz);
  ...
  for (i = 0; i < vl_target->nvox; i++) {
    ...
    if (zd < 0)
      ...
      zd = hdepth - 1;
    MRIsampleVolumeFrameType(mri_grad_source, xd, yd, zd, 0, SAMPLE_TRILINEAR,
                             &dxs);
    MRIsampleVolumeFrameType(mri_grad_source, xd, yd, zd, 1, SAMPLE_TRILINEAR,
                             &dys);
    MRIsampleVolumeFrameType(mri_grad_source, xd, yd, zd, 2, SAMPLE_TRILINEAR,
                             &dzs);
    norms = sqrt(dxs * dxs + dys * dys + dzs * dzs);
    normt = sqrt(dxt * dxt + dyt * dyt + dzt * dzt);
    dot   = dxs * dxt + dys * dyt + dzs * dzt;
