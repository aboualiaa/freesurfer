/**
 * @file  mri_linear_align_binary.c
...
int main(int argc, char *argv[]) {
  ...
  mri_whole_source = MRIcopy(mri_source, nullptr); // to keep track of orig geom
  ...
  if (FZERO(binarize) /* && !use_target_label*/) {
    ...
    mri_tmp = MRIextractRegion(mri_orig_source, nullptr, &box);
  ...
  if (surf_flag) {
    ...
    if (!mri_target)
      ErrorExit(ERROR_NOFILE, "%s: could not read target surface %s", Progname,
                target_fname);
    mri_target = MRISfillInterior(mris, .25, nullptr);
  ...
  mri_whole_target = MRIcopy(mri_target, nullptr); // to keep track of orig geom
  ...
  if (pf_overlap == compute_distance_transform_sse) {
    ...
    mri_dist_src = MRIdistanceTransform(mri_source, nullptr, binary_label, -1,
  ...
  m_L = nullptr; // to avoid compiler warning. npasses will never be 0
  for (i = 0; i < npasses; i++) {
    printf("------------- outer loop iteration %d of %d - skip %d, %d "
           "---------------\n",
           i + 1, npasses, source_skip, target_skip);

    vl_target = VLSTcreate(mri_target, binary_label, binary_label, nullptr,
                           target_skip, 0);
    // skip=0 for target!
    printf("%d voxels in target list\n", vl_target->nvox);
    vl_target->mri2 = mri_dist_target;

    vl_source = VLSTcreate(mri_source, 1, 255, nullptr, source_skip, 0);
    ...
    VLSTfree(&vl_source);
    vl_source = VLSTcreate(mri_source, 1, 255, nullptr, source_skip / 4, 0);
    vl_source->mri2 = mri_dist_src;
...
static void usage_exit(int ecode) {
  ...
  printf(
      "\t-h <target label>  -  assume source is hires hippocampal labeling\n");
  exit(ecode);
}
  ...
  *MATRIX_RELT(m_origin, 4, 4) = 1;
  ...
  if (transform == nullptr) {
    ...
    *MATRIX_RELT(m_trans, 3, 4) = -dz;
    ...
    VectorFree(&v_ch);
    if (Gdiag & DIAG_WRITE && parms->write_iterations > 0) {
      write_snapshot(mri_target, mri_orig_source, m_vox_xform, parms,
                     parms->start_t, conform, nullptr, nfilter);
    }
    parms->start_t++;
  } else {
...
#define MIN_SCALES 3
  ...
  do {
    ...
    printf("****************************************\n");
    max_overlap = find_optimal_linear_xform(
    ...
    if (parms->write_iterations != 0) {
      write_snapshot(mri_target, mri_orig_source,
                     ...
                     parms->start_t + niter, conform, nullptr, nfilter);
    }
...
#define IN_UNION 254
  ...
  for (intersection = Union = 0.0, i = 0; i < vl_source->nvox; i++) {
    ...
    if (xd == Gx && yd == Gy && zd == Gz)
      DiagBreak();
    if (xd >= 0 && xd < width && yd >= 0 && yd < height && zd >= 0 &&
        zd < depth) {
...
static double find_optimal_translation(VOXEL_LIST *vl_target,
  ...
  for (i = 0; i <= nreductions; i++) {
    ...
    if (FZERO(delta))
      return (max_overlap);
    if (Gdiag & DIAG_SHOW) {
      printf("scanning translations %2.2f->%2.2f (step %2.1f) ", min_trans,
             max_trans, delta);
      fflush(stdout);
    }
    ...
    if (Gdiag & DIAG_SHOW)
      printf("max overlap = %2.4f @ (%2.2f, %2.2f, %2.2f)\n", max_overlap,
             x_max, y_max, z_max);
...
static int powell_minimize(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
  ...
  do {
    ...
    write_snapshot(vl_target->mri, mri_orig_source, mat, &parms,
                   parms.start_t++, conform, nullptr, nfilter);
  } while (fret < fstart);
...
static int powell_minimize_rigid(VOXEL_LIST *vl_target, VOXEL_LIST *vl_source,
  ...
  printf("%3.3d: best alignment at after powell: "
         ...
         "%2.2f)\n",
         parms.start_t, fret, iter, DEGREES(p[1]), DEGREES(p[2]), DEGREES(p[3]),
         p[4], p[5], p[6]);
  ...
  do {
    ...
    *MATRIX_RELT(mat, 4, 4) = 1.0 ;
    ...
    printf("%3.3d: best alignment at after powell: "
           "%2.5f (%d steps)\n\tparms = (%2.4f, %2.4f, %2.4f) + (%2.2f, %2.2f, "
           "%2.2f)\n",
           parms.start_t, fret, iter, DEGREES(p[1]), DEGREES(p[2]),
