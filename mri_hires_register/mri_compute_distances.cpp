...
int main(int argc, char *argv[]) {
  ...
  for (i = 3; i < argc; i++) {
    ...
    fprintf(stderr, "computing %s distance for label %s (%d)...\n",
            centroid ? "centroid" : "Hausdorff", cma_label_to_name(label),
            label);
...
static float MRIcomputeAverageHausdorffDistance(MRI *mri1, MRI *mri2, int label,
  ...
  for (dsq = 0.0, total_dist = 0.0, s1 = 1; s1 < mriseg1->nsegments; s1++) {
    ...
    for (min_dist = -1.0, s2 = 0; s2 < mriseg2->nsegments; s2++) {
      ...
      if (centroid)
        dist = MRIcomputeSegmentPairCentroidDistance(mri1, mri2, mseg1, mseg2);
      else
        dist = MRIcomputeSegmentPairHausdorffDistance(mri1, mri2, mseg1, mseg2);
