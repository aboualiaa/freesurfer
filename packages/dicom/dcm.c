 ...
 */
static CONDITION newElementItem(DCM_ELEMENT *src, CTNBOOLEAN allocateData,
                                PRV_ELEMENT_ITEM **dst);
static CONDITION findCreateGroup(PRIVATE_OBJECT **object, unsigned short group,
                                 PRV_GROUP_ITEM **groupPtr);
static CONDITION insertNewElement(PRIVATE_OBJECT **object,
                                  DCM_ELEMENT *element);
static CONDITION updateObjectType(PRIVATE_OBJECT **object,
                                  DCM_ELEMENT *element);
static CONDITION updateSpecialElements(PRIVATE_OBJECT **object,
                                       PRV_ELEMENT_ITEM *item);
static void exportFixedFields(DCM_ELEMENT *element, unsigned char *b,
                              U32 length, int byteOrder, CTNBOOLEAN explicitVR,
                              U32 *rtnLength);
static CONDITION exportData(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *item,
                            unsigned char *src, unsigned char *dst, U32 length,
                            int byteOrder, U32 *rtnLength);
#ifdef MACOS
...
static CONDITION writeFile(void *buffer, U32 length, int flag,
                           void /* int */ *fd);
static CONDITION countBytes(void *buffer, U32 length, int flag,
                            void /* unsigned long */ *sizePtr);
static CONDITION exportStream(DCM_OBJECT **callerObject, unsigned long opt,
                              void *buffer, U32 bufferlength,
                              DCM_EXPORT_STREAM_CALLBACK *callback, void *ctx,
                              int sequenceLevel);
...
readFile1(const char *name, unsigned char *callerBuf, int fd, U32 size,
          ...
          PRIVATE_OBJECT **parentObject, DCM_OBJECT **callerObject,
          U32 *scannedLength, CTNBOOLEAN *remainOpenFlag, void *ctx,
          CONDITION (*rd)(void *ctx, void *buf, int toRead, int *bytesRead),
          CONDITION (*sk)(void *ctx, int offset, int flag));
...
static CONDITION copyData(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *item,
                          DCM_ELEMENT *to, U32 *rtnLength);
static CONDITION readLengthToEnd(int fd, const char *fileName,
                                 unsigned long opt, U32 *lengthToEnd);
#ifdef LITTLE_ENDIAN_ARCHITECTURE
static void swapATGroupElement(DCM_ELEMENT *e);
#endif
static void dumpBinaryData(void *d, DCM_VALUEREPRESENTATION vr, long vm,
                           long vmLimit);
static void compareGroup(PRV_GROUP_ITEM *g1, PRV_GROUP_ITEM *g2,
...
DCM_OpenFile(const char *name, unsigned long opt, DCM_OBJECT **callerObject) {
  ...
  if ((opt & (DCM_ORDERMASK | DCM_FILEFORMATMASK)) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION),
                              "Byte order", "DCM_OpenFile");
  ...
  if ((opt & DCM_LENGTHTOENDMASK) == DCM_USELENGTHTOEND) {
    cond =
        readLengthToEnd(fd, name, opt & (~DCM_LENGTHTOENDMASK), &lengthToEnd);
    if (cond != DCM_NORMAL) {
      (void)close(fd);
      return COND_PushCondition(DCM_FILEOPENFAILED,
                                DCM_Message(DCM_FILEOPENFAILED), name,
...
DCM_ReadStream(DCM_OBJECT **callerObject, unsigned long opt, long size,
  ...
  if ((opt & (DCM_ORDERMASK | DCM_FILEFORMATMASK)) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION),
                              "Byte order", "DCM_ReadStream");

  cond = readFile1("", NULL, fd, size, &fileOffset, 0, opt, NULL, callerObject,
                   NULL, &remainFileOpen, ctx, rd, sk);
  if (cond != DCM_NORMAL)
...
DCM_CreateObject(DCM_OBJECT **object, unsigned long opt) {
  ...
  *obj;
  ...
  if (object == NULL) {
    (void)COND_PushCondition(DCM_NULLADDRESS, DCM_Message(DCM_NULLADDRESS),
                             "DCM_CreateObject");
  ...
  if (obj == NULL) {
    (void)COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE),
                             sizeof(PRIVATE_OBJECT), "DCM_CreateObject");
    *object = NULL;
    return COND_PushCondition(DCM_OBJECTCREATEFAILED,
                              DCM_Message(DCM_OBJECTCREATEFAILED),
                              "DCM_CreateObject");
...
DCM_CloseObject(DCM_OBJECT **callerObject) {
  ...
  *sequenceItem;
  ...
  while ((group = (PRV_GROUP_ITEM *)LST_Pop(&(*object)->groupList)) != NULL) {
    ...
    while ((element = (PRV_ELEMENT_ITEM *)LST_Pop(&group->elementList)) !=
           NULL) {
      if (debug)
      ...
      if (element->element.representation == DCM_SQ) {
        ...
        if (element->element.d.sq != NULL) {
          while ((sequenceItem = (DCM_SEQUENCE_ITEM *)LST_Pop(
                      &element->element.d.sq)) != NULL) {
        ...
        if (element->element.d.fragments != NULL) {
          while ((fragmentItem = (DCM_FRAGMENT_ITEM *)LST_Pop(
                      &element->element.d.fragments)) != NULL) {
  ...
  if ((*object)->deleteFlag) {
    if (unlink((*object)->fileName) != 0) {
      ...
      **/
      cond = COND_PushCondition(
          DCM_FILEDELETEFAILED, DCM_Message(DCM_FILEDELETEFAILED),
          (*object)->fileName, strerror(errno), "DCM_CloseObject");
    }
  }
  CTN_FREE(*object);
  *object = NULL;
...
DCM_AddElement(DCM_OBJECT **callerObject, DCM_ELEMENT *element) {
  ...
  *groupItem;
  ...
  if ((DCM_TAG_ELEMENT(element->tag) == 0x0000))
    return COND_PushCondition(DCM_ILLEGALADD, DCM_Message(DCM_ILLEGALADD),
  ...
  if (cond != DCM_NORMAL) {
    ...
    if (element->representation != DCM_UN &&
      ...
      return COND_PushCondition(
          DCM_ILLEGALREPRESENTATION, DCM_Message(DCM_ILLEGALREPRESENTATION),
  ...
  if (cond != DCM_NORMAL)
    return COND_PushCondition(DCM_INSERTFAILED, DCM_Message(DCM_INSERTFAILED),
                              DCM_TAG_GROUP(element->tag),
                              DCM_TAG_ELEMENT(element->tag), "DCM_AddElement");
...
DCM_AddSequenceElement(DCM_OBJECT **callerObject, DCM_ELEMENT *element) {
  ...
  if (localElement.representation != DCM_SQ) {
    return COND_PushCondition(DCM_NOTASEQUENCE, DCM_Message(DCM_NOTASEQUENCE),
...
DCM_GetElementValue(DCM_OBJECT **callerObject, DCM_ELEMENT *element,
  ...
  *elementItem;
  ...
  if (groupItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag),
        "DCM_GetElementValue");
  ...
  if (elementItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(element->tag), DCM_TAG_GROUP(element->tag),
        "DCM_GetElementValue");
      ...
      *rtnLength = l;
      {
        if (elementItem->element.d.ot == NULL) {
          if ((*object)->fd != -1) {
            (void)lseek((*object)->fd, elementItem->dataOffset + (long)p,
                        SEEK_SET);
            nBytes = read((*object)->fd, element->d.ot, (int)l);
          } else {
            (*object)->sk((*object)->userCtx,
                          (long)(elementItem->dataOffset + (long)p), SEEK_SET);
            int nBytes_int;
          ...
          if ((unsigned)nBytes != l) {
            return COND_PushCondition(
                DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR),
                (*object)->fileName, "DCM_GetElementValue");
          }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
...
DCM_GetElementValueOffset(DCM_OBJECT **callerObject, DCM_ELEMENT *element,
...

  {
    ...
    ;
    if ((long)p > elementItem->element.length)
      return COND_PushCondition(DCM_BADOFFSET, DCM_Message(DCM_BADOFFSET),
                                (int)offset, (int)elementItem->element.length,
                                "DCM_GetElementValueLength");

    l = element->length;
    if (l + offset > elementItem->element.length) {
      return COND_PushCondition(
          DCM_BADLENGTH, DCM_Message(DCM_BADLENGTH), (int)offset, (int)l,
          (int)elementItem->element.length, "DCM_GetElementValueLength");
    }
    {
      if (elementItem->element.d.ot == NULL) {
        if ((*object)->fd != -1) {
        ...
        if ((unsigned)nBytes != l) {
          return COND_PushCondition(
              DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR),
              (*object)->fileName, "DCM_GetElementValueValue");
        }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
        if (elementItem->element.representation == DCM_AT) {
          DCM_ELEMENT e;
...
CONDITION
DCM_ImportStream(unsigned char *buf, unsigned long length, unsigned long opt,
  ...
  if ((opt & DCM_ORDERMASK) == 0)
    return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION),
                              "Byte order", "DCM_ImportStream");

  return readFile1("", buf, -1, length, 0, 0, opt, NULL, callerObject, NULL,
                   NULL, NULL, NULL, NULL);
}
...
CONDITION
DCM_ExportStream(DCM_OBJECT **callerObject, unsigned long opt, void *buffer,
  ...
  **object;
  ...
  return exportStream(callerObject, opt, buffer, bufferlength, callback, ctx,
                      0);
}
  ...
  *sq;
  ...
  while (groupItem != NULL) {
    ...
    if (elementItem != NULL)
      (void)LST_Position(&groupItem->elementList, elementItem);
    while (elementItem != NULL) {
      (void)printf("%04x %04x %8d ", DCM_TAG_GROUP(elementItem->element.tag),
      ...
      if (elementItem->element.d.ot == NULL)
        ...
        case DCM_US:
          ...
          if (vm > 1)
            dumpBinaryData(
                elementItem->element.d.ot, elementItem->element.representation,
  ...
  *sq;
  ...
  while (groupItem != NULL) {
    ...
    if (elementItem != NULL)
      (void)LST_Position(&groupItem->elementList, elementItem);
    while (elementItem != NULL) {
      (void)printf("%s%04x %04x %8d ", prefix,
  ...
  **object;
  ...
  if (fd < 0) {
    return COND_PushCondition(DCM_FILECREATEFAILED,
                              DCM_Message(DCM_FILECREATEFAILED), file,
                              strerror(errno), "DCM_WriteFile");
  }
  cond = DCM_ExportStream(callerObject, opt, buf, (unsigned long)sizeof(buf),
                          writeFile, &fd);
  if (cond != DCM_NORMAL)
    return cond;
...
*/
CONDITION
DCM_ParseObject(DCM_OBJECT **callerObject, DCM_ELEMENT *vector, int count,
...
DCM_RemoveGroup(DCM_OBJECT **callerObject, unsigned short group) {
  ...
  *elementItem;
  ...
  groupItem = (PRV_GROUP_ITEM *)LST_Head(&(*object)->groupList);
  if (groupItem == NULL)
    return COND_PushCondition(DCM_GROUPNOTFOUND, DCM_Message(DCM_GROUPNOTFOUND),
  ...
  while ((elementItem = (PRV_ELEMENT_ITEM *)LST_Pop(&groupItem->elementList)) !=
         NULL)
    CTN_FREE(elementItem);
...
DCM_GetSequenceList(DCM_OBJECT **object, DCM_TAG tag, LST_HEAD **list) {
  ...
  *elementItem;
  ...
  if (groupItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetSequenceList");
  ...
  if (elementItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(tag), DCM_TAG_GROUP(tag), "DCM_GetSequenceTag");
...
DCM_GetSequenceElement(DCM_OBJECT **object, DCM_TAG top, DCM_ELEMENT *e) {
  ...
  if (elementItem == NULL) {
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(top), DCM_TAG_ELEMENT(top), "DCM_GetElementSequence");
  }
...
*/
CONDITION
DCM_GetElementValueList(DCM_OBJECT **object, DCM_TAG tag, size_t structureSize,
  ...
  *elementItem;
  ...
  if (!found)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetElementValueList");

  if (!DCM_IsString(elementItem->element.representation)) {
    return COND_PushCondition(DCM_UNEXPECTEDREPRESENTATION,
                              DCM_Message(DCM_UNEXPECTEDREPRESENTATION),
                              "DCM_GetElementValueList", "string");
  }
...
DCM_GetElement(DCM_OBJECT **callerObject, DCM_TAG tag, DCM_ELEMENT *element) {
  ...
  *elementItem;
  ...
  if (elementItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetElementVM");
  ...
  *dst = (PRV_ELEMENT_ITEM *)CTN_MALLOC(sizeof(PRV_ELEMENT_ITEM) + l);
  ...
  if (*dst == NULL) {
    return COND_PushCondition(DCM_ELEMENTCREATEFAILED,
                              ...
                              "newElementItem", DCM_TAG_GROUP(src->tag),
                              DCM_TAG_ELEMENT(src->tag), l);
  }
  ...
  *item;
...

  {
    ...
    if (newGroupItem == NULL)
      return COND_PushCondition(
          DCM_ELEMENTCREATEFAILED, DCM_Message(DCM_ELEMENTCREATEFAILED),
          "findCreateGroup", group, 0xffff, sizeof(*newGroupItem));
    ...
    *groupItem = newGroupItem;
    ...
    if ((*object)->groupLengthFlag) {
      ...
      if (LST_Insert(&newGroupItem->elementList, groupLengthItem,
                     LST_K_AFTER) != LST_NORMAL)
        return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE),
                                  "findCreateGroup");
  ...
  *groupItem;
  ...
  if ((newItem->element.length & 1) != 0) {
    if (newItem->element.representation == DCM_AE) {
      ...
      CTN_FREE(newItem);
      return COND_PushCondition(
          DCM_UNEVENELEMENTLENGTH, DCM_Message(DCM_UNEVENELEMENTLENGTH),
          DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag),
          element->length, "insertNewElement");
    }
  } else if (newItem->element.representation != DCM_SQ) {
  ...
  if ((nextItem = (PRV_ELEMENT_ITEM *)LST_Head(&groupItem->elementList)) ==
      NULL) {
...
static CONDITION insertThisElementItem(PRIVATE_OBJECT **object,
  ...
  cond =
      findCreateGroup(object, DCM_TAG_GROUP(newItem->element.tag), &groupItem);
...
static void exportVRLength(DCM_ELEMENT *e, unsigned char *b, int byteOrder,
                           U32 *rtnLength) {
  const char *c = "xx";
  unsigned char *p;
  U16 shortLength;
...
static CONDITION exportPreamble(PRIVATE_OBJECT **obj, unsigned char *dst,
  ...
  *rtnLength = 0;
  if (bufferLength < (DCM_PREAMBLELENGTH + 4))
    return COND_PushCondition(DCM_EXPORTBUFFERTOOSMALL,
                              ...
                              (int)bufferLength, "exportPreamble");
...
static void exportFixedFields(DCM_ELEMENT *e, unsigned char *b, U32 length,
...
static CONDITION exportData(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *item,
                            unsigned char *src, unsigned char *b, U32 length,
                            int byteOrder, U32 *rtnLength) {
  ...
  if (element->d.ot == NULL) {
    ...
    if ((U32)nBytes != length) {
      ...
      (void)COND_PushCondition(
          DCM_GENERALWARNING, DCM_Message(DCM_GENERALWARNING), "exportData", b);
      return COND_PushCondition(DCM_FILEACCESSERROR,
                                DCM_Message(DCM_FILEACCESSERROR),
                                (*object)->fileName, "exportData");
    }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
  ...
  if (element->d.ot == NULL) {
    ...
    while (toExport > 0) {
      ...
      if ((*object)->fd != -1) {
        ...
        int nBytes_int;
        cond = (*object)->rd((*object)->userCtx, buffer, (long)length,
                             &nBytes_int);
        nBytes = (ssize_t)nBytes_int;
      }
      if ((U32)nBytes != length) {
        ...
        return COND_PushCondition(
            DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR),
            (*object)->fileName, "exportEncapsualtedPixels");
      }
    ...
    e.length = 0xffffffff;
    exportFixedFields(&e, buffer, bufferlength, LITTLE_ORDER /*byteOrder*/,
                      1 /* explicitV*/, &rtnLength);
    toExport = rtnLength;
    e.tag = 0xfffee000;
    e.length = 0;
    ...
    exportFixedFields(&e, buffer + toExport, bufferlength,
                      LITTLE_ORDER /*byteOrder*/, 1 /* explicitV*/, &rtnLength);
    toExport += rtnLength;
...
static CONDITION exportPixels(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *item,
                              int encapsulatedPixels, unsigned char *buffer,
                              U32 bufferlength,
                              DCM_EXPORT_STREAM_CALLBACK *callback, void *ctx,
  ...
  if (encapsulatedPixels) {
    return exportEncapsulatedPixels(object, item, buffer, bufferlength,
                                    callback, ctx);
  }
  ...
  dst = buffer;
  c = bufferlength;
  exportFixedFields(element, dst, bufferlength, byteOrder, explicitVR,
                    &rtnLength);
  dst += rtnLength;
  c -= rtnLength;
  ...
  while (remainingData > 0) {
    if (debug) {
      fprintf(stderr, "Export: (%08x) %d\n", (unsigned int)element->tag,
              element->length);
    }

    if (element->d.ot != NULL) {
      remainingData =
          element->length - (src - ((unsigned char *)element->d.ot));
    } else {
      remainingData =
          element->length - (item->currentOffset - item->dataOffset);
    }

    exportLength = (remainingData < c) ? remainingData : c;
    cond =
        exportData(object, item, src, dst, exportLength, byteOrder, &rtnLength);
    if (cond != DCM_NORMAL)
      return cond;
...
static CONDITION checkObject(PRIVATE_OBJECT **object, const char *caller) {
  ...
  if (strcmp((*object)->keyType, KEY_DCM_OBJECT) != 0)
    return COND_PushCondition(DCM_ILLEGALOBJECT, DCM_Message(DCM_ILLEGALOBJECT),
                              caller);
  return DCM_NORMAL;
}
...
static CONDITION writeFile(void *buffer, U32 length, int flag,
  ...
  if (bytesWritten != (int)length)
    return COND_PushCondition(DCM_FILEIOERROR, DCM_Message(DCM_FILEIOERROR), "",
                              strerror(errno), "writeFile");
  else
    return DCM_NORMAL;
}
...
static CONDITION setFileOptions(DCM_OBJECT **obj, unsigned long *opt) {
  ...
  DCM_ELEMENT e = {DCM_METATRANSFERSYNTAX, DCM_UI, "", 1,
                   sizeof(xferSyntax),     {NULL}};
...
static CONDITION extractFileOptions(unsigned long opt, CTNBOOLEAN *part10File,
...
static U32 computeGroupLength(PRV_GROUP_ITEM *groupItem,
                              CTNBOOLEAN explicitVR) {
  return (explicitVR) ? groupItem->baseLength + 4 * groupItem->longVRAttributes
                      : groupItem->baseLength;
}
...
static CONDITION exportStream(DCM_OBJECT **callerObject, unsigned long opt,
  ...
  *sequenceItem;
  ...
  CTNBOOLEAN
  unspecifiedSQLength = FALSE, explicitVR = FALSE, part10File = FALSE,
  encapsulatedPixels = FALSE;
  ...
  if (sequenceLevel != 0) {
    ...
    exportFixedFields(&itemMarker, dst, bufferlength, byteOrder, explicitVR,
                      &rtnLength);
    dst += rtnLength;
    c -= rtnLength;
    bytesExported += rtnLength;
  ...
  while (groupItem != NULL) {
    if (part10File && groupItem->group != DCM_GROUPFILEMETA) {
      if (opt != fileOptions) {
        opt = fileOptions;
        cond = extractFileOptions(opt, &part10File, &explicitVR, &byteOrder,
                                  &encapsulatedPixels);
        if (cond != DCM_NORMAL)
          return cond;
      }
      ...
      if (element.tag == DCM_PXLPIXELDATA) {
        ...
        if (element.d.sq != NULL) {
          ...
          while (sequenceItem != NULL) {
            cond = exportStream(&sequenceItem->object, opt, buffer,
                                bufferlength, callback, ctx, sequenceLevel + 1);
            if (cond != DCM_NORMAL)
              return cond;
        ...
        if (element.length == DCM_UNSPECIFIEDLENGTH) {
          ...
          exportFixedFields(&sequenceDelimiter, dst, bufferlength, byteOrder,
        ...
        while (remainingData > 0) {
          ...
          if (element.d.ot != NULL)
            ...
            remainingData = element.length - (elementItem->currentOffset -
                                              elementItem->dataOffset);

          exportLength = (remainingData < c) ? remainingData : c;
          cond = exportData(object, elementItem, src, dst, exportLength,
                            byteOrder, &rtnLength);
          if (cond != DCM_NORMAL)
            return cond;
  ...
  if ((sequenceLevel != 0) && unspecifiedSQLength) {
    ...
    exportFixedFields(&itemDelimiter, dst, bufferlength, byteOrder, explicitVR,
                      &rtnLength);
    dst += rtnLength;
    c -= rtnLength;
    bytesExported += rtnLength;
...
static CONDITION readPreamble(const char *name, unsigned char **ptr, int fd,
                              U32 *size, off_t *fileOffset,
                              CTNBOOLEAN knownLength, PRIVATE_OBJECT **object,
                              U32 *scannedLength) {
  int nBytes, tmp;
  CONDITION cond;
  char label[4];
  ...
  if (*ptr == NULL) {
    if (fd != -1) {
      ...
      cond = (*object)->rd((*object)->userCtx, label, sizeof(label), &tmp);
      nBytes += tmp;
    }
...
readVRLength(const char *name, unsigned char **ptr, int fd, U32 *size,
  ...
  if (explicitVR) {
    ...
    if (vrPtr->representation != e->representation) {
      if (vrPtr->representation == DCM_OB) {
      ...
      } else if (e->representation == DCM_UN || e->representation == DCM_CTX ||
        ...
        if (e->tag != DCM_PXLPIXELDATA)
          return COND_PushCondition(DCM_VRMISMATCH, DCM_Message(DCM_VRMISMATCH),
                                    vrCode, e->tag);
      }
    }
    if (vrPtr->representation != DCM_OW && vrPtr->representation != DCM_OB &&
        vrPtr->representation != DCM_UN && vrPtr->representation != DCM_UT &&
  ...
  if (((e->length & 1) != 0) && (e->length != DCM_UNSPECIFIEDLENGTH)) {
    ...
    return COND_PushCondition(
        DCM_UNEVENELEMENTLENGTH, DCM_Message(DCM_UNEVENELEMENTLENGTH),
        DCM_TAG_GROUP(e->tag), DCM_TAG_ELEMENT(e->tag), e->length, "readFile");
  }
  if ((e->length != (U32)DCM_UNSPECIFIEDLENGTH) && (e->length > (U32)(*size))) {
    if (debug)
      (void)DCM_DumpElements((DCM_OBJECT **)object, 0);
...
static CONDITION readSequence(
    const char *name, unsigned char **ptr, int fd, U32 *size, off_t *fileOffset,
    int recursionLevel, unsigned long opt, int byteOrder, CTNBOOLEAN explicitVR,
    CTNBOOLEAN acceptVRMismatch, CTNBOOLEAN fileFlag,
    CTNBOOLEAN *remainOpenFlag, CTNBOOLEAN convertFlag, PRIVATE_OBJECT **object,
    U32 *scannedLength, DCM_ELEMENT *e, PRV_ELEMENT_ITEM **elementItem) {
  ...
  *sequenceObject;
  ...
  while (!sequenceDone) {
    ...
    flag = readGroupElement(name, ptr, fd, &localLength, fileOffset,
                            knownLength, byteOrder, explicitVR,
                            acceptVRMismatch, object, &sequenceLength, &tagE);
    ...
    if (debug)
      fprintf(stderr, "Sequence item: %4x %4x %d (%x)\n",
              DCM_TAG_GROUP(tagE.tag), DCM_TAG_ELEMENT(tagE.tag), tagE.length,
              (unsigned int)tagE.length);
    if (tagE.tag == DCM_DLMITEM) {
      localPtr = *ptr;
      cond = readFile1(name, localPtr, fd, tagE.length, fileOffset,
                       recursionLevel + 1, opt, object, &sequenceObject,
                       &sequenceLength, remainOpenFlag, (*object)->userCtx,
                       (*object)->rd, (*object)->sk);
      if (!fileFlag)
...
static CONDITION scanCompressedPixels(
    const char *name, unsigned char **ptr, int fd, U32 *size, off_t *fileOffset,
    int recursionLevel, unsigned long opt, int byteOrder, CTNBOOLEAN explicitVR,
    CTNBOOLEAN acceptVRMismatch, CTNBOOLEAN fileFlag,
    CTNBOOLEAN *remainOpenFlag, CTNBOOLEAN convertFlag, PRIVATE_OBJECT **object,
  ...
  while (!sequenceDone) {
    sequenceLength = 0;
    flag = readGroupElement(name, ptr, fd, &localLength, fileOffset, FALSE,
                            byteOrder, explicitVR, acceptVRMismatch, object,
                            &sequenceLength, &tagE);
    if (flag == DCM_STREAMCOMPLETE)
      break;
...
static CONDITION readData(const char *name, unsigned char **ptr, int fd,
                          U32 *size, off_t *fileOffset, CTNBOOLEAN knownLength,
                          int byteOrder, CTNBOOLEAN explicitVR,
                          CTNBOOLEAN acceptVRMismatch, CTNBOOLEAN fileFlag,
                          CTNBOOLEAN *remainOpenFlag, CTNBOOLEAN convertFlag,
                          PRIVATE_OBJECT **object, U32 *scannedLength,
                          DCM_ELEMENT *e, PRV_ELEMENT_ITEM **elementItem) {
    ...
    *remainOpenFlag = TRUE;
    ...
    if (fileFlag) {
      if (fd != -1) {
        if ((*elementItem)->element.length != DCM_UNSPECIFIEDLENGTH)
          ...
          s1 = *size;
          scanCompressedPixels("", ptr, fd, &s1, /* size */
                               &f1,              /* fileOffset */
                               0, 0, byteOrder, explicitVR, acceptVRMismatch,
                               fileFlag, remainOpenFlag, convertFlag, object,
                               &l1, /* scannedLength */
                               e, elementItem);
        ...
        (*object)->sk((*object)->userCtx, (*elementItem)->element.length,
                      SEEK_CUR);
      }
      (*object)->fd = fd;
    }
    ...
    if (fileFlag) {
      if (fd != -1) {
        ...
        cond = (*object)->rd((*object)->userCtx, (*elementItem)->element.d.ot,
                             (long)(*elementItem)->element.length, &nBytes);
      }
      if (nBytes != (int)(*elementItem)->element.length) {
        (void)DCM_CloseObject((DCM_OBJECT **)object);
...
static CONDITION checkAttributeOrder(DCM_ELEMENT *e, long *lastGroup,
                                     long *lastElement,
  ...
  if ((long)group == *lastGroup) {
    ...
    if ((long)element <= *lastElement)
      return COND_PushCondition(DCM_ELEMENTOUTOFORDER,
                                DCM_Message(DCM_ELEMENTOUTOFORDER), group,
                                element, "checkAttributeOrder");
...
static CONDITION handleGroupItem(PRIVATE_OBJECT **obj,
                                 PRV_GROUP_ITEM **groupItem,
                                 unsigned short group) {
  ...
  if (createGroupFlag == TRUE) {
    *groupItem = (PRV_GROUP_ITEM *)CTN_MALLOC(sizeof(**groupItem));
    if (*groupItem == NULL) {
      ...
      return COND_PushCondition(
          DCM_ELEMENTCREATEFAILED, DCM_Message(DCM_ELEMENTCREATEFAILED),
          "handleGroupItem", group, 0xffff, sizeof(**groupItem));
    }
    (*groupItem)->group = group;
    (*groupItem)->baseLength = 0;
    (*groupItem)->longVRAttributes = 0;
...
readFile1(const char *name, unsigned char *callerBuf, int fd, U32 size,
  ...
  DCM_ELEMENT e;
  CTNBOOLEAN convertFlag = FALSE, done = FALSE, knownLength = TRUE,
             explicitVR = FALSE, acceptVRMismatch = FALSE, part10Flag = FALSE;
  ...
  if ((fileFlag) && ((opt & DCM_DELETEMASK) == DCM_DELETEONCLOSE) &&
      (recursionLevel == 0))
    (*object)->deleteFlag = TRUE;
  ...
  if (recursionLevel == 0 && part10Flag) {
    flag = readPreamble(name, &ptr, fd, &size, fileOffset, knownLength, object,
                        scannedLength);
    if (flag != DCM_NORMAL)
      goto abort;
  }
  while (!done) {
...
#endif
    flag =
        readVRLength(name, &ptr, fd, &size, fileOffset, knownLength, byteOrder,
                     explicitVR, acceptVRMismatch, object, scannedLength, &e);
    if (flag != DCM_NORMAL)
      goto abort;
    ...
    if (e.representation == DCM_SQ) {
      ...
      flag = readSequence(
          name, &ptr, fd, &sequenceLength, fileOffset, recursionLevel, opt,
          byteOrder, explicitVR, acceptVRMismatch, fileFlag, remainOpenFlag,
          convertFlag, object, &scannedSequenceLength, &e, &elementItem);
      if (flag != DCM_NORMAL)
        goto abort;
      if (size != (long)DCM_UNSPECIFIEDLENGTH)
      ...
      flag = readData(name, &ptr, fd, &size, fileOffset, knownLength, byteOrder,
                      explicitVR, acceptVRMismatch, fileFlag, remainOpenFlag,
                      convertFlag, object, scannedLength, &e, &elementItem);
      if (flag != DCM_NORMAL)
        goto abort;
    }
    computeVM(object, &elementItem->element);
    ...
    cond =
        checkAttributeOrder(&e, &lastGroup, &lastElement, allowRepeatElements);
    if (cond != DCM_NORMAL) {
...
static CONDITION copyData(PRIVATE_OBJECT **object, PRV_ELEMENT_ITEM *from,
                          DCM_ELEMENT *to, U32 *rtnLength) {
  ...
  if (from->element.d.ot == NULL) {
    if ((*object)->fd != -1) {
      ...
      (*object)->sk((*object)->userCtx, (long)(from->dataOffset + (long)p),
                    SEEK_SET);
    ...
    if (nBytes != (int)l) {
      return COND_PushCondition(DCM_FILEACCESSERROR,
                                DCM_Message(DCM_FILEACCESSERROR),
                                (*object)->fileName, "copyData (DCM internal)");
    }
#ifdef LITTLE_ENDIAN_ARCHITECTURE
...
DCM_GetCompressedValue(DCM_OBJECT **callerObject, DCM_TAG tag, void *buf,
  ...
  *elementItem;
  ...
  if (elementItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetEncodedValue");
  ...
  while (elementLength != 0) {
    sequenceLength = 0;
    memset(&e, 0, sizeof(e));
    flag = readGroupElement("", &ptr, (*object)->fd, &size, &fileOffset, FALSE,
                            byteOrder, explicitVR, acceptVRMismatch, object,
                            &sequenceLength, &e);
    if (flag == DCM_STREAMCOMPLETE)
      break;
    ...
    flag = readVRLength(
        "", &ptr, (*object)->fd, &size, &fileOffset, FALSE, /* Known length */
        byteOrder, explicitVR, acceptVRMismatch, object, &sequenceLength, &e);
    if (flag != DCM_NORMAL)
      return flag;
    ...
    if (firstBuffer) {
      ...
      while (l != 0) {
        ...
        callback(buf, toRead, index, (index != lastIndex) ? 1 : 0, 0,
                 startOfFragment, ctx);
        l -= toRead;
...
DCM_PrintSequenceList(DCM_OBJECT **object, DCM_TAG tag) {
  ...
  if (elementItem == NULL)
    return COND_PushCondition(
        DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND),
        DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_PrintSequenceList");
...
DCM_CopyObject(DCM_OBJECT **src, DCM_OBJECT **dst) {
  ...
  if (dstObj == NULL) {
    (void)COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE),
                             sizeof(PRIVATE_OBJECT), "DCM_CopyObject");
    *dst = NULL;
    return COND_PushCondition(DCM_OBJECTCREATEFAILED,
                              DCM_Message(DCM_OBJECTCREATEFAILED),
                              "DCM_CopyObject");
...
DCM_MergeObject(DCM_OBJECT **src, DCM_OBJECT **dst) {
  ...
  if (dstObj == NULL) {
    (void)COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE),
                             sizeof(PRIVATE_OBJECT), "DCM_MergeObject");
    *dst = NULL;
    return COND_PushCondition(DCM_OBJECTCREATEFAILED,
                              DCM_Message(DCM_OBJECTCREATEFAILED),
                              "DCM_MergeObject");
...
DCM_AddFragment(DCM_OBJECT **callerObject, void *fragment, U32 fragmentLength) {
  ...
  if (fragmentItem == NULL) {
    return COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE),
                              mallocLength, "DCM_AddFragment");
  }

  fragmentItem->fragment =
      (unsigned char *)fragmentItem + (long)sizeof(DCM_FRAGMENT_ITEM);
