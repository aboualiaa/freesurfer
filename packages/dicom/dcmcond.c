...
static VECTOR messageVector[] = {
    ...
    {DCM_ILLEGALSTREAMLENGTH, "DCM Illegal Stream Length (%ld) (Not enough "
                              "data to define a full element) in %s"},
    {DCM_ELEMENTCREATEFAILED,
     "DCM failed to create element in %s (%04x %04x %d)"},
    ...
    {DCM_ILLEGALREPRESENTATION, "DCM Caller specified illegal representation "
                                "for element (%04x %04x) in %s"},
    ...
    {DCM_INSERTFAILED, "DCM failed to insert new element (%04x %04x) in %s"},
    {DCM_CANNOTGETSEQUENCEVALUE, "DCM Cannot retrieve value of element with SQ "
                                 "representation (%08x) in (%s)"},
    ...
    {DCM_BADLENGTH, "DCM Combination of offset, length (%d %d) is longer than "
                    "element length (%d) in %s"},
    {DCM_NOTASEQUENCE, "DCM Attempt to perform sequence operation on element "
                       "(%04x %04x) not a sequence in %s"},
...
const char *DCM_Message(CONDITION condition) {
  int index;
  ...
  for (index = 0; index < (int)DIM_OF(messageVector); index++) {
    if (messageVector[index].message != NULL)
      printf("%8x %8ld %s\n", (unsigned int)messageVector[index].cond,
             messageVector[index].cond, messageVector[index].message);
  }
}
