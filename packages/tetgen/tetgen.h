...
#include <cstdio>  // Standard IO: FILE, NULL, EOF, printf(), ...
#include <cstdlib> // Standard lib: abort(), system(), getenv(), ...
#include <cstring> // String lib: strcpy(), strcat(), strcmp(), ...
#include <cmath>   // Math lib: sin(), sqrt(), pow(), ...
#include <cassert>
...
public:
  ...
  // // and points,  where subfaces and subsegments are triangles and edges
  // which // appear on boundaries.  A tetrahedralization of a 3D point set
  // comprises   // tetrahedra and points;  a surface mesh of a 3D domain
  // comprises subfaces  // subsegments and points.  The elements of all the
  // four types consist of a  // tetrahedral mesh of a 3D domain.  However,
  // TetGen uses three data types:  // 'tetrahedron', 'shellface', and 'point'.
  // A 'tetrahedron' is a tetrahedron;// while a 'shellface' can be either a
  // subface or a subsegment; and a 'point'// is a point.  These three data
  // types, linked by pointers comprise a mesh.  //
  //                                                                           //
  ...
  // // its adjoining subfaces (when subfaces are needed). Optinoally,
  // (depending // on the selected switches), it may contain an arbitrary number
  // of user-    // defined floating-point attributes,  an optional maximum
  // volume constraint // (for -a switch), and a pointer to a list of high-order
  // nodes (-o2 switch).// Since the size of a tetrahedron is not determined
  // until running time, it  // is not simply declared as a structure. //
  //                                                                           //
  // The data structure of tetrahedron also stores the geometrical
  // information.// Let t be a tetrahedron, v0, v1, v2, and v3 be the 4 nodes
  // corresponding   // to the order of their storage in t.  v3 always has a
  // negative orientation // with respect to v0, v1, v2 (ie,, v3 lies above the
  // oriented plane passes  // through v0, v1, v2). Let the 4 faces of t be f0,
  // f1, f2, and f3. Vertices // of each face are stipulated as follows: f0 (v0,
  // v1, v2), f1 (v0, v3, v1), // f2 (v1, v3, v2), f3 (v2, v3, v0). //
  //                                                                           //
  ...
  // // boundary marker(an integer). Like a tetrahedron, the pointers to
  // vertices,// subfaces, and subsegments are ordered in a way that indicates
  // their geom- // etric relation.  Let s be a subface, v0, v1 and v2 be the 3
  // nodes corres- // ponding to the order of their storage in s,  e0, e1 and e2
  // be the 3 edges,// then we have: e0 (v0, v1), e1 (v1, v2), e2 (v2, v0). //
  //                                                                           //
  ...
  // // subfaces are never represented by the NULL pointers; they are
  // represented // by two special records: `dummytet', the tetrahedron fills
  // "outer space",  // and `dummysh',  the vacuous subfaces which are
  // omnipresent.               //
  //                                                                           //
  // Tetrahedra and adjoining subfaces are glued together through the pointers
  // // saved in each data fields of them. Subfaces and adjoining subsegments
  // are // connected in the same fashion.  However, there are no pointers
  // directly   // gluing tetrahedra and adjoining subsegments.  For the purpose
  // of saving   // space, the connections between tetrahedra and subsegments
  // are entirely    // mediated through subfaces.  The following part explains
  // how subfaces are  // connected in TetGen. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  // // containing s as an edge.  The direction of s is stipulated from its
  // first // endpoint to its second (according to their storage in s). Once the
  // dir of // s is determined, the other two edges of f are oriented to follow
  // this dir.// The "directional normal" N_f is a vector formed from any point
  // in f and a // points orthogonally above f. //
  //                                                                           //
  // The face ring of s is a cyclic ordered set of subfaces containing s,
  // i.e.,// F(s) = {f1, f2, ..., fn}, n >= 1.  Where the order is defined as
  // follows: // let fi, fj be two faces in F(s), the "normal-angle",
  // NAngle(i,j) (range   // from 0 to 360 degree) is the angle between the N_fi
  // and N_fj;  then fi is // in front of fj (or symbolically, fi < fj) if there
  // exists another fk in   // F(s), and NAangle(k, i) < NAngle(k, j).  The face
  // ring of s is: f1 < f2 < //
  // ... < fn < f1. //
  //                                                                           //
  ...
  // // the direction of the subsegment. The face ring is connected following
  ...
  // // segment. However, a subsegment only has one pointer to a subface which
  // is // containing it. Such subface can be chosen arbitrarily, other subfaces
  // are // found through the face ring. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  // // to hold one particular part of the mesh, i.e., a tetrahedron, a
  // triangle, // an edge and a vertex.  However, these data types do not
  // themselves store  // any part of the mesh. The mesh is made of the data
  // types defined above.   //
  //                                                                           //
  ...
  // // (range from 0 to 5) as the edge version. A face of the tetrahedron can
  // be // uniquly determined by the pair (tet, loc), and an oriented edge of
  // this   // face can be uniquly determined by the triple (tet, loc, ver).
  // Therefore, // different usages of one triface are possible.  If we only use
  // the pair    // (tet, loc), it refers to a face, and if we add the 'ver'
  // additionally to  // the pair, it is an oriented edge of this face. //
  //                                                                           //
  ...
  // // one of two orientations of this subsegment, hence, it only can be 0
  // or 1. //
  //                                                                           //
  ...
  // // or 'ss' is still the same when it was stored; 'noppo' is the fifth
  // vertex // of a degenerate point set.  'previtem' and 'nextitem' implement a
  // double  // link for managing many basfaces. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  // // the segment ids of s1 and s2; 'ss' contains two segments belong to s1
  // and // s2, respectively. 'transmat' are two transformation matrices.
  // transmat[0] // transforms a point of f1 (or s1) into a point of f2 (or s2),
  // transmat[1] // does the inverse. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  //   (1) d(p, q) = d(q, p). (d is symmetric) // (2) d(p, q) = 0 if and only if
  //   p = q.                                   // (3) d(p, x) + d(x, q) >= d(p,
  //   q). (d satisfies triangle inequality)     //
  //                                                                           //
  ...
  // // piece of continuous memory. It allows quickly accessing each element of
  // S,// thus is suitable for storing a fix-sized set.  While a link stores its
  // // elements incontinuously. It allows quickly inserting or deleting an
  // item, // thus is suitable for storing a size-variable set.  A queue is
  // basically a // special case of a link where one data element joins the link
  // at the end   // and leaves in an ordered fashion at the other end. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  //                 +1  if x > y.
  using compfunc = int (*)(const void *, const void *);
  ...
  //         |__NULL___|<-- |__prev___|<-- |__prev___|<-- |__prev___| // | | |_
  //         _|    |_       _|    |         |          // |         |    |_
  //         Data1 _|    |_ Data2 _|    |         |          //
  ...
  // // For example, we have ordered the 4 vertices (from 0 to 3) and the 4
  // faces // (from 0 to 3) of a tetrahedron,  and for each face of the
  // tetrahedron, a  // sequence of vertices has stipulated,  therefore the
  // origin of any face of // the tetrahedron can be quickly determined by a
  // table 'locver2org', which  // takes the index of the face and the edge
  // version as inputs.  A list of    // fast lookup tables are defined below.
  // They're just like global variables. // These tables are initialized at the
  // runtime.                              //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  // // local modification, etc.,  is accomplished through a set of mesh
  // manipul- // ation primitives. These primitives are indeed very simple
  // functions which // take one or two handles ('triface's and 'face's) as
  // parameters,  perform  // basic operations such as "glue two tetrahedra at a
  // face",  "return the    // origin of a tetrahedron", "return the subface
  // adjoining at the face of a  // tetrahedron", and so on. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  // // are all public available. However both of them failed frequently.
  // Another // unconvenience is both codes only tell whether or not the two
  // triangles    // intersect without distinguishing the cases whether they
  // exactly intersect // in interior or they just share a vertex or share an
  // edge. The two latter  // cases are acceptable and should return not
  // intersection in TetGen.        //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  enum interresult tri_tri_inter(REAL *, REAL *, REAL *, REAL *, REAL *,
                                 REAL *);
  ...
  bool iscospheric(REAL *, REAL *, REAL *, REAL *, REAL *, REAL vol24,
                   REAL eps);
  ...
  void inscribedsphere(REAL *, REAL *, REAL *, REAL *, REAL *cent,
                       REAL *radius);
  ...
  // // used to transform (or replace) a set of mesh elements into another set
  // of // mesh elements. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  void undosite(enum insertsiteresult insresult, triface *splittet, point torg,
  ...
  void interecursive(shellface **subfacearray, int arraysize, int axis,
                     REAL bxmin, REAL bxmax, REAL bymin, REAL bymax, REAL bzmin,
                     REAL bzmax, int *internum);
  ...
  void formcavity(list *missingshlist, list *crossedgelist, list *equatptlist,
