 ...
 * by Bevin Brett CVS Revision Info: $Author: Brett $ $Date: 2017/11/17 12:46:00
 * $ $Revision: 1.00 $
 *
...
static Status get_dimension_ordering(int n_vol_dims, STRING vol_dim_names[],
                                     int n_file_dims, STRING file_dim_names[],
                                     int to_volume[], int to_file[]);
...
static BOOLEAN is_default_direction_cosine(int axis, double dir_cosines[]) {
  BOOLEAN is_default;
  int i;
...
static Status
output_world_transform(Minc_file file, STRING space_type,
    ...
    for_less(dim, 0, file->n_file_dimensions) {
      if (convert_dim_name_to_spatial_axis(file->dim_names[dim], &axis)) {
        if (file->to_volume_index[dim] == INVALID_AXIS)
          ...
          start[dim] = file->volume->starts[file->to_volume_index[dim]];
          step[dim] = file->volume->separations[file->to_volume_index[dim]];
          dir_cosines[dim][X] =
              file->volume->direction_cosines[file->to_volume_index[dim]][X];
          dir_cosines[dim][Y] =
              file->volume->direction_cosines[file->to_volume_index[dim]][Y];
          dir_cosines[dim][Z] =
              file->volume->direction_cosines[file->to_volume_index[dim]][Z];
        }
      }
    }
  } else /*--- convert the linear transform to starts/steps/dir cosines */
  {
    if (voxel_to_world_transform == NULL ||
      ...
      convert_transform_to_starts_and_steps(
          voxel_to_world_transform, file->n_file_dimensions, NULL, spatial_axes,
          start, step, dir_cosines);
    }
  }

  for_less(dim, 0, file->n_file_dimensions) {
    if (convert_dim_name_to_spatial_axis(file->dim_names[dim], &axis)) {
      file->dim_ids[dim] = micreate_std_variable(
          file->cdfid, file->dim_names[dim], NC_DOUBLE, 0, NULL);

      if (file->dim_ids[dim] < 0)
        return (ERROR);

      (void)miattputdbl(file->cdfid, file->dim_ids[dim], MIstep, step[dim]);
      (void)miattputdbl(file->cdfid, file->dim_ids[dim], MIstart, start[dim]);
      if (!is_default_direction_cosine(axis, dir_cosines[dim])) {
        (void)ncattput(file->cdfid, file->dim_ids[dim], MIdirection_cosines,
                       NC_DOUBLE, N_DIMENSIONS, dir_cosines[dim]);
      }
      (void)miattputstr(file->cdfid, file->dim_ids[dim], MIunits, UNITS);
      if (!equal_strings(space_type, MI_UNKNOWN_SPACE)) {
        (void)miattputstr(file->cdfid, file->dim_ids[dim], MIspacetype,
                          space_type);
      }
    } else
      file->dim_ids[dim] = -1;
...
static void create_image_variable(Minc_file file) {
  ...
  /* Create the variable */
  file->img_var_id =
      micreate_std_variable(file->cdfid, MIimage, file->nc_data_type,
                            file->n_file_dimensions, file->image_dims);

  /* Copy all attributes if required */
  if (file->src_img_var != MI_ERROR) {
    ncopts = 0;
    (void)micopy_all_atts(file->src_cdfid, file->src_img_var, file->cdfid,
                          file->img_var_id);
  ...
  if (file->signed_flag)
    (void)miattputstr(file->cdfid, file->img_var_id, MIsigntype, MI_SIGNED);
  else
    (void)miattputstr(file->cdfid, file->img_var_id, MIsigntype, MI_UNSIGNED);

  /* --- put the valid voxel range */

  if (file->valid_range[0] < file->valid_range[1]) {

    (void)miset_valid_range(file->cdfid, file->img_var_id, file->valid_range);
  }
}
...
VIOAPI void set_default_minc_output_options(minc_output_options *options) {
  int dim;

  for_less(dim, 0, MAX_DIMENSIONS) options->dimension_names[dim] = NULL;
...
VIOAPI Minc_file initialize_minc_output(
    STRING filename, int n_dimensions, STRING dim_names[], int sizes[],
    nc_type file_nc_data_type, BOOLEAN file_signed_flag, Real file_voxel_min,
    Real file_voxel_max, General_transform *voxel_to_world_transform,
    Volume volume_to_attach, minc_output_options *options) {
  ...
  if (dim_names == NULL) {
    if (n_dimensions != 3) {
      ...
      print_error("can't use NULL dim_names except with 3 dimensions.\n");
  ...
  if (file_nc_data_type == MI_ORIGINAL_TYPE) {
    file_nc_data_type =
        get_volume_nc_data_type(volume_to_attach, &file_signed_flag);
    get_volume_voxel_range(volume_to_attach, &file_voxel_min, &file_voxel_max);
  } else if ((file_nc_data_type == NC_FLOAT ||
              file_nc_data_type == NC_DOUBLE) &&
             file_voxel_min >= file_voxel_max) {
    get_volume_real_range(volume_to_attach, &file_voxel_min, &file_voxel_max);
  }
  ...
  if (get_dimension_ordering(n_volume_dims, (const char **)vol_dimension_names,
                             n_dimensions, dim_names, file->to_volume_index,
                             file->to_file_index) != OK) {
    free(file);
    return ((Minc_file)NULL);
  }
  ...
  for_less(d, 0, n_dimensions) {
    ...
    if (vol_index >= 0 && volume_sizes[vol_index] != sizes[d]) {
      ...
      print_error("volume size[%d]=%d does not match file[%d]=%d.\n", vol_index,
                  volume_sizes[vol_index], d, sizes[d]);
      return (NULL);
    }
  }
  ...
  (void)micreate_std_variable(file->cdfid, MIrootvariable, NC_INT, 0, NULL);
  ...
  if (file->image_range[0] < file->image_range[1]) {
    ...
    file->min_id = micreate_std_variable(file->cdfid, MIimagemin, NC_DOUBLE,
                                         n_range_dims, file->image_dims);
    file->max_id = micreate_std_variable(file->cdfid, MIimagemax, NC_DOUBLE,
                                         n_range_dims, file->image_dims);
  }
...
VIO_Status add_minc_history(Minc_file file, STRING history_string) {
...
static Status copy_auxiliary_data_from_open_minc_file(Minc_file file,
                                                      int src_cdfid,
...
#define excluded_list_size 9
  STRING excluded_list[excluded_list_size] = {
      ...
      MIyfrequency, MIzfrequency, MIvector_dimension};

  if (file->ignoring_because_cached)
    return (OK);

  if (file->end_def_done) {
    print_error("Cannot call copy_auxiliary_data_from_open_minc_file when not "
                "in define mode\n");
    return (ERROR);
  }
...
#undef excluded_list_size
  ...
  (void)micopy_all_var_defs(src_cdfid, file->cdfid, n_excluded, excluded_vars);

  if (src_min_id != MI_ERROR) {
    (void)micopy_all_atts(src_cdfid, src_min_id, file->cdfid, file->min_id);
  }

  if (src_max_id != MI_ERROR) {
    (void)micopy_all_atts(src_cdfid, src_max_id, file->cdfid, file->max_id);
  }

  if (src_root_id != MI_ERROR) {
    (void)micopy_all_atts(src_cdfid, src_root_id, file->cdfid,
  ...
  if (status == OK) {
    ...
    (void)micopy_all_var_values(src_cdfid, file->cdfid, n_excluded,
                                excluded_vars);
  }
...
VIOAPI Status copy_auxiliary_data_from_minc_file(Minc_file file,
                                                 STRING filename,
                                                 STRING history_string) {
  Status status;
  int src_cdfid;
...
static Status get_dimension_ordering(int n_vol_dims, STRING vol_dim_names[],
  ...
  for_less(f, 0, n_file_dims) to_volume[f] = -1;

  for_less(v, 0, n_vol_dims) to_file[v] = -1;
...
static Status check_minc_output_variables(Minc_file file) {
  int d, axis;
  ...
  if (!file->variables_written) {
    ...
    ncopts = NC_VERBOSE;
    for_less(d, 0, file->n_file_dimensions) mindex[d] = 0;

    dim_value = 0.0;
    for_less(d, 0, file->n_file_dimensions) {
      if (convert_dim_name_to_spatial_axis(file->dim_names[d], &axis)) {
        (void)mivarput1(file->cdfid, file->dim_ids[d], mindex, NC_DOUBLE,
                        MI_SIGNED, &dim_value);
      }
    }
    ...
    (void)miicv_setint(file->minc_icv, MI_ICV_TYPE, (int)volume->nc_data_type);
    ...
    if (file->image_range[0] < file->image_range[1]) {
      (void)mivarput1(file->cdfid, file->min_id, &start_index, NC_DOUBLE,
                      MI_SIGNED, &file->image_range[0]);
      (void)mivarput1(file->cdfid, file->max_id, &start_index, NC_DOUBLE,
                      MI_SIGNED, &file->image_range[1]);
    }
    ncopts = NC_VERBOSE | NC_FATAL;
  }
...
VIOAPI Status set_minc_output_random_order(Minc_file file) {
  Status status;
...
VIOAPI Status output_minc_hyperslab(Minc_file file, Data_types data_type,
                                    int n_array_dims, int array_sizes[],
                                    void *array_data_ptr, int to_array[],
                                    int file_start[], int file_count[]) {
  ...
  if (direct_from_array) /* hyperslab is consecutive chunk of memory */
  {
    ...
    for_less(dim, 0, n_tmp_dims) tmp_sizes[dim] = tmp_sizes[dim + tmp_ind + 1];

    for_less(dim, 0, n_array_dims) array_indices[dim] -= tmp_ind + 1;
    ...
    copy_multidim_data_reordered(get_type_size(data_type), void_ptr, n_tmp_dims,
                                 tmp_sizes, array_data_ptr, n_array_dims,
                                 array_sizes, array_counts, array_indices,
                                 TRUE);
  ...
  if (miicv_put(file->minc_icv, long_file_start, long_file_count, void_ptr) ==
      MI_ERROR)
    status = ERROR;
  else
    status = OK;
...
static void output_slab(Minc_file file, Volume volume, int to_volume[],
                        long file_start[], long file_count[]) {
  int file_ind, ind;
  ...
  if (volume->is_cached_volume) {
    ...
    for_less(dim, 0, get_volume_n_dimensions(volume)) volume_sizes[dim] = 1;

    for_less(dim, 0, MAX_DIMENSIONS) array_to_volume[dim] = 0;
    ...
    for_less(v[0], 0, size0) for_less(v[1], 0, size1) for_less(v[2], 0, size2)
        for_less(v[3], 0, size3) for_less(v[4], 0, size4) {
      value = get_volume_voxel_value(
          volume, volume_start[0] + v[0], volume_start[1] + v[1],
          volume_start[2] + v[2], volume_start[3] + v[3],
          volume_start[4] + v[4]);

      SET_MULTIDIM(array, v[array_to_volume[0]], v[array_to_volume[1]],
                   v[array_to_volume[2]], v[array_to_volume[3]],
    ...
    (void)output_minc_hyperslab(file, get_volume_data_type(volume), n_slab_dims,
                                slab_sizes, array_data_ptr, to_array,
                                int_file_start, int_file_count);
    delete_multidim_array(&array);
  } else
#endif
  {
    GET_MULTIDIM_PTR(array_data_ptr, volume->array, volume_start[0],
                     volume_start[1], volume_start[2], volume_start[3],
                     volume_start[4]);
    get_volume_sizes(volume, volume_sizes);

    (void)output_minc_hyperslab(file, get_volume_data_type(volume),
                                get_volume_n_dimensions(volume), volume_sizes,
                                array_data_ptr, to_volume, int_file_start,
                                int_file_count);
  }
}
...
static Status output_the_volume(Minc_file file, Volume volume,
                                int volume_count[], long file_start[]) {
  Status status;
  ...
  status = get_dimension_ordering(
      n_volume_dims, (const char **)vol_dimension_names,
      file->n_file_dimensions, file->dim_names, to_volume_index, to_file_index);
  ...
  if (file->image_range[0] >= file->image_range[1]) {
    ...
    for_less(r, 0, n_ranges) image_range[r] = real_min;

    (void)mivarput(file->cdfid, file->min_id, range_start, range_count,
                   NC_DOUBLE, MI_UNSIGNED, (void *)image_range);

    for_less(r, 0, n_ranges) image_range[r] = real_max;

    (void)mivarput(file->cdfid, file->max_id, range_start, range_count,
                   NC_DOUBLE, MI_UNSIGNED, (void *)image_range);
...
VIOAPI Status output_volume_to_minc_file_position(Minc_file file, Volume volume,
                                                  int volume_count[],
                                                  long file_start[]) {
  if (file->ignoring_because_cached)
    return (OK);
...
VIOAPI Status output_minc_volume(Minc_file file) {
  ...
  if (output_the_volume(file, file->volume, volume_count, file->indices) != OK)
    return (ERROR);
...
VIOAPI Status close_minc_output(Minc_file file) {
  int d;
  ...
  if (!file->ignoring_because_cached) {
    ...
    for_less(d, 0, file->n_file_dimensions) free((char *)file->dim_names[d]);
  }
...
VIOAPI void copy_minc_output_options(minc_output_options *src,
    ...
    *dest = *src;
    ...
    for_less(dim, 0, MAX_DIMENSIONS) {
      if (src->dimension_names[dim] != NULL)
        dest->dimension_names[dim] = strdup(src->dimension_names[dim]);
    }
  }
}
...
VIOAPI void delete_minc_output_options(minc_output_options *options) {
  int i;

  for_less(i, 0, MAX_DIMENSIONS) free((char *)options->dimension_names[i]);
}
...
VIOAPI void set_minc_output_dimensions_order(minc_output_options *options,
                                             int n_dimensions,
...
VIOAPI void set_minc_output_real_range(minc_output_options *options,
                                       Real real_min, Real real_max) {
...
VIOAPI void
set_minc_output_use_volume_starts_and_steps_flag(minc_output_options *options,
                                                 BOOLEAN flag) {
