...
static Status input_slice(volume_input_struct *volume_input);
...
VIO_Status initialize_free_format_input(const char *filename, Volume volume,
  ...
  file_status = open_file_with_default_suffix(filename, DEFAULT_SUFFIX,
  ...
  if (status == OK &&
      (input_real(file, &trans[X]) != OK || input_real(file, &trans[Y]) != OK ||
  ...
  */
  ...
  if (volume->spatial_axes[X] < 0 || volume->spatial_axes[Y] < 0 ||
  ...
  if (status == OK)
    for_less(axis, 0, N_DIMENSIONS) {
      ...
      case 'x':
      case 'X':
        volume_input->axis_index_from_file[axis] = volume->spatial_axes[X];
        break;

      case 'y':
      case 'Y':
        volume_input->axis_index_from_file[axis] = volume->spatial_axes[Y];
        break;

      case 'z':
      case 'Z':
        volume_input->axis_index_from_file[axis] = volume->spatial_axes[Z];
        break;
  ...
  if (status == OK && (volume_input->axis_index_from_file[X] ==
  ...
  if (status == OK) {
    if (volume_input->sizes_in_file[0] <= 0) {
      ...
      while (input_string(file, &slice_filename, (char)' ') == OK) {
        volume_input->slice_filenames =
            (const char **)realloc(volume_input->slice_filenames,
                                   (n_slices + 1) * sizeof(const char *));
        volume_input->slice_filenames[n_slices] = slice_filename;

        volume_input->slice_byte_offsets = (int *)realloc(
            volume_input->slice_byte_offsets, (n_slices + 1) * sizeof(int));
  ...
  if (status == OK) {
    for_less(axis, 0, N_DIMENSIONS) {
      ...
      if (volume_separations[axis] < 0.0) {
        trans[axis] += -volume_separations[axis] * (Real)(sizes[axis] - 1);
      }
    }
  ...
  n_voxels_in_slice =
      (int)volume_input->sizes_in_file[1] * (int)volume_input->sizes_in_file[2];

  if (status == OK)
    switch (volume_input->file_data_type) {
    case UNSIGNED_BYTE:
      volume_input->byte_slice_buffer =
          (unsigned char *)malloc(sizeof(unsigned char) * n_voxels_in_slice);
      break;

    case UNSIGNED_SHORT:
      volume_input->short_slice_buffer =
          (unsigned short *)malloc(sizeof(unsigned short) * n_voxels_in_slice);
      break;
...
VIOAPI void delete_free_format_input(volume_input_struct *volume_input) {
  long slice;
...
static Status input_slice(volume_input_struct *volume_input) {
  ...
  if ((long)volume_input->slice_index < volume_input->sizes_in_file[0]) {
    if (volume_input->one_file_per_slice) {
      ...
      status = open_file(slice_filename, READ_FILE, BINARY_FORMAT, &file);

      free((void *)slice_filename);

      if (status == OK)
        status = set_file_position(
            file, (long)(volume_input
                             ->slice_byte_offsets[volume_input->slice_index]));
    } else
...
VIOAPI BOOLEAN input_more_free_format_file(Volume volume,
                                           volume_input_struct *volume_input,
                                           Real *fraction_done) {
      ...
      case UNSIGNED_BYTE:
        ...
        for_less(i, 0, volume_input->sizes_in_file[1]) {
          ...
          for_less(*inner_index, 0, (int)volume_input->sizes_in_file[2]) {
            if (scaling_flag) {
              value =
                  ((Real)(*byte_buffer_ptr) - value_translation) / value_scale;
            ...
            set_volume_voxel_value(volume, indices[X], indices[Y], indices[Z],
                                   0, 0, value);
      ...
      case UNSIGNED_SHORT:
        ...
        for_less(i, 0, volume_input->sizes_in_file[1]) {
          ...
          for_less(*inner_index, 0, (int)volume_input->sizes_in_file[2]) {
            if (scaling_flag) {
              value =
                  ((Real)(*short_buffer_ptr) - value_translation) / value_scale;
            } else
