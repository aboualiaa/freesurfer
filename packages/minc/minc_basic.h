...
@RCSID      : $Header: /private-cvsroot/minc/libsrc/minc_basic.h,v 6.2
2001/04/17 18:40:13 neelin Exp $ MINC (MNI)
---------------------------------------------------------------------------- */

#include <math.h>

static long longMIN(long lhs, long rhs) { return (lhs < rhs) ? lhs : rhs; }

static long longMAX(long lhs, long rhs) { return (lhs > rhs) ? lhs : rhs; }
...
#define MI_TO_DOUBLE(dvalue, type, sign, ptr)                                  \
  ...
  case NC_BYTE:                                                                \
    ...
    case MI_PRIV_UNSIGNED:                                                     \
      dvalue = (double)*((unsigned char *)ptr);                                \
      break;                                                                   \
    case MI_PRIV_SIGNED:                                                       \
      dvalue = (double)*((signed char *)ptr);                                  \
      break;                                                                   \
    }                                                                          \
    break;                                                                     \
  case NC_SHORT:                                                               \
    switch (sign) {                                                            \
    case MI_PRIV_UNSIGNED:                                                     \
      dvalue = (double)*((unsigned short *)ptr);                               \
      break;                                                                   \
    case MI_PRIV_SIGNED:                                                       \
      dvalue = (double)*((signed short *)ptr);                                 \
      break;                                                                   \
    }                                                                          \
    break;                                                                     \
  case NC_INT:                                                                 \
    switch (sign) {                                                            \
    case MI_PRIV_UNSIGNED:                                                     \
      dvalue = (double)*((unsigned int *)ptr);                                 \
      break;                                                                   \
    case MI_PRIV_SIGNED:                                                       \
      dvalue = (double)*((signed int *)ptr);                                   \
      break;                                                                   \
    }                                                                          \
    break;                                                                     \
  case NC_FLOAT:                                                               \
