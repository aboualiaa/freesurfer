...
// /private-cvsroot/minc/libsrc/image_conversion.c,v 6.10.2.1 2004/09/28 20:23:40
// bert Exp $ MINC (MNI)";
#endif
...
PRIVATE int MI_icv_coords_tovar(mi_icv_type *icvp, long icv_start[],
                                long icv_count[], long var_start[],
                                long var_count[]);
...
MNCAPI int miicv_create() {
  ...
  icvp->user_vmax =
      MI_get_default_range(MIvalid_max, icvp->user_type, icvp->user_sign);
  icvp->user_vmin =
      MI_get_default_range(MIvalid_min, icvp->user_type, icvp->user_sign);
...
MNCAPI int miicv_free(int icvid) {
  ...
  /* Check icv id */
  if ((icvp = MI_icv_chkid(icvid)) == NULL)
    MI_RETURN_ERROR(MI_ERROR);
...
MNCAPI int miicv_setdbl(int icvid, int icv_property, double value) {
  ...
  case MI_ICV_XDIM_DIR:
    ival = value;
    icvp->user_xdim_dir =
        ((ival == MI_ICV_POSITIVE) || (ival == MI_ICV_NEGATIVE))
            ? ival
            : MI_ICV_ANYDIR;
    break;
  ...
  case MI_ICV_NUM_IMGDIMS:
    ...
    if ((ival < 0) || (ival > MI_MAX_IMGDIMS)) {
      MI_LOG_PKG_ERROR2(MI_ERR_BADPROP, "MI_ICV_NUM_IMGDIMS out of range");
      MI_RETURN_ERROR(MI_ERROR);
    }
...
MNCAPI int miicv_inqint(int icvid, int icv_property, int *value) {
  ...
  {MI_CHK_ERR(miicv_inqdbl(icvid, icv_property, &dvalue))} *value = dvalue;
...
MNCAPI int miicv_ndattach(int icvid, int cdfid, int varid) {
  ...
  icvp->do_scale = (icvp->user_do_range &&
...
PRIVATE int MI_icv_get_type(mi_icv_type *icvp, int cdfid, int varid) {
  ...
  MI_CHK_ERR(ncvarinq(cdfid, varid, NULL, &(icvp->var_type), &(icvp->var_ndims),
                      icvp->var_dim, NULL))
...
/* ARGSUSED */
{
  ...
  icvp->derv_var_float =
      ((icvp->var_type == NC_DOUBLE) || (icvp->var_type == NC_FLOAT));
  icvp->derv_usr_float =
      ((icvp->user_type == NC_DOUBLE) || (icvp->user_type == NC_FLOAT));
  ...
  if (!icvp->user_do_norm) {
    ...
    if ((vid[0] != MI_ERROR) && (vid[1] != MI_ERROR)) {
      for (imm = 0; imm < 2; imm++) {
        MI_CHK_ERR(ncvarinq(cdfid, vid[imm], NULL, NULL, &ndims, dim, NULL))
...
PRIVATE int MI_icv_access(int operation, mi_icv_type *icvp, long start[],
  ...
  if (!icvp->do_dimconvert)
    ...
    chunk_size = 0;
  for (idim = longMAX(icvp->derv_firstdim + 1, 0); idim < icvp->var_ndims;
       idim++) {
  ...
  while (chunk_start[0] < var_end[0]) {
    ...
    /* Get the values */
    MI_CHK_ERR(MI_varaccess(operation, icvp->cdfid, icvp->varid, chunk_start,
                            chunk_count, icvp->user_type, icvp->user_sign,
                            chunk_values, bufsize_step, icvp))

    /* Increment the start counter */
    chunk_start[firstdim] += chunk_count[firstdim];
    for (idim = firstdim; (idim > 0) && (chunk_start[idim] >= var_end[idim]);
         idim--) {
      chunk_start[idim] = var_start[idim];
      chunk_start[idim - 1]++;
    }
...
PRIVATE int MI_icv_zero_buffer(mi_icv_type *icvp, long count[], void *values) {
  ...
  for (bufptr = (char *)values, zeroptr = (char *)zerostart; bufptr < bufend;
       bufptr++, zeroptr++) {
    if (zeroptr >= zeroend)
...
PRIVATE int MI_icv_coords_tovar(mi_icv_type *icvp, long icv_start[],
  ...
  for (i = num_non_img_dims, j = icvp->user_num_imgdims - 1;
    ...
    /* Check coordinates. */
    icv_dim_size = (icvp->user_dim_size[j] > 0) ? icvp->user_dim_size[j]
                                                : icvp->var_dim_size[j];
    last_coord = icv_start[i] + icv_count[i] - 1;
    if ((icv_start[i] < 0) || (icv_start[i] >= icv_dim_size) ||
        (last_coord < 0) || (last_coord >= icv_dim_size) ||
        (icv_count[i] < 0)) {
      MI_LOG_PKG_ERROR2(MI_ERR_ICV_INVCOORDS, "Invalid icv coordinates");
      MI_RETURN_ERROR(MI_ERROR);
    }
    /* Remove offset */
...
PRIVATE int MI_icv_calc_scale(int operation, mi_icv_type *icvp, long coords[]) {
  ...
  if (!icvp->derv_var_float && !icvp->derv_usr_float && !icvp->user_do_norm) {
    ...
    if ((!icvp->derv_var_float || !icvp->user_do_norm) &&
      ...
      if (mitranslate_coords(icvp->cdfid, icvp->varid, coords, icvp->imgmaxid,
                             mmcoords) == NULL)
        MI_RETURN_ERROR(MI_ERROR);
      {
        MI_CHK_ERR(mivarget1(icvp->cdfid, icvp->imgmaxid, mmcoords, NC_DOUBLE,
                             NULL, &slice_imgmax))
      }
      if (mitranslate_coords(icvp->cdfid, icvp->varid, coords, icvp->imgminid,
                             mmcoords) == NULL)
        MI_RETURN_ERROR(MI_ERROR);
      {
        MI_CHK_ERR(mivarget1(icvp->cdfid, icvp->imgminid, mmcoords, NC_DOUBLE,
                             NULL, &slice_imgmin))
      }
    }
  ...
  /* Offset */
  icvp->offset =
      usr_vmin - icvp->scale * var_vmin + usr_scale * (var_imgmin - usr_imgmin);
...
PRIVATE int MI_icv_dimconv_init(int operation, mi_icv_type *icvp,
                                mi_icv_dimconv_type *dcp, long start[],
                                long count[], void *values, long bufstart[],
                                long bufcount[], void *buffer)
/* ARGSUSED */
{
  ...
  for (idim = fastdim - 1; idim >= 0; idim--) {
    ...
    dcp->usr_step[idim] =
        dcp->usr_step[idim + 1] * icvp->derv_icv_count[idim + 1];
  }
    ...
    if ((idim < imgdim_low) || (idim > imgdim_high)) {
      ...
      if (icvp->derv_dim_grow[jdim]) {
        ...
        if (!icvp->derv_dim_flip[jdim])
          values_index =
              bufstart[idim] * scale - icvp->derv_icv_start[idim] + offset;
        else
          values_index = (icvp->var_dim_size[jdim] - bufstart[idim]) * scale -
                         1 - icvp->derv_icv_start[idim] + offset;
      } else {
        dcp->end[idim] =
            (bufcount[idim] - 1 + bufstart[idim] % scale) / scale + 1;
  ...
  if (dcp->do_compress || dcp->do_expand) {
    ...
    for (jdim = 0; jdim < icvp->user_num_imgdims; jdim++) {
      ...
      usr_dcount[idim] = 0;
      var_dend[idim] =
          (icvp->derv_dim_grow[jdim]
               ? 1
               : longMIN(icvp->var_dim_size[jdim], icvp->derv_dim_scale[jdim]));
      usr_dend[idim] =
          (icvp->derv_dim_grow[jdim] ? icvp->derv_dim_scale[jdim] : 1);
    }
...
@MODIFIED   :
---------------------------------------------------------------------------- */
PRIVATE int MI_icv_dimconvert(int operation, mi_icv_type *icvp, long start[],
                              long count[], void *values, long bufstart[],
                              long bufcount[], void *buffer) {
  ...
  while (counter[0] < end[0]) {
    ...
    if (!dcp->do_compress) {
      {MI_TO_DOUBLE(dvalue, dcp->intype, dcp->insign, iptr)} out_of_range =
          (icvp->do_fillvalue && ((dvalue < dmin) || (dvalue > dmax)));
    } else {
      sum1 = 0.0;
      sum0 = 0.0;
      out_of_range = FALSE;
...
/* ARGSUSED */
{
  ...
  for (idim = 0; idim < icvp->user_num_imgdims; idim++) {
    if (icvp->derv_dim_grow[idim])
      ...
      icvp->derv_var_pix_num *=
          longMIN(icvp->var_dim_size[idim], icvp->derv_dim_scale[idim]);
  }
...
PRIVATE int MI_get_dim_flip(mi_icv_type *icvp, int cdfid, int dimvid[],
  ...
  for (idim = 0; idim < icvp->user_num_imgdims; idim++) {
    ...
    {MI_CHK_ERR(ncdiminq(cdfid, icvp->var_dim[subsc[idim]], dimname, NULL))}

    /* Should we look for dimension flipping? */
    icvp->derv_dim_flip[idim] = FALSE;
    if (!strcmp(dimname, MIxspace) || !strcmp(dimname, MIxfrequency))
      dim_dir = icvp->user_xdim_dir;
    else if (!strcmp(dimname, MIyspace) || !strcmp(dimname, MIyfrequency))
      dim_dir = icvp->user_ydim_dir;
    else if (!strcmp(dimname, MIzspace) || !strcmp(dimname, MIzfrequency))
...
PRIVATE int MI_get_dim_scale(mi_icv_type *icvp, int cdfid, int dimvid[]) {
  ...
  for (idim = 0; idim < icvp->user_num_imgdims; idim++) {
    ...
    if (icvp->user_dim_size[idim] <= 0) {
      ...
      /* Otherwise, shrinking. Things are complicated by the fact that
        ...
        icvp->derv_dim_scale[idim] =
            1 + (icvp->var_dim_size[idim] - 1) / icvp->user_dim_size[idim];
      }
    ...
    if (idim == 0) {
      ...
      if ((min_grow && !dim_grow) ||
          ((min_grow && dim_grow) && (min_scale > dim_scale)) ||
          ((!min_grow && !dim_grow) && (min_scale < dim_scale))) {
        min_grow = dim_grow;
        min_scale = dim_scale;
      }
  ...
  for (idim = 0; idim < icvp->user_num_imgdims; idim++) {
    ...
    user_dim_size =
        ((icvp->user_dim_size[idim] <= 0) ? icvp->var_dim_size[idim]
    ...
    /* Otherwise, shrinking. Things are complicated by the fact that
      ...
      icvp->derv_dim_off[idim] =
          (user_dim_size - 1 -
           (icvp->var_dim_size[idim] - 1) / icvp->derv_dim_scale[idim]) /
          2;
    }
    ...
    icvp->derv_dim_start[idim] =
        dimstart + (icvp->derv_dim_step[idim] - dimstep) / 2.0 -
...
int miicv_attach(int icvid, int cdfid, int varid) {
  ...
  for (idim = 0; idim < icvp->user_num_imgdims; idim++) {
    ...
    if (icvp->derv_dim_grow[idim])
      size_diff =
          user_dim_size - icvp->var_dim_size[idim] * icvp->derv_dim_scale[idim];
    else
