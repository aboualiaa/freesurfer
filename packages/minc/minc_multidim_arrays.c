...
void create_empty_multidim_array(VIO_multidim_array *array, int n_dimensions,
  ...
  if (n_dimensions < 1 || n_dimensions > VIO_MAX_DIMENSIONS) {
    fprintf(stderr,
            "create_empty_multidim_array(): n_dimensions (%d) not in range 1 "
            "to %d.\n",
...
void get_type_range(VIO_Data_types type, double *min_value, double *max_value) {
  switch (type) {
  case UNSIGNED_BYTE:
    *min_value = 0.0;
    *max_value = (double)UCHAR_MAX;
    break;
  case SIGNED_BYTE:
    *min_value = (double)SCHAR_MIN;
    *max_value = (double)SCHAR_MAX;
    break;
  case UNSIGNED_SHORT:
    *min_value = 0.0;
    *max_value = (double)USHRT_MAX;
    break;
  case SIGNED_SHORT:
    *min_value = (double)SHRT_MIN;
    *max_value = (double)SHRT_MAX;
    break;
  case UNSIGNED_INT:
    *min_value = 0.0;
    *max_value = (double)UINT_MAX;
    break;
  case SIGNED_INT:
    *min_value = (double)INT_MIN;
    *max_value = (double)INT_MAX;
    break;
  case FLOAT:
    *min_value = (double)-FLT_MAX;
    *max_value = (double)FLT_MAX;
    break;
  case DOUBLE:
    *min_value = (double)-DBL_MAX;
    *max_value = (double)DBL_MAX;
    break;
  default:
    fprintf(stderr, "%s:%d no default\n", __FILE__, __LINE__);
    exit(1);
...
void set_multidim_sizes(VIO_multidim_array *array, int sizes[]) {
  ...
  for_less(dim, 0, array->n_dimensions) array->sizes[dim] = sizes[dim];
}
...
void get_multidim_sizes(VIO_multidim_array *array, int sizes[]) {
  ...
  for_less(i, 0, array->n_dimensions) sizes[i] = array->sizes[i];
}
...
void alloc_multidim_array(VIO_multidim_array *array) {
  ...
  // The pointers are a tree, the leaves pointing to vectors of n-1 dimension
  // number of elements
  //
  ...
  for_less(dim, 0, array->n_dimensions) numberOfElements *=
      (size_t)array->sizes[dim]; // e.g.: 2*3*5*7 = 210
  ...
  for_less(dim, 0, array->n_dimensions - 1) numberOfPointers +=
      (numberOfPointersPerDim *= (size_t)array->sizes[dim]);
  // e.g.: 2 + 6 + 30 = 38
  ...
  for_less(dim, 1, array->n_dimensions - 1) {
    ...
    size_t numberOfPtrsToFillIn =
        prevNumberOfPtrsToFillIn * (size_t)array->sizes[dim - 1];
...
void copy_multidim_data_reordered(int type_size, void *void_dest_ptr,
                                  ...
                                  int to_dest_index[], BOOLEAN use_src_order) {
  ...
  if (use_src_order) {
    ...
    for_less(dest_index, 0, n_dest_dims) {
      for_less(d, 0, n_src_dims) if (to_dest_index[d] == dest_index) break;
  ...
  for (d = n_transfer_dims - 1; d >= 0; d--) {
    ...
    transfer_counts[d + VIO_MAX_DIMENSIONS - n_transfer_dims] =
        transfer_counts[d];
  }
...
void copy_multidim_reordered(VIO_multidim_array *dest, int dest_ind[],
                             VIO_multidim_array *src, int src_ind[],
                             int counts[], int to_dest_index[]) {
  ...
  copy_multidim_data_reordered(type_size, dest_ptr, n_dest_dims, dest_sizes,
                               src_ptr, n_src_dims, src_sizes, counts,
                               to_dest_index, true);
}
...
int main() {
  ...
  for (mode = 0; mode < 2; mode++) {
    ...
    for (i = 0; (i < limit) && (errors < 10); i++) {
      ...
      void *voidPtr;
      GET_MULTIDIM_PTR(voidPtr, array, indices[0], indices[1], indices[2],
                       indices[3], indices[4]);
      ...
      if (!mode)
        *ptr = i;
        ...
        if (i != *ptr) {
          errors++;
          printf("Error expected %ld != gotten %ld pointer %p at ", (long)i,
                 (long)*ptr, ptr);
