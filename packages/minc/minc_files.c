...
static int string_length(const char *s) { return s ? (int)strlen(s) : 0; }
...
static bool string_ends_in(const char *whole, const char *tail) {
  ...
  return (tailLen <= wholeLen) &&
         (0 == memcmp(whole + wholeLen - tailLen, tail, tailLen));
}

static void concat_char_to_string(char **lhs, char rhs) {
  if (!*lhs) {
    *lhs = (char *)malloc(2);
    *lhs = 0;
  }
...
static void concat_to_string(char **lhs, const char *rhs) {
  if (!rhs)
    return;
  size_t rhs_strlen = string_length(rhs);
  if (!*lhs) {
    *lhs = (char *)malloc(rhs_strlen + 1);
    lhs[0] = 0;
  }
  size_t lhs_strlen = string_length(*lhs);
...
static const char *compressed_endings[compressed_endings_size] = {".z", ".Z",
                                                                  ".gz"};
...
VIO_BOOL file_directory_exists(const char *filename) {
  VIO_BOOL exists;
  const char *dir;
...
VIO_BOOL check_clobber_file(const char *filename) {
  VIO_BOOL okay;
  const char *expanded;
  ...
  if (file_exists(filename)) {
    ...
    printf("File <%s> exists, do you wish to overwrite (y or n): ", expanded);
...
VIO_BOOL check_clobber_file_default_suffix(const char *filename,
...
static const char *create_backup_filename(const char *filename) {
  const char *const expanded = expand_filename(filename);
  const char *const date = get_date();

  char *const backup_filename =
      (char *)malloc(string_length(expanded) + string_length(date) + 100);

  int count = 0;
  do {
    if (count == 0) {
      (void)sprintf(backup_filename, "%s.%s.bkp", expanded, date);
    } else {
      (void)sprintf(backup_filename, "%s.%s.bkp_%d", expanded, date, count);
    }

    int len = string_length(backup_filename);
    while (len > 0 && (backup_filename[len - 1] == ' ' ||
...
VIO_Status make_backup_file(const char *filename,
...
void cleanup_backup_file(const char *filename, const char *backup_filename,
...
VIO_Status copy_file(const char *src, const char *dest) {
  ...
  if (system(command) != 0) {
    fprintf(stderr, "Error copying file %s to %s: ", src_expanded,
            dest_expanded);
    print_system_error();
    status = ERROR;
  }
...
VIO_Status move_file(const char *src, const char *dest) {
  VIO_Status status;
  const char *src_expanded = expand_filename(src);
  const char *dest_expanded = expand_filename(dest);
  ...
  if (system(command) != 0) {
    fprintf(stderr, "Error moving file %s to %s: ", src_expanded,
            dest_expanded);
    print_system_error();
    status = ERROR;
  } else
...
static const char *get_user_home_directory(const char *user_name) {
#if HAVE_GETPWNAM
  struct passwd *p;
...
char *expand_filename(const char *filename) {
  ...
  while (i < len) {
    ...
    if (!prev_was_backslash &&
      ...
      int temp_size = 0;
      while (filename[new_i] != '/' && filename[new_i] != '.' &&
             filename[new_i] != 0) {
...
bool filename_extension_matches(const char *expanded_filename_possibly_with_z,
  ...
  char *const expanded_filename_no_z =
      strdup(expanded_filename_possibly_with_z);
...
char *remove_directories_from_filename(const char *filename) {
  const char *expanded = expand_filename(filename);
...
char *file_exists_as_compressed(const char *expanded) // don't expand twice!
{
  int i;
  for (i = 0; i < compressed_endings_size; i++) {
...
VIO_Status open_file(const char *filename, VIO_IO_types io_type,
  ...
  switch (io_type) {
  case APPEND_FILE:
    access_str = create_string("a");
    break;
  case WRITE_FILE:
    access_str = create_string("w");
    break;
  case READ_FILE:
  default:
    access_str = create_string("r");
    break;
  }
  ...
  if (compressed) {
    ...
    if (command_status == 0) {
      ...
      status = ERROR;
      fprintf(stderr,
              "Error uncompressing %s into %s using gunzip and bunzip2\n",
              compressed, tmp_name);
    }
...
static bool has_no_extension(const char *filename) {
...
char *get_absolute_filename(const char *filename, const char *directory) {
...
VIO_Status flush_file(FILE *file) {
  VIO_Status status;
...
VIO_Status output_character(FILE *file, char ch) {
  VIO_Status status;
...
VIO_Status skip_input_until(FILE *file, char search_char) {
  VIO_Status status;
  char ch;
...
VIO_Status output_string(FILE *file, const char *str) {
  VIO_Status status;
...
VIO_Status input_quoted_string(FILE *file, char **str) {
  char ch, quote;
  VIO_Status status;
...
VIO_Status input_possibly_quoted_string(FILE *file, char **str) {
  char quote;
...
VIO_Status output_quoted_string(FILE *file, const char *str) {
  VIO_Status status;
...
VIO_Status input_binary_data(FILE *file, void *data, size_t element_size,
                             int n) {
  VIO_Status status;
  int n_done;
...
VIO_Status output_binary_data(FILE *file, void *data, size_t element_size,
                              int n) {
  VIO_Status status;
  int n_done;
...
VIO_Status output_newline(FILE *file) {
  VIO_Status status;
...
VIO_Status input_line(FILE *file, char **line) {
  VIO_Status status;
  char ch;
...
VIO_Status input_boolean(FILE *file, VIO_BOOL *b) {
  VIO_Status status;
  char ch;
...
VIO_Status output_boolean(FILE *file, VIO_BOOL b) {
  VIO_Status status;
  const char *str;
...
VIO_Status input_short(FILE *file, short *s) {
  VIO_Status status;
...
VIO_Status output_short(FILE *file, short s) {
  VIO_Status status;
...
VIO_Status input_unsigned_short(FILE *file, unsigned short *s) {
  int i;
  VIO_Status status;
...
VIO_Status output_unsigned_short(FILE *file, unsigned short s) {
  VIO_Status status;
...
VIO_Status output_int(FILE *file, int i) {
  VIO_Status status;
...
VIO_Status input_float(FILE *file, float *f) {
  VIO_Status status;
...
VIO_Status output_float(FILE *file, float f) {
  VIO_Status status;
...
VIO_Status output_double(FILE *file, double d) {
  VIO_Status status;
...
VIO_Status io_newline(FILE *file, VIO_IO_types io_flag,
...
VIO_Status io_quoted_string(FILE *file, VIO_IO_types io_flag,
                            VIO_File_formats format, char **str) {
  int length;
  VIO_Status status;
  ...
  if (format == ASCII_FORMAT) {
    ...
    if (status == OK) {
      status = io_binary_data(file, io_flag, (void *)(*str), sizeof((*str)[0]),
                              length);
    }
...
VIO_Status io_boolean(FILE *file, VIO_IO_types io_flag, VIO_File_formats format,
                      VIO_BOOL *b) {
  VIO_Status status;
...
VIO_Status io_short(FILE *file, VIO_IO_types io_flag, VIO_File_formats format,
                    short *short_int) {
  VIO_Status status;
  ...
  if (format == ASCII_FORMAT) {
    ...
    status =
        io_binary_data(file, io_flag, (void *)short_int, sizeof(*short_int), 1);
...
VIO_Status io_unsigned_short(FILE *file, VIO_IO_types io_flag,
                             VIO_File_formats format,
...
VIO_Status io_unsigned_char(FILE *file, VIO_IO_types io_flag,
                            VIO_File_formats format, unsigned char *c) {
  int i;
  VIO_Status status;
...
VIO_Status io_double(FILE *file, VIO_IO_types io_flag, VIO_File_formats format,
                     double *d) {
  VIO_Status status;
...
VIO_Status io_ints(FILE *file, VIO_IO_types io_flag, VIO_File_formats format,
                   int n, int *ints[]) {
  VIO_Status status;
  int i;
#define INTS_PER_LINE 8
  ...
  if (format == ASCII_FORMAT) {
    ...
    status =
        io_binary_data(file, io_flag, (void *)*ints, sizeof((*ints)[0]), n);
  }
...
VIO_Status io_unsigned_chars(FILE *file, VIO_IO_types io_flag,
                             VIO_File_formats format, int n,
  ...
  if (format == ASCII_FORMAT) {
    for (i = 0; i < n; i++) {
      status = io_unsigned_char(file, io_flag, format, &(*unsigned_chars)[i]);
