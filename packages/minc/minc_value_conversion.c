...
// /private-cvsroot/minc/libsrc/value_conversion.c,v 6.4.2.1 2004/09/28 20:23:40
// bert Exp $ MINC (MNI)";
#endif
...
@MODIFIED   :
---------------------------------------------------------------------------- */
SEMIPRIVATE int MI_varaccess(int operation, int cdfid, int varid, long start[],
                             long count[], nc_type datatype, int sign,
                             void *values, int *bufsize_step,
                             mi_icv_type *icvp) {
  ...
  MI_CHK_ERR(ncvarinq(cdfid, varid, NULL, &(strc.var_type), &ndims, NULL, NULL))
  ...
  strc.count = count;
  strc.values = values;
  MI_CHK_ERR(MI_var_loop(ndims, start, count, strc.var_value_size, bufsize_step,
                         MI_MAX_VAR_BUFFER_SIZE, (void *)&strc, MI_var_action))
  MI_RETURN(MI_NOERROR);
}
...
/* ARGSUSED */
{
  ...
  case MI_PRIV_GET:
    status = ncvarget(ptr->cdfid, ptr->varid, var_start, var_count, var_buffer);
    if (status != MI_ERROR) {
      /* If doing dimension conversion, let dimconvert function do all the
         work, including type conversion */
      if (!ptr->do_dimconvert) {
        status = MI_convert_type(nvalues, ptr->var_type, ptr->var_sign,
                                 var_buffer, ptr->call_type, ptr->call_sign,
                                 ptr->values, ptr->icvp);
      } else {
        status = (*(ptr->icvp->dimconvert_func))(
            ptr->operation, ptr->icvp, ptr->start, ptr->count, ptr->values,
  ...
  case MI_PRIV_PUT:
    ...
    if (!ptr->do_dimconvert) {
      status =
          MI_convert_type(nvalues, ptr->call_type, ptr->call_sign, ptr->values,
                          ptr->var_type, ptr->var_sign, var_buffer, ptr->icvp);
    } else {
      status = (*(ptr->icvp->dimconvert_func))(
          ptr->operation, ptr->icvp, ptr->start, ptr->count, ptr->values,
          var_start, var_count, var_buffer);
...
#endif
      status =
          ncvarput(ptr->cdfid, ptr->varid, var_start, var_count, var_buffer);
    }
    break;
  default:
    MI_LOG_PKG_ERROR2(MI_ERR_BADOP, "Illegal variable access operation");
  ...
  if (!ptr->do_dimconvert) {
    ptr->values =
        (void *)((char *)ptr->values + nvalues * ptr->call_value_size);
  }
...
---------------------------------------------------------------------------- */
SEMIPRIVATE int
MI_var_loop(int ndims, long start[], long count[], int value_size,
  ...
  for (firstdim = ndims - 1; firstdim >= 1; firstdim--) {
    ...
    if (newnvalues * value_size > max_buffer_size)
      break;
    nvalues = newnvalues;
  }
  ...
  if ((var_buffer = MALLOC(ntimes * nvalues * value_size, char)) == NULL) {
  ...
  for (i = 0; i < ndims; i++) {
    var_count[i] = (i > firstdim) ? count[i] : (i == firstdim) ? ntimes : 1;
    var_start[i] = start[i];
...
SEMIPRIVATE int MI_get_sign_from_string(nc_type datatype, char *sign) {
  ...
  MI_RETURN(MI_get_sign(datatype,
                        (sign == NULL) || (STRINGS_EQUAL(sign, MI_EMPTY_STRING))
                            ? MI_PRIV_DEFSIGN
                            : (STRINGS_EQUAL(sign, MI_SIGNED))
                                  ? MI_PRIV_SIGNED
                                  : (STRINGS_EQUAL(sign, MI_UNSIGNED))
                                        ? MI_PRIV_UNSIGNED
                                        : MI_PRIV_DEFSIGN));
}
...
PRIVATE int MI_get_sign(nc_type datatype, int sign) {
  ...
  MI_RETURN(((datatype == NC_FLOAT) || (datatype == NC_DOUBLE))
                ? MI_PRIV_SIGNED
                : ((sign == MI_PRIV_SIGNED) || (sign == MI_PRIV_UNSIGNED))
                      ? sign
                      : (datatype == NC_BYTE)
                            ? MI_PRIV_UNSIGNED
                            : (datatype == NC_SHORT)
...
---------------------------------------------------------------------------- */
SEMIPRIVATE int MI_convert_type(long number_of_values, nc_type intype,
                                int insign, void *invalues, nc_type outtype,
                                int outsign, void *outvalues,
                                mi_icv_type *icvp) {
  ...
  if ((intype == outtype) && (insgn == outsgn) && !do_scale && !do_fillvalue) {
    (void)memcpy(outvalues, invalues, (size_t)number_of_values * inincr);
  }
