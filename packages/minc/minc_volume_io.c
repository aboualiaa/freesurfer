...
static VIO_Status input_transform(FILE *file, const char *filename,
                                  General_transform *transform);

char *statusToString(VIO_Status status) {
  static char *table[] = {"OK", "ERROR", "INTERNAL_ERROR", "END_OF_FILE",
                          "QUIT"};
  return status >= 0 && status < VIO_Status__end ? table[status] : "Bad status";
};
...
---------------------------------------------------------------------------- */
static VIO_Colour make_rgba_Colour(int r, int g, int b, int a) {
  return (((VIO_Colour)(unsigned char)a) << 0) +
         (((VIO_Colour)(unsigned char)b) << 8) +
         (((VIO_Colour)(unsigned char)g) << 16) +
         (((VIO_Colour)(unsigned char)r) << 24);
}

VIO_Colour make_rgba_Colour_0_1(double r, double g, double b, double a) {
  return (make_rgba_Colour((int)(r * 255.0 + 0.5), (int)(g * 255.0 + 0.5),
                           (int)(b * 255.0 + 0.5), (int)(a * 255.0 + 0.5)));
}
...
bool close_to_identity(Transform *transform) {
  int i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      double expected = (i == j) ? 1.0 : 0.0;
      double actual = Transform_elem(*transform, i, j);
      if (fabsf(expected - actual) > 0.001)
        return false;
    }
  }
...
void concat_transforms(Transform *result, Transform *t1, Transform *t2) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      int k;
      for (k = 0; k < 4; k++) {
        sum += Transform_elem(*t2, i, k) * Transform_elem(*t1, k, j);
      }
...
static void homogenous_transform_point(Transform *transform, double x, double y,
                                       double z, double w, double *x_trans,
...
void transform_point(Transform *transform, double x, double y, double z,
                     double *x_trans, double *y_trans, double *z_trans) {
  homogenous_transform_point(transform, x, y, z, 1.0, x_trans, y_trans,
                             z_trans);
}

void get_transform_origin_real(Transform *transform, double origin[]) {
  origin[X] = Transform_elem(*transform, 0, 3);
  origin[Y] = Transform_elem(*transform, 1, 3);
  origin[Z] = Transform_elem(*transform, 2, 3);
}

void get_transform_x_axis_real(Transform *transform, double x_axis[]) {
  x_axis[X] = Transform_elem(*transform, 0, 0);
  x_axis[Y] = Transform_elem(*transform, 1, 0);
  x_axis[Z] = Transform_elem(*transform, 2, 0);
}

void get_transform_y_axis_real(Transform *transform, double y_axis[]) {
  y_axis[X] = Transform_elem(*transform, 0, 1);
  y_axis[Y] = Transform_elem(*transform, 1, 1);
  y_axis[Z] = Transform_elem(*transform, 2, 1);
}

void get_transform_z_axis_real(Transform *transform, double z_axis[]) {
...
@COPYRIGHT  :
              ...
              express or implied warranty.
---------------------------------------------------------------------------- */
static bool compute_transform_inverse(Transform *transform,
                                      Transform *inverse) {
  int i, j;
...
@COPYRIGHT  :
              ...
              express or implied warranty.
---------------------------------------------------------------------------- */
int get_n_concated_transforms(General_transform *transform) {
...
General_transform *get_nth_general_transform(General_transform *transform,
                                             int n) {
...
Transform *get_linear_transform_ptr(General_transform *transform) {
...
Transform *get_inverse_linear_transform_ptr(General_transform *transform) {
  if (transform->type == LINEAR) {
    if (transform->inverse_flag)
      return (transform->linear_transform);
    else
...
static void general_transform_point(General_transform *transform, double x,
                                    double y, double z, double *x_transformed,
                                    double *y_transformed,
                                    double *z_transformed);

static void general_inverse_transform_point(General_transform *transform,
                                            double x, double y, double z,
                                            double *x_transformed,
                                            double *y_transformed,
                                            double *z_transformed);

static void transform_or_invert_point(General_transform *transform,
                                      bool inverse_flag, double x, double y,
                                      double z, double *x_transformed,
                                      double *y_transformed,
                                      double *z_transformed) {
  switch (transform->type) {
  case LINEAR:
    if (inverse_flag)
      transform_point(transform->inverse_linear_transform, x, y, z,
                      x_transformed, y_transformed, z_transformed);
    else
      transform_point(transform->linear_transform, x, y, z, x_transformed,
                      y_transformed, z_transformed);
    break;

#if defined(BEVIN_ALL_TYPES_SUPPORTED)
  case THIN_PLATE_SPLINE:
    if (inverse_flag) {
      thin_plate_spline_inverse_transform(
          transform->n_dimensions, transform->n_points, transform->points,
          transform->displacements, x, y, z, x_transformed, y_transformed,
          z_transformed);
    } else {
      thin_plate_spline_transform(transform->n_dimensions, transform->n_points,
                                  transform->points, transform->displacements,
                                  x, y, z, x_transformed, y_transformed,
                                  z_transformed);
    }
    break;
  ...
  case GRID_TRANSFORM:
    if (inverse_flag) {
      grid_inverse_transform_point(transform, x, y, z, x_transformed,
                                   y_transformed, z_transformed);
    } else {
      grid_transform_point(transform, x, y, z, x_transformed, y_transformed,
                           z_transformed);
    }
    break;
  ...
  case USER_TRANSFORM:
    if (inverse_flag) {
      transform->user_inverse_transform_function(transform->user_data, x, y, z,
                                                 x_transformed, y_transformed,
                                                 z_transformed);
    } else {
      transform->user_transform_function(transform->user_data, x, y, z,
                                         x_transformed, y_transformed,
                                         z_transformed);
    }
    break;
    ...
    if (inverse_flag) {
      ...
      for (trans = transform->n_transforms - 1; trans >= 0; --trans) {
        general_inverse_transform_point(
            &transform->transforms[trans], *x_transformed, *y_transformed,
            *z_transformed, x_transformed, y_transformed, z_transformed);
      }
    } else {
      int trans;
      for (trans = 0; trans < transform->n_transforms; trans++) {
        general_transform_point(&transform->transforms[trans], *x_transformed,
                                *y_transformed, *z_transformed, x_transformed,
                                y_transformed, z_transformed);
      }
    }
    break;
...
static void alloc_linear_transform(General_transform *transform) {
...
static void create_linear_transform(General_transform *transform,
...
static void copy_and_invert_transform(General_transform *transform,
                                      bool invert_it, General_transform *copy) {
  Transform *swap;
  int trans;
  ...
  *copy = *transform;
  ...
  case LINEAR:
    ...
    *(copy->inverse_linear_transform) = *(transform->inverse_linear_transform);
  ...
  case THIN_PLATE_SPLINE:
    ...
    for_less(i, 0, copy->n_points) for_less(j, 0, copy->n_dimensions)
  ...
  case GRID_TRANSFORM:
    copy->displacement_volume =
        (void *)copy_volume((Volume)transform->displacement_volume);
  ...
  case USER_TRANSFORM:
    ...
    (void)memcpy(copy->user_data, transform->user_data, copy->size_user_data);
    if (invert_it)
      copy->inverse_flag = !copy->inverse_flag;
    break;
#endif
  ...
  case CONCATENATED_TRANSFORM:
    copy->transforms = (General_transform *)malloc(sizeof(General_transform) *
                                                   copy->n_transforms);
...
static void create_inverse_general_transform(General_transform *transform,
                                             General_transform *inverse) {
  copy_and_invert_transform(transform, true, inverse);
}

void copy_general_transform(General_transform *transform,
                            General_transform *copy) {
  copy_and_invert_transform(transform, false, copy);
}

void delete_general_transform(General_transform *transform) {
...
static VIO_Status mni_get_nonwhite_character(FILE *file, char *ch) {
...
static VIO_Status mni_skip_expected_character(FILE *file, char expected_ch) {
  char ch;
...
static VIO_Status mni_input_string(FILE *file, char **string,
  ...
  *string = NULL;
  ...
  while (status == OK && ch != termination_char1 && ch != termination_char2 &&
         ch != '\n') {
  ...
  while (buffer_size > 1 && buffer[buffer_size - 2] == ' ')
    buffer_size--;
...
static VIO_Status mni_input_double(FILE *file, double *d) {
  char *str;
...
static VIO_Status mni_input_keyword_and_equal_sign(FILE *file,
                                                   const char *keyword,
...
void concat_general_transforms(General_transform *first,
  ...
  first_inverted_concat =
      first->type == CONCATENATED_TRANSFORM && first->inverse_flag;
  second_inverted_concat =
      second->type == CONCATENATED_TRANSFORM && second->inverse_flag;
  ...
  result_ptr->n_transforms =
      fabs(first_end - first_start) + 1 + fabs(second_end - second_start) + 1;
  ...
  if (result_ptr->n_transforms == 1)
    ...
    result_ptr->transforms = (General_transform *)malloc(
  ...
  if (crunching_linear) {
    ...
    concat_transforms(get_linear_transform_ptr(transform), first_transform,
                      second_transform);
...
static VIO_Status input_one_transform(FILE *file, const char *filename,
                                      General_transform *transform) {
  VIO_Status status;
  int i, j;
  char *type_name;
  ...
  if (!strcmp(str, INVERT_FLAG_STRING)) {
    ...
    if (!strcmp(str, TRUE_STRING))
      ...
      fprintf(stderr, "Expected %s or %s after %s =\n", TRUE_STRING,
              FALSE_STRING, INVERT_FLAG_STRING);
      return (ERROR);
    }
  ...
  case THIN_PLATE_SPLINE:
    ...
    if (n_points % n_dimensions != 0) {
      print_error("Number of points (%d) must be multiple of number of "
                  "dimensions (%d)\n",
    ...
    create_thin_plate_transform_real(transform, n_dimensions, n_points, points,
                                     displacements);
  ...
  case GRID_TRANSFORM:
    ...
    if (mni_input_string(file, &volume_filename, (char)';', (char)0) != OK)
      return (ERROR);
    ...
    if (input_volume(volume_filename, 4, NULL, MI_ORIGINAL_TYPE, FALSE, 0.0,
                     0.0, TRUE, &volume, &options) != OK) {
      delete_string(volume_filename);
      return (ERROR);
    }
...
VIO_Status input_transform_file(const char *filename,
                                General_transform *transform) {
  ...
  FILE *file;
  status = open_file_with_default_suffix(filename, "xfm", READ_FILE,
                                         ASCII_FORMAT, &file);
  if (status == OK)
...
@COPYRIGHT  :
              ...
              express or implied warranty.
---------------------------------------------------------------------------- */
static double convert_voxel_to_value(Volume volume, double voxel) {
  if (volume->real_range_set)
    return (volume->real_value_scale * voxel + volume->real_value_translation);
  else
    return (voxel);
}

void set_volume_voxel_value(Volume volume, int x, int y, int z, int t, int v,
                            double voxel) {
...
double get_volume_voxel_value(Volume volume, int x, int y, int z, int t,
                              int v) {
  double voxel;
...
@COPYRIGHT  :
              ...
              express or implied warranty.
---------------------------------------------------------------------------- */
void delete_volume_input(volume_input_struct *input_info) {
...
@COPYRIGHT  :
              ...
              express or implied warranty.
---------------------------------------------------------------------------- */
static double dot_vectors(int n, double v1[], double v2[]) {
  int i;
  double d;
...
static void cross_3D_vector(double v1[], double v2[], double cross[]) {
  cross[0] = v1[1] * v2[2] - v1[2] * v2[1];
  cross[1] = v1[2] * v2[0] - v1[0] * v2[2];
  cross[2] = v1[0] * v2[1] - v1[1] * v2[0];
}

static void normalize_vector(double v1[], double v1_normalized[]) {
  int d;
  double mag;
...
static void assign_voxel_to_world_transform(Volume volume,
                                            General_transform *transform) {
...
static void reorder_voxel_to_xyz(Volume volume, double voxel[], double xyz[]) {
  int c;
...
void compute_world_transform(int spatial_axes[VIO_N_DIMENSIONS],
                             double separations[],
  ...
  if (n_axes == 1) {
    ...
    directions[a1][X] =
        directions[axis_list[0]][Y] + directions[axis_list[0]][Z];
    directions[a1][Y] =
        -directions[axis_list[0]][X] - directions[axis_list[0]][Z];
    directions[a1][Z] =
        directions[axis_list[0]][Y] - directions[axis_list[0]][X];

    cross_3D_vector(directions[axis_list[0]], directions[a1], directions[a2]);
  ...
  for (c = 0; c < VIO_N_DIMENSIONS; c++) {
    for (dim = 0; dim < VIO_N_DIMENSIONS; dim++) {
      Transform_elem(transform, dim, c) =
          directions[c][dim] * separations_3D[c];

      Transform_elem(transform, dim, 3) += directions[c][dim] * starts_3D[c];
    }
  }
...
void check_recompute_world_transform(Volume volume) {
  General_transform world_transform;

  if (!volume->voxel_to_world_transform_uptodate) {
    volume->voxel_to_world_transform_uptodate = true;

    compute_world_transform(volume->spatial_axes, volume->separations,
                            volume->direction_cosines, volume->starts,
                            &world_transform);
...
bool convert_dim_name_to_spatial_axis(const char *name, int *axis) {
  *axis = -1;
...
static void convert_transform_origin_to_starts(
    double origin[], int n_volume_dimensions, int spatial_axes[],
    double dir_cosines[][VIO_N_DIMENSIONS], double starts[]) {
  ...
  if (n_axes == 1) {
    double o_dot_c =
        dot_vectors(VIO_N_DIMENSIONS, origin, dir_cosines[which[0]]);
    double c_dot_c = dot_vectors(VIO_N_DIMENSIONS, dir_cosines[which[0]],
                                 dir_cosines[which[0]]);
    ...
    //
    double x_dot_v =
        dot_vectors(VIO_N_DIMENSIONS, dir_cosines[which[0]], origin);
    double y_dot_v =
        dot_vectors(VIO_N_DIMENSIONS, dir_cosines[which[1]], origin);

    double x_dot_x = dot_vectors(VIO_N_DIMENSIONS, dir_cosines[which[0]],
                                 dir_cosines[which[0]]);
    double y_dot_y = dot_vectors(VIO_N_DIMENSIONS, dir_cosines[which[1]],
                                 dir_cosines[which[1]]);

    double x_dot_y = dot_vectors(VIO_N_DIMENSIONS, dir_cosines[which[0]],
                                 dir_cosines[which[1]]);
    ...
    for (i = 0; i < VIO_N_DIMENSIONS; i++)
      for (j = 0; j < VIO_N_DIMENSIONS; j++) {
        matrix[Index4x4(i, j)] =
            dir_cosines[which[j]]
                       [i]; // I don't know why this transposes the matrix
      }
...
void convert_transform_to_starts_and_steps(
    General_transform *transform, int n_volume_dimensions, double step_signs[],
    int spatial_axes[], double starts[], double steps[],
  ...
  if (transform->type != LINEAR) {
    fprintf(stderr, "convert_transform_to_starts_and_steps(): non-linear "
                    "transform found.\n");
    exit(1);
  }
  ...
  convert_transform_origin_to_starts(origin, n_volume_dimensions, spatial_axes,
                                     dir_cosines, starts);
}

static const char
    *default_dimension_names[VIO_MAX_DIMENSIONS][VIO_MAX_DIMENSIONS] = {
        {MIxspace},
        {MIyspace, MIxspace},
...
void set_volume_space_type(Volume volume, const char *name) {
  free((void *)volume->coordinate_system_name);
  volume->coordinate_system_name = strdup(name);
}

int set_volume_irregular_starts(Volume volume, int idim, int count,
                                double *starts) {
#if !defined(BEVIN_ALL_VOLUME_MEMBERS)
...
int set_volume_irregular_widths(Volume volume, int idim, int count,
                                double *widths) {
#if !defined(BEVIN_ALL_VOLUME_MEMBERS)
...
static double get_volume_real_min(Volume volume) {
...
static double get_volume_real_max(Volume volume) {
...
void get_volume_voxel_range(Volume volume, double *voxel_min,
                            double *voxel_max) {
  *voxel_min = volume->voxel_min;
  *voxel_max = volume->voxel_max;
}

void set_volume_real_range(Volume volume, double real_min, double real_max) {
  ...
  if (get_volume_data_type(volume) == FLOAT ||
    ...
    if (voxel_min < voxel_max) {
      volume->real_value_scale =
          (real_max - real_min) / (voxel_max - voxel_min);
      volume->real_value_translation =
          real_min - voxel_min * volume->real_value_scale;
    } else {
...
void get_volume_real_range(Volume volume, double *min_value,
                           double *max_value) {
  *min_value = get_volume_real_min(volume);
  *max_value = get_volume_real_max(volume);
}

void set_volume_voxel_range(Volume volume, double voxel_min, double voxel_max) {
  ...
  if (voxel_min >= voxel_max) {
    ...
    case UNSIGNED_BYTE:
      voxel_min = 0.0;
      voxel_max = (double)UCHAR_MAX;
      break;
    case SIGNED_BYTE:
      voxel_min = (double)SCHAR_MIN;
      voxel_max = (double)SCHAR_MAX;
      break;
    case UNSIGNED_SHORT:
      voxel_min = 0.0;
      voxel_max = (double)USHRT_MAX;
      break;
    case SIGNED_SHORT:
      voxel_min = (double)SHRT_MIN;
      voxel_max = (double)SHRT_MAX;
      break;
    case UNSIGNED_INT:
      voxel_min = 0.0;
      voxel_max = (double)UINT_MAX;
      break;
    case SIGNED_INT:
      voxel_min = (double)INT_MIN;
      voxel_max = (double)INT_MAX;
      break;
    case FLOAT:
      voxel_min = (double)-FLT_MAX;
      voxel_max = (double)FLT_MAX;
      break;
    case DOUBLE:
      voxel_min = (double)-DBL_MAX;
      voxel_max = (double)DBL_MAX;
      break;
    default:
...
void set_volume_type(Volume volume, nc_type nc_data_type, bool signed_flag,
                     double voxel_min, double voxel_max) {
...
VIO_Data_types get_volume_data_type(Volume volume) {
  return (get_multidim_data_type(&volume->array));
}

int get_volume_n_dimensions(Volume volume) {
  return (get_multidim_n_dimensions(&volume->array));
}

void get_volume_sizes(Volume volume, int sizes[]) {
  get_multidim_sizes(&volume->array, sizes);
}

nc_type get_volume_nc_data_type(Volume volume, bool *signed_flag) {
  if (signed_flag != (BOOLEAN *)NULL)
    *signed_flag = volume->signed_flag;
  return (volume->nc_data_type);
}

void set_rgb_volume_flag(Volume volume, bool flag) {
  if (!flag || get_volume_data_type(volume) == UNSIGNED_INT)
    volume->is_rgba_data = flag;
}

Volume create_volume(int n_dimensions,
                     /*const*/ char *dimension_names[], // need compat with minc
                     nc_type nc_data_type, bool signed_flag, double voxel_min,
                     double voxel_max) {
  int i;
  VIO_Status status;
...
bool volume_is_alloced(Volume volume) {
  return (
#if defined(BEVIN_ALL_VOLUME_MEMBERS)
      volume->is_cached_volume && volume_cache_is_alloced(&volume->cache) ||
      !volume->is_cached_volume &&
#endif
          multidim_array_is_alloced(&volume->array));
}

void free_volume_data(Volume volume) {
#if defined(BEVIN_ALL_VOLUME_MEMBERS)
  if (volume->is_cached_volume)
...
void set_volume_sizes(Volume volume, int sizes[]) {
  set_multidim_sizes(&volume->array, sizes);
}

void alloc_volume_data(Volume volume) {
#if defined(BEVIN_ALL_VOLUME_MEMBERS)
...
void set_volume_separations(Volume volume, double separations[]) {
  int i;
...
void set_volume_direction_unit_cosine(Volume volume, int axis, double dir[]) {
...
void set_volume_direction_cosine(Volume volume, int axis, double dir[]) {
  double len, unit_vector[VIO_N_DIMENSIONS];

  len = dir[X] * dir[X] + dir[Y] * dir[Y] + dir[Z] * dir[Z];

  if (len == 0.0) {
    fprintf(stderr, "Warning: zero length direction cosine in "
                    "set_volume_direction_cosine()\n");
    exit(1);
    return;
  }
...
void set_volume_starts(Volume volume, double starts[]) {
  int c;
...
void set_volume_translation(Volume volume, double voxel[],
  ...
  if (n_axes == 1) {
    ...
    Transform_elem(transform, 0, a1) =
        Transform_elem(transform, 1, dim) + Transform_elem(transform, 2, dim);
    Transform_elem(transform, 1, a1) =
        -Transform_elem(transform, 0, dim) - Transform_elem(transform, 2, dim);
    Transform_elem(transform, 2, a1) =
        Transform_elem(transform, 1, dim) - Transform_elem(transform, 0, dim);
  ...
  if (n_axes == 1 || n_axes == 2) {
    ...
    Transform_elem(transform, 0, dim) =
        Transform_elem(transform, 1, a1) * Transform_elem(transform, 2, a2) -
        Transform_elem(transform, 1, a2) * Transform_elem(transform, 2, a1);
    Transform_elem(transform, 1, dim) =
        Transform_elem(transform, 2, a1) * Transform_elem(transform, 0, a2) -
        Transform_elem(transform, 2, a2) * Transform_elem(transform, 0, a1);
    Transform_elem(transform, 2, dim) =
        Transform_elem(transform, 0, a1) * Transform_elem(transform, 1, a2) -
        Transform_elem(transform, 0, a2) * Transform_elem(transform, 1, a1);

    /*--- normalize vector */

    len =
        Transform_elem(transform, 0, dim) * Transform_elem(transform, 0, dim) +
  ...
  transform_point(&inverse, world_space_origin[X], world_space_origin[Y],
                  world_space_origin[Z], &starts_3d[X], &starts_3d[Y],
                  &starts_3d[Z]);
...
General_transform *get_voxel_to_world_transform(Volume volume) {
  check_recompute_world_transform(volume);

  return (&volume->voxel_to_world_transform);
}

static const char *convert_spatial_axis_to_dim_name(int axis) {
  switch (axis) {
  case X:
    return (MIxspace);
  case Y:
...
char **get_volume_dimension_names(Volume volume) {
  char **names =
      (char **)calloc(sizeof(char *), get_volume_n_dimensions(volume));

  int i;
  for (i = 0; i < get_volume_n_dimensions(volume); i++)
    names[i] = strdup((volume->spatial_axes[i] >= 0)
...
void delete_dimension_names(Volume volume, char *dimension_names[]) {
  int i;
  for (i = 0; i < get_volume_n_dimensions(volume); i++)
...
void get_volume_separations(Volume volume, double separations[]) {
  int i;
...
void convert_voxel_to_world(Volume volume, double voxel[], double *x_world,
                            double *y_world, double *z_world) {
  ...
  general_transform_point(&volume->voxel_to_world_transform, xyz[X], xyz[Y],
                          xyz[Z], x_world, y_world, z_world);
}

VIO_Status
start_volume_input(const char *filename, int n_dimensions,
                   /*const*/ char *dim_names[], // need compat with minc
                   nc_type volume_nc_data_type, bool volume_signed_flag,
                   double volume_voxel_min, double volume_voxel_max,
                   bool create_volume_flag, Volume *volume,
                   minc_input_options *options,
  ...
  if (create_volume_flag || *volume == (Volume)NULL) {
    ...
    *volume =
        create_volume(n_dimensions, dim_names, volume_nc_data_type,
                      volume_signed_flag, volume_voxel_min, volume_voxel_max);
  ...
  case FREE_FORMAT:
    status =
        initialize_free_format_input(expanded_filename, *volume, input_info);
    break;
  }
...
bool input_more_of_volume(Volume volume, volume_input_struct *input_info,
                          double *fraction_done) {
  bool more_to_do = false;

  switch (input_info->file_format) {
  case MNC_FORMAT:
    more_to_do = input_more_minc_file(input_info->minc_file, fraction_done);
    break;

  case FREE_FORMAT:
    more_to_do = input_more_free_format_file(volume, input_info, fraction_done);
    break;
  }
