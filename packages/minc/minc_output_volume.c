...
VIOAPI Status get_file_dimension_names(STRING filename, int *n_dims,
                                       STRING *dim_names[]) {
  int i;
  Status status;
  ...
  status = start_volume_input(filename, -1, initial_dim_names, MI_ORIGINAL_TYPE,
                              FALSE, 0.0, 0.0, TRUE, &tmp_volume,
                              (minc_input_options *)NULL, &volume_input);
  ...
  if (status == OK) {
    ...
    *dim_names = (const char **)malloc(sizeof(char *) * *n_dims);
    ...
    for_less(i, 0, *n_dims) {
      (*dim_names)[i] = strdup(volume_input.minc_file->dim_names[i]);
    }
...
VIOAPI STRING *create_output_dim_names(Volume volume, STRING original_filename,
  ...
  if (options != NULL && strlen(options->dimension_names[0]) > 0) {
    for_less(i, 0, n_dims) dim_names[i] = strdup(options->dimension_names[i]);
  } else {
    if (original_filename != NULL && file_exists(original_filename) &&
      ...
      if (dim_index != n_dims) {
        for_less(i, 0, dim_index) free((char *)dim_names[i]);

        for_less(i, 0, n_dims) dim_names[i] = strdup(vol_dimension_names[i]);
      }

      /*--- free up the file dimension names */

      for_less(i, 0, n_file_dims) free((char *)file_dim_names[i]);
  ...
  if (n_found != n_dims) {
    print_error("create_output_dim_names: dimension name mismatch.\n");
...
VIOAPI Status copy_volume_auxiliary_and_history(Minc_file minc_file,
                                                STRING filename,
  ...
  if (copy_original_file_data) {
    status = copy_auxiliary_data_from_minc_file(minc_file, original_filename,
                                                history);
...
VIOAPI Status output_modified_volume(STRING filename, nc_type file_nc_data_type,
                                     BOOLEAN file_signed_flag,
                                     Real file_voxel_min, Real file_voxel_max,
                                     Volume volume, STRING original_filename,
                                     STRING history,
                                     minc_output_options *options) {
  Status status;
  Minc_file minc_file;
  ...
  dim_names =
      create_output_dim_names(volume, original_filename, options, sizes);
  ...
  minc_file = initialize_minc_output(
      filename, n_dims, dim_names, sizes, file_nc_data_type, file_signed_flag,
      file_voxel_min, file_voxel_max, get_voxel_to_world_transform(volume),
      volume, &used_options);
...
VIOAPI Status output_volume(STRING filename, nc_type file_nc_data_type,
                            BOOLEAN file_signed_flag, Real file_voxel_min,
                            Real file_voxel_max, Volume volume, STRING history,
                            minc_output_options *options) {
  return (output_modified_volume(filename, file_nc_data_type, file_signed_flag,
                                 file_voxel_min, file_voxel_max, volume, NULL,
                                 history, options));
}
