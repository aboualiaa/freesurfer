...
char *format_time( // free() the result
    const char *format, double seconds);
...
void initialize_progress_report(VIO_progress_struct *progress,
                                bool one_line_only, int n_steps,
                                const char *title);

void update_progress_report(VIO_progress_struct *progress, int current_step);

void terminate_progress_report(VIO_progress_struct *progress);
...
int miget_datatype(int cdfid, int imgid, nc_type *datatype, int *is_signed);
...
long *mitranslate_coords(int cdfid, int invar, long incoords[], int outvar,
                         long outcoords[]);

int mivarget1(int cdfid, int varid, long mindex[], nc_type datatype, char *sign,
              void *value);
int miattget_with_sign(int cdfid, int varid, char *name, char *insign,
                       nc_type datatype, char *outsign, int max_length,
                       void *value, int *att_length);
int miattget(int cdfid, int varid, char *name, nc_type datatype, int max_length,
             void *value, int *att_length);
int miattget1(int cdfid, int varid, char *name, nc_type datatype, void *value);
int mivarget(int cdfid, int varid, long start[], long count[], nc_type datatype,
             char *sign, void *values);
char *miattgetstr(int cdfid, int varid, char *name, int maxlen, char *value);
...
void create_empty_multidim_array(VIO_multidim_array *array, int n_dimensions,
                                 VIO_Data_types data_type);

void create_multidim_array(VIO_multidim_array *array, int n_dimensions,
                           int sizes[], VIO_Data_types data_type);

void copy_multidim_data_reordered(int type_size, void *void_dest_ptr,
                                  int n_dest_dims, int dest_sizes[],
                                  void *void_src_ptr, int n_src_dims,
                                  int src_sizes[], int counts[],
                                  int to_dest_index[], bool use_src_order);

int get_multidim_n_dimensions(VIO_multidim_array *array);

void get_multidim_sizes(VIO_multidim_array *array, int sizes[]);

void delete_multidim_array(VIO_multidim_array *array);

void alloc_multidim_array(VIO_multidim_array *array);

bool multidim_array_is_alloced(VIO_multidim_array *array);

void set_multidim_data_type(VIO_multidim_array *array,
                            VIO_Data_types data_type);

VIO_Data_types get_multidim_data_type(VIO_multidim_array *array);

int get_type_size(VIO_Data_types type);

void set_multidim_sizes(VIO_multidim_array *array, int sizes[]);
...
void set_default_minc_output_options(minc_output_options *options);

Minc_file initialize_minc_output(const char *filename, int n_dimensions,
                                 const char *dim_names[], int sizes[],
                                 nc_type file_nc_data_type,
                                 bool file_signed_flag, double file_voxel_min,
                                 double file_voxel_max,
                                 General_transform *voxel_to_world_transform,
                                 Volume volume_to_attach,
                                 minc_output_options *options);

VIO_Status copy_auxiliary_data_from_minc_file(Minc_file file,
                                              const char *filename,
                                              const char *history_string);

VIO_Status add_minc_history(Minc_file file, const char *history_string);

void set_minc_output_real_range(minc_output_options *options, double real_min,
                                double real_max);

void set_minc_output_use_volume_starts_and_steps_flag(
    minc_output_options *options, bool flag);

VIO_Status output_minc_volume(Minc_file file);

VIO_Status close_minc_output(Minc_file file);

// The following are in minc_files.c
//
bool file_exists(const char *expandedFilename);

char *file_exists_as_compressed( // returns the pathname of the compressed file,
                                 // use free() to delete
    const char *expandedFilename);

bool filename_extension_matches(const char *expanded_filename_possibly_with_z,
                                const char *extension);

// The following are in minc_input_free.c
//
VIO_Status initialize_free_format_input(const char *filename, Volume volume,
                                        volume_input_struct *volume_input);

void delete_free_format_input(volume_input_struct *volume_input);

bool input_more_free_format_file(Volume volume,
                                 volume_input_struct *volume_input,
                                 double *fraction_done);

// The following are in minc_input_mnc.c
//
Minc_file initialize_minc_input(const char *filename, Volume volume,
                                minc_input_options *options);

int get_minc_file_n_dimensions(const char *filename);

VIO_Status close_minc_input(Minc_file file);

bool input_more_minc_file(Minc_file file, double *fraction_done);
...
int MI_convert_type(long number_of_values, nc_type intype, int insign,
                    void *invalues, nc_type outtype, int outsign,
                    void *outvalues, mi_icv_type *icvp);

int MI_var_loop(int ndims, long start[], long count[], int value_size,
                int *bufsize_step, long max_buffer_size, void *caller_data,
                int (*action_func)(int, long[], long[], long, void *, void *));

int MI_varaccess(int operation, int cdfid, int varid, long start[],
                 long count[], nc_type datatype, int sign, void *values,
                 int *bufsize_step, mi_icv_type *icvp);

typedef double Double4x4[4 * 4];
#define Index4x4(I, J) (4 * (I) + (J))
bool invert_4x4_matrix(const Double4x4 *mat, // doubles are read, not changed
