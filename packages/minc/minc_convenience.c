/*
 * Original Author: (assumed to be) Peter Neelin, modified to compile within
 * freesurfer/utils by Bevin Brett CVS Revision Info: $Author: Brett $ $Date:
 * 2017/11/17 12:46:00 $ $Revision: 1.00 $
 *
 ...
 * Increase att_length by one in miappend_history() in order to read the entire
attribute
 *
...
// /private-cvsroot/minc/libsrc/minc_convenience.c,v 6.12.2.1 2004/09/28 20:23:40
// bert Exp $ MINC (MNI)";
#endif
...
#define MI_LOG_PKG_ERROR3(CODE, FMT, P1)                                       \
  {                                                                            \
...
MNCAPI int miget_image_range(int cdfid, double image_range[]) {
  ...
  if ((vid[0] == MI_ERROR) || (vid[1] == MI_ERROR)) {
    ...
    if (is_float) {
      if (miget_valid_range(cdfid, imgid, image_range) == MI_ERROR)
        MI_RETURN(MI_ERROR);
      no_range_found = (datatype == NC_FLOAT && image_range[1] == FLT_MAX) ||
  ...
  /* If the variables are there then get the max and min and fastest
    ...
    for (imm = 0; imm < 2; imm++) {
      ...
      /* Get values */
      if (mivarget(cdfid, vid[imm], miset_coords(ndims, 0L, start), count,
                   NC_DOUBLE, NULL, buffer) == MI_ERROR) {
        FREE(buffer);
        MI_RETURN_ERROR(MI_ERROR);
      }
...
@MODIFIED   :
---------------------------------------------------------------------------- */
MNCAPI int miget_datatype(int cdfid, int imgid, nc_type *datatype,
                          int *is_signed) {
  int old_ncopts;
  ...
  if ((miattgetstr(cdfid, imgid, MIsigntype, MI_MAX_ATTSTR_LEN, attstr) !=
       NULL)) {
...
MNCAPI int miget_valid_range(int cdfid, int imgid, double valid_range[]) {
  ...
  /* Get valid range */
  status = miattget_with_sign(cdfid, imgid, MIvalid_range, att_sign, NC_DOUBLE,
                              NULL, 2, valid_range, &length);
  ...
  if ((status == MI_ERROR) || (length != 2)) {
    ...
    (void)miattget_with_sign(cdfid, imgid, MIvalid_max, att_sign, NC_DOUBLE,
                             NULL, 1, &valid_range[1], NULL);

    /* Try to read the valid min */
    (void)miattget_with_sign(cdfid, imgid, MIvalid_min, att_sign, NC_DOUBLE,
                             NULL, 1, &valid_range[0], NULL);
  }
...
PRIVATE int MI_create_dim_variable(int cdfid, const char *name,
  ...
  if (STRINGS_EQUAL(name, MIvector_dimension)) {
    MI_LOG_PKG_ERROR3(MI_ERR_BAD_STDVAR, "%s is not a standard MINC variable",
                      name);
    MI_RETURN_ERROR(MI_ERROR);
  }
...
@MODIFIED   :
---------------------------------------------------------------------------- */
PRIVATE int MI_verify_maxmin_dims(int cdfid, int image_ndims, int image_dim[],
...
MNCAPI int miadd_child(int cdfid, int parent_varid, int child_varid) {
  ...
  ncopts = 0;
  status =
      ncattinq(cdfid, parent_varid, MIchildren, &datatype, &child_list_size);
  ncopts = oldncopts;
  ...
  if ((child_list =
           MALLOC(child_list_size + MAX_NC_NAME + strlen(MI_CHILD_SEPARATOR),
                  char)) == NULL) {
  ...
  if (ncvarinq(cdfid, child_varid, new_child, NULL, NULL, NULL, NULL) ==
      MI_ERROR) {
    FREE(child_list);
    MI_RETURN_ERROR(MI_ERROR);
  }
...
@MODIFIED   :
---------------------------------------------------------------------------- */
MNCAPI int miattput_pointer(int cdfid, int varid, const char *name,
                            int ptrvarid) {
  ...
  MI_CHK_ERR(ncvarinq(cdfid, ptrvarid, &(pointer_string[index]), NULL, NULL,
                      NULL, NULL))
  ...
  MI_CHK_ERR(ncvarinq(cdfid, varid, pointer_string, NULL, NULL, NULL, NULL))
...
@MODIFIED   :
---------------------------------------------------------------------------- */
PRIVATE int MI_create_image_variable(int cdfid, const char *name,
  ...
  if (max_varid != MI_ERROR) {
    ...
    MI_CHK_ERR(
        ncvarinq(cdfid, max_varid, NULL, NULL, &maxmin_ndims, maxmin_dim, NULL))
    MI_CHK_ERR(
        MI_verify_maxmin_dims(cdfid, ndims, dim, maxmin_ndims, maxmin_dim))
  }
  if (min_varid != MI_ERROR) {
    /* Get MIimagemin dimensions */
    MI_CHK_ERR(
        ncvarinq(cdfid, min_varid, NULL, NULL, &maxmin_ndims, maxmin_dim, NULL))
    MI_CHK_ERR(
        MI_verify_maxmin_dims(cdfid, ndims, dim, maxmin_ndims, maxmin_dim))
  }
...
@MODIFIED   :
---------------------------------------------------------------------------- */
PRIVATE int MI_create_imaxmin_variable(int cdfid, const char *name,
                                       nc_type datatype, int ndims, int dim[]) {
  int varid;                   /* Created variable id */
  ...
  if (image_varid != MI_ERROR) {
    ...
    MI_CHK_ERR(
        ncvarinq(cdfid, image_varid, NULL, NULL, &image_ndims, image_dim, NULL))
    MI_CHK_ERR(MI_verify_maxmin_dims(cdfid, image_ndims, image_dim, ndims, dim))
  }
...
MNCAPI int micreate_std_variable(int cdfid, const char *name, nc_type datatype,
  ...
  /* Check for a dimension width */
    ...
    MI_CHK_ERR(varid =
                   MI_create_dimwidth_variable(cdfid, name, datatype, ndims))
  }
  ...
  /* Check for a standard variable or group */
    ...
    if (STRINGS_EQUAL(name, MIimage))
      MI_CHK_ERR(
          varid = MI_create_image_variable(cdfid, name, datatype, ndims, dim))
    else if ((STRINGS_EQUAL(name, MIimagemax)) ||
             (STRINGS_EQUAL(name, MIimagemin)))
      MI_CHK_ERR(
          varid = MI_create_imaxmin_variable(cdfid, name, datatype, ndims, dim))
...
/* Private functions */
PRIVATE int MI_create_dim_variable(int cdfid, char *name, nc_type datatype,
                                   int ndims);
PRIVATE int MI_create_dimwidth_variable(int cdfid, char *name, nc_type datatype,
                                        int ndims);
...
MNCAPI int miattget_pointer(int cdfid, int varid, char *name) {
  ...
  /* Get the attribute */
  if (miattgetstr(cdfid, varid, name, sizeof(pointer_string), pointer_string) ==
      NULL)
    MI_RETURN_ERROR(MI_ERROR);
  ...
  for (index = 0; prefix_string[index] != '\0'; index++) {
    if (pointer_string[index] != prefix_string[index]) {
      MI_LOG_PKG_ERROR3(MI_ERR_NOTPOINTER,
                        "Attribute %s is not a pointer to a variable", name);
      MI_RETURN_ERROR(MI_ERROR);
    }
  }
...
@MODIFIED   :
---------------------------------------------------------------------------- */
const char *miget_version(void) { return (VERSION); }
