 ...
 * by Bevin Brett CVS Revision Info: $Author: Brett $ $Date: 2017/11/08 12:46:00
 * $ $Revision: 1.00 $
 *
...
static int string_length(const char *src) { return strlen(src); }

static char *create_string(const char *src) { return strdup(src); }

static void delete_string(const char *str) { free((void *)str); }

static void replace_string(const char **dst, const char *src) {
  const char *copy = create_string(src);
  delete_string(*dst);
  *dst = copy;
}
...
void set_default_minc_input_options(minc_input_options *options);

static BOOLEAN match_dimension_names(int n_volume_dims,
...
VIOAPI int get_minc_file_n_dimensions(const char *filename) {
  ...
  ncvarinq(cdfid, img_var, (char *)NULL, &file_datatype, &n_dims, dim_vars,
           (int *)NULL);
...
static BOOLEAN match_dimension_names(int n_volume_dims,
  ...
  for_less(i, 0, n_file_dims) to_volume_index[i] = INVALID_AXIS;
          ...
          if (to_volume_index[j] == INVALID_AXIS) {
            ...
            case 1:
              match = equal_strings(volume_dimension_names[i],
                      ...
                      convert_dim_name_to_spatial_axis(file_dimension_names[j],
                                                       &dummy);
              break;
            case 2:
...
VIOAPI Minc_file initialize_minc_input_from_minc_id(
    int minc_id, Volume volume, minc_input_options *options) {
  ...
  if (!match_dimension_names(get_volume_n_dimensions(volume),
                             volume->dimension_names, file->n_file_dimensions,
                             file->dim_names, file->to_volume_index)) {
  ...
  for_less(d, 0, file->n_file_dimensions) {
    ...
    if (dimvar != MI_ERROR) {
      ...
      if (spatial_dim_flags[d]) {
        ...
        if (miattget(file->cdfid, dimvar, MIdirection_cosines, NC_DOUBLE,
                     MI_NUM_SPACE_DIMS, (void *)tmp_cosines,
                     (int *)NULL) != MI_ERROR) {
         ...
         * only.
         */
        miattgetstr(file->cdfid, dimvar, MIspacing, MI_MAX_ATTSTR_LEN + 1,
                    spacing_type);
        if (!strcmp(spacing_type, MI_IRREGULAR)) {
          ...
          mivarget(file->cdfid, dimvar, start, count, NC_DOUBLE, MI_SIGNED,
                   irr_starts[d]);
    ...
    if (file->to_volume_index[d] == INVALID_AXIS) {
      ...
      volume_separations[file->to_volume_index[d]] = file_separations[d];

      if (file->to_volume_index[d] != INVALID_AXIS) {
        volume_starts[file->to_volume_index[d]] = start_position[d];
        set_volume_direction_unit_cosine(volume, file->to_volume_index[d],
                                         dir_cosines[d]);
      }
    }
  }
  ...
  compute_world_transform(file->spatial_axes, file_separations, dir_cosines,
                          start_position, &file->voxel_to_world_transform);
  ...
  if (file->converting_to_colour) {
    ...
    {
      if (miattgetstr(file->cdfid, file->img_var, MIsigntype, MI_MAX_ATTSTR_LEN,
                      signed_flag) != NULL) {
  ...
  if (file->converting_to_colour) {
    ...
    if (miget_valid_range(file->cdfid, file->img_var, valid_range) ==
        MI_ERROR) {
  ...
  if (options->promote_invalid_to_zero_flag) {
    ...
    if (!file->converting_to_colour) {
      if (real_min == real_max)
        ...
        voxel_zero = valid_range[0] + (valid_range[1] - valid_range[0]) *
                                          (0.0 - real_min) /
                                          (real_max - real_min);
      }
  ...
  for_less(d, 0, file->n_file_dimensions) file->indices[d] = 0;
...
VIOAPI Minc_file initialize_minc_input(STRING filename, Volume volume,
...
VIOAPI VIO_Status close_minc_input(Minc_file file) {
  int d;
  ...
  for_less(d, 0, file->n_file_dimensions) delete_string(file->dim_names[d]);
...
VIOAPI void
set_minc_input_promote_invalid_to_zero_flag(minc_input_options *options,
                                            BOOLEAN flag) {
...
VIOAPI void set_minc_input_vector_to_scalar_flag(minc_input_options *options,
                                                 BOOLEAN flag) {
...
VIOAPI void set_minc_input_vector_to_colour_flag(minc_input_options *options,
                                                 BOOLEAN flag) {
...
VIOAPI void set_minc_input_colour_dimension_size(minc_input_options *options,
                                                 int size) {
  if (size > 0)
...
VIOAPI void
set_minc_input_colour_max_dimension_size(minc_input_options *options,
                                         int size) {
  if (size > 0)
...
VIOAPI void set_minc_input_colour_indices(minc_input_options *options,
                                          int indices[4]) {
  int i;

  for_less(i, 0, 4) options->rgba_indices[i] = indices[i];
}
...
VIOAPI void set_minc_input_user_real_range(minc_input_options *options,
...
VIOAPI void set_default_minc_input_options(minc_input_options *options) {
...
VIOAPI Status input_minc_hyperslab(Minc_file file, Data_types data_type,
  ...
  for_less(ind, 0, n_array_dims) vol1_indices[ind] = -1;
  ...
  if (miicv_get(file->minc_icv, used_start, used_count, void_ptr) == MI_ERROR) {
    status = ERROR;
    if (file->converting_to_colour)
  ...
  if (status == OK && (!direct_to_array || file->converting_to_colour)) {
    if (file->converting_to_colour) {
      for_less(dim, n_tmp_dims, MAX_DIMENSIONS) tmp_sizes[dim] = 1;
      ...
      for_less(v[4], 0, size4) for_less(v[3], 0, size3) for_less(v[2], 0, size2)
          for_less(v[1], 0, size1) for_less(v[0], 0, size0) {
        for_less(d, 0, n_tmp_dims) voxel[d] = v[d];
        ...
        for_less(i, 0, 4) {
          if (file->rgba_indices[i] < 0) {
            ...
            GET_MULTIDIM(rgb[i], (Real), rgb_array, voxel[0], voxel[1],
                         voxel[2], voxel[3], voxel[4]);
          }
        }

        colour = make_rgba_Colour_0_1(rgb[0], rgb[1], rgb[2], rgb[3]);
        SET_MULTIDIM(buffer_array, voxel[0], voxel[1], voxel[2], voxel[3],
                     voxel[4], colour);
      }
    ...
    GET_MULTIDIM_PTR(void_ptr, buffer_array, 0, 0, 0, 0, 0);
    copy_multidim_data_reordered(
        get_type_size(data_type), array_data_ptr, n_array_dims, array_sizes,
        void_ptr, n_tmp_dims, tmp_sizes, tmp_sizes, vol1_indices, FALSE);
...
static void input_slab(Minc_file file, Volume volume, int to_volume[],
  ...
  (void)input_minc_hyperslab(file, get_multidim_data_type(&volume->array),
                             get_multidim_n_dimensions(&volume->array),
                             array_sizes, array_data_ptr, to_volume, file_start,
                             file_count);
}
...
VIOAPI BOOLEAN input_more_minc_file(Minc_file file, Real *fraction_done) {
  ...
  if (!volume_is_alloced(volume)) {
    ...
    if (volume->is_cached_volume) {
      open_cache_volume_input_file(&volume->cache, volume, file->filename,
...
#endif
  {
    ...
    for_less(ind, 0, file->n_file_dimensions) count[ind] = 1;
...
// Suppress everything else until proven needed
//
#ifdef BEVIN_UNSUPPRESS

#ifndef lint
static char rcsid[] =
    "$Header: /private-cvsroot/minc/volume_io/Volumes/input_mnc.c,v 1.65.2.2 "
    "2005/07/04 12:43:35 bert Exp $";
#endif
...
VIOAPI int get_n_input_volumes(Minc_file file) {
...
VIOAPI BOOLEAN advance_input_volume(Minc_file file) {
  int ind, c, axis;
  ...
  if (ind >= 0) {
    ...
    general_transform_point(&file->voxel_to_world_transform, voxel[0], voxel[1],
                            voxel[2], &world_space[X], &world_space[Y],
                            &world_space[Z]);

    for_less(c, 0, get_volume_n_dimensions(file->volume)) voxel[c] = 0.0;

    convert_voxel_to_world(file->volume, voxel, &vol_world_space[X],
                           &vol_world_space[Y], &vol_world_space[Z]);

    make_identity_transform(&offset);
    for_less(c, 0, N_DIMENSIONS) Transform_elem(offset, c, 3) =
        world_space[c] - vol_world_space[c];
...
VIOAPI void reset_input_volume(Minc_file file) {
  int d;
...
VIOAPI int get_minc_file_id(Minc_file file) { return (file->cdfid); }
...
VIOAPI void
set_minc_input_promote_invalid_to_min_flag(minc_input_options *options,
                                           BOOLEAN flag) {
