...
// /private-cvsroot/minc/volume_io/Prog_utils/progress.c,v 1.9.2.1 2004/10/04
// 20:19:22 bert Exp $";
#endif
...
static void show_one_line_progress(progress_struct *progress, int current_step);

static void show_multi_line_progress(progress_struct *progress,
                                     int current_step, double time_so_far,
...
VIOAPI void initialize_progress_report(VIO_progress_struct *progress,
...
VIOAPI void update_progress_report(VIO_progress_struct *progress,
                                   int current_step) {
  ...
  progress->sum_xy =
      RATIO_FOR_LINEAR * progress->sum_xy + (double)current_step * time_so_far;
  ...
  if (time_so_far > FIRST_MESSAGE_THRESHOLD) {
    ...
    if (est_total_time <= time_so_far) {
      est_total_time =
          time_so_far * (double)progress->n_steps / (double)current_step;
    }
    ...
    if (progress->force_one_line ||
      ...
      if (current_time - progress->previous_time >= progress->update_rate) {
        ...
        progress->update_rate =
            (current_time - progress->start_time) * UPDATE_RATE_FACTOR;
...
static void show_one_line_progress(progress_struct *progress,
                                   int current_step) {
  int i, n_dots;
...
static void show_multi_line_progress(progress_struct *progress,
                                     ...
                                     double est_total_time) {
  int percent_done =
      round(100.0 * (double)current_step / (double)progress->n_steps);
...
VIOAPI void terminate_progress_report(VIO_progress_struct *progress) {
  double total_time;
