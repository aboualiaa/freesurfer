...
static int append_to_data_b64(gxml_data *, char *, long long, const char *,
                              int);
/*
...
static int decode_ascii(gxml_data *, char *, int, int, void *, long long *,
                        int *);
...
int gxml_write_image(gifti_image *gim, const char *fname, int write_data) {
  ...
  if (GXD.verb > 1) {
    ...
    if (write_data)
      fprintf(stderr, " (%d DA elements = %lld MB)", gim->numDA,
              gifti_gim_DA_size(gim, 1));
    fputc('\n', stderr);
  }
...
static int init_gxml_data(gxml_data *dp, int doall, const int *dalist,
                          int len) {
  int errs = 0;
...
static int push_dspace(gxml_data *xd) {
  ...
  if (CSind < 0) {
    fprintf(stderr, "** PD: bad numCS %d in darray %d, skipping...", CSind + 1,
            xd->gim->numDA - 1);
    xd->skip = xd->depth;
    return 1;
  }
...
static int push_xspace(gxml_data *xd) {
  int CSind = xd->gim->darray[xd->gim->numDA - 1]->numCS - 1;
  if (CSind < 0) {
    fprintf(stderr, "** PX: bad numCS %d in darray %d, skipping...", CSind + 1,
            xd->gim->numDA - 1);
    xd->skip = xd->depth;
    return 1;
  }
...
static int append_to_data(gxml_data *xd, const char *cdata, int len) {
  ...
  case GIFTI_ENCODING_B64BIN:
    return append_to_data_b64(xd, (char *)da->data, da->nvals * da->nbyper,
                              cdata, len);
...
static int append_to_data_b64(gxml_data *xd, char *dest, long long tot_bytes,
  ...
  while (rem_bytes_in > 0) {
    ...
    xd->doff = decode_b64(xd, xd->ddata,        /* data source */
...
static int append_to_data_ascii(gxml_data *xd, const char *cdata, int len) {
  ...
  while (rem_len > 0) {
    ...
    xd->doff = decode_ascii(xd, xd->ddata,       /* data source */
    ...
    if (xd->doff > 0) {
      if (xd->verb > 5)
        fprintf(stderr, "++ A2D: move %d bytes from %d (blen %d)\n", xd->doff,
                xd->dlen - unused - xd->doff, xd->dlen);
      /* (subtract unused+1, since 1 byte is saved for null */
      memmove(xd->ddata, xd->ddata + xd->dlen - (unused + 1) - xd->doff,
              xd->doff);
      if (xd->verb > 6)
...
static int append_to_xform(gxml_data *xd, const char *cdata, int len) {
  ...
  if (!da || !xd->xlen || !xd->xdata || xd->dind < 0 || da->numCS <= 0) {
    fprintf(stderr, "** A2X: bad setup (%p,%d,%p,%lld,%d)\n", (void *)da,
            xd->xlen, (void *)xd->xdata, xd->dind, da->numCS);
    return 1;
  } else if (xd->verb > 4)
  ...
  while (rem_len > 0) {
    ...
    xd->doff = decode_ascii(xd, xd->xdata,                    /* data source */
                            xd->doff + copy_len,              /* data length */
                            type,                             /* data type */
                            dptr,      /* starting destination */
                            &rem_vals, /* nvals to read */
...
*/
static int decode_b64(gxml_data *xd, char *cdata, int cdlen, char *dptr,
                      long long *needed) {
...
static void XMLCALL cb_instr(void *udata, const char *target,
                             const char *data) {
  gxml_data *xd = (gxml_data *)udata;
  if (xd->verb > 3) {
    show_depth(xd->depth, 1, stderr);
...
static void XMLCALL cb_xml_dec(void *udata, const char *ver, const char *enc,
                               int standalone) {
  gxml_data *xd = (gxml_data *)udata;
  if (xd->verb > 2) {
    show_depth(xd->depth, 1, stderr);
...
static void XMLCALL cb_start_doctype(void *udata, const char *doctype,
  ...
  if (xd->verb > 2) {
    ...
    fprintf(stderr, "start_doctype, dt='%s', sid='%s',pid='%s', sub=%d\n",
            doctype, sysid ? sysid : "NULL", pubid ? pubid : "NULL",
            has_subset);
  }
}
...
static void XMLCALL cb_elem_dec(void *udata, const char *ename,
  ...
  if (xd->verb > 2) {
    ...
    fprintf(stderr, "%s: type=%d, quant=%d, name=%s, numc=%d, cp=%p\n", ename,
            content->type, content->quant, content->name, content->numchildren,
...
static int stack_is_valid(gxml_data *xd) {
  ...
  case GXML_ETYPE_MATRIXDATA:
    ...
    break;
  case GXML_ETYPE_CDATA:
    if (parent != GXML_ETYPE_NAME && parent != GXML_ETYPE_VALUE &&
        parent != GXML_ETYPE_LABEL && parent != GXML_ETYPE_DATASPACE &&
        parent != GXML_ETYPE_XFORMSPACE && parent != GXML_ETYPE_MATRIXDATA)
      bad_parent = 1;
    break;
  }
...
static int ewrite_darray(gxml_data *xd, giiDataArray *da, FILE *fp) {
  ...
  ewrite_str_attr("Encoding",
                  ...
                  offset, 0, fp);
  ewrite_str_attr(
      "Endian", /* set endian to that of this CPU */
...
static int ewrite_data(gxml_data *xd, giiDataArray *da, FILE *fp) {
  ...
  if (xd->dstore) {
    if (da->encoding == GIFTI_ENCODING_ASCII) {
      ...
      for (c = 0; c < rows; c++)
        ewrite_data_line(da->data, da->datatype, c, cols, spaces + xd->indent,
                         fp);
      fprintf(fp, "%*s", spaces, "");
      ...
      rv = compress2((Bytef *)xd->zdata, &blen, (Bytef *)da->data,
...
static int ewrite_data_line(void *data, int type, long long row, long long cols,
                            int space, FILE *fp) {
  int c;
...
static int ewrite_text_ele(int ele, const char *cdata, const char *attr,
  ...
  fprintf(fp, "%*s<%s%s>%s%s%s</%s>\n", spaces, "", enames[index],
          attr ? attr : "", in_CDATA ? "<![CDATA[" : "", cdata ? cdata : "",
          in_CDATA ? "]]>" : "", enames[index]);
...
static int ewrite_LT(gxml_data *xd, giiLabelTable *lt, int in_CDATA, FILE *fp) {
  ...
  for (c = 0; c < lt->length; c++) {
    ...
    ewrite_text_ele(GXML_ETYPE_LABEL, lt->label[c], attr, spaces + xd->indent,
                    in_CDATA, fp);
  }
  fprintf(fp, "%*s</LabelTable>\n", spaces, "");
...
static int ewrite_ex_atrs(gxml_data *xd, nvpairs *nvp, int offset, int first,
                          FILE *fp) {
...
static int ewrite_int_attr(const char *name, int value, int spaces, int first,
                           FILE *fp) {
...
static int disp_gxml_data(char *mesg, gxml_data *dp, int show_all) {
  ...
  if (show_all)
    fprintf(stderr,
            ...
            dp->b64_errors, dp->errors, dp->skip, dp->depth, dp->dind, dp->clen,
            dp->doff, dp->zlen, (void *)dp->cdata, (void *)dp->xdata,
            (void *)dp->ddata, (void *)dp->zdata, (void *)dp->gim);
