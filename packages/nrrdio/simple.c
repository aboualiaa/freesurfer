...
*/
int nrrdOriginCalculate(const Nrrd *nrrd, unsigned int *axisIdx,
...
#define ERROR                                                                  \
  ...
  if (!(nrrd &&
        (nrrdCenterCell == defaultCenter || nrrdCenterNode == defaultCenter) &&
        origin)) {
    ERROR;
  ...
  for (ai = 0; ai < axisIdxNum; ai++) {
    gotMaxOrSpacing &=
        (AIR_EXISTS(axis[ai]->max) || AIR_EXISTS(axis[ai]->spacing));
  }
  if (!gotMaxOrSpacing) {
    ERROR;
  ...
  for (ai = 0; ai < axisIdxNum; ai++) {
    size = axis[ai]->size;
    min = axis[ai]->min;
    center = (nrrdCenterUnknown != axis[ai]->center ? axis[ai]->center
                                                    : defaultCenter);
...
void _nrrdSpaceVecScaleAdd2(double sum[NRRD_SPACE_DIM_MAX], double sclA,
...
void _nrrdSpaceVecScale(double out[NRRD_SPACE_DIM_MAX], double scl,
...
double _nrrdSpaceVecNorm(int sdim, const double vec[NRRD_SPACE_DIM_MAX]) {
  int di;
  double nn;
...
void _nrrdSpaceVecSetNaN(double vec[NRRD_SPACE_DIM_MAX]) {
  int di;
...
int _nrrdContentSet_nva(Nrrd *nout, const char *func, char *content,
...
int _nrrdContentSet_va(Nrrd *nout, const char *func, char *content,
                       const char *format, ...) {
...
*/
int nrrdContentSet_va(Nrrd *nout, const char *func, const Nrrd *nin,
                      const char *format, ...) {
...
void nrrdDescribe(FILE *file, const Nrrd *nrrd) {
  ...
  if (file && nrrd) {
    ...
    fprintf(file, "Data at 0x%p is " _AIR_SIZE_T_CNV " elements of type %s.\n",
...
int _nrrdFieldCheckSpaceInfo(const Nrrd *nrrd, int useBiff) {
  ...
  if (!(nrrd->spaceDim <= NRRD_SPACE_DIM_MAX)) {
    sprintf(err,
            "%s: space dimension %d is outside valid range "
  ...
  if (nrrd->spaceDim) {
    if (nrrd->space) {
      if (nrrdSpaceDimension(nrrd->space) != nrrd->spaceDim) {
        sprintf(err, "%s: space %s has dimension %d but spaceDim is %d", me,
                airEnumStr(nrrdSpace, nrrd->space),
    ...
    for (dd = 0; dd < nrrd->spaceDim; dd++) {
      for (ii = 0; ii < nrrd->spaceDim; ii++) {
        if (exists ^ AIR_EXISTS(nrrd->measurementFrame[dd][ii])) {
          sprintf(err,
                  "%s: existance of measurement frame coefficients must "
    ...
    if (nrrd->space) {
      sprintf(err, "%s: space %s can't be set with spaceDim %d", me,
...
int _nrrdFieldCheck_block_size(const Nrrd *nrrd, int useBiff) {
  ...
  if (nrrdTypeBlock == nrrd->type && (!(0 < nrrd->blockSize))) {
    sprintf(err,
            "%s: type is %s but nrrd->blockSize (" _AIR_SIZE_T_CNV ") invalid",
            me, airEnumStr(nrrdType, nrrdTypeBlock), nrrd->blockSize);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
  if (nrrdTypeBlock != nrrd->type && (0 < nrrd->blockSize)) {
    sprintf(err, "%s: type is %s (not block) but blockSize is " _AIR_SIZE_T_CNV,
            me, airEnumStr(nrrdType, nrrd->type), nrrd->blockSize);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
  return 0;
...
int _nrrdFieldCheck_dimension(const Nrrd *nrrd, int useBiff) {
  ...
  if (!AIR_IN_CL(1, nrrd->dim, NRRD_DIM_MAX)) {
    sprintf(err, "%s: dimension %u is outside valid range [1,%d]", me,
...
int _nrrdFieldCheck_axis_mins(const Nrrd *nrrd, int useBiff) {
  ...
  for (ai = 0; ai < nrrd->dim; ai++) {
    if ((ret = airIsInf_d(val[ai]))) {
      sprintf(err, "%s: axis %d min %sinf invalid", me, ai,
              1 == ret ? "+" : "-");
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
...
int _nrrdFieldCheck_axis_maxs(const Nrrd *nrrd, int useBiff) {
  ...
  for (ai = 0; ai < nrrd->dim; ai++) {
    if ((ret = airIsInf_d(val[ai]))) {
      sprintf(err, "%s: axis %d max %sinf invalid", me, ai,
              1 == ret ? "+" : "-");
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
...
int _nrrdFieldCheck_kinds(const Nrrd *nrrd, int useBiff) {
  ...
  for (ai = 0; ai < nrrd->dim; ai++) {
    ...
    if (wantLen && wantLen != nrrd->axis[ai].size) {
      sprintf(err,
              "%s: axis %d kind %s requires size %d, but have " _AIR_SIZE_T_CNV,
...
int _nrrdCheck(const Nrrd *nrrd, int checkData, int useBiff) {
  ...
  for (fi = nrrdField_unknown + 1; fi < nrrdField_last; fi++) {
    ...
    if (_nrrdFieldCheck[fi](nrrd, AIR_TRUE)) {
      sprintf(err, "%s: trouble with %s field", me, airEnumStr(nrrdField, fi));
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
  }
...
int nrrdSameSize(const Nrrd *n1, const Nrrd *n2, int useBiff) {
  ...
  for (ai = 0; ai < n1->dim; ai++) {
    if (n1->axis[ai].size != n2->axis[ai].size) {
      sprintf(err,
              "%s: n1->axis[%d].size (" _AIR_SIZE_T_CNV
...
void _nrrdSplitSizes(size_t *pieceSize, size_t *pieceNum, Nrrd *nrrd,
                     unsigned int split) {
  unsigned int ai;
  size_t size[NRRD_DIM_MAX];
...
int _nrrdCheckEnums(void) {
  char me[] = "_nrrdCheckEnums", err[BIFF_STRLEN], which[AIR_STRLEN_SMALL];
...
int nrrdSanity(void) {
  ...
  if (airEnumValCheck(nrrdCenter, nrrdDefaultCenter)) {
    sprintf(err, "%s: nrrdDefaultCenter (%d) not in valid range [%d,%d]", me,
  ...
  if (!(nrrdTypeSize[nrrdTypeChar] == sizeof(char) &&
    ...
    sprintf(err,
            ...
            "but got (%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)",
            me, (int)nrrdTypeSize[nrrdTypeChar],
            (int)nrrdTypeSize[nrrdTypeUChar], (int)nrrdTypeSize[nrrdTypeShort],
            (int)nrrdTypeSize[nrrdTypeUShort], (int)nrrdTypeSize[nrrdTypeInt],
            (int)nrrdTypeSize[nrrdTypeUInt], (int)nrrdTypeSize[nrrdTypeLLong],
            (int)nrrdTypeSize[nrrdTypeULLong], (int)nrrdTypeSize[nrrdTypeFloat],
            (int)nrrdTypeSize[nrrdTypeDouble], (int)sizeof(char),
            (int)sizeof(unsigned char), (int)sizeof(short),
            (int)sizeof(unsigned short), (int)sizeof(int),
            (int)sizeof(unsigned int), (int)sizeof(airLLong),
            (int)sizeof(airULLong), (int)sizeof(float), (int)sizeof(double));
    biffAdd(NRRD, err);
    return 0;
  }
  ...
  if (tmpLLI != NRRD_LLONG_MAX) {
    sprintf(err,
            "%s: long long int can't hold NRRD_LLONG_MAX (" AIR_ULLONG_FMT ")",
            me, NRRD_LLONG_MAX);
    biffAdd(NRRD, err);
    return 0;
  }
  ...
  if (NRRD_LLONG_MIN != tmpLLI) {
    sprintf(err,
            "%s: long long int min (" AIR_LLONG_FMT ") or max (" AIR_LLONG_FMT
            ") incorrect",
            me, NRRD_LLONG_MIN, NRRD_LLONG_MAX);
    biffAdd(NRRD, err);
    return 0;
  }
  ...
  if (tmpULLI != 0) {
    sprintf(err,
            "%s: unsigned long long int max (" AIR_ULLONG_FMT ") incorrect", me,
            NRRD_ULLONG_MAX);
    biffAdd(NRRD, err);
    return 0;
  }
  ...
  if (!(NRRD_DIM_MAX >= 3)) {
    sprintf(err, "%s: NRRD_DIM_MAX == %d seems awfully small, doesn't it?", me,
            NRRD_DIM_MAX);
    biffAdd(NRRD, err);
    return 0;
  }
