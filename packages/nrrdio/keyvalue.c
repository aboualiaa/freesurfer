...
*/
unsigned int nrrdKeyValueSize(const Nrrd *nrrd) {
...
** to AIR_FALSE
*/
void nrrdKeyValueIndex(const Nrrd *nrrd, char **keyP, char **valueP,
                       unsigned int ki) {
...
int _nrrdKeyValueIdxFind(const Nrrd *nrrd, const char *key) {
  unsigned int nk, ki;
...
void nrrdKeyValueClear(Nrrd *nrrd) {
  unsigned int nk, ki;
...
int nrrdKeyValueErase(Nrrd *nrrd, const char *key) {
  unsigned int nk;
  int ki;
...
*/
int nrrdKeyValueAdd(Nrrd *nrrd, const char *key, const char *value) {
  int ki;
...
** to AIR_FALSE
*/
char *nrrdKeyValueGet(const Nrrd *nrrd, const char *key) {
  char *ret;
  int ki;
...
void _nrrdWriteEscaped(FILE *file, char *dst, const char *str) {
  size_t ci, sl;
...
*/
int _nrrdKeyValueWrite(FILE *file, char **stringP, const char *prefix,
  ...
  if (stringP) {
    ...
    *stringP =
        (char *)malloc(airStrlen(prefix) + 2 * airStrlen(key) + strlen(":=") +
...
*/
int nrrdKeyValueCopy(Nrrd *nout, const Nrrd *nin) {
  char *key, *value;
  unsigned int ki;
