...
** provides.
*/
int nrrdInvertPerm(unsigned int *invp, const unsigned int *pp,
                   unsigned int nn) {
  ...
  for (ii = 0; ii < nn; ii++) {
    if (!(pp[ii] <= nn - 1)) {
      sprintf(err, "%s: permutation element #%d == %d out of bounds [0,%d]", me,
              ii, pp[ii], nn - 1);
      biffAdd(NRRD, err);
      return 1;
    }
  ...
  for (ii = 0; ii < nn; ii++) {
    if (1 != invp[ii]) {
      sprintf(err, "%s: element #%d mapped to %d times (should be once)", me,
              ii, invp[ii]);
      biffAdd(NRRD, err);
      problem = AIR_TRUE;
    }
...
int nrrdAxesInsert(Nrrd *nout, const Nrrd *nin, unsigned int axis) {
  ...
  if (!(axis <= nin->dim)) {
    sprintf(err, "%s: given axis (%d) outside valid range [0, %d]", me, axis,
            nin->dim);
    biffAdd(NRRD, err);
    return 1;
  }
  if (NRRD_DIM_MAX == nin->dim) {
    sprintf(err, "%s: given nrrd already at NRRD_DIM_MAX (%d)", me,
            NRRD_DIM_MAX);
    biffAdd(NRRD, err);
    return 1;
  }
...
int nrrdAxesPermute(Nrrd *nout, const Nrrd *nin, const unsigned int *axes) {
  ...
  size_t idxOut, idxIn = 0, /* indices for input and output scanlines */
      ...
      szIn[NRRD_DIM_MAX], *lszIn, szOut[NRRD_DIM_MAX], *lszOut;
  char *dataIn, *dataOut;
  int axmap[NRRD_DIM_MAX];
  unsigned int cIn[NRRD_DIM_MAX], cOut[NRRD_DIM_MAX];
  unsigned int ai,          /* running index along dimensions */
      lowPax,               /* lowest axis which is "p"ermutated */
      ldim,                 /* nin->dim - lowPax */
      ip[NRRD_DIM_MAX + 1], /* inverse of permutation in "axes" */
      laxes[NRRD_DIM_MAX +
            1]; /* copy of axes[], but shifted down by lowPax
...
int nrrdShuffle(Nrrd *nout, const Nrrd *nin, unsigned int axis,
  ...
  char me[] = "nrrdShuffle", func[] = "shuffle", err[BIFF_STRLEN],
       buff1[NRRD_DIM_MAX * 30], buff2[AIR_STRLEN_SMALL];
  unsigned int ai, ldim, len, cIn[NRRD_DIM_MAX + 1], cOut[NRRD_DIM_MAX + 1];
  ...
  if (!(axis < nin->dim)) {
    sprintf(err, "%s: axis %d outside valid range [0,%d]", me, axis,
            nin->dim - 1);
    biffAdd(NRRD, err);
    return 1;
  }
  ...
  for (ai = 0; ai < len; ai++) {
    if (!(perm[ai] < len)) {
      sprintf(err,
              "%s: perm[%d] (" _AIR_SIZE_T_CNV ") outside valid range [0,%d]",
              me, ai, perm[ai], len - 1);
      biffAdd(NRRD, err);
      return 1;
    }
