...
int _nrrdReadNrrdParseField(NrrdIoState *nio, int useBiff) {
  char me[] = "_nrrdReadNrrdParseField", err[BIFF_STRLEN], *next, *buff, *colon,
       *keysep;
  ...
  if (noField || badField) {
    ...
    if (!keysep) {
      if (noField) {
        ...
        sprintf(err, "%s: failed to parse \"%s\" as field identifier", me,
                buff);
      }
      free(buff);
      biffMaybeAdd(NRRD, err, useBiff);
...
int _nrrdReadNrrdParse_nonfield(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                int useBiff) {
  AIR_UNUSED(file);
  AIR_UNUSED(nrrd);
  AIR_UNUSED(nio);
...
int _nrrdReadNrrdParse_comment(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                               int useBiff) {
...
int _nrrdReadNrrdParse_content(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                               int useBiff) {
...
int _nrrdReadNrrdParse_number(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                              int useBiff) {
  /*
...
int _nrrdReadNrrdParse_type(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                            int useBiff) {
...
int _nrrdReadNrrdParse_block_size(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                  int useBiff) {
...
int _nrrdReadNrrdParse_dimension(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                 int useBiff) {
...
#define _CHECK_GOT_ALL_VALUES                                                  \
  if (nrrd->dim != ret) {                                                      \
    sprintf(err, "%s: parsed %d values, but dimension is %d", me, ret,         \
            nrrd->dim);                                                        \
    biffMaybeAdd(NRRD, err, useBiff);                                          \
    return 1;                                                                  \
  }

int _nrrdReadNrrdParse_sizes(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                             int useBiff) {
  ...
  if (nrrd->dim + 1 == airParseStrZ(val, info, _nrrdFieldSep, nrrd->dim + 1)) {
    sprintf(err, "%s: seem to have more than expected %d sizes", me, nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_spacings(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                int useBiff) {
  ...
  if (nrrd->dim + 1 == airParseStrD(val, info, _nrrdFieldSep, nrrd->dim + 1)) {
    sprintf(err, "%s: seem to have more than expected %d spacings", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_thicknesses(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                   int useBiff) {
  ...
  /* HEY: this is a very imperfect check of excess info */
  if (nrrd->dim + 1 == airParseStrD(val, info, _nrrdFieldSep, nrrd->dim + 1)) {
    sprintf(err, "%s: seem to have more than expected %d thicknesses", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_axis_mins(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                 int useBiff) {
  ...
  /* HEY: this is a very imperfect check of excess info */
  if (nrrd->dim + 1 == airParseStrD(val, info, _nrrdFieldSep, nrrd->dim + 1)) {
    sprintf(err, "%s: seem to have more than expected %d axis mins", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_axis_maxs(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                 int useBiff) {
  ...
  /* HEY: this is a very imperfect check of excess info */
  if (nrrd->dim + 1 == airParseStrD(val, info, _nrrdFieldSep, nrrd->dim + 1)) {
    sprintf(err, "%s: seem to have more than expected %d axis maxs", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdSpaceVectorParse(double val[NRRD_SPACE_DIM_MAX], char **hhP,
                          unsigned int spaceDim, int useBiff) {
  char me[] = "_nrrdSpaceVectorParse", err[BIFF_STRLEN], *hh, *buff,
       sep[] = ",)";
  airArray *mop;
  ...
  if ((strstr(hh, _nrrdNoSpaceVector) == hh)) {
    ...
    if (')' != *hh) {
      sprintf(err, "%s: didn't see ')' at end of first vector in \"%s\"", me,
              hh);
      biffMaybeAdd(NRRD, err, useBiff);
      airMopError(mop);
      return 1;
    ...
    *(hh + 1) = 0;
    ...
    if (spaceDim != ret) {
      sprintf(err, "%s: parsed %d values, but space dimension is %d", me, ret,
              spaceDim);
      biffMaybeAdd(NRRD, err, useBiff);
      airMopError(mop);
      return 1;
  ...
  for (dd = 1; dd < spaceDim; dd++) {
    if (!!AIR_EXISTS(val[0]) ^ !!AIR_EXISTS(val[dd])) {
      sprintf(err,
              "%s: existance of all space vector coefficients must "
...
int _nrrdReadNrrdParse_space_directions(FILE *file, Nrrd *nrrd,
  ...
  for (dd = 0; dd < nrrd->dim; dd++) {
    if (_nrrdSpaceVectorParse(nrrd->axis[dd].spaceDirection, &info,
                              nrrd->spaceDim, useBiff)) {
      sprintf(err, "%s: trouble getting space vector %d of %d", me, dd + 1,
              nrrd->dim);
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
  ...
  if (strlen(info) != strspn(info, _nrrdFieldSep)) {
    sprintf(err, "%s: seem to have more than expected %d directions", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_centers(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                               int useBiff) {
  ...
  for (ai = 0; ai < nrrd->dim; ai++) {
    ...
    if (!tok) {
      sprintf(err, "%s: couldn't extract string for center %d of %d", me,
              ai + 1, nrrd->dim);
      biffMaybeAdd(NRRD, err, useBiff);
      airMopError(mop);
      return 1;
    ...
    if (!(nrrd->axis[ai].center = airEnumVal(nrrdCenter, tok))) {
      sprintf(err, "%s: couldn't parse center \"%s\" for axis %d", me, tok, ai);
      biffMaybeAdd(NRRD, err, useBiff);
      airMopError(mop);
      return 1;
    }
  ...
  if (airStrtok(!ai ? info : NULL, _nrrdFieldSep, &last)) {
    sprintf(err, "%s: seem to have more than expected %d centers", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    airMopError(mop);
    return 1;
...
int _nrrdReadNrrdParse_kinds(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                             int useBiff) {
  ...
  for (ai = 0; ai < nrrd->dim; ai++) {
    tok = airStrtok(!ai ? info : NULL, _nrrdFieldSep, &last);
    if (!tok) {
      sprintf(err, "%s: couldn't extract string for kind %d of %d", me, ai + 1,
              nrrd->dim);
      biffMaybeAdd(NRRD, err, useBiff);
      airMopError(mop);
      return 1;
    ...
    if (!(nrrd->axis[ai].kind = airEnumVal(nrrdKind, tok))) {
      sprintf(err, "%s: couldn't parse \"%s\" kind %d of %d", me, tok, ai + 1,
              nrrd->dim);
      biffMaybeAdd(NRRD, err, useBiff);
      airMopError(mop);
      return 1;
  ...
  if (airStrtok(!ai ? info : NULL, _nrrdFieldSep, &last)) {
    sprintf(err, "%s: seem to have more than expected %d kinds", me, nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    airMopError(mop);
    return 1;
  }
...
int _nrrdReadNrrdParse_labels(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                              int useBiff) {
  ...
  if (strlen(h) != strspn(h, _nrrdFieldSep)) {
    sprintf(err, "%s: seem to have more than expected %d labels", me,
            nrrd->dim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_units(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                             int useBiff) {
...
int _nrrdReadNrrdParse_min(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                           int useBiff) {
...
int _nrrdReadNrrdParse_max(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                           int useBiff) {
...
int _nrrdReadNrrdParse_old_min(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                               int useBiff) {
...
int _nrrdReadNrrdParse_old_max(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                               int useBiff) {
...
int _nrrdReadNrrdParse_endian(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                              int useBiff) {
...
int _nrrdReadNrrdParse_encoding(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                int useBiff) {
...
int _nrrdReadNrrdParse_line_skip(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                 int useBiff) {
...
int _nrrdReadNrrdParse_byte_skip(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                 int useBiff) {
...
int _nrrdReadNrrdParse_keyvalue(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                int useBiff) {
  ...
  keysep = strstr(line, ":=");
  if (!keysep) {
    sprintf(err, "%s: didn't see \":=\" key/value delimiter in \"%s\"", me,
            line);
    free(line);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
...
int _nrrdReadNrrdParse_sample_units(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                    int useBiff) {
...
int _nrrdReadNrrdParse_space(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                             int useBiff) {
...
int _nrrdReadNrrdParse_space_dimension(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                       int useBiff) {
...
int _nrrdReadNrrdParse_space_units(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                   int useBiff) {
  ...
  for (ai = 0; ai < nrrd->spaceDim; ai++) {
    if (!(nrrd->spaceUnits[ai] = _nrrdGetQuotedString(&h, useBiff))) {
      sprintf(err, "%s: couldn't get get space unit %d of %d", me, ai + 1,
              nrrd->spaceDim);
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
  ...
  if (_nrrdGetQuotedString(&h, AIR_FALSE)) {
    sprintf(err, "%s: seemed to have more than expected %d space units", me,
            nrrd->spaceDim);
    biffMaybeAdd(NRRD, err, useBiff);
    return 1;
  }
...
int _nrrdReadNrrdParse_space_origin(FILE *file, Nrrd *nrrd, NrrdIoState *nio,
                                    int useBiff) {
  ...
  if (_nrrdSpaceVectorParse(nrrd->spaceOrigin, &info, nrrd->spaceDim,
                            useBiff)) {
...
int _nrrdReadNrrdParse_measurement_frame(FILE *file, Nrrd *nrrd,
                                         NrrdIoState *nio, int useBiff) {
  ...
  for (dd = 0; dd < nrrd->spaceDim; dd++) {
    ...
    for (ii = 0; ii < NRRD_SPACE_DIM_MAX; ii++) {
      nrrd->measurementFrame[dd][ii] =
          (ii < nrrd->spaceDim ? colvec[ii] : AIR_NAN);
    }
  }
  if (strlen(info) != strspn(info, _nrrdFieldSep)) {
    sprintf(err, "%s: seem to have more than expected %d directions", me,
...
int _nrrdContainsPercentDAndMore(char *str) {
  char *hh, *tmp;
...
int _nrrdDataFNCheck(NrrdIoState *nio, Nrrd *nrrd, int useBiff) {
  ...
  if (nio->dataFileDim < nrrd->dim) {
    ...
    if (pieceNum != _nrrdDataFNNumber(nio)) {
      sprintf(err, "%s: expected %d filenames (of %d-D pieces) but got %d", me,
              (int)pieceNum, nio->dataFileDim, (int)_nrrdDataFNNumber(nio));
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
  } else {
    ...
    if (_nrrdDataFNNumber(nio) > nrrd->axis[nrrd->dim - 1].size) {
      sprintf(err,
              ...
              "datafile dimension are both %d",
              me, (int)_nrrdDataFNNumber(nio), nrrd->dim - 1,
              nrrd->axis[nrrd->dim - 1].size, nrrd->dim);
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
    if ((double)nrrd->axis[nrrd->dim - 1].size / _nrrdDataFNNumber(nio) !=
      ...
      sprintf(err,
              ...
              "number of axis %d slices (" _AIR_SIZE_T_CNV ")",
              me, (int)_nrrdDataFNNumber(nio), nrrd->dim - 1,
              nrrd->axis[nrrd->dim - 1].size);
      biffMaybeAdd(NRRD, err, useBiff);
      return 1;
    }
  }
...
** nio->path.
*/
int _nrrdReadNrrdParse_data_file(FILE *ffile, Nrrd *nrrd, NrrdIoState *nio,
                                 int useBiff) {
  ...
  if (_nrrdContainsPercentDAndMore(info)) {
    ...
    nums += tmp;
    if (!(3 == sscanf(nums, "%d %d %d", &(nio->dataFNMin), &(nio->dataFNMax),
                      &(nio->dataFNStep)))) {
      sprintf(err,
    ...
    if (4 == sscanf(nums, "%d %d %d %u", &(nio->dataFNMin), &(nio->dataFNMax),
                    &(nio->dataFNStep), &(nio->dataFileDim))) {
      if (!(nio->dataFileDim >= 1 && nio->dataFileDim <= nrrd->dim)) {
        sprintf(err, "%s: datafile dimension %d outside valid range [1,%d]", me,
    ...
    if (_nrrdHeaderCheck(nrrd, nio, AIR_TRUE)) {
      sprintf(err,
              "%s: NRRD header is incomplete. \"" NRRD_LIST_FLAG
    ...
    if (info[0]) {
      if (1 == sscanf(info, "%u", &(nio->dataFileDim))) {
        ...
        sprintf(err,
                "%s: couldn't parse info after \"" NRRD_LIST_FLAG
                "\" as an int",
                me);
        biffMaybeAdd(NRRD, err, useBiff);
        airMopError(mop);
...
*/
int (*nrrdFieldInfoParse[NRRD_FIELD_MAX + 1])(FILE *, Nrrd *, NrrdIoState *,
                                              int) = {
