...
char _airBoolStr[][AIR_STRLEN_SMALL] = {"(unknown bool)", "false", "true"};

char _airBoolDesc[][AIR_STRLEN_MED] = {"unknown boolean", "false", "true"};

int _airBoolVal[] = {-1, AIR_FALSE, AIR_TRUE};

char _airBoolStrEqv[][AIR_STRLEN_SMALL] = {"0",    "no",   "n",  "false", "f",
                                           "off",  "nope", "1",  "yes",   "y",
                                           "true", "t",    "on", "yea",   ""};

int _airBoolValEqv[] = {AIR_FALSE, AIR_FALSE, AIR_FALSE, AIR_FALSE, AIR_FALSE,
                        AIR_FALSE, AIR_FALSE, AIR_TRUE,  AIR_TRUE,  AIR_TRUE,
                        AIR_TRUE,  AIR_TRUE,  AIR_TRUE,  AIR_TRUE};

const airEnum _airBool = {"boolean",      2,
                          _airBoolStr,    _airBoolVal,
                          _airBoolDesc,   _airBoolStrEqv,
                          _airBoolValEqv, AIR_FALSE};
...
** given string "s";
try to parse "n" of them, as delimited by
...
** argument.
**
** This uses air's thread-safe strtok() replacement:
airStrtok()
*/
unsigned int airParseStrI(_PARSE_STR_ARGS(int)) { _PARSE_STR_BODY("%d") }
...
unsigned int airParseStrZ(_PARSE_STR_ARGS(size_t)) {
  _PARSE_STR_BODY(_AIR_SIZE_T_CNV)
}

unsigned int airParseStrF(_PARSE_STR_ARGS(float)) { _PARSE_STR_BODY("%f") }

unsigned int airParseStrD(_PARSE_STR_ARGS(double)) { _PARSE_STR_BODY("%lf") }

unsigned int airParseStrB(int *out, const char *_s, const char *ct,
                          unsigned int n, ...) {
  unsigned int i;
  char *tmp, *s, *last;
...
unsigned int airParseStrC(char *out, const char *_s, const char *ct,
                          unsigned int n, ...) {
  unsigned int i;
  char *tmp, *s, *last;
...
unsigned int airParseStrS(char **out, const char *_s, const char *ct,
                          unsigned int n, ...) {
  unsigned int i;
  int greedy;
  char *tmp, *s, *last;
...
unsigned int airParseStrE(int *out, const char *_s, const char *ct,
                          unsigned int n, ...) {
  unsigned int i;
  char *tmp, *s, *last;
  airArray *mop;
...
unsigned int (*airParseStr[AIR_TYPE_MAX + 1])(void *, const char *,
