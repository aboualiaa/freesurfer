...
float airFPPartsToVal_f(unsigned int sign, unsigned int expo,
                        unsigned int mant) {
  _airFloat f;
...
void airFPValToParts_f(unsigned int *signP, unsigned int *expoP,
...
double airFPPartsToVal_d(unsigned int sign, unsigned int expo,
...
#endif
void airFPValToParts_d(unsigned int *signP, unsigned int *expoP,
                       unsigned int *mant0P, unsigned int *mant1P, double v) {
  _airDouble d;
  d.v = v;
...
int airFPClass_d(double val) {
  ...
  expo = f.c.expo; /* this seems to be a WIN32 bug: on a quiet-NaN, f.c.exp
                      ...
                      that this function returned airFP_NEG_DENORM instead of
                              airFP_QNAN */
  mant0 = f.c.mant0;
  mant1 = f.c.mant1;
  hibit = mant0 >> 20;
...
*/
float airNaN(void) { return airFPGen_f(airFP_QNAN); }
