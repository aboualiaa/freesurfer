...
int _nrrdEncodingRaw_available(void) { return AIR_TRUE; }

int _nrrdEncodingRaw_read(FILE *file, void *data, size_t elementNum, Nrrd *nrrd,
                          NrrdIoState *nio) {
  ...
  if (airNoDio_okay == dio) {
    ...
    if (ret != bsize) {
      sprintf(err,
              "%s: airDioRead got read only " _AIR_SIZE_T_CNV
              " of " _AIR_SIZE_T_CNV " bytes "
              "(%g%% of expected)",
              me, ret, bsize, 100.0 * ret / bsize);
      biffAdd(NRRD, err);
      return 1;
    }
  } else {
    ...
    if (ret != elementNum) {
      sprintf(err,
              "%s: fread got read only " _AIR_SIZE_T_CNV " " _AIR_SIZE_T_CNV
              "-sized things, not " _AIR_SIZE_T_CNV " (%g%% of expected)",
...
int _nrrdEncodingRaw_write(FILE *file, const void *data, size_t elementNum,
  ...
  if (airNoDio_okay == dio) {
    ...
    if (ret != bsize) {
      sprintf(err,
              "%s: airDioWrite wrote only " _AIR_SIZE_T_CNV
              " of " _AIR_SIZE_T_CNV " bytes "
              "(%g%% of expected)",
              me, ret, bsize, 100.0 * ret / bsize);
      biffAdd(NRRD, err);
    ...
    if (ret != elementNum) {
      sprintf(err,
              "%s: fwrite wrote read only " _AIR_SIZE_T_CNV " " _AIR_SIZE_T_CNV
              "-sized things, not " _AIR_SIZE_T_CNV " (%g%% of expected)",
              me, ret, nrrdElementSize(nrrd), elementNum,
              100.0 * ret / elementNum);
      biffAdd(NRRD, err);
