...
*/
int nrrdSlice(Nrrd *nout, const Nrrd *nin, unsigned int saxi, size_t pos) {
  ...
  if (!(saxi < nin->dim)) {
    sprintf(err, "%s: slice axis %d out of bounds (0 to %d)", me, saxi,
            nin->dim - 1);
    biffAdd(NRRD, err);
    return 1;
  }
  ...
  if (AIR_EXISTS(nin->axis[saxi].spaceDirection[0])) {
    _nrrdSpaceVecScaleAdd2(nout->spaceOrigin, 1.0, nin->spaceOrigin, pos,
...
** along each axis.
*/
int nrrdCrop(Nrrd *nout, const Nrrd *nin, size_t *min, size_t *max) {
  ...
  size_t I, lineSize,     /* #bytes in one scanline to be copied */
      ...
      szIn[NRRD_DIM_MAX], szOut[NRRD_DIM_MAX],
  ...
  for (ai = 0; ai < nin->dim; ai++) {
    ...
    if (!(min[ai] < nin->axis[ai].size && max[ai] < nin->axis[ai].size)) {
      sprintf(err,
              "%s: axis %d min (" _AIR_SIZE_T_CNV ") or max (" _AIR_SIZE_T_CNV
              ") out of bounds [0," _AIR_SIZE_T_CNV "]",
  ...
  */
  ...
  if (nrrdAxisInfoCopy(nout, nin, NULL,
                       (NRRD_AXIS_INFO_SIZE_BIT | NRRD_AXIS_INFO_MIN_BIT |
  ...
  for (ai = 0; ai < nin->dim; ai++) {
    ...
    if (!nrrdStateKindNoop) {
      ...
      } else if (nrrdKind4Color == nin->axis[ai].kind && 3 == szOut[ai]) {
        nout->axis[ai].kind = nrrdKind3Color;
      } else if (nrrdKind4Vector == nin->axis[ai].kind && 3 == szOut[ai]) {
      ...
      } else if (nrrdKindRGBAColor == nin->axis[ai].kind && 0 == min[ai] &&
                 2 == max[ai]) {
        nout->axis[ai].kind = nrrdKindRGBColor;
      } else if (nrrdKind2DMaskedSymMatrix == nin->axis[ai].kind &&
                 1 == min[ai] && max[ai] == szIn[ai] - 1) {
        nout->axis[ai].kind = nrrdKind2DSymMatrix;
      } else if (nrrdKind2DMaskedMatrix == nin->axis[ai].kind && 1 == min[ai] &&
                 max[ai] == szIn[ai] - 1) {
      ...
      } else if (nrrdKind3DMaskedMatrix == nin->axis[ai].kind && 1 == min[ai] &&
                 max[ai] == szIn[ai] - 1) {
  ...
  for (ai = 0; ai < nin->dim; ai++) {
    if (AIR_EXISTS(nin->axis[ai].spaceDirection[0])) {
      _nrrdSpaceVecScaleAdd2(nout->spaceOrigin, 1.0, nout->spaceOrigin, min[ai],
