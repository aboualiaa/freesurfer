...
int _nrrdEncodingGzip_available(void) {
...
int _nrrdEncodingGzip_read(FILE *file, void *_data, size_t elNum, Nrrd *nrrd,
                           NrrdIoState *nio) {
  ...
  for (i = 0; i < nio->byteSkip; i++) {
    ...
    if (_nrrdGzRead(gzfin, &b, 1, &read) != 0 || read != 1) {
      sprintf(err, "%s: hit an error skipping byte %d of %d", me, i,
              nio->byteSkip);
      biffAdd(NRRD, err);
      return 1;
    }
  ...
  if (total_read != bsize) {
    sprintf(err,
            "%s: expected " _AIR_SIZE_T_CNV
            " bytes and received " _AIR_SIZE_T_CNV " bytes",
            me, bsize, total_read);
    biffAdd(NRRD, err);
    return 1;
  }
...
int _nrrdEncodingGzip_write(FILE *file, const void *_data, size_t elNum,
  ...
  if (total_written != bsize) {
    sprintf(err,
            "%s: expected to write " _AIR_SIZE_T_CNV " bytes, but only "
