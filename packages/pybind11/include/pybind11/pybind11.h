...
#pragma warning(                                                               \
    disable : 1334) // the "template" keyword used for syntactic disambiguation
...
#pragma GCC diagnostic pop
  ...
  static PyObject *dispatcher(PyObject *self, PyObject *args_in,
    ...
    function_record *overloads =
                        (function_record *)PyCapsule_GetPointer(self, nullptr),
                    *it = overloads;
    ...
    try {
      ...
      for (; it != nullptr; it = it->next) {
        ...
        for (; args_copied < args_to_copy; ++args_copied) {
          argument_record *arg_rec = args_copied < func.args.size()
...
public:
  ...
  class_(handle scope, const char *name, const Extra &... extra) {
    ...
    record.default_holder =
        std::is_same<holder_type, std::unique_ptr<type>>::value;
...
public:
  ...
  enum_(const handle &scope, const char *name, const Extra &... extra)
...
#endif
    def("__eq__", [](const Type &value, Type *value2) {
      return value2 && value == *value2;
    });
    def("__ne__", [](const Type &value, Type *value2) {
      return !value2 || value != *value2;
    });
    if (is_arithmetic) {
      def("__lt__", [](const Type &value, Type *value2) {
        return value2 && value < *value2;
      });
      def("__gt__", [](const Type &value, Type *value2) {
        return value2 && value > *value2;
      });
      def("__le__", [](const Type &value, Type *value2) {
        return value2 && value <= *value2;
      });
      def("__ge__", [](const Type &value, Type *value2) {
        return value2 && value >= *value2;
      });
    }
    if (std::is_convertible<Type, Scalar>::value) {
      // Don't provide comparison with the underlying type if the enum isn't
      // convertible, i.e. if Type is a scoped enum, mirroring the C++
      // behaviour.  (NB: we explicitly convert Type to Scalar below anyway
      // because this needs to compile).
    ...
    // Pickling and unpickling -- needed for use with the 'multiprocessing'
    // module
    def(pickle(
        [](const Type &value) { return pybind11::make_tuple((Scalar)value); },
...
public:
  ...
  exception(handle scope, const char *name, PyObject *base = PyExc_Exception) {
    ...
    m_ptr = PyErr_NewException(const_cast<char *>(full_name.c_str()), base,
                               nullptr);
