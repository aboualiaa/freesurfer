 ...
 */
static void xmlSchematronPErr(xmlSchematronParserCtxtPtr ctxt, xmlNodePtr node,
...
xmlSchematronAddTest(xmlSchematronParserCtxtPtr ctxt,
  ...
  if (comp == NULL) {
    xmlSchematronPErr(ctxt, node, XML_SCHEMAP_NOROOT,
                      "Failed to compile test expression %s", test, NULL);
    return (NULL);
  }
...
xmlSchematronAddRule(xmlSchematronParserCtxtPtr ctxt, xmlSchematronPtr schema,
  ...
  if (pattern == NULL) {
    xmlSchematronPErr(ctxt, node, XML_SCHEMAP_NOROOT,
                      "Failed to compile context expression %s", context, NULL);
  }
...
xmlSchematronAddPattern(xmlSchematronParserCtxtPtr ctxt,
                        xmlSchematronPtr schema, xmlNodePtr node,
                        xmlChar *name) {
 ...
 */
static void xmlSchematronAddNamespace(xmlSchematronParserCtxtPtr ctxt,
                                      const xmlChar *prefix,
                                      const xmlChar *ns) {
  if (ctxt->namespaces == NULL) {
    ctxt->maxNamespaces = 10;
    ctxt->namespaces = (const xmlChar **)xmlMalloc(ctxt->maxNamespaces * 2 *
    ...
    tmp = (const xmlChar **)xmlRealloc((xmlChar **)ctxt->namespaces,
                                       ctxt->maxNamespaces * 4 *
                                           sizeof(const xmlChar *));
    if (tmp == NULL) {
      xmlSchematronPErrMemory(NULL, "allocating parser namespaces", NULL);
      return;
...
static void xmlSchematronParseRule(xmlSchematronParserCtxtPtr ctxt,
  ...
  if (context == NULL) {
    xmlSchematronPErr(ctxt, rule, XML_SCHEMAP_NOROOT,
                      "rule has no context attribute", NULL, NULL);
    return;
  } else if (context[0] == 0) {
    xmlSchematronPErr(ctxt, rule, XML_SCHEMAP_NOROOT,
                      "rule has an empty context attribute", NULL, NULL);
    xmlFree(context);
    return;
  } else {
    ruleptr =
        xmlSchematronAddRule(ctxt, ctxt->schema, pattern, rule, context, NULL);
    if (ruleptr == NULL) {
      xmlFree(context);
      return;
    }
  ...
  while (cur != NULL) {
    if (IS_SCHEMATRON(cur, "assert")) {
      ...
      if (test == NULL) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "assert has no test attribute", NULL, NULL);
      } else if (test[0] == 0) {
        xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                          "assert has an empty test attribute", NULL, NULL);
        xmlFree(test);
      } else {
        /* TODO will need dynamic processing instead */
        report = xmlNodeGetContent(cur);

        testptr = xmlSchematronAddTest(ctxt, XML_SCHEMATRON_ASSERT, ruleptr,
                                       cur, test, report);
        if (testptr == NULL)
          xmlFree(test);
      }
      ...
      if (test == NULL) {
        ...
        testptr = xmlSchematronAddTest(ctxt, XML_SCHEMATRON_REPORT, ruleptr,
                                       cur, test, report);
        if (testptr == NULL)
          xmlFree(test);
      }
 ...
 */
static void xmlSchematronParsePattern(xmlSchematronParserCtxtPtr ctxt,
                                      xmlNodePtr pat) {
  xmlNodePtr cur;
  xmlSchematronPatternPtr pattern;
  int nbRules = 0;
  ...
  while (cur != NULL) {
    if (IS_SCHEMATRON(cur, "rule")) {
      ...
      xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                        "Expecting a rule element instead of %s", cur->name,
                        NULL);
    }
    cur = cur->next;
    NEXT_SCHEMATRON(cur);
  ...
  if (nbRules == 0) {
    xmlSchematronPErr(ctxt, pat, XML_SCHEMAP_NOROOT,
...
xmlSchematronPtr xmlSchematronParse(xmlSchematronParserCtxtPtr ctxt) {
  ...
  if (ctxt->URL != NULL) {
    ...
    if (doc == NULL) {
      xmlSchematronPErr(ctxt, NULL, XML_SCHEMAP_FAILED_LOAD,
    ...
    if (doc == NULL) {
      xmlSchematronPErr(ctxt, NULL, XML_SCHEMAP_FAILED_PARSE,
    ...
    xmlSchematronPErr(ctxt, NULL, XML_SCHEMAP_NOTHING_TO_PARSE,
                      "xmlSchematronParse: could not parse.\n", NULL, NULL);
    return (NULL);
  }
  ...
  if (root == NULL) {
    xmlSchematronPErr(ctxt, (xmlNodePtr)doc, XML_SCHEMAP_NOROOT,
  ...
  if (!IS_SCHEMATRON(root, "schema")) {
    xmlSchematronPErr(ctxt, root, XML_SCHEMAP_NOROOT,
  ...
  while (IS_SCHEMATRON(cur, "ns")) {
    ...
    if ((uri == NULL) || (uri[0] == 0)) {
      xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT, "ns element has no uri",
                        NULL, NULL);
    }
  ...
  while (cur != NULL) {
    if (IS_SCHEMATRON(cur, "pattern")) {
      ...
      xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT,
                        "Expecting a pattern element instead of %s", cur->name,
                        NULL);
    }
    cur = cur->next;
    NEXT_SCHEMATRON(cur);
  ...
  if (ret->nbPattern == 0) {
    xmlSchematronPErr(ctxt, root, XML_SCHEMAP_NOROOT,
                      "The schematron document '%s' has no pattern", ctxt->URL,
                      NULL);
    goto exit;
  }
...
static xmlNodePtr xmlSchematronGetNode(xmlSchematronValidCtxtPtr ctxt,
  ...
  if ((ret->type == XPATH_NODESET) && (ret->nodesetval != NULL) &&
...
xmlSchematronReportOutput(xmlSchematronValidCtxtPtr ctxt ATTRIBUTE_UNUSED,
                          xmlNodePtr cur ATTRIBUTE_UNUSED, const char *msg) {
  /* TODO */
...
static void xmlSchematronReportSuccess(xmlSchematronValidCtxtPtr ctxt,
  ...
  if (ctxt->flags & XML_SCHEMATRON_OUT_XML) {
    ...
    if (ctxt->flags & XML_SCHEMATRON_OUT_ERROR) {
      ...
      __xmlRaiseError(
          schannel, channel, data, NULL, cur, XML_FROM_SCHEMATRONV,
          (test->type == XML_SCHEMATRON_ASSERT) ? XML_SCHEMATRONV_ASSERT
...
static void xmlSchematronReportPattern(xmlSchematronValidCtxtPtr ctxt,
  ...
  if ((ctxt == NULL) || (pattern == NULL))
    return;
  if ((ctxt->flags & XML_SCHEMATRON_OUT_QUIET) ||
      (ctxt->flags & XML_SCHEMATRON_OUT_ERROR)) /* Error gives pattern name as
                                                   part of error */
    return;
  if (ctxt->flags & XML_SCHEMATRON_OUT_XML) {
    TODO
        ...
        if (xmlPatternMatch(rule->pattern, cur) == 1) {
          ...
          while (test != NULL) {
            xmlSchematronRunTest(ctxt, test, instance, cur,
                                 (xmlSchematronPatternPtr)rule->pattern);
            test = test->next;
          }
        }
        rule = rule->next;
