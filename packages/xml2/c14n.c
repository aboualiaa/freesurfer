...
static void xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns,
                                     xmlNodePtr node);
...
static int xmlExcC14NVisibleNsStackFind(xmlC14NVisibleNsStackPtr cur,
                                        xmlNsPtr ns, xmlC14NCtxPtr ctx);

static int xmlC14NIsNodeInNodeset(xmlNodeSetPtr nodes, xmlNodePtr node,
                                  xmlNodePtr parent);
...
#define xmlC14NIsVisible(ctx, node, parent)                                    \
  (((ctx)->is_visible_callback != NULL)                                        \
       ? (ctx)->is_visible_callback((ctx)->user_data, (xmlNodePtr)(node),      \
                                    (xmlNodePtr)(parent))                      \
       : 1)
...
static void xmlC14NErrParam(const char *extra) {
  __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_C14N,
                  XML_ERR_INTERNAL_ERROR, XML_ERR_ERROR, NULL, 0, extra, NULL,
                  NULL, 0, 0, "Invalid parameter : %s\n", extra);
}
...
static void xmlC14NErrInvalidNode(const char *node_type, const char *extra) {
  __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_C14N,
                  XML_C14N_INVALID_NODE, XML_ERR_ERROR, NULL, 0, extra, NULL,
                  NULL, 0, 0, "Node %s is invalid here : %s\n", node_type,
                  extra);
}
...
static void xmlC14NErrUnknownNode(int node_type, const char *extra) {
  __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_C14N,
                  XML_C14N_UNKNOW_NODE, XML_ERR_ERROR, NULL, 0, extra, NULL,
                  NULL, 0, 0, "Unknown node type %d found : %s\n", node_type,
                  extra);
}
...
static void xmlC14NErr(xmlC14NCtxPtr ctxt, xmlNodePtr node, int error,
  ...
  if (ctxt != NULL)
    ctxt->error = error;
  __xmlRaiseError(NULL, NULL, NULL, ctxt, node, XML_FROM_C14N, error,
...
static void xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns,
  ...
  if ((cur->nsTab == NULL) && (cur->nodeTab == NULL)) {
    cur->nsTab =
        (xmlNsPtr *)xmlMalloc(XML_NAMESPACES_DEFAULT * sizeof(xmlNsPtr));
    cur->nodeTab =
        (xmlNodePtr *)xmlMalloc(XML_NAMESPACES_DEFAULT * sizeof(xmlNodePtr));
    if ((cur->nsTab == NULL) || (cur->nodeTab == NULL)) {
...
static int xmlC14NIsXmlNs(xmlNsPtr ns) {
  return ((ns != NULL) && (xmlStrEqual(ns->prefix, BAD_CAST "xml")) &&
 ...
 */
static int xmlC14NProcessNamespacesAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur,
                                        int visible) {
  xmlNodePtr n;
  xmlNsPtr ns, tmp;
  xmlListPtr list;
 ...
 * Returns 0 on success or -1 on fail.
 */
static int xmlExcC14NProcessNamespacesAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur,
                                           int visible) {
  xmlNsPtr ns;
  xmlListPtr list;
  xmlAttrPtr attr;
  ...
  for (attr = cur->properties; attr != NULL; attr = attr->next) {
     ...
     */
    if ((attr->ns != NULL) && !xmlC14NIsXmlNs(attr->ns) &&
        xmlC14NIsVisible(ctx, attr, cur)) {
      already_rendered =
          xmlExcC14NVisibleNsStackFind(ctx->ns_rendered, attr->ns, ctx);
  ...
  if (visible && has_visibly_utilized_empty_ns && !has_empty_ns &&
    ...
    already_rendered =
        xmlExcC14NVisibleNsStackFind(ctx->ns_rendered, &ns_default, ctx);
    if (!already_rendered) {
      xmlC14NPrintNamespaces(&ns_default, ctx);
    }
 ...
 */
static xmlAttrPtr xmlC14NFindHiddenParentAttr(xmlC14NCtxPtr ctx, xmlNodePtr cur,
                                              const xmlChar *name,
                                              const xmlChar *ns) {
  xmlAttrPtr res;
 ...
 */
static xmlAttrPtr xmlC14NFixupBaseAttr(xmlC14NCtxPtr ctx,
  ...
  if ((ctx == NULL) || (xml_base_attr == NULL) ||
      (xml_base_attr->parent == NULL)) {
 ...
 * Returns 0 on success or -1 on fail.
 */
static int xmlC14NProcessAttrsAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur,
                                   int parent_visible) {
  xmlAttrPtr attr;
  xmlListPtr list;
  ...
  case XML_C14N_1_0:
     ...
     * XPath node-set is given as input and the element's parent is omitted from
     * the node-set. The method for processing the attribute axis of an element
     * E in the node-set is enhanced. All element nodes along E's ancestor axis
     * are examined for nearest occurrences of attributes in the xml namespace,
     * such as xml:lang and xml:space (whether or not they are in the node-set).
     * From this list of attributes, remove any that are in E's attribute axis
  ...
  case XML_C14N_1_1:
    /* The processing of an element node E MUST be modified slightly when an
     * XPath node-set is given as input and some of the element's ancestors are
     * omitted from the node-set.
     *
     * Simple inheritable attributes are attributes that have a value that
     * requires at most a simple redeclaration. This redeclaration is done by
     * supplying a new value in the child axis. The redeclaration of a simple
     * inheritable attribute A contained in one of E's ancestors is done by
     * supplying a value to an attribute Ae inside E with the same name. Simple
     * inheritable attributes are xml:lang and xml:space.
     *
     * The method for processing the attribute axis of an element E in the
     * node-set is hence enhanced. All element nodes along E's ancestor axis are
     * examined for the nearest occurrences of simple inheritable attributes in
     * the xml namespace, such as xml:lang and xml:space (whether or not they
     * are in the node-set). From this list of attributes, any simple
     * inheritable attributes that are already in E's attribute axis (whether or
     * not they are in the node-set) are removed. Then, lexicographically merge
     * this attribute list with the nodes of E's attribute axis that are in the
     * node-set. The result of visiting the attribute axis is computed by
     * processing the attribute nodes in this merged attribute list.
     *
     * The xml:id attribute is not a simple inheritable attribute and no
     * processing of these attributes is performed.
     *
     * The xml:base attribute is not a simple inheritable attribute and requires
     * special processing beyond a simple redeclaration.
     *
     * Attributes in the XML namespace other than xml:base, xml:id, xml:lang,
    ...
    attr = cur->properties;
    while (attr != NULL) {
      /* special processing for XML attribute kiks in only when we have
       * invisible parents */
    ...
    /* special processing for XML attribute kiks in only when we have invisible
     * parents */
    if ((parent_visible)) {
      ...
      if (xml_lang_attr == NULL) {
        xml_lang_attr = xmlC14NFindHiddenParentAttr(
            ctx, cur->parent, BAD_CAST "lang", XML_XML_NAMESPACE);
      }
      if (xml_lang_attr != NULL) {
      ...
      if (xml_space_attr == NULL) {
        xml_space_attr = xmlC14NFindHiddenParentAttr(
            ctx, cur->parent, BAD_CAST "space", XML_XML_NAMESPACE);
      }
      if (xml_space_attr != NULL) {
      ...
      if (xml_base_attr == NULL) {
        /* if we don't have base uri attribute, check if we have a "hidden" one
         * above */
        xml_base_attr = xmlC14NFindHiddenParentAttr(
            ctx, cur->parent, BAD_CAST "base", XML_XML_NAMESPACE);
      }
      if (xml_base_attr != NULL) {
 ...
 */
static int xmlC14NCheckForRelativeNamespaces(xmlC14NCtxPtr ctx,
                                             xmlNodePtr cur) {
  xmlNsPtr ns;
 ...
 */
static int xmlC14NProcessElementNode(xmlC14NCtxPtr ctx, xmlNodePtr cur,
                                     int visible) {
  int ret;
...
static xmlC14NCtxPtr xmlC14NNewCtx(xmlDocPtr doc,
                                   xmlC14NIsVisibleCallback is_visible_callback,
                                   void *user_data, xmlC14NMode mode,
                                   xmlChar **inclusive_ns_prefixes,
  ...
  if (buf->encoder != NULL) {
    xmlC14NErr(ctx, (xmlNodePtr)doc, XML_C14N_REQUIRES_UTF8,
               "xmlC14NNewCtx: output buffer encoder != NULL but C14N requires "
               "UTF8 output\n");
    return (NULL);
  }
...
int xmlC14NExecute(xmlDocPtr doc, xmlC14NIsVisibleCallback is_visible_callback,
  ...
  if (buf->encoder != NULL) {
    xmlC14NErr(NULL, (xmlNodePtr)doc, XML_C14N_REQUIRES_UTF8,
               "xmlC14NExecute: output buffer encoder != NULL but C14N "
               "requires UTF8 output\n");
    return (-1);
  }

  ctx = xmlC14NNewCtx(doc, is_visible_callback, user_data, c14n_mode,
 ...
 */
int xmlC14NDocDumpMemory(xmlDocPtr doc, xmlNodeSetPtr nodes, int mode,
                         xmlChar **inclusive_ns_prefixes, int with_comments,
                         xmlChar **doc_txt_ptr) {
  int ret;
  xmlOutputBufferPtr buf;
  ...
  *doc_txt_ptr = NULL;
   ...
   */
  ret = xmlC14NDocSaveTo(doc, nodes, mode, inclusive_ns_prefixes, with_comments,
                         buf);
  if (ret < 0) {
