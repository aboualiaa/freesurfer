...
typedef struct xmlNanoFTPCtxt {
  ...
  struct sockaddr_storage
      ftpAddr; /* this is large enough to hold IPv6 address*/
#else
 ...
 */
static void xmlFTPErrMemory(const char *extra) {
...
void xmlNanoFTPProxy(const char *host, int port, const char *user,
...
static void xmlNanoFTPScanURL(void *ctx, const char *URL) {
  ...
  if (uri->user != NULL) {
    ...
    if ((cptr = strchr(uri->user, ':')) == NULL)
      ...
      ctxt->user = (char *)xmlStrndup((xmlChar *)uri->user, (cptr - uri->user));
...
void xmlNanoFTPScanProxy(const char *URL) {
  ...
  uri = xmlParseURIRaw(URL, 1);
  if ((uri == NULL) || (uri->scheme == NULL) || (strcmp(uri->scheme, "ftp")) ||
      (uri->server == NULL)) {
 ...
 */
static int xmlNanoFTPGetMore(void *ctx) {
  xmlNanoFTPCtxtPtr ctxt = (xmlNanoFTPCtxtPtr)ctx;
  int len;
  int size;
 ...
 */
static int xmlNanoFTPReadResponse(void *ctx) {
  xmlNanoFTPCtxtPtr ctxt = (xmlNanoFTPCtxtPtr)ctx;
  char *ptr, *end;
  int len;
  int res = -1, cur = -1;
...
int xmlNanoFTPQuit(void *ctx) {
  ...
  xmlGenericError(xmlGenericErrorContext, "%s",
                  buf); /* Just to be consistent, even though we know it can't
                           have a % in it */
#endif
  res = send(ctxt->controlFd, buf, len, 0);
  if (res < 0) {
...
int xmlNanoFTPConnect(void *ctx) {
  ...
  if (have_ipv6()) {
    ...
    } else if (getaddrinfo(ctxt->hostname, NULL, &hints, &result) != 0) {
      __xmlIOErr(XML_FROM_FTP, 0, "getaddrinfo failed");
      return (-1);
    }
   ...
   * Do the connect.
   */
  if (connect(ctxt->controlFd, (struct sockaddr *)&ctxt->ftpAddr, addrlen) <
      0) {
...
int xmlNanoFTPDele(void *ctx, const char *file) {
  ...
  if ((ctxt == NULL) || (ctxt->controlFd < 0) || (file == NULL))
    return (-1);
  if (file == NULL)
    return (0);
...
int xmlNanoFTPGetConnection(void *ctx) {
  ...
  if (ctxt->passive) {
    ...
    if ((ctxt->ftpAddr).ss_family == AF_INET6) {
      ...
      memcpy(&((struct sockaddr_in6 *)&dataAddr)->sin6_addr,
             &((struct sockaddr_in6 *)&ctxt->ftpAddr)->sin6_addr,
    ...
    if ((ctxt->ftpAddr).ss_family == AF_INET6) {
      ...
      inet_ntop(AF_INET6, &((struct sockaddr_in6 *)&dataAddr)->sin6_addr, buf6,
                INET6_ADDRSTRLEN);
...
#endif
    {
      ...
      snprintf(buf, sizeof(buf), "PORT %d,%d,%d,%d,%d,%d\r\n", adp[0] & 0xff,
               adp[1] & 0xff, adp[2] & 0xff, adp[3] & 0xff, portp[0] & 0xff,
               portp[1] & 0xff);
    }
...
static int xmlNanoFTPParseList(const char *list, ftpListCallback callback,
                               void *userData) {
  const char *cur = list;
  char filename[151];
  char attrib[11];
  ...
  if (callback != NULL) {
    callback(userData, filename, attrib, owner, group, size, links, year, month,
             day, hour, minute);
  }
  return (cur - list);
}
...
int xmlNanoFTPList(void *ctx, ftpListCallback callback, void *userData,
                   const char *filename) {
  xmlNanoFTPCtxtPtr ctxt = (xmlNanoFTPCtxtPtr)ctx;
  char buf[4096 + 1];
  int len, res;
  ...
  do {
    ...
    if ((len = recv(ctxt->dataFd, &buf[indx], sizeof(buf) - (indx + 1), 0)) <
        0) {
...
int xmlNanoFTPGet(void *ctx, ftpDataCallback callback, void *userData,
                  const char *filename) {
  xmlNanoFTPCtxtPtr ctxt = (xmlNanoFTPCtxtPtr)ctx;
  char buf[4096];
  int len = 0, res;
 ...
 * 									*
 ************************************************************************/
static void ftpList(void *userData, const char *filename, const char *attrib,
...
#include <stdio.h>
int main(int argc, char **argv) {
  xmlGenericError(xmlGenericErrorContext, "%s : FTP support not compiled in\n",
                  argv[0]);
  return (0);
}
#endif /* STANDALONE */
