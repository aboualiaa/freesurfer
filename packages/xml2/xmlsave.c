 ...
 */
static int xmlEscapeEntities(unsigned char *out, int *outlen, const xmlChar *in,
                             int *inlen) {
...
static void xmlAttrSerializeContent(xmlOutputBufferPtr buf, xmlAttrPtr attr) {
  ...
  while (children != NULL) {
    ...
    case XML_TEXT_NODE:
      xmlAttrSerializeTxtContent(buf->buffer, attr->doc, attr,
                                 children->content);
      break;
    case XML_ENTITY_REF_NODE:
      xmlBufferAdd(buf->buffer, BAD_CAST "&", 1);
      xmlBufferAdd(buf->buffer, children->name, xmlStrlen(children->name));
      xmlBufferAdd(buf->buffer, BAD_CAST ";", 1);
      break;
    default:
 ...
 */
static void xmlNsDumpOutput(xmlOutputBufferPtr buf, xmlNsPtr cur) {
...
static void xmlDtdDumpOutput(xmlSaveCtxtPtr ctxt, xmlDtdPtr dtd) {
  ...
  if ((dtd->notations != NULL) &&
      ((dtd->doc == NULL) || (dtd->doc->intSubset == dtd))) {
...
static void xmlNodeListDumpOutput(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
  ...
  while (cur != NULL) {
    if ((ctxt->format) && (xmlIndentTreeOutput) &&
        ((cur->type == XML_ELEMENT_NODE) || (cur->type == XML_COMMENT_NODE) ||
         (cur->type == XML_PI_NODE)))
      xmlOutputBufferWrite(buf,
                           ctxt->indent_size * (ctxt->level > ctxt->indent_nr
                                                    ? ctxt->indent_nr
                                                    : ctxt->level),
                           ctxt->indent);
...
static int htmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
  ...
  if ((encoding != NULL) && (oldctxtenc == NULL) && (buf->encoder == NULL) &&
      (buf->conv == NULL)) {
...
static int xmlDocContentDumpOutput(xmlSaveCtxtPtr ctxt, xmlDocPtr cur) {
  ...
  if (ctxt->encoding != NULL) {
    ...
    encoding =
        (const xmlChar *)xmlGetCharEncodingName((xmlCharEncoding)cur->charset);
  }
  ...
  if (((cur->type == XML_HTML_DOCUMENT_NODE) &&
    ...
    if ((encoding != NULL) && (oldctxtenc == NULL) && (buf->encoder == NULL) &&
        (buf->conv == NULL) && ((ctxt->options & XML_SAVE_NO_DECL) == 0)) {
  ...
  while (cur != NULL) {
    if ((cur->ns == NULL) && (xmlStrEqual(cur->name, BAD_CAST "id")))
      ...
      xml_lang = cur;
             ...
             ((cur->children == NULL) || (cur->children->content == NULL) ||
...
static void xhtmlNodeDumpOutput(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
  ...
  if (cur->nsDef)
    xmlNsListDumpOutput(buf, cur->nsDef);
  if ((xmlStrEqual(cur->name, BAD_CAST "html") && (cur->ns == NULL) &&
       (cur->nsDef == NULL))) {
    /*
  ...
  if ((cur->type == XML_ELEMENT_NODE) && (cur->parent != NULL) &&
...
xmlSaveCtxtPtr xmlSaveToBuffer(xmlBufferPtr buffer, const char *encoding,
                               int options) {
  xmlSaveCtxtPtr ret;
...
void xmlAttrSerializeTxtContent(xmlBufferPtr buf, xmlDocPtr doc,
  ...
  while (*cur != 0) {
    if (*cur == '\n') {
      ...
      cur++;
      base = cur;
    } else if ((*cur >= 0x80) && ((doc == NULL) || (doc->encoding == NULL))) {
      /*
 ...
 * returns: the number of bytes written or -1 in case of failure.
 */
int xmlDocDump(FILE *f, xmlDocPtr cur) { return (xmlDocFormatDump(f, cur, 0)); }
...
int xmlSaveFormatFileTo(xmlOutputBufferPtr buf, xmlDocPtr cur,
  ...
  if (buf == NULL)
    return (-1);
  if ((cur == NULL) || ((cur->type != XML_DOCUMENT_NODE) &&
                        (cur->type != XML_HTML_DOCUMENT_NODE))) {
    xmlOutputBufferClose(buf);
    return (-1);
  }
