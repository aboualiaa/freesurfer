 ...
 */
static xmlSchemaTypePtr xmlSchemaInitBasicType(const char *name,
                                               xmlSchemaValType type,
  ...
  xmlHashAddEntry2(xmlSchemaTypesBank, ret->name, XML_SCHEMAS_NAMESPACE_NAME,
                   ret);
...
void xmlSchemaInitTypes(void) {
  ...
  xmlSchemaTypeAnyTypeDef =
      xmlSchemaInitBasicType("anyType", XML_SCHEMAS_ANYTYPE, NULL);
  ...
  xmlSchemaTypeTimeDef = xmlSchemaInitBasicType("time", XML_SCHEMAS_TIME,
                                                xmlSchemaTypeAnySimpleTypeDef);
  xmlSchemaTypeGYearDef = xmlSchemaInitBasicType("gYear", XML_SCHEMAS_GYEAR,
                                                 xmlSchemaTypeAnySimpleTypeDef);
  ...
  xmlSchemaTypeGDayDef = xmlSchemaInitBasicType("gDay", XML_SCHEMAS_GDAY,
                                                xmlSchemaTypeAnySimpleTypeDef);
  xmlSchemaTypeDurationDef = xmlSchemaInitBasicType(
      "duration", XML_SCHEMAS_DURATION, xmlSchemaTypeAnySimpleTypeDef);
  xmlSchemaTypeFloatDef = xmlSchemaInitBasicType("float", XML_SCHEMAS_FLOAT,
                                                 xmlSchemaTypeAnySimpleTypeDef);
  ...
  xmlSchemaTypeBase64BinaryDef = xmlSchemaInitBasicType(
      "base64Binary", XML_SCHEMAS_BASE64BINARY, xmlSchemaTypeAnySimpleTypeDef);
  xmlSchemaTypeNotationDef = xmlSchemaInitBasicType(
      "NOTATION", XML_SCHEMAS_NOTATION, xmlSchemaTypeAnySimpleTypeDef);
  xmlSchemaTypeQNameDef = xmlSchemaInitBasicType("QName", XML_SCHEMAS_QNAME,
                                                 xmlSchemaTypeAnySimpleTypeDef);
  ...
  xmlSchemaTypeNonPositiveIntegerDef = xmlSchemaInitBasicType(
      "nonPositiveInteger", XML_SCHEMAS_NPINTEGER, xmlSchemaTypeIntegerDef);
  ...
  xmlSchemaTypeNonNegativeIntegerDef = xmlSchemaInitBasicType(
      "nonNegativeInteger", XML_SCHEMAS_NNINTEGER, xmlSchemaTypeIntegerDef);
  xmlSchemaTypeUnsignedLongDef = xmlSchemaInitBasicType(
      "unsignedLong", XML_SCHEMAS_ULONG, xmlSchemaTypeNonNegativeIntegerDef);
  ...
  xmlSchemaTypeTokenDef = xmlSchemaInitBasicType("token", XML_SCHEMAS_TOKEN,
  ...
  xmlSchemaTypeNCNameDef = xmlSchemaInitBasicType("NCName", XML_SCHEMAS_NCNAME,
                                                  xmlSchemaTypeNameDef);
  xmlSchemaTypeIdDef =
      xmlSchemaInitBasicType("ID", XML_SCHEMAS_ID, xmlSchemaTypeNCNameDef);
  xmlSchemaTypeIdrefDef = xmlSchemaInitBasicType("IDREF", XML_SCHEMAS_IDREF,
                                                 xmlSchemaTypeNCNameDef);
  xmlSchemaTypeEntityDef = xmlSchemaInitBasicType("ENTITY", XML_SCHEMAS_ENTITY,
                                                  xmlSchemaTypeNCNameDef);
  /*
   * Derived list types.
   */
...
void xmlSchemaCleanupTypes(void) {
   ...
   */
  {
    ...
    xmlSchemaFreeWildcard(
        (xmlSchemaWildcardPtr)particle->children->children->children);
...
static const unsigned int daysInMonth[12] = {31, 28, 31, 30, 31, 30,
                                             31, 31, 30, 31, 30, 31};
static const unsigned int daysInMonthLeap[12] = {31, 29, 31, 30, 31, 30,
                                                 31, 31, 30, 31, 30, 31};
...
#define VALID_MDAY(dt)                                                         \
  (IS_LEAP(dt->year) ? (dt->day <= daysInMonthLeap[dt->mon - 1])               \
...
static const long dayInYearByMonth[12] = {0,   31,  59,  90,  120, 151,
                                          181, 212, 243, 273, 304, 334};
static const long dayInLeapYearByMonth[12] = {0,   31,  60,  91,  121, 152,
                                              182, 213, 244, 274, 305, 335};

#define DAY_IN_YEAR(day, month, year)                                          \
  ((IS_LEAP(year) ? dayInLeapYearByMonth[month - 1]                            \
...
#define DEBUG_DATE(dt)                                                         \
  xmlGenericError(                                                             \
      xmlGenericErrorContext, "type=%o %04ld-%02u-%02uT%02u:%02u:%03f",        \
      dt->type, dt->value.date.year, dt->value.date.mon, dt->value.date.day,   \
      dt->value.date.hour, dt->value.date.min, dt->value.date.sec);            \
 ...
 * Returns 0 or the error code
 */
static int _xmlSchemaParseTimeZone(xmlSchemaValDatePtr dt,
                                   const xmlChar **str) {
  const xmlChar *cur;
  int ret = 0;
...
#define RETURN_TYPE_IF_VALID(t)                                                \
  ...
  if ((type != XML_SCHEMAS_UNKNOWN) && (type != dt->type)) {
    ...
    if ((type == XML_SCHEMAS_DATE) && ((dt->type != XML_SCHEMAS_GYEAR) ||
                                       (dt->type != XML_SCHEMAS_GYEARMONTH)))
      goto error;
 ...
 *         number otherwise
 */
static int xmlSchemaValAtomicListNode(xmlSchemaTypePtr type,
                                      const xmlChar *value,
...
static int xmlSchemaValAtomicType(xmlSchemaTypePtr type, const xmlChar *value,
                                  ...
                                  int flags, xmlSchemaWhitespaceValueType ws,
                                  int normOnTheFly, int applyNorm,
        ...
        if (len != 0) {
          ...
          if (len != 0) {
            *cptr = 0;
            ...
            xmlSchemaParseUInt((const xmlChar **)&cptr, &v->value.decimal.lo,
                               &v->value.decimal.mi, &v->value.decimal.hi);
          }
        }
        /*
  ...
  case XML_SCHEMAS_BASE64BINARY: {
    ...
    if (pad == 0) {
      ...
      if (i % 4 != 3)
        goto return1;
      for (decc = _xmlSchemaBase64Decode(*cur); (decc < 0) || (decc > 63);
           decc = _xmlSchemaBase64Decode(*cur))
        --cur;
...
static int xmlSchemaCompareDecimals(xmlSchemaValPtr x, xmlSchemaValPtr y) {
  ...
  if ((x->value.decimal.sign) &&
      ((x->value.decimal.lo != 0) || (x->value.decimal.mi != 0) ||
 ...
 */
static xmlSchemaValPtr _xmlSchemaDateAdd(xmlSchemaValPtr dt,
                                         xmlSchemaValPtr dur) {
 ...
 */
static xmlSchemaValPtr xmlSchemaDateNormalize(xmlSchemaValPtr dt,
                                              double offset) {
  ...
  if (((dt->type != XML_SCHEMAS_TIME) && (dt->type != XML_SCHEMAS_DATETIME) &&
       (dt->type != XML_SCHEMAS_DATE)) ||
      (dt->value.date.tzo == 0))
 ...
 * case of error
 */
static int xmlSchemaComparePreserveReplaceStrings(const xmlChar *x,
                                                  const xmlChar *y,
                                                  int invert) {
  int tmp;
 ...
 * case of error
 */
static int xmlSchemaComparePreserveCollapseStrings(const xmlChar *x,
                                                   const xmlChar *y,
                                                   int invert) {
  int tmp;
 ...
 * case of error
 */
static int xmlSchemaCompareReplaceCollapseStrings(const xmlChar *x,
                                                  const xmlChar *y,
                                                  int invert) {
  int tmp;
 ...
 */
static int xmlSchemaCompareValuesInternal(
    xmlSchemaValType xtype, xmlSchemaValPtr x, const xmlChar *xvalue,
    xmlSchemaWhitespaceValueType xws, xmlSchemaValType ytype, xmlSchemaValPtr y,
    const xmlChar *yvalue, xmlSchemaWhitespaceValueType yws) {
  switch (xtype) {
  case XML_SCHEMAS_UNKNOWN:
  case XML_SCHEMAS_ANYTYPE:
    return (-2);
  ...
  case XML_SCHEMAS_DECIMAL:
    ...
    if ((ytype == XML_SCHEMAS_DECIMAL) || (ytype == XML_SCHEMAS_INTEGER) ||
        (ytype == XML_SCHEMAS_NPINTEGER) || (ytype == XML_SCHEMAS_NINTEGER) ||
        (ytype == XML_SCHEMAS_NNINTEGER) || (ytype == XML_SCHEMAS_PINTEGER) ||
        (ytype == XML_SCHEMAS_INT) || (ytype == XML_SCHEMAS_UINT) ||
        (ytype == XML_SCHEMAS_LONG) || (ytype == XML_SCHEMAS_ULONG) ||
        (ytype == XML_SCHEMAS_SHORT) || (ytype == XML_SCHEMAS_USHORT) ||
  ...
  case XML_SCHEMAS_DATETIME:
    if ((x == NULL) || (y == NULL))
      return (-2);
    if ((ytype == XML_SCHEMAS_DATETIME) || (ytype == XML_SCHEMAS_TIME) ||
        (ytype == XML_SCHEMAS_GDAY) || (ytype == XML_SCHEMAS_GMONTH) ||
        (ytype == XML_SCHEMAS_GMONTHDAY) || (ytype == XML_SCHEMAS_GYEAR) ||
  ...
  case XML_SCHEMAS_ANYURI: {
    ...
    if ((ytype == XML_SCHEMAS_ANYSIMPLETYPE) || (ytype == XML_SCHEMAS_STRING) ||
        (ytype == XML_SCHEMAS_NORMSTRING) || (ytype == XML_SCHEMAS_TOKEN) ||
        (ytype == XML_SCHEMAS_LANGUAGE) || (ytype == XML_SCHEMAS_NMTOKEN) ||
        (ytype == XML_SCHEMAS_NAME) || (ytype == XML_SCHEMAS_NCNAME) ||
        (ytype == XML_SCHEMAS_ID) || (ytype == XML_SCHEMAS_IDREF) ||
        (ytype == XML_SCHEMAS_ENTITY) || (ytype == XML_SCHEMAS_ANYURI)) {
...
int xmlSchemaCompareValues(xmlSchemaValPtr x, xmlSchemaValPtr y) {
  ...
  return (xmlSchemaCompareValuesInternal(x->type, x, NULL, xws, y->type, y,
                                         NULL, yws));
}
 ...
 * case of error
 */
static int xmlSchemaCompareValuesWhtspExt(
    xmlSchemaValType xtype, xmlSchemaValPtr x, const xmlChar *xvalue,
    xmlSchemaWhitespaceValueType xws, xmlSchemaValType ytype, xmlSchemaValPtr y,
    const xmlChar *yvalue, xmlSchemaWhitespaceValueType yws) {
 ...
 */
static int xmlSchemaValidateLengthFacetInternal(
    xmlSchemaFacetPtr facet, xmlSchemaTypeType valType, const xmlChar *value,
    xmlSchemaValPtr val, unsigned long *length,
...
int xmlSchemaValidateLengthFacet(xmlSchemaTypePtr type, xmlSchemaFacetPtr facet,
  ...
  if (type == NULL)
    return (-1);
  return (xmlSchemaValidateLengthFacetInternal(facet, type->builtInType, value,
                                               val, length,
...
static int xmlSchemaValidateFacetInternal(xmlSchemaFacetPtr facet,
  ...
  case XML_SCHEMA_FACET_ENUMERATION:
    if (ws == XML_SCHEMA_WHITESPACE_UNKNOWN) {
      ...
      ret = xmlSchemaCompareValuesWhtspExt(facet->val->type, facet->val,
                                           facet->value, fws, valType, val,
                                           value, ws);
      if (ret == -2)
        return (-1);
      if (ret == 0)
  ...
  case XML_SCHEMA_FACET_FRACTIONDIGITS:
    ...
    if ((val == NULL) ||
        ((val->type != XML_SCHEMAS_DECIMAL) &&
         ...
         (val->type != XML_SCHEMAS_LONG) && (val->type != XML_SCHEMAS_ULONG) &&
         (val->type != XML_SCHEMAS_SHORT) &&
         (val->type != XML_SCHEMAS_USHORT) && (val->type != XML_SCHEMAS_BYTE) &&
...
int xmlSchemaValidateFacet(xmlSchemaTypePtr base, xmlSchemaFacetPtr facet,
   ...
   */
  if (val != NULL)
    return (xmlSchemaValidateFacetInternal(facet, XML_SCHEMA_WHITESPACE_UNKNOWN,
                                           val->type, value, val,
                                           XML_SCHEMA_WHITESPACE_UNKNOWN));
  else if (base != NULL)
    return (xmlSchemaValidateFacetInternal(facet, XML_SCHEMA_WHITESPACE_UNKNOWN,
                                           base->builtInType, value, val,
                                           XML_SCHEMA_WHITESPACE_UNKNOWN));
  return (-1);
}
 ...
 * @fws: the whitespace type of the facet's value
 * @valType: the built-in type of the value
 * @value:  the lexical (or normalized for pattern) repr of the value to
 * validate
 * @val:  the precomputed value
 * @ws: the whitespace type of the value
 *
  ...
  *retValue = NULL;
  ...
  case XML_SCHEMAS_DECIMAL:
    ...
    if ((val->value.decimal.total == 1) && (val->value.decimal.lo == 0)) {
      *retValue = xmlStrdup(BAD_CAST "0.0");
      ...
      if (dec.hi != 0)
        snprintf(offs, bufsize - (offs - buf), "%lu%lu%lu", dec.hi, dec.mi,
                 dec.lo);
      else if (dec.mi != 0)
  ...
  case XML_SCHEMAS_UBYTE:
    if ((val->value.decimal.total == 1) && (val->value.decimal.lo == 0))
      ...
      *retValue = xmlMalloc(bufsize);
      ...
      if (dec.hi != 0) {
        if (dec.sign)
          snprintf((char *)*retValue, bufsize, "-%lu%lu%lu", dec.hi, dec.mi,
                   dec.lo);
        else
          snprintf((char *)*retValue, bufsize, "%lu%lu%lu", dec.hi, dec.mi,
                   dec.lo);
  ...
  case XML_SCHEMAS_DURATION: {
    ...
    if ((val->value.dur.mon < 0) || (val->value.dur.sec < 0))
      snprintf(buf, 100, "P%luY%luM%luDT%luH%luM%.14gS", year, mon, day, hour,
               min, sec);
    else
      snprintf(buf, 100, "-P%luY%luM%luDT%luH%luM%.14gS", year, mon, day, hour,
               min, sec);
  ...
  case XML_SCHEMAS_GMONTHDAY: {
    ...
    *retValue = xmlMalloc(8);
    if (*retValue == NULL)
      return (-1);
    snprintf((char *)*retValue, 8, "--%02u-%02u", val->value.date.mon,
             val->value.date.day);
  } break;
  case XML_SCHEMAS_GYEARMONTH: {
    char buf[35];
    ...
    if (val->value.date.year < 0)
      snprintf(buf, 35, "-%04ld-%02u", labs(val->value.date.year),
               val->value.date.mon);
    else
      snprintf(buf, 35, "%04ld-%02u", val->value.date.year,
               val->value.date.mon);
    *retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
  } break;
  case XML_SCHEMAS_TIME: {
    ...
    if (val->value.date.tz_flag) {
       ...
       */
      snprintf(buf, 30, "%02u:%02u:%02.14gZ", norm->value.date.hour,
               norm->value.date.min, norm->value.date.sec);
      xmlSchemaFreeValue(norm);
    } else {
      snprintf(buf, 30, "%02u:%02u:%02.14g", val->value.date.hour,
  ...
  case XML_SCHEMAS_DATE: {
    ...
    if (val->value.date.tz_flag) {
       ...
       */
      snprintf(buf, 30, "%04ld:%02u:%02uZ", norm->value.date.year,
               norm->value.date.mon, norm->value.date.day);
      xmlSchemaFreeValue(norm);
    } else {
      snprintf(buf, 30, "%04ld:%02u:%02u", val->value.date.year,
  ...
  case XML_SCHEMAS_DATETIME: {
    ...
    if (val->value.date.tz_flag) {
      ...
      snprintf(buf, 50, "%04ld:%02u:%02uT%02u:%02u:%02.14g",
               val->value.date.year, val->value.date.mon, val->value.date.day,
               val->value.date.hour, val->value.date.min, val->value.date.sec);
    }
    *retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
  } break;
  case XML_SCHEMAS_HEXBINARY:
 ...
 * not supported yet and -1 in case of API errors.
 */
int xmlSchemaGetCanonValueWhtsp(xmlSchemaValPtr val, const xmlChar **retValue,
                                xmlSchemaWhitespaceValueType ws) {
  if ((retValue == NULL) || (val == NULL))
    return (-1);
  if ((ws == XML_SCHEMA_WHITESPACE_UNKNOWN) ||
