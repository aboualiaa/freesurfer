...
static int xmlNsCheckScope(xmlNodePtr node, xmlNsPtr ns) {
  ...
  while ((node != NULL) &&
         ((node->type == XML_ELEMENT_NODE) ||
          (node->type == XML_ATTRIBUTE_NODE) || (node->type == XML_TEXT_NODE) ||
          (node->type == XML_XINCLUDE_START))) {
...
static void xmlDebugErr(xmlDebugCtxtPtr ctxt, int error, const char *msg) {
  ctxt->errors++;
  __xmlRaiseError(NULL, NULL, NULL, NULL, ctxt->node, XML_FROM_CHECK, error,
                  XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, 0, 0, "%s", msg);
}
static void xmlDebugErr2(xmlDebugCtxtPtr ctxt, int error, const char *msg,
                         int extra) {
  ctxt->errors++;
  __xmlRaiseError(NULL, NULL, NULL, NULL, ctxt->node, XML_FROM_CHECK, error,
                  XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, 0, 0, msg, extra);
}
static void xmlDebugErr3(xmlDebugCtxtPtr ctxt, int error, const char *msg,
                         const char *extra) {
  ctxt->errors++;
  __xmlRaiseError(NULL, NULL, NULL, NULL, ctxt->node, XML_FROM_CHECK, error,
                  XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, 0, 0, msg, extra);
 ...
 */
static void xmlCtxtNsCheckScope(xmlDebugCtxtPtr ctxt, xmlNodePtr node,
                                xmlNsPtr ns) {
  int ret;
...
static void xmlCtxtCheckString(xmlDebugCtxtPtr ctxt, const xmlChar *str) {
  ...
  if (ctxt->check) {
    if (!xmlCheckUTF8(str)) {
      xmlDebugErr3(ctxt, XML_CHECK_NOT_UTF8, "String is not UTF-8 %s",
                   (const char *)str);
    }
  }
}
...
static void xmlCtxtCheckName(xmlDebugCtxtPtr ctxt, const xmlChar *name) {
  if (ctxt->check) {
    ...
    if (xmlValidateName(name, 0)) {
      xmlDebugErr3(ctxt, XML_CHECK_NOT_NCNAME, "Name is not an NCName '%s'",
                   (const char *)name);
    }
    if ((ctxt->dict != NULL) && (!xmlDictOwns(ctxt->dict, name)) &&
        ((ctxt->doc == NULL) || ((ctxt->doc->parseFlags &
                                  (XML_PARSE_SAX1 | XML_PARSE_NODICT)) == 0))) {
...
static void xmlCtxtGenericNodeCheck(xmlDebugCtxtPtr ctxt, xmlNodePtr node) {
  ...
  if ((node->type != XML_ELEMENT_NODE) && (node->type != XML_ATTRIBUTE_NODE) &&
      ...
      (node->type != XML_DTD_NODE) && (node->type != XML_ELEMENT_DECL) &&
      (node->type != XML_HTML_DOCUMENT_NODE) &&
  ...
  case XML_TEXT_NODE:
    ...
    if ((ctxt->dict != NULL) &&
        (node->name == xmlDictLookup(ctxt->dict, BAD_CAST "nbktext", 7)))
      break;

    xmlDebugErr3(ctxt, XML_CHECK_WRONG_NAME, "Text node has wrong name '%s'",
                 (const char *)node->name);
    break;
  case XML_COMMENT_NODE:
    if (node->name == xmlStringComment)
      break;
    xmlDebugErr3(ctxt, XML_CHECK_WRONG_NAME, "Comment node has wrong name '%s'",
                 (const char *)node->name);
    break;
  case XML_PI_NODE:
    xmlCtxtCheckName(ctxt, node->name);
...
static void xmlCtxtDumpEntityCallback(xmlEntityPtr cur, xmlDebugCtxtPtr ctxt) {
  if (cur == NULL) {
    if (!ctxt->check)
      fprintf(ctxt->output, "Entity is NULL");
    return;
...
void xmlShellPrintXPathError(int errorType, const char *arg) {
  ...
  case XPATH_UNDEFINED:
    xmlGenericError(xmlGenericErrorContext, "%s: no such node\n", arg);
    break;

  case XPATH_BOOLEAN:
    xmlGenericError(xmlGenericErrorContext, "%s is a Boolean\n", arg);
    break;
  case XPATH_NUMBER:
    xmlGenericError(xmlGenericErrorContext, "%s is a number\n", arg);
    break;
  case XPATH_STRING:
    xmlGenericError(xmlGenericErrorContext, "%s is a string\n", arg);
    break;
  case XPATH_POINT:
    xmlGenericError(xmlGenericErrorContext, "%s is a point\n", arg);
    break;
  case XPATH_RANGE:
    xmlGenericError(xmlGenericErrorContext, "%s is a range\n", arg);
    break;
  case XPATH_LOCATIONSET:
    xmlGenericError(xmlGenericErrorContext, "%s is a range\n", arg);
    break;
  case XPATH_USERS:
    xmlGenericError(xmlGenericErrorContext, "%s is user-defined\n", arg);
    break;
  case XPATH_XSLT_TREE:
    xmlGenericError(xmlGenericErrorContext, "%s is an XSLT value tree\n", arg);
    break;
  }
#if 0
 ...
 * Print node to the output FILE
 */
void xmlShellPrintNode(xmlNodePtr node) { xmlShellPrintNodeCtxt(NULL, node); }
 ...
 */
static void xmlShellPrintXPathResultCtxt(xmlShellCtxtPtr ctxt,
  ...
  if (list != NULL) {
    ...
    case XPATH_NODESET: {
      ...
      if (list->nodesetval) {
        ...
        xmlGenericError(xmlGenericErrorContext, "Empty node set\n");
      }
      break;
#else
    ...
    case XPATH_BOOLEAN:
      xmlGenericError(xmlGenericErrorContext, "Is a Boolean:%s\n",
                      xmlBoolToText(list->boolval));
      break;
    case XPATH_NUMBER:
      xmlGenericError(xmlGenericErrorContext, "Is a number:%0g\n",
                      list->floatval);
      break;
    case XPATH_STRING:
      xmlGenericError(xmlGenericErrorContext, "Is a string:%s\n",
                      list->stringval);
      break;
 ...
 * Returns 0
 */
int xmlShellList(xmlShellCtxtPtr ctxt, char *arg ATTRIBUTE_UNUSED,
 ...
 * Returns 0
 */
int xmlShellBase(xmlShellCtxtPtr ctxt, char *arg ATTRIBUTE_UNUSED,
                 xmlNodePtr node, xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlChar *base;
  if (!ctxt)
    return 0;
 ...
 * Returns 0 on success and a negative value otherwise.
 */
static int xmlShellRegisterRootNamespaces(xmlShellCtxtPtr ctxt,
 ...
 * Returns 0
 */
static int xmlShellGrep(xmlShellCtxtPtr ctxt ATTRIBUTE_UNUSED, char *arg,
                        xmlNodePtr node, xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  if (!ctxt)
    return (0);
  if (node == NULL)
  ...
  if ((xmlStrchr((xmlChar *)arg, '?')) || (xmlStrchr((xmlChar *)arg, '*')) ||
      (xmlStrchr((xmlChar *)arg, '.')) || (xmlStrchr((xmlChar *)arg, '['))) {
  }
#endif
  while (node != NULL) {
 ...
 * Returns 0
 */
int xmlShellDir(xmlShellCtxtPtr ctxt ATTRIBUTE_UNUSED,
                char *arg ATTRIBUTE_UNUSED, xmlNodePtr node,
                xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  if (!ctxt)
    return (0);
...
static int xmlShellRNGValidate(xmlShellCtxtPtr sctxt, char *schemas,
  ...
  xmlRelaxNGSetValidErrors(vctxt, (xmlRelaxNGValidityErrorFunc)fprintf,
                           (xmlRelaxNGValidityWarningFunc)fprintf, stderr);
...
int xmlShellWrite(xmlShellCtxtPtr ctxt, char *filename, xmlNodePtr node,
  ...
  if (access((char *)filename, W_OK)) {
    xmlGenericError(xmlGenericErrorContext, "Cannot write to %s\n", filename);
    return (-1);
  }
#endif
  switch (node->type) {
  case XML_DOCUMENT_NODE:
    if (xmlSaveFile((char *)filename, ctxt->doc) < -1) {
      xmlGenericError(xmlGenericErrorContext, "Failed to write to %s\n",
                      filename);
      return (-1);
    }
    break;
...
int xmlShellSave(xmlShellCtxtPtr ctxt, char *filename,
...
#ifdef W_OK
  if (access((char *)filename, W_OK)) {
    xmlGenericError(xmlGenericErrorContext, "Cannot save to %s\n", filename);
    return (-1);
  }
#endif
  ...
  case XML_DOCUMENT_NODE:
    if (xmlSaveFile((char *)filename, ctxt->doc) < 0) {
      xmlGenericError(xmlGenericErrorContext, "Failed to save to %s\n",
                      filename);
    }
    break;
  case XML_HTML_DOCUMENT_NODE:
 ...
 * Returns 0 or -1 in case of error
 */
int xmlShellValidate(xmlShellCtxtPtr ctxt, char *dtd,
                     xmlNodePtr node ATTRIBUTE_UNUSED,
                     xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlValidCtxt vctxt;
  int res = -1;
 ...
 * Returns 0 or -1 in case of error
 */
int xmlShellDu(xmlShellCtxtPtr ctxt, char *arg ATTRIBUTE_UNUSED,
               xmlNodePtr tree, xmlNodePtr node2 ATTRIBUTE_UNUSED) {
  xmlNodePtr node;
  int indent = 0, i;
...
void xmlShell(xmlDocPtr doc, char *filename, xmlShellReadlineFunc input,
  ...
  while (1) {
    ...
    if (!strcmp(command, "help")) {
      ...
      fprintf(ctxt->output, "\tdir [path]   dumps informations about the node "
                            "(namespace, attributes, content)\n");
      fprintf(ctxt->output, "\tdu [path]    show the structure of the subtree "
                            "under path or the current node\n");
      ...
      fprintf(ctxt->output, "\tset xml_fragment replace the current node "
                            "content with the fragment parsed in context\n");
#ifdef LIBXML_XPATH_ENABLED
      fprintf(ctxt->output, "\txpath expr   evaluate the XPath expression in "
                            "that context and print the result\n");
      fprintf(ctxt->output, "\tsetns nsreg  register a namespace to a prefix "
                            "in the XPath evaluation context\n");
      fprintf(ctxt->output, "\t             format for nsreg is: "
                            "prefix=[nsuri] (i.e. prefix= unsets a prefix)\n");
      fprintf(
          ctxt->output,
      ...
      if (arg[0] == 0) {
        xmlGenericError(xmlGenericErrorContext, "xpath: expression required\n");
      } else {
          ...
          case XPATH_NODESET:
            if (list->nodesetval != NULL) {
              if (list->nodesetval->nodeNr == 1) {
                ...
                if ((ctxt->node != NULL) &&
                    (ctxt->node->type == XML_NAMESPACE_DECL)) {
                  xmlGenericError(xmlGenericErrorContext,
                                  "cannot cd to namespace\n");
                  ctxt->node = NULL;
                }
              } else
                xmlGenericError(xmlGenericErrorContext, "%s is a %d Node Set\n",
      ...
      xmlGenericError(xmlGenericErrorContext, "Unknown command %s\n", command);
    }
