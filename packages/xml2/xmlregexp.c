...
static int xmlRegCheckCharacterRange(xmlRegAtomType type, int codepoint,
                                     int neg, int start, int end,
                                     const xmlChar *blockName);
...
static void xmlRegexpErrMemory(xmlRegParserCtxtPtr ctxt, const char *extra) {
  ...
  __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_REGEXP,
                  XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, regexp,
...
#endif
    transitions =
        (int *)xmlMalloc((nbstates + 1) * (nbatoms + 1) * sizeof(int));
    if (transitions == NULL) {
      xmlFree(stateRemap);
      xmlFree(stringRemap);
      xmlFree(stringMap);
    ...
    for (i = 0; i < ret->nbStates; i++) {
      ...
      for (j = 0; j < state->nbTrans; j++) {
        ...
        if ((trans->atom->data != NULL) && (transdata == NULL)) {
          transdata = (void **)xmlMalloc(nbstates * nbatoms * sizeof(void *));
          if (transdata != NULL)
            memset(transdata, 0, nbstates * nbatoms * sizeof(void *));
          else {
            xmlRegexpErrMemory(ctxt, "compiling regexp");
            break;
          }
        ...
        if (prev != 0) {
          if (prev != targetno + 1) {
            ...
            printf("Indet: state %d trans %d, atom %d to %d : %d to %d\n", i, j,
                   trans->atom->no, trans->to, atomno, targetno);
 ...
 */
static xmlRegRangePtr xmlRegNewRange(xmlRegParserCtxtPtr ctxt, int neg,
                                     xmlRegAtomType type, int start, int end) {
  xmlRegRangePtr ret;
 ...
 */
static xmlRegRangePtr xmlRegCopyRange(xmlRegParserCtxtPtr ctxt,
 ...
 */
static xmlRegAtomPtr xmlRegNewAtom(xmlRegParserCtxtPtr ctxt,
                                   xmlRegAtomType type) {
  xmlRegAtomPtr ret;
 ...
 * Returns the new atom or NULL in case of error
 */
static xmlRegAtomPtr xmlRegCopyAtom(xmlRegParserCtxtPtr ctxt,
                                    xmlRegAtomPtr atom) {
  xmlRegAtomPtr ret;
  ...
  if (atom->nbRanges > 0) {
    ...
    ret->ranges =
        (xmlRegRangePtr *)xmlMalloc(sizeof(xmlRegRangePtr) * atom->nbRanges);
    if (ret->ranges == NULL) {
      xmlRegexpErrMemory(ctxt, "copying atom");
      goto error;
    }
...
static void xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom,
  ...
  if (atom->maxRanges == 0) {
    atom->maxRanges = 4;
    atom->ranges =
        (xmlRegRangePtr *)xmlMalloc(atom->maxRanges * sizeof(xmlRegRangePtr));
    if (atom->ranges == NULL) {
...
static int xmlRegGetCounter(xmlRegParserCtxtPtr ctxt) {
  if (ctxt->maxCounters == 0) {
    ctxt->maxCounters = 4;
    ctxt->counters =
        (xmlRegCounter *)xmlMalloc(ctxt->maxCounters * sizeof(xmlRegCounter));
    if (ctxt->counters == NULL) {
    ...
    xmlRegCounter *tmp;
    ctxt->maxCounters *= 2;
    tmp = (xmlRegCounter *)xmlRealloc(
        ctxt->counters, ctxt->maxCounters * sizeof(xmlRegCounter));
    if (tmp == NULL) {
      xmlRegexpErrMemory(ctxt, "allocating counter");
      ctxt->maxCounters /= 2;
      return (-1);
...
static int xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
  ...
  if (ctxt->maxAtoms == 0) {
    ctxt->maxAtoms = 4;
    ctxt->atoms =
        (xmlRegAtomPtr *)xmlMalloc(ctxt->maxAtoms * sizeof(xmlRegAtomPtr));
    if (ctxt->atoms == NULL) {
    ...
    xmlRegAtomPtr *tmp;
    ctxt->maxAtoms *= 2;
    tmp = (xmlRegAtomPtr *)xmlRealloc(ctxt->atoms,
                                      ctxt->maxAtoms * sizeof(xmlRegAtomPtr));
    if (tmp == NULL) {
      xmlRegexpErrMemory(ctxt, "allocating counter");
      ctxt->maxAtoms /= 2;
      return (-1);
...
static void xmlRegStateAddTransTo(xmlRegParserCtxtPtr ctxt,
                                  xmlRegStatePtr target, int from) {
  if (target->maxTransTo == 0) {
    target->maxTransTo = 8;
    target->transTo = (int *)xmlMalloc(target->maxTransTo * sizeof(int));
    ...
    int *tmp;
    target->maxTransTo *= 2;
    tmp = (int *)xmlRealloc(target->transTo, target->maxTransTo * sizeof(int));
    if (tmp == NULL) {
      xmlRegexpErrMemory(ctxt, "adding transition");
      target->maxTransTo /= 2;
      return;
...
static void xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
  ...
  if (state->maxTrans == 0) {
    state->maxTrans = 8;
    state->trans =
        (xmlRegTrans *)xmlMalloc(state->maxTrans * sizeof(xmlRegTrans));
    if (state->trans == NULL) {
      xmlRegexpErrMemory(ctxt, "adding transition");
      state->maxTrans = 0;
      return;
    ...
    xmlRegTrans *tmp;
    state->maxTrans *= 2;
    tmp = (xmlRegTrans *)xmlRealloc(state->trans,
                                    state->maxTrans * sizeof(xmlRegTrans));
    if (tmp == NULL) {
      xmlRegexpErrMemory(ctxt, "adding transition");
      state->maxTrans /= 2;
      return;
...
static int xmlRegStatePush(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state) {
  ...
  if (ctxt->maxStates == 0) {
    ctxt->maxStates = 4;
    ctxt->states =
        (xmlRegStatePtr *)xmlMalloc(ctxt->maxStates * sizeof(xmlRegStatePtr));
    if (ctxt->states == NULL) {
    ...
    xmlRegStatePtr *tmp;
    ctxt->maxStates *= 2;
    tmp = (xmlRegStatePtr *)xmlRealloc(
        ctxt->states, ctxt->maxStates * sizeof(xmlRegStatePtr));
    if (tmp == NULL) {
      xmlRegexpErrMemory(ctxt, "adding state");
      ctxt->maxStates /= 2;
      return (-1);
 ...
 *
 */
static void xmlFAGenerateAllTransition(xmlRegParserCtxtPtr ctxt,
 ...
 *
 */
static void xmlFAGenerateEpsilonTransition(xmlRegParserCtxtPtr ctxt,
                                           xmlRegStatePtr from,
                                           xmlRegStatePtr to) {
  if (to == NULL) {
    to = xmlRegNewState(ctxt);
 ...
 *
 */
static void xmlFAGenerateCountedEpsilonTransition(xmlRegParserCtxtPtr ctxt,
                                                  xmlRegStatePtr from,
                                                  xmlRegStatePtr to,
                                                  int counter) {
  if (to == NULL) {
 ...
 *
 */
static void xmlFAGenerateCountedTransition(xmlRegParserCtxtPtr ctxt,
                                           xmlRegStatePtr from,
                                           xmlRegStatePtr to, int counter) {
  if (to == NULL) {
    to = xmlRegNewState(ctxt);
    xmlRegStatePush(ctxt, to);
    ctxt->state = to;
 ...
 */
static int xmlFAGenerateTransitions(xmlRegParserCtxtPtr ctxt,
                                    xmlRegStatePtr from, xmlRegStatePtr to,
                                    xmlRegAtomPtr atom) {
  xmlRegStatePtr end;
  ...
  if (atom->type == XML_REGEXP_SUBREG) {
    ...
    case XML_REGEXP_QUANT_RANGE: {
      ...
      if ((atom->min == 0) && (atom->start0 == NULL)) {
        ...
        ctxt->counters[counter].max = atom->max - 1;
        /* count the number of times we see it again */
        xmlFAGenerateCountedEpsilonTransition(ctxt, atom->stop, atom->start,
                                              counter);
        /* allow a way out based on the count */
 ...
 *
 */
static void xmlFAReduceEpsilonTransitions(xmlRegParserCtxtPtr ctxt, int fromnr,
    ...
    if (to->trans[transnr].atom == NULL) {
      ...
      if (to->trans[transnr].to != fromnr) {
        if (to->trans[transnr].count >= 0) {
          ...
          xmlRegStateAddTrans(ctxt, from, NULL, ctxt->states[newto], -1,
                              to->trans[transnr].count);
        } else {
#ifdef DEBUG_REGEXP_GRAPH
          ...
          if (to->trans[transnr].counter >= 0) {
            xmlFAReduceEpsilonTransitions(ctxt, fromnr, to->trans[transnr].to,
      ...
      if (to->trans[transnr].counter >= 0) {
        xmlRegStateAddTrans(ctxt, from, to->trans[transnr].atom,
                            ctxt->states[newto], to->trans[transnr].counter,
                            -1);
      } else {
        xmlRegStateAddTrans(ctxt, from, to->trans[transnr].atom,
          ...
          for (j = 0; j < tmp->nbTrans; j++) {
            if (tmp->trans[j].to == statenr) {
              ...
              xmlRegStateAddTrans(ctxt, tmp, tmp->trans[j].atom,
                                  ctxt->states[newto], tmp->trans[j].counter,
                                  tmp->trans[j].count);
            }
          }
        }
...
static int xmlFACompareRanges(xmlRegRangePtr range1, xmlRegRangePtr range2) {
  ...
  if ((range1->type == XML_REGEXP_ANYCHAR) ||
    ...
    for (codepoint = range1->start; codepoint <= range1->end; codepoint++) {
      ret = xmlRegCheckCharacterRange(range2->type, codepoint, 0, range2->start,
                                      range2->end, range2->blockName);
      if (ret < 0)
        return (-1);
 ...
 */
static int xmlFACompareAtoms(xmlRegAtomPtr atom1, xmlRegAtomPtr atom2,
                             int deep) {
  int ret = 1;
 ...
 *
 */
static int xmlFARecurseDeterminism(xmlRegParserCtxtPtr ctxt,
                                   xmlRegStatePtr state, int to,
                                   xmlRegAtomPtr atom) {
  int ret = 1;
  int res;
  int transnr, nbTrans;
    ...
    for (transnr = 0; transnr < state->nbTrans; transnr++) {
      ...
      for (i = 0; i < transnr; i++) {
        ...
        if (t2->atom != NULL) {
           ...
           */
          ret = xmlFARecurseDeterminism(ctxt, ctxt->states[t1->to], t2->to,
                                        t2->atom);
...
static int xmlRegCheckCharacterRange(xmlRegAtomType type, int codepoint,
  ...
  case XML_REGEXP_NOTNAMECHAR:
    neg = !neg;
  case XML_REGEXP_NAMECHAR:
    ret = (IS_LETTER(codepoint) || IS_DIGIT(codepoint) || (codepoint == '.') ||
...
static int xmlRegCheckCharacter(xmlRegAtomPtr atom, int codepoint) {
  ...
  case XML_REGEXP_RANGES: {
    ...
    for (i = 0; i < atom->nbRanges; i++) {
      range = atom->ranges[i];
      if (range->neg == 2) {
        ret = xmlRegCheckCharacterRange(range->type, codepoint, 0, range->start,
                                        range->end, range->blockName);
        if (ret != 0)
...
static void xmlFARegDebugExec(xmlRegExecCtxtPtr exec) {
  ...
  if (exec->inputStack != NULL) {
    ...
    for (i = 0; (i < 3) && (i < exec->inputStackNr); i++)
      printf(
          "%s ",
          (const char *)exec->inputStack[exec->inputStackNr - (i + 1)].value);
  } else {
...
static void xmlFARegExecSave(xmlRegExecCtxtPtr exec) {
  ...
  if (exec->maxRollbacks == 0) {
    exec->maxRollbacks = 4;
    exec->rollbacks = (xmlRegExecRollback *)xmlMalloc(
    ...
    memset(exec->rollbacks, 0, exec->maxRollbacks * sizeof(xmlRegExecRollback));
  } else if (exec->nbRollbacks >= exec->maxRollbacks) {
    xmlRegExecRollback *tmp;
    int len = exec->maxRollbacks;

    exec->maxRollbacks *= 2;
    tmp = (xmlRegExecRollback *)xmlRealloc(
        exec->rollbacks, exec->maxRollbacks * sizeof(xmlRegExecRollback));
    if (tmp == NULL) {
      xmlRegexpErrMemory(NULL, "saving regexp");
      exec->maxRollbacks /= 2;
      return;
...
static int xmlFARegExec(xmlRegexpPtr comp, const xmlChar *content) {
  ...
  while ((exec->status == 0) &&
    ...
    for (; exec->transno < exec->state->nbTrans; exec->transno++) {
      ...
      if (trans->count >= 0) {
        ...
        if ((ret == 1) && (atom->min >= 0) && (atom->max > 0)) {
          ...
          if (trans->counter >= 0) {
            ...
            if ((exec->counts == NULL) || (exec->comp == NULL) ||
      ...
      if (ret == 1) {
        if ((trans->nd == 1) || ((trans->count >= 0) && (deter == 0) &&
...
static void xmlFARegExecSaveInputString(xmlRegExecCtxtPtr exec,
  ...
  if (exec->inputStackMax == 0) {
    ...
    exec->inputStack = (xmlRegInputTokenPtr)xmlMalloc(exec->inputStackMax *
    ...
    tmp = (xmlRegInputTokenPtr)xmlRealloc(
        exec->inputStack, exec->inputStackMax * sizeof(xmlRegInputToken));
    if (tmp == NULL) {
      xmlRegexpErrMemory(NULL, "pushing input string");
      exec->inputStackMax /= 2;
      return;
...
static int xmlRegStrEqualWildcard(const xmlChar *expStr,
 ...
 */
static int xmlRegCompactPushString(xmlRegExecCtxtPtr exec, xmlRegexpPtr comp,
                                   const xmlChar *value, void *data) {
 ...
 *     a negative value in case of error.
 */
static int xmlRegExecPushStringInternal(xmlRegExecCtxtPtr exec,
                                        const xmlChar *value, void *data,
                                        int compound) {
  xmlRegTransPtr trans;
  xmlRegAtomPtr atom;
  ...
  while ((exec->status == 0) &&
         ((value != NULL) ||
          ((final == 1) && (exec->state->type != XML_REGEXP_FINAL_STATE)))) {
        ...
        if ((value == NULL) && (final)) {
          ...
          for (i = 0; i < exec->state->nbTrans; i++) {
            ...
            if ((count >= counter->min) && (count < counter->max) &&
                (t->atom != NULL) && (xmlStrEqual(value, t->atom->valuep))) {
              ret = 1;
              break;
            }
          }
 ...
 */
static int xmlRegExecGetValues(xmlRegExecCtxtPtr exec, int err, int *nbval,
 ...
 * Returns: 0 in case of success or -1 in case of error.
 */
int xmlRegExecErrInfo(xmlRegExecCtxtPtr exec, const xmlChar **string,
...
static void xmlFAParseCharClassEsc(xmlRegParserCtxtPtr ctxt) {
  ...
  if (CUR == '.') {
    if (ctxt->atom == NULL) {
      ...
      xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, XML_REGEXP_ANYCHAR, 0, 0,
                         NULL);
    }
    NEXT;
    return;
  ...
  if (cur == 'p') {
    ...
    if (ctxt->atom == NULL) {
      ...
      xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, XML_REGEXP_CHARVAL, cur,
                         cur, NULL);
    }
    NEXT;
...
typedef enum { XML_EXP_NILABLE = (1 << 0) } xmlExpNodeInfo;
...
static xmlExpNode forbiddenExpNode = {XML_EXP_FORBID, 0,    0,       0, 0,
                                      NULL,           NULL, {{0, 0}}};
xmlExpNodePtr forbiddenExp = &forbiddenExpNode;
static xmlExpNode emptyExpNode = {XML_EXP_EMPTY, 1,    0,       0, 0,
                                  NULL,          NULL, {{0, 0}}};
 ...
 */
static unsigned short xmlExpHashNameComputeKey(const xmlChar *name) {
 ...
 */
static unsigned short xmlExpHashComputeKey(xmlExpNodeType type,
                                           xmlExpNodePtr left,
                                           xmlExpNodePtr right) {
  unsigned long value;
  unsigned short ret;
...
static xmlExpNodePtr xmlExpHashGetEntry(xmlExpCtxtPtr ctxt, xmlExpNodeType type,
  ...
  if (type == XML_EXP_ATOM) {
    ...
    if (left->type == XML_EXP_OR) {
      ...
      left->exp_right->ref++;
      tmp = xmlExpHashGetEntry(ctxt, XML_EXP_OR, left->exp_right, right, NULL,
                               0, 0);
      left->exp_left->ref++;
      tmp =
          xmlExpHashGetEntry(ctxt, XML_EXP_OR, left->exp_left, tmp, NULL, 0, 0);
    ...
    if (right->type == XML_EXP_OR) {
      ...
      if (left->key > right->exp_right->key) {
        xmlExpNodePtr tmp;
        right->exp_right->ref++;
        tmp = xmlExpHashGetEntry(ctxt, XML_EXP_OR, right->exp_right, left, NULL,
                                 0, 0);
        right->exp_left->ref++;
        tmp = xmlExpHashGetEntry(ctxt, XML_EXP_OR, right->exp_left, tmp, NULL,
                                 0, 0);
        xmlExpFree(ctxt, right);
        return (tmp);
      }
      ...
      if (left->key > right->exp_left->key) {
        xmlExpNodePtr tmp;
        right->exp_right->ref++;
        tmp = xmlExpHashGetEntry(ctxt, XML_EXP_OR, left, right->exp_right, NULL,
                                 0, 0);
        right->exp_left->ref++;
        tmp = xmlExpHashGetEntry(ctxt, XML_EXP_OR, right->exp_left, tmp, NULL,
                                 0, 0);
  ...
  if (ctxt->table[key] != NULL) {
    for (insert = ctxt->table[key]; insert != NULL; insert = insert->next) {
 ...
 * Returns the node or NULL in case of error
 */
xmlExpNodePtr xmlExpNewOr(xmlExpCtxtPtr ctxt, xmlExpNodePtr left,
                          xmlExpNodePtr right) {
  if (ctxt == NULL)
    return (NULL);
 ...
 * Returns the node or NULL in case of error
 */
xmlExpNodePtr xmlExpNewSeq(xmlExpCtxtPtr ctxt, xmlExpNodePtr left,
                           xmlExpNodePtr right) {
  if (ctxt == NULL)
    return (NULL);
  if ((left == NULL) || (right == NULL)) {
 ...
 * Returns the node or NULL in case of error
 */
xmlExpNodePtr xmlExpNewRange(xmlExpCtxtPtr ctxt, xmlExpNodePtr subset, int min,
                             int max) {
  if (ctxt == NULL)
    return (NULL);
  ...
  return (
      xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, subset, NULL, NULL, min, max));
}
...
static int xmlExpGetLanguageInt(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
...
tail:
  ...
  case XML_EXP_OR:
    ...
    if (tmp < 0)
      return (tmp);
    tmp2 = xmlExpGetLanguageInt(ctxt, exp->exp_right, list, len, nb + tmp);
    if (tmp2 < 0)
      return (tmp2);
    return (tmp + tmp2);
  }
 ...
 */
int xmlExpGetLanguage(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
...
static int xmlExpGetStartInt(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
                             const xmlChar **list, int len, int nb) {
  int tmp, tmp2;
tail:
  switch (exp->type) {
  ...
  case XML_EXP_SEQ:
    ...
    if (IS_NILLABLE(exp->exp_left)) {
      tmp2 = xmlExpGetStartInt(ctxt, exp->exp_right, list, len, nb + tmp);
      if (tmp2 < 0)
        return (tmp2);
      tmp += tmp2;
    }
 ...
 *         -2 if there is more than @len strings
 */
int xmlExpGetStart(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
...
static xmlExpNodePtr xmlExpStringDeriveInt(xmlExpCtxtPtr ctxt,
                                           xmlExpNodePtr exp,
                                           const xmlChar *str) {
  xmlExpNodePtr ret;
  ...
  case XML_EXP_OR: {
    ...
    ret = xmlExpHashGetEntry(ctxt, XML_EXP_OR, tmp, ret, NULL, 0, 0);
    return (ret);
  }
  case XML_EXP_SEQ:
#ifdef DEBUG_DERIV
...
#endif
      exp->exp_right->ref++;
      ret = xmlExpHashGetEntry(ctxt, XML_EXP_SEQ, ret, exp->exp_right, NULL, 0,
                               0);
    }
    return (ret);
  case XML_EXP_COUNT: {
    ...
    if (exp->exp_min > 0)
      ...
      min = 0;
    exp->exp_left->ref++;
    tmp = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, exp->exp_left, NULL, NULL,
                             min, max);
    if (ret == emptyExp) {
#ifdef DEBUG_DERIV
 ...
 */
xmlExpNodePtr xmlExpStringDerive(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
...
static int xmlExpDivide(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
                        xmlExpNodePtr sub, xmlExpNodePtr *mult,
                        xmlExpNodePtr *remain) {
  int i;
...
static xmlExpNodePtr xmlExpExpDeriveInt(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
  ...
  case XML_EXP_ATOM:
    ...
    if ((sub->type == XML_EXP_COUNT) && (sub->exp_max == 1) &&
  ...
  case XML_EXP_SEQ:
    ...
    if (xmlExpCheckCard(exp->exp_left, sub)) {
      ...
      if ((ret != forbiddenExp) && (ret != NULL)) {
         ...
         */
        exp->exp_right->ref++;
        return (xmlExpHashGetEntry(ctxt, XML_EXP_SEQ, ret, exp->exp_right, NULL,
                                   0, 0));
      }
#ifdef DEBUG_DERIV
    } else {
    ...
    if (sub->type == XML_EXP_COUNT) {
      ...
      if (ret != forbiddenExp) {
        ...
        if (sub->exp_min > 0)
          ...
          min = 0;
        exp->exp_right->ref++;
        tmp = xmlExpHashGetEntry(ctxt, XML_EXP_SEQ, ret, exp->exp_right, NULL,
                                 0, 0);
        if (tmp == NULL)
          return (NULL);

        sub->exp_left->ref++;
        tmp2 = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, sub->exp_left, NULL,
                                  NULL, min, max);
        if (tmp2 == NULL) {
          xmlExpFree(ctxt, tmp);
          return (NULL);
...
#endif
      exp->exp_left->ref++;
      tmp2 = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, exp->exp_left, NULL, NULL,
                                min, max);
      if (tmp2 == NULL) {
        return (NULL);
      }
  ...
  tab = (const xmlChar **)xmlMalloc(ctxt->tabSize * sizeof(const xmlChar *));
  if (tab == NULL) {
    return (NULL);
  }
  ...
  while (len < 0) {
    const xmlChar **temp;
    temp = (const xmlChar **)xmlRealloc(
        (xmlChar **)tab, ctxt->tabSize * 2 * sizeof(const xmlChar *));
    if (temp == NULL) {
      xmlFree((xmlChar **)tab);
      return (NULL);
    }
 ...
 */
xmlExpNodePtr xmlExpExpDerive(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
                              xmlExpNodePtr sub) {
...
static xmlExpNodePtr xmlExpParseOr(xmlExpCtxtPtr ctxt) {
  ...
  while ((CUR != 0) && (!(IS_BLANK(CUR))) && (CUR != '(') && (CUR != ')') &&
         (CUR != '|') && (CUR != ',') && (CUR != '{') && (CUR != '*') &&
         (CUR != '+') && (CUR != '?') && (CUR != '}'))
    NEXT;
...
parse_quantifier:
  if (CUR == '{') {
    ...
    NEXT min = xmlExpParseNumber(ctxt);
    if (min < 0) {
      xmlExpFree(ctxt, ret);
      return (NULL);
    }
    SKIP_BLANKS
    if (CUR == ',') {
      NEXT max = xmlExpParseNumber(ctxt);
      SKIP_BLANKS
    } else
      max = min;
    if (CUR != '}') {
    ...
    NEXT ret =
        xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, ret, NULL, NULL, min, max);
    SKIP_BLANKS
  } else if (CUR == '?') {
    NEXT ret = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, ret, NULL, NULL, 0, 1);
    SKIP_BLANKS
  } else if (CUR == '+') {
    NEXT ret = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, ret, NULL, NULL, 1, -1);
    SKIP_BLANKS
  } else if (CUR == '*') {
    NEXT ret = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, ret, NULL, NULL, 0, -1);
    SKIP_BLANKS
...
static xmlExpNodePtr xmlExpParseExpr(xmlExpCtxtPtr ctxt) {
  ...
  SKIP_BLANKS
  while (CUR == ',') {
    NEXT right = xmlExpParseSeq(ctxt);
    if (right == NULL) {
      xmlExpFree(ctxt, ret);
      return (NULL);
    }
