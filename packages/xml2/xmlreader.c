...
static void xmlTextReaderFreeNode(xmlTextReaderPtr reader, xmlNodePtr cur) {
  ...
  if (((cur->type == XML_ELEMENT_NODE) || (cur->type == XML_XINCLUDE_START) ||
    ...
    xmlTextReaderFreePropList(reader, cur->properties);
  if ((cur->content != (xmlChar *)&(cur->properties)) &&
      (cur->type != XML_ELEMENT_NODE) && (cur->type != XML_XINCLUDE_START) &&
...
static void xmlTextReaderDebug(xmlTextReaderPtr reader) {
  ...
  fprintf(stderr, "xmlTextReader: state %d depth %d ", reader->state,
          reader->depth);
  if (reader->node == NULL) {
  ...
  fprintf(stderr, "  input: base %d, cur %d, depth %d: ", reader->base,
          reader->cur, reader->ctxt->nodeNr);
...
static int xmlTextReaderEntPush(xmlTextReaderPtr reader, xmlNodePtr value) {
  if (reader->entMax <= 0) {
    reader->entMax = 10;
    reader->entTab =
        (xmlNodePtr *)xmlMalloc(reader->entMax * sizeof(reader->entTab[0]));
  ...
  if (reader->entNr >= reader->entMax) {
    reader->entMax *= 2;
    reader->entTab = (xmlNodePtr *)xmlRealloc(
        reader->entTab, reader->entMax * sizeof(reader->entTab[0]));
    if (reader->entTab == NULL) {
 ...
 */
static void xmlTextReaderStartElement(void *ctx, const xmlChar *fullname,
                                      const xmlChar **atts) {
 ...
 * called when an ending tag has been processed.
 */
static void xmlTextReaderEndElementNs(void *ctx, const xmlChar *localname,
                                      const xmlChar *prefix,
                                      const xmlChar *URI) {
  xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
  xmlTextReaderPtr reader = ctxt->_private;
...
static int xmlTextReaderPushData(xmlTextReaderPtr reader) {
  ...
  while (reader->state == XML_TEXTREADER_NONE) {
    if (inbuf->use < reader->cur + CHUNK_SIZE) {
      ...
      if (reader->mode != XML_TEXTREADER_MODE_EOF) {
        ...
        if ((val == 0) && (inbuf->alloc == XML_BUFFER_ALLOC_IMMUTABLE)) {
    ...
    if (inbuf->use >= reader->cur + CHUNK_SIZE) {
      ...
      val = xmlParseChunk(reader->ctxt,
                          (const char *)&inbuf->content[reader->cur], s, 0);
      reader->cur += s;
      if ((val != 0) || (reader->ctxt->wellFormed == 0))
        return (-1);
      break;
  ...
  if (reader->mode == XML_TEXTREADER_MODE_INTERACTIVE) {
    if (inbuf->alloc != XML_BUFFER_ALLOC_IMMUTABLE) {
      if ((reader->cur >= 4096) && (inbuf->use - reader->cur <= CHUNK_SIZE)) {
   ...
   */
    ...
    if (reader->state != XML_TEXTREADER_DONE) {
      s = inbuf->use - reader->cur;
      val = xmlParseChunk(reader->ctxt,
                          (const char *)&inbuf->content[reader->cur], s, 1);
  ...
  if ((reader->validate == XML_TEXTREADER_VALIDATE_DTD) &&
    ...
    if ((node->ns == NULL) || (node->ns->prefix == NULL)) {
      reader->ctxt->valid &= xmlValidatePushElement(
          &reader->ctxt->vctxt, reader->ctxt->myDoc, node, node->name);
    } else {
      ...
      reader->ctxt->valid &= xmlValidatePushElement(
          &reader->ctxt->vctxt, reader->ctxt->myDoc, node, qname);
      if (qname != NULL)
        xmlFree(qname);
    }
  }
 ...
 */
static void xmlTextReaderValidateCData(xmlTextReaderPtr reader,
                                       const xmlChar *data, int len) {
#ifdef LIBXML_VALID_ENABLED
  if ((reader->validate == XML_TEXTREADER_VALIDATE_DTD) &&
      (reader->ctxt != NULL) && (reader->ctxt->validate == 1)) {
    reader->ctxt->valid &=
        xmlValidatePushCData(&reader->ctxt->vctxt, data, len);
  }
#endif /* LIBXML_VALID_ENABLED */
#ifdef LIBXML_SCHEMAS_ENABLED
  if ((reader->validate == XML_TEXTREADER_VALIDATE_RNG) &&
  ...
  do {
    if (node->type == XML_ENTITY_REF_NODE) {
      ...
      if ((node->children == NULL) && (ctxt->sax != NULL) &&
        ...
        node->children = (xmlNodePtr)ctxt->sax->getEntity(ctxt, node->name);
      }
...
get_next_node:
  ...
  if (reader->node->next != NULL) {
    ...
    if ((reader->preserves == 0) &&
        ...
        (reader->node->prev->type != XML_DTD_NODE) && (reader->entNr == 0)) {
  ...
  if ((reader->node == NULL) || (reader->node->type == XML_DOCUMENT_NODE) ||
    ...
    if ((oldnode != NULL) && (reader->preserves == 0) &&
        ...
        ((oldnode->extra & NODE_IS_PRESERVED) == 0) && (reader->entNr == 0)) {
...
node_found:
  ...
  if ((reader->node != NULL) && (reader->node->type == XML_ENTITY_REF_NODE) &&
    ...
    if ((reader->node->children == NULL) && (reader->ctxt->sax != NULL) &&
      ...
      reader->node->children = (xmlNodePtr)reader->ctxt->sax->getEntity(
          reader->ctxt, reader->node->name);
    }
  ...
  if ((reader->node != NULL) && (reader->node->type == XML_ENTITY_DECL) &&
  ...
  if ((reader->validate == XML_TEXTREADER_VALIDATE_XSD) &&
      (reader->xsdValidErrors == 0) && (reader->xsdValidCtxt != NULL)) {
...
xmlTextReaderPtr xmlNewTextReader(xmlParserInputBufferPtr input,
  ...
  if (ret->input->buffer->use >= 4) {
    ret->ctxt = xmlCreatePushParserCtxt(
        ret->sax, NULL, (const char *)ret->input->buffer->content, 4, URI);
    ret->base = 0;
    ret->cur = 4;
  } else {
...
int xmlTextReaderPreservePattern(xmlTextReaderPtr reader,
  ...
  if (reader->patternMax <= 0) {
    reader->patternMax = 4;
    reader->patternTab = (xmlPatternPtr *)xmlMalloc(
  ...
  if (reader->patternNr >= reader->patternMax) {
    xmlPatternPtr *tmp;
    reader->patternMax *= 2;
    tmp = (xmlPatternPtr *)xmlRealloc(
        reader->patternTab, reader->patternMax * sizeof(reader->patternTab[0]));
    if (tmp == NULL) {
      xmlGenericError(xmlGenericErrorContext, "xmlRealloc failed !\n");
      reader->patternMax /= 2;
      return (-1);
...
static void XMLCDECL xmlTextReaderValidityErrorRelay(void *ctx, const char *msg,
  ...
  if (!reader->errorFunc) {
    ...
    reader->errorFunc(reader->errorFuncArg, str,
                      XML_PARSER_SEVERITY_VALIDITY_ERROR, NULL /* locator */);
  }
  if (str != NULL)
    xmlFree(str);
  va_end(ap);
...
static void XMLCDECL xmlTextReaderValidityWarningRelay(void *ctx,
                                                       const char *msg, ...) {
  xmlTextReaderPtr reader = (xmlTextReaderPtr)ctx;
  ...
  if (!reader->errorFunc) {
    ...
    reader->errorFunc(reader->errorFuncArg, str,
                      XML_PARSER_SEVERITY_VALIDITY_WARNING, NULL /* locator */);
  }
  if (str != NULL)
    xmlFree(str);
  va_end(ap);
...
static void xmlTextReaderValidityStructuredRelay(void *userData,
                                                 xmlErrorPtr error) {
...
int xmlTextReaderSetSchema(xmlTextReaderPtr reader, xmlSchemaPtr schema) {
  ...
  reader->xsdPlug = xmlSchemaSAXPlug(reader->xsdValidCtxt, &(reader->ctxt->sax),
...
int xmlTextReaderRelaxNGValidate(xmlTextReaderPtr reader, const char *rng) {
  ...
  if (reader->errorFunc != NULL) {
    xmlRelaxNGSetParserErrors(ctxt, xmlTextReaderValidityErrorRelay,
                              xmlTextReaderValidityWarningRelay, reader);
  }
  if (reader->sErrorFunc != NULL) {
    xmlRelaxNGSetValidStructuredErrors(
 ...
 *         -1 in case of error.
 */
static int xmlTextReaderSchemaValidateInternal(xmlTextReaderPtr reader,
                                               const char *xsd,
  ...
  if (xsd != NULL) {
    ...
    if (reader->errorFunc != NULL) {
      xmlSchemaSetParserErrors(pctxt, xmlTextReaderValidityErrorRelay,
                               xmlTextReaderValidityWarningRelay, reader);
    }
    ...
    reader->xsdPlug = xmlSchemaSAXPlug(
        reader->xsdValidCtxt, &(reader->ctxt->sax), &(reader->ctxt->userData));
    if (reader->xsdPlug == NULL) {
      xmlSchemaFree(reader->xsdSchemas);
      reader->xsdSchemas = NULL;
      xmlSchemaFreeValidCtxt(reader->xsdValidCtxt);
...
int xmlTextReaderSchemaValidateCtxt(xmlTextReaderPtr reader,
                                    xmlSchemaValidCtxtPtr ctxt, int options) {
...
static void xmlTextReaderGenericError(void *ctxt, xmlParserSeverities severity,
                                      char *str) {
 ...
 */
int xmlTextReaderSetup(xmlTextReaderPtr reader, xmlParserInputBufferPtr input,
    ...
    if (reader->ctxt == NULL) {
      if (reader->input->buffer->use >= 4) {
        reader->ctxt = xmlCreatePushParserCtxt(
            reader->sax, NULL, (const char *)reader->input->buffer->content, 4,
            URL);
        reader->base = 0;
        reader->cur = 4;
      } else {
 ...
 * Returns the new reader or NULL in case of error.
 */
xmlTextReaderPtr xmlReaderForFd(int fd, const char *URL, const char *encoding,
                                int options) {
  xmlTextReaderPtr reader;
  xmlParserInputBufferPtr input;
...
int xmlReaderNewDoc(xmlTextReaderPtr reader, const xmlChar *cur,
  ...
  len = xmlStrlen(cur);
  return (xmlReaderNewMemory(reader, (const char *)cur, len, URL, encoding,
                             options));
}
 ...
 * Returns 0 in case of success and -1 in case of error
 */
int xmlReaderNewMemory(xmlTextReaderPtr reader, const char *buffer, int size,
                       const char *URL, const char *encoding, int options) {
  xmlParserInputBufferPtr input;
  ...
  input =
      xmlParserInputBufferCreateStatic(buffer, size, XML_CHAR_ENCODING_NONE);
  if (input == NULL) {
    return (-1);
  }
 ...
 * Returns 0 in case of success and -1 in case of error
 */
int xmlReaderNewFd(xmlTextReaderPtr reader, int fd, const char *URL,
                   const char *encoding, int options) {
  xmlParserInputBufferPtr input;
...
int xmlReaderNewIO(xmlTextReaderPtr reader, xmlInputReadCallback ioread,
                   xmlInputCloseCallback ioclose, void *ioctx, const char *URL,
                   const char *encoding, int options) {
  xmlParserInputBufferPtr input;
 ...
 */
static int xmlBase64Decode(const unsigned char *in, unsigned long *inlen,
  ...
  while (1) {
    ...
    if (!is_ignore) {
      ...
      if (nbintmp == 4) {
        ...
        outtmp[1] = ((intmp[1] & 0x0F) << 4) | ((intmp[2] & 0x3C) >> 2);
