...
static xmlNs xmlXPathXMLNamespaceStruct = {
    NULL, XML_NAMESPACE_DECL, XML_XML_NAMESPACE, BAD_CAST "xml", NULL, NULL};
...
#define MAXERRNO                                                               \
  ((int)(sizeof(xmlXPathErrorMessages) / sizeof(xmlXPathErrorMessages[0])) - 1)
/**
 * xmlXPathErrMemory:
  ...
  if (ctxt != NULL) {
    if (extra) {
      ...
      ctxt->lastError.message =
          (char *)xmlStrdup(BAD_CAST "Memory allocation failed\n");
    }
    ...
    if (extra)
      __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH,
                      XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL,
                      NULL, 0, 0, "Memory allocation failed : %s\n", extra);
    else
      __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH,
                      XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, NULL, NULL,
                      NULL, 0, 0, "Memory allocation failed\n");
  }
}
 ...
 * Handle a redefinition of attribute error
 */
static void xmlXPathPErrMemory(xmlXPathParserContextPtr ctxt,
                               const char *extra) {
  if (ctxt == NULL)
...
void xmlXPathErr(xmlXPathParserContextPtr ctxt, int error) {
  ...
  if (ctxt == NULL) {
    __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH,
                    ...
                    XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, 0, 0, "%s",
  ...
  if (ctxt->context == NULL) {
    __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH,
                    error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK,
                    XML_ERR_ERROR, NULL, 0, (const char *)ctxt->base, NULL,
                    NULL, ctxt->cur - ctxt->base, 0, "%s",
                    xmlXPathErrorMessages[error]);
    return;
  }
  ...
  if (ctxt->context->error != NULL) {
    ...
    __xmlRaiseError(
        NULL, NULL, NULL, NULL, ctxt->context->debugNode, XML_FROM_XPATH,
        error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK, XML_ERR_ERROR,
        NULL, 0, (const char *)ctxt->base, NULL, NULL, ctxt->cur - ctxt->base,
        0, "%s", xmlXPathErrorMessages[error]);
  }
}
...
void xmlXPatherror(xmlXPathParserContextPtr ctxt,
                   const char *file ATTRIBUTE_UNUSED, int line ATTRIBUTE_UNUSED,
                   int no) {
  xmlXPathErr(ctxt, no);
}
 ...
 */
static int xmlPointerListAddSize(xmlPointerListPtr list, void *item,
                                 int initialSize) {
  if (list->items == NULL) {
    if (initialSize <= 0)
      initialSize = 1;
...
static void xmlXPathReleaseObject(xmlXPathContextPtr ctxt,
                                  xmlXPathObjectPtr obj);
static int xmlXPathCompOpEvalFirst(xmlXPathParserContextPtr ctxt,
                                   xmlXPathStepOpPtr op, xmlNodePtr *first);
static int xmlXPathCompOpEvalToBoolean(xmlXPathParserContextPtr ctxt,
                                       xmlXPathStepOpPtr op, int isPredicate);
...
static xmlXPathCompExprPtr xmlXPathNewCompExpr(void) {
  ...
  cur->nbStep = 0;
  cur->steps =
      (xmlXPathStepOp *)xmlMalloc(cur->maxStep * sizeof(xmlXPathStepOp));
  if (cur->steps == NULL) {
...
static int xmlXPathCompExprAdd(xmlXPathCompExprPtr comp, int ch1, int ch2,
                               xmlXPathOp op, int value, int value2, int value3,
  ...
  if ((comp->dict != NULL) &&
    ...
    if (value4 != NULL) {
      comp->steps[comp->nbStep].value4 =
          (xmlChar *)(void *)xmlDictLookup(comp->dict, value4, -1);
      xmlFree(value4);
    } else
      comp->steps[comp->nbStep].value4 = NULL;
    if (value5 != NULL) {
      comp->steps[comp->nbStep].value5 =
          (xmlChar *)(void *)xmlDictLookup(comp->dict, value5, -1);
      xmlFree(value5);
    } else
...
#define PUSH_FULL_EXPR(op, op1, op2, val, val2, val3, val4, val5)              \
  xmlXPathCompExprAdd(ctxt->comp, (op1), (op2), (op), (val), (val2), (val3),   \
                      (val4), (val5))
#define PUSH_LONG_EXPR(op, val, val2, val3, val4, val5)                        \
  xmlXPathCompExprAdd(ctxt->comp, ctxt->comp->last, -1, (op), (val), (val2),   \
                      (val3), (val4), (val5))
...
#define PUSH_BINARY_EXPR(op, ch1, ch2, val, val2)                              \
  xmlXPathCompExprAdd(ctxt->comp, (ch1), (ch2), (op), (val), (val2), 0, NULL,  \
                      NULL)
...
static void xmlXPathDebugDumpNodeSet(FILE *output, xmlNodeSetPtr cur,
                                     int depth) {
  int i;
  char shift[100];
...
static void xmlXPathDebugDumpValueTree(FILE *output, xmlNodeSetPtr cur,
                                       int depth) {
  int i;
  char shift[100];
...
static void xmlXPathDebugDumpLocationSet(FILE *output, xmlLocationSetPtr cur,
                                         int depth) {
  int i;
  char shift[100];
...
static void xmlXPathDebugDumpStepOp(FILE *output, xmlXPathCompExprPtr comp,
...
static void xmlXPathDebugObjUsageRequested(xmlXPathContextPtr ctxt,
...
static void xmlXPathDebugObjUsageReleased(xmlXPathContextPtr ctxt,
                                          xmlXPathObjectType objType) {
  int isCached = 0;
...
static void xmlXPathDebugObjUsageDisplay(xmlXPathContextPtr ctxt) {
  int reqAll, reqNodeset, reqString, reqBool, reqNumber, reqXSLTTree,
      reqUndefined;
  int caAll = 0, caNodeset = 0, caString = 0, caBool = 0, caNumber = 0,
      caXSLTTree = 0, caUndefined = 0;
  int reAll = 0, reNodeset = 0, reString = 0, reBool = 0, reNumber = 0,
      reXSLTTree = 0, reUndefined = 0;
 ...
 */
int xmlXPathContextSetCache(xmlXPathContextPtr ctxt, int active, int value,
                            int options) {
  if (ctxt == NULL)
    return (-1);
  if (active) {
 ...
 */
static xmlXPathObjectPtr xmlXPathCacheWrapNodeSet(xmlXPathContextPtr ctxt,
                                                  xmlNodeSetPtr val) {
  if ((ctxt != NULL) && (ctxt->cache != NULL)) {
    ...
    if ((cache->miscObjs != NULL) && (cache->miscObjs->number != 0)) {
      ...
      ret =
          (xmlXPathObjectPtr)cache->miscObjs->items[--cache->miscObjs->number];
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheWrapString(xmlXPathContextPtr ctxt,
                                                 xmlChar *val) {
  if ((ctxt != NULL) && (ctxt->cache != NULL)) {
    xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr)ctxt->cache;
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewNodeSet(xmlXPathContextPtr ctxt,
                                                 xmlNodePtr val) {
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewCString(xmlXPathContextPtr ctxt,
                                                 const char *val) {
  if ((ctxt != NULL) && (ctxt->cache)) {
    xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr)ctxt->cache;
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewString(xmlXPathContextPtr ctxt,
                                                const xmlChar *val) {
  if ((ctxt != NULL) && (ctxt->cache)) {
    xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr)ctxt->cache;
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewBoolean(xmlXPathContextPtr ctxt,
                                                 int val) {
  if ((ctxt != NULL) && (ctxt->cache)) {
    xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr)ctxt->cache;
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewFloat(xmlXPathContextPtr ctxt,
                                               double val) {
  if ((ctxt != NULL) && (ctxt->cache)) {
    xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr)ctxt->cache;
...
static xmlXPathObjectPtr xmlXPathCacheConvertString(xmlXPathContextPtr ctxt,
 ...
 */
static xmlXPathObjectPtr xmlXPathCacheObjectCopy(xmlXPathContextPtr ctxt,
                                                 xmlXPathObjectPtr val) {
  if (val == NULL)
    return (NULL);
  ...
  if (XP_HAS_CACHE(ctxt)) {
    ...
    case XPATH_NODESET:
      return (xmlXPathCacheWrapNodeSet(
          ctxt, xmlXPathNodeSetMerge(NULL, val->nodesetval)));
    case XPATH_STRING:
 ...
 */
static xmlXPathObjectPtr xmlXPathCacheConvertBoolean(xmlXPathContextPtr ctxt,
                                                     xmlXPathObjectPtr val) {
  xmlXPathObjectPtr ret;
 ...
 *         is done directly on @val)
 */
static xmlXPathObjectPtr xmlXPathCacheConvertNumber(xmlXPathContextPtr ctxt,
                                                    xmlXPathObjectPtr val) {
  xmlXPathObjectPtr ret;
...
int valuePush(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr value) {
  ...
  if (ctxt->valueNr >= ctxt->valueMax) {
    ...
    tmp = (xmlXPathObjectPtr *)xmlRealloc(
        ctxt->valueTab, 2 * ctxt->valueMax * sizeof(ctxt->valueTab[0]));
    if (tmp == NULL) {
    ...
    if (xmlXPathIsNaN(number)) {
      ...
      */
      ...
      if (((absolute_value > UPPER_DOUBLE) ||
        ...
        size = snprintf(work, sizeof(work), "%*.*e", integer_place,
...
int xmlXPathCmpNodes(xmlNodePtr node1, xmlNodePtr node2) {
  ...
  if ((node1->type == XML_ELEMENT_NODE) && (node2->type == XML_ELEMENT_NODE) &&
      (0 > (long)node1->content) && (0 > (long)node2->content) &&
      (node1->doc == node2->doc)) {
    long l1, l2;
  ...
  case XML_ELEMENT_NODE:
    if (node2->type == XML_ELEMENT_NODE) {
      if ((0 > (long)node1->content) && /* TODO: Would a != 0 suffice here? */
          (0 > (long)node2->content) && (node1->doc == node2->doc)) {
        l1 = -((long)node1->content);
        l2 = -((long)node2->content);
        if (l1 < l2)
          return (1);
...
xmlNodeSetPtr xmlXPathNodeSetCreate(xmlNodePtr val) {
  ...
  if (val != NULL) {
    ret->nodeTab =
        (xmlNodePtr *)xmlMalloc(XML_NODESET_DEFAULT * sizeof(xmlNodePtr));
    if (ret->nodeTab == NULL) {
      xmlXPathErrMemory(NULL, "creating nodeset\n");
      xmlFree(ret);
      return (NULL);
...
void xmlXPathNodeSetAddNs(xmlNodeSetPtr cur, xmlNodePtr node, xmlNsPtr ns) {
  ...
  if (cur->nodeMax == 0) {
    ...
    cur->nodeMax *= 2;
    temp = (xmlNodePtr *)xmlRealloc(cur->nodeTab,
...
xmlNodeSetPtr xmlXPathNodeSetMerge(xmlNodeSetPtr val1, xmlNodeSetPtr val2) {
  ...
  for (i = 0; i < val2->nodeNr; i++) {
    ...
    if (val1->nodeMax == 0) {
      ...
      val1->nodeMax *= 2;
      temp = (xmlNodePtr *)xmlRealloc(val1->nodeTab,
                                      val1->nodeMax * sizeof(xmlNodePtr));
      if (temp == NULL) {
        xmlXPathErrMemory(NULL, "merging nodeset\n");
        return (NULL);
      }
 ...
 */
static xmlNodeSetPtr xmlXPathNodeSetMergeAndClear(xmlNodeSetPtr set1,
                                                  xmlNodeSetPtr set2,
                                                  int hasNullEntries) {
  if ((set1 == NULL) && (hasNullEntries == 0)) {
    ...
    for (i = 0; i < set2->nodeNr; i++) {
      ...
      if (set1->nodeMax == 0) {
        ...
        set1->nodeMax *= 2;
        temp = (xmlNodePtr *)xmlRealloc(set1->nodeTab,
                                        set1->nodeMax * sizeof(xmlNodePtr));
        if (temp == NULL) {
          xmlXPathErrMemory(NULL, "merging nodeset\n");
          return (NULL);
        }
 ...
 * Returns @set1 once extended or NULL in case of error.
 */
static xmlNodeSetPtr xmlXPathNodeSetMergeAndClearNoDupls(xmlNodeSetPtr set1,
                                                         xmlNodeSetPtr set2,
                                                         int hasNullEntries) {
  if (set2 == NULL)
    return (set1);
 ...
 */
static void xmlXPathNodeSetClearFromPos(xmlNodeSetPtr set, int pos,
                                        int hasNsNodes) {
...
void xmlXPathRegisterFuncLookup(xmlXPathContextPtr ctxt,
                                xmlXPathFuncLookupFunc f, void *funcCtxt) {
  if (ctxt == NULL)
    return;
...
int xmlXPathRegisterVariableNS(xmlXPathContextPtr ctxt, const xmlChar *name,
  ...
  return (xmlHashUpdateEntry2(ctxt->varHash, name, ns_uri, (void *)value,
                              (xmlHashDeallocator)xmlXPathFreeObject));
}
...
xmlXPathObjectPtr xmlXPathVariableLookup(xmlXPathContextPtr ctxt,
  ...
  if (ctxt->varLookupFunc != NULL) {
    ...
    ret = ((xmlXPathVariableLookupFunc)ctxt->varLookupFunc)(ctxt->varLookupData,
                                                            name, NULL);
    return (ret);
  }
...
xmlXPathObjectPtr xmlXPathVariableLookupNS(xmlXPathContextPtr ctxt,
  ...
  return (xmlXPathCacheObjectCopy(
      ctxt, (xmlXPathObjectPtr)xmlHashLookup2(ctxt->varHash, name, ns_uri)));
}
...
int xmlXPathRegisterNs(xmlXPathContextPtr ctxt, const xmlChar *prefix,
  ...
  if (ns_uri == NULL)
    return (
        xmlHashRemoveEntry(ctxt->nsHash, prefix, (xmlHashDeallocator)xmlFree));
      ...
      if (obj->nodesetval != NULL) {
        if (obj->boolval) {
          ...
          obj->nodesetval = NULL;
                   ...
                   (XP_CACHE_WANTS(cache->nodesetObjs, cache->maxNodeset))) {
...
#define CHECK_CTXT(ctxt)                                                       \
  if (ctxt == NULL) {                                                          \
    __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH,              \
                    XML_ERR_INTERNAL_ERROR, XML_ERR_FATAL, __FILE__, __LINE__, \
                    NULL, NULL, NULL, 0, 0, "NULL context pointer\n");         \
    return (NULL);                                                             \
  }
...
xmlXPathCompParserContext(xmlXPathCompExprPtr comp, xmlXPathContextPtr ctxt) {
  ...
  ret->valueTab =
      (xmlXPathObjectPtr *)xmlMalloc(10 * sizeof(xmlXPathObjectPtr));
 ...
 */
static int xmlXPathCompareNodeSetFloat(xmlXPathParserContextPtr ctxt, int inf,
                                       int strict, xmlXPathObjectPtr arg,
                                       xmlXPathObjectPtr f) {
  int i, ret = 0;
  xmlNodeSetPtr ns;
  xmlChar *str2;
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathCompareNodeSetString(xmlXPathParserContextPtr ctxt, int inf,
                                        int strict, xmlXPathObjectPtr arg,
                                        xmlXPathObjectPtr s) {
  int i, ret = 0;
  xmlNodeSetPtr ns;
 ...
 */
static int xmlXPathCompareNodeSets(int inf, int strict, xmlXPathObjectPtr arg1,
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathCompareNodeSetValue(xmlXPathParserContextPtr ctxt, int inf,
                                       int strict, xmlXPathObjectPtr arg,
                                       xmlXPathObjectPtr val) {
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathEqualNodeSetString(xmlXPathObjectPtr arg, const xmlChar *str,
                                      int neq) {
  int i;
  xmlNodeSetPtr ns;
  xmlChar *str2;
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathEqualNodeSetFloat(xmlXPathParserContextPtr ctxt,
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathEqualNodeSets(xmlXPathObjectPtr arg1, xmlXPathObjectPtr arg2,
                                 int neq) {
  int i, j;
  unsigned int *hashs1;
  unsigned int *hashs2;
...
static int xmlXPathEqualValuesCommon(xmlXPathParserContextPtr ctxt,
...
int xmlXPathCompareValues(xmlXPathParserContextPtr ctxt, int inf, int strict) {
  ...
  if ((arg2->type == XPATH_NODESET) || (arg2->type == XPATH_XSLT_TREE) ||
      (arg1->type == XPATH_NODESET) || (arg1->type == XPATH_XSLT_TREE)) {
    /*
     * If either argument is a XPATH_NODESET or XPATH_XSLT_TREE the two
     * arguments are not freed from within this routine; they will be freed from
     * the called routine, e.g. xmlXPathCompareNodeSets or
     * xmlXPathCompareNodeSetValue
     */
    if (((arg2->type == XPATH_NODESET) || (arg2->type == XPATH_XSLT_TREE)) &&
        ((arg1->type == XPATH_NODESET) || (arg1->type == XPATH_XSLT_TREE))) {
 ...
 */
typedef xmlNodePtr (*xmlXPathTraversalFunction)(xmlXPathParserContextPtr ctxt,
                                                xmlNodePtr cur);
 ...
 */
typedef xmlNodePtr (*xmlXPathTraversalFunctionExt)(xmlNodePtr cur,
 ...
 */
typedef xmlNodeSetPtr (*xmlXPathNodeSetMergeFunction)(xmlNodeSetPtr,
                                                      xmlNodeSetPtr, int);
 ...
 * Returns the next element following that axis
 */
static xmlNodePtr xmlXPathNextChildElement(xmlXPathParserContextPtr ctxt,
                                           xmlNodePtr cur) {
  if ((ctxt == NULL) || (ctxt->context == NULL))
    return (NULL);
  if (cur == NULL) {
 ...
 */
static xmlNodePtr
xmlXPathNextDescendantOrSelfElemParent(xmlNodePtr cur, xmlNodePtr contextNode) {
  if (cur == NULL) {
    if (contextNode == NULL)
      return (NULL);
 ...
 * Returns the next element following that axis
 */
static xmlNodePtr xmlXPathNextPrecedingInternal(xmlXPathParserContextPtr ctxt,
                                                xmlNodePtr cur) {
  if ((ctxt == NULL) || (ctxt->context == NULL))
    return (NULL);
  if (cur == NULL) {
 ...
 */
static xmlNodeSetPtr xmlXPathGetElementsByIds(xmlDocPtr doc,
                                              const xmlChar *ids) {
  xmlNodeSetPtr ret;
  ...
  if ((cur->nodesetval == NULL) || (cur->nodesetval->nodeNr == 0)) {
    ...
    case XML_PI_NODE:
      if (cur->nodesetval->nodeTab[i]->name[0] == ' ')
        ...
        valuePush(ctxt, xmlXPathCacheNewString(
                            ctxt->context, cur->nodesetval->nodeTab[i]->name));
      break;
    case XML_NAMESPACE_DECL:
      valuePush(ctxt, xmlXPathCacheNewString(
                          ctxt->context,
  ...
  if ((cur->nodesetval == NULL) || (cur->nodesetval->nodeNr == 0)) {
    ...
    case XML_ATTRIBUTE_NODE:
      if (cur->nodesetval->nodeTab[i]->ns == NULL)
        ...
        valuePush(ctxt,
                  xmlXPathCacheNewString(
                      ctxt->context, cur->nodesetval->nodeTab[i]->ns->href));
      break;
    default:
      valuePush(ctxt, xmlXPathCacheNewCString(ctxt->context, ""));
    }
  ...
  if ((cur->nodesetval == NULL) || (cur->nodesetval->nodeNr == 0)) {
    ...
    case XML_ATTRIBUTE_NODE:
      if (cur->nodesetval->nodeTab[i]->name[0] == ' ')
        ...
        fullname =
            xmlBuildQName(cur->nodesetval->nodeTab[i]->name,
                          cur->nodesetval->nodeTab[i]->ns->prefix, NULL, 0);
...
void xmlXPathStringFunction(xmlXPathParserContextPtr ctxt, int nargs) {
  ...
  if (nargs == 0) {
    valuePush(ctxt,
              xmlXPathCacheWrapString(ctxt->context, xmlXPathCastNodeToString(
                                                         ctxt->context->node)));
    return;
  }
...
void xmlXPathStringLengthFunction(xmlXPathParserContextPtr ctxt, int nargs) {
  ...
  if (nargs == 0) {
    ...
    if (ctxt->context->node == NULL) {
      ...
      valuePush(ctxt,
                xmlXPathCacheNewFloat(ctxt->context, xmlUTF8Strlen(content)));
      xmlFree(content);
    }
    return;
  }
...
void xmlXPathSubstringBeforeFunction(xmlXPathParserContextPtr ctxt, int nargs) {
  ...
  if (target) {
    ...
    valuePush(ctxt,
              xmlXPathCacheNewString(ctxt->context, xmlBufferContent(target)));
...
static xmlChar *xmlXPathParseQName(xmlXPathParserContextPtr ctxt,
                                   xmlChar **prefix) {
  xmlChar *ret = NULL;
...
static xmlChar *xmlXPathParseNameComplex(xmlXPathParserContextPtr ctxt,
  ...
  while ((c != ' ') && (c != '>') && (c != '/') && /* test bigname.xml */
         ((IS_LETTER(c)) || (IS_DIGIT(c)) || (c == '.') || (c == '-') ||
          (c == '_') || ((qualified) && (c == ':')) || (IS_COMBINING(c)) ||
          (IS_EXTENDER(c)))) {
...
static void xmlXPathCompPathExpr(xmlXPathParserContextPtr ctxt) {
  ...
  SKIP_BLANKS;
  if ((CUR == '$') || (CUR == '(') || (IS_ASCII_DIGIT(CUR)) || (CUR == '\'') ||
      (CUR == '"') || (CUR == '.' && IS_ASCII_DIGIT(NXT(1)))) {
    lc = 0;
  } else if (CUR == '*') {
  ...
  if (lc) {
    ...
    if ((CUR == '/') && (NXT(1) == '/')) {
      ...
      PUSH_LONG_EXPR(XPATH_OP_COLLECT, AXIS_DESCENDANT_OR_SELF, NODE_TEST_TYPE,
 ...
 */
static xmlChar *xmlXPathCompNodeTest(xmlXPathParserContextPtr ctxt,
                                     xmlXPathTestVal *test,
                                     xmlXPathTypeVal *type,
                                     const xmlChar **prefix, xmlChar *name) {
  int blanks;
...
static void xmlXPathCompStep(xmlXPathParserContextPtr ctxt) {
  ...
  if ((CUR == '.') && (NXT(1) == '.')) {
    SKIP(2);
    SKIP_BLANKS;
    PUSH_LONG_EXPR(XPATH_OP_COLLECT, AXIS_PARENT, NODE_TEST_TYPE,
                   NODE_TYPE_NODE, NULL, NULL);
  } else if (CUR == '.') {
    NEXT;
    SKIP_BLANKS;
...
#endif
      PUSH_FULL_EXPR(XPATH_OP_COLLECT, op1, ctxt->comp->last, axis, test, type,
  ...
  if (CUR != '/') {
    ...
    while (CUR == '/') {
      if ((CUR == '/') && (NXT(1) == '/')) {
        ...
        NEXT;
        SKIP_BLANKS;
        if ((CUR != 0) && ((IS_ASCII_LETTER(CUR)) || (CUR == '_') ||
                           (CUR == '.') || (CUR == '@') || (CUR == '*')))
          xmlXPathCompRelativeLocationPath(ctxt);
      }
      CHECK_ERROR;
...
static int xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt,
                              xmlXPathStepOpPtr op);
...
static int xmlXPathCompOpEvalPredicate(xmlXPathParserContextPtr ctxt,
                                       xmlXPathStepOpPtr op, xmlNodeSetPtr set,
                                       int contextSize, int hasNsNodes) {
  if (op->ch1 != -1) {
    ...
    contextSize = xmlXPathCompOpEvalPredicate(ctxt, &comp->steps[op->ch1], set,
                                              contextSize, hasNsNodes);
    CHECK_ERROR0;
    if (contextSize <= 0)
      return (0);
  }
...
static int xmlXPathIsPositionalPredicate(xmlXPathParserContextPtr ctxt,
                                         xmlXPathStepOpPtr op, int *maxPos) {
...
static int xmlXPathNodeCollectAndTest(xmlXPathParserContextPtr ctxt,
                                      xmlXPathStepOpPtr op, xmlNodePtr *first,
                                      xmlNodePtr *last, int toBool) {
...
#ifdef DEBUG_STEP
  xmlXPathDebugDumpStepAxis(
      op, (obj->nodesetval != NULL) ? obj->nodesetval->nodeNr : 0);
#endif
  ...
  apply_predicates: /* --------------------------------------------------- */
    ...
    if ((predOp != NULL) && (seq->nodeNr > 0)) {
      ...
      if (hasPredicateRange != 0)
        newSize = xmlXPathCompOpEvalPositionalPredicate(
            ctxt, predOp, seq, size, maxPos, maxPos, hasNsNodes);
      else
        newSize =
            xmlXPathCompOpEvalPredicate(ctxt, predOp, seq, size, hasNsNodes);
...
static int xmlXPathCompOpEvalFilterFirst(xmlXPathParserContextPtr ctxt,
                                         xmlXPathStepOpPtr op,
                                         xmlNodePtr *first);
...
static int xmlXPathCompOpEvalFirst(xmlXPathParserContextPtr ctxt,
  ...
  case XPATH_OP_VALUE:
    valuePush(ctxt, xmlXPathCacheObjectCopy(ctxt->context,
 ...
 */
static int xmlXPathCompOpEvalLast(xmlXPathParserContextPtr ctxt,
                                  xmlXPathStepOpPtr op, xmlNodePtr *last) {
  int total = 0, cur;
  xmlXPathCompExprPtr comp;
  xmlXPathObjectPtr arg1, arg2;
  xmlNodePtr bak;
...
static int xmlXPathCompOpEvalFilterFirst(xmlXPathParserContextPtr ctxt,
  ...
  if ((op->ch1 != -1) && (op->ch2 != -1) &&
    ...
    if ((f != -1) && (comp->steps[f].op == XPATH_OP_FUNCTION) &&
        (comp->steps[f].value5 == NULL) && (comp->steps[f].value == 0) &&
      ...
      if ((ctxt->value != NULL) && (ctxt->value->type == XPATH_NODESET) &&
        ...
        ctxt->value->nodesetval->nodeTab[0] =
            ctxt->value->nodesetval
                ->nodeTab[ctxt->value->nodesetval->nodeNr - 1];
        ctxt->value->nodesetval->nodeNr = 1;
    ...
    for (i = 0; i < oldlocset->locNr; i++) {
      ...
      if (xmlXPathEvaluatePredicateResult(ctxt, res)) {
        xmlXPtrLocationSetAdd(
            newlocset,
            xmlXPathCacheObjectCopy(ctxt->context, oldlocset->locTab[i]));
      }
      /*
       * Cleanup
       */
...
static int xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt,
  ...
  case XPATH_OP_UNION:
    ...
    if ((arg1->nodesetval == NULL) ||
        ((arg2->nodesetval != NULL) && (arg2->nodesetval->nodeNr != 0))) {
      arg1->nodesetval =
        ...
        if (res->type == XPATH_LOCATIONSET) {
          ...
          for (j = 0; j < rloc->locNr; j++) {
            range = xmlXPtrNewRange(
                oldlocset->locTab[i]->user, oldlocset->locTab[i]->index,
                rloc->locTab[j]->user2, rloc->locTab[j]->index2);
            if (range != NULL) {
 ...
 */
static int xmlXPathRunStreamEval(xmlXPathContextPtr ctxt, xmlPatternPtr comp,
...
next_node:
  ...
  scan_children:
    ...
    do {
      ...
      if (cur->type == XML_ELEMENT_NODE) {
        ret = xmlStreamPop(patstream);
      } else if ((eval_all_nodes) && ((cur->type == XML_TEXT_NODE) ||
                                      (cur->type == XML_CDATA_SECTION_NODE) ||
...
static int xmlXPathRunEval(xmlXPathParserContextPtr ctxt, int toBool) {
  ...
  if (ctxt->comp->stream) {
    ...
    if (toBool) {
       ...
       */
      res =
          xmlXPathRunStreamEval(ctxt->context, ctxt->comp->stream, &resObj, 0);
 ...
 */
static xmlXPathCompExprPtr xmlXPathTryStreamCompile(xmlXPathContextPtr ctxt,
                                                    const xmlChar *str) {
  /*
...
static void xmlXPathRewriteDOSExpression(xmlXPathCompExprPtr comp,
                                         xmlXPathStepOpPtr op) {
  /*
 ...
 *         the caller has to free the object.
 */
static int xmlXPathCompiledEvalInternal(xmlXPathCompExprPtr comp,
                                        xmlXPathContextPtr ctxt,
                                        xmlXPathObjectPtr *resObj, int toBool) {
  xmlXPathParserContextPtr pctxt;
#ifndef LIBXML_THREAD_ENABLED
...
void xmlXPathEvalExpr(xmlXPathParserContextPtr ctxt) {
...
#endif
  {
     ...
     */
    if ((ctxt->error == XPATH_EXPRESSION_OK) && (ctxt->comp != NULL) &&
        (ctxt->base != NULL) && (ctxt->comp->nbStep > 2) &&
        (ctxt->comp->last >= 0) &&
 ...
 *
 */
static void xmlXPathEscapeUriFunction(xmlXPathParserContextPtr ctxt,
                                      int nargs) {
  xmlXPathObjectPtr str;
  int escape_reserved;
  xmlBufferPtr target;
