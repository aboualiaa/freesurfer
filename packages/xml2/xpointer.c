 ...
 * Handle a redefinition of attribute error
 */
static void xmlXPtrErr(xmlXPathParserContextPtr ctxt, int error,
                       const char *msg, const xmlChar *extra) {
  if (ctxt != NULL)
    ctxt->error = error;
  if ((ctxt == NULL) || (ctxt->context == NULL)) {
    __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPOINTER, error,
                    XML_ERR_ERROR, NULL, 0, (const char *)extra, NULL, NULL, 0,
                    0, msg, extra);
    return;
  }
  ...
  if (ctxt->context->error != NULL) {
    ...
    __xmlRaiseError(NULL, NULL, NULL, NULL, ctxt->context->debugNode,
                    XML_FROM_XPOINTER, error, XML_ERR_ERROR, NULL, 0,
 ...
 */
static int xmlXPtrCmpPoints(xmlNodePtr node1, int index1, xmlNodePtr node2,
                            int index2) {
...
static void xmlXPtrRangeCheckOrder(xmlXPathObjectPtr range) {
  ...
  if (range->user2 == NULL)
    return;
  tmp =
      xmlXPtrCmpPoints(range->user, range->index, range->user2, range->index2);
  if (tmp == -1) {
    tmp2 = range->user;
 ...
 */
static int xmlXPtrRangesEqual(xmlXPathObjectPtr range1,
...
void xmlXPtrLocationSetAdd(xmlLocationSetPtr cur, xmlXPathObjectPtr val) {
  ...
  if (cur->locMax == 0) {
    ...
    cur->locMax *= 2;
    temp = (xmlXPathObjectPtr *)xmlRealloc(
        cur->locTab, cur->locMax * sizeof(xmlXPathObjectPtr));
    if (temp == NULL) {
      xmlXPtrErrMemory("adding location to set");
      return;
    }
...
static void xmlXPtrStringRangeFunction(xmlXPathParserContextPtr ctxt,
                                       int nargs);
...
static void xmlXPtrRangeInsideFunction(xmlXPathParserContextPtr ctxt,
                                       int nargs);
 ...
 *
 */
static void xmlXPtrStartPointFunction(xmlXPathParserContextPtr ctxt,
                                      int nargs) {
  ...
  CHECK_ARITY(1);
  if ((ctxt->value == NULL) || ((ctxt->value->type != XPATH_LOCATIONSET) &&
 ...
 */
static xmlXPathObjectPtr xmlXPtrCoveringRange(xmlXPathParserContextPtr ctxt,
 ...
 * Returns a new location or NULL in case of error
 */
static xmlXPathObjectPtr xmlXPtrInsideRange(xmlXPathParserContextPtr ctxt,
                                            xmlXPathObjectPtr loc) {
  if (loc == NULL)
    return (NULL);
  if ((ctxt == NULL) || (ctxt->context == NULL) || (ctxt->context->doc == NULL))
 ...
 *            of the range
 */
static int xmlXPtrMatchString(const xmlChar *string, xmlNodePtr start,
                              int startindex, xmlNodePtr *end, int *endindex) {
  xmlNodePtr cur;
  int pos;       /* 0 based */
  int len;       /* in bytes */
...
#ifdef DEBUG_RANGES
          xmlGenericError(xmlGenericErrorContext,
                          "found range %d bytes at index %d of ->", stringlen,
                          pos + 1);
...
#ifdef DEBUG_RANGES
          xmlGenericError(xmlGenericErrorContext,
                          "found subrange %d bytes at index %d of ->", sub,
                          pos + 1);
          xmlDebugDumpString(stdout, cur->content);
          xmlGenericError(xmlGenericErrorContext, "\n");
#endif
 ...
 *            of the range
 */
static int xmlXPtrSearchString(const xmlChar *string, xmlNodePtr *start,
                               int *startindex, xmlNodePtr *end,
                               int *endindex) {
  xmlNodePtr cur;
  const xmlChar *str;
  int pos; /* 0 based */
    ...
    if ((cur->type != XML_ELEMENT_NODE) && (cur->content != NULL)) {
      ...
      while (pos <= len) {
        if (first != 0) {
...
#ifdef DEBUG_RANGES
            xmlGenericError(xmlGenericErrorContext,
                            "found '%c' at index %d of ->", first, pos + 1);
            xmlDebugDumpString(stdout, cur->content);
            xmlGenericError(xmlGenericErrorContext, "\n");
#endif
           ...
           */
#ifdef DEBUG_RANGES
          xmlGenericError(xmlGenericErrorContext, "found '' at index %d of ->",
                          pos + 1);
          xmlDebugDumpString(stdout, cur->content);
          xmlGenericError(xmlGenericErrorContext, "\n");
#endif
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetStartPoint(xmlXPathObjectPtr obj, xmlNodePtr *node,
                                int *indx) {
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetEndPoint(xmlXPathObjectPtr obj, xmlNodePtr *node,
                              int *indx) {
  if ((obj == NULL) || (node == NULL) || (indx == NULL))
    return (-1);
...
static void xmlXPtrStringRangeFunction(xmlXPathParserContextPtr ctxt,
  ...
  for (i = 0; i < oldset->locNr; i++) {
    ...
    do {
      fend = end;
      fendindex = endindex;
      found = xmlXPtrSearchString(string->stringval, &start, &startindex, &fend,
                                  &fendindex);
      if (found == 1) {
        if (position == NULL) {
          xmlXPtrLocationSetAdd(
              newset, xmlXPtrNewRange(start, startindex, fend, fendindex));
          ...
          if ((number != NULL) && (num > 0)) {
            ...
            if (xmlXPtrAdvanceChar(&rend, &rindx, num) == 0) {
              xmlXPtrLocationSetAdd(
                  newset, xmlXPtrNewRange(start, startindex, rend, rindx));
            }
          } else if ((number != NULL) && (num <= 0)) {
            xmlXPtrLocationSetAdd(
                newset, xmlXPtrNewRange(start, startindex, start, startindex));
          } else {
            xmlXPtrLocationSetAdd(
                newset, xmlXPtrNewRange(start, startindex, fend, fendindex));
          }
