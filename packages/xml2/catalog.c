...
typedef enum { XML_XML_CATALOG_TYPE = 1, XML_SGML_CATALOG_TYPE } xmlCatalogType;
...
static void xmlCatalogErrMemory(const char *extra) {
  __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_CATALOG,
                  XML_ERR_NO_MEMORY, XML_ERR_ERROR, NULL, 0, extra, NULL, NULL,
 ...
 */
static void xmlCatalogErr(xmlCatalogEntryPtr catal, xmlNodePtr node, int error,
                          const char *msg, const xmlChar *str1,
                          const xmlChar *str2, const xmlChar *str3) {
  __xmlRaiseError(NULL, NULL, NULL, catal, node, XML_FROM_CATALOG, error,
                  XML_ERR_ERROR, NULL, 0, (const char *)str1,
                  (const char *)str2, (const char *)str3, 0, 0, msg, str1, str2,
                  str3);
}
...
xmlNewCatalogEntry(xmlCatalogEntryType type, const xmlChar *name,
                   const xmlChar *value, const xmlChar *URL,
...
static void xmlFreeCatalogEntry(xmlCatalogEntryPtr ret) {
  ...
  if (xmlDebugCatalogs) {
    if (ret->name != NULL)
      xmlGenericError(xmlGenericErrorContext, "Free catalog entry %s\n",
                      ret->name);
    else if (ret->value != NULL)
      xmlGenericError(xmlGenericErrorContext, "Free catalog entry %s\n",
                      ret->value);
    else
      xmlGenericError(xmlGenericErrorContext, "Free catalog entry\n");
  }
 ...
 */
static xmlCatalogPtr xmlCreateNewCatalog(xmlCatalogType type,
                                         xmlCatalogPrefer prefer) {
  xmlCatalogPtr ret;
 ...
 */
static void xmlCatalogConvertEntry(xmlCatalogEntryPtr entry,
                                   xmlCatalogPtr catal) {
...
int xmlConvertSGMLCatalog(xmlCatalogPtr catal) {
  ...
  if (xmlDebugCatalogs) {
    xmlGenericError(xmlGenericErrorContext, "Converting SGML catalog to XML\n");
  }
...
static xmlChar *xmlLoadFileContent(const char *filename) {
  ...
  size = info.st_size;
#else
  if (fseek(fd, 0, SEEK_END) || (size = ftell(fd)) == EOF ||
      fseek(fd, 0, SEEK_SET)) { /* File operations denied? ok, just close and
                                   return failure */
    fclose(fd);
    return (NULL);
  }
...
static xmlCatalogEntryPtr xmlParseXMLCatalogFile(xmlCatalogPrefer prefer,
                                                 const xmlChar *filename);
static void xmlParseXMLCatalogNodeList(xmlNodePtr cur, xmlCatalogPrefer prefer,
                                       xmlCatalogEntryPtr parent,
                                       xmlCatalogEntryPtr cgroup);
static xmlChar *xmlCatalogListXMLResolve(xmlCatalogEntryPtr catal,
                                         const xmlChar *pubID,
                                         const xmlChar *sysID);
static xmlChar *xmlCatalogListXMLResolveURI(xmlCatalogEntryPtr catal,
                                            const xmlChar *URI);
...
xmlParseXMLCatalogOneNode(xmlNodePtr cur, xmlCatalogEntryType type,
  ...
  if (URL != NULL) {
    if (xmlDebugCatalogs > 1) {
      if (nameValue != NULL)
        xmlGenericError(xmlGenericErrorContext, "Found %s: '%s' '%s'\n", name,
                        nameValue, URL);
      else
        xmlGenericError(xmlGenericErrorContext, "Found %s: '%s'\n", name, URL);
    }
...
static void xmlParseXMLCatalogNode(xmlNodePtr cur, xmlCatalogPrefer prefer,
  ...
  if (xmlStrEqual(cur->name, BAD_CAST "group")) {
    ...
    entry = xmlParseXMLCatalogOneNode(cur, XML_CATA_PUBLIC, BAD_CAST "public",
                                      BAD_CAST "publicId", BAD_CAST "uri",
                                      prefer, cgroup);
  } else if (xmlStrEqual(cur->name, BAD_CAST "system")) {
    entry = xmlParseXMLCatalogOneNode(cur, XML_CATA_SYSTEM, BAD_CAST "system",
                                      BAD_CAST "systemId", BAD_CAST "uri",
                                      prefer, cgroup);
    ...
    entry = xmlParseXMLCatalogOneNode(
        cur, XML_CATA_DELEGATE_PUBLIC, BAD_CAST "delegatePublic",
        BAD_CAST "publicIdStartString", BAD_CAST "catalog", prefer, cgroup);
  } else if (xmlStrEqual(cur->name, BAD_CAST "delegateSystem")) {
    entry = xmlParseXMLCatalogOneNode(
        cur, XML_CATA_DELEGATE_SYSTEM, BAD_CAST "delegateSystem",
        BAD_CAST "systemIdStartString", BAD_CAST "catalog", prefer, cgroup);
  } else if (xmlStrEqual(cur->name, BAD_CAST "uri")) {
    entry = xmlParseXMLCatalogOneNode(cur, XML_CATA_URI, BAD_CAST "uri",
                                      BAD_CAST "name", BAD_CAST "uri", prefer,
                                      cgroup);
  } else if (xmlStrEqual(cur->name, BAD_CAST "rewriteURI")) {
    entry = xmlParseXMLCatalogOneNode(
        cur, XML_CATA_REWRITE_URI, BAD_CAST "rewriteURI",
        BAD_CAST "uriStartString", BAD_CAST "rewritePrefix", prefer, cgroup);
  } else if (xmlStrEqual(cur->name, BAD_CAST "delegateURI")) {
    entry = xmlParseXMLCatalogOneNode(
        cur, XML_CATA_DELEGATE_URI, BAD_CAST "delegateURI",
        BAD_CAST "uriStartString", BAD_CAST "catalog", prefer, cgroup);
...
static xmlCatalogEntryPtr xmlParseXMLCatalogFile(xmlCatalogPrefer prefer,
  ...
  if (doc == NULL) {
    if (xmlDebugCatalogs)
      xmlGenericError(xmlGenericErrorContext, "Failed to parse catalog %s\n",
                      filename);
    return (NULL);
  }

  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "%d Parsing catalog %s\n",
...
static int xmlFetchXMLCatalogFile(xmlCatalogEntryPtr catal) {
  ...
  if (xmlCatalogXMLFiles != NULL) {
    ...
    if (doc != NULL) {
      if (xmlDebugCatalogs)
        xmlGenericError(xmlGenericErrorContext, "Found %s in file hash\n",
                        catal->URL);
    ...
    if (xmlDebugCatalogs)
      xmlGenericError(xmlGenericErrorContext, "%s not found in file hash\n",
                      catal->URL);
  }
  ...
  if (xmlCatalogXMLFiles != NULL) {
    if (xmlDebugCatalogs)
      xmlGenericError(xmlGenericErrorContext, "%s added to file hash\n",
                      catal->URL);
 ...
 */
static int xmlAddXMLCatalog(xmlCatalogEntryPtr catal, const xmlChar *type,
  ...
  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "Adding element %s to catalog\n",
                    type);
  if (cur == NULL)
    catal->children =
        xmlNewCatalogEntry(typ, orig, replace, NULL, catal->prefer, NULL);
  else
    cur->next =
        xmlNewCatalogEntry(typ, orig, replace, NULL, catal->prefer, NULL);
  if (doregister) {
 ...
 */
static xmlChar *xmlCatalogXMLResolve(xmlCatalogEntryPtr catal,
                                     const xmlChar *pubID,
                                     const xmlChar *sysID) {
  xmlChar *ret = NULL;
  xmlCatalogEntryPtr cur;
  ...
  if (sysID != NULL) {
    ...
    if (rewrite != NULL) {
      if (xmlDebugCatalogs)
        xmlGenericError(xmlGenericErrorContext, "Using rewriting rule %s\n",
                        rewrite->name);
    ...
    while (cur != NULL) {
      ...
      case XML_CATA_PUBLIC:
        if (xmlStrEqual(pubID, cur->name)) {
          if (xmlDebugCatalogs)
            xmlGenericError(xmlGenericErrorContext, "Found public match %s\n",
                            cur->name);
          catal->depth--;
          return (xmlStrdup(cur->URL));
        }
 ...
 * Returns the URI of the resource or NULL if not found
 */
static xmlChar *xmlCatalogXMLResolveURI(xmlCatalogEntryPtr catal,
                                        const xmlChar *URI) {
  xmlChar *ret = NULL;
  xmlCatalogEntryPtr cur;
  int haveDelegate = 0;
  ...
  while (cur != NULL) {
    ...
    case XML_CATA_URI:
      if (xmlStrEqual(URI, cur->name)) {
        if (xmlDebugCatalogs)
          xmlGenericError(xmlGenericErrorContext, "Found URI match %s\n",
                          cur->name);
        return (xmlStrdup(cur->URL));
      }
      break;
  ...
  if (haveDelegate) {
    ...
    while (cur != NULL) {
      if (((cur->type == XML_CATA_DELEGATE_SYSTEM) ||
        ...
        if (cur->children != NULL) {
          if (xmlDebugCatalogs)
            xmlGenericError(xmlGenericErrorContext, "Trying URI delegate %s\n",
                            cur->URL);
...
static xmlChar *xmlCatalogListXMLResolveURI(xmlCatalogEntryPtr catal,
  ...
  if (!xmlStrncmp(URI, BAD_CAST XML_URN_PUBID, sizeof(XML_URN_PUBID) - 1)) {
    ...
    if (xmlDebugCatalogs) {
      if (urnID == NULL)
        xmlGenericError(xmlGenericErrorContext, "URN ID %s expanded to NULL\n",
                        URI);
      else
        xmlGenericError(xmlGenericErrorContext, "URN ID expanded to %s\n",
                        urnID);
    }
    ret = xmlCatalogListXMLResolve(catal, urnID, NULL);
    if (urnID != NULL)
 ...
 */
static const xmlChar *xmlParseSGMLCatalogPubid(const xmlChar *cur,
                                               xmlChar **id) {
 ...
 */
static const xmlChar *xmlParseSGMLCatalogName(const xmlChar *cur,
                                              xmlChar **name) {
 ...
 */
static int xmlParseSGMLCatalog(xmlCatalogPtr catal, const xmlChar *value,
    ...
    if ((cur[0] == '-') && (cur[1] == '-')) {
      ...
      if (cur == NULL) {
        ...
        if (filename != NULL) {
          ...
          entry = xmlNewCatalogEntry(type, name, filename, NULL,
 ...
 */
static const xmlChar *xmlCatalogGetSGMLPublic(xmlHashTablePtr catal,
                                              const xmlChar *pubID) {
  xmlCatalogEntryPtr entry;
  xmlChar *normid;
 ...
 * Returns the local resource if found or NULL otherwise.
 */
static const xmlChar *xmlCatalogGetSGMLSystem(xmlHashTablePtr catal,
                                              const xmlChar *sysID) {
  xmlCatalogEntryPtr entry;
 ...
 * Returns the URI of the resource or NULL if not found
 */
static const xmlChar *xmlCatalogSGMLResolve(xmlCatalogPtr catal,
                                            const xmlChar *pubID,
                                            const xmlChar *sysID) {
...
xmlCatalogPtr xmlLoadACatalog(const char *filename) {
  ...
  if (*first != '<') {
    ...
    catal->xml =
        xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, NULL, BAD_CAST filename,
...
static int xmlExpandCatalog(xmlCatalogPtr catal, const char *filename) {
  ...
  if (catal->type == XML_SGML_CATALOG_TYPE) {
    ...
    tmp = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, NULL, BAD_CAST filename,
                             xmlCatalogDefaultPrefer, NULL);
...
xmlChar *xmlACatalogResolveSystem(xmlCatalogPtr catal, const xmlChar *sysID) {
  ...
  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "Resolve sysID %s\n", sysID);
 ...
 * @pubID:  the public ID string
 *
 * Try to lookup the catalog local reference associated to a public ID in that
 * catalog
 *
...
xmlChar *xmlACatalogResolvePublic(xmlCatalogPtr catal, const xmlChar *pubID) {
  ...
  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "Resolve pubID %s\n", pubID);
 ...
 *      by the caller.
 */
xmlChar *xmlACatalogResolve(xmlCatalogPtr catal, const xmlChar *pubID,
                            const xmlChar *sysID) {
  xmlChar *ret = NULL;
  ...
  if (xmlDebugCatalogs) {
    if ((pubID != NULL) && (sysID != NULL)) {
      xmlGenericError(xmlGenericErrorContext, "Resolve: pubID %s sysID %s\n",
                      pubID, sysID);
    } else if (pubID != NULL) {
      xmlGenericError(xmlGenericErrorContext, "Resolve: pubID %s\n", pubID);
    } else {
...
xmlChar *xmlACatalogResolveURI(xmlCatalogPtr catal, const xmlChar *URI) {
  ...
  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "Resolve URI %s\n", URI);
 ...
 * Returns 0 if successful, -1 otherwise
 */
int xmlACatalogAdd(xmlCatalogPtr catal, const xmlChar *type,
                   const xmlChar *orig, const xmlChar *replace) {
  int res = -1;
 ...
 */
static void xmlInitializeCatalogData(void) {
    ...
    if (catal != NULL) {
      ...
      while (*cur != '\0') {
        ...
        if (*cur != 0) {
          ...
          if (path != NULL) {
            *nextent =
                xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, NULL, BAD_CAST path,
                                   xmlCatalogDefaultPrefer, NULL);
            if (*nextent != NULL)
...
void xmlCatalogCleanup(void) {
  ...
  xmlRMutexLock(xmlCatalogMutex);
  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "Catalogs cleanup\n");
  if (xmlCatalogXMLFiles != NULL)
 ...
 * Returns 0 if successful, -1 otherwise
 */
int xmlCatalogAdd(const xmlChar *type, const xmlChar *orig,
  ...
  if ((xmlDefaultCatalog == NULL) && (xmlStrEqual(type, BAD_CAST "catalog"))) {
    ...
    xmlDefaultCatalog->xml = xmlNewCatalogEntry(
        XML_CATA_CATALOG, NULL, orig, NULL, xmlCatalogDefaultPrefer, NULL);
 ...
 */
xmlCatalogAllow xmlCatalogGetDefaults(void) { return (xmlCatalogDefaultAllow); }
...
void xmlCatalogSetDefaults(xmlCatalogAllow allow) {
  if (xmlDebugCatalogs) {
    ...
    case XML_CATA_ALLOW_NONE:
      xmlGenericError(xmlGenericErrorContext, "Disabling catalog usage\n");
      break;
    ...
    case XML_CATA_ALLOW_ALL:
      xmlGenericError(xmlGenericErrorContext, "Allowing all catalogs\n");
      break;
    }
  }
...
void *xmlCatalogAddLocal(void *catalogs, const xmlChar *URL) {
  ...
  if (xmlDebugCatalogs)
    xmlGenericError(xmlGenericErrorContext, "Adding document catalog %s\n",
                    URL);
 ...
 *      by the caller.
 */
xmlChar *xmlCatalogLocalResolve(void *catalogs, const xmlChar *pubID,
                                const xmlChar *sysID) {
  xmlCatalogEntryPtr catal;
  xmlChar *ret;
  ...
  if (xmlDebugCatalogs) {
    if ((pubID != NULL) && (sysID != NULL)) {
      ...
      xmlGenericError(xmlGenericErrorContext, "Local Resolve: pubID %s\n",
                      pubID);
    } else {
      xmlGenericError(xmlGenericErrorContext, "Local Resolve: sysID %s\n",
                      sysID);
    }
  }
