...
void xmlNanoHTTPScanProxy(const char *URL) {
  ...
  uri = xmlParseURIRaw(URL, 1);
  if ((uri == NULL) || (uri->scheme == NULL) || (strcmp(uri->scheme, "http")) ||
      (uri->server == NULL)) {
...
static int xmlNanoHTTPSend(xmlNanoHTTPCtxtPtr ctxt, const char *xmt_ptr,
                           int outlen) {
  int total_sent = 0;
#ifdef HAVE_POLL_H
  struct pollfd p;
...
static void xmlNanoHTTPScanAnswer(xmlNanoHTTPCtxtPtr ctxt, const char *line) {
  ...
  } else if (!xmlStrncasecmp(BAD_CAST line,
                             BAD_CAST "Proxy-Authenticate:", 19)) {
    cur += 19;
    while ((*cur == ' ') || (*cur == '\t'))
      cur++;
    if (ctxt->authHeader != NULL)
...
retry:
  ...
  if (proxy) {
    if (ctxt->port != 80) {
      p += snprintf(p, blen - (p - bp), "%s http://%s:%d%s", method,
                    ctxt->hostname, ctxt->port, ctxt->path);
    } else
...
#ifdef DEBUG_HTTP
  xmlGenericError(xmlGenericErrorContext, "-> %s%s", proxy ? "(Proxy) " : "",
                  bp);
  ...
  if (xmt_bytes != blen)
    xmlGenericError(xmlGenericErrorContext,
                    ...
                    blen, "bytes of HTTP headers sent to host", ctxt->hostname);
#else
  ...
  if (input != NULL) {
    ...
    if (xmt_bytes != ilen)
      xmlGenericError(xmlGenericErrorContext,
                      "xmlNanoHTTPMethodRedir:  Only %d of %d %s %s\n",
                      xmt_bytes, ilen, "bytes of HTTP content sent to host",
                      ctxt->hostname);
#else
...
void *xmlNanoHTTPMethod(const char *URL, const char *method, const char *input,
                        char **contentType, const char *headers, int ilen) {
  return (xmlNanoHTTPMethodRedir(URL, method, input, contentType, NULL, headers,
                                 ilen));
}
