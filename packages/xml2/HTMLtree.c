...
static int htmlNodeDumpFormat(xmlBufferPtr buf, xmlDocPtr doc, xmlNodePtr cur,
                              int format);
 ...
 */
int htmlNodeDumpFileFormat(FILE *out, xmlDocPtr doc, xmlNodePtr cur,
 ...
 */
void htmlDocDumpMemoryFormat(xmlDocPtr cur, xmlChar **mem, int *size,
                             int format) {
  xmlOutputBufferPtr buf;
  xmlCharEncodingHandlerPtr handler = NULL;
  const char *encoding;
 ...
 */
static void htmlDtdDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
 ...
 */
static void htmlAttrDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
                               xmlAttrPtr cur,
                               const char *encoding ATTRIBUTE_UNUSED) {
  xmlChar *value;
 ...
 */
static void htmlAttrListDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
 ...
 */
static void htmlNodeListDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
...
void htmlNodeDumpFormatOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
  ...
  if ((info != NULL) && (info->empty)) {
    ...
    if ((format) && (!info->isinline) && (cur->next != NULL)) {
      if ((cur->next->type != HTML_TEXT_NODE) &&
          (cur->next->type != HTML_ENTITY_REF_NODE) && (cur->parent != NULL) &&
  ...
  if (((cur->type == XML_ELEMENT_NODE) || (cur->content == NULL)) &&
    ...
    if ((format) && (cur->next != NULL) && (info != NULL) &&
        (!info->isinline)) {
      if ((cur->next->type != HTML_TEXT_NODE) &&
          (cur->next->type != HTML_ENTITY_REF_NODE) && (cur->parent != NULL) &&
          (cur->parent->name != NULL) &&
  ...
  if (cur->children != NULL) {
    if ((format) && (info != NULL) && (!info->isinline) &&
        ...
        (cur->children != cur->last) && (cur->name != NULL) &&
 ...
 */
void htmlNodeDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc, xmlNodePtr cur,
                        const char *encoding) {
 ...
 */
void htmlDocContentDumpFormatOutput(xmlOutputBufferPtr buf, xmlDocPtr cur,
                                    const char *encoding, int format) {
  int type;
 ...
 */
void htmlDocContentDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr cur,
                              const char *encoding) {
 ...
 * returns: the number of byte written or -1 in case of failure.
 */
int htmlSaveFileFormat(const char *filename, xmlDocPtr cur,
                       const char *encoding, int format) {
  xmlOutputBufferPtr buf;
  xmlCharEncodingHandlerPtr handler = NULL;
  int ret;
