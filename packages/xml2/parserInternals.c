...
void __xmlErrEncoding(xmlParserCtxtPtr ctxt, xmlParserErrors xmlerr,
  ...
  if (ctxt != NULL)
    ctxt->errNo = xmlerr;
  __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER, xmlerr,
...
static void xmlErrInternal(xmlParserCtxtPtr ctxt, const char *msg,
  ...
  __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER,
                  XML_ERR_INTERNAL_ERROR, XML_ERR_FATAL, NULL, 0,
                  (const char *)str, NULL, NULL, 0, 0, msg, str);
  if (ctxt != NULL) {
    ctxt->wellFormed = 0;
    if (ctxt->recovery == 0)
      ctxt->disableSAX = 1;
 ...
 */
int xmlIsLetter(int c) { return (IS_BASECHAR(c) || IS_IDEOGRAPHIC(c)); }
...
static void check_buffer(xmlParserInputPtr in) {
  ...
  xmlGenericError(xmlGenericErrorContext,
                  ...
                  (int)in->buf->buffer->content, in->cur - in->base,
      ...
      if (c & 0x80) {
        ...
        if ((c & 0xe0) == 0xe0) {
          ...
          if (((val > 0xd7ff) && (val < 0xe000)) ||
              ((val > 0xfffd) && (val < 0x10000)) || (val >= 0x110000)) {
...
int xmlSwitchEncoding(xmlParserCtxtPtr ctxt, xmlCharEncoding enc) {
  ...
  case XML_CHAR_ENCODING_UTF8:
     ...
     * UTF-8
     */
    if ((ctxt->input != NULL) && (ctxt->input->cur[0] == 0xEF) &&
  ...
  case XML_CHAR_ENCODING_UTF16BE:
    ...
    if ((ctxt->input != NULL) && (ctxt->input->cur != NULL) &&
        (ctxt->input->cur[0] == 0xEF) && (ctxt->input->cur[1] == 0xBB) &&
  ...
  if (handler == NULL) {
    ...
    case XML_CHAR_ENCODING_8859_9:
       ...
       * values for this now.
       */
      if ((ctxt->inputNr == 1) && (ctxt->encoding == NULL) &&
          (ctxt->input != NULL) && (ctxt->input->encoding != NULL)) {
...
static int xmlSwitchInputEncodingInt(xmlParserCtxtPtr ctxt,
  ...
  if (input->buf != NULL) {
    ...
    if ((input->buf->buffer != NULL) && (input->buf->buffer->use > 0)) {
      ...
      if ((handler->name != NULL) && (!strcmp(handler->name, "UTF-16BE")) &&
       ...
       * UTF-8
       */
      if ((handler->name != NULL) && (!strcmp(handler->name, "UTF-8")) &&
          (input->cur[0] == 0xEF) && (input->cur[1] == 0xBB) &&
          (input->cur[2] == 0xBF)) {
        input->cur += 3;
      }
      ...
      if (ctxt->html) {
         ...
         */
        nbchars = xmlCharEncInFunc(input->buf->encoder, input->buf->buffer,
                                   input->buf->raw);
      } else {
        /*
         ...
         */
        nbchars = xmlCharEncFirstLineInt(
            input->buf->encoder, input->buf->buffer, input->buf->raw, len);
      }
      if (nbchars < 0) {
...
xmlParserInputPtr xmlNewInputFromFile(xmlParserCtxtPtr ctxt,
  ...
  if (buf == NULL) {
    if (filename == NULL)
      __xmlLoaderErr(ctxt, "failed to load external entity: NULL filename \n",
                     NULL);
    else
...
void xmlFreeParserCtxt(xmlParserCtxtPtr ctxt) {
  ...
  if (ctxt->version != NULL)
    xmlFree((char *)ctxt->version);
  if (ctxt->encoding != NULL)
    xmlFree((char *)ctxt->encoding);
  if (ctxt->extSubURI != NULL)
    xmlFree((char *)ctxt->extSubURI);
  if (ctxt->extSubSystem != NULL)
    xmlFree((char *)ctxt->extSubSystem);
...
void xmlParserAddNodeInfo(xmlParserCtxtPtr ctxt,
  ...
  /* Otherwise, we need to add new node to buffer */
    ...
    if (ctxt->node_seq.length + 1 > ctxt->node_seq.maximum) {
      ...
      byte_size =
          (sizeof(*ctxt->node_seq.buffer) * (2 * ctxt->node_seq.maximum));

      if (ctxt->node_seq.buffer == NULL)
        tmp_buffer = (xmlParserNodeInfo *)xmlMalloc(byte_size);
      else
        tmp_buffer =
            (xmlParserNodeInfo *)xmlRealloc(ctxt->node_seq.buffer, byte_size);
