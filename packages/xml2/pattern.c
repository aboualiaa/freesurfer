...
#define XML_PATTERN_NOTPATTERN                                                 \
  (XML_PATTERN_XPATH | XML_PATTERN_XSSEL | XML_PATTERN_XSFIELD)

#define XML_STREAM_XS_IDC(c)                                                   \
  ((c)->flags & (XML_PATTERN_XSSEL | XML_PATTERN_XSFIELD))
 ...
 */
static int xmlPatternAdd(xmlPatParserContextPtr ctxt ATTRIBUTE_UNUSED,
                         xmlPatternPtr comp, xmlPatOp op, xmlChar *value,
                         xmlChar *value2) {
  if (comp->nbStep >= comp->maxStep) {
    xmlStepOpPtr temp;
    temp = (xmlStepOpPtr)xmlRealloc(comp->steps,
                                    comp->maxStep * 2 * sizeof(xmlStepOp));
    if (temp == NULL) {
...
static int xmlPatPushState(xmlStepStates *states, int step, xmlNodePtr node) {
  if ((states->states == NULL) || (states->maxstates <= 0)) {
    ...
    tmp = (xmlStepStatePtr)xmlRealloc(states->states, 2 * states->maxstates *
                                                          sizeof(xmlStepState));
    if (tmp == NULL)
      return (-1);
    states->states = tmp;
...
static xmlChar *xmlPatScanName(xmlPatParserContextPtr ctxt) {
  ...
  while ((IS_LETTER(val)) || (IS_DIGIT(val)) || (val == '.') || (val == '-') ||
         (val == '_') || (IS_COMBINING(val)) || (IS_EXTENDER(val))) {
    cur += len;
    val = xmlStringCurrentChar(NULL, cur, &len);
  }
  if (ctxt->dict)
...
static void xmlCompileAttributeTest(xmlPatParserContextPtr ctxt) {
  ...
  if (CUR == ':') {
    ...
    if ((prefix[0] == 'x') && (prefix[1] == 'm') && (prefix[2] == 'l') &&
        (prefix[3] == 0)) {
 ...
 */
static int xmlStreamCompAddStep(xmlStreamCompPtr comp, const xmlChar *name,
                                const xmlChar *ns, int nodeType, int flags) {
  xmlStreamStepPtr cur;

  if (comp->nbStep >= comp->maxStep) {
    cur = (xmlStreamStepPtr)xmlRealloc(comp->steps, comp->maxStep * 2 *
...
static int xmlStreamCompile(xmlPatternPtr comp) {
   ...
   * special case for .
   */
  if ((comp->nbStep == 1) && (comp->steps[0].op == XML_OP_ELEM) &&
      (comp->steps[0].value == NULL) && (comp->steps[0].value2 == NULL)) {
  ...
  for (; i < comp->nbStep; i++) {
    ...
    case XML_OP_ROOT:
      ...
      break;
    case XML_OP_NS:
      s = xmlStreamCompAddStep(stream, NULL, step.value, XML_ELEMENT_NODE,
                               flags);
      if (s < 0)
        goto error;
      prevs = s;
    ...
    case XML_OP_ELEM:
      if ((step.value == NULL) && (step.value2 == NULL)) {
        ...
        if ((comp->nbStep == i + 1) && (flags & XML_STREAM_STEP_DESC)) {
          ...
          s = xmlStreamCompAddStep(stream, NULL, NULL, XML_STREAM_ANY_NODE,
                                   flags);
          if (s < 0)
            goto error;
          flags = 0;
...
static int xmlStreamCtxtAddState(xmlStreamCtxtPtr comp, int idx, int level) {
  ...
  if (comp->nbState >= comp->maxState) {
    ...
    cur = (int *)xmlRealloc(comp->states, comp->maxState * 4 * sizeof(int));
    if (cur == NULL) {
      ERROR(NULL, NULL, NULL, "xmlNewStreamCtxt: malloc failed\n");
      return (-1);
    }
 ...
 */
static int xmlStreamPushInternal(xmlStreamCtxtPtr stream, const xmlChar *name,
  ...
  while (stream != NULL) {
    ...
    while (i < m) {
      ...
      } else if (((step.ns != NULL) == (ns != NULL)) && (name != NULL) &&
                 (step.name[0] == name[0]) && xmlStrEqual(step.name, name) &&
 ...
 *    match and 0 otherwise.
 */
int xmlStreamPush(xmlStreamCtxtPtr stream, const xmlChar *name,
                  const xmlChar *ns) {
 ...
 * @ns: the namespace name
 *
 * Push new attribute data onto the stream. NOTE: if the call
 * xmlPatterncompile() indicated a dictionary, then strings for name and ns will
 * be expected to come from the dictionary. Both @name and @ns being NULL means
 * the / i.e. the root of the document. This can also act as a reset. Otherwise
 * the function will act as if it has been given an attribute-node.
 *
 * Returns: -1 in case of error, 1 if the current state in the stream is a
 *    match and 0 otherwise.
 */
 ...
 */
xmlPatternPtr xmlPatterncompile(const xmlChar *pattern, xmlDict *dict,
                                int flags, const xmlChar **namespaces) {
