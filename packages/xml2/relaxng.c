 ...
 */
static const xmlChar *xmlRelaxNGNs =
    (const xmlChar *)"http://relaxng.org/ns/structure/1.0";
 ...
 */
static void xmlRngPErrMemory(xmlRelaxNGParserCtxtPtr ctxt, const char *extra) {
  ...
  if (extra)
    __xmlRaiseError(schannel, channel, data, NULL, NULL, XML_FROM_RELAXNGP,
 ...
 * Handle a redefinition of attribute error
 */
static void xmlRngVErrMemory(xmlRelaxNGValidCtxtPtr ctxt, const char *extra) {
  xmlStructuredErrorFunc schannel = NULL;
  xmlGenericErrorFunc channel = NULL;
  void *data = NULL;
  ...
  if (extra)
    __xmlRaiseError(schannel, channel, data, NULL, NULL, XML_FROM_RELAXNGV,
                    XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL,
                    NULL, 0, 0, "Memory allocation failed : %s\n", extra);
  else
    __xmlRaiseError(schannel, channel, data, NULL, NULL, XML_FROM_RELAXNGV,
 ...
 */
static void xmlRngPErr(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node, int error,
  ...
  __xmlRaiseError(schannel, channel, data, NULL, node, XML_FROM_RELAXNGP, error,
 ...
 */
static void xmlRngVErr(xmlRelaxNGValidCtxtPtr ctxt, xmlNodePtr node, int error,
                       const char *msg, const xmlChar *str1,
                       const xmlChar *str2) {
  xmlStructuredErrorFunc schannel = NULL;
  xmlGenericErrorFunc channel = NULL;
  ...
  __xmlRaiseError(schannel, channel, data, NULL, node, XML_FROM_RELAXNGV, error,
                  XML_ERR_ERROR, NULL, 0, (const char *)str1,
                  (const char *)str2, NULL, 0, 0, msg, str1, str2);
}
...
typedef int (*xmlRelaxNGTypeCompare)(void *data, const xmlChar *type,
                                     const xmlChar *value1, xmlNodePtr ctxt1,
                                     void *comp1, const xmlChar *value2,
                                     xmlNodePtr ctxt2);
 ...
 * Returns the newly allocated structure or NULL in case or error
 */
static xmlRelaxNGDefinePtr xmlRelaxNGNewDefine(xmlRelaxNGParserCtxtPtr ctxt,
                                               xmlNodePtr node) {
  ...
  if (ctxt->defMax == 0) {
    ctxt->defMax = 16;
    ctxt->defNr = 0;
    ctxt->defTab = (xmlRelaxNGDefinePtr *)xmlMalloc(
    ...
    ctxt->defMax *= 2;
    tmp = (xmlRelaxNGDefinePtr *)xmlRealloc(
        ctxt->defTab, ctxt->defMax * sizeof(xmlRelaxNGDefinePtr));
    if (tmp == NULL) {
      xmlRngPErrMemory(ctxt, "allocating define\n");
      return (NULL);
    }
 ...
 * Returns the newly allocated structure or NULL in case or error
 */
static xmlRelaxNGStatesPtr xmlRelaxNGNewStates(xmlRelaxNGValidCtxtPtr ctxt,
                                               int size) {
 ...
 */
static int xmlRelaxNGAddStatesUniq(xmlRelaxNGValidCtxtPtr ctxt,
 ...
 * Return 1 in case of success and 0 if this is a duplicate and -1 on error
 */
static int xmlRelaxNGAddStates(xmlRelaxNGValidCtxtPtr ctxt,
                               xmlRelaxNGStatesPtr states,
                               xmlRelaxNGValidStatePtr state) {
  int i;
 ...
 */
static void xmlRelaxNGFreeStates(xmlRelaxNGValidCtxtPtr ctxt,
                                 xmlRelaxNGStatesPtr states) {
  if (states == NULL)
    return;
  if ((ctxt != NULL) && (ctxt->freeStates == NULL)) {
    ...
    ctxt->freeStatesNr = 0;
    ctxt->freeStates = (xmlRelaxNGStatesPtr *)xmlMalloc(
        ctxt->freeStatesMax * sizeof(xmlRelaxNGStatesPtr));
...
xmlRelaxNGCopyValidState(xmlRelaxNGValidCtxtPtr ctxt,
  ...
  if (state->nbAttrs > 0) {
    if (ret->attrs == NULL) {
      ...
      tmp = (xmlAttrPtr *)xmlRealloc(ret->attrs,
                                     state->maxAttrs * sizeof(xmlAttrPtr));
      if (tmp == NULL) {
        xmlRngVErrMemory(ctxt, "allocating states\n");
        ret->nbAttrs = 0;
        return (ret);
 ...
 */
static int xmlRelaxNGIncludePush(xmlRelaxNGParserCtxtPtr ctxt,
                                 xmlRelaxNGIncludePtr value) {
  if (ctxt->incTab == NULL) {
    ctxt->incMax = 4;
    ctxt->incNr = 0;
    ctxt->incTab = (xmlRelaxNGIncludePtr *)xmlMalloc(ctxt->incMax *
  ...
  if (ctxt->incNr >= ctxt->incMax) {
    ctxt->incMax *= 2;
    ctxt->incTab = (xmlRelaxNGIncludePtr *)xmlRealloc(
        ctxt->incTab, ctxt->incMax * sizeof(ctxt->incTab[0]));
    if (ctxt->incTab == NULL) {
      xmlRngPErrMemory(ctxt, "allocating include\n");
      return (0);
    }
 ...
 */
static int xmlRelaxNGRemoveRedefine(xmlRelaxNGParserCtxtPtr ctxt,
...
#endif
          if (xmlRelaxNGRemoveRedefine(ctxt, href, inc->doc->children->children,
                                       name) == 1) {
            found = 1;
          }
#ifdef DEBUG_INCLUDE
 ...
 */
static xmlRelaxNGIncludePtr xmlRelaxNGLoadInclude(xmlRelaxNGParserCtxtPtr ctxt,
                                                  const xmlChar *URL,
                                                  xmlNodePtr node,
                                                  const xmlChar *ns) {
...
#ifdef DEBUG_INCLUDE
  xmlGenericError(xmlGenericErrorContext, "xmlRelaxNGLoadInclude(%s)\n", URL);
#endif
  ...
  while (cur != NULL) {
    if (IS_RELAXNG(cur, "start")) {
      ...
      if (!found) {
        xmlRngPErr(
            ctxt, node, XML_RNGP_START_MISSING,
      ...
      if (name == NULL) {
        ...
        if (!found) {
          xmlRngPErr(ctxt, node, XML_RNGP_DEFINE_MISSING,
                     "xmlRelaxNG: include %s has a define %s but not the "
                     "included grammar\n",
                     URL, name);
        }
        xmlFree(name);
 ...
 * Returns 0 in case of error, the index in the stack otherwise
 */
static int xmlRelaxNGValidErrorPush(xmlRelaxNGValidCtxtPtr ctxt,
                                    xmlRelaxNGValidErr err, const xmlChar *arg1,
                                    const xmlChar *arg2, int dup) {
  xmlRelaxNGValidErrorPtr cur;

#ifdef DEBUG_ERROR
  xmlGenericError(xmlGenericErrorContext, "Pushing error %d at %d on stack\n",
                  err, ctxt->errNr);
#endif
  if (ctxt->errTab == NULL) {
    ctxt->errMax = 8;
 ...
 * Returns 0 in case of error, the index in the stack otherwise
 */
static int xmlRelaxNGDocumentPush(xmlRelaxNGParserCtxtPtr ctxt,
                                  xmlRelaxNGDocumentPtr value) {
  if (ctxt->docTab == NULL) {
    ctxt->docMax = 4;
    ctxt->docNr = 0;
    ctxt->docTab = (xmlRelaxNGDocumentPtr *)xmlMalloc(ctxt->docMax *
  ...
  if (ctxt->docNr >= ctxt->docMax) {
    ctxt->docMax *= 2;
    ctxt->docTab = (xmlRelaxNGDocumentPtr *)xmlRealloc(
        ctxt->docTab, ctxt->docMax * sizeof(ctxt->docTab[0]));
    if (ctxt->docTab == NULL) {
      xmlRngPErrMemory(ctxt, "adding document\n");
      return (0);
    }
 ...
 */
static xmlChar *xmlRelaxNGGetErrorString(xmlRelaxNGValidErr err,
                                         const xmlChar *arg1,
                                         const xmlChar *arg2) {
  char msg[1000];
 ...
 */
static void xmlRelaxNGShowValidError(xmlRelaxNGValidCtxtPtr ctxt,
  ...
  if (ctxt->errNo == XML_RELAXNG_OK)
    ctxt->errNo = err;
  xmlRngVErr(ctxt, (child == NULL ? node : child), err, (const char *)msg, arg1,
             arg2);
  xmlFree(msg);
}
...
#ifdef DEBUG_ERROR
  xmlGenericError(xmlGenericErrorContext, "Pop errors till level %d\n", level);
#endif
...
static void xmlRelaxNGDumpValidError(xmlRelaxNGValidCtxtPtr ctxt) {
...
#ifdef DEBUG_ERROR
  xmlGenericError(xmlGenericErrorContext, "Dumping error stack %d errors\n",
                  ctxt->errNr);
#endif
  for (i = 0, k = 0; i < ctxt->errNr; i++) {
    ...
    if (k < MAX_ERROR) {
      ...
      xmlRelaxNGShowValidError(ctxt, err->err, err->node, err->seq, err->arg1,
                               err->arg2);
      k++;
    }
  skip:
 ...
 */
static void xmlRelaxNGAddValidError(xmlRelaxNGValidCtxtPtr ctxt,
                                    xmlRelaxNGValidErr err, const xmlChar *arg1,
                                    const xmlChar *arg2, int dup) {
  if (ctxt == NULL)
    return;
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGSchemaTypeHave(void *data ATTRIBUTE_UNUSED,
                                    const xmlChar *type) {
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGSchemaTypeCheck(void *data ATTRIBUTE_UNUSED,
                                     const xmlChar *type, const xmlChar *value,
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
static void xmlRelaxNGSchemaFreeValue(void *data ATTRIBUTE_UNUSED,
                                      void *value) {
 ...
 */
static int xmlRelaxNGSchemaTypeCompare(void *data ATTRIBUTE_UNUSED,
                                       const xmlChar *type,
                                       const xmlChar *value1, xmlNodePtr ctxt1,
                                       void *comp1, const xmlChar *value2,
                                       xmlNodePtr ctxt2) {
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGDefaultTypeHave(void *data ATTRIBUTE_UNUSED,
                                     const xmlChar *type) {
  if (type == NULL)
    return (-1);
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGDefaultTypeCheck(void *data ATTRIBUTE_UNUSED,
...
static int xmlRelaxNGDefaultTypeCompare(
    void *data ATTRIBUTE_UNUSED, const xmlChar *type, const xmlChar *value1,
    xmlNodePtr ctxt1 ATTRIBUTE_UNUSED, void *comp1 ATTRIBUTE_UNUSED,
    const xmlChar *value2, xmlNodePtr ctxt2 ATTRIBUTE_UNUSED) {
  int ret = -1;
 ...
 */
static int xmlRelaxNGRegisterTypeLibrary(const xmlChar *namespace, void *data,
...
int xmlRelaxNGInitTypes(void) {
  ...
  xmlRelaxNGRegisterTypeLibrary(
      BAD_CAST "http://www.w3.org/2001/XMLSchema-datatypes", NULL,
      xmlRelaxNGSchemaTypeHave, xmlRelaxNGSchemaTypeCheck,
      xmlRelaxNGSchemaTypeCompare, xmlRelaxNGSchemaFacetCheck,
      xmlRelaxNGSchemaFreeValue);
  xmlRelaxNGRegisterTypeLibrary(xmlRelaxNGNs, NULL, xmlRelaxNGDefaultTypeHave,
                                xmlRelaxNGDefaultTypeCheck,
                                xmlRelaxNGDefaultTypeCompare, NULL, NULL);
...
void xmlRelaxNGCleanupTypes(void) {
  ...
  xmlHashFree(xmlRelaxNGRegisteredTypes,
              (xmlHashDeallocator)xmlRelaxNGFreeTypeLibrary);
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error
 */
static int xmlRelaxNGIsCompileable(xmlRelaxNGDefinePtr def) {
  int ret = -1;
...
#ifdef DEBUG_COMPILE
  if (ret == 1) {
    xmlGenericError(xmlGenericErrorContext, "RelaxNGIsCompileable %s : true\n",
                    xmlRelaxNGDefName(def));
  } else if (ret == 0) {
    xmlGenericError(xmlGenericErrorContext, "RelaxNGIsCompileable %s : false\n",
                    xmlRelaxNGDefName(def));
  } else {
    xmlGenericError(xmlGenericErrorContext,
 ...
 * Returns 0 if success and -1 in case of error
 */
static int xmlRelaxNGCompile(xmlRelaxNGParserCtxtPtr ctxt,
                             xmlRelaxNGDefinePtr def) {
  int ret = 0;
  xmlRelaxNGDefinePtr list;
    ...
    if ((def->dflags & IS_COMPILABLE) && (def->depth != -25)) {
      ...
      if (ret == 0) {
        if (def->type == XML_RELAXNG_START)
          xmlGenericError(xmlGenericErrorContext, "compiled the start\n");
        else
          xmlGenericError(xmlGenericErrorContext, "compiled element %s\n",
                          def->name);
      } else {
        if (def->type == XML_RELAXNG_START)
          xmlGenericError(xmlGenericErrorContext,
...
static xmlRelaxNGDefinePtr xmlRelaxNGParseElement(xmlRelaxNGParserCtxtPtr ctxt,
                                                  xmlNodePtr node);
static xmlRelaxNGDefinePtr xmlRelaxNGParsePatterns(xmlRelaxNGParserCtxtPtr ctxt,
                                                   xmlNodePtr nodes, int group);
static xmlRelaxNGDefinePtr xmlRelaxNGParsePattern(xmlRelaxNGParserCtxtPtr ctxt,
                                                  xmlNodePtr node);
...
xmlRelaxNGParseNameClass(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node,
                         xmlRelaxNGDefinePtr def);
static xmlRelaxNGGrammarPtr xmlRelaxNGParseGrammar(xmlRelaxNGParserCtxtPtr ctxt,
                                                   xmlNodePtr nodes);
 ...
 */
static xmlRelaxNGDefinePtr xmlRelaxNGParseValue(xmlRelaxNGParserCtxtPtr ctxt,
                                                xmlNodePtr node) {
  ...
  if (type != NULL) {
    ...
    lib = (xmlRelaxNGTypeLibraryPtr)xmlHashLookup(xmlRelaxNGRegisteredTypes,
                                                  library);
    if (lib == NULL) {
 ...
 * Returns the definition pointer or NULL in case of error
 */
static xmlRelaxNGDefinePtr xmlRelaxNGParseData(xmlRelaxNGParserCtxtPtr ctxt,
                                               xmlNodePtr node) {
 ...
 */
static int xmlRelaxNGCompareNameClasses(xmlRelaxNGDefinePtr def1,
...
static int xmlRelaxNGGenerateAttributes(xmlRelaxNGParserCtxtPtr ctxt,
  ...
  while (cur != NULL) {
    if ((cur->type == XML_RELAXNG_ELEMENT) || (cur->type == XML_RELAXNG_TEXT) ||
        ...
        (cur->type == XML_RELAXNG_PARAM) || (cur->type == XML_RELAXNG_LIST) ||
        (cur->type == XML_RELAXNG_VALUE) || (cur->type == XML_RELAXNG_EMPTY))
      return (0);
...
static xmlRelaxNGDefinePtr *xmlRelaxNGGetElements(xmlRelaxNGParserCtxtPtr ctxt,
  ...
  while (cur != NULL) {
    if (((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) ||
      ...
      if (ret == NULL) {
        max = 10;
        ret = (xmlRelaxNGDefinePtr *)xmlMalloc((max + 1) *
 ...
 */
static void xmlRelaxNGCheckChoiceDeterminism(xmlRelaxNGParserCtxtPtr ctxt,
                                             xmlRelaxNGDefinePtr def) {
  xmlRelaxNGDefinePtr **list;
      ...
      while ((*tmp != NULL) && (is_triable == 1)) {
        if ((*tmp)->type == XML_RELAXNG_TEXT) {
          ...
          if (((*tmp)->ns == NULL) || ((*tmp)->ns[0] == 0))
            ...
            res =
                xmlHashAddEntry2(triage, (*tmp)->name, (*tmp)->ns, (void *)cur);
          if (res != 0)
            is_triable = -1;
        } else if ((*tmp)->type == XML_RELAXNG_ELEMENT) {
          if (((*tmp)->ns == NULL) || ((*tmp)->ns[0] == 0))
            ...
            res = xmlHashAddEntry2(triage, BAD_CAST "#any", (*tmp)->ns,
                                   (void *)cur);
          if (res != 0)
            is_triable = -1;
        } else {
 ...
 */
static void xmlRelaxNGCheckGroupAttrs(xmlRelaxNGParserCtxtPtr ctxt,
                                      xmlRelaxNGDefinePtr def) {
  xmlRelaxNGDefinePtr **list;
  xmlRelaxNGDefinePtr cur;
  int nbchild = 0, i, j, ret;
 ...
 *     algorithm
 */
static void xmlRelaxNGComputeInterleaves(xmlRelaxNGDefinePtr def,
  ...
  xmlGenericError(xmlGenericErrorContext, "xmlRelaxNGComputeInterleaves(%s)\n",
                  name);
#endif
  cur = def->content;
  while (cur != NULL) {
...
#endif
  groups = (xmlRelaxNGInterleaveGroupPtr *)xmlMalloc(
      nbchild * sizeof(xmlRelaxNGInterleaveGroupPtr));
  if (groups == NULL)
    goto error;
  cur = def->content;
  while (cur != NULL) {
    groups[nbgroups] = (xmlRelaxNGInterleaveGroupPtr)xmlMalloc(
  ...
  for (i = 0; i < nbgroups; i++) {
    ...
    if ((tmp != NULL) && (*tmp != NULL)) {
      while (*tmp != NULL) {
        if ((*tmp)->type == XML_RELAXNG_TEXT) {
          res = xmlHashAddEntry2(partitions->triage, BAD_CAST "#text", NULL,
                                 (void *)(long)(i + 1));
          if (res != 0)
            is_determinist = -1;
        } else if (((*tmp)->type == XML_RELAXNG_ELEMENT) &&
                   ((*tmp)->name != NULL)) {
          if (((*tmp)->ns == NULL) || ((*tmp)->ns[0] == 0))
            res = xmlHashAddEntry2(partitions->triage, (*tmp)->name, NULL,
                                   (void *)(long)(i + 1));
          else
            res = xmlHashAddEntry2(partitions->triage, (*tmp)->name, (*tmp)->ns,
                                   (void *)(long)(i + 1));
          if (res != 0)
            is_determinist = -1;
        } else if ((*tmp)->type == XML_RELAXNG_ELEMENT) {
          if (((*tmp)->ns == NULL) || ((*tmp)->ns[0] == 0))
            res = xmlHashAddEntry2(partitions->triage, BAD_CAST "#any", NULL,
                                   (void *)(long)(i + 1));
          else
            res = xmlHashAddEntry2(partitions->triage, BAD_CAST "#any",
                                   (*tmp)->ns, (void *)(long)(i + 1));
 ...
 */
static int xmlRelaxNGParseInclude(xmlRelaxNGParserCtxtPtr ctxt,
                                  xmlNodePtr node) {
 ...
 * Returns 0 in case of success or -1 in case of error
 */
static int xmlRelaxNGParseDefine(xmlRelaxNGParserCtxtPtr ctxt,
                                 xmlNodePtr node) {
  xmlChar *name;
  int ret = 0, tmp;
  xmlRelaxNGDefinePtr def;
 ...
 */
static int xmlRelaxNGParseImportRefs(xmlRelaxNGParserCtxtPtr ctxt,
  ...
  if (IS_RELAXNG(node, "element")) {
    ...
    if (ctxt->grammar->refs == NULL) {
      ...
      if (tmp < 0) {
        ...
        prev =
            (xmlRelaxNGDefinePtr)xmlHashLookup(ctxt->grammar->refs, def->name);
        if (prev == NULL) {
          if (def->name != NULL) {
            xmlRngPErr(ctxt, node, XML_RNGP_REF_CREATE_FAILED,
                       "Error refs definitions '%s'\n", def->name, NULL);
    ...
    if (ctxt->parentgrammar->refs == NULL) {
      ...
      if (tmp < 0) {
        ...
        prev = (xmlRelaxNGDefinePtr)xmlHashLookup(ctxt->parentgrammar->refs,
                                                  def->name);
        if (prev == NULL) {
          xmlRngPErr(ctxt, node, XML_RNGP_PARENTREF_CREATE_FAILED,
  ...
  if (IS_RELAXNG(node, "name")) {
    ...
    if (node->children != NULL) {
      ret->nameClass = xmlRelaxNGParseExceptNameClass(
          ctxt, node->children, (def->type == XML_RELAXNG_ATTRIBUTE));
    }
 ...
 */
static int xmlRelaxNGParseStart(xmlRelaxNGParserCtxtPtr ctxt,
                                xmlNodePtr nodes) {
  int ret = 0;
 ...
 */
static void xmlRelaxNGCheckReference(xmlRelaxNGDefinePtr ref,
                                     xmlRelaxNGParserCtxtPtr ctxt,
                                     const xmlChar *name) {
  xmlRelaxNGGrammarPtr grammar;
 ...
 * element of a given grammar using the same name.
 */
static void xmlRelaxNGCheckCombine(xmlRelaxNGDefinePtr define,
                                   xmlRelaxNGParserCtxtPtr ctxt,
                                   const xmlChar *name) {
  xmlChar *combine;
 ...
 */
static void xmlRelaxNGCombineStart(xmlRelaxNGParserCtxtPtr ctxt,
                                   xmlRelaxNGGrammarPtr grammar) {
 ...
 */
static int xmlRelaxNGCheckCycles(xmlRelaxNGParserCtxtPtr ctxt,
 ...
 */
static void xmlRelaxNGSimplify(xmlRelaxNGParserCtxtPtr ctxt,
                               xmlRelaxNGDefinePtr cur,
  ...
  while (cur != NULL) {
    if ((cur->type == XML_RELAXNG_REF) ||
      ...
      cur->parent = parent;
      if ((parent != NULL) && ((parent->type == XML_RELAXNG_ATTRIBUTE) ||
      ...
      cur->parent = parent;
      if ((parent != NULL) && ((parent->type == XML_RELAXNG_ONEORMORE) ||
                               (parent->type == XML_RELAXNG_ZEROORMORE))) {
        parent->type = XML_RELAXNG_EMPTY;
        break;
      }
      if ((parent != NULL) && ((parent->type == XML_RELAXNG_GROUP) ||
                               (parent->type == XML_RELAXNG_INTERLEAVE))) {
        prev = xmlRelaxNGTryUnlink(ctxt, cur, parent, prev);
      } else
        prev = cur;
 ...
 */
static void xmlRelaxNGCleanupAttributes(xmlRelaxNGParserCtxtPtr ctxt,
                                        xmlNodePtr node) {
  xmlAttrPtr cur, next;
 ...
 */
static void xmlRelaxNGCleanupTree(xmlRelaxNGParserCtxtPtr ctxt,
                                  xmlNodePtr root) {
        ...
        if (xmlStrEqual(cur->name, BAD_CAST "externalRef")) {
          ...
          if (xmlStrEqual(cur->name, BAD_CAST "name")) {
            ...
            if (name != NULL) {
              ...
              if (local != NULL) {
                ...
                if (ns == NULL) {
                  xmlRngPErr(ctxt, cur, XML_RNGP_PREFIX_UNDEFINED,
          ...
          if (xmlStrEqual(cur->name, BAD_CAST "nsName")) {
            if (ctxt->flags & XML_RELAXNG_IN_NSEXCEPT) {
              xmlRngPErr(ctxt, cur, XML_RNGP_PAT_NSNAME_EXCEPT_NSNAME,
          ...
          if (ctxt->flags & XML_RELAXNG_IN_ANYEXCEPT) {
            xmlRngPErr(ctxt, cur, XML_RNGP_PAT_ANYNAME_EXCEPT_ANYNAME,
                       "Found anyName/except//anyName forbidden construct\n",
                       NULL, NULL);
          } else if (ctxt->flags & XML_RELAXNG_IN_NSEXCEPT) {
            xmlRngPErr(ctxt, cur, XML_RNGP_PAT_NSNAME_EXCEPT_ANYNAME,
...
static xmlDocPtr xmlRelaxNGCleanupDoc(xmlRelaxNGParserCtxtPtr ctxt,
  ...
  root = xmlDocGetRootElement(doc);
  if (root == NULL) {
    xmlRngPErr(ctxt, (xmlNodePtr)doc, XML_RNGP_EMPTY,
               "xmlRelaxNGParse: %s is empty\n", ctxt->URL, NULL);
    return (NULL);
  }
...
xmlRelaxNGPtr xmlRelaxNGParse(xmlRelaxNGParserCtxtPtr ctxt) {
   ...
   */
  if (ctxt->interleaves != NULL) {
    xmlHashScan(ctxt->interleaves, (xmlHashScanner)xmlRelaxNGComputeInterleaves,
                ctxt);
  }
 ...
 * Dump a RelaxNG structure back
 */
static void xmlRelaxNGDumpGrammar(FILE *output, xmlRelaxNGGrammarPtr grammar,
                                  int top) {
  if (grammar == NULL)
    return;
...
#ifdef DEBUG_COMPILE
  xmlGenericError(xmlGenericErrorContext, "Compiled callback for: '%s'\n",
                  token);
#endif
  if (ctxt == NULL) {
 ...
 * Returns 0 in case of success, -1 in case of error.
 */
static int xmlRelaxNGValidateCompiledContent(xmlRelaxNGValidCtxtPtr ctxt,
                                             xmlRegexpPtr regexp,
                                             xmlNodePtr content) {
 ...
 * Returns 0 in case of success and -1 in case of error.
 */
static int xmlRelaxNGElemPush(xmlRelaxNGValidCtxtPtr ctxt,
                              xmlRegExecCtxtPtr exec) {
  ...
  if (ctxt->elemNr >= ctxt->elemMax) {
    ctxt->elemMax *= 2;
    ctxt->elemTab = (xmlRegExecCtxtPtr *)xmlRealloc(
        ctxt->elemTab, ctxt->elemMax * sizeof(xmlRegExecCtxtPtr));
    if (ctxt->elemTab == NULL) {
      xmlRngVErrMemory(ctxt, "validating\n");
      return (-1);
    }
  ...
  while ((node != NULL) &&
         ((node->type == XML_COMMENT_NODE) || (node->type == XML_PI_NODE) ||
          ...
          (((node->type == XML_TEXT_NODE) ||
           ...
           ((ctxt->flags & FLAGS_MIXED_CONTENT) || (IS_BLANK_NODE(node)))))) {
    node = node->next;
  }
  return (node);
}
 ...
 */
static int xmlRelaxNGValidateDatatype(xmlRelaxNGValidCtxtPtr ctxt,
                                      const xmlChar *value,
                                      xmlRelaxNGDefinePtr define,
                                      xmlNodePtr node) {
  int ret, tmp;
  ...
  while ((ret == 0) && (cur != NULL) && (cur->type == XML_RELAXNG_PARAM)) {
    if (lib->facet != NULL) {
      tmp = lib->facet(lib->data, define->name, cur->name, cur->value, value,
                       result);
      if (tmp != 0)
        ret = -1;
    }
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateValueList(xmlRelaxNGValidCtxtPtr ctxt,
                                       xmlRelaxNGDefinePtr defines) {
  int ret = 0;
    ...
    if (!xmlStrEqual(value, define->value)) {
      if (define->name != NULL) {
        ...
        if ((lib != NULL) && (lib->comp != NULL)) {
          ret = lib->comp(lib->data, define->name, define->value, define->node,
                          (void *)define->attrs, value, ctxt->state->node);
        } else
          ret = -1;
        if (ret < 0) {
...
#ifdef DEBUG_LIST
    xmlGenericError(xmlGenericErrorContext, "list value: '%s' found %d items\n",
                    oldvalue, nb_values);
    nb_values = 0;
#endif
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateValueContent(xmlRelaxNGValidCtxtPtr ctxt,
                                          xmlRelaxNGDefinePtr defines) {
  int ret = 0;
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateAttribute(xmlRelaxNGValidCtxtPtr ctxt,
                                       xmlRelaxNGDefinePtr define) {
  int ret = 0, i;
 ...
 */
static int xmlRelaxNGNodeMatchesList(xmlNodePtr node,
                                     xmlRelaxNGDefinePtr *list) {
  xmlRelaxNGDefinePtr cur;
  int i = 0, tmp;
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateInterleave(xmlRelaxNGValidCtxtPtr ctxt,
                                        xmlRelaxNGDefinePtr define) {
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateDefinitionList(xmlRelaxNGValidCtxtPtr ctxt,
                                            xmlRelaxNGDefinePtr defines) {
  int ret = 0, res;
...
static int xmlRelaxNGValidateElementEnd(xmlRelaxNGValidCtxtPtr ctxt,
  ...
  if (state->seq != NULL) {
    ...
    if (state->seq != NULL) {
      if (dolog) {
        VALID_ERR3(XML_RELAXNG_ERR_EXTRACONTENT, state->node->name,
                   state->seq->name);
      }
      return (-1);
    }
  ...
  for (i = 0; i < state->nbAttrs; i++) {
    if (state->attrs[i] != NULL) {
      if (dolog) {
        VALID_ERR3(XML_RELAXNG_ERR_INVALIDATTR, state->attrs[i]->name,
                   state->node->name);
      }
      return (-1 - i);
    }
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateState(xmlRelaxNGValidCtxtPtr ctxt,
                                   xmlRelaxNGDefinePtr define) {
  xmlNodePtr node;
  ...
  for (i = 0; i < ctxt->depth; i++)
    xmlGenericError(xmlGenericErrorContext, " ");
  xmlGenericError(xmlGenericErrorContext, "Start validating %s ",
  ...
  case XML_RELAXNG_ELEMENT:
    ...
    if (define->contModel != NULL) {
      ...
      tmp = xmlRelaxNGValidateCompiledContent(ctxt, define->contModel,
                                              ctxt->state->seq);
      nseq = ctxt->state->seq;
      ctxt->state = tmpstate;
  ...
  case XML_RELAXNG_ZEROORMORE: {
    ...
    if (ctxt->state != NULL) {
      ...
      for (j = 0; j < ctxt->states->nbState; j++) {
        xmlRelaxNGAddStates(
            ctxt, res,
            xmlRelaxNGCopyValidState(ctxt, ctxt->states->tabState[j]));
      }
    }
    oldflags = ctxt->flags;
    ctxt->flags |= FLAGS_IGNORABLE;
      ...
      if (progress) {
        ...
        if (res->nbState - base == 1) {
          ...
          for (i = base; i < res->nbState; i++)
            xmlRelaxNGAddStates(
                ctxt, states, xmlRelaxNGCopyValidState(ctxt, res->tabState[i]));
          ctxt->states = states;
        }
      }
    } while (progress == 1);
  ...
  for (i = 0; i < ctxt->depth; i++)
    xmlGenericError(xmlGenericErrorContext, " ");
  xmlGenericError(xmlGenericErrorContext, "Validating %s ",
                  xmlRelaxNGDefName(define));
  if (define->name != NULL)
    xmlGenericError(xmlGenericErrorContext, "%s ", define->name);
  if (ret == 0)
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateDocument(xmlRelaxNGValidCtxtPtr ctxt,
                                      xmlDocPtr doc) {
  int ret;
  xmlRelaxNGPtr schema;
  xmlRelaxNGGrammarPtr grammar;
...
static void xmlRelaxNGCleanPSVI(xmlNodePtr node) {
  ...
  if ((node == NULL) ||
      ((node->type != XML_ELEMENT_NODE) && (node->type != XML_DOCUMENT_NODE) &&
