...
static xmlNsPtr xmlNewReconciliedNs(xmlDocPtr doc, xmlNodePtr tree,
                                    xmlNsPtr ns);
...
const xmlChar xmlStringTextNoenc[] = {'t', 'e', 'x', 't', 'n',
                                      'o', 'e', 'n', 'c', 0};
...
#define IS_STR_XML(str)                                                        \
  ((str != NULL) && (str[0] == 'x') && (str[1] == 'm') && (str[2] == 'l') &&   \
   (str[3] == 0))
 ...
 */
static xmlEntityPtr xmlGetEntityFromDtd(xmlDtdPtr dtd, const xmlChar *name) {
 ...
 * Returns A pointer to the entity structure or NULL if not found.
 */
static xmlEntityPtr xmlGetParameterEntityFromDtd(xmlDtdPtr dtd,
                                                 const xmlChar *name) {
  xmlEntitiesTablePtr table;
...
try_complex:
  ...
  while (IS_LETTER(c) || IS_DIGIT(c) || (c == '.') || (c == '-') ||
         (c == '_') || IS_COMBINING(c) || IS_EXTENDER(c)) {
    cur += l;
    c = CUR_SCHAR(cur, l);
  }
  if (space) {
...
try_complex:
  ...
  if (!(IS_LETTER(c) || IS_DIGIT(c) || (c == '.') || (c == ':') || (c == '-') ||
        (c == '_') || IS_COMBINING(c) || IS_EXTENDER(c)))
    return (1);
  cur += l;
  c = CUR_SCHAR(cur, l);
...
void xmlFreeDtd(xmlDtdPtr cur) {
  ...
  if (cur->children != NULL) {
    ...
    while (c != NULL) {
      next = c->next;
      if ((c->type != XML_NOTATION_NODE) && (c->type != XML_ELEMENT_DECL) &&
          (c->type != XML_ATTRIBUTE_DECL) && (c->type != XML_ENTITY_DECL)) {
        xmlUnlinkNode(c);
        xmlFreeNode(c);
      }
      c = next;
 ...
 */
xmlNodePtr xmlStringLenGetNodeList(xmlDocPtr doc, const xmlChar *value,
                                   int len) {
...
static xmlAttrPtr xmlNewPropInternal(xmlNodePtr node, xmlNsPtr ns,
  ...
  if ((node != NULL) && (node->type != XML_ELEMENT_NODE)) {
    if ((eatname == 1) &&
        ((node->doc == NULL) || (!(xmlDictOwns(node->doc->dict, name)))))
  ...
  if (cur == NULL) {
    if ((eatname == 1) && ((node == NULL) || (node->doc == NULL) ||
  ...
  if (value != NULL) {
    ...
    if (!xmlCheckUTF8(value)) {
      xmlTreeErr(XML_TREE_NOT_UTF8, (xmlNodePtr)doc, NULL);
      if (doc != NULL)
  ...
  if ((value != NULL) && (node != NULL) && (xmlIsID(node->doc, node, cur) == 1))
 ...
 */
xmlAttrPtr xmlNewProp(xmlNodePtr node, const xmlChar *name,
                      const xmlChar *value) {
 ...
 * Returns a pointer to the attribute
 */
xmlAttrPtr xmlNewDocProp(xmlDocPtr doc, const xmlChar *name,
                         const xmlChar *value) {
  xmlAttrPtr cur;
 ...
 */
xmlNodePtr xmlNewDocPI(xmlDocPtr doc, const xmlChar *name,
...
xmlNodePtr xmlNewDocNode(xmlDocPtr doc, xmlNsPtr ns, const xmlChar *name,
  ...
  if ((doc != NULL) && (doc->dict != NULL))
    cur = xmlNewNodeEatName(ns, (xmlChar *)xmlDictLookup(doc->dict, name, -1));
  else
 ...
 * Returns a pointer to the new node object.
 */
xmlNodePtr xmlNewDocNodeEatName(xmlDocPtr doc, xmlNsPtr ns, xmlChar *name,
                                const xmlChar *content) {
  xmlNodePtr cur;
  ...
  if (cur != NULL) {
    ...
    if ((name != NULL) && (doc != NULL) && (!(xmlDictOwns(doc->dict, name))))
      xmlFree(name);
  }
  return (cur);
}
 ...
 * Returns a pointer to the new node object.
 */
xmlNodePtr xmlNewDocRawNode(xmlDocPtr doc, xmlNsPtr ns, const xmlChar *name,
                            const xmlChar *content) {
  xmlNodePtr cur;
 ...
 * Returns a pointer to the new node object.
 */
xmlNodePtr xmlNewTextChild(xmlNodePtr parent, xmlNsPtr ns, const xmlChar *name,
                           const xmlChar *content) {
  xmlNodePtr cur, prev;
 ...
 */
static xmlNodePtr xmlAddPropSibling(xmlNodePtr prev, xmlNodePtr cur,
                                    xmlNodePtr prop) {
  xmlAttrPtr attr;
 ...
 * If the new node is ATTRIBUTE, it is added into properties instead of
 * children. If there is an attribute with equal name, it is first destroyed.
 *
...
xmlNodePtr xmlAddSibling(xmlNodePtr cur, xmlNodePtr elem) {
  ...
  if (cur == elem) {
#ifdef DEBUG_TREE
    xmlGenericError(xmlGenericErrorContext, "xmlAddSibling : cur == elem\n");
#endif
    return (NULL);
  }
  ...
  if ((cur->type != XML_ATTRIBUTE_NODE) && (cur->parent != NULL) &&
      (cur->parent->children != NULL) && (cur->parent->last != NULL) &&
...
xmlNodePtr xmlAddChild(xmlNodePtr parent, xmlNodePtr cur) {
  ...
  if (parent == cur) {
#ifdef DEBUG_TREE
    xmlGenericError(xmlGenericErrorContext, "xmlAddChild : parent == cur\n");
#endif
    return (NULL);
  }
  /*
   ...
   */
  if (cur->type == XML_TEXT_NODE) {
    if ((parent->type == XML_TEXT_NODE) && (parent->content != NULL) &&
  ...
  if (cur->type == XML_ATTRIBUTE_NODE) {
    ...
    if (parent->properties != NULL) {
      ...
      if ((lastattr != NULL) && (lastattr != (xmlAttrPtr)cur) &&
          (lastattr->type != XML_ATTRIBUTE_DECL)) {
        /* different instance, destroy it (attributes must be unique) */
...
void xmlFreeNodeList(xmlNodePtr cur) {
  ...
  while (cur != NULL) {
    ...
    if (cur->type != XML_DTD_NODE) {
       ...
       * dictionnary
       */
      if ((cur->name != NULL) && (cur->type != XML_TEXT_NODE) &&
          (cur->type != XML_COMMENT_NODE))
        DICT_FREE(cur->name)
      xmlFree(cur);
    }
...
void xmlFreeNode(xmlNodePtr cur) {
  ...
  if ((cur->type != XML_ELEMENT_NODE) && (cur->content != NULL) &&
      (cur->type != XML_ENTITY_REF_NODE) && (cur->type != XML_XINCLUDE_END) &&
      (cur->type != XML_XINCLUDE_START) &&
      (cur->content != (xmlChar *)&(cur->properties))) {
    DICT_FREE(cur->content)
  }
    ...
    if (doc != NULL) {
      if (doc->intSubset != NULL) {
        if (xmlHashLookup(doc->intSubset->entities, cur->name) == cur)
          xmlHashRemoveEntry(doc->intSubset->entities, cur->name, NULL);
        if (xmlHashLookup(doc->intSubset->pentities, cur->name) == cur)
          xmlHashRemoveEntry(doc->intSubset->pentities, cur->name, NULL);
      }
      if (doc->extSubset != NULL) {
        if (xmlHashLookup(doc->extSubset->entities, cur->name) == cur)
          xmlHashRemoveEntry(doc->extSubset->entities, cur->name, NULL);
        if (xmlHashLookup(doc->extSubset->pentities, cur->name) == cur)
          xmlHashRemoveEntry(doc->extSubset->pentities, cur->name, NULL);
      }
    }
  }
  if (cur->parent != NULL) {
...
#ifdef DEBUG_TREE
    xmlGenericError(xmlGenericErrorContext,
                    "xmlReplaceNode : Trying to replace attribute node with "
                    "other node type\n");
#endif
    return (old);
  }
...
#ifdef DEBUG_TREE
    xmlGenericError(xmlGenericErrorContext,
                    "xmlReplaceNode : Trying to replace a non-attribute node "
                    "with attribute node\n");
#endif
    return (old);
  }
...
static xmlNodePtr xmlStaticCopyNodeList(xmlNodePtr node, xmlDocPtr doc,
                                        xmlNodePtr parent);

static xmlAttrPtr xmlCopyPropInternal(xmlDocPtr doc, xmlNodePtr target,
                                      xmlAttrPtr cur) {
  xmlAttrPtr ret;
  ...
  if (cur->children != NULL) {
    ...
    ret->children =
        xmlStaticCopyNodeList(cur->children, ret->doc, (xmlNodePtr)ret);
    ret->last = NULL;
    tmp = ret->children;
    while (tmp != NULL) {
   ...
   * Try to handle IDs
   */
  if ((target != NULL) && (cur != NULL) && (target->doc != NULL) &&
      (cur->doc != NULL) && (cur->doc->ids != NULL) && (cur->parent != NULL)) {
...
static xmlNodePtr xmlStaticCopyNode(const xmlNodePtr node, xmlDocPtr doc,
                                    xmlNodePtr parent, int extended) {
  xmlNodePtr ret;
...
xmlDtdPtr xmlCopyDtd(xmlDtdPtr dtd) {
  ...
  if (dtd->entities != NULL)
    ret->entities =
        (void *)xmlCopyEntitiesTable((xmlEntitiesTablePtr)dtd->entities);
  if (dtd->notations != NULL)
    ret->notations =
        (void *)xmlCopyNotationTable((xmlNotationTablePtr)dtd->notations);
  if (dtd->elements != NULL)
    ret->elements =
        (void *)xmlCopyElementTable((xmlElementTablePtr)dtd->elements);
  if (dtd->attributes != NULL)
    ret->attributes =
        (void *)xmlCopyAttributeTable((xmlAttributeTablePtr)dtd->attributes);
  if (dtd->pentities != NULL)
    ret->pentities =
        (void *)xmlCopyEntitiesTable((xmlEntitiesTablePtr)dtd->pentities);
  ...
  while (cur != NULL) {
    ...
    if (cur->type == XML_ENTITY_DECL) {
      ...
      q = (xmlNodePtr)xmlGetDtdQAttrDesc(ret, tmp->elem, tmp->name,
                                         tmp->prefix);
...
long xmlGetLineNo(xmlNodePtr node) {
  ...
  else if ((node->prev != NULL) && ((node->prev->type == XML_ELEMENT_NODE) ||
...
xmlChar *xmlGetNodePath(xmlNodePtr node) {
  ...
  do {
    ...
    if ((cur->type == XML_DOCUMENT_NODE) ||
      ...
      while (tmp != NULL) {
        if ((tmp->type == XML_ELEMENT_NODE) &&
            (generic || (xmlStrEqual(cur->name, tmp->name) &&
                         ((tmp->ns == cur->ns) ||
      ...
      sep = "/";
      snprintf(nametemp, sizeof(nametemp) - 1, "processing-instruction('%s')",
               (char *)cur->name);
      nametemp[sizeof(nametemp) - 1] = 0;
      name = nametemp;
    ...
    if (xmlStrlen(buffer) + sizeof(nametemp) + 20 > buf_len) {
      buf_len = 2 * buf_len + xmlStrlen(buffer) + sizeof(nametemp) + 20;
    ...
    if (occur == 0)
      ...
      snprintf((char *)buf, buf_len, "%s%s[%d]%s", sep, name, occur,
               (char *)buffer);
 ...
 */
xmlNsPtr *xmlGetNsList(xmlDocPtr doc ATTRIBUTE_UNUSED, xmlNodePtr node) {
  xmlNsPtr cur;
  xmlNsPtr *ret = NULL;
  int nbns = 0;
  int maxns = 10;
...
xmlNsPtr xmlSearchNs(xmlDocPtr doc, xmlNodePtr node, const xmlChar *nameSpace) {
  ...
  while (node != NULL) {
    if ((node->type == XML_ENTITY_REF_NODE) ||
        (node->type == XML_ENTITY_NODE) || (node->type == XML_ENTITY_DECL))
      return (NULL);
    if (node->type == XML_ELEMENT_NODE) {
      cur = node->nsDef;
      while (cur != NULL) {
        ...
        if ((cur->prefix != NULL) && (nameSpace != NULL) &&
            (cur->href != NULL) && (xmlStrEqual(cur->prefix, nameSpace)))
          return (cur);
        cur = cur->next;
      }
      if (orig != node) {
...
static xmlNsPtr xmlNewReconciliedNs(xmlDocPtr doc, xmlNodePtr tree,
  ...
  while (def != NULL) {
    ...
    if (ns->prefix == NULL)
      ...
      snprintf((char *)prefix, sizeof(prefix), "%.20s%d", (char *)ns->prefix,
               counter++);
    def = xmlSearchNs(doc, tree, prefix);
  }
      ...
      if (i == nbCache) {
        ...
        if (n != NULL) { /* :-( what if else ??? */
          ...
          if (sizeCache <= nbCache) {
            sizeCache *= 2;
            oldNs = (xmlNsPtr *)xmlRealloc(oldNs, sizeCache * sizeof(xmlNsPtr));
            if (oldNs == NULL) {
              xmlTreeErrMemory("fixing namespaces");
              xmlFree(newNs);
              return (-1);
            }
            newNs = (xmlNsPtr *)xmlRealloc(newNs, sizeCache * sizeof(xmlNsPtr));
            if (newNs == NULL) {
              xmlTreeErrMemory("fixing namespaces");
              xmlFree(oldNs);
              return (-1);
          ...
          if (i == nbCache) {
            ...
            if (n != NULL) { /* :-( what if else ??? */
              ...
              if (sizeCache <= nbCache) {
                sizeCache *= 2;
                oldNs =
                    (xmlNsPtr *)xmlRealloc(oldNs, sizeCache * sizeof(xmlNsPtr));
                if (oldNs == NULL) {
                  xmlTreeErrMemory("fixing namespaces");
                  xmlFree(newNs);
                  return (-1);
                ...
                newNs =
                    (xmlNsPtr *)xmlRealloc(newNs, sizeCache * sizeof(xmlNsPtr));
                if (newNs == NULL) {
                  xmlTreeErrMemory("fixing namespaces");
                  xmlFree(oldNs);
                  return (-1);
...
static xmlAttrPtr xmlGetPropNodeInternal(xmlNodePtr node, const xmlChar *name,
    ...
    if (nsName == NULL) {
      ...
      while (*cur != NULL) {
        if (xmlStrEqual((*cur)->href, nsName)) {
          attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elemQName, name,
                                        (*cur)->prefix);
          if (attrDecl)
            break;
          if (doc->extSubset != NULL) {
            attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elemQName, name,
                                          (*cur)->prefix);
            if (attrDecl)
              break;
          }
 ...
 */
xmlAttrPtr xmlHasNsProp(xmlNodePtr node, const xmlChar *name,
 ...
 *     It's up to the caller to free the memory with xmlFree().
 */
xmlChar *xmlGetNsProp(xmlNodePtr node, const xmlChar *name,
                      const xmlChar *nameSpace) {
  xmlAttrPtr prop;
...
xmlAttrPtr xmlSetNsProp(xmlNodePtr node, xmlNsPtr ns, const xmlChar *name,
  ...
  if (prop != NULL) {
    ...
    if (value != NULL) {
      ...
      if (!xmlCheckUTF8(value)) {
        xmlTreeErr(XML_TREE_NOT_UTF8, (xmlNodePtr)node->doc, NULL);
        if (node->doc != NULL)
...
int xmlBufferDump(FILE *file, xmlBufferPtr buf) {
  ...
  if (buf == NULL) {
#ifdef DEBUG_BUFFER
    xmlGenericError(xmlGenericErrorContext, "xmlBufferDump: buf == NULL\n");
#endif
    return (0);
  }
  if (buf->content == NULL) {
 ...
 * Returns 0 (uncompressed) to 9 (max compression)
 */
int xmlGetCompressMode(void) { return (xmlCompressMode); }
 ...
 */
static xmlNsMapItemPtr xmlDOMWrapNsMapAddItem(xmlNsMapPtr *nsmap, int position,
 ...
 *         or internal error.
 */
static xmlNsPtr xmlDOMWrapStoreNs(xmlDocPtr doc, const xmlChar *nsName,
 ...
 *         API errors.
 */
static xmlNsPtr xmlTreeNSListLookupByPrefix(xmlNsPtr nsList,
                                            const xmlChar *prefix) {
  if (nsList == NULL)
    return (NULL);
  {
...
#define XML_TREE_ADOPT_STR_2(str)                                              \
  if (adoptStr && (str != NULL) && (sourceDoc != NULL) &&                      \
    ...
    if (destDoc->dict)                                                         \
      cur->content =                                                           \
          (xmlChar *)xmlDictLookup(destDoc->dict, cur->content, -1);           \
    else                                                                       \
 ...
 */
static int xmlDOMWrapNSNormAddNsMapItem2(xmlNsPtr **list, int *size,
                                         int *number, xmlNsPtr oldNs,
                                         xmlNsPtr newNs) {
  if (*list == NULL) {
    ...
    *size *= 2;
    *list = (xmlNsPtr *)xmlRealloc(*list, (*size) * 2 * sizeof(xmlNsPtr));
    if (*list == NULL) {
 ...
 */
int xmlDOMWrapRemoveNode(xmlDOMWrapCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr node,
  ...
  do {
    ...
    case XML_ELEMENT_NODE:
      if ((ctxt == NULL) && (node->nsDef != NULL)) {
        ns = node->nsDef;
        do {
          if (xmlDOMWrapNSNormAddNsMapItem2(&list, &sizeList, &nbList, ns,
                                            ns) == -1)
            goto internal_error;
          ns = ns->next;
        } while (ns != NULL);
    ...
    case XML_ATTRIBUTE_NODE:
      if (node->ns != NULL) {
        ...
        if (ns != NULL) {
           ...
           * Add mapping.
           */
          if (xmlDOMWrapNSNormAddNsMapItem2(&list, &sizeList, &nbList, node->ns,
                                            ns) == -1)
            goto internal_error;
        }
        node->ns = ns;
...
static int xmlSearchNsByNamespaceStrict(xmlDocPtr doc, xmlNodePtr node,
                                        const xmlChar *nsName, xmlNsPtr *retNs,
                                        int prefixed) {
  ...
  *retNs = NULL;
  ...
  do {
          ...
          if (prev != NULL) {
            ...
            do {
              if ((prevns->prefix == ns->prefix) ||
                  ((prevns->prefix != NULL) && (ns->prefix != NULL) &&
    ...
    } else if ((cur->type == XML_ENTITY_NODE) || (cur->type == XML_ENTITY_DECL))
      return (0);
    cur = cur->parent;
...
static int xmlSearchNsByPrefixStrict(xmlDocPtr doc, xmlNodePtr node,
                                     const xmlChar *prefix, xmlNsPtr *retNs) {
  xmlNodePtr cur;
  xmlNsPtr ns;
 ...
 *         and internal errors.
 */
static xmlNsPtr xmlDOMWrapNSNormDeclareNsForced(xmlDocPtr doc, xmlNodePtr elem,
 ...
 */
static int xmlDOMWrapNSNormAquireNormalizedNs(xmlDocPtr doc, xmlNodePtr elem,
                                              xmlNsPtr ns, xmlNsPtr *retNs,
                                              xmlNsMapPtr *nsMap,

                                              int depth, int ancestorsOnly,
                                              int prefixed) {
  xmlNsMapItemPtr mi;
  ...
  *retNs = NULL;
  ...
  if (elem == NULL) {
     ...
     * Insert mapping.
     */
    if (xmlDOMWrapNsMapAddItem(nsMap, -1, ns, tmpns, XML_TREE_NSMAP_DOC) ==
        NULL) {
      xmlFreeNs(tmpns);
      return (-1);
    }
...
int xmlDOMWrapReconcileNamespaces(xmlDOMWrapCtxtPtr ctxt ATTRIBUTE_UNUSED,
  ...
  int optRemoveRedundantNS =
      ((xmlDOMReconcileNSOptions)options & XML_DOM_RECONNS_REMOVEREDUND) ? 1
                                                                         : 0;
  ...
  do {
          ...
          if (optRemoveRedundantNS && XML_NSMAP_NOTEMPTY(nsMap)) {
            XML_NSMAP_FOREACH(nsMap, mi) {
              if ((mi->depth >= XML_TREE_NSMAP_PARENT) &&
                 ...
                 */
                if (xmlDOMWrapNSNormAddNsMapItem2(&listRedund, &sizeRedund,
                                                  &nbRedund, ns,
                                                  mi->newNs) == -1)
                  goto internal_error;
                /*
    ...
    case XML_ATTRIBUTE_NODE:
      ...
      if (xmlDOMWrapNSNormAquireNormalizedNs(
              doc, curElem, cur->ns, &ns, &nsMap, depth, ancestorsOnly,
 ...
 * Returns 0 if succeeded, -1 otherwise and on API/internal errors.
 */
static int xmlDOMWrapAdoptBranch(xmlDOMWrapCtxtPtr ctxt, xmlDocPtr sourceDoc,
                                 xmlNodePtr node, xmlDocPtr destDoc,
   ...
   */
  if ((sourceDoc != NULL) && (sourceDoc->dict == destDoc->dict))
    adoptStr = 0;
  else
    adoptStr = 1;
  ...
  while (cur != NULL) {
    ...
    case XML_ATTRIBUTE_NODE:
      ...
      if ((ctxt) && (ctxt->getNsForNodeFunc)) {
         ...
         * if not.
         */
        if (xmlDOMWrapNsMapAddItem(&nsMap, -1, cur->ns, ns,
                                   XML_TREE_NSMAP_CUSTOM) == NULL)
          goto internal_error;
        cur->ns = ns;
      } else {
        /*
...
int xmlDOMWrapCloneNode(xmlDOMWrapCtxtPtr ctxt, xmlDocPtr sourceDoc,
                        xmlNodePtr node, xmlNodePtr *resNode, xmlDocPtr destDoc,
                        xmlNodePtr destParent, int deep,
                        int options ATTRIBUTE_UNUSED) {
  int ret = 0;
  xmlNodePtr cur, curElem = NULL;
  xmlNsMapPtr nsMap = NULL;
   ...
   */
  int ancestorsOnly = 0;
  xmlNodePtr resultClone = NULL, clone = NULL, parentClone = NULL,
             prevClone = NULL;
 ...
 * Returns 0 if succeeded, -1 otherwise and on API/internal errors.
 */
static int xmlDOMWrapAdoptAttr(xmlDOMWrapCtxtPtr ctxt, xmlDocPtr sourceDoc,
                               xmlAttrPtr attr, xmlDocPtr destDoc,
                               xmlNodePtr destParent,
                               int options ATTRIBUTE_UNUSED) {
  xmlNodePtr cur;
  int adoptStr = 1;
  ...
  if (attr->ns != NULL) {
    ...
    if (IS_STR_XML(attr->ns->prefix)) {
       ...
       */
      if (xmlSearchNsByNamespaceStrict(destDoc, destParent, attr->ns->href, &ns,
                                       1) == -1)
        goto internal_error;
      if (ns == NULL) {
 ...
 *         -1 on API/internal errors.
 */
int xmlDOMWrapAdoptNode(xmlDOMWrapCtxtPtr ctxt, xmlDocPtr sourceDoc,
                        xmlNodePtr node, xmlDocPtr destDoc,
  ...
  if (node->type == XML_ELEMENT_NODE) {
    return (xmlDOMWrapAdoptBranch(ctxt, sourceDoc, node, destDoc, destParent,
                                  options));
  } else if (node->type == XML_ATTRIBUTE_NODE) {
    return (xmlDOMWrapAdoptAttr(ctxt, sourceDoc, (xmlAttrPtr)node, destDoc,
                                destParent, options));
  } else {
    xmlNodePtr cur = node;
    int adoptStr = 1;
