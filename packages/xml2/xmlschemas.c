 ...
 *     IDC XPath expression and chameleon includes: the targetNamespace is
 * changed, so XPath will have trouble to resolve to this namespace, since not
 * known.
 *
 *
 * CONSTRAINTS:
 ...
 */
static const xmlChar *xmlSchemaNs =
    (const xmlChar *)"http://www.w3.org/2001/XMLSchema";

static const xmlChar *xmlSchemaInstanceNs =
    (const xmlChar *)"http://www.w3.org/2001/XMLSchema-instance";

static const xmlChar *xmlNamespaceNs =
...
#define WXS_ATTRUSE_TYPEDEF(au)                                                \
  WXS_ATTR_TYPEDEF(WXS_ATTRUSE_DECL(WXS_ATTR_USE_CAST au))
 ...
 */
#define WXS_ATTR_GROUP_HAS_REFS(ag)                                            \
  ((WXS_ATTR_GROUP_CAST(ag))->flags & XML_SCHEMAS_ATTRGROUP_HAS_REFS)
#define WXS_ATTR_GROUP_EXPANDED(ag)                                            \
  ((WXS_ATTR_GROUP_CAST(ag))->flags & XML_SCHEMAS_ATTRGROUP_WILDCARD_BUILDED)
/*
 ...
 */
#define WXS_IS_BUCKET_INCREDEF(t)                                              \
  (((t) == XML_SCHEMA_SCHEMA_INCLUDE) || ((t) == XML_SCHEMA_SCHEMA_REDEFINE))

#define WXS_IS_BUCKET_IMPMAIN(t)                                               \
...
struct _xmlSchemaModelGroup {
  xmlSchemaTypeType type; /* XML_SCHEMA_TYPE_SEQUENCE, XML_SCHEMA_TYPE_CHOICE,
                             XML_SCHEMA_TYPE_ALL */
  xmlSchemaAnnotPtr annot;
  xmlSchemaTreeItemPtr next; /* not used */
  xmlSchemaTreeItemPtr
      children; /* first particle (OR "element decl" OR "wildcard") */
  xmlNodePtr node;
};
...
static int xmlSchemaParseRedefine(xmlSchemaParserCtxtPtr ctxt,
                                  xmlSchemaPtr schema, xmlNodePtr node);
static int xmlSchemaTypeFixup(xmlSchemaTypePtr type,
                              xmlSchemaAbstractCtxtPtr ctxt);
static const xmlChar *xmlSchemaFacetTypeToString(xmlSchemaTypeType type);
static int xmlSchemaParseImport(xmlSchemaParserCtxtPtr ctxt,
                                xmlSchemaPtr schema, xmlNodePtr node);
static int xmlSchemaCheckFacetValues(xmlSchemaTypePtr typeDecl,
...
xmlSchemaParseModelGroup(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
                         ...
                         int withParticle);
static const xmlChar *xmlSchemaGetComponentTypeStr(xmlSchemaBasicItemPtr item);
static xmlSchemaTypeLinkPtr
xmlSchemaGetUnionSimpleTypeMemberTypes(xmlSchemaTypePtr type);
static void xmlSchemaInternalErr(xmlSchemaAbstractCtxtPtr actxt,
                                 const char *funcName, const char *message);
static int xmlSchemaCheckCOSSTDerivedOK(xmlSchemaAbstractCtxtPtr ctxt,
                                        xmlSchemaTypePtr type,
                                        xmlSchemaTypePtr baseType, int subset);
static void xmlSchemaCheckElementDeclComponent(xmlSchemaElementPtr elemDecl,
                                               xmlSchemaParserCtxtPtr ctxt);
 ...
 */
static xmlNodePtr xmlSchemaGetComponentNode(xmlSchemaBasicItemPtr item) {
  switch (item->type) {
  case XML_SCHEMA_TYPE_ELEMENT:
 ...
 * string otherwise.
 */
static const xmlChar *xmlSchemaFormatQName(xmlChar **buf,
...
static const xmlChar *xmlSchemaFormatQNameNs(xmlChar **buf, xmlNsPtr ns,
                                             const xmlChar *localName) {
  if (ns != NULL)
...
static const xmlChar *xmlSchemaGetComponentName(xmlSchemaBasicItemPtr item) {
  switch (item->type) {
  case XML_SCHEMA_TYPE_ELEMENT:
...
xmlSchemaGetComponentTargetNs(xmlSchemaBasicItemPtr item) {
  ...
  case XML_SCHEMA_TYPE_ATTRIBUTE_USE:
    if (WXS_ATTRUSE_DECL(item) != NULL) {
      return (
          xmlSchemaGetComponentTargetNs(WXS_BASIC_CAST WXS_ATTRUSE_DECL(item)));
    }
...
static const xmlChar *xmlSchemaGetComponentQName(xmlChar **buf, void *item) {
  return (xmlSchemaFormatQName(
      buf, xmlSchemaGetComponentTargetNs((xmlSchemaBasicItemPtr)item),
      xmlSchemaGetComponentName((xmlSchemaBasicItemPtr)item)));
}

static const xmlChar *xmlSchemaGetComponentDesignation(xmlChar **buf,
                                                       void *item) {
  xmlChar *str = NULL;
  ...
  *buf = xmlStrcat(*buf, BAD_CAST " '");
  *buf = xmlStrcat(
      *buf, xmlSchemaGetComponentQName(&str, (xmlSchemaBasicItemPtr)item));
  *buf = xmlStrcat(*buf, BAD_CAST "'");
  FREE_AND_NULL(str);
  return (*buf);
}

static const xmlChar *xmlSchemaGetIDCDesignation(xmlChar **buf,
                                                 xmlSchemaIDCPtr idc) {
 ...
 */
static int xmlSchemaGetCanonValueWhtspExt(xmlSchemaValPtr val,
 ...
 */
static xmlChar *xmlSchemaFormatItemForReport(xmlChar **buf,
  ...
  if (itemDes != NULL) {
    *buf = xmlStrdup(itemDes);
    ...
    case XML_SCHEMA_TYPE_ATTRIBUTE_USE: {
      ...
      *buf = xmlStrdup(BAD_CAST "attribute use ");
      if (WXS_ATTRUSE_DECL(ause) != NULL) {
        *buf = xmlStrcat(*buf, BAD_CAST "'");
        *buf = xmlStrcat(
            *buf, xmlSchemaGetComponentQName(&str, WXS_ATTRUSE_DECL(ause)));
        FREE_AND_NULL(str)
        *buf = xmlStrcat(*buf, BAD_CAST "'");
      } else {
    ...
    case XML_SCHEMA_TYPE_ATTRIBUTE: {
      ...
      *buf = xmlStrcat(*buf, BAD_CAST " '");
      *buf = xmlStrcat(
          *buf, xmlSchemaFormatQName(&str, attr->targetNamespace, attr->name));
      FREE_AND_NULL(str)
      *buf = xmlStrcat(*buf, BAD_CAST "'");
    } break;
    case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
    ...
    case XML_SCHEMA_TYPE_ELEMENT: {
      ...
      *buf = xmlStrcat(*buf, BAD_CAST " '");
      *buf = xmlStrcat(
          *buf, xmlSchemaFormatQName(&str, elem->targetNamespace, elem->name));
      *buf = xmlStrcat(*buf, BAD_CAST "'");
    } break;
    case XML_SCHEMA_TYPE_IDC_UNIQUE:
    case XML_SCHEMA_TYPE_IDC_KEY:
  ...
  if ((itemNode != NULL) && (itemNode->type == XML_ATTRIBUTE_NODE)) {
    *buf = xmlStrcat(*buf, BAD_CAST ", attribute '");
    if (itemNode->ns != NULL) {
      *buf = xmlStrcat(
          *buf, xmlSchemaFormatQName(&str, itemNode->ns->href, itemNode->name));
      FREE_AND_NULL(str)
    } else
 ...
 */
static const xmlChar *
xmlSchemaFormatFacetEnumSet(xmlSchemaAbstractCtxtPtr actxt, xmlChar **buf,
                            xmlSchemaTypePtr type) {
  ...
  *buf = NULL;
  ...
  do {
    ...
    for (facet = type->facets; facet != NULL; facet = facet->next) {
      ...
      if (res == -1) {
        xmlSchemaInternalErr(actxt, "xmlSchemaFormatFacetEnumSet",
 ...
 */
static void xmlSchemaPErrMemory(xmlSchemaParserCtxtPtr ctxt, const char *extra,
                                xmlNodePtr node) {
  if (ctxt != NULL)
    ctxt->nberrors++;
 ...
 */
static void xmlSchemaPErr(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
                          int error, const char *msg, const xmlChar *str1,
                          const xmlChar *str2) {
  ...
  __xmlRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASP, error,
                  XML_ERR_ERROR, NULL, 0, (const char *)str1,
                  (const char *)str2, NULL, 0, 0, msg, str1, str2);
}
 ...
 * Handle a parser error
 */
static void xmlSchemaPErr2(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
                           xmlNodePtr child, int error, const char *msg,
 ...
 * Handle a parser error
 */
static void xmlSchemaPErrExt(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
                             int error, const xmlChar *strData1,
                             const xmlChar *strData2, const xmlChar *strData3,
  ...
  __xmlRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASP, error,
                  XML_ERR_ERROR, NULL, 0, (const char *)strData1,
                  (const char *)strData2, (const char *)strData3, 0, 0, msg,
 ...
 * Handle an out of memory condition
 */
static void xmlSchemaVErrMemory(xmlSchemaValidCtxtPtr ctxt, const char *extra,
                                xmlNodePtr node) {
  if (ctxt != NULL) {
    ctxt->nberrors++;
    ctxt->err = XML_SCHEMAV_INTERNAL;
  }
  __xmlSimpleError(XML_FROM_SCHEMASV, XML_ERR_NO_MEMORY, node, NULL, extra);
}

static void xmlSchemaPSimpleInternalErr(xmlNodePtr node, const char *msg,
                                        const xmlChar *str) {
  __xmlSimpleError(XML_FROM_SCHEMASP, XML_SCHEMAP_INTERNAL, node, msg,
                   (const char *)str);
}
 ...
 */
static void xmlSchemaErr4Line(xmlSchemaAbstractCtxtPtr ctxt,
                              xmlErrorLevel errorLevel, int error,
                              xmlNodePtr node, int line, const char *msg,
                              const xmlChar *str1, const xmlChar *str2,
  ...
  if (ctxt != NULL) {
    if (ctxt->type == XML_SCHEMA_CTXT_VALIDATOR) {
       ...
       */
      if (line == 0) {
        if ((node == NULL) && (vctxt->depth >= 0) && (vctxt->inode != NULL)) {
      ...
      __xmlRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASV,
                      error, errorLevel, file, line, (const char *)str1,
                      (const char *)str2, (const char *)str3, 0, 0, msg, str1,
                      str2, str3, str4);
      ...
      __xmlRaiseError(schannel, channel, data, ctxt, node, XML_FROM_SCHEMASP,
                      error, errorLevel, NULL, 0, (const char *)str1,
                      (const char *)str2, (const char *)str3, 0, 0, msg, str1,
                      str2, str3, str4);
    } else {
      TODO
 ...
 * Handle a validation error
 */
static void xmlSchemaErr3(xmlSchemaAbstractCtxtPtr actxt, int error,
                          xmlNodePtr node, const char *msg, const xmlChar *str1,
                          const xmlChar *str2, const xmlChar *str3) {
  xmlSchemaErr4Line(actxt, XML_ERR_ERROR, error, node, 0, msg, str1, str2, str3,
                    NULL);
}

static void xmlSchemaErr4(xmlSchemaAbstractCtxtPtr actxt, int error,
                          xmlNodePtr node, const char *msg, const xmlChar *str1,
                          const xmlChar *str2, const xmlChar *str3,
                          const xmlChar *str4) {
  xmlSchemaErr4Line(actxt, XML_ERR_ERROR, error, node, 0, msg, str1, str2, str3,
...
static void xmlSchemaErr(xmlSchemaAbstractCtxtPtr actxt, int error,
                         xmlNodePtr node, const char *msg, const xmlChar *str1,
                         const xmlChar *str2) {
  xmlSchemaErr4(actxt, error, node, msg, str1, str2, NULL, NULL);
}

static xmlChar *xmlSchemaFormatNodeForError(xmlChar **msg,
  ...
  if (node != NULL) {
    ...
    if (node->type == XML_ATTRIBUTE_NODE) {
      ...
      *msg = xmlStrdup(BAD_CAST "Element '");
      if (elem->ns != NULL)
        *msg = xmlStrcat(
            *msg, xmlSchemaFormatQName(&str, elem->ns->href, elem->name));
      else
    ...
    if (node->ns != NULL)
      *msg = xmlStrcat(*msg,
                       xmlSchemaFormatQName(&str, node->ns->href, node->name));
    else
      *msg = xmlStrcat(*msg, xmlSchemaFormatQName(&str, NULL, node->name));
    FREE_AND_NULL(str);
    *msg = xmlStrcat(*msg, BAD_CAST "': ");
    ...
    if (vctxt->inode->nodeType == XML_ATTRIBUTE_NODE) {
      ...
      *msg = xmlStrdup(BAD_CAST "Element '");
      *msg = xmlStrcat(
          *msg, xmlSchemaFormatQName(&str, ielem->nsName, ielem->localName));
      FREE_AND_NULL(str);
      *msg = xmlStrcat(*msg, BAD_CAST "', ");
      *msg = xmlStrcat(*msg, BAD_CAST "attribute '");
    } else {
    ...
    *msg = xmlStrcat(*msg, xmlSchemaFormatQName(&str, vctxt->inode->nsName,
...
static void xmlSchemaInternalErr2(xmlSchemaAbstractCtxtPtr actxt,
  ...
  if (actxt->type == XML_SCHEMA_CTXT_VALIDATOR)
    xmlSchemaErr(actxt, XML_SCHEMAV_INTERNAL, NULL, (const char *)msg, str1,
                 str2);

  else if (actxt->type == XML_SCHEMA_CTXT_PARSER)
    xmlSchemaErr(actxt, XML_SCHEMAP_INTERNAL, NULL, (const char *)msg, str1,
                 str2);
...
static void xmlSchemaCustomErr4(xmlSchemaAbstractCtxtPtr actxt,
                                xmlParserErrors error, xmlNodePtr node,
                                xmlSchemaBasicItemPtr item, const char *message,
                                const xmlChar *str1, const xmlChar *str2,
                                const xmlChar *str3, const xmlChar *str4) {
  xmlChar *msg = NULL;
  ...
  msg = xmlStrcat(msg, BAD_CAST ".\n");
  xmlSchemaErr4(actxt, error, node, (const char *)msg, str1, str2, str3, str4);
  FREE_AND_NULL(msg)
}

static void xmlSchemaCustomErr(xmlSchemaAbstractCtxtPtr actxt,
                               xmlParserErrors error, xmlNodePtr node,
                               xmlSchemaBasicItemPtr item, const char *message,
                               const xmlChar *str1, const xmlChar *str2) {
  xmlSchemaCustomErr4(actxt, error, node, item, message, str1, str2, NULL,
                      NULL);
}

static void xmlSchemaCustomWarning(xmlSchemaAbstractCtxtPtr actxt,
                                   xmlParserErrors error, xmlNodePtr node,
                                   xmlSchemaTypePtr type ATTRIBUTE_UNUSED,
                                   const char *message, const xmlChar *str1,
                                   const xmlChar *str2, const xmlChar *str3) {
  xmlChar *msg = NULL;
  ...
  xmlSchemaErr4Line(actxt, XML_ERR_WARNING, error, node, 0, (const char *)msg,
                    str1, str2, str3, NULL);

  FREE_AND_NULL(msg)
}

static void xmlSchemaKeyrefErr(xmlSchemaValidCtxtPtr vctxt,
  ...
  msg = xmlStrcat(msg, BAD_CAST ".\n");
  xmlSchemaErr4Line(
      ACTXT_CAST vctxt, XML_ERR_ERROR, error, NULL, idcNode->nodeLine,
      (const char *)msg,
...
static int xmlSchemaEvalErrorNodeType(xmlSchemaAbstractCtxtPtr actxt,
                                      xmlNodePtr node) {
  if (node != NULL)
    return (node->type);
  if ((actxt->type == XML_SCHEMA_CTXT_VALIDATOR) &&
...
static void xmlSchemaSimpleTypeErr(xmlSchemaAbstractCtxtPtr actxt,
                                   xmlParserErrors error, xmlNodePtr node,
                                   const xmlChar *value, xmlSchemaTypePtr type,
                                   int displayValue) {
  xmlChar *msg = NULL;
  ...
  if (displayValue ||
      (xmlSchemaEvalErrorNodeType(actxt, node) == XML_ATTRIBUTE_NODE))
  ...
  if (xmlSchemaIsGlobalItem(type)) {
    ...
    if (type->builtInType != 0) {
      ...
      msg = xmlStrcat(
          msg, xmlSchemaFormatQName(&str, type->targetNamespace, type->name));
    msg = xmlStrcat(msg, BAD_CAST "'");
    FREE_AND_NULL(str);
  }
  msg = xmlStrcat(msg, BAD_CAST ".\n");
...
static const xmlChar *xmlSchemaFormatErrorNodeQName(xmlChar **str,
...
static void xmlSchemaIllegalAttrErr(xmlSchemaAbstractCtxtPtr actxt,
                                    xmlParserErrors error,
                                    xmlSchemaAttrInfoPtr ni, xmlNodePtr node) {
  xmlChar *msg = NULL, *str = NULL;

  xmlSchemaFormatNodeForError(&msg, actxt, node);
  msg = xmlStrcat(msg, BAD_CAST "The attribute '%s' is not allowed.\n");
  xmlSchemaErr(
      actxt, error, node, (const char *)msg,
...
static void xmlSchemaComplexTypeErr(xmlSchemaAbstractCtxtPtr actxt,
                                    xmlParserErrors error, xmlNodePtr node,
                                    xmlSchemaTypePtr type ATTRIBUTE_UNUSED,
                                    const char *message, int nbval, int nbneg,
                                    xmlChar **values) {
...
static void xmlSchemaFacetErr(xmlSchemaAbstractCtxtPtr actxt,
                              xmlParserErrors error, xmlNodePtr node,
                              const xmlChar *value, unsigned long length,
                              xmlSchemaTypePtr type, xmlSchemaFacetPtr facet,
                              const char *message, const xmlChar *str1,
                              const xmlChar *str2) {
  xmlChar *str = NULL, *msg = NULL;
  ...
  if (message == NULL) {
    ...
    if ((facetType == XML_SCHEMA_FACET_LENGTH) ||
      ...
      if (nodeType == XML_ATTRIBUTE_NODE)
        xmlSchemaErr3(actxt, error, node, (const char *)msg, value,
      ...
      msg = xmlStrcat(msg, BAD_CAST "The value '%s' is not accepted "
                                    "by the pattern '%s'.\n");
      xmlSchemaErr(actxt, error, node, (const char *)msg, value, facet->value);
 ...
 */
static void xmlSchemaPMissingAttrErr(xmlSchemaParserCtxtPtr ctxt,
                                     xmlParserErrors error,
                                     xmlSchemaBasicItemPtr ownerItem,
                                     xmlNodePtr ownerElem, const char *name,
                                     const char *message) {
  xmlChar *des = NULL;
 ...
 */
static void
xmlSchemaPResCompAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
                         xmlSchemaBasicItemPtr ownerItem, xmlNodePtr ownerElem,
                         const char *name, const xmlChar *refName,
                         const xmlChar *refURI, xmlSchemaTypeType refType,
  ...
  xmlSchemaPErrExt(
      ctxt, ownerElem, error, NULL, NULL, NULL,
      "%s, attribute '%s': The QName value '%s' does not resolve to a(n) "
      "%s.\n",
      BAD_CAST des, BAD_CAST name, xmlSchemaFormatQName(&strA, refURI, refName),
 ...
 */
static void xmlSchemaPCustomAttrErr(xmlSchemaParserCtxtPtr ctxt,
                                    xmlParserErrors error, xmlChar **ownerDes,
                                    xmlSchemaBasicItemPtr ownerItem,
                                    xmlAttrPtr attr, const char *msg) {
  xmlChar *des = NULL;
  ...
  if (attr == NULL) {
    xmlSchemaPErrExt(ctxt, NULL, error, NULL, NULL, NULL,
                     "%s, attribute '%s': %s.\n", BAD_CAST des,
                     (const xmlChar *)"Unknown", (const xmlChar *)msg, NULL,
                     NULL);
  } else {
    xmlSchemaPErrExt(ctxt, (xmlNodePtr)attr, error, NULL, NULL, NULL,
                     "%s, attribute '%s': %s.\n", BAD_CAST des, attr->name,
 ...
 */
static void
xmlSchemaPIllegalAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
  ...
  xmlSchemaErr4(ACTXT_CAST ctxt, error, (xmlNodePtr)attr,
                ...
                xmlSchemaFormatQNameNs(&strB, attr->ns, attr->name), NULL,
                NULL);
  FREE_AND_NULL(strA);
  FREE_AND_NULL(strB);
}
 ...
 */
static void xmlSchemaPCustomErrExt(xmlSchemaParserCtxtPtr ctxt,
                                   xmlParserErrors error,
                                   xmlSchemaBasicItemPtr item,
                                   xmlNodePtr itemElem, const char *message,
                                   const xmlChar *str1, const xmlChar *str2,
                                   const xmlChar *str3) {
  ...
  xmlSchemaPErrExt(ctxt, itemElem, error, NULL, NULL, NULL, (const char *)msg,
                   BAD_CAST des, str1, str2, str3, NULL);
  FREE_AND_NULL(des);
  FREE_AND_NULL(msg);
}
 ...
 * Reports an error during parsing.
 */
static void xmlSchemaPCustomErr(xmlSchemaParserCtxtPtr ctxt,
                                xmlParserErrors error,
                                xmlSchemaBasicItemPtr item, xmlNodePtr itemElem,
                                const char *message, const xmlChar *str1) {
  xmlSchemaPCustomErrExt(ctxt, error, item, itemElem, message, str1, NULL,
                         NULL);
}
 ...
 */
static void xmlSchemaPAttrUseErr4(xmlSchemaParserCtxtPtr ctxt,
                                  xmlParserErrors error, xmlNodePtr node,
                                  xmlSchemaBasicItemPtr ownerItem,
  ...
  msg = xmlStrcat(msg, BAD_CAST ", ");
  msg = xmlStrcat(msg, BAD_CAST xmlSchemaFormatItemForReport(
                           &str, NULL, WXS_BASIC_CAST attruse, NULL));
  FREE_AND_NULL(str);
  msg = xmlStrcat(msg, BAD_CAST ": ");
  msg = xmlStrcat(msg, (const xmlChar *)message);
  msg = xmlStrcat(msg, BAD_CAST ".\n");
  xmlSchemaErr4(ACTXT_CAST ctxt, error, node, (const char *)msg, str1, str2,
                str3, str4);
  xmlFree(msg);
}
 ...
 */
static void xmlSchemaPIllegalFacetAtomicErr(xmlSchemaParserCtxtPtr ctxt,
                                            xmlParserErrors error,
                                            xmlSchemaTypePtr type,
  ...
  xmlSchemaPErrExt(
      ctxt, type->node, error, NULL, NULL, NULL,
 ...
 */
static void xmlSchemaPIllegalFacetListUnionErr(xmlSchemaParserCtxtPtr ctxt,
                                               xmlParserErrors error,
                                               xmlSchemaTypePtr type,
                                               xmlSchemaFacetPtr facet) {
  xmlChar *des = NULL;
  ...
  xmlSchemaFormatItemForReport(&des, NULL, WXS_BASIC_CAST type, type->node);
  xmlSchemaPErr(ctxt, type->node, error, "%s: The facet '%s' is not allowed.\n",
                BAD_CAST des, xmlSchemaFacetTypeToString(facet->type));
  FREE_AND_NULL(des);
}
 ...
 * Reports an illegal attribute.
 */
static void xmlSchemaPMutualExclAttrErr(xmlSchemaParserCtxtPtr ctxt,
                                        xmlParserErrors error,
                                        xmlSchemaBasicItemPtr ownerItem,
                                        xmlAttrPtr attr, const char *name1,
                                        const char *name2) {
  xmlChar *des = NULL;

  xmlSchemaFormatItemForReport(&des, NULL, WXS_BASIC_CAST ownerItem,
                               attr->parent);
  xmlSchemaPErrExt(ctxt, (xmlNodePtr)attr, error, NULL, NULL, NULL,
 ...
 */
static void xmlSchemaPSimpleTypeErr(
    xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
    xmlSchemaBasicItemPtr ownerItem ATTRIBUTE_UNUSED, xmlNodePtr node,
    xmlSchemaTypePtr type, const char *expected, const xmlChar *value,
  ...
  if (message == NULL) {
    ...
    if (type != NULL) {
      ...
      if (xmlSchemaIsGlobalItem(type)) {
        ...
        if (type->builtInType != 0) {
          ...
          msg = xmlStrcat(msg, xmlSchemaFormatQName(&str, type->targetNamespace,
                                                    type->name));
        msg = xmlStrcat(msg, BAD_CAST "'.");
        FREE_AND_NULL(str);
      }
    ...
    msg = xmlStrcat(msg, BAD_CAST message);
    msg = xmlStrcat(msg, BAD_CAST ".\n");
    xmlSchemaPErrExt(ctxt, node, error, NULL, NULL, NULL, (const char *)msg,
                     str1, str2, NULL, NULL, NULL);
  }
  /* Cleanup. */
  FREE_AND_NULL(msg)
}
 ...
 */
static void xmlSchemaPContentErr(xmlSchemaParserCtxtPtr ctxt,
                                 xmlParserErrors error,
                                 xmlSchemaBasicItemPtr ownerItem,
                                 xmlNodePtr ownerElem, xmlNodePtr child,
 ...
 * Returns the newly allocated structure or NULL in case or error
 */
static xmlSchemaAnnotPtr xmlSchemaNewAnnot(xmlSchemaParserCtxtPtr ctxt,
                                           xmlNodePtr node) {
...
static xmlSchemaItemListPtr xmlSchemaItemListCreate(void) {
  ...
  if (ret == NULL) {
    xmlSchemaPErrMemory(NULL, "allocating an item list structure", NULL);
    return (NULL);
  }
...
static int xmlSchemaItemListAdd(xmlSchemaItemListPtr list, void *item) {
  if (list->items == NULL) {
    list->items = (void **)xmlMalloc(20 * sizeof(void *));
    if (list->items == NULL) {
    ...
    list->sizeItems *= 2;
    list->items =
        (void **)xmlRealloc(list->items, list->sizeItems * sizeof(void *));
    if (list->items == NULL) {
...
static int xmlSchemaItemListAddSize(xmlSchemaItemListPtr list, int initialSize,
                                    void *item) {
  if (list->items == NULL) {
    if (initialSize <= 0)
      initialSize = 1;
...
static int xmlSchemaItemListInsert(xmlSchemaItemListPtr list, void *item,
                                   int idx) {
  if (list->items == NULL) {
    list->items = (void **)xmlMalloc(20 * sizeof(void *));
    if (list->items == NULL) {
 ...
 */
static void xmlSchemaItemListFree(xmlSchemaItemListPtr list) {
  if (list == NULL)
    return;
  if (list->items != NULL)
    xmlFree(list->items);
...
static void xmlSchemaBucketFree(xmlSchemaBucketPtr bucket) {
  if (bucket == NULL)
    return;
...
xmlSchemaBucketCreate(xmlSchemaParserCtxtPtr pctxt, int type,
                      const xmlChar *targetNamespace) {
  ...
  if (WXS_IS_BUCKET_IMPMAIN(type)) {
    ...
    if (mainSchema->schemasImports == NULL) {
      mainSchema->schemasImports =
          xmlHashCreateDict(5, WXS_CONSTRUCTOR(pctxt)->dict);
      if (mainSchema->schemasImports == NULL) {
        xmlSchemaBucketFree(ret);
        return (NULL);
      }
...
static int xmlSchemaAddItemSize(xmlSchemaItemListPtr *list, int initialSize,
                                void *item) {
  if (*list == NULL) {
 ...
 * Deallocate a annotation structure
 */
static void xmlSchemaFreeAnnot(xmlSchemaAnnotPtr annot) {
  if (annot == NULL)
    return;
  if (annot->next == NULL) {
    xmlFree(annot);
 ...
 */
static void xmlSchemaFreeAttribute(xmlSchemaAttributePtr attr) {
  if (attr == NULL)
    return;
  if (attr->annot != NULL)
 ...
 */
static void xmlSchemaFreeWildcardNsSet(xmlSchemaWildcardNsPtr set) {
 ...
 */
static void xmlSchemaFreeQNameRef(xmlSchemaQNameRefPtr item) { xmlFree(item); }
 ...
 */
static void xmlSchemaFreeTypeLinkList(xmlSchemaTypeLinkPtr link) {
...
static void xmlSchemaFreeIDCStateObjList(xmlSchemaIDCStateObjPtr sto) {
    ...
    return;
  {
    ...
    for (i = 0; i < list->nbItems; i++) {
      ...
      case XML_SCHEMA_TYPE_ATTRIBUTE_USE:
        ...
        break;
      case XML_SCHEMA_EXTRA_ATTR_USE_PROHIB:
        xmlSchemaFreeAttributeUseProhib((xmlSchemaAttributeUseProhibPtr)item);
        break;
      case XML_SCHEMA_TYPE_ELEMENT:
      ...
      case XML_SCHEMA_TYPE_ALL:
        ...
        break;
      case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
        xmlSchemaFreeAttributeGroup((xmlSchemaAttributeGroupPtr)item);
        break;
      case XML_SCHEMA_TYPE_GROUP:
...
void xmlSchemaFree(xmlSchemaPtr schema) {
  ...
  if (schema->volatiles != NULL)
    TODO
         ...
         * the schema buckets.
         */
        if (schema->notaDecl != NULL) xmlHashFree(schema->notaDecl, NULL);
...
static void xmlSchemaTypeDump(xmlSchemaTypePtr type,
                              FILE *output); /* forward */
 ...
 * Dump the element
 */
static void xmlSchemaElementDump(xmlSchemaElementPtr elem, FILE *output,
 ...
 */
static void xmlSchemaContentModelDump(xmlSchemaParticlePtr particle,
  ...
  if (term &&
      ...
      (term->children != NULL)) {
    xmlSchemaContentModelDump((xmlSchemaParticlePtr)term->children, output,
                              depth + 1);
  }
  if (particle->next != NULL)
    xmlSchemaContentModelDump((xmlSchemaParticlePtr)particle->next, output,
                              depth);
}
...
static void xmlSchemaTypeDump(xmlSchemaTypePtr type, FILE *output) {
  ...
  if ((type->type == XML_SCHEMA_TYPE_COMPLEX) && (type->subtypes != NULL)) {
...
void xmlSchemaDump(FILE *output, xmlSchemaPtr schema) {
  ...
  xmlHashScanFull(schema->elemDecl, (xmlHashScannerFull)xmlSchemaElementDump,
                  output);
}
 ...
 */
static xmlAttrPtr xmlSchemaGetPropNode(xmlNodePtr node, const char *name) {
  xmlAttrPtr prop;
 ...
 * Returns the attribute or NULL if not present.
 */
static xmlAttrPtr xmlSchemaGetPropNodeNs(xmlNodePtr node, const char *uri,
                                         const char *name) {
  xmlAttrPtr prop;
...
static const xmlChar *xmlSchemaGetNodeContent(xmlSchemaParserCtxtPtr ctxt,
                                              xmlNodePtr node) {
  xmlChar *val;
  const xmlChar *ret;
...
static const xmlChar *xmlSchemaGetNodeContentNoDict(xmlNodePtr node) {
 ...
 */
static const xmlChar *xmlSchemaGetProp(xmlSchemaParserCtxtPtr ctxt,
...
#define WXS_FIND_GLOBAL_ITEM(slot)                                             \
  ...
  if (xmlHashSize(schema->schemasImports) > 1) {                               \
    ...
    if (nsName == NULL)                                                        \
      import =                                                                 \
          xmlHashLookup(schema->schemasImports, XML_SCHEMAS_NO_NAMESPACE);     \
    else                                                                       \
 ...
 */
static xmlSchemaElementPtr xmlSchemaGetElem(xmlSchemaPtr schema,
                                            const xmlChar *name,
                                            const xmlChar *nsName) {
 ...
 */
static xmlSchemaTypePtr xmlSchemaGetType(xmlSchemaPtr schema,
                                         const xmlChar *name,
                                         const xmlChar *nsName) {
 ...
 */
static xmlSchemaAttributePtr xmlSchemaGetAttributeDecl(xmlSchemaPtr schema,
                                                       const xmlChar *name,
                                                       const xmlChar *nsName) {
 ...
 * Returns the group definition or NULL if not found.
 */
static xmlSchemaModelGroupDefPtr xmlSchemaGetGroup(xmlSchemaPtr schema,
                                                   const xmlChar *name,
                                                   const xmlChar *nsName) {
...
static xmlSchemaNotationPtr xmlSchemaGetNotation(xmlSchemaPtr schema,
                                                 const xmlChar *name,
                                                 const xmlChar *nsName) {
...
static xmlSchemaIDCPtr xmlSchemaGetIDC(xmlSchemaPtr schema, const xmlChar *name,
                                       const xmlChar *nsName) {
 ...
 */
static xmlSchemaBasicItemPtr
xmlSchemaGetNamedComponent(xmlSchemaPtr schema, xmlSchemaTypeType itemType,
                           const xmlChar *name, const xmlChar *targetNs) {
  switch (itemType) {
  case XML_SCHEMA_TYPE_GROUP:
    return ((xmlSchemaBasicItemPtr)xmlSchemaGetGroup(schema, name, targetNs));
  case XML_SCHEMA_TYPE_ELEMENT:
    return ((xmlSchemaBasicItemPtr)xmlSchemaGetElem(schema, name, targetNs));
  default:
    TODO return (NULL);
  }
}
 ...
 */
static xmlSchemaBasicItemPtr
xmlSchemaFindRedefCompInGraph(xmlSchemaBucketPtr bucket, xmlSchemaTypeType type,
    ...
    if (ret->type == type) {
      ...
      case XML_SCHEMA_TYPE_SIMPLE:
        if ((WXS_COMP_NAME(ret, xmlSchemaTypePtr) == name) &&
            (WXS_COMP_TNS(ret, xmlSchemaTypePtr) == nsName)) {
          return (ret);
        }
        break;
      case XML_SCHEMA_TYPE_GROUP:
        if ((WXS_COMP_NAME(ret, xmlSchemaModelGroupDefPtr) == name) &&
            (WXS_COMP_TNS(ret, xmlSchemaModelGroupDefPtr) == nsName)) {
          return (ret);
        }
        break;
      case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
        if ((WXS_COMP_NAME(ret, xmlSchemaAttributeGroupPtr) == name) &&
            (WXS_COMP_TNS(ret, xmlSchemaAttributeGroupPtr) == nsName)) {
          return (ret);
        }
        break;
      default:
...
static xmlSchemaRedefPtr xmlSchemaAddRedef(xmlSchemaParserCtxtPtr pctxt,
                                           ...
                                           void *item, const xmlChar *refName,
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaAttributeGroupPtr xmlSchemaAddAttributeGroupDefinition(
    xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema ATTRIBUTE_UNUSED,
    const xmlChar *name, const xmlChar *nsName, xmlNodePtr node) {
  xmlSchemaAttributeGroupPtr ret = NULL;
  ...
  if (pctxt->isRedefine) {
    pctxt->redef =
        xmlSchemaAddRedef(pctxt, pctxt->redefined, ret, name, nsName);
    if (pctxt->redef == NULL) {
      xmlFree(ret);
      return (NULL);
    }
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaElementPtr xmlSchemaAddElement(xmlSchemaParserCtxtPtr ctxt,
                                               const xmlChar *name,
                                               const xmlChar *nsName,
                                               xmlNodePtr node, int topLevel) {
  xmlSchemaElementPtr ret = NULL;
...
xmlSchemaAddType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
                 xmlSchemaTypeType type, const xmlChar *name,
                 const xmlChar *nsName, xmlNodePtr node, int topLevel) {
  xmlSchemaTypePtr ret = NULL;
...
static xmlSchemaQNameRefPtr xmlSchemaNewQNameRef(xmlSchemaParserCtxtPtr pctxt,
                                                 xmlSchemaTypeType refType,
                                                 const xmlChar *refName,
                                                 const xmlChar *refNs) {
  xmlSchemaQNameRefPtr ret;

  ret = (xmlSchemaQNameRefPtr)xmlMalloc(sizeof(xmlSchemaQNameRef));
  if (ret == NULL) {
    xmlSchemaPErrMemory(pctxt, "allocating QName reference item", NULL);
    return (NULL);
  }
  ret->node = NULL;
...
xmlSchemaAddAttributeUseProhib(xmlSchemaParserCtxtPtr pctxt) {
  ...
  ret = (xmlSchemaAttributeUseProhibPtr)xmlMalloc(
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaParticlePtr xmlSchemaAddParticle(xmlSchemaParserCtxtPtr ctxt,
                                                 xmlNodePtr node, int min,
                                                 int max) {
  ...
  if (ret == NULL) {
    xmlSchemaPErrMemory(ctxt, "allocating particle component", NULL);
    return (NULL);
  }
...
xmlSchemaAddModelGroupDefinition(xmlSchemaParserCtxtPtr ctxt,
                                 ...
                                 const xmlChar *nsName, xmlNodePtr node) {
  xmlSchemaModelGroupDefPtr ret = NULL;
...
xmlSchemaNewWildcardNsConstraint(xmlSchemaParserCtxtPtr ctxt) {
  ...
  ret = (xmlSchemaWildcardNsPtr)xmlMalloc(sizeof(xmlSchemaWildcardNs));
  if (ret == NULL) {
...
static xmlSchemaIDCPtr xmlSchemaAddIDC(xmlSchemaParserCtxtPtr ctxt,
                                       xmlSchemaPtr schema, const xmlChar *name,
                                       const xmlChar *nsName, int category,
                                       xmlNodePtr node) {
  xmlSchemaIDCPtr ret = NULL;
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaWildcardPtr xmlSchemaAddWildcard(xmlSchemaParserCtxtPtr ctxt,
                                                 xmlSchemaPtr schema,
                                                 xmlSchemaTypeType type,
                                                 xmlNodePtr node) {
...
static void xmlSchemaSubstGroupFree(xmlSchemaSubstGroupPtr group) {
  if (group == NULL)
    return;
...
xmlSchemaSubstGroupAdd(xmlSchemaParserCtxtPtr pctxt, xmlSchemaElementPtr head) {
  ...
  if (ret == NULL) {
    xmlSchemaPErrMemory(NULL, "allocating a substitution group container",
                        NULL);
    return (NULL);
  }
  ...
  if (xmlHashAddEntry2(WXS_SUBST_GROUPS(pctxt), head->name,
                       head->targetNamespace, ret) != 0) {
...
static xmlSchemaSubstGroupPtr
xmlSchemaSubstGroupGet(xmlSchemaParserCtxtPtr pctxt, xmlSchemaElementPtr head) {
  if (WXS_SUBST_GROUPS(pctxt) == NULL)
    return (NULL);
  return (xmlHashLookup2(WXS_SUBST_GROUPS(pctxt), head->name,
 ...
 * Returns the newly allocated structure or NULL in case or error
 */
static int xmlSchemaAddElementSubstitutionMember(xmlSchemaParserCtxtPtr pctxt,
                                                 xmlSchemaElementPtr head,
 ...
 */
static int xmlSchemaPValAttrNodeQNameValue(
    xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlSchemaBasicItemPtr ownerItem, xmlAttrPtr attr, const xmlChar *value,
  ...
  *local = NULL;
  ...
  if (ret > 0) {
    xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, ownerItem,
                            (xmlNodePtr)attr,
                            xmlSchemaGetBuiltInType(XML_SCHEMAS_QNAME), NULL,
  ...
  *local = xmlDictLookup(ctxt->dict, *local, -1);
  ...
  if (ns == NULL) {
    xmlSchemaPSimpleTypeErr(
        ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, ownerItem, (xmlNodePtr)attr,
 ...
 * if not valid and -1 if an internal error occurs.
 */
static int xmlSchemaPValAttrNodeQName(xmlSchemaParserCtxtPtr ctxt,
                                      xmlSchemaPtr schema,
                                      xmlSchemaBasicItemPtr ownerItem,
                                      xmlAttrPtr attr, const xmlChar **uri,
                                      const xmlChar **local) {
  const xmlChar *value;

  value = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr)attr);
  return (xmlSchemaPValAttrNodeQNameValue(ctxt, schema, ownerItem, attr, value,
                                          uri, local));
}
 ...
 * if not valid and -1 if an internal error occurs.
 */
static int xmlSchemaPValAttrQName(xmlSchemaParserCtxtPtr ctxt,
                                  xmlSchemaPtr schema,
                                  xmlSchemaBasicItemPtr ownerItem,
                                  xmlNodePtr ownerElem, const char *name,
                                  const xmlChar **uri, const xmlChar **local) {
  ...
  return (
      xmlSchemaPValAttrNodeQName(ctxt, schema, ownerItem, attr, uri, local));
}
 ...
 * if not valid and -1 if an internal error occurs.
 */
static int xmlSchemaPValAttrNodeID(xmlSchemaParserCtxtPtr ctxt,
                                   xmlAttrPtr attr) {
  int ret;
  const xmlChar *value;
  ...
  if (ret == 0) {
    ...
    if (attr->atype != XML_ATTRIBUTE_ID) {
      ...
      if (res == NULL) {
        ...
        xmlSchemaPSimpleTypeErr(
            ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNodePtr)attr,
            xmlSchemaGetBuiltInType(XML_SCHEMAS_ID), NULL, NULL,
    ...
    ret = XML_SCHEMAP_S4S_ATTR_INVALID_VALUE;
    xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL,
                            (xmlNodePtr)attr,
                            xmlSchemaGetBuiltInType(XML_SCHEMAS_ID), NULL, NULL,
...
static int xmlSchemaPValAttrID(xmlSchemaParserCtxtPtr ctxt,
                               xmlNodePtr ownerElem, const xmlChar *name) {
  xmlAttrPtr attr;
 ...
 */
static int xmlGetMaxOccurs(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
  ...
  if (xmlStrEqual(val, (const xmlChar *)"unbounded")) {
    if (max != UNBOUNDED) {
      xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE,
                              ...
                              NULL, (xmlNodePtr)attr, NULL, expected, val, NULL,
                              NULL, NULL);
      return (def);
    } else
 ...
 * Returns the default if not found, or the value
 */
static int xmlGetMinOccurs(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
                           int min, int max, int def, const char *expected) {
  const xmlChar *val, *cur;
  int ret = 0;
  xmlAttrPtr attr;
 ...
 * Returns 0 or 1.
 */
static int xmlSchemaPGetBoolNodeValue(xmlSchemaParserCtxtPtr ctxt,
                                      xmlSchemaBasicItemPtr ownerItem,
                                      xmlNodePtr node) {
  xmlChar *value = NULL;
  int res = 0;
  ...
  if (xmlStrEqual(BAD_CAST value, BAD_CAST "true"))
    ...
    xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_INVALID_BOOLEAN, ownerItem, node,
                            xmlSchemaGetBuiltInType(XML_SCHEMAS_BOOLEAN), NULL,
                            BAD_CAST value, NULL, NULL, NULL);
  }
  if (value != NULL)
    xmlFree(value);
  return (res);
 ...
 */
static int xmlGetBooleanProp(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
 ...
 * 									*
 ************************************************************************/
static xmlSchemaTypePtr xmlSchemaParseSimpleType(xmlSchemaParserCtxtPtr ctxt,
                                                 xmlSchemaPtr schema,
                                                 xmlNodePtr node, int topLevel);
...
static xmlSchemaBasicItemPtr
xmlSchemaParseLocalAttribute(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema,
                             xmlNodePtr node, xmlSchemaItemListPtr uses,
                             int parentType);
 ...
 */
static int xmlSchemaPValAttrNodeValue(xmlSchemaParserCtxtPtr pctxt,
                                      xmlSchemaBasicItemPtr ownerItem,
  ...
  if (ret < 0) {
    ...
    xmlSchemaPSimpleTypeErr(pctxt, ret, ownerItem, (xmlNodePtr)attr, type, NULL,
                            value, NULL, NULL, NULL);
  }
  return (ret);
}
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaPValAttrNode(xmlSchemaParserCtxtPtr ctxt,
                                 xmlSchemaBasicItemPtr ownerItem,
                                 xmlAttrPtr attr, xmlSchemaTypePtr type,
                                 const xmlChar **value) {
  const xmlChar *val;
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaPValAttr(xmlSchemaParserCtxtPtr ctxt,
                             xmlSchemaBasicItemPtr ownerItem,
                             xmlNodePtr ownerElem, const char *name,
  ...
  if (type->type != XML_SCHEMA_TYPE_BASIC) {
    if (value != NULL)
      *value = NULL;
    xmlSchemaPErr(ctxt, ownerElem, XML_SCHEMAP_INTERNAL,
...
static int xmlSchemaCheckReference(xmlSchemaParserCtxtPtr pctxt,
                                   xmlSchemaPtr schema ATTRIBUTE_UNUSED,
                                   xmlNodePtr node, xmlAttrPtr attr,
                                   const xmlChar *namespaceName) {
   ...
   */
  {
    ...
    if (namespaceName == NULL)
      ...
      xmlSchemaCustomErr(
          ACTXT_CAST pctxt, XML_SCHEMAP_SRC_RESOLVE, n, NULL,
 ...
 */
static int xmlSchemaParseLocalAttributes(xmlSchemaParserCtxtPtr ctxt,
                                         xmlSchemaPtr schema, xmlNodePtr *child,
 ...
 */
static xmlSchemaAnnotPtr xmlSchemaParseAnnotation(xmlSchemaParserCtxtPtr ctxt,
  ...
  while (attr != NULL) {
    if (((attr->ns == NULL) && (!xmlStrEqual(attr->name, BAD_CAST "id"))) ||
        ((attr->ns != NULL) && xmlStrEqual(attr->ns->href, xmlSchemaNs))) {

      xmlSchemaPIllegalAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED, NULL,
                               attr);
    }
    attr = attr->next;
  }
  ...
  while (child != NULL) {
    if (IS_SCHEMA(child, "appinfo")) {
       ...
       * Attribute "xml:lang".
       */
      attr = xmlSchemaGetPropNodeNs(child, (const char *)XML_XML_NAMESPACE,
                                    "lang");
      if (attr != NULL)
      ...
      if (!barked)
        xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node,
                             child, NULL, "(appinfo | documentation)*");
      barked = 1;
      child = child->next;
    }
  }
 ...
 */
static xmlSchemaFacetPtr xmlSchemaParseFacet(xmlSchemaParserCtxtPtr ctxt,
                                             xmlSchemaPtr schema,
                                             xmlNodePtr node) {
  xmlSchemaFacetPtr facet;
  xmlNodePtr child = NULL;
 ...
 */
static int xmlSchemaParseWildcardNs(xmlSchemaParserCtxtPtr ctxt,
                                    xmlSchemaPtr schema ATTRIBUTE_UNUSED,
  ...
  if ((pc == NULL) || (xmlStrEqual(pc, (const xmlChar *)"strict"))) {
    ...
    xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL,
                            node, NULL, "(strict | skip | lax)", pc, NULL, NULL,
                            NULL);
    wildc->processContents = XML_SCHEMAS_ANY_STRICT;
    ret = XML_SCHEMAP_S4S_ATTR_INVALID_VALUE;
  }
...
static int xmlSchemaPCheckParticleCorrect_2(
    xmlSchemaParserCtxtPtr ctxt, xmlSchemaParticlePtr item ATTRIBUTE_UNUSED,
    xmlNodePtr node, int minOccurs, int maxOccurs) {
  ...
  if (maxOccurs != UNBOUNDED) {
    ...
    if (maxOccurs < 1) {
       ...
       */
      xmlSchemaPCustomAttrErr(
          ctxt, XML_SCHEMAP_P_PROPS_CORRECT_2_1, NULL, NULL,
 ...
 */
static xmlSchemaParticlePtr xmlSchemaParseAny(xmlSchemaParserCtxtPtr ctxt,
                                              xmlSchemaPtr schema,
                                              xmlNodePtr node) {
 ...
 */
static xmlSchemaNotationPtr xmlSchemaParseNotation(xmlSchemaParserCtxtPtr ctxt,
                                                   xmlSchemaPtr schema,
                                                   xmlNodePtr node) {
  const xmlChar *name;
...
#define WXS_ATTR_DEF_VAL_FIXED 2
  ...
  if (attr != NULL) {
    if (xmlSchemaPValAttrNodeQName(pctxt, schema, NULL, attr, &tmpNs,
                                   &tmpName) != 0) {
      return (NULL);
    }
  ...
  while (attr != NULL) {
    if (attr->ns == NULL) {
      if (isRef) {
        ...
        if (xmlStrEqual(attr->name, BAD_CAST "name")) {
          ...
          xmlSchemaPValAttrNodeQName(pctxt, schema, NULL, attr, &tmpNs,
                                     &tmpName);
          goto attr_next;
          ...
          if (xmlStrEqual(attrValue, BAD_CAST "qualified")) {
            ...
            xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE,
                                    NULL, (xmlNodePtr)attr, NULL,
                                    "(qualified | unqualified)", attrValue,
                                    NULL, NULL, NULL);
          }
          goto attr_next;
        }
      ...
      if (xmlStrEqual(attr->name, BAD_CAST "use")) {
        ...
        if (xmlStrEqual(attrValue, BAD_CAST "optional"))
          ...
          xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_INVALID_ATTR_USE, NULL,
         ...
         */
        if (defValue) {
          xmlSchemaPMutualExclAttrErr(pctxt, XML_SCHEMAP_SRC_ATTRIBUTE_1, NULL,
                                      attr, "default", "fixed");
        } else {
    ...
    xmlSchemaPIllegalAttrErr(pctxt, XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED, NULL,
                             attr);
  ...
  if ((defValueType == WXS_ATTR_DEF_VAL_DEFAULT) &&
      (occurs != XML_SCHEMAS_ATTR_USE_OPTIONAL)) {
    xmlSchemaPSimpleTypeErr(
        pctxt, XML_SCHEMAP_SRC_ATTRIBUTE_2, NULL, node, NULL,
  ...
  if (!isRef) {
    ...
    if (attr == NULL) {
      xmlSchemaPMissingAttrErr(pctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node,
                               "name", NULL);
      return (NULL);
    }
    ...
    if (xmlStrEqual(name, BAD_CAST "xmlns")) {
      xmlSchemaPSimpleTypeErr(
          pctxt, XML_SCHEMAP_NO_XMLNS, NULL, (xmlNodePtr)attr,
          xmlSchemaGetBuiltInType(XML_SCHEMAS_NCNAME), NULL, NULL,
          "The value of the attribute must not match 'xmlns'", NULL, NULL);
      return (NULL);
    }
     ...
     */
    ref =
        xmlSchemaNewQNameRef(pctxt, XML_SCHEMA_TYPE_ATTRIBUTE, tmpName, tmpNs);
    if (ref == NULL)
      return (NULL);
    /*
...
check_children:
  ...
  if (occurs == XML_SCHEMAS_ATTR_USE_PROHIBITED) {
    ...
    if (child != NULL) {
      xmlSchemaPContentErr(pctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node,
                           child, NULL, "(annotation?)");
    }
    /*
    ...
    if (parentType == XML_SCHEMA_TYPE_ATTRIBUTEGROUP) {
      xmlSchemaCustomWarning(ACTXT_CAST pctxt,
                             XML_SCHEMAP_WARN_ATTR_POINTLESS_PROH, node, NULL,
      ...
      for (i = 0; i < uses->nbItems; i++) {
        ...
        if ((use->type == XML_SCHEMA_EXTRA_ATTR_USE_PROHIB) &&
          ...
          xmlSchemaCustomWarning(
              ACTXT_CAST pctxt, XML_SCHEMAP_WARN_ATTR_POINTLESS_PROH, node,
              NULL, "Skipping duplicate attribute use prohibition '%s'",
              xmlSchemaFormatQName(&str, tmpNs, tmpName), NULL, NULL);
          FREE_AND_NULL(str)
          return (NULL);
        }
      }
    ...
    if (isRef) {
      if (child != NULL) {
        if (IS_SCHEMA(child, "simpleType"))
           ...
           */
          xmlSchemaPContentErr(pctxt, XML_SCHEMAP_SRC_ATTRIBUTE_3_2, NULL, node,
                               child, NULL, "(annotation?)");
        else
          xmlSchemaPContentErr(pctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL,
                               node, child, NULL, "(annotation?)");
      }
    } else {
      if (IS_SCHEMA(child, "simpleType")) {
        if (WXS_ATTRUSE_DECL(use)->typeName != NULL) {
           ...
           */
          xmlSchemaPContentErr(
              pctxt, XML_SCHEMAP_SRC_ATTRIBUTE_4, NULL, node, child,
...
static xmlSchemaAttributePtr
xmlSchemaParseGlobalAttribute(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema,
                              xmlNodePtr node) {
  ...
  ret = xmlSchemaAddAttribute(pctxt, schema, attrValue, pctxt->targetNamespace,
                              node, 1);
  if (ret == NULL)
    return (NULL);
  ...
  xmlSchemaPValAttrQName(pctxt, schema, NULL, node, "type", &ret->typeNs,
                         &ret->typeName);
  ...
  if (IS_SCHEMA(child, "simpleType")) {
    if (ret->typeName != NULL) {
       ...
       * type and <simpleType> must not both be present.
       */
      xmlSchemaPContentErr(pctxt, XML_SCHEMAP_SRC_ATTRIBUTE_4, NULL, node,
                           child,
                           "The attribute 'type' and the <simpleType> child "
                           "are mutually exclusive",
                           NULL);
  ...
  if (child != NULL)
    xmlSchemaPContentErr(pctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node,
                         child, NULL, "(annotation?, simpleType?)");
...
xmlSchemaParseAttributeGroupRef(xmlSchemaParserCtxtPtr pctxt,
  ...
  if (pctxt->isRedefine && pctxt->redef &&
      ...
      (ref == pctxt->redef->refName) && (refNs == pctxt->redef->refTargetNs)) {
    /*
     * SPEC src-redefine:
    ...
    if (pctxt->redefCounter != 0) {
      ...
      xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_SRC_REDEFINE, node, NULL,
     ...
     */
    ret =
        xmlSchemaNewQNameRef(pctxt, XML_SCHEMA_TYPE_ATTRIBUTEGROUP, ref, refNs);
    if (ret == NULL)
      return (NULL);
    ret->node = node;
...
xmlSchemaParseAttributeGroupDefinition(xmlSchemaParserCtxtPtr pctxt,
  ...
  ret = xmlSchemaAddAttributeGroupDefinition(pctxt, schema, name,
  ...
  if (xmlSchemaParseLocalAttributes(
          pctxt, schema, &child, (xmlSchemaItemListPtr *)&(ret->attrUses),
  ...
  if (child != NULL) {
    xmlSchemaPContentErr(
        pctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL,
 ...
 */
static int xmlSchemaPValAttrFormDefault(const xmlChar *value, int *flags,
                                        int flagQualified) {
...
static int xmlSchemaPValAttrBlockFinal(const xmlChar *value, int *flags,
                                       int flagAll, int flagExtension,
                                       int flagRestriction,
                                       int flagSubstitution, int flagList,
                                       int flagUnion) {
  int ret = 0;
...
static int xmlSchemaCheckCSelectorXPath(xmlSchemaParserCtxtPtr ctxt,
                                        xmlSchemaIDCPtr idc,
                                        xmlSchemaIDCSelectPtr selector,
                                        xmlAttrPtr attr, int isField) {
  xmlNodePtr node;
   ...
   */
  if (selector == NULL) {
    xmlSchemaPErr(ctxt, idc->node, XML_SCHEMAP_INTERNAL,
  ...
  if (selector->xpath == NULL) {
    xmlSchemaPCustomErr(ctxt,
                        ...
                        XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, node,
    ...
    if (nsList != NULL) {
      ...
      nsArray = (const xmlChar **)xmlMalloc((count * 2 + 1) *
                                            sizeof(const xmlChar *));
      if (nsArray == NULL) {
        xmlSchemaPErrMemory(ctxt, "allocating a namespace array", NULL);
        xmlFree(nsList);
        return (-1);
      }
      for (i = 0; i < count; i++) {
     ...
     */
    if (isField)
      selector->xpathComp = (void *)xmlPatterncompile(
          selector->xpath, NULL, XML_PATTERN_XSFIELD, nsArray);
    else
      selector->xpathComp = (void *)xmlPatterncompile(
          selector->xpath, NULL, XML_PATTERN_XSSEL, nsArray);
    if (nsArray != NULL)
 ...
 */
static xmlSchemaAnnotPtr xmlSchemaAddAnnotation(xmlSchemaAnnotItemPtr annItem,
                                                xmlSchemaAnnotPtr annot) {
  ...
  default:
    xmlSchemaPCustomErr(NULL, XML_SCHEMAP_INTERNAL, NULL, NULL,
                        "Internal error: xmlSchemaAddAnnotation, "
                        "The item is not a annotated schema component",
                        NULL);
    break;
  }
  return (annot);
...
xmlSchemaParseIDCSelectorAndField(xmlSchemaParserCtxtPtr ctxt,
                                  xmlSchemaIDCPtr idc, xmlNodePtr node,
                                  int isField) {
  ...
  if (item == NULL) {
    xmlSchemaPErrMemory(
        ctxt, "allocating a 'selector' of an identity-constraint definition",
        NULL);
    return (NULL);
  }
  ...
  if (attr == NULL) {
    xmlSchemaPMissingAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node,
                             "name", NULL);
  } else {
    ...
    if (xmlSchemaCheckCSelectorXPath(ctxt, idc, item, attr, isField) == -1) {
      xmlSchemaPErr(ctxt, (xmlNodePtr)attr, XML_SCHEMAP_INTERNAL,
 ...
 * Returns the parsed identity-constraint definition.
 */
static xmlSchemaIDCPtr xmlSchemaParseIDC(xmlSchemaParserCtxtPtr ctxt,
                                         xmlSchemaPtr schema, xmlNodePtr node,
  ...
  item =
      xmlSchemaAddIDC(ctxt, schema, name, targetNamespace, idcCategory, node);
  if (item == NULL)
    return (NULL);
  ...
  if (idcCategory == XML_SCHEMA_TYPE_IDC_KEYREF) {
    ...
    if (attr == NULL) {
       ...
       */
      item->ref =
          xmlSchemaNewQNameRef(ctxt, XML_SCHEMA_TYPE_IDC_KEY, NULL, NULL);
      if (item->ref == NULL)
        return (NULL);
      xmlSchemaPValAttrNodeQName(ctxt, schema, NULL, attr,
  ...
  if (child == NULL) {
    xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_MISSING, NULL, node, child,
  ...
  if (IS_SCHEMA(child, "selector")) {
    ...
    if (IS_SCHEMA(child, "field")) {
      ...
      xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node,
                           child, NULL, "(annotation?, (selector, field+))");
    }
  }
  if (child != NULL) {
    xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node,
...
xmlSchemaParseElement(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
  ...
  if (isRef) {
     ...
     */
    if (nameAttr != NULL) {
      xmlSchemaPMutualExclAttrErr(ctxt, XML_SCHEMAP_SRC_ELEMENT_2_1, NULL,
                                  nameAttr, "ref", "name");
    }
    /*
     * Check for illegal attributes.
    ...
    while (attr != NULL) {
      if (attr->ns == NULL) {
        if (xmlStrEqual(attr->name, BAD_CAST "ref") ||
          ...
          /* SPEC (3.3.3 : 2.2) */
          xmlSchemaPCustomAttrErr(
              ctxt, XML_SCHEMAP_SRC_ELEMENT_2_2, NULL, NULL, attr,
...
declaration_part : {
  ...
  if (topLevel) {
    ...
    xmlSchemaPValAttrQName(ctxt, schema, NULL, node, "substitutionGroup",
    ...
    if (attr == NULL) {
      ...
      if (xmlSchemaPValAttrBlockFinal(
              attrValue, &(decl->flags), -1, XML_SCHEMAS_ELEM_FINAL_EXTENSION,
  ...
  if (attr == NULL) {
    ...
    attrValue = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr)attr);
    if (xmlSchemaPValAttrBlockFinal(
            attrValue, &(decl->flags), -1, XML_SCHEMAS_ELEM_BLOCK_EXTENSION,
  ...
  if (attr != NULL) {
    xmlSchemaPValAttrNodeQName(ctxt, schema, NULL, attr, &(decl->namedTypeNs),
                               &(decl->namedType));
    xmlSchemaCheckReference(ctxt, schema, node, attr, decl->namedTypeNs);
  }
  ...
  if (attr != NULL) {
    ...
    if (decl->value != NULL) {
       ...
       * default and fixed must not both be present.
       */
      xmlSchemaPMutualExclAttrErr(ctxt, XML_SCHEMAP_SRC_ELEMENT_1, NULL, attr,
                                  "default", "fixed");
    } else {
  ...
  if (IS_SCHEMA(child, "complexType")) {
    ...
    if (decl->namedType != NULL) {
      xmlSchemaPContentErr(
          ctxt, XML_SCHEMAP_SRC_ELEMENT_3, NULL, node, child,
          "The attribute 'type' and the <complexType> child are "
          "mutually exclusive",
          NULL);
    } else
      WXS_ELEM_TYPEDEF(decl) =
          xmlSchemaParseComplexType(ctxt, schema, child, 0);
    child = child->next;
  ...
  while ((IS_SCHEMA(child, "unique")) || (IS_SCHEMA(child, "key")) ||
         (IS_SCHEMA(child, "keyref"))) {
    if (IS_SCHEMA(child, "unique")) {
      curIDC =
          xmlSchemaParseIDC(ctxt, schema, child, XML_SCHEMA_TYPE_IDC_UNIQUE,
                            decl->targetNamespace);
    } else if (IS_SCHEMA(child, "key")) {
      curIDC = xmlSchemaParseIDC(ctxt, schema, child, XML_SCHEMA_TYPE_IDC_KEY,
                                 decl->targetNamespace);
    } else if (IS_SCHEMA(child, "keyref")) {
      curIDC =
          xmlSchemaParseIDC(ctxt, schema, child, XML_SCHEMA_TYPE_IDC_KEYREF,
                            decl->targetNamespace);
    }
    if (lastIDC != NULL)
      lastIDC->next = curIDC;
 ...
 */
static int xmlSchemaParseUnion(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
                               xmlNodePtr node) {
  xmlSchemaTypePtr type;
  xmlNodePtr child = NULL;
  xmlAttrPtr attr;
  ...
  if (attr != NULL) {
    ...
    do {
      ...
      if (xmlSchemaPValAttrNodeQNameValue(ctxt, schema, NULL, attr,
                                          BAD_CAST tmp, &nsName,
                                          &localName) == 0) {
        /*
         ...
         * Create a reference item.
         */
        ref = xmlSchemaNewQNameRef(ctxt, XML_SCHEMA_TYPE_SIMPLE, localName,
                                   nsName);
        if (ref == NULL) {
          FREE_AND_NULL(tmp)
          return (-1);
  ...
  if (IS_SCHEMA(child, "simpleType")) {
    ...
    while (IS_SCHEMA(child, "simpleType")) {
      subtype =
          (xmlSchemaTypePtr)xmlSchemaParseSimpleType(ctxt, schema, child, 0);
      if (subtype != NULL) {
        if (last == NULL) {
  ...
  if ((attr == NULL) && (type->subtypes == NULL)) {
    ...
    xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_SRC_UNION_MEMBERTYPES_OR_SIMPLETYPES,
                        ...
                        "at least one <simpleType> child must be present",
                        NULL);
  }
  return (0);
}
...
static xmlSchemaTypePtr xmlSchemaParseList(xmlSchemaParserCtxtPtr ctxt,
   ...
   * the "ref" fields.
   */
  xmlSchemaPValAttrQName(ctxt, schema, NULL, node, "itemType", &(type->baseNs),
                         &(type->base));
  /*
   * And now for the children...
   */
  ...
  if (IS_SCHEMA(child, "simpleType")) {
     ...
     */
    if (type->base != NULL) {
      xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_SRC_SIMPLE_TYPE_1, NULL, node,
    ...
    xmlSchemaPCustomErr(
        ctxt, XML_SCHEMAP_SRC_SIMPLE_TYPE_1, NULL, node,
  ...
  if ((type->base == NULL) && (type->subtypes == NULL) &&
...
static xmlSchemaTypePtr xmlSchemaParseSimpleType(xmlSchemaParserCtxtPtr ctxt,
  ...
  if (topLevel == 0) {
...
#else
    type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_SIMPLE, NULL,
                            ctxt->targetNamespace, node, 0);
#endif
    if (type == NULL)
      return (NULL);
     ...
     */
    type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_SIMPLE, attrValue,
    ...
    if (attr == NULL) {
      ...
      if (xmlSchemaPValAttrBlockFinal(attrValue, &(type->flags), -1, -1,
                                      XML_SCHEMAS_TYPE_FINAL_RESTRICTION, -1,
                                      XML_SCHEMAS_TYPE_FINAL_LIST,
                                      XML_SCHEMAS_TYPE_FINAL_UNION) != 0) {

        xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE,
                                WXS_BASIC_CAST type, (xmlNodePtr)attr, NULL,
  ...
  if (child == NULL) {
    xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_MISSING, NULL, node, child,
                         NULL, "(annotation?, (restriction | list | union))");
  ...
  if (topLevel && ctxt->isRedefine && (!hasRestriction)) {
    xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_SRC_REDEFINE, NULL, node,
                        ...
                        "<simpleType> must have a <restriction> child",
                        NULL);
  }
 ...
 */
static xmlSchemaTreeItemPtr
xmlSchemaParseModelGroupDefRef(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
                               xmlNodePtr node) {
  ...
  if (attr == NULL) {
    xmlSchemaPMissingAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node,
                             "ref", NULL);
    return (NULL);
  } else if (xmlSchemaPValAttrNodeQName(ctxt, schema, NULL, attr, &refNs,
                                        &ref) != 0) {
    return (NULL);
  }
  xmlSchemaCheckReference(ctxt, schema, node, attr, refNs);
   ...
   */
  item->children = (xmlSchemaTreeItemPtr)xmlSchemaNewQNameRef(
      ctxt, XML_SCHEMA_TYPE_GROUP, ref, refNs);
...
xmlSchemaParseModelGroupDefinition(xmlSchemaParserCtxtPtr ctxt,
  ...
  if (IS_SCHEMA(child, "all")) {
    item->children =
        xmlSchemaParseModelGroup(ctxt, schema, child, XML_SCHEMA_TYPE_ALL, 0);
    child = child->next;
 ...
 */
static void xmlSchemaCleanupDoc(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr root) {
...
static int xmlSchemaParseSchemaElement(xmlSchemaParserCtxtPtr ctxt,
                                       xmlSchemaPtr schema, xmlNodePtr node) {
  xmlAttrPtr attr;
  const xmlChar *val;
  ...
  */
  ...
  if (attr != NULL) {
    res = xmlSchemaPValAttrNode(
        ctxt, NULL, attr, xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI), NULL);
    HFAILURE;
    if (res != 0) {
  ...
  if (attr != NULL) {
    ...
    if (res != 0) {
      xmlSchemaPSimpleTypeErr(
          ctxt, XML_SCHEMAP_ELEMFORMDEFAULT_VALUE, NULL, (xmlNodePtr)attr, NULL,
  ...
  if (attr != NULL) {
    ...
    if (res != 0) {
      xmlSchemaPSimpleTypeErr(
          ctxt, XML_SCHEMAP_ATTRFORMDEFAULT_VALUE, NULL, (xmlNodePtr)attr, NULL,
          "(qualified | unqualified)", val, NULL, NULL, NULL);
    }
  }
  ...
  if (attr != NULL) {
    ...
    res = xmlSchemaPValAttrBlockFinal(
        val, &(schema->flags), -1, XML_SCHEMAS_FINAL_DEFAULT_EXTENSION,
        XML_SCHEMAS_FINAL_DEFAULT_RESTRICTION, -1,
        XML_SCHEMAS_FINAL_DEFAULT_LIST, XML_SCHEMAS_FINAL_DEFAULT_UNION);
    HFAILURE;
    if (res != 0) {
      xmlSchemaPSimpleTypeErr(
          ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNodePtr)attr,
          NULL, "(#all | List of (extension | restriction | list | union))",
          val, NULL, NULL, NULL);
    }
  }
  ...
  if (attr != NULL) {
    val = xmlSchemaGetNodeContent(ctxt, (xmlNodePtr)attr);
    res = xmlSchemaPValAttrBlockFinal(
        val, &(schema->flags), -1, XML_SCHEMAS_BLOCK_DEFAULT_EXTENSION,
    ...
    if (res != 0) {
      xmlSchemaPSimpleTypeErr(
          ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNodePtr)attr,
          NULL, "(#all | List of (extension | restriction | substitution))",
          val, NULL, NULL, NULL);
    }
  }
 ...
 */
static int xmlSchemaParseSchemaTopLevel(xmlSchemaParserCtxtPtr ctxt,
  ...
  while (child != NULL) {
    if (IS_SCHEMA(child, "complexType")) {
      ...
      xmlSchemaPContentErr(
          ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, child->parent, child,
          NULL,
...
static xmlSchemaSchemaRelationPtr xmlSchemaSchemaRelationCreate(void) {
  xmlSchemaSchemaRelationPtr ret;

  ret = (xmlSchemaSchemaRelationPtr)xmlMalloc(sizeof(xmlSchemaSchemaRelation));
  if (ret == NULL) {
...
xmlSchemaConstructionCtxtCreate(xmlDictPtr dict) {
  ...
  ret = (xmlSchemaConstructionCtxtPtr)xmlMalloc(
  ...
  if (ret->pending == NULL) {
    xmlSchemaPErrMemory(NULL, "allocating list of pending global components",
                        NULL);
...
static xmlSchemaParserCtxtPtr xmlSchemaParserCtxtCreate(void) {
 ...
 */
static xmlSchemaParserCtxtPtr xmlSchemaNewParserCtxtUseDict(const char *URL,
                                                            xmlDictPtr dict) {
  xmlSchemaParserCtxtPtr ret;
...
static int xmlSchemaCreatePCtxtOnVCtxt(xmlSchemaValidCtxtPtr vctxt) {
  if (vctxt->pctxt == NULL) {
    ...
    /* TODO: Pass user data. */
    xmlSchemaSetParserErrors(vctxt->pctxt, vctxt->error, vctxt->warning,
                             vctxt->errCtxt);
...
xmlSchemaGetSchemaBucketByTNS(xmlSchemaParserCtxtPtr pctxt,
                              const xmlChar *targetNamespace, int imported) {
  xmlSchemaBucketPtr cur;
  xmlSchemaItemListPtr list;
  ...
  if (list->nbItems == 0)
    ...
    for (i = 0; i < list->nbItems; i++) {
      ...
      if ((!IS_BAD_SCHEMA_DOC(cur)) &&
          ...
          ((imported && cur->imported) || ((!imported) && (!cur->imported))))
        return (cur);
    }
  }
  return (NULL);
...
static int xmlSchemaParseNewDocWithContext(xmlSchemaParserCtxtPtr pctxt,
                                           xmlSchemaPtr schema,
                                           xmlSchemaBucketPtr bucket) {
  int oldFlags;
  xmlDocPtr oldDoc;
...
static int xmlSchemaParseNewDoc(xmlSchemaParserCtxtPtr pctxt,
                                xmlSchemaPtr schema,
                                xmlSchemaBucketPtr bucket) {
  ...
  newpctxt = xmlSchemaNewParserCtxtUseDict((const char *)bucket->schemaLocation,
                                           pctxt->dict);
  if (newpctxt == NULL)
    return (-1);
...
static void xmlSchemaSchemaRelationAddChild(xmlSchemaBucketPtr bucket,
                                            xmlSchemaSchemaRelationPtr rel) {
...
static const xmlChar *xmlSchemaBuildAbsoluteURI(xmlDictPtr dict,
...
static int xmlSchemaAddSchemaDoc(xmlSchemaParserCtxtPtr pctxt,
  ...
  if (schemaLocation != NULL) {
    ...
    if ((bkt != NULL) && (pctxt->constructor->bucket == bkt)) {
      ...
      xmlSchemaCustomErr(ACTXT_CAST pctxt, err, invokingNode, NULL,
  ...
  if (WXS_IS_BUCKET_IMPMAIN(type)) {
    ...
    if (bkt != NULL) {
      ...
      if (bkt->schemaLocation == NULL) {
        ...
        if (!xmlStrEqual(schemaLocation, bkt->schemaLocation)) {
          ...
          xmlSchemaCustomWarning(
              ACTXT_CAST pctxt, XML_SCHEMAP_WARN_SKIP_SCHEMA, invokingNode,
              NULL,
    ...
    if (bkt != NULL) {
      ...
      if ((bkt->origTargetNamespace == NULL) &&
         ...
         *   Answer: after!
         */
        chamel = xmlSchemaGetChameleonSchemaBucket(pctxt, schemaLocation,
...
doc_load:
  ...
  if (schemaDoc != NULL) {
    ...
    if (schemaLocation != NULL) {
      /* Parse from file. */
      doc = xmlCtxtReadFile(parserCtxt, (const char *)schemaLocation, NULL,
                            SCHEMAS_PARSE_OPTIONS);
    } else if (schemaBuffer != NULL) {
      /* Parse from memory buffer. */
      doc = xmlCtxtReadMemory(parserCtxt, schemaBuffer, schemaBufferLen, NULL,
    ...
    if (doc == NULL) {
      ...
      if ((lerr == NULL) || (lerr->domain != XML_FROM_IO)) {
        ...
        xmlSchemaCustomErr(ACTXT_CAST pctxt, res, invokingNode, NULL,
  ...
  if (doc != NULL) {
    ...
    if (docElem == NULL) {
      xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_NOROOT, invokingNode,
                         NULL, "The document '%s' has no document element",
                         schemaLocation, NULL);
      goto exit_error;
    }
    /*
    ...
    if (!IS_SCHEMA(docElem, "schema")) {
      xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_NOT_SCHEMA, invokingNode,
                         NULL, "The XML document '%s' is not a schema document",
                         schemaLocation, NULL);
      goto exit_error;
    }
    /*
 ...
 */
static int xmlSchemaParseImport(xmlSchemaParserCtxtPtr pctxt,
                                xmlSchemaPtr schema, xmlNodePtr node) {
  xmlNodePtr child;
   ...
   */
  if (xmlSchemaPValAttr(pctxt, NULL, node, "namespace",
                        xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI),
                        &namespaceName) != 0) {
    xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL,
                            node, xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI),
                            NULL, namespaceName, NULL, NULL, NULL);
    return (pctxt->err);
  }

  if (xmlSchemaPValAttr(pctxt, NULL, node, "schemaLocation",
                        xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI),
                        &schemaLocation) != 0) {
    xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL,
                            node, xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI),
  ...
  if (namespaceName != NULL) {
    ...
    if (xmlStrEqual(thisTargetNamespace, namespaceName)) {
      xmlSchemaPCustomErr(
          pctxt, XML_SCHEMAP_SRC_IMPORT_1_1, NULL, node,
    ...
    if (thisTargetNamespace == NULL) {
      xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_SRC_IMPORT_1_2, NULL, node,
  ...
  ret = xmlSchemaAddSchemaDoc(pctxt, XML_SCHEMA_SCHEMA_IMPORT, schemaLocation,
                              NULL, NULL, 0, node, thisTargetNamespace,
  ...
  if ((bucket == NULL) && (schemaLocation != NULL)) {
    xmlSchemaCustomWarning(ACTXT_CAST pctxt, XML_SCHEMAP_WARN_UNLOCATED_SCHEMA,
                           node, NULL,
                           "Failed to locate a schema at location '%s'. "
                           "Skipping the import",
                           schemaLocation, NULL, NULL);
  }
...
static int xmlSchemaParseIncludeOrRedefineAttrs(xmlSchemaParserCtxtPtr pctxt,
                                                xmlSchemaPtr schema,
                                                xmlNodePtr node,
  ...
  *schemaLocation = NULL;
  ...
  if (xmlStrEqual(*schemaLocation, pctxt->URL)) {
    if (type == XML_SCHEMA_SCHEMA_REDEFINE) {
      xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_SRC_REDEFINE, NULL, node,
                          "The schema document '%s' cannot redefine itself.",
                          *schemaLocation);
    } else {
      xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_SRC_INCLUDE, NULL, node,
...
static int xmlSchemaParseIncludeOrRedefine(xmlSchemaParserCtxtPtr pctxt,
                                           xmlSchemaPtr schema, xmlNodePtr node,
                                           int type) {
  xmlNodePtr child = NULL;
  ...
  res = xmlSchemaParseIncludeOrRedefineAttrs(
      pctxt, schema, node, (xmlChar **)(&schemaLocation), type);
  if (res != 0)
    return (res);
  /*
   * Load and add the schema document.
   */
  res = xmlSchemaAddSchemaDoc(pctxt, type, schemaLocation, NULL, NULL, 0, node,
                              pctxt->targetNamespace, NULL, &bucket);
  if (res != 0)
    return (res);
  /*
  ...
  if ((bucket == NULL) || (bucket->doc == NULL)) {
    if (type == XML_SCHEMA_SCHEMA_INCLUDE) {
      ...
      xmlSchemaCustomErr(ACTXT_CAST pctxt, res, node, NULL,
    ...
    if (bucket->origTargetNamespace != NULL) {
      ...
      if (pctxt->targetNamespace == NULL) {
        xmlSchemaCustomErr(
            ACTXT_CAST pctxt, XML_SCHEMAP_SRC_INCLUDE, node, NULL,
        ...
        xmlSchemaPCustomErrExt(
            pctxt, XML_SCHEMAP_SRC_INCLUDE, NULL, node,
       ...
       */
      isChameleon = 1;
      if (bucket->parsed && bucket->origTargetNamespace != NULL) {
        xmlSchemaCustomErr(
            ACTXT_CAST pctxt, XML_SCHEMAP_SRC_INCLUDE, node, NULL,
            "The target namespace of the included/redefined schema "
  ...
  if (child != NULL) {
    ...
    if (type == XML_SCHEMA_SCHEMA_REDEFINE) {
      xmlSchemaPContentErr(pctxt, res, NULL, node, child, NULL,
...
static int xmlSchemaParseRedefine(xmlSchemaParserCtxtPtr pctxt,
                                  xmlSchemaPtr schema, xmlNodePtr node) {
  int res;
...
static int xmlSchemaParseInclude(xmlSchemaParserCtxtPtr pctxt,
                                 xmlSchemaPtr schema, xmlNodePtr node) {
  int res;
...
xmlSchemaParseModelGroup(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
  ...
  if (type == XML_SCHEMA_TYPE_ALL) {
    ...
    while (IS_SCHEMA(child, "element")) {
      part = (xmlSchemaParticlePtr)xmlSchemaParseElement(ctxt, schema, child,
                                                         &isElemRef, 0);
      /*
      ...
      if (part != NULL) {
        if (isElemRef)
          hasRefs++;
        if (part->minOccurs > 1) {
          xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_COS_ALL_LIMITED, NULL, child,
    ...
    while ((IS_SCHEMA(child, "element")) || (IS_SCHEMA(child, "group")) ||
           (IS_SCHEMA(child, "any")) || (IS_SCHEMA(child, "choice")) ||
           (IS_SCHEMA(child, "sequence"))) {

      if (IS_SCHEMA(child, "element")) {
        part = (xmlSchemaTreeItemPtr)xmlSchemaParseElement(ctxt, schema, child,
                                                           &isElemRef, 0);
        if (part && isElemRef)
          hasRefs++;
      } else if (IS_SCHEMA(child, "group")) {
        ...
        if (ctxt->isRedefine && ctxt->redef &&
            (ctxt->redef->item->type == XML_SCHEMA_TYPE_GROUP) && part &&
            part->children) {
          if ((xmlSchemaGetQNameRefName(part->children) ==
            ...
            if (ctxt->redefCounter != 0) {
              ...
              xmlSchemaCustomErr(
                  ACTXT_CAST ctxt, XML_SCHEMAP_SRC_REDEFINE, child, NULL,
                  "The redefining model group definition "
                  "'%s' must not contain more than one "
                  "reference to the redefined definition",
                  xmlSchemaFormatQName(&str, ctxt->redef->refTargetNs,
                                       ctxt->redef->refName),
                  NULL);
              FREE_AND_NULL(str)
              part = NULL;
        ...
        part = (xmlSchemaTreeItemPtr)xmlSchemaParseAny(ctxt, schema, child);
      } else if (IS_SCHEMA(child, "choice")) {
    ...
    if (child != NULL) {
      xmlSchemaPContentErr(
          ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL,
...
xmlSchemaParseRestriction(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
   ...
   */
  if (xmlSchemaPValAttrQName(ctxt, schema, NULL, node, "base", &(type->baseNs),
                             &(type->base)) == 0) {
    if ((type->base == NULL) && (type->type == XML_SCHEMA_TYPE_COMPLEX)) {
      ...
      if (type->base == NULL) {
         ...
         */
        xmlSchemaPCustomErrExt(
            ctxt, XML_SCHEMAP_SRC_REDEFINE, NULL, node,
            ...
            xmlSchemaFormatQName(&str2, type->targetNamespace, type->name),
            NULL);
        FREE_AND_NULL(str1);
        FREE_AND_NULL(str2);
  ...
  if (parentType == XML_SCHEMA_TYPE_SIMPLE) {
    ...
    if (IS_SCHEMA(child, "simpleType")) {
      if (type->base != NULL) {
         ...
         */
        xmlSchemaPContentErr(
            ctxt, XML_SCHEMAP_SRC_RESTRICTION_BASE_OR_SIMPLETYPE, NULL, node,
            child,
     ...
     */
    if (IS_SCHEMA(child, "all")) {
      type->subtypes = (xmlSchemaTypePtr)xmlSchemaParseModelGroup(
          ctxt, schema, child, XML_SCHEMA_TYPE_ALL, 1);
      child = child->next;
    } else if (IS_SCHEMA(child, "choice")) {
      type->subtypes = (xmlSchemaTypePtr)xmlSchemaParseModelGroup(
          ctxt, schema, child, XML_SCHEMA_TYPE_CHOICE, 1);
      child = child->next;
    } else if (IS_SCHEMA(child, "sequence")) {
      type->subtypes = (xmlSchemaTypePtr)xmlSchemaParseModelGroup(
          ctxt, schema, child, XML_SCHEMA_TYPE_SEQUENCE, 1);
      child = child->next;
      /*
      ...
      type->subtypes =
          (xmlSchemaTypePtr)xmlSchemaParseModelGroupDefRef(ctxt, schema, child);
      /*
  ...
  if ((parentType == XML_SCHEMA_TYPE_SIMPLE) ||
    ...
    while ((IS_SCHEMA(child, "minInclusive")) ||
           ...
           (IS_SCHEMA(child, "pattern")) || (IS_SCHEMA(child, "enumeration")) ||
           (IS_SCHEMA(child, "whiteSpace")) || (IS_SCHEMA(child, "length")) ||
           (IS_SCHEMA(child, "maxLength")) || (IS_SCHEMA(child, "minLength"))) {
    ...
    if (type->facets != NULL) {
      ...
      do {
        facetLink =
            (xmlSchemaFacetLinkPtr)xmlMalloc(sizeof(xmlSchemaFacetLink));
        if (facetLink == NULL) {
 ...
 */
static xmlSchemaTypePtr xmlSchemaParseExtension(xmlSchemaParserCtxtPtr ctxt,
                                                xmlSchemaPtr schema,
                                                xmlNodePtr node,
                                                xmlSchemaTypeType parentType) {
  xmlSchemaTypePtr type;
 ...
 * Returns the type definition or NULL in case of error
 */
static int xmlSchemaParseSimpleContent(xmlSchemaParserCtxtPtr ctxt,
                                       xmlSchemaPtr schema, xmlNodePtr node,
  ...
  *hasRestrictionOrExtension = 0;
  ...
  if (child == NULL) {
    xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_MISSING, NULL, node, NULL,
 ...
 * Returns the type definition or NULL in case of error
 */
static int xmlSchemaParseComplexContent(xmlSchemaParserCtxtPtr ctxt,
                                        xmlSchemaPtr schema, xmlNodePtr node,
                                        int *hasRestrictionOrExtension) {
  xmlSchemaTypePtr type;
  xmlNodePtr child = NULL;
...
static xmlSchemaTypePtr xmlSchemaParseComplexType(xmlSchemaParserCtxtPtr ctxt,
  ...
  if (topLevel == 0) {
    ...
    type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_COMPLEX,
                            xmlDictLookup(ctxt->dict, (const xmlChar *)buf, -1),
                            ctxt->targetNamespace, node, 0);
#else
    type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_COMPLEX, NULL,
                            ctxt->targetNamespace, node, 0);
#endif
    if (type == NULL)
      return (NULL);
     ...
     */
    type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_COMPLEX, name,
                            ctxt->targetNamespace, node, 1);
    if (type == NULL)
      return (NULL);
    type->node = node;
  ...
  while (attr != NULL) {
    if (attr->ns == NULL) {
      if (xmlStrEqual(attr->name, BAD_CAST "id")) {
         ...
         * Attribute "mixed".
         */
        if (xmlSchemaPGetBoolNodeValue(ctxt, NULL, (xmlNodePtr)attr))
          type->flags |= XML_SCHEMAS_TYPE_MIXED;
      } else if (topLevel) {
        /*
        ...
        if (xmlStrEqual(attr->name, BAD_CAST "name")) {
          ...
          if (xmlSchemaPValAttrBlockFinal(attrValue, &(type->flags), -1,
                                          ...
                                          -1, -1, -1) != 0) {
            xmlSchemaPSimpleTypeErr(
                ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL,
                (xmlNodePtr)attr, NULL,
                "(#all | List of (extension | restriction))", attrValue, NULL,
                NULL, NULL);
          } else
            final = 1;
...
xmlSchemaBuildContentModelForSubstGroup(xmlSchemaParserCtxtPtr pctxt,
                                        xmlSchemaParticlePtr particle,
                                        int counter, xmlAutomataStatePtr end) {
  ...
  if (counter >= 0) {
    ...
    xmlAutomataNewTransition2(pctxt->am, tmp, end, elemDecl->name,
                              elemDecl->targetNamespace, elemDecl);
    /*
    ...
    xmlAutomataNewEpsilon(
        pctxt->am,
        xmlAutomataNewTransition2(pctxt->am, start, NULL, elemDecl->name,
                                  elemDecl->targetNamespace, elemDecl),
        end);
    /*
     * Add subst. group members.
    ...
    for (i = 0; i < substGroup->members->nbItems; i++) {
       ...
       *		       1, 1, member);
       */
      tmp = xmlAutomataNewTransition2(pctxt->am, start, NULL, member->name,
                                      member->targetNamespace, member);
    ...
    for (i = 0; i < substGroup->members->nbItems; i++) {
      ...
      xmlAutomataNewEpsilon(
          pctxt->am,
          xmlAutomataNewTransition2(pctxt->am, start, NULL, member->name,
                                    member->targetNamespace, member),
          hop);
    }
...
static int xmlSchemaBuildContentModelForElement(xmlSchemaParserCtxtPtr ctxt,
  ...
  if (((xmlSchemaElementPtr)particle->children)->flags &
    ...
    if (particle->maxOccurs == 1) {
      start = ctxt->state;
      ctxt->state =
          xmlAutomataNewTransition2(ctxt->am, start, NULL, elemDecl->name,
                                    elemDecl->targetNamespace, elemDecl);
      ...
      ctxt->state = xmlAutomataNewTransition2(
          ctxt->am, ctxt->state, ctxt->state, elemDecl->name,
          elemDecl->targetNamespace, elemDecl);
    } else {
      int counter;
      ...
      ctxt->state =
          xmlAutomataNewCounterTrans(ctxt->am, ctxt->state, NULL, counter);
    }
    if (particle->minOccurs == 0) {
 ...
 */
static int xmlSchemaBuildAContentModel(xmlSchemaParserCtxtPtr pctxt,
                                       xmlSchemaParticlePtr particle) {
  int ret = 0, tmp2;
    ...
    if (particle->maxOccurs == 1) {
      if (wild->any == 1) {
        ...
        pctxt->state = xmlAutomataNewTransition2(
            pctxt->am, start, NULL, BAD_CAST "*", BAD_CAST "*", wild);
        ...
        do {
          pctxt->state = start;
          pctxt->state = xmlAutomataNewTransition2(
              pctxt->am, pctxt->state, NULL, BAD_CAST "*", ns->value, wild);
          xmlAutomataNewEpsilon(pctxt->am, pctxt->state, end);
          ns = ns->next;
        } while (ns != NULL);

      } else if (wild->negNsSet != NULL) {
        pctxt->state = xmlAutomataNewNegTrans(
            pctxt->am, start, end, BAD_CAST "*", wild->negNsSet->value, wild);
      }
    } else {
      int counter;
      xmlAutomataStatePtr hop;
      ...
      if (wild->any == 1) {
        ...
        do {
          pctxt->state = xmlAutomataNewTransition2(
              pctxt->am, start, NULL, BAD_CAST "*", ns->value, wild);
          xmlAutomataNewEpsilon(pctxt->am, pctxt->state, hop);
          ns = ns->next;
        } while (ns != NULL);
        ...
        pctxt->state = xmlAutomataNewNegTrans(
            pctxt->am, start, hop, BAD_CAST "*", wild->negNsSet->value, wild);
      }
      xmlAutomataNewCountedTrans(pctxt->am, hop, start, counter);
      xmlAutomataNewCounterTrans(pctxt->am, hop, end, counter);
    }
    ...
    if ((particle->minOccurs == 1) && (particle->maxOccurs == 1)) {
      ...
      if (particle->maxOccurs >= UNBOUNDED) {
        if (particle->minOccurs > 1) {
          ...
          counter = xmlAutomataNewCounter(pctxt->am, particle->minOccurs - 1,
                                          UNBOUNDED);

          sub = particle->children->children;
          while (sub != NULL) {
            tmp2 =
                xmlSchemaBuildAContentModel(pctxt, (xmlSchemaParticlePtr)sub);
            if (tmp2 != 1)
              ret = 0;
            sub = sub->next;
          }
          ...
          pctxt->state =
              xmlAutomataNewCounterTrans(pctxt->am, tmp, NULL, counter);
          if (ret == 1)
  ...
  case XML_SCHEMA_TYPE_ALL: {
    ...
    while (sub != NULL) {
      ...
      if (elemDecl->flags & XML_SCHEMAS_ELEM_SUBST_GROUP_HEAD) {
         ...
         */
        counter =
            xmlAutomataNewCounter(pctxt->am, sub->minOccurs, sub->maxOccurs);
        xmlSchemaBuildContentModelForSubstGroup(pctxt, sub, counter,
                                                pctxt->state);
      } else {
        if ((sub->minOccurs == 1) && (sub->maxOccurs == 1)) {
          xmlAutomataNewOnceTrans2(pctxt->am, pctxt->state, pctxt->state,
                                   elemDecl->name, elemDecl->targetNamespace, 1,
                                   1, elemDecl);
        } else if ((sub->minOccurs == 0) && (sub->maxOccurs == 1)) {

          xmlAutomataNewCountTrans2(pctxt->am, pctxt->state, pctxt->state,
  ...
  case XML_SCHEMA_TYPE_GROUP:
    ...
    break;
  default:
    xmlSchemaInternalErr2(ACTXT_CAST pctxt, "xmlSchemaBuildAContentModel",
 ...
 */
static void xmlSchemaBuildContentModel(xmlSchemaTypePtr type,
                                       xmlSchemaParserCtxtPtr ctxt) {
  if ((type->type != XML_SCHEMA_TYPE_COMPLEX) || (type->contModel != NULL) ||
      ((type->contentType != XML_SCHEMA_CONTENT_ELEMENTS) &&
       (type->contentType != XML_SCHEMA_CONTENT_MIXED)))
    return;

#ifdef DEBUG_CONTENT
  xmlGenericError(xmlGenericErrorContext, "Building content model for %s\n",
                  name);
#endif
  ctxt->am = NULL;
  ...
  if (type->contModel == NULL) {
    xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_INTERNAL, WXS_BASIC_CAST type,
                        type->node, "Failed to compile the content model",
                        NULL);
    ...
    xmlGenericError(xmlGenericErrorContext, "Content model of %s:\n",
                    type->name);
 ...
 * term.
 */
static void xmlSchemaResolveElementReferences(xmlSchemaElementPtr elemDecl,
                                              xmlSchemaParserCtxtPtr ctxt) {
  ...
  if ((elemDecl->subtypes == NULL) && (elemDecl->namedType != NULL)) {
    ...
    if (type == NULL) {
      xmlSchemaPResCompAttrErr(ctxt, XML_SCHEMAP_SRC_RESOLVE,
                               WXS_BASIC_CAST elemDecl, elemDecl->node, "type",
                               elemDecl->namedType, elemDecl->namedTypeNs,
  ...
  if (elemDecl->substGroup != NULL) {
    ...
    if (substHead == NULL) {
      xmlSchemaPResCompAttrErr(
          ctxt, XML_SCHEMAP_SRC_RESOLVE, WXS_BASIC_CAST elemDecl, NULL,
          "substitutionGroup", elemDecl->substGroup, elemDecl->substGroupNs,
 ...
 */
static int xmlSchemaIsDerivedFromBuiltInType(xmlSchemaTypePtr type,
                                             int valType) {
  if (type == NULL)
    return (0);
 ...
 */
static int xmlSchemaUnionWildcards(xmlSchemaParserCtxtPtr ctxt,
   ...
   */
  if (((completeWild->negNsSet != NULL) &&
       (completeWild->negNsSet->value != NULL) && (curWild->nsSet != NULL)) ||
      ((curWild->negNsSet != NULL) && (curWild->negNsSet->value != NULL) &&
       (completeWild->nsSet != NULL))) {
    ...
    if (nsFound && absentFound) {
       ...
       */
      xmlSchemaPErr(ctxt, completeWild->node, XML_SCHEMAP_UNION_NOT_EXPRESSIBLE,
 ...
 * internal error, 0 otherwise.
 */
static int xmlSchemaIntersectWildcards(xmlSchemaParserCtxtPtr ctxt,
                                       xmlSchemaWildcardPtr completeWild,
                                       xmlSchemaWildcardPtr curWild) {
  ...
  if ((completeWild->negNsSet != NULL) && (curWild->negNsSet != NULL) &&
    ...
    xmlSchemaPErr(
        ctxt, completeWild->node, XML_SCHEMAP_INTERSECTION_NOT_EXPRESSIBLE,
...
static int xmlSchemaCheckCOSNSSubset(xmlSchemaWildcardPtr sub,
   ...
   */
  if ((sub->negNsSet != NULL) && (super->negNsSet != NULL) &&
 ...
 */
static int xmlSchemaCheckCVCWildcardNamespace(xmlSchemaWildcardPtr wild,
                                              const xmlChar *ns) {
  if (wild == NULL)
    return (-1);
...
#define WXS_ACTION_STR(a)                                                      \
  ((a) == XML_SCHEMA_ACTION_DERIVE) ? (const xmlChar *)"base"                  \
                                    : (const xmlChar *)"redefined"
 ...
 *
 */
static int xmlSchemaCheckDerivationOKRestriction2to4(
    xmlSchemaParserCtxtPtr pctxt, int action, xmlSchemaBasicItemPtr item,
    xmlSchemaBasicItemPtr baseItem, xmlSchemaItemListPtr uses,
    xmlSchemaItemListPtr baseUses, xmlSchemaWildcardPtr wild,
  ...
  if (uses != NULL) {
        ...
        if ((WXS_ATTRUSE_DECL_NAME(cur) == WXS_ATTRUSE_DECL_NAME(bcur)) &&
          ...
          if ((cur->occurs == XML_SCHEMAS_ATTR_USE_OPTIONAL) &&
            ...
            xmlSchemaPAttrUseErr4(
                pctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_2_1_1,
                ...
                xmlSchemaGetComponentDesignation(&str, baseItem), NULL, NULL);
            FREE_AND_NULL(str);
            /* err = pctxt->err; */
          } else if (xmlSchemaCheckCOSSTDerivedOK(
                         ACTXT_CAST pctxt, WXS_ATTRUSE_TYPEDEF(cur),
             ...
             */
            xmlSchemaPAttrUseErr4(
                pctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_2_1_2,
                WXS_ITEM_NODE(item), item, cur,
             ...
             */
            xmlSchemaGetEffectiveValueConstraint(bcur, &effFixed, &bEffValue,
                                                 NULL);
            /*
             ...
             * �absent� or default.
             */
            if ((bEffValue != NULL) && (effFixed == 1)) {
              const xmlChar *rEffValue = NULL;

              xmlSchemaGetEffectiveValueConstraint(bcur, &effFixed, &rEffValue,
                                                   NULL);
              /*
              ...
              if ((effFixed == 0) ||
                ...
                xmlSchemaPAttrUseErr4(
                    pctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_2_1_3,
                    WXS_ITEM_NODE(item), item, cur,
                    "The effective value constraint of the "
                    "attribute use is inconsistent with "
                    "its correspondent in the %s %s",
                    WXS_ACTION_STR(action),
                    xmlSchemaGetComponentDesignation(&str, baseItem), NULL,
                    NULL);
                FREE_AND_NULL(str);
                /* err = pctxt->err; */
              }
    ...
    not_found:
      if (!found) {
        ...
        if ((baseWild == NULL) ||
            (xmlSchemaCheckCVCWildcardNamespace(
                 baseWild, (WXS_ATTRUSE_DECL(cur))->targetNamespace) != 0)) {
          xmlChar *str = NULL;

          xmlSchemaPAttrUseErr4(
              pctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_2_2,
              WXS_ITEM_NODE(item), item, cur,
    ...
    for (j = 0; j < baseUses->nbItems; j++) {
      ...
      if (!found) {
        ...
        xmlSchemaCustomErr4(ACTXT_CAST pctxt,
                            XML_SCHEMAP_DERIVATION_OK_RESTRICTION_3, NULL, item,
  ...
  if (wild != NULL) {
    ...
    if (baseWild == NULL) {
       ...
       */
      xmlSchemaCustomErr4(ACTXT_CAST pctxt,
                          XML_SCHEMAP_DERIVATION_OK_RESTRICTION_4_1, NULL, item,
                          "The %s has an attribute wildcard, "
                          "but the %s %s '%s' does not have one",
                          WXS_ITEM_TYPE_NAME(item), WXS_ACTION_STR(action),
       ...
       */
      xmlSchemaCustomErr4(ACTXT_CAST pctxt,
                          XML_SCHEMAP_DERIVATION_OK_RESTRICTION_4_2, NULL, item,
                          "The attribute wildcard is not a valid "
                          "subset of the wildcard in the %s %s '%s'",
                          WXS_ACTION_STR(action), WXS_ITEM_TYPE_NAME(baseItem),
                          xmlSchemaGetComponentQName(&str, baseItem), NULL);
      FREE_AND_NULL(str);
      return (pctxt->err);
    }
    ...
    if ((!WXS_IS_ANYTYPE(baseItem)) &&
      ...
      xmlChar *str = NULL;
      xmlSchemaCustomErr4(ACTXT_CAST pctxt,
                          XML_SCHEMAP_DERIVATION_OK_RESTRICTION_4_3, NULL,
                          baseItem,
...
static int xmlSchemaExpandAttributeGroupRefs(xmlSchemaParserCtxtPtr pctxt,
                                             xmlSchemaBasicItemPtr item,
                                             xmlSchemaWildcardPtr *completeWild,
                                             xmlSchemaItemListPtr list,
 ...
 */
static int xmlSchemaFixupTypeAttributeUses(xmlSchemaParserCtxtPtr pctxt,
                                           xmlSchemaTypePtr type) {
  ...
  if (uses != NULL) {
    if (WXS_IS_RESTRICTION(type)) {
       ...
       */
      if (xmlSchemaExpandAttributeGroupRefs(pctxt, WXS_BASIC_CAST type,
      ...
      for (i = 0; i < baseUses->nbItems; i++) {
        ...
        if (prohibs) {
          ...
          for (j = 0; j < prohibs->nbItems; j++) {
            ...
            if ((WXS_ATTRUSE_DECL_NAME(use) == pro->name) &&
                (WXS_ATTRUSE_DECL_TNS(use) == pro->targetNamespace)) {
              goto inherit_next;
            }
          }
        }
        if (usesCount) {
          ...
          for (j = 0; j < usesCount; j++) {
            tmp = uses->items[j];
            if ((WXS_ATTRUSE_DECL_NAME(use) == WXS_ATTRUSE_DECL_NAME(tmp)) &&
                (WXS_ATTRUSE_DECL_TNS(use) == WXS_ATTRUSE_DECL_TNS(tmp))) {
              goto inherit_next;
            }
          }
        }
...
static int xmlSchemaCheckCOSSTDerivedOK(xmlSchemaAbstractCtxtPtr actxt,
  ...
  if ((!WXS_IS_ANYTYPE(type->baseType)) &&
      (xmlSchemaCheckCOSSTDerivedOK(actxt, type->baseType, baseType, subset) ==
       0)) {
    return (0);
  }
  /*
    ...
    while (cur != NULL) {
      if (WXS_IS_TYPE_NOT_FIXED(cur->type))
        if (xmlSchemaTypeFixup(cur->type, actxt) == -1)
          return (-1);
      if (xmlSchemaCheckCOSSTDerivedOK(actxt, type, cur->type, subset) == 0) {
        /*
 ...
 */
static void xmlSchemaCheckTypeDefCircular(xmlSchemaTypePtr item,
                                          xmlSchemaParserCtxtPtr ctxt) {
  if ((item == NULL) || (item->type == XML_SCHEMA_TYPE_BASIC) ||
      (item->baseType == NULL))
    return;
    ...
    while ((memberType != NULL) &&
      ...
      if ((WXS_IS_UNION(memberType)) &&
        ...
        res = xmlSchemaCheckUnionTypeDefCircularRecur(
            pctxt, ctxType, xmlSchemaGetUnionSimpleTypeMemberTypes(memberType));
        memberType->flags ^= XML_SCHEMAS_TYPE_MARKED;
        if (res != 0)
          return (res);
      }
...
static int xmlSchemaCheckUnionTypeDefCircular(xmlSchemaParserCtxtPtr pctxt,
                                              xmlSchemaTypePtr type) {
  if (!WXS_IS_UNION(type))
    return (0);
  return (
      xmlSchemaCheckUnionTypeDefCircularRecur(pctxt, type, type->memberTypes));
}
 ...
 */
static void xmlSchemaResolveTypeReferences(xmlSchemaTypePtr typeDef,
                                           xmlSchemaParserCtxtPtr ctxt) {
  if (typeDef == NULL)
    return;
  ...
  if (typeDef->baseType == NULL) {
    typeDef->baseType =
        xmlSchemaGetType(ctxt->schema, typeDef->base, typeDef->baseNs);
    if (typeDef->baseType == NULL) {
      xmlSchemaPResCompAttrErr(
          ctxt, XML_SCHEMAP_SRC_RESOLVE, WXS_BASIC_CAST typeDef, typeDef->node,
          "base", typeDef->base, typeDef->baseNs, XML_SCHEMA_TYPE_SIMPLE, NULL);
      return;
    }
  }
   ...
   */
     ...
     */
    groupDef = WXS_MODEL_GROUPDEF_CAST xmlSchemaGetNamedComponent(
        ctxt->schema, ref->itemType, ref->name, ref->targetNamespace);
    if (groupDef == NULL) {
      xmlSchemaPResCompAttrErr(ctxt, XML_SCHEMAP_SRC_RESOLVE, NULL,
                               WXS_ITEM_NODE(WXS_TYPE_PARTICLE(typeDef)), "ref",
                               ref->name, ref->targetNamespace, ref->itemType,
                               NULL);
      /* Remove the particle. */
 ...
 * errors.
 */
static int xmlSchemaCheckSTPropsCorrect(xmlSchemaParserCtxtPtr ctxt,
                                        xmlSchemaTypePtr type) {
  ...
  if (baseType == NULL) {
    ...
    xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_ST_PROPS_CORRECT_1,
                        WXS_BASIC_CAST type, NULL, "No base type existent",
                        NULL);
  ...
  if ((WXS_IS_LIST(type) || WXS_IS_UNION(type)) &&
      (WXS_IS_RESTRICTION(type) == 0) && (!WXS_IS_ANY_SIMPLE_TYPE(baseType))) {
    xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_ST_PROPS_CORRECT_1,
                        WXS_BASIC_CAST type, NULL,
  ...
  if ((!WXS_IS_ATOMIC(type)) && (!WXS_IS_UNION(type)) && (!WXS_IS_LIST(type))) {
    xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_ST_PROPS_CORRECT_1,
                        WXS_BASIC_CAST type, NULL, "The variety is absent",
                        NULL);
    return (XML_SCHEMAP_ST_PROPS_CORRECT_1);
  }
 ...
 */
static int xmlSchemaCheckCOSSTRestricts(xmlSchemaParserCtxtPtr pctxt,
                                        xmlSchemaTypePtr type) {
  xmlChar *str = NULL;
  ...
  if (WXS_IS_ATOMIC(type)) {
    ...
    if (xmlSchemaTypeFinalContains(type->baseType,
                                   XML_SCHEMAS_TYPE_FINAL_RESTRICTION)) {
      xmlSchemaPCustomErr(
          pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_1_2, WXS_BASIC_CAST type, NULL,
    ...
    if ((!WXS_IS_ATOMIC(itemType)) && (!WXS_IS_UNION(itemType))) {
      xmlSchemaPCustomErr(
          pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_1, WXS_BASIC_CAST type, NULL,
      ...
      while (member != NULL) {
        if (!WXS_IS_ATOMIC(member->type)) {
          xmlSchemaPCustomErr(
              pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_1, WXS_BASIC_CAST type,
              NULL,
    ...
    if (WXS_IS_ANY_SIMPLE_TYPE(type->baseType)) {
      ...
      if (xmlSchemaTypeFinalContains(itemType, XML_SCHEMAS_TYPE_FINAL_LIST)) {
        xmlSchemaPCustomErr(
            pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_3_1_1, WXS_BASIC_CAST type,
            NULL, "The final of its item type '%s' must not contain 'list'",
            xmlSchemaGetComponentQName(&str, itemType));
        FREE_AND_NULL(str)
      ...
      if (type->facets != NULL) {
        ...
        do {
          if (facet->type != XML_SCHEMA_FACET_WHITESPACE) {
            xmlSchemaPIllegalFacetListUnionErr(
                pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_3_1_2, type, facet);
      ...
      if (!WXS_IS_LIST(type->baseType)) {
        xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_3_2_1,
                            WXS_BASIC_CAST type, NULL,
      ...
      if (xmlSchemaTypeFinalContains(type->baseType,
                                     XML_SCHEMAS_TYPE_FINAL_RESTRICTION)) {
        xmlSchemaPCustomErr(
            pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_3_2_2, WXS_BASIC_CAST type,
            NULL,
       ...
       */
      {
        ...
        if ((itemType != baseItemType) &&
          ...
          xmlSchemaPCustomErrExt(
              pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_3_2_3, WXS_BASIC_CAST type,
              NULL,
      ...
      if (type->facets != NULL) {
        ...
        do {
          ...
          default: {
            xmlSchemaPIllegalFacetListUnionErr(
                pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_2_3_2_4, type, facet);
            /*
    ...
    while (member != NULL) {
      ...
      if ((!WXS_IS_ATOMIC(member->type)) && (!WXS_IS_LIST(member->type))) {
        xmlSchemaPCustomErr(
            pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_1, WXS_BASIC_CAST type, NULL,
    ...
    if (type->baseType->builtInType == XML_SCHEMAS_ANYSIMPLETYPE) {
      ...
      while (member != NULL) {
        if (xmlSchemaTypeFinalContains(member->type,
          ...
          xmlSchemaPCustomErr(
              pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_3_1, WXS_BASIC_CAST type,
              NULL, "The 'final' of member type '%s' contains 'union'",
              xmlSchemaGetComponentQName(&str, member->type));
          FREE_AND_NULL(str)
       ...
       */
      if (type->facetSet != NULL) {
        xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_3_1_2,
                            WXS_BASIC_CAST type, NULL, "No facets allowed",
                            NULL);
      ...
      if (!WXS_IS_UNION(type->baseType)) {
        xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_3_2_1,
                            WXS_BASIC_CAST type, NULL,
      ...
      /*
       * 3.3.2.2 The {final} of the {base type definition} must not contain
       * restriction.
       */
      if (xmlSchemaTypeFinalContains(type->baseType,
                                     XML_SCHEMAS_TYPE_FINAL_RESTRICTION)) {
        xmlSchemaPCustomErr(
            pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_3_2_2, WXS_BASIC_CAST type,
            NULL,
            "The 'final' of its base type '%s' must not contain 'restriction'",
            xmlSchemaGetComponentQName(&str, type->baseType));
        FREE_AND_NULL(str)
          ...
          while (member != NULL) {
            if (baseMember == NULL) {
              PERROR_INT("xmlSchemaCheckCOSSTRestricts",
                       ...
                       (xmlSchemaCheckCOSSTDerivedOK(
                            ACTXT_CAST pctxt, member->type, baseMember->type,
                            0) != 0)) {
              xmlChar *strBMT = NULL, *strBT = NULL;

              xmlSchemaPCustomErrExt(
                  pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_3_2_3,
                  WXS_BASIC_CAST type, NULL,
      ...
      if (type->facets != NULL) {
        ...
        do {
          if ((facet->type != XML_SCHEMA_FACET_PATTERN) &&
              (facet->type != XML_SCHEMA_FACET_ENUMERATION)) {
            xmlSchemaPIllegalFacetListUnionErr(
                pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_3_3_2_4, type, facet);
            ok = 0;
          }
          facet = facet->next;
        } while (facet != NULL);
...
static int xmlSchemaVCheckCVCSimpleType(xmlSchemaAbstractCtxtPtr actxt,
                                        xmlNodePtr node, xmlSchemaTypePtr type,
                                        const xmlChar *value,
                                        xmlSchemaValPtr *retVal, int fireErrors,
                                        int normalize, int isNormalized);
 ...
 * errors.
 */
static int xmlSchemaParseCheckCOSValidDefault(xmlSchemaParserCtxtPtr pctxt,
                                              xmlNodePtr node,
                                              xmlSchemaTypePtr type,
                                              const xmlChar *value,
                                              xmlSchemaValPtr *val) {
  ...
  if
    WXS_IS_COMPLEX(type) {
      ...
      if ((!WXS_HAS_SIMPLE_CONTENT(type)) &&
        ...
        xmlSchemaPCustomErr(
            pctxt, XML_SCHEMAP_COS_VALID_DEFAULT_2_1, WXS_BASIC_CAST type,
            type->node,
  ...
  if (WXS_IS_SIMPLE(type))
    ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST pctxt, node, type, value, val,
                                       1, 1, 0);
  else if (WXS_HAS_SIMPLE_CONTENT(type))
    ret = xmlSchemaVCheckCVCSimpleType(
        ACTXT_CAST pctxt, node, type->contentTypeDef, value, val, 1, 1, 0);
  else
    return (ret);
 ...
 */
static int xmlSchemaCheckCTPropsCorrect(xmlSchemaParserCtxtPtr pctxt,
                                        xmlSchemaTypePtr type) {
  /*
   ...
   * Sub-components (�5.3)."
   */
  if ((type->baseType != NULL) && (WXS_IS_SIMPLE(type->baseType)) &&
     ...
     */
    xmlSchemaCustomErr(
        ACTXT_CAST pctxt, XML_SCHEMAP_SRC_CT_1, NULL, WXS_BASIC_CAST type,
  ...
  if (type->attrUses && (((xmlSchemaItemListPtr)type->attrUses)->nbItems > 1)) {
    ...
    for (i = uses->nbItems - 1; i >= 0; i--) {
      ...
      if (i > 0) {
        for (j = i - 1; j >= 0; j--) {
          ...
          if ((WXS_ATTRUSE_DECL_NAME(use) == WXS_ATTRUSE_DECL_NAME(tmp)) &&
            ...
            xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_AG_PROPS_CORRECT,
                               NULL, WXS_BASIC_CAST type, "Duplicate %s",
      ...
      if (WXS_ATTRUSE_TYPEDEF(use) != NULL) {
        if (xmlSchemaIsDerivedFromBuiltInType(WXS_ATTRUSE_TYPEDEF(use),
                                              XML_SCHEMAS_ID)) {
          if (hasId) {
            xmlChar *str = NULL;

            xmlSchemaCustomErr(
                ACTXT_CAST pctxt, XML_SCHEMAP_AG_PROPS_CORRECT, NULL,
                WXS_BASIC_CAST type,
...
static int xmlSchemaCheckCOSCTDerivedOK(xmlSchemaAbstractCtxtPtr actxt,
                                        xmlSchemaTypePtr type,
                                        xmlSchemaTypePtr baseType, int set) {
  ...
  if (WXS_IS_COMPLEX(type->baseType)) {
     ...
     * constraint."
     */
    return (xmlSchemaCheckCOSCTDerivedOK(actxt, type->baseType, baseType, set));
  } else {
    /*
     ...
     */
    return (xmlSchemaCheckCOSSTDerivedOK(actxt, type->baseType, baseType, set));
  }
}
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckCOSCTExtends(xmlSchemaParserCtxtPtr ctxt,
                                      xmlSchemaTypePtr type) {
  ...
  if (WXS_IS_COMPLEX(base)) {
	...
	*/
      ...
      if (base->contentType == XML_SCHEMA_CONTENT_EMPTY) {
        ...
        if ((type->contentType != base->contentType) ||
          ...
          xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_COS_CT_EXTENDS_1_1,
                              ...
                              "type, must either 'mixed' or 'element-only'",
                              NULL);
          return (XML_SCHEMAP_COS_CT_EXTENDS_1_1);
        }
        /*
    ...
    if (type->contentTypeDef != base) {
       ...
       */
      xmlSchemaPCustomErr(
          ctxt, XML_SCHEMAP_COS_CT_EXTENDS_1_1, WXS_BASIC_CAST type, NULL,
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckDerivationOKRestriction(xmlSchemaParserCtxtPtr ctxt,
                                                 xmlSchemaTypePtr type) {
  xmlSchemaTypePtr base;

  /*
   * TODO: Correct the error code; XML_SCHEMAP_DERIVATION_OK_RESTRICTION_1 is
  ...
  if (xmlSchemaCheckDerivationOKRestriction2to4(
          ctxt, XML_SCHEMA_ACTION_DERIVE, WXS_BASIC_CAST type,
          WXS_BASIC_CAST base, type->attrUses, base->attrUses,
  ...
  if (base->builtInType == XML_SCHEMAS_ANYTYPE) {
    ...
    if ((base->contentType == XML_SCHEMA_CONTENT_SIMPLE) ||
       ...
       *
       */
      err = xmlSchemaCheckCOSSTDerivedOK(ACTXT_CAST ctxt, type->contentTypeDef,
      ...
      if (err != 0) {
        ...
        if (err == -1)
          return (-1);
        xmlSchemaCustomErr(
            ACTXT_CAST ctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_1, NULL,
            WXS_BASIC_CAST type,
            "The {content type} %s is not validly derived from the "
            "base type's {content type} %s",
            xmlSchemaGetComponentDesignation(&strA, type->contentTypeDef),
            xmlSchemaGetComponentDesignation(&strB, base->contentTypeDef));
        FREE_AND_NULL(strA);
        FREE_AND_NULL(strB);
        return (ctxt->err);
      }
      ...
      xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_1,
                          ...
                          "a simple type or 'mixed' and an emptiable particle",
                          NULL);
      return (ctxt->err);
    }
  } else if (type->contentType == XML_SCHEMA_CONTENT_EMPTY) {
    ...
    if (base->contentType == XML_SCHEMA_CONTENT_EMPTY) {
      ...
      xmlSchemaPCustomErr(
          ctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_1, WXS_BASIC_CAST type,
          NULL,
          "The content type of the base type must be either "
    ...
    xmlSchemaPCustomErr(
        ctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_1, WXS_BASIC_CAST type,
        NULL, "The type is not a valid restriction of its base type", NULL);
    return (ctxt->err);
  }
  return (0);
}
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckCTComponent(xmlSchemaParserCtxtPtr ctxt,
                                     xmlSchemaTypePtr type) {
  int ret;
  /*
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckSRCCT(xmlSchemaParserCtxtPtr ctxt,
                               xmlSchemaTypePtr type) {
  xmlSchemaTypePtr base;
  int ret = 0;
  ...
  if (!WXS_HAS_SIMPLE_CONTENT(type)) {
    ...
    if (!WXS_IS_COMPLEX(base)) {
      xmlChar *str = NULL;
      xmlSchemaPCustomErr(
          ctxt, XML_SCHEMAP_SRC_CT_1, WXS_BASIC_CAST type, type->node,
    ...
    if (WXS_IS_SIMPLE(base)) {
      if (WXS_IS_EXTENSION(type) == 0) {
        ...
        xmlSchemaPCustomErr(
            ctxt, XML_SCHEMAP_SRC_CT_1, WXS_BASIC_CAST type, NULL,
   ...
   */
  /*
   * SPEC (4) If clause 2.2.1 or clause 2.2.2 in the correspondence
   * specification above for {attribute wildcard} is satisfied, the intensional
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckParticleRangeOK(int rmin, int rmax, int bmin,
                                         int bmax) {
  if (rmin < bmin)
    return (1);
  if ((bmax != UNBOUNDED) && (rmax > bmax))
    return (1);
  return (0);
}
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckRCaseNameAndTypeOK(xmlSchemaParserCtxtPtr ctxt,
   ...
   */
  if (xmlSchemaCheckParticleRangeOK(r->minOccurs, r->maxOccurs, b->minOccurs,
                                    b->maxOccurs) != 0)
    return (1);
  /*
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckRCaseNSCompat(xmlSchemaParserCtxtPtr ctxt,
                                       xmlSchemaParticlePtr r,
                                       xmlSchemaParticlePtr b) {
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckRCaseRecurseAsIfGroup(xmlSchemaParserCtxtPtr ctxt,
                                               xmlSchemaParticlePtr r,
                                               xmlSchemaParticlePtr b) {
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckRCaseNSSubset(xmlSchemaParserCtxtPtr ctxt,
                                       xmlSchemaParticlePtr r,
                                       xmlSchemaParticlePtr b,
                                       int isAnyTypeBase) {
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckCOSParticleRestrict(xmlSchemaParserCtxtPtr ctxt,
                                             xmlSchemaParticlePtr r,
                                             xmlSchemaParticlePtr b) {
  int ret = 0;
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckRCaseRecurse(xmlSchemaParserCtxtPtr ctxt,
                                      xmlSchemaParticlePtr r,
                                      xmlSchemaParticlePtr b) {
  /* xmlSchemaParticlePtr part; */
...
#define FACET_RESTR_MUTUAL_ERR(fac1, fac2)                                     \
  xmlSchemaPCustomErrExt(                                                      \
      pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, WXS_BASIC_CAST fac1, fac1->node, \
...
#define FACET_RESTR_ERR(fac1, msg)                                             \
  xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE,                  \
                      WXS_BASIC_CAST fac1, fac1->node, msg, NULL);

#define FACET_RESTR_FIXED_ERR(fac)                                             \
  xmlSchemaPCustomErr(                                                         \
      pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, WXS_BASIC_CAST fac, fac->node,   \
...
static void xmlSchemaDeriveFacetErr(xmlSchemaParserCtxtPtr pctxt,
                                    ...
                                    xmlSchemaFacetPtr facet2, int lessGreater,
                                    int orEqual, int ofBase) {
  xmlChar *msg = NULL;
  ...
  xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE,
                      WXS_BASIC_CAST facet1, NULL, (const char *)msg, NULL);
 ...
 * (st-restrict-facets)
 */
static int xmlSchemaDeriveAndValidateFacets(xmlSchemaParserCtxtPtr pctxt,
                                            xmlSchemaTypePtr type) {
  xmlSchemaTypePtr base = type->baseType;
  xmlSchemaFacetLinkPtr link, cur, last = NULL;
  xmlSchemaFacetPtr facet, bfacet,
      flength = NULL, ftotdig = NULL, ffracdig = NULL, fmaxlen = NULL,
      fminlen = NULL, /* facets of the current type */
      fmininc = NULL, fmaxinc = NULL, fminexc = NULL, fmaxexc = NULL,
      bflength = NULL, bftotdig = NULL, bffracdig = NULL, bfmaxlen = NULL,
      bfminlen = NULL, /* facets of the base type */
      bfmininc = NULL, bfmaxinc = NULL, bfminexc = NULL, bfmaxexc = NULL;
  ...
  for (cur = base->facetSet; cur != NULL; cur = cur->next) {
      ...
      if (facet->type == bfacet->type) {
        ...
        case XML_SCHEMA_FACET_WHITESPACE:
          ...
          if (facet->whitespace < bfacet->whitespace) {
            FACET_RESTR_ERR(facet, "The 'whitespace' value has to be equal to "
     ...
     */
    if (link == NULL) {
      link = (xmlSchemaFacetLinkPtr)xmlMalloc(sizeof(xmlSchemaFacetLink));
      if (link == NULL) {
        xmlSchemaPErrMemory(pctxt, "deriving facets, creating a facet link",
                            NULL);
        return (-1);
      }
    ...
    if (WXS_IS_UNION(link->type)) {
      ...
      if (subLink != NULL) {
        ...
        if (subLink->next != NULL) {
          ...
          while (subLink != NULL) {
            newLink =
                (xmlSchemaTypeLinkPtr)xmlMalloc(sizeof(xmlSchemaTypeLink));
            if (newLink == NULL) {
...
static void xmlSchemaTypeFixupOptimFacets(xmlSchemaTypePtr type) {
  ...
  if (has) {
    needVal =
        (type->baseType->flags & XML_SCHEMAS_TYPE_FACETSNEEDVALUE) ? 1 : 0;
    normVal =
        (type->baseType->flags & XML_SCHEMAS_TYPE_NORMVALUENEEDED) ? 1 : 0;
  }
  if (type->facets != NULL) {
...
static int xmlSchemaTypeFixupWhitespace(xmlSchemaTypePtr type) {
   ...
   */
  {
    ...
    for (anc = type->baseType;
         anc != NULL && anc->builtInType != XML_SCHEMAS_ANYTYPE;
         anc = anc->baseType) {
...
static int xmlSchemaFixupSimpleTypeStageOne(xmlSchemaParserCtxtPtr pctxt,
                                            xmlSchemaTypePtr type) {
  if (type->type != XML_SCHEMA_TYPE_SIMPLE)
    return (0);
...
#ifdef DEBUG_TYPE
static void xmlSchemaDebugFixedType(xmlSchemaParserCtxtPtr pctxt,
                                    xmlSchemaTypePtr type) {
  if (type->node != NULL) {
    xmlGenericError(xmlGenericErrorContext, "Type of %s : %s:%d :", name,
                    type->node->doc->URL, xmlGetLineNo(type->node));
  } else {
  ...
  if ((WXS_IS_SIMPLE(type)) || (WXS_IS_COMPLEX(type))) {
    ...
    case XML_SCHEMA_CONTENT_BASIC:
      ...
      break;
    default:
      xmlGenericError(xmlGenericErrorContext, "not registered !!!\n");
      break;
    }
  }
}
 ...
 */
static int xmlSchemaFixupSimpleTypeStageTwo(xmlSchemaParserCtxtPtr pctxt,
                                            xmlSchemaTypePtr type) {
  ...
  HFAILURE HERROR if ((type->facetSet != NULL) ||
...
static int xmlSchemaFixupComplexType(xmlSchemaParserCtxtPtr pctxt,
                                     xmlSchemaTypePtr type) {
  ...
  HFAILURE HERROR
     ...
     */
    if ((WXS_IS_COMPLEX(baseType)) && (baseType->contentTypeDef != NULL) &&
        (WXS_IS_RESTRICTION(type))) {
      ...
      content = xmlSchemaAddType(pctxt, pctxt->schema, XML_SCHEMA_TYPE_SIMPLE,
                                 tmpname, type->targetNamespace, type->node, 0);
#else
      content = xmlSchemaAddType(pctxt, pctxt->schema, XML_SCHEMA_TYPE_SIMPLE,
                                 NULL, type->targetNamespace, type->node, 0);
#endif
      if (content == NULL)
        goto exit_failure;
      /*
      ...
      HFAILURE HERROR res = xmlSchemaFixupSimpleTypeStageTwo(pctxt, content);
      HFAILURE HERROR
      ...
      if ((type->contentTypeDef == NULL) ||
         ...
         */
        xmlSchemaPCustomErr(
            pctxt, XML_SCHEMAP_INTERNAL, WXS_BASIC_CAST type, NULL,
      ...
      if (baseType->contentTypeDef == NULL) {
         ...
         */
        xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INTERNAL, WXS_BASIC_CAST type,
                            NULL,
                            "Internal error: xmlSchemaTypeFixup, "
    ...
    if ((particle == NULL) ||
      ...
      if (type->flags & XML_SCHEMAS_TYPE_MIXED) {
        ...
        if ((particle == NULL) ||
           ...
           * Create the particle.
           */
          particle = xmlSchemaAddParticle(pctxt, type->node, 1, 1);
          if (particle == NULL)
            goto exit_failure;
          /*
           * Create the model group.
           */ /* URGENT TODO: avoid adding to pending items. */
          particle->children = (xmlSchemaTreeItemPtr)xmlSchemaAddModelGroup(
    ...
    if (WXS_IS_RESTRICTION(type)) {
      ...
      if (type->contentType == XML_SCHEMA_CONTENT_EMPTY) {
        ...
        if ((WXS_TYPE_PARTICLE(type) != NULL) &&
            ...
            ((WXS_TYPE_PARTICLE_TERM(type))->type == XML_SCHEMA_TYPE_ALL)) {
          /*
           * SPEC cos-all-limited (1)
           */
          xmlSchemaCustomErr(ACTXT_CAST pctxt,
                             /* TODO: error code */
                             XML_SCHEMAP_COS_ALL_LIMITED, WXS_ITEM_NODE(type),
                             NULL,
          ...
          xmlSchemaCustomErr(
              ACTXT_CAST pctxt,
              /* TODO: error code */
              XML_SCHEMAP_COS_ALL_LIMITED, WXS_ITEM_NODE(type), NULL,
          ...
          particle->children->children =
              (xmlSchemaTreeItemPtr)xmlSchemaAddParticle(
                  pctxt, type->node,
                  ((xmlSchemaParticlePtr)type->subtypes)->minOccurs,
                  ((xmlSchemaParticlePtr)type->subtypes)->maxOccurs);
          if (particle->children->children == NULL)
            goto exit_failure;
          particle = (xmlSchemaParticlePtr)particle->children->children;
          particle->children =
...
int xmlSchemaCheckFacet(xmlSchemaFacetPtr facet, xmlSchemaTypePtr typeDecl,
  ...
  case XML_SCHEMA_FACET_ENUMERATION: {
     ...
     * of the facet.
     */
    ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST pctxt, facet->node, base,
    ...
    if (ret != 0) {
      if (ret < 0) {
        ...
        if (ctxtGiven) {
          xmlSchemaCustomErr(
              ACTXT_CAST pctxt, XML_SCHEMAP_INTERNAL, facet->node, NULL,
      ...
      if (ctxtGiven) {
        ...
        xmlSchemaCustomErr(
            ACTXT_CAST pctxt, ret, facet->node, WXS_BASIC_CAST facet,
  ...
  case XML_SCHEMA_FACET_PATTERN:
    ...
    if (facet->regexp == NULL) {
      ...
      /* No error message for RelaxNG. */
      if (ctxtGiven) {
        xmlSchemaCustomErr(ACTXT_CAST pctxt, ret, facet->node,
  ...
  case XML_SCHEMA_FACET_MINLENGTH:
    ...
    if (facet->type == XML_SCHEMA_FACET_TOTALDIGITS) {
      ret = xmlSchemaValidatePredefinedType(
          xmlSchemaGetBuiltInType(XML_SCHEMAS_PINTEGER), facet->value,
          &(facet->val));
    } else {
      ret = xmlSchemaValidatePredefinedType(
          xmlSchemaGetBuiltInType(XML_SCHEMAS_NNINTEGER), facet->value,
          &(facet->val));
    }
    if (ret != 0) {
      if (ret < 0) {
  ...
  for (; particle != NULL; particle = particle->next) {
    ...
    case XML_SCHEMA_TYPE_GROUP:
      ...
      if (gdef->children != NULL) {
        ...
        circ =
            xmlSchemaGetCircModelGrDefRef(groupDef, gdef->children->children);
        gdef->flags ^= XML_SCHEMA_MODEL_GROUP_DEF_MARKED;
        if (circ != NULL)
          return (circ);
      }
 ...
 */
static void xmlSchemaCheckGroupDefCircular(xmlSchemaModelGroupDefPtr item,
                                           xmlSchemaParserCtxtPtr ctxt) {
  /*
   ...
   * is the group itself.
   */
  if ((item == NULL) || (item->type != XML_SCHEMA_TYPE_GROUP) ||
      (item->children == NULL))
    return;
  {
    ...
    if (circ != NULL) {
       ...
       */
      xmlSchemaPCustomErr(
          ctxt, XML_SCHEMAP_MG_PROPS_CORRECT_2, NULL, WXS_ITEM_NODE(circ),
          "Circular reference to the model group definition '%s' "
          "defined",
          xmlSchemaFormatQName(&str, item->targetNamespace, item->name));
      FREE_AND_NULL(str)
      /*
 ...
 */
static void xmlSchemaModelGroupToModelGroupDefFixup(
    xmlSchemaParserCtxtPtr ctxt ATTRIBUTE_UNUSED, xmlSchemaModelGroupPtr mg) {
  xmlSchemaParticlePtr particle = WXS_MODELGROUP_PARTICLE(mg);

  while (particle != NULL) {
    if ((WXS_PARTICLE_TERM(particle) == NULL) ||
        ((WXS_PARTICLE_TERM(particle))->type != XML_SCHEMA_TYPE_GROUP)) {
    ...
    if ((ref->type == XML_SCHEMA_EXTRA_QNAMEREF) &&
      ...
      if ((gr->attrUses) && (gr->flags & XML_SCHEMAS_ATTRGROUP_HAS_REFS)) {
        ...
        circ = xmlSchemaCheckAttrGroupCircularRecur(
            ctxtGr, (xmlSchemaItemListPtr)gr->attrUses);
        gr->flags ^= XML_SCHEMAS_ATTRGROUP_MARKED;
        if (circ != NULL)
          return (circ);
      }
 ...
 */
static int xmlSchemaCheckAttrGroupCircular(xmlSchemaAttributeGroupPtr attrGr,
                                           xmlSchemaParserCtxtPtr ctxt) {
  /*
   * Schema Representation Constraint:
  ...
  if (attrGr->attrUses == NULL)
    ...
    circ = xmlSchemaCheckAttrGroupCircularRecur(
        attrGr, (xmlSchemaItemListPtr)attrGr->attrUses);
    if (circ != NULL) {
      xmlChar *str = NULL;
      /*
       * TODO: Report the referenced attr group as QName.
       */
      xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_SRC_ATTRIBUTE_GROUP_3, NULL,
                          WXS_ITEM_NODE(WXS_BASIC_CAST circ),
                          "Circular reference to the attribute group '%s' "
                          "defined",
                          xmlSchemaGetComponentQName(&str, attrGr));
      FREE_AND_NULL(str);
      /*
       * NOTE: We will cut the reference to avoid further
...
xmlSchemaAttributeGroupExpandRefs(xmlSchemaParserCtxtPtr pctxt,
  ...
  if (xmlSchemaExpandAttributeGroupRefs(pctxt, WXS_BASIC_CAST attrGr,
                                        &(attrGr->attributeWildcard),
                                        attrGr->attrUses, NULL) == -1)
    return (-1);
  return (0);
}
 ...
 */
static int xmlSchemaCheckAGPropsCorrect(xmlSchemaParserCtxtPtr pctxt,
                                        xmlSchemaAttributeGroupPtr attrGr) {
  /*
      ...
      if (i > 0) {
        for (j = i - 1; j >= 0; j--) {
          ...
          if ((WXS_ATTRUSE_DECL_NAME(use) == WXS_ATTRUSE_DECL_NAME(tmp)) &&
            ...
            xmlSchemaCustomErr(
                ACTXT_CAST pctxt, XML_SCHEMAP_AG_PROPS_CORRECT, attrGr->node,
                WXS_BASIC_CAST attrGr, "Duplicate %s",
                xmlSchemaGetComponentDesignation(&str, use), NULL);
            FREE_AND_NULL(str);
            /*
             * Remove the duplicate.
 ...
 */
static int xmlSchemaResolveAttrGroupReferences(xmlSchemaQNameRefPtr ref,
                                               xmlSchemaParserCtxtPtr ctxt) {
  ...
  if (ref->item != NULL)
    return (0);
  group =
      xmlSchemaGetAttributeGroup(ctxt->schema, ref->name, ref->targetNamespace);
  if (group == NULL) {
    xmlSchemaPResCompAttrErr(ctxt, XML_SCHEMAP_SRC_RESOLVE, NULL, ref->node,
 ...
 *   builded and checked.
 */
static int xmlSchemaCheckAttrPropsCorrect(xmlSchemaParserCtxtPtr pctxt,
                                          xmlSchemaAttributePtr attr) {
  ...
  if (attr->defValue != NULL) {
     ...
     */
    if (xmlSchemaIsDerivedFromBuiltInType(WXS_ATTR_TYPEDEF(attr),
                                          XML_SCHEMAS_ID)) {
      xmlSchemaCustomErr(
          ACTXT_CAST pctxt, XML_SCHEMAP_A_PROPS_CORRECT_3, NULL,
          WXS_BASIC_CAST attr,
     ...
     */
    ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST pctxt, attr->node,
                                       WXS_ATTR_TYPEDEF(attr), attr->defValue,
                                       &(attr->defVal), 1, 1, 0);
    if (ret != 0) {
      if (ret < 0) {
      ...
      xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_A_PROPS_CORRECT_2, NULL,
                         WXS_BASIC_CAST attr,
                         "The value of the value constraint is not valid", NULL,
                         NULL);
      return (pctxt->err);
    }
  }
 ...
 *   missing: (6)
 */
static int xmlSchemaCheckElemPropsCorrect(xmlSchemaParserCtxtPtr pctxt,
                                          xmlSchemaElementPtr elemDecl) {
  int ret = 0;
  ...
  if (WXS_SUBST_HEAD(elemDecl) != NULL) {
    ...
    if (circ != NULL) {
      ...
      xmlSchemaPCustomErrExt(pctxt, XML_SCHEMAP_E_PROPS_CORRECT_6,
                             ...
                             xmlSchemaGetComponentQName(&strB, head), NULL);
      FREE_AND_NULL(strA)
      FREE_AND_NULL(strB)
    ...
    if (typeDef != WXS_ELEM_TYPEDEF(WXS_SUBST_HEAD(elemDecl))) {
      ...
      if (xmlSchemaCheckCOSDerivedOK(ACTXT_CAST pctxt, typeDef,
        ...
        xmlSchemaPCustomErrExt(
            pctxt, XML_SCHEMAP_E_PROPS_CORRECT_4, WXS_BASIC_CAST elemDecl, NULL,
  ...
  if ((elemDecl->value != NULL) &&
     ...
     */
    if (typeDef == NULL) {
      xmlSchemaPErr(pctxt, elemDecl->node, XML_SCHEMAP_INTERNAL,
    ...
    if (elemDecl->node != NULL) {
      if (elemDecl->flags & XML_SCHEMAS_ELEM_FIXED)
        node = (xmlNodePtr)xmlHasProp(elemDecl->node, BAD_CAST "fixed");
      else
        node = (xmlNodePtr)xmlHasProp(elemDecl->node, BAD_CAST "default");
    }
    vcret = xmlSchemaParseCheckCOSValidDefault(
        pctxt, node, typeDef, elemDecl->value, &(elemDecl->defVal));
    if (vcret != 0) {
      if (vcret < 0) {
...
static void xmlSchemaCheckElemSubstGroup(xmlSchemaParserCtxtPtr ctxt,
    ...
    return;
  {
    ...
    for (head = WXS_SUBST_HEAD(elemDecl); head != NULL;
      ...
      while (type != NULL) {
        if (WXS_IS_COMPLEX(type)) {
          if ((type->flags & XML_SCHEMAS_TYPE_BLOCK_EXTENSION) &&
              ((set & XML_SCHEMAS_TYPE_BLOCK_EXTENSION) == 0))
            set |= XML_SCHEMAS_TYPE_BLOCK_EXTENSION;
          if ((type->flags & XML_SCHEMAS_TYPE_BLOCK_RESTRICTION) &&
 ...
 */
static int xmlSchemaCheckElementDeclConsistent(
    xmlSchemaParserCtxtPtr pctxt, xmlSchemaBasicItemPtr ctxtComponent,
    xmlSchemaParticlePtr ctxtParticle, xmlSchemaParticlePtr searchParticle,
    xmlSchemaParticlePtr curParticle, int search) {
  return (0);
  ...
  while (cur != NULL) {
    ...
    case XML_SCHEMA_TYPE_ELEMENT:
      if (search == 0) {
        ret = xmlSchemaCheckElementDeclConsistent(
            pctxt, ctxtComponent, ctxtParticle, cur, ctxtParticle, 1);
        if (ret != 0)
          return (ret);
      } else {
        xmlSchemaElementPtr elem = WXS_ELEM_CAST(WXS_PARTICLE_TERM(cur));
        /*
        ...
        if (xmlStrEqual(WXS_PARTICLE_TERM_AS_ELEM(cur)->name,
          ...
          xmlSchemaCustomErr(
              ACTXT_CAST pctxt,
              /* TODO: error code */
              XML_SCHEMAP_COS_NONAMBIG, WXS_ITEM_NODE(cur), NULL,
              "In the content model of %s, there are multiple "
              "element declarations for '%s' with different "
              "type definitions",
              xmlSchemaGetComponentDesignation(&strA, ctxtComponent),
              xmlSchemaFormatQName(
                  &strB, WXS_PARTICLE_TERM_AS_ELEM(cur)->targetNamespace,
    ...
    default:
      xmlSchemaInternalErr2(
          ACTXT_CAST pctxt, "xmlSchemaCheckElementDeclConsistent",
          "found unexpected term of type '%s' in content model",
 ...
 */
static void
xmlSchemaResolveModelGroupParticleReferences(xmlSchemaParserCtxtPtr ctxt,
  ...
  while (particle != NULL) {
    if ((WXS_PARTICLE_TERM(particle) == NULL) ||
        ((WXS_PARTICLE_TERM(particle))->type != XML_SCHEMA_EXTRA_QNAMEREF)) {
      goto next_particle;
    }
    ...
    refItem = xmlSchemaGetNamedComponent(ctxt->schema, ref->itemType, ref->name,
                                         ref->targetNamespace);
    if (refItem == NULL) {
      xmlSchemaPResCompAttrErr(ctxt, XML_SCHEMAP_SRC_RESOLVE, NULL,
                               WXS_ITEM_NODE(particle), "ref", ref->name,
    ...
    if (refItem->type == XML_SCHEMA_TYPE_GROUP) {
      ...
      if ((WXS_MODELGROUPDEF_MODEL(refItem))->type == XML_SCHEMA_TYPE_ALL) {
        ...
        xmlSchemaCustomErr(ACTXT_CAST ctxt,
                           /* TODO: error code */
                           XML_SCHEMAP_COS_ALL_LIMITED, WXS_ITEM_NODE(particle),
                           NULL,
...
static int xmlSchemaAreValuesEqual(xmlSchemaValPtr x, xmlSchemaValPtr y) {
  ...
  while (x != NULL) {
    ...
    if ((ptx->builtInType == XML_SCHEMAS_STRING) ||
      ...
      ret = xmlSchemaCompareValuesWhtsp(x, XML_SCHEMA_WHITESPACE_PRESERVE, y,
...
static int xmlSchemaResolveAttrUseReferences(xmlSchemaAttributeUsePtr ause,
...

  {
     ...
     * found.
     */
    ause->attrDecl = xmlSchemaGetAttributeDecl(ctxt->schema, ref->name,
                                               ref->targetNamespace);
    if (ause->attrDecl == NULL) {
      xmlSchemaPResCompAttrErr(
          ctxt, XML_SCHEMAP_SRC_RESOLVE, WXS_BASIC_CAST ause, ause->node, "ref",
          ref->name, ref->targetNamespace, XML_SCHEMA_TYPE_ATTRIBUTE, NULL);
      return (ctxt->err);
      ;
    }
  }
 ...
 *
 */
static int xmlSchemaCheckAttrUsePropsCorrect(xmlSchemaParserCtxtPtr ctxt,
                                             xmlSchemaAttributeUsePtr use) {
  ...
  if (((WXS_ATTRUSE_DECL(use))->defValue != NULL) &&
    ...
    xmlSchemaPCustomErr(
        ctxt, XML_SCHEMAP_AU_PROPS_CORRECT_2, WXS_BASIC_CAST use, NULL,
  ...
  if ((use->defVal != NULL) && (WXS_ATTRUSE_TYPEDEF(use) != NULL)) {
    ...
    if (xmlSchemaIsDerivedFromBuiltInType(WXS_ATTRUSE_TYPEDEF(use),
                                          XML_SCHEMAS_ID)) {
      xmlSchemaCustomErr(
          ACTXT_CAST ctxt, XML_SCHEMAP_AU_PROPS_CORRECT, NULL,
          WXS_BASIC_CAST use,
          "Value constraints are not allowed if the type definition "
          "is or is derived from xs:ID",
          NULL, NULL);
    ...
    ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST ctxt, use->node,
                                       WXS_ATTRUSE_TYPEDEF(use), use->defValue,
                                       &(use->defVal), 1, 1, 0);
    if (ret != 0) {
      if (ret < 0) {
      ...
      xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAP_AU_PROPS_CORRECT, NULL,
                         WXS_BASIC_CAST use,
                         "The value of the value constraint is not valid", NULL,
                         NULL);
      return (ctxt->err);
 ...
 */
static int xmlSchemaResolveAttrTypeReferences(xmlSchemaAttributePtr item,
                                              xmlSchemaParserCtxtPtr ctxt) {
  /*
  ...
  if (item->typeName != NULL) {
    ...
    type = xmlSchemaGetType(ctxt->schema, item->typeName, item->typeNs);
    if ((type == NULL) || (!WXS_IS_SIMPLE(type))) {
      xmlSchemaPResCompAttrErr(
          ctxt, XML_SCHEMAP_SRC_RESOLVE, WXS_BASIC_CAST item, item->node,
          "type", item->typeName, item->typeNs, XML_SCHEMA_TYPE_SIMPLE, NULL);
      return (ctxt->err);
    } else
      item->subtypes = type;
 ...
 */
static int xmlSchemaResolveIDCKeyReferences(xmlSchemaIDCPtr idc,
                                            xmlSchemaParserCtxtPtr pctxt) {
  if (idc->type != XML_SCHEMA_TYPE_IDC_KEYREF)
    return (0);
  if (idc->ref->name != NULL) {
    idc->ref->item = (xmlSchemaBasicItemPtr)xmlSchemaGetIDC(
        pctxt->schema, idc->ref->name, idc->ref->targetNamespace);
    if (idc->ref->item == NULL) {
      ...
      xmlSchemaPResCompAttrErr(pctxt, XML_SCHEMAP_SRC_RESOLVE,
                               WXS_BASIC_CAST idc, idc->node, "refer",
                               idc->ref->name, idc->ref->targetNamespace,
       ...
       */
      xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_C_PROPS_CORRECT, NULL,
                         WXS_BASIC_CAST idc, "The keyref references a keyref",
                         NULL, NULL);
      idc->ref->item = NULL;
      return (pctxt->err);
    } else {
      if (idc->nbFields != ((xmlSchemaIDCPtr)idc->ref->item)->nbFields) {
         ...
         */
        xmlSchemaCustomErr(
            ACTXT_CAST pctxt, XML_SCHEMAP_C_PROPS_CORRECT, NULL,
            WXS_BASIC_CAST idc,
            "The cardinality of the keyref differs from the "
            "cardinality of the referenced key/unique '%s'",
            xmlSchemaFormatQName(&str, refer->targetNamespace, refer->name),
            NULL);
        FREE_AND_NULL(str)
        return (pctxt->err);
      }
...
xmlSchemaResolveAttrUseProhibReferences(xmlSchemaAttributeUseProhibPtr prohib,
  ...
  if (xmlSchemaGetAttributeDecl(pctxt->schema, prohib->name,
    ...
    xmlSchemaPResCompAttrErr(pctxt, XML_SCHEMAP_SRC_RESOLVE, NULL, prohib->node,
...
#define WXS_REDEFINED_MODEL_GROUP_DEF(c)                                       \
  (((xmlSchemaModelGroupDefPtr)item)->flags &                                  \
...
static int xmlSchemaCheckSRCRedefineFirst(xmlSchemaParserCtxtPtr pctxt) {
  ...
  do {
     ...
     *
     */
    prev = xmlSchemaFindRedefCompInGraph(redef->targetBucket, item->type,
    ...
    if (prev == NULL) {
      ...
      */
      ...
      xmlSchemaCustomErr(
          ...
          xmlSchemaFormatQName(&str, redef->refTargetNs, redef->refName));
      FREE_AND_NULL(str);
      err = pctxt->err;
      redef = redef->next;
      continue;
    ...
    case XML_SCHEMA_TYPE_COMPLEX:
    case XML_SCHEMA_TYPE_SIMPLE:
      if ((WXS_TYPE_CAST prev)->flags & XML_SCHEMAS_TYPE_REDEFINED) {
        wasRedefined = 1;
        break;
      }
       ...
       * TODO: How
       */
      ((xmlSchemaTypePtr)item)->baseType = (xmlSchemaTypePtr)prev;
      break;
    case XML_SCHEMA_TYPE_GROUP:
      ...
      break;
    case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
      if ((WXS_ATTR_GROUP_CAST prev)->flags & XML_SCHEMAS_ATTRGROUP_REDEFINED) {
        wasRedefined = 1;
        break;
      }
    ...
    if (wasRedefined) {
      ...
      xmlSchemaCustomErr(ACTXT_CAST pctxt,
                         ...
                         xmlSchemaGetComponentDesignation(&str, prev), NULL);
      FREE_AND_NULL(str)
      err = pctxt->err;
      redef = redef->next;
      continue;
  ...
  do {
    ...
    case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
      ...
      err = xmlSchemaCheckDerivationOKRestriction2to4(
          pctxt, XML_SCHEMA_ACTION_REDEFINE, item, redef->target,
...
static int xmlSchemaAddComponents(xmlSchemaParserCtxtPtr pctxt,
                                  xmlSchemaBucketPtr bucket) {
  xmlSchemaBasicItemPtr item;
  int err;
...
#define WXS_GET_GLOBAL_HASH(c, slot)                                           \
  ...
  for (i = 0; i < bucket->globals->nbItems; i++) {
    ...
    if (err != 0) {
      ...
      xmlSchemaCustomErr(
          ACTXT_CAST pctxt, XML_SCHEMAP_REDEFINED_TYPE, WXS_ITEM_NODE(item),
          WXS_BASIC_CAST item, "A global %s '%s' does already exist",
...
static int xmlSchemaFixupComponents(xmlSchemaParserCtxtPtr pctxt,
...
#define FIXHFAILURE                                                            \
  if (pctxt->err == XML_SCHEMAP_INTERNAL)                                      \
    goto exit_failure;

  if ((con->pending == NULL) || (con->pending->nbItems == 0))
    return (0);
    ...
    case XML_SCHEMA_TYPE_ELEMENT:
      ...
      if ((WXS_ELEM_TYPEDEF(elemDecl) != NULL) &&
        ...
        xmlSchemaCheckElementDeclConsistent(
            pctxt, WXS_BASIC_CAST elemDecl,
            WXS_TYPE_PARTICLE(WXS_ELEM_TYPEDEF(elemDecl)), NULL, NULL, 0);
      }
#endif
      break;
    default:
...
xmlSchemaPtr xmlSchemaParse(xmlSchemaParserCtxtPtr ctxt) {
   ...
   */
  res = xmlSchemaAddSchemaDoc(ctxt, XML_SCHEMA_SCHEMA_MAIN, ctxt->URL,
                              ctxt->doc, ctxt->buffer, ctxt->size, NULL, NULL,
                              NULL, &bucket);
  if (res == -1)
    goto exit_failure;
  if (res != 0)
  ...
  if (bucket == NULL) {
    ...
    if (ctxt->URL)
      xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAP_FAILED_LOAD, NULL, NULL,
                         "Failed to locate the main schema resource at '%s'",
                         ctxt->URL, NULL);
    else
      xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAP_FAILED_LOAD, NULL, NULL,
                         "Failed to locate the main schema resource", NULL,
                         NULL);
    goto exit;
  }
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaAssembleByLocation(xmlSchemaValidCtxtPtr vctxt,
                                       xmlSchemaPtr schema, xmlNodePtr node,
                                       const xmlChar *nsName,
                                       const xmlChar *location) {
  int ret = 0;
   ...
   */
  ret = xmlSchemaAddSchemaDoc(pctxt, XML_SCHEMA_SCHEMA_IMPORT, location, NULL,
                              NULL, 0, node, NULL, nsName, &bucket);
  if (ret != 0)
    return (ret);
  if (bucket == NULL) {
    /*
     ...
     */
    xmlSchemaCustomWarning(
        ACTXT_CAST vctxt, XML_SCHEMAV_MISC, node, NULL,
        "The document at location '%s' could not be acquired", location, NULL,
        NULL);
    return (ret);
  }
  /*
   ...
   */
  if ((bucket != NULL) && (WXS_CONSTRUCTOR(pctxt)->bucket == NULL))
    WXS_CONSTRUCTOR(pctxt)->bucket = bucket;
  /*
...
xmlSchemaGetMetaAttrInfo(xmlSchemaValidCtxtPtr vctxt, int metaType) {
...
static int xmlSchemaAssembleByXSI(xmlSchemaValidCtxtPtr vctxt) {
   ...
   */
  iattr =
      xmlSchemaGetMetaAttrInfo(vctxt, XML_SCHEMA_ATTR_INFO_META_XSI_SCHEMA_LOC);
  if (iattr == NULL)
    iattr = xmlSchemaGetMetaAttrInfo(
        vctxt, XML_SCHEMA_ATTR_INFO_META_XSI_NO_NS_SCHEMA_LOC);
  if (iattr == NULL)
    return (0);
  cur = iattr->value;
  do {
    ...
    if (end == cur) {
      if (iattr->metaType == XML_SCHEMA_ATTR_INFO_META_XSI_SCHEMA_LOC) {
         ...
         */
        xmlSchemaCustomWarning(
            ACTXT_CAST vctxt, XML_SCHEMAV_MISC, iattr->node, NULL,
            "The value must consist of tuples: the target namespace "
            "name and the document's URI",
            NULL, NULL, NULL);
      }
      break;
    }
    ...
    cur = end;
    ret = xmlSchemaAssembleByLocation(vctxt, vctxt->schema, iattr->node, nsname,
                                      location);
    if (ret == -1) {
...
static const xmlChar *xmlSchemaLookupNamespace(xmlSchemaValidCtxtPtr vctxt,
  ...
  if (vctxt->sax != NULL) {
    ...
    for (i = vctxt->depth; i >= 0; i--) {
      if (vctxt->elemInfos[i]->nbNsBindings != 0) {
        ...
        for (j = 0; j < inode->nbNsBindings * 2; j += 2) {
          if (((prefix == NULL) && (inode->nsBindings[j] == NULL)) ||
              ((prefix != NULL) && xmlStrEqual(prefix, inode->nsBindings[j]))) {
    ...
    if ((vctxt->inode->node == NULL) || (vctxt->inode->node->doc == NULL)) {
 ...
 */
static int xmlSchemaValidateNotation(xmlSchemaValidCtxtPtr vctxt,
                                     xmlSchemaPtr schema, xmlNodePtr node,
    ...
    return (ret);
  {
    ...
    if (prefix != NULL) {
      ...
      if (xmlSchemaGetNotation(schema, value, NULL) != NULL) {
        if (valNeeded && (val != NULL)) {
          (*val) = xmlSchemaNewNOTATIONValue(BAD_CAST xmlStrdup(value), NULL);
          if (*val == NULL)
            ret = -1;
        }
      } else
...
static int xmlSchemaVAddNodeQName(xmlSchemaValidCtxtPtr vctxt,
                                  const xmlChar *lname, const xmlChar *nsname) {
  int i;
 ...
 */
static void xmlSchemaAugmentIDC(xmlSchemaIDCPtr idcDef,
                                xmlSchemaValidCtxtPtr vctxt) {
  ...
  if (aidc == NULL) {
    xmlSchemaVErrMemory(
        vctxt, "xmlSchemaAugmentIDC: allocating an augmented IDC definition",
        NULL);
    return;
  }
  aidc->keyrefDepth = -1;
   ...
   */
  if ((vctxt->hasKeyrefs == 0) && (idcDef->type == XML_SCHEMA_TYPE_IDC_KEYREF))
    vctxt->hasKeyrefs = 1;
}
 ...
 */
static void xmlSchemaAugmentImportedIDC(xmlSchemaImportPtr imported,
                                        xmlSchemaValidCtxtPtr vctxt) {
  if (imported->schema->idcDef != NULL) {
    xmlHashScan(imported->schema->idcDef, (xmlHashScanner)xmlSchemaAugmentIDC,
                vctxt);
  }
}
...
xmlSchemaIDCNewBinding(xmlSchemaIDCPtr idcDef) {
  ...
  ret = (xmlSchemaPSVIIDCBindingPtr)xmlMalloc(sizeof(xmlSchemaPSVIIDCBinding));
  if (ret == NULL) {
    xmlSchemaVErrMemory(NULL, "allocating a PSVI IDC binding item", NULL);
    return (NULL);
  }
 ...
 */
static int xmlSchemaIDCStoreNodeTableItem(xmlSchemaValidCtxtPtr vctxt,
                                          xmlSchemaPSVIIDCNodePtr item) {
  /*
   * Add to gobal list.
   */
  if (vctxt->idcNodes == NULL) {
    vctxt->idcNodes = (xmlSchemaPSVIIDCNodePtr *)xmlMalloc(
        20 * sizeof(xmlSchemaPSVIIDCNodePtr));
    if (vctxt->idcNodes == NULL) {
      xmlSchemaVErrMemory(vctxt, "allocating the IDC node table item list",
                          NULL);
      return (-1);
    }
    ...
    vctxt->idcNodes = (xmlSchemaPSVIIDCNodePtr *)xmlRealloc(
        vctxt->idcNodes, vctxt->sizeIdcNodes * sizeof(xmlSchemaPSVIIDCNodePtr));
    if (vctxt->idcNodes == NULL) {
      xmlSchemaVErrMemory(vctxt, "re-allocating the IDC node table item list",
                          NULL);
      return (-1);
    }
  }
 ...
 * Returns 0 if succeeded, -1 on internal errors.
 */
static int xmlSchemaIDCStoreKey(xmlSchemaValidCtxtPtr vctxt,
                                xmlSchemaPSVIIDCKeyPtr key) {
  /*
   * Add to gobal list.
   */
  if (vctxt->idcKeys == NULL) {
    vctxt->idcKeys = (xmlSchemaPSVIIDCKeyPtr *)xmlMalloc(
        40 * sizeof(xmlSchemaPSVIIDCKeyPtr));
    if (vctxt->idcKeys == NULL) {
      xmlSchemaVErrMemory(vctxt, "allocating the IDC key storage list", NULL);
      return (-1);
    }
    vctxt->sizeIdcKeys = 40;
  } else if (vctxt->sizeIdcKeys <= vctxt->nbIdcKeys) {
    vctxt->sizeIdcKeys *= 2;
    vctxt->idcKeys = (xmlSchemaPSVIIDCKeyPtr *)xmlRealloc(
        vctxt->idcKeys, vctxt->sizeIdcKeys * sizeof(xmlSchemaPSVIIDCKeyPtr));
    if (vctxt->idcKeys == NULL) {
      xmlSchemaVErrMemory(vctxt, "re-allocating the IDC key storage list",
                          NULL);
      return (-1);
    }
  }
 ...
 */
static int xmlSchemaIDCAppendNodeTableItem(xmlSchemaPSVIIDCBindingPtr bind,
                                           xmlSchemaPSVIIDCNodePtr ntItem) {
  if (bind->nodeTable == NULL) {
    bind->sizeNodes = 10;
    bind->nodeTable = (xmlSchemaPSVIIDCNodePtr *)xmlMalloc(
        10 * sizeof(xmlSchemaPSVIIDCNodePtr));
    if (bind->nodeTable == NULL) {
      xmlSchemaVErrMemory(NULL, "allocating an array of IDC node-table items",
                          NULL);
      return (-1);
    }
  } else if (bind->sizeNodes <= bind->nbNodes) {
    bind->sizeNodes *= 2;
    bind->nodeTable = (xmlSchemaPSVIIDCNodePtr *)xmlRealloc(
        bind->nodeTable, bind->sizeNodes * sizeof(xmlSchemaPSVIIDCNodePtr));
    if (bind->nodeTable == NULL) {
      xmlSchemaVErrMemory(
          NULL, "re-allocating an array of IDC node-table items", NULL);
      return (-1);
    }
  }
 ...
 */
static void xmlSchemaIDCReleaseMatcherList(xmlSchemaValidCtxtPtr vctxt,
                                           xmlSchemaIDCMatcherPtr matcher) {
  xmlSchemaIDCMatcherPtr next;
 ...
 * Returns 0 on success and -1 on internal errors.
 */
static int xmlSchemaIDCAddStateObject(xmlSchemaValidCtxtPtr vctxt,
                                      xmlSchemaIDCMatcherPtr matcher,
                                      xmlSchemaIDCSelectPtr sel, int type) {
  ...
  if (vctxt->xpathStatePool != NULL) {
    ...
    if (sto == NULL) {
      xmlSchemaVErrMemory(NULL, "allocating an IDC state object", NULL);
      return (-1);
    }
   ...
   */
  sto->xpathCtxt =
      (void *)xmlPatternGetStreamCtxt((xmlPatternPtr)sel->xpathComp);
  if (sto->xpathCtxt == NULL) {
 ...
 */
static int xmlSchemaXPathEvaluate(xmlSchemaValidCtxtPtr vctxt,
  ...
  while (sto != head) {
    ...
    if (sto->history == NULL) {
      ...
      if (sto->history == NULL) {
        xmlSchemaVErrMemory(NULL, "allocating the state object history", NULL);
        return (-1);
      }
      sto->sizeHistory = 5;
      ...
      sto->sizeHistory *= 2;
      sto->history =
          (int *)xmlRealloc(sto->history, sto->sizeHistory * sizeof(int));
      if (sto->history == NULL) {
        xmlSchemaVErrMemory(NULL, "re-allocating the state object history",
                            NULL);
        return (-1);
      }
    }
    ...
    if (sto->type == XPATH_STATE_OBJ_TYPE_IDC_SELECTOR) {
      ...
      while (sel != NULL) {
        if (xmlSchemaIDCAddStateObject(vctxt, sto->matcher, sel,
                                       XPATH_STATE_OBJ_TYPE_IDC_FIELD) == -1)
          return (-1);
        sel = sel->next;
      }
       ...
       */
#ifdef DEBUG_IDC
      xmlGenericError(xmlGenericErrorContext, "IDC:     key found\n");
#endif
      /*
       ...
       */
      if (resolved == 0) {
        if ((vctxt->inode->flags & XML_SCHEMA_NODE_INFO_VALUE_NEEDED) == 0)
...
static const xmlChar *xmlSchemaFormatIDCKeySequence(xmlSchemaValidCtxtPtr vctxt,
                                                    xmlChar **buf,
  ...
  for (i = 0; i < count; i++) {
    *buf = xmlStrcat(*buf, BAD_CAST "'");
    res = xmlSchemaGetCanonValueWhtspExt(
        seq[i]->val, xmlSchemaGetWhiteSpaceFacetValue(seq[i]->type), &value);
    if (res == 0)
      *buf = xmlStrcat(*buf, BAD_CAST value);
    else {
 ...
 * Returns 0 on success and -1 on internal errors.
 */
static int xmlSchemaXPathProcessHistory(xmlSchemaValidCtxtPtr vctxt,
                                        int depth) {
  ...
  {
    xmlChar *str = NULL;
    xmlGenericError(xmlGenericErrorContext, "IDC: BACK on %s, depth %d\n",
                    xmlSchemaFormatQName(&str, vctxt->inode->nsName,
                                         vctxt->inode->localName),
                    vctxt->depth);
    FREE_AND_NULL(str)
  ...
  while (sto != NULL) {
    ...
    if (sto->type == XPATH_STATE_OBJ_TYPE_IDC_FIELD) {
      ...
      if (simpleType == NULL) {
         ...
         */
        xmlSchemaCustomErr(
            ACTXT_CAST vctxt, XML_SCHEMAV_CVC_IDC, NULL,
      ...
      if ((key == NULL) && (vctxt->inode->val == NULL)) {
        ...
        if (matcher->keySeqs == NULL) {
          ...
          matcher->keySeqs = (xmlSchemaPSVIIDCKeyPtr **)xmlMalloc(
              matcher->sizeKeySeqs * sizeof(xmlSchemaPSVIIDCKeyPtr *));
          if (matcher->keySeqs == NULL) {
            xmlSchemaVErrMemory(NULL, "allocating an array of key-sequences",
                                NULL);
            return (-1);
          }
          ...
          matcher->keySeqs = (xmlSchemaPSVIIDCKeyPtr **)xmlRealloc(
              matcher->keySeqs,
              matcher->sizeKeySeqs * sizeof(xmlSchemaPSVIIDCKeyPtr *));
          if (matcher->keySeqs == NULL) {
            xmlSchemaVErrMemory(NULL, "reallocating an array of key-sequences",
                                NULL);
            return (-1);
          }
          /*
      ...
      create_key:
         ...
         */
        if (key == NULL) {
          key = (xmlSchemaPSVIIDCKeyPtr)xmlMalloc(sizeof(xmlSchemaPSVIIDCKey));
          if (key == NULL) {
            xmlSchemaVErrMemory(NULL, "allocating a IDC key", NULL);
            xmlFree(keySeq);
       ...
       */
      if ((matcher->keySeqs == NULL) || (matcher->sizeKeySeqs <= pos)) {
      ...
      if ((idc->type != XML_SCHEMA_TYPE_IDC_KEYREF) &&
        ...
        if (i != targets->nbItems) {
           ...
           */
          xmlSchemaCustomErr(
              ACTXT_CAST vctxt, XML_SCHEMAV_CVC_IDC, NULL, WXS_BASIC_CAST idc,
              "Duplicate key-sequence %s in %s",
              xmlSchemaFormatIDCKeySequence(vctxt, &str, (*keySeq), nbKeys),
       ...
       */
      ntItem = (xmlSchemaPSVIIDCNodePtr)xmlMalloc(sizeof(xmlSchemaPSVIIDCNode));
      if (ntItem == NULL) {
      ...
      if (idc->type != XML_SCHEMA_TYPE_IDC_KEYREF) {
         ...
         */
        ntItem->nodeQNameID = xmlSchemaVAddNodeQName(
            vctxt, vctxt->inode->localName, vctxt->inode->nsName);
 ...
 */
static int xmlSchemaIDCRegisterMatchers(xmlSchemaValidCtxtPtr vctxt,
                                        xmlSchemaElementPtr elemDecl) {
  ...
  {
    xmlChar *str = NULL;
    xmlGenericError(xmlGenericErrorContext, "IDC: REGISTER on %s, depth %d\n",
  ...
  do {
    if (idc->type == XML_SCHEMA_TYPE_IDC_KEYREF) {
      ...
      if (refIdc != NULL) {
        ...
        if ((aidc->keyrefDepth == -1) || (vctxt->depth < aidc->keyrefDepth))
      ...
      do {
        ...
        if (nbDupls) {
          ...
          do {
            if (nbFields == 1) {
              res =
                  xmlSchemaAreValuesEqual(keys[0]->val, dupls[j]->keys[0]->val);
              if (res == -1)
                goto internal_error;
              if (res == 1) {
                /*
...
static int xmlSchemaBubbleIDCNodeTables(xmlSchemaValidCtxtPtr vctxt) {
  ...
  xmlSchemaPSVIIDCBindingPtr *parTable,
      parBind = NULL; /* parent IDC bindings. */
  xmlSchemaPSVIIDCNodePtr node, parNode = NULL, *dupls,
                                *parNodes; /* node-table entries. */
  xmlSchemaIDCAugPtr aidc;
    ...
    if (parBind != NULL) {
      ...
      for (i = 0; i < bind->nbNodes; i++) {
        ...
        if (oldDupls) {
          ...
          while (j < oldDupls) {
            if (nbFields == 1) {
              ret = xmlSchemaAreValuesEqual(node->keys[0]->val,
                                            dupls[j]->keys[0]->val);
              if (ret == -1)
                goto internal_error;
              if (ret == 0) {
              ...
              parNode = dupls[j];
              for (k = 0; k < nbFields; k++) {
                ret = xmlSchemaAreValuesEqual(node->keys[k]->val,
                                              parNode->keys[k]->val);
                if (ret == -1)
                  goto internal_error;
                if (ret == 0)
        ...
        if (oldNum) {
          ...
          if (j != oldNum) {
            ...
            if (parBind->nodeTable == NULL) {
              parBind->nodeTable = (xmlSchemaPSVIIDCNodePtr *)xmlMalloc(
                  10 * sizeof(xmlSchemaPSVIIDCNodePtr));
              if (parBind->nodeTable == NULL) {
                xmlSchemaVErrMemory(
                    NULL, "allocating IDC list of node-table items", NULL);
                goto internal_error;
              }
              parBind->sizeNodes = 1;
            } else if (parBind->nbNodes >= parBind->sizeNodes) {
              parBind->sizeNodes *= 2;
              parBind->nodeTable = (xmlSchemaPSVIIDCNodePtr *)xmlRealloc(
                  parBind->nodeTable,
                  parBind->sizeNodes * sizeof(xmlSchemaPSVIIDCNodePtr));
              if (parBind->nodeTable == NULL) {
                xmlSchemaVErrMemory(
                    NULL, "re-allocating IDC list of node-table items", NULL);
                goto internal_error;
              }
            }
            parNodes = parBind->nodeTable;
...
static int xmlSchemaCheckCVCIDCKeyRef(xmlSchemaValidCtxtPtr vctxt) {
   ...
   */
  while (matcher != NULL) {
    if ((matcher->idcType == XML_SCHEMA_TYPE_IDC_KEYREF) && matcher->targets &&
...
xmlSchemaGetFreshAttrInfo(xmlSchemaValidCtxtPtr vctxt) {
  ...
  if (vctxt->attrInfos == NULL) {
    vctxt->attrInfos =
        (xmlSchemaAttrInfoPtr *)xmlMalloc(sizeof(xmlSchemaAttrInfoPtr));
    vctxt->sizeAttrInfos = 1;
    if (vctxt->attrInfos == NULL) {
      xmlSchemaVErrMemory(vctxt, "allocating attribute info list", NULL);
      return (NULL);
    }
  } else if (vctxt->sizeAttrInfos <= vctxt->nbAttrInfos) {
    vctxt->sizeAttrInfos++;
    vctxt->attrInfos = (xmlSchemaAttrInfoPtr *)xmlRealloc(
        vctxt->attrInfos, vctxt->sizeAttrInfos * sizeof(xmlSchemaAttrInfoPtr));
    if (vctxt->attrInfos == NULL) {
      xmlSchemaVErrMemory(vctxt, "re-allocating attribute info list", NULL);
      return (NULL);
    }
  } else {
...
static int xmlSchemaValidatorPushAttribute(xmlSchemaValidCtxtPtr vctxt,
                                           xmlNodePtr attrNode, int nodeLine,
                                           const xmlChar *localName,
                                           const xmlChar *nsName,
                                           int ownedNames, xmlChar *value,
                                           int ownedValue) {
 ...
 */
static void xmlSchemaClearElemInfo(xmlSchemaValidCtxtPtr vctxt,
                                   xmlSchemaNodeInfoPtr ielem) {
  ielem->hasKeyrefs = 0;
...
xmlSchemaGetFreshElemInfo(xmlSchemaValidCtxtPtr vctxt) {
  ...
  if (vctxt->elemInfos == NULL) {
    vctxt->elemInfos =
        (xmlSchemaNodeInfoPtr *)xmlMalloc(10 * sizeof(xmlSchemaNodeInfoPtr));
    if (vctxt->elemInfos == NULL) {
      xmlSchemaVErrMemory(vctxt, "allocating the element info array", NULL);
      return (NULL);
    }
    ...
    vctxt->elemInfos = (xmlSchemaNodeInfoPtr *)xmlRealloc(
        vctxt->elemInfos, vctxt->sizeElemInfos * sizeof(xmlSchemaNodeInfoPtr));
    if (vctxt->elemInfos == NULL) {
      xmlSchemaVErrMemory(vctxt, "re-allocating the element info array", NULL);
      return (NULL);
    }
    /*
  ...
  if (info == NULL) {
    ...
    if (info == NULL) {
      xmlSchemaVErrMemory(vctxt, "allocating an element info", NULL);
      return (NULL);
    }
...
static int xmlSchemaValidateFacets(xmlSchemaAbstractCtxtPtr actxt,
                                   xmlNodePtr node, xmlSchemaTypePtr type,
                                   xmlSchemaValType valType,
                                   const xmlChar *value, xmlSchemaValPtr val,
                                   unsigned long length, int fireErrors) {
  int ret, error = 0;
  ...
  for (facetLink = type->facetSet; facetLink != NULL;
    ...
    case XML_SCHEMA_FACET_MINLENGTH:
    case XML_SCHEMA_FACET_MAXLENGTH:
      ret = xmlSchemaValidateLengthFacetWhtsp(facetLink->facet, valType, value,
                                              val, &len, ws);
      break;
    default:
      ret = xmlSchemaValidateFacetWhtsp(facetLink->facet, ws, valType, value,
                                        val, ws);
      break;
    }
    if (ret < 0) {
      ...
      if (fireErrors)
        xmlSchemaFacetErr(actxt, ret, node, value, len, type, facetLink->facet,
                          NULL, NULL, NULL);
      else
        return (ret);
      if (error == 0)
...
WXS_IS_LIST:
  ...
  for (facetLink = type->facetSet; facetLink != NULL;
    ...
    case XML_SCHEMA_FACET_MINLENGTH:
    case XML_SCHEMA_FACET_MAXLENGTH:
      ret = xmlSchemaValidateListSimpleTypeFacet(facetLink->facet, value,
                                                 length, NULL);
      break;
    default:
      continue;
    ...
    if (ret < 0) {
      ...
      if (fireErrors)
        xmlSchemaFacetErr(actxt, ret, node, value, length, type,
                          facetLink->facet, NULL, NULL, NULL);
      else
        return (ret);
      if (error == 0)
        error = ret;
...
pattern_and_enum:
  if (error >= 0) {
    ...
    if (found && (ret == 0)) {
      ...
      if (fireErrors) {
        xmlSchemaFacetErr(actxt, ret, node, value, 0, type, NULL, NULL, NULL,
                          NULL);
      } else
        return (ret);
      if (error == 0)
  ...
  if (error >= 0) {
    ...
    do {
      ...
      if (found && (ret != 1)) {
        ...
        if (fireErrors) {
          xmlSchemaFacetErr(actxt, ret, node, value, 0, type, facet, NULL, NULL,
                            NULL);
        } else
          return (ret);
        if (error == 0)
...
static xmlChar *xmlSchemaNormalizeValue(xmlSchemaTypePtr type,
                                        const xmlChar *value) {
...
static int xmlSchemaValidateQName(xmlSchemaValidCtxtPtr vctxt,
                                  const xmlChar *value, xmlSchemaValPtr *val,
                                  int valNeeded) {
  int ret;
  const xmlChar *nsName;
  ...
  if (valNeeded && val) {
    if (nsName != NULL)
      *val = xmlSchemaNewQNameValue(BAD_CAST xmlStrdup(nsName), BAD_CAST local);
    else
      *val = xmlSchemaNewQNameValue(NULL, BAD_CAST local);
  } else
    xmlFree(local);
  return (0);
}
  ...
  if (WXS_IS_ANY_SIMPLE_TYPE(type) || WXS_IS_ATOMIC(type)) {
    ...
    if (actxt->type == XML_SCHEMA_CTXT_VALIDATOR) {
      ...
      case XML_SCHEMAS_NOTATION:
        ret = xmlSchemaValidateNotation((xmlSchemaValidCtxtPtr)actxt,
                                        ((xmlSchemaValidCtxtPtr)actxt)->schema,
                                        NULL, value, &val, valNeeded);
        break;
      case XML_SCHEMAS_QNAME:
        ret = xmlSchemaValidateQName((xmlSchemaValidCtxtPtr)actxt, value, &val,
                                     valNeeded);
        break;
      default:
        /* ws = xmlSchemaGetWhiteSpaceFacetValue(type); */
        if (valNeeded)
          ret = xmlSchemaValPredefTypeNodeNoNorm(biType, value, &val, node);
        else
          ret = xmlSchemaValPredefTypeNodeNoNorm(biType, value, NULL, node);
        break;
      }
    } else if (actxt->type == XML_SCHEMA_CTXT_PARSER) {
      switch (biType->builtInType) {
      case XML_SCHEMAS_NOTATION:
        ret = xmlSchemaValidateNotation(NULL,
                                        ((xmlSchemaParserCtxtPtr)actxt)->schema,
                                        node, value, &val, valNeeded);
        break;
      default:
        /* ws = xmlSchemaGetWhiteSpaceFacetValue(type); */
        if (valNeeded)
    ...
    do {
      ...
      if (valNeeded)
        ret = xmlSchemaVCheckCVCSimpleType(actxt, node, itemType, tmpValue,
                                           &curVal, fireErrors, 0, 1);
      else
        ret = xmlSchemaVCheckCVCSimpleType(actxt, node, itemType, tmpValue,
                                           NULL, fireErrors, 0, 1);
    ...
    if ((ret == 0) && (type->flags & XML_SCHEMAS_TYPE_HAS_FACETS)) {
       ...
       */
      ret = xmlSchemaValidateFacets(actxt, node, type, XML_SCHEMAS_UNKNOWN,
                                    value, val, len, fireErrors);
      if (ret != 0) {
        if (ret < 0) {
          AERROR_INT("xmlSchemaVCheckCVCSimpleType",
    ...
    while (memberLink != NULL) {
      if (valNeeded)
        ret = xmlSchemaVCheckCVCSimpleType(actxt, node, memberLink->type, value,
                                           &val, 0, 1, 0);
      else
        ret = xmlSchemaVCheckCVCSimpleType(actxt, node, memberLink->type, value,
                                           NULL, 0, 1, 0);
...
static int xmlSchemaVExpandQName(xmlSchemaValidCtxtPtr vctxt,
                                 const xmlChar *value, const xmlChar **nsName,
                                 const xmlChar **localName) {
  int ret = 0;
  ...
  }
  {
    ...
    *nsName = xmlSchemaLookupNamespace(vctxt, prefix);
    ...
    if (prefix != NULL) {
      ...
      if (*nsName == NULL) {
        xmlSchemaCustomErr(
            ACTXT_CAST vctxt, XML_SCHEMAV_CVC_DATATYPE_VALID_1_2_1, NULL,
            WXS_BASIC_CAST xmlSchemaGetBuiltInType(XML_SCHEMAS_QNAME),
            "The QName value '%s' has no "
            "corresponding namespace declaration in scope",
            value, NULL);
...
static int xmlSchemaProcessXSIType(xmlSchemaValidCtxtPtr vctxt,
                                   xmlSchemaAttrInfoPtr iattr,
  ...
  if (iattr == NULL)
    ...
    *localType = xmlSchemaGetType(vctxt->schema, local, nsName);
    if (*localType == NULL) {
      ...
      xmlSchemaCustomErr(
          ACTXT_CAST vctxt, XML_SCHEMAV_CVC_ELT_4_2, NULL,
          WXS_BASIC_CAST xmlSchemaGetBuiltInType(XML_SCHEMAS_QNAME),
    ...
    if (elemDecl != NULL) {
      ...
      if ((elemDecl->flags & XML_SCHEMAS_ELEM_BLOCK_EXTENSION) ||
          (elemDecl->subtypes->flags & XML_SCHEMAS_TYPE_BLOCK_EXTENSION))
        set |= SUBSET_EXTENSION;

      if ((elemDecl->flags & XML_SCHEMAS_ELEM_BLOCK_RESTRICTION) ||
          (elemDecl->subtypes->flags & XML_SCHEMAS_TYPE_BLOCK_RESTRICTION))
        set |= SUBSET_RESTRICTION;
      ...
      if (xmlSchemaCheckCOSDerivedOK(ACTXT_CAST vctxt, *localType,
        ...
        xmlSchemaCustomErr(
            ACTXT_CAST vctxt, XML_SCHEMAV_CVC_ELT_4_3, NULL, NULL,
            ...
            xmlSchemaFormatQName(&str, (*localType)->targetNamespace,
                                 (*localType)->name),
            NULL);
        FREE_AND_NULL(str);
        ret = vctxt->err;
...
static int xmlSchemaValidateElemDecl(xmlSchemaValidCtxtPtr vctxt) {
  ...
  if (vctxt->nbAttrInfos != 0) {
    ...
    if (iattr) {
       ...
       */
      ret = xmlSchemaVCheckCVCSimpleType(
          ACTXT_CAST vctxt, NULL, xmlSchemaGetBuiltInType(XML_SCHEMAS_BOOLEAN),
...
static int xmlSchemaVAttributesSimple(xmlSchemaValidCtxtPtr vctxt) {
  ...
  for (i = 0; i < vctxt->nbAttrInfos; i++) {
    ...
    if (!iattr->metaType) {
      ACTIVATE_ATTRIBUTE(iattr)
      xmlSchemaIllegalAttrErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_TYPE_3_1_1,
                              iattr, NULL);
...
static int xmlSchemaVAttributesComplex(xmlSchemaValidCtxtPtr vctxt) {
  ...
  for (i = 0; i < nbUses; i++) {
    ...
    if (attrUse->occurs == XML_SCHEMAS_ATTR_USE_REQUIRED) {
      ...
      tmpiattr->decl = attrDecl;
               ...
               ((attrUse->defValue != NULL) || (attrDecl->defValue != NULL))) {
      /*
  ...
  if (type->attributeWildcard != NULL) {
    ...
    for (i = 0; i < nbAttrs; i++) {
      ...
      if (xmlSchemaCheckCVCWildcardNamespace(type->attributeWildcard,
         ...
         */
        iattr->decl = xmlSchemaGetAttributeDecl(vctxt->schema, iattr->localName,
                                                iattr->nsName);
        if (iattr->decl != NULL) {
          iattr->state = XML_SCHEMAS_ATTR_ASSESSED;
          /*
  ...
  for (i = 0; i < vctxt->nbAttrInfos; i++) {
    ...
    if (iattr->state == XML_SCHEMAS_ATTR_DEFAULT) {
      ...
      if (defAttrOwnerElem) {
         ...
         * Normalize the value.
         */
        normValue = xmlSchemaNormalizeValue(iattr->typeDef, iattr->value);
        if (normValue != NULL)
        ...
        if (iattr->nsName == NULL) {
          ...
          ns = xmlSearchNsByHref(defAttrOwnerElem->doc, defAttrOwnerElem,
                                 iattr->nsName);
          if (ns == NULL) {
            xmlChar prefix[12];
            int counter = 0;
            ...
            do {
              ...
              ns = xmlSearchNs(defAttrOwnerElem->doc, defAttrOwnerElem,
                               BAD_CAST prefix);
              if (counter > 1000) {
                VERROR_INT("xmlSchemaVAttributesComplex",
            ...
            ns =
                xmlNewNs(vctxt->validationRoot, iattr->nsName, BAD_CAST prefix);
          }
          /*
           * TODO:
    ...
    if ((iattr->decl->flags & XML_SCHEMAS_ATTR_FIXED) ||
        ((iattr->use != NULL) && (iattr->use->flags & XML_SCHEMAS_ATTR_FIXED)))
      fixed = 1;
    else
      fixed = 0;
    /*
    ...
    if (xpathRes || fixed) {
       ...
       */
      res = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST vctxt, iattr->node,
                                         iattr->typeDef, iattr->value,
                                         &(iattr->val), 1, 1, 0);
    } else {
      res = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST vctxt, iattr->node,
                                         iattr->typeDef, iattr->value, NULL, 1,
                                         0, 0);
    }
    ...
    if (fixed) {
      ...
      if ((iattr->use != NULL) && (iattr->use->defValue != NULL)) {
  ...
  for (i = 0; i < vctxt->nbAttrInfos; i++) {
    ...
    case XML_SCHEMAS_ATTR_ERR_MISSING: {
      xmlChar *str = NULL;
      ACTIVATE_ELEM;
      xmlSchemaCustomErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_COMPLEX_TYPE_4, NULL,
                         NULL, "The attribute '%s' is required but missing",
                         xmlSchemaFormatQName(&str,
    ...
    case XML_SCHEMAS_ATTR_ERR_FIXED_VALUE:
      xmlSchemaCustomErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_AU, NULL, NULL,
    ...
    case XML_SCHEMAS_ATTR_UNKNOWN:
      ...
      if (type->attributeWildcard == NULL) {
        xmlSchemaIllegalAttrErr(
            ACTXT_CAST vctxt, XML_SCHEMAV_CVC_COMPLEX_TYPE_3_2_1, iattr, NULL);
      } else {
        xmlSchemaIllegalAttrErr(
            ACTXT_CAST vctxt, XML_SCHEMAV_CVC_COMPLEX_TYPE_3_2_2, iattr, NULL);
      }
      break;
    default:
      break;
...
static int xmlSchemaValidateElemWildcard(xmlSchemaValidCtxtPtr vctxt,
                                         int *skip) {
  ...
  *skip = 0;
  ...
  }
  {
    ...
    decl = xmlSchemaGetElem(vctxt->schema, vctxt->inode->localName,
                            vctxt->inode->nsName);
    if (decl != NULL) {
  ...
  if (vctxt->nbAttrInfos != 0) {
    ...
    iattr = xmlSchemaGetMetaAttrInfo(vctxt, XML_SCHEMA_ATTR_INFO_META_XSI_TYPE);
    if (iattr != NULL) {
      if (xmlSchemaProcessXSIType(vctxt, iattr, &(vctxt->inode->typeDef),
                                  NULL) == -1) {
...
static int xmlSchemaCheckCOSValidDefault(xmlSchemaValidCtxtPtr vctxt,
  ...
  if (WXS_IS_SIMPLE(inode->typeDef)) {
    ...
    ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST vctxt, NULL, inode->typeDef,
                                       value, val, 1, 1, 0);

  } else if (WXS_HAS_SIMPLE_CONTENT(inode->typeDef)) {

    ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST vctxt, NULL,
                                       inode->typeDef->contentTypeDef, value,
                                       val, 1, 1, 0);
  }
  if (ret < 0) {
...
xmlSchemaVContentModelCallback(xmlSchemaValidCtxtPtr vctxt ATTRIBUTE_UNUSED,
...
#ifdef DEBUG_CONTENT
  {
    ...
    if (item->type == XML_SCHEMA_TYPE_ELEMENT) {
      xmlGenericError(
          xmlGenericErrorContext, "AUTOMATON callback for '%s' [declaration]\n",
          xmlSchemaFormatQName(&str, inode->localName, inode->nsName));
    } else {
      xmlGenericError(
          xmlGenericErrorContext, "AUTOMATON callback for '%s' [wildcard]\n",
          xmlSchemaFormatQName(&str, inode->localName, inode->nsName));
    }
    FREE_AND_NULL(str)
  }
...
static int xmlSchemaVCheckINodeDataType(xmlSchemaValidCtxtPtr vctxt,
                                        xmlSchemaNodeInfoPtr inode,
                                        xmlSchemaTypePtr type,
                                        const xmlChar *value) {
  if (inode->flags & XML_SCHEMA_NODE_INFO_VALUE_NEEDED)
    return (xmlSchemaVCheckCVCSimpleType(ACTXT_CAST vctxt, NULL, type, value,
                                         &(inode->val), 1, 1, 0));
  else
    return (xmlSchemaVCheckCVCSimpleType(ACTXT_CAST vctxt, NULL, type, value,
                                         NULL, 1, 0, 0));
  ...
  if ((inode->typeDef->contentType == XML_SCHEMA_CONTENT_MIXED) ||
    ...
    if ((inode->flags & XML_SCHEMA_ELEM_INFO_ERR_BAD_CONTENT) == 0) {
      ...
      if (inode->regexCtxt == NULL) {
         ...
         */
        inode->regexCtxt = xmlRegNewExecCtxt(
        ...
        xmlGenericError(xmlGenericErrorContext, "AUTOMATON create on '%s'\n",
                        inode->localName);
#endif
      }
      /*
       ...
       */
      xmlRegExecNextValues(inode->regexCtxt, &nbval, &nbneg, &values[0],
                           &terminal);
      ...
      if ((ret < 0) || ((ret == 0) && (!INODE_NILLED(inode)))) {
        ...
        xmlSchemaComplexTypeErr(ACTXT_CAST vctxt, XML_SCHEMAV_ELEMENT_CONTENT,
                                NULL, NULL, "Missing child element(s)", nbval,
                                nbneg, values);
#ifdef DEBUG_AUTOMATA
        xmlGenericError(xmlGenericErrorContext,
        ...
        ret = 0;
#ifdef DEBUG_AUTOMATA
        xmlGenericError(xmlGenericErrorContext, "AUTOMATON succeeded on '%s'\n",
                        inode->localName);
#endif
      }
    }
...
character_content:
  ...
  if (inode->decl == NULL) {
    ...
    if (WXS_IS_SIMPLE(inode->typeDef)) {
      ...
      ret = xmlSchemaVCheckINodeDataType(
          vctxt, inode, inode->typeDef->contentTypeDef, inode->value);
    }
    if (ret < 0) {
      VERROR_INT("xmlSchemaValidatorPopElem",
                 "calling xmlSchemaVCheckCVCSimpleType()");
  ...
  if ((inode->decl->value != NULL) &&
      (inode->flags & XML_SCHEMA_ELEM_INFO_EMPTY) && (!INODE_NILLED(inode))) {
    /*
     * cvc-elt (3.3.4) : 5.1.1
    ...
    if (inode->flags & XML_SCHEMA_ELEM_INFO_LOCAL_TYPE) {
      ...
      ret = xmlSchemaCheckCOSValidDefault(vctxt, inode->decl->value,
                                          &(inode->val));
      if (ret != 0) {
        if (ret < 0) {
          VERROR_INT("xmlSchemaValidatorPopElem",
    ...
    if (WXS_IS_SIMPLE(inode->typeDef)) {
      ...
      ret = xmlSchemaVCheckINodeDataType(
          vctxt, inode, inode->typeDef->contentTypeDef, inode->decl->value);
    }
    if (ret != 0) {
      if (ret < 0) {
        VERROR_INT("xmlSchemaValidatorPopElem",
      ...
      if (inode->flags & XML_SCHEMA_ELEM_INFO_HAS_ELEM_CONTENT) {
        ...
        if (WXS_HAS_MIXED_CONTENT(inode->typeDef)) {
          ...
          if (!xmlStrEqual(inode->value, inode->decl->value)) {
            ...
            xmlSchemaCustomErr(
                ACTXT_CAST vctxt, ret, NULL, NULL,
          ...
          if (!xmlStrEqual(inode->value, inode->decl->value)) {
            ...
            xmlSchemaCustomErr(ACTXT_CAST vctxt, ret, NULL, NULL,
...
end_elem:
...
#ifdef DEBUG_IDC_NODE_TABLE
    xmlSchemaDebugDumpIDCTable(stdout, inode->nsName, inode->localName,
                               inode->idcTable);
#endif
    if ((vctxt->depth > 0) &&
        (vctxt->hasKeyrefs || vctxt->createIDCNodeTables)) {
  ...
  /*
   * VAL TODO: 7 If the element information item is the �validation root�, it
...
static int xmlSchemaValidateChildElem(xmlSchemaValidCtxtPtr vctxt) {
  ...
  if (ptype->builtInType == XML_SCHEMAS_ANYTYPE) {
     ...
     */
    vctxt->inode->decl = xmlSchemaGetElem(
        vctxt->schema, vctxt->inode->localName, vctxt->inode->nsName);
    ...
    if (vctxt->inode->decl == NULL) {
       ...
       */
      iattr =
          xmlSchemaGetMetaAttrInfo(vctxt, XML_SCHEMA_ATTR_INFO_META_XSI_TYPE);
      if (iattr != NULL) {
        ret = xmlSchemaProcessXSIType(vctxt, iattr, &(vctxt->inode->typeDef),
                                      NULL);
        if (ret != 0) {
          if (ret == -1) {
            VERROR_INT("xmlSchemaValidateChildElem",
  ...
  case XML_SCHEMA_CONTENT_ELEMENTS: {
    ...
    if (regexCtxt == NULL) {
      ...
      regexCtxt = xmlRegNewExecCtxt(
          ptype->contModel, (xmlRegExecCallbacks)xmlSchemaVContentModelCallback,
          vctxt);
      if (regexCtxt == NULL) {
        VERROR_INT("xmlSchemaValidateChildElem",
                   "failed to create a regex context");
     ...
     * (Particle) (�3.9.4)."
     */
    ret = xmlRegExecPushString2(regexCtxt, vctxt->inode->localName,
                                vctxt->inode->nsName, vctxt->inode);
#ifdef DEBUG_AUTOMATA
    if (ret < 0)
      xmlGenericError(xmlGenericErrorContext,
    ...
    if (ret < 0) {
      ...
      xmlSchemaComplexTypeErr(ACTXT_CAST vctxt, XML_SCHEMAV_ELEMENT_CONTENT,
                              NULL, NULL, "This element is not expected", nbval,
                              nbneg, values);
      ret = vctxt->err;
      goto unexpected_elem;
    } else
...
static int xmlSchemaVPushText(xmlSchemaValidCtxtPtr vctxt, int nodeType,
                              const xmlChar *value, int len, int mode,
                              int *consumed) {
  /*
  ...
  if ((vctxt->inode->typeDef->contentType == XML_SCHEMA_CONTENT_MIXED) &&
      ((vctxt->inode->decl == NULL) || (vctxt->inode->decl->value == NULL)))
    return (0);
  ...
  if (vctxt->inode->value == NULL) {
    ...
    if (vctxt->inode->flags & XML_SCHEMA_NODE_INFO_FLAG_OWNED_VALUES) {
      vctxt->inode->value =
          BAD_CAST xmlStrncat((xmlChar *)vctxt->inode->value, value, len);
    } else {
      vctxt->inode->value =
    ...
    if (nodeType == XML_ELEMENT_NODE) {
      ...
      if (ret == 1) {
        do {
           ...
           */
          if (xmlSchemaValidatorPushAttribute(
                  vctxt, NULL,
    ...
    leave_elem:
      ...
      if (vctxt->depth >= 0)
        ...
        ielem = NULL;
               ...
               (nodeType == XML_CDATA_SECTION_NODE) || (nodeType == WHTSP) ||
               (nodeType == SIGN_WHTSP)) {
      /*
       * Process character content.
       */
      ...
      ret = xmlSchemaVPushText(vctxt, nodeType, BAD_CAST value, -1,
                               XML_SCHEMA_PUSH_TEXT_CREATED, &consumed);
      if (!consumed)
        xmlFree(value);
      if (ret == -1) {
 ...
 */
static void xmlSchemaSAXHandleCDataSection(void *ctx, const xmlChar *ch,
                                           int len) {
  xmlSchemaValidCtxtPtr vctxt = (xmlSchemaValidCtxtPtr)ctx;
...
static void xmlSchemaSAXHandleReference(void *ctx ATTRIBUTE_UNUSED,
                                        const xmlChar *name ATTRIBUTE_UNUSED) {
  xmlSchemaValidCtxtPtr vctxt = (xmlSchemaValidCtxtPtr)ctx;
...
static void xmlSchemaSAXHandleStartElementNs(
    void *ctx, const xmlChar *localname, const xmlChar *prefix ATTRIBUTE_UNUSED,
    const xmlChar *URI, int nb_namespaces, const xmlChar **namespaces,
    int nb_attributes, int nb_defaulted ATTRIBUTE_UNUSED,
  ...
  if (nb_namespaces != 0) {
    ...
    for (i = 0, j = 0; i < nb_namespaces; i++, j += 2) {
      ...
      if (ielem->nsBindings == NULL) {
        ielem->nsBindings =
            (const xmlChar **)xmlMalloc(10 * sizeof(const xmlChar *));
        if (ielem->nsBindings == NULL) {
          xmlSchemaVErrMemory(
              vctxt, "allocating namespace bindings for SAX validation", NULL);
          goto internal_error;
        }
        ielem->nbNsBindings = 0;
        ...
        ielem->nsBindings = (const xmlChar **)xmlRealloc(
            (void *)ielem->nsBindings,
            ielem->sizeNsBindings * 2 * sizeof(const xmlChar *));
        if (ielem->nsBindings == NULL) {
          xmlSchemaVErrMemory(
              vctxt, "re-allocating namespace bindings for SAX validation",
              NULL);
          goto internal_error;
        }
      }
  ...
  if (nb_attributes != 0) {
    ...
    for (j = 0, i = 0; i < nb_attributes; i++, j += 5) {
       ...
       */
      value =
          xmlStrndup(attributes[j + 3], attributes[j + 4] - attributes[j + 3]);
      /*
       * TODO: Set the node line.
       */
      ret = xmlSchemaValidatorPushAttribute(vctxt, NULL, ielem->nodeLine,
                                            attributes[j], attributes[j + 2], 0,
                                            value, 1);
      if (ret == -1) {
        VERROR_INT("xmlSchemaSAXHandleStartElementNs",
                   "calling xmlSchemaValidatorPushAttribute()");
  ...
  while (node != NULL) {
    ...
    if (node->type == XML_ELEMENT_NODE) {
      ...
      if (node->properties != NULL) {
        ...
        do {
          ...
          ret = xmlSchemaValidatorPushAttribute(
              vctxt, (xmlNodePtr)attr,
              /*
               * Note that we give it the line number of the
               * parent element.
               */
              ielem->nodeLine, attr->name, nsName, 0,
      ...
      if ((ielem != NULL) && (ielem->flags & XML_SCHEMA_ELEM_INFO_EMPTY))
        ielem->flags ^= XML_SCHEMA_ELEM_INFO_EMPTY;
      ret = xmlSchemaVPushText(vctxt, node->type, node->content, -1,
                               XML_SCHEMA_PUSH_TEXT_PERSIST, NULL);
      if (ret < 0) {
...
static int xmlSchemaPreRun(xmlSchemaValidCtxtPtr vctxt) {
  ...
  if (vctxt->schema == NULL) {
     ...
     */
    if ((vctxt->pctxt == NULL) && (xmlSchemaCreatePCtxtOnVCtxt(vctxt) == -1))
      return (-1);
    pctxt = vctxt->pctxt;
    pctxt->xsiAssemble = 1;
    /*
  ...
  xmlHashScan(vctxt->schema->schemasImports,
              (xmlHashScanner)xmlSchemaAugmentImportedIDC, vctxt);
...
int xmlSchemaValidateDoc(xmlSchemaValidCtxtPtr ctxt, xmlDocPtr doc) {
  ...
  if (ctxt->node == NULL) {
    xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAV_DOCUMENT_ELEMENT_MISSING,
                       (xmlNodePtr)doc, NULL,
...
static void externalSubsetSplit(void *ctx, const xmlChar *name,
                                const xmlChar *ExternalID,
                                const xmlChar *SystemID) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
...
static xmlParserInputPtr resolveEntitySplit(void *ctx, const xmlChar *publicId,
                                            const xmlChar *systemId) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
      (ctxt->user_sax->resolveEntity != NULL))
    return (ctxt->user_sax->resolveEntity(ctxt->user_data, publicId, systemId));
  return (NULL);
}
...
static void entityDeclSplit(void *ctx, const xmlChar *name, int type,
  ...
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
    ...
    ctxt->user_sax->entityDecl(ctxt->user_data, name, type, publicId, systemId,
                               content);
}
...
static void attributeDeclSplit(void *ctx, const xmlChar *elem,
  ...
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
    ...
    ctxt->user_sax->attributeDecl(ctxt->user_data, elem, name, type, def,
                                  defaultValue, tree);
  } else {
...
static void elementDeclSplit(void *ctx, const xmlChar *name, int type,
...
static void notationDeclSplit(void *ctx, const xmlChar *name,
...
static void processingInstructionSplit(void *ctx, const xmlChar *target,
                                       const xmlChar *data) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if ((ctxt != NULL) && (ctxt->user_sax != NULL) &&
...
static void XMLCDECL fatalErrorSplit(void *ctx,
...
static void ignorableWhitespaceSplit(void *ctx, const xmlChar *ch, int len) {
  ...
  if (ctxt == NULL)
    return;
  if ((ctxt->user_sax != NULL) && (ctxt->user_sax->ignorableWhitespace != NULL))
...
static void cdataBlockSplit(void *ctx, const xmlChar *value, int len) {
  ...
  if (ctxt == NULL)
    return;
  if ((ctxt->user_sax != NULL) && (ctxt->user_sax->cdataBlock != NULL))
...
static void startElementNsSplit(void *ctx, const xmlChar *localname,
  ...
  if (ctxt == NULL)
    return;
  if ((ctxt->user_sax != NULL) && (ctxt->user_sax->startElementNs != NULL))
    ctxt->user_sax->startElementNs(ctxt->user_data, localname, prefix, URI,
                                   nb_namespaces, namespaces, nb_attributes,
                                   nb_defaulted, attributes);
  if (ctxt->ctxt != NULL)
    xmlSchemaSAXHandleStartElementNs(ctxt->ctxt, localname, prefix, URI,
                                     nb_namespaces, namespaces, nb_attributes,
                                     nb_defaulted, attributes);
}

static void endElementNsSplit(void *ctx, const xmlChar *localname,
                              const xmlChar *prefix, const xmlChar *URI) {
  xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
  if (ctxt == NULL)
    return;
  if ((ctxt->user_sax != NULL) && (ctxt->user_sax->endElementNs != NULL))
...
xmlSchemaSAXPlugPtr xmlSchemaSAXPlug(xmlSchemaValidCtxtPtr ctxt,
  ...
  if ((old_sax != NULL) && (old_sax->initialized != XML_SAX2_MAGIC))
    return (NULL);
  if ((old_sax != NULL) && (old_sax->startElementNs == NULL) &&
      (old_sax->endElementNs == NULL) &&
 ...
 *     number otherwise and -1 in case of an internal or API error.
 */
int xmlSchemaValidateFile(xmlSchemaValidCtxtPtr ctxt, const char *filename,
  ...
  if (input == NULL)
    return (-1);
  ret =
      xmlSchemaValidateStream(ctxt, input, XML_CHAR_ENCODING_NONE, NULL, NULL);
  return (ret);
}
