 ...
 * breakpoint to it to get the context in which the given block is allocated.
 */
...
void *xmlMallocLoc(size_t size, const char *file, int line) {
  ...
  if (xmlMemTraceBlockAt == ret) {
    xmlGenericError(xmlGenericErrorContext, "%p : Malloc(%ld) Ok\n",
...
void *xmlReallocLoc(void *ptr, size_t size, const char *file, int line) {
  ...
  if (xmlMemTraceBlockAt == ptr) {
    xmlGenericError(xmlGenericErrorContext, "%p : Realloced(%ld -> %ld) Ok\n",
...
int xmlMemBlocks(void) { return (debugMemBlocks); }
  ...
  if ((i < 4) && ((buf[i] != 0) || (i == 0))) {
    if (len >= 4) {
      ...
      for (j = 0; (j < len - 3) && (j < 40); j += 4) {
        ...
        if ((p != NULL) && (p == q)) {
          fprintf(fp, " pointer to #%lu at index %d", p->mh_number, j);
          return;
        }
      }
    }
