...
static void xmlWarnMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error,
  ...
  if (ctxt != NULL)
    ctxt->errNo = error;
  __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER, error,
                  XML_ERR_WARNING, NULL, 0, (const char *)str1, NULL, NULL, 0,
                  0, msg, str1);
}
 ...
 */
void xmlSAX2InternalSubset(void *ctx, const xmlChar *name,
...
void xmlSAX2ExternalSubset(void *ctx, const xmlChar *name,
  ...
  if (((ExternalID != NULL) || (SystemID != NULL)) &&
    ...
    ctxt->inputTab =
        (xmlParserInputPtr *)xmlMalloc(5 * sizeof(xmlParserInputPtr));
...
xmlEntityPtr xmlSAX2GetEntity(void *ctx, const xmlChar *name) {
  ...
  if ((ret != NULL) && ((ctxt->validate) || (ctxt->replaceEntities)) &&
      (ret->children == NULL) &&
 ...
 */
void xmlSAX2EntityDecl(void *ctx, const xmlChar *name, int type,
  ...
  if (ctxt->inSubset == 1) {
    ...
    if ((ent == NULL) && (ctxt->pedantic) && (ctxt->sax != NULL) &&
 ...
 */
void xmlSAX2AttributeDecl(void *ctx, const xmlChar *elem,
  ...
  xmlGenericError(xmlGenericErrorContext,
                  ...
                  fullname, type, def, defaultValue);
#endif
  ...
  ctxt->vctxt.valid = 1;
  if (ctxt->inSubset == 1)
    attr = xmlAddAttributeDecl(&ctxt->vctxt, ctxt->myDoc->intSubset, elem, name,
                               prefix, (xmlAttributeType)type,
                               (xmlAttributeDefault)def, defaultValue, tree);
  else if (ctxt->inSubset == 2)
    attr = xmlAddAttributeDecl(&ctxt->vctxt, ctxt->myDoc->extSubset, elem, name,
                               prefix, (xmlAttributeType)type,
                               (xmlAttributeDefault)def, defaultValue, tree);
  else {
 ...
 */
void xmlSAX2ElementDecl(void *ctx, const xmlChar *name, int type,
  ...
  if (ctxt->inSubset == 1)
    elem = xmlAddElementDecl(&ctxt->vctxt, ctxt->myDoc->intSubset, name,
                             (xmlElementTypeVal)type, content);
  else if (ctxt->inSubset == 2)
    elem = xmlAddElementDecl(&ctxt->vctxt, ctxt->myDoc->extSubset, name,
                             (xmlElementTypeVal)type, content);
  else {
    xmlFatalErrMsg(ctxt, XML_ERR_INTERNAL_ERROR,
 ...
 */
void xmlSAX2NotationDecl(void *ctx, const xmlChar *name,
                         const xmlChar *publicId, const xmlChar *systemId) {
  xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
...
void xmlSAX2UnparsedEntityDecl(void *ctx, const xmlChar *name,
  ...
  if (ctxt->inSubset == 1) {
    ent =
        xmlAddDocEntity(ctxt->myDoc, name, XML_EXTERNAL_GENERAL_UNPARSED_ENTITY,
                        publicId, systemId, notationName);
    if ((ent == NULL) && (ctxt->pedantic) && (ctxt->sax != NULL) &&
        (ctxt->sax->warning != NULL))
      ctxt->sax->warning(ctxt->userData,
    ...
    ent =
        xmlAddDtdEntity(ctxt->myDoc, name, XML_EXTERNAL_GENERAL_UNPARSED_ENTITY,
                        publicId, systemId, notationName);
    if ((ent == NULL) && (ctxt->pedantic) && (ctxt->sax != NULL) &&
        (ctxt->sax->warning != NULL))
      ctxt->sax->warning(ctxt->userData,
...
void xmlSAX2EndDocument(void *ctx) {
  ...
  if ((ctxt->inputTab != NULL) && (ctxt->inputNr > 0) &&
      (ctxt->inputTab[0] != NULL) && (ctxt->inputTab[0]->encoding != NULL) &&
      (ctxt->myDoc != NULL) && (ctxt->myDoc->encoding == NULL)) {
 ...
 * the element.
 */
static void xmlSAX2AttributeInternal(void *ctx, const xmlChar *fullname,
                                     const xmlChar *value,
...
#endif
  {
     ...
     */
    ctxt->vctxt.valid = 1;
    nval = xmlValidCtxtNormalizeAttributeValue(&ctxt->vctxt, ctxt->myDoc,
   ...
   */
  if ((!ctxt->html) && (ns == NULL) && (name[0] == 'x') && (name[1] == 'm') &&
     ...
     */
    if (nsret != NULL && ctxt->validate && ctxt->wellFormed && ctxt->myDoc &&
        ctxt->myDoc->intSubset)
  ...
  if ((!ctxt->html) && (ns != NULL) && (ns[0] == 'x') && (ns[1] == 'm') &&
  ...
  if ((!ctxt->html) && ctxt->validate && ctxt->wellFormed && ctxt->myDoc &&
    ...
    if (!ctxt->replaceEntities) {
      ...
      if (val == NULL)
         ...
         */
        nvalnorm = xmlValidNormalizeAttributeValue(ctxt->myDoc, ctxt->node,
                                                   fullname, val);
        if (nvalnorm != NULL) {
          xmlFree(val);
          val = nvalnorm;
 ...
 */
static void xmlCheckDefaultedAttributes(xmlParserCtxtPtr ctxt,
                                        const xmlChar *name,
...
process_external_subset:
  ...
  if (elemDecl != NULL) {
     ...
     */
    if ((ctxt->myDoc->standalone == 1) && (ctxt->myDoc->extSubset != NULL) &&
        (ctxt->validate)) {
      while (attr != NULL) {
        if ((attr->defaultValue != NULL) &&
            (xmlGetDtdQAttrDesc(ctxt->myDoc->extSubset, attr->elem, attr->name,
                                attr->prefix) == attr) &&
            (xmlGetDtdQAttrDesc(ctxt->myDoc->intSubset, attr->elem, attr->name,
                                attr->prefix) == NULL)) {
          xmlChar *fulln;
    ...
    while (attr != NULL) {
      ...
      if (attr->defaultValue != NULL) {
        ...
        if (((attr->prefix != NULL) &&
          ...
          tst = xmlGetDtdQAttrDesc(ctxt->myDoc->intSubset, attr->elem,
 ...
 */
void xmlSAX2StartElement(void *ctx, const xmlChar *fullname,
                         const xmlChar **atts) {
  xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
  xmlNodePtr ret;
  xmlNodePtr parent;
  ...
  if (ctxt->validate && (ctxt->myDoc->extSubset == NULL) &&
    ...
    xmlErrValid(ctxt, XML_ERR_NO_DTD, "Validation failed: no DTD found !", NULL,
                NULL);
    ctxt->validate = 0;
  }
 ...
 */
static xmlNodePtr xmlSAX2TextNode(xmlParserCtxtPtr ctxt, const xmlChar *str,
                                  int len) {
  xmlNodePtr ret;
 ...
 * Returns the newly allocated string or NULL if not needed or error
 */
static xmlChar *xmlSAX2DecodeAttrEntities(xmlParserCtxtPtr ctxt,
                                          const xmlChar *str,
                                          const xmlChar *end) {
  const xmlChar *in;
  xmlChar *ret;
...
decode:
  ctxt->depth++;
  ret = xmlStringLenDecodeEntities(ctxt, str, end - str, XML_SUBSTITUTE_REF, 0,
                                   0, 0);
  ctxt->depth--;
  return (ret);
}
...
static void xmlSAX2AttributeNs(xmlParserCtxtPtr ctxt, const xmlChar *localname,
  ...
  if (ctxt->freeAttrs != NULL) {
    ...
    if (ctxt->dictNames)
      ret = xmlNewNsPropEatName(ctxt->node, namespace, (xmlChar *)localname,
                                NULL);
    else
  ...
  if ((ctxt->replaceEntities == 0) && (!ctxt->html)) {
    ...
    if (*valueend != 0) {
      ...
      ret->children =
          xmlStringLenGetNodeList(ctxt->myDoc, value, valueend - value);
      tmp = ret->children;
      while (tmp != NULL) {
        tmp->doc = ret->doc;
        tmp->parent = (xmlNodePtr)ret;
  ...
  if ((!ctxt->html) && ctxt->validate && ctxt->wellFormed && ctxt->myDoc &&
     ...
     * validation level. Otherwise we have to do specific handling here.
     */
    if ((prefix == ctxt->str_xml) && (localname[0] == 'i') &&
...
void xmlSAX2StartElementNs(void *ctx, const xmlChar *localname,
  ...
  for (i = 0, j = 0; j < nb_namespaces; j++) {
    ...
    if ((!ctxt->html) && ctxt->validate && ctxt->wellFormed && ctxt->myDoc &&
        ctxt->myDoc->intSubset) {
      ctxt->valid &= xmlValidateOneNamespace(&ctxt->vctxt, ctxt->myDoc, ret,
                                             prefix, ns, uri);
    }
#endif /* LIBXML_VALID_ENABLED */
  }
...
void xmlSAX2CDataBlock(void *ctx, const xmlChar *value, int len) {
  ...
  xmlGenericError(xmlGenericErrorContext, "add chars to %s \n",
                  ctxt->node->name);
#endif
  if ((lastChild != NULL) && (lastChild->type == XML_CDATA_SECTION_NODE)) {
