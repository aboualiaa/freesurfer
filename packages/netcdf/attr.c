...
static NC_attr *dup_NC_attr(const NC_attr *rattrp) {
  NC_attr *attrp = new_NC_attr(rattrp->name->cp, rattrp->type, rattrp->nelems);
  if (attrp == NULL)
    return NULL;
...
static int incr_NC_attrarray(NC_attrarray *ncap, NC_attr *newelemp) {
  ...
  if (ncap->nalloc == 0) {
    ...
    vp = (NC_attr **)realloc(ncap->value, (ncap->nalloc + NC_ARRAY_GROWBY) *
                                              sizeof(NC_attr *));
    if (vp == NULL)
      return NC_ENOMEM;
 ...
 */
static int NC_lookupattr(int ncid, int varid,
...
int nc_inq_att(int ncid, int varid,
...
int nc_copy_att(int ncid_in, int varid_in, const char *name, int ncid_out,
                int ovarid) {
  int status;
  NC_attr *iattrp;
  NC *ncp;
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      (void)memcpy(attrp->xvalue, iattrp->xvalue, iattrp->xsz);
...
static int ncx_pad_putn_Iuchar(void **xpp, size_t nelems, const uchar *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Iuchar(const void **xpp, size_t nelems, uchar *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_putn_Ischar(void **xpp, size_t nelems, const schar *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Ischar(const void **xpp, size_t nelems, schar *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_putn_Ishort(void **xpp, size_t nelems, const short *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Ishort(const void **xpp, size_t nelems, short *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_putn_Iint(void **xpp, size_t nelems, const int *tp,
                             nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Iint(const void **xpp, size_t nelems, int *tp,
                             nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Ilong(const void **xpp, size_t nelems, long *tp,
                              nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_putn_Ifloat(void **xpp, size_t nelems, const float *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Ifloat(const void **xpp, size_t nelems, float *tp,
                               nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_putn_Idouble(void **xpp, size_t nelems, const double *tp,
                                nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
static int ncx_pad_getn_Idouble(const void **xpp, size_t nelems, double *tp,
                                nc_type type) {
  switch (type) {
  case NC_CHAR:
    return NC_ECHAR;
...
int nc_put_att_text(int ncid, int varid, const char *name, size_t nelems,
                    const char *value) {
  int status;
  NC *ncp;
  NC_attrarray *ncap;
...
int nc_put_att_schar(int ncid, int varid, const char *name, nc_type type,
                     size_t nelems, const signed char *value) {
  int status;
  NC *ncp;
  NC_attrarray *ncap;
  NC_attr **attrpp;
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Ischar(&xp, nelems, value, type);
      }
...
int nc_put_att_uchar(int ncid, int varid, const char *name, nc_type type,
                     size_t nelems, const unsigned char *value) {
  int status;
  NC *ncp;
  NC_attrarray *ncap;
  NC_attr **attrpp;
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Iuchar(&xp, nelems, value, type);
      }
...
int nc_put_att_short(int ncid, int varid, const char *name, nc_type type,
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Ishort(&xp, nelems, value, type);
      }
...
int nc_put_att_int(int ncid, int varid, const char *name, nc_type type,
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Iint(&xp, nelems, value, type);
      }
...
int nc_put_att_long(int ncid, int varid, const char *name, nc_type type,
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Ilong(&xp, nelems, value, type);
      }
...
int nc_put_att_float(int ncid, int varid, const char *name, nc_type type,
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Ifloat(&xp, nelems, value, type);
      }
...
int nc_put_att_double(int ncid, int varid, const char *name, nc_type type,
  ...
  if (attrpp != NULL) /* name in use */
  {
    if (!NC_indef(ncp)) {
      ...
      if (nelems != 0) {
        void *xp = attrp->xvalue;
        status = ncx_pad_putn_Idouble(&xp, nelems, value, type);
      }
...
int nc_put_att(int ncid, int varid, const char *name, nc_type type,
               size_t nelems, const void *value) {
  switch (type) {
  case NC_BYTE:
    return nc_put_att_schar(ncid, varid, name, type, nelems, (schar *)value);
  case NC_CHAR:
    return nc_put_att_text(ncid, varid, name, nelems, (char *)value);
  case NC_SHORT:
    return nc_put_att_short(ncid, varid, name, type, nelems, (short *)value);
  case NC_INT:
#if (SIZEOF_INT >= X_SIZEOF_INT)
    return nc_put_att_int(ncid, varid, name, type, nelems, (int *)value);
#elif SIZEOF_LONG == X_SIZEOF_INT
    return nc_put_att_long(ncid, varid, name, type, nelems, (long *)value);
#endif
  case NC_FLOAT:
    return nc_put_att_float(ncid, varid, name, type, nelems, (float *)value);
  case NC_DOUBLE:
    return nc_put_att_double(ncid, varid, name, type, nelems, (double *)value);
  }
  return NC_EBADTYPE;
}
...
int nc_get_att(int ncid, int varid, const char *name, void *value) {
  ...
  switch (attrp->type) {
  case NC_BYTE:
    return nc_get_att_schar(ncid, varid, name, (schar *)value);
  case NC_CHAR:
