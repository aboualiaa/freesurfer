 ...
 */
/* $Id: ncx.m4,v 2.47 2004/09/30 18:47:12 russ Exp $ */
  ...
  case 0:
    ...
    if (isp->mant_hi == min.ieee.mant_hi &&
      ...
      *vsp = min.s;
      ...
      unsigned mantissa =
          (isp->mant_hi << 16) | isp->mant_lo_hi << 8 | isp->mant_lo_lo;
...
static void put_ix_float(void *xp, const float *ip) {
  ...
  case 1: {
    ...
    unsigned mantissa = (vsp->mantissa1 << 16) | vsp->mantissa2;
    mantissa >>= 3 - vsp->exp;
 ...
 * of the address.
 */
static size_t word_align(const void *vp) {
...
static void put_ix_float(void *xp, const float *ip) {
  if (word_align(xp) == 0) {
    ...
    if (ieee_exp >= 0xff) {
      ...
      isp->mant = (((csp->mant << 1) & 0xffffffffffff) >> (48 - 23));
    } else if (ieee_exp > -23) {
...
static void get_ix_double(const void *xp, double *ip) {
  ...
  const struct ieee_double *const idp = (const struct ieee_double *)xp;
  {
    ...
    for (ii = 0, lim = dbl_limits;
         ii < sizeof(dbl_limits) / sizeof(struct dbl_limits); ii++, lim++) {
  ...
  {
    unsigned mant_hi = ((idp->mant_6 << 16) | (idp->mant_5 << 8) | idp->mant_4);
...
static void put_ix_double(void *xp, const double *ip) {
...

  {
    ...
    unsigned mant_hi = (vdp->mantissa1 << 13) | (vdp->mantissa2 >> 3);
...
static void put_ix_double(void *xp, const double *ip) {
  ...
  if (ieee_exp >= 0x7ff) {
    ...
    /* assumes cray rep is in normal form */
    assert(csp->mant & 0x800000000000);
    idp->mant = (((csp->mant << 1) & 0xffffffffffff) << (52 - 48));
...
#elif vax
int ncx_getn_float_float(const void **xpp, size_t nfloats, float *ip) {
...
#elif vax
int ncx_putn_float_float(void **xpp, size_t nfloats, const float *ip) {
...
#elif vax
int ncx_getn_double_double(const void **xpp, size_t ndoubles, double *ip) {
  ...
  while (ip < end) {
    ...
    {
      unsigned mant_hi =
          ((idp->mant_6 << 16) | (idp->mant_5 << 8) | idp->mant_4);
      unsigned mant_lo = SWAP4(idp->mant_lo);
      vdp->mantissa1 = (mant_hi >> 13);
      vdp->mantissa2 = ((mant_hi & MASK(13)) << 3) | (mant_lo >> 29);
      vdp->mantissa3 = (mant_lo >> 13);
...
#elif vax
int ncx_putn_double_double(void **xpp, size_t ndoubles, const double *ip) {
