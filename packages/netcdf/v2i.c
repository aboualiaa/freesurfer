 ...
 */
/* $Id: v2i.c,v 1.28 2004/09/14 13:41:23 ed Exp $ */
...
static size_t nvdims(int ncid, int varid) {
  NC *ncp;
...
#define A_DECL(name, type, ndims, rhs) ALLOC_ONSTACK(name, type, ndims)

#define A_FREE(name) FREE_ONSTACK(name)
...
int ncvarputs(int ncid, int varid, const long *start, const long *count,
              const long *stride, const void *value) {
  if (stride == NULL)
...
int ncvargets(int ncid, int varid, const long *start, const long *count,
              const long *stride, void *value) {
  if (stride == NULL)
...
int ncvarputg(int ncid, int varid, const long *start, const long *count,
              const long *stride, const long *map, const void *value) {
  if (map == NULL)
  ...
  /* else */
  {
    ...
    {
      const int status = nc_put_varm(ncid, varid, stp, cntp, strdp, imp, value);
      A_FREE(imp);
      A_FREE(strdp);
      A_FREE(cntp);
      A_FREE(stp);
...
int ncvargetg(int ncid, int varid, const long *start, const long *count,
              const long *stride, const long *map, void *value) {
  if (map == NULL)
  ...
  /* else */
  {
    ...
    A_INIT(imp, ptrdiff_t, ndims, map);
    {
      const int status = nc_get_varm(ncid, varid, stp, cntp, strdp, imp, value);
      A_FREE(imp);
      A_FREE(strdp);
      A_FREE(cntp);
      A_FREE(stp);
...
int ncattinq(int ncid, int varid, const char *name, nc_type *datatype,
  ...
  if (status != NC_NOERR) {
    nc_advise("ncattinq", status, "ncid %d; varid %d; attname \"%s\"", ncid,
              varid, name);
    return -1;
  }
...
int ncattcopy(int ncid_in, int varid_in, const char *name, int ncid_out,
              int varid_out) {
...
#ifndef NO_NETCDF_2
  /* else */
  nc_advise("nctypelen", NC_EBADTYPE, "Unknown type %d", (int)type);
#endif /* NO_NETCDF_2 */
  return -1;
}
