 ...
 */
/* $Id: nc.c,v 2.135 2004/09/30 18:47:12 russ Exp $ */
...
#define NC_NUMRECS_EXTENT 4
  status = ncp->nciop->get(ncp->nciop, NC_NUMRECS_OFFSET, NC_NUMRECS_EXTENT, 0,
                           (void **)&xp);
  /* cast away const */
  if (status != NC_NOERR)
    return status;
...
int write_numrecs(NC *ncp) {
  ...
  status = ncp->nciop->get(ncp->nciop, NC_NUMRECS_OFFSET, NC_NUMRECS_EXTENT,
                           RGN_WRITE, &xp);
  if (status != NC_NOERR)
    return status;
...
static int move_recs_r(NC *gnu, NC *old) {
  ...
  for (recno = (int)old_nrecs - 1; recno >= 0; recno--) {
    ...
    for (varid = (int)old->vars.nelems - 1; varid >= 0; varid--) {
      ...
      status = gnu->nciop->move(gnu->nciop, gnu_off, old_off, old_varp->len, 0);
 ...
 *  Flushes I/O buffers.
 */
static int NC_endef(NC *ncp, size_t h_minfree, size_t v_align, size_t v_minfree,
                    size_t r_align) {
  int status = NC_NOERR;
  ...
  if (ncp->old != NULL) {
    ...
    if (ncp->vars.nelems != 0) {
      ...
      } else { /* Even if (ncp->begin_rec == ncp->old->begin_rec)
...
#ifdef LOCKNUMREC
static int NC_init_pe(NC *ncp, int basepe) {
...
int nc__create(const char *path, int ioflags, size_t initialsz,
               size_t *chunksizehintp, int *ncid_ptr) {
  return nc__create_mp(path, ioflags, initialsz, 0, chunksizehintp, ncid_ptr);
}

int nc__create_mp(const char *path, int ioflags, size_t initialsz, int basepe,
                  size_t *chunksizehintp, int *ncid_ptr) {
  NC *ncp;
  int status;
  void *xp = NULL;
  ...
  status = ncio_create(path, ioflags, initialsz, 0, ncp->xsz, &ncp->chunk,
                       &ncp->nciop, &xp);
  if (status != NC_NOERR) {
...
int nc__open(const char *path, int ioflags, size_t *chunksizehintp,
             int *ncid_ptr) {
  return nc__open_mp(path, ioflags, 0, chunksizehintp, ncid_ptr);
}

int nc__open_mp(const char *path, int ioflags, int basepe,
                size_t *chunksizehintp, int *ncid_ptr) {
  NC *ncp;
  int status;
...
int nc_delete(const char *path) { return nc_delete_mp(path, 0); }
...
int nc_delete_mp(const char *path, int basepe) {
  ...
  if (basepe != 0)
    return NC_EINVAL;
#endif
  status = ncio_open(path, NC_NOWRITE, 0, 0, &ncp->chunk, &ncp->nciop, 0);
  if (status)
    goto unwind_alloc;
...
int nc_set_base_pe(int ncid, int pe) {
  ...
  shmem_short_get((shmem_t *)ncp->lock + LOCKNUMREC_SERVING,
                  (shmem_t *)ncp->lock + LOCKNUMREC_SERVING, 1,
                  ncp->lock[LOCKNUMREC_BASEPE]);

  shmem_short_get((shmem_t *)ncp->lock + LOCKNUMREC_LOCK,
                  (shmem_t *)ncp->lock + LOCKNUMREC_LOCK, 1,
                  ncp->lock[LOCKNUMREC_BASEPE]);
