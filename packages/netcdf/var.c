...
NC_var *new_x_NC_var(NC_string *strp, size_t ndims) {
  ...
  const size_t sz = M_RNDUP(sizeof(NC_var)) + o1 + o2 + ndims * sizeof(size_t);
...
NC_new_var()
 */
static NC_var *new_NC_var(const char *name, nc_type type, size_t ndims,
                          const int *dimids) {
  NC_string *strp;
  NC_var *varp;
...
static NC_var *dup_NC_var(const NC_var *rvarp) {
  NC_var *varp =
      new_NC_var(rvarp->name->cp, rvarp->type, rvarp->ndims, rvarp->dimids);
  if (varp == NULL)
    return NULL;
  ...
  (void)memcpy(varp->shape, rvarp->shape, rvarp->ndims * sizeof(size_t));
  (void)memcpy(varp->dsizes, rvarp->dsizes, rvarp->ndims * sizeof(size_t));
  varp->xsz = rvarp->xsz;
  varp->len = rvarp->len;
 ...
 */
static int incr_NC_vararray(NC_vararray *ncap, NC_var *newelemp) {
  NC_var **vp;
  ...
  if (ncap->nalloc == 0) {
    ...
    vp = (NC_var **)realloc(ncap->value, (ncap->nalloc + NC_ARRAY_GROWBY) *
                                             sizeof(NC_var *));
    if (vp == NULL)
      return NC_ENOMEM;
    ncap->value = vp;
...
static NC_var *elem_NC_vararray(const NC_vararray *ncap, size_t elem) {
  assert(ncap != NULL);
...
int NC_var_shape(NC_var *varp, const NC_dimarray *dims) {
   ...
   */
  for (ip = varp->dimids, op = varp->shape; ip < &varp->dimids[varp->ndims];
       ip++, op++) {
...
out:
  if (varp->xsz <=
      X_UINT_MAX / product) /* if integer multiply will not overflow */
  {
 ...
 */
int NC_check_vlen(NC_var *varp, size_t vlen_max) {
...
int nc_inq_varid(int ncid, const char *name, int *varid_ptr) {
  int status;
  NC *ncp;
  NC_var *varp;
  int varid;
...
int nc_inq_var(int ncid, int varid, char *name, nc_type *typep, int *ndimsp,
...
int nc_rename_var(int ncid, int varid, const char *newname) {
  int status;
  NC *ncp;
  NC_var *varp;
