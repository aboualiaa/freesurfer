...
static int fault_v1hs(v1hs *gsp, size_t extent) {
  ...
  status = gsp->nciop->get(gsp->nciop, gsp->offset, gsp->extent, gsp->flags,
                           &gsp->base);
  if (status)
    return status;
...
static int v1h_get_nc_type(v1hs *gsp, nc_type *typep) {
  ...
  assert(type == NC_BYTE || type == NC_CHAR || type == NC_SHORT ||
         type == NC_INT || type == NC_FLOAT || type == NC_DOUBLE);
...
/* assert(ncstrp->nchars == nchars || ncstrp->nchars - nchars < X_ALIGN); */
  ...
  status = ncx_pad_getn_text((const void **)(&gsp->pos), nchars, ncstrp->cp);
  if (status != ENOERR)
    goto unwind_alloc;
...
static int v1h_put_NC_var(v1hs *psp, const NC_var *varp) {
  ...
  if (status != ENOERR)
    return status;
  status = ncx_putn_int_int(&psp->pos, varp->ndims, varp->dimids);
  if (status != ENOERR)
    return status;
...
static int v1h_get_NC_var(v1hs *gsp, NC_var **varpp) {
  ...
  if (status != ENOERR)
    goto unwind_alloc;
  status = ncx_getn_int_int((const void **)(&gsp->pos), ndims, varp->dimids);
  if (status != ENOERR)
    goto unwind_alloc;
  ...
  if (status != ENOERR)
    goto unwind_alloc;
  status = ncx_get_off_t((const void **)&gsp->pos, &varp->begin,
                         gsp->version == 1 ? 4 : 8);
  if (status != ENOERR)
    goto unwind_alloc;
...
static size_t ncx_len_NC_vararray(const NC_vararray *ncap,
                                  size_t sizeof_off_t) {
  size_t xlen = X_SIZEOF_NCTYPE; /* type */
  xlen += X_SIZEOF_SIZE_T;       /* count */
  if (ncap == NULL)
...
int nc_get_NC(NC *ncp) {
...

  {
    ...
    status =
        ncx_getn_schar_schar((const void **)(&gs.pos), sizeof(magic), magic);
    if (status != ENOERR)
      goto unwind_get;
