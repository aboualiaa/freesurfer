 ...
 */
static NC_dim *new_NC_dim(const char *name, size_t size) {
  NC_string *strp;
  NC_dim *dimp;
...
static NC_dim *dup_NC_dim(const NC_dim *dimp) {
...
int find_NC_Udim(const NC_dimarray *ncap, NC_dim **dimpp) {
...

  {
    ...
    for (; (size_t)dimid < ncap->nelems && (*loc)->size != NC_UNLIMITED;
         dimid++, loc++) {
      /*EMPTY*/
    }
 ...
 * we moved a clearer 'break' inside the loop body to the loop test.
 */
static int NC_finddim(const NC_dimarray *ncap, const char *name,
                      NC_dim **dimpp) {
 ...
 */
static int incr_NC_dimarray(NC_dimarray *ncap, NC_dim *newelemp) {
  NC_dim **vp;
  ...
  if (ncap->nalloc == 0) {
    ...
    vp = (NC_dim **)realloc(ncap->value, (ncap->nalloc + NC_ARRAY_GROWBY) *
                                             sizeof(NC_dim *));
    if (vp == NULL)
      return NC_ENOMEM;
    ncap->value = vp;
...
int nc_def_dim(int ncid, const char *name, size_t size, int *dimidp) {
  int status;
  NC *ncp;
  int dimid;
  NC_dim *dimp;
...
int nc_inq_dimid(int ncid, const char *name, int *dimid_ptr) {
  int status;
  NC *ncp;
  int dimid;
...
int nc_inq_dim(int ncid, int dimid, char *name, size_t *sizep) {
  int status;
  NC *ncp;
  NC_dim *dimp;
  ...
  if (name != NULL) {
    (void)strncpy(name, dimp->name->cp, dimp->name->nchars);
...
int nc_rename_dim(int ncid, int dimid, const char *newname) {
  int status;
  NC *ncp;
  int existid;
  NC_dim *dimp;
