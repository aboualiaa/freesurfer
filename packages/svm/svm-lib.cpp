...
bool checkSvmParams(const DoubleMatrix &distMatrix, int posCount, int negCount)
// This function performs basic sanity checks on the parameters to
// the svm functions.
{
  if (distMatrix.rows() != distMatrix.cols()) {
    cerr << "SVM error: the kernel table is not square (" << distMatrix.rows()
         << "x" << distMatrix.cols() << ").\n";
    return false;
  }
  ...
  if (distMatrix.rows() != posCount + negCount) {
    cerr << "SVM error: the size of the kernel table (" << distMatrix.rows()
         << ") does not "
         << "match the size of the data set (" << posCount + negCount << ").\n";
    return false;
  }
...
// structures is correct.
{
  ...
  int status =
      pr_loqo(n, 1, c, h, a, b, l, u, primal, dual, svmParam.verbose - 2,
              svmParam.sigDig, svmParam.maxIterations, svmParam.optEpsilon,
              svmParam.C / 2, 0);
...
// point optimization from pr_loqo.
{
  ...
  if (solveSvmOptimization(alpha, b, kernelMatrix, svmParam, posCount,
                           negCount)) {
    sketch.setKernel(svmParam.kernel);
    return sketch.init(alpha, b, kernelMatrix, posCount, negCount,
...
// returning the set of sketches.
{
  ...
  for (int i = 0; i < dataCount; i++) {
    ...
    // Train
    if (solveSvmOptimization(alpha, b, newKernelMatrix, svmParam, newPosCount,
                             newNegCount)) {
      sketchArray[i].setKernel(svmParam.kernel);
      retCode &=
          sketchArray[i].init(alpha, dataIndex, b, kernelMatrix, newPosCount,
                              newNegCount, svmParam.alphaEpsilon);
    } else
      return false;
  }
  return retCode;
