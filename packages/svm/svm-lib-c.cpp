...
double SVMclassify(SVMreal *vec) {
  if (svmMode != SVM_MODEL_MODE) {
    std::cerr << "SVMclassify error: no classifier model has been created. "
                 "Must train/read in "
...
int SVMweights(SVMreal *weights, SVMreal *vec) {
  if (svmMode != SVM_MODEL_MODE) {
    std::cerr << "SVMweights error: no classifier model has been created. Must "
                 "train/read in "
...
int SVMcrossValidate(double *label, SVMreal **data, int posCount, int negCount,
                     int featureCount) {
  svmMode = SVM_NULL_MODE;
...
int SVMgetFeatureCount(int *count) {
  if (svmMode != SVM_MODEL_MODE) {
    std::cerr << "SVMgetSvCount error: no classifier model has been created. "
                 "Must train/read in "
              << "a classifier before using this function.\n";
    return 0;
  }
...
int SVMgetB(double *b) {
  if (svmMode != SVM_MODEL_MODE) {
    std::cerr << "SVMgetB error: no classifier model has been created. Must "
                 "train/read in "
              << "a classifier before using this function.\n";
    return 0;
  }
...
int SVMgetAlphas(SVMreal *alpha) {
  if (svmMode != SVM_MODEL_MODE) {
    std::cerr << "SVMgetAlphas error: no classifier model has been created. "
                 "Must train/read in "
              << "a classifier before using this function.\n";
    return 0;
  }
...
int SVMgetSvIndex(int *svIndex) {
  if (svmMode != SVM_MODEL_MODE) {
    std::cerr << "SVMgetSvIndex error: no classifier model has been created. "
                 "Must train/read in "
              << "a classifier before using this function.\n";
    return 0;
  }
...
int SVMgetDistMatrix(double **distMatrix) {
  if (svmMode == SVM_NULL_MODE) {
    std::cerr
        << "SVMgetDistMatrix error: no similarity table has been created. Must "
        << "train/cross-validate or read in a classifier before using this "
           "function.\n";
    return 0;
  }
...
void SVMprintSvmOptions() { svmParam.printUsage(std::cerr); }

void SVMparseSvmOptions(char *argv[], int argc) { svmParam.parse(argv, argc); }
...
int SVMparseDataOptions(char *argv[], int argc, int *k, int posCount,
                        int negCount) {
  svmDataParam.init(posCount, negCount);
  return svmDataParam.parse(argv, argc, *k);
}

void SVMprintDataOptions() { DataParam::printUsage(std::cerr); }

void SVMprintDataOptionHelp() { DataParam::printUsageHelp(std::cerr); }

int SVMgetBinaryFlag() { return svmDataParam.binary; }
