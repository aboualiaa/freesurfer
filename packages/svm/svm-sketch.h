...
public:
  ...
  bool init(const SvmRealVector &alpha, double b,
            const DoubleMatrix &kernelMatrix, int posCount, int negCount,
            double threshold = -1);
  ...
  int svCount() const { return _alpha.size(); }

  const Kernel &kernel() const { return _kernel; }

  SvmReal kernel(SvmReal dist) const { return _kernel(dist); }
  ...
  const SvmRealVector &alpha() const { return _alpha; }

  SvmReal alpha(int i) const { return _alpha[i]; }

  const IntVector &svIndex() const { return _svIndex; }

  int svIndex(int i) const { return _svIndex[i]; }

  const DoubleMatrix &kernelMatrix() const { return _kernelMatrix; }

  double kernelMatrix(int i, int j) const { return _kernelMatrix[i][j]; }
