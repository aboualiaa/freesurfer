...
typedef void (*GLUTjoystickFCB)(unsigned int *buttonMask, int *x, int *y,
                                int *z);
#endif
...
#define GLUT_DONT_PROPAGATE_FILTER_MASK                                        \
  (ButtonReleaseMask | ButtonPressMask | KeyPressMask | KeyReleaseMask |       \
   ...
   Button3MotionMask)
#define GLUT_HACK_STOP_PROPAGATE_MASK (KeyPressMask | KeyReleaseMask)
...
#define MAKE_CURRENT_OVERLAY(overlay) wglMakeCurrent(overlay->hdc, overlay->ctx)
#define UNMAKE_CURRENT() wglMakeCurrent(NULL, NULL)
#define SWAP_BUFFERS_WINDOW(window) SwapBuffers(window->hdc)
#define SWAP_BUFFERS_LAYER(window) SwapBuffers(window->renderDc)
#else
#define MAKE_CURRENT_LAYER(window)                                             \
...
#if !defined(_WIN32)
extern void (*__glutMenuItemEnterOrLeave)(GLUTmenuItem *item, int num,
                                          int type);
extern void (*__glutFinishMenu)(Window win, int x, int y);
extern void (*__glutPaintMenu)(GLUTmenu *menu);
extern void (*__glutStartMenu)(GLUTmenu *menu, GLUTwindow *window, int x, int y,
                               int x_win, int y_win);
extern GLUTmenu *(*__glutGetMenuByNum)(int menunum);
extern GLUTmenuItem *(*__glutGetMenuItem)(GLUTmenu *menu, Window win,
                                          int *which);
...
extern XVisualInfo *(*__glutDetermineVisualFromString)(
    char *string, Bool *treatAsSingle, Criterion *requiredCriteria,
    int nRequired, int requiredMask, void **fbc);
...
extern void __glutSetMenuItem(GLUTmenuItem *item, const char *label, int value,
                              Bool isTrigger);
...
extern void __glutSetupColormap(XVisualInfo *vi, GLUTcolormap **colormap,
                                Colormap *cmap);
#if !defined(_WIN32)
...
extern GLUTmenu *__glutGetMenu(Window win);
extern GLUTmenu *__glutGetMenuByNum(int menunum);
extern GLUTmenuItem *__glutGetMenuItem(GLUTmenu *menu, Window win, int *which);
extern void __glutStartMenu(GLUTmenu *menu, GLUTwindow *window, int x, int y,
                            int x_win, int y_win);
extern void __glutFinishMenu(Window win, int x, int y);
#endif
...
extern XVisualInfo *
__glutDetermineVisual(unsigned int mode, Bool *fakeSingle,
...
extern GLUTwindow *__glutCreateWindow(GLUTwindow *parent, int x, int y,
