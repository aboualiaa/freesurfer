...
static void noFaultXAllocColor(Display *dpy, Colormap cmap, int cmapSize,
                               XColor *color) {
...
static void menuVisualSetup(void) {
  ...
  for (layer = 3; layer > 0; layer--) {
    ...
    overlayVisuals = __glutXGetLayerVisualInfo(
        __glutDisplay, VisualScreenMask | VisualLayerMask, &template,
        &nVisuals);
    if (overlayVisuals) {
        ...
        if (visual->vinfo.colormap_size >= 3) {
          if (allocateHigh) {
            ...
            if (numPlaceHolders > 0) {
              placeHolders = (unsigned long *)malloc(numPlaceHolders *
          ...
          if (placeHolders) {
            ...
            XFreeColors(__glutDisplay, menuColormap, placeHolders,
                        numPlaceHolders, 0);
            free(placeHolders);
          }
  ...
  menuWhite = WhitePixel(__glutDisplay, __glutScreen);
  color.red = color.green = color.blue = 0xaa00;
  noFaultXAllocColor(__glutDisplay, menuColormap, menuVisual->map_entries,
                     &color);
  menuGray = color.pixel;
  ...
  presumablyMesa =
      !XQueryExtension(__glutDisplay, "GLX", &dummy, &dummy, &dummy);
...
static void menuSetup(void) {
  ...
  menuFont = XLoadQueryFont(
      __glutDisplay, "-*-helvetica-bold-o-normal--14-*-*-*-p-*-iso8859-1");
  if (!menuFont) {
    /* Try back up font. */
...
static void menuGraphicsContextSetup(Window win) {
  XGCValues gcvals;
  ...
  blackGC = XCreateGC(__glutDisplay, win, GCFont | GCForeground, &gcvals);
...
void __glutSetMenu(GLUTmenu *menu) { __glutCurrentMenu = menu; }

static void unmapMenu(GLUTmenu *menu) {
  if (menu->cascade) {
...
static void finishMenu(Window win, int x, int y) {
  Window dummy;
  int rc;
  ...
  if (__glutMenuWindow->overlay) {
    XInstallColormap(__glutDisplay, __glutMenuWindow->overlay->colormap->cmap);
  }
  ...
  if (__glutMenuStatusFunc) {
    if (win != __glutMenuWindow->win) {
      ...
      rc = XTranslateCoordinates(__glutDisplay, win, __glutMenuWindow->win, x,
                                 y, &x, &y, &dummy);
...
static void mapMenu(GLUTmenu *menu, int x, int y) {
  ...
  if (!menu->managed) {
    ...
    while (item) {
      ...
      XConfigureWindow(__glutDisplay, item->win, CWWidth | CWY, &itemupdate);
      item = item->next;
      num--;
    }
    menu->pixheight = MENU_GAP + fontHeight * menu->num + MENU_GAP;
    changes.height = menu->pixheight;
    changes.width = MENU_GAP + menu->pixwidth + subMenuExtension + MENU_GAP;
  ...
  if (x + menu->pixwidth + subMenuExtension >= __glutScreenWidth) {
    changes.x = __glutScreenWidth - menu->pixwidth + subMenuExtension;
  } else {
    changes.x = x;
  }
...
static void startMenu(GLUTmenu *menu, GLUTwindow *window, int x, int y,
                      int x_win, int y_win) {
  int grab;
  ...
  grab = XGrabPointer(__glutDisplay, __glutRoot, True,
                      ...
                      GrabModeAsync, __glutRoot, menuCursor, CurrentTime);
  if (grab != GrabSuccess) {
...
static void paintSubMenuArrow(Window win, int x, int y) {
  XPoint p[5];
  ...
  XFillPolygon(__glutDisplay, win, whiteGC, p, 4, Convex, CoordModeOrigin);
  XDrawLines(__glutDisplay, win, blackGC, p, 5, CoordModeOrigin);
}

static void paintMenuItem(GLUTmenuItem *item, int num) {
  ...
  XFillRectangle(__glutDisplay, win, gc, MENU_GAP,
                 y - fontHeight + menuFont->descent,
                 item->menu->pixwidth + subMenuExtension, fontHeight);
  XDrawString(__glutDisplay, win, blackGC, MENU_GAP, y, item->label, item->len);
  if (item->isTrigger) {
    paintSubMenuArrow(win, item->menu->pixwidth + MENU_ARROW_GAP + 1, y);
  }
}

static void paintMenu(GLUTmenu *menu) {
  GLUTmenuItem *item;
  int i = menu->num;
  ...
  while (item) {
    if (item->menu->highlighted == item) {
      ...
      XDrawString(__glutDisplay, menu->win, blackGC, 2, y, item->label,
                  item->len);
      if (item->isTrigger) {
        paintSubMenuArrow(menu->win, menu->pixwidth + MENU_ARROW_GAP + 1, y);
      }
    }
    i--;
    y -= fontHeight;
...
static GLUTmenuItem *getMenuItem(GLUTmenu *menu, Window win, int *which) {
  GLUTmenuItem *item;
  int i;
  ...
  while (item) {
    ...
    if (item->isTrigger) {
      ...
      subitem = __glutGetMenuItem(__glutMenuList[item->value], win, which);
      if (subitem) {
        menu->searched = False;
        return subitem;
      }
...
static int getMenuItemIndex(GLUTmenuItem *item) {
  int count = 0;
...
static GLUTmenu *getMenu(Window win) {
  GLUTmenu *menu;
...
static GLUTmenu *getMenuByNum(int menunum) {
...
static int getUnusedMenuSlot(void) {
  ...
  if (__glutMenuList) {
    __glutMenuList =
        (GLUTmenu **)realloc(__glutMenuList, menuListSize * sizeof(GLUTmenu *));
  } else {
...
static void menuItemEnterOrLeave(GLUTmenuItem *item, int num, int type) {
  int alreadyUp = 0;
  ...
  if (type == EnterNotify) {
    ...
    if (item->menu->cascade && item->menu->cascade->anchor == item) {
  ...
  if (item->isTrigger) {
    if (type == EnterNotify && !alreadyUp) {
      ...
      mapMenu(submenu,
              item->menu->x + item->menu->pixwidth + MENU_ARROW_GAP +
                  MENU_ARROW_WIDTH + MENU_GAP + MENU_BORDER,
...
/* Installs callback functions for use by glut_event.c  The point
   ...
   API is used. */
static void installMenuCallbacks(void) {
...
int APIENTRY glutCreateMenu(GLUTselectCB selectFunc) {
...
void __glutSetMenuItem(GLUTmenuItem *item, const char *label, int value,
                       Bool isTrigger) {
  GLUTmenu *menu;
...
void APIENTRY glutAddMenuEntry(const char *label, int value) {
  ...
  entry->win = XCreateWindow(
      __glutDisplay, __glutCurrentMenu->win, MENU_GAP,
      __glutCurrentMenu->num * fontHeight + MENU_GAP, /* x & y */
      entry->pixwidth, fontHeight,                    /* width & height */
      0, CopyFromParent, InputOnly, CopyFromParent, CWEventMask, &wa);
...
void APIENTRY glutAddSubMenu(const char *label, int menu) {
  XSetWindowAttributes wa;
