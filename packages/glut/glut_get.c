...
int APIENTRY glutGet(GLenum param) {
  ...
  case GLUT_WINDOW_X:
    XTranslateCoordinates(__glutDisplay, __glutCurrentWindow->win, __glutRoot,
                          0, 0, &x, &y, &win);
    return x;
  case GLUT_WINDOW_Y:
    XTranslateCoordinates(__glutDisplay, __glutCurrentWindow->win, __glutRoot,
  ...
  case GLUT_WINDOW_WIDTH:
    if (!__glutCurrentWindow->reshape) {
      XGetGeometry(__glutDisplay, __glutCurrentWindow->win, &root, &x, &y,
...
#define GET_CONFIG(attrib)                                                     \
  {                                                                            \
    if (__glutCurrentWindow->renderWin == __glutCurrentWindow->win) {          \
      glXGetConfig(__glutDisplay, __glutCurrentWindow->vis, attrib, &value);   \
    } else {                                                                   \
      glXGetConfig(__glutDisplay, __glutCurrentWindow->overlay->vis, attrib,   \
                   &value);                                                    \
    }                                                                          \
  }
  ...
  case GLUT_WINDOW_PARENT:
    return __glutCurrentWindow->parent ? __glutCurrentWindow->parent->num + 1
                                       : 0;
