...
static char *capstr[] = {"width", "height", "bpp", "hertz", "num"};
#endif
...
static DisplayMode *findMatch(DisplayMode *dmodes, int ndmodes,
  ...
  for (i = 0; i < ndmodes; i++) {
    if (dmodes[i].valid) {
      ...
      for (j = 0; j < ncriteria; j++) {
        ...
        if (verbose)
          printf("  %s %s %d to %d\n", capstr[cap],
                 compstr[criteria[j].comparison], cvalue, dvalue);
#endif
...
static int specialCaseParse(char *word, Criterion *criterion, int mask) {
  ...
  case ':':
    ...
    got = specialCaseParse(response, &criterion[1],
                           1 << DM_WIDTH | 1 << DM_PIXEL_DEPTH);
    if (got >= 0) {
      return got + 1;
    } else {
      return -1;
...
int APIENTRY glutEnterGameMode(void) {
  ...
  if (currentDm) {
    ...
    status = ChangeDisplaySettings(&currentDm->devmode, CDS_FULLSCREEN);
  ...
  window = __glutCreateWindow(NULL, 0, 0, width, height, /* game mode */ 1);
  win = window->win;
  ...
  __glutPutOnWorkList(window, GLUT_CONFIGURE_WORK);
#else
  __glutPutOnWorkList(window, GLUT_CONFIGURE_WORK | GLUT_FULL_SCREEN_WORK);
#endif
