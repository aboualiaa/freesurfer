/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_print.c,v 1.12 2003/08/07 16:42:55
 * dron Exp $ */
...
void TIFFPrintDirectory(TIFF *tif, FILE *fd, long flags) {
  ...
  fprintf(fd, "TIFF Directory at offset 0x%lx\n",
          (long unsigned int)tif->tif_diroff);
  td = &tif->tif_dir;
  ...
  if (TIFFFieldSet(tif, FIELD_IMAGEDIMENSIONS)) {
    ...
    if (TIFFFieldSet(tif, FIELD_IMAGEDEPTH))
      fprintf(fd, " Image Depth: %lu", (u_long)td->td_imagedepth);
    fprintf(fd, "\n");
  }
  ...
  if (TIFFFieldSet(tif, FIELD_IMAGEFULLWIDTH) ||
    ...
    fprintf(fd, "  Pixar Full Image Width: %lu Full Image Length: %lu\n",
            (u_long)td->td_imagefullwidth, (u_long)td->td_imagefulllength);
  }
  ...
  if (TIFFFieldSet(tif, FIELD_TILEDIMENSIONS)) {
    fprintf(fd, "  Tile Width: %lu Tile Length: %lu", (u_long)td->td_tilewidth,
            (u_long)td->td_tilelength);
    if (TIFFFieldSet(tif, FIELD_TILEDEPTH))
      fprintf(fd, " Tile Depth: %lu", (u_long)td->td_tiledepth);
    fprintf(fd, "\n");
  }
  if (TIFFFieldSet(tif, FIELD_RESOLUTION)) {
    ...
    if (TIFFFieldSet(tif, FIELD_RESOLUTIONUNIT)) {
      ...
      case RESUNIT_CENTIMETER:
        ...
        break;
      default:
        fprintf(fd, " (unit %u = 0x%x)", td->td_resolutionunit,
  ...
  if (TIFFFieldSet(tif, FIELD_EXTRASAMPLES) && td->td_extrasamples) {
    ...
    for (i = 0; i < td->td_extrasamples; i++) {
      ...
      case EXTRASAMPLE_UNASSALPHA:
        ...
        break;
      default:
        fprintf(fd, "%s%u (0x%x)", sep, td->td_sampleinfo[i],
                td->td_sampleinfo[i]);
        break;
      }
      sep = ", ";
  ...
  if (TIFFFieldSet(tif, FIELD_YCBCRSUBSAMPLING)) {
    ...
    TIFFGetField(tif, TIFFTAG_YCBCRSUBSAMPLING, subsampling + 0,
                 subsampling + 1);
  ...
  if (TIFFFieldSet(tif, FIELD_YCBCRCOEFFICIENTS))
    fprintf(fd, "  YCbCr Coefficients: %g, %g, %g\n", td->td_ycbcrcoeffs[0],
            td->td_ycbcrcoeffs[1], td->td_ycbcrcoeffs[2]);
#endif
  ...
  if (TIFFFieldSet(tif, FIELD_COLORMAP)) {
    ...
    if (flags & TIFFPRINT_COLORMAP) {
      ...
      for (l = 0; l < n; l++)
        fprintf(fd, "   %5lu: %5u %5u %5u\n", l, td->td_colormap[0][l],
                td->td_colormap[1][l], td->td_colormap[2][l]);
    } else
      fprintf(fd, "(present)\n");
  }
  ...
  if (TIFFFieldSet(tif, FIELD_TRANSFERFUNCTION)) {
    ...
    if (flags & TIFFPRINT_CURVES) {
      ...
      n = 1L << td->td_bitspersample;
      for (l = 0; l < n; l++) {
        fprintf(fd, "    %2lu: %5u", l, td->td_transferfunction[0][l]);
  ...
  */
  {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (fld->field_passcount) {
        ...
        fprintf(fd, "\n");
      } else if (!fld->field_passcount && fld->field_type == TIFF_ASCII) {
        char *data;
  ...
  if ((flags & TIFFPRINT_STRIPS) && TIFFFieldSet(tif, FIELD_STRIPOFFSETS)) {
    ...
    for (s = 0; s < td->td_nstrips; s++)
      fprintf(fd, "    %3lu: [%8lu, %8lu]\n", (u_long)s,
              (u_long)td->td_stripoffset[s], (u_long)td->td_stripbytecount[s]);
  }
}
