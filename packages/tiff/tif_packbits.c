/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_packbits.c,v 1.7 2003/07/08
...
static int PackBitsPreEncode(TIFF *tif, tsample_t s) {
  (void)s;
  /*
 ...
 */
static int PackBitsEncodeChunk(TIFF *tif, tidata_t bp, tsize_t cc,
                               tsample_t s) {
...
static int PackBitsDecode(TIFF *tif, tidata_t op, tsize_t occ, tsample_t s) {
  char *bp;
  tsize_t cc;
  long n;
  int b;
  ...
  tif->tif_rawcc = cc;
  if (occ > 0) {
    TIFFError(tif->tif_name, "PackBitsDecode: Not enough data for scanline %ld",
              (long)tif->tif_row);
    return (0);
  }
  return (1);
