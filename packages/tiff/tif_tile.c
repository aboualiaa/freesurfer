/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_tile.c,v 1.2 2003/11/09 22:04:58
 * warmerda Exp $ */
...
ttile_t TIFFComputeTile(TIFF *tif, uint32 x, uint32 y, uint32 z, tsample_t s) {
  ...
  if (dx != 0 && dy != 0 && dz != 0) {
    ...
    if (td->td_planarconfig == PLANARCONFIG_SEPARATE)
      tile = (xpt * ypt * zpt) * s + (xpt * ypt) * (z / dz) + xpt * (y / dy) +
             x / dx;
    else
...
int TIFFCheckTile(TIFF *tif, uint32 x, uint32 y, uint32 z, tsample_t s) {
  ...
  if (x >= td->td_imagewidth) {
    TIFFError(tif->tif_name, "Col %ld out of range, max %lu", (long)x,
              (u_long)td->td_imagewidth);
    return (0);
  }
  if (y >= td->td_imagelength) {
    TIFFError(tif->tif_name, "Row %ld out of range, max %lu", (long)y,
              (u_long)td->td_imagelength);
    return (0);
  }
  if (z >= td->td_imagedepth) {
    TIFFError(tif->tif_name, "Depth %ld out of range, max %lu", (long)z,
              (u_long)td->td_imagedepth);
    return (0);
  }
  if (td->td_planarconfig == PLANARCONFIG_SEPARATE &&
      s >= td->td_samplesperpixel) {
    TIFFError(tif->tif_name, "Sample %d out of range, max %u", (int)s,
...
tsize_t TIFFVTileSize(TIFF *tif, uint32 nrows) {
  ...
  if (td->td_planarconfig == PLANARCONFIG_CONTIG &&
     ...
     */
    tsize_t w = TIFFroundup(td->td_tilewidth, td->td_ycbcrsubsampling[0]);
