/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_compress.c,v 1.5 2002/04/08
 * 15:32:05 dron Exp $ */
  ...
  if (c) {
    if (!strncmp(c->name, "LZW", 3)) {
      TIFFError(tif->tif_name,
                "%s %s encoding is no longer implemented due to Unisys patent "
                "enforcement",
                c->name, method);
    } else {
      TIFFError(tif->tif_name, "%s %s encoding is not implemented", c->name,
                method);
    }
  } else {
    TIFFError(tif->tif_name,
...
static int TIFFNoDecode(TIFF *tif, char *method) {
  ...
  if (c)
    TIFFError(tif->tif_name, "%s %s decoding is not implemented", c->name,
              method);
  else
    TIFFError(tif->tif_name,
...
TIFFCodec *TIFFRegisterCODEC(uint16 scheme, const char *name,
                             TIFFInitMethod init) {
  codec_t *cd = (codec_t *)_TIFFmalloc(sizeof(codec_t) + sizeof(TIFFCodec) +
                                       strlen(name) + 1);
  ...
  if (cd != NULL) {
    ...
    TIFFError("TIFFRegisterCODEC", "No space to register compression scheme %s",
              name);
  return (cd->info);
}
