/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_predict.c,v 1.4 2003/07/08
 * 16:40:46 warmerda Exp $ */
...
static int PredictorSetup(TIFF *tif) {
  ...
  if (td->td_bitspersample != 8 && td->td_bitspersample != 16) {
    TIFFError(tif->tif_name,
              "Horizontal differencing \"Predictor\" not supported with %d-bit "
              "samples",
              td->td_bitspersample);
    return (0);
  }
 ...
 */
static int PredictorDecodeRow(TIFF *tif, tidata_t op0, tsize_t occ0,
                              tsample_t s) {
  TIFFPredictorState *sp = PredictorState(tif);
 ...
 */
static int PredictorDecodeTile(TIFF *tif, tidata_t op0, tsize_t occ0,
                               tsample_t s) {
  TIFFPredictorState *sp = PredictorState(tif);
...
static int PredictorEncodeTile(TIFF *tif, tidata_t bp0, tsize_t cc0,
                               tsample_t s) {
  TIFFPredictorState *sp = PredictorState(tif);
...
static const TIFFFieldInfo predictFieldInfo[] = {
    {TIFFTAG_PREDICTOR, 1, 1, TIFF_SHORT, FIELD_PREDICTOR, FALSE, FALSE,
     "Predictor"},
};
