...
static
  ...
  if (u <= 0.)
    ...
    ue = itrunc(UVSCALE * u, em);
  if (ue > 255)
    ue = 255;
  if (v <= 0.)
    ve = 0;
  else
    ve = itrunc(UVSCALE * v, em);
  if (ve > 255)
    ve = 255;
  /* combine encodings */
...
static void Luv32fromLuv48(LogLuvState *sp, tidata_t op, int n) {
  ...
  while (n-- > 0) {
    *luv++ = (uint32)luv3[0] << 16 |
             (itrunc(luv3[1] * (UVSCALE / (1 << 15)), sp->encode_meth) << 8 &
              0xff00) |
...
static int LogLuvSetupDecode(TIFF *tif) {
  ...
  default:
    TIFFError(tif->tif_name,
              "Inappropriate photometric interpretation %d for SGILog "
              "compression; %s",
...
static void LogLuvClose(TIFF *tif) {
   ...
   */
  td->td_samplesperpixel = (td->td_photometric == PHOTOMETRIC_LOGL) ? 1 : 3;
...
static int LogLuvVSetField(TIFF *tif, ttag_t tag, va_list ap) {
  ...
  case TIFFTAG_SGILOGDATAFMT:
    ...
    case SGILOGDATAFMT_8BIT:
      ...
      break;
    default:
      TIFFError(tif->tif_name, "Unknown data format %d for LogLuv compression",
                sp->user_datafmt);
      return (0);
    }
  ...
  case TIFFTAG_SGILOGENCODE:
    ...
    if (sp->encode_meth != SGILOGENCODE_NODITHER &&
      ...
      TIFFError(tif->tif_name, "Unknown encoding %d for LogLuv compression",
                sp->encode_meth);
      return (0);
    }
    return (1);
...
static const TIFFFieldInfo LogLuvFieldInfo[] = {
    {TIFFTAG_SGILOGDATAFMT, 0, 0, TIFF_SHORT, FIELD_PSEUDO, TRUE, FALSE,
     "SGILogDataFmt"},
    {TIFFTAG_SGILOGENCODE, 0, 0, TIFF_SHORT, FIELD_PSEUDO, TRUE, FALSE,
     "SGILogEncode"}};
