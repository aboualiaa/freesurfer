/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_dir.c,v 1.32 2003/12/26 10:19:41
 * dron Exp $ */
...
static int _TIFFVSetField(TIFF *tif, ttag_t tag, va_list ap) {
  ...
  case TIFFTAG_ORIENTATION:
    ...
    if (v < ORIENTATION_TOPLEFT || ORIENTATION_LEFTBOT < v) {
      TIFFWarning(tif->tif_name, "Bad value %ld for \"%s\" tag ignored", v,
  ...
  case TIFFTAG_TILEWIDTH:
    ...
    if (v32 % 16) {
      if (tif->tif_mode != O_RDONLY)
        goto badvalue32;
      TIFFWarning(tif->tif_name, "Nonstandard tile width %d, convert file",
                  v32);
    }
    td->td_tilewidth = v32;
  ...
  case TIFFTAG_TILELENGTH:
    ...
    if (v32 % 16) {
      if (tif->tif_mode != O_RDONLY)
        goto badvalue32;
      TIFFWarning(tif->tif_name, "Nonstandard tile length %d, convert file",
                  v32);
    }
    td->td_tilelength = v32;
    tif->tif_flags |= TIFF_ISTILED;
  ...
  case TIFFTAG_SAMPLEFORMAT:
    ...
    if (td->td_sampleformat == SAMPLEFORMAT_COMPLEXINT &&
        td->td_bitspersample == 32 && tif->tif_postdecode == _TIFFSwab32BitData)
      tif->tif_postdecode = _TIFFSwab16BitData;
      ...
      tif->tif_postdecode = _TIFFSwab32BitData;
             ...
             td->td_bitspersample == 128 && tif->tif_postdecode == NULL)
      tif->tif_postdecode = _TIFFSwab64BitData;
    break;
  ...
  case TIFFTAG_PIXAR_MATRIX_WORLDTOSCREEN:
    _TIFFsetFloatArray(&td->td_matrixWorldToScreen, va_arg(ap, float *), 16);
    break;
  case TIFFTAG_PIXAR_MATRIX_WORLDTOCAMERA:
    _TIFFsetFloatArray(&td->td_matrixWorldToCamera, va_arg(ap, float *), 16);
    break;
    /* End Pixar Tags */
  ...
  case TIFFTAG_SUBIFD:
    if ((tif->tif_flags & TIFF_INSUBIFD) == 0) {
      ...
      TIFFError(module, "%s: Sorry, cannot nest SubIFDs", tif->tif_name);
      status = 0;
    }
    break;
  ...
  case TIFFTAG_TRANSFERFUNCTION:
    ...
    for (i = 0; i < v; i++)
      _TIFFsetShortArray(&td->td_transferfunction[i], va_arg(ap, uint16 *),
  ...
  default: {
    ...
    if (fip == NULL || fip->field_bit != FIELD_CUSTOM) {
      TIFFError(module, "%s: Invalid %stag \"%s\" (not supported by codec)",
    ...
    if (tv == NULL) {
      ...
      if (td->td_customValueCount > 1)
        td->td_customValues = (TIFFTagValue *)_TIFFrealloc(
            td->td_customValues,
            sizeof(TIFFTagValue) * td->td_customValueCount);
      else
        td->td_customValues = (TIFFTagValue *)_TIFFmalloc(sizeof(TIFFTagValue));
    ...
    if (fip->field_passcount) {
      ...
      _TIFFmemcpy(tv->value, (void *)va_arg(ap, void *), tv->count * tv_size);
  ...
  return (status);
badvalue:
  TIFFError(module, "%.1000s: Bad value %d for \"%s\"", tif->tif_name, v,
            _TIFFFieldWithTag(tif, tag)->field_name);
  va_end(ap);
  return (0);
badvalue32:
  TIFFError(module, "%.1000s: Bad value %ld for \"%s\"", tif->tif_name, v32,
            _TIFFFieldWithTag(tif, tag)->field_name);
  va_end(ap);
  return (0);
badvaluedbl:
  TIFFError(module, "%.1000s: Bad value %f for \"%s\"", tif->tif_name, d,
            _TIFFFieldWithTag(tif, tag)->field_name);
  va_end(ap);
  return (0);
}
...
static int OkToChangeTag(TIFF *tif, ttag_t tag) {
  const TIFFFieldInfo *fip = _TIFFFindFieldInfo(tif, tag, TIFF_ANY);
  if (!fip) { /* unknown tag */
    TIFFError("TIFFSetField", "%s: Unknown %stag %u", tif->tif_name,
              isPseudoTag(tag) ? "pseudo-" : "", tag);
    return (0);
  }
  if (tag != TIFFTAG_IMAGELENGTH && (tif->tif_flags & TIFF_BEENWRITING) &&
     ...
     */
    TIFFError("TIFFSetField", "%s: Cannot modify tag \"%s\" while writing",
...
static int _TIFFVGetField(TIFF *tif, ttag_t tag, va_list ap) {
  ...
  case TIFFTAG_TILEBYTECOUNTS:
    *va_arg(ap, uint32 **) = td->td_stripbytecount;
    break;
  case TIFFTAG_MATTEING:
    *va_arg(ap, uint16 *) = (td->td_extrasamples == 1 &&
...
static int TIFFAdvanceDirectory(TIFF *tif, uint32 *nextdir, toff_t *off) {
  ...
  if (isMapped(tif)) {
    ...
    if (!SeekOK(tif, *nextdir) || !ReadOK(tif, &dircount, sizeof(uint16))) {
      TIFFError(module, "%s: Error fetching directory count", tif->tif_name);
      return (0);
    }
    if (tif->tif_flags & TIFF_SWAB)
      TIFFSwabShort(&dircount);
    if (off != NULL)
      *off = TIFFSeekFile(tif, dircount * sizeof(TIFFDirEntry), SEEK_CUR);
    else
 ...
 */
uint32 TIFFCurrentDirOffset(TIFF *tif) { return (tif->tif_diroff); }
...
int TIFFReassignTagToIgnore(enum TIFFIgnoreSense task, int TIFFtagID) {
  ...
  case TIS_STORE:
    if (tagcount < (FIELD_LAST - 1)) {
      for (j = 0; j < tagcount; ++j) { /* Do not add duplicate tag */
