...
typedef struct /* This module's private, per-image state variable */
{
  ...
  */
  ...
  int jpegquality,       /* Compression quality level */
      ...
      samplesperclump, scancount; /* No. of scan lines accumulated */
  J_COLOR_SPACE photometric;      /* IJG JPEG Library's photometry code */
  u_char h_sampling,              /* Luminance sampling factors */
      v_sampling, jpegcolormode;  /* Who performs RGB <-> YCbCr conversion? */
...
static const TIFFFieldInfo
    ojpegFieldInfo[] = /* JPEG-specific TIFF-record tags */
    {
        ...
        /* This is the current JPEG-in-TIFF metadata-encapsulation tag, and its
           ...
           "source" image conforming to the TIFF Version 6.0 specification, so
           we arrange to report an error if it appears.  But in order to support
           possible future conversion of "old" JPEG-in-TIFF encapsulations to
           "new" ones, we might wish to synthesize an equivalent value to be
           returned by the TIFF Library's "getfield" method.  So, this table
           tells the TIFF Library to pass these records to us in order to filter
           them below.
        */
        {TIFFTAG_JPEGTABLES, TIFF_VARIABLE, TIFF_VARIABLE, TIFF_UNDEFINED,
         FIELD_JPEGTABLES, FALSE, TRUE, "JPEGTables"},

        /* These tags are defined by the TIFF Version 6.0 specification and are
           now obsolete.  This module reads them from an old "source" image, but
           it never writes them to a new "destination" image.
        */
        {TIFFTAG_JPEGPROC, 1, 1, TIFF_SHORT, FIELD_JPEGPROC, FALSE, FALSE,
         "JPEGProc"},
        {TIFFTAG_JPEGIFOFFSET, 1, 1, TIFF_LONG, FIELD_JPEGIFOFFSET, FALSE,
         FALSE, "JPEGInterchangeFormat"},
        {TIFFTAG_JPEGIFBYTECOUNT, 1, 1, TIFF_LONG, FIELD_JPEGIFBYTECOUNT, FALSE,
        ...
        {TIFFTAG_JPEGLOSSLESSPREDICTORS, TIFF_VARIABLE, TIFF_VARIABLE,
         TIFF_SHORT, FIELD_JPEGLOSSLESSPREDICTORS, FALSE, TRUE,
        ...
        {TIFFTAG_JPEGQTABLES, TIFF_VARIABLE, TIFF_VARIABLE, TIFF_LONG,
         FIELD_JPEGQTABLES, FALSE, TRUE, "JPEGQTables"},
        {TIFFTAG_JPEGDCTABLES, TIFF_VARIABLE, TIFF_VARIABLE, TIFF_LONG,
         FIELD_JPEGDCTABLES, FALSE, TRUE, "JPEGDCTables"},
        {TIFFTAG_JPEGACTABLES, TIFF_VARIABLE, TIFF_VARIABLE, TIFF_LONG,
         FIELD_JPEGACTABLES, FALSE, TRUE, "JPEGACTables"},
        {TIFFTAG_WANG_PAGECONTROL, TIFF_VARIABLE, 1, TIFF_LONG,
         FIELD_WANG_PAGECONTROL, FALSE, FALSE, "WANG PageControl"},

        /* This is a pseudo tag intended for internal use only by the TIFF
           Library and its clients, which should never appear in an input/output
           image file.  It specifies whether the TIFF Library (or its client)
           should do YCbCr <-> RGB color-space conversion (JPEGCOLORMODE_RAW <=>
           0) or whether we should ask the JPEG Library to do it
           (JPEGCOLORMODE_RGB <=> 1).
        */
        {TIFFTAG_JPEGCOLORMODE, 0, 0, TIFF_ANY, FIELD_PSEUDO, FALSE, FALSE,
         "JPEGColorMode"}};
...
*/
static void TIFFojpeg_error_exit(register j_common_ptr cinfo) {
...
static void TIFFojpeg_output_message(register j_common_ptr cinfo) {
  char buffer[JMSG_LENGTH_MAX];
...
static int TIFFojpeg_create_compress(register OJPEGState *sp) {
...
/* The following subroutines comprise a JPEG Library "destination" data manager
   ...
   buffer.
*/
static void std_init_destination(register j_compress_ptr cinfo) {
} /* "Dummy" stub */

static boolean std_empty_output_buffer(register j_compress_ptr cinfo) {
...
static void std_term_destination(register j_compress_ptr cinfo) {
#define sp ((OJPEGState *)cinfo)
  register TIFF *tif = sp->tif;
...
static void tables_init_destination(register j_compress_ptr cinfo) {
#define sp ((OJPEGState *)cinfo)
...
static boolean tables_empty_output_buffer(register j_compress_ptr cinfo) {
  void *newbuf;
#define sp ((OJPEGState *)cinfo)
...
static void tables_term_destination(register j_compress_ptr cinfo) {
#define sp ((OJPEGState *)cinfo)
...
/*ARGSUSED*/ static int TIFFojpeg_tables_dest(register OJPEGState *sp,
                                              TIFF *tif) {
  ...
  */
  ...
  if (!(sp->jpegtables =
            (void *)_TIFFmalloc((tsize_t)(sp->jpegtables_length = 1000)))) {
...
static int _notSupported(register TIFF *tif) {
...
/* The following subroutines comprise a JPEG Library "source" data manager by
   ...
   buffer.
*/
static void std_init_source(register j_decompress_ptr cinfo) {
#define sp ((OJPEGState *)cinfo)
  register TIFF *tif = sp->tif;
...
static boolean std_fill_input_buffer(register j_decompress_ptr cinfo) {
...
static void std_skip_input_data(register j_decompress_ptr cinfo,
                                long num_bytes) {
#define sp ((OJPEGState *)cinfo)
...
/*ARGSUSED*/ static void std_term_source(register j_decompress_ptr cinfo) {
} /* "Dummy" stub */
...
static int alloc_downsampled_buffers(TIFF *tif, jpeg_component_info *comp_info,
  ...
  if (num_components > 0) {
    ...
    do {
      ...
      sp->samplesperclump += compptr->h_samp_factor * compptr->v_samp_factor;
#if defined(C_LOSSLESS_SUPPORTED) || defined(D_LOSSLESS_SUPPORTED)
      if (!(buf = CALLJPEG(sp, 0,
                           (*sp->cinfo.comm.mem->alloc_sarray)(
                               &sp->cinfo.comm, JPOOL_IMAGE,
...
/*ARGSUSED*/ static int OJPEGEncode(register TIFF *tif, tidata_t buf,
                                    tsize_t cc, tsample_t s) {
...
/*ARGSUSED*/ static int OJPEGEncodeRaw(register TIFF *tif, tidata_t buf,
                                       tsize_t cc, tsample_t s) {
  tsize_t rows; /* No. of unprocessed rows in file */
  ...
  while (--cc >= 0) {
    ...
    */
    ...
    if (++sp->scancount >= size) {
      if (CALLJPEG(sp, -1,
                   jpeg_write_raw_data(&sp->cinfo.c, sp->ds_buffer,
                                       lines_per_MCU)) != lines_per_MCU)
        return 0;
      sp->scancount = 0;
    };
    ++tif->tif_row++ buf += sp->bytesperline;
  };
  return 1;
}

static int OJPEGSetupEncode(register TIFF *tif) {
...
#define td (&tif->tif_dir)
  ...
  */
  ...
  case PHOTOMETRIC_YCBCR:
    ...
    */
    if ((td->td_ycbcrsubsampling[0] == 1 || td->td_ycbcrsubsampling[0] == 2 ||
         td->td_ycbcrsubsampling[0] == 4) &&
        (td->td_ycbcrsubsampling[1] == 1 || td->td_ycbcrsubsampling[1] == 2 ||
         td->td_ycbcrsubsampling[1] == 4))
      sp->cinfo.c.raw_data_in =
          ((sp->h_sampling = td->td_ycbcrsubsampling[0]) << 3 |
           (sp->v_sampling = td->td_ycbcrsubsampling[1])) != 011;
    else {
  ...
  L1:
    sp->jpegcolormode = JPEGCOLORMODE_RAW; /* No JPEG Lib. conversion */
  L2:
    sp->cinfo.d.in_color_space = sp->cinfo.d.jpeg_color - space;
    break;
  default:
  ...
  if (isTiled(tif)) {
    ...
    if ((size = sp->v_sampling * sp->cinfo.c.data_unit) < 16)
      size = 16;
#else
    if ((size = sp->v_sampling * DCTSIZE) < 16)
      size = 16;
#endif
    ...
    if ((size = sp->h_sampling * sp->cinfo.c.data_unit) < 16)
      size = 16;
#else
    if ((size = sp->h_sampling * DCTSIZE) < 16)
      size = 16;
#endif
  ...
  */
  ...
  if (!CALLVJPEG(sp, jpeg_set_defaults(&sp->cinfo.c)))
    status = 0;
  ...
  case JPEGTABLESMODE_HUFF:
    if (!CALLVJPEG(sp, jpeg_set_quality(&sp->cinfo.c, sp->jpegquality, FALSE)))
      return 0;
    if (qtbl = sp->cinfo.c.quant_tbl_ptrs[0])
  ...
  case JPEGTABLESMODE_HUFF | JPEGTABLESMODE_QUANT:
    if (!CALLVJPEG(sp,
                   jpeg_set_quality(&sp->cinfo.c, sp->jpegquality, FALSE)) ||
    ...
    if (qtbl = sp->cinfo.c.quant_tbl_ptrs[0])
      qtbl->sent_table = FALSE;
    if (htbl = sp->cinfo.c.dc_huff_tbl_ptrs[0])
      htbl->sent_table = FALSE;
    if (htbl = sp->cinfo.c.ac_huff_tbl_ptrs[0])
      htbl->sent_table = FALSE;
  ...
  if (sp->cinfo.c.raw_data_in &&
    ...
    status = 0;
  if (status == 0)
    return 0; /* If TIFF errors, don't bother to continue */
...
static int OJPEGPreEncode(register TIFF *tif, tsample_t s) {
  register OJPEGState *sp = OJState(tif);
#define td (&tif->tif_dir)
  ...
  /* If we are about to write the first row of an image plane, which should
     ...
     let the compressor run "as is" and return a "success" status without
     further ado.
  */
  if ((isTiled(tif) ? tif->tif_curtile : tif->tif_curstrip) %
    ...
    if ((sp->cinfo.c.comp_info[0].component_id = s) == 1)
           ...
           )
          {
              ...
              /* Scale expected strip/tile size to match a downsampled
               * component. */

              sp->cinfo.c.image_width =
                  TIFFhowmany(segment_width, sp->h_sampling);
              sp->cinfo.c.image_height =
...
static int OJPEGPostEncode(register TIFF *tif) {
  register OJPEGState *sp = OJState(tif);
...
#ifdef C_LOSSLESS_SUPPORTED
    if (sp->scancount < sp->cinfo.c.data_unit && sp->cinfo.c.num_components > 0)
#else
...
#ifdef C_LOSSLESS_SUPPORTED
      {
        ...
        do
          _TIFFmemcpy((tdata_t)sp->ds_buffer[ci][ypos],
                      (tdata_t)sp->ds_buffer[ci][ypos - 1], row_width);
        while (++ypos < compptr->v_samp_factor * size);
      } while (++compptr, ++ci < sp->cinfo.c.num_components);
    };
    n = sp->cinfo.c.max_v_samp_factor * size;
    if (CALLJPEG(sp, -1, jpeg_write_raw_data(&sp->cinfo.c, sp->ds_buffer, n)) !=
        n)
      return 0;
  };
...
/*ARGSUSED*/ static int OJPEGDecode(register TIFF *tif, tidata_t buf,
                                    tsize_t cc, tsample_t s) {
  tsize_t bytesperline =
              isTiled(tif) ? TIFFTileRowSize(tif) : tif->tif_scanlinesize,
          rows; /* No. of unprocessed rows in file */
  register OJPEGState *sp = OJState(tif);

  /* Decode a chunk of pixels, where the input data has not NOT been down-
     sampled, or else the TIFF Library's client has used the "JPEGColorMode"
     TIFF pseudo-tag to request that the JPEG Library do color-space conversion;
     this is the normal case.  The data is expected to be read in scan-line
     multiples, and this subroutine is called for both pixel-interleaved and
  ...
  */
  ...
  while (--cc >= 0) {
    if (CALLJPEG(sp, -1,
                 jpeg_read_scanlines(&sp->cinfo.d, (JSAMPARRAY)&buf, 1)) != 1)
      return 0;
    buf += bytesperline;
    ++tif->tif_row;
  };
  ...
  */
  if (sp->is_WANG)
    jpeg_reset_huff_decode(&sp->cinfo.d);
  return 1;
}

/*ARGSUSED*/ static int OJPEGDecodeRawContig(register TIFF *tif, tidata_t buf,
                                             tsize_t cc, tsample_t s) {
  tsize_t rows; /* No. of unprocessed rows in file */
  JDIMENSION lines_per_MCU, size;
  register OJPEGState *sp = OJState(tif);
  ...
  /* Decode a chunk of pixels, where the input data has pixel-interleaved color
     ...
     expand/duplicate image components according to the image's sampling
     factors, without changing its color space.  The data is expected to be read
     in scan- line multiples.
  */
  ...
  while (--cc >= 0) {
    ...
    {
      if (CALLJPEG(sp, -1,
                   jpeg_read_raw_data(&sp->cinfo.d, sp->ds_buffer,
                                      lines_per_MCU)) != lines_per_MCU)
        return 0;
      sp->scancount = 0;
    };
      ...
      if (compptr->h_samp_factor == 1) /* fast path */
        do {
          register JSAMPLE
              *inptr =
                  sp->ds_buffer[ci]
                               [sp->scancount * compptr->v_samp_factor + ypos],
...
/*ARGSUSED*/ static int OJPEGDecodeRawSeparate(TIFF *tif, register tidata_t buf,
                                               tsize_t cc, tsample_t s) {
  tsize_t rows;                   /* No. of unprocessed rows in file */
  JDIMENSION lines_per_MCU, size, /* ...of MCU */
  ...
  */
  ...
  if ((cc /= compptr->downsampled_width) /* No. of rows in caller's buffer */
      > (rows =
             (sp->cinfo.d.output_height - sp->cinfo.d.output_scanline + v - 1) /
             v))
...
L:
  ...
  do {
    ...
    do
      *buf++ = *inptr++;
    while (--clumps_per_line > 0); /* Copy scanline */
    tif->tif_row += v;
    if (--cc <= 0)
      return 1; /* End of caller's buffer? */
...
*/
/*ARGSUSED*/ METHODDEF(int) fake_SOS_marker(j_decompress_ptr cinfo) {
  return JPEG_REACHED_SOS;
}

/*ARGSUSED*/ METHODDEF(int) suspend(j_decompress_ptr cinfo) {
...
ycc_rgb_convert(register j_decompress_ptr cinfo, JSAMPIMAGE in, JDIMENSION row,
  ...
  case RGB_GREEN:
    ...
    while (--nrows >= 0) {
      register JSAMPROW Cb = *irow1p++, Cr = *irow2p++;
      register int i = cinfo->output_width;
      ...
      while (--i >= 0)
        *outp++ =
            range_limit[*Y++ + RIGHT_SHIFT(table0[*Cb++] + table1[*Cr++], 16)];
    };
    return;
  case RGB_BLUE:
    irow1p = in[1] + row;
...
null_convert(register j_decompress_ptr cinfo, JSAMPIMAGE in, JDIMENSION row,
  ...
  while (--nrows >= 0)
    _TIFFmemcpy(*out++, *irowp++, cinfo->output_width);
}

static int OJPEGSetupDecode(register TIFF *tif) {
...
#define td (&tif->tif_dir)
  ...
  */
  ...
  /* BOGOSITY ALERT!  The Wang Imaging application for Microsoft Windows
     produces images containing "JPEGInterchangeFormat[Length]" TIFF records
     that resemble JFIF-in-TIFF encapsulations but, in fact, violate the TIFF
     Version 6 specification in several ways; nevertheless, we try to handle
     ...
     rest of the JPEG data stream lies in a discontiguous file region,
     identified by the 0th Strip offset (which is *also* illegal!), where it
     begins with an SOS marker and apparently continues to the end of the file.
     There is no trailing EOI marker here, either.
  */
  is_JFIF = !sp->is_WANG && TIFFFieldSet(tif, FIELD_JPEGIFOFFSET);

  /* Initialize decompression parameters that won't be overridden by JPEG
     Library defaults set during the "jpeg_read_header()" call, below.
  */
  segment_width = td->td_imagewidth;
  if (isTiled(tif)) {
    ...
    */
    ...
    sp->bytesperline = TIFFVStripSize(tif, 1);
  ...
  */
  ...
  L:
    if (p - (unsigned char *)sp->jpegtables > 2) /* fake "JPEGTables" */
    {
      ...
      */
      ...
      p[-2] = 0xFF;
      p[-1] = JPEG_EOI; /* Append EOI marker */
      TIFFSetFieldBit(tif, FIELD_JPEGTABLES);
      tif->tif_flags |= TIFF_DIRTYDIRECT;
    } else
      sp->jpegtables = 0; /* Don't simulate "JPEGTables" */
    ...
    */
    ...
    do {
      if ((sp->cinfo.d.comp_info[i].h_samp_factor << 3 |
           sp->cinfo.d.comp_info[i].v_samp_factor) != subsampling_factors) {
  ...
  } else /* not JFIF image */
  {
    ...
    */
    ...
    sp->cinfo.d.marker->read_markers = sp->is_WANG ? suspend : fake_SOS_marker;
    ...
    */
    ...
    sp->cinfo.d.comp_info =
        (jpeg_component_info *)(*sp->cinfo.d.mem->alloc_small)(
            &sp->cinfo.comm, JPOOL_IMAGE,
    ...
    if (sp->is_WANG) /* produced by Wang Imaging on Microsoft Windows */
    {
      ...
      */
      ...
      sp->src.bytes_in_buffer = td->td_stripoffset[i] - td->td_stripoffset[0] +
  ...
  */
  ...
  /* If the image consists of separate, discontiguous TIFF "samples" (= color
     ...
     "buffered image" mode to decompress the entire image into temporary
     buffers, because the JPEG Library must parse the entire JPEG bit-stream in
     order to be satsified that it has a complete set of color components for
     each pixel, but the TIFF Library must allow our client to extract 1
     component at a time. Initializing the JPEG Library's "buffered image" mode
     is tricky:  First, we start its decompressor, then we tell the decompressor
     to "consume" (i.e., buffer) the entire bit-stream.

     WARNING:  Disabling "fancy" up-sampling seems to slightly reduce "noise"
     for certain old Wang Imaging files, but it absolutely *must* be enabled if
     the image has separate color planes, since in that case, the JPEG Library
     doesn't use an "sp->cinfo.d.cconvert" structure (so de-referencing this
     pointer below will cause a fatal crash) but writing our own code to up-
     sample separate color planes is too much work for right now.  Maybe
     someday?
  */
  ...
  if (!CALLJPEG(sp, 0, jpeg_start_decompress(&sp->cinfo.d)))
    return 0;
  ...
  L3:
    switch (CALLJPEG(sp, 0, jpeg_consume_input(&sp->cinfo.d))) {
    default:
      goto L3;
      ...
      */
    case JPEG_SUSPENDED:
      if (sp->cinfo.d.input_scan_number * sp->cinfo.d.image_height +
              sp->cinfo.d.input_iMCU_row * sp->cinfo.d.max_v_samp_factor
#ifdef D_LOSSLESS_SUPPORTED
                  * sp->cinfo.d.data_units_in_MCU *
                  sp->cinfo.d.min_codec_data_unit
#else
                  * sp->cinfo.d.blocks_in_MCU * DCTSIZE
#endif
          < td->td_samplesperpixel * sp->cinfo.d.image_height) {
        TIFFError(tif->tif_name, "Premature end of JPEG bit-stream");
        return 0;
      }
...
static int OJPEGPreDecode(register TIFF *tif, tsample_t s) {
  register OJPEGState *sp = OJState(tif);
#define td (&tif->tif_dir)
...
/*ARGSUSED*/ static void OJPEGPostDecode(register TIFF *tif, tidata_t buf,
                                         tsize_t cc) {
  register OJPEGState *sp = OJState(tif);
#define td (&tif->tif_dir)
  ...
  /* The JPEG Library decompressor has reached the end of a strip/tile.  If this
     ...
     components (color planes), then end the "scan".  If it ends the image's
     last sample/scan, then also stop the JPEG Library's decompressor.
  */
...
static int OJPEGVSetField(register TIFF *tif, ttag_t tag, va_list ap) {
  uint32 v32;
  register OJPEGState *sp = OJState(tif);
#define td (&tif->tif_dir)
  toff_t tiffoff = 0;
  ...
  */
  ...
  case TIFFTAG_JPEGPROC:
    switch (v32) {
    default:
      TIFFError(tif->tif_name, "Unknown JPEG process");
      return 0;
#ifdef C_LOSSLESS_SUPPORTED
    ...
    case JPEGPROC_BASELINE:
      sp->cinfo.d.process = JPROC_SEQUENTIAL;
    ...
    case JPEGPROC_LOSSLESS:
      sp->cinfo.d.process = JPROC_LOSSLESS;
      sp->cinfo.d.data_unit = 1;
#else  /* not C_LOSSLESS_SUPPORTED */
    case JPEGPROC_LOSSLESS:
      TIFFError(JPEGLib_name, "Does not support lossless Huffman coding");
      return 0;
    case JPEGPROC_BASELINE:;
  ...
  case TIFFTAG_JPEGLOSSLESSPREDICTORS:
    if (v32) {
      ...
      sp->jpeglosslesspredictors =
          _TIFFmalloc(sp->jpeglosslesspredictors_length * sizeof(uint16));
      if (sp->jpeglosslesspredictors == NULL) {
        return (0);
      }
  ...
  case TIFFTAG_JPEGPOINTTRANSFORM:
    if (v32) {
      ...
      if (sp->jpegpointtransform == NULL) {
        return (0);
      }
      for (i2 = 0; i2 < sp->jpegpointtransform_length; i2++) {
        ((uint16 *)sp->jpegpointtransform)[i2] = ((uint16 *)sp->cinfo.d.Al)[i2];
      }
  ...
  case TIFFTAG_JPEGQTABLES:
    if (v32) {
      ...
      for (i2 = 0; i2 < sp->jpegqtables_length; i2++) {
        ...
        TIFFReadFile(tif, &(((u_char *)(sp->jpegqtables))[bufoff]), 64);
        bufoff += 64;
      }
    ...
    if (v32) {
      ...
      if (tag == TIFFTAG_JPEGDCTABLES) {
        ...
        for (i2 = 0; i2 < sp->jpegdctables_length; i2++) {
          TIFFSeekFile(tif, v[i2], SEEK_SET);
          TIFFReadFile(tif, &(((u_char *)(sp->jpegdctables))[bufoff]), 16);
          code_count = 0;
          for (k2 = 0; k2 < 16; k2++) {
      ...
      if (tag == TIFFTAG_JPEGACTABLES) {
        ...
        for (i2 = 0; i2 < sp->jpegactables_length; i2++) {
          TIFFSeekFile(tif, v[i2], SEEK_SET);
          TIFFReadFile(tif, &(((unsigned char *)(sp->jpegactables))[bufoff]),
                       16);
          code_count = 0;
          for (k2 = 0; k2 < 16; k2++) {
      ...
      do /* copy each Huffman table */
      {
        ...
        */
        if (!(*h = CALLJPEG(sp, 0, jpeg_alloc_huff_table(&sp->cinfo.comm)))) {
  ...
  case TIFFTAG_JPEGCOLORMODE:
    ...
    */
    ...
    if (td->td_photometric == PHOTOMETRIC_YCBCR &&
        (td->td_ycbcrsubsampling[0] << 3 | td->td_ycbcrsubsampling[1]) != 011 &&
...
static int OJPEGVGetField(register TIFF *tif, ttag_t tag, va_list ap) {
  register OJPEGState *sp = OJState(tif);
...
static void OJPEGPrintDir(register TIFF *tif, FILE *fd, long flags) {
  register OJPEGState *sp = OJState(tif);

  if ((flags & (TIFFPRINT_JPEGQTABLES | TIFFPRINT_JPEGDCTABLES |
...
static uint32 OJPEGDefaultStripSize(register TIFF *tif, register uint32 s) {
  register OJPEGState *sp = OJState(tif);
#define td (&tif->tif_dir)
...
static void OJPEGDefaultTileSize(register TIFF *tif, register uint32 *tw,
                                 register uint32 *th) {
  register OJPEGState *sp = OJState(tif);
...
static void OJPEGCleanUp(register TIFF *tif) {
  ...
  if ((sp = OJState(tif))) {
    ...
    if (sp->jpegqtables) {
      _TIFFfree(sp->jpegqtables);
      sp->jpegqtables = 0;
    }
    if (sp->jpegactables) {
      _TIFFfree(sp->jpegactables);
      sp->jpegactables = 0;
    }
    if (sp->jpegdctables) {
      _TIFFfree(sp->jpegdctables);
      sp->jpegdctables = 0;
    }
...
int TIFFInitOJPEG(register TIFF *tif, int scheme) {
  register OJPEGState *sp;
#define td (&tif->tif_dir)
#ifndef never
  ...
  /* This module supports a decompression-only CODEC, which is intended strictly
     ...
     should, support compression for new images.  If a client application asks
     us to, refuse and complain loudly!
  */
  ...
  sp->err.error_exit = TIFFojpeg_error_exit;
  sp->err.output_message = TIFFojpeg_output_message;
  if (!CALLVJPEG(sp, jpeg_create_decompress(&sp->cinfo.d)))
    return 0;
  ...
  */
  if (tif->tif_header.tiff_diroff > sizeof tif->tif_header) {
    ...
    sp->src.bytes_in_buffer =
        tif->tif_header.tiff_diroff - sizeof tif->tif_header;
  } else /* this case is ugly! */
  {
    ...
    */
    if (tif->tif_nextdiroff)
      maxoffset = tif->tif_nextdiroff; /* Not EOF */
    _TIFFmemcpy(
        &dircount,
        (const tdata_t)(sp->src.next_input_byte =
                            tif->tif_base + tif->tif_header.tiff_diroff),
        sizeof dircount);
    if (tif->tif_flags & TIFF_SWAB)
      TIFFSwabShort(&dircount);
    sp->src.next_input_byte +=
        dircount * sizeof(TIFFDirEntry) + sizeof maxoffset + sizeof dircount;
    sp->src.bytes_in_buffer =
        tif->tif_base - sp->src.next_input_byte + maxoffset;
  };

  /* IJG JPEG Library Version 6B can be configured for either 8- or 12-bit
     sample precision, but we assume that "old JPEG" TIFF clients only need 8
     bits.
  */
