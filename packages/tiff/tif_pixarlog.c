...
static void horizontalAccumulateF(uint16 *wp, int n, int stride, float *op,
  ...
  if (n >= stride) {
    ...
    if (stride == 3) {
      ...
      n -= stride;
      while (n > 0) {
        REPEAT(stride, wp[stride] += *wp; *op = ToLinearF[*wp & mask]; wp++;
               op++)
        n -= stride;
      }
    }
...
#define CLAMP12(t) (((t) < 3071) ? (uint16)(t) : 3071)
  ...
  if (n >= stride) {
    ...
    if (stride == 3) {
      ...
      REPEAT(stride, t0 = ToLinearF[*wp & mask] * SCALE12; *op = CLAMP12(t0);
             wp++; op++)
      n -= stride;
      while (n > 0) {
        REPEAT(stride, wp[stride] += *wp;
               t0 = ToLinearF[wp[stride] & mask] * SCALE12; *op = CLAMP12(t0);
               wp++; op++)
        n -= stride;
      }
    }
...
static void horizontalAccumulate16(uint16 *wp, int n, int stride, uint16 *op,
  ...
  if (n >= stride) {
    ...
    if (stride == 3) {
      ...
      n -= stride;
      while (n > 0) {
        REPEAT(stride, wp[stride] += *wp; *op = ToLinear16[*wp & mask]; wp++;
               op++)
        n -= stride;
      }
    }
...
static void horizontalAccumulate8(uint16 *wp, int n, int stride,
  ...
  if (n >= stride) {
    ...
    if (stride == 3) {
      ...
      n -= stride;
      while (n > 0) {
        REPEAT(stride, wp[stride] += *wp; *op = ToLinear8[*wp & mask]; wp++;
               op++)
        n -= stride;
      }
    }
...
static void horizontalAccumulate8abgr(uint16 *wp, int n, int stride,
                                      unsigned char *op,
...
static int PixarLogMakeTables(PixarLogState *sp) {
  ...
  if (FromLT2 == NULL || From14 == NULL || From8 == NULL || ToLinearF == NULL ||
      ToLinear16 == NULL || ToLinear8 == NULL) {
    if (FromLT2)
      _TIFFfree(FromLT2);
    if (From14)
      _TIFFfree(From14);
...
static int PixarLogSetupDecode(TIFF *tif) {
  ...
  sp->stride =
      (td->td_planarconfig == PLANARCONFIG_CONTIG ? td->td_samplesperpixel : 1);
  sp->tbuf = (uint16 *)_TIFFmalloc(sp->stride * td->td_imagewidth *
                                   td->td_rowsperstrip * sizeof(uint16));
  if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN)
    sp->user_datafmt = PixarLogGuessDataFmt(td);
  if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN) {
    TIFFError(module,
              "PixarLog compression can't handle bits depth/data format "
              "combination (depth: %d)",
              td->td_bitspersample);
    return (0);
  }
 ...
 */
static int PixarLogPreDecode(TIFF *tif, tsample_t s) {
  PixarLogState *sp = DecoderState(tif);
...
static int PixarLogDecode(TIFF *tif, tidata_t op, tsize_t occ, tsample_t s) {
  TIFFDirectory *td = &tif->tif_dir;
  PixarLogState *sp = DecoderState(tif);
  ...
  case PIXARLOGDATAFMT_8BITABGR:
    ...
    break;
  default:
    TIFFError(tif->tif_name, "%d bit input not supported in PixarLog",
              td->td_bitspersample);
    return 0;
  }
  ...
  do {
    ...
    if (state == Z_DATA_ERROR) {
      TIFFError(module, "%s: Decoding error at scanline %d, %s", tif->tif_name,
                tif->tif_row, sp->stream.msg);
      if (inflateSync(&sp->stream) != Z_OK)
        return (0);
      continue;
    }
    if (state != Z_OK) {
      TIFFError(module, "%s: zlib error: %s", tif->tif_name, sp->stream.msg);
      return (0);
    }
  ...
  if (sp->stream.avail_out != 0) {
    TIFFError(module, "%s: Not enough data at scanline %d (short %d bytes)",
  ...
  for (i = 0; i < nsamples; i += llen, up += llen) {
    ...
    case PIXARLOGDATAFMT_FLOAT:
      ...
      break;
    case PIXARLOGDATAFMT_16BIT:
      horizontalAccumulate16(up, llen, sp->stride, (uint16 *)op,
                             sp->ToLinear16);
    ...
    case PIXARLOGDATAFMT_8BITABGR:
      ...
      break;
    default:
      TIFFError(tif->tif_name, "PixarLogDecode: unsupported bits/sample: %d",
                td->td_bitspersample);
      return (0);
    }
  }
 ...
 */
static int PixarLogPreEncode(TIFF *tif, tsample_t s) {
  PixarLogState *sp = EncoderState(tif);
...
static void horizontalDifferenceF(float *ip, int n, int stride, uint16 *wp,
                                  uint16 *FromLT2) {
...
#define CLAMP(v)                                                               \
  ((v < (float)0.)                                                             \
       ? 0                                                                     \
       : (v < (float)2.)                                                       \
             ? FromLT2[(int)(v * fltsize)]                                     \
             : (v > (float)24.2) ? 2047 : LogK1 * log(v * LogK2) + 0.5)
  ...
  if (n >= stride) {
    if (stride == 3) {
      ...
      n -= stride;
      while (n > 0) {
        REPEAT(stride, wp[0] = CLAMP(ip[0]); wp[stride] -= wp[0];
               wp[stride] &= mask; wp--; ip--)
        n -= stride;
      }
 ...
 */
static int PixarLogEncode(TIFF *tif, tidata_t bp, tsize_t cc, tsample_t s) {
  TIFFDirectory *td = &tif->tif_dir;
  PixarLogState *sp = EncoderState(tif);
  ...
  for (i = 0, up = sp->tbuf; i < n; i += llen, up += llen) {
    ...
    case PIXARLOGDATAFMT_FLOAT:
      ...
      break;
    case PIXARLOGDATAFMT_16BIT:
      horizontalDifference16((uint16 *)bp, llen, sp->stride, up, sp->From14);
  ...
  do {
    if (deflate(&sp->stream, Z_NO_FLUSH) != Z_OK) {
      TIFFError(module, "%s: Encoder error: %s", tif->tif_name, sp->stream.msg);
      return (0);
    }
    if (sp->stream.avail_out == 0) {
...
static int PixarLogVSetField(TIFF *tif, ttag_t tag, va_list ap) {
  PixarLogState *sp = (PixarLogState *)tif->tif_data;
  int result;
  ...
  case TIFFTAG_PIXARLOGQUALITY:
    ...
    if (tif->tif_mode != O_RDONLY && (sp->state & PLSTATE_INIT)) {
      if (deflateParams(&sp->stream, sp->quality, Z_DEFAULT_STRATEGY) != Z_OK) {
        TIFFError(module, "%s: zlib error: %s", tif->tif_name, sp->stream.msg);
        return (0);
      }
    }
...
static int PixarLogVGetField(TIFF *tif, ttag_t tag, va_list ap) {
  PixarLogState *sp = (PixarLogState *)tif->tif_data;
