/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_dirwrite.c,v 1.19 2003/12/20
 * 13:40:09 dron Exp $ */
...
static int TIFFWriteShortTable(TIFF *, ttag_t, TIFFDirEntry *, uint32,
                               uint16 **);
static int TIFFWriteShortArray(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                               uint32, uint16 *);
static int TIFFWriteLongArray(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                              uint32, uint32 *);
static int TIFFWriteRationalArray(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                                  uint32, float *);
static int TIFFWriteFloatArray(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                               uint32, float *);
static int TIFFWriteDoubleArray(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                                uint32, double *);
static int TIFFWriteByteArray(TIFF *, TIFFDirEntry *, char *);
static int TIFFWriteAnyArray(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                             uint32, double *);
...
static int TIFFWriteRational(TIFF *, TIFFDataType, ttag_t, TIFFDirEntry *,
                             float);
#endif
...
static int _TIFFWriteDirectory(TIFF *tif, int done) {
  ...
  if (tif->tif_diroff == 0 && !TIFFLinkDirectory(tif))
    goto bad;
  tif->tif_dataoff =
      (toff_t)(tif->tif_diroff + sizeof(uint16) + dirsize + sizeof(toff_t));
    ...
    */
    ...
    case FIELD_STRIPOFFSETS:
      ...
      if (tag != fip->field_tag)
        continue;
      if (!TIFFWriteLongArray(tif, TIFF_LONG, tag, dir, (uint32)td->td_nstrips,
                              td->td_stripoffset))
        goto bad;
      break;
    ...
    case FIELD_ROWSPERSTRIP:
      TIFFSetupShortLong(tif, TIFFTAG_ROWSPERSTRIP, dir, td->td_rowsperstrip);
      break;
    case FIELD_COLORMAP:
    ...
    case FIELD_COMPRESSION:
      TIFFSetupShort(tif, TIFFTAG_COMPRESSION, dir, td->td_compression);
      break;
    case FIELD_PHOTOMETRIC:
      TIFFSetupShort(tif, TIFFTAG_PHOTOMETRIC, dir, td->td_photometric);
      break;
    case FIELD_POSITION:
    ...
    case FIELD_SMAXSAMPLEVALUE:
      if (!TIFFWritePerSampleAnys(tif, _TIFFSampleToTagType(tif),
                                  fip->field_tag, dir))
        goto bad;
      break;
...
int TIFFWriteDirectory(TIFF *tif) { return _TIFFWriteDirectory(tif, TRUE); }
 ...
 */
static int TIFFWriteNormalTag(TIFF *tif, TIFFDirEntry *dir,
...
#define WRITEF(x, y) x(tif, fip->field_type, fip->field_tag, dir, wc, y)
  ...
  case TIFF_SSHORT:
    if (wc > 1) {
      uint16 *wp;
      if (wc == (u_short)TIFF_VARIABLE || fip->field_passcount)
 ...
 */
static void TIFFSetupShortLong(TIFF *tif, ttag_t tag, TIFFDirEntry *dir,
                               uint32 v) {
 ...
 */
static int TIFFWriteRational(TIFF *tif, TIFFDataType type, ttag_t tag,
 ...
 */
static int TIFFWritePerSampleAnys(TIFF *tif, TIFFDataType type, ttag_t tag,
                                  TIFFDirEntry *dir) {
  double buf[10], v;
  double *w = buf;
  int i, status;
 ...
 */
static int TIFFWriteShortTable(TIFF *tif, ttag_t tag, TIFFDirEntry *dir,
                               uint32 n, uint16 **table) {
  uint32 i, off;
 ...
 */
static int TIFFWriteShortArray(TIFF *tif, TIFFDataType type, ttag_t tag,
                               TIFFDirEntry *dir, uint32 n, uint16 *v) {
  dir->tdir_tag = (uint16)tag;
 ...
 */
static int TIFFWriteLongArray(TIFF *tif, TIFFDataType type, ttag_t tag,
                              TIFFDirEntry *dir, uint32 n, uint32 *v) {
  dir->tdir_tag = (uint16)tag;
  dir->tdir_type = (short)type;
  dir->tdir_count = n;
  if (n == 1) {
...
static int TIFFWriteRationalArray(TIFF *tif, TIFFDataType type, ttag_t tag,
                                  TIFFDirEntry *dir, uint32 n, float *v) {
  uint32 i;
  uint32 *t;
  int status;
    ...
    if (fv < 0) {
      if (type == TIFF_RATIONAL) {
        TIFFWarning(tif->tif_name,
                    "\"%s\": Information lost writing value (%g) as (unsigned) "
                    "RATIONAL",
...
static int TIFFWriteFloatArray(TIFF *tif, TIFFDataType type, ttag_t tag,
                               TIFFDirEntry *dir, uint32 n, float *v) {
  dir->tdir_tag = (uint16)tag;
  dir->tdir_type = (short)type;
  dir->tdir_count = n;
...
static int TIFFWriteDoubleArray(TIFF *tif, TIFFDataType type, ttag_t tag,
                                TIFFDirEntry *dir, uint32 n, double *v) {
  dir->tdir_tag = (uint16)tag;
  dir->tdir_type = (short)type;
  dir->tdir_count = n;
...
static int TIFFLinkDirectory(TIFF *tif) {
  ...
  if (tif->tif_flags & TIFF_INSUBIFD) {
    ...
    if (!WriteOK(tif, &diroff, sizeof(diroff))) {
      TIFFError(module, "%s: Error writing SubIFD directory link",
                tif->tif_name);
      return (0);
    }
    /*
