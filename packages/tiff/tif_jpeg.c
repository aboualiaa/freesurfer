/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_jpeg.c,v 1.16 2003/12/20 13:47:28
 * dron Exp $ */
...
static const TIFFFieldInfo jpegFieldInfo[] = {
    {TIFFTAG_JPEGTABLES, -1, -1, TIFF_UNDEFINED, FIELD_JPEGTABLES, FALSE, TRUE,
     "JPEGTables"},
    {TIFFTAG_JPEGQUALITY, 0, 0, TIFF_ANY, FIELD_PSEUDO, TRUE, FALSE, ""},
    {TIFFTAG_JPEGCOLORMODE, 0, 0, TIFF_ANY, FIELD_PSEUDO, FALSE, FALSE, ""},
    {TIFFTAG_JPEGTABLESMODE, 0, 0, TIFF_ANY, FIELD_PSEUDO, FALSE, FALSE, ""},
};
 ...
 */
static int JPEGPreDecode(TIFF *tif, tsample_t s) {
  JPEGState *sp = JState(tif);
  TIFFDirectory *td = &tif->tif_dir;
  ...
  if (td->td_planarconfig == PLANARCONFIG_CONTIG) {
    ...
    if (sp->cinfo.d.comp_info[0].h_samp_factor != sp->h_sampling ||
      ...
      TIFFWarning(module,
                  ...
                  sp->v_sampling, sp->cinfo.d.comp_info[0].h_samp_factor,
                  sp->cinfo.d.comp_info[0].v_samp_factor);

      sp->h_sampling = (uint16)sp->cinfo.d.comp_info[0].h_samp_factor;
      sp->v_sampling = (uint16)sp->cinfo.d.comp_info[0].v_samp_factor;
    }
 ...
 */
/*ARGSUSED*/ static int JPEGDecode(TIFF *tif, tidata_t buf, tsize_t cc,
                                   tsample_t s) {
  JPEGState *sp = JState(tif);
  tsize_t nrows;
 ...
 */
/*ARGSUSED*/ static int JPEGDecodeRaw(TIFF *tif, tidata_t buf, tsize_t cc,
                                      tsample_t s) {
  JPEGState *sp = JState(tif);
  tsize_t nrows;
 ...
 */
static int JPEGPreEncode(TIFF *tif, tsample_t s) {
  JPEGState *sp = JState(tif);
  TIFFDirectory *td = &tif->tif_dir;
...
static int JPEGPostEncode(TIFF *tif) {
  ...
  if (sp->scancount > 0) {
    ...
    for (ci = 0, compptr = sp->cinfo.c.comp_info;
         ci < sp->cinfo.c.num_components; ci++, compptr++) {
      int vsamp = compptr->v_samp_factor;
      tsize_t row_width = compptr->width_in_blocks * DCTSIZE * sizeof(JSAMPLE);
...
static int JPEGVSetField(TIFF *tif, ttag_t tag, va_list ap) {
  ...
  case TIFFTAG_JPEGTABLES:
    ...
    _TIFFsetByteArray(&sp->jpegtables, va_arg(ap, void *), (long)v32);
...
static void JPEGFixupTestSubsampling(TIFF *tif) {
   ...
   */
  if (!sp->cinfo.comm.is_decompressor || sp->ycbcrsampling_fetched ||
  ...
  TIFFSetField(tif, TIFFTAG_YCBCRSUBSAMPLING, (uint16)sp->h_sampling,
               (uint16)sp->v_sampling);
...
static uint32 JPEGDefaultStripSize(TIFF *tif, uint32 s) {
  JPEGState *sp = JState(tif);
  TIFFDirectory *td = &tif->tif_dir;
