/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_getimage.c,v 1.36 2003/12/21
 * 22:13:14 dron Exp $ */
...
int TIFFRGBAImageOK(TIFF *tif, char emsg[1024]) {
  ...
  case PHOTOMETRIC_PALETTE:
    if (td->td_planarconfig == PLANARCONFIG_CONTIG &&
      ...
      sprintf(emsg,
              ...
              photoTag, photometric, "Samples/pixel", td->td_samplesperpixel,
              td->td_bitspersample);
      return (0);
    }
    /*
  ...
  case PHOTOMETRIC_LOGL:
    if (td->td_compression != COMPRESSION_SGILOG) {
      sprintf(emsg, "Sorry, LogL data must have %s=%d", "Compression",
              COMPRESSION_SGILOG);
      return (0);
    }
    break;
  case PHOTOMETRIC_LOGLUV:
    if (td->td_compression != COMPRESSION_SGILOG &&
      ...
      sprintf(emsg, "Sorry, LogLuv data must have %s=%d or %d", "Compression",
...
static int isCCITTCompression(TIFF *tif) {
  ...
  return (
      compress == COMPRESSION_CCITTFAX3 || compress == COMPRESSION_CCITTFAX4 ||
      compress == COMPRESSION_CCITTRLE || compress == COMPRESSION_CCITTRLEW);
}
  ...
  case PHOTOMETRIC_MINISBLACK:
    if (planarconfig == PLANARCONFIG_CONTIG && img->samplesperpixel != 1 &&
      ...
      sprintf(emsg,
              "Sorry, can not handle contiguous data with %s=%d, "
              "and %s=%d and Bits/Sample=%d",
              photoTag, img->photometric, "Samples/pixel", img->samplesperpixel,
              img->bitspersample);
      return (0);
    }
    break;
 ...
 */
int TIFFReadRGBAImageOriented(TIFF *tif, uint32 rwidth, uint32 rheight,
                              uint32 *raster, int orientation, int stop) {
  char emsg[1024];
  TIFFRGBAImage img;
  int ok;
 ...
 */
int TIFFReadRGBAImage(TIFF *tif, uint32 rwidth, uint32 rheight, uint32 *raster,
                      int stop) {
 ...
 *	SamplesPerPixel == 1
 */
static int gtTileContig(TIFFRGBAImage *img, uint32 *raster, uint32 w,
                        uint32 h) {
  TIFF *tif = img->tif;
  ...
  for (row = 0; row < h; row += nrow) {
    ...
    for (col = 0; col < w; col += tw) {
      if (TIFFReadTile(tif, buf, col + img->col_offset, row + img->row_offset,
                       0, 0) < 0 &&
          img->stoponerr) {
        ret = 0;
        break;
      ...
      if (col + tw > w) {
        ...
        uint32 npix = w - col;
        fromskew = tw - npix;
        (*put)(img, raster + y * w + col, col, y, npix, nrow, fromskew,
               toskew + fromskew, buf + pos);
      } else {
        (*put)(img, raster + y * w + col, col, y, tw, nrow, 0, toskew,
               buf + pos);
      }
    }
 ...
 */
static int gtTileSeparate(TIFFRGBAImage *img, uint32 *raster, uint32 w,
                          uint32 h) {
  TIFF *tif = img->tif;
  ...
  for (row = 0; row < h; row += nrow) {
    ...
    nrow = (row + rowstoread > h ? h - row : rowstoread);
    for (col = 0; col < w; col += tw) {
      if (TIFFReadTile(tif, r, col + img->col_offset, row + img->row_offset, 0,
                       0) < 0 &&
          img->stoponerr) {
        ret = 0;
        break;
      }
      if (TIFFReadTile(tif, g, col + img->col_offset, row + img->row_offset, 0,
                       1) < 0 &&
          img->stoponerr) {
        ret = 0;
        break;
      }
      if (TIFFReadTile(tif, b, col + img->col_offset, row + img->row_offset, 0,
                       2) < 0 &&
          img->stoponerr) {
        ret = 0;
        break;
      ...
      if (alpha &&
          TIFFReadTile(tif, a, col + img->col_offset, row + img->row_offset, 0,
                       3) < 0 &&
          img->stoponerr) {
        ret = 0;
        break;
      ...
      if (col + tw > w) {
        ...
        fromskew = tw - npix;
        (*put)(img, raster + y * w + col, col, y, npix, nrow, fromskew,
               toskew + fromskew, r + pos, g + pos, b + pos, a + pos);
      } else {
        (*put)(img, raster + y * w + col, col, y, tw, nrow, 0, toskew, r + pos,
               g + pos, b + pos, a + pos);
      }
    }
 ...
 *	SamplesPerPixel == 1
 */
static int gtStripContig(TIFFRGBAImage *img, uint32 *raster, uint32 w,
                         uint32 h) {
  TIFF *tif = img->tif;
  tileContigRoutine put = img->put.contig;
  ...
  for (row = 0; row < h; row += nrow) {
    ...
    if (TIFFReadEncodedStrip(
            tif, TIFFComputeStrip(tif, row + img->row_offset, 0), buf,
 ...
 * We assume that all such images are RGB.
 */
static int gtStripSeparate(TIFFRGBAImage *img, uint32 *raster, uint32 w,
                           uint32 h) {
  TIFF *tif = img->tif;
  tileSeparateRoutine put = img->put.separate;
  u_char *buf;
  ...
  for (row = 0; row < h; row += nrow) {
    ...
    if (TIFFReadEncodedStrip(tif, TIFFComputeStrip(tif, offset_row, 0), r,
    ...
    if (TIFFReadEncodedStrip(tif, TIFFComputeStrip(tif, offset_row, 1), g,
                             ((row + img->row_offset) % rowsperstrip + nrow) *
                                 scanline) < 0 &&
        img->stoponerr) {
      ret = 0;
    ...
    if (TIFFReadEncodedStrip(tif, TIFFComputeStrip(tif, offset_row, 2), b,
                             ((row + img->row_offset) % rowsperstrip + nrow) *
                                 scanline) < 0 &&
        img->stoponerr) {
      ret = 0;
    ...
    if (alpha &&
        (TIFFReadEncodedStrip(tif, TIFFComputeStrip(tif, offset_row, 3), a,
                              ((row + img->row_offset) % rowsperstrip + nrow) *
                                  scanline) < 0 &&
         img->stoponerr)) {
      ret = 0;
    ...
    pos = ((row + img->row_offset) % rowsperstrip) * scanline;
    (*put)(img, raster + y * w, 0, y, w, nrow, fromskew, toskew, r + pos,
           g + pos, b + pos, a + pos);
    y += (flip & FLIP_VERTICALLY ? -(int32)nrow : (int32)nrow);
  }
...
#define PACKW4(r, g, b, a)                                                     \
  ((uint32)W2B(r) | ((uint32)W2B(g) << 8) | ((uint32)W2B(b) << 16) |           \
   ((uint32)W2B(a) << 24))

#define DECLAREContigPutFunc(name)                                             \
  static void name(TIFFRGBAImage *img, uint32 *cp, uint32 x, uint32 y,         \
                   uint32 w, uint32 h, int32 fromskew, int32 toskew,           \
                   u_char *pp)
...
DECLAREContigPutFunc(putRGBcontig8bittile) {
  ...
  while (h-- > 0) {
    UNROLL8(w, NOP, *cp++ = PACK(pp[0], pp[1], pp[2]); pp += samplesperpixel);
    cp += toskew;
    pp += fromskew;
  }
}
...
DECLAREContigPutFunc(putcontig8bitCIELab) {
  ...
  while (h-- > 0) {
    for (x = w; x-- > 0;) {
      TIFFCIELabToXYZ(img->cielab, (u_char)pp[0], (signed char)pp[1],
                      (signed char)pp[2], &X, &Y, &Z);
 ...
 */
#ifdef notdef
static void putcontig8bitYCbCrGenericTile(TIFFRGBAImage *img, uint32 *cp,
                                          uint32 x, uint32 y, uint32 w,
                                          uint32 h, int32 fromskew,
                                          int32 toskew, u_char *pp, int h_group,
                                          int v_group)
    ...
    while (h > 0) {
      for (x = w; x > 0;) {
        ...
        default:
          switch (h) {
          default:
            YCbCrtoRGB(cp1[1], pp[3]); /* FALLTHROUGH */
          case 1:
            YCbCrtoRGB(cp[1], pp[1]); /* FALLTHROUGH */
          }                           /* FALLTHROUGH */
        case 1:
          switch (h) {
          default:
            YCbCrtoRGB(cp1[0], pp[2]); /* FALLTHROUGH */
          case 1:
            YCbCrtoRGB(cp[0], pp[0]); /* FALLTHROUGH */
          }                           /* FALLTHROUGH */
        }
...
static tileContigRoutine initYCbCrConversion(TIFFRGBAImage *img) {
  ...
  if (img->ycbcr == NULL) {
    img->ycbcr = (TIFFYCbCrToRGB *)_TIFFmalloc(
        ...
        3 * 256 * sizeof(int32));
    if (img->ycbcr == NULL) {
      TIFFError(module, "No space for YCbCr->RGB conversion state");
      return (NULL);
    }
  }
...
static tileContigRoutine initCIELabConversion(TIFFRGBAImage *img) {
  ...
  if (!img->cielab) {
    img->cielab = (TIFFCIELabToRGB *)_TIFFmalloc(sizeof(TIFFCIELabToRGB));
    if (!img->cielab) {
      TIFFError(module, "No space for CIE L*a*b*->RGB conversion state.");
      return NULL;
    }
  }
  ...
  if (TIFFCIELabToRGBInit(img->cielab, &display_sRGB, refWhite) < 0) {
    TIFFError(module, "Failed to initialize CIE L*a*b*->RGB conversion state.");
...
static int makebwmap(TIFFRGBAImage *img) {
  ...
  img->BWmap = (uint32 **)_TIFFmalloc(256 * sizeof(uint32 *) +
                                      (256 * nsamples * sizeof(uint32)));
  if (img->BWmap == NULL) {
...
static int makecmap(TIFFRGBAImage *img) {
  ...
  img->PALmap = (uint32 **)_TIFFmalloc(256 * sizeof(uint32 *) +
                                       (256 * nsamples * sizeof(uint32)));
  if (img->PALmap == NULL) {
...

{
  ...
  for (i_row = 0; i_row < read_ysize; i_row++) {
    ...
    _TIFFmemset(raster + (tile_ysize - i_row - 1) * tile_xsize + read_xsize, 0,
                sizeof(uint32) * (tile_xsize - read_xsize));
  }

  for (i_row = read_ysize; i_row < tile_ysize; i_row++) {
    _TIFFmemset(raster + (tile_ysize - i_row - 1) * tile_xsize, 0,
                sizeof(uint32) * tile_xsize);
  }
