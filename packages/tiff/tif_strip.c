/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_strip.c,v 1.3 2003/11/07 11:26:38
 * dron Exp $ */
...
tstrip_t TIFFComputeStrip(TIFF *tif, uint32 row, tsample_t sample) {
  ...
  if (td->td_planarconfig == PLANARCONFIG_SEPARATE) {
    if (sample >= td->td_samplesperpixel) {
      TIFFError(tif->tif_name, "%u: Sample out of range, max %u", sample,
...
tsize_t TIFFVStripSize(TIFF *tif, uint32 nrows) {
  ...
  if (td->td_planarconfig == PLANARCONFIG_CONTIG &&
    ...
    return (
        (tsize_t)(nrows * scanline + 2 * (nrows * scanline / samplingarea)));
  } else
#endif
...
tsize_t TIFFRasterScanlineSize(TIFF *tif) {
  ...
  if (td->td_planarconfig == PLANARCONFIG_CONTIG) {
    ...
    return ((tsize_t)TIFFhowmany(scanline, 8) * td->td_samplesperpixel);
}
