/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_unix.c,v 1.4 2002/10/11 14:13:00
 * dron Exp $ */
...
static int _tiffCloseProc(thandle_t fd) { return (close((int)fd)); }
...
static int _tiffMapProc(thandle_t fd, tdata_t *pbase, toff_t *psize) {
  ...
  if (size != (toff_t)-1) {
    *pbase = (tdata_t)mmap(0, size, PROT_READ, MAP_SHARED, (int)fd, 0);
    if (*pbase != (tdata_t)-1) {
      *psize = size;
      return (1);
    }
...
TIFF *TIFFFdOpen(int fd, const char *name, const char *mode) {
  ...
  tif = TIFFClientOpen(name, mode, (thandle_t)fd, _tiffReadProc, _tiffWriteProc,
...
void *_TIFFrealloc(tdata_t p, tsize_t s) { return (realloc(p, (size_t)s)); }
...
static void unixWarningHandler(const char *module, const char *fmt,
                               va_list ap) {
  if (module != NULL)
