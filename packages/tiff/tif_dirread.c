/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_dirread.c,v 1.26 2003/12/20
 * 13:40:09 dron Exp $ */
...
int TIFFReadDirectory(TIFF *tif) {
  ...
  if (!tif->tif_dirlist) {
    TIFFError(module, "%.1000s: Failed to allocate space for IFD list",
              tif->tif_name);
    return (0);
  }
  ...
  if (!isMapped(tif)) {
    if (!SeekOK(tif, tif->tif_diroff)) {
      TIFFError(module, "%.1000s: Seek error accessing TIFF directory",
                tif->tif_name);
      return (0);
    }
    if (!ReadOK(tif, &dircount, sizeof(uint16))) {
      TIFFError(module, "%.1000s: Can not read TIFF directory count",
                tif->tif_name);
      return (0);
    }
    if (tif->tif_flags & TIFF_SWAB)
      TIFFSwabShort(&dircount);
    dir = (TIFFDirEntry *)CheckMalloc(tif, dircount * sizeof(TIFFDirEntry),
                                      "to read TIFF directory");
    if (dir == NULL)
      return (0);
    if (!ReadOK(tif, dir, dircount * sizeof(TIFFDirEntry))) {
      TIFFError(module, "%.100s: Can not read TIFF directory", tif->tif_name);
      goto bad;
    }
    /*
    ...
    if (off + dircount * sizeof(TIFFDirEntry) > tif->tif_size) {
      TIFFError(module, "%.1000s: Can not read TIFF directory", tif->tif_name);
      goto bad;
    } else
  ...
  for (dp = dir, n = dircount; n > 0; n--, dp++) {
    ...
    if (dp->tdir_tag < tif->tif_fieldinfo[fix]->field_tag) {
      if (!diroutoforderwarning) {
        TIFFWarning(module,
                    "%.1000s: invalid TIFF directory; tags are not sorted in "
                    "ascending order",
                    tif->tif_name);
  ...
  for (dp = dir, n = dircount; n > 0; n--, dp++) {
    ...
    case TIFFTAG_TRANSFERFUNCTION:
      ...
      if (dp->tdir_tag == TIFFTAG_COLORMAP || dp->tdir_count != (uint32)v) {
      ...
      if (cp != NULL) {
        if (TIFFFetchData(tif, dp, cp)) {
           ...
           */
          uint32 c = (uint32)1 << td->td_bitspersample;
          if (dp->tdir_count == c)
            v = 0;
...
#define BYTECOUNTLOOKSBAD                                                      \
  ((td->td_stripbytecount[0] == 0 && td->td_stripoffset[0] != 0) ||            \
   (td->td_compression == COMPRESSION_NONE &&                                  \
     ...
     */
    TIFFWarning(module,
                "%.1000s: Bogus \"%s\" field, ignoring and calculating from "
                "imagelength",
                tif->tif_name,
                _TIFFFieldWithTag(tif, TIFFTAG_STRIPBYTECOUNTS)->field_name);
    if (EstimateStripByteCounts(tif, dir, dircount) < 0)
...
static int EstimateStripByteCounts(TIFF *tif, TIFFDirEntry *dir,
  ...
  td->td_stripbytecount = (uint32 *)CheckMalloc(
...
static void MissingRequired(TIFF *tif, const char *tagname) {
  ...
  TIFFError(module, "%.1000s: TIFF directory is missing required \"%s\" field",
...
static int cvtRational(TIFF *tif, TIFFDirEntry *dir, uint32 num, uint32 denom,
                       float *rv) {
  if (denom == 0) {
    TIFFError(tif->tif_name, "%s: Rational with zero denominator (num = %lu)",
 ...
 */
static int TIFFFetchByteArray(TIFF *tif, TIFFDirEntry *dir, uint16 *v) {
  if (dir->tdir_count <= 4) {
    /*
...
static int TIFFFetchRationalArray(TIFF *tif, TIFFDirEntry *dir, float *v) {
  ...
  l = (uint32 *)CheckMalloc(
      tif, dir->tdir_count * TIFFDataWidth((TIFFDataType)dir->tdir_type),
...
static int TIFFFetchAnyArray(TIFF *tif, TIFFDirEntry *dir, double *v) {
  ...
  default:
    ...
    /* TIFF_ASCII */
    /* TIFF_UNDEFINED */
    TIFFError(tif->tif_name, "cannot read TIFF_ANY type %d for field \"%s\"",
              _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
    return (0);
  }
  return (1);
...
static int TIFFFetchNormalTag(TIFF *tif, TIFFDirEntry *dp) {
  ...
  if (dp->tdir_count > 1) { /* array of values */
    ...
    case TIFF_SBYTE:
      ...
      cp = CheckMalloc(tif, dp->tdir_count * sizeof(uint16), mesg);
    ...
    case TIFF_LONG:
    case TIFF_SLONG:
      cp = CheckMalloc(tif, dp->tdir_count * sizeof(uint32), mesg);
    ...
    case TIFF_FLOAT:
      ...
      break;
    case TIFF_DOUBLE:
      cp = CheckMalloc(tif, dp->tdir_count * sizeof(double), mesg);
 ...
 */
static int TIFFFetchPerSampleShorts(TIFF *tif, TIFFDirEntry *dir, int *pl) {
...
static int TIFFFetchStripThing(TIFF *tif, TIFFDirEntry *dir, long nstrips,
   ...
   */
  if (*lpp == NULL &&
      (*lpp = (uint32 *)CheckMalloc(tif, nstrips * sizeof(uint32),
  ...
  if (dir->tdir_type == (int)TIFF_SHORT) {
     ...
     */
    uint16 *dp = (uint16 *)CheckMalloc(tif, dir->tdir_count * sizeof(uint16),
                                       "to fetch strip tag");
    if (dp == NULL)
      return (0);
    ...
    uint32 *dp = (uint32 *)CheckMalloc(tif, dir->tdir_count * sizeof(uint32),
                                       "to fetch strip tag");
    if (dp == NULL)
      return (0);
...
static int TIFFFetchRefBlackWhite(TIFF *tif, TIFFDirEntry *dir) {
  ...
  if ((ok = (cp && TIFFFetchLongArray(tif, dir, (uint32 *)cp))) != 0) {
    float *fp =
        (float *)CheckMalloc(tif, dir->tdir_count * sizeof(float), mesg);
    if ((ok = (fp != NULL)) != 0) {
      uint32 i;
      for (i = 0; i < dir->tdir_count; i++)
