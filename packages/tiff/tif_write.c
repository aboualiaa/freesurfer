/* $Header: /cvsroot/osrs/libtiff/libtiff/tif_write.c,v 1.9 2003/08/05 02:45:58
 * warmerda Exp $ */
...
int TIFFWriteScanline(TIFF *tif, tdata_t buf, uint32 row, tsample_t sample) {
  ...
  if (td->td_planarconfig == PLANARCONFIG_SEPARATE) {
    if (sample >= td->td_samplesperpixel) {
      TIFFError(tif->tif_name, "%d: Sample out of range, max %d", sample,
  ...
  status =
      (*tif->tif_encoderow)(tif, (tidata_t)buf, tif->tif_scanlinesize, sample);
 ...
 */
tsize_t TIFFWriteEncodedStrip(TIFF *tif, tstrip_t strip, tdata_t data,
                              tsize_t cc) {
 ...
 */
tsize_t TIFFWriteTile(TIFF *tif, tdata_t buf, uint32 x, uint32 y, uint32 z,
                      tsample_t s) {
 ...
 */
tsize_t TIFFWriteEncodedTile(TIFF *tif, ttile_t tile, tdata_t data,
                             tsize_t cc) {
  ...
  if (tile >= td->td_nstrips) {
    TIFFError(module, "%s: Tile %lu out of range, max %lu", tif->tif_name,
...
tsize_t TIFFWriteRawTile(TIFF *tif, ttile_t tile, tdata_t data, tsize_t cc) {
  ...
  if (tile >= tif->tif_dir.td_nstrips) {
    TIFFError(module, "%s: Tile %lu out of range, max %lu", tif->tif_name,
              (u_long)tile, (u_long)tif->tif_dir.td_nstrips);
    return ((tsize_t)-1);
  }
  return (TIFFAppendToStrip(tif, tile, (tidata_t)data, cc) ? cc : (tsize_t)-1);
}
...
int TIFFSetupStrips(TIFF *tif) {
  ...
  td->td_stripbytecount =
      (uint32 *)_TIFFmalloc(td->td_nstrips * sizeof(uint32));
...
int TIFFWriteCheck(TIFF *tif, int tiles, const char *module) {
  ...
  if (tif->tif_dir.td_stripoffset == NULL && !TIFFSetupStrips(tif)) {
    ...
    TIFFError(module, "%s: No space for %s arrays", tif->tif_name,
...
int TIFFWriteBufferSetup(TIFF *tif, tdata_t bp, tsize_t size) {
  ...
  if (size == (tsize_t)-1) {
    size = (isTiled(tif) ? tif->tif_tilesize : tif->tif_scanlinesize);
    /*
 ...
 */
static int TIFFGrowStrips(TIFF *tif, int delta, const char *module) {
  TIFFDirectory *td = &tif->tif_dir;

  assert(td->td_planarconfig == PLANARCONFIG_CONTIG);
  td->td_stripoffset = (uint32 *)_TIFFrealloc(
      td->td_stripoffset, (td->td_nstrips + delta) * sizeof(uint32));
  td->td_stripbytecount = (uint32 *)_TIFFrealloc(
      td->td_stripbytecount, (td->td_nstrips + delta) * sizeof(uint32));
  if (td->td_stripoffset == NULL || td->td_stripbytecount == NULL) {
    td->td_nstrips = 0;
  ...
  _TIFFmemset(td->td_stripbytecount + td->td_nstrips, 0,
              delta * sizeof(uint32));
  td->td_nstrips += delta;
  return (1);
}
 ...
 */
static int TIFFAppendToStrip(TIFF *tif, tstrip_t strip, tidata_t data,
                             tsize_t cc) {
  TIFFDirectory *td = &tif->tif_dir;
  ...
  if (td->td_stripoffset[strip] == 0 || tif->tif_curoff == 0) {
    ...
    if (td->td_stripoffset[strip] != 0) {
      if (!SeekOK(tif, td->td_stripoffset[strip])) {
        TIFFError(module, "%s: Seek error at scanline %lu", tif->tif_name,
                  (u_long)tif->tif_row);
        return (0);
      }
    } else
      td->td_stripoffset[strip] = TIFFSeekFile(tif, (toff_t)0, SEEK_END);
    tif->tif_curoff = td->td_stripoffset[strip];
  }
  if (!WriteOK(tif, data, cc)) {
    TIFFError(module, "%s: Write error at scanline %lu", tif->tif_name,
              (u_long)tif->tif_row);
    return (0);
  }
  tif->tif_curoff += cc;
 ...
 */
void TIFFSetWriteOffset(TIFF *tif, toff_t off) { tif->tif_curoff = off; }
