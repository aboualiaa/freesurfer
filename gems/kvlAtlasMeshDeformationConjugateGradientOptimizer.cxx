...
//
//
double AtlasMeshDeformationConjugateGradientOptimizer ::
  ...
  if (!startingOrRestarting) {
    ...
    searchDirection = this->LinearlyCombineDeformations(
        m_Gradient, -1.0, m_OldSearchDirection, beta);

    if (this->ComputeInnerProduct(searchDirection, m_Gradient) >
        0) // ( p' * gradient > 0 )
    {
      // Senseless direction proposed - restart anyway
      if (m_Verbose) {
        std::cout << "Proposed direction is not a descent direction; restarting"
                  << std::endl;
      }
      startingOrRestarting = true;
    }
