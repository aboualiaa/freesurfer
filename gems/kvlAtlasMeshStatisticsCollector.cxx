...
//
//
AtlasMeshStatisticsCollector ::AtlasMeshStatisticsCollector() {
...
void AtlasMeshStatisticsCollector ::Rasterize(const AtlasMesh *mesh) {
  ...
  for (std::vector<StatisticsContainerType::Pointer>::const_iterator it =
           m_ThreadSpecificLabelStatistics.begin();
...
//
//
bool AtlasMeshStatisticsCollector ::RasterizeTetrahedron(
  ...
  this->GetContributionOfTetrahedron(
      ...
      statisticsInVertex1, statisticsInVertex2, statisticsInVertex3);

  // Add contribution of this tetrahedron to thread's results
  m_ThreadSpecificMinLogLikelihoods[threadNumber] += minLogLikelihood;

  m_ThreadSpecificLabelStatistics[threadNumber]->ElementAt(id0) +=
      statisticsInVertex0;
  m_ThreadSpecificLabelStatistics[threadNumber]->ElementAt(id1) +=
      statisticsInVertex1;
  m_ThreadSpecificLabelStatistics[threadNumber]->ElementAt(id2) +=
      statisticsInVertex2;
  m_ThreadSpecificLabelStatistics[threadNumber]->ElementAt(id3) +=
      statisticsInVertex3;
