...
//
//
void AtlasMeshPositionCostAndGradientCalculator ::Rasterize(
  ...
  clock.Reset();
  clock.Start();
  m_ThreadSpecificDataTermRasterizationTimers =
      std::vector<itk::TimeProbe>(this->GetNumberOfThreads());
  m_ThreadSpecificPriorTermRasterizationTimers =
      std::vector<itk::TimeProbe>(this->GetNumberOfThreads());
  m_ThreadSpecificOtherRasterizationTimers =
      std::vector<itk::TimeProbe>(this->GetNumberOfThreads());
#endif
  ...
  for (int threadNumber = 0; threadNumber < this->GetNumberOfThreads();
    ...
    dataTermRasterizationTime +=
        m_ThreadSpecificDataTermRasterizationTimers[threadNumber].GetTotal();
    priorTermRasterizationTime +=
        m_ThreadSpecificPriorTermRasterizationTimers[threadNumber].GetTotal();
    otherRasterizationTime +=
        m_ThreadSpecificOtherRasterizationTimers[threadNumber].GetTotal();
  }
  std::cout << "     dataTermRasterizationTime: " << dataTermRasterizationTime
            << std::endl;
  std::cout << "     priorTermRasterizationTime: " << priorTermRasterizationTime
            << std::endl;
  std::cout << "     otherRasterizationTime: " << otherRasterizationTime
            << std::endl;
...
#if KVL_ENABLE_TIME_PROBE
  clock.Stop();
  std::cout << "Time taken by result collection: " << clock.GetMean()
            << std::endl;
  clock.Reset();
  clock.Start();
#endif
...
#if KVL_ENABLE_TIME_PROBE
  clock.Stop();
  std::cout << "Time taken by boundary condition imposition: "
...
//
//
void AtlasMeshPositionCostAndGradientCalculator ::SetMeshToImageTransform(
    const TransformType *meshToImageTransform) {
  ...
  m_SlidingBoundaryCorrectionMatrices[1] =
      (C1 * vnl_inverse(C1.transpose() * C1) * C1.transpose());
  ...
  m_SlidingBoundaryCorrectionMatrices[2] =
      (C2 * vnl_inverse(C2.transpose() * C2) * C2.transpose());
  ...
  m_SlidingBoundaryCorrectionMatrices[3] =
      (C3 * vnl_inverse(C3.transpose() * C3) * C3.transpose());
  ...
  m_SlidingBoundaryCorrectionMatrices[4] =
      (C4 * vnl_inverse(C4.transpose() * C4) * C4.transpose());
  ...
  m_SlidingBoundaryCorrectionMatrices[5] =
      (C5 * vnl_inverse(C5.transpose() * C5) * C5.transpose());
  ...
  m_SlidingBoundaryCorrectionMatrices[6] =
      (C6 * vnl_inverse(C6.transpose() * C6) * C6.transpose());
...
//
//
bool AtlasMeshPositionCostAndGradientCalculator ::RasterizeTetrahedron(
  ...
  AtlasMesh::CellType::PointIdIterator pit =
      mesh->GetCells()->ElementAt(tetrahedronId)->PointIdsBegin();
  const AtlasMesh::PointIdentifier id0 = *pit;
  ++pit;
  const AtlasMesh::PointIdentifier id1 = *pit;
  ++pit;
  ...
  if (!m_IgnoreDeformationPrior) {
    ...
    if (!this->AddPriorContributionOfTetrahedron(
            p0, p1, p2, p3, info, priorPlusDataCost, gradientInVertex0,
            gradientInVertex1, gradientInVertex2, gradientInVertex3)) {
      m_Abort = true;
#if KVL_ENABLE_TIME_PROBE
  ...
  if (!m_OnlyDeformationPrior) {
    ...
    this->AddDataContributionOfTetrahedron(
        p0, p1, p2, p3, alphasInVertex0, alphasInVertex1, alphasInVertex2,
        alphasInVertex3, priorPlusDataCost, gradientInVertex0,
        gradientInVertex1, gradientInVertex2, gradientInVertex3);
...
//
//
bool AtlasMeshPositionCostAndGradientCalculator ::
  ...
  const double sumOfSquaresOfReciprocalsOfSingularValuesOfJ =
      traceOfKTransposeTimesK / (detJ * detJ);

  const double priorCost = referenceVolumeTimesK * (1 + detJ) *
                           (sumOfSquaresOfSingularValuesOfJ +
                            sumOfSquaresOfReciprocalsOfSingularValuesOfJ - 6);
  ...
  const double tmp1 = referenceVolumeTimesK *
                      (sumOfSquaresOfSingularValuesOfJ +
                       sumOfSquaresOfReciprocalsOfSingularValuesOfJ - 6);
  const double tmp2 = 2 * referenceVolumeTimesK * (1 + detJ);
  const double tmp3 = pow(detJ, 3);

  const double dcostdm11 =
      tmp1 * ddetJdm11 +
      tmp2 * (m11 + ((k22 * m33 - k23 * m23 - k32 * m32 + k33 * m22) * detJ -
                     traceOfKTransposeTimesK * ddetJdm11) /
                        tmp3);
  const double dcostdm21 =
      tmp1 * ddetJdm21 +
      tmp2 * (m21 + ((-k21 * m33 + k23 * m13 + k31 * m32 - k33 * m12) * detJ -
                     traceOfKTransposeTimesK * ddetJdm21) /
                        tmp3);
  const double dcostdm31 =
      tmp1 * ddetJdm31 +
      tmp2 * (m31 + ((k21 * m23 - k22 * m13 - k31 * m22 + k32 * m12) * detJ -
                     traceOfKTransposeTimesK * ddetJdm31) /
                        tmp3);

  const double dcostdm12 =
      tmp1 * ddetJdm12 +
      tmp2 * (m12 + ((-k12 * m33 + k13 * m23 + k32 * m31 - k33 * m21) * detJ -
                     traceOfKTransposeTimesK * ddetJdm12) /
                        tmp3);
  const double dcostdm22 =
      tmp1 * ddetJdm22 +
      tmp2 * (m22 + ((k11 * m33 - k13 * m13 - k31 * m31 + k33 * m11) * detJ -
                     traceOfKTransposeTimesK * ddetJdm22) /
                        tmp3);
  const double dcostdm32 =
      tmp1 * ddetJdm32 +
      tmp2 * (m32 + ((-k11 * m23 + k12 * m13 + k31 * m21 - k32 * m11) * detJ -
                     traceOfKTransposeTimesK * ddetJdm32) /
                        tmp3);

  const double dcostdm13 =
      tmp1 * ddetJdm13 +
      tmp2 * (m13 + ((k12 * m32 - k13 * m22 - k22 * m31 + k23 * m21) * detJ -
                     traceOfKTransposeTimesK * ddetJdm13) /
                        tmp3);
  const double dcostdm23 =
      tmp1 * ddetJdm23 +
      tmp2 * (m23 + ((-k11 * m32 + k13 * m12 + k21 * m31 - k23 * m11) * detJ -
                     traceOfKTransposeTimesK * ddetJdm23) /
                        tmp3);
  const double dcostdm33 =
      tmp1 * ddetJdm33 +
      tmp2 * (m33 + ((k11 * m22 - k12 * m12 - k21 * m21 + k22 * m11) * detJ -
                     traceOfKTransposeTimesK * ddetJdm33) /
                        tmp3);
...
//
//
void AtlasMeshPositionCostAndGradientCalculator ::ImposeBoundaryCondition(
    const AtlasMesh *mesh) {
...
void AtlasMeshPositionCostAndGradientCalculator ::
  ...
  if (m_AffineProjectionMatrix.empty()) {
    ...
    for (int columnNumber = 1; columnNumber < 4; columnNumber++) {
      vnl_vector<double> column =
          m_AffineProjectionMatrix.get_column(columnNumber);
      for (int previousColumnNumber = 0; previousColumnNumber < columnNumber;
           previousColumnNumber++) {
        column -= inner_product(column, m_AffineProjectionMatrix.get_column(
                                            previousColumnNumber)) *
...
void AtlasMeshPositionCostAndGradientCalculator ::
  ...
  AtlasPositionGradientContainerType::Iterator gradientIt =
      m_PositionGradient->Begin();
  for (int rowNumber = 0; rowNumber < numberOfRows;
       ++rowNumber, ++pointIt, ++gradientIt) {
    //
    X(rowNumber, 0) = 1.0;
