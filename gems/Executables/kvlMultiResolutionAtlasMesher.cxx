...
//
//
MultiResolutionAtlasMesher ::MultiResolutionAtlasMesher() {
...
void MultiResolutionAtlasMesher ::SetUp(
    ...
    for (int y = 0; y < m_InitialSize[1] - 1; y++) {
      for (int z = 0; z < m_InitialSize[2] - 1; z++) {
        ...
        double x1 = static_cast<double>(x) *
                    static_cast<double>(m_DomainSize[0] - 1) /
                    static_cast<double>(m_InitialSize[0] - 1);
        double y1 = static_cast<double>(y) *
                    static_cast<double>(m_DomainSize[1] - 1) /
                    static_cast<double>(m_InitialSize[1] - 1);
        double z1 = static_cast<double>(z) *
                    static_cast<double>(m_DomainSize[2] - 1) /
                    static_cast<double>(m_InitialSize[2] - 1);

        double x2 = static_cast<double>(x + 1) *
                    static_cast<double>(m_DomainSize[0] - 1) /
                    static_cast<double>(m_InitialSize[0] - 1);
        double y2 = static_cast<double>(y + 1) *
                    static_cast<double>(m_DomainSize[1] - 1) /
                    static_cast<double>(m_InitialSize[1] - 1);
        double z2 = static_cast<double>(z + 1) *
                    static_cast<double>(m_DomainSize[2] - 1) /
                    static_cast<double>(m_InitialSize[2] - 1);
        ...
        this->AddHexahedron(meshSource, m_Hexahedra, p0, p1, p2, p3, p4, p5, p6,
                            p7);
      }
    }
  }
  AtlasMesh::PointsContainer::Pointer referencePosition =
      meshSource->GetOutput()->GetPoints();
  ...
  for (int i = 0; i < m_NumberOfMeshes; i++) {
    AtlasMesh::PointsContainer::Pointer target =
        AtlasMesh::PointsContainer::New();

    for (AtlasMesh::PointsContainer::ConstIterator sourceIt =
             referencePosition->Begin();
...
//
//
void MultiResolutionAtlasMesher ::Go() {

  //
  const int numberOfUpsamplingSteps = m_InitialStiffnesses.size() - 1;
  for (unsigned int upsamplingStepNumber = 0;
       upsamplingStepNumber <= numberOfUpsamplingSteps;
       upsamplingStepNumber++) {
    std::cout << "running for upsamplingStepNumber: " << upsamplingStepNumber
              << std::endl;
...
//
//
AtlasMesh::CellsContainer::Pointer MultiResolutionAtlasMesher ::GetCells(
    const AtlasMesh::PointsContainer *position) const {
  ...
  for (int i = 0; i < tetgenOutput.numberoftetrahedra; i++) {
    ...
    meshSource->AddTetrahedron(point0Id, point1Id, point2Id, point3Id);

    {
      ...
      if (volume <= 0) {
        ...
        std::cout << "****************************************" << std::endl;
        std::cout
            << "Ouch: TetGen has generated a tetrahedron with negative volume! "
            << std::endl;
        std::cout << "      tetrahedronId: " << i << std::endl;
        std::cout << "                 p0: " << position->ElementAt(point0Id)
                  << std::endl;
        std::cout << "                 p1: " << position->ElementAt(point1Id)
                  << std::endl;
        std::cout << "                 p2: " << position->ElementAt(point2Id)
                  << std::endl;
        std::cout << "                 p3: " << position->ElementAt(point3Id)
                  << std::endl;
        std::cout << "****************************************" << std::endl;
        std::cout << "****************************************" << std::endl;
...
AtlasMeshCollection::Pointer MultiResolutionAtlasMesher ::GetMeshCollection(
  ...
  for (int cellGeneratingMeshNumber = m_NumberOfMeshes;
    ...
    if (cellGeneratingMeshNumber < static_cast<int>(m_NumberOfMeshes)) {
      ...
      std::cout << "Trying to generate cells from mesh number "
                << cellGeneratingMeshNumber << std::endl;
    } else {
      cellGeneratingPosition = referencePosition;
      std::cout << "Trying to generate cells from referencePosition"
                << std::endl;
    }
    ...
    for (int meshNumber = m_NumberOfMeshes; meshNumber >= 0; meshNumber--) {
      ...
      for (AtlasMesh::CellsContainer::ConstIterator cellIt = cells->Begin();
        ...
        if (volume <= 0) {
          std::cout << "****************************************" << std::endl;
          std::cout << "****************************************" << std::endl;
          std::cout << "Ouch: Upsampling has generated a tetrahedron with "
                       "negative volume in one of the meshes! "
                    << std::endl;
          if (meshNumber < static_cast<int>(m_NumberOfMeshes)) {
          ...
          std::cout << "                 p0: "
                    << thisPosition->ElementAt(point0Id) << std::endl;
          std::cout << "                 p1: "
                    << thisPosition->ElementAt(point1Id) << std::endl;
          std::cout << "                 p2: "
                    << thisPosition->ElementAt(point2Id) << std::endl;
          std::cout << "                 p3: "
                    << thisPosition->ElementAt(point3Id) << std::endl;
  ...
  //
  if (!cells) {
    std::cout << "Couldn't figure out how to make cells such that all meshes "
                 "are valid -- giving up"
              << std::endl;
  ...
  AtlasMesh::PointDataContainer::Pointer pointData =
      AtlasMesh::PointDataContainer::New();
  for (kvl::AtlasMesh::PointsContainer::ConstIterator pointIt =
           referencePosition->Begin();
    ...
    if ((fabs(pointIt.Value()[0]) < 1e-3) ||
        (fabs(pointIt.Value()[0] - (m_DomainSize[0] - 1)) < 1e-3)) {
    ...
    if ((fabs(pointIt.Value()[1]) < 1e-3) ||
        (fabs(pointIt.Value()[1] - (m_DomainSize[1] - 1)) < 1e-3)) {
    ...
    if ((fabs(pointIt.Value()[2]) < 1e-3) ||
        (fabs(pointIt.Value()[2] - (m_DomainSize[2] - 1)) < 1e-3)) {
...
//
//
void MultiResolutionAtlasMesher ::Upsample() {
  ...
  MeshSourceType::Pointer upsampledReferencePositionSource =
      MeshSourceType::New();
    ...
    if (true) // Switch off for non-sparse upsampling
    {
      ...
      for (unsigned int classNumber = 0; classNumber < m_NumberOfClasses;
        ...
        if (m_Current->GetPointParameters()
                ->ElementAt(p0Id)
                .m_Alphas[classNumber] > maximumAlpha) {
          maximumAlpha = m_Current->GetPointParameters()
                             ->ElementAt(p0Id)
                             .m_Alphas[classNumber];
      ...
      if ((m_Current->GetPointParameters()
               ->ElementAt(p0Id)
               .m_Alphas[maximumAlphaLabelNumber] >= threshold) &&
          (m_Current->GetPointParameters()
               ->ElementAt(p1Id)
               .m_Alphas[maximumAlphaLabelNumber] >= threshold) &&
          (m_Current->GetPointParameters()
               ->ElementAt(p2Id)
    ...
    if (subdivideHexahedron) {
      ...
      for (int meshNumber = m_NumberOfMeshes; meshNumber >= 0; meshNumber--) {
        ...
        if (meshNumber == static_cast<int>(m_NumberOfMeshes)) {
          ...
          this->GetUpsampledHexahedronPoints<int>(
              p0, p1, p2, p3, p4, p5, p6, p7, p01, p02, p13, p23, p15, p37, p26,
              p04, p57, p67, p46, p45, p0123, p1357, p2367, p0246, p0145, p4567,
              pMiddle);
        } else {
          this->GetUpsampledHexahedronPoints<double>(
              p0, p1, p2, p3, p4, p5, p6, p7, p01, p02, p13, p23, p15, p37, p26,
              p04, p57, p67, p46, p45, p0123, p1357, p2367, p0246, p0145, p4567,
              pMiddle);
        }
        ...
        if (meshNumber == static_cast<int>(m_NumberOfMeshes)) {
          ...
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p0, p01,
              p02, p0123, p04, p0145, p0246, pMiddle, p0UpsampledId,
              p01UpsampledId, p02UpsampledId, p0123UpsampledId, p04UpsampledId,
              p0145UpsampledId, p0246UpsampledId, pMiddleUpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p01, p1,
              p0123, p13, p0145, p15, pMiddle, p1357, p01UpsampledId,
              p1UpsampledId, p0123UpsampledId, p13UpsampledId, p0145UpsampledId,
              p15UpsampledId, pMiddleUpsampledId, p1357UpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p02, p0123,
              p2, p23, p0246, pMiddle, p26, p2367, p02UpsampledId,
              p0123UpsampledId, p2UpsampledId, p23UpsampledId, p0246UpsampledId,
              pMiddleUpsampledId, p26UpsampledId, p2367UpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p0123, p13,
              p23, p3, pMiddle, p1357, p2367, p37, p0123UpsampledId,
              p13UpsampledId, p23UpsampledId, p3UpsampledId, pMiddleUpsampledId,
              p1357UpsampledId, p2367UpsampledId, p37UpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p04, p0145,
              p0246, pMiddle, p4, p45, p46, p4567, p04UpsampledId,
              p0145UpsampledId, p0246UpsampledId, pMiddleUpsampledId,
              p4UpsampledId, p45UpsampledId, p46UpsampledId, p4567UpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p0145, p15,
              pMiddle, p1357, p45, p5, p4567, p57, p0145UpsampledId,
              p15UpsampledId, pMiddleUpsampledId, p1357UpsampledId,
              p45UpsampledId, p5UpsampledId, p4567UpsampledId, p57UpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, p0246,
              pMiddle, p26, p2367, p46, p4567, p6, p67, p0246UpsampledId,
              pMiddleUpsampledId, p26UpsampledId, p2367UpsampledId,
              p46UpsampledId, p4567UpsampledId, p6UpsampledId, p67UpsampledId);
          this->AddHexahedron(
              upsampledReferencePositionSource, upsampledHexahedra, pMiddle,
              p1357, p2367, p37, p4567, p57, p67, p7, pMiddleUpsampledId,
              p1357UpsampledId, p2367UpsampledId, p37UpsampledId,
          ...
          upsampledPositions[meshNumber]->InsertElement(p0123UpsampledId,
                                                        p0123);
          upsampledPositions[meshNumber]->InsertElement(p1357UpsampledId,
                                                        p1357);
          upsampledPositions[meshNumber]->InsertElement(p2367UpsampledId,
                                                        p2367);
          upsampledPositions[meshNumber]->InsertElement(p0246UpsampledId,
                                                        p0246);
          upsampledPositions[meshNumber]->InsertElement(p0145UpsampledId,
                                                        p0145);
          upsampledPositions[meshNumber]->InsertElement(p4567UpsampledId,
                                                        p4567);

          upsampledPositions[meshNumber]->InsertElement(pMiddleUpsampledId,
                                                        pMiddle);
        }
      ...
      for (int meshNumber = m_NumberOfMeshes; meshNumber >= 0; meshNumber--) {
        ...
        if (meshNumber == static_cast<int>(m_NumberOfMeshes)) {
          ...
          this->AddHexahedron(upsampledReferencePositionSource,
                              upsampledHexahedra, p0, p1, p2, p3, p4, p5, p6,
                              p7, p0UpsampledId, p1UpsampledId, p2UpsampledId,
                              p3UpsampledId, p4UpsampledId, p5UpsampledId,
                              p6UpsampledId, p7UpsampledId);
        } else {
          upsampledPositions[meshNumber]->InsertElement(p0UpsampledId, p0);
          upsampledPositions[meshNumber]->InsertElement(p1UpsampledId, p1);
          upsampledPositions[meshNumber]->InsertElement(p2UpsampledId, p2);
  ...
  for (AtlasMesh::PointsContainer::Iterator it =
           upsampledReferencePosition->Begin();
...
//
//
void MultiResolutionAtlasMesher ::AddHexahedron(
    ...
    AtlasMesh::PointIdentifier &p0Id, AtlasMesh::PointIdentifier &p1Id,
    AtlasMesh::PointIdentifier &p2Id, AtlasMesh::PointIdentifier &p3Id,
...
void MultiResolutionAtlasMesher ::GetUpsampledHexahedronPoints(
    const AtlasMesh::PointType &p0, const AtlasMesh::PointType &p1,
    const AtlasMesh::PointType &p2, const AtlasMesh::PointType &p3,
    const AtlasMesh::PointType &p4, const AtlasMesh::PointType &p5,
    const AtlasMesh::PointType &p6, const AtlasMesh::PointType &p7,
    AtlasMesh::PointType &p01, AtlasMesh::PointType &p02,
    AtlasMesh::PointType &p13, AtlasMesh::PointType &p23,
    AtlasMesh::PointType &p15, AtlasMesh::PointType &p37,
    AtlasMesh::PointType &p26, AtlasMesh::PointType &p04,
    AtlasMesh::PointType &p57, AtlasMesh::PointType &p67,
    AtlasMesh::PointType &p46, AtlasMesh::PointType &p45,
    AtlasMesh::PointType &p0123, AtlasMesh::PointType &p1357,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    p0123[i] =
        (internalP0[i] + internalP1[i] + internalP2[i] + internalP3[i]) / 4;
    p1357[i] =
        (internalP1[i] + internalP3[i] + internalP5[i] + internalP7[i]) / 4;
    p2367[i] =
        (internalP2[i] + internalP3[i] + internalP6[i] + internalP7[i]) / 4;
    p0246[i] =
        (internalP0[i] + internalP2[i] + internalP4[i] + internalP6[i]) / 4;
    p0145[i] =
        (internalP0[i] + internalP1[i] + internalP4[i] + internalP5[i]) / 4;
    p4567[i] =
        (internalP4[i] + internalP5[i] + internalP6[i] + internalP7[i]) / 4;

    // Point in the middle of the cube
    pMiddle[i] =
        (internalP0[i] + internalP1[i] + internalP2[i] + internalP3[i] +
         internalP4[i] + internalP5[i] + internalP6[i] + internalP7[i]) /
        8;
  }
