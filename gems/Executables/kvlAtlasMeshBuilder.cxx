...
//
//
void AtlasMeshBuilder ::PrintSelf(std::ostream &os, itk::Indent indent) const {}
...
//
//
void AtlasMeshBuilder ::Build(AtlasMeshCollection *explicitStartCollection,
                              double edgeCollapseEncouragementFactor) {
  ...
  if (explicitStartCollection == 0) {
    m_Mesher->Go();
    m_Current =
        const_cast<AtlasMeshCollection *>(m_Mesher->GetCurrentMeshCollection());
  } else {
    if (fabs(explicitStartCollection->GetK() - m_InitialStiffnesses.back()) <
        1e-2) {
      m_Current = explicitStartCollection;
    } else {
      //
      std::cout << "Got an explicitStartCollection, but it's stiffness does "
                   "not match: "
                << explicitStartCollection->GetK() << " vs. "
                << m_InitialStiffnesses.back() << std::endl;
      std::cout << "    difference is "
                << explicitStartCollection->GetK() - m_InitialStiffnesses.back()
                << std::endl;
      std::cout << "So re-estimating that collection first" << std::endl;
      explicitStartCollection->SetK(m_InitialStiffnesses.back());
      AtlasParameterEstimator::Pointer estimator =
          AtlasParameterEstimator::New();
      ...
      m_Current = const_cast<AtlasMeshCollection *>(
          estimator->GetCurrentMeshCollection());
  ...
  // ICM-like algorithm
  for (m_IterationNumber = 0; m_IterationNumber < m_MaximumNumberOfIterations;
       m_IterationNumber++) {
    ...
    for (AtlasMesh::PointsContainer::ConstIterator it =
             m_Current->GetReferencePosition()->Begin();
...
//
//
void AtlasMeshBuilder ::GetCurrentDataAndAlphasCost(
...
//
//
double AtlasMeshBuilder ::GetCurrentPositionCost() const {
  if (!m_Current) {
    return 0.0;
  }
...
//
//
double AtlasMeshBuilder ::GetCurrentCost() const {
...
//
//
AtlasMeshCollection::Pointer AtlasMeshBuilder ::TryToCollapseFast(
  ...
  kvl::AtlasMeshCollectionFastReferencePositionCost::ParametersType
      initialPosition(3);
  initialPosition[0] =
      child->GetReferencePosition()->ElementAt(unifiedPointId)[0];
  initialPosition[1] =
      child->GetReferencePosition()->ElementAt(unifiedPointId)[1];
  initialPosition[2] =
      child->GetReferencePosition()->ElementAt(unifiedPointId)[2];
  ...
  for (; pointParamIt != miniCollection->GetPointParameters()->End();
       ++pointParamIt) {
  ...
  kvl::AtlasMeshCollectionFastReferencePositionCost::ParametersType
      optimalPosition = initialPosition;
  ...
  if (!optimizeUnifiedPointReferencePosition) {
    ...
    const bool canMoveX =
        child->GetPointParameters()->ElementAt(unifiedPointId).m_CanMoveX;
    const bool canMoveY =
        child->GetPointParameters()->ElementAt(unifiedPointId).m_CanMoveY;
    const bool canMoveZ =
        child->GetPointParameters()->ElementAt(unifiedPointId).m_CanMoveZ;
...
//
//
AtlasMeshCollection::Pointer AtlasMeshBuilder ::TryToRetainFast(
    const AtlasMeshCollection *miniCollectionConst,
  ...
  for (AtlasMesh::CellsContainer::ConstIterator cellIt =
    ...
    if (cell->GetType() != itk::CommonEnums::CellGeometry::VERTEX_CELL) {
      continue;
    }
...
//
//
bool AtlasMeshBuilder ::OptimizeReferencePositionFast(
    AtlasMeshCollection *meshCollection, AtlasMesh::CellIdentifier vertexId,
    bool optimize) const {
  ...
  kvl::AtlasMeshCollectionFastReferencePositionCost::ParametersType
      initialPosition(3);
  initialPosition[0] =
      meshCollection->GetReferencePosition()->ElementAt(pointId)[0];
  initialPosition[1] =
      meshCollection->GetReferencePosition()->ElementAt(pointId)[1];
  initialPosition[2] =
      meshCollection->GetReferencePosition()->ElementAt(pointId)[2];
  kvl::AtlasMeshCollectionFastReferencePositionCost::Pointer costFunction =
      kvl::AtlasMeshCollectionFastReferencePositionCost::New();
  costFunction->SetLabelImages(m_LabelImages, m_CompressionLookupTable);
  ...
  const bool canMoveX =
      meshCollection->GetPointParameters()->ElementAt(pointId).m_CanMoveX;
  const bool canMoveY =
      meshCollection->GetPointParameters()->ElementAt(pointId).m_CanMoveY;
  const bool canMoveZ =
      meshCollection->GetPointParameters()->ElementAt(pointId).m_CanMoveZ;
  if (!canMoveX && !canMoveY && !canMoveZ) {
    return true;
  }
  ...
  AtlasMeshCollection::Pointer optimizedCollection =
      const_cast<AtlasMeshCollection *>(
          costFunction->GetCostCalculationMeshCollection());

  meshCollection->SetPositions(optimizedCollection->GetPositions());
  meshCollection->SetReferencePosition(
      optimizedCollection->GetReferencePosition());
...
//
//
double AtlasMeshBuilder ::GetPositionCost(
    const AtlasMeshCollection *meshCollection) const {
  return 0.0;
}
...
//
//
void AtlasMeshBuilder ::GetDataCostAndAlphasCost(
    const AtlasMeshCollection *meshCollection, double &dataCost,
    double &alphasCost) const {
...
//
//
std::vector<AtlasMesh::CellIdentifier> AtlasMeshBuilder ::Permutate(
    const std::vector<AtlasMesh::CellIdentifier> &edgeList) const {

  // Populate sortable vector
  std::vector<EdgeElement> edgeElements;
  for (std::vector<AtlasMesh::CellIdentifier>::const_iterator it =
           edgeList.begin();
       it != edgeList.end(); ++it) {
  ...
  for (std::vector<EdgeElement>::const_iterator it = edgeElements.begin();
...
AtlasMeshBuilder ::GetFakeCopy(const AtlasMeshCollection *input) const {
  ...
  for (unsigned int meshNumber = 0; meshNumber < input->GetNumberOfMeshes();
       meshNumber++) {
    AtlasMesh::PointsContainer::ConstPointer inputPosition =
        (input->GetPositions()[meshNumber]).GetPointer();
    AtlasMesh::PointsContainer::Pointer outputPosition =
        AtlasMesh::PointsContainer::New();
    for (AtlasMesh::PointsContainer::ConstIterator it = inputPosition->Begin();
         it != inputPosition->End(); ++it) {
...
bool AtlasMeshBuilder ::LoadBalancedAnalyzeEdgeFast(
  ...
  for (std::vector<AtlasMesh::CellIdentifier>::const_iterator it =
           nonExistingEdgesEncountered.begin();
       it != nonExistingEdgesEncountered.end(); ++it) {
    if (m_Verbose) {
      std::cout << "    [THREAD " << threadId << "] "
                << "Encountered non-existing edge " << *it << ". Erasing it"
                << std::endl;
    }
    edges.erase(*it);
  }
  ...
  if (edgeId == itk::NumericTraits<AtlasMesh::CellIdentifier>::max()) {
    ...
    if (m_Verbose) {
      std::cout << "    [THREAD " << threadId << "] "
                ...
                << " edges but they are all protected at this point"
                << std::endl;
    }
    ...
    if (m_StuckCount % 10 == 0) {
      std::cout << "    m_StuckCount is at " << m_StuckCount << " (exiting at "
                << stuckCountToGiveUpAt << ")" << std::endl;
    }
    ...
    const int maxRange =
        (1000000 * averageNumberOfSecondsToSleep * 2); // x2 because of average
    const int numberOfMicrosecondsToSleep =
        rand() % maxRange; // Integer between 0 and (maxRange-1)
  ...
  if (m_Verbose) {
    std::cout << "    [THREAD " << threadId << "] "
              << "    Analyzing edge with id: " << edgeId << " (pointIds "
              << p0Id << " and " << p1Id << " ) "
              << " (reference positions "
              << m_Current->GetReferencePosition()->ElementAt(p0Id) << " and "
              << m_Current->GetReferencePosition()->ElementAt(p1Id) << ")"
              << std::endl;
  }
  ...
  if (m_Verbose) {
    std::cout << "    [THREAD " << threadId << "] "
              << "         Removing edge with id: " << edgeId << " from edges ("
              << edges.size() << ")" << std::endl;
  }
  edges.erase(edgeId);

  // Get the mini collection surrounding the edge to try to collapse
  AtlasMeshCollection::ConstPointer miniCollection =
      m_Current->GetRegionGrown(edgeId, 1).GetPointer();
  ...
  if (minTotalCostIndex == 0) {
    if (m_Verbose) {
      std::cout << "    [THREAD " << threadId << "] "
                << "        => retaining edge is best solution" << std::endl;
    }
    ...
    for (unsigned int meshNumber = 0;
      ...
      m_Current->GetPositions()[meshNumber]->ElementAt(edgePoint0Id) =
          retainedMiniCollection->GetPositions()[meshNumber]->ElementAt(
              edgePoint0Id);
    ...
    for (AtlasMesh::CellDataContainer::ConstIterator infoIt =
         ...
         infoIt !=
         retainedMiniCollection->GetReferenceTetrahedronInfos()->End();
         ++infoIt) {
      const_cast<AtlasMesh::CellDataContainer *>(
          m_Current->GetReferenceTetrahedronInfos())
          ->ElementAt(infoIt.Index()) = infoIt.Value();
    }

  } else if (minTotalCostIndex == 1) {
    if (m_Verbose) {
      std::cout << "    [THREAD " << threadId << "] "
                << "        => collapsing edge is best solution" << std::endl;
    }
    ...
    for (unsigned int meshNumber = 0;
         meshNumber < m_Current->GetNumberOfMeshes(); meshNumber++) {
      m_Current->GetPositions()[meshNumber]->ElementAt(edgePoint0Id) =
          collapsedMiniCollection->GetPositions()[meshNumber]->ElementAt(
              edgePoint0Id);
    }
    ...
    for (AtlasMesh::CellDataContainer::ConstIterator infoIt =
         ...
         infoIt !=
         collapsedMiniCollection->GetReferenceTetrahedronInfos()->End();
         ++infoIt) {
      const_cast<AtlasMesh::CellDataContainer *>(
          m_Current->GetReferenceTetrahedronInfos())
          ->ElementAt(infoIt.Index()) = infoIt.Value();
...
AtlasMeshBuilder ::GetRandomizedEdgesAsSet() const {

  // Convert from vector to set
  std::vector<AtlasMesh::CellIdentifier> edgesVector =
      this->GetRandomizedEdges();
  std::set<AtlasMesh::CellIdentifier> edges;
  for (std::vector<AtlasMesh::CellIdentifier>::const_iterator it =
           edgesVector.begin();
...
AtlasMeshBuilder ::LoadBalancedThreaderCallback(void *arg) {
  ...
  while (true) {
    ...
    try {
      if (!str->m_Builder->LoadBalancedAnalyzeEdgeFast(
              str->m_Edges, str->m_PointOccupancies, threadId)) {
      ...
      std::cout << "Exception === Exception === Exception === Exception === "
                   "Exception === Exception"
                << std::endl;
      std::cout << "   An exception was thrown in thread " << threadId
                << std::endl;
      std::cout << "     " << e << std::endl;
      std::cout << "Exception === Exception === Exception === Exception === "
                   "Exception === Exception"
