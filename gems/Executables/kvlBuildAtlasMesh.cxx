...
public:
  ...
  virtual void Execute(const itk::Object *caller,
    ...
    if ((typeid(event) == typeid(itk::IterationEvent)) ||
      ...
      kvl::AtlasMeshBuilder::ConstPointer builder =
          static_cast<const kvl::AtlasMeshBuilder *>(caller);

      // Write out the current mesh collection
      std::ostringstream fileNameStream;
      fileNameStream << m_LogDirectory << "/CurrentMeshCollection"
                     << builder->GetIterationNumber();
      std::string fileName = fileNameStream.str();
      if (!builder->GetCurrentMeshCollection()->Write(fileName.c_str())) {
        std::cerr << "Couldn't write mesh collection to file " << fileName
                  << std::endl;
        exit(-1);
      }
      ...
      out << builder->GetIterationNumber() << "   " << currentDataCost << "   "
          << currentAlphasCost << "   " << currentPositionCost << "   "
      ...
      kvl::AtlasMeshBuilder::ConstPointer builder =
          static_cast<const kvl::AtlasMeshBuilder *>(caller);
      std::cout << "Progress: " << builder->GetProgress() * 100 << "%"
                << std::endl;
    }
  }
...
int main(int argc, char **argv) {
  ...
  if (argc < 8) {
    std::cerr << "Usage: " << argv[0]
              << " numberOfUpsamplingSteps meshSizeX meshSizeY meshSizeZ "
                 "stiffness logDirectory fileName1 [ fileName2 ... ]"
              << std::endl;
  ...
  for (int labelImageNumber = 0; labelImageNumber < labelImages.size();
       labelImageNumber++) {
    std::ostringstream fileNameStream;
  ...
  out << "iterationNumber     dataCost     alphasCost     positionCost    "
         "totalCost"
      << std::endl;
  out << "---------------------------------------------------------------------"
         "---------"
  ...
  if (itksys::SystemTools::FileExists(explicitStartCollectionFileName.c_str(),
    ...
    if (!explicitStartCollection->Read(
            explicitStartCollectionFileName.c_str())) {
      std::cerr << "Couldn't read mesh from file "
                << explicitStartCollectionFileName << std::endl;
      return -1;
    }
  }
  ...
  if (!(fs.fail())) {
    std::cout << "Reading " << edgeCollapseEncouragmentFactorFileName
              << std::endl;
    ...
    if (std::getline(fs, line)) {
      ...
      std::cout << "Using edgeCollapseEncouragmentFactor: "
                << edgeCollapseEncouragmentFactor << std::endl;
    } else {
