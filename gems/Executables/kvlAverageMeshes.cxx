...
int main(int argc, char **argv) {
  ...
  if (argc < 6) {
    std::cerr << "Usage: " << argv[0]
              << " inputMeshCollection initialMeshCollection Katlas "
                 "KtimePoints outputMeshCollection [numberOfMeshesToAverage]"
              << std::endl;
    std::cerr << "   initialMeshCollection is expected to have initial "
  ...
  kvl::AtlasMeshCollection::Pointer inputCollection =
      kvl::AtlasMeshCollection::New();
  inputCollection->Read(argv[1]);

  kvl::AtlasMeshCollection::Pointer initialCollection =
      kvl::AtlasMeshCollection::New();
  ...
  kvl::AtlasMeshAveragingConjugateGradientOptimizer::Pointer optimizer =
      kvl::AtlasMeshAveragingConjugateGradientOptimizer::New();
  ...
  for (int j = 0; j < 10; j++) {
    ...
    while (!ready) {
      ...
      printf("Iteration %d-%d: took a maximal step of %.3f and the new cost is "
             "now %.4f \n",
  ...
  if (!initialCollection->Write(argv[5])) {
    std::cerr << "Could not write mesh collection to " << argv[5] << ".gz"
              << std::endl;
    exit(-1);
  }
  std::cout << "Just wrote mesh collection to " << argv[5] << ".gz"
            << std::endl;
