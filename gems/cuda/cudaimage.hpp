...
namespace kvl {
namespace cuda {
template <typename ElementType, unsigned char nDims,
...
public:
  ...
  template <typename NextType, typename... Values>
  __device__ bool PointInRange(const NextType iVal,
                               const Values... vals) const {
  ...
  __device__ bool PointInRangeFromArray(const IndexType location[nDims]) const {
    bool result = true;
...
private:
  ...
  __device__ void copyToArray(IndexType *) const {}

  // Extracts variadic template arguments into a array of length nDims
  template <typename NextType, typename... Values>
  __device__ void copyToArray(IndexType *nextLoc, const NextType iVal,
                              const Values... vals) const {
    *nextLoc = iVal;
...
public:
  ...
  DimensionType GetDimensions() const { return this->dims; }

  size_t ElementCount() const { return this->dims.ElementCount(); }
  ...
  cudaPitchedPtr
  GetCudaPitchedPtr(const std::vector<ElementType> &target) const {
    cudaPitchedPtr res;
  ...
  void SetMemory(const int value) {
    CUDA_SAFE_CALL(
        cudaMemset3D(*(this->d_elements), value, this->GetCudaExtent()));
  }
...
private:
  ...
  void SetDimensionsCommon() {
    ...
    this->d_elements =
        std::unique_ptr<cudaPitchedPtr, CudaDeviceDeleter>(new cudaPitchedPtr);
