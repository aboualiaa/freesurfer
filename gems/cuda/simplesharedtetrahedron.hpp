...
public:
  ...
  __device__ SimpleSharedTetrahedron(const MeshSupplier &srcMesh,
      ...
      : mesh(srcMesh), tet(&tetrahedron[0][0]), transf(&M[0][0]) {
  ...
  __device__ void LoadAndBoundingBox(const size_t   iTet,
    ...
    if ((threadIdx.x < nDims) && (threadIdx.y == 0)) {
      for (unsigned int iVert = 0; iVert < nVertices; iVert++) {
        this->tet[(iVert * nDims) + threadIdx.x] =
            this->mesh.GetVertexCoordinate(this->tetId, iVert, threadIdx.x);
      }
  ...
  __device__ void ComputeBarycentricTransform() {
    ...
    if ((threadIdx.x == 0) && (threadIdx.y == 0)) {
      ...
      // of the tetrahedron
      const InvertType a =
          this->tet[(1 * nDims) + 0] - this->tet[(0 * nDims) + 0];
      const InvertType b =
          this->tet[(2 * nDims) + 0] - this->tet[(0 * nDims) + 0];
      const InvertType c =
          this->tet[(3 * nDims) + 0] - this->tet[(0 * nDims) + 0];
      const InvertType d =
          this->tet[(1 * nDims) + 1] - this->tet[(0 * nDims) + 1];
      const InvertType e =
          this->tet[(2 * nDims) + 1] - this->tet[(0 * nDims) + 1];
      const InvertType f =
          this->tet[(3 * nDims) + 1] - this->tet[(0 * nDims) + 1];
      const InvertType g =
          this->tet[(1 * nDims) + 2] - this->tet[(0 * nDims) + 2];
      const InvertType h =
          this->tet[(2 * nDims) + 2] - this->tet[(0 * nDims) + 2];
      const InvertType i =
          this->tet[(3 * nDims) + 2] - this->tet[(0 * nDims) + 2];
  ...
  __device__ void TransformToBarycentric(ArgType p[nDims + 1], const ArgType z,
                                         const ArgType y,
  ...
  __device__ bool PointInside(const ArgType z, const ArgType y,
                              const ArgType x) const {
    bool inside = true;
    ...
    if ((p[0] < 0) || (p[1] < 0) || (p[2] < 0) || (p[3] < 0)) {
      ...
      nxtColAdd[0] =
          -(this->transf[(0 * nDims) + 1] + this->transf[(1 * nDims) + 1] +
            this->transf[(2 * nDims) + 1]);
      ...
      nxtSliceAdd[0] =
          -(this->transf[(0 * nDims) + 2] + this->transf[(1 * nDims) + 2] +
            this->transf[(2 * nDims) + 2]);
      ...
      for (unsigned int iVert = 0; iVert < nVertices; iVert++) {
        if (inside && (p[iVert] == 0)) {
          if (this->CheckBorder(nxtRowAdd[iVert], nxtColAdd[iVert],
                                nxtSliceAdd[iVert])) {
            inside = false;
          }
        }
  ...
  __device__ bool CheckBorder(ArgType a, ArgType b, ArgType c) const {
  ...
  __device__ InterpolateType BarycentricInterpolation(
      const InterpolateType vertexValues[nVertices], const ArgType z,
      const ArgType y, const ArgType x) const {
    // Get the set of barycentric co-ordinates
    ArgType p[nDims + 1];
    this->TransformToBarycentric(p, z, y, x);
