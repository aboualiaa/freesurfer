...
public:
  ...
  __device__ CoordinateType GetVertexCoordinate(MeshIndexType iTet,
                                                MeshIndexType iVert,
                                                unsigned char iDim) const {
    MeshIndexType iVertexId = this->vertexMap(iTet, iVert);

    return this->vertices(iVertexId, iDim);
  }

  __device__ MeshIndexType GetTetrahedraCount() const {
    return this->vertexMap.dims[0];
  }

  __device__ AlphasType GetAlpha(MeshIndexType iTet, MeshIndexType iVert,
                                 MeshIndexType iAlpha) const {
    MeshIndexType iVertexId = this->vertexMap(iTet, iVert);
...
public:
  ...
  TetrahedralMesh_GPU<CoordinateType, MeshIndexType, AlphasType>
  getArg() const {
...
private:
  ...
  void SendVertices(kvl::AtlasMesh::ConstPointer mesh) {
    ...
    for (auto pointIt = mesh->GetPoints()->Begin();
      ...
      for (MeshIndexType i = 0; i < nDims; i++) {
        size_t idx = vertexDims.GetLinearIndex(
            static_cast<MeshIndexType>(pointIt.Index()), i);
  ...
  void SendAlphas(kvl::AtlasMesh::ConstPointer mesh) {
    ...
    for (auto pointDataIt = mesh->GetPointData()->Begin();
         pointDataIt != mesh->GetPointData()->End(); ++pointDataIt) {
      for (MeshIndexType iAlpha = 0; iAlpha < alphasDims[1]; iAlpha++) {
        AlphasType nxt = pointDataIt->Value().m_Alphas[iAlpha];

        size_t idx = alphasDims.GetLinearIndex(
            static_cast<MeshIndexType>(pointDataIt->Index()), iAlpha);
        alphas.at(idx) = nxt;
      }
    }
