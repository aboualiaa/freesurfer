...
namespace cuda {
template <typename MeshSupplier, typename VertexAction, typename IndexType,
          typename Internal = double>
void RunSimpleSharedTetrahedron(const IndexType nz, const IndexType ny,
                                const IndexType nx, const MeshSupplier &mesh,
                                VertexAction &va) {
  const unsigned int nBlockx = 1024;

  const size_t nTetrahedra = mesh.GetTetrahedraCount();

  const size_t volume = static_cast<size_t>(nz) * static_cast<size_t>(ny) *
  ...
  SimpleSharedTetrahedronInteriorKernel<MeshArg, VertexAction, IndexType,
                                        MeshCoordType, Internal>
      <<<grid, threads>>>(nz, ny, nx, mesh.getArg(), va);
