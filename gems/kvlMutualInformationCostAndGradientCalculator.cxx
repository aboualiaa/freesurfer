...
//
//
void MutualInformationCostAndGradientCalculator ::SetImage(
...
//
//
void MutualInformationCostAndGradientCalculator ::Rasterize(
  ...
  Histogrammer::ConditionalIntensityDistributionType uniformDistribution(
      numberOfBins, 1.0 / static_cast<double>(numberOfBins));
  ...
  for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
    ...
    for (int binNumber = 0; binNumber < numberOfBins; binNumber++) {
      const double jointProbability =
          histogram[classNumber][binNumber] / m_NumberOfVoxels + 1e-15;
    ...
    negativeMutualInformation +=
        marginalProbabilityOfClass * log(marginalProbabilityOfClass);
  }
  for (int binNumber = 0; binNumber < numberOfBins; binNumber++) {
    const double marginalProbabilityOfIntensity =
        marginalIntensityDistribution[binNumber];
    negativeMutualInformation +=
...
//
//
void MutualInformationCostAndGradientCalculator ::
  ...
  TetrahedronInteriorConstIterator<Histogrammer::BinnedImageType::PixelType> it(
      m_Histogrammer->GetBinnedImage(), p0, p1, p2, p3);
  ...
  for (; !it.IsAtEnd(); ++it) {
    ...
    for (unsigned int classNumber = 0; classNumber < numberOfClasses;
      ...
      //
      xGradientBasis += classConditionalLikelihood *
                        it.GetExtraLoadingNextRowAddition(classNumber);
      yGradientBasis += classConditionalLikelihood *
                        it.GetExtraLoadingNextColumnAddition(classNumber);
      zGradientBasis += classConditionalLikelihood *
    ...
    for (unsigned int classNumber = 0; classNumber < numberOfClasses;
      ...
      xGradientBasis += logClassConditionalLikelihood *
                        it.GetExtraLoadingNextRowAddition(classNumber);
      yGradientBasis += logClassConditionalLikelihood *
                        it.GetExtraLoadingNextColumnAddition(classNumber);
      zGradientBasis += logClassConditionalLikelihood *
                        it.GetExtraLoadingNextSliceAddition(classNumber);
    } // End loop over all classes
