...
//
//
void Histogrammer ::ComputeRobustRange(const ImageType *image,
                                       double &robustMin, double &robustMax) {
  ...
  for (itk::ImageRegionConstIterator<ImageType> it(image,
                                                   image->GetBufferedRegion());
       !it.IsAtEnd(); ++it) {
    const int binNumber = itk::Math::Round<int>(
        slope * (static_cast<double>(it.Value()) - offset));
    histogram[binNumber]++;
    sumOfHistogram++;
  }
  ...
  for (int binNumber = 1; binNumber < numberOfBins; binNumber++) {
    cumulativeHistogram[binNumber] = cumulativeHistogram[binNumber - 1] +
                                     histogram[binNumber] / sumOfHistogram;
  }
  ...
  for (int binNumber = 0; binNumber < 3; binNumber++) {
    std::cout << "histogram[ " << binNumber << " ]: " << histogram[binNumber]
              << std::endl;
  }
  for (int binNumber = (numberOfBins - 3); binNumber < numberOfBins;
       binNumber++) {
    std::cout << "histogram[ " << binNumber << " ]: " << histogram[binNumber]
              << std::endl;
  }
  ...
  for (int binNumber = 0; binNumber < 3; binNumber++) {
    std::cout << "cumulativeHistogram[ " << binNumber
              << " ]: " << cumulativeHistogram[binNumber] << std::endl;
  }
  for (int binNumber = (numberOfBins - 3); binNumber < numberOfBins;
       binNumber++) {
    std::cout << "cumulativeHistogram[ " << binNumber
...
//
//
void Histogrammer ::UpdateBinnedImage() {
  //
  ...
  for (; !it.IsAtEnd(); ++it, ++binnedIt) {
    int binNumber = itk::Math::Round<int>(
        slope * (static_cast<double>(it.Value()) - offset));

    // Intensities outside of robust range get a sentinel value
    if ((it.Value() < robustMin) || (it.Value() > robustMax)) {
      binNumber = -1;
    }
...
//
//
void Histogrammer ::Rasterize(const AtlasMesh *mesh) {
  ...
  if (mesh->GetPointData()->Begin().Value().m_Alphas.Size() !=
      numberOfClasses) {
    itkExceptionMacro(
        << "number of classes in m_ConditionalIntensityDistributions and "
           "provided mesh don't match")
  }
...
//
//
bool Histogrammer ::RasterizeTetrahedron(
  ...
  TetrahedronInteriorConstIterator<BinnedImageType::PixelType> it(
      m_BinnedImage, p0, p1, p2, p3);
  ...
  for (; !it.IsAtEnd(); ++it) {
    ...
    for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
      const double tmp =
          m_ConditionalIntensityDistributions[classNumber]
                                             [binNumber]      // Likelihood
    ...
    m_ThreadSpecificMinLogLikelihoods[threadNumber] -=
        weightToDistribute * log(denominator);
    for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
