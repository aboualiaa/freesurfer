...
protected:
  WrappedSingleValuedCostFunction() { m_CostFunction = 0; }
...
public:
  ...
  /** Run-time type information (and related methods). */
  itkTypeMacro(ParameterOrderPowellOptimizer,
  ...
  const ParametersType &GetCurrentPosition() const {
    ...
    for (unsigned int i = 0; i < m_ParameterOrder.Size(); i++) {
      if (m_ParameterOrder[i]) {
        currentPosition[i] =
            m_WrappedOptimizer->GetCurrentPosition()[m_ParameterOrder[i] - 1];
      }
    }
  ...
  void StartOptimization(void) {
    ...
    WrappedSingleValuedCostFunction::Pointer metric =
        WrappedSingleValuedCostFunction::New();
    metric->SetUp(const_cast<CostFunctionType *>(this->GetCostFunction()),
                  this->GetInitialPosition(), m_ParameterOrder);
    ...
    for (unsigned int i = 0; i < m_ParameterOrder.Size(); i++) {
      if (m_ParameterOrder[i]) {
        ...
        initialPosition[m_ParameterOrder[i] - 1] =
...
protected:
  ParameterOrderPowellOptimizer() {
    ...
    m_WrappedOptimizer->SetMetricWorstPossibleValue(
