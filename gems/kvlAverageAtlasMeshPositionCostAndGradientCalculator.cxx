...
//
//
void AverageAtlasMeshPositionCostAndGradientCalculator ::Rasterize(
  ...
  if ((m_Positions.size() == 0) || (m_Ks.size() != m_Positions.size())) {
    itkExceptionMacro(
        << "No positions set or Ks don't not same number as positions");
  }
  ...
  if (m_CachedInternalMeshes.size() == 0) {
    ...
    for (int meshNumber = 0; meshNumber < m_Positions.size(); meshNumber++) {
      ...
      helper->GenerateFromSingleMesh(const_cast<AtlasMesh *>(mesh), 0,
                                     m_Ks[meshNumber]);
      helper->SetReferencePosition(const_cast<AtlasMesh::PointsContainer *>(
          m_Positions[meshNumber].GetPointer()));
      m_CachedInternalMeshes.push_back(
          const_cast<AtlasMesh *>(helper->GetReferenceMesh().GetPointer()));
    }
  }
  ...
  for (int meshNumber = 0; meshNumber < m_CachedInternalMeshes.size();
    ...
    internalMesh->SetPoints(
        const_cast<AtlasMesh::PointsContainer *>(mesh->GetPoints()));

    // Now do the work
    AtlasMeshPositionCostAndGradientCalculator::Pointer calculator =
        AtlasMeshPositionCostAndGradientCalculator::New();
    calculator->Rasterize(internalMesh);

    // If successful, add contribution to cost
    const double minLogLikelihoodContribution =
    ...
    AtlasPositionGradientContainerType::ConstIterator sourceIt =
        calculator->GetPositionGradient()->Begin();
