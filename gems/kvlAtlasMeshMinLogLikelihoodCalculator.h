...
public:
  ...
  inline void StartNewSpan(int x, int y, int z,
                           const unsigned char *sourcePointer) {
  ...
  float GetMinLogLikelihood() const { return m_MinLogLikelihood; }

  void SetMapCompToComp(std::vector<unsigned char> *mapCompToComp) {
    m_mapCompToComp = mapCompToComp;
  }
  std::vector<unsigned char> *GetMapCompToComp() { return m_mapCompToComp; }
...
public:
  ...
  /** */
  void SetMapCompToComp(std::vector<unsigned char> *mapCompToComp) {
    for (std::vector<FragmentProcessorType>::iterator it =
             this->GetFragmentProcessors().begin();
