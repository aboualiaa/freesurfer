...
//
//
bool AtlasMeshAlphaDrawer ::RasterizeTetrahedron(
  ...
  const float alphaInVertex0 =
      (mesh->GetPointData()->ElementAt(id0).m_Alphas)[m_ClassNumber];
  const float alphaInVertex1 =
      (mesh->GetPointData()->ElementAt(id1).m_Alphas)[m_ClassNumber];
  const float alphaInVertex2 =
      (mesh->GetPointData()->ElementAt(id2).m_Alphas)[m_ClassNumber];
  const float alphaInVertex3 =
      (mesh->GetPointData()->ElementAt(id3).m_Alphas)[m_ClassNumber];
  ...
  it.AddExtraLoading(alphaInVertex0, alphaInVertex1, alphaInVertex2,
                     alphaInVertex3);
  for (; !it.IsAtEnd(); ++it) {
