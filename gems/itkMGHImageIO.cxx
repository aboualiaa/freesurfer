...
bool MGHImageIO::CanReadFile(const char *FileNameToRead) {
...
void MGHImageIO::PrintSelf(std::ostream &os, Indent indent) const {
  Superclass::PrintSelf(os, indent);
  std::string strSep = ", ";

  os << indent << "Data Dimensions: (" << m_Dimensions[0] << strSep
     << m_Dimensions[1] << strSep << m_Dimensions[2] << ")\n"
     << indent << "Data Spacing: (" << m_Spacing[0] << strSep << m_Spacing[1]
     << strSep << m_Spacing[2] << ")\n"
     << indent << "Scalar Type: " << m_ComponentType << std::endl
     << indent << "Number of Frames: " << m_NumberOfComponents << std::endl;

  os << indent << "RAS to IJK matrix: " << std::endl;
}

void MGHImageIO::ReadImageInformation() {
  gzFile fp;
...
void MGHImageIO::ReadVolumeHeader(gzFile fp) {
  ...
  TReadZ(fp, version);
  TReadZ(fp, bufInt);
  m_Dimensions[0] = (unsigned int)bufInt;
  TReadZ(fp, bufInt);
  m_Dimensions[1] = (unsigned int)bufInt;
  TReadZ(fp, bufInt);
  m_Dimensions[2] = (unsigned int)bufInt;
  // next is nframes
  TReadZ(fp, bufInt);
  m_NumberOfComponents = (unsigned int)bufInt;
  TReadZ(fp, type);
...
void MGHImageIO::Read(void *pData) {
  gzFile fp;
  fp = gzopen(m_FileName.c_str(), "rb");
  if (!fp) {
    itkExceptionMacro(<< "Can't find/open file: " << m_FileName);
  ...
  if (m_NumberOfComponents > 1) {
    ...
    for (unsigned int frameIndex = 0; frameIndex < m_NumberOfComponents;
         ++frameIndex) {
      // read current frame
      ...
      for (unsigned int ui = 0; ui < numPixels;
           ++ui, pSrc += componentSize, pDst += pixelSize) {
        for (unsigned int byteCount = 0; byteCount < componentSize; ++byteCount)
...
bool MGHImageIO::CanWriteFile(const char *name) {
...
void MGHImageIO::WriteImageInformation() {
...
void MGHImageIO::Write(const void *buffer) {
  std::string extension = GetExtension(m_FileName);
...
void MGHImageIO::PermuteFrameValues(const void *buffer, char *tempmemory) {
  ...
  for (unsigned int pixelIndex = 0; pixelIndex < numPixels;
    ...
    for (unsigned int componentIndex = 0; componentIndex < m_NumberOfComponents;
