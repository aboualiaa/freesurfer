...
public:
  ...
  PointDataContainerType *GetPointParameters() { return m_PointParameters; }
  ...
  PointsContainerType *GetReferencePosition() { return m_ReferencePosition; }
  ...
  //
  void
  SetPositions(const std::vector<PointsContainerType::Pointer> &positions) {
    m_Positions = positions;
    m_Meshes.clear();
  }

  void SetPosition(int n, PointsContainerType::Pointer position) {
    m_Positions[n] = position;
    m_Meshes.clear();
  }
  ...
  //
  void GenerateFromSingleMesh(AtlasMesh *mesh, unsigned int numberOfMeshes,
                              double K);
  ...
  GetRegionGrown(AtlasMesh::CellIdentifier seedId, unsigned int radius,
...
protected:
  ...
  //
  using MeshSourceType = itk::AutomaticTopologyMeshSource<kvl::AtlasMesh>;
  void FillCubeWithTetrahedra(
