...
//
//
bool AtlasMeshDeformationOptimizer ::Go() {
  bool hasMoved = false;
  while (true) {
...
double AtlasMeshDeformationOptimizer ::Step() {
  ...
  try {
    ...
    std::cout << "Something wrong with finding and/or optimizing the search "
                 "direction -- giving up"
              << std::endl;
  ...
  if (maximalDeformation <= m_MaximalDeformationStopCriterion) {
    if ((m_IterationNumber > 0) || (maximalDeformation == 0.0)) {
      if (m_Verbose) {
        std::cout << "Optimizer: maximalDeformation is too small; stopping"
                  << std::endl;
      }
      this->InvokeEvent(DeformationEndEvent());
      return 0.0;
...
//
//
double AtlasMeshDeformationOptimizer ::ComputeMaximalDeformation(
...
//
//
double AtlasMeshDeformationOptimizer ::ComputeInnerProduct(
...
AtlasMeshDeformationOptimizer ::LinearlyCombineDeformations(
  ...
  AtlasPositionGradientContainerType::ConstIterator defIt2 =
      deformation2->Begin();
  for (; defIt1 != deformation1->End(); ++defIt1, ++defIt2) {
    newDeformation->InsertElement(defIt1.Index(), beta1 * defIt1.Value() +
                                                      beta2 * defIt2.Value());
  }
...
//
AtlasPositionGradientContainerType::Pointer
AtlasMeshDeformationOptimizer ::ScaleDeformation(
...
//
//
void AtlasMeshDeformationOptimizer ::Initialize() {
  if (!m_Calculator) {
...
//
//
void AtlasMeshDeformationOptimizer ::DoLineSearch(
    const AtlasMesh::PointsContainer *startPosition, double startCost,
  ...
  const double initialDirectionalDerivative = this->ComputeInnerProduct(
      startGradient, searchDirection); // gradient' * p;
  ...
  //
  const double maximalDeformationOfSearchDirection =
      this->ComputeMaximalDeformation(searchDirection);
  const double alphaMax =
      m_LineSearchMaximalDeformationLimit / maximalDeformationOfSearchDirection;
  ...
  for (int bracketingIterationNumber = 0;
    ...
    const double directionalDerivative =
        this->ComputeInnerProduct(gradient, searchDirection); // gradient' * p
    ...
    if ((cost > (initialCost + c1 * alpha * initialDirectionalDerivative)) ||
      ...
      if (m_Verbose) {
        std::cout
            << "[BRACKETING] Hit a really bad solution -- ready for zooming: "
    ...
    if (m_Verbose) {
      std::cout << "[BRACKETING] Expanding from: " << maximalDeformation
                << std::endl;
    }
    ...
    if (d2Square > 0) {
      ...
      newAlpha =
          alpha -
          (alpha - previousAlpha) * (directionalDerivative + d2 - d1) /
              (directionalDerivative - previousDirectionalDerivative + 2 * d2);
      if (m_Verbose) {
        std::cout << "   interpolation suggestion: "
                  << newAlpha * maximalDeformationOfSearchDirection
                  << std::endl;
      }
  ...
  if (bracketingFailed) {
    ...
    if (m_Verbose) {
      std::cout << "Line search couldn't find a suitable bracketing interval"
                << std::endl;
    }
  ...
  if (m_Verbose) {
    std::cout << "[ZOOMING] Zooming with low="
              ...
              << " and high=" << highAlpha * maximalDeformationOfSearchDirection
              << std::endl;
  }
  ...
  while (true) {
    ...
    const double d2Square =
        pow(d1, 2) - leftDirectionalDerivative * rightDirectionalDerivative;
    if ((d2Square < 0) || (highCost == itk::NumericTraits<double>::max())) {
      ...
      alpha =
          rightAlpha -
          (rightAlpha - leftAlpha) * (rightDirectionalDerivative + d2 - d1) /
