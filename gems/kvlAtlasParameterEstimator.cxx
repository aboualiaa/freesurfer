...
//
//
void AtlasParameterEstimator ::Estimate(bool verbose) {
  // Sanity checking
  ...
  while ((((previousMinLogLikelihoodTimesPrior -
    ...
    if (verbose) {
      std::cout << "================ IterationNumber: " << m_IterationNumber
                << " CurrentMinLogLikelihoodTimesPrior: "
                << m_CurrentMinLogLikelihoodTimesPrior
                << "================" << std::endl;
    }
    m_IterationNumber++;
...
//
//
void AtlasParameterEstimator ::EstimateAlphas() {
  ...
  while ((((previousCost - currentCost) / fabsf(currentCost)) >
    ...
    for (int labelImageNumber = 0;
         labelImageNumber < numberOfLabelImagesToVisit; labelImageNumber++) {
      ...
      statisticsCollector->Rasterize(
          m_MeshCollection->GetMesh(labelImageNumber));

      // Add statistics to pooled statistics
      pooledIt = pooledStatistics->Begin();
      StatisticsContainerType::ConstIterator statIt =
    ...
    pooledIt = pooledStatistics->Begin();
    PointDataContainerType::Iterator pointParamIt =
        m_MeshCollection->GetPointParameters()->Begin();
    while (pooledIt != pooledStatistics->End()) {
...
//
//
void AtlasParameterEstimator ::SmoothAlphas() {

  if (m_AlphasSmoothingFactor != 0) {
    // Retrieve sigma of the smoothing kernel
    const double maximalSigma =
        0.05 * (m_LabelImages[0]->GetLargestPossibleRegion().GetSize()[0]);
    const double sigma =
        maximalSigma * (1 - log(2 - m_AlphasSmoothingFactor) / log(2));
...
//
//
double AtlasParameterEstimator ::EstimatePositions() {

  double totalMinLogLikelihoodTimesPrior = 0.0f;

  for (m_LabelImageNumber = 0; m_LabelImageNumber < m_NumberOfLabelImages;
       m_LabelImageNumber++) {
    //
    totalMinLogLikelihoodTimesPrior +=
...
//
double
AtlasParameterEstimator ::EstimatePosition(unsigned int labelImageNumber) {
  ...
  AtlasMeshToLabelImageCostAndGradientCalculator::Pointer calculator =
      AtlasMeshToLabelImageCostAndGradientCalculator::New();
  ...
  optimizer->SetIterationEventResolution(
...
AtlasParameterEstimator ::CalculateCurrentPositionCostAndGradient(
...
//
AtlasPositionGradientContainerType::Pointer
AtlasParameterEstimator ::GetCurrentPositionGradient(
...
//
//
void AtlasParameterEstimator ::HandleOptimizerEvent(
