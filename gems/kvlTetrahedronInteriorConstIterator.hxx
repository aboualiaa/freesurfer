...
TetrahedronInteriorConstIterator<TPixel>::TetrahedronInteriorConstIterator(
    const ImageType *ptr, const PointType &p0, const PointType &p1,
    const PointType &p2, const PointType &p3)
    : Superclass(ptr, RegionType()), m_InterpolatedValues(4),
      m_NextRowAdditions(4), m_NextColumnAdditions(4), m_NextSliceAdditions(4),
      m_ColumnBeginInterpolatedValues(4), m_SliceBeginInterpolatedValues(4) {
  ...
  for (int loadingNumber = 0; loadingNumber < 4; loadingNumber++) {
    m_ColumnBeginInterpolatedValues[loadingNumber] =
        m_InterpolatedValues[loadingNumber];
    m_SliceBeginInterpolatedValues[loadingNumber] =
        m_InterpolatedValues[loadingNumber];
  }
...
//
template <typename TPixel>
void TetrahedronInteriorConstIterator<TPixel>::AddExtraLoading(
    const double &alpha0, const double &alpha1, const double &alpha2,
    const double &alpha3) {

  //
  m_InterpolatedValues.push_back(
      alpha0 * m_InterpolatedValues[0] + alpha1 * m_InterpolatedValues[1] +
      alpha2 * m_InterpolatedValues[2] + alpha3 * m_InterpolatedValues[3]);
  m_ColumnBeginInterpolatedValues.push_back(
      alpha0 * m_ColumnBeginInterpolatedValues[0] +
      alpha1 * m_ColumnBeginInterpolatedValues[1] +
      alpha2 * m_ColumnBeginInterpolatedValues[2] +
      alpha3 * m_ColumnBeginInterpolatedValues[3]);
  m_SliceBeginInterpolatedValues.push_back(
      alpha0 * m_SliceBeginInterpolatedValues[0] +
      alpha1 * m_SliceBeginInterpolatedValues[1] +
      alpha2 * m_SliceBeginInterpolatedValues[2] +
      alpha3 * m_SliceBeginInterpolatedValues[3]);

  m_NextRowAdditions.push_back(
      alpha0 * m_NextRowAdditions[0] + alpha1 * m_NextRowAdditions[1] +
      alpha2 * m_NextRowAdditions[2] + alpha3 * m_NextRowAdditions[3]);
  m_NextColumnAdditions.push_back(
      alpha0 * m_NextColumnAdditions[0] + alpha1 * m_NextColumnAdditions[1] +
      alpha2 * m_NextColumnAdditions[2] + alpha3 * m_NextColumnAdditions[3]);
  m_NextSliceAdditions.push_back(
      alpha0 * m_NextSliceAdditions[0] + alpha1 * m_NextSliceAdditions[1] +
      alpha2 * m_NextSliceAdditions[2] + alpha3 * m_NextSliceAdditions[3]);
}
...
TetrahedronInteriorConstIterator<TPixel>::operator++() {
...
//
template <typename TPixel>
void TetrahedronInteriorConstIterator<TPixel>::MoveOnePixel() {
  ...
  if (this->m_PositionIndex[0] < (this->m_EndIndex[0] - 1)) {
    ...
    for (int loadingNumber = 0; loadingNumber < numberOfLoadings;
         loadingNumber++) {
      m_ColumnBeginInterpolatedValues[loadingNumber] +=
          m_NextColumnAdditions[loadingNumber];
      m_InterpolatedValues[loadingNumber] =
          m_ColumnBeginInterpolatedValues[loadingNumber];
    }
    ...
    for (int loadingNumber = 0; loadingNumber < numberOfLoadings;
         loadingNumber++) {
      m_SliceBeginInterpolatedValues[loadingNumber] +=
          m_NextSliceAdditions[loadingNumber];
      m_ColumnBeginInterpolatedValues[loadingNumber] =
          m_SliceBeginInterpolatedValues[loadingNumber];
      m_InterpolatedValues[loadingNumber] =
          m_SliceBeginInterpolatedValues[loadingNumber];
...
//
template <typename TPixel>
bool TetrahedronInteriorConstIterator<TPixel>::IsOutsideTetrahdron() const {
...
//
template <typename TPixel>
bool TetrahedronInteriorConstIterator<TPixel>::CheckBorderCase(double a,
                                                               double b,
                                                               double c) {
  if (a < 0) {
    return true;
