...
CroppedImageReader ::GetTransformOfFileName(const std::string &filename) {
  ...
  if (io) {
    ...
    itkGenericExceptionMacro(
        << "Couldn't reconstruct image-to-world transform for file ");
  }
...
//
//
void CroppedImageReader ::Read(const char *fileName,
  ...
  if (!boundingFileName) {
    ...
    for (std::vector<TransformType::InputPointType>::const_iterator it =
             meshCornerPoints.begin();
         it != meshCornerPoints.end(); ++it) {
      TransformType::OutputPointType mappedCorner =
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (max[i] >
          (reader->GetOutput()->GetBufferedRegion().GetSize()[i] - 1)) {
    ...
    itk::ImageRegionConstIterator<ImageType> originalImageIterator(
        reader->GetOutput(), m_OriginalImageRegion);
    itk::ImageRegionIterator<ImageType> croppedImageIterator(
        m_Image, m_CroppedImageRegion);
    for (; !originalImageIterator.IsAtEnd();
         ++originalImageIterator, ++croppedImageIterator) {
  ...
  if (m_DownSamplingFactor != 1) {
    ...
    m_WorldToImageTransform->Scale(1 /
                                   static_cast<double>(m_DownSamplingFactor));
