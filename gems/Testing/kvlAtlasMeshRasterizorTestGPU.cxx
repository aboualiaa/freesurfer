...
int main(int argc, char *argv[]) {
  ...
  kvl::AtlasMeshAlphaDrawer::Pointer referenceAlphaDrawer =
      kvl::AtlasMeshAlphaDrawer::New();
  ...
  clock.Stop();
  std::cout << "Time taken by reference alpha drawer: " << clock.GetMean()
            << std::endl;
  ...
  std::cout << "Maximum error in alpha drawer: " << maximumAlphaError
            << std::endl;
  ...
  //
  // ===================================================
  kvl::AtlasMeshVisitCounter::Pointer visitCounter =
  ...
  for (; !it.IsAtEnd(); ++it) {
    if (it.Value() > 1) {
      std::cout << "Got " << static_cast<int>(it.Value())
                << " visits in voxel with index " << it.GetIndex() << std::endl;
    }
  }
  ...
  gradientCalculator->SetImages(std::vector<ImageType::ConstPointer>(
      1, const_cast<ImageType *>(image.GetPointer())));
  gradientCalculator->SetParameters(means_, variances, mixtureWeights,
  ...
  for (int testRunNumber = 0; testRunNumber < 10; testRunNumber++) {
    ...
    gradientCalculator->Rasterize(mesh);
    clock.Stop();
    std::cout << "Average time taken for subsequent iterations of gradient "
                 "calculator: "
              << clock.GetMean() << std::endl;
  }
  ...
  // Compare against a reference implementation
  kvl::AtlasMeshToIntensityImageCostAndGradientCalculator::Pointer
      referenceGradientCalculator =
          kvl::AtlasMeshToIntensityImageCostAndGradientCalculator::New();
  referenceGradientCalculator->SetImages(std::vector<ImageType::ConstPointer>(
      1, const_cast<ImageType *>(image.GetPointer())));
  referenceGradientCalculator->SetParameters(means_, variances, mixtureWeights,
                                             numberOfGaussiansPerClass);
  clock.Reset();
  clock.Start();
  ...
  for (int testRunNumber = 0; testRunNumber < 10; testRunNumber++) {
    ...
    referenceGradientCalculator->Rasterize(mesh);
    clock.Stop();
    std::cout << "Average time taken for subsequent iterations of reference "
                 "gradient calculator: "
              << clock.GetMean() << std::endl;
  }
  ...
  const double referenceCost =
      referenceGradientCalculator->GetMinLogLikelihoodTimesPrior();
  std::cout << "Error in cost evaluation: " << referenceCost - cost
            << std::endl;
  std::cout << "      cost: " << cost << std::endl;
  std::cout << "      referenceCost: " << referenceCost << std::endl;

  kvl::AtlasPositionGradientContainerType::ConstPointer gradient =
      gradientCalculator->GetPositionGradient();
  kvl::AtlasPositionGradientContainerType::ConstPointer referenceGradient =
      referenceGradientCalculator->GetPositionGradient();
  kvl::AtlasPositionGradientContainerType::ConstIterator gradIt =
      gradient->Begin();
  kvl::AtlasPositionGradientContainerType::ConstIterator refGradIt =
      referenceGradient->Begin();
