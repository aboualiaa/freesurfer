...
typedef boost::mpl::list<unsigned char, unsigned short, unsigned int,
                         unsigned long>
    TestLengthTypes;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testEqualityOperators1d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testGetLinearIndexOutOfRange, LengthType,
                              TestLengthTypes) {
  ...
  BOOST_CHECK_THROW(d.GetLinearIndex((LengthType)(n0 + 1), zero, zero),
                    std::range_error);
  BOOST_CHECK_THROW(d.GetLinearIndex(zero, (LengthType)(n1 + 1), zero),
                    std::range_error);
  BOOST_CHECK_THROW(d.GetLinearIndex(zero, zero, (LengthType)(n2 + 1)),
                    std::range_error);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(testGetLinearIndex1d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testGetLinearIndex2d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
    const LengthType n0 = 1 + gPRNG.GetInteger<LengthType>(101);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testGetLinearIndex3d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
    const LengthType n0 = 1 + gPRNG.GetInteger<LengthType>(101);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testPointInRangePositive, LengthType,
                              TestLengthTypes) {
  const LengthType n0 = 4;
  ...
  BOOST_TEST(d.PointInRange((LengthType)(n0 - 1), (LengthType)(n1 - 1),
                            (LengthType)(n2 - 1)));
}

BOOST_AUTO_TEST_CASE_TEMPLATE(testPointInRangeNegative, LengthType,
                              TestLengthTypes) {
  const LengthType n0 = 4;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testElementCount, LengthType, TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
    ...
    const LengthType n0 =
        gPRNG.GetInteger<LengthType>(std::numeric_limits<LengthType>::max());
    const LengthType n1 =
        gPRNG.GetInteger<LengthType>(std::numeric_limits<LengthType>::max());
    const LengthType n2 =
        gPRNG.GetInteger<LengthType>(std::numeric_limits<LengthType>::max());
    ...
    const size_t total3 = static_cast<size_t>(n0) * static_cast<size_t>(n1) *
...
template <typename LengthType, int nDims> void checkLinearIndexToLocation() {
  ...
  // Check in range
  BOOST_CHECK_THROW(d.LinearIndexToLocation(elementCount, result),
                    std::range_error);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testLinearIndexToLocation1d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testLinearIndexToLocation2d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testLinearIndexToLocation3d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(testLinearIndexToLocation4d, LengthType,
                              TestLengthTypes) {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
