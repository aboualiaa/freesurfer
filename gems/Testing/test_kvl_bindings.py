...
def test_cropped_image_image_bindings(matlab_fixture, cropped_image_fixture):
    np.testing.assert_allclose(
        matlab_fixture["imageBuffer"], cropped_image_fixture.getImageBuffer()
    )
    expected_transform = matlab_fixture["imageToWorldTransformMatrix"]
    np.testing.assert_allclose(
        expected_transform,
        cropped_image_fixture.transform_matrix.as_numpy_array,
    )
...
def test_image_bindings(matlab_fixture, image_fixture):
    np.testing.assert_allclose(
        matlab_fixture["imageBuffer"], image_fixture.getImageBuffer()
    )
    np.testing.assert_allclose(
        matlab_fixture["imageToWorldTransformMatrix"],
...
def test_mesh_binding(matlab_fixture, image_fixture, mesh_fixture):
    ...
    np.testing.assert_allclose(
        matlab_fixture["nodePositions"], mesh_fixture.points
    )
    ...
    np.testing.assert_allclose(
        matlab_fixture["nodePositions_after_modification"],
        mesh_fixture.points + 1,
    )
    mesh_fixture.scale([1 / 10])
    np.testing.assert_allclose(
        matlab_fixture["nodePositions_after_scaling"], mesh_fixture.points
    )


@pytest.mark.slowtest
def test_mesh_binding_with_class_name(
    matlab_fixture, image_fixture, mesh_fixture
):
    buffer = image_fixture.getImageBuffer()
...
@pytest.mark.slowtest
def test_calculator_and_optimzer_bindings(
    matlab_fixture, image_fixture, mesh_fixture
):
    calculator = gems.KvlCostAndGradientCalculator(
        "MutualInformation", [image_fixture], "Affine"
    )
    (
        mutualInformation_cost,
        mutualInformation_gradient,
    ) = calculator.evaluate_mesh_position(mesh_fixture)
    np.testing.assert_almost_equal(
        matlab_fixture["mutualInformation_cost"][0][0], mutualInformation_cost
    )
    np.testing.assert_allclose(
        matlab_fixture["mutualInformation_gradient"], mutualInformation_gradient
    )
    ...
    np.testing.assert_almost_equal(
        matlab_fixture["minLogLikelihoodTimesPrior"][0][0],
        minLogLikelihoodTimesPrior,
    )
    np.testing.assert_almost_equal(
        matlab_fixture["maximalDeformation"][0][0], maximalDeformation
    )
