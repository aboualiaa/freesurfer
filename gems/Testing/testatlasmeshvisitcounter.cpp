...
typedef boost::mpl::list<kvl::cuda::VisitCounterSimple<float, float>,
    ...
    CUDAImplTypes;
#else
typedef boost::mpl::list<kvl::cuda::VisitCounterSimple<double, double>,
                         kvl::cuda::VisitCounterTetrahedralMesh>
    CUDAImplTypes;
#endif
#endif
...
void CheckVisitCounter(kvl::interfaces::AtlasMeshVisitCounter *visitCounter,
  ...
  itk::ImageRegionConstIteratorWithIndex<
      kvl::interfaces::AtlasMeshVisitCounter::ImageType>
      it(img, img->GetBufferedRegion());
  itk::ImageRegionConstIteratorWithIndex<kvl::AtlasMeshVisitCounter::ImageType>
      itOrig(originalVisitCounter->GetImage(),
             originalVisitCounter->GetImage()->GetBufferedRegion());
...
void LowerCorner(kvl::interfaces::AtlasMeshVisitCounter *visitCounter) {
  ...
  float verts[nVertices][nDims] = {{-delta, -delta, -delta},
...
void OriginOnly(kvl::interfaces::AtlasMeshVisitCounter *visitCounter) {
  ...
  float verts[nVertices][nDims] = {
      {-delta, -delta, -delta}, {delta, 0, 0}, {0, delta, 0}, {0, 0, delta}};
...
void FarCornerOnly(kvl::interfaces::AtlasMeshVisitCounter *visitCounter) {
  ...
  const float delta = 0.1f;
  BOOST_REQUIRE(delta < 0.5f);
  float verts[nVertices][nDims] = {{1 - delta, 1 - delta, 1 - delta},
                                   {1 + delta, 1, 1},
                                   {1, 1 + delta, 1},
                                   {1, 1, 1 + delta}};
...
void NoVertices(kvl::interfaces::AtlasMeshVisitCounter *visitCounter) {
  ...
  const float delta = 0.1f;
  BOOST_REQUIRE(delta < 0.5f);
  float verts[nVertices][nDims] = {{0.5f + delta, 0.5f + delta, 0.5f + delta},
...
void LowerCornerExact(kvl::interfaces::AtlasMeshVisitCounter *visitCounter) {
  ...
  float verts[nVertices][nDims] = {{0, 0, 0}, {1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
...
void CompareVisitImages(const ImageType *standard, const ImageType *compare) {
...
void CheckVisitCounterWithPermutations(
    kvl::interfaces::AtlasMeshVisitCounter *visitCounter,
  ...
  ImageType::ConstPointer standardVisit = nullptr;
  {
    ...
    standardVisit =
        ApplyVisitCounterToMesh(&origVisitCounter, targetImage, baseMesh);
  }
...
void AutoCorners(kvl::interfaces::AtlasMeshVisitCounter *visitCounter,
                 const float scaleTetrahedron, const int imageSize) {
  ...
  for (unsigned char corner = 0; corner < nCorners; corner++) {
    BOOST_TEST_CONTEXT("Corner : " << static_cast<unsigned int>(corner)) {
      ...
      ImageType::Pointer targetImage =
          kvl::Testing::CreateImageCube<ImageType>(imageSize + 1, 0);
      BOOST_TEST_CHECKPOINT("Generated target image");

      CheckVisitCounterWithPermutations(visitCounter, targetImage,
                                        baseVertices);
    }
  }
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(LowerCornerExactGPUSimple, ImplType,
                              CUDAImplTypes) {
  ImplType visitCounter;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(UpperCornerExactGPUSimple, ImplType,
                              CUDAImplTypes) {
  ImplType visitCounter;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(AutoCornersLargeImageLargeTetrahedronGPUSimple,
                              ImplType, CUDAImplTypes) {
  ImplType visitCounter;
...
BOOST_DATA_TEST_CASE(ConsistencyCheck, boost::unit_test::data::xrange(1, 2),
                     scale) {
  kvl::AtlasMeshVisitCounterCPUWrapper visitCounter;
  ...
  ImageType::Pointer targetImage =
      kvl::Testing::CreateImageCube<ImageType>(scale + 1, 0);
  ...
  for (int k = 0; k < 2; k++) {
    for (int j = 0; j < 2; j++) {
      for (int i = 0; i < 2; i++) {
        ...
        BOOST_TEST_INFO("(" << i << "," << j << "," << k << ")");
        BOOST_TEST_INFO(
            "Unit Tetrahedron : " << TetrahedronToString(unitVertices));
        BOOST_TEST_INFO(
            "Scale Tetrahedron : " << TetrahedronToString(scaleVertices));
        BOOST_CHECK_EQUAL(unitResult->GetPixel(idx),
                          scaleResult->GetPixel(idxScale));
      }
    }
  }
}
