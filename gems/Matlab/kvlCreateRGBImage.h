...
public:
  ...
  void Run(int nlhs, mxArray *plhs[], int nrhs,
    ...
    for (; !it.IsAtEnd(); ++it, ++data1, ++data2, ++data3, ++data4) {
      ...
      if (counter % 10000 == 0) {
        std::cout << "pixelValues: " << static_cast<unsigned>(*data1) << ","
                  << static_cast<unsigned>(*data2) << ","
                  << static_cast<unsigned>(*data3) << ","
                  << static_cast<unsigned>(*data4) << std::endl;
      }
      pixel.SetRed(*data1);
    ...
    const int imageHandle = kvl::MatlabObjectArray::GetInstance()->AddObject(
        RGBAImage.GetPointer());
