...
public:
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    const std::string usageString =
        "Usage: optimizer = kvlGetOptimizer( typeName, mesh, calculator )\n "
        "where typeName = "
        "{'FixedStepGradientDescent','GradientDescent','ConjugateGradient','L-"
        "BFGS'}";
    ...
    kvl::AtlasMeshPositionCostAndGradientCalculator::Pointer calculator =
        const_cast<kvl::AtlasMeshPositionCostAndGradientCalculator *>(
    ...
    for (int argumentNumber = 0; argumentNumber < (nrhs - 3) / 2;
         argumentNumber++) {
      if (!mxIsChar(prhs[3 + 2 * argumentNumber]) ||
          !mxIsDouble(prhs[3 + 2 * argumentNumber + 1])) {
    ...
    const int optimizerHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(optimizer);
