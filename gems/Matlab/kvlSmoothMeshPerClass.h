...
public:
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
      std::cout << "classesToSmooth[" << classNumber
                << "]: " << classesToSmooth[classNumber] << std::endl;
    }
    ...
    kvl::AtlasMeshCollection::ConstPointer smoothedMeshCollection =
        smoother->GetSmoothedMeshCollection().GetPointer();

    // Set the alphas of the mesh to the smoothed version
    mesh->SetPointData(const_cast<AtlasMesh::PointDataContainer *>(
        smoothedMeshCollection->GetPointParameters()));
  }
