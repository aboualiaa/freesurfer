...
public:
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    const std::string usageString =
        "Usage: calculator = kvlGetCostAndGradientCalculator( typeName, "
        "image(s), boundaryCondition, [ transform ], [ means ], [ variances ], "
        "[ mixtureWeights ], [ numberOfGaussiansPerClass ], [ targetPoints ] "
        ")\n where typeName = "
        "{'AtlasMeshToIntensityImage','ConditionalGaussianEntropy','"
        "MutualInformation','PointSet'}\n and boundaryCondition = {'Sliding', "
        "'Affine', 'Translation', 'None'}";
    if ((nrhs < 3) || !mxIsChar(prhs[0]) || !mxIsInt64(prhs[1]) ||
        !mxIsChar(prhs[2])) {
    ...
    if (nrhs > 4) {
      ...
      for (int gaussianNumber = 0; gaussianNumber < numberOfGaussians;
        ...
        for (int contrastNumber = 0; contrastNumber < numberOfContrasts;
             contrastNumber++) {
          mean[contrastNumber] = (mxGetPr(
              prhs[4]))[gaussianNumber + numberOfGaussians * contrastNumber];
        }
      ...
      if (false) {
        ...
        for (unsigned int gaussianNumber = 0;
             gaussianNumber < numberOfGaussians; gaussianNumber++) {
      ...
      for (unsigned int gaussianNumber = 0; gaussianNumber < numberOfGaussians;
        ...
        for (unsigned int row = 0; row < numberOfContrasts; row++) {
          for (unsigned int col = 0; col < numberOfContrasts; col++) {
            variance[row][col] =
                mxGetPr(prhs[5])[gaussianNumber + row * numberOfGaussians +
                                 col * numberOfGaussians * numberOfContrasts];
          }
        }
    ...
    if (nrhs > 7) {
      ...
      for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
        numberOfGaussiansPerClass[classNumber] =
            static_cast<int>((mxGetPr(prhs[7]))[classNumber]);
      }
    ...
    AtlasMesh::PointsContainer::Pointer targetPoints =
    ...
    if (nrhs > 8) {
      ...
      for (int pointNumber = 0; pointNumber < numberOfPoints; pointNumber++) {
        ...
        for (int dimensionNumber = 0; dimensionNumber < 3; dimensionNumber++) {
          point[dimensionNumber] = (mxGetPr(
              prhs[8]))[pointNumber + numberOfPoints * dimensionNumber];
        }
    ...
    case 'A': {
      ...
      myCalculator->SetParameters(means, variances, mixtureWeights,
    ...
    case 'C': {
      ...
      ConditionalGaussianEntropyCostAndGradientCalculator::Pointer
          myCalculator =
    ...
    case 'A': {
      ...
      calculator->SetBoundaryCondition(
          AtlasMeshPositionCostAndGradientCalculator::AFFINE);
      break;
    }
    case 'T': {
      std::cout << "TRANSLATION" << std::endl;
      calculator->SetBoundaryCondition(
          AtlasMeshPositionCostAndGradientCalculator::TRANSLATION);
      break;
    }
    case 'N': {
      std::cout << "NONE" << std::endl;
      calculator->SetBoundaryCondition(
          AtlasMeshPositionCostAndGradientCalculator::NONE);
      break;
    }
    default: {
    ...
    const int calculatorHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(calculator);
