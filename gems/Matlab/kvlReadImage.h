...
public:
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if ((nrhs != 1) || !mxIsChar(prhs[0]) || (nlhs != 2)) {
    ...
    const int imageHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(caster->GetOutput());
    typedef kvl::CroppedImageReader::TransformType TransformType;
    TransformType::Pointer transform = TransformType::New();
    reader->GetWorldToImageTransform()->GetInverse(transform);
    const int transformHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(transform);
