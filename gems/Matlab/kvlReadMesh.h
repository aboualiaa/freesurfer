...
public:
  ...
  itkTypeMacro(ReadMesh, itk::Object);
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs > 2) {
      startingMeshNumber = static_cast<int>(*(mxGetPr(prhs[2])));
    }

    if (nrhs > 3) {
      K = static_cast<float>(*(mxGetPr(prhs[3])));
    }

    std::cout << "meshCollectionFileName: " << meshCollectionFileName
              << std::endl;
    std::cout << "transform: " << transform.GetPointer() << std::endl;
    std::cout << "startingMeshNumber: " << startingMeshNumber << std::endl;
    ...
    if (transform) {
      ...
      if (determinant < 0) {
        std::cout << "Careful here: the applied transformation will turn "
                     "positive tetrahedra into negative ones."
                  << std::endl;
    ...
    if (startingMeshNumber == -1) {
      mesh = meshCollection->GetReferenceMesh();
    } else {
      mesh = meshCollection->GetMesh(startingMeshNumber);
    }

    // Store the mesh in persistent memory
    const int meshHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(mesh);
