...
class GetAverageAtlasMeshPositionCostAndGradientCalculator
    : public MatlabRunner {
public:
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    const std::string usageString =
        "Usage: calculator = "
        "kvlGetAverageAtlasMeshPositionCostAndGradientCalculator( "
        "meshCollection, K0, K1, transform )";
    if ((nrhs < 4) || !mxIsInt64(prhs[0]) || !mxIsInt64(prhs[3])) {
    ...
    for (int meshNumber = 0; meshNumber < meshCollection->GetPositions().size();
         meshNumber++) {
      positions.push_back(
          meshCollection->GetPositions()[meshNumber].GetPointer());
      Ks.push_back(K1);
    }
