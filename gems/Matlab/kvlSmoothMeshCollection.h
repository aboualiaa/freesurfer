...
public:
  ...
  virtual void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    smoother->SetMeshCollection(
        const_cast<kvl::AtlasMeshCollection *>(meshCollection.GetPointer()));
    ...
    const int smoothedMeshCollectionHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(
            smoothedMeshCollection);
