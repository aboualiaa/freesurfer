...
public:
  ...
  void Run(int nlhs, mxArray *plhs[], int nrhs,
    ...
    if ((nrhs < 2) || !mxIsInt64(prhs[0]) || (nlhs != 1)) {
    ...
    if (classNumber >= 0) {
      // Rasterize the specified prior. If the class number is 0, then pre-fill
      // everything so that parts not overlayed by the mesh are still considered
      // to the background
      ...
      if (classNumber == 0) {
        (const_cast<AlphaImageType *>(alphaDrawer->GetImage()))
            ->FillBuffer(1.0);
      }
      ...
      //
      // because mxCreateNumericArray initializes all the matrix values to zero,
