...
int main(int argc, char *argv[]) {
  ...
  if (con_and_fillx != 0) {
    int stat = 1;
    if (con_and_fillx_fname != nullptr) {
      // open file containing the list of fillseeds and path/label files
      FILE *fp2 = fopen(con_and_fillx_fname, "re");
      if (nullptr == fp2) {
      ...
      printf("Connecting and filling vertices in path at fillseed %d\n",
             fillseed);
      stat = con_and_fill_pathx(source_file, dest_file, surfacefname, fillseed);
    }
    exit(stat);
  }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (strcasecmp(option, "--help") == 0) {
      ...
      checkoptsonly = 0;
    } else if (strcasecmp(option, "--single") == 0) {
      single_path = 1;
    } else if (strcasecmp(option, "--label2path") == 0) {
      label_to_path = 1;
    } else if (strcasecmp(option, "--path2label") == 0) {
      path_to_label = 1;
...
static void print_usage() {
  ...
  printf("\n");
  printf("   --single : only convert a single path, and don't use sentinel "
         "values\n");
  ...
  printf("   --connect subject hemi : connect path (input and output must be "
         "paths)\n");
  printf(
...
static int guess_file_type(char *fname, int *is_path, int *is_label) {
  ...
  *is_label = 0;
  ...
  if (found == 0)
    ErrorReturn(ERROR_BADFILE, (ERROR_BADFILE, "Couldn't identify %s", fname));
...
static int convert_path_to_label(char *fname, char *ofname) {
  ...
  if (ERROR_NONE != err) {
    ErrorReturn(ERROR_BADFILE, (ERROR_BADFILE, "Couldn't read %s", fname));
  }
  ...
  label = LabelAlloc(label_size, nullptr, nullptr);
  if (nullptr == label) {
    ErrorReturn(
        ERROR_NO_MEMORY,
        (ERROR_NO_MEMORY, "Couldn't allocate label of %d points", label_size));
  }
...
static int convert_label_to_path(char *fname, char *ofname) {
  ...
  paths = static_cast<PATH **>(calloc(num_paths, sizeof(PATH *)));
  if (nullptr == paths) {
  ...
  for (path_index = 0; path_index < num_paths; path_index++) {
    ...
    /* Make the path. */
    paths[path_index] = PathAlloc(path_size, nullptr);
    if (nullptr == paths) {
      ErrorReturn(
          ERROR_NO_MEMORY,
          (ERROR_NO_MEMORY, "Couldn't allocate path of %d points", path_size));
    }
  ...
  if (0 != err) {
    ErrorReturn(ERROR_BADFILE, (ERROR_BADFILE, "Couldn't write to %s", ofname));
  }
...
static int convert_single_label_to_path(char *fname, char *ofname) {
  ...
  /* Allocate path objects. */
  paths = static_cast<PATH **>(calloc(1, sizeof(PATH *)));
  if (nullptr == paths) {
    ErrorReturn(ERROR_NO_MEMORY,
