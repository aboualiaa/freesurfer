...
with open(
    path.join(path.dirname(path.realpath(__file__)), "requirements.txt")
) as file:
    requirements = [
        line for line in file.read().splitlines() if not line.startswith("#")
    ]
...
def find_libs(libname, required=True):
    libraries = glob.glob(
        "**/%s.*%s*.so" % (libname, platform.system().lower()), recursive=True
    )
    if required and not libraries:
        print(
            "error: could not find %s library that matches the current python version"
            % libname
        )
        sys.exit(1)
...
setup(
    ...
    package_data={
        "freesurfer": operator.add(
            find_libs("bindings"), find_libs("labelfusion", required=False)
        ),
        "freesurfer.samseg": find_libs("gemsbindings"),
    },
