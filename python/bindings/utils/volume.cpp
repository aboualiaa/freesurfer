...
py::object Bridge::python() {
  ...
  if (!p_mri)
    throw std::runtime_error("cannot bridge to python as MRI instance is null");
  if (!p_mri->ischunked)
    throw std::runtime_error(
        "image is too large to fit into contiguous memory");
  ...
  if (mri_buffer.size() == 0) {
    ...
    case MRI_UCHAR:
      ...
      break;
    case MRI_SHORT:
      dtype = py::dtype::of<short>();
      break;
    case MRI_INT:
      dtype = py::dtype::of<int>();
      break;
    case MRI_LONG:
      dtype = py::dtype::of<long>();
      break;
    case MRI_FLOAT:
      dtype = py::dtype::of<float>();
      break;
    default:
      throw py::value_error("unknown MRI data type ID: " +
                            std::to_string(p_mri->type));
    }

    // squeeze the 4D MRI to determine the actual represented shape
    std::vector<ssize_t> shape = {p_mri->width};
    if (p_mri->height > 1)
      shape.push_back(p_mri->height);
    if (p_mri->depth > 1)
      shape.push_back(p_mri->depth);
    if (p_mri->nframes > 1)
      shape.push_back(p_mri->nframes);
  ...
  switch (ndims) {
  case 1:
    pyobj = fsmodule.attr("Overlay")(mri_buffer);
    break;
  case 2:
    pyobj = fsmodule.attr("Image")(mri_buffer);
    break;
  case 3:
    pyobj = fsmodule.attr("Volume")(mri_buffer);
    break;
  }
...
void Bridge::transferParameters(py::object &pyobj) {
  ...
  // extract resolution
  if (ndims == 2)
    pyobj.attr("pixsize") = py::make_tuple(p_mri->xsize, p_mri->ysize);
  if (ndims == 3)
    pyobj.attr("voxsize") =
        py::make_tuple(p_mri->xsize, p_mri->ysize, p_mri->zsize);
  ...
  if (ndims == 3) {
    if (p_mri->te == 0) {
      pyobj.attr("te") = py::none();
    } else {
      pyobj.attr("te") = p_mri->te;
    }
    if (p_mri->tr == 0) {
      pyobj.attr("tr") = py::none();
    } else {
      pyobj.attr("tr") = p_mri->tr;
    }
    if (p_mri->ti == 0) {
      pyobj.attr("ti") = py::none();
    } else {
      pyobj.attr("ti") = p_mri->ti;
    }
    if (p_mri->flip_angle == 0) {
      pyobj.attr("flip_angle") = py::none();
    } else {
      pyobj.attr("flip_angle") = p_mri->flip_angle;
    }
  }
}
...
void Bridge::updateSource() {
  ...
  if (!p_mri)
    throw std::runtime_error("cannot bridge to python as MRI instance is null");
  if (source.is(py::none()))
    throw py::value_error("cannot update source if it does not exist");
...
MRI *Bridge::mri() {
  ...
  if (source.is(py::none()))
    throw py::value_error("cannot generate MRI instance without source object");
  ...
  if (py::isinstance<py::array_t<double>>(mri_buffer))
    mri_buffer = py::array_t<float>(mri_buffer);
  if (py::isinstance<py::array_t<bool>>(mri_buffer))
    mri_buffer = py::array_t<uchar>(mri_buffer);
  if (py::isinstance<py::array_t<char>>(mri_buffer))
    mri_buffer = py::array_t<int>(mri_buffer);
  if (py::isinstance<py::array_t<long>>(mri_buffer))
    mri_buffer = py::array_t<int>(mri_buffer);

  // determine valid MRI type from numpy datatype
  int dtype;
  if (py::isinstance<py::array_t<uchar>>(mri_buffer)) {
    dtype = MRI_UCHAR;
  } else if (py::isinstance<py::array_t<int>>(mri_buffer)) {
    dtype = MRI_INT;
  } else if (py::isinstance<py::array_t<short>>(mri_buffer)) {
    dtype = MRI_SHORT;
  } else if (py::isinstance<py::array_t<long>>(mri_buffer)) {
    dtype = MRI_LONG;
  } else if (py::isinstance<py::array_t<float>>(mri_buffer)) {
    dtype = MRI_FLOAT;
  } else {
    throw py::value_error(
        "unsupported array dtype " +
        py::str(mri_buffer.attr("dtype")).cast<std::string>());
  }

  // initialize a header-only MRI structure with the known shape (expanded to 4D)
  std::vector<int> expanded,
      shape   = mri_buffer.attr("shape").cast<std::vector<int>>();
  int nframes = source.attr("nframes").cast<int>();
  int ndims   = source.attr("basedims").cast<int>();
  switch (ndims) {
  case 1:
    expanded = {shape[0], 1, 1, nframes};
    break;
  case 2:
    expanded = {shape[0], shape[1], 1, nframes};
    break;
  case 3:
    expanded = {shape[0], shape[1], shape[2], nframes};
    break;
  }
  ...
  // voxel size
  if (ndims == 2) {
    std::vector<float> pixsize =
        source.attr("pixsize").cast<std::vector<float>>();
    mri->xsize = pixsize[0];
    mri->ysize = pixsize[1];
    mri->zsize = 1.0;
  } else if (ndims == 3) {
    std::vector<float> voxsize =
        source.attr("voxsize").cast<std::vector<float>>();
    mri->xsize = voxsize[0];
    mri->ysize = voxsize[1];
    mri->zsize = voxsize[2];
  }
  ...
  if (!lookup.is(py::none())) {
    ...
    ctab->entries  = (COLOR_TABLE_ENTRY **)calloc(ctab->nentries,
    ...
    for (auto item : lut) {
      ...
      std::vector<int> color =
          item.second.attr("color").cast<std::vector<int>>();
      if (color.size() != 4)
        throw std::runtime_error("lookup table colors must be RGBA");
      ctab->entries[idx] = (CTE *)malloc(sizeof(CTE));
      strncpy(ctab->entries[idx]->name, name.c_str(),
              sizeof(ctab->entries[idx]->name));
  ...
  // transfer scan parameters if volume
  if (ndims == 3) {
    if (!py::object(source.attr("te")).is(py::none()))
      mri->te = source.attr("te").cast<float>();
    if (!py::object(source.attr("tr")).is(py::none()))
      mri->tr = source.attr("tr").cast<float>();
    if (!py::object(source.attr("ti")).is(py::none()))
      mri->ti = source.attr("ti").cast<float>();
    if (!py::object(source.attr("flip_angle")).is(py::none()))
      mri->flip_angle = source.attr("flip_angle").cast<double>();
  }
...
*/
template <typename T>
static void sampleIntoTypedVolume(py::array_t<T> volume, py::array_t<T> weights,
                                  arrayc<double> coords,
                                  arrayc<double> values) {
...
*/
void sampleIntoVolume(py::array volume, py::array weights,
                      arrayc<double> coords, arrayc<double> values) {
...
template <typename T>
static py::array_t<T>
resampleTypedVolumeLinear(py::array_t<T> source_vol, py::object target_shape,
                          py::array_t<double> target2source) {
  // make target volume
  py::module     numpy = py::module::import("numpy");
  py::array_t<T> target_vol =
      numpy.attr("zeros")(target_shape); // TODO construct with dtype instead
  ...
  double a11   = t2s_m(0, 0);
  double a12   = t2s_m(0, 1);
  double a13   = t2s_m(0, 2);
  double a14   = t2s_m(0, 3);
  double a21   = t2s_m(1, 0);
  double a22   = t2s_m(1, 1);
  double a23   = t2s_m(1, 2);
  double a24   = t2s_m(1, 3);
  double a31   = t2s_m(2, 0);
  double a32   = t2s_m(2, 1);
  double a33   = t2s_m(2, 2);
  double a34   = t2s_m(2, 3);
    ...
    for (int y = 0; y < ny; y++) {
      for (int x = 0; x < nx; x++) {
        ...
        for (int f = 0; f < nf; f++) {
          ...
          T v0 = (valid_xl && valid_yl && valid_zl)
                     ? source_m(sx_low, sy_low, sz_low, f)
                     : 0.0;
          T v1 = (valid_xh && valid_yl && valid_zl)
                     ? source_m(sx_high, sy_low, sz_low, f)
                     : 0.0;
          T v2 = (valid_xl && valid_yh && valid_zl)
                     ? source_m(sx_low, sy_high, sz_low, f)
                     : 0.0;
          T v3 = (valid_xl && valid_yl && valid_zh)
                     ? source_m(sx_low, sy_low, sz_high, f)
                     : 0.0;
          T v4 = (valid_xh && valid_yl && valid_zh)
                     ? source_m(sx_high, sy_low, sz_high, f)
                     : 0.0;
          T v5 = (valid_xl && valid_yh && valid_zh)
                     ? source_m(sx_low, sy_high, sz_high, f)
                     : 0.0;
          T v6 = (valid_xh && valid_yh && valid_zl)
                     ? source_m(sx_high, sy_high, sz_low, f)
                     : 0.0;
          T v7 = (valid_xh && valid_yh && valid_zh)
                     ? source_m(sx_high, sy_high, sz_high, f)
                     : 0.0;

          // interpolate
          T val = w0 * v0 + w1 * v1 + w2 * v2 + w3 * v3 + w4 * v4 + w5 * v5 +
                  w6 * v6 + w7 * v7;
...
template <typename T>
static py::array_t<T>
resampleTypedVolumeNearest(py::array_t<T> source_vol, py::object target_shape,
                           py::array_t<double> target2source) {
  // make target volume
  py::module     numpy = py::module::import("numpy");
  py::array_t<T> target_vol =
  ...
  for (int z = 0; z < nz; z++) {
    for (int y = 0; y < ny; y++) {
      for (int x = 0; x < nx; x++) {
        ...
        for (int f = 0; f < nf; f++)
          target_m(x, y, z, f) = source_m(rsx, rsy, rsz, f);
      }
    }
  }
...
*/
py::array resampleVolumeLinear(py::array source_vol, py::object target_shape,
                               py::array_t<double> target2source) {
  if (py::isinstance<py::array_t<double>>(source_vol)) {
    return resampleTypedVolumeLinear<double>(source_vol, target_shape,
                                             target2source);
  } else {
    return resampleTypedVolumeLinear<float>(source_vol, target_shape,
                                            target2source);
  }
}
...
/*
  ...
  \param target2source  4x4 affine matrix that transforms target coords to source coords.
*/
py::array resampleVolumeNearest(py::array source_vol, py::object target_shape,
                                py::array_t<double> target2source) {
  if (py::isinstance<py::array_t<double>>(source_vol)) {
    return resampleTypedVolumeNearest<double>(source_vol, target_shape,
                                              target2source);
  } else {
    return resampleTypedVolumeNearest<float>(source_vol, target_shape,
                                             target2source);
  }
}
