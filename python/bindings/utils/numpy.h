...
template <class T>
py::array_t<T> makeArray(std::vector<ssize_t> shape,
                         std::vector<ssize_t> strides, const T *const data,
                         bool free = true) {
  py::capsule capsule;
  if (free) {
...
template <class T>
py::array_t<T> makeArray(std::vector<ssize_t> shape, MemoryOrder order,
                         const T *const data, bool free = true) {
