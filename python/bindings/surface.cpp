...
void PySurface::setVertexPositions(FloatArrayC &positions) {
  if (positions.request().shape != std::vector<ssize_t>({m_ptr->nvertices, 3}))
    logFatal(1) << "vertex array shape must be (" << m_ptr->nvertices << ", 3)";
...
FloatArrayF PySurface::computeSurf2Vox(PyVolume &vol) {
  MATRIX *sras2ras;
  if (m_ptr->vg.valid) {
    MRI *tmp = MRIallocHeader(m_ptr->vg.width, m_ptr->vg.height,
                              m_ptr->vg.depth, MRI_UCHAR, 1);
  ...
  FloatArrayF pymat =
      FloatArrayF({4, 4}, cstrides({4, 4}, sizeof(float)), sras2vox->data);
  MatrixFree(&ras2vox);
  MatrixFree(&sras2vox);
  MatrixFree(&sras2ras);
  return pymat;
...
/*
  Wrapper for IsMRISselfIntersecting. Returns true if surface is
  self-intersecting.
*/
bool PySurface::isSelfIntersecting() { return IsMRISselfIntersecting(m_ptr); }
...
IntArray readAnnotation(const std::string &filename) {
  ...
  if (fp == nullptr)
    logFatal(1) << "could not read annotation file " << filename;
...
FloatArrayC PySurface::sampleParameterization(const FloatArrayC &param) {
  ...
  for (int vno = 0; vno < m_ptr->nvertices; vno++) {
    ...
    for (int f = 0; f < nframes; f++) {
      *ptr++ =
          du * dv * *(src + u1 * vdim * nframes + v1 * nframes + f) +
          (1.0f - du) * dv * *(src + u0 * vdim * nframes + v1 * nframes + f) +
          (1.0f - du) * (1.0f - dv) *
              *(src + u0 * vdim * nframes + v0 * nframes + f) +
