...
typedef py::array_t<float, py::array::forcecast | py::array::c_style>
    FloatArrayC;
typedef py::array_t<float, py::array::forcecast | py::array::f_style>
    FloatArrayF;
...
public:
  ...
  void write(const std::string &filename) {
    MRISwrite(m_ptr, filename.c_str());
  }
  ...
  // geometry
  void copyGeometry(PySurface &surf) {
    copyVolGeom(&surf.mris()->vg, &m_ptr->vg);
  }
  ...
  FloatArray getVerticesDeprecated() {
    logWarning << "The Surface vertices property has been replaced by "
                  "get_vertex_positions() and set_vertex_positions(). It will "
  ...
  void setVerticesDeprecated(FloatArrayC &array) {
    logWarning << "The Surface vertices property has been replaced by "
                  "get_vertex_positions() and set_vertex_positions(). It will "
                  "be completely removed soon.";
    setVertexPositions(array);
  }
  FloatArray getFacesDeprecated() {
    logWarning << "The Surface faces property has been replaced by "
                  "get_face_vertices(). It will be completely removed soon.";
...
inline void bindSurface(py::module &m) {
  ...
  surf.def("copy_geometry",
           (void (PySurface::*)(PySurface &)) & PySurface::copyGeometry);
  surf.def("copy_geometry",
           (void (PySurface::*)(PyVolume &)) & PySurface::copyGeometry);
  ...
  surf.def_property("vertices", &PySurface::getVerticesDeprecated,
                    &PySurface::setVerticesDeprecated);
  // faces
  ...
  surf.def("parameterization_map",
           &PySurface::computeParameterizationMapBarycentric);
  // misc utilities
