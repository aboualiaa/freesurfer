...
KvlImage::KvlImage(const std::string &imageFileName) {
  ...
  using CasterType =
      itk::CastImageFilter<kvl::CroppedImageReader::ImageType, ImageType>;
...
KvlImage::KvlImage(const std::string &imageFileName,
  ...
  for (int index = 0; index < 3; index++) {
    int start = reader->GetOriginalImageRegion().GetIndex(index) -
    ...
    m_non_cropped_image_size[index] =
        reader->GetOriginalImageOriginalRegion().GetSize(index);
  }
  std::cout << "Read image: " << imageFileName << " cropped by "
            << boundingFileName << std::endl;
}
...
py::tuple KvlImage::GetCropSlices() { return m_crop_slices; }
...
void KvlImage::Write(std::string fileName, KvlTransform &transform) {
  ...
  if (true) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        newDirection[j][i] =
            transform.m_transform->GetMatrix()[j][i] / normOfColumn;
      }
    }
