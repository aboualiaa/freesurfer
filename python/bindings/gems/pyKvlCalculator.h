...
public:
  ...
  KvlCostAndGradientCalculator(
    ...
    if (typeName == "AtlasMeshToIntensityImage") {
      ...
      for (int gaussianNumber = 0; gaussianNumber < numberOfGaussians;
        ...
        for (int contrastNumber = 0; contrastNumber < numberOfContrasts;
             contrastNumber++) {
          mean_converted[contrastNumber] =
              means.at(gaussianNumber, contrastNumber);
        }
      ...
      std::vector<double> mixtureWeights_converted =
          std::vector<double>(numberOfGaussians, 0.0f);
      for (int gaussianNumber = 0; gaussianNumber < numberOfGaussians;
           gaussianNumber++) {
        mixtureWeights_converted[gaussianNumber] =
            (mixtureWeights.at(gaussianNumber));
      }
      ...
      std::vector<int> numberOfGaussiansPerClass_converted =
          std::vector<int>(numberOfClasses, 0);
      for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
        numberOfGaussiansPerClass_converted[classNumber] =
            numberOfGaussiansPerClass.at(classNumber);
      }
      ...
      for (auto image : images) {
        ImageType::ConstPointer constImage =
            static_cast<const ImageType *>(image.m_image.GetPointer());
        images_converted.push_back(constImage);
      }
      myCalculator->SetImages(images_converted);
      myCalculator->SetParameters(means_converted, variances_converted,
                                  mixtureWeights_converted,
                                  numberOfGaussiansPerClass_converted);
      ...
      kvl::AtlasMesh::PointsContainer::Pointer alphaTargetPoints =
          kvl::AtlasMesh::PointsContainer::New();
    ...
    case 'S': {
      ...
      calculator->SetBoundaryCondition(
          kvl::AtlasMeshPositionCostAndGradientCalculator::SLIDING);

      // Retrieve transform if one is provided
      TransformType::ConstPointer constTransform = nullptr;
      constTransform = static_cast<const TransformType *>(
          transform.m_transform.GetPointer());
    ...
    case 'A': {
      ...
      calculator->SetBoundaryCondition(
          kvl::AtlasMeshPositionCostAndGradientCalculator::AFFINE);
      break;
    }
    case 'T': {
      std::cout << "TRANSLATION" << std::endl;
      calculator->SetBoundaryCondition(
          kvl::AtlasMeshPositionCostAndGradientCalculator::TRANSLATION);
      break;
    }
    case 'N': {
      std::cout << "NONE" << std::endl;
      calculator->SetBoundaryCondition(
          kvl::AtlasMeshPositionCostAndGradientCalculator::NONE);
      break;
    }
    default: {
  ...
  KvlCostAndGradientCalculator(KvlMeshCollection meshCollection, double K0,
                               double K1, KvlTransform transform) {
    kvl::AverageAtlasMeshPositionCostAndGradientCalculator::Pointer
        myCalculator =
            kvl::AverageAtlasMeshPositionCostAndGradientCalculator::New();
    myCalculator->SetBoundaryCondition(
        kvl::AtlasMeshPositionCostAndGradientCalculator::SLIDING);
    // Retrieve transform if provided
    TransformType::ConstPointer constTransform =
        static_cast<const TransformType *>(transform.m_transform.GetPointer());
    if (constTransform.GetPointer())
      myCalculator->SetMeshToImageTransform(constTransform);
    ...
    for (int meshNumber = 0;
         meshNumber < meshCollectionPtr->GetPositions().size(); meshNumber++) {
      positions.push_back(
          meshCollectionPtr->GetPositions()[meshNumber].GetPointer());
      Ks.push_back(K1);
    }
  ...
  EvaluateMeshPosition(const KvlMesh &mesh) {
    calculator->Rasterize(mesh.mesh);
    const double cost = calculator->GetMinLogLikelihoodTimesPrior();
    kvl::AtlasPositionGradientContainerType::ConstPointer gradient =
    ...
    py::array_t<double> gradient_np =
        createNumpyArrayCStyle({numberOfNodes, 3}, data);
