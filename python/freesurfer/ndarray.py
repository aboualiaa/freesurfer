...
class ArrayContainerTemplate:
    ...
    def __init__(self, data, lut=None, swap_batch_dim=False):
        ...
        if self.basedims is None:
            raise TypeError(
                "%s should never be initialized directly" % classname
            )
        ...
        if self.data.ndim < self.basedims or self.data.ndim > self.basedims + 1:
            raise ValueError(
                "%s (%dD) cannot be initialized by an array with %d dims"
                % (classname, self.basedims, self.data.ndim)
            )
    ...
    def ensure(cls, unknown):
        ...
        if isinstance(unknown, np.ndarray):
            return cls(unknown)
        raise ValueError(
            "Cannot convert object of type %s to %s"
            % (type(unknown).__name__, cls.__name__)
        )
    ...
    def read(cls, filename):
        ...
        if not isinstance(result, cls):
            warning(
                'reading file "%s" as a %s - not a %s'
                % (filename, result.__class__.__name__, cls.__name__)
            )
        return result
...
class Image(ArrayContainerTemplate, Transformable):
    ...
    def reslice(self, pixsize, interp_method="linear", smooth_sigma=0):
        ...
        src_vol = Volume(
            np.reshape(self.data, src_shape_3d),
            affine=self.affine,
            voxsize=(*self.pixsize[:2], 1.0),
        )
        trg_vol = src_vol.reslice(
            (*pixsize[:2], 1.0),
        ...
        resliced_image = Image(
            np.reshape(trg_vol.data, trg_shape),
            affine=trg_vol.affine,
            pixsize=trg_vol.voxsize[:2],
        )
...
class Volume(ArrayContainerTemplate, Transformable):
    ...
    def reslice(self, voxsize, interp_method="linear", smooth_sigma=0):
        ...
        target_shape = tuple(
            np.ceil(
                np.array(self.voxsize).astype(float) * src_shape / voxsize
            ).astype(int)
        )
    ...
    def crop(self, cropping):
        ...
        idxs = (
            [cropping] if not isinstance(cropping, (tuple, list)) else cropping
        )
        ...
        if Ellipsis in idxs:
            raise NotImplementedError(
                "Ellipsis is not yet supported in cropping indices"
            )
        ...
        for i in idxs:
            if isinstance(i, slice):
                ...
                raise ValueError(
                    "volume cropping does not support dimension removal - crop the data array directly"
                )
            else:
                raise ValueError(
                    "volume cropping must be indexed by slices (:) only"
                )
    ...
    def crop_to_bbox(self, thresh=0, margin=0):
        ...
        if margin > 0:
            ...
            stop = [
                min(self.shape[i], c.stop + n) for i, c in enumerate(cropping)
            ]
    ...
    def fit_to_shape(self, shape, center="image"):
        ...
        # construct cropped volume
        conformed_vol = Volume(
            conformed_data, affine=matrix, voxsize=self.voxsize
        )
