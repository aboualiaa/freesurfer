...
class VAE:
    def __init__(self, atlasDir, transform, imageSize):
        ...
        self.lesionPlaceholder = tf.placeholder(
            tf.float32,
            [1, self.net_shape[0], self.net_shape[1], self.net_shape[2], 1],
        )
    ...
    def get_encoder(self, data):
        ...
        conv_1 = (
            tf.nn.conv3d(
                data, conv_1_kernel, strides=[1, 2, 2, 2, 1], padding="VALID"
            )
            + conv_1_bias
        )
        ...
        batch_n_e_1 = (
            gamma_1 * ((conv_1 - mu_1) / tf.sqrt(var_1 + epsilon)) + beta_1
        )
        ...
        batch_n_e_2 = (
            gamma_2 * ((conv_2 - mu_2) / tf.sqrt(var_2 + epsilon)) + beta_2
        )
        ...
        batch_n_e_3 = (
            gamma_3 * ((conv_3 - mu_3) / tf.sqrt(var_3 + epsilon)) + beta_3
        )
        ...
        batch_n_e_4 = (
            gamma_4 * ((conv_4 - mu_4) / tf.sqrt(var_4 + epsilon)) + beta_4
        )
        ...
        batch_n_e_5 = (
            gamma_5 * ((conv_5 - mu_5) / tf.sqrt(var_5 + epsilon)) + beta_5
        )
        ...
        mu = (
            tf.nn.conv3d(
                batch_n_e_5, mu_kernel, strides=[1, 1, 1, 1, 1], padding="VALID"
            )
            + mu_bias
        )
    ...
    def get_decoder(self, code, imageSize):
        ...
        batch_n_d_1 = (
            gamma_1 * ((hidden_0_dec - mu_1) / tf.sqrt(var_1 + epsilon))
            + beta_1
        )
        ...
        batch_n_d_2 = (
            gamma_2 * ((hidden_2_dec - mu_2) / tf.sqrt(var_2 + epsilon))
            + beta_2
        )
        ...
        batch_n_d_3 = (
            gamma_3 * ((hidden_3_dec - mu_3) / tf.sqrt(var_3 + epsilon))
            + beta_3
        )
        ...
        batch_n_d_4 = (
            gamma_4 * ((hidden_4_dec - mu_4) / tf.sqrt(var_4 + epsilon))
            + beta_4
        )
        ...
        batch_n_d_5 = (
            gamma_5 * ((hidden_5_dec - mu_5) / tf.sqrt(var_5 + epsilon))
            + beta_5
        )
        ...
        hidden_6_dec = self.pad_up_to(
            hidden_6_dec,
            [1, imageSize[0], imageSize[1], imageSize[2], 1],
            constant_values=-100,
        )
    ...
    def pad_up_to(self, t, max_in_dims, constant_values):
        ...
        paddings = [
            [
                tf.cast((m - s[i]) / 2, tf.int32),
                (m - s[i]) - tf.cast((m - s[i]) / 2, tf.int32),
            ]
            for (i, m) in enumerate(max_in_dims)
        ]
        return tf.pad(
            t,
            tf.cast(tf.stack(paddings), tf.int32),
            "CONSTANT",
    ...
    def sample(self, lesion):
        ...
        lesionTrainSpace = affine_transform(
            lesion, self.trainToSubjectMat, output_shape=self.net_shape, order=1
        )
        ...
        lesionVAETrainSpace = np.squeeze(
            np.squeeze(
                self.sess.run(
                    self.net, {self.lesionPlaceholder: expandedLesion}
                ),
                4,
            ),
            0,
        ...
        lesionPriorVAE = affine_transform(
            lesionVAETrainSpace,
