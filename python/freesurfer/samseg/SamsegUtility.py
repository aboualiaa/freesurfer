...
def maskOutBackground(
    ...
    if os.environ.get("SAMSEG_LEGACY_BACKGROUND_MASKING") is not None:
        ...
        backgroundMask = np.ma.greater(
            backgroundPrior, (2 ** 16 - 1) * maskingProbabilityThreshold
        )
        visualizer.show(
            images=backgroundMask.astype(float), title="thresholded"
        )

        # Extend by distance of maskingDistance (in mm)
        distance = ndimage.distance_transform_edt(
            backgroundMask, sampling=voxelSpacing
        )
        print(voxelSpacing)
