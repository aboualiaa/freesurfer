...
def kvlWarpMesh(
    ...
    if targetNumberOfNodes == sourceNumberOfNodes:
        if False:
            deltaReferencePosition = (
                sourceReferencePosition - targetReferencePosition
            )
    ...
    imageSize = [
        int(1 + dim) for dim in np.max(sourceReferencePosition, axis=0)
    ]
    ...
    if np.max(np.absolute(deformation)) > 0:
        ...
        sourceReferenceMesh.alphas = (
            deformation - minDeformation
        ) / deltaDeformation
    ...
    calculator = KvlCostAndGradientCalculator(
        "PointSet",
        [image],
        "Sliding",
        transform,
