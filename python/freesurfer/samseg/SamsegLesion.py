...
class SamsegLesion(Samseg):
    def __init__(
        ...
        self.rho = rho
        self.intensityMaskingClassNumber = self.getClassNumber(
            intensityMaskingSearchString
        )
        ...
        if len(intensityMaskingPattern) != len(imageFileNames):
            raise ValueError(
                "Number of lesion mask patterns does not match the number of input images."
            )
        if not (
            all(pattern in (0, 1, -1) for pattern in intensityMaskingPattern)
        ):
            raise ValueError(
                "Lesion mask pattern values can be only 0, 1 or -1"
            )
        ...
        (
            self.lesionStructureNumber,
            self.lesionClassNumber,
            self.lesionGaussianNumber,
        ) = self.checkConditions(lesionSearchString)
        _, _, self.wmGaussianNumber = self.checkConditions(
            wmSearchString, checkStructureOwnClass=False
        )
    ...
    def initializeGMM(self):
        ...
        self.gmm.tiedGaussiansInit(
            self.wmGaussianNumber, self.lesionGaussianNumber, self.rho
        )
    ...
    def computeFinalSegmentation(self):
        ...
        (
            ...
            data,
            priors,
        ) = Samseg.computeFinalSegmentation(self)
        ...
        if self.intensityMaskingClassNumber is not None:
            ...
            intensityMaskingGaussianNumbers = [
                sum(
                    self.gmm.numberOfGaussiansPerClass[
                        0 : self.intensityMaskingClassNumber
                    ]
                )
            ]
            intensityMaskingMean = np.sum(
                self.gmm.means[intensityMaskingGaussianNumbers]
            ...
            for contrastNumber in range(self.gmm.numberOfContrasts):
                ...
                if direction == -1:
                    tmp = np.logical_and(
                        tmp,
                        data[:, contrastNumber]
                        < intensityMaskingMean[contrastNumber],
                    )
                elif direction == 1:
                    tmp = np.logical_and(
                        tmp,
        ...
        self.visualizer.show(
            image_list=[intensityMask.astype(float)], title="Intensity mask"
        )
        ...
        self.visualizer.show(
            image_list=[lesion], title="Initial lesion segmentation"
        )
        ...
        self.visualizer.start_movie(
            ...
            title="Lesion prior using VAE only -- the movie",
        )
        self.visualizer.start_movie(
            window_id="Lesion sample", title="Lesion sample -- the movie"
        )
        for sweepNumber in range(
            ...
            if hasattr(self.visualizer, "show_flag"):
                ...
                self.visualizer.show(
                    probabilities=tmp,
                    title="Lesion prior using VAE only",
                    window_id="Lesion prior using VAE only",
                )

            # Sample from the mean and variance, conditioned on the data and the lesion segmentation
            mean, variance = self.gmm.sampleMeansAndVariancesConditioned(
                data,
                lesion[self.mask].reshape(-1, 1),
                self.lesionGaussianNumber,
            )
            ...
            if hasattr(self.visualizer, "show_flag"):
                ...
                self.visualizer.show(
                    probabilities=tmp,
                    title="Lesion prior using VAE and atlas together",
            ...
            otherStructureNumbers = [
                i
                for i in range(numberOfStructures)
                if i != self.lesionStructureNumber
            ]
            otherStructurePriors = effectivePriors[:, otherStructureNumbers]
            otherStructurePriors /= (
                np.sum(otherStructurePriors, axis=1).reshape(-1, 1) + eps
            )
            effectivePriors[:, otherStructureNumbers] = otherStructurePriors * (
                1
                - effectivePriors[:, self.lesionStructureNumber].reshape(-1, 1)
            )
            likelihoods[
                :, self.lesionStructureNumber
            ] = self.gmm.getGaussianLikelihoods(data, mean, variance)
            posteriors = effectivePriors * likelihoods
            posteriors /= np.expand_dims(np.sum(posteriors, axis=1) + eps, 1)
            sample = (
                np.random.rand(numberOfVoxels)
                <= posteriors[:, self.lesionStructureNumber]
            )
            lesion = np.zeros(imageSize)
