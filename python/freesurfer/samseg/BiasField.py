...
class BiasField:
    def __init__(self, imageSize, smoothingKernelSize):
        self.fullBasisFunctions = self.getBiasFieldBasisFunctions(
            imageSize, smoothingKernelSize
        )
        self.basisFunctions = self.fullBasisFunctions.copy()
        self.coefficients = None

    def backprojectKroneckerProductBasisFunctions(
        self, kroneckerProductBasisFunctions, coefficients
    ):
        numberOfDimensions = len(kroneckerProductBasisFunctions)
        Ms = np.zeros(
            numberOfDimensions, dtype=np.uint32
        ...
        for dimensionNumber in range(numberOfDimensions):
            Ms[dimensionNumber] = kroneckerProductBasisFunctions[
                dimensionNumber
            ].shape[1]
            Ns[dimensionNumber] = kroneckerProductBasisFunctions[
                dimensionNumber
            ].shape[0]
            transposedKroneckerProductBasisFunctions.append(
                kroneckerProductBasisFunctions[dimensionNumber].T
            )
        y = self.projectKroneckerProductBasisFunctions(
            transposedKroneckerProductBasisFunctions,
    ...
    def projectKroneckerProductBasisFunctions(
        self, kroneckerProductBasisFunctions, T
    ):
        #
        # Compute
        #   c = W' * t
        ...
        for dimensionNumber in range(numberOfDimensions):
            ...
            currentSizeOfT[0] = kroneckerProductBasisFunctions[
                dimensionNumber
            ].shape[1]
    ...
    def computePrecisionOfKroneckerProductBasisFunctions(
        self, kroneckerProductBasisFunctions, B
    ):
        #
        # Compute
        #   H = W' * diag( B ) * W
        ...
        for dimensionNumber in range(numberOfDimensions):
            ...
            hessianKroneckerProductBasisFunctions[dimensionNumber] = np.kron(
                np.ones((1, M)), A
            ) * np.kron(A, np.ones((1, M)))
            Ms[dimensionNumber] = M
        result = self.projectKroneckerProductBasisFunctions(
            hessianKroneckerProductBasisFunctions, B
        )
        new_shape = list(np.kron(Ms, [1, 1]))
        new_shape.reverse()
        result = result.reshape(new_shape)
        permutationIndices = np.hstack(
            (2 * np.r_[:numberOfDimensions], 2 * np.r_[:numberOfDimensions] + 1)
        )
    ...
    def getBiasFieldBasisFunctions(self, imageSize, smoothingKernelSize):
        ...
        for dimensionNumber in range(3):
            ...
            A = np.array(
                [
                    [math.cos(freq * m) * scaling[m] for m in range(M)]
                    for freq in js
                ]
            )
    ...
    def getBiasFields(self, mask=None):
        ...
        imageSize = tuple(
            [functions.shape[0] for functions in self.basisFunctions]
        )
    ...
    def fitBiasFieldParameters(
        self, imageBuffers, gaussianPosteriors, means, variances, mask
    ):
        ...
        numberOfBasisFunctions = [
            functions.shape[1] for functions in self.basisFunctions
        ]
        ...
        for gaussianNumber in range(numberOfGaussians):
            precisions[gaussianNumber, :, :] = np.linalg.inv(
                variances[gaussianNumber, :, :]
            ).reshape((1, numberOfContrasts, numberOfContrasts))

        lhs = np.zeros(
            (
                numberOf3DBasisFunctions * numberOfContrasts,
                numberOf3DBasisFunctions * numberOfContrasts,
            )
        )  # left-hand side of linear system
        rhs = np.zeros(
            (numberOf3DBasisFunctions * numberOfContrasts, 1)
        ...
        for contrastNumber1 in range(numberOfContrasts):
            ...
            contrast1Indices = (
                np.arange(0, numberOf3DBasisFunctions)
            ...
            for contrastNumber2 in range(numberOfContrasts):
                ...
                classSpecificWeights = (
                    gaussianPosteriors
                    * precisions[:, contrastNumber1, contrastNumber2]
                )
                weights = np.sum(classSpecificWeights, 1)

                # Build up stuff needed for rhs
                predicted = np.sum(
                    classSpecificWeights * means[:, contrastNumber2], 1
                ) / (weights + eps)
                ...
                lhs[
                    np.ix_(contrast1Indices, contrast2Indices)
                ] = self.computePrecisionOfKroneckerProductBasisFunctions(
                    self.basisFunctions, weightsImageBuffer
                )

            tmpImageBuffer[mask] = tmp
            rhs[contrast1Indices] = self.projectKroneckerProductBasisFunctions(
                self.basisFunctions, tmpImageBuffer
            ).reshape(-1, 1)
        ...
        #
        self.coefficients = solution.reshape(
            (numberOfContrasts, numberOf3DBasisFunctions)
        ).transpose()
    ...
    def downSampleBasisFunctions(self, downSamplingFactors):
        self.basisFunctions = [
            np.array(biasFieldBasisFunction[::downSamplingFactor])
            for biasFieldBasisFunction, downSamplingFactor in zip(
                self.fullBasisFunctions, downSamplingFactors
            )
        ]
