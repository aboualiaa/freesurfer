...
class Affine:
    def __init__(self, imageFileName, meshCollectionFileName, templateFileName):
    ...
    def computeTalairach(self, imageToImageTransformMatrix):
        ...
        fnamedst = os.path.join(
            fs.fshome(), "subjects", "fsaverage", "mri", "orig.mgz"
        )
        ...
        M = (
            np.linalg.inv(RAS2LPS @ fsaorig.affine)
    ...
    def getTransformMatrix(
        ...
        pitchAngle=0.0,
        scale=1.0,
        horizontalTableShift=0.0,
        verticalTableShift=0.0,
        ...
        scalingCenter = np.array(
            [
                self.scalingCenter[0],
                self.scalingCenter[1],
                self.scalingCenter[2],
                1,
            ]
        ).reshape(-1, 1)
        ...
        scalingMatrix = np.linalg.solve(
            centeringMatrix, scalingMatrix @ centeringMatrix
        )
        ...
        rotationCenter = np.array(
            [self.pitchCenter[0], self.pitchCenter[1], self.pitchCenter[2], 1]
        ).reshape(-1, 1)
        rotationCenter = scalingMatrix @ translationMatrix @ rotationCenter
        centeringMatrix = np.eye(4)
        centeringMatrix[0:3, 3] = -rotationCenter[0:3, 0]
        rotationMatrix = np.linalg.solve(
            centeringMatrix, rotationMatrix @ centeringMatrix
        )
    ...
    def gridSearch(
                ...
                for horizontalTableShift in horizontalTableShifts:
                    for verticalTableShift in verticalTableShifts:
                        for pitchAngle in pitchAngles:
                            ...
                            worldToWorldTransformMatrix = self.getTransformMatrix(
                            ...
                            imageToImageTransformMatrix = np.linalg.solve(
                                self.imageToWorldTransformMatrix,
                                worldToWorldTransformMatrix
                                @ self.templateImageToWorldTransformMatrix,
                            )
                            ...
                            if cost < bestCost:
                                ...
                                (
                                    bestPitchAngle,
                                    bestScale,
                                    bestHorizontalTableShift,
                                    bestVerticalTableShift,
                                    bestInitialTableShift,
                                ) = (
                                    pitchAngle,
                                    scale,
                                    horizontalTableShift,
                                    verticalTableShift,
                                    initialTableShift,
                                )
                                bestImageToImageTransformMatrix = (
                                    imageToImageTransformMatrix
                                )
        ...
        mesh.points = ProbabilisticAtlas().mapPositionsFromTemplateToSubjectSpace(
            positionsInTemplateSpace,
            gems.KvlTransform(
                requireNumpyArray(bestImageToImageTransformMatrix)
            ),
        )
        self.visualizer.show(
            images=self.image.getImageBuffer(),
    ...
    def getInitialization(self, initializationOptions):
        ...
        positionsInTemplateSpace = ProbabilisticAtlas().mapPositionsFromSubjectToTemplateSpace(
        ...
        if initializationOptions.tryCenterOfGravity:
            ...
            centerOfGravityImage = (
                self.imageToWorldTransformMatrix[0:3, 0:3]
                @ centerOfGravityImage
    ...
    def setUp(self, initTransform, targetDownsampledVoxelSpacing, visualizer):
        ...
        templateImageToWorldTransformMatrix = (
            template.transform_matrix.as_numpy_array
        )
        ...
        imageToWorldTransformMatrix = np.linalg.solve(
            initialWorldToWorldTransformMatrix, imageToWorldTransformMatrix
        )
        ...
        downSamplingFactors = np.round(
            targetDownsampledVoxelSpacing / voxelSpacing
        )
        downSamplingFactors[downSamplingFactors < 1] = 1
        upSamplingTranformMatrix = np.diag(
            np.pad(
                downSamplingFactors, (0, 1), "constant", constant_values=(0, 1)
            )
        )
        ...
        imageToWorldTransformMatrix = (
            imageToWorldTransformMatrix @ upSamplingTranformMatrix
        )
    ...
    def registerMeshToImage(
        ...
        if worldToWorldTransformMatrix is not None:
            ...
            imageToImageTransformMatrix = self.getInitialization(
                initializationOptions
            )
    ...
    def saveResults(
        self, savePath, worldToWorldTransformMatrix, imageToImageTransformMatrix
    ):
