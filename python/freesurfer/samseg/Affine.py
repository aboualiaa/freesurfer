    ...
    def registerAtlas(
        ...
        if worldToWorldTransformMatrix is not None:
            ...
            imageToImageTransformMatrix = (
                np.linalg.inv(imageToWorldTransformMatrix)
            ...
            lineSearchMaximalDeformationIntervalStopCriterion = (
                self.maximalDeformationStopCriterion
            ...
            (
                rotationMatrix,
                scalingMatrix,
            ) = self.computeRotationAndScalingMatrixGuessEstimates()

            initialWorldToWorldTransformMatrix = (
                rotationMatrix @ initialWorldToWorldTransformMatrix
            )
            initialWorldToWorldTransformMatrix = (
                scalingMatrix @ initialWorldToWorldTransformMatrix
            )
            ...
            initialImageToImageTransformMatrix = np.linalg.solve(
                imageToWorldTransformMatrix, multiplied
            )
            ...
            downSamplingFactors = np.round(
                self.targetDownsampledVoxelSpacing / voxelSpacing
            )
            ...
            visualizer.show(
                images=imageBuffer,
                window_id="atlas initial",
                title="Initial Atlas Registration",
            )
            ...
            visualizer.show(
                mesh=mesh,
                shape=imageBuffer.shape,
                window_id="atlas mesh",
                title="Atlas Mesh",
            )
            ...
            centerOfGravityImage = np.array(
                scipy.ndimage.measurements.center_of_mass(imageBuffer)
            )
            ...
            for verticalDisplacement in (0, -10, 10):
                translation = baseTranslation + np.array(
                    (0, verticalDisplacement, 0)
                )
                mesh.points = nodePositions + translation
                trialCost, trialGradient = calculator.evaluate_mesh_position_b(
                    mesh
                )
                if trialCost >= cost:
                    ...
                    initialImageToImageTransformMatrix[
                        0:3, 3
                    ] = initialImageToImageTransformMatrix[0:3, 3] + (
                        np.diag(downSamplingFactors) @ translation
                    )
                    break
            ...
            visualizer.start_movie(
                window_id="atlas iteration",
                title="Atlas Registration - the movie",
            )
            while True:
                (
            ...
            originalY = np.vstack(
                (
                    np.diag(downSamplingFactors)
                    @ originalNodePositions[pointNumbers].T,
                    [1, 1, 1, 1],
                )
            )
            ...
            worldToWorldTransformMatrix = (
                ...
                @ imageToImageTransformMatrix
                @ np.linalg.inv(templateImageToWorldTransformMatrix)
            )
        ...
        if os.environ.get("SAMSEG_LEGACY_REGISTRATION") is not None:
            ...
            transformedTemplateFileName = os.path.join(
                savePath, "template_coregistered_legacy.nii"
            )
            template.write(
                transformedTemplateFileName,
                gems.KvlTransform(desiredTemplateImageToWorldTransformMatrix),
            )
