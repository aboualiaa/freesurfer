...
def initVisualizer(showfigs, movie):
    if showfigs or movie:
        if not import_graphical_libraries():
            raise RuntimeError(
                "the samseg visualization tool requires pyqtgraph and pyqt5 - "
                "please install these python packages to show figures"
            )
        else:
            return ShowFigures(
                show_flag=showfigs, movie_flag=movie, interactive=True
            )
    else:
...
class ShowFigures:
    ...
    def create_palette(self, layer_count):
        ...
        return (
            self.palette * (layer_count // self_size)
            + self.palette[: layer_count % self_size]
        )
    ...
    def show(
        ...
        if len(probability_layers) > probability_max:
            tail = np.sum(
                layer["data"]
                for layer in probability_layers[probability_max - 1 :]
            )
    ...
    def probability_layers(
        self, probabilities, names, prefix=None, visibility=True
    ):
        if prefix is None:
            prefix = "Label"
    ...
    def image_layers(
        self, image_list, prefix=None, visibility=True, alpha=None
    ):
        if len(image_list) == 0:
            return []
        if prefix is None:
    ...
    def create_image_list(self, source):
        ...
        if len(shape) > 3:
            layer_count = shape[3]
            image_list = [
                source[:, :, :, index] for index in range(layer_count)
            ]
        else:
            image_list = [source]
        return image_list
    ...
    def hdav_view(
        self, layers, window_id, title, handle_events=True, legend_width=None
    ):
        if layers:
            view(
                layers,
    ...
    def start_movie(self, window_id, title=None):
        if self.movie_flag:
            ...
            def update_movie(layers):
                ...
                self.hdav_view(
                    layers, window_id, layer_sequence.title, handle_events=False
                )
    ...
    def plot(self, data, title=None, window_id=None):
        if self.show_flag:
            ...
            self.plotWindows[window_id].plot(
                y=data, pen=pg.mkPen(color="l", width=3)
            )
...
class LayerSequence:
    ...
    def title(self):
        return "{0} {1}/{2}".format(
            self._title, self.time_index + 1, self.frame_count
        )
...
def transparency_color_map(
    top_rgb_color,
    bottom_rgb_color=None,
    start=0,
    stop=65535,
    bottom_alpha=0.0,
    top_alpha=1.0,
    ...
    def show_palette(palette=None):
        ...
        for color in range(color_count):
            ...
            for square_color in range(color_count):
                ...
                image[
                    square_base_column:square_end_column,
                    square_base_row:square_end_row,
                    :,
                    color,
                ] = OFF
                image[
