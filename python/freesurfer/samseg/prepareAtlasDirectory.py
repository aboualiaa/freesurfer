...
from freesurfer.samseg.io import (
    ...
    kvlWriteCompressionLookupTable,
...
def readAndSimplifyCompressionLookupTable(
    compressionLookupTableFileName, uninterestingStructureSearchStrings=None
):
    ...
    if uninterestingStructureSearchStrings:
        ...
        for structureNumber, uninteresting in enumerate(mask):
            if not uninteresting:
                survivingFreeSurferLabels.append(
                    FreeSurferLabels[structureNumber]
                )
...
def readAndSimplifyMeshCollection(
    ...
    # These will be merged into the very first structure in the list (which is assumed to be
    # background)
    _, names, _, mask = readAndSimplifyCompressionLookupTable(
        compressionLookupTableFileName, uninterestingStructureSearchStrings
    )
    alphas = meshCollection.reference_mesh.alphas
    alphas = np.concatenate(
        (
            alphas[:, mask].sum(axis=1, keepdims=True),
            alphas[:, np.logical_not(mask)],
        ),
        axis=1,
    )
    ...
    if sharedGMMParameters:
        ...
        if affineClassDefinitions:
            ...
            tmp = [
                GMMparameter(
                    mergedName="", numberOfComponents=-1, searchStrings=z
                )
...
def smoothMeshCollection(
    meshCollection, sigma, returnPriors=False, showFigures=False
):
    #
    if showFigures:
    ...
    #
    size = np.array(
        meshCollection.reference_position.max(axis=0) + 1.5, dtype=int
    )
    ...
    priors = ndimage.gaussian_filter(
        priors.astype(float), sigma=(sigma, sigma, sigma, 0)
    ).astype(priors.dtype)
    visualizer.show(probabilities=priors)
    alphas = meshCollection.reference_mesh.fit_alphas(priors)
    meshCollection.reference_mesh.alphas = alphas
    visualizer.show(
        probabilities=meshCollection.reference_mesh.rasterize(size, -1)
    )
...
def prepareAtlasDirectory(
    ...
    templateFileName,
    uninterestingStructureSearchStrings=None,
    smoothingSigmaForFirstLevel=2,
    meshCollectionFileNameForFirstLevel=None,
    smoothingSigmaForAffine=2,
    meshCollectionFileNameForAffine=None,
    affineClassDefinitions=None,
    ...
    meshCollection = readAndSimplifyMeshCollection(
        meshCollectionFileName,
        compressionLookupTableFileName,
    ...
    priors = (
        meshCollection.reference_mesh.rasterize(
            template.getImageBuffer().shape, -1
        )
        / 65535
    )
    size = priors.shape[0:3]
    numberOfClasses = priors.shape[-1]
    templateImageBuffer = priors.reshape((np.prod(size), -1)) @ np.arange(
        0, numberOfClasses
    )
    ...
    visualizer.show(
        mesh=meshCollection.reference_mesh, shape=size, title="Final atlas"
    )
    ...
    if meshCollectionFileNameForFirstLevel:
        ...
        meshCollection = readAndSimplifyMeshCollection(
            meshCollectionFileNameForFirstLevel,
            compressionLookupTableFileName,
            uninterestingStructureSearchStrings,
        )
    ...
    FreeSurferLabels, names, colors, _ = readAndSimplifyCompressionLookupTable(
        compressionLookupTableFileName, uninterestingStructureSearchStrings
    )
    kvlWriteCompressionLookupTable(
        os.path.join(directoryName, "compressionLookupTable.txt"),
        FreeSurferLabels,
        names,
        colors,
    )
    ...
    if meshCollectionFileNameForAffine:
        ...
        priors = smoothMeshCollection(
            meshCollection, smoothingSigmaForAffine, returnPriors=True
        )
        # visualizer.show( probabilities=priors )
    ...
    visualizer.show(
        mesh=meshCollection.reference_mesh, shape=size, title="Atlas for affine"
    )
    meshCollection.write(
        os.path.join(directoryName, "atlasForAffineRegistration.txt")
    )
