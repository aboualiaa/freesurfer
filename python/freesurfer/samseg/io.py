...
GMMparameter = namedtuple(
    "GMMparameter", "mergedName numberOfComponents searchStrings"
)
...
def kvlWriteSharedGMMParameters(sharedGMMParameters, fileName):
    ...
    with open(fileName, "w") as fid:
        print(
            "# The format is: mergedName numberOfComponents searchStrings\n\n",
            file=fid,
        )
        ...
        for GMMparameter in sharedGMMParameters:
            print(
                GMMparameter.mergedName,
                GMMparameter.numberOfComponents,
                *GMMparameter.searchStrings,
                file=fid
            )
...
def kvlReadCompressionLookupTable(fileName):
    ...
    with open(fileName) as fid:
        for line in fid.readlines():
            FreeSurferLabel, compressedLabel, name, R, G, B, A = [
                data_type(value)
                for data_type, value in zip(
                    (int, int, str, int, int, int, int), line.split()
                )
            ]
    ...
    FreeSurferLabels, names, colors = [
        [entry[key] for entry in table]
        for key in ["FreeSurferLabel", "name", "color"]
    ]
