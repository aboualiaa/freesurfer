...
def meshValidityTest(alphas, name):
    ...
    if probability_discrepancy > 1e-5:
        message = (
            "%s invalid: class probabilities in the mesh should sum to one in all nodes"
            % name
        )
...
def kvlGetMergingFractionsTable(names, mergeOptions):
    ...
    if not fractionsTable.any(axis=0).all():
        raise ValueError(
            "some structures are not associated with any super-structures"
        )
        ...
        for structureNumber in range(len(names)):
            ...
            if percentage > 0:
                print(
                    "    %s (%d%%)"
                    % (
                        names[structureNumber].ljust(len(max(names, key=len))),
                        percentage,
                    )
                )
