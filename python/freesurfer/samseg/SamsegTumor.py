...
import logging
from freesurfer.samseg.merge_alphas import (
    kvlMergeAlphas,
...
import pandas as pd
from freesurfer.samseg.io import (
...
class SamsegTumor(Samseg):
    def __init__(
        ...
        self.modelSpecifications = setTumorComp(
            self.nTumorComp, self.atlasDir, self.modelSpecifications
        )
    ...
    def checkConditions(self, searchString, checkStructureOwnClass=True):
        ...
        classFractions, _ = kvlGetMergingFractionsTable(
        ...
        if checkStructureOwnClass and structureNumbers.size is not 1:
            raise Exception(
                "Structure of interest should correspond to exactly one class (mixture model) and vice versa"
            )
        if len(gaussianNumbers) is not 1:
            raise Exception(
                "Structure of interest should have a mixture model with only a single component"
            )
    ...
    def initializeGMM(self):
        ...
        mesh = self.probabilisticAtlas.getMesh(
            join(self.atlasDir, "atlas_level2.txt.gz")
        )
    ...
    def initializeMeanConstraints(self):
        ...
        if "t1ce" in modeNames or "t1gd" in modeNames or "t1c" in modeNames:
            self.t1ce_idx = modeNames.index(
                [i for i in modeNames if i in ["t1ce", "t1gd", "t1c"]][0]
            )
            ...
            if not "flair" in modeNames:
                raise ValueError(
                    "Cannot initialize constraints on means. Neither flair or t1ce modes found."
                )
            A = A[:4]
            b = b[:4]
        b = -np.log(b)
    ...
    def estimateModelParameters(
        ...
        for multiResolutionLevel in range(numberOfMultiResolutionLevels):
            ...
            mask = self.mask
            orig_shape = np.array(
                nib.load(self.imageFileNames[0]).shape
            ).astype(np.int)
            uncropped_mask = np.zeros(orig_shape)
            uncropped_mask[
                cropping[0].start : cropping[0].stop,
                cropping[1].start : cropping[1].stop,
                cropping[2].start : cropping[2].stop,
            ] = mask
            uncropped_mask = uncropped_mask.astype(np.bool)
            uncropped_mask_ds = uncropped_mask[
                cropping[0].start
                % downSamplingFactors[0] :: downSamplingFactors[0],
                cropping[1].start
                % downSamplingFactors[1] :: downSamplingFactors[1],
                cropping[2].start
                % downSamplingFactors[2] :: downSamplingFactors[2],
            ]
            ...
            for iterationNumber in range(maximumNumberOfIterations):
                ...
                while True:
                    ...
                    if self.useMRF and (
                        EMIterationNumber > 0 or iterationNumber > 0
                    ):
                        likelihoods = self.gmm.getLikelihoods(
                            downSampledData, self.classFractions
                        )
                        likelihoods = np.dot(likelihoods, self.classFractions.T)
                        mergetable = downSampledClassPriors[
                            :, :-1
                        ] / np.expand_dims(
                            downSampledClassPriors[:, :-1].sum(axis=-1), 1
                        )
                        likelihood_0 = np.sum(
                            likelihoods[:, :-1] * mergetable, axis=-1
                        )
                        likelihood_1 = likelihoods[:, -1]
                        likelihood_ = np.zeros(
                            [2] + list(uncropped_mask_ds.shape)
                        )
                        ...
                        q = np.zeros_like(uncropped_mask_ds).astype(
                            np.float
                        )  # q is probability of tumor class
                        q1 = q.copy()
                        q0 = q.copy()
                        q[uncropped_mask_ds] = downSampledGaussianPosteriors[
                            :, -self.nTumorComp :
                        ].sum(axis=1)
                        ...
                        if (
                            ...
                            and numberOfMultiResolutionLevels > 1
                        ):
                            beta /= 8
                        ...
                        for iteration in range(n_iter):
                            for j in range(8):
                                ...
                                gamma_1 = (
                                    np.exp(
                                        -beta * np.sum(1 - neighbors_q, axis=-1)
                                    )
                                    * prior_[1]
                                )
                                gamma_0 = (
                                    np.exp(-beta * np.sum(neighbors_q, axis=-1))
                                    * prior_[0]
                                )
                        ...
                        llh_mrf = np.log(
                            q1[uncropped_mask_ds] + q0[uncropped_mask_ds]
                        ).sum()  # TODO: Make sure this is right

                        mrf_class_priors = np.concatenate(
                            [
                                (1 - q[uncropped_mask_ds][:, np.newaxis])
                                * downSampledClassPriors[:, :-1]
                                + 1e-16,
                                np.expand_dims(
                                    q[uncropped_mask_ds]
                                    * downSampledClassPriors[:, -1]
                        ...
                        mrf_class_priors /= np.expand_dims(
                            mrf_class_priors.sum(axis=1), 1
                        )
                        (
                            downSampledGaussianPosteriors,
                            minLogLikelihood,
                        ) = self.gmm.getGaussianPosteriors(
                            downSampledData, mrf_class_priors
                        )
                    ...
                    tumor_seg[tumor_mask] = (
                        np.argmax(
                            downSampledGaussianPosteriors[is_tumor][
                                :, -self.nTumorComp :
                            ],
                            axis=1,
                        )
                    ...
                    if not (
                        ...
                        if self.useMeanConstr:
                            self.gmm.fitGMMParametersWithConstraints(
                                downSampledData,
                                downSampledGaussianPosteriors,
                                self.meanConstraints[0],
                                self.meanConstraints[1],
                            )
                        else:
    ...
    def postProcess(self):
        ...
        if self.useShapeModel:
            (
                ...
                priors,
                posteriorsTumor,
            ) = self.computeFinalSegmentationShapeModel()
        else:
            (
                posteriors,
                biasFields,
        ...
        volumesInCubicMm = self.writeResults(
            biasFields, posteriors, posteriorsTumor
        )
    ...
    def writeResults(self, biasFields, posteriors, posteriorsTumor):
        ...
        if self.saveDeformed:
            ...
            posteriorsTmp = np.zeros(
                list(self.imageBuffers.shape[:3]) + [posteriors.shape[-1]],
                dtype=np.float32,
            )
            posteriorsTmp[self.mask] = posteriors
            posteriorsTemplateSpace = imagesInAtlasSpace(
                posteriorsTmp, self.deformation, self.transform, referenceMesh
            )
            posteriorsTemplateSpaceHard = fslabels[
                np.argmax(posteriorsTemplateSpace, axis=-1)
                .ravel()
                .astype(np.int32)
            ].astype(np.int32)
            posteriorsTemplateSpaceHard = posteriorsTemplateSpaceHard.reshape(
                posteriorsTemplateSpace.shape[:3]
            )
            self.writeDeformedImage(
                posteriorsTemplateSpaceHard,
                os.path.join(deformedPath, "seg.mgz"),
                saveLabels=True,
            )
            posteriorsTumorTmp = np.zeros(
                list(self.imageBuffers.shape[:3]) + [posteriorsTumor.shape[-1]],
                dtype=np.float32,
            )
            posteriorsTumorTmp[self.mask] = posteriorsTumor
            posteriorsTumorTemplateSpace = imagesInAtlasSpace(
                posteriorsTumorTmp,
                self.deformation,
                self.transform,
                referenceMesh,
            ...
            for structureNumber, name in enumerate(names):
                ...
                for searchString in self.savePosteriors:
                    if searchString in name:
                        ...
                        if self.saveDeformed:
                            self.writeDeformedImage(
                                posteriorsTemplateSpace[
                                    :, :, :, structureNumber
                                ],
                                os.path.join(
                                    join(deformedPath, "posteriors"),
                                    name + ".mgz",
                                ),
                            )
        ...
        for tumorComp in range(posteriorsTumor.shape[-1]):
            ...
            posteriorVol[self.mask] = posteriorsTumor[:, tumorComp].astype(
                np.float32
            )
            self.writeImage(
                posteriorVol, os.path.join(posteriorPath, name + ".mgz")
            )
            if self.saveDeformed:
                self.writeDeformedImage(
                    posteriorsTumorTemplateSpace[:, :, :, tumorComp],
                    os.path.join(
                        join(deformedPath, "posteriors"), name + ".mgz"
                    ),
                )
    ...
    def computeFinalSegmentation(self):
        ...
        if self.useMRF:
            ...
            posteriors = self.gmm.getPosteriors(
                data, priorsMRF, self.classFractions
            )
            posteriorsGaussian, _ = self.gmm.getGaussianPosteriors(
                data, priorsMRF @ self.classFractions.T
            )
        else:
            posteriors = self.gmm.getPosteriors(
                data, priors, self.classFractions
            )
            posteriorsGaussian, _ = self.gmm.getGaussianPosteriors(
                data, priors @ self.classFractions.T
            )
            atlasPriors = priors
        ...
        return (
            ...
            priors,
            atlasPriors,
            posteriorsGaussian[:, -self.nTumorComp :],
        )
    ...
    def computeFinalSegmentationShapeModel(self):
        ...
        tumorStructureNumber = np.flatnonzero(
            self.classFractions[tumorClassNumber, :] == 1
        )[0]
        trainToTemplateTransformFileName = os.path.join(
            self.atlasDir, "brats_template_transforms.mat"
        )  # because the VAE is trained on brats data
        trainToTemplateMat = np.linalg.inv(
            loadmat(trainToTemplateTransformFileName)[
                "imageToImageTransformMatrix"
            ]
        )
        ...
        nll_hist = [1 / eps]
        for j in range(150):
            tumorPosterior, nll = tumorGmm.getGaussianPosteriors(
                data[isTumor], tmpPrior
            )
            tumorPosterior /= np.expand_dims(tumorPosterior.sum(axis=1), 1)
            tumorGmm.fitGMMParameters(data[isTumor], tumorPosterior)
            nll_hist.append(nll)
            if (
                nll_hist[-2] - nll_hist[-1]
                < changeCostEMPerVoxelThreshold * isTumor.sum() / 100
            ):
                print(
                    "Initalized new tumor component: ran %d iterations"
                    % (j + 1)
                )
                break
        ...
        for tumorCompNumber in range(self.nTumorComp + 1):
            tumor[isTumorMask, 1 + tumorCompNumber] = (
                np.argmax(tumorPosterior, axis=-1) == tumorCompNumber
            )
        tumor[
            ~isTumorMask, 0
        ] = 1  # order: Background, tumorComp1, 2 ,3, (the initial configuration)
        tumorTrainSpace = np.zeros(
            np.concatenate([vaeShape, [self.nTumorComp + 2]])
        )
        for tumorCompNumber in range(self.nTumorComp + 2):
            tumorTrainSpace[:, :, :, tumorCompNumber] = affine_transform(
                tumor[:, :, :, tumorCompNumber],
                trainToSubjectMat,
                output_shape=vaeShape,
                order=1,
            )
        configLoss = []
        for config in configurations:
            ...
            vaeLoss = log_loss(
                np.argmax(tumorTmp.reshape(-1, tumor.shape[-1]), axis=1),
        ...
        tumor = tumor[
            :, :, :, np.array([0] + list(bestConfig))
        ]  # now the order is assumed to be the same as in the VAE: Background, core, edema, enhancing core
        ...
        gmm = samseg.GMM(
            self.gmm.numberOfGaussiansPerClass[:-1]
            + list(np.ones(self.nTumorComp + 1).astype(np.int)),
            numberOfContrasts=self.imageBuffers.shape[-1],
            useDiagonalCovarianceMatrices=self.modelSpecifications.useDiagonalCovarianceMatrices,
        )
        gmm.initializeGMMParameters(
            data,
            np.ones([data.shape[0], gmm.numberOfGaussians])
            / gmm.numberOfGaussians,
        )
        gmm.means[
            : self.gmm.numberOfGaussians - self.nTumorComp
        ] = self.gmm.means[: -self.nTumorComp]
        gmm.variances[
            : self.gmm.numberOfGaussians - self.nTumorComp
        ] = self.gmm.variances[: -self.nTumorComp]
        gmm.means[
            self.gmm.numberOfGaussians - self.nTumorComp :
        ] = tumorGmm.means
        gmm.variances[
        ...
        priorsNew[:, priors.shape[1] - 1 :] = (
            atlasPriors[:, tumorIdx] * tumorGmm.mixtureWeights
        )
        likelihoodsNew = np.zeros(priorsNew.shape)
        likelihoodsNew[:, : likelihoods.shape[1] - 1] = likelihoods[
            :, ~tumorIdx
        ]
        likelihoodsNew[:, likelihoods.shape[1] - 1 :] = np.array(
            [
                tumorGmm.getGaussianLikelihoods(
                    data,
                    np.expand_dims(tumorGmm.means[i], 1),
                    tumorGmm.variances[i],
                )
                for i in range(self.nTumorComp + 1)
            ]
        ).T
        ...
        priors = priorsNew
        classFractionsNew = np.zeros(
            [gmm.numberOfClasses, likelihoods.shape[1]]
        )
        tumorIdx = np.zeros(self.classFractions.shape[0]).astype(np.bool)
        tumorIdx[tumorClassNumber] = True
        classFractionsNew[
            : self.classFractions.shape[0] - 1, : self.classFractions.shape[1]
        ] = self.classFractions[~tumorIdx]
        classFractionsNew[
            self.classFractions.shape[0] - 1, : self.classFractions.shape[1]
        ] = self.classFractions[tumorIdx]
        classFractionsNew[
            self.classFractions.shape[0] :, self.classFractions.shape[1] :
        ] = np.eye(2)
        ...
        if not self.groundTruth is None:
            ...
            scores = np.array(
                [
                    dice_score(
                        self.groundTruth[self.cropping][self.mask].flatten()
                        == k,
                        np.argmax(tumor[self.mask], axis=-1).flatten() == k,
                    )
                    for k in range(4)
                ]
            )
        ...
        for sweepNumber in range(
            self.numberOfBurnInSteps + self.numberOfSamplingSteps
        ):
            tumorTrainSpace = np.zeros(
                np.concatenate([vaeShape, [self.nTumorComp + 2]])
            )
            ...
            if (
                sweepNumber < self.numberOfBurnInSteps and False
            ):  # can use this to switch configurations, leave it out for now
                configLoss = []
                configs = configurations
                for config in configs:
                    tumorTmp = tumorTrainSpace[
                        :, :, :, np.array([0] + list(config))
                    ]
                    mean, logvar = vae.encode(np.expand_dims(tumorTmp, 0))
                    z = vae.reparameterize(mean, logvar)
                    tumorVAE = vae.decode(z, apply_softmax=True).numpy()[0]
                    vaeLoss = log_loss(
                        np.argmax(
                            tumorTmp.reshape(-1, tumor.shape[-1]), axis=1
                        ),
                        tumorVAE.reshape(-1, tumorVAE.shape[-1]),
                    )
                    configLoss.append(vaeLoss)
                currentConfiguration = np.array(
                    configurations[np.argmin(configLoss)]
                )
            ...
            mean, logvar = vae.encode(
                np.expand_dims(
                    tumorTrainSpace[
                        :, :, :, np.array([0] + list(currentConfiguration))
                    ],
                    0,
                )
            )
            ...
            for tumorCompNumber in range(self.nTumorComp + 2):
                tumorPriorSubjSpace[
                    ...
                    tumorVAETrainSpace[:, :, :, tumorCompNumber],
                    subjectToTrainMat,
            ...
            if (
                sweepNumber > 0
            ):  # skip sampling parameters in the first iteration
                for j, compNumber in enumerate(
                    np.arange(self.nTumorComp + 1)[currentConfiguration - 1]
                ):
                    constraints = None
                    if self.useMeanConstr:
                        if j == 1 and self.flair_idx is not None:
                            constraint = (
                                ...
                                [
                                    max(
                                        self.gmm.means[
                                            self.wm_comp_number, self.flair_idx
                                        ],
                                        self.gmm.means[
                                            self.gm_comp_number, self.flair_idx
                                        ],
                                    )
                                    + np.log(1.15),
                                    np.inf,
                    ...
                    (
                        newMean,
                        newVariance,
                    ) = gmm.sampleMeansAndVariancesConditioned(
                        data,
                        np.expand_dims(
                            posteriors[:, posteriors.shape[1] - 3 + compNumber],
                            1,
                        ),
                        sum(self.gmm.numberOfGaussiansPerClass[:-1])
                        + compNumber,
                ...
                gmm.means[-(self.nTumorComp + 1) :] = np.array(
                    newMeans
                ).squeeze()
                gmm.variances[-(self.nTumorComp + 1) :] = np.array(newVariances)
            effectivePriors = priors.copy()
            effectivePriors[:, -(self.nTumorComp + 1) :] *= tumorPriorSubjSpace[
                :, 1:
            ]
            effectivePriors[:, : -(self.nTumorComp + 1)] *= tumorPriorSubjSpace[
                :, 0, np.newaxis
            ]
            effectivePriors /= np.expand_dims(
                np.sum(effectivePriors, axis=1) + eps, 1
            )
            ...
            tumorp = np.concatenate(
                [
                    np.expand_dims(1 - posteriorsTumor.sum(axis=1), 1),
                    posteriorsTumor,
                ],
                axis=1,
            )
                ...
                scores = np.array(
                    [
                        dice_score(
                            self.groundTruth[self.cropping][self.mask].flatten()
                            == k,
                            np.argmax(tumorp, axis=-1).flatten() == k,
                        )
                        for k in range(4)
                    ]
                )
                scores_avg = np.array(
                    [
                        dice_score(
                            self.groundTruth[self.cropping][self.mask].flatten()
                            == k,
                            np.argmax(averageTumorPosteriors, axis=-1).flatten()
                            == k,
                        )
                        for k in range(4)
                    ]
            ...
            if sweepNumber >= self.numberOfBurnInSteps:
                print(
                    "Sample "
                    + str(sweepNumber + 1 - self.numberOfBurnInSteps)
                    + " times"
                )
                averagePosteriors += posteriors / self.numberOfSamplingSteps
                averageTumorPosteriors += (
                    posteriorsTumor / self.numberOfSamplingSteps
                )
            else:
...
def imagesInAtlasSpace(images, deformation, transform, referenceMesh):
    ...
    if np.max(np.absolute(deformation)) > 0:
        ...
        validMask = np.sum(
            validMask, axis=3
        )  ## only zero where all three coordinates are zero
        denseDeformation = (tmp) / (2 ** 16 - 1) * (
            maxDeformation - minDeformation
        ) + minDeformation
        ...
        denseDeformation = denseDeformation[
            closestIndices[0], closestIndices[1], closestIndices[2]
        ]
    ...
    X, Y, Z = np.meshgrid(
        linspaces[0], linspaces[1], linspaces[2], indexing="ij"
    )
    ...
    atlas = samseg.ProbabilisticAtlas()
    densePositionsInSubjectSpace = atlas.mapPositionsFromTemplateToSubjectSpace(
        densePositions, transform
    )
    imagesOut = np.zeros(imageSize + [images.shape[-1]])
    for i in range(images.shape[-1]):
        denseIntensitiesInTemplateSpace = map_coordinates(
            images[:, :, :, i],
            densePositionsInSubjectSpace.T,
            order=1,
            mode="nearest",
        )
        imagesOut[:, :, :, i] = denseIntensitiesInTemplateSpace.reshape(
            imageSize
        )
    return imagesOut
...
def makeTumorAtlas(atlasDirOrig, atlasDirNew, tumorPrior=0.2, skull=True):
    ...
    levelFiles = [
        join(atlasDirOrig, i)
        for i in os.listdir(atlasDirOrig)
        if "level" in i.lower()
    ]
    for meshFile in levelFiles:
        ...
        if skull:
            ...
            tumorPossible = (
                mesh.alphas[:, 0] < 0.5
            )  # can only have tumor inside the brain
    ...
    otherFiles = [
        join(atlasDirOrig, i)
        for i in os.listdir(atlasDirOrig)
        if not "level" in i.lower()
        and not "parameters.txt" in i.lower()
        and not "compressionlookuptable.txt" in i.lower()
    ]
...
def getNeighborIndices(shape):
    ...
    index_arr = (
        XYZ[:, :, :, np.newaxis, :]
        - diffs[np.newaxis, np.newaxis, np.newaxis, :]
    )
    ...
    index_arr = ix[neighbors_x, neighbors_y, neighbors_z].reshape(
        list(shape) + [26]
    )
    return index_arr
...
class CatVAE(tf.keras.Model):
    def __init__(
        self, width, height, depth, numClasses, latent_dim, temperature
    ):
        ...
        self.generative_net = tf.keras.models.Sequential(
            [
                ...
                tf.keras.layers.Dense(
                    units=2 * 2 * 2 * 32, activation=tf.nn.leaky_relu
                ),
                tf.keras.layers.Dense(
                    units=4 * 4 * 3 * 32, activation=tf.nn.leaky_relu
                ),
    ...
    @tf.function
    def sample(
        self, eps=None, samples=1, sample_decode=False, apply_softmax=False
    ):
        if eps is None:
            eps = tf.random.normal(shape=(samples, self.latent_dim))
        return self.decode(
            eps, sample_decode=sample_decode, apply_softmax=apply_softmax
        )
    ...
    def decode(self, z, sample_decode=False, apply_softmax=False):
        ...
        if sample_decode:
            sample = tfp.distributions.RelaxedOneHotCategorical(
                temperature=self.temperature, logits=logits
            ).sample()
            return sample
        if apply_softmax:
    ...
    def padToProperSize(self, logits):
        ...
        shape = logits.shape
        return logits[
            :, 8 : shape[1] - 8, 8 : shape[2] - 8, 19 : shape[3] - 18, :
        ]
