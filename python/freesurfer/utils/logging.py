...
class Table:
    ...
    def addRow(self, cells):
        ...
        if len(cells) != len(self.columns):
            raise ValueError(
                "number of input cells does not match number of columns"
            )
    ...
    def __str__(self):
        ...
        # format header
        header = "  ".join(
            [self._pad(name, widths[n]) for n, name in enumerate(self.columns)]
        )
        ...
        for row in self.rows:
            strings.append(
                "  ".join(
                    [self._pad(cell, widths[n]) for n, cell in enumerate(row)]
                )
            )
        return "\n".join(strings)
