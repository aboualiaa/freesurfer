...
class HelpFormatter(argparse.HelpFormatter):
    ...
    def __init__(self, indent_increment=4, *args, **kwargs):
        super(HelpFormatter, self).__init__(
            indent_increment=indent_increment, *args, **kwargs
        )

    # format each argument
    def _format_action(self, action):
        parts = [
            self._current_indent * " " + self._format_action_invocation(action)
        ]
        if action.help:
            self._indent()
            help_text = self._expand_help(action)
            for line in self._split_lines(
                help_text, self._width - self._current_indent
            ):
    ...
    def _format_action_invocation(self, action):
        if not action.option_strings:
            ...
            if action.nargs != 0:
                ...
                msg += "  " + term.blue(
                    self._format_args(action, default).upper()
                )
            return msg
    ...
    def _format_usage(self, usage, actions, groups, prefix):
        ...
        # if optionals and positionals are available, calculate usage
            ...
            positionals = [
                action for action in actions if not action.option_strings
            ]
    ...
    class _Section(argparse.HelpFormatter._Section):
        ...
        def format_help(self):
            ...
            if (
                self.heading is not argparse.SUPPRESS
