...
class TableWriter:
    ...
    def assign_attributes(
        self, filename="stats.table", row1col1="", delimiter="\t"
    ):
    ...
    def parse(self, measure):
        ...
        for line in self.fp:
            ...
            if line.rfind("#") == -1:
                ...
                if (
                    self.exclude_structlist
                    and seg in self.exclude_structlist.keys()
                ):
                    continue
                self.id_name_map[seg] = strlst[4]  # Col 4 is Seg Name
                # Float of the measure we are interested in
                self.measurelist.append(
                    float(strlst[self.measure_column_map[measure]])
                )
        ...
        for line in self.fp:
            ...
            if measure == "volume" and self.include_vol_extras:
                beg_struct_tuple = (
                    ...
                    (
                        "# Measure lhCorticalWhiteMatter, lhCorticalWhiteMatterVol,",
                        "lhCorticalWhiteMatterVol",
                    ),
                    (
                        "# Measure rhCorticalWhiteMatter, rhCorticalWhiteMatterVol,",
                    ...
                    (
                        "# Measure SuperTentorial, SuperTentorialVol,",
                        "SuperTentorialVol",
                    ),
                    (
                    ...
                    (
                        "# Measure BrainVol-to-eTIV, BrainVol-to-eTIV,",
                        "BrainVol-to-eTIV",
                    ),
                    (
    ...
    def parse(self):
        ...
        for line in self.fp:
            ...
            if line.rfind("#") == -1:
                ...
                if (
                    self.exclude_structlist
                    and meas in self.exclude_structlist.keys()
                ):
                    continue
                val = float(strlist[1])
        ...
        if self.include_structlist:
            ...
            for omeasure in self.include_structlist.keys():
                ...
                if omeasure in measurelist:
                    tmp_measure_value_map[omeasure] = self.measure_value_map[
                        omeasure
                    ]
                else:
                    print(
                        "ERROR: The measure you requested: "
                        + omeasure
                        + " is not valid"
                    )
...
class LongQdecTable:
    ...
    # constructor
    def __init__(
        self, stpmap=None, vari=None, sdir=None, cval=None, cross=True
    ):
        if stpmap == None:
            return
        ...
        for line in fp:
            ...
            if strlst[0].upper() == "SUBJECTS_DIR":
                ...
                if key in self.subjects_tp_map:
                    ...
                    if self.cross:
                        print(
                            "\nERROR: no fsid-base in header, but fsid "
                            + key
                            + " seems to exists multiple times?\n"
                        )
                        sys.exit(1)
                    ...
                    for tpdata in self.subjects_tp_map[key]:
                        if tpdata[0] == tp:
                            print(
                                "ERROR: Multiple occurence of time point (fsid) '"
                                + tp
                                + "' in (fsid-base) "
                                + key
                                + "!"
    ...
    def split(self, col="fsid-base"):
        ...
        if col == "fsid-base":
            for key, value in self.subjects_tp_map.items():
                ...
                stpmap[key] = value
                alltables.append(
                    LongQdecTable(stpmap, self.variables, "", key, self.cross)
                )
            ...
            if len(poscols) != 1:
                print(
                    "ERROR: did not find "
                    + col
                    + " or found it in several columns!"
                )
                sys.exit(1)
            colnum = poscols[0] + 1
            ...
            for bid, value in self.subjects_tp_map.items():
                ...
                for tpdata in value:
                    if tpdata[colnum] != key:
                        print(
                            "ERROR: split: "
                            + col
                            + " value needs to be the same within each subject ("
                            + bid
                            + ")!"
                        )
                        sys.exit(1)
                if key not in allasdict:
                    stpmap = StableDict()
                    stpmap[bid] = value
                    allasdict[key] = LongQdecTable(
                        stpmap, self.variables, "", key, self.cross
                    )
                else:
    ...
    def sort(self, col):
        ...
        for key, value in self.subjects_tp_map.items():
            ...
            self.subjects_tp_map[key] = sorted(
                value, key=lambda tpdata: tpdata[colnum]
            )
    ...
    def append_table(self, filename):
        ...
        statstable.parse(
            filename, False
        )  # don't warn about being cross sectional table
            ...
            if not statstable.cross:
                ...
                for i, tpdata, addtpdata in itertools.izip(
                    ...
                    self.subjects_tp_map[subjectid][i] = list(
                        itertools.chain(
                            *[
                                self.subjects_tp_map[subjectid][i],
                                addtplist[i][1:],
                            ]
                        )
                    )
