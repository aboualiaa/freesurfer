...
class Surface(Transformable):
    ...
    def __init__(self, vertices, faces=None, hemi=None, geom=None):
        ...
        if isinstance(vertices, str):
            raise ValueError(
                "if loading from file, use the Surface.read() class method"
            )
    ...
    def neighboring_vertices(self, vertex):
        ...
        neighbors = np.unique(
            [self.faces[fno] for fno in self.neighboring_faces(vertex)]
        )
        return neighbors[
            neighbors != vertex
        ]  # be sure to remove the central vertex
    ...
    def surf2vox(self, vol=None):
        ...
        """
        if vol is not None:
            return LinearTransform.matmul(
                vol.geometry().ras2vox(), self.surf2ras()
            )
        else:
    ...
    def vox2surf(self, vol=None):
        ...
        if vol is not None:
            return LinearTransform.matmul(
                self.ras2surf(), vol.geometry().vox2ras()
            )
        else:
    ...
    def parameterization_map(self):  # TODEP
        ...
        raise DeprecationWarning(
            "parameterization_map() has been removed! Email andrew!"
        )

    def copy_geometry(self, vol):  # TODEP
        """Deprecated! """
        warning(
            "copy_geometry(vol) has been removed! Just use: surf.geom = vol.geometry()"
        )
    ...
    def isSelfIntersecting(self):  # TODEP
        """Deprecated!"""
        raise DeprecationWarning(
            "isSelfIntersecting has been removed! Email andrew if you get this!!"
        )

    def fillInterior(self):  # TODEP
        """Deprecated!"""
        raise DeprecationWarning(
            "fillInterior has been removed! Email andrew if you get this!!"
        )
    ...
    def get_face_vertices(self):  # TODEP
        ...
        warning(
            '"Surface.get_face_vertices()" is deprecated - use "Surface.faces" directly. Sorry for the back and forth...'
        )
        return self.faces
    ...
    def get_vertex_normals(self):  # TODEP
        ...
        warning(
            '"Surface.get_vertex_normals()"" is deprecated - use "Surface.vertex_normals" directly. Sorry for the back and forth...'
        )
    ...
    def get_vertex_faces(self):  # TODEP
        ...
        raise DeprecationWarning(
            "get_vertex_faces has been removed! Use Surface.neighboring_faces or email andrew if you get this!!!!"
        )
