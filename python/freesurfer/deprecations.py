...
def createFaceArrays(surf):
    raise DeprecationWarning(
        "createFaceArrays is no longer - use Surface.neighboring_faces instead"
    )
...
def source(filename):
    raise DeprecationWarning(
        "sourcing from python is no longer supported as it is quite dangerous"
    )
...
def read_annotation(*args, **kwargs):
    raise DeprecationWarning(
        "read_annotation is deprecated for now. email Andrew if you get this"
    )
