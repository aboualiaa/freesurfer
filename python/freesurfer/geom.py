...
class Slicing(tuple):
    ...
    def __new__(cls, start, stop):
        if len(start) != len(stop):
            raise ValueError(
                "start coord (%dD) does not match stop coord (%dD))"
                % (len(start), len(stop))
            )
        return super(Slicing, cls).__new__(
            cls, [slice(s, t) for s, t in zip(start, stop)]
        )
...
def resample(
    source, target_shape, trg2src, interp_method="linear", smooth_sigma=0
):
    """
    ...
    if source.ndim > 4:
        raise ValueError(
            "resampling can not be done on arrays with more than 4 dimensions"
        )
    elif source.ndim < 3:
        raise NotImplementedError(
            "%dD resampling is not yet supported (must be 3D or 4D)"
            % source.ndim
        )

    if len(target_shape) != source.ndim:
        raise ValueError(
            "resampled target shape (%sD) must match source dims (%sD)"
            % (len(target_shape), source.ndim)
        )
    ...
    if target_shape[-1] != source.shape[-1]:
        raise ValueError(
            "resampled target must have the same number of frames as the source"
        )

    # apply optional gaussian smoothing
    if smooth_sigma > 0:
        sigmas = (smooth_sigma, smooth_sigma, smooth_sigma, 0)
        source = scipy.ndimage.gaussian_filter(
            source, sigma=sigmas, order=0
        )  # TODO order?

    trg2src = LinearTransform.ensure(trg2src).matrix

    if interp_method == "linear":
        return bindings.vol.resample_volume_linear(
            source, target_shape, trg2src
        ).reshape(orig_target_shape)
    elif interp_method == "nearest":
        return bindings.vol.resample_volume_nearest(
            source, target_shape, trg2src
        ).reshape(orig_target_shape)
    else:
        raise ValueError(
            'invalid resample interpolation method "%s"' % interp_method
        )
...
def apply_warp(source, flow, interp_method="linear", indexing="ij"):
    ...
    if source.ndim > 4:
        raise ValueError(
            "warping can not be done on arrays with more than 4 dimensions"
        )
    elif source.ndim < 3:
        raise NotImplementedError(
            "%dD warping is not yet supported (must be 3D or 4D)" % source.ndim
        )

    linvec = [np.arange(0, dim) for dim in source.shape[:3]]
    loc = (
        np.rollaxis(np.array(np.meshgrid(*linvec, indexing=indexing)), 0, 4)
        + flow
    )
    sample = np.stack((loc[:, :, :, 0], loc[:, :, :, 1], loc[:, :, :, 2]), 3)

    warp = lambda src: interpn(
        linvec,
        src,
        sample,
    ...
    if source.ndim == 4:
        return np.stack(
            [warp(source[..., f]) for f in range(source.shape[-1])], axis=-1
        )
