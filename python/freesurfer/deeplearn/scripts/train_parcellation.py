...
):  # need to implement mirrored strategy for this
...
parc_fnames = glob.glob(
    os.path.join(dkt_dir, "*/label", hemi + "." + parc_name)
)
ntraining = 50
nval = 10
...
if read_hemi is not hemi or (
    "parcs" not in locals() and "parcs" not in globals()
):
    ...
    dkt_inflated_fnames = [
        os.path.join(dkt_dir, subject, "surf", hemi + "." + inflated_name)
        for subject in dkt_subjects
    ]
    dkt_spheres = [fs.Surface.read(fname) for fname in tqdm(dkt_sphere_fnames)]
    dkt_inflated = [
        fs.Surface.read(fname) for fname in tqdm(dkt_inflated_fnames)
    ]
    ...
    all_labels = nes.py.utils.flatten_lists(
        [list(np.where(parc.data < 0, 0, parc.data)) for parc in parcs]
    )
    ...
    for sno, subject in enumerate(tqdm(dkt_subjects)):
        ...
        mrisp_annot = np.transpose(
            dkt_spheres[sno].parameterize(parcs[sno], interp="nearest"), (1, 0)
        )
        if pad > 0:
    ...
    dkt_mrisps_onehot = [
        fsd.utils.np_one_hot(annot, nlabels) for annot in dkt_mrisps_annot
    ]
...
val_gen = fsd.surf_utils.parc_gen(
    dkt_mrisps_geom_val, dkt_mrisps_onehot_val, batch_size=1
)
test_gen = fsd.surf_utils.parc_gen(
    dkt_mrisps_geom_test, dkt_mrisps_onehot_test, batch_size=ntest
)
train_gen = fsd.surf_utils.parc_gen(
    dkt_mrisps_geom_training, dkt_mrisps_onehot_training, batch_size=8
)

# loss and compilation
lr = 5e-4
sphere_loss = fsd.losses.spherical_loss(
    mrisp_shape_nopad[0:2], pad=pad, threshold=0
)
...
nes.utils.check_and_compile(
    unet, gen=train_gen, optimizer=optimizer, loss=[dice_loss]
)
if use_lookahead:
...
nes.utils.plot_fit_callback(
    fit_results_fname, keys=["loss", "val_loss", "lr"], close_all=True, smooth=3
)
...
fv.vol(
    np.argmax(outb, axis=-1),
    name="targets",
    colormap="lut",
    opts=":linked=1:visible=0",
)
fv.vol(
    np.argmax(pred, axis=-1),
    name="pred",
    colormap="lut",
    opts=":linked=1,visible=1",
)
...
for sno, sphere in enumerate(tqdm(dkt_spheres[ntraining + nval :])):
    ...
    aparc = sphere.sample_parameterization(
        ind_to_lab[np.transpose(mrisp)], interp="nearest"
    )
    aparcs.append(aparc)

    mrisp = np.argmax(outb[sno, pad:-pad, pad:-pad, ...], axis=-1)
    mparc = sphere.sample_parameterization(
        ind_to_lab[np.transpose(mrisp)], interp="nearest"
    )
    mparcs.append(mparc)
...
for n in range(num_to_show):
    ...
    fv.surf(
        ...
        annot=[overlay, mparc],
        curvature=sulcs[n],
        opts=":visible=%d" % (n == num_to_show - 1),
    )
