...
parser.add_argument(
    "--uthresh", type=float, help="specify threshold to erase above"
)
parser.add_argument(
    "--border",
    help="number of border voxels to set threshold at",
    default=4,
    type=int,
)
...
if 0:
    ...
    right_pad = np.array((256, 256, 256)) - (
        np.array(mri_conf.shape) + left_pad
    )
    padding = (
...
if 0:
    ...
    mri_unconf = mri_unconf1.reslice(mri_in.voxsize).conform_to_shape(
        mri_in.shape
    )
else:
...
mri_out.data = mri_in.data * (mri_mask.data.squeeze() < args.border).astype(
    np.float
)
