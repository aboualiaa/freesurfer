...
if os.getenv("NGPUS"):
    ...
    print(
        "reading %d GPUS from env and setting CUDA_VISIBLE_DEVICES to %s"
        % (ngpus, gpu_str)
    )
...
if gpu_id >= 0:
    ...
    print(
        "using %d gpus %s on host %s with batch_size %d"
        % (ngpus, gpu_list, host, batch_size)
    )
...
batch_size = (batch_size // ngpus) * ngpus
print(
    "found %d gpus after configuring device, resetting batch_size to %d"
    % (ngpus, batch_size)
)
...
fs_atlas_fname = os.path.join(
    adir, hemi + ".average.curvature.filled.buckner40.tif"
)
...
if "fs_atlas" not in locals() and "fs_atlas" not in globals():
    ...
    fsa_surf = fs.Surface.read(
        os.path.join(fsdir, "fsaverage", "surf", hemi + ".sphere")
    )
    fsa_inf = fs.Surface.read(
        os.path.join(fsdir, "fsaverage", "surf", hemi + ".inflated")
    )
    ...
    for subject in tqdm(subjects):
        for fno, curv_name in enumerate(feats):
            surf_fname = os.path.join(
                atlas_dir, subject, "surf", hemi + "." + sphere_name
            )
            curv_fname = os.path.join(
                atlas_dir, subject, "surf", hemi + "." + curv_name
            )
            mrisp_tmp = surf_utils.loadSphere(
                surf_fname, curv_fname, padSize=pad
            )
            if fno == 0:
                mrisp = mrisp_tmp[..., np.newaxis]
            else:
                mrisp = np.concatenate(
                    (mrisp, mrisp_tmp[..., np.newaxis]), axis=-1
                )
    ...
    for fno, curv_name in enumerate(feats):
        ...
        atlas_tmp2 = np.sqrt(
            np.flip(fs_atlas.data[..., fno * 3 + 1], axis=1).transpose()
        )  # convert from variances
        if fno == 0:
            atlas_mean = atlas_tmp[..., np.newaxis]
            atlas_sigma = atlas_tmp2[..., np.newaxis]
        else:
            atlas_mean = np.concatenate(
                (atlas_mean, atlas_tmp[..., np.newaxis]), axis=-1
            )
            atlas_sigma = np.concatenate(
                (atlas_sigma, atlas_tmp2[..., np.newaxis]), axis=-1
            )
    ...
    if pad > 0:
        ...
        atlas_sigma = np.pad(
            atlas_sigma, ((0, 0), (pad, pad), (0, 0)), "reflect"
        )
...
warp_sphere_loss = fsd.losses.spherical_loss(
    tuple(np.array(mrisp_shape_nopad[0:2]) // warp_downsize), pad=pad
)
sphere_loss = fsd.losses.spherical_loss(
    mrisp_shape_nopad[0:2], pad=pad, threshold=0, win=[41, 41]
)
...
gen = surf_utils.fsgen(
    mrisps_geom, atlas_mean, batch_size=batch_size, mean_stream=False
)
...
if read_hemi is not hemi or (
    "dkt_mrisps_onehot" not in locals() and "dkt_mrisps_onehot" not in globals()
):
    ...
    dkt_sphere_fnames = [
        os.path.join(dkt_dir, subject, "surf", hemi + "." + sphere_name)
        for subject in dkt_subjects
    ]

    parcs = [fs.Overlay.read(parc_fname) for parc_fname in tqdm(parc_fnames)]
    dkt_spheres = [
        fs.Surface.read(sphere_fname)
        for sphere_fname in tqdm(dkt_sphere_fnames)
    ]

    # load the entire dataset to determine labels that exist and compact them
    parcs_all = [
        fs.Overlay.read(parc_fname) for parc_fname in tqdm(parc_fnames_all)
    ]
    all_labels = nes.py.utils.flatten_lists(
        [list(np.where(parc.data < 0, 0, parc.data)) for parc in parcs_all]
    )
    al = [l if l >= 0 else 0 for l in all_labels]
    lab_to_ind, ind_to_lab = fsd.utils.rebase_labels(al)

    parcs_training = [
        lab_to_ind[np.where(parc.data < 0, 0, parc.data)] for parc in parcs
    ]
    ...
    for sno, subject in enumerate(tqdm(dkt_subjects)):
        for fno, curv_name in enumerate(feats):  # geometry has 3 scalar fields
            surf_fname = os.path.join(
                dkt_dir, subject, "surf", hemi + "." + sphere_name
            )
            curv_fname = os.path.join(
                dkt_dir, subject, "surf", hemi + "." + curv_name
            )
            mrisp_tmp = surf_utils.loadSphere(
                surf_fname, curv_fname, padSize=pad
            )
        ...
        mrisp_annot = np.transpose(
            dkt_spheres[sno].parameterize(
                parcs_training[sno], interp="nearest"
            ),
            (1, 0),
        )
        if pad > 0:
    ...
    dkt_mrisps_onehot = [
        fsd.utils.np_one_hot(annot, nclasses) for annot in dkt_mrisps_annot
    ]
...
linear_target = 10
lgen = fsd.surf_utils.fsgen_segreg(
    dkt_mrisps_geom,
    atlas_mean,
    dkt_mrisps_onehot,
...
gen = fsd.surf_utils.fsgen_segreg(
    dkt_mrisps_geom,
    atlas_mean,
    dkt_mrisps_onehot,
    batch_size=batch_size,
...
vgen = fsd.surf_utils.fsgen_segreg(
    dkt_mrisps_geom_val,
    atlas_mean,
    dkt_mrisps_onehot_val,
    batch_size=vbatch,
    mean_stream=False,
    use_logprob=False,
)
lvgen = fsd.surf_utils.fsgen_segreg(
    dkt_mrisps_geom_val,
    atlas_mean,
    dkt_mrisps_onehot_val,
    batch_size=vbatch,
...
print(
    "training segreg atlas with dropout=%2.2f, nfeats=%d and nconvs=%d, dilations=%d"
    % (dropout, nfeats, nconv_layers, dilation_rate)
)
...
with strategy.scope():
    model_semi = vxms.networks.TemplateCreationSemiSupervised(
        mrisp_shape,
        nclasses=nclasses,
        atlas_feats=len(feats),
        src_feats=len(feats),
    ...
    stopping_callback = tf.keras.callbacks.EarlyStopping(
        monitor="val_loss", mode="min", verbose=1, patience=500
    )
    ...
    model_semi_fname = (
        "%s/%s.fs_atlas.warp%2.2f.geom%2.2f.dice%2.2f.%s.semi.test.h5"
        % (
            model_dir,
            hemi,
            warp_smooth_wt,
    ...
    channel_loss = nes.losses.channelwise_losses_decorator(
        [0, 1, 2],
        [l2_loss_c0, l2_loss_c1, l2_loss_c2],
    ...
    linear_loss_weights = [
        ...
        warp_smooth_wt,
        1.0 / 100,
    ]  # scale down to account for log-loss

    if fit1:  # do first round of fitting with linear outputs to init things
        nes.tf.utils.check_and_compile(
            model_semi.references.model_linear,
            check_losses=True,
            optimizer=keras.optimizers.Adam(lr=lr),
            loss=linear_losses,
            loss_weights=linear_loss_weights,
        )
        fhist1 = model_semi.references.model_linear.fit(
            lgen,
            epochs=30,
            steps_per_epoch=100,
            callbacks=callbacks,
            validation_data=lvgen,
            validation_steps=1,
        )
    ...
    if fit3:  # only train atlas
        ...
        model_semi_fname = (
            "%s/%s.fs_atlas.warp%2.2f.geom%2.2f.dice%2.2f.%s.semi.finetune.h5"
            % (
                model_dir,
                hemi,
                warp_smooth_wt,
...
y_img = vxm.layers.SpatialTransformer(interp_method="linear", fill_value=None)(
...
fs.Image(np.array(prior_atlas).mean(axis=0)).write(
    hemi + "." + "DKTatlas.priors.mgz"
)
