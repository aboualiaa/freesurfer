...
def extract_patches_dev(
    in_img_file, patch_size, intensity_threshold=0, step=1, img_fg_extents=None
):
    ...
    in_img_data_wmp_crop = in_img_data_wmp[
        min_I:max_I, min_J:max_J, min_K:max_K
    ]
    arr_out = skimage.util.view_as_windows(
        in_img_data_wmp_crop, patch_size, step
    )
...
def extract_patches(
    in_img_data, patch_size, intensity_threshold, step_size, *args
):
    ...
    for patch_iter in range(len(idx_x)):
        curr_patch = in_img_data_pad[
            idx_x[patch_iter] : idx_x[patch_iter] + patch_size[0],
...
def build_image_from_patches(
    ...
    patch_mask[
        0 + patch_crop_size[0] : patch_size[0] - patch_crop_size[0],
        0 + patch_crop_size[1] : patch_size[1] - patch_crop_size[1],
        0 + patch_crop_size[2] : patch_size[2] - patch_crop_size[2],
    ] = 1
    ...
    out_img_data = out_img_data[
        patch_size[0] : patch_size[0] + unpadded_img_size[0],
