...
def vae_encoder(input_shape, num_filters, num_poolings, latent_dim):
    ...
    for i in range(num_poolings):
        ...
        conv = MaxPoolingL(
            pool_size, name=("pool_D_%d" % (i)), data_format="channels_last"
        )(conv)
        pools.append(conv)
        convs.append(conv)
...
def vae_decoder(
    latent_dim, pre_sample_shape, num_filters, num_poolings, output_shape
):
    ...
    conv = Reshape(
        (pre_sample_shape[0], pre_sample_shape[1], pre_sample_shape[2])
    )(conv)
...
def vae(
    ...
    encoder, pre_sample_shape = vae_encoder(
    ...
    decoder = vae_decoder(
        latent_dim=latent_dim,
        pre_sample_shape=pre_sample_shape,
        num_filters=num_filters,
        num_poolings=num_poolings,
    ...
    kl_loss = (
        ...
        - K.square(encoder.get_layer("z_mean").output)
        - K.exp(encoder.get_layer("z_log_var").output)
    )
...
def unet(
    ...
    if output_shape != None:
        vec_mse_loss = lambda yt, yp: K.sqrt(
            K.mean(K.sum(K.square(yt - yp), -1))
        )
        loss = [vec_mse_loss]
...
def build_oriented_unet(
    ...
    for i in range(unet_depth):
        ...
        conv = ConvL(
            ...
            padding="same",
            kernel_initializer="he_normal",
            name=("conv3D_D_1_%d" % (i) + layer_prefix),
        )(prev)
        if batch_norm == True:
            conv = BatchNormalization(
                name=("bnorm_D_1_%d" % (i) + layer_prefix)
            )(conv)
        conv = ConvL(
            int(num_filters * (2 ** i) / downsize_filters_factor),
            filter_shape,
            ...
            padding="same",
            kernel_initializer="he_normal",
            name=("conv3D_D_2_%d" % (i) + layer_prefix),
        )(conv)
        if batch_norm == True:
            conv = BatchNormalization(
                name=("bnorm_D_2_%d" % (i) + layer_prefix)
            )(conv)
        if i < (unet_depth - 1):
            pools.append(
                PoolingL(
    ...
    for i in range(unet_depth - 1):
        ...
        conv = ConvL(
            ...
            activation="relu",
            kernel_initializer="he_normal",
            name=("conv3D_U_1_%d" % (level) + layer_prefix),
        )(up)
        if batch_norm == True:
            conv = BatchNormalization(
                name=("bnorm_U_1_%d" % (level) + layer_prefix)
            )(conv)
        conv = ConvL(
            num_filters * (2 ** level),
            filter_shape,
            ...
            activation="relu",
            kernel_initializer="he_normal",
            name=("conv3D_U_2_%d" % (level) + layer_prefix),
        )(conv)
        if batch_norm == True:
            convs.append(
                BatchNormalization(
                    name=("bnorm_U_2_%d" % (level) + layer_prefix)
                )(conv)
            )
        else:
            convs.append(conv)
    ...
    if is_seg_network == False:
        ...
        if output_shape != None:
            conv = AvgPoolingL(
                (4, 4, 4),
                name=("pool_before_reshape" + layer_prefix),
                data_format="channels_last",
            )(conv)
            conv = Flatten()(conv)
            ndown = 3
            oshape = tuple(
                (
                    int(output_shape[0] / 2 ** ndown),
                    int(output_shape[1] / 2 ** ndown),
                    output_shape[2],
                )
            )
            ...
            conv = Conv2D(
                ...
                padding="same",
                kernel_initializer="he_normal",
                name=("conv2D_reshape_0"),
            )(conv)
            for d in range(ndown):
                conv = UpSampling2D(
                    size=(2, 2), name=("upsampling_after_reshape_%d" % d)
                )(conv)
        ...
        if n_labels > 1:
            if num_outputs > 1:
                ...
                conv = Reshape(
                    (ishape[1], ishape[2], ishape[3], num_outputs, n_labels)
                )(reL)
            else:
                conv = ConvL(
                    n_labels,
...
def classnet(
    ...
    for iter_layer in range(depth):
        if iter_layer == 0:
            ...
            if dropout_frac != None:
                model.add(Dropout(dropout_frac))
        unet_num_filters_at_depth = int(
            unet_num_filters_at_depth * feature_scale_with_depth
        )
...
def resnet_model(
    ...
    x = conv_block(
        x, [16, 16, 16], stage=2, block="b", strides=(1, 1, 1), dilation_rate=1
    )
    x = conv_block(
        x, [16, 16, 16], stage=2, block="c", strides=(1, 1, 1), dilation_rate=1
    )
    ...
    x = conv_block(
        x, [32, 32, 32], stage=3, block="b", strides=(1, 1, 1), dilation_rate=2
    )
    x = conv_block(
        x, [32, 32, 32], stage=3, block="c", strides=(1, 1, 1), dilation_rate=2
    )

    x = conv_block(
        x, [64, 64, 64], stage=4, block="a", strides=(1, 1, 1), dilation_rate=4
    )
    x = conv_block(
        x, [64, 64, 64], stage=4, block="b", strides=(1, 1, 1), dilation_rate=4
    )
    x = conv_block(
        x, [64, 64, 64], stage=4, block="c", strides=(1, 1, 1), dilation_rate=4
    )
