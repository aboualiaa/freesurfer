...
def segment_image2D(
    net, im_intensity, wsize, box, nlabels=3, batch_size=256, stride=4
):
    whalf = wsize / 2
    ...
    if box == None:
        box = [
            0,
            im_intensity.shape[0],
            0,
            im_intensity.shape[1],
            0,
            im_intensity.shape[2],
        ]
    xstart = box[0]
    ystart = box[2]
    zstart = box[4]
        ...
        for y in np.arange(ystart, yend, stride):
            ...
            for z in np.arange(zstart, zend):
                ...
                if zmin < 0:
                    zmax -= zmin
                    zmin = 0
                x_test[i, :, :, 0] = im_intensity[
                    x, ymin:ymax, zmin:zmax
                ].squeeze()
                x_test[i, :, :, 1] = im_intensity[
                    xmin:xmax, y, zmin:zmax
                ].squeeze()
                x_test[i, :, :, 2] = im_intensity[
...
def BatchGenerator3D(
    ...
    while 1:
        ...
        if (unknown_whalf is not None) and (
            train_labels[
                x - unknown_whalf : x + unknown_whalf,
                y - unknown_whalf : y + unknown_whalf,
                z - unknown_whalf : z + unknown_whalf,
            ].max()
            < 1
        ):
        ...
        if augment_permute == True:
            ...
            intensity_patch[:, :, :, 0] = np.transpose(
                intensity_patch[:, :, :, 0], permuted_axes
            )
        batch_intensity[found, :] = np.reshape(
            intensity_patch, batch_intensity[found, :].shape
        )
        if use_class_net == True:
            for label in range(n_labels):
                batch_labels[found, label] = train_labels[x, y, z] == label
        else:
            label_patch[:, :, :, 0] = train_labels[
                x - whalf : x + whalf,
                y - whalf : y + whalf,
                z - whalf : z + whalf,
            ]
            if augment_permute == True:
                label_patch[:, :, :, 0] = np.transpose(
                    label_patch[:, :, :, 0], permuted_axes
                )
            batch_labels[found, :] = tf.keras.utils.np_utils.to_categorical(
                label_patch, num_classes=n_labels
            )
...
def segment_surface_3D(
    net, mris, mri_normals, mri_intensity, wsize, batch_size=1
):
    if isinstance(mri_normals, nib.freesurfer.mghformat.MGHImage):
        mri_normals = mri_normals.get_data().squeeze()
    whalf = wsize / 2
    ...
    batch_intensity = np.zeros(
        (batch_size, wsize, wsize, wsize, mri_intensity.shape[3])
    )
    for vno in range(nvertices):
        if ((vno + 1) % 1000) == 0:
            print(
                "segmenting v %d of %d: %2.2f%%"
                % (vno, nvertices, 100.0 * vno / nvertices)
            )
        patch = MRISsampleVertexPatch(
            mris, mri_normals, mri_intensity, vno, wsize
        )
        ...
        if found >= batch_size:
            ...
            surface_overlay[vno - found + 1 : vno + 1] = np.reshape(
                pred, surface_overlay[vno - found + 1 : vno + 1].shape
            )
            found = 0
    if found > 0:
...
def segment_FCD_3D(
    mris, mri_normals, mri_intensity, net, wsize, batch_size=100
):
    if len(mris) == 2:
        mris = mris[0]
    ...
    for vno in range(nvertices):
        ...
        if found == batch_size:
            ...
            pred_fcd[vno - found + 1 : vno + 1] = np.reshape(
                pred[:, 1], (found, 1)
            )
            found = 0
...
def segment_2D(
    ...
    im_padded = np.zeros(
        (width + wsize, height + wsize, depth + 2 * whalf_depth)
    )
    patches = np.zeros((batch_size, wsize, wsize, wsize_depth))
    for d in range(depth):
        im_padded[:, :, d + whalf_depth] = np.pad(
            mri_intensity[:, :, d].squeeze(), (whalf, whalf), "edge"
        )
        for d in range(whalf_depth):
            im_padded[:, :, d] = im_padded[:, :, whalf_depth]
            im_padded[:, :, depth + whalf_depth + d] = im_padded[
                :, :, whalf_depth + depth - 1
            ]
    pred_fcd = np.zeros(
        (im_padded.shape[0], im_padded.shape[1], im_padded.shape[2], n_labels)
    )
    ...
    for z in range(zmin, zmax):
        for x in range(whalf, width + whalf, stride):
            if x - whalf > 0 and ((x - whalf) % 10) == 0:
                print(
                    "processing z %d of %d, x %d of %d"
                    % (z - whalf_depth, zmax - whalf_depth, x - whalf, width)
                )
            for y in range(whalf, height + whalf):
                if x - whalf == Gx and y - whalf == Gy:
                    gdb.set_trace()
                intensity_patch = im_padded[
                    x - whalf : x + whalf,
                    y - whalf : y + whalf,
                    z - whalf_depth : z + whalf_depth + 1,
                ]
                ...
                if found >= batch_size or y == height + whalf - 1:
                    ...
                    for st in range(1, stride):
                        pred_fcd[x + st, y - found + 1 : y + 1, z, :] = pred[
                            0:found, :
                        ]
                    found = 0
    ...
    if use_class_net == False:
        counts[counts == 0] = 1
        pred_fcd /= counts
    return_image = pred_fcd[
        whalf : width + whalf,
        whalf : height + whalf,
        whalf_depth : depth + whalf_depth,
    ]
...
def segment_unet_3D(
    ...
    for x in range(whalf, width + whalf, stride):
        ...
        for z in range(zmin, zmax, stride):
            ...
            for y in range(whalf, height + whalf, stride):
                if (
                    Gx is not None
                    and (abs(x - whalf - Gx) < stride)
                    and (abs(y - whalf - Gy) < stride)
                    and (abs(z - whalf - Gz) < stride)
                ):
                    gdb.set_trace()
                intensity_patch[:, :, :, 0] = im_padded[
                    x - whalf : x + whalf,
                    y - whalf : y + whalf,
                    z - whalf : z + whalf,
                ]
    ...
    wts[wts == 0] = 1
    pred_labels /= wts
    return_image = pred_labels[
        whalf : width + whalf, whalf : height + whalf, whalf : depth + whalf
    ]
    ...
    for z in range(zmin, zmax, stride):
        ...
        for x in range(whalf, width + whalf, stride):
            if ((x - whalf) % (10 * stride)) == 0:
                print(
                    "processing z %d of %d, x %d of %d"
                    % (z - whalf, zmax - whalf, x - whalf, width)
                )
            found = 0
            for y in range(whalf, height + whalf, stride):
                if Gx is not None and (
                    abs(x - whalf - Gx) < stride
                    and abs(y - whalf - Gy) < stride
                    and abs(z - whalf - Gz) < stride
                ):
                    gdb.set_trace()
...
def segment_3D_lines(
    ...
    for z in range(zmin, zmax, stride):
        ...
        if planes >= nplanes:
            pred = net.predict(
                patches, batch_size=planes * width * height, verbose=0
            )
            pred_labels[:, :, z : z + nplanes, :] = np.reshape(
                pred[0:found, :], pred_labels[:, :, z : z + nplanes, :].shape
            )
            planes = 0
            for st in range(1, stride):
...
def compute_intensity_stats(
    paths, vname_list, method="mean", ranges=None, vol_list=None
):
        ...
        for vno, vname in enumerate(vname_list):
            ...
            if method == "mean":
                ...
                if ranges is not None:
                    ind = np.where(
                        np.logical_and(im > ranges[vno, 0], im < ranges[vno, 1])
                    )
                    im = im[ind]
                try:
...
def histo_norm_intensities(
    vol,
    nbins=100,
    target_range=[0.0, 1.0],
    anchors=[0.1, 0.9],
    force_zero_to_zero=True,
):
