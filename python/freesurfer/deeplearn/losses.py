...
def kl_loss(alpha):
    def loss(_, y_pred):
        ...
        assert (
            y_pred.shape[-1] > 2
        ), "kl_loss.loss: y_pred must have > 2 channels"
...
def kl_correlation_coefficient_loss(image_sigma, beta):
    def loss(y_true, y_pred):
        ...
        for i in range(area_elts.shape[1]):
            area_elts[:, i] = np.max(
                (math.sin(i * math.pi / area_elts.shape[1]), 0)
            )
        ...
        pearson_r, _ = tf.contrib.metrics.streaming_pearson_correlation(
            y_pred, y_true, weights=area_elts_tf
        )
...
class spherical_loss(object):
    def __init__(
        ...
        for di in range(-1, 2):
            for dj in range(-1, 2):
                ...
                self.dist_filters.append(
                    tf.cast(
                        tf.convert_to_tensor(
                            dist_filter[..., np.newaxis, np.newaxis]
                        ),
                        tf.float32,
                    )
                )
                ...
                dists = np.sqrt(np.square(dx) + np.square(dy))[
                    np.newaxis, ..., np.newaxis
                ]
                self.orig_dists.append(
                    tf.cast(tf.convert_to_tensor(dists), tf.float32)
                )
                ind += 1
    ...
    def laplacianLossEuclidean(self, penalty="l2"):
        def loss(y_true, y_pred):
            ...
            fn = lambda x: vxm.utils.transform(
                tf.stack((self.x0, self.y0, self.z0), axis=2), x
            )
            ...
            yp1 = tf.pad(mapped, ((0, 0), (0, 0), (1, 0), (0, 0)))[
                :, :, 1:, ...
            ]
            ym1 = tf.pad(mapped, ((0, 0), (0, 0), (0, 1), (0, 0)))[
                :, :, :-1, ...
            ]
            ...
            avg_surround = tf.math.divide_no_nan(
                avg_surround * self.radius, norm
            )
        ...
        def loss2(y_true, y_pred):
            ...
            diff = tf.math.squared_difference(
                mapped, tf.stack((self.x0, self.y0, self.z0), axis=2)
            )
            if penalty == "l2":
    ...
    def overlay_loss(self, y_true, y_pred):
        ...
        for ono in range(noverlays):
            pred = self.model.predict(
                [
                    self.curvature_list[ono],
                    self.curvature_list[0],
                    self.overlay_list[ono],
                ]
            )
    ...
    def l2_loss(self, weight, image_sigma):
        def loss(y_true, y_pred):
            ...
            if self.threshold > 0:
                yt_mask = tf.cast(
                    tf.logical_or(
                        tf.less(y_true, -self.threshold),
                        tf.greater(y_true, self.threshold),
                    ),
                    tf.float32,
                )
                ...
                yp_mask = tf.cast(
                    tf.logical_or(
                        tf.less(y_pred, -self.threshold),
                        tf.greater(y_pred, self.threshold),
                    ),
                    tf.float32,
                )
            ...
            try:
                ...
                if image_sigma is not None:
                    retval = weight * tf.sqrt(
                        tf.reduce_mean(
                            tf.math.divide_no_nan(0.5 * y_tf, image_sigma_sq)
                        )
                    )
        ...
        if image_sigma is not None:
            if self.pad > 0:
                image_sigma = tf.squeeze(
                    image_sigma[self.pad : -self.pad, self.pad : -self.pad, ...]
                )
            image_sigma_sq = tf.clip_by_value(
                image_sigma * image_sigma, 1e-3, 1e4
            )
    ...
    def warp_norm_loss(self, weight):
        def loss(y_true, y_pred):
            ...
            theta = self.theta + 2 * np.pi * (
                warp[..., 0] / tf.cast(warp.shape[0], tf.float32)
            )
            phi = self.phi + np.pi * (
                warp[..., 1] / tf.cast(warp.shape[1], tf.float32)
            )
            sin_phi = tf.sin(phi)
            cos_phi = tf.cos(phi)
    ...
    def atlas_loss(self, weight):
        def loss(y_true, y_pred):
            ...
            atlas_loss = -self.ncc(
                ...
                atlas_target[np.newaxis, ..., np.newaxis],
            )
