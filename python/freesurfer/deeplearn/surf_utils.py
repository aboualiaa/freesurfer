    ...
    for i in range(len(sphere_gen.get_inputs())):
        ...
        for g in range(len(feature_names)):
            outputs = model.predict(
                [input_features, target_atlas, input_features[..., g : g + 1]],
                verbose=0,
            )
...
def print_loss(model, step, training, train_loss):
    ...
    for i in range(len(train_loss)):
        ...
        if i == 0:
            ...
            s += ", "
        if i > 0 and hasattr(
            model.outputs[i - 1], "_name"
        ):  # -1 because first entry in loss is overall loss
...
def build_semi_model_old(vol_size, nf_enc, nf_dec, nchannels=1):
    ...
    overlay_output = vxm.layers.SpatialTransformer(name="overlay")(
        [overlay_input, warp_layer]
    )
    model_semi = keras.models.Model(
        inputs=model.inputs + [overlay_input],
        outputs=model.outputs + [overlay_output],
    )
...
def build_semi_model(
    mrisp_shape, nlabels, nb_feat, nb_levels, nb_conv_per_level=2, nchannels=1
):
    ...
    label_output = vxm.layers.SpatialTransformer(name="warped_label")(
        [label_input, model.references.pos_flow]
    )
    model_semi = keras.models.Model(
        inputs=model.inputs + [label_input],
        outputs=model.outputs + [label_output],
    )
    return model_semi


def build_semi_atlas_model(
    mrisp_shape, nlabels, nb_feat, nb_levels, nb_conv_per_level=2, nchannels=1
):
    model = vxm.networks.VxmDense(
        mrisp_shape,
    ...
    if nlabels > 0:
        label_input = KL.Input(
            shape=mrisp_shape + (nlabels,), name="semi_input"
        )
        label_output = vxm.layers.SpatialTransformer(name="warped_semi")(
            [label_input, model.references.pos_flow]
        )
        model_semi = keras.models.Model(
            inputs=model.inputs + [label_input],
...
def warp_all_inputs(
    model_semi, train_sphere_gen, atlas_target_mrisp_mean, pad=8
):
    ...
    for i in range(len(train_sphere_gen.get_inputs())):
        ...
        outputs = model_semi.predict(
            [input_feature, atlas_target_mrisp_mean, input_overlay], verbose=0
        )
...
def load_atlas(nfeats, hemi="lh", overlay_name=None, atlas_surf=None, pad=8):
    atlas_target_mrisp_mean_ov = fs.Overlay.read(
        "%s.atlas_means.mrisp.nfeats%d.mgz" % (hemi, nfeats)
    )
    atlas_target_mrisp_std_ov = fs.Overlay.read(
        "%s.atlas_stds.mrisp.nfeats%d.mgz" % (hemi, nfeats)
    )
    ...
    if overlay_name is not None:
        ...
        atlas_overlay_mrisp = np.pad(
            atlas_surf.parameterize(atlas_overlay).transpose(),
            ((pad, pad), (0, 0)),
            "wrap",
    ...
    if len(atlas_target_mrisp_mean.shape) == 2:
        atlas_target_mrisp_mean = atlas_target_mrisp_mean[
            np.newaxis, ..., np.newaxis
        ]
    if len(atlas_target_mrisp_std.shape) == 2:
        atlas_target_mrisp_std = atlas_target_mrisp_std[
            np.newaxis, ..., np.newaxis
        ]
...
def semi_surf_atlas_gen(
    ...
    atlas_input = np.repeat(
        np.stack((atlas_mean, atlas_var), axis=-1)[np.newaxis],
        batch_size,
        axis=0,
    )
...
def load_func_and_spheres(
    ...
    for spath in tqdm(spaths):
        ...
        for geom_name in geom_names:
            ...
            geom = normCurvature(
                nib.freesurfer.io.read_morph_data(geom_fname),
                which_norm="Median",
            )
            geoms.append(geom)
        ...
        if (sphere.get_vertex_positions().shape[0] != geoms[0].shape[0]) or (
            ...
            print(
                "%s sphere (%d) does not match geom (%d) or func (%d)"
                % (
                    sphere.get_vertex_positions().shape[0],
                    gomes[0].shape[0],
                    func.image.shape[0],
                )
            )
...
def mrisp_semi_gen(
    ...
    zero_warp = np.zeros(
        (batch_size, *tuple(np.array(mrisp_shape) // warp_downsize), 2)
    )
    if use_rand == False:
        ind_moving = 0
        ind_fixed = ndata - 1
...
def mrisp_stacked_atlas_gen(
    ...
    batch_atlas_geom = np.repeat(
        mrisp_geom_mean[np.newaxis], batch_size, axis=0
    )
    batch_atlas_func = np.repeat(
        mrisp_func_mean[np.newaxis], batch_size, axis=0
    )
...
def fsgen(
    ...
    if len(mrisps_geom[0].shape) == 2:  # add feature axis
        ngeom = 1
        batch_atlas = np.repeat(
            mrisp_atlas[np.newaxis, ..., np.newaxis], batch_size, axis=0
        )
        batch_moving = np.zeros((batch_size, *mrisp_shape, ngeom))
    else:
        ngeom = mrisps_geom[0].shape[-1]
...
def AddPatchLayers(input_model, psize=96, stride=64, nconvs=3, ksize=3):
    ...
    warped_seg = vxm.layers.SpatialTransformer(
        fill_value=0, interp_method="linear", name="warped_seg"
    )([input_model.outputs[3], input_model.references.pos_flow])
    warped_image = input_model.outputs[0]
    warped_stack = KL.Concatenate(axis=-1, name="warped_stack")(
        [warped_image, warped_seg]
    )
    ksize = 3
    stride = 64
    psize = 96
    src_feats = warped_stack.get_shape().as_list()[-1]
    source_patches = nes.layers.Patches2D(
        psize=psize, stride=stride, padding="VALID", name="source_patches"
    )(warped_stack)
    clist = []
    ...
    for pno in range(source_patches.shape[-1]):
        prev_tensor = KL.Lambda(
            lambda x: x[..., pno], name="lambda_patch%d" % pno
        )(patch_tensor)
        for cno in range(nconvs):
            conv_output = KL.Conv2D(
                nfeats,
                (3, 3),
                1,
                padding="same",
                name="patch%d_conv%d" % (pno, cno),
                activation="relu",
            )(prev_tensor)
    ...
    recon_tensor = nes.layers.ImageFromPatches2D(image_size, stride=stride)(
        concat
    )
    seg_atlas = KL.Conv2D(
        nclasses,
    ...
    seg_image = vxm.layers.SpatialTransformer(
        fill_value=0, interp_method="linear", name="seg_subject"
    )([seg_atlas, input_model.references.neg_flow])
    seg_output = KL.Activation("softmax", name="segreg_patch_out")(seg_image)

    print("creating final model")
    model_with_patches = tf.keras.Model(
        input_model.inputs, input_model.outputs[0:3] + [seg_output]
    )
    model_linear = tf.keras.Model(
        model_with_patches.inputs, model_with_patches.outputs[0:3] + [seg_image]
    )
...
class LearnedPatches(Layer):
    ...
    def call(self, x):
        ...
        for ch in range(self.nb_patches):
            map_fn = lambda z: tf.squeeze(
                tf.reshape(
                    tf.matmul(
                        tf.reshape(z, (1, tf.size(x))), self.kernel[..., ch]
                    ),
                    (*self.patch_size, 1),
                )
            )
...
class LearnedUnPatches(Layer):
    ...
    def call(self, x):
        ...
        for ch in range(x.shape[-1]):
            map_fn = lambda z: tf.reshape(
                tf.matmul(
                    tf.reshape(z[..., ch], (1, tf.size(z[..., ch]))),
                    self.kernel[..., ch],
                ),
                self.image_shape,
            )
...
class PatchModel2D(LoadableModel):
    ...
    def __init__(
        ...
        patches = LearnedPatches(
            patch_size, nb_patches, name="learned_patches"
        )(inp)
        prev_tensor = patches
        for cno in range(nconvs):
            prev_tensor = KL.Conv2D(
                nfeats,
                (3, 3),
                1,
                padding="same",
                name="patch_conv%d" % (cno),
                activation="relu",
            )(prev_tensor)

        image_recon = LearnedUnPatches(
            (*inshape, nb_input_channels), name="unpatched"
        )(prev_tensor)
        ...
        softmax_out = KL.Activation("softmax", name="patchout_softmax")(
            image_linear
        )
