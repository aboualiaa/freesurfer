/**
 * @file  mri_jacobian.c
 * @brief Computes the (determinant of the) jacobian of the input non-linear
 * morph.
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_jacobian.c,v 1.11 2011/12/10 22:47:57 fischl Exp $",
      "$Name:  $");
  ...
  if (Gx > 0)
    printf("area = %2.3f, orig = %2.3f\n",
           MRIgetVoxVal(mri_area, Gx, Gy, Gz, 0),
           MRIgetVoxVal(mri_orig_area, Gx, Gy, Gz, 0));
  if (lta) {
    double det;
  ...
  if (Gx > 0)
    printf("after smoothing area = %2.3f, orig = %2.3f\n",
           MRIgetVoxVal(mri_area, Gx, Gy, Gz, 0),
           MRIgetVoxVal(mri_orig_area, Gx, Gy, Gz, 0));
  mri_jacobian = MRIdivide(mri_area, mri_orig_area, nullptr);
  if (Gx > 0)
    printf("jacobian = %2.3f\n", MRIgetVoxVal(mri_jacobian, Gx, Gy, Gz, 0));
  if (atlas)
    mask_invalid(gcam, mri_jacobian);
  if (use_log) {
    MRIlog10(mri_jacobian, nullptr, mri_jacobian, 0);
    if (zero_mean)
...
static void usage_exit(int code) {
  ...
  printf("  -tm3d: the input morph (m3z) originated from tm3d "
         "(mri_cvs_register)\n");
...
static int find_debug_node(GCA_MORPH *gcam, int origx, int origy, int origz) {
  ...
  printf("Talairach voxel (%d, %d, %d) maps to node (%d, %d, %d) %s --> "
         "(%2.1f, %2.1f, %2.1f)\n",
         origx, origy, origz, xmin, ymin, zmin, cma_label_to_name(gcamn->label),
