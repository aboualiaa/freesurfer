...
//			The traditional MRIS struct, containing pointer to vectors
//of VERTEX_TOPOLOGY, VERTEX and FACE 				This is fully implemented in the
//Freesurfer code 			The MRIS_MP struct, which contains pointers to vectors of some
//of the vertices and faces properties, but not all 			and which can get some, but
//not all, of the other properties from an underlying MRIS struct 				This is fully
//implemented in the Freesurfer code, being initialized from an MRIS struct 			The
//MRISPV struct, which which contains pointers to vectors of all of the vertices
//and faces properties 				This is not yet implemented in the code
//
// For each of these representations, we want to generate a cascade of
// Surface,Face,Vertex accessing classes that provide only limited access, so we
// can try to restrict phases of the Surface construction and manipulation to
// only parts of the whole data structure to force the data structures to be
// internally consistent by restricting who can modify it, and to try to
// establish a dependency order onto the properties.
//
// To achieve this, the following code is composed of several sections
//
//		Section 1: Utilities					Useful classes
//and functions that are mostly independent of the goal 		Section 2: Abstract
//representation		Classes etc. that describe the abstraction
//representation 												These are used to create a Representations - the complete list
//of properties etc.
//		Section 3: Generator utilities			Useful classes
//etc. that all the source generators share 		Section 4: Representation Generators
//One for each of the representations described above 		Section 5: Accessor
//Generators			One for each of the representations described
//above 		Section 6: main
//Invoke some of the generators 		Section 7: Representations
//Define the properties, what each representation supports, what each phase can
//access
...
// Section 2: Abstract	representation		Classes etc. that describe the
...
struct Prop : public PropModifiable {
  ...
  Prop *setCommentNature(CommentNature to) {
    commentNature = to;
    return this;
  }
  Prop *setNoHash() {
...
struct AtomicType;
struct PointerType;
struct ArrayOfPointerType;
struct PointerToRepeatedAtomicType;
struct Type {
  Id const id;
  Type(Id i) : id(i) {}
...
template <typename Callable0, typename Callable1, typename Callable2,
          typename Callable3>
void walkClasses(Representation &representation, Callable0 propHowToClassId,
                 Callable1 initClass, Callable2 nextMember,
                 Callable3 finiClass) {
...
// Section 3: Generator utilities			Useful classes etc. that all the
...
// Section 4: Representation Generators			One for each of the
...
struct Generate_Representation : public Generate_Base {
  Generate_Representation(ostream &os, Representation &representation)
      : Generate_Base(os, representation) {}
  ...
  void generateRepresentationClasses() {
    ...
    walkClasses(
        ...
        [&](Prop &prop, How *how, bool write) {
          if (write || !how)
            return;
          if (!prop.type && prop.commentNature != ComGeneral)
            return;
          ...
          if (prop.type) {
            ...
            cols << memberType << endC << Just_left << how->memberId(prop)
                 << endC << ";";
            comment = "  //";
            if (prop.repeatedSize) {
              cols << ignoreWidth << comment << " size() is "
                   << prop.repeatedSize->id;
              comment = ".  ";
            }
          }
          if (prop.comment.size())
...
// Section 5: Accessor Generators				One for each of the
// representations described above
//
...
struct Generate_Accessor : public Generate_Base {
  ...
  std::string absolutePhaseNamespace(Phase::T p) {
    return "SurfaceFrom" + representation.reprClassName + "_" +
           Phase::namespaceName(p);
  }

  virtual Generate_abstract_spec_Base *toGenerate_abstract_spec_Base() {
    return nullptr;
  }
  virtual Generate_abstract_inco_Base *toGenerate_abstract_inco_Base() {
    return nullptr;
  }

  virtual Generate_Surface_spec *toGenerate_SurfaceFromMRIS_spec() {
    return nullptr;
  }
  virtual Generate_Surface_impl *toGenerate_SurfaceFromMRIS_impl() {
    return nullptr;
  }
  virtual Generate_Surface_inco *toGenerate_SurfaceFromMRIS_inco() {
    return nullptr;
  }
    ...
    walkClasses(
        ...
        [&](Prop &prop, How *how, bool write) {
          ...
          if (currClassId == "Vertex" && !writers && write) {
            ...
            cols << "void";
            maybeEndC();
            cols << classMemberPrefix(currClassId) << maybeJust(Just_left)
                 << "which_coords";
            maybeEndC(false);
            cols << ignoreWidth
          ...
          bool canWrite =
              (prop.firstWritingPhase <= p && p <= prop.lastWritingPhase);
          if (p == Phase::AllM)
            canWrite = (prop.firstWritingPhase != Phase::end);

          if ((!write && !canRead) || (write && !canWrite)) {

            // Throw away general comments that precede variables that are
            // thrown away.
            //
            deferredComGeneral.clear();
          ...
          if (write)
            cols << maybeJust(Just_right) << how->setToArg(prop);
          maybeEndC(false);
          cols << ")";
          if (!write)
            cols << " const";
...
struct Generate_abstract_spec_Base : public Generate_Accessor {
  ...
  virtual void beginClass(Phase::T p, std::string const &classId) {
    ...
    ColumnedOutput::T cols;
    cols << "inline " << classId << endC << "(" << endC << "" << endC << ");"
         << endR;
    cols << "inline " << classId << endC << "(" << endC << "" << classId
         << " const & src" << endC << ");" << endR;
    cols << "inline " << classId << endC << "(" << endC
         << "Representation* representation"
         << (isSurface ? "" : ", size_t idx") << endC << ");" << endR;
    ...
    for (auto pLater = Phase::T(p + 1); pLater < Phase::end;
      ...
      if (isModifier && pLater != Phase::AllM)
        continue;
      cols << "inline " << classId << endC << "(" << endC << pnsLater
           << "::" << classId << " const & src" << endC << ");" << endR;
    }
  ...
  void generateVariousSurfaceMethods(Phase::T p, ColumnedOutput::T &cols) {
    if (p == Phase::XYZPositionM || p == Phase::DistortM ||
        p == Phase::DistortM) {
      cols << "void freeDistsButNotOrig() { MRISfreeDistsButNotOrig(repr); }"
           << endR;
    }
  }
...
struct Generate_abstract_impl_Base : public Generate_Accessor {
  ...
  virtual void beginClass(Phase::T p, std::string const &classId) {
    ...
    cols << classMemberPrefix(classId) << classId << endC << "(" << endC
         << "Representation* representation"
         << (isSurface ? "" : ", size_t idx") << endC
         << ") : Repr_Elt(representation," << (isSurface ? "0" : "idx")
         << ") {}" << endR;
    cols << classMemberPrefix(classId) << classId << endC << "(" << endC << ""
         << classId << " const & src" << endC << ") : Repr_Elt(src) {}" << endR;
    ...
    for (auto pLater = Phase::T(p + 1); pLater < Phase::end;
      ...
      if (isModifier && pLater != Phase::AllM)
        continue;
      cols << classMemberPrefix(classId) << classId << endC << "(" << endC
           << Phase::namespaceName(pLater) << "::" << classId << " const & src"
...
struct Generate_Surface_impl : public Generate_abstract_impl_Base {
  ...
  void generate_which_coords_MRIS(Phase::T p, std::string const &classId,
    ...
    cols << "MRISvertexCoord2XYZ_float(&repr->vertices[idx], which, x, y, z);"
         << endR;
    cols << ignoreWidth << Just_left << "}" << endR;
  }
  void generate_which_coords_other(Phase::T p, std::string const &classId,
    ...
    cols << "    *x = this->FIELD##x();  *y = this->FIELD##y();  *z = "
            "this->FIELD##z(); \\"
         << endR;
    cols << "    break;" << endR;
    cols << "" << endR;
    ...
    for (auto &propHow : representation.implements) {
      ...
      cols << ("    CASE(" + prop.which + "," +
               prop.id.substr(0, prop.id.size() - 1) + ")")
           << endR;
    }

    cols << "    default:" << endR;
    cols << "      *x = *y = *z = 0.0;" << endR;
    cols << "      ErrorExit(ERROR_UNSUPPORTED, \"which_coords: unsupported "
            "which %d\", which);"
         << endR;
    cols << "      break;" << endR;
    cols << "  }" << endR;
...
// Section 6: main							Invoke some of the
// generators
//
...
int main(int argc, const char **argv) {
  // Build the data structure that describes all about the surface, vertex,
  // face, etc.
  std::vector<Abstract_Representation::Representation *> representations;
  Representations::build(representations);

  // Walk the data structure to output the sources needed for a variety of
  // different purposes
  //
...
// Section 7: Representations						Define the
// properties, what each representation supports, what each phase can access
//
namespace Representations {
...
struct How_VT_f : public How_VT_indexed {
  ...
  virtual std::string getSetExpr(std::string const &reprNoArrow,
    ...
    return "cheapAssert(" + reprNoArrow + " == to.repr); " + reprNoArrow +
           "->vertices_topology[idx].f[i] = to.idx";
  }
};
...
struct How_VT_v : public How_VT_indexed {
  ...
  virtual std::string getSetExpr(std::string const &reprNoArrow,
    ...
    if (!write)
      return How_VT_indexed::getSetExpr(reprNoArrow, prop, write);
    return "cheapAssert(" + reprNoArrow + " == to.repr); " + reprNoArrow +
           "->vertices_topology[idx].v[i] = to.idx";
  }
};
...
struct How_MRIS : public HowDirect {
  How_MRIS() { this->m_secondaryName = "MRIS"; }
};
struct How_MRIS_hidden : public How_MRIS {
...
struct How_MRIS_vertex : public How_MRIS {
  ...
  virtual std::string getSetExpr(std::string const &reprNoArrow,
    ...
    return "cheapAssert(" + reprNoArrow + " == to.repr); " + reprNoArrow +
           "->" + prop.id + " = " + reprNoArrow + "->vertices + to.idx";
  }
};
...
struct HowPV_MRIS : public HowDirect {
  HowPV_MRIS() { this->m_secondaryName = ""; }
};
struct HowPV_MRIS_hidden : public HowPV_MRIS {
...
//		and an even smaller set are requested from an underlying MRIS or
//another MRIS_MP
//
...
static void doMRIS_MP(RepresentationX &rep_MRIS, RepresentationX &rep_MRISPV,
                      RepresentationX &rep_MRIS_MP);

static void build(std::vector<Representation *> &final_representations) {
  // Types are used to describe how properties are passed in (to Set), stored,
  // and returned
  //
  ...
  auto t_PR_VERTEX_TOPOLOGY = new PointerToRepeatedAtomicType(
      "pSeveralVERTEX_TOPOLOGY", t_VERTEX_TOPOLOGY);
  auto t_PR_FACE = new PointerToRepeatedAtomicType("pSeveralFACE", t_FACE);
  auto t_PR_MRI_EDGE =
      new PointerToRepeatedAtomicType("pSeveralMRI_EDGE", t_MRI_EDGE);
  auto t_PR_MRI_CORNER =
      new PointerToRepeatedAtomicType("pSeveralMRI_CORNER", t_MRI_CORNER);
  auto t_PR_FaceNormCacheEntry = new PointerToRepeatedAtomicType(
      "pSeveralFaceNormCacheEntry", t_FaceNormCacheEntry);
  auto t_PR_FaceNormDeferredEntry = new PointerToRepeatedAtomicType(
      "pSeveralFaceNormDeferredEntry", t_FaceNormDeferredEntry);
  auto t_PR_STRIP = new PointerToRepeatedAtomicType("pSeveralSTRIP", t_STRIP);

  // As the following is executed, the properties go into all the
  // representations currently in the set.
  //
  ...
  auto rep_MRISPV =
      new RepresentationX("mrisurf_MRIS_PropertiesInVectors.h", "MRISPV");
  ...
  auto makeProp = [&](Type *t, Id i, Phase::T rb, Phase::T wb, Phase::T we,
    ...
    for (auto &r : representations) {
      if (!r)
        continue;
      if (auto how = r->hows.back())
        r->implements.push_back(Representation::PropHow(added, how));
    };
    return added;
  };
  ...
  addPropCom("put the pointers before the ints, before the shorts, before "
             "uchars, to reduce size");
  addPropCom("the whole fits in much less than one cache line, so further "
             "ordering is no use");

  howPush(new How_VT_f, new HowPV_V_f);
  auto vtx_f =
      addProp(t_PR_int, "f",
              "array[v->num] the fno's of the neighboring faces         ");
  howMod(new How_VT_n, new HowPV_V_n);
  auto vtx_n =
      addProp(t_PR_uchar, "n",
              "array[v->num] the face.v[*] index for this vertex        ");
  howMod(new How_VT_e, new HowPV_V_e);
  addProp(t_PR_int, "e",
          "edge state for neighboring vertices                      ");
  howPop();
  ...
  auto vtx_v =
      addProp(t_PR_int, "v",
              "array[v->vtotal or more] of vno, head sorted by hops     ");
  howPop();
  auto vtx_vnum =
      addProp(t_short, "vnum",
              "number of 1-hop neighbors    should use [p]VERTEXvnum(i) ");

  addProp(t_short, "v2num",
          "number of 1, or 2-hop neighbors                          ");
  addProp(t_short, "v3num",
          "number of 1,2,or 3-hop neighbors                         ");
  auto vtx_vtotal =
      addProp(t_short, "vtotal",
              "total # of neighbors. copy of vnum.nsizeCur              ");

  addProp(t_short, "nsizeMaxClock",
          "copy of mris->nsizeMaxClock when v#num                   ")
      ->setNoHash();
  addProp(t_uchar, "nsizeMax",
          "the max nsize that was used to fill in vnum etc          ");
  addProp(t_uchar, "nsizeCur",
          "index of the current v#num in vtotal                     ");
  auto vtx_num =
      addProp(t_uchar, "num",
  ...
  auto vtx_dist = addProp(t_PR_float, "dist",
                          "distance to neighboring vertices based on  xyz   ");
  vtx_dist->setPRSize(vtx_vtotal);
  auto vtx_dist_orig =
      addProp(t_PR_float, "dist_orig",
              "distance to neighboring vertices based on origxyz");
  vtx_dist_orig->setPRSize(vtx_vtotal);
  howPop();

  addProp(t_int, "dist_capacity",
          "-- should contain at least vtx_vtotal elements   ");
  addProp(t_int, "dist_orig_capacity",
          "-- should contain at least vtx_vtotal elements   ");
  ...
  addProp(t_float, "origx",
          "original coordinates, see also MRIS::origxyz_status")
      ->setWhich("ORIGINAL_VERTICES");
  ...
  addProp(t_float, "fieldsign",
          "fieldsign--final: -1, \"0\", \"1\" (file: rh.fs) ");
  ...
  addProp(t_int, "annotation",
          "area label (defunct--now from label file name!) ");
  addProp(t_char, "oripflag");
  ...
  addProp(t_int, "nvertices",
          "# of vertices on surface, change by calling "
          "MRISreallocVerticesAndFaces et al");
  addProp(t_int, "nfaces",
          "# of faces on surface, change by calling "
          "MRISreallocVerticesAndFaces et al");
  addProp(t_bool, "faceAttachmentDeferred",
          "defer connecting faces to vertices for performance reasons");
  ...
  howPop();
  addProp(t_ppVoid, "dist_storage",
          "the malloced/realloced vertex dist fields, so those fields can be "
          "quickly nulled and restored")
      ->setNoHash();
  addProp(t_ppVoid, "dist_orig_storage",
          "the malloced/realloced vertex dist_orig fields, so those fields can "
          "be quickly nulled and restored")
      ->setNoHash();
  addProp(t_int, "tempsAssigned",
          "State of various temp fields that can be borrowed if not already in "
          "use");
  ...
  howMod(new How_MRIS_indexed_FaceNormCacheEntry,
         new HowPV_MRIS_indexed_FaceNormCacheEntry);
  addProp(t_PR_FaceNormCacheEntry, "faceNormCacheEntries");
  howMod(new How_MRIS_indexed_FaceNormDeferredEntry,
  ...
  addPropCom("v_temporal_pole, v_frontal_pole, and v_occipital_pole don't "
             "appear to be used, and are unusual being pointers to vertices");
  howPush(new How_MRIS_vertex, nullptr);
  addProp(t_PVERTEX,
          "v_temporal_pole"); // WEIRD THAT THESE ARE POINTERS TO VERTICES
  ...
  addProp(t_MRIS_Status, "status",
          ...
          " plane)");
  addProp(t_MRIS_Status, "origxyz_status",
          "type of surface (e.g. sphere, plane) that this origxyz were "
          "obtained from");
  ...
  addProp(t_short, "nsizeMaxClock",
          "changed whenever an edge is added or removed, which invalidates the "
          "vertex v#num values")
      ->setNoHash();
  addProp(t_char, "max_nsize",
          "max the neighborhood size has been set to (typically 3)");
  addProp(t_char, "dist_nsize",
          "max mrisComputeVertexDistances has computed distances out to");
  addProp(
      t_char, "dist_orig_nsize",
      "max mrisComputeOriginalVertexDistances has computed distances out to");
  addProp(t_char, "dist_alloced_flags",
          "two flags, set when any dist(1) or dist_orig(2) allocated");
  ...
  addProp(t_int, "max_faces",
          "may be bigger than nfaces,    set by calling "
          "MRISreallocVerticesAndFaces");
  ...
  addProp(t_int, "useRealRAS",
          "if 0 (default), vertex position is a conformed volume RAS with "
          "c_(r,\"a\",\"s\")=0.  "
          "else is a real RAS (volume stored RAS)");
  addProp(
      t_VOL_GEOM, "vg",
      "volume info from which this surface is created. valid iff vg.valid = 1");
  addProp(t_MRIS_cmdlines_t, "cmdlines")->setNoHash();
  addProp(t_int, "ncmds");
  addProp(t_float, "group_avg_surface_area",
          "average of total surface area for group");
  addProp(t_int, "group_avg_vtxarea_loaded",
          "average vertex area for group at each vertex");
  addProp(t_int, "triangle_links_removed", "for quad surfaces");
  addProp(t_pVoid, "user_parms", "for whatever the user wants to hang here")
      ->setNoHash();
  ...
  // rather than polluting the above list with the MRIS_MP information, that is
  // pulled into here...
  //
  auto rep_MRIS_MP =
      new RepresentationX("mrisurf_MRIS_MPPropertiesInVectors.h", "MRIS_MP");
...
static void doMRIS_MP(RepresentationX &rep_MRIS, RepresentationX &rep_MRISPV,
  ...
  implementationDetail("MRIS*", "underlyingMRIS",
                       "for properties that are read from the underlying MRIS");
  implementationDetail(
      "MRIS_MP*", "in_src",
      "since the in are not written, they can be shared by copies");
  implementationDetail("int", "in_ref_count", "check the src doesn't go away");
  implementationDetail("VERTEX_TOPOLOGY const *", "vertices_topology",
                       "pointer copied from MRIS");
  implementationDetail("FACE_TOPOLOGY   const *", "faces_topology",
                       "pointer copied from MRIS");
  ...
  for (size_t i = 0; i < rep_MRIS.implements.size(); i++) {
    ...
    case FromMRIS:
      rep_MRIS_MP.implements.push_back(
          Representation::PropHow(prop, new HowMP_fromMRIS(propHow.how)));
      break;
    case Stored: {
