...
def predict_segmentation(
    ...
    model_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "model_files"
    )
    if model_file is None:
        model_file = opj(model_dir, "flt1t2_psacnn_p" + str(patch_dim) + ".h5")

    loss = "dice_coef_loss2"
    curr_unet = DeepImageSynth.DeepImageSynth.from_file(
        model_file,
        loss,
        net="unet_2d_v1",
    ...
    label_image = curr_unet.predict_segmentation(
        test_files,
        test_channel_names,
        out_membership_file,
        out_hard_file,
...
class ROBEXInputSpec(ANTSCommandInputSpec):
    input_image = File(
        exists=True,
        desc="T1 image to be skull stripped",
        argstr="%s",
        position=0,
        mandatory=True,
    )
    stripped_image = File(
        desc="output file name of stripped image",
    ...
    mask_image = File(
        desc="output file name of binary mask",
        name_source=["input_image"],
        hash_files=False,
        keep_extension=True,
    ...
    seed = traits.Int(
        desc="seed value for non-deterministic functions",
        argstr="%d",
        position=3,
    )
...
class PSACNNInputSpec(BaseInterfaceInputSpec):
    ...
    input_image = File(
        exists=True,
        desc="intensity image to be segmented",
        argstr="-i %s",
        position=0,
        mandatory=True,
    )
    output_dir = Directory(
        exists=True,
        desc="output directory to store results",
        argstr="-o %s",
        position=1,
        mandatory=True,
    )
    contrast = traits.String(
        desc="contrast of input image. t1w or t2w",
        argstr="-c %s",
        position=2,
        mandatory=True,
    )
    # gpu = traits.Int(desc='seed value for non-deterministic functions', argstr='-gpu %d', position=3)
    patch_size = traits.Enum(
        96,
        64,
        desc="dimension of cube patch one of 64 or 96. Default 96",
        argstr="-p %d",
        position=4,
        usedefault=True,
    )

    save_label_image = traits.Bool(
        True,
        desc="saves label image",
        argstr="-ol",
        position=5,
        mandatory=False,
    )

    save_prob_image = traits.Bool(
        False,
        desc="saves probability map. can be 1.5GB",
        argstr="-os",
        position=6,
        mandatory=False,
    )
    ...
    batch_size = traits.Int(
        4,
        desc="batch size for GPU processing",
        argstr="-batch_size %d",
        position=8,
        usedefault=True,
    )
    ...
    label_list_file = File(
        os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "model_files",
            "aseg_labels_skullstripped.txt",
        ),
        exists=True,
        desc="label list file",
        argstr="-l %s",
        position=9,
        mandatory=False,
        usedefault=True,
    ...
    sample_rate = traits.Int(
        20000,
        desc="predict only for N/sample_rate number of patches",
        argstr="-sample_rate %d",
        position=10,
        usedefault=True,
    )
...
class PSACNN(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        label_image = predict_segmentation(
            self.inputs.input_image,
            self.inputs.output_dir,
            contrast=self.inputs.contrast,
            model_file=self.inputs.model_file,
            save_label_image=self.inputs.save_label_image,
            save_prob_image=self.inputs.save_prob_image,
            patch_dim=self.inputs.patch_size,
            batch_size=self.inputs.batch_size,
            label_list_file=self.inputs.label_list_file,
...
def psacnn_workflow(
    ...
    if use_preprocess == True:
        ...
        conform = Node(
            MRIConvert(
                conform=True, out_type="niigz", out_file="conformed.nii.gz"
            ),
            name="conform",
        )
        n4 = Node(
        ...
        robex = Node(
            ROBEX(seed=1729, stripped_image="brain.nii.gz"), name="robex"
        )
