...
parser.add_argument(
    "-p", "--patch-size", type=int, choices=[64, 96], default=96
)
...
for argname in ["input_file"]:
    setattr(
        args,
        argname,
        os.path.abspath(os.path.expanduser(getattr(args, argname))),
    )
    if not os.path.exists(getattr(args, argname)):
        raise RuntimeError(
            "%s: %s does not exist. Exiting."
            % (argname.replace("_", " ").title(), getattr(args, argname))
        )
