...
class MultiGPUCheckpointCallback(Callback):
    def __init__(
        self, output_prefix, save_per_epoch, save_weights, initial_epoch=1
    ):
...
def fetch_training_data_files(
    ...
    """
    src_subj_dir_list = sorted(
        glob.glob(os.path.join(fs_dir, subjects_dir, "[!fs]*", "mri"))
    )
    ...
    for src_dir in input_subj_dir_list:
        training_data_files.append(
            os.path.join(src_dir, img_input_type + ".mgz")
        )
...
class DeepImageSynth(object):
    def __init__(
        ...
        if net == "unet":
            ...
            self.model, self.parallel_model = unet_model_3d(
                feature_shape,
            ...
            self.unet_num_filters = unet_num_filters
            self.unet_depth = unet_depth
            self.model, self.parallel_model = unet_model(
                feature_shape,
                num_filters=unet_num_filters,
                unet_depth=unet_depth,
                downsize_filters_factor=unet_downsampling_factor,
            ...
            self.model, self.parallel_model = unet_model_2d(
                feature_shape,
                num_filters=unet_num_filters,
                unet_depth=unet_depth,
                downsize_filters_factor=unet_downsampling_factor,
            ...
            self.model, self.parallel_model = unet_2d_v1(
        ...
        self.feature_generator = FeatureGenerator(
            ...
            labels=labels,
            n_labels=n_labels,
            wmp_standardize=wmp_standardize,
            rob_standardize=rob_standardize,
            use_patches=use_patches,
            dim=dim,
    ...
    def from_file(
        ...
        if loss == "dice_coef_loss":
            ...
            model = load_model(
                model_filename,
                custom_objects={"dice_coef_loss2": dice_coef_loss2},
            )
        elif loss == "grad_loss":
            model = load_model(
                model_filename, custom_objects={"grad_loss": grad_loss}
            )
        else:
    ...
    def load_training_images(
        ...
        is_src_label_img,
        is_trg_label_img,
        source_seg_filenames=None,
        target_seg_filenames=None,
        step_size=None,
        preprocessing=False,
    ):
        if self.model is None:
        ...
        self.feature_generator.create_data_storage()
        self.feature_generator.generate_src_trg_training_data(
            source_filenames,
            target_filenames,
            is_src_label_img,
            is_trg_label_img,
    ...
    def load_training_images_dynamic(
        self,
        source_filenames,
        target_filenames,
        is_src_label_img,
        ...
        self.feature_generator.create_data_storage()
        self.feature_generator.generate_src_trg_training_collection(
            source_filenames,
            target_filenames,
            is_src_label_img,
            is_trg_label_img,
    ...
    def load_training_slices_and_labels(
        self, source_filenames, label_list, is_src_label_img
    ):
        if self.model is None:
            raise RuntimeError("Model does not exist")
    ...
    def load_validation_images(
        ...
        self.feature_generator.generate_src_trg_validation_data(
            source_filenames,
            target_filenames,
            is_src_label_img,
            is_trg_label_img,
    ...
    def load_validation_images_dynamic(
        self,
        source_filenames,
        target_filenames,
        is_src_label_img,
        ...
        self.feature_generator.generate_src_trg_validation_collection(
            source_filenames,
            target_filenames,
            is_src_label_img,
            is_trg_label_img,
    ...
    def load_validation_slices_and_labels(
        self, source_filenames, label_list, is_src_label_img
    ):
        if self.model is None:
            raise RuntimeError("Model does not exist")
    ...
    def load_training_validation_image_slices(
        self, source_filename, target_filename, step_size=None
    ):
        # for the oct mgz file
        self.feature_generator.create_data_storage()
        self.feature_generator.generate_src_trg_slice_training_validation(
            source_filename, target_filename, step_size=step_size
        )
    ...
    def train_network(
        ...
        print(
            'Beginning Training. Using %s backend with "%s" data format.'
            % (backend.backend(), backend.image_data_format())
        )
        ...
        if self.n_labels == 0:
            if self.num_gpus == 1:
                callback = DeepImageSynthCallback(
                    output_prefix, save_per_epoch, save_weights, initial_epoch
                )
                if len(self.out_channel_names) == 1:
                    ...
                    if out_channel_name == "t1beta":
                        ...
                        self.model.fit_generator(
                            generator=self.feature_generator.training_generator(
                                batch_size=batch_size * self.num_gpus
                            ),
                            epochs=epochs,
                ...
                callback = MultiGPUCheckpointCallback(
                    output_prefix, save_per_epoch, save_weights, initial_epoch
                )
            ...
            self.model.fit_generator(
                generator=self.feature_generator.training_label_generator(
                    batch_size=batch_size * self.num_gpus
                ),
                epochs=epochs,
    ...
    def synthesize_image(
        ...
        for iter_channel in range(num_channels):
            ...
            if iter_channel == 0:
                (
                    ...
                    in_indices,
                    padded_img_size,
                ) = self.feature_generator.extract_patches(
                    in_img_file,
        ...
        (
            out_img_data,
            count_img_data,
        ) = self.feature_generator.build_image_from_patches(
            out_patches, in_indices, padded_img_size, patch_crop_size, step_size
        )
    ...
    def predict_segmentation(
        ...
        for iter_channel in range(num_channels):
            ...
            if iter_channel == 0:
                ...
                if center_voxel == False:
                    ...
                    in_img_data = self.feature_generator.preprocess_image(
                        in_img_data, channel_name=channel_names[iter_channel]
                    )
                    in_fg_idxs = np.argwhere(in_img_data > 0)
                    (
                        in_patches,
                        in_indices_subsampled,
                        padded_img_size,
                    ) = self.feature_generator.extract_centered_patches(
        ...
        if self.num_gpus > 1:
            ...
            out_patches = self.model.predict(
                input_feature_array, batch_size=batch_size, verbose=1
            )
        ...
        label_img_data = self.feature_generator.build_seg_from_patches(
            out_patches,
            in_indices_subsampled,
        ...
        if save_label_image:
            label_img = nib.MGHImage(
                label_img_data, in_img.affine, in_img.header
            )
            nib.save(label_img, out_hard_filename)

        return label_img_data

    def predict_slice_segmentation(
        self,
        in_img_file_list,
        channel_names,
        orientation,
        ...
        for iter_channel in range(num_channels):
            ...
            in_img = nib.load(in_img_file)
            if iter_channel == 0:
                (in_slices, in_indices) = self.feature_generator.extract_slices(
                    in_img_file,
                    intensity_threshold=0,
                    indices=None,
                    is_label_img=False,
        ...
        label_slices = self.feature_generator.map_inv_labels(
            label_slices, self.labels
        )
    ...
    def predict_labels(self, in_img_file):
        in_img = nib.load(in_img_file)
        (in_slices, in_indices) = self.feature_generator.extract_slices(
            in_img_file, intensity_threshold=0, is_label_img=False, indices=None
        )
    ...
    def apply_decoder(self, in_patches, layer_name, step_size):
        decoder_model = Model(
            inputs=self.model.get_layer(layer_name), outputs=self.model.output
        )
        # in_img = nib.load(in_img_file)
        # print("Shape is " + str(in_img.get_data().shape))
        # (in_patches, in_indices, padded_img_size) = self.feature_generator.extract_patches(in_img_file, intensity_threshold=0,
...
class FeatureGenerator(object):
    def __init__(
        ...
        augment=True,
        nmr_augment=False,
        prob_rot_augmentation=0.25,
        prob_int_augmentation=0.5,
        num_outputs=1,
        num_input_channels=1,
        channel_names=["t1w"],
        out_channel_names=["trg"],
        input_normalization_key=["wmp"],
        output_normalization_key=None,
        use_tal=True,
        rare_label_list=None,
        use_slices=False,
        orientation=None,
    ):
        ...
        if self.augment == True:
            ...
            augment_cubic_transforms_dict = pickle.load(
                open("pfizer_data_cubic_fits.pkl", "rb")
            )
            self.augment_cubic_transforms = np.vstack(
                (
                    augment_cubic_transforms_dict["triomprage"],
        ...
        if self.nmr_augment == True:
            ...
            theta_flash1 = np.load(
                "/autofs/space/mreuter/users/amod/deep_learn/data/GE14/ge14_flash_parameters.npy"
            )
            ...
            theta_mprage0 = np.load(
                "/autofs/space/bhim_001/users/aj660/PSACNN/data/fsm_greve/fsmgreve_mprage_parameters.npy"
            )
            theta_mprage1 = np.load(
                "/autofs/space/mreuter/users/amod/deep_learn/data/ThreeScanners/threescanners_mprage_parameters.npy"
            )
            theta_mprage2 = np.load(
                "/autofs/space/mreuter/users/amod/deep_learn/data/Siemens13/siemens13_mprage_parameters.npy"
            )
            theta_mprage3 = np.load(
                "/autofs/space/bhim_001/users/aj660/PSACNN/data/UiO/triplescans/MPRAGE_TI850_NORM/freesurfer6p0_skullstripped/triplescans_mprage850_parameters.npy"
            )
            theta_mprage4 = np.load(
                "/autofs/space/bhim_001/users/aj660/PSACNN/data/UiO/triplescans/MPRAGE_TI1000_NORM/freesurfer6p0_skullstripped/triplescans_mprage1000_parameters.npy"
            )

            self.theta_mprage = np.vstack(
                (
                    theta_mprage0,
                    theta_mprage1,
                    theta_mprage2,
            ...
            theta_t2space = np.load(
                "/autofs/space/bhim_001/users/aj660/PSACNN/data/fsm_greve/fsmgreve_t2space_parameters.npy"
            )
    ...
    def create_data_storage(self):
        if self.storage_loc == "memory":
            self.data_storage = tables.open_file(
                "tmp_data_storage.h5",
                "w",
                driver="H5FD_CORE",
            ...
            tmp_fp = tempfile.NamedTemporaryFile(
                "w", suffix=".h5", dir=self.temp_folder, delete=False
            )
    ...
    def load_data_storage(self, filename):
        if self.storage_loc == "memory":
            self.data_storage = tables.open_file(
                filename, "r", driver="core", driver_core_backing_store=False
            )
        elif self.storage_loc == "disk":
    ...
    def generate_src_trg_training_collection(
        self,
        source_filenames,
        target_filenames,
        is_src_label_img,
        ...
        for ch_iter in range(num_target_channels):
            for subj_iter in range(num_subjects):
                ...
                trg_img_data = self.preprocess_image(
                    trg_img_data, self.out_channel_names[ch_iter]
                )
        ...
        for ch_iter in range(num_source_channels):
            ...
            for subj_iter in range(num_subjects):
                ...
                src_img_data = self.preprocess_image(
                    src_img_data, channel_name=curr_channel
                )
                self.src_image_dict[curr_channel].append(src_img_data)

    def generate_src_trg_validation_collection(
        self,
        source_filenames,
        target_filenames,
        is_src_label_img,
        ...
        for ch_iter in range(num_target_channels):
            ...
            for subj_iter in range(num_subjects):
                ...
                if self.use_slices == True:
                    ...
                    self.val_trg_slice_indices_dict[curr_channel].append(
                        slice_idxs
                    )
                    # print(trg_img_data.shape)
                    # print(slice_idxs.shape)
    ...
    def generate_src_trg_training_data(
        self,
        source_filenames,
        target_filenames,
        is_src_label_img,
        ...
        for iter_channel in range(self.num_input_channels):
            ...
            if self.use_patches == True:
                if iter_channel == 0:
                    (_, _, indices_list) = self.create_training_feature_array(
                        source_filenames_channel,
                    ...
                    (_, _, _) = self.create_training_feature_array(
                        source_filenames_channel,
                ...
                self.create_training_label_array(
                    target_label_list, "trg", indices_list
                )

        for iter_channel in range(len(self.out_channel_names)):
            target_filenames_channel = target_filenames[iter_channel]
            self.create_training_feature_array(
                target_filenames_channel,
                "trg" + self.out_channel_names[iter_channel],
                indices_list,
                step_size=step_size,
    ...
    def generate_src_trg_validation_data(
        self,
        source_filenames,
        target_filenames,
        is_src_label_img,
        ...
        for iter_channel in range(self.num_input_channels):
            ...
            if self.use_patches == True:
                ...
                self.create_training_label_array(
                    target_label_list, "valtrg", indices_list
                )

        print(
            "number of output channels is " + str(len(self.out_channel_names))
        )
        for iter_channel in range(len(self.out_channel_names)):
            target_filenames_channel = target_filenames[iter_channel]
            self.create_training_feature_array(
    ...
    def generate_src_trg_slice_training_validation(
        self, source_filename, target_filename, step_size=None
    ):
        ...
        feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "src",
        ...
        target_array = self.data_storage.create_earray(
            self.data_storage.root,
            "trg",
            tables.Atom.from_dtype(image_dtype),
        ...
        #
        #
        index_array = self.data_storage.create_earray(
            self.data_storage.root,
            "src" + "_index",
            tables.Int16Atom(),
            shape=(0, 2),
        ...
        val_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "src_validation",
            tables.Atom.from_dtype(image_dtype),
            shape=(0,) + self.feature_shape + (1,),
        ...
        val_target_array = self.data_storage.create_earray(
            self.data_storage.root,
            "trg_validation",
            tables.Atom.from_dtype(image_dtype),
            shape=(0,) + self.feature_shape + (self.num_outputs,),
        ...
        #
        #
        val_index_array = self.data_storage.create_earray(
            self.data_storage.root,
            "val" + "_index",
            tables.Int16Atom(),
            shape=(0, 2),
        ...
        (slices, slice_labels, indices) = self.extract_training_image_slices(
            source_filename,
            target_filename,
            intensity_threshold=0,
            step_size=step_size,
        ...
        (
            ...
            val_slice_labels,
            val_indices,
        ) = self.extract_training_image_slices(
            source_filename,
            target_filename,
            intensity_threshold=0,
            step_size=step_size,
    ...
    def add_rare_training_feature_array(self, rare_label_list):
        ...
        mapped_rare_label_list = self.map_labels(
            np.asarray(rare_label_list), self.labels
        )
        ...
        src_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srct1w_rare",
        ...
        srctalx_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srctalx_rare",
            tables.Atom.from_dtype(t1w_dtype),
            shape=(0,) + self.feature_shape[0:3] + (1,),
        ...
        srctaly_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srctaly_rare",
            tables.Atom.from_dtype(t1w_dtype),
            shape=(0,) + self.feature_shape[0:3] + (1,),
        ...
        srctalz_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srctalz_rare",
            tables.Atom.from_dtype(t1w_dtype),
            shape=(0,) + self.feature_shape[0:3] + (1,),
        ...
        srcnmrpd_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srcnmrpd_rare",
            tables.Atom.from_dtype(t1w_dtype),
            shape=(0,) + self.feature_shape[0:3] + (1,),
        ...
        srcnmrt1_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srcnmrt1_rare",
            tables.Atom.from_dtype(t1w_dtype),
            shape=(0,) + self.feature_shape[0:3] + (1,),
        ...
        srcnmrt2_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "srcnmrt2_rare",
            tables.Atom.from_dtype(t1w_dtype),
            shape=(0,) + self.feature_shape[0:3] + (1,),
        ...
        trg_feature_array = self.data_storage.create_earray(
            self.data_storage.root,
            "trgseg_rare",
        ...
        for start_idx in range(0, len(index_list), block_size):
            print(start_idx)
            t1patches = self.data_storage.root.srct1w[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]
            talxpatches = self.data_storage.root.srctalx[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]
            talypatches = self.data_storage.root.srctaly[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]
            talzpatches = self.data_storage.root.srctalz[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]

            pd_patches = self.data_storage.root.srcnmrpd[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]
            t1_patches = self.data_storage.root.srcnmrt1[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]
            t2_patches = self.data_storage.root.srcnmrt2[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]

            labelpatches = self.data_storage.root.trgseg[
                start_idx : np.minimum(start_idx + block_size, len(index_list))
            ]
    ...
    def create_rare_training_feature_array(
        self,
        src_filenames,
        src_array_name,
        trg_filenames,
        ...
        if self.use_patches == True:
            src_index_array = self.data_storage.create_earray(
                self.data_storage.root,
                src_array_name + "_index",
                tables.Int16Atom(),
                shape=(0, 3),
                expectedrows=np.prod(nb_features_per_subject) * nb_subjects,
            )

            trg_index_array = self.data_storage.create_earray(
                self.data_storage.root,
                trg_array_name + "_index",
                tables.Int16Atom(),
                shape=(0, 3),
                expectedrows=np.prod(nb_features_per_subject) * nb_subjects,
            )
            ...
            for src_file, trg_file in zip(src_filenames, trg_filenames):
                (trg_features, trg_indices) = self.extract_training_patches(
                    trg_file,
                    intensity_threshold=0,
                    step_size=step_size,
                    indices=None,
                ...
                (src_features, indices) = self.extract_training_patches(
                    src_file,
                    intensity_threshold=0,
                    step_size=step_size,
                    indices=rare_trg_indices,
                    is_label_img=is_src_label_img,
                )
    ...
    def preprocess_image(self, in_img_data, channel_name):
        ...
        pad_width = (
            (self.feature_shape[0], self.feature_shape[0]),
        ...
        if "seg" in channel_name:
            ...
            if "t1w" in channel_name:
                ...
                # else:
                in_img_data = intensity_standardize_utils.wm_peak_normalize_t2w(
                    in_img_data
                )
    ...
    def create_training_image_array(
        self, image_filenames, array_name, indices_list, step_size, is_label_img
    ):
        nb_features_per_subject = 1000000
        ...
        if self.use_patches == True:
            ...
            if indices_list == None:
                ...
                for input_file in image_filenames:
                    (features, indices) = self.extract_training_patches(
                        input_file,
                        intensity_threshold=0,
                        step_size=step_size,
                        indices=None,
                ...
                for input_file, curr_indices in zip(
                    image_filenames, indices_list
                ):
            ...
            if indices_list == None:
                ...
                if self.dim == 2:
                    for input_file in image_filenames:
                        (features, indices) = self.extract_training_slices(
                            input_file,
                            intensity_threshold=0,
                            indices=None,
                            is_label_img=is_label_img,
    ...
    def create_training_feature_array(
        self, image_filenames, array_name, indices_list, step_size, is_label_img
    ):
        nb_features_per_subject = 1000000
        nb_subjects = len(image_filenames)
    ...
    def create_training_label_array(
        self, target_label_list, array_name, indices_list
    ):
        nb_features_per_subject = 200
        nb_subjects = len(target_label_list)

        label_array = self.data_storage.create_earray(
            self.data_storage.root,
            array_name,
            tables.Int16Atom(),
            shape=(0, 1),
    ...
    def extract_training_slices(
        self, in_img_file, intensity_threshold, indices, is_label_img
    ):

        if indices is not None:
            (slices, indices) = self.extract_slices(
                in_img_file,
                intensity_threshold,
                is_label_img=is_label_img,
                indices=indices,
    ...
    def extract_training_patches(
        ...
        if indices is not None:
            (patches, indices, _) = self.extract_patches(
                in_img_file,
                intensity_threshold,
                step_size,
                is_label_img=is_label_img,
    ...
    def extract_centered_patches(
        self, in_img_data, in_indices, sampling_rate=1000
    ):
        ...
        for index in in_indices_subsampled:
            ...
            patch = in_img_data[
                int(px)
                - self.feature_shape[0] // 2 : int(px)
                + self.feature_shape[0] // 2,
                int(py)
                - self.feature_shape[1] // 2 : int(py)
                + self.feature_shape[1] // 2,
                int(pz)
                - self.feature_shape[2] // 2 : int(pz)
                + self.feature_shape[2] // 2,
            ]
    ...
    def extract_patches(
        ...
        padding0 = (
            self.feature_shape[0] + step_size[0] + 1,
            self.feature_shape[0] + step_size[0] + 1,
        )
        padding1 = (
            self.feature_shape[1] + step_size[1] + 1,
            self.feature_shape[1] + step_size[1] + 1,
        )
        padding2 = (
            self.feature_shape[2] + step_size[2] + 1,
            self.feature_shape[2] + step_size[2] + 1,
        )

        in_img_data_pad = np.pad(
            in_img_data,
        ...
        if indices is not None:
            ...
            (idx_x_fg, idx_y_fg, idx_z_fg) = np.where(
                in_img_data_pad > intensity_threshold
            )
            ...
            idx_x, idx_y, idx_z = np.meshgrid(
                sampled_x, sampled_y, sampled_z, sparse=False, indexing="ij"
            )
            idx_x = idx_x.flatten()
            idx_y = idx_y.flatten()
            idx_z = idx_z.flatten()
        ...
        for patch_iter in range(len(idx_x)):
            curr_patch = in_img_data_pad[
                idx_x[patch_iter] : idx_x[patch_iter] + self.feature_shape[0],
                idx_y[patch_iter] : idx_y[patch_iter] + self.feature_shape[1],
    ...
    def build_image_from_patches(
        self, in_patches, indices, padded_img_size, patch_crop_size, step_size
    ):
        ...
        patch_mask[
            0 + patch_crop_size[0] : out_feature_shape[0] - patch_crop_size[0],
            0 + patch_crop_size[1] : out_feature_shape[1] - patch_crop_size[1],
            0 + patch_crop_size[2] : out_feature_shape[2] - patch_crop_size[2],
        ] = 1

        for patch_iter in range(len(idx_x)):
            out_img_data[
                idx_x[patch_iter] : idx_x[patch_iter] + out_feature_shape[0],
        ...
        # remove the padding
        unpadded_img_size = padded_img_size - np.multiply(
            np.asarray(self.feature_shape[:-1]) + step_size + 1, 2
        )
    ...
    def build_seg_from_patches(
        ...
        if center_voxel == False:
            ...
            patch_mask[
                0
                + patch_crop_size[0] : out_feature_shape[0]
                - patch_crop_size[0],
                0
                + patch_crop_size[1] : out_feature_shape[1]
                - patch_crop_size[1],
                0
                + patch_crop_size[2] : out_feature_shape[2]
                - patch_crop_size[2],
                :,
            ] = 1
            ...
            for patch_iter in range(len(idx_x)):
                out_img_data[
                    idx_x[patch_iter] : idx_x[patch_iter]
                    + out_feature_shape[0],
                    idx_y[patch_iter] : idx_y[patch_iter]
                    + out_feature_shape[1],
                    idx_z[patch_iter] : idx_z[patch_iter]
                    + out_feature_shape[2],
                    :,
                ] += np.multiply(
                    np.reshape(in_patches[patch_iter, :], out_feature_shape),
            ...
            unpadded_img_size = padded_img_size[0:3] - np.multiply(
                np.asarray(self.feature_shape[:-1]) + step_size + 1, 2
            )
            padding = np.asarray(self.feature_shape[:-1]) + step_size + 1
            ...
            unpadded_img_size = padded_img_size[0:3] - np.multiply(
                np.asarray(self.feature_shape[:-1]), 2
            )
    ...
    def extract_training_image_slices(
        self,
        source_filename,
        target_filename,
        intensity_threshold=0,
        ...
        padding2 = (0, 0)
        img_data_pad = np.pad(
            img_data, (padding0, padding1), "constant", constant_values=0
        )
        vec_data_pad = np.pad(
            vec_data,
            (padding0, padding1, padding2),
        ...
        (idx_x_fg, idx_y_fg, idx_z_fg) = np.where(
            vec_data_pad > intensity_threshold
        )
        min_idx_x_fg = np.min(idx_x_fg) - step_size[0]
        max_idx_x_fg = np.max(idx_x_fg) + step_size[0]
        min_idx_y_fg = np.min(idx_y_fg) - step_size[1]
        ...
        idx_x, idx_y = np.meshgrid(
            sampled_x, sampled_y, sparse=False, indexing="ij"
        )
        idx_x = idx_x.flatten()
        idx_y = idx_y.flatten()
        # idx_z = idx_z.flatten()
        ...
        if dataset == "train":
            ...
            iterrange = range(
                (len(idx_x) // 3 + 1), (len(idx_x) // 3) + (len(idx_x) // 5)
            )

        print("iter range is " + str(iterrange[0]) + "_" + str(iterrange[-1]))

        for patch_iter in iterrange:
            curr_patch = img_data_pad[
                idx_x[patch_iter] : idx_x[patch_iter] + self.feature_shape[0],
                idx_y[patch_iter] : idx_y[patch_iter] + self.feature_shape[1],
            ]
            vec_patch = vec_data_pad[
                idx_x[patch_iter] : idx_x[patch_iter] + self.feature_shape[0],
                idx_y[patch_iter] : idx_y[patch_iter] + self.feature_shape[1],
                0:2,
            ]
        ...
        while True:
            ...
            for subj_idx in subj_idxs:
                p_idxs = random.sample(
                    range(
                        len(
                            self.trg_fg_indices_dict[trg_channel_name][subj_idx]
                        )
                    ),
                    1,
                )
                for p_idx in p_idxs:
                    [px, py, pz] = self.trg_fg_indices_dict[trg_channel_name][
                        subj_idx
                    ][p_idx]
            ...
            synth_flash_idxs = range(
                num_orig_mprage, num_orig_mprage + num_synth_flash
            )
            synth_t2space_idxs = range(
                num_orig_mprage + num_synth_flash,
                num_orig_mprage + num_synth_flash + num_synth_t2space,
            )
            synth_mprage_idxs = range(
                num_orig_mprage + num_synth_flash + num_synth_t2space,
                batch_size,
            )
            ...
            x, y = self.convert_data(
                x_array, y_array, self.n_labels, self.labels
            )
    ...
    def dynamic_seg_validation_generator_singlechannel(self, batch_size):
        while True:
            ...
            y_list = list()
            trg_channel_name = self.out_channel_names[0]
            img_index_list = range(
                len(self.val_trg_image_dict[trg_channel_name])
            )
            #   list(range(self.data_storage.root.trgseg.shape[0]))
            ...
            for subj_idx in subj_idxs:
                p_idxs = random.sample(
                    range(
                        len(
                            self.val_trg_fg_indices_dict[trg_channel_name][
                                subj_idx
                            ]
                        )
                    ),
                ...
                for p_idx in p_idxs:
                    [px, py, pz] = self.val_trg_fg_indices_dict[
                        trg_channel_name
                    ][subj_idx][p_idx]
                    batch_index_list.append([subj_idx, px, py, pz])
    ...
    def dynamic_seg_training_generator_singlechannel_nmr_slice(
        self, batch_size, focus="ALL"
    ):
        while True:
            x_list = list()
            y_list = list()
            ...
            for subj_idx in subj_idxs:
                p_idxs = random.sample(
                    range(
                        len(
                            self.trg_slice_indices_dict[trg_channel_name][
                                subj_idx
                            ]
                        )
                    ),
                ...
                for p_idx in p_idxs:
                    slice_idx = self.trg_slice_indices_dict[trg_channel_name][
                        subj_idx
                    ][p_idx]
            ...
            for b_iter in range(orig_mprage_batch_idxs.shape[0]):
                ...
                xfeat = np.stack(
                    (x_t1w_patch, x_mod_patch, x_mod_patch1), axis=-1
                )
            ...
            for b_iter in range(synth_flash_batch_idxs.shape[0]):
                ...
                x_flash_patch = self.apply_flash(
                    curr_theta_flash,
                    x_nmrpd_patch,
                    x_nmrt1_patch,
                    x_nmrt2_patch,
                ...
                xfeat = np.stack(
                    (x_flash_patch, x_mod_patch, x_mod_patch1), axis=-1
                )
            ...
            for b_iter in range(synth_t2space_batch_idxs.shape[0]):
                ...
                x_t2space_patch = self.apply_t2space(
                    curr_t2space_theta,
                    x_nmrpd_patch,
                    x_nmrt1_patch,
                    x_nmrt2_patch,
                ...
                xfeat = np.stack(
                    (x_t2space_patch, x_mod_patch, x_mod_patch1), axis=-1
                )
            ...
            for b_iter in range(synth_mprage_batch_idxs.shape[0]):
                ...
                x_mprage_patch = self.apply_mprage(
                    curr_mprage_theta,
                    x_nmrpd_patch,
                    x_nmrt1_patch,
                    x_nmrt2_patch,
                ...
                xfeat = np.stack(
                    (x_mprage_patch, x_mod_patch, x_mod_patch1), axis=-1
                )
    ...
    def dynamic_seg_validation_generator_singlechannel_nmr_slice(
        self, batch_size, focus="ALL"
    ):
        while True:
            x_list = list()
            ...
            for subj_idx in subj_idxs:
                p_idxs = random.sample(
                    range(
                        len(
                            self.val_trg_slice_indices_dict[trg_channel_name][
                                subj_idx
                            ]
                        )
                    ),
                ...
                for p_idx in p_idxs:
                    slice_idx = self.val_trg_slice_indices_dict[
                        trg_channel_name
                    ][subj_idx][p_idx]
                    batch_index_list.append([subj_idx, slice_idx])
            ...
            for b_iter in range(synth_flash_batch_idxs.shape[0]):
                ...
                x_nmrpd_patch = self.val_src_image_dict["nmrpd"][subj_iter][
                    :, :, slice_idx
                ]

                x_nmrt1_patch = self.val_src_image_dict["nmrt1"][subj_iter][
                    :, :, slice_idx
                ]

                x_nmrt2_patch = self.val_src_image_dict["nmrt2"][subj_iter][
                    :, :, slice_idx
                ]
    ...
    def dynamic_seg_training_generator_singlechannel_nmr(
        self, batch_size, focus="ALL"
    ):
        while True:
            x_list = list()
    ...
    def dynamic_seg_training_generator_multichannel_nmr_t2(
        self, batch_size, focus="ALL"
    ):
        ...
        while True:
            ...
            for b_iter in range(orig_mprage_batch_idxs.shape[0]):
                ...
                xfeat = np.stack(
                    (x_t1w_patch, x_talx_patch, x_taly_patch, x_talz_patch),
                    axis=-1,
                )
                y_patch = self.trg_image_dict["seg"][subj_iter][
                    int(px)
            ...
            for b_iter in range(synth_flash_batch_idxs.shape[0]):
                ...
                xfeat = np.stack(
                    (x_flash_patch, x_talx_patch, x_taly_patch, x_talz_patch),
                    axis=-1,
                )
                y_patch = self.trg_image_dict["seg"][subj_iter][
                    int(px)
            ...
            for b_iter in range(synth_t2space_batch_idxs.shape[0]):
                ...
                xfeat = np.stack(
                    (x_t2space_patch, x_talx_patch, x_taly_patch, x_talz_patch),
                    axis=-1,
                )
                y_patch = self.trg_image_dict["seg"][subj_iter][
                    int(px)
            ...
            for b_iter in range(synth_mprage_batch_idxs.shape[0]):
                ...
                xfeat = np.stack(
                    (x_mprage_patch, x_talx_patch, x_taly_patch, x_talz_patch),
                    axis=-1,
                )
                y_patch = self.trg_image_dict["seg"][subj_iter][
                    int(px)
    ...
    def validation_generator_t1beta(self, batch_size):
        ...
        while True:
            ...
            xfeat = np.stack(
                (
                    self.data_storage.root.val_srct1w[
                        index_list[0:batch_size], :
                    ],
                    self.data_storage.root.val_srctalx[
                        index_list[0:batch_size], :
                    ],
                    self.data_storage.root.val_srctaly[
                        index_list[0:batch_size], :
                    ],
                    self.data_storage.root.val_srctalz[
                        index_list[0:batch_size], :
                    ],
                ),
                axis=-2,
            ...
            xfeat = xfeat.reshape(xfeat.shape[:-1])
            y = self.data_storage.root.val_trgt1beta[
                index_list[0:batch_size], :
            ]
            x = xfeat
    ...
    def validation_generator_t2beta(self, batch_size):
        ...
        while True:
            ...
            y = self.data_storage.root.val_trgt2beta[
                index_list[0:batch_size], :
            ]
    ...
    def validation_generator_pdbeta(self, batch_size):
        ...
        while True:
            ...
            y = self.data_storage.root.val_trgpdbeta[
                index_list[0:batch_size], :
            ]
    ...
    def map_inv_labels(self, input_label_patch, input_label_list):
        ...
        for curr_label in range(len(input_label_list)):
            output_label_patch[
                input_label_patch == curr_label
            ] = input_label_list[curr_label]
    ...
    def get_multi_class_labels(self, data, n_labels):
        ...
        new_shape = list(data.shape[0:-1]) + [
            n_labels
        ]  # [data.shape[0], n_labels] + list(data.shape[2:])
        y = np.zeros(new_shape, np.int8)
        if len(data.shape) == 4:  # 2D data
            for label_index in range(n_labels):
                if labels is not None:
                    y[:, :, :, label_index][
                        data[:, :, :, 0] == labels[label_index]
                    ] = 1
                else:
                    y[:, :, :, label_index][
                        data[:, :, :, 0] == (label_index + 1)
                    ] = 1
            return y
    ...
    def seg_training_generator(self, batch_size):
        ...
        while True:
            ...
            y = np.asarray(y_list)
            x, y = self.convert_data(
                x, y, n_labels=self.n_labels, labels=self.labels
            )
    ...
    def apply_flash(self, theta_flash, pd_img_data, t1_img_data, t2_img_data):
        flash_fg = np.exp(
            np.log(pd_img_data[pd_img_data > 0])
            + theta_flash[0]
        ...
        flash_img_data = gaussian_filter(
            flash_img_data, sigma=0.5 * np.ones((dim, 1))
        )
    ...
    def apply_mprage(self, theta_mprage, batch_pd, batch_t1, batch_t2):
        ...
        batch_mprage[batch_pd > 0] = np.exp(
            np.log(batch_pd[batch_pd > 0])
            + theta_mprage[0]
    ...
    def apply_t2space(
        self, theta_t2space, pd_img_data, t1_img_data, t2_img_data
    ):
        fg_idxs = (pd_img_data > 0) & (t1_img_data > 0) & (t2_img_data > 0)
        t2space_fg = (
            np.log(pd_img_data[fg_idxs])
            + theta_t2space[0]
    ...
    def seg_training_generator_multichannel_nmr(self, batch_size):
        index_list = list(range(self.data_storage.root.srct1w.shape[0]))
        index_list_rare = list(
            range(self.data_storage.root.srct1w_rare.shape[0])
        )
        ...
        while True:
            ...
            # t2 = time.time()
            x_t1w = self.data_storage.root.srct1w[
                index_list[0:num_ubiq_patches], :
            ]
            x_talx = self.data_storage.root.srctalx[
                index_list[0:num_ubiq_patches], :
            ]
            x_taly = self.data_storage.root.srctaly[
                index_list[0:num_ubiq_patches], :
            ]
            x_talz = self.data_storage.root.srctalz[
                index_list[0:num_ubiq_patches], :
            ]

            x_pd = self.data_storage.root.srcnmrpd[
                index_list[0:num_ubiq_patches], :
            ]
            ...
            xfeat = np.stack(
                (
                    x_t1w[0:num_orig_patches, :],
                    x_talx[0:num_orig_patches, :],
                    x_taly[0:num_orig_patches, :],
                    x_talz[0:num_orig_patches, :],
                ),
                axis=-2,
            )
            xfeat = xfeat.reshape(xfeat.shape[:-1])

            x_t1w_rare = self.data_storage.root.srct1w_rare[
                index_list_rare[0:num_rare_patches], :
            ]
            x_talx_rare = self.data_storage.root.srctalx_rare[
                index_list_rare[0:num_rare_patches], :
            ]
            x_taly_rare = self.data_storage.root.srctaly_rare[
                index_list_rare[0:num_rare_patches], :
            ]
            x_talz_rare = self.data_storage.root.srctalz_rare[
                index_list_rare[0:num_rare_patches], :
            ]

            x_pd_rare = self.data_storage.root.srcnmrpd_rare[
                index_list_rare[0:num_rare_patches], :
            ]
            ...
            y_rare = self.data_storage.root.trgseg_rare[
                index_list_rare[0:num_rare_patches], :
            ]

            xfeat_rare = np.stack(
                (
                    x_t1w_rare[0:num_orig_patches, :],
                    x_talx_rare[0:num_orig_patches, :],
                    x_taly_rare[0:num_orig_patches, :],
                    x_talz_rare[0:num_orig_patches, :],
                ),
                axis=-2,
            )
            ...
            # t3 = time.time()
            xflash_pd = x_pd[
                num_orig_patches : num_orig_patches + num_flash_patches, :
            ]
            xflash_t1 = x_t1[
                num_orig_patches : num_orig_patches + num_flash_patches, :
            ]
            ...
            xflash_pd_rare = x_pd_rare[
                num_orig_patches : num_orig_patches + num_flash_patches, :
            ]
            xflash_t1_rare = x_t1_rare[
                num_orig_patches : num_orig_patches + num_flash_patches, :
            ]
            xflash_t2_rare = x_t2_rare[
                num_orig_patches : num_orig_patches + num_flash_patches, :
            ]
            ...
            xflash_rare = self.apply_flash(
                curr_theta_flash, xflash_pd_rare, xflash_t1_rare, xflash_t2_rare
            )
            xflash_feat_rare = np.stack(
                (
                    xflash_rare,
                    x_talx_rare[
                        num_orig_patches : num_orig_patches + num_flash_patches
            ...
            xflash_feat_rare = xflash_feat_rare.reshape(
                xflash_feat_rare.shape[:-1]
            )
            ...
            # t4 = time.time()
            xmprage_pd = x_pd[
                num_orig_patches + num_flash_patches : num_ubiq_patches, :
            ]
            xmprage_t1 = x_t1[
                num_orig_patches + num_flash_patches : num_ubiq_patches, :
            ]
            ...
            xmprage = self.apply_mprage(
                curr_mprage_theta, xmprage_pd, xmprage_t1, xmprage_t2
            )

            xmprage_feat = np.stack(
                (
                    xmprage,
                    x_talx[
                        num_orig_patches + num_flash_patches : num_ubiq_patches,
            ...
            xmprage_pd_rare = x_pd_rare[
                num_orig_patches + num_flash_patches : num_rare_patches, :
            ]
            xmprage_t1_rare = x_t1_rare[
                num_orig_patches + num_flash_patches : num_rare_patches, :
            ]
            xmprage_t2_rare = x_t2_rare[
                num_orig_patches + num_flash_patches : num_rare_patches, :
            ]
            ...
            xmprage_rare = self.apply_mprage(
                curr_mprage_theta,
                xmprage_pd_rare,
                xmprage_t1_rare,
                xmprage_t2_rare,
            ...
            #
            xmprage_feat_rare = xmprage_feat_rare.reshape(
                xmprage_feat_rare.shape[:-1]
            )
            ...
            x, y = self.convert_data(
                xallfeat, yall, n_labels=self.n_labels, labels=self.labels
            )
    ...
    def seg_training_generator_multichannel_nmr_t2(self, batch_size):
        ...
        num_t2_patches = batch_size - (
            num_orig_patches + num_flash_patches + num_mprage_patches
        )
        ...
        while True:
            ...
            xmprage_pd = x_pd[
                num_orig_patches
                + num_flash_patches : num_orig_patches
                + num_flash_patches
                + num_mprage_patches,
                :,
            ]
            ...
            xt2space_pd = x_pd[
                num_orig_patches
                + num_flash_patches
                + num_mprage_patches : num_ubiq_patches,
                :,
            ]
            xt2space_t1 = x_t1[
                num_orig_patches
            ...
            xt2space = self.apply_t2space(
                curr_t2space_theta, xt2space_pd, xt2space_t1, xt2space_t2
            )
            # print(xt2space.shape)

            xt2space_feat = np.stack(
                (
                    xt2space,
                    x_talx[
                        num_orig_patches
            ...
            xallfeat = np.concatenate(
                (xfeat, xflash_feat, xmprage_feat, xt2space_feat)
            )
            # xallfeat = np.concatenate(
    ...
    def seg_training_generator_multichannel_nmr_finetune(self, batch_size):
        ...
        while True:
            ...
            x_t1 = self.data_storage.root.srcnmrt1[index_list[0:batch_size], :]
            x_t2 = self.data_storage.root.srcnmrt2[index_list[0:batch_size], :]
            y = self.data_storage.root.trgseg[index_list[0:batch_size], :]
    ...
    def seg_training_generator_augment(self, batch_size):
        ...
        while True:
            ...
            num_int_patches = int(batch_size * self.prob_int_augmentation)
            int_idxs = np.random.choice(
                batch_size, size=(num_int_patches,), replace=False
            )
            # print(int_idxs)
            for i_idx in int_idxs:
                intx = self.perturb_intensity(x[i_idx, :, :, :, 0])
    ...
    def seg_training_generator_singlechannel_nmr(self, batch_size):
        ...
        while True:
            ...
            # t4 = time.time()
            xmprage_pd = x_pd[
                num_orig_patches + num_flash_patches : batch_size, :
            ]
            xmprage_t1 = x_t1[
                num_orig_patches + num_flash_patches : batch_size, :
            ]
    ...
    def perturb_intensity(self, patch):
        ...
        hist_idx = np.random.randint(
            0, self.augment_cubic_transforms.shape[0] - 1, 1
        )[0]
...
if __name__ == "__main__":
    ...
    adp5_idxs = sorted(
        np.random.choice(len(adp5_ids), size=(3,), replace=False)
    )
    ...
    training_id_list = (
        list(young_training_ids[:-1])
        + list(mid_training_ids[:-1])
        + list(old_training_ids[:-1])
        + list(ad1_training_ids)
        + list(adp5_training_ids)
    )

    validation_id_list = (
        [young_training_ids[-1]]
        + [mid_training_ids[-1]]
        + [(old_training_ids[-1])]
    )
    ...
    src_filenames = fetch_training_data_files(
        subjects_dir, src_img_input_type, training_id_list
    )
    all_src_filenames.append(src_filenames)
    src_filenames_cha1 = fetch_training_data_files(
        subjects_dir, "xind", training_id_list
    )
    all_src_filenames.append(src_filenames_cha1)
    src_filenames_cha2 = fetch_training_data_files(
        subjects_dir, "yind", training_id_list
    )
    all_src_filenames.append(src_filenames_cha2)
    src_filenames_cha3 = fetch_training_data_files(
        subjects_dir, "zind", training_id_list
    )
    ...
    src_filenames_pd = fetch_result_data_files(
        pd_subjects_dir,
        "synth_pd_grad1e4_in_unet_depth_3_filts_32",
        subject_id_list,
        prefix="",
        extension=".mgz",
    )
    ...
    src_filenames_t1 = fetch_result_data_files(
        t1_subjects_dir,
        "synth_t1_grad1e4_in_unet_depth_3_filts_32",
        subject_id_list,
        prefix="",
        extension=".mgz",
    )
    ...
    src_filenames_t2 = fetch_result_data_files(
        t2_subjects_dir,
        "synth_t2_grad1e4_in_unet_depth_3_filts_32",
        subject_id_list,
        prefix="",
        extension=".mgz",
    )
    ...
    trg_filenames = fetch_training_data_files(
        subjects_dir, trg_img_input_type, training_id_list
    )
    ...
    val_src_filenames = fetch_training_data_files(
        subjects_dir, src_img_input_type, validation_id_list
    )
    all_val_src_filenames.append(val_src_filenames)

    val_src_filenames_cha1 = fetch_training_data_files(
        subjects_dir, "xind", validation_id_list
    )
    all_val_src_filenames.append(val_src_filenames_cha1)

    val_src_filenames_cha2 = fetch_training_data_files(
        subjects_dir, "yind", validation_id_list
    )
    all_val_src_filenames.append(val_src_filenames_cha2)

    val_src_filenames_cha3 = fetch_training_data_files(
        subjects_dir, "zind", validation_id_list
    )
    all_val_src_filenames.append(val_src_filenames_cha3)

    val_src_filenames_pd = fetch_result_data_files(
        pd_subjects_dir,
        "synth_pd_grad1e4_in_unet_depth_3_filts_32",
        validation_id_list,
        prefix="",
    ...
    val_src_filenames_t1 = fetch_result_data_files(
        t1_subjects_dir,
        "synth_t1_grad1e4_in_unet_depth_3_filts_32",
        validation_id_list,
        prefix="",
    ...
    val_src_filenames_t2 = fetch_result_data_files(
        t2_subjects_dir,
        "synth_t2_grad1e4_in_unet_depth_3_filts_32",
        validation_id_list,
        prefix="",
    ...
    val_trg_filenames = fetch_training_data_files(
        subjects_dir, trg_img_input_type, validation_id_list
    )
    ...
    output_dir = (
        "/autofs/space/bhim_001/users/aj660/PSACNN/results/TEST_skullstripped_aseg_atlas_unet_int_tal_rare_nmrsynth_t2space_depth_"
        + str(d)
        + "_filts_"
        + str(f)
        + "_patch"
    ...
    curr_unet.feature_generator.generate_src_trg_training_collection(
        source_filenames=all_src_filenames,
    ...
    curr_unet.feature_generator.generate_src_trg_validation_collection(
        source_filenames=all_val_src_filenames,
    ...
    (
        x,
        y,
    ) = curr_unet.feature_generator.dynamic_seg_training_generator_multichannel_nmr_t2(
        32
    ).next()
    (
        val_x,
        val_y,
    ) = curr_unet.feature_generator.dynamic_seg_validation_generator_multichannel_nmr_t2(
        32
    ).next()
