 ...
 * @brief register the resting state patterns from one label map to another
 * based on priors
 *
...
static int  powell_minimize(VECTOR **v_D, MATRIX **m_I, int nsubjects,
                            MRI *mri_mask, VECTOR *v_weights);
static MRI *compute_voxlist_surface_correlations(VOXEL_LIST *vl, int num_maps,
                                                 MRI *mri_fvol, MRI *mri_fsurf,
                                                 MRI *mri_cmat);
static MRI *compute_voxlist_surface_correlations_across_runs(
...
static MATRIX *compute_subcortical_map_weights(
    ...
    int nsubjects, MRI *mri_mask, int ndilate, char *out_prefix);
static int compute_surface_correlation_map_at_voxel(MRI *mri_fvol,
                                                    MRI *mri_fsurf, int x,
                                                    int y, int z, int frame,
                                                    MRI *mri_cmat);
#if 0
...
static int    write_snapshot(VECTOR *v_weights, MATRIX **m_I, MRI *mri_mask,
                             char *prefix, int iter, int nsubjects);
...
int main(int argc, char *argv[]) {
  ...
  if (downsample > 0 && mri_aseg) {
    MRI *mri_tmp2 = MRIdownsampleN(mri_aseg, NULL, downsample, downsample,
                                   downsample, 0),
        *mri_tmp;
  ...
  for (n = 0; n < nsubjects; n++) {
      ...
      for (run = 0; run < runs; run++) {
        ...
        if (mri_fsurf[n][run] == NULL)
          ErrorExit(ERROR_NOFILE,
                    "%s: could not load surface time series from %s", Progname,
                    fname);
        if (mri_fsurf[n][run]->height != 1 || mri_fsurf[n][run]->depth != 1) {
          mri_tmp =
              mri_reshape(mri_fsurf[n][run],
                          mri_fsurf[n][run]->width * mri_fsurf[n][run]->height *
                              mri_fsurf[n][run]->depth,
        ...
        if (mri_fvol[n][run] == NULL)
          ErrorExit(ERROR_NOFILE,
                    "%s: could not load volume time series from %s", Progname,
                    fname);
      }
      if (label_name) {
    ...
    } else // using surface-based correlation to drive warp
    {
      ...
      mri_label_avg[n] =
          average_within_label(mri_cmat, labels[n], offset, NULL);
      MRIfree(&mri_cmat);
  ...
  } else // using subcortical maps to drive warp
  {
	...
	mri_label_avg[n] = compute_voxlist_surface_correlations_across_runs(vl, num_maps, mri_fvol[n], mri_fsurf[n], runs, NULL) ;
      ...
      for (run = 0; run < runs; run++) {
        ...
        if (mri_fsurf[nsubjects][run] == NULL)
          ErrorExit(ERROR_NOFILE,
                    "%s: could not load surface time series from %s", Progname,
                    fname);
        if (mri_fsurf[nsubjects][run]->height != 1 ||
            mri_fsurf[nsubjects][run]->depth != 1) {
          mri_tmp = mri_reshape(mri_fsurf[nsubjects][run],
                                mri_fsurf[nsubjects][run]->width *
                                    mri_fsurf[nsubjects][run]->height *
                                    mri_fsurf[nsubjects][run]->depth,
      ...
      m_map_weights = compute_subcortical_map_weights(
          mris, mri_fvol, mri_fsurf, labels, runs, nsubjects, mri_mask, ndilate,
          output_name);
  ...
  ic    = 5;
  nvert = mri_aseg ? MRInvox(mri_label_avg[0]) / mri_label_avg[0]->nframes
  ...
  default:
    ErrorExit(ERROR_UNSUPPORTED,
              "only IC 5 or 6 supported (nvertices = %d detected)\n",
              MRInvox(mri_label_avg[0]) / 2);
  }
  ...
  {
    sprintf(fname, "%s/fsaverage%d/label/%s.%s", SUBJECTS_DIR, ic, hemi,
            prior_name);
    mri_prior = MRIread(fname);
    if (!mri_prior)
      ErrorExit(ERROR_NOFILE, "%s: could not prior overlay from %s", Progname,
                fname);

    sprintf(fname, "%s/%s/surf/%s", SUBJECTS_DIR, trgsubject, cmat_name);
    mri_cmat_mov = MRIread(fname);
    if (!mri_cmat_mov)
      ErrorExit(ERROR_NOFILE, "%s: could not read movable cmat from %s",
                Progname, fname);

    sprintf(fname, "%s/fsaverage%d/label/%s.MT.thresh.label", SUBJECTS_DIR, ic,
            hemi);
    ...
    warp_surface(mris_lh_mov, mris_rh_mov, mris_lh_fixed, mris_rh_fixed,
                 mri_cmat_mov, mri_stats, tol, target_area, offset, dt);
  } else // warping based on correlations with subcortical label(s)
  {
    warp_hemi(mris_lh_mov, mris_lh_fixed, mri_target_label, mri_stats,
              nsubjects, tol, dt);
  }
  sprintf(fname, "%s/fsaverage%d/surf/lh.sphere.%s", SUBJECTS_DIR, ic,
          output_name);
  printf("writing output surface to %s\n", fname);
  MRISwrite(mris_lh_mov, fname);
  sprintf(fname, "%s/fsaverage%d/surf/rh.sphere.%s", SUBJECTS_DIR, ic,
          output_name);
  printf("writing output surface to %s\n", fname);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      nargsused  = 1;
      printf("downsampling aseg volume %d times (time series will be "
             "downsampled to match resolution)\n",
             downsample);
      ...
      fmri_surf = pargv[1];
      nargsused = 2;
      printf("loading volume (%s) and surface (%s) rfSMRI time courses for "
             "each subject\n",
             fmri_vol, fmri_surf);
    } else if (!strcasecmp(option, "--aseg")) {
      mri_aseg = MRIread(pargv[0]);
      if (mri_aseg == NULL)
        ErrorExit(ERROR_NOFILE, "%s: could not load aseg from %s", Progname,
                  pargv[0]);
...
static MRI *compute_mean_and_variance(MRI **mri_label_avg, int nsubjects) {
  ...
  mri_stats = MRIallocSequence(mri->width, mri->height, mri->depth, MRI_FLOAT,
                               2 * nframes);
...
static MRI *average_within_label(MRI *mri_cmat, LABEL *area, int offet,
                                 MRI *mri_avg) {
  int   n, vno, x, y, z;
  float avg, val;

  if (mri_avg == NULL) {
    mri_avg = MRIallocSequence(mri_cmat->width, mri_cmat->height,
                               mri_cmat->depth, MRI_FLOAT, 1);
...
static int compute_vertex_permutation(MRI_SURFACE *mris_mov,
                                      MRI_SURFACE *mris_fixed, MHT *mht,
                                      int *vertices) {
  int     vno;
  VERTEX *v, *vfixed;
...
static MRI *permute_corrmat(MRI *mri_corrmat_src, int *lh_vertices,
                            int *rh_vertices, MRI *mri_corrmat_dst) {
...
static double compute_error(MRI_SURFACE *mris_mov, MRI_SURFACE *mris_fixed,
  ...
  for (sse = 0.0, vno = 0; vno < mris_mov->nvertices; vno++) {
    ...
    mean = sample_stats(mris_mov, mris_fixed, mht, mri_stats, v->x, v->y, v->z,
                        &var);
...
static double sample_vertex_error(MRI_SURFACE *mris_mov,
                                  ...
                                  MRI_SURFACE *mris_rh_fixed, MHT *mht,
                                  MRI *mri_stats, MRI *mri_corrmat_mov,
                                  MRI *mri_label_avg, int vno, double x,
                                  double y, double z) {
...
static double sample_stats(MRI_SURFACE *mris_mov, MRI_SURFACE *mris_fixed,
  ...
  MHTfindClosestFaceGeneric(mht, mris_fixed, x, y, z, 8000, -1, 0, &face, &fno,
                            &fdist);
  ...
  mean = MRISsampleFace(mris_fixed, fno, CURRENT_VERTICES, x, y, z, val0, val1,
                        val2);
  if (fabs(val0) > 100 || fabs(val1) > 100 || fabs(val2) > 100 ||
      fabs(mean) > 100)
    DiagBreak();
  ...
  var  = MRISsampleFace(mris_fixed, fno, CURRENT_VERTICES, x, y, z, val0, val1,
                       val2);
  if (fabs(val0) > 100 || fabs(val1) > 100 || fabs(val2) > 100 ||
      fabs(var) > 100)
    DiagBreak();
...
static int compute_warp_gradient(MRI_SURFACE *mris_lh_mov,
  ...
  for (vno = 0; vno < mris_lh_mov->nvertices; vno++) {
    ...
    y   = v->y + v->e1y * dt;
    z   = v->z + v->e1z * dt;
    e1p = sample_vertex_error(mris_lh_mov, mris_lh_fixed, mris_rh_fixed, mht_lh,
                              mri_stats, mri_corrmat_mov, mri_label_avg, vno, x,
                              y, z);
    x   = v->x - v->e1x * dt;
    y   = v->y - v->e1y * dt;
    z   = v->z - v->e1z * dt;
    e1m = sample_vertex_error(mris_lh_mov, mris_lh_fixed, mris_rh_fixed, mht_lh,
                              mri_stats, mri_corrmat_mov, mri_label_avg, vno, x,
                              y, z);
    x   = v->x + v->e2x * dt;
    y   = v->y + v->e2y * dt;
    z   = v->z + v->e2z * dt;
    e2p = sample_vertex_error(mris_lh_mov, mris_lh_fixed, mris_rh_fixed, mht_lh,
                              mri_stats, mri_corrmat_mov, mri_label_avg, vno, x,
                              y, z);
    x   = v->x - v->e2x * dt;
    y   = v->y - v->e2y * dt;
    z   = v->z - v->e2z * dt;
    e2m = sample_vertex_error(mris_lh_mov, mris_lh_fixed, mris_rh_fixed, mht_lh,
                              mri_stats, mri_corrmat_mov, mri_label_avg, vno, x,
                              y, z);
...
static int warp_surface(MRI_SURFACE *mris_lh_mov, MRI_SURFACE *mris_rh_mov,
  ...
  mht_lh      = MHTcreateVertexTable_Resolution(
      mris_lh_fixed, CURRENT_VERTICES, ceil(mris_lh_fixed->avg_vertex_dist));
  mht_rh = MHTcreateVertexTable_Resolution(
      mris_rh_fixed, CURRENT_VERTICES, ceil(mris_rh_fixed->avg_vertex_dist));
  mht_lh_faces = MHTcreateFaceTable_Resolution(
      mris_lh_fixed, CURRENT_VERTICES, ceil(mris_lh_fixed->avg_vertex_dist));
  mht_rh_faces = MHTcreateFaceTable_Resolution(
      mris_rh_fixed, CURRENT_VERTICES, ceil(mris_rh_fixed->avg_vertex_dist));
  ...
  mri_cmat_permuted = permute_corrmat(mri_cmat_mov, lh_vertices, rh_vertices,
                                      mri_cmat_permuted);
  mri_mov_avg = average_within_label(mri_cmat_permuted, area, offset, NULL);
  last_error  = compute_error(mris_lh_mov, mris_lh_fixed, mht_lh_faces,
                             mri_mov_avg, mri_stats);
  if (write_diags) {
    char fname[STRLEN];
    MRI *mri_fixed;

    mri_fixed =
        sample_fixed(mris_lh_mov, mris_lh_fixed, mht_lh_faces, mri_stats);
  ...
  do {
    compute_warp_gradient(mris_lh_mov, mris_rh_mov, mris_lh_fixed,
                          mris_rh_fixed, mht_lh_faces, mht_rh_faces,
                          mri_cmat_permuted, mri_stats, mri_mov_avg,
                          mri_cmat_permuted, dt);
    MRISapplyGradient(mris_lh_mov, 1);
    MRISprojectOntoSphere(mris_lh_mov, mris_lh_mov, mris_lh_mov->radius);
    MRISapplyGradient(mris_rh_mov, 1);
    MRISprojectOntoSphere(mris_rh_mov, mris_rh_mov, mris_rh_mov->radius);
    ...
    compute_vertex_permutation(mris_lh_mov, mris_lh_fixed, mht_lh, lh_vertices);
    compute_vertex_permutation(mris_rh_mov, mris_rh_fixed, mht_rh, rh_vertices);
    error = compute_error(mris_lh_mov, mris_lh_fixed, mht_lh_faces, mri_mov_avg,
                          mri_stats);
...
static int compute_surface_correlation_map_at_voxel(MRI *mri_fvol,
                                                    ...
                                                    MRI *mri_cmat) {
  int   vno, t;
  float corr, sval, vval, stotal, stotalsq, vtotal, vtotalsq, smean, vmean,
      sstd, vstd, denom;
...
static double sample_hemi_vertex_error(MRI_SURFACE *mris_mov,
  ...
  for (total_error = 0.0, frame = 0; frame < mri_label_avg->nframes; frame++) {
    mean  = sample_hemi_stats(mris_mov, mris_fixed, mht, mri_stats, x, y, z,
                             frame, &var);
...
static int compute_hemi_warp_gradient(MRI_SURFACE *mris_mov,
  ...
  for (vno = 0; vno < mris_mov->nvertices; vno++) {
    ...
    y   = v->y + v->e1y * dt;
    z   = v->z + v->e1z * dt;
    e1p = sample_hemi_vertex_error(mris_mov, mris_fixed, mht, mri_stats,
                                   mri_target_label, vno, x, y, z);
    x   = v->x - v->e1x * dt;
    y   = v->y - v->e1y * dt;
    z   = v->z - v->e1z * dt;
    e1m = sample_hemi_vertex_error(mris_mov, mris_fixed, mht, mri_stats,
                                   mri_target_label, vno, x, y, z);
    x   = v->x + v->e2x * dt;
    y   = v->y + v->e2y * dt;
    z   = v->z + v->e2z * dt;
    e2p = sample_hemi_vertex_error(mris_mov, mris_fixed, mht, mri_stats,
                                   mri_target_label, vno, x, y, z);
    x   = v->x - v->e2x * dt;
    y   = v->y - v->e2y * dt;
    z   = v->z - v->e2z * dt;
    e2m = sample_hemi_vertex_error(mris_mov, mris_fixed, mht, mri_stats,
                                   mri_target_label, vno, x, y, z);
    if (e1p < e1m && e1p < e2p && e1p < e2m) // move in e1p dir
    {
      dx = v->e1x * dt;
...
static double compute_hemi_error(MRI_SURFACE *mris_mov, MRI_SURFACE *mris_fixed,
                                 MHT *mht_faces, MRI *mri_stats,
                                 MRI *mri_target_label, int nsubjects) {
  int     vno, frame;
  double  sse;
  double  val, mean, var;
  VERTEX *v;
  ...
  for (sse = 0.0, vno = 0; vno < mris_mov->nvertices; vno++) {
    ...
    for (frame = 0; frame < mri_target_label->nframes; frame++) {
      mean = sample_hemi_stats(mris_mov, mris_fixed, mht_faces, mri_stats, v->x,
                               v->y, v->z, frame, &var);
...
static int warp_hemi(MRI_SURFACE *mris_mov, MRI_SURFACE *mris_fixed,
  ...
  mht_vertices = MHTcreateVertexTable_Resolution(
      mris_fixed, CURRENT_VERTICES, ceil(mris_fixed->avg_vertex_dist));
  mht_faces = MHTcreateFaceTable_Resolution(mris_fixed, CURRENT_VERTICES,
                                            ceil(mris_fixed->avg_vertex_dist));
#if 0
  if (write_diags)
  {
    char fname[STRLEN] ;
  ...
  last_error = compute_hemi_error(mris_mov, mris_fixed, mht_faces, mri_stats,
                                  mri_target_label, nsubjects);
  orig_dt    = dt;
  printf("%3.3d: dt = %2.3f, error = %2.4f\n", iter = 0, orig_dt, last_error);
  for (grad_averages = max_grad_averages; grad_averages >= min_grad_averages;
       grad_averages /= 4) {
    iter = 0;
    do {
      compute_hemi_warp_gradient(mris_mov, mris_fixed, mht_faces, mri_stats,
                                 mri_target_label, dt);
      MRISaverageGradients(mris_mov, grad_averages);
      MRISapplyGradient(mris_mov, 1);
      MRISprojectOntoSphere(mris_mov, mris_mov, mris_mov->radius);

      if (write_diags && (((iter + 1) % 10) == 0)) {
        char fname[STRLEN];
        sprintf(fname, "%s.%s.%3.3d", hemi, output_name,
                iter + 1 + iters_so_far);
        printf("writing snapshot to %s\n", fname);
        MRISwrite(mris_mov, fname);
      }
      error = compute_hemi_error(mris_mov, mris_fixed, mht_faces, mri_stats,
                                 mri_target_label, nsubjects);
      pct_change = 100 * (last_error - error) / last_error;
...
static MRI *compute_voxlist_surface_correlations(VOXEL_LIST *vl, int num_maps,
  ...
  mri_cmat =
      MRIallocSequence(mri_fsurf->width * mri_fsurf->height * mri_fsurf->depth,
                       1, 1, MRI_FLOAT, num_maps);
  ...
  for (n = 0; n < num_maps; n++) {
    ...
    if (x == Gx && y == Gy && z == Gz)
      DiagBreak();
    compute_surface_correlation_map_at_voxel(mri_fvol, mri_fsurf, x, y, z, n,
                                             mri_cmat);
  }
...
static MRI *compute_voxlist_surface_correlations_across_runs(
  ...
  for (r = 0; r < runs; r++) {
    mri_cmat = compute_voxlist_surface_correlations(vl, num_maps, mri_fvol[r],
                                                    mri_fsurf[r], NULL);
    if (mri_dst == NULL)
      mri_dst = mri_cmat;
    else {
...
static VECTOR *compute_desired_map(MRI_SURFACE *mris, int cols, LABEL *area,
                                   int ndilate) {
  VECTOR *v;
  int     n;
  ...
  if (ndilate > 0) {
    ...
    for (n = 0; n < mris->nvertices; n++)
      if (mris->vertices[n].marked &&
          mris->vertices[n].marked2 == 0) // in ring around label
...
static double compute_weight_functional(VECTOR **v_D, MATRIX **m_I,
                                        VECTOR *v_weights, int nsubjects) {
  double  total_error;
  int     n;
...
#pragma omp parallel for if_ROMP(experimental) firstprivate (v_D, v_weight_T) reduction(+:total_error) schedule(static,1)
#endif
      for (n = 0; n < nsubjects; n++) {
    ROMP_PFLB_begin VECTOR *I_weighted, *I_weighted_T;
    double                  error;
    ...
    VectorFree(&I_weighted_T);
    ROMP_PFLB_end
  }
  ROMP_PF_end VectorFree(&v_weight_T);
  return (total_error);
}
static VECTOR *compute_gradient_wrt_weights(VECTOR **v_D, MATRIX **m_I,
                                            VECTOR *v_weights, int nsubjects,
                                            VECTOR *v_gradient) {
  ...
  nvertices = v_D[0]->rows;
  nvox      = v_weights->rows;
  ROMP_PF_begin
#ifdef HAVE_OPENMP
#pragma omp parallel for if_ROMP(experimental) firstprivate(v_D)               \
    shared(v_diff) schedule(static, 1)
#endif
      for (n = 0; n < nsubjects; n++) {
    ROMP_PFLB_begin VECTOR *I_weighted, *I_weighted_T;
    I_weighted   = MatrixMultiply(v_weight_T, m_I[n], NULL);
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(l, v_gradient, nvertices) schedule(static, 1)
#endif
        for (n = 0; n < nsubjects; n++) {
      ROMP_PFLB_begin int i;
  ...
  for (n = 0; n < nsubjects; n++)
    VectorFree(&v_diff[n]);
  VectorScalarMul(v_gradient, 1.0 / ((float)nvertices * (float)nvox),
                  v_gradient);
  return (v_gradient);
}
...
static VECTOR *optimize_weight_functional(VECTOR **v_D, MATRIX **m_I,
                                          int nsubjects, MRI *mri_mask,
                                          char *out_prefix) {
  MATRIX *v_weights;
  ...
  last_error   = error =
      compute_weight_functional(v_D, m_I, v_weights, nsubjects);
  ...
  for (iter = 0; iter < max_wt_iters; iter++) {
    last_error  = error;
    v_d_weights = compute_gradient_wrt_weights(v_D, m_I, v_weights, nsubjects,
                                               v_d_weights);
...
static MATRIX *compute_subcortical_map_weights(
  ...
  */
  ...
  for (n = 0; n <= nsubjects; n++) {
    v_D[n] =
        compute_desired_map(mris, mri_fsurf[0][0]->width, labels[n], ndilate);
    if (write_diags) {
      char  fname[STRLEN];
      int   vno;
      float m;
...
#ifdef HAVE_OPENMP
      mri_cmat = NULL;
#pragma omp parallel for if_ROMP(experimental) firstprivate(mri_cmat)          \
    shared(m_I, vl, mri_fvol, mri_fsurf, runs) schedule(static, 1)
#endif
  for (n = 0; n <= nsubjects; n++) {
    ROMP_PFLB_begin mri_cmat = compute_voxlist_surface_correlations_across_runs(
        vl, vl->nvox, mri_fvol[n], mri_fsurf[n], runs, NULL);
  ...
  if (use_powell) {
    ...
    v_weights =
        optimize_weight_functional(v_D, m_I, nsubjects, mri_mask, out_prefix);
  if (write_diags) {
    char fname[STRLEN];
  ...
  if (write_diags) {
    ...
    mri_surf = MRIalloc(mri_fsurf[0][0]->width, mri_fsurf[0][0]->height,
                        mri_fsurf[0][0]->depth, MRI_FLOAT);
    MRIcopyHeader(mri_fsurf[0][0], mri_surf);

    v_weights_T = VectorTranspose(v_weights, NULL);
    for (n = 0; n < vl->nvox; n++) {
      MRIsetVoxVal(mri_wts, vl->xi[n], vl->yi[n], vl->zi[n], 0,
                   VECTOR_ELT(v_weights, n + 1));
...
static int write_snapshot(VECTOR *v_weights, MATRIX **m_I, MRI *mri_mask,
  ...
  sprintf(fname, "%s.%3.3d.wts.mgz", prefix, iter);
  printf("writing wt image to %s\n", fname);
  MRIwrite(mri_wts, fname);
  sprintf(fname, "%s.%3.3d.wts.txt", prefix, iter);
  printf("writing wt vector to %s\n", fname);
  MatrixWriteTxt(fname, v_weights);
...
static int powell_minimize(VECTOR **v_D, MATRIX **m_I, int nsubjects,
  ...
  //  Gdiag_no = 1 ;
  OpenPowell2(p, xi, v_weights->rows, powell_ftol, powell_linmin_tol,
              max_powell_iter, &iter, &fret, compute_powell_sse);
  for (i = 1; i <= v_weights->rows; i++)
    VECTOR_ELT(v_weights, i) = p[i];
  if (write_diags)
    write_snapshot(v_weights, m_I, mri_mask, "after_powell", iter + 1,
                   nsubjects);
  do {
    ...
    for (i = 1; i <= v_weights->rows; i++)
      VECTOR_ELT(v_weights, i) = p[i];
    if (write_diags)
      write_snapshot(v_weights, m_I, mri_mask, "after_second_powell", iter + 1,
                     nsubjects);
