...
const std::string Poistats::FLAG_SEEDS = Poistats::FLAG_DELIMITER + "seeds";
...
Poistats::Poistats(int inArgs, char **iaArgs)
  ...
  SetName("dmri_poistats", "find optimal path in tensor volume (**beta version "
                           "with limited support)");

  SetNextRequiredArgument(FLAG_INPUT_STEM, "dtensorinstem",
                          "Diffusion tensor input", "dtensor.nii",
                          "must specify a dtensor input filename");
  SetNextRequiredArgument(FLAG_OUTPUT_DIRECTORY, "outdir", "Output directory.",
                          "poistats", "must specify an output directory");
  SetNextRequiredArgument(
      FLAG_SEEDS, "seedstem",
  ...
  SetNextRequiredArgument(FLAG_SAMPLE_STEM, "samplestem",
                          ...
                          "fa.nii", "must specify a sampling volume");
  SetNextRequiredArgument(
      FLAG_NUM_CONTROL_POINTS, "ncontrolpoints",
      "Number of control points used to describe path. Number should be "
      "approximately the number of 'turns' in the path. Almost always 1 or 2.",
      "2", "must specify number of control points");
  SetNextRequiredArgument(
      FLAG_SIGMA, "sigmasize",
      "Search distance for path control points in units voxels. The search "
      "distance should be approximately the distance between the midpoint of "
      "the seed path and the target location",
      "10", "must specify search distance");

  SetNextOptionalArgument(
      FLAG_MASK_STEM, "maskstem",
      "Instem for mask. The path will not be allowed to contact the mask. The "
      "mask can be used to specify invalid regions, e.g., CSF");
  SetNextOptionalArgument(
      FLAG_NUM_SAMPLES, "nsamplepoints",
      "Number of points to sample along path from sample volume. For example, "
      "--ns 100 will sample 100 values along the path. Default: 100");
  SetNextOptionalArgument(
      FLAG_SEED_VALUES, "seednumvalue",
      "Use <seednumvalue> to define seed region. Eg, --seednums 1,2");
  SetNextOptionalArgument(
      FLAG_NUM_REPLICAS, "nreplicas",
      "Use <nreplicas> to specify the number of replicas.  For example, "
      "--nreplicas 100 will spawn 100 replicas. Default: 100");
  ...
  SetNextOptionalArgument(
      FLAG_IS_SYMMETRIC_DATA, "symmetric",
      "Is the tensor input stored symmetric (6 rather than 9 component "
      "storage)?  Default: false (not stored symmetrically).");
  SetNextOptionalArgument(
      FLAG_IS_OUTPUT_NII, "nii",
      "Should the output be stored as nifti?  Default: false");
  SetNextOptionalArgument(
      FLAG_EIGENVECTOR_STEM, "eigvec",
      "Should the paths be initialized by following the principle eigenvector? "
      " Specify the eigenvector volume following this flag.");
  ...
  SetNextOptionalArgument(
      FLAG_SHOULD_WRITE_BEST_PATHS, "writepaths",
      "TESTING -- Should the best paths from each replicas be written out?  "
      "This will save to InitialPaths.txt for the time being");

  std::string output = "";
  output =
      output + "PathDensity.nii - path probability density" + "\n   " +
...
bool Poistats::FillArguments() {
  ...
  if (isFilled) {
    ...
    m_ReplicaExchangeProbability =
        m_Parser->GetArgumentDouble(FLAG_REPLICA_EXCHANGE_PROBABILITY.c_str());

    m_SigmaTimeConstant =
        m_Parser->GetArgumentDouble(FLAG_SIGMA_TIME_CONSTANT.c_str());

    m_PointsToImageGamma =
        m_Parser->GetArgumentDouble(FLAG_POINTS_TO_IMAGE_GAMMA.c_str());

    m_IsSymmetricData =
        m_Parser->GetArgumentBoolean(FLAG_IS_SYMMETRIC_DATA.c_str());
    ...
    m_ShouldWriteBestPaths =
        m_Parser->GetArgumentBoolean(FLAG_SHOULD_WRITE_BEST_PATHS.c_str());
  }
...
bool Poistats::IsNifti(std::string fileExtension) {
...
bool Poistats::IsMGH(std::string fileExtension) {
...
mat44 Poistats::GetNiftiTransform(const char *filename) {
  ...
  rval =
      img->qto_xyz; // or img->sto_xyz -- you need to check the transform type
...
void Poistats::Run() {
  ...
  observer->PostMessage(
      GetFieldAndParameter("Command", m_CommandLineArguments));

  observer->PostMessage(
      GetFieldAndParameter("FreeSurfer Home", GetFreeSurferHome()));

  observer->PostMessage(
      GetFieldAndParameter("Subjects Directory", GetSubjectsDirectory()));

  observer->PostMessage(
      GetFieldAndParameter("Current Working Directory", GetCurrentDirectory()));

  observer->PostMessage(
      GetFieldAndParameter("Operating System", GetOperatingSystem()));
  ...
  if (IsNifti(imageFileExtension)) {
    ...
    for (int cRow = 0; cRow < 3; cRow++) {
      for (int cCol = 0; cCol < 3; cCol++) {
        direction(cRow, cCol) =
            transform.m[cRow][cCol] / tensors->GetSpacing()[cRow];
      }
    }
  ...
  if (m_SeedValues.size() == 1) {
    std::ostringstream output;
    output << "If specifying seed number, you must specify at least two labels."
           << std::endl;
    observer->PostErrorMessage(output.str());
    return;
  } else {
  ...
  if (m_ReplicaExchangeProbability > 0) {
    poistatsFilter->SetReplicaExchangeProbability(m_ReplicaExchangeProbability);
  }
  ...
  try {
    ...
    std::ostringstream output;
    output << "Error thrown in poistats filter." << std::endl
           << excp << std::endl;
    observer->PostErrorMessage(output.str());
    return;
  }
  ...
  std::string optimalDensityFileName = (std::string)m_OutputDir +
                                       (std::string) "/OptimalPathDensity" +
                                       imageFileExtension;
  OutputImageType::Pointer optimalPathDensity = poistatsFilter->GetOutput(
      PoistatsFilterType::OPTIMAL_PATH_DENSITY_OUTPUT);
  ...
  const std::string pathProbabilitiesFileName(
      (std::string)m_OutputDir + (std::string) "/OptimalPathProbabilities.txt");
  WriteData(pathProbabilitiesFileName, finalPathProbabilities.data_block(),
            finalPathProbabilities.size());
  ...
  if (m_ShouldWriteBestPaths) {
    ...
    PoistatsFilterType::MatrixListType paths =
        poistatsFilter->GetBestTrialPaths();

    bool isFirst = true;

    // iterate through the paths and write append them
    for (PoistatsFilterType::MatrixListType::iterator replicaIterator =
             paths.begin();
      ...
      if (isFirst) {
        ...
        WriteDataAppend(pathFileName, path->data_array(), path->rows(),
                        path->cols());
      }
    }
  }
