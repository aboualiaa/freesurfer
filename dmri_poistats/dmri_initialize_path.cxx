...
InitializePathExe::InitializePathExe(int inArgs, char **iaArgs)
  ...
  SetNextRequiredArgument(FLAG_SEEDS, "seeds", "Seed volume", "seeds.nii",
                          "must specify a seed volume");

  SetNextRequiredArgument(FLAG_OUTPUT_DIRECTORY, "output", "Output directory",
                          "output/", "must specify an output directory");

  SetNextOptionalArgument(
      FLAG_SEED_VALUES, "seednumvalue",
      "Use <seednumvalue> to define seed region.  If nothing is specified, "
      "then all seeds are used.  More than one must be available.  Eg, "
      "--seednums 1,2");

  SetNextOptionalArgument(FLAG_EIGENVECTOR, "eigvec",
                          "Principle eigenvector input");
  ...
  SetNextOptionalArgument(
      FLAG_RADIUS, "radius",
      "maximum distance the field line initialization can be away");

  std::string output = "";
  output = output + "InitialPath - volume of initial points" + "\n   " +
...
bool InitializePathExe::FillArguments() {
  bool isFilled = false;
  ...
  if (isFilled) {
    ...
    m_NumberOfReplicas =
        m_Parser->GetArgumentInt(FLAG_NUMBER_OF_REPLICAS.c_str());
...
void InitializePathExe::Run() {
  ...
  // if the eigenvectors were provided, then we'll run the eigenvector
  // intialization
