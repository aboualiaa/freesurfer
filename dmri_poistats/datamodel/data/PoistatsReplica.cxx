...
void PoistatsReplica::SetModel(PoistatsModel *model) {
  m_PoistatsModel = model;
}

double PoistatsReplica::GetCurrentMeanEnergy() const {
  return m_CurrentMeanEnergy;
}

void PoistatsReplica::SetCurrentMeanEnergy(const double energy) {
  m_CurrentMeanEnergy = energy;
}

double PoistatsReplica::GetPreviousMeanEnergy() const {
  return m_PreviousMeanEnergy;
}

void PoistatsReplica::SetPreviousMeanEnergy(const double energy) {
  m_PreviousMeanEnergy = energy;
}

double PoistatsReplica::GetTemperature() { return m_Temperature; }

void PoistatsReplica::SetTemperature(const double temperature) {
  m_Temperature = temperature;
}

void PoistatsReplica::CopyCurrentToPreviousEnergy() {
...
bool PoistatsReplica::ShouldUpdateEnergy() {

  const double delta =
      (m_CurrentMeanEnergy - m_PreviousMeanEnergy) / m_Temperature;
...
PoistatsReplica::MatrixPointer PoistatsReplica::GetCurrentTrialPath() {
  return m_CurrentTrialPath;
}

void PoistatsReplica::SetCurrentTrialPath(MatrixPointer path) {
  DeletePathIfNotNull(m_CurrentTrialPath);
  m_CurrentTrialPath = new MatrixType(*path);
}

PoistatsReplica::MatrixPointer PoistatsReplica::GetBestTrialPath() {
  return m_BestTrialPath;
}

void PoistatsReplica::SetBestTrialPath(MatrixPointer path) {
  DeletePathIfNotNull(m_BestTrialPath);
...
void PoistatsReplica::GetPerturbedBasePath(
    MatrixPointer perturbedPath, const double sigma,
  ...
  for (int cControlPoint = 0; cControlPoint < nBasePoints; cControlPoint++) {
    randomPoints[cControlPoint][cControlPoint] =
        m_PoistatsModel->GetRandomNumber();
  }
  ...
  for (unsigned int cRow = 1; cRow < currentBasePath->rows() - 1; cRow++) {
    for (unsigned int cColumn = 0; cColumn < currentBasePath->cols();
         cColumn++) {
  ...
  this->GenerateConstrainedRandomPoint3D(
      currentBasePath->get_row(0), possibleStartPoints, sigma, &newRandomPoint);
  ...
  this->GenerateConstrainedRandomPoint3D(
      currentBasePath->get_row(currentBasePath->rows() - 1), possibleEndPoints,
 ...
 */
void PoistatsReplica::GenerateUnitSphereRandom(MatrixPointer randomUnitSphere) {

  const int numberOfPoints = randomUnitSphere->rows();

  for (unsigned int cRow = 0; cRow < randomUnitSphere->rows(); cRow++) {
    for (unsigned int cColumn = 0; cColumn < randomUnitSphere->cols();
         cColumn++) {
...
void PoistatsReplica::GenerateConstrainedRandomPoint3D(

    vnl_vector<double> currentPoint, MatrixPointer possibleNewPoints,
    const double sigma, ArrayPointer newRandomPoint) {

  std::vector<int> indicesOfPointsWithinRadius;

  const bool isPointFound = FindPointsWithinRadius(
      &currentPoint, possibleNewPoints, sigma, &indicesOfPointsWithinRadius);
...
bool PoistatsReplica::FindPointsWithinRadius(vnl_vector<double> *center,
  ...
  for (unsigned int cRow = 0; cRow < points->rows(); cRow++) {
    for (unsigned int cColumn = 0; cColumn < points->cols(); cColumn++) {
      differenceMatrix[cRow][cColumn] =
          (*points)[cRow][cColumn] - (*center)[cColumn];
    }
  }
...
void PoistatsReplica::SetBestTrialPathProbabilities(
    ArrayPointer probabilities) {
  DeleteArrayIfNotNull(m_BestTrialPathProbabilities);
  m_BestTrialPathProbabilities = new ArrayType(*probabilities);
}

PoistatsReplica::ArrayPointer PoistatsReplica::GetBestTrialPathProbabilities() {
