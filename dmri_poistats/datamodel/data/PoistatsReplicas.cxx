...
int PoistatsReplicas::GetNumberOfReplicas() { return m_NumberOfReplicas; }

void PoistatsReplicas::SetNumberOfReplicas(const int nReplicas) {
...
double PoistatsReplicas::GetMinimumCurrentEnergy() {
...
void PoistatsReplicas::FillCurrentMeanEnergies(const double energy) {
  for (int cReplica = 0; cReplica < m_NumberOfReplicas; cReplica++) {
    m_Replicas[cReplica].SetCurrentMeanEnergy(energy);
  }
}

void PoistatsReplicas::FillPreviousMeanEnergies(const double energy) {
  for (int cReplica = 0; cReplica < m_NumberOfReplicas; cReplica++) {
    m_Replicas[cReplica].SetPreviousMeanEnergy(energy);
  }
}

double PoistatsReplicas::GetCurrentMeanOfEnergies() const {
  double sum = 0.0;
  for (int cReplica = 0; cReplica < m_NumberOfReplicas; cReplica++) {
...
double PoistatsReplicas::GetPreviousMeanOfEnergies() const {
  double sum = 0.0;
  for (int cReplica = 0; cReplica < m_NumberOfReplicas; cReplica++) {
...
double PoistatsReplicas::GetCurrentMeanEnergy(const int replica) {
...
double PoistatsReplicas::GetPreviousMeanEnergy(const int replica) {
  return m_Replicas[replica].GetPreviousMeanEnergy();
}

void PoistatsReplicas::GetRandomSortedFirstSecondReplicas(int &first,
                                                          int &second) {
...
void PoistatsReplicas::SortArray(itk::Array<double> *unsorted,
  ...
  for (unsigned int cSorted = 0; cSorted < sorted.size(); cSorted++) {
    ...
    for (unsigned int cUnsorted = 0; cUnsorted < unsorted->size();
         cUnsorted++) {
...
void PoistatsReplicas::CopyCurrentToPreviousEnergies() {
  for (int cReplica = 0; cReplica < m_NumberOfReplicas; cReplica++) {
    m_Replicas[cReplica].CopyCurrentToPreviousEnergy();
  }
}

void PoistatsReplicas::CopyCurrentToPreviousEnergy(int replica) {
...
void PoistatsReplicas::SetInitialPoints(const MatrixPointer points) {
  ...
  const int nTotalControlPoints =
      m_PoistatsModel->GetNumberOfControlPoints() + nEndPoints;

  MatrixPointer originalPath =
      m_PoistatsModel->RethreadPath(m_InitialPoints, nTotalControlPoints);
...
void PoistatsReplicas::InitializePaths() {
  ...
  for (int cReplica = 0; cReplica < m_NumberOfReplicas; cReplica++) {
    ...
    MatrixPointer base =
        m_PoistatsModel->RethreadPath(initialPath, nTotalControlPoints);
    m_Replicas[cReplica].SetBasePath(base);
    delete base;
    base = NULL;

    // rethread to the previous path
    MatrixPointer previous =
        m_PoistatsModel->RethreadPath(initialPath, this->GetNumberOfSteps());
...
int PoistatsReplicas::GetNumberOfSteps() { return m_NumberOfSteps; }
...
PoistatsReplicas::MatrixListType PoistatsReplicas::GetBestTrialPaths() {
...
void PoistatsReplicas::SetBestTrialPathProbabilities(
    const int replica, ArrayPointer probabilities) {

  m_Replicas[replica].SetBestTrialPathProbabilities(probabilities);
}

void PoistatsReplicas::GetBestTrialPathsProbabilities(
    MatrixPointer probabilities) {
  ...
  for (int cReplica = 0; cReplica < this->GetNumberOfReplicas(); cReplica++) {
    ...
    for (unsigned int cStep = 0; cStep < replicaProbabilities->size();
         cStep++) {
      (*probabilities)[cReplica][cStep] = (*replicaProbabilities)[cStep];
    }
  }
}
...
void PoistatsReplicas::SpaceTemperaturesEvenly(const double floorTemp,
...
void PoistatsReplicas::GetPerturbedBasePath(
    const int replica, MatrixPointer perturbedPath, const double sigma,
    const MatrixPointer possibleStartPoints,
    const MatrixPointer possibleEndPoints) {

  m_Replicas[replica].GetPerturbedBasePath(
