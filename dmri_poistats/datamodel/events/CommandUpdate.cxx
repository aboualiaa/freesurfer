...
void CommandUpdate::Execute(const itk::Object *object,
  ...
  if (itk::IterationEvent().CheckEvent(&event)) {
    ...
    output << currentIteration << "   lull: " << currentLull
           << "  denergy: " << energyDifference << "  mean: " << meanOfEnergies
           << "  min: " << minOfEnergies << "  global min: " << globalMin
           << "  exchs: " << exchanges << "  time: " << elapsedTime << "\n";
    ...
    if (itk::SeedsUsingAllEvent().CheckEvent(&event)) {
      ...
      PostMessage("Seeds to use not explicitly set, using all seeds in seed "
                  "volume...\n");
    ...
    if (itk::GenerateOptimalPathDensitiesEvent().CheckEvent(&event)) {
    ...
    } else if (itk::GenerateBestReplicaPathDensitiesEvent().CheckEvent(
                   &event)) {

      if (itk::GenerateBestReplicaPathDensitiesStartEvent().CheckEvent(
              &event)) {
      ...
      } else if (itk::GenerateBestReplicaPathDensitiesEndEvent().CheckEvent(
                     &event)) {
        PostMessage("\nfinished\n");
      }

    } else if (itk::GenerateFinalPathProbabilitiesEvent().CheckEvent(&event)) {
...
void CommandUpdate::PostMessage(const std::string message) {
...
void CommandUpdate::PostErrorMessage(const std::string message) {
...
void CommandUpdate::WriteMessage(const std::string message,
