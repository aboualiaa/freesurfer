...
double PoistatsModel::GetNormallyDistributedRandomNumber() {
  ...
  const double randomNumber = this->m_RandomNumberGenerator.normal64();
  return randomNumber;
}
...
bool PoistatsModel::IsDebug() { return m_IsDebug; }

void PoistatsModel::SetDebug(const bool isDebug) { m_IsDebug = isDebug; }
...
MRI *PoistatsModel::GetEigenVectors() { return m_EigenVectors; }

void PoistatsModel::SetEigenVectors(MRI *vectors) { m_EigenVectors = vectors; }

MRI *PoistatsModel::GetSeedVolume() { return m_SeedVolume; }

void PoistatsModel::SetSeedVolume(MRI *volume) { m_SeedVolume = volume; }

std::vector<int> *PoistatsModel::GetSeedValues() { return m_SeedValues; }

void PoistatsModel::SetSeedValues(std::vector<int> *values) {
  m_SeedValues = values;
}

void PoistatsModel::Init() {
...
void PoistatsModel::FreeVector(std::vector<int> *v) {
...
PoistatsModel::CubicSplineInterpolation(MatrixPointer originalPath,
  ...
  MatrixPointer rethreadedPath = new MatrixType(nNewSamples, spatialDimension);

  const double resampledStepSize =
      1.0 / (static_cast<double>(nNewSamples) - 1.0);
...
void PoistatsModel::SpaceEvenly(ArrayPointer outputArray, const double floor,
                                const double ceiling) {

  const double spacing = (ceiling - floor) / (outputArray->size() - 1);
  for (unsigned int cOutputArray = 0; cOutputArray < outputArray->size();
       cOutputArray++) {
 ...
 */
void PoistatsModel::CalculateCumulativeSum(ArrayPointer inputArray,
  ...
  for (unsigned int cRow = 1; cRow < cumulativeSum->size(); cRow++) {
    ...
    (*cumulativeSum)[cRow] =
        (*cumulativeSum)[cRow - 1] + (*inputArray)[cRow - 1];
  }
}
 ...
 */
void PoistatsModel::CalculatePathVectors(MatrixPointer path,
                                         MatrixPointer vectors) {
  ...
  for (int cRow = 0; cRow < static_cast<int>(path->rows()) - 1; cRow++) {
    ...
    for (int cColumn = 0; cColumn < spatialDimension; cColumn++) {
      ...
      double difference = (*path)[cRow + 1][cColumn] - (*path)[cRow][cColumn];
...
void PoistatsModel::CalculateMagnitude(MatrixPointer path,
  ...
  for (int cRow = 0; cRow < static_cast<int>(path->rows()); cRow++) {
    ...
    for (int cColumn = 0; cColumn < spatialDimension; cColumn++) {
      ...
      rowSumOfDifferenceSquared +=
          (*path)[cRow][cColumn] * (*path)[cRow][cColumn];
    }
    (*magnitude)[cRow] = sqrt(rowSumOfDifferenceSquared);
  }
}

int PoistatsModel::GetInitialSigma() const { return m_InitialSigma; }

void PoistatsModel::SetInitialSigma(const int initialSigma) {
  m_InitialSigma = initialSigma;
}

InitializePath *PoistatsModel::GetPathInitializer() const {
  return m_PathInitializer;
}

void PoistatsModel::SetInitializePathMode(const int mode) {
...
int PoistatsModel::GetInitializePathMode() const { return m_InitializeMode; }

void PoistatsModel::SetFieldLineRadius(const double radius) {
  m_FieldLineRadius = radius;
}

double PoistatsModel::GetFieldLineRadius() const { return m_FieldLineRadius; }
