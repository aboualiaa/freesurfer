...
void FieldLineInitPathStrategy::CalculateInitialPath() {
  ...
  PoistatsModel::MatrixType initialPoints =
      this->GetInitialPoints(startPoint, endPoint);
  ...
  for (int i = 0; i < nSamples; i++) {
    ...
    const double position[] = {(*rethreadedPath)[i][0], (*rethreadedPath)[i][1],
                               (*rethreadedPath)[i][2]};
...
void FieldLineInitPathStrategy::NormalizeVector(double *v, const int size) {
...
void FieldLineInitPathStrategy::GetUpVector(double *up, const double *lookAt,
                                            const double angle) {
...
vnl_matrix<double> FieldLineInitPathStrategy::GetRotationMatrix(
    const double *lookAt, const double *up, const double *right) {
...
void FieldLineInitPathStrategy::GetNewStartAndEndPoints(int *startPoint,
                                                        int *endPoint) {
...
FieldLineInitPathStrategy::GetInitialPoints(const int *startPoint,
                                            const int *endPoint) {
