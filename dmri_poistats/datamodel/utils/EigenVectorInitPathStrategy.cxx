...
void EigenVectorInitPathStrategy::SetEigenVectors(MRI *eigenVectors) {
  m_EigenVectors = eigenVectors;
}

void EigenVectorInitPathStrategy::CacheGradientVolumes() {
  ...
  if (seedVolume != NULL && seedValues != NULL) {
    ...
    for (int cSeeds = 0; cSeeds < nGradientVolumes; cSeeds++) {
      ...
      // create the distance transform, all the values should be set before
      // running
      MRI *distanceTransform = this->GetDistanceVolume(destinationSeedLabel);

      // now that we have the transform to get to the second seed, we want to
      // take the derivatives of the distance transform to see what direction we
      // need to take to get to the second seed
    ...
    std::cerr << "EigenVectorInitPathStrategy::CacheGradientVolumes() -- seed "
                 "volume is null"
              << std::endl;
  }
}

void EigenVectorInitPathStrategy::CalculateInitialPath() {
  ...
  for (unsigned int cSeeds = 0; cSeeds < (seedValues->size() - 1); cSeeds++) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      // the previous point is set to this initially so that when the
      // orientation of the eigenvector is determined, we can find the one that
      // points most toward the end point initially
    ...
    std::vector<int *> destinationSeeds =
        this->GetSeedPoints(destinationSeedLabel);
    ...
    while (!this->IsInRegion(currentPointInt, destinationSeeds)) {
      ...
      if (this->ShouldFlipEigenVector(previousPointDouble, currentPointDouble,
                                      eigenVector)) {
      ...
      for (int cDim = 0; cDim < 3; cDim++) {
        ...
        currentPointDouble[cDim] =
            currentPointDouble[cDim] - randomNumber * gradients[cDim] +
      ...
      for (int cDim = 0; cDim < 3; cDim++) {
        ...
        currentPointInt[cDim] =
            static_cast<int>(round(currentPointDouble[cDim]));
      }
    }
...
MRI *EigenVectorInitPathStrategy::GetDistanceVolume(const int label) {
  ...
  // from fastmarching
  MRI *distanceTransform =
      MRIextractDistanceMap(seedVolume, NULL, label, maxDistance, mode, NULL);
  MRIcopyHeader(seedVolume, distanceTransform);

  return distanceTransform;
}

void EigenVectorInitPathStrategy::GetGradient(MRI *gradientsVol, int *index,
                                              double *oGradient) {

  for (int cDim = 0; cDim < 3; cDim++) {
    // get the gradient at the current point
    oGradient[cDim] =
        MRIgetVoxVal(gradientsVol, index[0], index[1], index[2], cDim);
  }
}

void EigenVectorInitPathStrategy::GetEigenVector(int *index, double *oVector) {

  for (int cDim = 0; cDim < 3; cDim++) {
    // get the eigenvector at the current point
    oVector[cDim] =
        MRIgetVoxVal(m_EigenVectors, index[0], index[1], index[2], cDim);
  }
}

bool EigenVectorInitPathStrategy::ShouldFlipEigenVector(
    const double *const previousPoint, const double *const currentPoint,
    const double *const eigenVector) {
...
void EigenVectorInitPathStrategy::DeleteGradientVolumes() {

  // go through all the gradient volumes and deallocate them
  for (std::vector<MRI *>::iterator it = m_GradientVolumes.begin();
       it != m_GradientVolumes.end(); it++) {
    MRI *volume = (*it);
    if (volume != NULL) {
      MRIfree(&volume);
    }
