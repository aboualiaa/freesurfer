...
InitializePath::InitializePath(PoistatsModel *model) {
  ...
  std::cerr << "InitializePath::InitializePath( PoistatsModel *model )"
            << std::endl;
...
void InitializePath::SetSeedVolume(MRI *volume) { m_SeedVolume = volume; }

void InitializePath::SetSeedValues(std::vector<int> *values) {
  m_SeedValues = values;
}

itk::Array2D<double> *InitializePath::GetInitialPath() { return m_InitialPath; }

void InitializePath::GetRandomSeedPoint(int *startSeed, const int label) {
...
std::vector<int *> InitializePath::GetSeedPoints(const int label) {
...
bool InitializePath::IsInRegion(int *point, std::vector<int *> region) {

  bool isInRegion = false;

  // go through all the seeds and determine if point is in the region
  for (std::vector<int *>::iterator itRegion = region.begin();
       !isInRegion && itRegion != region.end(); itRegion++) {
...
void InitializePath::FreeVector(std::vector<int *> v) {
...
void InitializePath::FreeVector(std::vector<double *> v) {
...
void InitializePath::CopyPathToOutput(std::vector<double *> path) {
  ...
  int row = 0;
  // copy the elements
  for (std::vector<double *>::iterator it = path.begin(); it != path.end();
       it++, row++) {
...
void InitializePath::EnsureWithinBounds(double *point) {

  // bounds of the volume
  const double bounds[] = {m_SeedVolume->width - 1, m_SeedVolume->height - 1,
