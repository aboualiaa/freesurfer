...
const double PoistatsFilter<TInputImage, TOutputImage>::NO_ZERO_SHELL_252[][3] =
    {{-0.99419, 0, -0.107642},
 ...
 */
template <class TInputImage, class TOutputImage>
PoistatsFilter<TInputImage, TOutputImage>::PoistatsFilter() {
  ...
  m_Replicas =
      new PoistatsReplicas(this->m_PoistatsModel, DEFAULT_NUMBER_OF_REPLICAS);
...
template <class TInputImage, class TOutputImage>
int PoistatsFilter<TInputImage, TOutputImage>::GetNumberOfReplicas() {
  return this->m_Replicas->GetNumberOfReplicas();
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetNumberOfReplicas(
    const int nReplicas) {
...
PoistatsFilter<TInputImage, TOutputImage>::GetOdfLookUpTable() {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::AllocateOutputImage(
    const int outputIndex, OutputImagePointer image) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetPositiveMinimumInt(
    MatrixPointer points, const int radius, VoxelIndexPointer minimum) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetPositiveMaximumInt(
    MatrixPointer points, const int radius, VoxelIndexPointer maximum,
    OutputRegionType pointsRegion) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::ConvertPointsToImage(
    MatrixPointer points, OutputImagePointer image) {
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GenerateData() {
  ...
  for (this->m_CurrentIteration = 1, this->m_CurrentLull = 0;
    ...
    for (int cReplica = 0; cReplica < this->GetNumberOfReplicas(); cReplica++) {
      ...
      MatrixType lowTrialPath(currentBasePath->rows(), currentBasePath->cols());
      ...
      */
      ...
      if (shouldExchange) {
        ...
        const double probabilityExchange =
            this->m_Replicas->CalculateProbablityExchange(
                randomReplicaIndex, followingRandomReplicaIndex);
  ...
  //  std::cerr << "calculate best path probabilities for each replica..." <<
  //  std::endl; this->CalculateBestPathProbabilities();
...
template <class TInputImage, class TOutputImage>
double
PoistatsFilter<TInputImage, TOutputImage>::GetCurrentMeanOfEnergies() const {
  return this->m_Replicas->GetCurrentMeanOfEnergies();
}

template <class TInputImage, class TOutputImage>
double
PoistatsFilter<TInputImage, TOutputImage>::GetCurrentMinOfEnergies() const {
  return this->m_Replicas->GetMinimumCurrentEnergy();
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetMghEigenVectors(
    MRI *vectors) {
  this->m_PoistatsModel->SetEigenVectors(vectors);
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetMghSeeds(MRI *seeds) {
  this->m_PoistatsModel->SetSeedVolume(seeds);
}

template <class TInputImage, class TOutputImage>
typename PoistatsFilter<TInputImage, TOutputImage>::MatrixType
PoistatsFilter<TInputImage, TOutputImage>::GetBestPathsProbabilities() {

  // TODO: we're using the number of steps rather than the number of sample
  // points
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage,
                    TOutputImage>::CalculateBestPathProbabilities() {

  for (int cReplica = 0; cReplica < this->m_Replicas->GetNumberOfReplicas();
       cReplica++) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::ConstructOdfList() {
  /* MATLAB:
  ...
  */
  ...
  itk::ImageRegionConstIterator<InputImageType> inputImageIt(
  ...
  for (inputImageIt = inputImageIt.Begin(),
    ...
    if (!isPixelMasked) {
      ...
      for (TensorIterator tensorIterator = currentTensor.Begin();
           tensorIterator != currentTensor.End() && !hasZero;
           ++tensorIterator) {
      ...
      */
      ...
      for (int cTensorRow = 0; cTensorRow < nTensorRows; cTensorRow++) {
        ...
        for (int cTensorColumn = 0; cTensorColumn < nTensorColumns;
          ...
          //          const double flippedValue = currentTensor( cTensorRow,
          //          cTensorColumn );
          ...
          //          tensor[ cTensorRow ][ cTensorColumn ] = currentTensor(
          //          cTensorRow, cTensorColumn );
        }
      }
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetMagnetToSliceFrameRotation(
    MatrixPointer rotation) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GenerateRotationMatrix3u2v(
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::RoundPath(
    itk::Array2D<int> *outputRoundedArray, itk::Array2D<double> *inputArray) {
...
PoistatsFilter<TInputImage, TOutputImage>::GetOptimalDensity() {
...
template <class TInputImage, class TOutputImage>
typename PoistatsFilter<TInputImage, TOutputImage>::OutputImageType *
PoistatsFilter<TInputImage, TOutputImage>::GetDensity() {
  return dynamic_cast<OutputImageType *>(
      this->ProcessObject::GetOutput(OPTIMAL_PATH_DENSITY_OUTPUT));
}

template <class TInputImage, class TOutputImage>
int PoistatsFilter<TInputImage, TOutputImage>::GetNumberOfInitialPoints()
    const {
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::InitPaths() {
  /* MATLAB:
    % initialize paths
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetInitialPoints(
    MatrixPointer initialPoints) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetPointClosestToCenter(
    MatrixPointer seeds, ArrayPointer closestSeed) {
...
template <class TInputImage, class TOutputImage>
double PoistatsFilter<TInputImage, TOutputImage>::GetDistance(
    vnl_vector<double> *point1, ArrayPointer point2) {
...
template <class TInputImage, class TOutputImage>
int PoistatsFilter<TInputImage, TOutputImage>::GetNumberOfControlPoints() {
  return this->m_PoistatsModel->GetNumberOfControlPoints();
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetNumberOfControlPoints(
    const int nPoints) {
  this->m_PoistatsModel->SetNumberOfControlPoints(nPoints);
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetNumberOfSteps(
    const int nSteps) {
  this->m_Replicas->SetNumberOfSteps(nSteps);
}

template <class TInputImage, class TOutputImage>
int PoistatsFilter<TInputImage, TOutputImage>::GetNumberOfSteps() {
...
template <class TInputImage, class TOutputImage>
double PoistatsFilter<TInputImage, TOutputImage>::CalculateOdfPathEnergy(
    MatrixPointer path, ArrayPointer *odfs, ArrayPointer outputEnergies) {
  ...
  for (unsigned int cPath = 0; cPath < pathDifference.rows(); cPath++) {
    for (unsigned int cDimension = 0; cDimension < pathDifference.cols();
         cDimension++) {
  ...
  if (anglesBetweenPathVectors.max_value() > largeAngle) {
    ...
    for (unsigned int cRow = 0; cRow < dotProductPerGeoDirection.rows();
         cRow++) {
      for (unsigned int cColumn = 0; cColumn < dotProductPerGeoDirection.cols();
           cColumn++) {
    ...
    MatrixType pathGeoAngles(dotProductPerGeoDirection.rows(),
                             dotProductPerGeoDirection.cols());
    for (unsigned int cRow = 0; cRow < pathGeoAngles.rows(); cRow++) {
      for (unsigned int cColumn = 0; cColumn < pathGeoAngles.cols();
           cColumn++) {
    ...
    /* MATLAB:
      odflist = odflist(1:(end-1),:); % n - 1 sets of odfs, because they
                                    % correspond to path segments, not path
    points. % odfvalues are the baysian conditional posterior probability
    distribution %  sum(A,2) will sum along the rows odfvalues =
    sum(odflist.*dm,2);
    */
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::CalculateDensityMatrix(
    itk::Array2D<double> *angles, itk::Array2D<double> *densityMatrix) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::CalculateAnglesBetweenVectors(
    itk::Array2D<double> *vectors, itk::Array<double> *angles) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetOdfsAtPoints(
    itk::Array<double> **outputOdfs, itk::Array2D<int> *inputPoints) {
...
template <class TInputImage, class TOutputImage>
itk::Array2D<double> *
PoistatsFilter<TInputImage, TOutputImage>::GetStartSeeds() {

  return this->m_Seeds[0];
}

template <class TInputImage, class TOutputImage>
itk::Array2D<double> *PoistatsFilter<TInputImage, TOutputImage>::GetEndSeeds() {
  return this->m_Seeds[this->m_Seeds.size() - 1];
}

template <class TInputImage, class TOutputImage>
itk::Array<double> PoistatsFilter<TInputImage, TOutputImage>::GetSamples() {
...
void PoistatsFilter<TInputImage, TOutputImage>::TakeUnionOfSeeds(
  ...
  for (std::vector<SeedType>::iterator values2It = seeds2->begin();
    ...
    if (!isSeedFound) {
      itkGenericExceptionMacro(<< "seed " << value2
                               << " not found in seed volume.");
    }
  }
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetSeedVolume(
    SeedVolumePointer volume) {
...
void PoistatsFilter<TInputImage, TOutputImage>::ParseSeedVolume() {
  ...
  if (seedValueCountPairs.size() > 1) {
    ...
    for (SeedPairList::iterator valuesIt = seedValueCountPairs.begin();
      ...
      MatrixPointer currentSeeds = new MatrixType(
          nCurrentSeed, SeedVolumeIndexType::GetIndexDimension());
      ...
      for (seedIt = seedIt.Begin(); !seedIt.IsAtEnd(); ++seedIt) {
        ...
        if (pixelValue == seedValue) {
          ...
          for (unsigned int cIndex = 0;
               cIndex < SeedVolumeIndexType::GetIndexDimension(); cIndex++) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage,
                    TOutputImage>::CalculateFinalPathProbabilities() {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetPathProbabilities(
    MatrixPointer path, ArrayPointer probabilities) {
  ...
  */
  ...
  MatrixPointer outputProbabilities = m_PoistatsModel->CubicSplineInterpolation(
      &rawProbablities, &originalPathGrid, probabilities->size());

  for (unsigned int cProbability = 0; cProbability < probabilities->size();
       cProbability++) {

    (*probabilities)[cProbability] = (*outputProbabilities)[cProbability][0];
  }
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetRandomSeed(const long seed) {

  this->m_PoistatsModel->SetRandomSeed(seed);
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::GetAggregateReplicaDensities(
    MatrixListType replicaPaths, OutputImagePointer aggregateDensities) {
  ...
  */
  ...
  for (unsigned int cProbability = 0; cProbability < probability.size();
       cProbability++) {
    ...
    if (currentReplicaProbablity > minimumReplicaContributation) {
      ...
      for (currentReplicaIterator = currentReplicaIterator.Begin(),
          aggregateIterator = aggregateIterator.Begin();
           ...
           ++currentReplicaIterator, ++aggregateIterator) {
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetMaskVolume(
    MaskVolumePointer volume) {
  ...
  for (maskIterator = maskIterator.Begin(); !maskIterator.IsAtEnd();
       ++maskIterator) {
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetNextSeedValueToUse(
    int seedValue) {
...
PoistatsFilter<TInputImage, TOutputImage>::GetTensorGeometry() {
  ...
  if (this->m_TensorGeometry.empty()) {
    ...
    this->m_TensorGeometry =
        VnlMatrixType(*NO_ZERO_SHELL_252, numberOfOdfs, numberOfTensorAxes);
  }
 ...
 */
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::TakeUnionOfMaskAndSeeds() {
...
template <class TInputImage, class TOutputImage>
int PoistatsFilter<TInputImage, TOutputImage>::GetInitialSigma() const {
  return m_PoistatsModel->GetInitialSigma();
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetInitialSigma(
    const int initialSigma) {
  m_PoistatsModel->SetInitialSigma(initialSigma);
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetFieldLineRadius(
    const double radius) {
...
template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage,
                    TOutputImage>::SetUsingFieldLineInitialization() {
  m_PoistatsModel->SetInitializePathMode(PoistatsModel::INITIALIZE_FIELD_LINE);
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage,
                    TOutputImage>::SetUsingEigenVectorInitialization() {
  m_PoistatsModel->SetInitializePathMode(
      PoistatsModel::INITIALIZE_EIGEN_VECTOR);
}

template <class TInputImage, class TOutputImage>
void PoistatsFilter<TInputImage, TOutputImage>::SetUsingNormalInitialization() {
...
PoistatsFilter<TInputImage, TOutputImage>::GetBestTrialPaths() {
