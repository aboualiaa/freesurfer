...
void TestPoistatsReplicas::TestRethreadPath() {
  ...
  const double actualRethreadShortPath[][3] = {
      {65, 46, 37}, {68.9159, 60.3864, 39.0484}, {64, 73, 36}};
  ...
  const double actualRethreadPath[][3] = {{0, 0, 0},
  ...
  resultRethreadedPath = m_PoistatsModel->RethreadPath(&originalPath, nSamples);

  CPPUNIT_ASSERT_EQUAL(nSamples,
                       static_cast<int>(resultRethreadedPath->rows()));
  CPPUNIT_ASSERT_EQUAL(nInputMatrixColumns,
                       static_cast<int>(resultRethreadedPath->cols()));
...
void TestPoistatsReplicas::
    TestGetNormalizedMeanCurrentPreviousEnergiesDifference() {

  std::cerr << "TestGetNormalizedMeanCurrentPreviousEnergiesDifference"
            << std::endl;
...
void TestPoistatsReplicas::TestGetRandomSortedFirstSecondReplicas() {
  ...
  m_Replicas->GetRandomSortedFirstSecondReplicas(firstActual, secondActual);
