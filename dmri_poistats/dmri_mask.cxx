...
const std::string MaskExe::FLAG_OUTPUT_DIRECTORY =
    MaskExe::FLAG_DELIMITER + "o";
...
MaskExe::MaskExe(int inArgs, char **iaArgs)
  ...
  SetNextRequiredArgument(FLAG_EIGENVECTOR, "eigvec",
                          "Principle eigenvector input", "eigvec",
                          "must specify an eigenvector volume");

  SetNextRequiredArgument(FLAG_PATH, "path", "Path points input", "path",
...
void MaskExe::CalculatePathVectors(std::vector<double *> *iPathPoints,
                                   std::vector<double *> *oPathVectors) {
...
MRI *MaskExe::MaskPath(MRI *iEigenVectors, std::vector<double *> *iPathPoints,
                       std::vector<double *> *iPathVectors) {
  ...
  for (unsigned int n = 0; n < iPathVectors->size(); n++) {
    ...
    // volume
    const int roundedPoint[3] = {static_cast<int>(round(pathPoint[0])),
...
void MaskExe::GetEigenVector(MRI *iEigenVolume, int *iIndex, double *oVector) {

  for (int nDim = 0; nDim < 3; nDim++) {
    // get the eigenvector at the current point
    oVector[nDim] =
        MRIgetVoxVal(iEigenVolume, iIndex[0], iIndex[1], iIndex[2], nDim);
  }
...
bool MaskExe::IsInPath(double *iEigenVector, double *iPathVector) {
