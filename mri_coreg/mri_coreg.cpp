...
int main(int argc, char *argv[]) {
    ...
    for (c = 0; c < coreg->ref->width; c++) {
      for (r = 0; r < coreg->ref->width; r++) {
        for (s = 0; s < coreg->ref->width; s++) {
          ...
          if (m < 0.5)
            MRIsetVoxVal(coreg->ref, c, r, s, 0, 0.0);
        }
      }
    }
  }
    ...
    for (c = 0; c < coreg->mov->width; c++) {
      for (r = 0; r < coreg->mov->width; r++) {
        for (s = 0; s < coreg->mov->width; s++) {
          ...
          if (m < 0.5)
            MRIsetVoxVal(coreg->mov, c, r, s, 0, 0.0);
        }
      }
    }
  }
  ...
  // Initial parameters
  for (n = 0; n < 12; n++)
    coreg->params[n] = cmdargs->params[n];
  ...
  for (n = 0; n < cmdargs->nsep; n++) {
    ...
    if (coreg->seplist[n] < coreg->sepmin)
      coreg->sepmin = coreg->seplist[n];
  }
  ...
  for (n = 0; n < coreg->nsep; n++) {
    ...
    printf("sep = %d -----------------------------------\n", coreg->sep);
    if (n == 0 && cmdargs->DoBF)
      COREGoptBruteForce(coreg, cmdargs->BFLim, 1, cmdargs->BFNSamp);
    coreg->startmin = 1;
    COREGMinPowell();
  }
  if (coreg->fplogcost)
  ...
  if (cmdargs->outparamfile) {
    ...
    for (n = 0; n < coreg->nparams; n++)
      fprintf(fp, "%15.8lf ", coreg->params[n]);
    fprintf(fp, "\n");
    fclose(fp);
  }
  printf("nhits = %d out of %d, Percent Overlap: %5.1f\n", coreg->nhits,
  ...
  if (!cmdargs->refconf) {
    printf("To check run:\n");
    printf("   tkregisterfv --mov %s --targ %s --reg %s ", cmdargs->mov,
           cmdargs->ref, cmdargs->outreg);
    if (cmdargs->subject)
      printf("--s %s --surfs ", cmdargs->subject);
  }
  printf("\n\n");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      cmdargs->cras0 = 0;
    else if (!strcasecmp(option, "--conf-ref"))
      cmdargs->refconf = 1;
    else if (!strcasecmp(option, "--bf"))
      cmdargs->DoBF = 1;
    else if (!strcasecmp(option, "--no-bf"))
      cmdargs->DoBF = 0;
    else if (!strcasecmp(option, "--mov-oob"))
      cmdargs->MovOOBFlag = 1;
    else if (!strcasecmp(option, "--no-mov-oob"))
      cmdargs->MovOOBFlag = 0;
      ...
      nargsused = 1;
    } else if (!strcasecmp(option, "--6"))
      cmdargs->dof = 6;
    else if (!strcasecmp(option, "--9"))
      cmdargs->dof = 9;
    else if (!strcasecmp(option, "--12"))
      cmdargs->dof = 12;
      ...
      else if (!strcasecmp(option, "--diag-verbose")) Gdiag =
          (Gdiag & DIAG_VERBOSE);
      else {
  ...
  static void print_usage() {
    ...
    printf("   --xytrans+zrot : for 2D images uses shifts in x and y and rot "
           "about z (no scale)\n");
    printf("   --xztrans+yrot : for 2D images uses shifts in x and z and rot "
           "about y (no scale) (was --2dz)\n");
    printf("   --ref-mask refmaskvol : mask ref with refmaskvol\n");
    printf(
        "   --no-ref-mask : do not mask ref (good to undo aparc+aseg.mgz, put "
        "AFTER --s)\n");
    ...
    printf(
        "   --sep voxsep1 <--sep voxsep2> : set spatial scales (def is 2 vox "
        "and 4 vox)\n");
    printf("   --trans Tx Ty Tz : initial translation in mm (implies "
           "--no-cras0)\n");
    ...
    printf(
        "   --no-mov-oob : do not count mov voxels that are out-of-bounds as "
        "0 (default)\n");
...
#pragma omp parallel for if_ROMP(assume_reproducible) reduction(+ : nhits)
        ...
        for (rref = 0; rref < coreg->ref->height; rref += coreg->sep) {
          for (sref = 0; sref < coreg->ref->depth; sref += coreg->sep) {
            ...
            if (coreg->DoCoordDither) {
              ...
              dcref += coreg->sep *
                       MRIgetVoxVal(coreg->cdither, cref, rref, sref, 0);
              drref += coreg->sep *
                       MRIgetVoxVal(coreg->cdither, cref, rref, sref, 1);
              dsref += coreg->sep *
                       MRIgetVoxVal(coreg->cdither, cref, rref, sref, 2);
              if (dcref > coreg->ref->width - 1)
                dcref = coreg->ref->width - 1;
              if (drref > coreg->ref->height - 1)
                drref = coreg->ref->height - 1;
              if (dsref > coreg->ref->depth - 1)
                dsref = coreg->ref->depth - 1;
            }

            double dcmov =
                V2V[0] * dcref + V2V[4] * drref + V2V[8] * dsref + V2V[12];
            double drmov =
                V2V[1] * dcref + V2V[5] * drref + V2V[9] * dsref + V2V[13];

            int oob = 0;
            if (dcmov < 0 || dcmov > coreg->mov->width - 1)
              oob = 1;
            if (drmov < 0 || drmov > coreg->mov->height - 1)
              oob = 1;

            double dsmov = 0;
            if (coreg->optschema != 2 && coreg->optschema != 4) {
              dsmov =
                  V2V[2] * dcref + V2V[6] * drref + V2V[10] * dsref + V2V[14];
              if (dsmov < 0 || dsmov > coreg->mov->depth - 1)
                oob = 1;
            }

            double vf;
            if (!oob) {
              vf = COREGsamp(coreg->f, dcmov, drmov, dsmov, coreg->mov->width,
                             coreg->mov->height, coreg->mov->depth);
              nhits++;
            } else {
              if (coreg->MovOOBFlag)
                vf = 0;
            ...
            double vg =
                COREGsamp(coreg->g, dcref, drref, dsref, coreg->ref->width,
                          coreg->ref->height, coreg->ref->depth);
    ...
    if (!coreg->H0)
      coreg->H0 = AllocDoubleMatrix(256, 256);
    ...
    coreg->nhits = nhits;
    coreg->pcthits =
        pow(coreg->sep, 3) * (double)100.0 * nhits / coreg->nvoxref;
...
*/
  ...
  MATRIX *MRIgetVoxelToVoxelXformBase(MRI * mri_src, MRI * mri_dst,
    ...
    if (SrcRAS2DstRAS) {
      ...
      SrcVox2DstVox =
          MatrixMultiplyD(m_ras2vox_dst, m_vox2ras_src, SrcVox2DstVox);
...
*/
  ...
  int WriteDoubleMatrix(char *fname, char *fmt, double **M, int rows,
                        int cols) {
    FILE *fp;
    fp = fopen(fname, "w");
...
*/
  double *SumVectorDoubleMatrix(double **M, int rows, int cols, int dim,
    ...
    if (dim == 1) {
      if (!sumvect)
        sumvect = (double *)calloc(sizeof(double), rows);
      *nv = rows;
      for (r = 0; r < rows; r++) {
        sumvect[r] = 0;
        for (c = 0; c < cols; c++)
      ...
      if (!sumvect)
        sumvect = (double *)calloc(sizeof(double), cols);
      *nv = cols;
      for (c = 0; c < cols; c++) {
        sumvect[c] = 0;
        for (r = 0; r < rows; r++)
 ...
 */
  ...
  double COREGcost(COREG * coreg) {
    ...
    if (coreg->fplogcost) {
      ...
      for (n = 0; n < coreg->nparams; n++)
        fprintf(fp, "%7.5f ", coreg->params[n]);
      fprintf(fp, "  %9.7f\n", coreg->cost);
      fflush(fp);
    }
  ...
  float COREGcostPowell(float *pPowel) {
    ...
    if (false) {
      ...
      for (n = 0; n < coreg->nparams; n++)
        fprintf(fp, "%12.8f ", coreg->params[n]);
  ...
  int COREGMinPowell() {
    ...
    OpenPowell2(pPowel, xi, dof, coreg->ftol, coreg->linmintol,
                coreg->nitersmax, &coreg->niters, &coreg->fret,
                COREGcostPowell);
  ...
  int COREGpreproc(COREG * coreg) {
    ...
    for (n = 0; n < 3; n++) {
      ...
      if (coreg->movgstd[n] > 0)
        DoSmooth = 1;
    }
    printf("mov gstd %6.4f %6.4f %6.4f\n", coreg->movgstd[0], coreg->movgstd[1],
           coreg->movgstd[2]);
    ...
    for (n = 0; n < 3; n++) {
      ...
      if (coreg->refgstd[n] > 0)
        DoSmooth = 1;
    }
    printf("ref gstd %6.4f %6.4f %6.4f\n", coreg->refgstd[0], coreg->refgstd[1],
           coreg->refgstd[2]);
    if (DoSmooth && coreg->DoSmoothing) {
 ...
 */
  ...
  int COREGoptBruteForce(COREG * coreg, double lim0, int niters, int n1d) {
    ...
    for (iter = 0; iter < niters; iter++) {
      ...
      fp = stdout;
      fprintf(fp, "#BF# sep=%2d iter=%d lim=%4.1f delta=%4.2lf ", coreg->sep,
              iter, lim, pdelta);
      for (n = 0; n < coreg->nparams; n++)
        fprintf(fp, "%9.5f ", coreg->params[n]);
      fprintf(fp, "  %9.7f\n", mincost);
      fflush(fp);
    ...
    if (BakMovOOBFlag == 0)
      printf("Turning  MovOOB back off after brute force search\n");
