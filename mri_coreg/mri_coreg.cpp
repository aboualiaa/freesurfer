...
static void print_usage(void) {
  ...
  printf("   --no-ref-mask : do not mask ref (good to undo aparc+aseg.mgz, put "
         "AFTER --s)\n");
  ...
  printf("   --sep voxsep1 <--sep voxsep2> : set spatial scales (def is 2 vox "
         "and 4 vox)\n");
  printf(
...
double COREGsamp(unsigned char *f, const double c, const double r,
  ...
  val = cpd * rpd * spd * f[COREGvolIndex(ncols, nrows, nslices, cm, rm, sm)] +
      ...
      for (rref = 0; rref < coreg->ref->height; rref += coreg->sep) {
        for (sref = 0; sref < coreg->ref->depth; sref += coreg->sep) {
          ...
          if (coreg->DoCoordDither) {
            ...
            dcref +=
                coreg->sep * MRIgetVoxVal(coreg->cdither, cref, rref, sref, 0);
            drref +=
                coreg->sep * MRIgetVoxVal(coreg->cdither, cref, rref, sref, 1);
            dsref +=
                coreg->sep * MRIgetVoxVal(coreg->cdither, cref, rref, sref, 2);
