...
int main(int argc, char **argv) {
  ...
  if (maskid != nullptr) {
    ...
    if (vol->width != maskvol->width && vol->depth != maskvol->depth &&
  ...
  if (fwhm > 0) {
    ...
    for (n = 0; n < nclusters; n++) {
      if (D == 2)
        ClusterSize = ClusterList[n]->nmembers * colres * rowres;
      if (D == 3)
        ClusterSize = ClusterList[n]->nmembers * colres * rowres * sliceres;
      if (threshsign == 0)
        grfsearchspace = grfsearchspace / 2.0; // This is a hack for abs
      if (AdjustThreshWhenOneTail) {
        pval = RFprobZClusterSigThresh(ClusterSize, threshmin, fwhm,
                                       grfsearchspace, D);
        if (threshsign == 0)
          pval = 2 * pval;
      } else {
      ...
      if (threshsign == 0)
        pval = 2 * pval; // This is a hack for abs
      ClusterList[n]->pval_clusterwise = pval;
    }
  }
  if (Bonferroni > 0) {
  ...
  if (labelbase != nullptr) {
    for (nlabelcluster = 0; nlabelcluster < nclusters; nlabelcluster++) {
      ...
      label = clustCluster2Label(ClusterList[nlabelcluster], vol, frame, colres,
                                 rowres, sliceres, FSA2Func);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
    ...
    else if (!strcasecmp(option, "--allowdiag"))
      allowdiag = 1;
    else if (!strcmp(option, "--maskinvert"))
      maskinvert = 1;
    else if (!strcmp(option, "--nofixtkreg"))
      fixtkreg = 0;
    else if (!strcmp(option, "--fixtkreg"))
      fixtkreg = 1;
      ...
      FixMNI = 1;
    else if (!strcasecmp(option, "--fsaverage"))
      UseFSAverage = 1;
...
static void print_usage() {
  ...
  printf("   --no-adjust  : do not adjust thresh for one-tailed tests (assumes "
         "z for GRF)\n");
  printf(
...
static void print_help() {
  ...
  printf(
      ...
      "\n"
      "  --reg registration-file : registration file created by either "
      "tkregister\n"
      "    or tkmedit. Note: the subject listed in the registration file must "
      "\n"
      "    have an entry under $SUBJECTS_DIR.\n"
      "\n"
      "  --thmin minthresh : voxel values must exceed this amount to be "
      "considered\n"
      ...
      "    infinity.\n"
      "\n"
      "  --sign sign-name : the value of a voxel must be this sign to be "
      "considered\n"
      ...
      "\n"
      "    Load one or more CSD files. CSD stands for 'Cluster Simulation "
      "Data'. This\n"
      "    file is produced by running mri_glmfit with --sim. The the "
      "threshold and hemi\n"
      "    info are obtained from the CSD file and cannot be specified on the "
      "command-\n"
      "    line. If more than one CSD is specified, they are merged into one "
      "CSD internally.\n"
      "    When a CSD file is specified, three more columns are added to the "
      "summary table:\n"
      "      1. CWP - cluster-wise pvalue. The pvalue of the cluster corrected "
      "for \n"
      ...
      "distribution\n"
      "    In addition, the user can specify --ocp, which saves the "
      "sigificance map of \n"
      "    the clusters in which the value of each voxel is the -log10(pvalue) "
      "of cluster\n"
      "    to which the vertex belongs.\n"
      "\n"
      "--csdpdf csdpdfile\n"
      "\n"
      "    Compute PDF/CDF of CSD data and save in csdpdffile. This is mostly "
      "good for debugging.\n"
      "\n"
      "--csdpdf-only\n"
      "\n"
      ...
      "\n"
      "  --minsize volume : minimum volume (in mm^3) of contiguous voxels "
      "that\n"
      ...
      "    the threshold criteria needed to become a cluster. See also "
      "--minsize.\n"
      "\n"
      "  --mindistance distance : minimum distance (in mm) that the peaks of "
      "two\n"
      "    clusters must be separated in order for them to be considered two\n"
      "    distinct clusters. If two clusters are closer than this amount, "
      "the\n"
      ...
      "    over which clusters will be searched. For example, this could be "
      "used\n"
      "    to restrict the search space to only the brain (ie, exclude "
      "eyeballs).\n"
      ...
      "    on the threshold and sign, take only voxels that are outside of "
      "the\n"
      "    mask.\n"
      "\n"
      "  --outmask outmask-volid: path/name of the final binary mask after\n"
      "    considering thresholding, sign, and inversion. This is mainly "
      "useful\n"
      "    for debuggin.\n"
      "\n"
      "  --outmask_type typename : name of the file format type of the "
      "outmask\n"
      "    volume. See FILE TYPES below.\n"
      "\n"
      ...
      "    where the value at each voxel is the cluster number (as found in "
      "the\n"
      ...
      "    label file in the subject's anatomical space. Note: make sure that "
      "the\n"
      "    label file includes a forward slash (/) or the label will be saved\n"
      "    into the subjects anatomical direcotry. For example: "
      "./mylabel.label.\n"
      "    Requires --nlabelcluster.\n"
      "\n"
      "  --nlabelcluster n : save the nth cluster (see -label) as a label "
      "file.\n"
      "\n"
      ...
      "Input Volume:      "
      "grp-subj-p1p2vlor/bold/sxa-h20/tal-ffx-rfx/novvfix/sig\n"
      "Frame Number:      0\n"
      "Minimum Threshold: 2\n"
      ...
      "Voxel Size:        64 mm^3\n"
      "Registration:      "
      "grp-subj-p1p2vlor/bold/sxa-h20/tal-ffx-rfx/register.dat\n"
...
static void check_options() {
  ...
  if (csd != nullptr) {
    ...
    if (!strncmp(signstring, "a", 1))
      threshsign = 0;
    else if (!strncmp(signstring, "p", 1))
      threshsign = +1;
    else if (!strncmp(signstring, "n", 1))
      threshsign = -1;
    else {
...
static MRI *MRIbinarize01(MRI *vol, float thmin, float thmax, char *thsign,
  ...
  if (strncasecmp(thsign, "neg", 3) == 0)
    ithsign = -1;
  else if (strncasecmp(thsign, "abs", 3) == 0)
    ithsign = 0;
  else if (strncasecmp(thsign, "pos", 3) == 0)
    ithsign = +1;
  else {
    ...
    for (row = 0; row < vol->height; row++) {
      for (slice = 0; slice < vol->depth; slice++) {
        for (frame = 0; frame < vol->nframes; frame++) {
          ...
          if (r)
            MRIIseq_vox(binvol, col, row, slice, frame) = (short)highval;
          else
            MRIIseq_vox(binvol, col, row, slice, frame) = (short)lowval;
