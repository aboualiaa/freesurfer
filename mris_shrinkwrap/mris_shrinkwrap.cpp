...
main(int argc, char *argv[])
{
  ...
  sprintf(parms.base_name, "%s_outer_skull%s%s", vol_name, output_suffix,
          suffix);
  parms.mri_brain = mri_dist;
  MRISpositionSurface(mris, mri_dist, mri_masked_smooth, &parms);
  parms.l_spring *= 0.5;
  ...
  sprintf(parms.base_name, "%s_outer_skin%s%s", vol_name, output_suffix,
          suffix);
  parms.mri_brain = mri_dist;
  initialize_surface_position(mris, mri_masked, 1);
  MRISpositionSurface(mris, mri_dist, mri_masked_smooth, &parms);
...
static void print_help() {
  ...
  fprintf(stderr, "\nThis program produces three surface files which are\n"
                  ...
                  "  outer_skull.tri\n"
                  "  outer_skin.tri\n");
  fprintf(stderr, "\t-t <threshold>  apply threshold to image then deform on "
                  "distance transform\n");
