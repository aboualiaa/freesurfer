...
double pointDistance(const DoubleVector &pt1, const DoubleVector &pt2) {
  double dsum(0);
...
WrapperLineType ConvertLine(const Tracer::LineType &line) {
...
WrapperLineSetType ComputeProfiles(int offset, double dspacing,
  ...
  for (int count(offset), maxVal(referenceLine.size() - offset); count < maxVal;
       ++count) {
...
WrapperLineSetType ComputeIsolines(const DoubleVector &vec,
  ...
  for (DoubleVector::const_iterator cit = vec.begin(); cit != vec.end();
       ++cit) {
    Tracer::LineType bufLine = tracer.ComputeIsoline(*cit);
    if (sqr(x0 - bufLine.front()[0]) + sqr(y0 - bufLine.front()[1]) >
        sqr(x0 - bufLine.back()[0]) + sqr(y0 - bufLine.back()[1]))
