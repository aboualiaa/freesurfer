...
Preprocessor::PointSetPointer Preprocessor::convertInputToPointSet() {
  ...
  while (counter < 2 && lines->GetNextCell(numPoints, vtkIds)) {
    ...
    for (unsigned int ui = 0; ui < numPoints; ++ui, ++idPtr) {
  ...
  while (counter < 2 && lines->GetNextCell(numPoints, vtkIds)) {
    ...
    for (unsigned int ui = 0; ui < numPoints; ++ui, ++idPtr) {
      ...
      prevPt = pt; // compute the distance - at each moment, remember the last
                   // point to speed up things
    }              // next ui, idPtr

    for (MapType::const_iterator cit = distMap.begin(); cit != distMap.end();
         ++cit) {
...
Preprocessor::convertPointSetToBinaryImage(PointSetPointer pointSet) {
  ...
  for (unsigned int uiDim = 0; uiDim < Dimension; ++uiDim)
    origin[uiDim] =
        bbox[2 * uiDim] - imageResolution[uiDim] * paddingSize[uiDim];
...
void Preprocessor::generateImageMask(LabelImagePointer image) {
  ...
  for (iter.GoToBegin(), citerOutside.GoToBegin(); !iter.IsAtEnd();
       ++iter, ++citerOutside) {
    if (citerOutside.Get())
      iter.Set(outsideValue);
    else if (!iter.Get())
      iter.Set(insideValue);
...
void Preprocessor::generateInitialDataImage() {
  ...
  for (iterData.GoToBegin(), citer.GoToBegin(); !iterData.IsAtEnd();
    ...
    if (citer.Get() == zeroLevelValue) {
      ...
      iterData.Set(
          inputData->GetPointData()->GetScalars()->GetComponent(foundId, 0));
    }
  } // next iterData, citer
}
...
void Preprocessor::Update() {
  ...
  PointSetPointer pointSet =
      convertInputToPointSet(); // also adds a float array to the input
