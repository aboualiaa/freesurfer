...
public:
  ...
  typedef typename MaterialContainerType::iterator MaterialIteratorType;
  typedef
      typename MaterialContainerType::const_iterator MaterialConstIteratorType;
  ...
  void orientation_pb(Frame f, InsertIterator ii) const;
...
TMesh<Cstr, n>::TMesh()
    : m_cmin(), m_cmax(), m_vpNodes(), m_vpElements(), m_mpMaterial() {
  m_cmin.invalidate();
  m_cmax.invalidate();
}
...
template <class Cstr, int n>
TMesh<Cstr, n>::TMesh(const TMesh<Cstr, n> &mesh)
    : m_cmin(), m_cmax(), m_vpNodes(), m_vpElements(), m_mpMaterial() {
  m_cmin.invalidate();
  m_cmax.invalidate();
  clone(mesh);
...
template <class Cstr, int n>
TMesh<Cstr, n> &TMesh<Cstr, n>::operator=(const TMesh<Cstr, n> &mesh) {
  free();
  clone(mesh);
  return *this;
}

template <class Cstr, int n> TMesh<Cstr, n>::~TMesh() { free(); }

template <class Cstr, int n> void TMesh<Cstr, n>::free() {
  // nodes
  std::transform(m_vpNodes.begin(), m_vpNodes.end(), m_vpNodes.begin(),
  ...
  // elements
  std::transform(m_vpElements.begin(), m_vpElements.end(), m_vpElements.begin(),
                 FunctorDeletePointer());
#if 0
...
template <class Cstr, int n> void TMesh<Cstr, n>::load(const char *fname) {
  // open stream
...
template <class Cstr, int n> void TMesh<Cstr, n>::load(std::istream &is) {
  ...
  for (unsigned int ui = 0; ui < noItems; ++ui) {
    ...
    tNode *pnode = Cstr::node(id, coords, delta, is_active);
    node_map[id] = pnode;
  ...
  for (unsigned int ui = 0; ui < noItems; ++ui) {
    ...
    if (materialIter == m_mpMaterial.end()) {
      ...
      excOs << "TMesh load - invalid material label " << materialLabel;
    ...
    for (unsigned int uj = 0; uj <= n; ++uj) {
      NodeMapConstIterator cit = node_map.find(node[uj]);
      if (cit == node_map.end())
...
template <class Cstr, int n> void TMesh<Cstr, n>::clone(const TMesh &mesh) {
  ...
  for (MaterialConstIteratorType cit = cbegin; cit != cend; ++cit) {
    ++counter;
    VMaterial *material =
        Cstr::material(cit->first, cit->second->get_E(), cit->second->get_nu());
  ...
  for (unsigned int i = 0; i < no_of_nodes; ++i) {
    ...
    for (int u = 0; u < n; ++u)
      isActive[u] = cpnode->is_dof_active(u);
    pnode = Cstr::node(cpnode->get_id(), cpnode->coords(), cpnode->delta(),
                       isActive);
    m_vpNodes.push_back(pnode);
  }
  ...
  for (unsigned int i = 0; i < no_of_elts; ++i) {
    ...
    pelt = Cstr::elt(cpelt->get_id(), pmat, vpNodes);
    m_vpElements.push_back(pelt);
  }
...
template <class Cstr, int n>
void TMesh<Cstr, n>::save(const char *fname) const {
  // open stream
  ...
  ZlibStringCompressor compressor;
  std::string compressed = compressor.compress(os.str(), Z_BEST_COMPRESSION);

  std::ofstream ofs(fname, std::ios::binary);

  ofs.write(compressed.c_str(), sizeof(char) * compressed.size());

  ofs.close();
}

template <class Cstr, int n> void TMesh<Cstr, n>::save(std::ostream &os) const {
...
template <class Cstr, int n>
void TMesh<Cstr, n>::set_constants(double _e, double _nu) {
  clear_materials();
  ...
  for (typename std::vector<tElement *>::iterator it = m_vpElements.begin();
       it != m_vpElements.end(); ++it)
    (*it)->set_material(pmat);
}

struct FunctorAddMaterial {
  const VMaterial *cp_material;
  FunctorAddMaterial(const VMaterial *input) : cp_material(input) {}
  template <class T> T operator()(T v) const { v->set_material(cp_material); }
};

template <class Cstr, int n>
void TMesh<Cstr, n>::set_default_material(VMaterial *pmaterial) {
  clear_materials();

  if (pmaterial->label().empty())
    pmaterial->set_label("default");
...
template <class Cstr, int n> void TMesh<Cstr, n>::clear_materials() {
  if (m_mpMaterial.size()) {
    for (MaterialIteratorType it = m_mpMaterial.begin();
         it != m_mpMaterial.end(); ++it)
      delete it->second;
    m_mpMaterial.clear();
  }
}
...
template <class Cstr, int n>
void TMesh<Cstr, n>::add_material(std::string strLabel, VMaterial *mat) {
  if (!strLabel.size()) {
...
template <class Cstr, int n>
void TMesh<Cstr, n>::assign_material(unsigned i, std::string strLabel) {
  if (i > m_vpElements.size() - 1) {
    std::cerr << "TMesh::assign_material -> index " << i
              << " beyond max val \n";
    return;
  }
  MaterialConstIteratorType mit = m_mpMaterial.find(strLabel);
  if (mit == m_mpMaterial.end()) {
    std::cerr << "TMesh::assign_material -> cannot find label " << strLabel
...
template <class Cstr, int n>
bool TMesh<Cstr, n>::get_node(unsigned int i, tNode **ppnode) const {
  if (i >= m_vpNodes.size()) {
    std::cerr << "TMesh::get_node -> incorrect index " << i
              << " >= " << m_vpNodes.size() << std::endl;
    *ppnode = NULL;
    return false;
  }
  *ppnode = m_vpNodes[i];
...
template <class Cstr, int n>
void TMesh<Cstr, n>::get_dst_box(tCoords &cmin, tCoords &cmax) const {
  typename std::vector<tElement *>::const_iterator cit = m_vpElements.begin();
...
template <class Cstr, int n>
unsigned int TMesh<Cstr, n>::add_node(tNode *pnode) {
  m_vpNodes.push_back(pnode);
  return m_vpNodes.size();
}

template <class Cstr, int n> int TMesh<Cstr, n>::add_elt(tElement *pelt) {
  m_vpElements.push_back(pelt);
...
struct FunctorSourceVolumeAccumulator {
  template <class In, class T> T operator()(T init, In data) {
...
template <class Cstr, int n> double TMesh<Cstr, n>::src_volume() const {
#if 0
  double dvol = 0.0;
  ...
  return dvol;
#endif
  return std::accumulate(m_vpElements.begin(), m_vpElements.end(), 0.0,
                         FunctorSourceVolumeAccumulator());
}

template <class Cstr, int n> double TMesh<Cstr, n>::dst_volume() const {
#if 0
  double dvol = 0.0;
  for (typename std::vector<tElement*>::const_iterator cit
       = m_vpElements.begin();
...
TMesh<Cstr,n>::remove_elts(In begin, In end) /*throw(gmpErr)*/
{
  ...
  std::transform(m_vpElements.begin(), m_vpElements.end(),
                 std::back_inserter(vecInitialMesh), FunctorIndexGrabber());
  ...
  for (ElementConstIterator cit = m_vpElements.begin();
       cit != m_vpElements.end(); ++cit) {
    if (std::binary_search(vecDifference.begin(), vecDifference.end(),
                           (*cit)->get_id()))
...
template <int n> bool positive(const TCoords<int, n> &pt) {
  for (int i = 0; i < n; ++i)
    if (pt(i) <= 0)
      return false;
...
template <class Cstr, int n> bool TMesh<Cstr, n>::check_elt_id() const {
  int index = 0;
  for (typename std::vector<tElement *>::const_iterator cit =
           m_vpElements.begin();
...
template <class Cstr, int n>
bool TMesh<Cstr, n>::orientation_pb(Frame f) const {
  typename std::vector<tElement *>::const_iterator cit;
  for (cit = m_vpElements.begin(); cit != m_vpElements.end(); ++cit)
    if ((*cit)->orientation_pb(f))
      return true;
...
template <class Cstr, int n>
template <class InsertIterator>
void TMesh<Cstr, n>::orientation_pb(Frame f, InsertIterator ii) const {
  typename std::vector<tElement *>::const_iterator cit;
  for (cit = m_vpElements.begin(); cit != m_vpElements.end(); ++cit)
    if ((*cit)->orientation_pb(f))
      ii = (*cit)->get_id();
...
template <class Cstr, int n>
std::pair<TCoords<double, n>, TCoords<double, n>> TMesh<Cstr, n>::invert() {
  tCoords cmin, cmax;
  cmin.set(10.0);
  cmax.set(-10.0);
...
int TMesh<Cstr, n>::get_neighboring_elts(
  ...
  tNode *pnode;
  for (int step = 0; step < radius && !eltStack.empty(); ++step) {
  ...
  eltIndex.clear();
  for (IndexSetType::const_iterator cit = eltSet.begin(); cit != eltSet.end();
       ++cit)
