...
template <class Cstr, int n> class TPbMeshCrop {
public:
  ...
  void Crop(MeshType *outMesh, MapType &nodeMap, MapType &elementMap,
...
private:
  ...
  void bfs(unsigned int seed, unsigned int radius, unsigned int maxRadius,
           IndexSetType &eltVisited, IndexSetType &nodeVisited);
...
void TPbMeshCrop<Cstr, n>::Crop(MeshType *outMesh, MapType &nodeMap,
  ...
  this->bfs(seed, radius, radius, eltVisited, nodeVisited);
...
void TPbMeshCrop<Cstr, n>::bfs(unsigned int seed, unsigned int radius,
  ...
  for (unsigned int ui(0), nnodes(cpseed->no_nodes()); ui < nnodes; ++ui) {
    ...
    if (nodeVisited.find(pnode->get_id()) != nodeVisited.end())
      continue;

    nodeVisited.insert(pnode->get_id());

    for (pnode->get_elt_citer(eltBegin, eltEnd); eltBegin != eltEnd; ++eltBegin)
      tetSet.insert(*eltBegin);
  }

  for (typename IndexSetType::const_iterator tetCiter = tetSet.begin();
       tetCiter != tetSet.end(); ++tetCiter)
    this->bfs(*tetCiter, radius, maxRadius, eltVisited, nodeVisited);
}
...
TPbMeshCrop<Cstr, n>::ComputeBoundingBox(MeshType *pmesh) {
  ...
  for (; nodeCiter != nodeEnd; ++nodeCiter) {
