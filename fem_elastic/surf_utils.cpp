...
void SurfacePointer::GetTypeFromName(const std::string &name) {
  ...
  std::cout << " surface name = " << name << " type = " << this->type
            << std::endl;
}
...
void convert_vox_to_surf(MRI_SURFACE *mris, MRI *vol) {
  int const nvertices = mris->nvertices;
  ...
  for (int vno = 0; vno < nvertices; ++vno) {
    ...
    double vx, vy, vz;
    MRIvoxelToSurfaceRAS(vol, v->x, v->y, v->z, &vx, &vy, &vz);
...
void compute_bbox(const SurfaceVectorType &vmris, tDblCoords &cmin,
                  tDblCoords &cmax) {
...
void compute_bbox(MRI_SURFACE *mris, tDblCoords &cmin, tDblCoords &cmax,
                  bool initVars) {
  tDblCoords cbuf;
  ...
  for (unsigned int ui = 1; ui < nvertices; ++ui, ++pvtx) {
    cbuf(0) = pvtx->x;
    cbuf(1) = pvtx->y;
    cbuf(2) = pvtx->z;
