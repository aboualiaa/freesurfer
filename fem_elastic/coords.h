...
typedef enum { cValid, cInvalid, cOutOfBounds, cOther } CoordsStatusType;
...
template <class T, int n> class TCoords {
public:
  TCoords();
  TCoords(T _x);
...
template <int n>
std::stack<TCoords<int, n>> *neighbors(const TCoords<int, n> &c);
...
template <class T, int n> TCoords<T, n>::TCoords() : m_status(cValid) {
  assert(n > 0);
}

template <class T, int n> TCoords<T, n>::TCoords(T _x) : m_status(cValid) {
  assert(n > 0);
  std::fill_n(m_pdata, n, _x);
}

template <class T, int n> void TCoords<T, n>::set(T _x) {
  std::fill_n(m_pdata, n, _x);
}

template <class T, int n> void TCoords<T, n>::clone(const TCoords<T, n> &_x) {
  for (int i = 0; i < n; ++i)
    m_pdata[i] = _x.m_pdata[i];
  this->status() = _x.status();
}

template <class T, int n> TCoords<T, n>::TCoords(const TCoords<T, n> &_x) {
  assert(n > 0);
  clone(_x);
}

template <class T, int n>
TCoords<T, n> TCoords<T, n>::operator=(const TCoords<T, n> &_x) {
  clone(_x);
  return *this;
}
...
template <class T, int n> void TCoords<T, n>::print(std::ostream &os) const {
  // os << "( ";
  os << m_pdata[0];
  for (int i = 1; i < n; ++i)
    os << " " << m_pdata[i];
...
template <int n>
double dot(const TCoords<double, n> &a, const TCoords<double, n> &b) {
  double dret = 0.0;
  for (int i = 0; i < n; ++i)
    dret += a(i) * b(i);
...
template <int n>
std::stack<TCoords<int, n>> *neighbors(const TCoords<int, n> &c) {
  std::stack<TCoords<int, n>> *pstack = new std::stack<TCoords<int, n>>;
