...
template <class T> T mySqr(T x) { return x * x; }

namespace gmp {

AffineTransform3d::AffineTransform3d() : Transform<3>(), m_pdata(NULL) {}
...
AffineTransform3d::doOwnImg(const tCoords &pt) const {
  ...
  for (int j = 0; j < 3; ++j)
    ret(j) = m_pdata[j] * pt(0) + m_pdata[j + 3] * pt(1) +
             m_pdata[j + 6] * pt(2) + m_pdata[j + 9];

  return ret;
}

void AffineTransform3d::doInput(std::istream &is) {
...
void AffineTransform3d::doOutput(std::ostream &os) const {
  if (!m_pdata)
...
void DenseDisplacementField::set_field(FieldPointer field) {
  if (!m_fieldInterpolator)
    // by default, create a linear interpolator
    m_fieldInterpolator =
        itk::VectorLinearInterpolateImageFunction<FieldType>::New();
  else
...
void DenseDisplacementField::set_mask(MaskPointer mask) {
...
void DenseDisplacementField::doOutput(std::ostream &os) const {
  ...
  // write size
  strTag = CreateTag(tagSize, this->PrepareTagSize(field));
  os.write(strTag.c_str(), strTag.size());

  // write start
  strTag = CreateTag(tagStart, this->PrepareTagStart(field));
  os.write(strTag.c_str(), strTag.size());

  // write spacing
  strTag = CreateTag(tagSpacing, this->PrepareTagSpacing(field));
  os.write(strTag.c_str(), strTag.size());

  // write origin
  strTag = CreateTag(tagOrigin, this->PrepareTagOrigin(field));
  os.write(strTag.c_str(), strTag.size());

  // write data
  strTag = CreateTag(tagData, this->PrepareTagData(field));
  os.write(strTag.c_str(), strTag.size());

  // if present, write mask information
  if (m_maskInterpolator) {
    strTag = CreateTag(
        tagMask, this->PrepareTagMask(m_maskInterpolator->GetInputImage()));
    os.write(strTag.c_str(), strTag.size());
  }
}
...
DeltaTransform3d::DeltaTransform3d()
    : m_interpolation(SAMPLE_TRILINEAR), m_field(NULL), m_mask(NULL) {}
...
DeltaTransform3d::tCoords DeltaTransform3d::doOwnImg(const tCoords &pt) const {
  ...
  if (m_mask) {
    MRIsampleVolumeFrameType(m_mask, pt(0), pt(1), pt(2), 0, m_interpolation,
                             &val);
    if (!val) {
      retVal.invalidate();
      return retVal;
  ...
  for (unsigned int ui = 0; ui < 3; ++ui) {
    MRIsampleVolumeFrameType(m_field, pt(0), pt(1), pt(2), ui, m_interpolation,
                             &val);
    retVal(ui) = val;
  }
...
void DeltaTransform3d::doInput(std::istream &is) {
  if (m_field) {
    MRIfree(&m_field);
    m_field = NULL;
  }
    ...
    for (int uk = 0; uk < m_field->depth; ++uk)
      for (int uj = 0; uj < m_field->height; ++uj)
        for (int ui = 0; ui < m_field->width; ++ui)
          MRIsetVoxVal(m_field, ui, uj, uk, un, TRead<float>(is));
    ...
    for (int uk = 0; uk < m_mask->depth; ++uk)
      for (int uj = 0; uj < m_mask->height; ++uj)
        for (int ui = 0; ui < m_mask->width; ++ui) {
          ...
          if (MRIvox(m_mask, ui, uj, uk))
            validCount++;
        }
  }
}

void DeltaTransform3d::doOutput(std::ostream &os) const {
  if (!m_field)
  ...
  for (int un = 0; un < m_field->nframes; ++un)
    for (int uk = 0; uk < m_field->depth; ++uk)
      for (int uj = 0; uj < m_field->height; ++uj)
        for (int ui = 0; ui < m_field->width; ++ui) {
          TWrite(os, MRIgetVoxVal(m_field, ui, uj, uk, un));
        }
...
FemTransform3d::tCoords FemTransform3d::doOwnImg(const tCoords &pt) const {
  if (!m_sharedMesh)
...
void FemTransform3d::doInput(std::istream &is) {
  m_sharedMesh = std::shared_ptr<CMesh3d>(new CMesh3d);

  m_sharedMesh->load(is);
}

void FemTransform3d::doOutput(std::ostream &os) const {
  if (!m_sharedMesh)
...
void FemTransform3d::doOwnInit() { m_sharedMesh->build_index_src(); }

void FemTransform3d::invert() {
  if (m_pInitial)
  ...
  // ugly, but how else?
  std::shared_ptr<CMesh3d> pmesh(
      new CMesh3d(*dynamic_cast<CMesh3d *>(&*m_sharedMesh)));
  pmesh->invert();
...
MRI *VolumeMorph::apply_transforms(MRI *input, bool cacheField,
  ...
  // setup the matrix for the fixed side
  {
    ...
    mat_template = MatrixMultiply(ras2vox_morph, vox2ras_crt, NULL);
  ...
  // setup the matrix for the moving side
  {
    ...
    mat_subject = MatrixMultiply(ras2vox_crt, vox2ras_morph, NULL);
  ...
  if (cacheField) {
    if (mriCache)
      MRIfree(&mriCache);
    mriCache = MRIallocSequence(
        m_template->width, m_template->height, m_template->depth, MRI_FLOAT,
        4); // 4 frames - one for each direction + 1 to indicate a valid voxel
  }
    ...
    for (int z = 0; z < mriOut->depth; ++z) {
      ...
      for (int y = 0; y < mriOut->height; ++y)
        for (int x = 0; x < mriOut->width; ++x) {
          ...
          img(1) = V3_Y(vMoving);
          img(2) = V3_Z(vMoving);
          // std::cout << " img assigned " << img(0) <<"," << img(1) <<","
          // <<img(2) << std::endl;

          //--------------------------
          // do nothing if out of bounds
          if (img(0) < 0 || img(0) > input->width - 1 || img(1) < 0 ||
              img(1) > input->height - 1 || img(2) < 0 ||
              img(2) > input->depth - 1)
            continue;

          ++voxValid;
          if (nframes == 1) {
            MRIsampleVolumeType(input, img(0), img(1), img(2), &val,
                                m_interpolationType);
            MRIsetVoxVal(mriOut, x, y, z, 0, val);
          } else {
            MRIsampleSeqVolumeType(input, img(0), img(1), img(2), valvect, 0,
                                   nframes - 1, m_interpolationType);
...
MRI *VolumeMorph::convert_transforms() const
// MRI* input,
//    bool cacheField,
//         const VG* vgOutput) const
{
  ...
  if (cacheField) {
    if (mriCache)
      MRIfree(&mriCache);
    mriCache = MRIallocSequence(
        m_vgFixed.width,  // m_template->width,
  ...
  MRI *mriWarpAsVolume =
      MRIallocSequence(m_vgFixed.width,  // m_template->width,
                       m_vgFixed.height, // m_template->height,
                       m_vgFixed.depth,  // m_template->depth,
                       MRI_FLOAT, 3);
    ...
    for (int z = 0; z < m_vgFixed.depth; ++z) {
      ...
      for (int y = 0; y < m_vgFixed.height; ++y)
        for (int x = 0; x < m_vgFixed.width; ++x) {
          ...
          if (img(0) < 0 || img(0) > m_vgMoving.width - 1 || img(1) < 0 ||
              img(1) > m_vgMoving.height - 1 || img(2) < 0 ||
              img(2) > m_vgMoving.depth - 1)
            continue;

          ++voxValid;
          // MRIsampleVolumeType( input, img(0), img(1), img(2), &val,
          // m_interpolationType); MRIsetVoxVal( mriOut, x,y,z,0, val);
...
MRIS *VolumeMorph::apply_transforms(MRIS *input) const {
  ...
  for (int ui = 0; ui < nvertices; ++ui) {
    ...
    pt = image(pt);
    if (!pt.isValid())
      continue; // better leave it as it was if it's not working
...
void VolumeMorph::save(const char *fname) {
  std::string strTag;
  ...
  // vol geom fixed
  strTag = ftags::CreateTag(tagVgFixed, this->PrepareTagVolGeom(m_vgFixed));
  os.write(strTag.c_str(), strTag.size());

  // vol geom moving
  strTag = ftags::CreateTag(tagVgMoving, this->PrepareTagVolGeom(m_vgMoving));
  os.write(strTag.c_str(), strTag.size());

  // write transforms
  ZlibStringCompressor
      compressor; // may have a mem leak, so move outside of loop
  for (TransformContainerType::iterator it = m_transforms.begin();
       it != m_transforms.end(); ++it) {
    std::ostringstream osit(std::ios::binary);
    saveTransform(osit, *it);

    std::string strBuf = compressor.compress(osit.str(), Z_BEST_COMPRESSION);
    std::cout << " writing transform size = " << strBuf.size() << std::endl;

    strTag = ftags::CreateTag(tagTransform, strBuf);
    os.write(strTag.c_str(), strTag.size());
  } // next transform
  ...
  ofs.write(os.str().c_str(), sizeof(char) * os.str().size());
}

#endif

void VolumeMorph::load(const char *fname, unsigned int bufferMultiplier,
                       bool clearExisting) {
  // read the transform
...
*/
void VolumeMorph::load_old(const char *fname, unsigned int bufferMultiplier,
                           bool clearExisting) {
  std::ifstream ifs(fname, std::ios::binary);
  if (!ifs)
    throw "VolumeMorph load - failed to open input stream";
  ...
  if (noTransforms > 0 && clearExisting)
    m_transforms.clear();

  std::cout << " got here\n";
  for (unsigned int ui = 0; ui < noTransforms; ++ui) {
    // read buffer size
...
void VolumeMorph::load_new(const char *fname, unsigned int bufferMultiplier,
  ...
  while (tagReader.Read()) {
    switch (tagReader.m_tag) {
    case tagVgFixed:
      this->ReadTagVolGeom(std::string(tagReader.m_data, tagReader.m_len),
                           m_vgFixed);
    ...
    case tagVgMoving:
      ...
      break;
    case tagTransform: {
      // ZlibStringCompressor compressor;// might be a memleak, so made function
      // scope
      compressor.m_bufferAllocationMultiplier = bufferMultiplier;
...
std::string VolumeMorph::PrepareTagVolGeom(const VOL_GEOM &vg) {
  std::ostringstream oss;
  TWrite(oss, vg.valid);
...
void VolumeMorph::ReadTagVolGeom(const std::string &strData, VOL_GEOM &vg) {
...
GCA_MORPH *VolumeMorph::exportGcam(MRI *mriMoving, bool useTemplateBoundingBox,
                                   int thresh, int padding) const {
  ...
  if (!mriCache) {
    std::cerr << "VolumeMorph::exportGcam -> no mriCache - this will take some "
                 "time\n";
    VOL_GEOM vgLike;
    initVolGeom(&vgLike);
    getVolGeom(this->m_template, &vgLike);
    MRI *mriTmp = this->apply_transforms(mriMoving, true,
                                         &vgLike); // start by caching the data
    if (!mriTmp)
      return NULL;
    MRIfree(&mriTmp);
  } else
  ...
  if (useTemplateBoundingBox) {
    ...
    if (box.x - padding < 0 || box.y - padding < 0 || box.z - padding < 0 ||
  ...
  MRI *croppedTemplate = MRIextractRegion(this->m_template, NULL, &box);
  if (mriMoving)
    GCAMinitVolGeom(gcam, mriMoving, croppedTemplate);
  ...
  for (int z(0), zbox(box.z); z < box.dz; ++z, ++zbox)
    for (int y(0), ybox(box.y); y < box.dy; ++y, ++ybox)
      for (int x(0), xbox(box.x); x < box.dx; ++x, ++xbox) {
        ...
        pnode->z = pnode->origz =
            MRIgetVoxVal(mriCache, xbox, ybox, zbox, 2) + z;
        ...
        if (std::abs(dval) > 1.0e-10) {
          pnode->label =
              128; // something real here - taken from GCAMcreateIntensityImage
          dmean += dval;
          dnum += 1.0;
        }
      }
  ...
  for (int z(0); z < box.dz; ++z)
    for (int y(0); y < box.dy; ++y)
      for (int x(0); x < box.dx; ++x) {
        GMN *pnode = &gcam->nodes[x][y][z];
        if (!pnode || pnode->invalid == GCAM_POSITION_INVALID)
          continue;
        pnode->gc->covars[0] = dmean;
      }

  std::cout << " gcam export invalid voxels = " << gcamInvalidVoxels
            << std::endl;
  return gcam;
}

void VolumeMorph::invert() {
...
std::shared_ptr<Transform<3>> loadTransform(std::istream &is,
                                            unsigned int zlibBufferMultiplier) {
