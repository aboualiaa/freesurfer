...
void SmallMatrix::set(double val) {
  std::fill_n(m_pdata, m_rows * m_cols, val);
}

SmallMatrix &SmallMatrix::operator=(const SmallMatrix &sm) {
  clone(sm);
  return *this;
}
...
void SmallMatrix::identity(int size) {
  if (m_pdata)
    delete[] m_pdata;
  m_rows = size;
  m_cols = size;
  ...
  std::fill_n(m_pdata, size * size, 0.0);

  for (int i = 0; i < size; ++i)
    m_pdata[i + i * size] = 1.0;
}

double SmallMatrix::operator()(int i, int j) const {
  return m_pdata[i * m_cols + j];
}

double &SmallMatrix::operator()(int i, int j) {
  return m_pdata[i * m_cols + j];
}

void SmallMatrix::clone(const SmallMatrix &sm) {
  if (m_pdata)
    delete[] m_pdata;
...
void SmallMatrix::set_block(const SmallMatrix &m, size_t start_row,
                            size_t start_col) {
  ...
  for (size_t i = start_row; i < max_row; ++i)
    for (size_t j = start_col; j < max_col; ++j)
      (*this)(i, j) = m(i - start_row, j - start_col);
}

SmallMatrix SmallMatrix::operator*(const SmallMatrix &m) {
  if (this->cols() != m.rows()) {
...
SmallMatrix SmallMatrix::transpose() const {
...
void SmallMatrix::inplace_transpose() {
...
SmallMatrix SmallMatrix::operator+(const SmallMatrix &m) const {
...
void SmallMatrix::operator+=(const SmallMatrix &m) {
  // error if the two matrices do not have the same size
  if (this->rows() != m.rows() || this->cols() != m.cols()) {
    std::cerr << " SmallMatrix::operator+= -> matrices have different sizes\n";
    exit(1);
...
std::ostream &operator<<(std::ostream &os, const SmallMatrix &sm) {
