...
void initOctree2(gmp::VolumeMorph &morph) {
  ...
  for (VolumeMorph::TransformContainerType::iterator it =
           morph.m_transforms.begin();
       it != morph.m_transforms.end(); ++it) {
    (*it)->performInit();
  } // next it
}
...
int main(int ac, const char **av) {
  ...
  try {
    ...
    std::cerr << " Exception caught while parsing cmd-line: " << e.what()
              << "\n Type --help for command info! " << std::endl;
    exit(1);
  }
  std::cout << " using bounding box: " << (params.useBoundingBox ? "yes" : "no")
            << std::endl;
  ...
  GCA_MORPH *gcam =
      pmorph->exportGcam(mriMoving, params.useBoundingBox, params.threshold);
  ...
  if (params.doTest && (!params.useBoundingBox || 1)) {
    ...
    MRI *mriOut = GCAMmorphToAtlas(mriMoving, gcam, mriFixed, 0,
                                   pmorph->m_interpolationType);
    std::cout << " AFTER MORPH\n";
    std::cout << " out MRI params = "
              << " width = " << mriOut->width << " height = " << mriOut->height
              << " depth = " << mriOut->depth
