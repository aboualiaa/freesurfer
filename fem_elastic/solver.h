...
template <class Cstr, int n> struct BC {
  ...
  virtual ~BC(){};
  virtual bool find_candidate(tMesh *pmesh) { return false; }
};
...
template <class Cstr, int n> struct BCNatural : public BC<Cstr, n> {
  ...
  BCNatural(const tCoords &_pt, const tCoords &_delta)
      : BC<Cstr, n>(_pt, _delta), pnode(NULL) {}
  virtual ~BCNatural(){};
...
template <class Cstr, int n> struct BCMfc : public BC<Cstr, n> {
  ...
  BCMfc(const tCoords &_pt, const tCoords &_delta)
      : BC<Cstr, n>(_pt, _delta), pelt(NULL) {}
  virtual ~BCMfc(){};
  virtual bool find_candidate(tMesh *pmesh) {
...
public:
  ...
  void add_bc_natural(const tCoords &pt, const tCoords &delta);
...
template <class Cstr, int n> class TDirectSolver : public TSolver<Cstr, n> {
public:
  TDirectSolver();
  ~TDirectSolver();
...
template <class Cstr, int n> TSolver<Cstr, n>::TSolver() {
  m_pmesh = NULL;
...
template <class Cstr, int n> void TSolver<Cstr, n>::clone(const TSolver &s) {
  m_vBc = s.m_vBc;
  // m_ticks = s.m_ticks;
  m_pmesh = s.m_pmesh;
...
void TSolver<Cstr, n>::add_bc_natural(const tCoords &pt, const tCoords &delta) {
...
template <class Cstr, int n>
void TSolver<Cstr, n>::add_bc_mfc(const tCoords &pt, const tCoords &delta) {
...
template <class Cstr, int n> int TSolver<Cstr, n>::solve() {
  ...
  ierr = PetscOptionsGetReal(NULL, "-penalty_weight", &m_mfcWeight, NULL);
  CHKERRQ(ierr);
...

  {
    ...
    char buffer[maxLen];
    ierr =
        PetscOptionsGetString(NULL, "-fem_print", buffer, maxLen, &petscFlag);
    if (petscFlag) {
      femPrint = true;
      PetscViewer viewer;
      PetscViewerBinaryOpen(PETSC_COMM_SELF, buffer, FILE_MODE_WRITE, &viewer);
  ...
  if (femPrint) {
    PetscViewer viewer;
    PetscViewerBinaryOpen(PETSC_COMM_SELF, "final_stif.bin", FILE_MODE_WRITE,
                          &viewer);
    MatView(m_stiffness, viewer);
    PetscViewerDestroy(viewer);
  }
  ...
  ierr = KSPSetTolerances(ksp, 1.0e-9, PETSC_DEFAULT, PETSC_DEFAULT,
                          PETSC_DEFAULT);
  CHKERRQ(ierr);
  if (initGuessNonZero)
...
template <class Cstr, int n> int TSolver<Cstr, n>::done_bc_natural() {
    ...
    if (tBCNatural *bc = dynamic_cast<tBCNatural *>(*it)) {
      ...
      if (pnode) {
        if (!m_useThreshold ||
          ...
          if (m_displayLevel > 1)
            std::cout << "setting bc " << pnode->coords() << " -> " << bc->delta
                      << "\n"
                      << "\t instead " << bc->pt
                      << " -> norm = " << vdelta.back().norm() << std::endl;
        }
      } else {
        std::cerr << "TSolver::done_bc_natural -> failed to find node close to "
                  << bc->pt << std::endl;
        bFailed = true;
      }
    }
  ...
  if (m_displayLevel && bFailed)
    std::cout << " !!!!! There were FAILED BCs\n";
  if (m_displayLevel) {
    std::cout
    ...
    std::cout << " average norm of error in placement = " << dAvgNorm
              << std::endl;
  }
...
template <class Cstr, int n> int TSolver<Cstr, n>::done_bc_mfc() {
...
template <class Cstr, int n> int TSolver<Cstr, n>::setup_matrix(bool showInfo) {
  ...
  ierr =
      MatCreateSeqAIJ(PETSC_COMM_SELF, n_eqs, n_eqs, 100, NULL, &m_stiffness);
  CHKERRQ(ierr);
  ...
  int iold_val = -1;
  for (size_t i = size_t(0); i < m_pmesh->get_no_elts(); ++i) {
    if (showInfo) {
...
int TSolver<Cstr, n>::add_elt_matrix(const tElement *pelt) {
  ...
  ierr = MatSetValues(m_stiffness, elt_matrix.rows(), pindex_1,
                      elt_matrix.cols(), pindex_2, values, ADD_VALUES);
  CHKERRQ(ierr);
...
int TSolver<Cstr, n>::add_elt_mfc_lhs(const tElement *pelt, tCoords &pt) {
  ...
  for (int i = 0; i < pelt->no_nodes(); ++i) {
    ...
    elt_matrix.set_block(bufMatrix, 0, i * n);
  }
  ...
  PetscScalar *values = new PetscScalar[bufMatrix.rows() * bufMatrix.cols()];
  ...
  ierr = MatSetValues(m_stiffness, bufMatrix.rows(), pindex_1, bufMatrix.cols(),
                      pindex_2, values, ADD_VALUES);
  CHKERRQ(ierr);
...
template <class Cstr, int n> int TSolver<Cstr, n>::setup_load_mfc() {
  for (typename BcContainerType::iterator it = m_vBc.begin(); it != m_vBc.end();
       ++it) {
    if (!(*it)->isActive)
      continue;
...
template <class Cstr, int n> int TSolver<Cstr, n>::comm_solution() {
  PetscErrorCode ierr;
...
template <class Cstr, int n> int TSolver<Cstr, n>::setup_load() {
  ...
  IS is;
  ierr = ISCreateGeneral(PETSC_COMM_WORLD, (int)mrhs.size(), indices, &is);
  CHKERRQ(ierr);
  ...
  ierr = VecSetValues(m_load, (int)mrhs.size(), indices, values, INSERT_VALUES);
  CHKERRQ(ierr);
...
template <class Cstr, int n> int TSolver<Cstr, n>::setup_load_sym() {
  PetscErrorCode ierr;
  // unused: tNode* pnode;
...
#endif
  if (m_displayLevel > 1)
    std::cout << " done_bc_natural size of container = " << m_vBc.size()
              << std::endl;
  ;
  for (typename BcContainerType::iterator it = m_vBc.begin(); it != m_vBc.end();
       ++it) {
  ...
  if (m_displayLevel > 1)
    std::cout << " after building the map\n";
  ...
  ierr =
      VecSetValues(vecBcs, (int)mrhs.size(), indices, rhsValues, INSERT_VALUES);
  CHKERRQ(ierr);
...
int TSolver<Cstr, n>::check_bc_error(double &dRemainingRatio) {
  ...
  if (count) {
    std::cout << " Average of the error norm = " << dSum / (double)count
              << std::endl
              << " Initial error = " << dSumInitial / (double)count
              << std::endl;
  } else
    std::cout << " count = 0 !?!\n";
...
template <class Cstr, int n>
TDirectSolver<Cstr, n>::TDirectSolver() : TSolver<Cstr, n>() {
  this->m_displayLevel = 1;
}

template <class Cstr, int n> TDirectSolver<Cstr, n>::~TDirectSolver() {}

#undef __FUNCT__
#define __FUNCT__ "TDirectSolver::solve"
template <class Cstr, int n> int TDirectSolver<Cstr, n>::solve() {
  PetscErrorCode ierr;
  ...
  ierr = KSPCreate(PETSC_COMM_WORLD, &ksp);
  CHKERRQ(ierr);
  ierr = KSPSetOperators(ksp, this->m_stiffness, this->m_stiffness,
...

  {
    ...
    if (this->m_displayLevel)
      std::cout << "DirectSolverConvergence = " << kspConvergenceReason[reason]
                << std::endl;
  }
