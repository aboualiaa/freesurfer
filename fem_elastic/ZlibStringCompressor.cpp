...
ZlibStringCompressor::ZlibStringCompressor(unsigned long bufferSize)
    : m_bufferAllocationMultiplier(10), m_bufferSize(0), m_buffer(0),
      m_debugLevel(0) {
  if (bufferSize)
    setBufferSize(bufferSize);
}

unsigned long ZlibStringCompressor::getBufferSize() { return m_bufferSize; }
...
void ZlibStringCompressor::checkZlibResult(const int result, const char *file,
                                           const int line) {
  switch (result) {
  case (Z_OK): { /* "zlib returned ok" */
    break;
  }
  case (Z_MEM_ERROR): {
  ...
  case (Z_DATA_ERROR): {
    ...
    throw " ZlibStringCompressor::checkZlibResult - zlib reports data "
          "corrupter error";
  }
  default: {
    std::ostringstream oss;
    oss << "ZlibStringCompressor::checkZlibResult  - zlib returned the unknown "
           "value of `"
        << result << "'";
