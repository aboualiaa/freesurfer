...
int main(int argc, const char **argv) {
  ...
  if (!params.strTemplate.empty()) {
    ...
    if (!mri)
      std::cerr << " Failed to open template mri " << params.strTemplate
                << std::endl;
    else
  ...
  if (!params.strSubject.empty()) {
    ...
    if (!mri)
      std::cerr << " Failed to open subject mri " << params.strSubject
                << std::endl;
    else
  ...
  for (it = params.items.begin(); it != params.items.end(); ++it) {
    if (it->type == "affine") {
      std::shared_ptr<gmp::AffineTransform3d> paffine(
          new gmp::AffineTransform3d);
      ...
      std::shared_ptr<gmp::DeltaTransform3d> pvol(new gmp::DeltaTransform3d);
      MRI *field = MRIread(const_cast<char *>(it->file.c_str()));
      if (!field) {
      ...
      MRI *mri_template =
          MRIread(const_cast<char *>(params.strTemplate.c_str()));
      MRI *mriBuf = MRIalloc(gcam->image.width, gcam->image.height,
                             gcam->image.depth, MRI_UCHAR);
      useVolGeomToMRI(&gcam->image, mriBuf);
      GCAMrasToVox(gcam, mriBuf);
      MRIfree(&mriBuf);

      std::cout << " atlas geometry = " << gcam->atlas.width << " , "
                << gcam->atlas.height << " , " << gcam->atlas.depth << std::endl
                << " image geometry = " << gcam->image.width << " , "
                << gcam->image.height << " , " << gcam->image.depth
                << std::endl;

      MRI *mri = MRIallocSequence(mri_template->width, mri_template->height,
                                  mri_template->depth, MRI_FLOAT, 3);
      ...
      for (gmp::VolumeMorph::TransformContainerType::iterator transformIter =
           ...
           transformIter != tmpMorph->m_transforms.end(); ++transformIter)
      ...
      std::cerr << " unhandled transform type " << it->type << std::endl;
    }
  } // next it
...
inline int myNint(double x) { return static_cast<int>(std::floor(x + .5)); }

// returns the mask if necessary
MRI *CopyGcamToDeltaField(GCA_MORPH *gcam, MRI *field) {
  if (!field)
    throw std::string(" Field not set in CopyGcamToDeltaField\n");
  ...
  for (int z = 0, maxZ = gcam->depth; z < maxZ; ++z)
    for (int y = 0, maxY = gcam->height; y < maxY; ++y)
      for (int x = 0, maxX = gcam->width; x < maxX; ++x) {
        ...
        if (img_x < 0 || img_x > field->width - 1 || img_y < 0 ||
            img_y > field->height - 1 || img_z < 0 ||
            img_z > field->depth - 1) {
          maskCounter++;
          continue;
        }
        ...
        MRIsetVoxVal(field, img_x, img_y, img_z, 0, pnode->x - img_x);
        MRIsetVoxVal(field, img_x, img_y, img_z, 1, pnode->y - img_y);
        MRIsetVoxVal(field, img_x, img_y, img_z, 2, pnode->z - img_z);
      }
  ...
  if (!g_vDbgCoords.empty()) {
    ...
    std::cout << " linear transf to get gcam xyz = " << x << " , " << y << " , "
              << z << std::endl;
    if (x < 0 || x > gcam->width - 1 || y < 0 || y > gcam->height - 1 ||
      ...
      pnode = &gcam->nodes[x][y][z];
      std::cout << " value of gcam = " << pnode->x << " , " << pnode->y << " , "
                << pnode->z << std::endl;
    }
  }
