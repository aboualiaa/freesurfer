...
typedef enum { src, dst, both } Frame;

template <int n> class TElement {
public:
...
template <int n> TElement<n>::TElement() : m_id(-1) { m_cpMaterial = NULL; }

template <int n> void TElement<n>::set_id(int id) { m_id = id; }

template <int n> void TElement<n>::add_node(TNode<n> *pNode) {
  m_vpNodes.push_back(pNode);
  pNode->add_elt(m_id);
}

template <int n>
bool TElement<n>::get_node(int index, TNode<n> **ppNode) const {
  if (index >= (int)m_vpNodes.size()) {
    std::cerr << " TElement::get_node -> improper index value = " << index
              << std::endl;
    *ppNode = NULL;
    return false;
  }
...
template <int n>
void TElement<n>::dst_box(tCoords &c_min, tCoords &c_max) const {
  if (m_vpNodes.empty()) {
    c_min = tCoords(-1.0);
    c_max = tCoords(-1.0);
    return;
...
template <int n>
void TElement<n>::src_box(tCoords &c_min, tCoords &c_max) const {
  if (m_vpNodes.empty()) {
    c_min = tCoords(-1.0);
    c_max = tCoords(-1.0);
    return;
...
template <int n> void TElement<n>::print(std::ostream &os) const {
