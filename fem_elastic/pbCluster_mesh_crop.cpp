...
template <class Cstr, int n> class TMeshCrop {
public:
  ...
  void Crop(MeshType *outMesh, const IndexContainerType &i_vidxElements,
...
TopologySolver::TopologySolver(TMesh3d &mesh, unsigned int radius, double de,
                               double dnu)
...
void TopologySolver::BuildClusters() {
  ...
  for (IndexVectorType::const_iterator cit = vecOrigProblems.begin();
    ...
    bfs.Visit(
        *cit, m_radius,
        std::back_inserter<IndexVectorType>(*(m_clusterContainer.back())));
...
public:
  ...
  bool operator()(const TopologySolver::IndexVectorPointer other) const {
    ...
    std::set_intersection(other->begin(), other->end(), m_container->begin(),
                          m_container->end(),
...
template <class Container> struct RemoveIfAndCopy {
...
void TopologySolver::MergeIntersectingClusters() {
  ...
  for (ClusterContainerType::iterator it = m_clusterContainer.begin();
       it != m_clusterContainer.end(); ++it, ++counter) {
    bool bAgain = true;
    while (bAgain) {
      ...
      if (!tmpContainer.empty()) {
        IndexVectorPointer newCluster =
            std::accumulate(tmpContainer.begin(), tmpContainer.end(),
                            IndexVectorPointer(new IndexVectorType), do_union);
        *it = newCluster;
  ...
  std::cout << " TopologyClusters = " << m_clusterContainer.size() << std::endl;
}
...
void TopologySolver::DoLocalSmoothing() {
  ...
  for (ClusterContainerType::const_iterator cit = m_clusterContainer.begin();
       cit != m_clusterContainer.end(); ++cit) {
...

    {
      ...
      for (Vector::const_iterator cit = boundary.begin(); cit != boundary.end();
           ++cit) {
        cropped.get_node(*cit, &pnode);
        if (!pnode)
          throw " TopologySolver - NULL node ";
        solver.add_bc_natural(pnode, pnode->delta());
      }
    }
    ...
    // pass on results to the original mesh
    {
      ...
      for (unsigned int ui(0), nnodes(cropped.get_no_nodes()); ui < nnodes;
           ++ui) {
...
void TMeshCrop<Cstr, n>::Crop(MeshType *outMesh,
  ...
  this->PopulateNodes(i_vidxElements, std::back_inserter(vnodes));

  IndexContainerType origBoundaryNodes;
  this->PopulateBoundaryNodes(i_vidxElements, vnodes,
  ...
  // populate nodes
  {
    ...
    for (typename IndexContainerType::const_iterator cit = vnodes.begin();
         cit != vnodes.end(); ++cit, ++index) {
      m_pmesh->get_node(*cit, &pnodeIn);
      if (!pnodeIn)
        throw " TMeshCrop - NULL node while cropping ";
      pnodeOut = Cstr::node(index, pnodeIn->coords(), pnodeIn->delta(),
                            pnodeIn->is_active());
  ...
  // populate elements
  {
    ...
    for (typename IndexContainerType::const_iterator cit =
         ...
         cit != i_vidxElements.end(); ++cit, ++index) {
      vpnodes.clear();
      ...
      ElementType *pelt = Cstr::elt(index, NULL, vpnodes);
...
*/
template <class Cstr, int n>
void TMeshCrop<Cstr, n>::PopulateBoundaryNodes(const IndexContainerType &velts,
                                               const IndexContainerType &vnodes,
                                               IndexContainerII ii_boundary) {
  ...
  for (typename IndexContainerType::const_iterator node_cit = vnodes.begin();
    ...
    for (pnode->get_elt_citer(elt_cit, elt_end); elt_cit != elt_end;
         ++elt_cit) {
