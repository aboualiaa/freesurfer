...
public:
  ...
  static Material3d *material(std::string strLabel, double d_e, double d_nu) {
    Material3d *pmat = new Material3d(d_e, d_nu);
    pmat->set_label(strLabel);
    return pmat;
  }
  static TNode<3> *node(int id, tDblCoords coords, tDblCoords delta,
  ...
  static Element3d *elt(int id, const VMaterial *cpmat,
  ...
  static Element3d *elt(int id, TNode<3> *pn1, TNode<3> *pn2, TNode<3> *pn3,
                        TNode<3> *pn4) {
    Element3d *pelt = new Element3d;
    pelt->set_id(id);
    pelt->add_node(pn1);
...
public:
  ...
  DelaunayMesh(PointsListType &sp, tDblCoords cmin, tDblCoords cmax,
               double dEltVol, double de, double dnu);
...
class MeshCreator {
public:
  MeshCreator(tDblCoords cmin, tDblCoords cmax, tIntCoords ticks, double de,
              double dnu);
...
private:
  ...
  void setup_elt_group(int &id, TNode<3> *pcubeCenter, TNode<3> *pfaceCenter,
                       TNode<3> *p0, TNode<3> *p1, TNode<3> *p2, TNode<3> *p3);

  TNode<3> *get_node(int x, int y, int z) {
    NodeMapType::iterator it = m_mnodes.find(icoords(x, y, z));
    if (it == m_mnodes.end()) {
