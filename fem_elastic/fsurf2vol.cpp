...
static int process_input_data(const IoParams &params, MRI *&mri_fixed,
                              SurfaceVectorType &vmris_fixed, MRI *&mri_moving,
                              SurfaceVectorType &vmris_moving, MRI *&mri_aseg);

static void apply_lin_transform(SurfaceVectorType &vmris, float *transform);

static int do_vol_deformation(PointsContainerType &vmris, tSolver &solver,
                              tDblCoords cmin, tDblCoords cmax, int step);

static float *powell_lin(const SurfaceVectorType &mris_x,
...
(use the direct image to map the deformation of a precise voxel location and
then populate the volume)

  Steps in the process:

1. obtain the LS rigid registration between the 2 surfaces. This is currently
done using the Powell optimizer.
...
void dbg_surf2vol(const PointsContainerType &c, MRI *mri);
...
int main(int argc, char *argv[]) {
  ...
  process_input_data(params, mri_fixed, vmris_fixed, mri_moving, vmris_moving,
                     mri_aseg);
  ...
  }
  {
    ...
    for (cit_fixed = vmris_fixed.begin(); cit_fixed != vmris_fixed.end();
      ...
      if (cit_fixed->mris->nvertices != cit_moving->mris->nvertices) {
  ...
  if (params.strTransform.empty()) {
    ...
    std::cout << " trying to read transform from " << params.strTransform
              << std::endl;
  ...
  std::cout << " direct min = " << cmin << " direct max = " << cmax
            << std::endl;
  ...
  std::copy(initContainer.begin(), initContainer.end(),
            std::back_inserter(container));

  create_surf_container(initPoints, vmris_fixed, params.surfSubsample,
                        params.bUsePialForSurf);
  std::copy(initPoints.begin(), initPoints.end(),
  ...
  // dbg_surf2vol( initContainer, mri_fixed );
  {
    ...
    for (SurfaceVectorType::iterator it = vmris_moving.begin();
      ...
      char buf[120];
      sprintf(buf, "%s.dbg_surf.%s", it->mris->hemisphere ? "rh" : "lh",
              it->type == white ? "white" : "pial");
      std::cout << " writing surface to file " << buf << std::endl;
  ...
  printf("baseline %g", timer.minutes());
  ...
  try {
    ...
    for (int step = noSteps; step > params.iEndStep; --step) {
      std::cout << " ======================\n step = " << step
                << "\n===============\n";
      printf("step %d %g ", step, timer.minutes());
      PrintMemUsage(stdout);
      TSolver<Constructor, 3> solver;

      // linearly vary the element volume in the given range
      double deltVol =
          std::max(params.eltVolMin,
                   params.eltVolMax - (params.eltVolMax - params.eltVolMin) *
                                          (noSteps - step) / (noSteps - 1));
      std::cout << "elt_vol= " << deltVol << std::endl;

      DelaunayMesh dmesh(srcPoints, cmin, cmax, deltVol, params.YoungModulus,
                         params.poissonRatio);

      printf("preget %g ", timer.minutes());
      PrintMemUsage(stdout);
      CMesh3d *pmesh = dmesh.get();
      printf("postget %g", timer.minutes());
      PrintMemUsage(stdout);
      ...
      printf("prebuild %g ", timer.minutes());
      PrintMemUsage(stdout);
      pmesh->build_index_src();
      printf("postbuild %g ", timer.minutes());
      PrintMemUsage(stdout);
      ...
      if (params.compress) {
        ...
        update_bc_container(container, pBufTransform);

        std::cout << "dcds mesh direct image = " << pmesh->dir_img(dbgImage)
                  << std::endl
        ...
        std::cout << "dcds total image for source point = " << dbgImage
                  << std::endl;
      ...
      if (!params.strDebug.empty()) {
        std::shared_ptr<gmp::AffineTransform3d> paffine(
            new gmp::AffineTransform3d(inv_t));
...

    {
      ...
      MRIwrite(warped, const_cast<char *>(params.strOutput.c_str()));
    ...
    if (!params.strOutputAffine.empty()) {
      ...
      try {
        ...
        MRIwrite(warped, const_cast<char *>(params.strOutputAffine.c_str()));
      } catch (const char *msg) {
        std::cerr << " exception caught while trying to write affine morphed "
                     "input volume \n"
                  << msg << std::endl;
      }
    }
...
//------------------------------------------------------------------
static void apply_lin_transform(SurfaceVectorType &vmris, float *transform) {
  for (SurfaceVectorType::iterator it = vmris.begin(); it != vmris.end(); ++it)
...
static float *powell_lin(const SurfaceVectorType &vmris_x,
  ...
  PetscInt linInc = 1;
  ierr = PetscOptionsGetInt(NULL, "-lin_res", &linInc, NULL);
  ...
  powell_minimize(container, transform, energy);
  std::cout << " energy after Powell = " << energy(container, transform)
            << std::endl;
...
static int create_bc_container(PointsContainerType &container,
  ...
  ierr = PetscOptionsGetReal(NULL, "-dirty", &petreal, NULL);
  CHKERRQ(ierr);
  ...
  for (cit_fixed = vmris_fixed.begin(); cit_fixed != vmris_fixed.end();
    ...
    std::cout << " create BC container - ripCounter = " << ripCounter
              << std::endl;
  } // next cit_fixed, cit_moving
...
static void create_surf_container(PointsListType &srcPoints,
  ...
  for (SurfaceVectorType::const_iterator cit = vmris.begin();
    ...
    for (unsigned int ui = 0; ui < nvertices; ++ui, ++pvtx) {
      if (pvtx->ripflag)
        continue; // uses the aparc
      if (subsampleDist > 0 && !pvtx->val)
        continue;
...
static void update_sources(const PointsListType &initPoints,
  ...
  if (count)
    std::cerr << " errors while updating sources -> count = " << count
              << std::endl;
}
#endif
...
#if 1
static MRI *compute_morph(Transform3SPointer ptransform, MATRIX *mat_lt,
                          MRI *mri_x, MRI *mri_atlas, MRI *mri_src,
                          tDblCoords cmin, tDblCoords cmax) {
  MRI *warped = NULL;
...
static MRI *compute_morph(const CMesh3d &mesh, MATRIX *mat_lt, MRI *mri_x,
                          MRI *mri_atlas, MRI *mri_src, tDblCoords cmin,
                          tDblCoords cmax) {
...
IoParams::IoParams()
    : vstrFixedSurf(), vstrAparc(), hasAparc(false), strFixedMri(), strAseg(),
      vstrMovingSurf(), strMovingMri(), strOutput("out.mgz"),
      strOutputField("out_field.mgz"), strOutputMesh(),
...
int IoParams::parse(std::string &errMsg) {
  ...
  // help
  ierr = PetscOptionsGetString(NULL, "-help", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    help_exit();

  // fixed MRI
  ierr = PetscOptionsGetString(NULL, "-fixed_mri", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strFixedMri = buffer;
  else
    errMsg += " No fixed volume present (option -fixed_mri)\n";

  // moving MRI
  ierr = PetscOptionsGetString(NULL, "-moving_mri", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strMovingMri = buffer;
  else
    errMsg += " No moving volume present (option -moving_mri)\n";

  // aseg (for the fixed volume)
  ierr = PetscOptionsGetString(NULL, "-aseg", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strAseg = buffer;
  ...
  while (bContinue) {
    if (surfIndex == 1)
      ...
      sprintf(option, "-fixed_surf_%d", surfIndex);

    ierr = PetscOptionsGetString(NULL, option, buffer, maxLen, &petscFlag);
    CHKERRQ(ierr);
    if (petscFlag)
      vstrFixedSurf.push_back(buffer);
    else {
      if (surfIndex == 1)
  ...
  while (bContinue) {
    if (surfIndex == 1)
      ...
      sprintf(option, "-aparc_%d", surfIndex);
  ...
  if (hasAparc) {
    if (vstrAparc.size() != vstrFixedSurf.size())
      errMsg +=
          " Size mismatch for APARC files (use none if you want to skip one)\n";
  }
  ...
  while (bContinue) {
    if (surfIndex == 1)
      ...
      sprintf(option, "-moving_surf_%d", surfIndex);

    ierr = PetscOptionsGetString(NULL, option, buffer, maxLen, &petscFlag);
    CHKERRQ(ierr);
    if (petscFlag)
      vstrMovingSurf.push_back(buffer);
    else {
      if (surfIndex == 1)
  ...
  // Output options
  ierr = PetscOptionsGetString(NULL, "-out", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strOutput = buffer;
  else
  ...
  ierr = PetscOptionsGetString(NULL, "-out_field", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strOutputField = buffer;
  else
    std::cout << " No field output option specified\n"
              << "\t will use default value " << strOutputField << std::endl;

  ierr = PetscOptionsGetString(NULL, "-out_surf", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strOutputSurf = buffer;

  ierr = PetscOptionsGetString(NULL, "-out_mesh", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strOutputMesh = buffer;

  ierr = PetscOptionsGetString(NULL, "-out_surf_affine", buffer, maxLen,
                               &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strOutputSurfAffine = buffer;

  ierr = PetscOptionsGetString(NULL, "-gcam", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strGcam = buffer;

  ierr = PetscOptionsGetString(NULL, "-out_affine", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strOutputAffine = buffer;

  ierr = PetscOptionsGetString(NULL, "-dbg_output", buffer, maxLen, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    strDebug = buffer;
  ...
  // Other options
  PetscReal petreal;
  ierr = PetscOptionsGetReal(NULL, "-elt_vol", &petreal, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    eltVolMin = eltVolMax = petreal;

  {
    PetscReal rar[20];
    int nmax = 3;
    ierr = PetscOptionsGetRealArray(NULL, "-elt_vol_range", rar, &nmax,
                                    &petscFlag);
    CHKERRQ(ierr);
    if (petscFlag) {
      if (nmax > 2) {
        std::cerr << " Element value range contains more than 2 elts - "
                     "discarding...\n";
      } else if (nmax < 2) {
        std::cerr
            << " Element value range does not contain 2 elements - exiting "
            << nmax << std::endl;
        exit(1);
      }
      eltVolMin = rar[0];
  ...
  // Poisson ratio
  ierr = PetscOptionsGetReal(NULL, "-poisson", &petreal, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    poissonRatio = petreal;
  else
    std::cout << " No Poisson ratio specified (option -poisson)\n"
              << "\t will use default value " << poissonRatio << std::endl;

  // Young modulus
  ierr = PetscOptionsGetReal(NULL, "-young", &petreal, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    YoungModulus = petreal;
  else
    std::cout << " No Young-modulus specified (option -young)\n"
              << "\t will use default value " << YoungModulus << std::endl;

  ierr = PetscOptionsGetReal(NULL, "-surf_subsample", &petreal, &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag)
    surfSubsample = petreal;

  ierr = PetscOptionsGetString(NULL, "-cache_transform", buffer, maxLen,
                               &petscFlag);
  CHKERRQ(ierr);
  if (petscFlag) {
    strTransform = buffer;
  ...
  ierr = PetscOptionsHasName(NULL, "-compress", &petscFlag);
  CHKERRQ(ierr);
  compress = static_cast<bool>(petscFlag);

  ierr = PetscOptionsGetInt(NULL, "-fem_steps", &iSteps, &petscFlag);
  CHKERRQ(ierr);
  ierr = PetscOptionsGetInt(NULL, "-fem_end_step", &iEndStep, &petscFlag);
  CHKERRQ(ierr);

  ierr = PetscOptionsHasName(NULL, "-topology_old", &petscFlag);
  CHKERRQ(ierr);
  bUseOldTopologySolver = static_cast<bool>(petscFlag);

  ierr = PetscOptionsHasName(NULL, "-use_pial_for_surf", &petscFlag);
  CHKERRQ(ierr);
...
void IoParams::help_exit() {
  std::cout << " Usage : the following parameters are available:\n"
            ...
            << "\t -out_surf <root file name> (will be appended _<surf "
               "index>.white)\n"
            << "\t -out_surf_affine <root file name> (will be appended _<surf "
               "index>.white)\n"
            << "\t -out_mesh <file name>\n"
            << "\t -spacing_x <scalar>\n"
            << "\t -spacing_y <scalar>\n"
            ...
            << "\t -cache_transform <file name> (if more than one run, will "
               "write the transform in a file and use in subsequent runs)\n"
            << "\t -dirty factor (between 0 and 1)\n"
            << "\t -dbg_output - will write a morph file at each iteration\n"
            << "\n Also, all the Petsc KSP options apply (see Petsc manual for "
               "details)\n";
  exit(1);
}
...
public:
  SurfaceMarker(MRIS *psurf) {
    ...
    std::sort(m_vecAparcIgnored.begin(), m_vecAparcIgnored.end());

    std::cout << " ignored labels = ";
    std::copy(m_vecAparcIgnored.begin(), m_vecAparcIgnored.end(),
  ...
  void operator()(VERTEX *pvtx) {
    ...
    if (std::binary_search(m_vecAparcIgnored.begin(), m_vecAparcIgnored.end(),
                           ctIndex))
      pvtx->ripflag = 1;
  }
...
static int process_input_data(const IoParams &params, MRI *&mri_fixed,
  ...
  if (!params.strAseg.empty()) {
    ...
    if (!mri_aseg) {
      std::cerr << " Error reading aseg volume file " << params.strAseg
                << std::endl;
      exit(1);
    }
  }

  for (IoParams::StringVectorType::const_iterator cit =
           params.vstrFixedSurf.begin();
       cit != params.vstrFixedSurf.end(); ++cit) {
    mris = MRISread(const_cast<char *>(cit->c_str()));
    if (!mris) {
      std::cerr << " Error reading surface " << *cit << std::endl;
      exit(1);
    }
    SurfacePointer sp;
  ...
  if (params.hasAparc) {
    ...
    if (params.vstrAparc.size() != vmris_fixed.size())
      ...
      for (SurfaceVectorType::iterator it = vmris_fixed.begin();
           it != vmris_fixed.end(); ++it, ++citAparc) {
        // allow skipping one aparc
        if (*citAparc == "none")
          continue;

        if (MRISreadAnnotation(
                it->mris, const_cast<char *>(citAparc->c_str())) != NO_ERROR) {
          std::cerr << " Error reading APARC file " << *citAparc << std::endl;
          exit(1);
        }
...
void output_bc_locations(const tSolver &solver, MRI *mri) {
  ...
  MRI *mri_out = MRIalloc(mri->width, mri->height, mri->depth, MRI_FLOAT);
...
void compute_bc_error(const tSolver &solver) {
  ...
  for (; it != end; ++it) {
    double dNorm =
        (solver.get_mesh()->dir_img((*it)->pt) - (*it)->pt - (*it)->delta)
            .norm();
    if ((*it)->isActive)
      dActiveError += dNorm;
    dTotalError += dNorm;
...
static void surf_forward_morph(const CMesh3d *pmesh, std::string strName,
                               SurfaceVectorType &vmris_moving, MRI *mri_fixed,
                               TCoords<double, 3> cmin,
  ...
  for (SurfaceVectorType::iterator it = vmris_moving.begin();
    ...
    int errCount = 0;
    std::cout << " bbox for inverse stuff = " << cmin << " , " << cmax
              << std::endl;
    ...
    for (unsigned int ui = 0; ui < nvertices; ++ui, ++pvtx) {
      ...
      if (img(0) < 0 || img(0) > mri_fixed->width || img(1) < 0 ||
          img(1) > mri_fixed->height || img(2) < 0 || img(2) > mri_fixed->depth)
        continue;
    ...
    std::cout << " writing surface to file " << fname << std::endl;
...
static void gather_defect_info(CMesh3d::ElementIndexContainer &lstElts,
  ...
  for (CMesh3d::ElementIndexContainer::const_iterator cit = lstElts.begin();
    ...
    if (bcIter != mfcInfo.end()) {
      bcDefects++;
      std::cout << " elt index " << *cit
                << " -> norm var = " << bcIter->second.second << std::endl;
    }
  } // next cit
...
tDblCoords cross_product(const tDblCoords &a, const tDblCoords &b) {
  tDblCoords result;
...
int nint(float f) { return (int)std::floor(f + .5f); }

void dbg_surf2vol(const PointsContainerType &c, MRI *mri) {
  MRI *out_fixed = MRIalloc(mri->width, mri->height, mri->depth, MRI_UCHAR);
  MRIcopyHeader(mri, out_fixed);
  MRIvalueFill(out_fixed, 0);

  MRI *out_moving = MRIalloc(mri->width, mri->height, mri->depth, MRI_UCHAR);
