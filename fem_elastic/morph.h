...
public:
  ...
  std::shared_ptr<Transform> initial() const { return m_pInitial; }
...
protected:
  ...
  virtual tCoords doOwnImg(const tCoords &pt) const { return pt; }
};
...
public:
  ...
  float *pars() const { return m_pdata; }
...
public:
  ...
  const MRI *field() const { return m_field; }
...
public:
  ...
  MRI *apply_transforms(MRI *, bool cacheField = false,
  ...
  const MRI *cache() const { return mriCache; }
  ...
  GCA_MORPH *exportGcam(MRI *moving = NULL, bool useTemplateBoundingBox = false,
                        int thresh = 0, int padding = 1) const;
  ...
  void set_volGeom_fixed(const VOL_GEOM &vg) { m_vgFixed = vg; }
  void set_volGeom_fixed(const MRI *mri) { getVolGeom(mri, &m_vgFixed); }
  void set_volGeom_moving(const MRI *mri) { getVolGeom(mri, &m_vgMoving); }
  void set_volGeom_moving(const VOL_GEOM &vg) { m_vgMoving = vg; }

  const VOL_GEOM &vgFixed() const { return m_vgFixed; }
  const VOL_GEOM &vgMoving() const { return m_vgMoving; }
...
protected:
  ...
  enum IoTags { tagVgFixed = 1, tagVgMoving, tagTransform };
