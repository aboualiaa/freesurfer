...
void solve_topology_problems(TMesh3d &mesh, unsigned int neighborhood,
                             bool dbgMode) {
  ...
  /*
    ...
    remains this is done because it makes sense to localize problems and use as
    boundary valid tetrahedra (since solving a local problem while keeping
    consistent with the global mesh is facilitated much by this assumption)

    one key element in the clustering algorithm is an efficient set difference
    operation. This is done using the std::set_difference algorithm, which
    requires sorted inputs. Hence I am forced to use std::sort, which requires
    random access iterators. Using random access iterators in turn makes using
    lists impossible.

    Future suggestion: when an algorithm needs a container populated, as is the
    case of orientation_pb, provide a templatized version which provides an
    INSERT ITERATOR.
  */
  while (!vecContainer.empty()) {
    ...
    crop.Crop(&cropMesh, nodeMap, elementMap, boundaryNodes,
              vecContainer.front(), neighborhood);
    ...
    */
    ...
    std::set_difference(vecContainer.begin(), vecContainer.end(),
                        vecCrop.begin(), vecCrop.end(), cropii);
    ...
    // if using the same material in 2 consecutive iterations, the material is
    // destroyed solution - use smart pointers for materials or take out that
    ...
    for (unsigned int ui(0), nnodes(cropMesh.get_no_nodes()); ui < nnodes;
      ...
      if (std::find(boundaryNodes.begin(), boundaryNodes.end(), ui) !=
          boundaryNodes.end()) {
        btest = true;
  ...
  if (!vecAfter.empty()) {
    ...
    std::set_difference(vecAfter.begin(), vecAfter.end(),
                        ...
                        iiDelta);
    std::cout << " new problems size = " << deltaContainer.size() << std::endl;
  } else
