...
template <unsigned int N> class TCubicRegion {
public:
  typedef TCoords<double, N> CoordsType;
  TCubicRegion() : m_cmin(), m_cmax() {}
  TCubicRegion(const CoordsType &cmin, const CoordsType &cmax)
  ...
  no informative priors are used, so there are a lot of comparisons necessary
  for this
  ...
  */
  ...
  const CoordsType &min() const { return m_cmin; }
  const CoordsType &max() const { return m_cmax; }

  CoordsType &min() { return m_cmin; }
  CoordsType &max() { return m_cmax; }
...
However, in this case it seemed better to separate the leaves and the
intermediate nodes
...
protected:
  ...
  */
  static const DeterminantType &GetDeterminant() {
    static DeterminantType s_octantDeterminant = InitializeOctantDeterminant();
...
Specific implementation of intermediate nodes. These will not contain elements
themselves, only route requests toward specific branches.

Octree implementation => each node will hold exactly 8 branches.
The separation happens according to the orthogonal directions - the most likely
point to do the separation is the center.

The most time-critical operation occurs when some element data is inserted.
Hence, it makes sense to optimize for this.

In this case, a natural idea is to minimize the number of comparisons necessary
for the insertion. Namely, IF at the moment of the insertion it has already been
checked that the element overlaps with this branch, then all that is actually
needed is to decide which of the octants are overlapping with the element....
...
class TIntermediateNode : public TNode<ElementProxy, N> {
  ...
  // the next enum is used for bitwise flag operations on each enumeration when
  // deciding
...
public:
  ...
  TIntermediateNode(const CoordsType &cmin = CoordsType(),
                    const CoordsType &cmax = CoordsType())
      : Superclass(cmin, cmax), m_cmidPoint(), m_items() {
  ...
  bool insertItem(const ElementProxy *ep, Superclass *&pCaller,
    ...
    if (!inserted)
      std::cerr << " element NOT inserted - overlappingOctants = "
                << (int)overlappingOctants << std::endl;

    return inserted;
  }

  /*
    apply the same strategy as for the insertion, namely minimize the number of
    comparisons needed to decide which is the appropriate bucket
    ...
    use the fact that branches are encoded in the array as abc (binary
    decomposition), where a->x , b->y and c->z
  */
...
private:
  ...
  /*
    when this function is called, IT IS KNOWN there is an overlap with THIS
    region the purpose is to decide which of the sub-regions overlaps with the
    argument
  */
...
public:
  typedef typename Superclass::CoordsType CoordsType;
  TerminalNode(const CoordsType &cmin, const CoordsType &cmax)
      : Superclass(cmin, cmax), m_elements() {}
  ...
  bool insertItem(const ElementProxy *ep, Superclass *&pCaller,
    ...
    if (data.currentLevel < data.maxLevels &&
      ...
      // after creating the new nodes, re-insert all the current element proxies
...
template <class ElementProxy, unsigned int N> class Octree {
  typedef TNode<ElementProxy, N> NodeType;

public:
  typedef TCoords<double, N> CoordsType;
  Octree(const CoordsType &cmin, const CoordsType &cmax, unsigned int maxLevels,
         unsigned int maxElementsInLeaf)
      : m_octreeData() {
  ...
  bool insertItem(const ElementProxy *ep) {
    ...
    if (!this->m_pnode->isOverlapping(*ep))
      return false;
