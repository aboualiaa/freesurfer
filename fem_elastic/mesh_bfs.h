...
template <class TMesh> class TMeshBfs {
public:
  ...
  void Visit(unsigned int seed, unsigned int radius, OutputIterator oit);

protected:
  const MeshType *m_pmesh;
};

template <class T, unsigned int n> class CircularArray {
public:
  ...
  void shift(int value = 0) { m_counter += value; }
...
void TMeshBfs<TMesh>::Visit(unsigned int seed, unsigned int radius,
  ...
  while (radius-- && !queueSwitch.current().empty()) {
    ...
    while (!queueSwitch.current().empty()) {
      ...
      if (elementSet.find(eltIndex) != elementSet.end())
        continue;
      elementSet.insert(eltIndex);
      cpElt = m_pmesh->get_elt(eltIndex);
      if (!cpElt)
        throw std::string(" NULL element in TMeshBfs::Visit");
      ...
      for (unsigned int ui(0), nnodes(cpElt->no_nodes()); ui < nnodes; ++ui) {
        ...
        if (nodeSet.find(pnode->get_id()) != nodeSet.end())
          continue;
