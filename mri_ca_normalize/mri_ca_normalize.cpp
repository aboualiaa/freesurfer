...
main(int argc, char *argv[])
{
  ...
  if (read_ctrl_point_fname) {
    ...
    if (mri_ctrl == nullptr)
      ErrorExit(ERROR_NOFILE,
                "%s: could not read precomputed control points from %s",
  ...
  if (long_seg_fname) // read in a segmentation and turn it into a ctrl point
                      // volume
  {
  ...
  if (seg_fname == nullptr) {
    ...
    if (!transform)
      ErrorExit(ERROR_BADPARM, "%s: could not open xform file %s", Progname,
                xform_fname);
    ...
    gca = nullptr; /* don't need atlas if using segmentation */
  }
  ...
  for (input = 0; input < ninputs; input++) {
    ...
    if (mri_tmp->type != MRI_UCHAR && mri_in &&
        mri_in->type == MRI_UCHAR) // scale it down to fit
    {
    ...
    if (mri_tmp && ctrl_point_fname && !mri_ctrl) {
      mri_ctrl =
          MRIallocSequence(mri_tmp->width, mri_tmp->height, mri_tmp->depth,
                           MRI_FLOAT, nregions * 2); // labels and means
  ...
  if (seg_fname == nullptr) {
    ...
    if (example_T1) {
      ...
      if (!mri_seg)
        ErrorExit(ERROR_NOFILE,
                  "%s: could not read example segmentation from %s", Progname,
  ...
  if (seg_fname == nullptr) // only run this if using an aseg in the previous
                            // block, not a manual one (i.e. -aseg)
  {
    int j;

    gcas = GCAfindAllSamples(gca, &nsamples, nullptr, 1);
    printf("using %d sample points...\n", nsamples);
    GCAcomputeSampleCoords(gca, mri_in, gcas, nsamples, transform);
    if (sample_fname)
      GCAtransformAndWriteSamples(gca, mri_in, gcas, nsamples, sample_fname,
                                  transform);

    if (Gx >= 0) {
      int xp, yp, zp, xn, yn, zn;

      GCAsourceVoxelToPrior(gca, mri_ctrl, transform, Gx, Gy, Gz, &xp, &yp,
                            &zp);
    ...
    for (j = 0; j < 1; j++) {
      for (n = 1; n <= nregions; n++) {
        if (long_seg_fname) {
          gcas_norm = copy_ctrl_points_from_volume(gca, transform,
                                                   &norm_samples, mri_ctrl, 0);
          fill_in_sample_means(gcas_norm, gca, norm_samples);
        } else if (read_ctrl_point_fname) {
          gcas_norm = copy_ctrl_points_from_volume(
              gca, transform, &norm_samples, mri_ctrl, n - 1);
        } else {
          for (norm_samples = i = 0; i < NSTRUCTURES; i++) {
            ...
            {
              copy_ctrl_points_to_volume(gcas_struct, struct_samples, mri_ctrl,
                                         n - 1);
            }
            if (i) {
              GCA_SAMPLE *gcas_tmp;
              gcas_tmp = gcas_concatenate(gcas_norm, gcas_struct, norm_samples,
                                          struct_samples);
              free(gcas_norm);
        ...
        mri_norm = GCAnormalizeSamplesAllChannels(
            mri_in, gca, gcas_norm, file_only ? 0 : norm_samples, transform,
            ctl_point_fname, bias_sigma);
        if (Gdiag & DIAG_WRITE) {
          char fname[STRLEN];
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!strcmp(option, "FSAMPLES")) {
    ...
    printf("expanding norm range to [%2.1f --> %2.1f]\n",
           ...
           (1.0 + extra_norm_range) * MAX_WM_BIAS_PCT *
               DEFAULT_DESIRED_WHITE_MATTER_VALUE);
...
static GCA_SAMPLE *find_control_points(GCA *gca, GCA_SAMPLE *gcas_total,
                                       int total_samples, int *pnorm_samples,
                                       int nregions, int label, MRI *mri_in,
                                       TRANSFORM *transform, double min_prior,
  ...
  for (nsamples = i = 0; i < total_samples; i++) {
    ...
    nsamples++;
    if (mri_ctrl)
      MRIsetVoxVal(mri_ctrl, gcas_total[i].x, gcas_total[i].y, gcas_total[i].z,
                   0, 1);
  }
  ...
  *pnorm_samples = 0;
  ...
  gcas_bounding_box(gcas, nsamples, &xmin, &ymin, &zmin, &xmax, &ymax, &zmax,
                    label);
      ...
      for (z = 0; z < nregions; z++) {
        ...
        do {
          for (region_samples = i = 0; i < nsamples; i++) {
            ...
            if ((xi < 0 || xi >= nregions) || (yi < 0 || yi >= nregions) ||
                (zi < 0 || zi >= nregions))
              DiagBreak();
            ...
            if (min_region_prior(gca, gcas[i].xp, gcas[i].yp, gcas[i].zp,
            ...
            if (gca->ninputs >
                1) // 1st input is norm.mgz and we can depend on its intensities
            {
              double val_T1;
              MRIsampleVolumeFrame(mri_in, xv, yv, zv, 0, &val_T1);
              if (val_T1 <
                      MIN_WM_BIAS_PCT * DEFAULT_DESIRED_WHITE_MATTER_VALUE ||
                  val_T1 >
                      MAX_WM_BIAS_PCT * DEFAULT_DESIRED_WHITE_MATTER_VALUE) {
                if (mri_ctrl)
            ...
            } else // be more conservative
            {
              double val_T1;
              MRIsampleVolumeFrame(mri_in, xv, yv, zv, 0, &val_T1);
              if (val_T1 < .75 * MIN_WM_BIAS_PCT *
                               DEFAULT_DESIRED_WHITE_MATTER_VALUE ||
                  val_T1 > 1.25 * MAX_WM_BIAS_PCT *
                               DEFAULT_DESIRED_WHITE_MATTER_VALUE) {
#if 1
                if (mri_ctrl)
                  MRIsetVoxVal(mri_ctrl, xv, yv, zv, 0, 3);
            ...
            if (uniform_region(gca, mri_in, transform, xv, yv, zv, image_wsize,
                               &gcas[i], nsigma) == 0) {
              ...
              for (xk = -1; !found && xk <= 1; xk++) {
                for (yk = -1; !found && yk <= 1; yk++) {
                  for (zk = -1; !found && zk <= 1; zk++) {
                    if (uniform_region(gca, mri_in, transform, xv + xk, yv + yk,
                                       zv + zk, image_wsize, &gcas[i],
                                       nsigma) != 0) {
                      ...
                      if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
                        printf("moving control point (%d, %d, %d) to (%d, %d, "
                               "%d) for uniformity\n",
        ...
        for (n = 0; n < gca->ninputs; n++) {
          ...
          HISTOinit(histo, histo->nbins, 0, 255);
          HISTOinit(hsmooth, hsmooth->nbins, 0, 255);
          for (means[n] = vars[n] = 0.0, i = 0; i < region_samples; i++) {
            MRIsampleVolumeFrame(mri_in, gcas_region[i].x, gcas_region[i].y,
                                 gcas_region[i].z, n, &val);
            if (FZERO(val)) {
              if (i < (region_samples - 1))
                memmove(&gcas_region[i], &gcas_region[i + 1],
        ...
        GCAcomputeLogSampleProbability(gca, gcas_region, mri_in, transform,
                                       region_samples, DEFAULT_CLAMP);
...
#else
#if 1
        nremoved = GCAremoveOutlyingSamples(gca, gcas_region, mri_in, transform,
                                            region_samples, outlying_nsigma);
#endif
        for (used_in_region = i = 0; i < region_samples; i++) {
          ...
          {
            if (mri_ctrl)
              MRIsetVoxVal(mri_ctrl, gcas_region[i].x, gcas_region[i].y,
                           gcas_region[i].z, 0, 1);
            continue;
          }
          memmove(&gcas_norm[*pnorm_samples], &gcas_region[j],
                  sizeof(GCA_SAMPLE));
          (*pnorm_samples)++;
          used_in_region++;
        }
...
static GCA_SAMPLE *gcas_concatenate(GCA_SAMPLE *gcas1, GCA_SAMPLE *gcas2,
  ...
  if (!gcas)
    ErrorExit(ERROR_NOMEMORY, "gcas_concatenate: could not allocate %d samples",
              n1 + n2);
    ...
    for (xk = -whalf; xk <= whalf; xk++) {
      ...
      for (yk = -whalf; yk <= whalf; yk++) {
        ...
        for (zk = -whalf; zk <= whalf; zk++) {
          zi = mri->zi[z + zk];
          MRIsampleVolumeFrame(mri, (double)xi, (double)yi, (double)zi, n,
                               &val);
          if (val < min_val) {
            min_val = val;
          }
...
static int discard_unlikely_control_points(GCA *gca, GCA_SAMPLE *gcas,
  ...
  min_T1 = (1.0 - extra_norm_range) * MIN_WM_BIAS_PCT *
           DEFAULT_DESIRED_WHITE_MATTER_VALUE;
  max_T1 = (1.0 + extra_norm_range) * MAX_WM_BIAS_PCT *
           DEFAULT_DESIRED_WHITE_MATTER_VALUE;
  for (num = n = 0; n < gca->ninputs; n++) {
    int niter = 0;
    MRIvalRangeFrame(mri_in, &fmin, &fmax, n);
    h = HISTOinit(nullptr, nint(fmax - fmin) + 1, fmin, fmax);
    ...
    for (i = 0; i < nsamples; i++) {
      ...
      if (n >= 1) {
        double val_T1;
        MRIsampleVolumeFrame(mri_in, gcas[i].x, gcas[i].y, gcas[i].z, 0,
                             &val_T1);
    ...
    hsmooth = HISTOsmooth(h, nullptr, 2);
    do {
      ...
      HISTOclearBins(hsmooth, hsmooth, hsmooth->bins[start],
                     hsmooth->bins[end]);
      if (niter++ > 5) {
        break;
      }
    ...
    for (i = 0; i < nsamples; i++) {
      ...
      if (val - fmin < start || val - fmin > end) {
        ...
        if (val < min_T1 ||
            val > max_T1) // check norm.mgz to make sure it is in wm
        {
          num++;
          gcas[i].label = 0;
        }
  ...
  printf("%d of %d (%2.1f%%) samples deleted\n", num, nsamples,
         100.0f * (float)num / (float)nsamples);
  return (NO_ERROR);
}

#include "mri_ca_normalize.help.xml.h"
static void usage_exit(int code) {
  outputHelpXml(mri_ca_normalize_help_xml, mri_ca_normalize_help_xml_len);
  exit(code);
}
...
static MRI *normalize_from_segmentation_volume(MRI *mri_src, MRI *mri_dst,
  ...
  mri_tmp = MRIclone(mri_seg, nullptr);
  mri_bin = MRIclone(mri_seg, nullptr);
  ...
  MRInormGentlyFindControlPoints(mri_dst, 110, 20, 10, mri_bin, nullptr);
      ...
      for (z = 0; z < mri_ctrl->depth; z++) {
        ...
        if (label > 0) {
          ...
          if (gcas[nsamples].means == nullptr)
            ErrorExit(ERROR_NOMEMORY,
                      "%s: could not allocated %dth sample mean", Progname,
                      nsamples);
          gcas[nsamples].means[0] =
              MRIgetVoxVal(mri_ctrl, x, y, z, frame + nregions);
          gcas[nsamples].label = label;

          if (!GCAsourceVoxelToPrior(gca, mri_ctrl, transform, x, y, z, &xp,
                                     &yp, &zp)) {
            gcas[nsamples].xp = xp;
            gcas[nsamples].yp = yp;
            gcas[nsamples].zp = zp;
...
static int fill_in_sample_means(GCA_SAMPLE *gcas, GCA *gca, int nsamples) {
  ...
  {
    gc = GCAfindPriorGC(gca, gcas[n].xp, gcas[n].yp, gcas[n].zp, gcas[n].label);
    if (gc) {
      int r, c, v;
