 ...
 * @brief Normalize a set of longituindal volumes making use of subcortical
 * atlas data
 *
 */
/*
...
// static int normalize_timepoints_with_samples(MRI *mri, GCA_SAMPLE *gcas, int
// nsamples, int nsoap) ;
...
int main(int argc, char *argv[]) {
  ...
  MRI *mri_in, *mri_norm = nullptr, *mri_tmp, *mri_ctrl = nullptr,
               *mri_aseg = nullptr;
  GCA *gca;
  ...
  if (argc < 6)
    ErrorExit(ERROR_BADPARM,
              ...
              "name> <atlas> <transform file> <out vol name> \n",
              Progname);
  in_fname = argv[2];
  gca_fname = argv[3];
  xform_fname = argv[4];
  ...
  for (input = 0; input < ninputs; input++) {
    ...
    if (!mri_tmp)
      ErrorExit(ERROR_NOFILE,
                "%s: could not read input MR volume from %s . If you are "
                "trying to run this on top of existing longitudinal data, "
                "switch on -longinput.",
                Progname, fname);
    ...
    if (input == 0) {
      ...
      mri_aseg = MRIclone(mri_in, nullptr);
    }
  ...
  // don't do this (there are still bugs, I think. Instead use robust template
  // with --iscaleonly if necessary at all).
  if (false)
    ...
    for (j = 0; j < 1; j++) {
      for (n = 1; n <= nregions; n++) {
        for (norm_samples = i = 0; i < NSTRUCTURES; i++) {
          ...
          gcas_struct =
              find_control_points(gca, gcas, nsamples, &struct_samples, n,
                                  normalization_structures[i], mri_in,
                                  transform, min_prior, ctl_point_pct);
          discard_unlikely_control_points(
              gca, gcas_struct, struct_samples, mri_in, transform,
              cma_label_to_name(normalization_structures[i]));
          discard_control_points_with_different_labels(
              gcas_struct, struct_samples, mri_aseg);
  ...
  // now do cross-time normalization to bring each timepoint closer to the mean
  // at each location
#if 0
  ...
  if (mri_in->nframes > 1) {
    ...
    if (false)
      ...
      MRI *mri_temp = normalize_timepoints_with_parzen_window(mri_in, nullptr,
                                                              cross_time_sigma);
      MRIfree(&mri_in);
      mri_in = mri_temp;
    }
    ...
    MRIfree(&mri_frame1);
    MRIfree(&mri_frame2);
    printf("RMS (first 2 inputs) after intensity cohering  = %2.2f "
           "(sigma=%2.2f)\n",
...
static GCA_SAMPLE *find_control_points(GCA *gca, GCA_SAMPLE *gcas_total,
  ...
  int i, j, *ordered_indices, nsamples, xmin, ymin, zmin, xmax, ymax, zmax, xv,
      yv, zv, x, y, z, xi, yi, zi, region_samples, used_in_region,
      ...
      for (xk = -whalf; xk <= whalf; xk++) {
        ...
        for (yk = -whalf; yk <= whalf; yk++) {
          ...
          for (zk = -whalf; zk <= whalf; zk++) {
            zi = mri->zi[z + zk];
            MRIsampleVolumeFrame(mri, (double)xi, (double)yi, (double)zi, frame,
                                 &val);
            if (val < min_val)
              min_val = val;
            if (val > max_val)
...
static void usage_exit(int code) {
  ...
  printf("\t-sigma <bias sigma>          smoothing sigma for bias field if "
         "control points specified (def=4)\n");
  ...
  printf("\t-fonly <filename>            only use control points "
         "from filename\n");
  printf("\t-longinput                   load aseg and nu from "
         "<tp>.long.<base> dirs, "
...
static MRI *normalize_timepoints_with_parzen_window(MRI *mri_in, MRI *mri_out,
  ...
  // copying and returning here avoids numerical problems below (division by
  // zero)
    ...
    for (y = 0; y < mri_in->height; y++)
      for (z = 0; z < mri_in->depth; z++) {
        ...
        for (frame1 = 0; frame1 < mri_in->nframes; frame1++) {
          ...
          for (total = total_norm = 0.0, frame2 = 0; frame2 < mri_in->nframes;
               frame2++) {
...
static MRI *scale_all_images(MRI *mri_in, MRI *mri_out) {
  ...
  mri_f0 = MRIcopyFrame(mri_in, nullptr, 0, 0);
  MRIthresholdRangeInto(mri_f0, mri_f0, 50, 120);

  for (t = 1; t < mri_in->nframes; t++) {
    mri_f = MRIcopyFrame(mri_in, nullptr, t, 0);
    mri_ratio = MRIdivide(mri_f0, mri_f, nullptr);
