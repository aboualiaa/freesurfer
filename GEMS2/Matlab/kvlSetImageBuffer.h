...
template <class ImageType> class SetImageBufferHelper {
public:
  //
  static bool AssignImageBuffer(const mxArray *matlabObject,
    ...
    typename ImageType::Pointer sourceImage =
        static_cast<ImageType *>(itkObject.GetPointer());
    typename ImageType::Pointer destinationImage =
        static_cast<ImageType *>(imageObject);
...
public:
  ...
  void Run(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) override {
    ...
    if ((nrhs != 2) || !mxIsInt64(prhs[0]) || (nlhs != 0)) {
    ...
    if (!success) {
      success =
          SetImageBufferHelper<itk::Image<unsigned char, 3>>::AssignImageBuffer(
              prhs[1], imageObject);
    }
    if (!success) {
      success = SetImageBufferHelper<
          itk::Image<unsigned short, 3>>::AssignImageBuffer(prhs[1],
                                                            imageObject);
    }
    if (!success) {
      success = SetImageBufferHelper<itk::Image<short, 3>>::AssignImageBuffer(
          prhs[1], imageObject);
    }
    if (!success) {
      success = SetImageBufferHelper<itk::Image<float, 3>>::AssignImageBuffer(
          prhs[1], imageObject);
    }
    if (!success) {
