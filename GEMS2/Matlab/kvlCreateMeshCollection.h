...
public:
  ...
  void Run(int nlhs, mxArray *plhs[], int nrhs,
    ...
    ProbabilityImageType::Pointer probabilityImage =
    ...
    for (int iterationNumber = 0; iterationNumber < 10; iterationNumber++) {
      ...
      AtlasMesh::PointDataContainer::Iterator pointParamIt =
          meshCollection->GetPointParameters()->Begin();
      ...
      for (; pointParamIt != meshCollection->GetPointParameters()->End();
           ++pointParamIt, ++statIt) {
    ...
    const int meshCollectionHandle =
        kvl::MatlabObjectArray::GetInstance()->AddObject(meshCollection);
