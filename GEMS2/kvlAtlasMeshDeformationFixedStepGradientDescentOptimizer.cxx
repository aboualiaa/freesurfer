...
//
//
double AtlasMeshDeformationFixedStepGradientDescentOptimizer ::
    FindAndOptimizeNewSearchDirection() {

  // Compute the largest gradient magnitude in any point
  const double maximumGradientMagnitude =
      this->ComputeMaximalDeformation(m_Gradient);
  ...
  this->AddDeformation(m_Position, alpha, m_Gradient, trialPosition,
                       maximalDeformation);
  if (m_Verbose) {
    std::cout << "Using trialPosition with maximalDeformation: "
  ...
  if ((trialCost > m_Cost) || ((fabsf(m_Cost - trialCost) / fabsf(trialCost)) <
