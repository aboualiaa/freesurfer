...
BOOST_AUTO_TEST_CASE(Interpolation) {
  ...
  clock.Stop();
  BOOST_TEST_MESSAGE(
      "Time taken by reference alpha drawer: " << clock.GetMean());
...
BOOST_AUTO_TEST_CASE(DeformationGradients) {
  ...
  gradientCalculator->SetImages(
      std::vector<ImageType::ConstPointer>(1, image.GetPointer()));
  ...
  for (int testRunNumber = 0; testRunNumber < 10; testRunNumber++) {
    ...
    clock.Stop();
    BOOST_TEST_MESSAGE(
        "Average time taken for subsequent iterations of gradient calculator: "
        << clock.GetMean());
  }
  ...
  for (int testRunNumber = 0; testRunNumber < 10; testRunNumber++) {
    ...
    referenceGradientCalculator->Rasterize(mesh);
    clock.Stop();
    BOOST_TEST_MESSAGE("Average time taken for subsequent iterations of "
                       "reference gradient calculator: "
                       << clock.GetMean());
  }
