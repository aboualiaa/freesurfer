...
typedef boost::mpl::list<unsigned char, char, unsigned short, short,
...
template <typename T, int nDims> void SendSetReceiveDriver() {
...
template <typename T, int nDims> void PlusKernelDriver() {
  BOOST_TEST_CONTEXT("Seed: " << gPRNG.Reseed()) {
    kvl::cuda::Dimension<nDims, size_t> srcDims, resultDims;
