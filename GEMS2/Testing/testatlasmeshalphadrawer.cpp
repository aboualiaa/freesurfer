...
void CheckAlphaDrawer(kvl::interfaces::AtlasMeshAlphaDrawer *ad,
  ...
  itk::ImageRegionConstIteratorWithIndex<
      kvl::interfaces::AtlasMeshAlphaDrawer::ImageType>
      it(img, img->GetBufferedRegion());
  itk::ImageRegionConstIteratorWithIndex<kvl::AtlasMeshAlphaDrawer::ImageType>
      itOrig(originalAD->GetImage(),
             originalAD->GetImage()->GetBufferedRegion());
...
void SingleConstantTetrahedronContainedCube(
    kvl::interfaces::AtlasMeshAlphaDrawer *ad, const int classNumber,
    const int nAlphas, const int imageSize) {
  ...
  for (unsigned int k = 0; k < imageSize; k++) {
    for (unsigned int j = 0; j < imageSize; j++) {
      for (unsigned int i = 0; i < imageSize; i++) {
        ...
        BOOST_CHECK_CLOSE(img->GetPixel(idx), static_cast<float>(classNumber),
                          percentTolerance);
      }
    }
  }
...
BOOST_DATA_TEST_CASE(ContainedUnitCube, boost::unit_test::data::xrange(nAlphas),
                     classNumber) {
...
BOOST_DATA_TEST_CASE(ContainedLargeCube,
                     boost::unit_test::data::xrange(nAlphas), classNumber) {
  kvl::AtlasMeshAlphaDrawerCPUWrapper ad;
...
#ifdef CUDA_FOUND
BOOST_DATA_TEST_CASE(ContainedUnitCubeGPU,
                     boost::unit_test::data::xrange(nAlphas), classNumber) {
...
BOOST_DATA_TEST_CASE(ContainedLargeCubeGPU,
                     boost::unit_test::data::xrange(nAlphas), classNumber) {
  kvl::cuda::AtlasMeshAlphaDrawerCUDA ad;
...
BOOST_AUTO_TEST_CASE(MeshInformation) {
  ...
  for (auto pointIt = pointData->Begin(); pointIt != pointData->End();
       ++pointIt) {
    size_t nAlphas = pointIt->Value().m_Alphas.size();
    if (nAlphas < minAlphas) {
      minAlphas = nAlphas;
    }
    if (nAlphas > maxAlphas) {
      maxAlphas = nAlphas;
    }
  }
