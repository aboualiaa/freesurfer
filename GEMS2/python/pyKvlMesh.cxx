...
KvlMesh::KvlMesh(MeshPointer &aMesh) { mesh = aMesh; }

int KvlMesh::PointCount() const { return mesh->GetPoints()->Size(); }
...
void TransformCellData(MeshPointer mesh, const double *scaling) {
  // Also the reference position of the mesh has changed. Note, however, that we
  // don't actually have access to the original reference position, only some
  // sufficient statistics calculated from it. In particular, we have only the
  // three first columns of the matrix Z = inv( X ) = inv( [ p0 p1 p2 p3; 1 1 1
  // 1 ] ). The transformed position Xtrans is given by
  //
  ...
  // the deformation penatly, we can safely drop the translation part. In short,
  // we will calculate the first 3 columns of the matrix
  //
  ...
  //
  kvl::AtlasMesh::CellDataContainer *cellData =
      const_cast<kvl::AtlasMesh::CellDataContainer *>(mesh->GetCellData());
  for (kvl::AtlasMesh::CellDataContainer::Iterator it = cellData->Begin();
       it != mesh->GetCellData()->End(); ++it) {
    it.Value().m_ReferenceVolumeTimesK *=
...
void KvlMesh::Scale(const SCALE_3D &scaling) {
  ...
  if (scaleShape == 1) {
    ...
    itkExceptionMacro("mesh scaling dimensions must be either 1 or 3 not "
                      << scaleShape);
  }
...
void KvlMeshCollection::Construct(const SHAPE_3D &meshSize,
  ...
  if (domainSize.size() != 3) {
    itkExceptionMacro("domainSize must have 3 values not "
                      << domainSize.size());
  }
...
void KvlMeshCollection::Read(const std::string &meshCollectionFileName) {
  if (!meshCollection->Read(meshCollectionFileName.c_str())) {
    itkExceptionMacro("Couldn't read mesh collection from file "
...
void ReverseTetrahedronSidedness(MeshCollectionPointer meshCollection) {
  // std::cout << "Careful here: the applied transformation will turn positive
  // tetrahedra into negative ones." << std::endl; std::cout <<
  // transform->GetMatrix().GetVnlMatrix() << std::endl; std::cout << "
  // determinant: " << determinant << std::endl; std::cout << "Starting to swap
  // the point assignments of each tetrahedron..." << std::endl;
...
void KvlMeshCollection::Write(const std::string &meshCollectionFileName) {
  if (!meshCollection->Write(meshCollectionFileName.c_str())) {
    itkExceptionMacro("Couldn't write mesh collection to file "
                      << meshCollectionFileName);
  }
  std::cout << "Wrote mesh collection: " << meshCollectionFileName << std::endl;
}

void KvlMeshCollection::SetK(double k) { meshCollection->SetK(k); }

double KvlMeshCollection::GetK() const { return meshCollection->GetK(); }
...
void KvlMeshCollection::SetReferencePosition(
    const py::array_t<double> &source) {
  PointSetPointer points =
      const_cast<PointSetPointer>(meshCollection->GetReferencePosition());
  CopyNumpyToPointSet(points, source);
...
void KvlMeshCollection::SetPositions(
  ...
  std::vector<kvl::AtlasMeshCollection::PointsContainerType::Pointer>
      pointsVector;
...
KvlMesh *KvlMeshCollection::GetMesh(int meshNumber) {
  if (meshNumber < -1) {
    ...
    if (meshIndex >= meshCount) {
      itkExceptionMacro("meshNumber " << meshNumber << " exceeds mesh count of "
                                      << meshCount);
    } else {
...
py::array_t<uint16_t> KvlMesh::RasterizeMesh(std::vector<size_t> size,
  ...
  if (classNumber >= 0) {
    //        // Rasterize the specified prior. If the class number is 0, then
    //        pre-fill everything
    //        // so that parts not overlayed by the mesh are still considered to
    //        the background
    ...
    //            dims[ i ] = imageSize[ i ];
    //        }
    //        //plhs[ 0 ] = mxCreateNumericArray( 3, dims, mxSINGLE_CLASS,
    //        mxREAL );
    ...
    for (int classNumber = 0; classNumber < numberOfClasses; classNumber++) {
      ...
      itk::ImageRegionConstIterator<MultiAlphasImageType> alphasIt(
          alphasImage, alphasImage->GetBufferedRegion());
...
void CopyNumpyToPointSet(PointSetPointer points,
  ...
  if (sourceNumberOfNodes != currentNumberOfNodes) {
    itkGenericExceptionMacro("source point count of "
                             << sourceNumberOfNodes
  ...
  for (auto pointsIterator = points->Begin(); pointsIterator != points->End();
       ++pointsIterator, ++pointIndex) {
    for (int xyzAxisSelector = 0; xyzAxisSelector < XYZ_DIMENSIONS;
         xyzAxisSelector++) {
      pointsIterator.Value()[xyzAxisSelector] =
          *source.data(pointIndex, xyzAxisSelector);
    }
  }
}
...
void CreatePointSetFromNumpy(PointSetPointer targetPoints,
  ...
  for (unsigned int pointIndex = 0; pointIndex < sourceNumberOfNodes;
       ++pointIndex) {
...
void CopyNumpyToPointDataSet(PointDataPointer destinationAlphas,
  ...
  if (numberOfLabels <= 0) {
    itkGenericExceptionMacro("source data have positive number of labels not "
                             << numberOfLabels);
  }
  unsigned int pointIndex = 0;
  for (auto alphasIterator = destinationAlphas->Begin();
       alphasIterator != destinationAlphas->End();
       ++alphasIterator, ++pointIndex) {
