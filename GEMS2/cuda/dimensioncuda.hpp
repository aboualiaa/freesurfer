...
namespace kvl {
namespace cuda {
template <unsigned char nDims, typename IndexType = size_t> class Dimension {
public:
  Dimension() {
  ...
  bool operator!=(const Dimension &d) const { return !(this->operator==(d)); }
...
private:
  ...
  template <typename NextType, typename... Values>
  void copyToArray(IndexType *nextLoc, const NextType iVal,
  ...
  template <typename T> void CheckForIndexType() const {
