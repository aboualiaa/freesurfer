...
//
//
void AtlasMeshCollectionModelLikelihoodCalculator ::GetDataCostAndAlphasCost(
    double &dataCost, double &alphasCost, bool verbose) const {
  ...
  for (AtlasMesh::PointDataContainer::ConstIterator pointParamIt =
       ...
       pointParamIt != m_MeshCollection->GetPointParameters()->End();
       ++pointParamIt) {
  ...
  for (int labelImageNumber = 0; labelImageNumber < m_LabelImages.size();
    ...
    for (; statIt != statisticsCollector->GetLabelStatistics()->End();
         ++statIt, ++pixelCountIt) {
