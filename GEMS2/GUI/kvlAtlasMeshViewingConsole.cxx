...
//
//
void AtlasMeshViewingConsole ::LoadMeshCollection(
    const char *fileName, const int *templateSize,
    ...
    for (AtlasMesh::PointsContainer::ConstIterator it =
      ...
      for (int i = 0; i < 3; i++) {
        if (it.Value()[i] > size[i]) {
          size[i] =
              static_cast<ImageType::SizeType::SizeValueType>(it.Value()[i]);
        }
      }
    }
    size[0]++;
  ...
  if (backgroundImageFileName.size()) {
    ...
    m_BackgroundImage =
        AtlasMeshViewingConsole::ReadBackgroundImage(backgroundImageFileName);
    ...
    vtkSmartPointer<vtkLookupTable> overlayImageLookupTable =
        vtkLookupTable::New();
  ...
  if (m_Compressor->Read("compressionLookupTable.txt")) {
    ...
    std::cout << "Couldn't find a compressionLookupTable.txt -- using default "
                 "colors/names"
              << std::endl;
  ...
  m_ImageViewer->GetSliceLocation(sagittalSliceNumber, coronalSliceNumber,
                                  axialSliceNumber);
...
//
//
void AtlasMeshViewingConsole ::Show() {
  m_Window->show();
  Fl::run();
}
...
//
//
void AtlasMeshViewingConsole ::Draw() {
  if (!m_MeshCollection) {
    return;
  }
  ...
  if (!m_ShowSummary->value()) {
    ...
    AtlasMeshSummaryDrawer::Pointer summaryDrawer =
        AtlasMeshSummaryDrawer::New();
...
//
//
void AtlasMeshViewingConsole ::SetSliceLocation(
    unsigned int sagittalSliceNumber, unsigned int coronalSliceNumber,
    unsigned int axialSliceNumber) {
...
//
//
void AtlasMeshViewingConsole ::ShowSelectedView() {
...
//
//
void AtlasMeshViewingConsole ::GetScreenShot() {
...
//
//
void AtlasMeshViewingConsole ::GetScreenShotSeries() {
  ...
  for (unsigned int meshNumber = 0;
       meshNumber < m_MeshCollection->GetNumberOfMeshes(); meshNumber++) {
...
AtlasMeshViewingConsole ::ReadBackgroundImage(
    const std::string &backgroundImageFileName) {
...
//
//
void AtlasMeshViewingConsole ::GetScreenShotSeries(int directionNumber) {
  ...
  if (m_InvertOrder->value()) {
    startSliceNumber =
        m_ImageViewer->GetMaximumImageIndex()[directionNumber] - 1;
    incrementSliceNumber = -1;
  }

  sliceLocation[directionNumber] = startSliceNumber;
  for (int i = 0; i < m_ImageViewer->GetMaximumImageIndex()[directionNumber];
       i++, sliceLocation[directionNumber] += incrementSliceNumber) {
    this->SetSliceLocation(sliceLocation[0], sliceLocation[1],
                           sliceLocation[2]);
