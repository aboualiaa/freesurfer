...
AtlasParameterEstimationConsole ::AtlasParameterEstimationConsole() {
  ...
  command->SetCallbackFunction(
      this, &AtlasParameterEstimationConsole::HandleEstimatorEvent);
  ...
  m_PositionEstimationResolution->value(
      m_Estimator->GetPositionEstimationIterationEventResolution());
...
void AtlasParameterEstimationConsole ::SetLabelImages(
  ...
  for (int classNumber = 0; classNumber < lookupTable->GetNumberOfClasses();
       classNumber++) {
...
void AtlasParameterEstimationConsole ::InitializeMesh() {
  ...
  if (m_UseExplicitStartCollection->value() != 0) {
    ...
    std::cout << "meshSize: " << meshSize[0] << " " << meshSize[1] << " "
              << meshSize[2] << std::endl;
    std::cout << "domainSize: " << domainSize[0] << " " << domainSize[1] << " "
              << domainSize[2] << std::endl;
...
//
//
void AtlasParameterEstimationConsole ::Show() {
  m_Window->show();
  Fl::run();
}
...
//
//
void AtlasParameterEstimationConsole ::DisplayLabelImage(
  ...
  itk::ImageRegionIterator<CompressedImageType> compIt(
      compressedImage, compressedImage->GetLargestPossibleRegion());
  for (; !origIt.IsAtEnd(); ++origIt, ++compIt) {
    compIt.Set(static_cast<CompressedImageType::PixelType>(
        m_Estimator->GetCompressionLookupTable()->GetClassNumbers(
            origIt.Get())[0]));
  }
...
void AtlasParameterEstimationConsole ::Estimate() {
  ...
  if (m_UseExplicitStartCollection->value() != 0) {
    ...
    const int numberOfUpsamplingSteps =
        static_cast<const int>(m_NumberOfUpsamplingSteps->value());
    float initialStiffness = m_InitialStiffness->value();

    for (int upsamplingStepNumber = 0;
      ...
      AtlasMeshCollection::Pointer meshCollection =
          const_cast<AtlasMeshCollection *>(
              m_Estimator->GetCurrentMeshCollection());
      ...
      fileNameStream << "estimatedAtUpsamplingStep" << upsamplingStepNumber
                     << "Of" << numberOfUpsamplingSteps << ".txt";
      m_Estimator->GetCurrentMeshCollection()->Write(
...
//
//
void AtlasParameterEstimationConsole ::HandleEstimatorEvent(
  ...
  if (typeid(event) == typeid(itk::EndEvent)) {
    ...
    // Show total progress
    float totalProgress =
        static_cast<float>(m_Estimator->GetIterationNumber() + 1) /
    ...
    m_SubProgress->label(m_SubProgressLabel.c_str());
    float subProgress =
        static_cast<float>(m_Estimator->GetAlphasEstimationIterationNumber() +
                           1) /
        static_cast<float>(
            m_Estimator->GetAlphasEstimationMaximumNumberOfIterations());
    ...
    // Show sub progress
    std::ostringstream labelStream;
    labelStream << "Estimating position of label image "
                << m_Estimator->GetLabelImageNumber() + 1 << " / "
    ...
    float subProgress =
        (static_cast<float>(m_Estimator->GetLabelImageNumber()) +
         static_cast<float>(
             m_Estimator->GetPositionEstimationIterationNumber() + 1) /
             static_cast<float>(
                 m_Estimator
                     ->GetPositionEstimationMaximumNumberOfIterations())) /
...
//
//
void AtlasParameterEstimationConsole ::Interrupt() { m_Interrupted = true; }

//
//
//
void AtlasParameterEstimationConsole ::Step() {
  m_Stepping = true;
  this->Continue();
}
...
//
//
void AtlasParameterEstimationConsole ::Continue() { m_Interrupted = false; }

//
//
//
void AtlasParameterEstimationConsole ::SetPositionEstimationResolution(
    unsigned int positionEstimationResolution) {
  m_Estimator->SetPositionEstimationIterationEventResolution(
      positionEstimationResolution);
}
