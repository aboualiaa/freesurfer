...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: mris_parcellate_connectivity.c,v 1.2 "
  ...
  // The parameter used to be called pct_ but was multiplied by without dividing
  // by 100 so I don't know if the 100 is deliberate or not.  Left unchanged to
  // not change the behaviour.

  printf("ico %d read, %d faces, %d vertices (%d, %d max)\n", ico_no,
         mris_ico->nvertices, mris_ico->nfaces, mris_ico->max_vertices,
         mris_ico->max_faces);
  ...
  if (navgs > 0)
    MRISsmoothMRI(mris, mri_cmatrix, navgs, nullptr, mri_cmatrix);
  ...
  MRISwrite(mris_ico, "lh.ico");
  printf("dividing icosahedral edges...\n");
  ...
  // figure out which vertices are on boundary between regions
  {
    ...
    for (ico_down = ico_no - (nsub - 2); ico_down <= ico_no; ico_down++) {
      ...
      MRISfree(&mris_ico_base);
      mris_ico_base = mris_ico_down;
    }
  ...
  {
    char fname[STRLEN];
    sprintf(fname, "%s.sub%d.ico",
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh", nsub);
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input surface> <input correlations> <output "
         "parcellation>\n",
         Progname);
...
double MRIScomputeUniformityEnergy(MRI_SURFACE *mris) { return (0.0); }
