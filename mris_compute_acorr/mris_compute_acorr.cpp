...
#define ARGV_OFFSET 4
  ...
  for (n = 0; n < num_class1; n++) {
    fprintf(stderr, "processing subject %d of %d: %s\n", n + 1,
            num_class1 + num_class2, c1_subjects[n]);
  ...
  for (n = 0; n < num_class2; n++) {
    fprintf(stderr, "processing subject %d of %d: %s\n", n + 1 + num_class1,
            num_class1 + num_class2, c2_subjects[n]);
...
static int write_acorr(const char *fname, double *acorr, double *counts,
