...
static int        Lookat(int, int, int, unsigned char, int *, Cell **, int *,
                         Cell *adtab[27], STRIP_PARMS *parms, MRI_variables *MRI_var);
...
static int Merge(unsigned char i, unsigned char j, unsigned char k, int val,
...
static void MRISfit(MRI_variables *MRI_var,
                    void (*calcforce)(double &force0, double &force1,
                                      double &force, double sd,
                                      const TVector &Pos, const TVector &S,
                                      const TVector &N, MRI_variables *MRI_var,
                                      double forceParam),
                    double forceParam);
static void calcForce1(double &force0, double &force1, double &force, double sd,
                       const TVector &Pos, const TVector &S, const TVector &N,
                       MRI_variables *MRI_var, double forceParam);
static void calcForce2(double &force0, double &force1, double &force, double sd,
                       const TVector &Pos, const TVector &S, const TVector &N,
                       MRI_variables *MRI_var, double forceParam);
static void calcForceGM(double &force0, double &force1, double &force,
                        double sd, const TVector &Pos, const TVector &S,
                        const TVector &N, MRI_variables *MRI_var,
                        double forceParam);
static void calcForceMine(double &force0, double &force1, double &force,
                          double sd, const TVector &Pos, const TVector &S,
                          const TVector &N, MRI_variables *MRI_var,
                          double forceParam);
...
int (*MyvoxelToWorld)(MRI *mri, double xv, double yv, double zv, double *xw,
                      double *yw, double *zw);
int (*MyworldToVoxel)(MRI *mri, double xw, double yw, double zw, double *pxv,
...
void getTimeString(char *buf) {
  ...
  sprintf(buf, "%d-%d-%d", lTime->tm_mon + 1, lTime->tm_mday,
          lTime->tm_year + 1900);
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    fprintf(stderr, "\noptional command -forceParam val: change pushout force "
                    "(default 1.0)");
    fprintf(stderr,
...
MRI *MRIstripSkull(MRI *mri_with_skull, MRI *mri_without_skull,
  ...
  if (!mri_without_skull)
    mri_without_skull = MRIclone(mri_with_skull, nullptr);
...
static int Pre_CharSorting(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  ...
  // calculate initial estimate of COG coords and Imax (xCOG, yCOG, zCOG, and
  // Imax)
  ...
  if (parms->rb != -1) {
    ...
    r                  = (int)(MRI_var->rad_Brain / 2);
    fprintf(stderr, "\n      modification of the brain radius to %.2f",
            MRI_var->rad_Brain);
  }

  MRI_var->estimated_size =
      (unsigned long)(4.19 * pow(MRI_var->rad_Brain, 3)); // 4.19 = 4PI/3

  fprintf(stderr,
          "\n      first estimation of the main basin volume: %ld voxels",
          MRI_var->estimated_size);
  ...
  if (MRI_var->WM_INTENSITY < MRI_var->CSF_intensity) {
    ...
    fprintf(stderr,
            "********************************************************\n");
    fprintf(stderr,
            "White matter intensity %d is lower than CSF intensity %d.\n",
  ...
  for (k = 0; k < 256; k++) {
    fprintf(stderr, "\n%d %ld %f", k, number[k],
            (float)(number[k]) / MAX(1, intensity_percent[k]));
...
static int Lookat(int i, int j, int k, unsigned char val, int *dpt,
                  Cell **admax, int *nb, Cell *adtab[27], STRIP_PARMS *parms,
                  MRI_variables *MRI_var) {
  int           t, n;
  ...
  for (k = 2; k < MRI_var->depth - 2; k++)
    for (j = 2; j < MRI_var->height - 2; j++) {
      ...
      for (i = 2; i < MRI_var->width - 2; i++) {
        ...
        /*right now: only type 0="empty", 1=node to 2 or 3,2=auxiliar basin
         * ,3=main basin to be freed */
        ...
        case 2:
          if (parms->watershed_analyze) {
            if (true) /*Ambiguous(cell1) instead of 1
                                                                            if
                      you want to use the ambiguous variable*/
            {
              if (cell != cell1) {
                cell->type = 5;
              ...
              if (cell1->type == 9)
                ...
                if (TRYMERGE(i, j, k, MRI_var)) {
                  ...
                  if (bcell->ambiguous >= parms->threshold_analyze *
                                              ...
                                              100) {
                    fprintf(stderr,
                            "\n      ambiguous basin, merged: at least %ld "
                            "ambiguous voxels; size: %ld voxels",
      ...
      for (i = 2; i < MRI_var->width - 2; i++) {
        ...
        case 6:
          ...
          if (((BasinCell *)MRI_var->Basin[k][j][i].next)->ambiguous)
            fprintf(stderr,
                    "      ambiguous basin, non merged: %ld ambiguous voxels; "
                    "size: %ld voxels\n",
...
static void brain_params(MRI_variables *MRI_var) {
  ...
  if (MRI_var->rad_Brain < 30) {
    if (MRI_var->WM_INTENSITY == 110 && MRI_var->WM_VARIANCE == 5 &&
      ...
      fprintf(
          stderr,
          "\n      second estimation of the COG coord: x=%d,y=%d, z=%d, r=%d",
          (int)MRI_var->xCOG, (int)MRI_var->yCOG, (int)MRI_var->zCOG,
          (int)MRI_var->rad_Brain);
...
static void calcForce1(double &force0, double &force1, double &force, double sd,
  ...
  for (int h = -noutside; h < 0; h++) {
    ...
    MyworldToVoxel(MRI_var->mri_orig, (x - nx * h), (y - ny * h), (z - nz * h),
                   &tx, &ty, &tz);
    int kt = (int)(tz + 0.5);
    int jt = (int)(ty + 0.5);
    int it = (int)(tx + 0.5);
    ...
    if (samp_mean[0] < fzero && samp_mean[1] < fzero) // dark region.  pushin
      ...
      if (nb_TR >= 3)
        force = -0.2;
               ...
               samp_mean[0] > MRI_var->TRANSITION_INTENSITY) // push out
        force = 2.1;                                         // 0.7;
      else if (nb_GM == 2 &&
               samp_mean[0] > MRI_var->TRANSITION_INTENSITY) // push out
        force = 1.5;                                         // 0.5;
...
static void MRISfit(MRI_variables *MRI_var,
  ...
  for (iter = 0; niter; iter++) {
    ...
    for (k = 0; k < mris->nvertices; k++) {
      ...
      TVector dX;
      for (m = 0; m < vt->vnum; m++) {
        TVector Vert(mris->vertices[vt->v[m]].tx, mris->vertices[vt->v[m]].ty,
      ...
      MRISsetXYZ(mris, k, v->x + dX.x, v->y + dX.y, v->z + dX.z);
    }
