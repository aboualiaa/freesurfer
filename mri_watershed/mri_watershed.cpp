...
#define ISNT_L_OR_R(lab)                                                       \
  (IS_BRAIN(lab) && (lab == 14 || lab == 15 || lab == 16 || lab == 21 ||       \
                     lab == 22 || lab == 23 || lab == 24 || lab == 72 ||       \
                     lab == 77 || lab == 80 || lab == 85))
#define IS_LEFT(lab)                                                           \
  (IS_BRAIN(lab) && !ISNT_L_OR_R(lab) &&                                       \
   (lab < 40 || lab == 73 || lab == 75 || lab == 78 || lab == 81 ||            \
    lab == 83 || lab == 96))
...
// The old code was using a MRIS to hold this data but was not storing
// positional xyz coordinates in the xyz fields.  This class replaces that use
// of the MRIS.
//
class Sphere {
#define SPHERE_ELTS                                                            \
  ELT(int, marked)                                                             \
  SEP ELT(float, x) SEP ELT(float, y) SEP ELT(float, z) SEP ELT(float, tx) SEP \
  ELT(float, ty) SEP                                                           \
  ELT(float, tz) SEP                                                           \
  ELT(float, dx) SEP                                                           \
  ELT(float, dy) SEP                                                           \
...
public:
  Sphere(MRIS *mris)
      : mris(mris), status(mris->status), nvertices(mris->nvertices),
        vertices(nullptr) {
    ...
    for (int vno = 0; vno < nvertices; vno++) {
      ...
      for (int vnb = 0; vnb < vt->vnum; vnb++) {
        VERTEX const *const vn =
            &sphere->mris
                 ->vertices[vt->v[vnb]]; /* neighboring vertex pointer */
        val += vn->val;
      }
...
int calCOGMAX(MRI_variables *MRI_var, STRIP_PARMS *parms, int *x, int *y,
              int *z);
...
int Merge(unsigned char i, unsigned char j, unsigned char k, int val, int *n,
...
template <typename T>
void DebugCurve(const T *int_percent, const int max, const char *msg);
...
void calcForce1(double &force0, double &force1, double &force, const double &x,
                const double &y, const double &z, const double &sx,
                const double &sy, const double &sz, const double &sd,
                const double &nx, const double &ny, const double &nz,
                MRI_variables *mri_var, STRIP_PARMS *parms, int kv);
void calcForce2(double &force0, double &force1, double &force, const double &x,
                const double &y, const double &z, const double &sx,
                const double &sy, const double &sz, const double &sd,
                const double &nx, const double &ny, const double &nz,
                MRI_variables *mri_var, STRIP_PARMS *parms, int kv);
// pass a function pointer for force calculation
void FitShape(MRI_variables *MRI_var, STRIP_PARMS *parms, const int convLimit,
              const int maxIter,
              void (*calcForce)(double &force0, double &force1, double &force,
                                const double &x, const double &y,
                                const double &z, const double &sx,
                                const double &sy, const double &sz,
                                const double &sd, const double &nx,
                                const double &ny, const double &nz,
                                MRI_variables *mri_var, STRIP_PARMS *parms,
                                int kv));
...
void init_surf_to_image(float rx, float ry, float rz, MRI_variables *MRI_var);
void write_image(MRI_variables *MRI_var);
void init_direction(MRI_variables *MRI_var);
void find_normal(const float nx, const float ny, const float nz, float *n1,
                 float *n2, float direction[26][3]);
void local_params(STRIP_PARMS *parms, MRI_variables *MRI_var);
void analyseCSF(unsigned long CSF_percent[6][256], MRI_variables *MRI_var,
                int i);
void analyseGM(unsigned long CSF_percent[6][256],
               unsigned long int_percent[6][256], MRI_variables *MRI_var,
               int i);
...
double mrisComputeCorrelationErrorLocal(MRI_SURFACE *mris,
                                        INTEGRATION_PARMS *parms, int use_stds);

void SphereChangeCoordinates(MRIS *mris, const Sphere *input);
void MRISChangeCoordinates(MRIS *mris, const MRIS *input);

int mrisRigidBodyAlignGlobal(MRIS *mris, MRIS *mris_dist,
                             INTEGRATION_PARMS *parms, int mode,
                             float min_degrees, float max_degrees, int nangles);

int mrisLocalizeErrors(MRIS *mris_curv, MRIS *mris_dCOG, MRI_variables *MRI_var,
                       Sphere *sphere);
...
void MRISscaleFields(MRIS *mris_src, MRIS *mris_dst, MRIS *mris_vdst,
                     int whichfield);
void MRISgoToClosestDarkestPoint(MRI_variables *MRI_var);
/*mri->type correction*/

void MRI_weight_atlas(MRI *mri_with_skull, TRANSFORM *transform,
                      STRIP_PARMS *parms);
void FindSeedPrior(STRIP_PARMS *parms, MRI_variables *MRI_var);
int AtlasToRegion(int i, int j, int k, STRIP_PARMS *parms,
                  MRI_variables *MRI_var);
...
int (*myWorldToVoxel)(MRI *mri, double xw, double yw, double zw, double *xv,
                      double *yv, double *zv);
int (*myVoxelToWorld)(MRI *mri, double xv, double yv, double zv, double *xw,
                      double *yw, double *zw);
///////////////////////////////////////////////////////////////////

#include "mri_watershed.help.xml.h"
void usageHelp() {
  outputHelpXml(mri_watershed_help_xml, mri_watershed_help_xml_len);
}
...
int main(int argc, char *argv[]) {
  ...
  int nargs, c, r, s;
  MRI *mri_with_skull, *mri_without_skull = nullptr, *mri_mask;
  MRI *PreEditVol = nullptr, *PostEditVol = nullptr, *mritmp = nullptr;
  ...
  if ((parms->Tregion || parms->seedprior || parms->basinprior ||
       parms->preweight || parms->preweightemp) &&
      !parms->transform)
  ...
  if (parms->KeepEdits) {
    ...
    if (PreEditVol == nullptr) {
      exit(1);
    }
    PostEditVol = MRIread(parms->PostEditVolName);
    if (PostEditVol == nullptr) {
      exit(1);
    }
  }
  ...
  if (parms->transform) {
    ...
    if (parms->transform->type != LINEAR_VOX_TO_VOX) {
      MATRIX *i_to_r = nullptr, *r_to_i = nullptr, *tmpmat = nullptr, *vox2vox;
      MRI *mri_buf = MRIallocHeader(
          mri_with_skull->width, mri_with_skull->height, mri_with_skull->depth,
          mri_with_skull->type, mri_with_skull->nframes);
      ...
      // dst invalid
                                         ...
                                         // (usually ras-to-ras comes from MNI
                                         // transform)
      {
        mri_buf->c_r = -0.095;
        mri_buf->c_a = -16.51;
    ...
    parms->mri_atlas_brain =
        MRIalloc((parms->gca)->prior_width, (parms->gca)->prior_height,
                 (parms->gca)->prior_depth, MRI_FLOAT);
    parms->mri_atlas_cerebellum =
        MRIalloc((parms->gca)->prior_width, (parms->gca)->prior_height,
                 (parms->gca)->prior_depth, MRI_FLOAT);
    parms->mri_atlas_cerebellumgw =
        MRIalloc((parms->gca)->prior_width, (parms->gca)->prior_height,
                 (parms->gca)->prior_depth, MRI_FLOAT);
        ...
        for (x = 0; x < (parms->gca)->prior_width; x++) {
          ...
          for (k = 0; k < gcap->nlabels; k++) {
            ...
            value_cer += IS_CEREBELLAR_WM(gcap->labels[0]) *
                         IS_CEREBELLAR_WM(gcap->labels[k]) * gcap->priors[k];
  ...
  if (parms->preweightemp || conformed ||
      type_changed) /* make output volume the same type as input volume */
  {
    ...
    mri_without_skull = MRImask(mri_with_skull, mri_mask, nullptr, 0, 0);
  }
  mri_mask = mri_without_skull;
  ...
  if (parms->KeepEdits) {
    ...
    mritmp = MRIcopy(mri_without_skull, nullptr);
...
static STRIP_PARMS *init_parms() {
  ...
  sp->KeepEdits = 0;
  sp->PreEditVolName = nullptr;
  sp->PostEditVolName = nullptr;
...
static MRI_variables *init_variables(MRI *mri_with_skull) {
  ...
  v->mris = nullptr;
  v->mris_curv = nullptr;
  v->mris_var_curv = nullptr;
  v->mris_dCOG = nullptr;
  v->mris_var_dCOG = nullptr;
  v->sphere = nullptr;
      ...
      for (x = 0; x < mri_with_skull->width; x++) {
        ...
        */
...
#else
        GCAsourceVoxelToPrior(parms->gca, mri_with_skull, transform, x, y, z,
                              &xp, &yp, &zp);
#endif
        ...
        if (xp > 0 && yp > 0 && zp > 0 && xp < 128 && yp < 128 && zp < 128)
          value = val_buff *
                  (lambda + (1 - lambda) * MRIgetVoxVal(parms->mri_atlas_brain,
                                                        xp, yp, zp, 0));
        else {
          value = 0; // val_buff;
        }
...
#if INDIVIDUAL_TIMERS
  std::cout << __FUNCTION__ << ": Complete in " << tTotal.milliseconds()
            << " ms" << std::endl;
#endif
}
...
MRI *MRIstripSkull(MRI *mri_with_skull, MRI *mri_without_skull,
                   STRIP_PARMS *parms) {
  char fname[512];
  MRI_variables *MRI_var = nullptr;
  MRI *mri_tp;
  double vol_elt;

  if (mri_with_skull == nullptr) {
    Error("\nNULL input volume !\n");
  }
  ...
  mri_tp = MRIclone(mri_with_skull, nullptr);
  mri_tp = MRIcopy(mri_with_skull, nullptr);
  ...
  if (!mri_without_skull) {
    mri_without_skull =
        MRIclone(mri_with_skull, nullptr); // just copy to output
  ...
  //  MRI_var->mri_src    //sk weighted  //   mri_without_skull  // NO IMAGE
  //  //MRI_var->mri_orig // sk no weighed   //   mri_with_skull  // sk no
  //  weighed
...

  {
    ...
    if (parms->surf_dbg) {
      MRI_var->mri_dst = MRIclone(mri_with_skull, nullptr);
      MRI_var->mri_dst = MRIcopy(mri_with_skull, nullptr);
    }
    ////////////////////////////////////////////////////////////////////
    // gaussian step
      ...
      for (k = 0; k < MRI_var->depth; k++)
        for (j = 0; j < MRI_var->height; j++)
          for (i = 0; i < MRI_var->width; i++)
            MRIvox(MRI_var->mri_src, i, j, k) = (unsigned short)MIN(
                255, scale * MRIvox(mri_with_skull, i, j, k));
    } else {
      int i, j, k;
      for (k = 0; k < MRI_var->depth; k++)
        for (j = 0; j < MRI_var->height; j++)
    ...
    vol_elt = MRI_var->mri_src->xsize * MRI_var->mri_src->ysize *
  ...
  if (parms->template_deformation && parms->surf) {
    ...
    read_geometry(1, MRI_var, nullptr);

    init_surf_to_image(1.8 * MRI_var->rad_Brain, 1.8 * MRI_var->rad_Brain,
                       1.8 * MRI_var->rad_Brain, MRI_var);
  ...
  if (parms->template_deformation && parms->surf_dbg) {
    mri_without_skull = MRIcopy(MRI_var->mri_dst, nullptr);
  }
  /*normal mode saving*/
  else {
    mri_without_skull = MRIcopy(MRI_var->mri_src, nullptr);
  }
...
void AnalyzeT1Volume(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  ...
  if (parms->T1) {
    ...
    if (parms->hpf == 25) {
      ...
      fprintf(stdout,
              "\n      modification of the "
              "preflooding height to %d percent",
              parms->hpf);
    }
...
int Pre_CharSorting(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  ...
  if (MRI_var->WM_intensity < MRI_var->CSF_intensity) {
    ...
    fprintf(stdout, "***********************************************\n");
    fprintf(stdout,
            "White matter intensity %d is lower than CSF "
            "intensity %d.\n",
  ...
  if (retVal > 0) {
    ...
    if (i < 0 || i >= MRI_var->mri_src->width || j < 0 ||
        j >= MRI_var->mri_src->height || k < 0 ||
        k >= MRI_var->mri_src->depth) {
      printf("WM coord (%d, %d, %d) out of bounds (%d, %d, %d), normalization "
             "failed?\n",
             i, j, k, MRI_var->mri_src->width, MRI_var->mri_src->height,
             MRI_var->mri_src->depth);
    ...
    if (parms->T1) {
      ...
      mri_tmp = MRIclone(MRI_var->mri_src, nullptr);
      for (k = 0; k < MRI_var->depth; k++)
        for (j = 0; j < MRI_var->height; j++) {
    ...
    fprintf(stdout, "\n      global maximum in x=%d, y=%d, z=%d, Imax=%d",
            MRI_var->i_global_min, MRI_var->j_global_min, MRI_var->k_global_min,
            MRI_var->Imax);

    fprintf(stdout, "\n      CSF=%d, WM_intensity=%d, WM_VARIANCE=%d",
            MRI_var->CSF_intensity, MRI_var->WM_intensity,
            MRI_var->WM_VARIANCE);
    fprintf(stdout,
            "\n      WM_MIN=%d, WM_HALF_MIN=%d, "
            "WM_HALF_MAX=%d, WM_MAX=%d ",
            MRI_var->WM_MIN, MRI_var->WM_HALF_MIN, MRI_var->WM_HALF_MAX,
            MRI_var->WM_MAX);
    ...
    if (MRI_var->WM_MIN <= 2 * MRI_var->CSF_intensity) {
      ...
      MRI_var->WM_HALF_MIN = MAX(MRI_var->WM_HALF_MIN,
...
/*-----------------------------------------------------
  ...
  Description:
  ------------------------------------------------------*/
void AnalyzeCerebellum(STRIP_PARMS *parms, MRI_variables *MRI_var, float &mean,
                       float &var) {
#if INDIVIDUAL_TIMERS
  std::cout << __FUNCTION__ << ": Begin" << std::endl;
  Timer tTotal;
  ...
  for (k = 2; k < MRI_var->depth - 2; k++)
    for (j = 2; j < MRI_var->height - 2; j++)
      for (i = 2; i < MRI_var->width - 2; i++) {
        ...
        rp.GetFloor(xp, yp, zp);
#else
        GCAsourceVoxelToPrior(parms->gca, MRI_var->mri_src, parms->transform, i,
                              j, k, &xp, &yp, &zp);
#endif
        if (xp > 0 && yp > 0 && zp > 0 && xp < 128 && yp < 128 && zp < 128)
...
void FindSeedPrior(STRIP_PARMS *parms, MRI_variables *MRI_var) {
    ...
    for (j = 2; j < MRI_var->height - 2; j++) {
      for (i = 2; i < MRI_var->width - 2; i++) {
        ...
        if (n < 2) {
          ...
          rp.GetFloor(xp, yp, zp);
#else
          GCAsourceVoxelToPrior(parms->gca, MRI_var->mri_src, parms->transform,
                                i, j, k, &xp, &yp, &zp);
#endif
          ...
          if (xp > 0 && yp > 0 && zp > 0 && xp < 128 && yp < 128 && zp < 128 &&
            ...
            if (vox < mean_cer + var_cer / 2 && vox > mean_cer - var_cer / 2 &&
                MRIgetVoxVal(parms->mri_atlas_cerebellum, xp, yp, zp, 0) >
                    0.9) {
              seeds[n][0] = i;
              seeds[n][1] = j;
              seeds[n][2] = k;
  ...
  for (k = 2; k < MRI_var->depth - 2; k++)
    for (j = 2; j < MRI_var->height - 2; j++)
      for (i = 2; i < MRI_var->width - 2; i++) {
        if (n < number_seed) {
          vox = MRIvox(MRI_var->mri_src, i, j, k);
          if (vox >= MRI_var->WM_MIN && vox <= MRI_var->WM_MAX &&
            ...
            rp.GetFloor(xp, yp, zp);
#else
            GCAsourceVoxelToPrior(parms->gca, MRI_var->mri_src,
                                  parms->transform, i, j, k, &xp, &yp, &zp);
#endif
            if (xp > 0 && yp > 0 && zp > 0 && xp < 128 && yp < 128 &&
                zp < 128) {
  ...
  {
    MRI_seedpoint = MRIalloc(MRI_var->width, MRI_var->height, MRI_var->depth,
  ...
  if (MRI_var->WM_intensity + MRI_var->WM_VARIANCE >= 80) {
    ...
    for (k = 0; k < MRI_var->depth; k++)
      for (j = 0; j < MRI_var->height; j++)
        for (i = 0; i < MRI_var->width; i++)
          MRIvox(MRI_var->mri_src, i, j, k) = (unsigned short)MIN(
              255, scale * MRIvox(MRI_var->mri_src, i, j, k));
...
int Analyze(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  ...
  MRI_var->main_basin_size +=
      ((BasinCell *)MRI_var
           ->Basin[MRI_var->k_global_min][MRI_var->j_global_min]
  ...
  for (n = 0; n < parms->nb_seed_points; n++)
    MRI_var->main_basin_size +=
        ((BasinCell *)MRI_var
             ->Basin[parms->seed_coord[n][2]][parms->seed_coord[n][1]]
  ...
  MRIvox(MRI_var->mri_src, MRI_var->i_global_min, MRI_var->j_global_min,
         MRI_var->k_global_min) = MRI_var->int_global_min;
  for (n = 0; n < parms->nb_seed_points; n++)
    MRIvox(MRI_var->mri_src, parms->seed_coord[n][0], parms->seed_coord[n][1],
           parms->seed_coord[n][2]) = parms->seed_coord[n][3];
...
int Lookat(int i, int j, int k, unsigned char val, int *dpt, Cell **admax,
           int *nb, Cell *adtab[27], STRIP_PARMS *parms,
           MRI_variables *MRI_var) {
  int t, n;
  ...
  if (add->type) {
    ...
    return 0;
  } else if (100 * (val - MRIvox(MRI_var->mri_src, i, j, k)) <
             hp * MRI_var->Imax) {
    return 1;
  }
...
int MergeRoutine(unsigned char i, unsigned char j, unsigned char k, int val,
                 int *n, MRI_variables *MRI_var) {
  int cond = 15 * val;
  Bound *buff;
...
int TRYMERGE(int i, int j, int k, MRI_variables *MRI_var) {
  ...
  for (a = -1; a <= 1; a++)
    for (b = -1; b <= 1; b++)
      for (c = -1; c <= 1; c++)
        if (MRI_var->Basin[k + c][j + b][i + a].next ==
            MRI_var->Basin[k][j][i].next)
...
void BASIN_PRIOR(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  int i = 0, j = 0, k;
  Cell *cell = nullptr, *cell1 = nullptr, *cellg = nullptr;
      ...
      for (i = 2; i < MRI_var->width - 2; i++) {
        ...
        if (cell->type == 2 &&
            ((BasinCell *)cell->next)->prior / ((BasinCell *)cell->next)->size >
                nu) {
          cell->type = 1;
          ((BasinCell *)(cellg->next))->size +=
              ((BasinCell *)(cell->next))->size;
...
int PostAnalyze(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  int i, j, k, p, q;
  BUFTYPE *pb;
  Cell *cell, *cell1 = nullptr, *celltp = nullptr;
  Bound *buff;
  BasinCell *bcell;
  int val;
  ...
  for (k = 2; k < MRI_var->depth - 2; k++)
    for (j = 2; j < MRI_var->height - 2; j++) {
      ...
      for (i = 2; i < MRI_var->width - 2; i++) {
        ...
        case 2:
          if (parms->watershed_analyze) {
            if (true) /*Ambiguous(cell1) instead of 1
...
void Template_Deformation(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  ...
  read_geometry(0, MRI_var, nullptr);
  ...
  ////////////////////////////////////////////////////////////////////////
  init_surf_to_image(0.8 * MRI_var->rad_Brain, 0.8 * MRI_var->rad_Brain,
                     0.8 * MRI_var->rad_Brain, MRI_var);
  ...
  if (parms->template_deformation != 2) {
    ...
    MRISscaleFields(MRI_var->mris, MRI_var->mris_curv, MRI_var->mris_var_curv,
                    CURV_MODE);
    MRISscaleFields(MRI_var->mris, MRI_var->mris_dCOG, MRI_var->mris_var_dCOG,
                    DIST_MODE);
...
void brain_params(MRI_variables *MRI_var) {
  ...
  if (MRI_var->rad_Brain < 30) {
    if (MRI_var->WM_intensity == 110 && MRI_var->WM_VARIANCE == 5 &&
        MRI_var->WM_MAX == 110) {
      Error("\n Watershed Error !\n");
    } else {
      fprintf(stdout,
  ...
  for (k = 0; k < mris->nfaces; k++) {
    ...
    for (v = 0; v <= numv; v++) {
      ...
      for (u = 0; u <= numu; u++) {
        ...
        if (i >= 0 && i < MRI_var->width && j >= 0 && j < MRI_var->height &&
...
void find_normal(const float nx, const float ny, const float nz, float *n1,
  ...
  for (p = 0; p < 26; p++) {
    ps_buff =
        direction[p][0] * nx + direction[p][1] * ny + direction[p][2] * nz;
    if (ps_buff < 0) {
      ps_buff = -ps_buff;
    }
    if (ps_buff < ps) {
    ...
    for (h = 0; h < ninside - 4; h++) {
      ...
      for (c = 0; c < 4; c++)    // 3 x 3 x 4
        for (a = -1; a < 2; a++) // a and b are orthogonal to normal
          for (b = -1; b < 2; b++) {
            find_normal(mris->vertices[kv].nx, mris->vertices[kv].ny,
                        mris->vertices[kv].nz, n1, n2, MRI_var->direction);

            px = mris->vertices[kv].x - (h + c) * mris->vertices[kv].nx +
                 a * n1[0] + b * n2[0];
            py = mris->vertices[kv].y - (h + c) * mris->vertices[kv].ny +
                 a * n1[1] + b * n2[1];
            pz = mris->vertices[kv].z - (h + c) * mris->vertices[kv].nz +
                 a * n1[2] + b * n2[2];
            ...
            if (!(k < 0 || k >= MRI_var->depth || i < 0 ||
                  i >= MRI_var->width || j < 0 || j >= MRI_var->height)) {
...
void local_params(STRIP_PARMS *parms, MRI_variables *MRI_var) {
  ...
  for (try_bigger_wm_lims = 0; try_bigger_wm_lims <= 1; try_bigger_wm_lims++) {
    ...
    for (kv = 0; kv < mris->nvertices; kv++) {
      ...
      for (h = -2; h < 2; h++) // near surface area by h=-2, -1, 0, 1
      {
        ...
        if (!(k < 0 || k >= MRI_var->depth || i < 0 || i >= MRI_var->width ||
      ...
      if (val < 3 * MRI_var->CSF_intensity) {
        for (a = -1; a < 2; a++) // orthogonal to normal direction
        {
          ...
          {
            px = mris->vertices[kv].x - rp * mris->vertices[kv].nx + a * n1[0] +
                 b * n2[0];
            py = mris->vertices[kv].y - rp * mris->vertices[kv].ny + a * n1[1] +
                 b * n2[1];
            pz = mris->vertices[kv].z - rp * mris->vertices[kv].nz + a * n1[2] +
                 b * n2[2];
  ...
  if (parms->Tregion == 1) {
    for (j = 0; j < 6; j++)
      fprintf(stdout,
              ...
              region_to_name(j), MRI_var->CSF_MIN[j], MRI_var->CSF_intens[j],
              MRI_var->CSF_MAX[j], n[j]);
  } else
    fprintf(stdout,
            "\n %s   CSF_MIN=%d, CSF_intensity=%d, CSF_MAX=%d , nb = %i",
            region_to_name(GLOBAL), MRI_var->CSF_MIN[GLOBAL],
            MRI_var->CSF_intens[GLOBAL], MRI_var->CSF_MAX[GLOBAL], n[GLOBAL]);
  ...
  for (kv = 0; kv < mris->nvertices; kv++) {
    ...
    if (h < ninside - 4) // h is between 0 and ninside-4
    {
      ...
      for (; c < h; c++) {
        for (a = -1; a < 2; a++) {
          for (b = -1; b < 2; b++) {
            px = mris->vertices[kv].x - c * mris->vertices[kv].nx + a * n1[0] +
                 b * n2[0];
            py = mris->vertices[kv].y - c * mris->vertices[kv].ny + a * n1[1] +
                 b * n2[1];
            pz = mris->vertices[kv].z - c * mris->vertices[kv].nz + a * n1[2] +
                 b * n2[2];
            myWorldToVoxel(MRI_var->mri_dst, px, py, pz, &tx, &ty, &tz);
            i = (int)(tx + 0.5);
            j = (int)(ty + 0.5);
            ...
            if (!(k < 0 || k >= MRI_var->depth || i < 0 ||
              ...
              if (val_buff < MRI_var->WM_MIN - MRI_var->WM_VARIANCE / 2 &&
                  val_buff > MRI_var->WM_MIN / 4) {
                val = MRIvox(MRI_var->mri_src, i, j, k);
  ...
  for (j = 0; j < 6; j++) {
    ...
    if (DZERO(denom) || !isfinite((float)denom)) {
      ...
      MRI_var->TRANSITION_intensity[j] =
          ...
          (MRI_var->GM_intensity[j] - MRI_var->CSF_intens[j]) *
              (MRI_var->CSF_MAX[j] - MRI_var->CSF_intens[j]) / denom;
    }

    fprintf(stdout,
            "\n  %s  \n  before analyzing :    %d,      %d,        %d,   %d",
            region_to_name(j), MRI_var->CSF_MAX[j],
    ...
    MRI_var->GM_MIN[j] =
        MAX(MAX(MRI_var->TRANSITION_intensity[j], MRI_var->GM_MIN[j]),
            MRI_var->WM_MIN / 4);
    ...
    if (!parms->skull_type) // default
    {
      ...
      MRI_var->CSF_HALF_MAX[j] = MIN(
          MRI_var->CSF_intens[j] / 2 +
    ...
    } else if (parms->skull_type == 1) // more
    {
      if (MRI_var->GM_MIN[j] == MRI_var->TRANSITION_intensity[j]) {
        ...
        MRI_var->GM_intensity[j] =
            MIN(MRI_var->GM_MIN[j], (MRI_var->GM_intensity[j] +
                                     3 * MRI_var->TRANSITION_intensity[j]) /
                                        4);
      } else {
        MRI_var->TRANSITION_intensity[j] =
        ...
        MRI_var->GM_intensity[j] =
            (MRI_var->GM_intensity[j] + 3 * MRI_var->TRANSITION_intensity[j]) /
            4;
      }
      MRI_var->CSF_HALF_MAX[j] =
          MIN((MRI_var->CSF_intens[j] +
               MIN(MRI_var->CSF_MAX[j], MRI_var->TRANSITION_intensity[j])) /
                  2,
              MRI_var->CSF_HALF_MAX[j]);
      if (MRI_var->CSF_MAX[j] >= MRI_var->TRANSITION_intensity[j])
        MRI_var->CSF_MAX[j] =
    ...
    fprintf(stdout, "\n  after  analyzing :    %d,      %d,        %d,   %d",
            MRI_var->CSF_MAX[j], MRI_var->TRANSITION_intensity[j],
    ...
    if (parms->manual_params == 1) {
      ...
      fprintf(stdout,
              "\n    after  analyzing :    %d,      %d,        %d,   %d",
              MRI_var->CSF_MAX[j], MRI_var->TRANSITION_intensity[j],
              MRI_var->GM_intensity[j], MRI_var->GM_MIN[j]);
    }
  }
  fflush(stdout);
}
...
int AtlasToRegion(int i, int j, int k, STRIP_PARMS *parms,
  ...
  if (GCAsourceVoxelToPrior(parms->gca, MRI_var->mri_src, parms->transform, i,
                            j, k, &xp, &yp, &zp) != NO_ERROR) {
    return (-1);
  }
  gcap = &parms->gca->priors[xp][yp][zp];
  if (xp >= 0 && yp >= 0 && zp >= 0 && xp < parms->gca->prior_width &&
      yp < parms->gca->prior_height && zp < parms->gca->prior_depth &&
      gcap->nlabels > 0) {
    int n;
    double max_prior;
...
void analyseGM(unsigned long CSF_percent[6][256],
  ...
  for (; k < 256; k++)
    int_percent[i][k] = (long unsigned int)(MAX(
        0, int_percent[i][k] - tmp * CSF_percent[i][k] / buff));
  ...
  for (; k < 256; k++)
    MRI_var->gmnumber[k] = (long unsigned int)(MAX(
        0, MRI_var->gmnumber[k] - tmp * CSF_percent[i][k] / buff));
...
void shrinkstep(MRI_variables *MRI_var) {
  ...
  double tx, ty, tz;
  float xmin = MRI_var->width, xmax = 0, ymin = MRI_var->height, ymax = 0,
        zmin = MRI_var->depth, zmax = 0;
  MRIS *mris;
  ...
  MRISscaleThenTranslate(mris, rx, ry, rz, MRI_var->xsCOG, MRI_var->ysCOG,
                         MRI_var->zsCOG);
...
void MRIShighlyTesselatedSmoothedSurface(MRI_variables *MRI_var) {
  ...
  for (iter = 0; niter; iter++) {
    ...
    for (k = 0; k < mris->nvertices; k++) {
      ...
      if ((kt < 0 || kt >= MRI_var->depth || it < 0 || it >= MRI_var->width ||
      ...
      MRISsetXYZ(mris, k, v->x + dx, v->y + dy, v->z + dz);
    }
...
void mean(float tab[4][9], float *moy) {
  ...
  for (p = 0; p < 4; p++)
    moy[p] =
        (2 * tab[p][4] + tab[p][1] + tab[p][3] + tab[p][5] + tab[p][7]) / 6;
...
void MRISsmooth_surface(MRI_SURFACE *mris, int niter) {
  ...
  for (iter = 0; iter < niter; iter++) {
    ...
    for (k = 0; k < mris->nvertices; k++) {
      ...
      MRISsetXYZ(mris, k, (v->x + x) / 2, (v->y + y) / 2, (v->z + z) / 2);
    }
      ...
      for (h = 0; h < 4; h++)
        for (a = -1; a < 2; a++)
          for (b = -1; b < 2; b++) {
            ...
            if ((kt < 0 || kt >= MRI_var->depth || it < 0 ||
                 it >= MRI_var->width || jt < 0 || jt >= MRI_var->height)) {
              val = 0;
            } else {
      ...
      if (!val) {
        ...
        if (samp_mean[0] < fzero && samp_mean[1] < fzero) {
          ...
          if (nb_TR >= 3) {
            ...
            for (h = 0; h < 4; h++) {
              for (a = 0; a < 9; a++) {
                if (test_samp[h][a] >= MRI_var->TRANSITION_intensity[0]) {
                  nb_GM++;
...
void label_voxels(STRIP_PARMS *parms, MRI_variables *MRI_var,
  ...
  volume_skull =
      MRI_var->depth * MRI_var->width * MRI_var->height - volume_skull;
  vol_elt = MRI_var->mri_src->xsize * MRI_var->mri_src->ysize *
            MRI_var->mri_src->zsize;
...
int ValidationSurfaceShape(MRI_variables *MRI_var) {
  ...
  if (MRI_var->mris_curv) {
    MRISfree(&MRI_var->mris_curv);
    MRI_var->mris_curv = nullptr;
  }
  if (MRI_var->mris_var_curv) {
    MRISfree(&MRI_var->mris_var_curv);
    MRI_var->mris_var_curv = nullptr;
  }
  if (MRI_var->mris_dCOG) {
    MRISfree(&MRI_var->mris_dCOG);
    MRI_var->mris_dCOG = nullptr;
  }
  if (MRI_var->mris_var_dCOG) {
    MRISfree(&MRI_var->mris_var_dCOG);
    MRI_var->mris_var_dCOG = nullptr;
  }
  ...
  sprintf(surf_fname, "%s/average/rigidly_aligned_brain_template.tif", mri_dir);
  ...
  if (MRI_var->atlas) {
    ...
    fprintf(stdout, "\nNo Rigid alignment: -atlas Mode Off (basic atlas / no "
                    "registration)");
  }
...
double mrisComputeCorrelationErrorLocal(MRI_SURFACE *mris,
  ...
  for (sse = 0.0f, var_sse = 0.0f, count = 0, vno = 0; vno < mris->nvertices;
       vno++) {
    v = &mris->vertices[vno];
    ...
    target = MRISPfunctionVal(parms->mrisp_template, mris->radius, x, y, z,
                              parms->frame_no);
...
void SphereChangeCoordinates(MRIS *mris, const Sphere *input) {
  ...
  for (p = 0; p < mris->nvertices; p++) {
    MRISsetXYZ(mris, p, input->vertices[p].x, input->vertices[p].y,
               input->vertices[p].z);
  }
...
int mrisRigidBodyAlignGlobal(MRIS *mris_curv, MRIS *mris_dist,
                             INTEGRATION_PARMS *parms, int mode,
  ...
  for (degrees = max_degrees; degrees >= min_degrees; degrees /= 2.0f) {
    ...
    fprintf(stdout,
            ...
            (float)DEGREES(degrees), min_sse, (float)DEGREES(mina),
...
int mrisLocalizeErrors(MRIS *mris_curv, MRIS *mris_dCOG, MRI_variables *MRI_var,
  ...
  for (k = 0; k < nvertices; k++) {
    if (sphere->vertices[k].val > 0.0) {
      ...
      if ((mris_dCOG->vertices[k].curv - MRI_var->mris_dCOG->vertices[k].curv) >
          0) {
...
void MRISscaleFields(MRIS *mris_src, MRIS *mris_fdst, MRIS *mris_vdst,
                     int whichfield) {
  int k;
  double tp;
...
void MRISComputeLocalValues(MRI_variables *MRI_var) {
  ...
  for (ref = 0, k = 0; k < nvertices; k++) {
    ...
    */
    ...
    for (dist = noutside; dist > ninside; dist -= 0.5) {
      ...
      if (csf_val < val && val < gm_val && csf_val < BOUNDCSF &&
          gm_val > MRI_var->CSF_intensity)
      /* in right range */
      {
        MRIsampleVolumeDerivativeScale(mri, xw, yw, zw, nx, ny, nz, &mag, 1.);
  ...
  if (VERBOSE_MODE)
    fprintf(stdout,
            ...
            100. * (float)nmissing / nvertices, mean_csf, sqrt(var_csf),
            mean_gray, sqrt(var_gray));
  ...
  while (niter) {
    ...
    for (k = 0; k < nvertices; k++) {
      ...
      n = 1;
      for (m = 0; m < v->vnum; m++) {
        csf += sphere->vertices[v->v[m]].tx * sphere->vertices[v->v[m]].ty;
        w1 += sphere->vertices[v->v[m]].ty;
        gm += sphere->vertices[v->v[m]].tz * sphere->vertices[v->v[m]].ty;
        n++;
      }
      vsphere->x = csf / w1;
      vsphere->z = gm / w1;
  ...
  while (niter) {
    ...
    for (k = 0; k < nvertices; k++) {
      ...
      for (m = 0; m < v->vnum; m++) {
        ...
        w1 += sphere->vertices[v->v[m]].ty;
        gm += sphere->vertices[v->v[m]].tz * sphere->vertices[v->v[m]].ty;
        var_csf += sphere->vertices[v->v[m]].tdx * sphere->vertices[v->v[m]].ty;
        var_gray +=
            sphere->vertices[v->v[m]].tdz * sphere->vertices[v->v[m]].ty;
  ...
  for (k = 0; k < nvertices; k++) {
    ...
    mean_gray += gm;
    var_gray += SQR(gm);
    vsphere->y = (csf * vsphere->odz + gm * vsphere->odx) /
                 (vsphere->odx + vsphere->odz);
  ...
  if (VERBOSE_MODE)
    fprintf(stdout,
            ...
            100. * (float)nmissing / nvertices, mean_csf, sqrt(var_csf),
            mean_gray, sqrt(var_gray), mean_trans, sqrt(var_trans));
...
static double estimateNRG(MRI_variables *MRI_var, double cx, double cy,
                          double cz);
static void computeGradient(MRI_variables *MRI_var, double cx, double cy,
                            double cz, double *gx, double *gy, double *gz);
static int computeCOG(MRI_variables *MRI_var, double *xCOG, double *yCOG,
                      double *zCOG, int mode);
...
static void computeGradient(MRI_variables *MRI_var, double cx, double cy,
  ...
  for (tx = 0, ty = 0, tz = 0, n = 0; n < mris->nvertices; n++) {
    ...
    tx += (1.0 - MRI_var->mris_dCOG->vertices[n].curv / dCOG) /
          MRI_var->mris_var_dCOG->vertices[n].curv * (mris->vertices[n].x - cx);

    ty += (1.0 - MRI_var->mris_dCOG->vertices[n].curv / dCOG) /
          MRI_var->mris_var_dCOG->vertices[n].curv * (mris->vertices[n].y - cy);

    tz += (1.0 - MRI_var->mris_dCOG->vertices[n].curv / dCOG) /
          MRI_var->mris_var_dCOG->vertices[n].curv * (mris->vertices[n].z - cz);
  }
...
static int computeCOG(MRI_variables *MRI_var, double *xCOG, double *yCOG,
  ...
  case 1:
    ...
    fprintf(stdout,
            "Computing COG for corrected surface\n"
    ...
    while (NRG < last_NRG) {
      ...
      while (NRG >= last_NRG) {
        epsilon /= 2.0;
        NRG = estimateNRG(MRI_var, x + epsilon * dx, y + epsilon * dy,
                          z + epsilon * dz);
        d = sqrt(SQR(dx) + SQR(dy) + SQR(dz));
...
static MRI *generateFinalMRI(MRI_variables *MRI_var) {
  ...
  float x0, y0, z0, x1, y1, z1, x2, y2, z2, d0, d1, d2, dmax, dCOG, val, px0,
      py0, pz0, px1, py1, pz1, px, py, pz;
  FACE *face;
  ...
  mri = MRIclone(MRI_var->mri_orig, nullptr);
  ...
  for (k = 0; k < mris->nfaces; k++) {
    ...
    for (u = 0; u < 3; u++) {
      ...
      val =
          MAX(val,
              MRI_var->mris_dCOG->vertices[face->v[u]].curv - dCOG +
    ...
    for (v = 0; v <= numv; v++) {
      ...
      for (u = 0; u <= numu; u++) {
        ...
        j = (int)(ty + 0.5);
        i = (int)(tx + 0.5);
        if (i >= 0 && i < mri->width && j >= 0 && j < mri->height &&
...
void MRISFineSegmentation(MRI_variables *MRI_var) {
  ...
  for (iter = 0; niter; iter++) {
    ...
    for (k = 0; k < mris->nvertices; k++) {
      ...
      if (!MRIGRADIENT) {
        ...
        for (h = 0; h < 4; h++)
          for (a = -1; a < 2; a++)
            for (b = -1; b < 2; b++) {
              myWorldToVoxel(
                  MRI_var->mri_orig, (x - nx * h + n1[0] * a + n2[0] * b),
                  (y - ny * h + n1[1] * a + n2[1] * b),
                  (z - nz * h + n1[2] * a + n2[2] * b), &tx, &ty, &tz);
              kt = (int)(tz + 0.5);
              jt = (int)(ty + 0.5);
        ...
        if (!val) /*|| val>fmax)*/
        {
          ...
          for (h = 1; h < 10; h++) {
            prev_val = val;
            myWorldToVoxel(MRI_var->mri_orig, (x - nx * h), (y - ny * h),
                           (z - nz * h), &tx, &ty, &tz);
            kt = (int)(tz + 0.5);
            jt = (int)(ty + 0.5);
            it = (int)(tx + 0.5);
        ...
        MRIsampleVolumeDerivativeScale(mri, xw, yw, zw, nx, ny, nz, &GradVal,
                                       1.);
      ...
      dx = sxt * 0.8 + force1 * sxn + v->nx * force + v->nx * force3 +
           v->nx * force4;
      dy = syt * 0.8 + force1 * syn + v->ny * force + v->ny * force3 +
           v->ny * force4;
      dz = szt * 0.8 + force1 * szn + v->nz * force + v->nz * force3 +
           v->nz * force4;
    ...
    if (MRI_var->atlas && iter && ((iter % 10) == 0)) {
      ...
      MRISscaleFields(mris, MRI_var->mris_dCOG, MRI_var->mris_var_dCOG,
                      DIST_MODE);
      MRISscaleFields(mris, MRI_var->mris_curv, MRI_var->mris_var_curv,
                      CURV_MODE);
      if (VERBOSE_MODE) {
        fprintf(stdout, "\nCompute Local Values csf/gray");
      }
  ...
  for (iter = 0; niter; iter++) {
    ...
    for (k = 0; k < mris->nvertices; k++) {
      ...
      // for (h=2; h>-3; h--)
      {
        ...
        myWorldToVoxel(mri, x, y, z, &xw, &yw, &zw);
        // if outside, then
        if ((zw < 0 || zw >= MRI_var->depth || xw < 0 || xw >= MRI_var->width ||
...
void calcForce1(double &fST, double &fSN, double &fN, const double &x,
                const double &y, const double &z, const double &sx,
                const double &sy, const double &sz, const double &sd,
                const double &nx, const double &ny, const double &nz,
                MRI_variables *mri_var, STRIP_PARMS *parms, int kv) {
  ...
  for (h = -noutside; h < 0; h++) // up to 15 voxels inside
  {
    ...
    myWorldToVoxel(mri_var->mri_src, (x - nx * h), (y - ny * h), (z - nz * h),
                   &tx, &ty, &tz);
    kt = (int)(tz + 0.5);
    jt = (int)(ty + 0.5);
    it = (int)(tx + 0.5);
    ...
    if (samp_mean[1] < MRI_var->TRANSITION_intensity[label] &&
      ...
      if (nb_TR >= 3) {
        valforce -= 0.2;
                 ...
                 samp_mean[0] > MRI_var->TRANSITION_intensity[label]) {
        valforce += 0.7;
      } else if (nb_GM == 2 &&
                 samp_mean[0] > MRI_var->TRANSITION_intensity[label]) {
        valforce += 0.5;
...
template <typename T>
void DebugCurve(const T *percent, const int max, const char *msg) {
#if DEBUG_CURVE
  T tmp = 0;
  if (percent == 0) {
    return;
