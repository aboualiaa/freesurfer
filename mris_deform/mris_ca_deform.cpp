  ...
  for (n_averages = max_averages, i = 0; n_averages >= min_averages;
    ...
    //    if (i == 0)
    {
      if (read_ll)
        ...
        compute_gradient_target_positions(mris, mri_intensities, vi,
                                          current_sigma);
      else
        mri_ll = compute_target_intensities(
            mris, mri_labels, mri_intensities, histos, vi, current_sigma,
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input surface> <label vol> <transform> "
         "<intensity vol> <output surface>\n",
         Progname);
  exit(code);
}
  ...
  for (x = 0; x < mri_ll->width; x++) {
    ...
    for (y = 0; y < mri_ll->height; y++) {
      for (z = 0; z < mri_ll->depth; z++) {
        ...
        for (nin = 0, pin = 0.0, d = -WSIZE / vsize; d < 0; d += vsize) {
          if (DZERO(d))
            continue;
          xw = xi + d * nx;
          yw = yi + d * ny;
        ...
        for (nout = 0, pout = 0.0, d1 = -1 / vsize; d1 <= 1.0 / vsize;
          ...
          for (d2 = -1.0 / vsize; d2 <= 1.0 / vsize; d2 += vsize) {
            xw = xi + d * nx + d1 * e1x + d2 * e2x;
            yw = yi + d * ny + d1 * e1y + d2 * e2y;
            zw = zi + d * nz + d1 * e1z + d2 * e2z;
#if 0
...
static double externalLLSSE(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  for (sse = 0.0, vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (d = -mri_ll->xsize / 2; d >= -MAX_DIST / mri_ll->xsize;
      ...
      y2 = yvd + d * nyd;
      z2 = zvd + d * nzd;
      MRIsampleVolume(mri_dist, x2, y2, z2, &dist);
      dist = fabs(dist);
      MRIsampleVolumeDerivativeScale(mri_dist, x2, y2, z2, nxd, nyd, nzd, &mag,
                                     sigma);
#if 1
      if (mag < 0)
        break;
...
static double externalLLGradient(MRI_SURFACE *mris, INTEGRATION_PARMS *parms) {
  ...
  for (sse = 0.0, vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (vno == Gdiag_no)
      printf("l_extern: v %d, voxel (%2.1f, %2.1f, %2.1f), N=(%2.2f, %2.2f, "
             "%2.2f), max ll = %2.4f at distance %2.2f\n",
