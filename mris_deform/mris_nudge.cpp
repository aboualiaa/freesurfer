...
int main(int argc, char *argv[]) {
  ...
  MRISrepositionSurface(mris, mri, target_vnos, target_vals, nvertices, nsize,
                        sigma, 0);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "vavgs")) {
    ...
    case 'V':
      ...
      printf("moving vertex %d to value %2.0f\n", target_vnos[nvertices],
             target_vals[nvertices]);
      nvertices++;
      nargs = 2;
      break;
    default:
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input surface> <input volume> <vertex> <target "
         "val> <nbhd> <output surf>\n",
         Progname);
  printf("\t\n");
  exit(code);
}
