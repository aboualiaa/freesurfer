...
// these shouldn't overlap with the ones like WHITE_VERTICES defined in
// mrisurf.h
...
static int extract_image_intensities(MRI_SURFACE *mris, MRI *mri,
                                     double in_dist, double out_dist, double x0,
                                     double y0, double z0, double nx, double ny,
                                     double nz, double step, int tsteps,
...
static int compute_best_neighborhood_profile(MRI_SURFACE *mris, MRI *mri,
                                             int vno, DP *dp);
static double find_min_rms(
    double *kernel, double *intensity_profile, int profile_len, char *fname,
    DP *dp, double wm_dist, double wm_offset, double min_gm_offset,
    double max_gm_offset, double gm_offset_step, int which_profile, int wm_len,
    int ig_len, int sg_len,
...
static int vp_copy_from_surface(MRI_SURFACE *mris, int which_src,
                                int which_dst);
...
static int recompute_target_locations(MRI_SURFACE *mris, MRI *mri_white,
                                      MRI *mri_l4, MRI *mri_pial, DP *dp);
static double find_optimal_locations(MRI_SURFACE *mris, MRI *mri, int vno,
                                     double max_inwards, double max_outwards,
                                     DP *dp, VP *vp, int skip);
static double compute_linear_combination(double *kernel, double *iprofile,
...
static double compute_profile_L2(double *kernel, double *iprofile, int nsamples,
                                 char *plot_fname, double step, double wm_dist,
                                 double *errors);
static double compute_profile_norm_corr(double *kernel, double *iprofile,
                                        int nsamples, char *plot_fname,
                                        double step, double wm_dist,
                                        double *errors);
static int
...
static double compute_targets(MRI_SURFACE *mris, MRI *mri, double sigma, DP *dp,
                              int skip);
static int compute_intensity_offsets(MRI_SURFACE *mris, MRI *mri, MRI *mri_dist,
                                     DP *dp);
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_deform.c,v 1.29 2011/03/02 00:04:31 nicks Exp $",
      "$Name:  $");
  ...
  if (read_flag) // initialize  with previously computed surfaces
  {
    ...
    if (label_fname) {
      v1_prior = LabelRead(nullptr, label_fname);
      if (v1_prior == nullptr)
    ...
    if (dp.use_intensity) // use precomputed intensity info instead of
                          // optimizing it
    {
    ...
    if (invert) {
      ...
      if (lt->dst.valid == 0 || lt->src.valid == 0) {
        fprintf(stderr, "WARNING:**********************************************"
                        "*****************\n");
        fprintf(stderr, "WARNING:dst volume infor is invalid.  Most likely "
                        "produce wrong inverse.\n");
        fprintf(stderr, "WARNING:**********************************************"
                        "*****************\n");
      }
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
    ...
    MRISunrip(mris);
    sprintf(fname, "%s.%s.wnormals.mgz",
  ...
  for (skip = 4, i = 0, current_sigma = dp.sigma, n_averages = max_averages;
    ...
    printf("----------- deforming surfaces with %d smoothing iterations, "
           "sigma=%2.3f, skip=%d, max_dist=%2.1f -------\n",
           n_averages, current_sigma, skip, dp.max_dist);
#define MAX_ITERATIONS_PER_SCALE 3
    for (j = 0; j < MAX_ITERATIONS_PER_SCALE; j++) {
      dp.max_dist =
          MIN(3 * dp.sigma, dp.max_dist); // no point going out that far
      ...
      printf("--- positioning surface, skip=%d, dist=%2.0f, j=%d, "
             "mean_dist=%2.5f, sigma=%2.1f ---\n",
      ...
      recompute_target_locations(mris, nullptr, nullptr, nullptr, &dp);
      if (Gdiag_no > 0) {
        VERTEX *v;
        double xv, yv, zv;
        ...
        MRISsurfaceRASToVoxelCached(mris, mri, v->targx, v->targy, v->targz,
                                    &xv, &yv, &zv);
        fprintf(stderr,
                "v %d, target location = (%2.1f, %2.1f, %2.1f), vox=(%2.0f, "
                "%2.0f, %2.0f), "
                "mag = %2.1f, dist=%2.2f\n",
                Gdiag_no, v->targx, v->targy, v->targz, xv, yv, zv, v->mean,
                v->d);
      }
      ...
      if (white_only == 0) {
        ...
        vp_copy_to_surface(mris, LAYERIV_VERTICES,
                           ORIGINAL_VERTICES); // for surf repulse
        ...
        vp_copy_to_surface(mris, WHITE_VERTICES,
                           ORIGINAL_VERTICES); // for surf repulse
...
#define MIN_MEAN_DIST 0.25
      mean_dist =
          (vp_mean(mris, WHITE_VERTICES) + vp_mean(mris, PIAL_VERTICES) +
           vp_mean(mris, LAYERIV_VERTICES)) /
          3;
  ...
  fprintf(stderr,
          "surface deformation took %d minutes"
          " and %d seconds.\n",
          minutes, seconds);
  exit(0);
  return (0);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "vavgs")) {
    ...
    fprintf(stderr,
            "searching multiple surface normal angles in optimization\n");
  } else if (!stricmp(option, "max_offset") ||
             !stricmp(option, "max_wm_offset")) {
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input surface> <input volume> <xform> <output "
         "surface>\n",
         Progname);
  printf("\t\n");
  exit(code);
...
#define WSIZE_MM 5 // diameter of region to histogram within
  ...
  for (nfound = nmissed = vno = 0; vno < mris->nvertices; vno++) {
    ...
    vp = (VERTEX_PARMS *)(v->vp);
    h = MRIhistogramVoxel(mri, 0, nullptr, nint(xv), nint(yv), nint(zv), wsize,
                          mri_dist, 0);
    ...
    if (peak > 0 &&
        fabs(vp->wm_val - dp->wm_val) < dp->max_wm_intensity_offset) {
  ...
  printf("%d vertices found (%2.2f%%, %d missed), mean wm border %2.1f\n",
         nfound, 100 * nfound / (double)(nfound + nmissed), nmissed,
         mean_white_border);
  if (nfound == 0)
    DiagBreak();
  return (NO_ERROR);
...
static double compute_targets(MRI_SURFACE *mris, MRI *mri, double sigma, DP *dp,
  ...
  MRI *mri_white = MRIclone(mri, nullptr);
  MRI *mri_pial = MRIclone(mri, nullptr);
  MRI *mri_l4 = MRIclone(mri, nullptr);
  double xv, yv, zv, d, target_val, xr, yr, zr, grad, max_grad, target_dist,
      mean_white_border, mean_pial_border, mean_l4_border, mean_dist,
      inward_dist, outward_dist, mean_abs_dist;
  ...
  if (Gdiag & DIAG_WRITE) {
    ...
    MRISstoreRipFlags(mris);
    MRISunrip(mris);
    sprintf(fname, "%s.%s.normals.%d.init.mgz",
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh", parms.base_name,
            i);
    printf("writing surface normals to %s\n", fname);
  ...
  if (Gdiag & DIAG_WRITE) {
    ...
    MRISstoreRipFlags(mris);
    MRISunrip(mris);
    sprintf(fname, "%s.%s.normals.%d.final.mgz",
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh", parms.base_name,
            i);
    printf("writing surface normals to %s\n", fname);
  ...
  for (mean_dist = 0.0, nfound = nmissed = vno = 0; vno < mris->nvertices;
       vno++) {
    if ((vno % 5000) == 0)
      printf("%d of %d vertices processed (%2.1f%%)\n", vno, mris->nvertices,
             100.0 * vno / mris->nvertices);
    ...
    if (dp->use_max_grad) {
      ...
      for (d = -max_dist; d <= max_dist; d += dp->step) {
        ...
        grad = MRISnormalIntensityGradient(mris, mri, xr, yr, zr, vp->nx,
                                           vp->ny, vp->nz, sigma);
      ...
      if (target_dist > -100) // found wm boundary
      {
        ...
        vp->pty = vp->py;
        vp->ptz = vp->pz;
        vp->l4_dist = sqrt(SQR(vp->l4x - vp->wx) + SQR(vp->l4y - vp->wy) +
                           SQR(vp->l4z - vp->wz));
        vp->pial_dist = sqrt(SQR(vp->px - vp->wx) + SQR(vp->py - vp->wy) +
                             SQR(vp->pz - vp->wz));
      } else {
        vp->found = 0;
        continue;
...
#endif
    {
      if (vno == Gdiag_no)
        DiagBreak(); // break here XXX
      target_dist = find_optimal_locations(mris, mri, vno, inward_dist,
                                           outward_dist, dp, vp, skip);
      if (vno == Gdiag_no)
        DiagBreak(); // zzz
      ...
      if (dp->use_intensity && target_dist < -100) {
        ...
        extract_image_intensities(mris, mri, 0, max_dist, vp->wx, vp->wy,
                                  vp->wz, vp->nx, vp->ny, vp->nz, dp->step, 1,
                                  intensity_profile, nullptr);
        wm_found = ig_found = sg_found = 0;
        for (i = 1; i < nint(max_dist / dp->step); i++) {
          if (wm_found == 0 &&
              intensity_profile[i] > dp->wm_val + dp->max_wm_intensity_offset) {
            wm_found = 1;
      ...
      if (target_dist < -100) {
        ...
        if (vno == Gdiag_no) {
          MRISsurfaceRASToVoxelCached(mris, mri, vp->l4tx, vp->l4ty, vp->l4tz,
                                      &xv, &yv, &zv);
          printf("v %d: layer IV target = %2.0f %2.0f %2.0f\n", vno, xv, yv,
                 zv);
        }
        // make sure both targets are in volume
        MRISsurfaceRASToVoxelCached(mris, mri, vp->wtx, vp->wty, vp->wtz, &xv,
                                    &yv, &zv);
        if (vno == Gdiag_no)
        ...
        if (MRIindexNotInVolume(mri, xv, yv, zv))
          ...
          MRISsurfaceRASToVoxelCached(mris, mri, vp->ptx, vp->pty, vp->ptz, &xv,
                                      &yv, &zv);
          if (vno == Gdiag_no)
    ...
    if (target_val < 0) // couldn't find a reasonable guess at border
    {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri, vp->l4tx, vp->l4ty, vp->l4tz, &xv,
                                  &yv, &zv);
      if (nint(xv) == Gx && nint(yv) == Gy && nint(zv) == Gz)
        DiagBreak();
  ...
  printf("%d vertices found (%2.2f%%, %d missed), mean dist %2.2f (%2.3f), "
         "mean val %2.2f : %2.2f : %2.2f\n",
...
#endif
  {
    ...
    vp_copy_to_surface(mris, WHITE_TARGETS, TARGET_VERTICES);
    mean_vdist =
        MRIScomputeVertexSpacingStats(mris, &sigma_vdist, nullptr, nullptr,
    ...
    for (outliers = vno = 0; vno < mris->nvertices; vno++) {
      ...
      if ((fabs(v->d - mn) > 4 * std) ||
          (is_outlier(mris, vno, WHITE_VERTICES))) {
        if (vno == Gdiag_no)
      ...
      } else // check target locations of this vertex and distance to neighbors
      {
        ...
        if (d / sigma_vdist > 8 && d > 2.5) {
          if (vno == Gdiag_no)
            printf("discarding vertex %d white as an outlier (%2.1f - %2.1f) > "
                   "4*%2.1f\n",
    ...
    for (outliers = vno = 0; vno < mris->nvertices; vno++) {
      ...
      if ((fabs(v->d - mn) > 4 * std) ||
          (is_outlier(mris, vno, PIAL_VERTICES))) {
        if (vno == Gdiag_no)
          printf("replacing vno %d distance %2.2f with %2.2f (std=%2.2f)\n",
                 vno, v->d, mn, std);
      ...
      } else // check target locations of this vertex and distance to neighbors
      {
        ...
        if (d / sigma_vdist > 8 && d > 2.5) {
          if (vno == Gdiag_no)
            printf("discarding vertex %d layer IV as an outlier (%2.1f - "
                   "%2.1f) > 4*%2.1f\n",
                   vno, mn, mean_vdist, sigma_vdist);
    ...
    for (outliers = vno = 0; vno < mris->nvertices; vno++) {
      ...
      } else // check target locations of this vertex and distance to neighbors
      {
        ...
        if (d / sigma_vdist > 8 && d > 2.5) {
          if (vno == Gdiag_no)
            printf("discarding vertex %d pial as an outlier (%2.1f - %2.1f) > "
                   "4*%2.1f\n",
                   vno, mn, mean_vdist, sigma_vdist);
  ...
  if (Gdiag & DIAG_WRITE) {
    ...
    sprintf(fname, "%s.%s.wtvals.%3.3d.mgz",
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh", dp->base_name,
            i);
...
double MRISnormalIntensityGradient(MRI_SURFACE *mris, MRI *mri, double xr,
  ...
  for (n = 0, val_outside = val_inside = 0.0, dist = step_size;
       dist <= 2 * sigma_mm; dist += step_size, n++) {
...
static int extract_image_intensities(MRI_SURFACE *mris, MRI *mri,
                                     ...
                                     double nz, double step, int tsteps,
                                     double *intensity_profile, char *fname) {
  double avg_val, val, x, y, z, xv, yv, zv, dx, dy, dz, dist, e1x, e1y, e1z,
      e2x, e2y, e2z, xt, yt, zt, norm, dxn, dyn, dzn, vstep;
  ...
  dx -= xv;
  dy -= yv;
  dz -= zv; // (dx, dy, dz) is now (nx, ny, nz) in voxel coords
  ...
  e2y = dzn;
  e2z = dxn;
  CROSS3(e1x, e1y, e1z, dxn, dyn, dzn, e2x, e2y,
         e2z); // find 2 vectors perpindicular d[xyz]
...
#endif
static double find_max_distance(MRI_SURFACE *mris, MRI *mri, double x, double y,
                                double z, double nx, double ny, double nz,
                                int grad_sign, double max_dist, double sigma,
                                double step, double *pval, double low_thresh,
                                double hi_thresh, double max_inside_dist) {
  ...
  for (dist = 0; dist <= max_dist; dist += step) {
    ...
    z1 = z + dist * nz;
    grad =
        MRISnormalIntensityGradient(mris, mri, x1, y1, z1, nx, ny, nz, sigma);
    if (grad * grad_sign < 0)
      break;
    MRISsurfaceRASToVoxelCached(mris, mri, x1, y1, z1, &xv, &yv, &zv);
    MRIsampleVolume(mri, xv, yv, zv, &val);
    if (val < low_thresh ||
        val > hi_thresh) // outside of allowable interior intensity range
    {
      if (inside) // don't go back outside if were previously in the interior of
                  // the region
      {
        dist -= step;
        x1 = x + dist * nx;
  ...
  // now compute exact location on by looking for the dist where the intensities
  // decrease
  dist -= step / 2;
  MRISsurfaceRASToVoxelCached(mris, mri, x + dist * nx, y + dist * ny,
                              z + dist * nz, &xv, &yv, &zv);
  dist += step / 2;
  MRIsampleVolume(mri, xv, yv, zv, &val);
  for (; dist <= max_dist; dist += step / 2) {
...
static int find_next_peak(double *intensity_profile, int index,
                          double *pmax_val, int len, int whalf) {
  int i, found, j;
...
static int find_previous_peak(double *intensity_profile, int index,
                              double *pmax_val, int len, int whalf) {
  int i, found, j;
...
static int find_next_valley(double *intensity_profile, int index,
                            double *pmin_val, int len, int whalf) {
  int i, found, j;
...
static int smooth_profile(double *src_intensity_profile,
                          double *dst_intensity_profile, int len,
                          double sigma) {
  int i;
  HISTOGRAM *h, *hs;
  ...
  hs = HISTOsmooth(h, nullptr, sigma);
  for (i = 0; i < len; i++)
...
static int mark_sg_index_okay_dark_csf(double *orig_intensity_profile,
                                       int *sg_index_okay, double step, int len,
                                       char *fname, int current_index,
                                       double outside_val, int min_wm_index,
                                       int *sg_indices, int *pmax_len,
                                       int current_sg, int max_out) {
  int index, i, j, min_index, max_index, pv_len;
  double valley_val, peak_val, previous_peak_val,
      intensity_profile[MAX_PROFILE_LEN];
  ...
  for (index = min_wm_index; index < len; index++) {
    sg_index_okay[index] =
        0; // assume it's on an upwards slope unless violated below
    ...
    if (sg_index_okay[index]) {
      valley_index =
          find_next_valley(intensity_profile, index, &valley_val, len, whalf);
      if (valley_index < 0)
        continue;
      valley_val = orig_intensity_profile[valley_index];

      if (valley_val > orig_intensity_profile[i] -
                           .15 * orig_intensity_profile[i]) // not a deep valley
      {
        previous_peak = find_previous_peak(intensity_profile, valley_index,
                                           &previous_peak_val, len, whalf);
        peak_index = find_next_peak(intensity_profile, valley_index, &peak_val,
                                    len, whalf);
        if (peak_index >= 0 && previous_peak >= 0) {
          ...
          if ((peak_val - previous_peak_val) > previous_peak_val * .1)
            sg_index_okay[index] =
                0; // if the next peak is a lot higher than this one
        }
      }
    } else // on an upwards slope - mark everthing on the slope no good except
           // peak
    {
      peak_index =
          find_next_peak(intensity_profile, index, &peak_val, len, whalf);
      for (; index < peak_index; index++)
        sg_index_okay[index] = 0;
    }
    if (use_partial_volume_model && sg_index_okay[index]) {
      int j;
      /* smoothing and partial volume models mean that the edge location can
         move around some from the location of the peak, so test some additional
         locations
      */
  ...
  if (fp) {
    for (index = 0; index < len; index++)
      fprintf(fp, "%d %d %f\n", index, sg_index_okay[index],
              intensity_profile[index]);
    fclose(fp);
  }
...
static int mark_sg_index_okay_bright_csf(
    double *orig_intensity_profile, int *sg_index_okay, double step, int len,
    char *fname, int current_index, double outside_val, int min_wm_index,
    int *sg_indices, int *pmax_len, int current_sg, int max_out) {
  ...
  for (index = min_wm_index; index < len; index++) {
    sg_index_okay[index] =
        0; // assume it's on a downards slope unless violated below
    if (index < min_index || index > max_index)
      continue;
    for (i = index; i <= index + 1 && i < len; i++) {
      for (j = i + 1; j <= index + SMOOTH_LEN && j < len; j++)
...
static int mark_wm_index_okay(double *intensity_profile, int *wm_index_okay,
                              int wm_len, double min_wm_intensity,
                              double max_wm_intensity, int current_index,
                              int len, char *fname, int max_in) {
  ...
  max_wm_len = 0;
  for (inside = wm_samples = 0, index = current_index - (wm_len - 1);
       index < len; index++) {
    if ((intensity_profile[index] >
         max_wm_intensity + .75 * (max_wm_intensity - min_wm_intensity)) &&
        (max_wm_len > wm_len / 2))
  ...
  max_wm_len = 0;
  for (inside = wm_samples = 0, index = current_index + wm_len - 1; index >= 0;
       index--) {
    if ((intensity_profile[index] >
         max_wm_intensity + .75 * (max_wm_intensity - min_wm_intensity)) &&
        (max_wm_len > wm_len / 2))
  ...
  if (inside) {
    for (index = wm_start - wm_samples; index >= 0; index--)
      wm_index_okay[index] = 0;
    for (index = current_index; index >= 0 && index > wm_start - wm_samples;
         index--)
      wm_index_okay[index] = 1;
  }
...

{
  int start_index, current_index, min_gray_matter_len, max_wm_len, filter_type,
      ...
      best_wm_len, best_csf_len, best_start_index,
      sg_index_okay[MAX_PROFILE_LEN], index, wm_index_okay[MAX_PROFILE_LEN], n,
      sg_indices[MAX_PROFILE_LEN], nsg, wm_skip = skip;
  double errors[MAX_PROFILE_LEN], best_wm_dist, prior, kernel[MAX_PROFILE_LEN],
      nx, ny, nz, base_intensity_profile[MAX_PROFILE_LEN], wm_dist;
  ...
  int ig_len, sg_len, best_ig_len = 0, best_sg_len = 0, min_ig_len, max_ig_len,
                      ico_vno, min_sg_len, max_sg_len,
                      best_profile = PROFILE_GENERIC, tsteps, min_wm_index,
                      min_start_index;
  double best_wm_intensity_offset = 0.0, min_wm_intensity_offset,
         max_wm_intensity_offset, wm_intensity_offset, min_wm_intensity,
         max_wm_intensity, best_nx, best_ny, best_nz, best_sg_intensity_offset,
         sg_intensity_offset, ig_intensity_offset = 0, best_ig_intensity_offset,
         min_wm_len;
  ...
  static MRI_SURFACE *mris_ico = nullptr;
  ...
  if (vno == Gdiag_no || deform_debug) {
    ...
    fname1 = fname2 = fname3 = fname4 = nullptr;
  ...
  default:
  case ERROR_FUNC_L1:
    profile_error_func = compute_profile_L1;
    break;
  case ERROR_FUNC_L2:
    profile_error_func = compute_profile_L2;
    break;
  case ERROR_FUNC_NORM_CORR:
    profile_error_func = compute_profile_norm_corr;
    break;
  case ERROR_FUNC_NCORR_L1:
    profile_error_func = compute_linear_combination;
    break;
  }
  switch (error_type) {
  default:
  ...
  min_ig_len = nint(dp->min_ig_width / step);
  max_ig_len = nint(dp->max_ig_width / step);
  min_sg_len = nint(dp->min_sg_width / step);
  max_sg_len = nint(dp->max_sg_width / step);

  min_gray_matter_len = min_ig_len + min_sg_len;

  best_wm_len = best_ig_len = best_sg_len = best_start_index = best_csf_len = 0;

  ico_vno = MRISfindClosestVertex(mris_ico, vp->nx, vp->ny, vp->nz, nullptr,
                                  CURRENT_VERTICES);
  VERTEX_TOPOLOGY const *const vico = &mris_ico->vertices_topology[ico_vno];
  best_nx = vp->nx;
  best_ny = vp->ny;
  best_nz = vp->nz;
  best_ig_intensity_offset = best_sg_intensity_offset =
      best_wm_intensity_offset = 0.0;
  for (n = -1; n < vico->vnum; n++) {
    ...
    pdist = sqrt(SQR(vp->px - vp->wx) + SQR(vp->py - vp->wy) +
                 SQR(vp->pz - vp->wz));
    profile_len = max_profile_len =
        nint((max_inwards + max_outwards + pdist) / step);
    ...
    if (dp->use_max_grad) // white matter location is already found
    {
      ...
      max_profile_len =
          nint((2 * max_inwards + 2 * max_outwards + pdist) / step);
      extract_image_intensities(
          mris, mri, 2 * max_inwards, 2 * max_outwards + pdist, vp->wx, vp->wy,
          vp->wz, nx, ny, nz, step, tsteps, base_intensity_profile, fname2);
...
#define WHALF 5
      min_wm_index = HISTOfindMaxDerivative(h, min_wm_intensity,
                                            max_wm_intensity, WHALF, 1);
      if (vno == Gdiag_no)
        DiagBreak();
    ...
    else if (dp->fix_intensities)// mark a region around current locations
    {
      ...
      extract_image_intensities(mris, mri, max_inwards, max_outwards + pdist,
                                vp->wx, vp->wy, vp->wz, nx, ny, nz, step,
                                tsteps, base_intensity_profile, fname2);
      min_wm_index = mark_wm_index_okay(
          base_intensity_profile, wm_index_okay, nint(.4 / step),
          min_wm_intensity, max_wm_intensity, current_index, max_profile_len,
          fname4, nint(max_inwards / step));
      if (dp->dark_csf)
        mark_sg_index_okay_dark_csf(
            base_intensity_profile, sg_index_okay, step, max_profile_len,
            fname3, current_index, dp->outside_val + 10, min_wm_index,
            sg_indices, &nsg, current_index + nint(pdist / step),
            nint(max_outwards / step));
      else
        mark_sg_index_okay_bright_csf(
            base_intensity_profile, sg_index_okay, step, max_profile_len,
            fname3, current_index, dp->outside_val - 10, min_wm_index,
            sg_indices, &nsg, current_index + nint(pdist / step),
            nint(max_outwards / step));
    ...
    for (max_wm_index = min_wm_index + 1;
         max_wm_index < max_profile_len - min_wm_len; max_wm_index++)
      if (wm_index_okay[max_wm_index] == 0)
        break;

    for (start_index = min_start_index; start_index <= max_wm_index;
         start_index++) {
      ...
      for (wm_len = min_wm_len; wm_len <= max_wm_len; wm_len += wm_skip) {
        ...
        if (dp->fix_intensities) {
          ...
          wm_profile_val = compute_optimal_intensity(intensity_profile, 0,
                                                     wm_len - 2, filter_type);
        } else {
          wm_val = compute_optimal_intensity(intensity_profile, 0, wm_len - 2,
                                             filter_type);
        ...
        for (ig_len = min_ig_len; ig_len <= max_ig_len; ig_len += skip) {
          ...
          {
            ig_val =
                compute_optimal_intensity(intensity_profile, wm_len + 1,
          ...
          if (ig_val <=
              wm_profile_val + MIN_INTENISTY_PCT_OFFSET *
                                   wm_profile_val) // do some sanity checks
            continue;
          ...
          for (index = 0; index < nsg; index++) {
            ...
            for (csf_len = MIN_CSF_LEN; csf_len <= max_csf_len;
                 csf_len += skip) {
              // compute length of subset of total profile being used
              profile_len = wm_len + ig_len + sg_len + csf_len;
              if ((sg_val - ig_val > 2 * (ig_val - wm_val)) &&
                  (ig_val - wm_val < 25) && dp->use_max_grad == 0)
                continue; // enforce comparable contrast

              construct_model_profile(kernel, dp, wm_intensity_offset,
                                      ig_intensity_offset, sg_intensity_offset,
                                      0.0, wm_len, ig_len, sg_len, csf_len,
                                      PROFILE_GENERIC);

              prior = exp(-SQR(wm_dist) / (2 * SQR(dp->sigma)));
              rms =
                  (*profile_error_func)(kernel, intensity_profile, profile_len,
                                        nullptr, step, offset, errors);
              if (DZERO(prior))
                rms += 20;
              else
                rms -= log(prior);
              prior = exp(-SQR(wm_intensity_offset) /
                          (2 * SQR(dp->intensity_sigma)));
              if (DZERO(prior))
                rms += 20;
              else
                rms -= log(prior);
    ...
    if (dp->try_stria_model) {
      int stria_start, stria_end,
          stria_len = nint(dp->stria_width / dp->step),
          stria_offset_len =
              nint(STRIA_OFFSET /
                   dp->step); // amount of layerIV superficial to stria
      ...
      for (start_index = min_start_index; start_index <= max_wm_index;
        ...
        for (wm_len = min_wm_len; wm_len <= max_wm_len; wm_len += skip) {
          ...
          for (ig_len = min_ig_len; ig_len <= max_ig_len; ig_len += skip) {
            int i1 = MAX(wm_len + 2,
                         wm_len + ig_len - (stria_len + stria_offset_len) - 2);
            ...
            if (ig_val <= wm_val + MIN_INTENISTY_PCT_OFFSET *
                                       wm_val) // do some sanity checks
              continue;

            stria_start = wm_len + ig_len - (stria_len + stria_offset_len);
            stria_end = stria_start + stria_len - 1;
            stria_val = compute_optimal_intensity(
                intensity_profile, stria_start, stria_end, filter_type);
            ...
            for (index = 0; index < nsg; index++) {
              ...
              sg_val = compute_optimal_intensity(
                  ...
                  wm_len + ig_len + sg_len - 2, filter_type);
              if (sg_val <
                      dp->supra_granular_val - dp->max_sg_intensity_offset ||
                  sg_val > dp->supra_granular_val + dp->max_sg_intensity_offset)
                continue; // not in feasible region
              sg_intensity_offset = sg_val - dp->supra_granular_val;
              ...
              for (csf_len = MIN_CSF_LEN; csf_len <= max_csf_len;
                ...
                construct_model_profile(
                    kernel, dp, wm_intensity_offset, ig_intensity_offset,
                    sg_intensity_offset, stria_intensity_offset, wm_len, ig_len,
                    sg_len, csf_len, PROFILE_V1);

                prior = exp(-SQR(wm_dist) / (2 * SQR(dp->sigma)));
                rms = (*profile_error_func)(kernel, intensity_profile,
                                            profile_len, nullptr, step, offset,
                                            errors);
                if (DZERO(prior))
                  rms += 20;
                else
                ...
                if (rms < min_rms) {
                  if (vno == Gdiag_no || deform_debug) {
                    (*profile_error_func)(kernel, intensity_profile,
                                          profile_len, fname1, step, offset,
                                          errors);
                  }
      ...
      for (wm_len = min_wm_len; wm_len <= max_wm_len; wm_len += skip) {
        ...
        for (index = 0; index < nsg; index++) {
          ...
          for (csf_len = MIN_CSF_LEN; csf_len <= max_csf_len; csf_len += skip) {
            ...
            construct_model_profile(kernel, dp, wm_intensity_offset,
                                    ig_intensity_offset, sg_intensity_offset,
                                    0.0, wm_len, 0, sg_len, csf_len,
                                    PROFILE_AGRANULAR);

            prior = exp(-SQR(wm_dist) / (2 * SQR(dp->sigma)));
            rms = (*profile_error_func)(kernel, intensity_profile, profile_len,
                                        nullptr, step, offset, errors);
            if (DZERO(prior))
              rms += 20;
            else
              rms -= log(prior);
            prior =
                exp(-SQR(wm_intensity_offset) / (2 * SQR(dp->intensity_sigma)));
            if (DZERO(prior))
              rms += 20;
            else
              rms -= log(prior);
  ...
  if (dp->fix_intensities && best_wm_dist > -1000 &&
      false) { // do linear fit to intensities in range of wm boundary to get
               // subvoxel
    VECTOR *vY;
    MATRIX *mX, *mP, *mXinv;
    int i, wm_index;
...
#define NPOINTS 2
    ...
    if (wm_index > -NPOINTS) // enough points on both sides to do a fit
    {
      ...
      for (i = -NPOINTS; i <= NPOINTS; i++) {
        ...
        *MATRIX_RELT(vY, i + NPOINTS + 1, 1) =
            base_intensity_profile[i + wm_index];
      }
      mXinv = MatrixPseudoInverse(mX, nullptr);
      mP = MatrixMultiply(mXinv, vY, NULL);
      border_intensity = (dp->wm_val + best_wm_intensity_offset +
                          dp->infra_granular_val + best_ig_intensity_offset) /
                         2;
      best_wm_dist =
          (border_intensity - *MATRIX_RELT(mP, 1, 2)) / *MATRIX_RELT(mP, 1, 1);
      MatrixFree(&mX);
      VectorFree(&vY);
      MatrixFree(&mXinv);
      MatrixFree(&mP);
  ...
  if ((best_profile != PROFILE_AGRANULAR) && best_wm_dist > -100) {
    ...
    target_intensity = ((dp->supra_granular_val + best_sg_intensity_offset) +
                        (dp->infra_granular_val + best_ig_intensity_offset)) /
                       2;
    best_index = i0;
  ...
  if ((deform_debug || vno == Gdiag_no) && best_wm_dist > -100) {
    ...
    if (/*dp->fix_intensities || */ dp->use_max_grad) {
      ...
      pdist = sqrt(SQR(vp->px - vp->wx) + SQR(vp->py - vp->wy) +
                   SQR(vp->pz - vp->wz));
      extract_image_intensities(mris, mri, 2 * max_inwards,
                                2 * max_outwards + pdist, vp->wx, vp->wy,
                                vp->wz, best_nx, best_ny, best_nz, step, tsteps,
                                base_intensity_profile, fname2);
    } else
      extract_image_intensities(mris, mri, max_inwards, max_outwards + pdist,
                                vp->wx, vp->wy, vp->wz, best_nx, best_ny,
                                best_nz, step, tsteps, base_intensity_profile,
                                fname2);
    min_wm_index = mark_wm_index_okay(
        base_intensity_profile, wm_index_okay, nint(.4 / step),
        min_wm_intensity, max_wm_intensity, current_index, max_profile_len,
    ...
    if (dp->dark_csf)
      mark_sg_index_okay_dark_csf(
          base_intensity_profile, sg_index_okay, step, max_profile_len, fname3,
          current_index, dp->outside_val + 10, min_wm_index, sg_indices, &nsg,
    ...
    construct_model_profile(
        kernel, dp, best_wm_intensity_offset, best_ig_intensity_offset,
        best_sg_intensity_offset, best_stria_intensity_offset, best_wm_len,
        best_ig_len, best_sg_len, best_csf_len, best_profile);
    rms = (*profile_error_func)(kernel, intensity_profile, profile_len, fname1,
                                step, offset, errors);
    ...
    prior =
        exp(-(SQR(best_wm_intensity_offset) / (2 * SQR(dp->intensity_sigma))));
    if (DZERO(prior))
      rms += 20;
    else
      rms -= log(prior);

    MRISsurfaceRASToVoxelCached(mris, mri, vp->wtx, vp->wty, vp->wtz, &wx, &wy,
                                &wz);
    MRISsurfaceRASToVoxelCached(mris, mri, vp->ptx, vp->pty, vp->ptz, &px, &py,
                                &pz);
    MRISsurfaceRASToVoxelCached(mris, mri, vp->l4tx, vp->l4ty, vp->l4tz, &lx,
                                &ly, &lz);
...
static double compute_profile_L1(double *kernel, double *iprofile, int nsamples,
                                 char *fname, double step, double in_dist,
                                 double *errors) {
  double val, kval, L1;
  int i;
  FILE *fp = nullptr;
...
construct_model_profile(double *kernel, DP *dp, double wm_intensity_offset,
  ...
  case PROFILE_V1: // add the stria in
  {
    int stria_start,
        stria_len = nint(dp->stria_width / dp->step),
        stria_offset_len = nint(
            STRIA_OFFSET / dp->step); // amount of layerIV superficial to stria
    ...
    if (use_partial_volume_model) {
      stria_start =
          inside_len + infra_granular_len - (stria_len + stria_offset_len);
      if (stria_start > 0)
...
static double compute_linear_combination(double *kernel, double *iprofile,
                                         int nsamples, char *plot_fname,
                                         double step, double in_dist,
                                         double *errors) {
  double ncorr, L1;

  ncorr = compute_profile_norm_corr(kernel, iprofile, nsamples, plot_fname,
                                    step, in_dist, errors);
  L1 = compute_profile_L1(kernel, iprofile, nsamples, plot_fname, step, in_dist,
                          errors);
...
static int recompute_target_locations(MRI_SURFACE *mris, MRI *mri_white,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    if (mri_white) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_white, vp->wtx, vp->wty, vp->wtz,
                                  &xv, &yv, &zv);
      if (nint(xv) == Gx && nint(yv) == Gy && nint(zv) == Gz)
        DiagBreak();
      if (vno == Gdiag_no)
    ...
    if (mri_pial) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_pial, vp->ptx, vp->pty, vp->ptz,
                                  &xv, &yv, &zv);
      if (vno == Gdiag_no)
        printf("v %d: pial target = %2.0f %2.0f %2.0f\n", vno, xv, yv, zv);
      if (nint(xv) == Gx && nint(yv) == Gy && nint(zv) == Gz)
    ...
    if (mri_l4) {
      ...
      MRISsurfaceRASToVoxelCached(mris, mri_l4, vp->l4tx, vp->l4ty, vp->l4tz,
                                  &xv, &yv, &zv);
      if (vno == Gdiag_no)
        printf("v %d: layer IV target = %2.0f %2.0f %2.0f\n", vno, xv, yv, zv);
      if (nint(xv) == Gx && nint(yv) == Gy && nint(zv) == Gz)
...
static int fill_aseg(MRI *mri, char *aseg_fname, DP *dp) {
  ...
  if (mri_aseg == nullptr)
    ErrorExit(ERROR_NOFILE, "%s: couldn't read aseg volume from %s", Progname,
              aseg_fname);
...
static int is_outlier(MRI_SURFACE *mris, int vno, int which) {
  ...
  if (vno == Gdiag_no) {
    ...
    for (n = 0; n < vt->vtotal; n++) {
      ...
      vnp = (VERTEX_PARMS *)vn->vp;
      fprintf(fp, "%d %f\n", vt->v[n],
              which == WHITE_VERTICES ? vnp->white_dist : vnp->pial_dist);
    }
    fclose(fp);
  }
  if (num <= 1)
...
static int vp_copy_to_surface(MRI_SURFACE *mris, int which_src, int which_dst) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case TARGET_VERTICES:
      ...
      break;
    case ORIG_VERTICES:
      cheapAssert(!"vp_copy_to_surface should not use ORIG_VERTICES");
      break;
    default:
...
static int vp_copy_from_surface(MRI_SURFACE *mris, int which_src,
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case TARGET_VERTICES:
      ...
      break;
    case ORIG_VERTICES:
      cheapAssert(!"vp_copy_from_surface should not use ORIG_VERTICES");
      break;
    default:
...
static int vp_copy_to_surface_vals(MRI_SURFACE *mris, int which, DP *dp) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case LAYERIV_VERTICES:
      val = vp->l4_val;
      break;
    case WM_INTENSITY_OFFSET:
      val = vp->wm_intensity_offset;
      break;
    case SG_INTENSITY_OFFSET:
      val = vp->sg_intensity_offset;
      break;
    case RMS:
      val = vp->min_rms;
      break;
    case IG_INTENSITY_OFFSET:
      val = vp->ig_intensity_offset;
      break;
    case IG_INTENSITY:
      val = vp->ig_intensity_offset + dp->infra_granular_val;
      break;
    case SG_INTENSITY:
      val = vp->sg_intensity_offset + dp->supra_granular_val;
      break;
    case WM_INTENSITY:
      val = vp->wm_intensity_offset + dp->wm_val;
      break;
    case IG_WM_RATIO:
      val = (vp->ig_intensity_offset + dp->infra_granular_val) /
            (vp->wm_intensity_offset + dp->wm_val);
      break;
    case SG_WM_RATIO:
      val = (vp->sg_intensity_offset + dp->supra_granular_val) /
            (vp->wm_intensity_offset + dp->wm_val);
    case SG_IG_RATIO:
      val = (vp->sg_intensity_offset + dp->supra_granular_val) /
            (vp->ig_intensity_offset + dp->infra_granular_val);
      break;
    case DEEP_RATIO:
      val = vp->deep_ratio;
      break;
...
static int vp_copy_from_surface_vals(MRI_SURFACE *mris, int which, DP *dp) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case LAYERIV_VERTICES:
      ...
      break;
    case WM_INTENSITY_OFFSET:
      vp->wm_intensity_offset = v->val;
      break;
    case SG_INTENSITY_OFFSET:
      vp->sg_intensity_offset = v->val;
      break;
    case RMS:
      vp->min_rms = v->val;
      break;
    case IG_INTENSITY_OFFSET:
      vp->ig_intensity_offset = v->val;
      break;
    case IG_INTENSITY:
      vp->ig_intensity_offset = v->val - dp->infra_granular_val;
      break;
    case SG_INTENSITY:
      vp->sg_intensity_offset = v->val - dp->supra_granular_val;
      break;
    case WM_INTENSITY:
      vp->wm_intensity_offset = v->val - dp->wm_val;
      break;
    case DEEP_RATIO:
      vp->deep_ratio = v->val;
    default:
    ...
    case SG_IG_RATIO:
      ErrorExit(ERROR_UNSUPPORTED,
                "vp_copy_from_surface_vals: unsupported which %d", which);
      break;
    }
  }
  return (NO_ERROR);
...
static int vp_set(MRI_SURFACE *mris, int which, double val) {
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    case LAYERIV_VERTICES:
      ...
      break;
    case WM_INTENSITY_OFFSET:
      vp->wm_intensity_offset = val;
      break;
    case SG_INTENSITY_OFFSET:
      vp->sg_intensity_offset = val;
      break;
    case RMS:
      vp->min_rms = val;
      break;
    case IG_INTENSITY_OFFSET:
      vp->ig_intensity_offset = val;
      break;
    }
  }
  return (NO_ERROR);
...
static int compute_best_neighborhood_profile(MRI_SURFACE *mris, MRI *mri,
                                             int vno, DP *dp) {
  VP *vp;
  int best_profile, wm_len, ig_len, sg_len, i, profile_len, n, stria_len, num,
      tsteps;
  double kernel[MAX_PROFILE_LEN], nx, ny, nz, wm_intensity_offset,
      sg_intensity_offset, in_dist, wm_val, intensity_profile[MAX_PROFILE_LEN],
      norm, ig_dist, sg_dist, v1_rms_total, generic_rms_total, generic_rms,
      v1_rms;
  ...
  for (num = 0, v1_rms_total = generic_rms_total = 0.0, n = -1; n < 0; n++) {
    ...
    ig_dist = sqrt(SQR(vp->l4x - vp->wx) + SQR(vp->l4y - vp->wy) +
                   SQR(vp->l4z - vp->wz));
    sg_dist = sqrt(SQR(vp->px - vp->l4x) + SQR(vp->py - vp->l4y) +
                   SQR(vp->pz - vp->l4z));
    nx = vp->nx;
    ny = vp->ny;
    nz = vp->nz;

    in_dist = find_max_distance(
        mris, mri, vp->wx, vp->wy, vp->wz, -nx, -ny, -nz,
        dp->contrast_type == T1 ? 1 : -1, 2, dp->sigma, dp->step, &wm_val,
        dp->wm_val - dp->max_wm_intensity_offset,
    ...
    if (ig_len + sg_len <= stria_len)
      continue;
    extract_image_intensities(mris, mri, in_dist, ig_dist + sg_dist, vp->wx,
                              vp->wy, vp->wz, nx, ny, nz, dp->step, tsteps,
                              intensity_profile, fname2);
    if (wm_len > 0) {
    ...
    for (i = wm_len, sg_intensity_offset = 0.0;
         i < wm_len + (ig_len - stria_len); i++)
      sg_intensity_offset += (intensity_profile[i] -
                              (dp->infra_granular_val + wm_intensity_offset));
    for (; i < wm_len + ig_len + sg_len; i++)
      sg_intensity_offset += (intensity_profile[i] -
                              (dp->supra_granular_val + wm_intensity_offset));
    if (stria_len < ig_len)
      sg_intensity_offset /= (ig_len + sg_len - stria_len);
    else
      sg_intensity_offset /= (sg_len);

    generic_rms = find_min_rms(
        kernel, intensity_profile, profile_len, fname, dp, in_dist,
        wm_intensity_offset, sg_intensity_offset - dp->max_sg_intensity_offset,
        sg_intensity_offset + dp->max_sg_intensity_offset, 1, PROFILE_GENERIC,
        wm_len, ig_len, sg_len, profile_error_func);
    v1_rms = find_min_rms(
        kernel, intensity_profile, profile_len, fname, dp, in_dist,
        wm_intensity_offset, sg_intensity_offset - dp->max_sg_intensity_offset,
        sg_intensity_offset + dp->max_sg_intensity_offset, 1, PROFILE_V1,
        wm_len, ig_len, sg_len, profile_error_func);
    if (vno == Gdiag_no) {
      DiagBreak();
...
static double find_min_rms(
    double *kernel, double *intensity_profile, int profile_len, char *fname,
    DP *dp, double in_dist, double wm_intensity_offset,
    double min_sg_intensity_offset, double max_sg_offset,
    double sg_intensity_offset_step, int which_profile, int wm_len, int ig_len,
    int sg_len,
    double (*profile_error_func)(double *kernel, double *intensity_profile,
                                 int nsamples, char *fname, double step,
                                 double in_dist, double *errors)) {
  double min_rms, sg_intensity_offset,
      best_sg_intensity_offset = 0, rms, ig_intensity_offset,
      best_ig_intensity_offset = 0.0, stria_intensity_offset,
      best_stria_intensity_offset = 0.0;
  ...
  for (stria_intensity_offset = -dp->max_stria_intensity_offset;
    ...
    for (ig_intensity_offset = -dp->max_ig_intensity_offset;
      ...
      for (sg_intensity_offset = min_sg_intensity_offset;
        ...
        construct_model_profile(kernel, dp, wm_intensity_offset,
                                ig_intensity_offset, sg_intensity_offset,
                                stria_intensity_offset, wm_len, ig_len, sg_len,
                                0, which_profile);
        ;
#if 1
        rms = (*profile_error_func)(kernel, intensity_profile, profile_len,
                                    fname, dp->step, in_dist, nullptr);
        rms /= profile_len;
#else
        for (rms = 0.0, i = wm_len + ig_len - stria_len; i < wm_len + ig_len;
             i++)
  ...
  construct_model_profile(kernel, dp, wm_intensity_offset,
                          best_ig_intensity_offset, best_sg_intensity_offset,
                          best_stria_intensity_offset, wm_len, ig_len, sg_len,
                          0, which_profile);
  ;
  for (rms = 0.0, i = wm_len + ig_len - (stria_len + 1); i <= wm_len + ig_len;
       i++)
    rms += fabs(intensity_profile[i] - kernel[i]);
  rms /= (stria_len + 2);
  if (fname) {
    rms = (*profile_error_func)(kernel, intensity_profile, profile_len, fname,
                                dp->step, in_dist, nullptr);
    DiagBreak();
  }
  return (min_rms);
}
...
static int compute_laminar_ratios(MRI_SURFACE *mris, MRI *mri, DP *dp) {
  ...
  VERTEX *v;
  VERTEX_PARMS *vp;
  double intensity_profile[MAX_PROFILE_LEN], pdist, bottom, top, peak_bottom,
      valley_top;
  char *fname;
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    pdist = sqrt(SQR(vp->px - vp->wx) + SQR(vp->py - vp->wy) +
                 SQR(vp->pz - vp->wz));
    if (extract_image_intensities(
            mris, mri, WM_DIST, pdist, vp->wx, vp->wy, vp->wz, vp->nx, vp->ny,
            vp->nz, dp->step, tsteps, intensity_profile, fname) != NO_ERROR) {
      vp->found = 0;
      vp->deep_ratio = 0;
      continue;
    }
    ...
    for (num_top = 0, top = 0.0, index = stria_index; index <= ig_index;
         index++, num_top++) {
...
static int compute_normals(MRI_SURFACE *mris, VERTEX_PARMS *vp) {
  ...
  if (cno++ > 0)
    ...
    thick_parms.dt = 0.01;
  thick_parms.momentum = 0;
  thick_parms.l_nlarea = 0;
  ...
  if (mris_ico == nullptr) {
    ...
    if (!mris_ico)
      ErrorExit(ERROR_NOFILE, "%s: could not open surface file %s", Progname,
                fname);
