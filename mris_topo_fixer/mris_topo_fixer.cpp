...
static void freeTopoFixerParameters() {
  ...
  if (parms.mri)
    MRIfree(&parms.mri);
  if (parms.mri_wm)
    MRIfree(&parms.mri_wm);
  if (parms.mri_gray_white)
    MRIfree(&parms.mri_gray_white);
  if (parms.mri_k1_k2)
    MRIfree(&parms.mri_k1_k2);
  // histo
  if (parms.h_k1)
    HISTOfree(&parms.h_k1);
  if (parms.h_k2)
    HISTOfree(&parms.h_k2);
  if (parms.h_gray)
    HISTOfree(&parms.h_gray);
  if (parms.h_white)
    HISTOfree(&parms.h_white);
  if (parms.h_dot)
    HISTOfree(&parms.h_dot);
  if (parms.h_border)
    HISTOfree(&parms.h_border);
  if (parms.h_grad)
    HISTOfree(&parms.h_grad);
  if (parms.transformation_matrix)
    MatrixFree(&parms.transformation_matrix);
  // defect_list
...
int main(int argc, char *argv[]) {
  ...
  if (!mris)
    ErrorExit(ERROR_NOFILE, "%s: could not read input surface %s", Progname,
              fname);
  ...
  fprintf(stderr,
          "Before topology correction, eno=%d (nv=%d, nf=%d, ne=%d,"
          " g=%d)\n",
  ...
  for (int i = 0; i < dl->ndefects; i++) {
    ...
    if (false) { // parms.no_self_intersections){
    ...
    if (is_valid == 0) {
      ...
      fprintf(stderr, "The original surface is not a valid tessellated "
                      "manifold anymore!!!\\n");
      fprintf(stderr, "\nAbort !!!\n");
  ...
  fprintf(stderr,
          "after topology correction, eno=%d (nv=%d, nf=%d, ne=%d,"
          " g=%d)\n",
          eno, nvert, nfaces, nedges, (2 - eno) / 2);
