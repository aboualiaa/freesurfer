...
int Surface::_InitSurfaceConnectivity() {
  Vertex *v;
...
bool Surface::_InitFaceConnectivity() {
  bool is_closed = true;
...
int Surface::_InitFaceCoordinates() {
...
bool Surface::LoopValid(Loop &loop) {
  ...
  for (int n = 0; n < loop.npoints; n++) {
    int nf = 0;
    for (int i = 0; i < 3; i++) {
      if (faces[loop.points[n]].f[i] == -1)
        continue;
      if (faces[faces[loop.points[n]].f[i]].marked == 1)
        nf++;
    }
    if (nf != 2)
      return false;
  ...
  while (pos1 < loop.npoints || pos2 < pdisk->ring.npoints) {
    ...
    if (pos1 == loop.npoints) { // add the triangle pos1,pos2,pos2+1
      ...
      pos2++;
               ...
               pdisk->ring.npoints) { // add the triangle pos1,pos1+1,pos2
      v1 = loop[pos1];
      v2 = pdisk->ring[0];
      v0 = loop[(pos1 + 1) % loop.npoints];
      pos1++;
    } else { // determine which triangle to add
      if (fabs(pdisk->ring.npoints * pos1 - loop.npoints * (pos2 + 1.)) <
          fabs(pdisk->ring.npoints * (pos1 + 1.) - loop.npoints * pos2)) {
        v0 = loop[pos1];
        v1 = pdisk->ring[pos2];
        v2 = pdisk->ring[(pos2 + 1) % pdisk->ring.npoints];
        pos2++;
...
double Surface::_FaceDistance(int fdst, int fsrc) {
  return __norm(faces[fdst].x - faces[fsrc].x, faces[fdst].y - faces[fsrc].y,
                faces[fdst].z - faces[fsrc].z);
}
...
void Surface::CutLoop(Loop &loop, int very_small_patch) {
  ...
  if (very_small_patch == 1) {
    if (loop.npoints < 40)
      wd = 0; // 6
    else if (loop.npoints < 80)
      wd = 1; // 10
    else if (loop.npoints < 120)
      wd = 2; // 14
    else
      wd = 3;
  } else {
    if (loop.npoints < 10)
      wd = 0; // 6
    else if (loop.npoints < 14)
      wd = 1; // 10
    else if (loop.npoints < 20)
      wd = 2; // 14
    else
      wd = 3;
  }
  ...
  _OverAlloc(2 * loop.npoints + 2 * pdisk->disk.nvertices,
             2 * (2 * loop.npoints + pdisk->disk.nfaces + pdisk->ring.npoints));
...
int Surface::CutPatch(int seed, int maxinitface, int nattempts,
  ...
  if (maxinitface < 0 || maxinitface > nfaces)
    maxinitface = nfaces; // the max value of the seed face
  ...
  if (IsSurfaceValid() == false || euler != init_euler + 2)
    return -2; // failure
