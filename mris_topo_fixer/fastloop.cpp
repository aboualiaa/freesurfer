...
double FastLoop::_Distance(int fdst, int fsrc) {
  return __norm(facedata[fdst].x - facedata[fsrc].x,
                facedata[fdst].y - facedata[fsrc].y,
                facedata[fdst].z - facedata[fsrc].z);
}
...
int FastLoop::_Run(int &stopping_face) {
  while (!FM_trial_heap->empty()) {
    ...
    // we remove the face at the top of the stack and add it to the list of
    // alive faces
...
int FastLoop::_FindSeedFaces(int conflicting_face, int &init_fn1,
                             int &init_fn2) {
  ...
  for (int i = 0; i < 3; i++) {
    if (facedata[face->f[i]].fmState == eAlive)
      continue;
    if (facedata[face->f[i]].fmState == eForbidden)
      continue; // the border face
...
int FastLoop::_ExtractFirstLoop(Loop &loop, int init_fn1, int init_fn2) {
  ...
  loop.Alloc(number_of_faces); // we have at max number_of_faces faces (at least
                               // for now!)
...
int FastLoop::_FindNextFace(int next_fn, int vno) {
  ...
  if (next_fn == surface->nfaces)
    return -2;

  for (int i = 0; i < 3; i++) {
    int fn = facedata[next_fn].f[i];
    //  if(facedata[fn].fccLabel) continue;
    if (facedata[fn].fccLabel == 3)
      return -1;
    if (facedata[fn].fccLabel)
      continue;
    for (int j = 0; j < 3; j++) {
      if (facedata[fn].v[j] == vno)
        return fn;
    }
  }
...
int FastLoop::_SimplifyLoop(Loop &loop) {
  ...
  while (found) {
    ...
    int next_face = -1;
    for (int i = 0; i < 3; i++) {
      if (facedata[facedata[current_face].f[i]].fccState != eTemporary)
        continue; // not a loop face
      if (facedata[facedata[current_face].f[i]].fmFather ==
          0) { // not yet processed
    ...
    if (next_face == -1)
      break; // no more faces to process!

    // we have found another face
    found = true;

    if (facedata[next_face].fmState ==
        0) { // this face is non-triple -> continue
      current_face = next_face;
      continue;
    }

    // we have found a triple face - we need to carefully analyze the remaining
    // faces
    ...
    int fn1 = -1, fn2 = -1;
    for (int i = 0; i < 3; i++) {
      if (facedata[facedata[current_face].f[i]].fmFather == 1)
    ...
    if (facedata[fn2].fmState == 0) { // only fn1 is a triple face!
      ...
      while (flist.npoints) {
        ...
        flist.Pop();
        for (int i = 0; i < 3; i++) {
          if (facedata[facedata[fn].f[i]].fccState != eTemporary)
            continue;                                      // not a loop face
    ...
    // In this worst case situation, we have to decide which face (of fn1 and
    // fn2) is the right one to do so we find the connected components of the
    // fccState==eTemporary && fmFather==0 we arbitrarily start with the face
    // fn1 we need to find the two neighbors of fn1 (since fn1 is a triple face)
    int fn11 = -1, fn12 = -1;
    for (int i = 0; i < 3; i++) {
      if (facedata[facedata[fn1].f[i]].fmFather)
        continue; // already processed
      if (fn11 == -1)
    ...
    while (!found && flist.npoints) {
      ...
      for (int i = 0; i < 3; i++) {
        int nfn = facedata[fn].f[i];
        if (facedata[nfn].fccState != eTemporary)
          continue; // not part of the loop
    ...
    if (ending_face == -1) {              // two cases to consider
      if (facedata[fn12].fmFather == 0) { // fn1 and fn12 are the right faces
        ...
        int fn22 = -1;
        for (int i = 0; i < 3; i++) {
          if (facedata[facedata[fn2].f[i]].fmFather ==
              0) { // the only remaining face
...
void FastLoop::FindMinimalLoop(Loop &minimal_loop, int max_init_face,
                               int nattempts) {

  double minimal_length = _GetLoopLength(minimal_loop);

  if (max_init_face == -1)
    max_init_face = surface->nfaces;
  if (nattempts < 1)
    nattempts = 10;
