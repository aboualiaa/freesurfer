  ...
  for (int n = 0; n < nloops; n++) {
    if (parms.verbose >= VERBOSE_MODE_MEDIUM) {
      fprintf(WHICH_OUTPUT,
              ...
              parms.max_face, mris->nfaces, n + 1, nloops, parms.nattempts,
              parms.nminimal_attempts);
    }
...
bool MRISincreaseEuler(MRIS *&mris, TOPOFIX_PARMS &parms) {
  ...
  for (int n = 0; n < nattempts; n++) {
    if (parms.verbose >= VERBOSE_MODE_HIGH)
      fprintf(WHICH_OUTPUT, "\r      %3.2f%% [%5d,%5d]", n * 100.0 / nattempts,
              n, nattempts);
    ...
    fprintf(WHICH_OUTPUT, "BEFORE %d (%d: %d, %d)\n", euler, n,
            surface->nvertices, surface->nfaces);
#endif
    ...
    if (n == 0) // the first time, try to cut with a very small patch
      correct =
          surface->CutPatch(-2, parms.max_face, parms.nminimal_attempts, 1);
    else
      correct = surface->CutPatch(-1, parms.max_face, 10,
                                  1); // always trying 10 times at least
    ...
    if (selfintersect) { // the previous surface self-intersected! -> generate
                         // another one
      nintersections++;
      if (parms.verbose >= VERBOSE_MODE_HIGH)
      ...
      if (n == 0) {
        correct =
            surface->CutPatch(-2, parms.max_face, parms.nminimal_attempts);
      } else {
    ...
    if (selfintersect) {
      ...
      if (parms.verbose >= VERBOSE_MODE_HIGH)
        fprintf(WHICH_OUTPUT, "\r      SELF-INTERSECTING PATCH\n");
      if (parms.minimal_mode && best_mris == nullptr) {
...
bool doesMRISselfIntersect(MRIS *mris_work, TOPOFIX_PARMS &parms) {
  if (parms.no_self_intersections == false)
    return false;
...
int MRISgetEulerNumber(const MRIS *mris, const int *list_of_faces, int nfs) {
  ...
  for (int n = 0; n < nfs; n++)
    for (int i = 0; i < 3; i++) {
      VERTEX_TOPOLOGY *const vt =
          &mris->vertices_topology[mris->faces[list_of_faces[n]].v[i]];
...
void MRIPfree(MRIP **mrip) {
  ...
  *mrip      = nullptr;
  ...
  if (patch->vtrans_to)
    free(patch->vtrans_to);
  if (patch->ftrans_to)
    free(patch->ftrans_to);
  if (patch->vtrans_from)
    free(patch->vtrans_from);
  if (patch->ftrans_from)
    free(patch->ftrans_from);
...
static MRIS *SurfaceToMRISwkr_new(Surface *surface) {
  MRIS *mris = MRISoverAlloc(surface->maxvertices, surface->maxfaces,
                             surface->nvertices, surface->nfaces);
...
MRIS *SurfaceToMRIS(Surface *surface) {
  //
  // The possible difference in the order of faces around the vertices may cause
  // slight differences hence the old code is kept for comparison purposes
  //
  MRIS *mris =
      false ? SurfaceToMRISwkr_old(surface) : SurfaceToMRISwkr_new(surface);
...
bool MRISaddMRIP(MRIS *mris_dst, MRIP *mrip) {
  ...
  for (int n = 0; n < mris->nvertices; n++) {
    ...
    if (n >= nvertices)
      vto[n] = currNumVertices++;
    int const vno_dst = vto[n];

    MRISsetXYZ(mris_dst, vno_dst, vsrc->x, vsrc->y, vsrc->z);

    MRISsetOriginalXYZ(mris_dst, vno_dst, vsrc->x, vsrc->y, vsrc->z);
  }
