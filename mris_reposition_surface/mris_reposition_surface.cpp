...
int main(int argc, char **argv) {
  ...
  // help text
  parser.addHelp(mris_reposition_surface_help_xml,
                 mris_reposition_surface_help_xml_len);
  ...
  if (!mri)
    fs::fatal() << "could not read volume";
  ...
  if (js_pts.size() < 1)
    fs::fatal() << "could not read control points";
  ...
  if (parser.exists("size")) {
    ...
    if (nsize < 1)
      fs::fatal() << "size must be greater than 0";
  }
  ...
  if (parser.exists("sigma")) {
    ...
    if (sigma <= 0)
      fs::fatal() << "sigma must be greater than 0";
  }
  ...
  for (int i = 0; i < niterations; i++) {
    ...
    int    max_vno;
    float  distance;
    MRIScomputeVertexSpacingStats(surf, NULL, NULL, &max_spacing, NULL,
                                  &max_vno, CURRENT_VERTICES);
    MRIS_HASH_TABLE *hash =
        MHTcreateVertexTable_Resolution(surf, CURRENT_VERTICES, max_spacing);
    ...
    for (auto it = points.begin(); it != points.end(); it++) {
      ...
      MRISrepositionSurfaceToCoordinate(surf, mri, vno, p.x, p.y, p.z, nsize,
                                        sigma, 0);
    }
