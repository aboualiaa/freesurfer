...
static PyMethodDef sblcMethods[] = {
    ...
    {"checkCommandEvents", sblc_checkCommandEvents, METH_NOARGS,
     "Returns non-zero if user has cancelled command."},
    {"dataPath", sblc_dataPath, METH_NOARGS,
     "Returns the main config's dataPath."},
    {NULL, NULL, 0, NULL}};

// define our python module
struct PyModuleDef sblcModuleDef = {PyModuleDef_HEAD_INIT,
                                    "sblc",
                                    NULL,
                                    -1,
                                    sblcMethods,
...
PyMODINIT_FUNC sblcInitModule() { return PyModule_Create(&sblcModuleDef); }
...
void cleanUpPython() { Py_Finalize(); }
