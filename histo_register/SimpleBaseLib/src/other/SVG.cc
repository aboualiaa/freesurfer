...
void SVG::startGroup(int r, int g, int b, int rFill, int gFill, int bFill,
                     float lineWidth) {
  if (m_file) {
    m_file->writeF("  </g>\n");
    m_file->writeF("  <g style=\"fill-opacity:1.0; stroke:#%02x%02x%02x; "
                   "fill:#%02x%02x%02x; stroke-width:%4.2f;\">\n",
...
void SVG::addLine(float x1, float y1, float x2, float y2) {
  ...
  y2 *= m_scale;
  addObjectStr(
      sprintF("    <line x1='%5.3f' y1='%5.3f' x2='%5.3f' y2='%5.3f'/>\n", x1,
              y1, x2, y2));
}
...
void SVG::addCircle(float x, float y, float radius, const String &tooltip) {
  ...
  addObjectStr(sprintF("    <circle cx='%5.3f' cy='%5.3f' r='%5.3f' %s/>\n", x,
                       y, radius, tooltipText.c_str()));
}

/// add text to the current group
void SVG::addText(float x, float y, const String &text, TextAnchor anchor,
                  float size, const String &font, float rotate, float xRotate,
                  float yRotate, float baselineShift) {
  x *= m_scale;
  y *= m_scale;
  xRotate *= m_scale;
  yRotate *= m_scale;
  ...
  if (rotate)
    extraText += sprintF(" transform='rotate(%5.3f,%5.3f,%5.3f)'", rotate,
                         xRotate, yRotate);
  if (baselineShift)
    extraText +=
        sprintF(" dominant-baseline='middle'", baselineShift * m_scale);
  if (font.length())
    extraText += sprintF(" font-family='%s'", font.c_str());
  addObjectStr(
      sprintF("    <text x='%5.3f' y='%5.3f' font-size='%5.3f'%s>%s</text>\n",
...
void SVG::addArc(float x, float y, float radius, float startDegrees,
                 float endDegrees) {
  x *= m_scale;
  y *= m_scale;
  radius *= m_scale;
  ...
  addObjectStr(
      sprintF("    <path d='M%5.3f,%5.3f A%5.3f,%5.3f 0 0,1 %5.3f,%5.3f'/>",
              xStart, yStart, radius, radius, xEnd, yEnd));
}
