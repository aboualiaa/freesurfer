...
// basic constructor
PlotItem::PlotItem(const VectorD &x, const VectorD &y,
                   const Array<String> *labels, const String &caption, int r,
                   int g, int b, PlotStyle plotStyle) {
  m_x         = x;
  m_y         = y;
  m_r         = r;
  m_g         = g;
...
PlotItem::PlotItem(TaggedFile &file) {
  ...
  do {
    ...
    switch (tag) {
    case TAG_X:
      m_x = file.readVector<double>();
      break;
    case TAG_Y:
      m_y = file.readVector<double>();
      break;
    case TAG_LABELS:
      file.readStrings(m_labels);
      break;
    case TAG_CAPTION:
      m_caption = file.readString();
      break;
    case TAG_R:
      m_r = file.readInt();
      break;
    ...
    case TAG_B:
      ...
      break;
    case TAG_PLOT_STYLE:
      m_plotStyle = (PlotStyle)file.readInt();
      break;
    default:
      file.skipTag(tag);
    }
...
/// add data to plot
void Plot::add(const VectorD &x, const VectorD &y, const Array<String> *labels,
               const String &caption, bool updateBounds) {
  PlotItem *plotItem =
      new PlotItem(x, y, labels, caption, m_r, m_g, m_b, m_plotStyle);
...
/// add data to plot
void Plot::add(const VectorD &y, const Array<String> *labels,
               const String &caption, bool updateBounds) {
  int     len = y.length();
  VectorD x(len);
  for (int i = 0; i < len; i++)
...
void Plot::toImageCoord(double x, double y, float &xImg, float &yImg, int width,
                        int height) const {
  xImg =
      (float)((width - 7.0 - m_yLabelWidth) * (x - m_xMin) / (m_xMax - m_xMin) +
              m_yLabelWidth + 1.0);
  yImg = (float)((height - 8.0 - m_xLabelHeight - m_titleHeight) *
                     (1.0 - (y - m_yMin) / (m_yMax - m_yMin)) +
                 m_titleHeight + 6.0);
}
...
void Plot::save(const String &fileName, int width, int height) {
  ...
  svg.addRectangle(m_yLabelWidth, 5.0f + m_titleHeight, width - 5.0f,
                   height - 1.0f - m_xLabelHeight);
  ...
  for (int i = 0; i < m_plotItems.count(); i++) {
    ...
    svg.startGroup(plotItem.r(), plotItem.g(), plotItem.b(), rFill, gFill,
                   bFill, 2.0f);
  ...
  // add axis labels
  if (m_yLabel.length())
    svg.addText(yLabelX, (yLabelStart + yLabelEnd) * 0.5f, m_yLabel,
                ANCHOR_END);
  if (m_xLabel.length())
    svg.addText((xLabelStart + xLabelEnd) * 0.5f, xLabelY, m_xLabel,
                ANCHOR_MIDDLE);
  if (m_title.length())
    svg.addText((xLabelStart + xLabelEnd) * 0.5f, m_titleHeight - 2, m_title,
                ANCHOR_MIDDLE);
}
...
void Plot::loadData(const String &fileName) {
  ...
  if (file.openSuccess()) {
    ...
    do {
      ...
      switch (tag) {
      case TAG_TITLE:
        m_title = file.readString();
        break;
      case TAG_X_LABEL:
        m_xLabel = file.readString();
        break;
      case TAG_Y_LABEL:
        m_yLabel = file.readString();
        break;
      case TAG_X_MIN_LABEL:
        m_xMinLabel = file.readString();
        break;
      case TAG_X_MAX_LABEL:
        m_xMaxLabel = file.readString();
        break;
      case TAG_Y_MIN_LABEL:
        m_yMinLabel = file.readString();
        break;
      case TAG_Y_MAX_LABEL:
        m_yMaxLabel = file.readString();
        break;
      case TAG_X_MIN:
        m_xMin = file.readFloat();
        break;
      case TAG_X_MAX:
        m_xMax = file.readFloat();
        break;
      case TAG_Y_MIN:
        m_yMin = file.readFloat();
        break;
      case TAG_Y_MAX:
        m_yMax = file.readFloat();
        break;
      case TAG_BOUND_SET:
        m_boundSet = file.readBool();
        break;
      case TAG_TIME_AXIS:
        m_timeAxis = file.readBool();
        break;
      case TAG_ZERO_LINES:
        m_zeroLines = file.readBool();
        break;
      case TAG_ITEMS:
        file.readArray(m_plotItems);
        break;
      default:
        file.skipTag(tag);
      }
...
aptr<Plot> scatterPlot(const VectorD &v1, const VectorD &v2,
