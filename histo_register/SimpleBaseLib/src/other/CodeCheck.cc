...
Array<String> codeFileList(const String &path, const Array<String> &suffixes,
                           bool excludeUnderscore) {
  ...
  for (int i = 0; i < dirList.count(); i++) {
    ...
    if (dirItem.contains('.') == false &&
        (excludeUnderscore == false || dirItem.startsWith("_") == false)) {
      Array<String> subFileNames =
          codeFileList(path + "/" + dirItem, suffixes, excludeUnderscore);
      for (int j = 0; j < subFileNames.count();
           j++) // fix(clean): replace with append array function
...
void checkFile(const String &fileName, Table &table) {
  ...
  if (file.openSuccess()) {
    while (file.endOfFile() == false) {
      ...
      if (comment) {
        commentCount++;
      } else if (lastBlank == true && lineStrip != "private:" &&
                 lineStrip != "protected:") {
        missingCommentCount++;
      }
      ...
      if (comment) {
        if (lineStrip.contains("// fix")) {
          if (lineStrip.contains("fix(cleaner)") ||
              lineStrip.contains("fix(clean)"))
            fixCleanerCount++;
      ...
      if (header == false && line.get(0) > ' ' &&
          line.startsWith("namespace") == false) {
      ...
      if (lineStrip.startsWith("#include")) {
        ...
        if (includeFile.startsWith("<sbl") == false &&
            includeFile.startsWith("<pvl") == false) {
          externalIncludeCount++;
        }
      }

      // check statement formatting
      if (lineStrip.startsWith("if") || lineStrip.startsWith("for") ||
          lineStrip.startsWith("while")) {
...
void distributeCode(Config &conf) {
  ...
  // loop over all source files
  Array<String> fileNames =
      codeFileList(sourcePath, suffixes, excludeUnderscore);
  for (int fileIndex = 0; fileIndex < fileNames.count(); fileIndex++) {
    ...
    if (inFileName.contains("external") == false) {
      File licenseFile(sourcePath + "../../license-short.txt", FILE_READ,
                       FILE_TEXT); // fix(later): path
      while (licenseFile.endOfFile() == false)
        outFile.writeF("%s\x0A",
                       licenseFile.readLine()
                           .c_str()); // write linefeed only at end of line
    }
    ...
    // display diagnostic
    disp(1, "inFileName: %s, lineCount: %d, tabCount: %d", inFileName.c_str(),
         lineCount, tabCount);
  }
