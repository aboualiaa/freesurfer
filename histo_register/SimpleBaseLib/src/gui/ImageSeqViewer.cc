...
// basic constructor
ImageSeqViewer::ImageSeqViewer(wxWindow *parent, wxWindowID id,
                               bool storeInstance)
    : wxPanel(parent, id) {
  if (storeInstance)
    s_instance = this;
  ...
  Connect(1, wxEVT_COMMAND_SLIDER_UPDATED,
          wxCommandEventHandler(ImageSeqViewer::onSlider));
...
void testImageClickCallback(int imageIndex, double x, double y,
                            int keyModifier) {
  int          xInt  = round(x);
  int          yInt  = round(y);
