...
// basic constructor
ConfigEntryEditor::ConfigEntryEditor(wxWindow *parent, wxWindowID id,
                                     ConfigEntry &configEntry)
    : wxPanel(parent, id), m_configEntry(configEntry) {
  m_checkBox = NULL;
  m_text     = NULL;
  ...
  if (configEntry.type == CONFIG_ENTRY_SECTION) {
    wxFont boldFont(10, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL,
                    wxFONTWEIGHT_BOLD);
  ...
  if (configEntry.type == CONFIG_ENTRY_PATH ||
    ...
    button->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                    wxCommandEventHandler(ConfigEntryEditor::onBrowseButton),
                    NULL, this);
...
void ConfigEntryEditor::onBrowseButton(wxCommandEvent &event) {
  if (m_configEntry.type == CONFIG_ENTRY_PATH) {
    wxDirDialog *dirDialog =
        new wxDirDialog(this, "Select a path", m_configEntry.value.c_str());
...
// basic constructor
ConfigEditor::ConfigEditor(wxWindow *parent, wxWindowID id, Config &config)
    : wxPanel(parent, id), m_config(config) {
...
// basic constructor
ConfigEditorDialog::ConfigEditorDialog(wxWindow *parent, wxWindowID id,
                                       Config &config, const String &title)
    : wxDialog(parent, id, wxString(title.c_str()), wxDefaultPosition,
  ...
  okButton->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                    wxCommandEventHandler(ConfigEditorDialog::onOkButton), NULL,
                    this);
  cancelButton->Connect(
      wxEVT_COMMAND_BUTTON_CLICKED,
      wxCommandEventHandler(ConfigEditorDialog::onCancelButton), NULL, this);
}
...
void ConfigEditorDialog::onCancelButton(wxCommandEvent &event) { EndModal(0); }
...
bool editConfig(Config &conf, const String &title) {
  ConfigEditorDialog *configEditorDialog =
      new ConfigEditorDialog(dialogParent(), -1, conf, title);
