...
// basic constructor
ConfigViewer::ConfigViewer(wxWindow *parent, wxWindowID id)
    : wxPanel(parent, id) {
...
void ConfigViewer::addEntry(const ConfigEntry &entry) {
  ...
  m_valueControls.append(
      valueText); // just adding a blank entry to keep numbering correct

  // set appearance based on whether section heading
  int topBorder = 0;
  if (entry.type == CONFIG_ENTRY_SECTION) {
    wxFont headingFont(10, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL,
                       wxFONTWEIGHT_BOLD);
...
// basic constructor
ConfigViewerDialog::ConfigViewerDialog(wxWindow *parent, wxWindowID id,
                                       Config &conf, const String &title)
    : wxDialog(parent, id, wxString(title.c_str())) {
...
void testConfigViewer(Config &conf) {
  ...
  // show the dialog
  ConfigViewerDialog *configViewerDialog = new ConfigViewerDialog(
      dialogParent(), -1, testConf, "Config Viewer Test");
