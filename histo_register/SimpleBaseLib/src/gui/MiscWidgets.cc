...
ProgressDialog::ProgressDialog(wxWindow *parent, wxWindowID id)
    : wxDialog(parent, id, wxString("Progress"), wxDefaultPosition,
               wxDefaultSize, wxCAPTION) {

  // populate dialog contents
  wxBoxSizer *topSizer = new wxBoxSizer(wxVERTICAL);
  m_progressLabel      = new wxStaticText(this, -1, "", wxDefaultPosition,
  ...
  cancelButton->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                        wxCommandEventHandler(ProgressDialog::onCancelButton),
                        NULL, this);
}
...
void ProgressDialog::dispProgress(int index, int count) {
  ...
  if (index + 1 < count || count <= 0) {
    ...
    if (s_progressDialog->IsVisible() == false) {
      ...
      s_progressDialog->MakeModal(
          true); // use this rather than ShowModal because want to keep running our code
      wxBeginBusyCursor();
    }
...
void ToolBar::addButton(const String &caption, const String &command,
                        const String &tip) {
  ...
  button->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                  wxCommandEventHandler(ToolBar::onButton), NULL, this);
  m_sizer->Add(button);
...
void setDialogParent(wxFrame *parent) { g_dialogParent = parent; }
