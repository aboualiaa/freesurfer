...
// basic constructor
GraphicViewer::GraphicViewer(wxWindow *parent, wxWindowID id)
    : wxPanel(parent, id) {
...
void GraphicViewer::onPaint(wxPaintEvent &event) { draw(); }
...
void GraphicViewer::onMove(wxMouseEvent &event) {
  if (event.LeftIsDown() && event.ShiftDown() == false &&
    ...
    if (m_xDragStart == -1 &&
        m_yDragStart == -1) { // fix(clean): use flag instead of special values
...
Point2 GraphicViewer::dataToWindowCoord(Point2 dataPt) {
  return Point2((dataPt.x - m_xCenter + m_viewWidth * 0.5) *
                    (double)windowWidth() / m_viewWidth,
                windowHeight() - (dataPt.y - m_yCenter + m_viewHeight * 0.5) *
                                     (double)windowHeight() / m_viewHeight);
}

/// convert window coordinate to data coordinate
Point2 GraphicViewer::windowToDataCoord(Point2 winPt) const {
  return Point2(m_xCenter - m_viewWidth * 0.5 +
                    m_viewWidth * winPt.x / (double)windowWidth(),
                m_yCenter + m_viewHeight * 0.5 -
                    m_viewHeight * winPt.y / (double)windowHeight());
}
