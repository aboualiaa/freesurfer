...
MainWindow::MainWindow(const wxString &title, const wxPoint &pos,
  ...
  m_commandInputText->Connect(wxEVT_KEY_DOWN,
                              wxKeyEventHandler(MainWindow::onCommandInputKey),
                              NULL, this);
}
...
void MainWindow::switchTo(int tabIndex) { m_notebook->SetSelection(tabIndex); }
...
void MainWindow::addTimerEvent(int msecInterval, void (*callback)()) {
  ...
  if (m_timer == NULL) {
    m_timer =
        new wxTimer(this, -1); // fix(clean): do we need to deallocate this?
    m_timer->Connect(wxEVT_TIMER, wxTimerEventHandler(MainWindow::onTimer),
                     NULL, this);
    m_timer->Start(100);
  }
...
void dispErrorMainWindow(const char *text) {
  if (MainWindow::instance()) {
    MainWindow::instance()->dispText(text);
    wxMessageBox(text, MainApp::instance()->GetAppName().c_str(),
                 wxOK | wxICON_EXCLAMATION, MainWindow::instance());
  }
}
...
void exitUserInterface(Config &conf) { MainWindow::instance()->Close(true); }
...
bool MainApp::OnInit() {
  ...
  MainWindow *mainWindow =
      new MainWindow(GetAppName().c_str(), wxPoint(50, 50), wxSize(900, 500));
