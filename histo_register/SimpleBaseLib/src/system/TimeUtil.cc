...
float fracOfDayUTC(int timestamp) {
  ...
  int        secondOfDay =
      (timeTuple->tm_hour * 60 + timeTuple->tm_min) * 60 + timeTuple->tm_sec;
...
int dateUTC(int year, int month, int day) {
  if (year < 1980 || year > 2037 || month < 1 || month > 12 || day < 1 ||
      day > 31) {
...
int datePartUTC(int timestamp) {
  time_t     tsLong    = timestamp;
  struct tm *timeTuple = gmtime(&tsLong);
  return dateUTC(timeTuple->tm_year + 1900, timeTuple->tm_mon + 1,
                 timeTuple->tm_mday);
}

/// creates timestamp from date and time
int dateTimeLocal(int year, int month, int day, int hour, int minute,
                  int second) {
  if (year < 1980 || year > 2037 || month < 1 || month > 12 || day < 1 ||
      day > 31) {
    warning("invalid date");
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 ||
      second > 59) {
    warning("invalid time (hour: %d, minute: %d, second: %d)", hour, minute,
            second);
    return 0;
  }
  struct tm timeTuple;
...
/// creates timestamp from date and time
int dateTimeUTC(int year, int month, int day, int hour, int minute,
                int second) {
  if (year < 1980 || year > 2037 || month < 1 || month > 12 || day < 1 ||
      day > 31) {
    warning("invalid date");
...
int parseTimestamp(const String &data, const String &format, bool local,
                   int &msec) {
  msec     = 0;
  int year = 0, month = 0, day = 0, hour = 0, minute = 0, second = 0;
  // fix(safety): check number of positions in format
  sscanf(data.c_str(), format.c_str(), &year, &month, &day, &hour, &minute,
         &second, &msec);
  if (local)
