...
bool readSerialMessage(SerialPort &serialPort, String &messageName,
                       VectorI &values) {
  ...
  if (length && message[length - 1] == PROTOCOL_TERMINATOR) {
    ...
    if (protocolVersion != PROTOCOL_VERSION) {
      warning("invalid protocol version: %d (expected: %d)", protocolVersion,
              PROTOCOL_VERSION);
    } else if (terminator != 255) {
      warning("invalid terminatore: %d", terminator);
    } else if (localCheckSum != remoteCheckSum) {
      warning("invalid check sum: %d (remote: %d)", localCheckSum,
              remoteCheckSum);
    } else if (payloadLength != length - 6) {
      warning("invalid payload length: %d (message length: %d)", payloadLength,
              length);
    } else {
...
void writeSerialMessage(SerialPort &serialPort, const String &messageName,
                        int valueCount, int value0, int value1, int value2) {
...
VectorI readSensorValues(SerialPort &serialPort) {
  ...
  if (values.length())
    return subset(values, 1,
                  values.length() -
                      1); // exclude first value, since it is the time diff
  return values;
}
...
void initMicrocontroller() { registerCommand("ssread", readSerialSensors); }
