...
void copyFile(const String &srcFileName, const String &destFileName) {
  ...
  runSystemCommand(sprintF("copy %s %s", srcFileNameClean.c_str(),
                           destFileNameClean.c_str()));
#else
  runSystemCommand(
      sprintF("cp -f %s %s", srcFileName.c_str(), destFileName.c_str()));
#endif
}
...
void moveFile(const String &srcFileName, const String &destFileName) {
  ...
  String srcFileNameClean  = srcFileName.replace('/', '\\');
  String destFileNameClean = destFileName.replace('/', '\\');
  runSystemCommand(sprintF("move %s %s", srcFileNameClean.c_str(),
                           destFileNameClean.c_str()));
#else
  runSystemCommand(
      sprintF("mv -f %s %s", srcFileName.c_str(), destFileName.c_str()));
#endif
}
