...
void motionFieldStats(const MotionField &mf, float &uMinRet, float &vMinRet,
                      float &uMeanRet, float &vMeanRet, float &uMaxRet,
                      float &vMaxRet) {
...
void dispMotionStats(int indent, const MotionField &mf) {
  ...
  disp(indent, "mf: min: %f, %f, mean: %f, %f, max: %f, %f", uMin, vMin, uMean,
       vMean, uMax, vMax);
  if (mf.occDefined()) {
...
float meanGradMag(const MotionField &mf) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      float vDxVal = vDx->data(x, y);
      float vDyVal = vDy->data(x, y);
      sum += (double)sqrtf(uDxVal * uDxVal + uDyVal * uDyVal + vDxVal * vDxVal +
                           vDyVal * vDyVal);
    }
  }
...
aptr<MotionField> computeDenseMotion(const VectorF &x, const VectorF &y,
  ...
  aptr<ImageGrayF> uMap =
      triangulationInterpolation(points, uCopy, mfWidth, mfHeight);
  aptr<ImageGrayF> vMap =
      triangulationInterpolation(points, vCopy, mfWidth, mfHeight);
