...
void drawRectFilled(ImageColorU &img, int xMin, int xMax, int yMin, int yMax,
                    int r, int g, int b) {
...
/// draws a filled rectangle on the image
void drawRectFilled(ImageGrayU &img, int xMin, int xMax, int yMin, int yMax,
                    int v) {
...
/// draws a filled rectangle on the image
void drawRectFilled(ImageGrayI &img, int xMin, int xMax, int yMin, int yMax,
                    int v) {
...
/// draws a filled rectangle on the image
void drawRectFilled(ImageGrayF &img, int xMin, int xMax, int yMin, int yMax,
                    float v) {
...
void drawRect(ImageColorU &img, int xMin, int xMax, int yMin, int yMax, int r,
              int g, int b) {
...
void drawLine(ImageGrayU &img, int xStart, int yStart, int xEnd, int yEnd,
              int v, bool thick) {
...
/// draws a line between the two points
// fix(faster): make faster
void drawLine(ImageColorU &img, int xStart, int yStart, int xEnd, int yEnd,
              int r, int g, int b, bool thick) {
  float x     = (float)xStart;
  float y     = (float)yStart;
  float xDiff = (float)(xEnd - xStart);
  float yDiff = (float)(yEnd - yStart);
...
void drawCross(ImageColorU &img, int x, int y, int radius, int r, int g, int b,
               bool thick) {
...
void drawArrow(ImageColorU &img, int xStart, int yStart, int xEnd, int yEnd,
               int r, int g, int b, bool thick) {
  drawLine(img, xStart, yStart, xEnd, yEnd, r, g, b, thick);
  int xDiff = xEnd - xStart;
  int yDiff = yEnd - yStart;
  drawLine(img, xStart + xDiff * 4 / 5 - yDiff / 5,
           yStart + yDiff * 4 / 5 + xDiff / 5, xEnd, yEnd, r, g, b, thick);
  drawLine(img, xStart + xDiff * 4 / 5 + yDiff / 5,
           yStart + yDiff * 4 / 5 - xDiff / 5, xEnd, yEnd, r, g, b, thick);
}

/// draws a filled circle in the image
void drawCircleFilled(ImageColorU &img, int x, int y, int radius, int r, int g,
                      int b) {
...
void drawText(ImageColorU &img, const String &text, int x, int y, int r, int g,
              int b, bool alignRight, int scale) {
  ...
  if (g_textImg.get()) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if ((asc > 32 || asc < 128) && x + xDest >= 0 &&
          x + xDest + g_letterWidth * scale <= width) {
...
void colorize(int v, int &r, int &g, int &b) {
  ...
  // color table
  const int count           = 4;
  const int color[count][3] = {
      {200, 200, 200}, {0, 0, 150}, {150, 150, 0}, {200, 0, 0}};
  ...
  r = ((highValue - v) * (color[i][0]) + (v - lowValue) * (color[i + 1][0])) /
      (highValue - lowValue);
  g = ((highValue - v) * (color[i][1]) + (v - lowValue) * (color[i + 1][1])) /
      (highValue - lowValue);
  b = ((highValue - v) * (color[i][2]) + (v - lowValue) * (color[i + 1][2])) /
      (highValue - lowValue);
}
