...
void InputVideo::open(const String &fileName) {
  if (fileName.contains('*')) {
    ...
    disp(1, "image list path: %s, prefix: %s, suffix: %s, count: %d",
         m_imagePath.c_str(), prefix.c_str(), suffix.c_str(),
         m_imageList.count());
  } else if (fileName.endsWith(".txt")) {
    m_imagePath = fileName.leftOfLast('/') +
                  "/"; // assume images are in same folder as the list
    m_imageList = loadStrings(fileName, false);
    disp(1, "image list file count: %d, path: %s", m_imageList.count(),
         m_imagePath.c_str());
  } else {
#ifdef USE_OPENCV
...
aptr<ImageColorU> InputVideo::frame(int frameIndex) {
  if (frameIndex < 0 || frameIndex >= length())
    fatalError("invalid input frame: %d, video length: %d", frameIndex,
               length());
  ...
  if (m_cvCapture) {
    ...
    if (iplImg) {
      ImageColorU imgMustCopy(
          iplImg,
          false); // need to copy this image, because it contains reference to iplImage which we do not own
...
#ifdef USE_OPENCV
      m_length =
          round(cvGetCaptureProperty(m_cvCapture, CV_CAP_PROP_FRAME_COUNT));
#endif
    } else if (m_imageList.count()) {
...
int InputVideo::width() {
  if (m_width == 0) {
    aptr<ImageColorU> img = frame(
        0); // don't want to use CV_CAP_PROP_FRAME_WIDTH/HEIGHT because may not return valid value if not already loaded a frame
    if (img.get()) {
      m_width  = img->width();
      m_height = img->height();
    }
...
OutputVideo::OutputVideo(const String &fileName, int outputWidth,
                         int outputHeight, double fps) {
  m_videoWriter = NULL;
  open(fileName, outputWidth, outputHeight, fps);
}

/// open the output video file
void OutputVideo::open(const String &fileName, int outputWidth,
                       int outputHeight, double fps) {
#ifdef USE_OPENCV
  int fourcc = CV_FOURCC_DEFAULT;
  if (fileName.endsWith(".avi"))
    fourcc = CV_FOURCC('H', 'F', 'Y', 'U'); // HuffYUV (lossless)
  m_videoWriter = cvCreateVideoWriter(fileName.c_str(), fourcc, fps,
...
void OutputVideo::append(const ImageColorU &img) {
  ...
  if (m_videoWriter) {
    aptr<ImageColorU> flip =
        flipVert(img); // fix(asap): this shouldn't be needed!
#ifdef USE_OPENCV
...
bool OutputVideo::openSuccess() { return m_videoWriter ? true : false; }
