...
void initImageSeq(ImageGrayUSeq &seq, int width, int height, int length,
                  bool clear, int clearValue) {
...
/// create a sequence of images
void initImageSeq(ImageGrayFSeq &seq, int width, int height, int length,
...
/// create a sequence of images
void initImageSeq(ImageGrayISeq &seq, int width, int height, int length,
                  bool clear, int clearValue) {
  assertAlways(seq.count() == 0);
  for (int z = 0; z < length; z++) {
...
void resizeSeqZ(ImageGrayUSeq &inSeq, ImageGrayUSeq &outSeq, int newLength) {
  ...
  for (int z = 0; z < newLength; z++) {
    ...
    if (zOldInt >= oldLength - 1) {
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          out->data(x, y) = round(frac * (float)in2.data(x, y) +
                                  (1.0f - frac) * (float)in1.data(x, y));
        }
      }
    }
    outSeq.append(out);
  ...
  for (int z = 0; z < newLength; z++) {
    ...
    if (zOldInt >= oldLength - 1) {
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          out->data(x, y) =
              frac * in2.data(x, y) + (1.0f - frac) * in1.data(x, y);
        }
      }
    }
    outSeq.append(out);
...
void blurGaussSeqZ(const ImageGrayUSeq &inSeq, ImageGrayUSeq &outSeq,
                   float sigma) {
...
/// blur (in z) a sequences of images
void blurGaussSeqZ(const ImageGrayFSeq &inSeq, ImageGrayFSeq &outSeq,
                   float sigma) {
  int length = inSeq.count();
  int width = inSeq[0].width(), height = inSeq[0].height();
...
int floodFillXYZ(ImageGrayUSeq &seq, int minRegionValue, int maxRegionValue,
                 int fillValue, int x, int y, int z, float *xCentRet,
                 float *yCentRet, float *zCentRet, int *xMinRet, int *xMaxRet,
                 int *yMinRet, int *yMaxRet, int *zMinRet, int *zMaxRet) {
  ...
  int    count    = 0;
  bool   getStats = xCentRet || yCentRet || zCentRet || xMinRet || xMaxRet ||
                  yMinRet || yMaxRet || zMinRet || zMaxRet;
  ...
  if (zCentRet)
    *zCentRet = (float)zSum / (float)count;
  if (xMinRet)
    *xMinRet = xMin;
  if (xMaxRet)
    *xMaxRet = xMax;
