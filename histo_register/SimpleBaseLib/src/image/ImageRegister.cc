...
double evalImageTransform(const ImageTransform &transform,
                          ...
                          int step, int xBorder, int yBorder,
                          const ImageGrayU *srcMask, const ImageGrayU *destMask,
                          bool interp, bool verbose) {
  int xDestMin =
      xBorder; // note that we're using border on both source and dest
  ...
  for (int y = yBorder; y < height - yBorder; y += step) {
    for (int x = xBorder; x < width - xBorder; x += step) {
      if (srcMask == NULL || srcMask->data(x, y)) {
        ...
        if (xDestInt >= xDestMin && xDestInt <= xDestMax &&
            yDestInt >= yDestMin && yDestInt <= yDestMax &&
...
public:
  // basic constructor
  RegistrationObjective(const ImageGrayU &src, const ImageGrayU &dest, int step,
                        int xBorder, int yBorder, const ImageGrayU *srcMask,
                        const ImageGrayU *destMask, bool interp)
  ...
  double eval(const VectorD &point) {
    ...
    return evalImageTransform(transform, m_src, m_dest, m_step, m_xBorder,
                              m_yBorder, m_srcMask, m_destMask, m_interp,
                              m_verbose);
  }
...
/// the xBorder and yBorder specify areas to be ignored for the objective function;
/// the offsetBound value specifies the largest allowed translation between the images
aptr<ImageTransform> registerUsingImageTransform(
    const ImageGrayU &src, const ImageGrayU &dest, int transformParamCount,
    int step, int xBorder, int yBorder, float offsetBound,
    const ImageTransform *initTransform, const ImageGrayU *srcMask,
    const ImageGrayU *destMask, bool interp) {
  assertAlways(transformParamCount == 2 || transformParamCount == 6);

  // prepare objective function
  RegistrationObjective obj(src, dest, step, xBorder, yBorder, srcMask,
                            destMask, interp);
