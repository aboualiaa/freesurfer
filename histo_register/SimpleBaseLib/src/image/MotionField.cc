...
aptr<ImageGrayU> MotionField::mapBackward(const ImageGrayU &img, int fillColor,
                                          float frac) const {
...
/// map the given destination image back to the source coordinates
// fix(faster): use opencv?
aptr<ImageColorU> MotionField::mapBackward(const ImageColorU &img,
                                           int fillColor, float frac) const {
  int               width = m_u->width(), height = m_u->height();
