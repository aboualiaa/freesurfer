...
template <>
aptr<ImageColorU> crop(const ImageColorU &input, int xMin, int xMax, int yMin,
                       int yMax) {
  ...
  if (newWidth > input.width() || newHeight > input.height()) {
    ...
    disp(1, "w: %d  h: %d  new w: %d  new h: %d", input.width(), input.height(),
         newWidth, newHeight);
...
aptr<ImageType> crop(const ImageType &input, int xMin, int xMax, int yMin,
                     int yMax) {
  assertDebug(xMin > 0 && xMax < input.width());
  assertDebug(yMin > 0 && yMax < input.height());
  int             newWidth  = xMax - xMin + 1;
...
template aptr<ImageGrayU> crop(const ImageGrayU &input, int xMin, int xMax,
                               int yMin, int yMax);
template aptr<ImageGrayF> crop(const ImageGrayF &input, int xMin, int xMax,
                               int yMin, int yMax);

/// shrink or zoom image
template <typename ImageType>
aptr<ImageType> resize(const ImageType &input, int newWidth, int newHeight,
                       bool filter) {
  aptr<ImageType> output(new ImageType(newWidth, newHeight));
#ifdef USE_OPENCV
  int width = input.width();
  cvResize(input.iplImage(), output->iplImage(),
           filter ? (newWidth > width ? CV_INTER_LINEAR : CV_INTER_AREA)
                  : CV_INTER_NN);
#else
...
template aptr<ImageGrayU>  resize(const ImageGrayU &input, int newWidth,
                                  int newHeight, bool filter);
template aptr<ImageGrayF>  resize(const ImageGrayF &input, int newWidth,
                                  int newHeight, bool filter);
template aptr<ImageColorU> resize(const ImageColorU &input, int newWidth,
                                  int newHeight, bool filter);

/// translate and scale an image
template <typename ImageType>
aptr<ImageType> shiftScale(const ImageType &input, float xOffset, float yOffset,
                           float xScale, float yScale, int outputWidth,
                           int outputHeight) {
  ...
  cvWarpAffine(input.iplImage(), output->iplImage(), map,
...
template aptr<ImageGrayU>  shiftScale(const ImageGrayU &input, float xOffset,
                                      float yOffset, float xScale, float yScale,
                                      int outputWidth, int outputHeight);
template aptr<ImageGrayF>  shiftScale(const ImageGrayF &input, float xOffset,
                                      float yOffset, float xScale, float yScale,
                                      int outputWidth, int outputHeight);
template aptr<ImageColorU> shiftScale(const ImageColorU &input, float xOffset,
                                      float yOffset, float xScale, float yScale,
                                      int outputWidth, int outputHeight);

/// apply linear transformation to image
template <typename ImageType>
aptr<ImageType> warpAffine(const ImageType &input, float xOffset, float yOffset,
                           float x1, float y1, float x2, float y2,
                           int outputWidth, int outputHeight, int fillColor) {
  aptr<ImageType> output(new ImageType(outputWidth, outputHeight));
#ifdef USE_OPENCV
  CvMat *map = cvCreateMat(2, 3, CV_32FC1);
  cvmSet(map, 0, 0, x1);
  ...
  cvmSet(map, 1, 2, yOffset);
  cvWarpAffine(input.iplImage(), output->iplImage(), map,
               CV_INTER_CUBIC + CV_WARP_FILL_OUTLIERS, cvScalarAll(fillColor));
  // fix(later): use cvGetQuadrangleSubPix?
  cvReleaseMat(&map);
#else
  fatalError("not implemented");
...
template aptr<ImageGrayU>  warpAffine(const ImageGrayU &input, float xOffset,
                                      float yOffset, float x1, float y1,
                                      float x2, float y2, int outputWidth,
                                      int outputHeight, int fillColor);
template aptr<ImageGrayF>  warpAffine(const ImageGrayF &input, float xOffset,
                                      float yOffset, float x1, float y1,
                                      float x2, float y2, int outputWidth,
                                      int outputHeight, int fillColor);
template aptr<ImageColorU> warpAffine(const ImageColorU &input, float xOffset,
                                      float yOffset, float x1, float y1,
                                      float x2, float y2, int outputWidth,
                                      int outputHeight, int fillColor);
...
template <typename ImageType>
aptr<ImageType> flipHoriz(const ImageType &input) {
  aptr<ImageType> output(new ImageType(input.width(), input.height()));
#ifdef USE_OPENCV
...
template <typename ImageType>
aptr<ImageType> rotate180(const ImageType &input) {
  aptr<ImageType> output(new ImageType(input.width(), input.height()));
...
// fix(clean): use opencv
template <typename ImageType>
aptr<ImageType> rotate270(const ImageType &input) {
  int             newWidth  = input.height();
  int             newHeight = input.width();
  int             cc        = input.channelCount();
  aptr<ImageType> output(new ImageType(newWidth, newHeight));
...
template <typename ImageType>
aptr<ImageType> rotate(const ImageType &input, float angleDegrees,
                       int fillColor) {
  int   width = input.width(), height = input.height();
  ...
  aptr<ImageType> output = warpAffine(input, xOffset, yOffset, x1, y1, x2, y2,
                                      outputWidth, outputHeight, fillColor);
  return output;
}
template aptr<ImageGrayU>  rotate(const ImageGrayU &input, float angleDegrees,
                                  int fillColor);
template aptr<ImageGrayF>  rotate(const ImageGrayF &input, float angleDegrees,
                                  int fillColor);
template aptr<ImageColorU> rotate(const ImageColorU &input, float angleDegrees,
                                  int fillColor);
...
ImageTransform::ImageTransform(float xOffset, float yOffset, float xScale,
                               float yScale) {
...
void ImageTransform::save(File &file) const { file.writeVector(m_params); }
...
aptr<ImageGrayU> ImageTransform::mapForward(const ImageGrayU &img,
                                            int outputWidth, int outputHeight,
                                            int fillColor) const {
  if (m_params.length() == 6)
    return warpAffine(img, m_params[0], m_params[1], m_params[2], m_params[3],
                      m_params[4], m_params[5], outputWidth, outputHeight,
                      fillColor);
  assertAlways(m_params.length() == 2) return shiftScale(
      img, m_params[0], m_params[1], 1, 1, outputWidth, outputHeight);
}

/// map an image forward according to the transformation
aptr<ImageColorU> ImageTransform::mapForward(const ImageColorU &img,
                                             int outputWidth, int outputHeight,
                                             int fillColor) const {
  if (m_params.length() == 6)
    return warpAffine(img, m_params[0], m_params[1], m_params[2], m_params[3],
...
aptr<ImageGrayU> ImageTransform::mapBackward(const ImageGrayU &img,
                                             int outputWidth, int outputHeight,
                                             int fillColor) const {
  aptr<ImageTransform> inverseTransform = inverse();
  return inverseTransform->mapForward(img, outputWidth, outputHeight,
                                      fillColor);
}
...
void ImageTransform::display(int indent) {
  if (m_params.length() == 2) {
    ...
    disp(indent, "x: %f, y: %f, x1: %f, y1: %f, x2: %ff, y2: %f", m_params[0],
         m_params[1], m_params[2], m_params[3], m_params[4], m_params[5]);
  } else {
