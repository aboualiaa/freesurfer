...
aptr<ImageGrayU> toUChar(const ImageGrayF &input) {
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++)
      output->data(x, y) =
          bound(round((input.data(x, y) - min) * factor), 0, 255);
  return output;
}
...
aptr<ImageColorU> toUChar(const ImageColorF &input, float scaleFactor) {
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++)
      for (int c = 0; c < 3; c++)
        output->data(x, y, c) =
            bound(round(input.data(x, y, c) * scaleFactor), 0, 255);
  return output;
}
...
aptr<ImageType> blurBox(const ImageType &input, int boxSize) {
  int             width = input.width(), height = input.height();
...
template <typename ImageType>
aptr<ImageType> blurGauss(const ImageType &input, float sigma) {
  assertAlways(sigma > 0);
  int             width = input.width(), height = input.height();
  aptr<ImageType> output(new ImageType(width, height));
#ifdef USE_OPENCV
  cvSmooth(input.iplImage(), output->iplImage(), CV_GAUSSIAN, 0, 0, sigma,
           sigma);
#else
  fatalError("not implemented");
#endif
...
template <typename ImageType>
aptr<ImageType> median(const ImageType &input, int apertureSize) {
  int             width = input.width(), height = input.height();
  aptr<ImageType> output(new ImageType(width, height));
#ifdef USE_OPENCV
  cvSmooth(input.iplImage(), output->iplImage(), CV_MEDIAN, apertureSize,
           apertureSize);
#endif
  return output;
}
...
/// image x gradient
template <typename ImageType>
aptr<ImageGrayF> xGrad(const ImageType &input, int apertureSize) {
  int              width = input.width(), height = input.height();
  aptr<ImageGrayF> output(new ImageGrayF(width, height));
#ifdef USE_OPENCV
...
/// image y gradient
template <typename ImageType>
aptr<ImageGrayF> yGrad(const ImageType &input, int apertureSize) {
  int              width = input.width(), height = input.height();
  aptr<ImageGrayF> output(new ImageGrayF(width, height));
#ifdef USE_OPENCV
...
template <typename ImageType>
aptr<ImageType> threshold(const ImageType &input, float thresh, bool invert) {
  int             width = input.width(), height = input.height();
  aptr<ImageType> output(new ImageType(width, height));
#ifdef USE_OPENCV
  cvThreshold(input.iplImage(), output->iplImage(), thresh, 255,
              invert ? CV_THRESH_BINARY_INV : CV_THRESH_BINARY);
#else
  fatalError("not implemented");
#endif
  return output;
}
template aptr<ImageGrayU> threshold(const ImageGrayU &input, float thresh,
                                    bool invert);
template aptr<ImageGrayF> threshold(const ImageGrayF &input, float thresh,
                                    bool invert);
...
aptr<ImageGrayU> blurBoxAndThreshold(const ImageGrayU &input, int boxSize,
                                     int thresh) {
  int              width = input.width(), height = input.height();
  aptr<ImageGrayU> output(new ImageGrayU(width, height));
#ifdef USE_OPENCV
  cvSmooth(input.iplImage(), output->iplImage(), CV_BLUR, boxSize, boxSize);
  cvThreshold(output->iplImage(), output->iplImage(), thresh, 255,
              CV_THRESH_BINARY);
#else
  fatalError("not implemented");
#endif
...
/// apply a box filter and then threshold
aptr<ImageGrayU> blurBoxAndThreshold(const ImageGrayF &input, int boxSize,
                                     float thresh) {
  int              width = input.width(), height = input.height();
  ...
  cvThreshold(smooth.iplImage(), output->iplImage(), thresh, 255,
              CV_THRESH_BINARY);
#else
  fatalError("not implemented");
#endif
...
void channelMean(const ImageColorU &img, float &rMean, float &gMean,
                 float &bMean) {
  int width = img.width(), height = img.height();
...
/// compute mean absolute difference between pixel values
// fix(faster): add OpenCV version
float meanAbsDiff(const ImageGrayU &img1, const ImageGrayU &img2, int xBorder,
                  int yBorder) {
  int width = img1.width(), height = img1.height();
  assertAlways(img2.width() == width && img2.height() == height);
  double sum = 0;
...
float mutualInfo(const ImageGrayU &img1, const ImageGrayU &img2, int xBorder,
                 int yBorder, int bucketCount) {
  int width = img1.width(), height = img1.height();
  assertAlways(img2.width() == width && img2.height() == height);
  ...
  for (int y = yBorder; y < height - yBorder; y++) {
    for (int x = xBorder; x < width - xBorder; x++) {
      int i =
          bound(img1.data(x, y) * (bucketCount - 1) / 255, 0, bucketCount - 1);
      int j =
          bound(img2.data(x, y) * (bucketCount - 1) / 255, 0, bucketCount - 1);
      histogram.data(i, j)++;
    }
  }
...
void maskBounds(const ImageGrayU &mask, int &xMin, int &xMax, int &yMin,
                int &yMax) {
...
void imageStats(const ImageGrayU &img, int &minRet, float &meanRet,
                int &maxRet) {
  double sum   = 0;
...
/// compute min/mean/max pixel values
void imageStats(const ImageGrayF &img, float &minRet, float &meanRet,
                float &maxRet) {
  double sum   = 0;
...
VectorI imageHistogram(const ImageGrayU &image, int xMin, int xMax, int yMin,
                       int yMax) {
  VectorI hist(256);
  hist.clear(0);
...
template <typename ImageType>
void saveImage(const ImageType &img, const String &fileName) {
#ifdef USE_OPENCV
...
template <typename ImageType> aptr<ImageType> load(const String &fileName) {
  aptr<ImageType> img;
#ifdef USE_OPENCV
  IplImage *iplImg = cvLoadImage(fileName.c_str(), CV_LOAD_IMAGE_ANYDEPTH |
                                                       CV_LOAD_IMAGE_ANYCOLOR);
  if (iplImg) {
    img.reset(new ImageType(iplImg->width, iplImg->height));
    cvConvertImage(
        iplImg, img->iplImage(),
        CV_CVTIMG_FLIP); // image is loaded using TL origin, but we assume BL origin
...
int floodFill(ImageGrayU &img, int minRegionValue, int maxRegionValue,
              int fillValue, int x, int y, float *xCentRet, float *yCentRet,
              int *xMinRet, int *xMaxRet, int *yMinRet, int *yMaxRet) {
  ...
  int    count = 0;
  bool   getStats =
      xCentRet || yCentRet || xMinRet || xMaxRet || yMinRet || yMaxRet;
  ...
  // return results
  if (xCentRet)
    *xCentRet = (float)xSum / (float)count;
  if (yCentRet)
    *yCentRet = (float)ySum / (float)count;
  if (xMinRet)
    *xMinRet = xMin;
  if (xMaxRet)
    *xMaxRet = xMax;
...
bool lineIntersects(const ImageGrayU &mask, float x1, float y1, float x2,
                    float y2) {
  bool  found = false;
  float xDiff = x2 - x1;
  float yDiff = y2 - y1;
...
void drawMaskBoundary(ImageColorU &output, const ImageGrayU &mask, int thresh,
                      int r, int g, int b, int xBorder, int yBorder) {
...
// fix(faster): add opencv version
aptr<ImageColorU> blend(const ImageColorU &image1, const ImageColorU &image2,
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      for (int c = 0; c < 3; c++) {
        int v = round(image1.data(x, y, c) * alpha1 +
                      image2.data(x, y, c) * alpha2);
        if (v < 0)
          v = 0;
        if (v > 255)
          v = 255;
...
aptr<ImageColorU> splitScreen(const ImageColorU &image1,
                              const ImageColorU &image2) {
  int width = image1.width(), height = image1.height();
  assertAlways(image2.width() == width && image2.height() == height);
  aptr<ImageColorU> output(new ImageColorU(width, height));
  int               xMid = width / 2;
...
void initImageUtil() { registerFilter(blurBox); }
