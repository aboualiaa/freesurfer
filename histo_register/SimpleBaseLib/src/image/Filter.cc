...
void filterVideo(Config &conf, bool gray) {
  ...
  assertAlways(frameStep >
               0); // fix(later): display warning, handle negative frameStep

  // open output video
  // fix(clean): would be nice to defer output size until have output image so allow resizing
  OutputVideo outputVideo(outputFileName, inputVideo.width(),
                          inputVideo.height());
  ...
  for (int frameIndex = startFrameIndex; frameIndex <= endFrameIndex;
...
// apply a generalized filter to a video
void filterColorVideo(Config &conf) { filterVideo(conf, false); }

// apply a generalized filter to a mask sequence
void filterGrayVideo(Config &conf) { filterVideo(conf, true); }
