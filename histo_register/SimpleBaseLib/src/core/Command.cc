...
String nextHistoryCommand(int offset) {
  ...
  if (count) {
    ...
    g_commandHistoryIndex = bound(g_commandHistoryIndex, 0,
                                  count); // note: allow to be one past the end
...
void setCancelCommand(bool cancel) { g_cancelCommand = cancel; }
...
void runScript(Config &conf) {
  ...
  String fileName =
      addDataPath(conf.readString("fileName", dataPath() + "script.txt"));
  if (conf.initialPass())
    return;
...
void cancelCurrentCommands(Config &conf) { setCancelCommand(true); }
