...
TableColumn::TableColumn(TaggedFile &file) {
  ...
  do {
    ...
    case TAG_NAME:
      ...
      break;
    case TAG_VISIBLE:
      m_visible = file.readBool();
      break;
    case TAG_TOOL_TIP:
      m_toolTip = file.readString();
      break;
    case TAG_FORMAT:
      m_format = file.readString();
      break;
    case TAG_IS_TIMESTAMP:
      m_isTimestamp = file.readBool();
      break;
    case TAG_HIGHLIGHT_MIN_MAX:
      m_highlightMinMax = file.readBool();
      break;
    case TAG_DATA_INT:
      m_dataInt = file.readVector<int>();
      break;
    case TAG_DATA_DOUBLE:
      m_dataDouble = file.readVector<double>();
      break;
    case TAG_DATA_STRING:
      file.readStrings(m_dataString);
      break;
    default:
      file.skipTag(tag);
    }
...
String TableColumn::webStatText() const {
  ...
  if (m_dataInt.length()) {
    if (m_isTimestamp) {
      result =
          sprintF("min: %s<br>mean: %s<br>max: %s<br>frac: %1.2f<br>count: %d",
                  tsToStrLocal(m_minInt, true).c_str(),
                  tsToStrLocal(m_meanInt, true).c_str(),
                  tsToStrLocal(m_maxInt, true).c_str(),
                  (float)m_nonZeroCount / (float)m_dataInt.length(),
                  m_dataInt.length());
    } else {
      result =
          sprintF("min: %d<br>mean: %d<br>max: %d<br>frac: %1.2f<br>count: %d",
                  m_minInt, m_meanInt, m_maxInt,
                  (float)m_nonZeroCount / (float)m_dataInt.length(),
                  m_dataInt.length());
    }
  }
  if (m_dataDouble.length()) {
    result = sprintF(
        "min: %1.4f<br>mean: %1.4f<br>max: %1.4f<br>frac: %1.2f<br>count: %d",
        m_minDouble, m_meanDouble, m_maxDouble,
        (float)m_nonZeroCount / (float)m_dataDouble.length(),
        m_dataDouble.length());
  }
  return result;
}
...
String TableColumn::displayValue(int index, String &target, bool alignRight,
                                 String &color) const {
  String value;
  if (index >= 0) {
...
String TableColumn::exportValue(int index) const {
  ...
  if (index < m_dataDouble.length()) {
    ...
    if (value.contains('.') ==
        false) // fix(later): this is a hack to make sure double fields always have decimal point, so that we can load the csv correctly
      value += ".0";
  }
  if (index < m_dataString.count())
    value = String(m_dataString[index]);
...
/// add a double value
void Table::add(const String &colName, double val, bool highlightMinMax,
                const String &format) {
  bool         newCol = false;
...
/// add a float value
void Table::add(const String &colName, float val, bool highlightMinMax,
                const String &format) {
  bool         newCol = false;
  TableColumn &column = findAddColumn(colName.c_str(), newCol);
  column.add(val);
...
/// add an int value
void Table::add(const String &colName, int val, bool highlightMinMax,
                const String &format) {
  bool         newCol = false;
  TableColumn &column = findAddColumn(colName.c_str(), newCol);
  column.add(val);
