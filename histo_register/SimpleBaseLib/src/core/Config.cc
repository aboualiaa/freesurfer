...
int Config::readInt(const String &name, int defaultValue) {
  ...
  if (configEntry == NULL) {
    ...
    if (m_allowDefault) {
      if (defaultValue == -7777) {
        if (m_initialPass == false)
          warning("Config::readInt: entry not found: %s; no default specified",
                  name.c_str());
        if (m_dualPass)
          writeInt(name, 0);
        return 0;
  ...
  if (configEntry == NULL) {
    ...
    if (m_allowDefault) {
      if (defaultValue > -7777.78 && defaultValue < -7777.76) {
        if (m_initialPass == false)
          warning(
              "Config::readDouble: entry not found: %s; no default specified",
              name.c_str());
        if (m_dualPass)
...
void Config::writeString(const String &name, const String &val,
  ...
  if (configEntry == NULL) {
    if (m_allowNewEntries) {
      ...
      warning("failed attempt to add new config entry: %s=%s", name.c_str(),
              val.c_str());
      return;
    }
  }
...
bool Config::load(const String &fileName) {
  ...
  while (file.endOfFile() == false) {
    ...
    if (line.length() == 0) {
      ...
      if (configEntry->name.length() &&
          (configEntry->value.length() ||
           configEntry->type == CONFIG_ENTRY_SECTION)) {
        m_configEntries.append(configEntry);
      } else {
        delete configEntry;
...
void Config::update(const String &update) {
  ...
  for (int i = 0; i < split.count(); i++) {
    ...
    if (subSplit.count() == 2) {
      disp(1, "update param: %s, value: %s", subSplit[0].c_str(),
           subSplit[1].c_str());
