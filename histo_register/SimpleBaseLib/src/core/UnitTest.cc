...
// basic constructor
UnitTest::UnitTest(const String &name, bool (*callback)(),
...
bool UnitTest::run() {
  ...
  timer.stop();
  disp(2, "%s (%s) time: %1.1fms, result: %s", m_name.c_str(),
       m_fileName.c_str(), timer.timeSum(), result ? "passed" : "FAILED");
  return result;
}
...
void initUnitTest() { registerCommand("unittest", runUnitTests); }
