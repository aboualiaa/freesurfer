...
Array<String> sort(const Array<String> &strArr) {
  ...
  String const **data =
      new String const *[count]; // non-const array of pointers to const data
...
bool testSortStringArrayCase(const String &list, const String &listGoal) {
  ...
  unitAssert(sorted.count() == arrGoal.count()) for (int i = 0; i < arr.count();
                                                     i++)
...
// test string split
bool testSplitCase(const char *str, const char *splitChars, int count,
