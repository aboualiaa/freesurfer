...
template Vector<float> subset(const Vector<float> &v, int minIndex,
                              int maxIndex);
template Vector<double> subset(const Vector<double> &v, int minIndex,
                               int maxIndex);
...
VectorI splitNumbersI(const String &s) {
  ...
  for (int i = 0; i < split.count(); i++)
    result[i] =
        split[i].strip().toInt(); // fix(clean): make a to<type> template
  return result;
}
...
VectorF splitNumbersF(const String &s) {
  ...
  for (int i = 0; i < split.count(); i++)
    result[i] =
        split[i].strip().toFloat(); // fix(clean): make a to<type> template
  return result;
}

/// create a comma-delimited string of numbers suitable for display
String toString(const VectorF &v, int minIndex, int maxIndex,
                const String &format) {
  Array<String> strArr;
...
template void quantiles<int>(const Vector<int> &v, int &min, int &per25,
                             int &median, int &per75, int &max);
template void quantiles<float>(const Vector<float> &v, float &min, float &per25,
                               float &median, float &per75, float &max);
...
void medianSplit(VectorF &v) {
  ...
  disp(2, "median split thresh: %f, min: %f, mean: %f, max: %f", thresh,
       v.min(), v.mean(), v.max());
  for (int i = 0; i < len; i++) {
    if (v[i] > thresh)
      v[i] = 1;
