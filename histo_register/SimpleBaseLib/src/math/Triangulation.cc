...
aptr<MatrixF> triangulate(const MatrixF &points) {
  ...
  Mesh *mesh = new Mesh(Point2d(xMin, yMin), Point2d(xMax, yMin),
  ...
  int edgeCount = 0;
  for (LlistPos p = mesh->edges.first(); !mesh->edges.isEnd(p);
       p          = mesh->edges.next(p))
    edgeCount++;
...
aptr<MatrixI> triangulateIndex(const MatrixF &points) {
  ...
  for (int i = 0; i < pointCount; i++)
    mesh->InsertSite(
        Point2d(points.data(i, 0), points.data(i, 1), &(seq.data(i))));
...
aptr<ImageGrayF> triangulationInterpolation(const MatrixF &points,
                                            VectorF &values, int width,
                                            int height) {
  ImageGrayF *result = new ImageGrayF(width, height);
  result->clear(-777);

  // create mesh using image bounds
  float xMin = 0, xMax = (float)(width - 1), yMin = 0,
        yMax = (float)(height - 1);
  Mesh *mesh =
      new Mesh(Point2d(xMin, yMin, nearestValue(xMin, yMin, points, values)),
  ...
  for (int i = 0; i < pointCount; i++)
    mesh->InsertSite(
        Point2d(points.data(i, 0), points.data(i, 1), &(values.data(i))));
