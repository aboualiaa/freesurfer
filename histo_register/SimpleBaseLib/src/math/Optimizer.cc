...
VectorD Optimizer::repeatRun(double *finalObjValue) {
  ...
  for (int iter = 0; iter < 100; iter++) {
    ...
    if ((runValue - bestValue) / ((runValue + bestValue) * 0.5f + 1e-8) <
        m_finalTolerance) {
      break;
    }
...
void Optimizer::plotObjectiveSlices(const VectorD &center, int pointsPerDim,
...
void Optimizer::plotObjectiveSlice(const VectorD &center, int pointsPerDim,
                                   int dim, const String &fileName) const {
...
void Optimizer::plotObjectiveMap(const VectorD &center, int dim1, int dim2,
                                 int points1, int points2, int scaleFactor,
...
void Optimizer::dispHistoryStats(int indent) const {
  disp(indent, "eval count: %d, eval min: %f, eval max: %f",
       m_evalValues.length(), m_evalValues.min(), m_evalValues.max());
  disp(indent, "best count: %d, best min: %f, best max: %f",
       m_bestValues.length(), m_bestValues.min(), m_bestValues.max());
}
...
VectorD SimplexOptimizer::run(double *finalObjValue) {
  ...
  while (done == false) {
    ...
    if ((maxValue - minValue) / ((minValue + maxValue) * 0.5f + 1e-8) <
        m_finalTolerance)
      break;
    ;
