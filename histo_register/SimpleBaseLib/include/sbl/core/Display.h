...
/*! \file Display.h
    \brief The Display module provides functions for outputting text from
   commands (including logging and error messages).  The functions can be used
   with the GUI modules to display diagnostic information in the user interface.
*/
...
#define assertDebug(condition)                                                 \
  if (!(condition))                                                            \
    fatalError("assert failed at %s:%d: %s", __FILE__, __LINE__, #condition);
#else
#define assertDebug(condition)
#endif
...
#define assertAlways(condition)                                                \
  if (!(condition))                                                            \
    fatalError("assert failed at %s:%d", __FILE__, __LINE__, #condition);
...
/// set output files for warning() and fatalError() messages; can use "stdout" /
/// "stderr"
