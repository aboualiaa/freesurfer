...
// The Array class deallocates its contents, whereas the PtrArray class does
// not. As such, the PtrArray can store const types (whereas the Array class
// cannot). The PtrArray class is faster and simpler.

// fix(clean): unify this code; lots of duplication between Array and PtrArray

/// The ArrayItem class is a reference-counted pointer wrapper used for making
/// shallow copies of Array objects.
...
public:
  ...
  /// returns number of elements in array (some may be NULL if not set using
  /// set())
  ...
  inline int memUsed() const {
    return sizeof(Array<T>) + m_allocCount * sizeof(int *);
  }
  ...
  /// get index of matching object (according to == operator), or -1 if not
  /// found
...
template <typename T> void Array<T>::extend(int newLength) {
  ...
  // at the end of this function: every element from 0 to m_allocCount - 1
...
/// The PtrArray class holds a dynamically-resizable array of pointers to
/// objects. It will not deallocate the objects when it is deallocated.
...
public:
  ...
  /// returns number of elements in set (some may be NULL if not set using
  /// set())
  inline int size() const { return m_count; }
  inline int count() const { return m_count; }
  ...
  /// set an item in the set (replaces old item, if any); does not take
