...
public:
  ...
  /// true if the column contains timestamps (assuming timestamps are integer
  /// valued)
  inline bool isTimestamp() const { return m_isTimestamp; }

  /// string representations of the data
  String displayValue(int index, String &target, bool alignRight,
                      String &color) const;
...
// fix(clean): move paging and other web stuff into WebTableView class? (or just
// remove?)
class Table {
public:
  ...
  /// add a value
  void add(const String &colName, int val, bool highlightMinMax = false,
           const String &format = "");
  void add(const String &colName, float val, bool highlightMinMax = false,
           const String &format = "");
  void add(const String &colName, double val, bool highlightMinMax = false,
           const String &format = "");
  ...
  void setDefaultFormat(const String &defaultFormat) {
  ...
  static void setDisplay(Display<aptr<Table>> *tableDisplay) {
    s_tableDisplay = tableDisplay;
  }
...
private:
  ...
  // file format tags
  enum { TAG_TITLE = 101, TAG_COLUMNS = 201 + TAG_OFFSET_ARRAY };
