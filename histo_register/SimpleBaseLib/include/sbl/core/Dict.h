...
// fix(clean): make key type a template parameter (so just one dictionary class
// with string keys, integer keys, etc.)
...
/// (The class uses a power-of-2 number of bins, so certain key patterns could
/// have many collisions, resulting in slow performance.)
...
public:
  ...
  /// disables object deallocation; the dictionary no longer takes ownership of
  /// object pointers
...
public:
  ...
  /// access as array
  inline const String &refArrayKey(int index) const {
