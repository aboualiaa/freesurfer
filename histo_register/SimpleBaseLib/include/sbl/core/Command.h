...
/*! \file Command.h
    \brief The Command module manages the set of currently registered command
   functions. The command functions can be executed from scripts, the
   command-line, and optionally a graphical user interface.  Each command
   function has a string name and a description determined from the C++ function
   name.  All command function take a Config object as their only argument.
   These Config objects are populated with parameters for the command.
*/
...
/// register a command name and callback; the command description is determined
/// from the callback function name
#define registerCommand(name, callback)                                        \
  registerCommandInternal(name, callback, #callback)
void registerCommandInternal(const String &name, void (*callback)(Config &conf),
...
// get a command from the history, at the specified offset from the current list
// position
...
/// execute a command with arguments included in the command string (space
/// separated)
...
/// this callback will be called every time a long-running command calls
/// checkCommandEvents
