...
/// The aptr_ref class is used to assist copying aptr instances.  It is a direct
/// clone of std::auto_ptr_ref; we define our own so as not to include all of
...
/// The aptr class represents a simple smart pointer.  It is a direct clone of
/// std::auto_ptr; we define our own so as not to include all of the other
/// definitions.
template <class T> class aptr {
public:
  ...
  template <class T2> operator aptr<T2>() { return aptr<T2>(*this); }
