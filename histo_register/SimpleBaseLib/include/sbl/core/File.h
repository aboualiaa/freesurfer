...
/// file open modes
enum FileOpenMode { FILE_READ, FILE_WRITE, FILE_APPEND };
...
public:
  /// open a file
  explicit File(const String &fileName = "", FileOpenMode mode = FILE_READ,
                FileOpenType type = FILE_TEXT);
  ...
  /// returns true if end of file reached on last read; returns true if file not
  /// successfully opened
  bool endOfFile();
  ...
  virtual inline void writeUChar(unsigned char val) {
    if (m_binary)
      writeBlock(&val, sizeof(unsigned char));
    else
      fprintf(m_file, "%d ", val);
  }
  virtual inline void writeUShort(unsigned short val) {
    if (m_binary)
      writeBlock(&val, sizeof(unsigned short));
    else
      fprintf(m_file, "%d ", val);
  }
  virtual inline void writeInt(int val) {
    if (m_binary)
      writeBlock(&val, sizeof(int));
    else
      fprintf(m_file, "%d ", val);
  }
  virtual inline void writeFloat(float val) {
    if (m_binary)
      writeBlock(&val, sizeof(float));
    else
      fprintf(m_file, "%f ", val);
  }
  virtual inline void writeDouble(double val) {
    if (m_binary)
      writeBlock(&val, sizeof(double));
    else
  ...
  virtual inline unsigned char readUChar() {
    unsigned char val = 0;
    if (m_binary)
      readBlock(&val, sizeof(unsigned char));
    else {
      int valInt = 0;
      CHECK_READ(fscanf(m_file, "%d", &valInt));
      val = (unsigned char)valInt;
    }
    return val;
  } // fix(clean): what is correct format specifier? not %c, not %d, not %u?
  virtual inline unsigned short readUShort() {
    unsigned short val = 0;
    if (m_binary)
      readBlock(&val, sizeof(unsigned short));
    else
      CHECK_READ(fscanf(m_file, "%hu", &val));
    return val;
  }
  virtual inline int readInt() {
    int val = 0;
    if (m_binary)
      readBlock(&val, sizeof(int));
    else
      CHECK_READ(fscanf(m_file, "%d", &val));
    return val;
  }
  virtual inline float readFloat() {
    float val = 0;
    if (m_binary)
      readBlock(&val, sizeof(float));
    else
      CHECK_READ(fscanf(m_file, "%f", &val));
    return val;
  }
  virtual inline double readDouble() {
    double val = 0;
    if (m_binary)
      readBlock(&val, sizeof(double));
    else
      CHECK_READ(fscanf(m_file, "%lf", &val));
    return val;
  }
