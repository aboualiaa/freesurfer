...
public:
  ...
  inline String subString(int startPosition, int length) const {
    return String(*this, startPosition, length);
  }

  /// get string left/right of given position
  inline String leftOf(int position) const { return subString(0, position); }
  inline String rightOf(int position) const {
    return subString(position + 1, m_length - position - 1);
  }

  /// get string left/right of first/last instance of the specified character
  inline String rightOfFirst(unsigned short c) const {
    int pos = firstCharPos(c);
    if (pos >= 0)
      return rightOf(pos);
    return *this;
  }
  inline String rightOfLast(unsigned short c) const {
    int pos = lastCharPos(c);
    if (pos >= 0)
      return rightOf(pos);
    return *this;
  }
  inline String leftOfFirst(unsigned short c) const {
    int pos = firstCharPos(c);
    if (pos >= 0)
      return leftOf(pos);
    return *this;
  }
  inline String leftOfLast(unsigned short c) const {
    int pos = lastCharPos(c);
    if (pos >= 0)
      return leftOf(pos);
    return *this;
  ...
  /// split a string at any of the given split characters; see python split()
  /// examples
...
private:
  ...
  char *m_cstring; // hack: put m_cstring first so that printf of string object
                   // (but not pointer to string object) works (at least when
