...
/// The Config class represents a configuration file with a set of named
/// parameters.
class Config {
public:
  // basic constructor
  ...
  inline VectorI readVectorI(const String &name) {
    return splitNumbersI(readString(name, ""));
  }
  inline VectorF readVectorF(const String &name) {
    return splitNumbersF(readString(name, ""));
  }

  /// write value to parameter with given name
  inline void writeBool(const String &name, bool val) {
    writeString(name, sprintF("%d", (int)val), CONFIG_ENTRY_BOOL);
  }
  inline void writeInt(const String &name, int val) {
    writeString(name, sprintF("%d", val));
  }
  inline void writeFloat(const String &name, float val) {
  ...
  inline void writeVectorI(const String &name, const VectorI &vector) {
    writeString(name, toString(vector));
  }
  inline void writeVectorF(const String &name, const VectorF &vector) {
    writeString(name, toString(vector, 0, 1000, "%f"));
  }
  inline void writeSection(const String &name) {
    writeString(name, "", CONFIG_ENTRY_SECTION);
  }

  /// checks whether entry exists in this config
  inline bool entryExists(const String &name) const {
    return findEntry(name) ? true : false;
  }
  ...
  inline const ConfigEntry &entry(int index) const {
    return m_configEntries[index];
  }
  ...
  /// returns true if first pass (in dual-pass mode), meaning command should
  /// terminate after reading params (it will be run again in for the second
  /// pass)
  inline bool initialPass() {
    m_checkedInitialPass = true;
    return m_initialPass;
  }
  ...
  /// updates the config using this syntax:
  ...
  /// set an array of command arguments, to be assigned to subsequent calls to
  /// read() functions
  void setCommandArgs(const Array<String> &commandArgs);

  /// an array of command arguments, to be assigned to subsequent calls to
  /// read() functions
...
private:
  ...
  // dual-pass mode: first pass lets command specify args for GUI, second pass
  // actually runs the command
  bool m_dualPass;

  // true if first pass (in dual-pass mode), meaning command should terminate
  // after reading params (it will be run again in for the second pass)
  bool m_initialPass;
