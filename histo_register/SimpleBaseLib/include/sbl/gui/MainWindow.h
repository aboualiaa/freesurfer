...
public:
  ...
  inline void addKeyboardShortcut(int code, const String &command) {
    m_keyboardShortcuts.add(code, new KeyboardShortcut(command, 0));
  }

  /// add a keyboard shortcut that runs a command (uses wxWidgets key codes and
  /// modifiers); assumes no more than one shortcut per code
  inline void addKeyboardShortcut(int modifier, int code,
                                  const String &command) {
    m_keyboardShortcuts.add(code, new KeyboardShortcut(command, modifier));
  }
...
/// The MainApp class is the top-level object for the user interface; create a
/// sub-class to specify own start-up code.
