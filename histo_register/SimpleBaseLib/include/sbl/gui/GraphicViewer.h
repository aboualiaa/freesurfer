...
/// be notified of clicks within a GraphicViewer.
...
/// The GraphicViewer widget is a sub-class for other widgets that display
/// graphics, allowing zooming, panning, etc.
...
public:
  ...
  inline int windowWidth() const {
    return GetSize().GetWidth() - GetWindowBorderSize().GetX();
  }
  inline int windowHeight() const {
    return GetSize().GetHeight() - GetWindowBorderSize().GetY();
  }
...
protected:
  /// convert data coordinate to window coordinate (note: we take care of y flip
  /// here)
