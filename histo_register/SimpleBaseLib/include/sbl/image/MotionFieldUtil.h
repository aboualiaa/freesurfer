...
/*! \file MotionFieldUtil.h
    \brief The MotionFieldUtil module includes functions for manipulating and
   analyzing MotionField objects (2D vector fields representing pixel
...
void motionFieldStats(const MotionField &mf, float &uMin, float &vMin,
                      float &uMean, float &vMean, float &uMax, float &vMax);
...
/// create color visualization of motion field; different colors represent
/// different motions; gray corresponds to no motion; black indicates occluded
/// regions
...
/// compute motion field gradient magnitude squared [(du/dx)^2 + (du/dy)^2 +
/// (dv/dx)^2 + (dv/dy)^2]
...
aptr<MotionField> computeDenseMotion(const VectorF &x, const VectorF &y,
                                     const VectorF &u, const VectorF &v,
                                     int width, int height);

/// compute inverse motion field using point sample triangulation;
/// sampleStep determines distance between samples
aptr<MotionField> invertMotionField(const MotionField &mf,
                                    const ImageGrayU &mask, int sampleStep);
