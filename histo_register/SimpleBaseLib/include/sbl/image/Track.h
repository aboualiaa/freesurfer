...
/// The Track class represents the trajectory of a feature point in an image
/// sequence.
class Track {
public:
  ...
  inline float x(int frameIndex) const {
    return m_x[frameIndex + m_startFrameIndex];
  }
  inline float y(int frameIndex) const {
    return m_y[frameIndex + m_startFrameIndex];
  }

  /// get current start and end frame indices
  inline int startFrameIndex() const { return m_startFrameIndex; }
  inline int endFrameIndex() const {
    return m_startFrameIndex + m_x.length() - 1;
  }
  ...
  inline int inBounds(int frameIndex) const {
    return frameIndex >= m_startFrameIndex && frameIndex <= endFrameIndex();
  }
...
/// The TrackSet represents a set of tracks (assumed to be for a single image
/// sequence).
class TrackSet {
public:
