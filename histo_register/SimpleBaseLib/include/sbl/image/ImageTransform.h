...
/*! \file ImageTransform.h
    \brief The ImageTransform module provides functions for spatial
   transformations of images. Most of these functions are    simple wrappers for
   OpenCV functions. For other image filters, see the ImageUtil module.
*/
...
/// The ImageTransform class represents a 2D image transformation (currently
/// only linear transformations).
class ImageTransform {
public:
  /// create transformation from parameters (assumes correct number of
  /// parameters)
  ...
  /// transform a point
  inline float xTransform(float x, float y) const {
    return (m_params.length() == 2)
               ? m_params[0] + x
               : m_params[0] + x * m_params[2] + y * m_params[4];
  }
  inline float yTransform(float x, float y) const {
    return (m_params.length() == 2)
               ? m_params[1] + y
               : m_params[1] + x * m_params[3] + y * m_params[5];
  }
  ...
  inline void setOffset(float xOffset, float yOffset) {
    m_params[0] = xOffset;
    m_params[1] = yOffset;
  }
  ...
  aptr<ImageGrayU> mapBackward(const ImageGrayU &img, int outputWidth,
                               int outputHeight, int fillColor) const;
