...
/*! \file ImageUtil.h
    \brief The ImageUtil module provides various functions for working with
   image objects, including filtering, converting, and loading/saving.  Most of
   these functions are simple wrappers for OpenCV functions.  For spatial image
   transformations, see the ImageTransform module.
*/
...
int floodFill(ImageGrayU &img, int minRegionColor, int maxRegionColor,
              int fillValue, int x, int y, float *xCentRet = NULL,
              float *yCentRet = NULL, int *xMinRet = NULL, int *xMaxRet = NULL,
              int *yMinRet = NULL, int *yMaxRet = NULL);

/// keep only mask components that have a pixel count within the given range
/// (assumes mask has only 0 and 255 values)
void filterMaskComponents(ImageGrayU &mask, int minSize, int maxSize);

/// fill holes in mask smaller than given size (assumes mask has only 0 and 255
/// values)
...
void drawMaskBoundary(ImageColorU &output, const ImageGrayU &mask, int thresh,
                      int r, int g, int b, int xBorder = 0, int yBoder = 0);
