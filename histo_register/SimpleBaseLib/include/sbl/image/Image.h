...
CVAPI(void)
cvReleaseImage(IplImage **image); // we need this, but cxcore seems to be
                                  // incompatible with something else (windows?)
#else
...
// static assert; used here to make sure we don't use color methods on gray
...
public:
  ...
  Image(const Image &img) {
    ...
    memcpy(m_raw, img.rawConst(), m_rowBytes * m_height);
  }
  ...
  inline T &data(int x, int y) {
    assertStatic(CHANNEL_COUNT == 1);
    IMGCHK return m_ptr[y][x];
  }
  inline const T &data(int x, int y) const {
    assertStatic(CHANNEL_COUNT == 1);
    IMGCHK return m_ptr[y][x];
  }
  ...
  inline T &data(int x, int y, int c) {
    IMGCHK return m_ptr[y][x * CHANNEL_COUNT + c];
  }
  inline const T &data(int x, int y, int c) const {
    IMGCHK return m_ptr[y][x * CHANNEL_COUNT + c];
  }
  float interp(float x, float y, int c) const;

  /// get/set pixel values for RGB (actually BGR) color images (assumes
  /// CHANNEL_COUNT >= 3)
  inline T b(int x, int y) const {
    assertStatic(CHANNEL_COUNT >= 3);
    IMGCHK return m_ptr[y][x * CHANNEL_COUNT + B_CHANNEL];
  }
  inline T g(int x, int y) const {
    assertStatic(CHANNEL_COUNT >= 3);
    IMGCHK return m_ptr[y][x * CHANNEL_COUNT + G_CHANNEL];
  }
  inline T r(int x, int y) const {
    assertStatic(CHANNEL_COUNT >= 3);
    IMGCHK return m_ptr[y][x * CHANNEL_COUNT + R_CHANNEL];
  }
  ...
  inline void setB(int x, int y, T v) {
    assertStatic(CHANNEL_COUNT >= 3);
    IMGCHK m_ptr[y][x * CHANNEL_COUNT + B_CHANNEL] = v;
  }
  inline void setG(int x, int y, T v) {
    assertStatic(CHANNEL_COUNT >= 3);
    IMGCHK m_ptr[y][x * CHANNEL_COUNT + G_CHANNEL] = v;
  }
  inline void setR(int x, int y, T v) {
    assertStatic(CHANNEL_COUNT >= 3);
    IMGCHK m_ptr[y][x * CHANNEL_COUNT + R_CHANNEL] = v;
  }
  ...
  inline bool inBounds(float x, float y) const {
...
public:
  ...
  inline IplImage *iplImage() {
    if (m_iplImage == NULL)
      createIplImage();
    return m_iplImage;
  }
  inline const IplImage *iplImage() const {
    if (m_iplImage == NULL)
      createIplImage();
    return m_iplImage;
  }
...
void Image<T, CHANNEL_COUNT>::alloc(int width, int height) {
  ...
  m_raw = new T[size];
  if (m_raw == NULL)
    fatalError("error allocating ImageColor data");
  m_deleteRaw = true;
...
/// clear the image to the specified color
template <typename T, int CHANNEL_COUNT>
void Image<T, CHANNEL_COUNT>::clear(T r, T g, T b) {
...
/// clear the image to the specified color
template <typename T, int CHANNEL_COUNT>
void Image<T, CHANNEL_COUNT>::clear(T v) {
  for (int y = 0; y < m_height; y++) {
    for (int x = 0; x < m_width; x++) {
      m_ptr[y][x] = v;
    }
...
float Image<T, CHANNEL_COUNT>::interp(float x, float y) const {
  ...
  if (xInt == m_width - 1) { // fix(faster): make faster?
    if (yInt == m_height - 1) {
      ...
      float yFrac = y - yInt;
      val = (1.0f - yFrac) * m_ptr[yInt][xInt] + yFrac * m_ptr[yInt + 1][xInt];
    }
  } else if (yInt == m_height - 1) {
    float xFrac = x - xInt;
    val = (1.0f - xFrac) * m_ptr[yInt][xInt] + xFrac * m_ptr[yInt][xInt + 1];
  } else {
    float xFrac = x - xInt;
    float yFrac = y - yInt;
...
float Image<T, CHANNEL_COUNT>::interp(float x, float y, int c) const {
  ...
  float yFrac = y - yInt;
  float val =
      (1.0f - xFrac) * (1.0f - yFrac) * m_ptr[yInt][(xInt)*CHANNEL_COUNT + c] +
...
template <typename T, int CHANNEL_COUNT>
Image<T, CHANNEL_COUNT>::Image(IplImage *iplImage, bool deallocIplImage) {
...
/// create object if needed (m_iplImage is mutable so this func can be const)
template <typename T, int CHANNEL_COUNT>
void Image<T, CHANNEL_COUNT>::createIplImage() const {
