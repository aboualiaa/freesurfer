...
/*! \file Plot.h
    \brief The Plot module is used for generating plots of data, including line
   plots, scatter plots, and histograms. The plots can be saved as SVG files.
*/

/// drawing style of plot items
enum PlotStyle { PLOT_LINES = 1, PLOT_DOTS = 2, PLOT_CIRCLES = 4 };

/// The PlotItem class represents a set of data with a uniform appearance (a
/// line or set of dots of a given color).
class PlotItem {
public:
  // basic constructor
...
/// The Plot class can be used to generate a 2D graphical plot from a set of
/// data.
class Plot {
public:
  // basic constructor
  ...
  /// add data to plot
  void add(const VectorD &x, const VectorD &y,
           const Array<String> *labels = NULL, const String &caption = "",
           bool updateBounds = true);
  void add(const VectorD &y, const Array<String> *labels = NULL,
           const String &caption = "", bool updateBounds = true);
  ...
  inline void setAxisLabels(const String &xLabel, const String &yLabel) {
    m_xLabel = xLabel;
    m_yLabel = yLabel;
  }
  inline void setXBoundLabels(const String &xMinLabel,
                              const String &xMaxLabel) {
    m_xMinLabel = xMinLabel;
    m_xMaxLabel = xMaxLabel;
  }
  inline void setYBoundLabels(const String &yMinLabel,
                              const String &yMaxLabel) {
  ...
  inline void setXBounds(double xMin, double xMax) {
    m_xMin = xMin;
    m_xMax = xMax;
  }
  inline void setYBounds(double yMin, double yMax) {
    m_yMin = yMin;
    m_yMax = yMax;
  }
  ...
  static void setDisplay(Display<aptr<Plot>> *plotDisplay) {
    s_plotDisplay = plotDisplay;
  }
