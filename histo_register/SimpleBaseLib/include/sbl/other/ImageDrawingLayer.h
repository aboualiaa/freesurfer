...
public:
  ...
  void drawRectFilled(float x, float y, float width, float height, float r,
                      float g, float b);
  ...
  void drawCircleFilled(float x, float y, float radius, float r, float g,
                        float b);

  /// draw text
  void drawText(const String &text, float x, float y, float r, float g, float b,
                HorizAlignment alignment = HORIZ_ALIGN_LEFT);
