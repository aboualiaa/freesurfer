...
/*
    A tagged file consists of a sequence of items.  Each item has one of two
   forms:
    1. a single value (int, float, string, int array, float array, etc.)
        - a tag
        - the size of the data (in bytes)
        - followed by the data
    2. an array of sub-items (variable size objects, as opposed to an array of
   floats or an array of characters)
        - a tag
        - the number of items
...
/// The TaggedFile class provides facilities for reading/write tag-based file
/// formats.
...
public:
  ...
  inline void writeBool(bool val) {
    writeTagInfo(sizeof(int));
    writeInt((int)val);
  }
  inline void writeUChar(unsigned char val) {
    writeTagInfo(sizeof(unsigned char));
    fwrite(&val, sizeof(unsigned char), 1, m_file);
  }
  inline void writeUShort(unsigned short val) {
    writeTagInfo(sizeof(unsigned short));
    fwrite(&val, sizeof(unsigned short), 1, m_file);
  }
  inline void writeInt(int val) {
    writeTagInfo(sizeof(int));
    fwrite(&val, sizeof(int), 1, m_file);
  }
  inline void writeFloat(float val) {
    writeTagInfo(sizeof(float));
    fwrite(&val, sizeof(float), 1, m_file);
  }
  inline void writeDouble(double val) {
    writeTagInfo(sizeof(double));
    fwrite(&val, sizeof(double), 1, m_file);
  }
  void writeString(const String &s);

  /// read basic data types with size info
  inline bool readBool() {
    assertAlways(readTagInfo() == sizeof(int));
    return readInt() ? true : false;
  }
  inline unsigned char readUChar() {
    assertAlways(readTagInfo() == sizeof(unsigned char));
    unsigned char val = 0;
    CHECK_READ(fread(&val, sizeof(unsigned char), 1, m_file));
    return val;
  }
  inline unsigned short readUShort() {
    assertAlways(readTagInfo() == sizeof(unsigned short));
    unsigned short val = 0;
    CHECK_READ(fread(&val, sizeof(unsigned short), 1, m_file));
    return val;
  }
  inline int readInt() {
    assertAlways(readTagInfo() == sizeof(int));
    int val = 0;
    CHECK_READ(fread(&val, sizeof(int), 1, m_file));
    return val;
  }
  inline float readFloat() {
    assertAlways(readTagInfo() == sizeof(float));
    float val = 0;
    CHECK_READ(fread(&val, sizeof(float), 1, m_file));
    return val;
  }
  inline double readDouble() {
    assertAlways(readTagInfo() == sizeof(double));
    double val = 0;
    CHECK_READ(fread(&val, sizeof(double), 1, m_file));
    return val;
  }
  String readString();
  ...
  // note: could have used above readArray/writeArray but don't want to add
  // load/save methods to String class
  ...
  /// write a tag and return file reference, allowing this syntax: file.tag( TAG
  /// ).writeString( "foo" )
  inline TaggedFile &tag(int tagVal) {
    writeTagInfo(tagVal);
    return *this;
  }
  ...
  int readTag() {
    if (endOfFile())
      return TAG_END_SECTION;
    else
      return readTagInfo();
  }

  /// read/write data size for tagged file
  inline int readTagInfo() {
    int size = 0;
    CHECK_READ(fread(&size, sizeof(int), 1, m_file));
    return size;
  }
