...
enum TextAnchor { ANCHOR_START, ANCHOR_MIDDLE, ANCHOR_END };
...
public:
  ...
  void startGroup(int r, int g, int b, int rFill, int gFill, int bFill,
                  float lineWidth);
  ...
  void addText(float x, float y, const String &text,
               TextAnchor anchor = ANCHOR_START, float size = 16,
               const String &font = "", float rotate = 0, float xRotate = 0,
               float yRotate = 0, float baselineShift = 0);
  void addArc(float x, float y, float radius, float startDegrees,
              float endDegrees);
...
private:
  ...
  // the currently open file; appends to this file as shapes/etc. added to SVG
  // object
  File *m_file;
