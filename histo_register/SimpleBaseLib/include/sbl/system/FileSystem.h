...
/// obtain a list of files in the given path that match the given prefix and
/// suffix; (both prefix and suffix are allowed to be blank (matches any name))
Array<String> dirFileList(const String &path, const String &prefix,
                          const String &suffix);
...
/// create a directory (assumes all directories exist up to the one being
/// created)
void createDir(const String &path);

/// maintain a fixed number of files with the given extension and path by
/// removing the oldest matching files
void removeOldestFiles(const String &path, const String &extension,
                       int keepCount);
