...
/*! \file MatrixUtil.h
    \brief The MatrixUtil module provides various functions for creating,
   manipulating, and analyzing matrices (represented by Matrix class instances).
*/
...
/// compute eigenvectors and eigenvalues of symmetric matrix; returns
...
/// create a new matrix containing each row for which rowMask is non-zero and
/// each column for which colMask is non-zero
aptr<MatrixF> subset(const MatrixF &m, const VectorI &rowMask,
                     const VectorI &colMask);
