...
public:
  ...
  inline T data(int i, int j) const {
    assertDebug(i >= 0 && i < m_rows && j >= 0 && j < m_cols);
    return m_data[i][j];
  }
  ...
  inline int memUsed() const {
    return sizeof(Matrix<T>) + sizeof(float) * m_rows * m_cols;
  }
...
private:
  ...
  /// if true, matrix is allocated as a single block of memory; otherwise
  /// allocated per row
  bool m_contiguous;
...
template <typename T> T Matrix<T>::min() const {
  ...
  for (int i = 0; i < m_rows;
       i++) { // fix(faster): could make faster if contiguous
    for (int j = 0; j < m_cols; j++) {
      T val = m_data[i][j];
      if (val < min)
        min = val;
