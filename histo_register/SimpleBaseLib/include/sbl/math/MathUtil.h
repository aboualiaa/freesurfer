...
/*! \file MathUtil.h
    \brief The MathUtil module provides a set of basic numeric utility
   functions.
*/
...
inline float bound(float v, float min, float max) {
  if (v < min)
    v = min;
  if (v > max)
    v = max;
...
inline double bound(double v, double min, double max) {
  if (v < min)
    v = min;
  if (v > max)
    v = max;
...
inline float gauss(float distSqd, float gaussFactor) {
  return expf(distSqd * gaussFactor);
}
inline double gauss(double distSqd, double gaussFactor) {
  return exp(distSqd * gaussFactor);
}
