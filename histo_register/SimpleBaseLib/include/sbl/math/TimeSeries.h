...
/// The TimeSeries class represents a sequence of values, each associated with a
/// timestamp. The timestamps may be irregularly spaced.  We assume appended
/// items have monotonically increasing timestamps.
class TimeSeries {
public:
  // basic constructors
  TimeSeries() {}
  ...
  /// interpolate the time series to compute a value at the given timestamp (we
  /// assume timestamps increase monotonically)
  double interpolate(double timestamp) const;

  /// add an item to the series (we assume timestamps increase monotonically)
  inline void append(double timestamp, double value) {
