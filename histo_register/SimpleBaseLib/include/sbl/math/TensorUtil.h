...
/// compute sum of all elements within radius of tensor element indexed by ind
template <typename T>
float elementSum(const Tensor<T> &tensor, const int *ind, int radius) {
...
template <typename T>
void applyBlur(int *ind, int level, const T &source, T &dest, int blurRadius) {
...
/// compute negative entropy of probability distribution represented with a
/// tensor histogram
...
/// returns sum of all values for with the given index for the given level
template <typename T>
double marginalSum(const Tensor<T> &tensor, int level, int index) {
  double sum = 0;
  if (level == 0) {
