...
/// The Objective class provides an interface to an objective function to be
/// optimized.
class Objective {
public:
...
public:
  ...
  inline void setBounds(const VectorD &lBound, const VectorD &uBound) {
    m_lBound = lBound;
    m_uBound = uBound;
  }

  /// set scale factor for penalty applied to points outside bounds
  inline void setPenaltyFactor(double penaltyFactor) {
    m_penaltyFactor = penaltyFactor;
  }

  /// set termination tolerance (relative difference between objective values)
  inline void setFinalTolerance(double finalTolerance) {
    m_finalTolerance = finalTolerance;
  }

  /// run the optimization until termination (or user cancel); returns best
  /// point found
  virtual VectorD run(double *finalObjValue = 0) = 0;

  /// repeatedly run the optimization until termination (or user cancel);
  ...
  /// plot sequence of best objective values (values that were the best at the
  /// time they were evaluated)
  void plotBestHistory(const String &fileName) const;

  /// plot all 1D slices of the objective function, along with evaluation
  /// history
  void plotObjectiveSlices(const VectorD &center, int pointsPerDim,
                           const String &filePrefix) const;

  /// plot a 1D slice of the objective function, along with evaluation history
  void plotObjectiveSlice(const VectorD &center, int pointsPerDim, int dim,
                          const String &fileName) const;

  /// plot a 2D map of the objective function, along with evaluation history
  void plotObjectiveMap(const VectorD &center, int dim1, int dim2, int points1,
                        int points2, int scaleFactor,
                        const String &fileName) const;
...
protected:
  /// evaluate objective function at given point, applying a penality to
...
/// The SimplexOptimizer class performs a downhill simplex optimization (Nelder
/// and Mead, 1965)
...
public:
  ...
  /// set the fracion of the search space (defined by bounds) to use for the
  /// initial simplex
...
private:
  ...
  // the fracion of the search space (defined by bounds) to use for the initial
  // simplex
  double m_startFrac;
  ...
  // the objective funciton value associated with each point in the current
  // simplex
  VectorD m_values;
