...
/// The representation is recursive, facilitating recursive algorithms to
/// operate on the data. The Tensor1F class is the base of the recursion (a 1D
/// vector of floats).
class Tensor1F {
public:
  ...
  inline const float &elem(const int *ind) const {
    assertDebug(ind[0] >= 0 && ind[0] < m_size);
    return m_data[ind[0]];
  }
  inline float &elem(const int *ind) {
    assertDebug(ind[0] >= 0 && ind[0] < m_size);
    return m_data[ind[0]];
  }
  ...
  inline void operator=(const Tensor1F &t) {
    for (int i = 0; i < m_size; i++)
      m_data[i] = t.m_data[i];
  }
...
/// The representation is recursive, facilitating recursive algorithms to
/// operate on the data.
// fix(clean): make this a sub-class of Tensor1F to reduce code replication?
// (might have large costs of space and/or speed)
...
public:
  ...
  inline const float &elem(const int *ind) const {
    assertDebug(ind[0] >= 0 && ind[0] < m_size);
    return m_data[ind[0]].elem(ind + 1);
  }
  inline float &elem(const int *ind) {
    assertDebug(ind[0] >= 0 && ind[0] < m_size);
    return m_data[ind[0]].elem(ind + 1);
