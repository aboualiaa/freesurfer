...
public:
  ...
  Vector(int length, const T *data) {
    alloc(length);
  ...
  inline void append(const Vector &vector) {
  ...
  /// resets vector length; clobbers any existing data; leaves data
  /// uninitialized
  ...
  inline int memUsed() const {
    return sizeof(Vector) + sizeof(T) * m_allocLength;
  }
