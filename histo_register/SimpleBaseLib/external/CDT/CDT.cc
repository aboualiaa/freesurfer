...
Mesh::Mesh(const Point2d &a, const Point2d &b, const Point2d &c,
           const Point2d &d) {
  Boolean  InCircle(const Point2d &, const Point2d &, const Point2d &,
                    const Point2d &);
...
Boolean InCircle(const Point2d &a, const Point2d &b, const Point2d &c,
                 const Point2d &d)
...
// of e before it was flipped, to propagate the change.
{
  ...
  Edge *f = e->Oprev();
  Edge *g = e->Dnext();
  if (InCircle(*(e->Dest()), *(e->Onext()->Dest()), *(e->Org()),
               *(f->Dest()))) {
    Swap(e);
    FixEdge(f);
    FixEdge(g);
...
void Mesh::Apply(void (*f)(int isconstrained, double x1, double y1, double x2,
                           double y2)) const {
  QuadEdge *qp;
  for (LlistPos p = edges.first(); !edges.isEnd(p); p = edges.next(p)) {
    qp = (QuadEdge *)edges.retrieve(p);
    f(qp->isConstrained(), qp->edges()[0].Org2d().x, qp->edges()[0].Org2d().y,
...
void Mesh::ApplyTaggedMesh(void (*f)(int isconstrained, void *tag1,
                                     void *tag2)) const {
  QuadEdge *qp;
  for (LlistPos p = edges.first(); !edges.isEnd(p); p = edges.next(p)) {
    qp = (QuadEdge *)edges.retrieve(p);
    f(qp->isConstrained(), qp->edges()[0].Org2d().tag,
      qp->edges()[2].Org2d().tag);
  }
}
...
void Mesh::ApplyTaggedKey(void *key1, void *key2,
                          ...
                          void (*f)(int isconstrained, void *key1, void *key2,
                                    void *tag1, void *tag2)) const {
  QuadEdge *qp;
  for (LlistPos p = edges.first(); !edges.isEnd(p); p = edges.next(p)) {
    qp = (QuadEdge *)edges.retrieve(p);
    // do tags match?
    if (match(key1, qp->edges()[0].Org2d().tag) &&
        match(key2, qp->edges()[2].Org2d().tag))
      f(qp->isConstrained(), key1, key2, qp->edges()[0].Org2d().tag,
        qp->edges()[2].Org2d().tag);
    else if (match(key2, qp->edges()[0].Org2d().tag) &&
             match(key1, qp->edges()[2].Org2d().tag))
      f(qp->isConstrained(), key2, key1, qp->edges()[0].Org2d().tag,
        qp->edges()[2].Org2d().tag);
  }
}
...
extern "C" void *init_mesh_quadrilateral(float xll, float yll, float xlr,
                                         float ylr, float xur, float yur,
                                         float xul, float yul) {
  Mesh *mesh = new Mesh(Point2d(xll, yll), Point2d(xlr, ylr), Point2d(xur, yur),
                        Point2d(xul, yul));
...
// assumes that given 4 points form bounding convex quadrilateral
extern "C" void *
init_mesh_tagged_quadrilateral(float xll, float yll, void *tagll, float xlr,
                               float ylr, void *taglr, float xur, float yur,
                               void *tagur, float xul, float yul, void *tagul) {
  Mesh *mesh = new Mesh(Point2d(xll, yll, tagll), Point2d(xlr, ylr, taglr),
                        Point2d(xur, yur, tagur), Point2d(xul, yul, tagul));

  return (void *)mesh;
}

// insert a delaunay site
extern "C" void insert_mesh_site(Mesh *mesh, float x, float y) {
  mesh->InsertSite(Point2d(x, y));
}

// insert a constraint edge
extern "C" void insert_mesh_edge(Mesh *mesh, float x1, float y1, float x2,
                                 float y2) {
  mesh->InsertEdge(Point2d(x1, y1), Point2d(x2, y2));
}

// insert a tagged delaunay site
extern "C" void insert_tagged_mesh_site(Mesh *mesh, float x, float y,
                                        void *tag) {
  mesh->InsertSite(Point2d(x, y, tag));
}

// insert a constraint edge
extern "C" void insert_tagged_mesh_edge(Mesh *mesh, float x1, float y1,
                                        void *tag1, float x2, float y2,
                                        void *tag2) {
  mesh->InsertEdge(Point2d(x1, y1, tag1), Point2d(x2, y2, tag2));
}

extern "C" void draw_mesh(Mesh *mesh) { mesh->Draw(); }

extern "C" void apply_mesh(Mesh *mesh,
                           void (*f)(int isconstrained, double x1, double y1,
                                     double x2, double y2)) {
  mesh->Apply(f);
}

extern "C" void apply_tagged_mesh(Mesh *mesh,
                                  void (*f)(int isconstrained, void *tag1,
                                            void *tag2)) {
  mesh->ApplyTaggedMesh(f);
}

extern "C" void apply_tagged_key(Mesh *mesh, void *key1, void *key2,
                                 int (*match)(void *key, void *tag),
                                 void (*f)(int isconstrained, void *key1,
                                           void *key2, void *tag1,
                                           void *tag2)) {
...
extern "C" int tagged_edge_unmatched(Mesh *mesh, void *key1, void *key2,
                                     int (*match)(void *key, void *tag)) {
  return (mesh->TaggedEdgeUnmatched(key1, key2, match));
}

extern "C" void delete_mesh(Mesh *mesh) { delete mesh; }

#if STANDALONE
// only for testing vortest.c
// assumes that first 4 points form convex bounding quadrilateral
extern "C" void *build_dt_mesh(float data[][2], int numdata) {
  Mesh *mesh = new Mesh(
      Point2d(data[0][0], data[0][1]), Point2d(data[1][0], data[1][1]),
      Point2d(data[2][0], data[2][1]), Point2d(data[3][0], data[3][1]));

  for (int i = 4; i < numdata; ++i)
    mesh->InsertSite(Point2d(data[i][0], data[i][1]));

  mesh->InsertEdge(Point2d(data[4][0], data[4][1]),
                   Point2d(data[5][0], data[5][1]));
