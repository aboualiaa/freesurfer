...
public:
  ...
  // access position
  inline float x(int frameIndex) const {
    return m_x[frameIndex - m_startFrame];
  };
  inline float y(int frameIndex) const {
    return m_y[frameIndex - m_startFrame];
  };
  ...
  inline bool active(int startFrame, int endFrame) const {
    return startFrame >= m_startFrame && endFrame <= m_endFrame;
  }

  // get/set particle channels
  inline float channel(int frameIndex, int channelIndex) const {
    return m_channel[channelIndex][frameIndex - m_startFrame];
  }
  ...
  inline const VectorF &clusterSpacePosition() const {
    return m_clusterSpacePosition;
  }

  /// set the position of the particle in (an abstract) cluster space
  inline void setClusterSpacePosition(const VectorF &clusterSpacePosition) {
    m_clusterSpacePosition = clusterSpacePosition;
  }
...
private:
  ...
  // particle appearance in each frame (observed channel values for the
  // particle)
...
/// The SimpleParticleCluster class represents cluster membership within a
/// SimpleParticleSet.
...
enum SimpleParticleTimerType { TIMER_OPT, TIMER_SOLVER, TIMER_COUNT };

/// The SimpleParticleSet class represents a collection of particle
/// trajectories.
...
public:
  ...
  inline const SimpleParticle &ref(int index) const {
  ...
  /// access clusters
  inline SimpleParticleCluster &clusterRef(int index) {
    return m_simpleParticleClusterSet[index];
  };
  inline const SimpleParticleCluster &clusterRef(int index) const {
    return m_simpleParticleClusterSet[index];
  };
  inline int clusterCount() const {
    return m_simpleParticleClusterSet.count();
  };
  void clearClusters();
  ...
  /// add a particle cluster (takes ownership of pointer; do not deallocate
  /// outside)
  inline void appendCluster(SimpleParticleCluster *pc) {
    m_simpleParticleClusterSet.append(pc);
  }
