...
void estimateOpticalFlow(int argc, char *argv[]) {
  ...
  String outputVideoFileName =
      inputVideoFileName.leftOfLast('.') + "_mfVis.avi";
  OutputVideo outputVideo(outputVideoFileName, inputVideo.width(),
                          inputVideo.height(), 6);
  ...
  for (int frameIndex = 0; frameIndex < frameCount; frameIndex++) {
    ...
    if (frame.get() && lastFrame.get()) {
      aptr<MotionField> mf =
          varMotion(*lastFrame, *frame, varMotionConfFileName, NULL, NULL);

      // save estimated motion field
      String outputFileName = outputPath + sprintF("/motion.%05d.%05d.mf",
      ...
      if (saveText) {
        String textOutputFileName =
            outputPath +
            sprintF("/motion.%05d.%05d.txt", frameIndex - 1, frameIndex);
...
void buildParticleVideo(int argc, char *argv[]) {
  ...
  if (spConf.entryCount() == 0) {
    warning("unable to load simple particle config: %s",
            spConfFileName.c_str());
    return;
  }
  ...
  // open output video
  String outputVideoFileName =
      inputVideoFileName.leftOfLast('.') + "_spVis.avi";
  OutputVideo outputVideo(outputVideoFileName, inputVideo.width(),
                          inputVideo.height(), 6);
  if (outputVideo.openSuccess() == false) {
    warning("unable to open output video: %s", outputVideoFileName.c_str());
  ...
  for (int frameIndex = 0; frameIndex < frameCount; frameIndex++) {
    ...
    // load motion fields
    String motionFileName = motionPath + sprintF("/motion.%05d.%05d.mf",
    ...
    motionFileName           = motionPath + sprintF("/motion.%05d.%05d.mf",
                                          frameIndex - 1, frameIndex);
    aptr<MotionField> mfPrev = loadMotionField(motionFileName);

    // update particles
    if (frame.get()) {
      simpleParticleFrame(particleSet, frameIndex, *frame, mfPrev.get(), *mf,
                          spConf);
  ...
  if (doClustering) {
    buildParticleClusters(particleSet, inputVideo.width(), inputVideo.height(),
                          spConf, true);
  }
