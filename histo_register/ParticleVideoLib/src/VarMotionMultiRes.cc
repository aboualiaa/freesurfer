...
void varMultiResSolveSystem(
    const ImageGrayU &mask, int pixelCount, Config &varConf,
    const ImageGrayF &localSmoothness, const ImageGrayI &uIndex,
    const ImageGrayI &vIndex, const ImageGrayF &uDx, const ImageGrayF &uDy,
    const ImageGrayF &vDx, const ImageGrayF &vDy, const Array<ImageGrayF> &Ix,
        ...
        for (int yo = 0; yo < 2; yo++) {
          for (int xo = 0; xo < 2; xo++) {
            for (int k = 0; k < Ix.count(); k++) {
              ...
              float dataFactor =
                  dataFactorImg[k].data(xLarger + xo, yLarger + yo);

              // compute data coefs
              duCoef1 += dataFactor * ix * ix;
              dvCoef1 += dataFactor * ix * iy;
              b1 += -dataFactor * ix *
                    iz; // negate because subtract from both sides
...
void varMultiResInnerLoop(const ImageGrayU &maskMotion, int pixelCount,
                          Config &varConf, const ImageGrayF &localSmoothness,
                          ...
                          const Array<ImageGrayF> &Ix,
                          const Array<ImageGrayF> &Iy,
                          const Array<ImageGrayF> &Iz, const ImageGrayF &u,
                          const ImageGrayF &v, ImageGrayF &du, ImageGrayF &dv) {
  ...
  Array<ImageGrayF> dPsiData =
      allocChannels(chanWidth, chanHeight, chanCount, 0);
  ...
  for (int innerIter = 0; innerIter < innerIters; innerIter++) {
    ...
    // solve the system
    varMultiResSolveSystem(maskMotion, pixelCount, varConf, localSmoothness,
                           uIndex, vIndex, uDx, uDy, vDx, vDy, Ix, Iy, Iz,
...
void varMultiResIteration(MotionField &            mf,
  ...
  int chanCount = srcChanScaled.count(), chanWidth = srcChanScaled[0].width(),
      chanHeight = srcChanScaled[0].height();
  ...
  for (int y = 0; y < chanHeight; y++) {
    for (int x = 0; x < chanWidth; x++) {
      ...
      if (maskMotion.data(xSmaller, ySmaller) > 150 &&
          mf.occSafe(xSmaller, ySmaller) < 128) {
        ...
        if (xProj > 0 && xProj < chanWidth - 1 && yProj > 0 &&
            yProj < chanHeight - 1) {
          for (int k = 0; k < chanCount; k++) {
  ...
  varMultiResInnerLoop(maskMotion, pixelCount, varConf, *lSmoothness, *uIndex,
                       *vIndex, *uDx, *uDy, *vDx, *vDy, Ix, Iy, Iz, u, v, du,
                       dv);
