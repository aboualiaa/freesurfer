...
void dualFilterMotion(MotionField &mf, const ImageGrayF &src,
                      const ImageGrayU &filtMask, float distSigma,
                      float flowSigma, float imageSigma, float minWeight) {
...
void gaussianOcclusionMask(MotionField &mf, const ImageGrayF &src,
                           const ImageGrayF &dest,
                           const ImageGrayU &occUpdateRegion,
                           const ImageGrayF &div, Config &varConf,
                           bool visualize) {
...
void identifyOcclusions(MotionField &mf, const ImageGrayF &src,
                        const ImageGrayF &dest, Config &varConf,
                        ImageGrayU &filtMask, bool visualize) {
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++) {
      ...
      gradMagSqd.data(x, y) =
          uDxVal * uDxVal + uDyVal * uDyVal + vDxVal * vDxVal + vDyVal * vDyVal;
  ...
  gaussianOcclusionMask(mf, src, dest, *occUpdateRegionBlur, divergence,
                        varConf, visualize);
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++)
      filtMask.data(x, y) =
          filtRegionBlur->data(x, y); // fix(clean): use aptr to avoid copy
}
...
void filterMotionStep(MotionField &mf, MotionField *mfRev,
                      const ImageGrayF &src, const ImageGrayF &dest,
                      Config &varConf) {
  ...
  // run filtering
  dualFilterMotion(mf, src, filtMask, distSigma, flowSigma, imageSigma,
                   minWeight);
