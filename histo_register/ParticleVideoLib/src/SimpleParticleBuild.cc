...
int particleSeparation(float grad, float maxFlowGradient, int minSeparation,
                       int maxSeparation) {
  if (grad > maxFlowGradient)
    grad = maxFlowGradient;
  return minSeparation + round((float)(maxSeparation - minSeparation) *
                               (1.0f - grad / maxFlowGradient));
}

/// add new particles in gaps between existing particles
void simpleParticleAdd(SimpleParticleSet &particleSet, int frameIndex,
                       const Array<ImageGrayF> &chan, const MotionField &mf,
                       Config &spConf) {
  ...
  for (int i = 0; i < particleSet.count(); i++) {
    ...
    if (p.active(frameIndex)) {
      ...
      if (gradBlur->inBounds(x, y)) {
        int sep = particleSeparation(gradBlur->data(x, y), maxFlowGradient,
...
void simpleParticlePrune(SimpleParticleSet &particleSet, int frameIndex,
                         Config &spConf) {
  ...
  for (int i = 0; i < particleSet.count(); i++) {
    ...
    if (p.active(frameIndex - 1, frameIndex)) {
      ...
      for (int channelIndex = 0; channelIndex < count; channelIndex++) {
        float diff = p.channel(frameIndex, channelIndex) -
                     p.channelBlur(frameIndex, channelIndex);
        if (diff < 0)
          diff = -diff;
        sumDiff += diff;
      }
...
void simpleParticleUpdate(SimpleParticleSet &particleSet, int frameIndex,
  ...
  VectorF channelBlurKernel =
      gaussKernel(round(channelTimeSigma * 3), channelTimeSigma);
...
void simpleParticlePropagate(SimpleParticleSet &particleSet, int frameIndex,
                             const MotionField &mfPrev, Config &spConf) {
  ...
  for (int i = 0; i < particleSet.count(); i++) {
    ...
    if (p.active(frameIndex - 1)) {
      ...
      if (mfPrev.inBounds(xPrevRound, yPrevRound)) {
        if (mfPrev.occ(xPrevRound, yPrevRound) > 128 ||
            divBlur->data(xPrevRound, yPrevRound) < divThresh) {
          occCount++;
        } else {
          ...
          if (xNew > frameBorder && xNew < width - frameBorder &&
              yNew > frameBorder && yNew < height - frameBorder) {
            p.extend();
  ...
  disp(2, "propagated particles: %d (%d occluded, %d out of bounds)", propCount,
       occCount, boundCount);
}
...
void simpleParticleFrame(SimpleParticleSet &particleSet, int frameIndex,
                         const ImageColorU &frame, const MotionField *mfPrev,
                         const MotionField &mf, Config &spConf) {

  // check frame index
  if (frameIndex != particleSet.maxEndFrame() + 1) {
    warning("SimpleParticleFrame: invalid frame index: %d (expected next "
            "consecutive frame: %d)",
            frameIndex, particleSet.maxEndFrame() + 1);
    return;
  }
...
float particleDistanceSqd(const SimpleParticle &p1, const SimpleParticle &p2,
                          float maxDistSqd, float spaceSigmaSqd,
                          float chanSigmaSqd, float motionSigmaSqd) {
  ...
  for (int frameIndex = commonStartFrame; frameIndex <= commonEndFrame;
       frameIndex++) {
    ...
    for (int channelIndex = 0; channelIndex < channelCount; channelIndex++) {
      float chanDiff = p1.channel(frameIndex, channelIndex) -
                       p2.channel(frameIndex, channelIndex);
  ...
  int   maxLength   = max(p1.endFrame() - p1.startFrame() + 1,
                      p2.endFrame() - p2.startFrame() + 1);
  ...
  return (spaceDistSqd + chanDistSqd + motionDistSqd * overlapFrac) /
         (2 + overlapFrac);
}

/// display affinity matrix values along particle links
void visualizeParticleAffinity(const SimpleParticleSet &particleSet,
  ...
  for (int i = 0; i < links->rows(); i++) {
    ...
    if (p1.active(frameIndex) && p2.active(frameIndex)) {
      ...
      drawLine(vis, round(p1.x(frameIndex)), round(p1.y(frameIndex)),
               round(p2.x(frameIndex)), round(p2.y(frameIndex)), r, g, b, true);
    }
  }
  dispImage(vis);
}
...
void visualizeParticleClusters(const SimpleParticleSet &particleSet,
                               int frameIndex, int width, int height) {
  ImageColorU vis(width, height);
  vis.clear(255, 255, 255);
  ...
  for (int k = 0; k < particleSet.clusterCount(); k++) {
    ...
    for (int i = 0; i < pc.memberCount(); i++) {
      ...
      if (p.active(frameIndex))
        drawCircleFilled(vis, round(p.x(frameIndex)), round(p.y(frameIndex)), 4,
                         r, g, b);
    }
  }
  dispImage(vis);
...
void buildParticleClusters(SimpleParticleSet &particleSet, int width,
                           int height, Config &spConf, bool visualize) {
  ...
  if (particleCount == 0)
    return;
  disp(1, "computing %d by %d distance matrix...", particleCount,
       particleCount);
  checkCommandEvents();
  ...
  for (int i = 0; i < particleCount; i++) {
    ...
    for (int j = i + 1; j < particleCount; j++) {
      ...
      float                 distSqd =
          particleDistanceSqd(p1, p2, maxParticleDistSqd, spaceSigmaSqd,
                              chanSigmaSqd, motionSigmaSqd);
  ...
  disp(2, "filled %f entries per particle",
       (float)fillCount / (float)particleCount);

  // compute space for clustering
  disp(1, "running spectral transform to find %d-dimensional space...",
       clusterCount);
  checkCommandEvents();
