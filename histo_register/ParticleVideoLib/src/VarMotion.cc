...
void varSolveSystem(const ImageGrayU &mask, int pixelCount, Config &varConf,
                    const ImageGrayF &localSmoothness, const ImageGrayI &uIndex,
                    const ImageGrayI &vIndex, const ImageGrayF &uDx,
                    const ImageGrayF &uDy, const ImageGrayF &vDx,
                    const ImageGrayF &vDy, const Array<ImageGrayF> &Ix,
                    const Array<ImageGrayF> &Iy, const Array<ImageGrayF> &Iz,
                    const Array<ImageGrayF> &dataFactorImg,
                    const ImageGrayF &dPsiSmoothness, ImageGrayF &du,
                    ImageGrayF &dv, bool fast) {
    ...
    for (int x = 0; x < width; x++) {
      if (mask.data(x, y)) {
        ...
        for (int k = 0; k < Ix.count(); k++) {
          ...
          b1 +=
              -dataFactor * ix * iz; // negate because subtract from both sides
          duCoef2 += dataFactor * iy * ix;
          dvCoef2 += dataFactor * iy * iy;
          b2 +=
              -dataFactor * iy * iz; // negate because subtract from both sides
        }
        ...
        if (x < width - 1)
          b1 += uDx.data(x + 1, y) * smoothFactorPx;
        if (y < height - 1)
          b1 += uDy.data(x, y + 1) * smoothFactorPy;
        ...
        if (x < width - 1)
          b2 += vDx.data(x + 1, y) * smoothFactorPx;
        if (y < height - 1)
          b2 += vDy.data(x, y + 1) * smoothFactorPy;
        ...
        if (x == 0) {
          ...
          sparseSystem.addEquation(uInd, vInd, uMxInd, uPxInd, uMyInd, uPyInd,
                                   duCoef1, dvCoef1, duMxCoef, duPxCoef,
                                   duMyCoef, duPyCoef, b1);
          sparseSystem.addEquation(vInd, uInd, vMxInd, vPxInd, vMyInd, vPyInd,
                                   dvCoef2, duCoef2, dvMxCoef, dvPxCoef,
                                   dvMyCoef, dvPyCoef, b2);
        }
      }
    }
  }
...
void varInnerLoop(const ImageGrayU &mask, int pixelCount, Config &varConf,
                  const ImageGrayF &localSmoothness, const ImageGrayI &uIndex,
                  const ImageGrayI &vIndex, const ImageGrayF &uDx,
                  const ImageGrayF &uDy, const ImageGrayF &vDx,
                  const ImageGrayF &vDy, const Array<ImageGrayF> &Ix,
                  const Array<ImageGrayF> &Iy, const Array<ImageGrayF> &Iz,
                  const ImageGrayF &u, const ImageGrayF &v, ImageGrayF &du,
                  ImageGrayF &dv, bool fast) {
  ...
  for (int innerIter = 0; innerIter < innerIters; innerIter++) {
        ...
        if (mask.data(x, y)) {
          ...
          for (int k = 0; k < chanCount; k++) {
            float diff = Ix[k].data(x, y) * du.data(x, y) +
                         Iy[k].data(x, y) * dv.data(x, y) + Iz[k].data(x, y);
          ...
          dPsiSmoothness.data(x, y) =
              broxDistDeriv(uNewDx * uNewDx + uNewDy * uNewDy +
                            vNewDx * vNewDx + vNewDy * vNewDy);
        }
      }
    }
    ...
    varSolveSystem(mask, pixelCount, varConf, localSmoothness, uIndex, vIndex,
                   uDx, uDy, vDx, vDy, Ix, Iy, Iz, dPsiData, dPsiSmoothness, du,
                   dv, fast);
  }
}

/// computes an update (du, dv) to the flow field
void varIteration(MotionField &mf, const Array<ImageGrayF> &srcChanScaled,
  ...
  int chanCount = srcChanScaled.count(), width = mf.width(),
      height = mf.height();
  ...
  aptr<ImageGrayF> lSmoothness =
      localSmoothness(srcChanScaled[0], flatSigma, flatSmoothness);
      ...
      if (maskMotion.data(x, y) && mf.occSafe(x, y) < 128) {
        ...
        if (xProj > 0 && xProj < width - 1 && yProj > 0 && yProj < height - 1) {
          for (int k = 0; k < chanCount; k++) {
            ...
            Iz[k].data(x, y) = destChanScaled[k].interp(xProj, yProj) -
                               srcChanScaled[k].data(x, y);
          }
        }
      }
    }
...
aptr<MotionField> varScaleLoop(const Array<ImageGrayF> &srcChan,
                               const Array<ImageGrayF> &destChan,
                               const MotionField *init, const ImageGrayU &mask,
                               Config &varConf) {
  ...
  int   verbosity   = varConf.readInt("verbosity");
  bool  visualizeIntermediateFlow =
      varConf.readBool("visualizeIntermediateFlow");
  ...
  // compute scales
  VectorF scales =
      computeScales(scaleFactor, minScale * (float)multiScaleFactor);
  if (finalPass) {
  ...
  for (int i = scaleCount - 1; i >= 0; i--) {
    ...
    int mfWidth =
        sbl::round(srcChan.ref(0).width() * scale / (float)multiScaleFactor);
    int mfHeight =
        sbl::round(srcChan.ref(0).height() * scale / (float)multiScaleFactor);
    int chanWidth  = mfWidth * multiScaleFactor;
    int chanHeight = mfHeight * multiScaleFactor;
    if (verbosity >= 4)
      disp(2, "scale %f: motion: %d by %d, image: %d by %d", scale, mfWidth,
           mfHeight, chanWidth, chanHeight);
    ...
    Array<ImageGrayF> srcChanScaled =
        shrinkChannels(srcChan, chanWidth, chanHeight, sigma);
    Array<ImageGrayF> destChanScaled =
        shrinkChannels(destChan, chanWidth, chanHeight, sigma);
    ...
    if (chanWidth != mfWidth) {
      srcChanMotion = shrinkChannels(
          srcChanScaled, mfWidth, mfHeight,
          sigma); // fix(faster): don't need to resize all channels, just first
      destChanMotion = shrinkChannels(
          destChanScaled, mfWidth, mfHeight,
          sigma); // fix(faster): don't need to resize all channels, just first
    ...
    if (enableMultiScale)
      varMultiResIteration(*mf, srcChanScaled, destChanScaled, *maskScaled,
                           varConf);
    if (enableNormal)
      varIteration(*mf, srcChanScaled, destChanScaled, *maskScaled, varConf,
                   fast);
...
aptr<MotionField> varMotion(const ImageColorU &src, const ImageColorU &dest,
  ...
  // loop over scales
  aptr<MotionField> mf =
      varScaleLoop(srcChan, destChan, init, *maskSmall, varConf);
...
aptr<MotionField> varMotion(const ImageGrayU &src, const ImageGrayU &dest,
  ...
  return varMotion(*srcColor, *destColor, configFileName, init, mask,
                   scaleFactor);
}
