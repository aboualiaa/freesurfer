...
void SparseSystem::addEquation(int i1, int i2, int i3, int i4, int i5, int i6,
                               int i7, int i8, int i9, float a1, float a2,
                               float a3, float a4, float a5, float a6, float a7,
                               float a8, float a9, float b) {
  if (m_fixedEqn9 == NULL)
...
/// add eqn of form: a_i x_i + a_j x_j = b
void SparseSystem::addEquation(int i1, int i2, int i3, int i4, int i5, float a1,
...
void SparseSystem::checkEquations() {
  ...
  if (m_fixedEqn6) {
    ...
    for (int i = 0; i < m_eqnCount; i++) {
      if (m_fixedEqn6[i].i1 != i) {
        fatalError("first var is not on diagonal (%d, %d)", m_fixedEqn6[i].i1,
                   i);
        break;
      }
  ...
  if (m_fixedEqn9) {
    ...
    for (int i = 0; i < m_eqnCount; i++) {
      if (m_fixedEqn9[i].i1 != i) {
        fatalError("first var is not on diagonal (%d, %d)", m_fixedEqn9[i].i1,
                   i);
        break;
      }
  ...
  if (m_varEqn) {
    ...
    if (m_indent)
      disp(m_indent,
           "var-length system: eqns: %d, min terms: %d, max terms: %d, min "
           "coef: %f, max coef: %f, min inv: %f, max inv: %f",
           m_eqnCount, minTermCount, maxTermCount, minCoef, maxCoef, minInv,
           maxInv);
  }
}
...
double SparseSystem::residual(const VectorF &xVect) {
  ...
  if (m_fixedEqn5) {
    for (int i = 0; i < m_eqnCount; i++) {
      ...
      double          axSum = eqn->a1 * x[eqn->i1] + eqn->a2 * x[eqn->i2] +
                     eqn->a3 * x[eqn->i3] + eqn->a4 * x[eqn->i4] +
                     eqn->a5 * x[eqn->i5];
  ...
  if (m_fixedEqn6) {
    for (int i = 0; i < m_eqnCount; i++) {
      ...
      double          axSum = eqn->a1 * x[eqn->i1] + eqn->a2 * x[eqn->i2] +
                     eqn->a3 * x[eqn->i3] + eqn->a4 * x[eqn->i4] +
                     eqn->a5 * x[eqn->i5] + eqn->a6 * x[eqn->i6];
      double diff = axSum - m_b[i];
      sumSqDiff += diff * diff;
    }
  }
  ...
  if (m_fixedEqn9) {
    for (int i = 0; i < m_eqnCount; i++) {
      ...
      double          axSum =
          eqn->a1 * x[eqn->i1] + eqn->a2 * x[eqn->i2] + eqn->a3 * x[eqn->i3] +
          eqn->a4 * x[eqn->i4] + eqn->a5 * x[eqn->i5] + eqn->a6 * x[eqn->i6] +
          eqn->a7 * x[eqn->i7] + eqn->a8 * x[eqn->i8] + eqn->a9 * x[eqn->i9];
      double diff = axSum - m_b[i];
      sumSqDiff += diff * diff;
    }
  }
  ...
  if (m_fixedEqn5) {
    ...
    for (int iter = 0; iter < m_maxIter; iter++) {
      ...
      for (int i = 0; i < cols; i++) {
        ...
        float ax = eqn->a2 * x[eqn->i2] + eqn->a3 * x[eqn->i3] +
                   eqn->a4 * x[eqn->i4] + eqn->a5 * x[eqn->i5];
    ...
    for (int iter = 0; iter < m_maxIter; iter++) {
      ...
      if (m_adaptIterMask) {
        ...
        if ((iter & m_adaptIterMask) == m_adaptIterMask) {
          for (int i = 0; i < cols; i++) {
            xActive[i] = xUpdate[i] > m_adaptUpdateThresh ||
                         xUpdate[i] < -m_adaptUpdateThresh;
          }
        }
      ...
      if (m_adaptIterMask) {
        ...
        for (int i = 0; i < cols; i++) {
          if (xActive[i]) {
            ...
            float ax = eqn->a2 * x[eqn->i2] + eqn->a3 * x[eqn->i3] +
                       eqn->a4 * x[eqn->i4] + eqn->a5 * x[eqn->i5] +
                       eqn->a6 * x[eqn->i6] + eqn->a7 * x[eqn->i7] +
                       eqn->a8 * x[eqn->i8] + eqn->a9 * x[eqn->i9];
            ax = (m_b[i] - ax) * eqn->a1Inv;
    ...
    if (m_indent)
      disp(m_indent,
           "solved SOR system: eqns: %d, iters: %d, factor: %f, resid: %lf",
...
VectorF SparseSystem::solve() { return solveSOR(); }
