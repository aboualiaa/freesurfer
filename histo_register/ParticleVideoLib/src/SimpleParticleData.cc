...
void SimpleParticle::setChannels(int frameIndex, const Array<ImageGrayF> &chan,
  ...
  if (channelBlurKernel) {
    ...
    for (int i = 0; i < m_channel.count(); i++) {
      ...
      for (int j = -tableRadius; j <= tableRadius; j++) {
        int srcInd =
            frameIndex + j - m_startFrame; // index into channel vectors
...
/// basic constructor
SimpleParticleCluster::SimpleParticleCluster(const VectorF &mean,
                                             const VectorI &particleIndex) {
  m_mean          = mean;
...
void SimpleParticleSet::clearClusters() { m_simpleParticleClusterSet.reset(); }
  ...
  if (file.openSuccess()) {
    ...
    for (int i = 0; i < particleCount; i++) {
      SimpleParticle *sp = new SimpleParticle(
          0, 0, 0, 0); // fix(clean): create simple constructor
      if (sp->load(file)) {
...
aptr<ImageColorU> SimpleParticleSet::draw(int                frameIndex,
                                          const ImageColorU &frame) {
  ...
  for (int i = 0; i < m_simpleParticleSet.count(); i++) {
    ...
    if (particle.active(frameIndex)) {
      drawCircleFilled(*vis, round(particle.x(frameIndex)),
                       round(particle.y(frameIndex)), 2, 255, 0, 0);
    }
  }
  return vis;
}
