...
aptr<MatrixI> createLinks(const SimpleParticleSet &particleSet, int frameIndex,
                          bool activePrev) {
...
void plotLinks(const SimpleParticleSet &particleSet, const MatrixI &links,
  ...
  for (int i = 0; i < links.rows(); i++) {
    ...
    if (index1 == index2)
      disp(1, "self link");
    if (index1 < 0 || index1 >= particleSet.count() || index2 < 0 ||
        index2 >= particleSet.count())
    ...
    drawLine(vis, round(p1.x(frameIndex)), round(p1.y(frameIndex)),
             round(p2.x(frameIndex)), round(p2.y(frameIndex)), 0, 0, 255,
             false);
  }
  ...
  for (int i = 0; i < particleSet.count(); i++) {
    ...
    if (p.active(frameIndex))
      drawCross(vis, round(p.x(frameIndex)), round(p.y(frameIndex)), 2, 255, 0,
                0, true);
  }
...
aptr<VectorI> pointMapInverse(const SimpleParticleSet &particleSet,
                              int                      frameIndex) {
...
void checkPointMap(const SimpleParticleSet &particleSet, int frameIndex,
                   const VectorI &ptMap, const VectorI &ptMapInv) {
  ...
  for (int i = 0; i < ptMap.length(); i++) {
    ...
    if (particleSet.ref(particleIndex).active(frameIndex - 1, frameIndex) ==
        false)
...
void updateParticlePositions(SimpleParticleSet &particleSet, int frameIndex,
                             const VectorI &ptMap, const VectorF &dx,
                             const VectorF &dy, float maxMove) {
...
void addPairEquation(SparseSystem &sparseSystem, int eqnIndex, int varIndex1,
                     int varIndex2, float factor) {
...
void addDataEquation(SparseSystem &sparseSystem, int eqnIndex, float ixix,
                     float ixiy, float iyiy, float ixiz, float iyiz,
                     float factor, float condFactor) {

  sparseSystem.addTerm(eqnIndex + _X_, eqnIndex + _X_,
                       factor * ixix + condFactor);
  sparseSystem.addTerm(eqnIndex + _X_, eqnIndex + _Y_, factor * ixiy);
  sparseSystem.addToB(eqnIndex + _X_,
                      -factor *
                          ixiz); // negate because subtract from both sides

  sparseSystem.addTerm(eqnIndex + _Y_, eqnIndex + _Y_,
                       factor * iyiy + condFactor);
  sparseSystem.addTerm(eqnIndex + _Y_, eqnIndex + _X_, factor * ixiy);
  sparseSystem.addToB(eqnIndex + _Y_,
                      -factor *
                          iyiz); // negate because subtract from both sides
}

/// robust distance function from Brox et al.
float spPsi(float diffSqd) { return sqrtf(diffSqd + 1e-6f); }

/// derivative of robust distance function from Brox et al.
float spPsiDeriv(float diffSqd) { return 0.5f / sqrtf(diffSqd + 1e-6f); }

/// visualize offsets from inner loop
void visualizeOffsets(const SimpleParticleSet &particleSet,
                      const VectorI &ptMap, int frameIndex,
                      const Array<ImageGrayF> &chanScaled, const VectorF &dx,
                      const VectorF &dy) {
  float arrowScale = 10;
  ...
  for (int i = 0; i < ptMap.length(); i++) {
    ...
    drawArrow(*frame, round(x), round(y), round(x + dxVal * arrowScale),
              round(y + dyVal * arrowScale), 255, 0, 0, true);
  }
  ...
  if (count) {
    ...
    disp(3, "%d points: dx: (%f, %f, %f), dy: (%f, %f, %f)", count, dxMin,
         dxMean, dxMax, dyMin, dyMean, dyMax);
  }
...
void solveSystem(SimpleParticleSet &particleSet, int frameIndex, MatrixI &links,
                 ...
                 VectorF &ixix, VectorF &ixiy, VectorF &iyiy, VectorF &ixiz,
                 VectorF &iyiz, VectorF &dxVect, VectorF &dyVect) {
  int pointCount = ptMap.length();
  ...
  if (smoothFactor) {
    for (int linkIndex = 0; linkIndex < linkCount; linkIndex++) {
      ...
      if (partIndex < 0 || partIndex >= ptMapInv.length() ||
          otherPartIndex < 0 || otherPartIndex >= ptMapInv.length())
      ...
      addPairEquation(sparseSystem, varIndex, varIndex, otherVarIndex,
                      smoothFactor);
    }
  }
...
void simpleParticleOptimize(SimpleParticleSet &particleSet, int frameIndex,
  ...
  for (; outerIter < outerIters; outerIter++) {
    ...
    VectorF ixix(pointCount), ixiy(pointCount), iyiy(pointCount),
        ixiz(pointCount), iyiz(pointCount);
    ixix.clear(0);
    ixiy.clear(0);
    iyiy.clear(0);
    ixiz.clear(0);
    ...
    for (int i = 0; i < pointCount; i++) {
      ...
      if (x - boundary > 0 && x + boundary < width && y - boundary > 0 &&
          y + boundary < height && xPrev - boundary > 0 &&
          xPrev + boundary < width && yPrev - boundary > 0 &&
          yPrev + boundary < height) {
        for (int j = 0; j < channelCount; j++) {
          float ix = chanDx[j].interp(x, y);
          float iy = chanDy[j].interp(x, y);
          float iz = chan[j].interp(x, y) - p.channelBlur(frameIndex, j);
          float dPsi =
              1; //_PsiDeriv( iz * iz ); // fix(later): what is correct term here?
          ixixSum += ix * ix * dPsi;
          ixiySum += ix * iy * dPsi;
          iyiySum += iy * iy * dPsi;
          ixizSum += ix * iz * dPsi;
