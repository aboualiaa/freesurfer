...
int buildIndexMaps(const ImageGrayU &mask, aptr<ImageGrayI> &uIndex,
                   aptr<ImageGrayI> &vIndex) {
...
aptr<MotionField> createInitMotion(int width, int height,
...
aptr<ImageGrayF> localSmoothness(const ImageGrayF &src, float flatSigma,
  ...
  if (flatSmoothness) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        flatness->data(x, y) =
            flatSmoothness *
            gauss(srcDxVal * srcDxVal + srcDyVal * srcDyVal, flatGaussFactor);
      }
    }
  } else {
    flatness->clear(0);
...
Array<ImageGrayF> extractChannels(const ImageColorU &img, Config &varConf) {
  ...
  ImageGrayF *grayFloat =
      toFloat(*gray, 1.0f / 255.0f).release(); // scale to [0,1] range
  ...
  if (colorWeight) {
    ...
    for (int y = 0; y < height; y++)
      for (int x = 0; x < width; x++) {
        ...
        gr->data(x, y) =
            (g - imgMedian->r(x, y)) * colorWeight; // green minus red
        gb->data(x, y) =
            (g - imgMedian->b(x, y)) * colorWeight; // green minus blue
      }
    channels.append(gr);
    channels.append(gb);
  }
...
void dispChannels(const Array<ImageGrayF> &chan) {
  if (chan.count() > 0)
    dispChannel(chan[0], "gray");
  if (chan.count() > 1)
    dispChannel(chan[1], "grad-x");
  if (chan.count() > 2)
    dispChannel(chan[2], "grad-y");
  if (chan.count() > 3)
    dispChannel(chan[3], "color-gr");
  if (chan.count() > 4)
    dispChannel(chan[4], "color-gb");
}

/// scale and blur each channel
Array<ImageGrayF> shrinkChannels(const Array<ImageGrayF> &chan, int scaledWidth,
                                 int scaledHeight, float blurSigma) {
...
float broxDist(float diffSqd) { return sqrtf(diffSqd + 1e-6f); }

/// derivative of robust norm w.r.t. squared diff
float broxDistDeriv(float diffSqd) { return 0.5f / sqrtf(diffSqd + 1e-6f); }

/// the variational objective function
float varEnergy(const MotionField &mf, const ImageGrayU &mask,
                const Array<ImageGrayF> &srcChanBlurred,
                const Array<ImageGrayF> &destChanBlurred, Config &varConf,
                bool verbose, float &dataEnergyRet, float &smoothnessEnergyRet,
  ...
  if (dataEnergyMap)
    dataEnergyMap->clear(0);
  if (smoothnessEnergyMap)
    smoothnessEnergyMap->clear(0);

  // get flatness image
  aptr<ImageGrayF> lSmoothness =
      localSmoothness(srcChanBlurred[0], flatSigma, flatSmoothness);
  ...
  for (int y = 1; y < height - 1; y++)
    for (int x = 1; x < width - 1; x++) {
      ...
      if (mask.data(x, y)) {
        ...
        float smoothnessEnergy =
            (smoothness + lSmoothness->data(x, y)) *
            broxDist(uDx * uDx + uDy * uDy + vDx * vDx + vDy * vDy);
        sumSmoothnessEnergy += smoothnessEnergy;
        if (verbose)
          disp(1, "u: %f, v: %f, uDx: %f, uDy: %f, vDx: %f, vDy: %f",
               mf.u(x, y), mf.v(x, y), uDx, uDy, vDx, vDy);
        if (smoothnessEnergyMap)
        ...
        if (destChanBlurred.ref(0).inBounds(xProj, yProj)) {
          ...
          for (int i = 0; i < chanCount; i++) {
            float diff = (destChanBlurred[i].interp(xProj, yProj) -
                          srcChanBlurred[i].data(x, y));
...
float varEnergy(const MotionField &mf, const ImageGrayU &mask,
                const ImageColorU &src, const ImageColorU &dest,
  ...
  return varEnergy(mf, mask, srcChanBlurred, destChanBlurred, varConf, false,
                   dataEnergy, smoothnessEnergy, dataEnergyMap,
                   smoothnessEnergyMap);
}
...
float varEnergy(const MotionField &mf, const ImageGrayU &mask,
  ...
  float energy = varEnergy(mf, mask, srcChanBlurred, destChanBlurred, varConf,
                           false, dataEnergy, smoothnessEnergy, &dataEnergyMap,
                           &smoothnessEnergyMap);
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++)
      energyMap.data(x, y) =
          dataEnergyMap.data(x, y) + smoothnessEnergyMap.data(x, y);
  return energy;
}
...
aptr<ImageGrayF> varEnergyMap(MotionField &mf, ImageGrayU &mask,
                              Array<ImageGrayF> &srcChan,
                              Array<ImageGrayF> &destChan, Config &varConf) {
