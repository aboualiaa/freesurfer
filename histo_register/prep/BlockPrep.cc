...
void selectBlockFaceImageSubset(Config &conf) {
  ...
  String inputPath =
      addDataPath(conf.readString("inputPath", "blockface/extra"));
  String outputPath =
      addDataPath(conf.readString("outputPath", "blockface/raw"));
  ...
  for (int inputIndex = offset; inputIndex < fileList.count();
       inputIndex += imagesPerSlice) {
    disp(1, "index: %d, file: %s", inputIndex, fileList[inputIndex].c_str());

    // move file to output path
    moveFile(inputPath + "/" + fileList[inputIndex],
             outputPath + "/" + fileList[inputIndex]);
...
aptr<ImageGrayU> roughMask(const ImageColorU &image, int pixelstep = 1) {
  ...
  int    max1    = 0;
  double max1val = HistogramRaw.front();
  int    max2    = s - 1;
  double max2val = HistogramRaw.back();
  ...
  int    min    = max1;
  double minval = HistogramRaw[max1];
...
double normalizeBackground(ImageColorU &image, const ImageGrayU &mask) {
  ...
  disp(1, "mean background rgb %f %f %f", ((double)sumr) / bcount,
       ((double)sumg) / bcount, ((double)sumb) / bcount);
...
aptr<ImageTransform> computeSplitTransform(const String &fileName1,
                                           const String &fileName2,
                                           const String &visPath) {
  ...
  aptr<ImageTransform> transform   = registerUsingImageTransform(
      *mask1, *mask2, paramCount, step, xBorder, yBorder, offsetBound);
  ...
  if (useInternal) {
    ...
    transform                  = registerUsingImageTransform(
        *regImage1, *regImage2, paramCount, step, xBorder, yBorder, offsetBound,
        transform.get(), regMask1.get(), regMask2.get());
...
int computeMriSize(const String &mriPath) {
  ...
  for (int inputIndex = 0; inputIndex < fileList.count(); inputIndex++) {
    aptr<ImageGrayU> image =
        load<ImageGrayU>(mriPath + "/" + fileList[inputIndex]);
    int width = image->width(), height = image->height();
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (image->data(x, y)) {
...
void cropBlockFaceImages(Config &conf) {
  ...
  String outputPath =
      addDataPath(conf.readString("outputPath", "blockface/crop"));
  String outSegPath =
      addDataPath(conf.readString("outputPath", "blockface/roughseg"));
  ...
  int    visScale   = 8;
  int    pad =
      100; // note: the pad will end up being larger than this because of the split transform
  ...
  aptr<ImageColorU> firstImage =
      load<ImageColorU>(inputPath + "/" + fileList[0]);
  int width = firstImage->width(), height = firstImage->height();
  firstImage.release();

  // open visualization video
  int visWidth =
      ((width / visScale) / 8) * 8; // make sure video size divisible by 8
...
#pragma omp parallel for
  ...
  for (int inputIndex = 0; inputIndex < fileList.count(); inputIndex++) {
    ...
    if (!keepgoing)
      continue; // can't break in parallel loop
    // load the input image
    aptr<ImageColorU> image =
        load<ImageColorU>(inputPath + "/" + fileList[inputIndex]);
    ...
    saveImage(*mask,
              outSegPath + "/" + fileList[inputIndex].leftOfLast('.') + ".png");
...
#pragma omp critical
#endif
    disp(1, "inputIndex: %d, file: %s, insideCount: %d", inputIndex,
         fileList[inputIndex].c_str(), insideCount);
...
#endif
      {
        warning("bad slice (low inside count %i): %s; stopping command",
                insideCount, fileList[inputIndex].c_str());
        keepgoing = false;
        doreturn  = true;
        //			return;
      }
...
#pragma omp critical
  ...
  for (int inputIndex = 1; inputIndex < fileList.count(); inputIndex++) {
    // check for split
    int diff =
        iAbs((int)countVect[inputIndex] - (int)countVect[inputIndex - 1]);
    if (diff > 1000000) {
  ...
  if (splitIndex > 0) {
    ...
    int    diff =
        iAbs((int)countVect[splitIndex] - (int)countVect[splitIndex + 1]);
    disp(1, "split index: %d, diff: %d, max diff: %d, file 1: %s, file 2: %s",
         splitIndex, diff, maxDiff, fileName1.c_str(), fileName2.c_str());
    disp(1, "Computing Transform (microtome adjustment) ...");
    transform = computeSplitTransform(inputPath + "/" + fileName1,
                                      inputPath + "/" + fileName2, visPath);
  }
  ...
  disp(1, "block size: %d, mri size: %d, output width: %d, output height: %d",
       blockSize, mriSize, outputWidth, outputHeight);
...
#pragma omp critical
    ...
    aptr<ImageGrayU> mask = load<ImageGrayU>(
        outSegPath + "/" + fileList[inputIndex].leftOfLast('.') + ".png");
    ...
    // save to output path
    saveImage(*image,
              outputPath + "/" + fileList[inputIndex].leftOfLast('.') + ".png");
...
void segmentBlockFaceImages(Config &conf) {
  ...
  // get command parameters
  String inputPath =
      addDataPath(conf.readString("inputPath", "blockface/crop"));
  String outputPath =
      addDataPath(conf.readString("outputPath", "blockface/seg"));
  ...
  for (int inputIndex = 0; inputIndex < fileList.count();
       inputIndex += imageStep) {
    ...
    status("inputIndex: %d, fgCount: %d, bgCount: %d", inputIndex, fgCount,
           bgCount);
  ...
  for (int inputIndex = 0; inputIndex < fileList.count(); inputIndex++) {
    ...
    status("inputIndex: %d, fgCount: %d, bgCount: %d", inputIndex, fgOutCount,
           bgOutCount);
