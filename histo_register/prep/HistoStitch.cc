...
void computeError(const StitchNode &node1, const StitchNode &node2,
                  double &sumError, int &countError, bool mapBrightness,
                  bool verbose) {
  if (verbose) {
    disp(3, "node1: %d, %d, node2: %d, %d", node1.xImageIndex(),
         node1.yImageIndex(), node2.xImageIndex(), node2.yImageIndex());
    disp(3, "bright1: %f, %f, bright2: %f, %f", node1.minBrightness(),
         node1.maxBrightness(), node2.minBrightness(), node2.maxBrightness());
  }
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if (image2.inBounds(xIn, yIn)) {
        ...
        if (mapBrightness)
          err = node1.mapBrightness(image1.data(x, y)) -
                node2.mapBrightness(image2.data(xIn, yIn));
        else
...
void updateTransformsGlobal(StitchNodeSet &nodeSet, float xStep, float yStep,
                            float xSkew, float ySkew, double scaleFactor) {
  for (int xImageIndex = 0; xImageIndex < nodeSet.xCount(); xImageIndex++) {
    for (int yImageIndex = 0; yImageIndex < nodeSet.yCount(); yImageIndex++) {
      ...
      node.transform().setOffset(xOffset * (float)scaleFactor,
...
public:
  ...
  double eval(const VectorD &point) {
    ...
    for (int xImageIndex = 0; xImageIndex < m_nodeSet.xCount(); xImageIndex++) {
      for (int yImageIndex = 0; yImageIndex < m_nodeSet.yCount();
           yImageIndex++) {
    ...
    disp(3,
         "xStep: %4.2f, yStep: %4.2f, xSkew: %6.4f, ySkew: %6.4f, countError: "
         "%d meanError: %f",
         xStep, yStep, xSkew, ySkew, countError, meanError);
    return meanError;
  }
...
void optimizeGlobalTransform(StitchNodeSet &nodeSet, int &xStep, int &yStep,
                             float &xSkew, float &ySkew, int scaleFactor) {
  bool optSkew = true;
...
public:
  // basic constructor
  NodeObjective(StitchNodeSet &nodeSet, StitchNode &node)
      : m_nodeSet(nodeSet), m_node(node) {
    m_verbose = false;
  }
  ...
  double eval(const VectorD &point) {
    ...
    if (countError)
      meanError = sumError / (double)countError;
    if (m_verbose)
      disp(3, "min: %6.4f, max: %6.4f, countError: %d meanError: %f", point[0],
           point[1], countError, meanError);
    return meanError;
  }
...
void optimizeNode(StitchNodeSet &nodeSet, StitchNode &node) {
  ...
  disp(1, "node: %d, %d, bright: %f, %f", node.xImageIndex(),
       node.yImageIndex(), result[0], result[1]);
}

/// stitch together the images for a single slide
void stitchSingleSlide(const String &inputPath, const Array<String> &fileList,
                       int &imageIndex, const String &bgFileName,
                       const String &outputPath, const String &outputFileName) {
  ...
  aptr<ImageGrayU> bgImageSmall =
      resize(*bgImage, bgImage->width() / optScaleFactor,
             bgImage->height() / optScaleFactor, true);
  ...
  for (int xImageIndex = 0; xImageIndex < xSubCount; xImageIndex++) {
    for (int yImageIndex = 0; yImageIndex < ySubCount; yImageIndex++) {
      ...
      aptr<ImageGrayU> inputSmall =
          resize(*input, input->width() / optScaleFactor,
                 input->height() / optScaleFactor, true);
      ...
      for (int y = 0; y < smallHeight; y++) {
        for (int x = 0; x < smallWidth; x++) {
          int v = inputSmall->data(x, y) + brightnessBase -
                  bgImageSmall->data(x, y);
      ...
      nodeSet.append(
          new StitchNode(inputSmall, xImageIndex, yImageIndex, fileName));
  ...
  if (runGlobalOpt) {
    optimizeGlobalTransform(nodeSet, xStep, yStep, xSkew, ySkew,
                            optScaleFactor);
    ...
    disp(2, "new step (full pix): %d, %d, new step (mils): %f, %f", xStep,
         yStep, xStepMilNew, yStepMilNew);
    disp(2, "new size (mils): %d, %d", sbl::round(xStepMilNew + xOverlapMil),
         sbl::round(yStepMilNew + yOverlapMil));
    disp(2, "new skew: %f, %f", xSkew, ySkew);
  }
  updateTransformsGlobal(nodeSet, (float)xStep, (float)yStep, xSkew, ySkew,
                         1.0 / optScaleFactor);
      ...
      for (int y = 0; y < fullHeight; y++) {
        for (int x = 0; x < fullWidth; x++) {
          ...
          if (output->inBounds(xOut, yOut)) {
            ...
            output->data(xOut, yOut) =
                bound(sbl::round(node.mapBrightness(v)), 0, 255);
          }
        }
      }
...
void stitchHistoImages(Config &conf) {
  ...
  if (fileList.count() != imageCount) {
    warning("found %d images; expected %d images", fileList.count(),
            imageCount);
    return;
  }
  ...
  for (int x = 0; x < xSlideCount; x++) {
    for (int y = 0; y < ySlideCount; y++) {
      ...
      stitchSingleSlide(inputPath, fileList, imageIndex, bgFileName, outputPath,
                        outputFileName);
