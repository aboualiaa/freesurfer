...
void prepareHisto(ImageGrayU &image, int botThresh, int topThresh,
                  int peakStart) {
  ...
  if (botThresh == -1 || topThresh == -1) {
    VectorF histogram = toFloat(
        imageHistogram(image, border, width - border, border, height - border));
    ...
    disp(1, "botThresh: %d (%f), topThresh: %d (%f), peak: %d (%f)", botThresh,
         histogram[botThresh], topThresh, histogram[topThresh], peak,
         histogram[peak]);
  }
