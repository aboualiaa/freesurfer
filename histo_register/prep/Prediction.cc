...
int patchDiff(MultiImage &image1, MultiImage &image2, int x1, int y1, int x2,
              int y2, int radius) {
  int sumDiff = 0;
  for (int dy = -radius; dy <= radius; dy++) {
    for (int dx = -radius; dx <= radius; dx++) {
      for (int j = 0; j < image1.count(); j++) {
        int diff =
            image1[j].data(x1 + dx, y1 + dy) - image2[j].data(x2 + dx, y2 + dy);
        if (diff < 0)
          diff = -diff;
        sumDiff += diff;
      }
...
aptr<ImageGrayU> predict(Array<MultiImage> &srcTrainSet,
                         Array<ImageGrayU> &destTrainSet,
                         MultiImage &       srcPredImage) {
  ...
  for (int yp = radius; yp < height - radius; yp++) {
    for (int xp = radius; xp < width - radius; xp++) {
      ...
      for (int i = 0; i < srcTrainSet.count(); i++) {
        ...
        assertAlways(srcTrainImage[0].width() == width &&
                     srcTrainImage[0].height() == height);
        for (int yt = radius; yt < height - radius; yt++) {
          for (int xt = radius; xt < width - radius; xt++) {
            int diff =
                patchDiff(srcTrainImage, srcPredImage, xt, yt, xp, yp, radius);
...
void initPrediction() { registerCommand("pred", predictImages); }
