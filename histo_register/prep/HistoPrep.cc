...
aptr<ImageGrayU> createHistologyMask(const ImageGrayU &image,
  ...
  // compute a histogram
  VectorF hist =
      toFloat(imageHistogram(image, histogramBorder, width - histogramBorder,
                             histogramBorder, height - histogramBorder));
  hist       = gaussFilter(hist, 3);
  int thresh = nearestMinIndex(hist, initThresh);
  disp(1, "input: %s, thresh: %d, hist at thresh: %f", fileName.c_str(), thresh,
       hist[thresh]);
  ...
  if (savePlots) {
    ...
    String plotFileName =
        visPath + "/histogram_" + fileName.leftOfLast('.') + ".svg";
...
void findHistogramBounds(const VectorF &histogram, float lowerFrac,
                         float upperFrac, int &lowerThresh, int &upperThresh) {
...
aptr<ImageColorU> combineHistoWavelengths(const ImageGrayU &image700,
                                          const ImageGrayU &image800,
                                          const ImageGrayU &mask) {
...
void extractRegion(const ImageColorU &input, ImageGrayU &mask, int xCheck,
                   int yCheck, bool firstPass, int &outputWidth,
                   int &outputHeight, const String &outputPath, int rotateAngle,
                   int id) {
  int border = 100;
  ...
  floodFill(mask, 255, 255, 100, xBest, yBest, &xCent, &yCent, &xMin, &xMax,
            &yMin, &yMax);
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    for (int i = 0; i < fileList.count(); i++) {
      ...
      aptr<ImageGrayU> image800 =
          load<ImageGrayU>(inputFileName.leftOfLast('_') + "_800.png");
      int width = image700->width(), height = image700->height();
      ...
      if (split.count() < 3) {
        warning(
            "unexpected file name format (expected at least 2 underscores)");
        return;
      }
      int sliceId1 =
          split[split.count() - 3].toInt(); // will be zero if not numeric
      int sliceId2 = split[split.count() - 2].toInt();
      disp(2, "id1: %d, id2: %d", sliceId1, sliceId2);

      // compute mask
      aptr<ImageGrayU> mask =
          createHistologyMask(*image700, visPath, fileList[i]);

      // combine histo wavelengths into single color image
      aptr<ImageColorU> image =
          combineHistoWavelengths(*image700, *image800, *mask);

      // if two IDs, extract two regions
      if (sliceId1) {
        extractRegion(*image, *mask, width / 4, height / 4, pass == 0,
                      outputWidth, outputHeight, outputPath, rotateAngle,
                      sliceId1);
        extractRegion(*image, *mask, width * 3 / 4, height * 3 / 4, pass == 0,
                      outputWidth, outputHeight, outputPath, rotateAngle,
                      sliceId2);

        // if one ID, extract one region
      } else {
        extractRegion(*image, *mask, width / 2, height / 2, pass == 0,
                      outputWidth, outputHeight, outputPath, rotateAngle,
                      sliceId2);
      }
...
void createHistologyVolume(Config &conf) {
  String inputPath =
      addDataPath(conf.readString("inputPath", "histo/split/gray800"));
  ...
  convertImagesToMghFile(inputPath, outputVolumeFileName, xSpacing, ySpacing,
                         zSpacing);
}
...
void convertHistologyVolumeIntoImages(Config &conf) {
  ...
  String volumeFileName =
      addDataPath(conf.readString("volumeFileName", "splitVol.mgz"));
  String mapFileName =
      addDataPath(conf.readString("mapFileName", "splitVolMap.txt"));
  ...
  convertMghFileToImages(volumeFileName, outputPath, false, false, false, false,
                         false, false, autoScaleValues);
  ...
  while (mapFile.endOfFile() == false) {
    ...
    if (parts.count() == 2) {
      ...
      disp(1, "rename: %d -> %d", mgzIndex,
           histoIndex /*oldFileName.c_str(), newFileName.c_str()*/);
  ...
  for (int i = 0; i < fileList.count(); i++) {
    String fileName = fileList[i];
    moveFile(outputPath + "/" + fileName,
             outputPath + "/" + fileName.rightOf(0));
  }
}
