...
void saveAxisTransform(const String &outputPath, bool xySwap, bool xzSwap,
                       bool yzSwap, bool xFlip, bool yFlip, bool zFlip) {
  int x[] = {1, 0, 0};
  int y[] = {0, 1, 0};
  int z[] = {0, 0, 1};
  if (xySwap) {
  ...
  if (axisFile.openSuccess()) {
    axisFile.writeF(
        "0, 0, 0, "); // write zero translation offset so that this is a valid AffineTransform3 file
...
void convertMghFileToImages(const String &fileName, const String &outputPath,
                            bool xySwap, bool xzSwap, bool yzSwap, bool xFlip,
                            bool yFlip, bool zFlip, bool autoScaleValues) {

  // open the file
  File file(fileName, FILE_READ,
            fileName.endsWith("gz") ? FILE_GZIP_BINARY : FILE_BINARY);
  ...
  int zOutSize = depth;
  if (xySwap)
    swap(xOutSize, yOutSize);
  if (xzSwap)
    swap(xOutSize, zOutSize);
  if (yzSwap)
    swap(yOutSize, zOutSize);
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        if (yzSwap)
          swap(yOut, zOut);
        if (xFlip)
          xOut = xOutSize - xOut - 1;
        if (yFlip)
          yOut = yOutSize - yOut - 1;
        if (zFlip)
          zOut = zOutSize - zOut - 1;
...
void convertImagesToMghFile(const String &inputPath,
                            const String &outputFileName, float xSpacing,
                            float ySpacing, float zSpacing, int width,
                            int height, int valueScaleFactor) {
  ...
  if (width == 0 || height == 0) {
    aptr<ImageGrayU> firstImage =
        load<ImageGrayU>(inputPath + "/" + inputFileNames[0]);
...
void convertMghFileToImages(Config &conf) {
  ...
  convertMghFileToImages(fileName, outputPath, xySwap, xzSwap, yzSwap, xFlip,
                         yFlip, zFlip, autoScaleValues);
}
