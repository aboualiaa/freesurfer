...
void thresholdMData(Config &conf) {
  ...
  String inputPath =
      addDataPath(conf.readString("inputPath", "mri/rawFlash20"));
...
struct LabelPoint {
  LabelPoint(int xNew, int yNew, int zNew, int vNew)
      : x(xNew), y(yNew), z(zNew), v(vNew) {}
  int x;
  int y;
  int z;
  int v;
...
int interpolate(int x, int y, int z, const Array<LabelPoint> &labels,
                float sigma) {
...
void segmentMData(Config &conf) {
  ...
  // get command parameters
  String inputPath1 =
      addDataPath(conf.readString("inputPath1", "mri/rawFlash20"));
  ...
  disp(1, "fg label points: %d, bg label points: %d", fgLabels.count(),
       bgLabels.count());
  ...
  for (int z = 0; z < fileList.count(); z++) {
    ...
    disp(1, "input: %s, threshold: %f/%f/%f, fg: %d", fileList[z].c_str(),
         thresholdVect.min(), thresholdVect.mean(), thresholdVect.max(),
         fgCount);
