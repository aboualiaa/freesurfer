...
void readPolarizationImages(Config &conf) {
  ...
  String outputFileName =
      addDataPath(conf.readString("outputFileName", "polarization.csv"));
  ...
  if (angleCount * polarizationStateCount != fileList.count()) {
    warning("found images: %d, expected images: %d", fileList.count(),
            angleCount * polarizationStateCount);
    return;
  }
  ...
  for (int angleIndex = 0; angleIndex < angleCount; angleIndex++) {
    ...
    for (int i = 0; i < polarizationStateCount; i++) {
      ...
      aptr<ImageGrayU> input =
          load<ImageGrayU>(inputPath + "/" + fileList[index]);
      ...
      disp(1, "index: %d, angle: %d, file: %s, value: %f", index, angle,
           fileList[index].c_str(), value);
...
void predictFromPolarizationData(Config &conf) {
  ...
  String inputFileName =
      addDataPath(conf.readString("inputFileName", "polarization.csv"));
...
void preparePolarizationImages(Config &conf) {
  ...
  // get command parameters
  String inputPath =
      addDataPath(conf.readString("inputPath", "blockface/polarization/calib"));
  ...
  for (int i = 0; i < fileList.count(); i++) {
    ...
    // save for future use
    String outputFileName =
        inputPath + "/" + fileList[i].leftOfLast('.') + ".png";
...
void processPolarizationSlice(const Array<Array<ImageGrayU>> &calibImages,
                              ...
                              const Array<String> &           sampleFileList,
                              const String &outputPath, int sliceIndex,
                              int polarizationStateCount, int angleCount,
                              int angleStep, int sampleScaleFactor,
                              float angleInterpSigma) {
  ...
  for (int i = sliceIndex * polarizationStateCount;
       i < (sliceIndex + 1) * polarizationStateCount; i++) {
    assertAlways(i < sampleFileList.count());
    aptr<ImageGrayU> input =
        load<ImageGrayU>(samplePath + "/" + sampleFileList[i]);
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if (interpolate) {
        ...
        for (int angleIndex = minAngleIndex; angleIndex <= maxAngleIndex;
             angleIndex++) {
      ...
      if (savePixelPlots) {
        if (((y == height / 2 && (x % 120) == 0) ||
          ...
          distPlot->save(outputPath +
                         sprintF("/dist_%04d_%d_%d.svg", sliceIndex, x, y));
          disp(2, "pre-norm: %f / %f / %f", sampleFeatPreNorm.min(),
               sampleFeatPreNorm.mean(), sampleFeatPreNorm.max());
  ...
  // display statistics
  disp(2, "st. dev: %f / %f / %f", stDevVect.min(), stDevVect.mean(),
       stDevVect.max());
  disp(2, "bright: %f / %f / %f", brightVect.min(), brightVect.mean(),
       brightVect.max());
  disp(2, "angle: %f / %f / %f", angleVect.min(), angleVect.mean(),
       angleVect.max());

  // save results
  saveImage(outputAngle, outputPath + sprintF("/angle_%04d.jpg", sliceIndex));
  saveImage(outputAngleStrength,
            outputPath + sprintF("/angleStrength_%04d.jpg", sliceIndex));
  saveImage(outputBright,
            outputPath + sprintF("/brightness_%04d.jpg", sliceIndex));
  saveImage(outputStDev,
            outputPath + sprintF("/strength_%04d.jpg", sliceIndex));
...
void computeFullPolarizationImage(Config &conf) {
  ...
  // get command parameters
  String calibPath =
      addDataPath(conf.readString("calibPath", "blockface/polarization/calib"));
  String samplePath = addDataPath(
      conf.readString("samplePath", "blockface/polarization/input"));
  String outputPath = addDataPath(
      conf.readString("outputPath", "blockface/polarization/output"));
  int   angleCount             = conf.readInt("angleCount", 18);
  int   angleStep              = conf.readInt("angleStep", 10);
  int   polarizationStateCount = conf.readInt("polarizationStateCount", 16);
  ...
  if (angleCount * polarizationStateCount != calibFileList.count()) {
    warning("found calib images: %d, expected images: %d",
            calibFileList.count(), angleCount * polarizationStateCount);
    return;
  }
  ...
  if (sliceCount == 0 ||
      sliceCount * polarizationStateCount != sampleFileList.count()) {
    warning(
        "found sample images: %d, expected a (positive) multiple of %d images",
        sampleFileList.count(), polarizationStateCount);
    return;
  }
  ...
  for (int angleIndex = 0; angleIndex < angleCount; angleIndex++) {
    ...
    for (int i = 0; i < polarizationStateCount; i++) {
      ...
      aptr<ImageGrayU> input =
          load<ImageGrayU>(calibPath + "/" + calibFileList[index]);
  ...
  for (int sliceIndex = 0; sliceIndex < sliceCount; sliceIndex++) {
    processPolarizationSlice(calibImages, samplePath, sampleFileList,
                             outputPath, sliceIndex, polarizationStateCount,
                             angleCount, angleStep, sampleScaleFactor,
                             angleInterpSigma);
