...
/// The StitchNode class represents a single input image to be stitched together
/// into a larger image
class StitchNode {
public:
  // basic constructor; takes ownership of image
  StitchNode(aptr<ImageGrayU> image, int xImageIndex, int yImageIndex,
  ...
  inline void setBrightnessBounds(float minBrightness, float maxBrightness) {
    m_minBrightness = minBrightness;
    m_maxBrightness = maxBrightness;
  }

  /// map a pixel value according to the brightness bounds
  inline float mapBrightness(int v) const {
    return ((float)v - m_minBrightness) * 255.0f /
...
/// The StitchNodeSet class represents a set of images to be stitched together
