...
void checkImageSeqDimensions(const Array<ImageType> &seq, int width, int height,
                             int depth) {
  assertAlways(seq.count() == depth && seq[0].width() == width &&
               seq[0].height() == height);
}
...
void varSolveSystem(VarCorresInfo &vci, Config &varConf, bool fast) {
  ...
  // get parameters
  float smoothness =
      varConf.readFloat("smoothness"); // aka alpha (in Brox papers)
  ...
  int solverIters = sbl::round((float)solverItersPerMegapixel *
                               (float)vci.pixelCount / 1000000.0f);
  if (fast) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (mask.data(x, y)) {
          ...
          // check indices
          if (uInd < 0 || uInd >= init.length() || vInd < 0 ||
              vInd >= init.length() || wInd < 0 || wInd >= init.length())
          ...
          b1 +=
              -dataFactor * ix * ic; // negate because subtract from both sides
          duCoef2 += dataFactor * iy * ix;
          dvCoef2 += dataFactor * iy * iy;
          dwCoef2 += dataFactor * iy * iz;
          b2 +=
              -dataFactor * iy * ic; // negate because subtract from both sides
          duCoef3 += dataFactor * iz * ix;
          dvCoef3 += dataFactor * iz * iy;
          dwCoef3 += dataFactor * iz * iz;
          b3 +=
              -dataFactor * iz * ic; // negate because subtract from both sides

          // prep for smoothness term
          float alpha        = smoothness; // + localSmoothness->data( x, y );
          float smoothFactor = alpha * dPsiSmoothness.data(x, y);
          float smoothFactorPx =
              x < width - 1 ? alpha * dPsiSmoothness.data(x + 1, y) : 0;
          float smoothFactorPy =
              y < height - 1 ? alpha * dPsiSmoothness.data(x, y + 1) : 0;
          float smoothFactorPz =
              z < depth - 1 ? alpha * vci.dPsiSmoothness[z + 1].data(x, y) : 0;
          ...
          b1 -= (vci.uDx[z].data(x, y) + vci.uDy[z].data(x, y) +
                 vci.uDz[z].data(x, y)) *
                smoothFactor;
          if (x < width - 1)
            b1 += vci.uDx[z].data(x + 1, y) * smoothFactorPx;
          if (y < height - 1)
            b1 += vci.uDy[z].data(x, y + 1) * smoothFactorPy;
          if (z < depth - 1)
            b1 += vci.uDz[z + 1].data(x, y) * smoothFactorPz;
          ...
          b2 -= (vci.vDx[z].data(x, y) + vci.vDy[z].data(x, y) +
                 vci.vDz[z].data(x, y)) *
                smoothFactor;
          if (x < width - 1)
            b2 += vci.vDx[z].data(x + 1, y) * smoothFactorPx;
          if (y < height - 1)
            b2 += vci.vDy[z].data(x, y + 1) * smoothFactorPy;
          if (z < depth - 1)
            b2 += vci.vDz[z + 1].data(x, y) * smoothFactorPz;
          ...
          b3 -= (vci.wDx[z].data(x, y) + vci.wDy[z].data(x, y) +
                 vci.wDz[z].data(x, y)) *
                smoothFactor * wSmooth;
          if (x < width - 1)
            b3 += vci.wDx[z].data(x + 1, y) * smoothFactorPx * wSmooth;
          if (y < height - 1)
            b3 += vci.wDy[z].data(x, y + 1) * smoothFactorPy * wSmooth;
          if (z < depth - 1)
            b3 += vci.wDz[z + 1].data(x, y) * smoothFactorPz * wSmooth;
          ...
          sparseSystem.addEquation(uInd, vInd, wInd, uMxInd, uPxInd, uMyInd,
                                   uPyInd, uMzInd, uPzInd, duCoef1, dvCoef1,
                                   dwCoef1, duMxCoef, duPxCoef, duMyCoef,
                                   duPyCoef, duMzCoef, duPzCoef, b1);
          sparseSystem.addEquation(vInd, uInd, wInd, vMxInd, vPxInd, vMyInd,
                                   vPyInd, vMzInd, vPzInd, dvCoef2, duCoef2,
                                   dwCoef2, dvMxCoef, dvPxCoef, dvMyCoef,
                                   dvPyCoef, dvMzCoef, dvPzCoef, b2);
          sparseSystem.addEquation(wInd, uInd, vInd, wMxInd, wPxInd, wMyInd,
                                   wPyInd, wMzInd, wPzInd, dwCoef3, duCoef3,
                                   dvCoef3, dwMxCoef, dwPxCoef, dwMyCoef,
                                   dwPyCoef, dwMzCoef, dwPzCoef, b3);
        }
      }
    }
  }
  ...
  for (int z = 0; z < depth; z++) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (mask.data(x, y)) {
          float duBounded =
              bound(result.data(uIndex.data(x, y)), -maxMove, maxMove);
          float dvBounded =
              bound(result.data(vIndex.data(x, y)), -maxMove, maxMove);
          float dwBounded =
              bound(result.data(wIndex.data(x, y)), -maxMove, maxMove);
          ...
          for (int x = 1; x < width; x++) {
            ...
            if (mask.data(x, y)) {
              ...
              if (robustSmoothnessTerm) {
                ...
                float sum    = uNewDx * uNewDx + uNewDy * uNewDy +
                            uNewDz * uNewDz + vNewDx * vNewDx +
                            vNewDy * vNewDy + vNewDz * vNewDz +
...
void varIteration(Array<ImageGrayF> &uSeq, Array<ImageGrayF> &vSeq,
                  Array<ImageGrayF> &      wSeq,
                  const Array<ImageGrayF> &srcSeqScaled,
                  const Array<ImageGrayF> &destSeqScaled, Config &varConf,
                  bool fast) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        if (mask.data(x, y)) {
          ...
          if (((xProj >= 0 && xProj <= width - 1) ||
               (xProjInt == 0 && width == 1)) &&
              ((yProj >= 0 && yProj <= height - 1) ||
               (yProjInt == 0 && height == 1)) &&
              ((zProj >= 0 && zProj <= depth - 1) ||
               (zProjInt == 0 && depth == 1))) {
...
void resizeAndBlur(const ImageGrayFSeq &inSeq, ImageGrayFSeq &outSeq,
                   int newWidth, int newHeight, int newLength, float sigma) {
  ImageGrayFSeq tempSeq1,
      tempSeq2; // note: could use outSeq as tempSeq1 (unless inSeq is same as outSeq)
  for (int z = 0; z < inSeq.count(); z++)
    tempSeq1.append(resize(inSeq[z], newWidth, newHeight, true).release());
  outSeq
      .reset(); // needed for corres field components where outSeq is same as inSeq
  if (sigma) {
    ...
    assertAlways(outSeq[0].width() == newWidth &&
                 outSeq[0].height() == newHeight);
  } else {
...
void varScaleLoop(const Array<ImageGrayF> &srcSeq,
                  const Array<ImageGrayF> &destSeq, Array<CorresField3D> &cfSeq,
                  Config &varConf) {
  ...
  if (verbosity >= 4)
    disp(1, "iterating over %d scales (scale-factor = %f)", scaleCount,
         scaleFactor);
  ...
  for (int scaleIndex = scaleCount - 1; scaleIndex >= 0; scaleIndex--) {
    ...
    if (verbosity >= 4)
      disp(2, "scale %f: %d x %d x %d", scale, smallWidth, smallHeight,
           smallLength);

    // shrink and blur the channels to current size
    Array<ImageGrayF> srcSeqScaled, destSeqScaled;
    if (scales.length() > 1) {
      resizeAndBlur(srcSeq, srcSeqScaled, smallWidth, smallHeight, smallLength,
                    sigma);
      resizeAndBlur(destSeq, destSeqScaled, smallWidth, smallHeight,
                    smallLength, sigma);
    }
    ...
    if (scales.length() > 1)
      varIteration(uSeq, vSeq, wSeq, srcSeqScaled, destSeqScaled, varConf,
                   fast);
    else
...
void varCorres3D(const Array<ImageGrayU> &src, const Array<ImageGrayU> &dest,
  ...
  assertAlways(src[0].width() == dest[0].width() &&
               src[0].height() == dest[0].height());
