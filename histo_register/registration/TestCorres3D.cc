...
void transposeYZ(const Array<CorresField3D> &cfSeqInput,
                 Array<CorresField3D> &      cfSeqOutput) {
...
void initTestSequence(Array<ImageGrayU> &seq, int width, int height,
                      int length) {
...
void initCorresComponent(Array<ImageGrayF> &seq, int width, int height,
                         int length) {
  bool              flat           = (width <= 2 || height <= 2 || length <= 2);
...
void initCorres(Array<CorresField3D> &cfSeq, int width, int height,
                int length) {
...
void testCorres3D(Config &conf) {
  ...
  assertAlways(destSeq.count() == length && destSeq[0].width() == width &&
               destSeq[0].height() == height);
  ...
  for (int z = 0; z < length; z++) {
    ...
    cfVis                      = resize(*cfVis, width * 4, height * 4,
                   true); // make bigger just so easier to see
    ...
    aptr<ImageColorU> visLarge = resize(
        vis, width * 4, height * 4, true); // make bigger just so easier to see
    saveImage(*visLarge, outputPath + sprintF("/proj-%04d.png", z));
    aptr<ImageGrayU> destLarge =
        resize(destSeq[z], width * 4, height * 4,
               true); // make bigger just so easier to see
...
void initTestCorres3D() { registerCommand("cortest", testCorres3D); }
