...
bool HistoTransform::loadHToB(const String &mrRawPath,
                              const String &mrRegLinPath,
                              const String &mrRegPath,
                              const String &histoSplitPath,
                              const String &histoRegPath) {
  ...
  if (mfFileList.count() == 0) {
    warning("unable to find histo motion field files at: %s",
            histoRegPath.c_str());
    return false;
  }
  for (int i = 0; i < mfFileList.count(); i++) {
    ...
    aptr<ImageGrayU> image = sbl::load<ImageGrayU>(
        fileName); // fix(faster): we could get the image dimensions without loading the entire image
    int width = image->width(), height = image->height();
    m_histoShrinkX.append((double)mfWidth / (double)width);
    m_histoShrinkY.append((double)mfHeight / (double)height);
  }
  disp(1, "histo shrink for slice zero: %.2f, %.2f", m_histoShrinkX[0],
       m_histoShrinkY[0]);
  disp(1, "motion field size: %d x %d", m_mfHisto[0].width(),
       m_mfHisto[0].height());
  return true;
}

/// load transformation data generated by the block-to-MR registration commands
bool HistoTransform::loadBToM(const String &mrRawPath,
                              const String &mrRegLinPath,
                              const String &mrRegPath,
                              const String &histoSplitPath,
                              const String &histoRegPath) {
  ...
  if (regLinTransformFile.openSuccess()) {
    ...
    warning("unable to open transform file: %s",
            regLinTransformFileName.c_str());
    return false;
  }
  disp(1, "M/B reg lin scale: %.2f, %.2f, %.2f, offset: %.2f, %.2f, %.2f",
       m_regLinTransform.a.data[0][0], m_regLinTransform.a.data[1][1],
       m_regLinTransform.a.data[2][2], m_regLinTransform.b.x,
       m_regLinTransform.b.y, m_regLinTransform.b.z);
  ...
  if (mrTransformFile.openSuccess()) {
    ...
    warning("unable to open MR transform file: %s",
            mrTransformFileName.c_str());
    return false;
  }
  return true;
}
...
void HistoTransform::computeInversesHToB() {
  ...
  for (int i = 0; i < m_mfHisto.count(); i++) {
    ...
    disp(1, "%d of %d, %5.3f / %5.3f / %5.3f, %5.3f / %5.3f / %5.3f", i,
         m_mfHisto.count(), uMin, uMean, uMax, vMin, vMean, vMax);
...
void HistoTransform::computeInversesBToM() {
  ...
  disp(1,
       "M/B reg lin inverse scale: %.2f, %.2f, %.2f, offset: %.2f, %.2f, %.2f",
       m_regLinTransformInv.a.data[0][0], m_regLinTransformInv.a.data[1][1],
       m_regLinTransformInv.a.data[2][2], m_regLinTransformInv.b.x,
       m_regLinTransformInv.b.y, m_regLinTransformInv.b.z);
...
Point3 HistoTransform::projectForward(Point3 point, bool useMrTransform,
                                      bool smallHistoCoords,
                                      bool verbose) const {
...
Point3 HistoTransform::projectBackward(Point3 point, bool useMrTransform,
                                       bool smallHistoCoords,
                                       bool verbose) const {
...
Point3 HistoTransform::projectHToB(Point3 point, bool smallHistoCoords,
                                   bool verbose) const {
  if (verbose)
    disp(1, "init: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);
  ...
  if (verbose)
    disp(1, "after histo shrink: %3.1f, %3.1f, %3.1f", point.x, point.y,
         point.z);

  // map according to linear image transform
  double xNew =
      m_histoTransformInv[mfIndex].xTransform((float)point.x, (float)point.y);
  double yNew =
      m_histoTransformInv[mfIndex].yTransform((float)point.x, (float)point.y);
  point.x = xNew;
  point.y = yNew;
  if (verbose)
    disp(1, "after histo linear: %3.1f, %3.1f, %3.1f", point.x, point.y,
         point.z);
  ...
  if (mf.inBounds((float)point.x, (float)point.y) == false) {
    if (verbose)
      warning("projected out of motion field: %f, %f (mf size: %d, %d)",
              point.x, point.y, mf.width(), mf.height());
    point.x = 0;
    point.y = 0;
    point.z = 0;
    return point;
  ...
  point.y += v;
  if (verbose)
    disp(1, "after mf: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);

  // map z coord
  point.z = m_histoSliceIndex[mfIndex] + m_blockOffset;
  if (verbose)
    disp(1, "after block offset: %3.1f, %3.1f, %3.1f", point.x, point.y,
         point.z);
  return point;
}

/// project a point from block-face coordinates to MR coordinates
Point3 HistoTransform::projectBToM(Point3 point, bool useMrTransform,
                                   bool useNonLinear, bool verbose) const {
  ...
  if (useNonLinear) {
    ...
    if (xInt < 0 || xInt >= cfWidth || yInt < 0 || yInt >= cfHeight ||
        zInt < 0 || zInt >= m_cfSeq.count()) {
      if (verbose)
    ...
    if (verbose)
      disp(1, "after reg non-lin: %3.1f, %3.1f, %3.1f", point.x, point.y,
           point.z);
  }
  ...
  if (verbose)
    disp(1, "after reg lin: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);

  // apply MR transform
  if (useMrTransform) {
    point = m_mrTransform.transform(point);
    if (verbose)
      disp(1, "after MR: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);
  }
  return point;
}

/// project a point from MR coordinates to block-face coordinates
Point3 HistoTransform::projectMToB(Point3 point, bool useMrTransform,
                                   bool useNonLinear, bool verbose) const {
  if (verbose)
    disp(1, "init: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);
  ...
  if (useMrTransform) {
    ...
    if (verbose)
      disp(1, "after MR inv: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);
  }
...
/// project a point from block-face coordinates to MR coordinates
Point3 HistoTransform::projectBToH(Point3 point, bool smallHistoCoords,
                                   bool verbose) const {
  ...
  point.y += v;
  if (verbose)
    disp(1, "after mf inv: %3.1f, %3.1f, %3.1f", point.x, point.y, point.z);

  // map according to linear image transform
  double xNew =
      m_histoTransform[mfIndex].xTransform((float)point.x, (float)point.y);
  double yNew =
      m_histoTransform[mfIndex].yTransform((float)point.x, (float)point.y);
  point.x = xNew;
  point.y = yNew;
  if (verbose)
    disp(1, "after histo linear inv: %3.1f, %3.1f, %3.1f", point.x, point.y,
         point.z);
  ...
  if (verbose)
    disp(1, "after histo unshrink: %3.1f, %3.1f, %3.1f", point.x, point.y,
         point.z);
  return point;
}
...
void projectPoints(Config &conf) {
  ...
  bool   smallHistoCoords = conf.readBool(
      "smallHistoCoords", false); // true if using post-hprep coordinates
  String mrRawPath =
  ...
  String histoSplitPath = addDataPath(conf.readString("histoSplitPath"
                                                      "histo/split"));
  String histoRegPath   = addDataPath(conf.readString("histoRegPath"
                                                    "histo/reg"));
  ...
  if (histoTransform.loadHToB(mrRawPath, mrRegLinPath, mrRegPath,
                              histoSplitPath, histoRegPath) == false) {
    return; // don't need to print warning; transform load will do that
  }
  if (histoTransform.loadBToM(mrRawPath, mrRegLinPath, mrRegPath,
                              histoSplitPath, histoRegPath) == false) {
    return; // don't need to print warning; transform load will do that
  }
  ...
  while (inputFile.endOfFile() == false) {
    ...
    if (inputSplit.count()) {
      if (inputSplit.count() != 3) {
        warning("invalid line: %s (expect 3 numbers separated by commas)",
                inputLine.c_str());
      }
      Point3 point;
      ...
      if (projectForward) {
        point =
            histoTransform.projectForward(point, true, smallHistoCoords, false);

        // project MR coord to histo coord
      } else {
        point = histoTransform.projectBackward(point, true, smallHistoCoords,
                                               false);
      }
...
void testProjectPoints(Config &conf) {
  ...
  String mrRegLinPath =
      addDataPath(conf.readString("mrRegLinPath", "mri/regLin"));
  String mrRegPath = addDataPath(conf.readString("mrRegPath", "mri/reg"));
  String histoSplitPath =
      addDataPath(conf.readString("histoSplitPath", "histo/split"));
  String histoRegPath =
  ...
  for (int i = 0; i < fileList.count(); i++) {
    ...
    for (int y = 0; y < hHeight; y += step) {
      for (int x = 0; x < hWidth; x += step) {
        if (mask->data(x, y)) {
          ...
          point.z = hSliceIndex;
          Point3 forPoint =
              histoTransform.projectForward(point, true, false, verbose);
          if (forPoint.x == 0 && forPoint.y == 0 && forPoint.z == 0) {
            badCount++;
            continue;
          }
          Point3 backPoint =
              histoTransform.projectBackward(forPoint, true, false, verbose);
    ...
    disp(1, "slice: %d, error: %f / %f / %f", hSliceIndex, sliceError.min(),
         sliceError.mean(), sliceError.max());
  ...
  disp(1, "error: %f / %f / %f", allError.min(), allError.mean(),
       allError.max());
...
void projectVolume(Config &conf) {
  ...
  String outputPath =
      addDataPath(conf.readString("outputPath", "mri/labelProj"));
  ...
  aptr<ImageGrayU> hImage =
      load<ImageGrayU>(histoSplitPath + "/" + hFileList[0]);
  hWidth  = hImage->width();
    ...
    for (int y = 0; y < hHeight; y++) {
      for (int x = 0; x < hWidth; x++) {
        ...
        if (x == hWidth / 2 && y == hHeight / 2 && i == hFileList.count() / 2)
          verbose = true;
        Point3 mrPoint =
            histoTransform.projectForward(point, false, false, verbose);
...
void projectHistoToBlockFace(Config &conf) {
  ...
  String outputPath =
      addDataPath(conf.readString("outputPath", "histo/labelProj"));
          ...
          for (int x = 0; x < bWidth; x++) {
            ...
            if (interpType == 0) {
              ...
              if (inputImage->inBounds((float)point.x, (float)point.y)) {
                ...
                histoValue = (int)(xpd * ypd * inputImage->data(xm, ym) +
  ...
  for (int bIndex = 0; bIndex < bDepth; bIndex++) {
    ...
    for (int y = 0; y < bHeight; y++) {
      for (int x = 0; x < bWidth; x++) {
        ...
        point.y = y;
        point.z = bIndex;
        point = histoTransform.projectBToM(point, useMrTransform, useNonLinear,
                                           false);
        if (point.x || point.y || point.z) {
          int mValue = 0;
          if (interpType == 0) {
            ...
            if (point.z >= 0 && point.z <= (double)mrVolume.count() - 1) {
              ...
              if (slicem.inBounds((float)point.x, (float)point.y)) {
                ...
                mValue = (int)(xpd * ypd * zpd * slicem.data(xm, ym) +
