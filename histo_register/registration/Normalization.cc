...
aptr<ImageGrayU> entropyImage(const ImageGrayU &image, const ImageGrayU &mask,
                              int patchRadius, float patchSigma) {
        ...
        for (int yp = yMin; yp <= yMax; yp++) {
          for (int xp = xMin; xp <= xMax; xp++) {
            if (mask.data(xp, yp)) {
              ...
              float weight  = gauss(
                  distSqd,
                  patchGaussFactor); // note: might be faster to store in a table
