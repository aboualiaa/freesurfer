...
aptr<ImageColorU> colorizeDifference(const ImageGrayU &input1,
                                     const ImageGrayU &input2) {
...
void registerHistologySlice(int hIndex, int blockOffset, const String &hPath,
                            const String &bPath, const String &mPath,
                            String &outputPath, const Array<String> &hFileList,
                            const Array<String> &bFileList,
                            const Array<String> &mFileList) {
  ...
  bool   saveFlow           = true;
  String varMotionConfigFileName =
      "varMotion.conf"; // note: using current directory
...
#pragma omp critical
  ...
  aptr<ImageGrayU> mMaskBlurred = blurGauss(
      *mMask,
      maskBlurSigma); // note: we could use a volume blur for the MR data, but want it amount of blur to match histology (where we can't do a volume blur prior to alignment)
  ...
  aptr<ImageTransform> transform = registerUsingImageTransform(
      *mNorm, *hNorm, linearParamCount, 1, 10, 10, 100);
  aptr<ImageGrayU> hMaskLinear =
      transform->mapBackward(*hMask, mWidth, mHeight, 0);
  aptr<ImageGrayU> hNormLinear =
      transform->mapBackward(*hNorm, mWidth, mHeight, 0);
  aptr<ImageGrayU> hImageLinear =
      transform->mapBackward(*hImage, mWidth, mHeight, 255);
  String transformFileName =
      outputPath + sprintF("/transform_%d.dat", hSliceIndex);
  ...
  if (computeFlow) {
    mf = varMotion(*mNorm, *hNormLinear, varMotionConfigFileName, NULL, NULL,
                   motionScaleFactor);
...
void registerHistology(Config &conf) {
  ...
  String blockPath =
      addDataPath(conf.readString("blockPath", "blockface/crop"));
  ...
  // check input lists
  disp(1, "histo image count: %d, block image count: %d, MR image count: %d",
       hFileList.count(), bFileList.count(), mFileList.count());
...
#pragma omp parallel for
  ...
  for (int hIndex = 0; hIndex < hFileList.count(); hIndex += histoStep) {
    ...
    registerHistologySlice(hIndex, blockOffset, histoPath, blockPath, mrPath,
                           outputPath, hFileList, bFileList, mFileList);
...
void initHistoRegister() { registerCommand("hreg", registerHistology); }
