...
/// The CorresField3D class represents a 2D field of 3D vectors, representing
/// offsets from a volume slice into another volume.
class CorresField3D {
public:
  ...
  inline void set(int x, int y, float u, float v, float w) {
    m_u->data(x, y) = u;
    m_v->data(x, y) = v;
    m_w->data(x, y) = w;
  }
...
/// compute the inverse mapping of the given correspondence volume; assumes the
/// correspodnences are smooth
