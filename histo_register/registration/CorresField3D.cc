    ...
    for (int z = 0; z < sourceCount; z++) {
      ...
      for (int y = 0; y < sourceHeight; y++) {
        for (int x = 0; x < sourceWidth; x++) {
          ...
          if (xProj > 2 && xProj < destWidth - 2 && yProj > 2 &&
              yProj < destHeight - 2 && zProj >= 0 && zProj < destCount) {
            ImageGrayU &destImage = dest[zProj];
            for (int yOffset = -radius; yOffset <= radius; yOffset++) {
              for (int xOffset = -radius; xOffset <= radius; xOffset++) {
                destImage.data(xProj + xOffset, yProj + yOffset) =
                    sourceImage.data(x, y);
              }
            }
          }
        }
...
void mapBack(const Array<CorresField3D> &cfSeq, const Array<ImageGrayU> &seq,
             Array<ImageGrayU> &seqMapped) {

  // get dimensions
  int length = cfSeq.count();
  disp(1, " mapBack length: %d, seq: %d, mapped: %d", length, seq.count(),
       seqMapped.count());
  assertAlways(length);
  ...
  for (int z = 0; z < length; z++) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        int   v     = 0;
        if (zDest >= 0 && zDest <= length - 1 &&
            imageMapped.inBounds(xDest, yDest)) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        if (zDestInt >= 0 && zDestInt < depth) {
          if (xDestInt >= 0 && xDestInt < width && yDestInt >= 0 &&
              yDestInt < height) {
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          if (cf.u(x, y) == marker) {
            ...
            int dxMin = -1, dxMax = 1, dyMin = -1, dyMax = 1, dzMin = -1,
                dzMax = 1;
            if (x == 0)
              dxMin = 0;
            if (x == width - 1)
