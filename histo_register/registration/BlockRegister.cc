...
public:
  // basic constructor
  BlockRegisterData(const String &outputPath, float blockSlicePerMrSlice,
                    int xCrop, int yCrop) {
    m_saveResults          = false;
...
#pragma omp parallel for reduction(+ : sumDiff, count)
      ...
      for (int bx = xCrop; bx < bWidth - xCrop; bx += step) {
        ...
        if (mz < 0 || mz >= m_mrNormImages.count()) {
          ...
          if (mNorm.inBounds(mx, my)) {
            ...
            if (m_saveResults) {
              if (mPt.z > 0) {
                int vNorm = round(interp(m_mrNormImages, (float)mPt.x,
                                         (float)mPt.y, (float)mPt.z));
                int vOrig = round(interp(m_mrOrigImages, (float)mPt.x,
                                         (float)mPt.y, (float)mPt.z));
  ...
  disp(1, "t: %3.1f, %3.1f, %3.1f, s: %4.2f, %4.2f, %4.2f, score: %f",
       transform.b.x, transform.b.y, transform.b.z, transform.a.data[0][0],
       transform.a.data[1][1], transform.a.data[2][2], score);
  if (m_saveResults) {
    disp(1, "a[ 0 ] = { %4.2f, %4.2f, %4.2f }", transform.a.data[0][0],
         transform.a.data[0][1], transform.a.data[0][2]);
    disp(1, "a[ 1 ] = { %4.2f, %4.2f, %4.2f }", transform.a.data[1][0],
         transform.a.data[1][1], transform.a.data[1][2]);
    disp(1, "a[ 2 ] = { %4.2f, %4.2f, %4.2f }", transform.a.data[2][0],
         transform.a.data[2][1], transform.a.data[2][2]);
    File logFile(m_outputPath + "/vis/log.txt", FILE_APPEND, FILE_TEXT);
    if (logFile.openSuccess()) {
      logFile.writeF(
          "t: %3.1f, %3.1f, %3.1f, a: [%4.2f, %4.2f, %4.2f], [%4.2f, %4.2f, "
          "%4.2f], [%4.2f, %4.2f, %4.2f], score: %f\n",
          transform.b.x, transform.b.y, transform.b.z, transform.a.data[0][0],
          transform.a.data[0][1], transform.a.data[0][2],
          transform.a.data[1][0], transform.a.data[1][1],
          transform.a.data[1][2], transform.a.data[2][0],
          transform.a.data[2][1], transform.a.data[2][2], score);
    }
    File paramFile(m_outputPath + "/linearTransform.txt", FILE_WRITE,
                   FILE_TEXT);
...
void registerBlockToMrNonLinear(const BlockRegisterData &brd) {
  ...
  disp(1, "srcNormSeq: %d, destNormSeq: %d, srcOrigSeq: %d, destOrigSeq: %d",
       srcNormSeq.count(), destNormSeq.count(), srcOrigSeq.count(),
       destOrigSeq.count());
  ...
  for (int z = 0; z < depth; z++) {
    ...
    if (z == depth / 2) {
      ...
      saveImage(destNormSeqMapped[z],
                brd.outputPath() + "/vis/destNormSeqMapped.png");
    }
...
void normalizeVolume(const Array<ImageGrayU> &origImages,
                     const Array<ImageGrayU> &masks,
                     Array<ImageGrayU> &normImages, const String &visPrefix) {
...
#pragma omp parallel for schedule(guided)
  ...
  for (int i = 0; i < origImages.count(); i++) {
    ...
    aptr<ImageGrayU> normImage =
        normalize(blurredOrigImages[i], masks[i], blurredMasks[i]);
...
void createRegistrationVolumes(Config &conf) {
  ...
  String blockCropPath =
      addDataPath(conf.readString("blockCropPath", "blockface/crop"));
  String blockSegPath =
      addDataPath(conf.readString("blockSegPath", "blockface/seg"));
  String mrRawPath =
      addDataPath(conf.readString("mrRawPath", "mri/rawFlash20"));
  String mrRegLinPath =
      addDataPath(conf.readString("mrRegLinPath", "mri/regLin"));
  String mrRegPath = addDataPath(conf.readString("mrRegPath", "mri/reg"));
  String histoRegPath =
      addDataPath(conf.readString("histoRegPath", "histo/reg"));
  ...
  if (useTransform) {
    ...
    // compute new spacing
    if (dAbs(transform.a.data[0][0]) < 1e-6 ||
        dAbs(transform.a.data[1][1]) < 1e-6 ||
        dAbs(transform.a.data[2][2]) < 1e-6) {
      warning("invalid transform: %f, %f, %f", transform.a.data[0][0],
              transform.a.data[1][1], transform.a.data[2][2]);
      return;
    }
...
#pragma omp section
#endif
      convertImagesToMghFile(blockCropPath,
                             dataPath() + "blockface/blockCrop.mgz", xSpacing,
                             ySpacing, zSpacing);
#ifdef HAVE_OPENMP
#pragma omp section
#endif
      convertImagesToMghFile(blockSegPath,
                             dataPath() + "blockface/blockSeg.mgz", xSpacing,
                             ySpacing, zSpacing);
#ifdef HAVE_OPENMP
#pragma omp section
#endif
      convertImagesToMghFile(mrRegLinPath,
                             dataPath() + "blockface/mrRegLin.mgz", xSpacing,
                             ySpacing, zSpacing);
#ifdef HAVE_OPENMP
#pragma omp section
#endif
      convertImagesToMghFile(mrRegPath, dataPath() + "blockface/mrReg.mgz",
...
#endif
      convertImagesToMghFile(blockCropPath,
                             dataPath() + "blockface/blockCropNoTransform.mgz",
                             xSpacing, ySpacing, zSpacing);
#ifdef HAVE_OPENMP
#pragma omp section
#endif
      convertImagesToMghFile(blockSegPath,
                             dataPath() + "blockface/blockSegNoTransform.mgz",
                             xSpacing, ySpacing, zSpacing);
    }
  }
...
void createRegistrationVolume(Config &conf) {
  ...
  if (useTransform) {
    ...
    if (autoScaleValues)
      convertImagesToMghFile(inputPath, outputFileName, xSpacing, ySpacing,
                             zSpacing);
    else
      convertImagesToMghFile(inputPath, outputFileName, xSpacing, ySpacing,
                             zSpacing, 0, 0, 1);
...
void registerBlockToMrOnePass(Config &conf) {
  ...
  for (int i = 0; i < blockFileList.count(); i++) {
    aptr<ImageGrayU> origInput =
        load<ImageGrayU>(blockCropPath + "/" + blockFileList[i]);
    brd.blockOrigImages().append(origInput.release());

    // if available read entro images:
    if (blockEntroPath != "") {
      aptr<ImageGrayU> entInput =
          load<ImageGrayU>(blockEntroPath + "/" + blockFileList[i]);
      brd.blockNormImages().append(entInput.release());
    } else // get mask for normalization step below:
    {
      aptr<ImageGrayU> segInput =
          load<ImageGrayU>(blockSegPath + "/" + blockFileList[i]);
  ...
  // display diagnostics
  disp(1, "block images: %d, MR images: %d", brd.blockOrigImages().count(),
       brd.mrOrigImages().count());
  ...
  if (blockEntroPath == "") {
    ...
    normalizeVolume(brd.blockOrigImages(), blockMaskImages,
                    brd.blockNormImages(), outputPath + "/vis/block");
    if (checkCommandEvents())
      return;
  }
  disp(1, "Normalizing MR slices");
  normalizeVolume(brd.mrOrigImages(), mrMaskImages, brd.mrNormImages(),
                  outputPath + "/vis/mr");
  if (checkCommandEvents())
    return;
...
void normalizeImages(Config &conf) {
  ...
  for (int i = 0; i < fileList.count(); i++) {
    aptr<ImageGrayU> origInput =
        load<ImageGrayU>(inputPath + "/" + fileList[i]);
...
void registerBlockToMr(Config &conf) {
  ...
  conf.writeString("inputPath",
                   conf.readString("blockCropPath", "blockface/crop"));
...
void createHistologyBlockIndexTable(Config &conf) {
  ...
  String outputFileName =
      addDataPath(conf.readString("outputFileName", "blockface/mapping.txt"));
