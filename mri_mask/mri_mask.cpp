...
int main(int argc, char *argv[]) {
  ...
  if (!mri_mask)
    ErrorExit(ERROR_BADPARM, "%s: could not read mask volume %s", Progname,
              argv[2]);

  printf("DoAbs = %d\n", DoAbs);

  /* Read transform and apply it to mri_mask */
  if (xform_fname != nullptr && strcmp(xform_fname, "identity.nofile")) {
    ...
    if (transform->type != MORPH_3D_TYPE) {
      ...
      if (lta->xforms[0].src.valid == 0) // For LTAs such as FSLREG_TYPE.
      {
        ...
        LTAmodifySrcDstGeom(lta, lta_src, nullptr);
      }
      ...
      if (lta->xforms[0].dst.valid == 0) {
        ...
        LTAmodifySrcDstGeom(lta, nullptr, lta_dst);
      }
    ...
    mri_tmp = TransformApplyType(transform, mri_mask, nullptr, InterpMethod);
    MRIfree(&mri_mask);
    mri_mask = mri_tmp;
  ...
  if (lta_dst)
    MRIfree(&lta_dst);
  mri_mask_orig = MRIcopy(mri_mask, nullptr);
  ...
  if (DoBB) {
    printf("Computing bounding box, npad = %d, %d, %d, %d, %d, %d\n", nPadBB[0],
           nPadBB[1], nPadBB[2], nPadBB[3], nPadBB[4], nPadBB[5]);
    region = REGIONgetBoundingBoxM(mri_mask, nPadBB);
    REGIONprint(stdout, region);
    mri_tmp = MRIextractRegion(mri_mask, nullptr, region);
    if (mri_tmp == nullptr)
      exit(1);
    MRIfree(&mri_mask);
    mri_mask = mri_tmp;
  ...
  if (keep_mask_deletion_edits) {
    MRI *mri_tmp;
    mri_tmp = MRImask(mri_out, mri_mask_orig, nullptr, 1, 1); // keep voxels = 1
    if (!mri_tmp)
...
void usage(int exit_val) {
  outputHelpXml(mri_mask_help_xml, mri_mask_help_xml_len);
  exit(exit_val);
