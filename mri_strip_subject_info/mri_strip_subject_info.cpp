...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_strip_subject_info.c,v 1.6 2011/03/02 00:04:25 nicks Exp $",
      "$Name:  $");
  ...
  if (!S_ISDIR(stat_buf.st_mode)) {
    errno = 0;
    ErrorExit(ERROR_BADFILE, "%s: %s is not a directory", Progname,
              argv[argc - 1]);
    exit(1);
  }
...
int fix_genesis(char *fname, char *dname) {
  ...
  if (buf == nullptr) {
    ErrorReturn(ERROR_NOMEMORY,
                (ERROR_NOMEMORY, "error: no memory to read file %s", fname));
  }

  if (fread(buf, 1, file_length, fp) != file_length) {
    free(buf);
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE, "error reading from file %s", fname));
  }
  ...
  memset(&(buf[exam_header_offset + 212]), 0x00,
         33); /*  referring physician        */
  memset(&(buf[exam_header_offset + 245]), 0x00,
         33); /*  diagnostician/radiologist  */
  ...
  if (fwrite(buf, 1, file_length, fp) != file_length) {
    free(buf);
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE, "error writing to file %s", out_fname));
  }
