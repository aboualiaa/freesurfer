...
char *srclabelfile = nullptr;
static char *sample_surf_file = nullptr;
LABEL *srclabel = nullptr;
LABEL *tmplabel = nullptr;
char *srcsubject = nullptr;
char *trglabelfile = nullptr;
LABEL *trglabel = nullptr;
char *trgsubject = nullptr;
char *trgsurface = "white";

char *regmethod = nullptr;
char *hemi = nullptr;
char *srchemi = nullptr;
char *trghemi = nullptr;
char *surfreg = "sphere.reg";
char *srcsurfreg = nullptr;
char *trgsurfreg = nullptr;
char *srcsurfregfile = nullptr; // just spec the file name with hemi
...
MRI_SURFACE *PaintSurf = nullptr;
char *PaintSurfName = nullptr;
MATRIX *SrcVolReg;
MATRIX *TrgVolReg;
MATRIX *InvTrgVolReg;
...
static char *src_annot_fname = nullptr;
static char *trg_annot_fname = nullptr;

int UseScannerCoords = 0;
char *DminminFile = nullptr;
MRI *mritmp = nullptr;
...
int main(int argc, char **argv) {
  ...
  MHT *TrgHash, *SrcHash = nullptr, *PaintHash = nullptr;
  ...
  if (usepathfiles) {
    ...
    srclabel = LabelRead(nullptr, srclabelfile);
    if (srclabel == nullptr) {
      fprintf(stderr, "ERROR reading %s\n", srclabelfile);
      exit(1);
    }
  ...
  if (DoDirectSurface) {
    ...
    if (srcsurfregfile == nullptr)
      sprintf(tmpstr, "%s/%s/surf/%s.%s", SUBJECTS_DIR, srcsubject, srchemi,
              srcsurfreg);
    else
      strcpy(tmpstr, srcsurfregfile);
    SrcSurfReg = MRISread(tmpstr);
    if (SrcSurfReg == nullptr) {
      fprintf(stderr, "ERROR: could not read %s\n", tmpstr);
      exit(1);
      if (strcmp(srchemi, trghemi) == 0)
        sprintf(tmpstr, "%s/%s/surf/%s.%s", SUBJECTS_DIR, trgsubject, trghemi,
                trgsurfreg);
    }

    if (MRISreadAnnotation(SrcSurfReg, src_annot_fname) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not read annotation from %s", Progname,
                src_annot_fname);
    ...
    SrcHash =
        MHTcreateVertexTable_Resolution(SrcSurfReg, CURRENT_VERTICES, hashres);

    TrgSurfReg->ct = SrcSurfReg->ct;
    for (vno = 0; vno < TrgSurfReg->nvertices; vno++) {
      vtrg = &TrgSurfReg->vertices[vno];
      srcvtxno = MHTfindClosestVertexNoXYZ(SrcHash, SrcSurfReg, vtrg->x,
                                           vtrg->y, vtrg->z, &dmin);
      if (srcvtxno < 0) {
  ...
  if (!strcmp(regmethod, "volume")) {
    ...
    if (RegFile == nullptr) {
      if (XFMFile) {
        ...
        SrcVolReg = DevolveXFM(srcsubject, nullptr, nullptr);
        if (SrcVolReg == nullptr)
          exit(1);

        /*** Load the Trg2Tal registration ***/
        TrgVolReg = DevolveXFM(trgsubject, nullptr, nullptr);
        if (TrgVolReg == nullptr)
          exit(1);

        /* Compte the Src-to-Trg Registration */
        InvTrgVolReg = MatrixInverse(TrgVolReg, nullptr);
      ...
      err = regio_read_register(RegFile, &pc, &ipr, &bpr, &fscale,
                                &Src2TrgVolReg, &float2int);
      if (err) {
  ...
  if (!strcmp(regmethod, "surface")) {
    ...
    if (strcmp(trgsubject, "ico")) {
      ...
      if (TrgSurf->nvertices != TrgSurfReg->nvertices) {
        printf(
            "ERROR: vertex mismatch between target surface and registration\n");
        exit(1);
      }
      if (DoRescale) {
        printf("Rescaling ... ");
    ...
    if (usehash) {
      ...
      TrgHash = MHTcreateVertexTable_Resolution(TrgSurfReg, CURRENT_VERTICES,
                                                hashres);
      printf("Building source registration hash (res=%g).\n", hashres);
      SrcHash = MHTcreateVertexTable_Resolution(SrcSurfReg, CURRENT_VERTICES,
                                                hashres);
    }
    if (useprojfrac) {
      sprintf(fname, "%s/%s/surf/%s.thickness", SUBJECTS_DIR, srcsubject,
              srchemi);
    ...
    /* handle source mask */
    if (srcmaskfile != nullptr) {
      printf("INFO: masking label\n");
      // SrcMask = MRIloadSurfVals(srcmaskfile, srcmaskfmt, NULL,
      //      srcsubject, hemi, NULL);

      SrcMask = MRISloadSurfVals(srcmaskfile, srcmaskfmt, SrcSurfReg, nullptr,
                                 nullptr, nullptr);
      if (SrcMask == nullptr)
        exit(1);
      tmplabel = MaskSurfLabel(srclabel, SrcMask, srcmaskthresh, srcmasksign,
                               srcmaskframe);
      if (tmplabel == nullptr)
        exit(1);
      LabelFree(&srclabel);
    ...
    if (DoPaint) {
      ...
      if (PaintSurf == nullptr) {
        printf("ERROR: could not read %s\n", tmpstr);
        exit(1);
      }
      if (usehash)
        PaintHash = MHTcreateVertexTable_Resolution(PaintSurf, CURRENT_VERTICES,
                                                    hashres);
    }
    ...
    for (n = 0; n < srclabel->n_points; n++) {
      ...
      if (usehash) {
        trgvtxno = MHTfindClosestVertexNo2(TrgHash, TrgSurfReg, SrcSurfReg,
                                           srcvtx, &dmin);
        if (trgvtxno < 0) {
      ...
      for (trgvtxno = 0; trgvtxno < TrgSurf->nvertices; trgvtxno++) {
        ...
        if (usehash) {
          ...
          if (srcvtxno < 0) {
            ...
            srcvtxno =
                MRISfindClosestVertex(SrcSurfReg, trgregvtx->x, trgregvtx->y,
                                      trgregvtx->z, &dmin, CURRENT_VERTICES);
            printf("dmin = %g\n", dmin);
            exit(1);
          }
        } else {
    ...
    if (label_ring > 0) {
      ...
      label_interior = LabelCopy(trglabel, nullptr);
    ...
    if (OutMaskFile) {
      ...
      outmask = MRISlabel2Mask(TrgSurfReg, trglabel, nullptr);
      if (DoOutMaskStat) {
        printf("Saving output statistic\n");
        for (n = 0; n < trglabel->n_points; n++) {
          MRIsetVoxVal(outmask, trglabel->lv[n].vno, 0, 0, 0,
                       trglabel->lv[n].stat);
        }
      }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      usehash = 0;
    else if (!strcasecmp(option, "--norevmap"))
      reversemap = 0;
    else if (!strcasecmp(option, "--revmap"))
      reversemap = 1;
    else if (!strcasecmp(option, "--usepathfiles"))
      usepathfiles = 1;
    else if (!strcmp(option, "--xfm-invert"))
      InvertXFM = 1;
    else if (!strcmp(option, "--src-invert"))
      SrcInv = 1;
      ...
      nargsused = 2;
    } else if (!strcmp(option, "--trg-invert"))
      TrgInv = 1;
    ...
    /* -------- source inputs ------ */
      ...
      srcmasksign = pargv[0];
      nargsused = 1;
      if (strcmp(srcmasksign, "abs") && strcmp(srcmasksign, "pos") &&
    ...
    /* -------- target inputs ------ */
      ...
      if (!strcmp(regmethod, "surf"))
        regmethod = "surface";
      if (!strcmp(regmethod, "vol"))
        regmethod = "volume";
      nargsused = 1;
...
static void print_usage() {
  ...
  printf("   --ring   N    dilate the label N times then remove the original "
         "before writing\n");
  ...
  printf("   --direct <src annot> <trg annot>     use the [xyz] coords for src "
         "and trg surfaces to do direct lookup\n");
  ...
  printf("\n");
  printf("   --paint dmax surfname : map to closest vertex on source surfname "
         "if d < dmax\n");
  printf("   --dmindmin overlayfile : bin mask with vertex of closest label "
         "point when painting\n");
  printf("   --baryfill surf surflabel delta outlabel\n");
  printf("   --label-cortex surface aseg outlabel : create a label like "
         "?h.cortex.label\n");
  printf("\n");
...
static void print_help() {
  ...
  printf(
      ...
      "                    --regmethod surface --hemi lh\n"
      "\n"
      "    This will map from fred to sally using sphere.reg. The "
      "registration\n"
      "    surface can be changed with --surfreg.\n"
      "\n"
      ...
      "  another subject). The label is mapped to vertices on the pial "
      "surface\n"
...
static void check_options() {
  ...
  if (!strcmp(regmethod, "surface") &&
      (!strcmp(srcsubject, "talairach") || !strcmp(trgsubject, "talairach"))) {
