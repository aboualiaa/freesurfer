...
int main(int argc, char *argv[]) {
  ...
  for (num = 0, i = 3; i < argc - 1; i++, num++) {
    ...
    if (MRISreadAnnotation(mris, annot_name) != NO_ERROR)
      ErrorExit(Gerror, "%s: MRISreadAnnotation(%s) failed", Progname,
                annot_name);
    if (mris->ct == NULL)
      ErrorExit(Gerror, "%s: MRISreadAnnotation(%s): no color table", Progname,
                annot_name);
    if (num == 0) // first one
    {
      ...
      for (j = 0; j < nvertices; j++) {
        ...
        if (counts[j] == NULL)
          ErrorExit(ERROR_NOMEMORY,
                    "%s: could not allocate %d counts at vertex %d", Progname,
                    mris->ct->nentries, nvertices);
      }
    } else if (mris->nvertices != nvertices)
      ErrorExit(ERROR_BADPARM,
                "%s: surface %d nvertices doesn't match previous ones (%d)",
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <annot name> <hemi> <subject> <subject>  ... "
         "<output annot>\n",
         Progname);
  exit(code);
}
