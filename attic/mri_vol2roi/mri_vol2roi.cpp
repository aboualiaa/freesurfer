...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_vol2roi.c,v 1.32 2011/03/02 00:04:25 nicks Exp $",
      "$Name:  $");
  ...
  if (srcregfile != NULL) {
    err = regio_read_register(srcregfile, &srcsubject, &ipr, &bpr, &intensity,
                              &Dsrc, &float2int_src);
    if (err)
      exit(1);
    printf("srcreg Dsrc -------------\n");
  ...
  if (mskvolid != NULL) {
    ...
    if (!msksamesrc) {
      mSrcMskVol =
          vol2vol_linear(mMskVol, Qmsk, NULL, NULL, Dmsk, Qsrc, Fsrc, Wsrc,
                         Dsrc, mSrcVol->height, mSrcVol->width, mSrcVol->depth,
    ...
    mri_binarize(mSrcMskVol, mskthresh, msktail, mskinvert, mSrcMskVol,
                 &nmskhits);
  } else {
    mSrcMskVol = NULL;
    nmskhits = 0;
  ...
  if (Label != NULL) {
    mFinalMskVol = label2mask_linear(
        mSrcVol, Qsrc, Fsrc, Wsrc, Dsrc, mSrcMskVol, Msrc2lbl, Label,
        labelfillthresh, float2int_src, &nlabelhits, &nfinalhits);

    if (mFinalMskVol == NULL)
      exit(1);
  } else {
  ...
  if (!oldtxtstyle) {
    ...
    if (Label != NULL)
      nlabelhits = CountLabelHits(mSrcVol, Qsrc, Fsrc, Wsrc, Dsrc, Msrc2lbl,
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    else if (!strcasecmp(option, "--oldtxtstyle"))
      oldtxtstyle = 1;
    else if (!strcasecmp(option, "--plaintxtstyle"))
      plaintxtstyle = 1;
    else if (!strcasecmp(option, "--mskinvert"))
      mskinvert = 1;
    else if (!strcmp(option, "--fixxfm"))
      fixxfm = 1;
    else if (!strcmp(option, "--nofixxfm"))
      fixxfm = 0;
...
static void print_help(void) {
  ...
  printf(
      ...
      "\n"
      "Output volumes will be saved as that indicated by the "
      "FSF_OUTPUT_FORMAT\n"
      ...
      "be the same size/geometry as the srcvol f. register.dat is the "
      "registration \n"
...
int check_format(char *trgfmt) {
  if (strcasecmp(trgfmt, "bvolume") != 0 && strcasecmp(trgfmt, "bfile") != 0 &&
      strcasecmp(trgfmt, "bshort") != 0 && strcasecmp(trgfmt, "bfloat") != 0 &&
      strcasecmp(trgfmt, "cor") != 0) {
    fprintf(stderr, "ERROR: format %s unrecoginized\n", trgfmt);
    fprintf(stderr,
            "Legal values are: bvolume, bfile, bshort, bfloat, and cor\n");
    exit(1);
  }
  return (0);
}
...
static void dump_options(FILE *fp) {
  ...
  if (labelfile != NULL) {
    ...
    if (src2lblregfile != NULL)
      fprintf(fp, "labelreg = %s\n", src2lblregfile);
    else
      fprintf(fp, "labelreg unspecified\n");
  ...
  if (mskvolid != NULL)
    ...
    fprintf(fp, "mskvol unspecified\n");
  if (mskvolid != NULL) {
    if (mskfmt != NULL)
      fprintf(fp, "mskfmt = %s\n", mskfmt);
    else
      fprintf(fp, "mskfmt unspecified\n");
    if (!msksamesrc) {
      if (mskregfile != NULL)
        fprintf(fp, "mskreg = %s\n", mskregfile);
      else
        fprintf(fp, "mskreg unspecified\n");
    } else
      fprintf(fp, "msk volume same as source\n");
...
LABEL *LabelReadFile(char *labelfile) {
  ...
  if (!sscanf(cp, "%d", &area->n_points))
    ErrorReturn(NULL, (ERROR_BADFILE, "%s: could not scan # of lines from %s",
                       Progname, fname));
  area->max_points = area->n_points;
  area->lv = (LABEL_VERTEX *)calloc(area->n_points, sizeof(LABEL_VERTEX));
  if (!area->lv)
    ErrorExit(ERROR_NOMEMORY,
              "%s: LabelRead(%s) could not allocate %d-sized vector", Progname,
              labelfile, sizeof(LV) * area->n_points);
  nlines = 0;
  ...
  if (!nlines)
    ErrorReturn(
        NULL, (ERROR_BADFILE, "%s: no data in label file %s", Progname, fname));
  return (area);
}
...
int CountLabelHits(MRI *SrcVol, MATRIX *Qsrc, MATRIX *Fsrc, MATRIX *Wsrc,
  ...
  LabelMskVol =
      label2mask_linear(mSrcVol, Qsrc, Fsrc, Wsrc, Dsrc, NULL, Msrc2lbl, Label,
                        labelfillthresh, float2int, &nlabelhits, &nfinalhits);
