...
#define PAD 4
...
#if 1
  if (!MRIgeometryMatched(mri_seg,
                          mri_intensity)) // resample intensity to be like seg
  {
    MRI *mri_tmp;
  ...
  for (i = 4; i < argc - 1; i++) {
    ...
    if (gca) // make target volume from it
    {
      ...
      mri_template =
          MRIalloc(scale * mri_target->width, scale * mri_target->height,
    ...
    if (gcam)
      ...
      mri_cor =
          MRIalloc(mri_seg->width, mri_seg->height, mri_seg->depth, MRI_FLOAT);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    case 'A':
      ...
      gca = GCAread(argv[2]);
      if (gca == NULL)
        ErrorExit(ERROR_NOFILE, "%s: could not read atlas from %s", Progname,
                  argv[2]);
      nargs = 1;
      break;
    case 'R':
...
static void print_usage(void) {
  fprintf(stderr,
          "usage: %s [options] <aseg volume> <norm volume> <xform> <label 1> "
          "<label 2> ... <output name>\n",
          Progname);
  fprintf(stderr, "where options are:\n");
  fprintf(stderr, "\t-s <sigma>\tapply a Gaussian smoothing kernel\n"
