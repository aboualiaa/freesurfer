...
IBMC_SLICE *IBMCinitSlice(IBMC_STACK *stack, int Sno) {
  ...
  slice->Sno = Sno;
  slice->Vno = stack->Vno;
  slice->mri = MRIcrop(stack->mri, 0, 0, Sno, stack->mri->width - 1,
                       stack->mri->height - 1, Sno);
...
int IBMCinitParams(IBMC_STACK *stack, int a2bmethod) {
  ...
  if (AllocAlpha)
    stack->params->alpha =
        (double *)calloc(sizeof(double), stack->params->nalpha);
...
IBMC_PARAMS *IBMCreadParams(char *fname) {
  ...
  // fprintf(stderr,"IBMCreadParams: %s %d %d
...
int IBMCalpha2Beta(IBMC *ibmc, int nthstack) {
  ...
  for (s = 0; s < stack->mri->depth; s++) {
    slice = stack->slice[s];
    for (k = 0; k < 3; k++)
      angles[k] = slice->beta[k + 3] * M_PI / 180;
    slice->Rbeta = MRIangles2RotMatB(angles, slice->Rbeta);
    for (k = 0; k < 3; k++)
      slice->Rbeta->rptr[k + 1][4] = slice->beta[k];
...
int IBMCsynthStack(MRI *src, IBMC_STACK *dst) {
  ...
  for (nthslice = 0; nthslice < dst->mri->depth; nthslice++) {
    ...
    MRIconst(slcmri->width, slcmri->height, slcmri->depth, slcmri->nframes, 0,
             slcmri);
    MRIvol2VolTkRegVSM(src, slcmri, dst->slice[nthslice]->iRs, SAMPLE_TRILINEAR,
                       0, NULL);
  }
  return (0);
}
...
int IBMCsetSliceBeta(IBMC_SLICE *slice, const double beta[6]) {
  ...
  for (k = 0; k < 6; k++)
    slice->beta[k] = beta[k];
  for (k = 0; k < 3; k++)
    angles[k] = beta[k + 3] * M_PI / 180;
  slice->Rs = MRIangles2RotMatB(angles, slice->Rs);
  for (k = 0; k < 3; k++)
    slice->Rs->rptr[k + 1][4] = beta[k];
...
int IBMCpairCost(IBMC *ibmc, int nthpair) {
  ...
  UpdateNeeded = 0;
  for (k = 0; k < 6; k++) {
    if (pair->betaAprev[k] != sA->beta[k])
      UpdateNeeded = 1;
    if (pair->betaBprev[k] != sB->beta[k])
      UpdateNeeded = 1;
  }
  if (!UpdateNeeded && !ForceUpdate && !pair->InitNeeded)
    return (0);
  pair->InitNeeded = 0;
  ...
  for (nth = 0; nth < nL; nth++) {
    ...
    if (colA[nth] < 0 || colA[nth] > sA->mri->width - 1)
      continue;
    if (rowA[nth] < 0 || rowA[nth] > sA->mri->height - 1)
      continue;
    if (colB[nth] < 0 || colB[nth] > sB->mri->width - 1)
      continue;
    if (rowB[nth] < 0 || rowB[nth] > sB->mri->height - 1)
      continue;
...
int IBMCinit(IBMC *ibmc, int a2bmethod) {
  ...
  ibmc->npairs = ibmc->stack[0]->mri->depth * ibmc->stack[1]->mri->depth +
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--force-update"))
      ForceUpdate = 1;
      ...
      checkoptsonly = 0;
    else if (!strcasecmp(option, "--a2b-rigid"))
      a2bmethod = IBMC_A2B_RIGID;
    else if (!strcasecmp(option, "--a2b-equal"))
      a2bmethod = IBMC_A2B_EQUAL;
    else if (!strcasecmp(option, "--line-min"))
      DoLineMin = 1;
...
float compute_powell_cost(float *params) {
  ...
  if (newopt && kbeta != 0) {
    ...
    printf("%4d %3d  %9.6f    %9.6f %9.6f   t=%7.3f\n", nCostEvaluations,
           kbeta - 1, params[kbeta], cost, copt, secCostTime / 60.0);
    fflush(stdout);
    for (k = 0; k < ibmc->nstacks; k++) {
...
int MinPowell(double ftol, double linmintol, int nmaxiters) {
  ...
  printf("Starting Powell nitersmax = %d, ftol=%g  linmintol %g, \n", nmaxiters,
         ftol, linmintol);
  err = OpenPowell2(pPowel, xi, ibmc->dof, ftol, linmintol, nmaxiters, &niters,
                    &fret, compute_powell_cost);
...
int IBMCsetParam(IBMC *ibmc, int nthparam, double paramval) {
  ...
  for (c = 0; c < ibmc->nstacks; c++) {
    for (r = 0; r < ibmc->stack[c]->params->nalpha; r++) {
      if (n == nthparam)
        ibmc->stack[c]->params->alpha[r] = paramval;
      n++;
    }
  }
  return (0);
...
double *IBMCgetParams(IBMC *ibmc, double *params) {
  ...
  if (params == NULL)
    params = (double *)calloc(sizeof(double), ibmc->dof);
