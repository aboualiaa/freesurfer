...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: oct_train.c,v 1.1 2014/04/17 13:30:03 fischl Exp $",
      "$Name:  $");
  ...
  if ((parms.nlayers == 1) && !force_dbn && !force_cdbn) {
    printf("training RBM\n");
    rbm = RBMalloc(rbm_input_type, parms.ksize * parms.ksize, ngroups,
                   max_label + 1, RBM_INPUT_IMAGE);
    ...
    printf("training DBN\n");
    dbn = DBNalloc(rbm_input_type, parms.nlayers, parms.ksize * parms.ksize,
                   nhidden, max_label + 1, RBM_INPUT_IMAGE);
  ...
  } else // convolutional deep belief network
  {
    ...
    cdbn = CDBNalloc(rbm_input_type, parms.nlayers, ksizes, nhidden,
                     max_label + 1, mri_inputs);
      ...
      CDBNwriteNetwork(cdbn, -1, &parms, layer);
    {
      ...
      mri_tmp = CDBNcreateOutputs(cdbn, &parms, mri_inputs, 0,
                                  cdbn->nlayers - 1, &mri_seg);
