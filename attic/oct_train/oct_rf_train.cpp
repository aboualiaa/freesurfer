 ...
 *
 * program for trainin a random forest from a set of convolutional layers, and
 * applying the trained forest to an OCT image
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: oct_rf_train.c,v 1.1 2014/09/26 15:27:08 fischl Exp $",
      "$Name:  $");
  ...
  if (FileType(int_fname) == TEXT_FILE) {
    ...
    for (i = 0; i < nfeatures; i++) {
      ...
      if (i == 0) {
        mri_inputs = MRIallocSequence(mri_tmp->width, mri_tmp->height,
                                      mri_tmp->depth, MRI_FLOAT, nfeatures);
    ...
    if (mri_inputs == NULL)
      ErrorExit(ERROR_NOFILE, "%s: could not read inputs from %s", Progname,
                int_fname);
  }
  ...
  RFtrain(rf, feature_fraction, training_fraction, training_classes,
          training_data, vl->nvox);
  ...
  strcat(out_fname, ".mgz");
  mri_out = MRIallocSequence(mri_inputs->width, mri_inputs->height,
                             mri_inputs->depth, MRI_FLOAT, 2);
  MRIcopyHeader(mri_inputs, mri_out);
  for (x = 0; x < mri_inputs->width; x++) {
    for (y = 0; y < mri_inputs->height; y++) {
      if ((MRIgetVoxVal(mri_inputs, x, y, 0, 0) <
           .0001) /*| (MRIcountValInNbhd(mri_inputs, 7, x, y, 0, 0)  > 0)*/)
