...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_make_template.c,v 1.26 2011/03/02 00:04:22 nicks Exp $",
      "$Name:  $");
    ...
    for (which = BUILD_PRIORS; which <= OFF_STATS; which++) {
      ...
      for (dof = 0, i = 1; i < argc - 1; i++) {
        ...
        if (which != BUILD_PRIORS) {
          ...
          if (!mri_T1)
            ErrorExit(ERROR_NOFILE, "%s: could not open volume %s", Progname,
                      fname);
        }
        ...
        if (which == BUILD_PRIORS) {
          ...
          if (which == ON_STATS) {
            ...
            MRIaccumulateMaskedMeansAndVariances(mri_T1, mri_binary, mri_dof, 0,
                                                 WM_MIN_VAL - 1, mri_mean,
                                                 mri_std);
          MRIfree(&mri_T1);
        }
        MRIfree(&mri_binary);
