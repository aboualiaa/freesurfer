...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: rbftest.c,v 1.7 2011/03/02 00:04:35 nicks Exp $",
      "$Name:  $");
  ...
  if (!fp)
    ErrorExit(ERROR_NOFILE, "%s: could not read input from %s", Progname,
              input_file_name);
...
static int read_line(VECTOR *v_obs, int obs_no, void *vfp, int same_class,
  ...
  for (n = 0; n <= obs_no; n++) {
    ...
    if (!cp)
      return (-1);
    sscanf(cp, "%f %f %d", &VECTOR_ELT(v_obs, 1), &VECTOR_ELT(v_obs, 2),
           &classnum);
