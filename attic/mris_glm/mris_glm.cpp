...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_glm.c,v 1.55 2011/05/05 15:28:03 greve Exp $",
      "$Name:  $");
  ...
  if (beta_in_id != NULL) {
    ...
    fflush(stdout);
    if (beta_in_fmt != NULL)
      beta = MRIreadType(beta_in_id, beta_in_fmtid);
    else
      beta = MRIread(beta_in_id);
    if (beta == NULL) {
    ...
    if (beta->width != eresvar->width || beta->height != eresvar->height ||
    ...
    for (nthsubj = 0; nthsubj < nsubjects; nthsubj++) {
      ...
      if (surfmeasure != NULL) {
        if (stringmatch(inputfmt, "paint") || stringmatch(inputfmt, "w") ||
          ...
          inputfname = surfmeasure;
      } else
        inputfname = inputlist[nthsubj];
      ...
      if (stringmatch(inputfmt, "curv"))
        tmpmri = MRISloadSurfVals(inputfname, "curv", SurfReg, subject, hemi,
                                  SUBJECTS_DIR);
        ...
        if (inputfmt != NULL)
          tmpmri = MRIreadType(inputfname, inputfmtid);
        else
          tmpmri = MRIread(inputfname);
        nvoxels = tmpmri->width * tmpmri->height * tmpmri->depth;
        if (nvoxels != SurfReg->nvertices) {
          printf("ERROR: number of vertices in input file (%d) != number of "
                 "vertices on surface (%d)\n",
        ...
        if (tmpmri->height > 1 || tmpmri->depth > 1) {
          printf("Reshaping\n");
          tmpmri2 =
              mri_reshape(tmpmri, SurfReg->nvertices, 1, 1, tmpmri->nframes);
          MRIfree(&tmpmri);
          tmpmri = tmpmri2;
        }
      }
      ...
      if (frame > 0) {
        ...
        for (vtx = 0; vtx < SurfReg->nvertices; vtx++)
          MRIFseq_vox(tmpmri2, vtx, 0, 0, 0) =
              MRIFseq_vox(tmpmri, vtx, 0, 0, frame);
        MRIfree(&tmpmri);
        tmpmri = tmpmri2;
      }
      ...
      if (!stringmatch(trgsubject, subject)) {
        ...
        fflush(stdout);
        tmpmri2 = surf2surf_nnfr(tmpmri, SurfReg, IcoSurf, &SrcHits, &SrcDist,
                                 &TrgHits, &TrgDist, 1, 1);
        if (tmpmri2 == NULL) {
      ...
      for (vtx = 0; vtx < IcoSurf->nvertices; vtx++)
        MRIFseq_vox(SrcVals, vtx, 0, 0, nthsubj) =
            MRIFseq_vox(tmpmri2, vtx, 0, 0, 0);
  ...
  for (nthsim = 0; nthsim < nsim; nthsim++) {
    ...
    if (beta_in_id == NULL && SynthPDF != 0) {
      if (SynthPDF == 1)
        MRIrandn(IcoSurf->nvertices, 1, 1, nsubjects, SynthGaussianMean,
                 SynthGaussianStd, SrcVals);
      if (SynthPDF == 2)
        MRIsampleCDF(IcoSurf->nvertices, 1, 1, nsubjects, SynthXCDF, SynthCDF,
                     SynthNCDF, SrcVals);

      if (nsmooth > 0)
        MRISsmoothMRI(IcoSurf, SrcVals, nsmooth, NULL, SrcVals);
    } /*End syntheisze */

    if (abs_flag)
      SrcVals = MRIabs(SrcVals, SrcVals);

    /* Save the input data */
    if (beta_in_id == NULL && yid != NULL && MCSim == 0) {
      if (MRIwriteAnyFormat(SrcVals, yid, yfmt, -1, NULL))
        exit(1);
      if (fsgd != NULL) {
    ...
    if (beta_in_id == NULL) {
      ...
      if (betaid != NULL && MCSim == 0)
        if (MRIwriteAnyFormat(beta, betaid, betafmt, -1, NULL))
          exit(1);
      ...
      if (eresid != NULL && MCSim == 0)
        if (MRIwriteAnyFormat(eres, eresid, eresfmt, -1, NULL))
          exit(1);
      ...
      if (yhatid != NULL && MCSim == 0) {
        ...
        if (MRIwriteAnyFormat(yhat, yhatid, yhatfmt, -1, NULL))
          exit(1);
        MRIfree(&yhat);
      }
    ...
    if (tid != NULL || sigid != NULL || cesid != NULL || tmaxfile != NULL ||
      ...
      if (cesid != NULL && MCSim == 0) {
        if (IsSurfFmt(cesfmt)) {
          if (IcoSurf == NULL)
            IcoSurf =
                MRISloadSurfSubject(trgsubject, hemi, surfregid, SUBJECTS_DIR);
    ...
    if (tid != NULL || sigid != NULL || tmaxfile != NULL || SynthPDF != 0) {
      ...
      if (C->rows == 1)
        ...
        t = fMRIcomputeF(ces, X, C, eresvar, t);
      if (tid != NULL && MCSim == 0) {
        if (IsSurfFmt(tfmt) && IcoSurf == NULL)
          IcoSurf =
              MRISloadSurfSubject(trgsubject, hemi, surfregid, SUBJECTS_DIR);
        if (MRIwriteAnyFormat(t, tid, tfmt, 0, IcoSurf))
          exit(1);
      }
      if (tmaxfile != NULL) {
        tmax = fabs(MRIFseq_vox(t, 0, 0, 0, 0));
        for (vtx = 0; vtx < t->width; vtx++) {
          if (IcoSurf->vertices[vtx].ripflag)
            continue;
      ...
      if (!MCSim) {
        if (sigid != NULL) {
          if (IsSurfFmt(sigfmt) && IcoSurf == NULL)
            IcoSurf =
                MRISloadSurfSubject(trgsubject, hemi, surfregid, SUBJECTS_DIR);
          if (MRIwriteAnyFormat(sig, sigid, sigfmt, 0, IcoSurf))
            exit(1);
        }
      }
    }
    ...
    if (MCSim) {
      ...
      for (nth_ithr = 0; nth_ithr < cht->n_ithr; nth_ithr++) {
        ...
        for (nth_sthr = 0; nth_sthr < cht->n_sthr; nth_sthr++) {
          ...
          scs = sclustMapSurfClusters(IcoSurf, ithr, -1, cht->ithr_signid, sthr,
                                      &NClusters, NULL, NULL);
          printf("  %6.2f %6.2f %3d \n", ithr, sthr, NClusters);
          if (NClusters > 0)
            cht->hits[nth_ithr][nth_sthr]++;
          free(scs);
        }
      }
      cht->nsim++;
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--force"))
      Force = 1;
    else if (!strcasecmp(option, "--parseonly"))
      ParseOnly = 1;
    else if (!strcasecmp(option, "--allowsubjrep"))
      fsgdf_AllowSubjRep = 1; /* external, see fsgdf.h */
    else if (!strcmp(option, "--xmatonly"))
      xmatonly = 1;
      ...
      for (m = 0; m < nargsused; m++)
        C->rptr[1][m + 1] = fvtmp[m];
      MatrixPrint(stdout, C);
      ...
      if (nth_is_arg(nargc, pargv, 1)) {
        ...
        eresvarfmtid = getfmtid(eresvarid);
      ...
      if (nth_is_arg(nargc, pargv, 1)) {
        ...
        eresvar_in_fmtid = getfmtid(eresvar_in_id);
...
static void print_usage(void) {
  ...
  printf(
      "   --icoorder   order : order of icosahedral tesselation (default 7)\n");
  ...
  printf(
      "   --y       name <fmt> : input data after resampling and smoothing\n");
  ...
  printf("   --contrast   fname : file containing group contrast matrix "
         "(ascii)\n");
  ...
  printf(
      "   --mcsim nsim fname nithr ithrlo ithrhi ithrsign nsthr sthrlo sthrhi: "
      "MC Simulation.\n");
  printf("\n");
...
static void print_help(void) {
  ...
  printf(
      ...
      "--hemi hemisphere\n"
      "\n"
      "Specify that the input data either the left (lh) or right (rh) "
      "hemisphere.\n"
      "\n"
      "--trgsubject subject\n"
      "\n"
      "Resample each input data set from the individual's surface to that of "
      "the\n"
      "target subject. \n"
      "\n"
      "--icoorder order\n"
      ...
      "--y name <fmt>\n"
      "\n"
      "Save the raw data (after resampling and smoothing) into a single "
      "'volume'.\n"
      ...
      "text file has two columns. The first column is the x at which the cdf "
      "is \n"
...
static void check_options(void) {
  ...
  if (desmtxfname != NULL && !stringmatch(gd2mtx_method, "none")) {
    printf(
        "ERROR: cannot specify a design matrix and create matrix from fsgd\n");
    exit(1);
  }
  ...
  if (ces != NULL || tid != NULL || sigid != NULL) {
    if (C == NULL) {
      printf(
          "ERROR: must specify a contrast matrix with ces, t, or sig output\n");
      exit(1);
    }
    if (C != NULL) {
...
static int checkfmt(char *fmt) {
  ...
  if (stringmatch(fmt, "curv") || stringmatch(fmt, "paint") ||
      stringmatch(fmt, "wfile") || stringmatch(fmt, "w"))
    return (MRI_VOLUME_TYPE_UNKNOWN);
