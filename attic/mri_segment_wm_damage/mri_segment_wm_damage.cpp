...
#define COULD_BE_DAMAGED_WM(l)                                                 \
  ((l == Left_Cerebral_White_Matter) || (l == Left_Cerebral_Cortex) ||         \
   (l == Left_Caudate) || (l == Left_Lateral_Ventricle) ||                     \
   (l == WM_hypointensities) || (l == Right_Caudate) ||                        \
   (l == Right_Lateral_Ventricle) || (l == Right_Cerebral_Cortex) ||           \
...
int main(int argc, char *argv[]) {
  ...
  if (!mri_aseg)
    ErrorExit(ERROR_NOFILE, "%s: could not read aseg volume %s", Progname,
              aseg_fname);
  ...
  compute_damaged_wm_statistics(mri_T1, mri_PD, mri_T2, l_damaged_wm,
                                &v_means[WM_hypointensities],
                                &m_inv_covariances[WM_hypointensities]);
  compute_label_statistics(mri_T1, mri_PD, mri_T2, mri_aseg,
                           Left_Cerebral_White_Matter,
                           &v_means[Left_Cerebral_White_Matter],
                           &m_inv_covariances[Left_Cerebral_White_Matter], 2);
  compute_label_statistics(mri_T1, mri_PD, mri_T2, mri_aseg, Left_Caudate,
                           &v_means[Left_Caudate],
                           &m_inv_covariances[Left_Caudate], 1);
  compute_label_statistics(mri_T1, mri_PD, mri_T2, mri_aseg,
                           Left_Lateral_Ventricle,
                           &v_means[Left_Lateral_Ventricle],
                           &m_inv_covariances[Left_Lateral_Ventricle], 2);
  compute_label_statistics(mri_T1, mri_PD, mri_T2, mri_aseg,
                           Right_Cerebral_White_Matter,
                           &v_means[Right_Cerebral_White_Matter],
                           &m_inv_covariances[Right_Cerebral_White_Matter], 2);
  compute_label_statistics(mri_T1, mri_PD, mri_T2, mri_aseg, Right_Caudate,
                           &v_means[Right_Caudate],
                           &m_inv_covariances[Right_Caudate], 1);
  compute_label_statistics(mri_T1, mri_PD, mri_T2, mri_aseg,
                           Right_Lateral_Ventricle,
                           &v_means[Right_Lateral_Ventricle],
                           &m_inv_covariances[Right_Lateral_Ventricle], 2);

  mri_damaged_wm = label_damaged_wm(mri_T1, mri_PD, mri_T2, mri_aseg, v_means,
                                    m_inv_covariances, transform, gca);
  ...
  for (ndamaged = x = 0; x < mri_PD->width; x++) {
    ...
    for (y = 0; y < mri_PD->height; y++) {
      ...
      for (z = 0; z < mri_PD->depth; z++) {
        ...
        if (pwm_damage <
            MIN_PRIOR) // only occurs where one of the above classes is possible
          continue;
        ...
        for (l = 0; l < MAX_CMA_LABELS; l++)
          if (v_means[l]) {
            ...
            if (prior < MIN_PRIOR &&
                l != WM_hypointensities) // don't force prior check for hypos -
                                         // they occur all over
              continue;
