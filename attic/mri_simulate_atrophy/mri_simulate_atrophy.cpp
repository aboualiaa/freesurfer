 ...
 *
 * program to simulate atrophic changes in a cortical or subcortical structure
 * by darkening the T1 intensities according to decrease volume fractions
 *
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  mri_noise = MRIrandn(mri_norm->width, mri_norm->height, mri_norm->depth, 1, 0,
                       noise_sigma, NULL);
  MRImaskZero(mri_noise, mri_norm, mri_noise);
  mri_norm_atrophy =
      MRIsimulateAtrophy(mri_norm, mri_aseg, target_labels[0], border_labels[0],
                         nlabels, atrophy_pct, NULL);
  ...
  if (!stricmp(option, "DEBUG_VOXEL")) {
    ...
    case 'L':
      ...
      printf("simulating atrophy in label %s (%d)\n",
             cma_label_to_name(target_labels[nlabels_set]),
             target_labels[nlabels_set]);
      ++nlabels_set;
      nargs = 1;
      break;
    case 'N':
...
MRI *MRIsimulateAtrophy(MRI *mri_norm, MRI *mri_aseg, int target_label,
  ...
  total_volume = MRIvoxelsInLabelWithPartialVolumeEffects(
      mri_aseg, mri_norm, target_label, mri_mixing, mri_nbr_labels);
  border_volume = 0;
  ...
  printf("total volume = %2.0fmm^3, border volume = %2.0f "
         "mm^3,reduction=%2.1f%%\n",
