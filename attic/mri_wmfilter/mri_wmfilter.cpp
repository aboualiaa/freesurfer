...
static char vcid[] =
    "$Id: mri_wmfilter.c,v 1.18 2011/03/02 00:04:26 nicks Exp $";
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_wmfilter.c,v 1.18 2011/03/02 00:04:26 nicks Exp $",
      "$Name:  $");
            ...
            for (dk = -ws2; dk <= ws2; dk++)
              for (di = -ws2; di <= ws2; di++)
                for (dj = -ws2; dj <= ws2; dj++) {
                  ...
                  if (!grayscale_plane)
                    f = MRIvox(mri_src, j + dj, i + di,
                               k + dk); /* segmented image */
                  else
                    f = MRIvox(mri_dst, j + dj, i + di,
                               k + dk); /* gray-scale image */
                  sum2v[u] += f * f;
                  sumv[u] += f;
                  nv[u] += 1;
                  n += 1;
