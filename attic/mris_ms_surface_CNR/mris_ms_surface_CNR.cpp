...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mris_ms_surface_CNR.c,v 1.4 2011/03/02 00:04:33 nicks Exp $",
      "$Name:  $");
  ...
  if (out_fname == NULL) {
    ErrorExit(ERROR_BADPARM, "%s: please specify output CNR file name.\n",
              Progname);
  }
  ...
  if (mris == NULL)
    ErrorExit(ERROR_NOFILE, "%s: could not read white surface %s", Progname,
              in_fname);
  ...
  if (use_thickness) {
    ...
    if (thickness_fname == NULL) {
      ...
      if (MRISreadCurvatureFile(mris, thickness_fname) != 0) {
        ErrorExit(ERROR_NOFILE, "%s:could not read thickness from file %s",
                  Progname, thickness_fname);
      }
    }
  }
  ...
  mri_gm_profile =
      MRIallocSequence(mris->nvertices, 1, 1, MRI_FLOAT, nvolumes_total);
  mri_wm_profile =
      MRIallocSequence(mris->nvertices, 1, 1, MRI_FLOAT, nvolumes_total);
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (i = 0; i < nvolumes; i++) {
      MRIsampleVolumeType(mri_flash[i], vx, vy, vz, &value_out,
                          SAMPLE_TRILINEAR);
    ...
    for (i = 0; i < nvolumes; i++) {
      MRIsampleVolumeType(mri_flash[i], vx, vy, vz, &value_in,
                          SAMPLE_TRILINEAR);
  ...
  // backup a copy since one copy is needed to compute means; seems unnecessary!
  // since CNR will use mean and covariance, not original values
  ...
  mri_gm_cov_components =
      MRIallocSequence(mris->nvertices, 1, 1, MRI_FLOAT, num_of_cov_components);
  mri_wm_cov_components =
      MRIallocSequence(mris->nvertices, 1, 1, MRI_FLOAT, num_of_cov_components);
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (i = 0; i < nvolumes_total; i++)
      for (j = i; j < nvolumes_total; j++) {
        MRIFseq_vox(mri_gm_cov_components, vno, 0, 0, index) =
            MRIFseq_vox(mri_gm_profile, vno, 0, 0, i) *
            MRIFseq_vox(mri_gm_profile, vno, 0, 0, j);
        MRIFseq_vox(mri_wm_cov_components, vno, 0, 0, index) =
            MRIFseq_vox(mri_wm_profile, vno, 0, 0, i) *
            MRIFseq_vox(mri_wm_profile, vno, 0, 0, j);
        index++;
      }
  }

  if (nSmoothSteps > 0) { // this is a weighted mean, not true mean; should be
                          // equivalent! Well, weighted one may be better
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (i = 0; i < nvolumes_total; i++)
      for (j = i; j < nvolumes_total; j++) {
        SW1->rptr[i + 1][j + 1] =
            MRIFseq_vox(mri_gm_cov_components, vno, 0, 0, index) -
            MRIFseq_vox(mri_gm_profile, vno, 0, 0, i) *
                MRIFseq_vox(mri_gm_profile, vno, 0, 0,
                            j); // sigma^2 = mean(x^2) - mean(x)*mean(x)
        SW2->rptr[i + 1][j + 1] =
            MRIFseq_vox(mri_wm_cov_components, vno, 0, 0, index) -
            MRIFseq_vox(mri_wm_profile, vno, 0, 0, i) *
                MRIFseq_vox(mri_wm_profile, vno, 0, 0,
                            j); // sigma^2 = mean(x^2) - mean(x)*mean(x)
        index++;
      }
    ...
    /* weight would be the optimal weighting, but just intermediate value for
     * CNR computation too */
    for (i = 0; i < nvolumes_total; i++) {
      weight[i] = 0.0;
      for (j = 0; j < nvolumes_total; j++) {
        weight[i] += InvSW->rptr[i + 1][j + 1] *
                     (MRIFseq_vox(mri_wm_profile, vno, 0, 0, j) -
                      MRIFseq_vox(mri_gm_profile, vno, 0, 0, j));
      }
    }
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "out") || !stricmp(option, "out_file") ||
      !stricmp(option, "out_name") || !stricmp(option, "cnr")) {
    out_fname = argv[2];
...
static void usage_exit(int code) {
  ...
  printf("\t This program computes the image CNR sampled along the white "
         "surface \n");
  ...
  printf("\t -use_thickness  use thickness to guide the pick of GM sample (by "
         "default) \n");
