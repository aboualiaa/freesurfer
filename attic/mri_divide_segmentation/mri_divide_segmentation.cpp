...
int main(int argc, char *argv[]) {
  ...
  if (mri->ct == NULL) {
    ...
    if (fs == NULL)
      ErrorExit(ERROR_NOFILE,
                "%s: volume %s does not have an embedded color table\n",
                Progname, argv[1]);
  ...
  for (i = 0; i < nparts; i++) {
    ...
    mri->ct->entries[indices[i]]->TissueType =
        mri->ct->entries[segno]->TissueType;
    printf("%s: index %d\n", name, indices[i]);
  }
  ...
  if (num == 0) // unused parcellation
    ErrorExit(ERROR_BADPARM, "%s: could not find any voxels with index %d",
              Progname, segno);
  ...
  if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
    printf("using voxel (%d, %d, %d) as closest (%2.3f mm) from centroid "
           "(%2.1f, %2.1f, %2.1f)\n",
  ...
  seconds = seconds % 60;
  fprintf(stderr,
          "segmentation subdivisioning took %d minutes and %d seconds.\n",
          minutes, seconds);
  exit(0);
  return (0);
}
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <input aseg> <segmentation index> <# of "
         "subdivisions> <output aseg>\n",
         Progname);
  exit(code);
}
