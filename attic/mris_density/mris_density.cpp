...
static char vcid[] =
    "$Id: mris_density.c,v 1.7 2011/03/02 00:04:31 nicks Exp $";
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_density.c,v 1.7 2011/03/02 00:04:31 nicks Exp $",
      "$Name:  $");
...
static void print_usage(void) {
  fprintf(stderr, "usage: %s [options] <input surface> <output map>\n",
          Progname);
}
...
static void print_help(void) {
  print_usage();
  fprintf(stderr,
          "\nThis program computes a density map and attaches it to each "
