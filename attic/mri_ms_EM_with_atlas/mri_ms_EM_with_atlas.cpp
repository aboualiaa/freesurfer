...
/* If set at 0.7, then Mahalanobis distance is 26.x; if set to 0.5, it's
 * only 14.6. Of course, lower threshold includes more partial volumed voxels,
 * and increasecovariance */
...
static int remove_cerebellum =
    0; // if 1, remove cerebellum in final synthesized image
...
double NbhdLikelihood(MRI **mri_mem, MRI *mri_mask, int x, int y, int z,
                      int label, int num_classes);

void fuzzyLDAweights(float *weights, MATRIX **F, float **centroids,
                     int classID1, int classID2, float size1, float size2,
                     int nvolumes_total);

void update_centroids1D(MRI *mri_flash, MRI **mri_mem, MRI *mri_mask,
                        float *centroids1D, int num_classes);

void MRI_FCM(MRI *mri_flash, MRI **mri_mem, MRI *mri_mask, float *centroids1D,
             int num_classes);

void MRI_EM(MRI *mri_flash, MRI **mri_mem, MRI *mri_mask, float *centroids1D,
            int num_classes);

void compute_bias(MRI **mri_mult, MRI **mri_flash, MRI **mri_mem,
                  MRI **mri_lihood, MRI *mri_mask, float **centroids,
                  MATRIX **F, int nvolumes_total, int num_classes);

void update_centroids(MRI **mri_flash, MRI **mri_mem, MRI **mri_likelihood,
                      MRI **, MRI *mri_mask, float **centroids,
                      int nvolumes_total, int num_classes);

void update_F(MATRIX **F, MRI **mri_flash, MRI **mri_mem, MRI **mri_lihood,
              MRI **, MRI *mri_mask, float **centroids, int nvolumes_total,
              int num_classes);
...
double distancems(MRI **mri_flash, MRI **, float *centroids, MATRIX *F,
                  double detF, int x, int y, int z);
double distancems_noF(MRI **mri_flash, MRI **mri_mult, float *centroids,
                      MATRIX *F, double detF, int x, int y,
                      int z); // ignore off-diagnal terms

// this one ignores covariance matrix
double distancems2(MRI **mri_flash, MRI **mri_mult, float *centroids, MATRIX *F,
                   double detF, int x, int y, int z, int channel);
void update_LDAmeans(MRI **mri_flash, MRI **mri_mem, MRI *mri_mask,
                     float *LDAmean1, float *LDAmean2, int nvolumes_total,
                     int classID1, int classID2, float threshold);

void computeLDAweights(float *weights, MRI **mri_flash, MRI **mri_mem,
...
int main(int argc, char *argv[]) {
  ...
  MRI *cerebellum_mask;
  MRI *mri_prior[MAX_IMAGES]; // prior from atlas; set to equal for all classes
                              // at background? Where is prior for dura?
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_ms_EM_with_atlas.c,v 1.10 2011/10/25 14:13:02 fischl Exp $",
      "$Name:  $");
  ...
  if (ldaflag) {
    if (class1 == class2 || class1 < 1 || class1 > num_classes || class2 < 1 ||
        class2 > num_classes) {
  ...
  if (gca_fname == NULL || xform_fname == NULL) {
    printf(
        "This version assumes that an atlas and xformation is needed! Exit\n");
    exit(0);
  }
  ...
  printf("-----I need to fix mri_likelihood for dura to make sure it doesn't "
         "got exchanged with CSF ---\n");
  printf("---- Maybe unnecessary, Dura seems mainly compete with GM ---\n");

  printf("How to make dura a subclass of GM?? ---\n");

  printf("After restricting the detF, this method seems working for all pfizer "
         "dat: still some questions are pounding: 1. should I reduce the detF "
         "threshold; 2. should I normalize the volume of all classes, like in "
         "G-K scheme; 3. weirdly, for some subjects, the cross-correlation, "
         "i.e., off-diagnal of F[1] (dura) is positive, some are negative! 4. "
         "Should make sure the detF threshold independent of input signal "
         "scales; 5. why when I didn't make all images to positive, "
         "convergence become worse? Due to gain field? Seems so. 6. Relative "
         "class size seems alright, i.e., CSF = 3.6, dura = 1.3, GM = 4.8, WM "
         "= 4.4, but from the GM seems too much from the output segmentation. "
         "If looking at the synthesize image, GM is much less, somehow EM "
         "makes many CSF classified as GM; 7. should I tie the centroids of "
         "dura to CSF and GM, as I did now?? well, I guess it still work if I "
         "didn't. 8. It's now clear that every algorithm requires fine-tuning "
         "to be working! 9. Can I get rid of dependency on aseg? May be using "
         "histogram to segment WM, or using FReesurfer on flash30.  \n");

  printf("May need to simply normalize all covariance matrices, and only rely "
         "on classSize to control the relative size of each class, like in G-H "
         "scheme\n");

  // This brain-masking procedure works perfect for 1001, but for 1006,
  // eye-sockets are included, while brain.mgz doesn't! So, still no clear
  // advantage over mri_watershed
  // after I add an additional threshold, i.e., flash30 cannot be higher than
  // flash5*1.5, then eye-socks are gone
  ...
  if (!transform) {
    ErrorExit(ERROR_NOFILE, "%s:could not open transform %s", Progname,
              xform_fname);
  }
  ...
  for (z = 0; z < depth; z++)
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_flash[0], transform, x, y, z, &xn,
                                  &yn, &zn)) {
          ...
          if (MLE_label == 7 || MLE_label == 8 || MLE_label == 16 ||
              MLE_label == 46 || MLE_label == 47) {
            MRIvox(cerebellum_mask, x, y, z) =
                1; // for later removal of cerebellum and brain stem and maybe
                   // sth else if needed
          }
      ...
      for (x = 0; x < width; x++) {
        ...
        for (i = 0; i < num_classes; i++) {
          if (total_prior < 0.7) {
            ...
            MRIFvox(mri_mem_old[i], x, y, z) =
                1.0; // or just set all to total_prior??
          }
        }
  ...
  if (nvolumes_total != 2) {
    ...
    if (T1_PD == 0) {
      printf("Using ratio of volume1 and volume 2 to find CSF, assuming volume "
             "1 is flash30 and volume 2 is flash 5. Of cause should be able to "
             "decide from fa of each volume\n");
      ...
      for (z = 0; z < depth; z++)
        for (y = 0; y < height; y++)
          for (x = 0; x < width; x++) {
            ...
            if (MRIFvox(mri_flash[1], x, y, z) > 2.5 * value &&
                MRIFvox(mri_prior[0], x, y, z) > 0.3) {
      ...
      for (z = 0; z < depth; z++)
        for (y = 0; y < height; y++)
          for (x = 0; x < width; x++) {
            ...
            if (MRIFvox(mri_flash[0], x, y, z) > 2200 &&
                MRIFvox(mri_prior[0], x, y, z) > 0.3) {
              MRIFvox(mri_mem[0], x, y, z) = 1;
            } else {
              MRIFvox(mri_mem[0], x, y, z) = MRIFvox(mri_prior[0], x, y, z);
  ...
  printf("Note that the above step was performed on original inputs, i.e., "
         "unnormalized!!\n");
  ...
  printf("Compute centroids for first time, and use it to adjust prior (due to "
         "bad MEF atlas, maybe I should use MPRAGE atlas)\n");

  update_centroids(mri_flash, mri_mem, mri_mem_old, mri_mult, mri_mask,
                   centroids, nvolumes_total, num_classes);
  update_F(F, mri_flash, mri_mem, mri_mem_old, mri_mult, mri_mask, centroids,
           nvolumes_total, num_classes);
  mean1 = centroids[0][0];
    ...
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          // seems I need to use true CSF to estimate initial threshold; or just
          // finish the atlas!! note this ad hoc fix of atlas works only for INU
          // corrected flash30
          if (val1 < mean1 + std1) {
            //   if(val1 < mean1 + 0.5*std1){
            //     val2 =  0.5*(MRIFvox(mri_prior[0], x, y, z) +
            //     MRIFvox(mri_prior[2], x, y, z));
  ...
  while (max_change > tolerance && iter < max_iters) {
    ...
    */
      ...
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          for (c = 0; c < num_classes; c++) {
            ...
            distance2 = distancems(mri_flash, mri_mult, centroids[c], F[c],
                                   detF[c], x, y, z);
            ...
            */
            ...
            //     nbhdP = 100* NbhdLikelihood(mri_mem_old, mri_mask, x, y, z,
            //     c, num_classes);
            ...
            if (!FZERO(mybeta)) {
              //     if(0){
              nbhdP =
                  NbhdLikelihood(mri_mem, mri_mask, x, y, z, c, num_classes);
              if (debug_flag && x == Gx && y == Gy && z == Gz) {
                printf("c= %d, nbhdP =%g \n", c, nbhdP);
              }
              distance2 *= nbhdP;
          ...
          {
            sum_of_distance =
                1.0; // this hard truncation seems a bad idea! but sounds good!
                     // Anyway, using it lead to slow convergence
            num_outlier++; // the number will keep growing
            // printf("(x,y,z) = (%d,%d,%d), sum_of_distance = %g\n",x,y,z,
            // sum_of_distance); ErrorExit(ERROR_BADPARM, "%s: overflow in
    ...
    /* printf("oldmem(%d) = %g, new = %g\n", nc, MRIFvox(mri_mem_old[nc], nx,
     * ny, nz),MRIFvox(mri_mem[nc], nx, ny, nz)); */ // no longer valid
    ...
    //    if(!no_INU &&  iter == 1)
    {
      compute_bias(mri_mult, mri_flash, mri_mem, mri_mem_old, mri_mask,
                   centroids, F, nvolumes_total, num_classes);
    }
  ...
  scale =
      (centroids[1][0] - centroids[2][0]) / (centroids[2][0] - centroids[0][0]);
  if (scale > 1.0) {
    scale = 1.0;
  }
  ...
  printf("How to use prior in the fuzzification??? Direct apply not working "
         "well\n");
#if 1
  for (z = 0; z < depth; z++)
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        for (c = 0; c < num_classes; c++) {
          ...
          // CSF is more smeared!
          distance2 = distancems_noF(mri_flash, mri_mult, centroids[c], F[c],
                                     detF[c], x, y, z);
          //   distance2 = exp(-0.5*distance2);

          //  if(c == 0) distance2 *= scale;

          // 10-25-05: try to add in prior; this indeed removed PVE voxels, but
          // outside is noisy the atlas need to be improved for outside! Maybe I
          ...
          //   distance2 = sqrt(MRIFvox(mri_prior[c],x,y,z))/(distance2 +
          //   1e-30);
  ...
  if (!no_INU) {
    ...
    for (i = 0; i < nvolumes_total; i++) {
      for (z = 0; z < depth; z++)
        for (y = 0; y < height; y++)
          for (x = 0; x < width; x++) {
            MRIFvox(mri_flash_backup[i], x, y, z) /=
                (MRIFvox(mri_mult[i], x, y, z) + 1e-20);
          }
  ...
  if (num_classes == 4) {
    ...
    for (z = 1; z < (depth - 1); z++)
      for (y = 1; y < (height - 1); y++)
        for (x = 1; x < (width - 1); x++) {
          ...
          label = 0;
          for (c = 1; c < num_classes; c++) {
            if (MRIFvox(mri_mem[c], x, y, z) >
                MRIFvox(mri_mem[label], x, y, z)) {
              label = c;
            }
          }
    ...
    if (classSize[NRindex[2]] > classSize[NRindex[1]]) {
      printf("Switch to make sure dura has more background neighbors, and as "
             "final label 1 \n");
      i = NRindex[1] - 1;
      j = NRindex[2] - 1;
      c = NRindex[1];
  ...
  if (nvolumes_total == 2 && num_classes == 4) {
    ...
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          //   if(MRIvox(mri_brainmask, x, y, z) == 0 || MRIvox(mri_mask,x,y,z)
          //   == 0){
          if (MRIvox(mri_mask, x, y, z) == 0) {
            MRIsetVoxVal(mri_tmp, x, y, z, 0, 0);
          } else {
            scale = MRIFvox(mri_flash_backup[1], x, y, z) /
                    (MRIFvox(mri_flash_backup[0], x, y, z) + 1e-10);
            /*     if(scale < 1.2)
             value =  0.9527*MRIFvox(mri_flash_backup[0],x,y,z) - 0.3039*(1 +
             0.5*MRIFvox(mri_mem[1], x, y, z))*
             MRIFvox(mri_flash_backup[1],x,y,z); else value =
             0.9527*MRIFvox(mri_flash_backup[0],x,y,z) -
             0.3039*MRIFvox(mri_flash_backup[1],x,y,z);
            */

            value = weight30 * MRIFvox(mri_flash_backup[0], x, y, z) +
                    weight5 * MRIFvox(mri_flash_backup[1], x, y, z);
    ...
    for (z = 0; z < depth; z++)
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          std2 += MRIgetVoxVal(mri_tmp, x, y, z, 0) *
                  MRIgetVoxVal(mri_tmp, x, y, z, 0) * value;
          denom2 += value;
        }
    mean1 /= (denom1 + 1e-30);
    ...
    printf("CSF mean and std in the synthesized volume before scaling are %g "
           "and %g\n",
           mean2, std2);

    threshold = mean2 - 2.0 * std2;
    if (mean1 < threshold) {
      printf("Something is wrong: WM mean is less than lowest CSF in "
             "synthesized image\n");
    }
  ...
  if (!ldaflag) {
        ...
        for (y = 0; y < height; y++)
          for (x = 0; x < width; x++) {
            ...
            for (i = 0; i < num_classes; i++) {
              ...
              MRIFvox(mri_mem[i], x, y, z) =
                  0.5 * (sin((value - 0.5) * 3.14159265) + 1.0);
            }
    ...
    if (fuzzy_lda == 0) {
      /* Compute class means */
      update_LDAmeans(mri_flash, mri_mem, mri_mask, LDAmean1, LDAmean2,
                      nvolumes_total, class1, class2,
                      MEMTHRESHOLD); /* 0.7 is a threshold, as to what voxels
                                        will be counted as in class1 */
      printf("class means computed \n");

      /* Compute Fisher's LDA weights */
      computeLDAweights(LDAweight, mri_flash, mri_mem, mri_mask, LDAmean1,
                        LDAmean2, nvolumes_total, class1, class2, MEMTHRESHOLD);
    } else {
      /* Use fuzzy covariance matrix and centroids to compute LDA weights */
      update_F(F, mri_flash, mri_mem, mri_mem_old, mri_mult, mri_mask,
               centroids, nvolumes_total, num_classes);

      fuzzyLDAweights(LDAweight, F, centroids, class1, class2, 1, 1,
                      nvolumes_total);
    }
...
static int get_option(int argc, char *argv[]) {
  ...
  /*E* Interpolation method.  Default is trilinear, other options are
    ...
    if (InterpMethod == SAMPLE_SINC) {
      if ((argc < 4) ||
          !strncmp(argv[3], "-",
                   1)) /*E* i.e. no sinchalfwindow value supplied */
      {
    ...
    if ((argc < 3) ||
        !strncmp(argv[2], "-", 1)) /*E* i.e. no sinchalfwindow value supplied */
    {
      printf("using sinc interpolation (default windowwidth is 6)\n");
    } else {
...
double NbhdLikelihood(MRI **mri_mem, MRI *mri_mask, int x, int y, int z,
  ...
  for (index = 0; index < 6; index++) {
    ...
    } else /* soft penalty, will tie more closely to a smoothing of probability
              itself*/
    {
...
void fuzzyLDAweights(float *weights, MATRIX **F, float **centroids,
  ...
  for (m1 = 1; m1 <= nvolumes_total; m1++) {
    for (m2 = m1; m2 <= nvolumes_total; m2++) {
      SW->rptr[m1][m2] = F[classID1]->rptr[m1][m2] * size1 +
                         F[classID2]->rptr[m1][m2] *
                             size2; /* index starts from 1 for matrix */
    }
  ...
  for (m1 = 1; m1 <= nvolumes_total; m1++) {
    ...
    for (m2 = 1; m2 <= nvolumes_total; m2++) {
      weights[m1 - 1] += InvSW->rptr[m1][m2] * (centroids[classID1][m2 - 1] -
                                                centroids[classID2][m2 - 1]);
    }
    sumw += weights[m1 - 1];
...
void computeLDAweights(float *weights, MRI **mri_flash, MRI **mri_mem,
  ...
  if (denom <= 0.0) {
    ErrorExit(ERROR_BADPARM,
              "%s: overflow in computing fuzzy covariance matrix.\n", Progname);
  }
  ...
  if (1) {
    ...
    printf("Mahalanobis Distance between the two classes in the original "
           "feature space is %g\n",
           Mdistance);
  }
...
void update_centroids(MRI **mri_flash, MRI **mri_mem, MRI **mri_lihood,
                      MRI **mri_mult, MRI *mri_mask, float **centroids,
                      int nvolumes_total, int num_classes) {
  /* This step stays the same as FCM, just that the membership function is now
   * the probability function. No, not the same! No more power of membership
   */
...
double distancems(MRI **mri_flash, MRI **mri_mult, float *centroids, MATRIX *F,
                  double detF, int x, int y, int z) {
  ...
  for (i = 0; i < rows; i++) {
    data1 = MRIFvox(mri_flash[i], x, y, z) -
            MRIFvox(mri_mult[i], x, y, z) * centroids[i];
    for (j = 0; j < rows; j++) {
      data2 = MRIFvox(mri_flash[j], x, y, z) -
              MRIFvox(mri_mult[j], x, y, z) * centroids[j];
...
double distancems_noF(MRI **mri_flash, MRI **mri_mult, float *centroids,
                      MATRIX *F, double detF, int x, int y, int z) {
  /* F would be the inverse of the covariance matrix of the class */
  int i, rows;
  double data1;
...
double distancems2(MRI **mri_flash, MRI **mri_mult, float *centroids, MATRIX *F,
  ...
  data1 = MRIFvox(mri_flash[channel], x, y, z) -
          MRIFvox(mri_mult[channel], x, y, z) * centroids[channel];
        ...
        for (x = 0; x < width; x++) {
          if (MRIvox(mri_mask, x, y, z) <= 0) {
            ...
            for (c = 0; c < num_classes; c++) {
              ...
              scale = scale / (scale + kappa);
              if (T1_PD == 0) {
                tmp_h += centroids[c][i] * centroids[c][i] *
                         F[c]->rptr[i + 1][i + 1] *
                         MRIFvox(mri_mem[c], x, y, z) * scale;
              } else {
                tmp_h += centroids[c][i] * centroids[c][i] *
                         MRIFvox(mri_mem[c], x, y, z) * scale;
              }
              if (T1_PD == 0) {
                tmp_f += MRIFvox(mri_flash[i], x, y, z) * centroids[c][i] *
                         F[c]->rptr[i + 1][i + 1] *
                         MRIFvox(mri_mem[c], x, y, z) * scale;
              } else {
                tmp_f += MRIFvox(mri_flash[i], x, y, z) * centroids[c][i] *
        ...
        for (x = 0; x < width; x++) {
          ...
          for (m1 = 0; m1 < nvolumes_total; m1++) {
            ...
            for (m2 = m1; m2 < nvolumes_total; m2++) {
              ...
              data2 =
                  MRIFvox(mri_flash[m2], x, y, z) - gain2 * centroids[c][m2];
      ...
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          for (c = 0; c < num_classes; c++) {
            ...
            distance2 = piI[c] * exp(-0.5 * distance2 * distance2 / sigmaI[c]) /
                        (sqrt(sigmaI[c]) + 1e-30);
          ...
          if (sum_of_distance <= 0.0) {
            ErrorExit(ERROR_BADPARM,
                      "%s: overflow in computing membership function.\n",
                      Progname);
          }
...
void set_equilavent_classes(int *equivalent_classes) {
  ...
  for (i = 0; i < MAX_CLASSES; i++) {
    ...
    // Need some clever string comparison to automatically build equivalent
    // classes; for now I will just manually set
