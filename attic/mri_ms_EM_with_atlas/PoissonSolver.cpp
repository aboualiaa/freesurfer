...
void rstrct(MRI *out, MRI *in, int Xo, int Yo, int Zo, int Xi, int Yi, int Zi) {
  ...
  for (knew = 0; knew < Zo; knew++)
    for (inew = 0; inew < Yo; inew++)
      for (jnew = 0; jnew < Xo; jnew++) {
        ...
        MRIFvox(out, jnew, inew, knew) =
            0.125 *
            (MRIFvox(in, jold, iold, kold) + MRIFvox(in, jold + 1, iold, kold) +
             MRIFvox(in, jold, iold + 1, kold) +
             MRIFvox(in, jold + 1, iold + 1, kold) +
             MRIFvox(in, jold, iold, kold + 1) +
             MRIFvox(in, jold + 1, iold, kold + 1) +
             MRIFvox(in, jold, iold + 1, kold + 1) +
             MRIFvox(in, jold + 1, iold + 1, kold + 1));
      }

  return;
}

void interp(MRI *out, MRI *in, int Xo, int Yo, int Zo, int Xi, int Yi, int Zi) {
  ...
  for (kc = 0; kc < Zo; kc++)
    for (ic = 0; ic < Yo; ic++)
      for (jc = 0; jc < Xo; jc++) {
        ...
        HY = (i == (Yi - 1));
        HZ = (k == (Zi - 1));
        MRIFvox(out, jc, ic, kc) = MRIFvox(
            in, j, i, k); /* 0.5*(in[k][i][j] + in[k+1-HZ][i+1-HY][j+1-HX]); */
      }
  return;
}

void addin(MRI *uf, MRI *uc, MRI *res, int Xo, int Yo, int Zo, int Xi, int Yi,
           int Zi) {
  int i, j, k;
...
void resid(MRI *res, MRI *u, MRI *rhs, MRI *hu, int XN, int YN, int ZN,
           int lev) {
  int i, j, k;
  int h;
  float cmu;
  ...
  for (k = 0; k < ZN; k++) {
    ...
    for (i = 0; i < YN; i++) {
      ...
      for (j = 0; j < XN; j++) {
        ...
        MRIFvox(res, j, i, k) =
            MRIFvox(hu, j, i, k) * MRIFvox(u, j, i, k) + MRIFvox(rhs, j, i, k) -
            cmu * (MRIFvox(u, j, i, prek) + MRIFvox(u, j, i, nextk) +
                   MRIFvox(u, j, prei, k) + MRIFvox(u, j, nexti, k) +
                   MRIFvox(u, prej, i, k) + MRIFvox(u, nextj, i, k) -
                   MRIFvox(u, j, i, k) * 6);
...
void octant(MRI *u, MRI *rhs, MRI *hu, float hsquare, int XN, int YN, int ZN,
  ...
  for (k = kc; k < ZN; k += 2) {
    ...
    for (i = ic; i < YN; i += 2) {
      ...
      for (j = jc; j < XN; j += 2) {
        ...
        MRIFvox(u, j, i, k) =
            ((MRIFvox(u, j, i, prek) + MRIFvox(u, j, i, nextk) +
              MRIFvox(u, j, prei, k) + MRIFvox(u, j, nexti, k) +
              MRIFvox(u, prej, i, k) + MRIFvox(u, nextj, i, k)) -
             MRIFvox(rhs, j, i, k) * hsquare) /
            (6 + tmpv);
...
void Jacobi(MRI *u, MRI *rhs, MRI *hu, int lev, int XN, int YN, int ZN,
            int iter) {
  ...
  float alpha =
      0.9; /* 0.66666666666; alpha has to be large to converge in 3D */
  float hsquare;
    ...
    for (k = 0; k < ZN; k++) {
      ...
      for (i = 0; i < YN; i++) {
        ...
        for (j = 0; j < XN; j++) {
          ...
          MRIFvox(tmpvol, j, i, k) =
              (1 - alpha) * MRIFvox(u, j, i, k) +
              alpha *
                  (MRIFvox(u, j, i, prek) + MRIFvox(u, j, i, nextk) +
                   MRIFvox(u, j, prei, k) + MRIFvox(u, j, nexti, k) +
                   MRIFvox(u, prej, i, k) + MRIFvox(u, nextj, i, k) -
...
void Gauss_Seidel(MRI *u, MRI *rhs, MRI *hu, int lev, int XN, int YN, int ZN,
                  int iter) {
  int h, i;
  float hsquare;
...
void multigrid(MRI *u, MRI *f, MRI *hu, int XN, int YN, int ZN) {
  ...
  for (j = maxlevel - 1; j >= 1; j--) {
    rstrct(ih[j], ih[j + 1], lo[j], no[j], mo[j], lo[j + 1], no[j + 1],
           mo[j + 1]);
  }
  ...
  for (iters = 1; iters <= 10; iters++) {
    ...
    for (j = maxlevel - 1; j >= 1; j--) {
      rstrct(irho[j], irho[j + 1], lo[j], no[j], mo[j], lo[j + 1], no[j + 1],
             mo[j + 1]);
    }

    /*Now solve the PDE at level 1, the coarsest level */
    slvsml(iu[1], irho[1], ih[1], lo[1], no[1],
           mo[1]); /* Initial solution on coarest grid */

    /* Now start Full-Multigrid V or W cycle */
    for (j = 2; j <= maxlevel; j++) {

      interp(iu[j], iu[j - 1], lo[j], no[j], mo[j], lo[j - 1], no[j - 1],
             mo[j - 1]); /* Get the initial guess of the solution of the
                            original eq */

      copymem(irhs[j], irho[j], lo[j], no[j],
              mo[j]); /* Set up right hand side */
      ...
      for (jcycle = 1; jcycle <= 1; jcycle++) {
        for (jj = j; jj >= 2; jj--) { /* Down stroke of the V */
          if (JACOBI)
            ...
            Gauss_Seidel(iu[jj], irhs[jj], ih[jj], jj, lo[jj], no[jj], mo[jj],
                         n1);

          resid(ires[jj], iu[jj], irhs[jj], ih[jj], lo[jj], no[jj], mo[jj],
                jj); /* Defect */
          ...
          for (m = 0; m < mo[jj - 1]; m++) /*loop over image */
            for (n = 0; n < no[jj - 1]; n++)
              for (l = 0; l < lo[jj - 1]; l++) {
                MRIFvox(iu[jj - 1], l, n, m) =
                    0; /* Initial value for errors at levels from j-1 to 1 which
                          are iu[j-1] ~ iu[1] and are zeros */
              }

          rstrct(irhs[jj - 1], ires[jj], lo[jj - 1], no[jj - 1], mo[jj - 1],
                 lo[jj], no[jj],
                 mo[jj]); /* mf, nf is the size of the first parameter */
        }

        slvsml(iu[1], irhs[1], ih[1], lo[1], no[1],
               mo[1]); /* Bottom of the V */
        /* Now iu[1] stores the solution of the error at the coarest level */

        for (jj = 2; jj <= j; jj++) { /*Upard stroke of V */
          addin(iu[jj], iu[jj - 1], ires[jj], lo[jj], no[jj], mo[jj],
                lo[jj - 1], no[jj - 1], mo[jj - 1]);
