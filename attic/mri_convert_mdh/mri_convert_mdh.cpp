...
#define MDH_LASTSCANINSLICE                                                    \
  (1 << 29) // indicates last scan in slice (needed for time stamps)
#define MDH_TREFFECTIVEBEGIN                                                   \
  (1 << 30) // indicates the begin time stamp for TReff (triggered measurement)
#define MDH_TREFFECTIVEEND                                                     \
  (1 << 31) // indicates the end time stamp for TReff (triggered measurement)
...
#define MDH_RETRO_ENDOFMEAS                                                    \
  (1 << 13) // Marks an ADC at the end of the measurement
#define MDH_RETRO_REPEATTHISHEARTBEAT                                          \
  (1 << 14) // Repeat the current heartbeat when this bit is found
#define MDH_RETRO_REPEATPREVHEARTBEAT                                          \
  (1 << 15) // Repeat the previous heartbeat when this bit is found
#define MDH_RETRO_ABORTSCANNOW (1 << 16) // Just abort everything
#define MDH_RETRO_LASTHEARTBEAT                                                \
  (1 << 17) // This adc is from the last heartbeat (a dummy)
#define MDH_RETRO_DUMMYSCAN                                                    \
  (1 << 18) // This adc is just a dummy scan, throw it away
#define MDH_RETRO_ARRDETDISABLED                                               \
  (1 << 19) // Disable all arrhythmia detection when this bit is found
...
int main(int argc, char **argv) {
  ...
  sprintf(fname, "cp %s/%s.asc %s/%s.asc", srcdir, the_basename, outdir,
          the_basename);
  system(fname);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      dumpmdh = 1;
    else if (!strcasecmp(option, "--binary"))
      BinaryADCDump = 1;
    else if (!strcasecmp(option, "--adcstats"))
      adcstats = 1;
    else if (!strcasecmp(option, "--nopcn"))
      DumpPCN = 0;
    // else if (!strcasecmp(option, "--rev"))     rev = 1;
      ...
      if (nargc < 1)
        argnerr(option, 1);
      if (strcmp(pargv[0], "echo") == 0)
        FastestDim = 1;
      else if (strcmp(pargv[0], "line") == 0)
        FastestDim = 2;
      else {
      ...
      if (nargc < nEchos)
        argnerr(option, nEchos);
      for (n = 0; n < nEchos; n++)
        sscanf(pargv[n], "%f", &TE[n]);
      nargsused = nEchos;
    } else {
...
static void print_usage(void) {
  ...
  printf("\n");
  // printf("   --info       : no ouput, just dump info (default with no
  // outdir)\n");
}
/* --------------------------------------------- */
static void print_help(void) {
  ...
  printf(
      ...
      "Probes or converts a Siemens proprietary format (MDH) file to MGH "
      "format. \n"
      "\n"
      ...
      "contains the measured k-space data in Siemens MDH format (which "
      "includes\n"
...
MDH *ReadMiniHeader(FILE *fp, MDH *mdh, int mdhversion) {
  if (mdhversion == 15)
    return (ReadMiniHeader15(fp, mdh));
  if (mdhversion == 21)
    return (ReadMiniHeader21(fp, mdh));
  return (NULL);
}
/*----------------------------------------------------------*/
MDH *ReadMiniHeader15(FILE *fp, MDH *mdh) {
  ...
  if ((mdh->BitMask1 & MDH_BM_PHASECOR) != 0)
    mdh->IsPCN = 1;
  else
    mdh->IsPCN = 0;
...
int PrintMiniHeader(FILE *fp, MDH *mdh) {
  ...
  fprintf(fp, "SP  %7.2f %7.2f %7.2f \n", mdh->SlicePosSag, mdh->SlicePosCor,
          mdh->SlicePosTra);
  fprintf(fp, "Ncols        %d\n", mdh->Ncols);
...
int MDHfastestDim(char *measoutpath) {
  ...
  if (mdh1->Echo != mdh2->Echo)
    FastestDim = 1; // Echo is fastest
  else
    FastestDim = 2; // Phase Encode is fastest
...
int *MDHind2sub(int *siz, int ind, int ndim, int *sub) {
  ...
  if (sub == NULL)
    sub = (int *)calloc(sizeof(int), ndim);
...
int DumpMDH_VB13(char *measfile) {
  ...
  while (1) {
    ...
    printf("BM1 ");
    for (n = 0; n < 32; n++)
      printf("%d ", (mdh.aulEvalInfoMask[0] >> n) & 1);
    printf("\n");
    printf("BM2 ");
    for (n = 0; n < 32; n++)
      printf("%d ", (mdh.aulEvalInfoMask[1] >> n) & 1);
    printf("\n");

    if (mdh.aulEvalInfoMask[0] & MDH_BM_PHASECOR) {
      if (mdh.aulEvalInfoMask[0] & MDH_ONLINE)
        isPCN = 2;
      else
        isPCN = 1;
    } else
    ...
    ADCSizeBytes =
        mdh.ushSamplesInScan * 2 * sizeof(float); // 2 = real/imag, 4 = float
    fseek(fp, ADCSizeBytes, SEEK_CUR);
    nth = nth + 1;
  }
  fclose(fp);
...
int ConvertMDH_VB13(char *measfile, char *outbase) {
  ...
  while (1) {
    ...
    if (mdh.aulEvalInfoMask[0] & MDH_ACQEND)
      break;
    ...
    isReflected = 0;
    if (mdh.aulEvalInfoMask[0] & MDH_REFLECT)
      isReflected = 1;
    ...
    fprintf(fpi, "%3d %3d %3d   %3d %3d %3d   %3d %3d %3d  %d  %6d\n",
            mdh.ushLine, mdh.ushChannelId, mdh.ushSet, mdh.ushEcho,
            mdh.ushPhase, mdh.ushSeg, mdh.ushRepetition, mdh.ushPartition,
            mdh.ushSlice, isReflected, mdh.ulTimeStamp);
