 ...
 * @brief main program for applying a stacked autoencoder for feature
extraction.
 *
...
static int x0 = -1;
static int x1;
// for some reason y0 and y1 are defined in /usr/include/bits/mathcalls.h so
// have to include annoying _
static int y0_;
static int y1_;
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_apply_autoencoder.c,v 1.2 2013/11/22 19:41:39 fischl Exp $",
      "$Name:  $");
  ...
  if (mri == NULL)
    ErrorExit(ERROR_NOFILE, "%s: could not read input volume from %s", Progname,
              in_fname);
...
#define SIGMA_AE .1
          ...
          if (VectorNormalizedDot(v_ae_src, v_ae_dst) < 0)
            DiagBreak();
          MRIsetVoxVal(mri_ae_out, x, y, z, 0,
                       VectorNormalizedDot(v_ae_src, v_ae_dst));
          ...
          MRIsetVoxVal(mri_dot_out, x, y, z, 0,
                       VectorNormalizedDot(v_src, v_dst));
        }
    VectorFree(&v_src);
    VectorFree(&v_ae_src);
    VectorFree(&v_dst);
    ...
    if (x0 >= 0) {
      ...
      MRIextractInto(mri_ae_out, mri_tmp, 0, 0, 0, mri_ae_out->width,
                     mri_ae_out->height, mri_ae_out->depth, x0, y0_, z0);
      MRIfree(&mri_ae_out);
      mri_ae_out = mri_tmp;

      mri_tmp = MRIclone(mri_scaled, NULL);
      MRIextractInto(mri_dot_out, mri_tmp, 0, 0, 0, mri_dot_out->width,
                     mri_dot_out->height, mri_dot_out->depth, x0, y0_, z0);
      MRIfree(&mri_dot_out);
      mri_dot_out = mri_tmp;
    }
  ...
  if (synthesize) {
    ...
    for (x = sae->whalf; x < mri->width - sae->whalf; x++)
      for (y = sae->whalf; y < mri->height - sae->whalf; y++)
        for (z = sae->whalf; z < mri->depth - sae->whalf; z++) {
          ...
          MRIclear(mri_tmp2);
          MRIextractInto(mri_tmp, mri_tmp2, 0, 0, 0, mri_tmp2->width,
                         mri_tmp2->height, mri_tmp2->depth, x, y, z);
    ...
    if (mri_orig->type == MRI_UCHAR) {
      ...
      MRIextractInto(mri_total, mri_scaled, 0, 0, 0, mri_total->width,
                     mri_total->height, mri_total->depth, x0, y0_, z0);
      MRIwrite(mri_scaled, "synth.mgz");
      MRIscalarMul(mri_total, mri_total, 255.0f);
      mri_total = MRIchangeType(mri_total, MRI_UCHAR, 0, 255, 1);
    }
    MRIextractInto(mri_total, mri_orig, 0, 0, 0, mri_total->width,
                   mri_total->height, mri_total->depth, x0, y0_, z0);
