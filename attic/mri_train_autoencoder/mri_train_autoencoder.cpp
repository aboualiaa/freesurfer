 ...
 * @brief main program for creating and training a stacked autoencoder for
feature extraction.
 *
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_train_autoencoder.c,v 1.2 2013/11/14 16:17:41 fischl Exp $",
      "$Name:  $");
    ...
    for (x = sae->whalf; x < mri_orig->width - sae->whalf; x++)
      for (y = sae->whalf; y < mri_orig->height - sae->whalf; y++)
        for (z = sae->whalf; z < mri_orig->depth - sae->whalf; z++) {
          ...
          MRIsetVoxVal(mri_synth, x, y, z, 0,
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "DEBUG_VOXEL")) {
    ...
    case 'S':
      scale = atof(argv[2]);
      fprintf(stderr,
              "scaling hidden layer to be %2.2f as large as input layer\n",
              scale);
      nargs = 1;
      break;
    case 'M':
