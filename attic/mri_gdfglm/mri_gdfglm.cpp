...
int main(int argc, char **argv) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_gdfglm.c,v 1.9 2011/05/05 15:28:03 greve Exp $",
      "$Name:  $");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--keepid"))
      KeepSubjId = 1;
...
static void print_help(void) {
  ...
  printf(
      ...
      "and each row an observation for that subject. The arguments of "
      "--depvar\n"
...
static int WriteAllClassDat(char *base, FSGD *fsgd, MATRIX *y, MATRIX *yhat,
  ...
  for (nc = 0; nc < fsgd->nclasses; nc++) {
    err = WriteClassDat(base, fsgd->classlabel[nc], fsgd, y, yhat, X, beta);
    if (err)
      return (1);
  }
...
MATRIX *DVTgetDepVar(DVT *dvt, int nDepVars, char **DepVarList,
  ...
  if (nDepVars != 0) {
    for (n = 0; n < dvt->D->cols; n++) {
      ...
      if (index == -1)
        wc = 0;
      else if (DepVarWeight != NULL)
        wc = DepVarWeight[index];
      else
        wc = 1.0;
      W->rptr[n + 1][1] = wc;
    }
