...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4)
    ErrorExit(ERROR_BADPARM,
              "usage: %s <medial_axis_file> <output_file> <input_volume_1> "
              "<input_volume_2> ",
              Progname);
  ...
  if ((fp_in = fopen(argv[1], "r")) == NULL) {
    ErrorReturn(ERROR_BADFILE,
                (ERROR_BADFILE,
                 "medial axis measurement: file %s does not exist!", argv[1]));
  }
  ...
  if ((fp_out = fopen(argv[1], "w")) == NULL) {
    ErrorReturn(
        ERROR_BADFILE,
        (ERROR_BADFILE, "intensity output: file %s does not exist!", argv[2]));
  }
  fprintf(stdout, "reading medial axis atoms from %s\n", argv[2]);
  ...
  for (i = 0; i < n_sample; i++) {
    ...
    MRIsampleVolumeFrameType(mri_pd, cc_tal_x, y, x, 0, SAMPLE_TRILINEAR,
                             &val_pd);
    MRIsampleVolumeFrameType(mri_t1, cc_tal_x, y, x, 0, SAMPLE_TRILINEAR,
                             &val_t1);
