...
#define WSIZE 3
  ...
  if (!mri_aseg)
    ErrorExit(ERROR_NOFILE, "%s: could not read aseg file %s", Progname,
              input_aseg_name);
  ...
  for (nchanged = i = 0; i < vl_border->nvox; i++) {
    ...
    if (x == Gx && y == Gy && z == Gz)
      DiagBreak();
    svm_inputs = CAbuildInputsAtVoxel(
        vl_border, i, mri_smooth, mri_grad, mri_laplacian, mri_dtrans,
        mri_dtrans_grad, mri_2nd_deriv_s, WSIZE, NSCALES, svm_inputs, 0);
  ...
  printf("%d voxels changed (%2.1f%%)\n", nchanged,
         100 * nchanged / (float)vl_border->nvox);
