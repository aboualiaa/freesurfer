...
main(int argc, char *argv[])
{
  ...
  if (auto_detect_stats) {
    ...
    MRIScomputeClassModes(mris, mri_T1, &white_mode, &gray_mode, NULL,
                          &white_std, &gray_std,
                          NULL); // move this later if want to use the stds
  ...
  for (n_averages = max_white_averages, i = 0; n_averages >= min_white_averages;
    ...
    MRIScomputeBorderValues(
        mris, mri_T1, mri_smooth, MAX_WHITE, max_border_white, min_border_white,
        min_gray_at_white_border, max_border_white /*max_gray*/, current_sigma,
  ...
  if (orig_pial) {
    ...
    MRIScomputeMetricProperties(
        mris); // shouldn't this be done whenever orig_pial is used??? Maybe
               // that's why the cross-intersection was caused
  }
  /*    parms.l_convex = 1000 ;*/
  mri_T1 = mri_T1_pial;
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
  ...
  for (j = 0; j <= 0; parms.l_intensity *= 2, j++) /* only once for now */
  {
    ...
    for (n_averages = max_pial_averages, i = 0; n_averages >= min_pial_averages;
      ...
      /*
        ...
        after border values have been computed so that it doesn't mess up
        gradients.
      */
  ...
  if (in_out_in_flag) {
    ...
    for (n_averages = max_white_averages, i = 0;
      ...
      MRIScomputeBorderValues(
          ...
          parms.fp, GRAY_WHITE, NULL, 0, 0, NULL, -1, -1);
      MRISfindExpansionRegions(mris);
      if (vavgs) {
        fprintf(stderr, "averaging target values for %d iterations...\n",
                vavgs);
