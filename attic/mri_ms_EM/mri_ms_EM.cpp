...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_ms_EM.c,v 1.8 2011/10/04 18:37:30 nicks Exp $",
      "$Name:  $");
      ...
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          if (mask_subcortical == 1 && label_fname != NULL) {
             ...
             * label ==4 || label == 5 || label == 43 || label == 44 ) */
            if ((label >= 9 && label <= 12) || (label >= 48 && label <= 51)) {
              MRIvox(mri_mask, x, y, z) = 0;
            }
          }
  ...
  for (i = 0; i < num_classes; i++) {
    centroids1D[i] =
        min_val + (i + 1.0) * (max_val - min_val) / (1.0 + num_classes);
  }
          ...
          if (MRIvox(mri_mask, x, y, z) <= 0) {
            ...
            for (c = 0; c < num_classes; c++) {
              ...
              tmp_h += centroids[c][i] * centroids[c][i] *
                       F[c]->rptr[i + 1][i + 1] * MRIFvox(mri_mem[c], x, y, z) *
                       scale;
              // tmp_h += centroids[c][i] * centroids[c][i] *
...
static MRI *GaussianBlur(MRI *input, MRI *output, float myalpha) {
  ...
  for (d = 0; d < (int)depth; d++)
    for (i = 0; i < (int)height; i++) {
      ...
      MRIFvox(y1, 1, i, d) = a1 * (float)MRIgetVoxVal(input, 1, i, d, 0) +
                             a2 * (float)MRIgetVoxVal(input, 0, i, d, 0) +
                             b1 * MRIFvox(y1, 0, i, d);
    }
  ...
  for (d = 0; d < (int)depth; d++)
    for (i = 0; i < (int)height; i++)
      for (j = 2; j < (int)width; j++) {
        MRIFvox(y1, j, i, d) = a1 * (float)MRIgetVoxVal(input, j, i, d, 0) +
                               a2 * (float)MRIgetVoxVal(input, j - 1, i, d, 0) +
                               b1 * MRIFvox(y1, j - 1, i, d) +
                               b2 * MRIFvox(y1, j - 2, i, d);
      }

  for (d = 0; d < (int)depth; d++)
    for (i = 0; i < (int)height; i++) {
      MRIFvox(y2, width - 1, i, d) = 0.0;
      MRIFvox(y2, width - 2, i, d) =
          (float)MRIgetVoxVal(input, width - 1, i, d, 0) * a3 +
          b1 * MRIFvox(y2, width - 1, i, d);
    }
    ...
    for (i = 0; i < (int)height; i++)
      for (j = (int)(width - 3); j >= 0; j--) {
        MRIFvox(y2, j, i, d) = (float)MRIgetVoxVal(input, j + 1, i, d, 0) * a3 +
                               ...
                               b1 * MRIFvox(y2, j + 1, i, d) +
                               b2 * MRIFvox(y2, j + 2, i, d);
      }
  ...
  for (d = 0; d < (int)depth; d++)
    for (j = 0; j < (int)width; j++) {
      ...
      MRIFvox(y1, j, 1, d) = a1 * MRIFvox(r, j, 1, d) +
                             a2 * MRIFvox(r, j, 0, d) +
                             b1 * MRIFvox(y1, j, 0, d);
    }
  ...
  for (d = 0; d < (int)depth; d++)
    for (j = 0; j < (int)width; j++)
      for (i = 2; i < (int)height; i++) {
        MRIFvox(y1, j, i, d) =
            a1 * MRIFvox(r, j, i, d) + a2 * MRIFvox(r, j, i - 1, d) +
            b1 * MRIFvox(y1, j, i - 1, d) + b2 * MRIFvox(y1, j, i - 2, d);
      }

  for (d = 0; d < (int)depth; d++)
    for (j = 0; j < (int)width; j++) {
      MRIFvox(y2, j, height - 1, d) = 0;
      MRIFvox(y2, j, height - 2, d) = a3 * MRIFvox(r, j, height - 1, d) +
                                      b1 * MRIFvox(y2, j, height - 1, d);
    }
  ...
  for (d = 0; d < (int)depth; d++)
    for (j = 0; j < (int)width; j++)
      for (i = (int)height - 3; i >= 0; i--) {
        MRIFvox(y2, j, i, d) =
            a3 * MRIFvox(r, j, i + 1, d) + a4 * MRIFvox(r, j, i + 2, d) +
            b1 * MRIFvox(y2, j, i + 1, d) + b2 * MRIFvox(y2, j, i + 2, d);
      }
  ...
  for (i = 0; i < (int)height; i++)
    for (j = 0; j < (int)width; j++) {
      ...
      MRIFvox(y1, j, i, 1) = a1 * MRIFvox(r, j, i, 1) +
                             a2 * MRIFvox(r, j, i, 0) +
                             b1 * MRIFvox(y1, j, i, 0);
    }
  ...
  for (i = 0; i < (int)height; i++)
    for (j = 0; j < (int)width; j++)
      for (d = 2; d < (int)depth; d++)
        MRIFvox(y1, j, i, d) =
            a1 * MRIFvox(r, j, i, d) + a2 * MRIFvox(r, j, i, d - 1) +
            b1 * MRIFvox(y1, j, i, d - 1) + b2 * MRIFvox(y1, j, i, d - 2);

  for (i = 0; i < (int)height; i++)
    for (j = 0; j < (int)width; j++) {
      MRIFvox(y2, j, i, depth - 1) = 0.0;
      MRIFvox(y2, j, i, depth - 2) =
          a3 * MRIFvox(r, j, i, depth - 1) + b1 * MRIFvox(y2, j, i, depth - 1);
    }
  ...
  for (i = 0; i < (int)height; i++)
    for (j = 0; j < (int)width; j++)
      for (d = (int)depth - 3; d >= 0; d--) {
        MRIFvox(y2, j, i, d) =
            a3 * MRIFvox(r, j, i, d + 1) + a4 * MRIFvox(r, j, i, d + 2) +
            b1 * MRIFvox(y2, j, i, d + 1) + b2 * MRIFvox(y2, j, i, d + 2);
      }
