...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mris_svm_classify.c,v 1.6 2011/03/02 00:04:34 nicks Exp $",
      "$Name:  $");
  ...
  if (log_fname != NULL)
    printf("logging results to %s, true_class = %s\n", log_fname,
           true_class > 0 ? svm->class1_name : svm->class2_name);
  ...
  if (MRISreadCurvature(mris, input_name) != NO_ERROR)
    ErrorExit(ERROR_BADPARM, "%s: could not read curvature from %s",
              input_name);
  ...
  if (nannotations > 0) {
    ...
    if (MRISreadAnnotation(mris, annot_name) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not read annot file %s for subject %s",
  ...
  sprintf(fname, "%s/%s/surf/%s.%s", subjects_dir, output_subject_name, hemi,
          surf_name);
  ...
  if (mris->nvertices != svm->ninputs)
    ErrorExit(ERROR_BADPARM,
              "%s: svm input (%d) does not match # of "
              "surface vertices (%d)",
  ...
  if (log_fname) {
    ...
    fprintf(fp, "%-30.30s %s %d %f %f\n", subject_name, hemi,
            (true_class * classification) > 0, classification, true_class);
    fclose(fp);
  }
  free(inputs);
  MRISfree(&mris);
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <subject> <hemi> <canon surf name> <input curv "
         "name> "
