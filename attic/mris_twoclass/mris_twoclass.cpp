...
static int cvector_extract_best_avg(float *vbest_avgs, float *vsrc, float *vdst,
                                    int navgs, int num);
...
#endif
static int cvector_combine_variances_into_stderrs(float *c1_var, float *c2_var,
                                                  int num_class1,
                                                  int num_class2, int nvertices,
                                                  float *c_var);
...
static double cvector_compute_snr(float *c1_mean, float *c2_mean, float *verror,
...
static int cvector_track_best_snr(
    float *vsnr, float *vbest_snr, float *vbest_avgs, float *c1_mean,
    float *c2_mean, float *c1_best_mean, float *c2_best_mean, float *c1_var,
    float *c2_var, float *c1_best_var, float *c2_best_var,
    float **c1_avg_thickness, float **c2_avg_thickness,
    float **c1_best_thicknesses, int nc1, float **c2_best_thicknesses, int nc2,
    int avgs, int num, float fthresh, int *pnum_found);

static int cvector_track_best_stats(
    float *vsnr, float *vbest_snr, float *vbest_avgs, float *c1_mean,
    float *c2_mean, float *c1_best_mean, float *c2_best_mean, float *c1_var,
    float *c2_var, float *c1_best_var, float *c2_best_var,
    float **c1_avg_thickness, float **c2_avg_thickness,
...
int main(int argc, char *argv[]) {
  ...
  char **av, *curv_name, *surf_name, *hemi, fname[STRLEN], *cp, *subject_name,
      subjects_dir[STRLEN], *out_prefix, **c1_subjects, **c2_subjects,
      *stat_suffix;
  int ac, nargs, n, num_class1, num_class2, i, nvertices, avgs, max_snr_avgs,
      nlabels = 0, num_found = 0, total_found, num_above_thresh;
#if 0
  int          done, vno ;
#endif
  float **c1_thickness, **c2_thickness, *curvs, *total_mean, *c1_mean, *c2_mean,
      *c1_best_mean, *c2_best_mean, *c1_best_var, *c2_best_var, *class_mean,
      *c1_var, *c2_var, *class_var, *pvals, **c1_avg_thickness, *vbest_snr,
      *vbest_avgs, *vtotal_var, *vsnr, **c2_avg_thickness, *vbest_pvalues;
#if 0
...
#define ARGV_OFFSET 5
  ...
  if (read_dir) /* read in precomputed data */
  {
    ...
    for (n = 0; n < num_class1 + num_class2; n++) {
      ...
      if (MRISreadValues(mris, fname) != NO_ERROR)
        ErrorExit(Gerror, "%s: could not read curvature file %s", Progname,
                  fname);
      if (area)
    ...
    for (n = 0; n < num_class1 + num_class2; n++) {
      ...
      if (wfile_flag) {
        if (MRISreadValues(mris, fname) != NO_ERROR)
          ErrorExit(Gerror, "%s: could no read w file file %s", Progname,
                    fname);
      } else {
        if (MRISreadCurvatureFile(mris, fname) != NO_ERROR)
          ErrorExit(Gerror, "%s: could no read curvature file %s", Progname,
                    fname);
        if (normalize_flag)
    ...
    if (read_dir == NULL) /* recompute everything */
    {
      ...
      if (use_no_distribution)
        snr = cvector_compute_dist_free_snr(c1_thickness, num_class1,
                                            c2_thickness, num_class2, c1_mean,
                                            c2_mean, vsnr, nvertices, &i);
      else if (use_stats) {
        snr = cvector_compute_pvalues(c1_mean, c1_var, c2_mean, c2_var,
                                      num_class1, num_class2, vsnr, nvertices,
                                      stat_type, &i);
        fprintf(stderr, "raw pval %2.2f, n=%2.4f, d=%2.4f, vno=%d\n", snr,
                c1_mean[i] - c2_mean[i], sqrt(vtotal_var[i]), i);
        cvector_track_best_stats(
            vsnr, vbest_snr, vbest_avgs, c1_mean, c2_mean, c1_best_mean,
            c2_best_mean, c1_var, c2_var, c1_best_var, c2_best_var,
            c1_thickness, c2_thickness, c1_thickness, num_class1, c2_thickness,
            num_class2, 0, nvertices, fthresh, &num_found);
      } else {
        snr = cvector_compute_snr(c1_mean, c2_mean, vtotal_var, vsnr, nvertices,
                                  &i, 0.0f, stat_type);
        fprintf(stderr, "raw SNR %2.2f, n=%2.4f, d=%2.4f, vno=%d\n", sqrt(snr),
                c1_mean[i] - c2_mean[i], sqrt(vtotal_var[i]), i);
        cvector_track_best_snr(
            vsnr, vbest_snr, vbest_avgs, c1_mean, c2_mean, c1_best_mean,
            c2_best_mean, c1_var, c2_var, c1_best_var, c2_best_var,
            c1_thickness, c2_thickness, c1_thickness, num_class1, c2_thickness,
            num_class2, 0, nvertices,
      ...
      */
      ...
      for (avgs = 1; avgs <= max_avgs; avgs++) {
        ...
        if (use_buggy_snr)
          ...
          cvector_add_variances(c1_var, c2_var, num_class1, num_class2,
                                vtotal_var, nvertices);
        if (use_no_distribution)
          snr = cvector_compute_dist_free_snr(
              c1_avg_thickness, num_class1, c2_avg_thickness, num_class2,
              c1_mean, c2_mean, vsnr, nvertices, &i);
        else if (use_stats) {
          snr = cvector_compute_pvalues(c1_mean, c1_var, c2_mean, c2_var,
                                        num_class1, num_class2, vsnr, nvertices,
                                        stat_type, &i);
          cvector_track_best_stats(
              vsnr, vbest_snr, vbest_avgs, c1_mean, c2_mean, c1_best_mean,
              c2_best_mean, c1_var, c2_var, c1_best_var, c2_best_var,
              c1_avg_thickness, c2_avg_thickness, c1_thickness, num_class1,
              c2_thickness, num_class2, avgs, nvertices, fthresh, &num_found);
        } else {
          snr = cvector_compute_snr(
              c1_mean, c2_mean, vtotal_var, vsnr, nvertices, &i,
              bonferroni ? log((double)avgs) : 0.0f, stat_type);
          cvector_track_best_snr(
              vsnr, vbest_snr, vbest_avgs, c1_mean, c2_mean, c1_best_mean,
              c2_best_mean, c1_var, c2_var, c1_best_var, c2_best_var,
              c1_avg_thickness, c2_avg_thickness, c1_thickness, num_class1,
              c2_thickness, num_class2, avgs, nvertices,
              fthresh / ((float)(num_class1 + num_class2) / 2), &num_found);
        }
        total_found += num_found;
        if (write_flag && DIAG_VERBOSE_ON && !use_stats) {
          fprintf(fp, "%d %2.1f  %2.2f %2.2f %2.2f ", avgs, sqrt((float)avgs),
                  sqrt(snr), c1_mean[i] - c2_mean[i], sqrt(vtotal_var[i]));
          fflush(fp);
          for (n = 0; n < num_class1; n++)
        ...
        if (!(avgs % 10)) {
          if (use_stats)
            printf("%d vertices found at scale %d, max p = %2.1f,"
                   ...
                   total_found, avgs, snr, i, c1_mean[i], c2_mean[i], c1_var[i],
                   c2_var[i]);
          else
      ...
      snr = cvector_compute_pvalues(c1_best_mean, c1_best_var, c2_best_mean,
                                    c2_best_var, num_class1, num_class2,
                                    vbest_pvalues, nvertices, stat_type, NULL);
  ...
  } else /* use fixed scale smoothing kernel */
  {
    ...
    if (use_no_distribution)
      snr = cvector_compute_dist_free_snr(
          c1_avg_thickness, num_class1, c2_avg_thickness, num_class2,
          c1_best_mean, c2_best_mean, vsnr, nvertices, &i);
    else
      snr = cvector_compute_snr(
          c1_best_mean, c2_best_mean, vtotal_var, vsnr, nvertices, &i,
          bonferroni ? log((double)navgs) : 0.0f, stat_type);
  }

  /* now write out the optimal scale and statistic to out_prefix */
  cvector_compute_pvalues(c1_best_mean, c1_best_var, c2_best_mean, c2_best_var,
                          num_class1, num_class2, pvals, nvertices, stat_type,
                          NULL);
  ...
  if (conf > 0) {
    ...
    tconf =
        find_t_for_confidence_interval(conf / 2, num_class1 + num_class2 - 2);
    ...
    sprintf(fname, "%s_%s_p%2.1fsigma-%s.w", out_prefix, stat_suffix, sigma,
            hemi);
    ...
    sprintf(fname, "%s_%s_m%2.1fsigma-%s.w", out_prefix, stat_suffix, sigma,
            hemi);
    printf("writing mean diff - %2.1f sigma to %s...\n", sigma, fname);
    MRISwriteValues(mris, fname);
  }
  ...
  if (condition_0 >= 0) /* write means and variances */
  {
    ...
    fp = fopen(fname, "w");
    if (!fp)
      ErrorExit(ERROR_BADFILE, "%s: could not open dof file %s", Progname,
                fname);
    fprintf(fp, "%d\n", num_class1);
    fclose(fp);
  ...
  */
  ...
  if (test_subject) {
    ...
    sprintf(fname, "%s/%s/surf/%s.%s", subjects_dir, test_subject, hemi,
            surf_name);
    mris = MRISread(fname);
    if (!mris)
      ErrorExit(ERROR_NOFILE, "%s: could not read surface file %s", Progname,
    ...
    } else /* use sorting instead of connected areas */
    {
      ...
      fclose(fp);
      fprintf(stderr,
              "%d of %d correct = %2.1f%% (first wrong %d (%d)),"
              "min snr=%2.1f\n",
...
static int cvector_compute_variance(float *var, float *mean, int norm,
                                    int num) {
  int i;
...
static int cvector_add_variances(float *c1_var, float *c2_var, int num_class1,
                                 int num_class2, float *vtotal_var,
                                 int nvertices) {
  int i, total_dof;
  ...
  for (i = 0; i < nvertices; i++)
    vtotal_var[i] =
        (c1_var[i] * num_class1 + c2_var[i] * num_class2) / total_dof;
...
static int cvector_multiply_variances(float *c1_var, float *c2_var,
                                      int num_class1, int num_class2,
                                      float *vtotal_var, int nvertices) {
  int i, total_dof;
...
static int cvector_track_best_stats(
    float *vpvals, float *vbest_pvals, float *vbest_avgs, float *c1_mean,
    float *c2_mean, float *c1_best_mean, float *c2_best_mean, float *c1_var,
    float *c2_var, float *c1_best_var, float *c2_best_var,
    float **c1_avg_thickness, float **c2_avg_thickness,
    float **c1_best_thicknesses, int nc1, float **c2_best_thicknesses, int nc2,
...
static double cvector_compute_pvalues(float *c1_mean, float *c1_var,
  ...
  default:
  case STAT_T:
    return (cvector_compute_t_test(c1_mean, c1_var, c2_mean, c2_var, num_class1,
                                   num_class2, pvals, num, pvno));
    break;
  case STAT_MEAN:
...
static int cvector_combine_variances_into_stderrs(float *c1_var, float *c2_var,
  ...
  for (i = 0; i < nvertices; i++) {
    c_stderr[i] =
        sqrt((c1_var[i] * (num_class1 - 1) + c2_var[i] * (num_class2 - 1)) /
             (num_class1 + num_class2 - 2));
    c_stderr[i] =
        c_stderr[i] * sqrt(1.0f / (float)num_class1 + 1.0f / (float)num_class2);
  }
  return (NO_ERROR);
}
