...
static MRI *MRIthresholdFilled(MRI *mri_src, MRI *mri_T1, MRI *mri_mask,
                               ...
                               MRI *mri_dst, float threshold, float nsigma,
                               int out_label);
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_auto_fill.c,v 1.14 2011/03/02 00:04:13 nicks Exp $",
      "$Name:  $");
  ...
  if (old_flag)
    mri_rh_inverse_template =
        MRIapplyInverse3DMorph(mri_rh_template, m3d, NULL);
  else {
    mri_rh_inverse_template = MRIclone(mri_rh_template, NULL);
    mri_rh_inverse_template = GCAMmorphFromAtlas(
        mri_rh_template, gcam, mri_rh_inverse_template, SAMPLE_TRILINEAR);
  }
  ...
  if (old_flag)
    ...
    mri_inv_T1 =
        GCAMmorphFromAtlas(mri_tmp, gcam, mri_inv_T1, SAMPLE_TRILINEAR);
  }
  MRIfree(&mri_tmp);
  //////////////////////////////////////////////////////////////////////////
  if (strchr(template_fname, '#') == NULL)
  ...
  if (old_flag)
    ...
    mri_inv_T1_std =
        GCAMmorphFromAtlas(mri_tmp, gcam, mri_inv_T1_std, SAMPLE_TRILINEAR);
  }
  MRIfree(&mri_tmp);
  //////////////////////////////////////////////////////////////////////////
  ...
  /////////////////////////////////////////////////////////
  mri_rv =
      MRIfillVentricle(mri_inv_rv, mri_T1, 100, MRI_RIGHT_HEMISPHERE, NULL);
  MRIfree(&mri_inv_rv);
  if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
  ...
  */
  mri_filled = MRIcombineHemispheres(
      mri_lh, mri_rh, NULL, mri_lh_inverse_template, mri_rh_inverse_template);
  fprintf(stderr, "done.\n");
...
static int MRIfillVolume(MRI *mri) {
  ...
  MRItalairachToVoxel(mri, CORPUS_CALLOSUM_TAL_X + (SEED_SEARCH_SIZE + 1),
                      CORPUS_CALLOSUM_TAL_Y, CORPUS_CALLOSUM_TAL_Z, &xr, &yr,
                      &zr);
  MRItalairachToVoxel(mri, wm_lh_tal_x, wm_lh_tal_y, wm_lh_tal_z, &xr, &yr,
                      &zr);
  wm_lh_x = nint(xr);
  wm_lh_y = nint(yr);
  wm_lh_z = nint(zr);
  if ((MRIvox(mri, wm_lh_x, wm_lh_y, wm_lh_z) != MRI_LEFT_HEMISPHERE) ||
    ...
    for (z = wm_lh_z - SEED_SEARCH_SIZE; z <= wm_lh_z + SEED_SEARCH_SIZE; z++) {
      ...
      for (y = wm_lh_y - SEED_SEARCH_SIZE; y <= wm_lh_y + SEED_SEARCH_SIZE;
           y++) {
        yi = mri->yi[y];
        for (x = wm_lh_x - SEED_SEARCH_SIZE; x <= wm_lh_x + SEED_SEARCH_SIZE;
             x++) {
          xi = mri->xi[x];
  ...
  MRItalairachToVoxel(mri, CORPUS_CALLOSUM_TAL_X + (SEED_SEARCH_SIZE + 1),
                      CORPUS_CALLOSUM_TAL_Y, CORPUS_CALLOSUM_TAL_Z, &xr, &yr,
                      &zr);
  MRItalairachToVoxel(mri, wm_rh_tal_x, wm_rh_tal_y, wm_rh_tal_z, &xr, &yr,
                      &zr);
  wm_rh_x = nint(xr);
  wm_rh_y = nint(yr);
  wm_rh_z = nint(zr);
  if ((MRIvox(mri, wm_rh_x, wm_rh_y, wm_rh_z) == MRI_RIGHT_HEMISPHERE) ||
    ...
    for (z = wm_rh_z - SEED_SEARCH_SIZE; z <= wm_rh_z + SEED_SEARCH_SIZE; z++) {
      ...
      for (y = wm_rh_y - SEED_SEARCH_SIZE; y <= wm_rh_y + SEED_SEARCH_SIZE;
           y++) {
        yi = mri->yi[y];
        for (x = wm_rh_x - SEED_SEARCH_SIZE; x <= wm_rh_x + SEED_SEARCH_SIZE;
             x++) {
          xi = mri->xi[x];
