...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3)
    ErrorExit(ERROR_BADPARM, "usage: %s <subject name> <input volume>",
              Progname);
  ...
  fprintf(
      stdout,
      "corpus callosum matter segmentation and processing took %2.1f minutes\n",
...
static int find_mid_plane_wm(char *subject) {
  ...
  mri_tal =
      MRIalloc(mri_wm->width, mri_wm->height, mri_wm->depth, mri_wm->type);
  ...
  *MATRIX_RELT(mrot, 3, 4) = mtrans->rptr[3][4];
  ...
  MRIbinarize(mri_cc, mri_cc, DEFAULT_DESIRED_WHITE_MATTER_VALUE / 2 - 1, 0,
              110);
  ...
  fprintf(fp, "%s %d %d %d %d %d %d %d %d %d \n", subject, nint(volume[4]),
          nint(volume[3]), nint(volume[2]), nint(volume[1]), nint(volume[0]),
          yi_low, yi_high, zi_low, zi_high);
      ...
      for (ii = 0; ii < mri_filled->width; ii++) {
        for (jj = 0; jj < mri_filled->height; jj++) {
          MRIvox(mri_filled, ii, jj, 0) =
              MRIvox(mri_temp, jj, mri_temp->width - ii - 1, 0);
        }
      }
      MRIfree(&mri_temp);
    } else {
...
static int rotatevolume() {
  ...
  if (!NO_CORRECTION) {
      ...
      for (y_alpha = -1 * y_rotate; y_alpha <= y_rotate; y_alpha++)
        for (z_alpha = -1 * z_rotate; z_alpha <= z_rotate; z_alpha++) {
          ...
          x1_z = cos((double)y_alpha * M_PI / 180) *
                 sin((double)z_alpha * M_PI / 180);

          ez_x =
              sin((double)y_alpha * M_PI / 180); /* basis vectors for plane */
          ez_y = 0;
    ...
    fprintf(stdout,
            "counts: %d rotation x center: %d y_alpha: %d z_alpha: %d \n",
            number, p_x, p_y, p_z);
      ...
      for (yk = -whalf; yk < whalf; yk++) {
        ...
        for (zk = -whalf; zk < whalf; zk++) {
          ...
          if ((xi >= 0) && (xi < width) && (yi >= 0) && (yi < height) &&
              (zi >= 0) && (zi < depth)) {
            if (x == x_center)
              MRIvox(mri_cc_tal, x, yk + y_center, zk + z_center) =
                  MRIvox(mri_tal, xi, yi, zi);
            MRIvox(mri_temp, x, yk + y_center, zk + z_center) =
                MRIvox(mri_intensity, xi, yi, zi);
          }
        }
      }
    cc_tal_x      = x_center;
...
static int find_corpus_callosum(MRI *mri_tal, double *pccx, double *pccy,
  ...
  if (mri_tal->linear_transform || lta) {
    MRIworldToVoxel(mri_tal, 0.0, 0.0, 0.0, &xr, &yr,
                    &zr); /* everything is now in tal coords */
    xv = nint(xr);
    yv = nint(yr);
    zv = nint(zr);
  } else {
          ...
          if (y > max_y || (y == max_y && thickness > max_thick)) {
            ...
            if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON)
              fprintf(stdout,
                      ...
                      ycc, thickness);
          } else if (y == max_y && thickness == max_thick &&
                     (x == xcc + 1 || flag == 1)) {
            flag = 1;
            counts++;
          } else if (flag == 1) {
...
static int find_cc_slice(MRI *mri_tal, double *pccx, double *pccy, double *pccz,
  ...
  for (slice = 0; slice < max_slices; slice++) {
    ...
    if (!(area[slice] > 1100 && (nint(y) - region.y > 11)) ||
  ...
  *pccz = zvv;
  ...
  fprintf(stdout,
          "updating initial cc seed to Tal vol (%.2f, %.2f, %.2f) TAL (%.2f, "
          "%.2f, %.2f)\n",
          xvv, yvv, zvv, x, y, z);
...
static MRI *cc_slice_correction(MRI *mri_filled, int xv, int yv, int zv) {
  ...
  for (x = 1; x < width - 1; x++) {
    for (y = 1; y < height - 1; y++) {
      if (MRIvox(mri_filled, x - 1, y, 0) || MRIvox(mri_filled, x, y - 1, 0) ||
          MRIvox(mri_filled, x, y + 1, 0) || MRIvox(mri_filled, x + 1, y, 0) ||
  ...
  for (x = 1; x < width - 1; x++) {
    for (y = 1; y < height - 1; y++) {
      if (MRIvox(mri_temp1, x - 1, y, 0) && MRIvox(mri_temp1, x, y - 1, 0) &&
          MRIvox(mri_temp1, x, y + 1, 0) && MRIvox(mri_temp1, x + 1, y, 0) &&
  ...
  for (x_edge = xi_high - nint((xi_high - xi_low) / 3.5);
    ...
    while (edge_count < 3 && y_edge < height - 2) {
      ...
      if (edge_count == 1 && length > 1) {
        if (length >= 10 * ratio && x_edge > xi_low + 7 * ratio &&
            x_edge < xi_high - 10 * ratio) {
          temp = old_temp;
          if (x1 < x_edge) {
            y1 = old_temp;
            x1 = x_edge;
        ...
        if (length <= 1 || y_edge < yi_low + 3) {
          for (y = y_edge + 1; y > 0; y--)
            MRIvox(mri_filled, x_edge, y, 0) = 0;
          edge_count -= 1;
        } else if (length > 2 && x_edge > xi_low + 8 * ratio &&
                   x_edge < xi_high - 10 * ratio) {
          for (y = y_edge + 1; y < mri_filled->width; y++)
            MRIvox(mri_filled, x_edge, y, 0) = 0;
        }
      } else if (length > 1 && edge_count > 1 && y_edge < yi_high + 1 &&
                 y_edge > yv + 7.5 * ratio)
        edge_count -= 1;
    }
    ...
    if (edge_count >= 3 && flag == 0)
      ...
      if (y2 <= yi_low + 8 * ratio)
        break;
               ...
               (section1[i] >= 12 || (2 * section1[i - 1] < section1[i]))) {
      x2 = old_temp;
      y2 = y_edge;
      if (y2 <= yi_low + 8)
        break;
    ...
    for (j = 0; j < height; j++)
      for (i = 0; i < width; i++)
        if (MRIvox(cc_smoothed, i, j, k) > 0)
          if (MRIvox(cc_smoothed, i, cc_smoothed->yi[j - 1], k) == 0 ||
              MRIvox(cc_smoothed, i, cc_smoothed->yi[j + 1], k) == 0 ||
              MRIvox(cc_smoothed, cc_smoothed->xi[i - 1], j, k) == 0 ||
              MRIvox(cc_smoothed, cc_smoothed->xi[i + 1], j, k) == 0)
...
static MEDATOM *find_medial_axis(MEDATOM *medial_axis, int length, int n_sample,
  ...
  medial_axis =
      medial_axis_integration(medial_axis, cc_boundary, cc_smoothed, length);
  medial_axis =
      medial_axis_refine_leaf(medial_axis, cc_boundary, cc_smoothed, length);
#else
  {
    float val1, val2;
    length = 102;
  ...
  if (0) {
    double xw, yw, zw;
    for (j = 0; j < length; j++) {
      MRIvoxelToWorld(mri_cc_tal, cc_tal_x, medial_axis[j].y, medial_axis[j].x,
                      &xw, &yw, &zw);
  ...
  medial_axis = compute_normal(medial_axis, length);
  for (i = 0; i < length; i++)
    printf("%d  %f  %f  %f  %f\n", i + 1, medial_axis[i].x, medial_axis[i].y,
           medial_axis[i].nx, medial_axis[i].ny);
  ...
  for (j = 1; j < length; j++) {
    dist += sqrt((medial_axis[j].x - medial_axis[j - 1].x) *
                     (medial_axis[j].x - medial_axis[j - 1].x) +
                 (medial_axis[j].y - medial_axis[j - 1].y) *
                     (medial_axis[j].y - medial_axis[j - 1].y));
  ...
  for (n = 1; n <= n_sample; n++) {
    ...
    if (j == length)
      ...
      MRIsampleVolumeFrameType(mri_intensity, cc_tal_x, y, x, 0,
                               SAMPLE_TRILINEAR, &val);
      radius = sample_distance_map(cc_boundary, cc_smoothed, x, y);
      MRIsampleVolumeFrameType(mri_intensity, cc_tal_x, y + radius * dy / 2,
                               x + radius * dx / 2, 0, SAMPLE_TRILINEAR,
                               &upmid);
      MRIsampleVolumeFrameType(mri_intensity, cc_tal_x, y - radius * dy / 2,
                               x - radius * dx / 2, 0, SAMPLE_TRILINEAR,
                               &downmid);
      fprintf(fp, "%d   %f   %f   %f   %f   %f   %f   %f\n", n, x, y, radius,
              val, mean_val, upmid, downmid);
    }
  }
  MRIsampleVolumeFrameType(mri_intensity, cc_tal_x, medial_axis[length - 1].y,
                           medial_axis[length - 1].x, 0, SAMPLE_TRILINEAR,
                           &val);
  fprintf(fp, "%d   %f   %f   0   %f   %f   %f   %f\n", n,
          medial_axis[length - 1].x, medial_axis[length - 1].y, val, val, val,
          val);
...
static MEDATOM *medial_axis_integration(MEDATOM *medial_axis, MRI *cc_boundary,
  ...
  do {
    ...
    for (i = 1; i < length - 1; i++)
      move += (medial_axis[i].dx * medial_axis[i].dx) +
              (medial_axis[i].dy * medial_axis[i].dy);
    medial_axis =
        integrate_momentum(medial_axis, cc_boundary, cc_smoothed, length);
    sse = compute_energy(medial_axis, cc_boundary, cc_smoothed, length);
    if (alert)
  ...
  if ((n % 1) == 0) {
    fprintf(stdout,
            "%d iteration: min_sse= %f   sse=%f  old_sse=%f   ratio=%f tol=%f  "
            "p=%f move=%f number=%d\n",
            n, min_sse, sse, old_sse, ratio, tol, p, move, number);
    for (i = 0; i < length; i++) {
      fprintf(stdout, "%f  %f  %f  %f\n", medial_axis[i].x, medial_axis[i].y,
              medial_axis[i].nx, medial_axis[i].ny);
    }
  }
#endif
...
static float sample_distance_map(MRI *cc_boundary, MRI *cc_smoothed, float x,
  ...
  if (MRIvox(cc_smoothed, nint(x), nint(y), 0) == 0)
    closest *= -1;
  return (closest);
}
...
static MEDATOM *integrate_momentum(MEDATOM *medial_axis, MRI *cc_boundary,
  ...
  for (i = 1; i < length - 1; i++) {
    if ((fabs(medial_axis[i].x - medial_axis[i - 1].x) < 0.001) &&
        (fabs(medial_axis[i].y - medial_axis[i - 1].y) < 0.001)) {
      medial_axis[i].x = (medial_axis[i + 1].x - medial_axis[i - 1].x) / 2 +
                         medial_axis[i - 1].x;
      medial_axis[i].y = (medial_axis[i + 1].y - medial_axis[i - 1].y) / 2 +
                         medial_axis[i - 1].y;
    }
  }
  ...
  for (i = 0; i < length - 1; i++) {
    dist = sqrt((medial_axis[i + 1].x - medial_axis[i].x) *
                    (medial_axis[i + 1].x - medial_axis[i].x) +
...
static double compute_energy(MEDATOM *medial_axis, MRI *cc_boundary,
  ...
  for (i = 1; i < length - 1; i++) {
    ...
    if (sample_distance_map(cc_boundary, cc_smoothed, x, y) < 0)
      neg_count++;
...
static MEDATOM *medial_axis_refine_leaf(MEDATOM *medial_axis, MRI *cc_boundary,
  ...
  if (ROT) {
    for (i = 0;
         i < length && medial_axis[i].y - medial_axis[0].y - 10 * ratio <= 0;
         i++) {
      ;
    }
    ...
    for (i = length - 1;
         i > 0 &&
         medial_axis[length - 1].y - medial_axis[i].y - ratio * 10 <= 0;
         i--) {
      ;
    }
    new_length = nint(length / 5) > (length - i + 1) ? nint(length / 5)
                                                     : (length - i + 1);
    ...
    for (i = 0; i < length && medial_axis[i].x - medial_axis[0].x - 10 <= 0;
         i++) {
      ;
    }
    new_length = nint(length / 5) > i + 1 ? nint(length / 5) : i + 1;
    ...
    for (i = length - 1;
         i > 0 && medial_axis[length - 1].x - medial_axis[i].x - 10 <= 0; i--) {
      ;
    }
    new_length = nint(length / 5) > (length - i + 1) ? nint(length / 5)
                                                     : (length - i + 1);
...
static float compute_error(MEDATOM *medial_axis, MRI *cc_boundary,
  ...
  MRIwrite(cc_new,
           "/space/solo/4/users/recon/DONF55b_recon/callosum/cc_new.mgh");
  ...
  MRIwrite(cc_newb,
           "/space/solo/4/users/recon/DONF55b_recon/callosum/cc_newb.mgh");
  ...
  for (i = 0; i < width; i++)
    for (j = 0; j < height; j++)
      if (MRIvox(cc_boundary, i, j, 0)) {
        ...
        if ((!flag && node < new_length) ||
            (flag && node < length && node > length - new_length - 1)) {
          error += closest;
          count++;
        }
      }
...
static MRI *sample_medial_axis(MEDATOM *medial_axis, MRI *cc_medial_axis,
  ...
  for (j = 0; j < length - 1; j++) {
    ...
    do {
      ...
      x2 = medial_axis[j].x;
      y2 = medial_axis[j].y;
    } while (j < length - 1 && fabs(nint(x2) - nint(x1)) < 1 &&
             fabs(nint(y2) - nint(y1)) < 1);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "wm")) {
    ...
    case '?':
    case 'U':
      fprintf(stdout, "usage: %s <input volumes> <output volume>\n", Progname);
      exit(1);
      break;
    case 'T':
      dxi = atoi(argv[2]);
