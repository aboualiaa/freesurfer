...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3)
    ErrorExit(ERROR_BADPARM, "usage: %s <cc_volume> <P_value> <output_volume> ",
              Progname);
  ...
  if (!mri_cc)
    ErrorExit(ERROR_NOFILE, "%s: could not read cc volume %s", Progname,
              argv[1]);

  if ((fp = fopen(argv[2], "r")) == NULL) {
    ErrorReturn(
        ERROR_BADFILE,
        (ERROR_BADFILE, "p value input: file %s does not exist!", argv[2]));
  }
  ...
  for (i = 0; i < length; i++) {
    fscanf(fp, "%d %f %f %f %f\n", &n, &medial_axis[i].x, &medial_axis[i].y,
           &medial_axis[i].radius, &val);
    fprintf(stdout, "%d   %f   %f   %f   %f\n", n, medial_axis[i].x,
            medial_axis[i].y, medial_axis[i].radius, val);
  }
      ...
      if (MRIvox(mri_cc, cc_x, j, k) > 0) {
        ...
        for (i = 0; i < length; i++) {
          ...
          if (dist < nearest)
            ...
            fprintf(stdout,
                    "vertex %d %d %d has more than one nearest neighbor!", cc_x,
                    j, k);
        }
      }
