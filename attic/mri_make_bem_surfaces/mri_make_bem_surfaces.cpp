...
static void init_surf_to_image(void) {
  ...
  if (initsurftoimageflag) {
    printf("mri_strip_skull: ### init_surf_to_image failed:  already done "
           "(re-read ic?.tri)\n");
    PR return;
  }

  if (!MRIloaded) {
    printf("mri_strip_skull: ### init_surf_to_image failed:  MRI data not "
           "loaded\n");
    PR return;
  }
  xlo = ylo = zlo = 10000;
...
static float rtanh(float x) { return (x < 0.0) ? 0.0 : tanh(x); }

static void shrink(int niter, int nsmoothsteps) {
  float x, y, z, sx, sy, sz, val, inval, outval, nc, force, force0, force1,
      force2;
    ...
    for (k = 0; k < nvertices; k++) {
      ...
      if (MRIflag) {
        ...
        if (shrinkmode == 3) {
          ...
          */
          force = fstrength * (0.2 * tanh((insamp[0] - fzero) * fsteepness) +
                               rtanh((inmax - fzero) * fsteepness) -
...
return;} fscanf(fp,"%*s %lf",&brainval); fscanf(fp,"%*s %lf",&skullval);
