...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: label_area.c,v 1.9 2012/04/25 13:37:44 fischl Exp $",
      "$Name:  $");
...
static float compute_label_area(MRI_SURFACE *mris, char *subject_name,
  ...
  fp = fopen(fname, "r");
  if (!fp)
    ErrorExit(ERROR_NOFILE, "%s: could not open label file %s", Progname,
              fname);
  ...
  while ((cp = fgetl(line, 199, fp)) != NULL) {
    if (sscanf(cp, "%d %*f %*f %*f %f", &vno, &stat) != 2)
      ErrorExit(ERROR_BADFILE, "%s: could not parse %dth line in %s", Progname,
                nlines, fname);
...
static void print_usage(void) {
  ...
  printf("\tS <surface name>    use a surface other than 'white' for metric "
         "properties\n");
  printf("\tT <threshold>       only compute surface area for label vertices "
         "above thresh\n");
  exit(1);
}
