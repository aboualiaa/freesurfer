...
int main(int argc, char *argv[]) {
  ...
  if (mri_bc == NULL)
    ErrorExit(ERROR_NOFILE, "%s: could not read body coil PD map from %s",
              argv[2]);
  mri_sc = MRIread(argv[3]);
  if (mri_sc == NULL)
    ErrorExit(ERROR_NOFILE, "%s: could not read surface coil PD map from %s",
              argv[3]);
...
static void print_usage(void) {
  printf("usage: %s [options] <input volume> <BC PD map> <SC PD map> <output "
         "volume>\n",
         Progname);
}

static void print_help(void) {
  fprintf(stderr, "\nThis program will intensity correct a volume based on "
                  "surface coil and body coil images\n");
  exit(1);
}
