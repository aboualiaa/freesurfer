...
static char vcid[] =
    "$Id: mri_extract_conditions.c,v 1.5 2011/03/02 00:04:15 nicks Exp $";
...
int main(int argc, char *argv[]) {
  ...
  for (t = 0; t < mri_in->nframes; t++) {
    ...
    if (sscanf(line, "%f %d", &timepoints[t], &conditions[t]) != 2)
      ErrorExit(ERROR_BADFILE, "%s: line %d malformed (%s)", Progname, t, line);
  }
...
static void print_usage(void) {
  printf("usage: %s [options] <input volume> <paradigm file> <output file>\n",
         Progname);
...
static void print_help(void) {
  print_usage();
  fprintf(stderr, "\nThis program extract conditions from a run and output "
        ...
        for (mean = 0.0, i = t = 0; t < ntime; t++) {
          ...
          default:
            ErrorExit(ERROR_UNSUPPORTED,
                      "MRIdetrendVolume: input type %d unsupported",
                      mri_in->type);
          case MRI_FLOAT:
    ...
    for (y = 0; y < mri_out->height; y++) {
      for (z = 0; z < mri_out->depth; z++) {
        ...
        for (t = 0; t < ntime; t++) {
          ...
          default:
            ErrorExit(ERROR_UNSUPPORTED,
                      "MRIextractConditions: input type %d unsupported",
                      mri_in->type);
          case MRI_FLOAT:
            val = MRIFseq_vox(mri_in, x, y, z, t);
            break;
