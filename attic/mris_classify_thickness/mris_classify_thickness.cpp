 ...
 *
 * read in thickness vectors that have been mapped to a common coordinate sytem
 * (e.g. fsaverage5) then use them to train and optionally apply a classifier
 * (random forest at the moment)
 */
/*
...
#define ARGV_OFFSET 3
    ...
    if (naseg > 0) {
      ...
      for (a = 0; a < naseg; a++) {
        ...
        if (t >= aseg_stats->nlabels)
          ErrorExit(ERROR_BADPARM,
                    "%s: could not find aseg label %s (%d) in stats file",
                    Progname, cma_label_to_name(aseg_labels[a]),
                    aseg_labels[a]);
  ...
  RFtrain(rf, feature_fraction, training_fraction, training_class,
          training_data, num_class1 + num_class2);
  if (training_fraction < 1) {
    int correct;
    correct = RFcomputeOutOfBagCorrect(rf, training_class, training_data,
                                       num_class1 + num_class2);
    printf("out of bag accuracy = %d of %d = %2.1f%%\n", correct,
           num_class1 + num_class2,
           100.0 * correct / (num_class1 + num_class2));
  }
  if (rf_write_name) {
#if 0
    RANDOM_FOREST *rf2 ;
  ...
  if (test_subject) {
    ...
    for (h = 0; h <= 1; h++) {
      ...
      if (strchr(thickness_name, '/') != NULL)
        ...
        sprintf(fname, "%s/%s/surf/lh.%s", subjects_dir, test_subject,
                thickness_name);
      if (MRISreadCurvatureFile(mris, fname) != NO_ERROR)
        ErrorExit(Gerror, "%s: could no read curvature file %s", Progname,
                  fname);
    ...
    sprintf(line, "%s %s %d %d %lf\n", test_subject, hemi, classnum, test_class,
            pval);
...
#define ARGV_OFFSET 4
  ...
  if (!total_mean)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate mean list of %d curvatures", Progname, n,
              nvertices);
  c1_mean = (float *)calloc(nvertices, sizeof(float));
  if (!c1_mean)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate c1 mean list of %d curvatures", Progname,
              n, nvertices);
  pvals = (float *)calloc(nvertices, sizeof(float));
  if (!pvals)
    ErrorExit(ERROR_NOMEMORY, "%s: could not allocate pvals", Progname, n,
              nvertices);
  c2_mean = (float *)calloc(nvertices, sizeof(float));
  if (!c2_mean)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate c2 mean list of %d curvatures", Progname,
              n, nvertices);

  c1_var = (float *)calloc(nvertices, sizeof(float));
  if (!c1_var)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate c1 var list of %d curvatures", Progname,
              n, nvertices);
  c2_var = (float *)calloc(nvertices, sizeof(float));
  if (!c2_var)
    ErrorExit(ERROR_NOMEMORY,
              "%s: could not allocate c2 var list of %d curvatures", Progname,
              n, nvertices);
  ...
  for (n = 0; n < num_class1; n++) {
    ...
    if (!c1_thickness[n] || !c1_avg_thickness[n])
      ErrorExit(ERROR_NOMEMORY,
                "%s: could not allocate %dth list of %d curvatures", Progname,
                n, nvertices);
    ...
    for (avgs = 1; avgs <= max_avgs; avgs++) {
      ...
      if (use_no_distribution)
        snr = cvector_compute_dist_free_snr(
            c1_avg_thickness, num_class1, c2_avg_thickness, num_class2, c1_mean,
            c2_mean, vsnr, nvertices, &i);
      else
        snr = cvector_compute_snr(c1_mean, c2_mean, vtotal_var, vsnr, nvertices,
                                  &i, bonferroni ? log((double)avgs) : 0.0f);
      if (write_flag && DIAG_VERBOSE_ON) {
  ...
  if (nsort <= 0) {
    ...
    for (done = 0, current_fthresh = fthresh; !FZERO(current_fthresh) && !done;
      ...
      for (current_min_label_area = min_label_area;
           current_min_label_area > 0.5; current_min_label_area *= 0.75) {
        MRISclearMarks(mris);
        sprintf(fname, "%s-%s_thickness", hemi, prefix ? prefix : "");
        mark_thresholded_vertices(mris, vbest_snr, vbest_avgs, current_fthresh);
        segment_and_write_labels(output_subject, fname, mris, &pos_labels,
                                 &npos_labels, 0, current_min_label_area);
        MRISclearMarks(mris);
        mark_thresholded_vertices(mris, vbest_snr, vbest_avgs,
                                  -current_fthresh);
    ...
    printf("%d labels found with F > %2.1f and area > %2.0f\n", nlabels,
           current_fthresh, current_min_label_area);
    for (i = 0; i < nlabels; i++)
      fprintf(stderr, "label %d: %d points, %2.1f mm\n", i, labels[i]->n_points,
              LabelArea(labels[i], mris));
  }
  ...
  */
  ...
  } else /* read in precomputed optimal thicknesses */
  {
    ...
    for (n = 0; n < num_class1; n++) {
      ...
      if (MRISreadValues(mris, fname) != NO_ERROR)
        ErrorExit(Gerror, "%s: could not read thickness file %s", Progname,
                  fname);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help"))
    ...
    if (naseg >= MAX_ASEG_LABELS)
      ErrorExit(ERROR_NOMEMORY, "%s: too many aseg labels specified (%d)",
                Progname, naseg);
...
static void print_usage(void) {
  ...
  fprintf(stderr, "where surf must be a spherical surface suitable for "
                  "computing geodesics\n");
  fprintf(stderr,
          "The <c1_subject> ... is a list of subjects from one classnum\n"
