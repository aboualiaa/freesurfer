...
static void brain_params(void) {
  ...
  fprintf(stderr, "\n      first estimation of the COG coord: x=%d,y=%d, z=%d",
          (int)txCOG, (int)tyCOG, (int)tzCOG);
    ...
    for (j = 0; j < height; j++)
      for (i = 0; i < width; i++)
        if (MRIFvox(mri_dst, i, j, k) > threshold) {
          ...
          rad_Brain += (SQR(i - xCOG) + SQR(j - yCOG) + SQR(k - zCOG)) *
                       MRIFvox(mri_dst, i, j, k);
        }
  ...
  fprintf(stderr,
          ...
          (int)xCOG, (int)yCOG, (int)zCOG, (int)rad_Brain);

  txCOG = xCOG - txCOG;
  tyCOG = yCOG - tyCOG;
  tzCOG = zCOG - tzCOG;

  fprintf(stderr, "\n      direction of the top of the brain: x=%d,y=%d, z=%d",
          (int)txCOG, (int)tyCOG, (int)tzCOG);
}
    ...
    for (k = 0; k < nvertices; k++) {
      ...
      if (MRIflag) {
        ...
        for (h = -noutside; h < ninside; h++) {
          ...
          it = (int)(x - nx * h + 0.5);
          jt = (int)(y - ny * h + 0.5);
          if (kt < 0 || kt >= depth || it < 0 || it >= width || jt < 0 ||
              jt >= height)
            valt = 0;
          else {
...
static void analyse_curve(unsigned long *tab, int dim, int scale_factor) {
  ...
  fprintf(stderr, " k1=%d k2=%d SKULL_PD=%d %d", k1, k2,
          SKULL_PD / scale_factor, PEAK1);
...
static void PDcurve(MRI *mri) {
  ...
  fprintf(stderr, "max=%d scale=%d  %d ", max, scale * 10 / 15,
          max * scale / (HISTO_SIZE - 1) / (HISTO_SIZE - 1));
...
static void write_surface(char *fname) {
  ...
  for (k = 0; k < nvertices; k++) {
    MRISsurfaceRASFromVoxelCached(mris, mri_PD, vertex[k].x, vertex[k].y,
                                  vertex[k].z, &x, &y, &z);
    ...
    for (j = 0; j < height; j++)
      for (k = 0; k < depth; k++) {
        if (MRIFvox(mri_T1, i, j, k))
          MRIFvox(mri_dst, i, j, k) =
              (MRIFvox(mri_PD, i, j, k) * exp(-100 / MRIFvox(mri_T1, i, j, k)));
        else
  ...
  for (i = 0; i < width; i++)
    for (j = 0; j < height; j++)
      for (k = 0; k < depth; k++) {
        ...
        if (f >= 0) {
          MRIFvox(mri_dst, i, j, k) =
              ((float)MRIFvox(mri_dst, i, j, k) * (1 + f / rad_Brain * 0.5));
        }
      }
}
    ...
    for (j = 0; j < height; j++)
      for (k = 0; k < depth; k++) {
        ...
        if (MRIFvox(mri_CSF, i, j, k) == 5) {
          if (MRIFvox(mri_T1, i, j, k) > GM_max_T1 &&
              MRIFvox(mri_PD, i, j, k) > 80 * PEAK1 / 100)
