...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_svm_train.c,v 1.6 2011/03/02 00:04:34 nicks Exp $",
      "$Name:  $");
    ...
    if (nannotations > 0) {
      ...
      if (MRISreadAnnotation(mris, annot_name) != NO_ERROR)
        ErrorExit(ERROR_NOFILE,
                  "%s: could not read annot file %s for subject %s", Progname,
                  annot_name, subject_name);
...
static void print_help(void) {
  print_usage();
  fprintf(stderr, "\nThis program will train an SVM classifier based on "
                  "thickness (or other curv) "
                  " measures\n");
  fprintf(stderr, "\nvalid options are:\n\n");
  exit(1);
}
