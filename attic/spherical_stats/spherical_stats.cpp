...
static char vcid[] =
    "$Id: spherical_stats.c,v 1.8 2011/03/02 00:04:40 nicks Exp $";
...
#define NIMAGES 3
#define PARAM_IMAGES                                                           \
  (IMAGES_PER_SURFACE * NIMAGES) // 9 -> man_vs_auto / sse / correlation
...
static void printGlobalStats(MRIS *mris, char *fname) {
  ...
  for (m = 0; m < nlabels; m++) {
    ...
    fprintf(f,
            "Vertices# :  %3.0f - Area : %2.3f%% \n Fraction*sqrt(area) : "
            "%2.3f - Fraction : %2.3f - Mean Distance Error : %f \n",
            global_nvertices[m], 100.0 * mean_area[m], mean_fraction[m],
            100.0 * mean_global_Efract[m], mean_global_dist[m]);

    fprintf(f, "Median (25,50,75) = ( %f , %f , %f ) - Max : %f\n",
            mean_global_median_25[m], mean_global_median_50[m],
            mean_global_median_75[m], mean_max[m]);
...
static void printStats(MRIS *mris, char *fname) {
  ...
  for (m = 0; m < nlabels; m++) {
    ...
    if (Efract)
      mean_dist = mean_dist / (Efract * nvertices);

    larea = 0;
    for (n = 0; n < mris->nfaces; n++) {
      if (mris->vertices[mris->faces[n].v[0]].val != labels[m])
        continue;
      if (mris->vertices[mris->faces[n].v[1]].val != labels[m])
        continue;
      if (mris->vertices[mris->faces[n].v[2]].val != labels[m])
        continue;
    ...
    fprintf(f,
            "Vertices# :  %d - Area : %2.3f%% \nFraction : %2.3f%% - "
            "Fraction*sqrt(Area) : %2.3f \n",
            nvertices, 100.0 * larea, 100.0 * Efract,
            100.0 * Efract * sqrt(larea));
    fprintf(f,
            "Mean Distance Error : %f - Median (25,50,75) = ( %f , %f , %f ) - "
            "Max : %f\n",
...
static float sphericaldistance(float x1, float y1, float z1, float x2, float y2,
                               float z2) {
  ...
  if (global_stats_per_label && first) {
    ...
    for (m = 0; m < nlabels; m++) {
      ...
      if (index >= 0 || index < ct->nentries) {
        cte = ct->entries[index];
        if (suffix)
          sprintf(fname, "%s/stats/%s.%s.%s.txt", subjects_dir, hemi, suffix,
                  cte->name);
        else
        ...
        if (suffix)
          sprintf(fname, "%s/stats/%s.%s.%d.txt", subjects_dir, hemi, suffix,
                  labels[m]);
        else
...
int main(int argc, char *argv[]) {
  ...
  nargs = handleVersionOption(argc, argv, "spherical_stats");
  ...
  if (template_fname) {
    ...
    if (!mrisp_average)
      ErrorExit(ERROR_NOFILE, "%s: could not read template file %s", Progname,
                template_fname);
  }
  ...
  for (n = 0; n < nsubjects; n++) {
    ...
    /* save stats */
    if (suffix)
      sprintf(fname, "%s/%s/surf/%s.%s.stats.txt", subjects_dir, subject_fname,
              hemi, suffix);
    else
      sprintf(fname, "%s/%s/surf/%s.stats.txt", subjects_dir, subject_fname,
              hemi);
    ...
    if (suffix)
      sprintf(fname, "%s/%s/surf/%s.%s.%s", subjects_dir, subject_fname, hemi,
              suffix, distance_error);
    else
      sprintf(fname, "%s/%s/surf/%s.%s", subjects_dir, subject_fname, hemi,
              distance_error);
    fprintf(stderr,
            "writing out distaces errors in file %s.%s for subject %s...\n",
            hemi, man_vs_auto, subject_fname);
    MRISwriteCurvature(mris, fname);

    if (n == nsubjects - 1) { /* last subject */
      if (suffix)
        sprintf(fname, "%s/stats/%s.%s.global_stats_%d.txt", subjects_dir, hemi,
                suffix, nsubjects);
      else
        sprintf(fname, "%s/stats/%s.global_stats_%d.txt", subjects_dir, hemi,
                nsubjects);
    ...
    if (mrisp_average) {
      ...
      if (suffix)
        sprintf(fname, "%s/%s/surf/%s.%s.%s", subjects_dir, subject_fname, hemi,
                suffix, alignment_fname);
      else
        sprintf(fname, "%s/%s/surf/%s.%s", subjects_dir, subject_fname, hemi,
                alignment_fname);
      fprintf(stderr,
              "writing out sulcal differences in alignment into file %s.%s for "
              "subject %s...\n",
  ...
  if (all && do_combine) {
    for (n = 0; n < nsubjects; n++) {
      ...
      if (suffix)
        sprintf(fname, "%s/%s/surf/%s.%s.mean_distance_error", subjects_dir,
                subject_fname, hemi, suffix);
      else
        sprintf(fname, "%s/%s/surf/%s.mean_distance_error", subjects_dir,
                subject_fname, hemi);
      fprintf(stderr,
              "writting mean distance information into %s of subject %s \n",
              fname, subject_fname);
      MRISwriteCurvature(mris_atlas, fname);
      ...
      if (mrisp_average) {
        ...
        if (suffix)
          sprintf(fname, "%s/%s/surf/%s.%s.error", subjects_dir, subject_fname,
                  hemi, suffix);
        else
          sprintf(fname, "%s/%s/surf/%s.error", subjects_dir, subject_fname,
                  hemi);
...
static void print_usage(void) {
  fprintf(stderr,
          ...
          Progname);
  fprintf(stderr,
          "\nThis program computes statistics from a set of manually and "
          "automatically labeld brains.\n"
