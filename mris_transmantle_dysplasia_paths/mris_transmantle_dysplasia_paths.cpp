...
static VOXEL_LIST *find_optimal_spline(
    ...
    int mcmc_samples, double spline_length_penalty, double spline_nonwm_penalty,
...
// SPLINE_BELOW ;
static int energy_flags =
    SPLINE_WM_DIST | SPLINE_LENGTH | SPLINE_BELOW | SPLINE_ABOVE;

static double compute_spline_energy(VOXEL_LIST *vl, MRI *mri_intensity,
                                    MRI *mri_aseg, MRI *mri_wm_dist,
                                    double mean, int which,
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: mris_transmantle_dysplasia_paths.c,v 1.4 "
                                "2012/06/16 12:48:24 fischl Exp $",
                                "$Name:  $");
  ...
  if (xform == nullptr)
    ErrorExit(ERROR_NOFILE, "%s: could not load transform from %s", Progname,
              argv[4]);

  mri_pvals = compute_migration_probabilities(mris, mri, mri_aseg, xform,
                                              nullptr, spline_control_points,
                                              mcmc_samples, read_flag);
  ...
  seconds = seconds % 60;
  fprintf(stderr,
          "simulated neural migration took %d minutes and %d seconds.\n",
          minutes, seconds);
  exit(0);
  return (0);
}
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "DEBUG_VOXEL")) {
    ...
    case 'N':
      noise = atof(argv[2]);
      printf("using noise level = %2.2f to deflect ventricle distance "
             "transform negative gradient vectors\n",
             noise);
      nargs = 1;
      break;
    case '?':
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <surface> <aseg volume> <intensity volume> "
         "<xform> <output volume>\n",
         Progname);
...
static MRI *MRIbuildPossibleMigrationPaths(MRI *mri_aseg, MRI *mri_wm_interior,
  ...
  if (mri_paths == nullptr)
    mri_paths = MRIclone(mri_aseg, nullptr);
      ...
      for (y = 0; y < mri_aseg->height; y++)
        for (z = 0; z < mri_aseg->depth; z++) {
          ...
          if (MRIgetVoxVal(mri_dilated, x, y, z, 0) ==
              0) // not next to a voxel in the current set
            continue;
    ...
    for (x = 0; x < mri_aseg->width; x++)
      for (y = 0; y < mri_aseg->height; y++)
        for (z = 0; z < mri_aseg->depth; z++) {
          ...
          if (MRIgetVoxVal(mri_paths, x, y, z, 0)) // already in the current set
            continue;
          MRIsetVoxVal(mri_paths, x, y, z, 0,
                       MRIgetVoxVal(mri_dilated, x, y, z, 0) + 1);
          nadded++;
        }
    printf("round %d: %d voxels added\n", n, nadded);
    n++;
...
static MRI *compute_migration_probabilities(MRI_SURFACE *mris,
                                            MRI *mri_intensity, MRI *mri_aseg,
  ...
  if (read_flag) {
    ...
    sprintf(fname, "%s/%s.splines.mgz", FileNamePath(mris->fname, path),
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
    printf("reading optimal splines from %s\n", fname);
    mri_splines = MRIread(fname);
    if (mri_splines == nullptr)
      ErrorExit(ERROR_NOFILE, "%s: could not read splines from %s", Progname,
                fname);
  } else
    mri_splines =
  ...
  MRIreplaceValuesOnly(mri_eroded_aseg, mri_eroded_aseg, 1,
  ...
  mri_possible_migration_paths =
      MRIbuildPossibleMigrationPaths(mri_aseg, mri_wm_interior, nullptr);

  mri_wm_dist = MRIdistanceTransform(mri_wm_interior, nullptr, 1, 5,
  ...
  for (nvox_wm = nvox = x = 0; x < mri_intensity->width; x++)
    for (y = 0; y < mri_intensity->height; y++)
      for (z = 0; z < mri_intensity->depth; z++) {
        ...
        label = MRIgetVoxVal(mri_aseg, nint(V3_X(v_aseg)), nint(V3_Y(v_aseg)),
                             nint(V3_Z(v_aseg)), 0);
        if (IS_CORTEX(label)) {
          nvox++;
  ...
  gm_mean /= nvox;
  gm_var = gm_var / nvox - gm_mean * gm_mean;
  wm_mean /= nvox_wm;
  wm_var = wm_var / nvox_wm - wm_mean * wm_mean;
  ...
  mri_tmp =
      MRIconvolveGaussian(mri_possible_migration_paths, nullptr, mri_kernel);
  MRIfree(&mri_possible_migration_paths);
  mri_possible_migration_paths = mri_tmp;
  MRIfree(&mri_kernel);
  mri_path_grad = MRIsobel(mri_possible_migration_paths, nullptr, nullptr);
  MRInormalizeSequence(mri_path_grad, 1.0);
  vl_posterior =
      VLSTalloc(mri_aseg->width * mri_aseg->height * mri_aseg->depth);
  if (Gdiag_no >= 0) {
...
#pragma omp parallel for if_ROMP(experimental)                                 \
    firstprivate(n, v, vl, vl_spline, entropy, gm_mean, mcmc_samples)          \
        shared(mri_intensity, mri_aseg, mri_path_grad, mri_splines)            \
            schedule(static, 1)
#endif
  for (vno = 0; vno < mris->nvertices; vno++) {
    ROMP_PFLB_begin
        ...
        if ((vno % (mris->nvertices / 200)) == 0)
            printf("processed %d of %d: %2.1f%%\n", vno, mris->nvertices,
                   100.0 * vno / (float)mris->nvertices);
    v = &mris->vertices[vno];
    if (v->ripflag)
      continue;
    if (vno == Gdiag_no)
    ...
    if (read_flag == 0) {
      ...
      if (vl == nullptr)
        ROMP_PFLB_continue;
      vl_spline = find_optimal_spline(
          vl, mri_intensity, mri_aseg, mri_wm_dist, gm_mean, mcmc_samples, mris,
          spline_control_points, mcmc_samples, spline_length_penalty,
          spline_nonwm_penalty, spline_interior_penalty, mri_posterior,
          vl_posterior, &entropy, mri_total_posterior);
      ...
      if (vno == Gdiag_no) {
        ...
        energy = compute_spline_energy(
            vl_spline, mri_intensity, mri_aseg, mri_wm_dist, gm_mean,
        ...
        val = compute_spline_energy(vl_spline, mri_intensity, mri_aseg,
                                    mri_wm_dist, gm_mean, energy_flags,
                                    spline_length_penalty, spline_nonwm_penalty,
                                    spline_interior_penalty);

        vl_init_spline = VLSTsplineFit(vl, spline_control_points);
        VLSTwriteLabel(vl, "spline.init.label", mris, mri_aseg);
        VLSTwriteLabel(vl_init_spline, "spline.init.cpts.label", mris,
                       mri_aseg);
        init_energy = compute_spline_energy(
            vl_init_spline, mri_intensity, mri_aseg, mri_wm_dist, gm_mean,
            energy_flags, spline_length_penalty, spline_nonwm_penalty,
            spline_interior_penalty);
    ...
    val = compute_spline_energy(
        vl_spline, mri_intensity, mri_eroded_aseg, mri_wm_dist, wm_mean,
  ...
  ROMP_PF_end
      ...
      if (read_flag == 0) {
    if (randomize_data)
      sprintf(fname, "%s/%s.splines.rand.mgz", FileNamePath(mris->fname, path),
              mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
    else
      sprintf(fname, "%s/%s.splines.mgz", FileNamePath(mris->fname, path),
              mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
    ...
    if (randomize_data)
      sprintf(fname, "%s/%s.entropy.rand.mgz", FileNamePath(mris->fname, path),
              mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
    else
      sprintf(fname, "%s/%s.entropy.mgz", FileNamePath(mris->fname, path),
              mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
    ...
    if (randomize_data)
      ...
      sprintf(fname, "%s/%s.posterior.mgz", FileNamePath(mris->fname, path),
              mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
  ...
  mri_posterior_on_splines = compute_posterior_on_paths(
      mris, mri_splines, mri_total_posterior, mri_aseg, nullptr);
  if (randomize_data)
  ...
  mri_filtered_posterior_on_spline = compute_filtered_posterior_on_paths(
      mris, mri_splines, mri_total_posterior, mri_aseg, nullptr);
  if (randomize_data)
    ...
    sprintf(fname, "%s/%s.spline_posterior.filtered.mgz",
            ...
            mris->hemisphere == LEFT_HEMISPHERE ? "lh" : "rh");
  printf(
      "writing posterior projected and filtered onto optimal splines  to %s\n",
      fname);
  MRIwrite(mri_filtered_posterior_on_spline, fname);

  MRIfree(&mri_splines);
  MRIfree(&mri_posterior);
  VLSTfree(&vl_posterior);
  MRIfree(&mri_entropy);
  MRIfree(&mri_total_posterior);
  MRIfree(&mri_possible_migration_paths);
  MRIfree(&mri_path_grad);
  VectorFree(&v_intensity);
  VectorFree(&v_aseg);
  MatrixFree(&m_intensity2aseg);
  MRIfree(&mri_wm_dist);
  ...
  do {
    ...
    if (vl->nvox >= max_steps)
      ErrorReturn(NULL, (ERROR_BADPARM,
                         "could not find path to ventricles for vno %d", vno));
...
static VOXEL_LIST *find_optimal_spline(
  ...
  for (n = 0; n < nsamples; n++) {
    ...
    vl_spline->xd[cnum] += xn;
    vl_spline->yd[cnum] += yn;
    vl_spline->zd[cnum] += zn;
    ...
    {
      label = MRIgetVoxVal(mri_aseg, vl_spline->xi[cnum], vl_spline->yi[cnum],
                           vl_spline->zi[cnum], 0);
    ...
    energy = compute_spline_energy(
        vl_spline, mri_intensity, mri_aseg, mri_wm_dist, gm_mean, energy_flags,
    ...
    if (randomNumber(0.0, 1.0) < acceptance_val) {
      ...
      if (n > burnin && mri_posterior && n > (lastn + jump)) {
        ...
        VLSTinterpolateSplineIntoVolume(vl_spline_current, mri_total_posterior,
                                        ...
                                        1.0 / vl_interp->nvox);
        VLSTinterpolateSplineIntoVolume(vl_spline_current, mri_posterior,
                                        mri_posterior->xsize, vl_posterior,
                                        1.0 / vl_interp->nvox);
        VLSTfree(&vl_interp);
        nposterior++;
...
static double compute_spline_energy(VOXEL_LIST *vl, MRI *mri_intensity,
  ...
  for (n = num = 0; n < vl_interp->nvox; n++) {
    if (vl_interp->xi[n] == Gx && vl_interp->yi[n] == Gy &&
        vl_interp->zi[n] == Gz)
      DiagBreak();
    MRIsampleVolume(mri_wm_dist, vl_interp->xd[n], vl_interp->yd[n],
                    vl_interp->zd[n], &wm_dist);
    label = MRIgetVoxVal(mri_aseg, vl_interp->xi[n], vl_interp->yi[n],
                         vl_interp->zi[n], 0);
    ...
    MRIsampleVolume(mri_intensity, vl_interp->xd[n], vl_interp->yd[n],
                    vl_interp->zd[n], &val);
#if 0
  ...
  if (which & SPLINE_LENGTH)
    neg_log_p =
        spline_length_penalty * sqrt(vl_interp->nvox) + sqrt(neg_log_p / num);
  else
...
static MRI *compute_posterior_on_paths(MRI_SURFACE *mris, MRI *mri_splines,
  ...
  if (false) {
    ...
    MRIconvolveGaussian(mri_posterior_on_spline, mri_posterior_on_spline,
                        mri_kernel);
  ...
  if (mri_posterior_on_spline == nullptr)
  ...
  mri_masked_posterior = MRImask(mri_total_posterior, mri_mask, nullptr, 0, 0);
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    mean = VLSTcomputeSplineMedian(vl_spline, mri_masked_posterior,
                                   mri_total_posterior->xsize / 2);
    mean = VLSTcomputeSplineSegmentMean(vl_spline, mri_masked_posterior,
                                        mri_total_posterior->xsize / 2, 8, 12);
    VLSTfree(&vl_spline);
    MRIsetVoxVal(mri_posterior_on_spline, vno, 0, 0, 0, mean);
  }

  MRIfree(&mri_masked_posterior);
  MRIfree(&mri_mask);
...
compute_filtered_posterior_on_paths(MRI_SURFACE *mris, MRI *mri_splines,
  ...
  if (mri_filtered_posterior_on_spline == nullptr)
    mri_filtered_posterior_on_spline =
  ...
  for (vno = 0; vno < mris->nvertices; vno++) {
    ...
    for (total = 0.0, nspline = n = 0; n < vl->nvox; n++) {
      if (MRIgetVoxVal(mri_mask, nint(vl->xi[n]), nint(vl->yi[n]),
                       nint(vl->zi[n]), 0) == 0)
        continue; // not in wm
      MRIsampleVolume(mri_total_posterior, vl->xd[n], vl->yd[n], vl->zd[n],
                      &val);
      nm1 = n - 1;
      np1 = n + 1;
      if (n == 0)
      ...
      outside_bad = 0;
      for (nsamples = 0, outside = theta = 0.0; theta <= 2 * M_PI;
           theta += ANGLE_STEP) {
        ...
        ADD(p, x1, y1);
        if (MRIgetVoxVal(mri_mask, nint(vl->xd[n] + p[0]),
                         nint(vl->yd[n] + p[1]), nint(vl->zd[n] + p[2]),
                         0) == 0) {
          outside_bad = 1;
          break; // not in wm
        }
        MRIsampleVolume(mri_total_posterior, vl->xd[n] + p[0], vl->yd[n] + p[1],
                        vl->zd[n] + p[2], &val);
        nsamples++;
        outside += val;
      }
    ...
    VLSTfree(&vl);
    if (nspline > 0)
      MRIsetVoxVal(mri_filtered_posterior_on_spline, vno, 0, 0, 0,
                   total / (double)nspline);
  }
