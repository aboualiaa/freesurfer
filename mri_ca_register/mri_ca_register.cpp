...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_ca_register.c,v 1.99 2016/10/22 17:31:24 fischl Exp $",
      "$Name:  $");
  ...
  if (twm_fname) {
    ...
    for (count = i = 0; i < nctrl; i++) {
      switch (useRealRAS) {
      case 0:
        MRIsurfaceRASToVoxel(parms.mri_twm, pArray[i].x, pArray[i].y,
                             pArray[i].z, &xr, &yr, &zr);
        break;
      case 1:
        MRIworldToVoxel(parms.mri_twm, pArray[i].x, pArray[i].y, pArray[i].z,
                        &xr, &yr, &zr);
        break;
      default:
        ErrorExit(ERROR_BADPARM,
      ...
      if (MRIindexNotInVolume(parms.mri_twm, x, y, z) == 0) {
        ...
        gc = GCAfindSourceGC(gca, mri_inputs, transform, x, y, z,
  ...
  if (tl_fname) {
    ...
    if (!gca_tl)
      ErrorExit(ERROR_NOFILE, "%s: could not temporal lobe gca %s", Progname,
                tl_fname);
  ...
  if (tl_fname == nullptr && register_wm_flag) {
    ...
    GCAMsetLabelStatus(gcam, Right_Cerebellum_White_Matter,
                       GCAM_USE_LIKELIHOOD);
      ...
      //   if (read_lta == 0)
      {
        ...
        if (!do_secondpass_renorm) // just run it once
        {
          ...
          GCAcomputeRenormalizationWithAlignment(
              ...
              &lta, 0, label_scales, label_offsets, label_peaks,
              label_computed);
          got_scales = 1;
        } else // run it twice
        {
          ...
          GCAseqRenormalizeWithAlignment(gcam->gca, mri_morphed, transform,
                                         parms.log_fp, parms.base_name, &lta, 0,
                                         label_scales, label_offsets,
  ...
  if (parms.write_iterations != 0) {
    ...
    if (parms.diag_morph_from_atlas) {
      ...
      mri_gca = MRIalloc(gcam->atlas.width, gcam->atlas.height,
  ...
  if (false && handle_expanded_ventricles) { // one more less-restrictive morph
    ...
    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
           "!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    printf("!!!!!!!!!!!!!!!! PERFORMING LESS-CONSTRAINED BIG-VENT IN "
           "DEFORMATION!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
           "!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "DIST") || !stricmp(option, "DISTANCE")) {
    ...
    printf("limiting max grad to be %2.2f (scaling gradients that exceed this "
           "norm)\n",
           parms.max_grad);
    nargs = 1;
  ...
  } else if (!stricmp(option, "RENORM_MAP") ||
             !stricmp(option, "RENORMALIZE_MAP")) {
    renormalize = 1;
