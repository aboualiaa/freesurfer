/**
 * @file  mris_surface_to_vol_distances.c
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(argc, argv,
                                "$Id: mris_surface_to_vol_distances.c,v 1.4 "
  ...
  if (strlen(subjects_dir) == 0) {
    ...
    if (!cp)
      ErrorExit(ERROR_BADPARM,
                "%s: SUBJECTS_DIR must be specified on the command line "
                "(-sdir) or the env",
                Progname);
...
----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <average subject> <hemi> <subject 1> <subject 2> "
         "... <output prefix>\n",
         Progname);
  exit(code);
}
...
static int update_histograms(MRI_SURFACE *mris, MRI_SURFACE *mris_avg,
  ...
  for (vno_avg = 0; vno_avg < mris_avg->nvertices; vno_avg++) {
    ...
    vno = MHTfindClosestVertexNo2(mht, mris, mris_avg,
                                  &mris_avg->vertices[vno_avg], &min_dist);
    if (vno < 0)
      continue;
    v1 = &mris->vertices[vno];
    VECTOR_LOAD(vec1, v1->cx, v1->cy, v1->cz); /* radius vector */
