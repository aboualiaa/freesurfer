...
static char * write_likelihood = nullptr;
static double PRIOR_FACTOR     = 1.0;

static char *read_renorm_fname  = nullptr;
static char *write_renorm_fname = nullptr;
...
static char *save_gca_fname       = nullptr;

static char *surf_name = nullptr;
static char *surf_dir  = nullptr;
...
static MRI *GCArelabelUnlikely(GCA *gca, MRI *mri_inputs, TRANSFORM *transform,
                               MRI *mri_src_labeled, MRI *mri_dst_labeled,
                               MRI *  mri_independent_posterior,
                               double prior_thresh, int whalf);
static MRI *fix_putamen(GCA *gca, MRI *mri_inputs, MRI *mri_imp,
                        TRANSFORM *transform, MRI *mri_src_labeled,
                        MRI *mri_dst_labeled, double prior_thresh);
static MRI *gcaCheckForFCDs(MRI *mri_src, MRI *mri_dst, GCA *gca,
                            TRANSFORM *transform, MRI *mri_inputs);
#if 0
...
static MRI *replace_cortex_far_from_surface_with_wmsa(MRI *        mri_inputs,
                                                      ...
                                                      MRI_SURFACE *mris_lh,
                                                      MRI_SURFACE *mris_rh);
static MRI *MRIexpandVentricles(MRI *mri_labeled_src, MRI *mri_labeled,
                                GCA *gca, MRI *mri_inputs, int num_expansions);
int is_possible(GCA *gca, MRI *mri, TRANSFORM *transform, int x, int y, int z,
                int label);

int insert_thin_temporal_white_matter(MRI *mri_in, MRI *mri_labeled, GCA *gca,
                                      TRANSFORM *transform, GCA *gca_all);

int distance_to_label(MRI *mri_labeled, int label, int x, int y, int z, int dx,
                      int dy, int dz, int max_dist);

int preprocess(MRI *mri_in, MRI *mri_labeled, GCA *gca, TRANSFORM *transform,
               MRI *mri_fixed);

int edit_hippocampus(MRI *mri_in, MRI *mri_labeled, GCA *gca,
                     TRANSFORM *transform, MRI *mri_fixed);

int edit_amygdala(MRI *mri_in, MRI *mri_labeled, GCA *gca, TRANSFORM *transform,
                  MRI *mri_fixed);

int MRIcountNbhdLabels(MRI *mri, int x, int y, int z, int label);

int        main(int argc, char *argv[]);
static int get_option(int argc, char *argv[]);

static char * tl_gca_fname       = nullptr;
static double TR                 = -1;
...
MRI *        insert_wm_segmentation(MRI *mri_labeled, MRI *mri_wm,
                                    int parcellation_type, int fixed_flag, GCA *gca,
                                    TRANSFORM *transform);
...
int main(int argc, char *argv[]) {
  ...
  MRI *  mri_inputs, *mri_labeled, *mri_fixed = NULL, *mri_tmp,
                                 *mri_independent_posterior = NULL;
  ...
  if (gca->ninputs != (ninputs + extra) && !map_to_flash &&
      gca->type != GCA_FLASH && gca->type != GCA_PARAM)
...
#else
      mri_dir = MRIgradientDir2ndDerivative(mri_tmp, nullptr, 5);
  ...
  if (example_T1) {
    ...
    if (!mri_T1)
      ErrorExit(ERROR_NOFILE, "%s: could not read example T1 from %s", Progname,
                example_T1);
  ...
  if (gca->type == GCA_FLASH) {
    ...
    if (need_map_flag) {
      ...
      gca_tmp = GCAcreateFlashGCAfromFlashGCA(gca, TRs, fas, TEs,
                                              mri_inputs->nframes);
      GCAfree(&gca);
      gca = gca_tmp;
    }
  ...
  if (Ggca_x >= 0 && Gx < 0) {
    GCAsourceVoxelToNode(gca, mri_inputs, transform, Ggca_x, Ggca_y, Ggca_z,
                         &Gx, &Gy, &Gz);
    printf("source voxel (%d, %d, %d) maps to node (%d, %d, %d)\n", Ggca_x,
           Ggca_y, Ggca_z, Gx, Gy, Gz);
  ...
  if (heq_fname) {
    ...
    if (!mri_eq)
      ErrorExit(ERROR_NOFILE,
                "%s: could not read histogram equalization volume %s", Progname,
                heq_fname);
  ...
  if (renorm_with_histos) {
    GCAmapRenormalizeWithHistograms(gca, mri_inputs, transform, nullptr,
                                    "ca_label", nullptr, nullptr, nullptr,
                                    nullptr);
  }
  if (read_fname != nullptr &&
      reg_fname == nullptr) /* use given segmentation */
  {
    ...
    if (Ggca_x >= 0)
      printf("label(%d, %d, %d) = %s (%d), norm=%2.0f\n", Ggca_x, Ggca_y,
             Ggca_z,
             cma_label_to_name(
                 MRIgetVoxVal(mri_labeled, Ggca_x, Ggca_y, Ggca_z, 0)),
    ...
    GCAreclassifyUsingGibbsPriors(mri_inputs, gca, mri_labeled, transform,
                                  max_iter, mri_fixed, 0, nullptr, PRIOR_FACTOR,
                                  PRIOR_FACTOR);
  } else // don't read old one in - create new labeling
  {
    if (reg_fname == nullptr) // so read_fname must be NULL too
    {
      printf("labeling volume...\n");
      // create labeled volume by MAP rule
      mri_labeled               = GCAlabel(mri_inputs, gca, nullptr, transform);
      mri_independent_posterior = MRIcopy(mri_labeled, nullptr);
      // -wm fname option
      if (wm_fname) {
        MRI *mri_wm;
        ...
        mri_fixed = insert_wm_segmentation(
            mri_labeled, mri_wm, parcellation_type, fixed_flag, gca, transform);
        if (DIAG_VERBOSE_ON) {
      ...
      {
        mri_fixed = MRIclone(mri_labeled, nullptr);
      }
      ...
      if (renormalize_align) {
        ...
        if (read_renorm_fname) {
          ...
          GCAcomputeRenormalizationWithAlignment(
              gca, mri_inputs, transform, logfp, base_name, nullptr,
              handle_expanded_ventricles, label_scales, label_offsets,
              label_peaks, label_computed);
        ...
        */
        ...
        {
          MRI *mri_imp;
          mri_imp = GCAmarkImpossible(gca, mri_labeled, nullptr, transform);
          fix_putamen(gca, mri_inputs, mri_imp, transform, mri_labeled,
                      mri_labeled, -.1);
          if (Gdiag & DIAG_WRITE) {
    ...
    else /* processing longitudinal data (old version) */
    {
...

      {
        ...
        if (lta->xforms[0].src.valid == 0) {
          LTAmodifySrcDstGeom(lta, mri_labeled, nullptr); // add src info
        }
        if (lta->xforms[0].dst.valid == 0) {
          LTAmodifySrcDstGeom(lta, nullptr, mri_inputs); // add dst information
        }
      ...
      if (transform_long->type != LINEAR_VOX_TO_VOX)
        ErrorExit(ERROR_BADPARM,
                  "%s: transform type (%d) must be LINEAR_VOX_TO_VOX", Progname,
                  transform_long->type);
      mri_tmp = MRIalloc(mri_inputs->width, mri_inputs->height,
                         mri_inputs->depth, mri_labeled->type);
      MRIcopyHeader(mri_inputs, mri_tmp);

      mri_tmp = MRIlinearTransformInterp(
          mri_labeled, mri_tmp, ((LTA *)(transform_long->xform))->xforms[0].m_L,
          SAMPLE_NEAREST);
      ...
      if (renormalize_align) {
        ...
        GCAmapRenormalizeWithAlignment(gca, mri_inputs, transform, logfp,
                                       base_name, nullptr,
  ...
  if (false && reclassify_unlikely) {
    MRI *mri_sigma;
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
      MRIwrite(mri_labeled, "aseg_before.mgz");
    }
    mri_sigma =
        GCAcomputeOptimalScale(gca, transform, mri_inputs, mri_labeled, nullptr,
    ...
    GCAreclassifyVoxelsAtOptimalScale(gca, transform, mri_inputs, mri_labeled,
                                      mri_labeled, mri_sigma, unlikely_wsize);
#endif
    if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
  ...
  if (read_fname == nullptr) {
    GCAconstrainLabelTopology(
        gca, mri_inputs, mri_labeled, mri_labeled, transform, Gvent_topo_dist,
        Gvent_topo_volume_thresh1, Gvent_topo_volume_thresh2);
  }
  if (wmsa) {
    MRI *mri_tmp;
    if (surf_name) {
      ...
      if (mris_lh == nullptr) {
        ErrorExit(ERROR_NOFILE, "%s: could not read lh from %s", Progname,
                  fname);
      }
      sprintf(fname, "%s/rh.%s", surf_dir, surf_name);
      mris_rh = MRISread(fname);
      if (mris_rh == nullptr) {
        ErrorExit(ERROR_NOFILE, "%s: could not read rh from %s", Progname,
                  fname);
      }
      replace_cortex_far_from_surface_with_wmsa(mri_inputs, mri_labeled,
                                                mri_labeled, mris_lh, mris_rh);
    }
    mri_tmp =
        GCAlabelWMandWMSAs(gca, mri_inputs, mri_labeled, nullptr, transform);
    MRIfree(&mri_labeled);
    mri_labeled = mri_tmp;
  }
  if (wmsa_probs != nullptr)
  // if (wmsa_probs)
  {
    MRI *probs_tmp;
    char fname[STRLEN];
    ...
    probs_tmp = GCAsampleToVolWMSAprob(mri_inputs, gca, transform, nullptr);
  ...
  if (!mri_labeled->ct)
    mri_labeled->ct = CTABreadDefault();
  ...
  if (G_write_probs != NULL) {
    ...
    mri_probs = GCAcomputeTopNProbabilities(mri_inputs, gca, mri_labeled, NULL,
                                            transform, 3);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "NOGIBBS")) {
    ...
    printf("\t-renormalize %d %d\n\t-a %d\n\t-regularize %2.3f\n",
           renormalize_iter, renormalize_wsize, avgs, regularize);
...
static int cma_editable_labels[] = {Left_Hippocampus,
                                    Right_Hippocampus,
...
#define NEDITABLE_LABELS                                                       \
  sizeof(cma_editable_labels) / sizeof(cma_editable_labels[0])

MRI *insert_wm_segmentation(MRI *mri_labeled, MRI *mri_wm,
                            int parcellation_type, int fixed_flag, GCA *gca,
                            TRANSFORM *transform) {
  int x, y, z, width, depth, height, change_label[1000], n, label, nchanged, rh,
      lh, xn, yn, zn;
  MRI *      mri_fixed;
  double     wm_prior;
  GCA_PRIOR *gcap;

  mri_fixed = MRIclone(mri_wm, nullptr);
  ...
  for (nchanged = z = 0; z < depth; z++) {
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        if (!GCAsourceVoxelToNode(gca, mri_labeled, transform, x, y, z, &xn,
                                  &yn, &zn)) {
...
#define FIXED_PRIOR  0.5
          ...
          lh = MRIcountNbhdLabels(mri_labeled, x, y, z,
                                  Left_Cerebral_White_Matter);
          lh += MRIcountNbhdLabels(mri_labeled, x, y, z, Left_Cerebral_Cortex);
          rh = MRIcountNbhdLabels(mri_labeled, x, y, z,
                                  Right_Cerebral_White_Matter);
          rh += MRIcountNbhdLabels(mri_labeled, x, y, z, Right_Cerebral_Cortex);
          if (rh > lh) {
...
#define NEXPANDABLE_LABELS                                                     \
  sizeof(cma_expandable_labels) / sizeof(cma_expandable_labels[0])

int preprocess(MRI *mri_inputs, MRI *mri_labeled, GCA *gca,
               TRANSFORM *transform, MRI *mri_fixed) {
  int i;

  GCArelabel_cortical_gray_and_white(gca, mri_inputs, mri_labeled, mri_labeled,
                                     transform);
  if (gca_write_iterations != 0) {
    char fname[STRLEN];
  ...
  if (tl_gca_fname) {
    ...
    insert_thin_temporal_white_matter(mri_inputs, mri_labeled, gca_tl,
                                      transform, gca);
    GCAfree(&gca_tl);
    if (gca_write_iterations != 0) {
      char fname[STRLEN];
...
int edit_hippocampus(MRI *mri_inputs, MRI *mri_labeled, GCA *gca,
                     TRANSFORM *transform, MRI *mri_fixed) {
  int width, height, depth, x, y, z, label, nchanged, dleft, olabel, dright,
      dpos, dant, dup, ddown, dup_hippo, ddown_gray, i, left, ddown_ventricle,
      yi, found_wm, found_hippo, was_wm, was_hippo, was_unknown, total_changed,
      xn, yn, zn;
  MRI * mri_tmp;
  float phippo, pwm;
  GC1D *gc;

  mri_tmp = MRIcopy(mri_labeled, nullptr);
        ...
        if (!GCAsourceVoxelToNode(gca, mri_labeled, transform, x, y, z, &xn,
                                     ...
                                     x direction */
          {
            ...
            if (MRIgetVoxVal(mri_labeled, xi, y, z, 0) ==
                Left_Cerebral_Cortex) {
              if (GCAlabelExists(gca, mri_labeled, transform, xi, y, z,
                                 Left_Hippocampus) == 0) {
                continue;
              }
          ...
          case Left_Hippocampus:
            pwm    = GCAlabelProbability(mri_inputs, gca, transform, (float)x,
                                      (float)y, (float)z,
                                      Left_Cerebral_White_Matter);
            phippo = GCAlabelProbability(mri_inputs, gca, transform, (float)x,
                                         (float)y, (float)z, Left_Hippocampus);
            if (phippo > 2 * pwm) {
            ...
            dright = distance_to_label(mri_labeled, Left_Cerebral_White_Matter,
                                       x, y, z, 1, 0, 0, 3);
            ddown  = distance_to_label(mri_labeled, Left_Cerebral_White_Matter,
                                      x, y, z, 0, 1, 0, 2);
            dup  = distance_to_label(mri_labeled, Left_Cerebral_White_Matter, x,
                                    y, z, 0, -1, 0, 2);
            dpos = distance_to_label(mri_labeled, Left_Cerebral_White_Matter, x,
                                     y, z, 0, 0, -1, 3);
            dant = distance_to_label(mri_labeled, Left_Cerebral_White_Matter, x,
                                     y, z, 0, 0, 1, 3);

            if ((dpos <= 2 && dant <= 2) || (dleft <= 2 && dright <= 2) ||
                (dup <= 1 && (dleft == 1 || dright == 1))) {
              if (GCAlabelExists(gca, mri_labeled, transform, x, y, z,
          ...
          case Right_Hippocampus:
            pwm    = GCAlabelProbability(mri_inputs, gca, transform, (float)x,
                                      ...
                                      Right_Cerebral_White_Matter);
            phippo = GCAlabelProbability(mri_inputs, gca, transform, (float)x,
                                         (float)y, (float)z, Right_Hippocampus);
            if (phippo > 2 * pwm) {
              continue; /* don't let it change */
            }
            ...
            dright = distance_to_label(mri_labeled, Right_Cerebral_White_Matter,
                                       x, y, z, 1, 0, 0, 3);
            ddown  = distance_to_label(mri_labeled, Right_Cerebral_White_Matter,
                                      x, y, z, 0, 1, 0, 2);
            dup = distance_to_label(mri_labeled, Right_Cerebral_White_Matter, x,
                                    y, z, 0, -1, 0, 2);
            dpos = distance_to_label(mri_labeled, Right_Cerebral_White_Matter,
                                     x, y, z, 0, 0, -1, 3);
            dant = distance_to_label(mri_labeled, Right_Cerebral_White_Matter,
                                     x, y, z, 0, 0, 1, 3);
            if ((dpos <= 2 && dant <= 2) || (dleft <= 2 && dright <= 2) ||
                (dup <= 1 && (dleft == 1 || dright == 1))) {
              if (GCAlabelExists(gca, mri_labeled, transform, x, y, z,
    ...
    for (z = 0; z < depth; z++) {
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          switch (label) {
          case Left_Hippocampus:
            ddown = distance_to_label(mri_tmp, Left_Cerebral_White_Matter, x, y,
                                      z, 0, 1, 0, 2);
            dup   = distance_to_label(mri_tmp, Left_Cerebral_White_Matter, x, y,
                                    z, 0, -1, 0, 3);

            ddown_gray = distance_to_label(mri_tmp, Left_Cerebral_Cortex, x, y,
                                           z, 0, 1, 0, 3);
            dup_hippo = distance_to_label(mri_tmp, Left_Hippocampus, x, y, z, 0,
                                          -1, 0, 3);

            ddown_ventricle =
                MIN(distance_to_label(mri_tmp, Left_Lateral_Ventricle, x, y, z,
                                      0, 1, 0, 3),
                    distance_to_label(mri_tmp, Left_Inf_Lat_Vent, x, y, z, 0, 1,
                                      0, 3));
            ...
            break;
          case Right_Hippocampus:
            ddown = distance_to_label(mri_tmp, Right_Cerebral_White_Matter, x,
                                      y, z, 0, 1, 0, 3);
            dup = distance_to_label(mri_tmp, Right_Cerebral_White_Matter, x, y,
                                    z, 0, -1, 0, 3);

            ddown_gray = distance_to_label(mri_tmp, Right_Cerebral_Cortex, x, y,
                                           z, 0, 1, 0, 3);
            dup_hippo  = distance_to_label(mri_tmp, Right_Hippocampus, x, y, z,
                                          0, -1, 0, 3);

            ddown_ventricle =
                MIN(distance_to_label(mri_tmp, Right_Lateral_Ventricle, x, y, z,
                                      0, 1, 0, 3),
                    distance_to_label(mri_tmp, Right_Inf_Lat_Vent, x, y, z, 0,
                                      1, 0, 3));
          ...
          case Right_Cerebral_Cortex:
            ...
            */
            ...
            if (dup <= 2 && ddown <= 3) {
              ...
              if (GCAlabelExists(gca, mri_labeled, transform, x, y, z, label) ==
                  0) {
                continue;
              }
...
#define MIN_UNKNOWN 5
          ...
          if (was_unknown >= MIN_UNKNOWN) {
            if (GCAlabelExists(gca, mri_labeled, transform, x, y, z,
                               Left_Cerebral_Cortex) == 0 &&
                GCAlabelExists(gca, mri_labeled, transform, x, y, z,
                               Right_Cerebral_Cortex) == 0) {
              continue;
            }
            nchanged++;
    ...
    for (nchanged = z = 0; z < depth; z++) {
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          ...
          switch (label) {
          case Left_Cerebral_Cortex:
            if (MRIcountNbhdLabels(mri_labeled, x, y, z, Left_Hippocampus) >=
                20) {
              nchanged++;
              total_changed++;
              if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
            ...
            break;
          case Right_Cerebral_Cortex:
            if (MRIcountNbhdLabels(mri_labeled, x, y, z, Right_Hippocampus) >=
                20) {
              nchanged++;
              total_changed++;
              if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
...
int edit_amygdala(MRI *mri_inputs, MRI *mri_labeled, GCA *gca,
                  TRANSFORM *transform, MRI *mri_fixed) {
  int width, height, depth, x, y, z, label, nchanged, olabel, i, left, yi,
      found_wm, found_amygdala, was_wm, was_amygdala;
  MRI *mri_tmp;
      ...
      for (x = 0; x < width; x++) {
        ...
        case Right_Amygdala:
          ...
          if (found_wm && found_amygdala &&
              GCAisPossible(gca, mri_tmp, label, transform, x, y, z, 0)) {
            nchanged++;
            MRIsetVoxVal(mri_tmp, x, y, z, 0, label);
            if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
              printf("(%d, %d, %d) %s changed to "
...
int insert_thin_temporal_white_matter(MRI *mri_inputs, MRI *mri_labeled,
                                      GCA *gca, TRANSFORM *transform,
                                      GCA *gca_all) {
  MRI *mri_tmp, *mri_probs, *mri_tmp_labels;
  int  width, height, depth, x, y, z, n, nsamples, i, xp, yp, zp;
  int  xmin, xmax, ymin, ymax, zmin, zmax, yi, zi, yimax, ximax, zimax, **added,
      nchanged, label, r, c, v;
  GCA_PRIOR * gcap;
  GC1D *      gc;
  GCA_SAMPLE *gcas;
  double      p, pmax;

  mri_tmp        = MRIclone(mri_inputs, nullptr);
  mri_tmp_labels = MRIclone(mri_labeled, nullptr);
  ...
  GCAcomputeLogSampleProbabilityUsingCoords(gca, gcas, mri_inputs, transform,
  ...
  for (i = 0; i < nsamples; i++) {
    ...
    MRIsetVoxVal(mri_tmp_labels, gcas[i].x, gcas[i].y, gcas[i].z, 0,
                 gcas[i].label);
  }
  ...
  */
  ...
  for (i = 0, x = xmin; x <= xmax; x++) {
    for (z = zmin; z <= zmax; z++) {
      for (y = ymin; y <= ymax; y++) {
        if (MRIgetVoxVal(mri_tmp_labels, x, y, z, 0) !=
            Left_Cerebral_White_Matter) {
          continue;
        }
        yi = mri_probs->yi[y + 1];
    ...
    for (x = xmin; x <= xmax; x++) {
      for (z = zmin; z <= zmax; z++) {
        ...
        for (y = ymin; y <= ymax; y++) {
          ...
          nlabels = GCAcomputeVoxelLikelihoods(gca_all, mri_inputs, x, y, z,
                                               transform, labels, probs);
...
#else
          if (!GCAsourceVoxelToPrior(gca_all, mri_inputs, transform, x, yi, z,
                                     &xp, &yp, &zp)) {
    ...
    if (MRIgetVoxVal(mri_tmp, x, y, z, 0)) {
      if (x == Ggca_x && y == Ggca_y && z == Ggca_z) {
        printf("changing voxel (%d, %d, %d) from %s to %s\n", x, y, z,
               cma_label_to_name(MRIgetVoxVal(mri_labeled, x, y, z, 0)),
...
#define EXCLUDED(l)                                                            \
  (((l) == Unknown) || ((l) == CSF) || ((l) == Third_Ventricle))
static MRI *MRIexpandVentricles(MRI *mri_labeled_src, MRI *mri_labeled,
                                GCA *gca, MRI *mri_inputs, int num_expansions) {
  ...
  MRI *    mri_dilated = nullptr, *mri_border = nullptr;
  float label_means[MAX_CMA_LABELS], label_stds[MAX_CMA_LABELS], vdist, ldist,
      val;

  if (mri_labeled == nullptr) {
    mri_labeled = MRIcopy(mri_labeled_src, nullptr);
  }
  ...
  mri_vent = MRIclone(mri_labeled_src, nullptr);
  ...
  mri_vent_orig = MRIcopy(mri_vent, nullptr);
  thresh        = mean + .5 * std;
  printf(
      "expanding ventricles using %2.1f + 0.5 * %2.1f = %2.1f as threshold\n",
      mean, std, thresh);
  ...
  for (n = 0; n < num_expansions; n++) {
    ...
    vl          = VLSTcreate(mri_border, 1, 1, nullptr, 0, 0);
    nadded      = 0;
    for (i = 0; i < vl->nvox; i++) {
      x = vl->xi[i];
      y = vl->yi[i];
    ...
    for (y = 0; y < mri_vent->height; y++)
      for (z = 0; z < mri_vent->depth; z++) {
        ...
        if (MRIgetVoxVal(mri_vent, x, y, z, 0) > 0 &&
            MRIgetVoxVal(mri_vent_orig, x, y, z, 0) ==
                0) { // new ventricle label
          int lh, rh, third, lhi, rhi;
          lh    = MRIlabelsInNbhd(mri_labeled_src, x, y, z, num_expansions,
                               Left_Lateral_Ventricle);
          lhi   = MRIlabelsInNbhd(mri_labeled_src, x, y, z, num_expansions,
                                Left_Inf_Lat_Vent);
          rh    = MRIlabelsInNbhd(mri_labeled_src, x, y, z, num_expansions,
                               Right_Lateral_Ventricle);
          rhi   = MRIlabelsInNbhd(mri_labeled_src, x, y, z, num_expansions,
                                Right_Inf_Lat_Vent);
          third = MRIlabelsInNbhd(mri_labeled_src, x, y, z, num_expansions,
                                  Third_Ventricle);
...
static MRI *replace_cortex_far_from_surface_with_wmsa(MRI *        mri_inputs,
  ...
  v_tmp =
      MRImeanInLabelMultispectral(mri_inputs, mri_dst, Left_Cerebral_Cortex);
  v_gm_mean =
      MRImeanInLabelMultispectral(mri_inputs, mri_dst, Right_Cerebral_Cortex);
  VectorAdd(v_tmp, v_gm_mean, v_gm_mean);
  VectorScalarMul(v_gm_mean, 0.5, v_gm_mean);
  VectorFree(&v_tmp);

  v_tmp     = MRImeanInLabelMultispectral(mri_inputs, mri_dst,
                                      Left_Cerebral_White_Matter);
  v_wm_mean = MRImeanInLabelMultispectral(mri_inputs, mri_dst,
                                          Right_Cerebral_White_Matter);
  ...
  v_tmp =
      MRImeanInLabelMultispectral(mri_inputs, mri_dst, Left_WM_hypointensities);
  v_wmsa_mean = MRImeanInLabelMultispectral(mri_inputs, mri_dst,
          ...
          if (wmsa_dist < gm_dist && wmsa_dist < wm_dist) {
            ...
            if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
              printf("voxel (%d, %d, %d) is interior to surface "
                     ...
                     x, y, z, cma_label_to_name(label), wm_dist, gm_dist,
                     wmsa_dist, MRIgetVoxVal(mri_lh_dist, x, y, z, 0),
...
static MRI *fix_putamen(GCA *gca, MRI *mri_inputs, MRI *mri_imp,
  ...
  if (mri_dst_labeled == nullptr) {
    mri_dst_labeled = MRIcopy(mri_src_labeled, nullptr);
  }
  if (prior_thresh < 0) {
            ...
            if (n < gcap->nlabels) // found putamen
            {
              ...
              if (IS_PUTAMEN(label) && gcap->priors[n] < prior_thresh) {
                ...
                if (((IS_CORTEX(left) || IS_WM(left)) &&
                  ...
                  pwm = GCAlabelProbability(mri_inputs, gca, transform,
                                            (float)x, (float)y, (float)z, wm);
                  pgm = GCAlabelProbability(mri_inputs, gca, transform,
...
VOXEL_LIST *find_unlikely_voxels_in_region(
    GCA *gca, TRANSFORM *transform, MRI *mri_labeled, double prior_thresh,
    MRI *mri_prior_labels, MRI *mri_priors, int x, int y, int z, int whalf) {
...
static int change_unlikely_voxels(GCA *gca, MRI *mri_dst_label, MRI *mri_inputs,
                                  TRANSFORM *transform, VOXEL_LIST *vl,
                                  int label_to_change, MRI *mri_prior_labels) {
  ...
  for (nchanged = i = 0; i < vl->nvox; i++) {
    ...
    if (false && x == Ggca_x && y == Ggca_y && z == Ggca_z) {
      printf("change_unlikely_voxels 1: changing label at (%d, %d, %d) from %s "
             "to %s\n",
             Ggca_x, Ggca_y, Ggca_z, cma_label_to_name(label_to_change),
  ...
  for (x = 0; x < mri_inputs->width; x++)
    for (y = 0; y < mri_inputs->height; y++)
      for (z = 0; z < mri_inputs->depth; z++) {
        ...
        label = GCAgetMaxPriorLabelAtVoxel(gca, mri_dst_labeled, x, y, z,
                                           transform, &prior);
    ...
    for (index = 0; index < nindices; index++) {
      ...
      if ((label == (int)MRIgetVoxVal(mri_prior_labels, x, y, z, 0) &&
           (mri_independent_posterior &&
            label ==
                (int)MRIgetVoxVal(mri_independent_posterior, x, y, z, 0)))) {
        continue;
      }
...
#endif
      posterior_before = GCAwindowPosteriorLogProbability(
          gca, mri_dst_labeled, mri_inputs, transform, x, y, z, whalf);

      vl = find_unlikely_voxels_in_region(gca, transform, mri_dst_labeled,
                                          prior_thresh, mri_prior_labels,
                                          mri_priors, x, y, z, whalf);
      ...
      nchanged_tmp =
          change_unlikely_voxels(gca, mri_dst_labeled, mri_inputs, transform,
                                 vl, label, mri_prior_labels);
      posterior_after = GCAwindowPosteriorLogProbability(
          gca, mri_dst_labeled, mri_inputs, transform, x, y, z, whalf);
      if (posterior_after <= posterior_before) {
        ...
        if (Ggca_x >= 0) {
          int max_label =
              MRIgetVoxVal(mri_dst_labeled, Ggca_x, Ggca_y, Ggca_z, 0);
          if (old_label != max_label &&
              VLSTinList(vl, Ggca_x, Ggca_y, Ggca_z)) {

            printf("iter = %d: change_unlikely_voxels: changing label at (%d, "
                   "%d, %d) from %s to %s\n",
                   i, Ggca_x, Ggca_y, Ggca_z, cma_label_to_name(old_label),
                   cma_label_to_name(max_label));
          }
        }
        ...
        for (z = 0; z < mri_src->depth; z++) {
          ...
          if (Ggca_x == x && Ggca_y == y && Ggca_z == z)
            printf("label at (%d, %d, %d) = %s\n", x, y, z,
                   cma_label_to_name(label));
          if (IS_HYPO(label)) {
            out_label = Left_WM_hypointensities ? Left_non_WM_hypointensities
            ...
            if (MRIcountNbhdLabels(mri_src, x, y, z, Left_WM_hypointensities) >
                    0 ||
                MRIcountNbhdLabels(mri_src, x, y, z, Right_WM_hypointensities) >
                    0 ||
                MRIcountNbhdLabels(mri_src, x, y, z, WM_hypointensities) > 0 ||
                MRIcountNbhdLabels(mri_dst, x, y, z,
                                   Left_non_WM_hypointensities) > 0 ||
                MRIcountNbhdLabels(mri_dst, x, y, z,
                                   Right_non_WM_hypointensities) > 0)
            ...
            if (label == Left_Cerebral_White_Matter) {
              ...
              gm_gc    = GCAfindSourceGC(gca, mri_src, transform, x, y, z,
                                      Left_Cerebral_Cortex);
            } else {
            ...
            wm_gc   = GCAfindSourceGC(gca, mri_src, transform, x, y, z,
                                    Left_Cerebral_Cortex);
            gm_dist = gm_gc ? fabs(val - gm_gc->means[0])
                            : fabs(val - global_gm_mean);
            wm_dist = wm_gc ? fabs(val - wm_gc->means[0])
                            : fabs(val - global_wm_mean);
            if (x == Ggca_x && y == Ggca_y && z == Ggca_z)
              printf("%d, %d, %d) = %2.0f, WM dist = %2.1f, prior = %2.2f, GM: "
                     "dist = %2.1f,  prior = %2.2f, thresh %2.1f\n",
                     x, y, z, val, wm_dist, wm_prior, gm_dist, gm_prior,
                     thresh);
          ...
          if (label != out_label && (x == Ggca_x && y == Ggca_y && z == Ggca_z))
            printf("(%d %d %d): changing from %s to %s\n", x, y, z,
                   cma_label_to_name(label), cma_label_to_name(out_label));
          if (label != out_label) {
            nchanged++;
