...
static int postprocess_segmentation_with_aseg(MRI *mri_labeled, MRI *mri_aseg,
                                              int min_voxels);
...
main(int argc, char *argv[])
{
  ...
  if (single_classifier_flag) {
    ...
    if (!rfa)
      ErrorExit(ERROR_NOFILE,
                "%s: could not read random forest classifier array from %s",
                Progname, rfa_fname);

    if (rfa->ninputs !=
        (ninputs + extra + 3)) // +3 is for spatial coords or priors
  ...
  if (single_classifier_flag) {
    mri_labeled =
        label_with_random_forest(rf, transform, gca, wm_thresh, mri_inputs,
                                 nullptr, wmsa_whalf, mri_aseg, &mri_pvals);
    if (only_nbrs_rf_fname) {
      ...
      relabel_wmsa_nbrs_with_random_forest(rf_nbrs, transform, gca, mri_inputs,
                                           mri_labeled);
    }
    if (mri_aseg)
      postprocess_segmentation_with_aseg(mri_labeled, mri_aseg, min_voxels);
    if (nsurfs > 0)
      remove_wmsas_close_to_surface(surface_names, nsurfs, mri_labeled,
                                    surface_dist);
    if (pthresh >= 0)
      postprocess_grow_wmsas(mri_labeled, mri_pvals, pthresh, mri_aseg);
  } else
    mri_labeled = RFAlabel(mri_inputs, rfa, nullptr, transform);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "aseg")) {
    ...
    printf("removing WMSA segments that are more than %2.1f mm interior to the "
           "?h.%s surfaces\n",
           surface_dist, argv[2]);
    nargs = 1;
  } else if (!stricmp(option, "surface_dist")) {
    surface_dist = atof(argv[2]);
    printf("removing WMSA segments that are more than %2.1f mm interior "
           "surfaces\n",
           surface_dist);
    nargs = 1;
...
static MRI *label_with_random_forest(RANDOM_FOREST *rf, TRANSFORM *transform,
  ...
  mri_wmsa_possible = MRIclone(mri_labeled, nullptr);
...
static MRI *relabel_wmsa_nbrs_with_random_forest(RANDOM_FOREST *rf,
  ...
  mri_orig_labeled = MRIcopy(mri_labeled, nullptr);
  mri_mask = MRIcopy(mri_labeled, nullptr);
  ...
  printf("%2.0f%% (%d) WMSA added and %2.0f%% (%d) removed, %2.0f%% (%d)\n",
         100.0 * (float)non / total, non, 100.0 * (float)noff / total, noff,
         100.0 * (float)nunchanged / total, nunchanged);
  free(feature);
  MRIfree(&mri_mask);
  MRIfree(&mri_orig_labeled);
  return (mri_labeled);
}

static int close_labels[] = {CSF,
...
static int postprocess_segmentation_with_aseg(MRI *mri_labeled, MRI *mri_aseg,
    ...
    for (y = 0; y < mri_aseg->height; y++)
      for (z = 0; z < mri_aseg->depth; z++) {
        ...
        if (out_wmsa_label != in_wmsa_label) {
          if (x == Gx && y == Gy && z == Gz)
            printf("voxel (%d, %d, %d): WMSA label removed during "
                   "postprocessing\n",
                   x, y, z);
          nchanged++;
        }
  ...
  mri_closed = MRIclose(mri_labeled, nullptr);
        ...
        if (MRIgetVoxVal(mri_dist_total, x, y, z, 0) > surface_dist) {
          if (x == Gx && y == Gy && z == Gz)
            printf("removing voxel (%d, %d, %d): surface_dist = %2.3f\n", x, y,
                   z, MRIgetVoxVal(mri_dist, x, y, z, 0));
          nchanged++;
