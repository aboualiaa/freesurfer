      ...
      for (a = i->first; a; a = a->next)
        if (a->r_cap) {
          ...
          if (!j->parent) {
            ...
            break;
          else if (j->mark_count && j->mark_count <= i->mark_count &&
                   j->mark_d > i->mark_d) {
...
Graph::termtype Graph::what_segment(node_id i) {
  if (((node *)i)->parent && !((node *)i)->is_sink)
    return SOURCE;
  return SINK;
}
...
void map(int index, int *x, int *y, int *z, int xV, int yV, int zV, int xoff,
         int yoff, int zoff) {
  *z = index / (yV * xV);
  *y = (index - (*z) * yV * xV) / xV;
  *x = index - (*z) * yV * xV - (*y) * xV;
  ...
  for (int z = 0; z < zVol; z++) {
    for (int y = 0; y < yVol; y++) {
      for (int x = 0; x < xVol; x++) {
        if (x == 0 || x == xVol - 1 || y == 0 || y == yVol - 1 || z == 0 ||
            z == zVol - 1)
          pointer[z][y][x] = 0;
      }
    }
...
void mincut(int ***im_gcut, edgeW *hor, edgeW *ver, edgeW *tra, int length_h,
  ...
  */
  ...
  for (i = 0; i < m1; i++) {
    ...
    g->add_edge(nodes[hor[i].edge2 - 1], nodes[hor[i].edge1 - 1], hor[i].weight,
                hor[i].weight);
    //  if( i % 10000 == 0 )
  ...
  */
  ...
  for (i = 0; i < m2; i++) {
    ...
    g->add_edge(nodes[ver[i].edge2 - 1], nodes[ver[i].edge1 - 1], ver[i].weight,
                ver[i].weight);
    //  if( i % 10000 == 0 )
  ...
  */
  ...
  for (i = 0; i < m3; i++) {
    ...
    g->add_edge(nodes[tra[i].edge2 - 1], nodes[tra[i].edge1 - 1], tra[i].weight,
                tra[i].weight);
    //  if( i % 10000 == 0 )
...
void decide_bound(unsigned char ***image, double threshold, int xVol, int yVol,
...
double graphcut(unsigned char ***image, unsigned char ***label, int ***im_gcut,
                int ***foreSW, int ***backSW, int xVol, int yVol, int zVol,
  ...
  decide_bound(image, threshold, xVol, yVol, zVol, x_start, x_end, y_start,
               y_end, z_start, z_end);
  // new range
  ...
  for (i = 0, j = 0; i < length_h; j++) {
    ...
    if (hor[i].weight != 1 && hor[i].weight != 6 && hor[i].weight != 0) {
      unsigned char value = image[z][y][x] > image[z][y][x + 1]
                                ...
                                : image[z][y][x];
      hor[i].weight =
          (int)fabs(hor[i].weight * (exp(k * (value - threshold)) - 1));
    }
    if (hor[i].weight > 1 && hor[i].weight < 6)
      hor[i].weight = 6;
  ...
  for (i = 0, j = 0; i < length_v; j++) {
    ...
    if (ver[i].weight != 1 && ver[i].weight != 6 && ver[i].weight != 0) {
      unsigned char value = image[z][y][x] > image[z][y + 1][x]
                                ...
                                : image[z][y][x];
      ver[i].weight =
          (int)fabs(ver[i].weight * (exp(k * (value - threshold)) - 1));
    }
    if (ver[i].weight > 1 && ver[i].weight < 6)
      ver[i].weight = 6;
  ...
  for (i = 0, j = 0; i < length_t; j++) {
    ...
    if (tra[i].weight != 1 && tra[i].weight != 6 && tra[i].weight != 0) {
      unsigned char value = image[z][y][x] > image[z + 1][y][x]
                                ...
                                : image[z][y][x];
      tra[i].weight =
          (int)fabs(tra[i].weight * (exp(k * (value - threshold)) - 1));
    }
    if (tra[i].weight > 1 && tra[i].weight < 6)
      tra[i].weight = 6;
  ...
  // mincut
  printf("doing mincut...\n");
  mincut(im_gcut, hor, ver, tra, length_h, length_v, length_t, x_start, y_start,
         z_start, x_end, y_end, z_end);
...
int calculate_conv(int ***image, int xCor, int yCor, int zCor,
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        sum = sum +
              image[zCor + z][yCor + y][xCor + x] * conv_m[1 + z][1 + y][1 + x];
      }
    }
  }
  return sum;
