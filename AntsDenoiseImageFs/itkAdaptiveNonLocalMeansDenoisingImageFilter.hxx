...
AdaptiveNonLocalMeansDenoisingImageFilter<
    ...
    TMaskImage>::AdaptiveNonLocalMeansDenoisingImageFilter()
    : m_UseRicianNoiseModel(true), m_Epsilon(0.00001), m_MeanThreshold(0.95),
      m_VarianceThreshold(0.5), m_SmoothingFactor(1.0),
...
template <typename TInputImage, typename TOutputImage, typename TMaskImage>
void AdaptiveNonLocalMeansDenoisingImageFilter<
  ...
  typename MeanImageFilterType::Pointer                  meanImageFilter =
      MeanImageFilterType::New();
  meanImageFilter->SetInput(inputImage);
  meanImageFilter->SetRadius(
      this->GetNeighborhoodRadiusForLocalMeanAndVariance());

  this->m_MeanImage = meanImageFilter->GetOutput();
  this->m_MeanImage->Update();
  this->m_MeanImage->DisconnectPipeline();

  typedef VarianceImageFilter<InputImageType, RealImageType>
                                            VarianceImageFilterType;
  typename VarianceImageFilterType::Pointer varianceImageFilter =
      VarianceImageFilterType::New();
  varianceImageFilter->SetInput(inputImage);
  varianceImageFilter->SetRadius(
      this->GetNeighborhoodRadiusForLocalMeanAndVariance());
  ...
  this->m_MaximumInputPixelIntensity =
      static_cast<RealType>(statsFilter->GetMaximum());
  this->m_MinimumInputPixelIntensity =
      static_cast<RealType>(statsFilter->GetMinimum());

  this->m_ThreadContributionCountImage = RealImageType::New();
  this->m_ThreadContributionCountImage->CopyInformation(inputImage);
  this->m_ThreadContributionCountImage->SetRegions(
...
void AdaptiveNonLocalMeansDenoisingImageFilter<
    TInputImage, TOutputImage,
    TMaskImage>::ThreadedGenerateData(const RegionType &region,
  ...
  NeighborhoodOffsetListType neighborhoodPatchOffsetList =
      this->GetNeighborhoodPatchOffsetList();

  NeighborhoodRadiusType neighborhoodSearchRadius =
      this->GetNeighborhoodSearchRadius();

  ConstNeighborhoodIterator<RealImageType> ItV(neighborhoodSearchRadius,
                                               this->m_VarianceImage, region);
  ConstNeighborhoodIterator<RealImageType> ItM(neighborhoodSearchRadius,
                                               this->m_MeanImage, region);
  ...
  while (!ItM.IsAtEnd()) {
    ...
    if (inputCenterPixel > 0 && meanCenterPixel > this->m_Epsilon &&
        varianceCenterPixel > this->m_Epsilon &&
        (!maskImage || maskImage->GetPixel(centerIndex) !=
                           NumericTraits<MaskPixelType>::ZeroValue())) {
      // Calculate the minimum distance

      RealType minimumDistance = NumericTraits<RealType>::max();
      for (unsigned int m = 0; m < neighborhoodSearchSize; m++) {
        if (!ItM.IndexInBounds(m) ||
            m == static_cast<unsigned int>(0.5 * neighborhoodSearchSize)) {
          continue;
        }
        ...
        varianceNeighborhoodPixel =
            this->m_VarianceImage->GetPixel(neighborhoodIndex);

        if (meanNeighborhoodPixel <= this->m_Epsilon ||
            varianceNeighborhoodPixel <= this->m_Epsilon) {
          continue;
        }

        const RealType meanRatio = meanCenterPixel / meanNeighborhoodPixel;
        const RealType meanRatioInverse =
            (this->m_MaximumInputPixelIntensity - meanCenterPixel) /
            (this->m_MaximumInputPixelIntensity - meanNeighborhoodPixel);

        const RealType varianceRatio =
            varianceCenterPixel / varianceNeighborhoodPixel;

        if (((meanRatio > this->m_MeanThreshold &&
              meanRatio < itk::NumericTraits<RealType>::OneValue() /
                              this->m_MeanThreshold) ||
             (meanRatioInverse > this->m_MeanThreshold &&
              meanRatioInverse < itk::NumericTraits<RealType>::OneValue() /
                                     this->m_MeanThreshold)) &&
            varianceRatio > this->m_VarianceThreshold &&
            varianceRatio < itk::NumericTraits<RealType>::OneValue() /
                                this->m_VarianceThreshold) {
          ...
          for (unsigned int n = 0; n < neighborhoodPatchSize; n++) {
            IndexType neighborhoodPatchIndex =
                neighborhoodIndex + neighborhoodPatchOffsetList[n];

            if (!targetImageRegion.IsInside(neighborhoodPatchIndex)) {
              continue;
            }
            RealType neighborhoodInputImagePixel = static_cast<RealType>(
                inputImage->GetPixel(neighborhoodPatchIndex));
            RealType neighborhoodMeanImagePixel =
                this->m_MeanImage->GetPixel(neighborhoodPatchIndex);
            averageDistance += itk::Math::sqr(neighborhoodInputImagePixel -
                                              neighborhoodMeanImagePixel);
      ...
      if (itk::Math::AlmostEquals(minimumDistance,
      ...
      if (this->m_UseRicianNoiseModel) {
        for (unsigned int n = 0; n < neighborhoodPatchSize; n++) {
          IndexType neighborhoodPatchIndex =
              centerIndex + neighborhoodPatchOffsetList[n];
          if (!targetImageRegion.IsInside(neighborhoodPatchIndex)) {
            continue;
          }
          ...
          if (itk::Math::AlmostEquals(minimumDistance,
            ...
            this->m_RicianBiasImage->SetPixel(neighborhoodPatchIndex,
                                              minimumDistance);
          }
        }
      }
      ...
      for (unsigned int m = 0; m < neighborhoodSearchSize; m++) {
        ...
        if (((meanRatio > this->m_MeanThreshold &&
          ...
          RealType count           = 0.0;
          for (unsigned int n = 0; n < neighborhoodPatchSize; n++) {
            IndexType searchNeighborhoodPatchIndex =
                neighborhoodIndex + neighborhoodPatchOffsetList[n];
            IndexType centerNeighborhoodPatchIndex =
                centerIndex + neighborhoodPatchOffsetList[n];
            ...
            RealType distance1 =
                inputImage->GetPixel(searchNeighborhoodPatchIndex) -
                this->m_MeanImage->GetPixel(searchNeighborhoodPatchIndex);
            RealType distance2 =
                inputImage->GetPixel(centerNeighborhoodPatchIndex) -
          ...
          if (weight > itk::NumericTraits<RealType>::ZeroValue()) {
            for (unsigned int n = 0; n < neighborhoodPatchSize; n++) {
              ...
              if (this->m_UseRicianNoiseModel) {
                weightedAverageIntensities[n] +=
                    weight * itk::Math::sqr(
                                 inputImage->GetPixel(neighborhoodPatchIndex));
              } else {
                weightedAverageIntensities[n] +=
                    weight * inputImage->GetPixel(neighborhoodPatchIndex);
              }
            }
            sumOfWeights += weight;
          }
      ...
      if (itk::Math::AlmostEquals(maxWeight,
                                  NumericTraits<RealType>::ZeroValue())) {
    ...
    for (unsigned int n = 0; n < neighborhoodPatchSize; n++) {
      ...
      if (this->m_UseRicianNoiseModel) {
        weightedAverageIntensities[n] +=
            maxWeight *
            itk::Math::sqr(inputImage->GetPixel(neighborhoodPatchIndex));
      } else {
        weightedAverageIntensities[n] +=
            maxWeight * inputImage->GetPixel(neighborhoodPatchIndex);
      }
    }
    ...
    if (sumOfWeights > itk::NumericTraits<RealType>::ZeroValue()) {
      for (unsigned int n = 0; n < neighborhoodPatchSize; n++) {
        ...
        typename OutputImageType::PixelType estimate =
            outputImage->GetPixel(neighborhoodPatchIndex);
        ...
        this->m_ThreadContributionCountImage->SetPixel(
            neighborhoodPatchIndex,
            this->m_ThreadContributionCountImage->GetPixel(
                neighborhoodPatchIndex) +
                1);
      }
    }
...
template <typename TInputImage, typename TOutputImage, typename TMaskImage>
void AdaptiveNonLocalMeansDenoisingImageFilter<
    TInputImage, TOutputImage, TMaskImage>::AfterThreadedGenerateData() {
  const MaskImageType *maskImage = this->GetMaskImage();

  if (this->m_UseRicianNoiseModel) {
    typedef DiscreteGaussianImageFilter<RealImageType, RealImageType>
                                   SmootherType;
    ...
    ImageRegionConstIterator<RealImageType> ItS(
        smoother->GetOutput(), smoother->GetOutput()->GetRequestedRegion());
    ImageRegionConstIteratorWithIndex<RealImageType> ItM(
        this->m_MeanImage, this->m_MeanImage->GetRequestedRegion());
    ImageRegionIterator<RealImageType> ItB(
        this->m_RicianBiasImage, this->m_RicianBiasImage->GetRequestedRegion());
    ItS.GoToBegin();
    ItM.GoToBegin();
    ItB.GoToBegin();
    ...
    while (!ItS.IsAtEnd()) {
      if (ItS.Get() > itk::NumericTraits<RealType>::ZeroValue() &&
          (!maskImage || maskImage->GetPixel(ItM.GetIndex()) !=
                             NumericTraits<MaskPixelType>::ZeroValue())) {
        const RealType snr = ItM.Get() / std::sqrt(ItS.Get());

        RealType bias = static_cast<RealType>(2.0) * ItS.Get() /
                        this->CalculateCorrectionFactor(snr);
  ...
  ImageRegionIteratorWithIndex<OutputImageType> ItO(
      this->GetOutput(), this->GetOutput()->GetRequestedRegion());
  ImageRegionConstIterator<RealImageType> ItL(
      this->m_ThreadContributionCountImage,
  ...
  for (ItO.GoToBegin(), ItL.GoToBegin(); !ItO.IsAtEnd(); ++ItO, ++ItL) {
    ...
    if (itk::Math::FloatAlmostEqual(
            ItL.Get(), itk::NumericTraits<RealType>::ZeroValue())) {
      continue;
    }
...
template <typename TInputImage, typename TOutputImage, typename TMaskImage>
typename AdaptiveNonLocalMeansDenoisingImageFilter<TInputImage, TOutputImage,
                                                   TMaskImage>::RealType
AdaptiveNonLocalMeansDenoisingImageFilter<
    TInputImage, TOutputImage, TMaskImage>::CalculateCorrectionFactor(RealType
                                                                          snr) {
  ...
  RealType value =
      ...
      static_cast<RealType>(0.125) * static_cast<RealType>(Math::pi) *
          static_cast<RealType>(
              std::exp(static_cast<RealType>(-0.5) * snrSquared)) *
          itk::Math::sqr(
              (static_cast<RealType>(2.0) + snrSquared) *
                  static_cast<RealType>(
                      this->m_ModifiedBesselCalculator.ModifiedBesselI0(
                          static_cast<RealType>(0.25) * snrSquared)) +
              snrSquared *
                  static_cast<RealType>(
                      this->m_ModifiedBesselCalculator.ModifiedBesselI1(
                          static_cast<RealType>(0.25) * snrSquared)));

  if (value < static_cast<RealType>(0.001) ||
      value > static_cast<RealType>(10.0)) {
...
template <typename TInputImage, typename TOutputImage, typename TMaskImage>
void AdaptiveNonLocalMeansDenoisingImageFilter<
    TInputImage, TOutputImage, TMaskImage>::PrintSelf(std::ostream &os,
                                                      Indent indent) const {
  ...
  os << indent << "Variance threshold = " << this->m_VarianceThreshold
     << std::endl;
  os << indent << "Smoothing variance = " << this->m_SmoothingVariance
     << std::endl;

  os << indent << "Neighborhood radius for local mean and variance = "
     << this->m_NeighborhoodRadiusForLocalMeanAndVariance << std::endl;
}
