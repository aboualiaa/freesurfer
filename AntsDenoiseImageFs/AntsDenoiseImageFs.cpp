...
int main(int argc, char **argv) {
  ...
  for (int frame = 0; frame < mri->nframes; frame++) {
    ...
    // configure denoiser
    typedef itk::AdaptiveNonLocalMeansDenoisingImageFilter<ITKImageType,
                                                           ITKImageType>
                                   DenoiserType;
    ...
    typename DenoiserType::NeighborhoodRadiusType
        neighborhoodRadiusForLocalMeanAndVariance;
    neighborhoodRadiusForLocalMeanAndVariance.Fill(1);
    denoiser->SetNeighborhoodRadiusForLocalMeanAndVariance(
        neighborhoodRadiusForLocalMeanAndVariance);
