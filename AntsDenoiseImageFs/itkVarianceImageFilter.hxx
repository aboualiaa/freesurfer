...
template <typename TInputImage, typename TOutputImage>
void VarianceImageFilter<TInputImage, TOutputImage>::ThreadedGenerateData(
  ...
  typename NeighborhoodAlgorithm::ImageBoundaryFacesCalculator<
  ...
  for (fit = faceList.begin(); fit != faceList.end(); ++fit) {
    ...
    while (!bit.IsAtEnd()) {
      ...
      for (i = 0; i < neighborhoodSize; ++i) {
        ...
        sumOfSquares +=
            itk::Math::sqr(static_cast<InputRealType>(bit.GetPixel(i)));
      }

      // get the variance value
      const double    num = static_cast<double>(neighborhoodSize);
      OutputPixelType var =
          (sumOfSquares - (itk::Math::sqr(sum) / num)) / (num - 1.0);
