...
NonLocalPatchBasedImageFilter<TInputImage,
                              TOutputImage>::NonLocalPatchBasedImageFilter() {
...
template <typename TInputImage, typename TOutputImage>
void NonLocalPatchBasedImageFilter<TInputImage,
  ...
  for (unsigned int n = 0; n < this->m_NeighborhoodSearchSize; n++) {
    this->m_NeighborhoodSearchOffsetList.push_back(
        searchNeighborhood.GetOffset(n));
  }
  ...
  for (unsigned int n = 0; n < this->m_NeighborhoodPatchSize; n++) {
    this->m_NeighborhoodPatchOffsetList.push_back(
        patchNeighborhood.GetOffset(n));
  }
...
template <typename TInputImage, typename TOutputImage>
typename NonLocalPatchBasedImageFilter<TInputImage,
                                       TOutputImage>::InputImagePixelVectorType
NonLocalPatchBasedImageFilter<TInputImage, TOutputImage>::
    VectorizeImageListPatch(const InputImageList &imageList,
                            const IndexType index, const bool normalize) {
  InputImagePixelVectorType patchVector(this->m_NeighborhoodPatchSize *
                                        imageList.size());
  for (unsigned int i = 0; i < imageList.size(); i++) {
    InputImagePixelVectorType patchVectorPerModality =
        this->VectorizeImagePatch(imageList[i], index, normalize);
    for (unsigned int j = 0; j < this->m_NeighborhoodPatchSize; j++) {
      patchVector[i * this->m_NeighborhoodPatchSize + j] =
          patchVectorPerModality[j];
    }
  }
  return patchVector;
}
...
typename NonLocalPatchBasedImageFilter<TInputImage,
                                       TOutputImage>::InputImagePixelVectorType
NonLocalPatchBasedImageFilter<TInputImage, TOutputImage>::VectorizeImagePatch(
    const InputImagePointer image, const IndexType index,
    const bool normalize) {
  InputImagePixelVectorType patchVector(this->m_NeighborhoodPatchSize);
  for (SizeValueType i = 0; i < this->m_NeighborhoodPatchSize; i++) {
    IndexType neighborhoodIndex =
        index + this->m_NeighborhoodPatchOffsetList[i];
  ...
  if (normalize) {
    ...
    this->GetMeanAndStandardDeviationOfVectorizedImagePatch(patchVector, mean,
                                                            standardDeviation);

    standardDeviation =
        std::max(standardDeviation, NumericTraits<RealType>::OneValue());
...
template <typename TInputImage, typename TOutputImage>
void NonLocalPatchBasedImageFilter<TInputImage, TOutputImage>::
    GetMeanAndStandardDeviationOfVectorizedImagePatch(
        const InputImagePixelVectorType &patchVector, RealType &mean,
  ...
  mean = sum / count;
  standardDeviation =
      std::sqrt((sumOfSquares - count * itk::Math::sqr(mean)) /
                (count - itk::NumericTraits<RealType>::OneValue()));
}
...
NonLocalPatchBasedImageFilter<TInputImage, TOutputImage>::
    ComputeNeighborhoodPatchSimilarity(
        const InputImageList &imageList, const IndexType index,
  ...
  for (SizeValueType i = 0; i < numberOfImagesToUse; i++) {
    for (SizeValueType j = 0; j < this->m_NeighborhoodPatchSize; j++) {
      IndexType neighborhoodIndex =
          index + this->m_NeighborhoodPatchOffsetList[j];

      bool isInBounds = this->m_TargetImageRegion.IsInside(neighborhoodIndex);
      if (isInBounds && std::isfinite(patchVectorY[count])) {
        RealType x =
            static_cast<RealType>(imageList[i]->GetPixel(neighborhoodIndex));
...
template <typename TInputImage, typename TOutputImage>
void NonLocalPatchBasedImageFilter<TInputImage, TOutputImage>::PrintSelf(
    std::ostream &os, Indent indent) const {
  Superclass::PrintSelf(os, indent);

  if (this->m_SimilarityMetric == PEARSON_CORRELATION) {
    os << "Using Pearson correlation to measure the patch similarity."
       << std::endl;
  } else if (this->m_SimilarityMetric == MEAN_SQUARES) {
  ...
  os << indent
     << "Neighborhood search radius = " << this->m_NeighborhoodSearchRadius
     << std::endl;
  os << indent
     << "Neighborhood patch radius = " << this->m_NeighborhoodPatchRadius
     << std::endl;
}
