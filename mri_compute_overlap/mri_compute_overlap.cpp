  ...
  if (total_flag) {
    ...
    if (log_fp) {
      ...
      fprintf(log_fp, "%d %d %2.3f\n", total_same, total,
              (float)total_same / (float)total);
      fclose(log_fp);
    }
    for (s = 0; s < nstrings; s++)
      printf("%s ", strings[s]);
    printf("%s %s %d %d %2.3f\n", argv[1], argv[2], total_same, total,
           (float)total_same / (float)total);
    exit(0);
  }
  if (all_flag) {
    ...
    for (lno = 0; lno < lnoLimit; lno++) {
      ...
      if (!quiet) {
        if (isSeg) {
          printf("label = %d (%s), volume diff = |(%d - %d)|"
                 " / %2.1f (%%)= %2.2f%%\n",
                 lno, cma_label_to_name(lno), nvox1, nvox2, nvox_mean,
                 100.0f * (float)abs(nvox1 - nvox2) / nvox_mean);
          printf("label = %d (%s), volume overlap (Dice) = "
                 "%d / %2.1f (%%)= %2.2f %%\n",
                 lno, cma_label_to_name(lno), nshared, nvox_mean,
                 100.0f * (float)nshared / nvox_mean);
          printf("label = %d (%s), volume overlap (Jaccard) = "
                 "%d / %2.1f (%%)= %2.2f%%\n",
                 lno, cma_label_to_name(lno), nshared, nunion, 100.0f * tmp);
        } else {
          printf("volume diff = |(%d - %d)| / %2.1f (%%)= %2.2f %%\n", nvox1,
                 nvox2, nvox_mean,
                 100.0f * (float)abs(nvox1 - nvox2) / nvox_mean);
          printf("volume overlap (Dice) = %d / %2.1f (%%)= %2.2f%%\n", nshared,
                 nvox_mean, 100.0f * (float)nshared / nvox_mean);
    ...
    for (i = 3; i < argc; i++) {
      ...
      if (!quiet) {
        ...
        printf(
            "label %d: volume overlap (Jaccard) = %d / %2.1f (%%)= %2.2f%%\n",
      ...
      if (log_fp) {
        ...
        // volume_overlap_jacc) ;
        fprintf(log_fp, "%d\t%2.2f\t%2.2f\t%2.2f\n", lno, volume_diff,
                volume_overlap, volume_overlap_jacc);
        // fclose(log_fp) ;
      }
      total_nvox1 += nvox1;
      total_nvox2 += nvox2;
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
static void usage_exit(int code) {
  printf("usage: %s [options] <volume 1> <volume 2> ... <volume N> [label "
         "numbers]\n",
         Progname);

  printf("\n");
  printf("Options:\n");
  printf("  -a\t\tcompute overlap of all labels (if missing, labels of "
  ...
  printf("  -mask <vol>\tlimit the domain of the calculation to the nonzero "
         "voxels in <vol>\n");

  printf("  -l <fname>\tfilename to write results to\n");
  printf("  -q\t\tdo not display results on std display (default is false; if "
         "-l is used, this option is set)\n");
...
static void print_help() {
  ...
  printf("\n");
  printf("Computes three different types of overlap measures either for all "
         "the existing labels\n");
  printf("in the input volumes or a subset of them that the users lists in the "
         "command line. The\n");
  printf("three overlap measures are the following: \n");
  printf("(1) `volume difference` = 2*|A|-|B|/|A|+|B|\n");
  printf("(2) `volume overlap (Dice)`    = 2*|AB|/|A|+|B|; the intersection "
         "over the mean of the segmentation volumes\n");
  printf("(3) `volume overlap (Jaccard)` = |AB|/|A+B|; the intersection over "
         "the union of the segmentation volumes\n");
  printf("\n");
  printf("\n");
  usage_exit(1);
}
