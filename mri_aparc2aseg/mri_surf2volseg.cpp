...
public:
  ...
  MRIS *GetClosestVertexNo(int c, int r, int s, int AllowLH, int AllowRH,
                           ...
                           int PialDotDir, int *vtxno, int *surftype, int *hemi,
                           double *dmin);
...
int Surf2VolSeg::RelabelSegVox(int c, int r, int s, int *hemi, int *surftype,
                               double *dmin) {
  ...
  if (LabelCortex && IS_CORTEX(volsegid)) {
    newsegid = Surf2VolSeg::RelabelSegVoxWithSurf(involseg, c, r, s, 1, hemi,
                                                  surftype, dmin);
    if (newsegid != 0) {
      if (*hemi == 1)
  ...
  if (LabelWM && (IS_WHITE_CLASS(volsegid) || IS_HYPO(volsegid))) {
    newsegid = Surf2VolSeg::RelabelSegVoxWithSurf(involseg, c, r, s, 2, hemi,
                                                  surftype, dmin);
  ...
  if (debug)
    printf("RelabelSegVox(): %d %d %d   %d %d   %d %d %g\n", c, r, s, volsegid,
           newsegid, *hemi, *surftype, *dmin);
...
int Surf2VolSeg::RelabelSegVoxWithRibbon(int c, int r, int s) {
  ...
  double dmin;
  MRIS * ClosestSurf = Surf2VolSeg::GetClosestVertexNo(
      c, r, s, DoLH, DoRH, 1, 1, 0, 0, &vtxno, &surftype, &hemi, &dmin);
  if (ClosestSurf == NULL)
    return (0);
...
int Surf2VolSeg::RelabelSegVoxWithSurf(MRI *volseg, int c, int r, int s,
                                       int LabelType, int *hemi, int *surftype,
                                       double *dmin) {
  *hemi     = 0;
  *surftype = 0;
  *dmin     = 1e9;
  ...
  int   vtxno;
  MRIS *ClosestSurf =
      GetClosestVertexNo(c, r, s, AllowLH, AllowRH, AllowWhite, AllowPial,
                         WhiteDotDir, PialDotDir, &vtxno, surftype, hemi, dmin);
  if (ClosestSurf == NULL) {
    if (debug)
      printf("  Closest Surf is NULL\n");
    return (0);
  }
...
MRIS *Surf2VolSeg::GetClosestVertexNo(int c, int r, int s, int AllowLH,
                                      ...
                                      int PialDotDir, int *vtxno, int *surftype,
                                      int *hemi, double *dmin) {
  *vtxno    = -1;
  *surftype = -1;
  *hemi     = -1;
  *dmin     = 1e10;
        ...
        if (dot * WhiteDotDir < 0) {
          if (debug)
            printf("lh white dot check failed dot = %g, vtxno %d \n", dot,
                   lhwvtx);
          dlhw = MRISfindMinDistanceVertexWithDotCheck(
              lhwhite, c, r, s, involseg, WhiteDotDir, &lhwvtx);
        }
      }
    }
    if (AllowPial) {
        ...
        if (dot * PialDotDir < 0) {
          if (debug)
            printf("lh pial dot check failed dot = %g, vtxno %d\n", dot,
                   lhpvtx);
          dlhp = MRISfindMinDistanceVertexWithDotCheck(
              lhpial, c, r, s, involseg, PialDotDir, &lhpvtx);
        }
      }
    }
  }
        ...
        if (dot * WhiteDotDir < 0) {
          if (debug)
            printf("rh white dot check failed dot = %g, vtxno %d, d=%g\n", dot,
                   rhwvtx, drhw);
          drhw = MRISfindMinDistanceVertexWithDotCheck(
              rhwhite, c, r, s, involseg, WhiteDotDir, &rhwvtx);
          if (debug) {
            dot = dotfunc(x, y, z, rhwhite, rhwvtx);
        ...
        if (dot * PialDotDir < 0) {
          if (debug)
            printf("rh pial dot check failed dot = %g, vtxno %d, d=%g\n", dot,
                   rhpvtx, drhp);
          drhp = MRISfindMinDistanceVertexWithDotCheck(
              rhpial, c, r, s, involseg, PialDotDir, &rhpvtx);
          if (debug) {
            dot = dotfunc(x, y, z, rhpial, rhpvtx);
            printf("  new dot = %g, vtxno %d, d=%g\n", dot, rhpvtx, drhp);
          }
