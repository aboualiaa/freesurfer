...
public:
  ...
  std::string involsegpath, ribbonpath, lhwhitepath, rhwhitepath, lhpialpath,
      rhpialpath;
  std::string lhcortexlabelpath, rhcortexlabelpath;
  std::string lhannotname = "lh.aparc.annot", lhannotpath,
              rhannotname = "rh.aparc.annot", rhannotpath;
  ...
  int   RelabelSegVox(int c, int r, int s, int *ndotcheck, int *hemi,
                      int *surftype, double *dmin);
  int   RelabelSegVoxWithRibbon(int c, int r, int s, int *ndotcheck);
  int   RelabelSegVoxWithSurf(MRI *volseg, int c, int r, int s, int LabelType,
                              int *ndotcheck, int *hemi, int *surftype,
                              double *dmin);
  int   CRS2SurfRAS(int c, int r, int s, double *x, double *y, double *z);
  MRIS *GetClosestVertexNo(int c, int r, int s, int AllowLH, int AllowRH,
                           int AllowWhite, int AllowPial, int WhiteDotDir,
                           int PialDotDir, int *ndotcheck, int *vtxno,
                           int *surftype, int *hemi, double *dmin);
...
int main(int argc, char **argv) {
  ...
  if (crsTest) {
    ...
    double dmin;
    s2vseg.RelabelSegVox(ctest, rtest, stest, &ndotcheck, &hemi, &surftype,
                         &dmin);
    printf("Done test\n");
    return (0);
  }
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help") || !strcasecmp(option, "-h") ||
        !strcasecmp(option, "--usage") || !strcasecmp(option, "-u"))
      print_help();
    ...
    else if (!strcasecmp(option, "--no-hypo-as-wm"))
      s2vseg.LabelHypoAsWM = 0;
...
static void check_options(void) {
  ...
  if (s2vseg.DoLH &&
      (s2vseg.lhwhitepath.length() == 0 || s2vseg.lhpialpath.length() == 0)) {
    printf("ERROR: you must include lh white and pial surfaces\n");
    exit(1);
  }

  if (s2vseg.DoRH &&
      (s2vseg.rhwhitepath.length() == 0 || s2vseg.rhpialpath.length() == 0)) {
  ...
  if (s2vseg.FixPresurf + s2vseg.LabelCortex + s2vseg.LabelWM == 0) {
    printf("ERROR: must select one of --fix-presurf-with-ribbon, "
           "--label-cortex, or --label-wm\n");
    exit(1);
  }
  if (s2vseg.FixPresurf + s2vseg.LabelCortex + s2vseg.LabelWM != 1) {
    printf("ERROR: can only do one of --fix-presurf-with-ribbon, "
           "--label-cortex, or --label-wm\n");
    exit(1);
  }
  ...
  if (s2vseg.FixPresurf) {
    if (s2vseg.lhannotpath.length() || s2vseg.rhannotpath.length()) {
      printf("ERROR: do not include annotations when fixing presurf with "
             "ribbon\n");
      exit(1);
    }
    if (s2vseg.RipUnknown) {
      printf(
          "ERROR: do not use --rip-unknown when fixing presurf with ribbon\n");
      exit(1);
    }
  }

  if (s2vseg.LabelCortex || s2vseg.LabelWM) {
    if ((s2vseg.DoLH && s2vseg.lhannotpath.length() == 0) ||
        (s2vseg.DoRH && s2vseg.rhannotpath.length() == 0)) {
      printf(
          "ERROR: you must include annotations when labeling cortex or WM\n");
      exit(1);
    }
  }
...
int Surf2VolSeg::RelabelSegVox(int c, int r, int s, int *ndotcheck, int *hemi,
                               int *surftype, double *dmin) {
  ...
  (*ndotcheck) = 0;
  if (debug)
    printf("RelabelSegVox(): %d %d %d   %d %d %d   %d\n", c, r, s, FixPresurf,
           LabelCortex, LabelWM, volsegid);

  if (FixPresurf) {
    newsegid = Surf2VolSeg::RelabelSegVoxWithRibbon(c, r, s, ndotcheck);
  }
  if (LabelCortex && IS_CORTEX(volsegid)) {
    newsegid = Surf2VolSeg::RelabelSegVoxWithSurf(
        involseg, c, r, s, 1, ndotcheck, hemi, surftype, dmin);
    if (newsegid != 0) {
      if (*hemi == 1)
        newsegid += lhbaseoffset;
      if (*hemi == 2)
        newsegid += rhbaseoffset;
    }
  }
  if (LabelWM && (IS_WHITE_CLASS(volsegid) || IS_HYPO(volsegid))) {
    newsegid = Surf2VolSeg::RelabelSegVoxWithSurf(
        involseg, c, r, s, 2, ndotcheck, hemi, surftype, dmin);
    if (*dmin > wmparc_dist_thresh)
      newsegid = 0;
    if (*hemi == 1) {
      if (newsegid > 0)
        newsegid += lhbaseoffset;
  ...
  if (debug)
    printf("RelabelSegVox(): %d %d %d   %d %d   %d %d %d %g\n", c, r, s,
           volsegid, newsegid, *ndotcheck, *hemi, *surftype, *dmin);
...
int Surf2VolSeg::RelabelSegVoxWithRibbon(int c, int r, int s, int *ndotcheck) {
  ...
  if (debug)
    printf("RelabelSegVoxWithRibbon(): %d %d %d   %d %d \n", c, r, s, volsegid,
           RibbonLabel);
  ...
  if (!IS_CORTEX(volsegid) && !IS_WHITE_CLASS(volsegid) &&
      !IS_CEREBELLAR_GM(volsegid) && !IS_HYPO(volsegid) && volsegid != Unknown)
    return (volsegid);

  // Both are unknown. This will handle most of the background voxels
  if (RibbonLabel == Unknown && volsegid == Unknown)
    return (volsegid);

  if (IS_CORTEX(volsegid)) {
    if (RibbonLabel == Unknown)
      return (RibbonLabel); //relabel
    if (IS_CORTEX(RibbonLabel))
      return (volsegid); // not relabeled
  ...
  if (IS_CEREBELLAR_GM(volsegid)) {
    ...
    if (IS_CORTEX(RibbonLabel))
      return (RibbonLabel); //relabel
    if (IS_WHITE_CLASS(RibbonLabel))
      return (RibbonLabel); //relabled, rare
  }

  if (IS_HYPO(volsegid)) {
    if (RibbonLabel == Unknown)
      return (RibbonLabel); //relabeled, rare
    if (IS_CORTEX(RibbonLabel))
      return (RibbonLabel); //relabel
    if (IS_WHITE_CLASS(RibbonLabel)) {
      if (LabelHypoAsWM == 0)
  ...
  double dmin;
  MRIS *ClosestSurf = Surf2VolSeg::GetClosestVertexNo(c, r, s, DoLH, DoRH, 1, 1,
                                                      0, 0, ndotcheck, &vtxno,
                                                      &surftype, &hemi, &dmin);
  if (ClosestSurf == NULL)
    return (0);
...
int Surf2VolSeg::RelabelSegVoxWithSurf(MRI *volseg, int c, int r, int s,
                                       int LabelType, int *ndotcheck, int *hemi,
                                       int *surftype, double *dmin) {
  double x = 0, y = 0, z = 0;
  ...
  *dmin     = 1e9;
  ...
  if (debug) {
    ...
    printf("RelabelSegVoxWithSurf(): LabelType %d volsegid %d, LabelHypoAsWM "
           "%d, crs %d %d %d, xyz %g %g %g\n",
  ...
  if (LabelType == 1) { // Label Cortex
    ...
    AllowWhite = 1;
    WhiteDotDir =
        +1; // expecting voxel to be outside of the local plane of white surf
    AllowPial = 1;
    PialDotDir =
        -1; // expecting voxel to be outside of the local plane of pial surf
    ...
    if (!IS_WHITE_CLASS(volsegid) && !(LabelHypoAsWM && IS_HYPO(volsegid)))
      return (volsegid);
    AllowWhite  = 1;
    WhiteDotDir = -1;
    ...
    if (volsegid == WM_hypointensities)
      CRS2SurfRAS(c, r, s, &x, &y, &z);
    if (DoLH && (volsegid == Left_Cerebral_White_Matter ||
                 volsegid == Left_WM_hypointensities ||
                 (volsegid == WM_hypointensities && x <= 0))) {
      AllowLH = 1;
      AllowRH = 0;
    }
    if (DoRH && (volsegid == Right_Cerebral_White_Matter ||
                 volsegid == Right_WM_hypointensities ||
                 (volsegid == WM_hypointensities && x > 0))) {
      AllowLH = 0;
      AllowRH = 1;
    }
  ...
  if (debug)
    printf("  Allow LH %d RH %d White %d Pial %d, WDD %d, PDD %d\n", AllowLH,
           AllowRH, AllowWhite, AllowPial, WhiteDotDir, PialDotDir);

  int   vtxno;
  MRIS *ClosestSurf = GetClosestVertexNo(
      c, r, s, AllowLH, AllowRH, AllowWhite, AllowPial, WhiteDotDir, PialDotDir,
      ndotcheck, &vtxno, surftype, hemi, dmin);
  if (ClosestSurf == NULL) {
    // In the case where no surface is found, make sure to set the hemi so that the wmparc can be set
    // appropriately
    if (volsegid == Left_Cerebral_Cortex ||
        volsegid == Left_Cerebral_White_Matter ||
        volsegid == Left_WM_hypointensities)
      *hemi = 1;
    else if (volsegid == Right_Cerebral_Cortex ||
             volsegid == Right_Cerebral_White_Matter ||
             volsegid == Right_WM_hypointensities)
      *hemi = 2;
    else {
    ...
    if (debug)
      printf("  Closest Surf is NULL, hemi set to %d\n", *hemi);
    return (0);
  }
  ...
  if (ClosestSurf->ct)
    CTABfindAnnotation(ClosestSurf->ct, annot, &annotid);
  else
  ...
  if (annotid == -1) {
    ...
    if (debug)
      printf("  nearest annot %d   annotid %d (nHopsMax %d)\n", annot, annotid,
             nHopsMax);
    if (annotid == -1)
      annotid = 0;
  }
...
// the dot check.
MRIS *Surf2VolSeg::GetClosestVertexNo(int c, int r, int s, int AllowLH,
                                      int AllowRH, int AllowWhite,
                                      int AllowPial, int WhiteDotDir,
                                      int PialDotDir, int *ndotcheck,
                                      int *vtxno, int *surftype, int *hemi,
                                      double *dmin) {
  *vtxno       = -1;
  *surftype    = -1;
  *hemi        = -1;
  ...
  if (AllowLH) {
    if (AllowWhite) {
      ...
      if (lhwvtx < 0)
        ...
        if (dot * WhiteDotDir < 0) {
          if (debug)
            printf("lh white dot check failed dot = %g, vtxno %d \n", dot,
                   lhwvtx);
          dlhw = MRISfindMinDistanceVertexWithDotCheckXYZ(
              lhwhite, x, y, z, involseg, WhiteDotDir, &lhwvtx);
          (*ndotcheck)++;
        }
      }
    }
    if (AllowPial) {
      ...
      if (lhpvtx < 0)
        ...
        if (dot * PialDotDir < 0) {
          if (debug)
            printf("lh pial dot check failed dot = %g, vtxno %d\n", dot,
                   lhpvtx);
          dlhp = MRISfindMinDistanceVertexWithDotCheckXYZ(
              lhpial, x, y, z, involseg, PialDotDir, &lhpvtx);
          (*ndotcheck)++;
        }
      }
    }
  ...
  if (AllowRH) {
    if (AllowWhite) {
      ...
      if (rhwvtx < 0)
        ...
        if (dot * WhiteDotDir < 0) {
          if (debug)
            printf("rh white dot check failed dot = %g, vtxno %d, d=%g\n", dot,
                   rhwvtx, drhw);
          drhw = MRISfindMinDistanceVertexWithDotCheckXYZ(
              rhwhite, x, y, z, involseg, WhiteDotDir, &rhwvtx);
          (*ndotcheck)++;
          if (debug) {
            dot = dotfunc(x, y, z, rhwhite, rhwvtx);
    ...
    if (AllowPial) {
      ...
      if (rhpvtx < 0)
        ...
        if (dot * PialDotDir < 0) {
          if (debug)
            printf("rh pial dot check failed dot = %g, vtxno %d, d=%g\n", dot,
                   rhpvtx, drhp);
          drhp = MRISfindMinDistanceVertexWithDotCheckXYZ(
              rhpial, x, y, z, involseg, PialDotDir, &rhpvtx);
          (*ndotcheck)++;
          if (debug) {
            dot = dotfunc(x, y, z, rhpial, rhpvtx);
            printf("  new dot = %g, vtxno %d, d=%g\n", dot, rhpvtx, drhp);
...
// nHops
int Surf2VolSeg::GetNearestVertexWithAnnot(int CenterVtx, MRI_SURFACE *Surf,
                                           int nHops) {
...
int Surf2VolSeg::CRS2SurfRAS(int c, int r, int s, double *x, double *y,
                             double *z) {
  MATRIX *CRS, *RAS;
...
void Surf2VolSeg::Free(void) {
  ...
  if (ribbon)
    MRIfree(&ribbon);
  if (lhwhite_hash)
    MHTfree(&lhwhite_hash);
  if (rhwhite_hash)
    MHTfree(&rhwhite_hash);
  if (lhpial_hash)
    MHTfree(&lhpial_hash);
  if (rhpial_hash)
    MHTfree(&rhpial_hash);
...
int Surf2VolSeg::LoadData(void) {
  ...
  if (outvolseg == NULL) {
    outvolseg =
        MRIalloc(involseg->width, involseg->height, involseg->depth, MRI_INT);
    MRIcopyHeader(involseg, outvolseg);
    outvolseg->ct =
        CTABreadDefault(); // not sure this will be right always (coult cmd spec)
  }
  if (involseg == NULL)
    exit(1);
  ...
  if (DoLH) {
    ...
    if (lhpial == NULL)
      exit(1);
    lhwhite_hash =
        MHTcreateVertexTable_Resolution(lhwhite, CURRENT_VERTICES, hashres);
    lhpial_hash =
        MHTcreateVertexTable_Resolution(lhpial, CURRENT_VERTICES, hashres);
  ...
  if (DoRH) {
    ...
    if (rhpial == NULL)
      exit(1);
    rhwhite_hash =
        MHTcreateVertexTable_Resolution(rhwhite, CURRENT_VERTICES, hashres);
    rhpial_hash =
        MHTcreateVertexTable_Resolution(rhpial, CURRENT_VERTICES, hashres);
