...
int main(int argc, char **argv) {
  ...
  if (DoLH) {
    ...
    /* ------ Load lh annotation ------ */
    sprintf(annotfile, "%s/%s/label/lh.%s.annot", SUBJECTS_DIR, subject,
            annotname);
  ...
  if (DoRH) {
    ...
    /* ------ Load rh annotation ------ */
    sprintf(annotfile, "%s/%s/label/rh.%s.annot", SUBJECTS_DIR, subject,
            annotname);
  ...
  if (crsTest) {
    ...
    err = FindClosestLRWPVertexNo(ctest, rtest, stest, &lhwvtx, &lhpvtx,
                                  &rhwvtx, &rhpvtx, Vox2RAS, lhwhite, lhpial,
                                  rhwhite, rhpial, lhwhite_hash, lhpial_hash,
  ...
  for (c = 0; c < ASeg->width; c++) {
    ...
    for (r = 0; r < ASeg->height; r++) {
      for (s = 0; s < ASeg->depth; s++) {
        ...
        if (LHOnly && (asegid == Right_Cerebral_Cortex ||
                       asegid == Right_Cerebral_White_Matter))
          continue;
        if (RHOnly && (asegid == Left_Cerebral_Cortex ||
        ...
        if (asegid == Left_Cerebellum_Cortex ||
            asegid == Right_Cerebellum_Cortex || asegid == 172)
          IsCblumCtx = 1;
        else
          IsCblumCtx = 0;
        ...
        if (UseNewRibbon) {
          if (IsCortex || IsWM || (asegid == Unknown || asegid == CSF) ||
              IsCblumCtx) {
            RibbonVal = MRIgetVoxVal(RibbonSeg, c, r, s, 0);
            if (!IsCblumCtx && asegid != CSF)
        ...
        if (UseHash) {
          if (DoLH) {
            lhwvtx = MHTfindClosestVertexNoXYZ(lhwhite_hash, lhwhite, vtx.x,
                                               vtx.y, vtx.z, &dlhw);
            lhpvtx = MHTfindClosestVertexNoXYZ(lhpial_hash, lhpial, vtx.x,
                                               vtx.y, vtx.z, &dlhp);
          } else {
            lhwvtx = -1;
            lhpvtx = -1;
          ...
          if (DoRH) {
            rhwvtx = MHTfindClosestVertexNoXYZ(rhwhite_hash, rhwhite, vtx.x,
                                               vtx.y, vtx.z, &drhw);
            rhpvtx = MHTfindClosestVertexNoXYZ(rhpial_hash, rhpial, vtx.x,
                                               vtx.y, vtx.z, &drhp);
          } else {
            rhwvtx = -1;
            rhpvtx = -1;
          ...
          if (lhwvtx < 0 && lhpvtx < 0 && rhwvtx < 0 && rhpvtx < 0) {
            ...
            */
            if (DoLH) {
              lhwvtx = MRISfindClosestVertex(lhwhite, vtx.x, vtx.y, vtx.z,
                                             &dlhw, CURRENT_VERTICES);
              lhpvtx = MRISfindClosestVertex(lhpial, vtx.x, vtx.y, vtx.z, &dlhp,
                                             CURRENT_VERTICES);
            }
            if (DoRH) {
              rhwvtx = MRISfindClosestVertex(rhwhite, vtx.x, vtx.y, vtx.z,
                                             &drhw, CURRENT_VERTICES);
        ...
        */
        ...
        if (lhpvtx < 0)
          ...
          if (dot > 0) // pial surface normal should point in same direction as
                       // vector from voxel to vertex
          {
            if (MRIneighbors(ASeg, c, r, s, Left_Cerebral_Cortex) >
                0) // only do expensive check if it is possible
              dlhp = mrisFindMinDistanceVertexWithDotCheck(lhpial, c, r, s,
                                                           AParc, -1, &lhpvtx);
            else
              dlhp = 1000000000000000.0;
          }
        ...
        if (rhwvtx < 0)
          ...
          if (dot < 0) {
            if (MRIneighbors(ASeg, c, r, s, Right_Cerebral_Cortex) >
                0) // only do expensive check if it is possible
              drhw = mrisFindMinDistanceVertexWithDotCheck(rhwhite, c, r, s,
                                                           AParc, 1, &rhwvtx);
            else
              drhw = 1000000000000000.0;
          }
        ...
        if (rhpvtx < 0)
          ...
          if (dot > 0) {
            if (MRIneighbors(ASeg, c, r, s, Right_Cerebral_Cortex) >
                0) // only do expensive check if it is possible
              drhp = mrisFindMinDistanceVertexWithDotCheck(rhpial, c, r, s,
                                                           AParc, -1, &rhpvtx);
            else
              drhp = 1000000000000000.0;
          }
        ...
        if (annotid == 0 && !LabelWM) {
          if (asegid == Left_Cerebral_Cortex)
            MRIsetVoxVal(ASeg, c, r, s, 0, 1000);
          else if (asegid == Right_Cerebral_Cortex)
            MRIsetVoxVal(ASeg, c, r, s, 0, 2000);
          else
            MRIsetVoxVal(ASeg, c, r, s, 0, asegid);
          continue;
          //{if(hemi == 1)
          // MRIsetVoxVal(ASeg,c,r,s,0,Left_Cerebral_White_Matter); if(hemi ==
          // 2)
          // MRIsetVoxVal(ASeg,c,r,s,0,Right_Cerebral_White_Matter); continue;
        }
        ...
        if (debug || annotid == -1) {
          ...
          printf("crs = %d %d %d, ras = %6.4f %6.4f %6.4f \n", c, r, s, vtx.x,
                 vtx.y, vtx.z);
          if (lhwvtx > 0)
            printf("lhw  %d  %7.5f     %6.4f  %6.4f  %6.4f\n", lhwvtx, dlhw,
                   lhwhite->vertices[lhwvtx].x, lhwhite->vertices[lhwvtx].y,
                   lhwhite->vertices[lhwvtx].z);
          if (lhpvtx > 0)
            printf("lhp  %d  %7.5f     %6.4f  %6.4f  %6.4f\n", lhpvtx, dlhp,
                   lhpial->vertices[lhpvtx].x, lhpial->vertices[lhpvtx].y,
                   lhpial->vertices[lhpvtx].z);
          if (rhwvtx > 0)
            printf("rhw  %d  %7.5f     %6.4f  %6.4f  %6.4f\n", rhwvtx, drhw,
                   rhwhite->vertices[rhwvtx].x, rhwhite->vertices[rhwvtx].y,
                   rhwhite->vertices[rhwvtx].z);
          if (rhpvtx > 0)
            printf("rhp  %d  %7.5f     %6.4f  %6.4f  %6.4f\n", rhpvtx, drhp,
                   rhpial->vertices[rhpvtx].x, rhpial->vertices[rhpvtx].y,
          ...
          for (y = 0; y < mri_norm->height; y++)
            for (z = 0; z < mri_norm->depth; z++) {
              ...
              if ((MRIlabelsInNbhd(mri_tmp, x, y, z, 1,
                  ...
                  (MRIlabelsInNbhd(mri_tmp, x, y, z, 1,
...
#define REGION_WSIZE 3
                if (x == Gx && y == Gy && z == Gz)
                  DiagBreak();
                GCAsourceVoxelToNode(gca, mri_norm, xform, x, y, z, &xn, &yn,
                                     &zn);
                gcan = GCAbuildRegionalGCAN(gca, xn, yn, zn, REGION_WSIZE);
                GCAsourceVoxelToPrior(gca, mri_norm, xform, x, y, z, &xp, &yp,
                                      &zp);
                gcap = GCAbuildRegionalGCAP(
                    gca, xp, yp, zp,
                    REGION_WSIZE * gca->node_spacing / gca->prior_spacing - 1);
                ...
                if (max_label < 0) {
                  ...
                  for (n = 0; n < gcan->nlabels; n++) {
                    ...
                    if (IS_CORTEX(label) || label == 0 ||
                      ...
                      continue; // prohibited interior to white
                    mah_dist =
                        GCAmahDist(&gcan->gcs[n], vals, mri_norm->nframes);
                    prior = getPrior(gcap, label);
                    if (mah_dist + log(prior) < min_mah_dist) {
                      min_mah_dist = mah_dist + log(prior);
                      max_label    = gcan->labels[n];
                ...
                if (x == Gx && y == Gy && z == Gz)
                  printf("reclassifying unknown voxel (%d, %d, %d) that "
                         "neighbors ventricle %s --> %s (%d)\n",
                         x, y, z,
...
#define REGION_WSIZE 3
                ...
                if (IS_VENTRICLE(max_label)) {
                      ...
                      for (zk = -1; zk <= 1; zk++) {
                        ...
                        if (IS_VENTRICLE(olabel)) {
                          if (olabel == Right_Lateral_Ventricle)
                            vlabels = MRIlabelsInNbhd(mri_tmp, xi, yi, zi, 1,
                                                      Right_Lateral_Ventricle);
                          else
                            vlabels = MRIlabelsInNbhd(mri_tmp, xi, yi, zi, 1,
                  ...
                  if (is_vent) {
                    if (x == Gx && y == Gy && z == Gz)
                      printf(
                          "reclassifying unknown voxel (%d, %d, %d) that "
                          "neighbors ventricle %s --> %s (%d)\n",
                          x, y, z,
                          cma_label_to_name(MRIgetVoxVal(mri_tmp, x, y, z, 0)),
                          cma_label_to_name(max_label), max_label);
        ...
        for (y = 0; y < mri_norm->height; y++)
          for (z = 0; z < mri_norm->depth; z++) {
            ...
            if (MRIlabelsInNbhd(ASeg, x, y, z, 1, label_new) <= 1) {
              if (x == Gx && y == Gy && z == Gz)
                printf("voxel(%d, %d, %d): reverting label back to %s (%d), "
                       "was %s (%d)\n",
                       x, y, z, cma_label_to_name(label_orig), label_orig,
                       cma_label_to_name(label_new), label_new);
              MRIsetVoxVal(ASeg, x, y, z, 0, label_orig);
            }
          }

      MRIfree(&mri_tmp);
      MRIfree(&mri_aseg_orig);
    }
  ...
  if (!ASeg->ct)
    ASeg->ct = CTABreadDefault();
  if (!AParc->ct)
    AParc->ct = CTABreadDefault();
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help") || !strcasecmp(option, "-h") ||
      ...
      printf("relabeling unlikely voxels interior to white matter "
             "surface:\n\tnorm: %s\n\t XFORM: %s\n\tGCA: %s\n\tlabel "
             "intensities: %s\n",
             relabel_norm_name, relabel_xform_name, relabel_gca_name,
             relabel_label_intensities_name);
      nargsused = 4;
...
int FindClosestLRWPVertexNo(int c, int r, int s, int *lhwvtx, int *lhpvtx,
  ...
  *lhwvtx = MHTfindClosestVertexNoXYZ(lhwhite_hash, lhwhite, vtx.x, vtx.y,
                                      vtx.z, &dlhw);
  *lhpvtx = MHTfindClosestVertexNoXYZ(lhpial_hash, lhpial, vtx.x, vtx.y, vtx.z,
                                      &dlhp);
  *rhwvtx = MHTfindClosestVertexNoXYZ(rhwhite_hash, rhwhite, vtx.x, vtx.y,
                                      vtx.z, &drhw);
  *rhpvtx = MHTfindClosestVertexNoXYZ(rhpial_hash, rhpial, vtx.x, vtx.y, vtx.z,
                                      &drhp);
