...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_compute_change_map.c,v 1.7 2015/02/05 23:34:40 zkaufman Exp $",
      "$Name:  $");
  ...
  if (smooth_sigma > 0) {
    ...
    mri_smooth = MRIconvolveGaussian(mri1, nullptr, mri_kernel);
    MRIbinarize(mri1, mri1, 1, 0, 1);
    for (i = 0; i < erodes; i++) {
      mri_mask = MRIerodeZero(mri1, nullptr);
    ...
    mri_smooth = MRIconvolveGaussian(mri2, nullptr, mri_kernel);
    MRIbinarize(mri2, mri2, 1, 0, 1);
    for (i = 0; i < erodes; i++) {
      mri_mask = MRIerodeZero(mri2, nullptr);
  ...
  mri_change = MRIcomputeChangeMap(mri1, mri2, transform, nullptr, &std);
  ...
  if (nbhd_size > 0) {
    MRI *mri_tmp;
    mri_tmp = MRIcomputeNbhdPvalues(mri_change, nbhd_size, nullptr);
    MRIfree(&mri_change);
    mri_change = mri_tmp;
  }
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "mask")) {
    ...
    case 'E':
      erode = atoi(argv[2]);
      nargs = 1;
      printf("eroding input volumes %d times to reduce effects of skull-strip "
             "differences\n",
             erode);
      break;
    case 'M':
  ...
  for (x2 = 0; x2 < mri2->width; x2++)
    for (y2 = 0; y2 < mri2->height; y2++)
      for (z2 = 0; z2 < mri2->depth; z2++) {
        TransformSampleInverse(transform, (float)x2, (float)y2, (float)z2, &xf,
                               &yf, &zf);
...
MRI *MRIcomputeChangeMap(MRI *mri1, MRI *mri2, TRANSFORM *transform,
  ...
  for (x1 = 0; x1 < mri1->width; x1++)
    for (y1 = 0; y1 < mri1->height; y1++)
      for (z1 = 0; z1 < mri1->depth; z1++) {
        if (x1 == Gx && y1 == Gy && z1 == Gz)
          DiagBreak();
        TransformSampleInverse(transform, (float)x1, (float)y1, (float)z1, &x2,
                               &y2, &z2);
  ...
  HISTOfillHoles(h);
  hs = HISTOsmooth(h, nullptr, 20);
  HISTOrobustGaussianFit(hs, 0.5, &mean, &sigma);
  hs = HISTOsmooth(h, nullptr, 10);
  HISTOrobustGaussianFit(hs, 0.5, &mean, &sigma);
  ...
  hg = HISTOgaussianCDF(nullptr, mean, sigma, 10000);
  for (x1 = 0; x1 < mri1->width; x1++)
    for (y1 = 0; y1 < mri1->height; y1++)
      for (z1 = 0; z1 < mri1->depth; z1++) {
        if (x1 == Gx && y1 == Gy && z1 == Gz)
