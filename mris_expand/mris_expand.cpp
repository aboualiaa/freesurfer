...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mris_expand.c,v 1.16 2017/02/16 19:45:00 fischl Exp $",
      "$Name:  $");
  ...
  if (Gdiag & DIAG_WRITE) {
    ...
    if (parms.fp)
      ...
      ErrorExit(ERROR_BADPARM, "%s: could not open log file %s", Progname,
                log_fname);
  }
  if (use_thickness)
  ...
  seconds = seconds % 60;
  fprintf(stdout,
          "surface expansion took %d minutes"
          " and %d seconds.\n",
          minutes, seconds);
