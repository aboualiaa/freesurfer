 ...
 * either to an entire volume or to only a label (specified with -l <label>)
 * within it.
 */
/*
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_morphology.c,v 1.15 2012/03/15 18:31:07 lzollei Exp $",
      "$Name:  $");
  ...
  if (!stricmp(argv[2], "dilate"))
    ...
    ErrorExit(ERROR_UNSUPPORTED,
              "morphological operation '%s'  is not supported", argv[2]);
  }
  ...
  case MODE_FILTER: {
    ...
    mri_dst = MRImodeFilter(mri_src, nullptr, niter);
    break;
  }
  case FILL_HOLES: {
    int x, y, z, nfilled = 0;
    mri_dst = MRIbinarize(mri_src, nullptr, 1, 0, 1);
  ...
  case ERODE_BOTTOM:
    if (label < 0)
      ErrorExit(ERROR_BADPARM, "%s: must specify label with -l <label>",
                Progname);
    mri_dst = MRIerodeBottom(mri_src, label, nullptr);
    break;
  case DILATE:
    mri_dst = nullptr;
    for (i = 0; i < niter; i++) {
  ...
  case ERODE_THRESH: {
    ...
    if (argc < 7)
      ErrorExit(ERROR_BADPARM,
                "%s: for erode_thresh must specify <thresh> <intensity vol> on "
                "cmdline",
                Progname);
...
static void usage_exit(int code) {
  ...
  printf("\twhere <operation> can be "
         "[open,close,dilate,erode,mode,fill_holes,erode_bottom,dilate_thresh,"
         "erode_thresh]\n");
  printf("\tvalid options are:\n");
  printf("\t-l <label>  only apply operations to <label> instead of all "
         "nonzero voxels\n");
  exit(code);
}
