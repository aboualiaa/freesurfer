...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mrisp_paint.c,v 1.12 2016/03/22 14:47:57 fischl Exp $",
      "$Name:  $");
  ...
  if (variance) {
    ...
    } else /* compute curvature of surface */
    {
      ...
      MRISresetNeighborhoodSize(mris_var,
...
static int get_option(int argc, char *argv[]) {
  ...
  } else if (!stricmp(option, "NFRAMES")) // not implemented yet
  {
    ...
    fprintf(stderr,
            "generating variance map for with field %d (%s/surf/%s.%s) \n",
...
static void print_usage() {
  outputHelpXml(mrisp_paint_help_xml, mrisp_paint_help_xml_len);
}
