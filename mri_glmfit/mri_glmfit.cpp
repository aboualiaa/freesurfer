...
int main(int argc, char **argv) {
  ...
  if (!UseStatTable) {
    ...
    if (DoFisher) {
      ...
      mritmp = MRIfisherTransform(mriglm->y, nullptr, nullptr);
      if (mritmp == nullptr)
        exit(1);
      MRIfree(&mriglm->y);
      mriglm->y = mritmp;
  ...
  if (SubSample) {
    ...
    mritmp = fMRIsubSample(mriglm->y, SubSampStart, SubSampDelta, -1, nullptr);
    if (mritmp == nullptr)
      exit(1);
    MRIfree(&mriglm->y);
    mriglm->y = mritmp;
  ...
  if (DoDistance) {
    mritmp = fMRIdistance(mriglm->y, nullptr);
    MRIfree(&mriglm->y);
    mriglm->y = mritmp;
  }
  ...
  // X ---------------------------------------------------------
  // Load global X------------------------------------------------
  if ((XFile != nullptr) | usedti) {
    if (usedti == 0 || usedti == 3) {
      mriglm->Xg = MatrixReadTxt(XFile, nullptr);
      if (mriglm->Xg == nullptr)
      ...
      mriglm->Xg = MatrixCopy(dti->B, nullptr);
    }
  }
  ...
  if (fsgd != nullptr) {
    ...
    mriglm->Xg = gdfMatrix(fsgd, gd2mtx_method, nullptr);
    if (mriglm->Xg == nullptr)
      exit(1);
  }
  if (OneSampleGroupMean) {
    mriglm->Xg = MatrixConstVal(1.0, mriglm->y->nframes, 1, nullptr);
  }
  ...
  if (NSplits > 0) {
    ...
    printf("nframes=%d, NSplits=%d, SplitNo=%d, nPerSplit=%d, SplitMin=%d, "
           "SplitMax=%d, nEx=%d, RandSplit=%d\n",
           mriglm->y->nframes, NSplits, SplitNo, nPerSplit, SplitMin, SplitMax,
           nExclude, RandSplit);
    ...
    if (RandSplit) {
      ...
      // this is not important because it keeps the right order with the design
      // matrix
  ...
  if (ExcludeFrames) {
    ...
    mriglm->y = mritmp;
    if (mriglm->w) {
      mritmp = fMRIexcludeFrames(mriglm->w, ExcludeFrames, nExclude, nullptr);
      MRIfree(&mriglm->w);
      mriglm->w = mritmp;
    }
  }
  ...
  fflush(stdout);
  Xnorm = MatrixNormalizeCol(mriglm->Xg, nullptr, nullptr);
  ...
  if (labelFile != nullptr) {
    clabel = LabelRead(nullptr, labelFile);
    if (clabel == nullptr) {
      printf("ERROR reading %s\n", labelFile);
      exit(1);
    }
    printf("Found %d points in label.\n", clabel->n_points);
    mriglm->mask = MRISlabel2Mask(surf, clabel, nullptr);
    MRIcopyHeader(mriglm->y, mriglm->mask);
    MRIcopyPulseParameters(mriglm->y, mriglm->mask);
    mritmp = mri_reshape(mriglm->mask, mriglm->y->width, mriglm->y->height,
                         mriglm->y->depth, 1);
    MRIfree(&mriglm->mask);
    mriglm->mask = mritmp;
  }
  ...
  if (mriglm->mask) {
    ...
    if (nmask == 0) {
      ...
      if (prunemask)
        printf("  make sure at least one voxel has a non-zero value for each "
               "input\n");
      exit(1);
    }
    if (!DontSave) {
  ...
  if (RmSpatialMean) {
    ...
    mritmp = MRIremoveSpatialMean(mriglm->y, mriglm->mask, nullptr);
    MRIfree(&mriglm->y);
    mriglm->y = mritmp;
  }
  ...
  if (surf != nullptr) {
    ...
    if (mriglm->mask && mriglm->mask->height != surf->nvertices) {
      ...
      mritmp = mri_reshape(mriglm->mask, surf->nvertices, 1, 1,
                           mriglm->mask->nframes);
      if (mritmp == nullptr)
        exit(1);
    }
    for (n = 0; n < surf->nvertices; n++) {
      if (mritmp && MRIgetVoxVal(mritmp, n, 0, 0, 0) < 0.5)
        continue;
  ...
  if (wFile != nullptr) {
    ...
    mritmp = MRInormWeights(mriglm->w, weightsqrt, weightinv, mriglm->mask,
                            mriglm->w);
    if (mritmp == nullptr)
      exit(1);
    if (weightsqrt || weightinv) {
      sprintf(tmpstr, "%s/wn.%s", GLMDir, format);
      if (!DontSave && !DontSaveWn)
        MRIwrite(mriglm->w, tmpstr);
    }
  } else if (wgFile != nullptr) {
    // Global weight to use at every voxel.
    mriglm->wg = MatrixReadTxt(wgFile, nullptr);
  ...
  if (nSelfReg > 0) {
    ...
    for (n = 0; n < nSelfReg; n++) {
      ...
      if (c < 0 || c >= mriglm->y->width || r < 0 || r >= mriglm->y->height ||
      ...
      Mtmp = MatrixHorCat(Xselfreg, mriglm->glm->eres, nullptr);
      if (n > 0)
        MatrixFree(&Xselfreg);
      Xselfreg = Mtmp;
    }
  ...
  if (nContrasts > 0) {
    for (n = 0; n < nContrasts; n++) {
      if (!useasl && !useqa && !(fsgd != nullptr && fsgd->nContrasts != 0) &&
        ...
        // Read it in
        mriglm->glm->C[n] = MatrixReadTxt(CFile[n], nullptr);
        if (mriglm->glm->C[n] == nullptr) {
          printf("ERROR: loading C %s\n", CFile[n]);
          exit(1);
        }
        if (Gamma0File[n]) {
          printf("Loading gamma0 file %s\n", Gamma0File[n]);
          mriglm->glm->gamma0[n] = MatrixReadTxt(Gamma0File[n], nullptr);
          if (mriglm->glm->gamma0[n] == nullptr) {
  ...
  // Dump a voxel
  if (voxdumpflag) {
    sprintf(voxdumpdir, "%s/voxdump-%d-%d-%d", GLMDir, voxdump[0], voxdump[1],
            voxdump[2]);
    ...
    if (mriglm->w) {
      wvect =
          MRItoVector(mriglm->w, voxdump[0], voxdump[1], voxdump[2], nullptr);
      ...
      if (!strcmp(csd->simtype, "mc-full") || !strcmp(csd->simtype, "perm")) {
        ...
        if (VarFWHM > 0) {
          ...
          if (!DoSim)
            printf("Starting fit and test\n");
          MRIglmFitAndTest(mriglm);
          // If using permutation with non-stationary correction, compute
          // fwhmmap here
        ...
        for (nthSign = 0; nthSign < nSignList; nthSign++) {
          ...
          for (n = 0; n < mriglm->glm->ncontrasts; n++) {
            ...
            if (!strcmp(csd->simtype, "mc-full") ||
              ...
              if (nthThresh == 0 && nthSign == 0) {
                ...
                if (SmoothLevel > 0) {
                  ...
                  if (DiagCluster) {
                    sprintf(tmpstr, "./%s-zsm0.%s", mriglm->glm->Cname[n],
                            format);
            ...
            if (DoSimThreshLoop && (nThreshList > 1 || nSignList > 1)) {
              if (round(csd->threshsign) == 0)
                tmpstr2 = "abs";
              if (round(csd->threshsign) == +1)
                tmpstr2 = "pos";
              if (round(csd->threshsign) == -1)
                tmpstr2 = "neg";
              ...
              sprintf(tmpstr, "%s.th%02d.%s.j001-%s.csd", simbase,
                      (int)round(csd->thresh * 10), tmpstr2,
                      mriglm->glm->Cname[n]);
            } else
  ...
  if (ComputeFWHM) {
    ...
    if (surf != nullptr) {
      ...
      ar1 = fMRIspatialAR1(mriglm->eres, mriglm->mask, nullptr);
      if (ar1 == nullptr)
        exit(1);
      sprintf(tmpstr, "%s/sar1.%s", GLMDir, format);
      MRIwrite(ar1, tmpstr);
  ...
  if (DoTemporalAR1) {
    ...
    tar1 = fMRItemporalAR1(mriglm->eres, mriglm->beta->nframes, mriglm->mask,
                           nullptr);
  ...
  rstd = MRIsqrt(mriglm->rvar, nullptr);
  sprintf(tmpstr, "%s/rstd.%s", GLMDir, format);
  MRIwrite(rstd, tmpstr);

  if (mriglm->yhatsave)
    MRIwrite(mriglm->yhat, yhatFile);
  if (mriglm->condsave)
    MRIwrite(mriglm->cond, condFile);
  if (eresFile)
    MRIwrite(mriglm->eres, eresFile);
  if (eresSCMFile) {
  ...
  if (useqa) {
    ...
    fsnr = MRIdivide(mriglm->gamma[0], rstd, nullptr);
    sprintf(tmpstr, "%s/fsnr.%s", GLMDir, format);
    MRIwrite(fsnr, tmpstr);
    MRIsegStats(mriglm->mask, 1, fsnr, 0, &fsnrmin, &fsnrmax, &fsnrrange,
                &fsnrmean, &fsnrstd);
  ...
  for (n = 0; n < mriglm->glm->ncontrasts; n++) {
    ...
    if (mriglm->glm->C[n]->rows == 1) {
      // Save gammavar
      sprintf(tmpstr, "%s/%s/gammavar.%s", GLMDir, mriglm->glm->Cname[n],
              format);
    ...
    if (UseStatTable) {
      for (m = 0; m < OutStatTable->nrows; m++) {
        ...
        if (mriglm->glm->C[n]->rows == 1)
          OutStatTable->data[m][n] *=
              SIGN(MRIgetVoxVal(mriglm->gamma[n], m, 0, 0, 0));
    ...
    if (mriglm->glm->C[n]->rows == 1) {
      cnr = MRIdivide(mriglm->gamma[n], rstd, nullptr);
      sprintf(tmpstr, "%s/%s/cnr.%s", GLMDir, mriglm->glm->Cname[n], format);
      MRIwrite(cnr, tmpstr);
      MRIfree(&cnr);
      if (mriglm->glm->Dt[n] != nullptr) {
        // Write out the pcc
  ...
  if (usedti) {
    ...
    lowb = DTIbeta2LowB(mriglm->beta, mriglm->mask, nullptr);
    sprintf(tmpstr, "%s/lowb.%s", GLMDir, format);
    MRIwrite(lowb, tmpstr);

    tensor = DTIbeta2Tensor(mriglm->beta, mriglm->mask, nullptr);
    ...
    if (mriglm->glm->dof > 0) {
      ...
      dwirvar =
          fMRIcovariance(dwires, 0, mriglm->beta->nframes, nullptr, nullptr);
  ...
  if (DoMRTM1) {
    ...
    mritmp = BindingPotential(mriglm->gamma[1], mriglm->gamma[2], mriglm->mask,
                              nullptr);
    ...
    MRI *k2prime = MRIdivide(mriglm->gamma[1], mriglm->gamma[0], nullptr);
  ...
  if (DoMRTM2) {
    printf("Computing binding potentials\n");
    mritmp = BindingPotential(mriglm->gamma[0], mriglm->gamma[1], mriglm->mask,
                              nullptr);
    sprintf(tmpstr, "%s/bp.%s", GLMDir, format);
    err = MRIwrite(mritmp, tmpstr);
    if (err)
  ...
  if (false) {
    ...
    I = ImageFromMatrix(mriglm->Xg, nullptr);
  ...
  if (surf != nullptr)
    fprintf(fp, "anattype surface\n");
  else
    fprintf(fp, "anattype volume\n");
  fclose(fp);
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      yhatSave = 1;
    else if (!strcasecmp(option, "--yhat-save"))
      yhatSave = 1;
    else if (!strcasecmp(option, "--save-eres"))
      eresSave = 1;
    else if (!strcasecmp(option, "--eres-save"))
      eresSave = 1;
    else if (!strcasecmp(option, "--save-fwhm-map")) {
      SaveFWHMMap = 1;
      ComputeFWHM = 1;
    } else if (!strcasecmp(option, "--eres-scm"))
      eresSCMSave = 1;
    else if (!strcasecmp(option, "--save-cond"))
      condSave = 1;
    else if (!strcasecmp(option, "--dontsave"))
      DontSave = 1;
    else if (!strcasecmp(option, "--dontsavewn"))
      DontSaveWn = 1;
      ...
      prunemask = 0;
    else if (!strcasecmp(option, "--w-inv"))
      weightinv = 1;
    else if (!strcasecmp(option, "--w-sqrt"))
      weightsqrt = 1;
    else if (!strcasecmp(option, "--perm-1"))
      OneSamplePerm = 1;
      ...
      PermForce = 1;
    else if (!strcasecmp(option, "--perm-nonstatcor"))
      PermNonStatCor = 1;
    else if (!strcasecmp(option, "--logy"))
      logflag = 1;
    else if (!strcasecmp(option, "--no-logy"))
      logflag = 0;
    else if (!strcasecmp(option, "--kurtosis"))
      DoKurtosis = 1;
    else if (!strcasecmp(option, "--skew"))
      DoSkew = 1;
    else if (!strcasecmp(option, "--rm-spatial-mean"))
      RmSpatialMean = 1;
    else if (!strcasecmp(option, "--allow-zero-dof"))
      AllowZeroDOF = 1;
      ...
      format = "nii.gz";
    else if (!strcasecmp(option, "--mgh"))
      format = "mgh";
    else if (!strcasecmp(option, "--mgz"))
      format = "mgz";
    else if (!strcasecmp(option, "--allowsubjrep"))
      fsgdf_AllowSubjRep = 1; /* external, see fsgdf.h */
    else if (!strcasecmp(option, "--fsgd-rescale"))
      fsgdReScale = 1;
    else if (!strcasecmp(option, "--fisher"))
      DoFisher = 1;
    else if (!strcasecmp(option, "--pcc"))
      DoPCC = 1;
    else if (!strcasecmp(option, "--no-pcc"))
      DoPCC = 0;
    else if (!strcasecmp(option, "--rescale-x"))
      ReScaleX = 1;
    else if (!strcasecmp(option, "--no-rescale-x"))
      ReScaleX = 0;
    ...
    else if (!strcasecmp(option, "--distance"))
      DoDistance = 1;
    else if (!strcasecmp(option, "--illcond"))
      IllCondOK = 1;
    else if (!strcasecmp(option, "--no-illcond"))
      IllCondOK = 0;
      ...
      if (nargc < 1)
        CMDargNErr(option, 1);
      if (!strcmp(pargv[0], "abs"))
        tSimSign = 0;
      else if (!strcmp(pargv[0], "pos"))
        tSimSign = +1;
      else if (!strcmp(pargv[0], "neg"))
        tSimSign = -1;
      else {
      ...
      RandSplit = 1;
      nargsused = 2;
    } else if (!strcmp(option, "--really-use-average7"))
      ReallyUseAverage7 = 1;
      ...
      if (!strcmp(subject, "average7")) {
        if (!ReallyUseAverage7) {
          printf("\n");
          printf("ERROR: you have selected subject average7. It is recommended "
                 "that\n");
          printf(
          ...
          printf("\n");
          printf("INFO: you have selected subject average7 (and REALLY want to "
                 "use it)\n");
          printf("instead of fsaverage. So I'm going to turn off fixing of "
                 "vertex area\n");
	...
	exit(1);
      ...
      ComputeFWHM = 0;
    else if (!strcasecmp(option, "--no-est-fwhm"))
      ComputeFWHM = 0;
...
static void print_usage() {
  ...
  printf("\n");
  printf("   --y inputfile\n");
  printf("   --table stats-table : as output by asegstats2table or "
         "aparcstats2table \n");
  ...
  printf("   --dti siemensdicom : do DTI analysis extracting bvals and bvecs "
         "from dicom\n");
  printf("   --dti-X X.txt : do DTI analysis using provided matrix\n");
  printf("\n");
  printf("   --pvr pvr1 <--prv pvr2 ...> : per-voxel regressors\n");
  printf("   --selfreg col row slice   : self-regressor from index col row "
         "slice\n");
  printf("\n");
  ...
  printf("   --ffxdofdat ffxdof.dat : text file with dof for fixed effects "
         "analysis\n");
  printf("\n");
  ...
  printf("   --prune : remove voxels that do not have a non-zero value at each "
         "frame (def)\n");
  ...
  printf("   --no-fix-vertex-area : turn off fixing of vertex area (for back "
         "comapt only)\n");
...
static void print_help() {
  ...
  printf("  cond.mgh - design matrix condition at each voxel (with "
         "--save-cond)\n");
  ...
  printf("--y inputfile\n");
  printf("\n");
  printf("Path to input file with each frame being a separate input. This can "
         "be\n");
  ...
  printf(
      "(Different Offset, Same Slope) and dods (Different Offset, Different\n");
  printf("Slope). doss will create a design matrix in which each class has "
         "it's\n");
  printf("own offset but forces all classes to have the same slope. dods "
         "models\n");
  printf("each class with it's own offset and slope. In either case, you'll "
         "need\n");
  printf("to know the order of the regressors in order to correctly specify "
         "the\n");
  printf("contrast vector. For doss, the first NClass columns refer to the\n");
  printf(
      "offset for each class.  The remaining columns are for the continuous\n");
  printf("variables. In dods, the first NClass columns again refer to the "
         "offset\n");
  printf("for each class.  However, there will be NClass*NVar more columns "
         "(ie,\n");
  printf("one column for each variable for each class). The first NClass "
         "columns\n");
  printf("are for the first variable, etc. If neither of these models works "
         "for\n");
  printf(
      "you, you will have to specify the design matrix manually (with --X).\n");
  printf("\n");
  ...
  printf("--X design matrix file\n");
  printf("\n");
  printf("Explicitly specify the design matrix. Can be in simple text or in "
         "matlab4\n");
  printf("format. If matlab4, you can save a matrix with "
         "save('X.mat','X','-v4');\n");
  printf("\n");
  printf("--C contrast1.mtx <--C contrast2.mtx ...>\n");
  printf("\n");
  ...
  printf(
      "ASCII text file with the contrast matrix in it (make sure the last\n");
  printf("line is blank). The name can be (almost) anything. If the extension "
         "is\n");
  printf(
      ".mtx, .mat, .dat, or .con, the extension will be stripped of to form\n");
  ...
  printf("You cannot specify both --X and --osgm. A contrast cannot be "
         "specified\n");
  printf("either. The contrast name will be osgm.\n");
  printf("\n");
  printf("--pvr pvr1 <--prv pvr2 ...>\n");
  printf("\n");
  printf("Per-voxel (or vertex) regressors (PVR). Normally, the design matrix "
         "is\n");
  printf("'global', ie, the same matrix is used at each voxel. This option "
         "allows the\n");
  ...
  printf("\n");
  printf("Create a 'self-regressor' from the input data based on the waveform "
         "at\n");
  printf("index col row slice. This waveform is residualized and then added as "
         "a\n");
  printf("column to the design matrix. Note: the contrast matrices must "
         "include\n");
  ...
  printf(
      "of ordinary least squares (OLS).  This requires that the lower-level\n");
  printf("variances be available.  This is often the case with fMRI analysis "
         "but\n");
  printf(
      "not with an anatomical analysis. Note: this should not be confused\n");
  printf("with fixed effects analysis. The weights will be inverted,\n");
  printf("square-rooted, and normalized to sum to the number of inputs for "
         "each\n");
  ...
  printf("--ffxdofdat ffxdof.dat : text file with DOF for fixed effects "
         "analysis\n");
  printf("\n");
  printf("Perform fixed-effect analysis. This requires that the lower-level "
         "variances\n");
  printf("be available.  This is often the case with fMRI analysis but not "
         "with\n");
  printf("an anatomical analysis. Note: this should not be confused with "
         "weighted\n");
  printf("random effects analysis (wls). The dof is the sum of the DOFs from "
         "the\n");
  printf("lower levels.\n");
  printf("\n");
  printf("--w weightfile\n");
  ...
  printf("--w-sqrt\n");
  printf("\n");
  printf("Perform weighted LMS using per-voxel weights from the weightfile. "
         "The\n");
  ...
  printf("--fwhm fwhm\n");
  printf("\n");
  printf("Smooth input with a Gaussian kernel with the given "
         "full-width/half-maximum\n");
  printf(
      "(fwhm) specified in mm. If the data are surface-based, then you must\n");
  printf("specify --surf, otherwise mri_glmfit assumes that the input is a "
         "volume\n");
  ...
  printf("assumes that the input is a volume and will perform volume "
         "smoothing.\n");
  printf("\n");
  ...
  printf("--cortex \n");
  printf("\n");
  printf("Only perform analysis where mask=1. All other voxels will be set to "
         "0.\n");
  printf(
      "If using surface, then labelfile will be converted to a binary mask\n");
  ...
  printf("--no-prune\n");
  printf("\n");
  printf("This happens by default. Use --no-prune to turn it off. Remove "
         "voxels\n");
  printf("from the analysis if the ALL the frames at that voxel\n");
  printf(
      "do not have an absolute value that exceeds zero (actually FLT_MIN, \n");
  printf("or whatever is set by --prune_thr). This helps to prevent the "
         "situation \n");
  printf(
      "where some frames are 0 and others are not. If no mask is supplied, \n");
  printf("a mask is created and saved. If a mask is supplied, it is pruned, "
         "and \n");
  printf("the final mask is saved. Do not use with --sim. Rather, run the "
         "non-sim \n");
  printf("analysis with --prune, then pass the created mask when running "
         "simulation. \n");
  printf("It is generally a good idea to prune. --no-prune will turn off "
         "pruning \n");
  printf(
      "if it had been turned on. For DTI, only the first frame is used to \n");
  ...
  printf("\n");
  printf("Specify that the input has a surface geometry from the hemisphere of "
         "the\n");
  printf("given FreeSurfer subject. This is necessary for smoothing surface "
         "data\n");
  printf(
      "(--fwhm or --var-fwhm), specifying a label as a mask (--label), or\n");
  printf("running a simulation (--sim) on surface data. If --surf is not "
         "specified,\n");
  printf(
      "then mri_glmfit will assume that the data are volume-based and use\n");
  printf("the geometry as specified in the header to make spatial "
         "calculations.\n");
  printf(
      "By default, the white surface is used, but this can be overridden by\n");
  ...
  printf("--pca\n");
  printf("\n");
  printf("Flag to perform PCA/SVD analysis on the residual. The result is "
         "stored\n");
  printf("in glmdir/pca-eres as v.mgh (spatial eigenvectors), u.mtx (frame\n");
  printf("eigenvectors), sdiag.mat (singular values). eres = u*s*v'. The "
         "matfiles\n");
  ...
  printf("--save-yhat\n");
  printf("\n");
  printf("Flag to save the signal estimate (yhat) as glmdir/yhat.mgh. "
         "Normally, this\n");
  printf("pis not very useful except for debugging.\n");
  printf("\n");
  printf("--save-cond\n");
  printf("\n");
  printf("Flag to save the condition number of the design matrix at eaach "
         "voxel.\n");
  printf("Normally, this is not very useful except for debugging. It is "
         "totally\n");
  printf("useless if not using weights or PVRs.\n");
  printf("\n");
  printf("--nii, --nii.gz\n");
  printf("\n");
  printf("Use nifti (or compressed nifti) as output format instead of mgh. "
         "This\n");
  printf(
      "will work with surfaces, but you will not be able to open the output\n");
  ...
  printf("--seed seed\n");
  printf("\n");
  printf("Use seed as the seed for the random number generator. By default, "
         "mri_glmfit\n");
  printf("will select a seed based on time-of-day. This only has an effect "
         "with\n");
  printf("--sim or --synth.\n");
  printf("\n");
  printf("--synth\n");
  printf("\n");
  printf("Replace input data with whise gaussian noise. This is good for "
         "testing.\n");
  printf("\n");
  printf("--voxdump col row slice\n");
  printf("\n");
  printf("Save GLM data for a single voxel in directory "
         "glmdir/voxdump-col-row-slice.\n");
  printf("Exits immediately. Good for debugging.\n");
  printf("\n");
  printf("\n");
  printf("MONTE CARLO SIMULATION AND CORRECTION FOR MULTIPLE COMPARISONS\n");
  printf("\n");
  printf("One method for correcting for multiple comparisons is to perform "
         "simulations\n");
  printf(
      "under the null hypothesis and see how often the value of a statistic\n");
  printf("from the 'true' analysis is exceeded. This frequency is then "
         "interpreted\n");
  printf(
      "as a p-value which has been corrected for multiple comparisons. This\n");
  printf(
  ...
  printf(
      "on FSLs permuation simulator (randomise) and AFNIs null-z simulator\n");
  printf("(AlphaSim). Note that FreeSurfer also offers False Discovery Rate "
         "(FDR)\n");
  printf("correction in tkmedit and tksurfer.\n");
  printf("\n");
  printf("The estimation, simulation, and correction are done in three "
         "distinct\n");
  printf("phases:\n");
  printf(
      "  1. Estimation: run the analysis on your data without simulation.\n");
  ...
  printf("\n");
  printf("nulltype is the method of generating the null data. Legal values "
         "are:\n");
  printf("  (1) perm - perumation, randomly permute rows of X (cf FSL "
         "randomise)\n");
  ...
  printf("thresh - threshold, specified as -log10(pvalue) to use for "
         "clustering\n");
  printf(
      "csdbasename - base name of the file to store the CSD data in. Each\n");
  printf(
  ...
  printf("--sim-sign sign\n");
  printf("\n");
  printf("sign is either abs (default), pos, or neg. pos/neg tell mri_glmfit "
         "to\n");
...
static void check_options() {
  ...
  if (XOnlyFile != nullptr) {
    if (fsgdfile == nullptr) {
      printf("ERROR: you must spec --fsgd with --xonly\n");
      exit(1);
    }
    printf("INFO: gd2mtx_method is %s\n", gd2mtx_method);
    Xtmp = gdfMatrix(fsgd, gd2mtx_method, nullptr);
    if (Xtmp == nullptr)
      exit(1);
    Xnorm = MatrixNormalizeCol(Xtmp, nullptr, nullptr);
    Xcond = MatrixNSConditionNumber(Xnorm);
    printf("Matrix condition is %g\n", Xcond);
  ...
  if (labelFile != nullptr && surf == nullptr) {
...
static void dump_options(FILE *fp) {
  ...
  if (FWHM > 0) {
    fprintf(fp, "fwhm     %lf\n", FWHM);
    if (surf != nullptr)
      fprintf(fp, "niters    %lf\n", SmoothLevel);
  }
  if (VarFWHM > 0) {
    fprintf(fp, "varfwhm  %lf\n", VarFWHM);
    if (surf != nullptr)
      fprintf(fp, "varniters %lf\n", VarSmoothLevel);
  }
  ...
  if (fsgdfile)
    fprintf(fp, "FSGD %s\n", fsgdfile);
  if (labelFile)
    fprintf(fp, "labelmask  %s\n", labelFile);
  if (maskFile)
    fprintf(fp, "mask %s\n", maskFile);
  if (labelFile || maskFile)
  ...
  if (surf == nullptr) {
    ...
    if (!DoSim || debug || Gdiag_no > 0)
      printf("  Volume Smoothing by FWHM=%lf, Gstd=%lf, t=%lf\n", SmthLevel,
             gstd, mytimer.seconds());
    if (UseMaskWithSmoothing)
      MRImaskedGaussianSmooth(mri, mask, gstd, mri);
    else
      MRImaskedGaussianSmooth(mri, nullptr, gstd, mri);
    if (!DoSim || debug || Gdiag_no > 0)
    ...
    if (UseMaskWithSmoothing)
      ...
      MRISsmoothMRI(surf, mri, SmthLevel, nullptr, mri);
    if (!DoSim || debug || Gdiag_no > 0)
...
int MRISmaskByLabel(MRI *y, MRIS *surf, LABEL *lb, int invflag) {
  ...
  for (vtxno = 0; vtxno < surf->nvertices; vtxno++) {
    if (lbmask[vtxno] && !invflag)
      continue; // in-label and not inverting
    if (!lbmask[vtxno] && invflag)
      continue; // out-label but inverting
    c = crslut[0][vtxno];
    r = crslut[1][vtxno];
    s = crslut[2][vtxno];
    for (f = 0; f < y->nframes; f++)
      MRIsetVoxVal(y, c, r, s, f, 0);
  }
...
double GLMEfficiency(MATRIX *X, MATRIX *C) {
  ...
  M = MatrixMultiplyD(A, Ct, nullptr);
...
int GLMdiagnoseDesignMatrix(MATRIX *X) {
  ...
  printf("SumSq: Min=%f (col %d), Max=%f (col %d)\n", XsumsqMin, cXsumsqMin,
         XsumsqMax, cXsumsqMax);
  if (XsumsqMax / XsumsqMin > 10) {
    printf(" The scale is much different between columns %d and %d, you may "
           "want to \n",
           cXsumsqMin, cXsumsqMax);
    printf(" normalize by subtracting the mean and dividing by the standard "
           "deviation.\n");
    ret = 1;
  }
  free(Xsumsq);
...
MRI *MRIremoveSpatialMean(MRI *vol, MRI *mask, MRI *out) {
  ...
  if (out == nullptr) {
    out = MRIallocSequence(vol->width, vol->height, vol->depth, MRI_FLOAT,
                           vol->nframes);
