...
void vtkFDTensorGlyph::Execute() {
  ...
  const float cubePoints[8][3] = {{0.5, -0.5, 0.5},  {0.5, 0.5, 0.5},
  ...
  for (int x = 0; x < dim[0]; x++) {
    for (int y = 0; y < dim[1]; y++) {
      for (int z = 0; z < dim[2]; z++) {
        tensorComponents = (float *)input->GetScalarPointer(
            x + extent[0], y + extent[2], z + extent[4]);
        ...
        for (int i = 0; i < 3; i++) {
          normalizedEigenValues[i] = (tensorComponents[EVA + i] > 0)
                                         ? tensorComponents[EVA + i]
                                         : this->SMALL_EIGENVALUE;
          normalizedEigenValues[i] =
              tensorComponents[EVA + i] / eigenValue1Range[1];
        }
        ...
        for (int matrixRow = 0; matrixRow < 3; matrixRow++) {
          matrix->SetElement(matrixRow, 0,
                             eigvec1Rotated[matrixRow] *
                                 (this->UniformScaling
                                      ? uniformScaleFactors[0]
                                      : normalizedEigenValues[0]) *
                                 (this->FAScaling ? normalizedFA : 1) *
                                 inputSpacing[matrixRow] * this->ScaleFactor);
          matrix->SetElement(matrixRow, 1,
                             eigvec2Rotated[matrixRow] *
                                 (this->UniformScaling
                                      ? uniformScaleFactors[1]
                                      : normalizedEigenValues[1]) *
                                 (this->FAScaling ? normalizedFA : 1) *
                                 inputSpacing[matrixRow] * this->ScaleFactor);
          matrix->SetElement(matrixRow, 2,
                             eigvec3Rotated[matrixRow] *
                                 (this->UniformScaling
                                      ? uniformScaleFactors[2]
                                      : normalizedEigenValues[2]) *
                                 (this->FAScaling ? normalizedFA : 1) *
                                 inputSpacing[matrixRow] * this->ScaleFactor);
        ...
        colors->SetTuple4(
            colorCount, sqrt(faval) * fabs(eigvec1Aligned[0]) * 255.0,
            sqrt(faval) * fabs(eigvec1Aligned[1]) * 255.0,
            sqrt(faval) * fabs(eigvec1Aligned[2]) * 255.0 + .5, 255);
        colorCount++;
...
void vtkFDTensorGlyph::ComputeScalarRangeGreaterThanZero(const int component,
  ...
  for (int tupleIndex = 0; tupleIndex < numberOfScalars;
       tupleIndex += numberOfComponents) {
