...
public:
  bool operator()(const vtkCTFNode *node1, const vtkCTFNode *node2) {
...
inline double vtkRGBAColorTransferFunctionAngleDiff(double a1, double a2) {
  ...
  if (adiff < 0.0)
    adiff = -adiff;
  while (adiff >= 2 * vtkMath::Pi() * 2)
    adiff -= 2 * vtkMath::Pi() * 2;
  if (adiff > vtkMath::Pi() * 2)
    adiff = 2 * vtkMath::Pi() * 2 - adiff;
  return adiff;
}
...
inline void vtkRGBAColorTransferFunctionInterpolateDiverging(
  ...
  if ((msh1[1] > 0.05) && (msh2[1] > 0.05) &&
      (vtkRGBAColorTransferFunctionAngleDiff(msh1[2], msh2[2]) >
       0.33 * vtkMath::Pi() * 2)) {
...
int vtkRGBAColorTransferFunction::AddRGBAPoint(double x, double r, double g,
                                               double b, double a) {
...
int vtkRGBAColorTransferFunction::AddHSVAPoint(double x, double h, double s,
                                               double v, double a) {
  double r, b, g;
...
void vtkRGBAColorTransferFunction::SortAndUpdateRange() {
  std::sort(this->Internal->Nodes.begin(), this->Internal->Nodes.end(),
    ...
    // Otherwise, we are between two nodes - interpolate
      ...
      if (sharpness < 0.01) {
        ...
        if (this->ColorSpace == VTK_CTF_RGB) {
          ...
          vtkRGBAColorTransferFunctionInterpolateDiverging(s, rgba1, rgba2,
                                                           tptr);
        } else {
...
//----------------------------------------------------------------------------
const unsigned char *
vtkRGBAColorTransferFunction::GetTable(double xStart, double xEnd, int size) {
...
int vtkRGBAColorTransferFunction::GetNodeValue(int index, double val[7]) {
  ...
  if (index < 0 || index >= size) {
    ...
    val[0] =
        0; // initialize in case caller uses - detected as error by gcc4.5 on
    val[1] = 0;
    val[2] = 0;
    val[3] = 0;
    val[4] = 0;
...
void vtkRGBAColorTransferFunction::DeepCopy(vtkRGBAColorTransferFunction *f) {
  if (f != nullptr) {
    ...
    for (i = 0; i < f->GetSize(); i++) {
      ...
      this->AddRGBAPoint(val[0], val[1], val[2], val[3], val[4], val[5],
                         val[6]);
    }
    this->Modified();
  }
...
void vtkRGBAColorTransferFunctionMagMapData(vtkRGBAColorTransferFunction *self,
  ...
  vtkRGBAColorTransferFunctionMapData(self, mag, output, length, 1, outFormat,
                                      v);
...
void vtkRGBAColorTransferFunction::MapScalarsThroughTable2(
  ...
  switch (inputDataType) {
    vtkTemplateMacro(vtkRGBAColorTransferFunctionMapData(
