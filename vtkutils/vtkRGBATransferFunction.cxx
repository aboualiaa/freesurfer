...
int vtkRGBATransferFunction::AddRGBAPoint(double x, double r, double g,
  ...
  if (i < this->NumberOfPoints && this->Function[5 * i] == x) {
    if (this->Function[5 * i + 1] != r || this->Function[5 * i + 2] != g ||
        this->Function[5 * i + 3] != b || this->Function[5 * i + 4] != a) {
...
int vtkRGBATransferFunction::AddHSVAPoint(double x, double h, double s,
                                          double v, double a) {
  double r, b, g;
...
void vtkRGBATransferFunction::AddRGBASegment(double x1, double r1, double g1,
                                             double b1, double a1, double x2,
...
void vtkRGBATransferFunction::AddHSVASegment(double x1, double h1, double s1,
                                             double v1, double a1, double x2,
    ...
    if (loc == this->NumberOfPoints) {
      ...
      // We are somewhere in the middle. Use the correct interpolation.
        ...
        // HSV space
          ...
          vtkMath::RGBToHSV(*(fptr - 4), *(fptr - 3), *(fptr - 2), &h1, &s1,
                            &v1);
          vtkMath::RGBToHSV(*(fptr + 1), *(fptr + 2), *(fptr + 3), &h2, &s2,
                            &v2);
          s3 = (1.0 - weight) * s1 + weight * s2;
          v3 = (1.0 - weight) * v1 + weight * v2;
          // Do we need to cross the 0/1 boundary?
          if (this->HSVWrap && (h1 - h2 > 0.5 || h2 - h1 > 0.5)) {
  ...
  for (i = 0, x = x1; i < size; i++, x += xinc) {
    ...
    if (loc == this->NumberOfPoints) {
      ...
      // We are somewhere in the middle. Use the correct interpolation.
        ...
        // RGB space
        if (this->ColorSpace == VTK_CTF_RGB) {
          *(tptr++) = static_cast<float>((1.0 - weight) * *(fptr - 4) +
                                         weight * *(fptr + 1));
          *(tptr++) = static_cast<float>((1.0 - weight) * *(fptr - 3) +
                                         weight * *(fptr + 2));
          *(tptr++) = static_cast<float>((1.0 - weight) * *(fptr - 2) +
                                         weight * *(fptr + 3));
          *(tptr++) = static_cast<float>((1.0 - weight) * *(fptr - 1) +
                                         weight * *(fptr + 4));
        }
        // HSV space
        else {
          double h1, h2, h3, s1, s2, s3, v1, v2, v3;
          vtkMath::RGBToHSV(*(fptr - 3), *(fptr - 2), *(fptr - 1), &h1, &s1,
                            &v1);
          vtkMath::RGBToHSV(*(fptr + 1), *(fptr + 2), *(fptr + 3), &h2, &s2,
                            &v2);
          s3 = (1.0 - weight) * s1 + weight * s2;
          ...
          s3 = (s3 > 1.0) ? (1.0) : ((s3 < 0.0) ? (0.0) : (s3));
          v3 = (v3 > 1.0) ? (1.0) : ((v3 < 0.0) ? (0.0) : (v3));
          vtkMath::HSVToRGB(static_cast<float>(h3), static_cast<float>(s3),
                            static_cast<float>(v3), tptr, tptr + 1, tptr + 2);
          *(tptr + 3) = static_cast<float>((1.0 - weight) * *(fptr - 1) +
                                           weight * *(fptr + 4));
          tptr += 4;
        }
      }
  ...
  for (i = 0, x = x1; i < size; i++, x += xinc) {
    ...
    if (loc == this->NumberOfPoints) {
      ...
      // We are somewhere in the middle. Use the correct interpolation.
        ...
        // RGB space
        if (this->ColorSpace == VTK_CTF_RGB) {
          *(tptr++) = (unsigned char)(255 * ((1.0 - weight) * *(fptr - 4) +
                                             weight * *(fptr + 1)));
          *(tptr++) = (unsigned char)(255 * ((1.0 - weight) * *(fptr - 3) +
                                             weight * *(fptr + 2)));
          *(tptr++) = (unsigned char)(255 * ((1.0 - weight) * *(fptr - 2) +
                                             weight * *(fptr + 3)));
          *(tptr++) = (unsigned char)(255 * ((1.0 - weight) * *(fptr - 1) +
                                             weight * *(fptr + 4)));
        }
        // HSV space
        else {
...
void vtkRGBATransferFunction::PrintSelf(ostream &os, vtkIndent indent) {
  ...
  // These are old
  //  os << indent << "Color Transfer Function Total Points: " <<
...
void vtkRGBATransferFunction::DeepCopy(vtkRGBATransferFunction *f) {
  ...
  if (this->FunctionSize > 0) {
    this->Function = new double[5 * this->FunctionSize];
    memmove(this->Function, f->Function,
            5 * sizeof(double) * this->FunctionSize);
  } else {
    this->Function = nullptr;
  }
...
void vtkRGBATransferFunctionMapData(vtkRGBATransferFunction *self, T *input,
  ...
  while (--i >= 0) {
    ...
    {
      *(optr++) =
          (unsigned char)(rgba[0] * 76.5 + rgba[1] * 150.45 + rgba[2] * 28.05);
    }
...
//----------------------------------------------------------------------------
void vtkRGBATransferFunction::MapScalarsThroughTable2(
    void *input, unsigned char *output, int inputDataType, int numberOfValues,
    int inputIncrement, int outputFormat) {
  switch (inputDataType) {
    vtkTemplateMacro(vtkRGBATransferFunctionMapData(
