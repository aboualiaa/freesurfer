...
vtkKWBltGraph::vtkKWBltGraph()
    : XAxisTitle(NULL), YAxisTitle(NULL), mMouseoverDistanceToElement(10),
      mbCurrentlyOverElement(false), msCurrentMouseoverElement("") {
...
void vtkKWBltGraph::CreateWidget() {

  // Create our blt widget.
  if (!vtkKWCoreWidget::CreateSpecificTkWidget(
          this, "blt::graph",
          "-plotbackground white -relief raised -border 2")) {
...
void vtkKWBltGraph::GetPlotBackgroundColor(double *iRed, double *iGreen,
                                           double *iBlue) {

  this->GetConfigurationOptionAsColor("-plotbackground", iRed, iGreen, iBlue);
}

double *vtkKWBltGraph::GetPlotBackgroundColor() {

  return this->GetConfigurationOptionAsColor("-pltbackground");
}

void vtkKWBltGraph::SetPlotBackgroundColor(double iRed, double iGreen,
                                           double iBlue) {

  this->SetConfigurationOptionAsColor("-plotbackground", iRed, iGreen, iBlue);
}

void vtkKWBltGraph::SetRelief(int iRelief) {

  this->SetConfigurationOption("-relief",
                               vtkKWOptions::GetReliefAsTkOptionValue(iRelief));
}

void vtkKWBltGraph::SetReliefToRaised() {

  this->SetRelief(vtkKWOptions::ReliefRaised);
}

void vtkKWBltGraph::SetReliefToSunken() {

  this->SetRelief(vtkKWOptions::ReliefSunken);
}

void vtkKWBltGraph::SetReliefToFlat() {

  this->SetRelief(vtkKWOptions::ReliefFlat);
}

void vtkKWBltGraph::SetReliefToRidge() {

  this->SetRelief(vtkKWOptions::ReliefRidge);
}

void vtkKWBltGraph::SetReliefToSolid() {

  this->SetRelief(vtkKWOptions::ReliefSolid);
}

void vtkKWBltGraph::SetReliefToGroove() {

  this->SetRelief(vtkKWOptions::ReliefGroove);
}

int vtkKWBltGraph::GetRelief() {

  return vtkKWOptions::GetReliefFromTkOptionValue(
      this->GetConfigurationOption("-relief"));
}

void vtkKWBltGraph::SetBorderWidth(int iWidth) {

  this->SetConfigurationOptionAsInt("-border", iWidth);
}

int vtkKWBltGraph::GetBorderWidth() {

  return this->GetConfigurationOptionAsInt("-border");
}

void vtkKWBltGraph::SetLegendVisible(int ibVisible) {

  if (this->IsCreated()) {
    this->Script("%s legend configure -hide %d", this->GetWidgetName(),
                 !ibVisible);
  }
}

void vtkKWBltGraph::SetLegendVisibleToOn() { this->SetLegendVisible(1); }

void vtkKWBltGraph::SetLegendVisibleToOff() { this->SetLegendVisible(0); }

void vtkKWBltGraph::SetXAxisTitle(const char *isTitle) {

  if (NULL == XAxisTitle && NULL == isTitle)
    return;

  if (NULL != XAxisTitle && NULL != isTitle && 0 == strcmp(XAxisTitle, isTitle))
    return;
...
void vtkKWBltGraph::SetYAxisTitle(const char *isTitle) {

  if (NULL == YAxisTitle && NULL == isTitle)
    return;

  if (NULL != YAxisTitle && NULL != isTitle && 0 == strcmp(YAxisTitle, isTitle))
    return;
...
void vtkKWBltGraph::DeleteElement(const char *isLabel) {
  GraphElementList::iterator tElement;
  for (tElement = mElements.begin(); tElement != mElements.end(); ++tElement) {
    if (tElement->msLabel == isLabel) {
...
void vtkKWBltGraph::DeleteAllElements() {
  ...
  for (tElement = mElements.begin(); tElement != mElements.end(); ++tElement) {
    ...
    // This will erase it in the graph
    this->Script("%s element delete %s", this->GetWidgetName(),
...
void vtkKWBltGraph::UpdateXAxisTitle() {

  if (this->IsCreated()) {

    // Set the title in the graph.
    if (NULL != XAxisTitle) {
      this->Script("%s axis configure x -title {%s}", this->GetWidgetName(),
                   XAxisTitle);
    } else {
      this->Script("%s axis configure x -title {}", this->GetWidgetName());
    }
  }
}

void vtkKWBltGraph::UpdateYAxisTitle() {

  if (this->IsCreated()) {

    // Set the title in the graph.
    if (NULL != YAxisTitle) {
      this->Script("%s axis configure y -title {%s}", this->GetWidgetName(),
                   YAxisTitle);
    } else {
      this->Script("%s axis configure y -title {}", this->GetWidgetName());
    }
  }
}

void vtkKWBltGraph::Draw() {
  ...
  for (tElement = mElements.begin(); tElement != mElements.end(); ++tElement) {
    ...
    this->Script("%s element create %s "
                 ...
                 ssData.str().c_str(), (int)(element.mRed * 255.0),
                 (int)(element.mGreen * 255.0), (int)(element.mBlue * 255.0),
                 element.mLineWidth, element.msSymbol.c_str());
  }
}

void vtkKWBltGraph::MotionCallback(const char *isWidget, int iX, int iY) {
  ...
  if (sReply) {
    ...
    GraphElementList::iterator tElement;
    for (tElement = mElements.begin(); tElement != mElements.end();
         ++tElement) {
...
void vtkKWBltGraph::Button3DownCallback(const char *isWidget, int iX, int iY) {

  // If we're moused over an element...
  if (mbCurrentlyOverElement) {

    // Make a menu.
    vtkSmartPointer<vtkKWMenu> popup = vtkSmartPointer<vtkKWMenu>::New();
...
vtkKWBltGraph::GraphElement::GraphElement()
    : msLabel(""), msSymbol(""), mRed(0), mGreen(0), mBlue(0) {}

void vtkKWBltGraph::SavePostscript(const char *ifnPS) {
