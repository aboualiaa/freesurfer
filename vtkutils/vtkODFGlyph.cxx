...
vtkODFGlyph::vtkODFGlyph() : ScaleFactor(7), BrightnessLevels(100) {
  ...
  for (int i = 0; i < this->BrightnessLevels; i++) {
    ...
    double multiplier =
        (static_cast<double>(i) / static_cast<double>(this->BrightnessLevels)) *
            0.5 +
        0.5;
    for (int j = 0; j < this->NUM_SPHERE_POINTS; j++) {
      this->ColorTable->SetTableValue(
          i * this->NUM_SPHERE_POINTS + j,
          multiplier * fabs(this->SPHERE_POINTS[j][0]),
          multiplier * fabs(this->SPHERE_POINTS[j][1]),
          multiplier * fabs(this->SPHERE_POINTS[j][2]));
    }
  }
}
...
void vtkODFGlyph::Execute() {
  ...
  pcoords->SetNumberOfTuples(this->ODF_SIZE * dim[0] * dim[1] *
                             dim[2]); // can't be larger than this

  vtkIdTypeArray *scalars = vtkIdTypeArray::New();
  scalars->SetNumberOfValues(this->ODF_SIZE * dim[0] * dim[1] *
                             dim[2]); // can't be larger than this

  vtkCellArray *faces = vtkCellArray::New();

  // hope incrementing pointID each iteration is faster than calling
  // input->ComputePointId every time. will this work every time?
  vtkIdType pointID = -1; // is incremented to 0 in first iteration of loop
  for (int z = 0; z < dim[2]; z++) {
    for (int y = 0; y < dim[1]; y++) {
      for (int x = 0; x < dim[0]; x++) {
        pointID++; // need to increment even if we don't produce a glyph for
                   // this voxel

        float *inData = static_cast<float *>(input->GetScalarPointer(
            x + extent[0], y + extent[2], z + extent[4]));
        if (inData[0] == 0)
          continue; // no odf val == 0 unless there is no glyph at this voxel
        ...
        /*first this->ODF_SIZE components in inData are odf values. next 1
          component is gfa value. next 3*n components are n xyz triplets
          specifying qball connectivity*/

        // find min and max odf vals for normalization
        double minODF = 1, maxODF = 0;
        for (int i = 0; i < this->ODF_SIZE; i++) {
          double currentODF = static_cast<double>(inData[i]);
          if (currentODF < minODF)
            minODF = currentODF;
          if (currentODF > maxODF)
            maxODF = currentODF;
        }

        double normalizedGFA =
            (static_cast<double>(inData[this->ODF_SIZE]) - this->GFARange[0]) /
            (this->GFARange[1] - this->GFARange[0]);

        // for each face, where i = index of first face point
        for (int i = this->ODF_SIZE + 1;
             i < input->GetNumberOfScalarComponents(); i += 3) {
          vtkIdType faceIds[3], facePointId;

          // for each point in face, where j = index of point within the face
          for (int j = 0; j < 3; j++) {
            facePointId = static_cast<vtkIdType>(inData[i + j]) -
                          1; // input pt ids from 1-based index

            /*-2 indicates beginning of padding (because there are less faces
              than the maximum possible)*/
            if (facePointId == -2) {
              newId -= j; // shouldn't ever break in middle of 3 points, but
                          // just in case, roll back id
              break;
            } else {
              ...
              if (pointIds[facePointId] == -1) {
                ...
                double pt[3];
                double normalizedODF =
                    (static_cast<double>(inData[facePointId]) - minODF) /
                    (maxODF - minODF);

                // displace point by normalized ODF (and scaling factor?)
                double pointDisplacement =
                    normalizedODF; // * this->ScaleFactor;
                ...
                for (int k = 0; k < 3; k++) {
                  // deform sphere point w/ pointDisplacement, and translate to
                  // voxel location
                  pt[k] = this->SPHERE_POINTS[facePointId][k] *
                              pointDisplacement * (spacing[k] / 2) +
                          voxelLocation[k];
                }
                pcoords->SetTuple(newId, pt);

                // sphere point location determines color, normalized odf
                // determines brightness
                scalars->SetValue(
                    newId,
                    static_cast<int>(normalizedODF * this->BrightnessLevels) *
                            this->NUM_SPHERE_POINTS +
                        facePointId);
                faceIds[j] = newId;
                newId++;
