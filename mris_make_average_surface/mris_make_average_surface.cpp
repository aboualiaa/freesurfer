...
#define SCALE 1
  ...
  for (n = 0, i = 5; i < argc; i++) {
    ...
    if (fio_FileExistsReadable(fname))
      mri = MRIreadHeader(fname, MRI_MGH_FILE);
    else {
    ...
    if (!mri)
      ErrorExit(ERROR_BADPARM,
                "%s: could not read reference MRI volume from %s", Progname,
                fname);
    ...
    if (xform_name) {
      if (!strcmp(xform_name, "talairach.xfm")) {
        ...
        fflush(stdout);
        XFM =
            DevolveXFMWithSubjectsDir(argv[i], nullptr, "talairach.xfm", sdir);
        if (XFM == nullptr)
          exit(1);
    ...
    mrisp =
        MRIScoordsToParameterization(mris, nullptr, SCALE, ORIGINAL_VERTICES);
  ...
  for (vno = 0; vno < mris_ico->nvertices;
       vno++) { // MRISscale looks suitable but sets other things also
