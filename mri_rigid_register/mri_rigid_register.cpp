...
/*E*
  ...
  $ ~/dev/mri_rigid_register/mri_rigid_register
/tmp/ebeth/BERA13_recon/synth.mgh /tmp/ebeth/BERA13_recon/cor-rot30/
/tmp/ebeth/BERA13_recon/dmrr_synthmgh2corrot30cor-corras.lta

  and

  $ ~/dev/mri_rigid_register/mri_rigid_register
/tmp/ebeth/BERA13_recon/cor-rot30/ /tmp/ebeth/BERA13_recon/synth.mgh
/tmp/ebeth/BERA13_recon/dmrr_corrot30cor2synthmgh-corras.lta
...
/*static */
MATRIX *MRR_VoxelXformToCoronalRasXform(MRI *mri_src, MRI *mri_dst,
                                        MATRIX *m_voxel_xform,
                                        MATRIX *m_coronalras_xform);
// static int maxabs(int a, int b, int c); //E/ unquestionably the wrong tool

static int apply_transform(MRI *mri_in, MRI *mri_target, MATRIX *M_reg,
                           MRI *mri_xformed);

static char *B1_fname = nullptr;
...
static void estimate_rigid_regmatrix(MRI *mri_source, MRI *mri_target,
                                     MATRIX *M_reg, MRI *mri_B1);
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv,
      "$Id: mri_rigid_register.c,v 1.17 2015/02/05 23:34:40 zkaufman Exp $",
      "$Name:  $");
  ...
  if (!mri_src)
    ErrorExit(ERROR_NOFILE, "%s: could not read src MRI from %s", Progname,
              src_fname);
  if (!mri_target)
    ErrorExit(ERROR_NOFILE, "%s: could not read dst MRI from %s", Progname,
              target_fname);

  if (B1_fname) {
    mri_B1 = MRIread(B1_fname);
    if (!mri_B1)
      ErrorExit(ERROR_NOFILE, "%s: could not read B1 mapfrom %s", Progname,
                B1_fname);
  } else
    mri_B1 = nullptr;
  ...
  M_reg = MatrixIdentity(4, (MATRIX *)nullptr);
...
//#define _MRR_DEBUG
  ...
  if (voxel_xform) {
    ...
    vox_s2vox_t = MRIrasXformToVoxelXform(mri_src, mri_target, M_reg, nullptr);
  ...
  for (i = 0; i < nwrite; i++) {
...
#if 1
    mri_xformed = MRR_MRIhybrid(mri_in, mri_target, nullptr);
    ...
    if (apply_out_type == 1) // -out_like mri_target
    {
      ...
      apply_transform(mri_in, mri_target, M_reg, mri_xformed);
    } else if (apply_out_type ==
               2) // -out_like tkmedit template - coronal 1mm^3 256^3 c_ras=0
                  // but not UCHAR for compatibility with mri_transform, which
                  // won't know about target unless you specify -out_like.
    {
...

  {
    ...
    mri_xformed = MRR_MRIhybrid(mri_src, mri_target, nullptr);
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "sinc")) {
    ...
    noskull = 1;
    printf("assuming skull stripped images: discounting 0 voxels in fine "
           "alignment\n");
  ...
  /*  else if (!stricmp(option, "noconform")) // not implemented
  {
    ...
    case 'V':
      voxel_xform = 1;
      fprintf(stderr, "CAVEAT: e.g. mri_transform may not handle type "
                      "LINEAR_VOXEL_TO_VOXEL well - consider writing out a "
                      "LINEAR_CORONAL_RAS_TO_CORONAL_RAS transform.  Invoke "
                      "mri_transform -h for more info.\n");
      break;
    case 'R':
...
static void usage_exit(int code) {
  fprintf(
      stderr,
      "usage: %s [options] <src volume> <target volume> <transform fname>\n\n",
      Progname);
  fprintf(stderr,
          "Consider _not running with -v for vox2vox transform.  The default, "
          "coronal-ras-to-coronal-ras is pretty useful...\n\n");
  exit(code);
}
...
static void estimate_rigid_regmatrix(MRI *mri_source, MRI *mri_target,
                                     MATRIX *M_reg, MRI *mri_B1) {
  double xf, yf, zf, tx, ty, tz, ax, ay, az, ca, sa, val1, val2, err, sse,
      best_sse, dt = 0.1, da = RADIANS(0.005), scale_2_to_1, new_scale;
  int x, y, z, txi, tyi, tzi, axi, ayi, azi, indx, stepindx, changed, pass, num;
  int width = mri_source->width, height = mri_source->height,
      depth = mri_source->depth, dx = 10, dy = 10, dz = 10, nvalues, iter = 0;
#if 1
  ...
  MATRIX *vox2ras_source, *ras2vox_source, *vox2ras_target, *ras2vox_target,
      *vox_s2vox_t;
  MATRIX *M_reg_bak, *M_reg_opt, *M_tmp, *M_delta, *M_delta1, *M_delta2,
      *M_delta3, *M_delta4, *M_delta5, *M_delta6, *M_tmp2;
  MATRIX *voxmat1, *voxmat2;
  double voxval1[MAX_VOX], voxval2[MAX_VOX], sum1, sum2, wt[MAX_VOX], max_wt;
...
#define VOXELS_FROM_EDGE 2
    mri_tmp = MRIbinarize(mri_source, nullptr, 1, 128, 0);
    mri_tmp2 = nullptr;
    ...
    mri_tmp = MRIbinarize(mri_target, nullptr, 1, 128, 0);
    mri_tmp2 = nullptr;
    for (i = 0; i < VOXELS_FROM_EDGE; i++) {
      mri_tmp2 = MRIdilate(mri_tmp, mri_tmp2);
      MRIcopy(mri_tmp2, mri_tmp);
  ...
  if (nvalues >= MAX_VOX)
    ErrorExit(ERROR_NOMEMORY, "%s: # of samples %d exceeds max %d", Progname,
              nvalues, MAX_VOX);
  ...
  M_tmp2 = MatrixCopy(M_tmp, nullptr);
  ...
  if (write_iterations > 0) {
    ...
    mri_aligned = MRIlinearTransform(mri_source, nullptr, vox_s2vox_t);
  ...
  for (indx = 1; indx <= nvalues; indx++) {
    if (indx == Gdiag_no)
      DiagBreak();
    if ((voxmat1->rptr[1][indx] == Gx) && (voxmat1->rptr[2][indx] == Gy) &&
          ...
          for (tzi = -3; tzi <= 3; tzi++) {
            ...
            if (sse < best_sse - tol) {
              ...
              if ((write_iterations > 0) && (iter % write_iterations) == 0) {
                ...
                sprintf(fname, "step%04d", iter);
                mri_aligned =
                    MRIlinearTransform(mri_source, nullptr, vox_s2vox_t);
  ...
  printf("scale %d: after searching for optimal translation %2.1f, sse = %2.2f "
         "(%2.1f)\n",
        ...
        for (tyi = -1; tyi <= 1; tyi++)
          for (tzi = -1; tzi <= 1; tzi++)
            for (axi = -1; axi <= 1; axi++)
              for (ayi = -1; ayi <= 1; ayi++)
                for (azi = -1; azi <= 1; azi++)
                  if (((txi != 0) + (tyi != 0) + (tzi != 0) + (axi != 0) +
                       (ayi != 0) + (azi != 0)) <= max_ndelta) {
                    tx = txi * dt * scale;
                    ty = tyi * dt * scale;
                    tz = tzi * dt * scale;
                    ax = axi * da * scale;
                    ...
                    if (sse < best_sse - tol) {
                      ...
                      if ((write_iterations > 0) &&
                        ...
                        MatrixMultiply(ras2vox_target, M_tmp, vox_s2vox_t);
                        sprintf(fname, "step%04d", iter);
                        mri_aligned = MRIlinearTransform(mri_source, nullptr,
                                                         vox_s2vox_t);
                        printf("writing snapshots to %s...\n", fname);
                        MRIwriteImageViews(mri_aligned, fname, IMAGE_SIZE);
                        MRIfree(&mri_aligned);
                      ...
                      */
                      /*
                                printf("M_delta1\n");
                         MatrixPrint(stdout,M_delta1); printf("M_delta2\n");
                         MatrixPrint(stdout,M_delta2); printf("M_delta3\n");
                         MatrixPrint(stdout,M_delta3); printf("M_delta4\n");
                         MatrixPrint(stdout,M_delta4); printf("M_delta5\n");
                         MatrixPrint(stdout,M_delta5); printf("M_delta6\n");
                         MatrixPrint(stdout,M_delta6);
                      */
                      if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) {
                        printf("M_reg_opt\n");
...
static int apply_transform(MRI *mri_in, MRI *mri_target, MATRIX *M_reg,
  ...
  vox_s2vox_t = MatrixMultiply(ras2vox_target, m_tmp, NULL);
  m = MatrixInverse(vox_s2vox_t, nullptr);
  MatrixFree(&vox2ras_source);
  MatrixFree(&ras2vox_source);
  MatrixFree(&vox2ras_target);
  MatrixFree(&ras2vox_target);
  ...
  depth = mri_xformed->depth; // E/ Changing this from mri_target->* was
                              // correct.
      ...
      for (z = 0; z < depth; z++) {
        ...
        default:
          ErrorExit(ERROR_UNSUPPORTED,
                    "apply_transform: unsupported dst type %d",
                    mri_xformed->type);
          break;
        }
      }
...
MATRIX *MRR_VoxelXformToCoronalRasXform(MRI *mri_src, MRI *mri_dst,
                                        MATRIX *M_vox_src2trg,
                                        MATRIX *M_cor_src2trg)
...
// voxel xform.
{
  ...
  D_src_inv = MatrixInverse(D_src, nullptr);
...
MRI *MRR_MRIhybrid(MRI *mri_src, MRI *mri_target, MRI *mri_xformed)
...
{
  if (!mri_xformed)
    mri_xformed = MRIallocSequence(mri_target->width, mri_target->height,
                                   mri_target->depth,
                                   mri_src->type, // E/ !!
                                   mri_target->nframes);
