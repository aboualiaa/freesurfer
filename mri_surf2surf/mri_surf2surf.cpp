...
OPTIONS
  ...
  --tfmt typestring
    ...
    or anything accepted by mri_convert. If no type string  is given, then the
type is determined from the sourcefile (if possible). If using paint, w, or
curv, see also --frame.
  ...
  --surfreg registration_surface
    ...
    to register the two surfaces. sphere.reg is used as the default. Don't
change this unless you know what you are doing.
  ...
  --mapmethod methodname
    ...
    source values are averaged together. It does not seem to make much
difference.

  --fwhm-src fwhmsrc
  --fwhm-trg fwhmtrg (can also use --fwhm)

    Smooth the source or target with a gaussian with the given fwhm (mm). This
is actually an approximation done using iterative nearest neighbor smoothing.
  ...
  --frame framenumber
    ...
    format can store only one frame. The frame number is zero-based (default is
0).
...
MRI *MRIShksmooth(MRIS *Surf, MRI *Src, double sigma, int nSmoothSteps,
                  MRI *Targ);
MRI *MRISheatkernel(MRIS *surf, double sigma);

double MRISareaTriangle(double x0, double y0, double z0, double x1, double y1,
                        double z1, double x2, double y2, double z2);
int MRIStriangleAngles(double x0, double y0, double z0, double x1, double y1,
                       double z1, double x2, double y2, double z2, double *a0,
                       double *a1, double *a2);
...
int MRIScommonNeighbors(MRIS *surf, int vtxno1, int vtxno2, int *cvtxno1,
                        int *cvtxno2);
...
int usediff = 0;
char *LabelFile = nullptr, *LabelFile_Input = nullptr; // for masking smoothing

int debug = 0;
char *SrcDumpFile = nullptr;
char *TrgDumpFile = nullptr;
...
char *RMSDatFile = nullptr;
char *RMSMaskFile = nullptr;
MRI *RMSMask = nullptr;
int SplitFrames = 0;
int ConvGaussian = 0;
...
char *PatchFile = nullptr, *SurfTargName = nullptr;
int nPatchDil = 0;
struct utsname uts;
char *cmdline, cwd[2000];
char *TrgSurfVolFile = nullptr;
MRI *TrgSurfVol = nullptr;
int prunemask = 0;
...
int main(int argc, char **argv) {
  ...
  float *framepower = nullptr, x, y, z;
  char fname[4000];
  ...
  if (!strcmp(srcsubject, "ico")) { /* source is ico */
    ...
    if (!strcmp(srchemi, trghemi) && UseDualHemi == 0) {
      // hemis are the same
      sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, srcsubject, srchemi,
              srcsurfregfile);
    } else {
      // hemis are the different
      if (UseDualHemi)
        sprintf(fname, "%s/%s/surf/%s.%s.%s", SUBJECTS_DIR, srcsubject, srchemi,
                trghemi, srcsurfregfile);
      else
        sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, srcsubject, srchemi,
                srcsurfregfile);
    }
    ...
    if (cavtx > 0)
      printf("cavtx = %d, srcsurfregfile: %g, %g, %g\n", cavtx,
             SrcSurfReg->vertices[cavtx].x, SrcSurfReg->vertices[cavtx].y,
  ...
  for (n = 0; n < SrcSurfReg->nfaces && false; n++) {
    ...
    area = MRISareaTriangle(vtx0->x, vtx0->y, vtx0->z, vtx1->x, vtx1->y,
                            vtx1->z, vtx2->x, vtx2->y, vtx2->z);
    MRIStriangleAngles(vtx0->x, vtx0->y, vtx0->z, vtx1->x, vtx1->y, vtx1->z,
                       vtx2->x, vtx2->y, vtx2->z, &a0, &a1, &a2);
    printf("n=%d, area = %f, %f %f %f   %f\n", n, face->area, a0, a1, a2,
           a0 + a1 + a2);
  }
  ...
  if (!strcmp(srctypestring, "curv")) { /* curvature file */
    ...
    SrcVals = MRIcopyMRIS(nullptr, SrcSurfReg, 0, "curv");
  } else if (!strcmp(srctypestring, "paint") || !strcmp(srctypestring, "w")) {
    MRISreadValues(SrcSurfReg, srcvalfile);
    SrcVals = MRIcopyMRIS(nullptr, SrcSurfReg, 0, "val");
  } else if (UseSurfSrc) {
    if (PatchFile)
      SurfSrcName = srcsurfregfile;
    sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, srcsubject, srchemi,
            SurfSrcName);
    ...
    if (PatchFile) {
      if (fio_FileExistsReadable(PatchFile))
        strcpy(fname, PatchFile);
      else
        sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, srcsubject, srchemi,
                PatchFile);
    ...
    if (UseSurfSrc == SURF_SRC_XYZ || UseSurfSrc == SURF_SRC_TAL_XYZ) {
      if (DoProj) {
        if (ProjType == 2) {
          // Load thickness
          sprintf(tmpstr, "%s/%s/surf/%s.thickness", SUBJECTS_DIR, srcsubject,
                  srchemi);
      ...
      if (UseSurfSrc == SURF_SRC_TAL_XYZ) {
        XFM = DevolveXFM(srcsubject, nullptr, nullptr);
        if (XFM == nullptr) {
          exit(1);
        }
      ...
      SrcVals =
          MRIcopyMRIS(nullptr, SurfSrc, 2, "z"); // start at z to autoalloc
    ...
    if (UseSurfSrc == SURF_SRC_NXYZ) {
      ...
      SrcVals =
          MRIcopyMRIS(nullptr, SurfSrc, 2, "nz"); // start at z to autoalloc
    ...
    if (UseSurfSrc == SURF_SRC_AREA) {
      ...
      SrcVals = MRIcopyMRIS(nullptr, SurfSrc, 0, "area");
    ...
    if (UseSurfSrc == SURF_SRC_RIP)
      SrcVals = MRIcopyMRIS(nullptr, SurfSrc, 0, "ripflag");
    MRISfree(&SurfSrc);
  ...
  if (SynthPDF != 0) {
    ...
    srand48(SynthSeed);
    MRIrandn(SrcVals->width, SrcVals->height, SrcVals->depth, SrcVals->nframes,
             0, 1, SrcVals);
  }
  if (SynthOnes != 0) {
    printf("INFO: filling input with all 1s\n");
    MRIconst(SrcVals->width, SrcVals->height, SrcVals->depth, SrcVals->nframes,
             1, SrcVals);
  }

  if (SrcDumpFile != nullptr) {
  ...
  if (LabelFile_Input != nullptr) {
    printf("Reading source subject label mask %s\n", LabelFile_Input);
    MaskLabel = LabelRead(srcsubject, LabelFile_Input);
    if (MaskLabel == nullptr)
      exit(1);
    inmask = MRISlabel2Mask(SrcSurfReg, MaskLabel, nullptr);
  } else
    inmask = nullptr;
  ...
  if (nSmoothSteps_Input > 0) {
    if (!ConvGaussian) {
      ...
      // MRIShksmooth(SrcSurfReg, SrcVals, gstd_Input, nSmoothSteps_Input,
      // SrcVals);
    }
  } else {
    if (inmask) {
  ...
  if (strcmp(srcsubject, trgsubject) || strcmp(srchemi, trghemi) ||
      ...
      (!strcmp(srcsubject, "ico") && !strcmp(trgsubject, "ico") &&
       SrcIcoOrder != TrgIcoOrder)) {
    ...
    if (!strcmp(trgsubject, "ico")) {
      ...
      // Changed to this on 11/30/97
      sprintf(fname, "%s/%s/surf/%s.%s", SUBJECTS_DIR, trgsubject, trghemi,
              trgsurfregfile);
    ...
    if (UseOldSurf2Surf) {
      ...
      if (!jac) {
        ...
        TrgVals =
            surf2surf_nnfr(SrcVals, SrcSurfReg, TrgSurfReg, &SrcHits, &SrcDist,
                           &TrgHits, &TrgDist, ReverseMapFlag, UseHash);
      } else {
        printf("Mapping Source Volume onto Source Subject Surface "
               "with Jacobian Correction\n");
        TrgVals = surf2surf_nnfr_jac(SrcVals, SrcSurfReg, TrgSurfReg, &SrcHits,
                                     &SrcDist, &TrgHits, &TrgDist,
      ...
      TrgVals =
          MRISapplyReg(SrcVals, SurfRegList, 2, ReverseMapFlag, jac, UseHash);
    }
  ...
  if (LabelFile != nullptr) {
    ...
    if (MaskLabel == nullptr)
      exit(1);
    outmask = MRISlabel2Mask(TrgSurfReg, MaskLabel, nullptr);
  } else
    outmask = nullptr;
  ...
  if (!strcmp(trgtypestring, "paint") || !strcmp(trgtypestring, "w")) {
    ...
    if (RegTarg)
      ...
      getVolGeom(TrgSurfVol, &TrgSurfReg->vg);
    ...
    for (tvtx = 0; tvtx < SurfTrg->nvertices; tvtx++) {
      if (MRIgetVoxVal(TrgVals, tvtx, 0, 0, 0) < 0.000001)
        continue;
    ...
    for (tvtx = 0; tvtx < SurfTrg->nvertices; tvtx++)
      if (SurfTrg->vertices_topology[tvtx].num == 0 ||
          SurfTrg->vertices_topology[tvtx].vnum == 0)
        SurfTrg->vertices[tvtx].ripflag = 1;
    ...
    if (reshape) {
      ...
      mritmp = mri_reshape(TrgVals, TrgVals->width / reshapefactor, 1,
                           reshapefactor, TrgVals->nframes);
      if (mritmp == nullptr) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help")) {
      ...
      nargsused = 1;
               ...
               !strcasecmp(option, "--reg-inv")) {
      if (nargc < 1)
        argnerr(option, 1);
      nargsused = 1;
      if (!stricmp(FileNameExtension(pargv[0], tmp), "LTA")) {
        if (CMDnthIsArg(nargc, pargv, 1)) {
          printf("ERROR: do not include a template volume with --reg or "
                 "--reg-inv when using an LTA file\n");
          exit(1);
        }
        LTA *lta;
        lta = LTAread(pargv[0]);
        ...
        RegTarg =
            MRIallocHeader(lta->xforms[0].src.width, lta->xforms[0].src.height,
                           lta->xforms[0].src.depth, MRI_UCHAR, 1);
        MRIcopyVolGeomToMRI(RegTarg, &lta->xforms[0].src);
        LTAfree(&lta);
        err = 0;
      } else {
        err = regio_read_register(pargv[0], &regsubject, &ipr, &bpr, &intensity,
                                  &XFM, &float2int);
        if (err)
          exit(1);
        if (CMDnthIsArg(nargc, pargv, 1)) {
      ...
      nargsused = 1;
    } else if (!strcmp(option, "--fwhm") || !strcmp(option, "--fwhm-trg")) {
      if (nargc < 1) {
        argnerr(option, 1);
      }
...
static void print_usage() {
  ...
  printf("   --reg-inv register.dat <volgeom> : apply inv(register.dat) to "
         "sval-xyz\n");
  ...
  printf("   --tval-xyz volume: save tval as a surface file with source xyz "
         "(volume for geometry)\n");
  ...
  printf("   --jac  : turn on jacobian correction, needed when applying to "
         "area or volume \n");
  printf(
  ...
  printf("   --no-cortex : do NOT use ?h.cortex.label as a smoothing mask "
         "(default)\n");
...
static void print_help() {
  ...
  printf("icosahedron (ico). The source and target file formats can be "
         "anything\n");
  printf(
      "supported by mri_convert. The source format can also be a curvature\n");
  printf(
  ...
  printf("\n");
  printf("    Name of source subject as found in $SUBJECTS_DIR or ico for "
         "icosahedron.\n");
  printf("    The input data must have been sampled onto this subject's "
         "surface (eg,\n");
  ...
  printf("\n");
  printf("    Use measures from the input surface as the source (instead of "
         "specifying\n");
  printf("    a source file explicitly with --sval). --sval-xyz extracts the "
         "x, y, and\n");
  printf("    z of each vertex. --sval-tal-xyz is the same as --sval-xyz, but "
         "applies\n");
  printf("    the talairach transform from "
         "srcsubject/mri/transforms/talairach.xfm.\n");
  printf("    --sval-area extracts the vertex area. --sval-nxyz extracts the "
         "surface\n");
  ...
  printf("\n");
  printf("    Use xyz from surfname as the input, project it along the normal, "
         "and\n");
  printf("    save new xyz surface. Eg, to create a new surface halfway "
         "between\n");
  printf("    the white and the pial:\n");
  printf("      mri_surf2surf --s subject --projfrac white +0.5 --tval lh.mid "
         "--hemi lh\n");
  printf("    saves $SUBJECTS_DIR/subject/lh.mid.\n");
  printf("\n");
  printf("  --sval-annot annotfile\n");
  printf("\n");
  printf("    Map annotation file to the output. The target data will be saved "
         "as an\n");
  printf("    annotation.\n");
  printf("\n");
  printf("  --sfmt typestring\n");
  printf("\n");
  printf("    Format type string. Can be either curv (for FreeSurfer curvature "
         "file),\n");
  printf(
      "    paint or w (for FreeSurfer paint files), or anything accepted by\n");
  printf("    mri_convert. If no type string  is given, then the type is "
         "determined\n");
  printf("    from the sourcefile (if possible). If curv is used, then the "
         "curvature\n");
  printf("    file will be looked for in "
         "$SUBJECTS_DIR/srcsubject/surf/hemi.sourcefile.\n");
  printf("\n");
  printf("  --srcicoorder order\n");
  printf("\n");
  printf("    Icosahedron order of the source. Normally, this can be detected "
         "based\n");
  printf("    on the number of verticies, but this will fail with a .w file as "
         "input.\n");
  printf("    This is only needed when the source is a .w file.\n");
  printf("\n");
  printf("  --trgsubject subjectname\n");
  printf("\n");
  printf("    Name of target subject as found in $SUBJECTS_DIR or ico for "
         "icosahedron.\n");
  printf("\n");
  ...
  printf("\n");
  printf("    Name of file where the data on the target surface will be "
         "stored.\n");
  ...
  printf(
      "    into the surface file. This is a good way to map the surface of\n");
  printf("    one subject to an average (talairach) subject. Note: it will "
         "save\n");
  printf(
      "    targetfile as trgsubject/surf/targetfile unless targetfile has a\n");
  printf("    path.\n");
  ...
  printf("\n");
  printf("    Format type string. Can be paint or w (for FreeSurfer paint "
         "files) or curv\n");
  printf("    or anything accepted by mri_convert. If no type string  is "
         "given, then the type\n");
  printf("    is determined from the sourcefile (if possible). If using paint, "
         "w, or curv,\n");
  ...
  printf("\n");
  printf("    If the source and target subjects are not the same, this surface "
         "is used\n");
  printf("    to register the two surfaces. sphere.reg is used as the default. "
         "Don't change\n");
  printf("    this unless you know what you are doing.\n");
  printf("\n");
  printf("  --mapmethod methodname\n");
  printf("\n");
  printf("    Method used to map from the vertices in one subject to those of "
         "another.\n");
  printf("    Legal values are: nnfr (neighest-neighbor, forward and reverse) "
         "and nnf\n");
  printf("    (neighest-neighbor, forward only). Default is nnfr. The mapping "
         "is done\n");
  printf("    in the following way. For each vertex on the target surface, the "
         "closest\n");
  printf("    vertex in the source surface is found, based on the distance in "
         "the\n");
  printf("    registration space (this is the forward map). If nnf is chosen, "
         "then the\n");
  printf("    the value at the target vertex is set to that of the closest "
         "source vertex.\n");
  printf("    This, however, can leave some source vertices unrepresented in "
         "target (ie,\n");
  printf("    'holes'). If nnfr is chosen, then each hole is assigned to the "
         "closest\n");
  printf("    target vertex. If a target vertex has multiple source vertices, "
         "then the\n");
  printf("    source values are averaged together. It does not seem to make "
         "much difference.\n");
  printf("\n");
  printf("  --fwhm-src fwhmsrc\n");
  printf("  --fwhm-trg fwhmtrg (can also use --fwhm)\n");
  printf("\n");
  printf("    Smooth the source or target with a gaussian with the given fwhm "
         "(mm). This is\n");
  printf("    actually an approximation done using iterative nearest neighbor "
         "smoothing.\n");
  printf("    The number of iterations is computed based on the white surface. "
         "This\n");
  printf("    method is similar to heat kernel smoothing. This will give the "
         "same\n");
  printf("    results as --nsmooth-{in,out}, but automatically computes the "
         "the\n");
  ...
  printf("\n");
  printf("    Number of smoothing iterations. Each iteration consists of "
         "averaging each\n");
  printf("    vertex with its neighbors. When only smoothing is desired, just "
         "set the\n");
  printf("    the source and target subjects to the same subject. --smooth-in "
         "smooths\n");
  printf("    the input surface values prior to any resampling. --smooth-out "
         "smooths\n");
  ...
  printf("  --no-cortex\n");
  printf("\n");
  printf("    Only smooth within the given label. If --cortex is specified, "
         "then\n");
  printf("    ?h.cortex.label will be used (this is created by automatically "
         "be\n");
  printf(
      "    recon-all). Even if you do not have a label of interest, it is\n");
  printf("    recommended that you only smooth within cortex in order to "
         "prevent\n");
  ...
  printf("    default at which point you will have to use --no-cortex to turn "
         "it\n");
  printf("    off.  This documentation will reflect the change. For "
         "--label-src\n");
  printf(
      "    and --label-trg, if you do not give it a full path, it will look\n");
  printf("    in the subjects label dir. There is no need to specify both "
         "source\n");
  printf(
      "    and target unless you are smoothing on both (which you probably\n");
  ...
  printf("\n");
  printf("    When using paint/w output format, this specifies which frame to "
         "output. This\n");
  printf("    format can store only one frame. The frame number is zero-based "
         "(default is 0).\n");
  printf("\n");
  printf("  --mul Mul\n");
  printf("  --div Div\n");
  ...
  printf(
      "    icosahedrons, R=6. For others, R will be the prime factor of Nv\n");
  printf("    closest to 6 (can be changed with --reshape-factor). Reshaping "
         "is\n");
  printf("    for logistical purposes (eg, in the analyze/nifti format the "
         "size\n");
  printf("    of a dimension cannot exceed 2^15). Use this flag to prevent "
         "this\n");
  printf("    behavior. This has no effect when the output type is paint. At "
         "one\n");
  printf("    point, it was the default to reshape.\n");
  printf("\n");
  printf("  --reshape-factor Nfactor\n");
  printf("\n");
  printf("    Attempt to reshape to Nfactor 'slices' (will choose closest "
         "prime\n");
  ...
  printf("EXAMPLES:\n");
  printf("\n");
  printf("1. Resample a subject's thickness of the left cortical hemisphere on "
         "to a\n");
  ...
  printf("\n");
  printf("2. Resample data on the icosahedron to the right hemisphere of "
         "subject bert.\n");
  printf("   Note that both the source and target data are stored in mgh "
         "format\n");
  printf("   as 'volume-encoded suface' data.\n");
  printf("\n");
  printf("   mri_surf2surf --hemi rh --srcsubject ico --srcsurfval "
         "icodata-rh.mgh\n");
  printf("      --trgsubject bert --trgsurfval ./bert-ico-rh.mgh\n");
  printf("\n");
  printf(
      "3. Convert the surface coordinates of the lh.white of a subject to a\n");
  printf("   (talairach) average (ie, a subject created by "
  ...
  printf("      --sval-xyz white --tval-xyz template.nii.gz --tval "
         "./lh.white.func \n");
  printf("      --s yoursubject\n");
  printf("\n");
  printf("   This will create lh.white.func in the current directory. "
         "template.nii.gz\n");
  printf("   is a volume in the functional space. register.dat is the "
         "registration\n");
  ...
  printf(
      "   View result with:  freeview -v template.nii.gz -f lh.white.func\n");
  printf("\n");
  printf("   When using an LTA instead of a register.dat, do not include a "
         "target volume\n");
  printf("\n");
  ...
  printf(
      "   This will create $SUBJECTS_DIR/subj2/label/lh.subj1.aparc.annot.\n");
  printf("   The --sval-annot flag will also change the map method to nnf so "
         "that\n");
  printf("   the annot indices are not averaged. Note: this is not a "
         "substitute\n");
  printf(
      "   for running the cortical parcellation! The parcellations that it\n");
  printf(
      "   maps to the new subject may not be appropriate for that subject.\n");
  printf("\n");
  printf("BUG REPORTS: send bugs to analysis-bugs@nmr.mgh.harvard.edu. Make "
         "sure\n");
  printf("    to include the version and full command-line and enough "
         "information to\n");
  printf("    be able to recreate the problem. Not that anyone does.\n");
  printf("\n");
  printf("BUGS:\n");
  printf("\n");
  printf("  When the output format is paint, the output file must be specified "
         "with\n");
  printf("  a partial path (eg, ./data-lh.w) or else the output will be "
         "written into\n");
...
/* --------------------------------------------- */
static void check_options() {
  if (srcsubject == nullptr) {
    fprintf(stdout, "ERROR: no source subject specified\n");
    exit(1);
  }
  if (srcvalfile == nullptr && UseSurfSrc == 0 && PatchFile == nullptr) {
    fprintf(stdout, "A source value path must be supplied\n");
    exit(1);
  }

  if (UseSurfSrc == 0 && PatchFile == nullptr) {
...
double MRISdiffusionEdgeWeight(MRIS *surf, int vtxno0, int vtxnonbr) {
  ...
  MRIStriangleAngles(cv1->x, cv1->y, cv1->z, v0->x, v0->y, v0->z, vnbr->x,
                     vnbr->y, vnbr->z, &a0, &btmp, &ctmp);
  MRIStriangleAngles(cv2->x, cv2->y, cv2->z, v0->x, v0->y, v0->z, vnbr->x,
                     vnbr->y, vnbr->z, &a1, &btmp, &ctmp);
  w = 1 / tan(a0) + 1 / tan(a1);
  return (w);
}
/*-------------------------------------------------------------------------
...
int MRISdumpVertexNeighborhood(MRIS *surf, int vtxno) {
  ...
  for (nthnbr = 0; nthnbr < nnbrs; nthnbr++) {
    ...
    printf("   nbr vtx %5d v%d = [%6.3f %6.3f %6.3f]    ", nbrvtxno, nthnbr,
           v->x, v->y, v->z);
...
int DumpSurface(MRIS *surf, char *outfile) {
  ...
  for (vtxno = 0; vtxno < surf->nvertices; vtxno++) {
    ...
    VERTEX_TOPOLOGY const *const cvtxt = &surf->vertices_topology[vtxno];
    VERTEX const *const cvtx = &surf->vertices[vtxno];
    fprintf(fp, "%6d   %8.3f %8.3f %8.3f   %2d   ", vtxno + 1, cvtx->x, cvtx->y,
            cvtx->z, nnbrs);
