...
int main(int argc, char *argv[]) {
  ...
  if (!gca)
    ErrorExit(ERROR_NOFILE, "%s: could not read gca file %s", Progname,
              gca_name);
  ...
  case 1: {
    ...
    for (fa1 = MIN_FA1; fa1 <= MAX_FA1; fa1 += FA_STEP) {
      ...
      gca_flash =
          GCAcreateFlashGCAfromParameterGCA(gca, TRs, FAs, TEs, 1, lambda);
    ...
    gca_flash =
        GCAcreateFlashGCAfromParameterGCA(gca, TRs, FAs, TEs, 1, lambda);
    mri = GCAcomputeAmbiguity(gca_flash, nullptr, &amb, classnum);
  ...
  case 2: {
    ...
    for (fa1 = MIN_FA1; fa1 <= MAX_FA1; fa1 += FA_STEP) {
      ...
      for (fa2 = MIN_FA2; fa2 <= MAX_FA2; fa2 += FA_STEP) {
        ...
        FAs[1] = RADIANS(fa2);
        gca_flash =
            GCAcreateFlashGCAfromParameterGCA(gca, TRs, FAs, TEs, 2, lambda);
        /*    GCAscale(gca_flash, 0, 75) ;*/
        mri = GCAcomputeAmbiguity(gca_flash, mri, &amb, classnum);
        amb *= scale;
    ...
    for (fa1 = MIN_FA1; fa1 <= MAX_FA1; fa1 += FA_STEP) {
      ...
      for (fa2 = MIN_FA2; fa2 <= MAX_FA2; fa2 += FA_STEP) {
        ...
        for (fa3 = MIN_FA3; fa3 <= MAX_FA3; fa3 += FA_STEP) {
          ...
          gca_flash =
              GCAcreateFlashGCAfromParameterGCA(gca, TRs, FAs, TEs, 3, lambda);
          /*    GCAscale(gca_flash, 0, 75) ;*/
          mri = GCAcomputeAmbiguity(gca_flash, mri, &amb, classnum);
          amb *= scale;
...
static void print_usage() {
  fprintf(stderr, "usage: %s [options] <gca file> <output volume>\n", Progname);
}

static void print_help() {
  print_usage();
  fprintf(stderr, "\nThis program computes an ambiguity measure across a  GCA "
                  "and output an MR image of it\n");
  exit(1);
}
...
static MRI *GCAcomputeAmbiguity(GCA *gca, MRI *mri, double *pamb,
                                int classnum) {
  int xp, yp, zp, l1, l2, i1, i2, i,
      label_count[MAX_CMA_LABEL + 1][MAX_CMA_LABEL + 1],
      label1_count[MAX_CMA_LABEL + 1];
  GCA_PRIOR *gcap;
...
#define ISTEPS 100
              ...
              if (gca->ninputs == 1) {
                ...
                scale1 =
                    scale *
                    (1.0 / (pow(2 * M_PI, gca->ninputs / 2.0) *
                            sqrt(covariance_determinant(gc1, gca->ninputs))));
                scale2 =
                    scale *
                    (1.0 / (pow(2 * M_PI, gca->ninputs / 2.0) *
                            sqrt(covariance_determinant(gc2, gca->ninputs))));
...
#define ISTEPS 50
                ...
                for (ambiguity = 0, I1 = gc1->means[0] - (ISTEPS / 2) * Istep1;
                     I1 <= gc1->means[0] + (ISTEPS / 2) * Istep1;
                     I1 += Istep1) {
                  VECTOR_ELT(v1, 1) = I1 - gc1->means[0];
                  VECTOR_ELT(v2, 1) = I1 - gc2->means[0];
                  for (amb = 0.0, I2 = gc1->means[1] - (ISTEPS / 2) * Istep2;
                       I2 <= gc1->means[1] + (ISTEPS / 2) * Istep2;
                       I2 += Istep2) {
                ...
                for (I1 = gc2->means[0] - (ISTEPS / 2) * Istep1;
                     I1 <= gc2->means[0] + (ISTEPS / 2) * Istep1;
                     I1 += Istep1) {
                  VECTOR_ELT(v1, 1) = I1 - gc1->means[0];
                  VECTOR_ELT(v2, 1) = I1 - gc2->means[0];
                  for (amb = 0.0, I2 = gc2->means[1] - (ISTEPS / 2) * Istep2;
                       I2 <= gc2->means[1] + (ISTEPS / 2) * Istep2;
                       I2 += Istep2) {
                    VECTOR_ELT(v1, 2) = I2 - gc1->means[1];
                    MatrixMultiply(
                        m1_cov_inv, v1,
                ...
                ErrorExit(ERROR_UNSUPPORTED,
                          "%s: ambiguity  only supported  for  ninputs=1 or 2",
                          Progname);
...
static MRI *GCAcompute1DAmbiguity(GCA *gca, MRI *mri, double *pamb,
                                  int classnum) {
  int xp, yp, zp, l1, l2, i1, i2,
      label_count[MAX_CMA_LABEL + 1][MAX_CMA_LABEL + 1],
      label1_count[MAX_CMA_LABEL + 1], index1, index1_c1, index1_c2, n, ntotal;
  GCA_PRIOR *gcap;
            ...
            for (i2 = 0; i2 < i1; i2++) {
              ...
              for (ambiguity = 0, I1 = Imin1; I1 < Imax1; I1 += Istep1) {
                ...
                if (index1_c1 < 0 || index1_c1 > MAX_VAL)
                  continue;
                index1_c2 = nint(MAX_VAL * (I1 - Imins1[i2]) /
                                 (Imaxs1[i2] - Imins1[i2]));
...
static MRI *GCAcompute2DAmbiguity(GCA *gca, MRI *mri, double *pamb,
  ...
  int xp, yp, zp, l1, l2, i1, i2,
      label_count[MAX_CMA_LABEL + 1][MAX_CMA_LABEL + 1],
      label1_count[MAX_CMA_LABEL + 1], index1, index2, index1_c1, index1_c2,
      index2_c1, index2_c2, n, ntotal;
  GCA_PRIOR *gcap;
  double ambiguity, atotal, p1, p2, amax, total_ambiguity, Imin1, Imin2, Imax1,
            ...
            for (i2 = 0; i2 < i1; i2++) {
              ...
              for (ambiguity = 0, I1 = Imin1; I1 < Imax1; I1 += Istep1) {
                ...
                for (amb = 0.0, I2 = Imin2; I2 < Imax2; I2 += Istep2) {
                  index2_c1 = nint(MAX_VAL * (I2 - Imins2[i1]) /
                                   (Imaxs2[i1] - Imins2[i1]));
                  if (index2_c1 < 0 || index2_c1 > MAX_VAL)
                    continue;
                  index2_c2 = nint(MAX_VAL * (I2 - Imins2[i2]) /
                                   (Imaxs2[i2] - Imins2[i2]));
...
static MRI *GCAcompute3DAmbiguity(GCA *gca, MRI *mri, double *pamb,
  ...
  int xp, yp, zp, l1, l2, i1, i2,
      label_count[MAX_CMA_LABEL + 1][MAX_CMA_LABEL + 1],
      label1_count[MAX_CMA_LABEL + 1], index1, index2, index3, index1_c1,
      index1_c2, index2_c1, index2_c2, index3_c1, index3_c2, n, ntotal;
  GCA_PRIOR *gcap;
  double ambiguity, atotal, p1, p2, amax, total_ambiguity, Imin1, Imin2, Imin3,
      Imax1, Imax2, Imax3, pscale, dsq, amb, std1, std2, std3,

      Istep1, Istep2, Istep3, I1, I2, I3, Imins1[MAX_LABELS_PER_GCAN],
      ...
      for (zp = 0; zp < gca->prior_depth; zp++) {
        ...
        for (i1 = 0; i1 < gcap->nlabels; i1++) {
          ...
          for (index1 = 0, I1 = Imin1; I1 <= Imax1; I1 += Istep1, index1++) {
            ...
            for (index2 = 0, I2 = Imin2; I2 <= Imax2; I2 += Istep2, index2++) {
              VECTOR_ELT(v, 2) = I2 - gc1->means[1];
              for (index3 = 0, I3 = Imin3; I3 <= Imax3;
                   I3 += Istep3, index3++) {
              ...
              for (I2 = Imin2; I2 < Imax2; I2 += Istep2) {
                ...
                for (I3 = Imin3; I3 < Imax3; I3 += Istep3) {
                  ...
                  fprintf(fp, "%d %d %d %f %f %f %f\n", index1, index2, index3,
                          I1, I2, I3, pI_c[i1][index1][index2][index3]);
                }
              }
            }
            fclose(fp);
              ...
              for (ambiguity = 0, I1 = Imin1; I1 < Imax1; I1 += Istep1) {
                ...
                for (I2 = Imin2; I2 < Imax2; I2 += Istep2) {
                  ...
                  for (amb = 0.0, I3 = Imin3; I3 < Imax3; I3 += Istep3) {
                    index3_c1 = nint(MAX_VAL * (I3 - Imins3[i1]) /
                                     (Imaxs3[i1] - Imins3[i1]));
                    if (index3_c1 < 0 || index3_c1 > MAX_VAL)
                      continue;
                    index3_c2 = nint(MAX_VAL * (I3 - Imins3[i2]) /
                                     (Imaxs3[i2] - Imins3[i2]));
