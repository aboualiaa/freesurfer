...
static MRI *mri_segs[MAX_SUBJECTS];
static TRANSFORM *transforms[MAX_SUBJECTS];
...
int main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option(
      argc, argv, "$Id: mri_rf_train.c,v 1.2 2012/05/24 00:04:15 fischl Exp $",
      "$Name:  $");
  ...
  // rfa_inputs can be T1, PD, ...per subject
  if (parms.nvols == 0)
    parms.nvols = ninputs;
  ...
  for (nargs = i = 0; i < nsubjects + options; i++) {
      ...
      if (counts != ninputs) {
        for (o = 0; o < ninputs; o++)
          printf("previous input %d TR/TE/flip = %2.2f/%2.2f/%2.1f\n", o,
                 TRs[o], TEs[o], DEGREES(FAs[o]));
        printf("subject %s TR/TE/flip = %2.2f/%2.2f/%2.1f\n", subject_name,
               mri_tmp->tr, mri_tmp->te, DEGREES(mri_tmp->flip_angle));
    ...
    for (input = 0; input < ninputs; input++) {
      ...
      if (i == 0) {
        ...
        ErrorExit(ERROR_BADPARM,
                  ...
                  Progname, subject_name, fname, mri_tmp->tr,
                  DEGREES(mri_tmp->flip_angle), mri_tmp->te, TRs[input],
                  DEGREES(FAs[input]), TEs[input]);
      // first time do the following
      if (input == 0) {
        int nframes = ninputs;

        ///////////////////////////////////////////////////////////
        mri_in = MRIallocSequence(mri_tmp->width, mri_tmp->height,
                                  mri_tmp->depth, mri_tmp->type, nframes);
        if (!mri_in)
    ...
    if (do_sanity_check) {
      ...
      if (errs) {
        printf("ERROR: mri_ca_train: possible bad training data! subject:\n"
               "\t%s/%s\n\n",
    ...
    // RFAaddInput will hang onto these pointers, so don't free them
    if (single_classifier_flag) {
      mri_inputs[i] = mri_in;
      mri_segs[i] = mri_seg;
      transforms[i] = transform;
    } else
      RFAaddInput(rfa, mri_seg, mri_in, transform, &parms);
  }
  if (single_classifier_flag) {
    if (only_nbrs)
      rf = train_rforest_with_wmsa_nbrs(mri_inputs, mri_segs, transforms,
                                        nsubjects, gca, &parms);
    else
      rf = train_rforest(mri_inputs, mri_segs, transforms, nsubjects, gca,
                         &parms, wm_thresh, wmsa_whalf);
    ...
    RFAcompleteTraining(rfa, &parms);
    printf("writing random forest array to %s\n", out_fname);
    if (RFAwrite(rfa, out_fname) != NO_ERROR)
      ErrorExit(ERROR_BADFILE, "%s: could not write rfa to %s", Progname,
                out_fname);
  }
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "GRADIENT")) {
    ...
    only_nbrs = 1;
    printf(
        "training RF using voxels that neighbor both a wmsa and a non-wmsa\n");
  } else
    ...
    case 'F':
      force_inputs = 1;
      printf(
          "forcing use of inputs even if acquisition parameters don't match\n");
      break;
    case 'S':
      scale = atof(argv[2]);
...
static void usage_exit(int code) {
  ...
  printf("where SUBJECTS_DIR env variable must be set.\n"
         ...
         "\t-check          - conduct sanity-check of labels for obvious edit "
         "errors"
         "\n");
  exit(code);
}

static int input_labels[] = {Left_Cerebral_Exterior, Right_Cerebral_Exterior,
                             Left_Cerebellum_Exterior,
                             Right_Cerebellum_Exterior};
// replace the values above with the following
static int output_labels[] = {Left_Cerebral_Cortex, Right_Cerebral_Cortex,
    ...
    for (y = 0; y < mri_seg->height; y++) {
      for (z = 0; z < mri_seg->depth; z++) {
        ...
        if (label != Unknown) {
          ...
          // labelled as Unknown.
          if ((x < 6) || (y < 6) || (z < 6) || (x > 249) || (y > 249) ||
              (z > 249)) {
            printf("ERROR: %s: found label outside of brain: "
                   "%d %d %d\n",
        ...
        if ((label == Left_Hippocampus) || (label == Right_Hippocampus) ||
          ...
          transform_point(mri_seg->linear_transform, xw, yw, zw, &xmt, &ymt,
                          &zmt);                   // get mni tal coords
          ...
          case Left_Inf_Lat_Vent:
            if (xt > 5) {
              ...
              if (label == Left_Hippocampus)
                proper_label = Right_Hippocampus;
              else if (label == Left_Caudate)
                proper_label = Right_Caudate;
              else if (label == Left_Amygdala)
                proper_label = Right_Amygdala;
              else if (label == Left_Putamen)
                proper_label = Right_Putamen;
              else if (label == Left_Pallidum)
                proper_label = Right_Pallidum;
          ...
          case Right_Inf_Lat_Vent:
            if (xt < -5) {
              ...
              if (label == Right_Hippocampus)
                proper_label = Left_Hippocampus;
              else if (label == Right_Caudate)
                proper_label = Left_Caudate;
              else if (label == Right_Amygdala)
                proper_label = Left_Amygdala;
              else if (label == Right_Putamen)
                proper_label = Left_Putamen;
              else if (label == Right_Pallidum)
                proper_label = Left_Pallidum;
          ...
          switch (label) {
          case Left_Hippocampus:
            if (xt > max_xtal_l_hippo)
              max_xtal_l_hippo = xt;
            break;
          case Left_Caudate:
            if (xt > max_xtal_l_caudate)
              max_xtal_l_caudate = xt;
            break;
          case Left_Amygdala:
            if (xt > max_xtal_l_amygdala)
              max_xtal_l_amygdala = xt;
            break;
          case Left_Putamen:
            if (xt > max_xtal_l_putamen)
              max_xtal_l_putamen = xt;
            break;
          case Left_Pallidum:
            if (xt > max_xtal_l_pallidum)
              max_xtal_l_pallidum = xt;
            break;

          case Right_Hippocampus:
            if (xt < min_xtal_r_hippo)
              min_xtal_r_hippo = xt;
            break;
          case Right_Caudate:
            if (xt < min_xtal_r_caudate)
              min_xtal_r_caudate = xt;
            break;
          case Right_Amygdala:
            if (xt < min_xtal_r_amygdala)
              min_xtal_r_amygdala = xt;
            break;
          case Right_Putamen:
            if (xt < min_xtal_r_putamen)
              min_xtal_r_putamen = xt;
            break;
          case Right_Pallidum:
            if (xt < min_xtal_r_pallidum)
              min_xtal_r_pallidum = xt;
            break;
...
static RANDOM_FOREST *train_rforest(MRI **mri_inputs, MRI **mri_segs,
  ...
  printf("initial training set found with %dK FUTURE WMSA labels, %dK WMSA "
         "labels, and %dK non (ratio=%2.1f:%2.1f)\n",
         nfuture / 1000, nwmsa / 1000, nnot / 1000,
         (float)nnot / (float)nfuture, (float)nnot / (float)nwmsa);

  MRIfree(&mri_wmsa_possible);

  if (max_wm_wmsa_ratio * (nfuture + nwmsa) <
      nnot) // too many wm labels w.r.t. # of wmsas - remove some wm
  {
    int removed,
        total_to_remove = nnot - (max_wm_wmsa_ratio * (nfuture + nwmsa));
    double premove;

    premove = (double)total_to_remove / (double)nnot;
    printf(
        "removing %dK WM indices to reduce training set imbalance (p < %f)\n",
          ...
          for (z = 0; z < mri_in->depth; z++) {
            ...
            if (label == 1) // a WM voxel
            {
              if (randomNumber(0, 1) < premove) {
                removed++;
                MRIsetVoxVal(mri_training_voxels, x, y, z, n,
                             0); // remove it from training set
              }
            }
          }
    ntraining -= removed;
    printf(
        "%d WM voxels removed, new training set size = %dM (ratio = %2.1f)\n",
        removed, ntraining / (1024 * 1024),
        (double)(nnot - removed) / (double)(nwmsa + nfuture));
  }
  ...
  printf("training random forest with %dK FUTURE WMSA labels, %dK WMSA labels, "
         "and %dK non (ratio=%2.1f:%2.1f)\n",
         nfuture / 1000, nwmsa / 1000, nnot / 1000,
         (float)nnot / (float)nfuture, (float)nnot / (float)nwmsa);
  RFtrain(rf, parms->feature_fraction, parms->training_fraction,
          training_classes, training_data, ntraining);
  correct =
      RFcomputeOutOfBagCorrect(rf, training_classes, training_data, ntraining);
  ...
  if (log_file_name) {
    ...
    sprintf(line, "%f %d %d %f\n", rf->training_fraction, rf->max_depth,
            rf->ntrees, 100.0 * correct / ntraining);
  ...
  for (i = 0; i < ntraining;
       i++) // allow for augmenting with other wmsa examples
...
static int is_wmsa_border(MRI *mri_seg, int x, int y, int z) {
...
static RANDOM_FOREST *train_rforest_with_wmsa_nbrs(MRI **mri_inputs,
  ...
  if (max_wm_wmsa_ratio * (nfuture + nwmsa) <
      nnot) // too many wm labels w.r.t. # of wmsas - remove some wm
  {
    int total_to_remove = nnot - (max_wm_wmsa_ratio * (nfuture + nwmsa)),
        *random_indices;
    ...
    for (ignored = i = 0; i < ntraining && ignored < total_to_remove; i++) {
      ...
      {
        ignored++;
        ignore_indices[random_indices[i]] = 1;
      }
    }
    free(random_indices);
  ...
  for (nnot = nwmsa = nfuture = i = n = 0; n < nsubjects; n++) {
    mri_in = mri_inputs[n];
    mri_seg = mri_segs[n];
    transform = transforms[n];
    for (x = 0; x < mri_in->width; x++)
        ...
        for (z = 0; z < mri_in->depth; z++) {
          ...
          if (training_data[i] == nullptr)
            ErrorExit(
                ...
                "train_rforest: could not allocate %d-len feature vector #%d",
                nfeatures, i);
          training_classes[i] = training_classes[i + ignored];
          extract_feature(mri_in, parms->wsize, x, y, z, training_data[i], 0, 0,
                          0);
