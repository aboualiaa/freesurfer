...
int main(int argc, char *argv[]) {
  ...
  if (patch_flag) /* read in orig surface before reading in patch */
  {
    ...
    if (MRISreadPatch(mris, in_fname) != NO_ERROR)
      ErrorExit(ERROR_NOFILE, "%s: could not read patch file %s", Progname,
                in_fname);
  ...
  if (talxfmsubject) {
    XFM = DevolveXFM(talxfmsubject, nullptr, nullptr);
    if (XFM == nullptr) {
      exit(1);
    }
  ...
  if (curv_file_flag) {
    ...
    if (type == MRIS_ANNOT_FILE) {
      ...
      if (nullptr == mrisReadGIFTIdanum(annot_fname, mris, gifti_da_num)) {
        exit(1);
      }
    } else {
    ...
    if (parcstats_file_flag) {
      FILE *fp;
      if ((fp = fopen(parcstats_fname, "r")) == nullptr) {
        errno = 0;
    ...
    if (type == MRIS_ANNOT_FILE) {
      ...
      if (MRISwriteGIFTI(mris, NIFTI_INTENT_LABEL, out_fname, nullptr) !=
          NO_ERROR) {
        exit(1);
      }
    } else {
    ...
    LABEL *label = LabelRead(nullptr, label_fname);
    if (nullptr == label) {
    ...
    if (ct0) {
      ...
      for (cno = 0; cno < ct0->nentries; cno++) {
        if ((ct0->entries[cno]) &&
            (strcmp(label_name, ct0->entries[cno]->name) == 0)) {
          // we found this label! so update local colortable with info
          memcpy(miniColorTable.entries[1], ct0->entries[cno],
    ...
    mris->ct =
        nullptr; // to avoid calling CTABfree (our table is static memory)
    ...
    if (labelstats_file_flag) {
      ...
      if (type == MRIS_GIFTI_FILE) {
        if (MRISwriteGIFTI(mris, NIFTI_INTENT_UNIFORM, labelstats_fname,
                           nullptr) != NO_ERROR) {
          exit(1);
        }
      } else {
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "-help")) {
    ...
    if (surf == nullptr)
      exit(1);
    LABEL *srclabel = LabelRead(nullptr, argv[3]);
    if (srclabel == nullptr)
      exit(1);
    MRI *outmask = MRISlabel2Mask(surf, srclabel, nullptr);
    if (outmask == nullptr)
      exit(1);
    ...
    if (surf == nullptr)
      exit(1);
    MRI *SrcVals = MRIcopyMRIS(nullptr, surf, 0, "area");
...
static void print_help() {
  ...
  printf("     column is the vertex number, the 2nd col is the number of "
         "neighbors,\n");
  printf(
      "     the remaining cols are the vertex numbers of the neighbors.  \n");
  printf("     Note: there can be a different number of neighbors for each "
         "vertex.\n");
  ...
  printf("  --to-scanner : convert coordinates from native FS (tkr) coords to "
         "scanner coords\n");
  printf("  --to-tkr : convert coordinates from scanner coords to native FS "
         "(tkr) coords \n");
  printf("  --volume ?h.white ?h.pial ?h.volume : compute vertex-wise volume, "
         "no other args needed (uses th3)\n");
  printf("  --area surface area.mgz : compute vertex-wise area (no other args "
         "needed); rescales group if needed\n");
  printf(
      "  --angle surface angles.mgz : compute cortical orientation angles\n");
  printf("  --label2mask surface label mask.mgz : convert a surface-based "
         "label to a binary mask (no other args needed)\n");
  printf("  Note: --cras_add and --cras_subtract are depricated. They are "
         "included for backwards compatability\n");
  printf("    Use --to-tkr and --to-scanner instead\n");
  printf("  --cras_add : shift center to scanner coordinate center (was "
         "--cras_correction, which still works)\n");
  printf("  --cras_subtract : shift center from scanner coordinate center "
         "(reverses --cras_add)\n");
  printf("\n");
...
static int convertToWFile(char *in_fname, char *out_fname) {
  ...
  while ((cp = fgetl(line, 299, infp)) != nullptr) {
    l++;
...
MRI *computeAngles(MRIS *surf) {
  ...
  printf("************************************* CAUTION "
         "**************************************\n");
  printf("If you are running \"mris_convert --angle\" on an original surface "
         "(i.e. has not\n");
  printf("been transformed by mris_surf2surf), then you can ignore this "
         "message.\n");
  printf("However, if you need to calculate orientation angles of a "
         "TRANSFOMRED surface you \n");
  printf("will need to convert the .lta/.dat registration file first and force "
         "the original \n");
  printf("volume as the source. Otherwise important header information will be "
         "lost and the \n");
  printf("angles will be incorrect (do not be fooled, they might appear close, "
         "but they won't\n");
  printf("be exact). A detailed description and example can be found on the "
         "freesurfer wiki.\n");
  printf("*********************************************************************"
         "***************\n");
  ...
  MATRIX *vox2ras =
      vg_i_to_r(&surf->vg); // scanner space vox2ras from volume geometry
  MATRIX *ras2vox =
      MatrixInverse(vox2ras, nullptr); // inverse from imaging reference frame
                                       // to scanner coordinate system

  // imaging volume axes
  double vi_x[3] = {ras2vox->rptr[1][1], ras2vox->rptr[2][1],
                    ras2vox->rptr[3][1]};
  double vi_y[3] = {ras2vox->rptr[1][2], ras2vox->rptr[2][2],
                    ras2vox->rptr[3][2]};
  double vi_z[3] = {ras2vox->rptr[1][3], ras2vox->rptr[2][3],
                    ras2vox->rptr[3][3]};
  ...
  // the angle between normal and volume axes is simply the acos of the normal's
  // coordinate, the angle to the scanner axes is the acos of the dot product
  // between the normal and the scanner directions. Store all angles in one
  // overlay file. The first three will be w.r.t. B0, the other w.r.t. the
  // scanner volume
  MRI *angles = MRIallocSequence(surf->nvertices, 1, 1, MRI_FLOAT, 6);

  for (int vno = 0; vno < surf->nvertices; vno++) {
    VERTEX *v = &surf->vertices[vno];
    MRIsetVoxVal(angles, vno, 0, 0, 0,
                 acos((vi_x[0] * v->nx + vi_x[1] * v->ny + vi_x[2] * v->nz)) *
                     180 / PI);
    MRIsetVoxVal(angles, vno, 0, 0, 1,
                 acos((vi_y[0] * v->nx + vi_y[1] * v->ny + vi_y[2] * v->nz)) *
                     180 / PI);
    MRIsetVoxVal(angles, vno, 0, 0, 2,
                 acos((vi_z[0] * v->nx + vi_z[1] * v->ny + vi_z[2] * v->nz)) *
                     180 / PI);
