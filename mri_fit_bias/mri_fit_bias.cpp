...
char * srcfile = nullptr, *outfile = nullptr, *biasfile = nullptr;
char * maskfile = nullptr, *segfile = nullptr;
MRI *  src, *seg, *mask, *out;
int    nthreads    = 0;
double lpfcutoffmm = 23;
char * Xfile = nullptr, *yfile = nullptr, *dctfile = nullptr,
     *betafile = nullptr;
char *yhatfile = nullptr, *resfile = nullptr;
char *SUBJECTS_DIR;
...
DCTFS * DCTalloc();
MRI *   MRIapplyBiasField(MRI *input, MRI *bf, MRI *seg, MRI *mask,
                          double targwmval, MRI *bc);
...
int main(int argc, char *argv[]) {
  ...
  MRI *bc = MRIapplyBiasField(bf->input, bf->biasfield, bf->segerode,
                              bf->ubermask, 110, nullptr);
  ...
  if (Xfile)
    MatrixWriteTxt(Xfile, bf->X);
  if (yfile)
    MatrixWriteTxt(yfile, bf->y);
  if (betafile)
    MatrixWriteTxt(yfile, bf->beta);
...
static void print_usage() {
  ...
  printf("   --seg segvol : segmentation to define WM and Cortex (eg, "
         "aseg.presurf.mgz)\n");
  printf("   --mask maskvol : zero everthing outside of mask "
         "(eg,brainmask.mgz)\n");
...
MRI *MRIpolyfitBiasField(MRI *vol, int order, MRI *seg, MRI *bias) {
  int c, r, s, nseg, nX, *segids, nsegids, z, k, segid, *tmplist, nthseg,
      npoly = 3;
  ...
  for (c = 0; c < mask->width; c++)
    for (r = 0; r < mask->height; r++)
      for (s = 0; s < mask->depth; s++)
        if (MRIgetVoxVal(mask, c, r, s, 0) > 0.0001)
          nmask++;
...
MRI *MRIfitDCTField(MRI *field, MRI *mask, int nP[3]) {
  ...
  y = MRIvol2mat(field, mask, 1, nullptr);
  if (y == nullptr)
    return (nullptr);
  X = MRIvol2mat(P, mask, 1, nullptr);
  if (X == nullptr)
    return (nullptr);
  printf("nvox = %d\n", y->rows);
...
MRI *MRIdctField(MRI *mri_template, int nP[3], int offset) {
  ...
  for (k = 0; k < mri_template->width; k++)
    cdm[k] = (k - mri_template->width / 2.0) / mri_template->width;
  rdm = (double *)calloc(mri_template->height, sizeof(double));
  for (k = 0; k < mri_template->height; k++)
    rdm[k] = (k - mri_template->height / 2.0) / mri_template->height;
  sdm = (double *)calloc(mri_template->depth, sizeof(double));
  for (k = 0; k < mri_template->depth; k++)
    sdm[k] = (k - mri_template->depth / 2.0) / mri_template->depth;

  nPtot = nP[0] + nP[1] + nP[2] + offset;
  P     = MRIallocSequence(mri_template->width, mri_template->height,
                       mri_template->depth, MRI_FLOAT, nPtot);
...
int MRIbiasFieldCorLog(MRIBF *bf) {
  ...
  bf->seg = MRIcopy(bf->srcseg, nullptr);
...
#pragma omp parallel for reduction(+ : wmsum, nwm)
  ...
  bf->segerode = MRIerodeSegmentation(bf->seg, nullptr, bf->nerode, 0);
  ...
  bf->bbseg   = MRIextractRegion(bf->segerode, nullptr, bf->bb);
  bf->bbinput = MRIextractRegion(bf->input, nullptr, bf->bb);
  // MRIwrite(bf->bbseg,"bbseg.mgh");

  // Compute log of input
  bf->y = MRIvol2mat(bf->bbinput, bf->bbseg, 1, nullptr);
  ...
  MRI *dctfield = MRIextractRegion(bf->dct->field, nullptr, bf->bb);

  // construct regressors for the DCT
  bf->Xbf = MRIbiasDCTReg(dctfield, bf->bbseg, nullptr);

  // Compute the GLM. Repackage using fsglm?
  bf->X  = MatrixHorCat(bf->Xbf, bf->Xseg, nullptr);
  bf->Xt = MatrixTranspose(bf->X, nullptr);
  // bf->XtX  = MatrixMtM(bf->X, NULL); // something wrong with this in parallel
  bf->XtX     = MatrixMultiplyD(bf->Xt, bf->X, nullptr);
  bf->XtXcond = MatrixConditionNumber(bf->XtX);
  printf("MRIbiasFieldLog(): XtXcond=%g\n", bf->XtXcond);
  bf->iXtX = MatrixInverse(bf->XtX, nullptr);
  if (bf->iXtX == nullptr) {
    printf("ERROR: MRIbiasFieldLog(): matrix cannot be inverted\n");
    MatrixWriteTxt("X.mtx", bf->X);
    return (1);
  }
  bf->Xty  = MatrixMultiplyD(bf->Xt, bf->y, nullptr);
  bf->beta = MatrixMultiplyD(bf->iXtX, bf->Xty, nullptr);
  MatrixFree(&bf->Xt);
  MatrixFree(&bf->y);
  ...
  printf("VmPeak  %d\n", GetVmPeak());
  fflush(stdout);
  ones = MRIcopy(bf->bbseg, nullptr);
  MRIconst(ones->width, ones->height, ones->depth, 1, 1, ones);
  MatrixFree(&bf->Xbf);
  bf->Xbf = MRIbiasDCTReg(bf->dct->field, bf->input, nullptr);
  ...
  bf->biasfield = MRImat2vol(bffit, bf->input, 1, nullptr);
...
int DCTfield(DCTFS *dct) {
  ...
  MATRIX *vox2ras = nullptr, *ras2vox = nullptr; //*crs=NULL,*ras=NULL;
  ...
  for (n = 0; n < dct->nfields; n++) {
    ...
    for (c = 0; c < mri_template->width; c++) {
      ...
      for (r = 0; r < mri_template->height; r++) {
        // crs->rptr[2][1] = r;
        for (s = 0; s < mri_template->depth; s++) {
          if (dct->mask && MRIgetVoxVal(dct->mask, c, r, s, 0) < 0.5)
            continue;
          // crs->rptr[3][1] = s;
          if (period == 0) {
          ...
          switch (dim) {
          case 1:
            t = (c - offset) * dct->field->xsize;
            break;
          case 2:
            t = (r - offset) * dct->field->ysize;
            break;
          case 3:
            t = (s - offset) * dct->field->zsize;
            break;
          }
...
MRI *MRIapplyBiasField(MRI *input, MRI *bf, MRI *seg, MRI *mask,
  ...
  if (bc == nullptr) {
    bc = MRIallocSequence(input->width, input->height, input->depth, MRI_FLOAT,
                          input->nframes);
