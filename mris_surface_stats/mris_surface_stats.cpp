...
int main(int argc, char *argv[]) {
  ...
  if (mask_fname) {
    ...
    masklabel = LabelRead(nullptr, mask_fname);
  ...
  if (first_group_size < nsurfaces_total - 1) {
    ...
    for (index = 0; index < mri_surf->nvertices; index++) {
      ...
      scalar /= sqrt(0.5 * (MRIgetVoxVal(mriStd, index, 0, 0, 0) *
                                MRIgetVoxVal(mriStd, index, 0, 0, 0) +
                            std * std));
...
static int get_option(int argc, char *argv[]) {
  ...
  if (!stricmp(option, "out_fname") || !stricmp(option, "out_name") ||
      !stricmp(option, "out")) {
    stat_fname = argv[2];
    printf("using %s as output file \n", stat_fname);
    nargs = 1;
  } else if (!stricmp(option, "mean_fname") || !stricmp(option, "mean_name") ||
             !stricmp(option, "mean")) {
    mean_fname = argv[2];
    printf("using %s as output file for mean \n", mean_fname);
    nargs = 1;
  } else if (!stricmp(option, "mask_fname") || !stricmp(option, "mask_name") ||
             !stricmp(option, "mask")) {
    mask_fname = argv[2];
    ...
    nargs = 1;
             ...
             !stricmp(option, "absmean_name") || !stricmp(option, "absmean")) {
    absmean_fname = argv[2];
    printf("using %s as output file for abs-mean \n", absmean_fname);
    nargs = 1;
  } else if (!stricmp(option, "absstd_fname") ||
             !stricmp(option, "absstd_name") || !stricmp(option, "absstd")) {
    absstd_fname = argv[2];
...
static void usage_exit(int code) {
  ...
  printf("    "
         "$SUBJECTS_DIR/groupstudy/"
         "1???_${hemi}_thickness_diff_resampled_to_1001.mgh\n");
