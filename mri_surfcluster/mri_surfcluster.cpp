...
int main(int argc, char **argv) {
  ...
  if (clabelfile) {
    ...
    clabel = LabelRead(nullptr, clabelfile);
    if (clabel == nullptr) {
  ...
  XFM = DevolveXFM(srcsubjid, nullptr, xfmfile);
  if (XFM == nullptr)
    exit(1);
  ...
  if (annotname != nullptr) {
    sprintf(fname, "%s/%s/label/%s.%s.annot", subjectsdir, srcsubjid, hemi,
            annotname);
  ...
  totarea = 0;
  for (vtx = 0; vtx < srcsurf->nvertices; vtx++) {
    if (srcsurf->vertices[vtx].undefval == 0)
      continue; // mask
    if (!srcsurf->group_avg_vtxarea_loaded)
      totarea += srcsurf->vertices[vtx].area;
    else
      totarea += srcsurf->vertices[vtx].group_avg_area;
  }
  ...
  if (voxwisesigfile) {
    ...
    srcval = MRIcopyMRIS(nullptr, srcsurf, 0, "val");
    voxwisesig =
        CSDpvalMaxSigMap(srcval, csd, nullptr, nullptr, &maxmaxsig, Bonferroni);
  ...
  printf("thmin=%f (%f), thmax=%f (%g), thsignid=%d, minarea=%lf\n", thmin,
         thminadj, thmax, thmaxadj, thsignid, minarea);
  scs = sclustMapSurfClusters(srcsurf, thminadj, thmaxadj, thsignid, minarea,
                              &NClusters, XFM, fwhmmap);
  ...
  if (FixMNI) {
    ...
    for (n = 0; n < NClusters; n++) {
      FixMNITal(scs[n].xxfm, scs[n].yxfm, scs[n].zxfm, &scs[n].xxfm,
  ...
  if (fwhm > 0) { // use RFT
    ...
    grfnsearch = nsearch;
    if (thsignid == 0)
      grfnsearch = grfnsearch / 2.0; // hack for abs
    fwhmvtx = fwhm / srcsurf->avg_vertex_dist;
    printf("GRF: fwhm = %g, dist = %g, fwhmvtx = %g, thmin = %g, grfnsearch = "
           "%g\n",
           fwhm, srcsurf->avg_vertex_dist, fwhmvtx, thmin, grfnsearch);
    for (n = 0; n < NClusters; n++) {
      ClusterSize = scs[n].nmembers;
      pval =
          RFprobZClusterSigThresh(ClusterSize, thmin, fwhmvtx, grfnsearch, 2);
      if (thsignid == 0)
  ...
  if (sumfile != nullptr) {
    ...
    for (n = 0; n < NClusters; n++) {
      ...
      if (ReportCentroid) { // Report Centriod XYZ
        fprintf(fp, "%4d     %9.4f  %6d  %8.2f   %6.1f %6.1f %6.1f", n + 1,
                maxval, scs[n].vtxmaxval, scs[n].area, scs[n].cxxfm,
                scs[n].cyxfm, scs[n].czxfm);
      } else { // Report Max XYZ
        fprintf(fp, "%4d     %9.4f  %6d  %8.2f   %6.1f %6.1f %6.1f", n + 1,
                maxval, scs[n].vtxmaxval, scs[n].area, scs[n].xxfm, scs[n].yxfm,
                scs[n].zxfm);
      }
      if (csd != nullptr)
      ...
      if (annotname != nullptr)
        fprintf(fp, "  %s",
                annotation_to_name(
                    srcsurf->vertices[scs[n].vtxmaxval].annotation, nullptr));
      fprintf(fp, "\n");
    }
    fclose(fp);
  }
  ...
  if (outid != nullptr) {
    ...
    if (!strcmp(outfmt, "paint") || !strcmp(outfmt, "w"))
      ...
      mritmp = MRIcopyMRIS(nullptr, srcsurf, 0, "val");
  ...
  if (ocnid != nullptr) {
    ...
    if (!strcmp(ocnfmt, "paint") || !strcmp(ocnfmt, "w"))
      ...
      mritmp = MRIcopyMRIS(nullptr, srcsurf, 0, "undefval");
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
    ...
    if (!strcasecmp(option, "--help"))
      ...
      debug = 1;
    else if (!strcasecmp(option, "--fixmni"))
      FixMNI = 1;
    else if (!strcasecmp(option, "--nofixmni"))
      FixMNI = 0;
    else if (!strcasecmp(option, "--no-fixmni"))
      FixMNI = 0;
    else if (!strcasecmp(option, "--clabelinv"))
      clabelinv = 1;
    else if (!strcasecmp(option, "--mask-inv"))
      clabelinv = 1;
    else if (!strcasecmp(option, "--no-adjust"))
      AdjustThreshWhenOneTail = 0;
    else if (!strcmp(option, "--csdpdf-only"))
      csdpdfonly = 1;
    else if (!strcmp(option, "--cortex"))
      UseCortexLabel = 1;
    else if (!strcmp(option, "--centroid"))
      ReportCentroid = 1;
    else if (!strcmp(option, "--sig2p-max"))
      sig2pmax = 1;
      ...
      if (nargc < 1)
        argnerr(option, 1);
      thsign = pargv[0];
      if (!stringmatch(thsign, "abs") && !stringmatch(thsign, "pos") &&
...
static void print_usage() {
  ...
  printf("   --annot    annotname : report annotation for max vertex (eg, "
         "aparc)\n");
  ...
  printf("   --bonferroni-max N : apply bonf cor to maximum (only applies with "
         "--sig2p-max)\n");
  ...
  printf("   --thmax  threshold : maximum intensity threshold (only use if you "
...
static void print_help() {
  ...
  printf(
      ...
      "talairach coordinate. (2) Filtered - identical to the input except "
      "that\n"
      ...
      "--hemi hemi\n"
      "\n"
      "Specify the cortical hemisphere that the input represents. Valid "
      "values\n"
      "are lh and rh.\n"
      "\n"
      "--surf surface\n"
      ...
      "\n"
      "Report the cortical annotation that the maximum in each label falls "
      "into. \n"
      "The annotation used will be "
      "SUBJECTS_DIR/subject/label/hemi.annotationname.annot.\n"
      "Eg, --annot aparc\n"
      "\n"
      "--frame frameno\n"
      "\n"
      ...
      "will be adjusted when the --sign is pos or neg by subtracting "
      "log10(2.0).\n"
      "This assumes several things: (1) the source is a -log10(p) map, and "
      "(2)\n"
      "the pvalue was computed using a two-sided t-test. Under these "
      "conditions,\n"
      "subtracting log10(2.0) is like dividing the p-value by 2 (ie, changing "
      "it\n"
      "from a two-tailed test to a one-tailed test). If the input map does "
      "not\n"
      ...
      "\n"
      "Load one or more CSD files. CSD stands for 'Cluster Simulation Data'. "
      "This\n"
      "file is produced by running mri_glmfit with --sim. The the threshold "
      "and hemi\n"
      "info are obtained from the CSD file and cannot be specified on the "
      "command-\n"
      "line. If more than one CSD is specified, they are merged into one CSD "
      "internally.\n"
      "When a CSD file is specified, three more columns are added to the "
      "summary table:\n"
      ...
      "distribution\n"
      "In addition, the user can specify --cwsig, which saves the sigificance "
      "map of \n"
      "the clusters in which the value of each vertex is the -log10(pvalue) of "
      "cluster\n"
      "to which the vertex belongs (the cluster-wise significance). The user "
      "can also\n"
      "specify that the vertex-wise significance be computed and saved  with "
      "--vwsig.\n"
      "The significance is based on the distribution of the maximum "
      "significances \n"
      "found during the CSD simulation.\n"
      "\n"
      "--csdpdf csdpdfile\n"
      "\n"
      "Compute PDF/CDF of CSD data and save in csdpdffile. This is mostly good "
      "for debugging.\n"
      "\n"
      "--csdpdf-only\n"
      "\n"
...
static void check_options() {
  ...
  if (csd != nullptr) {
    ...
    if (srcsubjid == nullptr)
      srcsubjid = strcpyalloc(csd->subject);
    else {
    ...
    if (csd->threshsign > 0.5)
      thsign = "pos";
    else if (csd->threshsign < -0.5)
      thsign = "neg";
    else
      thsign = "abs";
    minarea = 0;
  ...
  if (thsign == nullptr)
    thsign = "abs";
  if (stringmatch(thsign, "pos"))
    thsignid = +1;
  if (stringmatch(thsign, "abs"))
    thsignid = 0;
  if (stringmatch(thsign, "neg"))
    thsignid = -1;
  ...
  if (maskid != nullptr) {
    ...
    if (masksubjid == nullptr)
      masksubjid = srcsubjid;
    if (masksignstr == nullptr)
      masksignstr = "abs";
    if (stringmatch(masksignstr, "abs"))
      masksign = 0;
    if (stringmatch(masksignstr, "pos"))
      masksign = 1;
    if (stringmatch(masksignstr, "neg"))
      masksign = -1;
  }
  ...
  if (UseCortexLabel) {
    sprintf(tmpstr, "%s/%s/label/%s.cortex.label", subjectsdir, srcsubjid,
            hemi);
    clabelfile = strcpyalloc(tmpstr);
  }

  if (clabelfile != nullptr && maskfile != nullptr) {
    printf("ERROR: cannot specify both --clabel and --mask\n");
    exit(1);
  }
  if (clabelinv && (clabelfile == nullptr && maskfile == nullptr)) {
...
static void dump_options(FILE *fp) {
  ...
  if (outid != nullptr)
    fprintf(fp, "outid    = %s %s\n", outid, outfmt);

  if (ocnid != nullptr)
    fprintf(fp, "ocnid    = %s %s\n", ocnid, ocnfmt);
