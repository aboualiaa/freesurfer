...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -s surface -m mask -o outputOverlayImage -d "
                 "outputDistanceImage -c overlay -l label -inv -r radius"
              << std::endl;
    return -1;
  }
  ...
  if (!cl.search("-inv")) {
    ...
    MRI *image = MRIread(maskFilename);
    MRI *output =
        MRIalloc(image->width, image->height, image->depth, MRI_FLOAT);
    MRIcopyHeader(image, output);
      ...
      for (int j = 0; j < image->height; j++) {
        for (int k = 0; k < image->depth; k++) {
          ...
          if (val == label) {
            ...
            MRIvoxelToSurfaceRAS(image, i, j, k, &point[0], &point[1],
                                 &point[2]);
    ...
    for (int j = 0; j < surf->nvertices; j++) {
      double pt[3];
      MRISsurfaceRASToVoxel(surf, image, surf->vertices[j].x,
                            surf->vertices[j].y, surf->vertices[j].z, &pt[0],
                            &pt[1], &pt[2]);
