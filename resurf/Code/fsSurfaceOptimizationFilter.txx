...
void fs::SurfaceOptimizationFilter<TInputSurface,
                                   TOutputSurface>::GenerateData() {
  ...
  for (int w = 0; w < 2000; w++) {
    ...
    for (unsigned int i = 0; it != itEnd; it++, i++) {
      ...
      typename InSurfaceType::PointType grad =
          point.EuclideanDistanceTo(adj[0]) - point.EuclideanDistanceTo(adj[1]);
      // std::cout << point.EuclideanDistanceTo(adj[0]) <<
      // point.EuclideanDistanceTo(adj[1]) <<std::endl; typename
      // InSurfaceType::PointType temp =  point.GetVectorFromOrigin()*2; temp =
      // temp - adj[0]; temp= temp -adj[1]; std::cout << adj[0] << adj[1]<<
      // std::endl;
      for (int j = 0; j < 3; j++) {
        grad[j] = point[j] +
                  0.0001 * grad[j] * (2 * point[j] - adj[0][j] - adj[1][j]);
      }
      this->GetOutput()->SetPoint(i, grad);
    }
  }
  const typename InSurfaceType::CellsContainer *cells =
      this->GetInput()->GetCells();
