...
private:
  ...
  bool isCSF(double val) { return (val < MAX_CSF); }
  ...
  bool isGrey(double val) { return val > T2_min && val < T2_max; }
  bool isPial(double val) { return val > T2_max; }
  float score(double val, double max_val, double mag, double max_mag, int t,
              int max_t) {

    //		std::cout << val/(max_val+1.1)+ fabs(mag)/(max_mag+1.1)  << " " ; //+
    //t/float(max_t);
    return val / (max_val + 101) +
           fabs(mag) / (max_mag + 1.1); //+ t/float(max_t);

    //	((val >= .9*max_val && mag > max_mag ) || ( val > max_val && mag >=.9
    //*max_mag)|| t<max_t) ;
  }
};
void MRIS_MultimodalRefinement::getTarget(MRIS *surf) {
  ...
  for (std::map<std::tuple<int, int, int>, int>::iterator it = priors.begin();
       it != priors.end(); ++it) {
  ...
  for (unsigned j = 0; j < surf->nvertices; j++) {
    ...
    for (int t = -this->numberOfSteps; t <= this->numberOfSteps; t++) {
      ...
      // int k=0;
      {
        ...
        //				MRIsampleVolumeDerivativeScaleV(images[0], x,
        //y, z, nx, ny, nz, &mag, this->gradientSigma, step, vertexDebug==j);
        MRIsampleVolumeDerivativeScale(
            images[0], x, y, z, nx, ny, nz, &mag,
        ...
        for (int i = 1; i<2 ;i++)
        {
                float w = exp(-pow((i-1)*step,2) / (2 * pow(this->gradientSigma
        ,2))); norm += w; x= xv +nx*(i+t)*step; y=yv +ny*(i+t)*step; z=zv
        +nz*(i+t)*step;
      ...
      if (vertexDebug == j) {
        std::cout << "distance from ori " << t * step << " " << mag << " "
                  << val << std::endl;
      }
    }
    float in = 0, out = 0;
    ...
    for(int i=0;i<intensities.size();i++)
    {
                    ...
                    (magnitudes[i]<opt_mag && !this->maximumGradient ))
            {
                    ...
                    //MRISsurfaceRASToVoxel(surf, images[0],x,y,z, &xv,&yv,&zv);
                    //MRIsampleVolumeType(segmentation, xv, yv, zv, &val,
    SAMPLE_NEAREST);
    //		MRIsampleVolume(segmentation, x, y, z, &val);
    ...
    //		if(val <.5)
                    {
                            ...
                            {
                                    std::cout << "TAKING: distance from ori " <<
    (i-numberOfSteps)*step <<  " " << opt_mag <<   " " << opt_val << std::endl;
...
MRIS_MultimodalRefinement::getPs(MRIS *surf) {
  ...
  for (unsigned j = 0; j < surf->nvertices; j++) {
    ...
    for (int t = -this->numberOfSteps; t <= this->numberOfSteps; t++) {
      ...
      MRIsampleVolumeDerivativeScale(images[0], x, y, z, nx, ny, nz, &mag,
                                     this->gradientSigma);
    ...
    for (int i = 0; i < intensities.size() / 2; i++) {
      // if(fabs(magnitudes[i])>=fabs(max_mag))
      if (intensities[i] > AVG_CSF ||
          intensities[i + this->numberOfSteps] > AVG_CSF) {
        foundCSF = 1;
      }
      in += intensities[i];
      out += intensities[i + this->numberOfSteps];
    ...
    std::tuple<int, int, int> tuple =
        std::make_tuple((int)in, (int)out, foundCSF);
...
void MRIS_MultimodalRefinement::getTarget2(MRIS *surf) {
  ...
  for (unsigned j = 0; j < surf->nvertices; j++) {
    ...
    for (int t = -this->numberOfSteps; t <= this->numberOfSteps; t++) {
      ...
      for (int i = 1; i < 3; i++) {
        float w =
            exp(-pow((i - 1) * step, 2) / (2 * pow(this->gradientSigma, 2)));
        norm += w;
        ...
        valp += w * prev_val;
        if (vertexDebug == j)
          std::cout << "prev  " << prev_val << " next " << next_val
                    << std::endl;
      }
      valn /= norm;
      valp /= norm;
...
void MRIS_MultimodalRefinement::getTargetAdHoc(MRIS *surf) {
  ...
  for (unsigned j = 0; j < surf->nvertices; j++) {
    double nx, ny, nz;
    MRISsurfaceRASToVoxel(surf, images[0], surf->vertices[j].nx,
                          surf->vertices[j].ny, surf->vertices[j].nz, &nx, &ny,
                          &nz);
    float dist = sqrt(nx * nx + ny * ny + nz * nz);
    if (dist > 0) {
      nx /= dist;
    ...
    if (vertexDebug == j) {
      std::cout << "first intensity" << first_val << " " << surf->vertices[j].x
                << " " << surf->vertices[j].y << " " << surf->vertices[j].z
                << std::endl;
    }
    for (int d = -1; d < 2; d += 2) {
      nx = -d * nx;
      ...
      for (int t = 0; t < numberOfSteps && keep_going; t++) {
        ...
        for (unsigned int k = 0; k < images.size(); k++) {
          ...
          MRIsampleVolumeDerivativeScale(images[k], xv, yv, zv, nx, ny, nz,
                                         &mag,
                                         this->gradientSigma); // expensive
          MRIsampleVolumeDerivativeScale(images[k], xvn, yvn, zvn, nx, ny, nz,
                                         &next_mag,
                                         this->gradientSigma); // expensive
          MRIsampleVolumeDerivativeScale(images[k], xvp, yvp, zvp, nx, ny, nz,
                                         &prev_mag,
                                         this->gradientSigma); // expensive
          bool take = false;
          if (continuousIncrease(prev_val, val, next_val) &&
              score(val, max_val, mag, max_mag, t, max_t) > 1 && isGrey(val) &&
              isPial(next_val) && (!csf_found)) {
            take = true;
            if (vertexDebug == j) {
              std::cout << "Found bright " << t * step << std::endl;
            }
          }
          if (continuousDecrease(prev_val, val, next_val) &&
              score(val, max_val, mag, max_mag, t, max_t) > 1 && isGrey(val) &&
              isPial(prev_val) && (!csf_found)) {
            take = true;
            if (vertexDebug == j) {
          ...
          if ((isPial(first_val) && isGrey(val) &&
               (isPial(prev_val) || isPial(next_val)))) {
            if (t < max_t) {
              take = true;
            } else {
              take = false;
            ...
            if (vertexDebug == j) {
              std::cout << "first val max intensity test  " << t * step
                        << std::endl;
            }
          }

          if (continuousDecrease(prev_val, val, next_val) &&
              fabs(mag) >= max_mag && isCSF(next_val) &&
              (isGrey(val) || isGrey(prev_val))) {
            csf_found = true;
            take = true;
            if (vertexDebug == j) {
          ...
          if (take) //||( t==0 && d==-1))
          {
            ...
            if (vertexDebug == j) {
              std::cout << "TAKING: distance from ori " << t * step * d
                        << std::endl;
            }
          ...
          if (isCSF(val) && continuousDecrease(prev_val, val, next_val)) {
            t = numberOfSteps;
            if (vertexDebug == j) {
              std::cout << "Breaking due to reach CSF , this leaving sulcus"
                        << t * step * d << std::endl;
            }
            keep_going = false;
          }
          if (isPial(val) && isGrey(prev_val)) {
            if (vertexDebug == j) {
              std::cout
                  << "Breaking due to reach max intensity , this leaving sulcus"
                  << t * step * d << std::endl;
            }
            keep_going = false;
            t = numberOfSteps;
          ...
          if (vertexDebug == j) {
            ...
            std::cout << "coordinates " << x << ", " << y << ", " << z
                      << std::endl;
            std::cout << "Intensity previous , val, next: " << prev_val << " "
                      << val << " " << next_val << std::endl;
            std::cout << "Gradient previous, mag, next: " << prev_mag << " "
                      << mag << next_mag << std::endl;
          }
        }
      }
    }
...
void MRIS_MultimodalRefinement::refine(MRIS *surf, float lambda,
                                       int iterations) {
    ...
    for (int t = 0; t < max_thickness * 2; t++) {
      ...
      for (unsigned k = 0; k < images.size(); k++) {
        ...
        MRIsampleVolumeDerivativeScale(images[k], xv, yv, zv, -nx, -ny, -nz,
                                       &mag, 1.0); // expensive
        MRIsampleVolumeDerivativeScale(images[k], xvn, yvn, zvn, -nx, -ny, -nz,
                                       &next_mag, 1.0); // expensive
        MRIsampleVolumeDerivativeScale(images[k], xvp, yvp, zvp, -nx, -ny, -nz,
                                       &prev_mag, 1.0); // expensive
        // std::cout << mag << " " << prev_mag  << "  " << next_mag << " val "<<
        // val <<std::endl;

        if ((fabs(mag) > fabs(prev_mag)) && (fabs(mag) > fabs(next_mag)) &&
            (val <= T2_max) && (val >= T2_min) && fabs(mag) * 1.2 > max_mag &&
            prev_val < val && val < next_val) {
          max_mag = fabs(mag);
          ...
          surf->vertices[j].x = x;
          surf->vertices[j].y =
              surf->vertices[j].y - ny * max_thickness * step + t * step * ny;
          surf->vertices[j].z =
              surf->vertices[j].z - nz * max_thickness * step + t * step * nz;
        }
      }
    }
  }
