...
public:
  ...
  void addImage(MRI *image) { images.push_back(image); }
  ...
  void getNormal(MRIS *surf, MRI *image, int vtxno, double *x, double *y,
                 double *z);
  std::vector<std::pair<float, float>> GetMeanAndVariance(MRIS *surf,
                                                          MRI * image);
...
void MRIS_MultimodalRefinement::getTarget(MRIS *surf) {
  ...
  std::cout << " min p " << 1.0 / pow(10, this->minPGrey) << " "
            << 1.0 / pow(10, this->minPGrey / 3) << std::endl;
  ...
  std::cout << " ps size " << ps.size() << " images size " << images.size()
            << std::endl;
  ...
  if (this->posteriorMR == nullptr) {
    for (int i = 0; i < images.size(); i++) {
      ...
      std::cout << " ah " << stats_vertex_images[i][vertexDebug].first << " "
                << stats_vertex_images[i][vertexDebug].second << std::endl;
    }
  }
  for (unsigned int j = 0; j < surf->nvertices; j++) {
    ...
    for (int t = 1; t < 5; t++) {
      ...
      //		vno2 = MHTfindClosestVertexNo2(mht, surf, surf, &v1, &dminv);
      if (j == vertexDebug)
        std::cout << "v1  " << v1.x << " " << surf->vertices[j].x << " "
                  << surf->vertices[j].nx << " " << sum << " " << dminv
                  << std::endl;
    ...
    if (j == vertexDebug)
      std::cout << "distance " << dmin << " distance to white " << dx + dy + dz
                << std::endl;
    ...
    bool   count    = false;
    int    counting = 0;
    for (int t = -this->numberOfSteps;
         (t <= this->numberOfSteps ||
          (ps[t + numberOfSteps - 1] > (1.0 / pow(10, this->minPGrey)) &&
           t + numberOfSteps < ps.size()));
         t++) {
      ...
      double label              = 0;
      MRIsampleVolumeFrameType(this->segmentation, x, y, z, 0, SAMPLE_NEAREST,
                               &label);
      ...
      for (int k = 0; k < images.size(); k++) {
        ...
        MRIsampleVolumeDerivativeScale(
            images[k], x, y, z, nx, ny, nz, &mag,
            this->gradientSigma); //, step, vertexDebug==j);
        //probability of grey
        //float p = (1.0/sqrt(2*varGrey)) *exp( - pow(val-meanGrey,2) /(2*varGrey));
        float p = exp(-pow(val - stats_vertex_images[k][j].first, 2) /
                      (2 * stats_vertex_images[k][j].second));
      ...
      if (vertexDebug == j) {
        std::cout << "distance from ori " << t * step << " "
                  << magnitudes[t + numberOfSteps] << " " << val << " "
                  << ps[t + numberOfSteps] << "  white intensity "
                  << whiteIntensity << " vesselIntensity " << vesselIntensity
                  << std::endl;
      }
    ...
    if (vertexDebug == j)
      std::cout << "dist to white" << (dx + dy + dz) << " dist to next sulcus "
                << dmin << std::endl;
    if (((dx + dy + dz > 1) && (dmin <= 3))) {
      ...
      for (int i = 1;
           (i < numberOfSteps + 1 ||
            (ps[i - 1] > (1.0 / pow(10, this->minPGrey)) && i < ps.size() - 1));
           i++) {
        if (vertexDebug == j)
          std::cout << " opt mag " << opt_mag << " mag " << magnitudes[i] << " "
                    << ps[i] << " " << ps[i - 1] << " " << ps[i - 2]
                    << std::endl;
        ...
        /*float leftP = this->DistanceToMidline(xt,yt,zt);				
				leftP /= fabs(leftP);*/
        label = segs[i];
        if (label == 53 || label == 17 || label == 16 || label == 47 ||
            label == 8 ||
            ((label != 2 && label != 41) && masksW[i] < .1 &&
             masks[i + 1] > .1 && masksW[i + 1] < .01)) {
          touchedStructure++;
        } else if ((segs[i + 1] > 1000 && lastCortexLabel > 1000 &&
                    fabs(segs[i + 1] - lastCortexLabel) > 900) ||
                   (segs[i + 1] == 42 && lastCortexLabel == 3) ||
                   (segs[i + 1] == 3 && lastCortexLabel == 42)) {
          changeHemisAseg++;
          changeHemis++;
        }
        if (label == 0) { /*if(fabs(leftW + leftP)  ==0)  
					{
						changeHemis ++;
					}*/
          zeroLabel++;
        ...
        if (vertexDebug == j)
          std::cout << lastCortexLabel << " " << segs[i + 1] << " "
                    << touchedStructure << std::endl;
        //label = (label>.5)?1:0;
        if ((fabs(magnitudes[i]) > opt_mag &&
             ps[i] > (1.0 / pow(10, this->minPGrey))) ||
            touchedStructure > 0 ||
            (changeHemis > 0 && fabs(magnitudes[i]) * 1.5 > opt_mag) ||
            (changeHemisAseg > 0 && zeroLabel < 3 && opt_mag < 5)) {

          int w = i;
          good  = true;
          for (; w > 0 && good &&
                 (ps[w - 1] < (1.0 / pow(10, this->minPGrey / 3.0)) ||
                  fabs(i - w) < 1);
               w--) {
            good = ps[w - 1] >= ps[w] && masks[w] < .1;
            if (vertexDebug == j)
              std::cout << ps[w - 1] << " " << ps[w] << " " << w << " "
                        << masks[w] << std::endl;
          }
          if ((((ps[w - 1] > (1.0 / pow(10, this->minPGrey / 3.0))) || good) ||
               (touchedStructure > 0 && ps[i - 1] > 0.0001)) &&
              label != 41 && label != 2) {
            ...
            if (vertexDebug == j) {
              std::cout << "TAKING: distance from ori "
                        << (i - numberOfSteps) * step << " " << opt_mag << " "
                        << opt_val << std::endl;
            }
          }
        }
        if (touchedStructure > 0 || changeHemis > 1 ||
            changeHemisAseg > 0) //|| (!IS_BRAIN(label) && masks[i] >.5 ))
          break;
...
void MRIS_MultimodalRefinement::getTargetO(MRIS *surf) {
  ...
  for (unsigned int j = 0; j < surf->nvertices; j++) {
    ...
    double vertexMask = 0;
    for (int t = -this->numberOfSteps;
         (t <= this->numberOfSteps || (ps[t + numberOfSteps - 1] > 1.0e-20 &&
                                       t + numberOfSteps < ps.size()));
         t++) {
      //ps[t+numberOfSteps] =1;
    ...
    if (vertexDebug == j)
      std::cout << "dist to white" << (dx + dy + dz) << " dist to next sulcus "
                << dmin << " vessel ?  " << vertexMask << std::endl;
    if (((dx + dy + dz > 1) && (dmin <= 3))) // || vertexMask >.5)
    {
      ...
      float opt_mag             = 0, opt_val;
      int   opt_t;
      float leftW = this->DistanceToMidline(surf->vertices[j].whitex,
                                            surf->vertices[j].whitey,
                                            surf->vertices[j].whitez);
      leftW /= fabs(leftW);
      int   label = 0, prevLabel = 0, lastCortexLabel = 0;
      float touchedStructure = 0;
      float changeHemis      = 0;
      ...
      int   zeroLabel        = 0;
      for (int i = 1;
           (i < numberOfSteps + 1 || (ps[i - 1] > 1e-15 && i < ps.size() - 1));
           i++) {
        if (vertexDebug == j)
          std::cout << " opt mag " << opt_mag << " mag " << magnitudes[i] << " "
                    << ps[i] << " " << ps[i - 1] << " " << ps[i - 2]
        ...
        if (vertexDebug == j)
          std::cout << lastCortexLabel << " " << segs[i + 1] << " "
                    << touchedStructure << " distance midplane " << leftP << " "
                    << leftW << std::endl;
        //label = (label>.5)?1:0;
        if ((fabs(magnitudes[i]) > opt_mag && ps[i] > 1e-15) ||
            touchedStructure > 0 ||
            (changeHemis > 0 && fabs(magnitudes[i]) * 1.5 > opt_mag) ||
            (changeHemisAseg > 0 && zeroLabel < 3 && opt_mag < 5)) {
          ...
          if ((((ps[w - 1] > 1e-05) || good) ||
               (touchedStructure > 0 && ps[i - 1] > 1e-1)) &&
              label != 41 && label != 2) {
            opt_mag                 = fabs(magnitudes[i]);
            opt_val                 = intensities[i];
            opt_t                   = i;
...
MRIS_MultimodalRefinement::GetMeanAndVariance(MRIS *surf, MRI *image) {
  ...
  for (int x = 0; x < t1->width; x++) {
    for (int y = 0; y < t1->height; y++) {
      for (int z = 0; z < t1->depth; z++) {
        ...
        } else if (1.3 * T1val > T2val && (T1val + T2val) >= 180 &&
                   T1val > 80 && T2val > 80) {
          val = 1;
        }
  ...
  for (int x = 0; x < t1->width; x++) {
    for (int y = 0; y < t1->height; y++) {
      for (int z = 0; z < t1->depth; z++) {
        ...
        } else if ((mult * T1val > T2val && (T1val + T2val < sum)) ||
                   (T1val < max && T2val < max) ||
                   (T1val > min && T2val > min)) {
          val = 1;
        }
...
void MRIS_MultimodalRefinement::getNormal(MRIS *surf, MRI *image, int vtxno,
  ...
  for (int i = 0; i < segmentation->width; i++) {
    for (int j = 0; j < segmentation->height; j++) {
      for (int k = 0; k < segmentation->depth; k++) {
        ...
        if (pixel == 14 || pixel == 254 || pixel == 253 || pixel == 252 ||
            (pixel == 3026 && m_baby) || (pixel == 4026 && m_baby) ||
            (pixel == 3010 && m_baby) || (pixel == 4010 && m_baby) ||
            (pixel == 175 && m_baby)) // || pixel == 255 || pixel == 251)
        {
          values.push_back(v);
          for (int w = 0; w < 3; w++)
            this->center[w] += v[w];
...
float MRIS_MultimodalRefinement::DistanceToMidline(double x, double y,
                                                   double z) {
  float d    = 0;
  float norm = 0;
  for (int w = 0; w < 3; w++) {
