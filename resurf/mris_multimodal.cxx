...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout
        << arg[0]
        << " -i surface -t surface -o surface -fillHoles --curvature "
           "--thickness -a anotationOutput -v overlayOutput -c output.csv -vtk "
        << std::endl;
    return -1;
  }
  ...
  if (cl.search("--fillHoles")) {
    ...
    decimate->SetTargetReduction(
        .5); // 99% reduction (if there was 100 triangles, now there will be 1)
  ...
  if (cl.search("--smooth")) {
    ...
    /*		itk::SmoothingQuadEdgeMeshFilter<SurfType, SurfType> smoothing =
       itk::SmoothingQuadEdgeMeshFilter<SurfType, SurfType>::New();
  ...
  if (cl.search("--thickness")) {
    ...
    vtkSmartPointer<vtkKdTreePointLocator> surfTree =
...
#else
    vtkSmartPointer<vtkPCACurvatureEstimation> curvature =
...
#else
      double *curvs = dynamic_cast<vtkDataArray *>(
                          polydata->GetPointData()->GetArray("PCACurvature"))
                          ->GetTuple3(i);
  ...
  if (cl.search("-vtk")) {
    using WriterType = itk::VTKPolyDataWriter<SurfType>;
