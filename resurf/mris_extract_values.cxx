...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -i surface -v overlay -a annotation -o csvfile -m "
                 "numberOfImages image1 image2 imaga3"
              << std::endl;
    return -1;
  }
  ...
  if (cl.search("-a")) {
    ...
    for (unsigned j = 0; j < surf->nvertices; j++) {
      ...
      clusterVals[surf->vertices[j].annotation].push_back(
          surf->vertices[j].curv);
    }
    ...
    for (auto &x : clusterVals) {
      float mean = std::accumulate(x.second.begin(), x.second.end(), 0.0) /
                   x.second.size();
      double accum = 0.0;
      std::for_each(std::begin(x.second), std::end(x.second),
