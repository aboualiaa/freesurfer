...
vtkSmartPointer<vtkImageData> CreateImage(MRI *rasMRI) {
  ...
  vtkUnsignedCharArray *ucharScalars = nullptr;
  vtkIntArray *intScalars = nullptr;
  vtkShortArray *shortScalars = nullptr;
  vtkLongArray *longScalars = nullptr;
  vtkFloatArray *floatScalars = nullptr;
  vtkIdType cValues;
...
int main(int argc, char *argv[]) {
  if (argc != 8) {
    ...
    std::cerr << argv[0];
    std::cerr << " <InputFileName> <OutputFileName> <Lower Threshold> <Upper "
                 "Threshold> <vtk smoothing iterations> <image smoothing size> "
  ...
  for (int i = 0; i < imageSmoothingIterations; i++) {
    vtkSmartPointer<vtkImageContinuousErode3D> erode3D =
    ...
    vtkSmartPointer<vtkImageContinuousDilate3D> dilate3D =
...
#endif
    gaussianSmoothFilter->SetRadiusFactors(
        2 * imageFS->xsize, 2 * imageFS->ysize, 2 * imageFS->zsize);
...
//	contour->ComputeScalarsOn();
  ...
  //	for(int i =0;i<vtkSmoothingIterations;i++)
  {
    ...
    vtkSmartPointer<vtkTriangleFilter> stripper2 =
    ...
    std::cout << " reduction " << reduction << " num points "
              << vtkSurface->GetNumberOfPoints() << std::endl;
    ...
    decimate->SetTargetReduction(reduction); // 99% reduction (if there was 100
                                             // triangles, now there will be 1)
    decimate->Update();
    vtkSurface = decimate->GetOutput();

    std::cout << "num points " << vtkSurface->GetNumberOfPoints() << std::endl;
    vtkSmartPointer<vtkTriangleFilter> stripper =
        vtkSmartPointer<vtkTriangleFilter>::New();
#if VTK_MAJOR_VERSION <= 5
...
vtk.vtkFlyingEdges3D(); isosurface->SetInputConnection(conn->GetOutputPort());
...
vtkSmartPointer<vtkImageThreshold>::New(); threshold2->SetInput( vtkImage);
        threshold2->ThresholdBetween(lowerThreshold,upperThreshold);
        threshold2->Update();

        vtkSmartPointer<vtkMarchingCubes> contour =
vtkSmartPointer<vtkMarchingCubes>::New(); contour->SetInput(vtkImage);
        //contour->SetInputConnection(threshold2->GetOutputPort());
        contour->SetValue(0, lowerThreshold+1);

        vtkSmartPointer<vtkPolyDataConnectivityFilter> conn =
vtkSmartPointer<vtkPolyDataConnectivityFilter>::New(); conn->SetInputConnection(
contour->GetOutputPort() ); conn->SetExtractionModeToLargestRegion();
        conn->Update();

        vtkSmartPointer<vtkPolyDataNormals> normals =
vtkSmartPointer<vtkPolyDataNormals>::New(); normals->SetInputConnection(
conn->GetOutputPort() ); normals->ConsistencyOn(); normals->Update();

        vtkSmartPointer<vtkTriangleFilter> stripper =
vtkSmartPointer<vtkTriangleFilter>::New(); stripper->SetInputConnection(
normals->GetOutputPort() ); stripper->Update();

        vtkSmartPointer<vtkCleanPolyData> clean=
vtkSmartPointer<vtkCleanPolyData>::New(); clean->SetInputConnection(
stripper->GetOutputPort() ); clean->Update();
...
vtkSmartPointer<vtkDecimatePro>::New();
        ...
        //decimate->SetPreserveTopology(true);
        //decimate->SplittingOff();
        decimate->SetTargetReduction(.95); //99% reduction (if there was 100
triangles, now there will be 1) decimate->Update();
  //
  //
        vtkSmartPointer<vtkSmoothPolyDataFilter> smoother2 =
vtkSmartPointer<vtkSmoothPolyDataFilter>::New(); smoother2->SetInputConnection(
decimate->GetOutputPort() ); smoother2->SetFeatureEdgeSmoothing(true);
...
vtkSmartPointer<vtkVoxelContoursToSurfaceFilter>::New(); #if VTK_MAJOR_VERSION
<= 5 contoursToSurface->SetInput(vtkSurface ); #else
...
/*		vtkSmartPointer<vtkPolyDataToImageStencil> pol2stenc =
   vtkSmartPointer<vtkPolyDataToImageStencil>::New(); #if VTK_MAJOR_VERSION <= 5
   ...
   vtkSmartPointer<vtkImageStencil>::New(); #if VTK_MAJOR_VERSION <= 5
        ...
        imgstenc->SetStencilConnection(pol2stenc->GetOutputPort());
                ...
                vtkSmartPointer<vtkImageGaussianSmooth> gaussianSmoothFilter =
   vtkSmartPointer<vtkImageGaussianSmooth>::New(); #if VTK_MAJOR_VERSION <= 5
                gaussianSmoothFilter->SetInput(vtkImage);
        #else
                gaussianSmoothFilter->SetInputData(vtkImage);
        #endif
