...
void function_cx_1_func(const real_1d_array &c, const real_1d_array &x,
  ...
  */
  func = c[6] + c[0] * tanh(c[1] * (x[0] - c[2])) -
         c[3] * tanh(c[4] * (x[0] - c[5]));
}
...
int main(int narg, char **arg) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout
        << arg[0]
        << " -s surface -i image -p profile 4D image -o output fitted image 4D "
        << std::endl;
    return -1;
  }
  ...
  for (SurfaceType::CellsContainerConstIterator itCells =
    ...
    index[3] = 0;
    std::string y_fit =
        std::string("[") + std::to_string(image->GetPixel(index));
    std::string x_fit =
        std::string("[[") + std::to_string(-halfSize) + std::string("]");
    for (int t = 1; t < image->GetLargestPossibleRegion().GetSize()[3]; t++) {
      ...
      x_fit +=
          std::string(",[") + std::to_string(t - halfSize) + std::string("]");
    }
