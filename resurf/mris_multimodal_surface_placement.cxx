...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -i surface -o surface -b spheresurf -n normals.vtk -v "
                 "values.vtk -d debugVertex -s step_size -k numberOfSteps  -g "
                 "gradientSigma -a aseg.aparc  -w whitesurface -p pOfCSF  "
                 "-min/max -t1 image -t2 image -flair image"
              << std::endl;
    return -1;
  }
  ...
  std::cout << " sphere " << sph->vertices[9].phi << " "
            << sph->vertices[9].theta << " " << MRISaverageRadius(sph)
            << std::endl;
  /*		MRISedges(surf);
		MRIScorners(surf);
