...
int main(int narg, char *arg[]) {
  ...
  if (cl.size() == 1 || cl.search(2, "--help", "-h")) {
    ...
    std::cout << arg[0]
              << " -i surface -o surface -n normals.vtk -v values.vtk -d "
                 "debugVertex -s step_size -k numberOfSteps  -g gradientSigma "
                 "-a aseg.aparc  -w whitesurface -p pOfCSF  -min/max -m "
                 "numberOfImages image1 image2 image3"
              << std::endl;
    return -1;
  }
  ...
  */
  std::cout << "debug vertex " << debugVertex << " "
            << surf->vertices[debugVertex].x << " "
            << surf->vertices[debugVertex].y << " "
            << surf->vertices[debugVertex].z << std::endl;
  ...
  /*		typedef itk::Image<float,3> ImageType;
                  typedef itk::ImageFileReader<itk::Image<float,3>>
     ImageReaderType; ImageReaderType::Pointer reader = ImageReaderType::New();
  ...
  */
  for (unsigned j = 0; j < surf->nvertices; j++) {
    ...
    /*
    MRISsurfaceRASToVoxel(surf, images[0],
    surf->vertices[j].x,surf->vertices[j].y,surf->vertices[j].z, &x,&y,&z);
    MRISsurfaceRASToVoxel(surf, images[0],
    surf->vertices[j].nx,surf->vertices[j].ny,surf->vertices[j].nz,
    &nx,&ny,&nz);
    ...
    */
    ...
    ids[0] = totalPoints;
    totalPoints++;
    points->InsertPoint(totalPoints, point[0] + normal[0], point[1] + normal[1],
                        point[2] + normal[2]);
    ids[1] = totalPoints;
    totalPoints++;
  ...
  vtkSmartPointer<vtkCellArray> cellsValues =
  ...
  vtkSmartPointer<vtkPolyData> polyDataValues =
      vtkSmartPointer<vtkPolyData>::New();
  polyDataValues->SetPoints(pointsValues);
  polyDataValues->SetLines(cellsValues);

  vtkSmartPointer<vtkPolyDataWriter> pdWriterValues =
      vtkSmartPointer<vtkPolyDataWriter>::New();
