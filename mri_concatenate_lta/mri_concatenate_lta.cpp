...
int main(int argc, char *argv[]) {
  ...
  if (type == MNI_TRANSFORM_TYPE) {
    ...
    if (tal_src_file == nullptr && lta2->xforms[0].src.valid == 0)
      ErrorExit(ERROR_BADFILE,
                "%s: pls use -tal option to give talairach src and "
                "template filenames",
                Progname);
    if (tal_dst_file == nullptr && lta2->xforms[0].dst.valid == 0)
      ErrorExit(ERROR_BADFILE,
                "%s: pls use -tal option to give talairach src and "
                "template filenames",
                Progname);

    if (tal_src_file != nullptr) {
      LTAmodifySrcDstGeom(lta2, tal_src,
                          nullptr); // add src and dst information
    }
    if (tal_dst_file != nullptr) {
      LTAmodifySrcDstGeom(lta2, nullptr,
                          tal_dst); // add src and dst information
    }
  } else {
  ...
  if (lta1->type == LINEAR_RAS_TO_RAS) {
    RAS_1_to_1 = MatrixCopy(lta1->xforms[0].m_L, nullptr);
  ...
  if (lta2->type == LINEAR_RAS_TO_RAS) {
    RAS_2_to_2 = MatrixCopy(lta2->xforms[0].m_L, nullptr);
  ...
  else if (strlen(lta2->subject) > 0 &&
           strcmp(lta2->subject, "subject-unknown") != 0)
    strcpy(lta_total->subject, lta2->subject);
  if (subject)
    strcpy(lta_total->subject, subject);
  lta_total->fscale = lta1->fscale;

  if (invertout) {
    LTA *ltatmp;
    printf("Inverting output LTA\n");
    ltatmp = LTAinvert(lta_total, nullptr);
    LTAfree(&lta_total);
    lta_total = ltatmp;
  }
...
static void usage(int exit_val) {
  ...
  fprintf(fout, "Required arguments\n\n");
  fprintf(fout, "  lta_1              maps some src1 to dst1 \n");
  fprintf(fout, "  lta_2              maps dst1(src2) to dst2 \n");
  fprintf(fout, "  lta_final          the combined LTA maps: src1 to dst2 = "
                "LTA2*LTA1\n\n");
  ...
  fprintf(
      ...
      "                        template (file2) for the talairach xfm.\n");
  fprintf(fout, "  -invert1           invert lta_1 before applying it\n");
  fprintf(fout, "  -invert2           invert lta_2 before applying it\n");
  fprintf(fout, "  -invertout         invert output lta\n");
  fprintf(fout,
          "  -out_type          set final LTA type: 0 VOX2VOX (default)\n");
  fprintf(fout, "                                         1 RAS2RAS\n\n");
  fprintf(fout, "  -subject subject : set subject in output lta\n");
  fprintf(fout, "\n");
  fprintf(fout, "-rmsdiff radius outputfile : computes RMS diff between "
                "transforms using MJ's formula\n");
  fprintf(fout, "   a radius of 70 is suggested. RMS will be saved in "
                "outputfile unless outputfile = nofile\n");
  fprintf(fout, "   Eg, mri_concatenate_lta -rmsdiff 70 rms.dat lta1 lta2, "
                "note: no output lta\n");
  fprintf(fout, "\n");
